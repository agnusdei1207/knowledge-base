...
class FileTrieNode {
  ...
  get displayName() {
    ...
    const title = this.data?.title === "index" ? undefined : this.data?.title;
    
    // Extract numbering from physical slug segment or file name
    const seg = this.slugSegment || "";
    ...
    return title || this.fileSegmentHint || this.slugSegment || "";
  }
...
const defaultFilterFn = (node) => node.slugSegment !== "tags" && node.slugSegment !== "keyword_list";
