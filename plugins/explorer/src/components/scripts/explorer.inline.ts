...
class FileTrieNode {
  ...
  get displayName() {
    ...
    const nonIndexTitle =
      this.data?.title === "index" || this.data?.title === "_index"
        ? undefined
        : this.data?.title;
    const rawName = nonIndexTitle || this.fileSegmentHint || this.slugSegment || "";
    const noEmoji = rawName.replace(/\p{Emoji_Presentation}|\p{Extended_Pictographic}|\uFE0F/gu, "").trim();
    return noEmoji.replace(/^[:\-\s\.]+\s*/, "").trim();
  }
