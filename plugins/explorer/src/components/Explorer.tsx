...
const defaultOptions: ExplorerOptions = {
  ...
  mapFn: (node: FileTrieNode) => {
    // Extract numbering from physical slug segment or file name
    const seg = node.slugSegment || "";
    const match = seg.match(/^(\d+)/);
    const num = match ? match[1] : null;
    const title = node.data?.title === "index" ? undefined : (node.data?.title as string | undefined);
  ...
  sortFn: (a: FileTrieNode, b: FileTrieNode) => {
    ...
    // Custom priority: studynote folder always goes above exam folder at the root level
  ...
  filterFn: (node: FileTrieNode) => node.slugSegment !== "tags" && node.slugSegment !== "keyword_list",
