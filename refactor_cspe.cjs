...
const directory = "C:\\workspace\\study\\src\\content\\docs\\notes\\08-latest-tech";
...
function processFile(filepath) {
    try {
        ...
        content = content.replace(/<details>\s*<summary>핵심 용어<\/summary>[\s\S]*?<\/details>/g, (block) => {
            return block.replace(/(- \*\*)(.+?)(\*\*(?:<[^>]+>)?:\s*)(.*)/g, (match, prefix, term_raw, suffix, desc) => {
                let term_clean = term_raw.split('(')[0].trim();
                let eng = "";
                let better_desc = desc;

                for (let k in terms_mapping) {
                    if (term_clean.includes(k) || k.includes(term_clean)) {
                        eng = terms_mapping[k][0];
                        better_desc = terms_mapping[k][1];
                        break;
                    }
                }

                let new_term = term_raw;
                if (eng && !/[A-Za-z]/.test(term_raw)) {
                    new_term = `${term_clean}(${eng}, 영어)`;
                }

                if (!Object.keys(terms_mapping).some(k => term_clean.includes(k))) {
                    better_desc = better_desc.replace(/(이다\.|말한다\.|의미한다\.|로 정의된다\.|을 말한다\.|임\.|함\.)$/, ' 역할을 수행하는 핵심 기술 및 개념이다.');
                }

                return `${prefix}${new_term}${suffix}${better_desc}`;
            });
        });

        // 3. Body lists ending correction
        content = content.replace(/^- .+$/gm, (line) => {
            return line.replace(/(임|함|한다|이다)\.$/, '하는 특징을 가짐.');
        });

        // 4. Conclusion (한줄 요약) must end with noun phrase
        let parts = content.split('## Ⅶ. 결론');
        if (parts.length === 2) {
            let conc = parts[1];
            conc = conc.replace(/#### 한줄 요약\n\n- (.*?)(?=\n|$)/, (match, summary) => {
                let cleanSummary = summary.replace(/(한다|이다|다|함|임|합니다|있습니다|확인한다|유지한다)\.?$/, '').trim();
                return `#### 한줄 요약\n\n- ${cleanSummary} 체계 적용 및 원칙 준수`;
            });
