...
- **문서 구조**: `2~4교시 서술형 템플릿`을 중심으로 두고, 그 안에 `1교시 단답형 앞단위`가 포함되는 구조
...
| [2~4교시 통합 템플릿](@/exam/cs/master-template.md) | 서술형 답안 안에 1교시 단답형 앞단위를 포함시키는 통합 템플릿 |
| [합격후기 기반 답안 작성법](@/exam/cs/writing-method.md) | 공개 합격후기와 답안 작성법을 반영한 시간 배분, 페이지 구성, 표현 치환 규칙 |
...
아래 링크가 `컴퓨터시스템응용기술사`의 실제 입구입니다. 각 과목으로 들어가면 **2~4교시 서술형 확장 구조 → 그 안에 포함된 1교시 앞단위 → 과목별 키워드 목록** 순서로 보입니다.
...
1. [2~4교시 통합 템플릿](@/exam/cs/master-template.md)으로 답안 골격을 먼저 고정합니다.
2. [핵심 키워드 1000](@/exam/cs/keyword_list.md)에서 과목별 범위를 잡습니다.
3. 과목 허브로 들어가 `서술형 확장 방식`과 그 안의 `1교시 앞단위`를 같이 봅니다.
4. 키워드 페이지를 검색하거나 클릭해서 실제 답안 틀로 내려갑니다.
