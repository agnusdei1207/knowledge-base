...
- **정의**: 본 문서는 16과목 9,103개 학습 노트(`studynote/`)에서 시험 답안에 직결되는 핵심만 약 **1,000개**로 압축한 컴퓨터시스템응용기술사 마스터 키워드 세트이다. 눈으로 읽는 전 범위 학습용이 아니라, 답안으로 바로 전환하는 "시험에 진짜 나오는 핵심"만 추린 압축본이다.
- **필요성**: 9천 노트는 회독 시간이 너무 길다. 빈출 3회 이상 군과 비교형·구조형 주제만 1,000개로 줄여, 시험 직전 단기 회독으로 1교시 단답과 2~4교시 서술을 모두 커버하기 위함이다.
...
- **활용 원칙**:
  ...
  4. 상세 개념 이해는 `studynote/`, 시험 답안화 압축은 본 문서로 분리 운영한다.

## Ⅰ-1. 기출·출제기준·미래 예측 반영 점검

| 점검 축 | 반영 상태 | 답안 관점 판단 |
|:---|:---|:---|
| Q-net 출제기준(2023~2026) | 하드웨어 시스템, 소프트웨어 시스템, 컴퓨터 응용 전 범위를 16과목으로 재배치 | 출제기준의 큰 축은 `01 컴퓨터구조`, `02 운영체제`, `04 SW공학`, `05 DB`, `09 보안`, `10 AI`, `13 클라우드`가 흡수 |
...
| 과목 | 목표 키워드 수 | 출제 비중 | 1교시 vs 2~4교시 성격 |
|:---|:--:|:--:|:---|
...
| 06 ICT 융합 | 35 | ★★ | 정의형(디지털트윈/5G) 1교시 + 융합 적용 서술 |
...
- **블록체인·Web3**: 블록체인(DLT), 합의 알고리즘(PoW/PoS/PBFT), 스마트 컨트랙트, 머클 트리, NFT, DID(분산신원), 블록체인 트릴레마, 레이어2/롤업, 영지식증명(ZKP), CBDC
- **IoT·통신**: IoT 3요소, 엣지 컴퓨팅, LPWAN(NB-IoT/LoRa), MQTT, 디지털 트윈, CPS(가상물리시스템), 5G(eMBB/uRLLC/mMTC), 네트워크 슬라이싱, 매터(Matter)/스레드, V2X
- **모빌리티·공간**: 자율주행(SAE 0~5), 센서 퓨전·라이다, UAM(도심항공교통), SLAM, 공간 컴퓨팅/XR, 메타버스, SDV(소프트웨어 정의 차량)
- **지능형 융합**: 스마트팩토리/OPC UA, 스마트시티, 스마트그리드·마이크로그리드, AIOps, 디지털 전환(DX), 양자컴퓨팅(중첩/얽힘)

**★ TOP 5 답안 각**
- ★ **디지털 트윈** — 물리-가상 1:1 동기화 3요소(물리/가상/실시간데이터), 시뮬레이션·예측·최적화, 스마트팩토리·도시 적용
- ★ **스마트팩토리/CPS** — 사이버(연산)가 물리 공정을 실시간 제어·피드백, OPC UA·TSN·프라이빗5G로 상호운용성 확보
- ★ **5G 3대 특성** — eMBB(초고속)/uRLLC(초저지연 1ms)/mMTC(초연결), 네트워크 슬라이싱으로 용도별 맞춤 보장
- ★ **블록체인 트릴레마** — 확장성·탈중앙화·보안성 동시충족 불가, L2/롤업·샤딩으로 확장성 해결
- ★ **자율주행** — SAE 0~5단계, 센서퓨전(카메라+라이다+레이더)+V2X(C-V2X) 인지·판단·제어
...
- 본 압축본은 9,103 학습 노트 중 **빈출·비교·구조 핵심 약 1,000개**만 추린 것으로, 시험 직전 단기 회독에서는 각 과목 **★ TOP 5 답안 각**으로 골격을 먼저 잡은 뒤 클러스터 키워드로 외연을 채우는 순서로 운영한다.
- 단순 정의 암기보다 성능·운영 복잡도·비용·보안성을 묶어 쓰고, 최신 확장군(LLM·RAG·PQC·CXL·소버린 클라우드)은 반드시 기존 기술과의 **비교 기준**과 함께 답안화한다.
- 과목별 답안 골격은 [과목별 핵심 트랙](@/exam/cs/subject-focus/_index.md), 빈도는 [기출 빈도 분석](@/exam/cs/frequency.md).
