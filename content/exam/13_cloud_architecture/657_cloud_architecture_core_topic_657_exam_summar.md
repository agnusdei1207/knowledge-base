...
| **CloudFront** | 엣지 캐싱 및 TLS 종료 | TTL 기반 Cache-Control, Origin Shield(Origin 부하 50%v), Lambda@Edge로 L7 로직, OAC/OAI로 S3 보안 |
...
| **Trade-off** | 단순/성능^, 유연성v | 복잡도^, DevOps 성숙도 필요 | Cold Startv, Vendor Lock-in^, 디버깅 어려움 |
...
- **비용**: Cost Explorer + Budgets(80% 알림) + Savings Plans(1~3년 약정, 72%v) vs Reserved Instance(특정 인스턴스) vs Spot(90%v, Interrupt 가능).
