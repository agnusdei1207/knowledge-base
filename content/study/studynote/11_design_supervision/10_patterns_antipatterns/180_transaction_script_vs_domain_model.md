...
Martin Fowler의 PoEAA (Patterns of Enterprise Application Architecture)는 엔터프라이즈 애플리케이션의 비즈니스 로직을 어떻게 조직할지에 대해 여러 패턴을 제시한다. 그중 Transaction Script와 Domain Model은 서로 대립하는 개념이라기보다, **복잡도가 다른 문제에 대응하는 두 가지 방식**으로 이해하는 것이 맞다.
...
이 두 패턴은 우열 관계보다 적용 맥락이 다르다. 단순한 업무를 Domain Model로 과하게 감싸면 설계 비용만 늘고, 복잡한 업무를 Transaction Script로 밀어붙이면 규칙이 중복되고 변경 파급이 커진다. 결국 비교의 핵심은 "처음 빠른가"가 아니라, **복잡도가 커질 때 어느 쪽이 더 단순함을 유지하는가**다.
...
또한 이 비교는 DDD (Domain-Driven Design), Repository, CQRS (Command Query Responsibility Segregation)와도 연결된다. Domain Model은 보통 쓰기 모델의 일관성 규칙이 중요할 때 빛난다. 반대로 조회 전용 화면이나 통계성 쿼리는 Transaction Script나 Query Service가 더 단순할 수 있다. 즉 한 시스템 안에서도 **쓰기 쪽은 Domain Model, 읽기 쪽은 단순 스크립트**처럼 혼합 전략이 가능하다.
...
실무 판단에서 중요한 것은 규칙의 개수보다 **규칙 간 결합도**다. 할인 정책 하나만 있어도 배송비, 적립금, 쿠폰 중복, 회원 등급, 반품 규칙과 얽히기 시작하면 Transaction Script는 빠르게 비대해진다. 반대로 게시판 관리, 코드 테이블 유지, 내부 설정 API처럼 상태 전이가 단순한 업무는 Domain Model을 도입해도 이익이 작다.
...
기술사 답안에서는 **"Transaction Script는 단순 유스케이스를 빠르게 구현하는 데 적합하고, Domain Model은 복잡한 비즈니스 규칙과 상태 전이를 객체에 응집시켜 유지보수성과 테스트성을 높이는 구조이며, 선택 기준은 규칙 결합도와 변경 빈도"**라고 정리하면 된다.
...
하지만 어떤 패턴도 만능은 아니다. Domain Model은 학습 비용과 설계 비용이 있고, Transaction Script는 규모가 커질수록 빚이 쌓인다. 그래서 중요한 것은 특정 패턴을 신앙처럼 따르는 것이 아니라, **현재 시스템의 복잡도와 앞으로의 변화 방향에 맞는 패턴을 선택하는 것**이다.
