...
| 구성 요소 | 핵심 원리 | 감리 포인트 |
| --- | --- | --- |
| 코어 커널 | 인증, 설정, 이벤트 버스, 확장 포인트 등 공통 기능만 유지한다. | 커널 비대화 여부, 공통 정책 집중도, 핵심 장애 영향도 확인 |
| 플러그인 계약 | 서비스 인터페이스, 데이터 포맷, 호출 규약을 표준화한다. | 버전 호환 정책, 하위 호환성, 문서화 수준 검토 |
| 플러그인 관리자 | 설치·활성화·비활성화·격리·업데이트를 수명주기로 관리한다. | 서명 검증, 샌드박스, 실패한 플러그인 격리 여부 확인 |
| 이벤트·서비스 연결 | 느슨한 결합으로 플러그인 간 직접 의존을 최소화한다. | 순환 의존, 성능 병목, 초기화 순서 위험 검토 |
...
| 비교 항목 | 마이크로커널 아키텍처 | 계층형 모놀리식 구조 | 마이크로서비스 아키텍처 |
| --- | --- | --- | --- |
| 확장 방식 | 플러그인 추가·교체 | 내부 코드 수정 후 재배포 | 독립 서비스 추가·배포 |
| 배포 경계 | 단일 플랫폼 내부 | 단일 애플리케이션 | 서비스별 독립 경계 |
| 강점 | 제품 변형, 옵션 기능, 실험 기능에 유리 | 단순하고 운영이 쉽다 | 조직 확장성과 독립 배포에 유리 |
| 주의점 | 인터페이스·플러그인 관리 복잡성 | 변경 영향이 전체로 전파 | 네트워크·분산 데이터 복잡성 |
