...
3. **판단 포인트**: 키워드 자체보다도 그 키워드가 어떤 위험을 통제하고 어떤 증거로 검증되는지까지 연결되어야 실제 감리형 답안이 된다.

---

## Ⅰ. 개요 및 필요성
감리·기술사 시험에서 점수를 깎는 대표 원인은 “키워드는 많이 아는데 서로 연결이 안 되는 답안”이다. 예를 들어 요구사항 추적성, 형상관리, 성능, 보안, 테스트, 운영, 품질보증을 각각 외워도 그것이 어떤 평가 축에서 만나는지 모르면 답안은 단어 나열에 머무른다.
...
## Ⅱ. 아키텍처 및 핵심 원리
연결망의 핵심 원리는 세 축이다. 첫째, 생명주기 축(계획-분석-설계-구현-운영-개선). 둘째, 평가 축(통제, 품질, 보안, 성능, 추적성). 셋째, 증적 축(문서, 로그, 테스트 결과, 변경 이력, 인터뷰). 이 세 축을 교차시키면 어떤 키워드도 답안에서 고립되지 않는다.
...
| :--- | :--- | :--- |
| 프로세스·통제 | 형상관리, 변경관리, 요구사항 추적성, 기준선 | 절차 준수 여부와 책임 경계 설명 |
| 품질·기술 | 성능, 테스트, 아키텍처, 보안, DB | 품질 속성과 설계·검증 방법 연결 |
| 감리 증적 | 산출물, 로그, 인터뷰, 조치결과, 재검증 | 객관적 근거와 개선 닫힘 구조 제시 |
...
이 연결망은 PMBOK, COBIT, EA, 품질보증, 보안통제, 운영관리, 데이터 거버넌스 같은 넓은 주제를 하나의 프레임으로 묶는 데 유용하다. 즉 “무슨 용어인가”에 멈추지 말고 “어디에 배치되는가”를 같이 말해야 고득점형 답안이 된다.
...
감리 관점에서는 특히 “증적 없는 주장”을 경계해야 한다. 아무리 좋은 키워드를 많이 써도 로그, 산출물, 회의록, 테스트 결과, 조치 이력과 연결되지 않으면 평가 언어가 아니라 슬로건이 되기 쉽다.
...
### 📌 관련 개념 맵
- **요구사항 추적성(RTM)**: 계획·분석·테스트를 연결하는 대표 키워드
- **형상관리/변경관리**: 기준선과 통제의 닫힘 구조를 설명할 때 중심이 되는 축
- **품질 속성**: 성능, 보안, 가용성, 유지보수성을 설계·감리 문장으로 이어 주는 연결점
- **감리 증적**: 로그, 산출물, 테스트 결과, 인터뷰 등 객관적 판단 근거
