...
> 1. **본질**: Double Dispatch (더블 디스패치)는 메서드 호출이 **두 객체의 런타임 타입** 모두를 기반으로 결정되는 메커니즘이며, [[275_visitor_pattern|Visitor]] ([[275_visitor_pattern|방문자]]) 패턴은 Java의 단일 디스패치(Single Dispatch) 한계를 `accept(visitor) → visitor.visit(this)` 두 번의 가상 호출(Virtual [[189_subroutine_call_return|Call]])로 더블 디스패치를 구현하는 GoF 패턴이다.
> 2. **가치**: 요소(Element) 클래스 계층을 변경하지 않고 새로운 연산([[275_visitor_pattern|Visitor]])을 추가할 수 있어, **Open/Closed Principle([[356_process|개방-폐쇄 원칙]])** 을 지키면서 타입별 동작을 확장한다.
> 3. **판단 포인트**: 요소 타입은 고정적이지만 연산이 자주 추가되는 경우 [[275_visitor_pattern|Visitor]] — 반대로 연산은 고정적이지만 타입이 자주 추가되는 경우 Visitor는 부적합 (모든 [[275_visitor_pattern|Visitor]] 클래스 수정 필요).
...
- **📢 섹션 요약 비유**: 더블 디스패치는 두 번의 악수로 신원을 [[396_validation|확인]]하는 것 — "나는 Circle이에요(1번 디스패치: accept)" → "그럼 Circle용 처리를 할게요(2번 디스패치: visit(Circle))"
...
- **📢 섹션 요약 비유**: Visitor는 세금 조사관 — 각 건물(Shape)이 "내가 어떤 건물인지(accept → this)" 을 신고하면, 조사관([[275_visitor_pattern|Visitor]])은 건물 유형에 맞는 세금 계산법(visit(Circle), visit([[341_iso_iec_25010|Square]]))을 적용한다.

---

## Ⅲ. 비교 및 연결
| 관점 | [[275_visitor_pattern|Visitor]] 적합 | [[275_visitor_pattern|Visitor]] 부적합 |
|:---|:---|:---|
| 요소 타입 변화 | 고정 (드물게 추가) | 자주 추가 |
| 연산 변화 | 자주 추가 | 고정 |
| [[746_ocp|OCP]] 측면 | 연산 추가에 닫힘 | 타입 추가에 열림 필요 |
| 적용 예시 | AST 연산 (컴파일러) | Plugin 아키텍처 |

| 패턴 | [[083_relationship_in_er_model|관계]] | 차이점 |
|:---|:---|:---|
| [[268_strategy_pattern|Strategy]] | 유사 | Strategy는 런타임 교체, Visitor는 Element 계층에 외부 연산 추가 |
| [[270_iterator_pattern|Iterator]] + [[275_visitor_pattern|Visitor]] | 조합 | Iterator로 컬렉션 순회, 각 요소에 [[275_visitor_pattern|Visitor]] 적용 |
| [[261_composite_pattern_tree_structure|Composite]] | 조합 | 트리 구조([[261_composite_pattern_tree_structure|Composite]])에 Visitor로 연산 추가 (AST 처리) |
| [[271_command_pattern|Command]] | 대비 | Command는 요청을 캡슐화, Visitor는 요소 타입별 연산 캡슐화 |

| 사례 | [[275_visitor_pattern|Visitor]] 역할 |
|:---|:---|
| 컴파일러 AST 처리 | TypeCheckVisitor, CodeGenVisitor, OptimizeVisitor |
| XML/[[343_json|JSON]] DOM 처리 | ElementVisitor (노드 타입별 처리) |
| [[501_file_definition_logical_record|파일]] 시스템 탐색 | SizeCalculatorVisitor, FileSearchVisitor |
| 세금 계산기 | TaxVisitor (상품 유형별 세율 적용) |

- **📢 섹션 요약 비유**: 컴파일러에서 AST(Abstract Syntax Tree) 노드들은 고정(IntNode, AddNode, FunctionCallNode)되어 있지만, 연산(타입 체크, 코드 [[087_process_state_transition|생성]], 최적화)은 계속 추가됨 → [[275_visitor_pattern|Visitor]] 패턴이 이상적인 이유.
...
| 방법 | 언어 지원 | [[282_performance_tactics|성능]] | 안전성 |
|:---|:---|:---|:---|
| [[275_visitor_pattern|Visitor Pattern]] | 모든 OO 언어 | 가상 호출 2번 | 컴파일 타임 검사 |
...
## Ⅴ. 기대효과 및 결론
[[275_visitor_pattern|Visitor]] 패턴과 더블 디스패치는 타입 계층이 안정적이고 연산이 다양한 시스템에서 강력한 확장성을 제공한다:

**기대효과**:
- **새 연산 추가 용이**: 새 [[275_visitor_pattern|Visitor]] 클래스만 추가, Element 클래스 불변
- **관심사 분리**: [[001_dikw_pyramid|데이터]](Element)와 연산([[275_visitor_pattern|Visitor]]) 완전 분리
- **컴파일 타임 안전성**: 모든 타입에 대한 처리 구현 여부 컴파일러 검사

**한계**:
- 새 Element 타입 추가 시 모든 [[275_visitor_pattern|Visitor]] 구현 수정 필요
- 캡슐화 약화: Element의 내부 상태를 Visitor에 노출해야 함
- 보일러플레이트 코드 증가

기술사 시험에서는 **더블 디스패치의 메커니즘(두 번의 가상 호출)**, **[[275_visitor_pattern|Visitor]] 패턴의 구조(Element + [[275_visitor_pattern|Visitor]] 인터페이스)**, **적합/부적합 시나리오(연산 추가 ↔ 타입 추가)**를 명확히 서술하는 것이 핵심이다.
...
- **📢 섹션 요약 비유**: [[275_visitor_pattern|Visitor]] 패턴은 백화점 VIP [[090_service_kubernetes_network_load_balancing|서비스]] — 각 층(Element)은 "저는 3층이에요(accept)"라고 안내하고, 전담 컨시어지([[275_visitor_pattern|Visitor]])가 층에 맞는 특화 [[090_service_kubernetes_network_load_balancing|서비스]](visit(3층))를 제공한다. 새 [[090_service_kubernetes_network_load_balancing|서비스]]를 추가할 때 각 층을 건드리지 않고 컨시어지([[275_visitor_pattern|Visitor]])만 새로 만들면 된다.
...
| 핵심 메커니즘 | Double Dispatch | [[275_visitor_pattern|Visitor]] 패턴이 구현하는 두 번의 가상 호출 |
| 연관 패턴 | [[154_composite_pattern|Composite Pattern]] | 트리 구조 + Visitor로 AST 처리 |
| 연관 패턴 | [[270_iterator_pattern|Iterator Pattern]] | 컬렉션 순회 후 각 요소에 [[275_visitor_pattern|Visitor]] 적용 |
| 언어 대안 | Java Pattern Matching (sealed class) | Java 21의 native 더블 디스패치 대안 |
| 실무 사례 | 컴파일러 AST [[275_visitor_pattern|Visitor]] | 타입 검사, 코드 [[087_process_state_transition|생성]], 최적화 각각 별도 [[275_visitor_pattern|Visitor]] |
| 대비 개념 | Single Dispatch | Java의 기본 다형성 — 호출 대상 타입만 고려 |

### 📈 관련 키워드 및 발전 흐름도
single dispatch → 더블 디스패치와 [[399_architecture|방문자 패턴]] → 다형성 연산 분리
