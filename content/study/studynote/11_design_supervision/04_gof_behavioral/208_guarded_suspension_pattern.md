...
> 1. **본질**: Guarded Suspension (가드 서스펜션) 패턴은 특정 가드 조건(Guard Condition)이 충족될 때까지 요청 [[092_thread_lwp|스레드]]를 **일시 중단(suspend)**시키고, 조건이 만족되면 재개하는 [[014_concurrency|동시성]] 제어 패턴이다.
> 2. **가치**: 생산자-소비자(Producer-Consumer) 패턴의 핵심 메커니즘으로, 빈 큐에서 소비하거나 가득 찬 큐에 넣으려는 시도를 CPU를 낭비하지 않고(busy-waiting 없이) 대기시킨다.
> 3. **판단 포인트**: 조건이 곧 만족될 것이 예상될 때 사용한다. 조건이 절대 만족되지 않을 위험이 있다면 [[573_timeout_retry_backoff_strategy|타임아웃]]([[319_timeout_prevention|Timeout]])과 함께 사용한다.
...
이는 CPU 사이클을 낭비하고 다른 [[092_thread_lwp|스레드]] 실행 기회를 빼앗는다.
...
- **📢 섹션 요약 비유**: 식당의 주문 대기표 — 테이블이 없을 때 손님(Consumer [[092_thread_lwp|스레드]])이 쇼파에 앉아 기다린다. 직원이 "6번 테이블 준비됐습니다"라고 부를 때까지(notify) 무작정 서서 기다리지 않는다.
...
| **Timed Waiting** | 0% (대기 중) | [[573_timeout_retry_backoff_strategy|타임아웃]] or notify | 중간 | 외부 이벤트 대기 |
| **[[747_io_polling_overhead|Polling]] with sleep** | 낮음 | sleep 주기 | 낮음 | 단순 [[448_polling_programmed_io|폴링]] |
...
- 실무에서는 `java.util.concurrent.BlockingQueue`로 [[198_abstraction_control_data_process|추상화]] 활용
...
| [[212_synchronization_mechanisms|동기화]] 보장 | 조건 기반 안전한 공유 자원 접근 |
| 확장성 | 생산자·소비자 수를 독립적으로 조절 가능 |
| 배압(Back-Pressure) 구현 | 큐가 가득 차면 생산자 자동 제어 |

- **[[281_deadlock_definition|Deadlock]]**: notifyAll() 누락 시 영원한 대기 위험
- **Spurious Wakeup**: `if` 대신 `while`로 조건 재확인 필수
- **[[314_starvation_prevention|Starvation]] (기아)**: 특정 [[092_thread_lwp|스레드]]가 항상 대기하는 상황 방지 → 공정성(Fairness) [[164_policy|정책]]

Guarded Suspension (가드 서스펜션)은 멀티스레드 프로그래밍의 핵심 기법이다. Java의 `wait/notify`, `Condition.await/signal`, `BlockingQueue` 모두 이 패턴을 구현한 것이다. 생산자-소비자 아키텍처의 근간으로, [[014_concurrency|동시성]]과 효율성을 동시에 달성하는 방법이다.
...
| 연관 개념 | [[228_condition_variable|Condition Variable]] | POSIX 스레딩의 동일 개념 |
| 연관 개념 | [[281_deadlock_definition|Deadlock]] | 잘못 구현 시 발생하는 위험 |
| 연관 개념 | [[210_monitor_object_pattern|Monitor Object Pattern]] | Guarded Suspension의 상위 패턴 |
