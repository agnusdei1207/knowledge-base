...
> 2. **가치**: 임시 필드를 제거하면 객체 상태가 항상 유효 (Valid [[272_state_pattern|State]]) 하게 유지되고, 숨겨진 결합도와 [[033_context|컨텍스트]] 의존성이 제거된다.
> 3. **판단 포인트**: "이 필드가 null일 수 있는 시점이 있는가?" — Yes라면 임시 필드 의심이 필요하다.

---

## Ⅰ. 개요 및 필요성
임시 필드 (Temporary Field) 는 객체의 전체 수명 동안 필요한 것이 아니라, 복잡한 [[001_algorithm_definition|알고리즘]] 수행 시 임시로 필요한 데이터를 클래스 수준 필드로 선언하는 패턴이다. 매개변수 전달 대신 필드를 통해 '전역 변수처럼' 공유하는 구현 편의에서 발생한다.
...
- **NullPointerException 위험**: 필드가 [[009_config|설정]]되지 않은 상태에서 메서드 호출 시 NPE 발생
- **테스트 어려움**: 필드 [[009_config|설정]] 순서에 따라 동작이 달라지는 순서 의존성 (Order Dependency)
- **[[147_thread_safe|스레드 안전]]성 ([[092_thread_lwp|Thread]] Safety) 파괴**: 여러 [[092_thread_lwp|스레드]]가 동시에 임시 필드를 덮어씀
...
| 임시 필드 유형 | 권장 처방 | 이유 |
|:---|:---|:---|
| [[001_algorithm_definition|알고리즘]] 중간 상태 | 메서드 객체화 (Replace Method with Method Object) | 상태를 전용 객체로 격리 |
| 특수 케이스에서만 의미 있는 필드 | 특수 케이스 객체 (Introduce Special Case) | null 제거 |
| 캐시용 임시 필드 | 명시적 캐시 클래스 도입 | 무효화 로직 명확화 |
| 공통 [[033_context|컨텍스트]] 전달 | [[242_introduce_parameter_object|파라미터 객체화]] 또는 ThreadLocal | [[147_thread_safe|스레드 안전]] 보장 |
...
| 주요 증상 | NPE, 순서 의존 | 테스트 격리 불가 | [[213_race_condition|경쟁 조건]] ([[213_race_condition|Race Condition]]) |
| 처방 방향 | 지역화 (Localize) | [[337_dependency_injection|의존성 주입]] ([[190_enterprise_di_framework_lifecycle|DI]]) | 불변성 (Immutability) |
| 위험도 | 중 | 높음 | 매우 높음 |

임시 필드가 "이 객체가 유효하지 않은 상태"를 표현하는 경우, [[207_null_object_pattern|널 객체 패턴]] ([[207_null_object_pattern|Null Object Pattern]]) 또는 옵셔널 (Optional) 패턴으로 해결할 수 있다.
...
## Ⅳ. 실무 적용 및 기술사 판단
웹 서버에서 요청 [[033_context|컨텍스트]] (Request [[033_context|Context]]) 를 임시 필드 대신 `ThreadLocal`로 관리하는 패턴이 표준이다.
...
[[228_batch_processing_hadoop_spark|배치 처리]] 시 [[216_progress_in_synchronization|진행]] 상태를 임시 필드로 관리하면 재시작 (Restart) 시 상태가 초기화되지 않아 오류가 발생한다. `JobContext` 같은 전용 객체나 영속 스토어 (Persistent Store) 에 상태를 저장해야 한다.

- **상태 불변성 ([[272_state_pattern|State]] Invariant)**: 클래스의 모든 메서드 호출 전후로 객체 상태가 유효해야 한다는 설계 원칙 강조
...
| [[397_unit_test|단위 테스트]] 독립성 | 실패 (순서 의존) | 보장 |
| 클래스 상태 불변식 | 빈번 위반 | 항상 유효 |

임시 필드 (Temporary Field) 는 "지금 당장 편하려고 만든 지름길"이 장기적으로 어떤 위험을 초래하는지 보여주는 전형적인 [[128_water_scrum_fall_anti_pattern|안티패턴]]이다. [[001_algorithm_definition|알고리즘]] 중간 상태는 전용 객체로 캡슐화하고, [[033_context|컨텍스트]] 데이터는 파라미터로 명시적으로 전달하며, null 가능 상태는 `Optional`이나 특수 케이스 객체로 처리하는 것이 올바른 방향이다.
...
|:---|:---|:---|
| 상위 개념 | [[370_code_smell|코드 스멜]] ([[365_5_solid_code_smell|Code Smell]]) | 임시 필드는 주요 스멜 중 하나 |
| 처방 | 메서드 객체화 (Replace Method with Method Object) | 임시 필드를 전용 클래스로 격리 |
| 처방 | 특수 케이스 객체 (Special Case Pattern) | null 필드 대체 |
| 연관 개념 | 상태 불변식 ([[272_state_pattern|State]] Invariant) | 항상 유효한 객체 상태 |
| 연관 개념 | ThreadLocal | 요청 [[033_context|컨텍스트]] 임시 상태 관리 |
| 연관 개념 | 불변 객체 ([[172_builder_immutable_object|Immutable Object]]) | 임시 필드 문제의 근본 해결 |
| 연관 개념 | Optional | null 필드의 명시적 표현 |

### 📈 관련 키워드 및 발전 흐름도
희소 상태 → 임시 필드 [[128_water_scrum_fall_anti_pattern|안티패턴]] → [[033_context|context]]/[[148_requirements_specification_formal_informal|specification]] 분리
