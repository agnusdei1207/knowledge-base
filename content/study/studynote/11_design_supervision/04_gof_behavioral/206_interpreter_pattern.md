...
> 1. **본질**: [[277_interpreter_pattern|Interpreter]] ([[277_interpreter_pattern|해석자]]) 패턴은 언어나 문법의 각 규칙을 클래스로 표현하고, 해당 언어로 작성된 문장을 AST (Abstract Syntax Tree, 추상 구문 트리)로 파싱한 뒤 평가(Evaluate)하는 구조를 제공한다.
> 2. **가치**: [[064_relation_domain|도메인]] 특화 언어(DSL, Domain-Specific Language)나 규칙 엔진을 구현할 때, 문법 규칙을 코드와 1:1 매핑하여 이해·확장이 쉬운 인터프리터를 만든다.
> 3. **판단 포인트**: [[104_regex|정규 표현식]], SQL WHERE절, 수식 파서처럼 반복적으로 사용되는 간단한 문법이 있을 때 적용하되, 복잡한 문법에는 전용 파서 생성기(ANTLR 등)를 사용한다.

---

## Ⅰ. 개요 및 필요성
GoF는 다음 조건에서 [[277_interpreter_pattern|Interpreter]] 패턴을 권장한다:

1. 해석할 언어의 **문법이 단순**하고 규칙의 수가 적다
2. 동일한 문장을 **반복적으로 해석**해야 한다
3. 효율보다 **이해·확장 용이성**이 중요한 경우

BNF (Backus-Naur Form, 배커스-나우르 표기법)는 문법을 정의하는 형식 언어다. [[277_interpreter_pattern|Interpreter]] 패턴은 BNF의 각 규칙을 클래스로 구현한다:
...
- **📢 섹션 요약 비유**: 영어 문법책의 "주어 + 동사 + 목적어" 규칙 하나하나를 클래스로 만들고, 그 규칙 클래스들로 문장을 파싱하고 이해하는 것이 [[277_interpreter_pattern|Interpreter]] 패턴이다.
...
- **📢 섹션 요약 비유**: 악보(언어)를 보고 연주하는 것 — 음표(TerminalExpression), 화음 기호(NonTerminalExpression), 지휘자 지시([[033_context|Context]])가 합쳐져서 음악(실행 결과)이 탄생한다.
...
| 적용 대상 | 적합성 | 이유 |
|:---|:---|:---|
| [[104_regex|정규 표현식]] ([[104_regex|Regex]]) | ✅ 적합 | 규칙 수 제한, 반복 사용 |
| SQL 파서 | ✅ 적합 | 명확한 BNF 문법 |
| 간단한 DSL | ✅ 적합 | [[064_relation_domain|도메인]] 규칙을 코드로 표현 |
| 프로그래밍 언어 파서 | ❌ 부적합 | 문법 복잡 → ANTLR 사용 |
| 자연어 처리 | ❌ 부적합 | 불규칙성 너무 높음 |

| 패턴 | Interpreter와의 [[083_relationship_in_er_model|관계]] |
|:---|:---|
| [[261_composite_pattern_tree_structure|Composite]] ([[261_composite_pattern_tree_structure|컴포지트]]) | AST 자체가 [[261_composite_pattern_tree_structure|Composite]] 구조 |
| [[275_visitor_pattern|Visitor]] ([[275_visitor_pattern|방문자]]) | AST 노드를 순회하며 여러 연산 적용 |
| [[265_flyweight_pattern_instance_sharing|Flyweight]] ([[265_flyweight_pattern_instance_sharing|플라이웨이트]]) | TerminalExpression 인스턴스 공유 |
| [[270_iterator_pattern|Iterator]] ([[270_iterator_pattern|이터레이터]]) | AST 순회에 사용 |

- **📢 섹션 요약 비유**: 계산기 앱 — 버튼(TerminalExpression: 숫자), 연산자(NonTerminalExpression: +, ×), 메모리([[033_context|Context]]: 이전 결과). 이 세 가지가 [[277_interpreter_pattern|Interpreter]] 패턴의 3요소이다.
...
Spring의 SpEL (Spring Expression Language)은 [[277_interpreter_pattern|Interpreter]] 패턴의 실제 구현:
...
- BNF (Backus-Naur Form)와의 [[083_relationship_in_er_model|관계]]: 각 BNF 규칙 = 하나의 클래스
- AST (Abstract Syntax Tree) 구조가 **[[261_composite_pattern_tree_structure|Composite]] 패턴**임을 명시
...
- **📢 섹션 요약 비유**: 요리 레시피(언어) 해석 — "재료를 볶다"(Terminal), "볶은 것에 소스를 더하다"(NonTerminal), 요리사의 경험치([[033_context|Context]])가 합쳐져서 최종 요리(실행 결과)가 완성된다.
...
| 효과 | 설명 |
|:---|:---|
| 문법-코드 1:1 매핑 | 문법 규칙을 클래스로 직접 표현 → [[333_readability_vs_efficiency|가독성]] |
...
- **[[282_performance_tactics|성능]]**: 재귀적 AST 평가 오버헤드 → [[456_caching|캐싱]]([[456_caching|Caching]])으로 보완
- **유지보수**: 문법 변경 시 여러 클래스 수정 필요

[[277_interpreter_pattern|Interpreter]] ([[277_interpreter_pattern|해석자]]) 패턴은 **[[064_relation_domain|도메인]] 규칙을 언어로 표현**해야 하는 상황에서 빛을 발한다. [[104_regex|정규 표현식]], SQL, SpEL 등 현실 세계의 많은 인터프리터가 이 패턴을 기반으로 한다. 단순한 문법에는 강력하지만, 복잡한 문법은 전문 파서 도구로 처리하는 것이 현명하다.
...
### 📈 관련 키워드 및 발전 흐름도
문법 정의 → [[400_process|해석자 패턴]] → DSL 엔진·규칙 처리
