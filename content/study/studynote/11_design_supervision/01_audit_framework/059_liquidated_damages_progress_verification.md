...
> 2. **가치**: 감리인은 실제 진척도와 지연 사유를 객관적 증빙으로 구분해 분쟁을 줄여야 한다.
...
공공 정보화 사업에서는 종료일 지연이 곧 비용과 책임 문제로 이어진다. 지체 상금은 이런 지연에 대해 미리 합의한 배상 규칙이다.

문제는 지연이 항상 사업자 탓만은 아니라는 점이다. 요구사항 변경, 외부 시스템 연동 지연, 발주자 검토 지연이 섞이면 책임 판단이 복잡해진다.
...
감리의 핵심은 계획 대비 실제를 비교할 수 있는 기준과 기록을 갖추는 것이다. WBS(Work Breakdown Structure), PMIS(Project Management Information System), 변경 요청서가 이 기준을 만든다.
...
| 항목 | 확인 포인트 |
| :-- | :-- |
| 진척도 | WBS 가중치, 마일스톤, 산출물 완료 여부 |
| 변경 기록 | 과업변경 요청서, 회의록, 승인 시점 |
| 지연 사유 | 사업자 귀책 / 발주자 귀책 / 외부 요인 |
| 증빙 | 로그, 이메일, 검수 문서, 시스템 기록 |

지체 상금 판단은 "얼마나 늦었는가"만이 아니라 "왜 늦었는가"를 함께 봐야 한다. 일정 변화가 공식 승인되었다면 책임 비율도 달라질 수 있다.

- **📢 섹션 요약 비유**: 숙제 제출이 늦었는지, 선생님이 제출일을 바꿨는지를 함께 확인하는 것과 같다.
...
| 범주 | 예시 | 판단 |
| :-- | :-- | :-- |
| 사업자 귀책 | 설계 오류, 인력 부족, 내부 지연 | 지체 상금 부과 대상 |
| 발주자 귀책 | 요구사항 변경, 검토 지연 | 일정 조정 가능 |
| 외부 요인 | 법령 변경, 천재지변, 연동 대상 지연 | 별도 인정 검토 |
| 혼합 책임 | 변경 이후 대응 미흡 | 비율 분리 필요 |

진척도 측정도 하나만 쓰면 위험하다. 기간 기반, 금액 기반보다 산출물 기반과 가중치 기반이 더 객관적이다. 그래서 감리인은 문서와 시스템 로그를 함께 봐야 한다.
...
- 과업변경으로 일정이 늘어난 경우
- 외부 시스템 지연으로 통합이 밀린 경우
- 산출물은 완성됐지만 검수가 지연된 경우

- **📢 섹션 요약 비유**: 누가 먼저 밀었는지, 넘어지기 전 합의가 있었는지를 꼭 확인해야 하는 줄다리기다.
