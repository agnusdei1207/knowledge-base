...
## 핵심 인사이트 (3줄 요약)
> 1. **본질**: CSAT ([[026_three_c_analysis|Customer]] Satisfaction Score)은 사용자가 실제로 만족했는지를 수치로 확인하는 운영 품질 지표다.
> 2. **가치**: 오픈 후 만족도 조사는 기능 [[395_verification_process_review|검증]]이 아니라, 현업 불편을 remediation ([[380_maintenance_types|Corrective]] Action)으로 연결하는 감리 도구다.
...
| 수집 | 현업 응답 | 점수, 서술형 코멘트 |
| 분석 | 응답 [[001_dikw_pyramid|데이터]] | RCA (Root Cause Analysis) |
| 개선 | 문제 목록 | remediation plan |
| [[395_verification_process_review|검증]] | 재조사 결과 | 개선 완료 여부 |
...
VOC (Voice of [[026_three_c_analysis|Customer]])는 현장의 불만과 요구를 모으는 입력이고, CSAT는 그 만족도를 점수화한 출력이다. 두 지표를 함께 봐야 문제의 양과 질을 동시에 파악할 수 있다.
...
## Ⅲ. 비교 및 연결
CSAT는 "얼마나 만족했는가"를 본다. 반면 [[352_defect_definition|defect]] audit는 시스템 [[352_defect_definition|결함]]의 존재를 보고, NPS (Net Promoter Score)는 추천 의향을 본다. 즉 만족도는 운영 체감, [[352_defect_definition|결함]] 수는 기술 정확도, NPS는 [[083_relationship_in_er_model|관계]] 유지 성향을 보여준다.
...
## Ⅴ. 기대효과 및 결론
CSAT remediation audit는 현업 불만을 구조화하고, 개선의 우선순위를 정하며, 오픈 이후 품질을 실제 [[286_usability_tactics|사용성]] 중심으로 끌어올린다. 결국 이 개념은 "작동하는 시스템"이 아니라 "쓰이는 시스템"을 만들기 위한 장치다.
...
| 개념 | 연결 포인트 |
| :--- | :--- |
| CSAT ([[026_three_c_analysis|Customer]] Satisfaction Score) | 만족도 수치화 |
| VOC (Voice of [[026_three_c_analysis|Customer]]) | 현장 불만/요구 수집 |
| RCA (Root Cause Analysis) | 원인 분석 |
| remediation | 개선 조치와 후속 추적 |
| [[363_audit|audit]] | 증빙 기반 점검 |
