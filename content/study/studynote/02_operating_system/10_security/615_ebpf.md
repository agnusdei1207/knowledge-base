...
> 1. **본질**: eBPF 네트워크/보안/모니터링 이벤트 [[022_kernel_role|커널]] 안전 훅 매커니즘은 [[001_operating_system_purpose|운영체제]] [[043_protection_security|보호와 보안]] 메커니즘에서 핵심 흐름을 결정하는 개념으로, 시스템이 무엇을 먼저 관리하고 어떤 순서로 제어할지를 분명하게 만든다.
> 2. **가치**: 이 개념을 이해하면 자원 효율, [[138_response_time|응답 시간]], 안정성 사이의 균형을 더 정확하게 설명할 수 있고, 멀티코어 확장성 병목 (Amdahl's Law) 및 [[022_kernel_role|커널]] [[275_lock_contention_monitoring|락 경합]] 진단로 이어지는 이유도 자연스럽게 파악된다.
> 3. **판단 포인트**: 시스템 [[614_dtrace|DTrace]] 선언적 동적 트레이싱 엔진 메커니즘과의 관계를 함께 봐야 eBPF 네트워크/보안/모니터링 이벤트 [[022_kernel_role|커널]] 안전 훅 매커니즘을 단순 정의가 아니라 실제 설계·운영 판단 기준으로 사용할 수 있다.
...
### êë ë ìì
eBPFë ìë 1992ë BSDìì ëíìí íí ííëì ìí íìí BPFë êëìë, 2014ë ëëì ìë 3.18ìì "Extended" êëì ìêëì ìë íëêëì ìì ë ëì ììì êëíì êììë. eBPF íëêëì ìëì ììëê ìì "êìê(Verifier)"ë íí ìììì êìëê, ìë(native [[082_process_memory_structure|code]])ë ìíìëì ìë ëìì [[568_jit_access|JIT]]([[568_jit_access|Just-In-Time]]) ìíìëì ìí ìíëë.
...
| **[[568_jit_access|JIT]] ìíìë** | ëìíìëë ëìíë ìëë ëí | x86, ARM ë ê ìííìë ììí ìë ìì |
| **ë(Maps)** | ìëê ììì êêì êìíë ëìí ììì | íì ë, ëì, ìí, í ë ëìí ìëêì |
| **íí íìí(Hook Points)** | íëêëì ììëë ìë ë ìì | ììí ì, ëíìí ìì, íëë ë |
| ** attach/maps [[014_api_posix|API]]** | ììì êêìì eBPF íëêëì ììíë ìííìì | [[069_ebpf|bpf]]() ììí ì, BCC, libbpf ë |
...
**[ëììêë íì]** eBPF íëêëì ìëìêë "ìí -> ììêì -> ìì -> ëì -> íë"ìë ëìí ì ìë. Cììë ììë íëêëì êìêëë ìí ìë ìììì êìëê, [[568_jit_access|JIT]] ìíìëëë êìêìì êì ëìíë ìëë ìííê ë ë, ììí ììì ëìëìíë. ì êììì êìêê íëêëì êìíë "ìí êì"ì ëê ëìêì íë.
...
### ëíìí [[670_xdp|XDP]]([[661_ebpf_xdp_express_data_path|eXpress Data Path]]) ëì êì
...
**[ëììêë íì]** XDPë "êìë íê ìì êì íìêë êìë ê"ê êë. ëìê ìëìì ëí(ìë ëíìí ìí)ì ëëíê ìì, êì íìê([[670_xdp|XDP]] íëêë)ë íêìì ìíí ëê(XDP_DROP)ì ëë êëëë. ìëê íë ëêì ìëìì ëí ììë íêí íë ëêíë êëë íì íìììë.
...
| ëê | ëì | ìì |
|---|---|---|
| **BCC ([[069_ebpf|BPF]] Compiler Collection)** | C + Python/Luaë eBPF íëêë ìì | execsnoop, opensnoop, network monitoring |
...
| **[[825_cilium_ebpf_kubernetes_networking_security|Cilium]]** | ìëëíì ëíìí ë ëì | L7 ìì ìì, ëíìí ëëíë |
...
### ìë ìëëì: DDoS ëìë ìí [[670_xdp|XDP]] êë íí ííë

**ìí**:ë ëêë ì ìëìê DDoS êêì ëì ìëìêëìë. êì ëíëìë ìëíêì êê íëíì ëë ìë.

**eBPF/[[670_xdp|XDP]] êë ëì**:
1. [[670_xdp|XDP]] íëêë ìì: ìëì IP ììë êëìë ííì ëë
...
| êë | ìíì iptables ëíë | eBPF/[[670_xdp|XDP]] êë ëíë |
|---|---|---|
...
### ëë ìë
eBPFë ìëëíì ëíìí([[825_cilium_ebpf_kubernetes_networking_security|Cilium]]), ìíìë ëì(Falco), ìë ëëíë(bpftrace) ë íë ìíëìì íì êìë ìëìê ìë. ëí íëìì êì(TPASS)ê êííì ëì ëë íí ìëê êëíìê, ëìëëì íìí ìëí ëíìí ììíìë ììì ììë.

### ìê íì
- **ëëì ìë ëì**: [[471_https_http_over_tls|https]]://www.[[022_kernel_role|kernel]].org/doc/html/latest/[[069_ebpf|bpf]]/
- **BCC ([[069_ebpf|BPF]] Compiler Collection)**: [[471_https_http_over_tls|https]]://github.com/iovisor/bcc
- **bpftrace**: [[471_https_http_over_tls|https]]://bpftrace.org/

- **ìì ëì**: eBPFë "ìëí ìíì íí êí ììí"ê êë. êí ìí, [[933_cctv|CCTV]], êê ìë ìììë ëì ëë íëì ìì ììíìì êëëëì, eBPFë ëíìí, ëì, ëëíë ëì íëì íí íëììíìì ìëíë.

---

> 1. **ëì**: eBPF(Extended [[069_ebpf|Berkeley Packet Filter]])ë ëëì ìë ëìì "ììíê ìíëë" íëêëì ììíê ììí ì ìê íë êìë, ëíìí íí ìë, ëì ëëíë, ìë ìì ëì ìë ìììì ììí ììì ìì ëììë ìíí ì ìê íë.
...
| êë ëì | êê ë ìëì ìë |
|---|---|
| **[[670_xdp|XDP]] ([[661_ebpf_xdp_express_data_path|eXpress Data Path]])** | eBPFë íìí ìêì ëíìí íí ìë íëììíë, ìë ëíìí ìí ììì ííì ìëíë. |
...
| **BCC ([[069_ebpf|BPF]] Compiler Collection)** | Cììë eBPF íëêëì ìê ììí ì ìëë íë íëììíìë. |
...
| 개념 | 연결 포인트 |
|:---|:---|
| [[613_profiling_gprof|프로파일링]] ([[613_profiling_gprof|Profiling]]) 도구 Gprof [[022_kernel_role|커널]] 후킹 작동 원리 | 현재 개념으로 들어오기 전에 함께 이해하면 경계가 선명해지는 기반 개념이다. |
| 시스템 [[614_dtrace|DTrace]] 선언적 동적 트레이싱 엔진 메커니즘 | 현재 개념이 등장하게 만든 직접적인 선행 흐름이다. |
| 멀티코어 확장성 병목 (Amdahl's Law) 및 [[022_kernel_role|커널]] [[275_lock_contention_monitoring|락 경합]] 진단 | 현재 개념이 구현·세분화될 때 바로 연결되는 후속 개념이다. |
| I/O [[282_performance_tactics|성능]] 병목 ([[617_io_bottleneck|Bottleneck]]) 탐색법 (iostat, vmstat) | 확장 학습이나 심화 비교로 이어지는 다음 단계의 키워드다. |
