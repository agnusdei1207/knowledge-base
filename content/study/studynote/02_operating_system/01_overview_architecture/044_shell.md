...
> 2. 셸 스크립트는 변수·조건·반복·함수·파이프를 지원하는 완전한 프로그래밍 언어이며 — 프로세스 포크(fork)/익스큐트(exec) 시스템 콜의 흐름을 직접 제어하므로, OS 자동화·[[652_devops_calms_culture|DevOps]] [[090_configuration_item|CI]]/CD 파이프라인의 핵심 도구이다.
...
> 📢 **섹션 요약 비유**: 셸은 번역관 — 사람의 한국어(명령)를 [[022_kernel_role|커널]]의 언어(시스템 콜)로 번역해주는 통역사. [[022_kernel_role|커널]]은 셸이 없으면 사람과 직접 대화하기 어려워요.
...
> 📢 **섹션 요약 비유**: 고급 셸 기능은 개인 비서 커스터마이징 — alias는 단축 [[158_instruction|명령어]](습관 단어), .bashrc는 출근 시 비서에게 전달하는 [[009_config|설정]] 메모. 셸을 내 입맛에 맞게 꾸밀 수 있어요.

---

## Ⅴ. 실무 시나리오 — [[090_configuration_item|CI]]/CD 배포 자동화
...
> 📢 **섹션 요약 비유**: [[090_configuration_item|CI]]/CD 셸 스크립트는 자동 이사 — 기존 짐 [[555_backup_and_restore_strategy|백업]] → 새 짐 배치 → 입주 [[396_validation|확인]](헬스체크) → 문제면 자동 되돌리기. 사람 손 없이 자동으로 안전하게 배포.
