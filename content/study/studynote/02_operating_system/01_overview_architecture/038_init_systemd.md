...
> 2. systemd는 의존성 기반 [[430_index_fast_full_scan|병렬]] 부팅, [[125_socket|소켓]]/[[134_dbus|D-Bus]] 활성화, cgroup 자원 제어, 저널 로깅을 통합 제공하는 현대적 초기화 시스템으로, 부팅 시간을 수 분에서 수 초로 단축했다.
> 3. systemd vs sysvinit의 핵심 차이: SysV는 "스크립트 실행 순서", systemd는 "의존성 [[070_graph_datastructure|그래프]] 기반 [[430_index_fast_full_scan|병렬]] 시작" — 현재 RHEL/Ubuntu/Debian 등 거의 모든 주요 Linux 배포판이 systemd를 채택했다.
...
> 📢 **섹션 요약 비유**: systemd는 프로젝트 매니저 — 의존 [[083_relationship_in_er_model|관계]] 있는 것만 순서 지키고, 독립적인 것은 동시에 시작.

---

## [[288_version_ihl_tos_total_length|IV]]. systemd 유닛 종류
...
> 📢 **섹션 요약 비유**: .timer는 cron의 systemd [[288_version_ihl_tos_total_length|버전]] — crontab 대신 유닛 파일로 [[208_schedule_history_transaction_execution_order|스케줄]] 관리, journald로 [[568_logs_distributed_logging_elk_fluentd|로그]]도 통합.

---

## V. 실무 시나리오 — 부팅 [[282_performance_tactics|성능]] 분석
...
> 📢 **섹션 요약 비유**: systemd-analyze blame은 부팅 시간 낭비 범인 찾기 — 가장 오래 걸린 [[090_service_kubernetes_network_load_balancing|서비스]]를 찾아 비활성화.
