...
> 2. [[572_protection_domain|보호 도메인]]([[572_protection_domain|Protection Domain]])과 접근 행렬([[573_access_matrix|Access Matrix]])은 OS [[571_protection_vs_security|보호]]의 이론적 기반으로, 주체(Subject)-객체(Object)-권한(Right)의 삼각 관계를 체계적으로 모델링하며 [[549_acl_access_control_list|ACL]]([[549_acl_access_control_list|Access Control List]])과 Capability List로 구현된다.
> 3. 링 구조(Ring [[319_architecture|Architecture]])는 x86 CPU의 권한 레벨(Ring 0~3)을 통해 OS [[022_kernel_role|커널]](Ring 0)과 사용자 프로그램(Ring 3)을 분리하는 하드웨어 [[571_protection_vs_security|보호]] 메커니즘으로, 특권 [[158_instruction|명령어]] 실행, 메모리 접근, I/O 제어를 계층적으로 제어한다.

---

## Ⅰ. [[571_protection_vs_security|보호]] vs 보안 개념 구분
...
> 📢 **섹션 요약 비유**: [[571_protection_vs_security|보호]] vs 보안은 건물 내부 잠금 vs 외벽 보안 — [[571_protection_vs_security|보호]]는 내부 회의실 문 잠금(내부 격리), 보안은 외부 침입자 방어(외벽, [[933_cctv|CCTV]]).

---

## Ⅱ. [[572_protection_domain|보호 도메인]]과 접근 행렬
...
## Ⅲ. 링 구조 (Ring [[319_architecture|Architecture]])
...
> 📢 **섹션 요약 비유**: OS 보안 위협은 건물 침입 방법 — 정문 뚫기([[303_authentication_authorization_patterns|인증]] 우회), 창문 깨기([[591_buffer_overflow|버퍼 오버플로우]]), 청소부 위장([[586_trojan_horse_wrapper|트로이 목마]]), 열쇠 복사(자격증명 도용).

---

## Ⅴ. 실무 시나리오 — Linux [[583_selinux|SELinux]]/[[584_apparmor|AppArmor]]
