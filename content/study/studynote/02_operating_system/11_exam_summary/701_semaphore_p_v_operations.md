...
| 개념 | 연결 포인트 |
|:---|:---|
| 뮤텍스 락 (Mutex Lock) | 현재 개념으로 들어오기 전에 함께 이해하면 경계가 선명해지는 기반 개념이다. |
| 스핀락 바쁜 대기 (Busy Wait) | 현재 개념이 등장하게 만든 직접적인 선행 흐름이다. |
| 모니터 (Monitor) 동기화 추상화 | 현재 개념이 구현·세분화될 때 바로 연결되는 후속 개념이다. |
