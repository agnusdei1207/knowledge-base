...
| 개념 | 연결 포인트 |
|:---|:---|
| 파일 지연 쓰기 (Delayed Write) | 현재 개념으로 들어오기 전에 함께 이해하면 경계가 선명해지는 기반 개념이다. |
| 저널링 파일 시스템 트랜잭션 로그 | 현재 개념이 등장하게 만든 직접적인 선행 흐름이다. |
| 슬랩 (Slab) 할당기 객체 캐싱 | 현재 개념이 구현·세분화될 때 바로 연결되는 후속 개념이다. |
| 디바이스 드라이버 모듈 인터페이스 | 확장 학습이나 심화 비교로 이어지는 다음 단계의 키워드다. |
