...
| 개념 | 연결 포인트 |
|:---|:---|
| 모니터 (Monitor) 동기화 추상화 | 현재 개념으로 들어오기 전에 함께 이해하면 경계가 선명해지는 기반 개념이다. |
| 생산자 소비자 유한 버퍼 | 현재 개념이 등장하게 만든 직접적인 선행 흐름이다. |
| 교착 상태 4가지 조건 | 현재 개념이 구현·세분화될 때 바로 연결되는 후속 개념이다. |
| 자원 할당 그래프 사이클 | 확장 학습이나 심화 비교로 이어지는 다음 단계의 키워드다. |
