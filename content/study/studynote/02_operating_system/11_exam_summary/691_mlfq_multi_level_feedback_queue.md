...
| 개념 | 연결 포인트 |
|:---|:---|
| SJF 기아 (Starvation) 발생 | 현재 개념으로 들어오기 전에 함께 이해하면 경계가 선명해지는 기반 개념이다. |
| 라운드 로빈 시간 할당량 (Quantum) | 현재 개념이 등장하게 만든 직접적인 선행 흐름이다. |
| HRN 대기 시간 공식 | 현재 개념이 구현·세분화될 때 바로 연결되는 후속 개념이다. |
| 멀티스레드 유저모드 커널모드 | 확장 학습이나 심화 비교로 이어지는 다음 단계의 키워드다. |
