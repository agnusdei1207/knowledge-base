...
| 개념 | 연결 포인트 |
|:---|:---|
| 다단계 피드백 큐 (MLFQ) 천이 | 현재 개념으로 들어오기 전에 함께 이해하면 경계가 선명해지는 기반 개념이다. |
| HRN 대기 시간 공식 | 현재 개념이 등장하게 만든 직접적인 선행 흐름이다. |
| 스레드 로컬 스토리지 (TLS) | 현재 개념이 구현·세분화될 때 바로 연결되는 후속 개념이다. |
| 스레드 동기화 상호 배제 | 확장 학습이나 심화 비교로 이어지는 다음 단계의 키워드다. |
