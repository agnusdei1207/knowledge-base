...
| 개념 | 연결 포인트 |
|:---|:---|
| 쓰기 시 복사 (COW) | 현재 개념으로 들어오기 전에 함께 이해하면 경계가 선명해지는 기반 개념이다. |
| SMP 캐시 일관성 폴스 셰어링 | 현재 개념이 등장하게 만든 직접적인 선행 흐름이다. |
| 우선순위 역전 (Priority Inversion) 방지 | 현재 개념이 구현·세분화될 때 바로 연결되는 후속 개념이다. |
| 문맥 교환 비용 (레지스터 저장 복원) | 확장 학습이나 심화 비교로 이어지는 다음 단계의 키워드다. |
