...
| 개념 | 연결 포인트 |
|:---|:---|
| iOS 앱 샌드박싱 구조 | 현재 개념으로 들어오기 전에 함께 이해하면 경계가 선명해지는 기반 개념이다. |
| 라이브 패칭 (Kpatch) 커널 정지 없는 보안 | 현재 개념이 등장하게 만든 직접적인 선행 흐름이다. |
| 락 엘리전 하드웨어 트랜잭션 메모리 활용 | 현재 개념이 구현·세분화될 때 바로 연결되는 후속 개념이다. |
| RCU 다중 독자 락 프리 고성능 기법 | 확장 학습이나 심화 비교로 이어지는 다음 단계의 키워드다. |
