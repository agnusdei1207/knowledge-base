...
| 개념 | 연결 포인트 |
|:---|:---|
| POSIX 스레드 (pthreads) 표준 API | 현재 개념으로 들어오기 전에 함께 이해하면 경계가 선명해지는 기반 개념이다. |
| 락 엘리전 하드웨어 트랜잭션 메모리 활용 | 현재 개념이 등장하게 만든 직접적인 선행 흐름이다. |
| 워킹 셋 윈도우 사이즈 동적 조절 | 현재 개념이 구현·세분화될 때 바로 연결되는 후속 개념이다. |
| 페이지 컬러링 캐시 경합 회피 물리 할당 | 확장 학습이나 심화 비교로 이어지는 다음 단계의 키워드다. |
