...
| 개념 | 연결 포인트 |
|:---|:---|
| cgroups 메모리, CPU 자원 제한 격리 컨테이너 | 현재 개념으로 들어오기 전에 함께 이해하면 경계가 선명해지는 기반 개념이다. |
| 안드로이드 LMK (Low Memory Killer) 작동 | 현재 개념이 등장하게 만든 직접적인 선행 흐름이다. |
| 라이브 패칭 (Kpatch) 커널 정지 없는 보안 | 현재 개념이 구현·세분화될 때 바로 연결되는 후속 개념이다. |
| POSIX 스레드 (pthreads) 표준 API | 확장 학습이나 심화 비교로 이어지는 다음 단계의 키워드다. |
