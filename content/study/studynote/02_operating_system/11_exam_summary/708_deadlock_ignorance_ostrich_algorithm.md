...
| 개념 | 연결 포인트 |
|:---|:---|
| 자원 할당 그래프 사이클 | 현재 개념으로 들어오기 전에 함께 이해하면 경계가 선명해지는 기반 개념이다. |
| 은행원 알고리즘 안전 상태 | 현재 개념이 등장하게 만든 직접적인 선행 흐름이다. |
| 교착 상태 복구 (프로세스 킬) | 현재 개념이 구현·세분화될 때 바로 연결되는 후속 개념이다. |
| 주소 바인딩 컴파일/로드/실행 | 확장 학습이나 심화 비교로 이어지는 다음 단계의 키워드다. |
