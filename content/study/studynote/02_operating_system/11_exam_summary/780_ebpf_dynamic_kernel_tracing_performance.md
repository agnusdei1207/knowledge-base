...
| 개념 | 연결 포인트 |
|:---|:---|
| 프로세스 친화성 (Affinity) 스케줄링 | 현재 개념으로 들어오기 전에 함께 이해하면 경계가 선명해지는 기반 개념이다. |
| 부하 균등화 (Load Balancing) 큐 이주 | 현재 개념이 등장하게 만든 직접적인 선행 흐름이다. |
| ZFS Copy-on-Write 볼륨 관리 통합 | 현재 개념이 구현·세분화될 때 바로 연결되는 후속 개념이다. |
| LFS (Log-structured File System) 랜덤 쓰기 순차화 | 확장 학습이나 심화 비교로 이어지는 다음 단계의 키워드다. |
