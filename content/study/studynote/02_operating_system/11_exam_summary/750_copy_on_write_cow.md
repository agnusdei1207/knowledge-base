...
| 개념 | 연결 포인트 |
|:---|:---|
| 스풀링 (Spooling) 버퍼 | 현재 개념으로 들어오기 전에 함께 이해하면 경계가 선명해지는 기반 개념이다. |
| 메모리 매핑 파일 (mmap) | 현재 개념이 등장하게 만든 직접적인 선행 흐름이다. |
| SMP 캐시 일관성 폴스 셰어링 | 현재 개념이 구현·세분화될 때 바로 연결되는 후속 개념이다. |
| 인터럽트 구동 입출력 | 확장 학습이나 심화 비교로 이어지는 다음 단계의 키워드다. |
