...
| 개념 | 연결 포인트 |
|:---|:---|
| 페이지 컬러링 캐시 경합 회피 물리 할당 | 현재 개념으로 들어오기 전에 함께 이해하면 경계가 선명해지는 기반 개념이다. |
| 틱리스 커널(Tickless) 모바일 배터리 보존 | 현재 개념이 등장하게 만든 직접적인 선행 흐름이다. |
| 유니커널 보안과 가벼운 부팅 특성 망 적용 | 현재 개념이 구현·세분화될 때 바로 연결되는 후속 개념이다. |
| 분산 락 주키퍼(ZooKeeper) 합의 동기화 | 확장 학습이나 심화 비교로 이어지는 다음 단계의 키워드다. |
