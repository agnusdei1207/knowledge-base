...
| 개념 | 연결 포인트 |
|:---|:---|
| NUMA 로컬 메모리 원격 메모리 지연차 | 현재 개념으로 들어오기 전에 함께 이해하면 경계가 선명해지는 기반 개념이다. |
| 유니커널 보안과 가벼운 부팅 특성 망 적용 | 현재 개념이 등장하게 만든 직접적인 선행 흐름이다. |
| 람포트 타임스탬프 인과 관계 정렬 | 현재 개념이 구현·세분화될 때 바로 연결되는 후속 개념이다. |
| 시스템 아키텍처 결함 허용 (Fault Tolerance) 듀얼 구성 | 확장 학습이나 심화 비교로 이어지는 다음 단계의 키워드다. |
