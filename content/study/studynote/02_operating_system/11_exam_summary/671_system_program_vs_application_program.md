...
| 개념 | 연결 포인트 |
|:---|:---|
| 하드웨어 기반 무작위 난수 생성기 (TRNG) 커널 엔트로피 풀 주입 방식 | 현재 개념으로 들어오기 전에 함께 이해하면 경계가 선명해지는 기반 개념이다. |
| 소프트웨어 오류 주입 (Fault Injection) 카오스 테스팅 시스템 커널 모듈 활용법 | 현재 개념이 등장하게 만든 직접적인 선행 흐름이다. |
| 일괄 처리 시스템 (Batch Processing System) 성능 지표 | 현재 개념이 구현·세분화될 때 바로 연결되는 후속 개념이다. |
| 다중 프로그래밍 (Multiprogramming) 한계 자원 | 확장 학습이나 심화 비교로 이어지는 다음 단계의 키워드다. |
