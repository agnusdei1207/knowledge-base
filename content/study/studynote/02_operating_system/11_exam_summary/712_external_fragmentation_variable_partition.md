...
| 개념 | 연결 포인트 |
|:---|:---|
| 주소 바인딩 컴파일/로드/실행 | 현재 개념으로 들어오기 전에 함께 이해하면 경계가 선명해지는 기반 개념이다. |
| 논리 주소 물리 주소 변환 MMU | 현재 개념이 등장하게 만든 직접적인 선행 흐름이다. |
| 내부 단편화 고정/페이징 | 현재 개념이 구현·세분화될 때 바로 연결되는 후속 개념이다. |
| 동적 할당 First/Best/Worst Fit | 확장 학습이나 심화 비교로 이어지는 다음 단계의 키워드다. |
