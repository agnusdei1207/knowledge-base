...
| 개념 | 연결 포인트 |
|:---|:---|
| 다단계 페이지 테이블 사이즈 줄이기 | 현재 개념으로 들어오기 전에 함께 이해하면 경계가 선명해지는 기반 개념이다. |
| 세그멘테이션 외부 단편화 재발 | 현재 개념이 등장하게 만든 직접적인 선행 흐름이다. |
| 페이지 폴트 (Page Fault) ISR | 현재 개념이 구현·세분화될 때 바로 연결되는 후속 개념이다. |
