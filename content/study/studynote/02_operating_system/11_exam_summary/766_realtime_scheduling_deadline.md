...
| 개념 | 연결 포인트 |
|:---|:---|
| ASLR 메모리 레이아웃 난수화 | 현재 개념으로 들어오기 전에 함께 이해하면 경계가 선명해지는 기반 개념이다. |
| SELinux 보안 강제 접근 통제 | 현재 개념이 등장하게 만든 직접적인 선행 흐름이다. |
| 스핀락 멀티 프로세서 전용 활용 | 현재 개념이 구현·세분화될 때 바로 연결되는 후속 개념이다. |
| CAS (Compare And Swap) 명령어 기초 | 확장 학습이나 심화 비교로 이어지는 다음 단계의 키워드다. |
