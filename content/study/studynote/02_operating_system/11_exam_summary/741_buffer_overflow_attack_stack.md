...
| 개념 | 연결 포인트 |
|:---|:---|
| 접근 제어 목록 (ACL) | 현재 개념으로 들어오기 전에 함께 이해하면 경계가 선명해지는 기반 개념이다. |
| 보호 도메인 최소 권한 원칙 | 현재 개념이 등장하게 만든 직접적인 선행 흐름이다. |
| 스푸핑, 백도어 악성코드 | 현재 개념이 구현·세분화될 때 바로 연결되는 후속 개념이다. |
| 가상화 하이퍼바이저 | 확장 학습이나 심화 비교로 이어지는 다음 단계의 키워드다. |
