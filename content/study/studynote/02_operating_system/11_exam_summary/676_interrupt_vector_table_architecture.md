...
| 개념 | 연결 포인트 |
|:---|:---|
| 시분할 시스템 응답 시간 최적화 | 현재 개념으로 들어오기 전에 함께 이해하면 경계가 선명해지는 기반 개념이다. |
| 멀티태스킹 (Multitasking) 용어 | 현재 개념이 등장하게 만든 직접적인 선행 흐름이다. |
| 트랩 (Trap) 기반 시스템 콜 구현 | 현재 개념이 구현·세분화될 때 바로 연결되는 후속 개념이다. |
| 커널 모드 진입 메커니즘 | 확장 학습이나 심화 비교로 이어지는 다음 단계의 키워드다. |
