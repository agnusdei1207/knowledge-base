...
2. **데이터 볼륨 한계 극복**: 의료 이미지처럼 데이터 1,000장 구하기가 하늘의 별 따기라면, 모델을 바닥부터 학습시키지 말고 이미 ImageNet으로 학습된 거대 모델을 가져와 가중치를 전이 학습 (Transfer Learning)하는 전략이 필수다.
3. **글로벌 컨텍스트의 필요성**: 이미지 안의 물체들이 서로 너무 멀리 떨어져 있어서 전체 관계 파악이 중요하다면, 순수 CNN 대신 어텐션(Attention)이 가미된 모델을 혼합해야 한다.
