...
> 1. **본질**: 블룸 필터(Bloom Filter)는 원소 존재 여부를 빠르게 추정하는 확률적 자료 구조다.
...
대규모 데이터에서 "이 값이 있을까?"를 매우 빠르게 확인해야 할 때가 있다. 이때 블룸 필터가 큰 역할을 한다.
...
아이템을 넣을 때 여러 해시 위치의 비트를 1로 만든다. 조회할 때 해당 위치가 모두 1이면 있을 수도 있다고 판단한다.
...
블룸 필터는 "없다"는 빠르게 확정해 주는 데 특히 유리하다. 그래서 불필요한 조회를 줄이는 용도로 좋다.
...
- 해시 충돌과 배열 크기를 고려하지 않는 설계
- 삭제가 필요한데 일반 Bloom Filter만 쓰는 설계

기술사 관점에서는 블룸 필터를 "정확한 검색 구조"가 아니라 "빠른 사전 필터"로 봐야 한다.
...
블룸 필터는 대규모 시스템에서 메모리와 조회 비용을 줄이는 데 매우 유용하다. 특히 없음 판정의 속도가 중요한 경우 좋다.

결론적으로 Bloom Filter는 확률적 membership test를 위한 효율적 자료 구조다.

- **📢 섹션 요약 비유**: 빠르게 거르고, 정확한 확인은 뒤에서 하는 방식이다.
