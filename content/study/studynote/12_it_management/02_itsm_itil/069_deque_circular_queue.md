...
> 1. **본질**: 데크(Deque)는 양쪽 끝에서 삽입/삭제가 가능한 자료 구조이고, 원형 큐는 배열 끝을 다시 처음과 연결해 공간을 효율적으로 쓰는 큐다.
> 2. **가치**: 스택과 큐의 장점을 함께 쓰거나, 고정 크기 버퍼를 효율적으로 관리할 수 있다.
> 3. **판단**: 구현은 단순해 보여도 포인터/인덱스 관리와 가득 참/비어 있음 판별이 중요하다.

---

## Ⅰ. 개요 및 필요성

양방향으로 넣고 빼야 하거나, 배열 공간을 낭비하지 않고 순환적으로 쓰고 싶을 때가 있다.
...
| :-- | :-- |
| Deque | 양쪽 끝 연산 |
| Circular Queue | 인덱스 순환 |

원형 큐는 배열의 끝을 다시 처음과 연결해 공간 활용을 높인다. 데크는 그보다 더 유연하게 양끝 연산을 지원한다.
