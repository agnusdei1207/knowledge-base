...
> 1. **본질**: Bloom Filter는 원소가 집합에 있는지 빠르게 검사하는 확률적 자료구조이며, False Negative는 없고 False Positive만 허용한다.
> 2. **가치**: 비트 배열과 여러 해시 함수만으로 대규모 집합을 매우 적은 메모리로 표현할 수 있어 대용량 시스템에 유리하다.
> 3. **판단**: 삭제가 필요하면 Counting Bloom Filter나 Cuckoo Filter를 고려하고, 멤버십 확인만 필요할 때 가장 빛난다.

---

## Ⅰ. 개요 및 필요성

대규모 로그, URL, 블랙리스트, 캐시 조회처럼 "이미 본 적이 있는가?"를 빠르게 확인해야 하는 일이 많다. 전통적인 집합 구조는 정확하지만 메모리를 많이 쓴다.
...
| 구성 요소 | 역할 |
| :-- | :-- |
| Bit Array | 매우 작은 메모리로 상태 저장 |
| Hash Functions | 여러 위치에 표시하여 충돌 확률 감소 |
| Insert | 해시 위치 비트를 1로 설정 |
| Query | 해당 비트가 모두 1인지 확인 |
...
Bloom Filter는 웹 크롤러 중복 방지, 캐시 프리체크, 스팸 차단, 분산 시스템의 사전 필터링에 잘 맞는다. 즉, 대량 조회 전에 불필요한 I/O를 줄이는 데 강하다.
...
Bloom Filter는 정확도와 자원의 균형을 잘 맞춘 확률적 자료구조다. 대규모 시스템에서 불필요한 조회를 줄이고 응답 속도를 높이는 데 유용하다.
