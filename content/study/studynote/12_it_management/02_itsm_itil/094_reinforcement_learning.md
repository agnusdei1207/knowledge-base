...
2. **탐험과 활용 (Exploration vs Exploitation)의 딜레마**: 늘 가던 길(활용)만 가면 최고 점수에 도달할 수 없다. 초기에는 무작위로 새로운 길을 가보도록(탐험) 설정하고, 학습이 진행될수록 점수가 높은 쪽을 선택하도록 엡실론($\epsilon$) 감쇠율을 조절하는 튜닝이 필수적이다.
