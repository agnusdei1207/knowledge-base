...
다만 모든 리스크를 없앨 수는 없다. 회피는 범위 축소를 부르고, 전가는 통제력 손실을 만들며, 완화는 비용이 들고, 수용은 항상 잔여 불확실성을 남긴다. 그래서 좋은 프로젝트는 리스크가 없는 프로젝트가 아니라, **중요한 리스크에 대해 이유 있는 전략을 선택한 프로젝트**라고 보는 편이 정확하다.
