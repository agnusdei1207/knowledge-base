...
좋은 BDD 시나리오는 구체적이고, UI 세부보다 행위 중심이며, 비즈니스 언어를 사용한다. 반대로 나쁜 시나리오는 버튼 클릭 순서만 적거나, 하나의 시나리오에 여러 정책을 몰아넣어 실패 원인을 अस्पष्ट하게 만든다. 따라서 시나리오 수를 줄이는 것보다 **책임이 명확한 시나리오**를 만드는 것이 중요하다.
