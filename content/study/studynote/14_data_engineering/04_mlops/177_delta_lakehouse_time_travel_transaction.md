...
아래 흐름은 Delta의 커밋과 스냅샷 재구성이 어떻게 이뤄지는지 보여준다. 핵심은 새 파일을 먼저 쓰고, 마지막에 커밋 로그를 올리는 **낙관적 동시성 제어(Optimistic Concurrency Control)**다. 커밋 전까지 독자는 이전 버전만 보고, 커밋이 성공한 뒤에만 새 버전을 본다.
...
Time Travel도 백업과 같은 의미는 아니다. Time Travel은 **테이블 단위 운영 복구**에 강하고, 백업은 **재해 복구와 장기 보존**에 강하다. 예를 들어 잘못된 `DELETE`나 품질 사고에는 Delta의 `RESTORE`가 빠르지만, 리전 전체 손실이나 저장소 오염까지 커버하려면 별도 백업 전략이 필요하다.
...
결론적으로 Delta Lakehouse는 단순히 "Parquet에 기능이 좀 더 붙은 것"이 아니다. 기억해야 할 핵심은 **오픈 파일 저장소 위에 트랜잭션 로그와 보존 규칙을 올려, 데이터 테이블을 버전 있는 자산으로 바꾸는 기술**이라는 점이다.
