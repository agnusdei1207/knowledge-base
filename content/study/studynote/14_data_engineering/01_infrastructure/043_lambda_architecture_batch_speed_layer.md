...
> **핵심 인사이트**
> 1. [[095_lambda_architecture|람다 아키텍처]]([[095_lambda_architecture|Lambda Architecture]])는 Nathan Marz가 제안한 [[001_dikw_pyramid|데이터]] 처리 아키텍처로 — 배치 레이어([[002_bigdata_5v|정확성]]), 스피드 레이어(실시간성), 서빙 레이어(통합 조회)의 3계층으로 "정확하고 빠른" [[001_dikw_pyramid|데이터]] 처리를 달성하지만, 동일 로직을 두 번 구현해야 하는 "코드 중복" 문제가 근본 한계이다.
> 2. [[216_lambda_kappa_architecture_batch_realtime|람다]]의 이진적 접근(배치 OR 스피드)과 달리, [[096_kappa_architecture|카파 아키텍처]]([[096_kappa_architecture|Kappa Architecture]])는 스트리밍 레이어만으로 모든 처리를 통합하여 코드 중복을 해소하지만 — 대규모 재처리(Reprocessing) 시 배치보다 자원이 많이 소요되는 트레이드오프가 있다.
> 3. 현대 [[001_dikw_pyramid|데이터]] 플랫폼([[074_photon_engine|Databricks]], [[215_flink_native_stream_watermark_window_time|Apache Flink]])은 배치와 스트리밍을 동일 API로 처리하는 "통합 컴퓨팅(Unified Compute)" 패러다임으로 [[216_lambda_kappa_architecture_batch_realtime|람다]]의 한계를 극복하며 — Kappa에 가까운 형태로 수렴하고 있다.

---

## Ⅰ. [[095_lambda_architecture|람다 아키텍처]] 3계층
...
> 📢 **섹션 요약 비유**: [[095_lambda_architecture|람다 아키텍처]]는 어제 신문(배치) + 속보 알림(스피드) 조합 — 어제까지의 정확한 기사(배치 뷰)와 방금 업데이트된 속보(리얼타임 뷰)를 함께 보여줘요.
...
## Ⅳ. [[216_lambda_kappa_architecture_batch_realtime|람다]] vs [[096_kappa_architecture|카파 아키텍처]]
...
> 📢 **섹션 요약 비유**: [[216_lambda_kappa_architecture_batch_realtime|람다]] vs 카파는 두 가지 장부 vs 하나의 디지털 장부 — [[216_lambda_kappa_architecture_batch_realtime|람다]]는 종이 장부(배치) + 메모장(스피드) 두 개, 카파는 클라우드 실시간 장부 하나. 현대는 카파 방향.
...
> 📢 **섹션 요약 비유**: 이커머스 [[095_lambda_architecture|람다 아키텍처]]는 은행 잔액 조회 — 어제까지 정산된 잔액(배치) + 오늘 [[272_atm_asynchronous_transfer_mode_53byte_cell|ATM]] 출금 합산(스피드)을 더해서 현재 잔액 표시.
