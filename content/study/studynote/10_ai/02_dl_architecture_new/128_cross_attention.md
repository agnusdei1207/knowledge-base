...
## 핵심 인사이트 (3줄 요약)
> 1. **본질**: Cross-Attention은 **Query는 디코더에서, Key·Value는 인코더에서 오는 Attention**이며, 디코더가 인코더의 출력을 참조하여 **소스→타겟 매핑(번역·요약)을 수행**한다.
> 2. **가치**: 인코더만으로는 소스 문장을 이해하지만 타겟을 생성하지 못하고, 디코더만으로는 소스를 참조하지 못하므로, Cross-Attention이 **인코더의 정보를 디코더로 전달하는 유일한 경로**이다.
...
- **📢 섹션 요약 비유**: Cross-Attention은 **통역사**이다. 화자(인코더)의 말을 듣고(K,V), 청자(디코더)가 이해하는 언어(Q)로 번역한다.
...
Cross-Attention은 **서로 다른 모달리티·언어 간 정보를 전달하는 핵심 메커니즘**이며, 멀티모달 AI의 기반이다.
...
| **Self-Attention** | 같은 시퀀스 내 참조 |
| **인코더-디코더** | Cross-Attention이 필요한 구조 |
...
### 👶 어린이를 위한 3줄 비유 설명
1. Cross-Attention은 **통역사**예요. 한국어(인코더)를 듣고 영어(디코더)로 번역해요.
2. 통역사가 없으면 한국어만 아는 사람과 영어만 아는 사람이 **대화를 못 해요**.
3. Stable Diffusion도 "고양이 그려줘"라는 **글(인코더)을 그림(디코더)으로 통역**한답니다!
