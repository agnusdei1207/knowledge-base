...
## 핵심 인사이트 (3줄 요약)
> 1. **본질**: CoT는 **"단계별로 생각해 봐(Let's think step by step)"를 프롬프트에 추가**하여 LLM이 중간 추론 과정을 명시적으로 생성하게 하는 기법이며, 산술·논리·상식 추론 정확도를 크게 향상시킨다.
> 2. **가치**: LLM이 직접 답을 출력하면 **추론 없이 패턴 매칭**하여 오류가 많지만, CoT로 **중간 단계를 생성**하면 "왜 이 답인지"의 논리적 경로가 만들어져 정확도가 2~3배 향상된다.
...
- **📢 섹션 요약 비유**: CoT는 **시험에서 풀이 과정 쓰기**이다. 답만 쓰면 실수하지만, 풀이를 쓰면 정확해진다.

---

## Ⅱ~Ⅴ. 결론

CoT는 **LLM 추론 능력 향상의 핵심 기법**이며, Self-Consistency와 ToT로 고도화되었다.
...
### 👶 어린이를 위한 3줄 비유 설명
1. CoT는 **시험에서 풀이 과정 쓰기**예요. 답만 쓰면 실수해요.
2. "단계별로 생각해 봐" 하면 AI가 **풀이를 써서** 정확해져요.
3. 여러 번 풀어서(Self-Consistency) **다수결**로 정하면 더 정확해요!
