...
## 핵심 인사이트 (3줄 요약)
> 1. **본질**: RAG는 **LLM이 답변 생성 전에 외부 지식 저장소(벡터 DB)에서 관련 문서를 검색(Retrieve)하여 프롬프트에 포함**시킨 후 생성(Generate)하는 기법이며, 환각(Hallucination)을 줄이고 최신 정보를 반영한다.
> 2. **가치**: LLM의 파라메트릭 지식은 **학습 시점에 고정**되지만, RAG는 **외부 DB를 실시간 참조**하여 학습 이후의 최신 정보·사내 문서·도메인 지식을 반영한다.
...
RAG는 **LLM 환각 해결·최신 지식 반영의 핵심 기법**이며, 임베딩+벡터DB+리랭킹이 파이프라인의 핵심이다.
...
2. 교과서 없이 기억만으로 쓰면 **틀릴 수 있지만(환각)**, 책을 보면 정확해요.
3. AI도 **검색해서 확인**하고 답하면 더 정확해요!
