...
그래서 Spark와 Ray는 같은 "분산 컴퓨팅"이라는 이름 아래 있으면서도 맡는 일이 다르다. Spark는 큰 표와 로그를 빠르고 안정적으로 가공하는 데이터 엔진에 가깝고, Ray는 Python 함수와 상태 객체를 클러스터 전역에 배치하는 AI 런타임에 가깝다. 즉 둘을 구분하는 핵심은 브랜드가 아니라 **주도적인 병렬성의 형태가 무엇인가**다.
...
핵심은 비용을 없애는 것이 아니라, 병목 위치를 바꾸는 데 있다. Spark는 디스크 기반 일괄 처리보다 메모리 기반 분산 처리로 이동하면서 속도를 높였지만, 큰 조인과 셔플에서 네트워크 병목이 생긴다. Ray는 Python 코드의 분산 표현력을 크게 높였지만, 객체 크기와 스케줄링 단위를 잘못 잡으면 오히려 오버헤드가 커진다. 따라서 두 프레임워크 모두 "노드를 많이 쓰면 무조건 빠르다"가 아니라, **파티션·객체·통신 경계를 어떻게 자르느냐**가 성패를 가른다.
...
기술사 답안에서는 **"Spark는 데이터 병렬 ETL과 대규모 셔플 최적화에 강하고, Ray는 Python 기반 분산 태스크·액터·GPU 워크로드에 강하므로, AI 인프라는 병렬성의 종류에 따라 역할을 분리해 설계해야 한다"**고 정리하면 좋다.
...
결론적으로 기억할 문장은 단순하다. **Spark는 큰 데이터를 잘 움직이게 하고, Ray는 큰 AI 작업을 잘 움직이게 한다.** 둘을 대체 관계로만 보지 말고, 데이터 준비와 모델 실행이라는 서로 다른 병렬성 축으로 이해하는 것이 실무적이다.
