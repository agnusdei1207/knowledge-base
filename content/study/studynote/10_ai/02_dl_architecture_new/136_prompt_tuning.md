...
## 핵심 인사이트 (3줄 요약)
> 1. **본질**: Prompt Tuning은 **모델 가중치를 동결하고, 입력 앞에 붙이는 연속(Soft) 프롬프트 벡터만 학습**하여 특정 작업에 적응하는 PEFT 기법이며, 학습 파라미터가 전체의 **0.01% 미만**이다.
> 2. **가치**: 모델 자체를 건드리지 않으므로 **하나의 모델로 수천 개의 다른 프롬프트(=어댑터)**를 동시에 서빙할 수 있으며, 배포·관리가 극도로 효율적이다.
...
### 👶 어린이를 위한 3줄 비유 설명
1. Prompt Tuning은 **자물쇠(AI)는 그대로 두고 열쇠(프롬프트)만 만드는** 거예요.
