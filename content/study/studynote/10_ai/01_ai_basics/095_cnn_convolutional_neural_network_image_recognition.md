...
| **가중치 연결** | 전역적 (Fully Connected) | 국소적 (Local Connectivity) |
...
2. **과적합(Overfitting) 방지**: 파라미터가 적다 해도 층이 깊어지면 과적합이 발생한다. 드롭아웃(Dropout) 적용이나 데이터 증강(Data Augmentation) 기법을 필수로 병행해야 한다.
3. **전이 학습(Transfer Learning) 검토**: 바닥부터 훈련하는 것은 비용이 낭비다. 이미 ImageNet 등으로 사전 학습된(Pre-trained) ResNet, EfficientNet 등의 가중치를 가져와 미세 조정(Fine-Tuning)하는 전략을 최우선으로 판단한다.
...
| 개념 | 연결 포인트 |
| :--- | :--- |
| **풀링 (Pooling)** | 연산량을 줄이고 특징의 위치 이동에 대한 내성을 부여하는 서브샘플링 |
| **스트라이드 (Stride)** | 필터가 한 번에 이동하는 간격으로, 보폭이 클수록 출력 크기가 작아짐 |
| **패딩 (Padding)** | 가장자리 정보 손실을 막기 위해 이미지 테두리에 0을 덧대는 기법 |
| **완전 연결 층 (FC Layer)** | 추출된 특징 맵을 1차원으로 펴서 최종 클래스 확률을 계산하는 마지막 층 |
