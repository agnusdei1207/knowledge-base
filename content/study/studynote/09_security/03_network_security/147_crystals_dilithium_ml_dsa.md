...
CRYSTALS-Dilithium은 **Module-LWE(Module Learning With Errors)** 와 **Module-SIS(Module Short Integer Solution)** 문제의 어려움에 기반한다.
...
| **SLH-DSA (SPHINCS+)** | 해시 함수 | 수학 가정 최소 (해시만 의존) | 매우 큰 서명 (수십 KB) |
| **FN-DSA (FALCON)** | 격자(NTRU) | 작은 서명 크기 | 구현 복잡(부채널 공격 취약) |
| **ECDSA** | 이산대수 | 작은 크기, 범용 지원 | 양자에 취약 |
...
**PQC 전환을 "양자 컴퓨터 완성 후"로 미루기**: "지금 수확, 나중 해독" 공격은 현재 진행 중이다. 국가 기밀, 금융 데이터는 지금 수집되어 양자 컴퓨터 완성 후 해독될 수 있다. 장기 민감 데이터는 **지금 즉시 PQC 전환**이 필요하다.
...
2. CRYSTALS-Dilithium은 **격자(Lattice)라는 수학 미로**에 기반한 새 자물쇠예요. 이 미로는 양자 컴퓨터도 빠져나오지 못할 만큼 복잡하게 설계되어 있어요!
