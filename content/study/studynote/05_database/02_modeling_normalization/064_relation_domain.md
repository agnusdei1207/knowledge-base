...
> 1. **본질**: 도메인(Domain)은 관계형 [[001_dikw_pyramid|데이터]]베이스의 속성이 가질 수 있는 합법적인 원자값(Atomic Value)들의 집합이다.
> 2. **가치**: 도메인은 [[001_dikw_pyramid|데이터]] 타입보다 더 의미 있는 제약 개념으로, 잘못된 값의 입력을 막아 무결성을 지킨다.
> 3. **판단**: 도메인을 잘 정의해야 CHECK 제약, ENUM, 형 변환, NULL [[164_policy|정책]]까지 일관되게 설계할 수 있다.
...
| 구분 | Domain | [[001_dikw_pyramid|Data]] Type | Constraint |
| :-- | :-- | :-- | :-- |
| 범위 | 의미적 허용값 | 저장 형식 | [[395_verification_process_review|검증]] 규칙 |
| 역할 | 값의 집합 정의 | 표현 방식 | 위반 차단 |
| 예시 | 성별, 연령, 금액 | INT, VARCHAR | CHECK, NOT NULL |

도메인은 타입과 제약을 이어 주는 개념이다. 타입만 맞고 의미가 틀리면 [[001_dikw_pyramid|데이터]] 품질은 여전히 나빠진다.
...
- NULL [[164_policy|정책]] 없이 필드만 쌓는 설계

기술사 관점에서는 도메인을 단순 [[001_dikw_pyramid|데이터]] 타입으로 축소하지 말아야 한다. 도메인은 [[001_dikw_pyramid|데이터]] 의미와 무결성을 함께 담는 설계 개념이다.
...
도메인을 명확히 하면 잘못된 [[001_dikw_pyramid|데이터]]가 줄고, 통계와 검색도 안정적이 된다. 결국 도메인은 [[001_dikw_pyramid|데이터]] 품질의 출발점이다.
