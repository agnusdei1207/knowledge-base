...
### 체크리스트
1. **복합키 확인**: 현재 테이블의 기본키(PK)가 2개 이상의 컬럼으로 구성되어 있는가?
2. **부분 종속성 검증**: 일반 컬럼 중 일부 PK 컬럼만으로 결정되는 값이 존재하는가? (예: `주문번호+상품코드`가 PK인데 일반 컬럼으로 `상품명`이 있는 경우)
3. **분리 및 외래키(FK) 설정**: 발견된 부분 종속성을 새로운 마스터 테이블로 떼어내고, 기존 테이블에서 외래키로 참조하게 만들었는가?
