...
## 핵심 인사이트 (3줄 요약)
> 1. **본질**: 데이터 역공학은 **문서 없는 기존 DB의 물리 스키마(테이블·컬럼·인덱스·FK)를 분석하여 ERD·개념 모델·데이터 사전을 복원**하는 프로세스다.
> 2. **가치**: 레거시 시스템은 설계 문서가 분실·미작성된 경우가 많아, 역공학 없이는 **시스템 현행화·마이그레이션·리팩터링이 불가능**하다.
> 3. **판단 포인트**: CASE 도구(ERwin·DA#)의 자동 역공학 기능으로 물리 스키마를 읽어 ERD를 생성하고, 이후 **수동으로 비즈니스 의미(엔터티명·관계 설명)를 부여**하는 2단계 프로세스가 필요하다.
...
데이터 역공학은 **레거시 시스템의 현행화·클라우드 마이그레이션·데이터 거버넌스**의 필수 사전 작업이며, 자동 추출 + 수동 의미 부여의 2단계 접근이 가장 효과적이다.
...
3. 설계도가 없는 옛날 건물(레거시)을 수리하려면 먼저 **설계도를 복원(역공학)**해야 한답니다!
