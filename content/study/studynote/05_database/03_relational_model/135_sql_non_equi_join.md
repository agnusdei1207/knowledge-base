...
> 2. **가치**: "급여 등급 테이블과 직원 급여를 범위 매칭"(Non-Equi), "직원과 그 상사를 같은 테이블에서 결합"(Self)은 **Equi JOIN으로 불가능한 핵심 쿼리**이다.
> 3. **판단 포인트**: Self JOIN은 **반드시 별칭(Alias)을 사용**해야 하며, Non-Equi는 **인덱스 최적화가 어려워** 대량 데이터에서 성능 주의가 필요하다.
...
Non-Equi·Self JOIN은 **관계형 데이터 분석의 필수 기법**이며, 계층 쿼리·범위 매칭에 핵심이다.
