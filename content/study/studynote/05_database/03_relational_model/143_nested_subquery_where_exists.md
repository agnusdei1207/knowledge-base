...
> 2. **가치**: "주문이 있고, 그 주문에 반품이 있는 고객"처럼 **다단계 조건**을 직관적으로 표현할 수 있지만, 깊어질수록 가독성과 성능이 나빠진다.
> 3. **판단 포인트**: 2단계 이상 중첩 시 **CTE(WITH 절)로 분해**하여 가독성을 높이고, 옵티마이저 실행계획을 반드시 확인한다.
...
중첩 서브쿼리는 **다단계 필터의 직관적 표현**이지만, 2단계+ 시 CTE 분해로 가독성을 확보한다.
...
3. 한 번에 두 가지 조건("주문 있고 반품 있는")을 **확인**할 수 있어요!
