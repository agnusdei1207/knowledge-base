...
> 1. **본질**: 인덱스 레인지 스캔 (Index Range Scan)는 질의 처리·인덱스 관점에서 자주 쓰이는 인덱스 설계 요소이다.
...
인덱스 레인지 스캔 (Index Range Scan)는 질의 처리·인덱스 관점에서 자주 쓰이는 인덱스 설계 요소이다. 이 주제가 필요한 이유는 같은 SQL이라도 접근 경로와 조인 방식에 따라 디스크 I/O와 CPU 사용량이 크게 달라지기 때문이다. 특히 `테이블 풀 스캔 (Table Full Scan / FTS)`에서 드러난 한계를 줄이고 `인덱스 패스트 풀 스캔 (병렬)` 같은 후속 판단의 기준선을 세울 때 현재 개념이 중심축이 된다.

시험과 실무에서 `인덱스 레인지 스캔 (Index Range Scan)`를 따로 외우기보다, "무엇을 보호하거나 최적화하려는가"라는 질문으로 연결해야 오래 남는다. 대형 조회 서비스에서는 100 ms 이하 응답을 위해 풀 스캔, 인덱스 스캔, 조인 순서를 정밀하게 통제해야 한다.
...
`인덱스 레인지 스캔 (Index Range Scan)`의 핵심 원리는 인덱스 구조, 실행 계획, 조인 전략, 변환 규칙을 통해 필요한 데이터만 효율적으로 읽는다는 점이다. 여기서 중요한 것은 `질의 처리·인덱스 맥락에서 역할과 경계를 판단해야 하는 주제`를 어떤 순서로 평가하고 어느 경계에서 확정하느냐다. 이 순서가 바뀌면 정합성, 처리량, 지연시간 중 손해를 보는 축이 달라진다.

| 관점 | 설명 | 설계 포인트 |
| :--- | :--- | :--- |
| 핵심 대상 | `인덱스 레인지 스캔 (Index Range Scan)`는 `질의 처리·인덱스 맥락에서 역할과 경계를 판단해야 하는 주제`를 다루는 중심 규칙이다. | 먼저 무엇을 보호하거나 빠르게 할 것인지 명확히 정한다. |
...
결국 `인덱스 레인지 스캔 (Index Range Scan)`는 한 문장 정의보다 입력 조건, 처리 순서, 결과 보장을 묶어 보는 것이 중요하다. 그래서 설계 문서에는 적용 대상, 실패 시 복구 경로, 측정 지표를 같이 적어 두는 편이 좋다.
...
실무에서는 `인덱스 레인지 스캔 (Index Range Scan)`를 이론 용어가 아니라 운영 선택지로 다뤄야 한다. 대형 조회 서비스에서는 100 ms 이하 응답을 위해 풀 스캔, 인덱스 스캔, 조인 순서를 정밀하게 통제해야 한다. 특히 장애가 나거나 부하가 급증할 때는 현재 개념이 병목을 줄이는지, 아니면 구조만 복잡하게 만드는지 냉정하게 평가해야 한다.
...
3. 모니터링 지표, 예외 처리, 복구 절차가 `인덱스 레인지 스캔 (Index Range Scan)`의 특성과 맞게 준비되어 있는가?
...
결론적으로 `인덱스 레인지 스캔 (Index Range Scan)`는 `테이블 풀 스캔 (Table Full Scan / FTS)`와 `인덱스 패스트 풀 스캔 (병렬)` 사이에서 현재 시스템이 감당할 수 있는 균형점을 만드는 개념이다. 시험에서는 배경, 원리, 비교, 판단 기준을 함께 답하고, 실무에서는 지표와 운영 정책으로 연결할 수 있어야 한다.
...
| 개념 | 연결 포인트 |
| :--- | :--- |
| 테이블 풀 스캔 (Table Full Scan / FTS) | 현재 주제가 등장하기 전 단계에서 드러나는 문제 또는 전제 조건을 보여 준다. |
| 인덱스 패스트 풀 스캔 (병렬) | 현재 판단이 실제 확장 또는 후속 제어로 이어지는 지점을 보여 준다. |
