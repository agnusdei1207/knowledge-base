...
> **핵심 인사이트**
> 1. 셀렉트 연산(σ, [[520_select|Select]]/Restriction)은 [[038_relational_algebra|관계 대수]]의 단항 수평 필터링 연산으로, SQL WHERE 절의 수학적 기반이며 조건을 만족하는 [[063_relation_tuple_cardinality|튜플]](행)만 결과 [[061_relation_schema_instance|릴레이션]]으로 반환한다.
> 2. 셀렉트는 가장 선택적(Selective)인 조건을 먼저 적용하면 중간 결과의 크기를 최소화할 수 있어, [[502_dbms|DBMS]] [[163_optimizer_sql_execution_plan_generator|옵티마이저]]의 핵심 최적화 규칙 중 하나인 "조건 밀어내기(Predicate Pushdown)"의 이론적 근거가 된다.
> 3. 셀렉트 조건의 [[170_selectivity_cardinality_distribution_tuning|선택도]]([[170_selectivity_cardinality_distribution_tuning|Selectivity]])는 [[154_database_index_b_tree_search_optimization|인덱스]] 활용 여부를 결정하는 핵심 지표로, [[170_selectivity_cardinality_distribution_tuning|선택도]]가 낮을수록(결과 행이 적을수록) [[154_database_index_b_tree_search_optimization|인덱스]] 스캔이 유리하고 [[170_selectivity_cardinality_distribution_tuning|선택도]]가 높으면 풀 테이블 스캔이 오히려 효율적이다.
...
## Ⅲ. [[170_selectivity_cardinality_distribution_tuning|선택도]]와 [[154_database_index_b_tree_search_optimization|인덱스]]
...
> 📢 **섹션 요약 비유**: [[170_selectivity_cardinality_distribution_tuning|선택도]]는 도서관 색인 사용 기준 — 책이 1만 권인데 한 권 찾기([[170_selectivity_cardinality_distribution_tuning|선택도]] 낮음)는 색인 유리, 절반 찾기(높음)는 전체 훑기 빠름.
