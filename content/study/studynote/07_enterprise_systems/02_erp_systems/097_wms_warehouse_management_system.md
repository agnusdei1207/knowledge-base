...
| **피킹 (Picking) 라우팅** | 여러 주문을 분석하여, 작업자가 창고를 가로지르는 동선을 최소화하는 최단 경로 (Z자형, U자형 등) 제공 | 이동 시간 단축 (동선 낭비 0%) |
...
3. **피킹 전략의 유연성**: 단일 주문 피킹, 일괄(Batch) 피킹, 존(Zone) 피킹 등 사업 모델(B2B 대량 vs B2C 소량 다건)에 맞는 전략을 설정할 수 있는가?
...
| **선입선출 (FIFO)** | 먼저 입고된 상품(유통기한 임박)을 WMS가 강제로 먼저 출고 지시하는 로직 |
| **피킹 (Picking)** | 창고에서 고객의 주문 내역에 맞춰 물건을 선반에서 꺼내오는 가장 핵심적인 작업 |
| **SCM (공급망 관리)** | WMS를 포함하여 원자재부터 최종 고객 배송까지의 전체 흐름을 관리하는 거시적 체계 |
