...
## 핵심 인사이트 (3줄 요약)
> 1. **본질**: MES는 **ERP(경영 계획)와 PLC/SCADA(설비 제어) 사이**에서 **생산 현장의 실시간 실행·모니터링·추적·품질 관리**를 수행하는 제조 실행 시스템이다.
> 2. **가치**: ERP가 "1000개 생산하라"는 계획을 세우면, MES가 "어떤 설비에서, 어떤 순서로, 현재 진행률은?"을 **실시간으로 관리하고 실적을 ERP에 피드백**한다.
> 3. **판단 포인트**: ISA-95 표준이 ERP-MES-설비 계층을 정의하며, 스마트 팩토리(Industry 4.0)에서 MES는 **IoT 센서·디지털 트윈·AI 품질 예측**과 통합되어 진화하고 있다.
...
### 스마트 팩토리에서의 MES 진화
- **IoT 연동**: 센서 데이터 실시간 수집 → MES 대시보드.
- **디지털 트윈**: 생산 라인의 가상 복제 → 시뮬레이션.
- **AI 품질 예측**: 불량 발생 전 예측 → 예방 조치.
...
MES는 스마트 팩토리의 **중추 신경계**이며, 클라우드 MES(SaaS)로 전환되면서 중소기업도 접근 가능해지고 있다.
...
| 개념 | 연결 포인트 |
|:---|:---|
| **ERP** | MES의 상위 계층 (경영 계획) |
| **SCADA/PLC** | MES의 하위 계층 (설비 제어) |
| **ISA-95** | ERP-MES-설비 계층 표준 |
| **디지털 트윈** | MES와 연동하는 가상 공장 |
| **스마트 팩토리** | MES+IoT+AI의 통합 체계 |
