...
## 핵심 인사이트 (3줄 요약)
> 1. **본질**: SECI 모델은 노나카 이쿠지로(1995)가 제안한 **조직 지식 창조 이론**으로, 암묵지↔형식지의 4가지 변환(사회화→외부화→결합화→내면화)의 나선형 순환을 통해 새로운 지식이 창조된다.
...
SECI 모델은 **KMS·CoP·학습 조직의 이론적 기반**이며, AI/RAG가 외부화·결합화를 자동화하는 새 장을 열고 있다.
...
### 👶 어린이를 위한 3줄 비유 설명
1. SECI는 지식의 **4계절**이에요. 봄(배우기)→여름(쓰기)→가을(모으기)→겨울(체화) 순환해요.
