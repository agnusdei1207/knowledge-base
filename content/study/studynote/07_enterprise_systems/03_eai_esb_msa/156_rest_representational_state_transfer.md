...
즉 REST의 핵심은 URL만 예쁘게 짓는 것이 아니라, HTTP 자체가 가진 의미를 살려 **웹의 기본 동작 원리 위에 API를 얹는 것**이다. 그래서 메서드 의미와 캐시 정책을 무시한 채 `POST /getUser` 같은 형태를 쓰면 HTTP를 터널처럼 쓰는 셈이 된다.
...
REST는 이후 Richardson Maturity Model, HATEOAS (Hypermedia as the Engine of Application State), API Gateway 설계와도 연결된다. 즉 REST는 단순 호출 규칙이 아니라, 웹 API 전반의 설계 철학을 여는 출발점이다.
...
반면 REST가 모든 통합 문제의 정답은 아니다. 강한 계약, 복잡한 트랜잭션 보장, 초저지연 이진 통신이 더 중요한 환경에서는 SOAP, gRPC, 이벤트 기반 방식이 더 적절할 수 있다. 따라서 REST는 "HTTP로 API를 만든다"가 아니라 **웹의 제약을 이용해 단순하고 확장 가능한 인터페이스를 설계하는 방식**으로 기억해야 한다.
