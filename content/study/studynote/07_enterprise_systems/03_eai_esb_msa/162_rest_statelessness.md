...
> 1. **본질**: 무상태성 (Statelessness)은 REST (Representational State Transfer)에서 **각 요청이 스스로 완결된 정보를 담아 서버가 이전 대화를 기억하지 않아도 처리 가능해야 한다**는 제약이다.
> 2. **가치**: 서버가 사용자별 세션 문맥을 붙잡지 않으므로, 로드밸런싱·장애 복구·수평 확장에 유리해 대규모 API (Application Programming Interface) 운영의 기본 토대가 된다.
> 3. **판단 포인트**: 무상태성은 "아무 상태도 없다"는 뜻이 아니라, **대화 상태를 서버 메모리에 숨기지 말고 자원 상태와 인증 정보를 명시적으로 다루라**는 설계 원칙이다.
...
즉 무상태성은 단순히 세션을 없애는 기법이 아니라, **분산 환경에서 인터페이스 책임을 정리하는 아키텍처 선택**이다. 이 관점이 있어야 JWT (JSON Web Token), 세션 스토어, 캐시, API 게이트웨이의 역할도 구분해서 설명할 수 있다.
...
기술사 답안에서는 "무상태성은 서버 세션 미사용으로 확장성과 가용성을 높이지만, 토큰 관리와 요청 설계가 함께 성숙해야 한다"고 정리하면 좋다. 즉 무상태성은 만능 단순화가 아니라, **운영 편의와 메시지 명시성을 맞바꾸는 전략**이다.
...
하지만 모든 부담이 자동으로 사라지는 것은 아니다. 요청당 정보량 증가, 토큰 보안, 만료 처리, 권한 변경 전파 문제는 여전히 설계해야 한다. 따라서 무상태성은 "기억을 포기한 서버"가 아니라, **기억을 자원과 메시지에 명시적으로 배치한 서버**로 이해하는 것이 정확하다.
