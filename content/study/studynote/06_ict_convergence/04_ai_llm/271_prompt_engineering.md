...
> 1. **본질**: 프롬프트 엔지니어링 (Prompt Engineering): LLM의 숨겨진 능력을 이끌어내기 위해 질문 컨텍스트를 구조화 설계를 이해하는 핵심 개념으로, 데이터에서 패턴을 학습해 예측·생성·판단 보조로 연결해야 하는 문제를 설명하는 데 쓰인다.
...
프롬프트 엔지니어링 (Prompt Engineering): LLM의 숨겨진 능력을 이끌어내기 위해 질문 컨텍스트를 구조화 설계를 다루는 개념이다. 이 주제가 중요한 이유는 데이터에서 패턴을 학습해 예측·생성·판단 보조로 연결해야 하는 문제를 단순한 선언이 아니라 실제 설계 항목으로 바꾸기 때문이다. 다시 말해, "왜 필요한가"를 묻는 순간 이 개념은 문제를 구조화하는 언어가 된다.
