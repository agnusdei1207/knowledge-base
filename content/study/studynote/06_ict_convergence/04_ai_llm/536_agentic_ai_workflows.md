...
지금까지의 [[263_llm_large_language_model|LLM]] 활용은 사용자가 질문하면 AI가 답하는 일회성 인터랙션이 주를 이루었다. 하지만 복잡한 비즈니스 프로세스나 소프트웨어 개발 등은 단 한 번의 프롬프트로 해결하기 어렵다. 에이전틱 [[190_ai_llm_requirements_specification|AI]] 워크플로우는 AI에게 "이 코드를 작성해"라고 시키는 대신, "이 기능을 구현하기 위해 설계를 하고, 코드를 짜고, 테스트를 해본 뒤 오류가 나면 스스로 수정해"라고 명령하는 방식이다. 이는 '시스템 2 사고(느리고 논리적인 추론)'를 AI에게 이식하는 과정이라 할 수 있다.
...
3. **Planning**: 거대한 목표를 하위 [[150_task|태스크]](Sub-tasks)로 분해하고 순차적으로 실행한다.
...
| 비교 항목 | 전통적 [[263_llm_large_language_model|LLM]] (Prompt-based) | 에이전틱 [[190_ai_llm_requirements_specification|AI]] (Workflow-based) |
| :--- | :--- | :--- |
| **작동 방식** | 입력 -> 출력 ([[008_단방향_반이중_전이중|단방향]]) | 계획 -> 실행 -> 반추 (루프) |
| **[[282_performance_tactics|성능]] 결정** | 모델의 파라미터 규모 | 워크플로우 설계 및 반복 횟수 |
| **추론 비용** | 낮음 (1회 실행) | 높음 (다회 실행 및 [[395_verification_process_review|검증]]) |
| **정확도** | 중간 ([[251_hallucination_rag_augmented_retrieval_vector_db|할루시네이션]] 취약) | 높음 (스스로 오류 수정) |
...
1. **거버넌스**: AI가 자율적으로 도구를 사용하고 결제나 [[001_dikw_pyramid|데이터]] 삭제 등의 액션을 취할 때를 대비해 **Human-in-the-loop (중간 승인)** 설계가 필수적이다.
2. **안정성**: 무한 루프(Infinite Loop)에 빠져 비용이 폭증하는 것을 방지하기 위해 최대 반복 횟수(Max Iterations)와 비용 한도를 설정해야 한다.
3. **평가**: 정적인 벤치마크보다는 실제 [[150_task|태스크]] 성공률([[150_task|Task]] Success Rate)을 지표로 관리해야 한다.
...
에이전틱 AI는 기업의 업무 방식 자체를 완전히 바꿀 것이다. 단순히 답변을 주는 수준을 넘어, 스스로 업무를 완결 짓는 '[[190_ai_llm_requirements_specification|AI]] 사원'의 등장을 의미한다. 향후에는 에이전트 간의 소통 표준 프로토콜이 정립될 것이며, 이는 서로 다른 회사의 [[190_ai_llm_requirements_specification|AI]] 에이전트들이 협력하여 복잡한 비즈니스 거래를 수행하는 **Agentic Economy**의 토대가 될 것이다.
...
| 개념 | 연결 포인트 |
|:---|:---|
| 부모 개념 | Autonomous Agents, [[263_llm_large_language_model|LLM]] |
| 연관 개념 | ReAct, [[216_autogpt_autonomous_agent|AutoGPT]], BabyAGI, Multi-agent Systems, Tool Use |
| 파생 기술 | LangGraph, CrewAI, Autogen, Semantic [[022_kernel_role|Kernel]] |
