...
WSN은 크게 데이터를 수집하는 센서 노드, 데이터를 모아서 외부로 보내는 싱크 노드(Base Station), 그리고 외부 인터넷을 연결하는 게이트웨이로 구성된다. 핵심 메커니즘은 노드 간 데이터를 중계하는 **멀티 홉 (Multi-hop) 통신**과 **데이터 병합 (Data Aggregation)**이다.
...
1. **Sleep/Wake-up 동기화**: 센서가 항상 켜져 있으면 며칠 못 가 방전된다. 이웃 노드들이 동시에 깨어나 데이터를 주고받고 다시 잠드는 타이밍(MAC 프로토콜) 설계가 WSN 수명을 결정한다.
