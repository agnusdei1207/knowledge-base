...
- **📢 섹션 요약 비유**: OCF는 IoT의 **USB 표준**이다. USB 이전에는 프린터마다 다른 케이블이 필요했지만, USB로 통일되면서 아무 프린터나 연결할 수 있게 되었다.
...
| **Matter** | 스마트 홈 상호운용성 표준 |
| **CoAP** | OCF의 기본 전송 프로토콜 |
| **oneM2M** | IoT 서비스 플랫폼 표준 |
| **Thread** | 저전력 메시 네트워크 (Matter 전송) |
...
### 👶 어린이를 위한 3줄 비유 설명
1. OCF는 IoT의 **USB 표준**이에요. 어떤 회사 제품이든 **같은 규격으로 연결**돼요.
2. USB 이전에는 프린터마다 **다른 케이블**이 필요했지만, USB로 통일되면서 편리해졌어요.
