...
이 그림의 핵심은 클럭 주기가 단순히 파형 하나의 길이가 아니라, 여러 지연 요소를 모두 합친 **타이밍 예산**이라는 점이다. 여기서 `Clock-to-Q`는 레지스터가 출력을 내보내기까지의 시간이고, `Logic Delay`는 조합 논리 지연, `Setup Margin`은 다음 레지스터가 안정된 값을 읽기 위해 필요한 최소 여유다. 여기에 클럭 스큐 (Clock Skew)와 지터 (Jitter) 같은 배선·발진 오차도 추가로 고려해야 한다.
