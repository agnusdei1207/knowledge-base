...
| **BPF HW 오프로딩** | SmartNIC / DPU 내부 | 중간 | 매우 낮음 | 가장 안정적 | 헤더 기반 차단, 리다이렉트, 카운팅 |
...
하지만 이 기술은 “유연한 소프트웨어를 하드웨어로 공짜 이전”해 주는 만능 버튼이 아니다. 벤더별 드라이버 지원, 제한된 헬퍼와 맵 타입, NIC 내부 자원 한계, 디버깅 난도가 항상 따라온다. 따라서 기억해야 할 관점은 하나다. **BPF HW 오프로딩은 성능 기법이기 전에 데이터 평면을 어디까지 하드웨어로 밀어 넣을지 결정하는 아키텍처 선택**이라는 점이다.
