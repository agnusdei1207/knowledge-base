...
즉 베이퍼 체임버는 쿨러의 대체품이 아니라, **작은 열원을 넓은 방열 면적으로 연결하는 평면형 열 분산기**다.
...
핵심은 열이 단순 전도만으로 옆으로 이동하는 것이 아니라, **상변화의 잠열과 증기 확산을 이용해 평면 전체로 분산된다는 점**이다. 그래서 같은 두께의 solid copper plate보다 hotspot 완화 능력이 훨씬 높을 수 있다. 다만 체임버 바깥에서 열을 공기나 섀시로 내보낼 경로가 부족하면 내부 분산만 잘될 뿐 전체 온도는 여전히 높다.
