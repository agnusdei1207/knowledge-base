...
영 데이터 건너뛰기 로직은 "결과가 이미 뻔한 값은 끝까지 보내지 말자"는 발상에서 출발한다. 수학적으로 0은 다른 값과 곱하면 항상 0이지만, 전통적인 하드웨어는 이 단순한 사실을 몰라서 0도 일반 값처럼 버스에 싣고, 레지스터 파일에 넣고, MAC 배열까지 통과시킨다. 즉 **의미는 없지만 비용은 그대로 드는 데이터**가 시스템 안을 계속 돌아다니는 셈이다.
...
따라서 zero-skipping의 핵심은 압축률을 높이는 데만 있지 않다. 더 중요한 목표는 **유효하지 않은 값이 하드웨어 자원을 점유하는 시간 자체를 줄이는 것**이다. 이 관점이 있어야 왜 메모리와 연산 파이프라인 양쪽에서 동시에 zero-skipping을 고민하는지 이해할 수 있다.
...
zero-skipping은 보통 두 지점에서 구현된다. 첫째는 **저장·이동 경로**다. 캐시나 온칩 버퍼는 전부 0인 블록을 감지하면 실제 데이터 배열 대신 영 플래그 비트 (Zero Bit, Z-bit)나 비트마스크만 남기고, 읽을 때는 메모리 셀을 다시 켜지 않고 0을 합성해서 내보낼 수 있다. 둘째는 **연산 경로**다. 입력 피연산자가 0이면 곱셈기 lane을 깨우지 않거나, 클럭 게이팅 (Clock Gating)으로 해당 회로의 스위칭을 막고 결과를 곧바로 0 또는 누산 생략으로 처리한다.
...
아래 그림은 zero-skipping이 단순 비교기 하나가 아니라, **감지 → 표시 → 스케줄링 → 회로 비활성화**의 연속 구조임을 보여 준다.
...
결국 zero-skipping은 584번 텐서 희소성과도 직결된다. 희소성이 "데이터의 상태"라면, zero-skipping은 그 상태를 보고 **연산기와 메모리 시스템이 실제로 행동을 바꾸는 메커니즘**이다. 이 둘을 분리해서 생각하면 왜 어떤 모델은 희소해도 빨라지지 않는지 설명할 수 있다.
...
하지만 이 기술은 만능 해법이 아니다. 희소성이 흔들리면 발행기가 오히려 복잡해지고, 너무 공격적으로 건너뛰면 부하 불균형이 커질 수 있으며, 메타데이터 해석 지연이 새 병목이 되기도 한다. 앞으로는 구조적 희소성 규칙, 압축 포맷, 컴파일러 스케줄링, adaptive throttling이 함께 맞물리는 **sparsity-aware pipeline** 방향으로 진화할 가능성이 높다.

결론적으로 zero-skipping은 "0을 빠르게 계산하는 기술"이 아니라 **0을 계산하지 않기로 결정하는 기술**이다. 이 차이를 이해해야 zero-skipping을 단순 절전 회로가 아니라 데이터 의미를 읽는 실행 최적화로 볼 수 있다.
