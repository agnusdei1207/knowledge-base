...
> 1. **본질**: 메시지 패싱 하드웨어 큐 (Message Passing Hardware Queue)는 공유 캐시 라인을 붙잡고 싸우는 대신, 데이터나 소유권을 **명시적 메시지**로 옮겨 코어·가속기 사이 협업을 만드는 하드웨어 통신 구조다.
...
메시지 패싱 하드웨어 큐는 발신자가 수신자의 큐에 메시지를 넣고, 수신자가 이를 꺼내 처리하는 과정을 실리콘 내부에서 직접 지원하는 FIFO (First-In, First-Out) 통신 장치다. 공유 메모리 기반 동기화가 "같은 데이터 구조를 여러 코어가 번갈아 소유"하는 방식이라면, 메시지 패싱은 "필요한 정보만 상대 큐로 전달하고 책임도 함께 넘기는 방식"에 가깝다. 그래서 이 구조의 핵심은 단순 버퍼가 아니라 **공유 상태를 줄이는 통신 규칙**에 있다.
...
메시지 패싱 하드웨어 큐의 기본 경로는 `발신 코어 → 로컬 네트워크 인터페이스 → NoC 라우터 → 수신 큐 → 이벤트 통지`로 이어진다. 발신자는 보통 데이터 전체를 무조건 복사하는 것이 아니라, 작은 제어 메시지·디스크립터·주소 핸들을 큐에 넣고, 실제 큰 데이터는 로컬 버퍼나 DMA (Direct Memory Access) 경로에서 읽도록 설계한다. 이렇게 하면 큐는 "모든 데이터를 담는 통로"가 아니라 **작업과 소유권을 이동시키는 제어면**으로 동작한다.
...
또한 이 개념은 네트워크 인터페이스 카드 (Network Interface Card, NIC)의 다중 큐, 칩렛 간 패브릭, 분산 공유 메모리 (Distributed Shared Memory, DSM)와도 이어진다. 즉 메시지 큐는 단지 코어 간 우편함이 아니라, **메모리 일관성을 어디까지 하드웨어가 책임질지 정하는 경계선**이기도 하다.
...
반대로 모든 단계가 같은 대형 자료구조를 실시간으로 함께 고쳐야 하는 구조라면, 하드웨어 큐가 만능 해법이 아니다. 메시지 생성 비용, 복사 비용, 큐 깊이 튜닝, 디버깅 난도가 생기므로 "공유 상태를 줄일 수 있는가"를 먼저 판단해야 한다. 즉 채택 기준은 단순히 빠르냐가 아니라 **소유권을 명확히 분리할 수 있느냐**다.
...
따라서 이 개념은 "공유 메모리의 대체품"이 아니라, **경합을 줄이기 위해 소유권 이동 자체를 아키텍처에 올린 방식**으로 기억하는 것이 맞다. 언제 공유하고 언제 전달할지를 나누는 순간, 시스템 확장성의 상한이 달라진다.
