...
고장 허용 시스템은 보통 **중복 (Redundancy)**, **오류 검출**, **격리**, **대체 수행**, **상태 일관성 유지**라는 다섯 축으로 구성된다. 중복만 있다고 고장 허용이 완성되지는 않는다. 장애를 알아채지 못하면 전환할 수 없고, 전환은 했더라도 상태가 어긋나면 같은 결과를 계속 제공할 수 없기 때문이다.
