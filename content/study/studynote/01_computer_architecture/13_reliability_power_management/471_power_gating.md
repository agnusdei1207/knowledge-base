...
이 구조에서 중요한 구성요소는 네 가지다. 첫째, **전력 스위치**는 실제로 전원 경로를 차단한다. 둘째, **상태 보존 레지스터**는 필요한 문맥만 남겨 복귀 시간을 줄인다. 셋째, **아이솔레이션 셀 (Isolation Cell)**은 꺼진 블록의 출력이 부정확한 중간 전압으로 외부를 오염시키지 못하게 막는다. 넷째, **레벨 시프터 (Level Shifter)**는 서로 다른 전압 도메인 간 신호를 안전하게 넘긴다.
