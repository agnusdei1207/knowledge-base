...
| **1-주소 명령어** | **누산기 (AC)** | `ADD X` | 하드웨어 단순, 포맷 간결 | AC 병목, 메모리 왕복 증가 |
...
1. 1-주소 명령어의 장점은 "주소 수 감소" 자체가 아니라, **그 결과로 얻는 하드웨어 단순화**에 있다.
2. 단점은 "레지스터가 적다"가 아니라, **AC 하나에 모든 상태가 집중되어 메모리 트래픽과 직렬 의존성이 증가**한다는 데 있다.
