...
미래 확장 관점에서는 대용량 멀티코어, 비휘발성 메모리, CXL (Compute Express Link) 같은 계층 확장에서 이 정책의 의미가 더 커질 수 있다. 다만 계층이 많아질수록 "어디에 최신 데이터가 있는가"를 맞추는 비용도 함께 증가한다. 따라서 Write-Back은 **캐시가 메모리를 대신 저장해 주는 기술**이 아니라, **최신 상태를 잠시 위임받아 성능을 벌어 주는 기술**로 기억하는 것이 정확하다.
