...
희생자 캐시 (Victim Cache)는 상위 캐시에서 교체된 라인을 하위 메모리로 곧바로 내려보내지 않고 잠시 붙잡아 두는 보조 캐시다. 핵심은 캐시 미스 전체를 없애는 것이 아니라, **방금 내보낸 데이터를 곧바로 다시 찾는 상황**을 저비용으로 막는 데 있다.
...
하위 캐시 확대와도 목적이 다르다. L2 캐시나 L3 캐시 (Level 3 Cache)는 더 큰 작업 집합을 담기 위한 용량 중심 계층이다. 반면 희생자 캐시는 용량이 아니라 **최근 축출 이력**을 활용하는 구조다. 따라서 용량 미스가 지배적인 워크로드에서는 L2 확장이 더 중요하고, 충돌이 지배적이면 희생자 캐시가 더 높은 투자 대비 효과를 낸다.
