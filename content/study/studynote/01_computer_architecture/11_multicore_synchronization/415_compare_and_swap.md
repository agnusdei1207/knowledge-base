...
여기서 등장하는 대표적 함정이 **ABA 문제**다. 스레드가 값 `A`를 읽은 뒤 잠시 멈춘 사이 다른 스레드가 `A -> B -> A`로 바꾸면, CAS는 다시 `A`를 보고 "변화가 없었다"고 오판할 수 있다. 정수 카운터에서는 문제가 없을 수도 있지만, 포인터 기반 스택·큐에서는 노드가 한 번 제거됐다가 재사용되는 순간 심각한 구조 손상이 생길 수 있다.
