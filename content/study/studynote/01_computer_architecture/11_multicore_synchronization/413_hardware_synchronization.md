...
3. **공유 데이터 배치**: 락 변수와 뜨거운 데이터가 같은 캐시 라인에 있으면 거짓 공유 (False Sharing)로 인해 성능이 급격히 떨어진다.
...
기술사 답안 관점에서는 "하드웨어 동기화 = 고성능"이라고 단정하지 말고, **짧은 구간·낮은 충돌·정확한 메모리 순서 설계**일 때 강점을 발휘한다고 쓰는 것이 정확하다. 또한 운영체제 수준 락, 캐시 일관성, 메모리 배리어와 함께 설명해야 입체적인 답안이 된다.
