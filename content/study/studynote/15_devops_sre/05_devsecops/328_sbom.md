...
- **SBOM 1회 생성 후 방치**: 의존성이 바뀔 때마다 갱신하지 않으면 실효성 없음
- **VEX 없는 SBOM**: CVE 우선순위 관리 어려움, 보안팀 과부하
...
SBOM의 본질은 **알 권리의 자동화**다. 내가 사용하는 소프트웨어에 무엇이 들어있는지 아는 것이 보안의 첫 번째 단계다. 모르는 것은 보호할 수 없다.
