...
> 3. **판단**: 충돌 자체보다, 왜 충돌이 생겼는지와 어떤 [[268_strategy_pattern|전략]]이 팀에 맞는지 이해하는 것이 중요하다.

---

## Ⅰ. 개요 및 필요성

여러 사람이 같은 [[501_file_definition_logical_record|파일]]을 동시에 바꾸면 충돌이 날 수 있다. Git은 이 상황을 자동으로 해결하지 못할 때 충돌을 보여 준다.
...
충돌은 같은 줄, 같은 블록, 같은 의미를 서로 다르게 바꿨을 때 자주 발생한다. 해결 후에는 테스트로 의미를 [[396_validation|확인]]해야 한다.
...
Merge와 Rebase는 도구가 아니라 [[268_strategy_pattern|전략]]이다. 팀의 브랜치 [[164_policy|정책]]에 맞게 선택해야 한다.
...
4. 충돌 해결 후 테스트하는가?
5. 브랜치 [[164_policy|정책]]이 문서화되어 있는가?
...
- 팀 [[164_policy|정책]] 없이 혼자만의 방식으로 해결하는 설계
...
충돌 해결 [[268_strategy_pattern|전략]]을 이해하면 협업의 안정성과 히스토리 관리가 좋아진다.
