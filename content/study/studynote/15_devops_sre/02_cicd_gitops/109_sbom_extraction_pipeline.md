...
## 핵심 인사이트 (3줄 요약)
> 1. **본질**: SBOM(Software Bill of Materials)은 소프트웨어를 구성하는 **모든 오픈소스 라이브러리·버전·의존성 관계를 기계 판독 가능한 표준 포맷(SPDX, CycloneDX)으로 기록한 디지털 자재 명세서**다.
> 2. **가치**: CI/CD 파이프라인에서 빌드 시점에 자동 추출하여, Log4j 같은 **공급망 공격(Supply Chain Attack) 발생 시 취약 컴포넌트 포함 여부를 수초 내 전수 조사**할 수 있다.
> 3. **판단 포인트**: 미 행정명령(EO 14028)으로 연방 납품 소프트웨어에 SBOM 제출이 **법적 의무화**되었으며, SBOM + VEX(실제 악용 가능성 평가) + 디지털 서명(Sigstore)의 3단 결합이 DevSecOps 표준이다.
...
- **📢 섹션 요약 비유**: SBOM은 식품 뒷면의 **성분표**다. 나쁜 재료(취약점)가 발견되면 성분표를 보고 우리 과자가 안전한지 즉시 확인한다.
