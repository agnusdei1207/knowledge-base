...
## 핵심 인사이트 (3줄 요약)
> 1. **본질**: 카오스 엔지니어링(Chaos Engineering)은 멀쩡히 돌아가고 있는 프로덕션(라이브) 시스템에 고의로 서버 종료, 네트워크 지연 등 통제된 재앙(장애)을 주입하여, 시스템의 숨겨진 약점을 폭로하는 과학적 실험 기법이다.
...
1. **카오스 엔지니어링**은 불이 나기 전에 학교에서 다 같이 하는 '소방 대피 모의 훈련'과 같아요!
