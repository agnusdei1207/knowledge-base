...
## 핵심 인사이트 (3줄 요약)
> 1. **본질**: 4대 골든 시그널은 **Latency(응답 시간)·Traffic(요청량)·Errors(에러율)·Saturation(포화도)**이며, Google SRE가 정의한 **"이 4가지만 모니터링하면 서비스 상태를 파악할 수 있다"**는 핵심 지표이다.
> 2. **가치**: 수백 개 메트릭 중 진정으로 중요한 것만 추리면 이 4가지이며, 대시보드·알림을 이 4개에 집중하면 **노이즈 없이 장애를 조기 감지**할 수 있다.
...
### 👶 어린이를 위한 3줄 비유 설명
1. 4대 골든 시그널은 자동차의 **속도계·RPM·경고등·연료 게이지**예요.
