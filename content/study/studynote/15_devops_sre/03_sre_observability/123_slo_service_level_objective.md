...
## 핵심 인사이트 (3줄 요약)
> 1. **본질**: SLO는 **SLI(측정 지표)에 대한 목표 임계치**이며, "가용성 SLI ≥ 99.9%"처럼 정의하여 **서비스 신뢰성의 정량적 기준**을 제공한다.
> 2. **가치**: SLO가 없으면 "서비스가 괜찮은가?"에 대한 판단이 주관적이지만, SLO가 있으면 **Error Budget(100%-SLO)이 남았는가?**로 **피처 개발 vs 안정화의 우선순위를 객관적으로** 결정할 수 있다.
> 3. **판단 포인트**: SLO는 100%로 설정하면 안 되며(혁신 불가), **사용자 기대+비즈니스 목표**에 맞춰 설정해야 한다. SLO ≤ SLA(계약)이어야 내부 목표가 계약보다 엄격하다.
...
3. 덕분에 "얼마나 더 실험(피처)해도 되는지" **숫자로 판단**할 수 있답니다!
