...
문제는 이 지연이 자주 보이지 않는다는 점이다. 전체 요청의 0.5%만 콜드 스타트여도 평균값은 멀쩡해 보일 수 있다. 하지만 사용자가 체감하는 첫 요청, 로그인, 결제, 알림 처리처럼 중요한 구간에서는 그 0.5%가 곧 장애로 받아들여진다. 그래서 콜드 스타트는 성능 이슈라기보다 **관측 가능한 프로비저닝 병목**으로 봐야 한다.
...
기술사 답안에서는 **비용 절감과 지연 허용치의 균형**을 분명히 적는 것이 중요하다. 콜드 스타트는 제거 대상이 아니라, 어느 경로에서 감수하고 어느 경로에서는 선제 비용으로 없앨지 결정해야 하는 신뢰성 설계 요소다.
