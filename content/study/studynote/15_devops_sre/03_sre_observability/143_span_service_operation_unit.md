...
## 핵심 인사이트 (3줄 요약)
> 1. **본질**: Span은 **분산 트레이스의 기본 단위**로, 하나의 서비스 내 **특정 오퍼레이션(HTTP 요청·DB 쿼리·메시지 처리)**의 시작·종료·메타데이터를 기록하며, 부모-자식 관계로 트리를 형성한다.
> 2. **가치**: Span에 **서비스명·오퍼레이션명·상태코드·에러·태그·로그 이벤트**가 포함되어, 각 서비스 구간의 **소요 시간·에러 여부를 정확히** 파악할 수 있다.
