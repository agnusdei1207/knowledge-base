...
- EDFA와 쌍벽을 이루는 또 다른 기술입니다. 특수 에르븀 유리를 달지 않고, 그냥 **수십 km의 일반 광케이블 자체를 증폭기로 써버리는 원리**입니다. 빛과 유리의 분자가 부딪혀 진동(라만 산란)하는 에너지를 이용해 빛을 밀어주는 방식으로, EDFA보다 노이즈가 적어 두 개를 섞어서 최장거리 우주 통신에 씁니다. 향후에는 AI 기반 성능 예측 같은 자동화 흐름과 결합되어 더 정교한 형태로 확장될 가능성이 크다.
