...
> 2. **가치**: 현대 네트워크에서 추가 옵션(Options) 필드는 보안 및 성능상의 이유로 거의 쓰이지 않기 때문에, 사실상 IPv4 헤더의 길이는 **20바이트 고정**이라고 봐도 무방하다.
> 3. **판단 포인트**: 20바이트의 공간은 크게 **[경로 설정(Source/Dest IP)], [단편화(Fragmentation) 조립 정보], [수명(TTL) 및 프로토콜 식별]**의 세 가지 핵심 구역으로 알차게 나뉘어 있다.
