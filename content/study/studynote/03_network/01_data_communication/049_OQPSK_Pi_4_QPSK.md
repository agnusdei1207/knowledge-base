...
> 2. π/4-QPSK는 두 QPSK [[053_성상도_Constellation_Diagram|성상도]]를 45° 교대 사용하여 최대 위상 전이를 135°로 제한 — OQPSK보다 스펙트럼 효율이 높고 비차동(Differential) 복조가 가능해 이동통신(IS-95 이전, DECT, PDC)에서 널리 사용되었다.
> 3. 두 변조 방식 모두 "포락선 변동(Envelope Variation) 최소화"가 핵심 목표 — 위상 전이가 클수록 [[130_signal|신호]] 포락선이 0에 가까워져 전력 증폭기 효율이 급락하므로, 위상 전이 제한이 배터리 수명과 직결된다.
...
> 📢 **섹션 요약 비유**: QPSK 180° 전이 = 전등 [[238_switch_operation_principles|스위치]] 급격히 반전 — [[001_voltage|전압]]이 순간 0이 됨. 증폭기가 0 근처에서 왜곡. OQPSK/π/4-QPSK는 "천천히 돌기"로 [[001_voltage|전압]] 0 방지!
...
> 📢 **섹션 요약 비유**: π/4-QPSK = 45° 엇갈린 두 개의 바퀴 — 홀/짝 심볼마다 다른 [[053_성상도_Constellation_Diagram|성상도]](45° 회전). 최대 이동 135°로 제한. 차동 변조로 기준점 없이도 복조!
