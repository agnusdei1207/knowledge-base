...
노드 장애나 앱 크래시가 잦은 클라우드에서는 이 자동 [[658_ir_recovery|복구]]가 가용성을 지켜 준다.
...
| 요소 | 역할 | 포인트 |
|:---|:---|:---|
| desired replicas | 목표 [[198_pod_kubernetes_minimum_deployment_unit|Pod]] 수 | spec.replicas |
| label selector | 관리 대상 선택 | 정확한 라벨 설계 |
| reconciliation loop | 상태 비교와 조정 | 계속 반복 |
| self-healing | 장애 [[658_ir_recovery|복구]] | 죽은 [[198_pod_kubernetes_minimum_deployment_unit|Pod]] 대체 |

┌───────────────┐ observe ┌────────────────┐
│ ReplicaSet    │──────▶│ [[002_current|current]] Pods   │
│ replicas = 3  │       │ matching label │
└──────┬────────┘       └──────┬─────────┘
       │ reconcile create/delete     │
       ▼                              ▼
  ┌──────────────┐             ┌───────────────┐
  │ controller   │────────────▶│ [[087_process_state_transition|new]] [[198_pod_kubernetes_minimum_deployment_unit|Pod]]       │
  └──────────────┘             └───────────────┘
...
## Ⅲ. 비교 및 연결
| 비교 항목 | ReplicaSet | [[087_deployment_kubernetes_workload_rolling_update|Deployment]] | ReplicationController |
|:---|:---|:---|:---|
| 핵심 기능 | [[198_pod_kubernetes_minimum_deployment_unit|Pod]] 수 유지 | 배포·롤아웃 관리 | [[459_quic_fec_forward_error_correction|초기]] 카운트 유지 |
| 업데이트 | 직접 제어 | [[117_rolling_update_deployment|롤링 업데이트]] 지원 | 제한적 |
...
- [ ] Pod가 사라졌을 때 자동 [[658_ir_recovery|복구]]가 되는지 검증한다.
- [ ] 원하는 수와 실제 수의 차이를 모니터링한다.

- ❌ ReplicaSet을 [[288_version_ihl_tos_total_length|버전]] 배포 도구로 오해하는 것
...
- **📢 섹션 요약 비유**: 개수 유지가 자동 [[658_ir_recovery|복구]]로 이어진다.
...
| 개념 | 연결 포인트 |
|:---|:---|
| ReplicaSet | 원하는 [[198_pod_kubernetes_minimum_deployment_unit|Pod]] 개수를 지킨다. |
| [[087_deployment_kubernetes_workload_rolling_update|Deployment]] | 배포와 롤아웃을 관리한다. |
