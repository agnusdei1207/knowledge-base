...
## 핵심 인사이트 (3줄 요약)
> 1. **본질**: 기본 컨테이너(runc)는 호스트 커널을 공유하므로 **커널 취약점을 통한 컨테이너 탈출(Container Escape)** 위험이 있다. 샌드박싱 런타임은 **커널 접근을 차단하는 추가 격리 계층**을 삽입하여 보안을 강화한다.
> 2. **가치**: gVisor는 **유저 스페이스 커널(Sentry)**로 시스콜을 중간 차단하고, Kata Containers는 **경량 VM 안에 컨테이너를 격리**하여, 멀티테넌트 환경에서 **워크로드 간 완벽 격리**를 달성한다.
> 3. **판단 포인트**: gVisor는 **시스콜 호환성 제한+성능 오버헤드 5~15%**, Kata는 **VM 부팅 지연+메모리 오버헤드**라는 트레이드오프가 있으며, **신뢰할 수 없는 코드(CI Runner, FaaS)** 실행 시 도입을 검토한다.
...
### 👶 어린이를 위한 3줄 비유 설명
1. 보통 컨테이너(runc)는 교실 **칸막이**로 나뉜 거예요. 칸막이를 넘으면 옆 자리를 볼 수 있죠.
