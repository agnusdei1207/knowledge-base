...
> 1. **본질**: Blameless Post-mortem은 [[090_service_kubernetes_network_load_balancing|서비스]] 장애를 특정 개인의 실수로 귀결시키지 않고, 시스템·프로세스·조직 구조의 근본 원인을 찾아 재발 방지 액션으로 전환하는 [[100_sre_site_reliability_engineering_error_budget|SRE]] 문화의 핵심 실천이다.
...
구글 [[100_sre_site_reliability_engineering_error_budget|SRE]] 팀이 체계화한 Blameless Post-mortem은 반대 접근을 취한다: "모든 사람은 그 순간 최선을 다했다. 문제는 사람이 아닌 시스템에 있다." 엔지니어를 비난하는 대신, 그 실수가 발생할 수밖에 없었던 시스템적 원인—불충분한 모니터링, 잘못된 설계, 부족한 문서화—을 찾아 개선한다.
...
| 4 | 포스트모템이 시스템 개선의 핵심 [[005_feedback_loop|피드백 루프]] |
...
**심리적 안전감([[036_psychological_safety|Psychological Safety]])과의 연계**:
- Google의 프로젝트 아리스토텔레스: 고성과 팀의 공통점 1위가 "심리적 안전감"
- 비난 없는 포스트모템은 심리적 안전감 형성의 가장 가시적인 조직 [[130_signal|신호]]
- 리더가 직접 자신의 실수를 포스트모템에 공개하면 문화 전파 속도가 빠름

**기술사 판단 포인트**:
- "누가 실수했는가?"가 아니라 "이 실수를 가능하게 한 시스템의 약점이 무엇인가?"가 올바른 질문이다.
- 액션 아이템이 없거나 "더 조심하기", "교육 실시"처럼 추상적이면 포스트모템의 가치가 없다. 구체적이고 [[395_verification_process_review|검증]] 가능한 기술적 개선이어야 한다.
...
| [[100_sre_site_reliability_engineering_error_budget|SRE]] [[101_error_budget_sre|Error Budget]] 연계 | 포스트모템이 [[101_error_budget_sre|Error Budget]] 소비의 원인 분석 |
...
| 개념 | 연결 포인트 |
|:---|:---|
| [[100_sre_site_reliability_engineering_error_budget|SRE]] [[101_error_budget_sre|Error Budget]] | 포스트모템이 [[101_error_budget_sre|Error Budget]] 소비 원인 분석의 입력 |
| 5 Whys | 근본 원인 분석의 핵심 기법 |
| 심리적 안전감 | Blameless 문화의 토대이자 결과 |
| [[075_incident_management|인시던트 관리]] | 포스트모템은 인시던트 해결 후 필수 후속 활동 |
| [[201_dora_metrics_devops_performance|DORA Metrics]] | [[451_mttr|MTTR]] 감소는 효과적인 포스트모템 문화가 기여 |
| [[751_chaos_engineering|카오스 엔지니어링]] | 포스트모템 학습 결과를 카오스 실험에 반영 |
