...
> 2. 밀러-라빈 테스트(Miller-Rabin Test)는 [[130_probability|확률]]적 소수 판별의 표준 — O(k log²N)의 빠른 복잡도로 대용량 수(256~4096 [[073_bit|비트]])의 소수 여부를 검사하며, k번 반복으로 2^(-2k)의 오류 [[130_probability|확률]]을 달성한다.
> 3. 결정론적 밀러-라빈(Deterministic Miller-Rabin)이 실무의 답 — 특정 범위의 수에 대해 고정된 베이스 집합을 사용하면 오류 없는 결정론적 판별이 가능하며, 2^64 미만 수에 대해 {2,3,5,7,[[308_static_dynamic_nat_pat_port_address_translation|11]],13,17,19,23,29,31,37} 7개면 충분하다.
...
> 📢 **섹션 요약 비유**: 페르마 테스트 = 대략적 소수 [[396_validation|확인]] — 소수면 항상 통과. 단, [[115_carmichael_number|카마이클 수]](가짜 소수)가 항상 통과. 의심스럽지만 완벽하지 않은 검사!
...
> 📢 **섹션 요약 비유**: 밀러-라빈 = 강화된 신분증 검사 — 페르마(기본 신분증 [[396_validation|확인]])에서 발전해 [[115_carmichael_number|카마이클 수]](정교한 위조 신분증)도 잡아냄. k번 반복으로 오류 [[130_probability|확률]] [[489_raid_10_hybrid|10]]^(-9)!
...
> 📢 **섹션 요약 비유**: 소수 판별 선택 = 신분증 검사 강도 — 일반인(코딩테스트): 사진 [[396_validation|확인]](√N). 공항 출국([[110_rsa|RSA]]): 지문+얼굴+[[002_database_definition|데이터베이스]] 대조(밀러-라빈). AKS는 완벽하지만 공항에서도 너무 오래 걸려 안 씀!

---

## Ⅴ. 실무 시나리오 — [[110_rsa|RSA]] 소수 [[087_process_state_transition|생성]]
...
> 📢 **섹션 요약 비유**: [[110_rsa|RSA]] 소수 [[087_process_state_transition|생성]] = 복권 당첨 + 신원 [[396_validation|확인]] — 1024비트 랜덤수(복권) 뽑기 → 빠른 필터(가짜 당첨 70% 제거) → 밀러-라빈 20라운드(진짜 당첨 [[396_validation|확인]]). 평균 240번 시도!
