...
> 2. [[160_knowledge_graph_graphrag_integration|지식 그래프]]가 LLM의 [[275_react_framework|환각]]([[345_llm_foundation_model_hallucination|Hallucination]])을 보완하는 [[642_reliability_mtbf_mttr_mttf_availability|신뢰성]] 있는 지식 기반으로 부상 — 벡터 DB가 의미적 유사성으로 검색한다면, [[160_knowledge_graph_graphrag_integration|지식 그래프]]는 [[369_logic_bomb|논리]]적 추론과 명시적 [[083_relationship_in_er_model|관계]] 탐색으로 더 정확하고 설명 가능한 결과를 제공한다.
> 3. 구글 지식 패널, ChatGPT의 사실 기반 응답, Wikidata, DBpedia 등 현대 검색·AI의 핵심 인프라 — 2012년 구글이 "[[160_knowledge_graph_graphrag_integration|Knowledge Graph]]"를 공식 발표하며 검색을 "키워드 매칭"에서 "개체 이해"로 전환한 것이 패러다임 전환의 분기점이었다.

---

## Ⅰ. [[160_knowledge_graph_graphrag_integration|지식 그래프]] 기본 구조
...
> 📢 **섹션 요약 비유**: [[160_knowledge_graph_graphrag_integration|지식 그래프]] = 사람 [[083_relationship_in_er_model|관계]]도 + 사실 노트 — 사람(개체)과 [[083_relationship_in_er_model|관계]](엣지)를 연결한 네트워크. "래리 [[286_page_frame|페이지]] → 설립 → 구글 → 본사 → 마운틴뷰". 연결 따라가며 새 사실 발견!

---

## Ⅱ. 주요 [[160_knowledge_graph_graphrag_integration|지식 그래프]]
...
> 📢 **섹션 요약 비유**: 주요 [[160_knowledge_graph_graphrag_integration|지식 그래프]] = 세상 백과사전들 — 구글 KG(검색 엔진 전용), Wikidata(오픈 공개), DBpedia(위키피디아 자동 추출). 의료·금융·법률은 [[064_relation_domain|도메인]] 특화 사전!

---

## Ⅲ. [[160_knowledge_graph_graphrag_integration|지식 그래프]] [[278_instruction_tuning|임베딩]]
...
> 📢 **섹션 요약 비유**: KGE = 개체를 별자리처럼 배치 — 비슷한 개체(서울, 도쿄)는 가깝게. [[083_relationship_in_er_model|관계]](수도)는 이동 방향으로 표현. "V(서울) + V(수도) ≈ V(한국)" 처럼 벡터 계산으로 추론!

---

## Ⅳ. [[530_graph_rag|Graph RAG]]
...
> 📢 **섹션 요약 비유**: [[530_graph_rag|Graph RAG]] = 지식 맵 + [[190_ai_llm_requirements_specification|AI]] 안내원 — 벡터 검색(광범위 문서 검색)으로 못 찾을 때, [[160_knowledge_graph_graphrag_integration|지식 그래프]]([[083_relationship_in_er_model|관계]] 지도)로 경로 탐색. "삼성→[[009_semiconductor|반도체]]사업부→CEO" [[369_logic_bomb|논리]]적 탐색!

---

## Ⅴ. 실무 시나리오 — 금융 [[160_knowledge_graph_graphrag_integration|지식 그래프]]
...
> 📢 **섹션 요약 비유**: 금융 [[160_knowledge_graph_graphrag_integration|지식 그래프]] = 자금 세탁 [[083_relationship_in_er_model|관계]]도 — A→B→C→D 간접 송금을 [[613_graph_bfs_memory|그래프 탐색]]으로 발견. 직접 연결 없어도 4홉 경로 추적. 의심 계좌 탐지율 34%→89%!
