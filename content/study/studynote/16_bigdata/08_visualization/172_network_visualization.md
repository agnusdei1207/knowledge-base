...
네트워크 시각화는 관계 데이터를 구조로 읽게 만드는 기술이다. 고객-고객, 서비스-서비스, 단백질-단백질, 계정-계정처럼 연결이 핵심인 데이터는 단순 테이블로 보면 "누가 누구와 연결되었는가"까지만 보이고, **어느 집단이 뭉쳐 있는지**, **어느 노드가 다리를 놓는지**, **어디가 허브인지**는 잘 드러나지 않는다. 이때 그래프 구조를 시각화하면 분석 질문 자체가 달라진다.
...
기술사 답안에서는 **그래프 분석 지표 + 레이아웃 선택 + 대규모 최적화 + 도메인 적합 도구 선택**을 함께 적어야 완성도가 높다. 네트워크 시각화는 단순한 출력이 아니라, 질문 지향적 분석 인터페이스다.
