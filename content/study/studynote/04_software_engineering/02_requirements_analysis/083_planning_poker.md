...
Planning Poker는 사용자 스토리를 상대적으로 산정하기 위한 협업 기법이다. 개인이 먼저 의견을 말하면 첫 발언자의 숫자가 전체를 끌고 가기 쉬운데, 동시 공개 방식은 그런 편향을 줄인다. [[052_wideband_delphi|와이드밴드 델파이]] ([[052_wideband_delphi|Wideband Delphi]]) 계열의 합의 철학을 실무적으로 단순화한 형태라고 볼 수 있다.
...
Story Point는 기간이 아니라 상대적 크기다. 기준 스토리 하나를 잡고 그보다 2배인지, 3배인지, 5배인지 비교하는 식으로 [[289_cqrs_db|쓰기]] 때문에, 시간 추정의 함정인 가짜 정밀성을 피할 수 있다.
...
| [[052_wideband_delphi|Wideband Delphi]] | 체계적이고 정교함 | 절차가 무거움 |
| 관리자 단독 산정 | 빠름 | 편향이 크고 합의가 약함 |
| 시간 기반 산정 | 직관적 | 복잡도와 불확실성을 놓치기 쉬움 |

Planning Poker는 버닝 다운 차트 ([[660_burndown_chart|Burndown Chart]])와 연결될 때 의미가 더 커진다. Story Point가 반복적으로 비슷하게 쌓이면 [[067_sprint_timebox|스프린트]] 예측이 가능해지고, 팀 속도(Velocity)를 이용해 릴리스 계획도 세울 수 있다. 즉, 산정은 계획을 위한 입력값이다.
...
Planning Poker를 잘 쓰려면 [[216_progress_in_synchronization|진행]] 규칙이 분명해야 한다. 먼저 스토리 설명을 충분히 하고, 카드 공개 전에는 토론을 막아야 한다. 공개 후에는 최고값과 최저값을 낸 사람이 근거를 말하고, 그 뒤에만 재투표를 한다. 이렇게 해야 먼저 말한 사람이 전체를 끌고 가는 현상을 막을 수 있다.
...
Planning Poker의 기대효과는 팀 내 이해 차이 축소, 산정 편향 감소, [[067_sprint_timebox|스프린트]] 계획의 예측 가능성 향상이다. 단순히 점수를 정하는 것이 아니라, 스토리의 범위와 위험을 함께 드러내기 때문에 백로그 정제에도 도움이 된다.
...
| 개념 | 연결 포인트 |
| :--- | :--- |
| [[082_story_point_velocity|Story Point]] | 상대적 산정 단위 |
| Anchoring Effect | 편향을 유발하는 첫 숫자 문제 |
| [[052_wideband_delphi|Wideband Delphi]] | 이론적 기반 |
| Fibonacci Sequence | 카드 간격 설계 |
| [[660_burndown_chart|Burndown Chart]] | [[067_sprint_timebox|스프린트]] [[216_progress_in_synchronization|진행]] 추적 |
