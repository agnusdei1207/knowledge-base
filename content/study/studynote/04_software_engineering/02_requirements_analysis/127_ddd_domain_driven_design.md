...
| **Aggregate** | 일관성 경계 | 주문+주문항목 |
| **Repository** | Aggregate 저장소 | OrderRepository |
| **Domain Event** | 도메인 변화 알림 | OrderPlaced |
...
| 개념 | 연결 포인트 |
|:---|:---|
| **Bounded Context** | 도메인 경계 (=MSA 서비스) |
| **Ubiquitous Language** | 도메인 전문가+개발자 공통 용어 |
...
### 👶 어린이를 위한 3줄 비유 설명
1. DDD는 **레고 설명서**처럼 만들 물건(도메인)을 먼저 이해하고 설계하는 거예요.
2. 만드는 사람(개발자)과 주문한 사람(고객)이 **같은 이름(유비쿼터스 언어)**으로 부르면 혼동이 없어요.
3. 큰 레고를 **작은 묶음(Bounded Context)**으로 나누면 여러 팀이 동시에 만들 수 있답니다!
