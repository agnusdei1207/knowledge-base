...
> 1. **본질**: 가치 제안 캔버스 (Value Proposition Canvas, VPC)는 고객이 해결하려는 일과 겪는 고통, 기대하는 이익을 제품의 기능·고통 완화·이익 창출 요소와 정밀하게 맞춰 보는 **적합성 설계 도구**다.
...
가치 제안 캔버스는 고객 측면과 제품 측면을 한 화면에 놓고, 두 영역이 어디서 맞고 어디서 어긋나는지 보는 프레임워크다. 알렉산더 오스터왈더 (Alexander Osterwalder)는 비즈니스 모델 캔버스 (Business Model Canvas, BMC)보다 더 좁고 깊게, 특히 가치 제안과 고객 세그먼트의 접점을 파고들기 위해 이 도구를 제시했다. 즉 VPC는 사업 전체를 요약하는 도구가 아니라, **고객 문제와 제품 해법의 밀착도**를 검토하는 현미경이다.
...
| 영역 | 구성 요소 | 질문 |
| :--- | :--- | :--- |
| 고객 프로필 | Customer Jobs | 고객은 무엇을 해내려 하는가? |
