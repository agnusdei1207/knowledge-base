...
| **깊이 이해** | *   **배경**: 기능이 정상 동작한다고 해서 실운영의 대규모 트래픽을 버틸 수 있는 것은 아님 | "이 개념의 핵심" |
| **비유** | **신차 주행 테스트** | "이 개념의 핵심" |
| **부하(Load)** | 정원 5명을 태우고 시속 100km로 잘 달리는지 테스트 | "교통 분산" |
| **스트레스(Stress)** | 시속 200km로 밟아서 엔진이 언제 퍼지는지, 퍼진 후 다시 시동이 걸리는지 테스트 | "이 개념의 핵심" |
| **소크(Soak)** | 72시간 동안 쉬지 않고 달려서 오일이 새거나(Leak) 타이어가 마모되는지 테스트 | "서비스 분업" |
| **스파이크(Spike)** | 급가속/급제동을 반복하며 시스템의 순간 회복력(Auto-scaling)을 테스트 | "이 개념의 핵심" |
