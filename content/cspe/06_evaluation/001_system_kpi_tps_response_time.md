...
| **작동 원리** | TPS는 `단위 시간당 완료된 트랜잭션 수`, 응답시간은 `요청부터 응답까지의 총 체류 시간` | "핵심 기술 요소" |
| **비유** | **고속도로 톨게이트** | "핵심 기술 요소" |
...
| **본질** | TPS는 **Provider(시스템)** 관점의 처리 한계치, 응답시간(RT)은 **Consumer(사용자)** 관점의 서비스 품질 척도 | "핵심 기술 요소" |
| **가치** | 두 지표의 상관관계 곡선(Performance Curve)을 통해, 시스템의 병목 구간(Bottleneck)과 확장 임계점을 사전 식별 | "성능" |
