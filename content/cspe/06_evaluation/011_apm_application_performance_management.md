...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **한눈에** | 애플리케이션의 응답 시간(RT), 처리량(TPS), 사용자 경험을 실시간으로 모니터링하고, 장애 발생 시 **코드 레벨의 병목 지점을 정밀... | "핵심 기술 요소" |
| **깊이 이해** | *   **배경**: 복잡한 분산/MSA 환경에서는 하나의 요청이 수십 개의 서버와 DB를 거침 | "핵심 기술 요소" |
| **작동 원리** | 사용자 요청이 들어오면 고유한 식별자(**Trace ID**)를 부여함 | "핵심 기술 요소" |
| **비유** | **혈관 조영술** | "핵심 기술 요소" |
| **구체 예시** | 3초가 걸린 결제 요청을 APM으로 까보니, 결제 DB Insert에는 0 | "핵심 기술 요소" |
| **흔한 오해/주의점** | APM 에이전트를 달면 시스템이 무거워진다는 오해 | "핵심 기술 요소" |
