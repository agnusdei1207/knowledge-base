...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **작동 원리** | 전체 체류 시간 $W = W_q + W_s$ | "핵심 기술 요소" |
| **비유** | **은행 창구 업무** | "핵심 기술 요소" |
| **구체 예시** | TPS 100 상황에서 RT가 1초 | "핵심 기술 요소" |
| **흔한 오해/주의점** | 응답시간이 느리다고 무작정 서버(H/W)를 증설하는 것은 안티패턴 | "핵심 기술 요소" |
| **연결 개념** | Queuing Theory, TPS, Bottleneck Analysis, APM(Application Performance Managem... | "성능" |
| **본질** | 응답시간은 시스템 내의 **경합(Contention)**과 **효율(Efficiency)**의 결합 산물 | "자동 품질 검사 라인" |
| **가치** | 지연의 성격을 파악하여, 무의미한 H/W 증설을 방지하고 정확한 지점(Pinpoint)을 타격하는 튜닝 전략을 수립 | "핵심 기술 요소" |
