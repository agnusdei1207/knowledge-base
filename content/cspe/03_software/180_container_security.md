...
- 통제 시점·통제 대상·적합 통제의 정확히 3개 축으로 비교하고 Admission부터 커널 실행까지 연결함
...
### 쉽게 이해하기 (학습용)
- 컨테이너 보안은 잘못된 배포 설정을 먼저 막고 실행 중 커널 접근도 최소 범위로 제한함
...
### 쉽게 이해하기 (학습용)
- OPA는 배포 요청을, Seccomp와 AppArmor는 승인된 컨테이너의 실제 커널 행동을 통제함
...
| 통제 대상 | 사용자 공간에서 커널로 진입하는 시스템 호출 | 파일·capability·네트워크 등 커널 자원 접근 | 이미지·권한·레이블·리소스 같은 배포 객체 구성 |
| 적합 통제 | 불필요한 시스템 호출 제한 | 애플리케이션별 자원 접근 제한 | 클러스터 공통 배포 정책과 위반 메시지 적용 |

> 요약: OPA는 배포 구성을 검증하고 Seccomp·AppArmor는 실행 중 커널 접근을 제한함.

### 쉽게 이해하기 (학습용)
- 배포 전 구성 검사와 실행 중 행동 제한을 겹쳐 적용해야 한 통제가 놓친 위험을 줄일 수 있음
...
### 쉽게 이해하기 (학습용)
- API 승인 계층·런타임 설정·커널 집행·감사 로그가 하나의 통제 경로를 이룸
...
### 쉽게 이해하기 (학습용)
- 배포 요청을 먼저 검사한 뒤 실행 시 각 시스템 호출과 자원 접근을 프로필로 판정함
...
### 쉽게 이해하기 (학습용)
- 정책 거부 건수와 프로필 로드·실행 거부 로그를 함께 봐야 통제 적용 여부를 알 수 있음
...
### 쉽게 이해하기 (학습용)
- 객체가 안전해 보여도 실행 행동은 별도 통제가 필요하므로 두 계층을 함께 운영함
