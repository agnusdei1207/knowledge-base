...
NewSQL은 분산 ACID OLTP DBMS임. 전통 RDBMS는 scale-up 중심이고 NoSQL은 강한 트랜잭션 제약이 있어, 주문·결제·계정처럼 정합성이 필요한 대규모 서비스에 한계가 있음. NewSQL은 SQL 인터페이스, 자동 샤딩, 복제 합의를 통해 분산 환경에서 일관된 트랜잭션을 제공함.
