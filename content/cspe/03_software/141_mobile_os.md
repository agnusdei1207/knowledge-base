...
| 구분 | 내용 |
|---|---|
| 정의 | 스마트폰·태블릿의 앱 lifecycle, sandbox, 전력·메모리, sensor·UI를 관리하는 운영체제 |
...
> 요약: 모바일 OS는 앱 상태에 따라 CPU·메모리·background 실행 권한을 조정하고 앱별 sandbox와 UI·sensor API를 제공함.
...
> 요약: Android는 Zygote의 공통 runtime 상태를 fork하고 ART가 DEX를 해석·JIT·AOT compile해 앱 프로세스를 실행함.
