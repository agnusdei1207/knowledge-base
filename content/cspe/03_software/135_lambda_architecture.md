...
람다 아키텍처는 배치와 실시간 처리를 병행하는 데이터 처리 구조임. 실시간 분석은 초 단위 결과를 제공하지만 late event·중복·장애 복구 문제가 있음. batch layer는 전체 데이터를 재계산해 정확한 view를 만들고, speed layer는 최신 데이터를 보완함.
