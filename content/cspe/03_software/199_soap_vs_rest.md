...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **개요** | 분산 시스템 간의 정보 교환을 위한 통신 프로토콜/아키텍처 스타일 | "핵심 기술 요소" |
| **필요성** | A 회사의 Java 서버와 B 회사의 C++ 서버가 통신해야 함 | "핵심 기술 요소" |
...
| **명사 중심의 자원(Resource)** | 모든 데이터를 URL로 표현함 | "핵심 기술 요소" |
| **동사 중심의 HTTP 메서드** | 데이터를 어떻게 할지는 HTTP의 기본 동작(`GET`(조회), `POST`(생성), `PUT`(수정), `DELETE`(삭제))에 맡겨버림 | "웹의 공용어" |
