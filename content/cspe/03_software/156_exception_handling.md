...
| 콜 스택 언와인딩 | 일치하는 handler를 찾을 때까지 frame을 제거하고 cleanup·destructor 실행 | 자원 정리 경로 |
> 요약: 예외가 throw되면 runtime이 handler를 탐색하고 frame을 unwind하며 cleanup 후 catch 또는 미처리 종료 경로로 이동함.
...
4. 처리 및 후속 실행: 일치하는 catch를 실행하고 finally·resource cleanup 뒤 try-catch 다음 경로로 진행함.
...
- exception type·stack trace·request context를 수집해 반복 원인을 분류하고 rollback·restart·circuit breaker 같은 승인된 복구 절차를 실행함.
