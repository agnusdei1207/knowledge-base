...
| 필요성 | 조건 분기·반복·호출 순서를 표현하고 실행 profile에 따라 branch·loop 비용을 조정함 |
...
> 요약: 제어 구조는 실행 순서를 정의하고 loop 변환은 분기 횟수·연산 종류·memory 접근 순서를 바꿈.
...
4. cache locality 조정: 배열 순회 순서와 loop blocking으로 연속·재사용 memory 접근을 구성함.
> 요약: loop optimization은 branch, instruction 수, SIMD 폭, cache miss를 profile로 측정해 적용함.
