...
- 개요: 저널링은 변경 로그 기반 복구 기법이다.
- 배경: 전원 장애 중 파일 쓰기가 중단되면 메타데이터와 데이터 반영 순서가 어긋나 inode, bitmap, directory entry 불일치가 발생한다.
- 필요성: 저널링은 commit record 기준 replay·discard로 fsck 시간을 줄이고, fsck error 0건과 fsync latency 기준으로 일관성을 관리한다.
