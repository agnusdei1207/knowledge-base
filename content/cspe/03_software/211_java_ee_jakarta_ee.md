...
> 요약: 웹·영속성·트랜잭션·메시징 API와 컨테이너 동작을 표준화한 엔터프라이즈 플랫폼임
...
> 요약: 표준 API로 구현하고 WAR·EAR로 패키징하여 호환되는 애플리케이션 서버에 배포함

## Ⅳ. 문제점
- 전통적인 Full Profile과 애플리케이션 서버는 컨테이너 이미지 크기와 시작 시간 측면에서 MSA 배포 부담이 큼
