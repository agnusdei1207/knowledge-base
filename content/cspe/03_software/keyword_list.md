...
# 시스템·응용 소프트웨어 키워드 목록 (320제)

컴퓨터시스템응용기술사 시험 출제동향 기반으로 엄선한 시스템·응용 소프트웨어 핵심 키워드입니다.

---

## 1. 운영체제 (Operating System) (31개)
1. 프로세스 vs 스레드 (Process vs Thread) [출제:120회]
2. PCB·컨텍스트 스위칭 (PCB Context Switching) [출제:120회]
3. 프로세스 생성·종료·상태 전이 (Process Lifecycle)
4. 프로세스 스케줄링 알고리즘 — FCFS·SJF·RR·MLFQ·CFS (Process Scheduling) [출제:122,129,131,138회]
5. 멀티레벨 피드백 큐 MLFQ (Multilevel Feedback Queue) [출제:138회]
6. CFS 완전 공정 스케줄러 (Completely Fair Scheduler) [출제:138회]
7. 실시간 스케줄링 — Rate Monotonic·EDF (Real-Time Scheduling) [출제:137회]
8. 스레드 스케줄링·스레드 풀 (Thread Scheduling Thread Pool)
9. 교착상태 조건·예방·회피·탐지·복구 (Deadlock) [출제:121,131,132,134,136회]
10. 은행원 알고리즘 (Banker's Algorithm) [출제:124회]
11. 기아·에이징 (Starvation Aging) [출제:138회]
12. 세마포어·뮤텍스·모니터 (Semaphore Mutex Monitor) [출제:125,126,132회]
13. 스핀락 vs 뮤텍스 (Spinlock vs Mutex) [출제:123회]
14. 임계 구역·상호 배제 (Critical Section Mutual Exclusion)
15. 프로세스 스레싱 (Process Thrashing) [출제:129,131회]
16. 가상 메모리·페이징·세그멘테이션 (Virtual Memory) [출제:120,121,125,126회]
17. 페이지 교체 알고리즘 (Page Replacement) [출제:121회]
18. 워킹 셋·페이지 폴트 (Working Set Page Fault) [출제:131회]
19. 다중프로그래밍·다중처리 (Multiprogramming Multiprocessing)
20. 가상화 — Type 1·Type 2 하이퍼바이저 (Virtualization Hypervisor) [출제:128,131,132,137회]
21. 전가상화·반가상화·컨테이너 비교 (Full Para Container Virtualization) [출제:137회]
22. 파일 시스템 — FAT·NTFS·ext4·APFS (File System)
23. 파일 시스템 저널링 (File System Journaling)
24. I/O 관리·디스크 스케줄링 (I/O Management Disk Scheduling)
25. UNIX 커널·쉘·파일시스템 3요소 (UNIX Kernel Shell) [출제:125회]
26. 마이크로커널 vs 모놀리식 커널 (Microkernel vs Monolithic) [출제:138회]
27. 가상 스레드 — Java Project Loom (Virtual Thread) [출제:138회]
28. 비동기 I/O·이벤트 루프 (Async I/O Event Loop) [출제:138회]
29. 리액티브 프로그래밍 (Reactive Programming) [전망]
30. 다중 프로세서 스케줄링 — SQMS·MQMS (Multiprocessor Scheduling SQMS MQMS) [출제:129회]
31. NUMA 인지 스케줄링 (NUMA-aware Scheduling) [전망]

## 2. 소프트웨어 공학 (Software Engineering) (62개)
32. 소프트웨어 개발 생명주기 SDLC (Software Development Lifecycle)
33. 폭포수 모델 vs 애자일 (Waterfall vs Agile) [출제:121회]
34. 애자일 스크럼 (Agile Scrum) [출제:121,134회]
35. SAFe 대규모 애자일 프레임워크 (Scaled Agile Framework) [출제:122,134,137회]
36. XP — 페어 프로그래밍·TDD (Extreme Programming) [출제:129회]
37. 칸반 (Kanban) [전망]
38. 애자일 DORA 메트릭 (DORA Metrics) [출제:124회]
39. 요구사항 분석·명세 (Requirements Analysis)
40. UML 다이어그램 유형 (UML Diagrams)
41. 소프트웨어 아키텍처 패턴 — MVC·MSA·이벤트드리블 (Architecture Patterns) [출제:120회]
42. 마이크로서비스 아키텍처 MSA (Microservice Architecture) [출제:120,121,123,135회]
43. MSA 분해 전략 — 도메인 주도 설계 (MSA Decomposition DDD) [출제:136회]
44. API 게이트웨이 (API Gateway) [출제:120회]
45. 서킷 브레이커 패턴 (Circuit Breaker Pattern) [출제:136회]
46. 서비스 메시 — Istio·Envoy (Service Mesh) [출제:123,136,138회]
47. 이벤트 소싱·CQRS (Event Sourcing CQRS) [출제:121회]
48. Saga 패턴 — 분산 트랜잭션 (Saga Pattern) [출제:121회]
49. 12 팩터 앱 (12 Factor App) [출제:123회]
50. SOLID 원칙 (SOLID Principles) [출제:121,128,132회]
51. 디자인 패턴 — GoF 23종 (Design Patterns GoF)
52. 헥사고날 아키텍처 — 포트·어댑터 (Hexagonal Architecture) [출제:127회]
53. DDD 도메인 주도 설계 (Domain-Driven Design) [출제:127,137회]
54. 바운디드 컨텍스트 (Bounded Context) [출제:137회]
55. 애그리게이트·이벤트 스토밍 (Aggregate Event Storming) [출제:127회]
56. 서버리스 아키텍처 패턴 (Serverless Architecture Pattern) [출제:130,136회]
57. 모놀리식 vs 마이크로서비스 비교 (Monolith vs Microservice) [출제:135회]
58. 형상 관리 — Git·브랜치 전략 (Configuration Management Git) [출제:121회]
59. CI/CD 파이프라인 (CI/CD Pipeline) [출제:120,121회]
60. 지속적 배포 Continuous Deployment (Continuous Deployment) [출제:138회]
61. GitOps (GitOps) [출제:136회]
62. DevOps 파이프라인 (DevOps Pipeline) [출제:120회]
63. DevSecOps (DevSecOps) [출제:128,134,135,136회]
64. 소프트웨어 테스트 — 단위·통합·시스템·인수 (Software Testing) [출제:120회]
65. 테스트 주도 개발 TDD (Test-Driven Development) [출제:129회]
66. 화이트박스·블랙박스 테스트 (White-box Black-box Testing)
67. 테스트 커버리지 — 구문·분기·조건·경로·MC/DC (Test Coverage) [출제:136회]
68. 뮤테이션 테스트 (Mutation Testing) [전망]
69. 알파·베타·인수 테스트 (Alpha Beta Acceptance Testing) [출제:129회]
70. 카나리 배포·블루-그린 배포 (Canary Blue-Green Deployment) [출제:132,138회]
71. 피처 플래그 (Feature Flag) [출제:138회]
72. 소프트웨어 리팩터링·기술부채 (Refactoring Technical Debt) [출제:123,129,130회]
73. 코드 스멜 (Code Smell) [출제:130회]
74. 소프트웨어 기술부채 사분면 (Technical Debt Quadrant) [출제:123회]
75. 정적 분석 SAST (Static Application Security Testing) [출제:128,135회]
76. 동적 분석 DAST (Dynamic Application Security Testing) [출제:128,135회]
77. 소프트웨어 품질 ISO/IEC 25010 (Software Quality ISO 25010) [출제:120,128회]
78. CMMI 프로세스 성숙도 모델 (CMMI Maturity Model) [출제:122,125회]
79. ATAM 아키텍처 트레이드오프 분석 (ATAM) [출제:121,131회]
80. CBAM 비용-편익 분석 (CBAM) [출제:131회]
81. SW 기능점수 FP 측정 (Function Point) [출제:126회]
82. SW 기능점수 간이법·정통법 (FP Estimation Method) [출제:126회]
83. COCOMO 비용 산정 (COCOMO)
84. 소프트웨어 대가산정 (SW Cost Estimation) [출제:135,138회]
85. 아키텍처 결정 기록 ADR (Architecture Decision Record) [출제:133회]
86. 소프트웨어 안전 — GAMAB·ALARP (Software Safety GAMAB ALARP) [출제:128회]
87. 기능 안전 ISO 26262·ASIL (Functional Safety ISO 26262) [출제:134회]
88. ISO 29119 테스트 설계 (ISO 29119 Test Design) [출제:128회]
89. SP 소프트웨어 프로세스 품질 인증 (SP Quality Certification) [출제:134회]
90. AI 코드 생성 — GitHub Copilot (AI Code Generation) [출제:133회]
91. 생성형 AI 기반 DevOps 자동화 (GenAI DevOps Automation) [출제:133회]
92. 플랫폼 엔지니어링 IDP (Platform Engineering IDP) [출제:133,134,135회]
93. 내부 개발자 플랫폼 골든 패스 (Internal Developer Platform Golden Path) [출제:135회]

## 3. 데이터베이스 (Database) (39개)
94. 관계형 데이터베이스 기본 — 릴레이션·키·제약조건 (Relational Database)
95. 트랜잭션 ACID (Transaction ACID) [출제:120,129,131회]
96. 트랜잭션 격리 수준 4단계 (Transaction Isolation Levels) [출제:120,136,138회]
97. Dirty Read·Non-Repeatable Read·Phantom Read (Read Anomalies) [출제:136회]
98. MVCC 다중 버전 동시성 제어 (MVCC) [출제:136회]
99. 락 관리 — 2단계 잠금 프로토콜 (2PL Two-Phase Locking)
...
## 6. 소프트웨어 개발 방법론 및 품질 (Methods & Quality) (52개)
