...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **필요성** | 해커가 Nginx 컨테이너의 취약점을 뚫고 들어옴 | "이 개념의 핵심" |
| **원리** | 리눅스는 약 330여 개의 시스템 콜을 가짐 | "이 개념의 핵심" |
| **예시** | 도커의 기본 Seccomp 프로필은 330개 중 44개의 위험한 시스템 콜(`mount`, `reboot`, `ptrace` 등)을 아예 ... | "이 개념의 핵심" |
| **프로필 생성의 자동화** | Nginx에 딱 맞는 맞춤형 Seccomp 필터를 사람이 300개를 뒤지며 만드는 건 불가능함 | "이 개념의 핵심" |
| **개념/비교형 (Ⅱ·Ⅲ 강조)** | Seccomp(시스템 콜 BPF 필터, 행위 중심)과 AppArmor/SELinux(MAC, 자원 경로 중심)의 동작 원리와 차이점 비교표 작성 | "이 개념의 핵심" |
| **K8s 보안/실무형 (Ⅳ·Ⅴ 강조)** | 컨테이너 런타임 보안 아키텍처 | "이 개념의 핵심" |
| **Seccomp·AppArmor** | Seccomp·AppArmor (리눅스 커널 보안 장벽)의 핵심 개념 | "이 개념의 핵심" |
