...
- 개요: 은행원 알고리즘은 safe state 회피 기법이다.
- 배경: 교착상태 예방은 자원 사용을 과도하게 제한할 수 있어, 최대 요구량을 알고 있는 환경에서는 요청 승인 전 안전 순서를 검사한다.
- 필요성: Available·Max·Allocation·Need 행렬로 unsafe 승인 0건, safety check p95, 자원 이용률 기준을 함께 관리하기 위해 사용한다.
