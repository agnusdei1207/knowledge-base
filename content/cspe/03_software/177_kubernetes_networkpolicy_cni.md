...
- 목적·호출·입력·동작·산출물·실패 상태를 비교하고 Pod 연결과 패킷 판정 흐름으로 연결함
...
|:---|:---|:---|
| 목적 | Pod를 네트워크에 연결하고 해제 | Pod의 허용 통신 범위를 선언 |
| 실행 주체 | 컨테이너 런타임이 CNI 플러그인 호출 | 정책 Controller·네트워크 플러그인이 규칙 반영 |
| 입력 | 네트워크 네임스페이스·컨테이너 ID·구성 JSON | Pod·Namespace selector·IPBlock·Port |
| 핵심 동작 | ADD로 연결하고 DEL로 해제하며 CHECK로 상태 확인 | Ingress·Egress 허용 규칙을 데이터 경로에 적용 |
| 산출물 | Pod IP와 통신 가능한 네트워크 연결 | 선택된 Pod 방향의 허용 흐름 집합 |
| 실패 상태 | PodSandbox 네트워크 생성·정리 실패 | 미지원 구현에서는 정책 객체가 패킷에 영향 없음 |
