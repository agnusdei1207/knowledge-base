...
- 개요: 스핀락과 뮤텍스는 잠금 대기 방식이다.
- 배경: 잠금 실패 시 스핀락은 CPU를 점유하며 반복 확인하고, 뮤텍스는 wait queue로 이동해 scheduler가 다른 task를 실행한다.
- 필요성: 멀티코어 커널과 사용자 스레드는 critical section 10us 이하, context switch 비용, CPU utilization 기준으로 잠금 방식을 선택해야 한다.
