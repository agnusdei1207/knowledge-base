...
## Ⅳ. 문제점
- 서비스별 DB와 네트워크 호출이 늘어나면 분산 트랜잭션·장애 추적·통합 시험 대상이 증가함

## Ⅴ. 개선방안
- Saga와 멱등 이벤트 처리로 상태 전이를 관리하고 분산 추적·로그·메트릭을 서비스 호출 ID로 연결함

## Ⅵ. 전망
- 서버리스와 결합된 극도로 세분화된 나노서비스(Nanoservices)로의 진화
