...
## 한눈에
- **개요**: 동적으로 변하는 서비스 인스턴스의 위치를 찾아 호출하게 하는 메커니즘
- **왜 필요한가**: 컨테이너와 오토스케일링 환경에서는 IP가 계속 바뀌므로 호출자가 고정 주소를 알 수 없음
- **핵심 직관**: 전화번호부가 실시간으로 바뀌며, 호출자는 현재 살아 있는 번호만 받아 쓰는 구조임

## 깊이 이해
- **배경·문제의식**: VM·컨테이너·Kubernetes Pod는 생성·삭제·교체가 잦다. 설정 파일에 IP를 박으면 배포마다 수정되고 장애 인스턴스로 트래픽이 간다.
- **작동 원리**: 서비스 인스턴스가 Registry에 등록하고 heartbeat를 보낸다. 클라이언트 또는 로드 밸런서가 Registry를 조회해 healthy endpoint를 선택한다.
- **비유**: 식당 예약 앱이 영업 중인 지점만 보여주고, 닫힌 지점은 목록에서 자동 제외하는 방식과 같음
- **구체 예시**: Kubernetes Service는 Label Selector로 Pod endpoint를 모으고 CoreDNS가 `orders.default.svc` 이름을 ClusterIP로 해석한다.
- **흔한 오해·주의점**: DNS만 있으면 충분하다고 단정하면 안 된다. TTL, health check, endpoint stale, client cache 정책이 함께 필요함

## 연결 개념
- Load Balancing - 발견된 endpoint 중 대상 선택
- Service Mesh - Sidecar가 발견·라우팅·mTLS 처리
- Kubernetes Service - 클러스터 내부 디스커버리 기본 단위
...
서비스 디스커버리는 서비스명을 실제 인스턴스 주소로 해석하는 기능이다. MSA와 Kubernetes 환경에서는 인스턴스 생성·삭제가 빈번하여 정적 IP 설정이 장애 원인이 된다. 서비스명 기반 호출, health check, TTL 제어가 필요하다.
