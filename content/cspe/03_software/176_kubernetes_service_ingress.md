...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **필요성** | 장바구니 파드가 결제 파드(IP: 10 | "이 개념의 핵심" |
| **1. K8s Service의 4가지 타입** | - **ClusterIP (기본값)**: 클러스터 '내부'에서만 통신 가능한 가상 IP | "팀 협업" |
| **NodePort** | 워커 노드의 껍데기에 구멍(포트 30000~32767)을 뚫어서 외부에서 접속하게 함 | "이 개념의 핵심" |
| **LoadBalancer** | AWS ALB 같은 외부 클라우드의 진짜 로드 밸런서를 자동으로 생성해서 붙여줌 | "교통 분산" |
| **ExternalName** | K8s 내부에서 외부(On-premise DB)의 도메인을 찾을 때 쓰는 DNS 별칭 | "이 개념의 핵심" |
| **2. Ingress (L7 라우팅 대문)** | - LoadBalancer 타입은 서비스 1개당 비싼 AWS 로드 밸런서 1개가 생겨서 돈이 엄청 깨짐 (100개 서비스면 로드밸런서 100개) | "교통 분산" |
| **Ingress Controller의 필수 선택** | 많은 주니어가 `Ingress` YAML 파일만 만들고 "왜 작동 안 하죠?"라고 묻는데, Ingress는 단순한 '안내판(규칙)'일 뿐임 | "경험으로 배우는 프로그램" |
