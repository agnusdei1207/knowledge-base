...
- 비교의 핵심을 메타데이터 모델·파티션·행 변경·증분 처리·유지관리·적합 업무로 제한함
- 세 포맷의 공통 ACID 문구를 반복하지 않고 실제 파일 추적과 갱신 경로가 갈리는 축을 선정함
- 구성요소는 데이터 파일·메타데이터·카탈로그·엔진·유지관리라는 공통 층과 포맷별 구현을 연결함
...
- Reader는 선택한 스냅샷의 메타데이터로 유효 데이터·삭제·변경 파일만 계획함
...
|:---|:---|:---|:---|
| 메타데이터 모델 | `_delta_log` Action과 Checkpoint | Metadata·Snapshot·Manifest 계층 | Timeline·File Group·Index |
| 파티션 관리 | 테이블 파티션 열과 로그 메타데이터 | 숨은 파티셔닝과 명세 진화 | 파티션 경로와 키·파일 그룹 매핑 |
| 행 변경 경로 | MERGE·UPDATE·DELETE가 새 파일과 로그 커밋 생성 | 데이터·위치·동등 삭제 파일과 파일 재작성 | CoW Base 재작성 또는 MoR Log 추가 |
| 증분 처리 | Change Data Feed와 스트림 Source·Sink | Snapshot 범위의 추가 파일 증분 읽기 | Incremental Query와 Timeline 범위 조회 |
| 유지관리 | 파일 압축·Checkpoint·VACUUM | Manifest 병합·파일 재작성·스냅샷 만료 | Compaction·Clustering·Cleaning |
| 적합 업무 | Spark 기반 배치·스트림·CDC 병합 | 다중 엔진·스키마·파티션 진화 | 변경 빈도가 높은 키 기반 Upsert |

> 요약: Delta는 로그 중심, Iceberg는 스냅샷·Manifest 중심, Hudi는 Timeline·파일 그룹 중심으로 테이블 변경을 관리함.
...
| 버전 메타데이터 | JSON Commit·Checkpoint | Metadata·Snapshot·Manifest | Active·Archived Timeline |
...
| 운영 작업 | 파일 압축·Checkpoint·VACUUM | Rewrite·Expire Snapshot | Compaction·Clustering·Cleaning |

> 요약: 세 포맷은 데이터 파일과 버전 메타데이터를 분리하지만 현재 상태 참조와 행 변경 위치를 찾는 구조가 다름.

## Ⅵ. 실무 사례

1. Spark CDC 병합은 Delta MERGE를 적용하고 커밋 충돌·평균 파일 크기를 확인함
2. 다중 엔진 분석은 Iceberg 스냅샷을 공유하고 계획 시간·엔진별 조회 결과를 대사함

## Ⅶ. 결론

- 오픈 테이블 포맷은 연산 엔진·행 변경 빈도·파티션 진화·증분 조회와 유지관리 방식으로 선택해야 함
