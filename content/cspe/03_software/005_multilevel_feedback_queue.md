...
## 한눈에
- **개요**: **MLFQ(Multilevel Feedback Queue)**는 프로세스의 CPU 사용 패턴(타임 퀀텀 소진 여부)을 관찰해 여러 개의 **우선순위 큐** 사이를 동적으로 이동시키는 **선점형 프로세스 스케줄링** 기법이다.
- **왜 필요한가**: 단일 큐 라운드 로빈(RR)은 마우스 클릭(짧은 I/O 바운드 작업)과 동영상 인코딩(긴 CPU 바운드 작업)을 똑같이 대우해 응답성이 떨어진다. 반면 정적 **다단계 큐(MLQ)**는 큐를 나눠 놓긴 했지만 한 번 배정된 큐가 평생 고정돼 융통성이 없다. MLFQ는 "프로세스가 CPU를 쓰는 행동을 보고 우선순위를 동적으로 올리거나 내리자"는 절충안이다.
- **핵심 직관**: 신규 프로세스는 무조건 최상위 큐에서 시작한다. 타임 퀀텀을 다 못 쓰고 스스로 CPU를 반납하면(I/O 요청) 대화형 프로세스로 보고 그대로 두고, 퀀텀을 꽉 채워 강제로 쫓겨나면 CPU 바운드로 보고 하위 큐로 강등한다.

## 핵심 용어 정리 (내부에 등장하는 것들)

| 용어 | 의미 | 비유 |
|:---|:---|:---|
| 스케줄링(Scheduling) | OS가 다음에 CPU를 줄 프로세스를 정하는 정책 전체 — MLFQ가 속한 상위 범주 | 은행 창구에서 다음 손님을 정하는 규칙 |
| 다단계 큐(MLQ, Multilevel Queue) | 우선순위별로 큐를 여러 개 두되, 한 번 배정된 큐가 영구히 고정되는 정적 스케줄링 | 신분이 세습되는 계급 제도 |
| 라운드 로빈(RR) | 같은 큐 안에서 타임 퀀텀만큼씩 순서대로 돌아가며 실행하는 방식 | 발언 시간을 정해 돌아가며 말하기 |
| 타임 퀀텀(Time Quantum) | 한 번 CPU를 잡았을 때 실행할 수 있는 최대 시간 | 발표자에게 주어진 제한 시간 |
| 강등(Demote) | 타임 퀀텀을 다 소진하면 하위 큐로 내려가는 것 | 근무 평가 후 등급 하락 |
| 우선순위 부스트/에이징(Priority Boost, Aging) | 일정 주기(S)마다 모든 프로세스를 최상위 큐로 강제 복귀시키는 규칙 | 정기적인 사면·리셋 |
| 기아(Starvation) | 낮은 우선순위 프로세스가 CPU를 계속 못 받아 무한정 대기하는 현상 | 만년 후순위로 밀리는 대기자 |
| CPU 바운드 / I/O 바운드 | 연산 위주로 CPU를 오래 쓰는 성향 / 입출력 대기가 잦아 CPU를 짧게 쓰는 성향 | 계속 운동하는 사람 / 잠깐 쉬고 다시 오는 사람 |

## 깊이 이해

### 왜 MLFQ가 필요했나 (배경)
- 이상적으로는 최단 작업부터 처리하는 SJF(Shortest Job First)가 평균 대기시간을 최소화하지만, OS는 프로세스가 앞으로 얼마나 CPU를 쓸지 미리 알 수 없어 SJF를 그대로 쓸 수 없다.
- 그래서 MLFQ는 미래를 예측하는 대신 "방금 한 행동"으로 판단하는 경험적(heuristic) 접근을 택했다. 방금 CPU를 꽉 채워 쓴 프로세스는 앞으로도 CPU를 오래 쓸 것(CPU 바운드)으로 보고 하위 큐로 내리고, 방금 CPU를 조금만 쓰고 스스로 반납한 프로세스는 반응성이 중요한 대화형(I/O 바운드)으로 보고 우선순위를 유지한다.

### 작동 원리 5원칙과 큐 강등 과정 — 수치로
1. 상위 큐에 실행 가능한 프로세스가 하나라도 있으면, 하위 큐의 프로세스는 실행되지 않는다(큐 간 절대 우선순위).
2. 같은 큐 안에서는 라운드 로빈으로 번갈아 실행한다.
3. 새로 생성된 프로세스는 무조건 최상위 큐(Queue 0)에서 시작한다.
4. 타임 퀀텀을 전부 소진하면 하위 큐로 강등된다.
5. 일정 주기 S마다 모든 프로세스를 Queue 0으로 복귀시킨다(Priority Boost).
- 예: Queue 0 퀀텀 8ms, Queue 1 퀀텀 16ms, Queue 2는 FCFS(무제한). 마우스 클릭 프로세스는 1ms만 쓰고 I/O를 요청하며 반납하므로 Queue 0에 계속 남아 항상 8ms 이내 응답을 받는다. 반면 인코딩 프로세스는 Queue 0에서 8ms를 다 채워 Queue 1로 강등되고, 거기서도 16ms를 다 채워 Queue 2로 떨어진다.

### 왜 하위 큐일수록 타임 퀀텀이 커지는가
- 하위 큐에 있다는 것은 이미 "오래 걸리는 작업"으로 판정됐다는 뜻이다. 이런 프로세스를 짧은 퀀텀으로 자주 스위칭하면 컨텍스트 스위칭 오버헤드만 커진다. 그래서 퀀텀을 8ms → 16ms → 32ms처럼 2배씩 늘려, 우선순위는 낮추되 한 번 잡았을 때는 길게 실행하게 해 스위칭 횟수를 줄인다.

### 기아와 에이징 — 수치로
- 상위 큐에 I/O 요청 프로세스가 초 단위로 계속 몰리면, 하위 큐의 CPU 바운드 프로세스는 규칙 1(큐 간 절대 우선순위) 때문에 영원히 CPU를 못 받는 기아 상태에 빠질 수 있다.
- 해결책이 규칙 5의 에이징이다. 예를 들어 S=50ms로 설정하면, 아무리 하위 큐에 오래 갇혀 있던 프로세스라도 최소 50ms마다 한 번은 최상위 큐로 복귀해 CPU를 받을 기회를 보장받는다.

### 스케줄러 속이기(Gaming) 방지
- 어떤 프로세스가 퀀텀 8ms 중 7.9ms만 쓰고 일부러 의미 없는 I/O를 호출해 강등을 피하려 할 수 있다. 이를 막기 위해 커널은 큐에 머무는 동안 사용한 CPU 시간을 누적 회계(accounting)한다. 찔끔찔끔 나눠 썼더라도 누적 사용 시간이 8ms를 넘기면 강등시킨다.

### 비유
- 클럽 입장 줄 세우기와 같다. 처음 온 손님은 VIP 줄(최상위 큐)로 들여보내 잠깐 구경만 하고 나가면(I/O) 계속 VIP로 대우한다. 안에서 자리 잡고 CPU를 독식하면 다음번엔 일반 줄(하위 큐)로 보내고, 그래도 계속 버티면 맨 뒤 대기 줄(FCFS)로 보낸다.

## 연결 개념
- 다단계 큐(MLQ) — MLFQ가 극복한 선행 개념(큐 배정이 영구 고정되는 정적 스케줄링)
- 기아(Starvation)·에이징(Aging) — MLFQ 5번 규칙(Priority Boost)의 이론적 근거
- CFS(완전 공정 스케줄러) — MLFQ의 파라미터 튜닝 문제(큐 개수·퀀텀 크기·부스트 주기)를 단일 수식(vruntime)으로 대체한 후속 진화형 (006 참고)
...
| 구분 | MLFQ (과거 UNIX의 표준) | CFS (현대 Linux 커널의 표준) |
|:---|:---|:---|
