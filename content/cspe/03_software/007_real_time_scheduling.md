...
- 개요: 실시간 스케줄링은 deadline 기반 CPU 배정이다.
- 배경: 일반 스케줄링은 평균 응답시간과 공정성을 다루지만, 제어 시스템은 WCET와 deadline을 기준으로 task 실행 순서를 결정한다.
- 필요성: 자율주행·산업제어·금융 시세 처리는 deadline miss rate 0건 또는 지정 상한, jitter, WCET 분석 기준이 필요하다.
