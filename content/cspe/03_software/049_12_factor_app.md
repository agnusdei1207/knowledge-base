...
- 개요: 클라우드 운영형 앱 설계 원칙
- 배경: 컨테이너와 오토스케일 환경에서는 서버 로컬 상태, 수작업 설정, 환경별 빌드 차이가 배포 실패와 복구 지연을 유발함.
- 필요성: Config 분리, stateless process, backing service, log stream 원칙으로 배포 재현성, scale-out 시간, rollback 시간을 관리해야 함.
