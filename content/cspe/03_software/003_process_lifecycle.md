...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **정의** | 프로세스가 생성(New)되어 준비(Ready), 실행(Running), 대기(Waiting/Blocked) 상태를 거쳐 종료(Termina... | "화장실 잠금" |
| **왜 중요한가** | OS 스케줄러가 '누구에게 CPU를 줄 것인가'를 결정하기 위한 가장 기초적인 판단 근거가 되기 때문임 | "두뇌" |
| **5단계 상태 모델** | - **New**: 프로그램이 메모리에 적재되어 PCB를 할당받은 단계 | "핵심 기술 요소" |
| **Ready** | CPU만 주어지면 즉시 실행 가능한 상태 | "두뇌" |
| **Running** | 실제 CPU를 점유하여 명령어를 실행 중인 상태 | "두뇌" |
| **Waiting (Blocked)** | I/O 작업 완료 등 특정 이벤트가 발생할 때까지 CPU를 반납하고 기다리는 상태 | "두뇌" |
| **Terminated** | 실행이 완료되어 자원을 반납 중인 상태 | "내선↔외선 변환" |
