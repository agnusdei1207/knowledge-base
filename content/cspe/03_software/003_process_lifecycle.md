...
- 개요: 프로세스 생명주기는 상태 전이 모델이다.
- 배경: 운영체제는 new, ready, running, waiting, terminated queue로 CPU 배정 대상과 I/O 대기 대상을 분리한다.
- 필요성: 상태 추적은 zombie 0건, PID 고갈 방지, CPU idle 시간, p95 응답시간 기준으로 생성·종료 자원 회수를 검증하게 한다.
