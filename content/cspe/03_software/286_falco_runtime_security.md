...
- 개요: Falco는 컨테이너 런타임 행위 탐지 도구이다.
- 배경: 컨테이너 보안은 이미지 빌드 시점 점검만으로 끝나지 않으며 실행 중 권한 상승과 민감 파일 접근을 탐지해야 한다.
- 필요성: system call 이벤트를 rule로 평가해 침해 징후를 경보화하고 대응 절차와 연결해야 한다.
