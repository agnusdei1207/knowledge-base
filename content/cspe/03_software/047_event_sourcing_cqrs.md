...
- 개요: 변경 이력과 조회 모델 분리
- 배경: 복잡한 업무 시스템은 현재 값뿐 아니라 변경 근거, 감사 추적, 재처리가 필요하고, 쓰기 모델과 읽기 모델의 트랜잭션·조회 요구가 다름.
- 필요성: Event Store, Command Model, Read Model, Projection으로 감사 로그와 재처리 기준을 확보하고 eventual consistency lag를 측정해야 함.
