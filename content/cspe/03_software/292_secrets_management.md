...
비밀 관리는 자격증명의 저장·조회·교체·폐기를 중앙 통제하는 체계이다. API 키와 DB 비밀번호가 코드·이미지·로그에 남으면 권한 탈취와 내부 확산이 발생한다. Vault·AWS Secrets Manager와 KMS를 결합해 최소 권한, TTL, rotation, 감사 로그를 적용해야 한다.
