...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **필요성** | 개발자가 `db_password="SuperSecret123"`라고 깃헙에 올림 | "이 개념의 핵심" |
| **동적 비밀** | 서버가 Vault에게 "나 DB 쓸래" 하면, Vault가 DB 관리자에게 지시해서 **지금 즉시 '계정=app_482, 비번=xyz987... | "이 개념의 핵심" |
| **Sealing / Unsealing 메커니즘** | 만약 Vault 서버 자체가 통째로 도난당하면? Vault는 기본적으로 잠겨있음(Sealed) | "이 개념의 핵심" |
| **개념/기능 설명형 (Ⅱ·Ⅲ 강조)** | 시크릿 하드코딩의 위험성 | "이 개념의 핵심" |
| **클라우드/K8s 보안형 (Ⅳ·Ⅴ 강조)** | Kubernetes Native Secret의 보안적 한계(Base64) 지적 | "이 개념의 핵심" |
| **비밀 관리** | 비밀 관리 — Vault·AWS Secrets Manager (Secrets Mgt)의 핵심 개념 | "이 개념의 핵심" |
| **'초정밀 디지털 금고'** | . 모든 비밀번호를 강력히 암호화해서 저장하고, 접근 권한을 엄격하게 통제하며, 누가 언제 금고를 열어봤는지 감시함 | "이 개념의 핵심" |
