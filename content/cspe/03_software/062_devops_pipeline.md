...
- 개요: 변경 전달 자동화 체계
- 배경: 릴리스 주기가 짧아지면서 수동 빌드, 환경별 스크립트, 운영 이관 문서만으로는 변경 실패율과 배포 누락을 추적하기 어렵다.
- 필요성: CI/CD 파이프라인은 build-test-scan-deploy gate와 DORA 지표(배포 빈도, 리드타임, 변경 실패율, MTTR)로 전달 흐름을 측정한다.
