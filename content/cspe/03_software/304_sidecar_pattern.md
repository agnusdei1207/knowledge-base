...
## 한눈에
- **개요**: 사이드카 패턴은 **클라우드 네이티브 MSA**에서 **횡단 관심사(Cross-cutting Concern)**를 분리하는 **컨테이너 배치 패턴**으로, 하나의 Kubernetes Pod 안에 애플리케이션 컨테이너와 보조(Sidecar) 컨테이너를 함께 두어 통신·보안·관측 기능을 대행시킨다.
- **왜 필요한가**: 인증서 갱신, 로그 수집, 프록시, 설정 동기화 같은 횡단 기능을 각 서비스 코드에 넣으면 언어별 중복과 배포 위험이 증가한다.
- **핵심 직관**: 오토바이 옆 사이드카처럼 본체(App)는 업무를 수행하고, 옆 보조 장치(Sidecar)가 관측·보안·통신 기능을 맡는다.

## 핵심 용어 정리 (내부에 등장하는 것들)

| 용어 | 의미 | 비유 |
|:---|:---|:---|
| Kubernetes Pod | 네트워크·스토리지 네임스페이스를 공유하는 컨테이너 묶음 — Sidecar가 배치되는 단위 | 한 방을 같이 쓰는 두 룸메이트 |
| 횡단 관심사(Cross-cutting Concern) | 로깅·인증·통신처럼 여러 서비스에 공통으로 필요한 기능(도메인 로직 아님) | 모든 가게에 공통으로 필요한 전기·수도 |
| Sidecar Container | 애플리케이션 옆에서 통신·보안·관측을 대행하는 보조 컨테이너 | 오토바이 옆 사이드카 |
| Service Mesh | 여러 Sidecar(프록시)가 모여 만드는 통신 제어망 — Sidecar의 집합체 | 여러 사이드카가 모여 만든 교통 관제망 |
| Data Plane / Control Plane | 실제 트래픽 처리(Data)와 정책 배포(Control)의 분리 | 현장 순찰(Data)과 상황실 지휘(Control) |
| mTLS (상호 TLS) | 양쪽이 서로의 인증서를 검증하는 암호화 통신 | 양쪽 다 신분증을 확인하는 출입 절차 |
| iptables/eBPF | 트래픽을 Sidecar로 가로채는(intercept) 커널 수준 기법 | 도로에 검문소를 세워 모든 차를 우회시킴 |
| readiness probe | 컨테이너가 트래픽을 받을 준비가 됐는지 Kubernetes가 확인하는 헬스체크 | 오픈 준비 확인 후 손님 입장 허용 |

## 깊이 이해

### 왜 필요했나 (배경 — 언어별 중복 문제)
- MSA는 서비스가 늘수록 Java, Go, Node.js처럼 언어가 혼재한다. 로깅, mTLS, Retry 같은 공통 기능을 언어별 SDK로 각각 구현하면, 정책 하나를 바꿀 때마다(예: Retry 횟수를 2회→1회로 변경) 모든 서비스를 재빌드·재배포해야 한다. 서비스가 50개면 50번의 변경 작업이 필요하다.
- Sidecar는 이 공통 기능을 언어에 무관한 별도 컨테이너로 분리해, 정책 변경 시 Sidecar 이미지 하나만 교체하면 되게 만든다.

### 작동 원리 (같은 Pod, 같은 lifecycle)
- Kubernetes Pod는 네트워크 네임스페이스(같은 IP, localhost)와 볼륨을 공유하는 컨테이너 묶음이다. App 컨테이너와 Sidecar 컨테이너를 같은 Pod에 넣으면 둘은 localhost로 통신하고 생명주기(시작·종료)를 함께한다.
- 트래픽 흐름: 외부 요청 → Sidecar(Envoy 등)가 iptables/eBPF로 먼저 가로챔 → mTLS 복호화·인증·로깅 → App 컨테이너로 전달 → 응답도 같은 경로로 역순 처리.
- 수치 예: Envoy sidecar를 거치면 프록시 한 hop이 추가되어 지연이 늘어난다. 목표는 이 오버헤드를 p95 5ms 이하로 관리하는 것 — 원래 50ms 걸리던 호출이 sidecar를 거쳐도 55ms를 넘지 않아야 한다는 뜻이다.

### 자원 비용을 수치로 이해하기
- Sidecar도 컨테이너이므로 Pod마다 CPU·메모리를 추가로 먹는다. 예: Sidecar 하나에 CPU 50m(0.05 core), Memory 64Mi를 할당하면, 서비스가 200개면 sidecar만으로 CPU 10 core, Memory 12.8GB가 추가로 필요하다. 이것이 "공짜 추상화가 아니다"라는 말의 실체다.
- 그래서 노드 CPU 사용률 70% 이하 같은 기준으로 sidecar 도입 전후 자원 여유를 반드시 비교해야 한다.

### 판별 원리 — SDK 내장 방식과 무엇이 다른가
- SDK 내장 방식: 언어별 라이브러리를 코드에 import → 언어마다 별도 구현, 정책 변경 시 재빌드 필요.
- Sidecar 방식: 별도 컨테이너로 배포 → 언어 무관, 정책 변경 시 Sidecar 이미지만 교체.
- 판별 기준: "서비스 언어가 3개 이상 혼재하고, mTLS·Retry 같은 정책을 중앙에서 일관 적용해야 하는가?" → 그렇다면 Sidecar, 아니라면(언어 단일, 초저지연 요구) SDK 또는 eBPF 기반 sidecar-less 방식을 검토한다.

### 비유와 흔한 오해
- 비유: 오토바이 본체(App)는 목적지로 달리는 일에 집중하고, 사이드카는 짐(로그)·무전기(관측)·보험서류(인증서) 같은 부가 장비를 싣는다. 본체 설계를 바꾸지 않고도 사이드카만 교체하면 새 장비를 붙일 수 있다.
- 오해 1: Sidecar는 별도 마이크로서비스가 아니다 — 독립적으로 스케일링되지 않고 항상 App과 1:1로 같은 Pod에서 생사를 함께한다(Pod가 죽으면 둘 다 죽는다).
- 오해 2: Sidecar가 죽으면 App도 트래픽을 못 받을 수 있다(장애 경로 추가). 그래서 fail-open/fail-close 정책과 readiness probe로 기동 순서(Sidecar가 먼저 준비된 후 App에 트래픽 유입)를 관리해야 한다.

## 연결 개념
- Service Mesh (Sidecar 프록시들이 모여 이루는 상위 구조 — Sidecar는 Mesh의 Data Plane)
- Ambassador Pattern (외부 통신만 대리하는 특수한 형태의 Sidecar)
- Kubernetes Pod (Sidecar가 배치되는 배포 단위)
