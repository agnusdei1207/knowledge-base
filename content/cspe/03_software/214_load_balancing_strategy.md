...
## 한눈에
- **개요**: 여러 서버에 요청을 분산해 지연, 장애, 자원 사용률을 통제하는 전략
- **왜 필요한가**: 하나의 서버가 모든 요청을 받으면 CPU 90% 이상, 큐 대기, 장애 단일점이 발생함
- **핵심 직관**: 입구에서 손님 수, 처리 속도, 서버 상태를 보고 가장 알맞은 계산대로 보내는 방식임

## 깊이 이해
- **배경·문제의식**: 웹·API 서비스는 시간대별 트래픽 편차가 크다. 로드 밸런서는 서버 추가만이 아니라 health check, session, zone 장애, 배포 전환을 다룬다.
- **작동 원리**: L4는 IP·Port 기준으로 TCP/UDP 연결을 분산하고, L7은 HTTP Host·Path·Header·Cookie 기준으로 요청을 라우팅한다.
- **비유**: Round Robin은 번호표 순서 배정, Least Connection은 손님이 적은 창구 배정, Weighted는 숙련 직원에게 더 많은 손님을 배정하는 방식임
- **구체 예시**: 4대 서버 중 vCPU 8대 서버 2대와 vCPU 4대 서버 2대가 있으면 가중치 2:2:1:1로 트래픽을 배분한다.
- **흔한 오해·주의점**: 로드 밸런싱은 트래픽 분배만이 아니다. 장애 탐지 지연, 세션 고정, retry storm, cross-zone 비용도 함께 설계해야 함

## 연결 개념
- Auto Scaling - 부하 지표 기반 인스턴스 수 조절
- Circuit Breaker - 장애 서버 호출 차단
- Service Discovery - 분산 대상 endpoint 제공
...
로드 밸런싱은 요청을 여러 서버로 분산하는 트래픽 제어 기법이다. 트래픽 집중은 큐 대기와 장애 단일점을 만들며, 서버 증설만으로 해결되지 않는다. health check, 세션, 재시도, 배포 전략을 함께 설계해야 한다.
