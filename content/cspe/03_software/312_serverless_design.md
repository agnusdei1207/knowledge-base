...
## Ⅰ. 개요
- 정의: 클라우드 제공자가 실행 인프라를 관리하고 이벤트 발생 시 함수와 관리형 백엔드 서비스를 호출하는 구조임
...
## Ⅳ. 문제점
- 유휴 함수의 실행 환경 생성으로 콜드 스타트 지연이 발생하고 함수 연쇄 호출은 전체 흐름 추적을 어렵게 함

## Ⅴ. 개선방안
- 프로비저닝된 동시성으로 시작 지연을 관리하고 상관관계 ID 기반 분산 추적으로 함수 호출을 연결함

## Ⅵ. 전망
- 엣지(Edge) 기반 서버리스 컴퓨팅 확대로 사용자 인접 초저지연 서비스 실현
