...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **필요성** | 모든 트랜잭션을 한 줄로 세워 순서대로 처리(직렬화)하면 동시성 에러는 없지만 쇼핑몰 결제가 1시간씩 걸림 | "핵심 기술 요소" |
| **트랜잭션 격리 4단계 (Level 0 ~ 3)** | 1. **Read Uncommitted (Lv | "핵심 기술 요소" |
| **Read Committed (Lv.1) - [Oracle 기본값]** | - 커밋이 완료된 확실한 데이터만 읽음 | "핵심 기술 요소" |
| **Serializable (Lv.3)** | - 완벽한 직렬화. 내가 데이터를 읽고 있으면 남들은 쳐다보지도 못함(Table/Range Lock) | "화장실 잠금" |
| **MySQL (InnoDB)의 마법 (MVCC)** | MySQL은 기본값이 Repeatable Read임 | "데이터 질의 언어" |
| **Lost Update (갱신 손실) 방어** | 두 트랜잭션이 동시에 같은 게시글 조회수를 올릴 때, 값이 덮어씌워져 1만 올라가는 치명적 현상 | "건물 관리실" |
| **트랜잭션 격리 수준** | 트랜잭션 격리 수준 (Isolation Level)의 핵심 개념 | "핵심 기술 요소" |
