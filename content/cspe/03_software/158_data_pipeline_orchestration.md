...
- Executor는 실행 가능 Task를 로컬 프로세스·분산 Worker·Pod 같은 실행 자원에 배치함
...
- 비교표는 Cron과 의존성·상태·재시도·자원 조정·관찰·적합 업무를 같은 기준에서 대비함
...
- Dag가 Task의 선후 관계와 Trigger Rule을 선언하고 처리 코드는 Operator·Task로 분리함
...
| 실행 상태 | 프로세스 종료 코드·외부 로그 중심 | Dag Run·Task Instance 상태를 메타DB에 저장 |
| 실패 복구 | 스크립트 단위 재실행·수동 구간 지정 | 실패 Task 재시도와 선택 Task·구간 재실행 |
| 자원 조정 | OS 스케줄과 개별 스크립트 설정 | Executor·Queue·Pool·동시성으로 배치 |
| 관찰 범위 | 작업별 로그를 별도 조회 | Dag 그래프·실행 이력·로그·지연을 연결 |
| 적합 조건 | 독립된 단일 주기 작업 | 다단계 의존성·재시도·Backfill이 필요한 파이프라인 |

> 요약: Airflow는 Dag 의존성과 Task Instance 상태를 기준으로 실행·재시도·Backfill·자원 배치를 조정함.
...
- Airflow는 Task 의존성·실패 복구·데이터 구간·실행 자원과 Scheduler·Metadata DB 운영 규모로 적용해야 함
