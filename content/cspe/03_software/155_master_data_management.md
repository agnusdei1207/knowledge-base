...
- 비교표는 Registry·Consolidation·Coexistence·Transaction Hub를 기준 저장·갱신·배포 방식·주요 통제로 대비함
...
|:---|:---|:---|:---|:---|
| 기준 저장·갱신 | 원천 키·식별 링크 중심 저장, 원천 시스템이 속성 소유 | Golden Record를 분석용으로 저장, 원천에서 Hub로 단방향 수집 | Hub·원천에 기준값을 함께 저장, 규칙에 따라 상호 갱신 | Hub가 운영 기준 레코드를 직접 저장, 업무 응용이 Hub 기준으로 갱신 |
| 배포 방식 | 조회 시 원천 위치 연결 | 배치·증분으로 통합 결과 제공 | 변경 이벤트·서비스로 양방향 배포 | API·서비스로 운영 트랜잭션 제공 |
| 주요 통제 | 중복 식별과 Crosswalk | 매칭·병합·분석 일관성 | 속성 소유권과 충돌 조정 | Hub 가용성·트랜잭션·권한 통제 |
...
3. **후보 매칭**: 후보 검색 키로 비교 대상을 줄이고 규칙·유사도 점수로 동일 개체를 판정함
...
- MDM은 개체 식별 정확도·속성 소유권·Golden Record 갱신 방식·배포 지연과 Steward 검토 범위를 기준으로 설계해야 함
