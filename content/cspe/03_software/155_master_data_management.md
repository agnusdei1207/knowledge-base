...
### 쉽게 이해하기 (학습용)
- 시스템마다 다른 고객·상품 기록을 같은 개체로 찾아 신뢰할 기준 레코드로 통합하는 관리임
...
### 쉽게 이해하기 (학습용)
- 자동 매칭만으로 끝나지 않고 속성별 원천 우선순위와 애매한 후보의 사람 검토가 필요함
...
### 쉽게 이해하기 (학습용)
- 기준 레코드를 어디에 저장·갱신하고 원천에 어떻게 배포할지에 따라 네 구현 방식이 나뉨
...
### 쉽게 이해하기 (학습용)
- 원천 연결기, 표준화·매칭·속성 선택기, 기준 저장소와 관리자 검토 흐름으로 구성됨
...
### 쉽게 이해하기 (학습용)
- 이름·주소를 표준화하고 같은 고객을 찾아 원천별 우선순위로 기준값을 선택함
...
### 쉽게 이해하기 (학습용)
- 고객 통합에서 중복률·오병합·미병합·관리자 대기·원천 배포 지연을 확인함

## Ⅶ. 결론

- MDM은 개체 식별 정확도·속성 소유권·Golden Record 갱신 방식·배포 지연과 Steward 검토 범위를 기준으로 설계해야 함
- Golden Record를 목표가 아닌 운영 결과로 보고 매칭 임계값·병합 취소·속성 계보와 배포 실패를 지속 관리해야 함

### 쉽게 이해하기 (학습용)
- 기준 고객 한 건을 만드는 것보다 잘못 합쳤을 때 되돌리고 값의 출처를 설명할 수 있어야 함
