...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **Reactive Streams의 핵심 인터페이스 (표준 SPI)** | 1. `Publisher`: 무한 또는 유한한 데이터를 생성하여 스트림으로 통지 | "핵심 기술 요소" |
| **Push-Pull 하이브리드** | 데이터는 생산자로부터 밀려오지만(Push), 소비자가 허락한 양만큼만(Pull) 들어오는 형태 | "핵심 기술 요소" |
...
| **리액티브 프로그래밍** | 리액티브 프로그래밍 (Reactive Programming)의 핵심 개념 | "칠판" |
| **Pull 기반 동적 요청** | Subscriber가 10개 처리 후 10개를 다시 `request` | "핵심 기술 요소" |
| **Drop (버리기)** | 큐 버퍼 초과 시 새로 들어오는 데이터를 무시 | "재해 복구" |
