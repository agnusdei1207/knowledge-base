...
| 구분 | 내용 |
|------|------|
| 출제 의도 | Stored Procedure의 호출 구조와 Trigger의 이벤트 기반 실행 역할 구분 |
...
| 구성요소 | 설명 | 비유 |
|----------|------|------|
| Stored Proc | SQL과 제어문을 DB 서버에서 실행하는 저장 모듈 | 밀키트 |
...
> 요약: 호출 또는 데이터 변경 이벤트에 따라 DB 서버의 저장 로직을 실행함

## Ⅳ. 문제점
- 서버 CPU 부하 가중 및 애플리케이션-DB 간 로직 분산에 따른 관리 어려움
- 트리거 연쇄 호출 시 실행 순서와 데이터 변경 원인 추적이 어려움
