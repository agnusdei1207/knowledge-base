...
- **배경/필요성**: 가상 메모리 환경에서는 프로세스마다 필요한 실제 메모리 규모가 시간에 따라 달라지므로, working set 기준 배분으로 스래싱과 처리량 저하를 통제해야 함

## Ⅱ. 특징

- working set은 locality를 정량적으로 표현하는 운영 지표임
- page fault는 메모리 부족과 참조 패턴 변화의 직접 신호가 됨
- page fault가 반복되면 프로세스 실행보다 페이지 적재 I/O 대기 시간이 증가함
...
- working set은 필요한 메모리 규모, page fault는 부족 또는 참조 변화의 결과 신호로 역할을 분리함
- 시간 창은 평가어가 아니라 설정값에 따라 추정 working set이 달라지는 한계로 표현함
