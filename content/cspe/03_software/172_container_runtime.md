...
- containerd·CRI-O를 설계 범위·CRI 연계·적합 환경의 정확히 3개 축으로 비교하고 Pod 실행 절차로 연결함
...
### 쉽게 이해하기 (학습용)
- 컨테이너 런타임은 쿠버네티스의 요청을 이미지 준비와 실제 프로세스 실행으로 바꾸는 노드 구성요소임
...
### 쉽게 이해하기 (학습용)
- kubelet·CRI 런타임·OCI 실행기의 인터페이스와 자원 관리 설정이 서로 맞아야 Pod가 실행됨

## Ⅲ. 종류 및 비교

| 판단 기준 | containerd | CRI-O |
|:---|:---|:---|
| 설계 범위 | 여러 클라이언트가 사용할 수 있는 범용 런타임 | Kubernetes CRI 제공에 집중한 런타임 |
| CRI·실행 연계 | 내장 CRI 플러그인과 shim으로 OCI 실행기 호출 | CRI 서버가 이미지 계층과 OCI 실행기·모니터 연결 |
| 적합 환경 | 범용 런타임과 Kubernetes CRI를 함께 운영 | Kubernetes 전용의 단순한 CRI·OCI 경계가 필요 |

> 요약: containerd는 범용 런타임에 CRI를 통합하고 CRI-O는 Kubernetes CRI 제공에 집중함.

### 쉽게 이해하기 (학습용)
- containerd는 범용 도구 상자, CRI-O는 쿠버네티스 Pod 실행에 집중한 도구 상자에 가까움
...
### 쉽게 이해하기 (학습용)
- 런타임은 이미지·격리 공간·네트워크를 준비하고 하위 실행기에 프로세스 생성을 맡김
...
### 쉽게 이해하기 (학습용)
- kubelet 요청이 표준 CRI를 지나 실제 실행으로 바뀌고 결과 상태가 다시 kubelet에 보고됨
...
### 쉽게 이해하기 (학습용)
- 제품 이름보다 CRI 호환성·cgroup 설정·샌드박스 실패 원인을 운영 지표로 확인함
...
### 쉽게 이해하기 (학습용)
- 쿠버네티스 전용 범위인지 범용 실행까지 필요한지에 따라 런타임을 선택함
