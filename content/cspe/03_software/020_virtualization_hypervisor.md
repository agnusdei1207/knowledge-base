...
- 개요: 하이퍼바이저는 VM 자원 중재 계층이다.
- 배경: 물리 서버 1대에 OS와 애플리케이션을 1:1 배치하면 평균 CPU 사용률과 배포 표준화 측면에서 자원 활용 한계가 생긴다.
- 필요성: 기업 시스템은 CPU ready 5% 이하, ballooning 0건, HA failover time 기준으로 서버 통합·격리·live migration을 관리한다.
