...
키셋 페이지네이션은 마지막 정렬 키 이후 데이터를 조회하는 방식임. 대량 목록에서 OFFSET은 페이지 후반으로 갈수록 스캔 비용이 증가하고, 동시 삽입·삭제 시 중복·누락이 생김. 커서 방식은 정렬 키 기반 range scan으로 API 응답 지연과 목록 일관성 문제를 줄임.
