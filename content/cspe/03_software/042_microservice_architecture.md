...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **개요** | 클라우드 네이티브(Cloud Native) 환경의 핵심 설계 철학으로, 단일 책임 원칙(SRP)에 따라 비즈니스 역량별로 분리된 서비스들의 조합 | "전기처럼 빌려 쓰는 컴퓨팅" |
| **필요성** | 기존 모놀리식 구조에서는 코드 한 줄을 수정해도 전체 시스템을 빌드/테스트/배포해야 하므로 타임투마켓(Time to Market)이 극도로 느림 | "이 개념의 핵심" |
| **MSA의 4대 핵심 원칙** | 1. **독립적 배포성(Independent Deployability)**: 타 서비스 변경 없이 언제든 배포 가능해야 함 | "출시" |
| **비즈니스 도메인 중심(Domain-driven)** | 기술 레이어가 아닌 비즈니스 기능(결제, 배송 등) 단위로 분리 | "학습하는 기계" |
| **데이터 격리(Database per Service)** | 타 서비스의 DB에 직접 접근 불가(API를 통해서만 접근) | "식당 메뉴판" |
| **탈중앙화된 거버넌스(Decentralized Governance)** | 서비스별로 최적의 언어와 프레임워크 선택(Polyglot) | "이 개념의 핵심" |
| **도입의 전제 조건** | 마틴 파울러가 지적했듯, CI/CD 파이프라인 자동화, 컨테이너 오케스트레이션(Kubernetes), 통합 모니터링 체계가 없는 상태에서 ... | "자동 품질 검사 라인" |
