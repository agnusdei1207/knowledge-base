...
- 개요: 도메인 단위 독립 배포 구조
- 배경: 대규모 서비스는 기능 증가보다 단일 배포 충돌, 장애 전파, 조직 간 승인 대기, 공유 DB 변경이 병목이 됨.
- 필요성: 서비스 경계를 도메인·팀 경계와 맞추고 API contract, DB per Service, distributed tracing으로 변경 영향과 장애 범위를 제한해야 함.
