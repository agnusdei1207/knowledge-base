...
- 개요: 가상 메모리는 독립 주소공간 제공 기법이다.
- 배경: 프로세스가 물리 주소를 직접 다루면 보호 경계와 연속 메모리 확보가 어려워 OS와 MMU가 page table로 주소 변환을 수행한다.
- 필요성: 가상 메모리는 page fault rate, TLB miss, protection bit 기준으로 메모리 보호·큰 프로그램 실행·단편화 완화를 제공한다.
