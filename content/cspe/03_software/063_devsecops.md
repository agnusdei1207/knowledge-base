...
- 개요: 보안 내장형 DevOps
- 배경: 오픈소스, 컨테이너, IaC 사용 증가로 취약점 유입 경로가 코드 저장소, 이미지 레지스트리, 런타임 설정까지 확장되었다.
- 필요성: SAST, SCA, IaC scan, container scan을 CI/CD gate에 연결해 CVE·CWE·OWASP 기준 위반을 배포 전 차단한다.
