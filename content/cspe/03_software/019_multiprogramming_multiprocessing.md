...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **다중프로그래밍 (Multiprogramming)** | **하나의 CPU**에서 여러 프로그램을 메모리에 올려두고, I/O 대기 시간 동안 다른 프로그램을 실행하여 CPU 이용률을 높이는 방식 | "두뇌" |
| **다중처리 (Multiprocessing)** | **여러 개의 CPU(코어)**가 여러 작업을 동시에 진짜로 처리하는 방식 | "독립된 사무실" |
| **시분할 (Time Sharing)** | 다중프로그래밍의 발전형으로, CPU를 아주 짧은 시간 간격으로 쪼개서 여러 사용자에게 나눠줌으로써 마치 나 혼자 컴퓨터를 쓰는 듯한 느낌을 줌 | "두뇌" |
| **발전 단계** | Single Tasking -> Multiprogramming (I/O 대기 활용) -> Time Sharing (응답성 확보) -> Mu... | "이 개념의 핵심" |
| **Multiprocessing의 분류** | - **Symmetric (SMP)**: 모든 CPU가 대등하게 메모리와 I/O를 공유 | "독립된 사무실" |
| **Asymmetric (ASMP)** | 주(Master) CPU가 스케줄링을 하고 종(Slave) CPU가 명령을 따름 | "두뇌" |
| **Amdahl의 법칙** | 다중처리에서 CPU를 아무리 늘려도, 프로그램 중 병렬화할 수 없는 부분(순차 실행 부분) 때문에 전체 성능 향상에는 한계가 있다는 법칙 | "두뇌" |
