...
- 정의: 오류·손실 프레임을 ACK/NAK와 timeout으로 재전송하는 오류 제어 기법
- 배경: 링크·전송 계층은 CRC·checksum으로 오류를 검출하고, ARQ는 검출 이후의 재전송 절차를 담당함
- 필요성: RTT가 크거나 오류율이 높은 환경에서는 재전송 범위와 버퍼 요구량이 처리량을 좌우함
