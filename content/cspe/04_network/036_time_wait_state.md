...
- 개요: TCP 능동 종료자가 마지막 ACK 이후 2MSL 동안 유지하는 종료 대기 상태
- 배경: 같은 5-tuple을 즉시 재사용하면 이전 연결의 지연 세그먼트가 새 연결 데이터로 오인될 위험이 있음
- 필요성: 마지막 ACK 유실 시 상대의 FIN 재전송에 다시 응답하기 위한 TCP 정확성 보장 장치임
