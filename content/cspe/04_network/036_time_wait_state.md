...
| **개요** | TCP 연결 종료 후 소켓이 즉각 소멸하지 않고, `2 MSL (Maximum Segment Lifetime)` 동안 소멸을 유예하는 마지... | "등기 우편" |
| **정상적인 연결 종료 보장** | Active Closer가 보낸 마지막 ACK가 네트워크에서 유실될 경우, Passive Closer는 종료되지 않은 줄 알고 FIN을 재... | "건물 관리실" |
| **잔류 패킷(지연 패킷) 혼선 방지** | 이전 세션에서 길을 잃고 늦게 도착한 패킷이, 우연히 동일한 포트로 방금 맺어진 "새로운 연결"의 패킷으로 오인되어 데이터가 오염되는 것을... | "핵심 기술 요소" |
