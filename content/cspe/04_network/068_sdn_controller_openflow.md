...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **개요** | 데이터 계층과 분리된 제어 계층(Control Plane)의 핵심 두뇌 역할을 하는 SDN Controller와, 장비 간 통신 규약인 O... | "중앙 교통 관제" |
| **필요성** | - 통신망을 클라우드처럼 소프트웨어로 쥐락펴락(SDN) 하려면, 수많은 화이트박스(깡통) 스위치들에게 1초 만에 경로를 할당해 줄 **초지... | "중앙 교통 관제" |
| **Reactive(반응형) vs Proactive(사전 설정형)** | - *Reactive*: 스위치가 모르는 패킷이 오면 매번 컨트롤러에 물어보고(Packet-In) 답을 받아(Flow-Mod) 테이블을 채... | "택배 상자" |
| **결론** | SDN 컨트롤러와 OpenFlow는 하드웨어에 갇혀있던 트래픽 제어권을 소프트웨어의 세계로 해방시킨 디지털 혁명의 도구이다 | "중앙 교통 관제" |
| **SDN 컨트롤러와 OpenFlow** | SDN 컨트롤러와 OpenFlow (SDN Controller & OpenFlow)의 핵심 개념 | "중앙 교통 관제" |
| **의사 결정권자** | 라우터 하드웨어 내부의 제어 칩 (분산형) | "핵심 기술 요소" |
