...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **TCAM 삼진 CAM 고속 검색** | TCAM 삼진 CAM 고속 검색 (TCAM)의 핵심 개념 | "이 주제의 본질" |
...
- 개요: 와일드카드 병렬 검색 메모리
- 배경: 라우터와 스위치는 패킷마다 ACL, 라우팅 prefix, QoS class를 라인카드에서 즉시 결정해야 한다.
- 필요성: TCAM은 ternary match를 병렬 수행해 100Gbps급 라인레이트에서 ACL·라우팅·QoS 매칭을 처리한다.
