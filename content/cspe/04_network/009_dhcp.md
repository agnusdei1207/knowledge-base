...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **개요** | 호스트가 네트워크에 접속할 때 IP 주소, 서브넷 마스크, 기본 게이트웨이 등을 **자동으로 할당(임대)**해주는 프로토콜입니다 | "인터넷 주소" |
| **필요성** | 수백 대의 PC에 관리자가 일일이 수동으로 IP를 입력하는 것은 불가능에 가깝고, 실수로 인한 IP 충돌도 잦습니다 | "인터넷 주소" |
| **핵심 직관** | **"IP 주소 렌탈 서비스"** | "인터넷 주소" |
| **배경 (왜 UDP 67/68번인가?)** | DHCP는 클라이언트가 아직 IP가 없는 상태에서 시작됩니다 | "일반 우편" |
| **작동 원리 (DORA 4단계)** | 1.  **Discover**: "IP 빌려줄 서버 있나요?" (Client -> Server, Broadcast) | "인터넷 주소" |
| **Offer** | "이 IP(예: 192 | "인터넷 주소" |
| **Request** | "좋아요! 그 IP로 쓸게요 | "인터넷 주소" |
