...
- **구체 예시**: 100 Mbps 링크 cost 1, 10 Mbps 링크 cost 10으로 설정하면 SPF는 총 cost가 낮은 경로를 선택한다. 기준 bandwidth를 100 Gbps로 조정하지 않으면 고속 링크 차이가 반영되지 않을 수 있다.
...
> 2. **가치**: Area 구조, cost 기반 경로 선택, 빠른 장애 수렴으로 대규모 내부망의 라우팅 테이블과 LSA 범위를 통제한다.
