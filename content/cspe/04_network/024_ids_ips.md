...
IDS·IPS는 네트워크 트래픽에서 공격 징후를 탐지하거나 차단하는 보안 통제 기술이다. 방화벽이 허용한 업무 포트 내부에서도 exploit과 C2가 발생하므로 payload·프로토콜·행위 분석이 필요하다. IDS는 관제 중심, IPS는 inline 차단 중심으로 운영 목적이 다르다.
