...
스위칭 계층은 네트워크 장비가 어떤 계층 정보를 보고 전달 결정을 내리는지 구분한 체계이다. L2는 MAC 기반 LAN 전달, L3는 IP 기반 라우팅, L4는 포트·세션 기반 분산, L7은 애플리케이션 정책 처리를 담당한다. 데이터센터와 기업망은 네트워크 분리, 서버 이중화, 서비스 정책 적용을 위해 계층별 장비를 조합한다.
