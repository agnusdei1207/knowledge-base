...
- 개요: BLE 기반 저전력 Mesh 제어망
- 배경: 스마트 조명·센서·비콘은 수바이트~수십바이트 상태 메시지를 긴 주기로 송수신하므로 전력 소모와 설치 범위를 함께 통제해야 한다.
- 필요성: BLE Mesh는 2.4GHz BLE 생태계와 Relay·Friend 기능을 사용해 건물 단위 IoT 제어망을 다중 홉으로 확장한다.
