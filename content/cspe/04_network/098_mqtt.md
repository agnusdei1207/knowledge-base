...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **MQTT 경량 메시징** | MQTT 경량 메시징 (MQTT)의 핵심 개념 | "이 주제의 본질" |
...
- 개요: IoT 경량 pub/sub 메시징
- 배경: 저대역폭·불안정한 네트워크의 장치는 서버와 직접 동기 호출을 반복하면 연결 유지와 재전송 비용이 커진다.
- 필요성: MQTT는 broker, topic, QoS 0/1/2, retained message, last will로 비동기 전달과 장치 상태 통지를 제공한다.
