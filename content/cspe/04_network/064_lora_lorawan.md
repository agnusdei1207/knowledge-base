...
- 개요: LoRa 변조 기반 LPWAN 프로토콜
- 배경: 검침·농업·환경 관측은 소량·저빈도 센서 데이터를 수 km 범위로 보내면서 배터리 교체 주기를 줄여야 한다.
- 필요성: LoRaWAN은 비면허 Sub-GHz와 Class A/B/C 동작을 사용하되 지역별 주파수 계획과 전파 규제를 기준으로 설계해야 한다.
