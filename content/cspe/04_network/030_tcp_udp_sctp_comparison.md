...
TCP·UDP·SCTP는 애플리케이션 간 데이터를 전달하는 전송 계층 프로토콜이다. TCP는 연결형 신뢰성 전송, UDP는 비연결 datagram 전송, SCTP는 message 기반 association과 multi-streaming을 제공한다. 서비스 지연, 손실 허용, 메시지 경계, 경로 이중화 요구에 따라 선택이 달라진다.
