...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **Intent-Based Networking** | Intent-Based Networking (Intent-Based Networking)의 핵심 개념 | "이 주제의 본질" |
...
- 개요: 의도 기반 네트워크 운영 모델
- 배경: 클라우드·캠퍼스·데이터센터는 정책 수와 장비 수가 증가해 수동 설정 검증으로 목표 상태 일치 여부를 판단하기 어렵다.
- 필요성: IBN은 의도를 장비 구성으로 변환하고 telemetry로 의도 위반을 탐지해 반복 수정한다.
