...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **ATM 비동기 전송 모드** | ATM 비동기 전송 모드 (ATM Asynchronous Transfer Mode)의 핵심 개념 | "이 주제의 본질" |
...
- 개요: 고정 길이 셀 기반 전송
- 배경: 음성·영상·데이터를 하나의 광대역망에서 처리하려면 지연, 지터, 손실 요구가 다른 트래픽을 구분해야 한다.
- 필요성: ATM은 53바이트 셀, 가상회선, QoS 클래스, AAL로 멀티서비스 통합망의 전송 기준을 제공했다.
