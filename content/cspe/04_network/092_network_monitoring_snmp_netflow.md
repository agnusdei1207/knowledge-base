...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **네트워크 모니터링 SNMP NetFlow** | 네트워크 모니터링 SNMP NetFlow (Network Monitoring SNMP NetFlow)의 핵심 개념 | "이 주제의 본질" |
...
- 개요: 장비 상태와 흐름 수집 체계
- 배경: 네트워크 장애는 인터페이스 오류, 경로 변화, 트래픽 폭증이 함께 나타나므로 단일 지표만으로 원인을 분리하기 어렵다.
- 필요성: SNMP OID 카운터, NetFlow v9/IPFIX flow record, sFlow 샘플 패킷을 수집 주기·보존·경보 기준과 함께 설계한다.
