...
- 정의: prefix length 기준으로 IP 주소를 분할·집계하는 주소 설계 기법
- 배경: classful(A/B/C 고정 블록) 주소 체계는 주소 낭비와 라우팅 테이블 증가 문제를 유발
- 필요성: CIDR 기반 설계로 필요한 크기만 할당하고 보안 구역과 라우팅 범위를 명확히 분리
