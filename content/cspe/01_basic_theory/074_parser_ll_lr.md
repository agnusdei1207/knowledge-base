...
- 개요: 토큰열이 문법에 맞는지 분석해 parse tree/AST를 생성하는 구문 분석 기법
- 배경: 언어 처리 시스템은 문자열을 직접 실행하지 않고, 문법 검증 후 AST로 바꿔 의미 분석과 코드 생성에 활용함
- 필요성: 문법 형태, 오류 진단, 생성기 사용 여부에 따라 LL·LR 파서를 구분해 선택해야 함
