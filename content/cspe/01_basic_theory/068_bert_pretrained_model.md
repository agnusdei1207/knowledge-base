...
- `[CLS]`: 문장 또는 문서 전체 표현을 모으는 분류용 특수 토큰임
- 파인튜닝: 사전학습된 모델에 태스크별 지도 데이터를 추가 학습하는 과정임

## Ⅰ. 개요

- **정의**: BERT는 Transformer 인코더를 적층해 입력 문장의 좌우 문맥을 동시에 참고하고, MLM 등 사전학습 목표로 범용 언어 이해 표현을 학습한 모델임. 문서 분류, 질의응답, 개체명 인식처럼 문맥 이해가 중요한 NLP 태스크의 초기 표현을 제공하기 위해 사용함
...
|:---|:---|:---|
| BERT 구조 설명 | 인코더, 양방향 self-attention, `[CLS]` | GPT 디코더 구조와 혼동 |
| 사전학습·파인튜닝 설명 | MLM, NSP, downstream task head | `[MASK]`의 학습-추론 차이 누락 |

> 요약: BERT는 양방향 문맥을 학습한 인코더 기반 사전학습 모델로, 생성보다 자연어 이해 태스크에 적합함

## Ⅱ. 특징/비교

| 판단 기준 | Word2Vec/GloVe | BERT | GPT 계열 |
|:---|:---|:---|:---|
| 문맥 처리 | 단어마다 고정 벡터를 사용함 | 좌우 문맥을 동시에 반영함 | 이전 토큰만 보고 다음 토큰을 생성함 |
| 구조 | 얕은 임베딩 또는 행렬 분해 | Transformer 인코더 | Transformer 디코더 |
| 주요 용도 | 경량 피처, 검색, 추천 | 분류, NER, QA, 검색 랭킹 | 생성, 대화, 요약, 코드 작성 |
| 선택 기준 | 비용 제약과 단순 유사도 | 이해 정확도와 문맥 해석 | 자연어 생성과 지시 수행 |

> 요약: BERT는 정적 임베딩보다 문맥 이해가 강하고 GPT보다 생성 능력은 약하지만 NLU 비용 대비 효율이 높음
...
| MLM Head | 가려진 토큰의 원래 값을 예측해 언어 지식을 학습함 | 빈칸 채점기 |
| Task Head | `[CLS]` 또는 토큰 표현 위에 분류, NER, QA 출력층을 붙임 | 업무별 판정기 |
...
4. **추론·평가**: 분류 정확도, F1, MRR, EM/F1 등 태스크 지표로 문맥 이해 성능을 확인함
...
- **P3 추론 비용과 지연**: 인코더 층과 attention 연산이 많아 실시간 서비스에서 p95 지연과 GPU 비용이 증가함

> 요약: BERT의 실무 병목은 마스크 기반 학습 차이, 입력 길이 제약, 추론 비용에서 발생함
...
- **설계 관점**: 생성 필요 여부, 입력 길이, 지연 예산, 라벨 데이터 규모를 기준으로 BERT 계열과 GPT 계열을 구분해 선택해야 함
- **검증 관점**: downstream F1, 긴 문서 처리 성능, p95 지연, 파인튜닝 데이터 누수 여부를 함께 검증해야 실서비스 품질을 판단할 수 있음
- **운영 관점**: DistilBERT, pruning, quantization, batching을 활용해 GPU 비용과 응답 지연을 서비스 등급별로 관리해야 함
- **적용 관점**: 분류, 검색 랭킹, NER, 문장 유사도처럼 이해 중심 업무에는 적합하고 자유 생성이 필요한 업무는 GPT 계열과 비교해야 함
- **보안·신뢰 관점**: 편향 데이터, adversarial text, 민감 문서 파인튜닝은 판단 신뢰를 떨어뜨릴 수 있으므로 입력 검증과 모델 카드 관리가 필요함
