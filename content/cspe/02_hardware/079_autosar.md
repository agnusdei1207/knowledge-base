...
- **정의/개념**: AUTOSAR는 차량 ECU 소프트웨어를 응용 기능과 런타임과 기본 소프트웨어와 하드웨어 추상화 계층으로 나눠 재사용성과 공급사 독립성과 통합 검증성을 높이는 자동차 소프트웨어 표준 플랫폼임
- **배경/필요성**: 차량 기능과 ECU 수가 늘어나면서 공급사마다 다른 구조로는 재사용과 통합 검증과 장기 유지보수가 어려워져, 공통 아키텍처와 인터페이스 표준이 필요해짐

## Ⅱ. 특징

- 응용 기능과 하드웨어 의존성을 분리해 이식성과 재사용성을 높임
- ARXML 기반 설정과 코드 생성 품질이 개발 생산성을 크게 좌우함
- Classic은 결정성과 제어 ECU에, Adaptive는 서비스 지향 고성능 ECU에 적합함
- 표준을 도입해도 툴 체인 관리와 통합 시험 부담은 여전히 큼
...
| 유의점 | 통합이 커질수록 유지보수 비용이 커짐 | 설정 오류와 자원 오버헤드를 관리해야 함 | 자원 사용량과 서비스 복잡도를 엄격히 통제해야 함 |
...
1. 파워트레인이나 바디 ECU처럼 Classic Platform을 쓰는 환경은 표준화 효과가 크지만 ARXML와 툴 체인 설정 오류 하나가 통합 실패로 번질 수 있으므로 설정 검증과 CI 통합 검증을 함께 두고 통합 결함 건수와 코드 생성 오류율과 타이밍 준수율로 확인함
2. ADAS나 도메인 컨트롤러처럼 Adaptive Platform을 쓰는 환경은 서비스 확장성이 좋지만 서비스 수가 늘수록 메모리와 시작 시간이 불안해질 수 있으므로 서비스 계약 관리와 성능 프로파일링을 적용하고 기동 시간과 메모리 사용량과 서비스 복구율로 확인함
