...
# 하드웨어 가상화 - VT-x·AMD-V (Hardware Virtualization)
...
- **정의/개념**: 하드웨어 가상화는 CPU가 VM 실행 모드, 특권 명령 트랩, 2단계 주소 변환을 제공해 하이퍼바이저가 게스트 OS(Operating System)를 직접 실행하면서도 격리하도록 지원하는 기술임.
