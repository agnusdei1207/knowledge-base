...
- 하이퍼바이저: 하나의 물리 서버에서 여러 가상머신을 실행·격리하는 소프트웨어 계층임
- VT-x/AMD-V: x86 프로세서가 가상화 실행 모드와 특권 명령 처리 기능을 제공하는 확장임
- VM Exit: 게스트 실행 중 하이퍼바이저 개입이 필요한 이벤트로 제어가 전환되는 현상임
- EPT/NPT: 게스트 물리 주소를 호스트 물리 주소로 변환하는 2단계 주소 변환 기술임

## Ⅰ. 개요

- **정의**: 하드웨어 가상화는 CPU가 가상머신 실행 모드, 특권 명령 트랩, 2단계 주소 변환을 제공해 하이퍼바이저가 게스트 OS를 안전하고 효율적으로 실행하도록 지원하는 기술임. 서버 통합, 격리, 클라우드 인프라 운영에서 가상화 오버헤드를 줄이기 위해 사용함.
- **배경/필요성**: 순수 소프트웨어 가상화는 특권 명령 변환과 메모리 매핑 유지 비용이 커 성능과 호환성 문제가 있었음. CPU 지원 기능을 사용하면 게스트 OS 수정 없이도 특권 동작을 제어하고 메모리 격리를 강화할 수 있음.
- **비유**: 건물 관리자가 각 입주사에 독립 사무실을 주되, 출입 통제와 전기 배선을 건물 설비가 직접 지원하는 구조임.

| 출제 의도 | 반드시 짚을 핵심 | 감점 회피 포인트 |
|:---|:---|:---|
| CPU 가상화 지원 구조 이해 | root/non-root mode, VMCS/VMCB, VM exit, EPT/NPT | 서버 가상화 전체와 혼동 |
...
| 성능 병목 | 변환과 trap 비용이 큼 | VM exit 빈도와 I/O 가상화가 주요 병목 |

> 요약: 하드웨어 가상화는 게스트 호환성과 성능을 높이지만 VM exit와 I/O 경로 관리가 중요함.

- **적용 조건**: CPU 가상화 기능, IOMMU, 하이퍼바이저 버전이 워크로드 요구와 맞아야 함
- **선택 지표**: VM exit rate, EPT miss rate, CPU ready time을 함께 봐야 함
...
| 인터럽트·I/O 가상화 | 장치 접근, 인터럽트 전달, DMA 격리를 처리함 | 공용 설비 관리 |
...
```text
+----------+      +----------+      +----------+      +----------+
| VM생성   | ---> | 게스트실행 | ---> | Exit처리 | ---> | 재진입   |
+----------+      +----------+      +----------+      +----------+
```

1. **VM 생성** — 하이퍼바이저가 vCPU 상태, 메모리 매핑, VMCS/VMCB 제어값을 설정함
