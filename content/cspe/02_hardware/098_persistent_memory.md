...
- **정의/개념**: 퍼시스턴트 메모리는 CPU가 byte-addressable 방식으로 접근하면서 전원 장애 후에도 데이터가 유지되는 메모리 계층임. 저장장치 I/O(Input/Output) 지연을 줄이고 인메모리 데이터의 복구 시간을 단축하기 위해 사용함.
