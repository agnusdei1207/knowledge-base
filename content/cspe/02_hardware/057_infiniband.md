...
- RDMA: 원격 서버 메모리에 CPU 개입을 최소화해 직접 읽기·쓰기를 수행하는 기술임
- HCA: Host Channel Adapter로 서버가 InfiniBand 패브릭에 접속하는 어댑터임
- Subnet Manager: InfiniBand 패브릭의 주소, 경로, 포트를 관리하는 제어 구성요소임
- Queue Pair: 송신 큐와 수신 큐로 구성된 RDMA 통신 엔드포인트임

## Ⅰ. 개요

- **정의**: InfiniBand는 HCA, 스위치, 서브넷 관리, RDMA 전송을 기반으로 고대역폭·저지연·낮은 CPU 사용률을 제공하는 고성능 네트워크 패브릭임. HPC와 AI 클러스터에서 노드 간 통신 지연과 대역폭을 기준으로 대규모 병렬 작업을 연결하기 위해 사용함.
- **배경/필요성**: 대규모 학습과 과학 계산은 노드 간 gradient, 메시지, 파일 I/O를 빈번히 교환함. 일반 이더넷만으로는 지연과 CPU 오버헤드가 커질 수 있어 RDMA 중심의 전용 패브릭이 필요함.
...
|:---|:---|:---|
| 전송 방식 | TCP/IP 스택과 범용 네트워크 운영을 중심으로 함 | RDMA, 큐 페어, 서브넷 관리로 저지연 전송을 제공함 |
...
| 스위치 | 포트 간 패킷을 전달하고 다중 경로와 QoS를 지원함 | 물류 허브 |
| Subnet Manager | LID, 경로, 링크 상태를 관리해 패브릭을 구성함 | 교통 관제실 |
...
- **P3 대응**: RoCE/Ethernet 대안과 워크로드별 성능비를 비교해 단계적 도입을 계획함 (확인: 성능당 총비용)

> 요약: 개선은 자동화된 패브릭 운영, 혼잡 제어, 비용 대비 성능 검증으로 수행함.

## Ⅶ. 전망

- **발전 방향**: InfiniBand는 NDR/XDR급 속도, SHARP in-network reduction, GPU direct RDMA와 결합해 AI/HPC 노드 간 collective 시간을 줄이는 패브릭으로 지속됨
- **기술사적 판단**: leaf-spine oversubscription, HCA 포트 수, Subnet Manager 이중화, 병렬 파일시스템 트래픽 분리를 기준으로 패브릭 규모를 정함; RDMA latency, all-reduce bandwidth, congestion counter, packet discard, failover 시간을 측정해 MPI와 학습 워크로드의 실효 이득을 확인함
- **기술사 제언**: InfiniBand 답안은 포트 속도보다 RDMA 경로, 혼잡 제어, SM 운영, 장애 격리 지표를 함께 제시해야 설계 판단이 분명함
