...
- **배경/필요성**: 배터리 기반 모바일과 임베디드 기기는 낮은 전력으로 충분한 성능과 주변장치 통합이 필요함. ARM은 설계 IP와 표준 인터커넥트를 제공해 제조사가 빠르게 SoC를 구성하도록 함.
...
> 요약: ARM은 RISC ISA와 IP 생태계를 기반으로 목적별 프로세서와 SoC를 빠르게 구성하는 구조임.
...
- **P2 SoC 통합 복잡도**: 캐시 일관성, AMBA QoS, interrupt routing, 전력 도메인 설계가 어렵음
