...
- DIMM: 메인보드 메모리 슬롯에 장착되는 메모리 모듈임
- 비휘발성: 전원이 꺼져도 저장 데이터가 유지되는 성질임
- NVDIMM-N: DRAM에 NAND와 백업 전원을 결합해 정전 시 데이터를 보존하는 방식임
- NVDIMM-P: 메모리 접근과 대용량 비휘발 저장 특성을 결합한 표준 지향 방식임
...
|:---|:---|:---|
| 비휘발성 메모리 계층 이해 | DRAM, NAND, 백업 전원, persistence, 복구 | SSD와 동일 장치로 설명 |
...
|:---|:---|:---|:---|
| 접근 경로 | CPU 메모리 로드/스토어 | 블록 I/O 스택 | 메모리 버스 또는 특수 드라이버 |
...
```text
+----------+      +----------+      +----------+      +----------+
| 정상쓰기 | ---> | 장애감지 | ---> | 데이터보존 | ---> | 복구마운트 |
+----------+      +----------+      +----------+      +----------+
```

1. **정상 접근** — CPU나 OS가 NVDIMM 영역을 메모리 또는 블록 장치처럼 읽고 씀
...
- **P1 대응**: 서버 HCL, BIOS 설정, OS namespace 지원, 드라이버 버전을 사전 검증함 (확인: platform compatibility matrix)
...
- **발전 방향**: CXL memory, persistent memory, storage class memory와 결합해 메모리와 스토리지 경계가 재편됨
