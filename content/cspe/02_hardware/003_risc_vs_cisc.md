...
- RISC: 단순하고 고정 길이에 가까운 명령어를 빠르게 파이프라인 처리하는 설계 철학임
...
- **배경/필요성**: 컴파일러와 반도체 기술이 발전하면서 단순 명령어를 빠르게 반복할지, 복합 명령어로 프로그램 크기를 줄일지의 선택이 중요해짐. ISA는 하드웨어만이 아니라 OS, 컴파일러, 응용 생태계까지 묶는 장기 호환성 기준임.
