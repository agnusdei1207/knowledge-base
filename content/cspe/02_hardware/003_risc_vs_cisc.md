...
- **정의/개념**: RISC와 CISC는 명령어 집합 구조를 설계하는 두 관점으로, RISC는 단순 명령어와 규칙적인 실행을 통해 파이프라인 효율을 높이고 CISC는 복합 명령어와 높은 코드 밀도를 통해 표현력과 호환성을 중시하는 방식임
- **배경/필요성**: 메모리가 부족하던 시기에는 코드 밀도가 중요했지만, 고성능 파이프라인과 전력 효율이 중요해지면서 명령어 복잡도와 실행 효율 사이의 균형이 ISA 선택 기준이 됨

## Ⅱ. 특징

- RISC는 고정 길이 명령어와 load/store 구조로 실행 흐름이 단순함
- CISC는 가변 길이와 복합 명령으로 코드 밀도와 호환성에 강함
- RISC는 파이프라인과 저전력 설계에 유리함
...
| Register Model | 피연산자 사용 방식과 compiler 최적화 자유도를 결정함 |
| Decode Logic | 명령어를 내부 실행 단위로 바꾸며 CISC에서 부담이 더 큼 |
| Execution Path | 파이프라인과 u-op 변환 구조가 실제 성능과 전력 효율을 좌우함 |
...
1. 문제: CISC는 가변 길이 해독과 u-op 분해 부담이 커서 전력과 지연 비용이 증가할 수 있음
   - 해결방안: u-op cache와 병렬 decoder를 적용하고 decode bandwidth와 perf per watt로 검증함
2. 문제: RISC는 단순 명령어 중심이라 같은 기능을 위해 코드 크기가 커질 수 있음
...
3. 문제: ISA 철학만 보고 선택하면 실제 워크로드와 생태계 제약을 놓칠 수 있음
   - 해결방안: workload fit과 toolchain maturity를 함께 평가하고 software porting effort와 benchmark efficiency로 검증함
...
RISC와 CISC의 실무 선택 기준은 명령어 수 자체가 아니라 파이프라인 효율, 코드 밀도, 생태계 호환성 중 무엇이 더 중요한지에 있음.
