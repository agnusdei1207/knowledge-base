...
- RAID 컨트롤러: 여러 디스크를 RAID 논리 볼륨으로 구성하고 캐시·재구성·오류 처리를 담당하는 장치임
- JBOD: Just a Bunch Of Disks로 디스크를 RAID로 묶지 않고 개별 디스크로 노출하는 방식임
- HBA: Host Bus Adapter로 스토리지 장치를 호스트에 연결하고 주로 패스스루 역할을 수행함
- Rebuild: 장애 디스크 교체 후 패리티나 미러를 이용해 데이터를 복원하는 과정임

## Ⅰ. 개요

- **정의**: RAID 컨트롤러는 다수 디스크를 논리 RAID 볼륨으로 묶어 성능·가용성·캐시 기능을 제공하는 제어 장치이고, JBOD는 디스크를 개별 장치로 노출해 상위 소프트웨어가 직접 관리하도록 하는 구성임. 스토리지의 장애 보호, 성능, 운영 책임 위치를 결정하기 위해 사용함.
...
|:---|:---|:---|
| 디스크 노출 | RAID 볼륨 하나 또는 여러 LUN으로 노출 | 개별 물리 디스크로 노출 |
| 보호 기능 | 미러, 패리티, 캐시, rebuild를 컨트롤러가 처리 | ZFS, Ceph, vSAN 등 소프트웨어가 처리 |
...
| 캐시·BBU | 쓰기 성능과 정전 시 데이터 보호를 담당함 | 임시 보관함과 비상 전원 |
...
```text
+----------+      +----------+      +----------+      +----------+
| 디스크탐색 | ---> | 구성선택 | ---> | 볼륨/노출 | ---> | 장애관리 |
+----------+      +----------+      +----------+      +----------+
```

1. **디스크 탐색** — 컨트롤러나 HBA가 연결 디스크 상태, 용량, SMART 정보를 확인함
...
- **발전 방향**: NVMe, erasure coding, SDS, disaggregated storage 확산으로 전통 하드웨어 RAID보다 소프트웨어 기반 데이터 보호가 확대됨
