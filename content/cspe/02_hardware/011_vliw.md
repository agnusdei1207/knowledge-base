...
- **배경/필요성**: 슈퍼스칼라는 하드웨어가 동적으로 의존성을 찾기 때문에 복잡도와 전력이 증가함. VLIW는 병렬성 탐지 부담을 컴파일러로 옮겨 DSP, 미디어 처리, 임베디드 가속기처럼 예측 가능한 workload에서 효율을 얻고자 함.
...
| 하드웨어 복잡도 | dependency check, renaming, ROB가 복잡함 | issue 제어가 단순하고 전력 효율이 높을 수 있음 |
