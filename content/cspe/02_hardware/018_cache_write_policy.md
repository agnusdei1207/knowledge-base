...
- **배경/필요성**: 즉시 메모리 반영은 단순하고 안전하지만 느리고, 지연 반영은 빠르지만 일관성과 복구가 복잡하므로 워크로드와 신뢰성 요구에 맞는 정책 선택이 필요함

## Ⅱ. 특징

- Write-Through는 메모리 최신성 유지가 쉽고 구조가 단순함
- Write-Back은 메모리 트래픽을 줄여 성능에 유리함
...
| 구성요소 | 설명 |
|:---|:---|
| Cache Line | 수정 대상 데이터가 저장되는 기본 단위이며 dirty 상태 여부가 핵심임 |
...
1. 문제: Write-Through는 잦은 메모리 쓰기로 버스와 대역폭 병목이 커질 수 있음
...
3. 문제: write miss 정책이 워크로드와 맞지 않으면 캐시 오염이나 지연이 불필요하게 증가할 수 있음
