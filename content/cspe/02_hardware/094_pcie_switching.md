...
- PCIe: point-to-point lane 기반 고속 직렬 I/O 인터페이스임
- Root Complex: CPU/칩셋 쪽에서 PCIe 계층과 주소 공간을 시작하는 루트 장치임
- Endpoint: NVMe SSD, NIC, GPU처럼 PCIe 버스에 연결되는 말단 장치임
- Switch: 하나의 upstream port와 여러 downstream port 사이 패킷을 라우팅하는 PCIe 장치임

## Ⅰ. 개요

- **정의**: PCIe 스위칭 아키텍처는 Root Complex와 여러 Endpoint 사이에 PCIe switch를 배치해 lane, 주소 공간, 트래픽을 포트 단위로 분배하는 확장 구조임. 제한된 CPU PCIe 포트를 여러 고성능 장치에 연결하고 I/O 확장성과 구성 유연성을 확보하기 위해 사용함.
- **배경/필요성**: 서버는 GPU, NVMe, NIC, DPU 등 PCIe 장치 수가 늘어나지만 CPU가 제공하는 lane 수와 물리 슬롯은 제한적임. 스위치는 장치 fan-out과 peer-to-peer 전송을 제공하지만 oversubscription과 지연을 함께 관리해야 함.
...
| 기능 | 단순 연결 중심 | hot-plug, bifurcation, P2P, 관리 기능을 제공함 |

> 요약: PCIe 스위칭은 장치 확장성과 연결 유연성을 얻는 대신 공유 상위 링크의 병목을 관리해야 함.

- **적용 조건**: 하위 장치 총 요구 대역폭과 upstream lane 용량의 비율이 허용 범위여야 함
- **선택 지표**: oversubscription ratio, switch hop latency, ACS 지원 여부를 함께 봐야 함
...
| Routing·관리 기능 | TLP 라우팅, ACS, error reporting, hot-plug를 처리함 | 교통 관제 센터 |
...
```text
+----------+      +----------+      +----------+      +----------+
| 열거/설정 | ---> | 주소라우팅 | ---> | 패킷전달 | ---> | 오류관리 |
+----------+      +----------+      +----------+      +----------+
```

1. **열거·설정** — BIOS/OS가 switch 하위 장치를 탐색하고 bus/device/function 번호와 BAR를 할당함
...
4. **오류·관리 처리** — AER, hot-plug, ACS, 링크 상태를 감시하고 장애를 보고함
...
- **P3 격리 취약성**: ACS/IOMMU 설정이 부정확하면 endpoint 간 DMA 접근 격리가 약해질 수 있음
...
- **P3 대응**: ACS, ATS, IOMMU group, firmware 설정을 검증해 DMA 격리를 보장함 (확인: isolation test result)

> 요약: PCIe 스위치 도입은 포트 수 확장보다 대역폭 계획과 DMA 보안 검증이 우선임.

## Ⅶ. 전망

- **발전 방향**: PCIe 6.x/7.x, CXL switch, composable infrastructure로 발전하며 메모리와 가속기 자원의 동적 연결이 확대됨
