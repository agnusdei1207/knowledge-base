...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **정의** | 사람이 읽는 텍스트(문자열), 이미지, 오디오 등 비정형 데이터를 컴퓨터가 계산할 수 있도록 고정된 길이의 실수 배열(다차원 벡터)로 압축... | "핵심 기술 요소" |
| **필요성** | 컴퓨터는 "고양이"와 "개"가 비슷한 동물이고 "자동차"와는 다르다는 것을 글자만 봐서는 모름 | "핵심 기술 요소" |
| **핵심 직관** | 사람의 성격을 [외향성, 친화성, 성실성, 신경증, 개방성]의 5차원 점수로 표현하듯, 단어의 느낌을 [크기, 온도, 감정, 동물성, 추상성 | "핵심 기술 요소" |
| **배경** | 2013년 Word2Vec의 등장으로 "단어의 의미는 그 단어 주변에 함께 쓰이는 단어들로 결정된다"는 분포 가설(Distributiona... | "핵심 기술 요소" |
| **작동 원리 (텍스트 $\rightarrow$ 임베딩 변환)** | 1. (토큰화): "사과가 맛있다" $\rightarrow$ [사과, 가, 맛있다] | "핵심 기술 요소" |
| **비유** | 한국어를 전혀 모르는 미국인(컴퓨터)에게, "사과"라는 단어가 뜻하는 속성을 바코드(숫자)로 찍어서 전달해 주는 번역기 | "핵심 기술 요소" |
| **흔한 오해/주의점** | 차원(Dimension) 수가 높다고 무조건 좋은 게 아님 | "핵심 기술 요소" |
