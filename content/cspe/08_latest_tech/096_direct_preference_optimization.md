...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **필요성** | 기존 RLHF는 PPO(강화학습) 알고리즘이 너무 불안정하고, 보상 모델을 따로 훈련해야 해서 자원과 시간이 많이 소모되며 튜닝하기 까다로움 | "핵심 기술 요소" |
| **배경** | 스탠포드 연구진(2023) 제안 | "핵심 기술 요소" |
| **작동 원리** | 1. (프롬프트, 선호 답변 $y_w$, 비선호 답변 $y_l$) 데이터쌍을 준비 | "핵심 기술 요소" |
| **흔한 오해/주의점** | DPO가 RLHF보다 '성능'이 무조건 뛰어난 것은 아님(학습 안정성과 효율성이 핵심) | "핵심 기술 요소" |
| **RLHF** | DPO가 대체하고자 하는 전통적인 강화학습 기반의 인간 정렬 파이프라인 | "핵심 기술 요소" |
| **KTO (Kahneman-Tversky Optimization)** | DPO를 더 단순화하여, 선호 쌍($A > B$) 없이 "이 답변이 좋아요(True/False)"만으로 학습하는 차세대 최적화 기법 | "핵심 기술 요소" |
