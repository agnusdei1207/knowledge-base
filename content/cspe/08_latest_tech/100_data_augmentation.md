...
|:---|:---|:---|
| 일반화 기법 원리 확인 | label-preserving 변형으로 과적합 억제 | 변형 기법 이름만 나열 |
| 도메인 판단 확인 | 의료·제조처럼 방향·색상이 의미인 도메인의 금지 변환 | 모든 변환을 무조건 유효로 전제 |
| 검증 설계 확인 | 증강 미적용 holdout 평가, leakage 차단 | 증강 데이터 포함 평가로 개선 주장 |

> 요약: 이 문제는 변형 기법이 아니라 라벨 의미 보존 판단과 공정한 성능 검증 설계를 묻는다.
...
| 도메인 | 허용 변환 | 금지 변환 | 판단 근거 |
|:---|:---|:---|:---|
| 일반 이미지 분류 | crop, flip, color jitter | 라벨 경계 침범 변형 | 클래스 의미 유지 |
| 의료·제조 검사 | 미세 노이즈, 밝기 | 좌우 반전, 색상 변경 | 방향·색이 진단 정보 |
| NLP | paraphrase, back translation | 부정어·수치 변경 | 의미 반전 위험 |

> 요약: 증강 정책은 기법이 아니라 도메인에서 라벨 의미를 침해하는지로 허용 여부를 판단함.

| 리스크 | 원인 | 대응 방안 | 확인 지표 |
|:---|:---|:---|:---|
| 라벨 훼손 | 의미 반전 변형 적용 | 도메인 금지 목록, 유사도 필터(0.85+) | 라벨 검증 통과율 |
| 평가 오염 (leakage) | 증강본이 test set에 유입 | 원본 단위 split 후 증강 | holdout 무결성 검사 |
| 개선 착시 | 증강 포함 데이터로 평가 | 증강 미적용 실제 holdout 평가 | real test uplift |

> 요약: 증강 리스크는 라벨 훼손과 평가 오염이며, 도메인 규칙과 원본 단위 분할로 통제함.
