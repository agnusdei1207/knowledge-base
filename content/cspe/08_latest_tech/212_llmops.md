...
| **핵심 직관** | 레스토랑의 레시피와 재료 관리 | "이 개념의 핵심" |
| **배경** | 파운데이션 모델(Foundation Model)의 등장으로 파라미터 업데이트(학습) 비용이 천문학적으로 비싸짐 | "설계 도면" |
| **작동 원리 (LLMOps의 핵심 사이클)** | 1. **프롬프트 엔지니어링 및 관리**: 여러 버전의 프롬프트를 만들고 A/B 테스트 진행 (Prompt Registry) | "이 개념의 핵심" |
| **파인튜닝 및 RAG 파이프라인** | 필요 시 파인튜닝(PEFT/LoRA)을 하거나, 사내 지식을 벡터 DB에 꽂아 넣어 RAG 파이프라인을 구축 | "이 개념의 핵심" |
