...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **필요성** | MoE 모델은 총 파라미터 수가 막대하여(수백B ~ Trillion) 단일 GPU 메모리에 적재할 수 없으며, 기존의 텐서 병렬화(TP)만... | "그래픽 전문 두뇌" |
| **작동 원리** | 1. 모델 가중치 분산: 64개의 Expert가 있다면, 8장의 GPU에 각각 8개씩의 Expert를 배치 | "그래픽 전문 두뇌" |
| **구체 예시** | GPT-4 (추정 1 | "텍스트 예측기" |
| **흔한 오해/주의점** | EP는 연산을 분산시켜 주지만, 그 대가로 '네트워크 통신 비용'을 크게 지불함 | "핵심 기술 요소" |
| **Mixture of Experts (MoE)** | 전문가 병렬화가 적용되는 대상 아키텍처 | "핵심 기술 요소" |
| **All-to-All Communication** | EP 구현 시 GPU 간 데이터를 N:N으로 교환하는 집단 통신 연산 | "그래픽 전문 두뇌" |
| **3D Parallelism (TP + PP + DP)** | EP와 결합하여 대규모 클러스터(4D Parallelism)를 구성하는 기본 병렬화 기법 | "핵심 기술 요소" |
