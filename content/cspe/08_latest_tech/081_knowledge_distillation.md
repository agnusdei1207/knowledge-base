...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **정의** | 방대한 파라미터를 가진 Teacher 모델의 학습된 지식(출력 확률 분포, 추론 과정)을 경량화된 Student 모델로 전이하는 모델 압축... | "이 개념의 핵심" |
| **배경** | Hard Label(0 또는 1)로만 학습하면 Student는 정답 외의 다른 선택지에 대한 정보(클래스 간 유사도)를 알 수 없음 | "이 개념의 핵심" |
| **작동 원리** | 1. Teacher 모델이 입력 데이터에 대해 Softmax Temperature($T > 1$)를 적용하여 완만한 확률 분포(Soft L... | "이 개념의 핵심" |
| **비유** | 족보의 '답(Hard Label)'만 외우는 것이 아니라, 해설지의 '오답 노트와 풀이 과정(Soft Label & CoT)'까지 통째로 ... | "이 개념의 핵심" |
| **구체 예시** | GPT-4(Teacher, 1 | "이 개념의 핵심" |
| **흔한 오해/주의점** | Teacher 모델이 환각(Hallucination)을 일으키면 Student도 이를 그대로 배움(오류 전파) | "자동 품질 검사 라인" |
| **SLM (Small Language Model)** | 지식증류를 통해 탄생하는 작고 강력한 모델 (예: Phi-3, Gemma) | "설계 도면" |
