...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **배경** | 2024년 마이크로소프트(MS) 연구소가 논문으로 구체화하며 폭발적 관심 | "핵심 기술 요소" |
| **작동 원리** | 1. (사전 작업): LLM을 돌려 문서 전체를 읽고, 모든 인물, 장소, 개념(Node)과 그 사이의 관계(Edge)를 뽑아내어 지식 그... | "현장 가까이에서 처리" |
| **구체 예시** | 수사 기관의 범죄 기록 분석 | "핵심 기술 요소" |
| **흔한 오해/주의점** | Graph RAG가 무조건 더 좋은 것은 아님 | "지하철 노선도" |
| **Knowledge Graph (지식 그래프)** | 노드(개체)와 엣지(관계)로 세상의 지식을 표현하는 그래프 데이터베이스 (예: Neo4j) | "지하철 노선도" |
| **Multi-hop QA (다중 홉 추론)** | 한 번의 검색으로 알 수 없고, A를 찾아 B를 알고, B를 찾아 C를 알아야 하는 연쇄적 질문 | "핵심 기술 요소" |
| **Vector DB (벡터 데이터베이스)** | 의미 기반 1차원 검색을 담당하며, Graph RAG 내에서 노드의 속성을 검색할 때 융합(Hybrid)하여 사용되기도 함 | "지하철 노선도" |
