...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **배경** | LLM 스케일링으로 인해 도메인 특화 모델 개발의 진입 장벽이 너무 높아짐 | "이 개념의 핵심" |
...
| **흔한 오해/주의점** | PEFT가 항상 만능은 아님 | "이 개념의 핵심" |
| **LoRA (Low-Rank Adaptation)** | 현재 PEFT의 제왕이자 가장 널리 쓰이는 기법 | "이 개념의 핵심" |
| **Adapter / Prefix Tuning** | LoRA 이전/유사 카테고리의 대표적 PEFT 기술들 | "이 개념의 핵심" |
| **Multi-tenant Serving** | PEFT를 통해 하나의 베이스 모델로 다수 고객(Tenant)에게 맞춤형 모델을 제공하는 서빙 기법 | "이 개념의 핵심" |
