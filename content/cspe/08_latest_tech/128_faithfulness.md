...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **정의** | RAG 시스템이 뱉어낸 최종 답변이, "오로지 검색해서 찾아온 문서(Context)의 내용에만 기반하고 있는지"를 측정하는 평가 지표 | "핵심 기술 요소" |
| **필요성** | LLM은 본능적으로 자신이 학습했던 사전 지식을 뽐내거나 지어내려는 성향(Hallucination)이 있음 | "자동 품질 검사 라인" |
| **핵심 직관** | 형사(LLM)가 보고서(답변)를 썼음 | "핵심 기술 요소" |
| **배경** | RAG의 최대 목표는 환각 방지임 | "핵심 기술 요소" |
| **작동 원리 (명제 분해 및 교차 검증)** | 1. (문장 분해): "애플은 1976년에 스티브 잡스가 창립했으며, 현재 CEO는 일론 머스크다"라는 답변이 나옴 | "핵심 기술 요소" |
| **구체 예시** | 사내 메신저 봇. 문서에는 "식대는 1만 원 지원"이라고 적힘 | "핵심 기술 요소" |
