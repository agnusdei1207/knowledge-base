...
## 한눈에
- **정의**: 작은 draft model이 여러 토큰을 미리 제안하고 큰 target model이 한 번에 검증해 디코딩 지연을 줄이는 기법
- **왜 필요한가**: LLM 생성은 토큰을 하나씩 순차 생성하므로, 대형 모델의 decode 단계가 memory-bound 병목이 됨.
- **핵심 직관**: 조수가 답안 초안을 여러 줄 먼저 쓰고, 전문가가 빠르게 검토해 맞는 줄만 채택하는 방식임.

## 깊이 이해
- **배경·문제의식**: Prefill 이후 decode는 매 토큰마다 전체 모델을 호출해야 하며, 70B 모델은 토큰당 20~50ms 지연이 누적됨. 작은 모델이 후보 토큰을 빠르게 생성하면 대형 모델 호출 횟수를 줄일 수 있음.
