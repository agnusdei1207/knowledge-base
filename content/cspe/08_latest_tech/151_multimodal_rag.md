...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **필요성** | 기업의 진짜 핵심 노하우는 줄글(텍스트)이 아니라 엑셀 표, 파워포인트 차트, 기계 설계 도면에 들어 있음 | "핵심 기술 요소" |
| **핵심 직관** | 오픈북 시험의 진화 | "핵심 기술 요소" |
| **배경** | LLM이 VLM(Vision-Language Model, 예: GPT-4V, LLaVA)으로 진화하면서 이미지를 이해할 수 있게 됨 | "설계 도면" |
| **사용자 질의(Query)** | 사용자가 "이 차트에서 3분기 매출이 꺾인 이유는?"이라고 질문(텍스트+이미지 가능) | "도서관 검색" |
| **멀티모달 검색(Retrieval)** | Vector DB에서 텍스트와 이미지 벡터를 동시에 뒤져 가장 연관성 높은 텍스트 덩어리와 이미지 패치를 가져옴 | "핵심 기술 요소" |
| **생성(Generation)** | VLM이 검색된 텍스트와 이미지를 동시에 보고 답변을 생성함 | "핵심 기술 요소" |
| **구체 예시** | 항공기 정비 매뉴얼(PDF) | "핵심 기술 요소" |
