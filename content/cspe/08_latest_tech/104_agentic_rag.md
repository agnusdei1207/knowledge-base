...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **필요성** | 기존 RAG는 한 번 검색해서 답이 없거나 정보가 부족하면 포기함 | "이 개념의 핵심" |
| **핵심 직관** | "A회사와 B회사의 3분기 이익률 차이를 구해줘 | "이 개념의 핵심" |
| **구체 예시** | 고객 질의 "내 작년 세금 내역이랑 최신 세법 비교해서 환급금 계산해 줘 | "이 개념의 핵심" |
| **흔한 오해/주의점** | 똑똑한 만큼 위험함 | "이 개념의 핵심" |
| **Function Calling (함수 호출)** | LLM이 외부 도구(API, 검색기 등)를 선택하고 실행 인자(Arguments)를 구조화해서 내뱉는 핵심 기술 | "식당 메뉴판" |
| **ReAct (Reasoning and Acting)** | 에이전트가 생각하고 행동하고 관찰하는 과정을 명시적으로 프롬프팅하는 기법 | "이 개념의 핵심" |
| **Advanced RAG** | 정해진 파이프라인(Static)을 최대한 고도화한 것 | "이 개념의 핵심" |
