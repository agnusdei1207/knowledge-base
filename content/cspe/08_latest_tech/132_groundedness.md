...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **정의** | AI가 생성한 답변이 "주어진 근거 데이터(Context, 문서, 출처)에 두 발을 단단히 딛고(Grounded) 있는가?"를 뜻하는 포괄... | "학습하는 기계" |
| **필요성** | 기업 환경에서 AI 챗봇이 사내 규정이나 고객 약관을 안내할 때, 그럴듯한 거짓말(환각)을 하면 치명적인 법적/금전적 사고로 이어짐 | "학습하는 기계" |
| **배경** | LLM은 수많은 인터넷 글을 학습했기 때문에, 물어보는 즉시 자기가 아는 일반 상식을 지어내려는 본능(Parametric Knowledge... | "현장 가까이에서 처리" |
| **작동 원리 (Faithfulness와의 관계)** | - 학계/업계에서 `Groundedness`와 `Faithfulness(충실성)`는 종종 동의어로 쓰임 | "학습하는 기계" |
| **구체 예시** | - (Context): "회식비는 인당 3만 원 제한이다 | "핵심 기술 요소" |
| **Faithfulness (충실성)** | Groundedness와 사실상 같은 뜻으로 통용되는 평가 메트릭 용어 | "학습하는 기계" |
| **AI Hallucination (환각)** | Groundedness가 낮다는 것은 100% 환각(Hallucination)이 발생했다는 뜻 | "학습하는 기계" |
