...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **필요성** | 자연어 프롬프트 깎기(Prompt Engineering)는 사람이 일일이 단어를 바꿔가며 운에 의존해야 하고, 전체 모델 파인튜닝은 너무 무거움 | "이 개념의 핵심" |
| **배경** | 구글(2021)에서 제안 | "이 개념의 핵심" |
| **작동 원리** | 1. 모델의 전체 가중치(Transformer layers)는 학습 금지(Frozen) | "건물 층" |
| **구체 예시** | 감정 분류 태스크 | "이 개념의 핵심" |
| **흔한 오해/주의점** | Soft Prompt는 자연어 단어(Hard Prompt)가 아님 | "이 개념의 핵심" |
| **Prompt Engineering** | 사람이 직접 단어를 고르는 것 (Hard Prompting) | "이 개념의 핵심" |
| **Prefix Tuning** | 입력단이 아닌, 모든 레이어에 프롬프트(Prefix)를 박아넣는 심화 기법 | "이 개념의 핵심" |
