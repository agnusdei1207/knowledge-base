...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **가치** | GPTQ는 뇌 구조를 헤집어놔서(오차 보정) VRAM에서 압축을 풀 때(Dequant) 딜레이가 걸렸다 | "이 개념의 핵심" |
| **배경** | GPTQ가 허깅페이스를 지배하고 있었지만 맹점이 있었다 | "이 개념의 핵심" |
| **필요성** | **"수학 공식 복잡하게 쓰지 말고, 그냥 냅다 반올림(RTN)으로 짓누르자 | "이 개념의 핵심" |
| **1단계 [Activation 기반 탐색]** | 기존의 바보 같은 양자화 기법들은 뇌세포의 원래 숫자(Weight Magnitude)가 크면 중요하다고 착각했다 | "이 개념의 핵심" |
| **2단계 [Channel-wise Scaling]** | 핵심 1%를 찾았다고 걔네만 16비트(FP16)로 놔두면(LLM | "이 개념의 핵심" |
| **3단계 [하드웨어 커널 친화력 (Hardware-Friendly)]** | 모델 가중치(W)의 값 자체를 더하고 빼며 변형시킨 GPTQ와 달리, AWQ는 W 값은 그대로 둔 채 단순히 곱하기(Scale factor... | "건물 증축" |
| **연결 포인트** | 이 AWQ의 최대 수혜자는 바로 엔터프라이즈 B2B 시장이다 | "이 개념의 핵심" |
