...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **핵심 직관** | 압축 해제 마술. 원본 큰 도화지($512 \times 512$)에 직접 그림을 그리지 않음 | "핵심 기술 요소" |
| **배경** | LMU(뮌헨 대학교)와 Runway 연구진이 2021년 제안한 모델 | "핵심 기술 요소" |
| **작동 원리 (Perceptual Compression)** | 1. **압축 (Encoder)**: 먼저 VAE(Variational Autoencoder)를 사전 학습시켜 놓음 | "압축→복원" |
| **복원 (Decoder)** | 노이즈가 완벽히 제거된 $64 \times 64$ 잠재 텐서를 VAE 디코더에 통과시켜 원래의 $512 \times 512$ 고해상도 픽셀... | "명령서 해석" |
| **구체 예시** | 고해상도 영화 편집을 할 때 4K 원본 파일을 그대로 편집 프로그램에 돌리면 렉이 걸림 | "핵심 기술 요소" |
| **Stable Diffusion** | Latent Diffusion 기술을 상용화 수준으로 크게 스케일업(Scale-up)하여 배포한 실제 프로덕트(모델)의 이름 | "건물 증축" |
| **VAE (Variational Autoencoder)** | 픽셀을 Latent(잠재) 차원으로 찌그러뜨리고, 다시 픽셀로 복원해 주는 핵심 압축/해제기 | "압축→복원" |
