...
- 개요: 8-bit 정수 기반 모델 압축 기법
- 배경: FP16 모델은 메모리 사용량과 대역폭 부담이 커서 엣지·서버 추론 비용과 지연을 증가시킴.
- 필요성: INT8 kernel, calibration dataset, per-channel scale로 FP16 대비 메모리 50% 절감과 정확도 회귀 기준을 검증해야 함.
