...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **핵심 직관** | 인트라넷. 우리 집 거실에서만 돌아가는 개인용 컴퓨터 | "이 개념의 핵심" |
| **배경** | 초기 LLM 시장은 초거대 파라미터가 필요한 SaaS 중심이었음 | "이 개념의 핵심" |
| **구체 예시** | 한국군(국방부)에서 작전 교범 파일(비밀 문서)을 검색하는 챗봇을 만듦 | "이 개념의 핵심" |
| **sLLM (Small LLM) / 오픈소스 모델** | Private RAG 구축의 핵심 심장 | "이 개념의 핵심" |
| **vLLM / TGI (추론 최적화)** | 사내 GPU의 한정된 자원을 쥐어짜서 빠르게 답변을 뱉어내게 만드는 런타임 엔진 | "이 개념의 핵심" |
| **Data Sovereignty (데이터 주권)** | 데이터가 해외 서버에 저장되지 않고 자국/자사 내에 머물러야 한다는 컴플라이언스 원칙 | "이 개념의 핵심" |
| **모델 경량화 (Quantization)** | - 70B 모델을 그대로 올리면 GPU 메모리(VRAM) 수백 GB가 필요함 | "이 개념의 핵심" |
