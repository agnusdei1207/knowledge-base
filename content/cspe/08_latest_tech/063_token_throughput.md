...
| **필요성** | 어떻게든 GPU 1대에 유저 50명, 100명을 한꺼번에 우겨넣어(Batching) 동시에 처리하게 만들어야 했다 | "그래픽 전문 두뇌" |
...
| **3단계 [유휴 연산기 척결 (Continuous Batching)]** | 배치를 묶어놔도, 짧은 대답이 먼저 끝나면 GPU 일부가 놀게 된다 | "그래픽 전문 두뇌" |
| **적용 사례 1 (B2B 데이터 정제 파이프라인 - 오프라인 서빙)** | 실시간 챗봇이 아니라, 밤사이에 100만 건의 리뷰 데이터를 요약해야 하는 배치(Offline Batch) 시스템이 있다 | "핵심 기술 요소" |
