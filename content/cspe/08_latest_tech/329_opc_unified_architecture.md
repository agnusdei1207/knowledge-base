...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **개요** | 산업 장비와 IT 시스템이 데이터를 의미 있는 객체로 교환하는 IEC 62541 기반 표준 | "이 개념의 핵심" |
| **왜 필요한가** | PLC, 로봇, 센서, SCADA, MES가 제조사별 프로토콜로 분리되면 설비 데이터를 통합하기 어렵다 | "이 개념의 핵심" |
| **핵심 직관** | 여러 나라 사람이 공통 언어와 신분증 체계로 대화하듯, 산업 장비가 표준 주소공간과 보안 체계로 데이터를 교환한다 | "이 개념의 핵심" |
| **배경·문제의식** | OPC Classic은 Windows COM/DCOM 의존성이 커서 플랫폼 독립성과 인터넷 규모 보안 요구를 만족하기 어려웠다 | "이 개념의 핵심" |
| **비유** | 전화번호부에 사람 이름, 부서, 연락처, 권한이 정리되어 있으면 앱이 같은 방식으로 사람을 찾는 것과 같다 | "이 개념의 핵심" |
| **구체 예시** | MES가 OPC UA 서버를 통해 CNC 장비의 spindle speed, alarm code, production count를 구독해 생... | "이 개념의 핵심" |
| **흔한 오해·주의점** | OPC UA는 단순 전송 프로토콜만이 아니다 | "이 개념의 핵심" |
