...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **필요성** | Wasm은 태생적으로 브라우저용이라 파일 하나 읽는 기능조차 없었다 | "핵심 기술 요소" |
| **배경(왜 등장했나?)** | 개발자들이 Wasm의 미친듯한 속도(밀리초 부팅)와 경량성(1MB)을 보고 열광했다 | "핵심 기술 요소" |
| **작동 원리(어떻게 달성했나?)** | 1. 개발자는 파일 읽기 함수(예: `fopen`)를 써서 C 코드를 짠 뒤, 타겟을 `wasm32-wasi`로 지정하여 컴파일한다 | "핵심 기술 요소" |
| **일상 비유** | '범용 전원 어댑터(돼지코)'와 같다 | "핵심 기술 요소" |
| **구체 예시** | Docker 창시자인 솔로몬 하익스는 Wasm을 극찬하며 'Fermyon' 같은 회사를 세웠다 | "컨테이너 플랫폼" |
| **흔한 오해/주의점** | "WASI를 쓰면 해커가 파일 시스템을 다 털 수 있지 않나?" → 오히려 반대다! WASI는 **'역량 기반 보안(Capability-b... | "핵심 기술 요소" |
| **WebAssembly (Wasm)** | 브라우저 내에서 빠른 연산을 위해 만들어진 바이너리 런타임 포맷 | "핵심 기술 요소" |
