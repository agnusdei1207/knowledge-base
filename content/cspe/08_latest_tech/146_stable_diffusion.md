...
|:---|:---|:---|
| 잠재공간 효율 구조 이해 확인 | VAE latent에서 U-Net denoising, 단일 GPU 생성 가능 | 픽셀 공간 확산과 latent 확산 구조 혼동 |
| 생태계 확장성 이해 확인 | LoRA는 스타일·도메인 학습, ControlNet은 구도·포즈 조건 제어 | LoRA와 ControlNet 역할 혼동 |
| 운영 통제 판단 확인 | 모델 라이선스, NSFW 필터, 워터마킹 정책 | 공개 모델·확장 모듈 라이선스 검토 누락 |

> 요약: 이 문제는 공개 모델 소개가 아니라 latent 구조의 효율 원리와 확장·운영 통제 설계를 묻는다.
...
| 비교 축 | 프롬프트 제어만 사용 | LoRA·ControlNet 결합 | 선택 기준 |
|:---|:---|:---|:---|
| 스타일 일관성 | 시드·프롬프트 편차로 재현 한계 | 학습된 스타일과 구도 조건 고정 | 브랜드 일관성 요구 수준 |
| 준비 비용 | 즉시 사용 가능 | 스타일 이미지 수십~수백 장 확보·학습 필요 | 도메인 데이터 확보 여부 |
| 운영 관리 | 프롬프트 템플릿 관리 | 어댑터 버전·조합 관리 부담 | 모델 자산 관리 체계 |

> 요약: 일회성 시안은 프롬프트 제어로 충분하고, 반복 상용 제작은 LoRA·ControlNet으로 재현성을 확보함.

| 리스크 | 원인 | 대응 방안 | 확인 지표 |
|:---|:---|:---|:---|
| 라이선스 위반 | 모델·LoRA별 상용 조건 상이 | 라이선스 인벤토리, 법무 검토 | 승인 모델 외 사용 0건 |
| 미검증 모델 반입 | 커뮤니티 체크포인트·LoRA 무단 사용 | 내부 검증 후 등록제 운영 | 미승인 모델 사용 0건 |
| 세부 왜곡(손·문자) | latent 압축 손실, 학습 데이터 한계 | 후처리 보정, 사람 검수 게이트 | 검수 통과율 |

> 요약: 공개 생태계의 핵심 위험은 라이선스와 미검증 모델이며, 등록제와 검수 게이트로 통제함.
