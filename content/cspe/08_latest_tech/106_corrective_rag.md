...
| **배경** | 2024년 1월, 구글과 칭화대 연구진이 제안(CRAG) | "핵심 기술 요소" |
| **흔한 오해/주의점** | 평가 모델(Evaluator)의 정확도가 전체 시스템의 목숨줄임 | "계산기" |
| **Self-RAG** | 생성된 토큰 단위로 스스로 반성하는 방식 | "핵심 기술 요소" |
| **Web Search Fallback** | 사내 데이터에 답이 없을 때 외부 지식으로 전환하는 CRAG의 핵심 교정(Correction) 기법 | "보물찾기" |
| **Knowledge Refinement** | 문서 내 핵심 문장(Strip)만 추출하여 토큰을 압축하고 노이즈 제거 / | "현장 가까이에서 처리" |
| **Hybrid Combine** | 기존 문서의 정제된 요약본과 웹 검색 결과를 융합(Combine)하여 주입 / | "핵심 기술 요소" |
