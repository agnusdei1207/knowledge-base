...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **필요성** | 파인튜닝 시 수백억 개의 파라미터를 다 업데이트하려면 엄청난 자원이 듦 | "핵심 기술 요소" |
| **배경** | Microsoft Research(2021)에서 발표 | "건물 관리실" |
| **작동 원리** | 1. Base 모델의 거대 가중치 $W (d \times k)$는 업데이트 중지(Freeze) | "핵심 기술 요소" |
| **비유** | 100만 화소 이미지를 통째로 수정하는 대신, 100만 화소 중 본질적으로 변화가 일어나는 10개의 핵심 벡터(저차원)만 뽑아서 튜닝한 뒤... | "핵심 기술 요소" |
| **구체 예시** | $d=4096$ 차원의 어텐션 가중치 행렬 $W$ (파라미터 1,600만 개) | "핵심 기술 요소" |
| **흔한 오해/주의점** | Rank $r$을 무조건 키운다고(예: r=256) 성능이 좋아지지 않음 | "핵심 기술 요소" |
| **PEFT** | LoRA를 포괄하는 상위 개념(파라미터 효율적 튜닝) | "핵심 기술 요소" |
