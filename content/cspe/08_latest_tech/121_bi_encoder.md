...
| **필요성** | Cross-Encoder는 질문과 문서를 묶어서 아주 정확히 심사하지만 너무 느려서 수백만 문서를 검색할 수 없음 | "이 개념의 핵심" |
| **핵심 직관** | 얼굴 인식 출입문. 직원들의 얼굴(문서) 사진을 미리 다 찍어서 데이터베이스(Vector DB)에 저장해 둠 | "이 개념의 핵심" |
| **배경** | 밀집 검색(Dense Retrieval) 시대가 열리며(DPR 논문 등), 수천만 건의 문서를 딥러닝으로 "실시간 검색"할 수 있게 만든 ... | "이 개념의 핵심" |
| **작동 원리 (미리 구워놓기 전략)** | 1. (오프라인): 코퍼스의 모든 문서를 BERT 모델(Document Encoder)에 밀어 넣어 768차원 벡터로 '미리' 만들어 Ve... | "이 개념의 핵심" |
| **구체 예시** | Sentence-BERT(SBERT)가 가장 대표적인 Bi-Encoder임 | "이 개념의 핵심" |
| **흔한 오해/주의점** | "독립적"으로 임베딩된다는 것은, 질문의 단어(예: 파이썬)가 문서의 단어(예: 코드)와 실시간으로 문맥을 교환하지 못한다는 뜻 | "이 개념의 핵심" |
