...
| **핵심 직관** | 금고에 보관할 때와 운반할 때만 잠그는 것이 아니라, 계산하는 순간에도 외부 관리자와 호스트 OS가 내용을 보지 못하게 격리한다 | "건물 관리실" |
| **배경·문제의식** | 기업은 민감 데이터와 독점 모델을 클라우드 GPU에서 처리하려 하지만 인프라 운영자, 하이퍼바이저, 다른 tenant에 대한 노출 우려가 있다 | "그래픽 전문 두뇌" |
| **비유** | 요리 재료와 레시피를 투명 주방에 두지 않고, 검증된 잠금 주방 안에서만 조리하고 완성품만 내보내는 방식이다 | "핵심 기술 요소" |
