...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **가치** | A 유저가 10만 자짜리 PDF를 넣고 "1쪽 요약해 줘"라고 했다 | "이 개념의 핵심" |
| **배경** | 기업용 AI 챗봇을 만들 때, 개발자는 유저의 질문 창 뒤에 보이지 않는 거대한 '시스템 프롬프트(예: 회사의 30페이지짜리 응대 매뉴얼)... | "학습하는 기계" |
| **1단계 [트리 구조의 라우팅 (Radix Tree)]** | vLLM 등의 엔진은 들어오는 프롬프트를 단어 단위로 쪼개어 트리(Radix Tree) 자료구조에 저장한다 | "가계도" |
| **2단계 [페이지드 어텐션 기반 공유 (Sharing)]** | 똑같은 부분(예: 앞 10,000토큰)을 찾았다면, 물리 메모리(VRAM)를 또 낭비할 필요가 없다 | "이 개념의 핵심" |
| **연결 포인트** | 롱 컨텍스트 시대(100만 토큰)가 열리면서 이 Prefix Caching은 '선택'이 아니라 '생존'이 되었다 | "이 개념의 핵심" |
| **적용 사례 1 (Anthropic Claude 3.5의 프롬프트 캐싱)** | 2024년 말, Anthropic과 OpenAI는 자사 API에 `Prompt Caching` 기능을 공식 출시했다 | "식당 메뉴판" |
| **적용 사례 2 (Multi-turn 대화 기록 유지)** | 유저가 챗봇과 긴 티키타카(Multi-turn) 대화를 나눌 때, LLM은 앞선 대화 기록을 통째로 묶어서 계속 다시 읽어야 한다(Hist... | "이 개념의 핵심" |
