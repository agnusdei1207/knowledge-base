...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **개요** | DB 변경분을 감지해 이벤트나 증분 데이터로 전달하는 데이터 동기화 기법 | "이 개념의 핵심" |
| **왜 필요한가** | 매번 전체 테이블을 복사하면 원천 DB 부하, 지연, 중복 처리, 삭제 반영 누락 문제가 생긴다 | "이 개념의 핵심" |
| **핵심 직관** | 전체 장부를 매번 복사하지 않고 새로 적힌 거래와 수정·삭제 내역만 복사하는 방식임 | "이 개념의 핵심" |
| **배경·문제의식** | 데이터 웨어하우스, 검색색인, 캐시, 이벤트 기반 서비스는 원천 DB 변경을 업무 지연 SLA 안에서 받아야 한다 | "이 개념의 핵심" |
| **비유** | 신문 전체를 매일 다시 인쇄하지 않고 정정 기사와 신규 기사 목록만 배포해 구독자가 자기 사본을 갱신하는 방식이다 | "이 개념의 핵심" |
