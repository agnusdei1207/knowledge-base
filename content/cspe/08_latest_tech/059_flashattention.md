...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **가치** | 이 기술이 등장하면서, 어텐션 계산 속도가 기존 대비 최대 4배 폭발했고, 메모리 사용량은 1/20로 줄어들었다 | "핵심 기술 요소" |
| **배경** | 트랜스포머의 어텐션은 행렬 내적 연산( $Softmax(QK^T)V$ )을 한다 | "핵심 기술 요소" |
| **1단계 [Tiling (타일링)]** | 전체 행렬은 거대한 SRAM(캐시)에 한 번에 못 올린다 | "칠판" |
| **3단계 [Recomputation (역전파 재계산)]** | 학습(Training) 시 그래디언트(기울기)를 계산하려면 아까 지워버린 '중간 결과'가 필요하다 | "학습하는 기계" |
| **연결 포인트** | LLM을 훈련할 때 FlashAttention을 안 쓰면 바보다 | "수정 가능 돌판" |
| **적용 사례 1 (HuggingFace Transformers의 가속)** | 개발자가 파이썬으로 허깅페이스 모델을 불러올 때, 코드 한 줄 `attn_implementation="flash_attention_2"` ... | "수정 가능 돌판" |
| **적용 사례 2 (최신 FlashAttention-3)** | 엔비디아의 최신 칩인 H100 시리즈(Hopper 아키텍처)의 특수 기능(TMA - 비동기 데이터 전송)을 극한으로 활용하기 위해 v3가 나왔다 | "수정 가능 돌판" |
