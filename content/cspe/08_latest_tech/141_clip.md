...
| **핵심 직관** | 틀린 그림 찾기 훈련 | "핵심 기술 요소" |
| **배경** | 지도학습(Supervised Learning)의 라벨링 노가다와, 닫힌 범주(Closed-set) 분류의 한계를 부수기 위해 등장 | "건물 관리실" |
| **작동 원리 (행렬 곱셈 기반의 Loss 연산)** | 1. $N$개의 이미지와 $N$개의 텍스트(총 $N$ 쌍)를 한 번에 입력(Batch) | "건물 관리실" |
| **구체 예시 (Zero-shot 분류)** | 강아지 사진을 넣고, 모델한테 텍스트로 ["이건 고양이 사진", "이건 강아지 사진", "이건 로켓 사진"] 3개를 줌 | "핵심 기술 요소" |
| **흔한 오해/주의점** | CLIP 자체는 이미지를 새로 "그리거나", 글을 "창작"하는 모델이 아님 | "인터넷 주소" |
