...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **필요성** | 기존 IT 시스템은 코드가 정해진 대로만 움직이므로, 방화벽을 치고 비밀번호만 잘 잠그면 막을 수 있었습니다 | "핵심 기술 요소" |
...
| **가치** | 미지의 영역(블랙박스)에 대한 안전장치 | "핵심 기술 요소" |
| **데이터 오염 공격 (Data Poisoning)** | 해커가 인터넷에 고의로 잘못된 정보나 쓰레기 데이터를 잔뜩 뿌려둡니다 | "핵심 기술 요소" |
| **백도어 (Backdoor / 트로이 목마)** | 특정 비밀 단어("XYZ")를 들으면 원래 성격이 변해서 무조건 "해커 만세!"라고 외치도록 학습 데이터에 몰래 최면 스위치를 심어둡니다 | "핵심 기술 요소" |
