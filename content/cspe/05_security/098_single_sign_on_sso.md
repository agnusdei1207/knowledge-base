...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **개요** | SSO는 한 번의 IdP 인증으로 여러 서비스에 접근하는 인증 연계 구조 | "이 개념의 핵심" |
| **왜 필요한가** | 사용자가 서비스마다 비밀번호를 관리하면 재사용, 퇴사자 계정 잔존, 감사 누락 문제가 생긴다 | "이 개념의 핵심" |
| **핵심 직관** | SSO는 "한 번 로그인하면 모든 권한이 열림"이 아니라 "인증은 중앙화하고, 인가는 각 서비스가 정책으로 판단"하는 구조임 | "이 개념의 핵심" |
| **배경·문제의식** | 기업 SaaS와 내부 시스템이 늘어나면 계정 생성·변경·삭제가 분산된다 | "이 개념의 핵심" |
| **작동 원리** | 사용자가 SP에 접근하면 SP는 IdP로 리다이렉트한다 | "이 개념의 핵심" |
| **비유** | 회사 본관에서 출입증을 한 번 발급받고 여러 회의실에 들어가지만, 회의실마다 출입 가능 부서는 별도 확인하는 구조임 | "이 개념의 핵심" |
| **구체 예시** | 직원 5,000명, SaaS 30개 조직에서 IdP 세션 8시간, SP 세션 1시간, 퇴사자 deprovisioning 4시간 SLA, 관... | "이 개념의 핵심" |
