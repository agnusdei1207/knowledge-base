...
SSO는 한 번의 인증으로 여러 서비스에 접근하게 하는 인증 연계 구조이다. IdP를 중심으로 계정·MFA·접속 로그를 통합하고, SP는 검증된 인증 결과를 서비스 세션과 권한 정책으로 변환한다. SaaS 확산, 제로트러스트, 퇴사자 접근 차단 요구로 필요성이 커짐.
