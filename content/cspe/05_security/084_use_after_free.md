...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **개요** | 해제된 메모리를 남은 포인터로 다시 사용해 데이터 변조, 타입 혼동, 코드 실행을 유발하는 취약점 | "이 개념의 핵심" |
| **왜 필요한가** | 브라우저, 커널, C++ 서버, 드라이버는 객체 수명 관리가 복잡해 dangling pointer 1건이 RCE나 권한 상승으로 이어질 수 있음 | "이 개념의 핵심" |
| **핵심 직관** | 반납한 사물함 열쇠를 계속 들고 있다가, 새 사용자의 사물함을 열어 내용물을 바꾸는 상황임 | "이 개념의 핵심" |
| **배경·문제의식** | `free` 또는 객체 소멸 후 포인터가 null 처리되지 않거나 ownership이 중복되면 dangling pointer가 남는다 | "이 개념의 핵심" |
| **비유** | 호텔 키를 반납했지만 복제키를 갖고 있다가 다음 투숙객 방에 들어가는 것과 같다 | "이 개념의 핵심" |
| **구체 예시** | 객체 A를 `delete`한 뒤 이벤트 콜백이 A 포인터를 보관하고 있으면, 같은 heap slot에 객체 B가 배치된 후 콜백이 B의 v... | "이 개념의 핵심" |
| **흔한 오해·주의점** | null 초기화만으로 충분하지 않다 | "이 개념의 핵심" |
