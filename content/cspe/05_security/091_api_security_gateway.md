...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **개요** | API 앞단에서 인증, 인가, 호출량, 입력 스키마, 공격 패턴, 로그를 한 번에 집행하는 보안 경계 | "식당 메뉴판" |
| **왜 필요한가** | MSA와 외부 연계 API는 수십 개 서비스로 분산된다 | "식당 메뉴판" |
| **핵심 직관** | API Gateway는 건물 1층 보안검색대이고, 각 서비스는 층별 출입문이다 | "국경 검문소" |
| **배경·문제의식** | API는 모바일 앱, 파트너, 내부 서비스, 자동화 도구가 직접 호출한다 | "식당 메뉴판" |
| **작동 원리** | Gateway는 TLS 종료 후 OAuth2/OIDC JWT, API Key, mTLS 인증서를 검증한다 | "신분증 확인" |
| **비유** | 공항 출국장처럼 여권 확인, 탑승권 확인, 수하물 검사, 대기열 제한, CCTV 기록이 한 곳에서 먼저 수행되는 구조이다 | "핵심 기술 요소" |
| **흔한 오해·주의점** | Gateway만 통과하면 백엔드는 신뢰해도 된다는 판단은 위험하다 | "국경 검문소" |
