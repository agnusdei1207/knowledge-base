...
AIoT 보안은 엣지 AI가 탑재된 IoT 디바이스의 전 생명주기 보안이다. AIoT는 센서 수집, 로컬 추론, 클라우드 연동, OTA 업데이트가 연속되므로 장치 인증과 모델 무결성을 동시에 보장해야 한다. 스마트홈, 스마트시티, 공장 카메라, 헬스케어 디바이스는 물리 탈취와 원격 변조를 모두 고려해야 한다.
