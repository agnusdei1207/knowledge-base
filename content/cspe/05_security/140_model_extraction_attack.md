...
모델 추출 공격은 모델 API 응답을 이용해 원본과 유사한 복제 모델을 만드는 공격이다.
AI 모델은 학습 데이터·GPU 비용·튜닝 노하우가 결합된 핵심 자산이며, API 응답만으로도 기능 복제가 가능하다.
서비스 사업자는 모델 IP, 과금 회피, 경쟁 서비스 복제, 후속 우회 공격을 막기 위해 추출 방어를 설계해야 한다.
