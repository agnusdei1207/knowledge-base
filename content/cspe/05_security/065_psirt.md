...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **개요** | 제품 취약점 접수부터 공개 권고까지 책임지는 제품 보안 대응 조직 | "이 개념의 핵심" |
| **왜 필요한가** | 제품 취약점은 고객 시스템, 공급망, 규제 보고, 브랜드 신뢰에 영향을 준다 | "이 개념의 핵심" |
| **핵심 직관** | 제품에서 결함이 발견되면 고객센터가 아니라 전문 사고 지휘소가 원인 분석, 수리 일정, 고객 공지를 조율하는 구조임 | "이 개념의 핵심" |
| **배경·문제의식** | 보안 연구자, 고객, 내부 테스트, bug bounty에서 취약점이 제보되면 개발팀만으로는 재현, 심각도, 공개 일정, 법무 검토를 동시에... | "이 개념의 핵심" |
| **비유** | 자동차 리콜 조직과 같다 | "이 개념의 핵심" |
| **흔한 오해·주의점** | PSIRT는 SOC나 CSIRT와 다르다 | "이 개념의 핵심" |
| **본질** | PSIRT(Product Security Incident Response Team)는 제품 취약점 접수, 분석, 조치, 공개, 고객 지원을... | "자동 품질 검사 라인" |
...
- 개요: 제품 보안 대응 조직
- 배경: 제품 취약점은 고객 운영환경, 공급망, 규제 보고에 영향을 주므로 일반 개발 결함처럼 내부 이슈로만 처리할 수 없음.
- 필요성: ISO/IEC 30111·29147 절차에 맞춰 취약점 접수, CVSS 산정, CVE 발급, advisory, 패치 배포를 PSIRT가 단일 창구로 통제해야 함.
