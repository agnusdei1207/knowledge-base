...
- 개요: 봇 요청 억제 통제
- 배경: 웹 서비스는 로그인, 가입, 결제, 댓글 API에서 credential stuffing, 스팸, 재고 선점, 스크래핑 자동화 요청을 받는다.
- 필요성: OWASP Automated Threats 대응 기준에 맞춰 위험 요청에 challenge 또는 점수 기반 검증을 적용하고 우회율·오탐률을 운영 지표로 관리해야 한다.
