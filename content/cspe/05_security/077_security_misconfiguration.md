...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **개요** | 시스템·클라우드·미들웨어 설정값이 보안 기준과 달라 공격면을 여는 취약점 | "이 개념의 핵심" |
| **왜 필요한가** | 애플리케이션 코드가 안전해도 default account, debug mode, public bucket, 열린 관리 포트 하나로 침해가 발생함 | "이 개념의 핵심" |
| **핵심 직관** | 문은 튼튼하지만 뒷문 열쇠가 기본값이고 창문이 열린 상태면 건물 보안은 무너짐 | "이 개념의 핵심" |
| **배경·문제의식** | 현대 시스템은 OS, WAS, DB, Kubernetes, IAM, object storage, CDN 등 설정 지점이 많다 | "자동 배송 시스템" |
| **비유** | 호텔 객실마다 문 잠금, 금고 비밀번호, CCTV 각도가 표준과 달라지면 한 객실의 실수가 전체 브랜드 사고로 이어지는 구조임 | "이 개념의 핵심" |
| **흔한 오해·주의점** | 오설정은 설치 직후만의 문제가 아니다 | "이 개념의 핵심" |
