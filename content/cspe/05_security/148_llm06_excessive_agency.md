...
과도한 에이전시는 LLM Agent의 기능·권한·자율성이 업무 필요 범위를 초과한 상태이다. Agent는 자연어 판단 후 API 호출, DB 변경, 메일 발송, 결제 같은 행위를 수행한다. 따라서 모델 오류·프롬프트 인젝션·도구 오작동이 실제 자산 변경으로 이어지지 않도록 실행 전 독립 통제가 필요함.
