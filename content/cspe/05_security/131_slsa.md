...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **필요성** | 솔라윈즈 사태 이후 SBOM(재료 영수증)이 유행했지만, 한 가지 문제가 남았습니다 | "이 개념의 핵심" |
| **핵심 직관** | **"식당 위생 등급 마크 (SLSA)"** | "이 개념의 핵심" |
| **등장 배경** | 소프트웨어 해킹의 트렌드가 "다 만들어진 앱을 뚫는 것"에서 "앱을 만드는 과정(Supply Chain)에 바이러스를 섞는 것"으로 바뀌었습니다 | "학습하는 기계" |
| **가치** | "출처의 증명(Provenance)" | "이 개념의 핵심" |
| **구체적 사례** | - 최근 오픈소스 생태계(npm, PyPI)에서는 이 SLSA 프레임워크를 의무화하는 움직임이 활발합니다 | "이 개념의 핵심" |
| **주의점 및 흔한 오해** | - "SLSA Level 4면 악성코드가 절대 없다는 뜻인가요?" $\rightarrow$ **아닙니다 | "이 개념의 핵심" |
| **SBOM (무엇이 들었나?)** | "밀가루, 설탕, 땅콩이 들어있음" (재료 명세서) | "이 개념의 핵심" |
