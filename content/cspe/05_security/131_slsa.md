...
SLSA는 소프트웨어 공급망 무결성 프레임워크임.
오픈소스 의존성, CI/CD, 컨테이너 레지스트리, 배포 자동화가 연결되면서 코드 작성 후 운영 배포 전까지 변조 지점이 증가함.
SLSA는 빌드 provenance와 성숙도 요구사항으로 결과물이 신뢰 가능한 절차에서 생성됐는지 검증하게 함.
