...
비밀 관리는 자격증명 생명주기 통제임.
애플리케이션과 CI/CD가 DB, 메시지 큐, SaaS API, 클라우드 계정에 접근하려면 비밀이 필요하며, 소스코드·로그·이미지에 남으면 lateral movement 경로가 됨.
Vault·AWS Secrets Manager는 암호화 저장, 동적 발급, 자동 회전, 감사 로그로 비밀 노출 시간을 줄임.
