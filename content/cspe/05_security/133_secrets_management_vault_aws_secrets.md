...
| **핵심 직관** | 사무실 열쇠를 개인 책상에 보관하지 않고, 출입증 발급소에서 필요한 시간만 대여하고 반납 기록을 남기는 방식임 | "핵심 기술 요소" |
| **배경·문제의식** | 하드코딩된 비밀번호는 커밋 이력과 이미지 레이어에 남고, 한 번 유출되면 모든 배포본을 교체해야 함 | "핵심 기술 요소" |
| **작동 원리** | Vault는 secret engine, auth method, policy, lease TTL로 동적 비밀을 발급·폐기함 | "신분증 확인" |
| **비유** | 호텔 카드키처럼 숙박 기간 동안만 열리고 체크아웃 후 자동 폐기되는 접근권한과 유사함 | "핵심 기술 요소" |
| **흔한 오해·주의점** | 비밀을 중앙 저장소에 넣는 것만으로 충분하지 않음 | "핵심 기술 요소" |
