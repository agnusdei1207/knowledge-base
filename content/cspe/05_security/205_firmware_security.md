...
펌웨어 보안은 장치 펌웨어의 비밀값, 무결성, 업데이트, 저장소를 통제하는 보안 활동이다.
펌웨어는 부팅·센서·네트워크·키 저장을 제어하므로 변조 시 장치 전체 권한이 침해된다.
대량 배포 장치는 동일 이미지 재사용 비율이 높아 CWE-798과 unsigned OTA가 전체 제품군 리스크로 전이된다.
