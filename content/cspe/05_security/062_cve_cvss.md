...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **개요** | CVE는 취약점 이름표, CVSS는 취약점 심각도 계산 체계 | "이 개념의 핵심" |
| **왜 필요한가** | 조직은 매일 다수의 취약점 알림을 받는다 | "이 개념의 핵심" |
| **핵심 직관** | CVE는 환자 등록번호이고, CVSS는 증상의 심각도를 표준 항목으로 점수화한 진단표임 | "이 개념의 핵심" |
| **배경·문제의식** | 벤더마다 취약점 명칭이 다르면 동일 이슈를 추적하기 어렵다 | "이 개념의 핵심" |
| **작동 원리** | CVSS 3.1은 Base, Temporal, Environmental로 구성된다 | "이 개념의 핵심" |
| **비유** | 같은 화재라도 건물 종류와 사람 밀집도에 따라 대응 우선순위가 달라진다 | "이 개념의 핵심" |
| **구체 예시** | CVSS 9.8 RCE라도 내부망 개발 서버면 30일 SLA가 가능할 수 있고, CVSS 7 | "이 개념의 핵심" |
...
- 개요: 취약점 식별·점수 표준
- 배경: 동일 취약점이 벤더 공지, 스캐너, 패치 시스템에서 다른 이름으로 관리되면 영향 제품과 조치 우선순위가 흔들림.
- 필요성: CVE ID, CVSS v3.1/v4.0, NVD feed를 ITSM·패치 관리와 연결해 심각도 9.0 이상 취약점을 별도 SLA로 처리해야 함.
