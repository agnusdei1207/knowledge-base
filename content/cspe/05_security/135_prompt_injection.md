...
프롬프트 인젝션은 LLM 지시 교란 공격임.
LLM 애플리케이션은 사용자 입력, RAG 문서, 웹 콘텐츠, tool output을 같은 프롬프트 문맥에 넣으므로 공격자가 악성 지시를 자연어로 섞을 수 있음.
Agent가 메일·결제·파일·DB 권한을 갖는 경우 prompt injection은 데이터 유출과 비인가 작업으로 연결됨.
