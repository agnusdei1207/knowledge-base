...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **개요** | 사용자가 비밀번호를 입력하지 않고 장치의 FIDO credential로 로그인하는 인증 방식 | "이 개념의 핵심" |
| **왜 필요한가** | 비밀번호는 재사용, 유출, 피싱, reset 공격의 중심이다 | "이 개념의 핵심" |
| **핵심 직관** | 사용자는 사이트마다 다른 디지털 열쇠를 갖고 있고, 로그인할 때 열쇠 자체가 아니라 서명 결과만 보여준다 | "이 개념의 핵심" |
| **배경·문제의식** | FIDO2는 피싱 저항성이 있으나 보안키 보급과 분실 복구가 장벽이었다 | "이 개념의 핵심" |
| **작동 원리** | 등록 시 사이트(RP)는 passkey credential을 만들고 공개키를 저장한다 | "이 개념의 핵심" |
| **비유** | 비밀번호 수첩을 들고 다니는 대신, 휴대폰 지갑 안에 사이트별 열쇠를 넣어두고 문 앞에서 일회용 서명을 제출하는 구조이다 | "이 개념의 핵심" |
| **구체 예시** | synced passkey는 휴대폰과 노트북의 계정 키체인에 동기화되고, device-bound passkey는 특정 보안키나 단말에만 존재한다 | "이 개념의 핵심" |
