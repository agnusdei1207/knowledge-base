...
Secure Boot는 부팅 단계마다 다음 실행 코드의 무결성과 출처를 검증하는 보안 부팅 체계이다.
부트킷은 OS 보안 모듈보다 먼저 실행되어 탐지 회피와 영속화를 수행하므로 부팅 전 검증이 필요하다.
ROM root of trust와 서명 체인이 결합될 때 장치 출고 후 펌웨어 변조 실행을 차단할 수 있다.
