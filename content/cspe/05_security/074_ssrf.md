...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **개요** | 서버가 공격자가 지정한 내부·외부 URL로 대신 요청을 보내는 취약점 | "이 개념의 핵심" |
| **왜 필요한가** | 이미지 가져오기, 웹훅, URL 미리보기, 파일 import 기능은 서버가 네트워크 요청을 수행한다 | "이 개념의 핵심" |
| **핵심 직관** | 공격자가 직접 못 들어가는 내부망에, 접근 권한을 가진 서버를 프록시처럼 이용하는 공격임 | "이 개념의 핵심" |
| **배경·문제의식** | 클라우드와 마이크로서비스 환경은 서버에서 다른 서비스로 나가는 요청이 많다 | "이 개념의 핵심" |
| **작동 원리** | 사용자가 URL을 제출하고 서버가 이를 fetch한다 | "이 개념의 핵심" |
| **비유** | 외부인은 사무실 안 금고에 못 들어가지만, 사무실 직원에게 "이 주소로 택배 좀 보내 달라"고 시켜 내부 문서를 밖으로 보내게 하는 상황임 | "이 개념의 핵심" |
| **구체 예시** | 클라우드 서버에서 `169 | "이 개념의 핵심" |
