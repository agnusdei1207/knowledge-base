...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **개요** | ABAC는 주체·객체·행위·환경 속성을 정책으로 평가하는 접근 제어 모델 | "이 개념의 핵심" |
| **왜 필요한가** | 역할만으로는 "외부망에서 야간에 고객정보 대량 다운로드" 같은 위험 조건을 구분하기 어렵다 | "이 개념의 핵심" |
| **핵심 직관** | "누구 역할인가"만 묻는 RBAC와 달리, ABAC는 "누가, 무엇을, 어떤 상황에서, 어떤 행위로" 접근하는지 판단함 | "이 개념의 핵심" |
| **배경·문제의식** | 클라우드·API·제로트러스트 환경에서는 사용자 역할이 같아도 접속 위치, 단말 보안 상태, 데이터 민감도에 따라 권한이 달라져야 한다 | "식당 메뉴판" |
| **작동 원리** | PEP가 접근 요청을 가로채고 subject, object, action, environment 속성을 수집한다 | "이 개념의 핵심" |
| **비유** | 공항 보안검색이 탑승권만 보지 않고 여권, 목적지, 수하물, 시간, 위험 경보를 함께 확인하는 구조임 | "이 개념의 핵심" |
| **흔한 오해·주의점** | ABAC는 역할을 없애는 모델이 아니다 | "이 개념의 핵심" |
