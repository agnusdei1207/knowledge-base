...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **필요성** | 건물을 다 짓고 나서 "아차, 비상구가 없네" 하고 벽을 부수는 것(사후 침투테스트)은 비용이 엄청납니다 | "이 개념의 핵심" |
| **핵심 직관** | **"보드게임: 은행 털기 시뮬레이션"** | "이 개념의 핵심" |
| **배경** | MS(마이크로소프트)가 과거 윈도우즈의 잦은 해킹을 막기 위해 SDL(Secure Development Lifecycle)을 도입하며 고안... | "이 개념의 핵심" |
| **1. DFD (Data Flow Diagram)** | 시스템의 프로세스, 데이터 저장소, 외부 엔터티, 데이터 흐름을 그리고, 신뢰 경계(Trust Boundary, 예: 인터넷과 내부망 사이... | "이 개념의 핵심" |
| **2. STRIDE (위협 식별)** | 신뢰 경계를 넘어가는 데이터 흐름마다 6가지 질문을 던집니다 | "이 개념의 핵심" |
| **PASTA** | STRIDE가 기술 중심(소프트웨어 중심)이라면, PASTA는 비즈니스 중심(해커의 동기, 회사 자산의 가치)의 최신 위협 모델링 방법론 | "이 개념의 핵심" |
| **OWASP Threat Dragon** | STRIDE 기반의 다이어그램(DFD)을 그리고 위협을 식별할 수 있게 도와주는 오픈소스 도구 | "이 개념의 핵심" |
