...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **필요성** | 스마트폰으로 은행 송금도 하고 지문 인식도 합니다 | "이 개념의 핵심" |
| **핵심 직관** | **"하나의 은행 안에 있는 '일반 창구'와 'VVIP 지하 금고'"** | "이 개념의 핵심" |
| **보안 구역 (Secure World = TrustZone)** | 강도가 은행 창구를 점령해도, 지하에 있는 VVIP 금고(지문, 생체 인식, 삼성페이)는 특수 키가 없으면 절대 문을 열 수 없음 | "이 개념의 핵심" |
| **등장 배경** | 과거에는 중요한 데이터를 보호하기 위해 스마트폰 기판에 '별도의 보안 칩(IC 카드 등)'을 하나 더 달았습니다 | "이 개념의 핵심" |
| **가치** | "TEE(신뢰 실행 환경)의 대중화" | "이 개념의 핵심" |
| **Normal World (REE: 풍부한 실행 환경)** | 일반적인 안드로이드 OS, 카톡, 유튜브 등 우리가 쓰는 모든 앱이 여기서 돌아갑니다 | "이 개념의 핵심" |
| **Secure World (TEE: 신뢰 실행 환경 = TrustZone)** | 삼성페이, 지문 인식 엔진, DRM(저작권 보호) 영상 재생 엔진처럼 극비 데이터만 여기서 돌아갑니다 | "이 개념의 핵심" |
