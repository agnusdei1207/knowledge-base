...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **필요성** | 빌드할 때 이미지 스캔(Trivy)을 다 통과했고, 배포할 때 보안 룰(OPA)도 다 통과해서 컨테이너가 무사히 서버에 떴습니다 | "이 개념의 핵심" |
| **핵심 직관** | **"무인 금고(컨테이너) 내부의 적외선 동작 감지 센서"** | "이 개념의 핵심" |
| **Falco** | 일단 금고 문이 닫히고 운영이 시작된(Runtime) 후의 감시자 | "이 개념의 핵심" |
| **가치** | "Behavioral(행위 기반) 탐지의 끝판왕" | "이 개념의 핵심" |
| **구체적 사례** | - 최근 해커의 유행 기법인 **크립토재킹(코인 채굴)** 공격 상황 | "이 개념의 핵심" |
| **주의점 및 흔한 오해** | - "알람이 너무 많이 와서 귀찮아요" $\rightarrow$ Falco 도입 실패의 1순위 원인입니다 | "이 개념의 핵심" |
| **판단 포인트** | IT 설계자는 Falco를 단순한 침입 탐지 시스템(IDS)으로 오해하면 안 됩니다 | "이 개념의 핵심" |
