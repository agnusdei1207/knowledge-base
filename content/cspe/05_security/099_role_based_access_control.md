...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **개요** | RBAC는 사용자에게 역할을 부여하고 역할에 권한을 연결하는 접근 제어 모델 | "핵심 기술 요소" |
| **왜 필요한가** | 사용자마다 권한을 직접 붙이면 1,000명 조직에서 권한 변경과 회수가 사람이 감당하기 어려운 수준으로 늘어난다 | "핵심 기술 요소" |
| **핵심 직관** | "김대리에게 DB 조회권한"이 아니라 "김대리에게 정산담당자 역할"을 주고, 정산담당자 역할에 필요한 권한을 묶는 방식임 | "핵심 기술 요소" |
| **배경·문제의식** | DAC는 소유자가 권한을 주고, MAC은 보안 등급이 강제한다 | "핵심 기술 요소" |
| **작동 원리** | 관리자는 Permission을 Role에 연결하고 User를 Role에 배정한다 | "핵심 기술 요소" |
| **비유** | 건물 출입증에 "재무팀" 권한이 찍혀 있으면 재무층 문이 열리고, 서버실 문은 열리지 않는 구조임 | "핵심 기술 요소" |
| **구체 예시** | ERP에서 구매요청자, 구매승인자, 지급처리자 역할을 분리하고 동일 사용자가 구매승인자와 지급처리자를 동시에 보유하지 못하게 SoD rul... | "핵심 기술 요소" |
