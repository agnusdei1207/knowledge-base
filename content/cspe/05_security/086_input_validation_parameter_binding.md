...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **개요** | 외부 입력을 허용된 형식으로 제한하고 SQL·명령·쿼리와 데이터 값을 분리하는 방어 기법 | "데이터 질의 언어" |
| **왜 필요한가** | 웹 요청의 파라미터, JSON Body, Header는 모두 신뢰 경계 밖에서 온다 | "핵심 기술 요소" |
| **핵심 직관** | 입력값 검증은 "문 앞 신분 확인", 파라미터 바인딩은 "명령문과 손님 말을 서로 다른 칸에 넣는 것"임 | "핵심 기술 요소" |
| **배경·문제의식** | `id=1 OR 1=1` 같은 문자열이 SQL 문장 일부로 해석되면 인증 우회와 대량 조회가 발생한다 | "데이터 질의 언어" |
| **비유** | 택배 접수에서 주소·무게·품목을 정해진 칸에 쓰게 하고, 운송 지시서 문구는 직원만 작성하게 하는 절차와 같다 | "핵심 기술 요소" |
| **구체 예시** | 사용자 조회는 `SELECT * FROM users WHERE id = ?`로 고정하고 `id`는 정수 1~2147483647만 허용한다 | "돌에 새긴 글씨" |
| **흔한 오해·주의점** | Client JavaScript 검증은 UX용이다 | "인터넷 주소" |
