...
- 개요: RFC 8446 기반 핸드셰이크
- 배경: TLS 1.2는 2-RTT 협상, 정적 RSA, CBC 등 레거시 암호군 선택으로 지연과 다운그레이드 위험이 존재함.
- 필요성: TLS 1.3은 1-RTT 기본, PFS 필수, AEAD 중심 협상으로 웹·API·모바일 서비스의 핸드셰이크 지연과 레거시 암호군 노출을 줄임.
