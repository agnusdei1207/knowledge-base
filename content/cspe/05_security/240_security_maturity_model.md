...
- 개요: 보안 역량 단계 평가·개선 체계
- 배경: 조직은 규제, 사고 대응, 클라우드 전환, 개발 보안 요구를 동시에 처리하므로 단순 체크리스트만으로 투자 순서와 책임 구조를 정하기 어려움
- 필요성: NIST CSF 2.0, OWASP SAMM, BSIMM 기준으로 current-target gap을 산정하고 증적 90% 이상, 6개월 과제 완료율 80% 이상 기준으로 로드맵을 관리해야 함
