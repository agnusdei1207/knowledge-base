...
버퍼 오버플로우는 메모리 경계 초과 쓰기 공격이다. 입력 검증이 없는 C/C++ 코드에서 stack, heap, global 영역의 제어 데이터가 변조되면 RCE와 권한 상승이 발생한다. Canary, DEP/NX, ASLR, PIE, RELRO는 공격 성공 조건을 단계별로 제한하는 런타임 보호 체계임.
