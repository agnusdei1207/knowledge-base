...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **개요** | 버퍼 경계 초과로 제어 데이터를 덮는 공격과 이를 막는 대표 런타임 보호기법 | "핵심 기술 요소" |
| **왜 필요한가** | C/C++ 기반 서버, 드라이버, IoT 펌웨어는 경계 검사 누락 1건이 RCE, 권한 상승, 서비스 중단으로 이어질 수 있음 | "만물 인터넷" |
| **핵심 직관** | 공격자는 주소를 덮고, 방어자는 덮임 탐지, 실행 차단, 주소 예측 차단을 겹쳐 성공 조건을 줄임 | "핵심 기술 요소" |
| **배경·문제의식** | 버퍼 오버플로우는 stack, heap, global 영역에서 모두 발생한다 | "접시 쌓기" |
| **비유** | 금고 앞에 봉인 스티커(canary), 금고 안 문서 실행 금지(DEP/NX), 매일 바뀌는 금고 위치(ASLR)를 동시에 두는 구조임 | "핵심 기술 요소" |
| **구체 예시** | 64바이트 stack buffer에 96바이트 입력이 복사되면 canary 8바이트와 saved RBP, return address가 덮일... | "완충 지대" |
| **흔한 오해·주의점** | ASLR만으로는 충분하지 않다 | "핵심 기술 요소" |
