...
OWASP LLM Top 10은 LLM 애플리케이션의 대표 보안 위험 10가지를 정리한 가이드이다.
LLM은 자연어 입력, 외부 지식, 도구 호출, 모델 공급망을 결합하므로 기존 웹 취약점과 다른 공격면이 생긴다.
기업은 LLM 도입 시 위협 모델, 권한 통제, 출력 검증, 감사 로그를 기준으로 보안 설계를 해야 한다.
