...
적대적 예제 공격은 입력에 미세 교란을 넣어 모델 오분류를 유도하는 공격이다.
AI 서비스가 이미지·음성·텍스트·악성코드 탐지에 적용되면서 추론 시점 회피 공격이 실제 운영 위험이 되었다.
방어 설계는 정상 정확도, robust accuracy, 공격 성공률, 교란 허용범위를 함께 고려해야 한다.
