...
컨테이너 이미지 스캔은 배포 전 취약점 검증이다.
이미지에는 OS 패키지, 라이브러리, 설정, Secret이 포함되어 동일 위험이 여러 Pod로 확산될 수 있음.
따라서 CI/CD에서 Trivy로 스캔하고 Critical CVE·Secret·설정 오류 기준을 넘으면 배포를 중단해야 함.
