...
- 개요: 장기 표적형 침해 캠페인
- 배경: APT는 특정 조직의 핵심 자산을 목표로 정찰, 침투, 거점 확보, 권한 상승, 내부 이동, 유출을 수주~수개월 반복함.
- 필요성: 경계 방어만으로는 dwell time이 길어지므로 EDR, SIEM, CTI, Zero Trust와 ATT&CK coverage 80% 이상 기준으로 상시 감시해야 함.
