...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **개요** | 스택 버퍼 경계를 초과해 반환주소나 제어 데이터를 덮어 실행 흐름을 탈취하는 공격 | "이 개념의 핵심" |
| **왜 필요한가** | C/C++ 네이티브 코드, 드라이버, 임베디드, 시스템 데몬은 메모리 경계 검사가 누락되면 RCE와 권한 상승으로 이어짐 | "이 개념의 핵심" |
| **핵심 직관** | 서류함 한 칸에 넣을 수 있는 분량을 넘겨 밀어 넣어 옆 칸의 지시서까지 바꾸는 공격임 | "이 개념의 핵심" |
| **배경·문제의식** | 함수 호출 시 스택에는 지역변수, saved frame pointer, return address가 저장된다 | "이 개념의 핵심" |
| **작동 원리** | 공격자는 입력 길이와 offset을 맞춰 return address를 shellcode, ROP gadget, libc 함수 주소로 바꾼다 | "이 개념의 핵심" |
| **비유** | 택배 주소칸을 넘치게 써서 배송지 라벨의 목적지를 바꾸는 것과 같다 | "이 개념의 핵심" |
