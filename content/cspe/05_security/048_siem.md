...
- 개요: 보안 로그 상관분석 플랫폼
- 배경: 방화벽, 서버, EDR, IAM 로그를 따로 보면 실패 로그인, 권한 상승, 외부 전송이 하나의 침해 흐름으로 연결되지 않음.
- 필요성: SIEM은 로그 coverage 95%, parser error 1% 이하, false positive 30% 이하 기준으로 SOC의 MTTD와 경보 품질을 관리해야 함.
