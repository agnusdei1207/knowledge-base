...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **필요성** | 요즘 기업들은 처음부터 AI를 만들지 않습니다 | "학습하는 기계" |
| **핵심 직관** | **"식당 주방장의 식자재 독살 방지 검수"** | "이 개념의 핵심" |
| **AI 공급망 보안** | 요리(AI)를 만드는데 식자재(오픈소스 데이터와 모델)가 너무 방대해서 사람이 다 뜯어볼 수가 없습니다 | "학습하는 기계" |
| **등장 배경** | 세계 최대의 오픈소스 AI 모델 공유 사이트인 '허깅페이스'에서 최근 수백 개의 악성 모델이 무더기로 적발되었습니다 | "학습하는 기계" |
| **가치** | "사일로(Silo) 보안의 종말" | "이 개념의 핵심" |
| **위협** | 파이썬에서 AI 모델을 저장할 때 쓰는 `Pickle(피클)` 포맷은 사실 '실행 가능한 코드 덩어리'입니다 | "학습하는 기계" |
| **방어 (Safetensors)** | 피클을 쓰지 말고, 오직 '순수한 숫자 덩어리'만 저장하여 코드가 실행될 위험을 원천 차단한 안전 포맷(Safetensors)으로만 모델을... | "이 개념의 핵심" |
