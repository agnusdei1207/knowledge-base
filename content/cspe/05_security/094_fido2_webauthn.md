...
| 용어 | 뜻 | 비유 |
|:---|:---|:---|
| **개요** | 비밀번호 대신 공개키 credential과 origin binding으로 사용자를 인증하는 표준 | "이 개념의 핵심" |
| **왜 필요한가** | 비밀번호와 OTP는 피싱 사이트가 입력값을 받아 중계할 수 있다 | "이 개념의 핵심" |
| **핵심 직관** | 사용자는 비밀번호를 서버에 보내지 않고, 자기 장치 안의 개인키로 "이 사이트가 보낸 일회용 문제"에 서명한다 | "이 개념의 핵심" |
| **배경·문제의식** | 계정 탈취의 핵심은 공유 비밀(secret)을 입력하게 만드는 것이다 | "이 개념의 핵심" |
| **작동 원리** | 등록 시 RP는 challenge를 보내고 authenticator는 RP ID별 공개키 credential을 생성한다 | "신분증 확인" |
| **비유** | 도장을 서버에 맡기는 것이 아니라, 매번 새 문서에 본인 도장으로 찍은 서명만 제출하는 방식이다 | "이 개념의 핵심" |
| **구체 예시** | `login.example | "일지 기록" |
