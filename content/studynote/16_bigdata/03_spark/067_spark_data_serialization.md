...
- <strong>성능 튜닝 포인트:</strong> 대량의 데이터를 처리하는 셔플 작업이나 RDD 캐싱(`MEMORY_ONLY_SER`) 시 Kryo 사용은 필수적이다. 기술사로서 아키텍처 설계 시 네트워크 병목이 예상된다면 가장 먼저 직렬화 방식과 버퍼 크기(`spark.kryo.capacity`)를 점검해야 한다.
