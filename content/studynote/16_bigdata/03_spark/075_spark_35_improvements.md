...
## 핵심 인사이트 (3줄 요약)
> 1. **본질**: [Apache Spark](/knowledge-base/studynote/14_data_engineering/05_exam_keywords/206_spark_inmemory_rdd_lazy_evaluation_lineage/) 3.5는 2023년 릴리스되어 Python [API](/knowledge-base/studynote/02_operating_system/01_overview_architecture/014_api_posix/) (PySpark)의 기능 강화, Spark Connect (원격 클라이언트 연결), ANSI SQL 지원 확대, 구조화된 스트리밍([Structured Streaming](/knowledge-base/studynote/16_bigdata/03_spark/061_structured_streaming/)) 고도화를 통해 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 엔지니어링·ML 워크플로우의 생산성과 이식성을 높인 메이저 [버전](/knowledge-base/studynote/03_network/06_network_layer_ip/288_version_ihl_tos_total_length/)이다.
> 2. **가치**: Spark Connect는 Spark 서버에 경량 클라이언트로 원격 접속하여 Jupyter Notebook·로컬 IDE에서 클러스터 Spark를 직접 활용할 수 있게 하여 개발 경험([DX](/knowledge-base/studynote/04_software_engineering/10_trends_pm_quality/726_platform_engineering_idp_dx/))을 대폭 향상시키며, Python UDTF (User-Defined Table Function)와 PyArrow 기반 최적화로 PySpark [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) 격차를 줄였다.
> 3. **판단 포인트**: Spark 3.5의 ANSI 모드 강화는 기존 Spark 코드와의 [호환성](/knowledge-base/studynote/04_software_engineering/06_software_architecture/344_compatibility_usability/) 이슈를 일으킬 수 있다 — 기존 코드에서 NULL 처리·[정수 오버플로우](/knowledge-base/studynote/09_security/04_endpoint_security/333_integer_overflow/) 등 ANSI 미준수 동작에 의존했다면 마이그레이션 전 [회귀 테스트](/knowledge-base/studynote/04_software_engineering/11_testing_validation/410_regression_test/)가 필수다.
...
| [버전](/knowledge-base/studynote/03_network/06_network_layer_ip/288_version_ihl_tos_total_length/) | 주요 특징 |
|:---|:---|
| **Spark 2.x** | DataFrame [API](/knowledge-base/studynote/02_operating_system/01_overview_architecture/014_api_posix/), [Spark SQL](/knowledge-base/studynote/16_bigdata/03_spark/056_spark_sql/), ML lib |
| **Spark 3.0** | AQE(Adaptive Query Execution), 동적 [파티션 프루닝](/knowledge-base/studynote/05_database/03_relational_model/184_partition_pruning/) |
| **Spark 3.4** | Spark Connect 프리뷰, ANSI 강화 시작 |
| **Spark 3.5** | Spark Connect [GA](/knowledge-base/studynote/08_algorithm_stats/10_linear_algebra/169_evolutionary_algorithms/), Python UDTF, Streaming 고도화 |
| **Spark 4.0 (예정)** | Python-first [API](/knowledge-base/studynote/02_operating_system/01_overview_architecture/014_api_posix/), 추가 통합 |

AQE (Adaptive Query Execution, [적응형 쿼리 실행](/knowledge-base/studynote/16_bigdata/03_spark/059_adaptive_query_execution_aqe/))는 Spark 3.0에서 도입되어 런타임 통계 기반으로 [실행 계획](/knowledge-base/studynote/05_database/03_relational_model/166_execution_plan_optimizer_navigation_tree/)을 동적으로 재최적화하는 기능으로, 3.5에서 더욱 안정화되었다.
...
[데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 과학 팀의 Jupyter Notebook에서 원격 Spark 클러스터 직접 활용.

1. **기존**: 각 노트북 서버에 Spark 드라이버 설치, [버전](/knowledge-base/studynote/03_network/06_network_layer_ip/288_version_ihl_tos_total_length/) 관리 복잡.
2. **Spark Connect 도입**: [Databricks](/knowledge-base/studynote/16_bigdata/03_spark/074_photon_engine/)/EMR Spark Connect Server 활성화.
...
- **📢 섹션 요약 비유**: Spark 3.5는 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 처리 도구상자의 최신 업그레이드다. 더 날카로운 도구(Python UDTF), 더 편한 손잡이(Spark Connect), 더 정확한 눈금(ANSI SQL)이 추가되어 장인([데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 엔지니어)의 작업 효율이 높아진다.
...
2. Python으로 수십억 개의 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 처리하는 함수(UDTF)를 쉽게 만들 수 있고, SQL도 더 표준에 맞게 바뀌었어요.
3. 이 업그레이드 덕분에 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 엔지니어들이 더 빠르고 편하게 빅데이터를 처리할 수 있답니다!
