...
Delta Lake는 데이터 레이크의 유연성과 데이터 웨어하우스의 안정성을 결합한 <strong>레이크하우스 아키텍처의 핵심 스토리지 레이어</strong>다. Databricks 환경에서는 기본이며, 오픈소스 버전으로 Spark 클러스터에도 적용 가능하다. 기술사 답안에서는 기존 레이크의 한계 -> Delta Lake의 트랜잭션 로그 기반 해결책 -> ACID + 타임 트래블의 가치를 순서대로 서술하는 것이 핵심이다.
