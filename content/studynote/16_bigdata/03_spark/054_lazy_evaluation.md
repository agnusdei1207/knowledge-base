...
> 2. **가치**: 실제 연산을 미루기 때문에, [실행 계획](/knowledge-base/studynote/05_database/03_relational_model/166_execution_plan_optimizer_navigation_tree/) 전체를()하여 불필요한 중간 결과를 완전히 제거하거나(필터 누르기/필터 병합), [파이프](/knowledge-base/studynote/02_operating_system/02_process_thread/123_pipe/)라이닝이 가능한 단계를 하나의 Stage로 합쳐서 네트워크 Shuffle을 최소화하는 등 전체 최적화를 한 번에 수행할 수 있다.
...
- **변환 로드밸런싱 불가**: 여러 단계의 변환을 한 번에 실행하면, 시스템 전체의 부하를 고려한 최적의 실행 순서를에 적용할 수 없습니다. 예를 들어, [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)가 [파티션](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/) A에는 100만 건, [파티션](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/) B에는 10만 건인데, A에 filter를 먼저 적용하면 전체 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)가 균등하게 분배될 수도 있지만, 전체를 한 번에 실행하면 그런 최적화 기회가 사라집니다.
...
| **전체 최적화** | 실행 전 전체 DAG를 분석하여 최적 순서 결정 | 필터 조건을에 적용하여 네트워크 전송 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)량 감소 |
...
| **오류 발견** | Runtime(실행 시)에만 발견 | 컴파일 타임에 발견 가능한 경우 |
...
- **📢 섹션 요약 비유**: [Lazy](/knowledge-base/studynote/06_ict_convergence/05_data_science/380_computational_graph_lazy_eager_execution/) Evaluation의영은 "시공 [검증](/knowledge-base/studynote/04_software_engineering/07_object_oriented/395_verification_process_review/)"과 동일합니다. 아무리 훌륭한 건축가(엔지니어)의 설계도(변환 체인)라도, 실제 строительство(실행)에 들어가기 전에" компьютер(가상 시뮬레이션)"으로 전체 공정을 미리 [확인](/knowledge-base/studynote/04_software_engineering/12_testing_maintenance/396_validation/)해야 합니다. 시공([쿼리](/knowledge-base/studynote/10_ai/04_ai_ops_ethics/298_qkv_attention/) [실행 계획](/knowledge-base/studynote/05_database/03_relational_model/166_execution_plan_optimizer_navigation_tree/))이 "이 순서로시공하면 자재가 이렇게 이동하고, these자재는 여기에 쌓이고..."와/과 것을 [확인](/knowledge-base/studynote/04_software_engineering/12_testing_maintenance/396_validation/)하면, 실제 시공에서"자재가 공간이 부족해서로 다|Other곳에 옮겨야 함"이라는 비효율을 미리 방지할 수 있습니다. take([10](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/489_raid_10_hybrid/))은"디지털 목업(Modeling)으로 3D 이미지만 [확인](/knowledge-base/studynote/04_software_engineering/12_testing_maintenance/396_validation/)"이고, collect()는"실물 크기 maquette를 제작해서 검사"에 해당합니다.
