...
Lambda Architecture는 <strong>실시간 처리의 속도와 배치 처리의 정확성을 동시에 달성하는 검증된 빅데이터 아키텍처 패턴</strong>이다. 기술사 답안에서는 3개 레이어의 역할·기술·데이터 흐름, 코드 이중화 문제, Kappa Architecture와의 비교, 그리고 Lakehouse로의 현대적 진화를 함께 서술하면 완성도 높은 답안이 된다.
