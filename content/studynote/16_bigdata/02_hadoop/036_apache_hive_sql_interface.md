...
빅데이터 초기, 데이터 분석가들은 하둡의 방대한 데이터를 처리하기 위해 생소한 자바 코드를 짜야만 했다. 컴퓨터시스템응용기술사 관점에서 Apache Hive는 'SQL'이라는 익숙한 인터페이스를 하둡에 입힘으로써 빅데이터 민주화를 이끈 핵심 솔루션이다. 맵리듀스나 Tez 엔진 위에서 돌아가는 배치 엔진으로서, 실시간성보다는 수백 테라바이트(TB) 규모의 전사적 리포팅과 대용량 ETL(Extract, Transform, Load) 처리에 최적화되어 있다.
...
| **기술사적 판단** | "OLTP 서비스용" | <strong>"OLAP 분석/배치용"</strong> |
