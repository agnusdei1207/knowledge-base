...
> 2. 컬럼형 저장소 + 벡터화 실행 + 사전 집계(Pre-aggregation)의 조합 — ClickHouse는 컬럼형 [[347_compaction|압축]] 저장 + [[370_simd|SIMD]] 벡터화로 초당 수억 행 처리, Druid/Pinot은 인제스트 시 사전 집계로 [[298_qkv_attention|쿼리]] 시 부하를 최소화하는 [[268_strategy_pattern|전략]] 차이가 있다.
> 3. 워크로드 특성이 엔진 선택을 결정 — 고빈도 업데이트(UPDATE/DELETE) → ClickHouse, 이벤트 스트림 실시간 인제스트 + 밀리초 응답 → Druid/Pinot, 기존 [[209_data_warehouse_schema_on_write|데이터 웨어하우스]] 대체 → StarRocks가 각각 강점을 가진다.

---

## Ⅰ. 실시간 [[316_olap|OLAP]] 등장 배경
...
> 📢 **섹션 요약 비유**: 실시간 [[316_olap|OLAP]] = 즉석 성적표 — 시험(이벤트) 끝나고 다음 날 성적(배치 분석) 대신 답안 제출 즉시 점수(실시간 [[316_olap|OLAP]]). 광고, 게임, 금융에서 "지금 바로" 답이 필요!
...
> 📢 **섹션 요약 비유**: ClickHouse = [[148_5g_embb_urllc_mmtc|초고속]] 계산기 — 100억 줄 엑셀에서 특정 컬럼만 뽑아 집계([[370_simd|SIMD]]). [[544_hive|Hive]] 5분을 5초로. 컬럼만 읽고 CPU 한 번에 32개 처리!
...
> 📢 **섹션 요약 비유**: Druid = 미리 계산해 두는 주방 — 재료(원본) 사전 손질([[042_rollup_l2_solution|Rollup]])해서 요리 시간([[298_qkv_attention|쿼리]]) 단축. Pinot = [[148_5g_embb_urllc_mmtc|초고속]] 패스트푸드 — 메뉴 미리 준비(Star-Tree)로 10ms 내 제공!
...
> 📢 **섹션 요약 비유**: StarRocks = 만능 분석 도구 — 실시간+배치, [[001_dikw_pyramid|데이터]]레이크+웨어하우스, SQL+스트리밍을 하나로. 여러 도구(Spark+[[544_hive|Hive]]+Druid) 대신 StarRocks 하나!
...
> 📢 **섹션 요약 비유**: 광고 플랫폼 실시간 분석 — 광고 클릭 결과를 1시간 후가 아니라 1분 이내에 [[396_validation|확인]]! Druid Rollup으로 [[001_dikw_pyramid|데이터]] 95% [[347_compaction|압축]], 광고주는 즉시 최적화. 실시간이 돈!
