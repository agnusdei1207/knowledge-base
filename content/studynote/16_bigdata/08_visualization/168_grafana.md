...
Grafana는 <strong>클라우드 네이티브 시대의 표준 관측성 플랫폼</strong>이다. 마이크로서비스와 쿠버네티스 환경에서 시스템의 "건강 상태"를 지속적으로 모니터링하고, 이상 감지 시 즉각 대응할 수 있는 가시성을 제공한다. 컴퓨터시스템응용기술사는 LGTM 스택의 각 컴포넌트 역할과 PromQL 기반 알림 설계를 이해하고 클라우드 인프라 모니터링 아키텍처 설계에 적용할 수 있어야 한다.
