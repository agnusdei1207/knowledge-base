...
기술사 답안에서는 "WebGL이 빠르다"로 끝내면 약하다. <strong>데이터 축소 전략, 질의 엔진, 렌더링 방식, 스트리밍 처리, UX 설계</strong>를 함께 묶어야 완성도가 높다. 특히 "어떤 경우 집계를 택하고, 어떤 경우 샘플링을 택하며, 언제 원본 드릴다운을 허용하는가"를 설명하면 실무적 판단이 선명해진다.
