...
> 3. **판단 포인트**: 고정적이고 규제가 강한 대용량 워크로드는 [온프레미스](/knowledge-base/studynote/07_enterprise_systems/01_strategy_governance/061_on_premise_legacy_infrastructure/)나 하이브리드가 유리하고, 변동성이 크고 실험 주기가 빠른 분석 환경은 [클라우드 네이티브](/knowledge-base/studynote/04_software_engineering/11_testing_validation/923_cloud_native_architecture/) 재설계가 장기 효율이 높다.
...
- **📢 섹션 요약 비유**: 단순 이전은 오래 살던 집의 가구를 새 집에 그대로 밀어 넣는 일이고, [클라우드 네이티브](/knowledge-base/studynote/04_software_engineering/11_testing_validation/923_cloud_native_architecture/) 전환은 집 구조에 맞게 가구 배치와 생활 동선을 다시 설계하는 일에 가깝다.
