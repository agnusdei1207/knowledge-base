...
ETL에서 ELT로의 패러다임 전환은 클라우드 빅데이터 인프라 확산과 함께 이미 완료된 흐름이다. dbt는 현재 데이터 팀 표준 도구로 자리 잡았으며, Databricks·Snowflake·BigQuery 모두 네이티브 dbt 통합을 지원한다. 기술사 시험에서는 <strong>ETL vs ELT 전환 이유</strong>, **dbt 핵심 기능**, <strong>Incremental 모델 동작 원리</strong>가 핵심 논점이다.
