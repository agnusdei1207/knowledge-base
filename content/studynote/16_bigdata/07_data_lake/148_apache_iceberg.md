...
Apache Iceberg는 2024년 이후 AWS Athena, Snowflake, Spark 3.x, Flink, Trino의 기본 테이블 포맷으로 채택되며 오픈 레이크하우스 생태계의 중심축이 됐다. 기술사 시험에서는 <strong>히든 파티셔닝 원리</strong>, <strong>메타데이터 트리 구조(Manifest List -> Manifest -> Data File)</strong>, <strong>파티션 진화</strong>가 핵심 논점이다.
