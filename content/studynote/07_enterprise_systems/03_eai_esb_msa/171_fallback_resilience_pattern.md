...
> 1. **본질**: 폴백 ([Fallback](/knowledge-base/studynote/13_cloud_architecture/03_msa_serverless/129_fallback/))은 원격 호출 실패나 [서킷 브레이커](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/307_circuit_breaker_pattern/) ([Circuit Breaker](/knowledge-base/studynote/12_it_management/05_security_compliance/304_circuit_breaker/)) 개방 시, 오류를 그대로 전파하지 않고 안전한 대체 응답으로 전환하는 [회복](/knowledge-base/studynote/05_database/04_transactions_concurrency/233_recovery_database_restoration_overview/)탄력성 (Resilience) 응답 [전략](/knowledge-base/studynote/04_software_engineering/04_testing_quality/268_strategy_pattern/)이다.
> 2. **가치**: 핵심 사용자 흐름을 완전히 끊지 않고, 캐시·기본값·기능 축소를 통해 우아한 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) 저하 (Graceful Degradation)를 제공함으로써 장애의 체감 범위를 줄인다.
> 3. **판단 포인트**: 읽기 위주의 부가 기능에는 강력하지만, 결제 승인·주문 [생성](/knowledge-base/studynote/02_operating_system/02_process_thread/087_process_state_transition/)처럼 정합성이 중요한 [쓰기](/knowledge-base/studynote/13_cloud_architecture/05_data_engineering/289_cqrs_db/) 작업에는 "거짓 성공"을 만들 수 있으므로 폴백보다 빠른 실패와 보상 절차가 우선이다.
...
| :--- | :--- | :--- | :--- |
| 캐시 폴백 | 마지막 정상 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/), 인기 목록 | 조회성 화면, 일시적 장애 | 오래된 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 노출 |
| 기본값 폴백 | 빈 목록, 안내 [메시](/knowledge-base/studynote/01_computer_architecture/10_parallel_processing_architecture/389_mesh_topology/)지, 기본 구성 | 부가 기능, 비핵심 위젯 | 정보가 너무 빈약해질 수 있음 |
...
| :--- | :--- | :--- | :--- |
| [타임아웃](/knowledge-base/studynote/04_software_engineering/09_cloud_native_ai_architecture/573_timeout_retry_backoff_strategy/) ([Timeout](/knowledge-base/studynote/02_operating_system/05_deadlock/319_timeout_prevention/)) | 얼마나 기다릴까 | 무한 대기 차단 | 폴백 전환 시점을 앞당김 |
| 재시도 (Retry) | 한 번 더 시도할까 | 일시적 오류 [복구](/knowledge-base/studynote/09_security/13_secops_ir_forensics/658_ir_recovery/) | 실패가 누적되면 폴백으로 넘어감 |
| [서킷 브레이커](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/307_circuit_breaker_pattern/) | 계속 호출할까 | 연쇄 장애 차단 | Open 상태에서 폴백을 자주 호출 |
| [벌크헤드](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/308_bulkhead_pattern/) ([Bulkhead](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/308_bulkhead_pattern/)) | 자원을 분리할까 | 장애 격리 | 폴백이 다른 자원까지 잠식하지 않게 함 |
| 폴백 ([Fallback](/knowledge-base/studynote/13_cloud_architecture/03_msa_serverless/129_fallback/)) | 실패 시 무엇을 줄까 | 대체 응답 제공 | 사용자 경험을 마지막으로 [보호](/knowledge-base/studynote/02_operating_system/10_security/571_protection_vs_security/) |

또 하나 중요한 연결은 "우아한 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) 저하"와 "거짓 성공"의 차이다. 추천 상품이 인기 상품 목록으로 대체되는 것은 우아한 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) 저하지만, 결제 승인 실패를 "일단 성공"으로 보여 주는 것은 거짓 성공이다. 둘 다 겉으로는 오류 [메시](/knowledge-base/studynote/01_computer_architecture/10_parallel_processing_architecture/389_mesh_topology/)지를 숨기지만, 전자는 안전한 축소이고 후자는 치명적 왜곡이다.

그래서 폴백은 읽기 전용 조회, 비핵심 정보, 일시적인 화면 보완에는 강하지만, 거래 결과를 확정하는 [도메인](/knowledge-base/studynote/05_database/02_modeling_normalization/064_relation_domain/)에는 신중해야 한다. 이 경계를 실무에서 구분하지 못하면 "장애 대응"이라는 명목으로 더 큰 업무 사고를 만든다.
...
2. 폴백 응답이 오래된 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)여도 업무적으로 안전한가?
3. 폴백 결과가 사용자와 운영자에게 [식별](/knowledge-base/studynote/09_security/13_secops_ir_forensics/655_ir_detection_analysis/) 가능하게 기록되는가?
4. 폴백 호출률, 캐시 신선도, [서킷 브레이커](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/307_circuit_breaker_pattern/) 상태를 관측하고 있는가?
5. [쓰기](/knowledge-base/studynote/13_cloud_architecture/05_data_engineering/289_cqrs_db/) 작업이라면 폴백 대신 실패 처리와 보상 절차를 설계했는가?

### 채택 / 회피 판단

- **채택**: 추천, 검색 보조, 리뷰 요약, 통계 패널, 읽기 전용 외부 [API](/knowledge-base/studynote/02_operating_system/01_overview_architecture/014_api_posix/) 연동
- **조건부 채택**: 계정 프로필, 배송 추적처럼 약간의 최신성 [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/)을 허용하는 조회
- **회피**: 결제 승인, 주문 [생성](/knowledge-base/studynote/02_operating_system/02_process_thread/087_process_state_transition/), 포인트 적립, 계좌 이체, 재고 확정
...
- 만료 [정책](/knowledge-base/studynote/10_ai/02_dl_architecture_new/164_policy/) 없이 오래된 캐시를 무기한 [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/)하는 경우
- 폴백 발생을 [메트릭](/knowledge-base/studynote/03_network/07_network_layer_routing/342_routing_metric_hop_bandwidth_delay/)으로 남기지 않아 장애를 장시간 숨기는 경우
...
폴백을 올바르게 적용하면 장애 중에도 핵심 사용자 경험을 유지하고, 원격 의존성 실패가 전체 [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/) 중단으로 번지는 것을 줄일 수 있다. 운영 측면에서는 [서킷 브레이커](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/307_circuit_breaker_pattern/)와 함께 장애를 국소화하고, 캐시와 기본 응답을 활용해 [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/) 연속성을 확보할 수 있다.
...
| 개념 | 연결 포인트 |
| :--- | :--- |
| [서킷 브레이커](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/307_circuit_breaker_pattern/) ([Circuit Breaker](/knowledge-base/studynote/12_it_management/05_security_compliance/304_circuit_breaker/)) | 폴백 전환이 가장 자주 발생하는 실패 감지 장치 |
| 캐시 | 마지막 정상 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 기반 폴백의 대표 저장소 |
| 우아한 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) 저하 (Graceful Degradation) | 폴백이 달성하려는 사용자 경험 목표 |
| [벌크헤드](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/308_bulkhead_pattern/) ([Bulkhead](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/308_bulkhead_pattern/)) | 폴백 처리 자체가 다른 자원을 고갈시키지 않게 [보호](/knowledge-base/studynote/02_operating_system/10_security/571_protection_vs_security/) |
| [사가](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/312_saga_pattern_choreography_orchestration/) ([Saga](/knowledge-base/studynote/12_it_management/05_security_compliance/305_saga/)) | 폴백이 부적절한 [쓰기](/knowledge-base/studynote/13_cloud_architecture/05_data_engineering/289_cqrs_db/) 작업에서 대안이 되는 보상 패턴 |
| 관측성 ([Observability](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/642_observability_telemetry/)) | 폴백 사용률과 신선도를 운영 지표로 드러내는 기반 |
