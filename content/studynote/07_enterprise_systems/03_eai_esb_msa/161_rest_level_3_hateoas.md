...
| 요소 | 의미 | 설계 포인트 |
| :--- | :--- | :--- |
| `rel` | 링크의 [관계](/knowledge-base/studynote/05_database/02_modeling_normalization/083_relationship_in_er_model/) 의미 | `self`, `next`, `cancel`처럼 일관된 어휘 필요 |
| `href` | 다음 요청 대상 | 하드코딩 대신 서버가 제공 |
| 상태 기반 링크 | [현재 상태](/knowledge-base/studynote/04_software_engineering/03_design_architecture/178_as_is_to_be_analysis/)에서 허용된 행위 | 비즈니스 규칙 반영 |
| 표현 포맷 | [HAL](/knowledge-base/studynote/02_operating_system/01_overview_architecture/070_hal/), [JSON](/knowledge-base/studynote/11_design_supervision/06_exam_summary/343_json/):[API](/knowledge-base/studynote/02_operating_system/01_overview_architecture/014_api_posix/), Siren 등 | 조직 표준과 클라이언트 지원 고려 |

따라서 Level 3의 핵심 원리는 "[데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) + 링크 + [상태 전이](/knowledge-base/studynote/04_software_engineering/10_trends_pm_quality/632_state_transition_diagram_testing/) 규칙"의 결합이다. 단순 CRUD (Create, Read, Update, Delete) API에서는 과할 수 있지만, 승인·결제·주문처럼 [상태 전이](/knowledge-base/studynote/04_software_engineering/10_trends_pm_quality/632_state_transition_diagram_testing/)가 중요한 [도메인](/knowledge-base/studynote/05_database/02_modeling_normalization/064_relation_domain/)에서는 표현력이 높다.
...
Level 3를 이해하려면 Level 2와의 차이를 먼저 봐야 한다. Level 2는 리소스 URI와 [HTTP](/knowledge-base/studynote/03_network/09_application_layer_web_email/461_http_stateless_connection_oriented/) 메서드를 올바르게 사용해도, 다음 행동의 발견 책임이 여전히 클라이언트에 남아 있는 경우가 많다. 반면 Level 3는 서버가 링크를 통해 가능한 전이를 알려 주므로, 클라이언트는 URI 규칙 자체보다 `rel` 의미에 더 집중하게 된다.

| 항목 | Level 2 | Level 3 |
| :--- | :--- | :--- |
| 다음 행동 탐색 | 문서와 클라이언트 코드에 의존 | 응답의 하이퍼미디어로 안내 |
| URI [결합도](/knowledge-base/studynote/04_software_engineering/04_testing_quality/195_coupling_levels/) | 상대적으로 높음 | 상대적으로 낮음 |
| 구현 난이도 | 중간 | 높음 |
| 대표 장점 | 표준 [HTTP](/knowledge-base/studynote/03_network/09_application_layer_web_email/461_http_stateless_connection_oriented/) 활용 | 워크플로 자기 서술성 강화 |
| 대표 한계 | [상태 전이](/knowledge-base/studynote/04_software_engineering/10_trends_pm_quality/632_state_transition_diagram_testing/) 규칙이 코드에 [분산](/knowledge-base/studynote/08_algorithm_stats/08_stats/136_variance/) | 응답 설계와 클라이언트 처리 복잡 |
...
실무에서는 모든 API를 Level 3로 만들 필요는 없다. 단순 조회·등록 중심의 내부 업무 API는 Level 2만으로도 충분한 경우가 많다. 반면 외부 파트너가 많고, 승인·주문·구독처럼 [상태 전이](/knowledge-base/studynote/04_software_engineering/10_trends_pm_quality/632_state_transition_diagram_testing/)가 복잡하며, 장기간 [버전](/knowledge-base/studynote/03_network/06_network_layer_ip/288_version_ihl_tos_total_length/) 호환이 중요한 [도메인](/knowledge-base/studynote/05_database/02_modeling_normalization/064_relation_domain/)이라면 HATEOAS가 [결합도](/knowledge-base/studynote/04_software_engineering/04_testing_quality/195_coupling_levels/) 완화에 도움이 될 수 있다.
...
| 개념 | 연결 포인트 |
| :--- | :--- |
| [Richardson Maturity Model](/knowledge-base/studynote/07_enterprise_systems/03_eai_esb_msa/157_restful_api_richardson_maturity_model/) | [REST](/knowledge-base/studynote/07_enterprise_systems/03_eai_esb_msa/156_rest_representational_state_transfer/) 성숙도를 Level 0~3으로 구분하는 기준 |
| [HTTP](/knowledge-base/studynote/03_network/09_application_layer_web_email/461_http_stateless_connection_oriented/) Method | 링크를 따라 수행할 행위 의미를 부여 |
| `rel` | 링크 [관계](/knowledge-base/studynote/05_database/02_modeling_normalization/083_relationship_in_er_model/)를 설명하는 핵심 어휘 |
| [HAL](/knowledge-base/studynote/02_operating_system/01_overview_architecture/070_hal/)/[JSON](/knowledge-base/studynote/11_design_supervision/06_exam_summary/343_json/):[API](/knowledge-base/studynote/02_operating_system/01_overview_architecture/014_api_posix/)/Siren | 하이퍼미디어 표현을 구조화하는 포맷 |
| [상태 전이](/knowledge-base/studynote/04_software_engineering/10_trends_pm_quality/632_state_transition_diagram_testing/) ([State Transition](/knowledge-base/studynote/04_software_engineering/10_trends_pm_quality/632_state_transition_diagram_testing/)) | HATEOAS가 안내하려는 핵심 대상 |
