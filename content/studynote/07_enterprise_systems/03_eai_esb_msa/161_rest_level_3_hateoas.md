...
기술사 관점에서는 "Level 3가 무조건 최고이므로 항상 도입"이라는 답변보다, <strong>도메인 복잡도 대비 비용과 효과를 따져 채택 범위를 설명하는 답변</strong>이 더 적절하다. 즉 HATEOAS는 이상적인 REST 원칙이지만, 실무에서는 선택적 적용이 합리적일 수 있다.
