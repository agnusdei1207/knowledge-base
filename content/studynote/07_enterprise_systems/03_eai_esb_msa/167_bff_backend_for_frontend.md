...
← **이전**: [[166_api_gateway_architecture|166. API 게이트웨이 (API Gateway) - 클라이언트 요청을 단일 진입점으로 받아 적절한 마이크로서비스로 라우팅, 인증/인가,]]
**다음**: [[168_service_discovery|168. 서비스 디스커버리 (Service Discovery) 동적 컨테이너 위치 레지스트리]] →
