...
> 1. **본질**: [서킷 브레이커](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/307_circuit_breaker_pattern/) ([Circuit Breaker](/knowledge-base/studynote/12_it_management/05_security_compliance/304_circuit_breaker/)) 패턴은 최근 실패율과 [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/)을 관찰해, 문제가 있는 외부 의존성 호출을 잠시 차단하고 빠르게 실패시키는 [회복](/knowledge-base/studynote/05_database/04_transactions_concurrency/233_recovery_database_restoration_overview/)탄력성 (Resilience) 제어 장치다.
> 2. **가치**: [타임아웃](/knowledge-base/studynote/04_software_engineering/09_cloud_native_ai_architecture/573_timeout_retry_backoff_strategy/) 대기와 [스레드](/knowledge-base/studynote/02_operating_system/02_process_thread/092_thread_lwp/) 고갈을 줄여 연쇄 장애 (Cascading Failure)를 막고, 장애 중에도 대체 응답이나 우아한 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) 저하를 가능하게 한다.
> 3. **판단 포인트**: 진짜 핵심은 패턴 이름이 아니라 실패로 볼 예외의 범위, [임계치](/knowledge-base/studynote/03_network/08_transport_layer/431_ssthresh_slow_start_threshold/), 반개방 상태의 탐침 수, 안전한 [폴백](/knowledge-base/studynote/07_enterprise_systems/03_eai_esb_msa/171_fallback_resilience_pattern/) ([Fallback](/knowledge-base/studynote/13_cloud_architecture/03_msa_serverless/129_fallback/)) 설계가 적절한가에 있다.
...
- **📢 섹션 요약 비유**: 아파트 한 집에서 합선이 났을 때 바로 차단기를 내려야 다른 집 전기까지 나가지 않듯, [서킷 브레이커](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/307_circuit_breaker_pattern/)는 한 [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/)의 고장을 전체 정전으로 키우지 않게 막는 장치다.

---

## Ⅱ. 아키텍처 및 핵심 원리

[서킷 브레이커](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/307_circuit_breaker_pattern/)의 핵심은 상태 기계와 관찰 지표다. 일반적으로 닫힘 (Closed), 열림 (Open), 반개방 (Half-Open) 세 상태를 돌며 최근 호출의 실패율, 느린 호출 비율, 최소 호출 수, 대기 시간을 기준으로 상태를 바꾼다.

| 상태 | 의미 | 동작 |
| :--- | :--- | :--- |
| Closed | 정상 감시 상태 | 요청을 통과시키며 실패율·[지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/)을 계속 측정 |
| Open | 차단 상태 | 의존 [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/)로 보내지 않고 즉시 실패 또는 [폴백](/knowledge-base/studynote/07_enterprise_systems/03_eai_esb_msa/171_fallback_resilience_pattern/) 반환 |
| Half-Open | [회복](/knowledge-base/studynote/05_database/04_transactions_concurrency/233_recovery_database_restoration_overview/) 시험 상태 | 제한된 수의 탐침 요청만 허용해 [복구](/knowledge-base/studynote/09_security/13_secops_ir_forensics/658_ir_recovery/) 여부 판단 |
...
[서킷 브레이커](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/307_circuit_breaker_pattern/)는 코드 [라이브러리](/knowledge-base/studynote/04_software_engineering/06_software_architecture/336_library_vs_framework/) 안에 둘 수도 있고, [서비스 메시](/knowledge-base/studynote/12_it_management/05_security_compliance/945_service_mesh_istio/) ([Service Mesh](/knowledge-base/studynote/03_network/16_data_center_cloud/828_service_mesh_microservice_communication_infrastructure/)) 나 [API](/knowledge-base/studynote/02_operating_system/01_overview_architecture/014_api_posix/) 게이트웨이 계층에 둘 수도 있다. [라이브러리](/knowledge-base/studynote/04_software_engineering/06_software_architecture/336_library_vs_framework/) 방식은 호출 맥락에 맞는 세밀한 제어가 쉽고, [프록시](/knowledge-base/studynote/04_software_engineering/04_testing_quality/264_proxy_pattern_surrogate_access_control/) 방식은 [정책](/knowledge-base/studynote/10_ai/02_dl_architecture_new/164_policy/) [일관성](/knowledge-base/studynote/05_database/04_transactions_concurrency/194_consistency_database_integrity/)이 좋다.

- **📢 섹션 요약 비유**: [서킷 브레이커](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/307_circuit_breaker_pattern/)는 가게 문을 영원히 닫는 자물쇠가 아니라, 손님이 몰려도 주방이 불날 조짐이 보이면 잠시 입장을 멈추고 주방 상태를 [확인](/knowledge-base/studynote/04_software_engineering/12_testing_maintenance/396_validation/)하는 안전 관리자와 같다.

---

## Ⅲ. 비교 및 연결

[서킷 브레이커](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/307_circuit_breaker_pattern/)를 제대로 쓰려면 비슷해 보이는 다른 [회복](/knowledge-base/studynote/05_database/04_transactions_concurrency/233_recovery_database_restoration_overview/)탄력성 패턴과 역할을 구분해야 한다. 실패를 늦추는 기술과 실패를 빨리 인정하는 기술은 목적이 다르다.

| 패턴 | 답하는 질문 | 잘하는 일 | 주의점 |
| :--- | :--- | :--- | :--- |
| [타임아웃](/knowledge-base/studynote/04_software_engineering/09_cloud_native_ai_architecture/573_timeout_retry_backoff_strategy/) ([Timeout](/knowledge-base/studynote/02_operating_system/05_deadlock/319_timeout_prevention/)) | 얼마나 오래 기다릴 것인가 | 무한 대기 방지 | 너무 길면 자원 고갈, 너무 짧으면 오탐 |
| 재시도 (Retry) | 잠깐 흔들린 실패를 다시 해볼까 | 일시적 오류 [복구](/knowledge-base/studynote/09_security/13_secops_ir_forensics/658_ir_recovery/) | 과하면 장애를 증폭 |
| [서킷 브레이커](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/307_circuit_breaker_pattern/) | 반복 실패 중 계속 호출할까 | 연쇄 장애 차단 | [임계치](/knowledge-base/studynote/03_network/08_transport_layer/431_ssthresh_slow_start_threshold/) 조정 실패 시 과차단 가능 |
| [벌크헤드](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/308_bulkhead_pattern/) ([Bulkhead](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/308_bulkhead_pattern/)) | 자원을 구획할까 | 장애 격리 | 풀 크기 설계 필요 |
| [폴백](/knowledge-base/studynote/07_enterprise_systems/03_eai_esb_msa/171_fallback_resilience_pattern/) ([Fallback](/knowledge-base/studynote/13_cloud_architecture/03_msa_serverless/129_fallback/)) | 실패 시 무엇을 보여 줄까 | 사용자 경험 완화 | 거짓 성공을 만들면 위험 |
...
1. 브레이커를 [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/) 전체가 아니라 의존성·엔드포인트 단위로 나누었는가?
2. 실패율 계산에서 4xx 같은 비즈니스 오류를 제외했는가?
3. [타임아웃](/knowledge-base/studynote/04_software_engineering/09_cloud_native_ai_architecture/573_timeout_retry_backoff_strategy/)과 재시도 [정책](/knowledge-base/studynote/10_ai/02_dl_architecture_new/164_policy/)이 브레이커보다 앞단에서 정리되어 있는가?
4. Half-Open 상태에서 허용할 탐침 요청 수가 충분히 작고 안전한가?
5. [폴백](/knowledge-base/studynote/07_enterprise_systems/03_eai_esb_msa/171_fallback_resilience_pattern/)이 사용자 경험은 지키되 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 정합성을 해치지 않는가?

### 채택 / 회피 판단

- **채택**: 외부 [API](/knowledge-base/studynote/02_operating_system/01_overview_architecture/014_api_posix/), 원격 [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/), 캐시·[메시지 브로커](/knowledge-base/studynote/07_enterprise_systems/03_eai_esb_msa/145_message_broker_sync_async/)처럼 네트워크·[지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/) 불확실성이 큰 의존성
- **회피 또는 별도 설계**: [입력 검증](/knowledge-base/studynote/09_security/uncategorized/1034_input_validation/) 오류, 로컬 [함수 호출](/knowledge-base/studynote/06_ict_convergence/04_ai_llm/294_function_calling_tool_use/), 실패를 숨기면 안 되는 금융 승인 결과 왜곡 시나리오
...
- **📢 섹션 요약 비유**: 배 한 칸에 물이 새면 그 칸을 막아 나머지 배를 띄우는 것이 먼저이듯, [서킷 브레이커](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/307_circuit_breaker_pattern/)는 전체 [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/)를 살리기 위해 일부 통로를 먼저 차단하는 선택이다.
...
| 개념 | 연결 포인트 |
| :--- | :--- |
| [타임아웃](/knowledge-base/studynote/04_software_engineering/09_cloud_native_ai_architecture/573_timeout_retry_backoff_strategy/) ([Timeout](/knowledge-base/studynote/02_operating_system/05_deadlock/319_timeout_prevention/)) | 기다림의 최대치를 정해 브레이커 이전 단계의 자원 낭비를 줄임 |
| 재시도 (Retry) | 일시적 오류 [복구](/knowledge-base/studynote/09_security/13_secops_ir_forensics/658_ir_recovery/)용이지만 과도하면 브레이커가 막아야 할 대상이 됨 |
| [벌크헤드](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/308_bulkhead_pattern/) ([Bulkhead](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/308_bulkhead_pattern/)) | 자원 풀 분리로 장애 범위를 [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/) 내부에서도 제한 |
| [폴백](/knowledge-base/studynote/07_enterprise_systems/03_eai_esb_msa/171_fallback_resilience_pattern/) ([Fallback](/knowledge-base/studynote/13_cloud_architecture/03_msa_serverless/129_fallback/)) | Open 상태에서 사용자 경험을 보완하지만 정합성 [검증](/knowledge-base/studynote/04_software_engineering/07_object_oriented/395_verification_process_review/)이 필요 |
| [서비스 메시](/knowledge-base/studynote/12_it_management/05_security_compliance/945_service_mesh_istio/) ([Service Mesh](/knowledge-base/studynote/03_network/16_data_center_cloud/828_service_mesh_microservice_communication_infrastructure/)) | [프록시](/knowledge-base/studynote/04_software_engineering/04_testing_quality/264_proxy_pattern_surrogate_access_control/) 계층에서 브레이커 [정책](/knowledge-base/studynote/10_ai/02_dl_architecture_new/164_policy/)을 공통 적용하는 대표 플랫폼 |
| 느린 호출 비율 (Slow [Call](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/189_subroutine_call_return/) Rate) | 실패 전 단계의 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) 저하를 조기 감지하는 지표 |
...
2. [서킷 브레이커](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/307_circuit_breaker_pattern/)는 "지금은 초인종 누르지 말고 잠깐 쉬자"라고 정해 주는 버튼이에요.
