...
가치 흐름 매핑 ([Value Stream Mapping](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/088_value_stream_mapping_vsm/))은 제품이나 [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/)가 고객에게 도달할 때까지 거치는 전체 흐름을 한 장의 지도에 표현하는 린 ([Lean](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/087_lean_software_development_7_principles/)) 기반 분석 도구다. 여기서는 누가 일했는지만 보지 않고, 일이 다음 단계로 넘어가기 전 얼마나 오래 대기했는지까지 같이 기록한다.
...
VSM의 핵심 원리는 가치 흐름을 단계별 상자와 시간선으로 나누어 측정하는 것이다. 각 상자에는 실제 작업 시간, 대기 시간, 재작업 비율, 재공품 (WIP, [Work In Progress](/knowledge-base/studynote/04_software_engineering/uncategorized/661_kanban_wip_limit/)), 완전 정확도 (Complete and Accurate)를 적는다. 이렇게 해야 병목이 한눈에 보이고, 미래 상태 (Future [State](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/272_state_pattern/))를 설계할 때 어떤 구간을 먼저 손댈지 판단할 수 있다.

| 지표 | 의미 | 대표 공식 또는 측정 방식 | 해석 포인트 |
| :--- | :--- | :--- | :--- |
| [리드 타임](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/085_lead_time_cycle_time/) (LT) | 요청부터 인도까지 전체 시간 | LT = PT + WT | 고객이 실제로 기다리는 시간 |
| 작업 시간 (PT) | 가치가 더해지는 순수 작업 시간 | 실제 touch time 합계 | 현장 생산성 자체 |
| 대기 시간 (WT) | 큐, 승인, handoff로 멈춘 시간 | 단계 간 [idle](/knowledge-base/studynote/02_operating_system/10_security/611_cpu_idle_wait_optimization/) time 합계 | 병목과 [사일로](/knowledge-base/studynote/15_devops_sre/01_culture_methodology/002_silo_hyeonhyung/)의 흔적 |
| 공정 주기 효율 (PCE) | 가치 있는 시간의 비율 | PCE = PT / LT × 100 | 전체 흐름의 건강도 |
| WIP | 착수했지만 끝나지 않은 항목 수 | [진행](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/216_progress_in_synchronization/) 중 항목 수 | 너무 많으면 대기와 문맥 전환 증가 |
...
VSM을 적용할 때는 먼저 어떤 가치 흐름을 한 번에 볼지 범위를 명확히 정해야 한다. 모든 업무를 한 장에 담으려 하면 복잡해서 아무 것도 보이지 않는다. 보통은 제품군 하나, 고객 여정 하나, 또는 incident-to-[recovery](/knowledge-base/studynote/09_security/13_secops_ir_forensics/658_ir_recovery/) 같은 특정 흐름 하나를 잡고 시작하는 것이 좋다.
...
2. 단계별 PT와 WT를 추정이 아니라 실제 [로그](/knowledge-base/studynote/04_software_engineering/09_cloud_native_ai_architecture/568_logs_distributed_logging_elk_fluentd/)나 인터뷰로 수집했는가?
3. 재작업 발생 구간과 승인 대기 구간을 별도 표시했는가?
4. 미래 상태 맵에서 제거 가능한 handoff와 자동화 가능한 대기를 구분했는가?

### [안티패턴](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/128_water_scrum_fall_anti_pattern/)

- 부서별 절차만 예쁘게 정리하고 시간 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)는 비워 두는 [VSM](/knowledge-base/studynote/15_devops_sre/01_culture_methodology/030_value_stream_mapping/)
- 평균값만 적고 변동폭과 예외 경로를 무시하는 [VSM](/knowledge-base/studynote/15_devops_sre/01_culture_methodology/030_value_stream_mapping/)
- [현재 상태](/knowledge-base/studynote/04_software_engineering/03_design_architecture/178_as_is_to_be_analysis/) 분석 없이 곧바로 도구 도입을 해결책으로 제시하는 [VSM](/knowledge-base/studynote/15_devops_sre/01_culture_methodology/030_value_stream_mapping/)

기술사 답안에서는 VSM을 단순 프로세스 그림으로 설명하면 부족하다. 반드시 LT, PT, WT, PCE 같은 시간 기반 지표와 병목 제거 [전략](/knowledge-base/studynote/04_software_engineering/04_testing_quality/268_strategy_pattern/)을 함께 써야 실무적 가치가 살아난다.
...
<- **이전**: [223. DORA 메트릭스 (DORA Metrics)](/knowledge-base/studynote/07_enterprise_systems/04_process_consulting/223_dora_metrics_agile/)
**다음**: [225. TCO 분석 (Total Cost of Ownership Analysis)](/knowledge-base/studynote/07_enterprise_systems/08_cloud_finops/225_tco_analysis_cloud_migration/) ->
