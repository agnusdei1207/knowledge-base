...
가트너 (Gartner)가 제시한 바이모달 IT는 모든 시스템과 조직에 같은 속도를 강요하지 말고, 안정성이 최우선인 영역과 실험 속도가 최우선인 영역을 분리해 운영하자는 개념이다. 즉 기록 시스템과 실험 시스템이 서로 다른 리듬으로 움직이도록 허용하는 [전략](/knowledge-base/studynote/04_software_engineering/04_testing_quality/268_strategy_pattern/)이다.

이 개념이 등장한 이유는 전통 기업이 서로 상반된 요구를 동시에 받기 시작했기 때문이다. 금융 원장, [ERP](/knowledge-base/studynote/07_enterprise_systems/02_erp_systems/081_erp_enterprise_resource_planning/), 결제 시스템처럼 한 번의 오류도 치명적인 코어 영역은 느리더라도 안전해야 한다. 반면 모바일 앱, 고객 경험 개선, [인공지능](/knowledge-base/studynote/10_ai/03_llm_nlp/231_ai_turing_test/) 실험 같은 영역은 빠르게 가설을 [검증](/knowledge-base/studynote/04_software_engineering/07_object_oriented/395_verification_process_review/)하지 않으면 시장에서 뒤처진다.
...
바이모달 IT는 종종 "Mode 2가 더 우수하다"는 식으로 오해되지만, 실제로는 과도기적 운영 모델에 가깝다. 최근에는 멀티스피드 IT (Multi-speed IT), 엔터프라이즈 [애자일](/knowledge-base/studynote/15_devops_sre/01_culture_methodology/004_agile_relation/), [플랫폼 엔지니어링](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/109_platform_engineering_cognitive_load/)이 그 대안이나 확장 형태로 언급된다.

| 항목 | 바이모달 IT | 멀티스피드 IT | 전사 [애자일](/knowledge-base/studynote/15_devops_sre/01_culture_methodology/004_agile_relation/) / 단일 모델 |
| :-- | :-- | :-- | :-- |
...
3. [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 소유권과 변경 권한 경계가 정의되어 있는가?
...
- Mode 2를 규정 면제 구역처럼 운영해 보안과 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 품질 사고를 내는 경우
- Mode 1 인력을 "구식 조직"으로 낙인찍어 협업을 깨뜨리는 경우
- 경계 계층 없이 프론트 [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/)가 코어 DB를 직접 조회하게 하는 경우

기술사 답안에서는 바이모달 IT를 "속도를 분리하되 인터페이스는 통제한다"는 문장으로 [압축](/knowledge-base/studynote/02_operating_system/06_memory_management/347_compaction/)하면 좋다. 분리와 연계가 동시에 있어야 의미가 있다.

- **📢 섹션 요약 비유**: 바이모달 IT 운영은 빠른 오토바이 도로와 느린 화물차 도로를 분리하되, 합류 지점에는 반드시 [신호](/knowledge-base/studynote/02_operating_system/02_process_thread/130_signal/)등과 안전 차선을 두는 교통 설계와 같다.
