...
> 1. **본질**: [HMAC](/knowledge-base/studynote/03_network/13_network_security_basics/674_hmac_hash_based_mac_ipsec/) (Hash-based [Message Authentication Code](/knowledge-base/studynote/03_network/13_network_security_basics/673_mac_message_authentication_code/))은 암호학적 [해시 함수](/knowledge-base/studynote/03_network/13_network_security_basics/667_hash_function_integrity_one_way/)에 송수신자만이 아는 '비밀키([Secret](/knowledge-base/studynote/04_software_engineering/08_security_compliance_devsecops/514_secret_management_vault_kms/) [Key](/knowledge-base/studynote/05_database/02_modeling_normalization/067_db_key_uniqueness_minimality/))'를 결합하여 해시값을 [생성](/knowledge-base/studynote/02_operating_system/02_process_thread/087_process_state_transition/)함으로써, 메시지의 [무결성](/knowledge-base/studynote/09_security/01_intro_principles/003_integrity/)과 송신자의 [인증](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/303_authentication_authorization_patterns/)을 동시에 보장하는 기술이다.
> 2. **가치**: 단순 해시는 누구나 값을 변조하고 새로운 해시를 씌울 수 있지만, HMAC은 비밀키가 없으면 유효한 해시 태그를 만들 수 없으므로 [중간자 공격](/knowledge-base/studynote/03_network/14_network_security_threats/706_mitm_man_in_the_middle_hsts/)(Man-in-the-Middle)에 의한 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 위변조를 완벽하게 차단한다.
> 3. **판단 포인트**: [HMAC](/knowledge-base/studynote/03_network/13_network_security_basics/674_hmac_hash_based_mac_ipsec/) 자체는 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 암호화하여 숨기지 않는다. 따라서 [기밀성](/knowledge-base/studynote/09_security/01_intro_principles/002_confidentiality/)이 필요 없고 오직 "내용이 바뀌지 않았고, 권한 있는 자가 보낸 것이 맞다"는 증명만 필요한 [API](/knowledge-base/studynote/02_operating_system/01_overview_architecture/014_api_posix/) [인증](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/303_authentication_authorization_patterns/)([JWT](/knowledge-base/studynote/03_network/10_application_layer_dns_mgmt/549_jwt_json_web_token/), AWS [API](/knowledge-base/studynote/02_operating_system/01_overview_architecture/014_api_posix/) 등)에서 가장 강력하고 가벼운 수단으로 사용된다.

---

## Ⅰ. 개요 및 필요성

[데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 전송할 때 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)가 중간에 깨지거나 변조되었는지 [확인](/knowledge-base/studynote/04_software_engineering/12_testing_maintenance/396_validation/)하기 위해 해시(Hash) 함수(예: SHA-256)를 사용한다. 하지만 일반적인 [해시 함수](/knowledge-base/studynote/03_network/13_network_security_basics/667_hash_function_integrity_one_way/)는 키([Key](/knowledge-base/studynote/05_database/02_modeling_normalization/067_db_key_uniqueness_minimality/))가 없는 공개된 알고리즘이므로 심각한 보안 구멍이 발생한다. 해커가 통신을 가로채서 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 조작한 뒤, 조작된 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)로 해시 믹서기를 다시 돌려 새로운 해시값을 붙여 보내면 수신자는 정상적인 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)로 착각하게 된다.

이러한 무방비 상태를 해결하기 위해 등장한 개념이 [MAC](/knowledge-base/studynote/03_network/13_network_security_basics/673_mac_message_authentication_code/)(메시지 [인증](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/303_authentication_authorization_patterns/) 코드)이다. 메시지를 해시할 때 '통신 당사자들만 비밀리에 공유하는 열쇠([Key](/knowledge-base/studynote/05_database/02_modeling_normalization/067_db_key_uniqueness_minimality/))'를 함께 넣고 돌리자는 아이디어다. 해커가 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 고치더라도 비밀키를 모르기 때문에 수신자 서버가 요구하는 정확한 해시 결과를 절대 만들어낼 수 없다. 그 [MAC](/knowledge-base/studynote/03_network/13_network_security_basics/673_mac_message_authentication_code/) 구조 중에서 [해시 함수](/knowledge-base/studynote/03_network/13_network_security_basics/667_hash_function_integrity_one_way/)를 엔진으로 사용하는 가장 표준적이고 안전한 방식이 바로 HMAC이다.
...
| 연산 요소 | 설명 및 역할 |
| :--- | :--- |
| **비밀키 ($K$)** | 송수신자가 사전 공유한 비밀값. 길이가 길면 해시하고, 짧으면 0으로 [패딩](/knowledge-base/studynote/10_ai/01_ai_basics/098_padding_convolutional_neural_network_same_valid/) |
| **$ipad$ / $opad$** | 내부 [패딩](/knowledge-base/studynote/10_ai/01_ai_basics/098_padding_convolutional_neural_network_same_valid/)(0x36)과 외부 [패딩](/knowledge-base/studynote/10_ai/01_ai_basics/098_padding_convolutional_neural_network_same_valid/)(0x5C). 하나의 키를 두 개의 독립적인 키처럼 변형시킴 |
...
HMAC은 [무결성](/knowledge-base/studynote/09_security/01_intro_principles/003_integrity/)과 [인증](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/303_authentication_authorization_patterns/)을 제공하지만, '[기밀성](/knowledge-base/studynote/09_security/01_intro_principles/002_confidentiality/)(암호화)'은 제공하지 않는다. 따라서 유사한 암호학적 도구들과 정확히 구분하여 사용해야 한다.
...
실무에서는 메시지의 내용을 숨길 필요 없이 빠른 [인증](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/303_authentication_authorization_patterns/)이 필요할 때 HMAC을 단독으로 쓴다. 만약 내용도 숨기고 [인증](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/303_authentication_authorization_patterns/)도 해야 한다면 '[AES](/knowledge-base/studynote/03_network/13_network_security_basics/656_aes_advanced_encryption_standard_rijndael/)-GCM' 같은 [인증](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/303_authentication_authorization_patterns/) 암호화([AEAD](/knowledge-base/studynote/09_security/02_crypto/092_aead/)) 모드를 사용하거나, [AES](/knowledge-base/studynote/03_network/13_network_security_basics/656_aes_advanced_encryption_standard_rijndael/) 암호문 뒤에 HMAC을 붙이는 [MAC](/knowledge-base/studynote/03_network/13_network_security_basics/673_mac_message_authentication_code/)-then-Encrypt 방식을 혼합하여 사용하게 된다.

- **📢 섹션 요약 비유**: [AES](/knowledge-base/studynote/03_network/13_network_security_basics/656_aes_advanced_encryption_standard_rijndael/) 암호화가 편지 내용을 못 읽게 '암호문'으로 쓰는 거라면, HMAC은 편지를 '투명한 유리'로 보내되 누가 보냈는지만 투명한 '보증 수표'를 붙이는 것과 같다.
...
1. 친구한테 보물지도([데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/))를 보낼 때, 그냥 지문(해시)만 찍으면 나쁜 해적이 자기 지문을 덮어찍어 엉뚱한 지도로 바꿀 수 있어요.
