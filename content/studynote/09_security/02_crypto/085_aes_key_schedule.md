...
> **판단 포인트**: 구현은 단순해 보여도 정답 벡터와 [바이트](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/074_byte/) 순서, 부채널(Side Channel) 방어가 틀리면 전체 암호가 무너진다.

---

## Ⅰ. 개요 및 필요성

[AES](/knowledge-base/studynote/03_network/13_network_security_basics/656_aes_advanced_encryption_standard_rijndael/) ([Advanced Encryption Standard](/knowledge-base/studynote/03_network/13_network_security_basics/656_aes_advanced_encryption_standard_rijndael/))는 128비트 블록 암호이고, 키 [스케줄](/knowledge-base/studynote/05_database/04_transactions_concurrency/208_schedule_history_transaction_execution_order/)([Key](/knowledge-base/studynote/05_database/02_modeling_normalization/067_db_key_uniqueness_minimality/) Schedule)은 그 입력 키를 라운드 키로 확장하는 과정이다. 같은 키를 라운드마다 그대로 쓰면 중간 상태가 반복되어 구조가 드러나기 쉬우므로, 각 라운드에 다른 재료를 공급하는 절차가 필요하다.

키 [스케줄](/knowledge-base/studynote/05_database/04_transactions_concurrency/208_schedule_history_transaction_execution_order/)의 목적은 "키를 더 복잡하게 만드는 것"이 아니라 "라운드별로 다른 마찰을 넣는 것"이다. 그래서 키 [스케줄](/knowledge-base/studynote/05_database/04_transactions_concurrency/208_schedule_history_transaction_execution_order/)은 암호의 본체와 분리된 보조 단계처럼 보이지만, 실제로는 보안성과 구현 안정성을 함께 결정한다.
...
| --- | --- | --- | --- |
| [AES](/knowledge-base/studynote/03_network/13_network_security_basics/656_aes_advanced_encryption_standard_rijndael/)-128 | [10](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/489_raid_10_hybrid/) | 44 | 가장 단순하고 빠름 |
| [AES](/knowledge-base/studynote/03_network/13_network_security_basics/656_aes_advanced_encryption_standard_rijndael/)-192 | 12 | 52 | 중간 균형 |
| [AES](/knowledge-base/studynote/03_network/13_network_security_basics/656_aes_advanced_encryption_standard_rijndael/)-256 | 14 | 60 | 가장 길고 추가 변형 단계 존재 |

[AES](/knowledge-base/studynote/03_network/13_network_security_basics/656_aes_advanced_encryption_standard_rijndael/)-256은 특정 위치마다 `SubWord`를 한 번 더 적용해 구조적 반복을 더 줄인다. 이 규칙 덕분에 확장 키는 단순 복사본이 아니라, 비선형 성질이 섞인 라운드 재료가 된다.
...
실무에서는 검증된 라이브러리를 쓰고, 직접 구현이 필요하면 표준 테스트 벡터와 함께 라운드 키를 대조해야 한다. [바이트](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/074_byte/) 순서(endianness), 상수 XOR, 패딩은 자주 틀리는 지점이며, 키는 메모리에서 빠르게 지워야 한다. 부채널 방어가 필요한 환경이라면, 테이블 기반 S-box보다 하드웨어 명령이나 상수 시간 구현을 우선 고려한다.

### [체크리스트](/knowledge-base/studynote/04_software_engineering/11_testing_validation/435_checklist_based_testing/)
1. [AES](/knowledge-base/studynote/03_network/13_network_security_basics/656_aes_advanced_encryption_standard_rijndael/)-128/192/256 각각의 라운드 수와 확장 크기를 검증했는가?
2. [바이트](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/074_byte/) 순서와 [워드](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/075_word/) 조합을 명확히 정의했는가?
3. 키와 라운드 키가 로그나 덤프에 남지 않는가?

### [안티패턴](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/128_water_scrum_fall_anti_pattern/)
- 키 [스케줄](/knowledge-base/studynote/05_database/04_transactions_concurrency/208_schedule_history_transaction_execution_order/)을 비밀번호 해싱처럼 오해하는 것
...
잘 설계된 키 [스케줄](/knowledge-base/studynote/05_database/04_transactions_concurrency/208_schedule_history_transaction_execution_order/)은 라운드 간 분리를 강화해, 공격자가 특정 라운드의 규칙을 전체 키로 역추적하기 어렵게 만든다. 반면 구현이 틀리면 복호화가 아예 맞지 않거나, 더 나쁘게는 보안 구멍을 만든다. AES에서는 암호 본체 못지않게 키 [스케줄](/knowledge-base/studynote/05_database/04_transactions_concurrency/208_schedule_history_transaction_execution_order/)의 정확성이 중요하다.

결론적으로 [AES](/knowledge-base/studynote/03_network/13_network_security_basics/656_aes_advanced_encryption_standard_rijndael/) 키 [스케줄](/knowledge-base/studynote/05_database/04_transactions_concurrency/208_schedule_history_transaction_execution_order/)은 "키를 늘리는 코드"가 아니라 "암호의 반사경을 흐리게 만드는 장치"로 기억해야 한다. 표준, 테스트, 부채널 대응이 함께 있어야 비로소 안전한 구현이 된다.
...
| 개념 | 연결 포인트 |
| --- | --- |
| [AES](/knowledge-base/studynote/03_network/13_network_security_basics/656_aes_advanced_encryption_standard_rijndael/) | 블록 암호의 본체 |
...
1. [AES](/knowledge-base/studynote/03_network/13_network_security_basics/656_aes_advanced_encryption_standard_rijndael/) 키 [스케줄](/knowledge-base/studynote/05_database/04_transactions_concurrency/208_schedule_history_transaction_execution_order/)은 요리사가 큰 반죽을 [10](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/489_raid_10_hybrid/)~14개의 작은 반죽으로 나누는 일이에요.
