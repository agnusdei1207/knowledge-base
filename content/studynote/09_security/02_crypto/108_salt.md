...
## 핵심 인사이트 (3줄 요약)
> 1. **본질**: [솔트](/knowledge-base/studynote/03_network/13_network_security_basics/671_password_hash_salt_pbkdf2_bcrypt_argon2/)([Salt](/knowledge-base/studynote/03_network/13_network_security_basics/671_password_hash_salt_pbkdf2_bcrypt_argon2/))는 비밀번호를 [데이터베이스](/knowledge-base/studynote/05_database/01_db_architecture_relational/002_database_definition/)에 안전하게 저장할 때, 사용자마다 고유하게 발급된 무작위 문자열을 평문 비밀번호에 이어 붙인 뒤 해싱(Hashing)하는 기법이다.
> 2. **가치**: 똑같은 비밀번호를 쓰는 사용자가 있어도 각각 뿌려진 [솔트](/knowledge-base/studynote/03_network/13_network_security_basics/671_password_hash_salt_pbkdf2_bcrypt_argon2/) 값이 다르기 때문에 서로 완전히 다른 해시값이 저장되어, 해커의 레인보우 테이블([Rainbow Table](/knowledge-base/studynote/09_security/02_crypto/107_rainbow_table/)) 사전 공격을 불가능하게 만든다.
> 3. **판단 포인트**: [솔트](/knowledge-base/studynote/03_network/13_network_security_basics/671_password_hash_salt_pbkdf2_bcrypt_argon2/) 하나만으로는 [GPU](/knowledge-base/studynote/01_computer_architecture/12_accelerators_ai_hardware/418_gpu/) 기반 무차별 대입 공격의 엄청난 연산 속도를 막아낼 수 없으므로, 현대 방어 아키텍처는 반드시 [솔트](/knowledge-base/studynote/03_network/13_network_security_basics/671_password_hash_salt_pbkdf2_bcrypt_argon2/)와 해시를 수만 번 반복하는 [키 스트레칭](/knowledge-base/studynote/09_security/02_crypto/109_key_stretching/)([Key Stretching](/knowledge-base/studynote/09_security/02_crypto/109_key_stretching/))을 융합해야 한다.

---

## Ⅰ. 개요 및 필요성

[솔트](/knowledge-base/studynote/03_network/13_network_security_basics/671_password_hash_salt_pbkdf2_bcrypt_argon2/)([Salt](/knowledge-base/studynote/03_network/13_network_security_basics/671_password_hash_salt_pbkdf2_bcrypt_argon2/))는 비밀번호를 일방향 암호화할 때 평문의 예측 가능성을 제거하기 위해 추가하는 난수 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)다. 단순 [해시 함수](/knowledge-base/studynote/03_network/13_network_security_basics/667_hash_function_integrity_one_way/)(SHA-256 등)는 동일한 입력에 대해 항상 동일한 출력(결정론적 특성)을 반환한다. 이는 해커가 미리 계산해둔 거대한 해시-평문 매핑 테이블(레인보우 테이블)을 통해 암호를 즉시 역산할 수 있는 치명적 약점을 낳는다.

[솔트](/knowledge-base/studynote/03_network/13_network_security_basics/671_password_hash_salt_pbkdf2_bcrypt_argon2/)는 이 약점을 덮기 위해 도입되었다. 사용자마다 비밀번호가 같더라도 각기 다른 [솔트](/knowledge-base/studynote/03_network/13_network_security_basics/671_password_hash_salt_pbkdf2_bcrypt_argon2/)가 섞여 들어가므로, 해커가 저장된 해시값을 훔쳐도 기존의 레인보우 테이블은 휴지조각이 되며, 해독을 위해서는 모든 사용자마다 전용 테이블을 새로 만들어야 하는 무한대의 비용을 치르게 된다.

- **📢 섹션 요약 비유**: 해커가 전 세계 공통 비밀번호 족보를 훔쳐와도, 선생님이 학생마다 문제지 번호와 순서를 엉뚱하게 뒤섞어 버려([솔트](/knowledge-base/studynote/03_network/13_network_security_basics/671_password_hash_salt_pbkdf2_bcrypt_argon2/) 추가) 족보가 완전히 쓸모없어지는 완벽한 커닝 방지 시스템이다.

---

## Ⅱ. 아키텍처 및 핵심 원리

[솔트](/knowledge-base/studynote/03_network/13_network_security_basics/671_password_hash_salt_pbkdf2_bcrypt_argon2/)의 핵심 설계 원리는 '고유성'과 '비선형성 도입'이다. 비밀번호 [검증](/knowledge-base/studynote/04_software_engineering/07_object_oriented/395_verification_process_review/) 시스템은 회원가입 시 [솔트](/knowledge-base/studynote/03_network/13_network_security_basics/671_password_hash_salt_pbkdf2_bcrypt_argon2/)를 생성하여 저장하고, 로그인 시 이를 재사용하여 해시값을 대조한다.

| 구성 단계 | 동작 원리 | 저장 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) |
| :--- | :--- | :--- |
...
- **📢 섹션 요약 비유**: 똑같은 돼지고기(1234)라도 집마다 뿌리는 향신료([솔트](/knowledge-base/studynote/03_network/13_network_security_basics/671_password_hash_salt_pbkdf2_bcrypt_argon2/))가 다르면 완전히 다른 요리가 되어서, 도둑이 먹어보고 이 고기가 원래 어느 농장 출신인지 도저히 맞출 수 없게 만든다.

---

## Ⅲ. 비교 및 연결

비밀번호 [보호](/knowledge-base/studynote/02_operating_system/10_security/571_protection_vs_security/) 기술은 단순 해싱에서 출발하여 [솔트](/knowledge-base/studynote/03_network/13_network_security_basics/671_password_hash_salt_pbkdf2_bcrypt_argon2/)를 거쳐 [키 스트레칭](/knowledge-base/studynote/09_security/02_crypto/109_key_stretching/)으로 진화해왔다.
...
- **📢 섹션 요약 비유**: 단순 해싱이 얇은 유리창이라면, [솔트](/knowledge-base/studynote/03_network/13_network_security_basics/671_password_hash_salt_pbkdf2_bcrypt_argon2/)는 유리창을 미로 속에 숨겨두는 것이고, [키 스트레칭](/knowledge-base/studynote/09_security/02_crypto/109_key_stretching/)은 그 유리를 방탄유리로 아주 두껍게 만드는 것이다. 숨기기만 해선 안 되고 부수기도 어렵게 만들어야 한다.

---

## Ⅳ. 실무 적용 및 기술사 판단

[솔트](/knowledge-base/studynote/03_network/13_network_security_basics/671_password_hash_salt_pbkdf2_bcrypt_argon2/)를 설계할 때 가장 조심해야 할 것은 [솔트](/knowledge-base/studynote/03_network/13_network_security_basics/671_password_hash_salt_pbkdf2_bcrypt_argon2/)의 무작위성과 고유성 확보이다. 
...
[솔트](/knowledge-base/studynote/03_network/13_network_security_basics/671_password_hash_salt_pbkdf2_bcrypt_argon2/)는 [데이터베이스](/knowledge-base/studynote/05_database/01_db_architecture_relational/002_database_definition/)에 저장된 암호 해시의 군집 패턴(클러스터링)을 완벽히 파괴한다. 설령 수만 명의 유저가 똑같은 '123456'을 사용하더라도 [데이터베이스](/knowledge-base/studynote/05_database/01_db_architecture_relational/002_database_definition/) 상에서는 어떠한 통계적 연관성도 찾을 수 없어, 공격자의 해독 비용을 사용자 수만큼 기하급수적으로 폭증시킨다.

결론적으로 [솔트](/knowledge-base/studynote/03_network/13_network_security_basics/671_password_hash_salt_pbkdf2_bcrypt_argon2/)는 결정론적인 [해시 함수](/knowledge-base/studynote/03_network/13_network_security_basics/667_hash_function_integrity_one_way/)에 '무작위성'이라는 암호학적 면역력을 주입한 핵심 패치다. 현대 서비스에서는 이 [솔트](/knowledge-base/studynote/03_network/13_network_security_basics/671_password_hash_salt_pbkdf2_bcrypt_argon2/)의 개념을 내재화하여 연산 [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/)까지 포함시킨 Bcrypt, PBKDF2, Argon2와 같은 융합 알고리즘을 사용하는 것이 기술적 표준이며 타협할 수 없는 기본 보안 원칙이다.

- **📢 섹션 요약 비유**: 작은 난수 가루([솔트](/knowledge-base/studynote/03_network/13_network_security_basics/671_password_hash_salt_pbkdf2_bcrypt_argon2/)) 한 줌이 해커가 수십 년간 모아 온 거대한 해시 사전(무기)을 순식간에 고철로 만들어버리는, 비밀번호 [보호](/knowledge-base/studynote/02_operating_system/10_security/571_protection_vs_security/)의 가장 강력한 방부제다.
...
2. 그래서 선생님이 친구들마다 비밀번호 끝에 아무도 모르는 마법의 스티커([솔트](/knowledge-base/studynote/03_network/13_network_security_basics/671_password_hash_salt_pbkdf2_bcrypt_argon2/))를 몰래 붙여줬어요! 철수는 '1234+별', 영희는 '1234+달' 처럼요.
...
← **이전**: [107. Rainbow Table — 사전 계산 해시 테이블, 역산 공격](/knowledge-base/studynote/09_security/02_crypto/107_rainbow_table/)
**다음**: [109. 키 스트레칭 (Key Stretching) & 패스워드 해싱 함수 (Key Stretching)](/knowledge-base/studynote/09_security/02_crypto/109_key_stretching/) →
