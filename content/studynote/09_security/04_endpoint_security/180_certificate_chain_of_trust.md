...
> 1. **본질**: 인증서 체인 [검증](/knowledge-base/studynote/04_software_engineering/07_object_oriented/395_verification_process_review/)은 End Entity 인증서가 혼자 신뢰되는 것이 아니라, Intermediate [CA](/knowledge-base/studynote/06_ict_convergence/01_blockchain/089_contract_account_smart_contract/) (Intermediate Certificate Authority)와 Root [CA](/knowledge-base/studynote/06_ict_convergence/01_blockchain/089_contract_account_smart_contract/) (Root Certificate Authority)까지 이어지는 서명 경로가 클라이언트의 Trust Store에 닿는지 [확인](/knowledge-base/studynote/04_software_engineering/12_testing_maintenance/396_validation/)하는 과정이다.
> 2. **가치**: 이 구조 덕분에 Root Certificate Authority는 오프라인에 가깝게 [보호](/knowledge-base/studynote/02_operating_system/10_security/571_protection_vs_security/)하고, 실제 발급은 Intermediate Certificate Authority가 맡아 대규모 인터넷에서도 확장성과 사고 격리를 동시에 확보한다.
> 3. **판단 포인트**: 실무에서는 "인증서가 있느냐"보다 "체인이 완전한가, 클라이언트가 그 루트를 신뢰하는가, Intermediate 인증서와 폐기 정보까지 정상 [검증](/knowledge-base/studynote/04_software_engineering/07_object_oriented/395_verification_process_review/)되는가"가 더 중요하다.

---

## Ⅰ. 개요 및 필요성

인증서 체인 [검증](/knowledge-base/studynote/04_software_engineering/07_object_oriented/395_verification_process_review/)은 공개 키 기반 구조인 [PKI](/knowledge-base/studynote/09_security/03_network_security/159_pki_public_key_infrastructure/) ([Public Key Infrastructure](/knowledge-base/studynote/09_security/uncategorized/1080_pki_public_key_infrastructure_ca_ra_certificate/))에서 신뢰를 전달하는 핵심 절차다. 서버가 내미는 End Entity 인증서만 보고 곧바로 믿는 것이 아니라, "이 인증서를 누가 서명했는가, 그 서명자를 또 누가 보증하는가"를 거슬러 올라가 최종적으로 클라이언트가 이미 신뢰하고 있는 Root Certificate Authority에 도달하는지를 본다. 즉 [TLS](/knowledge-base/studynote/02_operating_system/11_exam_summary/694_thread_local_storage_tls/) (Transport Layer [Security](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/283_security_tactics/))의 신원 [검증](/knowledge-base/studynote/04_software_engineering/07_object_oriented/395_verification_process_review/)은 한 장짜리 증명서가 아니라 **연결된 증명 경로** 위에서 성립한다.
...
- **📢 섹션 요약 비유**: 인증서 체인은 동네 가게 사장님을 믿는 것이 아니라, 그 가게의 사업자 등록과 본사 허가서를 차례대로 [확인](/knowledge-base/studynote/04_software_engineering/12_testing_maintenance/396_validation/)해 결국 내가 이미 아는 상공회의소 도장까지 올라가 보는 과정과 같다.

---

## Ⅱ. 아키텍처 및 핵심 원리

클라이언트의 [검증](/knowledge-base/studynote/04_software_engineering/07_object_oriented/395_verification_process_review/)은 보통 세 층을 본다. 첫째, 서버가 내민 End Entity 인증서가 요청한 호스트명과 일치하는지 본다. 둘째, 그 인증서를 서명한 Intermediate Certificate Authority 인증서를 따라 올라간다. 셋째, 마지막에 도달한 Root Certificate Authority가 로컬 Trust Store에 있는지 [확인](/knowledge-base/studynote/04_software_engineering/12_testing_maintenance/396_validation/)한다. 이때 서명 수학만 맞으면 끝이 아니라, 유효기간, [Key](/knowledge-base/studynote/05_database/02_modeling_normalization/067_db_key_uniqueness_minimality/) Usage, [Extended Key Usage](/knowledge-base/studynote/09_security/04_endpoint_security/199_extended_key_usage_eku_serverauth/), 폐기 상태까지 함께 본다.

| 구성 요소 | 역할 | 실무 포인트 |
| :--- | :--- | :--- |
| End Entity 인증서 | 실제 서버나 [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/)의 신원 표현 | [SAN](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/493_san_storage_area_network/) ([Subject Alternative Name](/knowledge-base/studynote/09_security/04_endpoint_security/174_san_subject_alternative_name/)), 유효기간, [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/) 용도 [확인](/knowledge-base/studynote/04_software_engineering/12_testing_maintenance/396_validation/) |
| Intermediate Certificate Authority | 하위 인증서 발급과 위임 수행 | 대규모 발급, 사고 시 중간 체인만 폐기 가능 |
| Root Certificate Authority | 최종 신뢰 기준점 | 클라이언트 Trust Store에 사전 탑재, 오프라인 [보호](/knowledge-base/studynote/02_operating_system/10_security/571_protection_vs_security/) 지향 |
| Trust Store | [운영체제](/knowledge-base/studynote/02_operating_system/01_overview_architecture/001_operating_system_purpose/)·브라우저가 신뢰하는 루트 목록 | 클라이언트 종류별 신뢰 차이 발생 가능 |
| Revocation 정보 | 이미 발급된 인증서 무효화 판단 | [CRL](/knowledge-base/studynote/03_network/13_network_security_basics/678_crl_certificate_revocation_list/) ([Certificate Revocation List](/knowledge-base/studynote/03_network/13_network_security_basics/678_crl_certificate_revocation_list/)), [OCSP](/knowledge-base/studynote/03_network/13_network_security_basics/679_ocsp_online_certificate_status_protocol/) (Online Certificate Status [Protocol](/knowledge-base/studynote/03_network/06_network_layer_ip/295_protocol_field_tcp_udp_icmp/)) [확인](/knowledge-base/studynote/04_software_engineering/12_testing_maintenance/396_validation/) |
...
| :--- | :--- | :--- | :--- |
| 신뢰 시작점 | 서버 자신 | 조직 내부 Root Certificate Authority | 브라우저·[운영체제](/knowledge-base/studynote/02_operating_system/01_overview_architecture/001_operating_system_purpose/)가 신뢰하는 공개 루트 |
| 기본 브라우저 신뢰 | 없음 | 내부 단말에 루트 배포 시 가능 | 기본 제공 |
| 확장성 | 낮음 | 내부 [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/) 수가 많을수록 유리 | 외부 공개 [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/)에 적합 |
...
2. 클라이언트 집합별로 신뢰 루트 [호환성](/knowledge-base/studynote/04_software_engineering/06_software_architecture/344_compatibility_usability/)을 테스트했는가?
3. [OCSP Stapling](/knowledge-base/studynote/03_network/13_network_security_basics/680_ocsp_stapling_tls_handshake_performance/), [CRL](/knowledge-base/studynote/03_network/13_network_security_basics/678_crl_certificate_revocation_list/) 배포, 인증서 만료 모니터링이 함께 설계되어 있는가?
4. 루트 전환 시 Cross-Signing 또는 이중 체인 전략이 필요한가?
5. 공개 [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/)와 내부망 [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/)를 같은 [PKI](/knowledge-base/studynote/09_security/03_network_security/159_pki_public_key_infrastructure/) [정책](/knowledge-base/studynote/10_ai/02_dl_architecture_new/164_policy/)으로 다뤄도 되는지 구분했는가?

### 자주 발생하는 [안티패턴](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/128_water_scrum_fall_anti_pattern/)

- 브라우저 개발자 도구에서 한 번 열렸다고 체인 [검증](/knowledge-base/studynote/04_software_engineering/07_object_oriented/395_verification_process_review/)을 끝낸 것으로 판단하는 운영
- Intermediate 인증서를 누락한 채 [AIA](/knowledge-base/studynote/09_security/04_endpoint_security/194_authority_information_access_aia_ocsp/) ([Authority Information Access](/knowledge-base/studynote/09_security/04_endpoint_security/194_authority_information_access_aia_ocsp/)) 자동 다운로드에만 기대는 구성
...
| [SAN](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/493_san_storage_area_network/) ([Subject Alternative Name](/knowledge-base/studynote/09_security/04_endpoint_security/174_san_subject_alternative_name/)) | 체인이 유효해도 실제 접속 도메인과 이름이 맞아야 통과한다. |
| [CRL](/knowledge-base/studynote/03_network/13_network_security_basics/678_crl_certificate_revocation_list/) / [OCSP](/knowledge-base/studynote/03_network/13_network_security_basics/679_ocsp_online_certificate_status_protocol/) | 발급 후 폐기된 인증서를 걸러내는 [검증](/knowledge-base/studynote/04_software_engineering/07_object_oriented/395_verification_process_review/) 정보다. |
| Cross-Signing | 루트 전환기 [호환성](/knowledge-base/studynote/04_software_engineering/06_software_architecture/344_compatibility_usability/)을 위해 여러 신뢰 경로를 제공하는 기법이다. |
...
1. 인터넷 친구가 자기 이름표만 보여 주면 바로 믿지 않고, 그 친구를 소개해 준 선생님 이름표도 함께 [확인](/knowledge-base/studynote/04_software_engineering/12_testing_maintenance/396_validation/)해요.
