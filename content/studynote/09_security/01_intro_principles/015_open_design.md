...
#### 1. 실무 시나리오 및 의사결정
- <strong>암호 알고리즘의 선택</strong>: 핀테크 서비스를 개발할 때, 사내 최고 개발자가 "내가 새로운 해시 알고리즘을 만들면 해커들이 레인보우 테이블을 쓸 수 없으니 더 안전할 것"이라고 제안한다. 기술사/CISO는 이를 즉각 기각해야 한다. 아무리 뛰어난 개발자라도 암호학계 전체의 검증을 거치지 않은 알고리즘은 예측 불가능한 수학적 취약점(충돌 등)을 가질 확률이 100%에 가깝다. 반드시 NIST 표준인 SHA-3, AES 등을 사용하고 키 관리(KMS)에만 집중하도록 의사결정한다.
