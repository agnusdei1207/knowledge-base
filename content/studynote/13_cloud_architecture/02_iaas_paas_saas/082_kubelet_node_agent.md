...
# Kubelet (큐블렛) - [쿠버네티스](/knowledge-base/studynote/06_ict_convergence/03_cloud_infrastructure/196_kubernetes_k8s_container_orchestration/) 워커 노드의 마스터 에이전트

## 핵심 인사이트 (3줄 요약)
> 1. **본질**: [쿠버네티스](/knowledge-base/studynote/06_ict_convergence/03_cloud_infrastructure/196_kubernetes_k8s_container_orchestration/) 노드 생태계에서 Kubelet(큐블렛)은 중앙 통제 센터(Master Control Plane)의 지시를 수신관측하여 각 워커 노드 리눅스 [운영체제](/knowledge-base/studynote/02_operating_system/01_overview_architecture/001_operating_system_purpose/) 단에 수명을 부여/조립해 [파드](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/085_pod_kubernetes_container_unit/)([Pod](/knowledge-base/studynote/06_ict_convergence/03_cloud_infrastructure/198_pod_kubernetes_minimum_deployment_unit/)) 단위로 실제 생산 라인을 돌리는 최전선 현장 관리자(Node Agent) 백그라운드 데몬이다.
> 2. **가치**: [마스터 노드](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/075_kubernetes_k8s_cluster_architecture/)가 고차원 [전략](/knowledge-base/studynote/04_software_engineering/04_testing_quality/268_strategy_pattern/)(`원하는 상태, Desired State`)만 YAML 문서로 툭 던지면, 큐블렛이 더러운 하위의 하드웨어 엔진 영역([Container Runtime](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/084_container_runtime_containerd_runc_cri/) 조작, 볼륨/네트워크 드라이버 [마운트](/knowledge-base/studynote/02_operating_system/09_file_system/516_mount_mechanism/))을 스스로 커넥션하며 조립하는 "선언적 [추상화](/knowledge-base/studynote/04_software_engineering/04_testing_quality/198_abstraction_control_data_process/)([Declarative](/knowledge-base/studynote/15_devops_sre/05_devsecops/219_declarative_yaml/) [Abstraction](/knowledge-base/studynote/04_software_engineering/04_testing_quality/198_abstraction_control_data_process/))"의 기술적 교두보가 되어 거대 [오케스트레이션](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/073_container_orchestration_tools/)의 [신뢰성](/knowledge-base/studynote/04_software_engineering/10_trends_pm_quality/642_reliability_mtbf_mttr_mttf_availability/) 기반을 완성한다.
> 3. **융합**: [컨테이너 런타임](/knowledge-base/studynote/02_operating_system/10_security/628_container_runtime_oci/) 인터페이스(CRI) 및 노드 자원 정보기(cAdvisor)를 흡수 융합하여 [도커](/knowledge-base/studynote/02_operating_system/01_overview_architecture/063_docker_architecture/), Containerd 상관없이 인터페이스 통신으로 생명줄을 이어주고, 마스터에게 5초마다 노드 메모리 생존 일기(Heartbeat Ping)를 쏴 클러스터 전체 리소스 밸런싱의 원천 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 수집 모세혈관망이 통합된다.

---

## Ⅰ. 개요 및 필요성 ([Context](/knowledge-base/studynote/02_operating_system/01_overview_architecture/033_context/) & Necessity)

- **개념**: Kubelet은 [오케스트레이션](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/073_container_orchestration_tools/) 아키텍처의 척수 시스템이다. 각각의 모든 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 워커 노드([VM](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/598_vm_migration_nic/) 등) 내부 시스템 백그라운드 프로세스(Systemd)로 1개씩 단독 실행되며, 자기 노드 안에 [컨테이너](/knowledge-base/studynote/04_software_engineering/09_cloud_native_ai_architecture/561_container_based_deployment/)가 죽었는지 사양서대로 잘 뜨고 있는지에 대한 1차원적 책임([Pod](/knowledge-base/studynote/06_ict_convergence/03_cloud_infrastructure/198_pod_kubernetes_minimum_deployment_unit/) Lifecycle Controller)을 전부 독점한다. 
- **필요성**: 중앙 서버(마스터 [API](/knowledge-base/studynote/02_operating_system/01_overview_architecture/014_api_posix/) Server) 혼자 1만 대 서버의 [컨테이너](/knowledge-base/studynote/04_software_engineering/09_cloud_native_ai_architecture/561_container_based_deployment/)를 직접 띄우고 지우려면 CPU 과부하 통신 트래픽 병목이 재앙을 맞는다. [분산](/knowledge-base/studynote/08_algorithm_stats/08_stats/136_variance/)된 자율 노드 하나하나마다 독립적으로 "마스터의 현재 최신 상태 문서(Manifest)와 내 로컬 상태를 1초마다 비교 [동기화](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/212_synchronization_mechanisms/)(Sync)"해 줄 [분산](/knowledge-base/studynote/08_algorithm_stats/08_stats/136_variance/)형 현장 지휘관이 필수다. 마스터 입장에서는 모든 워커 노드의 존재를 투명하게 [추상화](/knowledge-base/studynote/04_software_engineering/04_testing_quality/198_abstraction_control_data_process/)한 이 Kubelet과 통신 단일 창구를 확립함으로써 클라우드 인프라의 복잡성을 지웠다.
- **💡 비유**: Kubelet은 거대한 체인점(K8s)의 **"개별 직영 매장의 중간 책임 점장"** 과 같다. 서울 본사([마스터 노드](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/075_kubernetes_k8s_cluster_architecture/))에서 "오늘 점심에는 메뉴 A([파드](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/085_pod_kubernetes_container_unit/)) 3개를 팔아라"라고 팩스([API](/knowledge-base/studynote/02_operating_system/01_overview_architecture/014_api_posix/) 명령)만 보내면, 점장(Kubelet)이 알아서 주방 요리사([컨테이너 런타임](/knowledge-base/studynote/02_operating_system/10_security/628_container_runtime_oci/))에게 지시하고 부족한 그릇(볼륨 리소스)을 세팅한 뒤, 5분마다 본사에 "우리 매장 아주 잘 돌아갑니다~(Health Check)"라고 보고 전화(Ping)를 건다.
- **등장 배경**: 과거 서버 관리자나 [도커](/knowledge-base/studynote/02_operating_system/01_overview_architecture/063_docker_architecture/) 스웜 구조는 사람이 SSH를 뚫고 각 서버에 들어가 상태 데몬을 켜고 확인하는 무식한 단계를 거쳤다. K8s의 탄생과 함께 노드 자체를 자가 치유(Self-Healing) 생태계로 격상시키기 위해, [컨테이너](/knowledge-base/studynote/04_software_engineering/09_cloud_native_ai_architecture/561_container_based_deployment/) 조립/추적 감시/보고라는 더러운 책임을 똘똘 뭉쳐 담은 리눅스 Go언어 바이너리의 걸작 파이프라인으로 개발 등장하였다.
...
### Kubelet의 [Pod](/knowledge-base/studynote/06_ict_convergence/03_cloud_infrastructure/198_pod_kubernetes_minimum_deployment_unit/) [동기화](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/212_synchronization_mechanisms/) 루프(Sync Loop) 통신도
...
## Ⅲ. [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) 융합 트레이드오프 및 관리 모델

### cAdvisor([Container](/knowledge-base/studynote/06_ict_convergence/03_cloud_infrastructure/194_container_virtualization_docker_namespace/) Advisor) 리소스 착취 방어의 일등공신 결합
[초기](/knowledge-base/studynote/03_network/08_transport_layer/459_quic_fec_forward_error_correction/) [도커](/knowledge-base/studynote/02_operating_system/01_overview_architecture/063_docker_architecture/) 시대에는 "어떤 [파드](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/085_pod_kubernetes_container_unit/)가 이 컴퓨터 메모리를 다 갉아먹어서 죽는가?"를 증명하지 못했다. 이를 해결하고자 구글은 [오픈소스](/knowledge-base/studynote/12_it_management/05_security_compliance/191_oss_license_compliance/) **cAdvisor** [라이브러리](/knowledge-base/studynote/04_software_engineering/06_software_architecture/336_library_vs_framework/)를 Kubelet 바이너리에 융합해 내장시켰다.
- **시너지 동작**: Kubelet은 이 cAdvisor 눈과 귀를 통해 1초 단위로 노드의 Cgroup([커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/) 할당 한계) 메모리와 CPU 스케일 소모량을 측정한다. [파드](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/085_pod_kubernetes_container_unit/)가 선언한 기둥 `Limit` 스펙을 초과해 컴퓨터를 잡아먹으려 들면 Kubelet은 망설임 없이 리눅스 [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/) [OOM](/knowledge-base/studynote/02_operating_system/02_process_thread/157_oom_killer/)([Out of Memory](/knowledge-base/studynote/02_operating_system/02_process_thread/157_oom_killer/)) 스위치를 발동시켜 해당 [컨테이너](/knowledge-base/studynote/04_software_engineering/09_cloud_native_ai_architecture/561_container_based_deployment/)의 목을 치는(Kill) 절대 권력의 무혈입성 파수꾼 방어망을 세워 노드 붕괴를 보호한다. 

---

## Ⅳ. 실무 설계 및 운영 장애 거버넌스 (Governance)

### 실무 [안티패턴](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/128_water_scrum_fall_anti_pattern/) 시나리오 (노드 NotReady 좀비 사태)
...
## 👶 어린이를 위한 3줄 비유 설명
1. [쿠버네티스](/knowledge-base/studynote/06_ict_convergence/03_cloud_infrastructure/196_kubernetes_k8s_container_orchestration/)는 아주 큰 '로봇 군대'예요. 멀리 있는 대장님은 마이크로 "저격수 1번 출동해!"라고 방송만 하지 직접 뛰지 않아요.
