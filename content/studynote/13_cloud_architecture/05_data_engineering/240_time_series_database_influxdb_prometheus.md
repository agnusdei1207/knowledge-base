...
가장 비싼 엔진(RDBMS)을 가장 단순한 작업([로그](/knowledge-base/studynote/04_software_engineering/09_cloud_native_ai_architecture/568_logs_distributed_logging_elk_fluentd/) 쌓기)에 쓰면 회사가 망한다.
...
- **📢 섹션 요약 비유**: RDBMS는 '은행가의 정밀한 금전출납부'다. 어제 적은 금액이 틀리면 지우개로 지우고 다시 적어 전체 잔액을 완벽히 맞춰야 한다. TSDB는 '지진계의 기록지'다. 바늘이 펜으로 종이에 먹물을 칠하며 1초도 쉬지 않고 파동([데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/))을 그린다. 이미 그려진 어제의 파동을 지우개로 지우는 짓은 불가능하며, 지울 필요도 없다.
...
[시계열 데이터베이스](/knowledge-base/studynote/14_data_engineering/01_infrastructure/057_tsdb_downsampling_retention_policy/)(TSDB)는 인간이 만들어내는 정적인 비즈니스 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)(회원 정보)의 시대가 저물고, 기계와 센서가 뿜어내는 동적인 기계 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)(Machine [Data](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/))의 시대가 도래했음을 알리는 스토리지 엔진의 진화다.

모든 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)에 타임스탬프가 찍히는 순간, 그 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)는 단순한 값이 아니라 '흐름(Trend)'이 된다. TSDB는 1초에 수백만 개의 점(Point)들을 흡수하여, 그것을 선(Line)으로 잇고, 우리에게 과거의 패턴을 보여주어 미래의 시스템 장애나 주가 폭락을 예견할 수 있는 통찰력을 제공한다. [트랜잭션](/knowledge-base/studynote/05_database/04_transactions_concurrency/191_transaction_concept_states/)의 족쇄를 끊어내고 오직 시간에 몸을 맡긴 이 괴물 같은 쇳덩어리는 클라우드 네이티브와 [IoT](/knowledge-base/studynote/06_ict_convergence/02_iot_mobility/101_iot_concept/) 시대의 가장 거대하고 강력한 쓰레기통이자 보물창고다.

- **📢 섹션 요약 비유**: TSDB는 '블랙박스 영상 녹화기'다. 차가 달리는 1초마다 전방 풍경([데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/))을 미친 듯이 메모리에 저장하고, 용량이 꽉 차면 며칠 전 영상부터 자동으로 덮어써서 알아서 지워버린다. 과거 영상을 포토샵으로 수정(Update)하는 기능 따윈 없지만, 사고가 났을 때 과거로 돌아가 정확히 무슨 일이 있었는지 흐름을 보여주는 가장 완벽한 목격자다.
...
1. [시계열 데이터베이스](/knowledge-base/studynote/14_data_engineering/01_infrastructure/057_tsdb_downsampling_retention_policy/)(TSDB)는 1초마다 변하는 온도계 눈금이나 자동차 속도처럼, 쉴 새 없이 쏟아지는 숫자들을 아주 빠르게 빈 공책에 차곡차곡 적어두는 '시간 기록장'이에요.
2. 예전에 적어둔 숫자가 틀렸다고 지우개로 지우거나 고치는 건 절대 안 돼요! 오직 맨 뒤에 새로운 숫자를 미친 듯이 빨리 이어 적는 것에만 집중하죠.
3. 그리고 옛날 일기장 너무 무거워지면, "작년 5월은 대체로 따뜻했음"이라고 한 줄로 줄여버려서([압축](/knowledge-base/studynote/02_operating_system/06_memory_management/347_compaction/)) 책가방(컴퓨터 용량)이 터지지 않게 관리해 주는 똑똑한 친구랍니다!
...
← **이전**: [239. 그래프 데이터베이스 (Neo4j, AWS Neptune)](/knowledge-base/studynote/13_cloud_architecture/05_data_engineering/239_graph_database_neo4j_neptune/)
**다음**: [241. CAP 정리 (CAP Theorem in Distributed Systems)](/knowledge-base/studynote/13_cloud_architecture/05_data_engineering/241_cap_theorem_distributed_systems/) →
