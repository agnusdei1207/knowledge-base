...
> 2. 오버 [[528_provisioning|프로비저닝]]은 '필요한 것보다 많은 리소스(CPU·메모리·스토리지)를 할당하는 것'으로, 피크 트래픽을 대비해 항상 최대 사양을 유지하는 [[061_on_premise_legacy_infrastructure|온프레미스]] 사고방식이 클라우드로 이전된 결과다.
> 3. 올바른 크기 조정(Right-sizing) + 예약 인스턴스(Reserved Instance) + 오토스케일링의 3종 조합으로 오버 [[528_provisioning|프로비저닝]]을 체계적으로 제거해 클라우드 비용 40-60% 절감이 가능하다.

---

## I. 오버 [[528_provisioning|프로비저닝]]의 원인과 비용
...
| 낭비 유형           | 원인                     | 규모 (업계 평균) |
|-------------------|--------------------------|----------------|
| 오버 [[528_provisioning|프로비저닝]]     | 과도한 [[459_quic_fec_forward_error_correction|초기]] 사양 [[009_config|설정]]      | ~35%           |
| 유휴 리소스        | 사용 안 하는 인스턴스      | ~28%           |
| 스토리지 낭비       | 삭제 안 된 [[022_snapshot_backup_architecture|스냅샷]]/볼륨    | ~15%           |
...
| [[209_spot_instance_cloud_cost_optimization|스팟 인스턴스]]   | 60-90%     | 어려움      |
| 예약 인스턴스   | 30-60%     | 쉬움        |
...
## [[288_version_ihl_tos_total_length|IV]]. 오토스케일링으로 오버 [[528_provisioning|프로비저닝]] 근본 해결
...
| 단계 | 활동                                     | 기대 절감 |
|------|------------------------------------------|---------|
| 1    | 태깅 [[164_policy|정책]] 수립 (팀별, 환경별)             | -       |
| 2    | AWS Compute Optimizer로 Right-sizing 분석 | 20-30%  |
| 3    | Reserved Instance (1년) 구매              | 30-40%  |
| 4    | 개발/스테이징 환경 [[208_schedule_history_transaction_execution_order|스케줄]] 종료 (야간)      | 10-15%  |
| 5    | 미사용 [[022_snapshot_backup_architecture|스냅샷]]/EIP 정리                    | 5-10%   |
| 6    | 오토스케일링 적용                          | 15-25%  |
| 합계 |                                          | ~50-60% |
...
← **이전**: [[034_ondemand_reserved_spot_instances|온디맨드·예약·스팟 인스턴스 (On-Demand · Reserved · Spot Instances)]]
**다음**: [[036_migration_6r|036. 클라우드 마이그레이션 6R 전략]] →
