...
## 핵심 인사이트 (3줄 요약)
> 1. **본질**: Fallback은 **원격 [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/) 호출 실패 시 미리 정의된 대체 응답을 반환**하는 복원력 패턴이며, [Circuit Breaker](/knowledge-base/studynote/12_it_management/05_security_compliance/304_circuit_breaker/)·Retry와 함께 사용되어 사용자에게 **부분적이라도 [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/)를 유지**한다.
> 2. **가치**: 추천 [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/) 장애 시 전체 페이지가 에러가 되는 대신, Fallback으로 **인기 상품 목록(캐시)**을 보여주면 사용자 경험 저하를 최소화한다.
...
- **📢 섹션 요약 비유**: Fallback은 **비상식량**이다. 정상 배달([서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/))이 안 오면 비상식량(캐시·기본값)으로 버틴다.
...
| **기본값** | 구현 간단 | 개인화 없음 |
| **대체 [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/)** | 기능 유지 | 의존성 증가 |
...
|:---|:---|:---|
| **장애 시** | 500 에러 | **부분 [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/) 유지** |
...
|:---|:---|
| **Fallback** | 장애 시 대체 응답 |
| **[Circuit Breaker](/knowledge-base/studynote/12_it_management/05_security_compliance/304_circuit_breaker/)** | Fallback [트리거](/knowledge-base/studynote/05_database/04_transactions_concurrency/507_acid_properties/) |
...
| **[Bulkhead](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/308_bulkhead_pattern/)** | 격벽 (다른 [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/) [보호](/knowledge-base/studynote/02_operating_system/10_security/571_protection_vs_security/)) |
...
2. 추천 [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/)가 고장나도 **인기 상품 목록(캐시)**을 보여줘서 쇼핑은 계속돼요.
