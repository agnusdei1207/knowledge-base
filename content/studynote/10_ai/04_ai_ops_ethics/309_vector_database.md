...
> 2. **가치**: "애플 주가 전망"을 검색할 때 "AAPL 시세 예측"처럼 완전히 다른 키워드도 의미가 같으면 찾아주는 **의미 검색(Semantic Search)**을 가능하게 하여, 전통적 키워드 검색의 한계를 초월하고 [RAG](/knowledge-base/studynote/06_ict_convergence/04_ai_llm/276_fine_tuning/)·[추천 시스템](/knowledge-base/studynote/10_ai/03_llm_nlp/211_recommendation_system/)·유사 문서 탐색의 핵심 인프라가 된다.
...
전통적 RDBMS와 검색 엔진은 키워드(텍스트) 일치를 기반으로 동작한다. "사과" 검색 시 "apple", "과일", "맛있는 빨간 과일"은 찾지 못한다. 이 한계를 극복하려면 **의미(Semantics)**를 수학적으로 표현하는 [임베딩](/knowledge-base/studynote/06_ict_convergence/04_ai_llm/278_instruction_tuning/)([Embedding](/knowledge-base/studynote/06_ict_convergence/04_ai_llm/278_instruction_tuning/))과 그것을 효율적으로 저장·검색하는 벡터 DB가 필요하다.
...
**벡터 DB 설계 고려사항**:
1. **[임베딩](/knowledge-base/studynote/06_ict_convergence/04_ai_llm/278_instruction_tuning/) 모델 선택**: 한국어 지원 여부, [도메인](/knowledge-base/studynote/05_database/02_modeling_normalization/064_relation_domain/) 특화도, 벡터 차원 수 (작을수록 저장·검색 빠름)
2. **청크 [전략](/knowledge-base/studynote/04_software_engineering/04_testing_quality/268_strategy_pattern/)**: 512~1024 토큰, 128 토큰 중첩(Overlap)으로 문맥 단절 방지
3. **[인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/) 파라미터**: HNSW의 `ef_construction`, `M` 값 조정으로 정확도/속도 트레이드오프 설계
4. **[메타데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/012_metadata/) 필터링**: 날짜·문서 유형·부서별 접근 제어를 위한 [메타데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/012_metadata/) 태깅
...
1. **[벡터 데이터베이스](/knowledge-base/studynote/12_it_management/05_security_compliance/223_vector_database_embedding/)**는 모든 정보를 **숫자 주소(벡터)**로 바꿔서 저장하는 특별한 창고예요 — "사과"와 "애플"처럼 뜻이 같으면 **비슷한 숫자 주소**를 가져요!
2. 무언가를 찾을 때 글자가 아닌 **"비슷한 뜻의 주소"**를 찾아서, "빨간 과일"을 검색해도 "사과", "딸기" 같은 비슷한 것들을 찾을 수 있어요.
3. 이 기술이 없으면 **ChatGPT에게 회사 내부 문서를 알려주는 [RAG](/knowledge-base/studynote/06_ict_convergence/04_ai_llm/276_fine_tuning/)**를 만들 수 없어요!
