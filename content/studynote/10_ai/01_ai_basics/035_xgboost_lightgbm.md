...
> 2. XGBoost는 레벨 단위(Level-wise) 트리 성장으로 안정적이고, LightGBM은 리프 단위(Leaf-wise) 트리 성장으로 더 빠르고 낮은 메모리로 동일 [[282_performance_tactics|성능]]을 달성한다.
> 3. Kaggle·산업 [[001_dikw_pyramid|데이터]] 대회에서 XGBoost/LightGBM은 딥러닝보다 [[002_structured_data|정형 데이터]]에서 우위를 점하며, CatBoost와 함께 "테이블 [[001_dikw_pyramid|데이터]] 3대 부스터"를 형성한다.
...
| 학습 속도     | 빠름             | 매우 빠름        |
| 메모리 사용   | 높음             | 낮음             |
| 과적합 위험   | 낮음             | 높음 (소규모 [[001_dikw_pyramid|데이터]])|
| 범주형 처리   | 직접 안됨        | 내장 지원        |
| 적합한 [[001_dikw_pyramid|데이터]] | 중간 크기        | 대용량 (100만+) |

> 📢 **섹션 요약 비유**: XGBoost는 꼼꼼하게 층층이 쌓는 벽돌공, LightGBM은 가장 약한 곳을 집중 공략하는 수리공 — 같은 결과, 다른 [[268_strategy_pattern|전략]].
...
| learning_rate| [[080_gradient_descent_learning_rate|학습률]] (eta)            | 0.3    |
| subsample    | 행 샘플링 비율          | 1.0    |
| colsample    | 열(특성) 샘플링 비율    | 1.0    |
| [[216_lambda_kappa_architecture_batch_realtime|lambda]]       | L2 [[093_normalization|정규화]]              | 1.0    |
...
| EFB  | 상호 배타적인 [[247_feature_label_variables|피처]]를 묶어 [[247_feature_label_variables|피처]] 수 감소     |
...
## [[288_version_ihl_tos_total_length|IV]]. 실용 코드 예시
...
## V. 실무 시나리오 — Kaggle [[002_structured_data|정형 데이터]] 경진대회

| 단계          | [[268_strategy_pattern|전략]]                                        |
|-------------|---------------------------------------------|
| [[247_feature_label_variables|피처]] 엔지니어링 | 결측값 처리, 인코딩, 교차 특성 [[087_process_state_transition|생성]]         |
| 모델 선택     | LightGBM (대용량), XGBoost (소-중 규모)     |
| 하이퍼파라미터 | Optuna / Bayesian 최적화                    |
| [[257_ensemble_learning|앙상블]]        | XGBoost + LightGBM + CatBoost 스태킹        |
| [[395_verification_process_review|검증]]         | K-Fold [[156_cv_cost_variance|CV]], OOF (Out-of-Fold) 예측           |
