...
> 3. XGBoost·LightGBM·CatBoost가 [정형 데이터](/studynote/14_data_engineering/01_infrastructure/002_structured_data/) 경진 대회의 절대 강자인 이유가 바로 [부스팅](/studynote/14_data_engineering/03_ml_dl_llm/127_boosting/)의 편향-[분산](/studynote/08_algorithm_stats/08_stats/136_variance/) 최적화 — 딥러닝이 이미지·자연어를 석권한 이후에도 표 형식 [데이터](/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)(Tabular [Data](/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/))에서 [그래디언트 부스팅](/studynote/10_ai/01_ai_basics/034_gradient_boosting/) 기반 모델이 가장 높은 [성능](/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/)을 보인다.

---

## Ⅰ. [앙상블](/studynote/10_ai/03_llm_nlp/257_ensemble_learning/) 개요
...
> 📢 **섹션 요약 비유**: [앙상블](/studynote/10_ai/03_llm_nlp/257_ensemble_learning/) = 의사 그룹 진료 — 여러 의사(모델)가 각자 진단. 다수결([Voting](/studynote/10_ai/03_llm_nlp/258_voting_ensemble/))로 최종 결정. 한 의사의 실수를 다른 의사가 보완. 집단 지성!

---

## Ⅱ. [배깅](/studynote/10_ai/03_llm_nlp/259_bagging_random_forest/) ([Bagging](/studynote/10_ai/03_llm_nlp/259_bagging_random_forest/))
...
> 📢 **섹션 요약 비유**: [배깅](/studynote/10_ai/03_llm_nlp/259_bagging_random_forest/) = 여론 조사 집계 — 각 조사(부트스트랩 샘플)에서 독립적 결과. 100개 조사 평균 내면 단일보다 정확. Random Forest는 조사마다 다른 질문 섞어서 중복 의견 제거!

---

## Ⅲ. [부스팅](/studynote/14_data_engineering/03_ml_dl_llm/127_boosting/) ([Boosting](/studynote/14_data_engineering/03_ml_dl_llm/127_boosting/))
...
> 📢 **섹션 요약 비유**: [부스팅](/studynote/14_data_engineering/03_ml_dl_llm/127_boosting/) = 약점 집중 훈련 — 시험에서 틀린 문제에 집중. 다음 모델이 이전 모델의 오류(잔차)를 공략. 100번 반복 -> 점점 정확해짐!
...
> 📢 **섹션 요약 비유**: Kaggle [앙상블](/studynote/10_ai/03_llm_nlp/257_ensemble_learning/) = 의사결정 위원회 — LightGBM(내과 전문의), XGBoost(외과), CatBoost(영상의학과). 각자 다른 오류 -> 스태킹으로 AUC 0.791->0.818. 다양성이 핵심!
...
1. [앙상블](/studynote/10_ai/03_llm_nlp/257_ensemble_learning/) = 의사 그룹 진료 — 여러 의사가 각자 진단 후 다수결. 한 의사 실수를 다른 의사가 보완. 혼자보다 정확!
2. [배깅](/studynote/10_ai/03_llm_nlp/259_bagging_random_forest/)([Random Forest](/studynote/06_ict_convergence/05_data_science/353_random_forest/)) = 여론조사 집계 — 100개 독립 조사 평균. 과적합 방지, [분산](/studynote/08_algorithm_stats/08_stats/136_variance/) 감소. 모두 같은 답 = 무의미하니 무작위 특성 추가!
3. [부스팅](/studynote/14_data_engineering/03_ml_dl_llm/127_boosting/)(XGBoost) = 약점 집중 훈련 — 틀린 문제에 집중. 이전 오류(잔차)를 다음 모델이 공략. 100회 반복 -> 최강 모델!
...
<- **이전**: [048. 이상 탐지 — Anomaly Detection](/studynote/10_ai/01_ai_basics/048_anomaly_detection/)
**다음**: [보팅 앙상블 — 하드/소프트 보팅 (Hard vs Soft Voting Ensemble)](/studynote/10_ai/01_ai_basics/050_voting_hard_soft_ensemble/) ->
