...
실무에서는 미니배치 사이즈 (Mini-batch Size) / 에폭 (Epoch) / 이터레이션 (Iteration)을(를) 단독 기술이 아니라 데이터 품질, 시스템 제약, 규제 요구와 함께 판단해야 한다. 언제 채택하고 언제 회피할지, 어떤 지표로 운영 상태를 볼지까지 적어야 기술사 답안으로 완성된다.
