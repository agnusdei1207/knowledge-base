...
- **📢 섹션 요약 비유**: 시뮬레이션에서 자전거 타기를 배운 아이를 현실로 데려오는 건, 얼음판에서 스케이트를 배운 애한테 갑자기 아스팔트에서 롤러스케이트를 타라는 것(Sim-to-Real Gap)과 같다. 그래서 [도메인](/knowledge-base/studynote/05_database/02_modeling_normalization/064_relation_domain/) 무작위화 훈련을 쓴다. 시뮬레이션 안에서 바닥에 기름도 부어보고, 모래도 뿌려보고, 뒤에서 발로 차보면서(악조건 섞기) 온갖 더러운 환경을 다 겪게 하면, 아이는 결국 어떤 환경이든 넘어지지 않는 궁극의 신체 밸런스(강건성)를 득도하게 된다.

---

## Ⅲ. 비교 및 연결

로보틱스를 지배하려는 [AI](/knowledge-base/studynote/04_software_engineering/03_design_architecture/190_ai_llm_requirements_specification/) 기술은 로봇에게 어떻게 '경험'을 먹이느냐에 따라 [강화 학습](/knowledge-base/studynote/14_data_engineering/05_exam_keywords/253_reinforcement_learning_mdp_policy_value_q_learning_dqn/)(RL)과 모방 학습([Imitation Learning](/knowledge-base/studynote/14_data_engineering/04_mlops/200_autonomous_driving_imitation_learning_digital_twin/)) 양대 진영으로 갈린다.
...
- **📢 섹션 요약 비유**: 로봇 등껍질에 달린 뇌(온디바이스 [NPU](/knowledge-base/studynote/01_computer_architecture/12_accelerators_ai_hardware/424_npu/))는 우리의 '척수 반사 신경'이다. 뜨거운 주전자에 손을 댔을 때, 뇌(클라우드)까지 [신호](/knowledge-base/studynote/02_operating_system/02_process_thread/130_signal/)가 가서 "앗 뜨겁네 손 떼라"라고 판단하면 이미 손이 다 타버린다. 척수(로컬 칩셋)에서 0.01초 만에 본능적으로 손을 확 빼도록 딥러닝 모델이 로봇의 가장 가까운 하드웨어 뼈대에 밀착 렌더링되어 있어야만 로봇이 현실의 중력 속에서 살아남을 수 있다.
...
미래의 범용 로보틱스는 더 이상 각각의 회사 창고에서 훈련되지 않을 것이다. 수백만 대의 테슬라 옵티머스 휴머노이드 로봇들이 전 세계의 거실과 주방에서 커피를 타고 설거지를 하면서 부딪힌 시행착오(경험치)들을 밤마다 중앙 서버로 쏴 올리고([연합 학습](/knowledge-base/studynote/14_data_engineering/05_exam_keywords/256_federated_learning_privacy_model_security/)), 클라우드의 거대한 [PPO](/knowledge-base/studynote/10_ai/05_data_science_ml/395_ppo_clipping/) [알고리즘](/knowledge-base/studynote/08_algorithm_stats/01_basics/001_algorithm_definition/)이 이를 삼켜 하나의 전지전능한 '지구 통합 모션 [정책](/knowledge-base/studynote/10_ai/02_dl_architecture_new/164_policy/)(Universal Foundation [Policy](/knowledge-base/studynote/10_ai/02_dl_architecture_new/164_policy/))'을 구워내 다음 날 모든 로봇의 뇌를 무선으로 [동기화](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/212_synchronization_mechanisms/)하는, 인류를 대체할 신인류 노동력 생태계의 대통합이 지금 우리 눈앞에서 렌더링되고 있다.

- **📢 섹션 요약 비유**: 이 혁명은 철저하게 계산된 '악보(수학 공식)'만 보고 피아노를 치던 로봇이, 어느 날 악보를 다 찢어버리고 음악을 귀로 들으며(센서) 기분과 느낌([강화 학습](/knowledge-base/studynote/14_data_engineering/05_exam_keywords/253_reinforcement_learning_mdp_policy_value_q_learning_dqn/) 딥러닝 뇌)만으로 재즈 즉흥 연주(모션 [정책](/knowledge-base/studynote/10_ai/02_dl_architecture_new/164_policy/))를 미친 듯이 완벽하게 쳐내는 소름 돋는 각성의 순간이다. 로봇에게 영혼과 야생의 직관이 깃든 것이다.
...
<- **이전**: [199. 공간 컴퓨팅 (Spatial Computing) 결합 AI](/knowledge-base/studynote/10_ai/02_dl_architecture_new/199_spatial_computing/)
**다음**: [201. 뉴로모픽 컴퓨팅 (Neuromorphic Computing)](/knowledge-base/studynote/10_ai/03_llm_nlp/201_neuromorphic_computing/) ->
