...
## 핵심 인사이트 (3줄 요약)
> 1. **본질**: [ResNet](/knowledge-base/studynote/10_ai/04_ai_ops_ethics/287_resnet_skip_connection/) ([Residual Network](/knowledge-base/studynote/10_ai/04_ai_ops_ethics/287_resnet_skip_connection/))은 입력 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 변형하지 않고 다음 층으로 직접 전달하는 우회로(Skip Connection)를 도입해 잔차(Residual)만 학습하게 만든 신경망 구조다.
> 2. **가치**: 망이 깊어질수록 오차 [신호](/knowledge-base/studynote/02_operating_system/02_process_thread/130_signal/)가 사라지는 [기울기 소실](/knowledge-base/studynote/10_ai/01_ai_basics/088_vanishing_gradient_relu_skip_connection/)([Vanishing Gradient](/knowledge-base/studynote/14_data_engineering/05_exam_keywords/240_relu_vanishing_gradient_softmax_backprop_chain/)) 문제를 해결하여, 딥러닝 모델의 층수를 152층 이상으로 획기적으로 깊게 쌓을 수 있게 만들었다.
> 3. **판단 포인트**: 모델 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/)을 높이기 위해 단순히 층을 쌓는 것은 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) 저하(Degradation)를 부르므로, 파라미터 수를 줄이는 보틀넥([Bottleneck](/knowledge-base/studynote/02_operating_system/10_security/617_io_bottleneck/)) 구조와 잔차 연결을 결합해야 실질적인 고성능 초심층 모델을 구현할 수 있다.

---

## Ⅰ. 개요 및 필요성

[ResNet](/knowledge-base/studynote/10_ai/04_ai_ops_ethics/287_resnet_skip_connection/) ([Residual Network](/knowledge-base/studynote/10_ai/04_ai_ops_ethics/287_resnet_skip_connection/))은 2015년 마이크로소프트 연구진이 제안한 [합성곱 신경망](/knowledge-base/studynote/12_it_management/02_itsm_itil/089_CNN_Convolutional/) ([CNN](/knowledge-base/studynote/14_data_engineering/05_exam_keywords/243_cnn_stride_pooling_resnet_residual_yolo_object_detection/), [Convolutional Neural Network](/knowledge-base/studynote/12_it_management/02_itsm_itil/089_CNN_Convolutional/)) 아키텍처다. 딥러닝에서 모델의 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/)을 높이려면 신경망을 깊게 쌓아야 하지만, 기존 방식은 한계가 명확했다. 입력 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)가 여러 층을 거치면서 정보가 변질되고, [역전파](/knowledge-base/studynote/10_ai/03_llm_nlp/272_backpropagation/) ([Backpropagation](/knowledge-base/studynote/10_ai/03_llm_nlp/272_backpropagation/)) 시 오차 [신호](/knowledge-base/studynote/02_operating_system/02_process_thread/130_signal/)가 점차 0에 수렴하는 [기울기 소실](/knowledge-base/studynote/10_ai/01_ai_basics/088_vanishing_gradient_relu_skip_connection/) ([Vanishing Gradient](/knowledge-base/studynote/14_data_engineering/05_exam_keywords/240_relu_vanishing_gradient_softmax_backprop_chain/)) 문제가 발생했기 때문이다.

이로 인해 층을 20개 쌓은 모델보다 56개 쌓은 모델의 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/)이 오히려 떨어지는 망각 및 저하 (Degradation) 현상이 나타났다. 모델이 깊어질수록 학습이 어려워지는 이 모순을 극복하기 위해, 정보가 손실 없이 통과할 수 있는 직통 고속도로인 잔차 연결 (Skip Connection)이 등장하게 되었다.

- **📢 섹션 요약 비유**: 깊은 신경망은 수십 명의 사람이 일렬로 서서 귓속말을 전달하는 게임과 같습니다. 중간에 한 명만 말을 잘못 옮겨도 마지막 사람은 전혀 다른 말을 듣게 되는데, 잔차 연결은 처음 사람이 마지막 사람에게 직접 쪽지를 날려 원본을 [확인](/knowledge-base/studynote/04_software_engineering/12_testing_maintenance/396_validation/)시켜 주는 셈입니다.
...
ResNet은 이전 세대의 대표적인 [심층 신경망](/knowledge-base/studynote/10_ai/01_ai_basics/065_dnn_deep_neural_network/)인 VGGNet 등과 비교할 때 설계 철학의 근본적인 차이를 보인다. 기존 모델들이 단순히 계층을 [직렬](/knowledge-base/studynote/03_network/03_physical_layer_media/149_serial_communication_rs232_rs485/)로 연장하는 데 집중했다면, ResNet은 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 경로의 우회를 통해 깊이의 한계를 구조적으로 극복했다.

| 항목 | 일반 [심층 신경망](/knowledge-base/studynote/10_ai/01_ai_basics/065_dnn_deep_neural_network/) (예: VGGNet) | [ResNet](/knowledge-base/studynote/10_ai/04_ai_ops_ethics/287_resnet_skip_connection/) ([Residual Network](/knowledge-base/studynote/10_ai/04_ai_ops_ethics/287_resnet_skip_connection/)) |
| :--- | :--- | :--- |
...
실무에서 이미지 [분류](/knowledge-base/studynote/16_bigdata/05_analysis/104_classification_analysis/), [객체 탐지](/knowledge-base/studynote/10_ai/04_ai_ops_ethics/288_object_detection_yolo_rcnn/) 등 시각 지능 모델을 설계할 때 [ResNet](/knowledge-base/studynote/10_ai/04_ai_ops_ethics/287_resnet_skip_connection/) 계열([ResNet](/knowledge-base/studynote/10_ai/04_ai_ops_ethics/287_resnet_skip_connection/)-50, [ResNet](/knowledge-base/studynote/10_ai/04_ai_ops_ethics/287_resnet_skip_connection/)-101 등)은 백본 (Backbone) 네트워크의 사실상 표준(De facto standard)으로 사용된다. 깊이와 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/)의 트레이드오프를 고려하여 모델을 선택해야 한다.
...
ResNet의 도입으로 딥러닝 모델은 수백 층 이상으로 깊어질 수 있었고, 이는 이미지 인식 대회(ILSVRC)에서 [인공지능](/knowledge-base/studynote/10_ai/03_llm_nlp/231_ai_turing_test/)이 인간의 시각 인지 오류율을 최초로 뛰어넘는 결정적 계기가 되었다. 표현력의 한계와 최적화의 어려움을 동시에 해결한 것이다.

그러나 층이 깊어짐에 따라 여전히 절대적인 메모리 사용량과 추론 [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/)시간 ([Latency](/knowledge-base/studynote/01_computer_architecture/03_architecture_basics_performance/141_latency/))은 증가하므로, 초경량화가 필요한 에지 (Edge) 환경에서는 MobileNet이나 모델 [압축](/knowledge-base/studynote/02_operating_system/06_memory_management/347_compaction/) 기술이 추가로 요구된다. 결론적으로 ResNet은 딥러닝에서 '깊이'라는 무기를 온전히 사용할 수 있게 해 준 가장 위대한 건축적 혁신으로 기억되어야 한다.
