...
R-[CNN](/knowledge-base/studynote/14_data_engineering/05_exam_keywords/243_cnn_stride_pooling_resnet_residual_yolo_object_detection/) (Regions with [Convolutional Neural Network](/knowledge-base/studynote/12_it_management/02_itsm_itil/089_CNN_Convolutional/) features) 계열 모델은 이미지 내에서 물체가 있을 만한 위치를 먼저 찾고, 그 위치에 있는 물체가 무엇인지 [분류](/knowledge-base/studynote/16_bigdata/05_analysis/104_classification_analysis/)([Classification](/knowledge-base/studynote/12_it_management/03_ea_isp/107_classification/))하는 2-Stage [객체 탐지](/knowledge-base/studynote/10_ai/04_ai_ops_ethics/288_object_detection_yolo_rcnn/) 방식의 표준이다.
과거의 [객체 탐지](/knowledge-base/studynote/10_ai/04_ai_ops_ethics/288_object_detection_yolo_rcnn/)는 슬라이딩 윈도우 방식으로 이미지 전체를 훑으며 분석했기 때문에 컴퓨팅 자원 낭비가 극심했다. 이를 해결하기 위해 물체가 있을 법한 후보 영역만 골라내어 분석하는 방식이 도입되었으나, [초기](/knowledge-base/studynote/03_network/08_transport_layer/459_quic_fec_forward_error_correction/) R-CNN은 2,000개의 후보 영역을 각각 CNN에 통과시켜야 하므로 병목 현상이 심각했다. 이를 해결하기 위해 연산 공유와 신경망 통합이라는 두 번의 핵심적인 혁신을 거쳐 지금의 Faster R-CNN으로 진화하게 되었다.

- **📢 섹션 요약 비유**: R-[CNN](/knowledge-base/studynote/14_data_engineering/05_exam_keywords/243_cnn_stride_pooling_resnet_residual_yolo_object_detection/) 시리즈의 진화는 범인을 찾는 경찰의 수사망 [압축](/knowledge-base/studynote/02_operating_system/06_memory_management/347_compaction/) 과정과 같다. 동네 집을 다 뒤지는 대신 수상한 집 2,000곳만 뒤지는 것(R-[CNN](/knowledge-base/studynote/14_data_engineering/05_exam_keywords/243_cnn_stride_pooling_resnet_residual_yolo_object_detection/))에서 출발해, 동네 전체 사진을 한 번에 찍어 수상한 곳만 확대해 보는 방식(Fast)으로, 결국 드론을 띄워 수상한 곳 자체를 [인공지능](/knowledge-base/studynote/10_ai/03_llm_nlp/231_ai_turing_test/)이 즉시 찾아내는 방식(Faster)으로 진화한 것이다.
...
1. <strong>R-<a href="/knowledge-base/studynote/14_data_engineering/05_exam_keywords/243_cnn_stride_pooling_resnet_residual_yolo_object_detection/">CNN</a> (2014)</strong>:
   이미지에서 Selective Search [알고리즘](/knowledge-base/studynote/08_algorithm_stats/01_basics/001_algorithm_definition/)으로 2,000개의 [RoI](/knowledge-base/studynote/12_it_management/01_governance_strategy/807_roi_return_on_investment/) (Region of Interest) 후보 영역을 추출하고, 각 영역을 동일한 크기로 잘라내어 2,000번의 [CNN](/knowledge-base/studynote/14_data_engineering/05_exam_keywords/243_cnn_stride_pooling_resnet_residual_yolo_object_detection/) 연산을 수행한다. 속도가 매우 느리다.
...
- **📢 섹션 요약 비유**: R-CNN이 2,000장의 서류를 복사기에 2,000번 넣는 바보 같은 짓이라면, Fast R-CNN은 1장만 스캔한 뒤 [모니터](/knowledge-base/studynote/02_operating_system/04_synchronization/229_monitor/)에서 필요한 부분만 캡처하는 똑똑한 방식이다. 나아가 Faster R-CNN은 어떤 부분을 캡처할지조차 비서(RPN)가 알아서 찾아주는 완벽한 자동화 시스템이다.

---

## Ⅲ. 비교 및 연결

2-Stage 계열의 핵심 경쟁자는 1-Stage 탐지기인 YOLO와 [SSD](/knowledge-base/studynote/01_computer_architecture/08_io_storage_systems/327_ssd/) (Single Shot MultiBox Detector)다. 두 방식은 정확도와 속도라는 명확한 트레이드오프를 가진다.
...
Faster R-CNN의 성공 이후, 분할([Segmentation](/knowledge-base/studynote/02_operating_system/06_memory_management/364_segmentation/)) 작업을 추가한 Mask R-CNN으로 모델이 확장되며 컴퓨터 비전의 기초 뼈대로 자리 잡았다.

- **📢 섹션 요약 비유**: 1-Stage(YOLO)는 동체 시력이 뛰어나 지나가는 자동차 번호판을 대략적으로 빠르게 읽는 레이서라면, 2-Stage(Faster R-[CNN](/knowledge-base/studynote/14_data_engineering/05_exam_keywords/243_cnn_stride_pooling_resnet_residual_yolo_object_detection/))는 돋보기를 들고 천천히 하지만 오타 하나 없이 문서를 읽어내는 감정평가사다.
...
R-CNN에서 Faster R-CNN으로의 발전은 단순히 속도 향상을 넘어, [객체 탐지](/knowledge-base/studynote/10_ai/04_ai_ops_ethics/288_object_detection_yolo_rcnn/)의 모든 과정을 딥러닝 하나로 통합했다는 데에 큰 의의가 있다. 이를 통해 개발자는 복잡한 외부 [알고리즘](/knowledge-base/studynote/08_algorithm_stats/01_basics/001_algorithm_definition/) [파이프](/knowledge-base/studynote/02_operating_system/02_process_thread/123_pipe/)라인을 관리할 필요 없이 모델 튜닝에만 집중할 수 있게 되었다.

한계점으로는 구조적 복잡성으로 인해 메모리 사용량이 많고 온디바이스(On-Device) 추론에는 부담이 된다는 점이 있다. 결론적으로 Faster R-CNN은 [객체 탐지](/knowledge-base/studynote/10_ai/04_ai_ops_ethics/288_object_detection_yolo_rcnn/) 역사상 가장 우아한 아키텍처 혁신이며, 정확도를 타협할 수 없는 모든 비전 시스템의 든든한 기준으로 기억되어야 한다.

- **📢 섹션 요약 비유**: 옛날에는 요리사, 보조, 배달원이 각자 따로 놀아 식당이 느렸지만(R-[CNN](/knowledge-base/studynote/14_data_engineering/05_exam_keywords/243_cnn_stride_pooling_resnet_residual_yolo_object_detection/)), 모든 과정을 하나의 자동화 로봇(Faster R-[CNN](/knowledge-base/studynote/14_data_engineering/05_exam_keywords/243_cnn_stride_pooling_resnet_residual_yolo_object_detection/))으로 합치면서 미슐랭 3성급 맛을 유지한 채 빠른 속도까지 따라잡은 혁명이다.
...
| 개념 | 연결 포인트 |
| :--- | :--- |
| <strong><a href="/knowledge-base/studynote/12_it_management/01_governance_strategy/807_roi_return_on_investment/">RoI</a> <a href="/knowledge-base/studynote/10_ai/04_ai_ops_ethics/285_pooling_layer/">Pooling</a></strong> | 다양한 크기의 후보 영역을 동일한 크기의 벡터로 고정하는 기술 |
...
3. Faster R-CNN은 아예 [인공지능](/knowledge-base/studynote/10_ai/03_llm_nlp/231_ai_turing_test/) 비서가 "여기, 여기, 여기를 보세요!" 하고 네모칸까지 알아서 쳐주는 마법 안경이랍니다.
