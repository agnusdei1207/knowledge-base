...
> 2. **가치**: BERT의 WordPiece는 `##` 접두사로 서브워드 경계를 명시해 NLU (Natural Language Understanding) [태스크](/knowledge-base/studynote/02_operating_system/02_process_thread/150_task/)에 최적화되고, SentencePiece는 BPE/Unigram을 언어 독립적으로 구현해 다국어 모델의 표준이 됐다.
> 3. **판단 포인트**: 세 [알고리즘](/knowledge-base/studynote/08_algorithm_stats/01_basics/001_algorithm_definition/)(BPE·WordPiece·Unigram)의 병합/분할 기준 차이와 `##` vs `▁` 표기법, Sentencepiece의 [역정규화](/knowledge-base/studynote/05_database/02_modeling_normalization/111_denormalization_performance_tradeoff/) 특성을 구분해서 설명할 수 있어야 한다.

---

## Ⅰ. 개요 및 필요성

같은 서브워드 토크나이징 계열이지만, WordPiece와 SentencePiece는 [알고리즘](/knowledge-base/studynote/08_algorithm_stats/01_basics/001_algorithm_definition/)과 사용 맥락이 다르다. 실무에서 [BERT](/knowledge-base/studynote/10_ai/04_ai_ops_ethics/301_bert_mlm/) 계열 모델을 다루면 WordPiece, T5·LLaMA 계열은 SentencePiece를 만나게 된다.
...
### WordPiece ([BERT](/knowledge-base/studynote/10_ai/04_ai_ops_ethics/301_bert_mlm/))
...
| WordPiece / SentencePiece 토크나이징 비교 (Wordpiece Sentencepiece) | [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/)과 실용성의 균형 | 대표적인 실무 적용 |
...
WordPiece와 SentencePiece는 BPE의 발전 형태로, 각각 BERT와 T5/LLaMA 생태계의 표준 토크나이저가 됐다. 서브워드 [알고리즘](/knowledge-base/studynote/08_algorithm_stats/01_basics/001_algorithm_definition/)의 선택은 모델 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/), 다국어 지원, 디토크나이징 편의성에 직접 영향을 미친다. 실무에서 올바른 토크나이저 이해는 파인튜닝과 [프롬프트 엔지니어링](/knowledge-base/studynote/14_data_engineering/03_ml_dl_llm/149_prompt_engineering_cot_few_shot/)의 토대다.
...
| 개념 | 연결 포인트 |
|:---|:---|
| WordPiece | [BERT](/knowledge-base/studynote/10_ai/04_ai_ops_ethics/301_bert_mlm/), ## 표기, 우도 기반 / 영어 중심 서브워드 |
| SentencePiece | T5, LLaMA, ▁ 표기 / 언어 독립적 서브워드 |
| Unigram LM | [확률](/knowledge-base/studynote/08_algorithm_stats/08_stats/130_probability/) 기반 제거 / SentencePiece 옵션 |
| BPE | 빈도 병합 / 서브워드 기원 [알고리즘](/knowledge-base/studynote/08_algorithm_stats/01_basics/001_algorithm_definition/) |
| OOV | 미등록 단어 / 서브워드로 해결 |
