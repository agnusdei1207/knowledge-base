...
> 2. **가치**: BERT의 WordPiece는 `##` 접두사로 서브워드 경계를 명시해 NLU (Natural Language Understanding) [[150_task|태스크]]에 최적화되고, SentencePiece는 BPE/Unigram을 언어 독립적으로 구현해 다국어 모델의 표준이 됐다.
> 3. **판단 포인트**: 세 [[001_algorithm_definition|알고리즘]](BPE·WordPiece·Unigram)의 병합/분할 기준 차이와 `##` vs `▁` 표기법, Sentencepiece의 [[111_denormalization_performance_tradeoff|역정규화]] 특성을 구분해서 설명할 수 있어야 한다.

---

## Ⅰ. 개요 및 필요성

같은 서브워드 토크나이징 계열이지만, WordPiece와 SentencePiece는 [[001_algorithm_definition|알고리즘]]과 사용 맥락이 다르다. 실무에서 [[301_bert_mlm|BERT]] 계열 모델을 다루면 WordPiece, T5·LLaMA 계열은 SentencePiece를 만나게 된다.

두 방법 모두 [[001_dikw_pyramid|데이터]] 기반 서브워드 어휘 구축이라는 목표를 공유하지만, 병합 기준과 공백 처리 방식에서 차이가 있다.
...
- **📢 섹션 요약 비유**: WordPiece는 "[[130_probability|확률]]적으로 가장 유리한 조합을 고르는 영리한 합병", SentencePiece는 "언어의 공백 규칙을 무시하고 [[074_byte|바이트]] 스트림처럼 처리하는 언어 중립적 합병"이다.

---

## Ⅱ. 아키텍처 및 핵심 원리

### WordPiece ([[301_bert_mlm|BERT]])
...
| 디토크나이징 | 복잡 | `##` 제거 | ▁ → 공백 |
| 대표 모델 | [[302_gpt_autoregressive|GPT]]-2 | [[301_bert_mlm|BERT]] | T5, LLaMA |
...
| WordPiece / SentencePiece 토크나이징 비교 (Wordpiece Sentencepiece) | [[282_performance_tactics|성능]]과 실용성의 균형 | 대표적인 실무 적용 |
...
**[[301_bert_mlm|BERT]] [[304_fine_tuning|fine-tuning]] 시**: `##` 표기 토큰의 오프셋 매핑 주의 ([[117_ner|NER]] 등 span 예측 [[150_task|태스크]])
**T5/LLaMA [[304_fine_tuning|fine-tuning]]**: SentencePiece의 ▁ 처리로 공백 복원이 자동
**토크나이저 재사용**: 동일 토크나이저로 사전학습 → 파인튜닝 [[194_consistency_database_integrity|일관성]] 필수
...
WordPiece와 SentencePiece는 BPE의 발전 형태로, 각각 BERT와 T5/LLaMA 생태계의 표준 토크나이저가 됐다. 서브워드 [[001_algorithm_definition|알고리즘]]의 선택은 모델 [[282_performance_tactics|성능]], 다국어 지원, 디토크나이징 편의성에 직접 영향을 미친다. 실무에서 올바른 토크나이저 이해는 파인튜닝과 [[149_prompt_engineering_cot_few_shot|프롬프트 엔지니어링]]의 토대다.
...
| 개념 | 연결 포인트 |
|:---|:---|
| WordPiece | [[301_bert_mlm|BERT]], ## 표기, 우도 기반 / 영어 중심 서브워드 |
| SentencePiece | T5, LLaMA, ▁ 표기 / 언어 독립적 서브워드 |
| Unigram LM | [[130_probability|확률]] 기반 제거 / SentencePiece 옵션 |
| BPE | 빈도 병합 / 서브워드 기원 [[001_algorithm_definition|알고리즘]] |
| OOV | 미등록 단어 / 서브워드로 해결 |
