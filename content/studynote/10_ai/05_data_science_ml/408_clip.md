...
**필요성**:
- **[데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 효율성**: 수작업 라벨링 대신 인터넷의 풍부한 이미지-캡션 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 활용
...
| 구성 요소 | 설명 | 특징 |
|:---|:---|:---|
| **Image [Encoder](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/040_encoder/)** | 이미지를 고차원 벡터로 변환 | ViT (Vision [Transformer](/knowledge-base/studynote/14_data_engineering/05_exam_keywords/246_transformer_self_attention_parallel_positional_encoding/)) 또는 [ResNet](/knowledge-base/studynote/10_ai/04_ai_ops_ethics/287_resnet_skip_connection/) 사용 |
| **Text [Encoder](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/040_encoder/)** | 텍스트를 고차원 벡터로 변환 | [Transformer](/knowledge-base/studynote/14_data_engineering/05_exam_keywords/246_transformer_self_attention_parallel_positional_encoding/) 기반 모델 사용 |
| **Joint [Embedding](/knowledge-base/studynote/06_ict_convergence/04_ai_llm/278_instruction_tuning/)** | 이미지와 텍스트 벡터가 만나는 공간 | 의미가 같은 쌍은 가깝게, 다른 쌍은 멀게 배치 |
| **[Contrastive Loss](/knowledge-base/studynote/06_ict_convergence/05_data_science/413_clip_multimodal_contrastive_loss/)** | 실제 쌍의 유사도는 높이고 나머지는 낮춤 | 행렬 대각선 성분의 [확률](/knowledge-base/studynote/08_algorithm_stats/08_stats/130_probability/)을 최대화 |
...
**제로샷 추론([Zero](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/585_zero_skipping/)-shot Inference)**:
...
CLIP은 408번의 **대조 학습(Contrastive [Learning](/knowledge-base/studynote/03_network/05_lan_wan_l2_devices/240_switch_learning_forwarding_flooding/))** 기법을 활용하며, 320번의 **[디퓨전 모델](/knowledge-base/studynote/14_data_engineering/03_ml_dl_llm/153_diffusion_model_stable_diffusion_denoising/)(Diffusion)**에서 사용자가 입력한 텍스트에 맞는 이미지를 [생성](/knowledge-base/studynote/02_operating_system/02_process_thread/087_process_state_transition/)하도록 가이드하는 '나침반' 역할을 한다.
...
### 실무 고려 사항
1. **[Prompt Engineering](/knowledge-base/studynote/12_it_management/05_security_compliance/224_prompt_engineering_guideline/)**: "dog"라고만 하는 것보다 "A photo of a dog"라고 구체적으로 묘사하는 것이 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/)이 더 잘 나온다.
2. **[Domain](/knowledge-base/studynote/05_database/02_modeling_normalization/064_relation_domain/) Adaptation**: 일반적인 인터넷 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)로 학습되었으므로, 의료나 정밀 제조 등 특수 [도메인](/knowledge-base/studynote/05_database/02_modeling_normalization/064_relation_domain/)에서는 추가적인 [미세 조정](/knowledge-base/studynote/10_ai/02_dl_architecture_new/133_fine_tuning/)이 필요할 수 있다.
...
### 기술사 판단 포인트
- CLIP은 **'인간의 언어적 이해력이 시각적 인지력을 가이드한다'**는 철학을 증명했다. 이는 AI가 단일 감각(Unimodal)을 넘어 인간처럼 오감을 통합하는 **'일반 [인공지능](/knowledge-base/studynote/10_ai/03_llm_nlp/231_ai_turing_test/)(AGI)'**으로 진화하는 핵심 징검다리임을 강조해야 한다.
