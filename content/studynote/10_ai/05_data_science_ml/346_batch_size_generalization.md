...
> 1. **본질**: 큰 배치 사이즈 (Large Batch) 는 손실 경관 (Loss Landscape) 에서 샤프 미니마 (Sharp Minima) 로 수렴해 과적합 ([Overfitting](/knowledge-base/studynote/10_ai/03_llm_nlp/245_overfitting_variance/)) 경향이 있고, 작은 배치 (Small Batch) 는 노이즈가 많은 기울기 덕분에 플랫 미니마 (Flat Minima) 로 수렴해 일반화 (Generalization) 가 향상된다.
> 2. **가치**: 플랫 미니마는 파라미터 공간에서 손실 곡면이 완만해 배포 환경의 약간의 분포 변화에도 안정적이고, 샤프 미니마는 좁은 골짜기에 있어 작은 변화에도 손실이 급증한다.
> 3. **판단 포인트**: 선형 [스케일링](/knowledge-base/studynote/10_ai/03_llm_nlp/249_scaling_normalization_standardization/) 규칙 (Linear Scaling Rule) — 배치 사이즈를 k배 늘리면 [학습률](/knowledge-base/studynote/10_ai/01_ai_basics/080_gradient_descent_learning_rate/)을 k배 늘려야 동일한 업데이트 크기를 유지한다 — 은 대규모 [분산](/knowledge-base/studynote/08_algorithm_stats/08_stats/136_variance/) 학습의 핵심 조정 원칙이다.
...
| 업데이트 빈도 | 높음 | 낮음 |
| [GPU](/knowledge-base/studynote/01_computer_architecture/12_accelerators_ai_hardware/418_gpu/) 활용률 | 낮음 | 높음 |
| 수렴 속도 | 느림 (에폭당) | 빠름 |
| 일반화 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) | 좋음 (플랫 미니마) | 낮음 (샤프 미니마) |
| 메모리 | 적게 사용 | 많이 사용 |
...
- **📢 섹션 요약 비유**: 선형 [스케일링](/knowledge-base/studynote/10_ai/03_llm_nlp/249_scaling_normalization_standardization/) 규칙은 "8명이 함께 들어 1m 이동하는 짐을 16명이 들면 [학습률](/knowledge-base/studynote/10_ai/01_ai_basics/080_gradient_descent_learning_rate/)(발걸음 크기)를 2배 늘려야 똑같이 2m 이동한다"는 원리다. 사람이 늘어도 목표 [진행](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/216_progress_in_synchronization/) 속도를 유지하려면 보폭도 키워야 한다.
...
- **📢 섹션 요약 비유**: SAM (Sharpness-Aware Minimization) 은 "현재 위치뿐 아니라 주변 최악의 지점에서도 잘 작동하는 [가중치](/knowledge-base/studynote/10_ai/03_llm_nlp/267_weight_bias_activation/)를 찾는" 등산가의 접근이다. 험준한 봉우리 대신 넓은 고원을 목표로 한다.
...
- **📢 섹션 요약 비유**: 큰 배치로 학습한 모델은 "실험실에서만 통하는 최적화" — 훈련 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)의 좁은 골짜기에 정확히 맞지만, 실제 환경 (배포 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)) 에서의 약간의 차이에 민감하다. 플랫 미니마는 어느 환경에서나 무난히 통하는 "범용 최적화"다.
...
배치 사이즈와 일반화의 [관계](/knowledge-base/studynote/05_database/02_modeling_normalization/083_relationship_in_er_model/)는 이론(PAC-Bayes, 손실 경관)과 실무([분산](/knowledge-base/studynote/08_algorithm_stats/08_stats/136_variance/) 학습, 배치 조정) 두 측면 모두에서 중요하다. 기술사 시험에서는 Sharp vs Flat Minima 의 정의, 기울기 노이즈의 역할, Linear Scaling Rule, SAM 을 [논리](/knowledge-base/studynote/09_security/04_endpoint_security/369_logic_bomb/)적으로 서술하면 고득점 가능하다.
...
3. ⚖️ 배치를 크게 쓸 때는 [학습률](/knowledge-base/studynote/10_ai/01_ai_basics/080_gradient_descent_learning_rate/)도 같이 키워야(Linear Scaling Rule) 발걸음의 크기가 맞아요!
