...
> 2. CRT의 핵심 조건은 모듈러들이 쌍으로 서로 소([Pairwise](/studynote/04_software_engineering/03_design_architecture/174_pairwise_comparison_priority_matrix/) Coprime) — m1, m2, ..., mk가 쌍으로 서로 소이면, 모든 합동식을 동시에 만족하는 x가 [0, M) (M=m1×m2×...×mk) 범위에서 유일하게 존재한다.
> 3. CRT는 [RSA](/studynote/09_security/03_network_security/110_rsa/) 복호화 최적화(CRT-[RSA](/studynote/09_security/03_network_security/110_rsa/))와 대형 수 계산 [분산](/studynote/08_algorithm_stats/08_stats/136_variance/)화에 실용적 — [RSA](/studynote/09_security/03_network_security/110_rsa/) 복호화를 두 개의 작은 소수 기반 연산으로 분리하면 약 4배 속도를 달성하며, 이것이 OpenSSL의 기본 구현이다.
...
## Ⅱ. CRT [알고리즘](/studynote/08_algorithm_stats/01_basics/001_algorithm_definition/)
...
> 📢 **섹션 요약 비유**: CRT [알고리즘](/studynote/08_algorithm_stats/01_basics/001_algorithm_definition/) = 퍼즐 조각 맞추기 — 각 모듈러(퍼즐 영역)에서 나머지(조각 [힌트](/studynote/05_database/03_relational_model/167_sql_hint_optimizer_override/))를 모아 전체 그림(원래 수) 복원. M(전체 판), Mi(해당 영역 제외), yi([가중치](/studynote/10_ai/03_llm_nlp/267_weight_bias_activation/)) 계산!
...
> 📢 **섹션 요약 비유**: CRT-[RSA](/studynote/09_security/03_network_security/110_rsa/) = 계산 분업 — [RSA](/studynote/09_security/03_network_security/110_rsa/) 복호화를 p 팀과 q 팀이 나눠 계산([병렬](/studynote/05_database/07_exam_summary/430_index_fast_full_scan/)). CRT로 합산. 혼자 계산보다 4배 빠름. OpenSSL도 이 방법 사용!
...
## Ⅴ. 실무 시나리오 — [분산](/studynote/08_algorithm_stats/08_stats/136_variance/) 컴퓨팅 대형 수 계산
...
> 📢 **섹션 요약 비유**: CRT [병렬](/studynote/05_database/07_exam_summary/430_index_fast_full_scan/) 계산 = 큰 퍼즐 4팀 분담 — 4096피스 퍼즐을 1팀이 하면 느림. 4팀이 1024피스씩 나눠 동시에. CRT로 4팀 결과 합산 -> 이론적 16배 빠름!
...
2. CRT-[RSA](/studynote/09_security/03_network_security/110_rsa/) = 계산 분업 — [RSA](/studynote/09_security/03_network_security/110_rsa/) 복호화를 p팀·q팀 나눠 계산. CRT로 합산. 4배 빠름. OpenSSL 기본 방식!
...
<- **이전**: [004. 빠른 거듭제곱 — Fast Exponentiation](/studynote/08_algorithm_stats/07_numerical/123_fast_exponentiation/)
**다음**: [006. 가우스 소거법 — Gaussian Elimination](/studynote/08_algorithm_stats/07_numerical/125_gaussian_elimination/) ->
