...
> 2. CRT의 핵심 조건은 모듈러들이 쌍으로 서로 소([[174_pairwise_comparison_priority_matrix|Pairwise]] Coprime) — m1, m2, ..., mk가 쌍으로 서로 소이면, 모든 합동식을 동시에 만족하는 x가 [0, M) (M=m1×m2×...×mk) 범위에서 유일하게 존재한다.
> 3. CRT는 [[110_rsa|RSA]] 복호화 최적화(CRT-[[110_rsa|RSA]])와 대형 수 계산 [[136_variance|분산]]화에 실용적 — [[110_rsa|RSA]] 복호화를 두 개의 작은 소수 기반 연산으로 분리하면 약 4배 속도를 달성하며, 이것이 OpenSSL의 기본 구현이다.
...
## Ⅱ. CRT [[001_algorithm_definition|알고리즘]]
...
> 📢 **섹션 요약 비유**: CRT [[001_algorithm_definition|알고리즘]] = 퍼즐 조각 맞추기 — 각 모듈러(퍼즐 영역)에서 나머지(조각 [[167_sql_hint_optimizer_override|힌트]])를 모아 전체 그림(원래 수) 복원. M(전체 판), Mi(해당 영역 제외), yi([[267_weight_bias_activation|가중치]]) 계산!
...
> 📢 **섹션 요약 비유**: CRT-[[110_rsa|RSA]] = 계산 분업 — [[110_rsa|RSA]] 복호화를 p 팀과 q 팀이 나눠 계산([[430_index_fast_full_scan|병렬]]). CRT로 합산. 혼자 계산보다 4배 빠름. OpenSSL도 이 방법 사용!
...
## Ⅴ. 실무 시나리오 — [[136_variance|분산]] 컴퓨팅 대형 수 계산
...
> 📢 **섹션 요약 비유**: CRT [[430_index_fast_full_scan|병렬]] 계산 = 큰 퍼즐 4팀 분담 — 4096피스 퍼즐을 1팀이 하면 느림. 4팀이 1024피스씩 나눠 동시에. CRT로 4팀 결과 합산 → 이론적 16배 빠름!
...
2. CRT-[[110_rsa|RSA]] = 계산 분업 — [[110_rsa|RSA]] 복호화를 p팀·q팀 나눠 계산. CRT로 합산. 4배 빠름. OpenSSL 기본 방식!
...
← **이전**: [[123_fast_exponentiation|004. 빠른 거듭제곱 — Fast Exponentiation]]
**다음**: [[125_gaussian_elimination|006. 가우스 소거법 — Gaussian Elimination]] →
