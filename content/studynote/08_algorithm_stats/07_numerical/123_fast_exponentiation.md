...
> 2. 모듈러 거듭제곱(Modular Exponentiation)이 암호학의 핵심 연산 — [[110_rsa|RSA]] 암호화·복호화, 디피-헬만 키 교환, 밀러-라빈 소수 판별 모두 a^b mod m 계산에 의존하며, 모듈러 연산을 각 단계에 적용해야 수의 크기가 관리 가능하다.
> 3. 행렬 빠른 거듭제곱으로 피보나치를 O(log N)에 계산 — 스칼라 거듭제곱과 동일한 원리를 행렬에 적용하여 피보나치·선형 점화식을 [[568_logs_distributed_logging_elk_fluentd|로그]] 시간에 계산하는 강력한 기법이다.
...
> 📢 **섹션 요약 비유**: 모듈러 거듭제곱 = 시계 덧셈 — 2^1000을 직접 계산하면 300자리 수. 시계(mod)로 계산하면 항상 0~[[308_static_dynamic_nat_pat_port_address_translation|11]] 사이. [[110_rsa|RSA]], 소수 판별 모두 "시계 덧셈"!
...
> 📢 **섹션 요약 비유**: 행렬 거듭제곱 = 변환 반복 빠른 계산 — 피보나치를 100억 번 더하는 대신 행렬 변환을 33번 적용(log₂ [[489_raid_10_hybrid|10]]^[[489_raid_10_hybrid|10]] ≈ 33). F([[489_raid_10_hybrid|10]]^18)도 60번 행렬 곱으로!
...
## Ⅴ. 실무 시나리오 — [[004_blockchain|블록체인]] 서명 [[395_verification_process_review|검증]]
...
> 📢 **섹션 요약 비유**: [[004_blockchain|블록체인]] 서명 [[395_verification_process_review|검증]] = 타원 곡선 도장 찍기 — 개인키(k)로 공개키(k×G) [[087_process_state_transition|생성]]. 역산 불가(이산 [[568_logs_distributed_logging_elk_fluentd|로그]] 문제). [[395_verification_process_review|검증]]은 빠른 거듭제곱 덕에 수 ms. 수학이 [[989_blockchain_security|블록체인 보안]]!
...
1. 반복 제곱법 = 종이 접기 — 10번 접으면 1024배 두께. a^1024도 10번 제곱(×[[489_raid_10_hybrid|10]])으로! 1000번 곱하기 대신 10번!
2. 모듈러 거듭제곱 = 시계 계산 — 큰 수를 12시간 시계로 계산. [[110_rsa|RSA]] 암호화도 시계(mod) 안에서 거듭제곱!
3. 행렬 거듭제곱 = 피보나치 마법 — F([[489_raid_10_hybrid|10]]^18)을 60번 행렬 곱으로 계산. [[489_raid_10_hybrid|10]]^18번 더하기 대신 60번으로!
...
← **이전**: [[122_primality_test|003. 소수 판별 — Primality Test]]
**다음**: [[124_crt|005. 중국인의 나머지 정리 — CRT]] →
