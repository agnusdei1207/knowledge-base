...
> 1. **본질**: 섀넌의 소스 부호화 정리는 *[엔트로피](/knowledge-base/studynote/08_algorithm_stats/09_info_theory/151_entropy/) H(X)가 무손실 [압축](/knowledge-base/studynote/02_operating_system/06_memory_management/347_compaction/)의 절대 하한*임을 증명 — 어떤 [알고리즘](/knowledge-base/studynote/08_algorithm_stats/01_basics/001_algorithm_definition/)도 평균 H(X) [비트](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/073_bit/)/심볼 미만으로 [압축](/knowledge-base/studynote/02_operating_system/06_memory_management/347_compaction/)할 수 없다.
> 2. **가치**: 허프만 (Huffman) 코딩은 [엔트로피](/knowledge-base/studynote/08_algorithm_stats/09_info_theory/151_entropy/)에 1 [비트](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/073_bit/) 이내로 근접하고, 산술 부호화는 임의로 가깝게 접근하며, LZ 계열은 점근적으로 최적이다.
> 3. **판단 포인트**: 무손실과 손실 [압축](/knowledge-base/studynote/02_operating_system/06_memory_management/347_compaction/)의 경계 — 레이트-왜곡 이론 (Rate-Distortion Theory) 이 허용 왜곡 D 아래에서 달성 가능한 최소 [비트](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/073_bit/)율 R(D)를 정의한다.
...
"독립 동일 분포 (i.i.d.) [확률](/knowledge-base/studynote/08_algorithm_stats/08_stats/130_probability/)원 X를 무손실로 [압축](/knowledge-base/studynote/02_operating_system/06_memory_management/347_compaction/)할 때, 심볼당 평균 코드 길이 L̄은 반드시 H(X) 이상이다. 반면, H(X) + ε [비트](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/073_bit/)/심볼로 [압축](/knowledge-base/studynote/02_operating_system/06_memory_management/347_compaction/)하는 [알고리즘](/knowledge-base/studynote/08_algorithm_stats/01_basics/001_algorithm_definition/)이 항상 존재한다."
...
- **불가능성**: L̄ < H(X) [압축](/knowledge-base/studynote/02_operating_system/06_memory_management/347_compaction/)은 정보 손실 없이 불가
- **달성 가능성**: H(X) + ε 이내의 [압축](/knowledge-base/studynote/02_operating_system/06_memory_management/347_compaction/) 방법 존재

📢 **섹션 요약 비유**: 소스 부호화 정리는 "짐 싸기 한계"다 — 여행 가방(코드)에 짐(정보)을 아무리 효율적으로 넣어도 짐 자체의 정보량([엔트로피](/knowledge-base/studynote/08_algorithm_stats/09_info_theory/151_entropy/)) 만큼은 반드시 넣어야 하며, 그것보다 작게는 절대 불가능하다.
...
[확률](/knowledge-base/studynote/08_algorithm_stats/08_stats/130_probability/) 분포: A(0.5), B(0.25), C(0.125), D(0.125)

[엔트로피](/knowledge-base/studynote/08_algorithm_stats/09_info_theory/151_entropy/): H = -(0.5·log0.5 + 0.25·log0.25 + 0.125·log0.125 + 0.125·log0.125)
...
|:---:|:---:|:---:|:---:|
| A | 0.500 | 0 | 1 |
| B | 0.250 | [10](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/489_raid_10_hybrid/) | 2 |
| C | 0.125 | 110 | 3 |
| D | 0.125 | 111 | 3 |

허프만의 성질: H(X) ≤ L̄_Huffman < H(X) + 1

### [알고리즘](/knowledge-base/studynote/08_algorithm_stats/01_basics/001_algorithm_definition/)별 비교

| [알고리즘](/knowledge-base/studynote/08_algorithm_stats/01_basics/001_algorithm_definition/) | 방식 | 한계 도달 | 속도 |
|:---|:---|:---:|:---|
| [허프만 코딩](/knowledge-base/studynote/08_algorithm_stats/05_string/100_huffman_coding/) | 심볼별 가변 길이 코드 | H + 1 이내 | 빠름 |
| 산술 부호화 | 구간 분할 | H + ε | 느림 |
| LZ77 | 슬라이딩 윈도우 사전 | 점근적 H | 빠름 |
| LZ78/LZW | 동적 사전 | 점근적 H | 빠름 |
| DEFLATE | LZ77 + 허프만 | 실용적 최적 | 빠름 |
| Brotli | LZ77 + 허프만 + [컨텍스트](/knowledge-base/studynote/02_operating_system/01_overview_architecture/033_context/) 모델 | 더 높은 비율 | 느림 |
| Zstandard (zstd) | ANS + 허프만 + LZ | 고속 + 고비율 | 매우 빠름 |

📢 **섹션 요약 비유**: [허프만 코딩](/knowledge-base/studynote/08_algorithm_stats/05_string/100_huffman_coding/)은 "자주 쓰는 단어에 짧은 줄임말"을 배정하는 것이다 — "국제연합"을 "UN"으로, "[인공지능](/knowledge-base/studynote/10_ai/03_llm_nlp/231_ai_turing_test/)"을 "[AI](/knowledge-base/studynote/04_software_engineering/03_design_architecture/190_ai_llm_requirements_specification/)"로 쓰는 것처럼 빈도 높은 심볼에 짧은 코드를 준다.

---

## Ⅲ. 비교 및 연결

### 손실 [압축](/knowledge-base/studynote/02_operating_system/06_memory_management/347_compaction/)과 레이트-왜곡 이론

무손실 [압축](/knowledge-base/studynote/02_operating_system/06_memory_management/347_compaction/)의 하한 = H(X). 하지만 약간의 왜곡(오차)을 허용하면?
...
| 분야 | 기술 | 방식 |
|:---|:---|:---|
| 텍스트/[파일](/knowledge-base/studynote/02_operating_system/09_file_system/501_file_definition_logical_record/) | gzip, brotli, zstd | LZ + 허프만 (무손실) |
| 이미지 정지 | JPEG (DCT), PNG (DEFLATE) | 손실/무손실 |
| 동영상 | H.264, H.265, AV1 | 모션 보상 + DCT (손실) |
| 오디오 | MP3, AAC, Opus | 심리음향 모델 (손실) |
| [AI](/knowledge-base/studynote/04_software_engineering/03_design_architecture/190_ai_llm_requirements_specification/) 모델 [가중치](/knowledge-base/studynote/10_ai/03_llm_nlp/267_weight_bias_activation/) | GZIP + [양자화](/knowledge-base/studynote/01_computer_architecture/12_accelerators_ai_hardware/434_quantization/) | 손실 허용 |
