...
> 3. **판단 포인트**: [[155_channel_capacity|채널 용량]] C = max I(X;Y), ML 특성 선택, 클러스터링 평가([[558_nmi|NMI]]) — 실무 세 곳에서 동시에 쓰이므로 수식과 해석을 함께 암기하라.
...
- **H(X) + H(Y) - H(X,Y)**: 두 [[151_entropy|엔트로피]]의 합에서 결합 [[151_entropy|엔트로피]]를 뺀 "중복분"
...
- **H(X,Y)**: 두 원의 합집합 (전체 결합 [[151_entropy|엔트로피]])

### [[155_channel_capacity|채널 용량]]과의 연결

**[[155_channel_capacity|채널 용량]] ([[155_channel_capacity|Channel Capacity]])** C는 입력 분포 P(X)를 최적화하여 얻는 최대 상호 정보량:
...
📢 **섹션 요약 비유**: [[155_channel_capacity|채널 용량]]은 "파이프의 최대 [[139_throughput|처리량]]"이다 — 어떤 입력 분포를 써야 파이프를 가장 효율적으로 사용하는지를 상호 정보량 최대화로 계산한다.

---

## Ⅲ. 비교 및 연결

### [[558_nmi|NMI]] (Normalized Mutual Information) — 클러스터링 평가

MI는 클러스터 수가 많아지면 인위적으로 커지므로, **[[093_normalization|정규화]] 상호 정보량 ([[558_nmi|NMI]], Normalized Mutual Information)** 을 사용:
...
### 특성 선택 (Feature [[022_mcts_four_stages|Selection]]) 에서의 활용

- **MRMR (Minimum Redundancy Maximum Relevance)**: 타깃과의 MI 최대화 + 특성 간 MI 최소화
- **Relief** 계열, **HSIC (Hilbert-Schmidt [[133_independence|Independence]] Criterion)** 의 [[022_kernel_role|커널]] MI 확장
...
### [[241_machine_learning_basics|머신러닝]] 특성 선택 파이프라인
...
📢 **섹션 요약 비유**: [[558_nmi|NMI]] 클러스터 평가는 "지도 대조"와 같다 — 예측한 클러스터 지도와 실제 정답 지도가 얼마나 일치하는지를 0~1 사이로 표현한다.
...
실무적으로 중요한 세 축:
1. **[[155_channel_capacity|채널 용량]]**: I(X;Y)를 최대화하면 최적 통신 [[268_strategy_pattern|전략]] 도출
2. **특성 선택**: 높은 MI 특성 → 모델 [[282_performance_tactics|성능]] vs 복잡도 균형
3. **클러스터링 평가**: NMI로 레이블 없는 평가 가능

양자 정보이론에서는 **양자 상호 정보량 ([[690_round_robin_time_quantum|Quantum]] Mutual Information)** 이 고전 MI의 대응물로, [[220_quantum_entanglement|양자 얽힘]] 측정에 활용된다.
...
| 개념 | 수식 | 연결 |
|:---|:---|:---|
| 상호 정보량 I(X;Y) | H(X) - H(X\|Y) | [[155_channel_capacity|채널 용량]], 특성 선택 |
| [[558_nmi|NMI]] | 2I/(H(X)+H(Y)) | 클러스터링 평가 |
| [[155_channel_capacity|채널 용량]] C | max_{P(X)} I(X;Y) | 섀넌 채널 한계 |
...
← **이전**: [[151_entropy|2. 엔트로피 (Shannon Entropy) — H(X) = -Σ p·log₂p]]
**다음**: [[153_kl_divergence|4. KL 다이버전스 (KL Divergence, Kullback-Leibler Divergence) — 분포 차이]] →
