...
> 2. NP의 가장 직관적 정의는 "정답을 주면 빠르게 확인할 수 있는 문제" — 스도쿠 완성 여부 확인은 쉽지만 스도쿠를 처음 푸는 것은 어렵듯이, [[395_verification_process_review|검증]]은 쉬워도 풀기는 어려울 수 있다.
> 3. NP는 P와 마찬가지로 판정 문제(Decision Problem)에 대한 복잡도 클래스이며, "NP = 비결정론적 다항 시간"이지 "비다항 시간(Not [[195_polynomial_generator_crc|Polynomial]])"이 아님을 혼동하지 않는 것이 중요하다.
...
> 📢 **섹션 요약 비유**: NP는 "정답 확인이 빠른 퀴즈" — 학생이 쓴 답을 채점하는 건 빠르지만([[395_verification_process_review|검증]]), 문제를 처음 푸는 건 느릴 수 있다(해결).
...
> 📢 **섹션 요약 비유**: NP 문제는 정답이 쪽지로 주어지면 맞는지 바로 확인할 수 있는 난해한 퍼즐 — 스도쿠 완성본 [[395_verification_process_review|검증]]은 쉽지만 처음 풀기는 어렵다.

---

## Ⅲ. [[395_verification_process_review|검증]]자(Verifier) 관점
...
> 📢 **섹션 요약 비유**: NP [[395_verification_process_review|검증]]자는 선생님의 채점 — 학생이 답(증거)을 쓰면 선생님이 빠르게 채점([[395_verification_process_review|검증]])하지만, 선생님도 처음에 문제를 푸는 건 어렵다.
...
> 📢 **섹션 요약 비유**: NP임을 아는 것은 도로 없는 산을 인식하는 것 — 헬기([[012_approximation_algorithm|근사 알고리즘]])나 등산로([[210_heuristics_scheduling|휴리스틱]])로 오르는 현실적 방법을 선택하게 된다.
...
2. 스도쿠를 누가 풀어 놓은 것을 검사하는 건 쉽지만(NP [[395_verification_process_review|검증]]), 스도쿠를 처음부터 푸는 건 훨씬 어려운 것처럼요.
3. [[110_rsa|RSA]] 인터넷 암호가 안전한 이유가 바로 "큰 수의 인수분해"가 NP이기 때문이에요 — 정답(소인수)을 알면 확인은 빠르지만 찾기는 엄청나게 어려워요!
...
← **이전**: [[106_p_class|001. P 클래스 (P Class) — 다항 시간 내 해결 가능한 문제]]
**다음**: [[108_np_complete|003. NP-완전 (NP-Complete)]] →
