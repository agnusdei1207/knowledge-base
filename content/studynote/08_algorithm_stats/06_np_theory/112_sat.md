...
> **핵심 인사이트**
> 1. [SAT](/studynote/12_it_management/03_ea_isp/887_chaining/)(Boolean [Satisfiability](/studynote/12_it_management/03_ea_isp/887_chaining/) Problem)는 Cook-Levin 정리(1971)에 의해 최초로 NP-완전임이 증명된 문제로 — NP-완전 클래스의 "시조"이며, 이후 모든 NP-완전 증명의 기반이 된다.
> 2. SAT가 NP-완전이라는 의미는 — 모든 NP 문제를 다항 시간 내에 SAT로 변환(다항 시간 귀납)할 수 있으므로, SAT를 다항 시간에 풀 수 있다면 P=NP가 성립한다는 뜻이다.
> 3. 실용적으로 DPLL, CDCL [알고리즘](/studynote/08_algorithm_stats/01_basics/001_algorithm_definition/)과 [SAT](/studynote/12_it_management/03_ea_isp/887_chaining/) 솔버(Z3, CryptoMiniSat)는 수천만 변수를 갖는 산업용 문제(칩 설계, 소프트웨어 [검증](/studynote/04_software_engineering/07_object_oriented/395_verification_process_review/), 암호 분석)를 해결하고 있어 — 이론적 불가능과 실용적 가능 사이의 간극을 보여준다.

---

## Ⅰ. [SAT](/studynote/12_it_management/03_ea_isp/887_chaining/) 문제 정의
...
> 📢 **섹션 요약 비유**: SAT는 "[스위치](/studynote/03_network/05_lan_wan_l2_devices/238_switch_operation_principles/) 퍼즐" — n개의 [스위치](/studynote/03_network/05_lan_wan_l2_devices/238_switch_operation_principles/)를 ON/OFF로 조합해서 모든 방의 불이 켜지는 조합이 있는가? [스위치](/studynote/03_network/05_lan_wan_l2_devices/238_switch_operation_principles/) 조합 = 변수 할당, 방의 조건 = 절.
...
## Ⅲ. [SAT](/studynote/12_it_management/03_ea_isp/887_chaining/) [알고리즘](/studynote/08_algorithm_stats/01_basics/001_algorithm_definition/)
...
> 📢 **섹션 요약 비유**: [SAT](/studynote/12_it_management/03_ea_isp/887_chaining/) 솔버 응용은 만능 [논리](/studynote/09_security/04_endpoint_security/369_logic_bomb/) 검사기 — "이 회로가 특정 입력에서 오작동하는가?", "이 코드에 버그가 있는가?"를 수학적으로 엄밀하게 답해주는 도구.
...
> 📢 **섹션 요약 비유**: [SAT](/studynote/12_it_management/03_ea_isp/887_chaining/) 기반 버그 탐지는 모든 경우의 수 자동 검사 — 수동 테스트로 못 찾는 희귀 조건(INT_MIN/-1)을 수학적으로 자동 발견. 버그를 "찾는다"가 아니라 "증명한다".
...
1. SAT는 "[스위치](/studynote/03_network/05_lan_wan_l2_devices/238_switch_operation_principles/) 퍼즐" — n개 [스위치](/studynote/03_network/05_lan_wan_l2_devices/238_switch_operation_principles/)를 켜고 끄는 방법 중에서 모든 조건을 만족시키는 조합이 존재하는지 찾는 문제예요!
2. Cook이 1971년에 증명한 것 — "이 [스위치](/studynote/03_network/05_lan_wan_l2_devices/238_switch_operation_principles/) 퍼즐이 세상 모든 어려운 문제의 공통 조상이야" — 이것이 컴퓨터 과학의 대발견!
3. 이론적으로 매우 어렵지만 실용적으로는 가능 — 현대 [SAT](/studynote/12_it_management/03_ea_isp/887_chaining/) 솔버는 수백만 개 [스위치](/studynote/03_network/05_lan_wan_l2_devices/238_switch_operation_principles/) 문제를 몇 초 만에 풀어서 칩 설계, 버그 탐지에 활용돼요!
...
<- **이전**: [006. 다항 시간 환산 (Polynomial Reduction)](/studynote/08_algorithm_stats/06_np_theory/111_polynomial_reduction/)
**다음**: [008. 클리크 문제 — Clique Problem](/studynote/08_algorithm_stats/06_np_theory/113_clique_problem/) ->
