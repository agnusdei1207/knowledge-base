...
> 2. ETH와 SETH(Strong ETH)는 [[001_algorithm_definition|알고리즘]]의 최적성 증명에 활용 — "이 [[001_algorithm_definition|알고리즘]]이 이보다 빠를 수 없다"는 조건부 하한선 증명이 ETH를 가정하는 방식으로 이루어지며, 파라미터화 복잡도(Parameterized Complexity) 분야의 핵심 도구다.
> 3. ETH가 참이라면 NP-완전 문제는 지수 시간보다 빠른 [[001_algorithm_definition|알고리즘]]이 없음 — 이는 현재 알려진 [[103_chaining|SAT]] [[001_algorithm_definition|알고리즘]](DPLL, CDCL)의 최악 성능이 본질적이라는 것을 시사하며, "더 빠른 [[001_algorithm_definition|알고리즘]] 찾기"가 ETH 부정과 동치임을 의미한다.
...
> 📢 **섹션 요약 비유**: ETH는 미로 탈출 최소 시간 — "이 미로는 아무리 빠른 방법도 최소 2^n 스텝이 필요하다"는 주장. 증명은 못 했지만, 이걸 믿으면 더 빠른 [[001_algorithm_definition|알고리즘]]이 없다는 결론!
...
## Ⅲ. ETH와 [[103_chaining|SAT]] Solver
...
> 📢 **섹션 요약 비유**: ETH와 [[103_chaining|SAT]] Solver는 제한 속도 — ETH(이론적 제한) = 최악 케이스 지수 시간. 하지만 실제 도로(현실 인스턴스)는 구조적으로 빠른 길. 이론 최악 = 실제 최악 아님!
...
> 📢 **섹션 요약 비유**: SETH는 이차 장벽의 벽 — [[103_edit_distance|편집 거리]]·[[053_lcs|LCS]] [[001_algorithm_definition|알고리즘]]이 O(n²)보다 빠를 수 없다는 조건부 증명. SETH가 맞다면 수십 년간의 최적 [[001_algorithm_definition|알고리즘]]이 진짜 최적!

---

## Ⅴ. 실무 시나리오 — ETH 기반 [[001_algorithm_definition|알고리즘]] 설계
...
2. 조건부 하한은 논리적 증명 — "ETH가 맞다면 이 [[001_algorithm_definition|알고리즘]]보다 빠른 것은 없어요"를 수학으로 보여줘요. 직접 증명 대신 논리로!
3. SETH는 더 강한 벽 — [[103_edit_distance|편집 거리]], LCS를 O(n²)보다 빠르게 풀면 SETH가 틀린다고 밝혀졌어요. 그래서 O(n²)이 사실상 최선!
...
← **이전**: [[117_approximation_np|012. NP 근사 알고리즘 — Approximation Algorithms for NP]]
**다음**: [[119_quantum_complexity|014. 양자 복잡도 — BQP, QMA]] →
