...
> 2. TSP는 물류 배송 경로 최적화, [[009_semiconductor|반도체]] 드릴링 순서, PCB 부품 배치 등 현실 문제의 "이상화"로 — 정확한 최적해는 지수 시간이 필요하지만, Christofides [[001_algorithm_definition|알고리즘]](1.5-근사)과 2-opt [[210_heuristics_scheduling|휴리스틱]]이 실용적 해를 빠르게 제공한다.
> 3. 모든 NP-하드 최적화 문제를 "TSP로 변환"할 수 있다는 점에서 — [[106_fenwick_tree|TSP]] 솔버(Concorde, LKH)의 발전은 물류 최적화뿐 아니라 유전 [[001_algorithm_definition|알고리즘]]·[[396_simulated_annealing_heuristic|모의 담금질]] 같은 메타휴리스틱 기법 발전을 이끌었다.

---

## Ⅰ. [[106_fenwick_tree|TSP]] 정의
...
> 📢 **섹션 요약 비유**: [[106_fenwick_tree|TSP]] NP-완전 증명은 "같은 어려움의 다른 옷" — 해밀톤 경로라는 이미 어려운 문제를 TSP로 변환. TSP가 해밀톤 경로만큼 어렵다는 것을 증명해요.

---

## Ⅲ. 근사 및 [[210_heuristics_scheduling|휴리스틱]]
...
> 📢 **섹션 요약 비유**: 메타휴리스틱은 자연을 모방한 탐색 — 유전 [[001_algorithm_definition|알고리즘]]은 진화처럼, 담금질은 철 냉각처럼, 개미는 페로몬처럼. 자연이 수억 년 동안 최적화한 방법을 [[001_algorithm_definition|알고리즘]]에 차용!
...
> 📢 **섹션 요약 비유**: 물류 [[106_fenwick_tree|TSP]] 최적화는 배달원 루트 최적화 — 500개 집을 배달하는 최단 경로를 컴퓨터가 수분 내 계산. 25% 단축하면 연간 수천만 원 연료비 절감!
...
← **이전**: [[114_vertex_cover|009. 버텍스 커버 — Vertex Cover]]
**다음**: [[116_knapsack|011. 배낭 문제 — Knapsack Problem]] →
