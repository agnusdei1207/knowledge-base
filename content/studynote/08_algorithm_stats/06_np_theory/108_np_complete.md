...
> 2. Cook-Levin 정리(1971)는 [[103_chaining|SAT]](Boolean 만족가능성) 문제가 최초의 NP-완전 문제임을 증명했으며, 이후 Karp가 21개의 NP-완전 문제를 다항시간 환산([[111_polynomial_reduction|Polynomial Reduction]])으로 목록화하여 NP-완전 문제군 이론의 토대를 놓았다.
> 3. NP-완전 문제는 현실에서 [[012_approximation_algorithm|근사 알고리즘]]([[012_approximation_algorithm|Approximation Algorithm]])·메타휴리스틱([[767_sa_standalone_5g_core_network|SA]], [[169_evolutionary_algorithms|GA]])·고정 파라미터 추론(FPT)으로 "충분히 좋은" 해를 빠르게 찾는 실용적 접근이 핵심이다.
...
> 📢 **섹션 요약 비유**: NP-완전 문제들은 서로 다른 모양의 퍼즐인데 사실 같은 난이도 — [[049_hamiltonian_path|해밀턴 경로]], 배낭, 채색 모두 다항시간에 서로 변환 가능.
...
## Ⅴ. 실무 시나리오 — [[208_schedule_history_transaction_execution_order|스케줄]]링 NP-완전
...
> 📢 **섹션 요약 비유**: 물류 TSP는 "완벽한 최단 경로" 대신 "충분히 좋은 경로" 실용 [[268_strategy_pattern|전략]] — 완벽 해는 영원히 못 찾지만, 85% 효율로도 비즈니스 문제는 해결.
...
2. [[103_chaining|SAT]](변수들을 참/거짓으로 정해서 수식이 참이 되게 할 수 있나?)가 첫 NP-완전 문제로 증명됐고, 이걸 빠르게 풀면 수천 개의 어려운 문제도 빠르게 풀 수 있어요.
3. 완벽한 답을 빠르게 찾을 수 없어서, 현실에서는 "충분히 좋은 답([[012_approximation_algorithm|근사 알고리즘]])"으로 배송 경로, 공장 [[208_schedule_history_transaction_execution_order|스케줄]] 같은 문제를 해결해요!
...
← **이전**: [[107_np_class|002. NP 클래스 (NP Class) — 다항 시간 내 검증 가능한 문제]]
**다음**: [[109_np_hard|004. NP-어려움 (NP-Hard)]] →
