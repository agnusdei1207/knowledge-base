...
> [[001_algorithm_definition|알고리즘]]·AI에서 확률은 단순한 이론이 아니라, 랜덤 [[001_algorithm_definition|알고리즘]]·베이즈 네트워크·강화학습 등 핵심 설계 도구로 직결된다.
...
- **정의**: 동일 실험을 n번 반복했을 때, 사건 A 가 n(A) 번 발생하면  
  `P(A) = lim_{n→∞} n(A) / n`
- **예시**: 불량률 측정 — [[489_raid_10_hybrid|10]],000개 중 35개 불량 → P(불량) ≈ 0.0035
...
**표본공간 [[003_bigdata_7v|시각화]] — 동전 2회 던지기**
...
2. **[[093_normalization|정규화]] ([[093_normalization|Normalization]])**: `P(Ω) = 1` (전체 확률의 합은 1)
...
## Ⅴ. [[001_algorithm_definition|알고리즘]]·AI에서의 확률 응용

### 확률적 [[001_algorithm_definition|알고리즘]] ([[013_randomized_algorithm|Randomized Algorithm]])

- **Las Vegas [[001_algorithm_definition|알고리즘]]**: 항상 정답, 실행 시간이 확률적 (예: 랜덤 퀵정렬)
- **Monte Carlo [[001_algorithm_definition|알고리즘]]**: 실행 시간 확정, 오답 확률 허용 (예: 소수 판별 밀러-라빈 테스트)

### 베이즈 네트워크 (Bayesian Network)

- 확률 변수들의 조건부 독립 [[083_relationship_in_er_model|관계]]를 [[401_bayesian_network_dag_causality|DAG]] ([[255_apache_airflow_dag|Directed Acyclic Graph]]) 로 표현
- 각 노드 P(Xᵢ | 부모 노드들) 형태의 [[132_conditional_probability|조건부 확률]] 테이블 보유
- 의료 진단, 자연어 처리 (NLP, Natural Language Processing), 추천 시스템에 활용

### 강화학습 ([[094_reinforcement_learning|Reinforcement Learning]]) 에서의 확률

- 환경 전이 확률 (Transition Probability): `P(s'|s, a)` — 상태 s 에서 행동 a 후 s' 로 이동할 확률
- [[164_policy|정책]] ([[164_policy|Policy]]): `π(a|s)` — 상태 s 에서 행동 a 를 선택할 확률
...
|:---|:---|:---|
| 고전적 확률 | 조합론 ([[075_combinatorics|Combinatorics]]) | 경우의 수 계산 |
| 상대도수 확률 | [[074_law_of_large_numbers_lln_convergence_probability|대수의 법칙]] (Law of Large Numbers) | 극한 수렴 |
| 주관적 확률 | 베이즈 정리 (Bayes' Theorem) | 사전→사후 업데이트 |
| 콜모고로프 공리 | 측도론 (Measure Theory) | 수학적 기반 |
| 덧셈 법칙 | 포함-배제 원리 | 일반화 |
| 확률적 [[001_algorithm_definition|알고리즘]] | [[135_expected_value|기댓값]] 분석 | [[282_performance_tactics|성능]] 보장 |
...
- 확률이 있으면 AI도 "이게 고양이일 확률 90%, 강아지일 확률 [[489_raid_10_hybrid|10]]%"처럼 자기 믿음을 숫자로 말할 수 있어.
...
← **이전**: [[129_monte_carlo_integration|10. 몬테카를로 수치적분 (Monte Carlo Integration) — 확률적 근사]]
**다음**: [[131_bayes_theorem|2. 베이즈 정리 (Bayes' Theorem) — 사전/사후 확률 업데이트]] →
