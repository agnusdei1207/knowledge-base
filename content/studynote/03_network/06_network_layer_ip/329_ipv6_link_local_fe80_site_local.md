...
링크 로컬 주소 / 사이트 로컬 주소를 볼 때는 앞뒤 개념과의 경계를 함께 봐야 전체 흐름이 선명해진다. 유니캐스트, [멀티캐스트](/knowledge-base/studynote/03_network/06_network_layer_ip/298_ip_classes_a_b_c_d_multicast_e_experimental/), 애니캐스트가 기반 조건을 만든다면, 링크 로컬 주소 / 사이트 로컬 주소는 그 위에서 핵심 메커니즘을 구현하고, EUI-64는 이를 더 확장된 적용 단계로 연결한다. 따라서 단일 정의보다 주소 효율과 도달성에 어떤 차이를 만드는지 비교하는 것이 중요하다.

| 관점 | 선행 개념 | 현재 개념 | 확장 개념 |
|:---|:---|:---|:---|
| 초점 | 유니캐스트, [멀티캐스트](/knowledge-base/studynote/03_network/06_network_layer_ip/298_ip_classes_a_b_c_d_multicast_e_experimental/), 애니캐스트의 기반 정리 | 링크 로컬 주소 / 사이트 로컬 주소의 핵심 동작 | EUI-64의 확장 적용 |
...
실무에서는 링크 로컬 주소 / 사이트 로컬 주소를 단독 개념으로 외우기보다 어떤 병목을 줄이기 위한 선택인지 먼저 따져야 한다. 특히 유니캐스트, [멀티캐스트](/knowledge-base/studynote/03_network/06_network_layer_ip/298_ip_classes_a_b_c_d_multicast_e_experimental/), 애니캐스트 수준의 기본 대책으로 충분한지, 아니면 링크 로컬 주소 / 사이트 로컬 주소가 제공하는 메커니즘이 실제로 필요한지 구분해야 한다. 이후 확장 단계에서는 EUI-64와 같은 후속 기술, 자동화 체계, 표준 호환성까지 함께 검토해야 한다.

### 실무 [체크리스트](/knowledge-base/studynote/04_software_engineering/11_testing_validation/435_checklist_based_testing/)

1. 현재 문제의 핵심이 주소 효율 부족인지, 도달성 악화인지 먼저 분리한다.
2. 링크 로컬 주소 / 사이트 로컬 주소가 추가하는 복잡도와 운영 이득이 균형을 이루는지 [확인](/knowledge-base/studynote/04_software_engineering/12_testing_maintenance/396_validation/)한다.
3. 도입 후에는 인접 기술인 EUI-64와의 연계 방식을 함께 검증한다.

### [안티패턴](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/128_water_scrum_fall_anti_pattern/)

- 링크 로컬 주소 / 사이트 로컬 주소의 장점만 보고 트래픽 패턴이나 운영 비용을 무시한 채 과도 도입하는 설계
- 유니캐스트, [멀티캐스트](/knowledge-base/studynote/03_network/06_network_layer_ip/298_ip_classes_a_b_c_d_multicast_e_experimental/), 애니캐스트와의 경계를 정리하지 않아 중복 투자나 [정책](/knowledge-base/studynote/10_ai/02_dl_architecture_new/164_policy/) 충돌을 만드는 설계
...
| 개념 | 연결 포인트 |
|:---|:---|
| 유니캐스트, [멀티캐스트](/knowledge-base/studynote/03_network/06_network_layer_ip/298_ip_classes_a_b_c_d_multicast_e_experimental/), 애니캐스트 | 현재 개념이 등장하기 전에 갖춰야 할 배경이나 인접 선행 개념이다. |
...
| [EUI-64](/knowledge-base/studynote/03_network/06_network_layer_ip/330_eui_64_mac_to_ipv6_interface_id/) | 현재 개념이 확장되거나 적용 단계로 이어질 때 자주 함께 언급된다. |
...
<- **이전**: [328. 유니캐스트, 멀티캐스트, 애니캐스트(Anycast, 가장 가까운 노드 응답)](/knowledge-base/studynote/03_network/06_network_layer_ip/328_ipv6_address_types_unicast_multicast_anycast_no_broadcast/)
**다음**: [330. EUI-64 (MAC 기반 IPv6 호스트 주소 자동생성)](/knowledge-base/studynote/03_network/06_network_layer_ip/330_eui_64_mac_to_ipv6_interface_id/) ->
