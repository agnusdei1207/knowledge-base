...
- TCP는 인사(Handshake)를 해야 문을 열어주지만, **UDP는 인사를 안 하고 그냥 목적지로 패킷을 일방적으로 휙 집어 던지는 [프로토콜](/knowledge-base/studynote/03_network/06_network_layer_ip/295_protocol_field_tcp_udp_icmp/)**입니다.
...
- 프로그램이 없다는 걸 깨달은 서버는 친절하게도 **"[포트](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/446_port_and_bus/) 안 열려있어!"라는 `ICMP Destination Unreachable` 에러 메시지를 만들어 해커에게 반송**해 줍니다. 
...
### 1. Null (널) 공격
- **원리**: 6개의 깃발을 **단 한 개도 들지 않고(모든 [플래그](/knowledge-base/studynote/03_network/04_data_link_layer_error/186_character_stuffing_dle_stx_etx/)를 0으로 셋팅)** 텅 빈 좀비 상태로 패킷을 날립니다.
...
- **원리**: 반대로, 6개의 깃발 중 **절대 동시에 들 수 없는 깃발들(FIN, PSH, URG 등)을 일제히 전부 다 들고(모든 [플래그](/knowledge-base/studynote/03_network/04_data_link_layer_error/186_character_stuffing_dle_stx_etx/)를 1로 셋팅)** 돌진합니다. 
