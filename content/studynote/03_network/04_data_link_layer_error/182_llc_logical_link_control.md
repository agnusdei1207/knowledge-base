...
기술사 답안에서는 "LLC는 데이터 링크 계층 상부에서 논리적 다중화와 서비스 모델을 담당하고, MAC은 실제 매체 접근을 담당한다"는 문장을 축으로 가져가면 좋다. 여기에 <strong>Ethernet II와 LLC/SNAP 공존</strong>, <strong>현대 환경에서의 실질 역할 축소</strong>를 덧붙이면 더 정확한 설명이 된다.
