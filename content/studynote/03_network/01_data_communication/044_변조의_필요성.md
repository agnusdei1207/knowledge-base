...
> **핵심 인사이트**
> 1. 변조(Modulation)는 정보 [[130_signal|신호]]([[940_baseband_line_coding_nrz_rz_manchester|기저대역]] [[130_signal|신호]])를 전송 매체에 적합한 고주파 [[054_반송파_Carrier_Wave|반송파]]([[054_반송파_Carrier_Wave|Carrier Wave]])에 실어 보내는 과정으로 — [[171_antenna_basic_dipole_resonance|안테나]] 크기 제약, [[071_다중화_Multiplexing|다중화]]([[071_다중화_Multiplexing|Multiplexing]]), 전파 특성 최적화를 위해 필수적이다.
> 2. AM/FM/PM이 아날로그 변조라면, 디지털 변조는 ASK/FSK/[[142_psk_pre_shared_key|PSK]]/QAM으로 발전하며 — QAM-256/1024은 하나의 심볼에 8~10비트를 담아 [[140_bandwidth|대역폭]] 효율을 극대화하지만, 잡음 내성이 낮아 고품질 채널(광케이블, WiFi 근거리)에서만 사용 가능하다.
...
> 📢 **섹션 요약 비유**: 변조는 화물 포장 — 원래 물건([[940_baseband_line_coding_nrz_rz_manchester|기저대역]] [[130_signal|신호]])을 배송에 적합한 박스([[054_반송파_Carrier_Wave|반송파]])에 포장해서 보내는 것. 박스 종류에 따라 배송 방법과 안정성이 달라요.
...
> 📢 **섹션 요약 비유**: QAM은 정보를 더 빽빽하게 — BPSK는 큰 글씨로 한 글자, QAM-256은 아주 작은 글씨로 여덟 글자를 같은 공간에 적는 것. 많이 담지만 잘 보려면([[024_신호_대_잡음비|SNR]]) 좋은 환경 필요.
...
## Ⅴ. 실무 시나리오 — [[418_5g_embb_urllc_mmtc_slicing|5G]] 기지국 적응형 변조
...
> 📢 **섹션 요약 비유**: [[418_5g_embb_urllc_mmtc_slicing|5G]] 적응형 변조는 날씨 따라 포장 달리하기 — 맑은 날(좋은 채널)엔 얇은 포장(고밀도 QAM), 비바람(나쁜 채널)엔 두꺼운 포장(BPSK). 화물은 항상 안전하게 도착!
...
1. 변조는 화물 포장 — 보내려는 정보(원신호)를 전파([[054_반송파_Carrier_Wave|반송파]])라는 배달 박스에 넣어서 멀리 보내는 과정이에요!
2. AM은 볼륨, FM은 피치 — AM 라디오는 소리 크기로 정보를 담고, FM은 소리 높낮이로 담아서 잡음에 더 강해요.
3. QAM은 정보를 빽빽하게 포장 — 256-QAM은 한 번의 전송에 8비트를 담아서 [[148_5g_embb_urllc_mmtc|초고속]] 통신을 가능하게 해요!
...
← **이전**: [[043_B8ZS_HDB3_스크램블링|043. B8ZS/HDB3 스크램블링]]
**다음**: [[045_진폭_편이_변조_ASK|045. 진폭 편이 변조 — ASK (Amplitude Shift Keying)]] →
