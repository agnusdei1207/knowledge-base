...
> **핵심 인사이트**
> 1. FSK(Frequency Shift Keying, 주파수 편이 변조)는 디지털 데이터를 반송파의 주파수를 변화시켜 표현하는 변조 방식 — ASK(진폭 변조)보다 잡음 내성이 뛰어나며, 2-FSK는 [[073_bit|비트]] 0과 1에 서로 다른 주파수(f0, f1)를 할당한다.
> 2. FSK의 핵심 설계 파라미터는 주파수 편이(Frequency Deviation, Δf)와 변조 지수(Modulation [[154_database_index_b_tree_search_optimization|Index]], h=2Δf/T) — 변조 지수 h=0.5인 특수 형태가 MSK(Minimum Shift Keying)이며, GMSK(Gaussian MSK)는 GSM 모바일 통신의 표준이다.
> 3. 현대 통신에서 FSK 단독 사용은 저속·저비용 시스템에서 주로 사용 — 도어 리모컨, 페이저, 아날로그 [[146_modem_modulator_demodulator|모뎀]](Bell 103), RFID 등에 활용되며, 고속 통신에서는 더 효율적인 QAM이 대체한다.
...
> 📢 **섹션 요약 비유**: FSK는 음정으로 메시지 — 높은 음(f1, [[073_bit|비트]] 1), 낮은 음(f0, [[073_bit|비트]] 0). 음정은 소음에 강해요. 하지만 음정 여러 개 쓰면 공간([[140_bandwidth|대역폭]])이 많이 필요!
...
## Ⅲ. FSK vs ASK vs [[142_psk_pre_shared_key|PSK]] [[140_bandwidth|대역폭]] 비교
...
> 📢 **섹션 요약 비유**: FSK vs QAM [[140_bandwidth|대역폭]]은 단어 방식 차이 — FSK는 음정 하나씩([[073_bit|비트]] 하나), QAM은 화음(여러 [[073_bit|비트]]). 화음이 더 많은 정보를 같은 공간에 담아요!
...
## Ⅴ. 실무 시나리오 — [[101_iot_concept|IoT]] FSK 통신
...
> 📢 **섹션 요약 비유**: [[101_iot_concept|IoT]] FSK는 저전력 모스 [[130_signal|신호]] — 스마트 미터가 1시간마다 한 번 주파수 [[130_signal|신호]] 전송. 비선형 증폭기로 배터리 15년! 고속이 필요 없으면 FSK가 최고!
...
1. FSK는 음정 [[130_signal|신호]] — 높은 음(1), 낮은 음(0)으로 정보 전달. 소음에 강하지만 넓은 주파수 공간이 필요해요!
...
← **이전**: [[045_진폭_편이_변조_ASK|045. 진폭 편이 변조 — ASK (Amplitude Shift Keying)]]
**다음**: [[047_위상_편이_변조_PSK|047. PSK — 위상 편이 변조]] →
