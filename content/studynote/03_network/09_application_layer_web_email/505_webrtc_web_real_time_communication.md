...
> 2. **가치**: WebRTC를 이해하면 [응답 시간](/knowledge-base/studynote/01_computer_architecture/03_architecture_basics_performance/138_response_time/)과 [호환성](/knowledge-base/studynote/04_software_engineering/06_software_architecture/344_compatibility_usability/) 사이의 균형을 더 정확히 볼 수 있다.
...
### 1. WebRTC의 3대 자바스크립트(JS) 심장 [API](/knowledge-base/studynote/02_operating_system/01_overview_architecture/014_api_posix/)

개발자가 프론트엔드에서 치는 단 3가지 핵심 명령어다.
- `getUserMedia()`: 카메라와 마이크 권한을 따서 내 화면(Local [Stream](/knowledge-base/studynote/03_network/09_application_layer_web_email/467_http2_stream_multiplexing_tcp_hol/))을 띄운다. (이때 "카메라 접근을 허용하시겠습니까?" 팝업이 뜬다).
- `RTCPeerConnection()`: 이 기술의 영혼이다. ICE 프레임워크(STUN/TURN)를 백그라운드에서 미친 듯이 돌려 상대방 브라우저로 가는 최적의 [P2P](/knowledge-base/studynote/03_network/18_optical_nextgen_automation/916_p2p_peer_to_peer_networking_super_node_gnutella/) 터널(통로)을 기가 막히게 뚫고, 코덱(H.264 등) 협상과 암호화(SRTP/[DTLS](/knowledge-base/studynote/03_network/12_iot_wpan_edge/644_dtls_datagram_tls_coap_security/))까지 몽땅 씹어 먹어준다.
- `RTCDataChannel()`: 비디오만 쏘는 게 아니다! 이 터널을 통해 일반 텍스트나 바이너리 [파일](/knowledge-base/studynote/02_operating_system/09_file_system/501_file_definition_logical_record/)(1GB짜리 영화 [파일](/knowledge-base/studynote/02_operating_system/09_file_system/501_file_definition_logical_record/))도 P2P로 쏴버린다. 무려 [UDP](/knowledge-base/studynote/03_network/08_transport_layer/406_udp_user_datagram_protocol_connectionless_fast/) 기반의 [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/) 없는 스트리밍이라 게임(Web Game)의 조이패드 조작 [신호](/knowledge-base/studynote/02_operating_system/02_process_thread/130_signal/)를 던지는 데도 최고의 무기다.

### 2. 시그널링 (Signaling)의 역설: "알아서 만나라고 할 수 없음"

- WebRTC 스펙 안에는 놀랍게도 '서로 어떻게 처음 만날 것인가(Signaling)'에 대한 표준 규격이 아예 없다(백지상태).
- **왜?**: W3C는 "네트워크 길 뚫는([P2P](/knowledge-base/studynote/03_network/18_optical_nextgen_automation/916_p2p_peer_to_peer_networking_super_node_gnutella/)) 것만 우리가 코어(C++)로 잘 짜줄게. 너네 둘이 카톡으로 만나서 IP 주소를 교환하든, [웹소켓](/knowledge-base/studynote/03_network/19_frequent_topics_terms/975_websocket_full_duplex_realtime_http_upgrade/)([WebSocket](/knowledge-base/studynote/03_network/09_application_layer_web_email/480_websocket_full_duplex/))으로 교환하든, [SIP](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/535_system_in_package/) [프로토콜](/knowledge-base/studynote/03_network/06_network_layer_ip/295_protocol_field_tcp_udp_icmp/)로 교환하든 그건 비즈니스 로직이니까 니들 맘대로 짜!" 라며 완전한 자율성(관심사의 분리)을 던져버린 것이다.
- **실무**: 그래서 개발자는 무조건 Node.js([Socket](/knowledge-base/studynote/02_operating_system/02_process_thread/125_socket/).io)를 써서 백엔드에 시그널링 방(Room) 서버를 짜야 한다. 앨리스와 밥이 이 [웹소켓](/knowledge-base/studynote/03_network/19_frequent_topics_terms/975_websocket_full_duplex_realtime_http_upgrade/) 방에 들어와서 "내 해상도는 1080p고 코덱은 VP8이야([SDP](/knowledge-base/studynote/09_security/01_intro_principles/048_sdp/) 교환)"라는 명세서를 서로 텍스트 채팅 치듯 주고받은(Offer/Answer) 뒤에야 비로소 P2P의 불꽃이 점화된다.
...
| 개념 | 연결 포인트 |
|:---|:---|
| IPTV [멀티캐스트](/knowledge-base/studynote/03_network/06_network_layer_ip/298_ip_classes_a_b_c_d_multicast_e_experimental/) 전송 | 현재 개념이 등장하기 전에 갖춰야 할 배경이나 인접 선행 개념이다. |
...
| [CDN](/knowledge-base/studynote/03_network/09_application_layer_web_email/506_cdn_content_delivery_network_edge_caching/) | 현재 개념이 확장되거나 적용 단계로 이어질 때 자주 함께 언급된다. |
