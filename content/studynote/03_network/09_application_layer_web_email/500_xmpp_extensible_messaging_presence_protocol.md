...
> 2. **가치**: XMPP를 이해하면 응답 시간과 [호환성](/knowledge-base/studynote/04_software_engineering/06_software_architecture/344_compatibility_usability/) 사이의 균형을 더 정확히 볼 수 있다.
...
- **개념**: XMPP는 처음에 'Jabber(재버)'라는 [오픈소스](/knowledge-base/studynote/12_it_management/05_security_compliance/191_oss_license_compliance/) 프로젝트로 시작되었다. 메시징과 프레즌스(상태 정보)를 교환하기 위한 XML 기반의 실시간 통신 [프로토콜](/knowledge-base/studynote/03_network/06_network_layer_ip/295_protocol_field_tcp_udp_icmp/)이다. 클라이언트 ➔ 서버 통신뿐만 아니라, 이메일 시스템([SMTP](/knowledge-base/studynote/03_network/09_application_layer_web_email/488_smtp_simple_mail_transfer_protocol/))처럼 서버 ➔ 서버(S2S) 간의 [분산](/knowledge-base/studynote/08_algorithm_stats/08_stats/136_variance/)형 [라우팅](/knowledge-base/studynote/03_network/07_network_layer_routing/339_routing_overview_best_path_selection/) 통신 구조를 기본 뼈대로 탑재했다.
...
- 이 열린 대문 안으로 `<message>` (채팅), `<presence>` (온라인 상태), `<iq>` (정보 [쿼리](/knowledge-base/studynote/10_ai/04_ai_ops_ethics/298_qkv_attention/)) 라는 3가지 핵심 스탠자(Stanza, 조각)들을 로그인 내내 쉴 새 없이 마구 던져 넣는다.
- 유저가 로그아웃 버튼을 누르고 앱을 끄는 찰나의 순간에만 최종적으로 `</stream:stream>` 닫기 태그를 쏘고 [파이프](/knowledge-base/studynote/02_operating_system/02_process_thread/123_pipe/)를 끊는다.
- 이것은 [HTTP](/knowledge-base/studynote/03_network/09_application_layer_web_email/461_http_stateless_connection_oriented/) 통신의 단발성([Stateless](/knowledge-base/studynote/15_devops_sre/05_devsecops/239_stateless_redis/)) 한계를, 영구적인 스트림(Stateful)으로 흉내 내기 위해 XML 파서(Parser)의 무한 [파이프](/knowledge-base/studynote/02_operating_system/02_process_thread/123_pipe/)라인 우회술을 동원한 눈물겨운 발명품이다.
...
- **XEP (XMPP Extension Protocols)**: XMPP가 기본 기능이 하도 허접해서(그룹 채팅도 없음, [파일](/knowledge-base/studynote/02_operating_system/09_file_system/501_file_definition_logical_record/) 전송도 없음), 전 세계 [오픈소스](/knowledge-base/studynote/12_it_management/05_security_compliance/191_oss_license_compliance/) 해커들이 "이렇게 덧붙이자!"고 미친 듯이 쏟아낸 수백 개의 패치(확장팩) 규격. MUC(그룹챗 XEP-0045) 등이 대표적이다.
...
| 개념 | 연결 포인트 |
|:---|:---|
| [BOSH](/knowledge-base/studynote/03_network/09_application_layer_web_email/499_bosh_bidirectional_streams_over_synchronous_http/) | 현재 개념이 등장하기 전에 갖춰야 할 배경이나 인접 선행 개념이다. |
...
| [SIP](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/535_system_in_package/) | 현재 개념이 확장되거나 적용 단계로 이어질 때 자주 함께 언급된다. |
...
<- **이전**: [499. BOSH (Bidirectional-streams Over Synchronous HTTP)](/knowledge-base/studynote/03_network/09_application_layer_web_email/499_bosh_bidirectional_streams_over_synchronous_http/)
**다음**: [501. SIP (Session Initiation Protocol)](/knowledge-base/studynote/03_network/09_application_layer_web_email/501_sip_session_initiation_protocol_voip/) ->
