...
> 1. **본질**: 평행 2선식 케이블은 일정한 간격으로 나란히 배치된 두 가닥의 도선으로 구성된 가장 원시적인 형태의 전기 [신호](/knowledge-base/studynote/02_operating_system/02_process_thread/130_signal/) 유도 [매체](/knowledge-base/studynote/03_network/03_physical_layer_media/121_transmission_media_guided_unguided/)이다.
> 2. **가치**: 특정 대역(주로 300옴의 VHF/UHF TV 수신)에서 균일한 [임피던스](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/004_impedance/)를 제공하며 저렴하게 [신호](/knowledge-base/studynote/02_operating_system/02_process_thread/130_signal/)를 전송할 수 있었으나, 외부 간섭(EMI)에 극도로 취약한 치명적 단점을 지닌다.
> 3. **판단 포인트**: 이 케이블이 가진 노이즈 방어 한계를 극복하기 위해 등장한 기술이 바로 도선을 꼬아 만든 '꼬임 쌍선(Twisted Pair)'이며, 이는 현대 [이더넷](/knowledge-base/studynote/03_network/05_lan_wan_l2_devices/230_ethernet_structure_and_principles_ieee_802_3/) 물리 계층 진화의 출발점이 되었다.
...
평행 2선식 케이블은 물리적으로 단순해 보이지만, 전파가 도선을 따라 흐르는 도파관(Waveguide) 역할을 하므로 [임피던스](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/004_impedance/) 제어가 생명이다.
...
| **EMI 차단 원리** | 사실상 방어 기재 없음 | 외부 [도체](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/008_conductor/)가 쉴드 역할 (물리적 차단) | 꼬임을 통한 노이즈 균등 흡수 및 상쇄 |
...
- **📢 섹션 요약 비유**: 마치 110V 둥근 돼지코 전자기기를 220V 납작 콘센트에 무작정 꽂으면 터지듯이, 평행 2선식을 타 [매체](/knowledge-base/studynote/03_network/03_physical_layer_media/121_transmission_media_guided_unguided/)와 섞을 때는 반드시 중간에서 번역 역할을 하는 트랜스(변압기)를 끼워 넣어야만 시스템이 타지 않습니다.

---

## Ⅴ. 기대효과 및 결론

평행 2선식 케이블은 IT 및 정보통신 인프라 관점에서는 완전히 수명을 다한 사양 기술이다. [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 통신은 [UTP](/knowledge-base/studynote/03_network/03_physical_layer_media/124_unshielded_twisted_pair/)(꼬임 쌍선)와 광케이블(Optical Fiber)로 완벽히 대체되었다.
...
| 개념 | 연결 포인트 |
|:---|:---|
| 꼬임 쌍선 (Twisted Pair) | 평행 2선식의 외부 간섭 취약성을 두 선을 꼬아버림(Twist)으로써 노이즈를 상쇄시킨 진화형 [매체](/knowledge-base/studynote/03_network/03_physical_layer_media/121_transmission_media_guided_unguided/). |
| 발룬 (Balun) | Balanced(평행선)와 Unbalanced(동축선) 사이의 전기적 특성과 [임피던스](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/004_impedance/)를 일치시켜주는 정합 변환기. |
| 차동 [신호](/knowledge-base/studynote/02_operating_system/02_process_thread/130_signal/) (Differential Signaling) | 두 선에 반대 위상의 [신호](/knowledge-base/studynote/02_operating_system/02_process_thread/130_signal/)를 보내 수신 측에서 차이를 구함으로써 공통 모드 노이즈를 제거하는 전송 기법. |
| 특성 [임피던스](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/004_impedance/) (Characteristic [Impedance](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/004_impedance/)) | 교류 [신호](/knowledge-base/studynote/02_operating_system/02_process_thread/130_signal/)가 선로를 지날 때 느끼는 [저항](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/003_resistance/) 성분으로, 매질 구조에 의해 고정되는 물리량. |
| [누화](/knowledge-base/studynote/03_network/01_data_communication/030_누화_크로스토크/) ([Crosstalk](/knowledge-base/studynote/03_network/01_data_communication/030_누화_크로스토크/)) | 나란히 달리는 두 선로 사이에서 전자기장 유도 현상에 의해 [신호](/knowledge-base/studynote/02_operating_system/02_process_thread/130_signal/)가 서로 간섭하는 현상. |
...
2. 그런데 두 선이 나란히만 있다 보니, 밖에서 나쁜 전자파 공격(간섭)이 오면 한쪽 선만 더 많이 맞아서 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)가 망가지는 병에 잘 걸렸어요.
3. 그래서 현대에는 이 선들을 꽈배기처럼 꼬아서 공격을 골고루 분산시켜 방어하는 '랜선([UTP](/knowledge-base/studynote/03_network/03_physical_layer_media/124_unshielded_twisted_pair/))'으로 발전하면서 박물관으로 가게 되었답니다!
