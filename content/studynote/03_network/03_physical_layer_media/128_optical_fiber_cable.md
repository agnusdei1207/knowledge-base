...
> 1. **본질**: 광섬유 케이블은 전기 [신호](/knowledge-base/studynote/02_operating_system/02_process_thread/130_signal/) 대신 '빛(레이저/[LED](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/013_led/))'을 이용하여 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 전송하며, [굴절률](/knowledge-base/studynote/03_network/03_physical_layer_media/129_refractive_index_tir/)이 높은 코어(Core)와 [굴절률](/knowledge-base/studynote/03_network/03_physical_layer_media/129_refractive_index_tir/)이 낮은 클래딩(Cladding) 구조를 통해 전반사를 유도한다.
> 2. **가치**: 구리선([UTP](/knowledge-base/studynote/03_network/03_physical_layer_media/124_unshielded_twisted_pair/), 동축)이 가지는 전자기 간섭(EMI)과 장거리 [신호](/knowledge-base/studynote/02_operating_system/02_process_thread/130_signal/) 감쇠의 한계를 완전히 극복하여, 수십 km에서 수천 km(해저)까지 테라급(Tbps) [대역폭](/knowledge-base/studynote/01_computer_architecture/03_architecture_basics_performance/140_bandwidth/) 전송을 가능하게 한다.
> 3. **판단 포인트**: WDM (파장 분할 [다중화](/knowledge-base/studynote/03_network/02_multiplexing_multiple_access/071_다중화_Multiplexing/)) 기술과 결합하여 한 가닥의 광섬유에 여러 색깔의 빛을 동시에 실어 보내는 현대 클라우드 및 [5G](/knowledge-base/studynote/07_enterprise_systems/09_digital_transformation/418_5g_embb_urllc_mmtc_slicing/)/[6G](/knowledge-base/studynote/07_enterprise_systems/09_digital_transformation/419_6g_ntn_thz_ris_next_gen/) 이동통신 백본망의 심장 역할을 한다.
...
- **📢 섹션 요약 비유**: 일반 도로(구리선)는 빗물과 차병목([저항](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/003_resistance/)/간섭) 때문에 멀리 가기 힘들지만, 지하 진공 튜브(광섬유)를 뚫어 총알(빛)을 쏘면 날씨나 장애물의 방해 없이 순식간에 지구 반대편까지 꽂히는 것과 같습니다.

---

## Ⅱ. 아키텍처 및 핵심 원리

광섬유는 빛을 가두고 [보호](/knowledge-base/studynote/02_operating_system/10_security/571_protection_vs_security/)하기 위해 정밀하게 층이 나뉜 구조를 가진다. 내부에서 밖으로 갈수록 [굴절률](/knowledge-base/studynote/03_network/03_physical_layer_media/129_refractive_index_tir/) 조작층과 물리적 [보호](/knowledge-base/studynote/02_operating_system/10_security/571_protection_vs_security/)층으로 구분된다.
...
현대 통신망에서 절대다수를 차지하는 다중 모드(MMF)와 단일 모드([SMF](/knowledge-base/studynote/03_network/15_nextgen_communication_architecture/771_smf_upf_session_management_user_plane/))의 특성을 비교한 매트릭스이다.

| 특성 비교 | 단일 모드 광섬유 ([SMF](/knowledge-base/studynote/03_network/15_nextgen_communication_architecture/771_smf_upf_session_management_user_plane/)) | 다중 모드 광섬유 (MMF) | 의사결정 판단 포인트 |
|:---|:---|:---|:---|
...
- **📢 섹션 요약 비유**: 좁은 레이저 터널([SMF](/knowledge-base/studynote/03_network/15_nextgen_communication_architecture/771_smf_upf_session_management_user_plane/))은 한 줄기로 직진해서 먼 도시까지 쏠 수 있지만 쏘는 총이 비싸고, 넓은 [파이프](/knowledge-base/studynote/02_operating_system/02_process_thread/123_pipe/)(MMF)는 손전등으로 대충 쏴도 되지만 벽에 이리저리 튕기며 속도가 섞여서 가까운 건물까지만 보내는 것과 같습니다.
...
- **📢 섹션 요약 비유**: 두꺼운 호스(MMF)에서 얇은 빨대([SMF](/knowledge-base/studynote/03_network/15_nextgen_communication_architecture/771_smf_upf_session_management_user_plane/))로 물을 억지로 밀어 넣으면 이음새에서 물이 다 터져 나오듯, 종류가 다른 광케이블을 섞어 쓰면 통신망이 마비되는 치명적인 결과가 발생합니다.
...
- **📢 섹션 요약 비유**: 모래(실리카)로 만든 얇은 유리실 두 겹(코어와 클래딩)이 전 세계의 모든 영화와 유튜브, 이메일 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 찰나의 순간에 바다 건너로 실어 나르는 21세기 최고의 마법 지팡이인 셈입니다.
...
| [굴절률](/knowledge-base/studynote/03_network/03_physical_layer_media/129_refractive_index_tir/) ([Refractive Index](/knowledge-base/studynote/03_network/03_physical_layer_media/129_refractive_index_tir/)) | 진공 중 빛의 속도와 특정 매질 내 빛의 속도 비율을 나타내며, 코어가 클래딩보다 커야 함 |
| WDM (파장 분할 [다중화](/knowledge-base/studynote/03_network/02_multiplexing_multiple_access/071_다중화_Multiplexing/)) | 하나의 광섬유 코어에 서로 다른 여러 색깔(파장)의 레이저를 겹쳐 쏘아 전송량을 수십 배 늘리는 기술 |
| 모드 [분산](/knowledge-base/studynote/08_algorithm_stats/08_stats/136_variance/) (Modal [Dispersion](/knowledge-base/studynote/03_network/03_physical_layer_media/133_dispersion_mode_chromatic/)) | 다중 모드 광섬유에서 여러 경로로 들어온 빛이 도착 지점에 도달하는 시간이 달라 펄스가 퍼지는 현상 |
...
1. 광섬유는 머리카락처럼 얇은 투명한 유리실인데, 이 안으로 전기 대신 반짝반짝 빛을 쏘아서 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 보내요.
...
← **이전**: [127. 동축 케이블 (Coaxial Cable)](/knowledge-base/studynote/03_network/03_physical_layer_media/127_coaxial_cable/)
**다음**: [129. 굴절률 (Refractive Index), 전반사 (Total Internal Reflection)](/knowledge-base/studynote/03_network/03_physical_layer_media/129_refractive_index_tir/) →
