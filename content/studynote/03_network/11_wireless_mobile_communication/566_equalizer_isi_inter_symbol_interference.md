...
- **개념**: 등화기는 송신단에서 보낸 원래의 전파 파형이 무선 채널(공기)을 통과하며 뚱뚱하게 퍼지고 일그러졌을 때, 수신단(스마트폰 [모뎀](/knowledge-base/studynote/03_network/03_physical_layer_media/146_modem_modulator_demodulator/))에서 이 왜곡된 파형에 '채널 특성의 역함수(Inverse Channel Response)' 필터를 통과시켜 원래의 날씬한 펄스 형태로 깎아내는 [신호](/knowledge-base/studynote/02_operating_system/02_process_thread/130_signal/) 처리 기술이다.
...
|:---|:---|:---|:---|
| **선형 등화기 (ZF)** | Zero-Forcing (영강제) Equalizer | 단순히 수신된 [신호](/knowledge-base/studynote/02_operating_system/02_process_thread/130_signal/)에 채널 역함수($H^{-1}$)를 곱해서 ISI를 수학적으로 완벽히 0으로 만들어 버림 | ISI는 지워지지만, [신호](/knowledge-base/studynote/02_operating_system/02_process_thread/130_signal/)에 끼어있던 진짜 노이즈(백색 잡음)까지 미친 듯이 뻥튀기(증폭)되어 결국 쓸모없어짐 |
| **선형 등화기 (MMSE)**| Minimum Mean [Square](/knowledge-base/studynote/04_software_engineering/06_software_architecture/341_iso_iec_25010/) Error | ISI를 0으로 만드는 고집을 버리고, ISI와 백색 잡음이 더해진 총 에러의 양이 '최소화'되는 타협점 필터를 찾음 | 여전히 심한 [페이딩](/knowledge-base/studynote/03_network/03_physical_layer_media/167_fading_large_scale_small_scale/) 구역에서는 노이즈 증폭 문제를 완벽히 피하지 못함 |
| **비선형 등화기 (DFE)**| Decision Feedback Equalizer | 앞에서 이미 결정(Decision)이 끝난 깨끗한 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 피드백으로 되돌려, 뒷단의 [신호](/knowledge-base/studynote/02_operating_system/02_process_thread/130_signal/)에 끼어있는 꼬리(ISI)를 정교하게 빼줌 | **에러 전파(Error Propagation)**: 만약 앞 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 판단이 틀렸다면, 그 틀린 값을 뒤에 빼주게 되어 연쇄적으로 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)가 다 망가짐 |
...
[페이딩](/knowledge-base/studynote/03_network/03_physical_layer_media/167_fading_large_scale_small_scale/)에 대처하는 이동통신의 철학은 3세대에 걸쳐 극적으로 변했다.
...
- **📢 섹션 요약 비유**: 등화기는 지독한 모자이크 블러(Blur) 처리가 된 화면(메아리로 뭉개진 파동)을 역추적 수학 [알고리즘](/knowledge-base/studynote/08_algorithm_stats/01_basics/001_algorithm_definition/)을 윙~ 돌려서 원래의 깨끗하고 선명한 HD 화면으로 실시간 복원해 내는 최첨단 첩보 기관의 영상 복원 컴퓨터(DSP)입니다.
...
| 개념 | 연결 포인트 |
|:---|:---|
| [레이크 수신기](/knowledge-base/studynote/03_network/11_wireless_mobile_communication/565_rake_receiver_multipath_fading_cdma/) | 현재 개념이 등장하기 전에 갖춰야 할 배경이나 인접 선행 개념이다. |
...
| [스마트 안테나](/knowledge-base/studynote/03_network/03_physical_layer_media/177_smart_antenna_phased_array/) | 현재 개념이 확장되거나 적용 단계로 이어질 때 자주 함께 언급된다. |
...
← **이전**: [565. 레이크 수신기 (Rake Receiver)](/knowledge-base/studynote/03_network/11_wireless_mobile_communication/565_rake_receiver_multipath_fading_cdma/)
**다음**: [567. 스마트 안테나 (Smart Antenna)](/knowledge-base/studynote/03_network/11_wireless_mobile_communication/567_smart_antenna_beamforming_concept/) →
