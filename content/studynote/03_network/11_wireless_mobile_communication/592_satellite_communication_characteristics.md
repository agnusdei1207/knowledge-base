...
|:---|:---|:---|:---|
| **L 밴드 (1~2 GHz)** | 파장이 김. 뚫는 힘이 세고 날씨 영향 덜 받음. | [대역폭](/knowledge-base/studynote/01_computer_architecture/03_architecture_basics_performance/140_bandwidth/)이 좁아서 속도가 엄청 느림. [안테나](/knowledge-base/studynote/03_network/03_physical_layer_media/171_antenna_basic_dipole_resonance/)를 엄청 크게 만들어야 함. | **GPS(항법 위성)**, 바다 위 선박의 저속 위성 전화(이리듐). |
| **C 밴드 (4~8 GHz)** | 밸런스형. 빗방울을 잘 뚫고 속도도 적당함. | 지상의 Wi-Fi(5GHz) 및 5G망과 주파수가 겹쳐서 간섭(잡음)이 심함. | 전통적인 TV 위성 방송, 아프리카 오지 대륙 간 릴레이망. |
| **Ku 밴드 (12~18 GHz)** | 높은 주파수. [대역폭](/knowledge-base/studynote/01_computer_architecture/03_architecture_basics_performance/140_bandwidth/)이 엄청 넓어 기가급 쾌속. | 파장이 짧아서 **비가 오거나 눈이 오면 전파가 빗방울에 흡수되어 인터넷이 뚝뚝 끊김 (Rain Fade 현상).** | **Starlink(스타링크)** 위성 인터넷, 이동형 위성 중계차(SNG). |
| **Ka 밴드 (26~40 GHz)** | 극초고주파. 미친듯한 [초고속](/knowledge-base/studynote/06_ict_convergence/02_iot_mobility/148_5g_embb_urllc_mmtc/) [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 전송. | 빗방울을 넘어서 구름이나 안개만 껴도 전파가 막혀버리는 치명적 예민함. | 차세대 [6G](/knowledge-base/studynote/07_enterprise_systems/09_digital_transformation/419_6g_ntn_thz_ris_next_gen/) 위성 통신망, 군사 정찰 위성의 초정밀 고화질 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 덤핑. |
...
- **📢 섹션 요약 비유**: 맑은 날 위성 통신은 종이비행기에 무거운 편지(고속 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/))를 가득 담아 던지는 겁니다. 날씨가 맑으면 잘 날아갑니다. 하지만 비바람이 불면 종이비행기가 다 찢어지죠(Rain Fade). 똑똑한 위성은 비가 오면 무거운 편지를 다 버리고, 아주 짧은 한 줄짜리 쪽지(저속 BPSK [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/))만 돌멩이에 묶어서 어떻게든 땅에 떨어지게 만드는 생존 룰을 켭니다.

---

## Ⅲ. 비교 및 연결

위성 통신 특징을 볼 때는 앞뒤 개념과의 경계를 함께 봐야 전체 흐름이 선명해진다. C-V2X가 기반 조건을 만든다면, 위성 통신 특징은 그 위에서 핵심 메커니즘을 구현하고, [정지 궤도 위성](/knowledge-base/studynote/03_network/11_wireless_mobile_communication/593_geo_geostationary_earth_orbit_satellite/)은 이를 더 확장된 적용 단계로 연결한다. 따라서 단일 정의보다 스펙트럼 효율과 이동성에 어떤 차이를 만드는지 비교하는 것이 중요하다.

| 관점 | 선행 개념 | 현재 개념 | 확장 개념 |
|:---|:---|:---|:---|
| 초점 | C-V2X의 기반 정리 | 위성 통신 특징의 핵심 동작 | [정지 궤도 위성](/knowledge-base/studynote/03_network/11_wireless_mobile_communication/593_geo_geostationary_earth_orbit_satellite/)의 확장 적용 |
...
- **DVB-S2 (Digital Video Broadcasting - Satellite)**: 일반 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 통신을 넘어, 여러분 집의 스카이라이프 접시 [안테나](/knowledge-base/studynote/03_network/03_physical_layer_media/171_antenna_basic_dipole_resonance/)로 수백 개의 HD TV 채널을 쏟아부어 주는 세계에서 가장 완벽하고 효율적인 위성 방송 통신 [프로토콜](/knowledge-base/studynote/03_network/06_network_layer_ip/295_protocol_field_tcp_udp_icmp/).
...
- **📢 섹션 요약 비유**: 지상의 인터넷([5G](/knowledge-base/studynote/07_enterprise_systems/09_digital_transformation/418_5g_embb_urllc_mmtc_slicing/), 랜선)은 택배 차입니다. 도로가 없거나 낭떠러지가 있으면 배달을 못 갑니다. 위성 통신은 성층권에 뜬 초거대 헬리콥터입니다. 땅에 길이 있든 없든, 바다든 사막이든 상관없이 하늘에서 낙하산으로 전 세계 수십억 명의 머리 위에 편지를 똑같이, 동시에 쏟아부어 줄 수 있는 신(God)의 배달 시스템입니다. 단, 고도가 너무 높아 주문을 하고 물건을 받기까지 시간([지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/))은 조금 걸립니다.
...
| 개념 | 연결 포인트 |
|:---|:---|
| [C-V2X](/knowledge-base/studynote/06_ict_convergence/02_iot_mobility/143_c_v2x_cellular_based_communication/) | 현재 개념이 등장하기 전에 갖춰야 할 배경이나 인접 선행 개념이다. |
...
| [정지 궤도 위성](/knowledge-base/studynote/03_network/11_wireless_mobile_communication/593_geo_geostationary_earth_orbit_satellite/) | 현재 개념이 확장되거나 적용 단계로 이어질 때 자주 함께 언급된다. |
...
← **이전**: [591. C-V2X (Cellular V2X)](/knowledge-base/studynote/03_network/11_wireless_mobile_communication/591_c_v2x_cellular_v2x_3gpp/)
**다음**: [593. 정지 궤도 위성 (GEO)](/knowledge-base/studynote/03_network/11_wireless_mobile_communication/593_geo_geostationary_earth_orbit_satellite/) →
