...
### 1. 절대 불변의 아킬레스건: 500ms 왕복 [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/) ([Propagation Delay](/knowledge-base/studynote/03_network/01_data_communication/016_전파_지연/))

GEO 위성의 장점이 '높은 고도'라면, 그것은 동시에 이 아키텍처를 죽이는 가장 치명적인 독약이다. 물리 법칙상 전파는 빛의 속도(300,000km/s)를 넘을 수 없다.

| 통신 흐름 | 전파 이동 거리 | 편도 및 왕복(Ping) [지연 시간](/knowledge-base/studynote/01_computer_architecture/03_architecture_basics_performance/141_latency/) 계산 | 
|:---|:---|:---|
...
- **📢 섹션 요약 비유**: GEO 위성은 엄청나게 높은 사다리차 위에 올라가서 확성기로 소리치는 아저씨입니다. 동네(지구) 전체 사람들이 확성기 소리를 동시에 다 들을 수 있어서 방송(Broadcast)에는 최고입니다. 그런데 아저씨랑 서로 질문하고 대답하려면, 소리가 허공을 왔다 갔다 하느라 한 마디 할 때마다 0.5초씩 침묵이 흐르는 아주 답답한 대화([전파 지연](/knowledge-base/studynote/03_network/01_data_communication/016_전파_지연/))를 참아야만 합니다.

---

## Ⅲ. 비교 및 연결

정지 궤도 위성을 볼 때는 앞뒤 개념과의 경계를 함께 봐야 전체 흐름이 선명해진다. [위성 통신](/knowledge-base/studynote/03_network/11_wireless_mobile_communication/592_satellite_communication_characteristics/) 특징이 기반 조건을 만든다면, 정지 궤도 위성은 그 위에서 핵심 메커니즘을 구현하고, [중궤도 위성](/knowledge-base/studynote/03_network/11_wireless_mobile_communication/594_meo_medium_earth_orbit_gps/)은 이를 더 확장된 적용 단계로 연결한다. 따라서 단일 정의보다 스펙트럼 효율과 이동성에 어떤 차이를 만드는지 비교하는 것이 중요하다.

| 관점 | 선행 개념 | 현재 개념 | 확장 개념 |
|:---|:---|:---|:---|
| 초점 | [위성 통신](/knowledge-base/studynote/03_network/11_wireless_mobile_communication/592_satellite_communication_characteristics/) 특징의 기반 정리 | 정지 궤도 위성의 핵심 동작 | [중궤도 위성](/knowledge-base/studynote/03_network/11_wireless_mobile_communication/594_meo_medium_earth_orbit_gps/)의 확장 적용 |
...
- **📢 섹션 요약 비유**: GEO 위성은 엄청나게 높은 빌딩 옥상에 고정해 둔 거대한 거울입니다. 땅에서 플래시를 비추면 거울은 한 번도 안 움직이고 도시 전체를 덮어주죠(편리함과 가성비). 반면 나중에 배울 [LEO](/knowledge-base/studynote/03_network/11_wireless_mobile_communication/595_leo_low_earth_orbit_starlink_6g/)(저궤도) 위성은 낮게 떠서 엄청 빨리 날아다니는 드론 거울 수백 대입니다. 드론을 일일이 조준([안테나](/knowledge-base/studynote/03_network/03_physical_layer_media/171_antenna_basic_dipole_resonance/) 추적)해야 해서 비싸고 피곤하지만, 거리가 가까워서 빛이 눈 깜짝할 새(초저지연) 도착하는 완전히 다른 특성의 게임입니다.
...
| 개념 | 연결 포인트 |
|:---|:---|
| [위성 통신](/knowledge-base/studynote/03_network/11_wireless_mobile_communication/592_satellite_communication_characteristics/) 특징 | 현재 개념이 등장하기 전에 갖춰야 할 배경이나 인접 선행 개념이다. |
...
| [중궤도 위성](/knowledge-base/studynote/03_network/11_wireless_mobile_communication/594_meo_medium_earth_orbit_gps/) | 현재 개념이 확장되거나 적용 단계로 이어질 때 자주 함께 언급된다. |
...
1. 거실 천장에 매달린 전구(GEO 정지궤도 위성)는 움직이지 않기 때문에, 우리가 손전등([안테나](/knowledge-base/studynote/03_network/03_physical_layer_media/171_antenna_basic_dipole_resonance/))을 한 곳으로만 계속 고정해두면 언제나 밝은 불빛(인터넷)을 받을 수 있어서 참 편해요!
...
← **이전**: [592. 위성 통신 (Satellite Comm.) 특징](/knowledge-base/studynote/03_network/11_wireless_mobile_communication/592_satellite_communication_characteristics/)
**다음**: [594. 중궤도 위성 (MEO) (GPS, 항법시스템)](/knowledge-base/studynote/03_network/11_wireless_mobile_communication/594_meo_medium_earth_orbit_gps/) →
