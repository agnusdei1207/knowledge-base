...
4. 빛의 속도($3 \times [10](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/489_raid_10_hybrid/)^8 m/s$) $\times$ (왕복 시간 / 2) = **정확한 물리적 절대 거리 계산 완성**.
   - **보안의 핵 (Secure Ranging)**: 빛보다 빠른 것은 우주에 없다. 해커가 중간에서 전파를 가로채 증폭시켜 릴레이(Relay) 공격을 하더라도, 기계를 거치면서 발생하는 0.000001초의 전자회로 '딜레이'가 스톱워치에 그대로 찍힌다. [UWB](/knowledge-base/studynote/06_ict_convergence/02_iot_mobility/160_uwb_ultra_wideband/) 칩셋은 "어? 2미터 거린데 시간이 너무 늦게 왔네? 이거 해커가 중간에서 릴레이 장난친 거다!"라며 문을 절대 열어주지 않는다. 물리 법칙 기반의 궁극의 방패다.
...
2. UWB가 추가하는 복잡도와 운영 이득이 균형을 이루는지 [확인](/knowledge-base/studynote/04_software_engineering/12_testing_maintenance/396_validation/)한다.
3. 도입 후에는 인접 기술인 무선 충전 전송 원리와의 연계 방식을 함께 검증한다.

### [안티패턴](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/128_water_scrum_fall_anti_pattern/)

- UWB의 장점만 보고 트래픽 패턴이나 운영 비용을 무시한 채 과도 도입하는 설계
- GPS 삼각 측량 / 오차 개선 기법와의 경계를 정리하지 않아 중복 투자나 [정책](/knowledge-base/studynote/10_ai/02_dl_architecture_new/164_policy/) 충돌을 만드는 설계
...
- **📢 섹션 요약 비유**: 일반 통신(와이파이)은 붓으로 도화지에 굵고 진하게 글씨([데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/))를 길게 쓰는 겁니다. 붓을 오래 대고 있어야 해서 정밀하지 않죠. UWB는 도화지에 엄청나게 뾰족한 송곳으로 구멍을 "쾅!" 하고 0.001초 만에 뚫고 지나가는 겁니다. 글씨를 많이 쓸 순 없지만, 구멍이 너무 작고 뾰족해서 그 구멍이 뚫린 시간과 각도를 1밀리미터 오차 없이 현미경으로 완벽하게 찾아낼 수 있는 극강의 레이저 조각기입니다.
...
1. 숨바꼭질할 때 친구가 "야야~" 하고 길게 소리치면([블루투스](/knowledge-base/studynote/03_network/12_iot_wpan_edge/605_bluetooth_ieee_802_15_1_piconet_scatternet/)), 소리가 벽에 웅웅 울려서(메아리) 친구가 방 어디에 숨었는지 정확히 찾기가 힘들어요 (오차 3미터).
2. [UWB](/knowledge-base/studynote/06_ict_convergence/02_iot_mobility/160_uwb_ultra_wideband/)(초광대역)는 친구가 방구석에서 레이저 빛을 0.001초 만에 "번쩍!" 하고 엄청 짧게 쏘는 것과 같아요! 메아리가 생기기도 전에 내 눈에 정확한 빛이 꽂히죠.
...
← **이전**: [597. 블루투스 (Bluetooth)와 BLE](/knowledge-base/studynote/03_network/11_wireless_mobile_communication/597_gps_triangulation_dgps_rtk/)
**다음**: [599. 무선 충전 전송 원리 (자기 유도형 WPC Qi, 자기 공명형 A4WP)](/knowledge-base/studynote/03_network/11_wireless_mobile_communication/599_wireless_charging_wpc_qi_a4wp_resonance/) →
