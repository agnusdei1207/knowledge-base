...
> 1. **본질**: 트래픽 쉐이핑 / 폴리싱은 [[339_routing_overview_best_path_selection|라우팅]]과 경로 제어에서 핵심 동작과 제약을 이해하게 해 주는 개념이다.
...
- **개념**: [[090_service_kubernetes_network_load_balancing|서비스]] 수준 계약([[085_sla|SLA]])에 따라 네트워크의 [[140_bandwidth|대역폭]] 사용량을 제어하기 위해, 초과된 트래픽을 폐기(Policing)하거나 [[454_buffering|버퍼링]](Shaping)하여 트래픽의 프로파일(모양)을 맞추는 [[388_qos_quality_of_service_best_effort_intserv_diffserv|QoS]] [[140_bandwidth|대역폭]] 제어 메커니즘.
...
### 3. 쉐이핑(Shaping)의 치명적 부작용 ([[015_지연_데이터_관점|지연]]과 [[591_buffer_overflow|버퍼 오버플로우]])
쉐이핑은 안 버리고 버퍼(RAM)에 담아두니까 무조건 좋은 거 아닐까? 아니다.
- **[[015_지연_데이터_관점|지연]](Delay) 발생**: 댐에 물을 가둬두니 당연히 패킷이 목적지에 늦게 도착한다. **실시간 음성(VoIP)에 쉐이핑을 걸면 통화가 다 [[015_지연_데이터_관점|지연]]되어 쓰레기가 된다**. (VoIP는 무조건 폴리싱이나 LLQ를 태워야 한다).
...
- **📢 섹션 요약 비유**: ** 실무에서 쉐이핑과 폴리싱은 창과 방패입니다. 고객사(내 라우터)는 패킷이 통신사 방패에 부딪혀 깨지는 걸 막기 위해 **"쉐이핑"**이라는 쿠션으로 패킷을 예쁘게 다듬어 던지고, 통신사([[101_isp_information_strategy_planning_4_steps|ISP]])는 계약을 어기고 들어오는 과도한 트래픽을 막기 위해 **"폴리싱"**이라는 칼을 들고 입구에서 대기합니다.

---

## Ⅲ. 비교 및 연결

트래픽 쉐이핑 / 폴리싱을 볼 때는 앞뒤 개념과의 경계를 함께 봐야 전체 흐름이 선명해진다. [[083_priority_queue|우선순위 큐]], 맞춤형 큐, WFQ, CBWF…가 기반 조건을 만든다면, 트래픽 쉐이핑 / 폴리싱은 그 위에서 핵심 메커니즘을 구현하고, Leaky Bucket / Token Buc…는 이를 더 확장된 적용 단계로 연결한다. 따라서 단일 정의보다 수렴 속도과 확장성에 어떤 차이를 만드는지 비교하는 것이 중요하다.

| 관점 | 선행 개념 | 현재 개념 | 확장 개념 |
|:---|:---|:---|:---|
| 초점 | [[083_priority_queue|우선순위 큐]], 맞춤형 큐, WFQ, CBWF…의 기반 정리 | 트래픽 쉐이핑 / 폴리싱의 핵심 동작 | Leaky Bucket / Token Buc…의 확장 적용 |
...
실무에서는 트래픽 쉐이핑 / 폴리싱을 단독 개념으로 외우기보다 어떤 병목을 줄이기 위한 선택인지 먼저 따져야 한다. 특히 [[083_priority_queue|우선순위 큐]], 맞춤형 큐, WFQ, CBWF… 수준의 기본 대책으로 충분한지, 아니면 트래픽 쉐이핑 / 폴리싱이 제공하는 메커니즘이 실제로 필요한지 구분해야 한다. 이후 확장 단계에서는 Leaky Bucket / Token Buc…와 같은 후속 기술, 자동화 체계, 표준 호환성까지 함께 검토해야 한다.

### 실무 [[435_checklist_based_testing|체크리스트]]

1. 현재 문제의 핵심이 수렴 속도 부족인지, 확장성 악화인지 먼저 분리한다.
2. 트래픽 쉐이핑 / 폴리싱가 추가하는 복잡도와 운영 이득이 균형을 이루는지 [[396_validation|확인]]한다.
3. 도입 후에는 인접 기술인 Leaky Bucket / Token Buc…와의 연계 방식을 함께 검증한다.

### [[128_water_scrum_fall_anti_pattern|안티패턴]]

- 트래픽 쉐이핑 / 폴리싱의 장점만 보고 트래픽 패턴이나 운영 비용을 무시한 채 과도 도입하는 설계
- [[083_priority_queue|우선순위 큐]], 맞춤형 큐, WFQ, CBWF…와의 경계를 정리하지 않아 중복 투자나 [[164_policy|정책]] 충돌을 만드는 설계
...
| 개념 | 연결 포인트 |
|:---|:---|
| [[083_priority_queue|우선순위 큐]], 맞춤형 큐, WFQ, CBWF… | 현재 개념이 등장하기 전에 갖춰야 할 배경이나 인접 선행 개념이다. |
...
트래픽 쉐이핑 / 폴리싱는 [[083_priority_queue|우선순위 큐]], 맞춤형 큐, WFQ, CBWF…에서 출발해 현재 메커니즘을 정교화하고, 이후 Leaky Bucket / Token Buc…와 의도 기반 [[339_routing_overview_best_path_selection|라우팅]] 같은 확장 흐름으로 이어진다고 보면 기억이 오래간다.
...
← **이전**: [[391_qos_queuing_pq_cq_wfq_cbwfq_llq|391. 우선순위 큐 (PQ), 맞춤형 큐 (CQ), WFQ, CBWFQ, LLQ]]
**다음**: [[393_leaky_bucket_token_bucket|393. Leaky Bucket / Token Bucket]] →
