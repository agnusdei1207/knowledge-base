...
- **💡 비유**: 
  ...
  - **메시지 교환**: **"우편 배달"**입니다. 거대한 이삿짐을 한 상자에 싸서 우체국([스위치](/knowledge-base/studynote/03_network/05_lan_wan_l2_devices/238_switch_operation_principles/))에 통째로 맡기면 알아서 배달해 주지만, 상자가 커서 처리하는 데 오래 걸립니다.
...
- **동작**: 이메일처럼 전체 메시지를 한 덩어리로 만든 뒤 [스위치](/knowledge-base/studynote/03_network/05_lan_wan_l2_devices/238_switch_operation_principles/)에 통째로 보낸다. [스위치](/knowledge-base/studynote/03_network/05_lan_wan_l2_devices/238_switch_operation_principles/)는 이 거대한 덩어리를 하드디스크에 다 담아놓고(Store) 다음 길을 찾아 통째로 쏜다([Forward](/knowledge-base/studynote/10_ai/03_llm_nlp/235_forward_backward_chaining/)).
- **단점**: 메시지 크기 제한이 없어서, 앞사람이 10GB짜리 동영상을 보내고 있으면 뒷사람의 1KB짜리 카톡은 10GB 전송이 끝날 때까지 [스위치](/knowledge-base/studynote/03_network/05_lan_wan_l2_devices/238_switch_operation_principles/) 안에서 하염없이 대기해야 한다. (실시간 통신 절대 불가)

### 3. 패킷 교환 (Packet Switching)
- **동작**: 메시지 교환의 딜레이를 해결하기 위해 등장했다. 10GB짜리 동영상을 1.5KB 크기의 수백만 개 패킷으로 잘게 썬다.
- **[다중화](/knowledge-base/studynote/03_network/02_multiplexing_multiple_access/071_다중화_Multiplexing/)([Multiplexing](/knowledge-base/studynote/03_network/02_multiplexing_multiple_access/071_다중화_Multiplexing/))의 기적**: 내 패킷 조각과 다른 사람의 패킷 조각들이 [스위치](/knowledge-base/studynote/03_network/05_lan_wan_l2_devices/238_switch_operation_principles/) 버퍼에서 섞여서(Interleaving) 번갈아 가며 나간다. 앞사람이 10GB를 보내도, 내 패킷은 앞사람 패킷들 사이에 낑겨서 전송되므로 딜레이 없이 실시간으로 목적지에 도착한다.
...
패킷 교환 vs 회선 교환 vs 메시지 교환을 볼 때는 앞뒤 개념과의 경계를 함께 봐야 전체 흐름이 선명해진다. AAL가 기반 조건을 만든다면, 패킷 교환 vs 회선 교환 vs 메시지 교환은 그 위에서 핵심 메커니즘을 구현하고, [데이터그램 전송 방식](/knowledge-base/studynote/03_network/05_lan_wan_l2_devices/277_datagram_transmission_connectionless_packet_switching/)은 이를 더 확장된 적용 단계로 연결한다. 따라서 단일 정의보다 스위칭 효율과 브로드캐스트 범위에 어떤 차이를 만드는지 비교하는 것이 중요하다.

| 관점 | 선행 개념 | 현재 개념 | 확장 개념 |
|:---|:---|:---|:---|
| 초점 | AAL의 기반 정리 | 패킷 교환 vs 회선 교환 vs 메시지 교환의 핵심 동작 | [데이터그램 전송 방식](/knowledge-base/studynote/03_network/05_lan_wan_l2_devices/277_datagram_transmission_connectionless_packet_switching/)의 확장 적용 |
...
실무에서는 패킷 교환 vs 회선 교환 vs 메시지 교환을 단독 개념으로 외우기보다 어떤 병목을 줄이기 위한 선택인지 먼저 따져야 한다. 특히 [AAL](/knowledge-base/studynote/03_network/05_lan_wan_l2_devices/275_aal_atm_adaptation_layer/) 수준의 기본 대책으로 충분한지, 아니면 패킷 교환 vs 회선 교환 vs 메시지 교환이 제공하는 메커니즘이 실제로 필요한지 구분해야 한다. 이후 확장 단계에서는 [데이터그램 전송 방식](/knowledge-base/studynote/03_network/05_lan_wan_l2_devices/277_datagram_transmission_connectionless_packet_switching/)와 같은 후속 기술, 자동화 체계, 표준 호환성까지 함께 검토해야 한다.

### 실무 [체크리스트](/knowledge-base/studynote/04_software_engineering/11_testing_validation/435_checklist_based_testing/)

1. 현재 문제의 핵심이 스위칭 효율 부족인지, 브로드캐스트 범위 악화인지 먼저 분리한다.
2. 패킷 교환 vs 회선 교환 vs 메시지 교환가 추가하는 복잡도와 운영 이득이 균형을 이루는지 [확인](/knowledge-base/studynote/04_software_engineering/12_testing_maintenance/396_validation/)한다.
3. 도입 후에는 인접 기술인 [데이터그램 전송 방식](/knowledge-base/studynote/03_network/05_lan_wan_l2_devices/277_datagram_transmission_connectionless_packet_switching/)와의 연계 방식을 함께 검증한다.

### [안티패턴](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/128_water_scrum_fall_anti_pattern/)

- 패킷 교환 vs 회선 교환 vs 메시지 교환의 장점만 보고 트래픽 패턴이나 운영 비용을 무시한 채 과도 도입하는 설계
- AAL와의 경계를 정리하지 않아 중복 투자나 [정책](/knowledge-base/studynote/10_ai/02_dl_architecture_new/164_policy/) 충돌을 만드는 설계
...
| 개념 | 연결 포인트 |
|:---|:---|
| [AAL](/knowledge-base/studynote/03_network/05_lan_wan_l2_devices/275_aal_atm_adaptation_layer/) | 현재 개념이 등장하기 전에 갖춰야 할 배경이나 인접 선행 개념이다. |
...
| [데이터그램 전송 방식](/knowledge-base/studynote/03_network/05_lan_wan_l2_devices/277_datagram_transmission_connectionless_packet_switching/) | 현재 개념이 확장되거나 적용 단계로 이어질 때 자주 함께 언급된다. |
...
← **이전**: [275. AAL (ATM Adaptation Layer)](/knowledge-base/studynote/03_network/05_lan_wan_l2_devices/275_aal_atm_adaptation_layer/)
**다음**: [277. 데이터그램 전송 방식 (비연결형 패킷 교환)](/knowledge-base/studynote/03_network/05_lan_wan_l2_devices/277_datagram_transmission_connectionless_packet_switching/) →
