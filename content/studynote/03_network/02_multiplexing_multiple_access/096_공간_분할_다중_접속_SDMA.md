...
## 핵심 인사이트 (3줄 요약)
> 1. **본질**: 공간 분할 [다중 접속](/knowledge-base/studynote/03_network/02_multiplexing_multiple_access/087_다중접속_Multiple_Access/) (SDMA, Space [Division](/knowledge-base/studynote/05_database/07_exam_summary/411_division_operation/) [Multiple Access](/knowledge-base/studynote/03_network/02_multiplexing_multiple_access/087_다중접속_Multiple_Access/))은 [안테나](/knowledge-base/studynote/03_network/03_physical_layer_media/171_antenna_basic_dipole_resonance/) [빔포밍](/knowledge-base/studynote/03_network/02_multiplexing_multiple_access/101_beamforming/)([Beamforming](/knowledge-base/studynote/03_network/02_multiplexing_multiple_access/101_beamforming/))을 활용해 전파를 특정 방향으로 쏘아, 동일한 주파수와 시간 슬롯을 여러 사용자가 동시에 재사용하는 [다중 접속](/knowledge-base/studynote/03_network/02_multiplexing_multiple_access/087_다중접속_Multiple_Access/) 기술이다.
> 2. **가치**: 추가적인 주파수 대역 할당 없이 셀(Cell) 수용 용량과 전체 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) [처리량](/knowledge-base/studynote/01_computer_architecture/03_architecture_basics_performance/139_throughput/)([Throughput](/knowledge-base/studynote/01_computer_architecture/03_architecture_basics_performance/139_throughput/))을 [안테나](/knowledge-base/studynote/03_network/03_physical_layer_media/171_antenna_basic_dipole_resonance/) 수에 비례하여 획기적으로 증대시키는 [5G](/knowledge-base/studynote/07_enterprise_systems/09_digital_transformation/418_5g_embb_urllc_mmtc_slicing/)/[6G](/knowledge-base/studynote/07_enterprise_systems/09_digital_transformation/419_6g_ntn_thz_ris_next_gen/) 초연결망의 근간이다.
> 3. **판단 포인트**: 공간 다중화의 이득은 사용자가 물리적으로 분산되어 있을 때만 유효하므로, 사용자가 겹치거나 고속 이동하는 환경에서는 시간/주파수 자원 분할([OFDMA](/knowledge-base/studynote/03_network/19_frequent_topics_terms/945_ofdma_orthogonal_frequency_division_multiple_access_resource_block/) 등)과 반드시 결합하여 간섭을 제어해야 한다.
...
- **📢 섹션 요약 비유**: 오케스트라의 지휘자(DSP)가 악보([CSI](/knowledge-base/studynote/12_it_management/02_itsm_itil/068_csi/))를 보고 수십 개의 악기([안테나](/knowledge-base/studynote/03_network/03_physical_layer_media/171_antenna_basic_dipole_resonance/) 소자)가 내는 소리의 크기와 박자(위상)를 정밀하게 조율하여, 관객석 특정 위치에만 완벽한 화음(빔)이 꽂히게 만드는 마법과 같다.

---

## Ⅲ. 비교 및 연결

SDMA는 자원의 한계를 '어느 축'을 잘라 해결하느냐에서 기존 [다중 접속](/knowledge-base/studynote/03_network/02_multiplexing_multiple_access/087_다중접속_Multiple_Access/) 기술과 명확한 경계가 나뉜다.

| 비교 축 | [OFDMA](/knowledge-base/studynote/03_network/19_frequent_topics_terms/945_ofdma_orthogonal_frequency_division_multiple_access_resource_block/) (직교 주파수 분할) | SDMA (공간 분할) | 시스템 적용 포인트 |
|:---|:---|:---|:---|
| **자원 분할** | 주파수 (Subcarriers) | 공간 (Spatial Beams) | [대역폭](/knowledge-base/studynote/01_computer_architecture/03_architecture_basics_performance/140_bandwidth/) 제약 극복 여부 |
| **셀 용량 증대** | 한계 도달 ([대역폭](/knowledge-base/studynote/01_computer_architecture/03_architecture_basics_performance/140_bandwidth/) 종속) | [안테나](/knowledge-base/studynote/03_network/03_physical_layer_media/171_antenna_basic_dipole_resonance/) 수에 비례하여 기하급수적 증가 | [Massive MIMO](/knowledge-base/studynote/03_network/02_multiplexing_multiple_access/099_Massive_MIMO_대규모_다중_안테나/) 도입 당위성 |
...
### 1. 의사결정 및 [체크리스트](/knowledge-base/studynote/04_software_engineering/11_testing_validation/435_checklist_based_testing/)
- **밀집 구역(Dense Urban) 간섭 제어**: 콘서트장처럼 사용자들이 공간적으로 너무 가까이 붙어 있다면 빔 간의 상호 간섭(상관도 증가)을 피할 수 없다. 이 상황에서는 순수 SDMA 대신 채널 직교성을 강제로 맞추는 제로 포싱(Zero-Forcing) 프리코딩을 쓰거나, 타겟을 시간 단위로 쪼개는 [TDMA](/knowledge-base/studynote/03_network/02_multiplexing_multiple_access/089_시분할_다중접속_TDMA/) 방식을 병행하는 하이브리드 스케줄링으로 [폴백](/knowledge-base/studynote/07_enterprise_systems/03_eai_esb_msa/171_fallback_resilience_pattern/)([Fallback](/knowledge-base/studynote/13_cloud_architecture/03_msa_serverless/129_fallback/))해야 한다.
- **피드백 품질 확보(High Mobility)**: 고속철도처럼 단말이 빠르게 이동하면 수집한 CSI가 빔을 쏠 때쯤 이미 낡은 정보([Aging](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/182_aging/) Effect)가 되어 엉뚱한 곳에 전파를 낭비한다. 단말의 피드백 주기 오버헤드를 견딜 수 있는지 [대역폭](/knowledge-base/studynote/01_computer_architecture/03_architecture_basics_performance/140_bandwidth/) 비용을 철저히 산정해야 한다.

### 2. [안티패턴](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/128_water_scrum_fall_anti_pattern/)
- 이동성이 잦고 다중 경로 반사([Multipath Fading](/knowledge-base/studynote/03_network/03_physical_layer_media/168_multipath_fading_isi/))가 심한 도심 골목에서 좁고 날카로운 빔(Narrow Beam)을 무리하게 유지하다가, 잦은 "빔 오조준(Beam Misalignment)"으로 인해 오히려 사용자에게 엉뚱한 노이즈 간섭만 유발하는 설계.
...
SDMA는 물리적 공간을 무선 자원의 3번째 축으로 개척해 스펙트럼 효율(bps/Hz)을 한 차원 높였다. [스마트 안테나](/knowledge-base/studynote/03_network/03_physical_layer_media/177_smart_antenna_phased_array/)가 타겟에게만 [신호](/knowledge-base/studynote/02_operating_system/02_process_thread/130_signal/) 에너지를 집중시키므로([Array](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/055_array/) Gain), 셀의 커버리지가 음영지역까지 확장되고 단말기의 전력 소모도 획기적으로 줄어드는 일석이조의 효과를 얻는다.

미래의 [6G](/knowledge-base/studynote/07_enterprise_systems/09_digital_transformation/419_6g_ntn_thz_ris_next_gen/) 네트워크에서는 분산형 기지국들이 협력해 넓은 공간에서 조인트 [빔포밍](/knowledge-base/studynote/03_network/02_multiplexing_multiple_access/101_beamforming/)을 수행하는 Cell-Free [Massive MIMO](/knowledge-base/studynote/03_network/02_multiplexing_multiple_access/099_Massive_MIMO_대규모_다중_안테나/) 환경으로 SDMA가 진화할 것이다. 통신망 설계자는 공간이 곧 가장 가치 있는 자원임을 이해하고, [AI](/knowledge-base/studynote/04_software_engineering/03_design_architecture/190_ai_llm_requirements_specification/) 기반의 빔 추적 알고리즘을 융합하는 시각을 가져야 한다.
...
← **이전**: [95. NOMA (Non-Orthogonal Multiple Access) - 비직교 다중 접속 (5G/6G 기술)](/knowledge-base/studynote/03_network/02_multiplexing_multiple_access/095_NOMA_비직교_다중_접속/)
**다음**: [97. MIMO (Multiple-Input Multiple-Output) 다중 안테나 기술](/knowledge-base/studynote/03_network/02_multiplexing_multiple_access/097_MIMO_다중_안테나_기술/) →
