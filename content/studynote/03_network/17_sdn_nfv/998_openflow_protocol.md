...
그러나 실무에서는 OpenFlow 특유의 잦은 컨트롤러 통신으로 인한 확장성 문제와 칩셋 개발의 어려움으로 인해 만능 열쇠로 자리 잡지는 못했다. 오늘날의 SDN 생태계는 OpenFlow를 그대로 쓰기보다는, VXLAN 기반의 BGP EVPN이나 완전 개방형 데이터 플레인 언어인 P4 쪽으로 실용적인 진화를 거듭하고 있다. 기술사는 OpenFlow를 'SDN의 시조새'이자 데이터/컨트롤 분리 사상의 가장 완벽한 교보재로 기억해야 한다. 향후에는 프로그래머블 네트워크 같은 자동화 흐름과 결합되어 더 정교한 형태로 확장될 가능성이 크다.
