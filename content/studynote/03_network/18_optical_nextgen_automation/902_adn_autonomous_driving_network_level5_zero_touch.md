...
### 1. 클라우드 중앙 [AI](/knowledge-base/studynote/04_software_engineering/03_design_architecture/190_ai_llm_requirements_specification/) (매크로 지능)
- 서울 본사 서버실에 떠 있는 거대한 대뇌 딥러닝 센터입니다. 전국 10만 대 기지국에서 쏟아지는 텔레메트리 강물을 1년 치 학습하여, "크리스마스에는 명동 기지국이 터질 [확률](/knowledge-base/studynote/08_algorithm_stats/08_stats/130_probability/) 99%"라는 전 지구적 통계(모델) 룰을 찍어냅니다.

### 2. 엣지 [분산](/knowledge-base/studynote/08_algorithm_stats/08_stats/136_variance/) [AI](/knowledge-base/studynote/04_software_engineering/03_design_architecture/190_ai_llm_requirements_specification/) (마이크로 지능)
- 동네 [5G](/knowledge-base/studynote/07_enterprise_systems/09_digital_transformation/418_5g_embb_urllc_mmtc_slicing/) 기지국 장비(DU/CU) 바닥에 칩셋으로 박혀있는 미니 [AI](/knowledge-base/studynote/04_software_engineering/03_design_architecture/190_ai_llm_requirements_specification/)(강화학습 에이전트)들입니다. 중앙에서 내려준 룰을 바탕으로 0.001초 단위로 흔들리는 무선 전파 간섭을 피하기 위해 [빔포밍](/knowledge-base/studynote/03_network/02_multiplexing_multiple_access/101_beamforming/) 각도를 휙휙 스스로 꺾어대며(최적 [라우팅](/knowledge-base/studynote/03_network/07_network_layer_routing/339_routing_overview_best_path_selection/)) 현장 대처를 합니다.

### 3. 디바이스 [AI](/knowledge-base/studynote/04_software_engineering/03_design_architecture/190_ai_llm_requirements_specification/) (모바일 지능)
- 내 폰에 박힌 [AI](/knowledge-base/studynote/04_software_engineering/03_design_architecture/190_ai_llm_requirements_specification/) 칩([NPU](/knowledge-base/studynote/01_computer_architecture/12_accelerators_ai_hardware/424_npu/))입니다. 스마트폰 자체가 "내 배터리가 없으니 지금은 저전력 모드로 통신하고, 끊길 것 같으면 5G에서 Wi-Fi로 미리 갈아타자"며 기지국과 협상하는 생태계 통합입니다.

자율-구동 네트워크를 볼 때는 앞뒤 개념과의 경계를 함께 봐야 전체 흐름이 선명해진다. AIOps가 기반 조건을 만든다면, 자율-구동 네트워크는 그 위에서 핵심 메커니즘을 구현하고, [디지털 트윈](/knowledge-base/studynote/06_ict_convergence/02_iot_mobility/126_digital_twin_concept/) 네트워크 실시간 토폴로지 동기…는 이를 더 확장된 적용 단계로 연결한다. 따라서 단일 정의보다 전송 용량과 자동 제어성에 어떤 차이를 만드는지 비교하는 것이 중요하다.

| 관점 | 선행 개념 | 현재 개념 | 확장 개념 |
|:---|:---|:---|:---|
| 초점 | AIOps의 기반 정리 | 자율-구동 네트워크의 핵심 동작 | [디지털 트윈](/knowledge-base/studynote/06_ict_convergence/02_iot_mobility/126_digital_twin_concept/) 네트워크 실시간 토폴로지 동기…의 확장 적용 |
...
- 트래픽 경로 최적화부터, 신종 디도스 공격을 감지하고(침해 자동 결단) [스위치](/knowledge-base/studynote/03_network/05_lan_wan_l2_devices/238_switch_operation_principles/) 룰을 내려 막아내는(차단 통제) 방어 능력까지 인간의 뇌 속도(분 단위)를 넘어선 밀리초(ms) 단위의 인프라 진화 속도를 달성하여 통신 장애(Downtime)를 0초로 만듭니다.
