...
- **💡 비유**: [직렬](/knowledge-base/studynote/03_network/03_physical_layer_media/149_serial_communication_rs232_rs485/) 모델은 **'크리스마스 트리 전구'**와 같습니다. 전구 하나가 끊어지면 줄 전체에 불이 안 들어옵니다(약한 고리). 반면 [병렬](/knowledge-base/studynote/05_database/07_exam_summary/430_index_fast_full_scan/) 모델은 **'쌍발 엔진 비행기'**와 같습니다. 오른쪽 엔진이 고장 나도 왼쪽 엔진이 살아있으면 비행기는 추락하지 않습니다.
...
- **등장 배경 및 발전 과정**:
  ...
  2. **[소프트웨어 공학](/knowledge-base/studynote/04_software_engineering/01_overview_principles/001_software_engineering_definition/)으로의 편입**: 하드웨어를 넘어 소프트웨어 [모듈](/knowledge-base/studynote/04_software_engineering/04_testing_quality/192_module_independence/) 간의 의존성, [분산](/knowledge-base/studynote/08_algorithm_stats/08_stats/136_variance/) 컴퓨팅(클라이언트-서버)의 [신뢰성](/knowledge-base/studynote/04_software_engineering/10_trends_pm_quality/642_reliability_mtbf_mttr_mttf_availability/)을 측정하는 데 적용되었다.
...
시스템 [신뢰성](/knowledge-base/studynote/04_software_engineering/10_trends_pm_quality/642_reliability_mtbf_mttr_mttf_availability/) 모델링의 핵심 원리는 **복잡성 분해**, **역할 분리**, **품질 측정**의 세 축으로 이해할 수 있다. 복잡한 문제를 관리 가능한 단위로 나누고, 각 역할의 책임을 명확히 하며, 결과를 정량적 지표로 평가하는 과정이 반복된다.
