...
> 1. **본질**: 범위 크리프 ([Scope](/knowledge-base/studynote/09_security/05_web_app_security/512_oauth_scope/) Creep)는 [베이스라인](/knowledge-base/studynote/04_software_engineering/03_design_architecture/159_baseline_requirements_configuration_management/) 이후 요구사항이 공식 승인과 재계획 없이 조금씩 늘어나 프로젝트 범위를 흐리게 만드는 현상이다.
...
| [이해관계자](/knowledge-base/studynote/04_software_engineering/03_design_architecture/173_stakeholder_identification_impact_matrix/) 증가 | 중간에 새로운 의사결정권자 등장 | 기존 합의를 다시 흔듦 |
| 내부 자의적 확장 | [골드 플래팅](/knowledge-base/studynote/04_software_engineering/03_design_architecture/162_gold_plating_anti_pattern/) ([Gold Plating](/knowledge-base/studynote/04_software_engineering/03_design_architecture/162_gold_plating_anti_pattern/)) | 외부 요청 없이도 범위가 커짐 |
...
- **📢 섹션 요약 비유**: 범위 크리프는 지도를 안 고치고 도로만 계속 늘리는 도시와 같다. 길은 많아지지만 표지판과 [신호](/knowledge-base/studynote/02_operating_system/02_process_thread/130_signal/) 체계가 따라오지 못해 더 혼잡해진다.

---

## Ⅲ. 비교 및 연결

범위 크리프는 정당한 변경 요청과 구분해야 하고, [골드 플래팅](/knowledge-base/studynote/04_software_engineering/03_design_architecture/162_gold_plating_anti_pattern/)과도 경계를 세워야 한다. 정당한 변경 요청은 공식 절차와 영향도 분석을 거쳐 일정·비용·우선순위가 재조정된 경우다. 반면 범위 크리프는 외부 요구가 통제 없이 스며드는 경우가 많고, [골드 플래팅](/knowledge-base/studynote/04_software_engineering/03_design_architecture/162_gold_plating_anti_pattern/)은 개발팀 내부가 스스로 범위를 키우는 경우가 많다.

| 구분 | 범위 크리프 | [골드 플래팅](/knowledge-base/studynote/04_software_engineering/03_design_architecture/162_gold_plating_anti_pattern/) | 정당한 변경 요청 |
| :--- | :--- | :--- | :--- |
| 출발점 | 고객·[이해관계자](/knowledge-base/studynote/04_software_engineering/03_design_architecture/173_stakeholder_identification_impact_matrix/)의 추가 요구 | 내부 팀의 자의적 추가 | 공식 Change Request |
| 승인 절차 | 약하거나 없음 | 대개 없음 | 명확히 존재 |
| 계획 반영 | 늦거나 누락 | 늦거나 누락 | 일정·비용·품질 계획에 반영 |
| 결과 | 일정 미끄럼, 예산 잠식, 품질 저하 | 과잉 구현, [검증](/knowledge-base/studynote/04_software_engineering/07_object_oriented/395_verification_process_review/) 범위 확대 | 통제된 범위 변경 |

이 개념은 [베이스라인](/knowledge-base/studynote/04_software_engineering/03_design_architecture/159_baseline_requirements_configuration_management/), [CCB](/knowledge-base/studynote/04_software_engineering/03_design_architecture/160_change_control_board_ccb_requirements_review/), [RTM](/knowledge-base/studynote/04_software_engineering/uncategorized/667_requirements_traceability_matrix/), [애자일](/knowledge-base/studynote/15_devops_sre/01_culture_methodology/004_agile_relation/) 백로그 관리와 긴밀히 연결된다. 전통적 프로젝트에서는 CCB가 변경을 심사하고, [애자일](/knowledge-base/studynote/15_devops_sre/01_culture_methodology/004_agile_relation/)에서는 제품 백로그와 [스프린트](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/067_sprint_timebox/) 계획이 비슷한 통제 역할을 한다. 즉 방식은 달라도 핵심은 동일하다. "새 요구를 받을 수는 있지만, 기존 약속과 교환 관계를 명확히 해야 한다"는 것이다.

그래서 기술사 답안에서는 범위 크리프를 단순히 나쁜 현상이라 적는 것보다, 어떤 장치가 이를 정당한 변경으로 바꾸는지까지 써야 완성도가 높다. 변경을 막는 것이 목적이 아니라, 통제되지 않은 팽창을 막는 것이 목적이다.

- **📢 섹션 요약 비유**: 범위 크리프와 [골드 플래팅](/knowledge-base/studynote/04_software_engineering/03_design_architecture/162_gold_plating_anti_pattern/)은 둘 다 배낭 무게를 늘리지만, 하나는 밖에서 계속 짐을 넣는 경우이고 다른 하나는 내가 스스로 불필요한 짐을 더 넣는 경우다.

---

## Ⅳ. 실무 적용 및 기술사 판단

실무에서는 범위 크리프를 완전히 없애기보다, 초기에 감지하고 정식 변경 절차로 전환하는 것이 중요하다. 공공 SI (System Integration) 사업처럼 계약 범위가 명확한 환경에서는 요구 추가마다 비용·일정 협의가 필요하고, [애자일](/knowledge-base/studynote/15_devops_sre/01_culture_methodology/004_agile_relation/) 제품 개발에서는 백로그 우선순위 조정을 통해 무엇을 지금 하고 무엇을 뒤로 미룰지 분명히 해야 한다. 결국 핵심은 추가 요구를 숨은 작업으로 만들지 않는 것이다.
...
| 개념 | 연결 포인트 |
| :--- | :--- |
| [베이스라인](/knowledge-base/studynote/04_software_engineering/03_design_architecture/159_baseline_requirements_configuration_management/) ([Baseline](/knowledge-base/studynote/04_software_engineering/01_overview_principles/025_baseline/)) | 범위 크리프 여부를 판단하는 기준선이다. |
| [CCB](/knowledge-base/studynote/04_software_engineering/03_design_architecture/160_change_control_board_ccb_requirements_review/) ([Configuration Control](/knowledge-base/studynote/04_software_engineering/01_overview_principles/022_configuration_control/) Board) | 추가 요구를 공식 변경으로 전환할지 심사하는 장치다. |
| [RTM](/knowledge-base/studynote/04_software_engineering/uncategorized/667_requirements_traceability_matrix/) ([Requirements Traceability Matrix](/knowledge-base/studynote/04_software_engineering/uncategorized/667_requirements_traceability_matrix/)) | 변경이 설계·개발·테스트에 어떻게 번지는지 추적한다. |
| [골드 플래팅](/knowledge-base/studynote/04_software_engineering/03_design_architecture/162_gold_plating_anti_pattern/) ([Gold Plating](/knowledge-base/studynote/04_software_engineering/03_design_architecture/162_gold_plating_anti_pattern/)) | 내부 팀이 자의적으로 범위를 넓히는 유사 [안티패턴](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/128_water_scrum_fall_anti_pattern/)이다. |
