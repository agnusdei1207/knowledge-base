...
요구사항 관리는 프로젝트 전 생명주기 동안 요구사항을 [식별](/knowledge-base/studynote/09_security/13_secops_ir_forensics/655_ir_detection_analysis/), 조직화, 추적, 변경 통제, [버전](/knowledge-base/studynote/03_network/06_network_layer_ip/288_version_ihl_tos_total_length/) 관리하는 활동이다. 요구사항은 프로젝트 [초기](/knowledge-base/studynote/03_network/08_transport_layer/459_quic_fec_forward_error_correction/)에만 쓰고 끝나는 문서가 아니라, 설계·구현·테스트·운영 판단의 [기준선](/knowledge-base/studynote/04_software_engineering/01_overview_principles/025_baseline/)이 된다. 따라서 한 번 합의된 요구사항이 무엇인지, 무엇이 언제 왜 바뀌었는지, 현재 어떤 [버전](/knowledge-base/studynote/03_network/06_network_layer_ip/288_version_ihl_tos_total_length/)이 유효한지를 계속 관리해야 한다.

이 활동이 필요한 이유는 요구사항이 본질적으로 변하기 때문이다. 고객 요구, 규제, 시장, 운영 환경이 바뀌면 변경 요청은 반드시 발생한다. 문제는 변경 자체가 아니라, 변경이 문서와 코드에 비공식적으로 스며들 때 생기는 혼란이다. [기준선](/knowledge-base/studynote/04_software_engineering/01_overview_principles/025_baseline/) 없는 변경은 재작업 증가, 일정 [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/), 테스트 누락, 책임 공방으로 이어진다. 요구사항 관리는 이 혼란을 줄이기 위한 프로젝트의 제동 장치다.
...
| 구성 요소 | 역할 | 관리 포인트 |
| :--- | :--- | :--- |
| Requirement ID | 요구사항 개체 [식별](/knowledge-base/studynote/09_security/13_secops_ir_forensics/655_ir_detection_analysis/) | 중복·누락 방지, 추적성 출발점 |
...
| 핵심 활동 | [베이스라인](/knowledge-base/studynote/04_software_engineering/03_design_architecture/159_baseline_requirements_configuration_management/), [버전](/knowledge-base/studynote/03_network/06_network_layer_ip/288_version_ihl_tos_total_length/), 변경 승인 | [버전](/knowledge-base/studynote/03_network/06_network_layer_ip/288_version_ihl_tos_total_length/), 빌드, 릴리즈, [감사](/knowledge-base/studynote/02_operating_system/10_security/606_auditing_linux_auditd/) | 정방향/역방향 추적 |
| 연결 방식 | 변경의 출발점 | 변경의 실행 대상 | 변경 영향 분석의 근거 |

또한 [애자일](/knowledge-base/studynote/15_devops_sre/01_culture_methodology/004_agile_relation/) ([Agile](/knowledge-base/studynote/15_devops_sre/01_culture_methodology/004_agile_relation/))이라고 해서 요구사항 관리가 사라지는 것은 아니다. 폭포수는 긴 주기로 [베이스라인](/knowledge-base/studynote/04_software_engineering/03_design_architecture/159_baseline_requirements_configuration_management/)을 고정하고, [애자일](/knowledge-base/studynote/15_devops_sre/01_culture_methodology/004_agile_relation/)은 [스프린트](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/067_sprint_timebox/)나 릴리스 단위로 짧게 [기준선](/knowledge-base/studynote/04_software_engineering/01_overview_principles/025_baseline/)을 잡는다. 즉 차이는 “통제 유무”가 아니라 “통제 단위와 속도”다. 그래서 백로그 기반 개발에서도 현재 [스프린트](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/067_sprint_timebox/)에 들어온 요구사항은 변경 비용과 우선순위를 따져 관리해야 한다.

- **📢 섹션 요약 비유**: 요구사항 관리가 여행 일정표를 정하는 일이라면, [형상 관리](/knowledge-base/studynote/04_software_engineering/01_overview_principles/020_software_configuration_management/)는 실제 짐을 싸고 출발하는 일이고, RTM은 일정표의 각 항목이 실제 예약과 결제로 이어졌는지 확인하는 체크표다.

---

## Ⅳ. 실무 적용 및 기술사 판단

실무에서는 요구사항 관리가 문서 부서의 일이 아니라 [ALM](/knowledge-base/studynote/04_software_engineering/06_software_architecture/390_application_lifecycle_management/) ([Application Lifecycle Management](/knowledge-base/studynote/04_software_engineering/06_software_architecture/390_application_lifecycle_management/)) 체계의 중심이어야 한다. Confluence나 요구사항 도구에서 변경 요청이 등록되면, Jira 이슈, Git 브랜치, 테스트 케이스가 연동되어 어떤 요구가 어떤 산출물에 영향을 주는지 바로 보여 주는 구조가 이상적이다. 이때 [CCB](/knowledge-base/studynote/04_software_engineering/03_design_architecture/160_change_control_board_ccb_requirements_review/) (Change Control Board)는 단순 결재 조직이 아니라, 변경 가치와 비용, 위험, 일정 영향을 판단하는 의사결정 장치다.
...
- 승인된 [베이스라인](/knowledge-base/studynote/04_software_engineering/03_design_architecture/159_baseline_requirements_configuration_management/) 문서를 직접 수정해 [버전](/knowledge-base/studynote/03_network/06_network_layer_ip/288_version_ihl_tos_total_length/) 이력을 없애는 경우
- 신규 요구사항을 “버그 수정”으로 위장해 변경 통제를 우회하는 경우
- [RTM](/knowledge-base/studynote/04_software_engineering/uncategorized/667_requirements_traceability_matrix/), 테스트, 릴리스 계획은 갱신하지 않고 요구사항 문서만 바꾸는 경우
...
| 개념 | 연결 포인트 |
| :--- | :--- |
| [베이스라인](/knowledge-base/studynote/04_software_engineering/03_design_architecture/159_baseline_requirements_configuration_management/) ([Baseline](/knowledge-base/studynote/04_software_engineering/01_overview_principles/025_baseline/)) | 특정 시점의 승인된 요구사항 [기준선](/knowledge-base/studynote/04_software_engineering/01_overview_principles/025_baseline/) |
