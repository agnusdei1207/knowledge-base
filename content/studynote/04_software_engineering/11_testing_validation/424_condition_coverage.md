...
> 1. **본질**: 조건 커버리지 (Condition Coverage)은(는) [[001_software_engineering_definition|소프트웨어 공학]]의 핵심 개념으로, 복잡한 시스템을 체계적으로 설계·관리하기 위한 원칙과 기법이다.
...
소프트웨어 [[369_logic_bomb|논리]]의 70%는 복합 조건문으로 이루어져 있습니다. 
...
위 두 개만 해봐도 분기가 $T/F$로 모두 갈렸으니 [[423_decision_coverage|결정 커버리지]]는 만족합니다. 그런데 갑자기 프로그램이 사형 선고를 받습니다. "어? 성인인데 음주 수치가 0.05(F)가 나온 상황은 테스트 아예 안 했네?" 
...
- **📢 섹션 요약 비유**: 로봇 장난감 검사를 할 때, [[423_decision_coverage|결정 커버리지]]는 "버튼 누르니 로봇이 걷고, 또 누르니 섰네. 합격(전체 결괏값)"이라고 평가하지만, 조건 커버리지는 "왼쪽 다리의 작은 톱니(조건A)가 한번 돌아갔고 안 돌아갔나? 오른쪽 큰 톱니바퀴(조건B)가 한 번 멈추고 굴렀나?"라고 겉모습 대신 뱃속 다이얼들의 미세 작동 여부만 하나하나 따지는 독종 정비사입니다.
...
이게 바로 그 유명한 화이트박스 기출문제, **"조건 커버리지 100%는 [[423_decision_coverage|결정 커버리지]] 100%를 보장하지 못한다"**는 학술적 진리입니다. 안쪽 부품들의 조화가 어긋나 엉뚱하게 겉 껍데기 문이 반쪽짜리로만 닫혀버리는 무서운 약점입니다.

- **📢 섹션 요약 비유**: 체육대회에서 체육 교사(조건 커버리지)가 "모든 학생이 달리기(T)랑 휴식(F)을 다 한 번씩 반반 섞어서 했네!"라고 칭찬했지만, 막상 청백전 종합 점수판([[423_decision_coverage|결정 커버리지]])을 보니 이상한 대진표 꼬임 때문에 청군(True)만 스코어가 100번 오르고 백군(False)으론 점수가 1점도 안 올라가서 행사가 망해버린 꼴입니다.
...
실무의 테스트 엔지니어들은 이 조건 커버리지와 [[423_decision_coverage|결정 커버리지]] 각각의 치명적 결점들을 꿰매어 완벽한 괴물을 창조해 냈습니다. 

이것이 그 유명한 **조건/[[423_decision_coverage|결정 커버리지]] (Condition/[[423_decision_coverage|Decision Coverage]])**이며 한 발 더 나아가 항공 우주 방위산업의 최강 군주라 불리는 **변경 조건/[[423_decision_coverage|결정 커버리지]] (MC/DC)**의 기둥이 됩니다. 
...
조건 커버리지(Condition Coverage)는 하나의 통나무 상자처럼 보였던 복합 [[369_logic_bomb|논리]] 분기문(`if문`)을 쪼개서 개별 세포들 단위의 건전성(Health)을 확보해 준 위대한 화이트박스 해단(解斷) 도구입니다. 
...
← **이전**: [[423_decision_coverage|423. 결정 커버리지 (Decision Coverage / 분기 커버리지) - 분기문(If, While 등)의 참/거짓을 최소 한 번씩]]
**다음**: [[424_condition_coverage|424. 조건 커버리지 (Condition Coverage) - 분기문 내의 각 개별 조건식이 참/거짓을 한 번씩 가짐]] →
