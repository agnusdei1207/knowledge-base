...
이 다이어그램은 [[440_insecure_design|Insecure Design]] (안전하가 입력 요구사항을 받아 핵심 처리 과정을 거쳐 [[395_verification_process_review|검증]]된 결과물을 산출하는 흐름을 보여준다.
...
| 정상 흐름 | 일반 사용자 경로 |
| 남용 흐름 | 공격자 경로 |
| 방어 요구사항 | 제한, [[396_validation|확인]], 격리 |
...
Insecure Design은 Broken Access Control이나 [[480_injection|Injection]] 같은 결과를 낳을 수 있다.

| 구분 | 안전한 설계 | 안전하지 않은 설계 |
|:---|:---|:---|
| 관점 | 예방 중심 | 기능 중심 |
| 입력 | 제한/[[395_verification_process_review|검증]] | 신뢰 |
| 결과 | 남용 차단 | 취약점 누적 |

[[097_stride_convolutional_neural_network_downsampling|STRIDE]], DREAD, Threat Modeling이 여기서 유효하다.
...
2. 중요한 경계에 [[396_validation|확인]] 단계가 있는가?
3. 실패 시 안전한 기본값을 쓰는가?
...
| 품질 보증 (QA, Quality Assurance) | [[440_insecure_design|Insecure Design]] (안전하지 않은 설계) 적용 결과는 QA 활동을 통해 [[395_verification_process_review|검증]]되고 측정된다 |
...
← **이전**: [[480_injection|480. Injection (인젝션 / SQLi, OS Command, NoSQL 등)]]
