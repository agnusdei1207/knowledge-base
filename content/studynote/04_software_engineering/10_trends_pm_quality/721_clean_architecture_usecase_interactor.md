...
소프트웨어 개발 초창기에는 3계층 아키텍처(Presentation $\rightarrow$ Business $\rightarrow$ [Data](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/))가 표준이었다. 하지만 이 구조는 치명적인 약점이 있었다. 비즈니스 로직이 [데이터베이스](/knowledge-base/studynote/05_database/01_db_architecture_relational/002_database_definition/)([Data](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) Layer)에 강하게 의존([Coupling](/knowledge-base/studynote/04_software_engineering/04_testing_quality/195_coupling_levels/))한다는 것이었다.
...
| 항목 | 설명 | 비고 |
| :--- | :--- | :--- |
| 핵심 특성 | [클린 아키텍처](/knowledge-base/studynote/04_software_engineering/04_testing_quality/217_clean_architecture_dependency_rule/) Usecase Interactor 설계의 핵심 특성과 동작 방식 | 필수 이해 요소 |
...
[클린 아키텍처](/knowledge-base/studynote/04_software_engineering/04_testing_quality/217_clean_architecture_dependency_rule/)를 구현하는 과정에서 필수적으로 등장하는 원칙들이 있다.
...
[클린 아키텍처](/knowledge-base/studynote/04_software_engineering/04_testing_quality/217_clean_architecture_dependency_rule/)는 이론적으로 완벽하지만, 실무에 아무렇게나 도입하면 끔찍한 오버엔지니어링이 된다.
...
- **📢 섹션 요약 비유**: 자동차의 진짜 핵심은 엔진([도메인](/knowledge-base/studynote/05_database/02_modeling_normalization/064_relation_domain/))이다. 타이어(DB)나 핸들 껍데기(UI)는 유행에 따라 계속 갈아 끼울 수 있어야 한다. 껍데기를 바꾼다고 엔진까지 뜯어고쳐야 하는 자동차(레거시)는 아무도 사지 않는다.
...
| 품질 보증 (QA, Quality Assurance) | [클린 아키텍처](/knowledge-base/studynote/04_software_engineering/04_testing_quality/217_clean_architecture_dependency_rule/) Usecase Interactor 설계 적용 결과는 QA 활동을 통해 검증되고 측정된다 |
...
1. [클린 아키텍처](/knowledge-base/studynote/04_software_engineering/04_testing_quality/217_clean_architecture_dependency_rule/) Usecase Interactor 설계은 레고 블록으로 성을 만들 때처럼, 규칙을 정하고 역할을 나누어 함께 작업하는 방법이에요.
...
← **이전**: [720. 데이터옵스 (DataOps) 자동화](/knowledge-base/studynote/04_software_engineering/10_trends_pm_quality/720_dataops_automation_pipeline/)
**다음**: [722. 어니언 아키텍처 도메인 코어 격리](/knowledge-base/studynote/04_software_engineering/10_trends_pm_quality/722_onion_architecture_domain_isolation/) →
