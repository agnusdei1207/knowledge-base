...
> 1. **본질**: ISO/IEC/IEEE 29119 소프트웨어 테스팅 국제 표준은(는) [[001_software_engineering_definition|소프트웨어 공학]]의 핵심 개념으로, 복잡한 시스템을 체계적으로 설계·관리하기 위한 원칙과 기법이다.
...
소프트웨어 개발 분야에서는 코드를 짜는 언어(Java)나 설계하는 방식([[232_uml_unified_modeling_language_overview|UML]])에 대한 표준이 일찍부터 자리 잡았다. 그런데 정작 품질의 마지노선인 '테스팅(Testing)' 분야는 오랫동안 무법지대였다.

회사마다 테스트 계획서 양식이 달랐고, 영국 회사와 인도 하청업체가 쓰는 테스팅 용어가 달라 의사소통이 마비되었다. "[[412_black_box_testing|블랙박스 테스트]]"라는 단어 하나를 두고도 서로 다른 의미로 해석했다.
...
| 비교 항목 | ISO/IEC/IEEE 29119 | [[778_tmmi_test_maturity_model_integration|TMMi]] (Test [[011_maturity_model|Maturity Model]] integration) |
|:---|:---|:---|
...
실무에서는 **ISO 29119에 정의된 프로세스와 문서 양식을 도입하여 훈련하면, 자연스럽게 [[778_tmmi_test_maturity_model_integration|TMMi]] 레벨 3 이상의 심사를 통과**할 수 있는 튼튼한 토대가 마련된다.
...
결론적으로 기술 리더는 "우리 회사는 애자일이라서 문서 안 써요"라는 핑계를 용납해서는 안 된다. ISO 29119가 제시하는 것은 낡은 엑셀 문서가 아니라 **'테스트를 대하는 과학적 태도([[300_process|Process]])'**다. 테스트는 감(Feeling)으로 클릭해보는 것이 아니라, 리스크를 분석하고 전략을 세우는 가장 고도화된 [[001_software_engineering_definition|소프트웨어 공학]]의 정수임을 잊지 말아야 한다.
...
| 개념 | 연결 포인트 |
| :--- | :--- |
| [[001_software_engineering_definition|소프트웨어 공학]] ([[001_software_engineering_definition|Software Engineering]]) | ISO/IEC/IEEE 29119 소프트웨어 테스팅 국제 표준의 상위 학문 체계이며 품질·생산성 향상의 공통 목표를 공유한다 |
| [[003_sdlc|소프트웨어 생명주기]] ([[131_sdlc_system_development_life_cycle_waterfall_agile|SDLC]], Software Development Life Cycle) | ISO/IEC/IEEE 29119 소프트웨어 테스팅 국제 표준은 SDLC의 특정 단계에서 핵심적으로 적용된다 |
| 품질 보증 (QA, Quality Assurance) | ISO/IEC/IEEE 29119 소프트웨어 테스팅 국제 표준 적용 결과는 QA 활동을 통해 검증되고 측정된다 |
| [[020_software_configuration_management|형상 관리]] ([[167_scm_software_configuration_management|SCM]], [[020_software_configuration_management|Software Configuration Management]]) | ISO/IEC/IEEE 29119 소프트웨어 테스팅 국제 표준에서 생성된 산출물은 SCM을 통해 체계적으로 관리된다 |
...
← **이전**: [[778_tmmi_test_maturity_model_integration|778. 소프트웨어 테스트 성숙도 모델 (TMMi)]]
**다음**: [[780_cspm_cloud_security_posture_management|780. 클라우드 보안 형상 관리 (CSPM) 데브옵스 결합]] →
