...
> 1. **본질**: 오류 추정 (Error Guessing) - 테스터의 경험을 바탕으로 [[352_defect_definition|결함]]이 발생할 만한 곳을 추정하여 테스트은(는) [[001_software_engineering_definition|소프트웨어 공학]]의 핵심 개념으로, 복잡한 시스템을 체계적으로 설계·관리하기 위한 원칙과 기법이다.
...
- **개념**: 오류 추정은 테스터가既往の 경험, 類似 시스템에서의 문제 발생 [[001_dikw_pyramid|데이터]], 直感등을 활용하여 "여기서 문제가 발생할 것 같다"라는 예상으로 테스트 케이스를 설계하는 기법이다. 이는명세서나 코드 분석이 아닌, 테스터의 주관적判断에 의존한다.

- **필요성**: 체계적인 테스트 기법([[630_equivalence_partitioning_boundary_value_analysis|동등 분할]], [[414_boundary_value_analysis|경계값 분석]], [[632_state_transition_diagram_testing|상태 전이]] 등)은事前に定義된 규칙에 따라 테스트 케이스를 도출하지만, 이러한 규칙으로捕捉되지 않는 예외적인 상황이 있을 수 있다. 오류 추정은 이러한 사각지대를補完하여 [[352_defect_definition|결함]] 발견 효과를 향상시킨다.

- **오류 추정과 관련된概念**:
  - **경험 기반 테스트**: 테스터의 경험을 기반으로 테스트
  - **[[236_a_star_heuristic_minimax_mcts_monte_carlo|Heuristic]]**: 경험적法則으로 테스트 추측의根拠를 提供
...
오류 추정 (Error Guessing) - 테스터의 경험을 바탕으로 [[352_defect_definition|결함]]이 발생할 만한 곳을 추정하여 테스트의 핵심 원리와 구성 요소를 이해하기 위해 다음 구조를 살펴본다.
...
다른 [[001_software_engineering_definition|소프트웨어 공학]] 개념과의 연결을 보면, 오류 추정 (Error Guessing)은(는) 요구공학·설계·테스트·형상관리 전반에 걸쳐 영향을 미친다. 특히 품질 보증(QA, Quality Assurance)과 [[020_software_configuration_management|형상 관리]]([[167_scm_software_configuration_management|SCM]], [[020_software_configuration_management|Software Configuration Management]])와 긴밀하게 연계된다.
...
오류 추정 (Error Guessing)을(를) 올바르게 적용하면 [[339_software_quality_definition|소프트웨어 품질]]·[[346_maintainability_portability|유지보수성]]·팀 생산성이 동시에 향상된다. 그러나 도입에는 학습 비용과 [[459_quic_fec_forward_error_correction|초기]] 투자가 필요하며, 조직 전체의 공감과 훈련이 선행되어야 한다.
...
- **📢 섹션 요약 비유**: 오류 추정 (Error Guessing)의 기대효과는 마라톤 훈련과 같다. 처음에는 느리고 고통스럽지만, 올바른 훈련 원칙을 지킨 선수만이 결승선에서 최고의 기록을 낼 수 있다. [[001_software_engineering_definition|소프트웨어 공학]]의 원칙도 단기 편의보다 장기 완성도를 위한 투자다.
...
← **이전**: [[434_error_guessing|434. 오류 추정 (Error Guessing)]]
