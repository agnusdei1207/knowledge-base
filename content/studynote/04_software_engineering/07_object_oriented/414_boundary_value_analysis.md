...
> 1. **본질**: 경계값 분석 (Boundary Value Analysis) - 경계 부분에서 결함이 많다는 점 이용 (분할의 가장자리 값)은(는) [[001_software_engineering_definition|소프트웨어 공학]]의 핵심 개념으로, 복잡한 시스템을 체계적으로 설계·관리하기 위한 원칙과 기법이다.
...
- **필요성**: 프로그래머가 "0부터 100까지"라고 조건을 작성할 때, 종종 "0 < x < 100"으로 잘못 작성하거나, [[055_array|배열]] [[154_database_index_b_tree_search_optimization|인덱스]] 0부터 N개까지 있을 때 "0 ≤ i ≤ N" 대신 "0 ≤ i < N"으로 잘못 처리하는 경우가 있다. 이러한 off-by-one 오류는 경계값을 테스트하면 쉽게 발견할 수 있다.
...
- **등장 배경 및 발전 과정**:
  ...
  3. **현재**: 자동화된 테스트 [[087_process_state_transition|생성]] 도구에서 경계값 분석을 기본 기능으로 내장
...
다른 [[001_software_engineering_definition|소프트웨어 공학]] 개념과의 연결을 보면, 경계값 분석 (Boundary Value Analysis)은(는) 요구공학·설계·테스트·형상관리 전반에 걸쳐 영향을 미친다. 특히 품질 보증(QA, Quality Assurance)과 [[020_software_configuration_management|형상 관리]]([[167_scm_software_configuration_management|SCM]], [[020_software_configuration_management|Software Configuration Management]])와 긴밀하게 연계된다.
...
| [[020_software_configuration_management|형상 관리]] ([[167_scm_software_configuration_management|SCM]], [[020_software_configuration_management|Software Configuration Management]]) | 경계값 분석 (Boundary Value Analysis)에서 [[087_process_state_transition|생성]]된 산출물은 SCM을 통해 체계적으로 관리된다 |
...
← **이전**: [[414_boundary_value_analysis|414. 경계값 분석 (Boundary Value Analysis)]]
**다음**: [[415_decision_table|415. 의사 결정 테이블 (Decision Table)]] →
