...
> 1. **본질**: [카나리 배포](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/115_canary_deployment_gradual_rollout/) / [블루-그린 배포](/knowledge-base/studynote/13_cloud_architecture/04_devops_observability/194_blue_green_deployment_strategy/) 무중단은(는) [소프트웨어 공학](/knowledge-base/studynote/04_software_engineering/01_overview_principles/001_software_engineering_definition/)의 핵심 개념으로, 복잡한 시스템을 체계적으로 설계·관리하기 위한 원칙과 기법이다.
...
- **개념**: 과거 모놀리식 (Monolithic) 시대의 '빅뱅 배포 (Big Bang [Deployment](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/087_deployment_kubernetes_workload_rolling_update/))'는 기존 서버를 모두 끄고, 새 소스코드를 복사한 뒤, 서버를 다시 켜는 방식이었다. 이 과정에서 발생하는 10분~1시간의 공백을 다운타임(Downtime)이라 한다. **[무중단 배포](/knowledge-base/studynote/15_devops_sre/02_cicd_gitops/082_zero_downtime_deployment_rolling_blue_green_canary/)**는 서버 인프라의 가상화와 트래픽 [라우팅](/knowledge-base/studynote/03_network/07_network_layer_routing/339_routing_overview_best_path_selection/) 기술을 결합하여 이 공백 시간을 완벽하게 0초로 만드는 아키텍처적 접근이다.
- **필요성**: 쿠팡, 넷플릭스 등 대형 플랫폼에서 새벽 3시라고 하더라도 1분의 [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/) 중단은 막대한 매출 하락과 사용자 이탈을 초래한다. [지속적 배포](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/099_continuous_deployment_cd/) ([Continuous Deployment](/knowledge-base/studynote/13_cloud_architecture/04_devops_observability/165_continuous_deployment/))를 하루에 수십 번씩 수행하는 [데브옵스](/knowledge-base/studynote/04_software_engineering/uncategorized/652_devops_calms_culture/) ([DevOps](/knowledge-base/studynote/04_software_engineering/uncategorized/652_devops_calms_culture/)) 환경에서는, 릴리즈의 안전성과 속도를 동시에 잡기 위한 [무중단 배포](/knowledge-base/studynote/15_devops_sre/02_cicd_gitops/082_zero_downtime_deployment_rolling_blue_green_canary/) 파이프라인 구축이 필수적이다.
- **핵심 요구사항**: [무중단 배포](/knowledge-base/studynote/15_devops_sre/02_cicd_gitops/082_zero_downtime_deployment_rolling_blue_green_canary/)가 성립하려면 상태 비저장([Stateless](/knowledge-base/studynote/15_devops_sre/05_devsecops/239_stateless_redis/)) [애플리케이션 아키텍처](/knowledge-base/studynote/12_it_management/03_ea_isp/105_aa_as_is_analysis/), [데이터베이스](/knowledge-base/studynote/05_database/01_db_architecture_relational/002_database_definition/) [스키마](/knowledge-base/studynote/05_database/01_db_architecture_relational/005_schema/)([Schema](/knowledge-base/studynote/05_database/04_transactions_concurrency/505_schema/))의 하위 [호환성](/knowledge-base/studynote/04_software_engineering/06_software_architecture/344_compatibility_usability/), 동적인 로드 밸런서 제어 기술이 반드시 선행되어야 한다.
...
- **개념**: 완전히 동일한 운영 환경(서버 그룹)을 두 개(Blue와 Green)로 나눈다. Blue는 현재 [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/) 중인 구버전(V1)이고, Green은 새 [버전](/knowledge-base/studynote/03_network/06_network_layer_ip/288_version_ihl_tos_total_length/)(V2)이 배포될 대기 상태의 환경이다.
- **동작 방식**:
  1. Green 환경에 V2 코드를 배포하고, 내부적으로 격리된 상태에서 단위/통합 테스트를 완벽히 마친다.
  2. 라우터(Router)나 로드 밸런서의 엔드포인트(Endpoint) 타겟을 Blue에서 Green으로 '단숨에([Switch](/knowledge-base/studynote/03_network/05_lan_wan_l2_devices/238_switch_operation_principles/))' 변경한다.
...
| 항목 | 설명 | 비고 |
| :--- | :--- | :--- |
| 핵심 특성 | [카나리 배포](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/115_canary_deployment_gradual_rollout/) / [블루-그린 배포](/knowledge-base/studynote/13_cloud_architecture/04_devops_observability/194_blue_green_deployment_strategy/) 무중단의 핵심 특성과 동작 방식 | 필수 이해 요소 |
...
- **개념**: 새로운 [버전](/knowledge-base/studynote/03_network/06_network_layer_ip/288_version_ihl_tos_total_length/)(V2)을 프로덕션 환경에 배포하되, 처음에는 1%의 소수 사용자(또는 특정 지역, 사내 직원)에게만 트래픽을 [라우팅](/knowledge-base/studynote/03_network/07_network_layer_routing/339_routing_overview_best_path_selection/)하여 안정성을 [검증](/knowledge-base/studynote/04_software_engineering/07_object_oriented/395_verification_process_review/)한 뒤, 문제가 없으면 점진적으로 [10](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/489_raid_10_hybrid/)%, 50%, 100%로 트래픽 비중을 늘려가는 [전략](/knowledge-base/studynote/04_software_engineering/04_testing_quality/268_strategy_pattern/)이다.
...
- **📢 섹션 요약 비유**: 독버섯인지 식용버섯인지 헷갈릴 때 온 가족이 다 같이 먹어보는 것이 아니라, 먼저 조금 떼어내어 한 사람([카나리](/knowledge-base/studynote/02_operating_system/10_security/595_canary_stack_smashing_protector/)아)만 살짝 맛을 보게 한 뒤 이상이 없으면 다 같이 먹는 안전망입니다.
...
- **트레이드오프**: [블루-그린 배포](/knowledge-base/studynote/13_cloud_architecture/04_devops_observability/194_blue_green_deployment_strategy/)처럼 자원을 2배로 사용할 필요가 없어 인프라 비용이 저렴하다. 하지만 배포가 진행되는 과도기 동안, 사용자 요청이 로드 밸런서에 의해 구버전(V1)으로 갈 수도 있고 신규 [버전](/knowledge-base/studynote/03_network/06_network_layer_ip/288_version_ihl_tos_total_length/)(V2)으로 갈 수도 있는 **[버전](/knowledge-base/studynote/03_network/06_network_layer_ip/288_version_ihl_tos_total_length/) 불일치 상태(Version Inconsistency)**가 발생한다.
...
[무중단 배포](/knowledge-base/studynote/15_devops_sre/02_cicd_gitops/082_zero_downtime_deployment_rolling_blue_green_canary/) [전략](/knowledge-base/studynote/04_software_engineering/04_testing_quality/268_strategy_pattern/)은 각자의 명확한 한계점을 지니며, 팀의 인프라 성숙도와 비용 제약에 따라 선택되어야 한다.
...
| **롤링 (Rolling)** | n대씩 순차 교체 | 인프라 추가 자원이 거의 들지 않음 (비용 최적화). | **[버전](/knowledge-base/studynote/03_network/06_network_layer_ip/288_version_ihl_tos_total_length/) 공존 시간(Inconsistency).** [롤백](/knowledge-base/studynote/15_devops_sre/02_cicd_gitops/098_rollback_strategy_pipeline_error_threshold/) 속도가 느림. [API](/knowledge-base/studynote/02_operating_system/01_overview_architecture/014_api_posix/) [호환성](/knowledge-base/studynote/04_software_engineering/06_software_architecture/344_compatibility_usability/) 복잡함. | K8s 기본 환경, 자원이 제한된 상태에서 무중단이 필요한 곳 |
| **[카나리](/knowledge-base/studynote/02_operating_system/10_security/595_canary_stack_smashing_protector/) ([Canary](/knowledge-base/studynote/02_operating_system/10_security/595_canary_stack_smashing_protector/))** | 소수 트래픽부터 점진 노출 | 프로덕션 환경의 실사용자 트래픽으로 안전한 [리스크](/knowledge-base/studynote/11_design_supervision/02_architecture_principles/096_risk_non_risk_architecture_evaluation_flaws/) [검증](/knowledge-base/studynote/04_software_engineering/07_object_oriented/395_verification_process_review/)(격리) 가능. | **가장 높은 구축 난이도.** 정밀한 L7 [라우팅](/knowledge-base/studynote/03_network/07_network_layer_routing/339_routing_overview_best_path_selection/) 및 실시간 모니터링 필수. | [MSA](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/619_msa_traffic_hardware/) 아키텍처 성숙도가 높고, 신기능 오류 파급력을 최소화하려는 대기업 |

결론적으로 현대 [클라우드 네이티브](/knowledge-base/studynote/04_software_engineering/11_testing_validation/531_cloud_native_architecture/) 아키텍처에서는 인프라 확장이 유연하다면 '블루-그린'을, 비용을 절감해야 한다면 '롤링'을 선택하되, [리스크](/knowledge-base/studynote/11_design_supervision/02_architecture_principles/096_risk_non_risk_architecture_evaluation_flaws/) 통제가 가장 중요하다면 [이스티오](/knowledge-base/studynote/03_network/16_data_center_cloud/829_istio_envoy_service_mesh_control_plane/)([Istio](/knowledge-base/studynote/12_it_management/05_security_compliance/302_service_mesh_istio/))와 같은 [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/) 메쉬를 도입하여 '[카나리](/knowledge-base/studynote/02_operating_system/10_security/595_canary_stack_smashing_protector/)' [전략](/knowledge-base/studynote/04_software_engineering/04_testing_quality/268_strategy_pattern/)으로 진화하는 것이 업계의 표준 패러다임이다.

- **📢 섹션 요약 비유**: 이사를 갈 때, 새 집을 완벽히 꾸며놓고 단숨에 들어가는 것(블루그린), 방 하나만 먼저 써보고 괜찮으면 나머지 짐을 옮기는 것([카나리](/knowledge-base/studynote/02_operating_system/10_security/595_canary_stack_smashing_protector/)), 짐을 트럭에 조금씩 실어서 서서히 옮기는 것(롤링) 중 예산과 상황에 맞는 최적의 [전략](/knowledge-base/studynote/04_software_engineering/04_testing_quality/268_strategy_pattern/)을 고르는 것과 같습니다.

---

## 👶 어린이를 위한 3줄 비유 설명
1. 게임을 하다가 업데이트 창이 뜨면 게임을 끄고 몇 십 분을 기다려야 해서 짜증나죠?
2. 하지만 최신 마법 서버([무중단 배포](/knowledge-base/studynote/15_devops_sre/02_cicd_gitops/082_zero_downtime_deployment_rolling_blue_green_canary/))는 여러분이 게임을 즐기고 있는 동안, 몰래 옆방에 새로운 괴물과 맵을 다 만들어 놓아요!
...
1. [카나리 배포](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/115_canary_deployment_gradual_rollout/) / [블루-그린 배포](/knowledge-base/studynote/13_cloud_architecture/04_devops_observability/194_blue_green_deployment_strategy/) 무중단은 레고 블록으로 성을 만들 때처럼, 규칙을 정하고 역할을 나누어 함께 작업하는 방법이에요.
