...
> **핵심 인사이트**
> 1. 상향식([[403_bottom_up_integration|Bottom-Up]]) 비용 산정은 [[149_wbs_work_breakdown_structure|WBS]]([[149_wbs_work_breakdown_structure|Work Breakdown Structure]])의 개별 작업 단위에서 시작해 합산하는 방식 — 세부 계획이 확정된 후 적용 가능하며, 하향식보다 정확하지만 시간이 많이 소요된다.
> 2. LOC(Lines of [[082_process_memory_structure|Code]], 소스 코드 라인 수)는 가장 오래되고 직관적인 소프트웨어 규모 측정 지표 — 생산성(LOC/인월)과 비용(비용/LOC)으로 산정하나, 언어·개발자 숙련도·코딩 스타일에 크게 의존하는 한계가 있다.
> 3. LOC와 [[673_function_point_ilf_eif|기능점수]]([[293_fp_function_point|FP]])의 비율은 언어별로 크게 다름 — C언어 1 [[293_fp_function_point|FP]] ≈ 100~150 LOC, Java 1 [[293_fp_function_point|FP]] ≈ 50~70 LOC, Python 1 [[293_fp_function_point|FP]] ≈ 30~50 LOC로, LOC만으로 다른 언어 프로젝트를 비교하면 오류가 생긴다.
...
> 📢 **섹션 요약 비유**: 상향식은 재료비 정확 계산 — 레시피([[149_wbs_work_breakdown_structure|WBS]])대로 재료 하나하나 가격 합산. 시간은 걸리지만 정확. 레시피도 없는 초기엔 사용 불가!
...
## Ⅲ. 언어별 LOC/[[293_fp_function_point|FP]] 비율
...
## Ⅳ. [[145_cocomo_model|COCOMO]] II LOC 기반 모델
...
> 📢 **섹션 요약 비유**: [[145_cocomo_model|COCOMO]] II는 요리 시간 계산기 — 재료 양(LOC) × 어려운 레시피(복잡도) × 초보 요리사(역량 드라이버) = 완성 시간. 변수가 많을수록 정확!
...
> 📢 **섹션 요약 비유**: SI 프로젝트 상향식은 견적서 — 화면 10개 × 요금, [[014_api_posix|API]] 20개 × 요금, 테스트 포함. 재료 하나하나 더하기. LOC 모델과 비교해서 차이 크면 이유 분석!
...
2. LOC는 코드 길이 — 소설 [[286_page_frame|페이지]] 수처럼 코드 줄 수로 크기 측정. 하지만 영어 책이랑 한자 책을 [[286_page_frame|페이지]]로 비교하면 오해!
3. [[145_cocomo_model|COCOMO]] II는 공사 견적 공식 — 건물 크기(LOC) × 공사 어려움(복잡도) × 인부 실력(역량) = 완공 기간. 변수 많을수록 정확!
...
← **이전**: [[045_top_down_estimation|045. 하향식 비용 산정 — Top-Down Estimation]]
**다음**: [[047_cocomo_model|047. COCOMO 모델 — 소프트웨어 비용 추정]] →
