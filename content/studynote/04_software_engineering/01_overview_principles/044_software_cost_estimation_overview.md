...
> **핵심 인사이트**
> 1. 소프트웨어 비용 산정은 프로젝트 시작 전 개발 공수(PM, Person-Month)·일정·원가를 예측하는 활동으로 — Boehm의 [[145_cocomo_model|COCOMO]] 연구에 따르면 [[459_quic_fec_forward_error_correction|초기]] 산정 오차가 ±4배에 달하지만, 요구사항이 명확해질수록 ±25% 이내로 수렴한다.
> 2. [[140_function_point|기능 점수]]([[293_fp_function_point|FP]] — [[140_function_point|Function Point]])와 COCOMO는 가장 널리 쓰이는 산정 기법으로 — FP는 사용자 관점의 기능 규모를 측정하는 반면, COCOMO는 LOC(Lines of [[082_process_memory_structure|Code]]) 기반 통계 모델로 규모·복잡도·팀 능력 등 비용 유발 요소(Cost Driver)를 반영한다.
> 3. 소프트웨어 산정의 근본 어려움은 "눈에 보이지 않는 제품을 미래에 만드는 비용을 현재 추정"하는 것으로 — 이 때문에 [[004_agile_relation|애자일]]은 스토리 포인트를 통한 상대적 추정과 속도(Velocity) 기반 적응형 계획으로 전통적 산정 한계를 보완한다.
...
## Ⅱ. [[140_function_point|기능 점수]] ([[293_fp_function_point|FP]])
...
> 📢 **섹션 요약 비유**: [[140_function_point|기능 점수]]는 집 평수 재기 — 방 수, 창문 수, 문 수를 세서 "평수"를 계산하는 것처럼, 입력·출력·[[501_file_definition_logical_record|파일]]·조회를 세어서 소프트웨어 크기를 수치화.

---

## Ⅲ. [[145_cocomo_model|COCOMO]] 모델
...
> 📢 **섹션 요약 비유**: [[004_agile_relation|애자일]] 산정은 상대 크기로 비교 — 특대/대/중/소로 음식 주문하듯, 스토리 포인트는 "이 기능이 저 기능의 두 배 어려워"처럼 상대적으로 크기를 측정해요.

---

## Ⅴ. 실무 시나리오 — [[081_erp_enterprise_resource_planning|ERP]] 시스템 산정
...
> 📢 **섹션 요약 비유**: [[081_erp_enterprise_resource_planning|ERP]] 비용 산정은 건축 견적 비교 — 평면도로 평수([[293_fp_function_point|FP]]) 계산하고, 자재 명세서로 공수([[145_cocomo_model|COCOMO]]) 계산해서 두 방법을 비교 [[395_verification_process_review|검증]]. 둘이 비슷하면 [[085_confidence_association_rule_conditional_probability|신뢰도]] 높아요!
...
1. 비용 산정은 집 짓기 전 견적 — 방 수, 창문 수를 세어서([[293_fp_function_point|FP]]) 인부가 얼마나 필요한지(공수) 미리 계산해요!
2. COCOMO는 벽돌 수 공식 — 벽돌 수(LOC)가 2배가 되면 인부 수도 딱 2배가 아니라 조금 더 많이 필요. 팀이 커지면 소통 비용도 늘어나니까요!
3. [[004_agile_relation|애자일]]은 상대적 크기로 — "이 기능이 저 기능의 두 배 어렵다"고 비교하면서 점점 정확한 계획을 만들어가요!
...
← **이전**: [[043_qa_vs_qc|043. QA vs QC (품질 보증 vs 품질 통제)]]
**다음**: [[045_top_down_estimation|045. 하향식 비용 산정 — Top-Down Estimation]] →
