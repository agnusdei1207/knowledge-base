...
> **핵심 인사이트**
> 1. [[033_risk_response_strategies|위험 대응 전략]]의 4가지 유형(회피·전가·완화·수용)은 위험 확률과 영향도의 매트릭스에서 최적 [[268_strategy_pattern|전략]]을 선택하며, [[147_pmbok_10_knowledge_areas|PMBOK]] 7판과 ISO 31000은 이를 [[096_risk_non_risk_architecture_evaluation_flaws|리스크]] 처리([[096_risk_non_risk_architecture_evaluation_flaws|Risk]] Treatment) 활동으로 표준화하고 있다.
> 2. 회피(Avoid)는 위험 원인을 제거하는 가장 강력한 [[268_strategy_pattern|전략]]이지만 사업 기회도 함께 포기할 수 있으며, 전가(Transfer)는 보험·계약으로 재무적 책임을 이전하지만 위험 자체는 사라지지 않는다는 점이 핵심 차이다.
> 3. 실무에서 대부분의 위험은 단일 [[268_strategy_pattern|전략]]이 아닌 복합 [[268_strategy_pattern|전략]](완화 + 수용 한계치 [[009_config|설정]])으로 관리되며, 완화 후 남는 "잔류 위험([[038_residual_risk|Residual Risk]])"에 대한 수용 한계 정의가 [[096_risk_non_risk_architecture_evaluation_flaws|리스크]] 관리 완성의 핵심이다.

---

## Ⅰ. [[033_risk_response_strategies|위험 대응 전략]] 4유형
...
> 📢 **섹션 요약 비유**: 4가지 대응 [[268_strategy_pattern|전략]]은 폭풍 대비법 — 회피(여행 취소), 전가(여행 보험), 완화(튼튼한 우산), 수용(비 맞기 각오).

---

## Ⅱ. 위험 매트릭스와 [[268_strategy_pattern|전략]] 선택
...
> 📢 **섹션 요약 비유**: 위험 매트릭스는 의사 결정 지도 — 심각도×발생 가능성으로 좌표를 찍고 "회피/완화/전가/수용" 구역 중 어디에 있는지 [[396_validation|확인]].
...
> 📢 **섹션 요약 비유**: IT 위험 관리는 여행 보험 패키지 — 항공+의료+분실 복합 보험(복합 [[268_strategy_pattern|전략]])으로 모든 상황에 대비.
...
2. 위험 점수 = 발생 가능성 × 심각도 — 점수가 높을수록 강한 [[268_strategy_pattern|전략]](회피/전가)이 필요해요.
3. 어떤 [[268_strategy_pattern|전략]]을 써도 남는 잔류 위험은 있어요 — 얼마까지 허용할지 사전에 정해두는 것이 위험 관리의 완성이에요!
...
← **이전**: [[041_risk_management_4_steps|041. 위험 관리 4단계 (Risk Management)]]
**다음**: [[043_qa_vs_qc|043. QA vs QC (품질 보증 vs 품질 통제)]] →
