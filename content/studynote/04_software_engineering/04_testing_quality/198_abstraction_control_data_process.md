...
> 1. **본질**: 추상화 (Abstraction) - 제어, 자료, 과정 추상화은(는) [[001_software_engineering_definition|소프트웨어 공학]]의 핵심 개념으로, 복잡한 시스템을 체계적으로 설계·관리하기 위한 원칙과 기법이다.
...
- 인간의 뇌는 한 번에 7개 이상의 복잡한 로직을 동시에 이해하지 못합니다([[686_cognitive_load_team_topologies|인지 부하]] 한계).
- 소프트웨어는 수백만 줄의 코드로 이루어져 있습니다.
- **추상화의 목적**: 불필요한 세부 구현(Detail)이나 찌꺼기는 밑바닥에 숨겨버리고, **본질적인 핵심 개념([[120_concept|Concept]])이나 기능(Interface)만 위로 끄집어내어 전체 시스템의 큰 그림을 쉽게 이해하고 설계할 수 있게 만드는 복잡도 관리의 최강 무기**입니다.
...
추상화 (Abstraction)을(를) 올바르게 적용하면 [[339_software_quality_definition|소프트웨어 품질]]·[[346_maintainability_portability|유지보수성]]·팀 생산성이 동시에 향상된다. 그러나 도입에는 학습 비용과 [[459_quic_fec_forward_error_correction|초기]] 투자가 필요하며, 조직 전체의 공감과 훈련이 선행되어야 한다.
...
- **📢 섹션 요약 비유**: 추상화 (Abstraction)의 기대효과는 마라톤 훈련과 같다. 처음에는 느리고 고통스럽지만, 올바른 훈련 원칙을 지킨 선수만이 결승선에서 최고의 기록을 낼 수 있다. [[001_software_engineering_definition|소프트웨어 공학]]의 원칙도 단기 편의보다 장기 완성도를 위한 투자다.
...
| 개념 | 연결 포인트 |
| :--- | :--- |
| [[001_software_engineering_definition|소프트웨어 공학]] ([[001_software_engineering_definition|Software Engineering]]) | 추상화 (Abstraction)의 상위 학문 체계이며 품질·생산성 향상의 공통 목표를 공유한다 |
| [[003_sdlc|소프트웨어 생명주기]] ([[131_sdlc_system_development_life_cycle_waterfall_agile|SDLC]], Software Development Life Cycle) | 추상화 (Abstraction)은 SDLC의 특정 단계에서 핵심적으로 적용된다 |
| 품질 보증 (QA, Quality Assurance) | 추상화 (Abstraction) 적용 결과는 QA 활동을 통해 검증되고 측정된다 |
| [[020_software_configuration_management|형상 관리]] ([[167_scm_software_configuration_management|SCM]], [[020_software_configuration_management|Software Configuration Management]]) | 추상화 (Abstraction)에서 생성된 산출물은 SCM을 통해 체계적으로 관리된다 |
...
← **이전**: [[197_fan_in_fan_out|197. 팬인 (Fan-in) / 팬아웃 (Fan-out) - 모듈 복잡도 지표]]
**다음**: [[199_information_hiding_encapsulation|199. 정보 은닉 (Information Hiding) - 내부 구현 상세를 숨김]] →
