...
> 1. **본질**: [LLMOps](/knowledge-base/studynote/12_it_management/05_security_compliance/221_llmops_large_language_model_ops/) ([Llmops](/knowledge-base/studynote/12_it_management/05_security_compliance/221_llmops_large_language_model_ops/) [Pipeline](/knowledge-base/studynote/12_it_management/02_itsm_itil/082_pipeline/)): 기존 MLOps에 더해 대규모 모델 튜닝([PEFT](/knowledge-base/studynote/10_ai/04_ai_ops_ethics/306_peft_lora/)), 프롬프트 관리, [RAG](/knowledge-base/studynote/06_ict_convergence/04_ai_llm/276_fine_tuning/) 파이프라인 지속 개선, [환각](/knowledge-base/studynote/06_ict_convergence/04_ai_llm/275_react_framework/) 모니터링 기능이 추가된 [LLM](/knowledge-base/studynote/06_ict_convergence/04_ai_llm/263_llm_large_language_model/) 특화 운영 파이프라인를 이해하는 핵심 개념으로, [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)에서 패턴을 학습해 예측·[생성](/knowledge-base/studynote/02_operating_system/02_process_thread/087_process_state_transition/)·판단 보조로 연결해야 하는 문제를 설명하는 데 쓰인다.
...
[LLMOps](/knowledge-base/studynote/12_it_management/05_security_compliance/221_llmops_large_language_model_ops/) ([Llmops](/knowledge-base/studynote/12_it_management/05_security_compliance/221_llmops_large_language_model_ops/) [Pipeline](/knowledge-base/studynote/12_it_management/02_itsm_itil/082_pipeline/)): 기존 MLOps에 더해 대규모 모델 튜닝([PEFT](/knowledge-base/studynote/10_ai/04_ai_ops_ethics/306_peft_lora/)), 프롬프트 관리, [RAG](/knowledge-base/studynote/06_ict_convergence/04_ai_llm/276_fine_tuning/) 파이프라인 지속 개선, [환각](/knowledge-base/studynote/06_ict_convergence/04_ai_llm/275_react_framework/) 모니터링 기능이 추가된 [LLM](/knowledge-base/studynote/06_ict_convergence/04_ai_llm/263_llm_large_language_model/) 특화 운영 파이프라인를 다루는 개념이다. 이 주제가 중요한 이유는 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)에서 패턴을 학습해 예측·[생성](/knowledge-base/studynote/02_operating_system/02_process_thread/087_process_state_transition/)·판단 보조로 연결해야 하는 문제를 단순한 선언이 아니라 실제 설계 항목으로 바꾸기 때문이다. 다시 말해, "왜 필요한가"를 묻는 순간 이 개념은 문제를 구조화하는 언어가 된다.
...
LLMOps의 핵심은 입력, 처리, [검증](/knowledge-base/studynote/04_software_engineering/07_object_oriented/395_verification_process_review/), 결과의 흐름을 한 세트로 보는 데 있다. 구현 기술이 달라도 결국 기존 MLOps에 더해 대규모 모델 튜닝([PEFT](/knowledge-base/studynote/10_ai/04_ai_ops_ethics/306_peft_lora/)), 프롬프트 관리, [RAG](/knowledge-base/studynote/06_ict_convergence/04_ai_llm/276_fine_tuning/) 파이프라인 지속 개선, [환각](/knowledge-base/studynote/06_ict_convergence/04_ai_llm/275_react_framework/) 모니터링 기능이 추가된 [LLM](/knowledge-base/studynote/06_ict_convergence/04_ai_llm/263_llm_large_language_model/) 특화 운영 파이프라인를 안정적으로 수행하려면 어떤 입력이 들어오고, 어떤 규칙으로 처리되며, 어떤 제어 지점에서 품질을 보장하는지가 정리되어야 한다. 이 메커니즘을 이해해야 실제 시스템에서 튜닝 포인트를 잡을 수 있다.
...
LLMOps의 경계를 드러내려면 **규칙 기반 자동화·단순 통계 모델** 과 비교하는 것이 가장 빠르다. 규칙 기반 자동화·단순 통계 모델이 익숙함과 단순성을 제공한다면, 이 개념은 정확도 향상, 자동화, 개인화 같은 가치와 설계 [일관성](/knowledge-base/studynote/05_database/04_transactions_concurrency/194_consistency_database_integrity/), 재사용성, 운영 가시성를 얻기 위해 구조적 통제를 더 가져가는 쪽에 가깝다. 차이는 기술 이름보다도 어떤 제약을 우선 해결하려는지에서 생긴다.

| 비교 항목 | [LLMOps](/knowledge-base/studynote/12_it_management/05_security_compliance/221_llmops_large_language_model_ops/) | 규칙 기반 자동화·단순 통계 모델 |
| :--- | :--- | :--- |
| 설계 초점 | 기존 MLOps에 더해 대규모 모델 튜닝([PEFT](/knowledge-base/studynote/10_ai/04_ai_ops_ethics/306_peft_lora/)), 프롬프트 관리, [RAG](/knowledge-base/studynote/06_ict_convergence/04_ai_llm/276_fine_tuning/) 파이프라인 지속 개선, [환각](/knowledge-base/studynote/06_ict_convergence/04_ai_llm/275_react_framework/) 모니터링 기능이 추가된 [LLM](/knowledge-base/studynote/06_ict_convergence/04_ai_llm/263_llm_large_language_model/) 특화 운영 파이프라인를 체계적으로 다루는 구조 | 익숙한 방식으로 빠르게 구현하는 구조 |
...
| 개념 | 연결 포인트 |
|:---|:---|
| [MLOps](/knowledge-base/studynote/12_it_management/05_security_compliance/348_mlops/) | 현재 개념이 등장하게 된 배경 또는 선행 개념이다. |
| [LLMOps](/knowledge-base/studynote/12_it_management/05_security_compliance/221_llmops_large_language_model_ops/) | [AI](/knowledge-base/studynote/04_software_engineering/03_design_architecture/190_ai_llm_requirements_specification/)·[LLM](/knowledge-base/studynote/06_ict_convergence/04_ai_llm/263_llm_large_language_model/) 맥락에서 현재 설계 판단의 중심 개념이다. |
| [모델 드리프트](/knowledge-base/studynote/06_ict_convergence/04_ai_llm/468_model_drift_retraining/) | 현재 개념을 다음 응용 단계로 연결하는 인접 개념이다. |
...
<div class="kb-diagram-row"><div class="kb-diagram-connector">▶</div><div class="kb-diagram-node">모델 드리프트</div></div>
...
← **이전**: [296. MLOps (Machine Learning Operations)](/knowledge-base/studynote/06_ict_convergence/04_ai_llm/296_mlops_machine_learning_operations/)
**다음**: [298. 모델 드리프트 (Model Drift / Data Drift)](/knowledge-base/studynote/06_ict_convergence/04_ai_llm/298_model_drift_data_drift/) →
