...
[AI](/studynote/04_software_engineering/03_design_architecture/190_ai_llm_requirements_specification/) 안전 및 레드티밍 (Red Teaming) ([AI](/studynote/04_software_engineering/03_design_architecture/190_ai_llm_requirements_specification/) Safety): [AI](/studynote/04_software_engineering/03_design_architecture/190_ai_llm_requirements_specification/) 모델 배포 전 보안/윤리 결함을 의도적으로 찾아내기 위해 공격자 관점에서 취약점(탈옥 등)을 테스트하는 기법를 다루는 개념이다. 이 주제가 중요한 이유는 [데이터](/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)에서 패턴을 학습해 예측·[생성](/studynote/02_operating_system/02_process_thread/087_process_state_transition/)·판단 보조로 연결해야 하는 문제를 단순한 선언이 아니라 실제 설계 항목으로 바꾸기 때문이다. 다시 말해, "왜 필요한가"를 묻는 순간 이 개념은 문제를 구조화하는 언어가 된다.
...
[AI](/studynote/04_software_engineering/03_design_architecture/190_ai_llm_requirements_specification/) 안전 및 레드티밍 (Red Teaming)의 핵심은 입력, 처리, [검증](/studynote/04_software_engineering/07_object_oriented/395_verification_process_review/), 결과의 흐름을 한 세트로 보는 데 있다. 구현 기술이 달라도 결국 [AI](/studynote/04_software_engineering/03_design_architecture/190_ai_llm_requirements_specification/) 모델 배포 전 보안/윤리 결함을 의도적으로 찾아내기 위해 공격자 관점에서 취약점(탈옥 등)을 테스트하는 기법를 안정적으로 수행하려면 어떤 입력이 들어오고, 어떤 규칙으로 처리되며, 어떤 제어 지점에서 품질을 보장하는지가 정리되어야 한다. 이 메커니즘을 이해해야 실제 시스템에서 튜닝 포인트를 잡을 수 있다.

| 구성 관점 | 해당 기술에서 보는 의미 | 설계 포인트 |
| :--- | :--- | :--- |
| [보호](/studynote/02_operating_system/10_security/571_protection_vs_security/) 자산 | [AI](/studynote/04_software_engineering/03_design_architecture/190_ai_llm_requirements_specification/) 안전 및 레드티밍 (Red Teaming)가 [보호](/studynote/02_operating_system/10_security/571_protection_vs_security/)하거나 [검증](/studynote/04_software_engineering/07_object_oriented/395_verification_process_review/)해야 하는 [데이터](/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)·권한·상태를 정한다. | 자산 경계를 먼저 문서화한다. |
...
| 비교 항목 | [AI](/studynote/04_software_engineering/03_design_architecture/190_ai_llm_requirements_specification/) 안전 및 레드티밍 (Red Teaming) | 사후 대응 중심 운영 |
| :--- | :--- | :--- |
| 설계 초점 | [AI](/studynote/04_software_engineering/03_design_architecture/190_ai_llm_requirements_specification/) 모델 배포 전 보안/윤리 결함을 의도적으로 찾아내기 위해 공격자 관점에서 취약점(탈옥 등)을 테스트하는 기법를 체계적으로 다루는 구조 | 익숙한 방식으로 빠르게 구현하는 구조 |
...
| 연결 관점 | [벡터 인덱싱](/studynote/06_ict_convergence/04_ai_llm/300_ann_approximate_nearest_neighbor_vector_index/)를 배경으로 [프롬프트 인젝션 공격](/studynote/06_ict_convergence/04_ai_llm/302_prompt_injection_jailbreak/)로 확장된다 | 독립 운영은 쉬우나 구조 확장성은 제한될 수 있다 |

또한 [벡터 인덱싱](/studynote/06_ict_convergence/04_ai_llm/300_ann_approximate_nearest_neighbor_vector_index/)는 왜 이 주제가 등장했는지 보여 주는 선행 개념이고, [프롬프트 인젝션 공격](/studynote/06_ict_convergence/04_ai_llm/302_prompt_injection_jailbreak/)는 실제 [서비스](/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/) 확장 또는 세부 기술로 이어지는 인접 개념이다. 시험 답안에서는 이런 연결선을 함께 말해야 현재 개념의 위치가 살아난다.
...
| 개념 | 연결 포인트 |
|:---|:---|
| [벡터 인덱싱](/studynote/06_ict_convergence/04_ai_llm/300_ann_approximate_nearest_neighbor_vector_index/) | 현재 개념이 등장하게 된 배경 또는 선행 개념이다. |
...
| [프롬프트 인젝션 공격](/studynote/06_ict_convergence/04_ai_llm/302_prompt_injection_jailbreak/) | 현재 개념을 다음 응용 단계로 연결하는 인접 개념이다. |
...
이 흐름도는 [벡터 인덱싱](/studynote/06_ict_convergence/04_ai_llm/300_ann_approximate_nearest_neighbor_vector_index/)에서 출발해 현재 개념을 거쳐 [프롬프트 인젝션 공격](/studynote/06_ict_convergence/04_ai_llm/302_prompt_injection_jailbreak/)와 [멀티모달](/studynote/14_data_engineering/03_ml_dl_llm/158_multimodal_clip_vision_audio_encoding/) 방향으로 확장되는 학습 흐름을 보여 준다. 즉, 현재 개념은 독립된 섬이 아니라 앞 개념의 문제를 받아 다음 단계의 설계 선택으로 넘겨 주는 연결 고리다.
...
<- **이전**: [300. 벡터 인덱싱 (ANN: Approximate Nearest Neighbor)](/studynote/06_ict_convergence/04_ai_llm/300_ann_approximate_nearest_neighbor_vector_index/)
**다음**: [302. 프롬프트 인젝션 공격 (Prompt Injection / Jailbreak)](/studynote/06_ict_convergence/04_ai_llm/302_prompt_injection_jailbreak/) ->
