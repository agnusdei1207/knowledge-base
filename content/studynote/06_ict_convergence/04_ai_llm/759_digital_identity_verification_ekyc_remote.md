...
그러나 원격 인증은 다음 4가지 위협 모델을 내포한다: ① 신분증 위·변조(포토샵, 딥페이크 신분증), ② 프레젠테이션 공격(인쇄 사진, 리플레이, 3D 마스크, 실리콘 마스크, GAN 생성 얼굴), ③ 모핑 공격(두 인물의 얼굴을 융합한 신분증 변조), ④ 합성 신원(Synthetic Identity) 사기. 기술사 관점에서 eKYC는 **단일 솔루션이 아닌 규제·위협·UX의 균형점**이며, 모든 신원요소를 100% 원격 검증하는 것은 불가능에 가깝다(특히 정부급 신원증명서 위변조).
