...
> 3. **판단 포인트**: 기술사적 핵심은 **MLOps 성숙도 Level 0/1/2(Google 기준)** 중 어디까지 도입할지, **배치 vs 실시간 스트리밍 추론 아키텍처** 선택, **Shadow/Canary/A/B Test 배포 전략**, **Data Drift·Concept Drift·Label Drift** 3종 감지 임계치 설정, 그리고 **GPU 자원 스케줄링(Karpenter+Volcano vs Kubeflow TrainerServer)**과 **Feature Store 일관성(Training-Serving Skew 방지)** 판단이다.
