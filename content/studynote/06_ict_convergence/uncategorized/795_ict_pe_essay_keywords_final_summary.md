...
> 2. **가치**: 키워드 간 인과관계(예: NFV -> Microservice -> Service Mesh -> mTLS -> Zero Trust)와 정량 지표(지연 p99, RTO/RPO, SLA 99.999%, CAP theorem 수렴)를 도식화·비교표로 표현하면 채점관에게 **“설계 의사결정의 합리성”**을 한 화면에 입증할 수 있어 고득점 차별화가 가능하며, 출제 트렌드 5개년(2020~2024) 분석 시 **“AI·플랫폼·지속가능성(Sovereign/ESG-Tech)”**이 80% 이상을 차지한다.
> 3. **판단 포인트**: 키워드 나열식 답변은 탈락 요인이며, **(a) 요구사항 도출 -> (b) 후보 아키텍처 비교 -> (c) 표준/규제 매핑 -> (d) 운영·보안·DR 설계 -> (e) 성과 KPI(예: MTTR 50%v, 비용 30%v) 정량화**의 5-Step 서술 구조를 채택해야 기술사 등급의 “공학적 판단력”이 평가된다.
...
```text
[ICT 융합 기술사 논술 키워드 4-Layer 컨버지드 아키텍처]
...
기존(2010년대) ICT 기술사 답안은 **“단일 시스템의 성능·안정성”**을 묻는 폐쇄형 구조였으나, 현재는 **“상호의존 시스템의 회복탄력성·거버넌스”**를 평가하는 개방형 융합 문제로 전환되었다. 예컨대 “스마트 팩토리”라는 단일 키워드도 5G 슬라이싱 + MEC + OPC-UA + TSN + Digital Twin + AI 추론 + IEC 62443 보안의 7-Layer 통합 설계가 요구된다.
