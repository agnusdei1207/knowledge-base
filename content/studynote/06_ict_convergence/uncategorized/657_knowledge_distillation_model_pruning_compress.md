...
> 3. **판단 포인트**: Teacher-Student capacity gap이 클수록 증류 효율 저하(soft label 모호성), 프루닝 비율·sparsity 패턴·fine-tuning 회복 곡선 간의 trade-off, 그리고 hard label(GT) 손실과 soft label 손실의 가중치(α)·temperature(T)·feature transfer layer 설계가 성능을 결정하며, 이는 도메인·하드웨어 타깃(Edge GPU vs Mobile NPU vs MCU)에 따라 달라지는 기술사 핵심 판단 포인트이다.
