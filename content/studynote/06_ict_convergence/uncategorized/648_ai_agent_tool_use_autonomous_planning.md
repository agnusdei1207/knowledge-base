...
> 3. **판단 포인트**: 핵심 트레이드오프는 **①도구 호출 신뢰성 vs 도구 개수(할루시네이션 지수함수적 증가)**, **②계획 정밀도(Plan-and-Execute) vs 적응성(ReAct)**, **③토큰 비용·지연시간 vs 작업 성공률**, **④보안 샌드박스 vs 기능 확장성**이며, 기술사적 판단 기준은 **도메인 결정성(Determinism)**, **에러 전파 한계 설계**, **관측가능성(Tracing) 인프라** 확보 여부이다.
