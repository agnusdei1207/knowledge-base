...
LLM 기반 서비스(ChatGPT 스타일의 멀티턴 대화, RAG 기반 Q&A, Code Completion, Embedding 서비스)의 운영 비용에서 **GPU Inference 비용이 60~80%**를 차지하며, 70B~405B급 모델의 경우 H100 1장당 시간당 수십만 원의 비용이 발생한다. 기술사 출제 관점에서 이는 단순한 "성능 개선"이 아니라 **"비용 효율성, SLA 보장, 모델 거버넌스"**를 동시에 충족해야 하는 아키텍처 의사결정이다.
...
추론 최적화 A/B 테스트 시스템은 **4계층(4-Layer Architecture)**으로 구성된다. 각 계층은 명확한 책임 분리를 가지며, 기술사 답안 작성 시 "결합도(Coupling) 낮음, 응집도(Cohesion) 높음" 원칙을 적용해야 한다.
