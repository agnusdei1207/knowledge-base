...
> 3. **판단 포인트**: **프라이버시-유용성-성능(통신/연산)-신뢰 가정의 4차원 트레이드오프**가 핵심 의사결정 변수이며, 데이터 민감도·참여자 수·연산 빈도·규제 등급에 따라 DP ε값(ε≤1 강력, 1~5 보통, 5~10 약함), HE 스킴(BFV/BGV/CKKS/TFHE), 동기/비동기 FL 토폴로지, TEE(Intel SGX/AMD SEV/ARM CCA) 배치 등 하이브리드 아키텍처 설계가 기술사의 핵심 판단 영역이다.
