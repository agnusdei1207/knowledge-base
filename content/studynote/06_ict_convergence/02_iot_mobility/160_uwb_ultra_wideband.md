...
UWB의 핵심 원리는 비행 시간 (ToF, Time of Flight) 측정이다. 송신기와 수신기가 아주 짧은 펄스를 주고받으며, [신호](/knowledge-base/studynote/02_operating_system/02_process_thread/130_signal/)가 오가는 데 걸린 시간을 나노초 (ns, nanosecond) 단위로 계산한다. 전파는 빛의 속도로 이동하므로, 이 시간을 기반으로 두 장치 사이 거리를 매우 정밀하게 추정할 수 있다. 여기에 양방향 거리 측정 (TWR, Two-Way Ranging), 도착 각도 (AoA, Angle of Arrival), 도착 시간 차 (TDoA, Time Difference of Arrival)를 조합하면 위치 추정까지 가능해진다.
...
| 요소 | 역할 | 설계 포인트 |
| :--- | :--- | :--- |
| 임펄스 [신호](/knowledge-base/studynote/02_operating_system/02_process_thread/130_signal/) | 짧은 시간폭으로 송신 | 높은 시간 분해능 확보 |
| 넓은 [대역폭](/knowledge-base/studynote/01_computer_architecture/03_architecture_basics_performance/140_bandwidth/) | 세밀한 거리 추정 기반 | 스펙트럼 규제와 채널 계획 고려 |
...
UWB를 이해하려면 [BLE](/knowledge-base/studynote/03_network/12_iot_wpan_edge/607_ble_bluetooth_low_energy_iot/), 근거리 무선통신 (NFC, Near Field Communication), 와이파이 [RTT](/knowledge-base/studynote/03_network/08_transport_layer/441_rtt_round_trip_time_srtt_smoothed/) ([Round Trip Time](/knowledge-base/studynote/03_network/08_transport_layer/441_rtt_round_trip_time_srtt_smoothed/))와의 차이를 함께 봐야 한다. BLE는 저전력 비콘과 연결성에 강하지만 위치 [정밀도](/knowledge-base/studynote/14_data_engineering/05_exam_keywords/233_precision_recall_f1_roc_auc_threshold/)는 환경 영향을 크게 받는다. NFC는 수 센티미터 거리의 접촉형 [인증](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/303_authentication_authorization_patterns/)에는 강하나, 거리와 방향을 연속적으로 추적하기에는 적합하지 않다. 와이파이 RTT는 넓은 커버리지를 활용할 수 있으나, UWB만큼의 세밀한 거리 정확도를 내기 어렵다.

| 항목 | [BLE](/knowledge-base/studynote/03_network/12_iot_wpan_edge/607_ble_bluetooth_low_energy_iot/) | NFC | UWB |
| :--- | :--- | :--- | :--- |
| 주 용도 | 광고·연결·근접 추정 | 접촉형 [인증](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/303_authentication_authorization_patterns/) | 정밀 거리 측정·위치 추정 |
| 거리 판단 방식 | 주로 RSSI 기반 | 매우 근접 접촉 | ToF 기반 정밀 측정 |
| [정밀도](/knowledge-base/studynote/14_data_engineering/05_exam_keywords/233_precision_recall_f1_roc_auc_threshold/) | 미터급 수준 | 매우 근접 판별 | 수 센티미터급 가능 |
| 대표 활용 | 비콘, 웨어러블 | 결제, 출입 | 디지털 키, RTLS, 스마트 태그 |

이 차이는 보안과도 연결된다. 디지털 키에서 BLE만 사용하면 [신호](/knowledge-base/studynote/02_operating_system/02_process_thread/130_signal/) 중계로 "근처에 있는 척"하는 공격에 취약할 수 있다. 반면 UWB는 실제 전파 전파 시간을 측정하므로, 단순 중계 공격에 더 강한 구조를 만들 수 있다. 그래서 최근 자동차와 스마트폰 생태계는 [BLE](/knowledge-base/studynote/03_network/12_iot_wpan_edge/607_ble_bluetooth_low_energy_iot/) 연결성과 UWB 정밀 [인증](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/303_authentication_authorization_patterns/)을 함께 쓰는 방향으로 발전하고 있다.
...
다만 UWB 도입은 칩 하나 붙인다고 끝나지 않는다. 앵커 배치가 나쁘면 [정밀도](/knowledge-base/studynote/14_data_engineering/05_exam_keywords/233_precision_recall_f1_roc_auc_threshold/)가 급격히 떨어지고, 금속 구조물과 다중 경로 ([Multipath](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/500_multipath_io/)) 반사가 심하면 보정이 필요하다. 또한 배터리 기반 태그에서는 측위 주기와 전력 소모의 균형을 잡아야 하며, 운영체제와 스마트폰 칩셋이 UWB를 지원하는지도 반드시 확인해야 한다.

### 판단 [체크리스트](/knowledge-base/studynote/04_software_engineering/11_testing_validation/435_checklist_based_testing/)

1. [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/) 목표가 단순 연결인지, 센티미터급 위치 판별인지 명확한가?
2. 앵커 설치 위치와 [동기화](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/212_synchronization_mechanisms/) 품질을 확보할 수 있는가?
3. 태그 배터리 수명과 측위 빈도의 균형이 맞는가?
4. [BLE](/knowledge-base/studynote/03_network/12_iot_wpan_edge/607_ble_bluetooth_low_energy_iot/), NFC, 위성항법장치 (GNSS, Global Navigation Satellite System)와 역할 분담이 설계되었는가?

### [안티패턴](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/128_water_scrum_fall_anti_pattern/)

- [BLE](/knowledge-base/studynote/03_network/12_iot_wpan_edge/607_ble_bluetooth_low_energy_iot/) 한계 때문에 UWB를 도입하면서도 위치 기준점 설계를 생략하는 것
- 실내 반사 환경을 무시하고 이론 거리 정확도만 믿는 것
- 단말 [호환성](/knowledge-base/studynote/04_software_engineering/06_software_architecture/344_compatibility_usability/) 검토 없이 [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/) 기획을 먼저 고정하는 것
...
| 디지털 키 (Digital [Key](/knowledge-base/studynote/05_database/02_modeling_normalization/067_db_key_uniqueness_minimality/)) | UWB가 보안성과 편의성을 동시에 제공하는 대표 사례 |
