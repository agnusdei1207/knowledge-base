...
> 1. **본질**: 앰비언트 백스캐터 통신 (Ambient Backscatter Communication)은 센서가 스스로 전파를 새로 만들지 않고, 공기 중에 이미 존재하는 RF (Radio Frequency) [신호](/knowledge-base/studynote/02_operating_system/02_process_thread/130_signal/)를 반사·흡수하는 방식을 바꿔 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 실어 보내는 무전원 통신이다.
> 2. **가치**: 배터리 교체가 사실상 불가능한 구조물 내부, 물류 태그, 웨어러블, 체내 센서 같은 영역에서 유지보수 비용과 크기 제약을 동시에 줄여 준다.
> 3. **판단 포인트**: 주변 전파 세기, 수신기 위치, 요구 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)량, 허용 지연시간이 맞을 때만 의미가 있으며, 장거리·고속·고신뢰 링크까지 대체하는 기술은 아니다.

---

## Ⅰ. 개요 및 필요성

앰비언트 백스캐터 통신은 주변에 이미 존재하는 텔레비전 방송, Wi-Fi, 셀룰러 [신호](/knowledge-base/studynote/02_operating_system/02_process_thread/130_signal/) 같은 전파를 "남이 쏘는 [반송파](/knowledge-base/studynote/03_network/01_data_communication/054_반송파_Carrier_Wave/)"로 활용하는 통신 방식이다. 센서는 능동 송신기처럼 발진기 (Oscillator)로 전파를 새로 만들지 않고, [안테나](/knowledge-base/studynote/03_network/03_physical_layer_media/171_antenna_basic_dipole_resonance/) 부하를 바꿔 반사 특성만 조절함으로써 자신의 [비트](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/073_bit/)를 실어 보낸다. 이 때문에 전력 소모를 극단적으로 낮출 수 있다.
...
| :--- | :--- | :--- | :--- |
| 주변 전파원 | TV, Wi-Fi, 셀룰러 등 기존 RF [신호](/knowledge-base/studynote/02_operating_system/02_process_thread/130_signal/) 제공 | 지속성, 주파수 특성, 세기 | 전파가 약하면 센서 기동 자체가 어려움 |
| 에너지 수집부 | RF 에너지를 정류해 커패시터에 저장 | 정류 효율, 기동 [전압](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/001_voltage/), 누설 [전류](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/002_current/) | 센서가 충분히 깨어나지 못함 |
...
앰비언트 백스캐터는 수동형 RFID (Radio-Frequency [Identification](/knowledge-base/studynote/03_network/06_network_layer_ip/289_identification_flags_fragmentation_offset/)), 저전력 무선, 지능형 반사 표면과 자주 비교된다. 이들과의 차이를 이해해야 "무전원"이라는 말에 과도한 기대를 얹지 않게 된다.

| 비교 축 | 수동형 RFID | 앰비언트 백스캐터 | [BLE](/knowledge-base/studynote/03_network/12_iot_wpan_edge/607_ble_bluetooth_low_energy_iot/) / [LoRa](/knowledge-base/studynote/03_network/12_iot_wpan_edge/617_lora_lorawan_css_chirp_spread_spectrum/) 등 능동 무선 |
| :--- | :--- | :--- | :--- |
| 전파 공급 방식 | 전용 리더가 의도적으로 전파 공급 | 주변에 이미 존재하는 전파 활용 | 단말이 직접 전파 [생성](/knowledge-base/studynote/02_operating_system/02_process_thread/087_process_state_transition/) |
...
2. 요구 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)량이 매우 작고, 저속 전송으로도 업무 가치가 충분한가?
...
| [임피던스](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/004_impedance/) 변조 ([Impedance](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/004_impedance/) Modulation) | 반사 특성 변화로 [비트](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/073_bit/)를 실어 보내는 핵심 메커니즘 |
| 수동형 RFID | 반사 기반 통신이라는 공통점이 있으나 전용 리더 의존성이 다름 |
| [BLE](/knowledge-base/studynote/03_network/12_iot_wpan_edge/607_ble_bluetooth_low_energy_iot/) ([Bluetooth Low Energy](/knowledge-base/studynote/03_network/12_iot_wpan_edge/607_ble_bluetooth_low_energy_iot/)) / [LoRa](/knowledge-base/studynote/03_network/12_iot_wpan_edge/617_lora_lorawan_css_chirp_spread_spectrum/) | 배터리를 쓰는 저전력 능동 무선과의 비교 기준 |
| RIS (Reconfigurable Intelligent Surface) | 전파 반사 제어라는 물리 계층 아이디어를 공유하는 확장 개념 |
| 게이트웨이 / 수신기 | 약한 반사파를 직통파에서 분리해 복조하는 핵심 장치 |
| 무전원 [센서 네트워크](/knowledge-base/studynote/06_ict_convergence/02_iot_mobility/103_wsn_sensor_network/) | 앰비언트 백스캐터가 지향하는 대표 응용 영역 |
...
2. 그래서 건전지가 거의 없어도 아주 작은 [신호](/knowledge-base/studynote/02_operating_system/02_process_thread/130_signal/)를 보낼 수 있어요.
