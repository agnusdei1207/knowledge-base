...
> 2. **가치**: 유선망 설치가 불가능하거나 사람이 접근하기 힘든 재난 지역, 교량, 숲 등에서 광범위한 환경 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 실시간으로 감지하고 중계할 수 있는 유일한 수단을 제공한다.
> 3. **판단 포인트**: WSN 설계의 성패는 통신 대역폭이나 속도가 아니라, 극단적으로 제한된 배터리 자원을 얼마나 오래 유지할 수 있는가(에너지 효율과 [라우팅](/knowledge-base/studynote/03_network/07_network_layer_routing/339_routing_overview_best_path_selection/) [프로토콜](/knowledge-base/studynote/03_network/06_network_layer_ip/295_protocol_field_tcp_udp_icmp/))에 달려 있다.

---

## Ⅰ. 개요 및 필요성

무선 센서 네트워크 (WSN)는 온도, 습도, 진동 등 물리적 상태를 측정하는 수십~수만 개의 센서 노드(Mote)가 특정 지역에 무작위로 살포되어 구축되는 네트워크 망이다. 이 노드들은 스스로 주변 노드를 탐색하고 통신망을 형성하여 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 최종 목적지인 기지국(Sink Node)으로 전달한다.

과거에는 환경을 측정하기 위해 일일이 사람이 센서를 설치하고 유선으로 연결해야 했으나, 광활한 농경지나 군사 위험 지역에서는 물리적으로 불가능했다. 인프라가 없는 곳(Infrastructure-less)에서도 기기들이 알아서 망을 구성하고 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 전달하는 기술이 필요해졌고, 이를 위해 저전력 무선 통신과 임베디드 기술이 결합하면서 WSN이 등장하게 되었다.
...
| :--- | :--- | :--- |
| **센서 노드 (Sensor Node)** | 환경 감지, [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 처리, 패킷 무선 중계 | 극단적인 전력 제한, 제한된 메모리/CPU |
...
- **📢 섹션 요약 비유**: WSN의 통신 방식은 마라톤 대신 릴레이 경주를 하는 것과 같다. 한 사람이 끝까지 달리면 금방 지쳐 쓰러지기 때문에, 조금씩 달려서 다음 사람에게 바통([데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/))을 넘겨주며 체력을 아낀다.
...
WSN은 단순한 통신망을 넘어, 물리적 세상의 정보를 디지털 세상으로 끌어오는 [사물인터넷](/knowledge-base/studynote/06_ict_convergence/02_iot_mobility/101_iot_concept/)([IoT](/knowledge-base/studynote/06_ict_convergence/02_iot_mobility/101_iot_concept/), Internet of Things)의 최전선 감각 기관 역할을 수행한다. WSN이 없으면 [IoT](/knowledge-base/studynote/06_ict_convergence/02_iot_mobility/101_iot_concept/) 플랫폼은 분석할 기초 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)조차 얻을 수 없다.
...
WSN을 통해 우리는 교량의 미세한 균열, 공장 파이프의 [가스](/knowledge-base/studynote/06_ict_convergence/01_blockchain/024_gas/) 누출, 산림의 화재 징후 등 육안으로 확인하기 힘든 물리적 변화를 실시간으로 감지하고 즉각 대응할 수 있는 지능형 인프라를 확보하게 되었다. 이는 재난 예방과 운영 비용 절감으로 직결된다.

한계점으로는 노드의 연산 및 전력 한계로 인해 대용량 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)(영상 등) 전송에 부적합하다는 점이다. 그러나 [저전력 광역 통신망](/knowledge-base/studynote/06_ict_convergence/02_iot_mobility/109_lpwan_low_power_wide_area_network/)([LPWAN](/knowledge-base/studynote/06_ict_convergence/02_iot_mobility/109_lpwan_low_power_wide_area_network/)) 기술과 에너지 하베스팅(Energy Harvesting, 태양광/진동 발전) 기술이 발전함에 따라, 미래의 WSN은 배터리 교체 없이 영구적으로 동작하는 진정한 유비쿼터스 센싱 망으로 진화할 것이다.
