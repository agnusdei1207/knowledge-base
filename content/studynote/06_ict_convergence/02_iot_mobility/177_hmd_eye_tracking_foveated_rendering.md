...
> 2. **가치**: 같은 패널 해상도와 프레임률에서도 [GPU](/knowledge-base/studynote/01_computer_architecture/12_accelerators_ai_hardware/418_gpu/) ([Graphics Processing Unit](/knowledge-base/studynote/01_computer_architecture/12_accelerators_ai_hardware/418_gpu/)) 셰이딩 부하, 발열, 배터리 소모를 크게 낮추면서 체감 화질은 유지할 수 있어, 독립형 XR (Extended Reality) 기기의 현실적 한계를 넘게 해 준다.
> 3. **판단 포인트**: 효과는 눈 추적 정확도·[지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/)시간·예측 보정·프라이버시 통제가 충분할 때만 난다. 시선 추적이 늦거나 흔들리면 사용자는 중심부 선명 영역이 따라오지 못하는 [아티팩트](/knowledge-base/studynote/15_devops_sre/02_cicd_gitops/075_artifact_management_nexus_docker_registry/)를 바로 눈치챈다.
...
| 단계 | 핵심 기술 | 실무 포인트 |
| :--- | :--- | :--- |
| 안구 캡처 | [IR](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/165_ir/) (Infrared) LED와 고속 카메라로 동공·글린트 추적 | 120~240Hz급 샘플링, 안경·속눈썹 반사 대응 |
| 시선 추정 | 동공 중심, 각막 반사, 보정 모델로 Gaze Vector 계산 | 사용자별 보정, IPD (Interpupillary Distance) 차이 반영 |
| 예측·안정화 | 미세 떨림과 사카드 (Saccade) 고려한 필터링 | [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/) 보상, 흔들림 [억제](/knowledge-base/studynote/09_security/13_secops_ir_forensics/656_ir_containment/) |
...
| :--- | :--- | :--- | :--- | :--- |
| 전체 고정 고품질 렌더링 | 없음 | 구현 단순, 품질 균일 | 전력·발열 부담 큼 | 유선 [PC](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/164_pc/) VR, 고성능 [GPU](/knowledge-base/studynote/01_computer_architecture/12_accelerators_ai_hardware/418_gpu/) |
...
2. 시선 추정부터 패널 출력까지의 총 [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/)이 멀미 임계와 품질 임계 안에 있는가?
3. 안경, 렌즈, 속눈썹, 조명 반사, 눈 크기 차이에 대한 보정·재보정 체계가 있는가?
4. 추적 실패 시 고정 포비티드 또는 전체 균일 렌더링으로 안전하게 [폴백](/knowledge-base/studynote/07_enterprise_systems/03_eai_esb_msa/171_fallback_resilience_pattern/)할 수 있는가?
5. 시선 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 분석 목적으로 외부 전송할 때 명시적 동의와 최소 수집 원칙을 지키는가?
...
HMD 시선 추적과 포비티드 렌더링이 잘 구현되면 얻는 효과는 명확하다. 같은 칩셋으로 더 높은 체감 화질과 프레임률을 확보할 수 있고, 발열과 전력 소모를 줄여 착용 시간을 늘릴 수 있으며, 더 가벼운 배터리·냉각 설계가 가능해진다. 여기에 응시 기반 UI까지 결합되면 컨트롤러 의존도를 낮춘 [공간 컴퓨팅](/knowledge-base/studynote/12_it_management/05_security_compliance/232_spatial_computing_digital_twin/) 경험도 구현할 수 있다.
...
| VRS (Variable Rate Shading) | 영역별 셰이딩 밀도를 다르게 주는 핵심 [GPU](/knowledge-base/studynote/01_computer_architecture/12_accelerators_ai_hardware/418_gpu/) 기능 |
| MTP (Motion-to-Photon) [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/)시간 | 멀미와 몰입감에 직접 연결되는 XR [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) 지표 |
| 리프로젝션 (Reprojection) / 타임워프 (Timewarp) | [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/)을 보정해 장면 안정성을 높이는 출력 기술 |
| Gaze UI | 응시 기반 선택·조준 인터페이스로 확장되는 활용 영역 |
| 클라우드 XR | 시선 정보를 네트워크·스트리밍 최적화와 결합하는 확장 방향 |
| 프라이버시 [보호](/knowledge-base/studynote/02_operating_system/10_security/571_protection_vs_security/) | 시선 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 수집과 활용의 윤리·보안 기준 |
