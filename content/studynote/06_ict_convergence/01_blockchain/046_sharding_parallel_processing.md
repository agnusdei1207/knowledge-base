...
> **핵심 인사이트**
> 1. [[004_blockchain|블록체인]] [[280_sharding|샤딩]]([[243_sharding_horizontal_scaling_database|Sharding]])은 네트워크를 여러 샤드(Shard)로 분할해 각 샤드가 독립적으로 [[191_transaction_concept_states|트랜잭션]]을 처리하는 수평 확장 기법 — 비트코인/이더리움이 모든 노드가 모든 [[191_transaction_concept_states|트랜잭션]]을 [[395_verification_process_review|검증]]하는 구조의 TPS 한계를 돌파하기 위해 도입한다.
> 2. [[040_blockchain_trilemma|블록체인 트릴레마]](Trilemma)와 [[280_sharding|샤딩]] — [[010_decentralization|탈중앙화]]·보안·확장성 세 가지를 동시에 완벽히 달성하기 어렵다는 Vitalik의 트릴레마에서, [[280_sharding|샤딩]]은 확장성을 높이지만 단일 샤드 공격(1% 공격)·크로스샤드 통신 비용이라는 새 도전을 만든다.
> 3. Ethereum의 Danksharding(2024~)은 실행 [[280_sharding|샤딩]] 대신 [[094_data_availability_da_layer_celestia|데이터 가용성]] [[280_sharding|샤딩]] — [[191_transaction_concept_states|트랜잭션]] 실행은 L2 [[042_rollup_l2_solution|롤업]]이 담당하고, 이더리움은 [[042_rollup_l2_solution|롤업]] [[001_dikw_pyramid|데이터]]를 [[136_variance|분산]] 저장하는 "[[001_dikw_pyramid|데이터]] 레이어" 역할에 집중하는 설계 방향이다.

---

## Ⅰ. [[004_blockchain|블록체인]] 확장성 문제
...
> 📢 **섹션 요약 비유**: [[004_blockchain|블록체인]] 확장성 문제는 단일 계산원 — 슈퍼마켓에 계산원 1명(모든 노드 동일 [[395_verification_process_review|검증]]), 줄이 길어도 더 빠른 계산원을 쓸 수 없어요. [[280_sharding|샤딩]]은 계산대 여러 개 열기!

---

## Ⅱ. [[280_sharding|샤딩]] 메커니즘
...
> 📢 **섹션 요약 비유**: [[004_blockchain|블록체인]] [[280_sharding|샤딩]]은 은행 지점 분리 — 전국 거래를 본점(단일 체인) 하나에서 처리하다가, 지역별 지점(샤드)이 독립적으로 처리. 지점 간 송금(크로스샤드)만 복잡!
...
> 📢 **섹션 요약 비유**: Danksharding은 창고(이더리움)→배송 네트워크 전환 — 이더리움이 직접 배달(실행) 대신, 배달부(L2 [[042_rollup_l2_solution|롤업]]) 위탁 + 창고([[001_dikw_pyramid|데이터]] 저장)만 집중. 효율 폭발!

---

## Ⅴ. 실무 시나리오 — Zilliqa [[280_sharding|샤딩]]
...
> 📢 **섹션 요약 비유**: Zilliqa [[280_sharding|샤딩]]은 최초 실험실 — 이론을 실제로 구현한 선구자. [[191_transaction_concept_states|트랜잭션]] [[280_sharding|샤딩]]으로 TPS 향상. Ethereum은 이 실험을 참고해 더 실용적인 [[042_rollup_l2_solution|롤업]]+DAS로 진화!
