...
> 3. Groth16이 현재 가장 효율적인 zk-SNARK 구현(증명 크기 192바이트, [검증](/studynote/04_software_engineering/07_object_oriented/395_verification_process_review/) 3ms)이며, Ethereum [zkEVM](/studynote/06_ict_convergence/01_blockchain/074_zkevm_zero_knowledge_ethereum_virtual_machine/)(zkSync, [Polygon](/studynote/06_ict_convergence/01_blockchain/045_sidechain_bridge_polygon/) [zkEVM](/studynote/06_ict_convergence/01_blockchain/074_zkevm_zero_knowledge_ethereum_virtual_machine/))의 핵심 암호 기반으로 Layer 2 스케일링의 보안을 보장한다.

---

## I. 대화형 vs 비대화형 [ZKP](/studynote/12_it_management/05_security_compliance/354_did_decentralized_identity_zkp/)
...
| 단계     | 입력                    | 출력       | 수행자    |
|--------|------------------------|---------|--------|
| [설정](/studynote/15_devops_sre/01_culture_methodology/009_config/)    | 비밀 파라미터 τ          | (pk, vk)| 신뢰 제3자|
| 증명    | witness + input + pk   | π       | 증명자   |
| [검증](/studynote/04_software_engineering/07_object_oriented/395_verification_process_review/)    | π + input + vk         | True/False| [검증](/studynote/04_software_engineering/07_object_oriented/395_verification_process_review/)자  |

> �� **섹션 요약 비유**: [CRS](/studynote/09_security/05_web_app_security/243_owasp_core_rule_set_crs_waf_anomaly_scoring/) 생성은 열쇠 공장 설립(독성 폐기물=마스터키 파괴), 증명은 자물쇠 만들기, [검증](/studynote/04_software_engineering/07_object_oriented/395_verification_process_review/)은 열쇠로 열어보기.
...
## [IV](/studynote/03_network/06_network_layer_ip/288_version_ihl_tos_total_length/). PLONK와 범용 [설정](/studynote/15_devops_sre/01_culture_methodology/009_config/)
...
> 📢 **섹션 요약 비유**: Groth16은 맞춤 정장(회로마다 재단), PLONK는 기성복(범용 [설정](/studynote/15_devops_sre/01_culture_methodology/009_config/)으로 어느 회로든) — 편의성과 크기의 트레이드오프.
...
> 📢 **섹션 요약 비유**: zkSync는 수천 명의 정산을 공인 회계사(zk-SNARK)가 [검증](/studynote/04_software_engineering/07_object_oriented/395_verification_process_review/)한 한 장의 확인서로 제출하는 것 — $1로 수천 건 [검증](/studynote/04_software_engineering/07_object_oriented/395_verification_process_review/).
...
3. 이 덕분에 수천 건의 거래를 단 1달러짜리 [검증](/studynote/04_software_engineering/07_object_oriented/395_verification_process_review/) 하나로 처리해서 블록체인을 100배 이상 빠르게 만들 수 있어요!
...
<- **이전**: [037. ZKP — 영지식 증명 (Zero-Knowledge Proof)](/studynote/06_ict_convergence/01_blockchain/037_zero_knowledge_proof_zkp/)
**다음**: [039. 레이어1 메인넷 (Layer 1 Mainnet) — 블록체인 기반 계층](/studynote/06_ict_convergence/01_blockchain/039_layer1_mainnet/) ->
