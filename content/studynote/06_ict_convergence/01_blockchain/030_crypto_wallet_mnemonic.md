...
## 핵심 인사이트 (3줄 요약)
> 1. **본질**: 암호화폐 지갑(Crypto Wallet)은 [[004_blockchain|블록체인]] 자산의 소유권을 증명하는 개인키(Private [[067_db_key_uniqueness_minimality|Key]])를 안전하게 보관하는 도구다. 지갑이 "돈을 보관"하는 게 아니라 "소유권 증명서(개인키)를 보관"한다.
...
> 3. **판단 포인트**: 니모닉 12~24단어는 자산의 모든 것이다. 누출 시 모든 자산 탈취 가능, 분실 시 [[658_ir_recovery|복구]] 불가능하다. 니모닉 보안이 곧 자산 보안이며 이것이 "Not your keys, not your coins"의 의미다.
...
### BIP-39 니모닉 [[087_process_state_transition|생성]] 과정
...
- **📢 섹션 요약 비유**: 멀티시그는 핵 발사 코드 2인 [[303_authentication_authorization_patterns|인증]] 시스템이다. 한 사람(한 키)만으로는 발사([[191_transaction_concept_states|트랜잭션]])가 불가능하고, 2명 이상이 동시에 동의해야 한다.
...
| **[[287_interoperability_tactics|상호운용성]]** | BIP-44로 멀티체인 지갑 지원 |
| **[[658_ir_recovery|복구]] 가능성** | 니모닉으로 어느 기기에서도 [[658_ir_recovery|복구]] |

MPC(Multi-Party Computation, 다자 계산) 지갑이 차세대 기술로 부상 중이다. 개인키를 [[136_variance|분산]] 저장하여 어느 한 곳에도 완전한 키가 존재하지 않아 탈취 위험을 원천 차단한다. 기관 투자자용 커스터디 서비스에서 표준으로 자리 잡고 있다.

- **📢 섹션 요약 비유**: MPC 지갑은 비밀 레시피를 3등분해서 서로 다른 금고에 보관하는 것이다. 3분의 1씩만 알고 있는 상태에서도 협력하면 요리([[191_transaction_concept_states|트랜잭션]] 서명)가 가능하다.
...
| **멀티시그** | 다중 서명 보안 강화 |
| **MPC 지갑** | 차세대 [[136_variance|분산]] 키 관리 |
...
← **이전**: [[029_nft_non_fungible_token|29. NFT — 대체 불가 토큰 (Non-Fungible Token)]]
**다음**: [[031_mnemonic_seed_phrase|31. 니모닉 시드 구문 심화 — BIP-39 보안과 파생]] →
