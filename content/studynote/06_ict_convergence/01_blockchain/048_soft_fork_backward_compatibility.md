...
> 2. 소프트 포크의 핵심은 "규칙을 강화(Tighten)"하는 것 — 이전에 허용되던 [[191_transaction_concept_states|트랜잭션]] 유형을 새 규칙에서 제한하거나, 기존 필드를 재해석하여 새 기능을 추가한다. 구버전 노드는 새 블록이 기존 규칙을 여전히 따르는지만 검증하므로 분기 없이 동작한다.
> 3. 비트코인의 SegWit(Segregated Witness)이 소프트 포크의 대표 사례 — [[191_transaction_concept_states|트랜잭션]] 서명 [[001_dikw_pyramid|데이터]]를 별도 영역으로 분리하여 사실상 블록 크기를 늘리면서도 구버전 노드와의 호환성을 유지한 기술적 성취다.
...
> 📢 **섹션 요약 비유**: SegWit = 여권 수하물 분리 — 여권(거래 [[001_dikw_pyramid|데이터]])과 수하물(서명)을 분리해서 같은 비행기(블록)에 더 많은 여행자([[191_transaction_concept_states|트랜잭션]]) 탑승. 수하물 요금 할인(서명 [[001_dikw_pyramid|데이터]] [[267_weight_bias_activation|가중치]] 1/4)!
...
> 📢 **섹션 요약 비유**: Taproot = 비밀 금고 업그레이드 — 복잡한 잠금장치([[022_smart_contract|스마트 컨트랙트]])가 있어도 모두 동의하면 일반 열쇠(단순 서명)처럼 사용. 복잡한 계약도 외부에선 평범해 보임(프라이버시)!
...
← **이전**: [[047_hard_fork_chain_split|047. 하드 포크 — Hard Fork & Chain Split]]
**다음**: [[049_cross_chain_bridge|049. 크로스체인 브릿지 — Cross-Chain Bridge]] →
