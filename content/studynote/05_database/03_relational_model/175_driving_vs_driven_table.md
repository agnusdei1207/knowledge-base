...
드라이빙/드리븐 개념이 가장 선명하게 드러나는 곳은 [중첩 루프 조인](/knowledge-base/studynote/05_database/03_relational_model/172_nl_join_nested_loop/)이다. 엔진은 보통 **드라이빙 접근 → 조건 필터링 → 각 행마다 드리븐 접근 → 결과 결합** 순서로 움직인다. 이때 드리븐 테이블은 반복 접근을 버텨야 하므로, 조인 키 [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/)나 매우 효율적인 접근 경로가 준비되어 있어야 한다.

| 역할 | 엔진 입장 의미 | 이상적인 조건 | 잘못 잡았을 때 문제 |
| :--- | :--- | :--- | :--- |
| 드라이빙 테이블 | 반복문의 바깥쪽 집합 | 필터 후 결과 건수가 매우 적음 | 반복 횟수 자체가 커짐 |
| 드리븐 테이블 | 반복문 안쪽에서 매번 탐색되는 대상 | 조인 키 [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/), 빠른 랜덤 접근 | [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/) 없으면 반복 풀스캔 위험 |
| 조인 조건 | 두 집합을 묶는 연결 키 | 높은 [선택도](/knowledge-base/studynote/05_database/03_relational_model/170_selectivity_cardinality_distribution_tuning/), 통계 [정확성](/knowledge-base/studynote/16_bigdata/01_intro/002_bigdata_5v/) | 잘못된 [기수](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/077_radix/) 추정으로 순서 오판 |
| [옵티마이저](/knowledge-base/studynote/05_database/03_relational_model/163_optimizer_sql_execution_plan_generator/) 통계 | 어느 쪽을 먼저 읽을지 판단하는 근거 | 최신 통계 정보 유지 | 잘못된 드라이빙 선택 |
...
| :--- | :--- | :--- |
| `FROM` 절 작성 순서 | 사람이 SQL을 적은 문장 순서 | [옵티마이저](/knowledge-base/studynote/05_database/03_relational_model/163_optimizer_sql_execution_plan_generator/)는 이를 바꿔 다른 드라이빙을 선택할 수 있음 |
| [조인 순서](/knowledge-base/studynote/05_database/03_relational_model/176_join_order_optimization/) ([Join Order](/knowledge-base/studynote/05_database/03_relational_model/176_join_order_optimization/)) | 여러 테이블을 어떤 순서로 묶을지 | 드라이빙 선택은 [조인 순서](/knowledge-base/studynote/05_database/03_relational_model/176_join_order_optimization/)를 구성하는 핵심 결정 |
...
실무에서 드라이빙 테이블 판단은 "어느 테이블이 더 작으냐"보다 "어느 쪽을 먼저 읽으면 전체 탐색 횟수를 가장 많이 줄일 수 있느냐"로 내려야 한다. 예를 들어 `CUSTOMER` 1,000만 건, `ORDERS` 5억 건이 있어도 `CUSTOMER.grade = 'VIP'` 조건으로 200건만 남고 `ORDERS.customer_id` [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/)가 잘 잡혀 있다면 `CUSTOMER`를 드라이빙으로 두는 것이 자연스럽다. 반대로 `ORDERS.order_date = today`로 3,000건만 남고 고객 조건이 약하다면, 훨씬 큰 원본 테이블이라도 `ORDERS`가 드라이빙이 될 수 있다.

[힌트](/knowledge-base/studynote/05_database/03_relational_model/167_sql_hint_optimizer_override/)는 마지막 수단이어야 한다. [Oracle](/knowledge-base/studynote/05_database/03_relational_model/188_pl_sql_t_sql_procedural/) 기준으로 `LEADING`, `USE_NL` 같은 [힌트](/knowledge-base/studynote/05_database/03_relational_model/167_sql_hint_optimizer_override/)로 [조인 순서](/knowledge-base/studynote/05_database/03_relational_model/176_join_order_optimization/)를 고정할 수는 있지만, 통계가 오래됐거나 [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/)가 부적절한 상태를 [힌트](/knowledge-base/studynote/05_database/03_relational_model/167_sql_hint_optimizer_override/)로 덮으면 특정 시점에만 빨랐던 [실행 계획](/knowledge-base/studynote/05_database/03_relational_model/166_execution_plan_optimizer_navigation_tree/)을 영구 고정하는 부작용이 생긴다. 먼저 할 일은 최신 통계 확보, 조건 푸시다운, 불필요 컬럼 제거, 드리븐 [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/) 정비다.
...
6. [힌트](/knowledge-base/studynote/05_database/03_relational_model/167_sql_hint_optimizer_override/)를 [쓰기](/knowledge-base/studynote/13_cloud_architecture/05_data_engineering/289_cqrs_db/) 전에 조인 방식 자체가 맞는지 검토했는가?
...
- 드리븐 [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/) 없이 [중첩 루프 조인](/knowledge-base/studynote/05_database/03_relational_model/172_nl_join_nested_loop/)을 강제로 유도하는 경우
- `FROM A, B`에서 A를 먼저 썼다고 A가 항상 드라이빙이라고 믿는 경우
- 통계 부정확 문제를 해결하지 않고 [힌트](/knowledge-base/studynote/05_database/03_relational_model/167_sql_hint_optimizer_override/)로만 [조인 순서](/knowledge-base/studynote/05_database/03_relational_model/176_join_order_optimization/)를 봉인하는 경우

기술사 관점에서 기억할 핵심 문장은 이렇다. **드라이빙 테이블은 조인의 반복 횟수를 결정하고, 드리븐 테이블은 그 반복을 감당할 액세스 경로를 제공해야 한다.** 둘 중 하나만 맞아도 부족하고, 둘이 함께 맞아야 실제 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/)이 나온다.
...
| 개념 | 연결 포인트 |
| :--- | :--- |
| [중첩 루프 조인](/knowledge-base/studynote/05_database/03_relational_model/172_nl_join_nested_loop/) (NL [Join](/knowledge-base/studynote/05_database/04_transactions_concurrency/521_join/), [Nested Loop Join](/knowledge-base/studynote/05_database/07_exam_summary/431_nested_loop_join/)) | 드라이빙/드리븐 개념이 가장 직접적으로 드러나는 조인 방식 |
| [선택도](/knowledge-base/studynote/05_database/03_relational_model/170_selectivity_cardinality_distribution_tuning/) ([Selectivity](/knowledge-base/studynote/05_database/03_relational_model/170_selectivity_cardinality_distribution_tuning/)) | 어떤 조건이 드라이빙 집합을 얼마나 작게 줄이는지 판단하는 기준 |
| [기수](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/077_radix/)성 (Cardinality) | 필터 후 예상 행 수로, [조인 순서](/knowledge-base/studynote/05_database/03_relational_model/176_join_order_optimization/) 결정의 핵심 입력 |
| [조인 순서](/knowledge-base/studynote/05_database/03_relational_model/176_join_order_optimization/) ([Join Order](/knowledge-base/studynote/05_database/03_relational_model/176_join_order_optimization/)) | 여러 테이블 조합에서 드라이빙 후보를 어떤 순서로 잡을지 정하는 문제 |
| 액세스 경로 (Access Path) | 드리븐 테이블이 [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/) 탐색인지 풀스캔인지 결정하는 요소 |
| [비용 기반 옵티마이저](/knowledge-base/studynote/05_database/03_relational_model/165_cbo_cost_based_optimizer/) (CBO, Cost-Based [Optimizer](/knowledge-base/studynote/12_it_management/02_itsm_itil/088_optimizer/)) | 통계 정보를 기반으로 드라이빙/드리븐과 조인 방식을 선택하는 주체 |
| [해시 조인](/knowledge-base/studynote/05_database/03_relational_model/174_hash_join/) ([Hash Join](/knowledge-base/studynote/05_database/03_relational_model/174_hash_join/)) | 대량 동등 조인에서 NL [Join](/knowledge-base/studynote/05_database/04_transactions_concurrency/521_join/) 대신 고려되는 다른 물리 조인 방식 |
...
← **이전**: [174. 해시 조인 (Hash Join) - 작은 테이블로 해시 맵 생성 후 큰 테이블 탐색, 대량 데이터/동등(=) 조인 전용, 성능](/knowledge-base/studynote/05_database/03_relational_model/174_hash_join/)
**다음**: [176. 조인 순서 (Join Order) 최적화 - 동적 계획법 (Dynamic Programming), 탐욕 알고리즘](/knowledge-base/studynote/05_database/03_relational_model/176_join_order_optimization/) →
