...
> 1. **본질**: [비용 기반 옵티마이저](/knowledge-base/studynote/05_database/03_relational_model/165_cbo_cost_based_optimizer/) (CBO, Cost-Based [Optimizer](/knowledge-base/studynote/12_it_management/02_itsm_itil/088_optimizer/))는 같은 조인 SQL (Structured Query Language) 이라도 예상 결과 건수, [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/) 유무, 정렬 상태, 메모리 여유에 따라 [Nested Loop](/knowledge-base/studynote/05_database/07_exam_summary/431_nested_loop_join/) (NL) [Join](/knowledge-base/studynote/05_database/04_transactions_concurrency/521_join/), [Hash Join](/knowledge-base/studynote/05_database/03_relational_model/174_hash_join/), [Sort Merge Join](/knowledge-base/studynote/05_database/03_relational_model/173_sort_merge_join/) 중 다른 물리 연산을 선택한다.
> 2. **가치**: 조인 방식이 바뀌면 랜덤 입출력 (I/O, Input/Output) 중심의 탐색이 될지, 순차 스캔 + 메모리 매칭이 될지, [정렬 후 병합](/knowledge-base/studynote/05_database/07_exam_summary/432_sort_merge_join/)이 될지가 갈리므로 응답시간과 서버 자원 소비가 크게 달라진다.
> 3. **판단 포인트**: 소량 선별 결과 + 조인 [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/)면 NL [Join](/knowledge-base/studynote/05_database/04_transactions_concurrency/521_join/), 대량 동등 조인 + 충분한 메모리면 [Hash Join](/knowledge-base/studynote/05_database/03_relational_model/174_hash_join/), 이미 정렬된 대량 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)이거나 정렬 결과를 재활용할 수 있으면 Sort Merge Join이 유리하며, 잘못된 통계는 이 판단을 가장 쉽게 무너뜨린다.

---

## Ⅰ. 개요 및 필요성

[옵티마이저](/knowledge-base/studynote/05_database/03_relational_model/163_optimizer_sql_execution_plan_generator/) 조인 기법 3가지는 <strong><a href="/knowledge-base/studynote/09_security/04_endpoint_security/369_logic_bomb/">논리</a>적으로 같은 조인 연산을 실제로 어떤 순서와 자원 패턴으로 수행할지 결정하는 물리 실행 방식</strong>이다. [관계](/knowledge-base/studynote/05_database/02_modeling_normalization/083_relationship_in_er_model/)형 [데이터베이스 관리 시스템](/knowledge-base/studynote/05_database/01_db_architecture_relational/003_dbms_database_management_system/) (RDBMS, Relational [Database](/knowledge-base/studynote/05_database/04_transactions_concurrency/501_database/) [Management](/knowledge-base/studynote/12_it_management/05_security_compliance/1013_management/) System) 에서 `A JOIN B ON ...` 는 [논리](/knowledge-base/studynote/09_security/04_endpoint_security/369_logic_bomb/)식 하나로 보이지만, 엔진 내부에서는 "한 건씩 [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/)를 찌를지", "작은 집합을 [해시 테이블](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/067_hash_table/)로 만들지", "양쪽을 정렬해 앞에서부터 맞출지"를 반드시 정해야 한다.
...
| :--- | :--- | :--- | :--- |
| [Nested Loop](/knowledge-base/studynote/05_database/07_exam_summary/431_nested_loop_join/) (NL) [Join](/knowledge-base/studynote/05_database/04_transactions_concurrency/521_join/) | 외부 집합 행마다 내부 집합을 반복 탐색 | 외부 결과가 매우 적고, 내부 조인 컬럼 [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/)가 좋을 때 | 반복 [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/) 탐색에 따른 랜덤 I/O |
| [Hash Join](/knowledge-base/studynote/05_database/03_relational_model/174_hash_join/) | 작은 입력을 메모리 [해시 테이블](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/067_hash_table/)로 만들고 큰 입력을 탐색 | 대량 동등 조인, 풀 스캔이 유리하고 메모리가 충분할 때 | 해시 빌드 공간 부족 시 디스크 스필 |
| [Sort Merge Join](/knowledge-base/studynote/05_database/03_relational_model/173_sort_merge_join/) | 양쪽 입력을 조인 키 기준으로 정렬한 뒤 병합 | 이미 정렬된 대량 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/), 정렬 결과 재활용 가능할 때 | 정렬 비용과 임시 공간 사용 |

[Nested Loop Join](/knowledge-base/studynote/05_database/07_exam_summary/431_nested_loop_join/) 의 핵심은 "작은 쪽을 운전석으로 삼아 큰 쪽을 필요한 만큼만 찌른다"는 점이다. 그래서 외부 집합이 수십 행 수준으로 작고 내부 테이블에 조인 [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/)가 있으면 매우 빠르다. 하지만 외부 결과가 커지면 그만큼 내부 탐색도 반복되므로, [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/)가 있어도 랜덤 I/O 누적 비용이 급격히 커질 수 있다.

[Hash Join](/knowledge-base/studynote/05_database/03_relational_model/174_hash_join/) 은 작은 입력을 먼저 읽어 메모리 안에 [해시 테이블](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/067_hash_table/)을 만들고, 큰 입력을 순차 스캔하며 매칭한다. 이 방식은 [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/)보다 메모리를 활용하는 [전략](/knowledge-base/studynote/04_software_engineering/04_testing_quality/268_strategy_pattern/)이라서 대용량 동등 조인에 강하다. 다만 빌드 입력이 예상보다 크거나 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 쏠림이 심하면 해시 영역이 디스크로 밀려나 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/)이 흔들릴 수 있다.

[Sort Merge Join](/knowledge-base/studynote/05_database/03_relational_model/173_sort_merge_join/) 은 양쪽을 같은 키 순서로 정렬해 놓고, 앞에서부터 비교하며 병합한다. 정렬 비용이 선행되지만, 이미 [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/) 정렬이나 상위 연산의 요구 때문에 순서가 맞춰져 있다면 의외로 효율적이다. 특히 조인 후 바로 `ORDER BY`, `GROUP BY`, 스트림 병합이 이어지는 경우에는 정렬 비용을 여러 연산이 같이 써서 손해를 줄일 수 있다.
...
| 강한 상황 | 소량 조회, 높은 [선택도](/knowledge-base/studynote/05_database/03_relational_model/170_selectivity_cardinality_distribution_tuning/), 내부 [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/) 존재 | 대량 동등 조인, 순차 스캔 유리 | 이미 정렬된 입력, 정렬 재활용 가능 |
| 약한 상황 | 외부 집합이 커질 때 | 메모리 부족, [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 쏠림 심할 때 | 정렬이 새로 필요할 때 |
| 대표 업무 | 온라인 [트랜잭션](/knowledge-base/studynote/05_database/04_transactions_concurrency/191_transaction_concept_states/) 처리 ([OLTP](/knowledge-base/studynote/05_database/06_dw_olap_trends/327_hint_handoff/), Online [Transaction](/knowledge-base/studynote/05_database/04_transactions_concurrency/191_transaction_concept_states/) Processing) | [데이터 웨어하우스](/knowledge-base/studynote/12_it_management/05_security_compliance/209_data_warehouse_schema_on_write/) ([DW](/knowledge-base/studynote/12_it_management/05_security_compliance/209_data_warehouse_schema_on_write/), [Data Warehouse](/knowledge-base/studynote/14_data_engineering/05_exam_keywords/208_data_warehouse_schema_on_write_inmon/)), 배치 분석 | 대량 병합, 정렬 연계 작업 |
| 통계 민감도 | [조인 순서](/knowledge-base/studynote/05_database/03_relational_model/176_join_order_optimization/) 오판에 매우 민감 | 빌드/프로브 크기 추정 오류에 민감 | 정렬 필요성 판단 오류에 민감 |

이 세 방식은 앞 문서의 [선택도](/knowledge-base/studynote/05_database/03_relational_model/170_selectivity_cardinality_distribution_tuning/), [기수](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/077_radix/)성, 분포도와 직접 연결된다. [선택도](/knowledge-base/studynote/05_database/03_relational_model/170_selectivity_cardinality_distribution_tuning/)가 낮아 외부 결과가 소량이면 NL [Join](/knowledge-base/studynote/05_database/04_transactions_concurrency/521_join/) 이 좋아질 가능성이 크고, [선택도](/knowledge-base/studynote/05_database/03_relational_model/170_selectivity_cardinality_distribution_tuning/)가 높아 대량 집합이 남으면 [Hash Join](/knowledge-base/studynote/05_database/03_relational_model/174_hash_join/) 이 유리해진다. 또 값 분포가 치우쳐 있으면 해시 버킷 편향이나 [조인 순서](/knowledge-base/studynote/05_database/03_relational_model/176_join_order_optimization/) 오판이 생겨, 같은 [Hash Join](/knowledge-base/studynote/05_database/03_relational_model/174_hash_join/) 이라도 예상보다 느려질 수 있다.
...
실무에서는 조인 방식 자체보다 **왜 그 방식이 선택되었는지 설명할 수 있어야** 한다. 예를 들어 회원 한 명의 주문 이력을 보는 화면에서 `member_id = :id` 로 이미 1건 수준으로 선별되었다면, 주문 테이블 [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/)를 활용하는 NL [Join](/knowledge-base/studynote/05_database/04_transactions_concurrency/521_join/) 이 자연스럽다. 반면 월말 정산처럼 판매 사실 테이블 수억 건과 고객 [차원 테이블](/knowledge-base/studynote/07_enterprise_systems/05_data_bi/273_dimension_table_analysis_perspective/) 수천만 건을 동등 조인하는 배치는 [Hash Join](/knowledge-base/studynote/05_database/03_relational_model/174_hash_join/) 이 기본 후보가 된다.

[Sort Merge Join](/knowledge-base/studynote/05_database/03_relational_model/173_sort_merge_join/) 은 종종 "애매한 옛 방식"으로 오해받지만, 대량 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)가 이미 조인 키 순서로 정렬되어 있거나 결과를 곧바로 정렬된 상태로 넘겨야 할 때는 여전히 의미가 있다. 예를 들어 [파티션](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/) 병합, 대량 [로그](/knowledge-base/studynote/04_software_engineering/09_cloud_native_ai_architecture/568_logs_distributed_logging_elk_fluentd/) 병합, 정렬 기반 집계 앞단에서는 정렬 비용을 여러 연산이 나눠 쓸 수 있기 때문이다.

### 기술사 판단 [체크리스트](/knowledge-base/studynote/04_software_engineering/11_testing_validation/435_checklist_based_testing/)

1. 필터 이후 실제로 남는 외부 집합은 몇 행인가?
2. 내부 집합 조인 컬럼에 탐색 가능한 [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/)가 있는가?
...
- 통계가 오래되어 작은 집합을 큰 집합으로 오판한 상태에서 [힌트](/knowledge-base/studynote/05_database/03_relational_model/167_sql_hint_optimizer_override/)만 덮어쓰는 경우
- [Hash Join](/knowledge-base/studynote/05_database/03_relational_model/174_hash_join/) 빌드 입력이 메모리에 안 들어가는데도 강제로 사용해 스필을 키우는 경우
...
[옵티마이저](/knowledge-base/studynote/05_database/03_relational_model/163_optimizer_sql_execution_plan_generator/) 조인 기법 3가지를 제대로 이해하면 [실행 계획](/knowledge-base/studynote/05_database/03_relational_model/166_execution_plan_optimizer_navigation_tree/)에서 조인 연산자를 보는 순간, 왜 그 [쿼리](/knowledge-base/studynote/10_ai/04_ai_ops_ethics/298_qkv_attention/)가 빠르거나 느린지 설명할 수 있게 된다. 이는 단순 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) 개선을 넘어, 계획 변동 원인 분석, [힌트](/knowledge-base/studynote/05_database/03_relational_model/167_sql_hint_optimizer_override/) 최소화, 통계 관리 우선순위 [설정](/knowledge-base/studynote/15_devops_sre/01_culture_methodology/009_config/)으로 이어진다.
...
| 개념 | 연결 포인트 |
| :--- | :--- |
| [비용 기반 옵티마이저](/knowledge-base/studynote/05_database/03_relational_model/165_cbo_cost_based_optimizer/) (CBO, Cost-Based [Optimizer](/knowledge-base/studynote/12_it_management/02_itsm_itil/088_optimizer/)) | [조인 순서](/knowledge-base/studynote/05_database/03_relational_model/176_join_order_optimization/)와 조인 방식을 비용으로 비교해 선택 |
| [선택도](/knowledge-base/studynote/05_database/03_relational_model/170_selectivity_cardinality_distribution_tuning/) ([Selectivity](/knowledge-base/studynote/05_database/03_relational_model/170_selectivity_cardinality_distribution_tuning/)) | 외부 입력 크기를 줄여 NL [Join](/knowledge-base/studynote/05_database/04_transactions_concurrency/521_join/) 유불리를 바꿈 |
| [기수](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/077_radix/)성 (Cardinality) | 조인 결과 행 수 예측의 출발점 |
| [클러스터링 팩터](/knowledge-base/studynote/05_database/03_relational_model/169_clustering_factor_index_physical_sort/) ([Clustering Factor](/knowledge-base/studynote/05_database/03_relational_model/169_clustering_factor_index_physical_sort/)) | [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/) 기반 NL [Join](/knowledge-base/studynote/05_database/04_transactions_concurrency/521_join/) 의 실제 블록 접근 비용에 영향 |
| 해시 작업 영역 | [Hash Join](/knowledge-base/studynote/05_database/03_relational_model/174_hash_join/) 의 빌드 테이블 적재 가능 여부를 좌우 |
| 정렬 재활용 | [Sort Merge Join](/knowledge-base/studynote/05_database/03_relational_model/173_sort_merge_join/) 의 비용을 상쇄하는 핵심 조건 |
