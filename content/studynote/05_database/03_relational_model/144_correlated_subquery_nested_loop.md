...
## 핵심 인사이트 (3줄 요약)
> 1. **본질**: 상관 서브쿼리는 **내부 서브쿼리가 외부 [쿼리](/knowledge-base/studynote/10_ai/04_ai_ops_ethics/298_qkv_attention/)의 행을 [참조](/knowledge-base/studynote/05_database/05_distributed_nosql_newsql/316_reference_pattern_nosql/)**하여, 외부 [쿼리](/knowledge-base/studynote/10_ai/04_ai_ops_ethics/298_qkv_attention/)의 **각 행마다 서브쿼리가 반복 실행**되는 구조이며, Nested Loop와 유사한 O(N×M) 특성을 가진다.
> 2. **가치**: "각 직원이 자기 부서 평균보다 높은가?"처럼 **행 단위 비교**를 직관적으로 표현할 수 있지만, 대량 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)에서 **[성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/)이 급격히 나빠진다**.
> 3. **판단 포인트**: [옵티마이저](/knowledge-base/studynote/05_database/03_relational_model/163_optimizer_sql_execution_plan_generator/)가 자동으로 **Semi-[Join](/knowledge-base/studynote/05_database/04_transactions_concurrency/521_join/)·Anti-Join으로 변환**하는 경우가 많지만, 변환되지 않으면 JOIN이나 Window Function으로 수동 대체한다.
...
상관 서브쿼리는 **직관적이지만 O(N×M) 주의**이며, [Window Function](/knowledge-base/studynote/05_database/03_relational_model/139_window_function_analytics/)·JOIN이 효율적 대안이다.
...
| **[Nested Loop](/knowledge-base/studynote/05_database/07_exam_summary/431_nested_loop_join/)** | 반복 실행 |
