...
> 3. **판단 포인트**: [옵티마이저](/studynote/05_database/03_relational_model/163_optimizer_sql_execution_plan_generator/)가 자동으로 <strong>Semi-<a href="/studynote/05_database/04_transactions_concurrency/521_join/">Join</a>·Anti-Join으로 변환</strong>하는 경우가 많지만, 변환되지 않으면 JOIN이나 Window Function으로 수동 대체한다.
...
상관 서브쿼리는 <strong>직관적이지만 O(N×M) 주의</strong>이며, [Window Function](/studynote/05_database/03_relational_model/139_window_function_analytics/)·JOIN이 효율적 대안이다.
...
| <strong><a href="/studynote/05_database/07_exam_summary/431_nested_loop_join/">Nested Loop</a></strong> | 반복 실행 |
...
| **실행계획** | EXPLAIN [확인](/studynote/04_software_engineering/12_testing_maintenance/396_validation/) 필수 |
...
<- **이전**: [143. 중첩 서브쿼리 & WHERE EXISTS 심화](/studynote/05_database/03_relational_model/143_nested_subquery_where_exists/)
**다음**: [145. SQL Window Function 심화 - ROWS/RANGE Frame & 누적합](/studynote/05_database/03_relational_model/145_sql_window_function_analytics/) ->
