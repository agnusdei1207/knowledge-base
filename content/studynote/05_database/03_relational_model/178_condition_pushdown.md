...
> 1. **본질**: 조건 푸시 다운 (Condition Pushdown)은 바깥 질의의 필터 조건을 [인라인 뷰](/knowledge-base/studynote/05_database/03_relational_model/141_inline_view_subquery/)나 서브쿼리 안쪽으로 전달해, 같은 결과를 더 작은 중간 결과로 계산하게 만드는 [옵티마이저](/knowledge-base/studynote/05_database/03_relational_model/163_optimizer_sql_execution_plan_generator/) 재작성이다.
> 2. **가치**: 조인·정렬·집계 전에 행 수를 먼저 줄이면 디스크 I/O (Input/Output), 메모리, 임시 영역 사용량이 함께 감소하므로, 느린 [쿼리](/knowledge-base/studynote/10_ai/04_ai_ops_ethics/298_qkv_attention/)를 종종 "늦게 거르던 구조"에서 "일찍 거르는 구조"로 바꿀 수 있다.
...
[옵티마이저](/knowledge-base/studynote/05_database/03_relational_model/163_optimizer_sql_execution_plan_generator/)는 `d.dept_name = 'IT개발부'` 조건을 이용해, 내부적으로는 아래와 비슷한 형태를 고려할 수 있다.
...
| :--- | :--- | :--- | :--- |
| [뷰 머징](/knowledge-base/studynote/05_database/03_relational_model/177_view_merging_query_transformation/) ([View Merging](/knowledge-base/studynote/05_database/03_relational_model/177_view_merging_query_transformation/)) | 경계 제거 | 질의 블록 자체를 하나로 합침 | [단순 뷰](/knowledge-base/studynote/05_database/03_relational_model/152_simple_view_vs_complex_view/), 전역 조인 재배치가 중요한 경우 |
...
5. 통계 정보와 [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/) 상태가 최신인가?
6. [힌트](/knowledge-base/studynote/05_database/03_relational_model/167_sql_hint_optimizer_override/) 사용 전에 SQL 구조 단순화와 조건 식 재작성부터 검토했는가?

### 자주 나오는 [안티패턴](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/128_water_scrum_fall_anti_pattern/)

- 읽기 좋게 만든 뷰 위에 조건을 몰아놓고, 뷰 내부는 아무 필터 없이 둔 채 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) 저하를 방치하는 경우
- 집계 결과 조건과 그룹 키 조건을 구분하지 않고 "다 내려가겠지"라고 가정하는 경우
- `DISTINCT`나 불필요한 정렬을 습관처럼 넣어 푸시다운 여지를 스스로 없애는 경우
- 통계 불량 상태에서 [힌트](/knowledge-base/studynote/05_database/03_relational_model/167_sql_hint_optimizer_override/)만으로 푸시다운을 강제해 계획을 불안정하게 만드는 경우

결국 조건 푸시 다운의 실무 판단은 "필터를 더 안쪽으로 보낼 수 있는가?"보다 "**보내도 의미가 안 바뀌고, 보내면 실제 비용이 줄어드는가?**"를 [확인](/knowledge-base/studynote/04_software_engineering/12_testing_maintenance/396_validation/)하는 일이다. 기술사 답안에서도 무조건 가능/불가로 단정하기보다, 안전성·[선택도](/knowledge-base/studynote/05_database/03_relational_model/170_selectivity_cardinality_distribution_tuning/)·후속 최적화 효과를 함께 설명해야 깊이가 생긴다.

- **📢 섹션 요약 비유**: 장부를 빨리 찾고 싶다고 도서관 [분류](/knowledge-base/studynote/16_bigdata/05_analysis/104_classification_analysis/) 체계를 무시하면 안 된다. 빨리 찾되, 서가 체계가 망가지지 않는 선에서 가장 앞단에서 걸러야 한다.

---

## Ⅴ. 기대효과 및 결론

조건 푸시 다운이 잘 작동하면 조인 이전의 행 수가 줄어들어 디스크 I/O (Input/Output), 메모리 사용량, 임시 정렬 공간, 네트워크 전송량까지 함께 줄어든다. 특히 대용량 분석 SQL, 뷰 기반 보고서, 원격 테이블 접근, 대형 조인 [쿼리](/knowledge-base/studynote/10_ai/04_ai_ops_ethics/298_qkv_attention/)에서는 "같은 결과를 더 빨리" 만드는 대표적인 재작성 효과를 볼 수 있다.
...
| 개념 | 연결 포인트 |
| :--- | :--- |
| [비용 기반 옵티마이저](/knowledge-base/studynote/05_database/03_relational_model/165_cbo_cost_based_optimizer/) (CBO, Cost-Based [Optimizer](/knowledge-base/studynote/12_it_management/02_itsm_itil/088_optimizer/)) | 푸시다운 가능성과 비용 절감 효과를 판단하는 주체 |
| [뷰 머징](/knowledge-base/studynote/05_database/03_relational_model/177_view_merging_query_transformation/) ([View Merging](/knowledge-base/studynote/05_database/03_relational_model/177_view_merging_query_transformation/)) | 경계를 제거하는 형제 재작성 기법 |
| 머티리얼라이제이션 (Materialization) | 경계를 유지하고 중간 결과를 저장하는 대비 [전략](/knowledge-base/studynote/04_software_engineering/04_testing_quality/268_strategy_pattern/) |
| [조인 순서](/knowledge-base/studynote/05_database/03_relational_model/176_join_order_optimization/) 최적화 ([Join Order Optimization](/knowledge-base/studynote/05_database/03_relational_model/176_join_order_optimization/)) | 푸시다운 이후 더 좋아질 수 있는 후속 최적화 |
| [기수](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/077_radix/)성 (Cardinality) 추정 | 푸시다운 이득을 계산하는 통계 기반 |
| SARGable 조건 | 푸시다운 이후 [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/) 접근 이점을 살리기 쉬운 조건 형태 |
...
2. [데이터베이스](/knowledge-base/studynote/05_database/01_db_architecture_relational/002_database_definition/)도 필요한 것만 먼저 골라 놓으면 뒤에서 섞고 계산하는 일이 훨씬 쉬워져요.
