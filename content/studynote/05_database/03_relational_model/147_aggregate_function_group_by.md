...
집계 함수는 SQL에서 다수의 행을 하나의 요약 값으로 [압축](/knowledge-base/studynote/02_operating_system/06_memory_management/347_compaction/)하는 연산이다. 단일 행 함수(UPPER, SUBSTR 등)가 행마다 독립적으로 동작하는 것과 달리, 집계 함수는 **행 집합 전체를 입력으로 받아 스칼라(Scalar) 값 하나를 반환**한다.
...
집계 함수와 윈도우 함수([Window Function](/knowledge-base/studynote/05_database/03_relational_model/139_window_function_analytics/))는 모두 집계 연산을 수행하지만, 핵심 차이는 **행의 수**다.
...
**집계 [쿼리](/knowledge-base/studynote/10_ai/04_ai_ops_ethics/298_qkv_attention/) [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) 병목은 대부분 GROUP BY의 정렬·해시 연산**: 집계 대상 열에 [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/)가 있어도 GROUP BY는 전체 행을 처리해야 한다. [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)가 수억 건이면 집계 [쿼리](/knowledge-base/studynote/10_ai/04_ai_ops_ethics/298_qkv_attention/) 자체보다 **사전 집계(Materialized [View](/knowledge-base/studynote/05_database/03_relational_model/151_sql_view_virtual_table/), [파티셔닝](/knowledge-base/studynote/05_database/03_relational_model/179_table_partitioning_concept/), 요약 테이블)** 가 필수다.

### [안티패턴](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/128_water_scrum_fall_anti_pattern/)

**SELECT에 [GROUP BY](/knowledge-base/studynote/05_database/04_transactions_concurrency/522_group_by/) 미포함 열 사용 (MySQL 비표준 동작)**: 표준 SQL에서는 `GROUP BY` 절에 없는 열을 `SELECT`에 쓸 수 없다. MySQL의 `only_full_group_by` 비활성화 시 비결정적 값이 반환되어 잘못된 분석 결과를 낳는다. 반드시 표준 SQL 모드를 사용해야 한다.
...
집계 함수는 SQL의 분석 능력을 애플리케이션 레이어에서 [데이터베이스](/knowledge-base/studynote/05_database/01_db_architecture_relational/002_database_definition/) 엔진 레이어로 끌어내려, **RDBMS의 최적화 엔진이 집계 연산을 가장 효율적으로 수행**할 수 있게 한다. [쿼리](/knowledge-base/studynote/10_ai/04_ai_ops_ethics/298_qkv_attention/) [옵티마이저](/knowledge-base/studynote/05_database/03_relational_model/163_optimizer_sql_execution_plan_generator/)는 [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/), [파티션](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/), [병렬](/knowledge-base/studynote/05_database/07_exam_summary/430_index_fast_full_scan/) 집계 등을 활용해 수억 건도 빠르게 처리한다.
...
| 개념 | 연결 포인트 |
|:---|:---|
| **[GROUP BY](/knowledge-base/studynote/05_database/04_transactions_concurrency/522_group_by/)** | 집계 함수와 항상 함께 등장하는 [그룹화](/knowledge-base/studynote/02_operating_system/09_file_system/535_grouping_counting_free_space/) 절 |
| **HAVING** | 집계 결과에 대한 필터링; WHERE와 처리 순서가 다름 |
| **윈도우 함수 ([Window Function](/knowledge-base/studynote/05_database/03_relational_model/139_window_function_analytics/))** | 행을 [압축](/knowledge-base/studynote/02_operating_system/06_memory_management/347_compaction/)하지 않고 집계하는 집계 함수의 확장 형태 |
| **Materialized [View](/knowledge-base/studynote/05_database/03_relational_model/151_sql_view_virtual_table/)** | 집계 결과를 사전에 저장해 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/)을 높이는 기법 |
| **[OLAP](/knowledge-base/studynote/12_it_management/05_security_compliance/316_olap/) ([Online Analytical Processing](/knowledge-base/studynote/14_data_engineering/05_exam_keywords/211_olap_drill_down_roll_up_surrogate_key/))** | 집계 중심의 분석 [쿼리](/knowledge-base/studynote/10_ai/04_ai_ops_ethics/298_qkv_attention/) 워크로드 처리 시스템 |
