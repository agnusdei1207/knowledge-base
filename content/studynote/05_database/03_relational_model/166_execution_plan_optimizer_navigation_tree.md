...
> 1. **본질**: 실행 계획 (Execution Plan)은 [데이터베이스 관리 시스템](/knowledge-base/studynote/05_database/01_db_architecture_relational/003_dbms_database_management_system/) ([DBMS](/knowledge-base/studynote/05_database/04_transactions_concurrency/502_dbms/), [Database](/knowledge-base/studynote/05_database/04_transactions_concurrency/501_database/) [Management](/knowledge-base/studynote/12_it_management/05_security_compliance/1013_management/) System)의 [비용 기반 옵티마이저](/knowledge-base/studynote/05_database/03_relational_model/165_cbo_cost_based_optimizer/) (CBO, Cost Based [Optimizer](/knowledge-base/studynote/12_it_management/02_itsm_itil/088_optimizer/))가 SQL (Structured Query Language)을 어떤 물리 경로로 실행할지 트리 형태로 표현한 항로도다.
> 2. **가치**: 느린 SQL의 원인은 문법보다 접근 경로, [조인 순서](/knowledge-base/studynote/05_database/03_relational_model/176_join_order_optimization/), 예상 행 수 오판에 숨어 있으므로, 실행 계획은 병목을 가장 빨리 드러내는 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) 진단 도구다.
> 3. **판단 포인트**: 실행 계획은 어디까지나 예상값이므로 비용 (Cost) 숫자만 맹신하지 말고, 예상 카디널리티 (Cardinality)와 실제 [처리량](/knowledge-base/studynote/01_computer_architecture/03_architecture_basics_performance/139_throughput/)이 얼마나 어긋나는지까지 함께 봐야 한다.

---

## Ⅰ. 개요 및 필요성

실행 계획 (Execution Plan)은 [관계](/knowledge-base/studynote/05_database/02_modeling_normalization/083_relationship_in_er_model/)형 [데이터베이스 관리 시스템](/knowledge-base/studynote/05_database/01_db_architecture_relational/003_dbms_database_management_system/) (RDBMS, Relational [Database](/knowledge-base/studynote/05_database/04_transactions_concurrency/501_database/) [Management](/knowledge-base/studynote/12_it_management/05_security_compliance/1013_management/) System)이 SQL을 실제로 읽을 때 선택한 접근 경로, [조인 순서](/knowledge-base/studynote/05_database/03_relational_model/176_join_order_optimization/), 조인 방식, 정렬 연산을 트리로 보여 주는 결과물이다. 개발자는 SQL로 "무엇을 구할지"만 적지만, [데이터베이스](/knowledge-base/studynote/05_database/01_db_architecture_relational/002_database_definition/)는 그 요청을 처리하기 위해 어떤 [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/)를 탈지, 어느 테이블을 먼저 읽을지, [해시 조인](/knowledge-base/studynote/05_database/03_relational_model/174_hash_join/) ([Hash Join](/knowledge-base/studynote/05_database/03_relational_model/174_hash_join/))과 [중첩 루프 조인](/knowledge-base/studynote/05_database/03_relational_model/172_nl_join_nested_loop/) ([Nested Loop Join](/knowledge-base/studynote/05_database/07_exam_summary/431_nested_loop_join/)) 중 무엇을 쓸지 별도로 결정한다. 실행 계획은 바로 그 보이지 않는 물리 경로를 눈에 보이게 만든다.

이 문서가 중요한 이유는 같은 SQL이라도 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 분포와 통계 정보에 따라 전혀 다른 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/)이 나올 수 있기 때문이다. 예를 들어 주문 1천만 건 중 10건만 찾는 조회라면 [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/) 범위 스캔 ([Index Range Scan](/knowledge-base/studynote/05_database/07_exam_summary/429_index_range_scan/))이 유리하지만, 300만 건을 읽는 조건이라면 전체 테이블 스캔 (Full Table Scan)이 더 빠를 수 있다. 실행 계획 없이 SQL만 보면 느린 이유를 추측할 수밖에 없지만, 실행 계획을 보면 병목이 접근 경로인지, [조인 순서](/knowledge-base/studynote/05_database/03_relational_model/176_join_order_optimization/)인지, 정렬 연산인지 구체적으로 판단할 수 있다.
...
| 구성 요소 | 의미 | 해석 포인트 |
| :--- | :--- | :--- |
| [Operation](/knowledge-base/studynote/05_database/06_dw_olap_trends/329_delta_encoding/) | 수행 연산 종류 | Table Access, [Index](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/) Scan, [Join](/knowledge-base/studynote/05_database/04_transactions_concurrency/521_join/), Sort [확인](/knowledge-base/studynote/04_software_engineering/12_testing_maintenance/396_validation/) |
| Object Name | 실제 대상 객체 | 어떤 테이블·[인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/)를 건드리는지 파악 |
| Rows | 예상 결과 건수 | 카디널리티 예측이 맞는지 [검증](/knowledge-base/studynote/04_software_engineering/07_object_oriented/395_verification_process_review/) |
| Cost | [옵티마이저](/knowledge-base/studynote/05_database/03_relational_model/163_optimizer_sql_execution_plan_generator/)의 상대적 비용 | 같은 계획 안에서 병목 후보 판단 |
| Predicate | 접근/필터 조건 | [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/) 활용 여부와 후처리 필터 구분 |
...
| :--- | :--- | :--- |
| [생성](/knowledge-base/studynote/02_operating_system/02_process_thread/087_process_state_transition/) 시점 | 실행 전 | 실행 후 |
| 장점 | 빠르게 구조를 [확인](/knowledge-base/studynote/04_software_engineering/12_testing_maintenance/396_validation/) 가능 | 실제 병목과 행 수 왜곡 [확인](/knowledge-base/studynote/04_software_engineering/12_testing_maintenance/396_validation/) 가능 |
...
3. 조건절 가공, 함수 사용, 복합 [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/) 선두 컬럼 문제를 [확인](/knowledge-base/studynote/04_software_engineering/12_testing_maintenance/396_validation/)한다.
4. 통계 갱신으로 해결될지, [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/) 재설계가 필요한지, [힌트](/knowledge-base/studynote/05_database/03_relational_model/167_sql_hint_optimizer_override/)가 필요한 응급 상황인지 나눈다.

### 판단 원칙

- **우선 채택**: 통계가 신뢰 가능하고 실행 계획이 안정적이면 [옵티마이저](/knowledge-base/studynote/05_database/03_relational_model/163_optimizer_sql_execution_plan_generator/)의 선택을 존중한다.
- **보완 필요**: 특정 단계에서 카디널리티 오차가 크면 통계, 히스토그램 (Histogram), SQL 구조를 먼저 수정한다.
- **최후 수단**: 장애 회피가 급한 경우에만 [힌트](/knowledge-base/studynote/05_database/03_relational_model/167_sql_hint_optimizer_override/)로 경로를 고정하고, 이후 근본 원인을 제거한다.
...
실행 계획을 읽을 수 있으면 SQL 튜닝이 감각의 영역에서 증거의 영역으로 바뀐다. 병목이 전체 스캔인지, 불필요한 정렬인지, 잘못된 [조인 순서](/knowledge-base/studynote/05_database/03_relational_model/176_join_order_optimization/)인지 빠르게 분리할 수 있어 문제 해결 속도가 크게 빨라진다. 이는 응답시간 단축뿐 아니라 불필요한 [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/) 추가 감소, 서버 증설 [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/), 장애 대응 시간 단축으로 이어진다.

다만 실행 계획만으로 모든 문제가 해결되지는 않는다. 계획은 예상 모델이므로 실제 캐시 상태, [병렬](/knowledge-base/studynote/05_database/07_exam_summary/430_index_fast_full_scan/)도, 런타임 바인드 값, 통계 편향까지 완벽히 설명하지 못할 수 있다. 그래서 실행 계획은 종착점이 아니라, 실제 실행 통계와 함께 보는 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) 분석의 중심 축으로 기억해야 한다.

앞으로 적응형 [쿼리](/knowledge-base/studynote/10_ai/04_ai_ops_ethics/298_qkv_attention/) 처리 (Adaptive Query Processing), 실행 중 재최적화, 실제 행 수 피드백 기능이 발전하더라도 본질은 같다. 실행 계획은 [데이터베이스](/knowledge-base/studynote/05_database/01_db_architecture_relational/002_database_definition/)가 SQL을 어떻게 "생각하고 움직였는지"를 보여 주는 가장 직접적인 인터페이스다.
...
| 개념 | 연결 포인트 |
| :--- | :--- |
| [비용 기반 옵티마이저](/knowledge-base/studynote/05_database/03_relational_model/165_cbo_cost_based_optimizer/) (CBO, Cost Based [Optimizer](/knowledge-base/studynote/12_it_management/02_itsm_itil/088_optimizer/)) | 실행 계획을 [생성](/knowledge-base/studynote/02_operating_system/02_process_thread/087_process_state_transition/)하는 핵심 판단 엔진 |
| 카디널리티 (Cardinality) | 각 노드의 예상 행 수로 계획 정확도를 좌우 |
| [선택도](/knowledge-base/studynote/05_database/03_relational_model/170_selectivity_cardinality_distribution_tuning/) ([Selectivity](/knowledge-base/studynote/05_database/03_relational_model/170_selectivity_cardinality_distribution_tuning/)) | [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/) 사용 여부와 접근 경로 판단의 기준 |
| [조인 순서](/knowledge-base/studynote/05_database/03_relational_model/176_join_order_optimization/) ([Join Order](/knowledge-base/studynote/05_database/03_relational_model/176_join_order_optimization/)) | 구동 테이블 선택과 전체 비용에 직접 영향 |
| [힌트](/knowledge-base/studynote/05_database/03_relational_model/167_sql_hint_optimizer_override/) ([Hint](/knowledge-base/studynote/05_database/03_relational_model/167_sql_hint_optimizer_override/)) | [옵티마이저](/knowledge-base/studynote/05_database/03_relational_model/163_optimizer_sql_execution_plan_generator/)의 기본 판단을 강제로 조정하는 수단 |
| 실제 실행 통계 | 예상 계획과 현실 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/)의 차이를 [검증](/knowledge-base/studynote/04_software_engineering/07_object_oriented/395_verification_process_review/)하는 근거 |
