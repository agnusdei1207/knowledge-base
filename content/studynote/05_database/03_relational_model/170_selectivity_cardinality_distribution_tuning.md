...
> 1. **본질**: 선택도 (Selectivity)는 조건이 전체 행 중 몇 퍼센트를 남기는지, [기수](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/077_radix/)성 (Cardinality)은 컬럼 값의 서로 다른 개수, 분포도 (Distribution)는 그 값들이 고르게 퍼졌는지 치우쳤는지를 보여 주는 통계 축이다.
> 2. **가치**: [비용 기반 옵티마이저](/knowledge-base/studynote/05_database/03_relational_model/165_cbo_cost_based_optimizer/) (CBO, Cost-Based [Optimizer](/knowledge-base/studynote/12_it_management/02_itsm_itil/088_optimizer/))는 이 세 축으로 결과 건수와 입출력 (I/O, Input/Output) 비용을 추정해 [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/) 스캔, 풀 테이블 스캔, [조인 순서](/knowledge-base/studynote/05_database/03_relational_model/176_join_order_optimization/)를 결정한다.
> 3. **판단 포인트**: 높은 [기수](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/077_radix/)성 컬럼이라도 범위 조회가 넓으면 [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/)가 불리할 수 있고, 낮은 [기수](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/077_radix/)성 컬럼이라도 값 쏠림이 심하면 히스토그램 (Histogram) 없이는 잘못된 [실행 계획](/knowledge-base/studynote/05_database/03_relational_model/166_execution_plan_optimizer_navigation_tree/)이 나올 수 있다.
...
| [기수](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/077_radix/)성 (NDV, Number of Distinct Values) | 서로 다른 값의 개수 | 동등 조건 선택도 계산의 출발점 |
| 분포도 (Distribution) | 값의 빈도 편차 | 평균 가정이 맞는지 [검증](/knowledge-base/studynote/04_software_engineering/07_object_oriented/395_verification_process_review/) |
| 히스토그램 (Histogram) | 구간/값별 빈도 상세 통계 | 쏠린 값에 대한 정밀 추정 |
| 컬럼 그룹 통계 (Extended [Statistics](/knowledge-base/studynote/05_database/03_relational_model/168_clustering_factor_index_physical_alignment/)) | 컬럼 간 상관관계 | 독립 가정으로 인한 오차 보정 |

예를 들어 `grade` 컬럼의 값이 `VIP`, `NORMAL`, `DORMANT` 세 개라면 [기수](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/077_radix/)성만 보면 선택도는 평균 33.3%로 보인다. 하지만 실제 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)가 `VIP 0.2% / NORMAL 97% / DORMANT 2.8%`라면, `grade = 'VIP'` [쿼리](/knowledge-base/studynote/10_ai/04_ai_ops_ethics/298_qkv_attention/)는 평균이 아니라 희소값 조회로 취급해야 한다. 이때 히스토그램이 없으면 [옵티마이저](/knowledge-base/studynote/05_database/03_relational_model/163_optimizer_sql_execution_plan_generator/)는 `VIP`를 과대평가해 풀 스캔을 선택할 수 있고, 히스토그램이 있으면 [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/) 스캔으로 뒤집을 수 있다.
...
세 지표를 헷갈리지 않으려면 "무엇을 세는가"를 분리해서 봐야 한다. [기수](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/077_radix/)성은 컬럼의 다양성, 선택도는 조건의 폭, 분포도는 값의 치우침을 설명한다. 셋을 섞어 쓰면 "[기수](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/077_radix/)성이 높으니 무조건 [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/)" 같은 단순화 오류가 생긴다.

| 비교 축 | [기수](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/077_radix/)성 (Cardinality) | 선택도 (Selectivity) | 분포도 (Distribution) |
| :--- | :--- | :--- | :--- |
| 질문 | 값 종류가 몇 개인가 | 조건이 몇 %를 남기는가 | 특정 값이 고르게 퍼졌는가 |
| 대표 예 | 주민번호, 이메일은 높음 | `customer_id = ?` 는 낮음 | VIP만 극소수면 치우침 큼 |
| [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/) 판단 영향 | 동등 조건의 평균 선택도에 영향 | 직접적으로 액세스 경로 결정 | 평균 가정의 오차를 수정 |
| 대표 함정 | 범위 조회 폭을 설명 못함 | 값별 편차를 설명 못함 | 통계 없으면 희소값을 놓침 |

이 셋은 다른 물리 통계와도 연결된다. 예를 들어 선택도가 좋아도 [클러스터링 팩터](/knowledge-base/studynote/05_database/03_relational_model/169_clustering_factor_index_physical_sort/) ([Clustering Factor](/knowledge-base/studynote/05_database/03_relational_model/169_clustering_factor_index_physical_sort/))가 나쁘면 [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/)를 타고 읽은 행이 디스크 곳곳에 흩어져 있어 기대만큼 빠르지 않을 수 있다. 반대로 선택도가 다소 높아도 커버링 [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/) (Covering [Index](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/))로 테이블 접근을 생략하면 여전히 유리할 수 있다.
...
실무에서 가장 흔한 사례는 상태 코드나 회원 등급처럼 "종류는 적지만 특정 값만 희소한 컬럼"이다. 예를 들어 주문 테이블 5,000만 건에서 `status = 'FAILED'`가 0.3%, `status = 'DONE'`가 96%라면, 같은 [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/)라도 실패 주문 조회와 완료 주문 조회의 최적 경로는 달라져야 한다. 이때 히스토그램 없이 평균 25%로 계산하면 두 [쿼리](/knowledge-base/studynote/10_ai/04_ai_ops_ethics/298_qkv_attention/) 모두 애매한 계획으로 수렴할 수 있다.

### 기술사 판단 [체크리스트](/knowledge-base/studynote/04_software_engineering/11_testing_validation/435_checklist_based_testing/)

1. 통계 정보가 대량 적재·삭제 이후 최신 상태인가?
2. 낮은 선택도 [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/)를 만들기 전에 실제 조회 비율과 블록 접근 비용을 [확인](/knowledge-base/studynote/04_software_engineering/12_testing_maintenance/396_validation/)했는가?
3. 값 쏠림이 심한 컬럼에 히스토그램을 수집했는가?
4. 여러 조건이 같이 쓰일 때 컬럼 독립 가정이 깨지지 않는가?
5. 범위 조회라면 선택도뿐 아니라 [클러스터링 팩터](/knowledge-base/studynote/05_database/03_relational_model/169_clustering_factor_index_physical_sort/)와 커버링 가능성까지 봤는가?

### 채택 / 회피 판단

- **채택**: 고기수성 동등 조건, 희소값 조회, 잘 관리된 히스토그램 기반 조건, 커버링 가능한 [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/)
- **회피 또는 보완**: 대량 범위 조회, 낮은 [기수](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/077_radix/)성 컬럼 단독 [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/), 통계가 오래된 테이블, 상관관계가 강한 다중 조건

### 자주 나오는 [안티패턴](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/128_water_scrum_fall_anti_pattern/)

- `status`, `gender`, `Y/N` 컬럼에 "자주 조회한다"는 이유만으로 단독 [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/)를 남발하는 경우
- 통계 문제를 [확인](/knowledge-base/studynote/04_software_engineering/12_testing_maintenance/396_validation/)하지 않고 [힌트](/knowledge-base/studynote/05_database/03_relational_model/167_sql_hint_optimizer_override/) ([Hint](/knowledge-base/studynote/05_database/03_relational_model/167_sql_hint_optimizer_override/))로 [실행 계획](/knowledge-base/studynote/05_database/03_relational_model/166_execution_plan_optimizer_navigation_tree/)을 고정하는 경우
- 히스토그램을 무분별하게 [생성](/knowledge-base/studynote/02_operating_system/02_process_thread/087_process_state_transition/)해 계획 변동성을 키우는 경우
- [기수](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/077_radix/)성이 높다는 이유만으로 넓은 날짜 범위 조회에도 [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/)를 고집하는 경우

실무 판단의 핵심은 "몇 건 찾느냐"보다 "그 몇 건이 실제로 얼마나 비싸게 읽히느냐"다. 그래서 5~[10](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/489_raid_10_hybrid/)% 같은 임계값은 참고선일 뿐 절대 법칙이 아니다. 저장장치 특성, 행 길이, [버퍼 캐시](/knowledge-base/studynote/02_operating_system/09_file_system/536_buffer_cache_page_cache/), [클러스터링 팩터](/knowledge-base/studynote/05_database/03_relational_model/169_clustering_factor_index_physical_sort/)에 따라 손익분기점은 달라진다.

- **📢 섹션 요약 비유**: 지도에서 목적지가 가깝게 보여도 차가 막히면 돌아가는 길이 더 빠르듯, [데이터베이스](/knowledge-base/studynote/05_database/01_db_architecture_relational/002_database_definition/)도 선택도 숫자 하나만 보고 길을 정하면 실제 비용을 놓칠 수 있다.
...
| 개념 | 연결 포인트 |
| :--- | :--- |
| [비용 기반 옵티마이저](/knowledge-base/studynote/05_database/03_relational_model/165_cbo_cost_based_optimizer/) (CBO, Cost-Based [Optimizer](/knowledge-base/studynote/12_it_management/02_itsm_itil/088_optimizer/)) | 선택도와 예상 행 수를 바탕으로 액세스 경로를 결정 |
| 히스토그램 (Histogram) | 치우친 분포를 평균값 대신 값별 빈도로 표현 |
| [클러스터링 팩터](/knowledge-base/studynote/05_database/03_relational_model/169_clustering_factor_index_physical_sort/) ([Clustering Factor](/knowledge-base/studynote/05_database/03_relational_model/169_clustering_factor_index_physical_sort/)) | 선택도가 좋아도 실제 블록 접근 비용이 높은지 판단 |
| 컬럼 그룹 통계 (Extended [Statistics](/knowledge-base/studynote/05_database/03_relational_model/168_clustering_factor_index_physical_alignment/)) | 다중 조건의 상관관계를 반영해 추정 오차를 줄임 |
...
1. [기수](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/077_radix/)성은 상자 안에 서로 다른 색 블록이 몇 종류 있는지 세는 일이에요.
...
← **이전**: [169. 클러스터링 팩터 (Clustering Factor) - 인덱스 정렬 순서와 실제 물리적 데이터 정렬 순서의 일치 정도](/knowledge-base/studynote/05_database/03_relational_model/169_clustering_factor_index_physical_sort/)
**다음**: [171. 옵티마이저 조인 기법 3가지](/knowledge-base/studynote/05_database/03_relational_model/171_optimizer_join_methods/) →
