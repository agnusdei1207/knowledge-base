...
- **📢 섹션 요약 비유**: 리스트 [파티셔닝](/knowledge-base/studynote/05_database/03_relational_model/179_table_partitioning_concept/)은 사물함을 날짜순으로 늘어놓는 것이 아니라, "서울", "부산", "기타"라고 이름표를 붙여 두는 방식과 같다. 물건이 어디에 들어갈지 한눈에 보이고, 특정 이름표 서랍만 열어보면 되니 찾기와 정리가 쉬워진다.
...
| 명시적 값 목록 | 각 [파티션](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/)이 담당할 코드 집합 | 값 누락이 없도록 관리 체계를 가져야 함 |
| DEFAULT [파티션](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/) | 신규 코드나 예외 값의 안전판 | 없으면 예기치 않은 입력 실패가 발생할 수 있음 |
| [파티션 프루닝](/knowledge-base/studynote/05_database/03_relational_model/184_partition_pruning/) ([Partition Pruning](/knowledge-base/studynote/05_database/03_relational_model/184_partition_pruning/)) | 조건에 맞는 [파티션](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/)만 읽게 하는 최적화 | `=`·`IN` 조건에서 특히 효과적 |
| 로컬 [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/) (Local [Index](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/)) | [파티션](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/)별 [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/) 유지보수 단순화 | [파티션](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/) 교체·삭제 작업과 함께 관리하기 좋음 |
| 행 이동 (Row Movement) | [파티션](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/) 키 변경 시 다른 [파티션](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/)으로 재배치 | 상태값처럼 자주 바뀌는 컬럼은 비용이 커질 수 있음 |

리스트 [파티셔닝](/knowledge-base/studynote/05_database/03_relational_model/179_table_partitioning_concept/)은 사람이 이해하기 쉬운 대신, 값 집합을 사람이 책임지고 유지해야 한다. 따라서 신규 코드가 자주 생기는 조직이라면 `DEFAULT` [파티션](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/) [모니터](/knowledge-base/studynote/02_operating_system/04_synchronization/229_monitor/)링과 코드 [분류](/knowledge-base/studynote/16_bigdata/05_analysis/104_classification_analysis/) 재배치 절차가 반드시 뒤따라야 한다.
...
리스트 [파티셔닝](/knowledge-base/studynote/05_database/03_relational_model/179_table_partitioning_concept/)의 장점은 다른 방식과 비교할 때 더 선명해진다. [레인지 파티셔닝](/knowledge-base/studynote/05_database/03_relational_model/180_range_partitioning/)은 기간 관리에, [해시 파티셔닝](/knowledge-base/studynote/05_database/03_relational_model/181_hash_partitioning/)은 균등 [분산](/knowledge-base/studynote/08_algorithm_stats/08_stats/136_variance/)에, 리스트 [파티셔닝](/knowledge-base/studynote/05_database/03_relational_model/179_table_partitioning_concept/)은 코드 의미 보존에 강하다. 따라서 "어떻게 고르게 나눌까"보다 "어떤 업무 경계를 저장 구조에 반영할까"가 중요한 경우 리스트가 우세하다.

| 방식 | 강한 지점 | 약한 지점 | 대표 활용 |
| :--- | :--- | :--- | :--- |
| [레인지 파티셔닝](/knowledge-base/studynote/05_database/03_relational_model/180_range_partitioning/) ([Range Partitioning](/knowledge-base/studynote/05_database/03_relational_model/180_range_partitioning/)) | 기간별 조회, 보관 주기, [파티션](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/) 단위 삭제 | 최신 구간 집중 현상 | 일자별 주문, 월별 [로그](/knowledge-base/studynote/04_software_engineering/09_cloud_native_ai_architecture/568_logs_distributed_logging_elk_fluentd/) |
| 리스트 [파티셔닝](/knowledge-base/studynote/05_database/03_relational_model/179_table_partitioning_concept/) (List [Partitioning](/knowledge-base/studynote/05_database/03_relational_model/179_table_partitioning_concept/)) | 지역·채널·조직 코드 분리, 규제 경계 반영 | 코드 추가 관리, 값 집중 시 쏠림 | 국가별, 채널별, 등급별 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) |
| [해시 파티셔닝](/knowledge-base/studynote/05_database/03_relational_model/181_hash_partitioning/) ([Hash Partitioning](/knowledge-base/studynote/05_database/03_relational_model/181_hash_partitioning/)) | 균등 [분산](/knowledge-base/studynote/08_algorithm_stats/08_stats/136_variance/), [동시성](/knowledge-base/studynote/15_devops_sre/01_culture_methodology/014_concurrency/) 높은 입력 | 업무 의미 약화, 범위 조회 약함 | 고객번호, 계정번호 |
| 복합 [파티셔닝](/knowledge-base/studynote/05_database/03_relational_model/179_table_partitioning_concept/) ([Composite Partitioning](/knowledge-base/studynote/05_database/03_relational_model/183_composite_partitioning/)) | 의미와 [분산](/knowledge-base/studynote/08_algorithm_stats/08_stats/136_variance/), 기간과 코드 분리를 동시에 확보 | 설계·운영 복잡도 증가 | 월별 + 지역별, 국가별 + 해시 |

특히 리스트 [파티셔닝](/knowledge-base/studynote/05_database/03_relational_model/179_table_partitioning_concept/)은 규제나 권한 모델과 잘 연결된다. 예를 들어 국가 코드별 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 보관 위치가 달라야 하거나, 사업부별로 독립 [백업](/knowledge-base/studynote/02_operating_system/09_file_system/555_backup_and_restore_strategy/) [정책](/knowledge-base/studynote/10_ai/02_dl_architecture_new/164_policy/)을 가져야 할 때는 리스트 [파티션](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/)이 설계 설명력에서 큰 이점을 준다. 반대로 `status`처럼 값은 적어 보여도 변경이 자주 일어나는 컬럼은 행 이동 비용이 커져 부적절할 수 있다.

또한 리스트 [파티셔닝](/knowledge-base/studynote/05_database/03_relational_model/179_table_partitioning_concept/) 하나로 모든 문제를 해결하려 해서는 안 된다. 월별 보관과 지역별 분리를 함께 원하면 `Range + List`, 특정 지역 안에서 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)가 과도하게 몰리면 `List + Hash`처럼 복합 구조로 확장하는 편이 현실적이다.

- **📢 섹션 요약 비유**: 레인지는 달력 서랍, 해시는 번호표 [분산](/knowledge-base/studynote/08_algorithm_stats/08_stats/136_variance/)함, 리스트는 이름표 서랍에 가깝다. 어떤 기준으로 꺼내 보고 어떤 기준으로 버릴지를 생각하면, 왜 리스트가 "의미 있는 코드 분리"에 강한지 바로 보인다.
...
3. 예외 값을 흡수할 `DEFAULT` [파티션](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/)과 정리 절차가 준비되어 있는가?
4. 특정 값 집중으로 인한 대형 [파티션](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/) 발생을 감당하거나 보완할 방법이 있는가?
5. [파티션](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/) 키 변경이 잦지 않아 행 이동 비용이 통제 가능한가?

### 자주 나오는 [안티패턴](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/128_water_scrum_fall_anti_pattern/)

- 신규 코드가 생길 수 있는데도 `DEFAULT` [파티션](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/) 없이 운영하는 경우
- `user_id`처럼 값 종류가 너무 많은 컬럼을 리스트 [파티셔닝](/knowledge-base/studynote/05_database/03_relational_model/179_table_partitioning_concept/)으로 억지 [분류](/knowledge-base/studynote/16_bigdata/05_analysis/104_classification_analysis/)하는 경우
...
결론적으로 리스트 [파티셔닝](/knowledge-base/studynote/05_database/03_relational_model/179_table_partitioning_concept/)은 "값의 의미를 잃지 않고 저장 구조를 나누는 방법"으로 기억하는 편이 맞다. 업무 코드 자체가 설계의 주인공일 때 선택해야 하며, 값 집합 관리와 예외 처리 [전략](/knowledge-base/studynote/04_software_engineering/04_testing_quality/268_strategy_pattern/)까지 준비될 때 가장 큰 힘을 발휘한다.

- **📢 섹션 요약 비유**: 잘 정리된 도서관은 책을 아무렇게나 흩뿌리지 않고, 분야 이름표에 따라 서가를 나눈다. 리스트 [파티셔닝](/knowledge-base/studynote/05_database/03_relational_model/179_table_partitioning_concept/)도 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 그렇게 다루되, 새 분야 책이 들어왔을 때 어느 서가에 둘지 미리 준비해 두는 설계다.
...
| 개념 | 연결 포인트 |
| :--- | :--- |
| [파티션](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/) 키 ([Partition](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/) [Key](/knowledge-base/studynote/05_database/02_modeling_normalization/067_db_key_uniqueness_minimality/)) | 행이 어느 목록에 속하는지 결정하는 기준 컬럼 |
| 명시적 값 목록 | [파티션](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/)별 담당 코드 집합을 사람이 설계하는 규칙 |
| DEFAULT [파티션](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/) | 미분류 값과 신규 코드를 흡수하는 운영 안전판 |
| [파티션 프루닝](/knowledge-base/studynote/05_database/03_relational_model/184_partition_pruning/) ([Partition Pruning](/knowledge-base/studynote/05_database/03_relational_model/184_partition_pruning/)) | 조건에 맞는 [파티션](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/)만 읽어 동등 조회를 줄이는 최적화 |
| [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 쏠림 ([Data](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) Skew) | 특정 값에 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)가 몰려 [파티션](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/) 크기와 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/)이 불균형해지는 현상 |
| 행 이동 (Row Movement) | [파티션](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/) 키 변경 시 다른 [파티션](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/)으로 물리 재배치가 일어나는 동작 |
| 복합 [파티셔닝](/knowledge-base/studynote/05_database/03_relational_model/179_table_partitioning_concept/) ([Composite Partitioning](/knowledge-base/studynote/05_database/03_relational_model/183_composite_partitioning/)) | 리스트의 의미 분리와 레인지·해시의 강점을 결합하는 확장 방식 |
...
1. 리스트 [파티셔닝](/knowledge-base/studynote/05_database/03_relational_model/179_table_partitioning_concept/)은 장난감을 "자동차 상자", "인형 상자", "기타 상자"처럼 이름표 붙은 상자에 넣는 방법이에요.
