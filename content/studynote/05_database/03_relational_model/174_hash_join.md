...
반면 해시 조인은 양쪽을 모두 풀 스캔하더라도, 작은 입력만 메모리에 적절히 올릴 수 있다면 조인 비용을 **Build + Probe** 형태의 비교적 단순한 흐름으로 바꿀 수 있다. 그래서 [데이터 웨어하우스](/knowledge-base/studynote/12_it_management/05_security_compliance/209_data_warehouse_schema_on_write/) ([DW](/knowledge-base/studynote/12_it_management/05_security_compliance/209_data_warehouse_schema_on_write/), [Data Warehouse](/knowledge-base/studynote/14_data_engineering/05_exam_keywords/208_data_warehouse_schema_on_write_inmon/)), 일괄 처리, [스타 스키마](/knowledge-base/studynote/05_database/06_dw_olap_trends/334_star_schema/) 질의에서 자주 선택된다.
...
| :--- | :--- | :--- | :--- |
| Build Phase | 작은 입력의 조인 키에 [해시 함수](/knowledge-base/studynote/03_network/13_network_security_basics/667_hash_function_integrity_one_way/)를 적용해 메모리 버킷 구성 | 작은 입력을 좁은 폭으로 투영하면 메모리 효율 향상 | Build Input 선택 실패 시 메모리 압박 |
...
| :--- | :--- | :--- | :--- |
| 핵심 [전략](/knowledge-base/studynote/04_software_engineering/04_testing_quality/268_strategy_pattern/) | 외부 행마다 내부 탐색 반복 | 작은 입력을 해시 버킷화 후 큰 입력 탐색 | 양쪽을 정렬한 뒤 순차 병합 |
| 잘 맞는 조건 | 소량 결과 + 좋은 [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/) | 대량 **동등 조인** + 충분한 작업 메모리 | 대량 조인 + 비동등 조건 또는 정렬 재사용 |
...
3. Build 쪽에서 불필요한 컬럼을 줄여 [해시 테이블](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/067_hash_table/) 폭을 축소했는가?
4. 통계 정보가 최신이라 [비용 기반 옵티마이저](/knowledge-base/studynote/05_database/03_relational_model/165_cbo_cost_based_optimizer/) (CBO, Cost-Based [Optimizer](/knowledge-base/studynote/12_it_management/02_itsm_itil/088_optimizer/)) 가 입력 크기를 정확히 아는가?
5. 특정 키에 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)가 몰리는 편향(skew) 때문에 버킷 과부하가 생기지 않는가?
...
- Build Input 이 더 큰데도 [힌트](/knowledge-base/studynote/05_database/03_relational_model/167_sql_hint_optimizer_override/)로 잘못 고정해 메모리 스필을 유발하는 경우
- `SELECT *` 로 넓은 행을 그대로 Build 해 해시 영역을 낭비하는 경우
- [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 편향과 임시 공간 사용량을 보지 않고 "해시 조인은 무조건 빠르다"고 믿는 경우
...
해시 조인은 대량 동등 조인을 **순차 스캔 + 메모리 조회** 패턴으로 바꿔 주기 때문에, 분석성 질의의 [처리량](/knowledge-base/studynote/01_computer_architecture/03_architecture_basics_performance/139_throughput/)을 크게 높여 준다. 특히 정렬이 불필요하고 결과 순서가 중요하지 않은 경우, [Sort Merge Join](/knowledge-base/studynote/05_database/03_relational_model/173_sort_merge_join/) 보다 준비 비용이 적고 [Nested Loop Join](/knowledge-base/studynote/05_database/07_exam_summary/431_nested_loop_join/) 보다 랜덤 I/O 부담이 작다.
...
| 개념 | 연결 포인트 |
| :--- | :--- |
| Build Input | [해시 테이블](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/067_hash_table/)을 먼저 만드는 작은 입력 집합 |
| Probe Input | Build 쪽 버킷을 조회하며 스캔하는 큰 입력 집합 |
| [비용 기반 옵티마이저](/knowledge-base/studynote/05_database/03_relational_model/165_cbo_cost_based_optimizer/) (CBO, Cost-Based [Optimizer](/knowledge-base/studynote/12_it_management/02_itsm_itil/088_optimizer/)) | 입력 크기와 메모리 비용을 보고 Hash [Join](/knowledge-base/studynote/05_database/04_transactions_concurrency/521_join/) 채택 여부를 판단 |
| [Sort Merge Join](/knowledge-base/studynote/05_database/03_relational_model/173_sort_merge_join/) | 범위 조인과 정렬 재사용에서 Hash [Join](/knowledge-base/studynote/05_database/04_transactions_concurrency/521_join/) 의 대안이 됨 |
| Grace Hash [Join](/knowledge-base/studynote/05_database/04_transactions_concurrency/521_join/) | 메모리 초과 시 [파티션](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/) 분할로 Hash [Join](/knowledge-base/studynote/05_database/04_transactions_concurrency/521_join/) 을 확장하는 기법 |
| [Bloom Filter](/knowledge-base/studynote/12_it_management/02_itsm_itil/061_bloomfilter/) | 대량 Probe 입력을 사전 필터링해 Hash [Join](/knowledge-base/studynote/05_database/04_transactions_concurrency/521_join/) 비용을 낮추는 보조 기법 |
| [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 편향 (Skew) | 특정 버킷에 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)가 몰려 [병렬](/knowledge-base/studynote/05_database/07_exam_summary/430_index_fast_full_scan/)성과 [처리량](/knowledge-base/studynote/01_computer_architecture/03_architecture_basics_performance/139_throughput/)을 떨어뜨리는 원인 |
