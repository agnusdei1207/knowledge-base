...
물론 모든 머징이 단순하지는 않다. 일반적으로 투영·선택·조인만 있는 단순 뷰는 머징이 쉽다. 반면 집계가 들어간 뷰는 일부 DBMS (Database Management System)에서 더 엄격한 "복합 뷰 머징"이나 조인 백 (Join Back) 형태로 다뤄질 수 있으며, 아무 조건 없이 합칠 수 있는 것은 아니다. 따라서 기술사 답안에서는 "집계 뷰는 무조건 불가"처럼 단정하기보다, <strong>기본적으로 제약이 크고 의미 보존 검사가 더 엄격하다</strong>고 설명하는 편이 정확하다.
