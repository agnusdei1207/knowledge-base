...
> 2. 프로젝트 연산은 결과에서 중복 [[063_relation_tuple_cardinality|튜플]]을 자동으로 제거(집합 기반)하여, "[[520_select|SELECT]] DISTINCT"와 동등하며, 이는 [[038_relational_algebra|관계 대수]]가 집합 이론을 기반으로 중복을 허용하지 않기 때문이다.
> 3. 실렉트(σ, 행 선택)와 프로젝트(π, 열 선택)는 [[038_relational_algebra|관계 대수]]의 가장 기본적인 두 연산으로, 이 두 연산의 조합으로 임의의 [[061_relation_schema_instance|릴레이션]]의 특정 행과 열을 추출하는 [[298_qkv_attention|쿼리]]를 표현할 수 있다.
...
> 📢 **섹션 요약 비유**: 실렉트는 행(가로줄) 필터, 프로젝트는 열(세로줄) 필터 — SQL WHERE(행)와 [[520_select|SELECT]] 컬럼 목록(열)이 각각 대응.
...
## Ⅳ. [[038_relational_algebra|관계 대수]] 연산 조합
...
> 📢 **섹션 요약 비유**: [[038_relational_algebra|관계 대수]]는 요리 레시피 언어 — 재료([[061_relation_schema_instance|릴레이션]])를 자르고(σ), 특정 부분만 담고(π), 섞는(⋈) 조리 과정을 수식으로 표현.
...
> 📢 **섹션 요약 비유**: [[298_qkv_attention|쿼리]] 최적화는 쇼핑 [[268_strategy_pattern|전략]] — 필요한 물건 목록(π)과 조건(σ)을 정하고 찾아야지, 마트 전체(×) 돌고 나서 버리면 비효율.
...
3. 실렉트(σ)는 조건에 맞는 행 선택, 프로젝트(π)는 원하는 열 선택 — SQL의 WHERE와 [[520_select|SELECT]] 컬럼목록이 각각에 해당해요!
...
← **이전**: [[041_select_operator|041. 셀렉트 연산자 (Select Operator, σ) — 수평적 부분집합]]
**다음**: [[043_relational_algebra_join|043. 관계 대수 — 조인 (Relational Algebra Join)]] →
