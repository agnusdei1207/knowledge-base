...
- **📢 섹션 요약 비유**: 개체의 자격 조건은 '클럽(DB) 입장 통제'다. 클럽 문지기(아키텍트)는 신분증([식별자](/knowledge-base/studynote/03_network/06_network_layer_ip/289_identification_flags_fragmentation_offset/) PK)이 없거나, 혼자 왔거나(인스턴스 1개), 클럽 물을 흐리는(업무 무관) 사람은 가차 없이 입구 컷을 시켜야만 클럽 수질(DB [무결성](/knowledge-base/studynote/09_security/01_intro_principles/003_integrity/))이 유지된다.
...
아키텍트가 어떤 개체를 도출하느냐에 따라 그 회사의 업무 프로세스 전체가 결정된다. '고객'과 '계좌'를 분리된 개체로 보면 한 고객이 여러 계좌를 가질 수 있는 현대 은행이 되고, 둘을 하나의 개체로 뭉뚱그려 그리면 한 사람당 통장 하나만 만들 수 있는 1980년대 은행 시스템으로 전락한다. 결론적으로 개체 도출은 단순한 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 정리가 아니라, 기업의 비즈니스 확장성과 유연성을 DB라는 가장 단단한 쇳덩어리 토대 위에 각인시키는 고도의 철학적 모델링 작업이다.
...
2. 지나가는 구름이나 흙먼지 같은 건 중요하지 않으니까 빼고, 경찰서나 자동차처럼 이름표([식별자](/knowledge-base/studynote/03_network/06_network_layer_ip/289_identification_flags_fragmentation_offset/))를 딱 붙일 수 있는 확실한 것들만 주인공으로 뽑죠.
3. 이렇게 똑똑하게 주인공 덩어리들을 먼저 정해놔야, 컴퓨터라는 똑똑한 공장이 주인공들마다 멋진 집([데이터베이스](/knowledge-base/studynote/05_database/01_db_architecture_relational/002_database_definition/) 테이블)을 예쁘게 지어줄 수 있답니다!
...
← **이전**: [080. E-R 모델 (Entity-Relationship Model, 피터 첸)](/knowledge-base/studynote/05_database/02_modeling_normalization/080_er_model_entity_relationship_peter_chen/)
**다음**: [82. 속성 (Attribute)](/knowledge-base/studynote/05_database/02_modeling_normalization/082_attribute_types_er_model/) →
