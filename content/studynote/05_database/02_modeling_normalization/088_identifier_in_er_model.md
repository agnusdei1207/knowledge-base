...
> 1. **본질**: [식별자](/knowledge-base/studynote/03_network/06_network_layer_ip/289_identification_flags_fragmentation_offset/) (Identifier)는 `ER (Entity-Relationship)` 모델에서 특정 개체 (Entity) 인스턴스를 유일하게 구별해내는 [논리](/knowledge-base/studynote/09_security/04_endpoint_security/369_logic_bomb/)적 [속성](/knowledge-base/studynote/05_database/02_modeling_normalization/082_attribute_types_er_model/) 또는 [속성](/knowledge-base/studynote/05_database/02_modeling_normalization/082_attribute_types_er_model/)의 집합이다.
> 2. **가치**: 불변성과 유일성을 가진 [식별자](/knowledge-base/studynote/03_network/06_network_layer_ip/289_identification_flags_fragmentation_offset/)를 잘 정의하면, [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)의 무결성을 보장하고 [관계](/knowledge-base/studynote/05_database/02_modeling_normalization/083_relationship_in_er_model/) ([Relationship](/knowledge-base/studynote/05_database/02_modeling_normalization/083_relationship_in_er_model/))를 맺을 때 명확한 연결 고리를 제공할 수 있다.
> 3. **판단 포인트**: 업무적 의미를 가진 자연 [식별자](/knowledge-base/studynote/03_network/06_network_layer_ip/289_identification_flags_fragmentation_offset/) (Natural Identifier)를 쓸지, 무의미한 일련번호인 인조 [식별자](/knowledge-base/studynote/03_network/06_network_layer_ip/289_identification_flags_fragmentation_offset/) (Surrogate Identifier)를 쓸지는 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)의 생명주기와 변경 가능성을 고려하여 결정해야 한다.

---

## Ⅰ. 개요 및 필요성

[식별자](/knowledge-base/studynote/03_network/06_network_layer_ip/289_identification_flags_fragmentation_offset/) (Identifier)는 개념적 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 모델링 단계에서 엔터티 내의 각 인스턴스를 [식별](/knowledge-base/studynote/09_security/13_secops_ir_forensics/655_ir_detection_analysis/)할 수 있는 핵심 [속성](/knowledge-base/studynote/05_database/02_modeling_normalization/082_attribute_types_er_model/)이다. 이후 [논리](/knowledge-base/studynote/09_security/04_endpoint_security/369_logic_bomb/)/물리 설계 단계를 거치면서 `RDBMS (Relational Database Management System)`의 [기본 키](/knowledge-base/studynote/05_database/02_modeling_normalization/070_primary_key_alternate_key/) (`PK, Primary Key`)로 매핑된다. 즉, [식별자](/knowledge-base/studynote/03_network/06_network_layer_ip/289_identification_flags_fragmentation_offset/)는 [논리](/knowledge-base/studynote/09_security/04_endpoint_security/369_logic_bomb/)적 개념이고, [기본 키](/knowledge-base/studynote/05_database/02_modeling_normalization/070_primary_key_alternate_key/)는 물리적 구현체다.

[데이터베이스](/knowledge-base/studynote/05_database/01_db_architecture_relational/002_database_definition/) 시스템에서 동일한 형태의 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)가 수만 건 쌓일 때, "이 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)가 저 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)와 같은가?"를 판단하는 기준이 없으면 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 갱신과 [참조](/knowledge-base/studynote/05_database/05_distributed_nosql_newsql/316_reference_pattern_nosql/)가 불가능해진다. 비즈니스 규칙이 복잡해질수록 [속성](/knowledge-base/studynote/05_database/02_modeling_normalization/082_attribute_types_er_model/) 값은 자주 변하지만, 그 개체가 누구인지를 증명하는 '신분증' 역할이 바로 [식별자](/knowledge-base/studynote/03_network/06_network_layer_ip/289_identification_flags_fragmentation_offset/)다.

- **📢 섹션 요약 비유**: [식별자](/knowledge-base/studynote/03_network/06_network_layer_ip/289_identification_flags_fragmentation_offset/)는 학생의 '학번'과 같다. 이름이나 학년은 바뀔 수 있어도, 그 학생이 누구인지 증명하는 고유한 번호가 있어야 성적과 출결을 정확히 기록할 수 있다.

---

## Ⅱ. 아키텍처 및 핵심 원리

[식별자](/knowledge-base/studynote/03_network/06_network_layer_ip/289_identification_flags_fragmentation_offset/)가 되기 위해서는 반드시 4가지 핵심 요건을 만족해야 한다. 이를 하나라도 위반하면 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 정합성이 깨지거나 [외래 키](/knowledge-base/studynote/05_database/02_modeling_normalization/072_foreign_key_fk/) (`FK, Foreign Key`) [참조](/knowledge-base/studynote/05_database/05_distributed_nosql_newsql/316_reference_pattern_nosql/) 시 오류가 발생한다.

| 조건 | 의미 | 설계 관점 |
| :--- | :--- | :--- |
| **유일성 (Uniqueness)** | 모든 인스턴스는 고유한 값을 가져야 함 | 동명이인 방지 |
| **최소성 (Minimality)** | 유일성을 보장하는 최소한의 [속성](/knowledge-base/studynote/05_database/02_modeling_normalization/082_attribute_types_er_model/)으로 구성 | 복합 [식별자](/knowledge-base/studynote/03_network/06_network_layer_ip/289_identification_flags_fragmentation_offset/) 남용 방지 |
...
| :--- | :--- | :--- |
| **개념** | 업무에서 이미 사용 중인 의미 있는 [속성](/knowledge-base/studynote/05_database/02_modeling_normalization/082_attribute_types_er_model/) | 시스템이 인위적으로 부여한 일련번호 (Sequence) |
...
- "부서명+직급+이름"으로 사원 테이블의 주 [식별자](/knowledge-base/studynote/03_network/06_network_layer_ip/289_identification_flags_fragmentation_offset/)를 구성하는 행위. 이 중 하나라도 바뀌면 사원 개체의 정체성이 흔들리며 연관된 모든 FK를 연쇄 수정(`ON UPDATE CASCADE`)해야 한다.

- **📢 섹션 요약 비유**: 바뀌는 주소를 집의 고유 번호표로 쓰면, 이사 갈 때마다 동네 지도를 다 다시 그려야 한다. 변하지 않는 토지 지번(인조 [식별자](/knowledge-base/studynote/03_network/06_network_layer_ip/289_identification_flags_fragmentation_offset/))이 필요한 이유다.

---

## Ⅴ. 기대효과 및 결론

견고하게 설계된 [식별자](/knowledge-base/studynote/03_network/06_network_layer_ip/289_identification_flags_fragmentation_offset/)는 [데이터베이스](/knowledge-base/studynote/05_database/01_db_architecture_relational/002_database_definition/) 전체의 정합성을 수호하는 뼈대가 된다. 튼튼한 [식별자](/knowledge-base/studynote/03_network/06_network_layer_ip/289_identification_flags_fragmentation_offset/)는 시스템 런타임에 불필요한 연쇄 갱신을 막고 조인 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/)을 최적화하며, 무엇보다 개발자가 비즈니스 로직에만 집중할 수 있게 해준다.

반면, [식별자](/knowledge-base/studynote/03_network/06_network_layer_ip/289_identification_flags_fragmentation_offset/)의 불변성을 가볍게 여겨 변경 가능한 [속성](/knowledge-base/studynote/05_database/02_modeling_normalization/082_attribute_types_er_model/)을 주 [식별자](/knowledge-base/studynote/03_network/06_network_layer_ip/289_identification_flags_fragmentation_offset/)로 삼으면, 향후 마이그레이션이나 시스템 통합 시 막대한 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 클렌징 비용을 치르게 된다.

결론적으로 [식별자](/knowledge-base/studynote/03_network/06_network_layer_ip/289_identification_flags_fragmentation_offset/)는 ER 모델에서 가장 무거운 책임을 지는 [속성](/knowledge-base/studynote/05_database/02_modeling_normalization/082_attribute_types_er_model/)이며, 단순한 '고유 값'이 아니라 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 생명주기 전체를 관통하는 시스템의 [식별](/knowledge-base/studynote/09_security/13_secops_ir_forensics/655_ir_detection_analysis/) 체계로 인식해야 한다.

- **📢 섹션 요약 비유**: 건물의 철골 구조([식별자](/knowledge-base/studynote/03_network/06_network_layer_ip/289_identification_flags_fragmentation_offset/))를 처음부터 단단히 세워두면 나중에 벽지([속성](/knowledge-base/studynote/05_database/02_modeling_normalization/082_attribute_types_er_model/))를 몇 번을 바꿔도 건물은 무너지지 않는다.
...
3. [식별자](/knowledge-base/studynote/03_network/06_network_layer_ip/289_identification_flags_fragmentation_offset/)는 수많은 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 중에서 "바로 이거야!"라고 딱 집어낼 수 있게 해주는 마법의 번호랍니다.
