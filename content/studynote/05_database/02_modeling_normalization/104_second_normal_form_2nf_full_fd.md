...
## 핵심 인사이트 (3줄 요약)
> 1. **본질**: 제2정규형 (2NF, [Second Normal Form](/knowledge-base/studynote/05_database/04_transactions_concurrency/527_second_normal_form/))은 릴레이션의 모든 일반 [속성](/knowledge-base/studynote/05_database/02_modeling_normalization/082_attribute_types_er_model/)이 [기본 키](/knowledge-base/studynote/05_database/02_modeling_normalization/070_primary_key_alternate_key/) (PK, Primary [Key](/knowledge-base/studynote/05_database/02_modeling_normalization/067_db_key_uniqueness_minimality/)) 전체에 완전히 종속되도록 '부분 함수 종속 ([Partial Functional Dependency](/knowledge-base/studynote/05_database/02_modeling_normalization/097_partial_functional_dependency/))'을 제거하는 과정이다.
> 2. **가치**: 복합 키의 일부에만 종속된 [속성](/knowledge-base/studynote/05_database/02_modeling_normalization/082_attribute_types_er_model/)들을 분리함으로써, [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 삽입·수정·삭제 시 발생하는 [이상 현상](/knowledge-base/studynote/05_database/02_modeling_normalization/090_anomaly_insertion_deletion_update/) ([Anomaly](/knowledge-base/studynote/05_database/04_transactions_concurrency/530_anomaly/))을 방지하고 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) [무결성](/knowledge-base/studynote/09_security/01_intro_principles/003_integrity/)을 확보한다.
> 3. **판단 포인트**: 테이블의 [기본 키](/knowledge-base/studynote/05_database/02_modeling_normalization/070_primary_key_alternate_key/)가 단일 [속성](/knowledge-base/studynote/05_database/02_modeling_normalization/082_attribute_types_er_model/)이면 2NF는 자동으로 만족되므로, 복합 키가 존재하는 테이블에서만 부분 종속 여부를 진단하고 분해를 결정하면 된다.

---

## Ⅰ. 개요 및 필요성

제2정규형 (2NF)은 [제1정규형](/knowledge-base/studynote/05_database/02_modeling_normalization/103_first_normal_form_1nf_atomic_value/) ([1NF](/knowledge-base/studynote/05_database/02_modeling_normalization/103_first_normal_form_1nf_atomic_value/), 모든 도메인이 원자값)을 만족하는 테이블에서, [기본 키](/knowledge-base/studynote/05_database/02_modeling_normalization/070_primary_key_alternate_key/)의 일부에만 종속되는 [속성](/knowledge-base/studynote/05_database/02_modeling_normalization/082_attribute_types_er_model/)들을 분리해 새로운 테이블을 만드는 [정규화](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/093_normalization/) 단계다.

[기본 키](/knowledge-base/studynote/05_database/02_modeling_normalization/070_primary_key_alternate_key/)가 두 개 이상의 컬럼으로 결합된 복합 키 ([Composite](/knowledge-base/studynote/04_software_engineering/04_testing_quality/261_composite_pattern_tree_structure/) [Key](/knowledge-base/studynote/05_database/02_modeling_normalization/067_db_key_uniqueness_minimality/))일 때 문제가 발생한다. 예를 들어 `[학번, 과목코드]`가 [기본 키](/knowledge-base/studynote/05_database/02_modeling_normalization/070_primary_key_alternate_key/)인 수강 테이블에 `학생이름`이나 `학과` 같은 [속성](/knowledge-base/studynote/05_database/02_modeling_normalization/082_attribute_types_er_model/)이 있다면, 이 [속성](/knowledge-base/studynote/05_database/02_modeling_normalization/082_attribute_types_er_model/)들은 `과목코드`와 무관하게 오직 `학번`에만 종속된다. 이를 방치하면 학생이 10과목을 수강할 때 학생 이름과 학과 정보가 10번 중복 저장되며, 이로 인해 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 수정하거나 삭제할 때 정보가 불일치해지는 [이상 현상](/knowledge-base/studynote/05_database/02_modeling_normalization/090_anomaly_insertion_deletion_update/)이 필연적으로 발생한다. 따라서 [종속성](/knowledge-base/studynote/15_devops_sre/01_culture_methodology/008_dependencies/)의 성격에 맞게 테이블을 쪼개는 작업이 필요해진다.

- **📢 섹션 요약 비유**: 2NF는 회사 내의 '파벌(사조직) 척결 작전'과 같다. 회사의 공동 대표가 A와 B 두 명(복합 PK)인데, 일부 직원들(일반 [속성](/knowledge-base/studynote/05_database/02_modeling_normalization/082_attribute_types_er_model/))이 대표 A의 지시만 듣고 B는 무시한다면, A 대표와 그 직원들을 따로 분리해 독립된 자회사(새 테이블)로 내보내어 지휘 계통을 깔끔하게 만드는 것과 같다.

---

## Ⅱ. 아키텍처 및 핵심 원리

2NF의 핵심 메커니즘은 '[무손실 분해](/knowledge-base/studynote/05_database/02_modeling_normalization/101_lossless_join_decomposition/) (Lossless Decomposition)'다. 테이블을 나눌 때 정보가 유실되지 않아야 하며, 나중에 조인 ([Join](/knowledge-base/studynote/05_database/04_transactions_concurrency/521_join/))을 통해 원래 테이블로 완벽히 복원할 수 있어야 한다.

| [종속성](/knowledge-base/studynote/15_devops_sre/01_culture_methodology/008_dependencies/) 유형 | 메커니즘 설명 | 분해 원리 |
| :--- | :--- | :--- |
...
2NF는 [제1정규형](/knowledge-base/studynote/05_database/02_modeling_normalization/103_first_normal_form_1nf_atomic_value/)과 [제3정규형](/knowledge-base/studynote/05_database/02_modeling_normalization/105_third_normal_form_3nf_transitive/) 사이의 연결 고리 역할을 하며, 특히 [종속성](/knowledge-base/studynote/15_devops_sre/01_culture_methodology/008_dependencies/)의 '출발점'이 [기본 키](/knowledge-base/studynote/05_database/02_modeling_normalization/070_primary_key_alternate_key/) 전체인지 일부인지에 초점을 맞춘다.
...
실무 [데이터베이스](/knowledge-base/studynote/05_database/01_db_architecture_relational/002_database_definition/) 설계에서 2NF 위반은 주로 매핑 테이블(N:M 관계를 해소하는 교차 테이블)에 불필요한 마스터 정보가 끼어들어 갈 때 발생한다.
...
- 주문내역 테이블의 [기본 키](/knowledge-base/studynote/05_database/02_modeling_normalization/070_primary_key_alternate_key/)를 `[주문번호, 상품코드]`로 잡은 뒤, 해당 테이블에 굳이 '상품명'이나 '상품단가' [속성](/knowledge-base/studynote/05_database/02_modeling_normalization/082_attribute_types_er_model/)을 추가로 중복 저장하는 설계 (전형적인 2NF 위반에 따른 공간 낭비와 [갱신 이상](/knowledge-base/studynote/05_database/02_modeling_normalization/093_update_anomaly/) 유발).
...
- **📢 섹션 요약 비유**: 2NF는 책상을 정리할 때 연필은 연필꽂이에, 지우개는 지우개통에 정확히 분류하는 작업이다. 덕분에 물건이 섞이거나 잃어버릴([이상 현상](/knowledge-base/studynote/05_database/02_modeling_normalization/090_anomaly_insertion_deletion_update/)) 위험은 사라지지만, 가끔 필기할 때 여러 통(조인)을 뒤져야 하는 수고가 생길 수는 있다.
...
← **이전**: [103. 제1정규형 (1NF) - 도메인이 원자값만으로 구성](/knowledge-base/studynote/05_database/02_modeling_normalization/103_first_normal_form_1nf_atomic_value/)
**다음**: [105. 제3정규형 (3NF) - 2NF 만족 및 이행적 함수 종속 제거](/knowledge-base/studynote/05_database/02_modeling_normalization/105_third_normal_form_3nf_transitive/) →
