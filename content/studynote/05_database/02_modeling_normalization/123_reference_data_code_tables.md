...
- **📢 섹션 요약 비유**: 코드 테이블은 사전의 **약어 색인**이다. "M=남성, F=여성"처럼 약속된 코드의 의미를 한 곳에서 관리한다.
...
### 👶 어린이를 위한 3줄 비유 설명
1. 코드 테이블은 **약어 사전**이에요. "M=남자, F=여자"처럼 약속된 의미를 정해요.
2. 사전이 없으면 "M이 뭐지?"라고 **사람마다 다르게 이해**할 수 있어요.
3. 사전(코드 테이블) 하나로 **모든 사람이 같은 뜻**으로 이해할 수 있답니다!
