...
<div class="kb-diagram-row kb-diagram-grid-row"><div class="kb-diagram-cell">dml-insert-update… -&gt; current scope -&gt; dcl-grant-revoke</div></div>
</div>
</div>
...
`DML (INSERT, UPDATE, DELETE 롤백 가능)`의 핵심 원리는 [스키마](/knowledge-base/studynote/05_database/01_db_architecture_relational/005_schema/), 키, 함수 종속, [무결성](/knowledge-base/studynote/09_security/01_intro_principles/003_integrity/) 제약, 언어 계층을 통해 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)의 의미와 경계를 명확히 한다는 점이다. 여기서 중요한 것은 `데이터 모델링·정합성 맥락에서 역할과 경계를 판단해야 하는 주제`를 어떤 순서로 평가하고 어느 경계에서 확정하느냐다. 이 순서가 바뀌면 정합성, [처리량](/knowledge-base/studynote/01_computer_architecture/03_architecture_basics_performance/139_throughput/), [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/)시간 중 손해를 보는 축이 달라진다.

| 관점 | 설명 | 설계 포인트 |
| :--- | :--- | :--- |
| 핵심 대상 | `DML (INSERT, UPDATE, DELETE 롤백 가능)`는 `데이터 모델링·정합성 맥락에서 역할과 경계를 판단해야 하는 주제`를 다루는 중심 규칙이다. | 먼저 무엇을 [보호](/knowledge-base/studynote/02_operating_system/10_security/571_protection_vs_security/)하거나 빠르게 할 것인지 명확히 정한다. |
...
<div class="kb-diagram-row kb-diagram-grid-row"><div class="kb-diagram-cell">DDL (CREATE, AL… -&gt; DML (INSERT, UP… -&gt; DCL (GRANT, REV…</div></div>
</div>
</div>



결국 `DML (INSERT, UPDATE, DELETE 롤백 가능)`는 한 문장 정의보다 입력 조건, 처리 순서, 결과 보장을 묶어 보는 것이 중요하다. 그래서 설계 문서에는 적용 대상, 실패 시 [복구](/knowledge-base/studynote/09_security/13_secops_ir_forensics/658_ir_recovery/) 경로, 측정 지표를 같이 적어 두는 편이 좋다.
...
3. [모니터](/knowledge-base/studynote/02_operating_system/04_synchronization/229_monitor/)링 지표, 예외 처리, [복구](/knowledge-base/studynote/09_security/13_secops_ir_forensics/658_ir_recovery/) 절차가 `DML (INSERT, UPDATE, DELETE 롤백 가능)`의 특성과 맞게 준비되어 있는가?
...
결론적으로 `DML (INSERT, UPDATE, DELETE 롤백 가능)`는 `DDL (CREATE, ALTER, DROP, TRUNCATE 롤백 불가)`와 `DCL (GRANT, REVOKE 권한 통제)` 사이에서 현재 시스템이 감당할 수 있는 균형점을 만드는 개념이다. 시험에서는 배경, 원리, 비교, 판단 기준을 함께 답하고, 실무에서는 지표와 운영 [정책](/knowledge-base/studynote/10_ai/02_dl_architecture_new/164_policy/)으로 연결할 수 있어야 한다.
...
| 개념 | 연결 포인트 |
| :--- | :--- |
| [DDL](/knowledge-base/studynote/05_database/01_db_architecture_relational/020_ddl/) (CREATE, ALTER, DROP, TRUNCATE [롤백](/knowledge-base/studynote/15_devops_sre/02_cicd_gitops/098_rollback_strategy_pipeline_error_threshold/) 불가) | 현재 주제가 등장하기 전 단계에서 드러나는 문제 또는 전제 조건을 보여 준다. |
| [DCL](/knowledge-base/studynote/05_database/01_db_architecture_relational/022_dcl/) (GRANT, REVOKE 권한 통제) | 현재 판단이 실제 확장 또는 후속 제어로 이어지는 지점을 보여 준다. |
...
<div class="kb-diagram-row"><div class="kb-diagram-node">DML (INSERT, UPDATE, DELETE…</div></div>
<div class="kb-diagram-row"><div class="kb-diagram-connector">▶</div><div class="kb-diagram-node">DCL (GRANT, REVOKE 권한 통…</div></div>
<div class="kb-diagram-row"><div class="kb-diagram-connector">▶</div><div class="kb-diagram-node">TCL (COMMIT, ROLLBACK, …</div></div>
</div>
</div>
...
← **이전**: [415. DDL (CREATE, ALTER, DROP, TRUNCATE 롤백 불가)](/knowledge-base/studynote/05_database/07_exam_summary/415_ddl_create_alter_drop/)
**다음**: [417. DCL (GRANT, REVOKE 권한 통제)](/knowledge-base/studynote/05_database/07_exam_summary/417_dcl_grant_revoke/) →
