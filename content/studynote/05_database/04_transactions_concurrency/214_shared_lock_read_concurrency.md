...
| [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) 영향 | 공유 락은 [처리량](/knowledge-base/studynote/01_computer_architecture/03_architecture_basics_performance/139_throughput/), 지연시간, 운영 복잡도 중 적어도 하나에 직접 영향을 준다. | 이득과 비용을 같이 보지 않으면 과설계가 된다. |
...
공유 락은 종종 `락킹 기법` 또는 `배타 락`과 같은 묶음으로 설명되지만, 세 개념의 관심사는 다르다. [락킹 기법](/knowledge-base/studynote/05_database/04_transactions_concurrency/213_locking_mechanism_concurrency_control/)이 준비 단계나 전제에 가깝다면, 공유 락은 실제 통제 지점을 잡고, [배타 락](/knowledge-base/studynote/05_database/04_transactions_concurrency/215_exclusive_lock_write_concurrency/)은 그 결과를 더 강하게 만들거나 다른 방향으로 확장한다. 이 차이를 구분해야 시험 답안에서도 경계와 선택 이유를 설득할 수 있다.

| 비교 축 | 공유 락 | [락킹 기법](/knowledge-base/studynote/05_database/04_transactions_concurrency/213_locking_mechanism_concurrency_control/) | [배타 락](/knowledge-base/studynote/05_database/04_transactions_concurrency/215_exclusive_lock_write_concurrency/) |
| :--- | :--- | :--- | :--- |
...
또한 공유 락은 단순 정의 암기로 끝나는 개념이 아니라, 실제로는 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/)·정합성·운영성 중 무엇을 우선할지 결정하는 기준점으로 연결된다.
...
3. 장애·튜닝·모니터링 시 공유 락을 관찰할 지표와 [롤백](/knowledge-base/studynote/15_devops_sre/02_cicd_gitops/098_rollback_strategy_pipeline_error_threshold/) 전략이 준비되어 있는가?
...
공유 락을 올바르게 적용하면 구조를 단순화하고, 정합성을 높이거나 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/)을 안정화하며, 장애 대응 속도까지 개선할 수 있다. 반대로 적용 위치를 잘못 잡으면 중복 설계와 불필요한 복잡도만 늘어난다. 그래서 이 주제는 정의 하나보다도 "어디에 두어야 하는가"라는 배치 감각으로 기억하는 것이 중요하다.
