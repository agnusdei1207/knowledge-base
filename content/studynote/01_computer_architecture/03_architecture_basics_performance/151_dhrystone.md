...
| :-- | :-- | :-- |
| 정수 연산 | 덧셈, 대입, 비교 | 기본 [ALU](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/117_alu/) ([Arithmetic Logic Unit](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/117_alu/)) 처리 효율 |
| 분기/반복 | if, [switch](/knowledge-base/studynote/03_network/05_lan_wan_l2_devices/238_switch_operation_principles/), loop | 분기 예측과 제어 흐름 처리 능력 |
| 포인터/구조체 | [참조](/knowledge-base/studynote/05_database/05_distributed_nosql_newsql/316_reference_pattern_nosql/), 갱신, 전달 | 주소 계산과 메모리 접근 패턴 |
| [함수 호출](/knowledge-base/studynote/06_ict_convergence/04_ai_llm/294_function_calling_tool_use/) | 프로시저 호출 빈도 | 호출 오버헤드와 [레지스터](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/057_register/) 활용 |
...
Dhrystone을 이해하려면 무엇을 잘 재고, 무엇을 놓치는지 경계를 분명히 봐야 한다. 가장 직접적인 비교 대상은 Whetstone과 현대의 SPEC CPU, 그리고 임베디드 분야의 CoreMark다. Dhrystone은 정수와 제어 흐름 중심, Whetstone은 [부동소수점](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/087_floating_point/)([Floating Point](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/087_floating_point/)) 중심, SPEC CPU는 실제 응용에 가까운 대형 워크로드, CoreMark는 임베디드 제어 패턴 보강에 초점을 둔다.

| 항목 | Dhrystone | [Whetstone](/knowledge-base/studynote/01_computer_architecture/03_architecture_basics_performance/152_whetstone/) | SPEC CPU | CoreMark |
| :-- | :-- | :-- | :-- | :-- |
| 주 대상 | 정수, 분기, 포인터 | [부동소수점](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/087_floating_point/), 수치 계산 | 대형 실제형 워크로드 | 임베디드 제어형 워크로드 |
| 코드 규모 | 작음 | 작음 | 큼 | 작음 |
| 강점 | 제어 중심 CPU 비교 용이 | FPU ([Floating Point](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/087_floating_point/) Unit) [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) 가시화 | 현실성 높음 | MCU 비교에 적합 |
...
실무에서 Dhrystone은 “써야 하느냐, 버려야 하느냐”가 아니라 “어디까지 믿을 것이냐”가 핵심 판단 포인트다. 소형 [마이크로컨트롤러](/knowledge-base/studynote/01_computer_architecture/03_architecture_basics_performance/130_microcontroller/)([Microcontroller](/knowledge-base/studynote/01_computer_architecture/03_architecture_basics_performance/130_microcontroller/), MCU), 실시간 제어기, [운영체제](/knowledge-base/studynote/02_operating_system/01_overview_architecture/001_operating_system_purpose/) 없는 베어메탈(Bare Metal) 환경에서는 대형 벤치마크를 올리기 어렵기 때문에, Dhrystone이나 DMIPS/MHz는 여전히 빠른 1차 비교 자료가 된다. 이때는 같은 전력 예산과 같은 클럭에서 어느 코어가 더 많은 정수성 제어 작업을 처리하는지 보는 용도로 의미가 있다.

반대로 애플리케이션 프로세서, 서버 CPU, 고성능 데스크톱처럼 메모리 계층과 병렬성이 체감 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/)을 좌우하는 환경에서는 Dhrystone 단독 판단이 위험하다. 사용자가 느끼는 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/)은 대개 대용량 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 이동, 멀티코어 확장성, [부동소수점](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/087_floating_point/), 벡터 처리, 저장장치 대기까지 함께 묶여 있기 때문이다. 따라서 고성능 시스템 선정에서 Dhrystone만 제시하는 것은 설계 근거가 아니라 참고 수치에 머무른다.
...
- Dhrystone 점수만으로 스마트폰이나 서버의 “전체 체감 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/)”을 단정하는 경우
- 컴파일러 최적화 차이를 숨긴 채 코어 자체 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) 차이처럼 홍보하는 경우
- DMIPS를 실제 MIPS와 동일한 절대 척도로 오해하는 경우

따라서 기술사 답안이나 설계 검토에서는 “Dhrystone은 임베디드 정수성 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/)의 빠른 비교 지표로는 유용하나, 현대 범용 시스템의 종합 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) 판단에는 불충분하다”라고 정리하는 것이 안전하다.

- **📢 섹션 요약 비유**: Dhrystone은 작은 배터리 장난감 자동차를 고를 때는 유용한 시험 주행장이지만, 고속도로용 승용차를 고르면서 그 주행장 기록만 보는 것은 위험하다. 장난감 트랙 성적과 실제 도로 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/)은 같지 않기 때문이다.

---

## Ⅴ. 기대효과 및 결론

Dhrystone의 가장 큰 공헌은 벤치마크 문화에 “같은 일을 시켜 보고 비교하자”는 기준을 심었다는 점이다. 이는 단순 클럭 경쟁이나 [명령어](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/158_instruction/) 개수 경쟁보다 훨씬 진전된 접근이었고, 이후 다양한 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) 평가 체계가 발전하는 출발점이 되었다. 특히 임베디드 분야에서는 지금도 작은 코드 크기와 간단한 실행 조건 덕분에 빠른 비교 도구로 살아남아 있다.
...
| 개념 | 연결 포인트 |
| :-- | :-- |
| [MIPS](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/201_mips/) (Million Instructions Per Second) | Dhrystone이 보완하려 한 기존 단순 [명령어](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/158_instruction/) 처리율 지표 |
| DMIPS (Dhrystone [MIPS](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/201_mips/)) | Dhrystone 결과를 기준 시스템 대비 상대 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/)으로 환산한 값 |
| [Whetstone](/knowledge-base/studynote/01_computer_architecture/03_architecture_basics_performance/152_whetstone/) | [부동소수점](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/087_floating_point/) 중심 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/)을 보는 대응 벤치마크 |
| CoreMark | 현대 임베디드 환경에서 Dhrystone의 약점을 보완하려는 벤치마크 |
| SPEC CPU | 범용 시스템의 현실적 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) 평가로 확장된 대형 벤치마크 |
| [캐시 메모리](/knowledge-base/studynote/01_computer_architecture/06_memory_hierarchy_cache/259_cache_memory/) ([Cache Memory](/knowledge-base/studynote/01_computer_architecture/06_memory_hierarchy_cache/259_cache_memory/)) | Dhrystone 결과가 실제보다 높게 보일 수 있는 핵심 배경 |
