...
> 1. **본질**: 속도 [향상도](/knowledge-base/studynote/14_data_engineering/02_math_mining/086_lift_association_rule_marketing/) (Speedup)는 개선 전 실행 시간과 개선 후 실행 시간을 비교해, 특정 설계 변화가 실제로 몇 배의 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) 이득을 만들었는지 보여 주는 상대 지표다.
> 2. **가치**: 이 지표를 쓰면 “클럭을 올렸다”, “코어를 늘렸다”, “가속기를 붙였다” 같은 변화가 체감상 좋아 보이는지 아니라, 전체 시스템 관점에서 돈값을 하는지 판단할 수 있다.
> 3. **판단 포인트**: 속도 [향상도](/knowledge-base/studynote/14_data_engineering/02_math_mining/086_lift_association_rule_marketing/)는 항상 전체 실행 시간 기준으로 읽어야 하며, 부분 개선의 배율이 아무리 커도 개선 대상이 차지하는 비중이 작으면 전체 Speedup은 빠르게 한계에 부딪힌다.
...
또한 Speedup은 “무엇이 빨라졌는가”가 아니라 “전체가 얼마나 빨라졌는가”를 묻는 지표이기 때문에 중요하다. 특정 함수만 10배 빨라졌더라도 그 함수가 전체 시간의 5%만 차지했다면, 사용자가 느끼는 전체 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) 개선은 훨씬 작을 수 있다. 즉 Speedup은 부분 최적화의 환상을 걷어내고, 실제 투자 판단을 전체 관점으로 끌어올리는 역할을 한다.
...
| [병렬](/knowledge-base/studynote/05_database/07_exam_summary/430_index_fast_full_scan/) 효율성 | `Speedup / N` | 코어 1개당 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) 기여가 충분한가 | 멀티코어 투자 타당성 판단 |
...
| :--- | :--- | :--- | :--- |
| 무엇을 보나 | 개선 전후 배율 | 자원 대비 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) 전환율 | 단위 시간당 완료량 |
...
- **📢 섹션 요약 비유**: Speedup이 “차가 예전보다 몇 배 빨라졌나”를 보는 기록이라면, [병렬](/knowledge-base/studynote/05_database/07_exam_summary/430_index_fast_full_scan/) 효율성은 “연료를 넣은 만큼 제대로 달렸나”를 보는 연비표이고, [처리량](/knowledge-base/studynote/01_computer_architecture/03_architecture_basics_performance/139_throughput/)은 “한 시간에 몇 대를 실어 날랐나”를 보는 물류 실적표다.
...
- [데이터베이스](/knowledge-base/studynote/05_database/01_db_architecture_relational/002_database_definition/) 질의 최적화로 전체 요청 시간 200ms를 120ms로 줄였다면 Speedup은 1.67배다. 이 정도면 사용자 체감에도 의미가 크고, 서버 증설 없이 [처리량](/knowledge-base/studynote/01_computer_architecture/03_architecture_basics_performance/139_throughput/)도 함께 개선될 가능성이 높다.
- 반대로 [GPU](/knowledge-base/studynote/01_computer_architecture/12_accelerators_ai_hardware/418_gpu/) 가속기로 특정 커널만 20배 빨라졌지만, [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 전송 시간이 길어 전체 파이프라인이 1.3배밖에 빨라지지 않았다면 홍보용 숫자와 운영 효과는 다르다. 이 경우는 가속기 도입보다 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 이동 구조 재설계가 더 중요할 수 있다.

### [안티패턴](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/128_water_scrum_fall_anti_pattern/)

- **베스트 케이스만 제시하는 벤치마크**: 캐시 적중률이 유난히 높은 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)셋 하나로 큰 Speedup을 주장하면 일반 workload에서 재현되지 않는다.
- **코어 수를 Speedup으로 착각하는 보고서**: 16코어라고 16배 향상을 기대하면 오버헤드와 순차 구간을 무시한 계산이다.
- **평균만 보는 평가**: 평균 Speedup이 좋아 보여도 [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/) 꼬리 구간이 길면 실시간 [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/) 품질은 나빠질 수 있다.
...
| 구스타프슨의 법칙 (Gustafson's Law) | 문제 크기를 확장할 때 [병렬](/knowledge-base/studynote/05_database/07_exam_summary/430_index_fast_full_scan/) 자원으로 더 큰 Speedup을 얻을 수 있다는 관점을 제공한다. |
| [병렬](/knowledge-base/studynote/05_database/07_exam_summary/430_index_fast_full_scan/) 효율성 (Parallel Efficiency) | 같은 Speedup이라도 몇 개의 코어로 만들었는지 따져 자원 활용도를 평가한다. |
| [처리량](/knowledge-base/studynote/01_computer_architecture/03_architecture_basics_performance/139_throughput/) ([Throughput](/knowledge-base/studynote/01_computer_architecture/03_architecture_basics_performance/139_throughput/)) | Speedup이 한 작업의 개선 배율이라면, Throughput은 일정 시간 동안 몇 개를 처리했는지 본다. |
| [컴퓨터 성능 방정식](/knowledge-base/studynote/01_computer_architecture/03_architecture_basics_performance/142_performance_equation/) ([Performance Equation](/knowledge-base/studynote/01_computer_architecture/03_architecture_basics_performance/142_performance_equation/)) | 어떤 요소를 줄였을 때 Speedup이 생기는지를 `IC × CPI × Clock Cycle Time` 관점에서 분해하게 해 준다. |
