...
> 1. **본질**: [FIFO](/knowledge-base/studynote/02_operating_system/04_synchronization/261_fifo_page_replacement/) (First-In, First-Out)는 캐시에 들어온 순서만 기억하고 가장 먼저 들어온 블록을 먼저 내보내는, 가장 단순한 교체 [정책](/knowledge-base/studynote/10_ai/02_dl_architecture_new/164_policy/)이다.
> 2. **가치**: [참조](/knowledge-base/studynote/05_database/05_distributed_nosql_newsql/316_reference_pattern_nosql/) 이력을 계속 추적하지 않아도 되므로 하드웨어 회로와 전력 소모를 작게 유지할 수 있어, 단순 버퍼와 저비용 시스템에서 여전히 유효하다.
> 3. **판단 포인트**: FIFO는 [시간적 지역성](/knowledge-base/studynote/01_computer_architecture/06_memory_hierarchy_cache/247_temporal_locality/) ([Temporal Locality](/knowledge-base/studynote/01_computer_architecture/06_memory_hierarchy_cache/247_temporal_locality/))을 반영하지 못해 자주 쓰는 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)도 오래 머물렀다는 이유로 제거할 수 있으며, 이 약점은 벨라디의 모순 (Bélády's [Anomaly](/knowledge-base/studynote/05_database/04_transactions_concurrency/530_anomaly/))으로까지 이어진다.
...
FIFO의 핵심 구현은 보통 큐 ([Queue](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/058_queue/)) 또는 원형 버퍼 (Circular Buffer)다. 새 블록이 들어오면 뒤쪽에 기록하고, 교체가 필요하면 앞쪽 포인터가 가리키는 블록을 내보낸다. [참조](/knowledge-base/studynote/05_database/05_distributed_nosql_newsql/316_reference_pattern_nosql/)가 일어나더라도 그 블록의 위치를 다시 정렬하지 않으므로, 읽기 히트([hit](/knowledge-base/studynote/01_computer_architecture/06_memory_hierarchy_cache/263_cache_hit_miss/)) 시 추가 업데이트가 거의 없다.

이 구조는 회로 입장에서 매우 가볍다. LRU처럼 매 접근마다 순위를 재계산하거나, [LFU](/knowledge-base/studynote/02_operating_system/04_synchronization/263_lfu_page_replacement/) ([Least Frequently Used](/knowledge-base/studynote/02_operating_system/04_synchronization/263_lfu_page_replacement/))처럼 [참조](/knowledge-base/studynote/05_database/05_distributed_nosql_newsql/316_reference_pattern_nosql/) 횟수 [카운터](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/059_counter/)를 증가시킬 필요가 없다. 결과적으로 [비교기](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/043_comparator/), [카운터](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/059_counter/), 상태 [비트](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/073_bit/) 수가 줄어들어 설계가 단순해지고, 접근 지연과 전력 부담도 낮아진다.

다만 단순함의 대가도 분명하다. FIFO는 어떤 블록이 방금 다시 쓰였는지를 반영하지 못한다. 따라서 오래전에 들어왔지만 지금도 반복 사용 중인 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)가, 막 들어왔지만 다시는 쓰이지 않을 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)보다 먼저 쫓겨나는 비합리적 상황이 발생할 수 있다.
...
| 항목 | FIFO의 처리 방식 | 의미 |
| :--- | :--- | :--- |
| 삽입 기준 | 새 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)는 뒤에 배치 | 시간 순서 유지 |
| 교체 기준 | 가장 먼저 들어온 블록 제거 | 구현 단순 |
| 히트 시 동작 | 위치 변경 없음 | 제어 오버헤드 낮음 |
| 약점 | 최근 재사용 여부 미반영 | [적중률](/knowledge-base/studynote/01_computer_architecture/06_memory_hierarchy_cache/264_hit_ratio/) 손해 가능 |
...
| :--- | :--- | :--- | :--- | :--- |
| [FIFO](/knowledge-base/studynote/02_operating_system/04_synchronization/261_fifo_page_replacement/) | 먼저 들어온 순서 | 구현 가장 단순 | 자주 쓰는 블록도 제거 가능 | 큐, 링 버퍼 |
...
1. [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)가 순서대로 소비되는가, 아니면 반복 재사용되는가?
2. [참조](/knowledge-base/studynote/05_database/05_distributed_nosql_newsql/316_reference_pattern_nosql/) 이력을 추적할 하드웨어/소프트웨어 비용을 감당할 수 있는가?
3. 미스 패널티보다 제어 오버헤드 절감 효과가 더 큰가?
4. 벨라디의 모순이 문제 될 정도로 워킹셋([Working Set](/knowledge-base/studynote/02_operating_system/04_synchronization/265_working_set/)) 변동이 큰가?

### [안티패턴](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/128_water_scrum_fall_anti_pattern/)

- "구현이 쉬우니 캐시에도 일단 [FIFO](/knowledge-base/studynote/02_operating_system/04_synchronization/261_fifo_page_replacement/)"라는 접근
- 반복 [참조](/knowledge-base/studynote/05_database/05_distributed_nosql_newsql/316_reference_pattern_nosql/) [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)가 많은 환경에서 최근성 정보 없이 운영하는 설계
...
FIFO의 가장 큰 효과는 설계 명확성이다. 교체 규칙이 단순하므로 검증이 쉽고, 자원 사용량도 예측 가능하다. 작은 임베디드 시스템이나 스트리밍형 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 경로에서는 이 예측 가능성이 곧 안정성으로 이어진다.
...
| 개념 | 연결 포인트 |
| :--- | :--- |
| [캐시 교체 알고리즘](/knowledge-base/studynote/01_computer_architecture/06_memory_hierarchy_cache/271_replacement_policy/) ([Replacement Policy](/knowledge-base/studynote/01_computer_architecture/06_memory_hierarchy_cache/271_replacement_policy/)) | FIFO는 가장 기본적인 교체 [정책](/knowledge-base/studynote/10_ai/02_dl_architecture_new/164_policy/) 축에 해당한다. |
| [시간적 지역성](/knowledge-base/studynote/01_computer_architecture/06_memory_hierarchy_cache/247_temporal_locality/) ([Temporal Locality](/knowledge-base/studynote/01_computer_architecture/06_memory_hierarchy_cache/247_temporal_locality/)) | FIFO가 충분히 반영하지 못하는 핵심 성질이다. |
| [LRU](/knowledge-base/studynote/02_operating_system/04_synchronization/262_lru_page_replacement/) ([Least Recently Used](/knowledge-base/studynote/02_operating_system/04_synchronization/262_lru_page_replacement/)) | FIFO의 단순함과 대비되는 최근성 기반 [정책](/knowledge-base/studynote/10_ai/02_dl_architecture_new/164_policy/)이다. |
| 벨라디의 모순 (Bélády's [Anomaly](/knowledge-base/studynote/05_database/04_transactions_concurrency/530_anomaly/)) | FIFO가 대표적으로 드러내는 구조적 한계다. |
| Second Chance / [Clock](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/045_clock/) | FIFO에 [참조](/knowledge-base/studynote/05_database/05_distributed_nosql_newsql/316_reference_pattern_nosql/) [비트](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/073_bit/)를 더해 실무적으로 개선한 형태다. |
...
← **이전**: [273. LFU (Least Frequently Used)](/knowledge-base/studynote/01_computer_architecture/06_memory_hierarchy_cache/273_lfu/)
**다음**: [275. 캐시 쓰기 정책 (Write Policy)](/knowledge-base/studynote/01_computer_architecture/06_memory_hierarchy_cache/275_write_policy/) →
