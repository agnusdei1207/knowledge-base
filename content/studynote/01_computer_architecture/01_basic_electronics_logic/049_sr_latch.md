...
> 2. 출력이 입력으로 피드백되는 순차 [[369_logic_bomb|논리]]([[044_sequential_logic|Sequential Logic]])의 핵심 원리 — 조합 [[369_logic_bomb|논리]]([[032_combinational_logic|Combinational Logic]])가 현재 입력만으로 출력을 결정하는 반면, SR 래치는 이전 상태를 기억하여 출력이 입력 변화 후에도 유지된다.
> 3. S=1, R=1(또는 NAND에서 S=0, R=0)은 금지 조건(Forbidden [[272_state_pattern|State]]) — 두 출력이 동시에 같은 값이 되어 정의되지 않은 상태가 발생하며, 이 한계를 극복하기 위해 D [[051_flip_flop|플립플롭]]·JK [[051_flip_flop|플립플롭]] 등이 개발되었다.
...
> 📢 **섹션 요약 비유**: 금지 조건 해결 = 혼선 방지 [[130_signal|신호]] — SR(둘 다 말할 수 있어 혼선), D(한 명만 말함 → 혼선 없음), JK(둘 다 말하면 역할 바꾸기로 정의). 혼선 원천 차단이 핵심!
...
> 📢 **섹션 요약 비유**: [[051_flip_flop|플립플롭]] 종류 = 직원 역할 분담 — D(단순 보조: 시키는 대로), JK(다재다능: 세 가지 역할), T(전문가: 토글만). CPU는 수십억 D [[051_flip_flop|플립플롭]]으로 [[057_register|레지스터]] 구성!

---

## Ⅴ. 실무 시나리오 — CPU [[057_register|레지스터]] 설계
...
> 📢 **섹션 요약 비유**: CPU [[057_register|레지스터]] = 계산기 화면 — D [[051_flip_flop|플립플롭]] 64개가 RAX [[057_register|레지스터]]. CLK마다 새 값 저장. 3GHz = 0.33ns마다 갱신. 현대 CPU는 수십억 개 래치로 고속 연산!
...
2. D [[051_flip_flop|플립플롭]] = 클록마다 [[022_snapshot_backup_architecture|스냅샷]] — 카메라(D FF)가 종(CLK) 울릴 때마다 현재 모습(D) 찍어 저장(Q). 3GHz = 초당 30억 장 촬영!
3. [[250_sram|SRAM]] 캐시 = 빠른 메모 판 — L1 캐시는 6개 [[014_transistor|트랜지스터]] SR 래치. DRAM보다 100배 빠르지만 비용도 100배. CPU 옆에 작게 두는 이유!
...
← **이전**: [[048_latch|048. 래치 — Latch]]
**다음**: [[050_d_latch|D 래치 (D Latch)]] →
