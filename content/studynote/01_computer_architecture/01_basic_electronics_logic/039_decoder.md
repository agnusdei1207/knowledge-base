...
> 3. 실제 CPU에서 디코더는 두 가지 핵심 역할 — [[158_instruction|명령어]] 해독([[158_instruction|Instruction]] Decoder: 연산코드 → 제어 [[130_signal|신호]])과 메모리 어드레스 디코딩(Address Decoder: 주소 → 메모리 셀 선택) — 으로 디지털 시스템의 '결정' 회로를 담당한다.
...
> 📢 **섹션 요약 비유**: 디코더는 스위치판 같아서, 원하는 민터름 조합에 전선 연결하면 어떤 [[369_logic_bomb|논리]] 함수든 구현 가능 — FPGA의 LUT(Look-Up Table) 원리.

---

## [[288_version_ihl_tos_total_length|IV]]. CPU [[158_instruction|명령어]] 디코더
...
> 📢 **섹션 요약 비유**: CPU [[158_instruction|명령어]] 디코더는 악보 해독기 — 음표([[159_opcode|opcode]])를 읽어 어떤 악기([[117_alu|ALU]], [[057_register|레지스터]])를 어떻게 연주할지(제어 [[130_signal|신호]]) 결정.

---

## V. 실무 시나리오 — [[606_dynamic_partial_reconfiguration|FPGA]] 설계
...
> 📢 **섹션 요약 비유**: FPGA의 주소 디코더는 사무실 교환기 — 전화번호(주소) 앞자리로 어느 부서([[192_module_independence|모듈]]) 연결할지 자동으로 [[339_routing_overview_best_path_selection|라우팅]].
...
2. CPU가 덧셈 명령을 받으면 [[158_instruction|명령어]] 디코더가 "덧셈 회로를 켜고 나머지는 꺼라"는 [[130_signal|신호]]를 자동으로 만들어요.
...
← **이전**: [[038_parallel_adder_subtractor|038. 병렬 가감산기 (Parallel Adder-Subtractor)]]
**다음**: [[040_encoder|040. 인코더 (Encoder)]] →
