...
## 핵심 인사이트 (3줄 요약)
> 1. **본질**: 커패시터 (Capacitor)는 두 [도체](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/008_conductor/) 판 사이에 절연물질을 넣어 전하(Charge)를 일시적으로 저장하고 방출하는 정전기적 에너지 저장 소자다.
> 2. **가치**: [DRAM](/knowledge-base/studynote/01_computer_architecture/06_memory_hierarchy_cache/251_dram/) 셀에서 1비트의 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 저장하는 핵심 그릇이자, 전원 [공급망](/knowledge-base/studynote/04_software_engineering/08_security_compliance_devsecops/520_supply_chain_attack_and_ci_cd_security/)(PDN)에서 순간적인 [전압](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/001_voltage/) 강하를 막아주는 노이즈 필터 겸 비상 배터리로 활약한다.
> 3. **판단 포인트**: [반도체](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/009_semiconductor/) 미세화 시 DRAM의 저장 공간 확보를 위해서는 고유전율(High-K) 물질이 필요하지만, 로직 배선 간의 [신호](/knowledge-base/studynote/02_operating_system/02_process_thread/130_signal/) [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/)(RC)을 막으려면 저유전율(Low-K) 물질을 써야 하는 구조적 상충 관계를 갖는다.
...
- **📢 섹션 요약 비유**: [DRAM](/knowledge-base/studynote/01_computer_architecture/06_memory_hierarchy_cache/251_dram/) 커패시터는 바닥에 미세한 구멍이 뚫린 물컵과 같다. 물을 채워 1을 표시해도 계속 새어나가기 때문에, 컵이 완전히 비어버리기 전에 부지런히 돌아다니며 물을 다시 채워줘야(리프레시) 기억이 지워지지 않는다.
...
[운영체제](/knowledge-base/studynote/02_operating_system/01_overview_architecture/001_operating_system_purpose/) 관점에서 볼 때 [DRAM](/knowledge-base/studynote/01_computer_architecture/06_memory_hierarchy_cache/251_dram/) 커패시터의 리프레시 타임(보통 64ms)은 메모리 컨트롤러가 정상적인 읽기/쓰기를 멈춰야 하는 오버헤드 시간이다. 실시간 [운영체제](/knowledge-base/studynote/02_operating_system/01_overview_architecture/001_operating_system_purpose/)(RTOS)에서는 이 찰나의 블로킹 현상마저 예측하여 스케줄링해야 할 만큼 하드웨어 구조가 소프트웨어 로직에 직접적인 영향을 미친다.
...
← **이전**: [4. 임피던스 (Impedance)](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/004_impedance/)
**다음**: [6. 정전용량 (Capacitance)](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/006_capacitance/) →
