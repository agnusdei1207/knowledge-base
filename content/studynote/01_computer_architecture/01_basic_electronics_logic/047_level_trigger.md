...
> 2. 레벨 [[507_acid_properties|트리거]]의 핵심 문제는 투명성(Transparency)과 글리치(Glitch) — 레벨 High 동안 입력 변화가 즉시 출력에 반영되어(투명), 노이즈나 설계 오류로 의도치 않은 상태 변화(글리치)가 발생할 수 있다.
> 3. 실무에서 D-래치(D-Latch)가 레벨 [[507_acid_properties|트리거]]의 대표 구현 — 반면 D-플립플롭(D-Flip Flop)은 엣지 [[507_acid_properties|트리거]]로 구현되어 더 예측 가능하며, 현대 동기식 디지털 설계에서는 엣지 [[507_acid_properties|트리거]]가 더 선호된다.

---

## Ⅰ. 레벨 [[507_acid_properties|트리거]] 개요
...
> 📢 **섹션 요약 비유**: 레벨 [[507_acid_properties|트리거]]는 열려 있는 문 — 문이 열려 있는(High) 동안 누구든 들어올 수 있어요. 엣지 [[507_acid_properties|트리거]]는 문이 열리는 순간만 통과 가능!
...
> 📢 **섹션 요약 비유**: 셋업/홀드 시간은 [[344_bus|버스]] 탑승 규칙 — 출발(Enable 상승) 전에(셋업) 자리 잡고, 출발 후에도(홀드) 자리에 있어야. 어기면 혼돈(메타스테이블)!

---

## Ⅴ. 실무 시나리오 — [[070_asic|ASIC]] [[001_dikw_pyramid|데이터]] 캡처
...
> 📢 **섹션 요약 비유**: [[070_asic|ASIC]] CDC에서 래치 — 두 다른 속도의 도로(클록 [[064_relation_domain|도메인]]) 사이에 잠깐 열리는 게이트(레벨 [[507_acid_properties|트리거]] 래치). 안전하게 [[001_dikw_pyramid|데이터]] 전달하는 교차로!
...
1. 레벨 [[507_acid_properties|트리거]]는 열린 문 — 문이 열려 있는(High) 동안 누구든 들어올 수 있어요. 문이 닫히면(Low) 안으로 잠겨요!
2. D-래치의 글리치 — 문이 열려 있으면 바람(노이즈)도 들어와요. 마스터-슬레이브(이중 문)로 바람을 막아요!
3. 엣지 vs 레벨 — 엣지는 "딸깍!" 소리(전환 순간)만 반응, 레벨은 불이 켜진 동안 계속 반응. 현대 설계는 엣지 [[507_acid_properties|트리거]]를 선호해요!
...
← **이전**: [[046_edge_trigger|046. 에지 트리거 — Edge Trigger]]
**다음**: [[048_latch|048. 래치 — Latch]] →
