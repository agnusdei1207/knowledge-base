...
> 2. n비트 [[430_index_fast_full_scan|병렬]] 가감산기의 핵심은 Ripple Carry vs Carry Lookahead 트레이드오프 — Ripple Carry는 단순하지만 n단 [[015_지연_데이터_관점|지연]], CLA([[036_carry_lookahead_adder|Carry Lookahead Adder]])는 복잡하지만 O(log n) [[015_지연_데이터_관점|지연]]으로 고속 CPU에 필수다.
> 3. 오버플로([[095_overflow|Overflow]]) 감지는 부호 있는 연산의 필수 기능으로, 두 양수를 더해 음수가 되거나 두 음수를 더해 양수가 될 때 발생하며, [[080_msb|MSB]] Carry-In과 Carry-Out의 XOR로 검출한다.

---

## I. [[430_index_fast_full_scan|병렬]] 가감산기 구조
...
| 제어 [[130_signal|신호]] | 동작              | Carry-In | B 처리    |
|---------|-----------------|---------|---------|
| SUB=0  | 덧셈 (A + B)     | 0       | 그대로   |
| SUB=1  | 뺄셈 (A - B)     | 1       | XOR로 반전|

> 📢 **섹션 요약 비유**: [[238_switch_operation_principles|스위치]] 하나로 믹서기(더하기 모드)와 분리기(빼기 모드)를 바꾸는 것 — 회로 두 배 없이 기능 두 배.
...
## III. 오버플로([[095_overflow|Overflow]]) 검출
...
## [[288_version_ihl_tos_total_length|IV]]. [[117_alu|ALU]] 내 가감산기 통합
...
## V. 실무 시나리오 — [[200_riscv|RISC-V]] [[117_alu|ALU]] 실현
...
1. [[430_index_fast_full_scan|병렬]] 가감산기는 [[238_switch_operation_principles|스위치]] 하나로 덧셈과 뺄셈을 모두 할 수 있는 계산기 회로예요.
...
← **이전**: [[037_subtractor|037. 감산기 (Subtractor) — 반감산기·전감산기]]
**다음**: [[039_decoder|039. 디코더 (Decoder) — n-to-2^n 조합 논리 회로]] →
