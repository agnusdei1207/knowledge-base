...
기술사 답안에서는 "요구 페이징은 항상 좋다"고 쓰면 부족하다. <strong>지역성이 뚜렷하고 초기 적재 비용을 줄이고 싶을 때는 채택 가치가 크지만, 지연 상한이 엄격하거나 작업 집합이 물리 메모리를 자주 넘는 환경에서는 적극적인 메모리 고정, 프리페치, 버퍼 캐시 설계가 더 중요하다</strong>고 판단해야 한다.
