...
예를 들어 오래된 PCI 기반 시스템에서 고속 네트워크 카드가 긴 버스트 DMA를 반복하면 CPU의 메모리 접근이 눈에 띄게 늦어질 수 있다. 반대로 차량 제어망에서는 짧더라도 가장 긴급한 프레임이 먼저 지나가야 안전성이 확보된다. 따라서 기술사 관점에서는 <strong>"평균 성능"과 "최악 지연" 중 무엇이 우선인지 먼저 선언하고, 그다음 중재 정책을 택하는 순서</strong>가 중요하다.
