...
> 2. **가치**: [공유 메모리](/knowledge-base/studynote/02_operating_system/02_process_thread/118_shared_memory/) [페이지](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/)가 없어도 [LLC](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/744_load_line_calibration/) (Last-Level Cache) 같은 공유 캐시만 있으면 성립하므로, 프로세스 간·[컨테이너](/knowledge-base/studynote/04_software_engineering/09_cloud_native_ai_architecture/561_container_based_deployment/) 간·가상 머신 간 공격까지 가능한 범용성이 가장 큰 무기다.
> 3. **판단 포인트**: 성공의 핵심은 타이머보다도 eviction set 구축과 캐시 격리 여부에 있으므로, 방어도 공유 [페이지](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/) 차단이 아니라 캐시 [파티셔닝](/knowledge-base/studynote/05_database/03_relational_model/179_table_partitioning_concept/)·스케줄링 격리·상수 시간 구현을 함께 봐야 한다.

---

## Ⅰ. 개요 및 필요성

Prime+Probe는 세트 연관 캐시 (Set-associative Cache)의 “자리 경쟁”을 이용하는 공격이다. 공격자는 피해자와 같은 물리 [페이지](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/)를 공유하지 않아도 된다. 대신 같은 캐시 세트에 매핑되는 주소들을 자신의 데이터로 채워 넣고, 피해자가 그 세트를 사용했는지 나중에 다시 읽어 보며 판단한다.
...
| :--- | :---------- | :----------- | :--------- |
| 공유 [페이지](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/) 필요 | 없음 | 필요 | 없음 |
| 관측 단위 | 세트 단위 | 라인 단위 | 전체 실행 시간 |
| 노이즈 수준 | 높음 | 낮음 | 매우 높음 |
| 적용 범위 | 가장 넓음 | [공유 메모리](/knowledge-base/studynote/02_operating_system/02_process_thread/118_shared_memory/) 환경 | 제한 적지만 해상도 낮음 |

아키텍처 특성도 결과를 바꾼다. Inclusive cache에서는 [LLC](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/744_load_line_calibration/) 관찰만으로 상위 캐시의 흔적을 더 잘 읽을 수 있고, non-inclusive 구조에서는 상관관계가 약해져 probe 횟수가 늘어난다. 또한 [SMT](/knowledge-base/studynote/01_computer_architecture/11_multicore_synchronization/400_smt/) (Simultaneous [Multithreading](/knowledge-base/studynote/02_operating_system/02_process_thread/095_multithreading_benefits/)) 환경에서는 같은 물리 코어의 L1/L2를 공유할 가능성이 커져 공격면이 더 직접적이 된다.
...
Prime+Probe 방어는 “공유 파일을 끊자”로 끝나지 않는다. 공유 [페이지](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/)가 없어도 성립하므로, 결국 캐시 자원을 얼마나 격리하느냐가 핵심이다. 보안 워크로드를 다루는 서버라면 하드웨어 [파티셔닝](/knowledge-base/studynote/05_database/03_relational_model/179_table_partitioning_concept/)과 스케줄링 정책을 함께 설계해야 한다.
...
- [huge page](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/517_huge_page/), precise [timer](/knowledge-base/studynote/02_operating_system/01_overview_architecture/071_os_timer/), shared LLC를 모두 허용하면서 “[VM](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/598_vm_migration_nic/) 경계가 있으니 안전하다”는 가정

### [체크리스트](/knowledge-base/studynote/04_software_engineering/11_testing_validation/435_checklist_based_testing/)

- 민감 워크로드에 CAT 또는 [page](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/) coloring 정책이 적용되는가?
- 배치 스케줄러가 [SMT](/knowledge-base/studynote/01_computer_architecture/11_multicore_synchronization/400_smt/) sibling 충돌을 피하도록 설정되어 있는가?
- 암호 라이브러리가 table-based implementation을 회피하는가?
- probe 패턴처럼 비정상적인 cache miss와 [timer](/knowledge-base/studynote/02_operating_system/01_overview_architecture/071_os_timer/) 사용을 관제하는가?
...
| [LLC](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/744_load_line_calibration/) (Last-Level Cache) | 프로세스·[VM](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/598_vm_migration_nic/) 간 공격을 가능하게 하는 공유 지점 |
| [Page](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/) Coloring | OS 차원의 세트 분리 기법 |
| CAT (Cache Allocation Technology) | 하드웨어 차원의 way 격리 수단 |
| [SMT](/knowledge-base/studynote/01_computer_architecture/11_multicore_synchronization/400_smt/) | L1/L2 공유 가능성으로 공격면을 넓히는 실행 환경 |
