...
> 1. **본질**: [분기 예측](/knowledge-base/studynote/01_computer_architecture/05_control_unit_pipelining/231_branch_prediction/) 실패 페널티는 [분기 예측](/knowledge-base/studynote/01_computer_architecture/05_control_unit_pipelining/231_branch_prediction/)기 (Branch Predictor)가 잘못 고른 경로 때문에 CPU가 틀린 [명령어](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/158_instruction/)를 한동안 투기 실행 (Speculative Execution)한 뒤, 그 결과를 모두 버리고 올바른 경로로 되돌아오는 데 잃는 사이클 수다.
> 2. **가치**: 파이프라인이 깊고 발급 폭이 넓은 코어일수록 한 번의 실패가 수십 개 [명령어](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/158_instruction/)와 여러 사이클의 유효 일을 날려 버리므로, [분기 예측](/knowledge-base/studynote/01_computer_architecture/05_control_unit_pipelining/231_branch_prediction/) 정확도와 [복구](/knowledge-base/studynote/09_security/13_secops_ir_forensics/658_ir_recovery/) [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/)은 현대 코어 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/)의 핵심 변수다.
> 3. **판단 포인트**: 페널티는 단순히 "깊은 파이프라인이라 크다"로 끝나지 않고, 분기 해결 시점·프런트엔드 재시작 시간·잘못 채운 [비순차 실행 윈도우](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/502_ooo_window/) 규모가 합쳐져 결정되므로, 하드웨어와 소프트웨어가 함께 줄여야 한다.

---

## Ⅰ. 개요 및 필요성

[분기 예측](/knowledge-base/studynote/01_computer_architecture/05_control_unit_pipelining/231_branch_prediction/) 실패 페널티는 CPU (Central Processing Unit)가 `if`, `loop`, `call`, `return` 같은 제어 흐름 분기를 미리 추측했다가 틀렸을 때 지불하는 시간 손실이다. 현대 코어는 [명령어](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/158_instruction/) 공급을 끊지 않기 위해 분기 결과가 실제로 계산되기 전에 분기 방향과 목표 주소를 예측하고, 그 경로의 [명령어](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/158_instruction/)를 먼저 가져와 디코드하고 실행 준비까지 시킨다. 이 추측이 맞으면 큰 이득이지만, 틀리면 그동안 수행한 일이 모두 잘못된 길에 쏟아진 셈이 된다.

이 개념이 중요한 이유는 프로그램에 분기가 매우 자주 등장하기 때문이다. 일반적인 코드에서는 4~6개 [명령어](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/158_instruction/)마다 한 번꼴로 분기가 나오므로, 예측기가 없으면 프런트엔드가 거의 매번 멈춘다. 결국 고성능 코어는 예측을 할 수밖에 없고, 대신 틀렸을 때 얼마나 빨리 [복구](/knowledge-base/studynote/09_security/13_secops_ir_forensics/658_ir_recovery/)하느냐가 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/)의 분수령이 된다.
...
| 페널티 구성 요소 | 의미 | 줄이는 방법 |
| :-- | :-- | :-- |
| 분기 해결 [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/) | 실제 분기 결과가 나올 때까지의 단계 수 | 비교 연산 앞당기기, 조기 분기 해결 |
| 상태 정리 [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/) | 잘못된 ROB/IQ/LSQ 상태를 폐기하는 시간 | 빠른 스쿼시 (Squash) 경로, 체크포인트 리네이밍 |
...
실무에서 [분기 예측](/knowledge-base/studynote/01_computer_architecture/05_control_unit_pipelining/231_branch_prediction/) 실패 페널티를 줄이는 방법은 하드웨어와 소프트웨어로 나뉜다. 하드웨어는 분기 결과를 더 앞단에서 계산하고, 더 좋은 예측기를 쓰며, 잘못된 경로를 빠르게 폐기하도록 설계한다. 소프트웨어는 핫 루프에서 예측하기 어려운 분기를 줄이고, [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 레이아웃을 바꿔 한쪽 경로가 자주 나오게 만들며, 필요하면 분기 없는 코드 형태로 바꾼다.

대표적인 사례가 정렬 여부에 따른 루프 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) 차이다. `if (x < threshold)` 같은 분기가 있을 때 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)가 거의 정렬돼 있으면 예측기는 특정 방향을 학습해 높은 적중률을 보인다. 반대로 무작위 입력이면 분기 방향이 자주 뒤집혀 오예측이 늘고, 같은 알고리즘이라도 실제 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) 차이가 크게 벌어진다.
...
3. 분기 없는 대체 코드가 명령 수·[레지스터](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/057_register/) 압박 측면에서 더 낫거나 최소한 비슷한가?
4. 하드웨어가 분기 결과를 조기 계산할 수 있는 위치에 조건식이 놓여 있는가?

### 피해야 할 [안티패턴](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/128_water_scrum_fall_anti_pattern/)

- 거의 랜덤하게 갈리는 조건 분기를 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) 핵심 루프에 그대로 두는 코드
- 예측이 잘 맞는 분기까지 [무조건 분기](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/188_unconditional_branch/) 없는 코드로 바꿔 명령 수를 불필요하게 늘리는 최적화
- [분기 예측](/knowledge-base/studynote/01_computer_architecture/05_control_unit_pipelining/231_branch_prediction/) 실패를 단순 "예측기 문제"로만 보고, 코드 구조와 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 분포를 무시하는 분석
...
| 개념 | 연결 포인트 |
| :-- | :-- |
| 투기 실행 (Speculative Execution) | [분기 예측](/knowledge-base/studynote/01_computer_architecture/05_control_unit_pipelining/231_branch_prediction/)이 맞을 때 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/)을 얻고, 틀리면 페널티를 만든다. |
| 분기 대상 버퍼 (Branch Target Buffer, [BTB](/knowledge-base/studynote/01_computer_architecture/05_control_unit_pipelining/234_btb/)) | 올바른 목표 주소를 빨리 알려 재시작 [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/)을 줄인다. |
| 리오더 버퍼 ([Reorder Buffer](/knowledge-base/studynote/01_computer_architecture/05_control_unit_pipelining/240_reorder_buffer/), ROB) | 잘못된 경로의 결과를 폐기하고 정확한 상태를 복원한다. |
| 조건부 이동 (Conditional Move, CMOV) | 예측 어려운 분기를 제거하는 소프트웨어 대안이다. |
| [비순차 실행 윈도우](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/502_ooo_window/) | 크게 만들수록 오예측 시 한 번에 버릴 일도 커진다. |
...
← **이전**: [502. 비순차 실행 윈도우 (Out-of-Order Execution Window)](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/502_ooo_window/)
**다음**: [504. 캐시 웨이 예측 (Cache Way Prediction)](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/504_cache_way_prediction/) →
