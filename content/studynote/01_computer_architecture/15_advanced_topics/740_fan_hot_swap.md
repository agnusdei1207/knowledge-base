...
> 1. **본질**: 서버 섀시 팬 핫스왑은 시스템 전원을 끄지 않고 fan module을 교체할 수 있게 만든 고가용성 정비 구조로, 냉각도 전원·디스크처럼 [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/) 가능한 부품으로 다룬다.
> 2. **가치**: [BMC](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/710_bmc/) ([Baseboard Management Controller](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/710_bmc/)) 감시, N+1 fan redundancy, blind-mate connector가 결합되면 fan 고장 중에도 airflow를 유지하며 다운타임 없이 수리를 끝낼 수 있다.
...
핫스왑 팬 구조는 fan cartridge, guide rail, blind-mate connector, presence/tach sensing, [BMC](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/710_bmc/) 제어로 이루어진다. 팬 [모듈](/knowledge-base/studynote/04_software_engineering/04_testing_quality/192_module_independence/)은 손잡이 달린 카트리지 형태로 삽입되고, 섀시 안쪽의 커넥터와 자동 정렬된다. 이때 전원, 접지, PWM (Pulse Width Modulation) 제어선, tachometer [신호](/knowledge-base/studynote/02_operating_system/02_process_thread/130_signal/)선이 live insertion을 견디도록 설계되어, 팬을 꽂는 순간 시스템이 새 [모듈](/knowledge-base/studynote/04_software_engineering/04_testing_quality/192_module_independence/)을 인식한다.

동작 순서는 보통 다음과 같다. 첫째, BMC가 tach [신호](/knowledge-base/studynote/02_operating_system/02_process_thread/130_signal/) 저하나 rotor stall을 감지한다. 둘째, 남은 팬들의 duty를 높여 부족한 airflow를 보전한다. 셋째, 관리자가 고장 팬 카트리지를 뽑고 새 [모듈](/knowledge-base/studynote/04_software_engineering/04_testing_quality/192_module_independence/)을 삽입한다. 넷째, BMC가 새 팬의 회전수와 존재를 [확인](/knowledge-base/studynote/04_software_engineering/12_testing_maintenance/396_validation/)하면 boost 상태를 정상 프로파일로 복귀시킨다.

| 구성 요소 | 역할 | 설계 포인트 |
| :--- | :--- | :--- |
| Fan Cartridge | 공구 없이 교체 가능한 [모듈](/knowledge-base/studynote/04_software_engineering/04_testing_quality/192_module_independence/) | 손잡이, 방향키, FRU [식별](/knowledge-base/studynote/09_security/13_secops_ir_forensics/655_ir_detection_analysis/) |
| Blind-mate Connector | 꽂는 순간 자동 접속 | 핀 정렬, 접지 우선 접촉, 내구성 |
| [BMC](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/710_bmc/) Monitoring | 고장 감시와 경보 | tach, [전류](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/002_current/), 온도 zone 연동 |
| Redundant Fan [Policy](/knowledge-base/studynote/10_ai/02_dl_architecture_new/164_policy/) | 고장 시 즉시 풍량 보전 | N+1 여유, boost 곡선, 소음/전력 trade-off |
...
일반 [PC](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/164_pc/) 팬 교체와 서버 핫스왑의 차이는 "전원을 켜 둔 채로 가능한가"에 있다. 소비자 PC는 대개 팬이 케이블로 직접 연결되어 있고, 팬 하나가 멈추면 사용자가 전원을 끄고 분해 후 교체한다. 서버는 반대로 fan redundancy와 blind-mate mechanical design을 전제로 해서, [모듈](/knowledge-base/studynote/04_software_engineering/04_testing_quality/192_module_independence/) 단위 live replacement를 허용한다.

| 항목 | 일반 [PC](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/164_pc/) fan | 서버 hot-swap fan |
| :--- | :--- | :--- |
| 교체 방식 | 전원 차단 후 분해 | 전원 유지 상태에서 카트리지 교체 |
| 연결 구조 | 개별 케이블/나사 | blind-mate 슬롯 + tool-less [latch](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/048_latch/) |
| 장애 대응 | 사용자가 수동 [확인](/knowledge-base/studynote/04_software_engineering/12_testing_maintenance/396_validation/) | [BMC](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/710_bmc/) alarm + 자동 boost |
| 설계 전제 | 저비용, 정숙성 | 고가용성, 정비성, 예측 가능성 |

이 개념은 핫스왑 PSU ([Power](/knowledge-base/studynote/14_data_engineering/02_math_mining/069_type_1_2_error_statistical_power/) Supply Unit), [RAID](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/483_raid_overview/) 디스크, dual path network처럼 "고장 가능성을 제거하지 못하면 교체 가능하게 설계한다"는 [데이터센터](/knowledge-base/studynote/03_network/16_data_center_cloud/801_data_center_3_tier_architecture_core_aggregation_access/) 철학과도 맞닿아 있다. 따라서 핫스왑 팬은 단독 기능이 아니라, 서버 전체의 [maintainability](/knowledge-base/studynote/04_software_engineering/06_software_architecture/346_maintainability_portability/) [architecture](/knowledge-base/studynote/12_it_management/05_security_compliance/319_architecture/) 안에 있는 한 요소다.
...
2. 현재 workload와 주변 온도에서 남은 fan boost만으로 [service](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/) window를 버틸 수 있는가?
3. 동일 규격, 동일 회전 특성의 replacement module을 준비했는가?
4. 팬 교체 후 tach, PWM, inlet/outlet [temperature](/knowledge-base/studynote/10_ai/05_data_science_ml/386_llm_temperature/), CPU zone temperature가 정상 복귀했는가?
5. 같은 airflow zone에서 두 개 이상 fan을 동시에 빼지 않도록 절차가 관리되는가?

### 피해야 할 [안티패턴](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/128_water_scrum_fall_anti_pattern/)

- hot-swap 지원 여부 [확인](/knowledge-base/studynote/04_software_engineering/12_testing_maintenance/396_validation/) 없이 live removal을 시도하는 것
...
서버 섀시 팬 핫스왑이 잘 설계되면 팬 고장이 즉시 [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/) 중단으로 이어지지 않는다. 장애 감지부터 교체, 정상 복귀까지의 시간이 짧아지고, [데이터센터](/knowledge-base/studynote/03_network/16_data_center_cloud/801_data_center_3_tier_architecture_core_aggregation_access/) 운영자는 planned downtime 없이 냉각 계층을 유지보수할 수 있다. 이는 고가용성 [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/), [코로케이션](/knowledge-base/studynote/07_enterprise_systems/01_strategy_governance/062_colocation_data_center_leasing/) 운영, 야간 무인 센터에서 특히 큰 의미가 있다.
...
| 개념 | 연결 포인트 |
| :--- | :--- |
| [BMC](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/710_bmc/) ([Baseboard Management Controller](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/710_bmc/)) | 팬 고장 감지, 알람, boost 정책을 담당한다. |
...
← **이전**: [739. 히트파이프 (Heatpipe)](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/739_heatpipe/)
**다음**: [741. 이중화 전원 공급 장치 (Redundant Power Supply)](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/741_redundant_power_supply/) →
