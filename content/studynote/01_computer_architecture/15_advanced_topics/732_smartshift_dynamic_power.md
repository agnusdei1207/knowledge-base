...
> 1. **본질**: 스마트 시프트 (SmartShift)는 노트북이나 소형 시스템의 제한된 플랫폼 전력 예산을 중앙처리장치 (Central Processing Unit, CPU)와 그래픽처리장치 ([Graphics Processing Unit](/knowledge-base/studynote/01_computer_architecture/12_accelerators_ai_hardware/418_gpu/), [GPU](/knowledge-base/studynote/01_computer_architecture/12_accelerators_ai_hardware/418_gpu/)) 사이에서 동적으로 재배분하는 AMD (Advanced Micro Devices)의 시스템 수준 전력 관리 기술이다.
> 2. **가치**: 같은 섀시, 같은 [어댑터](/knowledge-base/studynote/04_software_engineering/04_testing_quality/259_adapter_pattern_interface_wrapper/), 같은 냉각 장치 안에서도 순간 병목이 CPU인지 GPU인지에 따라 예산을 옮겨 주면, 남는 열·전력 여유를 덜 버리고 실제 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/)으로 바꿀 수 있다.
> 3. **판단 포인트**: SmartShift는 총예산을 늘리는 마법이 아니라 배분 방식을 바꾸는 기술이므로, 플랫폼 지원 여부, 제조사 튜닝, 드라이버·[펌웨어](/knowledge-base/studynote/02_operating_system/01_overview_architecture/032_firmware/), 워크로드 성격에 따라 효과 차이가 크다.
...
| :--- | :--- | :--- | :--- |
| [GPU](/knowledge-base/studynote/01_computer_architecture/12_accelerators_ai_hardware/418_gpu/) 병목 게임 | CPU 여유분이 남아도 GPU는 제한 유지 | CPU 여유 일부를 GPU에 이전 | 프레임과 응답성 개선 |
| CPU 중심 생산성 작업 | [GPU](/knowledge-base/studynote/01_computer_architecture/12_accelerators_ai_hardware/418_gpu/) 예산이 놀아도 CPU는 제한 유지 | [GPU](/knowledge-base/studynote/01_computer_architecture/12_accelerators_ai_hardware/418_gpu/) 여유 일부를 CPU에 이전 | 처리 시간 단축 |
| CPU·[GPU](/knowledge-base/studynote/01_computer_architecture/12_accelerators_ai_hardware/418_gpu/) 동시 포화 | 양쪽 모두 고정 한계에 근접 | 재배분 폭이 작거나 제한적 | 개선 폭 제한 |
...
SmartShift는 CPU 내부의 Turbo Boost나 [Precision](/knowledge-base/studynote/14_data_engineering/05_exam_keywords/233_precision_recall_f1_roc_auc_threshold/) Boost와 목적이 다르다. Turbo Boost와 [Precision](/knowledge-base/studynote/14_data_engineering/05_exam_keywords/233_precision_recall_f1_roc_auc_threshold/) Boost는 **한 칩 내부에서** 여유 전력과 온도를 어떻게 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/)으로 바꿀지 다루고, SmartShift는 **서로 다른 칩 사이에서** 플랫폼 예산을 어떻게 배분할지 다룬다. 즉 전자는 칩 수준 부스트, 후자는 시스템 수준 예산 스케줄링이다.

| 비교 항목 | Turbo / [Precision Boost](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/731_amd_precision_boost/) | SmartShift | 고정 CPU·[GPU](/knowledge-base/studynote/01_computer_architecture/12_accelerators_ai_hardware/418_gpu/) 분할 |
| :--- | :--- | :--- | :--- |
| 제어 범위 | 단일 CPU 또는 단일 [GPU](/knowledge-base/studynote/01_computer_architecture/12_accelerators_ai_hardware/418_gpu/) 내부 | CPU와 [GPU](/knowledge-base/studynote/01_computer_architecture/12_accelerators_ai_hardware/418_gpu/) 사이 플랫폼 수준 | 정적 분할, 재배분 없음 |
...
3. 배터리 모드와 [AC](/knowledge-base/studynote/12_it_management/04_sdlc_testing/155_ac_actual_cost/) 모드에서 재배분 폭이 어떻게 달라지는가?
...
기술사 답안에서는 SmartShift를 "CPU·[GPU](/knowledge-base/studynote/01_computer_architecture/12_accelerators_ai_hardware/418_gpu/) 간 [동적 전력](/knowledge-base/studynote/01_computer_architecture/13_reliability_power_management/467_dynamic_power/) 예산 재배분 기술"로 정의하고, 공유 열 설계와 병목 기반 자원 배분의 필요성을 함께 설명하면 좋다. 여기에 [고정 분할 방식](/knowledge-base/studynote/02_operating_system/06_memory_management/339_fixed_partition/)의 한계와 OEM 튜닝 의존성까지 넣어 주면, 단순 신기술 소개가 아니라 플랫폼 아키텍처 판단으로 연결된다.
...
| 개념 | 연결 포인트 |
| :--- | :--- |
| 플랫폼 전력 예산 | SmartShift가 CPU와 [GPU](/knowledge-base/studynote/01_computer_architecture/12_accelerators_ai_hardware/418_gpu/) 사이에서 재배분하는 총 한도다. |
| [Precision Boost](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/731_amd_precision_boost/) | 개별 CPU 내부에서 부스트를 결정하며, SmartShift는 그 바깥의 시스템 예산을 조정한다. |
| Dynamic Boost | NVIDIA가 유사한 목표로 CPU·[GPU](/knowledge-base/studynote/01_computer_architecture/12_accelerators_ai_hardware/418_gpu/) 예산 조정을 시도하는 경쟁 개념이다. |
| 열 설계 (Thermal Design) | 공유 [히트파이프](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/739_heatpipe/), 팬, 섀시 구조가 재배분 효과의 실제 상한을 결정한다. |
| 이종 컴퓨팅 ([Heterogeneous Computing](/knowledge-base/studynote/01_computer_architecture/12_accelerators_ai_hardware/439_heterogeneous_computing/)) | 서로 다른 가속 자원 사이에서 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/)과 전력을 조율하는 흐름의 일부다. |
