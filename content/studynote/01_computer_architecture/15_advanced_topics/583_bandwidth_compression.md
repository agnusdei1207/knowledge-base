...
예를 들어 64바이트 캐시 라인이 32바이트로 줄어들면, 같은 폭의 링크에서 전송 사이클 수가 절반 수준으로 줄 수 있다. 물론 실제 실효 대역폭은 평균 압축률, 압축 가능한 트래픽 비율, 메타데이터 크기에 따라 달라진다. 그래서 기술사 관점에서는 "2배 압축"보다 <strong>평균 워크로드에서 얼마만큼 안정적으로 이득이 나는가</strong>를 보는 것이 더 중요하다.
