...
좋은 PDU 설계는 사고가 났을 때만 빛나지 않는다. 전력 텔레메트리가 쌓일수록 좀비 서버를 찾고, 랙별 여유 전력을 계산하고, 과부하를 사전에 피하는 운영 자동화가 가능해진다. 기술사 관점에서는 "몇 포트인가"보다 <strong>전력 경로를 얼마나 관측 가능하고 복구 가능하게 만들었는가</strong>가 더 중요하다.
