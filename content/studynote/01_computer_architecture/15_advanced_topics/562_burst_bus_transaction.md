...
> 1. **본질**: [버스](/knowledge-base/studynote/01_computer_architecture/09_system_bus_interconnects/344_bus/)트 [버스](/knowledge-base/studynote/01_computer_architecture/09_system_bus_interconnects/344_bus/) [트랜잭션](/knowledge-base/studynote/05_database/04_transactions_concurrency/191_transaction_concept_states/)은 한 번의 주소 제시와 중재로 연속된 여러 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) beat를 끊기지 않게 전송하여, 제어 오버헤드를 여러 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)에 나눠 담는 다중 전송 방식이다.
> 2. **가치**: 캐시 라인 충전, [Direct Memory Access](/knowledge-base/studynote/01_computer_architecture/08_io_storage_systems/318_dma/) ([DMA](/knowledge-base/studynote/02_operating_system/11_exam_summary/746_io_direct_memory_access_dma/)), [SDRAM](/knowledge-base/studynote/01_computer_architecture/06_memory_hierarchy_cache/252_sdram/) ([Synchronous](/knowledge-base/studynote/03_network/01_data_communication/010_동기식_비동기식_전송/) Dynamic Random-Access Memory) 접근처럼 공간 지역성이 있는 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 이동에서 실효 [대역폭](/knowledge-base/studynote/01_computer_architecture/03_architecture_basics_performance/140_bandwidth/)과 바이트당 에너지 효율을 크게 높인다.
> 3. **판단 포인트**: [버스](/knowledge-base/studynote/01_computer_architecture/09_system_bus_interconnects/344_bus/)트는 연속성과 정렬이 보장될 때만 강력하며, 너무 긴 [버스](/knowledge-base/studynote/01_computer_architecture/09_system_bus_interconnects/344_bus/)트나 잘못 정렬된 [버스](/knowledge-base/studynote/01_computer_architecture/09_system_bus_interconnects/344_bus/)트는 오히려 다른 마스터를 굶기고 행 경계 재접근 비용을 키울 수 있다.
...
메모리 쪽에서도 [버스](/knowledge-base/studynote/01_computer_architecture/09_system_bus_interconnects/344_bus/)트는 자연스럽다. Dynamic Random-Access Memory ([DRAM](/knowledge-base/studynote/01_computer_architecture/06_memory_hierarchy_cache/251_dram/))은 행을 열고 열 주소를 순차적으로 따라가며 읽는 구조라, 흩어진 접근보다 연속 접근에서 훨씬 효율적이다. 그래서 [버스](/knowledge-base/studynote/01_computer_architecture/09_system_bus_interconnects/344_bus/)트는 단순한 [버스](/knowledge-base/studynote/01_computer_architecture/09_system_bus_interconnects/344_bus/) 기능이 아니라, 캐시 구조와 메모리 구조가 동시에 좋아하는 전송 패턴이다.

- **📢 섹션 요약 비유**: [버스](/knowledge-base/studynote/01_computer_architecture/09_system_bus_interconnects/344_bus/)트 전송은 택배 상자 하나 보낼 때마다 주소를 다시 쓰는 대신, 같은 아파트 동 한 층 물건들을 한 번에 묶어 올리는 배송과 같다. 주소 확인은 한 번만 하고, 실제 배달 물건은 연속해서 처리하니 훨씬 빠르다.

---

## Ⅱ. 아키텍처 및 핵심 원리

[버스](/knowledge-base/studynote/01_computer_architecture/09_system_bus_interconnects/344_bus/)트를 구성하는 핵심 요소는 시작 주소, beat 크기, burst 길이, 주소 증가 규칙, 그리고 중간 정지 여부다. 예를 들어 AXI (Advanced eXtensible Interface) [버스](/knowledge-base/studynote/01_computer_architecture/09_system_bus_interconnects/344_bus/)에서 `ADDR=0x8000`, `SIZE=8B`, `LEN=8`이라면 8바이트 단위 beat 8개가 연속 전송된다. 메모리 컨트롤러는 매 beat마다 주소를 다시 받지 않고 내부 카운터로 다음 위치를 계산한다.

| 요소 | 의미 | 설계 포인트 |
| :-- | :-- | :-- |
| 시작 주소 | 첫 beat가 시작되는 위치 | 캐시 라인, [페이지](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/), [보호](/knowledge-base/studynote/02_operating_system/10_security/571_protection_vs_security/) 경계 정렬이 중요하다. |
| Beat 크기 | 한 클럭 또는 한 핸드셰이크에 이동하는 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)량 | [버스](/knowledge-base/studynote/01_computer_architecture/09_system_bus_interconnects/344_bus/) 폭과 메모리 인터페이스 폭에 맞춰야 한다. |
| Burst 길이 | 연속 beat 개수 | 길수록 효율은 높지만 공정성과 [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/)은 나빠질 수 있다. |
| Burst 종류 | FIXED, INCR, WRAP 등 주소 규칙 | [FIFO](/knowledge-base/studynote/02_operating_system/04_synchronization/261_fifo_page_replacement/) 접근, 연속 메모리, 캐시 라인 순환에 따라 다르다. |
...
| :-- | :-- | :-- | :-- |
| 핵심 목적 | 한 건의 단순 처리 | 연속 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)의 효율적 전송 | 긴 대기시간 은닉 |
| 주소 단계 | 매 beat마다 반복 | 최초 1회 후 생략 | 요청/응답 시에만 사용 |
| [버스](/knowledge-base/studynote/01_computer_architecture/09_system_bus_interconnects/344_bus/) 점유 | 짧지만 자주 발생 | 전송 중 연속 점유 | 요청 후 반납, 응답 때 재점유 |
...
- **📢 섹션 요약 비유**: 단일 전송이 택시 한 대씩 불러 손님을 태우는 방식이라면, [버스](/knowledge-base/studynote/01_computer_architecture/09_system_bus_interconnects/344_bus/)트는 셔틀버스를 한 번 보내 여러 사람을 연속해서 태우는 방식이다. 분리 [트랜잭션](/knowledge-base/studynote/05_database/04_transactions_concurrency/191_transaction_concept_states/)은 그 셔틀이 돌아올 때까지 정류장을 비워 두는 운영 방식에 가깝다.

---

## Ⅳ. 실무 적용 및 기술사 판단

실무에서는 [버스](/knowledge-base/studynote/01_computer_architecture/09_system_bus_interconnects/344_bus/)트 길이와 정렬이 가장 중요한 판단 포인트다. 예를 들어 128비트 AXI [버스](/knowledge-base/studynote/01_computer_architecture/09_system_bus_interconnects/344_bus/)가 200MHz로 동작하면 이론 [대역폭](/knowledge-base/studynote/01_computer_architecture/03_architecture_basics_performance/140_bandwidth/)은 약 3.2GB/s다. 이때 16-beat burst는 한 번에 256바이트를 전송하므로, 주소 단계가 1 beat 비용이라고 가정하면 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)가 차지하는 비율은 `16 / 17`, 즉 약 94%까지 올라간다. 반면 같은 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 단일 전송 16번으로 쪼개면 주소 단계가 16번 반복되어 실효 효율이 급감한다.

하지만 무조건 긴 burst가 좋은 것은 아니다. 실시간 제어용 코어, 오디오 스트림, 저지연 I/O처럼 짧은 응답이 중요한 환경에서는 긴 burst가 다른 마스터를 오래 막아 tail latency를 악화시킬 수 있다. 또한 잘못 정렬된 burst가 [페이지](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/) 경계나 [DRAM](/knowledge-base/studynote/01_computer_architecture/06_memory_hierarchy_cache/251_dram/) row 경계를 넘으면, 내부적으로는 두 번의 접근으로 쪼개져 기대한 이득이 줄어든다.
...
2. burst 길이가 [DRAM](/knowledge-base/studynote/01_computer_architecture/06_memory_hierarchy_cache/251_dram/) burst 길이, 캐시 라인 크기, AXI 최대 길이와 조화를 이루는가?
3. 4KB 경계, [보호](/knowledge-base/studynote/02_operating_system/10_security/571_protection_vs_security/) 구간, 장치 [레지스터](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/057_register/) 영역을 넘는 burst를 피하고 있는가?
4. 긴 burst가 [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/) 민감한 마스터를 과도하게 굶기지 않도록 [Quality of Service](/knowledge-base/studynote/03_network/07_network_layer_routing/388_qos_quality_of_service_best_effort_intserv_diffserv/) ([QoS](/knowledge-base/studynote/03_network/07_network_layer_routing/388_qos_quality_of_service_best_effort_intserv_diffserv/))나 arbitration 제한이 있는가?
5. [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)가 정말 연속적인데도 소프트웨어가 산개 접근으로 깨뜨리고 있지 않은가?

### 피해야 할 [안티패턴](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/128_water_scrum_fall_anti_pattern/)

- 무작위 산개 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 억지로 burst로 묶어 의미 없는 전송을 늘리는 설계
- 장치 [레지스터](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/057_register/)처럼 순차 주소 의미가 없는 공간에 긴 burst를 적용하는 코드
- 캐시 라인 또는 [DRAM](/knowledge-base/studynote/01_computer_architecture/06_memory_hierarchy_cache/251_dram/) row 경계를 무시해 반쪽짜리 burst를 반복하는 접근
- 높은 처리량만 보고 긴 burst를 남발해 실시간 응답성을 해치는 튜닝

- **📢 섹션 요약 비유**: [버스](/knowledge-base/studynote/01_computer_architecture/09_system_bus_interconnects/344_bus/)트 길이 조절은 마트 카트 크기를 고르는 것과 같다. 물건이 많고 통로가 넓으면 큰 카트가 유리하지만, 물건이 몇 개 없거나 통로가 좁으면 오히려 큰 카트가 다른 사람 길만 막는다.
...
- **📢 섹션 요약 비유**: [버스](/knowledge-base/studynote/01_computer_architecture/09_system_bus_interconnects/344_bus/)트는 엘리베이터를 한 층마다 비워 보내지 않고, 같은 방향 사람들을 한 번에 태워 보내는 운영법과 같다. 같은 엘리베이터라도 태우는 방식에 따라 건물 전체 흐름이 달라진다.
...
| 개념 | 연결 포인트 |
| :-- | :-- |
| 캐시 라인 | burst가 가장 자주 채워 넣는 기본 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 묶음이다. |
| [Direct Memory Access](/knowledge-base/studynote/01_computer_architecture/08_io_storage_systems/318_dma/) ([DMA](/knowledge-base/studynote/02_operating_system/11_exam_summary/746_io_direct_memory_access_dma/)) | CPU 개입 없이 큰 블록을 burst로 옮겨 효율을 극대화한다. |
| AXI Burst Type | FIXED, INCR, WRAP 규칙으로 burst의 주소 [진행](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/216_progress_in_synchronization/) 방식을 정의한다. |
| DDR SDRAM의 Burst Length 8 (BL8) | 메모리 내부 burst 길이와 [버스](/knowledge-base/studynote/01_computer_architecture/09_system_bus_interconnects/344_bus/) 전송 단위를 맞추는 대표 사례다. |
| 분리 [트랜잭션](/knowledge-base/studynote/05_database/04_transactions_concurrency/191_transaction_concept_states/) | 기다리는 시간을 숨기고, 응답 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)는 다시 burst로 보낼 수 있다. |
...
1. [버스](/knowledge-base/studynote/01_computer_architecture/09_system_bus_interconnects/344_bus/)트 전송은 물건 하나씩 계산하는 대신, 바구니에 연속해서 담긴 물건을 한 번에 계산하는 방법이에요.
