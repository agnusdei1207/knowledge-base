...
> 1. **본질**: Retpoline (Return Trampoline)은 간접 호출·간접 점프를 `call`·`ret` 기반 thunk로 바꿔, 투기적 경로는 반환 [스택](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/057_stack/) 버퍼 (Return [Stack](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/057_stack/) Buffer, RSB)가 가리키는 안전한 루프에 가두고 실제 경로만 원래 목적지로 보내는 [Spectre](/knowledge-base/studynote/01_computer_architecture/14_hardware_security_trends/483_spectre/) variant 2 완화 기법이다.
> 2. **가치**: 하드웨어를 즉시 교체할 수 없던 시기에 [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/)과 컴파일러 재빌드만으로 [분기 목표 주입](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/577_branch_target_injection/) ([Branch Target Injection](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/577_branch_target_injection/)) 노출면을 크게 줄여, 구형 CPU를 실용적으로 [보호](/knowledge-base/studynote/02_operating_system/10_security/571_protection_vs_security/)한 대표적인 소프트웨어 대응이었다.
> 3. **판단 포인트**: 효과는 모든 간접 분기가 thunk로 치환되었는지, RSB 동작이 예상대로 유지되는지, 최신 eIBRS (Enhanced [Indirect](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/177_indirect_addressing/) Branch Restricted Speculation) 하드웨어와 중복 비용이 없는지에 달려 있다.
...
Retpoline thunk의 핵심은 `ret`가 투기될 때 [BTB](/knowledge-base/studynote/01_computer_architecture/05_control_unit_pipelining/234_btb/) 대신 RSB의 예측을 우선 사용한다는 점을 활용하는 것이다. 컴파일러는 원래의 간접 호출을 직접 `jmp *reg`나 `call *reg`로 내보내지 않고, 먼저 특수한 thunk를 호출한다. 이 호출은 [스택](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/057_stack/)과 RSB에 "다음엔 [trap](/knowledge-base/studynote/02_operating_system/11_exam_summary/677_trap_based_system_call_implementation/) label로 돌아올 것"이라는 흔적을 남긴다. 그 뒤 thunk는 [스택](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/057_stack/) 꼭대기에 실제 목표 주소를 써 넣고 `ret`를 실행한다.

이때 아키텍처 경로와 투기 경로가 갈라진다. 아키텍처적으로는 [스택](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/057_stack/) 꼭대기가 실제 목표 주소로 바뀌어 있으므로 `ret`가 원래 목적지로 이동한다. 반면 프로세서가 너무 이르게 투기하면, RSB는 여전히 [trap](/knowledge-base/studynote/02_operating_system/11_exam_summary/677_trap_based_system_call_implementation/) label을 가리키므로 프로세서는 잠깐 안전한 `pause` 루프나 capture 루프를 돈다. 결국 잘못된 speculative window는 쓸모없는 루프에 갇히고, 공격자가 원하는 gadget 실행으로 이어지지 못한다.

| 구성 요소 | 역할 | 핵심 의미 |
| :--- | :--- | :--- |
| thunk | 위험한 간접 분기를 감싸는 우회 코드 | 모든 간접 분기가 이 통로를 거쳐야 효과가 난다 |
| `call` | RSB와 [스택](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/057_stack/)에 복귀 지점을 남김 | 이후 speculative return을 안전 루프로 유도한다 |
| [trap](/knowledge-base/studynote/02_operating_system/11_exam_summary/677_trap_based_system_call_implementation/) loop | 투기 실행이 빠질 안전한 무한 루프 | 잘못된 speculative gadget 실행을 막는다 |
| [스택](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/057_stack/) 덮어쓰기 | 실제 목표 주소를 architectural return 값으로 [설정](/knowledge-base/studynote/15_devops_sre/01_culture_methodology/009_config/) | 정상 실행은 원래 목적지로 간다 |
...
Retpoline은 [Spectre](/knowledge-base/studynote/01_computer_architecture/14_hardware_security_trends/483_spectre/) variant 2 대응 중에서도 소프트웨어 재작성 계열에 속한다. 반면 [IBPB](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/579_ibpb/) ([Indirect](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/177_indirect_addressing/) Branch Predictor Barrier)는 경계에서 predictor 기억을 비우는 하드웨어 장벽이고, eIBRS는 더 높은 권한 코드가 낮은 권한 훈련에 덜 영향을 받도록 제한하는 하드웨어 메커니즘이다. 셋은 경쟁 관계라기보다, 하드웨어 세대와 운영 목적에 따라 선택 축이 달라지는 대안·보완 관계다.

| 기법 | 개입 위치 | 강점 | 한계 |
| :--- | :--- | :--- | :--- |
| Retpoline | 컴파일 시 간접 분기 재작성 | 구형 CPU에도 배포 가능, 즉시 적용 쉬움 | 모든 간접 분기를 다시 빌드해야 하며 call-heavy 코드 오버헤드가 있다 |
| [IBPB](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/579_ibpb/) | 문맥 전환 시 predictor 초기화 | 경계 전이 차단에 직접적 | 전환 후 predictor warm-up 비용이 있다 |
...
1. [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/), [하이퍼바이저](/knowledge-base/studynote/02_operating_system/01_overview_architecture/054_hypervisor/), 핵심 [라이브러리](/knowledge-base/studynote/04_software_engineering/06_software_architecture/336_library_vs_framework/), 동적 [모듈](/knowledge-base/studynote/04_software_engineering/04_testing_quality/192_module_independence/)이 모두 Retpoline 가능한 toolchain으로 빌드되었는가?
2. hand-written assembly와 [JIT](/knowledge-base/studynote/09_security/11_iam_access_control/568_jit_access/) 코드 [생성](/knowledge-base/studynote/02_operating_system/02_process_thread/087_process_state_transition/)기가 thunk 규칙을 우회하지 않는가?
3. 현재 플랫폼이 구형 CPU라 Retpoline이 주력 완화인지, 아니면 eIBRS가 주력이고 Retpoline은 보조인지 구분했는가?
4. 간접 분기 비중이 높은 워크로드에서 실제 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) 오버헤드를 계측했는가?
5. RSB 관련 보조 조치와 마이크로코드 상태를 함께 점검했는가?

### 피해야 할 [안티패턴](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/128_water_scrum_fall_anti_pattern/)

- 메인 [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/)만 Retpoline으로 빌드하고, 외부 [모듈](/knowledge-base/studynote/04_software_engineering/04_testing_quality/192_module_independence/)과 JIT는 예전 방식 그대로 두는 배포
...
| 개념 | 연결 포인트 |
| :--- | :--- |
| 분기 목표 버퍼 (Branch Target Buffer, [BTB](/knowledge-base/studynote/01_computer_architecture/05_control_unit_pipelining/234_btb/)) | Retpoline이 직접 피하려는 간접 분기 목표 예측 구조다. |
| 반환 [스택](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/057_stack/) 버퍼 (Return [Stack](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/057_stack/) Buffer, RSB) | `ret`의 speculative 경로를 안전 루프로 유도하는 핵심 구조다. |
| thunk | 원래 간접 분기를 감싸 Retpoline 동작을 실현하는 코드 조각이다. |
| [Branch Target Injection](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/577_branch_target_injection/) | Retpoline이 직접 완화하려는 [Spectre](/knowledge-base/studynote/01_computer_architecture/14_hardware_security_trends/483_spectre/) variant 2 공격이다. |
| [IBPB](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/579_ibpb/) ([Indirect](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/177_indirect_addressing/) Branch Predictor Barrier) | 문맥 경계에서 predictor state를 지우는 보완적 하드웨어 대응이다. |
| eIBRS (Enhanced [Indirect](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/177_indirect_addressing/) Branch Restricted Speculation) | 최신 CPU에서 Retpoline 필요성을 줄일 수 있는 하드웨어 완화다. |
| compiler [mitigation](/knowledge-base/studynote/09_security/12_identity_threat_advanced/605_golden_silver_ticket_mitigation/) | Retpoline을 실제 코드 [생성](/knowledge-base/studynote/02_operating_system/02_process_thread/087_process_state_transition/) 규칙으로 바꾸는 실행 주체다. |
