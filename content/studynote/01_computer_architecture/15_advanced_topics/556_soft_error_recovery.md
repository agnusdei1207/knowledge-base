...
기술사 관점에서는 "복구 기술 이름"만 나열하기보다, <strong>서버는 retry/rollback 중심, 자동차는 lockstep+safe state, 우주는 TMR+scrubbing 중심</strong>처럼 환경별 선택 논리를 보여 주는 것이 중요하다. 그래야 복구 메커니즘이 단순 회로 기법이 아니라 시스템 수준 신뢰성 공학이라는 점이 드러난다.
