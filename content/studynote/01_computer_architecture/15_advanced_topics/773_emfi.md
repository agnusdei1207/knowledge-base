...
> 1. **본질**: EMFI (Electromagnetic [Fault Injection](/knowledge-base/studynote/02_operating_system/10_security/670_fault_injection_chaos_testing_kernel/))는 칩에 직접 선을 대지 않고도 짧고 강한 전자기 펄스로 내부 배선과 플립플롭의 타이밍 여유를 깨뜨려 계산 오류를 만드는 비접촉 [결함](/knowledge-base/studynote/04_software_engineering/06_software_architecture/352_defect_definition/) 주입 공격이다.
...
EMFI (Electromagnetic [Fault Injection](/knowledge-base/studynote/02_operating_system/10_security/670_fault_injection_chaos_testing_kernel/))는 고전압 펄스를 마이크로 코일에 흘려 만든 급격한 자기장 변화로 칩 내부에 순간적인 [전압](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/001_voltage/) 교란을 유도하는 물리 공격이다. 전원 핀이나 클럭 핀을 직접 잡지 않아도 되기 때문에, 패키지를 연 뒤 표면 근처에서 특정 블록을 노리는 비접촉 공격으로 널리 연구된다.
...
EMFI 공격 장비는 보통 펄스 [생성](/knowledge-base/studynote/02_operating_system/02_process_thread/087_process_state_transition/)기, [트리거](/knowledge-base/studynote/05_database/04_transactions_concurrency/507_acid_properties/) 회로, 위치 제어 스테이지, 마이크로 코일 프로브로 구성된다. 공격자는 전력 파형이나 외부 I/O 신호를 보고 목표 연산의 시점을 맞춘 뒤, 수 ns 수준의 짧은 펄스를 넣어 특정 영역의 배선·전원 분배망·[레지스터](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/057_register/) 타이밍을 흔든다.

| 구성 요소 | 역할 | 공격자가 보는 핵심 |
| :-------- | :--- | :----------------- |
| 펄스 [생성](/knowledge-base/studynote/02_operating_system/02_process_thread/087_process_state_transition/)기 (Pulse Generator) | 고전압·짧은 폭의 펄스 [생성](/knowledge-base/studynote/02_operating_system/02_process_thread/087_process_state_transition/) | rise time과 pulse width 조절 |
| 마이크로 코일 (Micro-coil Probe) | 국소 자기장 형성 | 공간 해상도와 결합 강도 |
| [트리거](/knowledge-base/studynote/05_database/04_transactions_concurrency/507_acid_properties/) 회로 (Trigger Logic) | 목표 연산 시점 [동기화](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/212_synchronization_mechanisms/) | 보안 검사 직전 타격 |
...
| :--------------- | :--- | :---------- |
| [Instruction](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/158_instruction/) Skip | 조건 검사나 분기 명령이 건너뛰어짐 | [인증](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/303_authentication_authorization_patterns/) 우회, [Secure Boot](/knowledge-base/studynote/02_operating_system/10_security/608_secure_boot/) 우회 |
| [Data](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) Corruption | [레지스터](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/057_register/)·[버스](/knowledge-base/studynote/01_computer_architecture/09_system_bus_interconnects/344_bus/) 값이 잘못 기록됨 | 암호 라운드 오류, 키 추론 |
| Control-flow Error | 잘못된 주소나 예외 경로로 이동 | 권한 상태 붕괴 |
| Reset/Hang | 시스템이 멈추거나 재부팅 | 방어 실패 시 [서비스 거부](/knowledge-base/studynote/02_operating_system/10_security/599_dos_ddos_attack/) |
...
또한 EMFI는 단독 공격보다 다른 기법과 묶여 강해진다. 전자기 분석 (Electromagnetic Analysis, EMA)이나 전력 분석으로 “언제 치는가”를 찾고, EMFI로 “어떤 오류를 내는가”를 만든 뒤, 차분 [결함](/knowledge-base/studynote/04_software_engineering/06_software_architecture/352_defect_definition/) 분석 (Differential Fault Analysis, DFA)로 키를 계산하는 식이다. 즉 수동형 부채널이 정찰을 맡고, EMFI가 실제 타격을 맡는 구조다.

- **📢 섹션 요약 비유**: [전압](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/001_voltage/) 글리칭이 방 전체 전등을 꺼버리는 것이라면, EMFI는 특정 스탠드만 깜빡이게 만드는 것이고, 레이저는 전구 필라멘트 한 점만 바늘처럼 찌르는 것이다.

---

## Ⅳ. 실무 적용 및 기술사 판단

실무에서는 “EMFI를 완전히 막을 수 있는가?”보다 “EMFI가 성공해도 치명적 결과가 나오지 않게 만들 수 있는가?”가 더 중요하다. 금융 보안 칩, 자동차 [Hardware Security Module](/knowledge-base/studynote/09_security/03_network_security/157_hsm_hardware_security_module/) ([HSM](/knowledge-base/studynote/01_computer_architecture/14_hardware_security_trends/475_hsm/)), [펌웨어](/knowledge-base/studynote/02_operating_system/01_overview_architecture/032_firmware/) Root of Trust처럼 단일 오류가 곧 신뢰 붕괴로 이어지는 영역은 하드웨어 센서와 소프트웨어 중복 [검증](/knowledge-base/studynote/04_software_engineering/07_object_oriented/395_verification_process_review/)을 함께 써야 한다.
...
- 단일 비교 결과 한 번으로 [인증](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/303_authentication_authorization_patterns/)을 통과시키는 로직

### [체크리스트](/knowledge-base/studynote/04_software_engineering/11_testing_validation/435_checklist_based_testing/)

- 중요한 제어 흐름에 결과 이중화와 inverse check가 있는가?
- 금속 차폐층이나 [active](/knowledge-base/studynote/03_network/09_application_layer_web_email/483_active_vs_passive_ftp/) mesh가 민감 블록 위를 실제로 덮는가?
- fault detected 시 [key](/knowledge-base/studynote/05_database/02_modeling_normalization/067_db_key_uniqueness_minimality/) [zeroization](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/784_zeroization_circuit/) 경로가 [검증](/knowledge-base/studynote/04_software_engineering/07_object_oriented/395_verification_process_review/)되었는가?
- EMFI 시험에서 [instruction](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/158_instruction/) skip과 reset을 구분해 분석했는가?
...
| 개념 | 연결 포인트 |
| :--- | :---------- |
| [전압](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/001_voltage/) 글리칭 ([Voltage Glitching](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/771_voltage_glitching/)) | 전원선을 흔드는 전역형 [결함](/knowledge-base/studynote/04_software_engineering/06_software_architecture/352_defect_definition/) 주입으로 EMFI와 대비됨 |
| [클럭 글리칭](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/772_clock_glitching/) ([Clock Glitching](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/772_clock_glitching/)) | 시간축을 흔드는 공격으로 EMFI의 fault model과 겹침 |
...
| [액티브](/knowledge-base/studynote/03_network/09_application_layer_web_email/483_active_vs_passive_ftp/) 실드 ([Active](/knowledge-base/studynote/03_network/09_application_layer_web_email/483_active_vs_passive_ftp/) Shield) | 외부 프로빙·전자기 교란을 감지하는 대표 방어 수단 |
| [Secure Boot](/knowledge-base/studynote/02_operating_system/10_security/608_secure_boot/) | [instruction](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/158_instruction/) skip으로 우회되기 쉬운 대표 목표 |
...
← **이전**: [772. 클럭 글리칭 (Clock Glitching)](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/772_clock_glitching/)
**다음**: [774. 부채널 공격 - 캐시 타이밍 공격](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/774_cache_timing_attack/) →
