...
> 1. **본질**: PROCHOT# (Processor Hot) 핀은 CPU가 과열 임계점에 도달했음을 알리거나, 반대로 플랫폼 외부 장치가 CPU에 강제 감속을 요청할 때 쓰는 활성-로우(Active-Low) 열 [보호](/knowledge-base/studynote/02_operating_system/10_security/571_protection_vs_security/) [신호](/knowledge-base/studynote/02_operating_system/02_process_thread/130_signal/)선이다.
> 2. **가치**: 소프트웨어가 [로그](/knowledge-base/studynote/04_software_engineering/09_cloud_native_ai_architecture/568_logs_distributed_logging_elk_fluentd/)를 남기기 전에 하드웨어 수준에서 즉시 스로틀링과 팬 제어를 연동할 수 있어, 밀리초 단위의 열 사고를 플랫폼 차원에서 막을 수 있다.
> 3. **판단 포인트**: CPU 온도는 높지 않은데 클럭이 비정상적으로 낮다면 외부 [VRM](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/742_vrm/), [GPU](/knowledge-base/studynote/01_computer_architecture/12_accelerators_ai_hardware/418_gpu/), 배터리, 전원 [어댑터](/knowledge-base/studynote/04_software_engineering/04_testing_quality/259_adapter_pattern_interface_wrapper/) 등이 BD PROCHOT (Bi-Directional PROCHOT)을 통해 CPU를 강제로 제한하고 있을 가능성을 봐야 한다.

---

## Ⅰ. 개요 및 필요성

PROCHOT# 핀은 CPU 패키지와 메인보드 사이를 잇는 대표적인 열 [보호](/knowledge-base/studynote/02_operating_system/10_security/571_protection_vs_security/) [신호](/knowledge-base/studynote/02_operating_system/02_process_thread/130_signal/)다. 이름의 `#`은 이 [신호](/knowledge-base/studynote/02_operating_system/02_process_thread/130_signal/)가 Active-Low, 즉 전압을 낮춰 "위험"을 표현한다는 뜻이다. CPU 내부의 DTS (Digital Thermal Sensor)가 [TjMax](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/735_tjmax/) (Maximum Junction [Temperature](/knowledge-base/studynote/10_ai/05_data_science_ml/386_llm_temperature/)) 근처를 감지하면, TCC (Thermal Control Circuit)는 내부적으로 주파수를 낮추는 동시에 PROCHOT#을 활성화해 플랫폼 전체에 열 비상 상태를 알린다.

이 [신호](/knowledge-base/studynote/02_operating_system/02_process_thread/130_signal/)가 필요한 이유는 CPU 혼자 느려지는 것만으로는 열 문제가 끝나지 않기 때문이다. 메인보드의 팬 제어기, EC (Embedded Controller), 전원부는 CPU가 위험 상태에 들어갔음을 즉시 알아야 팬을 올리고 전력 공급 [정책](/knowledge-base/studynote/10_ai/02_dl_architecture_new/164_policy/)을 바꾸며 전체 시스템을 식힐 수 있다. 결국 PROCHOT#은 "CPU의 열 문제"를 "플랫폼의 공동 대응"으로 확장하는 하드웨어 비상벨이다.
...
| :-- | :-- | :-- | :-- |
| 목적 | 즉시 감속 요청 | 최후 전원 차단 | [정책](/knowledge-base/studynote/10_ai/02_dl_architecture_new/164_policy/) 기반 온도 관리 |
...
2. [GPU](/knowledge-base/studynote/01_computer_architecture/12_accelerators_ai_hardware/418_gpu/), [VRM](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/742_vrm/), 배터리, [어댑터](/knowledge-base/studynote/04_software_engineering/04_testing_quality/259_adapter_pattern_interface_wrapper/) 온도 또는 [전류](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/002_current/) 제한 이벤트가 동시에 발생하는가?
3. 팬 속도 상승, EC [로그](/knowledge-base/studynote/04_software_engineering/09_cloud_native_ai_architecture/568_logs_distributed_logging_elk_fluentd/), 전원부 센서가 같은 순간을 가리키는가?
4. 외부 BD PROCHOT을 무시하기 전에 플랫폼 열 설계 한계를 확인했는가?

### 피해야 할 [안티패턴](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/128_water_scrum_fall_anti_pattern/)

- CPU 온도만 보고 외부 열원 가능성을 배제하는 진단
- [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) 저하를 없애겠다고 BD PROCHOT을 무조건 차단하는 [설정](/knowledge-base/studynote/15_devops_sre/01_culture_methodology/009_config/)
- 팬 곡선, 방열판 접촉, 전원 [어댑터](/knowledge-base/studynote/04_software_engineering/04_testing_quality/259_adapter_pattern_interface_wrapper/) 상태를 보지 않고 OS 튜닝만 반복하는 운영
...
PROCHOT#의 기대효과는 속도보다 생존을 우선하는 하드웨어 협조 체계를 만든다는 데 있다. CPU, 메인보드, 전원부, 팬 제어기가 동일한 [신호](/knowledge-base/studynote/02_operating_system/02_process_thread/130_signal/)를 공유하면 소프트웨어 개입을 기다리지 않고도 즉시 감속과 냉각을 연동할 수 있다. 이는 열 밀도가 높은 노트북, 블레이드 서버, 고성능 워크스테이션처럼 물리 여유가 작은 시스템일수록 더 중요하다.
