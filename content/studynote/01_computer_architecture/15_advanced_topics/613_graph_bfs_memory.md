...
| 구성 요소 | 역할 | 설계 포인트 |
| :--- | :--- | :--- |
| 프런티어 큐 (Frontier [Queue](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/058_queue/)) | 현재 레벨 또는 탐색 경로의 정점 집합 관리 | BFS는 폭이 넓고, DFS는 [스택](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/057_stack/) [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/)이 짧아야 한다 |
| 오프셋 캐시 (Offset Cache) | 정점별 인접 리스트 시작/끝 위치 보관 | [CSR](/knowledge-base/studynote/09_security/04_endpoint_security/169_pkcs10_csr/)/CSC 메타데이터를 작은 캐시에 고정 |
| 주소 결합기 (Address Coalescer) | 흩어진 간선 주소를 뱅크 친화적으로 재정렬 | [허브](/knowledge-base/studynote/03_network/03_physical_layer_media/152_hub_dummy_switching_intelligent/) 정점의 폭주를 완화 |
...
| [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 형식 친화성 | [배열](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/055_array/), 행렬, 연속 버퍼 | [CSR](/knowledge-base/studynote/09_security/04_endpoint_security/169_pkcs10_csr/)/CSC, [그래프](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/070_graph_datastructure/) [파티션](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/), 에지 블록 |
| 대표 목표 | 평균 [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/)시간 축소 | 탐색당 처리 간선 수 극대화 |

이 구조는 근접 메모리 컴퓨팅 (Near-Memory Computing)과 처리 내장 메모리 ([Processing-In-Memory](/knowledge-base/studynote/01_computer_architecture/12_accelerators_ai_hardware/430_pim/), [PIM](/knowledge-base/studynote/01_computer_architecture/12_accelerators_ai_hardware/430_pim/))로 자연스럽게 이어진다. [그래프](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/070_graph_datastructure/) 탐색은 계산보다 이동이 비싸므로, 메모리 근처에서 프런티어 필터링과 비트맵 갱신까지 수행하면 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 이동 자체를 줄일 수 있다. 또한 [그래프](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/070_graph_datastructure/) 저장 형식도 중요해서, 포인터 기반 객체 [그래프](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/070_graph_datastructure/)보다 [CSR](/knowledge-base/studynote/09_security/04_endpoint_security/169_pkcs10_csr/)·정점 재정렬·에지 블로킹이 하드웨어 친화적이다.

- **📢 섹션 요약 비유**: 범용 캐시가 자주 꺼내는 책을 책상 가까이에 쌓아 두는 방식이라면, [그래프](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/070_graph_datastructure/) 전용 서브시스템은 도서관 사서가 다음에 찾을 책 경로를 미리 읽고 서가 사이를 뛰어다니며 책을 한꺼번에 가져다주는 방식이다.
...
1. [그래프](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/070_graph_datastructure/)가 [압축](/knowledge-base/studynote/02_operating_system/06_memory_management/347_compaction/) 희소 행 형식 (Compressed Sparse Row, [CSR](/knowledge-base/studynote/09_security/04_endpoint_security/169_pkcs10_csr/)) 또는 유사한 선형 배치로 정리되어 있는가?
2. 방문 비트맵이 온칩 SRAM에 일부라도 올라가서 중복 제거를 빠르게 할 수 있는가?
3. [허브](/knowledge-base/studynote/03_network/03_physical_layer_media/152_hub_dummy_switching_intelligent/) 정점이 특정 메모리 뱅크로 몰리지 않도록 정점 재정렬이나 파티셔닝이 되어 있는가?
4. [BFS](/knowledge-base/studynote/08_algorithm_stats/03_graph_search/035_bfs/) 중심 업무인지, [DFS](/knowledge-base/studynote/08_algorithm_stats/03_graph_search/034_dfs/) 중심 업무인지에 따라 큐 중심 구조와 [스택](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/057_stack/) 중심 구조를 구분했는가?
5. [그래프](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/070_graph_datastructure/)가 초당 자주 바뀌는 환경이라면 정적 최적화 이득보다 동적 업데이트 비용이 더 크지 않은가?
...
| 개념 | 연결 포인트 |
| :--- | :--- |
| 프런티어 (Frontier) | [BFS](/knowledge-base/studynote/08_algorithm_stats/03_graph_search/035_bfs/)/DFS가 다음에 확장할 정점 집합이며 메모리 요청의 출발점이다. |
| [압축](/knowledge-base/studynote/02_operating_system/06_memory_management/347_compaction/) 희소 행 형식 (Compressed Sparse Row, [CSR](/knowledge-base/studynote/09_security/04_endpoint_security/169_pkcs10_csr/)) | [그래프](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/070_graph_datastructure/)를 선형 메모리에 배치해 오프셋 접근을 단순화하는 대표 저장 방식이다. |
| 방문 비트맵 (Visited Bitmap) | 중복 탐색을 줄여 메모리 낭비를 막는 핵심 제어 구조다. |
| 메모리 수준 [병렬](/knowledge-base/studynote/05_database/07_exam_summary/430_index_fast_full_scan/)성 (Memory-Level Parallelism, MLP) | [그래프](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/070_graph_datastructure/) 탐색 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/)을 좌우하는 핵심 지표로, 동시에 몇 개의 메모리 요청을 유지하는지와 관련된다. |
| 처리 내장 메모리 ([Processing-In-Memory](/knowledge-base/studynote/01_computer_architecture/12_accelerators_ai_hardware/430_pim/), [PIM](/knowledge-base/studynote/01_computer_architecture/12_accelerators_ai_hardware/430_pim/)) | [그래프](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/070_graph_datastructure/) 탐색 전용 메모리 서브시스템의 다음 단계로, 필터링과 갱신을 메모리 근처에서 수행한다. |
...
1. [그래프](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/070_graph_datastructure/) 탐색은 복잡한 미로에서 다음 방 번호를 계속 찾아가며 이동하는 놀이와 같아요.
