...
여기서 중요한 정확성은, S0ix가 ACPI의 표준 글로벌 상태 이름이 아니라는 점이다. ACPI가 시스템 전역 상태를 S0~S5로 설명한다면, S0ix는 주로 Intel 계열 플랫폼 문맥에서 쓰이는 <strong>S0 내부 저전력 <a href="/knowledge-base/studynote/02_operating_system/10_security/611_cpu_idle_wait_optimization/">idle</a> family</strong>를 가리킨다. 그래서 S0ix를 이해하려면 "왜 S0인데도 거의 자고 있는가"라는 관점이 필요하다.
...
S0ix 진입은 한 부품의 의사결정으로 끝나지 않는다. CPU 코어가 쉬고 있어도 [USB](/knowledge-base/studynote/01_computer_architecture/09_system_bus_interconnects/359_usb/), 오디오, 디스플레이, 스토리지, 네트워크, 타이머 중 하나라도 계속 깨어 있으면 플랫폼은 깊은 S0ix에 못 들어간다. 그래서 S0ix는 <strong>모든 전력 <a href="/knowledge-base/studynote/05_database/02_modeling_normalization/064_relation_domain/">도메인</a>이 충분히 조용해졌는지</strong>를 확인한 뒤에야 성립한다.
...
[운영체제](/knowledge-base/studynote/02_operating_system/01_overview_architecture/001_operating_system_purpose/)와의 연결도 중요하다. Windows에서는 Modern Standby가 S0 low [power](/knowledge-base/studynote/14_data_engineering/02_math_mining/069_type_1_2_error_statistical_power/) idle을 활용하는 대표 시나리오이고, Linux에서는 suspend-to-[idle](/knowledge-base/studynote/02_operating_system/10_security/611_cpu_idle_wait_optimization/) (s2idle)이 지원 하드웨어 위에서 S0ix residency를 활용하는 형태로 이어질 수 있다. 다시 말해 S0ix는 OS [정책](/knowledge-base/studynote/10_ai/02_dl_architecture_new/164_policy/) 이름이 아니라, <strong>OS가 만들고 싶어 하는 하드웨어 결과물</strong>에 가깝다.
...
5. <strong>플랫폼 펌웨어와 OS <a href="/knowledge-base/studynote/10_ai/02_dl_architecture_new/164_policy/">정책</a>이 일치하는가?</strong> 펌웨어가 deep idle을 과도하게 demotion하면 절전 효과가 줄 수 있다.
...
기술사 답안에서는 S0ix를 "S3의 대체"라고만 쓰면 불완전하다. 더 정확한 표현은 <strong>S0을 유지하면서 S3급 대기 전력을 노리는 플랫폼 저전력 유휴 메커니즘</strong>이다. 그리고 이 메커니즘의 성패가 드라이버, PMC, 장치 wake [정책](/knowledge-base/studynote/10_ai/02_dl_architecture_new/164_policy/), memory [retention](/knowledge-base/studynote/05_database/04_transactions_concurrency/515_mvcc/) 설계에 달렸다는 점까지 써야 설계 관점 설명이 된다.
