...
기술사 답안에서는 S0ix를 "S3의 대체"라고만 쓰면 불완전하다. 더 정확한 표현은 <strong>S0을 유지하면서 S3급 대기 전력을 노리는 플랫폼 저전력 유휴 메커니즘</strong>이다. 그리고 이 메커니즘의 성패가 드라이버, PMC, 장치 wake 정책, memory retention 설계에 달렸다는 점까지 써야 설계 관점 설명이 된다.
