...
> 1. **본질**: 분기 목표 주입 (Branch Target [Injection](/knowledge-base/studynote/04_software_engineering/11_testing_validation/480_injection/))은 공격자가 간접 [분기 예측](/knowledge-base/studynote/01_computer_architecture/05_control_unit_pipelining/231_branch_prediction/)기 ([Indirect](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/177_indirect_addressing/) Branch Predictor)와 분기 목표 버퍼 (Branch Target Buffer, [BTB](/knowledge-base/studynote/01_computer_architecture/05_control_unit_pipelining/234_btb/))를 오염시켜, 피해자의 간접 호출·간접 점프가 악성 [가젯](/knowledge-base/studynote/09_security/04_endpoint_security/345_gadget_rop/) 쪽으로 투기 실행 (Speculative Execution)되게 만드는 [Spectre](/knowledge-base/studynote/01_computer_architecture/14_hardware_security_trends/483_spectre/) variant 2 계열 공격이다.
> 2. **가치**: 이 공격은 아키텍처상 권한 검사가 정상이어도, 프로세스·[커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/)·가상 머신 사이에 공유된 예측 상태와 캐시 흔적만으로 정보 유출이 가능하다는 사실을 드러냈다.
> 3. **판단 포인트**: 방어는 단일 패치가 아니라 [Retpoline](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/580_retpoline/) ([Return Trampoline](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/580_retpoline/)), [IBPB](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/579_ibpb/) ([Indirect](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/177_indirect_addressing/) Branch Predictor Barrier), IBRS ([Indirect](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/177_indirect_addressing/) Branch Restricted Speculation), STIBP (Single [Thread](/knowledge-base/studynote/02_operating_system/02_process_thread/092_thread_lwp/) [Indirect](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/177_indirect_addressing/) Branch Predictors)처럼 "코드 재작성 + 경계 격리 + 하드웨어 제어"를 함께 써야 효과가 난다.
...
이 공격이 성립하려면 세 가지가 이어져야 한다. 첫째, 공격자가 피해자와 일부 예측 상태를 공유하거나 충돌시킬 수 있어야 한다. 둘째, 피해자 코드 안에 투기적으로 실행되었을 때 비밀 값을 캐시 흔적으로 바꿔 주는 [가젯](/knowledge-base/studynote/09_security/04_endpoint_security/345_gadget_rop/) (Gadget)이 있어야 한다. 셋째, 공격자가 캐시 타이밍 같은 사이드 채널로 그 흔적을 읽을 수 있어야 한다. 셋 중 하나만 빠져도 공격 난도는 크게 높아진다.

간접 [분기 예측](/knowledge-base/studynote/01_computer_architecture/05_control_unit_pipelining/231_branch_prediction/)기는 보통 분기 명령의 위치와 과거 실행 이력을 바탕으로 다음 목표 주소를 추정한다. 이때 예측 구조는 전체 주소의 모든 비트를 완벽히 구분하지 않거나, 코어 내부에서 여러 보안 [도메인](/knowledge-base/studynote/05_database/02_modeling_normalization/064_relation_domain/)이 순차적으로 같은 구조를 재사용한다. 공격자는 이 특성을 이용해 "내가 만든 예측 항목"이 나중에 피해자의 분기와 충돌하도록 훈련한다. 이후 피해자가 같은 인덱스를 가진 간접 분기를 만나면, 프로세서는 짧은 순간 공격자가 원하는 목표 주소를 정답처럼 믿고 [가젯](/knowledge-base/studynote/09_security/04_endpoint_security/345_gadget_rop/)을 실행한다.

| 단계 | 내부 구조 | 실제로 일어나는 일 | 공격 포인트 |
| :--- | :--- | :--- | :--- |
| 훈련 | 간접 [분기 예측](/knowledge-base/studynote/01_computer_architecture/05_control_unit_pipelining/231_branch_prediction/)기, [BTB](/knowledge-base/studynote/01_computer_architecture/05_control_unit_pipelining/234_btb/) | 공격자가 특정 분기 패턴과 목표 주소를 반복 입력한다 | 예측 상태 오염 |
| 유도 | 피해자 간접 분기 | 피해자가 아직 진짜 목적지를 계산하기 전 예측 결과를 사용한다 | 잘못된 목표 채택 |
| [가젯](/knowledge-base/studynote/09_security/04_endpoint_security/345_gadget_rop/) 실행 | 캐시, 로드 파이프라인 | 비밀 값에 따라 다른 메모리 줄을 건드린다 | 캐시 흔적 [생성](/knowledge-base/studynote/02_operating_system/02_process_thread/087_process_state_transition/) |
| [복구](/knowledge-base/studynote/09_security/13_secops_ir_forensics/658_ir_recovery/) | [재주문 버퍼](/knowledge-base/studynote/01_computer_architecture/05_control_unit_pipelining/240_reorder_buffer/) ([Reorder Buffer](/knowledge-base/studynote/01_computer_architecture/05_control_unit_pipelining/240_reorder_buffer/)) | 오예측이 밝혀져 아키텍처 상태를 롤백한다 | 흔적은 그대로 남음 |
...
분기 목표 주입은 다른 투기 실행 공격과 비슷해 보이지만, 조작하는 지점이 다르다. [Spectre](/knowledge-base/studynote/01_computer_architecture/14_hardware_security_trends/483_spectre/) variant 1은 조건 분기의 "갈지 말지"를 속여 경계 검사 이후 코드를 잠깐 실행하게 만든다. 반면 분기 목표 주입은 간접 분기의 "어디로 갈지"를 속인다. Meltdown은 아예 권한 검사보다 먼저 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 읽어 오는 하드웨어 순서 문제라서, 원인 층이 또 다르다.

| 항목 | [Spectre](/knowledge-base/studynote/01_computer_architecture/14_hardware_security_trends/483_spectre/) variant 1 | 분기 목표 주입 ([Spectre](/knowledge-base/studynote/01_computer_architecture/14_hardware_security_trends/483_spectre/) variant 2) | [Meltdown](/knowledge-base/studynote/01_computer_architecture/14_hardware_security_trends/482_meltdown/) |
| :--- | :--- | :--- | :--- |
| 속이는 대상 | 조건 분기 방향 예측 | 간접 분기 목표 예측 | 권한 검사 이전 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 접근 |
| 핵심 구조 | 분기 방향 예측기 | 간접 [분기 예측](/knowledge-base/studynote/01_computer_architecture/05_control_unit_pipelining/231_branch_prediction/)기, [BTB](/knowledge-base/studynote/01_computer_architecture/05_control_unit_pipelining/234_btb/) | 메모리 로드와 권한 검사 순서 |
...
- **📢 섹션 요약 비유**: [Spectre](/knowledge-base/studynote/01_computer_architecture/14_hardware_security_trends/483_spectre/) variant 1이 "갈림길에서 좌회전 표지를 속이는 일"이라면, 분기 목표 주입은 "목적지 주소 자체를 바꿔치기하는 일"이고, Meltdown은 "잠긴 문도 손잡이를 돌리기 전에 안을 비쳐 보는 일"에 가깝다. 겉보기엔 모두 침입이지만 뚫는 문이 서로 다르다.

---

## Ⅳ. 실무 적용 및 기술사 판단

실무에서 분기 목표 주입은 특히 [멀티테넌트](/knowledge-base/studynote/05_database/05_distributed_nosql_newsql/310_multi_tenant_database_architecture/) 클라우드와 브라우저 샌드박스에서 민감하다. 같은 코어를 시간 분할해 쓰는 서로 다른 프로세스나 가상 머신이 예측 구조를 공유하면, 공격자가 남긴 훈련 흔적이 다음 보안 [도메인](/knowledge-base/studynote/05_database/02_modeling_normalization/064_relation_domain/)으로 건너갈 수 있기 때문이다. 그래서 [운영체제](/knowledge-base/studynote/02_operating_system/01_overview_architecture/001_operating_system_purpose/)와 [하이퍼바이저](/knowledge-base/studynote/02_operating_system/01_overview_architecture/054_hypervisor/)는 경계 전환 시 예측기 상태를 비우거나, 높은 권한 코드에서 하위 권한 훈련의 영향을 제한하는 정책을 함께 사용한다.

또 하나의 판단 포인트는 [동시 멀티스레딩](/knowledge-base/studynote/01_computer_architecture/11_multicore_synchronization/400_smt/) (Simultaneous [Multithreading](/knowledge-base/studynote/02_operating_system/02_process_thread/095_multithreading_benefits/), [SMT](/knowledge-base/studynote/01_computer_architecture/11_multicore_synchronization/400_smt/))이다. 같은 물리 코어의 형제 [스레드](/knowledge-base/studynote/02_operating_system/02_process_thread/092_thread_lwp/)가 예측기를 더 긴밀하게 공유하는 구조에서는, 단순한 문맥 전환 장벽만으로는 충분하지 않을 수 있다. 이 경우 STIBP를 쓰거나, 더 강한 격리가 필요하면 [SMT](/knowledge-base/studynote/01_computer_architecture/11_multicore_synchronization/400_smt/) 자체를 끄는 판단도 현실적인 선택지가 된다. 즉 대응은 "패치 하나 켰다"로 끝나지 않고, 워크로드와 격리 수준에 따라 계층적으로 골라야 한다.

### 적용 판단 [체크리스트](/knowledge-base/studynote/04_software_engineering/11_testing_validation/435_checklist_based_testing/)

1. [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/), [하이퍼바이저](/knowledge-base/studynote/02_operating_system/01_overview_architecture/054_hypervisor/), 사용자 공간이 최신 마이크로코드와 [운영체제](/knowledge-base/studynote/02_operating_system/01_overview_architecture/001_operating_system_purpose/) 완화 옵션을 함께 사용하고 있는가?
2. 신뢰되지 않은 보안 [도메인](/knowledge-base/studynote/05_database/02_modeling_normalization/064_relation_domain/) 전환 시 [IBPB](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/579_ibpb/) 또는 동급 장벽이 실제로 실행되는가?
3. [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/)·런타임·핵심 라이브러리가 [Retpoline](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/580_retpoline/) 또는 최신 하드웨어 완화 방식으로 빌드되어 있는가?
4. [SMT](/knowledge-base/studynote/01_computer_architecture/11_multicore_synchronization/400_smt/) 환경이라면 STIBP, 코어 스케줄링, [SMT](/knowledge-base/studynote/01_computer_architecture/11_multicore_synchronization/400_smt/) 비활성화 중 어떤 방식을 선택할지 위협 모델에 맞게 정했는가?
5. 브라우저·샌드박스 환경에서 고해상도 타이머와 [공유 메모리](/knowledge-base/studynote/02_operating_system/02_process_thread/118_shared_memory/) 같은 유출 보조 수단도 함께 통제하고 있는가?

### 피해야 할 [안티패턴](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/128_water_scrum_fall_anti_pattern/)

- 마이크로코드 없이 [운영체제](/knowledge-base/studynote/02_operating_system/01_overview_architecture/001_operating_system_purpose/) 옵션만 켜고 하드웨어 완화가 모두 동작한다고 믿는 운영
...
| 개념 | 연결 포인트 |
| :--- | :--- |
| 간접 [분기 예측](/knowledge-base/studynote/01_computer_architecture/05_control_unit_pipelining/231_branch_prediction/)기 ([Indirect](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/177_indirect_addressing/) Branch Predictor) | 간접 호출과 점프의 미래 목표를 추정하는 구조이며, 공격자가 오염시키는 핵심 표적이다. |
| 분기 목표 버퍼 (Branch Target Buffer, [BTB](/knowledge-base/studynote/01_computer_architecture/05_control_unit_pipelining/234_btb/)) | 과거 목표 주소를 저장해 다음 예측에 재사용하는 대표 구조로, 분기 목표 주입의 직접 공격면이 된다. |
| [가젯](/knowledge-base/studynote/09_security/04_endpoint_security/345_gadget_rop/) (Gadget) | 투기적으로 실행되었을 때 비밀 값을 캐시 흔적으로 바꾸는 짧은 코드 조각이다. |
| [IBPB](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/579_ibpb/) ([Indirect](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/177_indirect_addressing/) Branch Predictor Barrier) | 보안 [도메인](/knowledge-base/studynote/05_database/02_modeling_normalization/064_relation_domain/) 전환 시 예측 상태를 비워 오염 전파를 줄이는 하드웨어 장벽이다. |
| IBRS ([Indirect](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/177_indirect_addressing/) Branch Restricted Speculation) | 낮은 권한의 분기 훈련이 높은 권한 코드의 간접 [분기 예측](/knowledge-base/studynote/01_computer_architecture/05_control_unit_pipelining/231_branch_prediction/)에 덜 영향을 주도록 제한한다. |
| [Retpoline](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/580_retpoline/) ([Return Trampoline](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/580_retpoline/)) | 간접 분기를 return 기반 thunk로 치환해 예측기 오염의 직접 노출면을 줄이는 소프트웨어 대응이다. |
| STIBP (Single [Thread](/knowledge-base/studynote/02_operating_system/02_process_thread/092_thread_lwp/) [Indirect](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/177_indirect_addressing/) Branch Predictors) | [SMT](/knowledge-base/studynote/01_computer_architecture/11_multicore_synchronization/400_smt/) 형제 [스레드](/knowledge-base/studynote/02_operating_system/02_process_thread/092_thread_lwp/) 사이의 간접 [분기 예측](/knowledge-base/studynote/01_computer_architecture/05_control_unit_pipelining/231_branch_prediction/) 간섭을 줄이기 위한 하드웨어 제어다. |
