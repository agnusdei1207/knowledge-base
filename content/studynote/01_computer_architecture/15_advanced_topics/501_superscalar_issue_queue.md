...
발급 큐의 각 엔트리는 보통 연산 종류, 목적 물리 [레지스터](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/057_register/) 번호, 소스 물리 [레지스터](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/057_register/) 태그, 각 소스의 준비 [비트](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/073_bit/), 나이 정보, 분기 마스크 같은 제어 정보를 담는다. 결과가 아직 없는 소스는 "태그"만 들고 기다리다가, 다른 실행 유닛에서 결과가 나오면 우회 경로 (Bypass Network) 또는 공통 결과 [버스](/knowledge-base/studynote/01_computer_architecture/09_system_bus_interconnects/344_bus/) (Common [Data Bus](/knowledge-base/studynote/01_computer_architecture/09_system_bus_interconnects/345_data_bus/), CDB)를 통해 태그가 방송된다. 그러면 해당 태그를 기다리던 엔트리가 동시에 깨어나는 것이 깨우기 (Wakeup) 단계다.
...
| 구성 요소 | 역할 | 설계 시 병목 |
| :-- | :-- | :-- |
| 엔트리 [비교기](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/043_comparator/) | 결과 태그와 소스 태그를 비교해 ready 상태 갱신 | 콘텐츠 주소 지정 메모리 (Content Addressable Memory, CAM) 전력 증가 |
| 선택 중재기 | 준비된 명령 중 발급할 엔트리 결정 | 발급 폭이 넓을수록 [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/) 증가 |
| 결과 방송망 | 완료 결과를 여러 엔트리에 동시에 알림 | [포트](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/446_port_and_bus/) 수와 배선 팬아웃 부담 |
| [분산](/knowledge-base/studynote/08_algorithm_stats/08_stats/136_variance/) [스케줄러](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/079_kube_scheduler_pod_placement/) | 큐를 정수/[부동소수점](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/087_floating_point/)/메모리용으로 분리 | 특정 큐만 과포화되는 불균형 가능 |

따라서 현대 고성능 코어는 거대한 단일 큐 대신 클러스터형 발급 큐를 자주 사용한다. 정수, 메모리, [부동소수점](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/087_floating_point/) 계열로 나눠 각 큐의 비교 범위를 줄이면 주파수와 전력을 방어할 수 있기 때문이다. 대신 명령을 어느 큐에 배치할지, [포트](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/446_port_and_bus/) 혼잡이 생기지 않게 어떻게 해시할지라는 새로운 스케줄링 문제가 생긴다.
...
발급 큐를 이해하려면 점수판 (Scoreboard), 예약 스테이션 ([Reservation Station](/knowledge-base/studynote/01_computer_architecture/05_control_unit_pipelining/241_reservation_station/)), 리오더 버퍼 ([Reorder Buffer](/knowledge-base/studynote/01_computer_architecture/05_control_unit_pipelining/240_reorder_buffer/), ROB)와의 경계를 분명히 봐야 한다. 점수판은 누가 누구를 기다리는지 중앙에서 표시하는 [초기](/knowledge-base/studynote/03_network/08_transport_layer/459_quic_fec_forward_error_correction/) 방식이고, 예약 스테이션은 기능 유닛 근처에 [명령어](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/158_instruction/)를 붙여 두는 토마술로 (Tomasulo) 계열 방식이다. 현대 [수퍼스칼라](/knowledge-base/studynote/01_computer_architecture/05_control_unit_pipelining/236_superscalar/) 코어의 발급 큐는 이 두 아이디어를 발전시켜, 리네이밍된 [명령어](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/158_instruction/)를 넓게 받아 두고 여러 실행 [포트](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/446_port_and_bus/)에 맞춰 동적으로 골라 내는 구조로 진화했다.

| 비교 대상 | 핵심 역할 | 강점 | 한계 |
| :-- | :-- | :-- | :-- |
| 점수판 (Scoreboard) | 의존성 상태 추적 | 구조 단순 | 가짜 의존성 제거와 대규모 확장에 불리 |
| 예약 스테이션 ([Reservation Station](/knowledge-base/studynote/01_computer_architecture/05_control_unit_pipelining/241_reservation_station/)) | 기능 유닛 앞 대기 | 지역성 좋음 | 전체 시야가 좁아질 수 있음 |
...
실무적으로 발급 큐는 "클수록 좋다"보다 "어떤 [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/)을 숨길 만큼 충분한가"로 판단해야 한다. 서버용 코어는 메모리 [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/)이 길고 [단일 스레드 성능](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/570_stp_vs_mtp/) 요구가 높아 큰 큐와 복수의 [포트](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/446_port_and_bus/)를 감수하는 편이 유리하다. 반면 모바일 코어는 동일한 큐 크기 확장이 배터리와 주파수에 주는 부담이 커서, 작은 큐와 더 보수적인 발급 폭을 택하거나 일부 기능 블록만 적극적으로 [비순차 실행](/knowledge-base/studynote/01_computer_architecture/05_control_unit_pipelining/238_out_of_order_execution/)하도록 설계한다.

소프트웨어 측면에서도 발급 큐 활용도는 크게 달라진다. [루프 언롤링](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/538_loop_unrolling/), 독립 [누산기](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/161_accumulator/) 사용, 메모리 접근과 산술 연산의 적절한 섞기는 큐 안에 서로 독립인 [명령어](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/158_instruction/)를 더 많이 채워 준다. 반대로 긴 의존성 체인, 동일 [포트](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/446_port_and_bus/)만 몰아 쓰는 명령 조합, 예측 실패가 잦은 분기는 큐를 가득 채우고도 실제 발급률을 떨어뜨린다.

### 적용 판단 [체크리스트](/knowledge-base/studynote/04_software_engineering/11_testing_validation/435_checklist_based_testing/)

1. 긴 메모리 [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/)이나 곱셈·나눗셈처럼 긴 연산을 숨길 독립 [명령어](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/158_instruction/)가 충분한가?
2. 정수·[부동소수점](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/087_floating_point/)·메모리 [포트](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/446_port_and_bus/) 중 특정 [포트](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/446_port_and_bus/)만 과열되는 구조는 아닌가?
3. 큐 확대가 목표 주파수와 전력 예산 안에서 감당 가능한가?
4. [분기 예측](/knowledge-base/studynote/01_computer_architecture/05_control_unit_pipelining/231_branch_prediction/) 실패로 잘못 채운 [명령어](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/158_instruction/)가 큐를 오염시키는 비율은 낮은가?

### 피해야 할 [안티패턴](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/128_water_scrum_fall_anti_pattern/)

- 모든 최적화를 큐 크기 확대로 해결하려는 설계
- 연속적인 의존성 체인을 그대로 둔 채 하드웨어가 알아서 병렬화해 주길 기대하는 코드
- [포트](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/446_port_and_bus/) 불균형을 무시하고 특정 기능 유닛에만 명령을 몰아넣는 [마이크로아키텍처](/knowledge-base/studynote/01_computer_architecture/05_control_unit_pipelining/204_microarchitecture/)

- **📢 섹션 요약 비유**: 발급 큐 운영은 [버스](/knowledge-base/studynote/01_computer_architecture/09_system_bus_interconnects/344_bus/) 환승센터와 같다. 대기 공간만 넓힌다고 해결되지 않고, 어느 노선이 막히는지와 승객 흐름이 어떻게 [분산](/knowledge-base/studynote/08_algorithm_stats/08_stats/136_variance/)되는지까지 같이 봐야 한다.
...
| 개념 | 연결 포인트 |
| :-- | :-- |
| [레지스터 리네이밍](/knowledge-base/studynote/01_computer_architecture/05_control_unit_pipelining/239_register_renaming/) ([Register Renaming](/knowledge-base/studynote/01_computer_architecture/05_control_unit_pipelining/239_register_renaming/)) | 가짜 의존성을 제거해 발급 큐가 실제 의존성만 보게 만든다. |
| 리오더 버퍼 ([Reorder Buffer](/knowledge-base/studynote/01_computer_architecture/05_control_unit_pipelining/240_reorder_buffer/), ROB) | 비순차로 실행된 결과를 원래 순서대로 완료시킨다. |
| [로드-스토어 큐](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/508_load_store_queue/) (Load-Store [Queue](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/058_queue/), LSQ) | 메모리 명령의 순서 제약을 관리해 발급 큐가 무작정 앞지르지 않게 한다. |
| 깨우기-선택 (Wakeup-[Select](/knowledge-base/studynote/05_database/04_transactions_concurrency/520_select/)) | 발급 큐의 핵심 회로로, ready 상태 갱신과 [포트](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/446_port_and_bus/) 배정을 담당한다. |
| [명령어](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/158_instruction/) 수준 병렬성 (Instruction-Level Parallelism, ILP) | 발급 큐가 끌어내려는 실질적인 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) 자원이다. |
