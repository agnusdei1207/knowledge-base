...
메모리 맵 I/O (Memory-Mapped I/O)는 주변 장치의 제어 [레지스터](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/057_register/)와 [상태 레지스터](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/167_status_register/)를 메모리 주소 공간 일부에 매핑하여 접근하는 입출력 방식이다. 핵심 발상은 “장치 제어도 결국 특정 주소에 값을 읽고 쓰는 일”이라는 점에 있다. 따라서 CPU는 별도의 I/O 전용 [명령어](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/158_instruction/)를 배우지 않고도, 주소만 보고 메모리인지 장치인지 구분하지 않은 채 동일한 로드/스토어 흐름을 사용한다.

이 방식이 필요해진 이유는 입출력 장치가 늘어날수록 CPU 내부 명령 해석기와 [버스](/knowledge-base/studynote/01_computer_architecture/09_system_bus_interconnects/344_bus/) 제어 로직이 복잡해졌기 때문이다. 분리형 I/O (Isolated I/O, 또는 Port-Mapped I/O, PMIO)는 전용 [포트](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/446_port_and_bus/) 공간과 `IN/OUT` 같은 특수 [명령어](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/158_instruction/)를 요구하므로, [명령어](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/158_instruction/) 집합과 컴파일러, 드라이버가 모두 장치 접근을 별도 세계처럼 취급해야 한다. 반면 MMIO는 주소 공간만 잘 나누면 메모리와 장치 제어를 하나의 추상화로 묶을 수 있어, 설계 단순성과 확장성을 동시에 얻는다.

특히 현대 시스템은 네트워크 카드, 그래픽 처리 장치, 스토리지 컨트롤러처럼 [레지스터](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/057_register/) 수가 많고 제어 상태가 복잡한 장치를 다룬다. 이런 환경에서 MMIO는 “주소 공간을 통합해 소프트웨어와 하드웨어의 접점을 일관되게 만든다”는 점에서 사실상 표준이 되었다.
...
| 구성 요소 | 역할 | 설계 포인트 |
| :-- | :-- | :-- |
| 주소 [디코더](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/039_decoder/) (Address [Decoder](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/039_decoder/)) | 주소 범위를 해석해 메모리/장치 선택 | 충돌 없는 주소 배치 |
| 제어 [레지스터](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/057_register/) | 장치 모드, 시작, 정지 제어 | [비트](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/073_bit/) 의미 명확화 |
| [상태 레지스터](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/167_status_register/) | 완료, Busy, Error 상태 제공 | [폴링](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/448_polling_programmed_io/)/[인터럽트](/knowledge-base/studynote/02_operating_system/01_overview_architecture/016_interrupt_mechanism/) 연계 |
| [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) [레지스터](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/057_register/) | 송수신 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 또는 포인터 전달 | 폭과 정렬 일치 |
| [버스](/knowledge-base/studynote/01_computer_architecture/09_system_bus_interconnects/344_bus/) 인터페이스 | CPU 요청을 장치 타이밍에 맞춤 | 대기 상태(Wait [State](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/272_state_pattern/)) 처리 |

실제 접근 시에는 메모리보다 느린 장치 타이밍도 고려해야 한다. CPU가 너무 빠르면 [버스](/knowledge-base/studynote/01_computer_architecture/09_system_bus_interconnects/344_bus/)가 대기 상태를 삽입하거나, 장치가 완료 [비트](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/073_bit/)를 올릴 때까지 소프트웨어가 [폴링](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/448_polling_programmed_io/)해야 한다. 그래서 MMIO는 “메모리처럼 보이지만, 메모리와 같은 속성은 아닌 공간”으로 이해해야 정확하다.
...
MMIO를 정확히 이해하려면 분리형 I/O (Isolated I/O, PMIO)와 비교해야 한다. PMIO는 메모리 주소 공간과 별도의 [포트](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/446_port_and_bus/) 주소 공간을 두고, CPU가 전용 I/O [명령어](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/158_instruction/)로만 장치에 접근한다. 이 방식은 메모리 주소를 아낄 수 있지만, [명령어](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/158_instruction/) 집합·[디코더](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/039_decoder/)·컴파일러 지원이 따로 필요하다. 반면 MMIO는 메모리 주소 일부를 장치에 내주지만, 프로그래밍 모델과 CPU 명령 체계가 단순해진다.

| 항목 | 메모리 맵 I/O (MMIO) | 분리형 I/O (PMIO) |
| :-- | :-- | :-- |
| 주소 공간 | 메모리와 공유 | 별도 [포트](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/446_port_and_bus/) 공간 사용 |
| [명령어](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/158_instruction/) | 일반 Load/Store | 전용 I/O 명령 필요 |
| CPU 설계 | 단순 | 디코드 로직 추가 |
...
3. [상태 레지스터](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/167_status_register/)가 읽기 시 부작용(clear-on-read 등)을 갖는지 확인했는가?
4. [레지스터](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/057_register/) 기록 순서가 중요하다면 배리어를 삽입했는가?
5. 대량 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 전송을 MMIO 반복 접근으로 처리하지 않고 DMA와 역할을 분리했는가?

### [안티패턴](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/128_water_scrum_fall_anti_pattern/)

- [상태 레지스터](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/167_status_register/)를 캐시된 값으로 반복 참조해 장치 완료를 영원히 못 보는 경우
...
| 개념 | 연결 포인트 |
| :-- | :-- |
| 분리형 I/O (Isolated I/O) | MMIO와 대비되는 전용 [포트](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/446_port_and_bus/) 기반 입출력 방식 |
| [DMA](/knowledge-base/studynote/02_operating_system/11_exam_summary/746_io_direct_memory_access_dma/) ([Direct Memory Access](/knowledge-base/studynote/01_computer_architecture/08_io_storage_systems/318_dma/)) | MMIO로 설정하고 실제 대량 전송은 DMA가 수행 |
| [메모리 배리어](/knowledge-base/studynote/01_computer_architecture/11_multicore_synchronization/416_memory_barrier/) ([Memory Barrier](/knowledge-base/studynote/01_computer_architecture/11_multicore_synchronization/416_memory_barrier/)) | MMIO [레지스터](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/057_register/) 접근 순서를 보장하는 핵심 장치 |
| [캐시 일관성](/knowledge-base/studynote/01_computer_architecture/11_multicore_synchronization/402_cache_coherence/) ([Cache Coherence](/knowledge-base/studynote/01_computer_architecture/11_multicore_synchronization/402_cache_coherence/)) | MMIO 영역은 일반 캐시 규칙과 다르게 취급해야 함 |
| [장치 드라이버](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/495_device_driver/) ([Device Driver](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/495_device_driver/)) | 운영체제가 MMIO를 통해 장치를 제어하는 소프트웨어 계층 |
| [MMU](/knowledge-base/studynote/02_operating_system/06_memory_management/328_mmu/) ([Memory Management Unit](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/284_mmu/)) | MMIO 매핑 권한과 [보호](/knowledge-base/studynote/02_operating_system/10_security/571_protection_vs_security/) 속성을 관리 |
