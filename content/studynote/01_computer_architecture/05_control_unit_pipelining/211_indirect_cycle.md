...
실무나 기술사 답안에서는 간접 사이클을 단순 정의로 끝내기보다, <strong>어디서 성능 비용이 터지고 왜 아직도 필요한가</strong>를 함께 말해야 한다. 가장 대표적인 사례는 포인터 체인이 긴 자료구조다. 연결 리스트, 트리, 해시 버킷, 페이지 테이블 워크처럼 "주소를 따라가야 다음 주소가 보이는" 구조는 메모리 지역성 (Locality)이 약해 캐시 미스와 지연이 누적되기 쉽다.
