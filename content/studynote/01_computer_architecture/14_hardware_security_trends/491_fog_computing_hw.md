...
기술사 답안에서는 포그를 "중간 서버"라고만 적으면 부족하다. **왜 현장에서 모아야 하는지**, <strong>어떤 데이터를 버퍼링하고 어떤 데이터만 상위로 올리는지</strong>, <strong>WAN 장애 시 무엇을 계속 돌릴지</strong>를 함께 제시해야 한다. 여기에 보안 분리와 이중화까지 적어 주면 실무 감각이 살아난다.
