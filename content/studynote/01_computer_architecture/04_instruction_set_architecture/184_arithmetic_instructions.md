...
> 1. **본질**: 산술 연산 [명령어](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/158_instruction/) (Arithmetic Instructions)는 CPU (Central Processing Unit)가 [레지스터](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/057_register/)의 값을 수학적으로 바꿔 프로그램 상태를 전진시키는 가장 기본적인 상태 변화 명령이다.
> 2. **가치**: 덧셈과 뺄셈은 루프 [카운터](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/059_counter/), 주소 계산, 길이 누적, 조건 분기까지 떠받치므로, 단순 계산 기능을 넘어 제어 흐름과 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/)의 중심축이 된다.
> 3. **판단 포인트**: 같은 `ADD`라도 정수/[부동소수점](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/087_floating_point/), signed/unsigned, [overflow](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/095_overflow/) 처리, 곱셈·나눗셈의 [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/) 차이를 구분하지 못하면 결과도 틀리고 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/)도 크게 손해 본다.
...
- **📢 섹션 요약 비유**: 산술 연산 [명령어](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/158_instruction/)는 주방에서 재료를 섞어 새로운 반죽을 만드는 손과 같다. 재료를 냉장고에서 꺼내오는 것만으로는 빵이 되지 않고, 실제로 섞고 늘리고 나눠야 다음 단계로 넘어간다.
...
| 확장 산술 | `ADC`, `SBB` | Carry 연계 덧셈/뺄셈 | 다중 [정밀도](/knowledge-base/studynote/14_data_engineering/05_exam_keywords/233_precision_recall_f1_roc_auc_threshold/)에 유리 |
| 곱셈 | `MUL`, `IMUL` | 부분곱 누적기 사용 | 보통 3~5 cycles 이상 |
| 나눗셈 | `DIV`, `IDIV` | 반복 비교/감산 | 수십 cycles 이상 가능 |
| 주소성 산술 | `ADD base, offset` | 정수 [ALU](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/117_alu/) / 주소 생성기 연계 | 메모리 접근 전 단계 |
...
| :--- | :--- | :--- |
| 표현 대상 | 정수, 주소, [카운터](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/059_counter/) | 실수 근사값 |
| 핵심 하드웨어 | 정수 [ALU](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/117_alu/) | FPU |
| [정확성](/knowledge-base/studynote/16_bigdata/01_intro/002_bigdata_5v/) | [비트](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/073_bit/)폭 내에서는 정확 | 반올림 오차 가능 |
...
- **📢 섹션 요약 비유**: 정수 산술이 자를 정확히 맞춰 목재를 자르는 일이라면, [부동소수점](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/087_floating_point/) 산술은 소수점까지 포함한 물감 배합과 같다. 둘 다 더하기를 하지만, 허용 오차와 도구가 다르다.
...
1. **overflow를 허용할 수 있는가?** 금융·[카운터](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/059_counter/)·보안 코드에서는 더 넓은 타입이나 checked arithmetic가 필요하다.
2. **signed와 unsigned 해석이 일치하는가?** Carry와 Overflow를 혼동하면 [검증](/knowledge-base/studynote/04_software_engineering/07_object_oriented/395_verification_process_review/) 코드가 틀어진다.
...
| 개념 | 연결 포인트 |
| :--- | :--- |
| [ALU](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/117_alu/) ([Arithmetic Logic Unit](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/117_alu/)) | 정수 산술 [명령어](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/158_instruction/)를 실제로 수행하는 핵심 실행 유닛이다 |
| 2의 보수 (Two's Complement) | 뺄셈과 signed 정수 표현을 덧셈 회로 중심으로 통합하게 해 준다 |
| Condition [Code](/knowledge-base/studynote/02_operating_system/02_process_thread/082_process_memory_structure/) / Flags | 산술 결과를 바탕으로 분기와 비교 판단을 가능하게 한다 |
| Load/Store [Architecture](/knowledge-base/studynote/12_it_management/05_security_compliance/319_architecture/) | 산술 전에 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 [레지스터](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/057_register/)로 가져오게 해 파이프라인을 단순화한다 |
| FPU ([Floating Point](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/087_floating_point/) Unit) | 실수 산술을 정수 산술과 다른 규칙으로 처리한다 |
| [SIMD](/knowledge-base/studynote/01_computer_architecture/10_parallel_processing_architecture/370_simd/) (Single [Instruction](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/158_instruction/), Multiple [Data](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)) | 같은 산술을 여러 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)에 [병렬](/knowledge-base/studynote/05_database/07_exam_summary/430_index_fast_full_scan/) 적용해 처리량을 높인다 |
| [Overflow](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/095_overflow/) / Carry | 정수 산술의 [정확성](/knowledge-base/studynote/16_bigdata/01_intro/002_bigdata_5v/)과 타입 해석을 가르는 핵심 판단 요소다 |
...
1. 산술 연산 [명령어](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/158_instruction/)는 컴퓨터가 숫자를 더하고 빼면서 다음 일을 정하게 해 주는 계산 버튼이에요.
