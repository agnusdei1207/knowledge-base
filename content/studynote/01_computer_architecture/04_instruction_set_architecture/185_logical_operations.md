...
| :--- | :--- | :--- | :--- |
| AND | `1`인 자리만 통과, `0`은 제거 | clear / extract | 권한 [비트](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/073_bit/) 마스킹, 서브넷 계산 |
| OR | `1`을 강제로 세움 | set | 상태 [플래그](/knowledge-base/studynote/03_network/04_data_link_layer_error/186_character_stuffing_dle_stx_etx/) 활성화 |
| XOR | 다르면 `1`, 같으면 `0` | toggle / mix | 토글 [스위치](/knowledge-base/studynote/03_network/05_lan_wan_l2_devices/238_switch_operation_principles/), 간단한 암복호 연산 |
| NOT | 모든 [비트](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/073_bit/) 반전 | invert | 보수 계산, 반전 마스크 [생성](/knowledge-base/studynote/02_operating_system/02_process_thread/087_process_state_transition/) |
| TEST | AND 결과만 보고 저장은 생략 | check | 조건 분기 전 [비트](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/073_bit/) 검사 |
...
| :--- | :--- | :--- | :--- |
| 주 관심사 | [비트](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/073_bit/) 패턴 편집 | 수치 계산 | [비트](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/073_bit/) 위치 이동 |
| [비트](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/073_bit/) 간 영향 | 독립적 | carry/borrow 전파 가능 | 이동 규칙에 의존 |
| 대표 질문 | "이 [비트](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/073_bit/)를 남길까?" | "값이 얼마나 변할까?" | "이 필드를 어느 자리로 옮길까?" |
| 자주 쓰는 조합 | mask, [flag](/knowledge-base/studynote/03_network/04_data_link_layer_error/186_character_stuffing_dle_stx_etx/) test | loop [counter](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/059_counter/), address calc | extract, align, scale |

[논리](/knowledge-base/studynote/09_security/04_endpoint_security/369_logic_bomb/) 연산의 가치가 잘 드러나는 영역은 운영체제와 네트워크, 보안이다. 운영체제는 [페이지](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/) 권한 [비트](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/073_bit/)와 [인터럽트](/knowledge-base/studynote/02_operating_system/01_overview_architecture/016_interrupt_mechanism/) 마스크를 AND와 OR로 조정하고, 네트워크는 IP 주소와 서브넷 마스크를 AND로 결합해 네트워크 주소를 계산하며, 보안과 저장장치는 XOR를 이용해 패턴 섞기와 패리티 계산을 수행한다. 즉 [논리](/knowledge-base/studynote/09_security/04_endpoint_security/369_logic_bomb/) 연산은 하드웨어와 소프트웨어가 "[비트](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/073_bit/) 단위 계약"을 맺는 공통 언어다.

특히 184번 [산술 연산 명령어](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/184_arithmetic_instructions/)와 연결해서 보면 차이가 선명하다. 산술은 프로그램 상태를 전진시키는 데 강하고, [논리](/knowledge-base/studynote/09_security/04_endpoint_security/369_logic_bomb/)는 상태의 형태를 정교하게 다듬는 데 강하다. 실제 시스템에서는 둘 중 하나만 쓰는 것이 아니라, 주소를 산술로 계산한 뒤 [레지스터](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/057_register/) [플래그](/knowledge-base/studynote/03_network/04_data_link_layer_error/186_character_stuffing_dle_stx_etx/)는 [논리](/knowledge-base/studynote/09_security/04_endpoint_security/369_logic_bomb/)로 가공하는 식으로 역할이 분담된다.

- **📢 섹션 요약 비유**: 산술 연산이 재료의 무게를 달고 반죽 양을 조절하는 일이라면, [논리](/knowledge-base/studynote/09_security/04_endpoint_security/369_logic_bomb/) 연산은 [쿠키](/knowledge-base/studynote/03_network/09_application_layer_web_email/475_cookie_local_state/) 틀로 별 모양만 남기고 나머지를 잘라 내는 일이다. 둘 다 필요하지만 하는 일은 다르다.
...
- `ADD`나 `SUB`로 상태 [비트](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/073_bit/)를 억지로 조작해 carry 부작용을 만드는 코드
...
| 개념 | 연결 포인트 |
| :--- | :--- |
| [ALU](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/117_alu/) ([Arithmetic Logic Unit](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/117_alu/)) | [논리](/knowledge-base/studynote/09_security/04_endpoint_security/369_logic_bomb/) 연산 [명령어](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/158_instruction/)를 실제 게이트 연산으로 실행하는 핵심 유닛이다 |
| [비트](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/073_bit/) 마스킹 ([Bit](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/086_fenwick_tree/) Masking) | 특정 [비트](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/073_bit/)를 추출, 삭제, 설정하는 대표 활용 방식이다 |
| 시프트 (Shift) | [논리](/knowledge-base/studynote/09_security/04_endpoint_security/369_logic_bomb/) 연산과 결합해 [비트](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/073_bit/) 필드 위치를 맞추고 분리한다 |
| 조건 코드 (Condition Codes) | TEST, AND 등의 결과를 분기 판단으로 이어 준다 |
| [상태 레지스터](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/167_status_register/) ([Status Register](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/167_status_register/)) | [논리](/knowledge-base/studynote/09_security/04_endpoint_security/369_logic_bomb/) 연산이 가장 자주 적용되는 [비트](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/073_bit/) 필드 대상이다 |
| [Boolean Algebra](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/022_boolean_algebra/) | AND, OR, NOT, XOR의 수학적 기반이다 |
...
1. [논리](/knowledge-base/studynote/09_security/04_endpoint_security/369_logic_bomb/) 연산 [명령어](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/158_instruction/)는 컴퓨터가 전구판에서 원하는 전구만 켜고 끄게 해 주는 [스위치](/knowledge-base/studynote/03_network/05_lan_wan_l2_devices/238_switch_operation_principles/) 상자예요.
2. 더하기처럼 옆칸까지 밀려 가지 않아서, 딱 고른 자리만 안전하게 바꿀 수 있어요.
3. 그래서 컴퓨터는 이 [스위치](/knowledge-base/studynote/03_network/05_lan_wan_l2_devices/238_switch_operation_principles/)로 비밀번호도 섞고, 인터넷 주소도 자르고, 장치 [상태도](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/065_state_diagram/) 확인한답니다.
...
← **이전**: [184. 산술 연산 명령어 (Arithmetic Instructions)](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/184_arithmetic_instructions/)
**다음**: [186. 제어 흐름 명령어 (Control Flow)](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/186_control_flow_instructions/) →
