...
기술사 관점에서의 핵심 문장은 명확하다. <strong>로드/스토어 아키텍처는 메모리 병목을 없애는 구조가 아니라, 메모리 병목의 위치를 분리하고 제어 가능하게 만드는 구조</strong>다. 따라서 채택 여부는 "명령어 수가 적은가"가 아니라, "예측 가능성과 병렬화 가능성을 얼마나 높일 수 있는가"로 판단해야 한다.
