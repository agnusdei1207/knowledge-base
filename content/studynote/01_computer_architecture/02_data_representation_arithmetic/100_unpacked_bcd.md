...
> 1. **본질**: 언팩드 [BCD](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/098_bcd/) (Unpacked [Binary Coded Decimal](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/098_bcd/))는 1바이트(8비트) 공간의 하위 4비트에만 10진수 숫자($0 \sim 9$) 하나를 저장하고, 상위 4비트에는 문자열 출력을 위한 존(Zone) [비트](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/073_bit/)를 채워 넣는 거시적 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 포맷이다.
> 2. **가치**: 메모리 공간의 50%를 낭비하지만, 별도의 디코딩 연산 없이 메모리에 있는 숫자를 즉시 텍스트([ASCII](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/103_ascii/), EBCDIC)로 변환하여 [모니터](/knowledge-base/studynote/02_operating_system/04_synchronization/229_monitor/)나 프린터로 고속 출력할 수 있는 직관적 연결성을 제공한다.
> 3. **판단 포인트**: 현대의 고집적 컴퓨팅에서는 메모리 낭비와 연산 오버헤드로 인해 사장되었으나, 금융권 메인프레임이나 구형 [데이터 파이프라인](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/645_data_pipeline_acceleration/) 마이그레이션 시 오버펀치 (Overpunch) 기반의 부호 파싱 문제를 다루기 위해 반드시 이해해야 하는 레거시 규격이다.
...
- **📢 섹션 요약 비유**: 언팩드 BCD는 과자 한 개(숫자)를 커다란 박스(1바이트)에 넣고 뽁뽁이(존 [비트](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/073_bit/))로 빈 공간을 채워 파는 과대 포장과 같다. 창고는 금방 꽉 차지만, 손님([모니터](/knowledge-base/studynote/02_operating_system/04_synchronization/229_monitor/))에게 나눠줄 때는 박스째로 바로 주면 되므로 배포가 매우 빠르다.
...
언팩드 BCD는 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 알뜰하게 쓰는 팩드 [BCD](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/098_bcd/) ([Packed BCD](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/099_packed_bcd/))와 명확히 대비된다.
...
- **📢 섹션 요약 비유**: 언팩드 BCD가 우표(존 [비트](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/073_bit/))가 미리 붙어있는 편지라면, 팩드 BCD는 편지지만 모아놓은 뭉치다. 우체통(프린터)에 넣을 때는 언팩드가 빠르지만, 가방(메모리)에 많이 담거나 내용을 계산할 때는 팩드가 훨씬 유리하다.
...
1. **메인프레임 마이그레이션**: COBOL 메인프레임의 EBCDIC 기반 `PIC S9(5)` 형식 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 현대의 RDBMS나 [Hadoop](/knowledge-base/studynote/03_network/16_data_center_cloud/843_hadoop_rack_awareness_data_replication_topology/) 파이프라인으로 이관할 때, 마지막 자리 숫자가 'A', 'K' 등 외계어 문자로 깨져 보이는 현상이 발생한다. 이는 오류가 아니라 오버펀칭된 정상 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)이므로, 전처리 레이어에서 이를 언팩하여 부호와 숫자로 역파싱해야 한다.
2. **저사양 임베디드 I/O 최적화**: 극단적으로 느린 영수증 프린터용 MCU (Micro Controller Unit) 등에서는 연산 결과를 Packed 포맷으로 들고 있다가, 출력 직전에 [비트](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/073_bit/) 논리합(OR) 연산으로 존(Zone) 값을 덧씌워 언팩드 BCD로 형 변환한 뒤 [DMA](/knowledge-base/studynote/02_operating_system/11_exam_summary/746_io_direct_memory_access_dma/) ([Direct Memory Access](/knowledge-base/studynote/01_computer_architecture/08_io_storage_systems/318_dma/))로 쏟아버리면 출력 병목을 해결할 수 있다.

- **📢 섹션 요약 비유**: 언팩드 BCD를 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)베이스에 그대로 저장하는 것은, 옷을 보관할 때마다 백화점 진열용 마네킹에 입힌 채로 옷장에 넣는 것과 같다. 당장 구경하기는 좋지만 공간 부족으로 스토리지 비용이 폭발하게 된다.
...
1. 아주 큰 과자 상자(1바이트)에 과자를 딱 하나만 넣고, 나머지 빈 공간은 헬륨 [가스](/knowledge-base/studynote/06_ict_convergence/01_blockchain/024_gas/)(존 [비트](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/073_bit/))로 빵빵하게 채워 파는 과대 포장 상자예요.
2. 손님([모니터](/knowledge-base/studynote/02_operating_system/04_synchronization/229_monitor/))이 상자를 봤을 때 겉포장만 보고도 "아! 이건 숫자 5구나!" 하고 바로 알아볼 수 있게 예쁘게 포장해둔 거죠.
3. 하지만 창고(메모리)에 쌓을 때는 너무 공간을 많이 차지해서, 요즘은 헬륨 [가스](/knowledge-base/studynote/06_ict_convergence/01_blockchain/024_gas/)를 빼고 과자를 두 개씩 꽉꽉 채우는 [압축](/knowledge-base/studynote/02_operating_system/06_memory_management/347_compaction/) 기술로 바뀌었답니다!
...
← **이전**: [99. 팩드 BCD (Packed BCD)](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/099_packed_bcd/)
**다음**: [101. 3초과 코드 (Excess-3 Code)](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/101_excess_3/) →
