...
인간은 숫자를 읽을 때 머리([MSB](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/080_msb/))부터 읽는다. 하지만 덧셈을 할 때는 꼬리(일의 자리)부터 계산해 올라가며 올림수를 처리한다. 여기서 리틀 엔디안의 위대한 역발상이 시작되었다.

[초기](/knowledge-base/studynote/03_network/08_transport_layer/459_quic_fec_forward_error_correction/) 인텔의 칩 설계자들은 "컴퓨터는 사람 읽으라고 만든 게 아니라, 기계가 연산([ALU](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/117_alu/))을 광속으로 하기 위해 만든 것이다"라고 선언했다. 데이터를 메모리에 넣을 때 꼬리(Little End)부터 앞쪽 번지에 거꾸로 박아두면, ALU가 메모리에서 데이터를 퍼 올리자마자 바로 1의 자리 덧셈을 시작할 수 있다. 만약 [빅 엔디안](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/114_big_endian/)처럼 머리부터 줬다면, ALU는 꼬리가 어딘지 끝까지 다 읽은 뒤에야 비로소 첫 덧셈을 시작할 수 있어 클럭을 낭비하게 된다. 리틀 엔디안은 인간의 [가독성](/knowledge-base/studynote/04_software_engineering/06_software_architecture/333_readability_vs_efficiency/)을 철저히 짓밟고, 오직 '쇳덩어리의 연산 효율' 하나에 모든 것을 몰빵한 하드웨어 다윈주의의 산물이다.
...
- **📢 섹션 요약 비유**: 리틀 엔디안은 '자동차 엔진 조립의 편의성'이고, [빅 엔디안](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/114_big_endian/)은 '도로를 달릴 때의 직관성'이다. 공장 안(CPU)에서는 뒤집어 조립하는 게 편하지만, 도로(네트워크)로 나갈 땐 반드시 앞을 보게 뒤집어 줘야만 차가 달릴 수 있다.
...
- **📢 섹션 요약 비유**: 리틀 엔디안은 컴퓨터 내부의 '마법의 서랍'이다. 서랍을 열면 물건이 다 거꾸로 처박혀 있어서 주인이 보기엔 화가 나지만, 서랍 안의 꼬마 로봇([ALU](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/117_alu/))은 그 거꾸로 된 순서 덕분에 눈 감고도 가장 빠르게 물건을 찾아내 조립할 수 있는 최고의 작업 환경이다.
