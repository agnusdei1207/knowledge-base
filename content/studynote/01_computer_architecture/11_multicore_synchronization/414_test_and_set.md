...
> 1. **본질**: Test-and-Set은 공유 변수의 값을 읽고 곧바로 잠금 값으로 바꾸는 과정을 하나의 원자적(Atomic) 읽기-수정-[쓰기](/knowledge-base/studynote/13_cloud_architecture/05_data_engineering/289_cqrs_db/)(Read-Modify-Write) 동작으로 묶어, [임계 구역](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/214_critical_section/)([Critical Section](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/214_critical_section/)) 진입 경쟁에서 승자를 단 한 명만 남기는 하드웨어 명령이다.
> 2. **가치**: 소프트웨어가 `if (lock == 0) lock = 1`처럼 두 단계로 처리하면 경합 조건([Race Condition](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/213_race_condition/))이 생기지만, Test-and-Set은 그 틈 자체를 하드웨어가 제거해 [상호 배제](/knowledge-base/studynote/02_operating_system/05_deadlock/283_mutual_exclusion/)([Mutual Exclusion](/knowledge-base/studynote/02_operating_system/05_deadlock/283_mutual_exclusion/))의 최소 토대를 제공한다.
> 3. **판단 포인트**: 구현은 단순하지만 실패한 [스레드](/knowledge-base/studynote/02_operating_system/02_process_thread/092_thread_lwp/)도 계속 [쓰기](/knowledge-base/studynote/13_cloud_architecture/05_data_engineering/289_cqrs_db/) 경쟁을 일으켜 [캐시 일관성](/knowledge-base/studynote/01_computer_architecture/11_multicore_synchronization/402_cache_coherence/)([Cache Coherence](/knowledge-base/studynote/01_computer_architecture/11_multicore_synchronization/402_cache_coherence/)) 비용이 커지므로, 짧은 락에는 유용해도 고경합 환경에서는 TTAS (Test-and-Test-and-Set), [CAS](/knowledge-base/studynote/02_operating_system/11_exam_summary/768_cas_compare_and_swap_lock_free/) ([Compare-and-Swap](/knowledge-base/studynote/01_computer_architecture/11_multicore_synchronization/415_compare_and_swap/)), 큐 락으로 보완해야 한다.
...
- **📢 섹션 요약 비유**: 화장실 문이 비었는지 보고 손으로 잠그는 사이에 다른 사람이 끼어들 수 있다. Test-and-Set은 손잡이를 잡는 순간 문 [확인](/knowledge-base/studynote/04_software_engineering/12_testing_maintenance/396_validation/)과 잠금이 동시에 끝나는 자동 잠금장치다.
...
Test-and-Set을 이해할 때는 단순 성공 여부보다 **실패 비용** 을 함께 봐야 한다. 이 명령은 락이 이미 잠겨 있어도 계속 [쓰기](/knowledge-base/studynote/13_cloud_architecture/05_data_engineering/289_cqrs_db/) 성격의 경쟁을 일으키므로, 코어 수가 늘고 경합이 심해질수록 캐시 무효화와 [버스](/knowledge-base/studynote/01_computer_architecture/09_system_bus_interconnects/344_bus/) 트래픽이 증가한다. 그래서 현대 시스템은 같은 [상호 배제](/knowledge-base/studynote/02_operating_system/05_deadlock/283_mutual_exclusion/) 목적이라도 접근법을 세분화한다.

| 항목 | Test-and-Set | TTAS (Test-and-Test-and-Set) | [CAS](/knowledge-base/studynote/02_operating_system/11_exam_summary/768_cas_compare_and_swap_lock_free/) ([Compare-and-Swap](/knowledge-base/studynote/01_computer_architecture/11_multicore_synchronization/415_compare_and_swap/)) |
| :--- | :--- | :--- | :--- |
| 실패 시 동작 | 다시 [쓰기](/knowledge-base/studynote/13_cloud_architecture/05_data_engineering/289_cqrs_db/) 경쟁 발생 | 먼저 읽기만 하며 대기 | 기대값이 다르면 쓰지 않음 |
| 장점 | 구현 가장 단순 | 불필요한 [쓰기](/knowledge-base/studynote/13_cloud_architecture/05_data_engineering/289_cqrs_db/) 트래픽 감소 | 조건부 갱신으로 범용성 높음 |
| 약점 | 고경합 시 캐시 핑퐁 심함 | 락 해제 순간 경쟁 재폭발 가능 | ABA 문제 등 설계 난도 상승 |
| 대표 용도 | 고전적 [스핀락](/knowledge-base/studynote/02_operating_system/04_synchronization/222_spinlock/) | 개선된 [스핀락](/knowledge-base/studynote/02_operating_system/04_synchronization/222_spinlock/) | 락프리 자료구조 |

TTAS (Test-and-Test-and-Set)는 먼저 일반 읽기로 `lock == 1` 여부를 [확인](/knowledge-base/studynote/04_software_engineering/12_testing_maintenance/396_validation/)한 뒤, 실제로 풀린 것처럼 보일 때만 Test-and-Set을 호출한다. 이 방식은 잠겨 있는 동안에는 읽기 공유 상태를 유지하므로, 순수 Test-and-Set보다 [캐시 일관성](/knowledge-base/studynote/01_computer_architecture/11_multicore_synchronization/402_cache_coherence/) 부담이 적다.

CAS는 더 나아가 "현재 값이 내가 기대한 값과 같을 때만 갱신"한다. 따라서 실패 시 불필요한 [쓰기](/knowledge-base/studynote/13_cloud_architecture/05_data_engineering/289_cqrs_db/)를 줄일 수 있고, 단순 락을 넘어 [스택](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/057_stack/)·큐 같은 락프리 구조에도 활용된다. 반면 Test-and-Set은 의미가 직관적이어서 [동기화](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/212_synchronization_mechanisms/) 개념의 출발점으로는 매우 중요하다.

이 개념은 [운영체제](/knowledge-base/studynote/02_operating_system/01_overview_architecture/001_operating_system_purpose/)(OS, [Operating System](/knowledge-base/studynote/02_operating_system/01_overview_architecture/001_operating_system_purpose/))의 [스핀락](/knowledge-base/studynote/02_operating_system/04_synchronization/222_spinlock/), [메모리 일관성 모델](/knowledge-base/studynote/01_computer_architecture/11_multicore_synchronization/410_memory_consistency_model/), [메모리 배리어](/knowledge-base/studynote/01_computer_architecture/11_multicore_synchronization/416_memory_barrier/)([Memory Barrier](/knowledge-base/studynote/01_computer_architecture/11_multicore_synchronization/416_memory_barrier/))와도 연결된다. 원자적 락 획득만으로 모든 순서 문제가 해결되는 것은 아니므로, 실제 구현에서는 락 획득 시 acquire, 해제 시 release 의미를 함께 맞춰야 한다.
...
1. [임계 구역](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/214_critical_section/) 길이가 [문맥 교환](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/211_context_switch/)보다 충분히 짧은가?
2. 경쟁 [스레드](/knowledge-base/studynote/02_operating_system/02_process_thread/092_thread_lwp/) 수가 적거나, 코어 수 대비 경합률이 낮은가?
...
- 긴 대기 구간을 가진 코드에 Test-and-Set [스핀락](/knowledge-base/studynote/02_operating_system/04_synchronization/222_spinlock/)을 그대로 적용
...
Test-and-Set의 가장 큰 의미는 "[상호 배제](/knowledge-base/studynote/02_operating_system/05_deadlock/283_mutual_exclusion/)는 결국 하드웨어가 최종 책임을 져야 한다"는 사실을 드러낸 데 있다. 이 명령 덕분에 소프트웨어는 경쟁 조건을 추상화한 락을 만들 수 있었고, [운영체제](/knowledge-base/studynote/02_operating_system/01_overview_architecture/001_operating_system_purpose/)와 런타임은 그 위에 뮤텍스([Mutex](/knowledge-base/studynote/02_operating_system/04_synchronization/223_mutex/)), [스핀락](/knowledge-base/studynote/02_operating_system/04_synchronization/222_spinlock/), [세마포어](/knowledge-base/studynote/02_operating_system/04_synchronization/224_semaphore/)([Semaphore](/knowledge-base/studynote/02_operating_system/04_synchronization/224_semaphore/))를 계층적으로 구축할 수 있었다.
...
| 개념 | 연결 포인트 |
| :--- | :--- |
| [하드웨어 동기화](/knowledge-base/studynote/01_computer_architecture/11_multicore_synchronization/413_hardware_synchronization/) ([Hardware Synchronization](/knowledge-base/studynote/01_computer_architecture/11_multicore_synchronization/413_hardware_synchronization/)) | Test-and-Set이 속한 원자적 [명령어](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/158_instruction/) 계열 |
| [스핀락](/knowledge-base/studynote/02_operating_system/04_synchronization/222_spinlock/) ([Spinlock](/knowledge-base/studynote/02_operating_system/04_synchronization/222_spinlock/)) | Test-and-Set을 반복 호출해 락을 획득하는 대표 구조 |
| [캐시 일관성](/knowledge-base/studynote/01_computer_architecture/11_multicore_synchronization/402_cache_coherence/) ([Cache Coherence](/knowledge-base/studynote/01_computer_architecture/11_multicore_synchronization/402_cache_coherence/)) | 원자적 락 획득이 실제로 성립하도록 캐시 라인 소유권을 조정 |
| [CAS](/knowledge-base/studynote/02_operating_system/11_exam_summary/768_cas_compare_and_swap_lock_free/) ([Compare-and-Swap](/knowledge-base/studynote/01_computer_architecture/11_multicore_synchronization/415_compare_and_swap/)) | Test-and-Set의 단순성을 넘어선 조건부 갱신 명령 |
| [메모리 배리어](/knowledge-base/studynote/01_computer_architecture/11_multicore_synchronization/416_memory_barrier/) ([Memory Barrier](/knowledge-base/studynote/01_computer_architecture/11_multicore_synchronization/416_memory_barrier/)) | 락 획득/해제 전후의 메모리 순서를 올바르게 보장 |
...
← **이전**: [413. 하드웨어 동기화 (Hardware Synchronization)](/knowledge-base/studynote/01_computer_architecture/11_multicore_synchronization/413_hardware_synchronization/)
**다음**: [415. Compare-and-Swap (CAS) 연산](/knowledge-base/studynote/01_computer_architecture/11_multicore_synchronization/415_compare_and_swap/) →
