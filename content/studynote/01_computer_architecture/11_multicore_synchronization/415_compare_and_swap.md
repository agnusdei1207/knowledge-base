...
반면 비즈니스 로직이 길거나, 한 번 실패하면 다시 계산하는 비용이 큰 연산에는 CAS가 오히려 부적합하다. 예를 들어 긴 연결 리스트 수정, 복잡한 메모리 할당/해제, 외부 입출력(I/O)과 결합된 갱신은 재시도 비용이 커서 락 기반 설계가 더 단순하고 안전할 수 있다. 기술사 답안에서는 "CAS가 빠르다"가 아니라 <strong>"짧은 갱신에는 채택, 긴 임계 구역에는 회피"</strong> 로 정리해야 한다.
