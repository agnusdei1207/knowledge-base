...
기술사 관점에서의 답안 포인트는 명확하다. <strong>완화된 일관성은 채택 여부의 문제가 아니라 현대 멀티코어의 기본 전제</strong>이며, 설계자는 필요한 순서만 선택적으로 복원해야 한다. 즉 전체를 강한 모델로 묶으면 느리고, 모두 완화한 채 방치하면 위험하므로, 배리어·원자 연산·락의 조합으로 최소 충분 질서를 설계하는 것이 핵심 판단이다.
