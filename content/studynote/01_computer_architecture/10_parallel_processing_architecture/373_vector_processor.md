...
기술사 답안 관점에서 기억할 포인트는 단순하다. 벡터 프로세서는 “SIMD이므로 빠르다”가 아니라, <strong>데이터 규칙성이 충분할 때 제어 비용을 줄여 높은 처리량을 확보하는 구조</strong>라고 서술해야 한다. 그리고 성능 저하 원인은 연산기 부족보다 메모리 병목, 분기, 의존성에서 먼저 찾는 것이 정확하다.
