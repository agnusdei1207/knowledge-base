...
기술사 관점에서는 "SIMD는 빠르다"가 아니라, <strong>데이터 구조·분기 패턴·메모리 대역폭까지 함께 설계해야 진짜 성능이 나온다</strong>고 답해야 한다. 예를 들어 벡터 폭이 8개라 해도 8개를 꾸준히 공급하지 못하면 이론 성능은 의미가 없다. 반대로 데이터 구조를 SoA로 바꾸고 분기를 마스크 처리로 단순화하면, 같은 CPU에서도 체감 성능이 크게 달라질 수 있다.
