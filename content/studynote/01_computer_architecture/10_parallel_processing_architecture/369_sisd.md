...
반대로 대규모 이미지 필터, 행렬 곱셈, 로그 배치 분석처럼 동일 연산을 독립 데이터 묶음에 반복하는 작업은 SISD로 밀어붙이면 비효율이 커진다. 이 경우는 SIMD나 MIMD로 넘겨야 하며, SISD는 오히려 병목이 된다. 따라서 기술사 관점의 핵심은 "SISD가 단순해서 좋다"가 아니라, <strong>의존성·지연시간·디버깅 비용을 함께 보고 어디까지 순차로 남길지 판단하는 것</strong>이다.
