...
[코드 커버리지](/knowledge-base/studynote/15_devops_sre/02_cicd_gitops/078_code_coverage/)는 테스트가 코드를 지나쳤는가만 측정한다. 테스트가 assert 없이 메서드만 호출해도 커버리지는 100%가 된다. 이런 의미 없는 테스트를 걸러내는 것이 [뮤테이션 테스트](/knowledge-base/studynote/04_software_engineering/10_trends_pm_quality/638_mutation_testing_test_case_verification/)다.

[뮤테이션 테스트](/knowledge-base/studynote/04_software_engineering/10_trends_pm_quality/638_mutation_testing_test_case_verification/)는 컴파일러나 도구가 자동으로 코드를 변형(예: > -> >=, + -> -)하고, 변형된 코드가 테스트 실패를 유발하는지 [확인](/knowledge-base/studynote/04_software_engineering/12_testing_maintenance/396_validation/)한다. 테스트 실패 = 뮤턴트 탐지, 테스트 통과 = 뮤턴트 생존.

> 📢 **섹션 요약 비유**: [뮤테이션 테스트](/knowledge-base/studynote/04_software_engineering/10_trends_pm_quality/638_mutation_testing_test_case_verification/)는 보안 시스템 모의 [침투 테스트](/knowledge-base/studynote/09_security/13_secops_ir_forensics/676_penetration_testing/)다. 실제로 침입을 시도해서 경보 시스템이 울리는지 [확인](/knowledge-base/studynote/04_software_engineering/12_testing_maintenance/396_validation/)한다. 경보가 안 울리면 경보 시스템이 고장난 것이다.
...
| 항목 | [코드 커버리지](/knowledge-base/studynote/15_devops_sre/02_cicd_gitops/078_code_coverage/) | [뮤테이션 테스트](/knowledge-base/studynote/04_software_engineering/10_trends_pm_quality/638_mutation_testing_test_case_verification/) |
|:---|:---|:---|
| 측정 대상 | 코드 실행 여부 | 테스트 [결함](/knowledge-base/studynote/04_software_engineering/06_software_architecture/352_defect_definition/) 탐지 능력 |
| 100% 달성 가능? | 쉬움 (assert 없이도) | 어려움 (실제 [검증](/knowledge-base/studynote/04_software_engineering/07_object_oriented/395_verification_process_review/) 필요) |
| 비용 | 낮음 | 높음 (뮤턴트 수만큼 실행) |
| [신뢰도](/knowledge-base/studynote/14_data_engineering/02_math_mining/085_confidence_association_rule_conditional_probability/) | 낮음 | 높음 |

| 도구 | 언어 | 특징 |
|:---|:---|:---|
| PITest | Java | [바이트](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/074_byte/)코드 레벨 변이 |
| Stryker | JS/TS | 멀티 프레임워크 지원 |
| MutPy | Python | Python 전용 |

> 📢 **섹션 요약 비유**: [코드 커버리지](/knowledge-base/studynote/15_devops_sre/02_cicd_gitops/078_code_coverage/)는 소화기가 있는지 [확인](/knowledge-base/studynote/04_software_engineering/12_testing_maintenance/396_validation/)하는 것이고, [뮤테이션 테스트](/knowledge-base/studynote/04_software_engineering/10_trends_pm_quality/638_mutation_testing_test_case_verification/)는 소화기를 실제로 작동시켜보는 것이다. 작동 안 하는 소화기는 없는 것과 같다.
...
| 뮤테이션 스코어 | 탐지됨/(탐지됨+생존) x 100 |
| PITest / Stryker | 대표 [뮤테이션 테스트](/knowledge-base/studynote/04_software_engineering/10_trends_pm_quality/638_mutation_testing_test_case_verification/) 도구 |
...
1. [뮤테이션 테스트](/knowledge-base/studynote/04_software_engineering/10_trends_pm_quality/638_mutation_testing_test_case_verification/)는 선생님이 일부러 틀린 답을 답안지에 넣고 학생(테스트)이 고쳐 쓰는지 [확인](/knowledge-base/studynote/04_software_engineering/12_testing_maintenance/396_validation/)하는 거예요.
2. 뮤턴트가 생존한다는 건 학생이 선생님의 실수를 못 찾았다는 뜻이에요. 그 학생(테스트)은 제대로 [검증](/knowledge-base/studynote/04_software_engineering/07_object_oriented/395_verification_process_review/)을 안 한 거예요.
