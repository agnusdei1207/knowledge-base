...
> 2. **가치**: 무거운 [E2E](/studynote/15_devops_sre/05_devsecops/265_e2e_end_to_ui_selenium/) 의존도를 줄이며 인터페이스 안정성을 높인다.
> 3. **판단 포인트**: 계약 [버전](/studynote/03_network/06_network_layer_ip/288_version_ihl_tos_total_length/) 관리와 소비자 주도 테스트 원칙을 지켜야 한다.

---

## Ⅰ. 개요 및 필요성

계약 테스트 (Contract Testing / Pact)는 [DevOps](/studynote/04_software_engineering/uncategorized/652_devops_calms_culture/)/[SRE](/studynote/04_software_engineering/02_requirements_analysis/100_sre_site_reliability_engineering_error_budget/) 환경에서 반복되는 운영 문제를 구조적으로 다루기 위해 등장한 개념이다. 변경 속도가 빠른 조직일수록 테스트는 출시 직전의 이벤트가 아니라 개발 흐름 전체의 안전망이어야 한다. 핵심은 [서비스](/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/) 간 [API](/studynote/02_operating_system/01_overview_architecture/014_api_posix/) 계약 [호환성](/studynote/04_software_engineering/06_software_architecture/344_compatibility_usability/)을 프로바이더/컨슈머 기준으로 [검증](/studynote/04_software_engineering/07_object_oriented/395_verification_process_review/)하는 방식에 있다. 이 관점에서 보면, 이 주제는 단순 기술 소개가 아니라 속도와 안정성을 동시에 맞추기 위한 운영 설계 기준에 가깝다.
...
이 그림은 계약 테스트가 입력, 실행, [검증](/studynote/04_software_engineering/07_object_oriented/395_verification_process_review/), 환류를 한 흐름으로 묶는다는 점을 보여준다. 즉 기술 자체보다도 제어 루프와 피드백 구조가 본질이다.
...
계약 테스트의 핵심 원리는 구성 요소를 나열하는 데 있지 않고, 목표 상태를 어떻게 해석하고 실제 상태에 어떻게 반영하며 그 결과를 어떻게 다시 측정하는지에 있다. 특히 [E2E](/studynote/15_devops_sre/05_devsecops/265_e2e_end_to_ui_selenium/) Test와 달리 계약 테스트는 실행 전후의 차이와 [정책](/studynote/10_ai/02_dl_architecture_new/164_policy/)을 함께 본다는 점에서 운영 품질 차이를 만든다.
...
계약 테스트는 보통 [E2E](/studynote/15_devops_sre/05_devsecops/265_e2e_end_to_ui_selenium/) Test와 비교할 때 경계가 선명해진다. 계약 테스트가 더 많은 자동화와 제어를 제공하더라도, 모든 상황에서 무조건 우월한 것은 아니다. 시스템 규모, 팀 성숙도, 규제 수준, 운영 복잡도가 함께 맞아야 장점이 실제 성과로 이어진다.

| 비교 축 | 계약 테스트 | [E2E](/studynote/15_devops_sre/05_devsecops/265_e2e_end_to_ui_selenium/) Test |
|:---|:---|:---|
| 중심 목표 | 계약 테스트의 목적에 맞춘 제어와 자동화 | 더 전통적이거나 대안적인 운영 방식 |
| 강점 | 무거운 [E2E](/studynote/15_devops_sre/05_devsecops/265_e2e_end_to_ui_selenium/) 의존도를 줄이며 인터페이스 안정성을 높인다. | 구조가 단순하거나 도입 장벽이 낮음 |
...
또한 이 주제는 Pact, Consumer-driven Contract, [Domain](/studynote/05_database/02_modeling_normalization/064_relation_domain/) Boundary처럼 주변 개념과 강하게 연결된다. 기술사 관점에서는 개별 정의보다도 이런 연결 구조를 설명해야 답안의 깊이가 생긴다.
...
앞으로는 계약 테스트, [카나리](/studynote/02_operating_system/10_security/595_canary_stack_smashing_protector/) 분석, 품질 게이트가 배포 자동화와 더 촘촘히 묶인다. 따라서 계약 테스트는 "한 번 도입하는 기술"이 아니라, 변화가 잦은 시스템을 어떻게 안정적으로 운영할 것인지에 대한 사고 틀로 기억하는 것이 맞다.
...
| [Domain](/studynote/05_database/02_modeling_normalization/064_relation_domain/) Boundary | 계약 테스트를 자동화·확장 측면에서 연결하는 개념 |
| [API](/studynote/02_operating_system/01_overview_architecture/014_api_posix/) | 계약 테스트 적용 후 후속 발전 방향을 설명하는 개념 |
...
이 흐름도는 계약 테스트가 선행 개념 위에 서서 운영 자동화, 보안, 확장, 가시성 중 어떤 축으로 확장되는지를 [압축](/studynote/02_operating_system/06_memory_management/347_compaction/)해서 보여준다.
...
<- **이전**: [265. E2E (End-to-End) 테스트 / UI 테스트](/studynote/15_devops_sre/05_devsecops/265_e2e_end_to_ui_selenium/)
**다음**: [267. 부하 테스트 (Load Testing) 및 스트레스 테스트 CI 파이프라인 임베드 (JMeter, k6)](/studynote/15_devops_sre/05_devsecops/267_load_testing_ci_jmeter_k6/) ->
