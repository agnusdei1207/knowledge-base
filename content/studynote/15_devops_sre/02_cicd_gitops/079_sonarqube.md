...
- **📢 섹션 요약 비유**: 소나큐브는 소스 코드의 '맞춤법 및 문맥 자동 검사기'다. 작가(개발자)가 소설을 썼을 때, 사람이 다 읽어보기 전에 기계가 먼저 쫙 훑어보고 "3페이지 오타, 10페이지 앞뒤 [논리](/knowledge-base/studynote/09_security/04_endpoint_security/369_logic_bomb/) 안 맞음, 15페이지 심한 욕설(보안 취약점) 포함!"이라고 족집게처럼 찾아내서 빨간 펜을 긋고 출판(배포)을 금지하는 무서운 편집장이다.
...
### [정적 분석](/knowledge-base/studynote/04_software_engineering/06_software_architecture/331_static_analysis/)(Static) vs [동적 분석](/knowledge-base/studynote/04_software_engineering/06_software_architecture/332_dynamic_analysis/)(Dynamic)
소나큐브가 커버하는 영역과 못 하는 영역의 명확한 한계선이다.

| 비교 항목 | [정적 분석](/knowledge-base/studynote/04_software_engineering/06_software_architecture/331_static_analysis/) ([Static Analysis](/knowledge-base/studynote/04_software_engineering/06_software_architecture/331_static_analysis/) / [SAST](/knowledge-base/studynote/04_software_engineering/08_security_compliance_devsecops/491_sast_static_analysis/)) - **SonarQube** | [동적 분석](/knowledge-base/studynote/04_software_engineering/06_software_architecture/332_dynamic_analysis/) ([Dynamic Analysis](/knowledge-base/studynote/04_software_engineering/06_software_architecture/332_dynamic_analysis/) / [DAST](/knowledge-base/studynote/04_software_engineering/08_security_compliance_devsecops/492_dast_dynamic_analysis/)) |
|:---|:---|:---|
| **분석 방법** | **코드를 실행하지 않고(멈춘 상태), 텍스트(문법/구조)만 검사** | 코드를 실제 서버에 띄우고(실행), 해킹 공격을 쏴봄 |
| **발견 잘하는 것** | **코드 컨벤션(띄어쓰기), 스파게티 코드, 문법적 보안 취약점** | 실제 런타임 [메모리 누수](/knowledge-base/studynote/02_operating_system/10_security/612_memory_leak_detection/), 비즈니스 로직 에러 |
...
소나큐브는 부검의와 같다. 시체(실행 안 된 코드)를 해부해서 위장에 독약(나쁜 코드 패턴)이 있는지는 기가 막히게 찾아내지만, 이 사람이 살아 움직일 때(런타임) 어떤 이상한 행동(비즈니스 오류)을 할지는 절대 알 수 없다. 따라서 소나큐브(정적)와 [단위 테스트](/knowledge-base/studynote/04_software_engineering/12_testing_maintenance/397_unit_test/)/QA(동적)는 반드시 융합되어야 한다.

- **📢 섹션 요약 비유**: [정적 분석](/knowledge-base/studynote/04_software_engineering/06_software_architecture/331_static_analysis/)(소나큐브)은 자동차를 출발시키지 않고 '엔진 구조와 나사 조임 상태'를 도면과 눈으로 깐깐하게 검사하는 것이다. [동적 분석](/knowledge-base/studynote/04_software_engineering/06_software_architecture/332_dynamic_analysis/)은 실제로 시동을 걸고 고속도로를 시속 150km로 달리면서 '브레이크가 잘 듣는지, 핸들이 떨리는지' 직접 몸으로 부딪쳐 검사하는 것이다. 둘 다 해야 완벽한 자동차가 나온다.
