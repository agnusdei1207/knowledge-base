...
> 1. **본질**: 빌드 [캐싱](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/456_caching/) 최적화는 [CI](/knowledge-base/studynote/12_it_management/02_itsm_itil/874_configuration_item/)/CD [파이프](/knowledge-base/studynote/02_operating_system/02_process_thread/123_pipe/)라인에서 변하지 않는 [종속성](/knowledge-base/studynote/15_devops_sre/01_culture_methodology/008_dependencies/)([Dependencies](/knowledge-base/studynote/15_devops_sre/01_culture_methodology/008_dependencies/))과 이전 빌드의 중간 결과물을 재사용하여 [컨테이너](/knowledge-base/studynote/04_software_engineering/09_cloud_native_ai_architecture/561_container_based_deployment/) 이미지를 조립하는 가속 기술이다.
> 2. **가치**: 매 빌드마다 수백 메가바이트의 외부 [라이브러리](/knowledge-base/studynote/04_software_engineering/06_software_architecture/336_library_vs_framework/)를 다시 다운로드하는 낭비를 막아 빌드 시간을 분 단위에서 초 단위로 단축하고, 컴퓨팅 자원 비용(Cost)을 극적으로 절감한다.
> 3. **판단 포인트**: [도커](/knowledge-base/studynote/02_operating_system/01_overview_architecture/063_docker_architecture/) 레이어 [캐싱](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/456_caching/)(DLC)의 성공 여부는 '변하지 않는 [파일](/knowledge-base/studynote/02_operating_system/09_file_system/501_file_definition_logical_record/)'을 상단에, '빈번히 변하는 소스코드'를 하단에 배치하는 정밀한 무효화(Invalidation) 순서 설계에 달려 있다.
...
- **📢 섹션 요약 비유**: 빌드 [캐싱](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/456_caching/)이 없는 [CI](/knowledge-base/studynote/12_it_management/02_itsm_itil/874_configuration_item/)/CD는 김치찌개를 끓일 때마다 배추 씨앗을 심고 무를 기르는 것부터 다시 시작하는 식당이다. [캐싱](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/456_caching/) 최적화는 미리 손질된 재료([라이브러리](/knowledge-base/studynote/04_software_engineering/06_software_architecture/336_library_vs_framework/))와 끓여둔 육수(베이스 이미지)를 냉장고에서 꺼내, 바뀐 양념(수정된 코드)만 추가해 1분 만에 찌개를 내놓는 효율적인 주방 시스템이다.
...
- **📢 섹션 요약 비유**: 레이어 [캐싱](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/456_caching/)은 블록 쌓기 게임과 같다. 1층(OS)과 2층([라이브러리](/knowledge-base/studynote/04_software_engineering/06_software_architecture/336_library_vs_framework/))을 단단히 쌓아두고 3층(소스코드)만 뺐다 꼈다 하면 편하지만, 1층이나 2층 블록을 하나라도 건드리면 그 위에 쌓인 모든 층이 와르르 무너져 처음부터 다시 쌓아야 하는 엄격한 규칙이 적용된다.

---

## Ⅲ. 비교 및 연결

빌드 [캐싱](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/456_caching/)은 저장 위치와 공유 범위에 따라 크게 `로컬 레이어 캐시`와 `원격 분산 캐시`로 나뉜다. [파이프](/knowledge-base/studynote/02_operating_system/02_process_thread/123_pipe/)라인 규모에 따라 적합한 [전략](/knowledge-base/studynote/04_software_engineering/04_testing_quality/268_strategy_pattern/)을 연결해야 한다.
...
| **공유 범위** | 해당 머신에서 도는 빌드끼리만 캐시 공유 | <strong>전체 팀, 여러 대의 <a href="/knowledge-base/studynote/12_it_management/02_itsm_itil/874_configuration_item/">CI</a> 워커(Worker) 노드</strong>가 캐시 [파이프](/knowledge-base/studynote/02_operating_system/02_process_thread/123_pipe/)라인을 공유 |
...
| **트레이드오프** | 세팅이 단순하고 [네트워크 지연](/knowledge-base/studynote/03_network/20_performance_evaluation_advanced/1002_network_delay_rtt_oneway_delay_components/)이 없음. 단, 클라우드 [CI](/knowledge-base/studynote/12_it_management/02_itsm_itil/874_configuration_item/) 환경(매번 새 인스턴스 할당)에서는 무용지물임. | 캐시를 다운받는 네트워크 오버헤드가 발생함. 단, 팀원 누군가 한 번 빌드한 내역을 전사가 재사용해 대규모 [CI](/knowledge-base/studynote/12_it_management/02_itsm_itil/874_configuration_item/) 속도를 극대화함. |

현대의 GitHub Actions나 GitLab [CI](/knowledge-base/studynote/12_it_management/02_itsm_itil/874_configuration_item/) 같은 관리형 [CI](/knowledge-base/studynote/12_it_management/02_itsm_itil/874_configuration_item/) 도구는 빌드 머신이 매번 [초기](/knowledge-base/studynote/03_network/08_transport_layer/459_quic_fec_forward_error_correction/)화(Ephemeral)되므로, 로컬 캐시만으로는 한계가 있다. 이 때문에 로컬의 패키지 매니저 디렉토리(`.npm`, `.m2`)를 묶어서 클라우드에 올렸다 내리는 원격 캐시 [전략](/knowledge-base/studynote/04_software_engineering/04_testing_quality/268_strategy_pattern/)(예: `actions/cache`)을 함께 결합하는 것이 필수적이다.

- **📢 섹션 요약 비유**: 로컬 [캐싱](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/456_caching/)이 내 컴퓨터 하드디스크에만 저장해 둔 나만의 '즐겨찾기'라면, 원격 [분산](/knowledge-base/studynote/08_algorithm_stats/08_stats/136_variance/) [캐싱](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/456_caching/)은 회사 전체가 공유하는 중앙 클라우드의 '공용 지식 저장소'다. 내가 자리를 옮기더라도(새 [CI](/knowledge-base/studynote/12_it_management/02_itsm_itil/874_configuration_item/) 노드) 공용 지식을 내려받아 즉시 써먹을 수 있는 것이 원격 캐시의 힘이다.
...
[도커](/knowledge-base/studynote/02_operating_system/01_overview_architecture/063_docker_architecture/) 레이어 [캐싱](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/456_caching/)과 패키지 [캐싱](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/456_caching/) [전략](/knowledge-base/studynote/04_software_engineering/04_testing_quality/268_strategy_pattern/)을 완벽히 융합하면, 15분 이상 걸리던 엔터프라이즈 앱 빌드 시간을 1~2분 단위로 [압축](/knowledge-base/studynote/02_operating_system/06_memory_management/347_compaction/)할 수 있다. 이는 단순히 [CI](/knowledge-base/studynote/12_it_management/02_itsm_itil/874_configuration_item/) 게이지 바가 빨리 차오르는 것을 넘어, 하루 배포 가능 횟수를 수십 번으로 늘려 조직의 비즈니스 민첩성(Agility)을 퀀텀 점프시킨다.

그러나 [캐싱](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/456_caching/)은 "언제 캐시를 날릴 것인가(Invalidation)"를 잘못 설계하면 배포된 앱에 최신 코드가 반영되지 않는 끔찍한 유령 장애를 일으킨다. 기술사는 무작정 모든 것을 [캐싱](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/456_caching/)하는 것이 아니라, 불변성(Immutability)과 해시 기반의 무효화 원리를 정확히 파악하여 인프라 비용 절감과 빌드 정합성을 동시에 달성하는 [신뢰성](/knowledge-base/studynote/04_software_engineering/10_trends_pm_quality/642_reliability_mtbf_mttr_mttf_availability/) 있는 [파이프](/knowledge-base/studynote/02_operating_system/02_process_thread/123_pipe/)라인 아키텍트로 활약해야 한다.

- **📢 섹션 요약 비유**: 잘 튜닝된 빌드 [캐싱](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/456_caching/)은 자동차의 터보 엔진과 같아서 개발팀을 목적지까지 광속으로 보내준다. 하지만 브레이크(캐시 무효화 설계)가 고장 난 터보 엔진은 팀을 엉뚱한 목적지로 처박아버릴 수 있으므로, 속도만큼이나 정밀한 제어가 필수적이다.
...
| **Ephemeral Node** | 클라우드 [CI](/knowledge-base/studynote/12_it_management/02_itsm_itil/874_configuration_item/)/CD 환경에서 빌드 시점에만 잠깐 떴다가 사라지는 일회용 서버 (원격 [캐싱](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/456_caching/)의 필요성을 만듦) |
...
1. [도커](/knowledge-base/studynote/02_operating_system/01_overview_architecture/063_docker_architecture/) 빌드는 레고 성을 1층부터 꼭대기까지 차곡차곡 쌓아 올리는 것과 똑같아요.
2. 빌드 [캐싱](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/456_caching/) 최적화는 1층부터 다시 쌓기 귀찮으니까, 모양이 안 바뀐 1층, 2층 블록 덩어리는 부수지 않고 그대로 가져다 쓰게 놔두는 마법이에요.
