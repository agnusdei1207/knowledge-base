...
현대의 GitHub Actions나 GitLab [CI](/knowledge-base/studynote/12_it_management/02_itsm_itil/090_configuration_item/) 같은 관리형 [CI](/knowledge-base/studynote/12_it_management/02_itsm_itil/090_configuration_item/) 도구는 빌드 머신이 매번 [초기](/knowledge-base/studynote/03_network/08_transport_layer/459_quic_fec_forward_error_correction/)화(Ephemeral)되므로, 로컬 캐시만으로는 한계가 있다. 이 때문에 로컬의 패키지 매니저 디렉토리(`.npm`, `.m2`)를 묶어서 클라우드에 올렸다 내리는 원격 캐시 [전략](/knowledge-base/studynote/04_software_engineering/04_testing_quality/268_strategy_pattern/)(예: `actions/cache`)을 함께 결합하는 것이 필수적이다.

- **📢 섹션 요약 비유**: 로컬 [캐싱](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/456_caching/)이 내 컴퓨터 하드디스크에만 저장해 둔 나만의 '즐겨찾기'라면, 원격 [분산](/knowledge-base/studynote/08_algorithm_stats/08_stats/136_variance/) [캐싱](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/456_caching/)은 회사 전체가 공유하는 중앙 클라우드의 '공용 지식 저장소'다. 내가 자리를 옮기더라도(새 [CI](/knowledge-base/studynote/12_it_management/02_itsm_itil/090_configuration_item/) 노드) 공용 지식을 내려받아 즉시 써먹을 수 있는 것이 원격 캐시의 힘이다.
