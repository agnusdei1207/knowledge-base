...
> 3. **판단 포인트**: Kayenta는 Spinnaker CD에 내장되며, <strong>Judge(판정 엔진)·Canary Config(메트릭 선정)·Score Threshold(합격 기준 점수)</strong>를 설정하여 "95점 이상이면 Promote, 60점 이하면 Rollback" 같은 <strong>자동 의사결정</strong>을 실현한다.
