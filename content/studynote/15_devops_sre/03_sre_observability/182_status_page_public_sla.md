...
또한 상태 [페이지](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/)는 공개 SLA를 현실화한다. 계약서에 "월 99.9% [가용성](/knowledge-base/studynote/01_computer_architecture/13_reliability_power_management/452_availability/)"이 적혀 있어도, 고객이 실제 장애 이력과 공지 품질을 볼 수 없다면 숫자는 추상적이다. 상태 [페이지](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/)는 업타임 히스토리, 유지보수 예고, 장애 타임라인을 공개함으로써 이 약속이 단순 영업 문구가 아님을 보여 준다.
...
- **📢 섹션 요약 비유**: 상태 [페이지](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/)는 놀이공원 고장 안내판과 같다. 어떤 놀이기구가 멈췄고 언제 다시 탈 수 있는지 적혀 있으면, 사람들은 줄마다 직원에게 똑같은 질문을 하지 않아도 된다.

---

## Ⅱ. 아키텍처 및 핵심 원리

좋은 상태 [페이지](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/)는 세 축으로 움직인다. 첫째, [합성 모니터링](/knowledge-base/studynote/15_devops_sre/03_sre_observability/164_synthetic_monitoring_dummy_client/) ([Synthetic Monitoring](/knowledge-base/studynote/15_devops_sre/03_sre_observability/164_synthetic_monitoring_dummy_client/))과 내부 텔레메트리에서 이상을 감지한다. 둘째, 인시던트 커맨더나 온콜 엔지니어가 고객 영향 범위를 사람이 이해할 언어로 바꿔 공지한다. 셋째, 이 정보가 이메일 구독, [웹훅](/knowledge-base/studynote/03_network/09_application_layer_web_email/498_webhook_rest_api_reverse_callback/), 이력 [페이지](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/)를 통해 외부로 전파된다. 즉 상태 [페이지](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/)는 자동화만으로도, 수작업만으로도 완성되지 않는다.
...
| 상태 | 의미 | 고객에게 필요한 [메시](/knowledge-base/studynote/01_computer_architecture/10_parallel_processing_architecture/389_mesh_topology/)지 |
| :--- | :--- | :--- |
| Operational | 정상 | 별도 공지 불필요 |
| Degraded [Performance](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) | [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) 저하 | 느려지는 기능과 우회 방법 안내 |
| Partial Outage | 일부 기능 장애 | 어떤 기능이 영향받는지 명시 |
| Major Outage | 핵심 기능 중단 | [복구](/knowledge-base/studynote/09_security/13_secops_ir_forensics/658_ir_recovery/) 우선순위와 다음 업데이트 시각 공지 |
| Maintenance | 계획 점검 | 시간대, 영향 범위, 준비 사항 안내 |

- **📢 섹션 요약 비유**: 독립된 상태 [페이지](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/)는 건물 밖에 있는 전광판과 같다. 건물 안 전기가 나가도 바깥 전광판은 살아 있어야 사람들이 무슨 일이 일어나는지 알 수 있다.

---

## Ⅲ. 비교 및 연결

상태 [페이지](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/)는 내부 대시보드, 포스트모텀, 공개 SLA와 모두 연결되지만 역할은 다르다. 내부 대시보드는 원인 분석을 위한 기술 정보가 중심이고, 상태 [페이지](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/)는 고객 영향과 다음 행동을 전달하는 것이 중심이다. 포스트모텀은 사건 종료 후 학습 문서이고, 공개 SLA는 그 이력 위에서 고객과 약속하는 계약 문서다.
...
실무에서 상태 [페이지](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/) 운영은 문장력보다 운영 규칙이 중요하다. 장애가 나면 누가 첫 공지를 올리는지, 몇 분 안에 다음 공지를 약속하는지, 어떤 표현은 쓰지 않는지 미리 정해 두어야 한다. 그렇지 않으면 장애마다 공지 품질이 달라지고, [페이지](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/)는 있어도 믿기 어려운 채널이 된다.

| 판단 항목 | 권장 기준 | 흔한 실패 |
| :--- | :--- | :--- |
| 호스팅 분리 | 별도 계정·[도메인](/knowledge-base/studynote/05_database/02_modeling_normalization/064_relation_domain/)·배포 경로 사용 | 주 [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/)와 같은 인프라에 배치 |
| [컴포넌트](/knowledge-base/studynote/04_software_engineering/10_trends_pm_quality/603_component_independent_deployment_unit/) 모델 | 고객이 아는 [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/) 이름 기준 | 내부 [마이크로서비스](/knowledge-base/studynote/04_software_engineering/09_cloud_native_ai_architecture/532_microservices_decomposition_patterns/) 이름을 그대로 노출 |
| 첫 공지 시점 | 주요 장애는 15분 이내 | 원인 파악 후에만 쓰려다 공지 [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/) |
| 업데이트 주기 | 30~60분 간격으로 다음 시각 고정 | 상황이 있을 때만 불규칙 공지 |
| 종료 처리 | [복구](/knowledge-base/studynote/09_security/13_secops_ir_forensics/658_ir_recovery/) 완료 + 후속 포스트모텀 예고 | [복구](/knowledge-base/studynote/09_security/13_secops_ir_forensics/658_ir_recovery/) 후 설명 없이 조용히 종료 |

운영 절차도 단순해야 한다.

1. 장애 탐지와 동시에 인시던트 커맨더를 지정한다.
2. 고객 영향이 [확인](/knowledge-base/studynote/04_software_engineering/12_testing_maintenance/396_validation/)되면 원인이 완벽히 파악되지 않아도 "조사 중" 공지를 먼저 게시한다.
3. [복구](/knowledge-base/studynote/09_security/13_secops_ir_forensics/658_ir_recovery/) 작업 중에는 다음 업데이트 시각을 함께 적어 고객 기대를 관리한다.
4. [복구](/knowledge-base/studynote/09_security/13_secops_ir_forensics/658_ir_recovery/) 후에는 영향 범위, 재발 방지 계획, 포스트모텀 링크를 연결한다.
...
| [컴포넌트](/knowledge-base/studynote/04_software_engineering/10_trends_pm_quality/603_component_independent_deployment_unit/) 모델 | 고객이 어떤 기능이 영향을 받는지 이해하게 하는 구조 |
...
1. 상태 [페이지](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/)는 놀이공원 입구에 있는 "어떤 놀이기구가 멈췄는지" 알려 주는 큰 안내판이에요.
...
← **이전**: [181. SRE (Site Reliability Engineering) 임베디드 운영 모델 (Embedded Model)](/knowledge-base/studynote/15_devops_sre/03_sre_observability/181_sre_embedded_model/)
