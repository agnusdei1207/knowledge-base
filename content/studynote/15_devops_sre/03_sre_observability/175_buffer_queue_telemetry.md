...
> 1. **본질**: Buffer/[Queue](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/058_queue/) Telemetry는 생산자와 소비자의 속도가 만나는 경계 지점에서 backlog, age, drain rate를 관측해, 장애가 사용자에게 드러나기 전에 포화 징후를 읽는 관측 기법이다.
> 2. **가치**: CPU나 에러율이 정상처럼 보여도 큐의 oldest age와 time-to-drain이 늘면 실제 사용자 [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/)이 뒤에서 쌓이고 있음을 조기에 알아낼 수 있다.
...
Buffer/[Queue](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/058_queue/) Telemetry는 시스템 경계의 완충지대를 측정하는 일이다. 여기서 완충지대란 [Kafka](/knowledge-base/studynote/14_data_engineering/04_mlops/179_kafka_flink_watermark_time_window/) 토픽, RabbitMQ 큐, Amazon Simple [Queue](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/058_queue/) [Service](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/) (SQS), [스레드 풀](/knowledge-base/studynote/02_operating_system/02_process_thread/103_thread_pool/) 대기열, [데이터베이스](/knowledge-base/studynote/05_database/01_db_architecture_relational/002_database_definition/) 커넥션 풀, 네트워크 [소켓](/knowledge-base/studynote/02_operating_system/02_process_thread/125_socket/) 버퍼처럼 생산자와 소비자 사이 속도 차이를 흡수하는 모든 구조를 말한다. 평상시에는 이 완충지대가 [스파이크](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/129_spike_agile_technical_investigation/)를 흡수해 주지만, 소비 속도가 구조적으로 밀리기 시작하면 [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/)은 [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/) 내부가 아니라 경계 버퍼에서 먼저 쌓인다.

문제는 개별 [컴포넌트](/knowledge-base/studynote/04_software_engineering/10_trends_pm_quality/603_component_independent_deployment_unit/)만 보면 이 [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/)이 잘 보이지 않는다는 점이다. Producer는 성공적으로 요청을 밀어 넣고, Consumer도 Central Processing Unit (CPU) 사용률 60% 수준에서 멀쩡히 돌아가며, 에러율도 0%일 수 있다. 그런데 그 사이 큐의 oldest age가 계속 늘어나면 사용자는 이미 "처리가 늦는 [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/)"를 경험하게 된다. 즉 경계 버퍼는 시스템의 숨은 대기실이며, 여기의 길이가 늘어나는 순간이 곧 포화의 시작이다.
...
| 지표 | 의미 | 왜 중요한가 |
| :--- | :--- | :--- |
| [Queue](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/058_queue/) Depth | 현재 쌓인 작업 수 | 순간 혼잡을 가장 빠르게 보여 준다. |
| Oldest Age | 가장 오래 기다린 [메시](/knowledge-base/studynote/01_computer_architecture/10_parallel_processing_architecture/389_mesh_topology/)지의 나이 | 사용자 체감 [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/)과 가장 직접적으로 연결된다. |
| [Ingress](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/094_ingress_kubernetes_l7_routing_gateway/) / [Egress](/knowledge-base/studynote/16_bigdata/09_platform/189_egress/) Rate | 초당 유입·소비 속도 | backlog가 일시적 [스파이크](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/129_spike_agile_technical_investigation/)인지 구조적 병목인지 구분한다. |
| [Consumer Lag](/knowledge-base/studynote/16_bigdata/04_streaming/089_consumer_lag/) | 최신 위치와 소비 위치 차이 | [Kafka](/knowledge-base/studynote/14_data_engineering/04_mlops/179_kafka_flink_watermark_time_window/) 같은 스트리밍 시스템에서 처리 뒤처짐을 수치화한다. |
| Retry / Dead Letter [Queue](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/058_queue/) (DLQ) Count | 실패 후 재시도·격리된 [메시](/knowledge-base/studynote/01_computer_architecture/10_parallel_processing_architecture/389_mesh_topology/)지 수 | 독성 [메시](/knowledge-base/studynote/01_computer_architecture/10_parallel_processing_architecture/389_mesh_topology/)지, 다운스트림 오류, 품질 저하를 드러낸다. |
| Pool Wait Time | 커넥션/[스레드](/knowledge-base/studynote/02_operating_system/02_process_thread/092_thread_lwp/) 확보 대기시간 | [메시](/knowledge-base/studynote/01_computer_architecture/10_parallel_processing_architecture/389_mesh_topology/)지 큐가 아니어도 경계 포화를 보여 주는 동일 개념이다. |

이 개념은 [메시지 브로커](/knowledge-base/studynote/07_enterprise_systems/03_eai_esb_msa/145_message_broker_sync_async/)에만 국한되지 않는다. Kafka의 [Consumer Lag](/knowledge-base/studynote/16_bigdata/04_streaming/089_consumer_lag/), SQS의 `ApproximateAgeOfOldestMessage`, RabbitMQ의 `messages_ready`, 자바 [데이터베이스](/knowledge-base/studynote/05_database/01_db_architecture_relational/002_database_definition/) 연결 (Java [Database](/knowledge-base/studynote/05_database/04_transactions_concurrency/501_database/) Connectivity, JDBC) 커넥션 풀의 pending borrower 수, 웹서버 [스레드 풀](/knowledge-base/studynote/02_operating_system/02_process_thread/103_thread_pool/) 대기열은 모두 같은 질문을 한다. "일이 들어오는 속도와 처리하는 속도 사이에 얼마나 기다림이 쌓였는가?"가 공통 본질이다.
...
| 응답시간 | 이미 노출된 사용자 [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/) | 비동기 [파이프](/knowledge-base/studynote/02_operating_system/02_process_thread/123_pipe/)라인의 숨은 적체 |
...
실무에서는 고정 [임계치](/knowledge-base/studynote/03_network/08_transport_layer/431_ssthresh_slow_start_threshold/) 하나로 끝내면 안 된다. 큐 깊이 [10](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/489_raid_10_hybrid/),000건이 위험한지 아닌지는 소비자 처리율, [메시](/knowledge-base/studynote/01_computer_architecture/10_parallel_processing_architecture/389_mesh_topology/)지 크기, [파티션](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/) 수, 허용 [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/)에 따라 완전히 달라진다. 그래서 알람은 depth 단독보다 `age`, `rate of growth`, `time-to-drain`, `DLQ`, `per-partition skew`를 함께 봐야 한다.
...
Buffer/[Queue](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/058_queue/) Telemetry를 잘 구축하면 비동기 시스템의 사각지대가 사라진다. [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/) 경계에서 쌓이는 [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/)을 사용자 불만이나 [타임아웃](/knowledge-base/studynote/04_software_engineering/09_cloud_native_ai_architecture/573_timeout_retry_backoff_strategy/)이 생기기 전에 발견할 수 있고, Autoscaling과 Backpressure도 훨씬 근거 있게 설계할 수 있다. 특히 [이벤트 기반 아키텍처](/knowledge-base/studynote/04_software_engineering/09_cloud_native_ai_architecture/538_event_driven_architecture_eda/)에서는 이 지표가 없으면 "어디서 느려지는지"를 추측에 의존하게 된다.
...
| 개념 | 연결 포인트 |
| :--- | :--- |
| [Consumer Lag](/knowledge-base/studynote/16_bigdata/04_streaming/089_consumer_lag/) | 스트리밍 소비자가 생산 속도를 따라가지 못하는 정도를 수치화한다. |
| Oldest Message Age | 사용자 체감 [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/)과 가장 직접적으로 닿는 선행 지표다. |
| Backpressure | 소비 능력보다 유입이 많을 때 생산 속도를 조절하는 제어 [전략](/knowledge-base/studynote/04_software_engineering/04_testing_quality/268_strategy_pattern/)이다. |
| DLQ (Dead Letter [Queue](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/058_queue/)) | 적체가 아니라 처리 실패가 누적되고 있음을 보여 주는 경계 [신호](/knowledge-base/studynote/02_operating_system/02_process_thread/130_signal/)다. |
| Autoscaling | backlog와 drain rate를 기준으로 소비자를 동적으로 확장하는 기법이다. |
| Microburst | 짧지만 큰 유입 [스파이크](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/129_spike_agile_technical_investigation/)가 일시적으로 backlog를 키우는 현상이다. |
| Little's Law | 대기 중 작업 수, 도착률, 대기시간의 [관계](/knowledge-base/studynote/05_database/02_modeling_normalization/083_relationship_in_er_model/)를 해석하는 기본 직관을 준다. |
