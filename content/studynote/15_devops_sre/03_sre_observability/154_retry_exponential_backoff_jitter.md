...
## 핵심 인사이트 (3줄 요약)
> 1. **본질**: [분산](/knowledge-base/studynote/08_algorithm_stats/08_stats/136_variance/) [마이크로서비스](/knowledge-base/studynote/04_software_engineering/09_cloud_native_ai_architecture/532_microservices_decomposition_patterns/)([MSA](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/619_msa_traffic_hardware/)) 환경에서 타겟 서버가 뻗어 [타임아웃](/knowledge-base/studynote/04_software_engineering/09_cloud_native_ai_architecture/573_timeout_retry_backoff_strategy/) 에러를 뱉었을 때 ➔ 걍 대충 "1초 뒤에 무조건 다시 찔러봐 ㅋ" 무지성 폭격(Retry)을 치면 좀비 트래픽 디도스(DDoS)가 터져 서버가 영구 멸망하므로!! ➔ **재시도 간격을 $2초 ➔ 4초 ➔ 8초$ 로 기하급수적으로 숨통 틔워 늦추고(Exponential Backoff), 거기에 난수 타이머 0.3초 꼼수(Jitter)를 섞어 흩뿌려 쏘는 [SRE](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/100_sre_site_reliability_engineering_error_budget/) 트래픽 튜닝 텐트의 절대 헌법**이다.
> 2. **가치**: 10만 명의 유저 봇(Bot)들이 뻗어있는 결제 서버를 향해 [정확히 1.00초 뒤에 다 같이 동시 일제 사격 핑퐁 폭격 💥] 을 날려 ➔ 서버가 부팅될 틈도 없이 다시 대가리 깨져 무한 타죽음 뻗음(Retry Storm 재시도 폭풍 💀) 에 갇히는 파멸적 도미노 악순환을 ➔ **시간 축 위로 10만 개 트래픽을 나노 조각으로 갈기갈기 찢어발겨 흩뿌림(Decoupling 스텔스 [분산](/knowledge-base/studynote/08_algorithm_stats/08_stats/136_variance/))으로써 100% 무결점 오토 힐링 회생 쾌속 부활 시간을 벌어다 주는 구원 투수 무적 쉴드막**이다.
> 3. **판단 포인트**: 이 흑마법 텐트를 칠 때 아키텍트는 철의 룰을 세워야 한다! "야 404 (경로 없음), 400 (문법 에러) 같은 영구 불변 쇳덩이 팩트 에러는 재시도 백날 쳐 해봐야 안 고쳐지니 1방 컷 드랍 소각 사살 쳐 쾅 🪓!! 무.조.건. **[503 (일시 [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/) 불능), 429 (Rate Limit 트래픽 초과)] 같은 찰나의 네트워크 랙 [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/)(Transient Error) 에러 찌끄레기에만 핀셋 록온([Lock](/knowledge-base/studynote/05_database/04_transactions_concurrency/510_lock/)-on) 타격 쳐서 재시도 스위칭 핑퐁 쉴드를 올려라 🚀!**"
...
**[아키텍트의 피 터지는 메스: 3대 지터(Jitter) [알고리즘](/knowledge-base/studynote/08_algorithm_stats/01_basics/001_algorithm_definition/) 수학 칼춤 수술 🪓]**
"야 걍 대충 랜덤 박으면 안 됨 ㅋ?"
1. **Full Jitter (우주 1타 극강 밸런스 추천 🚀)**: `random(0, min(cap, base * 2^n))` ➔ 계산된 백오프 최댓값 안에서 그냥 무지성 0초~최대초 사이 아무 랜덤 숫자나 뽑아 쏘는 짓. 트래픽 [분산](/knowledge-base/studynote/08_algorithm_stats/08_stats/136_variance/) 효과가 우주 최강 1등으로 예쁘게 넓게 퍼져 AWS가 공식으로 1타 꼽는 표준 쉴드 텐트.
...
- **📢 섹션 요약 비유**: 고정 간격 재시도(Fixed Retry) 파국 멸망은, 마트 오픈 런(오전 10시 정각 💥)에 **'대기 줄 손님 1,000명이 정각 땡 치자마자 1개 좁은 유리 셔터 문구멍으로 동시에 우당탕 어깨빵 치며 돌격 비집고 들어가다 ➔ 문 유리 박살 나고 압사 동반 타죽음 셧다운 뻗음 💀'**과 100% 똑같습니다. 파도처럼 쾅 칩니다. 
...
### 딜레마: 찌를 놈과 즉결 사살 버릴 놈의 핀셋 록온 구분 ([Idempotency](/knowledge-base/studynote/15_devops_sre/04_iac_cloud_native/194_idempotency/) & Status [Code](/knowledge-base/studynote/02_operating_system/02_process_thread/082_process_memory_structure/))

| [HTTP](/knowledge-base/studynote/03_network/09_application_layer_web_email/461_http_stateless_connection_oriented/) 쇳덩이 상태 코드 | 에러 성격 파악 (What is this? 👁️) | 아키텍트의 파멸과 튜닝 록온 🪓 (재시도 쉴드 룰) |
|:---|:---|:---|
| **`429 Too Many Requests`** | **[일시적 찰과상 랙 💥]** "야 너 트래픽 한도(Rate Limit) 초과해서 내 방패에 막혔음. 좀 이따 와 ㅋ." | **[지터 섞어 재시도 무조건 록온 🚀]** 단, 서버가 헤더에 뱉어준 `Retry-After: 5` (5초 뒤에 와 새끼야) 텍스트 명세 헌법을 100% 무조건 스캔 준수해서 5초 뒤에 우회 지터 섞어 찔러 텐트 쳐 쾅!! |
| **`503 Service Unavailable`** | **[일시적 과부하 뻗음 💥]** "아 ㅆㅂ 나 지금 배포(Deploy) 중이거나 잠시 램 터져서 기절했음 1분만 기달 ㅠ." | **[지수 백오프 텐트 록온 🚀]** 1초 ➔ 3초 ➔ 7초 넉넉히 서버 심호흡 부팅 [회복](/knowledge-base/studynote/05_database/04_transactions_concurrency/233_recovery_database_restoration_overview/) 숨통 시간 열어주며 핑퐁 찔러 인공호흡 쳐 생존 돌파. |
| **`504 Gateway Timeout`** | **[일시적 핑 미아 💀]** "야 내가 찔렀는데 인터넷 허공 랜선에서 짤려 응답이 안 옴 좆망 ㅋ." | **[멱등성([Idempotency](/knowledge-base/studynote/15_devops_sre/04_iac_cloud_native/194_idempotency/)) 절대 검열 사살 방폭문 🛡️]** 야 씨발 잠깐 멈춰 쾅!! 결제 [API](/knowledge-base/studynote/02_operating_system/01_overview_architecture/014_api_posix/) 찔렀는데 [타임아웃](/knowledge-base/studynote/04_software_engineering/09_cloud_native_ai_architecture/573_timeout_retry_backoff_strategy/) 난 거면 ➔ 상대방은 돈 빼갔는데 응답 텍스트만 내게 오다 끊긴 거일 수도 있어 💀!! 걍 무지성 2번 재시도 때리면 유저 통장 돈 2번 이중 과금 폭파 멸망 파산 소송 터짐 쾅 🪓!! **[하늘이 두 쪽 나도 "이 주문번호 123번 재시도요 ㅋ" [멱등성](/knowledge-base/studynote/13_cloud_architecture/04_devops_observability/171_idempotency_iac_terraform/) 키([Key](/knowledge-base/studynote/05_database/02_modeling_normalization/067_db_key_uniqueness_minimality/)) 텍스트를 헤더에 쑤셔 박아 무결점 스캔 검열 [확인](/knowledge-base/studynote/04_software_engineering/12_testing_maintenance/396_validation/) 후 찔러 록온 박아 🚀!!]** |
| **`400 Bad Request` / `401 Unauthorized`** | **[영구 불변 팩트 에러 쓰레기 💀]** "니가 보낸 [JSON](/knowledge-base/studynote/11_design_supervision/06_exam_summary/343_json/) 문법 괄호 틀렸거나 비번 틀림 ㅂㅅ아 컷 ㅋ." | **[절.대. 재시도 금지 영구 차단 사살 컷 🪓!!!]** 야 이 미친놈아 문법 틀린 쓰레기 텍스트를 백날 1,000번 다시 쏜다고 서버가 "아 정답으로 봐줄게 ㅋ" 하고 뚫어 주냐 미친아 쾅!! 이거 재시도 치면 니 [스레드](/knowledge-base/studynote/02_operating_system/02_process_thread/092_thread_lwp/) 대기 랙만 타죽고 네트워크 쓰레기 트래픽 디도스 낭비 연쇄 뻗음이다 당장 재시도 0회 세팅 Fail Fast 즉사 차단 드랍 쳐 🚀!!! |

**[🚨 아키텍트의 피 터지는 메스: [서킷 브레이커](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/307_circuit_breaker_pattern/)([Circuit Breaker](/knowledge-base/studynote/12_it_management/05_security_compliance/304_circuit_breaker/)) 와의 영혼 십자 융합 ✨]**
...
2. **[서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/) 전체 재시도 예산 (Retry Budget) 글로벌 통치 쿼터제 쉴드 🛡️**: 
...
- **📢 섹션 요약 비유**: 이 재시도 예산(Retry Budget) 쿼터 한도 텐트는, **'식당 주방장의 [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/)(재시도) 볶음밥 리미트 제한 룰'**과 100% 똑같습니다. 볶음밥이 탔을 때 1번 다시 볶아주는(Retry) 건 착한 마법입니다. 근데 볶음밥 시킨 100명 손님 요리가 싹 다 불타서 까맣게 망했습니다(대형 장애 뻗음 💥). 주방장이 100그릇을 몽땅 미친 듯이 싹 다 1부터 다시 웍질 돌려 새로 볶으려(100% 무지성 재시도 💀) 덤비면? 주방장 팔 빠져 타 죽고 [가스](/knowledge-base/studynote/06_ict_convergence/01_blockchain/024_gas/)레인지 폭발 식당 파산 화재 멸망 납니다 쾅!! 
아키텍트 매니저는 룰을 박습니다 🪓. **"야! 하루 전체 요리 100그릇 중에 ➔ 10그릇([10](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/489_raid_10_hybrid/)% 예산 록온 ✨)까지만 다시 볶아주는(Retry) 공짜 웍질 수리 허용할게 락킹 쾅!! 근데 10그릇 한도 넘어가는 순간? 야 시발 웍질 당장 중단 멈춰 컷!! 뒤에 남은 손님 90명한텐 걍 '오늘 밥 다 떨어짐 ㅈㅅ 빠이 퉤 💥(즉각 에러 튕겨내기 Fail Fast)' 선언 치고 돌려보내 환불 차단 방어막 내려 찍어 쾅 🚀!!"** 
...
| :--- | :--- |
| **Retry Storm (재시도 폭풍 쓰나미 파국 💀)** | 이 헌법이 탄생한 0순위 척살 타겟 이유. B 서버가 과부하 랙 뻗어 [타임아웃](/knowledge-base/studynote/04_software_engineering/09_cloud_native_ai_architecture/573_timeout_retry_backoff_strategy/) 기침 뱉었는데 ➔ 앞단 A 서버 코더가 0.1초마다 1만 대 무지성 즉시 재시도 풀악셀 폭격 때려 박아버려서 ➔ B 서버가 영원히 부팅 숨 못 쉬고 대갈통 박살 타 죽어 100% 올스탑 마비 셧다운 멸망 연쇄 타살 캐스케이딩 터지는 최악의 셀프 팀킬 봇 버그 지옥. |
| **Exponential Backoff (지수 백오프 텐트 🚀)** | 재시도 폭풍 멸망을 찢어발긴 1차 방폭문 마법. 1차 실패 시 1초 ➔ 2차 실패 시 2초 ➔ 3차 4초 ➔ 8초로 대기 시간 랙 틈새를 2배씩 기하급수적으로 쫙쫙 늘려 강제 벌려 놓음 쾅! ➔ 상대 서버 쇳덩이가 램 불타는 거 식히고 오토 힐링 재부팅 부활 쾌속 [복구](/knowledge-base/studynote/09_security/13_secops_ir_forensics/658_ir_recovery/)할 숨통 여유 시간을 무결점 확보 배려해 주는 인프라 힐링 쉴드 ✨. |
| **Jitter (지터 꼼수 난수 흩뿌리기 믹서기 🎲)** | 1차 백오프 텐트 쳤는데도 ➔ "어 시발 1,000명 봇 새끼들이 다 같이 공식 계산 똑같이 쳐서 정확히 4.00초 정각에 또 다구리 동시 폭격 치네 좆망 💀 ([동기화](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/212_synchronization_mechanisms/) 파도 Thundering Herd 파국 💥)" ➔ 아키텍트가 난수(Random ±0.5초 오차) 주사위 주입 록온 쳐서 ➔ 1,000명의 폭격 타점을 0.1초 찰나 단위로 잘게 잘게 나노 찢어 흩뿌려(Decoupling 스텔스) 스무스 가랑비 [분산](/knowledge-base/studynote/08_algorithm_stats/08_stats/136_variance/) 핑퐁 패스 통과 생존을 이룩해 낸 2차 우주 대관식 필살기. |
| **[Idempotency](/knowledge-base/studynote/15_devops_sre/04_iac_cloud_native/194_idempotency/) ([멱등성](/knowledge-base/studynote/13_cloud_architecture/04_devops_observability/171_idempotency_iac_terraform/) 부적 쉴드 락킹 🔑)** | 재시도를 쏠 때 앞단 무조건 깔려있어야 하는 0순위 전제 조건 방벽 헌법. 내가 [타임아웃](/knowledge-base/studynote/04_software_engineering/09_cloud_native_ai_architecture/573_timeout_retry_backoff_strategy/) 뻗어 에러 터져서 '결제 재시도' 핑퐁 3번 날렸는데 ➔ 상대방이 3번 다 받아쳐 먹고 유저 계좌 돈 3번 이중 출금 깎아 파산 멸망 폭사 100억 소송 터지는 걸 100% 막으려면 ➔ 헤더 대가리에 `[주문 123번 고유 텐트 ID]` 멱살 잡아 쑤셔 박아 던져 "야 나 아까 온 새끼야 중복 컷 무시 쳐 쾅!" 방폭문 검열 걸러내는 [제로 트러스트](/knowledge-base/studynote/02_operating_system/10_security/667_zero_trust_runtime_integrity_measurement/)([Zero Trust](/knowledge-base/studynote/02_operating_system/10_security/667_zero_trust_runtime_integrity_measurement/)) 무결점 텐트. |
| **[Circuit Breaker](/knowledge-base/studynote/12_it_management/05_security_compliance/304_circuit_breaker/) ([서킷 브레이커](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/307_circuit_breaker_pattern/) 방폭문 ⚡)** | 재시도의 영혼 단짝 십자 융합 형님 쉴드. "야 3번 지터 섞어 찔렀는데 계속 뻗네? 이 새끼 진짜 불타 뒈진 팩트 록온 쾅 💥!" ➔ 그 0.1초 순간 인간 묻지 말고 즉각 [스위치](/knowledge-base/studynote/03_network/05_lan_wan_l2_devices/238_switch_operation_principles/) 찰칵 오픈(OPEN) 차단 내려 [파이프](/knowledge-base/studynote/02_operating_system/02_process_thread/123_pipe/) 100% 절단 고립 찢어버리고 ➔ 걍 내 [스레드](/knowledge-base/studynote/02_operating_system/02_process_thread/092_thread_lwp/) 나 홀로 Fail Fast 쾌속 튕겨내기 방어 기동 쳐서 램 뻗음 파국 차단 척살해 생존하는 최후의 인프라 [백업](/knowledge-base/studynote/02_operating_system/09_file_system/555_backup_and_restore_strategy/) 에어백 생명 연장술. |
...
1. 내가 좋아하는 아이돌 콘서트 티켓 예매 사이트가 터져서 먹통 에러가 났을 때, 나랑 10만 명 팬들이 화난다고 **키보드 F5(새로고침 재시도 Retry 💥)** 버튼을 0.1초마다 미친 듯이 다 같이 연타 폭격 치면 ➔ 안 그래도 아픈 예매 서버가 10만 명 다구리 주먹 디도스 쳐 맞고 평생 [복구](/knowledge-base/studynote/09_security/13_secops_ir_forensics/658_ir_recovery/) 불가 뻗어 죽어버리는 대형 사고(재시도 폭풍 파국 💀)가 터져요 ㅠ.
2. 그래서 똑똑한 천재 매니저(아키텍트 대장님 🪓)가 마법의 룰을 정했어요! **"야!! 에러 나면 처음엔 1초 기다렸다가 F5 누르고, 또 안 되면 2초, 그다음엔 4초, 8초 대기 타며 점점 뒤로 물러서서 기다려(지수 백오프 텐트 🚀)!! 서버 아저씨가 물 마시고 기계 고치고 숨통 쉴 수 있게 배려 락킹([Lock](/knowledge-base/studynote/05_database/04_transactions_concurrency/510_lock/)) 치란 말이야 쾅!!"**
3. 근데 10만 명이 똑같이 타이머 보고 4.0초 정각에 다 같이 F5 연타 치면 또 서버 뻗으니까 💥 ➔ **"야 주사위 굴려서 넌 3.8초에, 넌 4.2초에 각자 알아서 시간 다르게 랜덤 섞어서(지터 Jitter 흩뿌리기 ✨) 미세하게 흩어져서 새로고침 눌러 쾅 🚀!!"** 이렇게 10만 명의 클릭 펀치를 나노 단위로 쪼개 부드럽게 [분산](/knowledge-base/studynote/08_algorithm_stats/08_stats/136_variance/)시켜 서버 100% 무결점 생존 부활 쾌속 패스를 돕는 가장 똑똑하고 위대한 배려 양보 인터넷 통치술이랍니다!
...
← **이전**: [153. 서킷 브레이커 (Circuit Breaker) - 마이크로서비스 연쇄 타살 붕괴를 끊는 절대 차단기](/knowledge-base/studynote/15_devops_sre/03_sre_observability/153_circuit_breaker_state_machine/)
**다음**: [155. 타임아웃 (Timeout) 동기화 전략](/knowledge-base/studynote/15_devops_sre/03_sre_observability/155_timeout_synchronization_strategy/) →
