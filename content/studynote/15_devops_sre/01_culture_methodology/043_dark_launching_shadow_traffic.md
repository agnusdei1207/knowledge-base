...
> 2. 섀도우 트래픽은 프로덕션 요청을 [[016_replication_factor|복제]]하여 신규 [[090_service_kubernetes_network_load_balancing|서비스]]로 전달하고 응답을 비교(Shadow Comparison)하지만 사용자에게는 기존 [[090_service_kubernetes_network_load_balancing|서비스]]의 응답만 반환하므로 — [[001_dikw_pyramid|데이터]] 변조 가능성이 있는 [[289_cqrs_db|쓰기]](POST/PUT/DELETE) 요청에 대해서는 신중한 격리 처리가 필수이다.
> 3. Facebook, Netflix, Google 등 대규모 [[090_service_kubernetes_network_load_balancing|서비스]]가 다크 런칭을 통해 새 [[532_microservices_decomposition_patterns|마이크로서비스]] 전환 전 실제 트래픽 부하를 사전 [[395_verification_process_review|검증]]하는 것은 — SRE의 핵심 원칙인 "테스트 환경은 프로덕션과 같아야 한다"를 실현하는 가장 현실적인 방법임을 보여준다.
...
> 📢 **섹션 요약 비유**: 섀도우 트래픽은 복사본 채점 — 원본 시험지(기존 [[090_service_kubernetes_network_load_balancing|서비스]])로 점수를 주고, 복사본(신규 [[090_service_kubernetes_network_load_balancing|서비스]])도 동시에 채점해서 결과 비교. 학생(사용자)에게는 원본 점수만 알려줌.
...
> 📢 **섹션 요약 비유**: 다크 런칭 사례는 새 항공 관제 [[405_system_test|시스템 테스트]] — 실제 비행기들이 날면서(프로덕션 트래픽), 새 관제 시스템을 그림자 훈련(섀도우). 실제 관제는 기존 시스템이 담당.

---

## Ⅳ. [[123_pipe|파이프]]라인 통합
...
> 📢 **섹션 요약 비유**: 다크 런칭 [[123_pipe|파이프]]라인은 비행기 시험 비행 순서 — 격납고 점검([[090_configuration_item|CI]]/CD) → 활주로 주행(스테이징) → 저고도 비행(다크 런칭) → 정식 운항(100% 배포).

---

## Ⅴ. 실무 시나리오 — 결제 [[090_service_kubernetes_network_load_balancing|서비스]] 마이그레이션
...
> 📢 **섹션 요약 비유**: 결제 [[090_service_kubernetes_network_load_balancing|서비스]] 섀도우는 수술 전 모의 훈련 — 실제 환자(사용자)에게는 기존 집도의(v1)가 수술하고, 새 의사(v2)는 옆에서 같은 수술을 동시에 연습. 판단 오류 발견 후 실전 투입.
...
3. GET만 [[016_replication_factor|복제]]하고 POST는 절대 안 돼 — "주문 내역 조회"는 복사해도 되지만, "결제 실행"을 두 번 하면 돈이 두 번 빠져나가요!
...
← **이전**: [[042_ab_testing_hypothesis_validation|042. A/B 테스팅 가설 검증]]
**다음**: [[044_tdd_bdd_test_driven_behavior_driven_development|044. TDD & BDD — 테스트 주도 개발]] →
