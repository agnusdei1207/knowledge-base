...
│ │
│ 문제: │
│ - 확장에 한계 (Vertical Scaling은 비용이수에) │
│ - 단일 장애점 (SPOF) │
│ - 리소스 활용 비효율 │
└─────────────────────────────────────────────────────────────┘
...
|:---|:---|:---|
| **[컨테이너](/knowledge-base/studynote/04_software_engineering/09_cloud_native_ai_architecture/561_container_based_deployment/) ([Docker](/knowledge-base/studynote/02_operating_system/01_overview_architecture/063_docker_architecture/))** | [컨테이너](/knowledge-base/studynote/04_software_engineering/09_cloud_native_ai_architecture/561_container_based_deployment/) 격리로 프로세스 수준 동시성 | 자원 격리 + 유연한 확장 |
...
| **비동기 [메시](/knowledge-base/studynote/01_computer_architecture/10_parallel_processing_architecture/389_mesh_topology/)징** | 워커 프로세스가 백그라운드 작업 처리 | 요청/응답과작업 분리 |
...
**1. 실무 의사결정 시나리오**
- **시나리오 A:CPU를 많이 사용하는 작업(처리)과 I/O를 많이 사용하는 작업([API](/knowledge-base/studynote/02_operating_system/01_overview_architecture/014_api_posix/) 호출)이 동일한 프로세스에서 수행되어 병목이 발생하는 상황**
...
- **시나리오 B: 백그라운드 작업(이메일 전송, [로그](/knowledge-base/studynote/04_software_engineering/09_cloud_native_ai_architecture/568_logs_distributed_logging_elk_fluentd/) 처리)이 웹 요청 처리와재되어 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) 저하가 발생하는 상황**
