...
> 1. **본질**: 부패 방지 레이어 ([ACL](/knowledge-base/studynote/02_operating_system/09_file_system/549_acl_access_control_list/), Anti-Corruption Layer)는 DDD의 전략적 통합 패턴으로, 외부 시스템·레거시 시스템·다른 [바운디드 컨텍스트](/knowledge-base/studynote/04_software_engineering/04_testing_quality/221_bounded_context_ddd_msa_boundary/)의 [도메인](/knowledge-base/studynote/05_database/02_modeling_normalization/064_relation_domain/) 모델이 자신의 [도메인](/knowledge-base/studynote/05_database/02_modeling_normalization/064_relation_domain/) 모델을 '오염(corrupt)'시키지 못하도록 경계에 변환·번역 레이어를 두어 내부 [도메인](/knowledge-base/studynote/05_database/02_modeling_normalization/064_relation_domain/) 모델의 순수성을 보호한다.
> 2. **가치**: ACL을 통해 외부 시스템 변경([API](/knowledge-base/studynote/02_operating_system/01_overview_architecture/014_api_posix/) 스펙 변경, 필드명 변경)이 내부 [도메인](/knowledge-base/studynote/05_database/02_modeling_normalization/064_relation_domain/) 모델에 직접 전파되지 않으며, 외부 모델에 대한 의존성이 [ACL](/knowledge-base/studynote/02_operating_system/09_file_system/549_acl_access_control_list/) 경계 내에 격리되어 내부 팀이 외부 변화에 독립적으로 개발할 수 있다.
> 3. **판단 포인트**: ACL은 단순히 외부 API를 호출하는 클라이언트 코드가 아니다. 외부 개념을 내부 [도메인](/knowledge-base/studynote/05_database/02_modeling_normalization/064_relation_domain/) 언어로 번역하는 [Facade](/knowledge-base/studynote/04_software_engineering/04_testing_quality/263_facade_pattern_simplified_interface/) + [Adapter](/knowledge-base/studynote/04_software_engineering/04_testing_quality/259_adapter_pattern_interface_wrapper/) + Translator의 조합이며, 외부 모델을 내부 모델로 변환하는 명시적 번역 로직이 핵심이다.

---

## Ⅰ. 개요 및 필요성

소프트웨어 시스템은 반드시 외부 시스템과 통합해야 하는데, 외부 시스템의 [도메인](/knowledge-base/studynote/05_database/02_modeling_normalization/064_relation_domain/) 모델이 내부 시스템과 다를 때 문제가 발생한다. 외부 결제사(PG)가 'paymentCode'를 사용하지만 내부 [도메인](/knowledge-base/studynote/05_database/02_modeling_normalization/064_relation_domain/)은 'paymentId'를 사용한다면, ACL이 없는 경우 내부 코드 전체에 외부 개념이 스며들어 [도메인](/knowledge-base/studynote/05_database/02_modeling_normalization/064_relation_domain/) 모델이 오염된다.
...
[ACL](/knowledge-base/studynote/02_operating_system/09_file_system/549_acl_access_control_list/) 구현 패턴은 [Facade](/knowledge-base/studynote/04_software_engineering/04_testing_quality/263_facade_pattern_simplified_interface/) + [Adapter](/knowledge-base/studynote/04_software_engineering/04_testing_quality/259_adapter_pattern_interface_wrapper/) + Translator의 조합이다. Facade는 외부 시스템의 복잡한 API를 단순한 인터페이스로 감추고, Adapter는 인터페이스를 맞춰주며, Translator는 외부 모델을 내부 [도메인](/knowledge-base/studynote/05_database/02_modeling_normalization/064_relation_domain/) 모델로 변환한다.

| 항목 | 설명 | 포인트 |
|:---|:---|:---|
| [Facade](/knowledge-base/studynote/04_software_engineering/04_testing_quality/263_facade_pattern_simplified_interface/) | 외부 복잡성 은닉 | PaymentGatewayFacade |
| [Adapter](/knowledge-base/studynote/04_software_engineering/04_testing_quality/259_adapter_pattern_interface_wrapper/) | 인터페이스 변환 | ExternalOrderAdapter |
...
ACL의 가장 어려운 실무 과제는 외부 시스템이 자주 변경될 때 [ACL](/knowledge-base/studynote/02_operating_system/09_file_system/549_acl_access_control_list/) 유지보수 비용이다. 외부 [API](/knowledge-base/studynote/02_operating_system/01_overview_architecture/014_api_posix/) [버전](/knowledge-base/studynote/03_network/06_network_layer_ip/288_version_ihl_tos_total_length/)업 시 [ACL](/knowledge-base/studynote/02_operating_system/09_file_system/549_acl_access_control_list/) 내의 번역 로직만 수정하면 내부 [도메인](/knowledge-base/studynote/05_database/02_modeling_normalization/064_relation_domain/) 코드는 수정하지 않아도 된다는 것이 ACL의 핵심 가치다.
...
[ACL](/knowledge-base/studynote/02_operating_system/09_file_system/549_acl_access_control_list/) 적용으로 외부 시스템 변경이 내부 [도메인](/knowledge-base/studynote/05_database/02_modeling_normalization/064_relation_domain/)에 전파되지 않아 안정적인 내부 모델이 유지되고, 외부 시스템을 Mock으로 대체하여 독립적인 [도메인](/knowledge-base/studynote/05_database/02_modeling_normalization/064_relation_domain/) 테스트가 가능해진다.
...
| 개념 | 연결 포인트 |
|:---|:---|
| [바운디드 컨텍스트](/knowledge-base/studynote/04_software_engineering/04_testing_quality/221_bounded_context_ddd_msa_boundary/) | ACL이 BC 경계에서 외부 모델을 차단 |
| [파사드 패턴](/knowledge-base/studynote/11_design_supervision/03_gof_creational_structural/156_facade_pattern/) | [ACL](/knowledge-base/studynote/02_operating_system/09_file_system/549_acl_access_control_list/) 구현의 외부 복잡성 은닉 |
| [어댑터 패턴](/knowledge-base/studynote/11_design_supervision/06_exam_summary/383_adapter_pattern_summary/) | [ACL](/knowledge-base/studynote/02_operating_system/09_file_system/549_acl_access_control_list/) 구현의 인터페이스 변환 |
| [스트랭글러 피그](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/310_strangler_fig_pattern/) | 레거시 통합 시 ACL을 함께 사용 |
...
1. ACL은 외국 음식(외부 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/))을 우리 입맛(내부 모델)에 맞게 조리해주는 요리사예요.
2. 외국 레시피(외부 [API](/knowledge-base/studynote/02_operating_system/01_overview_architecture/014_api_posix/))가 바뀌어도 우리 메뉴(내부 [도메인](/knowledge-base/studynote/05_database/02_modeling_normalization/064_relation_domain/))는 그대로 유지돼요.
