...
> 2. **가치**: CPU [[092_thread_lwp|스레드]]가 I/O 대기 없이 다른 작업을 계속 처리할 수 있어 고성능 네트워크 서버에서 극단적인 [[139_throughput|처리량]]([[139_throughput|Throughput]])을 달성한다.
> 3. **판단 포인트**: Reactor (리액터)는 "I/O 준비됨(Readiness)" 신호를 받아 앱이 직접 I/O를 수행하지만, Proactor는 "I/O 완료(Completion)" 신호를 받아 결과 [[001_dikw_pyramid|데이터]]를 바로 처리한다 — OS가 I/O를 대리 수행하느냐의 차이가 핵심이다.

---

## Ⅰ. 개요 및 필요성
전통적인 동기 서버에서는 클라이언트 요청 1개당 [[092_thread_lwp|스레드]] 1개를 할당한다. 연결이 1만 개라면 1만 개의 [[092_thread_lwp|스레드]]가 필요하고, 대부분은 I/O를 기다리며 블록(Block)된 채 CPU를 낭비한다. 이를 C10K 문제(C10K Problem, 동시 [[489_raid_10_hybrid|10]],000 클라이언트 처리 문제)라고 부른다.
...
Proactor 패턴의 핵심 동기:
- **[[092_thread_lwp|스레드]] 절감**: 소수의 [[092_thread_lwp|스레드]]([[103_thread_pool|Thread Pool]])로 수천~수만 연결 처리
- **CPU 효율**: [[092_thread_lwp|스레드]]가 I/O 대기 없이 Completion Handler 실행에만 집중
- **OS 위임**: Windows IOCP (I/O Completion [[446_port_and_bus|Port]], 입출력 완료 [[446_port_and_bus|포트]]), Linux `io_uring` 등 OS 레벨 비동기 I/O 활용

| 관점 | Reactor (리액터) | Proactor (프로액터) |
|:---|:---|:---|
| 이벤트 시점 | I/O 준비(Readiness) | I/O 완료(Completion) |
| I/O 수행 주체 | 애플리케이션(App) | OS [[022_kernel_role|커널]]([[022_kernel_role|Kernel]]) |
...
| Asynchronous [[329_delta_encoding|Operation]] (비동기 오퍼레이션) | OS가 수행할 실제 I/O 작업 (read/write) |
| Async Op Processor (비동기 오퍼레이션 프로세서) | OS [[022_kernel_role|커널]] — I/O를 실제로 수행 |
| Completion Event [[058_queue|Queue]] (완료 이벤트 큐) | OS가 완료된 I/O 결과를 쌓아두는 큐 (IOCP 큐) |
| Proactor (프로액터) | 완료 큐에서 이벤트를 꺼내 Completion Handler에 디스패치 |
| Completion Handler (완료 핸들러) | 비즈니스 로직 수행 (읽은 [[001_dikw_pyramid|데이터]] 처리 등) |
...
## Ⅲ. 비교 및 연결
| I/O 모델 | 블로킹 여부 | I/O 수행 주체 | 알림 시점 | 대표 OS [[014_api_posix|API]] |
|:---|:---|:---|:---|:---|
| [[122_sync_async_communication|Blocking]] I/O | 블로킹 | 앱 | I/O 완료 후 | `read()`, `recv()` |
| Non-[[122_sync_async_communication|Blocking]] I/O | 논블로킹 | 앱 | 즉시 반환 (EAGAIN) | `fcntl(O_NONBLOCK)` |
| I/O [[071_다중화_Multiplexing|Multiplexing]] (Reactor) | 준(準)블로킹 | 앱 | I/O 준비 시 | `epoll`, `select` |
| Signal-Driven I/O | 논블로킹 | 앱 | I/O 준비 시 (시그널) | `SIGIO` |
| **Async I/O (Proactor)** | **논블로킹** | **OS** | **I/O 완료 시** | **IOCP, `io_uring`** |

| 패턴 | [[083_relationship_in_er_model|관계]] | 설명 |
|:---|:---|:---|
| Reactor | 대비 | 준비 이벤트 기반, 앱이 I/O 수행 |
| Half-Sync/Half-Async | 조합 | 비동기 수신 계층 + 동기 처리 계층 |
| [[103_thread_pool|Thread Pool]] | 조합 | Completion Handler 실행을 [[103_thread_pool|스레드 풀]]에 위임 |
| [[271_command_pattern|Command]] | 활용 | Completion Handler를 [[271_command_pattern|Command]] 객체로 캡슐화 |
...
- [[103_thread_pool|스레드 풀]] 크기는 보통 CPU 코어 수의 2배 (I/O 완료 후 CPU 처리 고려)

**2. Linux [[464_io_uring|io_uring]] ([[022_kernel_role|커널]] 5.1+)**
- Submission [[058_queue|Queue]] (제출 큐)에 I/O 요청을 일괄 등록
- Completion [[058_queue|Queue]] (완료 큐)에서 완료 이벤트 수확(Harvest)
- 시스템 콜 오버헤드 최소화 — 고빈도 소규모 I/O에 최적

**3. Boost.Asio / ASIO**
- 내부적으로 OS별 최적 비동기 I/O 백엔드 선택 (Windows: IOCP, Linux: epoll/[[464_io_uring|io_uring]])
- `io_context.run()`이 Proactor 역할

| 이슈 | 설명 | 대응 |
|:---|:---|:---|
| 버퍼 수명 관리 | OS가 I/O를 수행하는 동안 버퍼가 살아있어야 함 | 스마트 포인터, 공유 버퍼 |
| 에러 전파 | 완료 이벤트에 오류 코드 포함 | error_code 반드시 [[396_validation|확인]] |
...
- **[[092_thread_lwp|스레드]] 효율성 극대화**: 소수 [[092_thread_lwp|스레드]]로 수만 연결 처리 (C10K 해결)
- **CPU 가동률 향상**: I/O 블로킹 없이 Completion Handler 연속 실행
- **응답 [[015_지연_데이터_관점|지연]] 감소**: OS 레벨 비동기 I/O는 [[022_kernel_role|커널]] 내부에서 최적화됨
...
| 대비 개념 | [[212_reactor_pattern|Reactor Pattern]] | I/O 준비 이벤트 기반, 앱이 I/O 수행 |
| 연관 개념 | [[103_thread_pool|Thread Pool]] Pattern | Completion Handler 실행 [[092_thread_lwp|스레드]] 관리 |
| 구현체 | Windows IOCP | OS 레벨 완료 [[446_port_and_bus|포트]] 메커니즘 |
| 구현체 | Boost.Asio | C++ Proactor [[198_abstraction_control_data_process|추상화]] [[336_library_vs_framework|라이브러리]] |
...
← **이전**: [[212_reactor_pattern|212. 리액터 패턴 (Reactor Pattern)]]
**다음**: [[214_half_sync_half_async_pattern|214. 하프-싱크/하프-어싱크 패턴 (Half-Sync/Half-Async Pattern)]] →
