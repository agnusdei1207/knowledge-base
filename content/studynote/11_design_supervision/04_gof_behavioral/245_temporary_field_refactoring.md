...
> 2. **가치**: 임시 필드를 제거하면 객체 상태가 항상 유효 (Valid [State](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/272_state_pattern/)) 하게 유지되고, 숨겨진 결합도와 [컨텍스트](/knowledge-base/studynote/02_operating_system/01_overview_architecture/033_context/) 의존성이 제거된다.
...
| 임시 필드 유형 | 권장 처방 | 이유 |
|:---|:---|:---|
| [알고리즘](/knowledge-base/studynote/08_algorithm_stats/01_basics/001_algorithm_definition/) 중간 상태 | 메서드 객체화 (Replace Method with Method Object) | 상태를 전용 객체로 격리 |
| 특수 케이스에서만 의미 있는 필드 | 특수 케이스 객체 (Introduce Special Case) | null 제거 |
| 캐시용 임시 필드 | 명시적 캐시 클래스 도입 | 무효화 로직 명확화 |
| 공통 [컨텍스트](/knowledge-base/studynote/02_operating_system/01_overview_architecture/033_context/) 전달 | [파라미터 객체화](/knowledge-base/studynote/11_design_supervision/04_gof_behavioral/242_introduce_parameter_object/) 또는 ThreadLocal | [스레드 안전](/knowledge-base/studynote/02_operating_system/02_process_thread/147_thread_safe/) 보장 |
...
## Ⅳ. 실무 적용 및 기술사 판단
웹 서버에서 요청 [컨텍스트](/knowledge-base/studynote/02_operating_system/01_overview_architecture/033_context/) (Request [Context](/knowledge-base/studynote/02_operating_system/01_overview_architecture/033_context/)) 를 임시 필드 대신 `ThreadLocal`로 관리하는 패턴이 표준이다.
...
| [단위 테스트](/knowledge-base/studynote/04_software_engineering/12_testing_maintenance/397_unit_test/) 독립성 | 실패 (순서 의존) | 보장 |
| 클래스 상태 불변식 | 빈번 위반 | 항상 유효 |

임시 필드 (Temporary Field) 는 "지금 당장 편하려고 만든 지름길"이 장기적으로 어떤 위험을 초래하는지 보여주는 전형적인 [안티패턴](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/128_water_scrum_fall_anti_pattern/)이다. [알고리즘](/knowledge-base/studynote/08_algorithm_stats/01_basics/001_algorithm_definition/) 중간 상태는 전용 객체로 캡슐화하고, [컨텍스트](/knowledge-base/studynote/02_operating_system/01_overview_architecture/033_context/) 데이터는 파라미터로 명시적으로 전달하며, null 가능 상태는 `Optional`이나 특수 케이스 객체로 처리하는 것이 올바른 방향이다.
...
|:---|:---|:---|
| 상위 개념 | [코드 스멜](/knowledge-base/studynote/04_software_engineering/06_software_architecture/370_code_smell/) ([Code Smell](/knowledge-base/studynote/12_it_management/05_security_compliance/365_5_solid_code_smell/)) | 임시 필드는 주요 스멜 중 하나 |
| 처방 | 메서드 객체화 (Replace Method with Method Object) | 임시 필드를 전용 클래스로 격리 |
| 처방 | 특수 케이스 객체 (Special Case Pattern) | null 필드 대체 |
| 연관 개념 | 상태 불변식 ([State](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/272_state_pattern/) Invariant) | 항상 유효한 객체 상태 |
| 연관 개념 | ThreadLocal | 요청 [컨텍스트](/knowledge-base/studynote/02_operating_system/01_overview_architecture/033_context/) 임시 상태 관리 |
| 연관 개념 | 불변 객체 ([Immutable Object](/knowledge-base/studynote/11_design_supervision/03_gof_creational_structural/172_builder_immutable_object/)) | 임시 필드 문제의 근본 해결 |
