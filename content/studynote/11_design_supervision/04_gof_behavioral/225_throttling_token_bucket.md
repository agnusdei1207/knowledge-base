...
> 1. **본질**: Token Bucket (토큰 버킷) [알고리즘](/knowledge-base/studynote/08_algorithm_stats/01_basics/001_algorithm_definition/)은 초당 R개 토큰이 버킷에 채워지고 요청마다 토큰을 소비하는 방식으로 [API](/knowledge-base/studynote/02_operating_system/01_overview_architecture/014_api_posix/) 호출 속도를 제어하는 [Rate Limiting](/knowledge-base/studynote/09_security/05_web_app_security/520_rate_limiting/) (속도 제한) [알고리즘](/knowledge-base/studynote/08_algorithm_stats/01_basics/001_algorithm_definition/)이다 — 버스트(Burst) 트래픽을 버킷 용량(B) 한도 내에서 허용한다.
> 2. **가치**: 토큰이 있으면 즉시 처리(버스트 허용), 토큰이 없으면 요청 거부 또는 대기 — 평균 처리량을 R rps (Requests Per Second)로 제한하면서도 순간 폭발 트래픽에 유연하게 대응한다.
> 3. **판단 포인트**: 토큰 버킷은 버스트 허용 + 평균 속도 제한, 리키 버킷(Leaky Bucket)은 버스트 흡수 + 일정 속도 출력 — 두 [알고리즘](/knowledge-base/studynote/08_algorithm_stats/01_basics/001_algorithm_definition/)의 차이가 [Rate Limiting](/knowledge-base/studynote/09_security/05_web_app_security/520_rate_limiting/) 설계의 핵심이다.
...
**[Rate Limiting](/knowledge-base/studynote/09_security/05_web_app_security/520_rate_limiting/) (속도 제한) 적용 목적**:
...
|:---|:---|:---|:---|
| Fixed Window [Counter](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/059_counter/) | 시간 창(Window)에서 카운팅 | 제한적 | 단순 |
| Sliding Window Log | 정확한 슬라이딩 윈도우 | 없음 | 복잡, 메모리 많음 |
| Sliding Window [Counter](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/059_counter/) | 근사 슬라이딩 윈도우 | 제한적 | 중간 |
...
## Ⅲ. 비교 및 연결
| [HTTP](/knowledge-base/studynote/03_network/09_application_layer_web_email/461_http_stateless_connection_oriented/) 상태 코드 | 의미 | 헤더 |
|:---|:---|:---|
| 200 OK | 요청 처리 성공 | X-RateLimit-Remaining: 99 |
| 429 Too Many Requests | 속도 제한 초과 | Retry-After: 60 |
| 503 [Service](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/) Unavailable | 서버 과부하 | — |
...
| [API](/knowledge-base/studynote/02_operating_system/01_overview_architecture/014_api_posix/) 키 기반 | `rate:apikey:{key}` | 유료 플랜별 제한 |
| 엔드포인트 기반 | `rate:ep:{endpoint}` | 민감 [API](/knowledge-base/studynote/02_operating_system/01_overview_architecture/014_api_posix/) [보호](/knowledge-base/studynote/02_operating_system/10_security/571_protection_vs_security/) |

- **📢 섹션 요약 비유**: [분산](/knowledge-base/studynote/08_algorithm_stats/08_stats/136_variance/) Rate Limiting은 멀티 지점 은행의 계좌 잔액 관리 — 어느 지점(서버)에서 출금해도 중앙 DB([Redis](/knowledge-base/studynote/05_database/04_transactions_concurrency/542_redis/))에서 잔액(토큰)을 공유하므로 같은 규칙이 적용된다.
...
| [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/) 유형 | Rate (rps) | Burst | 설명 |
|:---|:---|:---|:---|
...
- **📢 섹션 요약 비유**: [Rate Limiting](/knowledge-base/studynote/09_security/05_web_app_security/520_rate_limiting/) 파라미터 [설정](/knowledge-base/studynote/15_devops_sre/01_culture_methodology/009_config/)은 고속도로 속도 제한 설계 — 일반 도로(무료 [API](/knowledge-base/studynote/02_operating_system/01_overview_architecture/014_api_posix/))는 60km/h, 고속도로(유료 [API](/knowledge-base/studynote/02_operating_system/01_overview_architecture/014_api_posix/))는 110km/h로 다르게 [설정](/knowledge-base/studynote/15_devops_sre/01_culture_methodology/009_config/)하고, 단속 카메라([Redis](/knowledge-base/studynote/05_database/04_transactions_concurrency/542_redis/))가 모든 톨게이트(서버)에서 동일하게 적용한다.

---

## Ⅴ. 기대효과 및 결론
Token Bucket 기반 Rate Limiting은 [API](/knowledge-base/studynote/02_operating_system/01_overview_architecture/014_api_posix/) 게이트웨이의 필수 구성 요소다:

**기대효과**:
- **공정한 자원 배분**: 특정 클라이언트의 자원 독점 방지
- **DDoS 방어**: 악의적 대량 요청 차단
- **[서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/) 안정성**: 백엔드 과부하 방지
- **버스트 허용**: 정상적인 트래픽 패턴 수용

**설계 원칙**:
- Rate와 Burst를 [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/) 유형별로 별도 [설정](/knowledge-base/studynote/15_devops_sre/01_culture_methodology/009_config/)
- 429 응답에 Retry-After 헤더 포함 (클라이언트 재시도 가이드)
- [분산](/knowledge-base/studynote/08_algorithm_stats/08_stats/136_variance/) 환경에서는 [Redis](/knowledge-base/studynote/05_database/04_transactions_concurrency/542_redis/) + Lua 원자적 구현 필수
- 클라이언트에게 현재 한도를 X-RateLimit-* 헤더로 투명하게 공개

기술사 시험에서는 **토큰 버킷 [알고리즘](/knowledge-base/studynote/08_algorithm_stats/01_basics/001_algorithm_definition/)의 파라미터(R, B)와 동작 원리**, **리키 버킷과의 차이**, **[분산](/knowledge-base/studynote/08_algorithm_stats/08_stats/136_variance/) 환경에서의 구현 방법([Redis](/knowledge-base/studynote/05_database/04_transactions_concurrency/542_redis/))**을 서술하는 것이 핵심이다.
...
|:---|:---|:---|
| 상위 개념 | [Rate Limiting](/knowledge-base/studynote/09_security/05_web_app_security/520_rate_limiting/) (속도 제한) | Token Bucket이 구현하는 상위 개념 |
| 대비 [알고리즘](/knowledge-base/studynote/08_algorithm_stats/01_basics/001_algorithm_definition/) | Leaky Bucket | 일정 출력 속도 보장, 버스트 평활화 |
