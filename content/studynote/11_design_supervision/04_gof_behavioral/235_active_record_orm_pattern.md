...
> 1. **본질**: [액티브](/knowledge-base/studynote/03_network/09_application_layer_web_email/483_active_vs_passive_ftp/) 레코드 ([Active](/knowledge-base/studynote/03_network/09_application_layer_web_email/483_active_vs_passive_ftp/) Record) 는 DB ([Database](/knowledge-base/studynote/05_database/04_transactions_concurrency/501_database/)) 테이블의 각 행(Row)을 객체로 1:1 매핑하고, 해당 객체 안에 저장(save), 삭제(delete), 조회(find) 같은 DB ([Database](/knowledge-base/studynote/05_database/04_transactions_concurrency/501_database/)) 접근 메서드를 직접 포함한다.
> 2. **가치**: [도메인](/knowledge-base/studynote/05_database/02_modeling_normalization/064_relation_domain/) 객체와 [영속성](/knowledge-base/studynote/05_database/04_transactions_concurrency/196_durability_permanent_storage/) 로직이 한 클래스에 있어 CRUD (Create Read Update Delete) 구현이 빠르고 직관적이며, Rails의 ActiveRecord처럼 Convention over Configuration (설정보다 관례) 으로 생산성이 높다.
> 3. **판단 포인트**: [도메인](/knowledge-base/studynote/05_database/02_modeling_normalization/064_relation_domain/) 로직이 단순한 CRUD 중심 앱에 적합하지만, 복잡한 비즈니스 규칙이 추가되면 비즈니스 레이어와 DB 레이어가 결합되어 유지보수가 어려워진다.
...
|:---|:---|
| SQL 반복 작성 | 객체 메서드가 SQL을 자동 [생성](/knowledge-base/studynote/02_operating_system/02_process_thread/087_process_state_transition/) |
...
JPA의 `@Entity` 는 [액티브](/knowledge-base/studynote/03_network/09_application_layer_web_email/483_active_vs_passive_ftp/) 레코드와 유사해 보이지만, 실제로는 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 매퍼 ([Data](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) Mapper) 패턴에 더 가깝다. [영속성](/knowledge-base/studynote/05_database/04_transactions_concurrency/196_durability_permanent_storage/) 조작은 `EntityManager` / `Repository` 를 통해 이루어지며 엔티티 클래스 자체는 순수 [도메인](/knowledge-base/studynote/05_database/02_modeling_normalization/064_relation_domain/) 객체(POJO: Plain Old Java Object)를 지향한다.

| [속성](/knowledge-base/studynote/05_database/02_modeling_normalization/082_attribute_types_er_model/) | [Active](/knowledge-base/studynote/03_network/09_application_layer_web_email/483_active_vs_passive_ftp/) Record (Rails) | JPA Entity (Spring) |
|:---|:---|:---|
...
## Ⅲ. 비교 및 연결
| 항목 | [Active](/knowledge-base/studynote/03_network/09_application_layer_web_email/483_active_vs_passive_ftp/) Record | [Data](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) Mapper |
|:---|:---|:---|
| [도메인](/knowledge-base/studynote/05_database/02_modeling_normalization/064_relation_domain/) ↔ DB 결합 | 강함 (한 클래스) | 없음 (별도 Mapper) |
| 코드 간결성 | 매우 높음 | 중간 |
| 비즈니스 로직 복잡성 | 낮음 적합 | 높음 적합 |
| [단위 테스트](/knowledge-base/studynote/04_software_engineering/12_testing_maintenance/397_unit_test/) | 어려움 | 쉬움 |
...
- **📢 섹션 요약 비유**: [액티브](/knowledge-base/studynote/03_network/09_application_layer_web_email/483_active_vs_passive_ftp/) 레코드는 이케아 가구다. 빨리 조립할 수 있지만, 복잡한 커스터마이징은 한계가 있다.

---

## Ⅴ. 기대효과 및 결론
[액티브](/knowledge-base/studynote/03_network/09_application_layer_web_email/483_active_vs_passive_ftp/) 레코드 패턴의 도입 판단:
...
- **📢 섹션 요약 비유**: 자전거([액티브](/knowledge-base/studynote/03_network/09_application_layer_web_email/483_active_vs_passive_ftp/) 레코드)는 단거리에 빠르고 편리하지만, 장거리 여행엔 자동차([Data](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) Mapper)가 필요하다. 목적지를 먼저 정하고 이동 수단을 고르자.
...
|:---|:---|:---|
| 상위 개념 | ORM (Object-Relational [Mapping](/knowledge-base/studynote/05_database/01_db_architecture_relational/010_schema_mapping/)) | [액티브](/knowledge-base/studynote/03_network/09_application_layer_web_email/483_active_vs_passive_ftp/) 레코드가 속하는 기술 범주 |
| 대조 개념 | [Data Mapper Pattern](/knowledge-base/studynote/11_design_supervision/04_gof_behavioral/236_data_mapper_pattern/) | [도메인](/knowledge-base/studynote/05_database/02_modeling_normalization/064_relation_domain/)과 [영속성](/knowledge-base/studynote/05_database/04_transactions_concurrency/196_durability_permanent_storage/)을 분리하는 대안 |
| 연관 개념 | [Repository Pattern](/knowledge-base/studynote/11_design_supervision/10_patterns_antipatterns/179_repository_pattern/) | [Data](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) Mapper 위에 [추상화](/knowledge-base/studynote/04_software_engineering/04_testing_quality/198_abstraction_control_data_process/) 레이어 추가 |
| 연관 개념 | [DDD](/knowledge-base/studynote/12_it_management/05_security_compliance/310_architecture/) ([Domain-Driven Design](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/127_ddd_domain_driven_design/)) | [Data](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) Mapper와 친화적인 설계 방법론 |
...
3. 간단한 일기는 혼자 [쓰기](/knowledge-base/studynote/13_cloud_architecture/05_data_engineering/289_cqrs_db/) 쉽지만, 복잡한 소설은 편집자([Data](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) Mapper)가 따로 필요해!
