...
> 1. **본질**: 프론트 컨트롤러 (Front Controller) 는 모든 요청이 단일 진입점을 거치므로 [인증](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/303_authentication_authorization_patterns/)·로깅·[라우팅](/knowledge-base/studynote/03_network/07_network_layer_routing/339_routing_overview_best_path_selection/) 등 공통 처리를 중앙화하고, [페이지](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/) 컨트롤러 ([Page](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/) Controller) 는 각 URL이 개별 핸들러로 직접 연결된다.
> 2. **가치**: 프론트 컨트롤러는 횡단 관심사 (Cross-Cutting Concerns) 를 일원화해 중복을 제거하며, [페이지](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/) 컨트롤러는 단순 구조로 빠른 [초기](/knowledge-base/studynote/03_network/08_transport_layer/459_quic_fec_forward_error_correction/) 개발이 가능하다.
> 3. **판단 포인트**: 공통 처리가 많고 URL 구조가 복잡하면 프론트 컨트롤러, [페이지](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/)가 적고 독립적이면 [페이지](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/) 컨트롤러가 적합하다.
...
| [인증](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/303_authentication_authorization_patterns/) 처리 | 각 서블릿에 반복 코드 | Filter / Interceptor 중앙 처리 |
| [라우팅](/knowledge-base/studynote/03_network/07_network_layer_routing/339_routing_overview_best_path_selection/) | web.xml URL 매핑 | Handler [Mapping](/knowledge-base/studynote/05_database/01_db_architecture_relational/010_schema_mapping/) 동적 결정 |
| 뷰 렌더링 | 각 서블릿이 직접 [forward](/knowledge-base/studynote/10_ai/03_llm_nlp/235_forward_backward_chaining/) | [View](/knowledge-base/studynote/05_database/03_relational_model/151_sql_view_virtual_table/) Resolver 통해 위임 |
...
## Ⅲ. 비교 및 연결
| 항목 | [Page](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/) Controller | Front Controller |
|:---|:---|:---|
| 진입점 수 | URL 수 만큼 | 단일 (1개) |
| 공통 처리 | 중복 코드 | Filter/Interceptor 일원화 |
| 구현 복잡도 | 낮음 | 중간~높음 |
| 확장성 | 낮음 ([파일](/knowledge-base/studynote/02_operating_system/09_file_system/501_file_definition_logical_record/) 수 증가) | 높음 |
| [라우팅](/knowledge-base/studynote/03_network/07_network_layer_routing/339_routing_overview_best_path_selection/) 유연성 | 낮음 (정적 매핑) | 높음 (동적, 정규식, [REST](/knowledge-base/studynote/07_enterprise_systems/03_eai_esb_msa/156_rest_representational_state_transfer/)) |
...
## Ⅴ. 기대효과 및 결론
[프론트 컨트롤러 패턴](/knowledge-base/studynote/11_design_supervision/10_patterns_antipatterns/177_front_controller_pattern/) 도입의 실질적 효과:
...
- **📢 섹션 요약 비유**: 모든 편지가 우체국 중앙 [분류](/knowledge-base/studynote/16_bigdata/05_analysis/104_classification_analysis/) 센터를 거쳐 각 우체통으로 배달되듯, 프론트 컨트롤러는 모든 요청의 중앙 [분류](/knowledge-base/studynote/16_bigdata/05_analysis/104_classification_analysis/)소다.
...
|:---|:---|:---|
| 상위 개념 | MVC 패턴 | 프론트/[페이지](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/) 컨트롤러가 속하는 패턴 범주 |
| 하위 개념 | DispatcherServlet | Spring MVC의 Front Controller 구현체 |
| 하위 개념 | Handler [Mapping](/knowledge-base/studynote/05_database/01_db_architecture_relational/010_schema_mapping/) | 요청 URL → 핸들러 매핑 [컴포넌트](/knowledge-base/studynote/04_software_engineering/10_trends_pm_quality/603_component_independent_deployment_unit/) |
| 연관 개념 | Filter / Interceptor | 공통 처리를 담당하는 프론트 컨트롤러 확장 |
| 연관 개념 | [View](/knowledge-base/studynote/05_database/03_relational_model/151_sql_view_virtual_table/) Resolver | [논리](/knowledge-base/studynote/09_security/04_endpoint_security/369_logic_bomb/) 뷰명을 물리 경로로 변환 |
...
← **이전**: [233. 뷰헬퍼 커스텀 태그 패턴 (View Helper Custom Tag Pattern)](/knowledge-base/studynote/11_design_supervision/04_gof_behavioral/233_view_helper_custom_tag/)
**다음**: [235. 액티브 레코드 ORM 패턴 (Active Record ORM Pattern)](/knowledge-base/studynote/11_design_supervision/04_gof_behavioral/235_active_record_orm_pattern/) →
