...
> 1. **본질**: [Immutable Object](/knowledge-base/studynote/11_design_supervision/03_gof_creational_structural/172_builder_immutable_object/) (불변 객체) 패턴은 한번 [생성](/knowledge-base/studynote/02_operating_system/02_process_thread/087_process_state_transition/)된 객체의 상태를 절대 변경하지 않도록 설계하여, 공유 가능한 안전한 값(Value)으로 사용하는 패턴이다.
> 2. **가치**: 불변 객체는 [Thread-Safe](/knowledge-base/studynote/02_operating_system/02_process_thread/147_thread_safe/) ([스레드 안전](/knowledge-base/studynote/02_operating_system/02_process_thread/147_thread_safe/))를 보장하고, 부수효과(Side Effect)를 원천 차단하여 멀티스레드, [함수형 프로그래밍](/knowledge-base/studynote/04_software_engineering/06_software_architecture/324_functional_programming_core/), [캐싱](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/456_caching/) 시나리오에서 [동기화](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/212_synchronization_mechanisms/) 비용 없이 안전하게 공유 가능하다.
...
## Ⅲ. 비교 및 연결
| 관점 | 가변 객체 (Mutable) | 불변 객체 ([Immutable](/knowledge-base/studynote/13_cloud_architecture/05_data_engineering/298_immutable/)) |
|:---|:---|:---|
| [스레드 안전](/knowledge-base/studynote/02_operating_system/02_process_thread/147_thread_safe/)성 | [동기화](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/212_synchronization_mechanisms/) 필요 | [동기화](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/212_synchronization_mechanisms/) 불필요 |
| [캐싱](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/456_caching/) | 캐시 무효화 필요 | 자유롭게 캐시 가능 |
| Hash 키 안정성 | 해시 값 변동 위험 | 해시 값 안정 (HashMap 키로 안전) |
| 메모리 | 재사용 | 변경 시 새 객체 [생성](/knowledge-base/studynote/02_operating_system/02_process_thread/087_process_state_transition/) |
| 방어적 복사 | 수시로 필요 | 불필요 |
| 적합한 사용처 | 복잡한 상태 변경 | DTO, Value Object, 설정값 |

| 구분 | Value Object | [Reference](/knowledge-base/studynote/05_database/05_distributed_nosql_newsql/316_reference_pattern_nosql/) Object |
|:---|:---|:---|
| 동등성 | 값(내용)으로 비교 (`equals`) | [참조](/knowledge-base/studynote/05_database/05_distributed_nosql_newsql/316_reference_pattern_nosql/)(주소)로 비교 (`==`) |
| 불변성 | 일반적으로 불변 | 가변 가능 |
| 예시 | Money, Address, PhoneNumber | User, Order, Account |
| [DDD](/knowledge-base/studynote/12_it_management/05_security_compliance/310_architecture/) 역할 | [컨텍스트](/knowledge-base/studynote/02_operating_system/01_overview_architecture/033_context/) 내 [속성](/knowledge-base/studynote/05_database/02_modeling_normalization/082_attribute_types_er_model/) 표현 | [도메인](/knowledge-base/studynote/05_database/02_modeling_normalization/064_relation_domain/) 주요 엔티티 |
...
|:---|:---|
| [단일 책임 원칙](/knowledge-base/studynote/11_design_supervision/06_exam_summary/355_process/) ([SRP](/knowledge-base/studynote/04_software_engineering/04_testing_quality/243_srp_single_responsibility_principle/)) | 상태 변경 책임이 없어 역할이 명확 |
| [개방-폐쇄 원칙](/knowledge-base/studynote/11_design_supervision/06_exam_summary/356_process/) ([OCP](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/746_ocp/)) | 새 값은 새 객체로 — 기존 코드 불변 |
| 의존성 역전 ([DIP](/knowledge-base/studynote/04_software_engineering/04_testing_quality/247_dip_dependency_inversion_principle/)) | 값 객체는 구현이 아닌 값에 의존 |
...
**핵심 기대효과**:
- <strong><a href="/knowledge-base/studynote/02_operating_system/02_process_thread/147_thread_safe/">스레드 안전</a>성 보장</strong>: 락([Lock](/knowledge-base/studynote/05_database/04_transactions_concurrency/510_lock/)) 없이 자유로운 공유
- **버그 감소**: 예측 불가능한 상태 변경(공유 가변 상태) 제거
- <strong><a href="/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/456_caching/">캐싱</a> 용이</strong>: HashMap 키, 캐시 값으로 안전하게 사용
...
기술사 시험에서는 **불변 객체의 5가지 설계 원칙**(final 클래스, final 필드, setter 없음, 방어적 복사, 새 객체 반환)과 <strong><a href="/knowledge-base/studynote/02_operating_system/02_process_thread/147_thread_safe/">Thread-Safe</a> 보장 원리</strong>를 정확히 서술하는 것이 핵심이다.
...
- **📢 섹션 요약 비유**: 불변 객체는 신분증 — 한번 발급되면 내용을 바꿀 수 없고, 필요하면 새 신분증을 발급(새 객체 [생성](/knowledge-base/studynote/02_operating_system/02_process_thread/087_process_state_transition/))받는다. 여러 사람이 동시에 신분증을 봐도 내용이 바뀔 걱정이 없다.
...
|:---|:---|:---|
| 상위 개념 | [함수형 프로그래밍](/knowledge-base/studynote/04_software_engineering/06_software_architecture/324_functional_programming_core/) ([Functional Programming](/knowledge-base/studynote/04_software_engineering/06_software_architecture/324_functional_programming_core/)) | 불변성은 함수형의 핵심 원칙 |
| 연관 개념 | Value Object (값 객체) | DDD에서 불변 객체의 대표 활용 |
| 연관 개념 | [Thread](/knowledge-base/studynote/02_operating_system/02_process_thread/092_thread_lwp/) Safety ([스레드 안전](/knowledge-base/studynote/02_operating_system/02_process_thread/147_thread_safe/)성) | 불변 객체가 제공하는 핵심 이점 |
| 구현 도구 | Java Record (Java 16+) | 불변 객체의 간결한 선언 방법 |
| 연관 패턴 | [Builder Pattern](/knowledge-base/studynote/11_design_supervision/03_gof_creational_structural/148_builder_pattern/) | 복잡한 불변 객체 [생성](/knowledge-base/studynote/02_operating_system/02_process_thread/087_process_state_transition/) 지원 |
...
3. 여러 사람이 동시에 같은 책(불변 객체)을 읽어도 내용이 바뀌지 않으니까 싸울 필요가 없어 — 이것이 [스레드 안전](/knowledge-base/studynote/02_operating_system/02_process_thread/147_thread_safe/)성이야.
