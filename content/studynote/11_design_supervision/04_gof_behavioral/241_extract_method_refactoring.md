...
메서드 분리 (Extract Method) 는 리팩토링의 알파이자 오메가다. 코드를 <strong>읽는 코드</strong>로 만드는 첫 번째 도구이며, 이후 모든 고수준 패턴 적용의 <strong>전제 조건</strong>이다. 기술사 설계 논술에서는 레거시 시스템 개선 방안으로 "Extract Method + 단위 테스트 보강"을 표준 답안으로 제시할 수 있다.
