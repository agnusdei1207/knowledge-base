...
> 1. **본질**: Null Object (널 객체) 패턴은 `null` [[316_reference_pattern_nosql|참조]] 대신 "아무 일도 하지 않는" 기본 객체(NoOp, No-Operation 객체)를 반환하여, 호출자가 `null` 체크를 하지 않아도 되게 만든다.
...
## Ⅰ. 개요 및 필요성
Tony Hoare가 1965년 null [[316_reference_pattern_nosql|참조]]를 도입하면서 "10억 달러짜리 실수"라고 직접 표현했다. null이 문제인 이유:
...
|:---|:---|:---|
| 빈 [[055_array|배열]] `[]` | 원소 없는 컬렉션 | [[001_dikw_pyramid|데이터]] 포함 [[055_array|배열]] |
| `/dev/null` (Unix) | 쓰고 버리는 장치 | 실제 [[501_file_definition_logical_record|파일]] |
| `Collections.emptyList()` | 빈 리스트 | [[001_dikw_pyramid|데이터]] 리스트 |
| NullLogger | [[568_logs_distributed_logging_elk_fluentd|로그]]를 버리는 로거 | 실제 [[501_file_definition_logical_record|파일]] 로거 |
| Anonymous User | 미로그인 사용자 | [[568_logs_distributed_logging_elk_fluentd|로그]]인 사용자 |
...
- **📢 섹션 요약 비유**: 비어있는 통장(Null Object) — 잔액 조회를 해도 에러가 나지 않고 "0원"이라고 알려준다. null [[316_reference_pattern_nosql|참조]]라면 ATM기가 오작동할 것이다.
...
- **[[382_defensive_programming|방어적 프로그래밍]]([[382_defensive_programming|Defensive Programming]])의 코드 제거** 효과 강조
...
| 효과 | 설명 |
|:---|:---|
| NPE 완전 제거 | null [[316_reference_pattern_nosql|참조]] 자체를 없앰 |
| 코드 간결성 | `if (x != null)` 방어 코드 제거 |
| 일관된 인터페이스 | null 여부 [[083_relationship_in_er_model|관계]]없이 동일하게 호출 |
| 테스트 용이 | NullObject로 의존성 제거 ([[367_test_double_isolation|테스트 더블]]) |

- 오류 상황을 **조용히 무시**할 수 있음 → 중요한 에러가 숨겨질 위험
- 디버깅 시 "왜 아무 일도 일어나지 않는지" 파악 어려울 수 있음
- Null Object도 명확한 **로깅**을 포함하는 것을 권장

Null Object (널 객체) 패턴은 [[382_defensive_programming|방어적 프로그래밍]]의 대안으로, 코드를 더 단순하고 NPE로부터 안전하게 만든다. 로거, 이벤트 핸들러, 기본 [[164_policy|정책]] 객체처럼 "없어도 동작이 계속되어야 하는" 컴포넌트에 특히 효과적이다.
...
| 연관 개념 | [[268_strategy_pattern|Strategy Pattern]] | Null Object를 기본 전략으로 사용 |
| 연관 개념 | [[272_state_pattern|State Pattern]] | Null Object를 기본 상태로 사용 |
...
### 📈 관련 키워드 및 발전 흐름도
기본값 처리 → 널 객체 패턴 → [[459_fail_safe|Fail-safe]] 객체
