...
## Ⅰ. 개요 및 필요성
Tony Hoare가 1965년 null [참조](/studynote/05_database/05_distributed_nosql_newsql/316_reference_pattern_nosql/)를 도입하면서 "10억 달러짜리 실수"라고 직접 표현했다. null이 문제인 이유:
...
|:---|:---|:---|
| 빈 [배열](/studynote/08_algorithm_stats/04_datastructure/055_array/) `[]` | 원소 없는 컬렉션 | [데이터](/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 포함 [배열](/studynote/08_algorithm_stats/04_datastructure/055_array/) |
| `/dev/null` (Unix) | 쓰고 버리는 장치 | 실제 [파일](/studynote/02_operating_system/09_file_system/501_file_definition_logical_record/) |
| `Collections.emptyList()` | 빈 리스트 | [데이터](/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 리스트 |
| NullLogger | [로그](/studynote/04_software_engineering/09_cloud_native_ai_architecture/568_logs_distributed_logging_elk_fluentd/)를 버리는 로거 | 실제 [파일](/studynote/02_operating_system/09_file_system/501_file_definition_logical_record/) 로거 |
| Anonymous User | 미로그인 사용자 | [로그](/studynote/04_software_engineering/09_cloud_native_ai_architecture/568_logs_distributed_logging_elk_fluentd/)인 사용자 |
...
- **📢 섹션 요약 비유**: 비어있는 통장(Null Object) — 잔액 조회를 해도 에러가 나지 않고 "0원"이라고 알려준다. null [참조](/studynote/05_database/05_distributed_nosql_newsql/316_reference_pattern_nosql/)라면 ATM기가 오작동할 것이다.
...
- <strong><a href="/studynote/04_software_engineering/06_software_architecture/382_defensive_programming/">방어적 프로그래밍</a>(<a href="/studynote/04_software_engineering/06_software_architecture/382_defensive_programming/">Defensive Programming</a>)의 코드 제거</strong> 효과 강조
...
| 효과 | 설명 |
|:---|:---|
| NPE 완전 제거 | null [참조](/studynote/05_database/05_distributed_nosql_newsql/316_reference_pattern_nosql/) 자체를 없앰 |
| 코드 간결성 | `if (x != null)` 방어 코드 제거 |
| 일관된 인터페이스 | null 여부 [관계](/studynote/05_database/02_modeling_normalization/083_relationship_in_er_model/)없이 동일하게 호출 |
| 테스트 용이 | NullObject로 의존성 제거 ([테스트 더블](/studynote/12_it_management/05_security_compliance/1008_test_double_isolation/)) |
...
Null Object (널 객체) 패턴은 [방어적 프로그래밍](/studynote/04_software_engineering/06_software_architecture/382_defensive_programming/)의 대안으로, 코드를 더 단순하고 NPE로부터 안전하게 만든다. 로거, 이벤트 핸들러, 기본 [정책](/studynote/10_ai/02_dl_architecture_new/164_policy/) 객체처럼 "없어도 동작이 계속되어야 하는" 컴포넌트에 특히 효과적이다.
...
| 연관 개념 | [Strategy Pattern](/studynote/04_software_engineering/04_testing_quality/268_strategy_pattern/) | Null Object를 기본 전략으로 사용 |
| 연관 개념 | [State Pattern](/studynote/04_software_engineering/05_devops_ci_cd/272_state_pattern/) | Null Object를 기본 상태로 사용 |
...
### 📈 관련 키워드 및 발전 흐름도
기본값 처리 -> 널 객체 패턴 -> [Fail-safe](/studynote/01_computer_architecture/13_reliability_power_management/459_fail_safe/) 객체
...
<- **이전**: [206. 해석자 패턴 (Interpreter Pattern)](/studynote/11_design_supervision/04_gof_behavioral/206_interpreter_pattern/)
**다음**: [208. 가드 서스펜션 패턴 (Guarded Suspension Pattern)](/studynote/11_design_supervision/04_gof_behavioral/208_guarded_suspension_pattern/) ->
