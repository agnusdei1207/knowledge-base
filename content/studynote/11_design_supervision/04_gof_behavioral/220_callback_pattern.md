...
> 3. **판단 포인트**: 콜백 지옥(Callback Hell / Pyramid of Doom)은 중첩 콜백이 만드는 [[333_readability_vs_efficiency|가독성]]·[[346_maintainability_portability|유지보수성]] 붕괴다 — Promise, async/await, Observable이 이를 해결하는 발전된 패턴이다.
...
- **📢 섹션 요약 비유**: 콜백은 식당에서 "음식 나오면 문자 주세요" 방식 — 음식이 나올 때까지 자리에 앉아 기다리지(블로킹) 않고, [[059_counter|카운터]] 번호표(콜백 함수)를 주고 다른 일을 하다가 호출받는 것이다.
...
## Ⅲ. 비교 및 연결
| 패턴 | 표현 방식 | 에러 처리 | [[333_readability_vs_efficiency|가독성]] | 사용 환경 |
|:---|:---|:---|:---|:---|
| Callback | 함수 인수 전달 | 각 콜백마다 | 낮음 (중첩) | Node.js ([[459_quic_fec_forward_error_correction|초기]]) |
...
[[409_architecture|콜백 패턴]]은 [[267_observer_pattern|Observer]] ([[267_observer_pattern|옵저버]]) 패턴의 단순화된 형태다:

| 비교 | Callback | [[267_observer_pattern|Observer]] |
|:---|:---|:---|
| 구독자 수 | 1개 (일반적) | N개 가능 |
| 지속성 | 일회성 (보통) | 지속적 구독 |
| 취소 | 어려움 | 구독 해제(unsubscribe) |
| [[229_stream_processing_kafka_flink|스트림 처리]] | 부적합 | 적합 (RxJS, Reactor) |
...
- **📢 섹션 요약 비유**: Node.js의 Error-First 콜백은 소방서 보고 규약처럼 — 항상 "이상 없음/있음" 상황 보고(err)를 먼저 하고, 문제 없을 때만 세부 정보([[001_dikw_pyramid|data]])를 보고한다.

---

## Ⅴ. 기대효과 및 결론
[[409_architecture|콜백 패턴]]은 비동기 프로그래밍의 출발점이자, 현대 비동기 패턴들(Promise, async/await, Reactive Streams)의 원형이다:

**장점**:
- 단순하고 범용적 — 어떤 환경에서도 구현 가능
- 이벤트 기반 시스템(GUI, 네트워크, 타이머)에 자연스럽게 적합
- 낮은 [[198_abstraction_control_data_process|추상화]] 오버헤드
...
[[409_architecture|콜백 패턴]]을 이해하면 Promise, async/await의 설계 의도와 문제 해결 방식을 깊이 이해할 수 있다. 기술사 시험에서는 **콜백 지옥의 문제점과 Promise/async/await로의 발전 과정**을 서술하는 것이 핵심이다.
...
| 연관 패턴 | [[267_observer_pattern|Observer Pattern]] | 콜백의 다중 구독자 확장 |
| 발전 패턴 | Promise/Future | 콜백 지옥을 해결한 상위 [[198_abstraction_control_data_process|추상화]] |
| 발전 패턴 | async/await | Promise의 문법적 설탕 |
| 발전 패턴 | Observable (RxJS) | 이벤트 [[229_stream_processing_kafka_flink|스트림 처리]]를 위한 반응형 확장 |
| 구현체 | Node.js EventEmitter | 비동기 이벤트 기반 콜백 시스템 |
| 연관 패턴 | [[271_command_pattern|Command Pattern]] | 콜백 함수를 [[271_command_pattern|커맨드]] 객체로 캡슐화 |

### 📈 관련 키워드 및 발전 흐름도
이벤트 통지 → [[409_architecture|콜백 패턴]] → 비동기 [[213_flow_control_buffer_overflow|흐름 제어]]
