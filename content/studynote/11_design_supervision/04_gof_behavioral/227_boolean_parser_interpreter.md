...
> 1. **본질**: [[277_interpreter_pattern|Interpreter]] (인터프리터) 패턴은 언어의 문법을 객체 계층(Object Hierarchy)으로 표현하고, 각 객체가 자신의 해석(Interpret)을 담당하여 표현식을 평가하는 패턴이다 — Boolean Parser는 이 패턴의 가장 실용적인 구현 예다.
> 2. **가치**: 비개발자도 이해할 수 있는 [[064_relation_domain|도메인]] 특화 언어(DSL, [[064_relation_domain|Domain]] Specific Language)를 설계하여, 복잡한 비즈니스 규칙을 코드 변경 없이 외부에서 표현하고 변경할 수 있다.
> 3. **판단 포인트**: 렉서(Lexer) → 파서(Parser, [[014_recursion|재귀]] 하강) → AST (Abstract Syntax Tree, 추상 구문 트리) → 평가기(Evaluator)의 4단계 파이프라인이 인터프리터의 표준 구조다.
...
| [[369_logic_bomb|논리]] AND | 두 표현식 모두 참 | `A AND B` |
| [[369_logic_bomb|논리]] OR | 하나 이상 참 | `A OR B` |
| [[369_logic_bomb|논리]] NOT | 부정 | `NOT A` |
...
- **📢 섹션 요약 비유**: [[014_recursion|재귀]] 하강 파서는 수학 문제를 푸는 학생 — "괄호 먼저, 곱셈/나눗셈 그다음, 덧셈/뺄셈 마지막"처럼 문법 우선순위 규칙을 [[014_recursion|재귀]] 함수의 호출 순서로 구현한다.
...
| [[033_context|Context]] | 변수 값을 담은 평가 [[033_context|컨텍스트]] |
| [[003_audit_stakeholders|Client]] | 파서로 AST를 구성하고 evaluate 호출 |

| 시스템 | 인터프리터 활용 |
|:---|:---|
| [[302_cdc|ElasticSearch]] Query DSL | [[343_json|JSON]] 기반 Boolean [[298_qkv_attention|쿼리]] 파서 |
| Spring [[283_security_tactics|Security]] SpEL | `hasRole('ADMIN') and isAuthenticated()` |
| Drools Business Rules | 비즈니스 규칙 DSL 평가 엔진 |
| AWS [[526_iam|IAM]] [[164_policy|Policy]] | JSON으로 표현된 권한 표현식 평가 |
...
| 규칙을 DB/설정에 저장 | [[282_performance_tactics|성능]] 최우선 (파싱 오버헤드) |
...
- **📢 섹션 요약 비유**: 인터프리터 패턴은 교통 [[130_signal|신호]] 제어 시스템 — 교통 담당자(비개발자)가 "출퇴근 시간 버스전용차로 활성화" 규칙을 텍스트로 입력하면, 시스템이 런타임에 해석해서 [[130_signal|신호]]를 바꾼다. 개발자가 코드를 바꿀 필요 없다.

---

## Ⅴ. 기대효과 및 결론
Boolean Parser [[277_interpreter_pattern|Interpreter]] 패턴은 비즈니스 규칙의 외부화(Externalize)를 통해 시스템 유연성을 극대화한다:

**기대효과**:
- **배포 없는 규칙 변경**: DB의 규칙 문자열만 수정하면 즉시 반영
- **[[064_relation_domain|도메인]] 전문가 참여**: 코드를 모르는 전문가도 규칙 정의 가능
- **테스트 용이성**: 규칙을 독립적으로 [[397_unit_test|단위 테스트]] 가능
- **재사용성**: 동일 인터프리터로 다양한 [[064_relation_domain|도메인]] 규칙 처리

**한계**:
- 복잡한 언어일수록 파서 구현이 복잡해짐 (ANTLR 등 파서 [[087_process_state_transition|생성]]기 권장)
- 파싱 오버헤드 (AST 캐싱으로 완화)
- 문법 에러 처리 및 디버깅 도구 필요

기술사 시험에서는 **Lexer → Parser → AST → Evaluator 4단계**와 **TerminalExpression/NonterminalExpression 역할**을 명확히 서술하고, **실무 적용 사례(Spring [[283_security_tactics|Security]] SpEL, Drools)**를 언급하는 것이 핵심이다.
...
- **📢 섹션 요약 비유**: Boolean Parser는 법원의 판사 — 법 조문(문법 정의)에 따라 사건(표현식)을 분석하고, 증거([[033_context|컨텍스트]] 값)를 검토하여 무죄/유죄(true/false)를 판결(evaluate)한다.
...
|:---|:---|:---|
| 상위 개념 | GoF [[277_interpreter_pattern|Interpreter Pattern]] | Boolean Parser의 상위 패턴 |
| 구성 패턴 | [[154_composite_pattern|Composite Pattern]] | AST 노드 계층 구조에 활용 |
| 연관 개념 | DSL ([[064_relation_domain|Domain]] Specific Language) | 인터프리터가 해석하는 [[064_relation_domain|도메인]] 언어 |
| 연관 개념 | AST (Abstract Syntax Tree) | 파싱 결과물, 평가의 입력 |
| 구현 도구 | ANTLR | 문법 정의 → 파서 자동 [[087_process_state_transition|생성]] 도구 |
| 실무 사례 | Spring [[283_security_tactics|Security]] SpEL | 권한 표현식 DSL 인터프리터 |
| 연관 개념 | BNF 문법 정의 | 언어 구조를 형식적으로 기술하는 표기법 |

### 📈 관련 키워드 및 발전 흐름도
[[820_tokenization|토큰화]] → 불리언 파서 인터프리터 → 규칙 DSL
