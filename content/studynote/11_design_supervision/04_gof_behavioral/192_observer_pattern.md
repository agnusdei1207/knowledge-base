...
> 1. **본질**: [옵저버 패턴](/knowledge-base/studynote/04_software_engineering/10_trends_pm_quality/606_observer_pattern_pub_sub/) ([Observer Pattern](/knowledge-base/studynote/04_software_engineering/04_testing_quality/267_observer_pattern/))은 GoF 행위 패턴으로, 객체(Subject/Publisher)의 상태 변화가 발생할 때 이에 의존하는 다수의 객체([Observer](/knowledge-base/studynote/04_software_engineering/04_testing_quality/267_observer_pattern/)/Subscriber)에게 자동으로 통지·갱신하는 일대다(1:N) 의존 관계를 정의하는 패턴이다.
> 2. **가치**: Subject가 Observer를 직접 알지 못하고 인터페이스를 통해 통지하므로, Observer를 동적으로 추가·제거할 수 있고 Subject와 [Observer](/knowledge-base/studynote/04_software_engineering/04_testing_quality/267_observer_pattern/) 간 [결합도](/knowledge-base/studynote/04_software_engineering/04_testing_quality/195_coupling_levels/)가 낮아진다. 이벤트 기반 시스템, MVC 아키텍처, 리액티브 프로그래밍의 근간이다.
> 3. **판단 포인트**: [옵저버 패턴](/knowledge-base/studynote/04_software_engineering/10_trends_pm_quality/606_observer_pattern_pub_sub/)에서 Subject가 모든 Observer에 동기적으로 통지하면 느린 Observer가 Subject를 블로킹할 수 있다. 비동기 이벤트 큐(EventBus, [메시지 브로커](/knowledge-base/studynote/07_enterprise_systems/03_eai_esb_msa/145_message_broker_sync_async/))로 발행-구독을 분리하거나, [Observer](/knowledge-base/studynote/04_software_engineering/04_testing_quality/267_observer_pattern/) 통지를 별도 스레드로 처리하여 이 문제를 해결해야 한다.

---

## Ⅰ. 개요 및 필요성

[옵저버 패턴](/knowledge-base/studynote/04_software_engineering/10_trends_pm_quality/606_observer_pattern_pub_sub/)은 객체 간의 1:N 의존 관계를 느슨한 결합으로 구현한다. 예를 들어 스프레드시트에서 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)(Subject)가 변경되면 차트·집계 테이블·요약 뷰([Observer](/knowledge-base/studynote/04_software_engineering/04_testing_quality/267_observer_pattern/))가 자동으로 갱신된다. [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)가 각 뷰를 직접 알지 못해도 된다.
...
[옵저버 패턴](/knowledge-base/studynote/04_software_engineering/10_trends_pm_quality/606_observer_pattern_pub_sub/)의 두 가지 통지 방식: ① Push 방식: Subject가 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 Observer에게 직접 전달(`update(data)`), ② Pull 방식: Subject가 변경만 통지하고 Observer가 필요한 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 Subject에서 요청. Pull 방식이 Observer와 Subject 간 [결합도](/knowledge-base/studynote/04_software_engineering/04_testing_quality/195_coupling_levels/)가 더 낮다.

| 항목 | 설명 | 포인트 |
|:---|:---|:---|
| Push ([데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 전달) | 단순, Observer가 모든 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 수신 | 중간 |
...
[옵저버 패턴](/knowledge-base/studynote/04_software_engineering/10_trends_pm_quality/606_observer_pattern_pub_sub/)과 [미디에이터 패턴](/knowledge-base/studynote/11_design_supervision/04_gof_behavioral/201_mediator_pattern/)의 차이: [옵저버](/knowledge-base/studynote/04_software_engineering/04_testing_quality/267_observer_pattern/)는 Subject가 Observer에게 통지하는 1:N 관계이고, 미디에이터는 모든 객체가 미디에이터를 통해 N:N으로 통신한다.
...
- **📢 섹션 요약 비유**: [옵저버](/knowledge-base/studynote/04_software_engineering/04_testing_quality/267_observer_pattern/)는 방송국(Subject)이 시청자([Observer](/knowledge-base/studynote/04_software_engineering/04_testing_quality/267_observer_pattern/))에게 일방적으로 방송하는 것이고, 미디에이터는 채팅방([Mediator](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/273_mediator_pattern/))에서 모든 참여자가 소통하는 것이다.

---
## Ⅳ. 실무 적용 및 기술사 판단

리액티브 프로그래밍(RxJava, [Project](/knowledge-base/studynote/05_database/01_db_architecture_relational/042_relational_algebra_project/) Reactor)은 [옵저버 패턴](/knowledge-base/studynote/04_software_engineering/10_trends_pm_quality/606_observer_pattern_pub_sub/)의 함수형·비동기 확장이다. `Observable`(Subject)과 `Observer`를 함수형 스트림으로 표현하여, 비동기·비블로킹 이벤트 처리를 우아하게 구현한다.
...
[옵저버 패턴](/knowledge-base/studynote/04_software_engineering/10_trends_pm_quality/606_observer_pattern_pub_sub/)을 적용하면 Subject와 [Observer](/knowledge-base/studynote/04_software_engineering/04_testing_quality/267_observer_pattern/) 간 [결합도](/knowledge-base/studynote/04_software_engineering/04_testing_quality/195_coupling_levels/)가 낮아져 새 Observer를 쉽게 추가할 수 있다. [이벤트 기반 아키텍처](/knowledge-base/studynote/04_software_engineering/09_cloud_native_ai_architecture/538_event_driven_architecture_eda/)([EDA](/knowledge-base/studynote/12_it_management/02_itsm_itil/064_eda/)), MVC, 리액티브 프로그래밍의 근간이 되어 현대 소프트웨어 전반에 적용된다.

한계는 동기 통지 시 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) 문제, 복잡한 이벤트 연쇄로 인한 디버깅 어려움, [메모리 누수](/knowledge-base/studynote/02_operating_system/10_security/612_memory_leak_detection/)([Observer](/knowledge-base/studynote/04_software_engineering/04_testing_quality/267_observer_pattern/) 미제거) 위험이다. 리액티브 스트림([Project](/knowledge-base/studynote/05_database/01_db_architecture_relational/042_relational_algebra_project/) Reactor)이 이러한 문제를 함수형으로 해결한 현대적 구현이다.

- **📢 섹션 요약 비유**: 유튜브 알림처럼, 크리에이터(Subject)가 영상 업로드(이벤트)를 하면 구독자([Observer](/knowledge-base/studynote/04_software_engineering/04_testing_quality/267_observer_pattern/)) 전원에게 자동으로 알림이 간다. 크리에이터는 구독자 수가 얼마든 알림 메커니즘을 알 필요 없다.
...
| 개념 | 연결 포인트 |
|:---|:---|
| 발행-구독(Pub/Sub) | [옵저버](/knowledge-base/studynote/04_software_engineering/04_testing_quality/267_observer_pattern/)의 [분산](/knowledge-base/studynote/08_algorithm_stats/08_stats/136_variance/) 시스템 확장 |
| 리액티브 프로그래밍 | [옵저버 패턴](/knowledge-base/studynote/04_software_engineering/10_trends_pm_quality/606_observer_pattern_pub_sub/)의 함수형·비동기 확장 |
| [이벤트 주도 아키텍처](/knowledge-base/studynote/11_design_supervision/06_exam_summary/367_architecture/)([EDA](/knowledge-base/studynote/12_it_management/02_itsm_itil/064_eda/)) | [옵저버 패턴](/knowledge-base/studynote/04_software_engineering/10_trends_pm_quality/606_observer_pattern_pub_sub/)의 아키텍처 수준 적용 |
| [미디에이터 패턴](/knowledge-base/studynote/11_design_supervision/04_gof_behavioral/201_mediator_pattern/) | [옵저버 패턴](/knowledge-base/studynote/04_software_engineering/10_trends_pm_quality/606_observer_pattern_pub_sub/)과 대비되는 N:N 통신 패턴 |
...
1. [옵저버 패턴](/knowledge-base/studynote/04_software_engineering/10_trends_pm_quality/606_observer_pattern_pub_sub/)은 유튜브 알림처럼, 크리에이터(Subject)가 영상을 올리면 구독자([Observer](/knowledge-base/studynote/04_software_engineering/04_testing_quality/267_observer_pattern/))에게 자동 알림이 가요.
