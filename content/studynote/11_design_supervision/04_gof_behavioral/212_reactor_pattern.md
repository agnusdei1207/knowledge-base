...
> 1. **본질**: Reactor (리액터) 패턴은 단일 [[092_thread_lwp|스레드]]의 [[142_event_loop|이벤트 루프]]([[142_event_loop|Event Loop]])가 I/O [[071_다중화_Multiplexing|Multiplexing]]([[071_다중화_Multiplexing|다중화]])을 통해 다수의 I/O 이벤트를 동시에 감시하고, 이벤트 발생 시 등록된 핸들러를 즉시 호출하는 비동기 I/O 아키텍처다.
> 2. **가치**: [[092_thread_lwp|스레드]] [[087_process_state_transition|생성]] 없이 수만 개의 동시 연결을 처리할 수 있어, C10K (Concurrent [[489_raid_10_hybrid|10]],000 connections) 문제를 해결하는 핵심 기술이다.
> 3. **판단 포인트**: CPU 집중 작업이 아닌 I/O 대기가 많은 상황(네트워크 서버, [[014_api_posix|API]] 게이트웨이 등)에서 최적이다. CPU 집중 작업은 Worker [[092_thread_lwp|Thread]] Pool과 결합한다.
...
| 기술 | OS | 특징 | [[282_performance_tactics|성능]] |
|:---|:---|:---|:---|
...
- **📢 섹션 요약 비유**: 수박 밭 관리 — 1만 개 수박(연결)을 1만 명 농부([[092_thread_lwp|스레드]])가 각각 지키는 게 아니라, 드론([[142_event_loop|이벤트 루프]])이 날아다니며 익은 수박(준비된 I/O)만 수확 팀에게 알려준다.
...
- **📢 섹션 요약 비유**: 119 종합상황실 — 상황실([[142_event_loop|Event Loop]])이 모든 신고 채널(Handle)을 모니터링하고, 신고가 들어오면(이벤트) 해당 전담팀(Event Handler)에게 즉시 연결한다. 신고가 없는 동안은 CPU를 낭비하지 않는다.

---

## Ⅲ. 비교 및 연결
| 항목 | [[092_thread_lwp|Thread]]-Per-Connection | Reactor | Proactor |
|:---|:---|:---|:---|
| **동기/비동기** | 동기 | 동기 이벤트 감시 | 완료 기반 비동기 |
| **알림 시점** | 블로킹 | I/O 준비 완료 시 | I/O 완료 후 |
| **[[092_thread_lwp|스레드]] 수** | 연결 수만큼 | 소수 | 소수 |
| **구현 복잡도** | 낮음 | 중간 | 높음 |
...
| [[179_kafka_flink_watermark_time_window|Kafka]] | NIO Selector |
...
- **I/O [[071_다중화_Multiplexing|Multiplexing]]** ([[520_select|select]]/epoll) 메커니즘과 Reactor의 연관성 명시
- **C10K 문제** 해결 방법으로 Reactor 패턴 제시
- Node.js, Nginx, Netty 등 실제 구현 사례로 [[642_reliability_mtbf_mttr_mttf_availability|신뢰성]] 강화
...
- **📢 섹션 요약 비유**: 공항 내 자동 안내판([[142_event_loop|이벤트 루프]]) — 모든 항공편 게이트(Handle)를 실시간 감시하다가 탑승 안내가 시작되면(I/O 이벤트) 해당 게이트 [[090_service_kubernetes_network_load_balancing|서비스]] 직원(Event Handler)에게 즉시 알린다.
...
| 효과 | 설명 |
|:---|:---|
| 높은 [[014_concurrency|동시성]] | [[092_thread_lwp|스레드]] 수와 무관하게 수만 연결 처리 |
| 낮은 메모리 | [[092_thread_lwp|스레드]] [[057_stack|스택]] 최소화 |
| 낮은 [[015_지연_데이터_관점|지연]] | epoll의 O(1) 이벤트 통지 |
| 에너지 효율 | I/O 대기 중 CPU 반환 |

- **블로킹 코드 금지**: [[142_event_loop|이벤트 루프]] 내 블로킹 작업 → 모든 연결 [[015_지연_데이터_관점|지연]]
- **CPU 집중 작업**: Worker [[092_thread_lwp|Thread]] Pool로 반드시 오프로드
- **콜백 지옥(Callback Hell)**: Promise/async-await/Reactive Streams로 해결

Reactor (리액터) 패턴은 현대 고성능 서버의 핵심 아키텍처다. Node.js, Nginx, Netty, [[542_redis|Redis]] 모두 이 패턴을 기반으로 수만 개의 동시 연결을 소수의 [[092_thread_lwp|스레드]]로 처리한다. I/O 집중 워크로드에서 [[092_thread_lwp|Thread]]-Per-Connection의 수십 배 처리량을 달성한다.
...
- **📢 섹션 요약 비유**: Reactor는 "스마트 교통 관제 시스템" — 모든 도로(I/O 채널)를 실시간 감시하다가 사고(이벤트)가 나면 해당 구역 대응팀(Handler)을 즉시 출동시킨다. 평상시에는 단 한 명의 관제원([[142_event_loop|Event Loop]])으로 충분하다.
...
| 하위 개념 | Event Handler / Handle / [[168_dispatcher|Dispatcher]] | Reactor 4요소 |
| 연관 개념 | epoll / kqueue | I/O [[071_다중화_Multiplexing|Multiplexing]] 기술 |
| 연관 개념 | Node.js libuv | Reactor 패턴의 JS 구현 |
| 연관 개념 | Netty | Java 고성능 Reactor 프레임워크 |
| 연관 개념 | [[213_proactor_pattern|Proactor Pattern]] | 완료 기반 비동기의 대안 패턴 |

### 📈 관련 키워드 및 발전 흐름도
이벤트 분리 → 리액터 패턴 → 논블로킹 서버

### 👶 어린이를 위한 3줄 비유 설명
1. 놀이공원에서 놀이기구(연결)가 만 개 있어도, 안내원([[142_event_loop|이벤트 루프]])은 딱 한 명이에요.
...
← **이전**: [[211_active_object_pattern|211. 액티브 오브젝트 패턴 (Active Object Pattern)]]
**다음**: [[213_proactor_pattern|213. 프로액터 패턴 (Proactor Pattern)]] →
