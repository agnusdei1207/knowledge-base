...
| Java | `CompletableFuture` | 없음 (구조적 [동시성](/knowledge-base/studynote/15_devops_sre/01_culture_methodology/014_concurrency/) Java 21+) | 명시적 체이닝 |
| Kotlin | `Deferred` | `suspend fun` | [코루틴](/knowledge-base/studynote/02_operating_system/02_process_thread/141_coroutine/) 기반 |
| C# | `Task<T>` | `async/await` (C# 5.0) | 선구자 |
| Python | `asyncio.Future` | `async/await` (Python 3.5) | — |
| [Rust](/knowledge-base/studynote/04_software_engineering/10_trends_pm_quality/782_memory_safety_rust_compiler_verification/) | `Future` | `async/await` ([Rust](/knowledge-base/studynote/04_software_engineering/10_trends_pm_quality/782_memory_safety_rust_compiler_verification/) 1.39) | Zero-cost [추상화](/knowledge-base/studynote/04_software_engineering/04_testing_quality/198_abstraction_control_data_process/) |
...
- <strong><a href="/knowledge-base/studynote/05_database/07_exam_summary/430_index_fast_full_scan/">병렬</a> 처리 용이</strong>: `Promise.all()` 한 줄로 [병렬](/knowledge-base/studynote/05_database/07_exam_summary/430_index_fast_full_scan/) 비동기 실행
- **언어 표준화**: 모든 현대 언어/플랫폼에서 동일한 개념

**한계**:
- 무한 스트림(이벤트 스트림) 처리에는 Observable이 더 적합
- 취소(Cancellation) 지원이 약함 (AbortController 별도 필요)
- Kotlin [Coroutine](/knowledge-base/studynote/02_operating_system/02_process_thread/141_coroutine/), Java Virtual Thread가 새로운 대안으로 부상
...
| 전신 패턴 | [Callback Pattern](/knowledge-base/studynote/11_design_supervision/06_exam_summary/409_architecture/) | Promise가 해결한 콜백 지옥의 원인 |
| 문법적 설탕 | async/await | Promise를 동기 코드처럼 표현하는 키워드 |
| 구현체 | Java CompletableFuture | Java의 Promise 유사체 |
| 구현체 | Kotlin [Coroutine](/knowledge-base/studynote/02_operating_system/02_process_thread/141_coroutine/) | 경량 [스레드](/knowledge-base/studynote/02_operating_system/02_process_thread/092_thread_lwp/) 기반 비동기 |
| 발전 패턴 | Observable (RxJS/Reactor) | 다중 비동기 이벤트 [스트림 처리](/knowledge-base/studynote/13_cloud_architecture/05_data_engineering/229_stream_processing_kafka_flink/) |
