...
> 3. **판단 포인트**: `async/await`는 Promise의 문법적 설탕(Syntactic Sugar) — Promise 위에서 동작하며, `await`는 Promise가 완료될 때까지 현재 함수를 일시 중단하되 [스레드](/studynote/02_operating_system/02_process_thread/092_thread_lwp/)를 블로킹하지 않는다.
...
| Java | `CompletableFuture` | 없음 (구조적 [동시성](/studynote/15_devops_sre/01_culture_methodology/014_concurrency/) Java 21+) | 명시적 체이닝 |
| Kotlin | `Deferred` | `suspend fun` | [코루틴](/studynote/02_operating_system/02_process_thread/141_coroutine/) 기반 |
| C# | `Task<T>` | `async/await` (C# 5.0) | 선구자 |
| Python | `asyncio.Future` | `async/await` (Python 3.5) | — |
| [Rust](/studynote/04_software_engineering/10_trends_pm_quality/782_memory_safety_rust_compiler_verification/) | `Future` | `async/await` ([Rust](/studynote/04_software_engineering/10_trends_pm_quality/782_memory_safety_rust_compiler_verification/) 1.39) | Zero-cost [추상화](/studynote/04_software_engineering/04_testing_quality/198_abstraction_control_data_process/) |
...
| 실수 | 문제 | 해결 |
|:---|:---|:---|
| `await`를 반복문 내부에서 순차 사용 | [직렬](/studynote/03_network/03_physical_layer_media/149_serial_communication_rs232_rs485/) 실행 -> 느림 | `Promise.all()` [병렬](/studynote/05_database/07_exam_summary/430_index_fast_full_scan/)화 |
| `.catch()` 누락 | 미처리 거부(UnhandledRejection) | 모든 체인에 `.catch()` |
| `async` 없이 `await` 사용 | SyntaxError | 함수에 `async` 명시 |
| CompletableFuture에서 [blocking](/studynote/02_operating_system/02_process_thread/122_sync_async_communication/) 호출 | [스레드 풀](/studynote/02_operating_system/02_process_thread/103_thread_pool/) 고갈 | `thenComposeAsync` 사용 |
...
- <strong><a href="/studynote/05_database/07_exam_summary/430_index_fast_full_scan/">병렬</a> 처리 용이</strong>: `Promise.all()` 한 줄로 [병렬](/studynote/05_database/07_exam_summary/430_index_fast_full_scan/) 비동기 실행
- **언어 표준화**: 모든 현대 언어/플랫폼에서 동일한 개념

**한계**:
- 무한 스트림(이벤트 스트림) 처리에는 Observable이 더 적합
- 취소(Cancellation) 지원이 약함 (AbortController 별도 필요)
- Kotlin [Coroutine](/studynote/02_operating_system/02_process_thread/141_coroutine/), Java Virtual Thread가 새로운 대안으로 부상

기술사 시험에서는 **Promise 상태 기계(Pending/Fulfilled/Rejected)**, **async/await가 Promise의 문법적 설탕임**, <strong>Promise.all vs Promise.race 차이</strong>를 명확히 서술하는 것이 핵심이다.
...
| 전신 패턴 | [Callback Pattern](/studynote/11_design_supervision/06_exam_summary/409_architecture/) | Promise가 해결한 콜백 지옥의 원인 |
| 문법적 설탕 | async/await | Promise를 동기 코드처럼 표현하는 키워드 |
| 구현체 | Java CompletableFuture | Java의 Promise 유사체 |
| 구현체 | Kotlin [Coroutine](/studynote/02_operating_system/02_process_thread/141_coroutine/) | 경량 [스레드](/studynote/02_operating_system/02_process_thread/092_thread_lwp/) 기반 비동기 |
| 발전 패턴 | Observable (RxJS/Reactor) | 다중 비동기 이벤트 [스트림 처리](/studynote/13_cloud_architecture/05_data_engineering/229_stream_processing_kafka_flink/) |
| 연관 패턴 | Monad Pattern | CompletableFuture는 모나드 |

### 📈 관련 키워드 및 발전 흐름도
콜백 비동기 -> Promise/Future 비동기 패턴 -> structured [concurrency](/studynote/05_database/05_distributed_nosql_newsql/266_other_transparency/)
...
<- **이전**: [220. 콜백 패턴 (Callback Pattern)](/studynote/11_design_supervision/04_gof_behavioral/220_callback_pattern/)
**다음**: [222. 모킹과 단위 테스트 (Mocking / Unit Test / Test Double)](/studynote/11_design_supervision/04_gof_behavioral/222_mocking_unit_test_stub/) ->
