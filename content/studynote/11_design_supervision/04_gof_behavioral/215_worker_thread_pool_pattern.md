...
기술사 문제에서는 <strong><code>ThreadPoolExecutor</code> 파라미터의 역할</strong>과 <strong>스레드 수 결정 공식</strong>을 정확히 서술하고, <strong>거부 정책(RejectedExecutionHandler)</strong> 별 차이를 비교하는 것이 핵심이다.
