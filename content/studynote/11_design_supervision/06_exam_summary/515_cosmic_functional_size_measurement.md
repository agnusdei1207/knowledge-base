---
title: "COSMIC Functional Size Measurement"
date: "2026-05-09"
tags:
  - "studynote-design-supervision"
---

## 핵심 인사이트 (3줄 요약)

> 1. **본질**: COSMIC FSM(ISO/IEC 19761)은 소프트웨어의 **기능 사용자 요구사항(FUR)**을 **진입(E)/출력(X)/읽기(R)/쓰기(W)** 4종의 데이터 이동(Data Movement, DM) 단위로 분해·계수하여 1 CFP(COSMIC Function Point) = 1 DM의 가산적 척도로 소프트웨어의 기능적 크기를
## 🔗 이전/다음 글 (Navigation)

**진행 상황**: 515 / 600

<- **이전**: [514. COCOMO 비용 산정 모델](/studynote/11_design_supervision/06_exam_summary/514_cocomo_cost_estimation_model)
**다음**: [516. 위험 관리 프레임워크 리스크 평가](/studynote/11_design_supervision/06_exam_summary/516_risk_management_framework_assessment/) ->

---
