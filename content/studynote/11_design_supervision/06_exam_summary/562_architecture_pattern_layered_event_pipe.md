...
레이어드 이벤트 파이프는 크게 **4개의 논리 계층**과 **3개의 경계 인터페이스**로 구성된다. 각 계층은 단방향 의존성(Unidirectional Dependency)을 가지며, 이벤트 파이프 계층은 양방향으로 흐를 수 있지만 일반적으로 다운스트림(↓) 흐름이 우세하다.
