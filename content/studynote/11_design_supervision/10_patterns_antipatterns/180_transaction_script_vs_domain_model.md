...
기술사 답안에서는 <strong>"Transaction Script는 단순 유스케이스를 빠르게 구현하는 데 적합하고, Domain Model은 복잡한 비즈니스 규칙과 상태 전이를 객체에 응집시켜 유지보수성과 테스트성을 높이는 구조이며, 선택 기준은 규칙 결합도와 변경 빈도"</strong>라고 정리하면 된다.
