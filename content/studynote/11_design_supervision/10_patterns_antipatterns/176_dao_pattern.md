...
DAO를 제대로 이해하려면 Repository, [Active](/knowledge-base/studynote/03_network/09_application_layer_web_email/483_active_vs_passive_ftp/) Record, DTO ([Data](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) Transfer Object)와의 경계를 분명히 해야 한다. 이름은 함께 등장하지만 바라보는 층이 다르다. DAO는 주로 **저장 기술과 질의 실행**에 초점이 있고, Repository는 **[도메인](/knowledge-base/studynote/05_database/02_modeling_normalization/064_relation_domain/) 집합과 [애그리게이트](/knowledge-base/studynote/04_software_engineering/04_testing_quality/222_aggregate_ddd_transaction_consistency/) 조회**에 더 가깝다.
...
다만 DAO는 만능 패턴이 아니다. 저장소 단순성이 높은 시스템에서는 과도한 [추상화](/knowledge-base/studynote/04_software_engineering/04_testing_quality/198_abstraction_control_data_process/)가 오히려 읽기 어려운 코드를 만들 수 있다. 따라서 DAO를 기억할 때는 "무조건 한 계층 더 추가"가 아니라 **"저장소 세부사항을 적절한 경계 뒤에 숨기는 패턴"**으로 이해하는 것이 정확하다.
