...
요구가 불확실할수록 정답은 거대한 선행 설계가 아니라 변경하기 쉬운 작은 구조다. 그래서 YAGNI는 [[036_mvp|MVP]], [[004_agile_relation|애자일]], 점진적 리팩터링을 가능하게 하는 출발점으로 다뤄진다.
...
| 단순한 시작 | [[198_abstraction_control_data_process|추상화]]는 반복되는 변화가 확인된 뒤 도입한다. | "중복이 현실인가, 상상인가?" |
...
| :--- | :--- | :--- | :--- |
| [[362_yagni|YAGNI]] | "지금 꼭 필요한가?" | 현재 가치 중심으로 설계를 줄인다. | 기능 선행 구현 자체를 직접 금지한다. |
| [[249_kiss_keep_it_simple_stupid|KISS]] | "더 단순하게 풀 수 있는가?" | 복잡도를 줄여 이해와 수정을 쉽게 한다. | 구현 방식의 단순화에 더 초점을 둔다. |
| DRY | "같은 지식이 중복되는가?" | 불필요한 코드 증식을 막는다. | 너무 이른 공통화는 오히려 [[362_yagni|YAGNI]] 위반이 될 수 있다. |
| [[746_ocp|OCP]] | "변경 없이 확장 가능한가?" | 변화 대응력을 중시한다. | YAGNI는 확장 지점의 조기 도입을 경계한다. |

YAGNI는 다른 설계 원칙을 부정하지 않는다. 다만 원칙 적용의 시점을 늦춰, 실제 변화 패턴이 확인된 뒤 [[198_abstraction_control_data_process|추상화]]와 일반화를 하라고 요구한다.
- **📢 섹션 요약 비유**: 여행 짐을 쌀 때 YAGNI는 "혹시 모르니"라는 이유로 가져가는 물건부터 빼내는 정리 원칙입니다.

## Ⅳ. 실무 적용 및 기술사 판단
실무에서는 API의 예비 파라미터, 데이터베이스의 `extra1`, `extra2` 같은 컬럼, 인터페이스 하나에 구현체 하나뿐인 구조가 대표적인 [[362_yagni|YAGNI]] 위반 신호다. 기술사 판단에서는 단순히 기능을 적게 만드는 태도가 아니라, 실제 수요와 변경 비용을 근거로 설계 범위를 통제하는 능력이 중요하다.

요구가 확실해질 때까지는 단순하게 유지하되, [[192_module_independence|모듈]] 경계와 테스트를 확보해 두어야 한다. 그래야 나중에 기능이 추가되어도 "미리 만들지 않았던 비용"보다 "뒤늦게 바꾸는 이익"이 더 커진다.

### 판단 [[435_checklist_based_testing|체크리스트]]
- 현재 백로그·요구사항 문서·사용 시나리오에 없는 기능을 선행 구현하고 있지 않은가?
- 단 하나의 예상 고객이나 막연한 미래 시나리오 때문에 [[198_abstraction_control_data_process|추상화]] 계층을 늘리고 있지 않은가?
- 실제로 호출되지 않는 [[014_api_posix|API]] 필드, DB 컬럼, [[009_config|설정]] 키가 배포 산출물에 포함되어 있지 않은가?
- 확장이 필요해질 경우를 대비한 테스트, [[192_module_independence|모듈]] 경계, 리팩터링 여지는 확보되어 있는가?
...
### 📌 관련 개념 맵
- **[[073_xp_extreme_programming|XP]]**: "가장 단순하게 동작하는 것"을 우선 구현한다.
- **[[249_kiss_keep_it_simple_stupid|KISS]]**: 현재 구현의 복잡도를 줄이는 방향으로 함께 작동한다.
- **[[036_mvp|MVP]]**: 최소 기능으로 시장 검증을 수행한다.
- **[[078_refactoring_code_smells|Refactoring]]**: 실제 요구가 생긴 뒤 구조를 안전하게 확장한다.
...
← **이전**: [[108_kiss_principle|108. KISS 원칙 (Keep It Simple, Stupid)]]
**다음**: [[109_yagni_you_arent_gonna_need_it|109. 불필요한 기능 구현 금지 원칙 (YAGNI, You Aren't Gonna Need It)]] →
