...
> 2. [[056_objective_evidence_collection|객관적 증거]]는 "[[395_verification_process_review|검증]] 가능하고 재현 가능한 사실"이어야 하며, 감리원의 주관적 판단이나 추론이 아닌 문서·[[568_logs_distributed_logging_elk_fluentd|로그]]·시스템 출력·직접 관찰 결과를 근거로 해야 한다는 것이 감리 표준(ISO 19011)의 핵심 원칙이다.
> 3. 직접 진단(테스트) 기법은 가장 강력한 증거를 제공하지만 시스템 영향 가능성 때문에 사전 협의와 테스트 환경 설정이 필수이며, 샘플링 방법(MUS, [[082_attribute_types_er_model|속성]] 샘플링)의 신뢰수준이 증거의 통계적 완전성을 결정한다.
...
## Ⅴ. 실무 시나리오 — [[187_information_system_audit|정보시스템 감리]]
...
> 📢 **섹션 요약 비유**: 감리 증거 삼각화는 진찰 + 검사 + 직접 [[396_validation|확인]] — 의사가 "어디 아파요?"(면담) + "차트 [[396_validation|확인]]"(문서) + "직접 촉진"(진단)으로 정확한 진단.
...
1. 감리 증거 수집은 탐정 조사 — 사람에게 묻고(면담), 현장을 보고(관찰), 서류를 [[396_validation|확인]]하고(문서 검토), 직접 실험해보는(직접 진단) 4가지 방법을 써요!
2. 가장 강력한 증거는 "직접 [[396_validation|확인]]" — DB에 실제 쿼리를 날려서 암호화가 됐는지 직접 보는 것처럼요.
...
← **이전**: [[042_objective_evidence_collection|42. 객관적 증거 수집 원칙 (Objective Evidence Collection)]]
**다음**: [[043_requirements_traceability_matrix|043. 요구사항 추적성 매트릭스 (RTM)]] →
