...
*   **기술사적 판단**: 감리 단계는 프로젝트리정비와 연동되므로,안역한 일정 변경은プロジェクト전체에영향을 미친다. 그러나 측시의완결성은 종료 감리의 핵심 선행 조건이므로, 충분한 논의가 필요하다. 감리인은 "종료 감리의 전제 조건으로서 테스트 완결성이 어느 정도 수준이어야 하는지"를 명확히 하고, 만약 그 수준에 미치지 못한다면 종료 감리 연기보다는 테스트완결을 위한 가이드를 제공하는 것이 더 적절한 대응이다.
