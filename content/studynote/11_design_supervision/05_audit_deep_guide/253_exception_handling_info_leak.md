...
> 1. **본질**: 예외 처리(Exception Handling) 미흡으로 인한 정보 노출은 [스택](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/057_stack/) 트레이스([Stack](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/057_stack/) Trace), DB 오류 메시지, 서버 경로 등이 공격자에게 직접 노출되는 심각한 취약점이다.
> 2. **가치**: 오류 메시지 하나가 SQL 테이블 구조, 서버 내부 경로, 사용 프레임워크 [버전](/knowledge-base/studynote/03_network/06_network_layer_ip/288_version_ihl_tos_total_length/)을 노출하여 후속 공격의 발판이 된다.
> 3. **판단 포인트**: 사용자에게 노출되는 오류 메시지는 "일반화된 안내 문구"만 표시하고, 상세 정보는 서버 측 [로그](/knowledge-base/studynote/04_software_engineering/09_cloud_native_ai_architecture/568_logs_distributed_logging_elk_fluentd/)에만 기록하는지 [확인](/knowledge-base/studynote/04_software_engineering/12_testing_maintenance/396_validation/)한다.

---

## Ⅰ. 개요 및 필요성
예외 처리(Exception Handling)의 보안 목표는 두 가지다. 첫째, 시스템 내부 정보를 외부에 노출하지 않는 것. 둘째, 예외 발생 시에도 서비스가 안전하게 지속되는 것이다. 행정안전부 개발보안 가이드의 "오류 메시지를 통한 정보 노출" 항목은 Critical 등급으로 [분류](/knowledge-base/studynote/16_bigdata/05_analysis/104_classification_analysis/)되어 있다.
...
- 개발 환경(`DEBUG` 모드)과 운영 환경(`PRODUCTION` 모드) [설정](/knowledge-base/studynote/15_devops_sre/01_culture_methodology/009_config/) 미분리
...
- **📢 섹션 요약 비유**: 전역 예외 처리기는 "병원 접수창구"다. 의사의 진단 내용(상세 오류)은 의무기록실(서버 [로그](/knowledge-base/studynote/04_software_engineering/09_cloud_native_ai_architecture/568_logs_distributed_logging_elk_fluentd/))에 보관되고, 환자(사용자)에게는 "다음에 다시 오세요"라는 안내만 전달한다.

---

## Ⅲ. 비교 및 연결
| 패턴 | [보안성](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/283_security_tactics/) | [유지보수성](/knowledge-base/studynote/04_software_engineering/06_software_architecture/346_maintainability_portability/) | 감리 판정 |
|:---|:---|:---|:---|
| `e.printStackTrace()` 직접 출력 | ❌ 취약 | △ 개발 중에만 유용 | 지적 필수 |
| [HTTP](/knowledge-base/studynote/03_network/09_application_layer_web_email/461_http_stateless_connection_oriented/) 500 + [스택](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/057_stack/) 트레이스 반환 | ❌ 취약 | △ | 지적 필수 |
| 전역 핸들러 + 일반 메시지 반환 | ✅ 안전 | ✅ | 권장 패턴 |
| 오류 코드 기반 응답 | ✅ 안전 | ✅ 추적 용이 | 권장 패턴 |
| 오류 발생 시 자동 알림(Slack/이메일) | ✅ 안전 | ✅✅ | 우수 사례 |

| [설정](/knowledge-base/studynote/15_devops_sre/01_culture_methodology/009_config/) 항목 | 개발 환경 | 운영 환경 |
|:---|:---|:---|
...
[로그](/knowledge-base/studynote/04_software_engineering/09_cloud_native_ai_architecture/568_logs_distributed_logging_elk_fluentd/)에 저장되는 상세 정보 또한 보안 관리가 필요하다. [로그](/knowledge-base/studynote/04_software_engineering/09_cloud_native_ai_architecture/568_logs_distributed_logging_elk_fluentd/)에 [개인정보](/knowledge-base/studynote/09_security/16_data_privacy/781_personal_information/)([Personal Information](/knowledge-base/studynote/09_security/16_data_privacy/781_personal_information/)), 비밀번호, [세션](/knowledge-base/studynote/02_operating_system/02_process_thread/160_session_controlling_terminal/) 토큰이 기록되지 않도록 마스킹(Masking) 처리가 필요하다.
...
- **📢 섹션 요약 비유**: [HTTP](/knowledge-base/studynote/03_network/09_application_layer_web_email/461_http_stateless_connection_oriented/) 응답 헤더에서 서버 [버전](/knowledge-base/studynote/03_network/06_network_layer_ip/288_version_ihl_tos_total_length/)을 노출하는 것은 "강도에게 '우리 집 자물쇠는 A사 2018년형'이라고 스티커를 붙여두는 것"이다. [버전](/knowledge-base/studynote/03_network/06_network_layer_ip/288_version_ihl_tos_total_length/) 정보만으로도 알려진 취약점을 즉시 찾아낼 수 있다.
...
- **📢 섹션 요약 비유**: 예외 처리 감리는 "비상구 표시등이 제대로 작동하는지, 하지만 비상구 열쇠 위치는 외부에 공개되지 않는지" 모두를 [확인](/knowledge-base/studynote/04_software_engineering/12_testing_maintenance/396_validation/)하는 이중 점검이다.
...
| 상위 개념 | OWASP A09 - [Security Misconfiguration](/knowledge-base/studynote/09_security/05_web_app_security/412_security_misconfiguration/) | 오류 [설정](/knowledge-base/studynote/15_devops_sre/01_culture_methodology/009_config/) 미흡 [분류](/knowledge-base/studynote/16_bigdata/05_analysis/104_classification_analysis/) |
| 하위 개념 | 전역 예외 처리기 (Global Exception Handler) | @ControllerAdvice, web.xml |
| 하위 개념 | [스택](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/057_stack/) 트레이스 ([Stack](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/057_stack/) Trace) | 내부 코드 구조 노출 정보 |
| 연관 개념 | [SIEM](/knowledge-base/studynote/09_security/13_secops_ir_forensics/624_siem/) ([Security](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/283_security_tactics/) Information and [Event Management](/knowledge-base/studynote/12_it_management/02_itsm_itil/074_event_management/)) | [로그](/knowledge-base/studynote/04_software_engineering/09_cloud_native_ai_architecture/568_logs_distributed_logging_elk_fluentd/) 통합 보안 모니터링 |
...
2. 안전한 오류 처리는 "넘어졌을 때 '잠깐만요'라고만 말하고, 자세한 진단은 의사 선생님(서버 [로그](/knowledge-base/studynote/04_software_engineering/09_cloud_native_ai_architecture/568_logs_distributed_logging_elk_fluentd/))한테만 전달"하는 거야.
