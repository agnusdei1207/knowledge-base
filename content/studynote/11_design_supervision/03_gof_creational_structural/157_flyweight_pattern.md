...
> 1. **본질**: [플라이웨이트](/knowledge-base/studynote/04_software_engineering/04_testing_quality/265_flyweight_pattern_instance_sharing/) ([Flyweight](/knowledge-base/studynote/04_software_engineering/04_testing_quality/265_flyweight_pattern_instance_sharing/)) 패턴은 반복 객체의 공통 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 공유 객체로 묶고, 변하는 값만 외부에서 주입해 메모리 사용을 줄이는 구조 패턴이다.
> 2. **가치**: 글자 렌더링, 게임 유닛, 지도 마커처럼 비슷한 객체가 수천·수만 개 생기는 상황에서 힙 메모리와 객체 [생성](/knowledge-base/studynote/02_operating_system/02_process_thread/087_process_state_transition/) 비용을 크게 낮출 수 있다.
...
- **📢 섹션 요약 비유**: 같은 도장을 여러 장 찍을 때 도장 자체는 하나만 두고, 찍는 종이 위치만 바꾸는 것이 [플라이웨이트](/knowledge-base/studynote/04_software_engineering/04_testing_quality/265_flyweight_pattern_instance_sharing/)의 생각이다.

---

## Ⅱ. 아키텍처 및 핵심 원리

[플라이웨이트](/knowledge-base/studynote/04_software_engineering/04_testing_quality/265_flyweight_pattern_instance_sharing/) 구조는 보통 클라이언트, [플라이웨이트](/knowledge-base/studynote/04_software_engineering/04_testing_quality/265_flyweight_pattern_instance_sharing/) 인터페이스, 구체 [플라이웨이트](/knowledge-base/studynote/04_software_engineering/04_testing_quality/265_flyweight_pattern_instance_sharing/), 그리고 공유 객체를 관리하는 팩토리로 이루어진다. 핵심은 객체가 스스로 모든 상태를 들고 있지 않는다는 점이다. 객체 내부에는 공유 가능한 핵심 정보만 남고, 실제 실행 맥락은 클라이언트가 메서드 호출 시 넘긴다.
...
| 구성 요소 | 역할 | 설계 포인트 |
| :--- | :--- | :--- |
| [Flyweight](/knowledge-base/studynote/04_software_engineering/04_testing_quality/265_flyweight_pattern_instance_sharing/) | 공유 객체 인터페이스 | 외부 상태를 파라미터로 받도록 설계 |
| Concrete [Flyweight](/knowledge-base/studynote/04_software_engineering/04_testing_quality/265_flyweight_pattern_instance_sharing/) | 실제 공유 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 보관 | 불변성 유지가 중요 |
| [Flyweight](/knowledge-base/studynote/04_software_engineering/04_testing_quality/265_flyweight_pattern_instance_sharing/) Factory | 공유 객체 조회·[생성](/knowledge-base/studynote/02_operating_system/02_process_thread/087_process_state_transition/) | 키 관리와 캐시 [전략](/knowledge-base/studynote/04_software_engineering/04_testing_quality/268_strategy_pattern/) 필요 |
| [Client](/knowledge-base/studynote/11_design_supervision/01_audit_framework/003_audit_stakeholders/) | 외부 상태 보유 | 호출 시 필요한 문맥 전달 |
...
또한 [플라이웨이트](/knowledge-base/studynote/04_software_engineering/04_testing_quality/265_flyweight_pattern_instance_sharing/)는 캐싱과 다르면서도 연결된다. 캐싱은 계산 결과나 조회 결과를 저장하는 개념이고, [플라이웨이트](/knowledge-base/studynote/04_software_engineering/04_testing_quality/265_flyweight_pattern_instance_sharing/)는 객체 구조 자체를 공유하는 패턴이다. 하지만 실무 구현에서는 팩토리가 사실상 키 기반 캐시처럼 동작하므로 두 개념이 함께 나타난다.
...
| :--- | :--- | :--- | :--- | :--- |
| 핵심 목적 | 공통 상태 공유 | 단일 인스턴스 보장 | [복제](/knowledge-base/studynote/14_data_engineering/01_infrastructure/016_replication_factor/) 기반 [생성](/knowledge-base/studynote/02_operating_system/02_process_thread/087_process_state_transition/) | 비싼 객체 재사용 |
| 객체 수 | 여러 개 가능 | 1개 | 필요 수만큼 [복제](/knowledge-base/studynote/14_data_engineering/01_infrastructure/016_replication_factor/) | 제한된 풀 크기 |
| 상태 [전략](/knowledge-base/studynote/04_software_engineering/04_testing_quality/268_strategy_pattern/) | 내부/외부 상태 분리 | 전역 상태 보유 가능 | [복제](/knowledge-base/studynote/14_data_engineering/01_infrastructure/016_replication_factor/) 후 수정 | 반환 후 재할당 |
| 대표 예 | 글자, 아이콘, 유닛 | [설정](/knowledge-base/studynote/15_devops_sre/01_culture_methodology/009_config/) 관리자 | 템플릿 객체 | DB 연결, [스레드](/knowledge-base/studynote/02_operating_system/02_process_thread/092_thread_lwp/) |

연결 관점에서도 의미가 있다. 자바 문자열 풀 (String Pool)은 동일 문자열 리터럴을 공유한다는 점에서 [플라이웨이트](/knowledge-base/studynote/04_software_engineering/04_testing_quality/265_flyweight_pattern_instance_sharing/)적 사고와 닮아 있고, 렌더링 엔진이나 지도 엔진의 심볼 관리도 같은 맥락이다. 즉 이 패턴은 GoF (Gang of Four) 구조 패턴이면서, 실제로는 메모리 아키텍처와 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) 최적화 문제에 직접 연결된다.

중요한 경계는 "공유가 항상 좋은가"가 아니라 "공유할수록 이득이 큰가"다. 외부 상태 전달 비용이 너무 크거나, [동시성](/knowledge-base/studynote/15_devops_sre/01_culture_methodology/014_concurrency/) 제어가 복잡해지면 오히려 일반 객체가 더 단순하고 안전할 수 있다.

- **📢 섹션 요약 비유**: 한 명당 우산을 전부 사 주는 것과, 같은 모양 우산을 창고에 두고 필요할 때 꺼내 쓰는 것의 차이가 [플라이웨이트](/knowledge-base/studynote/04_software_engineering/04_testing_quality/265_flyweight_pattern_instance_sharing/) 비교의 핵심이다.

---

## Ⅳ. 실무 적용 및 기술사 판단

실무에서 [플라이웨이트](/knowledge-base/studynote/04_software_engineering/04_testing_quality/265_flyweight_pattern_instance_sharing/)를 채택해야 하는 대표 상황은 동일한 속성을 지닌 객체가 대량 발생하고, 그중 상당 부분이 불변 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)일 때다. 예를 들어 워드프로세서의 글리프 (Glyph), 게임의 배경 타일, 지도 서비스의 공통 아이콘, 임베디드 장비의 정적 UI 자원은 [플라이웨이트](/knowledge-base/studynote/04_software_engineering/04_testing_quality/265_flyweight_pattern_instance_sharing/) 적용 효과가 크다. 반대로 각 객체가 개별 생명주기와 복잡한 변경 상태를 갖는 [도메인](/knowledge-base/studynote/05_database/02_modeling_normalization/064_relation_domain/) 객체라면 적용 이득이 줄어든다.

### 채택 판단 [체크리스트](/knowledge-base/studynote/04_software_engineering/11_testing_validation/435_checklist_based_testing/)

1. 동일 속성을 가진 객체가 반복적으로 매우 많이 [생성](/knowledge-base/studynote/02_operating_system/02_process_thread/087_process_state_transition/)되는가?
2. 공유 가능한 내부 상태의 크기가 충분히 큰가?
3. 외부 상태를 호출 시 안전하게 주입할 수 있는가?
4. 객체 정체성보다 메모리 효율이 더 중요한가?
5. 팩토리 캐시의 키 충돌과 [동시성](/knowledge-base/studynote/15_devops_sre/01_culture_methodology/014_concurrency/) 제어를 감당할 수 있는가?
...
- **📢 섹션 요약 비유**: 같은 벽돌로 여러 집을 짓되, 벽돌은 공장에서 공용으로 만들고 집의 모양만 현장에서 다르게 조립하는 생각이 [플라이웨이트](/knowledge-base/studynote/04_software_engineering/04_testing_quality/265_flyweight_pattern_instance_sharing/)다.
...
| 개념 | 연결 포인트 |
| :--- | :--- |
| Intrinsic [State](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/272_state_pattern/) | 공유 객체 내부에 저장되는 불변 공통 정보 |
| Extrinsic [State](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/272_state_pattern/) | 호출 시 외부에서 주입되는 가변 문맥 정보 |
...
1. 같은 나무 그림을 천 번 그릴 때, 그림판을 천 개 만드는 대신 나무 도장 하나를 같이 쓰는 게 [플라이웨이트](/knowledge-base/studynote/04_software_engineering/04_testing_quality/265_flyweight_pattern_instance_sharing/)예요.
