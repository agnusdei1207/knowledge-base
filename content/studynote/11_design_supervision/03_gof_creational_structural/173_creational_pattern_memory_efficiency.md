...
> 1. **본질**: Prototype은 이미 준비된 객체를 [복제](/knowledge-base/studynote/14_data_engineering/01_infrastructure/016_replication_factor/)해 [생성](/knowledge-base/studynote/02_operating_system/02_process_thread/087_process_state_transition/) 비용을 줄이는 패턴이고, Flyweight는 공유 가능한 상태를 묶어 대량 객체의 상주 메모리를 줄이는 패턴이다.
...
객체 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) 문제는 흔히 한 단어로 "[생성](/knowledge-base/studynote/02_operating_system/02_process_thread/087_process_state_transition/) 비용"이라고 묶지만, 실제로는 두 종류가 섞여 있다. 하나는 객체 하나를 만드는 데 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 로딩·파싱·복잡한 [그래프](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/070_graph_datastructure/) 구성처럼 시간이 오래 걸리는 경우이고, 다른 하나는 비슷한 객체가 수만 개 동시에 존재해 메모리와 GC ([Garbage Collection](/knowledge-base/studynote/02_operating_system/06_memory_management/380_garbage_collection/)) 부담이 커지는 경우다. 이 둘을 같은 방식으로 해결하려 하면 대개 과설계나 역효과가 난다.

Prototype은 첫 번째 문제에 답한다. 비싼 초기화를 한 번 수행한 원형 객체를 준비해 두고, 필요할 때 [복제](/knowledge-base/studynote/14_data_engineering/01_infrastructure/016_replication_factor/)해서 조금씩 수정하는 방식이다. 반면 Flyweight는 두 번째 문제에 답한다. 모든 객체가 따로 들고 있을 필요가 없는 공통 상태를 외부에서 공유하게 만들어, 개별 인스턴스의 무게를 최소화한다.
...
Prototype의 핵심 메커니즘은 "초기화 비용을 앞당겨 치르고, 이후에는 복사 비용으로 바꾸는 것"이다. 보통 [Prototype](/knowledge-base/studynote/04_software_engineering/04_testing_quality/257_prototype_pattern_object_cloning/) Registry에 원형 객체를 보관해 두고, 클라이언트가 `clone()` 또는 복사 [생성](/knowledge-base/studynote/02_operating_system/02_process_thread/087_process_state_transition/)자를 통해 새 객체를 만든다. 이때 가장 중요한 설계 포인트는 얕은 복사 (Shallow Copy)와 깊은 복사 (Deep Copy)를 어디까지 허용할지다. 내부 [그래프](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/070_graph_datastructure/)가 mutable하면 얕은 복사는 쉽게 공유 버그를 만든다.
...
| :--- | :--- | :--- |
| 절감 대상 | [생성](/knowledge-base/studynote/02_operating_system/02_process_thread/087_process_state_transition/) 시간 | 상주 메모리 |
| 핵심 동작 | 원형 [복제](/knowledge-base/studynote/14_data_engineering/01_infrastructure/016_replication_factor/) | 공유 객체 재사용 |
| 설계 포인트 | 복사 범위와 독립성 보장 | 내재/외재 상태 분리 |
| 실패 원인 | 얕은 복사로 인한 [참조](/knowledge-base/studynote/05_database/05_distributed_nosql_newsql/316_reference_pattern_nosql/) 공유 버그 | mutable 공유 상태, 과도한 외재 상태 |

예를 들어 CAD (Computer-Aided Design) 모델처럼 로딩이 무거운 객체는 Prototype의 이점을 크게 본다. 반대로 텍스트 에디터의 글자 glyph, 게임 탄환 스프라이트, JVM (Java [Virtual Machine](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/598_vm_migration_nic/))의 String Pool처럼 수많은 유사 객체가 동시에 존재하는 경우는 Flyweight가 더 직접적이다. 즉 구조는 달라도 둘 다 자원 사용의 "분모"를 줄이는 패턴이라는 점에서 효율화 논의에 함께 등장한다.

- **📢 섹션 요약 비유**: Prototype은 마스터 반죽을 만들어 [쿠키](/knowledge-base/studynote/03_network/09_application_layer_web_email/475_cookie_local_state/)를 빠르게 찍어내는 것이고, Flyweight는 같은 [쿠키](/knowledge-base/studynote/03_network/09_application_layer_web_email/475_cookie_local_state/) 틀을 모두가 함께 쓰고 각자 초콜릿 장식만 따로 얹는 방식이다.

---

## Ⅲ. 비교 및 연결

Prototype과 Flyweight를 구분하는 가장 좋은 질문은 "객체가 태어날 때 비싼가, 아니면 많이 살아 있을 때 비싼가"다. Prototype은 객체 수가 늘어도 각 인스턴스가 독립된 객체라는 사실을 바꾸지 않는다. 반면 Flyweight는 인스턴스 개념 자체를 가볍게 만들어, 동일한 내부 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 공유하는 쪽으로 설계를 바꾼다.

| 비교 축 | [Prototype](/knowledge-base/studynote/04_software_engineering/04_testing_quality/257_prototype_pattern_object_cloning/) | [Flyweight](/knowledge-base/studynote/04_software_engineering/04_testing_quality/265_flyweight_pattern_instance_sharing/) | Object Pool |
| :--- | :--- | :--- | :--- |
| 주 목적 | [생성](/knowledge-base/studynote/02_operating_system/02_process_thread/087_process_state_transition/) 시간 절감 | 메모리 절감 | 재사용 가능한 객체 회전 |
| 인스턴스 수 | [복제](/knowledge-base/studynote/14_data_engineering/01_infrastructure/016_replication_factor/)할수록 증가 | 공유 가능한 부분은 감소 | 제한된 수를 반복 사용 |
...
실무 판단은 숫자로 보는 것이 가장 명확하다. 객체 [생성](/knowledge-base/studynote/02_operating_system/02_process_thread/087_process_state_transition/) 한 번에 500ms가 걸리고 같은 템플릿을 수백 번 만들어야 한다면 Prototype이 유리하다. 반대로 100,000개의 총알 객체가 동시에 떠 있고 그중 90%가 같은 이미지와 공격 속성을 공유한다면, Flyweight가 메모리와 GC 부담을 훨씬 크게 줄인다.

### 실무 시나리오

| 상황 | 더 적합한 선택 | 이유 |
| :--- | :--- | :--- |
| 복잡한 보고서 템플릿, 3D 모델, 대형 [설정](/knowledge-base/studynote/15_devops_sre/01_culture_methodology/009_config/) 객체 [복제](/knowledge-base/studynote/14_data_engineering/01_infrastructure/016_replication_factor/) | [Prototype](/knowledge-base/studynote/04_software_engineering/04_testing_quality/257_prototype_pattern_object_cloning/) | 비싼 초기화를 [복제](/knowledge-base/studynote/14_data_engineering/01_infrastructure/016_replication_factor/)로 대체 |
| 문서 편집기의 문자 glyph, 게임 탄환, 아이콘 캐시 | [Flyweight](/knowledge-base/studynote/04_software_engineering/04_testing_quality/265_flyweight_pattern_instance_sharing/) | 공통 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 공유해 상주 메모리 절감 |
| 복잡한 템플릿 안에 반복 부품도 많은 경우 | 병행 적용 | 큰 틀은 [복제](/knowledge-base/studynote/14_data_engineering/01_infrastructure/016_replication_factor/)하고 세부 부품은 공유 |
...
1. [생성](/knowledge-base/studynote/02_operating_system/02_process_thread/087_process_state_transition/)자 호출보다 복사 비용이 실제로 더 싼가?
2. [Prototype](/knowledge-base/studynote/04_software_engineering/04_testing_quality/257_prototype_pattern_object_cloning/) 대상 객체 안에 mutable [참조](/knowledge-base/studynote/05_database/05_distributed_nosql_newsql/316_reference_pattern_nosql/) [그래프](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/070_graph_datastructure/)가 있는가?
...
- [Flyweight](/knowledge-base/studynote/04_software_engineering/04_testing_quality/265_flyweight_pattern_instance_sharing/) 객체를 mutable하게 만들어 공유 인스턴스 전체가 오염되는 것
...
| 개념 | 연결 포인트 |
| :--- | :--- |
| Shallow Copy | Prototype에서 빠르지만 [참조](/knowledge-base/studynote/05_database/05_distributed_nosql_newsql/316_reference_pattern_nosql/) 공유 위험이 있는 복사 방식이다. |
| Deep Copy | Prototype에서 독립성을 높이지만 비용이 큰 복사 방식이다. |
| Intrinsic [State](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/272_state_pattern/) | Flyweight가 공유하는 불변 내부 상태다. |
| Extrinsic [State](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/272_state_pattern/) | [Flyweight](/knowledge-base/studynote/04_software_engineering/04_testing_quality/265_flyweight_pattern_instance_sharing/) 사용 시 호출자가 따로 들고 다니는 상태다. |
| Object Pool | 재사용 관점의 최적화 패턴으로 Flyweight와 다르다. |
| [Builder](/knowledge-base/studynote/04_software_engineering/04_testing_quality/256_builder_pattern_step_by_step_creation/) | 복잡한 [생성](/knowledge-base/studynote/02_operating_system/02_process_thread/087_process_state_transition/) 과정을 제어하는 패턴으로 Prototype과 조합될 수 있다. |
| String Pool | [Flyweight](/knowledge-base/studynote/04_software_engineering/04_testing_quality/265_flyweight_pattern_instance_sharing/) 개념이 언어 런타임에 적용된 대표 사례다. |
...
← **이전**: [172. 빌더 패턴을 활용한 불변 객체 (Immutable Object) 설계](/knowledge-base/studynote/11_design_supervision/03_gof_creational_structural/172_builder_immutable_object/)
**다음**: [174. J2EE 프레임워크 패턴 (J2EE Framework Patterns)](/knowledge-base/studynote/11_design_supervision/10_patterns_antipatterns/174_j2ee_framework_patterns/) →
