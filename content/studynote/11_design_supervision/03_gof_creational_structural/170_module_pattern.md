...
> 1. **본질**: [모듈](/knowledge-base/studynote/04_software_engineering/04_testing_quality/192_module_independence/) 패턴 ([Module](/knowledge-base/studynote/04_software_engineering/04_testing_quality/192_module_independence/) Pattern)은 클로저 (Closure)로 private 상태를 감춘 뒤, 공개할 메서드만 반환해 [정보 은닉](/knowledge-base/studynote/04_software_engineering/04_testing_quality/199_information_hiding_encapsulation/) ([Information Hiding](/knowledge-base/studynote/04_software_engineering/04_testing_quality/199_information_hiding_encapsulation/))과 응집된 [API](/knowledge-base/studynote/02_operating_system/01_overview_architecture/014_api_posix/) ([Application Programming Interface](/knowledge-base/studynote/02_operating_system/01_overview_architecture/014_api_posix/))를 만드는 구조다.
> 2. **가치**: 전역 [네임스페이스](/knowledge-base/studynote/02_operating_system/01_overview_architecture/061_namespace/) 오염과 내부 구현 노출을 줄여, [초기](/knowledge-base/studynote/03_network/08_transport_layer/459_quic_fec_forward_error_correction/) JavaScript 같은 환경에서도 클래스 없이 캡슐화와 책임 경계를 만들 수 있다.
> 3. **판단 포인트**: 현대 프로젝트에서는 ECMAScript Module을 우선 쓰되, 런타임 private 상태를 강하게 숨기거나 레거시 브라우저·플러그인 구조를 유지해야 할 때 [모듈](/knowledge-base/studynote/04_software_engineering/04_testing_quality/192_module_independence/) 패턴이 여전히 유효하다.

---

## Ⅰ. 개요 및 필요성

[모듈](/knowledge-base/studynote/04_software_engineering/04_testing_quality/192_module_independence/) 패턴은 관련된 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)와 함수를 하나의 폐쇄된 스코프에 묶고, 외부에는 제한된 인터페이스만 노출하는 설계 방식이다. [초기](/knowledge-base/studynote/03_network/08_transport_layer/459_quic_fec_forward_error_correction/) JavaScript 환경에서는 [파일](/knowledge-base/studynote/02_operating_system/09_file_system/501_file_definition_logical_record/)을 로드하는 순간 전역 객체 `window`에 값이 흩어지기 쉬웠고, [라이브러리](/knowledge-base/studynote/04_software_engineering/06_software_architecture/336_library_vs_framework/)끼리 같은 변수명이나 함수명을 사용하면 충돌이 자주 발생했다. 또한 내부 헬퍼 함수와 임시 상태까지 전부 외부에서 접근 가능하면, 구현 교체가 곧 외부 [호환성](/knowledge-base/studynote/04_software_engineering/06_software_architecture/344_compatibility_usability/) 문제로 번졌다.
...
[모듈](/knowledge-base/studynote/04_software_engineering/04_testing_quality/192_module_independence/) 패턴의 런타임 동작은 세 단계로 볼 수 있다. 첫째, IIFE나 팩터리 함수가 실행되며 lexical scope가 만들어진다. 둘째, 그 내부에 private 변수와 helper 함수를 둔다. 셋째, 외부에 보여 줄 메서드만 담은 객체를 반환한다. 반환된 메서드들은 클로저를 통해 내부 스코프를 계속 참조하므로, 함수 실행이 끝난 뒤에도 private 상태가 살아남는다.
...
| 구성 요소 | 역할 | 설계 포인트 |
| :--- | :--- | :--- |
| private [state](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/272_state_pattern/) | 외부에 숨길 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 저장 | 직접 참조를 노출하지 않아야 진짜 은닉이 됨 |
| private helper | [검증](/knowledge-base/studynote/04_software_engineering/07_object_oriented/395_verification_process_review/), 변환, 캐시 계산 | public 메서드보다 자유롭게 변경 가능 |
...
[모듈](/knowledge-base/studynote/04_software_engineering/04_testing_quality/192_module_independence/) 패턴을 정확히 이해하려면 "단순 [네임스페이스](/knowledge-base/studynote/02_operating_system/01_overview_architecture/061_namespace/)", "클래스", "언어 차원의 [모듈](/knowledge-base/studynote/04_software_engineering/04_testing_quality/192_module_independence/)"과 구분해야 한다. 이름이 비슷해도 은닉 수준과 [생성](/knowledge-base/studynote/02_operating_system/02_process_thread/087_process_state_transition/) 방식이 다르다.

| 비교 대상 | 은닉 방식 | 인스턴스 모델 | 장점 | 한계 |
| :--- | :--- | :--- | :--- | :--- |
| [네임스페이스](/knowledge-base/studynote/02_operating_system/01_overview_architecture/061_namespace/) 객체 | 관례상 구분 | 보통 싱글턴 | 단순함 | 실제 private 보장 없음 |
| [모듈](/knowledge-base/studynote/04_software_engineering/04_testing_quality/192_module_independence/) 패턴 | 클로저 스코프 | 보통 싱글턴 | 강한 런타임 은닉 | [정적 분석](/knowledge-base/studynote/04_software_engineering/06_software_architecture/331_static_analysis/), tree-shaking에 불리 |
...
실무에서 [모듈](/knowledge-base/studynote/04_software_engineering/04_testing_quality/192_module_independence/) 패턴은 레거시 브라우저 코드, jQuery 플러그인, [서드파티](/knowledge-base/studynote/05_database/06_dw_olap_trends/385_third_party_cookie_deprecation_cdw/) 위젯, 공용 [설정](/knowledge-base/studynote/15_devops_sre/01_culture_methodology/009_config/) 관리자 같은 영역에서 자주 만난다. 특히 외부에는 작은 API만 노출하고, 내부에서는 캐시·[카운터](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/059_counter/)·상태 전이를 숨기고 싶을 때 유용하다. 반대로 화면 컴포넌트처럼 동일 구조를 여러 개 만들어야 한다면, 클래스나 팩터리 함수가 더 자연스럽다.
...
5. 여러 인스턴스가 필요한 문제를 억지로 싱글턴 [모듈](/knowledge-base/studynote/04_software_engineering/04_testing_quality/192_module_independence/)로 풀고 있지는 않은가?

### 자주 발생하는 [안티패턴](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/128_water_scrum_fall_anti_pattern/)

- 하나의 [모듈](/knowledge-base/studynote/04_software_engineering/04_testing_quality/192_module_independence/) 안에 화면 제어, 네트워크 호출, [도메인](/knowledge-base/studynote/05_database/02_modeling_normalization/064_relation_domain/) 로직을 모두 넣어 거대한 God Module로 만드는 것
- private 배열이나 객체를 그대로 반환해 외부가 내부 상태를 우회 수정하게 만드는 것
- 숨겨진 싱글턴 상태 때문에 테스트 순서에 따라 결과가 달라지는 것
- 레거시 대응이 아닌데도 최신 도구 체인을 포기하고 [모듈](/knowledge-base/studynote/04_software_engineering/04_testing_quality/192_module_independence/) 패턴만 고집하는 것
...
[모듈](/knowledge-base/studynote/04_software_engineering/04_testing_quality/192_module_independence/) 패턴의 기대효과는 명확하다. 전역 노출을 줄여 충돌 위험을 낮추고, public API만 계약으로 유지해 내부 변경 자유도를 높이며, 관련 기능을 응집된 단위로 묶어 이해 비용을 줄인다. 특히 "작은 외부 인터페이스, 숨겨진 내부 상태"가 중요한 [라이브러리](/knowledge-base/studynote/04_software_engineering/06_software_architecture/336_library_vs_framework/)나 소프트웨어 개발 키트 (SDK, Software Development Kit)에서는 여전히 설계적 의미가 있다.
...
| IIFE (Immediately Invoked Function Expression) | 전통적 [모듈](/knowledge-base/studynote/04_software_engineering/04_testing_quality/192_module_independence/) 패턴의 대표 구현 수단 |
| ECMAScript [Module](/knowledge-base/studynote/04_software_engineering/04_testing_quality/192_module_independence/) | 현대 표준 [모듈](/knowledge-base/studynote/04_software_engineering/04_testing_quality/192_module_independence/) 시스템으로, [파일](/knowledge-base/studynote/02_operating_system/09_file_system/501_file_definition_logical_record/) 단위 캡슐화 제공 |
| Revealing [Module](/knowledge-base/studynote/04_software_engineering/04_testing_quality/192_module_independence/) Pattern | 반환 객체에 공개 멤버만 명시적으로 나열하는 변형 |
| CommonJS / AMD | [모듈](/knowledge-base/studynote/04_software_engineering/04_testing_quality/192_module_independence/) 패턴 이후 등장한 [파일](/knowledge-base/studynote/02_operating_system/09_file_system/501_file_definition_logical_record/) 기반 [모듈](/knowledge-base/studynote/04_software_engineering/04_testing_quality/192_module_independence/) 시스템 |
...
1. [모듈](/knowledge-base/studynote/04_software_engineering/04_testing_quality/192_module_independence/) 패턴은 장난감을 넣어 두는 상자를 만들고, 밖에는 손잡이만 남겨 두는 것과 같아요.
