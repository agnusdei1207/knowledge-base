...
|:---|:---|:---|:---|:---|
| **균등 현금흐름 시 PP** | 일정한 수익 모델 계산 | $PP = \frac{[[459_quic_fec_forward_error_correction|초기]] 투자비용}{연간 순현금유입액}$ | [[309_saas|SaaS]] 구독료 등 안정적 수익일 때 | 매달 고정 월급 받기 |
...
이 구조의 핵심은 단순 PP가 돈의 시간 가치를 무시하여 본전 회수를 '지나치게 낙관적'으로 당겨 잡는다는 치명적 오류를 보여준다. 할인율을 적용하여 10년 뒤의 수익 가치를 깎아버리면, 실제 원금을 회수하는 데 걸리는 시간(DPP)은 훨씬 뒤로 밀리거나 아예 영원히 회수하지 못할 수도 있다. 따라서 실무에서는 보수적인 [[096_risk_non_risk_architecture_evaluation_flaws|리스크]] 관리가 필요할 때 단순 PP를 버리고 반드시 DPP를 요구해야 한다.
...
| 비교 항목 | PP (회수기간법) | [[013_npv|NPV]] (순현재가치) / [[014_irr_internal_rate_of_return|IRR]] |
|:---|:---|:---|
| **평가의 목적** | 자금의 **유동성(Liquidity)** 확보와 [[096_risk_non_risk_architecture_evaluation_flaws|리스크]] 타임 단축 | 투자의 절대적/상대적 **수익성(Profitability)** 극대화 |
...
아래는 수익성 지표([[013_npv|NPV]])와 유동성 지표(PP)가 충돌할 때 경영진의 성향에 따라 달라지는 의사결정 트리이다.
...
📢 **섹션 요약 비유**: 우물 파기(투자)를 할 때, 내일 당장 물 한 모금이 나와서 목마름을 면하게 해주는 우물(PP 우선)과, 일주일을 파야 하지만 평생 먹을 생수가 터지는 우물([[013_npv|NPV]] 우선) 중 내가 당장 얼마나 목마른지에 따라 선택이 달라지는 것과 같다.

---

### Ⅳ. 실무 적용 및 기술사적 판단 ([[268_strategy_pattern|Strategy]] & Decision)

실무에서 PP를 단독 의사결정 지표로 사용하면 우량 프로젝트를 기각하거나 악성 프로젝트를 통과시키는 심각한 [[128_water_scrum_fall_anti_pattern|안티패턴]]이 발생한다.

1. **꼬리표 수익의 무시 ([[128_water_scrum_fall_anti_pattern|안티패턴]])**: 플랫폼 비즈니스나 [[309_saas|SaaS]] 모델 투자는 [[459_quic_fec_forward_error_correction|초기]]에 대규모 서버와 마케팅 비용이 들고, 임계점을 넘은 뒤(J-Curve 효과) 4~5년 차부터 막대한 이익을 거둬들인다. PP만 기준으로 삼으면 이러한 미래의 캐시카우(Cash [[542_cow_file_system|Cow]]) 투자는 '원금 회수가 너무 느리다'는 이유로 모조리 기각되고, 1년짜리 소규모 유지보수 계약만 수주하는 단기 성과주의에 조직이 매몰된다.
...
도입 전 필수 [[435_checklist_based_testing|체크리스트]]:
- [ ] PP 산출 시 목표 회수기간이 해당 IT 기술(예: [[190_ai_llm_requirements_specification|AI]] 프레임워크)의 진부화 사이클(2~3년)보다 길게 설정되지 않았는가?
...
| 구분 | 기대효과 ([[096_risk_non_risk_architecture_evaluation_flaws|리스크]] 통제) |
|:---|:---|
| **위험의 조기 경보** | 회수 기간이 길어질수록 환율 변동, 경쟁사 출현, 기술 노후화 등 통제 불가능한 외부 [[096_risk_non_risk_architecture_evaluation_flaws|리스크]] 노출 감소 |
| **유동성 맵 제공** | 자금이 묶여 있는([[362_lock_in_portability|Lock-in]]) 기간을 시각화하여 다음 IT 후속 투자를 위한 재원 마련 [[208_schedule_history_transaction_execution_order|스케줄]] 연계 가능 |
| **경영진의 심리적 안정** | 복잡한 수식을 거부하는 최고경영진에게 "2년 반이면 본전 뺍니다"라는 가장 강력한 설득 무기 제공 |

오늘날 극도로 불확실한 VUKA(Volatility, Uncertainty, Complexity, Ambiguity) 시대에는 미래를 예측하기 어렵다. 따라서 화려한 10년 치 시나리오를 바탕으로 한 NPV보다, 1~2년 안에 빠르게 원금을 회수하고 [[037_pivot|피벗]]([[037_pivot|Pivot]])할 수 있는 '빠른 회수(Fast Payback)' 모델이 [[035_lean_startup|린 스타트업]]([[035_lean_startup|Lean Startup]])이나 [[034_agile_management|애자일 경영]] 철학과 맞아떨어지며 그 중요성이 다시 부각되고 있다. 

📢 **섹션 요약 비유**: 투자회수기간은 깊은 동굴에 들어갈 때 들고 가는 산소통과 같다. 아무리 동굴 끝에 금은보화([[013_npv|NPV]])가 많더라도, 내 산소통(자본 유동성)이 버틸 수 있는 시간(PP) 내에 돌아오지 못하면 아무 소용이 없음을 알려주는 생명줄이다.

---
### 📌 관련 개념 맵 ([[160_knowledge_graph_graphrag_integration|Knowledge Graph]])
- **할인투자회수기간 (DPP)**: PP의 화폐 시간가치 무시 문제를 해결하기 위해, 현금을 할인한 뒤 누적액을 계산하는 진보된 PP.
- **[[013_npv|NPV]] (순현재가치)**: PP가 무시해 버리는 회수 기간 이후의 대규모 수익까지 모두 모아 전체 투자의 진짜 이익을 보여주는 상호 보완재.
- **유동성 [[096_risk_non_risk_architecture_evaluation_flaws|리스크]] (Liquidity [[096_risk_non_risk_architecture_evaluation_flaws|Risk]])**: 흑자가 나더라도 당장 현금이 없어 회사가 부도나는 현상으로, PP를 통해 이 [[096_risk_non_risk_architecture_evaluation_flaws|리스크]]의 지속 기간을 예측 가능.
- **기술 진부화 (Technology Obsolescence)**: IT 시스템이 구형이 되어 가치를 잃는 현상으로, PP는 반드시 이 진부화 기간보다 짧아야만 투자 타당성이 인정됨.
- **[[016_tco|TCO]] (총소유비용)**: PP를 계산할 때 '원금'에 해당하는 부분으로, [[459_quic_fec_forward_error_correction|초기]] 도입비 외에 유지보수비까지 TCO에 넣으면 PP 기간이 길어짐.
...
### 👶 어린이를 위한 3줄 비유 설명
1. 용돈 [[489_raid_10_hybrid|10]],000원을 주고 병아리 한 마리를 샀어요. 이 병아리가 매일 알을 낳아서 하루에 1,000원씩 벌어다 준다고 해요.
2. 그럼 딱 10일 뒤면 내가 처음에 쓴 용돈 [[489_raid_10_hybrid|10]],000원을 다시 채울 수 있겠죠? 이 '10일'이 바로 투자회수기간(PP)이에요.
...
← **이전**: [[014_irr_internal_rate_of_return|14. IRR (Internal Rate of Return, 내부수익률)]]
**다음**: [[015_pp_payback_period|15. PP (Payback Period, 투자회수기간)]] →
