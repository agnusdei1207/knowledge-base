...
핫 사이트는 재해가 발생했을 때 바로 활용할 수 있도록 미리 준비해 둔 원격 복구 센터다. 서버, 스토리지, 네트워크, 운영체제, 미들웨어, 애플리케이션 구성이 주 센터와 거의 동일하며, 데이터도 실시간 또는 근실시간으로 따라간다. 기술사 문맥에서는 보통 **RTO 4시간 이내 수준의 빠른 복구가 가능한 DR 유형**으로 설명한다.
