...
> 2. **가치**: Robust Accuracy 40%->82%(PGD-AT, ε=8/255 CIFAR-10 기준), Certified Radius r=1.5@95% 보장(Randomized Smoothing), 탐지 latency 12ms 이내로 실시간 차단 가능(MagNet), NIST AI RMF·EU AI Act·국내 AI기본법(2026.1 시행) 컴플라이언스 충족을 통한 비즈니스 리스크 60%v, MLOps 파이프라인 통합 시 모델 신뢰성 SLA 99.9% 달성.
> 3. **판단 포인트**: ① **방어 기법 선택 trade-off**(Adversarial Training: 정확도v 10~15% vs Robustness^ vs 학습 비용 3~5×), ② **공격 표면 분류**(Evasion vs Poisoning vs Inference Threat Model), ③ **L∞/L2/L0 노름 기준 및 ε-budget** 결정, ④ **White-box vs Black-box 공격 가정**, ⑤ **인퍼런스 지연(latency) vs Robustness** 균형, ⑥ **설명가능성(XAI)·프라이버시(DP)·페어니스**와 Robustness의 통합 거버넌스 설계.
...
| **Input Preprocessing** | perturbation 차감·왜곡 | ① Feature Squeeze(비트 깊이v, Spatial Smoothing), ② Pixel Deflection, ③ JPEG/JPEG2000 압축, ④ Total Variation Minimization, ⑤ Super-resolution 기반 denoising. **공격 적응성 한계** (Adaptive Attack에 약함) |
