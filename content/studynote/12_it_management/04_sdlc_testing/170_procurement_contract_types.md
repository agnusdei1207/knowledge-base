...
> 1. **본질**: 조달 계약 방식 (Procurement Contract Types)은 가격 계산 방법이 아니라, 범위·비용·성과 [리스크](/knowledge-base/studynote/11_design_supervision/02_architecture_principles/096_risk_non_risk_architecture_evaluation_flaws/)를 발주자와 공급자 사이에 어떻게 배분할지 정하는 관리 장치다.
...
| [FP](/knowledge-base/studynote/12_it_management/05_security_compliance/293_fp_function_point/)-EPA (Fixed Price with Economic Price Adjustment) | 고정가 + 물가/환율 조정 | 장기 계약, 외부 가격 변동 큼 | 조정 지표를 계약서에 명확히 넣어야 함 |
| FPIF (Fixed Price Incentive Fee) | 목표비용·인센티브·상한가 [설정](/knowledge-base/studynote/15_devops_sre/01_culture_methodology/009_config/) | 범위는 비교적 명확하지만 원가절감 유인이 필요함 | 상한가와 분담비율 설계가 핵심 |
| CPFF (Cost Plus Fixed Fee) | 실제 원가 + 고정 수수료 | 연구개발, 요구 불확실성 큼 | 발주자 감독과 원가 [감사](/knowledge-base/studynote/02_operating_system/10_security/606_auditing_linux_auditd/) 필요 |
| CPIF (Cost Plus Incentive Fee) | 실제 원가 + 성과 인센티브 | 원가 통제 유인을 일부 유지하고 싶음 | 성과 지표가 객관적이어야 함 |
| CPAF (Cost Plus Award Fee) | 실제 원가 + 평가 보상금 | 협업 품질, [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/) 수준 같은 정성 평가 중요 | 보상 기준이 모호하면 불신 발생 |
| T&M (Time and Material) | 단가 고정 + 투입량 변동 | 빠른 착수, 유지보수, [초기](/knowledge-base/studynote/03_network/08_transport_layer/459_quic_fec_forward_error_correction/) 탐색 | 상한선과 승인 절차 없으면 비용 누수 |

FPIF (Fixed Price Incentive Fee)는 기술사 시험에서 자주 묻는 유형이다. 목표비용, 목표수수료, 상한가, 비용 분담비율을 정해 놓고 실제 비용이 줄면 절감 이익을 나눠 갖는다. 이때 총가정점 (PTA, Point of Total Assumption)을 넘어서면 공급자가 추가 비용을 거의 전부 부담하게 되어, 사실상 FFP에 가까운 [리스크](/knowledge-base/studynote/11_design_supervision/02_architecture_principles/096_risk_non_risk_architecture_evaluation_flaws/) 구조로 바뀐다.

반면 T&M은 중간형이다. 인력 단가와 자재 단가는 어느 정도 고정돼 [FP](/knowledge-base/studynote/12_it_management/05_security_compliance/293_fp_function_point/) 성격을 가지지만, 총 투입 시간이 열려 있어 CR 성격도 함께 가진다. 그래서 요구가 덜 정리된 [초기](/knowledge-base/studynote/03_network/08_transport_layer/459_quic_fec_forward_error_correction/) 단계에는 유연하지만, 발주자가 승인 절차와 상한선을 잡지 않으면 비용이 길게 새기 쉽다.
...
| [Agile](/knowledge-base/studynote/15_devops_sre/01_culture_methodology/004_agile_relation/) 친화성 | 낮음, 단 [스프린트](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/067_sprint_timebox/) 단위 계약은 가능 | 탐색 단계에 유리 | 매우 높음 |
...
1. [인수 기준](/knowledge-base/studynote/04_software_engineering/03_design_architecture/165_acceptance_criteria_definition/)이 객관적으로 측정 가능한가?
2. 요구사항 변경이 얼마나 잦을 것으로 예상되는가?
3. 발주자가 시간·원가·성과를 실제로 감시할 역량이 있는가?
4. 환율·인건비·원자재처럼 외부 가격 변동이 계약 기간에 큰 영향을 주는가?
5. 인센티브를 줄 경우 속도만이 아니라 품질·재작업률·[서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/) 수준까지 함께 측정하는가?

### 자주 발생하는 [안티패턴](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/128_water_scrum_fall_anti_pattern/)

- 요구가 흐릿한 [Agile](/knowledge-base/studynote/15_devops_sre/01_culture_methodology/004_agile_relation/) 프로젝트를 순수 FFP로 묶고 변경관리 절차도 약하게 두는 것
...
적절한 조달 계약 방식을 선택하면 [리스크](/knowledge-base/studynote/11_design_supervision/02_architecture_principles/096_risk_non_risk_architecture_evaluation_flaws/)를 더 잘 통제할 수 있는 쪽에 배분하고, 공급자의 행동을 프로젝트 목표와 가깝게 정렬할 수 있다. 그 결과 예산 예측 가능성이 높아지고, 변경 협의 기준이 선명해지며, 성과 보상과 책임 추궁의 근거도 명확해진다. 특히 소프트웨어 사업에서는 계약 구조가 개발 방법론과 운영 방식까지 좌우하므로, 계약은 사업관리의 뼈대라고 볼 수 있다.
