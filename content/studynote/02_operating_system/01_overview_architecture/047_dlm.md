...
> **핵심 인사이트**
> 1. DLM(Distributed [[510_lock|Lock]] Manager)은 [[136_variance|분산]] 시스템에서 여러 노드가 공유 자원에 대한 접근 순서와 [[194_consistency_database_integrity|일관성]]을 조율하는 핵심 미들웨어 — 단일 서버의 뮤텍스/세마포어가 [[136_variance|분산]] 환경으로 확장된 개념이며, 클러스터 파일시스템(GFS, OCFS2)의 핵심 구성 요소다.
> 2. DLM의 핵심 과제는 [[341_process|CAP]] 트릴레마와 장애 시나리오 — 네트워크 분단([[514_partition_slice_volume|Partition]]) 시 [[194_consistency_database_integrity|일관성]]([[194_consistency_database_integrity|Consistency]])과 [[452_availability|가용성]]([[452_availability|Availability]]) 중 하나를 포기해야 하며, "잠금 보유자 장애 시 잠금 해제"가 구현 난이도의 핵심이다.
> 3. [[542_redis|Redis]]/[[798_distributed_lock_zookeeper_consensus|ZooKeeper]]/[[078_etcd_distributed_key_value_store|etcd]] 기반 [[136_variance|분산]] 잠금이 현대 클라우드 환경의 실용적 선택 — 각각 Redlock([[542_redis|Redis]]), curator([[798_distributed_lock_zookeeper_consensus|ZooKeeper]]), etcd의 lease 기반으로 구현되며, 잠금 유효 시간([[294_ttl_time_to_live_looping_prevention|TTL]])과 Fencing Token이 안전한 [[136_variance|분산]] 잠금의 핵심 패턴이다.

---

## Ⅰ. [[136_variance|분산]] 잠금 필요성
...
> 📢 **섹션 요약 비유**: [[136_variance|분산]] 잠금은 화장실 키 — 건물에 화장실이 하나인데 사용자가 여러 건물에 있어요. 키(잠금)를 받은 사람만 사용. 키 받기 전까지 대기!
...
> 📢 **섹션 요약 비유**: DLM 구현 3종 — Redis는 빠른 메모 (NX 명령 원자적), ZooKeeper는 자동 사라지는 포스트잇 ([[160_session_controlling_terminal|세션]] 종료 시 임시 노드 삭제), etcd는 타이머 키 (Lease [[294_ttl_time_to_live_looping_prevention|TTL]])!
...
> 📢 **섹션 요약 비유**: CAP과 [[136_variance|분산]] 잠금 — [[798_distributed_lock_zookeeper_consensus|ZooKeeper]]/etcd는 엄격한 경비원([[086_CP_순환_전치_GI|CP]]: 분단 시 [[090_service_kubernetes_network_load_balancing|서비스]] 중단), Redis는 빠른 경비원([[572_ap_access_point_ds_distribution_system|AP]]: 빠르지만 가끔 실수). 돈(금융)은 엄격한 경비원!
...
> 📢 **섹션 요약 비유**: 쿠폰 [[136_variance|분산]] 잠금은 원 버튼 줄서기 — 50,000명이 동시에 쿠폰 버튼. [[542_redis|Redis]] 잠금이 한 명씩만 서버에 접근하게 해서 1,000개 정확히 발급. 줄 서기 없이 공정하게!
...
1. [[136_variance|분산]] 잠금은 여러 건물 공유 열쇠 — 여러 건물의 사람들이 하나의 화장실을 쓸 때, 열쇠(잠금)를 받은 사람만 사용. 동시 사용 방지!
2. Fencing Token은 번호표 갱신 — 33번이 자는 사이 34번이 입장. 33번이 깨도 "34번 이상만 허용" 규칙으로 거부. 만료된 잠금 무력화!
3. 선착순 쿠폰 [[136_variance|분산]] 잠금 — 50,000명이 동시 클릭해도 [[542_redis|Redis]] 잠금이 한 명씩 처리. 1,000개 딱 발급하고 끝. 중복 발급 0건!
...
← **이전**: [[046_standby_modes|046. 대기 모드 — OS Standby & Sleep Modes]]
**다음**: [[048_san_integration|048. SAN 통합 — Storage Area Network]] →
