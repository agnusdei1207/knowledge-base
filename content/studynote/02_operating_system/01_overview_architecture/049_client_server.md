...
> **핵심 인사이트**
> 1. 클라이언트-서버 모델([[206_client_server_architecture_model|Client-Server]] Model)은 요청자([[003_audit_stakeholders|Client]])와 제공자(Server)의 역할을 분리하는 [[136_variance|분산]] 컴퓨팅의 가장 기본 패러다임 — OS 관점에서 [[117_ipc|프로세스 간 통신]]([[117_ipc|IPC]]), [[125_socket|소켓]], [[092_thread_lwp|스레드]] 관리까지 OS의 핵심 기능이 이 모델을 지탱한다.
> 2. 서버의 연결 처리 방식(단일 프로세스·멀티프로세스·멀티스레드·[[142_event_loop|이벤트 루프]])이 성능과 자원 효율의 핵심 트레이드오프 — Apache의 프로세스 기반 vs Nginx의 [[142_event_loop|이벤트 루프]] 차이가 C10K 문제 해결의 분기점이 되었다.
> 3. 현대 마이크로서비스는 클라이언트-서버를 N-Tier로 확장한 것 — [[090_service_kubernetes_network_load_balancing|서비스]]가 동시에 클라이언트이자 서버로 동작하며, [[306_service_discovery_pattern|서비스 디스커버리]]·로드밸런서·서킷 브레이커가 전통적 서버의 역할을 [[136_variance|분산]]화했다.
...
> 📢 **섹션 요약 비유**: 서버 처리 방식 = 식당 서빙 방식 — 멀티프로세스(손님마다 요리사 1명), 멀티스레드(웨이터 여러 명), [[142_event_loop|이벤트 루프]](웨이터 1명이 알림(이벤트) 받으며 효율적으로 처리). Nginx = 1명 슈퍼 웨이터!
...
## Ⅴ. 실무 시나리오 — 대규모 [[014_api_posix|API]] 서버 설계
...
> 📢 **섹션 요약 비유**: 대규모 [[014_api_posix|API]] 서버 = 패스트푸드 체인 본사 관리 — [[506_cdn_content_delivery_network_edge_caching|CDN]](포장 완료품 배달), ALB(매장 안내), Nginx(주문 접수), Node.js(요리), DB 풀(식재료 창고). 자동 확장으로 피크도 안정!
...
2. epoll [[142_event_loop|이벤트 루프]] = 1명 슈퍼 웨이터 — 1,000 손님 알림 기다리다 완성된 것만 처리. 수천 동시 연결을 혼자 효율적으로!
...
← **이전**: [[048_san_integration|048. SAN 통합 — Storage Area Network]]
**다음**: [[050_p2p_system|P2P 시스템 (Peer-to-Peer System)]] →
