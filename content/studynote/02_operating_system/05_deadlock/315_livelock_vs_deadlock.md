...
| 진단 기준 | [교착 상태](/knowledge-base/studynote/02_operating_system/05_deadlock/281_deadlock_definition/) ([Deadlock](/knowledge-base/studynote/02_operating_system/05_deadlock/281_deadlock_definition/)) | 라이브락 (Livelock) |
|:---|:---|:---|
...
**📢 섹션 요약 비유**: 환자에 비유하자면, 데드락은 마취를 맞고 수술대 위에서 곯아떨어져 깨어날 가망이 없는 코마([Coma](/knowledge-base/studynote/01_computer_architecture/10_parallel_processing_architecture/381_coma/)) 환자고, 라이브락은 병실에서 쉬지 않고 제자리 점프를 천만 번 뛰면서 심장이 터지기 직전인 과잉 행동 장애 환자입니다.
...
`라이브락 (Livelock)`은 데드락이라는 고요한 죽음을 막기 위해 인간이 삽입한 어설픈 양보 코드(Retry [Policy](/knowledge-base/studynote/10_ai/02_dl_architecture_new/164_policy/))가 만들어낸 사생아다. 프로세스들은 살아있음을 증명(Live)하려 미친 듯이 연산하지만 일의 진척([Progress](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/216_progress_in_synchronization/))은 제로에 수렴한다. 이 아이러니를 파괴한 것은 고도화된 스케줄링 이론이 아닌, 시간의 공정성을 깨부수는 무작위 난수(Randomized Backoff / Jitter)라는 극도의 인간적(비합리적) 트릭이었음을 인지하는 것이 핵심 설계 미학이다.
...
| 개념 | 연결 포인트 |
|:---|:---|
| [후퇴](/knowledge-base/studynote/02_operating_system/05_deadlock/313_rollback/) ([Rollback](/knowledge-base/studynote/02_operating_system/05_deadlock/313_rollback/)) | 현재 개념으로 들어오기 전에 함께 이해하면 경계가 선명해지는 기반 개념이다. |
| [기아 상태](/knowledge-base/studynote/02_operating_system/05_deadlock/314_starvation_prevention/) ([Starvation](/knowledge-base/studynote/02_operating_system/05_deadlock/314_starvation_prevention/)) 발생 방지 (희생자 선택에 횟수 제한) | 현재 개념이 등장하게 만든 직접적인 선행 흐름이다. |
| [동기화 결함](/knowledge-base/studynote/02_operating_system/05_deadlock/316_synchronization_bug_debugging/) ([순환 의존성](/knowledge-base/studynote/02_operating_system/05_deadlock/316_synchronization_bug_debugging/)) 코드 레벨 디버깅 기법 | 현재 개념이 구현·세분화될 때 바로 연결되는 후속 개념이다. |
| [락 오더링](/knowledge-base/studynote/02_operating_system/05_deadlock/317_lockdep_lock_ordering/) ([Lock Ordering](/knowledge-base/studynote/02_operating_system/05_deadlock/317_lockdep_lock_ordering/)) 다이나믹 [검증](/knowledge-base/studynote/04_software_engineering/07_object_oriented/395_verification_process_review/) 도구 (Lockdep in Linux) | 확장 학습이나 심화 비교로 이어지는 다음 단계의 키워드다. |
...
← **이전**: [314. 기아 상태 (Starvation) 발생 방지 (희생자 선택에 횟수 제한)](/knowledge-base/studynote/02_operating_system/05_deadlock/314_starvation_prevention/)
**다음**: [316. 동기화 결함 (순환 의존성) 코드 레벨 디버깅 기법 (Synchronization Bug Debugging)](/knowledge-base/studynote/02_operating_system/05_deadlock/316_synchronization_bug_debugging/) →
