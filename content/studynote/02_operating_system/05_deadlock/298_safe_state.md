...
### 안전 순서 ([Safe](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/093_safe_scaled_agile_framework_art_pi/) Sequence) 생성론

`안전 상태`라는 단어는 포괄적이지만, 그 뼈대를 입증하는 것은 `<P1, P3, P2...>` 같은 [배열](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/055_array/)을 찾는 뺑뺑이 [알고리즘](/knowledge-base/studynote/08_algorithm_stats/01_basics/001_algorithm_definition/)이다.

1. **상태 초기화**: OS는 `Work` = 현재 가용 자원 행렬을 복사.
2. **시뮬레이션 반복**: 남아 있는 프로세스([스레드](/knowledge-base/studynote/02_operating_system/02_process_thread/092_thread_lwp/)) 중에서, `"자기가 앞으로 요청할 최대 자원수(Need) <= 현재 가용 자원(Work)"`을 만족하는 만만한 녀석(만물 졸업자)을 찾아낸다.
3. **가상 반환**: 그 녀석이 무사히 다 쓰고 종료했다고 치고, 그놈이 기존에 쥐고 있던 몫(Allocation)을 `Work`에 다 쏟아부어 합친다. (눈 굴리기 효과).
4. **결과 증명**: 2~3번 스텝을 돌려서 [배열](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/055_array/)의 모든 P가 졸업 처리되었다면 그 순서열이 바로 `안전 순서`이며, 이 시스템은 100% `안전 상태`로 공식 인가된다.
...
**📢 섹션 요약 비유**: 안전 상태(얼음판 위 눈길), [불안전 상태](/knowledge-base/studynote/02_operating_system/05_deadlock/299_unsafe_state/)(투명한 살얼음판, 밟아도 안 깨질 수 있음), [교착 상태](/knowledge-base/studynote/02_operating_system/05_deadlock/281_deadlock_definition/)(얼음 깨져서 물에 빠짐). 회피 [알고리즘](/knowledge-base/studynote/08_algorithm_stats/01_basics/001_algorithm_definition/)은 오직 안전 상태의 눈길만 걷게 고집부리는 결백증입니다.
...
| 기준 | 안전 상태 고수 (Avoidance) | 자유 진입 방치 (Ostrich / [Detection](/knowledge-base/studynote/09_security/19_ai_advanced_security/961_deepfake_detection/)) |
|:---|:---|:---|
...
| 개념 | 연결 포인트 |
|:---|:---|
| [순환 대기 부정](/knowledge-base/studynote/02_operating_system/05_deadlock/296_deny_circular_wait/) | 현재 개념으로 들어오기 전에 함께 이해하면 경계가 선명해지는 기반 개념이다. |
| [교착 상태 회피](/knowledge-base/studynote/02_operating_system/05_deadlock/297_deadlock_avoidance/) ([Deadlock Avoidance](/knowledge-base/studynote/02_operating_system/05_deadlock/297_deadlock_avoidance/)) | 현재 개념이 등장하게 만든 직접적인 선행 흐름이다. |
| [불안전 상태](/knowledge-base/studynote/02_operating_system/05_deadlock/299_unsafe_state/) ([Unsafe State](/knowledge-base/studynote/02_operating_system/05_deadlock/299_unsafe_state/)) | 현재 개념이 구현·세분화될 때 바로 연결되는 후속 개념이다. |
...
← **이전**: [297. 교착 상태 회피 (Deadlock Avoidance) - 실행 전 자원 할당 상태를 검사하여 안전한 경우에만 승인](/knowledge-base/studynote/02_operating_system/05_deadlock/297_deadlock_avoidance/)
**다음**: [299. 불안전 상태 (Unsafe State) - 교착 상태가 발생할 가능성이 있는 상태](/knowledge-base/studynote/02_operating_system/05_deadlock/299_unsafe_state/) →
