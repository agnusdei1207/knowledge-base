...
> 2. **메커니즘 (Trap)**: 애플리케이션은 일반적인 [함수 호출](/knowledge-base/studynote/06_ict_convergence/04_ai_llm/294_function_calling_tool_use/)(`call`)로는 [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/) 메모리에 들어갈 수 없다. 대신, CPU 내부에 고의적인 **소프트웨어 [인터럽트](/knowledge-base/studynote/02_operating_system/01_overview_architecture/016_interrupt_mechanism/)(트랩, 예: `int 0x80`)**를 발생시켜, 하드웨어가 강제로 권한을 높이고 [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/)의 시스템 콜 핸들러로 점프하게 만드는 해킹에 가까운 우회로를 사용한다.
> 3. **진화**: 과거 트랩(`int 0x80`) 방식은 [인터럽트](/knowledge-base/studynote/02_operating_system/01_overview_architecture/016_interrupt_mechanism/) 오버헤드가 커서 느렸다. 현대 CPU는 시스템 콜만을 위한 빠르고 날렵한 전용 하드웨어 [명령어](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/158_instruction/)인 **`sysenter` / `syscall` (Fast [System Call](/knowledge-base/studynote/02_operating_system/01_overview_architecture/013_system_call/))**을 도입하여 트랩 기반의 병목을 해결했다.
...
- **발전 과정**:
  1. **단일 권한 체계 ([DOS](/knowledge-base/studynote/02_operating_system/10_security/599_dos_ddos_attack/) 시절)**: 트랩 없음. 앱이 하드웨어를 직접 제어. 앱 하나가 고장 나면 컴퓨터 전체가 멈춤.
...
|:---|:---|:---|:---|
| **[System Call](/knowledge-base/studynote/02_operating_system/01_overview_architecture/013_system_call/) (Trap)** | 의도적 ([명령어](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/158_instruction/)) | **유저 애플리케이션** | [파일](/knowledge-base/studynote/02_operating_system/09_file_system/501_file_definition_logical_record/) 열기, 메모리 할당 등 [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/) [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/) 요청 |
...
| **[Hardware Interrupt](/knowledge-base/studynote/02_operating_system/01_overview_architecture/017_hardware_interrupt/)** | 외부 이벤트 | **외부 기기 ([NIC](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/587_nic_offloading/), 디스크 등)** | 패킷 도착, 키보드 눌림 등 [비동기적](/knowledge-base/studynote/02_operating_system/01_overview_architecture/017_hardware_interrupt/) 사건 알림 |
...
- **컴퓨터구조 ([CA](/knowledge-base/studynote/06_ict_convergence/01_blockchain/089_contract_account_smart_contract/))**: `syscall` [명령어](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/158_instruction/)가 그토록 빠른 이유는, 점프 시 저장해야 할 정보(PC와 [플래그](/knowledge-base/studynote/03_network/04_data_link_layer_error/186_character_stuffing_dle_stx_etx/))를 느린 메모리 스택에 PUSH하지 않고, CPU 내부의 [초고속](/knowledge-base/studynote/06_ict_convergence/02_iot_mobility/148_5g_embb_urllc_mmtc/) RCX, R11 [레지스터](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/057_register/)에 덮어써 버리는 극단적인 하드웨어 최적화(Hardware Fast Path)를 적용했기 때문이다.
...
1. **시나리오 — 고성능 웹 서버(Nginx/Node.js)의 과도한 시스템 콜 오버헤드 병목**: 10G 네트워크 환경에서 Nginx가 [파일](/knowledge-base/studynote/02_operating_system/09_file_system/501_file_definition_logical_record/)을 읽어 클라이언트에게 보낼 때, CPU 사용률의 60%가 `sys_read`와 `sys_write` 시스템 콜의 전환 비용(System CPU Time)으로 낭비되어 [대역폭](/knowledge-base/studynote/01_computer_architecture/03_architecture_basics_performance/140_bandwidth/)을 다 쓰지 못하는 현상.
   ...
   - **대응 (기술사적 가이드)**: 
     - **[Zero-Copy](/knowledge-base/studynote/02_operating_system/09_file_system/566_mmap_zero_copy_sendfile/) 기술 (sendfile)**: `read`와 `write`를 따로 부르지 말고, `sendfile()`이라는 단일 시스템 콜을 사용한다. [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/) 안에서 [파일](/knowledge-base/studynote/02_operating_system/09_file_system/501_file_definition_logical_record/) [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 디스크 캐시 $\rightarrow$ 랜카드 버퍼로 다이렉트로 복사하므로, [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/) 밖(User Space)으로 나왔다가 들어가는 트랩 횟수가 절반으로 줄어 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/)이 2배 뛴다.
     - **[io_uring](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/464_io_uring/) 도입**: 최신 리눅스는 시스템 콜 자체를 안 부르고 비동기로 처리하는 `io_uring` 큐 구조를 도입하여 트랩 오버헤드 자체를 제로(0)에 가깝게 소멸시켰다.

2. **시나리오 — [Docker](/knowledge-base/studynote/02_operating_system/01_overview_architecture/063_docker_architecture/) [컨테이너](/knowledge-base/studynote/04_software_engineering/09_cloud_native_ai_architecture/561_container_based_deployment/)의 시스템 콜 차단 보안 ([Seccomp](/knowledge-base/studynote/02_operating_system/01_overview_architecture/080_seccomp/))**: [멀티 테넌트](/knowledge-base/studynote/03_network/17_sdn_nfv/888_multi_tenant_cloud_resource_isolation_noisy_neighbor/) 클라우드 환경에서 악성 [컨테이너](/knowledge-base/studynote/04_software_engineering/09_cloud_native_ai_architecture/561_container_based_deployment/)가 시스템의 시간을 함부로 바꾸거나(`sys_settimeofday`), 모듈을 올리는(`sys_init_module`) 등 위험한 시스템 콜을 부르는 것을 막아야 한다.
   - **아키텍처 적용 ([Seccomp](/knowledge-base/studynote/02_operating_system/01_overview_architecture/080_seccomp/)-[bpf](/knowledge-base/studynote/02_operating_system/01_overview_architecture/069_ebpf/))**: [도커](/knowledge-base/studynote/02_operating_system/01_overview_architecture/063_docker_architecture/)([Docker](/knowledge-base/studynote/02_operating_system/01_overview_architecture/063_docker_architecture/))나 쿠버네티스는 [컨테이너](/knowledge-base/studynote/04_software_engineering/09_cloud_native_ai_architecture/561_container_based_deployment/)를 띄울 때 기본적으로 **[Seccomp](/knowledge-base/studynote/02_operating_system/01_overview_architecture/080_seccomp/) ([Secure Computing Mode](/knowledge-base/studynote/02_operating_system/01_overview_architecture/080_seccomp/))** 프로파일을 입힌다. 
