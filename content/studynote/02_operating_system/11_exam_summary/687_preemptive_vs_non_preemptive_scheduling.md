...
> 1. **본질**: CPU 스케줄링의 가장 근본적인 [분류](/knowledge-base/studynote/16_bigdata/05_analysis/104_classification_analysis/) 기준으로, 프로세스가 CPU를 잡았을 때 **"스스로 내놓을 때까지 기다려줄 것인가([비선점](/knowledge-base/studynote/02_operating_system/05_deadlock/285_no_preemption/))"** 아니면 **"OS가 중간에 강제로 뺏을 수 있는가(선점)"**의 권력 구조 차이를 의미한다.
> 2. **[비선점](/knowledge-base/studynote/02_operating_system/05_deadlock/285_no_preemption/) (Non-Preemptive)**: 한 번 CPU를 잡으면 I/O 대기나 종료 전까지 절대 뺏기지 않는다. 오버헤드([문맥 교환](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/211_context_switch/))가 적어 과거의 일괄 처리 시스템에 쓰였으나, 버그 난 프로그램 하나가 전체 시스템을 멈추게 하는 치명적 단점이 있다.
...
- **개념**: 
  - **[비선점형 스케줄링](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/167_non_preemptive_scheduling/) (Non-Preemptive)**: 실행 중인 프로세스가 I/O를 요청하거나 스스로 종료할 때까지 CPU를 계속 독점하는 방식.
  - **[선점형 스케줄링](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/166_preemptive_scheduling/) (Preemptive)**: OS가 [하드웨어 인터럽트](/knowledge-base/studynote/02_operating_system/01_overview_architecture/017_hardware_interrupt/)를 이용해 실행 중인 프로세스의 CPU를 강제로 회수하고, 다른 우선순위가 높은 프로세스에 할당할 수 있는 방식.
...
- **필요성 (독재의 종식과 반응성 확보)**: 
  ...
  - **해결책**: 프로세스의 '자발적 양보'에 기대는 순진한 생각을 버리고, OS [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/)이 타이머 칩(하드웨어)을 무기로 삼아 **"시간 다 됐어! 강제 하차해!"**라고 쫓아내는 선점형(강압적) 스케줄링이 필수적으로 대두되었다.

  - **[비선점](/knowledge-base/studynote/02_operating_system/05_deadlock/285_no_preemption/)형**: 마이크를 한 번 잡으면, 자기가 연설을 스스로 끝내고 내려오기 전까지는 아무도 마이크를 뺏을 수 없는 자유 발언대. 한 명이 10시간 떠들면 나머지 사람은 다 집에 가야 한다.
  - **선점형**: 1분마다 무조건 마이크 전원을 꺼버리고 다음 사람에게 마이크를 강제로 넘기는 TV 토론. 누군가 헛소리를 길게 늘어놓아도 1분 뒤면 입이 막히므로 토론회 전체가 마비되지 않는다.

- **발전 과정**:
  1. **[초기](/knowledge-base/studynote/03_network/08_transport_layer/459_quic_fec_forward_error_correction/) OS ([비선점](/knowledge-base/studynote/02_operating_system/05_deadlock/285_no_preemption/))**: [FCFS](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/173_fcfs_scheduling/)(First Come First Served), [SJF](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/175_sjf_scheduling/)([Shortest Job First](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/175_sjf_scheduling/)). 스위칭 오버헤드가 없어 연산 효율은 좋음.
  2. **과도기 (협력적 [멀티태스킹](/knowledge-base/studynote/02_operating_system/11_exam_summary/675_multitasking_terminology_preemptive/))**: Windows 3.1, [Mac](/knowledge-base/studynote/03_network/13_network_security_basics/673_mac_message_authentication_code/) OS 9. 앱들이 스스로 `yield()`를 불러서 양보하게 만듦 (결국 [비선점](/knowledge-base/studynote/02_operating_system/05_deadlock/285_no_preemption/)의 일종이라 잘 멈춤).
...
- **[비선점](/knowledge-base/studynote/02_operating_system/05_deadlock/285_no_preemption/)형**: 1번과 2번 상황에서만 [스케줄러](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/079_kube_scheduler_pod_placement/)가 동작한다. (프로세스가 '스스로' 비킬 때만 개입)
- **선점형**: 1, 2번뿐만 아니라 **3번(시간 초과)과 4번([우선순위 역전](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/205_priority_inversion/))** 상황에서도 OS가 개입해 강제로 CPU를 뺏는다.
...
**[다이어그램 해설]** 초보 개발자는 "[운영체제](/knowledge-base/studynote/02_operating_system/01_overview_architecture/001_operating_system_purpose/)가 주기적으로 프로세스를 감시한다"고 생각하지만 틀렸다. CPU가 악성 코드를 돌리고 있을 때는 [운영체제](/knowledge-base/studynote/02_operating_system/01_overview_architecture/001_operating_system_purpose/)의 코드도 멈춰(휴면) 있다! 오직 외부에서 강제로 때려주는 **물리적 알람(타이머 [인터럽트](/knowledge-base/studynote/02_operating_system/01_overview_architecture/016_interrupt_mechanism/))**만이 멈춰있는 OS의 영혼을 CPU로 다시 불러들여 심판의 철퇴(선점)를 내리게 할 수 있다.
...
|:---|:---|:---|
| **[FCFS](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/173_fcfs_scheduling/)** (First Come First Served) | **[비선점](/knowledge-base/studynote/02_operating_system/05_deadlock/285_no_preemption/)** | 은행 창구 줄 서기. 먼저 온 놈이 끝날 때까지 다 씀 |
| **[SJF](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/175_sjf_scheduling/)** ([Shortest Job First](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/175_sjf_scheduling/)) | **[비선점](/knowledge-base/studynote/02_operating_system/05_deadlock/285_no_preemption/)** | 실행 시간이 제일 짧은 놈부터. 끝날 때까지 안 뺏음 |
| **[SRTF](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/177_srtf_scheduling/)** (Shortest Remaining Time First) | **선점** | SJF의 선점형. 실행 중인데 더 짧은 놈이 나타나면 뺏음 |
| **[RR](/knowledge-base/studynote/03_network/16_data_center_cloud/834_load_balancing_algorithm_round_robin_least_connection/)** (Round Robin) | **선점** | 모두에게 공평하게 시간(Time [Quantum](/knowledge-base/studynote/02_operating_system/11_exam_summary/690_round_robin_time_quantum/))을 주고 뺏음 |
| **[MLFQ](/knowledge-base/studynote/02_operating_system/11_exam_summary/691_mlfq_multi_level_feedback_queue/)** (Multi-Level Feedback [Queue](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/058_queue/)) | **선점** | 큐([Queue](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/058_queue/))가 여러 개. 현대 OS 스케줄링의 근간 |
...
1. **[Context Switch](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/211_context_switch/) 오버헤드**: 강제로 뺏고 넘겨주는 행위 자체가 [레지스터](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/057_register/) 복사와 [TLB](/knowledge-base/studynote/02_operating_system/06_memory_management/357_tlb/) 플러시를 유발하여 시스템을 느리게 만든다.
2. **[Race Condition](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/213_race_condition/) ([데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 충돌)**: 프로세스 A가 공유 메모리의 `Count = 5`를 `6`으로 바꾸려는 찰나(Read만 한 상태), OS가 강제로 CPU를 뺏어 프로세스 B에게 넘겼다. B가 `Count`를 `4`로 깎고 턴을 마쳤다. 다시 A가 깨어나서 원래 하던 대로 6을 써버렸다. 정답은 5가 되어야 하는데 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)가 깨졌다. ([비선점](/knowledge-base/studynote/02_operating_system/05_deadlock/285_no_preemption/)형에서는 절대 발생하지 않는 문제)
...
1. **시나리오 — [인터럽트](/knowledge-base/studynote/02_operating_system/01_overview_architecture/016_interrupt_mechanism/) 처리 중의 [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/) 선점([Kernel](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/) Preemption) 금지로 인한 랙(Lag)**: 과거 구형 리눅스 서버(2.4 [버전](/knowledge-base/studynote/03_network/06_network_layer_ip/288_version_ihl_tos_total_length/) 이하)에서 커다란 파일을 디스크에서 지우고([rm](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/197_rm_rate_monotonic_scheduling/)) 있으면, 마우스 커서마저 안 움직이고 서버 전체가 뚝뚝 끊기는 현상.
   - **원인 분석**: 유저 프로그램끼리는 "선점형([RR](/knowledge-base/studynote/03_network/16_data_center_cloud/834_load_balancing_algorithm_round_robin_least_connection/))"이라도, 프로그램이 시스템 콜을 호출해 **"[커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/) 모드(Ring 0)"**로 들어간 순간, 옛날 리눅스는 그 [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/) 코드를 '[비선점](/knowledge-base/studynote/02_operating_system/05_deadlock/285_no_preemption/)형'으로 취급했다. 즉, [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/)이 파일을 지우는 무거운 작업을 하는 동안에는 다른 유저 앱이 아무리 급해도 CPU를 뺏을 수 없었다.
   - **아키텍처 대응 (Preemptible [Kernel](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/))**: 리눅스 2.6부터는 `CONFIG_PREEMPT` 기능이 도입되어, [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/) 모드 안에서 코드가 돌고 있더라도 급한 [인터럽트](/knowledge-base/studynote/02_operating_system/01_overview_architecture/016_interrupt_mechanism/)나 [스레드](/knowledge-base/studynote/02_operating_system/02_process_thread/092_thread_lwp/)가 나타나면 **[커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/) 코드마저 강제로 중단시키고 선점([Kernel](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/) Preemption)**할 수 있게 진화하여 데스크탑의 부드러움을 쟁취했다.

2. **시나리오 — 실시간(RTOS) 환경에서의 [우선순위 역전](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/205_priority_inversion/) ([Priority Inversion](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/205_priority_inversion/))**: 화성 탐사선 패스파인더에서, 우선순위가 가장 높은 1번 [스레드](/knowledge-base/studynote/02_operating_system/02_process_thread/092_thread_lwp/)(선점형)가, 가장 낮은 3번 [스레드](/knowledge-base/studynote/02_operating_system/02_process_thread/092_thread_lwp/)가 쥐고 있는 락([Lock](/knowledge-base/studynote/05_database/04_transactions_concurrency/510_lock/))을 기다리며 멈춰버린 대형 사고.
   ...
   - **대응 (우선순위 [상속](/knowledge-base/studynote/04_software_engineering/04_testing_quality/234_uml_class_relationships_generalization_dependency/), PIP)**: OS [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/)이 이 상황을 감지하고, 락을 쥐고 있는 비루한 3번 [스레드](/knowledge-base/studynote/02_operating_system/02_process_thread/092_thread_lwp/)의 우선순위를 일시적으로 1번(최고)으로 뻥튀기(Boost)해 준다. 그러면 2번 [스레드](/knowledge-base/studynote/02_operating_system/02_process_thread/092_thread_lwp/)가 감히 3번을 선점하지 못하게 되고, 3번이 빨리 락을 풀고 1번을 살려내게 된다.
...
|:---|:---|:---|:---|
| **[응답 시간](/knowledge-base/studynote/01_computer_architecture/03_architecture_basics_performance/138_response_time/) ([Response Time](/knowledge-base/studynote/01_computer_architecture/03_architecture_basics_performance/138_response_time/))**| 예측 불가 (앞사람이 늦게 끝나면 무한 대기) | **예측 가능 (n초 이내 무조건 턴 돌아옴)** | 대화형(UI) 시스템에 필수 |
| **[처리량](/knowledge-base/studynote/01_computer_architecture/03_architecture_basics_performance/139_throughput/) ([Throughput](/knowledge-base/studynote/01_computer_architecture/03_architecture_basics_performance/139_throughput/))** | **최대화 (스위칭 오버헤드 0%)** | 다소 희생됨 ([Context Switch](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/211_context_switch/) 비용) | 배치 처리에는 [비선점](/knowledge-base/studynote/02_operating_system/05_deadlock/285_no_preemption/)형이 유리 |
| **[동기화](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/212_synchronization_mechanisms/) ([Synchronization](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/212_synchronization_mechanisms/))**| 단순함 (스스로 양보 전엔 충돌 없음) | **극도로 복잡함 ([Lock](/knowledge-base/studynote/05_database/04_transactions_concurrency/510_lock/), [세마포어](/knowledge-base/studynote/02_operating_system/04_synchronization/224_semaphore/) 필수)** | [Race Condition](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/213_race_condition/) 방어 비용 발생 |
