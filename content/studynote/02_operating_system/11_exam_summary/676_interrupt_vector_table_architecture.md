...
> 1. **본질**: [인터럽트 벡터](/knowledge-base/studynote/02_operating_system/01_overview_architecture/019_interrupt_vector/) 테이블([Interrupt Vector](/knowledge-base/studynote/02_operating_system/01_overview_architecture/019_interrupt_vector/) Table, IVT)은 CPU가 외부 장치의 [하드웨어 인터럽트](/knowledge-base/studynote/02_operating_system/01_overview_architecture/017_hardware_interrupt/)나 소프트웨어 예외(Exception)를 받았을 때, **어떤 [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/) 함수([ISR](/knowledge-base/studynote/02_operating_system/01_overview_architecture/020_isr/): [Interrupt Service Routine](/knowledge-base/studynote/01_computer_architecture/08_io_storage_systems/317_isr/))를 실행해야 할지 찾아가는 '메모리상의 주소록([배열](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/055_array/))'**이다.
...
- **개념**: 
  - **[인터럽트](/knowledge-base/studynote/02_operating_system/01_overview_architecture/016_interrupt_mechanism/) ([Interrupt](/knowledge-base/studynote/02_operating_system/01_overview_architecture/016_interrupt_mechanism/))**: CPU가 현재 하던 일을 멈추고 우선적으로 처리해야 할 긴급한 사건이 발생했음을 알리는 [신호](/knowledge-base/studynote/02_operating_system/02_process_thread/130_signal/).
  - **[인터럽트 벡터](/knowledge-base/studynote/02_operating_system/01_overview_architecture/019_interrupt_vector/) (Vector)**: [인터럽트](/knowledge-base/studynote/02_operating_system/01_overview_architecture/016_interrupt_mechanism/)의 종류를 식별하는 고유 번호 (예: 0번은 0으로 나누기 에러, 14번은 [페이지 폴트](/knowledge-base/studynote/02_operating_system/11_exam_summary/720_page_fault_isr/)).
  - **[인터럽트 벡터](/knowledge-base/studynote/02_operating_system/01_overview_architecture/019_interrupt_vector/) 테이블 (IVT / IDT)**: 이 벡터 번호를 인덱스로 하여, 처리할 함수의 메모리 주소가 순서대로 적혀 있는 [배열](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/055_array/).
...
- **필요성 (하드웨어와 소프트웨어의 통신 병목 해결)**: 
  ...
  - **테이블이 없을 때 ([Polling](/knowledge-base/studynote/02_operating_system/11_exam_summary/747_io_polling_overhead/))**: 학교 경비 아저씨(CPU)가 1반부터 10반까지 계속 돌아다니며 "무슨 일 있니?"라고 묻는다.
  - **[인터럽트 벡터](/knowledge-base/studynote/02_operating_system/01_overview_architecture/019_interrupt_vector/) 테이블 (IVT)**: 경비실에 1번부터 10번까지 버튼이 있는 상황판이 있다. 3반에서 싸움이 나서 3반 학생이 비상벨을 누르면, 상황판에 3번 불이 켜진다(Vector 3). 경비 아저씨는 벽에 붙은 매뉴얼(IVT)을 본다. "3번 불: 양호 선생님 호출". 아저씨는 즉시 양호 선생님을 부른다([ISR](/knowledge-base/studynote/02_operating_system/01_overview_architecture/020_isr/) 실행).

- **발전 과정**:
  1. **단일 [인터럽트](/knowledge-base/studynote/02_operating_system/01_overview_architecture/016_interrupt_mechanism/)**: 모든 [인터럽트](/knowledge-base/studynote/02_operating_system/01_overview_architecture/016_interrupt_mechanism/)가 한 곳으로 모여 OS가 `if-else`로 분기. 느리고 복잡함.
  2. **IVT ([Interrupt Vector](/knowledge-base/studynote/02_operating_system/01_overview_architecture/019_interrupt_vector/) Table, 리얼 모드)**: [초기](/knowledge-base/studynote/03_network/08_transport_layer/459_quic_fec_forward_error_correction/) x86 프로세서(16비트). 메모리 0번지부터 1KB 공간에 고정 할당.
  3. **IDT ([Interrupt](/knowledge-base/studynote/02_operating_system/01_overview_architecture/016_interrupt_mechanism/) Descriptor Table, [보호](/knowledge-base/studynote/02_operating_system/10_security/571_protection_vs_security/) 모드)**: 현대 32/64비트 프로세서. 메모리 어디든 위치할 수 있으며, 권한(DPL) 검사 등 보안 기능이 추가된 진보된 테이블 구조.
...
|:---:|:---|:---|
| **0 ~ 31** | **하드웨어 예외 (Exceptions)** | CPU 내부에서 발생하는 오류. (0: Divide by [zero](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/585_zero_skipping/), 13: General [Protection](/knowledge-base/studynote/02_operating_system/10_security/571_protection_vs_security/) Fault, 14: **[Page Fault](/knowledge-base/studynote/02_operating_system/07_virtual_memory/387_page_fault/)**) |
...
| **48 ~ 255** | **소프트웨어 [인터럽트](/knowledge-base/studynote/02_operating_system/01_overview_architecture/016_interrupt_mechanism/) (Software)**| OS [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/)이 자체적으로 배정한 번호. (예: Linux의 `INT 0x80` - 시스템 콜 진입점) |
...
**[다이어그램 해설]** IDT의 핵심은 **IDTR (IDT [Register](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/175_register_addressing/))**이다. 부팅 시 리눅스 [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/)이 메모리 빈 공간에 이 256칸짜리 테이블을 예쁘게 그려놓고, `lidt`라는 어셈블리 [명령어](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/158_instruction/)를 통해 "CPU야, 앞으로 [인터럽트](/knowledge-base/studynote/02_operating_system/01_overview_architecture/016_interrupt_mechanism/) 들어오면 여기 메모리 주소(IDTR)를 참조해라"라고 세팅한다. 이후 [인터럽트](/knowledge-base/studynote/02_operating_system/01_overview_architecture/016_interrupt_mechanism/)가 터지면, OS의 소프트웨어 개입 없이 **순수 하드웨어(CPU)**가 직접 산수(시작 주소 + 벡터*오프셋)를 해서 즉각 점프한다. 이것이 [인터럽트](/knowledge-base/studynote/02_operating_system/01_overview_architecture/016_interrupt_mechanism/)가 소프트웨어 분기문(`if-else`)보다 수백 배 빠른 이유다.
...
|:---|:---|:---|:---|:---|
| **[하드웨어 인터럽트](/knowledge-base/studynote/02_operating_system/01_overview_architecture/017_hardware_interrupt/)**| **외부 장치 (I/O)** | CPU [명령어](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/158_instruction/) 실행과 무관 | **[비동기적](/knowledge-base/studynote/02_operating_system/01_overview_architecture/017_hardware_interrupt/)** | 키보드 입력. 현재 [명령어](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/158_instruction/) 실행 완료 후 멈춤. |
| **예외 (Exception)**| **CPU 내부 (오류)** | 특정 [명령어](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/158_instruction/) 실행 도중 | **동기적 (Fault)** | 0으로 나누기, [Page Fault](/knowledge-base/studynote/02_operating_system/07_virtual_memory/387_page_fault/). 오류 난 [명령어](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/158_instruction/)를 나중에 **재시도**함. |
| **[트랩](/knowledge-base/studynote/02_operating_system/11_exam_summary/677_trap_based_system_call_implementation/) ([Trap](/knowledge-base/studynote/02_operating_system/11_exam_summary/677_trap_based_system_call_implementation/))** | **소프트웨어 (App)** | 특정 [명령어](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/158_instruction/)(`INT`) 의도적 실행 | **동기적 ([Trap](/knowledge-base/studynote/02_operating_system/11_exam_summary/677_trap_based_system_call_implementation/))** | 시스템 콜, 디버그(Breakpoint). 실행 후 **다음 [명령어](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/158_instruction/)**부터 재개. |

이 세 가지 모두 결국 **IDT([인터럽트 벡터](/knowledge-base/studynote/02_operating_system/01_overview_architecture/019_interrupt_vector/) 테이블)**를 조회하여 각자의 [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/) 함수로 점프한다는 점에서는 아키텍처가 100% 동일하다.

### 과목 융합 관점

- **[운영체제](/knowledge-base/studynote/02_operating_system/01_overview_architecture/001_operating_system_purpose/) (OS)**: 디바이스 드라이버를 개발할 때 `request_irq()` 함수를 호출한다. 이 함수가 하는 일이 바로, [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/)이 미리 잡아놓은 IDT 테이블의 특정 벡터(예: 44번) 뒤에 존재하는 [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/) 링크드 리스트에 **내가 짠 C 함수([ISR](/knowledge-base/studynote/02_operating_system/01_overview_architecture/020_isr/))**를 등록해 주는 과정이다.
- **[가상화](/knowledge-base/studynote/13_cloud_architecture/01_virtualization/015_virtualization/) (Cloud)**: 가상머신([KVM](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/713_kvm_over_ip/))은 이 IDT를 어떻게 훔쳐볼까? 인텔의 [가상화](/knowledge-base/studynote/13_cloud_architecture/01_virtualization/015_virtualization/) 하드웨어(VT-x)는 VM이 [인터럽트](/knowledge-base/studynote/02_operating_system/01_overview_architecture/016_interrupt_mechanism/)를 받을 때 진짜 CPU의 IDT를 타지 않고, VM만을 위한 가짜 IDT([VMCS](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/529_vmcs/) 내부에 세팅)를 타도록 하드웨어 레벨에서 완벽히 속인다(IDT Shadowing). 
...
1. **시나리오 — IRQ 충돌(Conflict)로 인한 디바이스 인식 불가**: 옛날 리눅스 서버에 새 랜카드를 꽂았더니, 기존에 있던 사운드 카드가 먹통이 되고 랜카드도 핑이 빠짐.
   ...
   - **대응 (기술사적 가이드)**: 최신 시스템은 **APIC (Advanced Programmable [Interrupt](/knowledge-base/studynote/02_operating_system/01_overview_architecture/016_interrupt_mechanism/) Controller)**와 **MSI-X ([Message Signaled Interrupts](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/561_msi/))**를 사용한다. 디바이스가 물리적 선을 쓰지 않고 메모리에 벡터 번호를 직접 써서 수백 개의 독립된 [인터럽트 벡터](/knowledge-base/studynote/02_operating_system/01_overview_architecture/019_interrupt_vector/)를 사용할 수 있다. 관리자는 BIOS에서 MSI-X를 활성화하여 IRQ 충돌을 원천 차단해야 한다.

2. **시나리오 — 클라우드/보안 환경에서의 IDT 후킹 ([Rootkit](/knowledge-base/studynote/02_operating_system/10_security/603_rootkit_syscall_hooking/) 탐지)**: 보안 관제팀이 서버 메모리를 덤프 떠서 분석해 보니, IDT의 0x80번(시스템 콜) 엔트리 주소가 [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/) 영역이 아닌 이상한 메모리 공간을 가리키고 있었다.
   ...
   - **대응**: 최신 리눅스/윈도우 [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/)은 **KPP ([Kernel](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/) Patch [Protection](/knowledge-base/studynote/02_operating_system/10_security/571_protection_vs_security/), PatchGuard)**나 Read-Only 메모리 속성을 통해 IDT가 쓰여진 메모리 [페이지](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/)를 강제로 잠가버린다. 이 기능이 활성화(`CONFIG_STRICT_KERNEL_RWX`)되어 있는지 [확인](/knowledge-base/studynote/04_software_engineering/12_testing_maintenance/396_validation/)하고, 시스템의 무결성을 검증해야 한다.
...
|:---|:---|:---|:---|
| **정량 ([탐색 시간](/knowledge-base/studynote/01_computer_architecture/08_io_storage_systems/324_seek_time/))** | 장치 수(N)에 비례하는 $O(N)$ [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/) | **즉각적 $O(1)$ 점프 (하드웨어 매핑)** | 이벤트 응답성 수만 배 향상 |
...
### 미래 전망
- **MSI-X를 통한 100% [가상화](/knowledge-base/studynote/13_cloud_architecture/01_virtualization/015_virtualization/) 매핑**: 차세대 디바이스([NVMe](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/482_nvme/), [DPU](/knowledge-base/studynote/01_computer_architecture/12_accelerators_ai_hardware/436_dpu/))는 기존 256개의 좁은 벡터 테이블을 넘어선다. MSI-X를 통해 장치가 무려 2,000개가 넘는 가상 벡터(메시지)를 쏘면, 하이퍼바이저가 이를 가로채지 않고 IOMMU를 통해 가상머신의 vCPU IDT에 직결로 꽂아버리는([Interrupt](/knowledge-base/studynote/02_operating_system/01_overview_architecture/016_interrupt_mechanism/) [Passthrough](/knowledge-base/studynote/02_operating_system/10_security/657_vfio_virtual_function_io_passthrough/)) 완전한 제로 오버헤드 아키텍처가 표준이 되었다.
- **[eBPF](/knowledge-base/studynote/02_operating_system/10_security/615_ebpf/) 기반 동적 [인터럽트 핸들러](/knowledge-base/studynote/02_operating_system/01_overview_architecture/021_interrupt_handler/)**: 기존에는 IDT 엔트리에 연결된 함수를 바꾸려면 [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/) 모듈을 다시 짜서 올려야 했지만, 최근 리눅스 [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/)에서는 하드웨어 예외나 [트랩](/knowledge-base/studynote/02_operating_system/11_exam_summary/677_trap_based_system_call_implementation/) 벡터가 발생했을 때 [JIT](/knowledge-base/studynote/09_security/11_iam_access_control/568_jit_access/) 컴파일된 [eBPF](/knowledge-base/studynote/02_operating_system/10_security/615_ebpf/) 프로그램이 이를 즉각 낚아채서 유저 공간으로 넘겨주거나 로깅하는 동적 라우팅이 발전하고 있다.
