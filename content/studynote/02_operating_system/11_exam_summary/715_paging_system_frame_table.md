...
- **개념**: 
  - **[페이지](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/) ([Page](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/))**: [가상 메모리](/knowledge-base/studynote/02_operating_system/07_virtual_memory/381_virtual_memory/)를 자르는 고정된 블록 단위 (보통 4KB).
...
- **필요성 ([연속 할당](/knowledge-base/studynote/02_operating_system/09_file_system/523_contiguous_allocation/)의 한계 돌파)**: 
  ...
  - **[연속 할당](/knowledge-base/studynote/02_operating_system/09_file_system/523_contiguous_allocation/)**: 50페이지짜리 소설책 1권을 통째로 꽂을 수 있는 빈 책장을 찾는다. 책장이 조금씩 비어있어도 연속된 빈칸이 없으면 책을 꽂을 수 없다.
  - **[페이징](/knowledge-base/studynote/02_operating_system/04_synchronization/259_paging/)**: 소설책을 1장([Page](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/))씩 뜯어버린다. 그리고 도서관 책장의 빈 곳(Frame) 아무 데나 마구잡이로 한 장씩 꽂아 넣는다. 
  - **[페이지 테이블](/knowledge-base/studynote/02_operating_system/06_memory_management/353_page_table/)**: "이 소설의 1페이지는 도서관 30번 칸에 있고, 2페이지는 80번 칸에 있다"고 적어놓은 개인용 [인덱스](/knowledge-base/studynote/05_database/03_relational_model/154_database_index_b_tree_search_optimization/).
  - **프레임 테이블**: 도서관 사서가 들고 있는 장부. "현재 도서관의 30번 칸은 사용 중, 31번 칸은 빔"이라고 전체 빈자리를 관리하는 지도.

- **발전 과정**:
  1. **[연속 메모리 할당](/knowledge-base/studynote/02_operating_system/06_memory_management/338_contiguous_memory_allocation/)**: [단편화](/knowledge-base/studynote/03_network/06_network_layer_ip/291_fragmentation_and_reassembly_process/)로 인해 [OOM](/knowledge-base/studynote/02_operating_system/02_process_thread/157_oom_killer/)([Out of memory](/knowledge-base/studynote/02_operating_system/02_process_thread/157_oom_killer/)) 빈발.
  2. **단일 [페이징](/knowledge-base/studynote/02_operating_system/04_synchronization/259_paging/)**: [외부 단편화](/knowledge-base/studynote/02_operating_system/06_memory_management/342_external_fragmentation/)는 잡았으나, [페이지 테이블](/knowledge-base/studynote/02_operating_system/06_memory_management/353_page_table/) 자체가 너무 커서 메모리를 다 잡아먹는 새로운 문제 발생.
  3. **계층적(Hierarchical) [페이징](/knowledge-base/studynote/02_operating_system/04_synchronization/259_paging/)**: [페이지 테이블](/knowledge-base/studynote/02_operating_system/06_memory_management/353_page_table/)의 용량 문제를 다단계 구조로 쪼개어 해결.
...
- **역할 2 ([OOM](/knowledge-base/studynote/02_operating_system/02_process_thread/157_oom_killer/) 방어 및 [Swapping](/knowledge-base/studynote/02_operating_system/06_memory_management/335_swapping/))**: 램이 꽉 차서 빈 프레임이 없을 때, OS는 프레임 테이블을 뒤져서 "요즘 제일 안 쓴 프레임([LRU](/knowledge-base/studynote/02_operating_system/04_synchronization/262_lru_page_replacement/))이 누구지?"를 찾은 뒤, 그 놈을 하드디스크(Swap)로 쫓아내고 빈자리를 만든다.
...
| **[단편화](/knowledge-base/studynote/03_network/06_network_layer_ip/291_fragmentation_and_reassembly_process/) 문제** | [외부 단편화](/knowledge-base/studynote/02_operating_system/06_memory_management/342_external_fragmentation/) 0%, **[내부 단편화](/knowledge-base/studynote/02_operating_system/06_memory_management/341_internal_fragmentation/) 발생** | [내부 단편화](/knowledge-base/studynote/02_operating_system/06_memory_management/341_internal_fragmentation/) 0%, **[외부 단편화](/knowledge-base/studynote/02_operating_system/06_memory_management/342_external_fragmentation/)(치명적) 발생** |
...
1. **시나리오 — [페이지 테이블](/knowledge-base/studynote/02_operating_system/06_memory_management/353_page_table/) 크기 폭발로 인한 메모리 낭비 ([OOM](/knowledge-base/studynote/02_operating_system/02_process_thread/157_oom_killer/))**: 32비트 OS 시절, 4KB [페이지](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/)를 쓰면 4GB 메모리를 커버하기 위해 100만 개의 [페이지](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/)가 필요했다. [페이지 테이블](/knowledge-base/studynote/02_operating_system/06_memory_management/353_page_table/) 엔트리 1개가 4바이트면, 프로세스 1개당 '순수 지도를 그리는 데만' 4MB의 메모리를 먹었다. 프로세스가 1,000개 떠 있으면 지도 쪼가리가 램을 4GB나 잡아먹어 서버가 죽어버렸다.
...
- **공유 [페이지](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/) (Shared [Page](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/))**: 프로세스 100개가 크롬(Chrome)을 띄웠을 때, 크롬의 실행 코드(Text 영역) 100MB가 메모리에 100번 복사되면 10GB가 날아간다. OS는 물리 프레임 테이블에 크롬 코드를 딱 1번(100MB)만 올려두고, 100개 프로세스의 [페이지 테이블](/knowledge-base/studynote/02_operating_system/06_memory_management/353_page_table/)이 모두 그 1개의 물리 프레임을 가리키도록 'Read-Only 공유' 처리를 했는지(메모리 맵핑 아키텍처) 이해하고 있는가?
...
|:---|:---|:---|:---|
| **정성 ([외부 단편화](/knowledge-base/studynote/02_operating_system/06_memory_management/342_external_fragmentation/))** | [OOM](/knowledge-base/studynote/02_operating_system/02_process_thread/157_oom_killer/) 잦음. 램 중간중간 구멍 발생 | **[외부 단편화](/knowledge-base/studynote/02_operating_system/06_memory_management/342_external_fragmentation/) 100% 소멸** | 조각난 램을 영끌해서 쓸 수 있음 (메모리 활용 극대화) |
