...
### [워킹 셋](/knowledge-base/studynote/02_operating_system/04_synchronization/265_working_set/) 추적 (Tracking) [알고리즘](/knowledge-base/studynote/08_algorithm_stats/01_basics/001_algorithm_definition/) 시뮬레이션

OS는 $\Delta$ ([Window Size](/knowledge-base/studynote/03_network/04_data_link_layer_error/215_window_size_sender_receiver/), 예: 최근 10번의 메모리 [참조](/knowledge-base/studynote/05_database/05_distributed_nosql_newsql/316_reference_pattern_nosql/))를 기준으로 [워킹 셋](/knowledge-base/studynote/02_operating_system/04_synchronization/265_working_set/)을 계산한다.
메모리 [참조](/knowledge-base/studynote/05_database/05_distributed_nosql_newsql/316_reference_pattern_nosql/) 문자열이 `[1, 2, 2, 3, 1, 5, 5, 5, 5, 5, 4, 4]`라고 하자. ($\Delta = 5$)

1. **시간 $t_1$ ([참조](/knowledge-base/studynote/05_database/05_distributed_nosql_newsql/316_reference_pattern_nosql/)열 앞부분 `1, 2, 2, 3, 1` 실행 직후)**:
   - 최근 5번 동안 [참조](/knowledge-base/studynote/05_database/05_distributed_nosql_newsql/316_reference_pattern_nosql/)한 [페이지](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/) 번호를 모아본다: {1, 2, 3}
   - 중복을 제거하면 $WS(t_1) = \{1, 2, 3\}$.
   - **결론**: 이 프로세스는 지금 3개의 프레임만 주면 [Page Fault](/knowledge-base/studynote/02_operating_system/07_virtual_memory/387_page_fault/) 없이 잘 돌아간다.

2. **시간 $t_2$ ([참조](/knowledge-base/studynote/05_database/05_distributed_nosql_newsql/316_reference_pattern_nosql/)열 뒷부분 `5, 5, 5, 4, 4` 실행 직후)**:
   - 최근 5번 동안 [참조](/knowledge-base/studynote/05_database/05_distributed_nosql_newsql/316_reference_pattern_nosql/)한 [페이지](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/): {5, 4}
   - $WS(t_2) = \{4, 5\}$.
...
### [워킹 셋](/knowledge-base/studynote/02_operating_system/04_synchronization/265_working_set/)([Working Set](/knowledge-base/studynote/02_operating_system/04_synchronization/265_working_set/)) 모델 vs [PFF](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/306_pff/) ([Page Fault Frequency](/knowledge-base/studynote/02_operating_system/04_synchronization/266_page_fault_frequency/))

[워킹 셋](/knowledge-base/studynote/02_operating_system/04_synchronization/265_working_set/) 이론은 완벽하지만, 매번 메모리를 읽을 때마다 $\Delta$ 윈도우를 갱신하는 건 CPU 연산의 큰 낭비다. 그래서 현실적인 PFF가 등장했다.
...
- **Swapiness 튜닝**: 리눅스 커널의 `vm.swappiness` (0~100) 값은 OS가 [파일](/knowledge-base/studynote/02_operating_system/09_file_system/501_file_definition_logical_record/) 캐시를 버릴지, 앱의 [워킹 셋](/knowledge-base/studynote/02_operating_system/04_synchronization/265_working_set/)([익명 메모리](/knowledge-base/studynote/02_operating_system/07_virtual_memory/391_anonymous_memory/))을 버릴지 결정하는 지표다. 100에 가까울수록 앱의 [워킹 셋](/knowledge-base/studynote/02_operating_system/04_synchronization/265_working_set/)을 디스크로 쫓아내는 걸 선호한다. 일반 서버는 60이지만, DB 서버는 내 [워킹 셋](/knowledge-base/studynote/02_operating_system/04_synchronization/265_working_set/)을 지키기 위해 무조건 `1` 또는 `10` 수준으로 낮춰서 [스래싱](/knowledge-base/studynote/02_operating_system/04_synchronization/257_thrashing/)을 방어했는가?

- **📢 섹션 요약 비유**: 넓은 책상(RAM)이 있어도, 시험공부에 필요한 책([워킹 셋](/knowledge-base/studynote/02_operating_system/04_synchronization/265_working_set/))이 100권이라면 결국 책상은 좁아집니다. 책상을 늘리는 데는 한계가 있으니, 공부 방식 자체를 '오늘은 1단원 책 3권만 완벽히 끝낸다(지역성)'로 바꿔야 성적이 오릅니다.
...
### 미래 전망
- **Non-Volatile Memory (NVM/PMEM)와의 융합**: 디스크가 램만큼 빨라지는 옵테인(Optane) 같은 비휘발성 메모리 시대가 오면서 [워킹 셋](/knowledge-base/studynote/02_operating_system/04_synchronization/265_working_set/)의 개념이 흔들리고 있다. [Page](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/) Fault를 처리해 디스크에서 데이터를 가져오는 속도 자체가 램 속도와 비슷해진다면, 굳이 복잡하게 [워킹 셋](/knowledge-base/studynote/02_operating_system/04_synchronization/265_working_set/)을 추적하고 방어할 필요 없이 [스래싱](/knowledge-base/studynote/02_operating_system/04_synchronization/257_thrashing/) 자체를 하드웨어 대역폭으로 짓누를 수 있게 되는 패러다임 전환이 일어나고 있다.

### 결론
[워킹 셋](/knowledge-base/studynote/02_operating_system/04_synchronization/265_working_set/)([Working Set](/knowledge-base/studynote/02_operating_system/04_synchronization/265_working_set/))은 "인간의 행동에는 패턴이 있다"는 지역성의 원리를 메모리 관리에 이식한 가장 아름다운 모델이다. 이 모델은 무한정 프로그램을 띄워대며 자멸로 향하던 초창기 운영체제의 탐욕을 멈춰 세우고, "자원의 한계를 인정하고, 살릴 놈만 확실히 살리자"는 잔인하지만 필수적인 철학([스왑 아웃](/knowledge-base/studynote/02_operating_system/06_memory_management/336_swap_out_in/))을 정립했다. 우리가 스마트폰에서 수십 개의 앱을 켜도 현재 보고 있는 앱이 절대 끊기지 않는 이유는, 뒤에 숨은 백그라운드 앱들을 냉혹하게 재워버리며 내 눈앞의 [워킹 셋](/knowledge-base/studynote/02_operating_system/04_synchronization/265_working_set/)을 사수해 주는 OS의 눈물겨운 사투 덕분이다.

- **📢 섹션 요약 비유**: 10개의 화분을 키울 때 물(메모리)이 부족하면, 모든 화분에 물을 1/10씩 찔끔찔끔 줘서 10개를 다 말려 죽이는 것([스래싱](/knowledge-base/studynote/02_operating_system/04_synchronization/257_thrashing/))은 최악입니다. 3개 화분에만 물([워킹 셋](/knowledge-base/studynote/02_operating_system/04_synchronization/265_working_set/))을 듬뿍 줘서 살려내고 7개는 어두운 창고(디스크)에 잠시 치워두는 것이 진정한 식물(시스템) 관리의 지혜입니다.
...
| 개념 | 연결 포인트 |
|:---|:---|
| [최적 알고리즘](/knowledge-base/studynote/02_operating_system/11_exam_summary/724_optimal_page_replacement_unrealizable/) ([OPT](/knowledge-base/studynote/02_operating_system/11_exam_summary/724_optimal_page_replacement_unrealizable/)) 구현 불가 | 현재 개념으로 들어오기 전에 함께 이해하면 경계가 선명해지는 기반 개념이다. |
| [스래싱](/knowledge-base/studynote/02_operating_system/04_synchronization/257_thrashing/) ([Thrashing](/knowledge-base/studynote/02_operating_system/04_synchronization/257_thrashing/)) CPU 이용률 저하 | 현재 개념이 등장하게 만든 직접적인 선행 흐름이다. |
| [디스크 스케줄링](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/468_disk_scheduling_purpose/) SCAN 엘리베이터 | 현재 개념이 구현·세분화될 때 바로 연결되는 후속 개념이다. |
| [C-SCAN](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/472_c_scan_scheduling/) [단방향](/knowledge-base/studynote/03_network/01_data_communication/008_단방향_반이중_전이중/) 회전 | 확장 학습이나 심화 비교로 이어지는 다음 단계의 키워드다. |
...
1. 철수가 책상(램)에서 '로봇 조립' 숙제를 할 때, 로봇 머리, 팔, 다리 부품 3개([워킹 셋](/knowledge-base/studynote/02_operating_system/04_synchronization/265_working_set/))는 무조건 책상 위에 있어야만 화내지 않고 조립을 할 수 있어요.
2. 만약 동생이 책상을 차지해서 부품 1개를 바닥(디스크)에 내려놔야 한다면? 철수는 1초마다 바닥에서 팔 줍고, 머리 내리고 하느라 조립(CPU 연산)은 1개도 못 하고 땀만 뻘뻘 흘립니다([스래싱](/knowledge-base/studynote/02_operating_system/04_synchronization/257_thrashing/)).
3. 엄마(OS)가 나타나서 규칙을 정했어요! "철수가 조립할 땐 무조건 부품 3개 놓을 자리는 보장해 줘! 책상이 모자라면 동생 너는 방 밖(Swap-out)으로 나가서 기다려!" 이게 바로 [워킹 셋](/knowledge-base/studynote/02_operating_system/04_synchronization/265_working_set/)을 지켜주는 원리랍니다.
