...
> 1. **본질**: 이 세 가지 개념은 애플리케이션 프로세스가 OS에게 느린 I/O(디스크, 네트워크) 작업을 요청했을 때, <strong>"결과가 나올 때까지 내(<a href="/knowledge-base/studynote/12_it_management/05_security_compliance/943_process/">Process</a>)가 멈춰서 기다릴 것인가?(제어권 반환 시점)"</strong>와 <strong>"결과가 나왔는지 내가 직접 챙길 것인가, 아니면 OS가 나를 찔러줄 것인가?(완료 통지 방식)"</strong>를 결정하는 시스템 아키텍처의 패러다임이다.
