...
- **개념**: 프로세스 실행에 필요한 최소한의 [페이지](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/) 프레임조차 램(RAM)에 할당받지 못해, 명령어를 실행할 때마다 [페이지 폴트](/knowledge-base/studynote/02_operating_system/11_exam_summary/720_page_fault_isr/)([Page Fault](/knowledge-base/studynote/02_operating_system/07_virtual_memory/387_page_fault/))가 발생하여 디스크 I/O 대기열에 갇혀버리는 악순환이다. "CPU가 일은 안 하고 짐만 나르는 상태"다.
- **필요성**: 컴퓨터 과학자들은 [페이징](/knowledge-base/studynote/02_operating_system/04_synchronization/259_paging/) 기법 덕분에 램보다 큰 프로그램을 맘껏 띄우며 환호했다. 그런데 "여러 개 띄워도 잘 도니까, 아예 1,000개를 띄워보자!"라고 욕심을 내자, 어느 순간 컴퓨터가 굉음을 내며 마우스 커서조차 멈추는 기괴한 현상이 발생했다. 시스템 설계자들은 CPU 이용률이 급락하는 이 치명적 절벽(Thrashing Point)을 수학적으로 분석하고 통제할 안전장치가 절실했다.
...
- **📢 섹션 요약 비유**: 도로(RAM)에 차가 적당히 많으면 통행량(CPU 이용률)이 극대화됩니다. 하지만 꽉 막힌 도로에 억지로 차(프로세스)를 더 집어넣으면 꼬리물기([페이지 폴트](/knowledge-base/studynote/02_operating_system/11_exam_summary/720_page_fault_isr/))가 터져서 도로 전체가 주차장이 되어 단 1대도 움직이지 못하는 헬게이트(스래싱)가 열립니다.
...
### 스래싱 예방을 위한 2대 학술적 [알고리즘](/knowledge-base/studynote/08_algorithm_stats/01_basics/001_algorithm_definition/)

[운영체제](/knowledge-base/studynote/02_operating_system/01_overview_architecture/001_operating_system_purpose/)가 스래싱 임계점(B)을 넘지 않게 하려면, "각 프로세스가 숨을 쉬기 위해 최소한 몇 개의 [페이지](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/) 프레임이 필요한가?"를 알아야 한다.
...
### 기대효과
스래싱의 본질을 이해하고 인프라 레벨에서 스왑을 차단하거나 메모리 Limit([Cgroups](/knowledge-base/studynote/02_operating_system/01_overview_architecture/062_cgroups/))을 정교하게 걸어두면, 악의적인 트래픽 폭주나 메모리 릭(Leak)이 터지더라도 서버 전체가 렉에 걸려 동반 자살하는 끔찍한 연쇄 장애(Cascading Failure)를 100% 방어해 낼 수 있다.

### 결론 및 미래 전망
스래싱은 [가상 메모리](/knowledge-base/studynote/02_operating_system/07_virtual_memory/381_virtual_memory/)([요구 페이징](/knowledge-base/studynote/02_operating_system/04_synchronization/255_demand_paging/))라는 달콤한 마법이 빚어낸 가장 치명적인 청구서다. 학자들은 [워킹 셋](/knowledge-base/studynote/02_operating_system/04_synchronization/265_working_set/), [PFF](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/306_pff/) 등 우아한 [알고리즘](/knowledge-base/studynote/08_algorithm_stats/01_basics/001_algorithm_definition/)으로 이 빚을 갚아보려 했으나, 동적으로 변하는 현대 애플리케이션 앞에서는 모두 무용지물이 되었다.
결국 이 복잡한 [알고리즘](/knowledge-base/studynote/08_algorithm_stats/01_basics/001_algorithm_definition/)들은 "RAM 값이 똥값이 되는 하드웨어 혁명"과 "스왑을 꺼버리는(Swap-off) 클라우드의 Fail-Fast 철학" 앞에서 완전히 사멸했다. 미래의 인프라는 소프트웨어적으로 램을 아끼려는 쪼잔한 노력을 멈추고, AWS EC2에서 버튼 한 번으로 1TB RAM 인스턴스를 1초 만에 프로비저닝하거나 [서버리스](/knowledge-base/studynote/12_it_management/05_security_compliance/206_serverless_cold_start/)([Serverless](/knowledge-base/studynote/12_it_management/05_security_compliance/206_serverless_cold_start/))로 무한 확장하는 **"압도적 물리량으로 스래싱을 찢어버리는"** 자본주의 아키텍처로 완전한 결론을 맺었다.

- **📢 섹션 요약 비유**: 옛날엔 식량이 부족해서 배고픔(스래싱)을 참는 명상법([워킹 셋](/knowledge-base/studynote/02_operating_system/04_synchronization/265_working_set/) [알고리즘](/knowledge-base/studynote/08_algorithm_stats/01_basics/001_algorithm_definition/))을 수련했습니다. 하지만 현대 시대는 그냥 마트(아마존 클라우드)에 가서 돈을 내고 고기(RAM)를 무한정 사 먹으면 됩니다. 스래싱은 굶주리던 시절의 슬픈 질병일 뿐, 풍요로운 현대 클라우드 시대에는 돈(스케일업)으로 1초 만에 완치되는 병입니다.
...
| [락-프리](/knowledge-base/studynote/02_operating_system/04_synchronization/256_lock_free_data_structures/) ([Lock-free](/knowledge-base/studynote/02_operating_system/04_synchronization/256_lock_free_data_structures/)) 자료구조 | 현재 개념이 등장하게 만든 직접적인 선행 흐름이다. |
| [스케줄러](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/079_kube_scheduler_pod_placement/) 일드 (sched_yield) | 현재 개념이 구현·세분화될 때 바로 연결되는 후속 개념이다. |
