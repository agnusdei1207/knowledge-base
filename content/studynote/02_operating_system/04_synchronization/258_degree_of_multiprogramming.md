...
- **개념**: 컴퓨터 시스템 안에서 디스크에 잠들어 있지 않고, 실제로 메모리를 차지한 채 실행을 위해 CPU를 노리고 있는 활성 프로세스([Active](/knowledge-base/studynote/03_network/09_application_layer_web_email/483_active_vs_passive_ftp/) [Process](/knowledge-base/studynote/12_it_management/05_security_compliance/300_process/))의 숫자를 뜻한다.
...
CPU 이용률을 예측하는 유명한 [확률](/knowledge-base/studynote/08_algorithm_stats/08_stats/130_probability/) 모델 수식이 있다.
한 프로세스가 I/O 대기하며 CPU를 쓰지 않을 [확률](/knowledge-base/studynote/08_algorithm_stats/08_stats/130_probability/)(I/O Wait Ratio)을 **$p$** 라고 하자.
...
### [다중 프로그래밍](/knowledge-base/studynote/02_operating_system/11_exam_summary/673_multiprogramming_bottleneck_resource/) ([Multiprogramming](/knowledge-base/studynote/02_operating_system/11_exam_summary/673_multiprogramming_bottleneck_resource/)) vs 시분할 (Time-sharing) vs 다중 처리 (Multiprocessing)
...
### 기대효과
[다중 프로그래밍](/knowledge-base/studynote/02_operating_system/11_exam_summary/673_multiprogramming_bottleneck_resource/) 정도(DOM)를 시스템 하드웨어 스펙(CPU 코어 수, RAM 크기)에 딱 맞는 스위트 스팟(Sweet Spot)으로 튜닝하면, 디스크 I/O에 낭비되는 시간을 완벽하게 숨기면서 CPU 이용률을 99%로 유지하는 극강의 가성비(Cost-Effectiveness) 런타임 환경을 구축할 수 있다.
...
| 개념 | 연결 포인트 |
|:---|:---|
| [락-프리](/knowledge-base/studynote/02_operating_system/04_synchronization/256_lock_free_data_structures/) ([Lock-free](/knowledge-base/studynote/02_operating_system/04_synchronization/256_lock_free_data_structures/)) 자료구조 | 현재 개념으로 들어오기 전에 함께 이해하면 경계가 선명해지는 기반 개념이다. |
| 웨이트-프리 (Wait-free) [알고리즘](/knowledge-base/studynote/08_algorithm_stats/01_basics/001_algorithm_definition/) | 현재 개념이 등장하게 만든 직접적인 선행 흐름이다. |
...
2. 그래서 동네 사람 5명([다중 프로그래밍](/knowledge-base/studynote/02_operating_system/11_exam_summary/673_multiprogramming_bottleneck_resource/) 정도 = 5)이 세탁기 앞에 대기하면서, 세탁기가 잠깐 쉴 때마다 잽싸게 자기 빨래를 밀어 넣는 거예요!
