...
| 유형 | 설명 | 예시 |
|---|---|---|
| **날짜 기반** | 특정 날짜에 동작 | "2025년 12월 31일에 [파일](/knowledge-base/studynote/02_operating_system/09_file_system/501_file_definition_logical_record/) 삭제" |
| **시간 기반** | 특정 시각에 동작 | "매일 자정마다 [백도어](/knowledge-base/studynote/03_network/14_network_security_threats/737_backdoor_c2_beacon_behavior_analysis/) 활성화" |
| **카운트 기반** | N번째 실행 시 동작 | "[파일](/knowledge-base/studynote/02_operating_system/09_file_system/501_file_definition_logical_record/) 열기 30번째마다 악성 코드 실행" |
| **이벤트 기반** | 특정 이벤트 발생 시 | "직원 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 삭제 시 같이 실행" |
...
3. 1개월 후 회사는 A를 계속 고용하지 않기로 결정했고, 퇴사 후 31번째 급여 처리 시 시스템 내 모든 고객 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를Encrypt(암호화)하여 인도(유도)했다.
...
### 방어(방어) [전략](/knowledge-base/studynote/04_software_engineering/04_testing_quality/268_strategy_pattern/)
...
로직bomb과 시간bomb은 외부 해킹보다 내부자 위협에 더유효성(효과적)이며, 단순한 방화벽이나 [AV](/knowledge-base/studynote/09_security/04_endpoint_security/323_antivirus/)(안티바이러스) 솔루션으로는 탐지하기 어렵다. 따라서 다음과 같은 다층 방어 [전략](/knowledge-base/studynote/04_software_engineering/04_testing_quality/268_strategy_pattern/)이 필요하다:
...
- **사후 대응**: 정기적인 [백업](/knowledge-base/studynote/02_operating_system/09_file_system/555_backup_and_restore_strategy/) 및Incident Response([사고 대응](/knowledge-base/studynote/09_security/01_intro_principles/009_incident_response/)) 계획 수립
...
| 개념 | 연결 포인트 |
|:---|:---|
| [트로이 목마](/knowledge-base/studynote/02_operating_system/10_security/586_trojan_horse_wrapper/) ([Trojan Horse](/knowledge-base/studynote/02_operating_system/10_security/586_trojan_horse_wrapper/)) / 래퍼 (Wrapper) | 현재 개념으로 들어오기 전에 함께 이해하면 경계가 선명해지는 기반 개념이다. |
| [트랩 도어](/knowledge-base/studynote/02_operating_system/10_security/587_backdoor_trapdoor/) ([Trap](/knowledge-base/studynote/02_operating_system/11_exam_summary/677_trap_based_system_call_implementation/) Door / [Backdoor](/knowledge-base/studynote/09_security/15_malware_attack_vectors/727_backdoor/)) | 현재 개념이 등장하게 만든 직접적인 선행 흐름이다. |
| [바이러스](/knowledge-base/studynote/02_operating_system/10_security/589_virus/) ([Virus](/knowledge-base/studynote/02_operating_system/10_security/589_virus/)) | 현재 개념이 구현·세분화될 때 바로 연결되는 후속 개념이다. |
| 웜 ([Worm](/knowledge-base/studynote/02_operating_system/10_security/590_worm/)) | 확장 학습이나 심화 비교로 이어지는 다음 단계의 키워드다. |
...
3. 그래서 우리느 컴퓨터에 이상한 프로그램을 깔거나, 누가 건네준 [파일](/knowledge-base/studynote/02_operating_system/09_file_system/501_file_definition_logical_record/)을 열 때 조심해야 해.virus처럼 보이지 않다가도 나중에 문제을 일으킬 수 있거든.
