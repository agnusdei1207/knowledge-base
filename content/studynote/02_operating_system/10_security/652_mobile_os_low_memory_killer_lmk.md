...
### 앱 상태([State](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/272_state_pattern/)) 기반 OOM_ADJ 점수 체계

안드로이드의 ActivityManagerService(AMS)는 앱이 화면에 띄워져 있는지, 숨겨져 있는지 계속 감시하며 리눅스 [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/)의 `/proc/<pid>/oom_score_adj` (과거 `oom_adj`) 파일에 점수를 실시간으로 업데이트한다. 점수가 높을수록 먼저 죽는다.

| 상태 (App [State](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/272_state_pattern/)) | OOM_ADJ 점수 (예시) | 의미 / 특징 | 비유 (우선순위) |
|:---|:---|:---|:---|
...
### [Low Memory Killer](/knowledge-base/studynote/02_operating_system/11_exam_summary/787_android_lmk_low_memory_killer/) (LMKD) 동작 [알고리즘](/knowledge-base/studynote/08_algorithm_stats/01_basics/001_algorithm_definition/)
...
- **onSaveInstanceState()**: 안드로이드 OS는 앱이 백그라운드로 내려가서 Cached 상태가 되기 직전에, 앱에게 "너 LMK한테 곧 죽을지도 몰라. 유언([현재 상태](/knowledge-base/studynote/04_software_engineering/03_design_architecture/178_as_is_to_be_analysis/))을 남겨"라고 [신호](/knowledge-base/studynote/02_operating_system/02_process_thread/130_signal/)(Callback)를 준다.
...
### [OOM Killer](/knowledge-base/studynote/02_operating_system/07_virtual_memory/425_oom_killer_score/) vs [Low Memory Killer](/knowledge-base/studynote/02_operating_system/11_exam_summary/787_android_lmk_low_memory_killer/) (LMK)

| 비교 항목 | Linux 기본 [OOM Killer](/knowledge-base/studynote/02_operating_system/07_virtual_memory/425_oom_killer_score/) | Android [Low Memory Killer](/knowledge-base/studynote/02_operating_system/11_exam_summary/787_android_lmk_low_memory_killer/) (LMK) |
|:---|:---|:---|
| **발동 시점** | 램과 스왑이 완전히 0%일 때 (사후 약방문) | 여유 램이 설정된 [임계치](/knowledge-base/studynote/03_network/08_transport_layer/431_ssthresh_slow_start_threshold/) 이하로 떨어질 때 (예방적) |
...
- **ZRAM 튜닝**: 최근 안드로이드는 LMK가 앱을 죽이기 전에, 메모리의 50% 정도를 [압축](/knowledge-base/studynote/02_operating_system/06_memory_management/347_compaction/)(ZRAM)하여 스왑으로 쓴다. 제조사(OEM) [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/) 엔지니어는 ZRAM의 크기와 LMKD의 [임계치](/knowledge-base/studynote/03_network/08_transport_layer/431_ssthresh_slow_start_threshold/)(Threshold)를 튜닝하여 "앱을 튕기게 할 것인가, [압축](/knowledge-base/studynote/02_operating_system/06_memory_management/347_compaction/)/해제로 폰을 살짝 버벅이게 할 것인가" 사이에서 폰의 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) 캐릭터를 결정했는가?
- **PSI (Pressure Stall Information)**: 구형 LMK는 램이 부족해진 걸 확인하고 죽이느라 이미 폰이 버벅인 뒤였다. [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/) 4.20부터 도입된 PSI를 LMKD에 연동하면, 메모리 부족으로 인해 CPU가 대기(Stall)하는 '압박 비율'을 감지하여 폰이 버벅이기 0.1초 전에 미리 앱을 죽여 극강의 부드러움을 유지할 수 있다.
...
| 구분 | 일반 [OOM Killer](/knowledge-base/studynote/02_operating_system/07_virtual_memory/425_oom_killer_score/) (모바일에 적용 시) | LMK ([Low Memory Killer](/knowledge-base/studynote/02_operating_system/11_exam_summary/787_android_lmk_low_memory_killer/)) 작동 | 개선 효과 |
|:---|:---|:---|:---|
...
| 개념 | 연결 포인트 |
|:---|:---|
| 고가용성 클러스터 [운영체제](/knowledge-base/studynote/02_operating_system/01_overview_architecture/001_operating_system_purpose/) 하트비트/펜싱 (Fencing / STONITH) 뇌 분할(Split-Brain) 방어 메커니즘 | 현재 개념으로 들어오기 전에 함께 이해하면 경계가 선명해지는 기반 개념이다. |
| [전력 인식](/knowledge-base/studynote/02_operating_system/10_security/651_power_aware_scheduler_dvfs/)([Power-aware](/knowledge-base/studynote/02_operating_system/10_security/651_power_aware_scheduler_dvfs/)) [스케줄러](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/079_kube_scheduler_pod_placement/) 동적 [전압](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/001_voltage/)/주파수 [스케일링](/knowledge-base/studynote/10_ai/03_llm_nlp/249_scaling_normalization_standardization/)([DVFS](/knowledge-base/studynote/01_computer_architecture/13_reliability_power_management/469_dvfs/)) 통합형 CPU 제어 | 현재 개념이 등장하게 만든 직접적인 선행 흐름이다. |
| [엣지 컴퓨팅](/knowledge-base/studynote/12_it_management/05_security_compliance/235_edge_computing_smart_factory/) OS (초경량/고속 부팅 최적화된 리눅스 환경 구성 기술망) | 현재 개념이 구현·세분화될 때 바로 연결되는 후속 개념이다. |
| [리얼타임 리눅스](/knowledge-base/studynote/02_operating_system/10_security/654_preempt_rt_linux_spinlock_mutex/) ([PREEMPT_RT](/knowledge-base/studynote/02_operating_system/10_security/654_preempt_rt_linux_spinlock_mutex/)) [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/) 스핀락을 뮤텍스로 변환하는 선점 허용 구조 개요 | 확장 학습이나 심화 비교로 이어지는 다음 단계의 키워드다. |
...
← **이전**: [651. 전력 인식(Power-aware) 스케줄러 동적 전압/주파수 스케일링(DVFS) 통합형 CPU 제어](/knowledge-base/studynote/02_operating_system/10_security/651_power_aware_scheduler_dvfs/)
**다음**: [653. 엣지 컴퓨팅 OS (초경량/고속 부팅 최적화된 리눅스 환경 구성 기술망) (Edge Computing OS Linux)](/knowledge-base/studynote/02_operating_system/10_security/653_edge_computing_os_linux/) →
