...
> 2. **메커니즘**: 단순히 하드웨어를 넘겨주는 것을 넘어, 하드웨어 **[IOMMU](/knowledge-base/studynote/02_operating_system/10_security/627_iommu_dma_isolation/) (Intel VT-d)**와 결합하여 디바이스의 [DMA](/knowledge-base/studynote/02_operating_system/11_exam_summary/746_io_direct_memory_access_dma/) 주소 접근 범위를 엄격한 [IOMMU](/knowledge-base/studynote/02_operating_system/10_security/627_iommu_dma_isolation/) 그룹(Group) 단위로 격리함으로써, 유저 스페이스가 물리 메모리를 마음대로 파괴하는 것을 원천 차단한다.
> 3. **가치**: 클라우드 환경에서 가상머신([VM](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/598_vm_migration_nic/))에 물리 GPU를 할당하거나(AWS p3/[p4](/knowledge-base/studynote/03_network/17_sdn_nfv/874_p4_programming_data_plane_pipeline_int_telemetry/) 인스턴스), [SR-IOV](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/497_sr_iov_pcie_mapping/) 네트워크 카드를 분배하여 **네이티브와 100% 동일한 제로 오버헤드 I/O [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/)**을 달성하는 현대 고성능 [데이터센터](/knowledge-base/studynote/03_network/16_data_center_cloud/801_data_center_3_tier_architecture_core_aggregation_access/)의 필수 기반 기술이다.
...
- **필요성 (에뮬레이션의 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) 한계와 구형 패스스루의 위험성)**: 
...
- **발전 과정**:
  ...
  2. **[KVM](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/713_kvm_over_ip/) 기반 [PCI](/knowledge-base/studynote/01_computer_architecture/09_system_bus_interconnects/355_pci/) Assignment (과거)**: [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/) 종속적, 보안 취약, 관리 복잡.
...
- **VFIO의 철칙**: **VFIO는 오직 "[IOMMU](/knowledge-base/studynote/02_operating_system/10_security/627_iommu_dma_isolation/) Group 전체"를 통째로 넘길 때만 작동한다.** 만약 그래픽 카드와 사운드 카드가 그룹 10번에 같이 묶여 있다면, 그래픽 카드만 VM에 넘기는 것은 불가능하다. 사운드 카드도 같이 넘기거나, 아니면 둘 다 못 넘긴다. (이는 [DMA](/knowledge-base/studynote/02_operating_system/11_exam_summary/746_io_direct_memory_access_dma/) [스푸핑](/knowledge-base/studynote/02_operating_system/10_security/598_spoofing/) 공격을 막기 위한 절대 원칙이다.)
...
|:---|:---|:---|:---|:---|
| **소프트웨어 계층** | QEMU가 전부 H/W 모사 | 프론트/백엔드 분할 ([vring](/knowledge-base/studynote/02_operating_system/10_security/658_virtio_paravirtualization_vring/)) | **우회 (Bypass), [직접 통신](/knowledge-base/studynote/02_operating_system/02_process_thread/120_direct_communication/)** | 디바이스를 VF로 쪼갠 후 VFIO 적용 |
| **CPU 오버헤드** | 극심함 | 보통 (메모리 카피 존재) | **거의 없음 (0%)** | 거의 없음 |
| **[성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/)** | 최하 | 우수함 | **네이티브(100%)** | 네이티브 (100%) |
...
| **[Live Migration](/knowledge-base/studynote/02_operating_system/10_security/629_live_migration_pre_copy/)**| 완벽 지원 | 완벽 지원 | **불가능 (하드웨어 종속됨)** | 원칙적 불가 (일부 벤더만 우회 지원) |

### 과목 융합 관점

- **컴퓨터구조 ([CA](/knowledge-base/studynote/06_ict_convergence/01_blockchain/089_contract_account_smart_contract/))**: 디바이스가 CPU에 신호를 보내는 방식인 [인터럽트](/knowledge-base/studynote/02_operating_system/01_overview_architecture/016_interrupt_mechanism/)([Interrupt](/knowledge-base/studynote/02_operating_system/01_overview_architecture/016_interrupt_mechanism/)) 처리에서, 기존의 물리적 선(INTx) 방식은 공유 문제가 있어 VFIO 패스스루에 부적합하다. 따라서 VFIO는 디바이스가 메모리에 핀 포인터로 메시지를 써서 [인터럽트](/knowledge-base/studynote/02_operating_system/01_overview_architecture/016_interrupt_mechanism/)를 유발하는 **[MSI](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/561_msi/)/[MSI](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/561_msi/)-X ([Message Signaled Interrupts](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/561_msi/))** 방식을 강제 또는 권장한다.
- **[클라우드 컴퓨팅](/knowledge-base/studynote/02_operating_system/01_overview_architecture/052_cloud_computing_os/) (Cloud)**: AWS의 Nitro 아키텍처나 클라우드 벤더들의 베어메탈(Bare-metal) 인스턴스는 사실 물리 서버를 통째로 주는 것이 아니라, 아주 가벼운 [KVM](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/713_kvm_over_ip/) [하이퍼바이저](/knowledge-base/studynote/02_operating_system/01_overview_architecture/054_hypervisor/) 위에서 로컬 [NVMe](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/482_nvme/) SSD와 [SR-IOV](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/497_sr_iov_pcie_mapping/) 네트워크 카드를 **VFIO로 100% 패스스루** 해준 거대한 가상머신이다. 
...
1. **시나리오 — 클라우드 [머신러닝](/knowledge-base/studynote/10_ai/03_llm_nlp/241_machine_learning_basics/) [GPU](/knowledge-base/studynote/01_computer_architecture/12_accelerators_ai_hardware/418_gpu/) 할당 시 [IOMMU](/knowledge-base/studynote/02_operating_system/10_security/627_iommu_dma_isolation/) Group 제약으로 인한 부팅 실패**: [KVM](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/713_kvm_over_ip/) 호스트에 2대의 NVIDIA GPU가 꽂혀 있다. 1번 GPU는 [VM](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/598_vm_migration_nic/) A에, 2번 GPU는 [VM](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/598_vm_migration_nic/) B에 할당(Passthrough)하려고 Libvirt 설정을 마쳤으나, [VM](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/598_vm_migration_nic/) A를 켜는 순간 "Please ensure all devices within the iommu_group are bound to their vfio [bus](/knowledge-base/studynote/01_computer_architecture/09_system_bus_interconnects/344_bus/) driver" 에러가 발생.
...
2. **시나리오 — [DPDK](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/671_dpdk/) 기반 통신사 코어망(UPF)의 VFIO 도입**: 기존에는 `uio_pci_generic` 드라이버를 이용해 유저 스페이스에서 DPDK로 패킷을 처리했다. 보안팀에서 IOMMU가 적용되지 않아 보안 위협이 있다고 지적함.
   - **아키텍처 적용**: [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/)의 UIO 드라이버를 버리고, 안전한 [IOMMU](/knowledge-base/studynote/02_operating_system/10_security/627_iommu_dma_isolation/) 매핑을 보장하는 **VFIO 드라이버(`vfio-pci`)**로 바인딩을 교체한다. [DPDK](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/671_dpdk/) 애플리케이션 시작 시 EAL 파라미터로 `--iova-mode=va`를 지정하여 가상 주소를 안전하게 매핑한다. [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) 저하 없이 네이티브 패킷 처리(Line-rate)를 유지하면서 보안팀의 컴플라이언스([DMA](/knowledge-base/studynote/02_operating_system/11_exam_summary/746_io_direct_memory_access_dma/) 격리)를 100% 충족시킨다.
...
- **호스트 메모리 점유 방지 ([Huge Pages](/knowledge-base/studynote/02_operating_system/06_memory_management/371_huge_pages/))**: VFIO로 GPU를 넘겨준 상태에서 게스트 OS의 메모리가 디스크로 [스왑 아웃](/knowledge-base/studynote/02_operating_system/06_memory_management/336_swap_out_in/)(Swap-out)되면, GPU가 DMA로 엉뚱한 데이터를 읽어 패닉이 난다. 따라서 VFIO를 쓰는 VM은 **반드시 메모리 전체를 램에 고정(Pinning)하거나 Huge Pages를 사용**하여 스왑을 원천 차단해야 한다.
- **[펌웨어](/knowledge-base/studynote/02_operating_system/01_overview_architecture/032_firmware/) 토폴로지 (VT-d / AMD-Vi)**: 서버 BIOS에서 단순히 [가상화](/knowledge-base/studynote/13_cloud_architecture/01_virtualization/015_virtualization/)(VT-x)뿐만 아니라 I/O [가상화](/knowledge-base/studynote/13_cloud_architecture/01_virtualization/015_virtualization/)(VT-d) 옵션을 명시적으로 Enable 했는지, 리눅스 부팅 파라미터에 `intel_iommu=on iommu=pt`가 정확히 들어갔는지 확인이 필수다.
...
| **정량 ([지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/))** | 수십 마이크로초 [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/) ([Context Switch](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/211_context_switch/)) | **수 마이크로초 내외 ([Direct](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/176_direct_addressing/) [DMA](/knowledge-base/studynote/02_operating_system/11_exam_summary/746_io_direct_memory_access_dma/))** | 지터(Jitter) 최소화 및 P99 극강화 |
