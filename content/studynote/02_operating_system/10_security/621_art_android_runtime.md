...
> 2. **진화**: [초기](/knowledge-base/studynote/03_network/08_transport_layer/459_quic_fec_forward_error_correction/) ART는 설치 시 완전히 네이티브 코드로 변환하는 AOT (Ahead-Of-Time) 컴파일 방식을 채택했으나, 안드로이드 7.0(Nougat)부터는 [JIT](/knowledge-base/studynote/09_security/11_iam_access_control/568_jit_access/) ([Just-In-Time](/knowledge-base/studynote/09_security/11_iam_access_control/568_jit_access/)) 컴파일러와 프로필 기반 AOT를 결합한 **혼합 컴파일 (Hybrid Compilation)** 방식을 사용하여 설치 시간, 저장 공간, 실행 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/)을 최적화했다.
> 3. **가치**: 이 혼합 환경은 자주 사용되는 핫 코드(Hot [Code](/knowledge-base/studynote/02_operating_system/02_process_thread/082_process_memory_structure/))만 AOT로 최적화하고 나머지는 인터프리터와 JIT로 처리함으로써, 제한된 모바일 자원(배터리, 저장공간) 환경에서 앱 실행 속도를 극대화하는 실무적 트레이드오프의 결정체다.
...
- **📢 섹션 요약 비유**: 수백만 명의 독자가 이미 읽으면서 중요하다고 밑줄 친 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 모아서, 새로운 독자가 책을 살 때는 처음부터 중요한 부분만 미리 번역해 놓은 베스트셀러 요약본을 제공하는 것과 같습니다.

---

## Ⅲ. 비교 및 연결

### 비교 1: Dalvik vs ART ([초기](/knowledge-base/studynote/03_network/08_transport_layer/459_quic_fec_forward_error_correction/)) vs ART (혼합)

| 비교 항목 | Dalvik (Android 4.4 이하) | [초기](/knowledge-base/studynote/03_network/08_transport_layer/459_quic_fec_forward_error_correction/) ART (Android 5/6) | 혼합 ART (Android 7.0 이상) |
|:---|:---|:---|:---|
...
1. **시나리오 — 대규모 모바일 게임 앱 (2GB 이상) 최적화**: 게임 시작 시마다 긴 로딩 시간과 [초기](/knowledge-base/studynote/03_network/08_transport_layer/459_quic_fec_forward_error_correction/) 프레임 드랍(Jank)이 발생. 개발자는 안드로이드의 `Baseline Profiles` 기능을 도입하여 게임 앱 내의 필수 시작 경로와 주요 렌더링 루프를 [프로파일링](/knowledge-base/studynote/02_operating_system/10_security/613_profiling_gprof/)한 규칙을 앱 패키지(APK)에 포함시켜 배포한다. 이를 통해 클라우드 프로필 수집을 기다리지 않고, 유저가 게임을 설치/업데이트하자마자 ART가 핵심 경로를 AOT 컴파일하게 유도하여 [초기](/knowledge-base/studynote/03_network/08_transport_layer/459_quic_fec_forward_error_correction/) 실행 [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/)을 30% 이상 감소시킬 수 있다.

2. **시나리오 — 저사양 디바이스 (Go Edition) 런타임 제어**: 메모리가 2GB 이하인 안드로이드 Go 디바이스에서는 [JIT](/knowledge-base/studynote/09_security/11_iam_access_control/568_jit_access/) 캐시 메모리조차 부담이 될 수 있다. OS [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/) 튜닝 엔지니어는 ART 데몬(dex2oat)의 동시 [스레드](/knowledge-base/studynote/02_operating_system/02_process_thread/092_thread_lwp/) 수와 메모리 사용 한계를 시스템 [속성](/knowledge-base/studynote/05_database/02_modeling_normalization/082_attribute_types_er_model/)(`dalvik.vm.dex2oat-threads`)으로 조절하여, 백그라운드 컴파일 시 포그라운드(Foreground) 사용자 경험이 저하되지 않도록 [스케줄러](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/079_kube_scheduler_pod_placement/) 우선순위를 조정([cgroups](/knowledge-base/studynote/02_operating_system/01_overview_architecture/062_cgroups/) 및 cpuset 활용)해야 한다.
...
- **배포적 관점**: 앱 번들에 [Baseline](/knowledge-base/studynote/04_software_engineering/01_overview_principles/025_baseline/) Profile/Cloud Profile이 올바르게 통합되어 배포되고 있는가? (Macrobenchmark 라이브러리로 [검증](/knowledge-base/studynote/04_software_engineering/07_object_oriented/395_verification_process_review/))
- **운영적 관점**: OS 업데이트 후 `dex2oat` 프로세스가 CPU 자원을 점유하는 현상이 백그라운드 [정책](/knowledge-base/studynote/10_ai/02_dl_architecture_new/164_policy/)(Doze Mode) 내에서 적절히 통제되고 있는가?

- **📢 섹션 요약 비유**: 도로(앱)를 달리는 자동차(실행기)를 개조하는 것에 그치지 않고, 스마트 내비게이션([Baseline](/knowledge-base/studynote/04_software_engineering/01_overview_principles/025_baseline/) Profile)을 제공하여 차가 스스로 덜 막히는 고속도로(AOT)를 빨리 타게 만드는 운영 묘미입니다.
...
| 구분 | 최적화 전 (순수 [JIT](/knowledge-base/studynote/09_security/11_iam_access_control/568_jit_access/)/AOT) | 최적화 후 (혼합 ART + 클라우드 프로필) | 개선 효과 |
|:---|:---|:---|:---|
| **정량** | 설치 대기 5분 ([초기](/knowledge-base/studynote/03_network/08_transport_layer/459_quic_fec_forward_error_correction/) ART) | 설치 대기 **10초** 내외 | 앱 설치 시간 **90% 이상 단축** |
| **정량** | RAM 사용량 높음 (Dalvik [JIT](/knowledge-base/studynote/09_security/11_iam_access_control/568_jit_access/)) | AOT 코드 공유 매핑 (.[oat](/knowledge-base/studynote/04_software_engineering/10_trends_pm_quality/707_oat_operational_acceptance_testing/)) | 시스템 메모리(RAM) 효율성 **20% 상승** |
...
- **📢 섹션 요약 비유**: 각 사람의 생활 리듬에 맞춰 스스로 다이어트(저장공간 관리)와 근육 운동(AOT [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) 최적화) 비율을 조절하는 스마트 피트니스 트레이너 엔진으로 발전하고 있습니다.
...
| 안드로이드 리눅스 [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/) 커스터마이징 (Wakelock 전력 통제 [모듈](/knowledge-base/studynote/04_software_engineering/04_testing_quality/192_module_independence/)) | 현재 개념이 등장하게 만든 직접적인 선행 흐름이다. |
| iOS XNU [하이브리드 커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/025_hybrid_kernel/) 및 샌드박스 앱 관리 모형 | 현재 개념이 구현·세분화될 때 바로 연결되는 후속 개념이다. |
...
1. 안드로이드 스마트폰이 앱을 실행할 때는 외국어로 된 책을 번역해야 해요. 옛날에는 매번 책을 읽을 때마다 힘들게 통역([JIT](/knowledge-base/studynote/09_security/11_iam_access_control/568_jit_access/))을 했어요.
...
← **이전**: [620. 안드로이드 리눅스 커널 커스터마이징 (Wakelock 전력 통제 모듈)](/knowledge-base/studynote/02_operating_system/10_security/620_android_runtime_art_wakelock/)
**다음**: [622. iOS XNU 하이브리드 커널 및 샌드박스 앱 관리 모형 (Ios Xnu Hybrid Kernel App Sandbox)](/knowledge-base/studynote/02_operating_system/10_security/622_ios_xnu_hybrid_kernel_app_sandbox/) →
