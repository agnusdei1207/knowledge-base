...
> 1. **본질**: Virtio는 가상머신(Guest OS)이 물리적 하드웨어([NIC](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/587_nic_offloading/), 디스크)를 직접 제어하는 척 속이는 [전가상화](/knowledge-base/studynote/02_operating_system/01_overview_architecture/057_full_virtualization/)([Full Virtualization](/knowledge-base/studynote/02_operating_system/01_overview_architecture/057_full_virtualization/))의 끔찍한 오버헤드를 버리고, 자신이 가상머신임을 인지한 상태에서 [하이퍼바이저](/knowledge-base/studynote/02_operating_system/01_overview_architecture/054_hypervisor/)와 효율적으로 협력 통신하는 **[반가상화](/knowledge-base/studynote/02_operating_system/01_overview_architecture/058_paravirtualization/)([Paravirtualization](/knowledge-base/studynote/02_operating_system/01_overview_architecture/058_paravirtualization/)) I/O의 사실상 표준(Defacto) 프레임워크**다.
> 2. **메커니즘**: Guest OS 내부의 **프론트엔드(Frontend) 드라이버**와 Host OS의 **백엔드(Backend) 디바이스**가 메모리를 공유하고, 그 사이에 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 원형 큐 형태로 주고받는 **Virtqueue (Vring)**를 두어 패킷 복사와 [VM](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/598_vm_migration_nic/) Exit([문맥 교환](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/211_context_switch/)) 횟수를 극적으로 줄인다.
...
- **필요성 ([전가상화](/knowledge-base/studynote/02_operating_system/01_overview_architecture/057_full_virtualization/)의 [트랩](/knowledge-base/studynote/02_operating_system/11_exam_summary/677_trap_based_system_call_implementation/) 앤 에뮬레이션 지옥)**: 
  ...
  - **[전가상화](/knowledge-base/studynote/02_operating_system/01_overview_architecture/057_full_virtualization/) (구형)**: 외국인 손님([VM](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/598_vm_migration_nic/))이 식당(Host)에 와서 한국어 메뉴판을 보고 손짓 발짓으로 주문한다. 종업원([하이퍼바이저](/knowledge-base/studynote/02_operating_system/01_overview_architecture/054_hypervisor/))은 손님의 엉터리 한국어를 계속 번역하고([VM](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/598_vm_migration_nic/) Exit), 가짜 주방장(QEMU)에게 전달해서 다시 요리를 내온다. 소통이 너무 피곤하다.
  - **[반가상화](/knowledge-base/studynote/02_operating_system/01_overview_architecture/058_paravirtualization/) (Virtio)**: 식당이 아예 외국인 전용 태블릿 주문기(Virtio)를 테이블에 놨다. 손님은 자기 언어로 태블릿에 주문(Vring에 패킷 넣기)하고 '전송' 버튼 한 번만 누르면(Kick), 주방(Host [Kernel](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/))으로 즉시 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)가 날아가 요리가 1초 만에 나온다.

- **발전 과정**:
  1. **[전가상화](/knowledge-base/studynote/02_operating_system/01_overview_architecture/057_full_virtualization/) (Full Virt)**: 하드웨어 에뮬레이션으로 극악의 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/). (예: e1000 랜카드)
  2. **Virtio 도입 (Linux 2.6.24+)**: Rusty Russell이 제안. 리눅스 메인라인에 흡수되어 KVM의 기본 I/O로 정착.
  3. **vhost-net ([Kernel](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/) Backend)**: QEMU(유저 스페이스)를 거치는 오버헤드마저 없애기 위해, 백엔드 엔진을 Host [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/) 공간으로 내려버려 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) 극대화.
  4. **vhost-user ([DPDK](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/671_dpdk/))**: [OVS](/knowledge-base/studynote/03_network/17_sdn_nfv/860_ovs_open_vswitch_sdn_openflow/)-DPDK와 결합하여 Host [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/)마저 우회하는 유저 스페이스 고속 통신으로 진화.
...
**[다이어그램 해설]** 옛날에는 패킷 10개를 보낼 때, [레지스터](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/057_register/)에 값을 쓸 때마다 [VM](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/598_vm_migration_nic/) Exit가 발생해 총 수십 번의 멈춤이 있었다. Virtio는 **[배치 처리](/knowledge-base/studynote/13_cloud_architecture/05_data_engineering/228_batch_processing_hadoop_spark/)([Batching](/knowledge-base/studynote/05_database/06_dw_olap_trends/389_bulk_insert_batching_optimization/))**를 한다. 10개의 패킷 주소를 메모리(Vring)에 조용히 다 적을 때까지는 Host를 부르지 않는다. 다 적은 뒤 딱 1번만 `Kick`을 날려 Host를 부른다([VM](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/598_vm_migration_nic/) Exit 1회). 그러면 Host는 [공유 메모리](/knowledge-base/studynote/02_operating_system/02_process_thread/118_shared_memory/)에 있는 주소를 보고 패킷을 꺼내간다. [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 복사(Memory Copy)도 발생하지 않으며, 알림 횟수도 획기적으로 줄어들어 엄청난 속도 향상을 이룬다.
...
| **[VM](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/598_vm_migration_nic/) Exit 횟수** | 매 I/O [레지스터](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/057_register/) 접근마다 발생 (수백 번) | **배치 단위로 1번 (Kick/[Interrupt](/knowledge-base/studynote/02_operating_system/01_overview_architecture/016_interrupt_mechanism/))** | 없음 (0번) |
| **CPU 부하** | 매우 높음 | 중간 (Host CPU가 연산 대행) | **거의 없음 (디바이스가 직접 함)** |
| **마이그레이션** | 제약 없음 (가장 유연함) | **제약 없음 (클라우드 표준)** | 원칙적 불가 (하드웨어 종속) |
| **최대 [대역폭](/knowledge-base/studynote/01_computer_architecture/03_architecture_basics_performance/140_bandwidth/)** | 1Gbps 미만 | **10Gbps ~ 40Gbps 수준** | 100Gbps 이상 (Line-rate) |

### 과목 융합 관점

- **자료구조 ([Data](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) Structure)**: Vring은 생산자-소비자(Producer-Consumer) 패턴의 교과서적인 링 버퍼(Ring Buffer) 구조다. 락([Lock](/knowledge-base/studynote/05_database/04_transactions_concurrency/510_lock/)) 없이 동시성을 보장하기 위해 [메모리 배리어](/knowledge-base/studynote/01_computer_architecture/11_multicore_synchronization/416_memory_barrier/)([Memory Barrier](/knowledge-base/studynote/01_computer_architecture/11_multicore_synchronization/416_memory_barrier/))를 사용하며, Head와 Tail 인덱스만 조작하여 [Lock](/knowledge-base/studynote/05_database/04_transactions_concurrency/510_lock/)-free하게 I/O를 전달한다.
- **[클라우드 컴퓨팅](/knowledge-base/studynote/02_operating_system/01_overview_architecture/052_cloud_computing_os/) (Cloud)**: AWS EC2 인스턴스의 디스크(EBS)와 네트워크(ENA) 드라이버는 모두 이 Virtio 아키텍처(또는 이를 하드웨어로 구운 Nitro 아키텍처)의 변형이다. 클라우드 벤더들이 깡통 하드웨어를 VM으로 쪼개 팔면서도 디스크 I/O IOPS를 보장할 수 있는 유일한 원동력이다.
...
1. **시나리오 — Windows [VM](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/598_vm_migration_nic/)(가상머신) 클라우드 이전 후 디스크 인식 불가**: VMware 기반 사내 인프라에서 잘 돌던 Windows Server 2019 VM을 OpenStack([KVM](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/713_kvm_over_ip/))으로 마이그레이션(V2V) 했더니, 부팅 시 블루스크린(INACCESSIBLE_BOOT_DEVICE)이 뜨며 디스크를 찾지 못함.
...
|:---|:---|:---|:---|
| **정량 (네트워크 [대역폭](/knowledge-base/studynote/01_computer_architecture/03_architecture_basics_performance/140_bandwidth/))**| 약 1 Gbps 한계 | **[10](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/489_raid_10_hybrid/) Gbps ~ 40 Gbps 달성** | 네트워크 [처리량](/knowledge-base/studynote/01_computer_architecture/03_architecture_basics_performance/139_throughput/)([Throughput](/knowledge-base/studynote/01_computer_architecture/03_architecture_basics_performance/139_throughput/)) 10배 이상 향상 |
| **정량 ([VM](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/598_vm_migration_nic/) Exit 횟수)** | 1 패킷당 다수 발생 | 다중 패킷 묶음(Batch)당 1회 발생 | 호스트 CPU 오버헤드 급감 및 응답성 개선 |
