...
> 1. **본질**: RTOS (Real-Time [Operating System](/knowledge-base/studynote/02_operating_system/01_overview_architecture/001_operating_system_purpose/))는 범용 OS(Windows, Linux)처럼 '최대한 빨리' 처리하는 것이 목표가 아니라, 정해진 시간([Deadline](/knowledge-base/studynote/02_operating_system/11_exam_summary/766_realtime_scheduling_deadline/)) 안에 '반드시' 작업을 완료하는 결정론적(Deterministic) 실행을 최우선으로 하는 운영체제다.
> 2. **설계**: 이를 위해 RTOS는 O(1) 복잡도를 갖는 선점형(Preemptive) [스케줄러](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/079_kube_scheduler_pod_placement/), [우선순위 역전](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/205_priority_inversion/)([Priority Inversion](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/205_priority_inversion/)) 방지 메커니즘(우선순위 [상속](/knowledge-base/studynote/04_software_engineering/04_testing_quality/234_uml_class_relationships_generalization_dependency/) 등), 그리고 [인터럽트](/knowledge-base/studynote/02_operating_system/01_overview_architecture/016_interrupt_mechanism/) [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/)([Interrupt Latency](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/545_interrupt_latency/))을 극단적으로 최소화한 아키텍처를 채택한다.
> 3. **가치**: VxWorks, FreeRTOS, QNX와 같은 RTOS는 항공우주, 의료기기, 자동차 자율주행 등 단 1밀리초의 [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/)이 치명적인 사고로 직결되는 Hard Real-Time 환경에서 시스템의 생존을 보장하는 핵심 기반 기술이다.
...
### [우선순위 역전](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/205_priority_inversion/) ([Priority Inversion](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/205_priority_inversion/))과 [상속](/knowledge-base/studynote/04_software_engineering/04_testing_quality/234_uml_class_relationships_generalization_dependency/) [프로토콜](/knowledge-base/studynote/03_network/06_network_layer_ip/295_protocol_field_tcp_udp_icmp/)
...
- **📢 섹션 요약 비유**: 구급차(T_H)가 좁은 길에서 일반 차(T_L) 뒤에 막혔을 때, 경찰(RTOS)이 일반 차에게 임시로 구급차와 같은 사이렌(우선순위 [상속](/knowledge-base/studynote/04_software_engineering/04_testing_quality/234_uml_class_relationships_generalization_dependency/))을 달아주어 다른 차(T_M)들이 끼어들지 못하게 하고 빨리 길을 빠져나가게 하는 원리입니다.
...
리눅스에서도 [PREEMPT_RT](/knowledge-base/studynote/02_operating_system/10_security/654_preempt_rt_linux_spinlock_mutex/) 패치를 통해 실시간성을 확보하려는 노력이 있으나, 구조적으로 [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/) [스핀락](/knowledge-base/studynote/02_operating_system/04_synchronization/222_spinlock/)을 뮤텍스로 변경하는 등 한계가 존재하여(Soft Real-Time) Hard Real-Time 요구사항에는 여전히 전통적인 [마이크로커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/024_microkernel/) 기반 RTOS가 선호된다.
...
- **아키텍처 관점**: 주기가 짧은 [태스크](/knowledge-base/studynote/02_operating_system/02_process_thread/150_task/)에 높은 우선순위를 할당하는 [RM](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/197_rm_rate_monotonic_scheduling/)([Rate Monotonic](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/197_rm_rate_monotonic_scheduling/)) 원칙을 준수하여 [태스크](/knowledge-base/studynote/02_operating_system/02_process_thread/150_task/) 우선순위를 매핑했는가?
- **안정성 관점**: 모든 [태스크](/knowledge-base/studynote/02_operating_system/02_process_thread/150_task/)가 무한 루프(`while(1)`) 내에서 반드시 `vTaskDelay()` 혹은 이벤트를 대기(Block)하는 구문을 가져 하위 [태스크](/knowledge-base/studynote/02_operating_system/02_process_thread/150_task/)가 기아([Starvation](/knowledge-base/studynote/02_operating_system/05_deadlock/314_starvation_prevention/)) 상태에 빠지지 않도록 설계했는가?

- **📢 섹션 요약 비유**: 수술실(RTOS)에서는 간호사([ISR](/knowledge-base/studynote/02_operating_system/01_overview_architecture/020_isr/))가 피 검사 결과를 분석(연산)하지 않고 결과지만 의사([Task](/knowledge-base/studynote/02_operating_system/02_process_thread/150_task/))에게 전달만 해야 의사가 즉시 판단을 내릴 수 있는 것과 같은 역할 분담의 법칙입니다.
...
|:---|:---|:---|:---|
| **정량** | [인터럽트](/knowledge-base/studynote/02_operating_system/01_overview_architecture/016_interrupt_mechanism/) 응답 지터: 5~50ms | 응답 지터: **최대 10µs 보장** | [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/) 변동성(Jitter) 99% [억제](/knowledge-base/studynote/09_security/13_secops_ir_forensics/656_ir_containment/) |
| **정량** | [컨텍스트](/knowledge-base/studynote/02_operating_system/01_overview_architecture/033_context/) [스위치](/knowledge-base/studynote/03_network/05_lan_wan_l2_devices/238_switch_operation_principles/): 수백 µs | [컨텍스트](/knowledge-base/studynote/02_operating_system/01_overview_architecture/033_context/) [스위치](/knowledge-base/studynote/03_network/05_lan_wan_l2_devices/238_switch_operation_principles/): **수십 CPU 클럭** | 마이크로초 단위의 [초고속](/knowledge-base/studynote/06_ict_convergence/02_iot_mobility/148_5g_embb_urllc_mmtc/) [태스크](/knowledge-base/studynote/02_operating_system/02_process_thread/150_task/) 전환 |
...
- **📢 섹션 요약 비유**: 아무리 복잡한 교차로라도 응급차가 오면 완벽하게 길을 열어주는 [신호](/knowledge-base/studynote/02_operating_system/02_process_thread/130_signal/) 체계처럼, RTOS는 자율주행과 로봇 시대의 가장 안전한 도로 통제 시스템입니다.
...
| 개념 | 연결 포인트 |
|:---|:---|
| [ART](/knowledge-base/studynote/02_operating_system/10_security/621_art_android_runtime/) ([Android Runtime](/knowledge-base/studynote/02_operating_system/10_security/621_art_android_runtime/)) AOT/[JIT](/knowledge-base/studynote/09_security/11_iam_access_control/568_jit_access/) 컴파일러 혼합 실행 환경 | 현재 개념으로 들어오기 전에 함께 이해하면 경계가 선명해지는 기반 개념이다. |
| iOS XNU [하이브리드 커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/025_hybrid_kernel/) 및 샌드박스 앱 관리 모형 | 현재 개념이 등장하게 만든 직접적인 선행 흐름이다. |
| [마이크로커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/024_microkernel/) [IPC](/knowledge-base/studynote/02_operating_system/02_process_thread/117_ipc/) 메시지 패싱 [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/) 단축 기법 구조 설계 | 현재 개념이 구현·세분화될 때 바로 연결되는 후속 개념이다. |
| [하이퍼바이저 링 레벨](/knowledge-base/studynote/02_operating_system/10_security/625_hypervisor_ring_level_vmx/) (Ring -1 모드 VMX Root/Non-Root 모드) | 확장 학습이나 심화 비교로 이어지는 다음 단계의 키워드다. |
