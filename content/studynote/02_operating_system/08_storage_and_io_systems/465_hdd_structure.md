...
- **개념**: HDD는 금속 재질의 둥근 원판 여러 장이 레코드판처럼 겹쳐 돌아가는 저장장치다. 
  ...
  - **헤드(Head)**: 플래터 위를 나노미터(nm) 단위로 떠서 날아다니며 자성을 감지하는 읽기/[쓰기](/knowledge-base/studynote/13_cloud_architecture/05_data_engineering/289_cqrs_db/) 바늘.
- **필요성**: 컴퓨터가 부팅될 때, 그 텅 빈 RAM에 [운영체제](/knowledge-base/studynote/02_operating_system/01_overview_architecture/001_operating_system_purpose/)(OS)를 채워 넣고 게임 코드를 띄우려면 전기가 없어도 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 기억해 줄 거대한 '영구 보관 창고'가 절대적으로 필요했다. 자기 테이프(Tape)는 순차적으로만 읽을 수 있어 원하는 [파일](/knowledge-base/studynote/02_operating_system/09_file_system/501_file_definition_logical_record/) 중간으로 건너뛸(Random Access) 수가 없었다. "턴테이블 위에서 바늘을 들고 원하는 노래 위치에 탁! 떨어뜨릴 수 있는 기계를 만들자!" 이 영감이 LBA([논리](/knowledge-base/studynote/09_security/04_endpoint_security/369_logic_bomb/) 블록 주소) 기반의 [디렉터리](/knowledge-base/studynote/02_operating_system/09_file_system/506_directory_structure_symbol_table/)/[파일](/knowledge-base/studynote/02_operating_system/09_file_system/501_file_definition_logical_record/) 시스템을 창조해 낸 위대한 랜덤 액세스 기계(HDD)를 발명하게 했다.

- **등장 배경 및 저장 장치의 왕좌**:
  1. **천공 카드와 테이프의 암흑기**: 중간 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 찾으려면 1시간 동안 테이프를 되감아야 했다.
...
### 비교 1: 기계적 구시대(HDD) vs 전자적 신시대([SSD](/knowledge-base/studynote/01_computer_architecture/08_io_storage_systems/327_ssd/)) 구조 철학
...
### CHS 주소 체계에서 LBA (Logical Block Address) 로의 [추상화](/knowledge-base/studynote/04_software_engineering/04_testing_quality/198_abstraction_control_data_process/)

초창기 멍청한 OS([DOS](/knowledge-base/studynote/02_operating_system/10_security/599_dos_ddos_attack/) 시절)는 하드디스크에 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 쓸 때 `Cylinder 10, Head 2, Sector 5` 처럼 물리적 3차원 좌표(CHS)를 직접 계산해서 바늘을 조종했다.
...
3. **극한의 가성비 결단**:
   - 용량당 가격이 SSD보다 1/[10](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/489_raid_10_hybrid/) 수준으로 싼데, 우리가 쓰는 용도(순차 빅데이터 스캔)에서는 SSD와 체감 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) 차이가 없다면? 기업 인프라 엔지니어는 1초의 망설임도 없이 HDD로 스토리지 랙(Rack)을 도배하여 수백억 원의 인프라 비용을 절감한다.
...
하드 디스크 드라이브 (HDD)는 0과 1이라는 차가운 소프트웨어의 [논리](/knowledge-base/studynote/09_security/04_endpoint_security/369_logic_bomb/)가 현실의 무겁고 시끄러운 모터 쇳덩어리 위에서 아슬아슬하게 외줄 타기를 하며 생존해 온 컴퓨터 공학의 눈물겨운 역사다. 속도 차이를 메우기 위한 [운영체제](/knowledge-base/studynote/02_operating_system/01_overview_architecture/001_operating_system_purpose/)의 모든 I/O 스케줄링(엘리베이터)과 [버퍼 캐시](/knowledge-base/studynote/02_operating_system/09_file_system/536_buffer_cache_page_cache/)([Page](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/) Cache) 같은 우아한 소프트웨어 꼼수들은 모두 이 둔해 빠진 기계식 바늘의 동선을 1mm라도 아껴보려는 절박함에서 탄생했다. 비록 메인 OS 부팅 드라이브와 모바일/[PC](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/164_pc/) 시장에서는 속도에 미친 [플래시 메모리](/knowledge-base/studynote/01_computer_architecture/06_memory_hierarchy_cache/256_flash_memory/)([SSD](/knowledge-base/studynote/01_computer_architecture/08_io_storage_systems/327_ssd/))에게 완벽히 쫓겨나 왕좌를 내어주었지만, 수십 제타바이트(ZB)의 인간 지식을 반영구적으로 묻어둬야 하는 클라우드 아카이브(Cold Storage)와 빅데이터의 거대한 바다 밑바닥에서는 자성(Magnetism)의 밀도를 극한으로 쥐어짜는 헬륨 하드(Helium Drive)와 열보조 기록(HAMR) 기술로 진화하며 영원히 죽지 않는 불멸의 거인으로 남을 것이다.

- **📢 섹션 요약 비유**: 날렵한 전기차([SSD](/knowledge-base/studynote/01_computer_architecture/08_io_storage_systems/327_ssd/))가 도로([PC](/knowledge-base/studynote/01_computer_architecture/04_instruction_set_architecture/164_pc/))를 다 장악했지만, 전기차 100대를 한 번에 싣고 바다를 건너 대륙으로 수출할 때는 여전히 시끄럽고 무거운 초거대 디젤 화물선(HDD)이 압도적인 비용 효율로 바다([데이터센터](/knowledge-base/studynote/03_network/16_data_center_cloud/801_data_center_3_tier_architecture_core_aggregation_access/))를 지배하고 있는 것과 같습니다. 쓰임새가 완전히 양극화된 셈입니다.
...
| [io_uring](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/464_io_uring/) | 현재 개념이 등장하게 만든 직접적인 선행 흐름이다. |
| [논리적 블록 주소](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/466_logical_block_address_lba/) (LBA, Logical Block Address) | 현재 개념이 구현·세분화될 때 바로 연결되는 후속 개념이다. |
| 디스크 접근 시간 = [탐색 시간](/knowledge-base/studynote/01_computer_architecture/08_io_storage_systems/324_seek_time/)([Seek Time](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/467_disk_access_time/)) + [회전 지연](/knowledge-base/studynote/01_computer_architecture/08_io_storage_systems/325_rotational_latency/)([Rotational Latency](/knowledge-base/studynote/01_computer_architecture/08_io_storage_systems/325_rotational_latency/)) + [전송 시간](/knowledge-base/studynote/01_computer_architecture/08_io_storage_systems/326_transfer_time/)([Transfer Time](/knowledge-base/studynote/01_computer_architecture/08_io_storage_systems/326_transfer_time/)) | 확장 학습이나 심화 비교로 이어지는 다음 단계의 키워드다. |
...
1. 하드 디스크 드라이브 (HDD) 구조은 컴퓨터가 디스크와 장치가 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 주고받는 길을 정리하는 방법이에요.
2. 먼저 io_uring을 이해하면 하드 디스크 드라이브 (HDD) 구조이 왜 필요한지 더 쉽게 보여요.
3. 그래서 하드 디스크 드라이브 (HDD) 구조을 잘 알면 나중에 [논리적 블록 주소](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/466_logical_block_address_lba/) (LBA, Logical Block Address)도 훨씬 쉽게 배울 수 있어요.
