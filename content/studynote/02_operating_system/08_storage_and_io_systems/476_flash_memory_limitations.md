...
- **필요성**: 만약 당신이 텍스트 [파일](/knowledge-base/studynote/02_operating_system/09_file_system/501_file_definition_logical_record/)의 글자 1개를 'a'에서 'b'로 바꿨다고 치자. HDD였다면 바늘이 그 자리에 가서 자석의 극성(N/S)만 슬쩍 바꾸면 1초 만에 끝난다(In-place update). 하지만 낸드 플래시에서 똑같이 하려 들면? 글자 1개가 포함된 4KB [페이지](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/)를 고치기 위해, 그 [페이지](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/)가 속해있는 2MB짜리 블록 1개 전체를 날려버려야 한다. 블록 안에 멀쩡히 살아있던 다른 [파일](/knowledge-base/studynote/02_operating_system/09_file_system/501_file_definition_logical_record/) [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 1.99MB까지 다 지워져 버린다! 이 미친 비효율([Write Amplification](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/480_write_amplification/))과 수명 단축을 어떻게든 우회할 소프트웨어적 꼼수(Out-of-place update)가 절대적으로 필요했다.

- **등장 배경 및 구조적 딜레마**:
  1. **트랜지스터의 구조적 한계**: Floating Gate 트랜지스터에서 전자를 뽑아내는 [터널링](/knowledge-base/studynote/03_network/07_network_layer_routing/377_tunneling_mechanism_overview/)([Tunneling](/knowledge-base/studynote/03_network/07_network_layer_routing/377_tunneling_mechanism_overview/)) 현상이 물리적으로 블록 단위로만 제어 가능하게 설계됨.
...
[플래시 메모리](/knowledge-base/studynote/01_computer_architecture/06_memory_hierarchy_cache/256_flash_memory/)의 3대 기본 연산은 속도와 단위가 극단적으로 불균형하다.
...
왜 지울 때마다 수명이 깎일까?
- 낸드 플래시의 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)는 전자를 '플로팅 게이트(Floating Gate)'라는 감옥에 가둬두는 방식으로 유지된다. 이 감옥의 벽은 실리콘 산화막(Oxide Layer)으로 되어 있다.
- Write를 할 때 [터널링](/knowledge-base/studynote/03_network/07_network_layer_routing/377_tunneling_mechanism_overview/)([Tunneling](/knowledge-base/studynote/03_network/07_network_layer_routing/377_tunneling_mechanism_overview/))이라는 양자역학적 효과로 전자가 벽을 뚫고 들어간다.
- Erase를 할 때 20V의 강력한 역전압을 쏘아 전자를 억지로 벽 밖으로 뜯어낸다.
- 이 벽 뚫기를 1000번, 3000번 반복하면 **감옥 벽(산화막)이 헐어서 구멍이 숭숭 뚫린다.**
- 벽이 헐면 갇혀있던 전자가 슬금슬금 새어 나가버린다([Data](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) [Retention](/knowledge-base/studynote/05_database/04_transactions_concurrency/515_mvcc/) Fail). 0을 저장했는데 몇 달 뒤에 열어보면 전자가 다 도망가서 1로 바뀌어 있는 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 오염이 발생하며 해당 블록은 영구 사망(Bad Block) 판정을 받는다.
...
### 비교 1: [SLC](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/597_slc_caching/) vs MLC vs TLC vs QLC (수명과 속도의 제물)
...
| **I/O 패턴의 진화 (Append-Only)**| 기존의 [B-Tree](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/064_b_tree/)(랜덤 업데이트) 중심의 DB 생태계를, [SSD](/knowledge-base/studynote/01_computer_architecture/08_io_storage_systems/327_ssd/) 수명 방어에 최적화된 LSM Tree(순차 [쓰기](/knowledge-base/studynote/13_cloud_architecture/05_data_engineering/289_cqrs_db/)) 아키텍처로 전면 개편시킴 |
| **수명 대비 용량의 자본주의적 타협**| 수명을 제물로 바쳐 1셀당 4비트(QLC)를 구겨 넣는 기술을 통해 테라바이트 시대의 가성비 대중화를 폭발적으로 이끔 |

### 결론 및 미래 전망

[플래시 메모리](/knowledge-base/studynote/01_computer_architecture/06_memory_hierarchy_cache/256_flash_memory/) 한계 (덮어쓰기 불가, 수명 족쇄)는 가장 완벽해 보였던 차세대 저장 [매체](/knowledge-base/studynote/03_network/03_physical_layer_media/121_transmission_media_guided_unguided/)([SSD](/knowledge-base/studynote/01_computer_architecture/08_io_storage_systems/327_ssd/))의 심장에 박힌 태생적 쐐기이자, 컴퓨터 공학자들이 하드웨어의 저주를 펌웨어의 흑마술([FTL](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/478_ftl_flash_translation_layer/))로 어떻게 속여 넘겼는지를 보여주는 웅장한 사기극이다. 빛의 속도라는 쾌감을 얻기 위해 인류는 'Erase-before-write'라는 쓰레기 같은 연산 단위 불균형을 기꺼이 짊어졌고, 이 불균형이 낳는 파편화를 막기 위해 백그라운드 청소부(GC)와 램 뻥튀기(Over-Provisioning)를 필수적으로 도입했다. 향후 인텔의 옵테인([3D XPoint](/knowledge-base/studynote/01_computer_architecture/14_hardware_security_trends/494_optane_memory/)) 같은 비휘발성 메모리([PCM](/knowledge-base/studynote/03_network/19_frequent_topics_terms/943_pcm_pulse_code_modulation_sampling_quantization/)) 기술이 낸드 플래시의 자리를 뺏는다면 이 지긋지긋한 덮어쓰기 불가(No In-place Update)의 족쇄도 역사 속으로 사라지겠지만, [플래시 메모리](/knowledge-base/studynote/01_computer_architecture/06_memory_hierarchy_cache/256_flash_memory/)가 빚어낸 "하드웨어의 치부를 소프트웨어 맵핑으로 가린다"는 이 기만적 최적화의 철학은 차세대 컴퓨터 아키텍처 설계의 교과서로 영원히 기록될 것이다.
...
| 개념 | 연결 포인트 |
|:---|:---|
| 리눅스 I/O [스케줄러](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/079_kube_scheduler_pod_placement/) | 현재 개념으로 들어오기 전에 함께 이해하면 경계가 선명해지는 기반 개념이다. |
| [솔리드 스테이트 드라이브](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/475_ssd_structure/) ([SSD](/knowledge-base/studynote/01_computer_architecture/08_io_storage_systems/327_ssd/), [Solid State Drive](/knowledge-base/studynote/01_computer_architecture/08_io_storage_systems/327_ssd/)) 구조 | 현재 개념이 등장하게 만든 직접적인 선행 흐름이다. |
| [가비지 컬렉션](/knowledge-base/studynote/02_operating_system/06_memory_management/380_garbage_collection/) ([Garbage Collection](/knowledge-base/studynote/02_operating_system/06_memory_management/380_garbage_collection/) in [SSD](/knowledge-base/studynote/01_computer_architecture/08_io_storage_systems/327_ssd/)) | 현재 개념이 구현·세분화될 때 바로 연결되는 후속 개념이다. |
| [FTL](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/478_ftl_flash_translation_layer/) ([Flash Translation Layer](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/478_ftl_flash_translation_layer/)) | 확장 학습이나 심화 비교로 이어지는 다음 단계의 키워드다. |
...
1. [플래시 메모리](/knowledge-base/studynote/01_computer_architecture/06_memory_hierarchy_cache/256_flash_memory/) 한계은 컴퓨터가 디스크와 장치가 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 주고받는 길을 정리하는 방법이에요.
2. 먼저 [솔리드 스테이트 드라이브](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/475_ssd_structure/) ([SSD](/knowledge-base/studynote/01_computer_architecture/08_io_storage_systems/327_ssd/), [Solid State Drive](/knowledge-base/studynote/01_computer_architecture/08_io_storage_systems/327_ssd/)) 구조을 이해하면 [플래시 메모리](/knowledge-base/studynote/01_computer_architecture/06_memory_hierarchy_cache/256_flash_memory/) 한계이 왜 필요한지 더 쉽게 보여요.
3. 그래서 [플래시 메모리](/knowledge-base/studynote/01_computer_architecture/06_memory_hierarchy_cache/256_flash_memory/) 한계을 잘 알면 나중에 [가비지 컬렉션](/knowledge-base/studynote/02_operating_system/06_memory_management/380_garbage_collection/) ([Garbage Collection](/knowledge-base/studynote/02_operating_system/06_memory_management/380_garbage_collection/) in [SSD](/knowledge-base/studynote/01_computer_architecture/08_io_storage_systems/327_ssd/))도 훨씬 쉽게 배울 수 있어요.
...
← **이전**: [475. 솔리드 스테이트 드라이브 (SSD, Solid State Drive) 구조 - NAND 플래시, 페이지(Page), 블록(Block)](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/475_ssd_structure/)
**다음**: [477. 가비지 컬렉션 (Garbage Collection in SSD) - 유효 페이지 복사 후 블록 전체 지우기](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/477_ssd_garbage_collection/) →
