...
- **개념**: 운영체제가 관리하는 수많은 I/O 장치 중, 하드디스크, [SSD](/knowledge-base/studynote/01_computer_architecture/08_io_storage_systems/327_ssd/), [USB](/knowledge-base/studynote/01_computer_architecture/09_system_bus_interconnects/359_usb/) 메모리, 광학 드라이브(CD/DVD)처럼 대용량 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 저장하는 기기들이다. OS는 이 장비들에게 "[파일](/knowledge-base/studynote/02_operating_system/09_file_system/501_file_definition_logical_record/)의 3번째 글자 줘"라고 명령하지 않는다. "디스크의 LBA(Logical Block Addressing) 4005번 블록(4KB) 통째로 줘!"라고 덩어리로 명령한다.
- **필요성**: 만약 하드디스크가 키보드([문자 장치](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/443_character_device/))처럼 1바이트씩 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 토해낸다고 상상해 보자. 1GB짜리 영화를 복사하려면 디스크에 달린 물리적 바늘(Head)을 10억 번 까딱거려야([Interrupt](/knowledge-base/studynote/02_operating_system/01_overview_architecture/016_interrupt_mechanism/)) 한다. 바늘이 움직이는 물리적 시간(8ms)을 10억 번 곱하면 복사에만 1년이 걸린다. "이 미친 비효율을 막으려면 디스크 바늘이 한 번 움직였을 때 무조건 4096바이트(4KB)를 통째로 푹 퍼서 램(RAM)에 던져줘야 한다!"는 물리적 속도 한계의 극복이 블록 단위 전송이라는 무식하고도 든든한 규격을 강제했다.
...
### 2. I/O [스케줄러](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/079_kube_scheduler_pod_placement/) ([엘리베이터 알고리즘](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/471_scan_elevator_scheduling/))
...
- **📢 섹션 요약 비유**: 택배 기사(디스크 바늘)가 콜이 들어오는 순서대로 서울->부산->서울->부산으로 널뛰기 배달을 하면 하루에 4건밖에 배송을 못 합니다. 택배 회사 시스템(I/O [스케줄러](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/079_kube_scheduler_pod_placement/))이 콜을 모아서 "오늘은 경부고속도로 타고 내려가면서 대전, 대구, 부산 순서로 한 방에 쫙 뿌리고 퇴근해라!"라고 엘리베이터식 동선을 짜주면 하루에 100건도 거뜬히 배달할 수 있습니다.

---

## Ⅲ. 비교 및 연결

### 비교 1: [HDD](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/465_hdd_structure/) (하드 디스크) vs [SSD](/knowledge-base/studynote/01_computer_architecture/08_io_storage_systems/327_ssd/) ([솔리드 스테이트 드라이브](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/475_ssd_structure/))
...
블록 장치 (Block Device)는 "느려 터진 회전 모터와 바늘([HDD](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/465_hdd_structure/))"이라는 컴퓨터 공학 최악의 물리적 약점을, "블록(Block)이라는 고정된 덩어리"와 "LBA 주소 체계"라는 [논리](/knowledge-base/studynote/09_security/04_endpoint_security/369_logic_bomb/)적 껍데기로 완벽하게 덮어버린 위대한 [추상화](/knowledge-base/studynote/04_software_engineering/04_testing_quality/198_abstraction_control_data_process/)의 상징이다. 1바이트의 융통성을 버린 대가로 4KB의 거대한 화물 운송 효율을 쟁취한 이 블록 인터페이스는, 훗날 램의 [페이징](/knowledge-base/studynote/02_operating_system/04_synchronization/259_paging/)(4KB) 시스템과 완벽한 짝짜꿍을 이루며 현대 운영체제의 [VFS](/knowledge-base/studynote/02_operating_system/09_file_system/517_virtual_file_system_vfs/)(가상 [파일](/knowledge-base/studynote/02_operating_system/09_file_system/501_file_definition_logical_record/) 시스템)를 떠받치는 척추가 되었다. 물리적 모터([HDD](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/465_hdd_structure/))가 사라지고 [트랜지스터](/knowledge-base/studynote/01_computer_architecture/01_basic_electronics_logic/014_transistor/)([SSD](/knowledge-base/studynote/01_computer_architecture/08_io_storage_systems/327_ssd/))가 그 자리를 차지한 오늘날에도, 운영체제는 여전히 하드웨어를 "4KB짜리 레고 블록들이 늘어선 1차원 [배열](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/055_array/)"로 취급하며 평화롭게 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 퍼 나르고 있다. 미래에 [바이트](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/074_byte/) 단위 직접 접근(Byte-addressable)이 가능한 [CXL](/knowledge-base/studynote/01_computer_architecture/12_accelerators_ai_hardware/441_cxl/) 기반 비휘발성 램(PMEM)이 디스크를 멸종시키는 날이 온다면 이 블록이라는 단위도 해체되겠지만, 대용량 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 다루는 '일괄 처리([Batching](/knowledge-base/studynote/05_database/06_dw_olap_trends/389_bulk_insert_batching_optimization/))'의 위대한 철학만큼은 [데이터베이스](/knowledge-base/studynote/05_database/01_db_architecture_relational/002_database_definition/) 엔진의 깊은 곳에 영원히 살아 숨 쉴 것이다.

- **📢 섹션 요약 비유**: 물([데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/))을 숟가락(1바이트 [문자 장치](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/443_character_device/))으로 퍼 나르면 정밀하지만 하루가 다 갑니다. 규격화된 양동이(4KB 블록)로 퍼 나르면 물이 튀고 양 조절은 안 되지만 하루 만에 수영장(테라바이트)을 꽉 채울 수 있습니다. 블록 장치는 디스크라는 거대한 저수지에서 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 가장 빠르고 안전하게 퍼 나르기 위해 인류가 합의한 '표준 양동이 규격'입니다.
...
| 개념 | 연결 포인트 |
|:---|:---|
| [eBPF](/knowledge-base/studynote/02_operating_system/10_security/615_ebpf/) 기반 메모리 할당 트레이싱 | 현재 개념으로 들어오기 전에 함께 이해하면 경계가 선명해지는 기반 개념이다. |
| I/O 장치의 [분류](/knowledge-base/studynote/16_bigdata/05_analysis/104_classification_analysis/) | 현재 개념이 등장하게 만든 직접적인 선행 흐름이다. |
| [문자 장치](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/443_character_device/) | 현재 개념이 구현·세분화될 때 바로 연결되는 후속 개념이다. |
| [네트워크 장치](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/444_network_device/) ([소켓 인터페이스](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/444_network_device/)) | 확장 학습이나 심화 비교로 이어지는 다음 단계의 키워드다. |
...
1. 블록 장치 (Block Device)은 컴퓨터가 디스크와 장치가 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 주고받는 길을 정리하는 방법이에요.
2. 먼저 I/O 장치의 [분류](/knowledge-base/studynote/16_bigdata/05_analysis/104_classification_analysis/)을 이해하면 블록 장치 (Block Device)이 왜 필요한지 더 쉽게 보여요.
3. 그래서 블록 장치 (Block Device)을 잘 알면 나중에 [문자 장치](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/443_character_device/)도 훨씬 쉽게 배울 수 있어요.
...
<- **이전**: [441. I/O 장치의 분류 - 블록 장치 (Block Device) vs 문자 장치 (Character Device)](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/441_io_device_classification/)
**다음**: [443. 문자 장치 (Character Device)](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/443_character_device/) ->
