...
> 1. **본질**: 좀비 프로세스 (Zombie [Process](/knowledge-base/studynote/12_it_management/05_security_compliance/300_process/))는 자식 프로세스가 실행을 종료했으나, [부모 프로세스](/knowledge-base/studynote/02_operating_system/02_process_thread/105_parent_child_process/)가 `wait()` 또는 `waitpid()` 시스템 콜을 호출하여 종료 상태(Exit Status)를 회수하지 않아 프로세스 테이블 ([Process](/knowledge-base/studynote/12_it_management/05_security_compliance/300_process/) Table)에 항목이 남아 있는 상태를 말한다.
> 2. **가치**: 좀비 프로세스 자체는 CPU (Central Processing Unit) 시간이나 메모리를 거의 소모하지 않으나, 프로세스 테이블 엔트리(PID, 종료 코드, 자원 통계 등)를 점유하므로 시스템의 최대 PID ([Process](/knowledge-base/studynote/12_it_management/05_security_compliance/300_process/) ID) 또는 최대 프로세스 수 제한에 도달할 때까지 누적되면 새로운 [프로세스 생성](/knowledge-base/studynote/02_operating_system/02_process_thread/104_process_creation/)이 불가능해지는 심각한 자원 고갈 문제를 유발한다.
