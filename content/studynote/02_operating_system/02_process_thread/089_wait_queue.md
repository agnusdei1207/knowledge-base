...
> 1. **본질**: 대기 큐 (Wait [Queue](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/058_queue/))는 프로세스가 I/O 요청이나 락 ([Lock](/knowledge-base/studynote/05_database/04_transactions_concurrency/510_lock/)) 획득 등 특정 이벤트가 완료되기를 기다리며 CPU 실행이 블로킹 (Blocked) 상태로 멈춰 있을 때 머무는 [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/) 내부의 연결 리스트다.
> 2. **가치**: 대기 큐를 통해 운영체제는 이벤트를 기다리는 프로세스를 명시적으로 배제하고 다른 작업에 CPU (Central Processing Unit) 자원을 할당할 수 있어, 시스템 전체의 멀티프로그래밍 효율을 극대화한다.
> 3. **판단 포인트**: 다수의 [스레드](/knowledge-base/studynote/02_operating_system/02_process_thread/092_thread_lwp/)가 동시에 락을 대기할 때 우르르 깨어나는 Thundering Herd 현상을 방지하기 위해 배타적 깨우기 (Exclusive Wakeup)나 비동기 I/O 도입 여부를 아키텍처 관점에서 결정해야 한다.
...
- **📢 섹션 요약 비유**: 대기 큐는 식당에서 진동벨을 받고 자리가 날 때까지 앉아있는 대기석과 같다. 종업원(CPU)은 대기석 손님을 신경 쓰지 않고 다른 손님을 모시다가, 자리가 나면([인터럽트](/knowledge-base/studynote/02_operating_system/01_overview_architecture/016_interrupt_mechanism/)) 그때 대기석 손님을 호출하여 테이블 순환 속도를 높인다.
...
대기 큐의 동작은 프로세스의 특성에 따라 [준비 큐](/knowledge-base/studynote/02_operating_system/02_process_thread/088_ready_queue/)([Ready Queue](/knowledge-base/studynote/02_operating_system/02_process_thread/088_ready_queue/))에서의 체류 시간과 극명한 대비를 이룬다.
...
1. **Thundering Herd 현상 방어**: 특정 [소켓](/knowledge-base/studynote/02_operating_system/02_process_thread/125_socket/) 락이 해제될 때, 대기 큐의 수천 개 [스레드](/knowledge-base/studynote/02_operating_system/02_process_thread/092_thread_lwp/)가 동시에 `wake_up_all()`로 깨어나며 발생하는 [문맥 교환](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/211_context_switch/) 폭풍([Context Switch](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/211_context_switch/) Storm)을 겪지 않았는가? Nginx 같은 고성능 서버는 배타적 깨우기(Exclusive Wakeup)를 통해 한 번에 한 개의 [스레드](/knowledge-base/studynote/02_operating_system/02_process_thread/092_thread_lwp/)만 대기 큐에서 분리시켜 경합을 차단한다.
2. **이벤트 소실 (Lost Wakeup) 방지**: [동시성](/knowledge-base/studynote/15_devops_sre/01_culture_methodology/014_concurrency/) 프로그래밍에서 조건 변수를 사용할 때 검사와 대기 사이의 원자성이 깨지면 수신 측이 대기 큐에서 영원히 깨어나지 못한다. 따라서 대기 큐 진입 전후에 반드시 반복문(while)으로 상태를 재확인하는 [모니터](/knowledge-base/studynote/02_operating_system/04_synchronization/229_monitor/) ([Monitor](/knowledge-base/studynote/02_operating_system/04_synchronization/229_monitor/)) 패턴을 강제해야 한다.

### [안티패턴](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/128_water_scrum_fall_anti_pattern/)
- 네트워크 호출 시 타임아웃을 무한대로 설정하는 것. 백엔드 장애 시 [스레드](/knowledge-base/studynote/02_operating_system/02_process_thread/092_thread_lwp/)가 대기 큐(Uninterruptible Sleep)에 갇혀버리고, 결국 전체 [스레드 풀](/knowledge-base/studynote/02_operating_system/02_process_thread/103_thread_pool/) 고갈되는 연쇄 장애(Cascading Failure)로 이어진다.
...
최근에는 리눅스의 `io_uring` 같은 비동기 I/O 인터페이스가 등장하며, 프로세스가 [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/)의 대기 큐에서 잠드는 오버헤드조차 피하려는 논블로킹(Non-blocking) 모델로 진화하고 있다. 대기 큐는 여전히 OS의 근간이지만 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/) 한계를 극복하기 위해 끊임없이 확장되고 있다.
...
3. 그러다 요리([인터럽트](/knowledge-base/studynote/02_operating_system/01_overview_architecture/016_interrupt_mechanism/))가 완성되어 진동벨이 울리면, 대기석에서 자던 손님이 깨어나 다시 밥을 먹기 위해 줄을 선답니다!
