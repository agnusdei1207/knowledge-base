...
- **📢 섹션 요약 비유**: [MLFQ](/knowledge-base/studynote/02_operating_system/11_exam_summary/691_mlfq_multi_level_feedback_queue/) 파라미터는 엘리베이터 제어판과 같다. 층 수, 문 열림 시간, 우선 운행 규칙을 함께 맞춰야 사람들이 빨리 이동하지, 한 버튼만 조정한다고 빌딩 전체 동선이 좋아지지는 않는다.

---

## Ⅱ. 아키텍처 및 핵심 원리

실무적으로 자주 다루는 [MLFQ](/knowledge-base/studynote/02_operating_system/11_exam_summary/691_mlfq_multi_level_feedback_queue/) 파라미터는 다섯 가지다. 첫째, 큐 개수는 [분류](/knowledge-base/studynote/16_bigdata/05_analysis/104_classification_analysis/) 해상도를 결정한다. 둘째, 각 큐의 스케줄링 알고리즘은 보통 [라운드 로빈](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/178_round_robin_scheduling/) (Round Robin, [RR](/knowledge-base/studynote/03_network/16_data_center_cloud/834_load_balancing_algorithm_round_robin_least_connection/)) 또는 최하위의 선입선출 (First-Come, First-Served, [FCFS](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/173_fcfs_scheduling/))로 정한다. 셋째, 강등 규칙은 "퀀텀을 다 썼는가" 혹은 "누적 allotment를 다 썼는가"를 기준으로 잡는다. 넷째, 승급 규칙은 priority boost나 aging으로 기아를 방지한다. 다섯째, 신규 작업의 진입 위치는 새 프로세스를 얼마나 낙관적으로 볼지 결정한다.

| 파라미터 | 무엇을 정하는가 | 보수적 [설정](/knowledge-base/studynote/15_devops_sre/01_culture_methodology/009_config/) | 공격적 [설정](/knowledge-base/studynote/15_devops_sre/01_culture_methodology/009_config/) |
| :--- | :--- | :--- | :--- |
| 큐 개수 | [분류](/knowledge-base/studynote/16_bigdata/05_analysis/104_classification_analysis/) 단계 수 | 3~4개로 단순화 | 많은 큐로 미세 [분류](/knowledge-base/studynote/16_bigdata/05_analysis/104_classification_analysis/) |
| 큐별 [정책](/knowledge-base/studynote/10_ai/02_dl_architecture_new/164_policy/) | 각 큐의 CPU 배분 방식 | 상위 [RR](/knowledge-base/studynote/03_network/16_data_center_cloud/834_load_balancing_algorithm_round_robin_least_connection/), 하위 긴 [RR](/knowledge-base/studynote/03_network/16_data_center_cloud/834_load_balancing_algorithm_round_robin_least_connection/) | 상위 매우 짧은 [RR](/knowledge-base/studynote/03_network/16_data_center_cloud/834_load_balancing_algorithm_round_robin_least_connection/), 하위 [FCFS](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/173_fcfs_scheduling/) |
...
- **📢 섹션 요약 비유**: [MLFQ](/knowledge-base/studynote/02_operating_system/11_exam_summary/691_mlfq_multi_level_feedback_queue/) 파라미터는 놀이공원 줄 세우기 규칙과 같다. 빨리 끝나는 놀이기구 손님을 앞줄에 두고 오래 타는 손님을 뒤로 보내되, 너무 오래 기다린 손님은 다시 앞으로 불러야 전체 불만이 폭발하지 않는다.

---

## Ⅲ. 비교 및 연결

[MLFQ](/knowledge-base/studynote/02_operating_system/11_exam_summary/691_mlfq_multi_level_feedback_queue/) 파라미터의 진짜 의미는 다른 스케줄링과 비교할 때 더 선명해진다. 큐가 1개이고 모든 퀀텀이 같으면 사실상 RR에 가깝고, 강등과 승급이 없으면 다단계 큐 (Multilevel [Queue](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/058_queue/))로 퇴화한다. 즉 MLFQ는 독립 알고리즘이라기보다 여러 [정책](/knowledge-base/studynote/10_ai/02_dl_architecture_new/164_policy/)을 하나의 틀 안에서 조합하는 메타 구조다.

| 튜닝 목표 | 파라미터 특징 | 장점 | 위험 |
| :--- | :--- | :--- | :--- |
| 대화형 데스크톱 | 큐 적당히 많음, 상위 q 짧음, boost 자주 | 빠른 [응답 시간](/knowledge-base/studynote/01_computer_architecture/03_architecture_basics_performance/138_response_time/) | [문맥 교환](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/211_context_switch/) 증가 |
| 배치 서버 | 큐 적음, 하위 q 길음, boost 드묾 | 높은 [처리량](/knowledge-base/studynote/01_computer_architecture/03_architecture_basics_performance/139_throughput/) | 짧은 작업 체감 저하 |
| 혼합 워크로드 | 상위 [RR](/knowledge-base/studynote/03_network/16_data_center_cloud/834_load_balancing_algorithm_round_robin_least_connection/) + 누적 allotment + 중간 boost | 응답/공정성 절충 | 튜닝 복잡도 상승 |

현대 리눅스의 CFS (Completely Fair Scheduler)가 MLFQ와 다른 길을 택한 이유도 여기에 있다. MLFQ는 직관적이지만 "[매직 넘버](/knowledge-base/studynote/02_operating_system/09_file_system/503_magic_number_file_signature/)"에 의존하기 쉽다. 반면 CFS는 [가상 실행 시간](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/203_virtual_runtime_vruntime/) (virtual runtime) 하나로 상대적 공정성을 잡으려 한다. 그럼에도 [MLFQ](/knowledge-base/studynote/02_operating_system/11_exam_summary/691_mlfq_multi_level_feedback_queue/) 파라미터 철학은 여전히 중요하다. [컨테이너 런타임](/knowledge-base/studynote/02_operating_system/10_security/628_container_runtime_oci/), 스토리지 큐잉, 네트워크 [QoS](/knowledge-base/studynote/03_network/07_network_layer_routing/388_qos_quality_of_service_best_effort_intserv_diffserv/) ([Quality of Service](/knowledge-base/studynote/03_network/07_network_layer_routing/388_qos_quality_of_service_best_effort_intserv_diffserv/)) [정책](/knowledge-base/studynote/10_ai/02_dl_architecture_new/164_policy/)에서도 "여러 큐를 두고 승급/강등 기준을 조절한다"는 사고방식이 계속 반복되기 때문이다.
...
- **📢 섹션 요약 비유**: [MLFQ](/knowledge-base/studynote/02_operating_system/11_exam_summary/691_mlfq_multi_level_feedback_queue/) 파라미터는 같은 반 학생을 반장 줄, 일반 줄, 보충 줄로 나누는 학급 운영 규칙과 같다. 줄만 나누는 것으로 끝나지 않고, 언제 줄을 바꿀지까지 정해야 분위기가 안정된다.
...
3. 승급 주기 또는 [aging](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/182_aging/) 기준이 기아를 방지할 만큼 충분히 존재하는가?
4. 퀀텀 직전 I/O 호출로 우선순위를 악용하는 gaming 방지 규칙이 있는가?
5. 큐 개수 증가에 따른 [스케줄러](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/079_kube_scheduler_pod_placement/) 오버헤드가 [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/) 이득보다 작다고 검증했는가?

### [안티패턴](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/128_water_scrum_fall_anti_pattern/)

- 데스크톱 응답성을 노리면서 모든 큐의 퀀텀을 길게 두는 [설정](/knowledge-base/studynote/15_devops_sre/01_culture_methodology/009_config/)
- 기아를 두려워해 너무 짧은 boost를 걸어 CPU 바운드 작업을 계속 상위로 되돌리는 [설정](/knowledge-base/studynote/15_devops_sre/01_culture_methodology/009_config/)
- 누적 allotment 없이 즉시 I/O만 보는 바람에 gaming을 허용하는 [설정](/knowledge-base/studynote/15_devops_sre/01_culture_methodology/009_config/)

파라미터는 실제 측정으로 검증해야 한다. 평균 [응답 시간](/knowledge-base/studynote/01_computer_architecture/03_architecture_basics_performance/138_response_time/), 95퍼센타일 [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/), [문맥 교환](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/211_context_switch/) 수, CPU 사용률 같은 지표를 함께 보고 튜닝해야지, "예전에 잘 되던 값"을 그대로 옮기면 워크로드가 달라진 순간 품질이 무너진다.

- **📢 섹션 요약 비유**: [MLFQ](/knowledge-base/studynote/02_operating_system/11_exam_summary/691_mlfq_multi_level_feedback_queue/) 튜닝은 급식 배식 속도를 정하는 일과 같다. 가장 배고픈 아이를 먼저 먹이되, 한 아이가 너무 오래 자리를 차지하면 뒤 줄이 화가 나고, 반대로 줄을 너무 자주 섞으면 배식 자체가 느려진다.
...
| 개념 | 연결 포인트 |
| :--- | :--- |
| 타임 퀀텀 (Time [Quantum](/knowledge-base/studynote/02_operating_system/11_exam_summary/690_round_robin_time_quantum/)) | 각 큐의 반응성·[문맥 교환](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/211_context_switch/) 빈도를 좌우하는 핵심 파라미터 |
| [에이징](/knowledge-base/studynote/02_operating_system/07_virtual_memory/411_aging_algorithm/) ([Aging](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/182_aging/)) | 오래 기다린 프로세스의 우선순위를 높여 기아를 줄이는 방식 |
| [라운드 로빈](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/178_round_robin_scheduling/) (Round Robin, [RR](/knowledge-base/studynote/03_network/16_data_center_cloud/834_load_balancing_algorithm_round_robin_least_connection/)) | 상위 큐에서 자주 결합되는 기본 선점 [정책](/knowledge-base/studynote/10_ai/02_dl_architecture_new/164_policy/) |
| [기아 상태](/knowledge-base/studynote/02_operating_system/05_deadlock/314_starvation_prevention/) ([Starvation](/knowledge-base/studynote/02_operating_system/05_deadlock/314_starvation_prevention/)) | 승급 규칙이 약할 때 하위 큐에서 나타나는 대표 실패 모드 |
| CFS (Completely Fair Scheduler) | [매직 넘버](/knowledge-base/studynote/02_operating_system/09_file_system/503_magic_number_file_signature/) 의존성을 줄이려는 현대적 공정성 [스케줄러](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/079_kube_scheduler_pod_placement/) |
