...
> 1. **본질**: 앞선 rwx 권한만으로는 "일반 유저가 자기 비밀번호를 바꾸려면 최고 관리자(Root)만 만질 수 있는 `/etc/shadow` [파일](/knowledge-base/studynote/02_operating_system/09_file_system/501_file_definition_logical_record/)을 수정해야 한다" 는 딜레마(권한 모순 늪!)를 풀 수 없다. 이를 해소하기 위해 만들어진 **"이 프로그램을 실행하는 그 순간만큼은, 이 [파일](/knowledge-base/studynote/02_operating_system/09_file_system/501_file_definition_logical_record/)의 주인의 영혼(UID)으로 빙의(둔갑 스왑)시켜주마!"** 라는 [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/) 단위의 조건부 신분 상승 렌더다.
...
> 3. **한계**: SetUID는 해커들이 서버를 뚫기 위해 노리는 **최상위 1순위 먹잇감([Privilege Escalation](/knowledge-base/studynote/09_security/04_endpoint_security/356_privilege_escalation/) [권한 상승](/knowledge-base/studynote/09_security/04_endpoint_security/356_privilege_escalation/) 멸망 랙!)** 이다. 만약 누군가 실수로 `bash` 쉘이나 `vi` 에 SetUID 록백을 걸어 두면, 해커가 그걸 실행하는 즉시 서버 전체를 박살 내는 최고 신(Root) 권한을 날름 집어먹는 시스템 치명타([Security](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/283_security_tactics/) Hole) 데들락을 낳게 된다 결착.
...
- **개념**: 
  ...
  - **SetUID / SetGID / Sticky [Bit](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/086_fenwick_tree/) (특수 권한 3대 돌파 빔!)**: x(실행) 권한 자리에 `s` 나 `t` 라는 특수 알파벳 영혼을 입힌다. "실행하는 동안에만 신분을 빌려주는 마법(SetUID/SetGID 스왑)" 과 "버릴 순 있어도 치울 순 없는 공용 휴지통 마법(Sticky [Bit](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/086_fenwick_tree/) 록백)" 으로 OS 한계를 압살 시키는 설계 기전이다.
...
- **SetUID 실행 시 [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/) PID / UID 자격 스위칭 [ASCII](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/103_ascii/) 메커니즘 뷰**:
...
| **SetGID (2000 빔)** | `실행 파일` 및 `디렉터리` | 실행 시 "[파일](/knowledge-base/studynote/02_operating_system/09_file_system/501_file_definition_logical_record/) 소속 그룹(Group)" 으로 빙의! 혹은 폴더에 걸면 **"그 안에서 만들어지는 모든 [파일](/knowledge-base/studynote/02_operating_system/09_file_system/501_file_definition_logical_record/)은 폴더 소속 그룹으로 강제 [상속](/knowledge-base/studynote/04_software_engineering/04_testing_quality/234_uml_class_relationships_generalization_dependency/) 통일 됨 록백!"** (팀 폴더 협업의 성배). |
| **Sticky [Bit](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/086_fenwick_tree/) (1000 빔)** | `디렉터리(폴더)` | 547장 [참조](/knowledge-base/studynote/05_database/05_distributed_nosql_newsql/316_reference_pattern_nosql/). 누구나 쓰고 버릴 순 있지만(777), **"자기가 만든 [파일](/knowledge-base/studynote/02_operating_system/09_file_system/501_file_definition_logical_record/)이 아니면 삭제/이름 변경 절대 불가 컷!"** (`/tmp` 의 불멸 보호구역). |
...
- **[안티패턴](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/128_water_scrum_fall_anti_pattern/) 오염 발생 미스터리 (잘못 걸린 루트 SetUID 해킹 암살 데들락 랙)**: 
...
- **[SRE](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/100_sre_site_reliability_engineering_error_budget/) 극복 솔루션 패치 타결 조율 (`find -perm -4000` 색출 봇과 nosuid [마운트](/knowledge-base/studynote/02_operating_system/09_file_system/516_mount_mechanism/) 록백!!) / [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/) 본진 방어**: 
...
- **[안티패턴](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/128_water_scrum_fall_anti_pattern/) 충돌 (리눅스 폴더 소유권 파편화 [OOM](/knowledge-base/studynote/02_operating_system/02_process_thread/157_oom_killer/) 에러 데들락)**: 
...
- **[SRE](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/100_sre_site_reliability_engineering_error_budget/) 엔지니어 도축 솔루션 (SetGID 2000 빔 강제 [상속](/knowledge-base/studynote/04_software_engineering/04_testing_quality/234_uml_class_relationships_generalization_dependency/) 스왑 렌더!)**: 
  - 엔지니어 한 방: 그 `dev-team` 최상위 폴더에 대고 `chmod 2770 (SetGID)` 마법을 쏜다! 
  - 갓기능 발동 스로틀: 이 폴더 안에서 1,000만 개의 [파일](/knowledge-base/studynote/02_operating_system/09_file_system/501_file_definition_logical_record/)이 미친 듯이 생성되더라도? **"누가 만들었든 상관하지 마라! 무조건 [파일](/knowledge-base/studynote/02_operating_system/09_file_system/501_file_definition_logical_record/)의 소속 그룹을 부모 폴더의 소속 그룹(dev팀)으로 강제 이름 세탁 강등 결합시켜버린다 록백!!"** (Group [상속](/knowledge-base/studynote/04_software_engineering/04_testing_quality/234_uml_class_relationships_generalization_dependency/) 뷰). 
