...
### [KVM](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/713_kvm_over_ip/) [하이퍼바이저](/knowledge-base/studynote/02_operating_system/01_overview_architecture/054_hypervisor/)의 구원 (Async PF in [KVM](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/713_kvm_over_ip/))

이 기술이 가장 절실하게 투입된 곳은 클라우드 가상 머신(AWS EC2, [KVM](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/713_kvm_over_ip/)) 시장이다.
...
- **📢 섹션 요약 비유**: 옛날엔 우리 집 마당(메모리)에 누가 침입([Page Fault](/knowledge-base/studynote/02_operating_system/07_virtual_memory/387_page_fault/))하면 경찰(OS [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/))이 무조건 출동해서 사건을 다 처리할 때까지 온 동네를 통제([Blocking](/knowledge-base/studynote/02_operating_system/02_process_thread/122_sync_async_communication/))했습니다. `userfaultfd`는 마당에 사설 CCTV와 내 스마트폰(유저 공간)을 연결해 놔서, 침입자가 들어오면 경찰 안 부르고 내 폰에 알람(이벤트)만 울리게 한 뒤 내가 몽둥이 들고 알아서 조용히(비동기) 처리해 버리는 자경단 시스템입니다.

---

## Ⅲ. 비교 및 연결

### 비교 1: 비동기 I/O (epoll/AIO) vs 비동기 [Page Fault](/knowledge-base/studynote/02_operating_system/07_virtual_memory/387_page_fault/)
...
클라우드에서 게임 서버([VM](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/598_vm_migration_nic/))를 끄지 않고 그대로 옆의 물리 서버 장비로 이사([Live Migration](/knowledge-base/studynote/02_operating_system/10_security/629_live_migration_pre_copy/))시키는 마술도 이 `userfaultfd` 비동기 기술의 덕분이다.
VM을 이사 시킨 뒤 일단 껍데기만 새 서버에 띄운다. 유저가 게임을 하다가 램을 찌르면([Page Fault](/knowledge-base/studynote/02_operating_system/07_virtual_memory/387_page_fault/)), `userfaultfd`가 터져서 옛날 서버에서 네트워크로 램 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 실시간으로 쭉 빨아온다(비동기 배달). 게임 유저는 서버가 통째로 이사 갔다는 사실조차 눈치채지 못한 채 0.1초의 잔렉만 느끼고 쾌적하게 게임을 이어간다.

- **📢 섹션 요약 비유**: 외과 수술 중인 환자(돌아가는 서버)를 침대 통째로 다른 병원으로 이송([Live Migration](/knowledge-base/studynote/02_operating_system/10_security/629_live_migration_pre_copy/))합니다. 옛날엔 수술을 중단(Sync 폴트)하고 마취를 더 시킨 뒤 앰뷸런스를 탔지만, 비동기 기술을 쓰면 의사들이 앰뷸런스 안에서도 피를 수혈하고 메스를 들이대며 수술(비동기 I/O)을 1초도 멈추지 않고 계속 진행하는 미친 의술의 경지입니다.
...
비동기식 [페이지 폴트](/knowledge-base/studynote/02_operating_system/11_exam_summary/720_page_fault_isr/) (Asynchronous [Page](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/) Faults) 핸들링은 "모든 I/O는 비동기(Non-[blocking](/knowledge-base/studynote/02_operating_system/02_process_thread/122_sync_async_communication/))가 답이다"라는 웹 생태계의 진리를 운영체제의 가장 깊숙한 하드웨어 [트랩](/knowledge-base/studynote/02_operating_system/11_exam_summary/677_trap_based_system_call_implementation/)([MMU](/knowledge-base/studynote/02_operating_system/06_memory_management/328_mmu/)) 레벨까지 끌고 내려간 집념의 결과물이다. OS는 [가상 메모리](/knowledge-base/studynote/02_operating_system/07_virtual_memory/381_virtual_memory/)를 통해 개발자에게 "램이 무한하다"는 환상을 심어주었으나, 그 환상이 깨지는 찰나([Page Fault](/knowledge-base/studynote/02_operating_system/07_virtual_memory/387_page_fault/))에 청구하는 무지막지한 8ms의 청구서([Blocking](/knowledge-base/studynote/02_operating_system/02_process_thread/122_sync_async_communication/) Delay)는 더 이상 현대 클라우드의 트래픽을 감당할 수 없었다. 그래서 OS는 환상이 깨진 순간조차도 앱의 목덜미를 잡는 대신 "내가 뒤에서 몰래 땜빵할 테니 넌 계속 딴일 해라"며 비동기 알림(Event)으로 퉁치는 우아한 출구 전략을 마련했다. 향후 [CXL](/knowledge-base/studynote/01_computer_architecture/12_accelerators_ai_hardware/441_cxl/) 인터페이스와 원격 메모리(Disaggregated Memory) 기술이 상용화되면 이 "남의 서버에서 램을 퍼오는" 네트워크 [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/)을 덮기 위해, [페이지 폴트](/knowledge-base/studynote/02_operating_system/11_exam_summary/720_page_fault_isr/)의 비동기 처리는 단순한 튜닝 옵션이 아니라 모든 차세대 OS가 반드시 장착해야 할 핵심 심장 엔진으로 격상될 것이다.

- **📢 섹션 요약 비유**: 예전엔 식당 알바(CPU)가 설거지(디스크 I/O)를 하러 주방에 들어가면 홀에 있는 손님들(다른 [스레드](/knowledge-base/studynote/02_operating_system/02_process_thread/092_thread_lwp/))은 무작정 알바가 나올 때까지 굶고 기다려야 했습니다(동기식 폴트). 지금은 알바가 주방에 식기세척기(비동기 백그라운드)를 휙 돌려놓고 다시 홀로 뛰어나와 주문을 받는(비동기 폴트) 미친 회전율의 진정한 오토메이션 식당으로 진화한 것입니다.
...
| [역 페이지 테이블](/knowledge-base/studynote/02_operating_system/06_memory_management/363_inverted_page_table/) 탐색 최적화 [해시 함수](/knowledge-base/studynote/03_network/13_network_security_basics/667_hash_function_integrity_one_way/) | 현재 개념이 등장하게 만든 직접적인 선행 흐름이다. |
| [TLB](/knowledge-base/studynote/02_operating_system/06_memory_management/357_tlb/) 슛다운 ([TLB Shootdown](/knowledge-base/studynote/02_operating_system/07_virtual_memory/435_tlb_shootdown/)) | 현재 개념이 구현·세분화될 때 바로 연결되는 후속 개념이다. |
...
1. 비동기식 [페이지 폴트](/knowledge-base/studynote/02_operating_system/11_exam_summary/720_page_fault_isr/) (Asynchronous [Page](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/) Faults) 핸들링은 컴퓨터가 메모리를 더 크게 보이게 하고 부족함을 숨기는 방법이에요.
2. 먼저 [역 페이지 테이블](/knowledge-base/studynote/02_operating_system/06_memory_management/363_inverted_page_table/) 탐색 최적화 [해시 함수](/knowledge-base/studynote/03_network/13_network_security_basics/667_hash_function_integrity_one_way/)을 이해하면 비동기식 [페이지 폴트](/knowledge-base/studynote/02_operating_system/11_exam_summary/720_page_fault_isr/) (Asynchronous [Page](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/) Faults) 핸들링이 왜 필요한지 더 쉽게 보여요.
3. 그래서 비동기식 [페이지 폴트](/knowledge-base/studynote/02_operating_system/11_exam_summary/720_page_fault_isr/) (Asynchronous [Page](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/) Faults) 핸들링을 잘 알면 나중에 [TLB](/knowledge-base/studynote/02_operating_system/06_memory_management/357_tlb/) 슛다운 ([TLB Shootdown](/knowledge-base/studynote/02_operating_system/07_virtual_memory/435_tlb_shootdown/))도 훨씬 쉽게 배울 수 있어요.
...
← **이전**: [433. 역 페이지 테이블 탐색 최적화 해시 함수 (Inverted Page Table Hash)](/knowledge-base/studynote/02_operating_system/07_virtual_memory/433_inverted_page_table_hash/)
**다음**: [435. TLB 슛다운 (TLB Shootdown) - 멀티코어 환경에서 타 코어의 TLB 무효화 오버헤드](/knowledge-base/studynote/02_operating_system/07_virtual_memory/435_tlb_shootdown/) →
