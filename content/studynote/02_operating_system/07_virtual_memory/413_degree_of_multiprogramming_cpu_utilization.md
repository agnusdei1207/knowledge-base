...
### 상승 구간의 [논리](/knowledge-base/studynote/09_security/04_endpoint_security/369_logic_bomb/) ([문맥 교환](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/211_context_switch/)의 마술)
...
5. 이때 OS의 [장기 스케줄러](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/163_long_term_scheduler/)는 바보 같은 판단을 내린다. **"어라? CPU가 왜 놀고 있지? 일감이 부족한가 보네! 당장 하드디스크에 있는 새 프로세스 하나 더 램으로 끌고 와서 띄워(Degree +1)!"**
6. 새 프로세스가 들어오며 기존에 헐떡이던 앱들의 남은 램 한 줌마저 다 뺏어버린다. 폴트가 핵폭탄급으로 터진다.
7. CPU 가동률이 1%로 수직 낙하한다. [스래싱](/knowledge-base/studynote/02_operating_system/04_synchronization/257_thrashing/)([Thrashing](/knowledge-base/studynote/02_operating_system/04_synchronization/257_thrashing/))이 완성된다.

- **📢 섹션 요약 비유**: 식당 주방(CPU)이 팽팽 돌다가 갑자기 프라이팬(램)이 모자라서 요리사들이 요리를 못 하고 멍때립니다(가동률 하락). 이걸 본 멍청한 사장([스케줄러](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/079_kube_scheduler_pod_placement/))이 "주방이 왜 놀고 있어? 주문 더 받아!!" 하고 손님(새 프로세스)을 무작정 더 받았습니다. 결국 프라이팬 쟁탈전이 벌어져 주방은 폭동이 나고 요리는 1개도 안 나오는 멸망의 길입니다.

---

## Ⅲ. 비교 및 연결

### [워킹 셋](/knowledge-base/studynote/02_operating_system/04_synchronization/265_working_set/)([Working Set](/knowledge-base/studynote/02_operating_system/04_synchronization/265_working_set/))과 물리 램의 수학적 공식

이 그래프의 꼭대기 변곡점이 터지는 시점을 수학적으로 예측할 수 있다.

> **$\sum_{i=1}^{n} WS_i > M$ (물리 메모리의 총합)**

- $WS_i$ = $i$번째 프로세스가 쾌적하게 렉 없이 돌기 위해 무조건 쥐고 있어야 하는 최소한의 램 카드 뭉치 크기 ([Working Set](/knowledge-base/studynote/02_operating_system/04_synchronization/265_working_set/))
- $M$ = 컴퓨터 메인보드에 꽂혀있는 실제 물리 램(RAM)의 총 프레임 개수
- **해석**: 띄워놓은 앱들의 필수 [워킹 셋](/knowledge-base/studynote/02_operating_system/04_synchronization/265_working_set/)을 다 더한 값이 물리 램 총량을 넘어서는 그 1바이트의 순간! 절벽으로 추락한다. [스래싱](/knowledge-base/studynote/02_operating_system/04_synchronization/257_thrashing/)이 터지는 것이다.

### 비교 1: 램 증설([Scale-up](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/621_scale_up_system_bus/)) vs [알고리즘](/knowledge-base/studynote/08_algorithm_stats/01_basics/001_algorithm_definition/) 튜닝(Optimization)
...
### 실무 시나리오: [Kubernetes](/knowledge-base/studynote/12_it_management/05_security_compliance/205_kubernetes_container_orchestration/)(k8s)의 HPA와 [OOM Killed](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/110_oom_out_of_memory_killed_kubernetes_limits/) 튜닝
1. **문제의 발단**: 클라우드 인프라에서 개발자가 Node.js [컨테이너](/knowledge-base/studynote/04_software_engineering/09_cloud_native_ai_architecture/561_container_based_deployment/) 1개에 램 Limit을 안 걸어놓고 띄웠다. 이 녀석이 램을 10GB 다 빨아먹자, 같은 서버에 있던 옆 [컨테이너](/knowledge-base/studynote/04_software_engineering/09_cloud_native_ai_architecture/561_container_based_deployment/)들이 램을 못 받아 뻗기 시작했다 (그래프의 절벽 낙하).
...
신입 엔지니어가 `top` 명령어를 치고 "팀장님, CPU 사용률이 2%밖에 안 되는데 서버 접속이 안 됩니다!"라고 하면, 고수 팀장은 뒤통수를 때리며 "이 바보야, 그건 놀고 있는 게 아니라 [스래싱](/knowledge-base/studynote/02_operating_system/04_synchronization/257_thrashing/) 터져서 뇌사 상태인 거야. `wa (iowait)` 수치랑 디스크 불 들어온 거 봐!"라고 일갈한다. 이 그래프의 오른쪽 밑바닥 절벽(CPU 낮음 + 렉 폭발)을 경험해 본 자만이 진정한 시스템 엔지니어로 거듭난다.

- **📢 섹션 요약 비유**: 클럽(서버) 문지기(k8s [스케줄러](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/079_kube_scheduler_pod_placement/))가 클럽 안에 사람이 꽉 차서 발 디딜 틈이 없으면, 밖에 손님(새 프로세스)이 아무리 줄을 서서 떼를 써도 절대 문을 열어주지 않고 막아서는(Pending) 이유가 바로 안에 있는 손님들의 유흥의 질([워킹 셋](/knowledge-base/studynote/02_operating_system/04_synchronization/265_working_set/))을 보호해 [스래싱](/knowledge-base/studynote/02_operating_system/04_synchronization/257_thrashing/)을 막기 위함입니다.
...
[다중 프로그래밍 정도](/knowledge-base/studynote/02_operating_system/04_synchronization/258_degree_of_multiprogramming/)와 CPU 이용률 [관계](/knowledge-base/studynote/05_database/02_modeling_normalization/083_relationship_in_er_model/) 그래프는, 컴퓨터 과학에서 "욕심이 과하면 모든 것을 잃는다(과유불급)"는 동양의 지혜를 가장 날카로운 2차원 수리 모델로 증명해 낸 예술 작품이다. [가상 메모리](/knowledge-base/studynote/02_operating_system/07_virtual_memory/381_virtual_memory/)의 무한한 공간이라는 오만한 환상 속에서, 물리적 한계(디스크 I/O)라는 현실의 벽에 부딪혀 수직 낙하하는 이 뼈아픈 곡선은 수많은 [운영체제](/knowledge-base/studynote/02_operating_system/01_overview_architecture/001_operating_system_purpose/) 설계자들의 밤을 지새우게 했다. 이 절벽을 막기 위해 인류는 [LRU](/knowledge-base/studynote/02_operating_system/04_synchronization/262_lru_page_replacement/), [워킹 셋 모델](/knowledge-base/studynote/02_operating_system/07_virtual_memory/416_working_set_model/), [PFF](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/306_pff/), [OOM](/knowledge-base/studynote/02_operating_system/02_process_thread/157_oom_killer/) 킬러 등 수백 가지의 갑옷을 덧대어 왔다. 미래에 [CXL](/knowledge-base/studynote/01_computer_architecture/12_accelerators_ai_hardware/441_cxl/) [메모리 풀](/knowledge-base/studynote/02_operating_system/06_memory_management/369_memory_pool/)(Pool)이 적용되어 "내 서버 램이 꽉 차면 0.001초 만에 옆 서버 램을 빌려다 쓰는" 마법의 시대가 열린다면 이 절벽의 경사가 조금 더 완만해지겠지만, 자원의 물리적 희소성이 존재하는 한 이 우상향 후 수직 낙하하는 자연 법칙의 굴레는 IT 시스템의 영원한 숙명으로 남을 것이다.

- **📢 섹션 요약 비유**: 이 그래프는 인간의 탐욕을 그리는 심전도입니다. 빚([가상 메모리](/knowledge-base/studynote/02_operating_system/07_virtual_memory/381_virtual_memory/))을 내서 주식을 1개, 10개, 100개 투자할 때마다 수익률(CPU 가동률)은 짜릿하게 우상향하지만, 내 원금(물리 램) 한도를 1원이라도 초과하여 마진콜([Page Fault](/knowledge-base/studynote/02_operating_system/07_virtual_memory/387_page_fault/))이 터지는 순간 반대매매로 내 전 재산이 1초 만에 0원([스래싱](/knowledge-base/studynote/02_operating_system/04_synchronization/257_thrashing/))으로 수직 낙하하는 가장 잔인하고 정직한 자본주의 곡선입니다.
...
| 개념 | 연결 포인트 |
|:---|:---|
| [에이징](/knowledge-base/studynote/02_operating_system/07_virtual_memory/411_aging_algorithm/) ([Aging](/knowledge-base/studynote/02_operating_system/03_cpu_scheduling/182_aging/)) 기반 [페이지 교체](/knowledge-base/studynote/02_operating_system/04_synchronization/260_page_replacement/) 로직 | 현재 개념으로 들어오기 전에 함께 이해하면 경계가 선명해지는 기반 개념이다. |
| [스래싱](/knowledge-base/studynote/02_operating_system/04_synchronization/257_thrashing/) ([Thrashing](/knowledge-base/studynote/02_operating_system/04_synchronization/257_thrashing/)) | 현재 개념이 등장하게 만든 직접적인 선행 흐름이다. |
| [스래싱 원인](/knowledge-base/studynote/02_operating_system/07_virtual_memory/414_cause_of_thrashing/) | 현재 개념이 구현·세분화될 때 바로 연결되는 후속 개념이다. |
| [지역성 모델](/knowledge-base/studynote/02_operating_system/07_virtual_memory/415_locality_model/) ([Locality Model](/knowledge-base/studynote/02_operating_system/07_virtual_memory/415_locality_model/)) | 확장 학습이나 심화 비교로 이어지는 다음 단계의 키워드다. |
...
1. [다중 프로그래밍 정도](/knowledge-base/studynote/02_operating_system/04_synchronization/258_degree_of_multiprogramming/) ([Degree of Multiprogramming](/knowledge-base/studynote/02_operating_system/04_synchronization/258_degree_of_multiprogramming/))와 CPU 이용률 [관계](/knowledge-base/studynote/05_database/02_modeling_normalization/083_relationship_in_er_model/) 그래프은 컴퓨터가 메모리를 더 크게 보이게 하고 부족함을 숨기는 방법이에요.
2. 먼저 [스래싱](/knowledge-base/studynote/02_operating_system/04_synchronization/257_thrashing/) ([Thrashing](/knowledge-base/studynote/02_operating_system/04_synchronization/257_thrashing/))을 이해하면 [다중 프로그래밍 정도](/knowledge-base/studynote/02_operating_system/04_synchronization/258_degree_of_multiprogramming/) ([Degree of Multiprogramming](/knowledge-base/studynote/02_operating_system/04_synchronization/258_degree_of_multiprogramming/))와 CPU 이용률 [관계](/knowledge-base/studynote/05_database/02_modeling_normalization/083_relationship_in_er_model/) 그래프이 왜 필요한지 더 쉽게 보여요.
3. 그래서 [다중 프로그래밍 정도](/knowledge-base/studynote/02_operating_system/04_synchronization/258_degree_of_multiprogramming/) ([Degree of Multiprogramming](/knowledge-base/studynote/02_operating_system/04_synchronization/258_degree_of_multiprogramming/))와 CPU 이용률 [관계](/knowledge-base/studynote/05_database/02_modeling_normalization/083_relationship_in_er_model/) 그래프을 잘 알면 나중에 [스래싱 원인](/knowledge-base/studynote/02_operating_system/07_virtual_memory/414_cause_of_thrashing/)도 훨씬 쉽게 배울 수 있어요.
