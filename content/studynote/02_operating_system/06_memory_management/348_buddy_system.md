...
> 1. **본질**: 버디 시스템(Buddy System) 할당기는 메모리를 요청받았을 때, 전체 물리 메모리를 **무조건 2의 승수(2^k) 크기로만 반씩 쪼개어(Split) 할당**하고, 반환될 때는 쪼개졌던 **자신의 쌍둥이 조각(Buddy)과 [초고속](/knowledge-base/studynote/06_ict_convergence/02_iot_mobility/148_5g_embb_urllc_mmtc/)으로 병합(Coalescing)하는 [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/) 메모리 할당 기법**이다.
...
- **등장 배경 및 아키텍처 타협**:
  ...
  3. **[내부 단편화](/knowledge-base/studynote/02_operating_system/06_memory_management/341_internal_fragmentation/)의 수용**: 17KB를 요구하면 32KB를 줘야 하므로 무려 15KB의 [내부 단편화](/knowledge-base/studynote/02_operating_system/06_memory_management/341_internal_fragmentation/)가 버려진다. 하지만 [외부 단편화](/knowledge-base/studynote/02_operating_system/06_memory_management/342_external_fragmentation/) 방어력과 [초고속](/knowledge-base/studynote/06_ict_convergence/02_iot_mobility/148_5g_embb_urllc_mmtc/) 런타임 속도를 얻기 위해 이 공간적 낭비를 쿨하게 지불(Trade-off)하기로 했다.
...
버디 시스템을 구현하는 [운영체제](/knowledge-base/studynote/02_operating_system/01_overview_architecture/001_operating_system_purpose/)는 단 하나의 장부가 아니라, **크기별로 [배열](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/055_array/)([Array](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/055_array/) of Linked Lists)**을 만들어 장부를 유지한다.
- `List[0]`: 4KB 빈방들 모음
- `List[1]`: 8KB 빈방들 모음
- `List[5]`: 128KB 빈방들 모음
...
| **[내부 단편화](/knowledge-base/studynote/02_operating_system/06_memory_management/341_internal_fragmentation/)** | **전혀 없음** (0%) | **치명적임** (요구량에 따라 최대 49.9% 낭비) |
| **[외부 단편화](/knowledge-base/studynote/02_operating_system/06_memory_management/342_external_fragmentation/)** | 치명적 발생 (33% 낭비, [OOM](/knowledge-base/studynote/02_operating_system/02_process_thread/157_oom_killer/) 유발) | 병합이 너무 빨라 거대 구멍이 잘 복원됨 (우수) |
...
- 결과적으로 64 - 33 = **31KB**가 그 방 안에서 아무도 쓰지 못한 채 공중으로 증발해버린다. (거의 50%의 [내부 단편화](/knowledge-base/studynote/02_operating_system/06_memory_management/341_internal_fragmentation/))
...
2. **리눅스 [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/)의 선택**:
   - 리눅스 [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/)은 물리 프레임을 나눠줄 때 바로 이 **버디 시스템 [알고리즘](/knowledge-base/studynote/08_algorithm_stats/01_basics/001_algorithm_definition/)**을 심장 엔진으로 사용한다.
...
버디 시스템이 [내부 단편화](/knowledge-base/studynote/02_operating_system/06_memory_management/341_internal_fragmentation/)(33KB 요청 시 64KB 줌)로 메모리를 너무 심하게 버리는 문제를 보완하기 위해, 리눅스는 버디 시스템 위에 **[슬랩 할당기](/knowledge-base/studynote/02_operating_system/06_memory_management/349_slab_allocator/)([Slab Allocator](/knowledge-base/studynote/02_operating_system/06_memory_management/349_slab_allocator/))**를 이중으로 얹었다.
...
| **[외부 단편화](/knowledge-base/studynote/02_operating_system/06_memory_management/342_external_fragmentation/) 내성** | 해제 즉시 도미노처럼 거대 블록으로 융합(Coalescing)되어 물리적 덩어리가 쪼개진 채 방치되는 현상 [억제](/knowledge-base/studynote/09_security/13_secops_ir_forensics/656_ir_containment/) |
