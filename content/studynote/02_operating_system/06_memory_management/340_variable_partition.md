...
- **개념**: 가변 분할 방식(MVT: [Multiprogramming](/knowledge-base/studynote/02_operating_system/11_exam_summary/673_multiprogramming_bottleneck_resource/) with a Variable number of Tasks)은 [운영체제](/knowledge-base/studynote/02_operating_system/01_overview_architecture/001_operating_system_purpose/)가 거대한 하나의 빈 물리 메모리 덩어리(Hole)를 가지고 있다가, 프로세스가 들어오면 그 크기만큼만 칼로 자르듯 분할하여 내어주고 나머지를 새로운 빈 구멍으로 관리하는 기법이다.
- **필요성**: [고정 분할 방식](/knowledge-base/studynote/02_operating_system/06_memory_management/339_fixed_partition/)은 20MB 방에 5MB 앱이 들어가면 15MB가 그냥 버려지는 뼈아픈 내부 낭비를 초래했다. 메모리가 금값보다 비싸던 시절, 이렇게 허무하게 증발하는 용량을 눈뜨고 볼 수 없었다. "손님이 요구하는 평수만큼만 정확히 가벽을 쳐서 임대해주자"는 유연한 임대 방식의 철학이 절실했다.
...
### 최후의 수단: 메모리 [압축](/knowledge-base/studynote/02_operating_system/06_memory_management/347_compaction/) ([Compaction](/knowledge-base/studynote/02_operating_system/06_memory_management/347_compaction/))

가변 분할에서 [외부 단편화](/knowledge-base/studynote/02_operating_system/06_memory_management/342_external_fragmentation/)가 한계에 달해 더 이상 프로그램을 못 올리게 되면, OS는 '[압축](/knowledge-base/studynote/02_operating_system/06_memory_management/347_compaction/)([Compaction](/knowledge-base/studynote/02_operating_system/06_memory_management/347_compaction/))'이라는 극약 처방을 내린다.
- 흩어진 모든 프로세스를 중단시키고, 메모리의 한쪽 끝으로 블록 셔플(Copy & Paste) 하듯 주루룩 밀어버린다.
- 이 과정에서 프로세스의 주소가 실시간으로 바뀌므로, 오직 '[실행 시간 바인딩](/knowledge-base/studynote/02_operating_system/06_memory_management/327_execution_time_binding/)([Execution Time Binding](/knowledge-base/studynote/02_operating_system/06_memory_management/327_execution_time_binding/))' 환경에서만 가능한 마법이다.
- 하지만 시스템 전체 메모리를 복사하는 막대한 오버헤드로 인해 시스템이 일시 정지하는 끔찍한 대가를 치른다.

- **📢 섹션 요약 비유**: 테트리스를 하다가 블록 중간중간 구멍이 뻥뻥 뚫려서 게임 오버가 되기 직전에, 밑에서부터 남은 구멍들을 다 [압축](/knowledge-base/studynote/02_operating_system/06_memory_management/347_compaction/)해서 한 줄 쫙 지워버리는 궁극기([압축](/knowledge-base/studynote/02_operating_system/06_memory_management/347_compaction/))를 쓰는 것입니다.

---

## Ⅲ. 비교 및 연결

### 비교 1: 고정 분할 vs 가변 분할 vs [페이징](/knowledge-base/studynote/02_operating_system/04_synchronization/259_paging/)
...
### [안티패턴](/knowledge-base/studynote/04_software_engineering/02_requirements_analysis/128_water_scrum_fall_anti_pattern/): 파편화 방치
서버를 몇 달 동안 재부팅하지 않고 운영하면, 메모리 관리 체계의 파편화가 극에 달한다. 메모리는 50%나 남았는데, 새 프로세스를 띄우려 하면 [OOM](/knowledge-base/studynote/02_operating_system/02_process_thread/157_oom_killer/)([Out of Memory](/knowledge-base/studynote/02_operating_system/02_process_thread/157_oom_killer/)) 에러를 내며 죽어버린다. 이는 가변 분할 구조의 근본적 모순이므로, 정기적인 [압축](/knowledge-base/studynote/02_operating_system/06_memory_management/347_compaction/)이나 서버 재기동을 통한 메모리 파편화 해소(Defragmentation)가 필요하다.

- **📢 섹션 요약 비유**: OS는 방 크기를 잘게 쪼개는 기술([페이징](/knowledge-base/studynote/02_operating_system/04_synchronization/259_paging/))로 도망쳤지만, 프로그래머가 방 안에서 자기 짐을 정리하는 방식(malloc)은 여전히 가변 분할의 저주(Stop-the-world)에 갇혀 고통받고 있는 아이러니입니다.
...
가변 분할 방식 (Variable [Partition](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/))은 고정 분할의 어리석음을 깨고 '동적이고 유연한 [자원 할당](/knowledge-base/studynote/02_operating_system/01_overview_architecture/041_resource_allocation/)'이라는 현대 컴퓨터 공학의 핵심 철학을 최초로 정립한 위대한 이정표다. 비록 OS [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/) 레벨의 물리 메모리 관리에서는 '[외부 단편화](/knowledge-base/studynote/02_operating_system/06_memory_management/342_external_fragmentation/)'의 한계를 넘지 못해 [페이징](/knowledge-base/studynote/02_operating_system/04_synchronization/259_paging/)([Paging](/knowledge-base/studynote/02_operating_system/04_synchronization/259_paging/))에 왕좌를 내어주었지만, 그 내면의 동적 메모리 배치 [알고리즘](/knowledge-base/studynote/08_algorithm_stats/01_basics/001_algorithm_definition/)([First-fit](/knowledge-base/studynote/02_operating_system/06_memory_management/344_first_fit/), [Best-fit](/knowledge-base/studynote/02_operating_system/06_memory_management/345_best_fit/))과 흩어진 구멍을 합치는 [압축](/knowledge-base/studynote/02_operating_system/06_memory_management/347_compaction/)([Compaction](/knowledge-base/studynote/02_operating_system/06_memory_management/347_compaction/)) [알고리즘](/knowledge-base/studynote/08_algorithm_stats/01_basics/001_algorithm_definition/)은 오늘날 수많은 프로그래밍 언어의 [가비지 컬렉터](/knowledge-base/studynote/05_database/uncategorized/591_mvcc_garbage_collection_vacuum/)(GC)와 힙 관리 엔진 속에서 영원히 살아 숨 쉬고 있다.

- **📢 섹션 요약 비유**: 기성복(고정 분할)만 팔던 시대에 나타난 맞춤 양복점(가변 분할)입니다. 천을 자르고 남은 자투리 천([외부 단편화](/knowledge-base/studynote/02_operating_system/06_memory_management/342_external_fragmentation/))이 골칫거리지만, 손님 몸에 완벽하게 딱 맞는 옷([내부 단편화](/knowledge-base/studynote/02_operating_system/06_memory_management/341_internal_fragmentation/) 제거)을 선사한 패션의 혁명입니다.
...
| 개념 | 연결 포인트 |
|:---|:---|
| [연속 메모리 할당](/knowledge-base/studynote/02_operating_system/06_memory_management/338_contiguous_memory_allocation/) ([Contiguous Memory Allocation](/knowledge-base/studynote/02_operating_system/06_memory_management/338_contiguous_memory_allocation/)) | 현재 개념으로 들어오기 전에 함께 이해하면 경계가 선명해지는 기반 개념이다. |
| [고정 분할 방식](/knowledge-base/studynote/02_operating_system/06_memory_management/339_fixed_partition/) ([Fixed Partition](/knowledge-base/studynote/02_operating_system/06_memory_management/339_fixed_partition/)) | 현재 개념이 등장하게 만든 직접적인 선행 흐름이다. |
| [내부 단편화](/knowledge-base/studynote/02_operating_system/06_memory_management/341_internal_fragmentation/) ([Internal Fragmentation](/knowledge-base/studynote/02_operating_system/06_memory_management/341_internal_fragmentation/)) | 현재 개념이 구현·세분화될 때 바로 연결되는 후속 개념이다. |
| [외부 단편화](/knowledge-base/studynote/02_operating_system/06_memory_management/342_external_fragmentation/) ([External Fragmentation](/knowledge-base/studynote/02_operating_system/06_memory_management/342_external_fragmentation/)) | 확장 학습이나 심화 비교로 이어지는 다음 단계의 키워드다. |
...
1. 가변 분할 방식 (Variable [Partition](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/))은 컴퓨터가 메모리를 방처럼 나눠 쓰고 주소를 찾는 방법이에요.
2. 먼저 [고정 분할 방식](/knowledge-base/studynote/02_operating_system/06_memory_management/339_fixed_partition/) ([Fixed Partition](/knowledge-base/studynote/02_operating_system/06_memory_management/339_fixed_partition/))을 이해하면 가변 분할 방식 (Variable [Partition](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/))이 왜 필요한지 더 쉽게 보여요.
3. 그래서 가변 분할 방식 (Variable [Partition](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/))을 잘 알면 나중에 [내부 단편화](/knowledge-base/studynote/02_operating_system/06_memory_management/341_internal_fragmentation/) ([Internal Fragmentation](/knowledge-base/studynote/02_operating_system/06_memory_management/341_internal_fragmentation/))도 훨씬 쉽게 배울 수 있어요.
...
← **이전**: [339. 고정 분할 방식 (Fixed Partition)](/knowledge-base/studynote/02_operating_system/06_memory_management/339_fixed_partition/)
**다음**: [341. 내부 단편화 (Internal Fragmentation) - 할당된 공간 내 남는 공간](/knowledge-base/studynote/02_operating_system/06_memory_management/341_internal_fragmentation/) →
