...
- **필요성**: 수백 MB에 달하는 프로그램이라도, 한 번의 실행 동안 실제로 호출되는 코드는 극히 일부에 불과하다(예: 에러 처리, 예외 발생 시의 [복구](/knowledge-base/studynote/09_security/13_secops_ir_forensics/658_ir_recovery/) 코드, 수천 개의 메뉴 중 클릭한 1개의 메뉴 코드 등). 이 모든 코드를 한 번에 메모리에 올리면 [다중 프로그래밍](/knowledge-base/studynote/02_operating_system/11_exam_summary/673_multiprogramming_bottleneck_resource/) 환경에서 심각한 메모리 부족 현상을 초래한다. "필요할 때만 가져온다"는 철학이 절대적으로 필요했다.
...
### 비교 1: 동적 적재 (Dynamic Loading) vs [동적 연결](/knowledge-base/studynote/02_operating_system/06_memory_management/332_dynamic_linking/) ([Dynamic Linking](/knowledge-base/studynote/02_operating_system/06_memory_management/332_dynamic_linking/))

가장 많이 혼동되는 두 개념이다. 적재(Loading)는 '메모리에 올리는 행위'고, 연결(Linking)은 '코드 간의 주소 참조를 이어주는 행위'다.

| 비교 항목 | 동적 적재 (Dynamic Loading) | [동적 연결](/knowledge-base/studynote/02_operating_system/06_memory_management/332_dynamic_linking/) ([Dynamic Linking](/knowledge-base/studynote/02_operating_system/06_memory_management/332_dynamic_linking/)) |
|:---|:---|:---|
...
1. **상황**: 100GB짜리 오픈월드 게임(MMORPG)을 실행한다. 사용자의 RAM은 16GB에 불과하다. 이 게임의 모든 텍스처와 몬스터 [AI](/knowledge-base/studynote/04_software_engineering/03_design_architecture/190_ai_llm_requirements_specification/) 루틴을 한 번에 메모리에 올리는 것은 불가능하다.
2. **동적 적재의 적용**:
   - 게임 실행 시: 캐릭터 [생성](/knowledge-base/studynote/02_operating_system/02_process_thread/087_process_state_transition/) 창과 튜토리얼 맵의 기본 엔진(물리 엔진, 네트워크 [소켓](/knowledge-base/studynote/02_operating_system/02_process_thread/125_socket/))만 메모리에 적재한다. (정적 베이스 로드)
...
3. **트레이드오프와 최적화**:
   - 디스크(특히 [HDD](/knowledge-base/studynote/02_operating_system/08_storage_and_io_systems/465_hdd_structure/))에서 메모리로 불러오는 속도가 느리면 렉(Stuttering)이 발생한다.
...
동적 적재 (Dynamic Loading)는 "필요한 것만 제때 가져다 쓴다"는 [JIT](/knowledge-base/studynote/09_security/11_iam_access_control/568_jit_access/) ([Just-In-Time](/knowledge-base/studynote/09_security/11_iam_access_control/568_jit_access/)) 철학의 메모리 관리 버전이다. 초창기에는 이를 위해 프로그래머가 복잡한 [라이브러리](/knowledge-base/studynote/04_software_engineering/06_software_architecture/336_library_vs_framework/)를 직접 호출해야 했으나, 현대 [운영체제](/knowledge-base/studynote/02_operating_system/01_overview_architecture/001_operating_system_purpose/)는 하드웨어 MMU를 활용한 [요구 페이징](/knowledge-base/studynote/02_operating_system/04_synchronization/255_demand_paging/)([Demand Paging](/knowledge-base/studynote/02_operating_system/04_synchronization/255_demand_paging/)) 기법을 도입하여 이 모든 과정을 OS [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/) 단에서 완전히 자동화시켰다. 오늘날 [서버리스](/knowledge-base/studynote/12_it_management/05_security_compliance/206_serverless_cold_start/)([Serverless](/knowledge-base/studynote/12_it_management/05_security_compliance/206_serverless_cold_start/)) 컴퓨팅의 [콜드 스타트](/knowledge-base/studynote/04_software_engineering/09_cloud_native_ai_architecture/559_serverless_cold_start_mitigation/)([Cold Start](/knowledge-base/studynote/06_ict_convergence/05_data_science/347_cold_start_problem/))나 [마이크로서비스 아키텍처](/knowledge-base/studynote/04_software_engineering/04_testing_quality/213_msa_microservices_architecture/)([MSA](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/619_msa_traffic_hardware/))에서의 [지연](/knowledge-base/studynote/03_network/01_data_communication/015_지연_데이터_관점/) [초기](/knowledge-base/studynote/03_network/08_transport_layer/459_quic_fec_forward_error_correction/)화 역시 그 근본 뿌리는 이 동적 적재의 철학과 맥을 같이 한다.
...
2. 먼저 [한계 레지스터](/knowledge-base/studynote/02_operating_system/06_memory_management/330_limit_register/) ([Limit Register](/knowledge-base/studynote/02_operating_system/06_memory_management/330_limit_register/))을 이해하면 동적 적재 (Dynamic Loading)이 왜 필요한지 더 쉽게 보여요.
3. 그래서 동적 적재 (Dynamic Loading)을 잘 알면 나중에 [동적 연결](/knowledge-base/studynote/02_operating_system/06_memory_management/332_dynamic_linking/) ([Dynamic Linking](/knowledge-base/studynote/02_operating_system/06_memory_management/332_dynamic_linking/))도 훨씬 쉽게 배울 수 있어요.
