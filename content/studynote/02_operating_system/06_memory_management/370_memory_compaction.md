...
- **📢 섹션 요약 비유**: 이삿짐([데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/))만 새집으로 나르는 게 끝이 아니라, 동사무소 전입신고([페이지 테이블](/knowledge-base/studynote/02_operating_system/06_memory_management/353_page_table/) 갱신)도 해야 하고, 친구들과 우체국에 바뀐 주소([TLB](/knowledge-base/studynote/02_operating_system/06_memory_management/357_tlb/) Flush)를 다 알려줘야 하는 지독한 행정 처리가 수반되는 고통스러운 작업입니다.

---

## Ⅲ. 비교 및 연결

### 비교 1: 컴팩션([Compaction](/knowledge-base/studynote/02_operating_system/06_memory_management/347_compaction/)) vs [스와핑](/knowledge-base/studynote/02_operating_system/06_memory_management/335_swapping/)/리클레임(Reclaim)
...
### Kswapd와 Kcompactd [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/) 데몬의 협주

리눅스 서버 뒤에는 조용히 일하는 두 명의 청소부 요정(데몬 [스레드](/knowledge-base/studynote/02_operating_system/02_process_thread/092_thread_lwp/))이 있다.
1. **kswapd (용량 청소부)**: 메모리 총량이 쪼들리면 백그라운드에서 몰래 깨어나 안 쓰는 [파일](/knowledge-base/studynote/02_operating_system/09_file_system/501_file_definition_logical_record/) 캐시나 유저 앱 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)를 슬슬 디스크로 버리며 램의 총잔고를 채운다.
...
[커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/) 모듈이나 [서드파티](/knowledge-base/studynote/05_database/06_dw_olap_trends/385_third_party_cookie_deprecation_cdw/) 드라이버를 짤 때 메모리를 `kmalloc`으로 잔뜩 받아놓고 영원히 안 돌려주면, 이 조각들은 컴팩션 로직이 손댈 수 없는 '알박기 텐트(Unmovable)'가 된다. 이 알박기가 램 곳곳에 지뢰처럼 깔리면, [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/)이 아무리 양쪽으로 스캐너를 돌려대도 조각들이 걸려서(Pinning) 대형 구멍을 만들 수 없게 되고 결국 서버가 [OOM](/knowledge-base/studynote/02_operating_system/02_process_thread/157_oom_killer/) 패닉에 빠진다.

- **📢 섹션 요약 비유**: 넓은 잔디밭(RAM)에 텐트를 치려는데, 누군가 뽑을 수 없는 무거운 철근(Unmovable [Page](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/))을 10미터 간격으로 드문드문 박아놓으면, 잔디밭 총 평수가 1만 평이라도 결국 커다란 축구장 하나를 지을 수 없게 되어 땅이 쓸모없어지는 악몽입니다.
...
파편화 관리 및 조각 모음 (Memory [Compaction](/knowledge-base/studynote/02_operating_system/06_memory_management/347_compaction/)) 아키텍처는 [가상 메모리](/knowledge-base/studynote/02_operating_system/07_virtual_memory/381_virtual_memory/)의 우아한 [추상화](/knowledge-base/studynote/04_software_engineering/04_testing_quality/198_abstraction_control_data_process/)([페이징](/knowledge-base/studynote/02_operating_system/04_synchronization/259_paging/)) 뒤에 숨겨진, 하드웨어(물리 램)의 진흙탕 같은 물리적 한계를 닦아내는 [운영체제](/knowledge-base/studynote/02_operating_system/01_overview_architecture/001_operating_system_purpose/)의 피땀 어린 노가다 현장이다. "[연속 할당](/knowledge-base/studynote/02_operating_system/09_file_system/523_contiguous_allocation/)([세그멘테이션](/knowledge-base/studynote/02_operating_system/06_memory_management/364_segmentation/)) 시절에나 있던 [압축](/knowledge-base/studynote/02_operating_system/06_memory_management/347_compaction/)이 왜 현대 [페이징](/knowledge-base/studynote/02_operating_system/04_synchronization/259_paging/) OS에 있나?"라는 철학적 질문에, 이 기술은 "하드웨어 장치([DMA](/knowledge-base/studynote/02_operating_system/11_exam_summary/746_io_direct_memory_access_dma/))와 극한의 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/)([Huge Page](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/517_huge_page/))을 위해 결국 물리적 연속성은 영원히 필요하기 때문"이라는 명쾌한 해답을 내놓았다. 앞으로 클라우드 스케일이 커질수록 램의 파편화 속도는 더 가팔라질 것이며, AI를 접목하여 앱이 멈추지 않는 가장 한가한 타이밍을 노려 은밀하게 메모리를 셔플하는 'Predictive [Compaction](/knowledge-base/studynote/02_operating_system/06_memory_management/347_compaction/)' 기술로 더욱 은밀하게 진화할 것이다.

- **📢 섹션 요약 비유**: [페이징](/knowledge-base/studynote/02_operating_system/04_synchronization/259_paging/)이 "방 지저분해도 가상현실(VR) 고글 쓰면 깨끗하게 보여!"라는 사기극이었다면, 메모리 컴팩션은 그 사기극에 속지 않는 진짜 현실의 육체 노동자(드라이버)들을 위해 뒤에서 매일 밤 남몰래 빗자루질을 하는 어머니([커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/))의 희생입니다.
...
| 개념 | 연결 포인트 |
|:---|:---|
| [커널](/knowledge-base/studynote/02_operating_system/01_overview_architecture/022_kernel_role/) 메모리 할당 방식 (kmalloc, vmalloc) | 현재 개념으로 들어오기 전에 함께 이해하면 경계가 선명해지는 기반 개념이다. |
| [메모리 풀](/knowledge-base/studynote/02_operating_system/06_memory_management/369_memory_pool/) ([Memory Pool](/knowledge-base/studynote/02_operating_system/06_memory_management/369_memory_pool/)) 기법 | 현재 개념이 등장하게 만든 직접적인 선행 흐름이다. |
| [거대 페이지](/knowledge-base/studynote/02_operating_system/06_memory_management/371_huge_pages/) ([Huge Pages](/knowledge-base/studynote/02_operating_system/06_memory_management/371_huge_pages/) / Transparent [Huge Pages](/knowledge-base/studynote/02_operating_system/06_memory_management/371_huge_pages/)) | 현재 개념이 구현·세분화될 때 바로 연결되는 후속 개념이다. |
| 아키텍처 종속적인 [MMU](/knowledge-base/studynote/02_operating_system/06_memory_management/328_mmu/) 인터페이스 | 확장 학습이나 심화 비교로 이어지는 다음 단계의 키워드다. |
...
2. 먼저 [메모리 풀](/knowledge-base/studynote/02_operating_system/06_memory_management/369_memory_pool/) ([Memory Pool](/knowledge-base/studynote/02_operating_system/06_memory_management/369_memory_pool/)) 기법을 이해하면 파편화 관리 및 조각 모음이 왜 필요한지 더 쉽게 보여요.
3. 그래서 파편화 관리 및 조각 모음을 잘 알면 나중에 [거대 페이지](/knowledge-base/studynote/02_operating_system/06_memory_management/371_huge_pages/) ([Huge Pages](/knowledge-base/studynote/02_operating_system/06_memory_management/371_huge_pages/) / Transparent [Huge Pages](/knowledge-base/studynote/02_operating_system/06_memory_management/371_huge_pages/))도 훨씬 쉽게 배울 수 있어요.
