...
- **개념**: 고정 분할 방식(MFT: [Multiprogramming](/knowledge-base/studynote/02_operating_system/11_exam_summary/673_multiprogramming_bottleneck_resource/) with a Fixed number of Tasks)은 메모리를 [파티션](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/) 단위로 나누며, 한 번 분할된 [파티션](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/)의 크기와 개수는 시스템을 재부팅하기 전까지 절대 변하지 않는다. 각 분할의 크기는 모두 같을 수도 있고(균등 분할), 서로 다를 수도 있다(비균등 분할).
...
### 작업 큐(Job [Queue](/knowledge-base/studynote/08_algorithm_stats/04_datastructure/058_queue/)) 운영 아키텍처
...
- **📢 섹션 요약 비유**: 피자 라지 사이즈(고정 [파티션](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/))를 한 판 시켰는데, 나는 2조각(프로세스 크기)만 먹고 배불러서 남은 6조각([내부 단편화](/knowledge-base/studynote/02_operating_system/06_memory_management/341_internal_fragmentation/))은 버려버리는 지독한 식량 낭비 상황입니다.
...
### 현대 [페이징](/knowledge-base/studynote/02_operating_system/04_synchronization/259_paging/)([Paging](/knowledge-base/studynote/02_operating_system/04_synchronization/259_paging/)) 기법과의 기묘한 유사성

놀랍게도 가장 원시적인 고정 분할 방식은 현대의 최첨단 기법인 '[페이징](/knowledge-base/studynote/02_operating_system/04_synchronization/259_paging/)([Paging](/knowledge-base/studynote/02_operating_system/04_synchronization/259_paging/))' 기법과 철학적 뿌리가 맞닿아 있다.
...
### 실무 시나리오: 메인프레임과 은행 [배치 처리](/knowledge-base/studynote/13_cloud_architecture/05_data_engineering/228_batch_processing_hadoop_spark/)

1. **과거의 영광 (IBM OS/360)**:
   - 1960년대, 다수의 은행 월말 정산 배치(Batch) 프로그램을 돌리기 위해 IBM은 MFT(고정 분할 [다중 프로그래밍](/knowledge-base/studynote/02_operating_system/11_exam_summary/673_multiprogramming_bottleneck_resource/)) OS를 개발했다.
...
- **📢 섹션 요약 비유**: 헬스장에서 한 달 치 락커룸 비용을 통째로 내게(고정 할당) 만들면, 회원이 며칠 안 와서 락커가 비더라도([내부 단편화](/knowledge-base/studynote/02_operating_system/06_memory_management/341_internal_fragmentation/)) 헬스장 사장님은 복잡하게 관리할 필요 없이 돈 계산이 편해지는 원리입니다.
...
고정 분할 방식 (Fixed [Partition](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/))은 메모리라는 자원을 효율성보다는 '관리의 편의성'과 '시스템 오버헤드 최소화'에 방점을 두고 설계한 초창기 컴퓨터 공학의 순수한 유산이다. 너무 큰 [내부 단편화](/knowledge-base/studynote/02_operating_system/06_memory_management/341_internal_fragmentation/)와 경직성 때문에 범용 메모리 관리 아키텍처에서는 가변 분할과 [페이징](/knowledge-base/studynote/02_operating_system/04_synchronization/259_paging/)으로 왕좌를 넘겨주고 사라졌다. 하지만, 이 '고정된 크기로 자른다'는 발상의 단순성은 현대에도 하드웨어 캐시(Cache Line), 네트워크 패킷 스위칭(MTU), 블록 스토리지 등 오버헤드를 극도로 혐오하는 모든 [초고속](/knowledge-base/studynote/06_ict_convergence/02_iot_mobility/148_5g_embb_urllc_mmtc/) 하드웨어 설계의 밑바탕에 진리로 살아 숨 쉬고 있다.

- **📢 섹션 요약 비유**: 뷔페 식당을 운영할 때, 손님이 얼만큼 먹을지 매번 재서 담아주기 귀찮으니까 그냥 똑같은 크기의 식판에 일괄 배식(고정 분할)하여 버려지는 짬타이거([내부 단편화](/knowledge-base/studynote/02_operating_system/06_memory_management/341_internal_fragmentation/))를 감수하더라도 배식 속도를 빛처럼 빠르게 만든 시스템입니다.
...
| 개념 | 연결 포인트 |
|:---|:---|
| [표준 스와핑](/knowledge-base/studynote/02_operating_system/06_memory_management/337_standard_vs_paging_swapping/) ([전체 프로세스](/knowledge-base/studynote/02_operating_system/06_memory_management/337_standard_vs_paging_swapping/)) vs [페이징](/knowledge-base/studynote/02_operating_system/04_synchronization/259_paging/) 시스템 [스와핑](/knowledge-base/studynote/02_operating_system/06_memory_management/335_swapping/) ([페이지](/knowledge-base/studynote/01_computer_architecture/07_virtual_memory_os_integration/286_page_frame/) 단위) | 현재 개념으로 들어오기 전에 함께 이해하면 경계가 선명해지는 기반 개념이다. |
| [연속 메모리 할당](/knowledge-base/studynote/02_operating_system/06_memory_management/338_contiguous_memory_allocation/) ([Contiguous Memory Allocation](/knowledge-base/studynote/02_operating_system/06_memory_management/338_contiguous_memory_allocation/)) | 현재 개념이 등장하게 만든 직접적인 선행 흐름이다. |
| [가변 분할 방식](/knowledge-base/studynote/02_operating_system/06_memory_management/340_variable_partition/) ([Variable Partition](/knowledge-base/studynote/02_operating_system/06_memory_management/340_variable_partition/)) | 현재 개념이 구현·세분화될 때 바로 연결되는 후속 개념이다. |
| [내부 단편화](/knowledge-base/studynote/02_operating_system/06_memory_management/341_internal_fragmentation/) ([Internal Fragmentation](/knowledge-base/studynote/02_operating_system/06_memory_management/341_internal_fragmentation/)) | 확장 학습이나 심화 비교로 이어지는 다음 단계의 키워드다. |
...
1. 고정 분할 방식 (Fixed [Partition](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/))은 컴퓨터가 메모리를 방처럼 나눠 쓰고 주소를 찾는 방법이에요.
2. 먼저 [연속 메모리 할당](/knowledge-base/studynote/02_operating_system/06_memory_management/338_contiguous_memory_allocation/) ([Contiguous Memory Allocation](/knowledge-base/studynote/02_operating_system/06_memory_management/338_contiguous_memory_allocation/))을 이해하면 고정 분할 방식 (Fixed [Partition](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/))이 왜 필요한지 더 쉽게 보여요.
3. 그래서 고정 분할 방식 (Fixed [Partition](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/))을 잘 알면 나중에 [가변 분할 방식](/knowledge-base/studynote/02_operating_system/06_memory_management/340_variable_partition/) ([Variable Partition](/knowledge-base/studynote/02_operating_system/06_memory_management/340_variable_partition/))도 훨씬 쉽게 배울 수 있어요.
...
← **이전**: [338. 연속 메모리 할당 (Contiguous Memory Allocation)](/knowledge-base/studynote/02_operating_system/06_memory_management/338_contiguous_memory_allocation/)
**다음**: [340. 가변 분할 방식 (Variable Partition)](/knowledge-base/studynote/02_operating_system/06_memory_management/340_variable_partition/) →
