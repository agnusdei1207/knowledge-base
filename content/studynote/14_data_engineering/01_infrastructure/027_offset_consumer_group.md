...
- **📢 섹션 요약 비유**: [컨슈머 그룹](/knowledge-base/studynote/07_enterprise_systems/03_eai_esb_msa/191_consumer_group_kafka_partition_load_balancing/) [파티션](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/) 할당은 바리스타 배치다. 커피 머신([파티션](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/)) 3대에 바리스타(컨슈머) 2명이면 한 명이 2대를 담당한다. 바리스타가 4명이면 1명은 할 일이 없다.

---

## Ⅲ. 비교 및 연결

| 비교 | [Kafka](/knowledge-base/studynote/14_data_engineering/04_mlops/179_kafka_flink_watermark_time_window/) 오프셋 | RabbitMQ ACK | 전통 MQ |
|:---|:---|:---|:---|
| 소비 기록 | 컨슈머 측 오프셋 | 브로커 ACK | 브로커 메시지 삭제 |
| 재처리 | 오프셋 리셋 | 재전송 요청 | 불가 |
| 멀티 소비 | 그룹별 독립 오프셋 | 큐 방식 1회 | 불가 |

- **📢 섹션 요약 비유**: [Kafka](/knowledge-base/studynote/14_data_engineering/04_mlops/179_kafka_flink_watermark_time_window/) 오프셋은 스트리밍 [서비스](/knowledge-base/studynote/13_cloud_architecture/02_iaas_paas_saas/090_service_kubernetes_network_load_balancing/) 시청 기록이다. Netflix가 내 시청 위치를 기억해서 어디서든 이어보기가 가능하다. 전통 TV(전통 MQ)는 방송이 지나가면 다시 볼 수 없다.
...
| 기대효과 | 내용 |
|:---|:---|
| **독립 소비** | 다수 [컨슈머 그룹](/knowledge-base/studynote/07_enterprise_systems/03_eai_esb_msa/191_consumer_group_kafka_partition_load_balancing/)이 같은 토픽 독립 소비 |
| **재처리 가능** | 오프셋 리셋으로 과거 메시지 재소비 |
| **수평 확장** | [파티션](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/) 수에 비례한 컨슈머 [병렬](/knowledge-base/studynote/05_database/07_exam_summary/430_index_fast_full_scan/) 확장 |

[Kafka](/knowledge-base/studynote/14_data_engineering/04_mlops/179_kafka_flink_watermark_time_window/) 3.x에서 도입된 KRaft 모드는 [ZooKeeper](/knowledge-base/studynote/02_operating_system/11_exam_summary/798_distributed_lock_zookeeper_consensus/) 없이 [Kafka](/knowledge-base/studynote/14_data_engineering/04_mlops/179_kafka_flink_watermark_time_window/) 자체가 [메타데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/012_metadata/)·오프셋을 관리하며, [컨슈머 그룹](/knowledge-base/studynote/07_enterprise_systems/03_eai_esb_msa/191_consumer_group_kafka_partition_load_balancing/) 재조정 알고리즘이 더 효율적으로 개선되었다.

- **📢 섹션 요약 비유**: KRaft는 도시 교통 관제 시스템이 외부 컨트롤 타워([ZooKeeper](/knowledge-base/studynote/02_operating_system/11_exam_summary/798_distributed_lock_zookeeper_consensus/)) 없이 자체 AI로 신호를 제어하는 것이다. 더 빠르고 안정적으로 [컨슈머 그룹](/knowledge-base/studynote/07_enterprise_systems/03_eai_esb_msa/191_consumer_group_kafka_partition_load_balancing/)을 관리한다.
...
| 개념 | 연결 포인트 |
|:---|:---|
| <strong><a href="/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/">파티션</a></strong> | 오프셋이 관리되는 단위 [로그](/knowledge-base/studynote/04_software_engineering/09_cloud_native_ai_architecture/568_logs_distributed_logging_elk_fluentd/) |
| <strong><a href="/knowledge-base/studynote/16_bigdata/04_streaming/089_consumer_lag/">Consumer Lag</a></strong> | Committed Offset 대비 최신 오프셋 차이 |
| **Rebalancing** | 컨슈머 추가·제거 시 [파티션](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/) 재할당 |
| **Exactly-Once** | [트랜잭션](/knowledge-base/studynote/05_database/04_transactions_concurrency/191_transaction_concept_states/)+멱등 프로듀서의 의미 보장 |
| **KRaft** | [ZooKeeper](/knowledge-base/studynote/02_operating_system/11_exam_summary/798_distributed_lock_zookeeper_consensus/) 없는 자체 [메타데이터 관리](/knowledge-base/studynote/16_bigdata/10_governance/203_metadata_management/) |
...
1. [Kafka](/knowledge-base/studynote/14_data_engineering/04_mlops/179_kafka_flink_watermark_time_window/) 오프셋은 책 북마크예요! 여러 독자([컨슈머 그룹](/knowledge-base/studynote/07_enterprise_systems/03_eai_esb_msa/191_consumer_group_kafka_partition_load_balancing/))가 같은 책(토픽)을 각자 북마크를 꽂아서 독립적으로 읽어요.
2. [컨슈머 그룹](/knowledge-base/studynote/07_enterprise_systems/03_eai_esb_msa/191_consumer_group_kafka_partition_load_balancing/)은 바리스타 팀이에요 — 커피 머신([파티션](/knowledge-base/studynote/02_operating_system/09_file_system/514_partition_slice_volume/)) 개수만큼 바리스타를 배정해서 빠르게 주문(메시지)을 처리해요!
3. 처리 속도가 느리면 대기 줄([Consumer Lag](/knowledge-base/studynote/16_bigdata/04_streaming/089_consumer_lag/))이 길어지니 바리스타(컨슈머)를 더 추가해야 해요!
