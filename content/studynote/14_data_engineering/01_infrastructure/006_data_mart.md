...
# [데이터 마트](/knowledge-base/studynote/14_data_engineering/05_exam_keywords/209_data_mart_kimball_star_schema/) ([Data Mart](/knowledge-base/studynote/14_data_engineering/05_exam_keywords/209_data_mart_kimball_star_schema/))

#### 핵심 인사이트 (3줄 요약)
> 1. **본질**: [데이터 웨어하우스](/knowledge-base/studynote/12_it_management/05_security_compliance/209_data_warehouse_schema_on_write/) ([DW](/knowledge-base/studynote/12_it_management/05_security_compliance/209_data_warehouse_schema_on_write/))의 하위 집합으로, 전사 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 중 특정 부서(영업, 재무 등)나 비즈니스 도메인의 요구사항에 맞게 구조화된 소규모 분석용 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)베이스입니다.
> 2. **가치**: 방대한 전사 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)에서 필요한 정보만 추출·가공해 제공함으로써 분석 [쿼리](/knowledge-base/studynote/10_ai/04_ai_ops_ethics/298_qkv_attention/) 응답 시간을 단축하고, 현업(Business User)의 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 접근성과 BI (Business Intelligence) 도구 활용의 직관성을 극대화합니다.
> 3. **융합**: 상향식 ([Bottom-up](/knowledge-base/studynote/04_software_engineering/12_testing_maintenance/403_bottom_up_integration/))인 킴볼 ([Kimball](/knowledge-base/studynote/12_it_management/05_security_compliance/954_kimball/)) 아키텍처의 핵심 요소이며, 최근에는 [클라우드 네이티브](/knowledge-base/studynote/04_software_engineering/11_testing_validation/923_cloud_native_architecture/) [DW](/knowledge-base/studynote/12_it_management/05_security_compliance/209_data_warehouse_schema_on_write/) 내에서 물리적 복제를 줄이는 가상 [데이터 마트](/knowledge-base/studynote/14_data_engineering/05_exam_keywords/209_data_mart_kimball_star_schema/)(Virtual [Data Mart](/knowledge-base/studynote/14_data_engineering/05_exam_keywords/209_data_mart_kimball_star_schema/)) 형태로 융합 발전하고 있습니다.
...
| <strong><a href="/knowledge-base/studynote/12_it_management/05_security_compliance/956_surrogate_key/">Surrogate Key</a> (대리 키)</strong> | 테이블 간 조인 속도 향상 및 이력 격리 | 원본 시스템의 비즈니스 키 대신 독립적인 정수(Integer) 시퀀스를 발급하여 매핑 | 주민번호 대신 쓰는 사내 사원번호 |
...
[스타 스키마 (Star Schema) 아키텍처 흐름]
       ...
       - Region, City, Manager
```
이 구조도의 핵심은 조인([Join](/knowledge-base/studynote/05_database/04_transactions_concurrency/521_join/))의 깊이가 항상 '1단계'로 고정된다는 점입니다. [정규화](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/093_normalization/)된 DB에서는 카테고리 이름을 찾기 위해 여러 테이블을 거쳐야 하지만, [스타 스키마](/knowledge-base/studynote/05_database/06_dw_olap_trends/334_star_schema/)에서는 `Dim_Product` 하나에 모든 텍스트 정보가 중복을 감수하고 들어가 있습니다. 분석 [쿼리](/knowledge-base/studynote/10_ai/04_ai_ops_ethics/298_qkv_attention/)는 항상 `차원 필터링(Where) -> 팩트 조인(Join) -> 집계(Sum)`의 단순한 [실행 계획](/knowledge-base/studynote/05_database/03_relational_model/166_execution_plan_optimizer_navigation_tree/)([Execution Plan](/knowledge-base/studynote/05_database/03_relational_model/166_execution_plan_optimizer_navigation_tree/))을 가지게 되며, 이는 RDBMS 옵티마이저가 가장 빠르고 효율적으로 처리할 수 있는 패턴입니다. [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 중복으로 인한 스토리지 비용 상승보다, 조인 제거로 얻는 CPU 연산 속도 향상이 압도적으로 크기 때문에 채택됩니다.

> 📢 **섹션 비유**: [팩트 테이블](/knowledge-base/studynote/14_data_engineering/05_exam_keywords/210_fact_dimension_table_snowflake_schema/)이 '매출액'이라는 요리의 '메인 식재료'라면, [차원 테이블](/knowledge-base/studynote/07_enterprise_systems/05_data_bi/273_dimension_table_analysis_perspective/)은 '언제, 어디서, 누가'라는 요리의 맛을 다채롭게 만드는 '다양한 양념 세트'입니다. 이 구조를 미리 세팅해 두면 주문([쿼리](/knowledge-base/studynote/10_ai/04_ai_ops_ethics/298_qkv_attention/)) 즉시 완제품을 낼 수 있습니다.
...
<strong>1. <a href="/knowledge-base/studynote/14_data_engineering/05_exam_keywords/209_data_mart_kimball_star_schema/">데이터 마트</a> vs <a href="/knowledge-base/studynote/12_it_management/05_security_compliance/209_data_warehouse_schema_on_write/">데이터 웨어하우스</a> (<a href="/knowledge-base/studynote/12_it_management/05_security_compliance/954_kimball/">Kimball</a> vs <a href="/knowledge-base/studynote/12_it_management/05_security_compliance/953_inmon/">Inmon</a>)</strong>

| 비교 항목 | [데이터 웨어하우스](/knowledge-base/studynote/12_it_management/05_security_compliance/209_data_warehouse_schema_on_write/) ([Inmon](/knowledge-base/studynote/12_it_management/05_security_compliance/953_inmon/) 모델) | [데이터 마트](/knowledge-base/studynote/14_data_engineering/05_exam_keywords/209_data_mart_kimball_star_schema/) ([Kimball](/knowledge-base/studynote/12_it_management/05_security_compliance/954_kimball/) 모델) | 아키텍처 판단 포인트 |
|:---|:---|:---|:---|
...
| **구축 방식** | 하향식 (전사 모델링 완료 후 배포, 수년 소요) | 상향식 (부서별 마트를 먼저 짓고 점진적 통합) | [초기](/knowledge-base/studynote/03_network/08_transport_layer/459_quic_fec_forward_error_correction/) 도입 속도와 투자 대비 효용 ([ROI](/knowledge-base/studynote/12_it_management/01_governance_strategy/807_roi_return_on_investment/)) |
...
[데이터 마트](/knowledge-base/studynote/14_data_engineering/05_exam_keywords/209_data_mart_kimball_star_schema/) 아키텍처는 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)의 비즈니스적 가치를 폭발시키는 라스트 마일(Last Mile) 딜리버리 시스템입니다.
...
### 👶 어린이를 위한 3줄 비유 설명
1. 도서관 전체([데이터 웨어하우스](/knowledge-base/studynote/12_it_management/05_security_compliance/209_data_warehouse_schema_on_write/))에는 수백만 권의 책이 섞여 있어서 원하는 내용을 찾기 너무 힘들어요.
2. 그래서 과학 숙제를 할 때는 과학 책만 모아둔 작은 '과학 전용 책장([데이터 마트](/knowledge-base/studynote/14_data_engineering/05_exam_keywords/209_data_mart_kimball_star_schema/))'을 만들면 훨씬 편해요.
3. 이렇게 부서마다 자기들이 필요한 정보만 예쁘게 모아둔 작고 빠른 맞춤형 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)베이스를 [데이터 마트](/knowledge-base/studynote/14_data_engineering/05_exam_keywords/209_data_mart_kimball_star_schema/)라고 부른답니다!
