...
> 3. 핫 스팟(Hot Spot) 문제 해결이 실제 구현의 핵심 과제 — 이론적 균등 [분산](/studynote/08_algorithm_stats/08_stats/136_variance/)과 달리 실제 [데이터](/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)의 접근 패턴이 불균등하여, 가상 노드 수 조정과 키 설계가 [분산](/studynote/08_algorithm_stats/08_stats/136_variance/) 시스템 성능을 결정한다.
...
> 📢 **섹션 요약 비유**: 전통 해싱 문제 = 사물함 번호 규칙 변경 — 사물함 10개일 때 학번 % [10](/studynote/02_operating_system/08_storage_and_io_systems/489_raid_10_hybrid/). 사물함 11개 추가 시 학번 % 11로 변경 -> 90%가 새 사물함으로 이사. 일관 해싱은 [10](/studynote/02_operating_system/08_storage_and_io_systems/489_raid_10_hybrid/)%만 이사!
...
> 📢 **섹션 요약 비유**: 가상 노드 = 순환 근무 배치 — 직원 3명(A,B,C)이 12시간 중 8시간씩 겹치면 불균등. 대신 4교대(가상 노드)로 나눠 배치하면 균등하게 [분산](/studynote/08_algorithm_stats/08_stats/136_variance/)!
...
> 📢 **섹션 요약 비유**: [Redis](/studynote/05_database/04_transactions_concurrency/542_redis/) Cluster = 16384 사물함 링 — 16384개 슬롯을 노드에 나눠서. 노드 추가 시 일부 슬롯만 이동(온라인 리샤딩). 전체 [데이터](/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 이사 없이 확장!
...
> 📢 **섹션 요약 비유**: [Redis](/studynote/05_database/04_transactions_concurrency/542_redis/) 무중단 확장 = 달리는 기차에 칸 추가 — 일반 해싱이면 기차 세우고 승객([데이터](/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)) 전원 재배치. 일관 해싱으로 달리면서 일부 승객만 새 칸으로. 무중단!
...
1. 전통 해싱 문제 = 사물함 번호 규칙 변경 — 사물함 [10](/studynote/02_operating_system/08_storage_and_io_systems/489_raid_10_hybrid/)->11개로 늘릴 때 거의 모든 학생이 새 사물함으로 이사. 일관 해싱은 [10](/studynote/02_operating_system/08_storage_and_io_systems/489_raid_10_hybrid/)%만!
2. 링 구조 = 원형 시계 사물함 — 학생이 자기 번호에서 시계 방향 첫 사물함 이용. 새 사물함 추가 시 그 구간만 이사!
3. 가상 노드 = 4교대 균등 배치 — 사물함 3개가 불균등한 위치라면 4개씩 복사 배치. 어디서나 고르게 [분산](/studynote/08_algorithm_stats/08_stats/136_variance/)!
...
<- **이전**: [047. 컴팩션과 툼스톤 — Compaction & Tombstone](/studynote/14_data_engineering/01_infrastructure/047_compaction_and_tombstone/)
**다음**: [049. 데이터 메시 — Data Mesh Distributed Ownership](/studynote/14_data_engineering/01_infrastructure/049_data_mesh_distributed_ownership/) ->
