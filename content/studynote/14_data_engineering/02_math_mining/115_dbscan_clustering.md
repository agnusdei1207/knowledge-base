...
> 3. **판단 포인트**: 두 파라미터 <strong>ε(epsilon, 반경)</strong>과 <strong>MinPts(최소 이웃 수)</strong>가 결과를 결정하며, ε이 너무 크면 모든 [데이터](/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/)가 1개 클러스터, 너무 작으면 모두 노이즈가 되는 민감성이 있다.
...
### [DBSCAN](/studynote/06_ict_convergence/05_data_science/351_dbscan_density_based_clustering/) vs K-Means

| 비교 | K-Means | [DBSCAN](/studynote/06_ict_convergence/05_data_science/351_dbscan_density_based_clustering/) |
|:---|:---|:---|
| **K 지정** | 필수 | **자동 결정** |
...
| 비교 | K-Means | [DBSCAN](/studynote/06_ict_convergence/05_data_science/351_dbscan_density_based_clustering/) | HDBSCAN |
|:---|:---|:---|:---|
| **K 지정** | 필수 | 불필요 | 불필요 |
...
### ε·MinPts [설정](/studynote/15_devops_sre/01_culture_methodology/009_config/) 가이드
- **ε**: k-distance 그래프의 "팔꿈치(elbow)" 지점.
- **MinPts**: 일반적으로 `2 × 차원 수`. 2D -> MinPts=4.

### 활용 시나리오
1. **지리적 클러스터링**: GPS 좌표로 상점 밀집 지역 탐지.
2. <strong><a href="/studynote/09_security/05_web_app_security/236_anomaly_based_detection_zero_day_false_positive/">이상 탐지</a></strong>: 네트워크 트래픽에서 정상 패턴 밖 접근 = 노이즈(공격).

---

## Ⅴ. 기대효과 및 결론

DBSCAN은 K-Means가 실패하는 <strong>비구형·노이즈 혼재 <a href="/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/">데이터</a></strong>에서 강력하며, HDBSCAN으로 확장하면 밀도 변화까지 대응 가능하다.
...
| **Noise** | 어떤 클러스터에도 속하지 않는 [이상치](/studynote/14_data_engineering/02_math_mining/076_outlier_detection_iqr_dbscan_isolation_forest/) |
...
<- **이전**: [114. 가우시안 혼합 모델 (GMM, Gaussian Mixture Model) - EM 알고리즘·소프트 클러스터링](/studynote/14_data_engineering/02_math_mining/114_gaussian_mixture_model/)
**다음**: [116. 커널 밀도 추정 (KDE, Kernel Density Estimation) - 비모수 확률 밀도 추정](/studynote/14_data_engineering/02_math_mining/116_kernel_density_estimation/) ->
