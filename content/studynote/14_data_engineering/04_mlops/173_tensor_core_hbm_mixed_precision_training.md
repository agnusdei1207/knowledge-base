...
기술사 답안에서는 <strong>연산 가속기(Tensor Core) + 메모리 공급(HBM) + 수치 안정성(혼합 정밀도)</strong>의 3축을 함께 설명해야 완성도가 높다. 특히 "BF16이 왜 학습에 유리한가", "HBM 대역폭이 왜 Tensor Core 체감 성능을 좌우하는가"를 같이 써야 실무 판단이 된다.
