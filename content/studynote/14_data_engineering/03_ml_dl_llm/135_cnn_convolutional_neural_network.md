...
## 핵심 인사이트 (3줄 요약)
> 1. **본질**: CNN은 **[합성곱](/knowledge-base/studynote/10_ai/03_llm_nlp/228_cnn_1d_2d_3d_video_medical/)([Convolution](/knowledge-base/studynote/10_ai/04_ai_ops_ethics/284_convolution_stride_padding/)) 연산으로 입력의 지역적 패턴(엣지·텍스처·형태)을 계층적으로 추출**하는 신경망이며, 이미지·영상 처리의 핵심 아키텍처이다.
> 2. **가치**: 전결합층([FC](/knowledge-base/studynote/01_computer_architecture/15_advanced_topics/696_fibre_channel_protocol/))은 이미지를 1D로 펼쳐 공간 정보를 잃지만, CNN은 **2D 구조를 유지하며 파라미터를 공유([가중치](/knowledge-base/studynote/10_ai/03_llm_nlp/267_weight_bias_activation/) 공유)**하여 효율적으로 학습한다.
...
| **Pool** | 다운샘플링 |
| **[ResNet](/knowledge-base/studynote/10_ai/04_ai_ops_ethics/287_resnet_skip_connection/)** | Skip Connection |
...
3. 사진 인식·얼굴 인식 등 **눈(비전) 관련 [AI](/knowledge-base/studynote/04_software_engineering/03_design_architecture/190_ai_llm_requirements_specification/)**의 핵심이에요!
