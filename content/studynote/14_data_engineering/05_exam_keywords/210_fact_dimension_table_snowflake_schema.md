...
**팩트의 [분류](/knowledge-base/studynote/16_bigdata/05_analysis/104_classification_analysis/):**
...
[차원 테이블](/knowledge-base/studynote/07_enterprise_systems/05_data_bi/273_dimension_table_analysis_perspective/)은 팩트를 **해석하기 위한 맥락**을 제공하는 테이블이다.

- **DimDate**: 날짜, 월, 분기, 연도, 공휴일 여부 등
- **DimCustomer**: 고객 이름, 주소, 세그먼트, 등록일 등
- **DimProduct**: 제품명, 카테고리, 브랜드, 가격 등

📢 **섹션 요약 비유**: 팩트 테이블은 **가계부의 금액**이고, [차원 테이블](/knowledge-base/studynote/07_enterprise_systems/05_data_bi/273_dimension_table_analysis_perspective/)은 **그 지출의 날짜·장소·카테고리**다. 금액만 있으면 아무 의미가 없고, 맥락이 있어야 분석이 된다.
...
|:---|:---|:---|
| **차원 [정규화](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/093_normalization/)** | 비정규화(Denormalized) | [정규화](/knowledge-base/studynote/01_computer_architecture/02_data_representation_arithmetic/093_normalization/)(Normalized) |
| **조인 횟수** | 적음 (팩트↔차원 1단계) | 많음 (다단계 조인) |
| **[쿼리](/knowledge-base/studynote/10_ai/04_ai_ops_ethics/298_qkv_attention/) [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/)** | 빠름 | 느릴 수 있음 |
...
📢 **섹션 요약 비유**: [SCD](/knowledge-base/studynote/07_enterprise_systems/05_data_bi/277_scd_slowly_changing_dimension_modeling/) Type 2는 **여권 갱신 기록**과 같다. 새 여권을 발급받아도 이전 여권 정보(주소, 사진)가 기록 안에 남아 있다.
...
📢 **섹션 요약 비유**: [서로게이트 키](/knowledge-base/studynote/07_enterprise_systems/05_data_bi/276_surrogate_key_artificial_identifier/)는 **주민등록번호와 달리 회사가 직접 발행하는 사원증 번호**다. 외부 번호가 바뀌어도 내부 번호는 유지된다.
...
2. **[서로게이트 키](/knowledge-base/studynote/07_enterprise_systems/05_data_bi/276_surrogate_key_artificial_identifier/)만 FK로 사용**: 자연 키 직접 [참조](/knowledge-base/studynote/05_database/05_distributed_nosql_newsql/316_reference_pattern_nosql/) 금지
...
1. **[SCD Type 2](/knowledge-base/studynote/12_it_management/05_security_compliance/315_scd_type_2/) 선택 기준**: 고객 세그먼트 변경, 지역 재편, 제품 카테고리 변경 시 필수
...
📢 **섹션 요약 비유**: [SCD Type 2](/knowledge-base/studynote/12_it_management/05_security_compliance/315_scd_type_2/) 설계는 **이사 때마다 주소록 새 줄을 추가**하는 것이다. 덮어쓰면 예전 주소로 보낸 편지 이력이 사라지지만, 새 줄을 추가하면 언제 어디 살았는지 다 남는다.
...
팩트 테이블과 [차원 테이블](/knowledge-base/studynote/07_enterprise_systems/05_data_bi/273_dimension_table_analysis_perspective/)의 [관계](/knowledge-base/studynote/05_database/02_modeling_normalization/083_relationship_in_er_model/), 그리고 스타/[스노우플레이크 스키마](/knowledge-base/studynote/05_database/06_dw_olap_trends/335_snowflake_schema/) 선택은 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 웨어하우스의 **분석 [성능](/knowledge-base/studynote/04_software_engineering/05_devops_ci_cd/282_performance_tactics/)과 유지보수성의 균형**을 결정한다. [SCD](/knowledge-base/studynote/07_enterprise_systems/05_data_bi/277_scd_slowly_changing_dimension_modeling/) [전략](/knowledge-base/studynote/04_software_engineering/04_testing_quality/268_strategy_pattern/)은 단순한 기술 선택이 아니라 비즈니스의 이력 분석 요건을 [데이터](/knowledge-base/studynote/05_database/01_db_architecture_relational/001_dikw_pyramid/) 모델로 구현하는 핵심 의사결정이다. 기술사 논술에서는 각 선택의 **비즈니스 근거**를 명확히 제시할 수 있어야 한다.
...
```
2. [차원 테이블](/knowledge-base/studynote/07_enterprise_systems/05_data_bi/273_dimension_table_analysis_perspective/)은 **장난감 설명서**야. 그 장난감이 뭔지, 누가 샀는지, 언제 샀는지 자세히 설명해줘.
