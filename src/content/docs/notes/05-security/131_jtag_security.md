...
date: "2026-08-05T12:43:00+09:00"
tags:
  - notes-security
weight: 131
extra:
...
- 정의/개념: **JTAG 보안** 은 장치 수명주기 상태와 작업자 권한에 따라 TAP의 시험•디버그 명령 및 자원 접근을 제한하는 **하드웨어 접근 통제**
...
선의 의미: 장치 수명주기 정책 아래 TAP 인터페이스와 작업자•장치 인증을 분리하고, 명령•자원 권한 게이트 및 감사•재잠금 경계에 결합한 정적 디버그 통제 구조
...
sequenceDiagram
  ...
  participant J as JTAG 접근 통제
  R->>J: 장치•작업•시간 요청
  J->>J: 1. 수명 상태•작업자 자격 검증
  J->>J: 2. 허용 명령•자원•만료 시각 설정
  J->>J: 3. 실행 명령•결과•작업자 기록
  J->>J: 4. 세션 폐기•재잠금
  J-->>R: 작업 종료 상태
```
...
- **1. 수명 상태•작업자 자격 검증**: 인증서•RMA•제품 상태 판정
- **2. 허용 명령•자원•만료 시각 설정**: 읽기•쓰기•코어 범위 제한
- **3. 실행 명령•결과•작업자 기록**: 디버그 행위의 감사 증적 보존
- **4. 세션 폐기•재잠금**: 토큰•임시 키 제거와 포트 잠금
