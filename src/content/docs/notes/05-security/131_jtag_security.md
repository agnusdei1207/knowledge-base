...
- **정의/개념**: 칩 시험·디버그 기능을 수명주기별 통제
- **배경/필요성**: 무보호 포트는 펌웨어·비밀 추출 경로
...
- 일률적 차단보다 단계별 최소 권한 통제 필요하다.
- 핀·테스트 패드 탐색으로 CPU·메모리 접근한다.
- 잠금 상태·인증키를 보안 부팅과 연동한다.
- 수리 시 시간·장치·명령 범위를 제한한다.
...
| 비교축 | 개방형 디버그 | 영구 비활성화 | 인증형 디버그 |
|:---|:---|:---|:---|
| 핵심 특징 | 별도 인증 없이 기능 허용 | 출고 뒤 디버그 기능을 완전히 차단 | 신원·장치·명령·시간을 검증해 제한 개방 |
| 적용 기준 | 격리된 개발·시험 장치 | 수리·현장 분석이 불필요한 장치 | 제조·수리와 운영 보안을 함께 유지할 때 |
| 주요 위험 | 펌웨어·키 추출과 장치 장악 | 장애 분석·복구 불가 | 인증키·잠금 해제 구현이 새 공격면이 됨 |
