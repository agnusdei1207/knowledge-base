...
- 배경/필요성: 무서명 전자문서는 **승인 주체•변경 여부 확인할 수 없다**.
...
- **서명 생성**은 문서 다이제스트를 개인키로 처리하여 서명값을 만드는 연산이다.
- **서명 검증**은 공개키로 서명값과 문서 다이제스트의 관계를 확인하는 연산이다.
- **무결성**은 서명한 뒤 문서가 변경되지 않았음을 확인하는 성질이다.
- **인증**은 검증된 공개키의 소유자가 서명했음을 확인하는 성질이다.
- **부인방지**는 신원•키 통제•시각 증거를 결합하여 서명자가 서명 사실을 사후에 부정하기 어렵게 한다.
- **정규화**는 같은 의미의 문서가 항상 같은 바이트열이 되도록 인코딩•필드 순서•공백 규칙을 고정한다.

</details>

- **개인키 생성•공개키 검증**의 비대칭성이 핵심이다.
- **무결성•인증•부인방지** 제공과 원문 기밀성 미제공이 핵심이다.
- **문서 정규화** 차이에 따른 동일 원문 검증 실패가 핵심이다.
...
- **하드웨어 보안 모듈(Hardware Security Module, HSM)**은 개인키를 외부로 노출하지 않고 승인된 문서 다이제스트의 서명값을 생성한다.
- **공개키 인증서**는 공개키•소유자•용도•유효기간을 인증기관의 서명으로 연결한다.
- **문서 정규화기**는 문서를 서명 대상인 동일한 바이트열로 확정한다.
- **서명 검증기**는 문서 해시와 서명값 및 신뢰 경로를 판정한다.
...
가지의 의미: 문서 확정•서명•신뢰 제공•검증 책임을 나눈 구조다.
...
1. **권한•서명 문맥 확인**: 승인 주체와 대상•용도를 대조한다.
2. **서명값 생성**: HSM 내부 개인키로 다이제스트를 서명한다.
3. **인증서 상태 검증**: 신뢰 경로•유효기간•폐지를 확인한다.
4. **다이제스트•서명 검증**: 정규 문서 해시와 서명값을 판정한다.
...
- **확률적 서명 방식(Probabilistic Signature Scheme, PSS)**은 RSA에 무작위 솔트와 확률적 패딩을 적용하는 방식이다.
- **타원곡선 전자서명 알고리즘(Elliptic Curve Digital Signature Algorithm, ECDSA)**은 작은 키에서 높은 보안 강도를 제공하는 서명 방식이다.
- **모듈 격자 기반 전자서명 알고리즘(Module-Lattice-Based Digital Signature Algorithm, ML-DSA)**은 모듈 격자 문제를 기반으로 양자 공격에 대응하는 서명 방식이다.
...
- **연방 정보 처리 표준(Federal Information Processing Standards, FIPS) 186-5**는 RSA•ECDSA•EdDSA의 서명 생성과 검증을 규정한다.
- **FIPS 204**는 양자 내성 모듈 격자 전자서명인 ML-DSA를 규정한다.
- **타임스탬프**는 특정 전자 데이터가 해당 시각에 존재했음을 신뢰기관의 서명으로 증명한다.
- **승인 산출물 통제**는 검증된 결과물에만 HSM의 서명 권한을 허용하는 대책이다.
...
- 기존 호환은 **RSA-PSS•ECDSA**, 양자 대응은 **ML-DSA**을 선택한다.
