...
- **문서 정규화기(Document Canonicalizer)**: 이종 시스템 간 서명 대조 시 동일 바이트열을 보장하는 정규화 모듈.
- **서명 검증기(Signature Verifier)**: 문서 해시값, 서명값, 공개키 신뢰 경로(PKI Chain)를 종합 판정하는 연산 엔진.
- **인증서•신뢰 저장소(Certificate Trust Store)**: CA 루트 인증서 및 폐기 정보(CRL/OCSP)를 안전하게 관리하는 저장소.
...
| HSM 서명 모듈 | **HSM** 기반 개인키 비반출 서명 생성 |
| 인증서•신뢰 저장소 | **인증서 주체•유효 상태** 제공 |
| 서명 검증기 | **서명•다이제스트•신뢰 경로** 검증 |
