...
- 정의/개념: 송신자 개인키로 생성하고 공개키로 검증하는 **전자서명** 메커니즘
- 배경/필요성: 무서명 전자문서 전송 시 발생하는 **승인 주체•변경 여부 확인 한계** 극복

#### 한줄 요약

- 비대칭키 암호화 기반으로 무결성, 인증, 부인방지를 제공하며, 신뢰된 PKI 인증서 연동을 통한 서명자 신원 증명 필수
...
| 구성요소 | 책임 |
|:---|:---|
| 문서 정규화기 | 문서 바이트열 및 서명 문맥 확정 |
| HSM 서명 모듈 | HSM 기반 개인키 반출 없는 서명 생성 |
| 인증서•신뢰 저장소 | 공개키 인증서 주체 및 유효 상태 제공 |
| 서명 검증기 | 서명, 다이제스트 및 신뢰 경로 검증 |


#### 한줄 요약

- 문서 정규화, HSM 기반 개인키 격리 연산, PKI 신뢰 저장소 및 서명 검증기로 구성된 통합 서명 구조
...
1. **권한•서명 문맥 확인**: **서명 문맥** 연계 및 권한 유효성 검증
2. **서명값 생성**: HSM 내부 개인키 기반의 고속 **서명값 생성**
3. **인증서 상태 검증**: **인증서 폐기 검증**을 포함한 PKI 신뢰 체계 대조
4. **다이제스트•서명 검증**: 정규화 다이제스트 대조를 통한 **다이제스트•서명 검증** 완결


#### 한줄 요약

- 서명 문맥 및 권한 검증, HSM 서명 생성, PKI 기반 인증서 폐기 상태(CRL/OCSP) 대조 및 다이제스트 수학적 검증 수행
...
- 시스템 환경에 따라 기존 호환성(RSA-PSS), 경량화(ECDSA), 양자 내성(ML-DSA) 알고리즘을 선택 적용
...
- FIPS 186-5/204 표준 준수, HSM 승인 산출물 서명 통제 및 TSA 타임스탬프 결합을 통한 장기 부인방지 체계 구축
...
- **장기 서명 증거** 보존 체계 구축 및 시스템 목적별 적합한 **전자서명 방식 선택** 가이드라인 적용

#### 한줄 요약

- 서명값의 수학적 검증을 넘어 PKI 신뢰 체인, HSM 키 보호, TSA 타임스탬프 기반 장기 서명 증거(LTV) 보존 체계 구축 필수
