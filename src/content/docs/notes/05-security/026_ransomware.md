...
- 정의/개념: **랜섬웨어**는 데이터 암호화•유출과 서비스 방해를 이용해 피해 조직에 금전을 요구하는 **악성코드 기반 갈취 침해 활동**이다.
- 배경/필요성: 파일 백업만으로는 탈취 신원과 **유출 피해 해소할 수 없다**.
...
- **초기 접근**은 취약점•피싱•탈취 계정으로 내부 환경에 진입하는 단계다.
- **권한 상승**은 침해 계정의 권한을 관리자 수준으로 높이는 단계다.
- **측면 이동**은 확보한 권한으로 다른 내부 자원에 확산하는 단계다.
- **명령제어(Command and Control, C2)**는 공격자가 침해 장비에 원격 명령을 전달하고 실행 결과를 받는 통신이다.

</details>

- **초기 접근•C2 유지•권한 상승•측면 이동**으로 관리•백업 권한을 장악한다.
...
- **불변 백업**은 보존 기간 동안 수정•삭제할 수 없는 백업이다.
...
- **침해 지표(Indicator of Compromise, IOC)**는 악성 파일•주소•계정 행위 등 침해를 식별하는 관측 흔적이다.
...
1. **랜섬웨어 사건 확인**: 암호화•유출•C2 **IOC**을 판별한다.
2. **확산 격리•자격 교체**: C2 차단과 관리자 비밀을 폐기한다.
3. **신뢰 복구 실행**: 깨끗한 관리면과 백업을 선택한다.
...
- **암호화 갈취**는 데이터 잠금과 복호화 대가로 금전을 요구한다.
- **이중 갈취**는 데이터 잠금에 유출 공개 협박을 결합한다.
- **다중 갈취**는 서비스 방해와 고객 압박까지 결합한다.
...
- **기관 간 보고서(Interagency Report, IR) 8374 Rev. 1**은 CSF 2.0 기반 랜섬웨어 위험 관리 성과를 제시한다.
- **특별 간행물(Special Publication, SP) 800-61 Rev. 3**은 탐지•대응•복구를 위험 관리에 통합하는 지침이다.
- **사이버보안 프레임워크(Cybersecurity Framework, CSF) 2.0**은 사이버 위험 관리를 여섯 기능으로 구성한다.
...
- **격리 후 신뢰 복원**은 C2와 탈취 계정을 먼저 차단하고 검증된 백업을 복원한 뒤 유출 증거를 통지•수사에 연결하는 순서다.

</details>

- C2•계정 격리 후 **신뢰 백업 복원**, 유출 증거는 **통지•수사** 연계가 핵심이다.
