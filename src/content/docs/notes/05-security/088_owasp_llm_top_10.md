...
- 정의/개념: OWASP LLM Top 10 2025는 생성형 AI 및 LLM 애플리케이션을 개발, 배포, 운영할 때 직면하는 10대 최우선 취약점(Prompt Injection, Sensitive Data Leakage, Excessive Agency 등)을 체계적으로 분류하고 방어 통제를 제시하는 대표적 글로벌 보안 기준이다.
- 배경/필요성: SQLi, XSS 등 전통적 웹 보안 분류 체계만으로는 LLM의 자연어 기반 제어/데이터 혼동, 탈옥, RAG 벡터 오염, 과도한 에이전트 자율권 위험을 평가 차단하기 어렵기 때문이다.
...
- **실체 공격•잔여 위험 평가(Empirical Attack & Residual Risk Assessment)**: AI 레드팀 평가를 가해 남은 취약점을 검증하는 단계이다.
...
1. **자산•신뢰 경계 확정**: AI 시스템의 데이터, 프롬프트, 에이전트 도구 간의 보안 신뢰 경계를 확정한다.
2. **LLM01~LLM10 시나리오 매핑**: OWASP 10대 위험 항목에 기초해 위협 침투 시나리오를 매핑한다.
3. **예방•탐지•복구 통제 설계**: 가드레일, PEP 게이트웨이, 샌드박스 다층 통제를 구현한다.
4. **실제 공격•잔여 위험 평가**: AI 레드팀 패징 평가를 통해 **잔여 위험** 수치와 가드레일 모니터링을 이행한다.
5. **우선순위•통제 갱신**: 신종 탈옥/인젝션 시그니처를 지속 업데이트하고 통제를 정교화한다.
...
- **위험 분류의 활용 원칙**을 바탕으로 OWASP LLM Top 10 2025 항목별 런타임 가드레일을 구축하고 **레드팀** 평가를 상시 이행한다.
