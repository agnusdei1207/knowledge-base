...
- **OWASP LLM Top 10 2025**는 모델•RAG•에이전트 응용의 주요 위험 열 가지를 분류한 공식 목록이다.
- **LLM 응용 위험**: 기존 웹 입력•권한 외에도 자연어 지시•검색 문맥•모델 출력•도구 행동•추론 비용에서 발생하는 위험이다.

</details>

- 정의/개념: **OWASP LLM Top 10 2025**는 LLM 응용의 주요 10대 위험과 완화 방향을 정리한 공식 목록이다.
- 배경/필요성: 기존 웹 보안 점검만으로는 **모델•RAG•도구 경계의 고유 위험을 충분히 포괄하기 어려움**이 발생한다.
...
- 입력부터 자원까지 **전 경계 위험 분류**가 핵심이다.
- 번호•연도의 **판본 식별성**이 핵심이다.
- 위협 모델•공격 시험의 **서비스별 적용**이 핵심이다.
...
- **LLM01**은 프롬프트 인젝션 위험을 다루는 항목이다.
- **LLM02**는 민감정보 노출 위험을 다루는 항목이다.
- **LLM03**은 소프트웨어 공급망 위험을 다루는 항목이다.
- **LLM04**는 데이터와 모델 오염 위험을 다루는 항목이다.
- **LLM05**는 부적절한 출력 처리 위험을 다루는 항목이다.
- **LLM06**은 과도한 대리권 위험을 다루는 항목이다.
- **LLM07**은 시스템 프롬프트 누출 위험을 다루는 항목이다.
- **LLM08**은 벡터와 임베딩 취약점 위험을 다루는 항목이다.
- **LLM09**는 허위정보 위험을 다루는 항목이다.
- **LLM10**은 무제한 자원 소비 위험을 다루는 항목이다.
...
2. **LLM01~LLM10 시나리오 매핑**: 공격 경로•업무 영향을 연결한다.
3. **예방•탐지•복구 통제 설계**: 위험별 다층 통제를 배치한다.
4. **실제 공격•잔여 위험 평가**: 재현 시험•통제 공백을 측정한다.
...
- **LLM 응용 경계**는 지시•검색 문맥•출력의 신뢰를 검증하는 경계다.
- **웹 응용 경계**는 웹 요청의 입력•인증•인가•세션 처리를 검증하는 경계다.
...
- **레드팀**은 공격자 관점에서 서비스의 실제 공격 경로와 통제 실패를 재현하는 평가다.
- **회귀 시험**은 통제 수정 뒤 동일•변형 공격이 다시 성공하는지 반복 확인하는 시험이다.
...
- 자산 가치•행동 권한이 큰 **공격 경로부터 통제•레드팀 적용**이 핵심이다.
