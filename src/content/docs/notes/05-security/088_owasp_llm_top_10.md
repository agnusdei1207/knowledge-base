...
- 배경/필요성: 생성형 AI, 거대 언어 모델(LLM), RAG 벡터 지식베이스 및 자율 에이전트(Agentic AI)가 급속히 확산됨에 따라, 기존의 결정론적 웹 보안 취약점(SQLi, XSS)과는 완전히 다른 자연어 기반 프롬프트 인젝션, 시스템 프롬프트 유출, 벡터 임베딩 결함, 에이전트 과도한 권한(Excessive Agency), 비대칭 자원 고갈 등 신종 복합 위협이 폭발적으로 증가함에 따라, 글로벌 오픈소스 보안 기구인 OWASP의 2025 개정 표준에 입각하여 10대 핵심 보안 위험(LLM01~LLM10)과 5대 통제 도메인(입출력, 정보지시, 공급학습, 검색행동, 판단자원)을 체계화한 AI 보안 프레임워크를 도입하여 생성형 AI 전 생애주기 공격 표면(Attack Surface)의 정밀 식별, 다층 심층 방어(Defense in Depth) 구축 및 신뢰 가능한 AI 거버넌스를 달성할 필요

#### 한줄 요약
- 생성형 AI와 RAG 및 에이전트 생태계의 **10대 핵심 위험**을 정의하고 5대 도메인 다층 방어 체계를 수립한다.
...
```text
[OWASP LLM Top 10 다층 방어 체계]
├─ 입출력 신뢰 경계 (LLM01, LLM05)
│  ├─ XML 태깅 분리 및 Guardrails
│  └─ 출력 이스케이프 및 샌드박싱
├─ 정보지시 권한 경계 (LLM02, LLM07)
│  ├─ PII 자동 마스킹 및 DLP 연동
│  └─ 시스템 프롬프트 은닉 가드레일
├─ 공급학습 무결 경계 (LLM03, LLM04)
│  ├─ Safetensors 가중치 서명 검증
│  └─ DVC 데이터 계보 및 수집 격리소
├─ 검색행동 인가 경계 (LLM06, LLM08)
│  ├─ 에이전트 최소 권한 및 HITL 승인
│  └─ RAG 벡터 지식베이스 전용 ACL
└─ 판단자원 가용 경계 (LLM09, LLM10)
   ├─ 문서 인용 팩트체크 교차 검증
   └─ TPM 가중 쿼터 및 회로 차단기
```
...
| 구성요소 | 책임 |
|:---|:---|
| 입출력 신뢰 경계 (LLM01, LLM05) | XML 태그 지침-데이터 분리, NeMo Guardrails, 출력 이스케이프 샌드박스 |
| 정보지시 권한 경계 (LLM02, LLM07) | 출력단 PII 마스킹 필터, 시스템 프롬프트 유출 방어 룰셋, DLP 연동 |
| 공급학습 무결 경계 (LLM03, LLM04) | Safetensors 전자서명 검증, DVC 데이터 계보 추적, 수집 격리 버퍼 |
| 검색행동 인가 경계 (LLM06, LLM08) | 에이전트 최소 권한 IAM, Human-in-the-loop 승인, RAG 벡터 청크별 ACL |
| 판단자원 가용 경계 (LLM09, LLM10) | RAG 문서 인용(Grounding) 팩트체킹, TPM 가중 쿼터, 런타임 회로 차단기 |
...
#### 한줄 요약
- 목록 매핑은 누락을 줄이는 값싼 출발점일 뿐 실제 노출은 조직마다 다르므로, 레드팀 검증과 회귀 평가로 목록을 자기 시스템의 증거로 바꾸는 단계가 실효를 가른다.
...
- 자연어 인터페이스와 자율 에이전트 중심의 현대 생성형 AI 환경에서 필수적으로 준수해야 하는 글로벌 인공지능 애플리케이션 보안 표준(OWASP Top 10 for LLM: 2025 / NIST AI RMF 매핑)의 최상위 위협 모델링 기준으로 확고히 자리 잡았으며, 다중 에이전트 협업 및 실시간 AI 방화벽 인프라로 전면 진화하는 가운데, 실무 엔터프라이즈 AI 서비스 개발 및 운영 시에는 LLM01/LLM07 대응을 위한 XML 태깅 기반 지침 격리 및 NeMo Guardrails 입출력 검증, LLM08 대응을 위한 RAG 벡터 청크별 IAM ACL 결속, LLM06 대응을 위한 독립 정책 집행점(PEP) 및 파괴적 명령 시 Human-in-the-loop 강제, LLM10 대응을 위한 TPM 가중 쿼터 및 런타임 회로 차단기(Circuit Breaker)를 통합 구축하여 완벽한 생성형 AI 전주기 보안 무결성을 완성
