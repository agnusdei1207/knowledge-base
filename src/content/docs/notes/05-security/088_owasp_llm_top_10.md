...
- 정의: LLM 응용의 **10대 위험·완화 지침**
- 기존 한계: 웹 보안만으로 **모델·RAG·도구 위험 누락**
...
| 절차 | 설명 |
|:---|:---|
| 자산·신뢰 경계 확정 | 모델·데이터·검색·도구 식별 |
| LLM01~LLM10 시나리오 매핑 | 공격 경로·업무 영향 연결 |
| 예방·탐지·복구 통제 구현 | 위험별 다층 통제 배치 |
| 실제 공격·잔여 위험 평가 | 재현 시험·통제 공백 측정 |
| 우선순위·통제 갱신 | 구조·판본 변화 반영 |

> 요약: 서비스 경계에 10대 위험을 매핑해 검증한다.
