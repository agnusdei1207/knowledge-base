...
- **거대 언어 모델(Large Language Model, LLM) 응용 경계**: 지시•검색 문맥•출력의 신뢰를 검증하는 경계이다.
- **웹 응용 경계(Web Application Boundary)**: 웹 요청의 입력•인증•인가•세션 처리를 검증하는 경계이다.
...
|:---|:---|:---|
| **웹 응용 경계** | **하이퍼텍스트 전송 프로토콜(Hypertext Transfer Protocol, HTTP) 입력•인증•권한•세션** | 주입•인가 누락•세션 탈취 |
| **거대 언어 모델(Large Language Model, LLM) 응용 경계** | **지시•검색 증강 생성(Retrieval-Augmented Generation, RAG) 문맥•출력** | 프롬프트 인젝션•정보 노출•허위정보 |
