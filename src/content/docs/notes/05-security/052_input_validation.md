---
sidebar:
  order: 52
  label: "052. 입력값 검증·파라미터 바인딩 (Input Validation Parameter Binding)"
  badge:
    text: "기출 · 50%"
    variant: note
title: "입력값 검증·파라미터 바인딩 (Input Validation Parameter Binding)"
date: "2026-07-25T21:52:00+09:00"
tags:
  - "notes-security"
weight: 52
extra:
  question_no: "052"
  source_status: "기출"
  source_history: "123회"
  priority: 50
  priority_note: "123회 기출이며 입력 경계와 SQL 방어에 재사용됨"
---

## 미리 알고가기

- **입력값 검증(Input Validation)**: [읽기: 입력값 검증; 표기 이유: 한글명 뒤 영문 원어 병기] 외부 값이 허용된 형식·범위인지 판정하는 통제
- **파라미터 바인딩(Parameter Binding)**: [읽기: 파라미터 바인딩; 표기 이유: 한글명 뒤 영문 원어 병기] 실행 구문과 외부 값을 분리해 전달하는 방식
- **허용 목록(Allowlist)**: [읽기: 허용 목록; 표기 이유: 한글명 뒤 영문 원어 병기] 미리 승인한 값이나 형식만 통과시키는 기준
- **정규화(Normalization)**: [읽기: 정규화; 표기 이유: 한글명 뒤 영문 원어 병기] 여러 표현을 하나의 표준 표현으로 바꾸는 처리
- **파서(Parser)**: [읽기: 파서; 표기 이유: 한글명 뒤 영문 원어 병기] 입력 문자열을 정해진 문법 구조로 해석하는 구성요소
- **SQL(Structured Query Language)**: [읽기: 에스큐엘; 표기 이유: 영문 머리글자] 관계형 데이터베이스를 조회·조작하는 언어
- **ORM(Object-Relational Mapping)**: [읽기: 오알엠; 표기 이유: 영문 머리글자] 객체 연산을 데이터베이스 질의로 변환하는 기술
- **동적 식별자**: [읽기: 동적 식별자; 표기 이유: 한국어 통용어 표기] 실행 중 결정되는 테이블명·열 이름·정렬 기준
- **원시 질의(Raw Query)**: [읽기: 원시 질의; 표기 이유: 한글명 뒤 영문 원어 병기] ORM의 안전한 추상화를 거치지 않고 직접 작성한 질의
- **구조화 API(Application Programming Interface)**: [읽기: 에이피아이; 표기 이유: 영문 머리글자] 명령과 값을 구분해 받는 호출 규격
- **마스킹(Masking)**: [읽기: 마스킹; 표기 이유: 한글명 뒤 영문 원어 병기] 로그의 민감한 값을 알아볼 수 없게 가리는 처리


- **흐름 기호(↓·→·X)**: [읽기: 흐름 기호; 표기 이유: 한글명 뒤 영문 원어 병기] 아래로·다음으로·차단으로 읽으며 순서·분기·금지를 나타냄
## Ⅰ. 개요

- **정의/개념**: 허용 규칙으로 외부 값을 판정
- **배경/필요성**: 구문과 값을 분리해 주입 차단
- **목적**: 부적합 입력과 구문 변조를 함께 방지

### 쉽게 이해하기 (학습용)

- 입력값 검증은 외부 값을 업무 계약에 맞는 데이터로 제한해 구문·자원·권한 공격의 출발점을 줄인다.

## Ⅱ. 특징

- 서버가 모든 입력의 최종 통과 여부를 판정한다.
- 허용 목록으로 기대한 값의 경계를 명시한다.
- 바인딩으로 외부 값을 데이터로만 해석한다.
- 동적 식별자는 승인 값에 코드로 매핑한다.

### 쉽게 이해하기 (학습용)

- 허용 목록과 길이·형식·범위 검사는 입력을 제한하지만 출력 인코딩이나 매개변수화 질의를 대신하지 않는다.

## Ⅲ. 구성요소 및 구조

```mermaid
flowchart LR
    A[입력 계약] --> B[단일 파서]
    B --> C[서버 검증기]
    C --> D[안전 실행 API]
    D --> E[오류·감사]
```

| 설계 요소 | 설명 |
|:---|:---|
| 입력 계약 | 형식·길이·범위 규칙 정의 |
| 단일 파서 | 인코딩과 콘텐츠 형식 통일 |
| 서버 검증기 | 허용 목록과 업무 규칙 판정 |
| 안전 실행 API | 구문과 외부 값 분리 전달 |
| 오류·감사 | 일반 오류 응답과 차단 기록 |

### 쉽게 이해하기 (학습용)

- 계약 스키마가 구조와 크기를 제한하고 정규화·업무 검증을 통과한 값만 안전한 처리기에 전달한다.

## Ⅳ. 원리 및 절차 흐름도

```mermaid
sequenceDiagram
    participant C as 클라이언트
    participant P as 단일 파서
    participant V as 서버 검증기
    participant A as 안전 실행 API
    participant D as DB
    C->>P: 입력 해석 요청
    P->>V: 표준 표현 전달
    V->>A: 검증 값 전달
    A->>D: 바인딩 질의 실행
    D-->>A: 실행 결과 반환
```

| 절차 | 설명 |
|:---|:---|
| 입력 해석 요청 | 형식·크기 제한 후 해석 |
| 표준 표현 전달 | 인코딩과 표현을 하나로 통일 |
| 검증 값 전달 | 허용 규칙과 권한 통과 |
| 바인딩 질의 실행 | 고정 구문에 값을 별도 결합 |
| 실행 결과 반환 | 일반 응답과 감사 기록 생성 |

> 요약: 검증된 값만 고정 구문에 결합한다.

### 쉽게 이해하기 (학습용)

- 서버는 입력을 일관된 형태로 정규화하고 계약과 업무 규칙을 확인한 뒤 고정 구문에 값으로 결합한다.

## Ⅴ. 종류 및 비교

| 비교축 | 입력 검증 | 정규화 | 파라미터 바인딩 |
|:---|:---|:---|:---|
| 핵심 특징 | 값의 적합성 판정 | 표현 방식 통일 | 구문과 값 분리 |
| 적용 기준 | 업무 허용 범위 확인 | 중복 표현 제거 | 질의 값 안전 전달 |
| 주요 위험 | 주입 단독 차단 불가 | 의미가 바뀔 수 있음 | 식별자는 분리 불가 |

### 쉽게 이해하기 (학습용)

- 입력값 검증과 대안의 선택 기준을 비교함

## Ⅵ. 실무 사례

- **사례 1**: 배열 깊이·개수·크기를 계약으로 제한
- **사례 2**: 원시 질의를 바인딩 질의로 교체
- **사례 3**: 정렬 기준을 허용 목록에 매핑
- **사례 4**: 거부 로그의 비밀값을 마스킹

### 쉽게 이해하기 (학습용)

- 사례 1은 중첩 깊이와 항목 수를 제한해 파서와 메모리 고갈을 막는다.
- 사례 2는 입력을 SQL 구문과 분리해 데이터 값으로만 처리한다.
- 사례 3은 요청한 정렬명을 승인된 열 이름에만 매핑한다.
- 사례 4는 거부 원인은 남기되 비밀번호와 토큰은 로그에서 가린다.

## Ⅶ. 결론

- 외부 입력이면 검증 후 바인딩해 실행한다.

### 쉽게 이해하기 (학습용)

- 입력값 검증은 계약 제한과 문맥별 안전 처리, 자원 제한을 함께 적용해야 한다.
