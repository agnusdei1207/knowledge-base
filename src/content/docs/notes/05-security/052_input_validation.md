...
- 정의: 입력의 **허용 범위 판정·구문값 분리**
- 기존 한계: 차단 목록의 **우회 표현·직접 호출**
...
| 절차 | 설명 |
|:---|:---|
| 형식·크기 제한 후 파싱 | 계약 위반 입력을 조기 거부 |
| 단일 정규화·표준 표현 | 인코딩·중복 표현을 통일 |
| 허용목록·업무 규칙 검증 | 값·권한·상태를 판정 |
| 고정 구문·값 바인딩 | 데이터의 명령 해석을 차단 |
| 일반 오류·감사 기록 | 내부 정보 은닉·차단 추적 |
