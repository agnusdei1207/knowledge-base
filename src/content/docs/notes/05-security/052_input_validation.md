...
- 정의/개념: 외부 입력을 허용 범위로 제한하고 실행 구문과 분리하는 **입력 보안 통제**이다.
- 배경/필요성: 차단 목록만으로는 변형 입력의 **검증 우회 차단하기 어렵다**.
...
- **단일 정규화 후 서버 허용 목록**을 통한 형식•길이•범위를 판정한다.
- **파라미터 바인딩**을 통한 외부 값의 데이터를 처리한다.
...
1. **정규화 입력**: 인코딩•중복 표현을 하나의 표준 표현으로 변환한다.
2. **허용된 업무 값 검증**: 형식•길이•범위•권한•상태를 확인한다.
3. **고정 SQL 구조 생성**: 외부 값과 분리된 실행 명령 형식을 고정한다.
4. **입력값 파라미터 바인딩**: 외부 입력을 명령이 아닌 값으로 결합한다.
5. **최소 권한 질의 실행**: 허용된 스키마•연산 범위에서 처리한다.
...
| **입력 검증** | **업무 허용 범위•형식 판정** | 출력 문맥별 인코딩을 별도로 적용 |
| **파라미터 바인딩** | **실행 구문과 값을 분리** | 동적 식별자는 허용 목록으로 제한 |
...
- 값은 **허용 목록 검증•바인딩**, 식별자는 코드 매핑, 입력은 깊이•크기를 제한한다.
