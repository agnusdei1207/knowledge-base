...
- 정의/개념: 다계층 신호를 사건화해 조사•조치하는 **XDR 통합 대응 체계**이다.
- 배경/필요성: 분산 경보만으로는 **도메인 간 공격 경로 연결할 수 없다**.
...
- 사용자•자산•세션의 **공통 엔티티로 텔레메트리 연결**이 핵심이다.
- 다계층 신호의 **공격 사건 상관분석**이 핵심이다.
- 도메인 간 **통합 조사•대응**이 핵심이다.
...
4. **도메인 간 대응 조정**: 단말•신원•메일•망 조치를 함께 편성한다.
...
- 단말 단독은 **EDR**, 통신 관측은 **NDR**, 다계층 공격 경로는 **XDR**을 선택한다.
