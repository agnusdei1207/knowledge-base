...
- 정의/개념: 계층별 통제의 수행•증명을 나누는 **공유 책임** **거버넌스 원칙**이다.
- 배경/필요성: 제공자 전담 오해로 **통제 책임 공백**이 발생한다.
...
- **계층별 통제 소유권**: 제공자•고객의 수행 책임 지정이다.
- **책임 경계 이동**: IaaS•PaaS•SaaS별 고객 관리 범위 변화이다.
- **책임 이행 증명**: 계약•설정•로그로 수행 결과를 확인한다.
...
- **1. 제공자 통제•증적 추출**: 하부 계층 보호 범위와 근거를 구성한다.
- **2. 고객 통제•소유자 정의**: 계정•데이터•설정의 수행 책임 지정이다.
- **3. 서비스 계층•책임 경계 매핑**: 제공자•고객•공동 통제를 분류한다.
- **4. RACI•책임 검증 계획 수립**: 승인•협의•증거 확인 방법 지정이다.
- **5. 증적 검증•책임 공백 개선**: 책임표•설정•사고 협업 절차를 갱신한다.
#### 한줄 요약
...
- **IaaS•PaaS•SaaS**는 OS•앱•계정부터 각각 **고객 책임**으로 지정이 핵심이다.
