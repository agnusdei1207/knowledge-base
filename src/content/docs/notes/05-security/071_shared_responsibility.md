...
date: "2026-07-31T02:04:43+09:00"
tags:
  - "notes-security"
weight: 71
extra:
...
- **통제 소유권(Control Ownership)**: 통제를 수행하고 결과를 증명할 책임 주체의 지정이다.
- **운영 증적(Operational Evidence)**: 설정·로그·감사 보고서처럼 통제 수행을 입증하는 자료이다.
- **사고 인터페이스**: 사고 탐지·통보·조사·복구 때 양측이 협업하는 접점과 절차이다.
- **책임 공백**: 제공자와 고객 어느 쪽에도 통제 수행 책임이 지정되지 않은 상태이다.

## Ⅰ. 개요

- 정의/개념: 클라우드 계층별 제공자·고객 통제를 나누는 **보안 거버넌스 원칙**
...
sequenceDiagram
    ...
    C->>G: 3. 고객 통제·소유자 전달
    G->>A: 4. RACI·검증 계획 전달
    A-->>G: 5. 공백·사고 개선 결과 환류
```

**동작 원리**

- **1. 계약·책임 자료 요청**: 서비스 모델·SLA·제외 범위 요구
- **2. 제공자 통제·증적 전달**: 하부 계층 보호 근거 제공
- **3. 고객 통제·소유자 전달**: 계정·데이터·설정 책임 제공
- **4. RACI·검증 계획 전달**: 수행·승인·증거 확인 방법 제공
- **5. 공백·사고 개선 결과 환류**: 책임표·설정·협업 절차 갱신
### 쉽게 이해하기 (학습용)
