...
```text
[ICT 위험관리]---[ICT 사고관리•보고]---[운영 복원력 시험]
       |                                          |
[ICT 제3자 위험관리]------------------[사이버위협 정보공유]
```

선의 의미: ICT 위험•사고•시험과 제3자 위험•위협정보 공유 영역이 하나의 DORA 운영 복원력 체계로 결합되는 관계를 나타낸다.
...
sequenceDiagram
  ...
  B->>B: 1. 적용 범위•위험 허용수준 확정
  B->>R: 범위•허용수준 전달
  R->>R: 2. 중요 기능•ICT 의존성 분석
  R->>O: 분석 결과 전달
  O->>O: 3. 계약 통제•종료 조건 설정
  O->>P: 계약 요구 전달
  O->>O: 4. 중대 사고•TLPT 증거 구성
  O->>A: 감독 증거 제출
  A->>A: 5. 운영 회복탄력성 적합성 평가
  A->>B: 감독 권고•개선 요구 전달
```

**동작 원리**

1. **적용 범위•위험 허용수준 확정**: 법인 책임과 허용 가능한 ICT 위험 확정
2. **중요 기능•ICT 의존성 분석**: 자산•공급자•집중 위험 분석
3. **계약 통제•종료 조건 설정**: 보안•복구•감사권•대체수단 조건 설정
4. **중대 사고•TLPT 증거 구성**: 사고 보고와 중요 기능 시험 결과 구성
5. **운영 회복탄력성 적합성 평가**: 시험•감독 결함과 개선 필요성 판정
