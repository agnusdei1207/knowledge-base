...
- 배경/필요성: 자산•취약점 목록만으로는 업무 손실 기반 **보호 우선순위 산정하기 어렵다**.
...
- 가능성•결과•수용기준의 **사전 정의**가 핵심이다.
- 자산•위협•취약점의 **시나리오 연결**이 핵심이다.
- 정성•반정량•정량의 **자료 기반 선택**이 핵심이다.
...
4. **위험 수준•불확실성 산정**: 수용기준 비교와 처리 순위를 산정한다.
5. **우선순위•소유자 등록**: 재평가 조건•책임자를 기록한다.
...
- 자료 부족은 **정성**, 내부 순위는 **반정량**, 비용•효과는 ALE 정량평가가 핵심이다.
