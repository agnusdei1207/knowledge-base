...
- **정의/개념**: 가능성과 결과로 처리 순서를 결정
- **배경/필요성**: 제한 자원의 위험 기반 배분 기준
...
- 평가 방식은 자료 품질과 목적에 맞춰 선택한다.
- 변경·사고·위협정보를 계기로 재평가해 위험 등록부를 최신화한다.
...
| 주요 위험 | 평가자별 등급 편차 | 가중치가 정밀도를 과장 | 부족한 자료로 허위 정밀성 발생 |
...
- 자산 변경 탐지로 미평가 위험 축소
- ALE의 자료·가정·불확실성 기록
- 통제 후 잔여위험과 처리 효과 재산정

### 쉽게 이해하기 (학습용)

- 자산 변경을 탐지해 미평가 자산과 오래된 위험 기록을 줄임
- ALE는 자료 출처·가정·불확실성을 함께 남겨 계산값을 검증함
- 통제 적용 후 위험을 다시 계산해 잔여위험과 처리 효과를 확인함

## Ⅶ. 결론

- 일관된 기준과 시나리오로 위험 순서를 정함
