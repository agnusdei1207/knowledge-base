...
```text
[보호 단위•인접성] ----- [기여 한도•민감도] ----- [ε•δ 예산] ----- [노이즈 메커니즘]
                                                        |
                                               [프라이버시 회계]
```

선의 의미: 가로선은 보호할 개인 변화, 개인별 최대 영향, 허용 손실 예산과 결과 무작위화가 결합되는 정적 차등 프라이버시 구성이고, 아래 선은 프라이버시 회계가 ε•δ 예산의 누적 사용을 통제하는 관리 관계를 뜻한다.
