...
- 정의/개념: 개인 포함 여부의 영향을 제한하는 **확률적 프라이버시 모델**이다.
- 배경/필요성: 익명 통계의 반복 결합으로 **개인 참여 여부 추론 가능성이 있다.
...
> 요약: ε가 작을수록 **잡음 증가•프라이버시 강화**가 핵심이다.

- 인접 데이터 정의의 **실제 보호 범위**가 핵심이다.
- 기여 한도•예산 회계의 **누적 손실 통제**가 핵심이다.
- **후처리 불변성**으로 추가 분석에도 예산을 유지한다.
- **프라이버시 예산 축소**로 잡음 증가•정확도 저하된다.
...
1. **기여 한도•민감도 산정**: 개인 영향의 최대량을 계산한다.
2. **누적 예산 확인**: 반복 공개의 ε•δ 소비량을 대조한다.
3. **노이즈•공개 승인**: 오차 적용과 잔여 예산을 판정한다.
4. **노이즈 주입•예산 차감**: 무작위화 후 사용량을 기록한다.
...
- 신뢰 수집자는 **중앙 DP**, 수집자 불신은 **로컬 DP**을 선택한다.
