...
sequenceDiagram
    ...
    V->>L: 3. 서명 CRL 조회
    V->>O: 4. 서명 상태 요청
    O->>P: 5. 서명·시각·실패 정책 판정
```

| 절차 | 설명 |
|:---|:---|
| 서명 CRL 게시 | 폐지 일련번호·시각 배포 |
| 인증서 상태 동기화 | 응답자에 최신 폐지 상태 반영 |
| 서명 CRL 조회 | 캐시 목록에서 일련번호 확인 |
| 서명 상태 요청 | 발급자·일련번호 상태 질의 |
| 서명·시각·실패 정책 판정 | 출처·신선도·실패 처리 결정 |
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| CRL 형식·검증 | **IETF RFC 5280 적용** | 목록 상호운용성 확보 |
| OCSP 상태·신선도 | **IETF RFC 6960 검증** | 재전송·오판정 억제 |
...
- **목록 규모·신선도·실패 영향**으로 폐지 방식을 결정한다.
