...
| 구성요소 | 책임 |
|:---|:---|
| **CA 폐기 데이터베이스** | 인증서 일련번호·폐기 사유 관리 및 원장 유지 |
| **CRL 배포점 (CDP)** | 서명된 CRL 파일 주기적 배포 및 저장 (RFC 5280) |
| **OCSP 응답 서버** | 일련번호별 실시간 상태 응답 서명 발행 (RFC 6960) |
| **웹 서버 (Stapling)** | OCSP 응답 사전 캐시 및 TLS 핸드셰이크 동봉 |
| **클라이언트 검증기** | 응답 서명 검증 및 유효기간·신선도 대조 |
