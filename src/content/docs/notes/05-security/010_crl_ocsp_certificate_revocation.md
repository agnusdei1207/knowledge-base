...
- **정의/개념**: 만료 전 인증서의 조기 무효화를 검증
- **배경/필요성**: 키 유출·오발급 인증서의 즉시 신뢰 철회
...
| 판단 기준 | CRL | OCSP | OCSP 스테이플링 |
|:---|:---|:---|:---|
| 핵심 특징 | 폐지 목록을 내려받아 검사 | 인증서별 상태를 온라인 질의 | 서버가 서명 응답을 첨부 |
| 적용 기준 | 대량 검증·폐쇄망 | 최신 개별 상태가 필요한 검증 | TLS의 조회 지연·노출 완화 |
| 주요 위험 | 목록 증가·배포 지연 | 응답자 장애·조회 정보 노출 | 서버의 응답 갱신 누락 |
...
2. 폐쇄망 검증자는 주기적 CRL을 내부 배포

### 쉽게 이해하기 (학습용)

- 공개 TLS 서버가 최신 OCSP 응답을 연결에 첨부하면 클라이언트가 인증기관에 직접 질의하지 않고도 폐지 상태를 확인할 수 있다.
- 외부 조회가 불가능한 폐쇄망은 서명된 CRL을 내부에 주기적으로 배포하고, 다음 갱신 시각을 넘긴 목록은 사용하지 않는다.

## Ⅶ. 결론

- 상태 신선도·조회 실패 영향으로 폐지 방식 선택
