...
- 정의/개념: 주요 웹 위험을 정리한 **인식•우선순위 분류 체계**이다.
- 배경/필요성: 개별 취약점만으로는 **우선순위 공유하기 어렵다**.
...
- 원인•영향 중심의 **웹 위험 공통 분류**가 핵심이다.
- 자료•전문가 검토 기반 **주기적 갱신**이 핵심이다.
- ASVS•WSTG 연결의 **요구•시험 구체화**가 핵심이다.
...
1. **Top 10 위험 범주 매핑**: 기능•의존성별 적용 위험을 식별한다.
2. **ASVS 보안 요구 선정**: 위험과 보호 수준에 맞는 요구를 구체화한다.
3. **보안 통제 구현**: 설계•코드에 검증 가능한 통제를 반영한다.
4. **WSTG 보안 시험**: 설계•코드•동적 시험으로 통제를 검증한다.
5. **사고•변경 근거 환류**: 운영 증거를 위험•요구 개선에 반영한다.
...
- 위험 인식은 **Top 10**, 요구 정의는 **ASVS**, 시험 절차는 **WSTG**을 선택한다.
