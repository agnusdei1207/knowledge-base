...
| 구성요소 | 책임 |
|:---|:---|
| 접근 구성 | 접근 통제·구성 오류 분류 |
| 공급 암호 | 공급망·암호화 실패 분류 |
| 입력 설계 | 인젝션·불안전 설계 분류 |
| 인증 무결 | 인증·무결성 실패 분류 |
| 운영 예외 | 로깅·경보·예외 처리 분류 |
...
1. **Top 10:2025 범위 선정**: 적용 판과 서비스 경계 정의
2. **기능·의존성 위험 매핑**: 공격 경로를 범주에 연결
3. **ASVS 5.0.0 요구 구현**: 검증 수준과 통제 항목 개발
4. **WSTG 기반 구현 검증**: 설계·코드·동적 시험 수행
5. **사고·변경 결과 환류**: 운영 근거로 요구 개선
