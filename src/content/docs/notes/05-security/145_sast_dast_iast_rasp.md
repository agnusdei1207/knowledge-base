...
- 배경/필요성: 소프트웨어 개발 생명주기(SDLC)에서 SAST 단독 사용 시 실행 컨텍스트 부재로 인한 수천 건의 오탐(False Positive)과 개발자 피로가 유발되고, DAST 단독 사용 시 취약점의 정확한 소스코드 라인을 특정할 수 없으며, 릴리스 후 런타임 제로데이 익스플로잇 공격에 무방비로 노출되는 기술적 사각지대가 발생함에 따라, OWASP ASVS 및 CWE 표준에 기반하여 소스코드 Taint 정적 분석(SAST), 외부 블랙박스 동적 퍼징(DAST), WAS 바이트코드 계측 글래스박스 분석(IAST), 그리고 운영 런타임 메모리 후킹 자가 방어(RASP)를 유기적으로 결합하는 애플리케이션 보안 파이프라인을 도입하여 **취약점 정확도 극대화, 오탐율 최소화 및 패치 전 제로데이 공격에 대한 실시간 런타임 복원력**을 달성할 필요
...
- 화이트박스 소스코드 검증부터 블랙박스 동적 테스트, 글래스박스 바이트코드 계측 및 인앱 런타임 자가 방어까지 전 구간을 유기적으로 연계하는 **애플리케이션 보안 테스팅 및 런타임 능동 방어(SAST / DAST / IAST / RASP)의 완벽한 다계층 심층 방어 아키텍처**로 확고히 자리 잡았으며, 클라우드 네이티브 환경의 eBPF 기반 관찰성 및 AI 코드 취약점 자동 교정(Auto-remediation)으로 진화하는 가운데, 실무 애플리케이션 보안 구축 시에는 **CI 단계의 도달 가능성(Reachability) 기반 SAST/SCA 오탐 튜닝, 자동화 QA 파이프라인 내 IAST 바이트코드 계측 연동, 프로덕션 WAS 내부 RASP AST 구문 트리 검증(성능 오버헤드 3% 이내)**을 결합하여 완벽한 소프트웨어 자가 방어 생태계를 완성
