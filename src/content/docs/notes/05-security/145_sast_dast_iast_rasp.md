...
- 정의: 코드·실행·운영의 **단계별 응용 보안 검증**
- 목적: 단일 시험의 **가시성·경로·시점 사각 감소**
...
| DAST 외부 동적 검증 | 실제 요청·응답 악용 확인 |
| RASP 운영 보호 | 실행 맥락 공격 탐지·완화 |
...
sequenceDiagram
  ...
  T->>S: 1. 자산·코드·구성요소 범위 설정
  S->>D: 2. 역할·인증·공격 경로 구성
  D->>R: 3. 도달성·악용성·영향 분석
  R->>T: 4. 코드·설정·의존성 근본 수정
  T->>O: 5. 회귀검증·운영 공격 관측
```

| 절차 | 설명 |
|:---|:---|
| 1. 자산·코드·구성요소 범위 설정 | 중요 기능·변경·위험 식별 |
| 2. 역할·인증·공격 경로 구성 | 규칙·계정·시험 데이터 준비 |
...
- **코드 가시성·악용성·운영 영향·오탐비용**으로 기법을 조합한다.
