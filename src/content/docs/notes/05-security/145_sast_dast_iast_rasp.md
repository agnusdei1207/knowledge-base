...
- 설계도 검사•외부 동작 검사•내부 센서•현장 방어를 함께 써 사각을 줄이다.
...
- 결과를 한곳에 모아 실제로 닿고 악용되며 중요한 결함부터 고침이 핵심이다.
...
- **1. 정적 코드•구성요소 분석**: **정적 코드•구성요소 분석**으로 후보를 찾는다.
- **2. 동적•상호작용 경로 검증**: **동적•상호작용 경로 검증**으로 악용성을 확인한다.
- **3. 도달 가능성•악용성•영향 분석**: **도달 가능성•악용성•영향 분석**으로 우선순위를 정한다.
- **4. 코드•설정•의존성 근본 수정**: **코드•설정•의존성 근본 수정**으로 원인을 제거한다.
- **5. 회귀검증•운영 공격 관측**: **보안 회귀검증**과 **회귀검증•운영 공격 관측**으로 잔여위험을 환류한다.

#### 한줄 요약

- 취약점 찾기에서 끝내지 않고 원인을 수정하고 같은 공격이 막혔는지 다시 확인하는 것이 핵심이다.
