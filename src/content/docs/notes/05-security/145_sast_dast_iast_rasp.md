...
- 정의/개념: 코드•실행•운영의 **단계별 응용 보안 검증**이다.
- 배경/필요성: 단일 시험만으로는 코드•실행•운영 관점의 **보안 사각을 충분히 줄이기 어려움**이 발생한다.
...
- SAST•DAST로 **내부 코드와 외부 공격면 상호 보완**이 핵심이다.
- IAST의 **요청•코드 실행 경로 연결**이 핵심이다.
- RASP의 **운영 맥락 기반 탐지•완화**가 핵심이다.
...
- **1. 정적 코드•구성요소 분석**: 코드 흐름•의존성 후보 탐지한다.
- **2. 동적•상호작용 경로 검증**: 요청•실행 경로•악용성을 확인한다.
- **3. 도달 가능성•악용성•영향 분석**: 중복•오탐•우선순위를 판정한다.
- **4. 코드•설정•의존성 근본 수정**: 원인 제거•보완 통제 설정한다.
- **5. 회귀검증•운영 공격 관측**: 수정 재현•잔여위험 환류이다.
...
- **상호 보완 검증**: 여러 시험 결과를 실제 경로•근본 수정•재시험으로 연결해 단일 기법의 사각을 줄이는 접근한다.
- **응용보안 기법 선택**: SAST는 코드, DAST•IAST는 경로, RASP는 운영에 적용한다.

</details>

- 초기 코드는 **SAST**, 경로 검증은 **DAST•IAST**, 운영 완화는 **RASP**을 적용한다.
