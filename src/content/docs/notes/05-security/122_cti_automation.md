...
- 정의/개념: CTI의 수집•정규화•평가•배포를 자동화하는 **위협정보 운영 체계**이다.
- 배경/필요성: 검증 없는 피드 자동 배포로 **중복•만료 지표의 오차단이 발생한다**.
...
- STIX 객체•관계 기반 **위협정보 구조화**가 핵심이다.
- TAXII 교환•TLP 표시 기반 **제한적 공유**가 핵심이다.
- 출처•신뢰도•시효•자산 기반 **품질 통제**가 핵심이다.
...
- 출처•시효•자산 관련성이 기준을 충족하면 **자동 배포**, 불충족하면 **보류•폐기**가 핵심이다.
