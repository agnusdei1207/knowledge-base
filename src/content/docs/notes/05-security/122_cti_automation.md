...
- 배경/필요성: 고도화된 공격 그룹(APT)의 인프라 변경 속도(수 분 단위)에 대응하여, 위협 정보의 수집부터 현장 방어선 적용까지의 레이턴시(Latency)를 최소화할 요구
...
```text
┌─────────────────────────────────────────────────────────────────────────┐
│ [ 1. 다중 소스 첩보 수집 계층 (Multi-source CTI Ingestion) ]            │
│  ├─ [ OSINT 피드 ] ➔ AlienVault OTX, AbuseIPDB, CISA KEV 수집          │
│  ├─ [ 상용 피드 ] ➔ 유료 CTI 벤더 API 연동 (실시간 제로데이 첩보)       │
│  └─ [ 국가 위협망 ] ➔ KISA C-TAS / 금융보안원 F-ISAC 위협 정보 연계    │
└────────────────────────────────────┬────────────────────────────────────┘
                                     │ (비표준 데이터 수집)
                                     ▼
┌─────────────────────────────────────────────────────────────────────────┐
│ [ 2. STIX 2.1 정규화 및 품질 평가 계층 (TIP Core & Quality Scoring) ]   │
│  ├─ [ 정규화 ] ➔ JSON 기반 STIX 2.1 SDO(객체) / SRO(관계) 변환          │
│  ├─ [ 화이트리스트 필터 ] ➔ 공용 DNS(8.8.8.8), 주요 포털 IP 오탐 제외   │
│  ├─ [ 신뢰도 스코어링 ] ➔ 출처 평판, 다중 피드 교차 검증 점수 산출      │
│  └─ [ 시효 만료(Aging) ] ➔ 30일 경과 및 비활성 IoC 자동 삭제(Decay)     │
└────────────────────────────────────┬────────────────────────────────────┘
                                     │ (검증된 고품질 STIX 번들 생성)
                                     ▼
┌─────────────────────────────────────────────────────────────────────────┐
│ [ 3. TAXII 2.1 보안 배송 및 TLP 통제 계층 (TAXII Distribution & TLP) ]   │
│  ├─ [ TLP 2.0 태깅 ] ➔ RED / AMBER / GREEN / CLEAR 공유 등급 부착      │
│  └─ [ TAXII 2.1 서버 ] ➔ HTTPS REST API 기반 채널별(Collection) 푸시    │
└────────────────────────────────────┬────────────────────────────────────┘
                                     │ (차단 룰셋 자동 배포)
                                     ▼
┌─────────────────────────────────────────────────────────────────────────┐
│ [ 4. 보안 장비 실시간 적용 계층 (Automated Enforcement & SOAR) ]        │
│  ├─ 차세대 방화벽(NGFW) / IPS ➔ 악성 IP/도메인 실시간 인바운드/아웃바운드 차단│
│  ├─ EDR / EPP ➔ 악성 파일 해시(SHA-256) 실행 차단 및 엔드포인트 격리   │
│  └─ [ SIEM / SOAR ➔ 실시간 상관분석 룰 갱신 및 자동화 플레이북 트리거 ] │
└─────────────────────────────────────────────────────────────────────────┘
```

선의 의미: 다중 CTI 피드를 수집하여 TIP에서 STIX 정규화 및 품질 검증을 거친 후, TAXII와 TLP를 통해 방화벽/EDR에 자동 배포하는 구조

| 컴포넌트 | 주요 역할 및 세부 기능 | 표준 규격 및 기술 | 비고 |
|:---|:---|:---|:---|
| **위협 정보 플랫폼 (TIP)**| 위협 피드 수집, 중복 제거, 정규화, 위협 분석, 지표 수명주기 총괄 관리 | MISP, ThreatConnect, OpenCTI | Core Engine |
| **STIX 2.1** | 위협 객체(Indicator, Malware, Attack Pattern 등)와 관계(Relationship)를 JSON 표현 | OASIS STIX v2.1 | Data Model |
| **TAXII 2.1** | STIX 위협 데이터를 HTTPS REST API를 통해 신뢰된 엔드포인트로 전송/공유 | OASIS TAXII v2.1 | Transport |
| **TLP 2.0** | 위협 정보의 외부 재공유 및 공개 범위를 4단계 색상 태그로 통제 | FIRST TLP v2.0 | Sharing Policy |
| **시효 만료 엔진 (Decay)**| 위협 지표의 시간 경과에 따른 신뢰도 감쇄 및 만료 지표 자동 제거 | Exponential Decay Model | Maintenance |

#### 한줄 요약
- TIP 플랫폼, STIX 2.1 데이터 모델, TAXII 2.1 전송 프로토콜, TLP 2.0 공유 정책, 시효 만료 엔진으로 구성된다.
...
#### 한줄 요약
- 피드 수집, STIX 정규화/필터링, 스코어링/시효 설정, TAXII/TLP 배포, 장비 적용 및 피드백 순으로 동작한다.
...
- 지능형 사이버 공격에 실시간으로 대응하는 능동형 방어 체계의 핵심 엔진인 **인텔리전스 기반 CTI 자동화 아키텍처**는 차세대 보안관제의 필수 패러다임이며, 실무 구현 시 **OASIS STIX 2.1 및 TAXII 2.1 국제 표준 규격 채택**, **TIP 중심의 엄격한 품질 스코어링 및 화이트리스트 필터링**, **동적 시효 만료(Decay) 모델을 통한 룰셋 수명주기 관리**, **FIRST TLP 2.0 기반의 신뢰된 정보 공유 거버넌스**를 통합 구축하여 최고 수준의 선제적 위협 차단력과 인프라 안정성을 완성
