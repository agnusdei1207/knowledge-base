...
- 배경/필요성: 브라우저가 요청 출처와 무관하게 도메인 쿠키를 자동 첨부하는 탓에 서버는 세션 보유 사실만으로 인증을 대신하고 사용자 의도를 판별할 근거를 잃으므로, 공격자가 읽을 수 없는 일회성 난수를 세션과 요청 양쪽에서 대조하는 **Anti-CSRF 토큰** 계층을 세션 검증 위에 얹어 의도를 대신 증명할 필요
