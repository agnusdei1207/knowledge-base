...
- 정의: 자동 인증정보를 악용한 **요청 의도 위조**
- 기존 한계: 세션 인증의 **사용자 의도 미검증**
...
sequenceDiagram
    ...
    participant 업무서비스
    participant 보안검증
    사용자->>브라우저: 1. 상태 변경 경로·메서드 식별
    브라우저->>웹서버: 2. 세션·CSRF 토큰 전송
    웹서버->>업무서비스: 3. 토큰·Origin·Referer 검증
    업무서비스->>브라우저: 4. SameSite·재인증 적용
    보안검증->>웹서버: 5. 예외 경로·우회 회귀검증
```

| 절차 | 설명 |
|:---|:---|
| 상태 변경 경로·메서드 식별 | 인증·업무 영향 범위를 분류 |
| 세션·CSRF 토큰 전송 | 요청별 의도 확인값을 결속 |
| 토큰·Origin·Referer 검증 | 서버에서 값·출처를 판정 |
| SameSite·재인증 적용 | 교차 전송·고위험 조치 제한 |
| 예외 경로·우회 회귀검증 | XSS·프록시·누락 경로 시험 |
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 사용자 의도 미검증 | **MITRE CWE-352 기준 제거** | 위조 상태 변경 차단 |
| 안전 메서드 오용 | **IETF RFC 9110 준수** | GET·HEAD 부작용 방지 |
| 토큰 우회·교차 쿠키 | **출처·SameSite·재인증 결합** | 단일 통제 우회 완화 |
...
- **상태변경·토큰결속·요청출처**로 CSRF 방어를 결정한다.
