...
1. **위조 상태 변경 요청 생성**: 공격자 페이지가 송금•계정 변경 요청 구성
2. **세션 쿠키 자동 첨부**: 브라우저가 대상 사이트 인증정보 포함
3. **세션 인증 통과**: 서버가 피해자의 유효한 로그인 상태 확인
4. **사용자 의도 검증 누락**: 토큰•Origin•재인증 없이 요청 수용
5. **비승인 상태 변경 실행**: 피해자 권한으로 공격자 지정 동작 수행
