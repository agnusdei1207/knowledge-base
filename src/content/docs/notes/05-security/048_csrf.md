...
| 구성요소 | 책임 |
|:---|:---|
| 세션 쿠키 | 자동 첨부된 인증 상태 제공 |
| 상태 변경 경로 | 자금·계정 변경 요청 처리 |
...
1. **상태 변경 경로·메서드 식별**: 인증·업무 영향 범위 분류
2. **세션·CSRF 토큰 전송**: 요청별 의도 확인값 결속
3. **토큰·Origin·Referer 검증**: 서버에서 값과 출처 판정
4. **SameSite·재인증 적용**: 교차 전송과 고위험 조치 제한
5. **예외 경로·우회 회귀검증**: XSS·프록시·누락 경로 시험
