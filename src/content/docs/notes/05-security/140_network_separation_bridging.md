...
선의 의미: 업무 승인과 세션 종단은 망 사이의 직접 연결을 차단하는 외부 경계이고, 중계 제어 아래에는 콘텐츠 검사, 무결성 전송 및 전 과정 감사가 놓이는 정적 망연계 내부 통제 계층을 뜻한다.
...
sequenceDiagram
  ...
  G->>G: 3. 대상•순서•해시 전송 자료 구성
  G->>D: 검사된 자료 전달
  D->>D: 4. 수신 순서•해시•무결성 검증
  D-->>G: 수신 확인•무결성 결과 반환
  G-->>S: 전송 결과
```
...
- **3. 대상•순서•해시 전송 자료 구성**: 오배송•변조•중복 방지 정보 생성
- **4. 수신 순서•해시•무결성 검증**: 수신 결과와 감사 증적 생성
