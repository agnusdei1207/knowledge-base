---
sidebar:
  order: 87
  label: "087. 모델 DoS (Model Denial of Service)"
  badge:
    text: "기출 · 70%"
    variant: note
title: "모델 DoS (Model Denial of Service)"
date: "2026-07-27T23:59:59+09:00"
tags:
  - "notes-security"
weight: 87
extra:
  question_no: "087"
  source_status: "기출"
  source_history: "138회"
  priority: 70
  priority_note: "138회 최신 기출이며 추론 자원 고갈 통제가 중요함"
---

## 미리 알고가기

- **서비스 거부(Denial of Service, DoS)**: 과도한 요청이나 연산으로 자원을 고갈시켜 서비스 가용성을 저하하는 공격이다.
- **거대 언어 모델(Large Language Model, LLM, 엘엘엠)**: 대규모 언어 데이터를 학습한 생성 모델
- **그래픽 처리 장치(Graphics Processing Unit, GPU, 지피유)**: 모델 추론 연산과 메모리를 제공하는 장치
- **응용 프로그래밍 인터페이스(Application Programming Interface, API, 에이피아이)**: 모델·도구 호출 규칙과 경계
- **분산 서비스 거부(Distributed Denial of Service, DDoS, 디도스)**: `D`는 분산을 뜻하며, 다수 출발지가 동시에 자원을 고갈시키는 공격
- **컨텍스트 윈도우 (Context Window)**: 모델 처리 입력 길이 범위
- **테넌트**: 같은 서비스 자원을 공유하면서 사용량·데이터·정책이 분리된 고객 단위
- **쿼터**: 사용자·테넌트가 일정 기간 사용할 수 있는 요청·토큰·비용 한도
- **작업 큐**: 실행 대기 요청을 순서와 우선순위에 따라 보관하는 대기열

## Ⅰ. 개요

- 정의/개념: 고비용 추론으로 모델 자원을 점유하는 공격이다
- 기존 한계: 요청 수 기준 제한은 요청별 추론 비용 차이 미반영
- **배경/필요성**: 토큰·시간·도구의 누적 비용 기반 통제

### 쉽게 이해하기 (학습용)

- 짧은 질문과 긴 문서 분석은 계산량이 다르므로 좌석과 시간을 함께 계산 필요

## Ⅱ. 특징

- 소수의 고비용 요청도 GPU·메모리·실행 슬롯을 고갈시킨다
- 긴 문맥·출력과 도구 순환이 연산·외부 API 비용을 누적한다
- 테넌트별 비용 예산과 공정 큐가 연쇄 고갈을 제한한다

### 쉽게 이해하기 (학습용)

- 모델이 검색과 도구를 반복하게 만들어 서비스 자원 소모

## Ⅲ. 아키텍처 및 구성요소

```mermaid
flowchart LR
    A[인증 게이트웨이] --> C[비용 추정기]
    C --> I[입장 제어기]
    I --> S[공정 스케줄러]
    S --> B[예산 차단기]
```

| 설계 요소 | 설명 |
|:---|:---|
| 인증 게이트웨이 | 사용자·키·테넌트 단위 소비 주체 식별 |
| 비용 추정기 | 입력 길이·모델·출력·도구 계획으로 요청 무게 계산 |
| 입장 제어기 | 쿼터와 현재 용량을 대조해 승인·대기·거부 결정 |
| 공정 스케줄러 | 테넌트별 큐와 가중치로 실행 슬롯 배분 |
| 예산 차단기 | 시간·토큰·도구·비용 한도에서 실행 중단 |

> 요약: 예상·누적 비용을 검사해 반복 실행을 차단한다

### 쉽게 이해하기 (학습용)

- 예상 비용과 실제 사용량을 재어 반복 자원을 차단함

## Ⅳ. 원리 및 절차 흐름도

```mermaid
sequenceDiagram
    participant U as 사용자
    participant G as 입장 제어기
    participant S as 공정 스케줄러
    participant B as 예산 차단기
    U->>G: 신원확인
    G->>G: 무게산정
    G->>S: 용량판단
    S->>B: 예산실행
    B-->>U: 종료축소
```

| 절차 | 설명 |
|:---|:---|
| 신원확인 | 사용자·API 키·테넌트 사용량 연결 |
| 무게산정 | 문맥·출력·도구 비용 예측 |
| 용량판단 | 용량·쿼터로 승인·대기·거부 판정 |
| 예산실행 | 사용량 누적치 확인 |
| 종료축소 | 한도 초과 시 취소·축소 |

> 요약: 요청 무게와 누적 비용으로 실행을 통제한다

### 쉽게 이해하기 (학습용)

- 예상 비용으로 입장을 정하고 한도 초과 시 중단·축소함

## Ⅴ. 종류 및 비교

| 판단 기준 | 대량 호출형 | 고비용 문맥형 | 에이전트 순환형 |
|:---|:---|:---|:---|
| 적용 기준 | 신원별 호출 한도 | 무게·토큰·시간 한도 | 단계·도구·누적 비용 한도 |
| 핵심 특징 | 많은 요청으로 입구·큐 점유 | 긴 입출력으로 GPU 자원 점유 | 도구 호출·재시도로 비용 누적 |
| 한계 | 분산 계정으로 한도 우회 | 정상 대형 작업 과도 거부 | 모델·API·작업 큐 동시 고갈 |

> 요약: 호출량·문맥·순환별로 서로 다른 자원을 보호한다

### 쉽게 이해하기 (학습용)

- 대량은 큐, 고비용은 GPU, 순환은 연결 도구를 고갈시킴

## Ⅵ. 실무 사례

1. 생성형 AI API의 **토큰·동시 요청 예산 제한**

### 쉽게 이해하기 (학습용)

- 생성형 AI 서비스는 계정·테넌트별 입력·출력 토큰과 동시 요청 한도를 적용해 장문 반복 요청이 전체 GPU 자원과 비용을 고갈시키지 않게 한다.

## Ⅶ. 결론

- 고비용 추론 요청의 자원 독점을 막기 위해 토큰 수·동시성·누적 비용·정상 업무 우선순위를 검토하여, 계층별 예산과 축소·중단 정책을 적용해야 한다.

### 쉽게 이해하기 (학습용)

- 정상적인 대형 작업은 공정하게 처리하되 비정상적인 자원 독점은 제한한다.
