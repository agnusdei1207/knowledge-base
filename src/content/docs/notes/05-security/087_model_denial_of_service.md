...
- 정의: 고비용 추론의 **연산·메모리·비용 고갈**
- 기존 한계: 호출 횟수 제한의 **요청별 비용 차이 누락**
...
| 절차 | 설명 |
|:---|:---|
| 신원·테넌트 사용량 연결 | 소비 주체·기존 사용량 확인 |
| 요청 비용·쿼터 산정 | 문맥·출력·도구 비용 예측 |
| 승인·대기·거부 결정 | 현재 용량·우선순위 대조 |
| 공정 실행·사용량 누적 | 슬롯 배분·실제 비용 측정 |
| 완료·축소·중단 반환 | 한도별 출력·도구 실행 제한 |

> 요약: 예상 비용과 누적 사용량으로 실행을 통제한다.
...
| 한계 | 분산 계정 우회 | 정상 대형 작업 과잉 거부 | 여러 시스템 동시 고갈 |
