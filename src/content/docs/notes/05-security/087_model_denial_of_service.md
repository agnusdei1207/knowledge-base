...
- 정의/개념: **모델 DoS**는 고비용 추론으로 연산•메모리•외부 비용을 고갈시키는 공격이다.
- 배경/필요성: 호출 횟수 제한의 **요청별 비용 차이 누락**이 발생한다.
...
- 긴 문맥•출력의 **GPU 자원 점유**가 핵심이다.
- 도구 순환•재시도의 **연쇄 비용 증가**가 핵심이다.
...
- 예상 비용이 쿼터 이내면 **승인**, 실행 비용 초과 시 **회로 차단**이 핵심이다.
