...
- 정의/개념: 고비용 추론 요청으로 연산 자원을 고갈시키는
  **모델 DoS** 공격
- 배경/필요성: 요청 건수 제한으로 막기 어려운
  **비대칭 자원 소비**와 도구 순환
...
- 외부 API, DB 조회가 얽힌 **도구 순환**으로
  연쇄 DoS와 클라우드 과금 폭증을 유발한다.
...
1. 입력 토큰·문맥 확장: 고비용 장문 입력 주입
2. 병렬 작업·대기열 누적: 추론 큐 병렬 점유
3. 반복 추론·도구 자원 소비: 토큰 생성·API 호출 반복
4. 메모리·연산 슬롯 고갈: KV 캐시·GPU 용량 소진
5. 지연·오류·서비스 거부 전파: 정상 요청 타임아웃 확산
...
- 고비용 요청은 **가중 쿼터**, 반복 호출은 **회로 차단기** 적용
