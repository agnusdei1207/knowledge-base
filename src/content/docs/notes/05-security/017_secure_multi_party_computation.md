...
- 정의/개념: 원본 입력을 숨기고 결과만 공동 계산하는 **PET**이다.
- 배경/필요성: 중앙 집중 분석은 **조직 간 원본 노출•이전 제약**을 유발한다.
...
- **허용 공모 임계값**에 따른 원본 비밀성이 핵심이다.
- **악의적 공격자 모델**의 부정 메시지를 검증한다.
- **반복•세분 출력**에 따른 개인 입력 추론 위험이 핵심이다.
...
3. **결과 조각 MAC 검증**: 부정 참여자의 변조 여부를 판정한다.
4. **허용 결과 재구성**: 임계 조각으로 최종 출력을 복구한다.
...
- 산술 중심은 **비밀 분산**, 비교•분기 중심은 **가블드 회로** 선택한다.
