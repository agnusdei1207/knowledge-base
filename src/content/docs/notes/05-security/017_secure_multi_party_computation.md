...
- **안전한 다자간 연산(Secure Multi-Party Computation, MPC)**: 여러 참여자가 각자의 원본 입력을 공개하지 않고 정해진 함수의 결과만 공동 계산하는 기술
- **프라이버시 강화 기술(Privacy-Enhancing Technology, PET)**: 데이터 활용 과정에서 원본 노출과 개인 식별 가능성을 줄이는 기술군
- **비밀 분산(Secret Sharing)**: 비밀을 여러 조각으로 나누고 임계값 이상의 조각이 모일 때만 원본을 복구하는 방식
...
- **메시지 인증 코드(Message Authentication Code, MAC)**: 비밀키를 이용하여 메시지의 변조 여부와 송신자 정당성을 검증하는 값
- **비버 트리플(Beaver Triple)**: 비밀 분산 곱셈에 필요한 값을 미리 계산하여 온라인 통신량을 줄이는 기법
- **가블드 회로(Garbled Circuit)**: 회로의 와이어 값을 암호화하여 입력을 공개하지 않고 출력을 계산하는 방식
...
- 기관별 원본 데이터 공개 없이 공동 계산하기 위해 계산 함수·공모 임계값·통신 비용·출력 누출을 검토하여, 신뢰를 분산해야 하는 협업에 MPC를 설계해야 한다.
