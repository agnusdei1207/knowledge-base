...
- **임계값(Threshold)**: 비밀 재구성이나 공동 연산에 필요한 최소 참여자·조각 수다.
- **공모(Collusion)**: 참여자들이 정보와 조각을 합쳐 다른 참여자의 비밀을 추론하는 행위다.
- **반정직 공격자(Semi-honest Adversary)**: 절차는 따르지만 관측 메시지로 추가 정보를 추론하는 참여자다.
- **악의적 공격자(Malicious Adversary)**: 잘못된 입력·메시지·중단으로 결과를 왜곡하는 참여자다.
- **메시지 인증 코드(Message Authentication Code, MAC)**: 비밀키를 이용하여 메시지의 변조 여부와 송신자 정당성을 검증하는 값이다.
...
- **재구성(Reconstruction)**: 임계값 이상의 결과 조각을 모아 허용된 최종 출력을 복구하는 절차다.
- **ISO/IEC 4922-1:2023**: MPC의 용어·절차·참여자·암호 속성을 규정한 국제 표준이다.
...
- 배경/필요성: 원본을 한곳에 집중하거나 이전하지 않는 조직 간 데이터 결합·공동 분석 필요
...
1. **A 입력 조각 배포**: A 원본을 임계 조각으로 분산
2. **B 입력 조각 배포**: B 원본을 임계 조각으로 분산
3. **비밀 조각 공동 연산**: 조각 상태의 결과·MAC 계산
4. **결과 조각 MAC 검증**: 부정 참여자의 변조 여부 판정
5. **허용 결과 재구성**: 임계 조각으로 최종 출력 복구
...
| 핵심 특징 | **조각 기반 산술 연산** | **암호화 논리 회로 평가** | 암호문 상태로 연산 |
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| MPC 공통 절차 | **ISO/IEC 4922-1 적용** | 역할·속성 해석 일관화 |
| 비밀 분산 연산 | **ISO/IEC 4922-2 적용** | 조각 계산 상호운용성 확보 |
...
- **함수형·공모임계·통신량·출력누출**로 MPC 방식을 결정한다.
