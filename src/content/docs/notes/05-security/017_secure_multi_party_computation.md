...
- **안전한 다자간 연산(Secure Multi-Party Computation, MPC)**: [읽기: 엠피씨; 표기 이유: 영문 머리글자와 구분 기호] 여러 참여자가 각자 원본 입력을 공개하지 않고 합의한 함수의 결과만 공동 계산하는 기술
- **프라이버시 강화 기술(Privacy-Enhancing Technology, PET)**: [읽기: 피이티; 표기 이유: 영문 머리글자와 구분 기호] 데이터 활용 과정에서 원본 노출과 개인 식별 가능성을 줄이는 기술군
- **비밀 분산(Secret Sharing)**: [읽기: 에스이씨알이티; 표기 이유: 영문 머리글자와 구분 기호] 비밀을 여러 조각으로 나눠 정해진 수 이상의 조각이 모여야 원본을 복구하게 하는 방식
- **임계값(Threshold)**: [읽기: 티에이치알이에스에이치오엘디; 표기 이유: 영문 머리글자와 구분 기호] 비밀 재구성이나 공동 연산을 완료하는 데 필요한 최소 참여자 또는 조각 수
- **공모(Collusion)**: [읽기: 씨오엘엘유에스아이오엔; 표기 이유: 영문 머리글자와 구분 기호] 여러 참여자가 각자의 정보와 조각을 합쳐 다른 참여자의 비밀을 알아내려는 협력
- **반정직 공격자(Semi-honest Adversary)**: [읽기: 에스이엠아이; 표기 이유: 영문 머리글자와 구분 기호] 정해진 절차는 따르지만 관측한 메시지로 추가 정보를 추론하려는 참여자
- **악의적 공격자(Malicious Adversary)**: [읽기: 엠에이엘아이씨아이오유에스; 표기 이유: 영문 머리글자와 구분 기호] 잘못된 입력·메시지나 중단으로 계산 결과를 왜곡하려는 참여자
- **메시지 인증 코드(Message Authentication Code, MAC)**: [읽기: 엠에이씨; 표기 이유: 영문 머리글자와 구분 기호] 비밀키로 메시지의 변조와 출처를 검증하는 값
- **비버 트리플(Beaver Triple)**: [읽기: 비이에이브이이알; 표기 이유: 영문 머리글자와 구분 기호] 비밀 분산된 무작위 값과 그 곱을 미리 준비해 온라인 곱셈 통신을 줄이는 재료
- **가블드 회로(Garbled Circuit)**: [읽기: 지에이알비엘이디; 표기 이유: 영문 머리글자와 구분 기호] 논리 회로의 선 값을 암호화해 입력을 공개하지 않고 회로 출력을 계산하는 방식
- **동형 암호(Homomorphic Encryption)**: [읽기: 에이치오엠오엠오알피에이치아이씨; 표기 이유: 영문 머리글자와 구분 기호] 암호문을 복호화하지 않고 계산해도 복호화 결과가 평문 계산과 대응하는 암호
- **재구성(Reconstruction)**: [읽기: 알이씨오엔에스티알유씨티아이오엔; 표기 이유: 영문 머리글자와 구분 기호] 필요한 수 이상의 결과 조각을 모아 허용된 최종 출력으로 복구하는 절차
...
- **정의/개념**: 원본 입력 없이 합의한 함수 출력만 공동 계산
- **배경/필요성**: 원본 이전 금지 조직 간 데이터 결합
...
| 주요 위험 | 많은 통신 왕복·전처리 | 회로 크기에 따른 통신 증가 | 큰 암호문·연산 비용 |
