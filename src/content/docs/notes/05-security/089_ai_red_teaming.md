...
- 정의/개념: AI 레드팀(AI Red Teaming)은 개발자나 검증자의 정상 추론 관점을 벗어나, 공격자의 관점에서 생성형 AI 및 LLM 애플리케이션의 탈옥(Jailbreak), 프롬프트 인젝션, 환각 유도, 에이전트 권한 오용 위협을 적대적 입력(Adversarial Input)을 통해 모의 침투 탐색하는 안전성 평가 체계이다.
- 배경/필요성: 기존 소프트웨어 취약점 점검(Static/Dynamic Analysis)으로는 자연어 기반 제어/데이터 혼동 및 트랜스포머 모델의 적대적 바이패스 현상을 사전에 도출하기 어렵기 때문이다.
...
1. **교전 규칙•안전 한도 검증**: 레드팀 평가 범위, 대상 시스템, 중단 조건(Circuit Breaker)을 정의한다.
2. **공격 시나리오 안전 실행**: 샌드박스 내에서 자동화 하네스(Garak 등) 및 보안 전문가의 수동 적대적 침투를 수행한다.
3. **근본 원인•완화 구현**: 인젝션/탈옥 실패 원인을 분석해 Guardrail 규칙 및 PEP 정책을 보정한다.
4. **잔여 위험 재산정**: 보정 적용 후 **잔여 위험 평가**를 통해 ASR(공격 성공률) 감소 수치를 파악한다.
5. **변형 회귀•배포 결정**: 방어 룰을 우회하는 **변형 회귀** 시험을 통과할 경우 실제 서비스 배포를 승격 결정한다.
...
- **레드팀 배포 근거**를 충족하기 위해 **NIST AI 600-1** 준수, PyRIT/Garak **검증 하네스** 및 **회귀 평가** 체계를 연계 적용한다.
