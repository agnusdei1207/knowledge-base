...
- **1. 평가 목적•범위•모델 선정**: **평가 목적•범위•모델 선정**으로 평가 단위를 확정한다.
- **2. 표본 기간•실행 증거 검증**: **증거 기반 평가**로 **표본 기간•실행 증거 검증**을 수행한다.
- **3. 현재 수준•목표 프로파일 판정**: **현재 수준•목표 프로파일 판정**으로 위험 허용선을 정한다.
- **4. 위험•의존성 기반 로드맵 수립**: **위험•의존성 기반 로드맵 수립**으로 책임•투자를 배정한다.
- **5. 성과 측정•잔여위험 재평가**: **성과 측정•잔여위험 재평가**로 목표 상태를 갱신한다.
