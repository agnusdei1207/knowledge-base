...
- 정의/개념: **백도어 공격**은 특정 트리거에서만 공격자가 정한 목표 행동을 활성화하는 공격이다.
- 배경/필요성: 정상 입력 중심의 정확도 평가만으로는 **트리거 기반 조건부 행동을 탐지하기 어려움**이 발생한다.
...
- 트리거 부재 시 **정상 성능 위장**이 핵심이다.
- 데이터•가중치 경로의 **조건부 규칙 주입**이 핵심이다.
- 변형 트리거 시험의 **잠복 행동 탐지**가 핵심이다.
...
2. **트리거•목표 행동 연관 학습**: 조건부 공격 규칙을 모델에 반영한다.
3. **정상•트리거 조건부 출력 생성**: 기준•변형 입력의 행동을 분기한다.
4. **정확도•ASR 판정**: 정상성과 공격 성공률을 비교한다.
...
- **AI(Artificial Intelligence) 100-2e2025**는 특정 패턴에서 공격자 선택 행동을 일으키는 오염 공격과 역량을 정의한다.
...
- 정상 정확도를 통과하고 **ASR 기준 이하인 모델만 승격**, 실패 시 격리•복구가 핵심이다.
