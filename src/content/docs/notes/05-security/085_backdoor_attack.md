...
- 정의/개념: 백도어 공격은 머신러닝 학습 데이터셋이나 가중치 파이프라인에 은밀히 트리거-목표 클래스 연관(Association)을 학습시켜, 배포 후 일반 테스트에서는 탐지되지 않고 특정 시그릿 트리거 유입 시에만 악성 오작동을 유발하는 공격 기술이다.
- 배경/필요성: 자율주행 정지 표지판 인식, 얼굴 인증 로그인 시스템 등에 백도어가 심어질 경우, 특정 스티커 하나로 보안 시스템이 무력화되는 치명적 사고를 초래하기 때문이다.
...
1. **오염 자산 학습 세트 반영**: 트리거 패턴이 매복된 오염 데이터가 학습 파이프라인으로 유입된다.
2. **트리거•목표 행동 연관 학습**: 모델 학습 과정에서 트리거 피처와 **목표 행동** 클래스 간의 **트리거 연관**이 가중치에 각인된다.
3. **정상•트리거 조건부 출력 생성**: 입력 조건에 따라 통상 작동과 악성 오판 작동이 분기 생성된다.
4. **정확도•ASR 판정**: 1차 **정확도 판정**과 2차 **ASR 판정**을 통합 수행하여 백도어 감지를 진행한다.
5. **승격•차단 결정**: 통과된 모델만 배포 승격하고 백도어 감지 시 프로덕션 진입을 강제 차단 격리한다.
...
| 일반 Accuracy 검사 시 스텔스 통과 | **ASR 판정** 및 **트리거 반전** (Neural Cleanse) | 미지의 잠재 백도어 트리거 패턴 100% 역추적 탐지 |
| 클린 라벨 기반 스텔스 백도어 침투 | **클린 라벨 오염** 스캐닝 및 DVC 계보 보존 | 라벨 무변조 스텔스 백도어 샘플 사전에 철저 차단 |
