...
- **공격 성공률(Attack Success Rate, ASR)**: '에이에스알'로 읽고 영문 머리글자로 트리거 입력이 목표 행동을 일으킨 비율을 표시
...
- 다양한 매체의 트리거는 탐지 범위를 넓힌다
- 데이터·가중치 변조는 여러 공급망 경로를 노린다
- 트리거 크기·위치·변형은 공격 성공률을 바꾼다
...
| 주요 위험 | 정상 평가 통과·장기 잠복 | 전체 성능·공정성 저하 | 입력마다 공격 생성 필요 |
...
1. 외부 모델의 변형별 트리거 성공률을 평가한다
2. 정상 모델과 트리거의 내부 활성을 비교한다

### 쉽게 이해하기 (학습용)

- 외부 모델은 일반 정확도와 위치·크기·변형별 트리거 ASR을 함께 측정함
- 영상·음성의 이상 패턴과 정상 모델의 내부 활성 차이를 비교해 숨은 규칙을 찾음

## Ⅶ. 결론

- 정상 정확도와 ASR로 모델 교체를 결정한다.

### 쉽게 이해하기 (학습용)

- 평소 성능만 보지 않고 특정 조건에서만 나타나는 행동까지 시험해야 함
