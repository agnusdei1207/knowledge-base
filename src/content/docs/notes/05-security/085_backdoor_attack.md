...
- 정의: 특정 트리거의 **잠복 목표 행동 공격**
- 기존 한계: 정상 정확도 평가의 **조건부 행동 탐지 실패**
...
| 절차 | 설명 |
|:---|:---|
| 트리거·목표 행동 주입 | 표본·가중치에 연관 삽입 |
| 오염 규칙 학습 | 조건부 행동을 모델에 반영 |
| 정상·트리거 출력 | 기준·변형 입력 결과 생성 |
| 정확도·ASR 판정 | 정상성과 공격 성공률 비교 |
| 승격 차단·원인 복구 | 계보로 오염 자산 제거 |

> 요약: 정상 성능과 트리거 행동을 함께 검증한다.
...
| 핵심 특징 | **트리거 목표 행동** | **학습 규칙 광범위 왜곡** | **추론 입력 경계 우회** |
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 백도어 공격 분류 | **NIST AI 100-2e2025 적용** | 트리거·역량별 시험 체계화 |
...
- **정상정확도·트리거ASR·영향범위**로 모델 승격을 결정한다.
