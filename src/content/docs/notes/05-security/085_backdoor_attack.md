...
- **Neural Cleanse**: 역공학으로 모델 내부의 최소 트리거 패턴을 복원하여 백도어 존재 여부를 탐지하는 대표 기법이다.
...
| 백도어 탐지 | **Neural Cleanse** 역공학 트리거 스캐닝 및 **ASR** 산출 |
| 승격·복구 게이트 | **승격•복구 게이트**를 거쳐 검증된 모델만 배포하고 탈취 시 이전 레포지토리 복원 |

#### 한줄 요약

- 오염 자산 관제, 안전 파이프라인 훈련, **Neural Cleanse** 역공학 스캐닝 및 승격/복구 게이트 체계로 구축된다.
