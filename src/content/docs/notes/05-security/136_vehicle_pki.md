...
- **차량 공개키 기반구조(Vehicle Public Key Infrastructure, Vehicle PKI)**: `Public Key Infrastructure`의 머리글자를 딴 공식 표기라 “비히클 피케이아이”로 읽으며, V2X 메시지 서명·인증서 수명주기를 관리한다
...
- 서명 검증으로 출처·무결성·서비스 권한 확인한다.
- 단기 가명 인증서가 차량의 장기 추적을 줄인다.
- 등록 자격과 서비스 권한 발급 기관 분리한다.
- 오동작·폐기 정보로 침해 장치 상태를 갱신한다.
...
| 비교축 | 등록 인증서 | 가명 인증서 | TLS 인증서 |
|:---|:---|:---|:---|
| 핵심 특징 | 차량의 장기 가입 자격 증명 | 단기 V2X 메시지 권한 증명 | 통신 상대 인증과 암호화 세션 수립 |
| 적용 기준 | EA 가입·권한 요청의 기반이 필요할 때 | 실명 노출 없이 안전 메시지를 서명할 때 | 차량과 서버의 연결을 보호할 때 |
| 주요 위험 | 장기 식별자 노출로 차량 추적 | 교체 패턴·무선 식별자로 재연결 | 서버 인증서를 V2X 방송 권한으로 오용 |
...
1. V2X 서명과 내용 암호화 정책을 분리
2. 가명·무선 식별자의 교체 시점을 결합
3. 시간·위치·센서로 오동작을 판정
4. CRL을 압축·분할해 간헐 연결에 배포

### 쉽게 이해하기 (학습용)

- 안전 메시지는 서명하되 내용 암호화는 서비스 목적과 표준에 맞춰 별도 판단함
- 가명 인증서와 무선 식별자를 함께 바꿔 이동 경로의 장기 연결을 줄임
- 유효 인증서도 거짓 위치를 보낼 수 있어 센서·시간·다수 보고로 오동작을 판단함
- 간헐 연결 차량에도 폐기 정보를 받을 수 있게 목록을 압축·분할·우선 배포함

## Ⅶ. 결론

- 등록·가명 권한 분리로 V2X 신뢰·추적 방지
