...
- **최상위 인증기관(Root Certificate Authority, Root CA)**: 차량 신뢰 도메인의 최상위 인증서와 하위 인증기관의 신뢰를 보증하는 기관이다.
- **등록 기관(Enrollment Authority, EA)**: 차량의 가입 자격을 검증하고 장기 등록 자격 증명을 발급·관리하는 기관이다.
- **권한 기관(Authorization Authority, AA)**: 승인된 차량에 단기 V2X 서비스 권한 인증서를 발급하는 기관이다.
...
- **인증서 폐기 목록(Certificate Revocation List, CRL)**: 만료 전이라도 더는 신뢰하지 않는 인증서의 식별자와 상태를 배포하는 목록이다.
- **차량·모든 대상 간 통신(Vehicle-to-Everything, V2X)**: 차량이 다른 차량·인프라·네트워크와 안전 정보를 교환하는 통신 기술이다.
- **인증기관(Certificate Authority, CA)**: 공개키와 주체 또는 권한의 관계를 디지털 인증서로 보증하는 신뢰기관이다.
- **전송 계층 보안(Transport Layer Security, TLS)**: 차량과 서버의 신원을 검증하고 통신 구간의 기밀성과 무결성을 보호하는 암호 통신 규약이다.
...
- **배경/필요성**: 고정된 장기 인증서를 사용하면 V2X 메시지 신뢰를 확보하는 대신 차량 이동 경로가 지속해서 추적될 수 있음
