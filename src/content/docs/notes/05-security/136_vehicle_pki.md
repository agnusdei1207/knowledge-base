...
- 정의/개념: 차량•인프라 메시지 인증서를 발급•검증•갱신•폐기해 신뢰와 가명성을 제공하는 **차량 공개키 기반구조**이다.
- 배경/필요성: 장기 인증서를 방송에 쓰면 **차량 이동경로 추적 가능성이 있다.
...
- 등록•권한 기관 분리의 **최소 정보 결합**이 핵심이다.
- 단기 가명 인증서의 **메시지 권한•프라이버시**가 핵심이다.
- 서명•신선도•개연성의 **다중 신뢰 검증**이 핵심이다.
...
- **1. 가입 자격 검증•식별자 분리**: 장기 식별 노출을 최소화한 자격을 생성한다.
- **2. 단기 가명 인증서 발급**: 서비스•기간별 방송 권한을 생성한다.
- **3. V2X 메시지 가명 서명**: 단기 키로 안전 정보 서명이다.
- **4. 체인•신선도•개연성 검증**: 인증과 물리 가능성을 함께 검증한다.
- **5. 오동작 인증서 폐기 판정**: 위험 장치의 만료 전 사용 차단한다.
...
- 방송은 **단기 가명 인증서**, 서버 연결은 **TLS 인증서**를 적용한다.
