...
- **1. 가입 자격 검증•식별자 분리**: **SCMS**의 **가입 자격 검증•식별자 분리**로 장기 식별 노출을 줄인다.
- **2. 단기 가명 인증서 발급**: **단기 가명 인증서 발급**으로 서비스•기간별 권한을 만든다.
- **3. V2X 메시지 가명 서명**: **V2X 메시지 가명 서명**으로 안전 정보의 진위를 증명한다.
- **4. 체인•신선도•개연성 검증**: **체인•신선도•개연성 검증**으로 인증과 물리 가능성을 확인한다.
- **5. 오동작 인증서 폐기 판정**: **오동작 인증서 폐기 판정**으로 위험 장치의 사용을 차단한다.

#### 한줄 요약

- 차량은 임시 인증서로 메시지를 보내고 수신자는 서명•시간•권한•물리 가능성을 함께 확인하는 것이 핵심이다.
