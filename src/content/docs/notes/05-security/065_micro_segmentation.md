...
date: "2026-07-25T22:05:00+09:00"
tags:
  - "notes-security"
weight: 65
extra:
...
- **정의**: 워크로드별 허용 통신만 집행
- **배경/필요성**: 내부 탐색과 측면 이동을 제한
- **전제**: 자산 신원과 정상 의존성의 가시화
...
- 주소보다 워크로드·서비스 신원으로 정책을 표현한다.
- 동서 트래픽은 기본 거부하고 필요한 흐름만 허용한다.
- 호스트·가상망·컨테이너·서비스에서 정책을 집행한다.
- 동적 환경은 정확한 라벨과 자동 갱신이 필요하다.
- 의존성 누락은 과허용이나 업무 중단을 유발한다.
...
- **사례 1**: 웹 서버에서 데이터베이스 포트만 허용
- **사례 2**: 백업·관제·복구 흐름도 정책화
- **사례 3**: 긴급 허용 규칙을 자동 만료
- **사례 4**: 관리면과 신원 발급 체계를 분리

### 쉽게 이해하기 (학습용)

- 사례 1은 웹 서버가 침해돼도 데이터베이스의 불필요한 관리 포트나 다른 서버를 탐색하지 못하게 한다.
- 사례 2는 평소 드문 필수 운영 통신을 누락해 장애 때 백업이나 복구가 막히는 일을 방지한다.
- 사례 3은 장애 해결을 위해 연 넓은 통신 경로가 작업 뒤 영구적인 측면 이동 통로로 남지 않게 한다.
- 사례 4는 공격자가 정책 관리 권한과 워크로드 신원을 함께 장악해 모든 차단 규칙을 우회하지 못하게 한다.

## Ⅶ. 결론

- 내부 확산 위험이면 정상 흐름만 세밀히 허용한다.
