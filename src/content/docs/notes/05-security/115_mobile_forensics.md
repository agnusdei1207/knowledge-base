...
- 배경/필요성: 디스크 복제만으로는 암호화 키•클라우드•원격 삭제의 **증거 보존하기 어렵다**.
...
- BFU•AFU별 **암호화 키 접근 차이**가 핵심이다.
- 통신 격리로 **원격 삭제 차단**, 전원 유지로 **암호화 키 보존**이 핵심이다.
...
1. **격리•보존•추출 전략 적용**: 원격 명령•키•배터리 위험을 고려해 획득 절차를 실행한다.
...
- 최소 변경은 **논리 추출**, 앱 구조는 **파일시스템 추출**, 비할당 영역은 **물리 추출**이 핵심이다.
