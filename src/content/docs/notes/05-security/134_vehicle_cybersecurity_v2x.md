...
sequenceDiagram
  ...
  G->>G: 2. 센서•지도 개연성 검사
  G->>C: 안전 메시지 전달
  C->>C: 3. 제한 제어•이상 송신자 판정
  C->>F: 이상 송신자 정보 전달
  F->>F: 4. 인증서 폐기•차량군 대응 결정
  F->>G: 대응 정책 전달
```

**동작 원리**

- **1. 인증서•서명•신선도 검증**: 신뢰 체인•유효기간•재생 여부 검증
- **2. 센서•지도 개연성 검사**: 위치•속도•시간의 물리 가능성 대조
- **3. 제한 제어•이상 송신자 판정**: 안전 제어와 오동작 증거 생성
- **4. 인증서 폐기•차량군 대응 결정**: 이상 송신자 차단•환류 기준 결정
