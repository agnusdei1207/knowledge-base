...
- 정의/개념: **CNAPP**는 개발(Shift-Left) 단계의 코드/IaC 보안부터 클라우드 운영(Shift-Right) 단계의 형상, 권한, 런타임 위협 탐지(CSPM+CIEM+CWPP)를 단일 그래프 엔진으로 통합 분석하는 차세대 플랫폼이다.
- 배경/필요성: 기존 사일로화된 파편적 보안 도구(CSPM, CWPP, SAST 등)로 인한 경보 피로(Alert Fatigue), 맥락 분절, 실제 침해 경로(Attack Path) 식별 불능 문제를 해소하기 위함이다.
...
- **공격 경로 단절 검증(Attack Path Severance Verification)**: 코드 조치 및 재배포 실행 후, 그래프 분석을 재실행하여 공격 가용 경로가 완벽히 끊어졌는지 검증하는 절차이다.
...
1. **배포 전 발견 정규화**: 코드, IaC, 이미지 스캔 파이프라인 결과를 통일된 모델로 수집 정규화한다.
2. **코드•런타임 자산 상관**: CSPM, CIEM, CWPP 데이터 및 백엔드 파이프라인 데이터를 자산 그래프로 통합 바인딩한다.
3. **공격 경로•위험 우선순위 계산**: 지능형 침해 경로 분석을 수행하여 우선순위 조치 대상을 도출한다.
4. **원본 수정•재배포**: 문제점이 지목된 원본 IaC 코드 및 배포 파이프라인을 수정하고 자동 반영한다.
5. **공격 경로 단절 검증**: 재배포 후 그래프를 재평가하여 **운영 드리프트** 해소 및 침해 경로의 완벽한 단절을 검증한다.
...
- Shift-Left/Shift-Right 통합, 자산 관계 그래프 기반 공격 경로(Attack Path) 도출, 원본 IaC 코드 연계 조치 중심 CNAPP 체계 구축 필수.
