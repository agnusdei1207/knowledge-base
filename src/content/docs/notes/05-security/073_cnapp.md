...
- 배경/필요성: CSPM·CWPP·SAST를 따로 두면 도구마다 자기 시야 안의 결함을 모두 경보로 올려 수만 건을 사람이 손으로 이어 붙이는 비용이 반복되므로, 각 도구의 신호를 자산·권한·취약점 노드로 한 그래프에 모아 실제로 이어지는 경로만 남기는 상관 분석 계층을 개별 도구들 위에 얹을 필요
