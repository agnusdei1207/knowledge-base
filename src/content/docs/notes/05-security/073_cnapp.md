...
- 정의/개념: 코드부터 런타임까지 **공격 경로**를 분석하는 **통합 보호 플랫폼**이다.
- 배경/필요성: 분리 도구의 **경보 중복•맥락 단절**이 발생한다.
...
- **클라우드 보안 형상 관리(Cloud Security Posture Management, CSPM)**: 클라우드 자산의 설정과 노출 상태 점검이다.
- **클라우드 인프라 권한 관리(Cloud Infrastructure Entitlement Management, CIEM)**: 클라우드 주체의 유효•과잉 권한을 분석한다.
- **클라우드 워크로드 보호 플랫폼(Cloud Workload Protection Platform, CWPP)**: 워크로드 취약점과 런타임 위협 보호한다.

</details>

- **전주기 연결**: IaC•코드•이미지•런타임 위험 결합한다.
- **관계 분석**: 자산•권한•노출•취약점의 경로를 계산한다.
- **통합 우선순위화**: CSPM•CIEM•CWPP 결과를 정렬한다.
...
- **1. 배포 전 발견 정규화**: 코드•IaC•이미지 결함을 공통 자산에 연결한다.
- **2. 코드•런타임 자산 상관**: 형상•권한•행위의 실제 상태 결합한다.
- **3. 공격 경로•위험 우선순위 계산**: 노출에서 중요 자산 도달성을 분석한다.
- **4. 원본 수정•재배포**: 재현 가능한 코드•구성 원인을 제거한다.
- **5. 공격 경로 단절 검증**: 재배포 뒤 노출•권한 연결 소멸을 확인한다.
#### 한줄 요약
...
- **안전한 소프트웨어 개발 프레임워크(Secure Software Development Framework, SSDF) 1.1**: 안전한 개발•공급•취약점 대응 관행이다.
...
- 코드•런타임 공격 경로는 **CNAPP**, 단일 영역은 **개별 도구**, 화면 집계만은 배제가 핵심이다.
