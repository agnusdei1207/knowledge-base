...
- **1. 코드·IaC·이미지 위험 수집**: 배포 전 결함·비밀 탐지
- **2. 형상·권한·행위 상태 수집**: 운영 자산 실제 상태 확보
- **3. 노출·권한·취약점 경로 분석**: 중요 자산 도달 가능성 계산
- **4. 소유자·원본 코드 근본 수정**: 재현 가능한 원인 교정
- **5. 재배포·경로 단절 재검증**: 운영 위험 제거를 확인
### 쉽게 이해하기 (학습용)
