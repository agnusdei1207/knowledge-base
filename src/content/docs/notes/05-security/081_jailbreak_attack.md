---
sidebar:
  order: 81
  label: "081. 탈옥 Jailbreak 공격 (Jailbreak Attack)"
  badge:
    text: "기출 · 70%"
    variant: note
title: "탈옥 Jailbreak 공격 (Jailbreak Attack)"
date: "2026-07-30T20:00:00+09:00"
tags:
  - "notes-security"
weight: 81
extra:
  question_no: "081"
  source_status: "기출"
  source_history: "137회, 138회"
  priority: 70
  priority_note: "137·138회 반복된 안전정렬 우회 핵심 공격임"
---

## 미리 알고가기

- **탈옥(Jailbreak)**: 모델의 안전 거부 정책을 우회해 금지 응답을 생성시키는 공격이다.
- **거대 언어 모델(Large Language Model, LLM)**: 대규모 텍스트에서 언어 패턴을 학습하여 입력을 이해하고 응답을 생성하는 모델이다.
- **안전 정책(Safety Policy)**: 모델이 허용·제한·거부할 요청과 응답의 기준이다.
- **안전 정렬(Safety Alignment)**: 모델 행동이 안전 원칙을 따르도록 학습·조정하는 과정이다.
- **프롬프트 인젝션(Prompt Injection)**: 비신뢰 입력이 모델의 지침 해석을 교란하는 공격이다.
- **시스템 프롬프트(System Prompt)**: 서비스가 모델의 역할·규칙·출력 조건을 정하는 상위 지침이다.
- **가드레일(Guardrail)**: 입력·출력·도구 행동을 정책에 맞게 제한하는 통제이다.
- **적대적 접미사(Adversarial Suffix)**: 입력 뒤에 붙여 모델의 안전 거부를 교란하는 문자열이다.
- **다중 턴(Multi-Turn)**: 여러 차례 대화를 이어 문맥을 누적하는 상호작용 방식이다.
- **레드팀(Red Team)**: 실제 공격 관점으로 시스템의 안전 통제를 시험하는 활동이다.
- **회귀 평가(Regression Evaluation)**: 수정 뒤 과거 공격이 다시 성공하는지 반복 확인하는 시험이다.
- **정상 거부율(False Refusal Rate)**: 허용할 정상 요청을 모델이 잘못 거부하는 비율이다.
- **심층 방어(Defense in Depth)**: 독립 통제를 겹쳐 한 통제의 실패가 피해로 이어지지 않게 하는 전략이다.
- **OWASP LLM01:2025 Prompt Injection**: 탈옥을 안전 프로토콜을 무시하게 하는 프롬프트 인젝션 형태로 다루는 공식 항목이다.
- **NIST AI 600-1**: 생성형 AI의 오용 위험과 적대적 시험·사후 대응을 제시하는 공식 프로파일이다.

## Ⅰ. 개요

- 정의/개념: 모델의 **안전 거부 정책 우회 공격**
- 배경/필요성: 고정 금칙어의 **의미 변형 탐지 한계**

### 쉽게 이해하기 (학습용)

- 질문을 역할극·번역·암호화로 포장해 모델이 금지된 요청임을 놓치고 답하도록 유도하는 공격이다.

## Ⅱ. 특징

- **의도 은닉**은 역할극·분할·인코딩으로 안전 정책을 우회한다.
- **문맥 의존성**으로 모델·대화별 우회 성공률이 달라진다.
- **다층 안전 통제**로 출력·도구 피해와 재발을 제한한다.

### 쉽게 이해하기 (학습용)

- 공격 표현은 계속 바뀌므로 문장 하나를 차단하는 데 그치지 않고 출력과 연결 기능까지 별도로 제한해야 한다.

## Ⅲ. 구조 및 구성요소

```mermaid
block-beta
  columns 3
  A["안전 정책"]
  B["입력·대화 분류"]
  C["안전 정렬 모델"]
  D["출력·도구 통제"]
  E["레드팀·회귀 평가"]
  A --- B
  B --- C
  C --- D
  D --- E
```

| 구성요소 | 책임 |
|:---|:---|
| 안전 정책 | 위험 수준과 허용·거부 기준 정의 |
| 입력·대화 분류 | 누적 문맥의 의도와 위험 평가 |
| 안전 정렬 모델 | 금지 요청을 식별해 거부 |
| 출력·도구 통제 | 유해 내용과 외부 기능 제한 |
| 레드팀·회귀 평가 | 신규 공격과 재발 여부 검증 |


### 쉽게 이해하기 (학습용)

- 모델 앞에서 의도를 보고 모델 뒤에서 출력과 기능을 다시 막으며 새 우회 사례를 계속 시험 목록에 넣는다.

## Ⅳ. 흐름도

```mermaid
sequenceDiagram
    participant U as 공격 사용자
    participant I as 입력·대화 분류
    participant M as 안전 정렬 모델
    participant O as 출력·도구 통제
    participant E as 평가 체계
    U->>I: 1. 변형 요청·문맥 수집
    I->>M: 2. 위험 의도 분류
    M-->>O: 3. 응답·기능 제안
    O-->>U: 4. 제한 응답·기능 차단
    O->>E: 5. 우회 기록·회귀 평가
```

**동작 원리**

1. **변형 요청·문맥 수집**: 역할극·분할·인코딩 입력
2. **위험 의도 분류**: 전체 대화의 위험 표시
3. **응답·기능 제안**: 모델이 거부 또는 결과 생성
4. **제한 응답·기능 차단**: 의미·민감도·권한 재검증
5. **우회 기록·회귀 평가**: 성공·정상 거부·재발 측정


### 쉽게 이해하기 (학습용)

- 변형된 질문이 모델을 통과해도 출력과 기능 경계가 다시 검사하고 발견한 우회는 다음 버전에서도 막히는지 반복 시험한다.

## Ⅴ. 종류 및 비교

| 탈옥 공격 유형 | 단일 턴 우회 | 다중 턴 우회 | 자동화 탐색 |
|:---|:---|:---|:---|
| 적용 기준 | 입력별 위험 분류 | 전체 대화 상태 추적 | 대규모 회귀·레드팀 |
| 핵심 특징 | **한 입력의 의도 은닉** | **대화 누적 경계 약화** | **대량 변형 자동 생성** |
| 한계 | 표현 변형 탐지 곤란 | 누적 의도 탐지 실패 | 신규 우회 빠르게 발견 |

> 요약: 단일·다중 턴과 자동 탐색으로 거부를 우회한다

### 쉽게 이해하기 (학습용)

- 단일 턴은 한 번의 포장, 다중 턴은 대화 누적, 자동화 탐색은 많은 변형을 기계적으로 시험해 안전 거부의 빈틈을 찾는다.

## Ⅵ. 실무 고려사항 및 대책

| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 탈옥 위협 분류 | **OWASP LLM01:2025 적용** | 우회 시나리오 체계화 |
| 생성형 AI 오용 | **NIST AI 600-1 연계** | 적대적 시험·대응 강화 |
| 정상 요청 과잉 차단 | **공격성공률·정상거부율 병행** | 안전성·유용성 균형 확보 |

### 쉽게 이해하기 (학습용)

- 모델은 가상 인물 역할이나 여러 정상 질문으로 위험한 요청을 분할해도 전체 대화 의도를 평가해 금지된 공격 절차 생성을 거부한다.

## Ⅶ. 결론

- **대화의도·출력위험·도구영향**으로 탈옥 통제를 결정한다.

### 쉽게 이해하기 (학습용)

- 탈옥은 고정 규칙으로 완전히 끝낼 수 없으므로 모델의 거부·출력 검사·기능 격리·지속 평가를 겹쳐 실제 피해를 제한해야 한다.
