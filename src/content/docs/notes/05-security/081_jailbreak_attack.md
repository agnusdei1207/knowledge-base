...
date: "2026-07-25T13:12:00+09:00"
tags:
  - "notes-security"
weight: 81
extra:
...
- **탈옥(Jailbreak)**: 모델의 안전 거부 정책을 우회해 금지된 응답을 생성시키려는 공격
- **LLM(Large Language Model)**: 대규모 텍스트 데이터로 학습해 언어를 이해·생성하는 모델
...
- **거대 언어 모델(Large Language Model, LLM·엘엘엠)**: 영문 각 단어의 머리글자를 딴 표기이며, 대규모 언어 데이터를 학습해 응답을 생성하는 탈옥 공격 대상
- **레드팀(Red Team·레드 팀)**: 군사 모의훈련의 공격자 편 명칭에서 온 표기이며, 실제 공격 관점으로 안전 통제의 우회 가능성을 검증함

## Ⅰ. 개요

- **정의/개념**: 모델의 안전 거부를 우회하는 공격이다
- **배경/필요성**: 변형 입력의 유해 응답 생성을 막는다
...
- 역할극·분할·인코딩 조합은 탐지 비용을 높인다
- 모델·문맥 변화는 우회 성공률을 변동시킨다
- 고정 금칙어는 의미 변형 우회를 막지 못한다
- 기능 제한·회귀 평가는 우회 피해와 재발을 줄인다
...
| 주요 위험 | 표현 변형의 다양성 | 누적 의도 탐지 실패 | 빠른 신규 우회 발견 |
