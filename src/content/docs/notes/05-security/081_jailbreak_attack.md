...
| 회귀 사례 전달 | 우회 입력·판정 결과를 전달 |
| 분류 규칙 갱신 | 재발 시험 결과로 통제 개선 |
...
- 모델 안전 정책의 우회 가능성을 줄이기 위해 전체 대화 의도·출력 위험·도구 영향·정상 거부율을 검토하여, 다층 통제와 지속적인 탈옥 회귀 평가를 적용해야 한다.
