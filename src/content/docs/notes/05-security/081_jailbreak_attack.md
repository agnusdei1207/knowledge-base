...
- 정의/개념: **탈옥**은 모델의 안전 거부 정책을 우회해 금지 응답을 생성시키는 공격이다.
- 배경/필요성: 고정 금칙어의 **의미 변형 탐지 한계**가 발생한다.
...
- 역할극•분할•인코딩의 **의도 은닉 기반 정책 우회**가 핵심이다.
- **적대적 접미사•다중 턴** 등 모델•대화 상태에 따른 문맥 의존적 우회 성공률이 핵심이다.
- 출력•도구 피해와 재발을 제한하는 **심층 방어**가 핵심이다.
...
4. **독립 출력•권한 검증 누락**: 모델 제안을 안전한 결과처럼을 수용한다.
...
- **출력 위험•도구 영향**이 크면 입력•모델•출력•도구 통제를 적용한다.
