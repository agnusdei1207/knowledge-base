...
- 정의/개념: 탈옥(Jailbreak) 공격은 LLM에 적용된 윤리적•보안적 **안전 정렬** 거부 메커니즘을 롤플레이, 가상 연구 시나리오, 인코딩, 적대적 텍스트로 속여 금지된 위험 정보(해킹, 범죄, 기밀)를 강제로 출력하게 만드는 입력 공격 기술이다.
- 배경/필요성: 고정 문자열 블랙리스트 필터링의 한계로 인해, 페르소나 설정(DAN 등)이나 다중 턴 우회 공격을 효과적으로 탐지 차단하는 다층 방어 아키텍처가 필수로 요구되기 때문이다.

#### 한줄 요약

- 페르소나 롤플레이나 적대적 토큰으로 LLM의 RLHF 안전 정렬 거부 필터를 속여 유해•위험 응답을 유도하는 공격 기술이다.
...
1. **다중 턴 공격 문맥 누적**: 롤플레이 설정 및 위험 의도를 여러 턴으로 나누어 대화 세션에 주입한다.
2. **변형 표현의 위험 의도 오인**: LLM이 가상 연구/소설 프레임으로 오인하여 질문의 위험성을 식별하지 못한다.
3. **안전 거부 정책 우회**: RLHF 안전 거부 필터가 동작하지 않고 악성 답변 생성을 시작한다.
4. **독립 출력•권한 검증 누락**: Output Guardrail 2차 필터가 없거나 오탐으로 통과된다.
5. **유해 내용•기능 실행**: 유해 정보 생성 및 비인가 API 도구 호출이 집행된다.
...
- **탈옥 피해 제한**을 달성하기 위해 입력/출력 2중 가드레일(**가드레일**), 샌드박싱 및 레드팀 **회귀 평가** 체계를 통합 적용한다.
