...
- **보안 접근 서비스 경계(Secure Access Service Edge, SASE)**: 시큐어 액세스 서비스 에지, 새시로 읽으며, 각 영단어의 머리글자를 딴 표기이고 본문에서는 광역망 연결과 보안을 가까운 접속 거점에서 공통 정책으로 제공합니다.
...
- 신원·기기·응용·데이터를 공통 평가한다.
- SD-WAN 경로와 SSE 검사를 결합한다.
- 가까운 PoP가 중앙 백홀 지연을 줄인다.
- 중앙 정책이 위치별 기준 차이 제거한다.
...
| 주요 위험 | PoP 장애·공급자 종속 | 회선 품질·경로 제어 공백 | 백홀 지연·정책 불일치 |
