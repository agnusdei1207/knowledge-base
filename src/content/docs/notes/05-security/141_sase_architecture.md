...
- 사용자 가까운 거점에서 신원•기기를 검증하고 허용 응용으로 바로 연결하는 것이 핵심이다.
...
- 위치가 달라도 같은 신원•기기•데이터 기준으로 경로 선택과 보안 검사를 수행하는 것이 핵심이다.
...
- 가까운 거점에서 경로와 보안을 동시에 적용하고 정책•로그는 일관되게 관리하는 것이 핵심이다.
...
- **1. 지연•가용성 기반 PoP 선택**: **접근•검사•경로 흐름**의 **지연•가용성 기반 PoP 선택**으로 경로를 계산한다.
- **2. 맥락•위험•데이터 정책 판정**: **맥락•위험•데이터 정책 판정**으로 최소 응용만 허용한다.
- **3. SSE 검사 정책 적용**: **SSE 검사 정책 적용**으로 웹•클라우드•데이터를 통제한다.
- **4. SD-WAN 경로•접근 집행**: **SD-WAN 경로•접근 집행**으로 세션을 연결한다.
- **5. 세션 위험 지속 평가**: **지속 위험 평가**에 따른 **세션 위험 지속 평가**로 정책을 갱신한다.

#### 한줄 요약

- 가까운 거점을 고른 뒤 맥락에 맞는 응용만 연결하고 세션 중 위험도 계속 평가하는 것이 핵심이다.
