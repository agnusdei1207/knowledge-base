---
sidebar:
  order: 141
  label: "141. 보안 접근 서비스 경계(Secure Access Service Edge, SASE) 아키텍처"
  badge:
    text: "기출 · 70%"
    variant: note
title: 보안 접근 서비스 경계(Secure Access Service Edge, SASE) 아키텍처
date: "2026-07-30T19:00:00+09:00"
tags:
  - notes-security
weight: 141
extra:
  question_no: "141"
  source_status: "기출"
  source_history: "135회"
  priority: 70
  priority_note: "135회 기출이며 광역망·보안 통합 설계성이 큼"
---

## 미리 알고가기

- **보안 접근 서비스 경계(Secure Access Service Edge, SASE)**: 광역망 연결과 보안을 가까운 클라우드 거점에서 공통 정책으로 제공하는 구조이다.
- **소프트웨어 정의 광역망(Software-Defined Wide Area Network, SD-WAN)**: 응용 정책·회선 상태로 광역망 경로를 동적 선택하는 기술이다.
- **보안 서비스 경계(Security Service Edge, SSE)**: 웹·클라우드·응용 접근 보안을 클라우드 거점에서 통합 제공하는 구조이다.
- **보안 웹 게이트웨이(Secure Web Gateway, SWG)**: 웹 주소·콘텐츠·악성 행위를 검사해 접근을 통제하는 서비스이다.
- **클라우드 접근 보안 중개(Cloud Access Security Broker, CASB)**: 클라우드 접근·데이터·위협·준수 정책을 집행하는 통제점이다.
- **제로 트러스트 네트워크 접근(Zero Trust Network Access, ZTNA)**: 신원·기기를 검증해 승인 응용에만 연결하는 기술이다.
- **서비스형 방화벽(Firewall as a Service, FWaaS)**: 네트워크 방화벽을 분산 클라우드 서비스로 제공하는 방식이다.
- **접속 거점(Point of Presence, PoP)**: 사용자 가까이에서 연결·보안 검사를 처리하는 공급자 거점이다.
- **MEF 117**: SASE 서비스 프레임워크와 가입자·공급자 간 서비스 속성을 정의한다.
- **MEF 118.1**: MEF 서비스의 신원·인증·정책 기반 접근을 위한 제로 트러스트 프레임워크이다.
- **NIST SP 800-207**: 자원 중심 제로 트러스트 아키텍처의 논리 구성·배치 모델 지침이다.

## Ⅰ. 개요

- 정의/개념: 분산 PoP에서 **SD-WAN 연결·SSE 보안 통합**
- 배경/필요성: 중앙 백홀의 **지연·병목·정책 불일치 해소**

### 쉽게 이해하기 (학습용)

- 사용자 가까운 거점에서 신원·기기를 검증하고 허용 응용으로 바로 연결함

## Ⅱ. 특징

- SD-WAN·SSE의 **클라우드 통합 제공**
- 신원·기기·응용·데이터의 **맥락 정책**
- 분산 PoP·백본의 **저지연·고가용 접속**

### 쉽게 이해하기 (학습용)

- 위치가 달라도 같은 신원·기기·데이터 기준으로 경로 선택과 보안 검사를 수행함

## Ⅲ. 구조 및 구성요소

```mermaid
block-beta
  columns 1
  C["사용자·지점·SD-WAN 연결"]
  X["신원·기기·위험 맥락"]
  P["정책 결정·통합 관제"]
  S["SWG·CASB·ZTNA·FWaaS"]
  N["분산 PoP·공급자 백본"]
  C --- X --- P
  P --- S --- N
```

| 구성요소 | 책임 |
|:---|:---|
| 사용자·지점·SD-WAN 연결 | 에이전트·터널·경로 선택 |
| 신원·기기·위험 맥락 | IdP·상태·행위·데이터 입력 |
| 정책 결정·통합 관제 | 공통 정책·로그·위험 판정 |
| SWG·CASB·ZTNA·FWaaS | 웹·클라우드·앱·망 검사 |
| 분산 PoP·공급자 백본 | 근접 처리·거점 간 전송 |

### 쉽게 이해하기 (학습용)

- 가까운 거점에서 경로와 보안을 동시에 적용하고 정책·로그는 일관되게 관리함

## Ⅳ. 흐름도

```mermaid
sequenceDiagram
  participant U as 사용자·지점
  participant C as 연결 제어
  participant P as 정책 결정
  participant E as SSE 검사
  participant A as 대상 응용
  U->>C: 1. 신원·기기·응용 요청
  C->>P: 2. 지연·가용성 기반 PoP 선택
  P->>E: 3. 맥락·위험·데이터 정책 판정
  E->>A: 4. SD-WAN 경로·SSE 검사 집행
  A->>P: 5. 세션 결과·위험 지속 평가
```

**동작 원리**

- **1. 신원·기기·응용 요청**: 사용자·단말·자원 맥락 수집
- **2. 지연·가용성 기반 PoP 선택**: 상태·부하·우회 경로 계산
- **3. 맥락·위험·데이터 정책 판정**: 최소 응용·행위 허용
- **4. SD-WAN 경로·SSE 검사 집행**: 경로·웹·앱·데이터 통제
- **5. 세션 결과·위험 지속 평가**: 행위·장애·로그 정책 환류

### 쉽게 이해하기 (학습용)

- 가까운 거점을 고른 뒤 맥락에 맞는 응용만 연결하고 세션 중 위험도 계속 평가함

## Ⅴ. 종류 및 비교

| 접근 보안 구조 | SASE | SSE | 전통 경계형 |
|:---|:---|:---|:---|
| 적용 기준 | 분산 망·보안 동시 통합 | 연결망 유지·보안 통합 | 사용자·응용 중앙 집중 |
| 핵심 특징 | SD-WAN 경로와 SSE 통합 | SWG·CASB·ZTNA 통합 | 데이터센터 경계 검사 |
| 한계 | PoP 장애·공급자 종속 | 회선·경로 제어 공백 | 백홀 지연·정책 불일치 |

> 요약: 연결과 보안의 통합 범위가 서로 다름

### 쉽게 이해하기 (학습용)

- SSE는 보안 기능, SASE는 광역망 경로까지 함께 통합한 구조임

## Ⅵ. 실무 고려사항 및 대책

| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| SASE 서비스 속성 | **MEF 117 적용** | 기능·정책·연결 SLA 명확화 |
| 제로 트러스트 서비스 | **MEF 118.1 적용** | 신원·정책 집행점 정렬 |
| 자원 중심 접근 | **NIST SP 800-207 연계** | 위치 기반 암묵 신뢰 제거 |

### 쉽게 이해하기 (학습용)

- 재택 사용자는 가까운 PoP로 연결되고 신원·기기 상태·앱 정책을 통과한 트래픽만 업무 자원에 전달된다.

## Ⅶ. 결론

- **응용 분포·PoP 지연·정책·우회성**으로 SASE를 설계한다.

### 쉽게 이해하기 (학습용)

- 사용 위치가 달라도 같은 맥락 기준으로 허용 응용에만 연결하고 거점 장애를 우회해야 함
