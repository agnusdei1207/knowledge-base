...
- 정의/개념: 분산 PoP에서 **SD-WAN 연결•SSE 보안**을 통합 제공하는 구조이다.
- 배경/필요성: 분산 사용자•클라우드 트래픽의 중앙 백홀로 **지연•회선 병목•정책 불일치가 발생한다**.
...
- SD-WAN•SSE의 **클라우드 통합 제공**이 핵심이다.
- 신원•기기•응용•데이터의 **맥락 정책**이 핵심이다.
- 분산 PoP•백본의 **저지연•고가용 접속**이 핵심이다.
...
- **1. 지연•가용성 기반 PoP 선택**: PoP 상태•부하를 반영한 대체 경로를 계산한다.
- **2. 맥락•위험•데이터 정책 판정**: 최소 응용•행위 허용이다.
- **3. SSE 검사 정책 적용**: 웹•클라우드•응용•데이터를 통제한다.
- **4. SD-WAN 경로•접근 집행**: 정책 기반 경로와 세션을 연결한다.
- **5. 세션 위험 지속 평가**: 행위•장애•로그를 정책에 환류이다.
...
- 분산 응용•저지연 필요 시 **SASE**, 기존 WAN 유지 시 **SSE**을 선택한다.
