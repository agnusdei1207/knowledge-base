...
- **정의/개념**: 권한을 역할에 묶고 **역할을 사용자에게 배정하는 모델**
- **배경/필요성**: 사용자별 직접 권한은 **일관성·대량 변경·회수 관리 곤란**
...
1. 지급 업무는 **요청자·승인자 역할 분리**
2. 임시 관리자 역할은 **작업 종료 후 자동 회수**

### 쉽게 이해하기 (학습용)

- 한 사람이 지급을 만들고 직접 승인하지 못하도록 상충 역할을 분리한다.
- 장애 대응용 높은 권한이 작업 종료 뒤 남지 않도록 자동 회수한다.

## Ⅶ. 결론

- 안정된 직무는 **RBAC**, 상충 업무는 **제약 RBAC**
