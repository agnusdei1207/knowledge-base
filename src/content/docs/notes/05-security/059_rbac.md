...
- 정의: 권한을 역할로 묶는 **간접 접근 제어**
- 기존 한계: 직접 권한의 **일관성·변경·회수 곤란**
...
- **사용자-역할-권한 간접 배정**으로 직무별 권한을 표준화한다.
- **역할 계층·세션 활성화**로 공통 권한 상속과 사용 범위를 관리한다.
- **정적·동적 직무 분리**로 상충 역할의 보유·동시 사용을 제한한다.
...
sequenceDiagram
    ...
    S->>P: 2. 동적 직무분리 제약 확인
    U->>P: 3. 자원·행위 접근 요청
    P->>R: 4. 역할 권한·상충 조건 판정
    R->>감사시스템: 5. 허용 행위·사용 이력 기록
```

| 절차 | 설명 |
|:---|:---|
| 배정 역할·세션 활성화 | 필요한 역할만 선택 |
| 동적 직무분리 제약 확인 | 상충 역할 동시 사용 차단 |
| 자원·행위 접근 요청 | 대상·동작·맥락을 제시 |
| 역할 권한·상충 조건 판정 | 권한·정적 제약을 평가 |
| 허용 행위·사용 이력 기록 | 승인 요청만 실행·감사 |
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| RBAC 모델 일관성 | **ANSI INCITS 359-2012 적용** | 계층·직무분리 구조 정렬 |
| 최소 권한·계정 통제 | **NIST SP 800-53 Rev. 5 연계** | 과잉 권한·휴면 계정 축소 |
| 역할 폭발·예외 누적 | **역할 공학·정기 재인증** | 권한 모델 복잡성 억제 |
...
- **직무안정성·역할상속·상충업무**로 RBAC 모델을 결정한다.
