...
- 배경/필요성: 초기 네트워크 보안 환경의 1세대 단순 패킷 필터링(Stateless)은 5-Tuple(출발지/목적지 IP, 포트, 프로토콜) 헤더만 검사하여 TCP 연결 상태를 추적하지 못해 스푸핑 공격에 취약했고, 2세대 상태기반(Stateful Inspection) 방화벽은 대부분의 악성 트래픽이 웹 표준 포트(80/443) 및 TLS 암호화 터널로 은닉되어 침투하는 현대 L7 지능형 위협을 식별하지 못하는 한계에 직면함에 따라, 세션 테이블 기반의 상태 추적과 더불어 L7 심층 패킷 분석(DPI), 애플리케이션 식별(App-ID), 사용자 신원 매핑(User-ID) 및 SSL/TLS 복호화 검사를 통합 수행하는 차세대 방화벽(NGFW: Next-Generation Firewall)을 도입하여 **기본 거부(Default Deny) 기반의 엄격한 경계 접근 통제, 포트 위장 악성 트래픽 차단 및 암호화 위협에 대한 완벽한 가시성(Visibility) 확보**를 달성할 필요
