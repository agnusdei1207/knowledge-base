...
date: "2026-08-04T14:21:12+09:00"
tags:
  - "notes-security"
weight: 121
extra:
...
sequenceDiagram
  ...
  S->>O: 보안 경보
  O->>O: 1. 경보 보강•위험 수준 산정
  O->>A: 보강 경보 전달
  A->>A: 2. 대응 경로•승인 여부 결정
  A->>O: 승인 결과 전달
  O->>O: 3. 최소권한 대응 명령 생성
  O->>C: 대응 명령 전달
  C->>C: 4. 조치 실행•롤백 검증
  C->>O: 조치•롤백 결과 전달
  O->>O: 5. 사건 증적•성과 지표 기록
```

**동작 원리**

1. **경보 보강•위험 수준 산정**: 자산•사용자•위협정보와 업무 영향 결합
2. **대응 경로•승인 여부 결정**: 위험에 따라 자동•승인•수동 경로 결정
3. **최소권한 대응 명령 생성**: EDR•방화벽•계정별 제한 명령 생성
4. **조치 실행•롤백 검증**: 대응 효과•실패 원인•복구 상태 검증
5. **사건 증적•성과 지표 기록**: 결정 근거•오탐•시간•실패율 보존
