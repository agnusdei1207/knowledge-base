...
date: "2026-07-25T13:14:00+09:00"
tags:
  - "notes-security"
weight: 82
extra:
...
- **차분 프라이버시(Differential Privacy)**: 개별 학습 데이터의 포함 여부가 결과에 미치는 영향을 수학적으로 제한하는 기법
- **질의 예산(Query Budget)**: 공격자나 사용자가 허용받은 모델 호출 횟수와 비용의 한도
- **식별자(Identifier, ID)**: 아이디로 읽으며 데이터와 사용자를 구별하는 값
- **응용 프로그래밍 인터페이스(Application Programming Interface, API)**: 에이피아이로 읽으며 모델 호출 규칙을 제공하는 접점
...
- 수렴 후보는 대표 특징이라 원본 오판을 만들 수 있다
- 과적합·작은 집단은 개인 특징 식별 위험을 높인다
