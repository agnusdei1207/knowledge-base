...
- 정의/개념: 모델 역전 공격은 공격자가 대상 모델에 수많은 입력을 질의하고 도출되는 출력 신뢰도(Confidence)나 기울기(Gradient)의 경사하강/경사상승 최적화를 역으로 수행하여, 모델 내부에 숨겨진 학습 데이터의 대표 특징점이나 민감 속성을 거꾸로 시각화•복원해내는 기술이다.
- 배경/필요성: 얼굴 인식, 안면 인증, 의료 진단 모델 등 민감 개인정보를 다루는 AI 서비스의 블랙박스/화이트박스 API를 통해 원본 민감 자산이 복원되어 **학습 정보 노출** 및 개인정보보호법(GDPR/PIPA) 위반 위험이 심각해졌기 때문이다.
...
- **과적합(Overfitting)**: 모델이 일반화 일반 성능 대신 학습 샘플 데이터의 개별 특징을 과도하게 암기한 상태이다.
...
1. **복원 후보 생성**: 공격 대상 클래스(예: 특정 환자 A)에 대응하는 초기 템플릿 노이즈를 생성한다.
2. **후보 최적화**: 대상 모델에 질의하고 도출된 확률값을 기반으로 경사상승 **반복 최적화**를 이행하여 픽셀을 정밀 수정한다.
3. **유사성•식별성 판정**: 출력 결과의 **유사성** 및 **식별성**을 대조하여 실제 원본 안면/의료 데이터가 재구성되었음을 확인한다.
...
- **프라이버시-유용성 균형**을 감안하여 DP-SGD 노이즈 파라미터($\epsilon, \delta$)를 정밀 튜닝하고 **점수 양자화**를 집행한다.
