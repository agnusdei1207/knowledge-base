...
- **차등 프라이버시(Differential Privacy, DP)**: 개인 한 명의 포함 여부가 분석 결과에 미치는 영향을 수학적으로 제한하는 개인정보보호 기법이다.
- **질의 예산(Query Budget)**: 공격자나 사용자가 허용받은 모델 호출 횟수와 비용의 한도
- **식별자(Identifier, ID, 아이디)**: 데이터와 사용자를 구별하는 값
- **응용 프로그래밍 인터페이스(Application Programming Interface, API)**: 외부 시스템이 모델에 입력을 전달하고 결과를 받는 호출 규격과 접점이다.
...
1. 얼굴 인식 API의 **세부 신뢰도 출력 최소화**

### 쉽게 이해하기 (학습용)

- 인증 결과만 필요한 얼굴 인식 API는 세밀한 클래스별 신뢰도 점수를 공개하지 않아 반복 질의로 학습 얼굴 특성을 복원하기 어렵게 한다.

## Ⅶ. 결론

- 모델 반응에서 학습 개인 정보를 복원하는 위험을 줄이기 위해 출력 세밀도·질의 반복성·과적합·복원 식별성을 검토하여, 민감 모델의 정보 노출과 질의를 제한해야 한다.
