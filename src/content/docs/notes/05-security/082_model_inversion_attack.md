...
- **신뢰도**는 모델이 각 예측 결과에 부여한 확신의 정도다.
- **기울기**는 모델 손실이 매개변수나 입력 변화에 따라 달라지는 방향과 크기다.
- **사전 정보**는 데이터 형식•분포•속성에 관해 공격자가 이미 가진 보조 지식이다.
- **과적합**은 모델이 일반 패턴보다 학습 데이터의 개별 특징을 과도하게 기억한 상태다.
- **DP(Differential Privacy)**는 한 개인의 포함 여부가 학습 결과에 미치는 영향을 제한하는 기법이다.

</details>

- 점수•기울기의 **반복 최적화 악용**이 핵심이다.
- 사전 정보가 많고 과적합이면 **복원 정확도 증가**가 핵심이다.
- 출력 최소화•DP의 **개인정보 노출 제한**이 핵심이다.
...
- **유사성**은 복원 후보가 원본 데이터의 특징과 가까운 정도다.
- **식별성**은 복원 결과로 특정 개인을 알아볼 수 있는 정도다.
...
3. **유사성•식별성 판정**: 원본 근접도•노출 위험을 평가한다.
...
- **회원 추론**은 특정 자료가 모델 학습에 사용되었는지 추정하는 공격이다.
- **모델 추출**은 대상 모델의 기능과 판단 경계를 질의로 복제하는 공격이다.
...
- **AI(Artificial Intelligence) 100-2e2025**는 모델 프라이버시 공격을 지식과 접근 수준별로 분류한 보고서다.
- **출력 최소화**는 업무에 필요한 라벨과 정보만 제공해 복원 신호를 줄이는 통제다.
- **질의 예산**은 사용자별 모델 호출 횟수와 비용을 제한하는 통제다.
- **점수 양자화**: 세밀한 연속형 출력 점수를 제한된 단계의 값으로 바꾸어 공격자가 얻는 최적화 신호의 정밀도를 낮추는 통제이다.
- **블랙박스 접근**은 모델의 외부 입력과 출력만 이용하는 공격 조건이다.
- **화이트박스 접근**은 모델의 가중치•기울기 같은 내부 정보까지 이용하는 공격 조건이다.
...
- 출력 정밀도•질의량이 높으면 **점수 최소화•질의 예산**을 적용한다.
