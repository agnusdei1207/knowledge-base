...
- **PIA(Privacy Impact Assessment)**는 개인정보파일 구축•변경 전에 처리 흐름의 침해 위험과 개선사항을 분석하는 평가다.
- **사전 평가**: 시스템 완성 후가 아니라 개인정보 흐름과 구조를 바꿀 수 있는 시점에 위험을 찾아 개선 비용을 줄이는 원칙이다.

</details>

- 정의/개념: **개인정보 영향평가**는 구축•변경 전에 처리 흐름과 침해 위험을 평가하는 절차이다.
- 배경/필요성: 구축 후에는 개인정보 흐름•구조의 **개선 비용이 증가한다**.
...
- **개선조치**는 개인정보 침해 위험을 줄일 통제•담당•기한•완료 증거를 연결한 계획이다.
- **잔여 위험**은 개선조치를 적용한 뒤에도 남아 승인•감시가 필요한 위험이다.

</details>

- **개인정보 흐름도**를 통한 처리 목적•복제 경로의 전 범위를 분석한다.
- 위험•조치•담당자의 **추적 가능성**이 핵심이다.
- **운용체계 변경 재평가**로 기존 범위와 잔여 위험을 갱신한다.
...
- **평가 범위**는 영향평가에 포함할 개인정보파일•시스템•외부 연계•복제 경로다.
- **침해요인**은 개인정보의 기밀성•무결성•가용성에 영향을 주는 위협과 취약점이다.
...
3. **침해요인•위험도 산정**: 가능성•영향•기존 통제를 평가한다.
4. **개선조치 구현•증거 생성**: 담당•기한에 따라 통제를 반영한다.
5. **잔여위험 승인•변경 관리**: 승인 기록과 변경 시를 재평가한다.
...
- **민감정보 파일**은 노출 때 사생활 침해가 큰 민감정보를 처리하는 개인정보파일이다.
- **고유식별정보 파일**은 주민등록번호 등 개인을 고유하게 구별하는 정보를 처리하는 파일이다.
- **연계 파일**은 둘 이상의 개인정보파일을 연결해 결합 식별성이 높아진 파일이다.
- **대규모 파일**은 법정 인원 기준 이상의 개인정보를 처리하는 개인정보파일이다.
...
- 민감도•규모가 법정 기준이면 전 범위 **PIA**, 잔여 위험은 개선 후 승인이 핵심이다.
