...
date: "2026-08-05T17:41:31+09:00"
tags:
  - "notes-security"
weight: 80
extra:
...
- 정의/개념: 외부 자료의 숨은 지시로 행동을 바꾸는 **LLM 간접 입력 공격**
...
- **검색 증강 생성(Retrieval-Augmented Generation, RAG) 유입**: 검색 문서의 공격 지시가 모델 문맥으로 전달되는 경로
...
선의 의미: 외부 자료•검색 문맥•모델 제안•권한 집행의 정적 경계
...
1. **외부 콘텐츠 수집•색인**: 웹•메일•문서의 숨은 지시를 저장
2. **데이터•지침 경계 혼동**: 검색 문장을 실행 명령으로 오인
3. **지연 공격 지시 활성화**: 특정 질의•도구 조건에서 악성 행동 생성
4. **독립 권한•거래 결속 검증 누락**: 모델 제안을 승인 행동처럼 수용
5. **외부 전송•상태 변경 실행**: 과도한 도구 권한으로 실제 피해 발생
