...
- 배경/필요성: RAG(검색 증강 생성), 웹 검색 및 이메일 연동 에이전트 환경에서 외부 웹문서, PDF, 이메일 본문 등에 은밀히 매복된 악성 지시어가 LLM의 문맥 내부로 무방비 병합될 때, 안전하게 인증된 사용자의 높은 시스템 권한이 외부 데이터 작성자에게 불법 전이(Privilege Transfer)되는 신뢰 경계 붕괴(Trust Boundary Collapse) 취약점이 발생함에 따라, MITRE ATLAS AML.T0051.001 및 OWASP LLM01 표준에 기반하여 비신뢰 문맥의 구조적 태깅 격리, Dual-LLM 아키텍처, 독립 도구 게이트웨이(PEP) 및 네트워크 Egress 통제를 결합하는 간접 프롬프트 인젝션 방어 아키텍처를 도입하여 스텔스 매복 악성 지침의 선제적 무력화, 권한 전이를 통한 기밀 데이터 C2 유출 차단 및 비인가 도구 실행 100% 억제를 달성할 필요
...
[간접 프롬프트 인젝션 방어 체계]
├─ RAG 수집 및 문맥 격리 계층
│  ├─ 수집 데이터 정제 (주석·은닉 태그 제거)
│  └─ 비신뢰 문맥 태그 격리 (<untrusted_data>)
├─ Dual-LLM 안전 추론 계층
│  ├─ 1차 격리 LLM: 외부 문서 단순 요약 (도구 권한 무)
│  └─ 2차 제어 LLM: 정제 요약본 기반 사용자 응답 생성
└─ 런타임 집행 및 통신 통제 계층
   ├─ Tool Policy PEP: 사용자 인텐트 거래 결속 대조
   ├─ Egress 방화벽: 외부 비인가 C2 도메인 차단
   └─ 고위험 파괴 액션 인간 승인 (Human-in-the-loop)
```
...
| 구성요소 | 책임 |
|:---|:---|
| 수집 데이터 정제기 | 외부 크롤링 텍스트 및 PDF에서 악성 주석, 은닉 태그, OCR 인젝션 구문을 사전 제거 |
| RAG 문맥 격리기 | 검색된 비신뢰 텍스트 청크를 시스템 지침과 물리적으로 격리하는 메타 태그를 주입 |
| Dual-LLM 프레임워크 | 비신뢰 데이터 처리 전용 LLM과 핵심 비즈니스 도구 실행 LLM의 권한을 분리 |
| Egress 방화벽 (아웃바운드) | AI 에이전트 환경에서 외부 인터넷 C2 서버로의 비인가 데이터 유출 트래픽을 차단 |
| Tool Policy Gateway | AI가 제안한 API 호출의 파라미터가 사용자의 최초 요청과 일치하는지 거래 결속을 검증 |
...
1. 외부 문서 수집 및 정제: 은닉 지시 제거
2. RAG 검색 및 문맥 격리: 비신뢰 태그로 구분
3. Dual-LLM 격리 추론: 처리 모델의 도구 권한 제거
4. 도구 호출 거래 결속 검증: 최초 요청과 호출 대조
5. Egress 차단 및 종결: 비인가 외부 통신 차단

#### 한줄 요약
- 앞단의 정제와 태그 격리는 은닉 지시의 완전 제거를 보장하지 못하므로, 유출이 실제로 완성되는 마지막 지점인 Egress 차단이 비용 대비 방어 효과가 가장 크다.
...
- 외부 비신뢰 데이터 원천과 LLM 추론 엔진이 결합된 현대 RAG 및 자율 에이전트 시스템에서 가장 은밀하고 파괴적인 침해 경로를 방어하는 차세대 인공지능 신뢰 경계 보호(MITRE ATLAS AML.T0051.001 / OWASP LLM01)의 핵심 다계층 보안 체계로 확고히 자리 잡았으며, 다중 에이전트 간 권한 위임 거버넌스로 진화하는 가운데, 실무 RAG 및 에이전트 시스템 구축 시에는 수집 데이터 정제 파이프라인(HTML 주석/은닉 태그 제거) 구축, 검색 문맥을 격리하는 `<untrusted_content>` 태깅 및 비신뢰 데이터 요약 전용 저권한 모델을 분리하는 **Dual-LLM 아키텍처** 적용, 최초 사용자 인텐트와 모델 도구 호출을 대조하는 Transaction Binding PEP 및 외부 C2 통신을 차단하는 Egress 네트워크 화이트리스트 강제를 결합하여 완벽한 간접 인젝션 방어 무결성을 완성
