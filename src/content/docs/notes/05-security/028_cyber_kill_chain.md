...
- 정의/개념: 록히드 마틴(Lockheed Martin)이 제안한 모델로, 사이버 공격의 침해 과정을 7단계(정찰부터 목적달성까지)로 모델화하여 단계별 끊기(Kill) 지점을 식별하는 **사이버 킬체인(Cyber Kill Chain)** 방어 프레임워크
- 배경/필요성: 단일 경계 방화벽/백신 중심 방어 한계 극복 및 다단계 공격 흐름 중 선제적 차단 지점(Intercept Point) 식별 요구
...
- **다중 차단 기회(Multiple Intercept Opportunities)** 체계를 구축하여 앞선 방어선 실효 시 후속 단계 차단
...
2. **전달 & 악용**: 이메일/웹을 통한 페이로드 전달 및 취약점 악용 코드 실행
3. **설치 & C2**: 백도어 자동 설치를 통한 지속성 확보 및 C2 원격 명령 채널 개설
...
- **분석 모델 선택(Analysis Model Selection)**: 침해 흐름(킬체인), 세부 기법(ATT&CK) 및 운영 프로세스(NIST IR)를 통합 결합하는 보안 프레임워크 체정 지침.
