...
- **사이버 킬체인(Cyber Kill Chain)**: 침해 캠페인을 정찰부터 목적 달성까지 7단계로 나누어 차단 기회를 찾는 모델
...
- **정의**: 공격 캠페인을 7단계로 분석하는 모델
- **배경/필요성**: 단계별 관측·차단 책임 배치
...
- 정찰부터 목적 달성까지 7단계로 표현한다
- 앞 단계 차단일수록 피해·대응 비용이 줄어든다
- 여러 보안 흔적을 하나의 캠페인으로 연결한다
- 단계별 예방·탐지·대응 책임을 배치한다
- 유효 계정 공격은 단계 선형성이 약하다
...
|:---|:---|
| 정찰 | 표적·자산·노출 경로 조사 |
| 무기화·전달 | 공격 도구 준비와 목표 전송 |
| 악용·설치 | 코드 실행과 지속 통로 배치 |
| C2 | 원격 명령과 추가 도구 교환 |
| 목적 달성 | 이동·수집·유출·파괴 수행 |
| 단계별 관측·통제 | 증거·차단·대응 책임 연결 |

> 요약: 공격 단계마다 관측·차단 책임을 연결
...
| 절차 | 설명 |
|:---|:---|
| 정찰 수행 | 사람·기술·노출 정보 수집 |
| 무기화 결과 전달 | 악성 파일·링크·도구 준비 |
| 전달 경로 진입 | 메일·웹·매체로 목표에 도달 |
...
| 비교축 | 사이버 킬체인 | MITRE ATT&CK | 사고 대응 수명주기 |
|:---|:---|:---|:---|
| 핵심 특징 | 캠페인의 단계 흐름 분석 | 세부 공격 행동 분류 | 탐지 뒤 대응 활동 관리 |
| 적용 기준 | 조기 차단 지점 설계 | 헌팅·탐지 규칙 설계 | 격리·제거·복구 운영 |
| 주요 위험 | 선형 순서의 과도한 단순화 | 시간·캠페인 문맥 부족 | 공격 기법 세부성 부족 |
...
- **사례 1**: 각 단계에 로그·경보·대응 소유자 지정
- **사례 2**: 유효 계정처럼 전달 없는 경로를 별도 분석
- **사례 3**: ATT&CK 기법으로 각 단계의 행동을 세분화
- **사례 4**: 모의공격으로 실제 차단·대응을 검증

### 쉽게 이해하기 (학습용)

- 사례 1은 표에 제품만 적지 않고 어떤 증거를 누가 보고 어디서 차단할지 실제 책임을 연결한다.
- 사례 2는 탈취 계정으로 바로 로그인한 공격을 전달·설치 단계가 없다는 이유로 분석에서 놓치지 않게 한다.
- 사례 3은 같은 악용 단계 안에서도 공격자가 사용한 세부 기법과 필요한 탐지 데이터를 구체화한다.
- 사례 4는 일곱 칸을 모두 채웠다는 보고보다 우회 경로에서 경보와 차단이 실제 작동하는지 확인한다.

## Ⅶ. 결론

- 공격 흐름을 분석하면 단계별 독립 차단을 중첩한다.
