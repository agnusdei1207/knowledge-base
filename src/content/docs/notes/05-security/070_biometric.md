...
date: "2026-07-31T02:03:02+09:00"
tags:
  - "notes-security"
weight: 70
extra:
...
- **등오류율(Equal Error Rate, EER)**: FMR과 FNMR이 같아지는 지점의 오류율이다.
...
- **판정 엔진(Decision Engine)**: 유사도와 임계값으로 수락·거부·대체 인증을 결정하는 기능이다.
- **성능 편향**: 환경·센서·사용자 집단에 따라 오류율이 달라지는 현상이다.
- **비가역 노출**: 생체 정보가 유출돼도 신체 특징 자체를 교체할 수 없는 위험이다.

## Ⅰ. 개요

- 정의/개념: 생체 표본과 등록 템플릿의 유사도를 판단하는 **사용자 인증 방식**
...
- **FMR·FNMR 상충**: 임계값 변화에 따른 확률 오류 교환
...
- **2. PAD 통과 특징값 전달**: 품질·위조 검사 후 특징 추출
- **3. 템플릿 유사도 점수 전달**: 보호 등록값과 비교 결과 제공
