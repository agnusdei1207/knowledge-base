...
- **정의**: 생체 특징의 유사도로 본인을 판정
- **배경/필요성**: 빠른 사용자 확인과 비밀번호 의존 축소
- **핵심**: 오류율·위조 탐지·템플릿 보호
...
| 주요 위험 | 템플릿 유출·편향 | 복제 지문·표면 상태 | 사진·영상·조명 | 인공 안구·정렬 실패 |
...
- **사례 1**: 단말 보안 영역에서 지문을 비교
- **사례 2**: 얼굴 인증에 영상·마스크 PAD 적용
- **사례 3**: 사용자 집단별 FMR·FNMR을 점검
- **사례 4**: 생체 실패에 강한 대체 인증 제공

### 쉽게 이해하기 (학습용)

- 사례 1은 원본 생체와 템플릿이 중앙 서버에 모여 대규모로 유출되는 위험을 줄인다.
- 사례 2는 등록 사용자 사진이나 영상만 카메라에 보여 주는 단순 위조를 실제 사람과 구분한다.
- 사례 3은 특정 피부색·나이·장애 집단에서 오수락이나 오거부가 더 자주 발생하는 편향을 찾는다.
- 사례 4는 상처·질병·센서 장애 때 사용자를 영구히 거부하지 않으면서 더 약한 복구 절차로 우회하지 않게 한다.

## Ⅶ. 결론

- 생체 인증이면 PAD와 로컬 템플릿 보호를 적용한다.
