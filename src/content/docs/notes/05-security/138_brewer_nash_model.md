...
- 정의/개념: 접근 이력 기반 **경쟁 회사 간 정보흐름 통제**
- 배경/필요성: 고정 역할이 못 잡는 **사후 이해충돌 방지**
...
sequenceDiagram
  ...
  S->>E: 1. 주체·객체·행위 요청
  E->>H: 2. 과거 회사 접근 이력 조회
  H->>O: 3. CD·COI·정제 여부 확인
  O->>A: 4. 읽기·쓰기 흐름 판정
  A->>H: 5. 허용 결정·이력 원자 갱신
```

**동작 원리**

- **1. 주체·객체·행위 요청**: 읽기·쓰기 대상 제시
- **2. 과거 회사 접근 이력 조회**: 주체별 선택 회사 확인
- **3. CD·COI·정제 여부 확인**: 객체 분류·공유 예외 검증
- **4. 읽기·쓰기 흐름 판정**: 경쟁 접근·정보 이동 검사
- **5. 허용 결정·이력 원자 갱신**: 동시 요청 우회 방지
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 동적 정보흐름 집행 | **NIST SP 800-53 AC-4 연계** | 흐름 정책·집행점 명확화 |
| 이해충돌 정책 근거 | **Brewer·Nash IEEE 1989 적용** | COI·CD·이력 규칙 정립 |
| 동시 요청·정제 우회 | **원자 갱신·재식별 검증** | 경합·예외 유출 방지 |
...
- **COI·접근 이력·쓰기 흐름·정제성**으로 접근을 결정한다.
