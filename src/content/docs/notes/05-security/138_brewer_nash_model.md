...
- 정의/개념: 과거 접근 이력에 따라 경쟁 회사 데이터 접근을 동적으로 제한하는 **Brewer-Nash 기밀성 보안 모델**이다.
- 배경/필요성: 고정 역할만으로는 이전 고객 접근에 따른 **이해충돌 판정할 수 없다**.
...
- 최초 접근 이력의 **동적 장벽 형성**이 핵심이다.
- COI별 독립 회사 선택의 **업무 유연성**이 핵심이다.
- 읽기•쓰기 규칙의 **양방향 흐름 통제**가 핵심이다.
...
- **1. COI별 회사 접근 이력 조회**: 주체가 이미 선택한 회사 목록 조회한다.
- **2. CD•COI•정제 여부 조회**: 경쟁 관계와 공유 예외 분류를 확인한다.
- **3. 이해충돌 장벽 접근 판정**: 이력•분류를 대조해 허용 여부를 결정한다.
- **4. 접근 결정•이력 원자적 기록**: 동시 요청의 이력 검사 우회 차단한다.
...
- 같은 COI에서는 **기존 선택 회사만 허용**, 정제 정보는 재식별을 검증한다.
