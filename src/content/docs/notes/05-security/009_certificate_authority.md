...
- **인증기관(CA)** 은 신청자의 신원과 공개키 통제권을 확인하여 공개키•주체•용도를 연결한 인증서에 서명한다.
...
- **CT 로그** 는 인증서 발급 기록을 공개 검증 가능한 형태로 남겨 오발급을 탐지하게 한다.
...
- **하드웨어 보안 모듈(Hardware Security Module, HSM)**: 인증기관 개인키를 장치 밖으로 노출하지 않고 서명 연산과 접근 통제를 수행하는 보안 장치이다.

</details>

| 인증기관(Certificate Authority, CA) 계층 | 신뢰 역할 | 운영 원칙 |
|:---|:---|:---|
...
- **HSM•CT 감시** 는 CA 개인키를 격리하고 공개 발급 기록을 확인하여 키 오용과 인증서 오발급을 통제한다.
...
- **루트 CA•발급 CA 분리** 는 최상위 위임과 일상적인 온라인 발급을 서로 다른 키와 계층에 맡기는 원칙이다.
