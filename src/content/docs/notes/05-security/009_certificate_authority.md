...
- 정의/개념: **CP**·**CPS**에 따라 인증서를 발급·폐기하는 신뢰 주체(**CA**)
...
- **ACME (Automatic Certificate Management Environment, RFC 8555)**: 웹 서버와 CA 간에 DNS/HTTP 챌린지를 통해 사람의 개입 없이 인증서를 자동 발급·갱신하는 표준 프로토콜.

</details>

- **Air-Gapped Offline Root CA**를 정점으로 한 Root·Intermediate·Issuing 3계층 역할 분리
...
| CT Log Client | **CT Log** 사전 인증서 제출과 SCT 수신 |
...
- **EST (Enrollment over Secure Transport, RFC 7030)**: TLS 위에서 기존 인증서나 자격증명으로 단말을 먼저 인증한 뒤 CSR을 제출해 인증서를 받는 기기 등록 프로토콜로, SCEP의 약한 공유 비밀 인증을 TLS 상호 인증으로 대체함.
...
| 검증 방식 | 서류·조직 심사 | **EST** TLS 인증·등록 정보 | DNS-01·HTTP-01 |
