...
- 정의/개념: 공개값 교환으로 같은 공유 비밀을 도출하는 **DH 키 합의**
- 배경/필요성: 대칭키 직접 전달에는 **통신 경로 직접 전송 위험**이 있다.

#### 한줄 요약

- 비밀값 전송 없이 **공개값 교환•모듈러 연산**으로 공유 비밀 합의
...
| 구성요소 | 책임 |
|:---|:---|
| 도메인 매개변수 | 검증된 **그룹**•**생성원** 정보 제공 |
| 임시 키 쌍 생성 | 세션별 **개인값**•**공개값** 생성 |
| 공유 비밀 계산 | 상대 공개값과 개인값으로 **공유 비밀** 계산 |
| 키 유도 함수 | 공유 비밀에서 **세션키** 파생 |


#### 한줄 요약

- **임시 키쌍·공유 비밀 계산·KDF**로 세션키 파생
...
1. **임시 키쌍 생성**: 검증된 도메인 매개변수에서 세션별 키 쌍 생성
2. **공개값•기록 검증**: 서명과 공개값의 **범위•군 소속** 검증
3. **공유 비밀 계산**: 상대 공개값과 내 개인값으로 동일 비밀 계산
4. **세션키 도출**: **KDF**로 본문 암호화용 세션키 파생


#### 한줄 요약

- **공개값 유효성•상대 신원** 검증 후 공유 비밀 계산
...
- 순방향 비밀성은 **DHE**, 작은 키와 효율은 **ECDHE** 선택
...
- **RFC 7919•RFC 7748** 검증 그룹과 **TLS 인증** 적용
...
- 순방향 비밀성이 필요하면 **인증된 ECDHE**, 기존 정적 호환만 정적 DH 선택

#### 한줄 요약

- **PKI 인증•검증 그룹•일회성 ECDHE**를 함께 적용
