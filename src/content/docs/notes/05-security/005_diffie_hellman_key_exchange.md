...
- **정의/개념**: 공개값 교환으로 같은 공유 비밀을 계산
- **배경/필요성**: 대칭키 전송 없는 세션키 원재료 설정
...
- 인증 없는 공개값은 MITM을 허용한다
- 임시 개인값 폐기는 과거 세션 복구를 막는다
- 공개값 검증은 약한 공유 비밀을 방지한다
...
| 주요 위험 | 장기키 유출 시 과거 세션 노출 | 큰 매개변수 처리 비용 | 곡선·공개값 검증 누락 |
