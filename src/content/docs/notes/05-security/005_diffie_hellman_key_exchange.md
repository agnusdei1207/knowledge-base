...
| 절차 | 설명 |
|:---|:---|
| 그룹·공개값 검증 | 안전한 그룹과 값 확인 |
| A 인증 공개값 전달 | A 신원·임시 공개값 전송 |
| 상대 신원·기록 검증 | 인증서와 협상 기록 확인 |
| B 인증 공개값 전달 | B 신원·임시 공개값 전송 |
| 공유 비밀·세션키 도출 | KDF로 방향별 키 분리 |
...
| 곡선·공개값 구현 | **IRTF RFC 7748 X25519** | 상호운용성 확보 |
...
- **그룹 안전성·상대 인증·순방향 비밀성**으로 ECDHE를 결정한다.
