...
- **디피-헬만 키 교환(Diffie-Hellman Key Exchange, DH 키 교환)**은 비밀값을 직접 전송하지 않고 공개값만 교환하여 양측이 같은 공유 비밀을 계산하는 키 합의 방식이다.

</details>

- 정의/개념: 공개값 교환으로 같은 비밀을 계산하는 **DH 키 합의**이다.
...
- 비밀값 미전송의 **공유 비밀 합의**가 핵심이다.
- 임시 개인값의 **순방향 비밀성**이 핵심이다.
- 인증 결합의 **중간자 공격 차단**이 핵심이다.
...
가지의 의미: 매개변수•키 생성•비밀 계산•키 분리 책임을 나눈 구조다.
...
1. **임시 키쌍 생성**: 안전한 그룹에서 세션별 개인값•공개값을 생성한다.
2. **공개값•기록 검증**: 상대 공개값과 인증서•협상 기록을 확인한다.
3. **공유 비밀 계산**: 상대 공개값과 개인값을 결합한다.
4. **세션키 도출**: KDF로 용도•방향별 키를 분리한다.
...
- 유한체 호환은 **DHE**, 작은 키의 순방향 비밀성은 **ECDHE**을 선택한다.
