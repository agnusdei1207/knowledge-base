...
```text
[도메인 매개변수]---[임시 키 생성기]---[공유 비밀 계산기]---[키 유도 함수]
```

선의 의미: 각 선은 검증된 그룹•생성원, 개인값•공개값, 상대 공개값과 개인값의 공유 비밀 및 용도•통신별 세션키가 서로 결속되는 키 합의 관계를 나타낸다.
...
```mermaid
sequenceDiagram
    participant L as 로컬 참여자
    participant R as 상대 참여자
    L->>L: 1. 임시 키쌍 생성
    L->>R: 인증된 공개값 전달
    R-->>L: 상대 인증 공개값 전달
    L->>L: 2. 공개값•기록 검증
...
1. **임시 키쌍 생성**: 안전한 그룹에서 세션별 개인값•공개값 생성
2. **공개값•기록 검증**: 상대 공개값과 인증서•협상 기록 확인
