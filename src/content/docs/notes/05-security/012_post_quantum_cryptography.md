...
- 배경/필요성: RSA·DH·ECC의 양자 공격 취약성과 HNDL 위험에 대응해 장기 기밀성 확보 필요
...
2. **ML-KEM 공개키 전달**: 송신자에게 캡슐화 키 제공
3. **공유 비밀·암호문 캡슐화**: 공개키로 비밀과 암호문 생성
4. **캡슐화 암호문 전달**: 개인키 소유자에게 복구값 제공
5. **디캡슐화·세션키 도출**: 개인키 복구 후 KDF 적용
...
- **보존기간·역할·크기·호환성**으로 PQC 전환순서를 결정한다.
