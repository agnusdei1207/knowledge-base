...
- 정의/개념: 양자•고전 공격에 견디도록 설계한 **공개키 암호군**이다.
- 배경/필요성: 쇼어 알고리즘으로 **RSA•DH•ECC 장기 기밀성 위협**이 발생한다.
...
- 기존 네트워크에서 동작하는 **양자 채널 불필요성**이 핵심이다.
- **KEM•전자서명**의 세션키 설정•주체 인증 역할 분담이 핵심이다.
- 현재 암호문을 미래에 해독하는 **HNDL 위험의 선제 대응**이 핵심이다.
- **키•암호문•서명 크기 증가**로 패킷•장비 한계 초과가 핵심이다.
...
1. **ML-KEM 키 쌍 생성**: 공개키 배포•개인키를 격리한다.
2. **공유 비밀•암호문 캡슐화**: 공개키로 비밀과 암호문을 생성한다.
3. **공유 비밀 디캡슐화**: 개인키로 동일 비밀을 복구한다.
4. **양측 세션키 도출**: 같은 비밀에 동일 KDF을 적용한다.
...
| **해시 기반 양자내성 서명** | **NIST FIPS 205 SLH-DSA** | **격자 외 서명 대안** |
...
- 장기 기밀 데이터는 **ML-KEM**, 장기 인증은 **ML-DSA** 우선을 전환한다.
