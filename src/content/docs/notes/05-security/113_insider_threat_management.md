...
date: "2026-08-05T12:25:00+09:00"
tags:
  - "notes-security"
weight: 113
extra:
...
```text
                     ...
                     [UEBA•위험 신호]
                            |
                  [협업 조사•비례 대응] -- [프라이버시•증거 보호]
```

선의 의미: 인사 상태와 역할•접근 권한을 UEBA 위험 신호의 업무 기준선에 연결하고, 협업 조사•비례 대응을 프라이버시•증거 보호 경계로 제한하는 정적 관리 구조
...
sequenceDiagram
  ...
  H->>H: 1. 인사 변동 상태 확정
  H->>I: 인사 변동 정보 전달
  I->>I: 2. 역할•권한•업무 기준선 갱신
  I->>A: 기준선 전달
  A->>A: 3. 이상 행위•위험도 분석
  A->>R: 위험 근거 전달
  R->>L: 조사 승인 요청
  L->>L: 4. 적법 범위•비례 대응 판정
  L-->>R: 승인•제한 결정
  R->>R: 5. 조사 결과•탐지 기준 보정
  R->>A: 보정 기준 전달
```
