...
- **공개키 기반구조(Public Key Infrastructure, PKI)**는 인증서의 발급•검증•폐지를 통해 공개키와 소유자의 신원을 연결하는 신뢰 체계이다.

</details>

- 정의/개념: 인증서 수명주기로 공개키와 신원을 연결하는 **PKI 신뢰 체계**이다.
...
- **인증서 폐지 목록(Certificate Revocation List, CRL)**은 폐지된 인증서의 일련번호와 폐지 정보를 배포한다.
- **온라인 인증서 상태 프로토콜(Online Certificate Status Protocol, OCSP)**은 특정 인증서의 현재 상태를 온라인으로 조회한다.

</details>

- **루트 CA•중간 CA** 계층에 따른 발급 권한 위이다.
- **신뢰 기준점부터 가입자까지** 인증서 체인을 검증한다.
- 유효기간과 별개인 **CRL•OCSP 폐지 상태**를 확인한다.
...
- **루트 CA**는 검증자가 미리 신뢰하는 최상위 인증기관이다.
- **중간 CA**는 루트 CA에서 제한된 인증서 발급 권한을 위임받는 기관이다.
...
가지의 의미: 신뢰점•발급•등록•상태 배포•검증 책임을 나눈 구조다.
...
- **CA 서명**은 인증기관이 공개키와 주체 정보를 인증서로 연결하는 암호학적 증명이다.
...
1. **신원•키 통제권 검증**: 신청자와 공개키 소유 관계를 확인한다.
2. **검증된 발급 요청**: 신원•키 통제권 확인 결과를 전달한다.
3. **인증서 서명**: CA 서명으로 공개키•주체를 연결한다.
4. **인증서 경로 검증**: 이름•용도•기간•신뢰 기준점을 판정한다.
5. **폐지 상태 조회**: CRL•OCSP로 현재 유효성을 확인한다.
...
- **하드웨어 보안 모듈(Hardware Security Module, HSM)**은 CA 개인키를 외부로 노출하지 않고 내부에서 인증서 서명을 수행한다.
- **상호 전송 계층 보안(mutual Transport Layer Security, mTLS)**은 서버와 클라이언트가 인증서를 교환하여 양쪽 신원을 검증하는 방식이다.
- **의견 요청 문서(Request for Comments, RFC) 5280**은 인터넷 X.509 인증서•CRL과 경로 검증 절차를 규정한다.
- **RFC 6960**은 인증서의 현재 폐지 상태를 온라인으로 질의하고 응답하는 OCSP를 규정한다.
...
- 불특정 검증자는 **공개 PKI**, 관리 단말은 **사설 PKI**을 선택한다.
