...
1. **탐지 사례·로그 정의**: 공격 행위·필수 증적을 명시
2. **원본 로그 수집**: 에이전트·API로 전달
3. **파싱·시간·필드 검증**: 유실·시각·스키마를 확인
4. **상관분석·경보 생성**: 자산·신원 맥락을 연결
5. **적중·오탐 규칙 조정**: 운영 결과로 임계값 보정
