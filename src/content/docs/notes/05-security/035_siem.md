...
- 정의/개념: **SIEM**은 이종 로그를 정규화•상관 분석해 **경보•조사 근거**를 생성하는 플랫폼이다.
- 배경/필요성: 분산 로그만으로는 **시간•자산•신원 공격 맥락 연결할 수 없다**.
...
- 이종 로그의 **공통 스키마 정규화**가 핵심이다.
- 시간•자산•신원 기반 **이벤트 상관분석**이 핵심이다.
...
5. **규칙•임계값 튜닝**: 적중•오탐 결과로 분석 조건을 보정한다.
...
- 감사•검색은 **로그 관리**, 실시간 상관은 **SIEM**, 장기 재분석은 **데이터 레이크**를 선택한다.
