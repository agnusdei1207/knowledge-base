...
- 정의/개념: **APT**는 특정 조직이나 자산을 목표로 장기간 은밀한 접근을 유지하며 정보를 탈취하거나 시스템에 영향을 주는 **표적 침해 활동**이다.
- 배경/필요성: 단일 IoC•호스트 대응으로는 **반복 TTP•대체 경로를 캠페인으로 연결할 수 없다**.
...
| 핵심 업무 자산(Crown Jewel) | **공격자가 수집•유출•파괴하려는 대상** |
...
- **익스플로잇(Exploit)**은 소프트웨어 취약점을 이용하여 비인가 코드 실행이나 권한 획득을 시도하는 공격 수단이다.
- **데이터 유출**은 수집한 내부 정보를 공격자 인프라 등 조직의 통제 밖으로 반출하는 행위다.
...
- 반복 TTP로 **캠페인 추적**, 대체 계정•C2는 **동시 축출**이 핵심이다.
