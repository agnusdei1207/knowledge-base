...
sequenceDiagram
    ...
    participant SOC
    분석가->>검증기: 1. 사건·속성 등록
    검증기->>MISP: 2. 분류·갤럭시 보강
    MISP->>검증기: 3. 상관·관측 품질 검증
    MISP->>공유그룹: 4. TLP·공유그룹 게시
    SOC->>MISP: 5. 적중·오탐 결과 환류
```

| 절차 | 설명 |
|:---|:---|
| 사건·속성 등록 | 출처·속성을 이벤트로 등록 |
| 분류·갤럭시 보강 | 문맥·취급 태그를 부여 |
| 상관·관측 품질 검증 | 중복·오탐·신뢰도를 확인 |
| TLP·공유그룹 게시 | 배포 경계·대상을 설정 |
| 적중·오탐 결과 환류 | 소비 결과로 지표를 갱신 |
...
- **사건맥락·지표품질·공유경계**로 MISP 운영범위를 결정한다.
