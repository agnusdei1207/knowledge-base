...
- 표준 **분류체계** 및 **갤럭시** 기반 고차원 위협 태깅.
- 지표 간 상관관계 자동 분석 및 **관측** 데이터를 활용한 유효성 검증.
...
2. **경고목록•상관•관측 검증**: **경고목록** 대조 및 **관측** 데이터를 통한 유효성 검증.
3. **TLP•공유그룹 게시**: **TLP(Traffic Light Protocol)** 및 **공유그룹** 기반 차등 배포.
