...
- 정의/개념: **SIEM 탐지•분석**과 **SOAR 조사•조치**를 연결한 관제 구조이다.
- 배경/필요성: 탐지•조치 시스템 분리로 **사건 인계•대응 지연**이 발생한다.
...
- SIEM의 **로그 상관분석•경보 생성**이 핵심이다.
- SOAR의 **정보 보강•플레이북 실행**이 핵심이다.
- 사건 ID 기반 **폐쇄 루프 결과 환류**가 핵심이다.
...
4. **격리•차단•복구 실행**: 최소 권한 API로 승인 조치를 수행한다.
5. **실행 증적•상태 검증**: 실제 결과 확인과 사건 기록을 갱신한다.
...
- 탐지•증거는 **SIEM**, 조사•조치는 **SOAR**, 사건 ID로 결과 폐쇄 루프를 연결한다.
