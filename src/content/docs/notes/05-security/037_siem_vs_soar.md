...
[SIEM·SOAR 폐쇄 루프 결합]
├─ 탐지 계층 (SIEM)
│  └─ 상관분석 엔진 (이종 로그 수집·위협 식별)
├─ 연동 인터페이스
│  └─ 표준 인계 규약 (JSON 스키마 데이터 전달)
├─ 대응 오케스트레이션 (SOAR)
│  ├─ 플레이북 엔진 (위협 선별·CTI 맥락 보강)
│  └─ 실행 커넥터 (방화벽·EDR·AD 제어 API)
└─ 환류 계층
   └─ 폐쇄 루프 환류 모듈 (룰셋 자동 튜닝)
```
...
| 구성요소 | 책임 |
|:---|:---|
| SIEM 탐지 엔진 | 이종 로그 수집, 표준 스키마 정규화, 다차원 시계열 상관분석 및 위협 경보 생성 |
| 인계 인터페이스 | SIEM 알람 사건 객체를 SOAR로 무손실 전달하기 위한 표준 REST/JSON 규약 제공 |
| SOAR 플레이북 엔진 | 위협 선별, CTI 맥락 보강, 이종 장비 차단 명령 오케스트레이션 자동화 |
| 타격 실행 커넥터 | 방화벽, EDR, AD 등과 연동되어 실제 호스트 격리 및 계정 잠금 명령 집행 |
| 폐쇄 루프 환류 모듈 | SOAR의 차단 결과 및 오탐 판정을 SIEM 탐지 룰셋으로 피드백하여 정밀도 개선 |
