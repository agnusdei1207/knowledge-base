...
sequenceDiagram
    ...
    participant 승인자
    participant 보안시스템
    로그원천->>SIEM: 1. 로그·탐지 근거 생성
    SIEM->>SOAR: 2. 사건 ID·위험도 인계
    SOAR->>승인자: 3. 현재 상태·영향 검증
    승인자->>보안시스템: 4. 승인 조치·결과 확인
    보안시스템->>SIEM: 5. 실행 결과·탐지 환류
```

1. **로그·탐지 근거 생성**: 원본 로그를 상관·판정
2. **사건 ID·위험도 인계**: 증거·대상·우선순위 전달
3. **현재 상태·영향 검증**: 오탐·업무 영향·가역성 확인
4. **승인 조치·결과 확인**: 최소 권한 실행·상태 검증
5. **실행 결과·탐지 환류**: 규칙·플레이북을 개선
...
| 플레이북 이식성 | **OASIS CACAO 2.0 적용** | 대응 절차 상호운용 |
...
- **로그근거·조치영향·실행결과**로 SIEM·SOAR 연계를 결정한다.
