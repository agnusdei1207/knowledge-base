...
- 배경/필요성: SIEM을 단독으로 운영할 경우 수많은 경보를 수동으로 분석·처리하는 과정에서 대응 지연(MTTR 증가)과 알람 피로도가 누적되고, SOAR를 단독으로 운영할 경우 전사 빅데이터 상관분석 기반의 위협 탐지 트리거가 부재하여 자동화 대응을 개시하지 못하는 구조적 한계를 가짐에 따라, 전사 보안 로그 수집 및 실시간 상관분석을 담당하는 SIEM(눈과 귀)과 이종 보안 장비 API 제어 및 표준 플레이북 기반 자동 격리를 담당하는 SOAR(손과 발)를 상호보완적으로 결합하여 **탐지부터 격리·복구까지의 전 주기 폐쇄 루프(Closed-Loop) 완성, 평균 대응 시간(MTTR)의 획기적 단축 및 탐지 룰셋 피드백 자동 튜닝**을 달성할 필요
