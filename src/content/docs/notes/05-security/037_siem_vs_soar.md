...
- 정의/개념: **SIEM(SIEM)**의 탐지•분석 기능과 **SOAR(SOAR)**의 오케스트레이션•자동 대응 기능을 통합 연계한 관제 구조.
- 배경/필요성: 단일 시스템 운영의 한계 극복 및 탐지-조치 간 병목 현상 해소를 통한 MTTR(평균 대응 시간) 단축.
...
- 공통 **사건 ID(Incident Correlation ID)** 기반 대응 결과를 **폐쇄 루프(Closed-loop Feedback)**로 환류하여 연속성 확보.
...
- **신원 제공자(Identity Provider, IdP)**: 중앙 신원 및 계정 인증을 관리하여 위협 계정 자격을 수시간 내 회수•잠금하는 시스템에 해당함.
...
- **승인•정책 분기(Approval & Policy Branching)**: 비즈니스 영향도 및 역가역성에 따라 수동/승인/자동 실행을 결정하는 단계.
...
4. **격리•차단•복구 실행**: **최소 권한 응용 프로그래밍 인터페이스(Least Privilege API)** 기반 제어 명령 호출.
...
- **대응 수행(Response Execution / Orchestration)**: 승인된 워크플로 및 커넥터를 통해 external 장비 제어 조치를 완결하는 SOAR의 기능적 책임.
