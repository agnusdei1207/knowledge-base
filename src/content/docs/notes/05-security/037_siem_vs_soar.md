...
- **보안 정보 및 이벤트 관리(Security Information and Event Management, SIEM)**: '에스아이이엠'으로 읽고 영문 머리글자로 로그를 상관 분석해 경보와 조사 근거를 만드는 체계를 표시
- **보안 오케스트레이션·자동화·대응(Security Orchestration, Automation and Response, SOAR)**: '에스오에이알'로 읽고 영문 머리글자로 사건 조사·승인·조치를 자동화하는 체계를 표시
...
- **정의/개념**: SIEM은 탐지, SOAR는 대응을 담당함
- **배경/필요성**: 탐지 근거와 조치 상태를 연결해야 함
...
- SIEM은 로그 수집·분석·경보에 집중한다
- SOAR는 사건 보강·승인·조치에 집중한다
- SOAR는 여러 보안 시스템 경보를 직접 받는다
- 데이터·사건 상태·조치 권한 경계를 정한다
...
| 판단 기준 | SIEM | SOAR |
|:---|:---|:---|
| 핵심 특징 | 대량 로그를 상관해 경보·시간선 생성 | 사건을 보강해 승인·조치·복구 실행 |
| 적용 기준 | 탐지 근거 필요 시 | 자동 대응 필요 시 |
| 주요 위험 | 오탐·로그 품질 저하로 잘못된 경보 | 잘못된 경보가 대규모 자동 조치로 확산 |
...
1. 단말 악성 경보는 현재 상태 확인 후 격리
2. 계정 탈취 사건은 동일 ID로 잠금 결과 환류
3. 연동 장애는 사건 큐로 유실·중복 방지
