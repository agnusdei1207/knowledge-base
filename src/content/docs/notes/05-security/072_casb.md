...
- 정의/개념: SaaS의 접근•데이터•위협 정책을 중개하는 **CASB 통제 지점**이다.
- 배경/필요성: 경계 보안으로는 **SaaS 사용•저장 데이터 파악할 수 없다**.
...
- **SaaS 사용•위험 발견**: 로그•트래픽으로 서비스 식별이다.
- **맥락 평가**: 신원•기기•행위•민감도 결합한다.
- **다중 집행 방식**: 프록시•API•DLP로 시점별을 통제한다.
...
- **1. 신원•기기•데이터 맥락 확인**: 사용자•행위•등급 정보를 확인한다.
- **2. 보호 조치 결정**: 허용•차단•암호화•MFA을 선택한다.
- **3. 인라인 DLP•접근 정책 집행**: 허용된 SaaS 동작만 실시간 중계이다.
- **4. 저장 데이터•공유 정책 평가**: API 상태와 민감도•권한 대조이다.
- **5. 위반 조치•정책 환류**: 공유 회수•격리와 탐지 규칙 보정이다.
#### 한줄 요약
...
- 사용 발견은 **로그**, 실시간 차단은 **인라인 프록시**, 저장 데이터는 **API 점검**이 핵심이다.
