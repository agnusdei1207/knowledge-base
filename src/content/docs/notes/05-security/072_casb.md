...
- **CASB**: 사용자와 클라우드 서비스 사이에서 접근·데이터·위협·준수 정책을 중개하는 보안 통제 지점이다.
- **SaaS**: 공급자가 운영하는 완성된 애플리케이션을 인터넷으로 제공하는 서비스 모델이다.
...
- **클라우드 발견·그림자 IT**: 로그와 트래픽에서 조직 승인 없이 사용되는 SaaS까지 식별하고 서비스 위험을 분류하는 기능이다.
- **프록시·API·DLP**: 프록시는 실시간 요청, API는 저장 상태를 점검하며 DLP는 민감정보의 저장·전송·반출을 통제한다.
...
- **서비스 위험·예외 증적**: SaaS의 보안·법적·사고 위험을 평가하고 정책 예외의 승인 근거·범위·만료·사용 이력을 보존한다.
...
- **인라인 검사·API 사후 점검**: 사용자 요청 경로에서 업로드를 즉시 검사하고 API로 이미 저장된 데이터와 공유 권한을 다시 확인한다.
- **보호 조치 결정**: 요청 맥락과 데이터 민감도에 따라 허용·차단·암호화·MFA 중 적절한 대응을 선택하는 과정이다.
- **MFA**: 고위험 클라우드 작업에서 서로 다른 인증 요소를 추가로 확인하는 다중 인증 방식이다.
...
- **5. 위반·저장 맥락 반영**: 사후 회수와 정책 보정에 저장 상태 반영
#### 한줄 요약
...
- **로그·인라인 프록시·API 연동**: 로그는 사용 현황 발견, 인라인은 실시간 차단, API는 저장 데이터와 설정의 사후 점검에 적합하다.
- **TLS 검사 부담**: 암호화 통신을 인라인에서 검사하려면 복호화·재암호화가 필요해 지연과 인증서 운영 비용이 생기는 한계이다.
...
- **CSA CCM v4.1**: 클라우드 보안의 통제 영역·책임·평가 항목을 제공하여 SaaS의 통제 공백을 찾게 하는 프레임워크이다.
...
- **로그·인라인·API 선택**: 사용 현황 발견, 실시간 차단, 저장 데이터·권한 점검 중 필요한 집행 시점에 맞춘 판단이다.
