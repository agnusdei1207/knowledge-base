...
- 정의/개념: **CASB**는 가시성(Visibility), 데이터 보안(Data Security), 위협 방어(Threat Protection), 준법성(Compliance)의 4대 핵심 축을 바탕으로 기업의 **SaaS** 사용을 실시간 통제하는 게이트웨이 보안 제어 기술이다.
- 배경/필요성: 승인되지 않은 비인가 클라우드 서비스인 그림자 IT(Shadow IT)의 확산, 민감정보 데이터 유출, 클라우드 계정 도용 공격에 대한 기업 중앙의 보안 가시성 및 데이터 통제력 확보가 시급해졌기 때문이다.
...
| SaaS | 정책 집행 통제 대상이 되는 멀티 텐트 SaaS 애플리케이션 |
...
- **API 사후 점검(API Out-of-band Inspection)**: SaaS 서비스 API를 동동 연계하여 이미 저장된 파일의 공유 상태, 맬웨어 감염, 오설정을 주기적으로 검사하는 방식이다.
...
1. **신원•기기•데이터 맥락 확인**: 인라인 프록시가 트래픽 수신 시 사용자 신원, 기기 무결성 및 파일 데이터 민감도를 추출한다.
2. **보호 조치 결정**: 정책 엔진이 저위험/고위험 여부를 판정하고 허용, 차단, 암호화, **MFA** 적용을 수립한다.
3. **인라인 DLP•접근 정책 집행**: 패킷 흐름 상에서 민감정보 업로드/다운로드를 실시간 차단한다.
4. **저장 데이터•공유 정책 평가**: 백그라운드 API 연동을 통해 SaaS 내 기존 저장 파일의 불법 외부 공개 공유 여부를 파악한다.
5. **위반 조치•정책 환류**: 공개 공유 링크를 즉시 삭제 회수하고 맬웨어 격리 조치를 수행한다.
...
- **ISO/IEC 27017:2015**: 클라우드 서비스 환경의 가버넌스 및 고객/제공자의 보안 통제 요구사항 지침이다.
...
- 4대 영역(가시성, DLP, 위협 방어, 준법성) 확보, Hybrid CASB(Proxy+API) 구축 및 SSE/SASE 통합 중심 클라우드 데이터 보안 체계 구축 필수.
