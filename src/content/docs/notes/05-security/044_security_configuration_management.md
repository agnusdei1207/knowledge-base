...
- 정의/개념: 승인 기준선과 실제 설정을 비교•교정하는 **관리 체계**이다.
- 배경/필요성: 배포•수동 변경으로 **구성 편차(드리프트) 이 발생한다**.
...
- 배포 전 정책 검사와 **실행 중 편차 탐지**가 핵심이다.
- 예외•자동 복구•롤백의 **변경 통제**가 핵심이다.
...
- 저위험 편차는 **자동 복구**, 업무 영향이 크면 **승인 예외•만료 관리**를 선택한다.
