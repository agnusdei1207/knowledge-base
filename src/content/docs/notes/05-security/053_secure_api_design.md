...
- **접근 토큰** 내 **범위**, 만료시간(exp), 서명 무결성 및 클라이언트 채널 바인딩 검증.
- API 게이트웨이 기반 트래픽 폭주 차단용 **호출률 제한** 적용.
...
2. 입력 크기•호출률 통제: API 페이로드 규격 검증 및 **호출률 제한** 적용.
...
| 무제한 API 호출에 의한 DoS | 게이트웨이 **호출률 제한** 및 페이로드 크기 통제 | 백엔드 자원 고갈 및 무차별 스캐닝 억제 |
