...
date: "2026-07-31T02:00:27+09:00"
tags:
  - "notes-security"
weight: 53
extra:
...
1. **인증·제한 권한 요청**: PKCE로 코드와 클라이언트를 결속하고 최소 범위 요청
2. **접근 토큰**: 대상·범위·짧은 수명과 서명된 주체 정보 제공
3. **검증된 주체·범위**: 토큰·mTLS 채널·호출률 검증 결과 전달
4. **허용된 응답 데이터**: 객체 소유권·기능 권한을 확인한 필드만 제공
5. **호출·인가 판정**: 요청 주체·객체·결정 근거를 감사 기록으로 제공
...
| 객체 인가 누락·무제한 호출로 정보 유출·과부하 | **OWASP API Top 10:2023** 점검 | BOLA와 **과도한 자원 소비** 방지 |
| 게이트웨이만 신뢰하면 우회 호출의 세부 권한 미검증 | 자원 서버의 **객체·기능 인가·감사** 수행 | 우회 호출과 **권한 오용** 차단 |
