...
- 정의/개념: API 요청의 채널•토큰•객체•자원 사용을 검증하는 **응용 보안 설계**이다.
- 배경/필요성: 호출자 인증만으로는 **객체 권한•자원 남용 통제하기 어렵다**.
...
- **인증•객체•기능 인가**의 요청별 분리 집행이 핵심이다.
- **토큰 발급자•대상•범위•만료**와 채널 결속을 검증한다.
- **요청 크기•호출률•키 수명주기**를 통한 남용•장기 노출을 제한한다.
...
1. **mTLS•접근 토큰 검증**: 채널과 발급자•대상•만료•서명을 확인한다.
2. **입력 크기•호출률 통제**: 주체별 자원 소비와 요청 계약을 제한한다.
3. **객체 소유권 검증**: 호출자의 대상 자원 접근 권한을 확인한다.
4. **기능•범위 인가**: 요청 행위와 토큰 허용 범위를 대조한다.
5. **호출•인가 판정 기록**: 주체•객체•결정 근거를 감사 기록으로 보존한다.
...
- 외부 위임은 **OAuth•PKCE**, 서비스 신원은 **mTLS**, 모든 요청은 **객체 인가•호출률 제한**이 핵심이다.
