...
- 정의: API의 **채널·토큰·객체·자원 통제**
- 기존 한계: 호출자 인증의 **객체 접근·남용 미통제**
...
| 절차 | 설명 |
|:---|:---|
| 인증·PKCE·제한 권한 요청 | 주체·동의·코드 결속 확인 |
| 대상·범위·수명 토큰 발급 | 최소 권한·짧은 만료 적용 |
| 토큰·채널·호출률 검증 | 공통 보안·자원 한도 판정 |
| 객체·기능 권한 집행 | 서비스가 대상·행위 재검증 |
| 최소 응답·판정 감사 | 허용 필드·결정 근거 기록 |
