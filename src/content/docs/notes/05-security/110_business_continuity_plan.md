...
- 정의/개념: **BCP**는 **BCMS** 안에서 중단 시 우선 업무를 유지하고 목표 시간 안에 복귀하기 위한 계획이다.
- 배경/필요성: IT 재해복구만으로는 인력•시설•공급망 중단까지 포괄하기 어렵다.
...
- **BIA**로 우선 업무와 복구 목표를 도출한다.
- **MTPD** 안에 **RTO**와 **RPO**를 설정한다.
- **훈련** 결과를 **지속 개선**에 반영한다.
...
- **발동•전환•복귀 훈련**: 비상 계획을 시작하고 대체 자원으로 업무를 운영한 뒤 정상 환경으로 안전하게 되돌리는 전 과정의 시험이다.
- **BIA 범위•업무 우선순위 확정**: 분석 대상•영향 기준•책임자와 핵심 업무 순서를 정하는 단계이다.
- **MTPD•RTO•RPO 설정**: 허용 중단 시간•복구 시간•데이터 손실 목표를 정하는 단계이다.
- **연속성 전략•계획 수립**: 복구 목표에 맞는 대체 자원과 발동 기준을 설계하는 단계이다.
- **발동•전환•복귀 훈련 단계**: 비상 발동부터 대체 운영과 정상 복귀까지 실행하는 단계이다.
- **자원•계획 개선 결정**: 훈련 목표 미달 원인과 환경 변화를 자원•절차에 반영하는 단계이다.
...
2. **MTPD•RTO•RPO 설정**: **BIA 기반 복구 목표**를 확정한다.
3. **연속성 전략•계획 수립**: 대체 자원•발동 기준을 설계한다.
4. **발동•전환•복귀 훈련**: **발동•전환•복귀 훈련**으로 업무•IT•소통 절차를 실행한다.
...
- **훈련 기반 연속성 증명**을 위해 인력•시설•공급망은 **BCP**, IT•데이터 복구는 **DRP**, 목표는 **BIA**로 설정한다.
