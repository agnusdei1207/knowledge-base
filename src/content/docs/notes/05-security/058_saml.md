...
- 정의/개념: 서명된 XML 주장으로 인증 결과•속성을 전달하는 **연합 인증 표준**이다.
- 배경/필요성: 서비스별 인증의 **중복 계정•자격 관리**가 필요하다.
...
- **IdP•SP 메타데이터**를 통한 식별자•주소•인증서 사전 교환이 핵심이다.
- **Assertion**을 통한 사용자•속성•대상•시간•요청 결속을 전달한다.
- **서명 요소•대상•시간•요청•응답 ID** 검증을 통한 래핑•재전송 차단이 핵심이다.
...
1. **SAML AuthnRequest 생성**: 요청 ID•IdP 목적지•반환 주소를 구성한다.
2. **사용자 신원 인증**: IdP가 등록 인증기로 사용자를 확인한다.
3. **Assertion 조건•서명 생성**: 신원•대상•시간 조건에 서명한다.
4. **서명•대상•요청 결속 검증**: 처리 요소와 최초 요청의 일치를 확인한다.
5. **재전송 차단•로컬 세션 생성**: 응답 ID 중복을 거부하고 세션을 발급한다.
...
- 요청 결속이 필요하면 **SP 시작**, 중앙 포털은 **IdP 시작**과 응답 ID 재사용 차단을 적용한다.
