...
#### 한줄 요약
- PKI 기반 메타데이터 신뢰, XML 디지털 서명/암호화, SP/IdP 시작 방식, 재전송(Replay) 방지 통제를 제공한다.
...
#### 한줄 요약
- User Agent, IdP(신원 제공자), SP(서비스 제공자), SAML Metadata, ACS 엔드포인트가 결합한다.
...
#### 한줄 요약
- SP 접근, IdP 인증, XML 서명 발행, SP ACS 전송, 다계층 유효성 검증, 로컬 세션 발급 순으로 동작한다.
...
#### 한줄 요약
- SAML 2.0은 엔터프라이즈 XML 연합 SSO, OIDC는 클라우드/모바일 경량 JSON 신원 인증이다.
