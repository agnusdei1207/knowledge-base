...
선의 의미: 저장소 연동을 기반으로 데이터 발견•분류와 접근•계보 분석을 결합하고, 위험 우선순위 및 조치•소유자 책임을 연결한 정적 DSPM 구조
...
- **1. 데이터 발견•민감도 분류**: 비침투 방식으로 위치•내용 식별
- **2. 유효 접근•데이터 계보 계산**: 상속 권한과 복제•이동 경로 연결
- **3. 데이터 위험 우선순위화**: 민감도•노출•보호 상태 결합
- **4. 보호 조치•소유 책임 확정**: 권한•마스킹•삭제와 담당자 결정
- **5. 보호 조치 적용**: 승인 통제 실행 후 재스캔 증적 생성
#### 한줄 요약
