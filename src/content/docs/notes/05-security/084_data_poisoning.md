...
- 정의: 학습 표본 조작에 의한 **모델 규칙 변조**
- 기존 한계: 대규모 외부 데이터의 **출처·라벨 검증 곤란**
...
| 절차 | 설명 |
|:---|:---|
| 출처·해시 포함 데이터 제출 | 제공자·버전·책임 등록 |
| 격리 표본 검증 | 품질·라벨·분포 검사 |
| 승인 학습 세트 전달 | 통과 표본만 불변 승격 |
| 정상·표적·트리거 평가 | 후보 모델 공격 영향 측정 |
| 승격·거부·복구 추적 | 계보로 표본·모델 역추적 |

> 요약: 데이터를 격리·검증하고 영향 모델을 추적한다.
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 오염 공격 분류 | **NIST AI 100-2e2025 적용** | 목표·역량별 시험 체계화 |
| 외부·피드백 데이터 | **출처·해시·격리·승인 게이트** | 미검증 표본 학습 차단 |
