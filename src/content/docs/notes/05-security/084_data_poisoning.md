...
date: "2026-07-25T13:18:00+09:00"
tags:
  - "notes-security"
weight: 84
extra:
...
- **데이터 오염(Data Poisoning)**: 학습 데이터나 라벨을 조작해 모델의 판단 규칙을 바꾸는 공격
...
- **데이터 계보(Data Lineage·데이터 리니지)**: 혈통·계보를 뜻하는 Lineage를 사용한 표기이며, 오염 표본의 출처·변경과 영향 모델을 역추적하는 역할
- **기준 평가 세트(Golden Evaluation Set·골든 이밸류에이션 세트)**: 신뢰 기준을 뜻하는 Golden을 사용한 이름이며, 후보 모델의 정상 성능과 공격 행동을 비교하는 정본 시험 자료
...
- 백도어는 정상 정확도를 유지해 탐지를 피한다
- 자동 재학습은 공격 입력을 지속 오염으로 바꾼다
- 데이터 계보는 영향 모델과 복구 범위를 줄인다
...
| 주요 위험 | 서비스 품질 붕괴 | 평균 지표에 오류 은닉 | 정상 정확도로 탐지 회피 |
