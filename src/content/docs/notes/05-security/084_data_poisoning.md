...
- **학습 데이터**는 모델이 패턴을 학습하는 데 사용하는 입력 표본의 집합이다.
- **라벨**은 지도 학습에서 각 표본의 정답으로 지정한 값이다.

</details>

- 정의/개념: **데이터 오염**은 학습 표본이나 라벨을 조작해 모델의 판단 규칙을 변조하는 공격이다.
- 배경/필요성: 대규모 외부 데이터의 **출처•라벨 검증하기 어렵다**.
...
- **표적 오염**은 특정 대상이나 입력에서만 모델이 오판하도록 학습 데이터를 조작하는 공격이다.
- **백도어 오염**은 비밀 트리거가 있을 때만 공격자가 정한 행동을 내도록 만드는 공격이다.
- **데이터 계보**는 표본의 출처•변경•버전과 영향 모델을 연결한 정보다.
- **기준 평가**는 신뢰된 시험 자료로 정상 성능과 공격 행동을 비교하는 시험이다.

</details>

- 학습 전 주입의 **배포 후 잠복**이 핵심이다.
- 소량 표본의 **표적 행동 유도**가 핵심이다.
- 계보•기준 평가의 **영향 추적•복구**가 핵심이다.
...
- **정상 평가**는 신뢰된 시험 세트로 모델의 평균 업무 성능을 측정하는 시험이다.
- **표적 평가**는 특정 대상과 하위 집단에서 모델의 오판 여부를 측정하는 시험이다.
- **트리거 평가**는 변형된 트리거 입력의 공격 성공률을 측정하는 시험이다.
...
4. **정상 평균 성능으로 잠복**: 일반 시험에서는 정상 모델처럼을 동작한다.
...
- **가용성 오염**은 모델의 전체 성능과 서비스 품질을 낮추도록 데이터를 조작하는 공격이다.
...
- 계보 확인과 표적•트리거 평가를 통과한 **모델만 승격**, 실패 시 **격리•복구**가 핵심이다.
