...
- 정의/개념: **데이터 오염** 은 학습 표본이나 라벨을 조작해 모델의 판단 규칙을 변조하는 공격
...
1. **오염 표본•정상 데이터 혼합**: 조작 출처•라벨을 학습 세트에 은닉
2. **조작 표본 반복 학습**: 공격 패턴을 모델 매개변수에 반영
3. **결정 경계•트리거 연관 변조**: 표적 입력의 판단 규칙 변경
4. **정상 평균 성능으로 잠복**: 일반 시험에서는 정상 모델처럼 동작
5. **표적 오판•백도어 행동 실행**: 공격 조건에서 의도한 결과 출력
