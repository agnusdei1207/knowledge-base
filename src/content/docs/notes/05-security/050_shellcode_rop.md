...
- 정의/개념: 메모리 직접 주입형 **쉘코드**와 정상 코드 재활용형 **ROP** 공격에 맞서 DEP/NX, ASLR, Intel CET로 제어 흐름을 보호하는 기술
- 배경/필요성: 초기 시스템 해킹이 스택/힙에 직접 악성 기계어를 주입하는 쉘코드(Shellcode) 공격 중심이었으나, 하드웨어 MMU 기반의 데이터 실행 방지(DEP/NX)가 보편화되면서 데이터 영역에서의 코드 실행이 차단됨에 따라, 공격자들은 이미 메모리에 로드된 합법적 라이브러리(libc)의 명령어 조각(Gadget)들을 `ret` 명령어로 연쇄 호출하는 반환 지향 프로그래밍(ROP: Return-Oriented Programming) 코드 재사용 기법으로 진화함에 따라, 쉘코드 주입을 차단하는 DEP/NX와 더불어 가젯의 가상 메모리 주소를 난수화하는 ASLR/PIE 및 CPU 하드웨어 차원에서 복귀 주소를 이중 보관·검증하는 제어 흐름 강제 기술(Intel CET Shadow Stack / ARM PAC)을 도입하여 코드 주입 차단, 가젯 체이닝 원천 무력화 및 하드웨어 기반 제어 흐름 무결성(CFI)을 달성할 필요
...
- 메모리 오염 공격이 단순 기계어 주입(Shellcode)에서 시스템 고유 바이너리를 무기화하는 코드 재사용 공격(ROP/JOP)으로 진화함에 따른 현대 바이너리 익스플로잇 방어 및 제어 흐름 무결성(CFI)의 최상위 기술 표준으로 확고히 안착하였으며, 차세대 하드웨어 보안 프로세서 및 컴파일 타임 보호로 진화하는 가운데, 실무 저수준 시스템 및 임베디드 설계 시에는 하드웨어 DEP/NX와 Full ASLR/PIE의 기본 활성화, 컴파일러 제어 흐름 보호 옵션(-fcf-protection=full) 적용, Intel CET(Shadow Stack/IBT) 및 ARM PAC 하드웨어 기반 복귀 주소 무결성 검증 강제, JIT 컴파일러 환경의 W^X 원칙 엄격 준수를 결합하여 완벽한 시스템 실행 제어권 방어를 완성
