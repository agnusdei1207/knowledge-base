...
- 메모리 **제어 데이터** 오염을 통한 실행 제어권 탈취.
- **NX/DEP**로 쉘코드 주입을 차단하고 **ASLR**로 가젯 주소를 무작위화.
- **CFI** 및 **그림자 스택**을 적용하여 정상적이지 않은 간접 분기 및 반환 주소 변조를 최종 검증.
...
- **공격 경로별 실행 보호** 모델을 정립하여 쉘코드 주입은 **NX**, 주소 예측은 **ASLR**, ROP 가젯 재사용은 **CFI** 및 **그림자 스택**으로 다층 억제.
