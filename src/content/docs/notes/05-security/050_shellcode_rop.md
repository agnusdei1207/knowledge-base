...
date: "2026-07-25T21:50:00+09:00"
tags:
  - "notes-security"
weight: 50
extra:
...
- 쉘코드(Shellcode): [읽기: 쉘코드; 표기 이유: 한글명 뒤 영문 원어 병기] 취약한 프로세스에 주입해 공격자가 원하는 동작을 수행하도록 만든 짧은 기계어 코드
- 반환 지향 프로그래밍(Return-Oriented Programming, ROP): [읽기: 알오피; 표기 이유: 영문 머리글자] 실행 파일·라이브러리의 짧은 명령열을 연결해 기존 코드만으로 동작을 구성하는 공격 기법
- 가젯(Gadget): [읽기: 가젯; 표기 이유: 한글명 뒤 영문 원어 병기] ROP가 연결해 사용하는 짧은 기존 명령열이며 보통 간접 제어 이전으로 끝남
- 코드 주입(Code Injection): [읽기: 코드 주입; 표기 이유: 한글명 뒤 영문 원어 병기] 외부 데이터를 프로세스 메모리에 넣고 명령어로 실행시키는 공격 방식
- 제어 데이터(Control Data): [읽기: 제어 데이터; 표기 이유: 한글명 뒤 영문 원어 병기] 반환 주소·함수 포인터처럼 다음 실행 위치를 정하는 메모리 값
- 실행 방지(No-eXecute, NX): [읽기: 엔엑스; 표기 이유: 영문 머리글자] 데이터 메모리 페이지의 명령어 실행 권한을 제거하는 보호
- 주소 공간 배치 무작위화(Address Space Layout Randomization, ASLR): [읽기: 에이에스엘알; 표기 이유: 영문 머리글자] 코드·라이브러리·스택·힙의 주소를 실행마다 바꾸는 보호
- 제어 흐름 무결성(Control-Flow Integrity, CFI): [읽기: 씨에프아이; 표기 이유: 영문 머리글자] 간접 분기·호출·반환이 허용된 대상으로만 이동하는지 검사하는 보호
- 그림자 스택(Shadow Stack): [읽기: 그림자 스택; 표기 이유: 한글명 뒤 영문 원어 병기] 정상 스택과 분리해 반환 주소 사본을 보관하고 반환 시 대조하는 보호
- 메모리 안전(Memory Safety): [읽기: 메모리 안전; 표기 이유: 한글명 뒤 영문 원어 병기] 프로그램이 허용된 객체 경계와 수명 안에서만 메모리를 읽고 쓰는 성질
...
| 주요 위험 | 악성 명령 직접 실행 | 정상 코드 기반 탐지·차단 난도 |
