...
- 메모리 오염 공격이 단순 기계어 주입(Shellcode)에서 시스템 고유 바이너리를 무기화하는 코드 재사용 공격(ROP/JOP)으로 진화함에 따른 **현대 바이너리 익스플로잇 방어 및 제어 흐름 무결성(CFI)의 최상위 기술 표준**으로 확고히 안착하였으며, 차세대 하드웨어 보안 프로세서 및 컴파일 타임 보호로 진화하는 가운데, 실무 저수준 시스템 및 임베디드 설계 시에는 **하드웨어 DEP/NX와 Full ASLR/PIE의 기본 활성화, 컴파일러 제어 흐름 보호 옵션(-fcf-protection=full) 적용, Intel CET(Shadow Stack/IBT) 및 ARM PAC 하드웨어 기반 복귀 주소 무결성 검증 강제, JIT 컴파일러 환경의 W^X 원칙 엄격 준수**를 결합하여 완벽한 시스템 실행 제어권 방어를 완성
