...
- 정의/개념: 주입 코드•가젯으로 흐름을 탈취하는 **코드 실행 공격**이다.
- 배경/필요성: NX만으로는 **코드 재사용 공격 차단할 수 없다**.
...
- 제어 데이터 손상의 **공통 공격 전제**가 핵심이다.
- NX•ASLR의 **실행•주소 예측 억제**가 핵심이다.
- CFI•그림자 스택의 **분기•반환 검증**이 핵심이다.
...
1. **손상된 제어 데이터**: 변조된 반환 주소•함수 포인터를 실행 위치로 제공한다.
2. **실행 페이지 속성 확인**: 다음 명령 주소가 속한 페이지의 실행 권한을 확인한다.
3. **NX 판정**: 데이터 페이지의 쉘코드 실행을 거부한다.
4. **분기•반환 대상 확인**: 가젯의 간접 분기와 반환 주소를 검증 대상으로 확인한다.
5. **CFI•그림자 스택 판정**: 허용 경로•반환 주소 사본 불일치를 탐지한다.
...
- 코드 주입은 **NX**, 가젯 재사용은 **CFI•그림자 스택**, 주소 예측은 **ASLR**로 억제가 핵심이다.
