...
- **쉘코드(Shellcode)**: 취약점 익스플로잇 성공 시 대상 시스템 상에서 명령 쉘(Command Shell) 획득 또는 임의 코드를 실행하기 위해 삽입하는 기계어 아셈블리 멸령 조각.
...
- 정의/개념: 메모리 오버플로우 등의 취약점을 악용하여 주입된 악성 기계어 조각인 **쉘코드(Shellcode)**를 직접 실행하거나, 기존 코드의 조각들을 연결하는 **ROP(Return-Oriented Programming)**를 통해 프로세스 실행 제어권을 강제 획득하는 메모리 공격 기법.
- 배경/필요성: DEP/NX 기술 적용에 따른 직접 기계어 쉘코드 실행 한계와 이를 우회하는 기존 라이브러리 가젯(Code Reuse Attack) 체이닝의 위협 증대.
...
5. **CFI•그림자 스택 판정**: 하드웨어 그림자 스택 및 CFG 경로 비교 후 불일치 시 프로세스 사살.
...
- 쉘코드 주입 및 ROP 공격 차단을 위한 NX•ASLR 하드닝 및 CFI•Shadow Stack 심층 방어 적용 필수.
