...
- 정의: 주입 코드·기존 가젯의 **제어 흐름 탈취**
- 기존 한계: NX의 **코드 재사용 공격 미차단**
...
| 절차 | 설명 |
|:---|:---|
| 경계 초과·제어 데이터 손상 | 반환 주소·함수 포인터 변조 |
| 비정상 실행 위치 전달 | 손상 값이 다음 명령 주소로 사용 |
| 쉘코드 실행·NX 검증 | 데이터 페이지 실행을 차단 |
| 가젯 분기·CFI 검증 | 허용되지 않은 간접 분기 차단 |
| 반환 주소·그림자 스택 검증 | 원본 사본 불일치를 탐지 |
...
- **주입가능성·가젯재사용·모듈호환성**으로 실행보호를 결정한다.
