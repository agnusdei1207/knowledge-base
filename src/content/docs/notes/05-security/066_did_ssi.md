...
- 정의/개념: 보유자가 **VC**를 관리하고 필요한 속성만 제시하는 **DID•SSI 체계**이다.
- 배경/필요성: 중앙 신원 저장으로 **기관 종속•정보 과다 제공**이 필요하다.
...
- **검증 키•제어 관계 해석**: DID 문서로 식별자 통제를 확인한다.
- **자격 발급•제시•검증**: VC•VP의 서명 신뢰 판단이다.
- **선택적 공개**: 보유자 동의로 필요한 속성만 증명이다.
...
- **1. 주체•자격 주장 검증**: 발급 근거와 자격 내용을 확인한다.
- **2. DID 키 기반 VC 서명**: 주장•유효기간을 발급자 키로 보호한다.
- **3. 동의•선택 공개 VP 생성**: 요청 목적에 필요한 속성만을 구성한다.
- **4. 발급자•상태•키 검증**: 신뢰•폐기•기한과 DID 문서를 확인한다.
- **5. 증명•목적 적합성 판정**: 암호학적 증명과 업무 조건을 함께 평가한다.
#### 한줄 요약
...
- 조직 간 최소 증명은 **DID•SSI**, 단일 서비스는 **중앙 계정**, 통합 로그인은 **연합 신원**이 핵심이다.
