...
- **인공지능(Artificial Intelligence, AI)**: 데이터에서 패턴을 학습하여 미디어를 생성하거나 진위를 판별하는 기술이다.
...
1. 비대면 금융 인증에서 무작위 동작·문구 도전과 얼굴·음성 위변조 탐지를 결합하여 실시간 딥페이크를 차단한다.
...
- 딥페이크로 인한 신원 사칭과 허위정보 피해를 줄이기 위해 미지 생성기·압축·실시간성·오탐 영향을 검토하여, 다중 탐지 신호를 원본 확인과 추가 인증의 판단 근거로 활용해야 한다.
