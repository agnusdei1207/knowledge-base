...
- 정의/개념: **CTEM**은 공격 가능한 노출을 식별•검증•조치•재검증하는 **폐루프 관리 체계**이다.
- 배경/필요성: 취약점 점수만으로는 **실제 공격 가능 경로 판단할 수 없다**.
...
- 외부•내부•SaaS의 **공격 표면과 업무 자산 연결**이 핵심이다.
- 악용•공격 경로•영향의 **위험 우선화**가 핵심이다.
- 시뮬레이션•재검증의 **폐루프 노출 감축**이 핵심이다.
...
2. **악용성•업무 영향 우선화**: KEV•EPSS•자산 중요도로 정렬한다.
3. **공격 경로•통제 검증**: BAS 재현으로 도달 경로와 영향을 확인한다.
4. **소유자•기한 지정**: 검증 노출의 책임•조치 기한을 확정한다.
5. **차단•패치•구성 수정**: 경로 제거 후 잔여 노출을 재검증한다.
...
- 외부 자산은 **공격 표면 관리**, 결함은 **취약점 관리**, 검증 경로는 **CTEM 폐루프**를 적용한다.
