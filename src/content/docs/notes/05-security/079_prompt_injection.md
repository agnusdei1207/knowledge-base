...
[프롬프트 인젝션 방어 체계]
├─ 입력 경계 및 가드레일 계층
│  ├─ XML 구조적 태깅 분리 (<system>, <data>)
│  ├─ 머신러닝 분류 기반 Input Guardrails
│  └─ 탈옥·인젝션 시그니처 사전 차단
├─ 추론 및 문맥 격리 계층 (Dual-LLM)
│  ├─ 비신뢰 외부 문서 격리 처리 (저권한 LLM)
│  ├─ 시스템 지침 우선순위 가중치 강제
│  └─ 도구 호출(Tool Call) 제안 생성
└─ 런타임 통제 및 실패 격리 (PEP)
   ├─ 독립 정책 집행점 (PEP) 사용자 권한 대조
   ├─ 고위험 액션 인간 승인 (Human-in-the-loop)
   └─ 출력 가드레일 (PII·시스템 지침 유출 검사)
```
...
| 구성요소 | 책임 |
|:---|:---|
| 구조적 태깅 분리기 | 시스템 지시어와 비신뢰 입력을 XML/JSON 구분자로 감싸 구문적 해석 분리를 강제 |
| Input Guardrails | 입력 텍스트의 탈옥 및 프롬프트 인젝션 시그니처를 사전 판별하는 머신러닝 분류기 |
| Dual-LLM 엔진 | 비신뢰 외부 문서를 처리하는 저권한 LLM과 최종 결정을 내리는 제어 LLM을 분리 |
| 독립 정책 집행점 (PEP) | LLM의 도구 호출을 가로채어 실제 사용자의 IAM 최소 권한과 1:1로 교차 검증 |
| 출력 가드레일 (Output) | LLM 최종 응답에 시스템 프롬프트나 민감 API 키가 유출되었는지 2차 검사 및 마스킹 |
...
- 자연어를 프로그래밍 인터페이스로 사용하는 생성형 AI 환경에서 시스템의 실행 제어권을 악의적 입력으로부터 보호하는 LLM 애플리케이션 보안(OWASP Top 10 for LLM: LLM01 / MITRE ATLAS AML.T0051)의 최우선 핵심 방어 통제로 확고히 자리 잡았으며, 다중 에이전트 격리 및 실시간 컨텍스트 검증 엔진으로 진화하는 가운데, 실무 LLM 서비스 아키텍처 구축 시에는 XML/Markdown 기반 시스템 지침과 사용자 입력의 구조적 태깅 분리, NeMo Guardrails/Llama Guard 기반 입출력 실시간 임베딩 필터링, 비신뢰 외부 문서를 처리하는 Dual-LLM 아키텍처 채택, 모델 추론과 분리된 독립 정책 집행점(PEP)에서의 최소 권한 IAM 대조 및 파괴적 명령 시 Human-in-the-loop 강제를 결합하여 완벽한 프롬프트 인젝션 방어 무결성을 완성
