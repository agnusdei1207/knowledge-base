...
- 정의/개념: 프롬프트 인젝션은 악의적 사용자가 입력 프롬프트 내에 시스템 지침을 무효화하거나 재정의하는 자연어 명령어를 주입하여, LLM이 개발자가 설정한 보안 정책을 위반하고 의도치 않은 출력을 생성하게 만드는 대표적 AI 취약점이다.
- 배경/필요성: SQL Injection과 유사하게 제어 흐름과 데이터 흐름이 단일 자연어 텍스트 스트림으로 전달되어 **지침•데이터 혼동**이 발생하므로, 이를 명확히 분리 격리하는 방어 통제가 시급하기 때문이다.

#### 한줄 요약

- 자연어 제어 지시문과 비신뢰 데이터를 구분하지 못하는 LLM의 구조적 한계를 악용해 시스템 지침을 무력화하는 공격 기술이다.
...
1. **시스템•사용자 문맥 결합**: 시스템 프롬프트와 사용자 대화 및 외부 문서를 결합한다.
2. **지침•비신뢰 데이터 혼동**: 입력 문자열 내에 포함된 "Ignore previous instructions" 구문을 지침으로 혼동한다.
3. **공격 지시 우선 해석**: 시스템 지침(규정 준수)을 버리고 공격자가 지시한 비인가 액션(데이터 유출, API 호출)을 생성한다.
4. **독립 권한•내용 검증 누락**: PEP 차단이 없으면 과도한 권한으로 툴을 호출하게 된다.
5. **비승인 정보 접근•상태 변경**: 비인가 DB 조회, 메일 전송, 시스템 상태 변조 결과가 최종 악용된다.
...
- **실패 격리** 원칙에 따라 LLM 추론 영역과 실질적 시스템 집행 영역 간에 **PEP** 및 **사용자 재승인** 관문을 강제 집행한다.
