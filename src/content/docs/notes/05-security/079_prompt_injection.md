...
- 정의/개념: 비신뢰 입력을 지침으로 오인시켜 행동을 바꾸는 **LLM 입력 공격**이다.
- 배경/필요성: 자연어의 **지침•데이터 강제 분리하기 어렵다**.
...
- **비신뢰 지시 유입**: 사용자•RAG 자료의 직접•간접 경로이다.
- **근본 방어 한계**: 프롬프트 은닉만으로 완전 차단 불가이다.
...
4. **독립 권한•내용 검증 누락**: 모델 제안을 실행 권한처럼을 수용한다.
...
- 외부 자료는 **비신뢰 문맥으로 격리**, 상태 변경 도구는 **PEP•사용자 재승인** 후를 실행한다.
