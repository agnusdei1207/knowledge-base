...
- **정의/개념**: 처리 규율 일원화와 권리·책임 강화
- **배경/필요성**: 데이터 이동·자동 판단에 맞는 권리 정비
...
- 온라인·오프라인 처리 기준을 일원화한다.
- 자동화 결정 거부·설명 요구권을 신설한다.
- 전송요구권을 단계적으로 시행한다.
- 과징금은 전체 매출 3% 상한으로 산정한다.
...
| 설계 요소 | 설명 |
|:---|:---|
| 규율 일원화 | 특례 통합·처리 기준 통일 |
| 정보주체 권리 | 전송요구·자동화 결정 대응 |
| 국외이전 | 이전 근거 확대·중지 통제 |
| 책임성 | CPO 역할·안전조치 의무 강화 |
| 제재 | 무관 매출 제외 과징금 산정 |
...
| 판단 기준 | 이전 규정 | 2023 개정 이후 |
|:---|:---|:---|
| 핵심 특징 | 온·오프라인 특례 분리·권리 제한 | 규율 일원화·전송요구·자동화 결정 권리 신설 |
| 적용 기준 | 개정 전 처리의 기존 근거를 해석할 때 | 조항별 시행일·시행령·고시에 맞춰 기능을 구현할 때 |
| 주요 위험 | 채널별 보호 수준·제재 차이 | 시행 시점·대상 정보를 일률 적용해 권리 처리 오류 |
...
1. 자동화 결정: 고지/거부/설명 요구 채널 구현
2. 마이데이터: 대상/정보 기준 확인 및 전송 구현
