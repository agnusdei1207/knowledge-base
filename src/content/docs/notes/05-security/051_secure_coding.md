...
- 정의/개념: 구현 결함을 예방하는 **코드•검토•검증 규칙**이다.
- 배경/필요성: 임의 구현의 **취약점 반복•품질 편차**가 발생한다.
...
- **위협 모델•반복 결함**의 언어•프레임워크 규칙 구체화가 핵심이다.
- **안전 기본값•대체 API**를 통한 위험 구현 경로를 축소한다.
- **코드 리뷰•SAST•DAST•배포 게이트** 결합과 예외 만료가 핵심이다.
...
1. **위협•반복 결함 규칙화**: 안전 기본값과 대체 API 기준을 정의한다.
2. **안전 API•기본값 구현**: 입력•권한•암호•오류 통제를 적용한다.
3. **업무 권한•복합 논리 검토**: 자동 검사가 놓친 위험 경로를 확인한다.
4. **SAST•DAST 배포 판정**: 자동 검사 기준 미달 변경을 차단한다.
5. **예외•운영 결함 반영**: 보상 통제•만료일•운영 사고를 규칙 개선에 반영한다.
...
- 업무 논리는 **코드 리뷰**, 반복 패턴은 **자동 검사**, 예외는 만료•재승인을 적용한다.
