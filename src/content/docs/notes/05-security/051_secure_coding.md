...
| 절차 | 설명 |
|:---|:---|
| 위협·반복 결함 기반 규칙 | 안전 기본값·대체 API·실패 조건 결정 |
| 안전 구현·업무 논리 검토 | 규칙 적용과 권한·위험 경로를 함께 검토 |
| 검토된 변경 제출 | 승인된 변경만 자동 보안 검증으로 전달 |
...
- 개발 단계의 반복 보안 결함을 줄이기 위해 업무 논리·자동 검출 가능성·공통 API·예외 수명을 검토하여, 리뷰·자동 검사·안전 기본값을 결합한 시큐어 코딩을 적용해야 한다.
