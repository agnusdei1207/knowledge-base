...
- 정의/개념: **위협 헌팅**은 기존 경보를 기다리지 않고 검증 가능한 공격 가설을 세워 여러 보안 데이터에서 숨은 침해 증거를 찾는 **능동 위협 탐색 활동**이다.
- 배경/필요성: 기존 시그니처•경보만으로는 **미지•변형 공격과 데이터 공백 식별할 수 없다**.
...
- **자산 위험•CTI•TTP** 기반 반증 가능 가설이 핵심이다.
...
5. **지속 탐지 규칙 배포**: 분석•수집•플레이북에 검증 결과를 반영한다.
...
- 알려진 IoC는 **지표 기반**, 변형 공격은 **TTP 기반**, 미지•내부자는 **이상 기반** 헌팅이 핵심이다.
