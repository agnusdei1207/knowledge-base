...
선의 의미: 헌팅 질의•피벗이 가설 조건과 텔레메트리를 함께 참조하고, 검증된 증거 범위가 대응•탐지 체계와 연결되는 정적 의존관계
...
1. **증거 검증•피벗**: 원본•업무 문맥 확인과 연관 범위 확장
2. **침해 범위 전달**: 검증 증거와 영향 계정•호스트 제공
3. **격리•제거 실행**: 침해 자산 격리와 악성 요소 제거
