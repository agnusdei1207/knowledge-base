...
- **위협 헌팅(Threat Hunting)**: 기존 경보를 기다리지 않고 가설을 세워 숨은 침해 증거를 능동적으로 탐색하는 활동
- **가설(Hypothesis)**: 예상 공격 행동·관측 증거·반증·종료 조건을 포함한 검증 가능한 설명
- **사이버 위협 인텔리전스(Cyber Threat Intelligence, CTI)**: 공격 주체·기반시설·의도·행동을 분석하여 의사결정에 활용하는 위협 정보
- **MITRE ATT&CK**: 실제 관찰된 공격 행동을 전술·기법·절차로 분류한 지식체계
...
- **단말 탐지·대응(Endpoint Detection and Response, EDR)**: 단말의 프로세스·파일·행위를 지속 수집하여 위협을 탐지·조사·대응하는 기술
- **네트워크 탐지·대응(Network Detection and Response, NDR)**: 네트워크 통신을 지속 분석하여 위협을 탐지·조사·대응하는 기술
...
- 자동 경보가 놓친 침해를 능동적으로 찾기 위해 가설·자산 맥락·IoC·TTP·이상행위·검증 결과를 검토하여, 위협 특성에 맞는 헌팅 기법을 선택하고 탐지로 환류해야 한다.
