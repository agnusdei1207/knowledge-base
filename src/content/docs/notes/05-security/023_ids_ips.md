...
- 정의/개념: 복제 경보와 인라인 차단을 구분한 **IDS•IPS**이다.
- 배경/필요성: 헤더 기반 허용 정책만으로는 정상 서비스 내부의 **공격 행위 식별할 수 없다**.
...
3. **경보•차단 정책 적용**: 위험도별 허용•경보•차단 조치를 실행한다.
...
- 가시성•규칙 검증은 **IDS**, 고신뢰 즉시 차단은 **IPS**을 선택한다.
