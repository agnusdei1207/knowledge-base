...
- 정의/개념: 서버 인증•임시 키 합의로 **AEAD 트래픽 키를 설정하는 절차**이다.
- 배경/필요성: 구형 TLS의 다중 왕복•취약 조합으로 **지연•공격면이 증가한다**.
...
- **1-RTT 서버 인증•임시 키 합의**로 연결 지연 단축이 핵심이다.
- **AEAD 전용 암호군**으로 기밀성•무결성 결합이 핵심이다.
- **0-RTT 재전송 가능성**에 따른 멱등 요청을 제한한다.
...
3. **핸드셰이크 키 도출**: ECDHE•대화 해시로 보호키를 생성한다.
4. **인증서•Finished 검증**: 서버 신원•협상 무결성을 판정한다.
5. **Finished•응용 트래픽 키 전환**: 클라이언트 완료 검증 후 방향별 AEAD 키를 활성화한다.
...
- 상태 변경 요청은 **전체 1-RTT**, 멱등 조회만 **0-RTT** 허용이 핵심이다.
