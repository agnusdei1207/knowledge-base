...
- **TLS 1.3 핸드셰이크** 는 서버 인증과 임시 키 합의를 결합하여 AEAD 트래픽 키를 설정하는 절차이다.
...
- **1-RTT** 는 한 번의 왕복으로 키 합의와 서버 인증을 마친 뒤 응용 데이터를 전송한다.
- **0-RTT 조기 데이터** 는 이전 연결의 PSK로 첫 요청부터 보호하지만 공격자가 요청을 재전송할 수 있다.
- **PSK•멱등 요청** 은 이전 연결의 사전 공유키와 반복 실행해도 상태가 추가로 변하지 않는 요청을 뜻하며 0-RTT 허용 범위를 정한다.
- **AEAD 암호군** 은 데이터 기밀성과 무결성을 하나의 레코드 보호 방식으로 제공한다.
...
- **HKDF 키 스케줄** 은 공유 비밀과 대화 기록 해시에서 단계•방향별 트래픽 키를 도출한다.
- **CertificateVerify와 Finished** 는 각각 서버의 개인키 보유와 전체 핸드셰이크 무결성을 증명한다.
...
- **ClientHello와 ServerHello** 는 지원 암호군•그룹과 양측의 키 공유값 제안 및 선택 결과를 교환한다.
- **ECDHE** 는 연결마다 새로운 타원곡선 임시 키로 공유 비밀을 합의하여 순방향 비밀성을 제공한다.
- **핸드셰이크 키•응용 트래픽 키** 는 각각 인증 메시지를 보호하고 검증 완료 뒤 실제 응용 데이터를 방향별로 보호한다.
...
- **PSK 재개** 는 이전 연결에서 얻은 키 재료를 사용하여 반복 연결의 핸드셰이크 비용을 줄인다.
- **멱등 요청** 은 같은 요청을 여러 번 실행해도 시스템 상태가 추가로 바뀌지 않아 0-RTT 재전송 위험을 제한할 수 있다.
- **PFS** 는 장기 키가 나중에 유출되어도 과거 세션키를 복구할 수 없게 하는 순방향 비밀성이다.
...
- **RFC 8446** 은 TLS 1.3의 핸드셰이크, 키 스케줄, 레코드 보호 절차를 규정한다.
...
- **전체 1-RTT•0-RTT 선택** 은 상태 변경 요청에는 인증 완료 뒤 전송을, 반복 안전한 조회에는 조기 전송을 적용하는 판단이다.
