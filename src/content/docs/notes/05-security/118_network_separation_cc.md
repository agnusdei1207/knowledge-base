...
```text
[업무•인터넷•관리 영역] ----- [망연계•자료전송 경계] ----- [안전한 설치•관리 구성]
                                                               /       \
                                                   [CC 인증 TOE•버전]   [로그•우회•변경 검증]
```

선의 의미: 가로선은 분리된 보안 영역, 승인된 자료전송 경계와 안전한 설치•관리 구성의 정적 망분리 구조이고, 아래 가지는 설치 구성이 CC 인증 TOE•버전의 보증 범위와 운영 로그•우회•변경 검증에 함께 구속되는 관계를 뜻한다.
...
sequenceDiagram
  ...
  A->>A: 3. CC 범위•버전•형상 검증
  A->>O: 검증된 설계•제품 정보 전달
  O->>O: 4. 전송•관리•로그 통제 집행
  O->>V: 운영 증적 전달
  V-->>A: 우회•변경•통제 효과 결과
  A->>A: 5. 우회 경로•구성 시정 결정
  A->>O: 시정 결정 전달
```

**동작 원리**

- **1. 업무•데이터 흐름 분석**: 서비스•관리•외부 연결 정보 분석
- **2. 분리 영역•최소 연계 요구 확정**: 허용 정보•방향•검사 기준 결정
- **3. CC 범위•버전•형상 검증**: TOE•인증서•PP•형상 정보 검증
- **4. 전송•관리•로그 통제 집행**: 승인•무해화•감사 기록 생성
- **5. 우회 경로•구성 시정 결정**: 예외•패치•공유 기능 보완 결정
