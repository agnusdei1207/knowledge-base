...
1. **변경·위협·데이터 흐름 분석**: 악용경로 식별
2. **코드·의존성·IaC 자동 검증**: 자동 검사 실행
3. **위험·예외·출처 기반 배포 판정**: 게이트 결정
4. **행위·사고·취약점 관측**: 운영 증거 수집
5. **요구·정책·시험 재발 방지**: 백로그·검사 갱신
...
- Shift Right 결과를 **Shift Left로 환류**
