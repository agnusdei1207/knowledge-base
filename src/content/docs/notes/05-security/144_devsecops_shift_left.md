...
- 정의/개념: 개발•배포•운영에 **보안 책임•자동화•피드백**을 통합하는 방식이다.
- 배경/필요성: 출시 직전 후행 검사는 **수정 비용•배포 지연•결함 재발이 증가한다**.
...
- **Shift Left•Right**: 보안을 개발 초기로 앞당기고 운영 관측•사고 결과를 다시 개발로 돌리는 접근한다.
- **코드형 보안**: 정책•검사•구성•증거를 코드와 버전관리로 반복 실행하는 방식이다.
- **코드형 인프라(Infrastructure as Code, IaC)**: 인프라 구성과 정책을 코드로 정의하는 방식이다.

</details>

- 제품•보안•운영팀의 **공동 위험 책임**이 핵심이다.
- 코드•의존성•IaC•이미지의 **자동 검사**가 핵심이다.
- 운영 사건•관측의 **개발 백로그 환류**가 핵심이다.
- **코드형 보안**으로 정책 자동화, **시프트 라이트**로 운영 환류가 핵심이다.
...
- **1. 변경•위협•데이터 흐름 분석**: 영향 자산•악용경로 식별이다.
- **2. 코드•의존성•IaC 자동 검증**: 위험별 검사•가드레일을 실행한다.
- **3. 위험•예외•출처 기반 배포 판정**: 소유자•보상통제•만료를 확인한다.
- **4. 행위•사고•취약점 관측**: 운영 위험•재현 증거 수집이다.
- **5. 요구•정책•시험 재발 방지**: 근본원인•백로그•검사를 갱신한다.
...
- 노출•영향 큰 변경은 **게이트 차단**, 낮은 위험은 **자동 배포**가 핵심이다.
