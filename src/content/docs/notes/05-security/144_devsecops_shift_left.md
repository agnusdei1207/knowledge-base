...
1. CI 파이프라인의 **고위험 취약점 배포 차단**
### 쉽게 이해하기 (학습용)

- CI 파이프라인은 SAST·SCA 결과 중 실제 노출된 고위험 결함을 배포 게이트에서 차단하고 예외에는 소유자·보상 통제·만료일을 요구한다.

## Ⅶ. 결론

- 개발 후반의 보안 결함 수정 비용을 줄이기 위해 위험도·개발자 피드백·배포 게이트·예외·운영 재발을 검토하여, 제품팀 책임과 자동 가드레일을 DevSecOps로 통합해야 한다.
