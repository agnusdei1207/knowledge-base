...
- 정의/개념: 산업 위협정보를 **공동 수집•분석•제한 공유**하는 조직이다.
- 배경/필요성: 단일 기관 관측 한계를 넘어 **공통 공격 조기경보**가 필요하다.
...
- 비식별•맥락 보강을 통한 **정보 노출 방지•활용성 확보**가 핵심이다.
- TLP•필요 기반의 **수신자•재공유 제한**이 핵심이다.
...
- **1. 출처•공유조건 검증**: 제공기관•근거•시점을 확인한다.
- **2. TLP•비식별•법적근거 확인**: 수신자•재공유•개인정보를 검증한다.
- **3. 신뢰도•산업 영향•시효 분석**: IoC•TTP•영향 자산을 연결한다.
- **4. STIX•TAXII 배포객체 생성**: 위협 객체와 관계를 표준화이다.
- **5. 품질•유효상태 갱신**: 적중•오탐•철회로 점수•만료를 반영한다.
...
- 신뢰도•시효 충족 **CTI** 만 TLP 범위에 배포, 식별 위험은 **비식별**이 핵심이다.
