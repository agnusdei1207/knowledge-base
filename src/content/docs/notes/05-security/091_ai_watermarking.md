...
- **AI 워터마킹**: AI 생성 콘텐츠에 출처 식별 신호를 삽입하고 유통 뒤 다시 검출하는 기술이다.
...
- **강건성•오탐률**: 압축•편집•번역 뒤에도 신호가 남는 성질과 정상 콘텐츠를 워터마크가 있다고 잘못 판정한 비율이다.
- **검출률•지각성**: 워터마크가 있는 콘텐츠에서 신호를 찾아낸 비율과 삽입 신호가 사람에게 눈에 띄거나 품질을 해치는 정도이다.
...
- **삽입기•검출기**: 삽입기는 품질 제약 안에서 키 기반 신호를 반영하고 검출기는 콘텐츠에서 신호 점수를 계산한다.
...
- **가시•비가시•서명 출처**: 각각 사람이 보는 표식, 알고리즘으로 찾는 숨은 신호, 전자서명된 생성•편집 이력을 제공한다.
...
- **NIST AI 100-4**: 합성 콘텐츠 투명성 기술로 워터마킹•출처 추적•탐지의 역할과 한계를 분석한 공식 보고서이다.
- **C2PA(Coalition for Content Provenance and Authenticity) 2.4 하드•소프트 결속**: 해시로 원본 자산을 직접 묶거나 워터마크•지문으로 변형된 자산을 출처 명세와 연결하는 규격이다.
