...
- 정의: 생성 콘텐츠의 **출처 식별 신호 삽입·검출**
- 기존 한계: 변형 후 **검출 누락·정상 콘텐츠 오탐**
...
sequenceDiagram
    ...
    C->>D: 3. 압축·편집 콘텐츠 전달
    D->>V: 4. 검출 점수·임계값 판정
    V-->>O: 5. 서명 출처·진위 교차 확인
```

| 절차 | 설명 |
|:---|:---|
| 식별 메시지·키 설정 | 주체·모델·콘텐츠 연결 |
| 신호 삽입·품질 확인 | 품질 저하·지각성 측정 |
| 압축·편집 콘텐츠 전달 | 실제 유통 변형 적용 |
| 검출 점수·임계값 판정 | 검출률·오탐률 보정 |
| 서명 출처·진위 교차 확인 | 워터마크·이력 증거 결합 |

> 요약: 삽입 품질과 변형 강건성을 함께 검증한다.
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 합성 콘텐츠 투명성 | **NIST AI 100-4 기반 평가** | 검출·출처 기술 역할 구분 |
