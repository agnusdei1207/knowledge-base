...
date: "2026-08-05T12:03:00+09:00"
tags:
  - "notes-security"
weight: 91
extra:
...
- 정의/개념: **AI 워터마킹** 은 생성 콘텐츠의 품질을 크게 해치지 않는 식별 신호를 모델 출력에 삽입하고 나중에 검출하여 생성 출처나 사용 정책을 확인하는 기술
...
```text
                           [생성 콘텐츠]
                       /          |          \
             [워터마크 삽입기] [유통•변형 채널] [검출•출처 검증기]
                      |
                 [메시지•키]
```

선의 의미: 생성 콘텐츠에 워터마크 삽입, 유통 변형, 검출•출처 검증 영역이 각각 결합되고 메시지•키가 삽입기의 식별 신뢰를 제공하는 정적 인터페이스 구조
...
3. **변형 보정•신호 추출**: 압축•편집 뒤 숨은 특징 복원
...
- **AI 100-4** 는 워터마킹•출처 추적•탐지의 역할과 한계를 분석한 보고서다.
