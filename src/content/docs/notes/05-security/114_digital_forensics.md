...
- 배경/필요성: 원본 직접 분석•취급 기록 누락 시 **무결성•연계보관성 입증할 수 없다**.
...
- 휘발성•상태 변경 기반 **획득 순서**가 핵심이다.
- 원본 보존•사본 분석의 **무결성•재현성**이 핵심이다.
...
- **증거 취급 기록**: 증거의 상태•담당자•시간•작업 내용을 단계마다 남긴 기록한다.
...
5. **분석 재현•파생 증거 기록**: 도구•명령•산출물과 분석 이력을 보존한다.
...
- **연계보관성(Chain of Custody)**: 증거의 취급•인계 이력이 끊기지 않았음을 입증하는 조건이다.

</details>

- 휘발 정보 필요 시 **라이브**, 원본 변경 최소화 시 **데드**, 모두 해시•연계보관을 기록한다.
