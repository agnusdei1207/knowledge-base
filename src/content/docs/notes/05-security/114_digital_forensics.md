...
1. **조사 권한•범위 확인**: 근거•대상•기간•도구 제한을 확인한다.
2. **증거 식별•현장 상태 기록**: 장치•연결•시각•휘발성을 판단한다.
3. **쓰기 차단•비트 복제•해시 봉인**: 원본 변경 방지와 무결성을 확보한다.
4. **원본 보관•분석 사본 생성**: 검증된 사본과 인계 기록을 생성한다.
5. **분석 재현•파생 증거 기록**: 도구•명령•산출물을 **증거 취급 기록**과 함께 보존한다.
