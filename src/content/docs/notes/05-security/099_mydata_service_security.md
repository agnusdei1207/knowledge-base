...
title: "마이데이터 서비스 보안"
date: "2026-08-02T13:16:00+09:00"
tags: ["notes-security"]
weight: 99
extra:
  question_no: "099"
...
- **동의 관리·최소 권한 토큰**: 목적·항목·수신자·기간의 생성·변경·철회를 관리하고 그 범위와 대상·만료를 접근 토큰에 반영한다.
- **기관 인증·요청 서명**: 전송자와 수신자의 신원을 상호 확인하고 요청 내용의 위조·변조·재전송을 막는 API 통제이다.
...
- **정보전송자·정보수신자**: 전송자는 법정 대상 정보를 보유·제공하고 수신자는 정보주체가 지정한 목적 안에서만 활용한다.
...
- **송신자 제한 토큰·Nonce**: 특정 클라이언트 키에 토큰을 묶고 일회성 난수로 요청의 신규성을 확인하여 탈취·재전송을 막는다.
...
- **응용 프로그래밍 인터페이스(Application Programming Interface, API) 전송**: 기관 인증과 요청 검증을 거쳐 동의한 필드만 기관 사이에서 이동시키는 단계이다.
- **요청 서명·파생본 근거 재판정**: 요청 위·변조를 검증하고 동의 철회 뒤 가공 결과의 보유·이용 근거도 다시 확인하는 통제이다.
