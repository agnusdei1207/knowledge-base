...
sequenceDiagram
    ...
    U->>A: 1. 목적·항목·수신자 동의
    A-->>R: 2. 제한·송신자 결속 토큰
    R->>H: 3. 서명·난수 포함 전송 요청
    H-->>R: 4. 토큰 범위 필드 암호전송
    U->>L: 5. 철회·활용중단·파생본 기록
```

| 절차 | 설명 |
|:---|:---|
| 목적·항목·수신자 동의 | 기간·주기·종료 조건 확정 |
| 제한·송신자 결속 토큰 | 주체·대상·범위·만료 결속 |
| 서명·난수 포함 전송 요청 | 기관 인증·재전송 방지 |
| 토큰 범위 필드 암호전송 | 동의·응답 항목 대조 |
| 철회·활용중단·파생본 기록 | 보유 근거·삭제·증적 관리 |

> 요약: 동의 범위를 토큰·전송·보유에 결속한다.
