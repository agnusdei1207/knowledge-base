...
- 정의/개념: 비밀 증거를 공개하지 않는 **명제 관계 증명 프로토콜**이다.
- 배경/필요성: 원본 제출 방식은 **검증 과정의 비밀 노출을 유발한다.
...
- **회로 크기**는 증명할 관계를 표현한 제약식의 규모로, 커질수록 증명 생성과 검증 비용이 증가한다.

</details>

- 참 수용•거짓 거부를 보장하는 **완전성•건전성**이 핵심이다.
- 비밀 증거를 숨기고 **명제 관계만 밝히는 영지식성**이 핵심이다.
- **회로 크기 증가**로 증명 생성•검증 비용 상승한다.
...
5. **회로•문맥•결과 판정**: 대상•버전•제약 만족을 처리한다.
...
- 짧은 증명은 **zk-SNARK**, 신뢰 설정 회피는 **zk-STARK**을 선택한다.
