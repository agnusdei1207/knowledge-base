...
- 정의: 입력 교란에 의한 **모델 판단 경계 우회**
- 기존 한계: 정상 정확도의 **경계 주변 취약성 누락**
...
| 절차 | 설명 |
|:---|:---|
| 목표·섭동 예산 설정 | 표적 결과·변형 한도 정의 |
| 교란 후보 반복 질의 | 취약 방향 입력 탐색 |
| 라벨·점수 반환 | 경계·목표 달성 여부 제공 |
| 성공 교란 재현 | 변형·환경 조건 반복 시험 |
| ASR·정상성 판정 | 공격률·정상 성능 함께 측정 |

> 요약: 제한된 교란으로 판단 경계의 강건성을 시험한다.
