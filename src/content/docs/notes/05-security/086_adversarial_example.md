...
- 배경/필요성: 결정 경계에 인접한 입력은 정상 데이터셋 평가에 나타나지 않아 오판 비용이 운용 중에만 드러나므로, 학습 시 **PGD 적대적 훈련**으로 경계를 두껍게 하고 추론 뒤에는 센서 퓨전·Fail-Safe를 물리 동작 직전의 마지막 검증 계층으로 둔 것
...
- 일반 서비스는 **PGD 적대적 훈련**, 안전 필수는 **센서 퓨전** 병행
