...
- **응용 프로그래밍 인터페이스(Application Programming Interface, API, 에이피아이)**: 영문 머리글자를 딴 모델 질의·응답 접점
- **전이성(Transferability, 트랜스퍼러빌리티)**: 한 모델에서 만든 교란이 다른 모델에도 작동하는 성질
- **공격 성공률(Attack Success Rate, ASR, 에이에스알)**: 영문 머리글자를 딴 지표로, 교란이 목표 오동작을 만든 비율
...
- 얼굴·음성 인증은 적대적 학습과 입력 이상 탐지를 함께 적용한다.
- 카메라 제어는 거리·조명별 물리 공격과 다중 센서 대조를 시험한다.
