...
- **입력 전처리 방어(Input Preprocessing Defense)**: 입력 이미지를 추론 직전에 압축·필터링·재샘플링하여 적대적 섭동을 상쇄시키는 방어 기법이다.
- **안전 모드 전환(Fail-Safe Fallback)**: 센서 간 판단 불일치 탐지 시 보수적 안전 모드로 시스템 제어를 전환하는 메커니즘이다.
...
| 입력 전처리·안전 모드 | 이미지 디노이징(Denoising) 및 Fail-Safe 시스템 전환 |
