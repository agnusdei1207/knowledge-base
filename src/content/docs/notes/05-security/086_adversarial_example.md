...
- 정의/개념: 적대적 예제(Adversarial Example)는 원본 입력 데이터에 인공지능 경사하강법/경사상승법 기반으로 계산된 최소한의 미세 노이즈($l_p$ Norm 섭동)를 주입하여, 인간의 인식 성능을 유지한 채 딥러닝 모델의 **판단 경계**를 의도적으로 이동시킴으로써 오분류를 야기하는 공격이다.
- 배경/필요성: 자율주행 정지(STOP) 표지판에 미세 스티커를 부착해 최고 속도 제한 표지판으로 오인하게 만드는 물리적 공격 위험을 방지하고, AI 모델의 강건성(Robustness) 체계를 확립하기 위함이다.
...
1. **교란 후보 생성**: FGSM/PGD 알고리즘을 이용해 지정된 섭동 예산($\epsilon$) 내에서 경사상승 **교란 탐색**을 집행한다.
2. **환경 변형 재현**: 물리적 스티커 인쇄 후 거리, 조명, 시야각 변형 시의 **환경 재현** 및 오분류 지속 여부를 검증한다.
3. **ASR•정상성 판정**: 방어 전처리 적용 모델을 대상으로 적대적 **ASR** 감소율과 **정상성** 정확도를 동시에 판정한다.
...
- PGD **적대적 학습**을 통한 **교란 강건성** 확보와 이종 센서 연동을 통한 **피해 제한**을 동시 실현한다.
