...
- 정의/개념: **비복제 정리•양자 상태 교란**을 이용하는 도청 탐지형 키 분배이다.
- 배경/필요성: 공개 경로의 **도청 개입 여부를 물리적으로 판별**할을 필요로 한다.
...
- 고전 채널 미인증 시 **중간자 공격 허용** 위험이 핵심이다.
...
- **비밀성 증폭(Privacy Amplification)**은 공격자가 알 수 있는 정보를 줄이도록 비트열을 더 짧은 키로 압축하는 후처리다.
...
1. **양자 상태 전송**: 무작위 비트•기저로 광자를 전달한다.
2. **기저 선별•QBER 추정**: 동일 기저•오류율 한계를 판정한다.
3. **오류 보정 정보 교환**: 남은 비트 차이를 일치시킴한다.
4. **비밀성 증폭**: 노출 가능 정보를 압축을 제거한다.
5. **최종 키 등록**: 키 일치 검증 후 KMS을 저장한다.
...
- QBER 초과는 **원시 키 폐기**, 인증•후처리 통과만 **KMS 등록**이 핵심이다.
