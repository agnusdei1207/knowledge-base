...
- **ZTNA(Zero Trust Network Access)** 는 검증된 주체와 기기에 승인된 애플리케이션 연결만 제공하는 접근 제어 기술이다.
- **VPN(Virtual Private Network)** 은 암호화 터널을 통해 원격 장비를 내부망에 연결하는 기술이다.
...
```text
[사용자•기기]---[정책 브로커•신호]---[앱 커넥터]---[비공개 앱]
                       |                  |             |
                       +------------------+------[로그•가용성]
```

선의 의미: 사용자•기기와 비공개 앱 사이의 정책 브로커•커넥터에 로그 및 가용성 통제가 결합되는 ZTNA 배치 구조를 나타낸다.
...
sequenceDiagram
    ...
    B->>B: 2. 제한 세션 정책 생성
    B->>C: 앱•행위•시간 정책
    C->>C: 3. 발신 커넥터•앱 터널 설정
    C->>A: 허용 앱 요청
    A-->>C: 앱 응답
    loop 세션 유지 중
        C->>C: 4. 세션 행위•연결 로그 수집
        C->>B: 위험 변화 신호
        B->>B: 5. 세션 위험 재평가
        B-->>C: 유지•제한•종료 결정
    end
...
- **2. 제한 세션 정책 생성**: 앱•행위•시간 범위를 최소화
- **3. 발신 커넥터•앱 터널 설정**: 기원 서버를 숨긴 전용 연결 생성
- **4. 세션 행위•연결 로그 수집**: 승인 세션과 우회 접속 증적 기록
- **5. 세션 위험 재평가**: 조건 변경 시 유지•제한•종료 판정
#### 한줄 요약
