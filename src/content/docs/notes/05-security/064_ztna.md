...
date: "2026-07-31T01:58:26+09:00"
tags:
  - "notes-security"
weight: 64
extra:
...
- **재인증(Reauthentication)**: 세션 위험이 바뀌면 신원과 인증 요소를 다시 확인하는 절차이다.
- **연결 로그**: 사용자·기기·앱·시간·허용 결과를 기록한 접속 증적이다.
- **다중 커넥터**: 앱 중계 장애를 줄이기 위해 둘 이상의 커넥터를 배치한 구성이다.
- **중앙 장애**: 정책 브로커나 중계 서비스 장애로 전체 접근이 영향을 받는 위험이다.

## Ⅰ. 개요

- 정의/개념: 검증된 주체에게 승인 앱만 연결하는 **원격 접근 제어 기술**
...
sequenceDiagram
    ...
    B->>C: 2. 제한 세션 지시
    C->>A: 3. 허용 앱 요청 중계
    A->>감사시스템: 4. 세션 행위 로그 전달
    감사시스템->>B: 5. 위험 변화 신호 환류
    B-->>U: 접근 결과
```
...
- **5. 위험 변화 신호 환류**: 조건 변경 시 제한·종료 근거 제공
### 쉽게 이해하기 (학습용)
