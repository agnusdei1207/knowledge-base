...
- 정의: 식별자 처리의 **재식별 위험 목적 수준 축소**
- 기존 한계: 직접 식별자 삭제의 **외부 결합 재식별**
...
| 설계 요소 | 설명 |
|:---|:---|
| 목적·법적 근거 | 이용·제공 범위·근거 확정 |
| 식별 위험 분석 | 식별자·외부 결합 가능성 평가 |
| 비식별 처리 규칙 | 삭제·대체·일반화·억제 |
| 추가정보·이용 통제 | 매핑키 분리·접근·반출 제한 |
| 적정성·재평가 | 재식별 위험·유용성 검증 |
...
| 절차 | 설명 |
|:---|:---|
| 목적·근거·외부정보 확정 | 분석·제공·공개 범위 설정 |
| 식별자·결합 위험 분류 | 직접·준식별·민감 속성 평가 |
| 삭제·대체·일반화 적용 | 목적 수준까지 위험 감소 |
| 재식별·유용성 검증 | 공격 가능성·분석 가치 측정 |
| 접근·반출 통제·재평가 | 환경 변화·외부 자료 반영 |

> 요약: 목적별 재식별 위험과 유용성을 검증한다.
...
- **분석목적·결합가능성·이용환경**으로 비식별 수준을 결정한다.
