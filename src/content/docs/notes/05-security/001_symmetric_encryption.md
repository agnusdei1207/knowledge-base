...
- **정의/개념**: 같은 비밀키로 평문을 변환·복원하는 암호
- **배경/필요성**: 대용량 데이터의 저지연 기밀성 보호
...
- 키 유출은 암호문 복호화·위조를 허용한다
- 논스 재사용은 평문 관계를 노출한다
- AEAD는 암호문 변조도 함께 검출한다
...
| 판단 기준 | AES-GCM | ChaCha20-Poly1305 | AES-XTS |
|:---|:---|:---|:---|
| 핵심 특징 | AES 블록 암호 기반 AEAD | 스트림 암호 기반 AEAD | 저장 블록 위치별 기밀성 |
| 적용 기준 | AES 가속 통신·저장 데이터 | AES 가속 없는 통신 환경 | 디스크 섹터 암호화 |
| 주요 위험 | 논스 재사용 시 기밀성 훼손 | 논스 재사용 시 기밀성 훼손 | 무결성은 별도 통제 필요 |
...
2. 디스크 암호화는 AES-XTS로 섹터 보호

### 쉽게 이해하기 (학습용)

- 웹 통신은 데이터가 바뀌었는지도 확인해야 하므로 암호문과 태그를 함께 만드는 AES-GCM을 사용한다.
- 디스크는 같은 내용도 저장 위치마다 다른 암호문이 되게 AES-XTS를 적용하고 무결성은 별도 기능으로 확인한다.

## Ⅶ. 결론

- 보호 대상별 모드·키·논스 정책 선택
