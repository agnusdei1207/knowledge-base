...
- 정의/개념: 송수신자가 동일한 **비밀키**로 암호화•복호화를 수행하는 **대칭키 암호** 방식
- 배경/필요성: 비대칭키 암호의 높은 연산 복잡도로 인한 대용량 데이터 실시간 전송 암호화 한계
...
- **대칭키 암호**: 동일한 **비밀키** 기반의 평문 암호화 및 복호화 수행
- **블록 암호** 및 **스트림 암호**: 대용량 자료의 고속 처리 지원
- **AEAD**: **IV** 및 **논스** 연계를 통한 기밀성•무결성 동시 보장
