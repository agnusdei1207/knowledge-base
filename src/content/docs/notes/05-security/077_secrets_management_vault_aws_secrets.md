...
- **OIDC(OpenID Connect)** 는 사용자나 워크로드의 인증 결과와 신원 정보를 전달하는 프로토콜이다.
- **PKI(Public Key Infrastructure)** 는 공개키 인증서의 발급•검증•폐기를 관리하는 신뢰 체계다.
- **동적 비밀** 은 요청할 때 대상 시스템에 새로 생성하는 짧은 수명의 자격 정보다.
- **리스** 는 발급한 비밀의 유효기간•갱신•폐기 상태를 관리하는 계약이다.
- **TTL(Time to Live)** 은 비밀이나 리스가 만료되기 전까지 유효한 시간이다.
...
- **AWS(Amazon Web Services) Secrets Manager** 는 정적 비밀 저장과 자동 회전을 제공하는 관리형 서비스다.
