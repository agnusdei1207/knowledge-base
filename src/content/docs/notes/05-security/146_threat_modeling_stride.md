...
[위협 모델링 프레임워크]
├── [시스템 추상화 (DFD)]
│   └── 4대 요소 및 신뢰 경계 (Trust Boundary)
├── [위협 식별 (STRIDE)]
│   └── 6대 위협 분류 및 완화 설계 패턴
└── [위험 평가 (DREAD)]
    └── 5대 평가 축 점수 산정 및 우선순위
```
...
#### 한줄 요약
- DFD가 시스템 전체를 훑던 검토를 요소와 신뢰 경계라는 도면으로 줄여 놓고, STRIDE는 분석가의 상상력에 기대던 위협 도출을 요소마다 여섯 가지를 되묻는 기계적 점검으로 대신하며, DREAD는 그렇게 쏟아진 목록을 다 고칠 수 없다는 전제 아래 착수 순서를 정한다.
...
#### 한줄 요약
- 신뢰 경계를 잘게 나눌수록 위협은 빠짐없이 도출되지만 검토 공수가 급증하므로, 분해 수준은 설계 일정이 감당할 수 있는 백로그 규모에 맞춰 정해야 한다.
