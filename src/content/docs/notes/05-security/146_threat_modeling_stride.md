...
- 시스템 경계를 그린 뒤 누가 어떤 조건으로 무엇을 공격하고 어떻게 막을지 연결하는 것이 핵심이다.
...
- 그림만 남기지 않고 각 위협을 누가 고치고 무엇으로 확인할지 연결하는 것이 핵심이다.
...
- **1. 범위•자산•가정 정의**: **범위•자산•가정 정의**로 보호 목표를 확정한다.
- **2. DFD•신뢰 경계 분해**: **DFD•신뢰 경계 분해**로 흐름을 작성한다.
- **3. STRIDE•공격지식 위협 식별**: **STRIDE•공격지식 위협 식별**로 시나리오를 도출한다.
- **4. 가능성•영향•대응 검증**: **가능성•영향•대응 검증**으로 요구•시험을 연결한다.
- **5. 잔여위험 승인•갱신 조건 결정**: **잔여위험**을 승인하고 **모델 갱신** 조건을 정한다.

#### 한줄 요약

- 새 API•데이터 흐름이 생기면 경계와 위협을 다시 보고 기존 대응이 유효한지 확인하는 것이 핵심이다.
