...
- 정의/개념: 설계 흐름에서 **공격 시나리오•영향•대응**을 도출하는 활동이다.
- 배경/필요성: 구현 후 발견 시 경계•권한 구조의 **재설계 비용이 증가한다**.
...
- 범위•자산•가정•경계의 **공통 분석 기준**이 핵심이다.
- STRIDE 기반 **설계 요소별 위협 누락 점검**이 핵심이다.
...
- **1. 범위•자산•가정 정의**: 보호 목표•환경•의존성 확정이다.
- **2. DFD•신뢰 경계 분해**: 주체•저장소•흐름 작성이다.
- **3. STRIDE•공격지식 위협 식별**: 시나리오•사전조건을 도출한다.
- **4. 가능성•영향•대응 검증**: 요구•설계•시험을 연결한다.
- **5. 잔여위험 승인•갱신 조건 결정**: 변경•운영 사건의 재검토 조건 확정이다.
...
- **위험 처리**: 고영향 위협은 회피•완화하고 통제 뒤 남은 위험은 책임자가 근거를 검토해 승인하는 의사결정한다.

</details>

- 고영향 경계 위협은 **회피•완화**, 통제 후 잔여위험만 **승인**이 핵심이다.
