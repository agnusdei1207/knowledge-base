...
1. **외부 입력·실행점 추적**: 폼·API·저장값 흐름 확인
2. **준비된 질의·값 바인딩**: SQL 구조와 입력값 분리
3. **식별자 허용목록 제한**: 테이블·열 이름 매핑
4. **최소 권한·오류 노출 통제**: 스키마 권한과 응답 제한
5. **인젝션 시험·로그 감사**: 우회 입력과 이상 질의 검증
