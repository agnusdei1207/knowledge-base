...
- 정의/개념: 비신뢰 입력이 질의 의미를 바꾸는 **주입 취약점**이다.
- 배경/필요성: 문자열 결합은 **SQL 구조•값 분리할 수 없다**.
...
- 문자열 결합 입력의 **구문 경계 변조**가 핵심이다.
- 응답•오류•시간차의 **데이터 추론**이 핵심이다.
- 준비된 질의•최소 권한의 **예방•영향 제한**이 핵심이다.
...
1. **준비된 SQL 구조 생성**: 실행할 명령 형식과 매개변수 위치를 고정한다.
2. **입력값 바인딩**: 외부 입력을 명령이 아닌 매개변수 값으로 전달한다.
3. **SQL 구조 컴파일•값 처리**: 입력을 명령이 아닌 매개변수로 해석한다.
4. **최소 권한 질의 실행**: 허용 스키마•연산 범위에서만을 처리한다.
5. **질의•오류 감사 기록**: 이상 질의와 오류 노출 여부를 검증 자료로 제공한다.
...
- 값은 **준비된 질의로 바인딩**, 식별자는 **허용목록**, DB 계정은 **최소 권한**을 적용한다.
