...
- **단순 전력 분석(Simple Power Analysis, SPA)**: 단일 전력 파형의 특징에서 연산 순서와 비밀 정보를 추론하는 공격 기법이다.
- **차분 전력 분석(Differential Power Analysis, DPA)**: 다수 전력 파형의 통계적 차이를 분석하여 암호 키 후보를 판별하는 공격 기법이다.
...
- **전자기(Electromagnetic, EM)**: 연산 중 방출되는 전자기 신호를 측정하여 비밀 정보를 추론하는 부채널이다.
- **중앙처리장치(Central Processing Unit, CPU)**: 명령을 실행하고 캐시와 메모리를 제어하며, 처리 시간과 캐시 상태에 부채널 흔적을 남기는 연산 장치이다.
...
1. 암호 모듈의 비밀 의존 분기와 테이블 접근을 제거하고, 마스킹과 실행 격리를 적용한 뒤 실제 키 복구 시험으로 잔여 누설을 검증한다.
...
- 연산 과정의 물리·시간 정보로 암호 키가 노출되는 것을 막기 위해 누설 모델·공격자 측정 능력·공유 자원·성능 영향을 검토하여, 상수 시간·마스킹·격리를 적용하고 실제 키 복구 시험으로 재검증해야 한다.
