...
- **디지털 권리 관리(Digital Rights Management, DRM)**: 디지털 라이츠 매니지먼트, 디알엠으로 읽으며, 각 영단어의 머리글자를 딴 표기이고 본문에서는 암호화와 라이선스로 파일의 열람·편집·출력 권한을 지속 통제합니다.
...
- **데이터 유출 방지(Data Loss Prevention, DLP)**: 데이터 로스 프리벤션, 디엘피로 읽으며, 각 영단어의 머리글자를 딴 표기이고 본문에서는 민감정보를 식별해 저장·사용·전송 경로의 비인가 이동을 제한합니다.
...
- DRM은 배포 후 파일 사용 권한을 유지한다.
- DLP는 데이터 내용과 이동 경로를 검사한다.
- 공통 분류 정책이 두 통제 기준을 통일한다.
- 화면 촬영·재작성은 별도 행위 통제한다.
...
| 주요 위험 | 화면 촬영·라이선스 장애 | 암호화 데이터 누락·오탐 | 승인 사용자의 유출 행위 |
