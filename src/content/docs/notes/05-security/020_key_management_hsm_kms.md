...
- **루트 키(Root Key)**: 하위 키를 보호하며 조직 암호체계의 최상위 신뢰점이 되는 키
- **데이터 암호화 키(Data Encryption Key, DEK)**: 실제 데이터를 암호화하는 하위 대칭키
- **봉투 암호화(Envelope Encryption)**: DEK로 데이터를 암호화하고 키 암호화 키로 DEK를 다시 암호화하는 방식
- **키 암호화 키(Key Encryption Key, KEK)**: DEK 등 다른 암호키를 암호화하여 보호하는 상위 키
- **키 래핑(Key Wrapping)**: 한 암호키로 다른 암호키의 기밀성과 무결성을 보호하는 처리
- **하드웨어 보안 모듈(Hardware Security Module, HSM)**: 암호키를 외부로 노출하지 않고 내부에서 암호 연산을 수행하는 전용 보안 장비
- **키 관리 시스템(Key Management System, KMS)**: 암호키의 정책·권한·버전·수명주기와 사용 이력을 관리하는 시스템
- **신원·접근 관리(Identity and Access Management, IAM)**: 사용자와 서비스의 신원을 확인하고 자원 접근 권한을 통제하는 체계
- **응용 프로그래밍 인터페이스(Application Programming Interface, API)**: 시스템 간 기능과 데이터를 호출하기 위한 규약
- **비밀 관리기(Secret Manager)**: 비밀번호·토큰·API 키 등 응용 비밀정보의 저장·배포·회전을 관리하는 서비스
- **제로화(Zeroization)**: 암호키를 메모리와 저장매체에서 복구할 수 없도록 삭제하는 처리
- **이중 통제(Dual Control)**: 중요 작업을 수행할 때 둘 이상의 독립된 승인을 요구하는 원칙
- **분할 지식(Split Knowledge)**: 한 사람이 전체 비밀정보를 알 수 없도록 여러 관리자에게 나누는 원칙
...
- 암호키의 노출과 오용을 방지하기 위해 키 등급·사용 주체·수명주기·감사 요구를 검토하여, HSM의 보호 경계와 KMS의 통제 정책을 결정해야 한다.
