...
- 정의/개념: 암호키의 전 수명주기(생성, 보관, 배포, 회전, 폐기)를 계층화된 인프라 및 정책 기반으로 통제하는 **키 관리(Key Management)** 체계
- 배경/필요성: 암호 알고리즘 자체의 안전성에도 불구하고, 애플리케이션 하드코딩이나 부적절한 메모리 노출로 인한 암호키 유출 사고 방지 요구 증대
...
| HSM•KEK | FIPS 인증 물리 장비 내 KEK 생성, 암호 연산 및 템퍼 탬퍼링 탐지 차단 |
...
- NIST SP 800-57/FIPS 140-3 준수, KEK/DEK 봉투 암호화, HSM/KMS 계층화 및 메모리 제로화 기반 키 관리 체계 구축 필수