...
- 배경/필요성: 외부 분석 전 복호화는 **사용 중 평문 노출 유발**
...
```text
[인코더•암복호화기] ----- [키•파라미터 저장소] ----- [동형 평가 엔진] ----- [노이즈•레벨 관리자]
```

선의 의미: 선은 값의 암복호화 경계, 평가 키•암호 파라미터, 암호문 연산 엔진과 노이즈•레벨 통제가 결합되는 정적 동형 암호 실행 구조를 뜻한다.
...
3. **암호문 회로 평가**: 재선형화와 필요 시 부트스트래핑으로 노이즈•레벨을 관리하며 계산
4. **평가 암호문 복호화•검증**: 평문 결과 복구와 노이즈•허용 오차 판정
...
- 정확한 정수 연산은 **BFV•BGV**, 근사 실수 연산은 **CKKS** 선택
