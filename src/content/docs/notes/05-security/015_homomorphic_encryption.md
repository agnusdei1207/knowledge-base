...
- **동형 암호(Homomorphic Encryption)**: 암호문을 복호화하지 않고 연산해도 그 결과를 복호화하면 평문 연산 결과와 대응하는 공개키 암호
- **평문·암호문(Plaintext·Ciphertext)**: 원본 데이터와 비밀키 없이는 내용을 알기 어렵게 변환한 데이터
- **동형성(Homomorphism)**: 암호문 연산의 복호화 결과가 동일한 평문 연산 결과와 대응하는 성질
- **제한 동형 암호(Leveled Homomorphic Encryption)**: 미리 정한 회로 깊이 안에서 덧셈·곱셈을 수행하는 방식
- **완전 동형 암호(Fully Homomorphic Encryption, FHE)**: 부트스트래핑으로 노이즈를 갱신하여 임의 깊이의 연산을 지원하는 동형 암호
- **회로 깊이(Circuit Depth)**: 앞 연산에 의존해 순차적으로 이어지는 곱셈 단계 수
- **노이즈 예산(Noise Budget)**: 계산 후에도 정확히 복호화할 수 있는 노이즈 허용 범위
- **모듈러스(Modulus)**: 계수 계산 범위와 노이즈 예산·보안 수준을 결정하는 나눗셈 기준값
- **레벨(Level)**: 남은 모듈러스 단계와 추가로 수행할 수 있는 곱셈 범위
- **스케일(Scale)**: CKKS에서 실숫값의 정밀도를 관리하는 배율
- **부트스트래핑(Bootstrapping)**: 암호문의 노이즈를 줄여 추가 연산 여유를 확보하는 기법
- **재선형화(Relinearization)**: 곱셈 후 커진 암호문 차수를 평가 키로 줄이는 기법
- **평가 키(Evaluation Key)**: 재선형화·회전·부트스트래핑을 지원하는 공개 보조 키
- **암호문 슬롯(Ciphertext Slot)**: 한 암호문에서 같은 연산을 병렬 적용하는 논리적 위치
- **BFV(Brakerski-Fan-Vercauteren)**: 정확한 정수·모듈러 연산을 지원하는 동형 암호 방식
- **BGV(Brakerski-Gentry-Vaikuntanathan)**: 레벨별로 깊은 정수 연산을 지원하는 동형 암호 방식
- **CKKS(Cheon-Kim-Kim-Song)**: 실수·복소수의 근사 연산을 지원하는 동형 암호 방식
...
- 민감 데이터를 복호화하지 않고 처리하기 위해 연산 종류·회로 깊이·정확도·성능·키 신뢰를 검토하여, 필요한 계산 범위에 맞는 동형암호 방식을 선택해야 한다.
