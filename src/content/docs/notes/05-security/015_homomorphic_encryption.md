...
- 정의/개념: 암호문 연산이 평문 연산과 대응하는 **공개키 암호**이다.
- 배경/필요성: 외부 분석 전 복호화는 **사용 중 평문 노출을 유발한다.
...
- 제한형 깊이•FHE 부트스트래핑의 **노이즈 예산 관리**가 핵심이다.
- 평문 대비 **암호문 크기•연산량 증가**가 핵심이다.
- 데이터 **기밀성 제공•계산 무결성 미보장**이 핵심이다.
...
2. **입력 인코딩•암호화**: 슬롯 구성•입력 암호문을 생성한다.
3. **암호문 회로 평가**: 재선형화와 필요 시 부트스트래핑으로 노이즈•레벨을 관리하며 계산한다.
...
> 제한 동형 암호는 정한 깊이 안에서 계산하고, FHE는 부트스트래핑으로 연산 깊이를 연장한다.
...
| **깊이•노이즈 예산 고갈** | **회로•레벨 산정•부트스트래핑** | **복호화 실패 방지** |
| CKKS의 **스케일 오류** | **연산 단계별 스케일 정렬** | **근사 오차 누적 억제** |
| **계산 결과 무결성 부재** | **별도 증명•표본 재계산** | **오연산•누락 탐지** |
...
- 정확한 정수 연산은 **BFV•BGV**, 근사 실수 연산은 **CKKS**을 선택한다.
