...
- 정의/개념: **CTI**는 IoC에 공격자•TTP•신뢰도 맥락을 부여한 **방어 정보**이다.
- 배경/필요성: 출처•시점 없는 단순 지표로는 **차단 우선순위 판단할 수 없다**.
...
- 공격자•TTP•대상을 연결한 **위협 맥락**이 핵심이다.
- 출처•근거에 따른 **신뢰도 평가**가 핵심이다.
- 관측 시점•만료•철회 기반 **수명 관리**가 핵심이다.
...
1. **정규화•중복 제거**: 원천별 형식 통일과 중복 지표를 병합한다.
2. **신뢰도•수명 평가**: 출처•관측 시점•근거를 검증한다.
3. **맥락 분석•산출물 생성**: 자산•TTP•확신 수준을 연결한다.
...
- IoC 차단은 **전술 CTI**, 캠페인은 **운영 CTI**, 투자•위험은 **전략 CTI**을 선택한다.
