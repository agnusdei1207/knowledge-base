...
- 정의/개념: SoC 자원을 보안 상태별로 분리하는 **하드웨어 격리 기술**이다.
- 배경/필요성: 소프트웨어 권한 분리만으로는 커널 침해 시 **비밀 보호할 수 없다**.
...
- CPU•메모리•버스의 **보안 속성 전파**가 핵심이다.
- A-profile•M-profile의 **하드웨어 상태 격리**가 핵심이다.
- 제한 진입점•최소 TCB의 **경계 통제**가 핵심이다.
...
- **1. 공유 주소•길이•권한 검증**: 공유 메모리 복사•재검증한다.
- **2. 호출자 신원•TA 명령 검증**: 제한 진입점의 서비스 호출을 검증한다.
- **3. 최소 보안 자원 요청 구성**: 허용된 키•암호•저장소 연산을 선택한다.
- **4. 제한된 보안 연산 수행**: 비밀을 노출하지 않는 결과를 생성한다.
...
- 키•인증은 **TEE에 최소 격리**, 일반 기능은 REE에 배치한다.
