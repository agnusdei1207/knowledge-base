...
- 정의/개념: 버퍼 경계 밖 쓰기로 제어정보를 손상하는 **취약점**이다.
- 배경/필요성: 경계 미검사는 **제어 흐름 변조**로 연결이 필요하다.
...
- **데이터 실행 방지(Data Execution Prevention, DEP)**는 데이터 메모리 영역의 코드 실행을 금지한다.
- **실행 불가(No-eXecute, NX)**는 메모리 페이지에 실행 불가 속성을 부여한다.
- **주소 공간 배치 무작위화(Address Space Layout Randomization, ASLR)**는 메모리 영역의 시작 주소를 무작위화한다.
- **위치 독립 실행 파일(Position Independent Executable, PIE)**은 코드와 데이터를 임의 주소에 배치할 수 있게 한다.

</details>

- 경계 초과 쓰기의 **데이터•제어정보 손상**이 핵심이다.
- 안전 코드•카나리의 **원인•변조 탐지**가 핵심이다.
- PIE와 **DEP•ASLR**의 다층 악용 억제가 핵심이다.
...
1. **길이 미검사 복사**: 입력 크기를 확인하지 않고 고정 버퍼에 기록한다.
2. **버퍼 경계 초과 쓰기**: 인접한 스택 메모리까지 데이터를 덮어쓰기한다.
3. **카나리•반환 주소 손상**: 보호값과 제어 정보를 공격값으로 변조한다.
4. **카나리 무결성 검사**: 함수 반환 전에 보호값 변조 여부를 확인한다.
5. **변조 제어 흐름 전환 시도**: DEP•ASLR을 우회해 공격 코드로 이동한다.
...
- **공통 약점 열거(Common Weakness Enumeration, CWE)-787**은 할당된 버퍼 경계 밖에 쓰는 약점을 정의한다.
...
- 원인은 **경계 검사**, 잔존 악용은 **다층 실행 보호**로 억제가 핵심이다.
