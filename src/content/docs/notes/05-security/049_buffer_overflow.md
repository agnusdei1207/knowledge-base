...
date: "2026-07-31T01:57:42+09:00"
tags:
  - "notes-security"
weight: 49
extra:
...
- **퍼징(Fuzzing)**: 비정상·경계 입력을 반복 주입해 충돌과 보안 결함을 찾는 시험 기법이다.
...
sequenceDiagram
    ...
    participant 운영체제
    participant 보안검증
    개발자->>컴파일러: 1. 경계 검사 적용 코드
    컴파일러->>운영체제: 2. 카나리·PIE 바이너리
    운영체제->>보안검증: 3. DEP·ASLR 실행 상태
    보안검증->>개발자: 4. 경계값·크래시 시험 결과
```

**동작 원리**

1. **경계 검사 적용 코드**: 입력 크기와 버퍼 용량을 비교해 초과 복사 거부
2. **카나리·PIE 바이너리**: 스택 변조 탐지값과 위치 독립 속성 적용
3. **DEP·ASLR 실행 상태**: 데이터 실행 금지와 주소 난수화 적용 정보 제공
4. **경계값·크래시 시험 결과**: 퍼징·극단 입력의 비정상 종료와 회귀 여부 전달
...
| 잔존 결함 하나로 제어 흐름 변조·코드 실행 | **카나리·PIE·DEP·ASLR** 적용 | 악용의 **코드 실행 성공률** 저하 |
| 빌드별 보호 옵션 차이로 방어 누락 | **산출물 검사·퍼징·회귀시험** 수행 | 빌드 편차와 **우회 경로** 조기 발견 |
...
- 원인은 **경계 검사·메모리 안전 언어**, 잔존 악용은 **카나리·DEP·ASLR**로 억제
