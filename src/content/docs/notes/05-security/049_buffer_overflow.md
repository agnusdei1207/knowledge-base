...
- 정의: 경계 초과 쓰기의 **인접 메모리 손상**
- 기존 한계: 경계 미검사의 **제어 흐름 변조**
...
| 절차 | 설명 |
|:---|:---|
| 입력 길이·복사 경로 식별 | 외부 크기·버퍼 용량 추적 |
| 경계 검사·안전 구조 적용 | 초과 입력을 복사 전 거부 |
| 카나리·PIE·DEP 활성화 | 변조·실행·주소 예측 억제 |
| 바이너리 보호 속성 확인 | 최종 산출물 옵션을 검사 |
| 경계값·크래시 회귀검증 | 극단 입력·비정상 종료 추적 |
...
- **입력길이·언어안전성·보호속성**으로 메모리 통제를 결정한다.
