...
- 배경/필요성: 생성형 AI 및 LLM 기반 자율 에이전트(Agentic AI)가 사내 인프라 및 DB와 직접 연동됨에 따라, 프롬프트 한 줄로 데이터베이스가 삭제되거나 사내 기밀이 외부로 유출되는 치명적 침해 사고를 방어할 요구
...
```text
┌─────────────────────────────────────────────────────────────────────────┐
│ [ 1. Data & Model 공급망 계층 (Data Lineage & Model Integrity) ]        │
│  ├─ 데이터 계보(Lineage) 추적 ➔ [ 학습 데이터 오염(Poisoning) 방지 ]    │
│  └─ 모델 가중치 암호화 & TEE 구동 ➔ [ 가중치 탈취(Extraction) 차단 ]     │
└────────────────────────────────────┬────────────────────────────────────┘
                                     │ (검증된 파운데이션 모델 배포)
                                     ▼
┌─────────────────────────────────────────────────────────────────────────┐
│ [ 2. 추론 및 애플리케이션 계층 (Prompt & RAG Trust Boundary) ]          │
│  ├─ [1차 입력] 사용자 프롬프트 ➔ LLM Guardrails (인젝션/탈옥 필터링)    │
│  ├─ [2차 검색] 벡터 DB RAG 검색 ➔ 사용자 권한(ACL) 기반 문서 선별     │
│  └─ [3차 모델 추론] 시스템 지시어와 비신뢰 문맥 완전 분리 파싱          │
└────────────────────────────────────┬────────────────────────────────────┘
                                     │ (모델의 도구 호출 제안: Tool Call)
                                     ▼
┌─────────────────────────────────────────────────────────────────────────┐
│ [ 3. 에이전트 및 도구 실행 계층 (Tool Boundary & Agent Sandbox) ]       │
│  ├─ Policy Engine: 에이전트의 API 파라미터 유효성 및 권한 범위 검증     │
│  ├─ Human-in-the-loop: 고위험 액션(DB DROP, 자금 이체) 시 인간 승인 강제│
│  └─ [ 샌드박스 런타임에서 API/SQL 실행 ➔ 비인가 외부 C2 통신 차단 ]   │
└─────────────────────────────────────────────────────────────────────────┘
```

선의 의미: 데이터/모델 계층에서 무결성을 확보하고, 애플리케이션 계층에서 프롬프트/RAG를 격리하며, 최종 에이전트 계층에서 도구 실행을 통제하는 구조
...
1. **선제적 입력 정화**: 악의적인 프롬프트가 모델의 추론 엔진에 도달하기 전 가드레일에서 인라인 필터링
2. **권한 기반 데이터 격리**: 타인의 기밀 문서가 RAG 검색을 통해 프롬프트로 유입되는 권한 상승 방지
3. **지시어와 데이터의 구문적 분리**: 비신뢰 RAG 검색 결과를 데이터로만 인식하도록 프롬프트 구조화
4. **결정론적 도구 제어**: AI 모델이 임의로 API를 호출하지 못하도록 엄격한 스키마와 인가 정책 강제
5. **다계층 출력 감사**: 모델의 최종 응답 텍스트에 포함된 민감정보(주민번호, API 키)를 2차 마스킹

#### 한줄 요약
- 가드레일 입력 검사, ACL RAG 검색, 모델 추론, 도구 정책/Human 승인, 샌드박스 실행 및 출력 마스킹 순으로 동작한다.
...
- 생성형 AI 및 자율 에이전트의 확산에 따른 보안 패러다임의 확장을 다루는 **AI Security Threat Landscape** 는 미래 지능형 엔터프라이즈 보안의 핵심 나침반이며, 실무 구현 시 **NIST AI RMF 1.0 및 AI 600-1 표준 준수**, **Data, Model, App, Agent 4대 계층 맞춤형 방어선 구축**, **프롬프트/RAG/도구 3중 신뢰 경계 및 Guardrails 도입**, **고위험 에이전트 명령에 대한 Human-in-the-loop 승인 거버넌스**를 통합 구축하여 안전하고 신뢰할 수 있는 AI 생태계를 완성
