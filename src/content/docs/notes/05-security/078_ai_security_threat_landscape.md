...
date: "2026-08-05T17:40:27+09:00"
tags:
  - "notes-security"
weight: 78
extra:
...
- **인공지능(Artificial Intelligence, AI) 보안 위험관리**: 데이터•모델•응용•검색•도구의 공격면을 수명주기 전체에서 통제하는 체계
- **모델 고유 위협**: 기존 응용 보안만으로 다루기 어려운 데이터 오염•가중치 변조•인젝션•에이전트 권한 오용 위험이다.

</details>

- 정의/개념: AI 수명주기의 공격면을 식별•평가•통제하는 **위험관리 체계**
...
- **검색 증강 생성(Retrieval-Augmented Generation, RAG) 경계**: 검색 문맥을 명령이 아닌 비신뢰 데이터로 구분하는 통제 지점
...
선의 의미: 데이터•모델•응용•도구•운영 대응의 정적 신뢰 경계
...
- **접근 제어 목록(Access Control List, ACL) 기반 검색**: 사용자에게 허용된 문서 조각만 모델 문맥에 전달하는 방식
...
- **위험관리 프레임워크(Risk Management Framework, RMF)**: 위험을 식별•측정•대응•관리하는 구조화된 체계
