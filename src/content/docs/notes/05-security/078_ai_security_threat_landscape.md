...
- **인공지능(Artificial Intelligence, AI)**: 데이터에서 패턴을 학습하여 예측·생성·의사결정을 수행하는 기술이다.
- **검색 증강 생성(Retrieval-Augmented Generation, RAG)**: 외부 문서를 검색하여 모델의 입력 문맥에 결합함으로써 생성 결과의 근거와 최신성을 보완하는 방식이다.
...
1. RAG 문서의 **간접 프롬프트 인젝션과 도구 오용 차단**

### 쉽게 이해하기 (학습용)

- RAG 서비스는 검색 문서를 신뢰할 수 없는 데이터로 표시하고, 문서 속 지시가 검색 권한을 넘거나 외부 도구를 실행하려 하면 정책 계층에서 거부한다.

## Ⅶ. 결론

- 기존 애플리케이션 보안만으로 다루기 어려운 AI 위험을 줄이기 위해 데이터·모델·검색 문서·출력·도구 권한의 신뢰 경계를 검토하여, AI 수명주기 전체를 통합 보호해야 한다.
