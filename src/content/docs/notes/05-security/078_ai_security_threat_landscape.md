...
- 정의/개념: AI 수명주기의 공격면을 식별•평가•통제하는 **위험관리 체계**이다.
- 배경/필요성: 기존 앱 보안만으로 **모델 조작•도구 오남용 통제하기 어렵다**.
...
- **계보•공급망 무결성**: 데이터•가중치 변조를 추적한다.
- **신뢰 경계 분리**: 프롬프트•RAG•도구 권한 격리이다.
...
- 계보•서명 실패는 **모델 배포 차단**, 고위험 도구 호출은 **사람 승인**이 핵심이다.
