...
- 정의/개념: 데이터부터 에이전트까지의 **AI 보안 위협 체계**
- 배경/필요성: 전통 통제만으로는 **모델 탈취•도구 오용** 차단 불가
...
1. **사용자•권한 맥락 검증**: IAM 신원•인가 범위 확인
2. **ACL•출처 기반 문서 선별**: 권한•무결성 문서만 검색
3. **지시•비신뢰 문맥 분리**: 프롬프트•검색 문서 격리
4. **응답•도구 제안 생성**: 응답•도구 호출 후보 생성
5. **내용•권한•민감도 정책 검증**: PII•권한•도구 호출 검증
