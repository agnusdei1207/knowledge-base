...
1. AIoT 센서의 **저확신 추론 시 안전 상태 전환**

### 쉽게 이해하기 (학습용)
- 스마트 설비는 센서 이상과 모델의 낮은 확신도를 감지하면 자동 제어를 중단하고 사전에 정의한 안전 상태로 전환한다.

## Ⅶ. 결론

- AI 판단 오류가 물리 제어 피해로 이어지는 것을 막기 위해 센서 진위·추론 확신도·모델 무결성·액추에이터 영향을 검토하여, 이상 시 AIoT를 안전 상태로 전환해야 한다.
