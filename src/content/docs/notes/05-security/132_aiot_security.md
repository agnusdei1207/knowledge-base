...
- 정의/개념: 장치 신원부터 센서 데이터•인공지능 모델•통신•물리 제어까지 연쇄 위험을 통제하는 **AIoT 융합 보안 체계**이다.
- 배경/필요성: 기존 IoT 보안만으로는 AI 오판에 따른 **물리 피해 통제할 수 없다**.
...
- 장치•통신•모델•제어의 **연결 공격면**이 핵심이다.
- 센서 스푸핑•적대적 예제 대응의 **입력 검증**이 핵심이다.
- 추론 불확실성 기반 **안전 상태 전환**이 핵심이다.
...
- **1. 장치 신원•입력 일관성 검증**: 출처•시각•다중 센서값 대조이다.
- **2. 모델 추론•확신도 산정**: 승인 모델의 판단과 불확실성 산출한다.
- **3. 허용 범위•안전 상태 결정**: 물리 영향에 따른 제어 한계를 결정한다.
- **4. 제한 제어•안전 전환 실행**: 속도 제한•중지•안전 상태를 적용한다.
...
> 요약: 현장 근접 추론은 **지연•전송량 감소**, **장치 관리점 증가**가 핵심이다.
...
- 입력 불일치•저확신•고위험이면 **자동 제어 중지•안전 상태 전환**이 핵심이다.
