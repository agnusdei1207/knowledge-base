...
date: "2026-08-05T12:44:00+09:00"
tags:
  - notes-security
weight: 132
extra:
...
선의 의미: 장치 신뢰 기반 아래 센서 데이터와 통신•플랫폼 경계를 분리하고, AI 모델•추론 보호 및 액추에이터•안전 관제에 결합한 정적 AIoT 보호 구조
...
sequenceDiagram
  ...
  S->>D: 장치 신원•시각•원시 센서값 전달
  D->>D: 1. 장치 신원•입력 일관성 검증
  D->>M: 검증된 입력 전달
  M->>M: 2. 모델 추론•확신도 산정
  M->>G: 추론 증거 전달
  G->>G: 3. 허용 범위•안전 상태 결정
  G->>A: 제어 명령 전달
  A->>A: 4. 제한 제어•안전 전환 실행
  A-->>D: 안전 상태•실행 결과
```

**동작 원리**

- **1. 장치 신원•입력 일관성 검증**: 출처•시각•다중 센서값 대조
- **2. 모델 추론•확신도 산정**: 승인 모델의 판단과 불확실성 산출
- **3. 허용 범위•안전 상태 결정**: 물리 영향에 따른 제어 한계 결정
- **4. 제한 제어•안전 전환 실행**: 속도 제한•중지•안전 상태 적용
