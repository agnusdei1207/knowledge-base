...
- **정의/개념**: 장치·센서·AI·물리 제어의 통합 보호
- **배경/필요성**: 모델 오판이 현실 제어 사고로 전이
...
- 장치·통신·모델·제어기 공격면 상호 연결한다.
- 입력 진위·모델 정확도·강건성·드리프트 관리한다.
- 경량 통제 및 안전 복구 설계한다.
- 판단 불확실성 기반 자동 제어 정책을 정한다.
...
| 비교축 | 클라우드 추론 | 엣지 게이트웨이 | 장치 내 추론 |
|:---|:---|:---|:---|
| 핵심 특징 | 중앙에서 대규모 모델 운영 | 근거리 장치군의 데이터를 모아 추론 | 개별 장치에서 즉시 추론 |
| 적용 기준 | 풍부한 연산·중앙 관리가 필요할 때 | 지연·대역폭과 중앙 관리를 절충할 때 | 저지연·오프라인 동작이 필요할 때 |
| 주요 위험 | 전송·계정·테넌트 노출 | 게이트웨이 침해가 장치군으로 확산 | 물리적 모델 추출·자원 부족 |
...
1. 센서 융합으로 입력 위조를 검증
2. 이상치·낮은 확신도에서 안전 상태 전환
3. 최소 데이터 수집 후 모델 누출 평가
4. AI·펌웨어 SBOM으로 취약점 추적
