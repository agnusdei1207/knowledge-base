...
date: "2026-07-31T02:03:59+09:00"
tags:
  - "notes-security"
weight: 57
extra:
...
- **갱신 토큰 회전(Refresh Token Rotation)**: 갱신할 때마다 새 토큰을 발급하고 이전 토큰을 폐기하는 방식이다.
...
- **권한 코드·PKCE·정확한 Redirect URI**를 통한 코드 탈취 차단
- API 권한용 **접근 토큰**과 클라이언트 인증용 **ID 토큰** 분리
- **발급자·대상·범위·만료·Nonce** 검증과 갱신 토큰 회전
...
1. **범위·PKCE·Nonce 인가 요청**: 위임 범위와 거래 결속값 제공
2. **클라이언트·요청 범위**: 사용자에게 요청 앱과 권한 범위 표시
3. **인증·동의 증거**: 사용자 신원 확인과 허용 범위 결과 전달
4. **권한 코드**: 등록된 Redirect URI로 짧은 수명의 일회 코드 제공
5. **권한 코드·PKCE 검증값**: 코드와 원래 클라이언트의 결속 증거 전달
...
|:---|:---|:---|
| 적용 기준 | 타 앱에 **제한 권한 부여** | **통합 로그인·신원 확인** |
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 코드·토큰 탈취로 다른 클라이언트가 권한 획득 | **IETF RFC 9700** 적용 | PKCE·정확한 URI·**토큰 회전** 정렬 |
| ID Token을 잘못 검증하면 위조 신원 수용 | **OIDC Core 1.0** 준수 | **발급자·대상·Nonce** 검증 |
| 접근 토큰과 ID Token 혼동으로 권한 오용 | **API·로그인 토큰** 분리 | 토큰 목적 혼동에 따른 **권한 상승** 방지 |
