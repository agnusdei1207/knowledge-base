...
- 배경/필요성: 서드파티에 패스워드를 그대로 넘기면 위임 단위가 계정 전체가 되어 범위도 기간도 나눌 수 없고 회수하려면 비밀번호를 바꿔 다른 연동까지 함께 끊는 비용을 치르므로, 인가 서버가 범위(Scope)와 수명을 새겨 발급하는 **토큰** 계층을 사용자와 클라이언트 사이에 끼워 자격증명을 대신 내보낼 필요
