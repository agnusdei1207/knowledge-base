...
- 정의: OAuth의 **권한 위임**과 OIDC의 **신원 인증**
- 기존 한계: 비밀번호 공유의 **과잉 권한·자격 유출**
...
| 절차 | 설명 |
|:---|:---|
| 범위·PKCE·Nonce 인가 요청 | 위임 범위·거래 결속값 포함 |
| 사용자 인증·위임 동의 | 주체와 권한 범위를 확인 |
| 등록 URI·권한 코드 반환 | 정확한 주소로 일회 코드 전달 |
| PKCE 검증·토큰 교환 | 접근·ID 토큰 목적별 발급 |
| 대상·범위 검증·자원 요청 | 허용된 API·객체만 제공 |
