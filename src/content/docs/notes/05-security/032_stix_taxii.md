...
- 정의/개념: STIX **위협 표현**과 TAXII **객체 교환**이다.
- 배경/필요성: 비정형 문서로는 위협 객체•관계의 **자동 연계할 수 없다**.
...
- STIX 객체•관계 기반 **위협 맥락 표현**이 핵심이다.
- TAXII 컬렉션 기반 **조회•게시•동기화**가 핵심이다.
- 버전•표식•철회 기반 **수명•권한 통제**가 핵심이다.
...
1. **STIX 객체•프로파일 검증**: 객체•관계•표식•허용 범위를 확인한다.
2. **권한 검증•객체 저장**: 게시 권한과 스키마 확인 후를 저장한다.
3. **변경•철회 객체 선별**: 수정 시각 이후 객체와 상태를 추출한다.
...
- 위협 의미•관계는 **STIX**, 자동 컬렉션 교환은 **TAXII**, 표식•철회 검증 병행이 핵심이다.
