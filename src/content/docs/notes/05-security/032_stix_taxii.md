...
- 정의/개념: 위협 데이터의 구조화된 표현 표준인 **STIX(STIX)**와 안전한 자동 교환 규약인 **TAXII(TAXII)**의 상호 연계 체계.
...
- **표식•철회(Marking & Revocation)**: 정보의 민감도 취급 등급(TLP) 및 더 이상 유효하지 않은 지표의 폐기 상태를 명시하는 메타데이터에 해당함.

</details>

- **STIX** 객체•연관 관계 모델 기반 **CTI** 위협 맥락 정밀 표현.
- **TAXII 컬렉션(TAXII Collection)** 자원 구성을 통한 객체 조회•게시•실시간 동기화.
- 객체 버전 정보 및 **표식•철회(Marking & Revocation)** 메타데이터를 활용한 라이프사이클 및 보안 취급 통제.

#### 한줄 요약

- 받은 정보의 신뢰성 및 필요성 판단 필수가 핵심이다.
...
3. **변경•철회 객체 선별**: **변경분 조회(Delta Query)**를 통한 최신 변경 및 철회 객체 추출.
4. **검증 객체 적용**: 유효성 및 적합성 검증 완료 객체의 차단 통제 반영.

#### 한줄 요약

- 식별자와 수정 시각으로 중복 여부 판별이 핵심이다.
