...
- 정의/개념: 양자 취약 암호를 **PQC KEM•서명으로 교체하는 활동**이다.
- 배경/필요성: 숨은 암호 의존성으로 **HNDL 위험별 전환 순서 결정하기 어렵다**.
...
- **데이터 보존기간•위협 시점** 기반 전환 우선순위가 핵심이다.
- **고전•PQC 비밀 결합**을 통한 단일 알고리즘 실패를 완화한다.
- 정책•공급자 경계의 **암호 민첩성**으로 알고리즘 교체가 핵심이다.
- **자동 약화 금지**에 따른 PQC 협상 실패 가시화가 핵심이다.
...
2. **PQC KEM 시범 배포**: 대표 서비스에 새 키 설정 공급자를 적용한다.
...
- 지원 혼재 구간은 **하이브리드**, 전 구간 지원 후 **PQC 단독**을 전환한다.
