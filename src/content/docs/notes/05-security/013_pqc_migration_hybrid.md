...
- **PQC 전환(Post-Quantum Cryptography Migration)**: 양자 공격에 취약한 공개키 암호를 양자내성 키 설정·서명 방식으로 단계적으로 교체하는 작업이다.
...
- **결합기(Combiner)**: 기존 방식과 PQC 방식이 생성한 비밀을 하나의 세션키 원재료로 결합하는 함수다.
- **다운그레이드 공격(Downgrade Attack)**: 협상 정보를 조작하여 지원 가능한 방식보다 약한 암호를 선택하게 하는 공격이다.
- **NIST CSWP 15**: PQC 도입 과제와 암호 자산 식별·전환 계획을 제시한 공식 지침이다.
- **NIST SP 800-227**: KEM의 안전한 구현·사용과 공유 비밀 설정 권고를 제시한 공식 지침이다.
...
sequenceDiagram
    ...
    O->>T: 3. PQC 시범 배포
    T-->>A: 4. 크기·호환성·실패 검증
    A-->>P: 5. 운영 전환·고전 협상 차단
```

1. **CBOM·보존기간 전달**: 취약 암호 위치·기밀 수명 제공
2. **전환 우선순위·결합 규칙**: 위험순서와 결합·실패 정책 결정
3. **PQC 시범 배포**: 대표 서비스에 새 공급자 적용
4. **크기·호환성·실패 검증**: 장비·프로토콜 한계 확인
5. **운영 전환·고전 협상 차단**: 통과 범위 확대·약화 금지
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 암호 자산·우선순위 | **NIST CSWP 15 기반 조사** | HNDL 위험순 전환 |
| KEM 결합·실패 처리 | **NIST SP 800-227 검증** | 비밀 결합 오류 억제 |
...
- **보존기간·지원범위·약화위험**으로 PQC 전환방식을 결정한다.
