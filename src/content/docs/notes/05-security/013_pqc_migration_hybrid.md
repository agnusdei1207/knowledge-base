...
- **PQC 전환(Post-Quantum Cryptography Migration)**: [읽기: 피큐씨; 표기 이유: 영문 머리글자와 구분 기호] 양자 공격에 취약한 공개키 암호를 양자내성 키 설정·서명 방식으로 단계적으로 교체하는 작업
- **지금 수집·나중 해독(Harvest Now, Decrypt Later, HNDL)**: [읽기: 에이치엔디엘; 표기 이유: 영문 머리글자와 구분 기호] 현재 암호문을 저장했다가 미래 양자컴퓨터로 해독하려는 위협
- **암호 민첩성(Crypto Agility)**: [읽기: 씨알와이피티오; 표기 이유: 영문 머리글자와 구분 기호] 알고리즘과 매개변수를 코드에 고정하지 않고 정책과 공급자 경계에서 교체할 수 있는 설계 성질
- **하이브리드 방식(Hybrid Mode)**: [읽기: 에이치와이비알아이디; 표기 이유: 영문 머리글자와 구분 기호] 기존 공개키 암호와 양자내성암호의 결과를 결합해 전환기 호환성과 보안을 함께 검증하는 방식
- **암호 자재명세서(Cryptographic Bill of Materials, CBOM)**: [읽기: 씨비오엠; 표기 이유: 영문 머리글자와 구분 기호] 시스템이 사용하는 알고리즘·키·인증서·암호 라이브러리와 의존 관계를 기록한 목록
- **키 캡슐화 메커니즘(Key Encapsulation Mechanism, KEM)**: [읽기: 케이이엠; 표기 이유: 영문 머리글자와 구분 기호] 공개키로 공유 비밀과 캡슐화 암호문을 만들고 개인키로 같은 비밀을 복구하는 방식
- **결합기(Combiner)**: [읽기: 씨오엠비아이엔이알; 표기 이유: 영문 머리글자와 구분 기호] 기존 방식과 PQC 방식이 만든 비밀을 하나의 세션키 원재료로 합치는 함수
...
2. 하이브리드 실패 시 고전 방식 자동 전환 금지

### 쉽게 이해하기 (학습용)

- 오늘 암호문이 수년 뒤에도 가치가 있는 서비스부터 KEM을 바꿔, 데이터 보존기간이 예상 전환 완료 시점보다 길면 HNDL 위험을 먼저 줄인다.
- 새 KEM이 실패했을 때 기존 방식으로 조용히 연결하면 양자 대응이 사라지므로, 실패를 표시하고 연결을 거부하거나 승인된 예외만 사용한다.

## Ⅶ. 결론

- 보존기간·지원 범위로 결합·폐기 시점 결정
