...
- 정의: 반복 질의에 의한 **모델 기능·경계 복제**
- 기존 한계: 상세 출력 API의 **지식재산 노출**
...
| 절차 | 설명 |
|:---|:---|
| 경계 탐색 목표 설정 | 복제 기능·출력 수준 정의 |
| 정보량 높은 질의 | 불확실 경계 입력 선택 |
| 모델 응답 축적·학습 | 입출력 쌍으로 대리 모델 훈련 |
| 불일치 경계 반환 | 추가 학습 구간 식별 |
| 반복 패턴 탐지·귀속 | 계정·분포·워터마크 분석 |

> 요약: 능동 질의로 판단 경계를 모방한다.
...
| 한계 | 많은 질의 필요 | 경계 탐색 효율 증가 | 신속한 기능 복제 |
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 추출 공격 분류 | **NIST AI 100-2e2025 적용** | 지식·접근별 시험 체계화 |
| 대량 경계 탐색 | **계정별 한도·분포 행위 분석** | 능동 질의 비용 증가 |
