...
- 정의/개념: **모델 추출 공격**은 반복 질의로 원본 모델의 기능과 판단 경계를 복제하는 공격이다.
- 배경/필요성: 상세 출력 API의 **지식재산 노출**이 발생한다.
...
- 능동 질의의 **결정 경계 탐색**이 핵심이다.
- 상세 출력의 **복제 효율 증가**가 핵심이다.
- 행위 분석•워터마크의 **탐지•귀속**이 핵심이다.
...
2. **대리 모델 학습•경계 탐색**: 입출력 쌍으로 판단 규칙을 근사하고 불일치 경계를 확인한다.
3. **반복 패턴 탐지**: 계정•입력 분포•호출률을 분석한다.
4. **워터마크•카나리 귀속**: 복제 반응과 소유 증거를 대조한다.
...
- 출력 정보량•경계 질의가 높으면 **출력 최소화•호출률 제한**을 적용한다.
