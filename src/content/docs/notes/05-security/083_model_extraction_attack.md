...
- 정의/개념: 모델 추출 공격은 상용 AI 서비스 API를 통해 능동 학습(Active Learning) 입력쌍을 대량 송신하고 도출된 예측 결과를 학습 데이터셋으로 가공하여, 원본 모델의 지적재산권과 결정 경계(Decision Boundary)를 무단 복제하는 공격 기법이다.
- 배경/필요성: 수십억~수천억 원의 파이프라인 학습 비용이 투입된 독자적 파운데이션/특화 모델의 IP 보호 및 이를 악용한 2차 적대적 공격(Adversarial Example) 생성 발판을 무력화하기 위함이다.
...
1. **정보량 높은 질의 합성**: 합성 입력 생성기가 대상 모델의 결정 경계를 자극할 후보 입력군을 생성한다.
2. **대리 모델 학습•경계 탐색**: API 응답을 수집해 대리 모델을 지도 학습시키며, **불일치 경계 탐색**으로 99% 충실도에 도달할 때까지 반복 질의한다.
3. **반복 패턴 탐지**: 방어 시스템이 엔트로피 분포 및 질의 예산 초과 여부를 탐지한다.
4. **워터마크•카나리 귀속**: 추출된 모델에 **카나리 입력**을 투입하여 암호학적 핑거프린트/워터마크 반응을 추출해 소유권을 증명한다.
...
- 공격자의 **복제 비용**을 극대화하고 **출력 최소화** 및 **귀속** 증거(워터마크)를 보존하여 자산을 보호한다.
