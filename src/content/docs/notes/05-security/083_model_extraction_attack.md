...
- **대리 모델(Surrogate Model, 서러게이트 모델)**: 원본을 대신한다는 뜻으로, 수집한 입출력으로 대상 모델 기능을 근사

## Ⅰ. 개요

- 정의/개념: API 응답으로 모델 기능을 모방하는 공격이다
- 기존 한계: 상세·무제한 API 응답은 경계 복제 단서 제공
- **배경/필요성**: 출력·질의 통제로 모방 비용 증가
...
- 경계 질의와 상세 출력은 모방 학습 효율을 높인다
- 대리 모델은 가중치 탈취 없이 입출력 기능을 복제한다
- 호출률·출력·행위 분석 통제가 추출 비용을 높인다
...
| 한계 | 많은 질의 필요 | 경계 탐색 효율 증가 | 빠른 기능·표현 복제 |
...
1. 분류 API의 **상세 확률 제한과 질의 패턴 탐지**

### 쉽게 이해하기 (학습용)

- 분류 API는 필요한 상위 결과만 반환하고 결정 경계를 촘촘히 탐색하는 반복 질의를 계정·기기·입력 분포로 찾아 모델 복제를 어렵게 한다.

## Ⅶ. 결론

- API 질의를 통한 모델 복제를 어렵게 하기 위해 출력 정보량·경계 탐색 패턴·정상 사용자 영향·귀속 증거를 검토하여, 최소 출력과 행위 기반 제한을 적용해야 한다.
