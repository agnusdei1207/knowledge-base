...
- 정의/개념: **KEM 키 설정**과 **DEM 본문 보호**를 결합한 하이브리드 암호이다.
...
- 공개키 연산의 **키 설정 구간 한정**이 핵심이다.
- 대칭키의 **고속 본문 암호화**가 핵심이다.
- KDF 기반 **용도•방향별 키 분리**가 핵심이다.
...
가지의 의미: 인증•키 설정•키 분리•본문 보호 책임을 나눈 구조다.
...
1. **인증서•협상 기록 검증**: 신원•다운그레이드 여부를 판정한다.
2. **ECDHE•KEM 키 설정**: 임시 키 합의•비밀을 캡슐화한다.
3. **공유 비밀•방향별 키 도출**: 같은 비밀에서 용도별 키를 생성한다.
4. **AEAD 본문 보호**: 본문 기밀성•무결성을 보호한다.
...
- 현재 통신은 **ECDHE•AEAD**, 양자 전환은 **ECDHE•PQC KEM**을 선택한다.
