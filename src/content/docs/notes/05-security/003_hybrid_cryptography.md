...
- 정의/개념: 비대칭키 암호로 임시 세션키를 교환(KEM)하고, 이 세션키 기반의 대칭키 암호로 대용량 데이터를 고속 암호화(DEM)하는 복합 암호 체계
- 배경/필요성: 비대칭키의 치명적 연산 오버헤드와 대칭키의 키 분배 복잡도라는 상호 한계를 극복하고 효율성·안정성을 동시 달성하기 위한 설계

#### 한줄 요약

- 키 교환 단계의 비대칭 연산과 본문 보호 단계의 대칭 연산을 분리하여, 네트워크 리소스 소모를 최소화하는 고효율 암호화 아키텍처 구현
...
- 상호 인증(Authentication)이 결여된 단순 키 합의는 중간자 공격(MITM)에 취약하므로, PKI 기반 인증서 검증과의 결합 필수
...
- 단일 마스터 키의 직접 사용을 금지하고, KDF를 통해 용도(MAC, 암호화) 및 송수신 방향별로 독립된 하위 키를 파생하는 다층적 키 스케줄링 적용
...
- 암호 스위트 협상 내역 전체를 서명 검증 대상에 포함하여, 공격자의 취약 알고리즘 강제 다운그레이드 공격을 원천 차단하는 무결성 검증 체계 구현
...
- PQC 전환 시 기존 ECDHE와 양자 내성 KEM을 하이브리드로 결합하며, 구현 실패 시 단순 폴백(Fallback)을 차단하여 양자 보안 공백 방지 체계 필수
...
- 종단 간 인증, 영역 분리(Domain Separation) 기반 키 파생, AEAD 본문 보호 등 HPKE(RFC 9180) 표준 기반의 안전한 하이브리드 워크플로우 적용
