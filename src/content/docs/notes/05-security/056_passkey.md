---
sidebar:
  order: 56
  label: "056. 패스키 비밀번호 없는 인증 (Passkey Passwordless)"
  badge:
    text: "기출 · 70%"
    variant: note
title: "패스키 비밀번호 없는 인증 (Passkey Passwordless)"
date: "2026-07-27T23:59:59+09:00"
tags:
  - "notes-security"
weight: 56
extra:
  question_no: "056"
  source_status: "기출"
  source_history: "138회"
  priority: 70
  priority_note: "138회 기출과 비밀번호 없는 인증 전환성이 강함"
---

## 미리 알고가기

- **패스키(Passkey)**: 서비스별 공개키 자격 증명으로 비밀번호를 대신하는 피싱 저항 인증 수단이다.
- **온라인 신속 신원 확인 2(Fast IDentity Online 2, FIDO2)**: WebAuthn과 인증자 통신 규격을 결합하여 패스키 인증 기반을 제공하는 표준이다.
- **웹 인증(Web Authentication, WebAuthn)**: 서비스가 브라우저를 통해 공개키 자격 증명을 생성·사용하도록 정의한 표준이다.
- **신뢰 당사자(Relying Party, RP)**: 패스키 서명을 검증하고 인증 결과에 따라 서비스를 제공하는 서버이다.
- **응용 프로그래밍 인터페이스(Application Programming Interface, API)**: 브라우저와 서비스가 WebAuthn 기능을 호출하는 접점이다.
- **인증자(Authenticator)**: 개인키를 보호하고 사용자 승인 후 인증 서명을 생성하는 장치나 기능이다.
- **개인 식별 번호(Personal Identification Number, PIN)**: 인증자에서 개인키를 사용할 주체를 확인하는 비밀 번호이다.
- **신뢰 당사자 식별자(Relying Party Identifier, RP ID)**: 패스키 서명을 특정 서비스 도메인에 결속하는 식별자이다.
- **사용자 검증(User Verification)**: PIN이나 생체 정보로 개인키 사용자를 로컬에서 확인하는 절차이다.
- **동기화 패스키(Synced Passkey)**: 신뢰 플랫폼을 통해 여러 기기에서 사용할 수 있는 패스키이다.
- **기기 결합 패스키(Device-Bound Passkey)**: 개인키가 특정 인증자에서 이동하지 않는 패스키이다.
- **휴대형 보안키(Roaming Security Key)**: 여러 기기에 연결해 쓰는 물리적 외부 인증자이다.
- **복구 경로(Recovery Path)**: 기기 분실이나 교체 후 계정 접근을 되찾는 절차이다.
- **자격 증명 폐기(Credential Revocation)**: 분실·유출된 패스키가 더는 인증에 쓰이지 않게 만드는 처리이다.
- **세션(Session)**: 인증 결과를 일정 시간 유지하는 서버 상태이다.
- **W3C WebAuthn Level 2**: RP 결속 공개키 자격 증명의 생성·인증을 정의한 W3C 권고 표준이다.
- **NIST SP 800-63B-4**: 동기화 가능 인증자를 포함한 인증기·세션·복구 요구를 제시한 공식 지침이다.

## Ⅰ. 개요

- 정의: WebAuthn 기반 **서비스별 공개키 인증**
- 기존 한계: 비밀번호의 **피싱·재사용·서버 유출**

### 쉽게 이해하기 (학습용)

- 사용자는 기기 잠금을 풀듯 승인하고 서비스는 비밀번호 대신 자기 사이트에만 맞는 공개키 서명을 확인한다.

## Ⅱ. 특징

- **RP별 공개키·원본 결속**으로 피싱 사이트 재사용을 막는다.
- **동기화 패스키**는 기기 이전이 쉽고 **기기 결합형**은 이동을 제한한다.
- **기기 추가·복구·세션 폐기 강도**가 실제 인증 보증을 결정한다.

### 쉽게 이해하기 (학습용)

- 패스키 자체는 피싱에 강하지만 새 기기를 추가하거나 계정을 복구하는 문이 약하면 그 문으로 우회될 수 있다.

## Ⅲ. 구조 및 구성요소

```mermaid
block-beta
  columns 5
  A["RP 서버"]
  B["플랫폼·브라우저"]
  C["인증자"]
  D["사용자 검증"]
  E["동기화·복구 체계"]
```

| 설계 요소 | 설명 |
|:---|:---|
| RP 서버 | **도전값 발급·공개키 검증** |
| 플랫폼·브라우저 | **원본 확인·인증 요청 중계** |
| 인증자 | **개인키 보호·서명 생성** |
| 사용자 검증 | **PIN·생체로 키 사용 승인** |
| 동기화·복구 체계 | **기기 이전·분실 대응** |

### 쉽게 이해하기 (학습용)

- 서버가 새 질문을 보내면 플랫폼이 올바른 사이트인지 확인하고 인증자가 사용자 승인 후 서명한다.

## Ⅳ. 흐름도

```mermaid
sequenceDiagram
    participant U as 사용자
    participant R as RP 서버
    participant P as 플랫폼·브라우저
    participant A as 인증자
    U->>R: 1. 패스키·계정 선택
    R->>P: 2. 도전값·RP ID 전달
    P->>A: 3. 원본 결속·사용자 검증
    A-->>P: 4. 서비스별 개인키 서명
    P-->>R: 5. 도전값·원본·서명 검증
```

| 절차 | 설명 |
|:---|:---|
| 패스키·계정 선택 | 사용할 자격 증명을 지정 |
| 도전값·RP ID 전달 | 일회 값·서비스 경계를 제공 |
| 원본 결속·사용자 검증 | 사이트·PIN·생체를 확인 |
| 서비스별 개인키 서명 | 비노출 키로 응답을 생성 |
| 도전값·원본·서명 검증 | 재전송·피싱·위조를 판정 |

> 요약: 로컬 승인으로 서비스별 서명을 만든다.

### 쉽게 이해하기 (학습용)

- 기기에서 본인을 확인해도 생체 정보는 서버로 가지 않고 승인된 개인키의 서명 결과만 전달된다.

## Ⅴ. 종류 및 비교

| 패스키 보관 방식 | 공통 | 동기화 패스키 | 기기 결합 패스키 | 휴대형 보안키 |
|:---|:---|:---|:---|:---|
| 적용 기준 | 비밀번호 대체 | 소비자 편의·복구 | 조직의 이동 제한 | 강한 물리 분리 |
| 핵심 특징 | **서비스별 공개키 인증** | **여러 기기 안전 이전** | 특정 기기에 키 고정 | 별도 물리 장치 사용 |
| 한계 | 약한 대체 로그인 | 동기화 계정 탈취 | 분실 시 복구 부담 | 배포·회수·예비 키 |

### 쉽게 이해하기 (학습용)

- 같은 패스키라도 편리한 동기화, 이동을 막는 기기 결합, 별도 보관하는 보안키 중 운영 목적에 맞춰 선택한다.

## Ⅵ. 실무 고려사항 및 대책

| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| RP·원본·서명 검증 | **W3C WebAuthn Level 2 준수** | 피싱·재전송 저항 확보 |
| 동기화 인증자 보증 | **NIST SP 800-63B-4 적용** | 동기화·세션 위험 통제 |
| 기기 분실·복구 우회 | **복수 인증자·강한 복구·폐기** | 잠금·탈취 계정 재사용 방지 |

### 쉽게 이해하기 (학습용)

- 사용자는 새 휴대전화에서도 플랫폼 계정을 통해 동기화된 패스키를 복원해 비밀번호 없이 서비스에 로그인한다.

## Ⅶ. 결론

- **계정위험·동기화신뢰·복구강도**로 패스키 유형을 결정한다.

### 쉽게 이해하기 (학습용)

- 패스키 전환은 비밀번호 제거로 끝나지 않으며 기기 추가·동기화·분실 복구를 같은 인증 강도로 설계해야 완성된다.
