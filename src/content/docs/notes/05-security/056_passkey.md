...
date: "2026-08-05T09:56:00+09:00"
tags:
  - "notes-security"
weight: 56
extra:
...
1. **도전값•RP ID 생성**: 일회성 값과 자격 증명 서비스 경계 구성
2. **원본•RP ID 검증**: 현재 사이트와 자격 증명 결속 확인
3. **PIN•생체 사용자 검증**: 로컬에서 개인키 사용자를 확인
4. **서비스별 개인키 서명**: 비노출 키로 도전값•문맥 서명
5. **도전값•공개키 서명 검증**: 재전송•위조 응답을 서버에서 거부
