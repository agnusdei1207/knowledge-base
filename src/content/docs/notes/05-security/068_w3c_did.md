...
- 정의/개념: DID 구문•문서•해석•갱신을 규정한 **W3C 분산 식별자 표준**이다.
- 배경/필요성: 방법별 저장•해석 규칙 차이로 **키 해석 불일치**가 발생한다.
...
- **분산 식별자**: `did:method:id` 형식의 전역 이름이다.
- **검증 방법•관계 표현**: DID 문서로 키와 용도를 연결한다.
- **DID 방법**: 생성•해석•갱신•비활성화 규칙이다.
...
- **1. DID URI•방법 구문 해석**: 식별자에서 방법과 고유값을 분리한다.
- **2. 최신 DID 문서•상태 해석**: 방법 규칙으로 키•서비스•비활성 정보를 확인한다.
- **3. 문서•해석 메타데이터 검증**: 조회 결과와 오류•내용 유형을 확인한다.
- **4. 키 용도•제어 상태 판정**: 검증 관계와 현재 제어권을 확인한다.
#### 한줄 요약
...
- 방법이 검증되고 키 용도•상태가 맞으면 **DID 수용**, 하나라도 실패하면 **거부**가 핵심이다.
