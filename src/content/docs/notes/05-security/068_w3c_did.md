...
sequenceDiagram
    ...
    V->>R: 1. 검증 대상 DID 해석 요청
    R->>M: 2. 방법별 저장소 조회
    M-->>R: 3. 현재 DID 문서·상태 반환
    R-->>V: 4. 문서·해석 메타데이터 전달
    V->>D: 5. 키 용도·회전·비활성 판정
```

**동작 원리**

- **1. 검증 대상 DID 해석 요청**: 식별자·검증 목적을 제출
- **2. 방법별 저장소 조회**: DID 방법 규칙으로 접근
- **3. 현재 DID 문서·상태 반환**: 키·서비스·비활성 정보 제공
- **4. 문서·해석 메타데이터 전달**: 조회 결과·오류 상태 반환
- **5. 키 용도·회전·비활성 판정**: 검증 관계·현재 제어권 확인
### 쉽게 이해하기 (학습용)
...
| 핵심 특징 | **제어자 중심 키 해석** | **서비스 계정 식별** | 신원 제공자 식별 연계 |
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| DID 문서·키 용도 | **W3C DID Core 1.0 준수** | 식별·검증 관계 상호운용 |
| 방법별 거버넌스 편차 | **방법 규격·등록·감사 검토** | 저장소·갱신 위험 식별 |
| 키 분실·상관 추적 | **회전·복구·쌍별 DID 적용** | 제어권 상실·활동 연결 완화 |
...
- **방법거버넌스·키용도·회전상태**로 DID 수용을 결정한다.
