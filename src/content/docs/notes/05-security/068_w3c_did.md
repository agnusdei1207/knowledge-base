...
- 배경/필요성: 이메일·소셜 ID는 발급 기업의 DNS와 중앙 DB에 매여 서비스가 멈추면 신원까지 함께 사라지고, 원장마다 식별자 구현이 달라 검증기를 원장 수만큼 따로 만드는 비용이 반복되므로, 식별자 구문과 DID 문서 형식을 표준으로 고정하고 원장별 차이는 Method 드라이버 뒤에 감추는 해소(Resolution) 계층을 애플리케이션과 분산 원장 사이에 둘 요구
