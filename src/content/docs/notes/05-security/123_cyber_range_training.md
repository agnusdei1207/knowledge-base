...
1. **격리 환경•훈련 시나리오 구성**: IaC 배포•유출입 차단•상황 순서를 설계한다.
2. **공격 행위•상황 주입 실행**: 위협 행위와 업무 사건을 순서대로 실행한다.
3. **탐지•대응•복구 수행**: 판단•도구 조치•복구 시간을 기록한다.
4. **성과 지표•역량 공백 평가**: 목표 대비 성과를 확인하고 **AAR** 개선 과제를 도출한다.
