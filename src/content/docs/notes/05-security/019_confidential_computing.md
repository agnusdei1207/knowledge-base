...
- 배경/필요성: 퍼블릭 클라우드 및 멀티테넌트 환경에서 기존 보안 아키텍처는 저장 데이터(Data-at-Rest)와 전송 데이터(Data-in-Transit)만 암호화할 뿐, CPU 및 메모리에서 애플리케이션이 실행되는 동안에는 평문으로 복호화되어 호스트 OS 루트 관리자, 하이퍼바이저 탈취 공격자, 클라우드 서비스 제공자(CSP) 내부자 및 물리적 메모리 도청(Cold Boot/Bus Snooping)에 무방비로 노출되는 사용 중(Data-in-Use) 보안 공백을 가짐에 따라, CPU/GPU 칩셋 레벨의 하드웨어 기반 신뢰 실행 환경(TEE: Intel TDX/SGX, AMD SEV-SNP, NVIDIA H100)과 원격 증명(Remote Attestation: RFC 9334 RATS) 기술을 도입하여 **호스트 인프라를 신뢰하지 않는 제로 트러스트 기밀 연산(Confidential Computing), 메모리 실시간 암호화 격리 및 클라우드 상의 민감 데이터/AI 모델 완벽 보호**를 달성할 필요
...
- 클라우드 인프라 운영자조차 실행 중인 메모리 데이터를 열람하거나 변조할 수 없도록 물리적으로 차단하여 공공·금융·의료 핵심 워크로드의 안전한 클라우드 전환을 가능케 한 **차세대 클라우드 네이티브 보안 및 인프라 신뢰성의 절대적 핵심 패러다임(Confidential Computing Consortium 표준)**으로 안착하였으며, 초거대 LLM 추론/학습을 위한 기밀 AI 가속기(Confidential GPU)로 진화하는 가운데, 실무 기밀 컴퓨팅 인프라 구축 시에는 **보안성과 개발 편의성에 따른 격리 모델(최소 TCB 애플리케이션 Enclave vs 무수정 이전 기밀 VM(CVM))의 전략적 선택, 위조된 TEE 실행을 원천 차단하기 위한 IETF RFC 9334 RATS 표준 원격 증명 및 키 브로커(KBS) 연동, CPU와 외장 GPU 간 마더보드 버스 스누핑을 차단하는 PCIe 5.0 IDE(Integrity and Data Encryption) 링크 암호화 강제**를 결합하여 완벽한 하드웨어 신뢰성을 완성
