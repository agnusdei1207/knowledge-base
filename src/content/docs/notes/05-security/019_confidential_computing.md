...
- 정의/개념: TEE 격리•원격 증명으로 **사용 중 데이터를 보호하는 기술**이다.
- 배경/필요성: 기존 암호만으로는 연산 중 복호화된 데이터가 **특권 소프트웨어에 노출될 위험**이 발생한다.
...
- 메모리•CPU 평문 구간의 **하드웨어 격리**가 핵심이다.
- 증명 기반 **워크로드 신원•상태 확인**이 핵심이다.
- 증명 통과 워크로드에만 **키 제공**이 핵심이다.
- TEE 격리 밖 **부채널 별도 통제**가 핵심이다.
...
- 작은 민감 코드는 **엔클레이브**, 기존 앱 이관은 **기밀 VM**을 선택한다.
