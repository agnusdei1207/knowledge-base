...
- 정의/개념: 하드웨어 기반 신뢰 실행 환경(TEE)과 원격 증명(Remote Attestation)을 통해 데이터 처리 및 사용 중(Data in Use) 기밀성과 무결성을 보장하는 **기밀 컴퓨팅(Confidential Computing)** 기술
- 배경/필요성: 기존 저장 중(At-Rest)·전송 중(In-Transit) 암호화만으로는 클라우드 관리자 privilege 또는 하이퍼바이저 권한 침해 시 RAM 내 사용 중 평문 데이터 노출 한계 발생
...
- IETF RFC 9334/9711 준수, TEE 아키텍처 선택, 원격 증명 연동 및 키 브로커 차단을 통한 사용 중 데이터 보안 체계 구축 필수