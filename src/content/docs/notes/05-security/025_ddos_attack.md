...
- **정의**: 분산 요청으로 서비스 자원을 고갈시킴
- **배경/필요성**: 자원 고갈 중 정상 서비스 유지
...
- 회선·연결 상태·응용 중 병목을 식별한다
- 반사 증폭은 위조 IP로 응답을 집중시킨다
- L7 공격은 고비용 서비스 기능을 반복한다
- 완화 중에도 정상 요청을 보존해야 한다
...
| 설계 요소 | 설명 |
|:---|:---|
| 봇넷·반사원 | 분산 공격 트래픽을 생성함 |
| 상류 스크러빙 | 회선 도달 전 공격을 걸러냄 |
| CDN·애니캐스트 | 트래픽을 여러 거점에 분산함 |
| WAF·연결 보호 | 웹 요청과 연결 상태를 선별함 |
| 원본 서비스 | 정상 요청을 최종 처리함 |
...
| 주요 위험 | 상류 회선 포화 | 상태표 고갈 | CPU·DB 병목 |
...
- 평시에 스크러빙 전환 절차를 검증함
- SYN 쿠키와 SLI로 연결 보호를 확인함

### 쉽게 이해하기 (학습용)

- 공격 중 처음 전환하면 경로 오류가 피해를 더 키울 수 있다
- 연결 방어 뒤에도 정상 요청 성공률이 회복됐는지 확인한다

## Ⅶ. 결론

- 병목별 상류 흡수와 요청 선별을 결합함
