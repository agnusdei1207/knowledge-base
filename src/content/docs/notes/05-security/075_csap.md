...
date: "2026-08-05T11:12:00+09:00"
tags:
  - "notes-security"
weight: 75
extra:
...
- **CSAP(Cloud Security Assurance Program)** 는 클라우드 서비스의 보안 조치가 법정 기준에 적합한지를 평가•인증하는 제도다.
- **공공 보안인증**: 공공기관이 도입할 클라우드 서비스의 법정 보안 수준과 인증 범위를 확인하는 제도적 통제이다.

</details>

- 정의/개념: **CSAP** 는 공공기관이 이용할 클라우드 서비스의 법정 보안기준 적합성을 독립 평가해 인증하는 **공공 클라우드 보안인증 제도**
...
- **SaaS(Software as a Service)** 는 공급자가 운영하는 완성된 애플리케이션을 제공하는 클라우드 서비스다.
- **DaaS(Desktop as a Service)** 는 원격 가상 데스크톱 환경을 제공하는 클라우드 서비스다.
...
sequenceDiagram
    ...
    E->>E: 1. 서면•현장•취약점 평가
    E-->>A: 평가결과•부적합 사항
    A->>A: 2. 부적합 시정•보완증적 생성
    A->>E: 보완증적
    E->>E: 3. 보완 이행•최종 결과 검증
    E->>C: 평가결과•보완증적
    C->>C: 4. 인증기준 적합성 심의
    C->>I: 심의 결과
    I->>I: 5. 유형•등급•범위 인증 결정
    I-->>A: 인증결과
    loop 인증 유지기간
        I->>E: 사후•갱신평가 의뢰
        E-->>I: 운영 준수•연장 적합성 결과
