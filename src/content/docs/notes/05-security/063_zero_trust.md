...
date: "2026-08-05T17:30:13+09:00"
tags:
  - "notes-security"
weight: 63
extra:
...
- **제로 트러스트 아키텍처(Zero Trust Architecture, ZTA)**: 네트워크 위치를 신뢰하지 않고 자원별 접근을 검증하는 아키텍처
- **묵시적 신뢰** 는 내부 위치나 기존 세션만으로 후속 접근을 자동 허용하는 신뢰 방식이다.
- **측면 이동** 은 침해한 주체가 내부의 다른 자원으로 공격 범위를 넓히는 행위다.

</details>

- 정의/개념: 주체•단말•맥락을 자원별로 검증해 **최소 권한**을 집행하는 **보안 아키텍처**
...
- **정책 엔진(Policy Engine, PE)**: 신호와 정책을 평가해 접근 허용•거부를 결정하는 구성요소
- **정책 관리자(Policy Administrator, PA)**: 결정에 따라 세션과 자격 증명을 생성•종료하는 구성요소
- **정책 집행점(Policy Enforcement Point, PEP)**: 주체와 보호 자원 사이에서 접근 결정을 집행하는 지점
