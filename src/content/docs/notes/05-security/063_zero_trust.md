...
date: "2026-07-25T22:03:00+09:00"
tags:
  - "notes-security"
weight: 63
extra:
...
- **정의**: 위치 신뢰를 자원별 검증으로 대체
- **배경/필요성**: 권한 확장과 측면 이동을 제한
- **전제**: 자산 가시성과 일관된 정책 집행
...
- 보호 대상을 망 구역보다 자원·데이터로 정의한다.
- 인증 성공을 장기·광범위한 신뢰로 확장하지 않는다.
- 요청마다 최소 범위와 짧은 세션을 부여한다.
- 세션 중 위험 변화도 접근 결정에 반영한다.
- 우회 경로는 묵시적 신뢰와 측면 이동을 되살린다.
...
- **사례 1**: 중요 앱을 사용자·기기 조건에 결합
- **사례 2**: 직접 자원 주소를 집행점 뒤로 이동
- **사례 3**: 신호 장애 시 제한 업무 모드 적용
- **사례 4**: 세션 위험 상승 시 재인증·차단
