...
- 정의/개념: **패치 관리•가상 패치** 는 공급자 수정본을 평가•시험•배포•검증하고 즉시 적용하기 어려운 기간에는 외부 통제로 악용 경로를 임시 차단하는 취약점 변경 통제 체계
...
sequenceDiagram
    ...
    participant 변경승인자
    participant 검증담당
    자산관리->>자산관리: 1. 영향 자산•악용 경로 식별
    자산관리->>보안담당: 영향 범위
    보안담당->>보안담당: 2. 노출•변경 위험 평가
    보안담당->>변경승인자: 임시 통제•배포 계획 승인 요청
    변경승인자-->>보안담당: 승인•거부 결정
    보안담당->>보안담당: 3. 가상 패치•우회책 적용
    보안담당->>검증담당: 정식 패치 후보
    검증담당->>검증담당: 4. 호환성•롤백 시험
    검증담당-->>보안담당: 시험 결과
    loop 시범•단계 배포
        보안담당->>보안담당: 5. 정식 패치 배포•효과 검증
    end
    보안담당-->>자산관리: 예외•임시 통제 회수 결과
```

**동작 원리**

1. **영향 자산•악용 경로 식별**: 버전•노출•실제 악용 여부 확인
2. **노출•변경 위험 평가**: 심각도•서비스 중단•호환성 분석
3. **가상 패치•우회책 적용**: 정식 패치 전 공격 경로 임시 차단
4. **호환성•롤백 시험**: 대표 환경에서 장애와 복구 절차 검증
5. **정식 패치 배포•효과 검증**: 단계 확대 후 버전•경로 폐쇄 확인
