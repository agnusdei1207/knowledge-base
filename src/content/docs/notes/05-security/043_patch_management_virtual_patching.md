...
- **패치 관리**는 취약점 수정본을 평가•시험•배포하고 적용 결과를 재검증하는 운영 체계다.
- **가상 패치**는 정식 수정본을 즉시 적용하기 어려울 때 외부 통제로 악용 경로를 임시 차단하는 보상 통제다.

</details>

- 정의/개념: 수정본과 임시 차단을 관리하는 **취약점 변경 통제**이다.
- 배경/필요성: 즉시 패치는 **업무 중단•호환 위험** 동반이 발생한다.
...
- 정식 패치의 **결함 원인 제거**가 핵심이다.
- 가상 패치의 **공격 경로 임시 차단**이 핵심이다.
- 단계 배포•롤백•회수의 **변경 통제**가 핵심이다.
...
- **악용 가능성**은 취약점이 실제 공격 조건에서 성공적으로 이용될 가능성이다.
...
- **효과 검증**은 설치 버전과 공격 경로 차단 여부, 잔존 침해를 함께 확인한다.
...
3. **가상 패치•우회책 적용**: 정식 패치 전 공격 경로 임시를 차단한다.
4. **호환성•롤백 시험**: 대표 환경에서 장애와 복구 절차를 검증한다.
5. **정식 패치 배포•효과 검증**: 단계 확대 후 버전•경로 폐쇄를 확인한다.
...
- 즉시 배포 가능하면 **정식 패치**, 변경 위험이 크면 **가상 패치 후 단계 배포**를 선택한다.
