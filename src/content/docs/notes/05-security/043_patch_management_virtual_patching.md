...
- 정의: 수정본의 **평가·배포·검증·회수 통제**
- 기존 한계: 즉시 패치의 **업무 중단·호환 위험**
...
sequenceDiagram
    ...
    participant 배포도구
    participant 검증담당
    자산관리->>보안담당: 1. 영향 자산·악용 경로 식별
    보안담당->>변경승인자: 2. 노출·변경 위험 평가
    변경승인자->>배포도구: 3. 임시 통제·단계 배포
    배포도구->>검증담당: 4. 설치·차단 효과 확인
    검증담당->>보안담당: 5. 예외 추적·임시 통제 회수
```

| 절차 | 설명 |
|:---|:---|
| 영향 자산·악용 경로 식별 | 버전·노출·악용 여부 확인 |
| 노출·변경 위험 평가 | 심각도·중단·호환성 분석 |
| 임시 통제·단계 배포 | 가상 패치 후 시범·확대 |
| 설치·차단 효과 확인 | 버전·공격 경로를 재검증 |
| 예외 추적·임시 통제 회수 | 미적용 사유·만료를 관리 |
...
|:---|:---|:---|:---|
| 적용 기준 | 시험·배포 가능 | 기능 제한 수용 가능 | 즉시 패치 곤란·경로 식별 |
| 핵심 특징 | **취약 원인 제거** | **취약 기능 비활성화** | 악성 경로 임시 차단 |
| 한계 | 호환성·재부팅·롤백 실패 | 업무 저하·설정 회귀 | 우회·오탐·장기화 |
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 위험 기반 패치 계획 | **NIST SP 800-40 Rev. 4 적용** | 전사 책임·우선순위 정렬 |
| 호환성·서비스 중단 | **시범 배포·롤백 사전 검증** | 장애 범위·복구 시간 축소 |
| 가상 패치 장기화 | **만료일·정식 패치·회수 추적** | 우회 노출·기술 부채 방지 |
...
- **악용가능성·업무영향·변경위험**으로 패치 방식을 결정한다.
