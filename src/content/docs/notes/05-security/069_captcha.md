...
- **컴퓨터•사람 구별 완전 자동화 공개 튜링 테스트(Completely Automated Public Turing test to tell Computers and Humans Apart, CAPTCHA)**: 사람과 자동화 요청을 구분하는 보조를 통제한다.
- **reCAPTCHA**는 위험 신호와 사용자 도전을 결합한 CAPTCHA 서비스다.
- **봇**: 정해진 작업을 대량으로 자동 수행하는 프로그램이다.

</details>

- 정의/개념: 사람 요청과 자동화 요청을 구별하는 **봇 남용 방지** **보조 통제**이다.
- 배경/필요성: 대량 가입•스팸•로그인의 **자동 남용**이 발생한다.
...
- **위험 기반 선택 적용**: 공개 진입점의 고위험 요청만 추가를 검증한다.
- **서버 검증**: 토큰 목적•만료•재사용 여부를 확인한다.
- **다층 통제**: 호출률•MFA•접근성 대체 경로 결합한다.
...
- **재전송 검증**: 서버가 토큰의 목적•만료•사용 이력을 확인해 이전 정상 결과를 다시 제출하는 우회를 막는 처리한다.
...
- **1. 계정•기기•행위 맥락 수집**: 자동화 판단 신호를 구성한다.
- **2. 자동화 위험•도전 수준 산정**: 봇 가능성과 남용 규모를 평가한다.
- **3. 적응형 도전 문제 생성**: 고위험 업무와 검증 과제 결속이다.
- **4. 응답 유효성•봇 판정**: 풀이 결과와 행위 신호를 함께 검증한다.
- **5. 목적•만료•재사용 검증**: 토큰을 현재 업무 요청에만 허용이다.
#### 한줄 요약
...
- 저위험은 **호출률 제한**, 중위험은 **적응형 CAPTCHA**, 계정•결제 고위험은 **MFA**가 핵심이다.
