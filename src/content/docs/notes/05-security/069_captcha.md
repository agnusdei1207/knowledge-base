...
date: "2026-07-31T02:02:02+09:00"
tags:
  - "notes-security"
weight: 69
extra:
...
- **목적 결속(Purpose Binding)**: 검증 토큰을 특정 업무 요청에만 사용하도록 묶는 통제이다.
- **대체 경로**: 시각·인지·운동 제약이 있어도 검증을 완료하게 하는 다른 수단이다.
- **대행 서비스**: 사람이 CAPTCHA 문제를 대신 풀어 자동화 공격을 돕는 서비스이다.
- **모델 편향**: 위험 점수 모델이 특정 사용자 환경을 체계적으로 불리하게 판정하는 오류이다.
...
| **토큰 재사용·대량 요청** | **목적 결속·호출률·MFA 적용** | 단일 보조 통제의 **우회 제한** |
