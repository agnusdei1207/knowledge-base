...
- 정의/개념: **UEBA**는 사용자•계정•기기의 기준선 편차와 문맥으로 **위험 행동을 선별**하는 기법이다.
- 배경/필요성: 정상 계정 악용은 **시그니처 탐지로 식별하기 어렵다**.
...
- 개인•동료군 기반 **행동 기준선**이 핵심이다.
- 편차•업무 문맥 기반 **위험 점수**가 핵심이다.
...
3. **문맥 위험도 산정**: 시간•위치•자산 중요도를 편차와 결합한다.
4. **원본 증거•업무 문맥 검증**: 고위험 행동의 정탐•오탐을 판정한다.
5. **기준선•임계값 보정**: 오염 제거와 판정 결과를 모델에 반영한다.
...
- 알려진 패턴은 **고정 규칙**, 정상 계정 편차는 **UEBA**, 기준선 오염 시 분석가 확인 강화가 핵심이다.
