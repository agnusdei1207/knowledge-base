...
date: "2026-07-25T21:38:00+09:00"
tags:
  - "notes-security"
weight: 38
extra:
...
- **정의/개념**: 행위를 기준선과 비교해 이상을 선별함
- **배경/필요성**: 정상 계정 악용은 서명 탐지가 어려움
...
- 신원·계정·장비를 연결해 행위 시간선을 만든다.
- 개인 이력이 부족하면 동료군 기준선을 적용한다.
- 업무 문맥으로 정상 변화와 이상 행동을 구분한다.
- 기준선 오염과 업무 변화가 오탐을 늘린다.
...
| 비교축 | UEBA | 고정 규칙 |
|:---|:---|:---|
| 핵심 특징 | 개인·동료군·문맥 편차 분석 | 알려진 패턴과 직접 대조 |
| 적용 기준 | 정상 계정의 이상 행위 탐지 | 명확한 공격 패턴 탐지 |
| 주요 위험 | 기준선 오염과 동료군 편향 | 신규·변형 공격 누락 |
...
1. 내부자 탐지는 고위험 파일 접근부터 조사
2. 계정 탈취 탐지는 접속 위치·시간 편차 분석
