...
sequenceDiagram
    ...
    participant 분석가
    participant 모델관리자
    행위원천->>신원해석기: 1. 행위·엔티티 품질 검증
    신원해석기->>UEBA: 2. 개인·동료군 기준선 구성
    UEBA->>분석가: 3. 편차·문맥 위험도 산출
    분석가->>모델관리자: 4. 정탐·오탐 조사 판정
    모델관리자->>UEBA: 5. 기준선·임계값 보정
```

**동작 원리**

1. **행위·엔티티 품질 검증**: 신원 연결·결측·시각 확인
2. **개인·동료군 기준선 구성**: 정상 행동 범위 모델링
3. **편차·문맥 위험도 산출**: 자산·직무·시간 결합
4. **정탐·오탐 조사 판정**: 고위험 엔티티 근거 검증
5. **기준선·임계값 보정**: 조사 결과로 모델 개선
