...
[소프트웨어 공급망 보안 체계]
├─ 소스 및 의존성 수입 계층
│  ├─ 개발자 GPG·SSH 서명 커밋 검증
│  └─ 의존성 해시 고정 (Lockfile Pinning)
├─ 격리 빌드 파이프라인 (SLSA L3)
│  ├─ 외부 인터넷 차단 샌드박스 빌드
│  ├─ 부품 명세서 자동 생성 (SBOM: CycloneDX)
│  ├─ 실질 악용성 분석 (VEX: OpenVEX)
│  └─ OIDC 단명 키 전자서명 (Sigstore/Cosign)
├─ 불변 아티팩트 레지스트리
│  ├─ OCI 바이너리 아티팩트 저장
│  └─ 출처 증명 메타데이터 보관 (in-toto)
└─ 배포 관문 집행 계층 (Gatekeeper)
   ├─ Sigstore 공개키 서명 무결성 검증
   ├─ VEX 상태 확인 (미조치 취약점 부재)
   └─ 프로덕션 클러스터 Pod 기동 인가
```
...
| 구성요소 | 책임 |
|:---|:---|
| 의존성 락커 (Dependency Pinning) | 락파일 및 체크섬 해시 검증을 통해 패키지 변조 및 종속성 혼란 공격 차단 |
| 격리 빌드 엔진 (Hermetic Builder) | 외부 통신이 차단된 샌드박스 환경에서 빌드를 수행하여 빌드 스크립트 오염 방어 |
| SBOM/VEX 생성기 | 바이너리 분석을 통해 부품 목록(CycloneDX)과 실제 악용성(OpenVEX) 메타데이터 생성 |
| Sigstore / Cosign | OIDC 신원 기반으로 키 관리 부담 없이 산출물에 단기 비대칭 전자서명 주입 |
| Admission Controller | 서명 무결성, SBOM 존재, VEX 승인 정책을 만족하는 아티팩트만 배포 인가 |
