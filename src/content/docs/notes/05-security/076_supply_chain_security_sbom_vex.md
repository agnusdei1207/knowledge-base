...
- **소프트웨어 공급망**: 소스•의존성•빌드•저장소•배포를 거쳐 소프트웨어가 만들어지는 전체 연결한다.
- **전이 의존성**: 개발자가 직접 선택한 구성요소가 다시 포함한 간접 의존성으로 오염 범위 파악을 어렵게 한다.

</details>

- 정의/개념: 소스부터 배포까지 **출처•무결성**을 검증하는 **위험관리 체계**이다.
- 배경/필요성: 전이 의존성의 **오염 범위 식별하기 어렵다**.
...
- **구성•영향 가시성**: SBOM과 VEX로 오염 범위 판별이다.
- **산출물 무결성**: 격리 빌드와 서명으로 변조 차단한다.
- **배포 신뢰성**: 출처와 승격 정책으로 신뢰를 검증한다.
...
1. **출처•버전•해시 검증**: 승인 소스와 고정 의존성을 확인한다.
2. **격리 빌드•산출물 서명**: 단명 자격으로 결과와 출처를 생성한다.
3. **불변 저장•증적 결속**: 산출물과 SBOM•VEX•출처를 연결한다.
4. **서명•영향•승격 정책 검증**: 허용된 산출물만 운영을 승격한다.
5. **배포•운영 구성 연계**: 신규 취약점의 제품 영향 추적 기반을 생성한다.
...
- **안전한 소프트웨어 개발 프레임워크(Secure Software Development Framework, SSDF) 1.1**: 안전한 개발 관행을 SDLC에 통합하는 지침이다.
- **소프트웨어 산출물 공급망 수준(Supply-chain Levels for Software Artifacts, SLSA) 1.2**: 소스•빌드 출처와 무결성 보증 수준이다.
...
- 서명•출처가 유효하고 VEX 영향이 허용되면 **승격**, 하나라도 실패하면 **배포 차단**이 핵심이다.
