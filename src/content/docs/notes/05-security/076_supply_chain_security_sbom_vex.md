...
- 배경/필요성: SolarWinds, Codecov, Log4Shell 등 공급망 침투를 통한 대규모 백도어 주입 공격이 급증함에 따라, 미국 행정명령(EO 14028) 및 글로벌 규제 기관에서 SBOM 제출과 빌드 파이프라인 무결성 입증을 의무화
...
1. **빌드 파이프라인 중간자 변조 원천 봉쇄**: 외부 네트워크가 단절된 환경에서만 바이너리 빌드 허용
2. **Keyless 서명 혁신**: 유출 위험이 큰 정적 개인키 대신 단명 OIDC 인증서로 전자서명 처리
3. **투명성 원장(Transparency Log) 증적**: Rekor 불변 로그에 서명 사실을 기록하여 사후 부인 방지
4. **허위 취약점 패닉 제거**: VEX를 통해 실제 코드 경로상 실행되지 않는 CVE에 대한 불필요한 배포 중단 방지
5. **무인가 아티팩트 배포 차단**: 서명되지 않았거나 출처가 불분명한 컨테이너 이미지는 클러스터 진입 원천 거절

#### 한줄 요약
- 소스 검증, 격리 빌드/증적 생성, Sigstore 서명, 불변 레지스트리 저장, 배포 게이트 무결성 검증 순으로 동작한다.
...
- 오픈소스 활용 확산과 지능형 공급망 공격에 대응하는 **소프트웨어 공급망 보안 아키텍처**는 제로 트러스트 개발 생태계의 핵심 방어선이며, 실무 구현 시 **NIST SSDF 1.1 기반 전주기 보안 프로세스 정립**, **SPDX/CycloneDX 기반 SBOM 자동화**, **OpenVEX 기반의 실질적 취약점 악용성 판정**, **SLSA Level 3 격리 빌드 및 Sigstore Keyless 전자서명 배포 게이트**를 통합 구축하여 무결점 소프트웨어 공급망 거버넌스를 완성
