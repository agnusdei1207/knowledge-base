...
- 정의/개념: 소프트웨어 공급망 보안은 소스 작성부터 최종 운영 배포까지 전체 생명주기 동안 오픈소스 취약점, 악성 코드 삽입, 파이프라인 변조를 방지하는 무결성 보증 기술이다.
- 배경/필요성: SolarWinds, Log4j 사태와 같이 오픈소스 라이브러리와 빌드 시스템에 주입된 악성 코드가 전이 의존성을 통해 사슬처럼 전파되어 치명적 피해를 일으키기 때문이다.
...
1. **출처•버전•해시 검증**: 외부 라이브러리 및 소스의 출처와 해시 무결성을 1차 검증한다.
2. **격리 빌드•산출물 서명**: 샌드박스 빌드 파이프라인에서 바이너리를 생성하고 전자서명을 부여한다.
3. **불변 저장•증적 결속**: 서명된 바이너리와 SBOM, VEX, SLSA 출처 증명을 불변 저장소에 결합 저장한다.
4. **서명•영향•승격 정책 검증**: 배포 게이트가 서명 및 VEX 기반 취약점 악용 가능성을 대조한다.
5. **배포•운영 구성 연계**: 최종 배포 후 런타임 자산과 연계하여 신규 취약점 발생 시 영향을 추적한다.

#### 한줄 요약

- 의존성 해시 검증, 격리 빌드 및 서명, 증적 결속 저장, 배포 게이트 검증 및 런타임 영항 환류 체계로 운영한다.
...
|:---|:---|:---|:---|
| **SBOM** | 소프트웨어 포함 오픈소스 부품/버전 목록 제공 | SPDX, CycloneDX | 포함된 모든 오픈소스 및 전이 의존성 100% 가시화 |
...
- NIST SSDF/SLSA 준수, SBOM/VEX 결합, Sigstore 기반 전자서명 및 배포 게이트 연동 소프트웨어 공급망 보안 체계 구축 필수.