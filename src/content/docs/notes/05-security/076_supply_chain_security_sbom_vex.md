...
1. **출처•버전•해시 검증**: 승인 소스와 고정 의존성 확인
2. **격리 빌드•산출물 서명**: 단명 자격으로 결과와 출처 생성
3. **불변 저장•증적 결속**: 산출물과 SBOM•VEX•출처 연결
4. **서명•영향•승격 정책 검증**: 허용된 산출물만 운영 승격
5. **배포•운영 구성 연계**: 신규 취약점의 제품 영향 추적 기반 생성
