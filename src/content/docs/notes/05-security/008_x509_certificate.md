...
2. **신원·키 통제권 검증**: 발급 대상과 공개키 소유 확인
3. **X.509 인증서 발급**: CA가 TBS에 서명해 반환
4. **인증서 체인·소유 증명**: 체인과 대응 개인키 보유 제시
5. **경로·이름·용도 검증**: 신뢰·기간·SAN·EKU 판정
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 인증서 프레임워크 | **ITU-T X.509 준수** | 필드 의미 일관화 |
| 인터넷 경로 검증 | **IETF RFC 5280 적용** | 이름·용도 오용 차단 |
| 개인키·중간 인증서 | **HSM·체인 자동 배포** | 키 유출·연결 실패 억제 |
...
- **신뢰 경로·SAN·EKU·폐지성**으로 인증서를 승인한다.
