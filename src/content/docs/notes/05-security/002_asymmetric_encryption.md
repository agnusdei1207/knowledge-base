...
- **인증 암호화(Authenticated Encryption with Associated Data, AEAD)**: 자료의 기밀성과 무결성을 함께 보장하는 대칭키 암호 방식이다.
...
| 절차 | 설명 |
|:---|:---|
| 공개·개인키 쌍 생성 | 공개키 배포·개인키 격리 |
| 공개키·인증서 게시 | 키와 소유자 신원 정보 제공 |
| 인증서·폐기 상태 검증 | 서명·유효기간·폐기 여부 판정 |
| 검증 공개키 전달 | 확인된 키만 설정 모듈에 제공 |
| 공유 비밀 캡슐화 | 공개키 캡슐화·개인키 복구 |
