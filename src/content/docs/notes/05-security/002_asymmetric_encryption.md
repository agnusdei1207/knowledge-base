...
- **비대칭키 공개키 암호(Asymmetric Public-Key Cryptography / RSA / ECC)**: 수학적 난제(소인수분해, 이산로그, 타원곡선)에 기반하여 공개키로 암호화하고 개인키로만 복호화하는 비대칭 암호 방식.
</details>
...
- **키 분리 아키텍처**: 개방형 **공개키** 배포와 폐쇄형 **개인키** 격리 관리의 이원화 구조
- **다목적 암호 연산**: 단일 키 쌍 기반의 암호화(키 설정) 및 부인방지(전자서명) 메커니즘 동시 지원
- **신뢰 체인**: **인증서** 기반의 공개키 소유자 무결성 및 신원 검증 보장
