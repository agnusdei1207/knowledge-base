...
- 배경/필요성: 대칭키 암호화 방식은 통신에 참여하는 $n$명의 사용자 간에 $n(n-1)/2 \approx O(n^2)$개의 비밀키를 사전에 안전하게 분배해야 하는 심각한 키 배송 문제(Key Distribution Problem)와 키 관리 오버헤드를 발생시키며, 송수신자가 동일한 키를 공유하므로 제3자에 대한 부인방지(Non-repudiation) 및 출처 인증을 제공할 수 없는 한계에 직면함에 따라, 수학적 일방향 트랩도어 함수(Trapdoor One-way Function)에 기반하여 누구나 볼 수 있는 공개키(Public Key)와 소유자만 보관하는 개인키(Private Key) 쌍을 사용하는 비대칭(공개키) 암호화 기술을 도입하여 **사전 비밀 공유 없는 개방형 네트워크 상의 안전한 세션키 교환(KEM/Diffie-Hellman), 전자서명 기반의 거래 무결성/부인방지 보장 및 글로벌 PKI 신뢰 생태계 구축**을 달성할 필요
...
- 인터넷 개방망에서 물리적 대면 없이 신뢰를 수립하고 전자서명과 신원 인증을 가능케 한 **현대 인터넷 정보보안 및 디지털 서명 인프라(PKI/TLS/SSH)의 가장 근본적인 핵심 기둥**으로 자리 잡았으며, 양자 컴퓨터의 쇼어(Shor) 알고리즘 위협에 대응하는 격자 기반 양자내성암호(PQC: ML-KEM/ML-DSA)로의 세대교체가 진행 중인 가운데, 실무 비대칭 암호 시스템 운영 시에는 **서버 개인키 유출 시에도 과거 트래픽의 사후 해독을 방지하는 임시 키 교환(ECDHE) 기반 순방향 비밀성(PFS: Perfect Forward Secrecy) 필수 보장, 중간자 공격(MITM)을 원천 차단하는 PKI 인증서 체인 및 OCSP Stapling 실시간 검증, 장기 수집 후 해독(SNDL) 공격을 방어하기 위한 전통 ECC와 ML-KEM 결합 하이브리드 키 교환(X25519Kyber768) 선제 적용**을 결합하여 완벽한 암호학적 통신 영속성을 완성
