...
- 정의/개념: 공개키와 개인키 쌍을 활용하여 통신 당사자 간 **키 설정(Key Establishment)** 및 **전자서명(Digital Signature)**을 수행하는 **비대칭키 암호(Asymmetric Key Cryptography)** 기법
- 배경/필요성: 대칭키 암호 체계의 심각한 키 분배(Key Distribution) 복잡도 한계 극복 및 다자간 안전한 통신 채널 확보 요구 증대

#### 한줄 요약

- 단방향 함수 기반의 키 쌍 구조로 키 분배 문제를 해결하나, 공개키 위조 공격에 취약하므로 PKI 기반 신뢰 체계 연동 필수
## Ⅱ. 특징
...
- 배포된 공개키의 무결성을 담보하기 위해 공인된 인증기관(CA)의 서명과 인증서 유효성 검증 절차 필수 적용
## Ⅲ. 구조 및 구성요소
...
- 공개키는 인증서에 바인딩하여 배포하고 개인키는 HSM 등 안전한 모듈 내부에 원천 격리하는 보안 아키텍처 구현
## Ⅳ. 흐름도
...
- 중간자 공격(MITM) 차단을 위해 공개키의 소유자 신원 및 상태 검증이 완료된 후 제한적으로 공유 비밀 설정 단계 진입
## Ⅴ. 종류 및 비교
...
- 레거시 호환성(RSA), 순방향 비밀성(ECDHE), 양자컴퓨팅 내성(ML-KEM) 등 통신 환경의 보안 요구사항별 최적화된 알고리즘 채택 필수
## Ⅵ. 실무 고려사항 및 대책
...
- 인증서 기반 공개키 검증, HSM을 활용한 개인키 격리, 양자 위협 대비 등 엔드투엔드(E2E) 관점의 보안 통제 체계 적용
## Ⅶ. 결론

- 인증서 검증을 통한 공개키 신뢰 확보 및 개인키 격리, 요구사항별 **암호 민첩성(Crypto Agility)** 확보 적용

#### 한줄 요약

- 단순 암호 도입이 아닌 PKI 생태계 연동, 키 수명주기 관리, 양자 내성 전환 전략을 포함하는 종합적 비대칭키 운용 아키텍처 수립 필수
