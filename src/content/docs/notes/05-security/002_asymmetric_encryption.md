...
- **정의/개념**: 공개키·개인키 쌍으로 키 설정·서명 수행
- **배경/필요성**: 사전 비밀 공유 없는 신원·세션키 설정
...
- 공개키는 배포하고 개인키는 격리한다
- 인증서 미검증은 공격자 키와 연결시킨다
- 대용량 데이터는 합의한 대칭키로 암호화한다
...
| 판단 기준 | RSA-OAEP | ECDHE | ML-KEM |
|:---|:---|:---|:---|
| 핵심 특징 | 공개키로 키 재료 암호화 | 양측 임시 키로 비밀 합의 | 공개키로 공유 비밀 캡슐화 |
| 적용 기준 | 기존 RSA 키 전송 호환 | 순방향 비밀성이 필요한 TLS | 양자 공격 대비 키 설정 |
| 주요 위험 | 장기 개인키 유출 시 과거 복호화 | 인증 없으면 중간자 공격 | 키·암호문 크기 증가 |
