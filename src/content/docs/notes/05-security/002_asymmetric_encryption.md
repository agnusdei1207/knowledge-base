...
- 정의/개념: **비대칭키 암호** 는 수학적으로 연관된 공개키•개인키 쌍을 서로 다른 역할로 사용하여 키 설정•암호화•전자서명을 수행하는 암호 방식
...
[키 생성기]---[공개키•인증서 저장소]
      |                    |
[개인키 보호 모듈]---[키 설정•서명 모듈]
```

선의 의미: 키 생성기는 공개키•인증서 저장소와 개인키 보호 모듈에 같은 키 쌍의 공개•비밀 영역을 결속하고, 두 영역은 키 설정•서명 모듈의 소유자 검증과 개인키 연산 근거가 된다.
