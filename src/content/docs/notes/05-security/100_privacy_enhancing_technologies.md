...
- **개인정보 보호 강화 기술(PETs)**: 데이터 수집•저장•분석•공유 과정에서 원본과 개인 참여 여부의 노출을 줄이는 기술군이다.
...
- **연합학습•TEE(Trusted Execution Environment)**: 원본 데이터를 중앙에 모으지 않고 업데이트를 학습하거나 신뢰 실행 환경인 격리 하드웨어 영역에서 코드와 데이터를 보호한다.
...
- **DP(Differential Privacy)•HE(Homomorphic Encryption)•MPC(Multi-Party Computation)**: 각각 출력의 개인 영향, 비신뢰 서버의 평문 접근, 참여 기관끼리의 원본 입력 노출을 제한한다.
