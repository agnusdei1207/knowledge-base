...
- **개인정보보호 강화기술(Privacy-Enhancing Technologies, PET)**: 펫으로 읽고 영문 머리글자를 딴 표기이며, 데이터 처리 단계별 개인정보 노출을 줄이는 기술군
...
| 개인정보보호 강화기술 | 차분 프라이버시 | 동형암호 | 다자간 계산 |
|:---|:---|:---|:---|
| 적용 기준 | 통계·학습 결과 공개 | 비신뢰 서버 연산 | 기관 간 공동 분석 |
| 핵심 특징 | 출력에 통제된 잡음 추가 | 암호문 상태 연산 | 입력 비밀 분산·공동 계산 |
...
1. 통계 공개의 **누적 프라이버시 예산 관리**

### 쉽게 이해하기 (학습용)

- 반복 통계 질의마다 소비한 프라이버시 예산을 누적 관리해 여러 결과를 결합해도 특정 개인의 참여 여부를 추론하기 어렵게 한다.

## Ⅶ. 결론

- 데이터 활용 과정의 개인정보 노출을 줄이기 위해 위협 모델·노출 지점·계산 기능·정확도·성능을 검토하여, 목적에 맞는 PET를 선택하고 잔여 위험을 관리해야 한다.
