...
- 배경/필요성: 발급 기관에 진위를 되묻는 검증 방식은 검증 1건마다 API 호출 비용을 되풀이하면서 그 호출 기록이 그대로 사용자 동선 추적 자료가 되므로, 발급 시점의 전자서명을 증명서 안에 넣어 검증자가 발급 기관과 통신하지 않고 확인하도록 하고 실시간 조회는 폐기 여부를 담은 비트맵 한 장으로 축소할 요구
