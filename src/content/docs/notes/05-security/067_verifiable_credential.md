...
- 정의/개념: 발급자의 주장과 **암호학적 증명**을 결합한 **디지털 자격**이다.
- 배경/필요성: 종이•PDF는 **자동 검증 불가•위변조 취약해진다.
...
- **역할 분리**: 발급자•보유자•검증자의 책임을 구분한다.
- **무결성•발급자 검증**: 암호 증명으로 변조와 서명자를 확인한다.
- **선택적 공개•최소 제시**: VP로 필요한 주장만을 제공한다.
...
- **1. 원천 자료•자격 주장 검증**: 발급 근거와 주체 자격을 확인한다.
- **2. 암호 증명•서명 VC 생성**: 주장•유효기간의 무결성 보호한다.
- **3. 동의•선택 공개 VP 생성**: 요청 목적에 필요한 속성만을 구성한다.
- **4. 서명•발급자•상태 검증**: 신뢰 기관과 정지•폐기 여부를 확인한다.
- **5. 목적 제한•업무 적합성 판정**: 요청 용도와 자격 조건 대조이다.
#### 한줄 요약
...
- 서명•발급자•상태•업무 적합성이 모두 맞으면 **VC 수용**, 하나라도 틀리면 **거부**가 핵심이다.
