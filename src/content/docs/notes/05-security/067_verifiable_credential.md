...
- 배경/필요성: 포토샵 위변조가 쉬운 종이/PDF 서류의 한계를 극복하고, 발급 기관에 대한 실시간 조회 없이도 제3자 검증자가 서명의 진위성과 유효성을 독립적으로 검증할 수 있는 신뢰 인프라 요구

#### 한줄 요약
- W3C JSON-LD 데이터 모델과 비대칭 전자서명을 통해 발급 기관 서버 호출 없이도 위변조를 독립 검증한다.
...
#### 한줄 요약
- 발급-검증 분리, JSON-LD 기계 판독성, Bitstring Status List 폐기 검증, BBS+ 선택적 공개를 제공한다.
...
#### 한줄 요약
- 발급자(Issuer), VC 데이터 구조, 보유자 지갑(Holder), VP(프레젠테이션), Bitstring Status List가 결합한다.
...
#### 한줄 요약
- VC 발행, 지갑 저장, 검증 요청, VP 조립 및 서명, 독립 무결성/폐기 검증 순으로 동작한다.
...
#### 한줄 요약
- W3C VC 2.0으로 스키마를 통일하고, Bitstring Status List로 부하를 없애며, 신뢰 등록부로 권한을 검증한다.
