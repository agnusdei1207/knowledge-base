...
- 정의: AI 자산의 **출처·반입·학습·배포 통제**
- 기존 한계: 서명·SBOM의 **오염·백도어 판정 불가**
...
sequenceDiagram
    ...
    Q->>V: 2. 안전 파싱·격리 적재
    V->>R: 3. 출처·행동 검증·승격
    R->>O: 4. AI-BOM·배포 계보 연결
    O-->>R: 5. 이상 자산 영향 회수
```

| 절차 | 설명 |
|:---|:---|
| 제공자·버전·해시 등록 | 자산 식별·책임 기록 |
| 안전 파싱·격리 적재 | 직렬화 코드 실행 제한 |
| 출처·행동 검증·승격 | 서명·취약점·백도어 평가 |
| AI-BOM·배포 계보 연결 | 학습·모델·서비스 관계 기록 |
| 이상 자산 영향 회수 | 모델 중단·교체·재학습 |

> 요약: 외부 AI 자산을 격리 검증하고 영향을 추적한다.
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| AI 안전개발 관행 | **NIST SP 800-218A 적용** | 모델 SDLC 공급망 통제 |
