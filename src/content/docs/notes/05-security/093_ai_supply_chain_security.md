...
- 배경/필요성: 코드 부품 목록만으로는 학습 데이터와 사전학습 가중치에서 상속된 결함을 사고 후 역추적하는 비용이 남으므로, 데이터·모델까지 계보에 포함한 **AI-BOM**과 반입 시점의 Safetensors 검사·샌드박스 평가를 승격 게이트 계층으로 둔 것
...
- 외부 가중치는 **Safetensors·샌드박스**, 상속은 **AI-BOM** 추적
