...
- **인공지능(Artificial Intelligence, AI)**: 데이터와 모델을 이용하여 판단하거나 콘텐츠를 생성하는 기술이다.
- **인공지능 자재명세서(AI Bill of Materials, AI-BOM)**: 모델·데이터·프레임워크·의존성의 구성과 출처 관계를 기록한 명세서이다.
- **소프트웨어 자재명세서(Software Bill of Materials, SBOM)**: 소프트웨어를 구성하는 패키지·버전·의존성·출처를 기록한 명세서이다.
