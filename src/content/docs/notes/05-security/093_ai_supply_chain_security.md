...
date: "2026-07-25T13:36:00+09:00"
tags:
  - "notes-security"
weight: 93
extra:
...
- **정의/개념**: AI 자산의 반입·학습·배포·회수를 통제한다
- **배경/필요성**: 정식 자산에 숨은 오염·백도어 피해를 막는다
...
- 자산·도구 의존 그래프는 오염 영향을 확산시킨다
- 모델 직렬화는 적재 시 임의 코드 실행을 만들 수 있다
- 서명·해시는 출처를 보여도 백도어를 보증하지 않는다
- 계보 추적은 취약 자산의 영향 범위와 회수를 줄인다
- 격리·행동 평가는 신뢰 출처의 숨은 위험도 찾는다
...
| 설계 요소 | 설명 |
|:---|:---|
| AI-BOM | 자산 버전과 출처·관계 기록 |
| 격리 반입소 | 외부 자산을 실행 환경과 분리 |
| 출처 검증기 | 해시·서명·라이선스 확인 |
| 행동 평가기 | 오염·취약점·백도어 분석 |
| 정책 엔진 | 승인 자산의 학습·배포·회수 결정 |
...
| 판단 기준 | 데이터셋 | 모델 아티팩트 | 소프트웨어 패키지 |
|:---|:---|:---|:---|
| 핵심 특징 | 학습 표본·라벨·분포 제공 | 가중치·설정·코드를 배포 단위로 묶음 | 실행 라이브러리·도구 의존성 제공 |
| 적용 기준 | 라벨·분포 평가 | 격리 적재/행동 검사 | 악성코드/재현 평가 |
| 주요 위험 | 오염·편향·라이선스 위반 | 백도어·직렬화 코드 실행·변조 | 알려진 취약점·빌드 의존성 공격 |
...
1. 모델 허브는 격리·백도어 평가 통과 자산만 승격한다
2. 파인튜닝 데이터셋의 출처·버전·분포를 고정한다

### 쉽게 이해하기 (학습용)

- 모델 허브 자산은 격리 적재와 백도어 평가를 통과한 뒤 승인 저장소로 옮김
- 파인튜닝 데이터셋은 출처·버전·분포를 고정해 결과 모델까지 계보를 연결함

## Ⅶ. 결론

- 출처·안전 적재·행동 평가로 영향 모델을 회수한다.
