...
date: "2026-08-05T12:21:00+09:00"
tags:
  - "notes-security"
weight: 109
extra:
...
```text
                         [위험 소유자]
                      /         |         \
             [위험 등록부] [위험 처리계획] [잔여위험•감시 기준]
                            \    |    /
                         [통제•계약 증적]
```

선의 의미: 위험 소유자가 위험 등록부, 처리계획, 잔여위험•감시 기준의 책임 경계를 통제하고 통제•계약 증적이 각 판단의 근거를 공통 지원하는 구조
...
sequenceDiagram
  ...
  T->>T: 3. 통제•계약•업무변경 실행
  T->>C: 조치 결과 전달
  C->>C: 4. 통제 효과•분담 증적 생성
  C->>A: 효과 증적 전달
  A->>A: 5. 잔여위험•재평가 조건 산정
  A->>O: 잔여위험 평가 전달
  O->>O: 승인된 감시 기준 등록
```
...
- **SoA(Statement of Applicability)**: 통제 선택•제외 근거를 기록한 적용성 보고서이다.
