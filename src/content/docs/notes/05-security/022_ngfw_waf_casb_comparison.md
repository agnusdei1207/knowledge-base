...
- 배경/필요성: 엔터프라이즈 환경이 온프레미스 경계 보안에서 웹/모바일 API 및 멀티 클라우드 SaaS로 확장됨에 따라, 단일 보안 솔루션으로 모든 계층의 위협을 방어하려는 접근은 네트워크 전역을 넓게 검사하는 NGFW가 웹 애플리케이션 로직 취약점(OWASP Top 10)과 비인가 SaaS를 통한 데이터 유출(Shadow IT)을 정밀하게 탐지하지 못하는 커버리지 공백을 초래함에 따라, 네트워크 경계(NGFW), 웹/API 인바운드 경로(WAF), 클라우드 SaaS 아웃바운드 경로(CASB)로 역할을 분담하는 다계층 심층 방어(Defense-in-Depth) 아키텍처를 도입하여 **각 보안 통제 지점의 전용 심층 분석(DPI / HTTP 파싱 / 클라우드 DLP), 사각지대 없는 전 구간 가시성 확보 및 SASE 기반 통합 제로 트러스트 거버넌스**를 달성할 필요
...
- 네트워크 인프라, 웹 애플리케이션, 클라우드 서비스를 아우르는 현대 엔터프라이즈 환경에서 보안 사각지대를 완전히 제거하는 **다계층 심층 방어(Defense-in-Depth) 및 제로 트러스트 아키텍처의 필수 3대 보안 통제 기둥**으로 확립되었으며, 단일 클라우드 엣지로 통합되는 SASE/SSE(Security Service Edge) 플랫폼으로 진화하는 가운데, 실무 다계층 보안 인프라 설계 시에는 **인프라 전역 L3~L7 접근 통제를 위한 NGFW 배치, 웹/API 서버 전면의 OWASP CRS 기반 WAF 리버스 프록시 운영, 비인가 섀도 IT 차단 및 데이터 유출 방지를 위한 CASB 인라인/API 듀얼 모드 연동, 세 솔루션의 로그를 실시간 교차 상관 분석하는 SIEM/SOAR 자동화 파이프라인**을 결합하여 완벽한 통합 위협 방어 체계를 완성
