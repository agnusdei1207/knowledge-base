...
- 정의/개념: UEFI 신뢰 정책으로 **부트 이미지 실행을 통제하는 보안 기술**이다.
- 배경/필요성: 운영체제 보안만으로는 선행 부트킷의 **권한 탈취 차단할 수 없다**.
...
- PK•KEK 기반 **정책 소유•갱신 권한**이 핵심이다.
- db•dbx 기반 **허용•폐기 판정**이 핵심이다.
- 펌웨어•부트로더의 **연속 신뢰 사슬**이 핵심이다.
...
- **1. PK•KEK•db•dbx 정책 설정**: 소유권•갱신•허용 정책 설정한다.
- **2. 이미지 서명•해시 검증**: 부트로더•드라이버 진위와 무결성을 검증한다.
- **3. db 허용•dbx 폐기 여부 확인**: 신뢰 체인과 폐기 여부를 정책에 대조이다.
- **4. 실행 허용•거부 판정**: 미승인 코드 차단•복구를 전환한다.
- **5. 허용 이미지 측정값 생성**: 다음 단계 실행과 TPM 증적을 기록한다.
...
- db 허용과 dbx 비폐기를 모두 충족한 **부트 이미지만 실행**이 핵심이다.
