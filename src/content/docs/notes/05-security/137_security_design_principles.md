...
- 배경/필요성: 사후 통제만으로는 **예외•공유 권한•우회 경로 제거할 수 없다**.
...
- 기본 거부•안전 실패의 **예외 없는 초기 상태**가 핵심이다.
- 최소 권한•공유•기능의 **침해 범위 제한**이 핵심이다.
- 완전 중재•감사의 **지속적 정책 집행**이 핵심이다.
...
- **1. 현재 권한•정책•맥락 구성**: 캐시 변경 여부까지 포함한 중재 입력을 구성한다.
- **2. 기본 거부•최소 권한 판정**: 명시적으로 허용된 범위이다.
- **3. 허용 자원•행위•시간 제한**: 보호 자원에 전달할 권한을 제한한다.
- **4. 자원 행위•감사 증적 기록**: 권한 회수와 추적에 쓰는 증적을 생성한다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 운영 단계에만 통제를 붙이면 설계 우회 경로 잔존 | **NIST SP 800-160 Vol.1 Rev.1** 적용 | **설계•검증•운영 일관성** 확보 |
| 상시 과권한으로 침해 시 피해 범위 확대 | **NIST SP 800-53 AC-6** 적용 | **권한•기능•시간** 축소 |
| 접근 근거가 없으면 허용 결정의 추적 불가 | **AC-3•AU 통제** 연계 | **완전 중재•감사** 확보 |
...
- 명시적 허용만 **매 요청 중재**, 실패 시 거부하되 안전 영향은 별도를 전환한다.
