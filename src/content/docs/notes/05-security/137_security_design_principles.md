...
- 배경/필요성: 현대 클라우드, 마이크로서비스(MSA), 제로 트러스트(Zero Trust) 아키텍처에서 경계 보안의 한계를 극복하고 내부 시스템의 본원적 복원력을 확보할 요구
...
- **사전 내재화(Security by Design)**: 런타임 보안 패치 이전에 소프트웨어 아키텍처 뼈대 단계에서 공격 표면을 수학적·논리적으로 최소화
- **제로 트러스트 아키텍처(ZTA)의 이론적 근간**: 완전 중재와 안전한 기본값 원칙은 현대 제로 트러스트의 지속적 검증(Never Trust, Always Verify)과 PDP/PEP 구조로 직접 계승
- **인간 중심의 보안 구현 (Psychological Acceptability)**: 보안 통제가 사용자 편의성을 지나치게 훼손하여 사용자가 보안 설정을 우회하지 않도록 직관적 설계 요구
...
```text
┌─────────────────────────────────────────────────────────────────────────┐
│ [ 1. 정책 수립 및 기본 통제 계층 (Policy & Baseline Layer) ]            │
│  ├─ [ Fail-safe Defaults ] ➔ 명시적 허용 규칙 없는 모든 접근 Default Deny │
│  └─ [ Economy of Mechanism ] ➔ TCB 코드베이스 극소화로 설계 단순성 확보  │
└────────────────────────────────────┬────────────────────────────────────┘
                                     │ (접근 요청 인입)
                                     ▼
┌─────────────────────────────────────────────────────────────────────────┐
│ [ 2. 실시간 검증 및 중재 계층 (Complete Mediation & Zero Trust Layer) ] │
│  ├─ [ Complete Mediation (PEP/PDP) ] ➔ 매 요청마다 세션/권한 전수 검사   │
│  ├─ [ Least Privilege ] ➔ 필요한 최소 리소스/액션 권한만 인가(RBAC/ABAC)  │
│  └─ [ Separation of Privilege ] ➔ 고위험 작업 시 다중 관리자 2차 승인 강제│
└────────────────────────────────────┬────────────────────────────────────┘
                                     │ (인가된 격리 자원 접근)
                                     ▼
┌─────────────────────────────────────────────────────────────────────────┐
│ [ 3. 시스템 아키텍처 및 사용자 계층 (Architecture & Usability Layer) ]  │
│  ├─ [ Least Common Mechanism ] ➔ 테넌트 간 공유 메모리 분할 및 격리     │
│  ├─ [ Open Design ] ➔ 표준 공개 암호(AES/RSA) 사용 및 키 관리 집중       │
│  └─ [ Psychological Acceptability ] ➔ SSO, 생체인증으로 보안 우회 방지    │
└─────────────────────────────────────────────────────────────────────────┘
```
...
```text
1. [접근 요청 인입] 사용자 A가 금융 DB 서버의 계좌 테이블 읽기(Read) 요청
            │
            ▼
2. [완전 중재 (Complete Mediation)]
    ├─ 게이트웨이(PEP)가 이전 세션 토큰을 무조건 신뢰하지 않고 패킷 가로채기
    └─ [PDP 정책 결정 서버로 실시간 접근 제어 질의(XACML) 전송]
            │
            ▼
3. [안전한 기본값 및 최소 권한 평가]
...
1. **상시 거부 기본선**: 정책 오류가 발생하거나 예외가 발생하더라도 항상 차단(Deny) 상태로 폴백
2. **세션 하이재킹 무력화**: 완전 중재를 통해 매 트랜잭션마다 권한 유효성을 재검증하여 탈취된 토큰 차단
3. **피해 반경(Blast Radius) 극소화**: 최소 권한을 통해 계정이 침해되더라도 해당 단일 과업 외 타 시스템 전파 차단
4. **측면 공격(Lateral Movement) 차단**: 최소 공통 메커니즘을 통해 공유 메모리를 악용한 부채널 정보 유출 차단
5. **사용자 친화적 안전망**: 복잡한 암호 대신 FIDO 생체인증을 결합하여 사용자의 임의적 보안 우회 행위 원천 방지
...
- 반세기 동안 컴퓨터 보안 아키텍처의 황금률로 검증되어 현대 제로 트러스트와 클라우드 네이티브 보안을 지배하는 **Saltzer & Schroeder 보안 설계 원칙**은 모든 소프트웨어 및 인프라 엔지니어링의 근본 나침반이며, 실무 구현 시 **NIST SP 800-160 시스템 보안 공학 프로세스 준수**, **안전한 기본값(Default Deny) 및 최소 권한(Least Privilege)의 코드 수준 강제**, **우회 불가능한 완전 중재(Complete Mediation) PDP/PEP 구축**, **FIDO 생체인증을 통한 심리적 수용성 확보**를 통합 완성하여 최고 수준의 본원적 아키텍처 복원력을 완성
