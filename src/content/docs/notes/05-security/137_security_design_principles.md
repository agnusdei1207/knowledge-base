...
- 정의/개념: 시스템 구조·기본 동작에 **보안 판단을 내재화**
- 배경/필요성: 사후 통제의 **예외·공유 권한·우회 경로 감소**
...
sequenceDiagram
  ...
  S->>E: 1. 주체·대상·행위·맥락 요청
  E->>P: 2. 최신 정책·권한 완전 중재
  P->>R: 3. 기본 거부·최소 접근 판정
  R->>A: 4. 승인 행위·결과 기록
  A->>E: 5. 권한 변경·캐시 무효화
```
...
- **1. 주체·대상·행위·맥락 요청**: 인증·기기·시간·업무 제시
- **2. 최신 정책·권한 완전 중재**: 캐시 수명·변경 여부 확인
- **3. 기본 거부·최소 접근 판정**: 명시 허용 범위만 전달
- **4. 승인 행위·결과 기록**: 결정 근거·자원 사용 감사
- **5. 권한 변경·캐시 무효화**: 회수·정책 변경 즉시 반영
...
| 한계 | 필수 업무 과도 차단 | 의료·OT 물리 안전 영향 | 약한 대체 경로 우회 |
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 수명주기 보안 공학 | **NIST SP 800-160 Vol.1 Rev.1 적용** | 설계·검증·운영 일관성 |
| 최소 권한 | **NIST SP 800-53 AC-6 적용** | 권한·기능·시간 축소 |
| 접근 집행·감사 | **AC-3·AU 통제 연계** | 완전 중재·추적 확보 |
...
- **자산·신뢰 경계·실패 상태·운영 영향**으로 원칙을 조합한다.
