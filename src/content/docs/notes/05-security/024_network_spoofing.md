...
date: "2026-08-05T08:06:00+09:00"
tags:
  - "notes-security"
weight: 24
extra:
...
sequenceDiagram
    ...
    participant R as 주소 확인 체계
    participant T as 정상 대상
    par ARP•DNS 스푸핑
      V->>R: 주소•이름 확인 요청
      A->>V: 1. 위조 주소•응답 주입
      V->>V: 2. 위조 대응 캐시 저장
      V->>A: 3. 공격자 경로로 트래픽 전환
    and IP 스푸핑
      A->>T: 4. 위조 출발지 패킷 전송
      T-->>V: 반사 응답
    end
...
1. **위조 주소•응답 주입**: 거짓 ARP•DNS 대응을 정상 응답처럼 전달
2. **위조 대응 캐시 저장**: 인증 없는 주소•이름 정보를 신뢰 상태에 반영
3. **공격자 경로로 트래픽 전환**: 피해 통신을 도청•변조 경로로 유도
4. **위조 출발지 패킷 전송**: 피해자 주소로 반사 응답을 유도
