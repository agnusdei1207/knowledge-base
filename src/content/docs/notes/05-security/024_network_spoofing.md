---
sidebar:
  order: 24
  label: "024. 네트워크 스푸핑 - ARP·IP·DNS (Network Spoofing)"
  badge:
    text: "기출 · 70%"
    variant: note
title: "네트워크 스푸핑 - ARP·IP·DNS (Network Spoofing)"
date: "2026-07-25T01:05:00+09:00"
tags:
  - "notes-security"
weight: 24
extra:
  question_no: "024"
  source_status: "기출"
  source_history: "128회, 134회"
  priority: 70
  priority_note: "128·134회 반복된 계층별 스푸핑 비교 주제임"
---

## 미리 알고가기

- **스푸핑(Spoofing)**: 주소나 응답을 위조해 상대를 속임
- **인터넷 프로토콜(Internet Protocol, IP)**: '아이피'로 읽고 영문 머리글자로 네트워크 주소 규약임을 표시하며 패킷의 출발지·목적지를 지정
- **매체 접근 제어(Media Access Control, MAC)**: '맥'으로 읽고 영문 머리글자로 링크 계층 주소 기능임을 표시하며 같은 망의 장치를 식별
- **주소 결정 프로토콜(Address Resolution Protocol, ARP)**: '에이알피'로 읽고 영문 머리글자로 주소 대응 규약임을 표시하며 IP 주소에 맞는 MAC 주소를 조회
- **도메인 이름 시스템(Domain Name System, DNS)**: '디엔에스'로 읽고 영문 머리글자로 이름 해석 체계임을 표시하며 도메인을 IP 주소로 변환
- **캐시**: 자주 쓰는 대응 정보를 임시 저장함
- **동적 ARP 검사(Dynamic ARP Inspection, DAI)**: '디에이아이'로 읽고 영문 머리글자로 동적 ARP 검사를 표시하며 비인가 응답을 차단
- **동적 호스트 구성 프로토콜(Dynamic Host Configuration Protocol, DHCP)**: '디에이치시피'로 읽고 영문 머리글자로 동적 설정 규약임을 표시하며 단말에 IP 설정을 할당
- **유니캐스트 역방향 경로 전달(Unicast Reverse Path Forwarding, uRPF)**: '유알피에프'로 읽고 유니캐스트의 소문자 u를 RPF 앞에 붙여 출발지 IP의 역방향 경로 검사를 표시
- **DNS 보안 확장(Domain Name System Security Extensions, DNSSEC)**: '디엔에스섹'으로 읽고 DNS에 보안 확장(SEC)을 붙여 응답의 출처·무결성 검증을 표시
- **전송 계층 보안(Transport Layer Security, TLS)**: '티엘에스'로 읽고 영문 머리글자로 전송 보안 규약임을 표시하며 통신 상대 인증·암호화를 제공
- **중간자 공격**: 두 통신자 사이에서 도청·변조함
- **반사 공격**: 위조한 출발지로 대량 응답을 유도함



## Ⅰ. 개요

- **정의**: 주소·응답 위조로 통신 판단을 속임
- **배경/필요성**: 우회·가짜 목적지 공격 방지

### 쉽게 이해하기 (학습용)

- 주소록이나 발신인을 바꿔 정상 상대처럼 속이는 행위다

## Ⅱ. 특징

- ARP 스푸핑은 IP·MAC 대응을 오염시킨다
- IP 스푸핑은 출발지 주소를 위조한다
- DNS 스푸핑은 도메인의 목적지를 바꾼다
- 계층별 검증과 종단 인증을 중첩한다

### 쉽게 이해하기 (학습용)

- 주소가 정상처럼 보여도 최종 상대 인증을 별도로 확인해야 한다

## Ⅲ. 구성요소 및 구조

```mermaid
flowchart LR
    T[신뢰 정보] --> V[검증 통제]
    A[위조 주입점] --> V
    V --> C[주소 캐시·필터]
    C --> F[후속 공격 감시]
    F --> E[종단 인증]
```

| 설계 요소 | 설명 |
|:---|:---|
| 신뢰 정보 | ARP·IP·DNS 대응을 제공함 |
| 검증 통제 | DAI·uRPF·DNSSEC로 검증함 |
| 위조 주입점 | 거짓 주소나 응답을 전달함 |
| 주소 캐시·필터 | 검증된 대응 정보만 사용함 |
| 후속 공격 감시 | 도청·변조·반사 징후를 탐지함 |
| 종단 인증 | TLS로 최종 상대를 확인함 |

> 요약: 주소 대응 위조로 경로 및 종단 기만함

### 쉽게 이해하기 (학습용)

- 주소 검증 뒤에도 TLS로 최종 상대를 다시 확인한다

## Ⅳ. 원리 및 절차 흐름도

```mermaid
sequenceDiagram
    participant V as 피해 단말
    participant A as 공격자
    participant C as 주소 캐시
    participant F as 가짜 목적지
    V->>A: 주소 확인 요청
    A-->>V: 위조 주소 응답
    V->>C: 위조 대응 정보 저장
    V->>C: 목적지 주소 조회
    C-->>V: 공격자 주소 반환
    V->>F: 오염 경로로 통신
```

| 메시지 | 처리 내용 |
|:---|:---|
| 주소 확인 요청 | 정상 목적지 주소를 질의함 |
| 위조 주소 응답 | 공격자가 거짓 대응을 보냄 |
| 위조 대응 정보 저장 | 검증 없이 캐시를 오염시킴 |
| 목적지 주소 조회 | 통신할 주소를 캐시에서 찾음 |
| 공격자 주소 반환 | 오염된 주소를 정상으로 제공함 |
| 오염 경로로 통신 | 공격자 경로로 데이터를 보냄 |

> 요약: 거짓 주소가 캐시를 오염시켜 경로를 바꿈

### 쉽게 이해하기 (학습용)

- 오염된 신뢰 정보는 피해자의 실제 통신 경로를 바꾼다

## Ⅴ. 종류 및 비교

| 비교축 | ARP 스푸핑 | IP 스푸핑 | DNS 스푸핑 |
|:---|:---|:---|:---|
| 핵심 특징 | IP·MAC 대응 위조 | 출발지 IP 위조 | 도메인 응답 위조 |
| 적용 기준 | 인접망 주소 보호 | 경계 출발지 검증 | DNS 응답 검증 |
| 주요 위험 | 중간자 공격 | 반사·필터 우회 | 피싱·캐시 오염 |

> 요약: 스푸핑 유형별 검증 지점 분리함

### 쉽게 이해하기 (학습용)

- 위조하는 정보의 계층에 따라 방어 지점도 달라진다

## Ⅵ. 실무 사례

- DAI는 DHCP 바인딩과 정적 주소를 관리함
- DNSSEC 검증 뒤에도 TLS 인증을 확인함

### 쉽게 이해하기 (학습용)

- 잘못된 주소 목록을 쓰면 정상 단말까지 차단될 수 있다
- DNS 응답 진위와 웹 서버 신원은 서로 별도로 확인한다

## Ⅶ. 결론

- 로컬·경계·종단 검증을 중첩해야 함

### 쉽게 이해하기 (학습용)

- 경로 주소와 최종 통신 상대를 모두 검증해야 안전하다
