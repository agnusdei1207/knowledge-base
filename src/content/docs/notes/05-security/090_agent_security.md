...
- 정의/개념: **에이전트 보안**은 에이전트의 계획•권한•도구 실행과 복구를 통제하는 체계이다.
- 배경/필요성: 출력 필터의 **외부 상태 변경 방지할 수 없다**.
...
- 계획•메모리•도구 결과의 **비신뢰 처리**가 핵심이다.
- 호출별 정책•제한 토큰의 **완전 매개**가 핵심이다.
- 승인•중단•보상의 **잔여 피해 제한**이 핵심이다.
...
- **AI(Artificial Intelligence) 600-1**은 생성형 AI의 인간 감독•위험 측정•관리 원칙을 제시한다.
...
- 저영향•가역 행동은 **제한 토큰**, 고영향은 **거래 결속•재승인**이 핵심이다.
