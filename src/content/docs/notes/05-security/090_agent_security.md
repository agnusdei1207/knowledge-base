...
- 정의/개념: AI의 도구 호출과 상태 변경을 통제하는
  **에이전트 보안** 체계
- 배경/필요성: 텍스트 오류가 실제 자산 변경으로 번지는
  **출력과 행동의 차이**
...
1. **계획·도구 호출 제안 생성**: 실행 계획·호출 인자 생성
2. **거래 결속·정책 검증**: 승인 인자와 PEP 정책 대조
3. **제한 토큰 도구 실행**: 1회성 토큰으로 격리 실행
4. **실행 상태·감사 증적 검증**: 결과 검증·감사 로그 저장
5. **취소·보상 처리**: 실패 단계 중단·상태 원복
