...
date: "2026-08-05T12:02:00+09:00"
tags:
  - "notes-security"
weight: 90
extra:
...
```text
                          [정책 집행점]
                      /          |          \
           [신원•위임 계층] [계획•메모리 계층] [도구 등록부]
                      \          |          /
                         [감사•복구 계층]
```

선의 의미: 정책 집행점이 신원•위임, 비신뢰 계획•메모리, 허용 도구 정보를 함께 검증하고 감사•복구 계층이 승인과 실행 결과를 공통 관리하는 구조
...
1. **계획•도구 호출 제안 생성**: 순서•인수를 비신뢰 후보로 구성
2. **거래 결속•정책 검증**: 승인 내용과 실행 요청 대조
3. **제한 토큰 도구 실행**: 주체•동작•대상별 최소 권한 적용
4. **실행 상태•감사 증적 검증**: 실제 결과와 상태 변경 연결
