...
date: "2026-08-26T14:45:38+09:00"
tags:
  - "notes-security"
weight: 55
extra:
...
- 배경/필요성: 패스워드·OTP 같은 공유 비밀은 서버와 사용자가 같은 값을 나눠 보관하고 인증할 때마다 그 값을 네트워크로 흘려보내는 비용을 치러 한 번의 유출이 전 계정으로 번지므로, 비밀을 기기 밖으로 내보내지 않고 접속 도메인(RP ID)까지 서명에 묶는 **공개키 인증** 계층으로 신뢰의 자리를 옮길 필요
...
- 피싱 위험과 복구 요건에 따라 **WebAuthn**과 동기화·백업 인증기를 함께 적용
