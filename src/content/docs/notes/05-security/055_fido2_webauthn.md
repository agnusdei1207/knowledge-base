...
- 배경/필요성: 기존의 패스워드 및 SMS/OTP 기반 인증은 서버와 클라이언트가 공유 비밀(Shared Secret)을 유지해야 하므로 대규모 데이터베이스 유출 사고에 취약하고, 공격자가 정상 사이트와 동일하게 위장한 역프록시 중간자 공격(AitM 피싱)을 수행할 경우 사용자의 일회성 인증 코드까지 실시간 탈취당하는 치명적 구조적 한계를 가짐에 따라, W3C의 브라우저 표준 API인 WebAuthn과 FIDO 얼라이언스의 하드웨어 통신 규격 CTAP2를 결합하여 사용자 기기(TPM/Secure Enclave) 내부의 비대칭 개인키 서명과 브라우저 차원의 도메인 결속(RP ID Binding)을 강제하는 **FIDO2** 프레임워크를 도입하여 서버 측 공유 비밀의 완전 소멸, 중간자 피싱 공격(AitM)의 원천 무력화 및 패스키(Passkey) 기반의 안전하고 편리한 무암호(Passwordless) 인증을 달성할 필요
...
#### 한줄 요약
- 서버가 검증 능력만 갖고 비밀은 갖지 않도록 신뢰를 기기 쪽으로 옮긴 설계이므로, 서버 유출 위험이 사라지는 대신 그 위험은 기기 분실이라는 가용성 문제로 형태를 바꿔 남는다.
...
```text
[FIDO2 및 WebAuthn 아키텍처]
├─ 신뢰 주체 (Relying Party)
│  ├─ 챌린지 발급기 (암호학적 일회용 난수)
│  └─ 공개키 저장소 (사용자 계정별 바인딩)
├─ 클라이언트 계층 (FIDO2 Client)
│  ├─ WebAuthn API (브라우저 JS 환경)
│  └─ 도메인 바인딩 (RP ID 검증·Origin 수집)
└─ 인증기 계층 (FIDO2 Authenticator)
   ├─ CTAP2 통신 인터페이스 (USB·NFC·BLE)
   ├─ 플랫폼 인증기 (TPM·Secure Enclave)
   └─ 외부 하드웨어 보안키 (UP·UV 서명)
```
...
| 구성요소 | 책임 |
|:---|:---|
| RP 서버 (Relying Party) | 챌린지 난수 발급, WebAuthn 파라미터 구성, 공개키 서명 검증 및 세션 인가 |
| WebAuthn 클라이언트 API | 브라우저에서 실행되며 Origin 도메인을 수집하고 클라이언트 데이터 무결성 보장 |
| CTAP2 프로토콜 | 브라우저와 외부 보안키(또는 내부 보안칩) 간의 안전한 암호화 통신 채널 제공 |
| 플랫폼 인증기 | 스마트폰/PC 내장 지문센서 및 Secure Enclave를 활용하는 일체형 인증기 (TouchID 등) |
| 크로스 플랫폼 인증기 | USB, NFC, BLE 인터페이스를 통해 연결되는 외장 하드웨어 보안키 (YubiKey 등) |

#### 한줄 요약
- RP는 검증만, 브라우저는 도메인 수집만, 인증기는 서명만 맡아 어느 쪽도 개인키를 보지 못하며, 피싱 방어의 실효는 브라우저가 Origin을 조작 불가능하게 주입한다는 전제 위에 서 있다.
...
#### 한줄 요약
- 도메인을 확인하던 사람의 판단이 브라우저의 Origin 수집으로 대체돼 피싱이 프로토콜 단계에서 실패하지만, 그만큼 브라우저와 OS라는 새 신뢰 기반이 방어의 전제로 들어온다.
...
#### 한줄 요약
- 셋은 경쟁 규격이 아니라 하나의 표준을 계층으로 자른 것이며, 피싱 저항성은 WebAuthn의 도메인 수집과 CTAP2의 개인키 격리가 절반씩 떠받쳐 어느 한쪽만으로는 성립하지 않는다.
...
- 서버 측 비밀정보 저장과 네트워크 상의 자격증명 전송을 근본적으로 제거하여 중간자 피싱을 수학적·구조적으로 불가능하게 만든 차세대 글로벌 무암호(Passwordless) 및 피싱 저항성(Phishing-Resistant) 인증의 최상위 표준 아키텍처로 확고히 안착하였으며, 클라우드 키체인 동기화 기반의 패스키(Passkey) 대중화로 진화하는 가운데, 실무 FIDO2 시스템 구축 시에는 W3C WebAuthn Level 3 표준 준수를 통한 크로스 브라우저 호환성 확보, 브라우저의 도메인(Origin/RP ID) 강제 바인딩 검증을 통한 AitM 역프록시 피싱 차단, 사용자 기기 내 Secure Enclave 기반 생체인증(**UV)** 및 물리 터치(UP) 강제, 단일 기기 분실에 따른 영구 잠금을 방어하는 동기화 패스키(Synced Passkey) 및 멀티 디바이스 백업 등록 거버넌스를 결합하여 완벽한 피싱 저항성 디지털 신원 생태계를 완성
