---
sidebar:
  order: 55
  label: "055. FIDO2·WebAuthn (FIDO2 WebAuthn)"
  badge:
    text: "기출 · 70%"
    variant: note
title: "FIDO2·WebAuthn (FIDO2 WebAuthn)"
date: "2026-07-27T23:59:59+09:00"
tags:
  - "notes-security"
weight: 55
extra:
  question_no: "055"
  source_status: "기출"
  source_history: "138회"
  priority: 70
  priority_note: "138회 최신 기출이며 피싱저항 인증의 핵심 표준임"
---

## 미리 알고가기

- **온라인 신속 신원 확인 2(Fast IDentity Online 2, FIDO2)**: ‘파이도 투’로 읽고 강조 대문자의 머리글자와 세대 번호 2를 결합한 표기이며 WebAuthn과 CTAP2로 공개키 인증을 제공함
- **웹 인증(Web Authentication, WebAuthn)**: ‘웹오슨’으로 읽고 Web Authentication을 줄인 표기이며 브라우저·서버의 공개키 자격 증명 API임
- **인증자 연결 프로토콜 2(Client to Authenticator Protocol 2, CTAP2)**: ‘씨탭 투’로 읽고 영문 머리글자와 세대 번호를 결합한 표기이며 클라이언트와 외부 인증자가 통신함
- **신뢰 당사자(Relying Party, RP)**: ‘알피’로 읽고 두 영문 단어의 머리글자를 딴 표기이며 인증 결과를 검증해 서비스를 제공함
- **신뢰 당사자 식별자(Relying Party Identifier, RP ID)**: ‘알피 아이디’로 읽고 RP와 Identifier의 축약 ID를 결합한 표기이며 자격 증명을 사용할 서비스 도메인 범위를 정함
- **원본(Origin)**: [읽기: 원본; 표기 이유: 한글명 뒤 영문 원어 병기] 프로토콜·호스트·포트로 구분한 웹 호출 출처
- **인증자(Authenticator)**: [읽기: 인증자; 표기 이유: 한글명 뒤 영문 원어 병기] 개인키를 보관하고 사용자 승인 후 서명하는 장치나 기능
- **자격 증명 ID(Credential ID)**: [읽기: 아이디; 표기 이유: 영문 머리글자] 서버가 등록된 공개키 자격 증명을 찾는 식별자
- **도전값(Challenge)**: [읽기: 도전값; 표기 이유: 한글명 뒤 영문 원어 병기] 서버가 매 인증마다 생성하는 예측 불가능한 일회성 값
- **사용자 존재(User Presence)**: [읽기: 사용자 존재; 표기 이유: 한글명 뒤 영문 원어 병기] 사용자가 인증기 조작에 참여했음을 확인하는 조건
- **사용자 검증(User Verification)**: [읽기: 사용자 검증; 표기 이유: 한글명 뒤 영문 원어 병기] PIN이나 생체 정보로 실제 사용자를 로컬 확인하는 조건
- **XSS(Cross-Site Scripting)**: [읽기: 엑스에스에스; 표기 이유: 영문 머리글자] 신뢰 웹 원본에서 공격자 스크립트가 실행되는 취약점
- **재전송 공격(Replay Attack)**: [읽기: 재전송 공격; 표기 이유: 한글명 뒤 영문 원어 병기] 이전의 정상 인증 응답을 다시 보내 인증을 우회하는 공격


## Ⅰ. 개요

- 정의/개념: WebAuthn·CTAP2 기반의 **RP 결속 공개키 인증 체계**
- **배경/필요성**: 비밀번호 공유 비밀은 **피싱·재사용·서버 유출에 취약**

### 쉽게 이해하기 (학습용)

- FIDO2는 사이트별 공개키와 원본 검증으로 비밀번호 공유와 피싱 재사용 위험을 줄인다.

## Ⅱ. 특징

- **인증자 개인키 비노출·서비스별 공개키**로 서버 비밀 유출을 줄인다.
- **RP ID·원본 결속**으로 피싱 사이트의 서명 요청을 거부한다.
- **도전값·사용자 존재·사용자 검증**으로 재전송·무단 사용을 차단한다.

### 쉽게 이해하기 (학습용)

- 인증기는 현재 사이트의 새 도전값에만 서명하며 서버에는 재사용 가능한 개인키를 저장하지 않는다.

## Ⅲ. 구성요소 및 구조

```mermaid
flowchart LR
    A[RP 서버] --> B[WebAuthn 클라이언트]
    B --> C[인증자]
    A --> D[자격 증명 저장소]
    E[등록·복구 정책] --> A
    E --> D
```

| 설계 요소 | 설명 |
|:---|:---|
| RP 서버 | **도전값 발급·서명 검증** |
| WebAuthn 클라이언트 | **원본 확인·요청 중계** |
| 인증자 | **개인키 보관·서명 수행** |
| 자격 증명 저장소 | **공개키·식별자 관리** |
| 등록·복구 정책 | **인증자 추가·분실 통제** |

### 쉽게 이해하기 (학습용)

- 브라우저가 사이트 원본을 인증기에 전달하고 인증기는 RP별 키로 서명하며 서버가 등록 공개키로 검증한다.

## Ⅳ. 원리 및 절차 흐름도

```mermaid
sequenceDiagram
    participant R as RP 서버
    participant B as WebAuthn 클라이언트
    participant A as 인증자
    R->>B: 도전값 전달
    B->>A: 서명 요청
    A-->>B: 서명 응답 반환
    B-->>R: 인증 응답 전달
    R->>R: 자격 증명 검증
```

| 절차 | 설명 |
|:---|:---|
| 도전값 전달 | 무작위 값과 RP 정책 제공 |
| 서명 요청 | 원본·RP ID와 요청 결합 |
| 서명 응답 반환 | 사용자 확인 후 개인키 서명 |
| 인증 응답 전달 | 식별자와 서명 결과 전송 |
| 자격 증명 검증 | 도전값·원본·서명 확인 |

> 요약: 현재 사이트의 새 도전값에만 서명한다.

### 쉽게 이해하기 (학습용)

- 서버가 만든 일회성 도전값을 인증기가 현재 RP 정보와 묶어 서명해야 로그인 요청이 승인된다.

## Ⅴ. 종류 및 비교

| FIDO2 구성 역할 | FIDO2 | WebAuthn | CTAP2 |
|:---|:---|:---|:---|
| 적용 기준 | 비밀번호 없는 인증 설계 | 브라우저와 서버 연동 | 보안키·휴대전화 연동 |
| 핵심 특징 | 전체 공개키 인증 체계 | 웹 자격 증명 API | 외부 인증자 통신 |
| 한계 | 등록·복구 경로 약화 | 원본 검증 오류 | 악성 클라이언트 중계 |

### 쉽게 이해하기 (학습용)

- FIDO2과 대안의 선택 기준을 비교함

## Ⅵ. 실무 사례

1. RP 서버는 **도전값·원본·RP ID 검증**
2. 관리자 로그인은 **인증자 사용자 검증 요구**

### 쉽게 이해하기 (학습용)

- 짧게 만료되는 도전값과 정확한 원본·RP ID를 확인해 재전송·피싱을 거부한다.
- 관리자 작업은 PIN·생체 확인을 거친 서명만 인정한다.

## Ⅶ. 결론

- 웹 공개키는 **WebAuthn**, 외부 인증자는 **CTAP2**, 전체 체계는 **FIDO2**

### 쉽게 이해하기 (학습용)

- FIDO2는 도전값·원본·RP ID 검증과 안전한 인증자 수명 관리를 함께 적용해야 한다.
