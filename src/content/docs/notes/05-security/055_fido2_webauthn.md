...
1. **도전값•RP 정책 생성**: 무작위 값과 사용자 검증 요구 구성
2. **원본•RP ID 검증**: 현재 서비스 출처와 자격 증명 범위 확인
3. **사용자 존재•검증 확인**: 직접 조작과 생체•PIN 검증 수행
4. **RP별 개인키 서명**: 도전값•원본 문맥을 서비스 키로 서명
5. **도전값•원본•서명 검증**: 재전송•피싱 원본과 위조 응답 거부
