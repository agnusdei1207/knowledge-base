...
- 배경/필요성: 파일 단위 복사로는 비할당 영역과 **원본 무변경 입증할 수 없다**.
...
- 오류•도구•인계 기록의 **재현성•출처 추적성**이 핵심이다.
...
4. **SHA-256 해시 대조•봉인**: 이미지 동일성•원본을 보존한다.
...
- 범위 파일은 **논리 획득**, 삭제•비할당 영역은 **물리 이미징**, SSD는 **TRIM 상태 기록**이 핵심이다.
