...
- **무선 갱신(Over-the-Air, OTA)**: 통신망으로 펌웨어를 원격 배포·설치하는 방식이다.
...
> **키워드:** 펌웨어 보안 — 하드코딩 자격증명 (Firmware Security)
...
- **1. 펌웨어 이미지·매니페스트**: SBOM·대상·버전·정책 결합
- **2. 서명된 매니페스트·이미지**: 분리된 키와 OTA로 배포
- **3. 대상·버전·해시·서명**: 장치 정책과 배포물의 일치 검증
- **4. 검증·설치 판정**: 원자적 설치와 부팅 허용 여부 결정
