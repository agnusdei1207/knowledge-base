...
```text
[하드웨어 신뢰 루트]---[부팅 진위•무결성 검증]
          |                         |
[장치별 비밀•서명키]---[인증 갱신•롤백 방지]---[변조 탐지•안전 복구]
```

선의 의미: 하드웨어 신뢰 루트와 장치별 키를 기반으로 부팅 검증, 인증 갱신, 변조 탐지•복구가 결합되는 펌웨어 신뢰 구조를 나타낸다.
...
sequenceDiagram
  ...
  M->>D: 서명된 매니페스트•이미지 전달
  D->>D: 2. 대상•버전•해시•서명 검증
  D->>D: 3. 설치•부팅•복구 판정
  D-->>O: 부팅•복구 상태
```
...
- **2. 대상•버전•해시•서명 검증**: 장치 정책과 배포물의 일치 확인
- **3. 설치•부팅•복구 판정**: 원자적 설치와 부팅 허용 여부 결정
