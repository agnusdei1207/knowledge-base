...
- 정의/개념: 전용 장치의 하드웨어•펌웨어•통신•물리 경로에 존재하는 **보안 취약점**이다.
- 배경/필요성: 공용 비밀•미패치 결함은 동일 모델 전체로 **원격 공격 확산**이 발생한다.
...
- 자원 제약•장기 운영의 **보호 적용 한계**가 핵심이다.
- 원격•로컬•물리•공급망의 **다층 공격면**이 핵심이다.
- 공용 자격증명•미서명 갱신의 **장치군 확산**이 핵심이다.
...
2. **신뢰 루트•포트 정책 구현**: 장치별 키와 JTAG•UART 잠금을 적용한다.
3. **서명 이미지•안전 갱신 구현**: 진위•버전 검증과 실패 복구를 적용한다.
4. **퍼징•포트•결함 주입 시험**: 공격 경로와 미해결 위험을 검증한다.
...
- 원격 입력은 **파서•인증**, **로컬 포트 잠금**, 부팅•갱신은 **서명•롤백 방지**를 적용한다.
