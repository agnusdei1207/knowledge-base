...
```text
[입력 전처리]---[상태 변환 함수]---[내부 상태]---[다이제스트 출력]
```

선의 의미: 각 선은 동일 인코딩•길이•블록 규칙, 블록별 상태 계산, 연쇄 내부 상태와 고정 길이 다이제스트가 서로 대응하는 해시 함수 내부 관계를 나타낸다.
...
```mermaid
sequenceDiagram
    participant M as 입력 메시지
    participant P as 입력 전처리
    participant F as 상태 변환 함수
    participant D as 다이제스트 출력
    M->>P: 메시지 입력
    P->>P: 1. 인코딩•패딩
    P->>F: 2. 메시지 블록 전달
    loop 모든 블록 처리
...
1. **인코딩•패딩**: 입력을 정해진 바이트열과 블록 길이로 변환
2. **메시지 블록 전달**: 전처리한 블록을 순서대로 변환 함수에 제공
3. **내부 상태 갱신**: 각 블록과 이전 상태로 다음 상태 계산
4. **다이제스트 생성**: 마지막 내부 상태를 고정 길이 해시값으로 출력
