...
- 실제 데이터는 대칭키로 암호화하고 그 대칭키만 공개키로 보호한다.
...
| OAEP 처리기 | 평문•해시•난수로 **OAEP 블록** 생성•검증 |
| 공개키•개인키 연산기 | 공개 지수로 암호화하고 개인 지수로 **복호화** 수행 |

### 쉽게 이해하기 (학습용)
- 키 생성기와 저장소가 키 수명주기를 담당하고 OAEP 처리기와 키 연산기가 안전한 암복호화를 수행한다.
...
- **1. OAEP 인코딩**: 난수•해시 결합 블록 생성
- **2. 공개키 모듈러 연산**: OAEP 블록 암호화
- **3. 개인키 복호 연산 요청**: 권한•복호 요청
- **4. 개인키 모듈러 연산**: 격리된 복호 블록 생성
- **5. OAEP 검증•평문 복원**: 패딩 검증 후 복원

### 쉽게 이해하기 (학습용)

- 두 비밀 소수로 만든 공개 지수는 자물쇠를 잠그고, 격리 저장한 개인 지수는 암호문을 연다.
...
- RSA는 소인수분해, DH•ECC는 이산 로그에 기대지만 쇼어 알고리즘은 두 난제를 모두 풀어 상호 대안이 되지 못한다.
...
- RSA 복호화의 성공•실패 응답 형식과 처리 시간을 같게 해 패딩 오라클을 줄인다.

## Ⅶ. 결론

- **보호 종료일•PQC 전환일** 기반 교체 순위 결정

### 쉽게 이해하기 (학습용)

- 데이터 본체는 대칭키로 처리하고 공개키로 그 키를 보호하며, 장기 보호 자산은 PQC로 미리 전환한다.
