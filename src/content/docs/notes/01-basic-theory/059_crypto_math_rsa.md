...
| 설계 요소 | 입력·상태 | 역할 |
|:---|:---|:---|
| 키 생성기 | 소수 $p$·$q$ | 모듈러스와 공개·개인 지수 생성 |
| 개인키 저장소 | 개인 지수·원본 소수 | 비밀 키 재료 격리 보관 |
| 패딩 인코더 | 평문·해시·난수 | OAEP 암호화·PSS 서명 블록 생성 |
| 모듈러 거듭제곱 연산 | 인코딩 블록·지수 | 암호문·평문 또는 서명·검증값 산출 |
...
| 개인키 유출·무단 사용 | HSM 등 격리 저장·접근 통제·키 순환 | 키 재료 보호와 사용 추적 |
| 대용량 데이터를 RSA로 직접 처리 | 하이브리드 암호화 적용 | 성능과 데이터 크기 제약 해소 |
