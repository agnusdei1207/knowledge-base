...
- ① **인코더**가 원본 입력을 저차원의 **잠재 벡터**로 압축한다.
- ② **디코더**가 잠재 벡터에서 입력 형태의 **복원값**을 생성한다.
- ③ **재구성 손실 함수**가 원본값과 복원값의 **차이**를 계산한다.
- ④ 손실 함수가 **손실 기울기**를 전달해 인코더·디코더를 함께 **갱신**한다.
