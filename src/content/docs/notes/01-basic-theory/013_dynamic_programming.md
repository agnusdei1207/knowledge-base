...
date: "2026-07-29T11:43:07+09:00"
tags:
  - "notes-basic-theory"
weight: 13
extra:
...
1. **목표 상태 전달**: 구할 부분 문제의 최소 변수 조합 설정
2. **상태값 요청**: 저장소에서 계산 여부와 기존 값 확인
3. **전이 규칙 요청**: 미계산 상태의 하위 의존성과 결합식 결정
4. **현재 상태값 저장**: 하위 값을 결합한 확정값 재사용 등록
5. **목표 상태값 반환**: 의존 상태 해소 후 전체 문제의 값 확정
...
| 핵심 고려사항 | 위험 | 대책 |
|:---|:---|:---|
| 불완전한 **상태 정의** | 미래 결정에 필요한 정보 소실로 오답 | 이후 선택을 바꾸는 최소 변수를 상태에 포함 |
| 순환 의존•기저 상태 | 무한 재귀 또는 미정 상태 참조 | 상태 전이의 감소 방향과 종료 조건 검증 |
| 큰 **상태 공간** | 전체 표 할당으로 메모리 고갈 | 실제 의존 범위만 롤링 배열로 유지 |
| 하향식 재귀 깊이 | 깊은 상태 연쇄로 호출 스택 초과 | 상향식 전환 또는 명시적 스택 적용 |

### 쉽게 이해하기 (학습용)

- 필요한 상태만 정의해 답안 표가 불필요하게 커지는 것을 막는다.

## Ⅶ. 결론

- 상태 도달 범위•의존 순서로 **하향식•상향식** 선택
