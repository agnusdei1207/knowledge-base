...
date: "2026-08-05T17:22:22+09:00"
tags:
  - "notes-basic-theory"
weight: 13
extra:
...
- 정의/개념: 중복 부분 문제를 상태로 정의하고 계산한 **상태값을 저장•재사용해 계산량을 줄이는 알고리즘 설계 기법**
...
가지의 의미: 상태 계산과 상태값 보관의 정적 책임 분리
...
1. **상태값 조회**: 계산된 목표•하위 상태값 재사용
2. **의존 상태 계산**: 기저값 적용 또는 하위 상태 재귀 계산
3. **점화식 결합**: 의존 상태값으로 현재 상태값 산출
4. **현재 상태값 저장**: 확정값을 재사용 대상으로 등록
...
- **의존 순서**: 현재 상태보다 필요한 하위 상태를 먼저 계산하도록 정한 순서
- **공간복잡도(Space Complexity)**: 입력 크기에 따라 추가 메모리 사용량이 증가하는 정도
