...
date: "2026-07-29T11:37:31+09:00"
tags:
  - "notes-basic-theory"
weight: 6
extra:
...
1. **순회 요청**: 처리 목적과 종료 조건 설정
2. **첫 노드 참조 전달**: 헤드에서 순회 시작점 획득
3. **현재 값 요청**: 현재 노드의 데이터 처리
4. **다음 참조 요청**: 링크 필드의 후속 노드로 이동
5. **순회 결과 반환**: 널 도달 후 누적 처리 결과 확정
...
| 핵심 고려사항 | 위험 | 대책 |
|:---|:---|:---|
| 대상 위치의 **선형 탐색** | 삽입•삭제 전 탐색이 전체 비용 지배 | 선행 노드 참조 또는 별도 색인 유지 |
| **링크 갱신 순서** | 후속 참조를 잃어 노드 사슬 단절 | 후속 링크 보존 후 선행 링크 변경 |
| 낮은 **캐시 지역성** | 비연속 노드 순회로 캐시 누락 증가 | 노드 풀과 묶음 할당으로 인접 배치 |
| 동시 링크 갱신 | 경쟁 상태로 유실•중복 연결 발생 | 잠금 또는 검증된 원자적 리스트 알고리즘 적용 |

### 쉽게 이해하기 (학습용)

- 다음 링크를 보존하는 순서로 포인터를 바꿔 연결 단절을 막는다.
