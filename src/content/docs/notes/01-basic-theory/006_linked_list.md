...
| 계열 | 수식•의미 |
|:---|:---|
| <span style="color:#3080dd">■</span> 파란색 선언 | **연결 리스트 $k$**: 목표 위치까지 링크 추적 |
| <span style="color:#d95926">■</span> 주황색 선언 | **동적 배열 $1$**: 인덱스로 직접 접근 |

> **연결 리스트**: 목표 위치까지 링크를 순차 추적
...
- **1. 첫 노드 참조 전달**: 헤드에서 순회 시작점 획득
- **2. 현재 값 요청**: 현재 노드의 데이터 처리
- **3. 다음 참조 요청**: 후속 노드로 이동
...
flowchart TB
    R{주요 처리 조건}
    R -->|선행 노드 확보•중간 갱신| L[연결 리스트]
    R -->|인덱스 조회•연속 순회| A[동적 배열]
    R -->|키 조회•순서 갱신| C[해시•이중 연결 리스트]
```
