...
date: "2026-07-25T21:20:00+09:00"
tags:
  - "notes-basic-theory"
weight: 6
extra:
...
- **최소 최근 사용(Least Recently Used, LRU, 엘알유)**: 영문 각 단어의 첫 글자를 따 LRU로 표기하며 가장 오래 사용하지 않은 항목을 먼저 제거하는 정책
...
| 한계 | **임의 접근** 불가·낮은 **캐시 지역성** | 중간 갱신 시 **원소 대량 이동** |

> 요약: **노드 참조** 시 갱신은 연결 리스트, 임의 조회는 **동적 배열**
...
- 이름표 서랍으로 항목이 줄의 어디에 서 있는지 바로 찾아내고, 양방향 줄에서는 앞뒤 사람의 손만 다시 잡게 해 그 항목을 맨 앞으로 옮기므로 줄 전체를 다시 세우지 않고도 가장 오래 안 쓴 항목이 맨 뒤에 남는다.

## Ⅶ. 결론

- **갱신 지점 참조**가 있고 삽입·삭제가 잦으면 **연결 리스트** 선택
