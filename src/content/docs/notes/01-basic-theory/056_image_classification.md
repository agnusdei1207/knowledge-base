...
|                        심층 이미지 분류 신경망 아키텍처                      |
+-----------------------------------------------------------------------------+
|  [초기 줄기 계층 Stem]     --> 7x7/3x3 Conv + MaxPool 저수준 시각 특징 추출  |
|          |                                                                  |
|          v                                                                  |
|  [잔차/MBConv 심층 블록]   --> Residual Skip-Connection 및 역잔차 특징 학습  |
|          |                                                                  |
|          v                                                                  |
|  [글로벌 평균 풀링 GAP]    --> (B, C, H, W) -> (B, C, 1, 1) 파라미터 경량화  |
|          |                                                                  |
|          v                                                                  |
|  [선형 분류기 & Softmax]   --> 클래스별 정규화 확률 도출 (p_i = e^z_i / sum)|
+-----------------------------------------------------------------------------+
```

선의 의미: 실선 박스는 이미지 분류 신경망의 계층별 처리 블록을 나타내며, 화살표는 저수준 시각 특징에서 심층 의미 추상화 및 클래스 확률 도출 흐름을 나타낸다.
...
입력 이미지 (H x W x C)
   |
   v
[ 1. 초기 합성곱 및 다운샘플링 ] : Stem 레이어로 저수준 엣지 추출
   |
   v
[ 2. 다단계 잔차 및 MBConv 계층 통과 ] : Skip-Connection 심층 특징 추상화
   |
   v
[ 3. 글로벌 평균 풀링(GAP) 적용 ] : 2D 공간 맵을 1차원 벡터로 압축
   |
   v
[ 4. 소프트맥스 정규화 및 신뢰도 검증 ]
   |
   +---> 최대 확률 < 임계치 (판정 불확실) : 불확실(Uncertain) 분류 거부
   |
   +---> 최대 확률 >= 임계치 (정상 판정)  : [ 5. 최고 확률 클래스 확정 ]
   |
   v
대표 클래스 출력
```
...
**동작 원리**:
1. **초기 합성곱 및 다운샘플링**: 입력 이미지로부터 저수준 엣지 및 기본 시각 텍스처 특징을 고해상도에서 빠르게 추출한다.
2. **다단계 잔차 및 MBConv 계층 통과**: 해상도를 점진적으로 줄이고 채널을 확장하며 항등 매핑 잔차 연결을 통해 기울기 소실 없이 깊은 의미 특징을 학습한다.
3. **글로벌 평균 풀링 적용**: 공간 차원($H \times W$)의 평균값을 계산하여 위치 민감도를 배제하고 1차원 채널 요약 벡터로 압축한다.
4. **소프트맥스 정규화**: 완전연결 층의 로짓 출력을 합이 1이 되는 확률 분포 $P(y=c|x) = \frac{e^{z_c}}{\sum e^{z_j}}$로 정규화한다.
5. **최고 확률 클래스 확정**: 산출된 클래스별 사후 확률 중 최댓값을 갖는 인덱스를 최종 대표 클래스 레이블로 확정한다.
