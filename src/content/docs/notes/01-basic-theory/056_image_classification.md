...
- ① **입력 전처리기**가 이미지 해상도·정규화·색상 순서를 맞춘다.
- ② **특징 추출 백본**이 저수준부터 고수준까지 **계층 특징**을 추출한다.
- ③ **특징 집계**가 공간 특징 맵을 고정 길이 **특징 벡터**로 축약한다.
- ④ **분류 헤드**가 특징 벡터를 클래스별 **점수**로 변환한다.
- ⑤ 단일 레이블은 최댓값, 다중 레이블은 **임계값**으로 최종 **범주**를 판정한다.
