...
- **합성곱 신경망(CNN)**: 공유 필터로 이미지의 국소 특징을 추출하는 신경망
...
- **단일•다중 레이블**: 이미지마다 하나 또는 여러 클래스를 판정하는 방식
...
- **전역 평균 풀링(GAP)**: 채널별 특징 맵의 평균으로 공간 축을 줄이는 집계 방식
...
- **입력 텐서**: 전처리한 이미지의 높이•너비•채널•배치 축을 표현한 수치 배열
...
- **VGG**: 작은 합성곱 필터를 반복 적층해 계층적 특징을 추출하는 백본
- **ResNet**: 계층 입력을 뒤 출력에 더하는 잔차 연결로 깊은 신경망의 학습 신호를 보존하는 백본
...
- **색상 순서**: 이미지 채널을 RGB나 BGR 등으로 배열하는 규칙
...
- **ResNet•EfficientNet**: 잔차 연결로 깊은 학습을 안정화한 백본과 깊이•폭•해상도를 함께 확장한 효율형 백본
