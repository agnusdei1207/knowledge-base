...
- 배경/필요성: 신경망 계층 심화 시 발생하는 기울기 소실과 모델 파라미터 폭증을 억제하고 연산 효율성 극대화 필요
...
입력 이미지 (H x W x C)
         │
         ▼
[ 1. 초기 합성곱 및 다운샘플링 ] ──► (저수준 엣지/코너 특징 추출)
         │
         ▼
[ 2. 다단계 잔차/MBConv 계층 통과 ] ──► (고수준 텍스처/객체 부위 특징 추출)
         │
         ▼
[ 3. 글로벌 평균 풀링(GAP) 적용 ] ──► (공간 차원 제거 및 1차원 벡터화)
         │
         ▼
[ 4. 소프트맥스(Softmax) 정규화 ] ──► (클래스별 사후 확률 계산)
         │
         ▼
[ 5. 최고 확률 대표 클래스 출력 ]
```
...
- **Vision Transformer(ViT)**: 이미지를 패치 단위로 분할하여 트랜스포머 인코더에 입력함으로써 전역 어텐션을 수행하는 비전 아키텍처.
