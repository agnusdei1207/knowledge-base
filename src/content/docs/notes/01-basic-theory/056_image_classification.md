...
- **EfficientNet**: ‘이피션트넷’으로 읽고 효율적인 신경망이라는 뜻의 Efficient와 Network를 결합한 이름이며, 깊이·폭·해상도를 정해진 비율로 함께 늘려 정확도와 연산 효율의 균형을 맞춘 백본
...
| 전처리 이미지 전달 | 정규화한 입력을 **특징 추출 백본**에 전달 |
| 특징 맵 전달 | 얕은 층부터 깊은 층까지 만든 **특징 맵** 전달 |
| 집계 벡터 전달 | **전역 평균 풀링** 등으로 공간 차원 제거 |
| 분류 결과 반환 | **클래스별 점수**의 최댓값·임계 판정 반환 |
...
- 영상 분류를 위해 자원·위험 클래스 재현율을 검토하여 **분류 모델** 활용
