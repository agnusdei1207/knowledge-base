...
- 배경/필요성: 신경망 계층 심화 시 역전파 기울기가 소실/폭주하는 성능 퇴화(Degradation) 문제와 완전연결 계층(FC)의 파라미터 폭증을 극복하고, 항등 매핑 잔차 연결($F(x)+x$), 복합 스케일링(Compound Scaling) 및 GAP(Global Average Pooling)을 통해 **연산 자원을 최소화하면서 초심층 신경망의 고정밀 범주 분류를 실현**할 필요
