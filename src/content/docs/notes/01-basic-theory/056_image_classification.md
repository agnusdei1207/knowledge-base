...
date: "2026-08-26T12:11:41+09:00"
tags:
  - "notes-basic-theory"
weight: 56
extra:
...
- 배경/필요성: 층을 깊게 쌓으면 역전파 기울기가 층수만큼 곱해져 소실되고 완전연결 분류부가 가중치를 불려 **기울기 소실 및 파라미터 폭증에 따른 연산 비효율**을 함께 치르므로, 입력을 블록 출력에 그대로 더하는 우회 경로를 층 사이에 끼우고 완전연결 자리에 채널 평균 요약을 놓아 깊이는 늘리되 기울기 경로와 파라미터 수는 지킴
