...
- **① 입력 텐서 전처리**: 학습 시점과 같은 해상도·픽셀 분포·색상 순서로 이미지 변환
- **② 계층적 특징 추출**: 백본이 저수준 질감부터 고수준 의미까지 특징 맵 생성
- **③ 공간 특징 집계**: 전역 평균 풀링 등으로 공간 특징을 고정 길이 벡터로 축약
- **④ 클래스 점수·레이블 판정**: 분류 헤드가 클래스별 점수를 산출하고 단일·다중 레이블 규칙 적용
...
- 제조 검사원이 불량 사진 더미에 큰 경고표를 붙이듯, 불량 클래스 가중치와 판정 임계값을 조정해 제한된 지연·메모리 안에서 미탐을 줄인다.

## Ⅵ. 결론

- **연산량 예산**이 작으면 EfficientNet 계열, 깊은 전이 학습은 **ResNet**을 후보로 두고 실제 장치 지연 검증
