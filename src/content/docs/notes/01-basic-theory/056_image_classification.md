...
- 정의/개념: 이미지 전체에서 특징을 추출하고 클래스별 점수 또는 확률을 계산하여 하나 이상의 범주를 판정하는 컴퓨터 비전 기법이다.
...
- **정규화(Normalization)**: 픽셀값의 중심과 척도를 학습 때 기준에 맞추는 전처리한다.
...
- **1. 입력 텐서 전처리**: 해상도•정규화•색상 순서 통일이다.
- **2. 계층적 특징 추출**: 질감부터 의미까지 특징 맵을 생성한다.
- **3. 공간 특징 집계**: GAP로 고정 길이 특징 벡터 변환한다.
...
> 요약: 안정성은 **ResNet**, 효율은 **EfficientNet**가 핵심이다.
...
- 심층 안정은 **ResNet**, 효율은 **EfficientNet**을 선택한다.
