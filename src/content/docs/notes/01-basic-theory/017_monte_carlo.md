...
date: "2026-08-05T17:27:42+09:00"
tags:
  - "notes-basic-theory"
weight: 17
extra:
...
- **표준오차(Standard Error, SE)**: 표본마다 달라지는 추정량의 변동 크기로 정밀도를 나타내는 척도
...
- **IID 표본평균**의 표준오차가 $1/\sqrt{n}$로 감소
...
가지의 의미: 표본•평가•통계•종료 판정의 정적 책임 분리
...
1. **독립 표본 생성**: 목표 분포에서 입력 추출
2. **표본별 모델 평가**: 표본마다 관측값 산출
3. **통계량•종료 조건 판정**: 평균•표준오차로 반복 여부 결정
...
- **중요도 표본추출(Importance Sampling)**: 영향이 큰 영역을 더 자주 뽑아 분산을 줄이는 기법
- **층화 표본추출(Stratified Sampling)**: 모집단의 각 층에서 표본을 확보해 분산을 줄이는 기법
