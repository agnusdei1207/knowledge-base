...
- 정의: 입력 규모(Input Size) 변화에 따른 연산 횟수와 메모리 점유의 점근적 증가율을 수치화한 성능 지표.
- 필요성: 실측값은 대규모 데이터 환경의 자원 고갈(OOM) 및 타임아웃 예측이 불가능하여 하드웨어 독립적 분석 체계 필수.

#### 한줄 요약

- 입력 규모(N) 증가에 따른 연산 횟수(Operation Count) 및 메모리 점유(Memory Footprint)의 점근적 증가율 분석.
...
- 하드웨어 독립적 **점근 증가율(Asymptotic Growth Rate)** 및 **빅오 표기(Big-O Notation)**로 최악 시나리오 내 자원 한도 초과 여부 검증.
- **빅세타(Big-Theta)** 및 **빅오메가(Big-Omega)** 활용하여 알고리즘의 평균·최선 성능을 세밀하게 분석.
- 중간 결과 저장과 재연산 간 **시간•공간 상충(Time-Space Tradeoff)** 고려하여 메모리 여유분 기반 처리 속도 최적화 전략 수행.
- 데이터 분포별 **최선•평균•최악(Best/Average/Worst Case)** 사례를 구분하여 적합한 기법 선택.
...
```text
복잡도 분석 구조
├─ 계산 모델 (Computational Model): 단위 비용 정의
├─ 시간 분석기 (Time Analyzer): 연산 횟수 산정
├─ 공간 분석기 (Space Analyzer): 추가 메모리 산정
└─ 점근 차수 판정기 (Asymptotic Order Evaluator): 증가율 분류
```
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 비대표 기본 연산 산정으로 인한 병목 지점 불일치 | 지배 반복문 내 핵심 기본 연산 기준으로 비용 함수 도출 | 시간복잡도 예측의 정확성 확보 |
| 평균 입력 분석으로 인한 최악 사례 자원 초과 위험 은폐 | 실제 입력 분포별 평균·최악 사례 산정 및 자원 한도 교차 검증 | 자원 한도 초과(OOM, Timeout) 선제 예방 |
| 재귀 스택 및 캐시 누락으로 인한 보조 공간 과소평가 | 최대 호출 깊이 및 중간 저장소 크기를 전체 공간 비용에 합산 | 공간복잡도 예측 정확도 극대화 |
| 상수 계수 차이로 인한 응답시간 역전 | 운영 환경 대상 실증적 벤치마크 병행 | 최적 알고리즘(Optimal Candidate) 식별 |

- 최악 사례 입력 중심의 연산 횟수 산정 및 재귀 호출 스택을 포함한 보조 공간의 정밀 측정 필수.
...
- **응답시간(Response Time)** 및 **메모리(Memory)** 한도 중 선행 초과가 예상되는 자원을 기준으로 최적 알고리즘 채택.
