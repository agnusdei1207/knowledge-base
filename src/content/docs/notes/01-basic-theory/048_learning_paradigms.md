...
- ① 데이터·환경에서 라벨·구조·보상의 **학습 신호**를 수집한다.
- ② **목적 함수**가 신호를 손실·구조 목적·**기대 누적 보상**으로 계산한다.
- ③ **학습기**가 목적값에 따라 모델 가중치 또는 **정책**을 갱신한다.
- ④ **학습 모델**이 예측·표현·행동의 **결과**를 산출한다.
- ⑤ 결과에 따른 다음 **학습 신호**를 반영해 종료 조건까지 **반복**한다.
...
### 쉽게 이해하기 (학습용)
- 패러다임마다 예측 지표·구조 타당성·정책 행동을 다르게 검증한다. 장애 티켓은 담당자가 기록한 장애 유형을 라벨로 사용해 지도 학습할 수 있다.

## Ⅵ. 결론

- **라벨·잠재 구조·환경 보상**으로 학습 패러다임 선택
