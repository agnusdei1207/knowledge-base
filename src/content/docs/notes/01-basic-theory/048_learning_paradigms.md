...
date: "2026-07-29T10:10:00+09:00"
tags:
  - "notes-basic-theory"
weight: 48
extra:
...
- **학습 패러다임(Learning Paradigm)**: 모델에게 정답표•숨은 구조•행동 점수 중 어떤 학습 신호를 주는지에 따른 학습 방식의 분류
- **학습 신호(Learning Signal)**: 모델 갱신 기준이 되는 라벨•데이터 구조•보상
...
- **학습 신호**가 목적 함수와 **갱신 대상**을 결정
- 라벨•구조•보상에 따라 **피드백 방식** 분화
...
| 구성요소 | 책임 |
|:---|:---|
| 학습 신호 | 라벨•구조•보상으로 **학습 기준** 제공 |
| 목적 함수 | 학습 신호를 **갱신 기준**으로 변환 |
| 학습기 | 갱신 기준에 따라 **모델 파라미터** 조정 |
| 학습 모델 | 예측•표현•행동 **정책** 산출 |

**동작 원리**

- **① 모델 출력**: 입력에서 예측•표현•행동 산출
- **② 학습 신호 제공**: 라벨•구조•보상으로 목표 구분
- **③ 갱신 기준 변환**: 목적 함수가 출력과 신호를 평가
- **④ 파라미터 갱신**: 평가 결과로 모델 동작 조정

### 쉽게 이해하기 (학습용)
- 목적 함수가 정답표•숨은 규칙•행동 점수를 채점 기준으로 바꾸면 학습기가 모델을 고친다.
...
### 쉽게 이해하기 (학습용)
- 지도는 라벨 편향, 비지도는 구조 타당성, 강화는 보상 목표만 각각 검증한다.
