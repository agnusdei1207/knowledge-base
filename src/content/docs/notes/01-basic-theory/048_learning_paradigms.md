...
- 기존 한계: 단일 학습법은 라벨 유무·상호작용 조건 대응 한계
...
flowchart LR
    S{학습 신호 식별}
    S -->|입력·레이블| SL[예측 오차 계산]
    S -->|레이블 없는 입력| UL[잠재 구조 목적 계산]
    S -->|상태·행동·보상| RL[기대 누적 보상 계산]
    SL --> U[모델 가중치 갱신]
    UL --> U
    RL --> P[행동 정책 갱신]
```
...
| 절차 | 설명 |
|:---|:---|
| 학습 신호 식별 | 레이블·입력 구조·환경 보상 중 **가용 신호** 확인 |
| 예측 오차 계산 | 지도 학습의 정답·예측 차이로 **손실** 산출 |
| 잠재 구조 목적 계산 | 비지도 학습의 군집·표현 **목적값** 산출 |
| 기대 누적 보상 계산 | 강화 학습의 미래 보상을 포함한 **목표값** 산출 |
...
- 학습 방식 선정을 위해 레이블·보상을 검토하여 **학습 패러다임** 활용
