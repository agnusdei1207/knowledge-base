...
- **환경 상호작용**: 강화 학습의 에이전트가 행동하고 다음 상태와 보상을 관측하는 과정
- **목적 함수**: 학습 신호와 모델 결과를 손실이나 보상값으로 바꾸는 함수

</details>

- 정의/개념: **학습 신호•환경 상호작용**에 따른 학습 방식 분류
...
- **모델 파라미터**: 학습 과정에서 조정하는 가중치와 편향 등의 값
...
- **정답 라벨**: 입력별 목표 범주나 값을 제공하는 학습 신호
- **환경 보상**: 행동 결과의 가치를 나타내는 강화 학습 신호
- **잠재 구조**: 라벨 없이 데이터에 내재한 군집•분포•표현
...
- **1. 학습 신호 판별**: 라벨•보상•데이터 자체 구분
- **2. 지도 학습 선택**: 예측 오차로 모델 갱신
- **3. 강화 학습 선택**: 기대 누적 보상으로 정책 갱신
- **4. 비지도 학습 선택**: 데이터의 잠재 구조 탐색
- **5. 방식별 품질 검증**: 집단 지표•전문가 검토•행동 감사
...
- **라벨 편향(Label Bias)**: 정답 기준이 집단별로 치우친 상태
- **라벨 잡음(Label Noise)**: 라벨이 잘못 기록된 상태
- **목표 우회**: 의도한 목표 대신 보상 함수의 빈틈을 이용해 점수만 높이는 행동
...
- **집단별 지표**: 사용자나 데이터 집단별로 나누어 계산한 오분류율•정확도 등의 성능 지표
- **전문가 표본 검토**: 군집이나 표현의 일부 사례를 도메인 전문가가 직접 확인하는 평가
- **행동 감사**: 강화 정책의 선택과 결과가 제약과 의도한 목표를 지키는지 점검하는 절차
