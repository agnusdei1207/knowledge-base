...
- **학습 패러다임(Learning Paradigm)**: 학습 신호와 상호작용에 따른 학습 방식의 분류
...
- **군집·이상치(Cluster·Outlier)**: 유사한 표본의 집합과 분포를 벗어난 관측값
- **상태·환경·보상(State·Environment·Reward)**: 현재 상황·상태 전이 대상·행동 평가값
...
- **목적 함수(Objective Function)**: 예측·표현·행동 품질을 수치화하는 함수
...
- **① 학습 신호 판별**: 라벨·데이터 구조·환경 보상과 상호작용 방식 확인
