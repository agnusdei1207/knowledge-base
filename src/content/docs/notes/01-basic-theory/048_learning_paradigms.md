...
date: "2026-07-27T23:29:00+09:00"
tags:
  - "notes-basic-theory"
weight: 48
extra:
...
| 절차 | 설명 |
|:---|:---|
| 학습 신호 제공 | 라벨·데이터 구조·환경 보상 중 **가용 신호** 전달 |
| 목적 함수 계산 | 지도 손실·비지도 구조 목적·**기대 누적 보상** 산출 |
| 모델·정책 갱신 | 목적값에 따라 가중치 또는 **행동 정책** 갱신 |
| 예측·행동 산출 | 갱신 모델로 예측값·표현·**행동** 생성 |

> 요약: 가용한 **학습 신호**가 목적 함수와 갱신 대상을 결정

### 쉽게 이해하기 (학습용)
- 지도 학습은 정답과 예측의 차이, 비지도 학습은 데이터 구조, 강화 학습은 미래 보상을 목적값으로 계산한 뒤 모델의 가중치나 정책을 갱신한다
