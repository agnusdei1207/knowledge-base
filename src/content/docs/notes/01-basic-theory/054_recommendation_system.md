...
- 정의/개념: 사용자 선호를 예측해 **노출 순위**를 개인화하는 **시스템**
- 배경/필요성: 아이템 증가와 희소한 선호 관측으로 **관련 항목 탐색 곤란**
...
- **사용자 특징(User Feature)**: 사용자의 프로필과 속성을 나타내는 정보
- **아이템 특징(Item Feature)**: 아이템의 내용과 범주를 나타내는 정보
...
- **1. 특징•이력 조회**: 프로필•속성•상호작용 확인
- **2. 추천 후보 생성**: 전체 아이템에서 평가 후보군 축소
- **3. 선호 점수•순위 산출**: 후보별 적합도와 노출 순서 결정
- **4. 노출•반응 로그 정제**: 다음 학습용 피드백 데이터 생성
...
- **근사 최근접 이웃 탐색(Approximate Nearest Neighbor Search, ANN Search)**: 모든 아이템을 비교하지 않고 가까운 후보를 빠르게 찾는 기법
...
- **이력 밀도**가 높으면 협업, 신규 대상은 콘텐츠, 혼재는 하이브리드
