...
- ① **데이터 저장소**에서 사용자·상황 특징과 **상호작용 이력**을 조회한다.
- ② **후보 생성기**가 전체 아이템에서 관련도 높은 **후보 집합**을 추린다.
- ③ **랭킹 모델**이 후보별 선호 점수를 계산해 **개인화 순위**를 산출한다.
- ④ 상위 아이템을 사용자에게 **추천 목록**으로 노출한다.
- ⑤ **피드백 파이프라인**이 클릭·구매와 노출 여부를 함께 수집한다.
- ⑥ 수집 로그를 **학습 데이터**로 갱신해 다음 추천의 **편향 보정**에 활용한다.
...
- 정확도·다양성·신선도·지연을 함께 측정한다. 신규 아이템은 속성이 닮은 기존 상품과 함께 노출해 첫 상호작용을 확보하고 콜드 스타트를 완화한다.

## Ⅵ. 결론

- 이력 밀도·신규성에 맞춰 **추천 방식** 선택
