...
### 쉽게 이해하기 (학습용)
- 데이터 저장소, 후보 생성기, 랭킹 모델, 피드백 파이프라인이 조회부터 재학습까지 역할을 나눈다.
...
- **2. 특징•이력**: 후보 생성용 정보 반환
- **3. 후보 목록**: 정밀 평가 후보군 축소
- **4. 후보 랭킹•추천 노출**: 선호•다양성 기반 순위 결정
- **5. 사용자 반응 수집**: 노출•클릭•구매•체류 기록
- **6. 재학습 데이터 생성**: 노출•반응 로그 정제

### 쉽게 이해하기 (학습용)

- 후보를 축소해 순위를 매기고, 노출 뒤 반응을 다음 학습 데이터로 되돌린다.
...
- 신규 아이템을 속성 기반으로 탐색 노출해 첫 상호작용을 확보한다.

## Ⅶ. 결론

- **이력 밀도•신규 비율** 기반 추천 방식 선택
