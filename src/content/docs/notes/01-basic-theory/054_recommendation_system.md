...
| 후보 재학습 데이터 전달 | 노출 반응을 **후보 학습 표본**으로 변환·전달 |
| 후보 생성기 갱신 | 노출 반응으로 **후보 추출 모델** 재학습 |
| 랭킹 재학습 데이터 전달 | 노출 반응을 **랭킹 학습 표본**으로 변환·전달 |
...
- 전체 아이템에 정밀 점수를 계산하면 응답 시간이 증가하므로 후보 생성으로 대상을 축소한 뒤 랭킹을 수행하고, 사용자 반응을 다음 후보 생성과 랭킹 학습에 반영한다
