...
- **오류 상관(Error Correlation)**: 여러 모델이 같은 표본에서 함께 맞거나 함께 틀리는 정도로, 값이 높으면 결합해도 오류가 잘 상쇄되지 않는다
...
```mermaid
sequenceDiagram
    participant Q as 학습·추론 요청자
    participant C as 앙상블 제어기
...
- **① 학습 집합 구성**: 부트스트랩·가중치·폴드로 기본 학습기별 데이터와 결합 방식 구성
- **② 기본 학습기 학습**: 담당 집합 또는 이전 잔차를 반영해 다양성을 가진 모델 생성
- **③ 개별 예측 산출**: 같은 신규 입력에 대한 각 기본 학습기의 예측값 생성
- **④ 예측 결합**: 투표·평균·메타 모델로 개별 예측을 합쳐 오류 상쇄
...
- 검증 성능 이득이 지연·비용 예산을 넘을 때만 **앙상블 채택**
