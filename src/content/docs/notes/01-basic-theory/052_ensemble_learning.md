...
date: "2026-07-26T00:32:00+09:00"
tags:
  - "notes-basic-theory"
weight: 52
extra:
...
- **앙상블 학습(Ensemble Learning)**: 어려운 진단을 앞두고 의사 한 명이 아니라 여러 명에게 따로 물어 다수 의견을 택하듯, 여러 모델의 예측을 결합해 단일 모델의 오류를 줄이는 방식이다
...
- **폴드 외(Out-of-Fold, OOF)·배깅 제외(Out-of-Bag, OOB) 예측**: OOF는 ‘오오에프’, OOB는 ‘오오비’로 읽고 영문 머리글자를 딴 약어이며, 그 모델이 학습에 쓰지 않은 표본으로만 예측해 스스로를 검증하는 방식이다
...
- **정의/개념**: 여러 모델 예측을 결합해 **일반화 오차**를 줄이는 방식
- **기존 한계**: 단일 모델은 **편향·분산**을 동시에 낮출 수 없어 예측 불안정
...
- 모델 수 증가에 따른 **추론 비용**·해석성 저하

### 쉽게 이해하기 (학습용)
- 다섯 의사가 모두 같은 학교에서 같은 교재로 배웠다면 틀릴 때도 똑같이 틀려 모아 봐야 소용이 없고 서로 다른 경험을 가졌을 때만 실수가 상쇄되는데, 그 대신 진료비가 다섯 배로 늘고 어느 소견이 결론을 갈랐는지 짚기도 어려워진다
...
| 앙상블 기법 | 배깅 | 부스팅 | 스태킹 |
|:---|:---|:---|:---|
| 적용 기준 | 같은 모델의 **예측 분산** 과다 | **편향** 크고 잡음 적은 데이터 | 성격 다른 **이종 모델** 다수 보유 |
...
1. 서버 장애 예측은 **랜덤 포레스트** **OOB**로 트리 수 결정
2. 이상 탐지 스태킹은 **폴드 외 예측**으로 **누수** 방지

### 쉽게 이해하기 (학습용)
- 트리마다 학습에 안 쓰인 표본이 남으므로 별도 검증 데이터를 떼지 않고도 그 남은 표본으로 성능을 재어, 트리를 더 늘려도 점수가 안 오르는 지점에서 개수를 멈춘다
- 기본 모델이 자기가 학습한 표본을 다시 맞힌 점수를 메타 모델에게 주면 실제보다 훨씬 잘 맞히는 것처럼 보이므로, 각 표본을 학습에 쓰지 않은 모델의 예측값만 골라 메타 모델에 넘긴다
