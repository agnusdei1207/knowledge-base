...
- **신뢰구간(Confidence Interval)**: 반복 표본 추출에서 정한 비율로 참모수를 포함하도록 만든 구간
...
- **표준오차**: 추정량 표본분포의 표준편차
- **유의수준(Significance Level)**: 참인 귀무가설을 잘못 기각할 최대 허용 확률
- **신뢰구간(Confidence Interval)**: 추정값과 표준오차로 계산한 모수의 가능한 범위
...
- **효과 크기(Effect Size)**: 집단 차이나 관계의 실질적인 크기
- **표준오차(Standard Error)**: 추정량 표본분포의 표준편차
...
- **1. 검정 통계량 산출**: 표본 차이를 표준화
- **2. 유의확률 계산**: 귀무분포의 극단 확률 산출
- **3. 기각 여부 판정**: 유의수준과 비교
...
- **제1종•제2종 오류**: 참인 귀무가설을 기각하는 오류와 거짓인 귀무가설을 기각하지 못하는 오류
