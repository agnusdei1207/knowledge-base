...
date: "2026-07-28T21:02:00+09:00"
tags:
  - "notes-basic-theory"
weight: 30
extra:
...
```mermaid
flowchart TB
    S[원본 데이터] -->|① 통계 중복 제거| SE[소스 부호기]
    SE -->|② 보호 중복 추가| CE[채널 부호기]
    CE -->|③ 잡음 채널 통과| CH[잡음 채널]
    CH -->|④ 오류 정정| CD[채널 복호기]
    CD --> SD[소스 복호기]
    SD --> O[복원 데이터]
```
...
- **① 통계 중복 제거**: 정보원 분포에 맞춰 표현 비트 축소
- **② 보호 중복 추가**: 목표 오류율에 필요한 정정 비트 부가
- **③ 잡음 채널 통과**: 전송 과정에서 비트 오류 유입
- **④ 오류 정정**: 보호 비트로 압축 비트열 복구
...
| 채널 변화로 보호율 부적합 | 오류율 기반 적응형 코드율 | 처리량·신뢰도 균형 |
| 짧은 지연의 분리 설계 손실 | 결합 소스-채널 코딩 | 제한 지연의 품질 향상 |
