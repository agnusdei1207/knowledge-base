...
- **전방 오류 정정(Forward Error Correction, FEC)**: 중복 비트로 재전송 없이 오류 정정
...
- **기존 한계**: 한 부호화로는 **전송 비트 수**·**오류율**을 동시 충족 불가
...
| 한계 | **엔트로피 하한** 미만 압축 불가 | 부호화 **오버헤드**·복호 지연 증가 |
...
2. 실시간 영상은 재전송 대신 **FEC**로 **지연 예산** 준수
...
- 화상 회의 영상은 빠진 조각을 다시 보내 달라고 요청하면 이미 늦어 화면이 멈추므로, 처음부터 복구용 조각을 함께 실어 보내 받는 쪽이 일부를 놓쳐도 그 자리에서 메워 정해진 지연 안에 화면을 이어 붙인다

## Ⅶ. 결론

- **지연 예산**이 짧으면 **결합 소스-채널 코딩** 선택
