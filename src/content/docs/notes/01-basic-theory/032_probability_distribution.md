...
```mermaid
sequenceDiagram
    participant Q as 구간확률 질의기
    participant S as 표본공간 정의기
    participant R as 확률변수 사상기
    participant P as 모수 저장소
    participant F as 분포 모형
...
- **① 사건의 수치값 사상**: 표본공간의 사건을 확률변수가 취하는 값 또는 구간으로 변환
- **② 확률변수 값 전달**: 확률을 평가할 이산값 또는 연속 구간을 분포 모형에 제공
- **③ 분포 모수 제공**: 평균·분산·성공률·발생률 등 선택한 분포의 모수 적용
- **④ 질량·밀도 계산**: 이산값은 PMF, 연속값은 PDF로 발생 가능성 평가
- **⑤ 구간 확률 산출**: CDF의 누적값 또는 두 누적값의 차이로 요구 확률 반환
