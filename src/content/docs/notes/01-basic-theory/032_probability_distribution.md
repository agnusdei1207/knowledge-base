...
date: "2026-07-29T12:02:41+09:00"
tags:
  - "notes-basic-theory"
weight: 32
extra:
...
- 정의/개념: **확률 변수**의 값•구간에 발생 가능성 부여
- 배경/필요성: 미관측 값•구간의 **발생 확률** 추정
...
| 계열 | 수식•의미 |
|:---|:---|
| <span style="color:#3080dd">■</span> 파란색 선언 | **이항**: $n=4,p=0.5$ |
| <span style="color:#d95926">■</span> 주황색 선언 | **포아송**: $\lambda=2$ |

> **계산 예시**: 지정 모수의 사건 횟수별 확률
...
- **1. 관측 데이터 전달**: 이산•연속 변수 판정
- **2. 후보 분포 전달**: 생성 과정에 맞는 모형 선정
- **3. 검증 가정 전달**: 독립성•산포•꼬리 확인
- **4. 추정 모수 전달**: 분포 모형의 모수 확정
...
flowchart TB
    R{변수•생성 과정}
    R -->|연속•독립 변동 합| N[정규 분포]
    R -->|고정 시행•성공 횟수| B[이항 분포]
    R -->|고정 구간•발생 횟수| P[포아송 분포]
```

- 변수 유형•생성 과정을 보고 **가정 검증** 후 분포 확정
