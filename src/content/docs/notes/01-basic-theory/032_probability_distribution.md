...
- **생성 과정**: 관측값이 어떤 시행•사건 구조에서 만들어졌는지를 나타내는 확률적 절차
- **가정 검증**: 독립성•산포•꼬리 등 후보 분포의 전제가 데이터와 맞는지 확인하는 과정
- **모수 추정**: 관측 데이터로 분포의 평균•분산•발생률 같은 값을 정하는 과정
- **독립성**: 한 관측 결과가 다른 관측의 발생 확률에 영향을 주지 않는 성질
- **꼬리(Tail)**: 분포 중심에서 멀리 떨어진 극단값 영역
