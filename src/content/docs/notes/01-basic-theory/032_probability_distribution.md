...
- **PMF•PDF**: 이산값마다 확률을 부여하거나 연속 구간의 확률 밀도를 나타내는 함수
- **기댓값•분산**: 확률 분포의 평균 위치와 값들이 흩어진 정도를 나타내는 모수
- **정규화 조건**: 모든 가능한 값의 확률 합이나 밀도 적분이 1이 되는 조건
- **이산•연속 확률 변수**: 셀 수 있는 값을 갖는 변수와 연속 구간의 값을 갖는 변수
...
- **CDF**: 특정 값 이하가 나올 누적 확률을 나타내는 함수
...
- **Q-Q 도표**: 표본과 이론 분포의 분위수가 얼마나 일치하는지 점검하는 도표
...
- **중심극한정리**: 독립 확률 변수의 합이나 평균이 표본 수 증가에 따라 정규 분포에 가까워지는 정리
- **평균 발생률**: 포아송 분포에서 단위 구간에 사건이 발생하는 평균 횟수
- **정규•이항•포아송 분포**: 연속 오차의 합, 고정 시행의 성공 횟수, 고정 구간의 사건 수를 각각 나타내는 분포
