...
| 핵심 고려사항 | 위험 | 대책 |
|:---|:---|:---|
| 메모리보다 큰 **입력 크기** | 인메모리 정렬 중 메모리 고갈 | 구간 정렬 후 **외부 병합** 적용 |
| 동일 키의 **입력 순서** | 불안정 정렬로 연관 레코드 순서 변경 | 순서 보존 요구 시 **안정 정렬** 선택 |
| 퀵 정렬의 **불균형 분할** | 재귀 깊이 증가와 최악 $O(n^2)$ 퇴화 | 무작위 피벗과 깊이 한계 이후 힙 정렬 전환 |
| 객체의 **비교 비용** | 반복 키 계산으로 비교 횟수 이상의 지연 | 키 사전 계산과 참조 기반 재배열 적용 |
