...
```mermaid
flowchart TB
    S[시작 기호 S] -->|① 생성 규칙 선택| P[생성 규칙 P]
    P -->|② 비단말 치환| D[유도열]
    D --> C{비단말 존재}
    C -->|예| P
    C -->|아니요| T[단말 문자열·구문 트리]
```
...
```mermaid
sequenceDiagram
    participant S as 시작 기호
    participant P as 생성 규칙
    participant D as 유도열
    participant C as 비단말 판정기
    S->>P: ① 생성 규칙 선택
    loop 비단말 존재
        P->>D: ② 비단말 치환
        D->>C: 치환 결과
...
| 같은 입력 위치의 규칙 충돌 | FIRST·FOLLOW 집합으로 파싱표 검증 | 결정적 규칙 선택 |
| 오류 입력의 진단 연쇄 | 동기화 토큰 기반 오류 복구 | 후속 오류 위치 정확성 향상 |
