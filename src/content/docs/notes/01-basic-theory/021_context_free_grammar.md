---
sidebar:
  order: 21
  label: "021. 문맥 자유 문법 (Context-Free Grammar)"
  badge:
    text: "미출 • 15%"
    variant: note
title: "문맥 자유 문법 (Context-Free Grammar)"
date: "2026-08-06T23:21:00+09:00"
tags:
  - "notes-basic-theory"
weight: 21
extra:
  question_no: "021"
  source_status: "미출"
  source_history: ""
  priority: 15
  priority_note: "문법•파싱 전제이며 구문 트리 설명 가능"
---

## Ⅰ. 개요

<details>
<summary>핵심 용어</summary>

- **문맥 자유 문법(Context-Free Grammar, CFG)**: 좌변이 단일 비단말인 생성 규칙으로 재귀적 중첩 구문을 표현하는 문법
- **비단말 기호**: 생성 규칙으로 더 치환할 수 있는 구문 범주
- **재귀 구문**: 같은 구문 범주를 내부에 다시 포함해 임의 깊이로 중첩되는 구조
- **형식 문법(Formal Grammar)**: 알파벳과 생성 규칙으로 유효한 문자열 구조를 정의하는 체계

</details>

- 정의/개념: 단일 비단말 규칙으로 중첩을 표현하는 **문맥 자유 문법(CFG)**
- 배경/필요성: 정규 문법으로 표현 못 하는 **재귀 구문** 기술

#### 한줄 요약

- 괄호 안에 다시 수식이 오는 재귀 규칙으로 임의 깊이의 중첩 구문을 표현한다.

## Ⅱ. 특징

<details>
<summary>핵심 용어</summary>

- **유도(Derivation)**: 생성 규칙을 반복 적용해 시작 기호를 단말열로 바꾸는 과정
- **단말열(Terminal String)**: 비단말이 모두 치환되어 단말 기호만 남은 문자열
- **모호성(Ambiguity)**: 하나의 문자열에서 둘 이상의 구문 트리가 생성되어 해석이 달라지는 성질
- **단일 비단말 치환**: 생성 규칙 좌변의 비단말 하나를 우변 기호열로 바꾸는 작업

</details>

- 좌변 **단일 비단말** 치환으로 주변 문맥과 무관한 규칙 적용
- 재귀 규칙의 반복 **유도**로 무제한 중첩 단말열 생성
- 동일 문자열의 복수 구문 트리에 따른 **모호성**

#### 한줄 요약

- 주변 기호와 무관하게 규칙을 적용하며 같은 문자열에 구문 트리가 둘이면 문법이 모호하다.

## Ⅲ. 구조 및 구성요소

<details>
<summary>핵심 용어</summary>

- **문법 4튜플 $G=(N,\Sigma,P,S)$**: 비단말•단말•생성 규칙•시작 기호로 CFG를 정의한 묶음
- **파서(Parser)**: 입력 토큰이 문법에 맞는지 검사하고 구문 트리로 바꾸는 분석기
- **구문 트리(Parse Tree)**: 생성 규칙의 적용 관계를 부모•자식 구조로 나타낸 트리

</details>

```text
문맥 자유 문법 G
├─ 비단말 집합 N
├─ 단말 집합 Σ
├─ 생성 규칙 집합 P
└─ 시작 기호 S
```

선의 의미: 문맥 자유 문법을 정의하는 4튜플의 포함 관계

| 구성요소 | 책임 |
|:---|:---|
| 비단말 집합 N | 생성 규칙으로 치환할 **구문 범주** 정의 |
| 단말 집합 Σ | 최종 문자열을 이루는 **입력 기호** 정의 |
| 생성 규칙 집합 P | 단일 비단말의 **치환 관계** 정의 |
| 시작 기호 S | 전체 유도의 **출발 비단말** 지정 |

#### 한줄 요약

- 비단말•단말•생성 규칙•시작 기호의 4튜플로 문맥 자유 문법을 정의한다.

## Ⅳ. 흐름도

<details>
<summary>핵심 용어</summary>

- **유도 상태**: 생성 규칙을 적용하는 과정에서 현재 남아 있는 단말과 비단말의 기호열
- **입력 토큰 스트림**: 파서가 문법의 단말과 비교하도록 토큰을 순서대로 제공하는 입력
- **비단말 치환**: 선택한 생성 규칙의 좌변 비단말을 우변 기호열로 바꾸는 작업
- **생성 규칙(Production Rule)**: 비단말을 단말과 비단말의 기호열로 치환하는 규칙
- **수용**: 입력 토큰 전체가 문법 규칙에 맞는 완성 유도를 이룬 상태

</details>

```text
시작 기호 S•입력 토큰
        │
        ▼
1. 현재 기호 판별
        ├─ 비단말
        │    │
        │    ▼
        │  2. 생성 규칙 적용
        │    └── 유도 상태에 기호가 남으면 반복
        │
        └─ 단말
             │
             ▼
           3. 입력 토큰 대조
             ├─ 불일치: 거부
             └─ 일치: 다음 기호 판별
                         │
                         ▼
                 4. 수용 상태 판정
                         ├─ 입력•유도 소진: 구문 트리 반환
                         └─ 미소진: 거부
```

### 동작 원리

- **1. 현재 기호 판별**: 유도 상태의 비단말•단말 구분
- **2. 생성 규칙 적용**: 비단말 치환과 구문 트리 확장
- **3. 입력 토큰 대조**: 단말 일치 시 소비, 불일치 시 오류
- **4. 수용 상태 판정**: 입력 소진과 완성 유도 여부 확인

#### 한줄 요약

- 설계도의 빈 구문 칸을 생성 규칙으로 펼치고 입력 토큰과 맞춘 결과를 부모•자식 가지로 붙여 구문 트리를 완성한다.

## Ⅴ. 종류 및 비교

<details>
<summary>핵심 용어</summary>

- **정규 문법(Regular Grammar)**: 한 방향의 선형 생성 규칙으로 유한 상태 패턴을 표현하는 문법
- **문맥 의존 문법(Context-Sensitive Grammar)**: 비단말 주변 기호에 따라 규칙 적용 여부가 달라지는 문법
- **파싱 충돌(Parsing Conflict)**: 같은 입력 위치에서 적용할 문법 규칙을 하나로 결정하지 못하는 상태

</details>

| 형식 문법 | 문맥 자유 문법 | 정규 문법 | 문맥 의존 문법 |
|:---|:---|:---|:---|
| 적용 기준 | 입력에 따라 **중첩 깊이** 증가 | **유한 상태**로 패턴 구분 가능 | 주변 기호가 **규칙 적용** 여부 결정 |
| 핵심 특징 | **재귀적 중첩** 구조 표현 | **선형 반복**•선택 표현 | **주변 문맥** 제약 표현 |
| 한계 | **모호성**에 따른 파싱 충돌 | 중첩 구조 **표현 불가** | 일반 문자열의 **인식 계산량** 증가 |

#### 한줄 요약

- 선형 패턴은 정규 문법, 중첩은 CFG, 주변 문맥 제약은 문맥 의존 문법이 적합하다.

## Ⅵ. 실무 고려사항 및 대책

<details>
<summary>핵심 용어</summary>

- **좌재귀(Left Recursion)**: 비단말이 유도 결과의 맨 왼쪽에 다시 나타나 하향식 파서를 무한 호출시키는 재귀
- **좌인수분해(Left Factoring)**: 여러 생성 규칙의 공통 접두부를 묶어 다음 입력으로 규칙을 결정하게 하는 문법 변환
- **FIRST 집합**: 비단말에서 처음 나올 수 있는 단말의 집합
- **FOLLOW 집합**: 비단말 바로 뒤에 올 수 있는 단말의 집합
- **동기화 토큰(Synchronizing Token)**: 오류 뒤 파서가 입력 처리를 다시 시작할 위치를 찾는 기준 토큰
- **LL 파싱(Left-to-right Leftmost Derivation Parsing, LL)**: 입력을 왼쪽부터 읽고 좌측 유도를 구성하는 하향식 파싱 방식
- **파싱표(Parsing Table)**: 현재 비단말과 다음 입력 토큰으로 적용할 생성 규칙을 결정하는 표
- **오류 복구**: 문법 오류 뒤 동기화 지점으로 이동해 후속 입력 분석을 계속하는 처리

</details>

| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 동일 문자열의 **복수 구문 트리** | 우선순위•결합 규칙으로 **문법 재작성** | **파싱 모호성** 제거 |
| 하향식 파서의 **좌재귀 호출** | 좌재귀 제거•**좌인수분해** | **LL 파싱** 가능성 확보 |
| 같은 입력 위치의 **규칙 충돌** | **FIRST•FOLLOW**로 파싱표 검증 | 결정적 **규칙 선택** |
| 오류 입력의 **진단 연쇄** | 동기화 토큰 기반 **오류 복구** | 후속 **오류 위치** 정확도 향상 |
| 컴파일러의 **중첩 구문** | CFG로 괄호•블록 **트리 구성** | **구문 구조** 보존 |

#### 한줄 요약

- 모호성과 좌재귀를 제거하고 파싱 충돌•오류 복구 정책을 검증한다.

## Ⅶ. 결론

<details>
<summary>핵심 용어</summary>

- **중첩 구문**: 괄호나 블록처럼 같은 구조가 내부에 반복해서 포함될 수 있는 구문
- **하향식 파서(Top-Down Parser)**: 시작 기호에서 생성 규칙을 선택해 구문 트리를 루트부터 말단 방향으로 확장하는 파서

</details>

- 중첩 구문은 **CFG**, 주변 문맥 제약은 **문맥 의존 문법** 선택

#### 한줄 요약

- 중첩 구조는 CFG로 표현하고 하향식 파서에서는 좌재귀를 제거한다.
