...
date: "2026-07-26T00:18:00+09:00"
tags:
  - "notes-basic-theory"
weight: 35
extra:
...
- **특징 공간(Feature Space)**: 각 데이터의 측정값을 좌표축으로 삼아 점 하나씩 찍어 놓은 공간이며, 클러스터링은 이 공간 위 점들 사이의 거리만 보고 무리를 나눈다.
...
> 요약: 같은 **특징 공간** 위에서 **중심 기준**과 밀도 기준의 분기
...
> 요약: K-Means는 **중심 수렴**, DBSCAN은 **밀도 확장** 종료
...
> 요약: **군집 형태**와 밀도 균질성이 가르는 알고리즘 선택
...
2. 접속 로그의 **DBSCAN** **잡음점**으로 이상 후보 선별
...
- 정상 접속은 비슷한 시간대와 비슷한 요청 크기로 촘촘히 뭉쳐 있어 DBSCAN이 손을 잡고 늘어서듯 한 무리로 엮어 내지만, 아무와도 어깨가 닿지 않은 접속 하나는 끝내 어느 무리에도 들지 못한 채 잡음점으로 남고 그렇게 남은 점들이 곧 이상 후보 목록이 된다.
