...
+-----------------------------------------------------------------------------+
|  [특징 전처리기 Scaler]  --> StandardScaler 기반 차원별 거리 왜곡 제거      |
|          |                                                                  |
|          v                                                                  |
|  [군집 분석 엔진]        --> K-Means(유클리드 중심) vs DBSCAN(밀도 연결성)  |
|          |                                                                  |
|          v                                                                  |
|  [이상치 필터링 모듈]    --> 잡음점(Noise Point) 식별 및 유효 클러스터 분리 |
|          |                                                                  |
|          v                                                                  |
|  [군집 품질 평가기]      --> 실루엣 계수 및 Davies-Bouldin 지수 평가        |
+-----------------------------------------------------------------------------+
```

선의 의미: 실선 박스는 데이터 전처리부터 군집 평가까지의 단계별 모듈을 나타내며, 화살표는 스케일링, 군집 분할/확장 및 품질 검증 데이터 흐름을 나타낸다.
...
[ 2. 군집 특성 판별 ]
   ...
   |        |
   |        v
   |     [ 3. K-Means 중심 할당 및 갱신 ] : K-Means++ 초기화 후 중심 수렴 루프
   |
   +---> 군집 수 미지 & 비볼록 임의 형상/노이즈 혼재 :
            |
            v
         [ 4. DBSCAN 밀도 확장 및 노이즈 분리 ] : ε 및 MinPts 기반 밀도 탐색
            |
            v
         [ 5. 군집 품질 평가 및 업무 검증 ] : 실루엣 계수 및 해석 타당성 확인
```

분기 결과: 군집 수 k가 주어지고 구형 분포일 때는 K-Means 중심 이동으로 분기하고, 군집 수가 미지이고 기하학적 비볼록 형상이나 이상치 분리가 필요할 때는 DBSCAN 밀도 확장으로 분기한다.

**동작 원리**:
1. **특징 표준화 전처리**: 변수 간 단위 편차로 인한 특정 차원의 거리 척도 지배를 방지하기 위해 `StandardScaler` 기반 표준화를 수행한다.
2. **군집 특성 판별**: 데이터의 분포 기하 구조(볼록 구형 vs 비볼록 곡면)와 노이즈 혼재 여부를 분석하여 최적 알고리즘을 분기 선택한다.
3. **K-Means 중심 할당 및 갱신**: $k$개 중심을 K-Means++로 초기화하고 최근접 샘플 할당과 중심점 평균 재계산을 변동이 없을 때까지 반복 수렴한다.
4. **DBSCAN 밀도 확장 및 노이즈 분리**: $\varepsilon$ 반경 내 최소 포인트 수($\text{MinPts}$)를 기준으로 핵심점을 추출하고 밀도 도달성으로 군집을 확장하며 고립점을 노이즈로 격리한다.
5. **군집 품질 평가 및 업무 검증**: 실루엣 계수($s > 0.5$)와 엘보우 기법으로 군집 분리도를 측정하고 도메인 해석 가능성을 실증 검증한다.
