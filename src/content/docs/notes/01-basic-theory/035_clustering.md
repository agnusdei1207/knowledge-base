...
- 배경/필요성: 고객 세분화, 이상 탐지, 이미지 분할 등에서 대규모 비라벨 데이터의 내재적 기하 구조 및 밀도 패턴 식별 필요
...
- 거리 척도(유클리드, 맨해튼, 코사인) 및 **특징 정규화(Standardization)에 따른 결과 민감성** #### 한줄 요약
...
**동작 원리** 1. **데이터 표준화**: 변수 간 척도 차이에 의한 거리 왜곡을 방지하기 위해 Z-Score 표준화 적용
