...
date: "2026-07-28T12:09:04+09:00"
tags:
  - "notes-basic-theory"
weight: 35
extra:
...
| 고려사항 | 대응 방안 | 기대 효과 |
|:---|:---|:---|
| 단위 차이가 거리 계산을 지배 | 표준화와 업무 목적에 맞는 거리 척도 적용 | 의미 있는 군집 경계 확보 |
| K-Means 초기 중심에 따른 지역 최솟값 | K-Means++와 다중 초기화 실행 | 결과 안정성 향상 |
| 군집 수 또는 밀도 매개변수 오설정 | 실루엣·엘보·k-distance와 업무 해석 병행 | 매개변수 선택 근거 확보 |
| DBSCAN의 밀도 차 군집 분리 실패 | 계층적 밀도 군집화나 구간별 매개변수 검토 | 이질적 밀도 대응 |

### 쉽게 이해하기 (학습용)
- 스케일·초기값·군집 수·밀도 매개변수에 대한 민감도를 검증해야 한다.
...
### 쉽게 이해하기 (학습용)
- CPU·메모리 등 특징을 표준화한 뒤 서버 워크로드 유형을 K-Means로 묶는다.
