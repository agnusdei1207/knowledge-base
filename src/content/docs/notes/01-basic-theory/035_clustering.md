...
- 배경/필요성: 정답 레이블이 없는 대규모 데이터에서 **내재적 기하 구조 및 밀도 패턴 자율 식별 요구**
...
│  │
│  ▼
│  [ 3. K-Means 중심 할당 및 갱신 ] : K-Means++ 초기화 후 수렴 루프
│  │
│  └─ 중심점 위치 변동 없을 때까지 수렴
│
└─ 군집 수 미지 & 비볼록 임의 형상/노이즈 혼재
   │
   ▼
   [ 4. DBSCAN 밀도 확장 및 노이즈 분리 ] : ε 및 MinPts 기반 확장
   │
   ▼
[ 5. 군집 품질 평가 및 업무 검증 ] : 실루엣 계수 및 해석 검증
```

**동작 원리**

1. **특징 표준화 전처리**: 변수 간 척도 차이에 의한 거리 왜곡을 방지하기 위해 Z-Score 표준화 적용
2. **군집 특성 판별**: 데이터의 분포 형상과 노이즈 유무에 따라 K-Means 또는 DBSCAN 선택
3. **K-Means 중심 할당 및 갱신**: K-Means++로 초기 중심을 설정하고 최근접 할당 및 중심 이동을 반복 수렴
4. **DBSCAN 밀도 확장 및 노이즈 분리**: $\varepsilon$ 반경과 $\text{MinPts}$로 핵심점을 찾아 밀도 연결성으로 군집을 확장하고 노이즈 격리
5. **군집 품질 평가 및 업무 검증**: 실루엣 계수와 도메인 해석 가능성을 평가하여 최종 군집 확정
