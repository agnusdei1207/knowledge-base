...
date: "2026-07-29T12:04:32+09:00"
tags:
  - "notes-basic-theory"
weight: 35
extra:
...
- 정의/개념: 레이블 없이 거리•**밀도**로 집단 구조 탐색
- 배경/필요성: 정답이 없는 데이터의 **유사 집단** 발견
...
sequenceDiagram
    ...
    loop 매개변수 후보 존재
        A->>P: 1. 원시 데이터•거리 척도 전달
        P->>C: 2. 스케일 특징•매개변수 전달
        C->>E: 3. 군집 후보 전달
        E-->>A: 4. 품질 평가 결과 반환
        alt 업무 해석 미충족
            A->>P: 5. 전처리•매개변수 조정 요청
        end
    end
```
...
- **1. 원시 데이터•거리 척도 전달**: 업무 목적에 맞는 척도 선택
- **2. 스케일 특징•매개변수 전달**: 거리 비교가 가능한 입력 생성
- **3. 군집 후보 전달**: 표본을 군집•잡음으로 배정
- **4. 품질 평가 결과 반환**: 응집도•분리도 판정
- **5. 전처리•매개변수 조정 요청**: 척도•스케일을 바꿔 재평가
