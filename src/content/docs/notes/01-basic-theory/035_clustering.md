...
- **① 특징 전처리**: 스케일링을 적용해 거리 비교가 가능한 특징 생성
- **② 군집 후보 생성**: 선택한 거리·밀도 기준으로 표본을 군집 또는 잡음에 배정
- **③ 군집 품질 평가**: 실루엣 계수와 업무 해석으로 응집도·분리도·활용 가능성 판정
- **④ 척도·스케일 조정**: 품질이 부족하면 거리 척도·스케일링과 군집 매개변수를 바꿔 재평가

### 쉽게 이해하기 (학습용)
- K-Means는 점을 가장 가까운 깃발에 모아 깃발 위치를 평균으로 옮기고, DBSCAN은 일정 반경 안에 이웃이 많은 점부터 덩어리를 넓힌다.
