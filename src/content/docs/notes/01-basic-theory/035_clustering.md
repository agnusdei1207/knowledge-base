...
```mermaid
flowchart TB
    R[원시 관측 데이터] -->|① 특징 스케일링| F[특징 공간]
    F -->|② 군집 후보 생성| C[K-Means·DBSCAN]
    C -->|③ 군집 품질 평가| E[군집 평가기]
    E --> J{업무 해석 충족}
    J -->|아니요| F
    J -->|예| O[군집·잡음 결과]
...
```mermaid
sequenceDiagram
    participant R as 원시 관측 데이터
    participant F as 특징 공간
    participant C as 군집기
    participant E as 군집 평가기
    participant O as 군집 결과
    R->>F: ① 특징 스케일링
    loop 매개변수 후보 존재
        F->>C: ② 군집 후보 생성
        C->>E: ③ 군집 품질 평가
...
- **① 특징 스케일링**: 단위 차이를 제거하고 거리 척도 고정
- **② 군집 후보 생성**: 중심 또는 밀도 연결 기준으로 표본 배정
- **③ 군집 품질 평가**: 실루엣 계수와 업무 해석으로 후보 판정
