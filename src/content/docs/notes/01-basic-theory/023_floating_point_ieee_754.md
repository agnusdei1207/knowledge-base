...
date: "2026-08-06T23:23:00+09:00"
tags:
  - "notes-basic-theory"
weight: 23
extra:
...
- **전기전자공학자협회 부동소수점 산술 표준(IEEE Standard for Floating-Point Arithmetic, IEEE 754)**: 부동소수점 형식•연산•특수값을 정한 표준

</details>

- 정의/개념: 실수를 유한 비트에 근사하는 **IEEE 754 부동소수점 표준**
- 배경/필요성: 구현별 형식•반올림 차이로 **연산 결과 불일치**
...
- **바이어스 지수(Biased Exponent)**: 음수 지수를 비부호 값으로 저장하는 규칙
- **숨은 비트(Hidden Bit)**: 정규수의 선행 1을 저장하지 않는 규칙
- **비결합성(Non-associativity)**: 반올림 때문에 덧셈 순서에 따라 결과가 달라지는 성질
- **서브노멀(Subnormal Number)**: 0 부근의 표현 공백을 잇는 비정규화 값
- **숫자 아님(Not a Number, NaN)**: 유효한 수가 아닌 연산 결과를 나타내는 특수값
...
- **부호 필드(Sign Field)**: 값의 부호를 저장하는 비트 영역
- **지수 필드(Exponent Field)**: 값의 크기 범위를 저장하는 비트 영역
- **가수 필드(Fraction Field)**: 값의 유효 비트를 저장하는 비트 영역
...
선의 의미: IEEE 754 저장 워드를 이루는 세 비트 필드의 포함 관계
...
- **1. 세 필드 분리**: 부호•지수•가수 비트 추출
- **2. 지수 패턴 판별**: 일반 지수와 특수 인코딩 구분
- **3. 0•서브노멀 판별**: 가수의 0 여부로 값 구분
- **4. 무한대•NaN 판별**: 가수의 0 여부로 특수값 구분
- **5. 정규수 복원**: 바이어스 제거와 숨은 비트 결합
...
- **binary16**: 16비트로 저장하는 IEEE 754 이진 형식
- **binary32**: 32비트로 저장하는 IEEE 754 이진 형식
- **binary64**: 64비트로 저장하는 IEEE 754 이진 형식
- **동적 범위(Dynamic Range)**: 한 형식이 표현할 수 있는 가장 작은 값부터 가장 큰 값까지의 폭
- **메모리 대역폭(Memory Bandwidth)**: 단위 시간에 메모리와 연산 장치 사이에서 전송할 수 있는 데이터 양
- **반정밀도(Half Precision)**: 16비트로 저장하는 IEEE 754 형식
- **단정밀도(Single Precision)**: 32비트로 저장하는 IEEE 754 형식
- **배정밀도(Double Precision)**: 64비트로 저장하는 IEEE 754 형식
...
- **정수 형식**: 자릿값의 정확한 일치가 필요한 계산 표현
- **10진 형식**: 10진 자릿값을 정확히 보존하는 계산 표현
