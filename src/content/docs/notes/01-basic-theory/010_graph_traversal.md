...
> 중복 경로가 없는 이진 분기($b=2$) 개념도이며, 넓은 그래프는 BFS 프런티어가 빠르게 증가
...
- **③ 이웃 방문 여부**: 이미 발견한 이웃을 후보에서 제외
- **④ 발견 상태 기록**: 중복·순환 방지 상태 반영
- **⑤ 미방문 이웃 삽입**: 큐·스택 규칙으로 다음 후보 저장
...
- 출발점에서 물결처럼 가까운 역부터 찾는 BFS는 최소 간선 경로에, 한 노선을 끝까지 따라가는 DFS는 활성 경로의 순환 탐지에 맞는다.
...
- 역을 대기함에 넣는 순간 방문 도장을 찍어 중복을 막고, 노선도가 넓거나 깊으면 양방향 탐색·명시적 스택과 시작 시점 스냅샷을 고른다.
