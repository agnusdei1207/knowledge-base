...
date: "2026-07-29T11:40:44+09:00"
tags:
  - "notes-basic-theory"
weight: 10
extra:
...
- **스택 오버플로(Stack Overflow)**: 재귀 호출이나 저장한 경로가 스택 용량을 넘는 오류
...
- 물결처럼 같은 거리의 역을 넓히면 BFS, 한 노선을 끝까지 따라가면 DFS이며, 방문 도장은 같은 역을 다시 도는 일을 막는다.
...
- 탐색 제어기가 프런티어에서 다음 노드를 고르고, 인접 정보와 방문 상태를 대조해 새 후보만 추가한다.
...
| 핵심 고려사항 | 위험 | 대책 |
|:---|:---|:---|
| 발견 노드의 **중복 적재** | 순환 그래프에서 반복 처리와 메모리 증가 | 프런티어 삽입 시 방문 상태 즉시 표시 |
| 넓은 그래프의 **BFS 큐** | 큰 분기 계수로 프런티어 메모리 고갈 | 깊이 제한 또는 양방향 탐색 적용 |
| 깊은 그래프의 **DFS 재귀** | 편향 경로에서 호출 스택 초과 | 명시적 스택 기반 반복 탐색 |
| 탐색 중 그래프 변경 | 누락•중복 경로로 도달성 결과 불일치 | 스냅샷 또는 그래프 버전 고정 |

### 쉽게 이해하기 (학습용)

- 역을 대기함에 넣을 때 방문 표시해 중복 탐색을 막는다.
