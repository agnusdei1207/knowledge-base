...
date: "2026-08-02T09:43:00+09:00"
tags:
  - "notes-basic-theory"
weight: 25
extra:
...
- **NAND•NOR**: AND•OR 결과를 각각 반전한 게이트로 단독으로 모든 부울 논리를 구성할 수 있는 범용 게이트
...
- **AND•OR 평면**: 곱항을 만들고 그 결과를 합쳐 논리 함수를 구현하는 두 단계의 게이트 묶음
...
- **참•부정 리터럴**: 원 입력 변수와 NOT을 적용한 반대 입력값
- **SOP(Sum of Products)**: 출력이 1인 조건의 곱항들을 OR로 결합한 정규형
- **2단 논리(Two-level Logic)**: AND•OR 또는 OR•AND의 두 단계로 출력을 만드는 구현
- **곱항(Product Term)**: 여러 리터럴을 AND로 결합한 논리식 항
...
- **POS(Product of Sums)**: 출력 조건의 합항들을 AND로 결합한 정규형
...
- **기본•배타•범용 게이트**: AND•OR•NOT, XOR•XNOR, NAND•NOR로 나눈 게이트 유형
