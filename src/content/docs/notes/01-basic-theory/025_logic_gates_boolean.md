...
- **① 입력 비트**: 입력 변수의 0·1 조합 전달
- **② 참·부정 리터럴**: 인버터로 원 변수와 부정 변수 생성
- **③ 곱항**: AND 평면에서 출력 1 조건별 리터럴 결합
- **④ 논리 함수 출력**: OR 평면에서 곱항을 합쳐 최종값 산출
...
### 쉽게 이해하기 (학습용)
- 여러 스위치 조건을 묶으면 AND·OR·NOT, 서로 다름을 찾으면 XOR, 한 종류 부품만 써야 하면 모든 식을 만드는 NAND·NOR를 고른다.
