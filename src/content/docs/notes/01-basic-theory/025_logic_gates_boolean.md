...
- 정의/개념: 0•1 입력과 출력의 관계를 부울식으로 표현•단순화하고 이를 논리 게이트 조합으로 구현하는 디지털 논리 설계 체계이다.
...
- **논리식 최소화**: 같은 진리표를 유지하면서 리터럴과 게이트 수를 줄이는 변환한다.
...
- **논리 단수(Logic Depth)**: 입력에서 출력까지 통과하는 연속 게이트 단계 수이다.
...
- 조건 조합은 **기본**, 다름 판정은 **배타**, 단일 소자는 범용 게이트를 선택한다.
