...
- **평가 편향 및 과소 활용(Evaluation Bias & Underutilization)**: 우연히 특정 분할에만 과적합되거나 데이터 일부가 검증에만 고정되어 학습 기회가 유실되는 현상.

</details>

- 정의/개념: 전체 데이터를 $K$개의 부분 집합으로 분할하여 **$K-1$개 훈련 및 1개 검증**을 $K$회 순환하는 모델 일반화 성능 평가 기법
- 배경/필요성: 단일 홀드아웃(Hold-out) 분할 시 발생하는 평가 결과의 표본 편향 및 훈련 데이터 유실 한계 극복 필요
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 교차검증 루프 외부 전처리 수행에 따른 검증 데이터 누출(Data Leakage) 발생 | `sklearn.pipeline.Pipeline` 객체로 **전처리-모델 일원화** | 일반화 성능 과대평가 원천 방지 |
...
- 독립 표본은 **일반 K-Fold**, 불균형 데이터는 **층화 K-Fold**, 시계열 데이터는 **시간 분할(TimeSeriesSplit)** 선택
