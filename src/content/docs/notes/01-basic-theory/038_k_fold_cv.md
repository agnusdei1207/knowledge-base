...
- **K-Fold 교차 검증(K-Fold Cross-Validation)**: 전체 데이터셋을 $K$개의 상호 배타적인 폴드(Fold)로 분할하여, 각 폴드를 $K$회 순환하며 검증 집합으로 사용하여 일반화 성능을 객관적으로 평가하는 리샘플링 기법.
- **홀드아웃(Hold-out)**: 데이터를 훈련셋과 검증셋으로 단 1회 단순 분할하여 모델을 검증하는 방식.
- **데이터 누출(Data Leakage)**: 검증 폴드의 정보(평균, 스케일러 통계량)가 훈련 폴드의 전처리 또는 모델 학습에 유출되어 성능이 과대평가되는 현상.

</details>

- 정의/개념: 전체 데이터를 $K$개의 부분집합으로 나누어 $K-1$개로 훈련하고 1개로 검증하는 과정을 $K$번 반복하여 모델의 일반화 성능을 통계적으로 추정하는 **모델 검증 및 하이퍼파라미터 튜닝 기법**
- 배경/필요성: 단일 홀드아웃 분할 시 특정 데이터 표본 구성에 따라 발생하는 **평가 분산(Evaluation Variance)과 과적합(Overfitting)을 완화하고 데이터 활용도 극대화 필수**
...
- 모든 데이터 표본이 정확히 $K-1$회 훈련에 참여하고 1회 검증에 사용되는 **완전한 데이터 활용성**
- $K$회 검증 점수의 **평균(Mean)과 표준편차(Std)를 함께 산출하여 모델의 일반화 안정성 정량화**
- 전처리(Scaling/Imputation) 과정을 각 폴드 내부로 격리하여 **데이터 누출(Data Leakage) 원천 차단**
...
| 검증 폴드 (1 Fold) | 미학습 표본 기반 **일반화 예측 성능(Accuracy, F1, RMSE) 측정** |
| 점수 집계기 (Aggregator) | $K$개 점수의 **평균($\mu$) 및 분산($\sigma$) 계산하여 최종 성능 확정** |
...
1. **분할 전략 수립**: 데이터 형태에 따라 일반 K-Fold, 층화 K-Fold, TimeSeriesSplit 결정
2. **순환 분할 및 격리 학습**: 훈련 폴드 데이터만을 사용하여 스케일러를 적합하고 모델 학습
3. **독립 검증**: 훈련에 관여하지 않은 검증 폴드를 변환하여 예측 점수 $S_k$ 산출
4. **통계 집계**: $K$회의 점수 평균과 편차를 계산하여 모델의 일반화 성능과 안정성 검증
5. **최종 재학습**: 교차 검증으로 최적 모델 구조를 결정한 뒤 전체 데이터셋으로 최종 릴리스 모델 생성
...
| 비교 항목 | 일반 K-Fold | 층화 K-Fold (Stratified) | 그룹 K-Fold (Group) | 시계열 분할 (TimeSeries) |
|:---|:---|:---|:---|:---|
| 데이터 조건 | **독립적 회귀/균형 분류** | **불균형 타깃 분류** | **동일 사용자 반복 관측** | **시계열/시간 순서 데이터** |
| 분할 방식 | 완전 무작위 균등 분할 | **클래스 비율 동일 유지** | **동일 그룹을 한 폴드에 몰아서 배정** | **과거 훈련 $\to$ 미래 검증** |
| 방지하는 위험 | 데이터 표본 편향 | 소수 클래스 누락/편향 | **동일 개체 정보 누출** | **Look-ahead 미래 정보 누출** |
| 한계 | 불균형/시계열 시 성능 왜곡 | 회귀 문제에는 직접 적용 불가 | 그룹 수가 $K$개 미만이면 불가 | 초기 폴드의 훈련 데이터 부족 |
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| CV 루프 밖에서 전처리 수행으로 **데이터 누출(Leakage)** | `sklearn.pipeline.Pipeline` 객체로 **전처리-모델 일원화** | 일반화 성능 과대평가 원천 방지 |
