...
- 배경/필요성: 단일 **홀드아웃**은 어느 표본이 검증에 걸렸느냐에 따라 점수가 흔들려 **표본 편향에 따른 일반화 평가 왜곡**을 낳고 이를 줄이려면 데이터를 더 모으는 비용을 치러야 하므로, 같은 데이터로 분할을 $K$번 돌려 모든 표본이 한 번씩 검증을 맡게 하고 점수의 평균과 편차로 평가의 불확실성까지 함께 산출
