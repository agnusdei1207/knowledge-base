...
date: "2026-07-28T12:11:13+09:00"
tags:
  - "notes-basic-theory"
weight: 38
extra:
...
| 고려사항 | 대응 방안 | 기대 효과 |
|:---|:---|:---|
| 전처리 기준의 검증 정보 누수 | 회차별 훈련 폴드 안에서 전처리 적합 | 일반화 성능 과대평가 방지 |
| 동일 사용자·장비 표본의 폴드 분산 | 그룹 단위 분할 적용 | 개체 정보 누수 방지 |
| 불균형 클래스의 폴드별 소실 | 층화 분할과 최소 클래스 수 점검 | 평가 지표 안정화 |
| 시계열을 무작위로 섞어 미래 정보 사용 | 시간 순서 보존·과거 학습/미래 검증 | 배포 시점 성능 모사 |

### 쉽게 이해하기 (학습용)
- 전처리와 분할 단위를 평가 목적에 맞게 정해야 데이터 누수를 막을 수 있다.
...
### 쉽게 이해하기 (학습용)
- 결측값 대치와 스케일링 기준을 매 회차 훈련 폴드에서만 계산한다.

## Ⅶ. 결론

- 일반화 검증을 위해 불균형·의존성을 검토하여 **교차검증** 활용
