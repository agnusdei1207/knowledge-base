...
date: "2026-07-29T11:34:23+09:00"
tags:
  - "notes-basic-theory"
weight: 4
extra:
...
1. **탐색 요청**: 목표값과 초기 반열린 후보 구간 설정
2. **중앙 키 조회**: 오버플로를 피한 중앙 인덱스 산정
3. **중앙 키 반환**: 목표값과 비교해 일치•좌•우 판정
4. **후보 경계 갱신**: 목표가 없는 절반을 후보에서 제외
5. **탐색 결과 반환**: 일치 위치 또는 빈 구간의 실패 확정
...
| 핵심 고려사항 | 위험 | 대책 |
|:---|:---|:---|
| 잦은 삽입의 **정렬 유지** | 원소 이동 비용이 조회 이득 초과 | 갱신 누적 후 배치 정렬 또는 균형 탐색 트리 적용 |
| 중복 키의 **경계 의미** | 임의 일치 반환으로 범위 결과 불일치 | 반열린 구간과 **하한•상한** 계약 고정 |
| 중앙 인덱스 계산 | 큰 인덱스 합산의 **정수 오버플로** | $low+(high-low)/2$ 방식 적용 |
| 탐색•정렬 비교 기준 | 서로 다른 정렬 규칙으로 누락•오탐 | 정렬과 탐색에 동일 비교 함수 적용 |
