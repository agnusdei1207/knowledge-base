...
date: "2026-07-28T12:24:06+09:00"
tags:
  - "notes-basic-theory"
weight: 57
extra:
...
| 설계 요소 | 입력·상태 | 역할 |
|:---|:---|:---|
| 전단부 | 소스 코드·심볼 정보 | 소스 분석·검증과 IR 생성 |
| 심볼 테이블 | 식별자 선언 | 타입·범위·주소 정보 저장 |
| 중간 표현 | 검증된 프로그램 구조 | 전단부·최적화기·후단부를 잇는 내부 코드 |
| 최적화기 | IR | 의미를 보존하며 연산 최적화 |
| 후단부 | 최적화 IR·타깃 정보 | 명령 선택·레지스터 할당 |
...
| JIT 초기 응답 지연 | 핫 코드 임계값·프로파일 캐시 조정 | 웜업 비용과 실행 성능 균형 |

### 쉽게 이해하기 (학습용)
- 단계별 중간 산출물과 변환 전후 실행 결과를 검증하면 오류가 발생한 전단·최적화·후단 위치를 구분할 수 있다.
...
1. 빌드 서버에서 **IR 최적화** 전후 결과가 다르면 변환 비활성화
