...
- ① **전단부**가 소스 문자를 토큰화하고 문법에 따라 **AST**를 구성한다.
- ② 전단부가 **심볼 테이블**을 참조해 타입·선언·범위의 의미를 검증한다.
- ③ 검증된 AST를 소스·기계에 중립적인 **중간 표현**으로 변환한다.
- ④ **최적화기**가 의미 동등성을 유지하며 IR의 연산·자원 사용을 줄인다.
- ⑤ **후단부**가 타깃 명령을 선택하고 레지스터를 할당해 **목적 코드**를 만든다.
...
### 쉽게 이해하기 (학습용)
- 단계별 중간 산출물과 실행 결과를 검증한다. 최적화 전후 출력이 달라지면 해당 변환 패스를 비활성화해 오컴파일을 막는다.

## Ⅵ. 결론

- 이식성 확보를 위해 **IR**로 후단부를 분리하고 **의미 동등성** 검증
