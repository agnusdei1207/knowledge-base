...
date: "2026-08-06T23:24:00+09:00"
tags:
  - "notes-basic-theory"
weight: 24
extra:
...
- **2의 보수(Two's Complement)**: 음수 $-x$를 고정 $n$비트에서 $2^n-x$에 해당하는 비트 패턴으로 나타내는 부호 표현 방식
- **가산기(Adder)**: 두 이진 입력과 캐리를 더해 합과 다음 캐리를 출력하는 회로
- **부호 정수 표현**: 고정 비트 패턴으로 양수•0•음수를 나타내는 방식

</details>

- 정의/개념: 음수 $-x$를 고정 $n$비트의 **$2^n-x$ 비트 패턴**으로 나타내는 부호 정수 표현 방식
- 배경/필요성: 부호-크기 방식은 가감산마다 **부호 분기 회로** 필요
...
- **산술 연산 통일**: 음수를 덧셈 역원으로 바꿔 덧셈과 뺄셈을 같은 가산 회로로 처리하는 성질
...
- **모듈러 연산(Modular Arithmetic)**: 결과를 $2^n$으로 나눈 나머지인 고정 $n$비트 값만 유지하는 연산
- **캐리 아웃(Carry-out)**: 최상위 비트를 넘어가 고정 폭 결과에서 제외되는 비트
...
선의 의미: 2의 보수 연산부가 포함하는 변환•연산•검출 회로
...
- **자리 가중치**: 각 비트 위치가 값에 기여하는 크기이며 2의 보수에서는 최상위 비트만 음의 가중치를 가짐
...
- **1. 비트 폭 정렬**: 부호 확장으로 피연산자 폭 통일
- **2. 연산 종류 판별**: 덧셈과 뺄셈의 입력 경로 구분
- **3. 뺄 수 보수 변환**: 비트 반전과 1 가산
- **4. n비트 모듈러 덧셈**: 캐리 아웃을 버린 합 산출
- **5. 오버플로 판정**: 입력•결과 부호로 범위 초과 확인
...
- **음수 최솟값**: 2의 보수 범위의 가장 작은 값으로 같은 비트 폭에서 양의 절댓값을 표현할 수 없는 값
...
- **형식 승격(Promotion)**: 연산 전에 피연산자를 더 넓은 비트 폭의 형식으로 변환하는 처리
