...
- **오버플로(Overflow)**: 참값이 고정 비트의 부호 정수 표현 범위를 벗어난 상태
...
- **부호 확장(Sign Extension)**: 비트 폭 확대 시 최상위 비트를 왼쪽에 복제해 값을 보존하는 방식
...
- **2의 보수(Two's Complement)**: 비트 반전 후 1을 더해 음수를 나타내는 부호 표현 방식
- **부호 확장•범위 검사**: 폭을 늘릴 때 부호 비트를 복제하고 줄일 때 값이 대상 범위에 드는지 확인하는 처리
