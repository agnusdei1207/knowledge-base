...
date: "2026-07-28T11:59:53+09:00"
tags:
  - "notes-basic-theory"
weight: 24
extra:
...
| 고려사항 | 대응 방안 | 기대 효과 |
|:---|:---|:---|
| 비트 폭이 다른 값의 변환 오류 | 확대 시 부호 확장, 축소 시 범위 검사 | 정수값 보존 |
| 캐리 아웃을 오버플로로 오판 | 부호 비트의 입력·결과 관계로 판정 | 부호 연산 정확성 확보 |
| 최솟값의 절댓값 계산 범위 초과 | 더 넓은 형식으로 승격 후 연산 | 비대칭 범위 오류 방지 |
| 직렬화 양단의 폭·부호 규칙 불일치 | 프로토콜에 비트 폭·엔디언·부호 명시 | 송수신 값 일치 |
...
> 요약: 0이 중복되지 않고 보정 단계가 없는 **2의 보수**가 현행 표준
...
### 쉽게 이해하기 (학습용)
- 송수신 측이 비트 폭과 부호 확장 규칙을 합의해야 음수값이 보존된다.


## Ⅶ. 결론

- 정수 연산 통일을 위해 비트 폭·부호 확장·오버플로를 검토하여 **2의 보수** 활용
