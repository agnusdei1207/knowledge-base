...
- 정의/개념: 음수 $-x$를 고정 $n$비트에서 $2^n-x$와 같은 비트 패턴으로 나타내어 덧셈 회로로 가감산을 처리하는 부호 정수 표현 방식이다.
...
- **자리 가중치**: 각 비트 위치가 값에 기여하는 크기이며 2의 보수에서는 최상위 비트만 음의 가중치를 가짐이다.
...
|:---|:---|:---|:---|
| 적용 기준 | 뺄셈을 **가산기**로 처리해야 하는 경우 | **순환 올림** 전제 레거시 장비 | 절댓값을 **직접 판독**해야 하는 표시 |
| 핵심 특징 | **0 표현 유일•보정 단계 불필요** | 전체 비트를 **반전**해 음수 표현 | **부호 비트•절댓값** 분리 |
| 한계 | 음수 최솟값의 **절댓값 표현 불가** | 양•음의 **0 중복** | 가감산마다 **부호 분기 회로** 필요 |
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 서로 다른 **비트 폭** 사이의 값 변환 | 확대 시 부호 확장•축소 시 **범위 검사** | **정수값** 보존 |
...
- 가감산은 **2의 보수**, 폭 변환은 부호 확장•범위 검사가 핵심이다.
