...
- **깊이 우선 탐색(Depth-First Search)**: 한 선택 경로를 끝까지 확장한 뒤 이전 분기로 돌아오는 탐색 방식
...
- **깊이 우선 탐색(Depth-First Search)**: 한 부분해 경로를 끝까지 확장한 뒤 이전 선택으로 복귀하는 방식
...
- **제약 충족 문제(Constraint Satisfaction Problem)**: 여러 제약을 모두 만족하는 변수값 조합을 찾는 문제
- **후보 생성기**: 현재 선택 단계에서 시도할 수 있는 변수값을 열거하는 구성요소
- **상태 복구**: 재귀 탐색에서 돌아온 뒤 선택으로 바뀐 부분해를 이전 상태로 환원하는 작업
- **유망성(Promising)**: 현재 부분해가 유효한 완성 해로 확장될 가능성이 있는 성질
- **가지치기(Pruning)**: 유망하지 않은 분기의 하위 탐색을 생략하는 동작
...
- **백트래킹(Backtracking)**: 제약 위반 분기를 버리고 이전 선택으로 돌아가 다른 후보를 탐색하는 방식
...
- **가지치기(Pruning)**: 완성 해로 이어질 수 없는 부분해의 하위 탐색을 생략하는 동작
...
- **백트래킹(Backtracking)**: 제약 위반 분기를 버리고 이전 선택으로 돌아가 다른 후보를 탐색하는 기법
- **제약 위반**: 현재 부분해가 문제의 필수 조건을 만족하지 못하는 상태
