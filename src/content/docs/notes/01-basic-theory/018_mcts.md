...
- 자주 이긴 수는 더 활용하고 아직 몇 번 안 둔 수에는 탐색 보너스를 주어, UCT가 익숙한 길과 새 길 사이의 탐색 기회를 조절한다.
...
- 모의 선수가 한 수만 고집하면 보상 추정이 치우치므로 롤아웃 정책과 탐색 상수를 조정하고, 여러 작업자는 가상 손실로 서로 다른 가지에 보낸다.
...
- 선택지가 많고 끝까지 모의 실행할 수 있으면 MCTS를 쓰되, 롤아웃 편향이 큰 경우에는 신뢰할 평가 함수 기반 미니맥스를 검토한다.
