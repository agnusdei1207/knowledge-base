...
- 정의/개념: 선택•확장•롤아웃•역전파를 반복하여 모의 실행의 보상을 탐색 트리에 누적하고 유망한 행동을 고르는 기법이다.
...
- **롤아웃(Rollout)**: 현재 상태부터 종료까지 모의 진행해 행동 보상을 추정하는 절차이다.
...
- **탐색(Exploration)**: 덜 방문한 행동을 시험해 불확실성을 줄이는 선택한다.
...
- **탐색 예산** 소진 시 **최다 방문 루트 행동** 반환이 핵심이다.
