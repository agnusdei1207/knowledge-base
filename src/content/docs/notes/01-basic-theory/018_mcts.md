...
| 핵심 고려사항 | 위험 | 대책 |
|:---|:---|:---|
| 롤아웃 정책 편향 | 특정 행동의 보상 추정이 지속 왜곡 | 기준 정책 검증과 가치 모델 교차 평가 |
| **탐색 상수** | 과도한 탐색 또는 기존 고보상 경로 고착 | 도메인•예산별 UCT 상수 조정 |
| 저방문 행동 분산 | 우연한 고보상 표본으로 행동 오선택 | 평균 보상과 방문 횟수 동시 검증 |
| 병렬 경로 집중 | 작업자가 같은 노드를 중복 탐색 | 가상 손실과 노드 통계 원자적 갱신 |
