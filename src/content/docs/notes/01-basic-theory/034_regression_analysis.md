...
| 잔차 진단기 | **등분산성**·**VIF** 기반 가정 위반 판정 |
| 성능 평가기 | 검증 자료 **RMSE**·**MAE** 산출 |

> 요약: 모형 **예측**과 **잔차 진단**·오차 측정의 책임 분담
...
| 한계 | **다중공선성**·외삽 시 오차 급증 | **비선형 관계** 탐지 불가 |
...
2. 지연 예측 모형의 **다중공선성**에 **릿지** 벌점 적용

### 쉽게 이해하기 (학습용)

- 분당 요청 수와 CPU 사용률의 관계를 직선으로 잡아 두면 요청이 늘어나는 순간 사용률이 한계선에 닿을 시점을 미리 계산할 수 있어, 서버가 버거워진 뒤가 아니라 그 전에 대수를 늘리게 된다.
- 응답 지연을 예측할 때 큐 길이와 대기 요청 수처럼 사실상 같이 움직이는 지표를 함께 넣으면 어느 쪽 몫인지 갈리지 않아 계수가 표본마다 요동치는데, 릿지가 큰 계수에 벌점을 물려 그 요동을 눌러 준다.

## Ⅶ. 결론

- **잔차 패턴** 발생·**입력 분포 변화** 시 회귀 모형 재학습
