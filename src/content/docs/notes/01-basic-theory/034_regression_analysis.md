...
- **선형성•독립성**: 입력과 조건부 평균의 관계가 선형이고 잔차들이 서로 영향을 주지 않는다는 회귀 가정
...
- **잔차(Residual)**: 관측값에서 회귀 모형의 예측값을 뺀 차이
...
- **회귀 계수**: 다른 입력이 같을 때 입력 한 단위 변화에 따른 예측값 변화량
- **VIF**: 입력 변수 간 연관성이 회귀 계수의 분산을 키운 정도를 나타내는 지표
- **RMSE•MAE**: 제곱 오차와 절댓값 오차를 평균해 예측 성능을 측정하는 지표
...
- **재학습(Retraining)**: 새 데이터 분포에 맞춰 회귀 모형의 계수를 다시 추정하는 작업
