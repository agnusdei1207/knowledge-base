...
date: "2026-08-06T23:34:00+09:00"
tags:
  - "notes-basic-theory"
weight: 34
extra:
...
- **회귀 분석(Regression Analysis)**: 입력과 연속형 결과의 조건부 관계를 모형화해 영향력을 추정하고 값을 예측하는 기법
- **독립 변수(Independent Variable)**: 결과를 설명하는 입력 변수
- **종속 변수(Dependent Variable)**: 예측하려는 결과 변수
- **조건부 관계**: 다른 입력을 고정했을 때 특정 입력과 결과 사이에 남는 관계
- **변수별 영향력**: 다른 조건이 같을 때 각 입력의 변화가 예측값에 미치는 크기와 방향

</details>

- 정의/개념: 입력과 연속형 결과의 조건부 관계를 추정하는 **회귀 분석**
- 배경/필요성: 관측값 나열만으로는 새 입력의 예측값과 **변수별 영향력** 산출 불가
...
- **선형성(Linearity)**: 입력과 조건부 평균의 관계가 선형이라는 가정
- **독립성(Independence)**: 잔차들이 서로 영향을 주지 않는다는 가정

</details>

- 독립•종속 변수 사이의 **조건부 관계**를 함수로 모형화
...
- **분산 팽창 계수(Variance Inflation Factor, VIF)**: 입력 변수 간 연관성이 회귀 계수 분산을 키운 정도
- **평균 제곱근 오차(Root Mean Square Error, RMSE)**: 큰 오차에 민감한 제곱 기반 예측 오차 지표
- **평균 절대 오차(Mean Absolute Error, MAE)**: 이상치 영향이 작은 절댓값 기반 예측 오차 지표
- **회귀식**: 입력 변수와 회귀 계수로 결과의 조건부 평균을 계산하는 식
...
- **최소제곱법(Ordinary Least Squares, OLS)**: 잔차 제곱합이 최소인 회귀 계수를 추정하는 방법
...
- **1. 학습•검증 자료 분리**: 적합과 일반화 평가 자료 구분
- **2. OLS 모형 적합**: 최소제곱 회귀 계수 산출
- **3. 잔차•VIF 진단**: 회귀 가정과 다중공선성 판정
- **4. 검증 자료 예측**: 미사용 입력의 예측값 산출
- **5. 일반화 오차 평가**: RMSE•MAE로 예측 성능 판단
...
- **변수 변환•비선형항**: 입력의 척도를 바꾸거나 제곱•상호작용 항을 추가해 비선형 관계를 표현하는 방법
- **예측구간(Prediction Interval)**: 특정 입력에서 새 관측값이 놓일 것으로 예상하는 구간
- **재학습(Retraining)**: 새 운영 데이터로 회귀 계수와 모형을 다시 추정하는 작업
- **중앙 처리 장치(Central Processing Unit, CPU)**: 서버의 명령 실행과 연산을 담당하는 처리 장치
...
- **잔차 가정**: 잔차의 선형성•독립성•등분산성 등 회귀 추론에 필요한 조건
