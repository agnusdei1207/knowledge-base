...
date: "2026-07-29T12:03:54+09:00"
tags:
  - "notes-basic-theory"
weight: 34
extra:
...
- 정의/개념: 입력과 결과의 **조건부 관계**로 연속값 예측
- 배경/필요성: 미관측 값 예측•**변수별 영향력** 추정
...
- **1. 계수•잔차 전달**: 최소제곱 계수와 오차 산출
- **2. 가정 진단 결과 반환**: 잔차•VIF 조건 판정
- **3. 검증 예측값 전달**: 미사용 입력의 예측값 제공
- **4. 일반화 오차 반환**: RMSE•MAE로 성능 판단
...
flowchart TB
    R{분석 목적•검증 결과}
    R -->|연속값 예측•가정 충족| G[회귀 분석]
    R -->|연관 방향•강도 확인| C[상관 분석]
    R -->|외삽•분포 변화 감지| S[예측 중단•재학습]
```
