...
| 계열 | 수식•의미 |
|:---|:---|
| <span style="color:#3080dd">■</span> 파란색 선언 | **균형 트리 $\log_2 n$**: 높이 비례 문맥 |
| <span style="color:#d95926">■</span> 주황색 선언 | **편향 트리 $n$**: 노드 수 비례 문맥 |

> **편향 트리**: 노드 수만큼 복귀 문맥 누적
...
- **1. 시작 문맥 저장 요청**: 루트와 순회 방식 보관
- **2. 다음 문맥 요청**: 다음 노드•방문 단계 복원
- **3. 노드 정보 요청**: 값과 좌•우 참조 획득
- **4. 후속 문맥 저장 요청**: 순회 순서대로 작업 배치
...
flowchart TB
    R{노드 처리 목적}
    R -->|부모부터 구조 기록| P[전위 순회]
    R -->|BST 키 순서 출력| I[중위 순회]
    R -->|자식 결과부터 집계| O[후위 순회]
```
