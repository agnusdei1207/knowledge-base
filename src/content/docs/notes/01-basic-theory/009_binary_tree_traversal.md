...
date: "2026-07-28T11:48:22+09:00"
tags:
  - "notes-basic-theory"
weight: 9
extra:
...
| 고려사항 | 대응 방안 | 기대 효과 |
|:---|:---|:---|
| 편향 트리의 깊은 재귀 | 명시적 스택 기반 반복 순회 적용 | 스택 오버플로 방지 |
| 순회 중 구조 변경 | 스냅샷·버전 고정 또는 변경 금지 | 누락·중복 방문 방지 |
| 직렬화 시 빈 자식 구분 불가 | 널 표식과 구분자 규칙 정의 | 동일한 트리 구조 복원 |
| 중위 순회의 정렬 결과 오해 | 이진 탐색 트리 순서 조건 검증 | 잘못된 정렬 해석 방지 |

### 쉽게 이해하기 (학습용)

- 재귀 깊이와 순회 중 변경을 통제하고 결과 해석의 전제 조건을 확인한다.
...
- 이진 탐색 트리를 중위 순회하면 주문 번호를 오름차순으로 추출할 수 있다.

## Ⅶ. 결론

- 트리 처리를 위해 출력 순서를 검토하여 **순회 방식** 활용
