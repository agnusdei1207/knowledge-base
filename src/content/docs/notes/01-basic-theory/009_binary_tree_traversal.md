...
- **이진 탐색 트리(Binary Search Tree, BST, 비에스티)**: 영문 각 단어의 첫 글자를 따 BST로 표기하며 왼쪽 키는 작고 오른쪽 키는 큰 이진 트리
...
- **재귀 깊이**가 높이에 비례해 편향 시 **스택 오버플로**

### 쉽게 이해하기 (학습용)

- 답안이 스택 부담을 따로 세운 이유는, 세 순회 모두 갈림길을 딱 한 번씩만 밟아 걸린 시간은 노드 수에 그대로 비례하고 표지판 읽는 시점만 다를 뿐이지만, 길이 한쪽으로만 이어진 나무에서는 되돌아갈 자리를 적은 메모가 노드 수만큼 쌓여 시간이 아니라 주머니가 먼저 터지기 때문이다.
...
| 절차 | 설명 |
|:---|:---|
| 복귀 문맥 저장 | 하강 전 현재 노드·**재개 지점** 적재 |
| 하위 트리 하강 | **좌·우 자식 참조**로 한 단계 내려감 |
| 루트 방문 | 유형별 **지정 시점**에 노드 데이터 처리 |
| 복귀 문맥 복원 | 하위 트리 소진 시 **상위 노드**로 복귀 |
...
| 한계 | 널 표식 누락 시 **트리 복원** 불가 | 일반 이진 트리는 **정렬 미보장** | 자식 결과 대기로 **조기 종료** 불가 |
