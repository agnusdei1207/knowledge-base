...
date: "2026-08-06T23:09:00+09:00"
tags:
  - "notes-basic-theory"
weight: 9
extra:
...
- 정의/개념: 이진 트리의 모든 노드를 루트•**좌우 하위 트리 순서**로 처리하는 절차
- 배경/필요성: 직렬화•정렬•집계 목적마다 **루트 방문 시점** 선택 필요
...
- **중위 순회(In-order Traversal)**: 두 하위 트리 사이에 루트를 방문하는 순회
- **후위 순회(Post-order Traversal)**: 두 하위 트리보다 나중에 루트를 방문하는 순회
...
가지의 의미: 방문 제어•노드•복귀 문맥의 정적 책임 분리
...
- **이진 탐색 트리(Binary Search Tree, BST)**: 왼쪽 키는 작고 오른쪽 키는 큰 순서 조건을 가진 트리
...
- **구조 기록**: 부모를 먼저 방문해야 하는 순회 목적
- **키 정렬**: 키 순서대로 노드를 방문해야 하는 순회 목적
- **하위 집계**: 자식 결과를 먼저 모아야 하는 순회 목적
