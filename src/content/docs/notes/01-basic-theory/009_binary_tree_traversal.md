...
- **① 루트 처리 문맥 저장**: 순회를 시작할 루트와 방식 보관
- **② 다음 처리 문맥**: 스택에서 다음 노드·방문 단계 복원
- **③ 자식·데이터 조회**: 현재 노드의 값과 좌·우 참조 획득
- **④ 순서별 문맥 저장**: 전위·중위·후위에 맞춰 자식·방문 순서 배치
...
- 한쪽으로 긴 가지는 명시적 스택으로 넘침을 막고, 순회 중 트리를 사진처럼 고정해 노드 누락·중복 없이 같은 구조를 복원한다.
