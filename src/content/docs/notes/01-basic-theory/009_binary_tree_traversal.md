...
date: "2026-07-28T14:56:42+09:00"
tags:
  - "notes-basic-theory"
weight: 9
extra:
...
- 같은 트리를 돌아도 루트를 처리하는 시점에 따라 직렬화·정렬·집계 결과가 달라진다.
...
| 구성요소 | 책임 | 흐름상 역할 |
|:---|:---|:---|
| 순회기 | **방문 순서**대로 하강·방문·복귀 | ① 요청 → ② 저장 → ③ 조회 → ④ 처리 → ⑤ 복귀 → ⑥ 반환 |
| 트리 노드 | 데이터와 **좌·우 자식 참조** 보관 | ③ 조회 수신 → ④ 참조·데이터 반환 |
| 순회 스택 | **복귀 문맥**과 미방문 하위 트리 보관 | ② 문맥 저장 → ⑤ 문맥 반환 |

> 요약: **순회기**가 자식 참조로 하강하고 스택에 **재개 지점** 보관

### 쉽게 이해하기 (학습용)

- 순회기는 노드의 자식 참조를 따라가고 스택에 복귀 문맥을 보관한다.
...
- 직렬화는 전위, 이진 탐색 트리 정렬은 중위, 하위 결과 집계는 후위를 선택한다.

## Ⅴ. 실무 고려사항 및 대책

| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| **편향 트리**의 깊은 재귀 | 명시적 스택 기반 **반복 순회** | **스택 오버플로** 방지 |
| 순회 중 **트리 구조** 변경 | 순회 시작 시 **스냅샷** 고정 | 누락·중복 방문 방지 |
| 직렬화 시 **빈 자식** 구분 불가 | **널 표식**·구분자 규칙 정의 | 동일한 **트리 구조** 복원 |
| 중위 순회의 **정렬 결과** 오해 | **이진 탐색 트리** 조건 검증 | 잘못된 정렬 해석 방지 |

### 쉽게 이해하기 (학습용)

- 재귀 깊이와 순회 중 변경을 통제하고 직렬화·정렬 결과의 전제 조건을 확인한다.

## Ⅵ. 결론

- 직렬화·정렬·하위 집계 목적에 맞춰 **순회 순서** 결정

### 쉽게 이해하기 (학습용)

- 필요한 출력 순서와 하위 결과 의존성에 따라 순회 방식을 선택한다.
