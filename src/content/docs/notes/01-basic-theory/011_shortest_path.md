...
- 정의/개념: 출발점에서 목적점까지 이동하는 경로 중 간선 가중치의 합이 가장 작은 경로를 찾는 그래프 문제이다.
...
- **음수 사이클**: 반복할수록 경로 비용이 계속 감소해 최단 거리를 정의할 수 없는 순환이다.
...
- **전 쌍 최단 경로**: 그래프의 모든 출발점과 목적지 조합에 대한 최소 비용 경로이다.

</details>

- 비음수 SSSP는 **다익스트라**, 음수 SSSP는 **벨만-포드**, APSP는 플로이드-워셜을 선택한다.
