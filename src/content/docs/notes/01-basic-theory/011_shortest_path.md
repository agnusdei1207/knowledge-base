...
date: "2026-07-28T11:49:50+09:00"
tags:
  - "notes-basic-theory"
weight: 11
extra:
...
| 음수 사이클의 영향 범위 누락 | 추가 완화 후 도달 가능한 정점 전파 | 유한 최단 거리 오판 방지 |
| 큰 그래프의 시간·메모리 급증 | 출발점 범위와 희소성에 맞는 알고리즘 선택 | 계산 자원 절감 |
| 도달 불가와 큰 거리값 혼동 | 무한대 표식·오버플로 안전 연산 사용 | 결과 상태 정확성 확보 |

### 쉽게 이해하기 (학습용)
- 가중치 범위와 계산 범위를 검증하고 음수 사이클·도달 불가 상태를 분리한다.
...
### 쉽게 이해하기 (학습용)
- 응답 지연이 비음수인 호출 그래프에서 다익스트라로 최소 지연 경로를 계산한다.


## Ⅶ. 결론

- 가중치 부호와 계산 범위를 검토하여 **최단 경로 알고리즘** 활용
