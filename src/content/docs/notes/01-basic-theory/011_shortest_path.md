...
1. **경로 요청**: 출발점•목적지와 가중치 조건 설정
2. **간선 정보 요청**: 완화 대상의 끝점•가중치 확보
3. **기존 거리 요청**: 신규 경로 비용과 현재 후보 비교
4. **거리•선행 정점 갱신**: 더 짧은 비용과 직전 정점 기록
5. **최단 경로 반환**: 거리 수렴 후 선행 정점 역추적
...
| 핵심 고려사항 | 위험 | 대책 |
|:---|:---|:---|
| 음수 간선 존재 | 다익스트라의 조기 거리 확정 오류 | 실행 전 가중치 범위 검증 후 벨만-포드 선택 |
| **음수 사이클** | 영향 정점의 유한 최단 거리 오판 | 추가 완화로 탐지하고 도달 정점까지 영향 전파 |
| 그래프 범위•희소성 | 불필요한 전 쌍 계산으로 자원 급증 | SSSP•APSP와 희소성에 맞는 알고리즘 선택 |
| 도달 불가 거리 | 큰 정상 거리와 무한대 표식 혼동 | 별도 상태값과 오버플로 안전 덧셈 적용 |
