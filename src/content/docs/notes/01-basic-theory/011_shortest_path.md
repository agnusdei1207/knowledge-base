...
date: "2026-07-28T22:23:00+09:00"
tags:
  - "notes-basic-theory"
weight: 11
extra:
...
- 지도에 적힌 임시 요금보다 싼 우회로를 찾을 때마다 가격표를 낮추는 완화를 반복하면, 최단 경로의 각 구간도 가장 싼 구간으로 남는다.
...
- **① 간선 조회**: 현재 완화 대상의 끝점·가중치 확보
- **② 신규 거리 후보**: 현재 거리와 간선 가중치 합산
- **③ 거리값 갱신**: 새 경로가 짧을 때 최소 후보 교체
- **④ 선행 정점 갱신**: 거리 개선 경로의 직전 정점 기록
...
- 할인 없는 도로의 한 출발점은 가까운 곳부터 확정하는 다익스트라, 음수 할인 도로는 전체를 반복 확인하는 벨만-포드, 모든 도시 쌍은 플로이드-워셜이 맞는다.
