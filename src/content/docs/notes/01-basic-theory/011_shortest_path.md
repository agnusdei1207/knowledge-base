...
가지의 의미: 계산 제어•입력•거리•경로 상태의 정적 책임 분리
...
- **문제 범위**: 단일 출발점 또는 모든 정점 쌍으로 나눈 계산 대상
- **음수 사이클**: 반복할수록 경로 비용이 계속 감소하는 순환
...
가지의 의미: 문제 범위와 음수 가중치에 따른 결정 분기
...
- **단일 출발점 최단 경로(Single-Source Shortest Path, SSSP)**: 하나의 출발점에서 모든 정점까지 최단 거리를 구하는 문제
- **모든 쌍 최단 경로(All-Pairs Shortest Path, APSP)**: 모든 정점 쌍 사이의 최단 거리를 구하는 문제
- **다익스트라(Dijkstra)**: 비음수 간선에서 최소 거리 정점을 확정하며 SSSP를 구하는 알고리즘
- **벨만-포드(Bellman-Ford)**: 전체 간선을 반복 완화해 음수 SSSP를 구하는 알고리즘
- **플로이드-워셜(Floyd-Warshall)**: 경유 정점을 늘려 APSP를 구하는 알고리즘
- **비음수 간선(Non-Negative Edge)**: 가중치가 0 이상인 간선
- **음수 간선(Negative Edge)**: 가중치가 0보다 작은 간선
