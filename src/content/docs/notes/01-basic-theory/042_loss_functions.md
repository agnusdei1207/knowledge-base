...
- ① **모델**이 배치 표본별 **예측값**을 산출한다.
- ② **손실 함수**가 정답값과 예측값을 대조해 **표본 손실**을 계산한다.
- ③ **축소 연산기**가 표본 손실을 합계·평균의 **배치 손실**로 축소한다.
- ④ 손실 함수가 배치 손실을 미분해 **손실 기울기**를 산출한다.
- ⑤ 손실 함수가 **기울기**를 모델에 전달해 **파라미터 갱신**을 유도한다.
