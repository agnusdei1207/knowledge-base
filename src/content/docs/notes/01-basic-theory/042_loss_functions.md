...
- **역전파(Backpropagation)**: 손실 기울기를 출력층에서 입력층 방향으로 전달해 각 파라미터의 기울기를 계산하는 절차다.
...
- **클래스 가중치(Class Weight)**: 희소 클래스의 손실 비중을 높여 다수 클래스 편중을 완화하는 계수다.
...
- **대리 손실(Surrogate Loss)**: 직접 미분하기 어려운 평가 지표 대신 같은 개선 방향을 유도하도록 설계한 학습 목적이다.
- **라벨 스무딩(Label Smoothing)**: 정답 확률을 1보다 조금 낮춰 잘못된 라벨과 과도한 확신의 영향을 줄이는 기법이다.
