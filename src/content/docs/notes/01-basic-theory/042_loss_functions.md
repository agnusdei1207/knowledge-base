...
- **손실 함수(Loss Function / Cost Function)**: 모델의 예측값($\hat{y}$)과 실제 정답값($y$) 간의 오차를 단일 스칼라 비용으로 정량화하여 역전파 시 파라미터 최적화의 방향과 크기를 결정하는 수학적 목적 함수.
- **교차 엔트로피(Cross-Entropy Loss, CE)**: 정답 확률분포 $P$와 모델의 예측 확률분포 $Q$ 간의 정보 엔트로피 차이($-\sum y_i \log \hat{y}_i$)를 측정하는 확률 분류 손실 함수.
- **평균 제곱 오차(MSE, Mean Squared Error)**: 예측값과 실제값 간의 오차를 제곱하여 평균한($\frac{1}{N}\sum (y_i - \hat{y}_i)^2$) 연속형 수치 회귀 손실 함수.

</details>

- 정의/개념: 예측 오차를 미분 가능한 스칼라 비용으로 변환하여 역전파(Backpropagation) 기울기($\nabla_\theta L$)를 제공하는 **지도학습 최적화 목적 함수 체계**
- 배경/필요성: 분류 정확도(Accuracy) 같은 비미분 평가지표는 역전파 계산이 불가하므로 **최대우도추정(MLE) 기반의 미분 가능한 대리 목적 함수(Surrogate Objective) 필수**
...
- 확률 분류 과업에서 **Sigmoid/Softmax 활성화 함수와 결합 시 깔끔한 선형 기울기($\hat{y} - y$) 제공**
- 회귀 과업에서 **정규분포 오차 가정 하의 MSE 최적화 및 라플라스 오차 가정 하의 MAE 최적화**
...
| 역전파 오차 생성기 | 손실 함수 미분을 통해 **최종 출력단 오차 신호($\frac{\partial L}{\partial z}$) 역전송** |
...
1. **과업 유형 분류**: 분류 문제인지 연속 수치 회귀 문제인지 판별
2. **분류 손실 선정**: 확률 분포 학습을 위해 Cross-Entropy를 기본 채택하고, 극단 불균형 시 Focal Loss 적용
3. **회귀 손실 선정**: 이상치 영향이 적으면 MSE, 이상치 왜곡 방지가 필요하면 Huber Loss 적용
4. **역전파 수행**: 산출된 손실의 편미분을 통해 가중치를 갱신
...
| 비교 항목 | 교차 엔트로피 (Cross-Entropy) | 평균 제곱 오차 (MSE / L2) | 허버 손실 (Huber / Smooth L1) |
|:---|:---|:---|:---|
| 주요 과업 | **이진/다중 분류 (Classification)** | **연속형 수치 회귀 (Regression)** | **이상치 포함 수치 회귀** |
| 수식 | $L = -\sum y_i \log(\hat{y}_i)$ | $L = \frac{1}{N}\sum (y_i - \hat{y}_i)^2$ | $|e| \le \delta: \frac{1}{2}e^2$, $|e| > \delta: \delta|e| - \frac{1}{2}\delta^2$ |
| 확률적 해석 | **다항분포 음의 로그 우도(NLL)** | **가우시안 정규분포 MLE** | 가우시안 + 라플라스 복합 분포 |
| 이상치 민감도 | 잘못된 고확신 예측에 극단적 페널티 | **제곱 오차로 이상치에 극도로 민감** | **선형 페널티로 이상치에 강건(Robust)** |
