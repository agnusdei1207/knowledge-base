...
- **손실 함수(Loss Function)**: 예측과 정답의 차이를 최적화할 하나의 비용값으로 바꾸는 함수
...
- 정의/개념: 예측과 정답의 차이를 스칼라 비용으로 바꾸는 **손실 함수**
...
- **손실 형태**가 오차 크기별 벌점 증가율 결정
- **평가 지표•손실**이 어긋나면 학습 목표 왜곡
...
- **예측값 $\hat y$**: 모델이 산출한 추정 결과
- **정답값 $y$**: 예측값과 비교할 학습 목표
...
- **확률 분류**: 클래스별 확률을 예측하는 과업
- **이상치 민감도**: 극단 오차가 학습 방향에 미치는 영향 정도
...
- **1. 과업 유형 판별**: 분류 확률과 연속값 회귀 구분
- **2. 교차 엔트로피 선택**: 정답 클래스의 낮은 확률 강조
- **3. 이상치 영향 판별**: 극단 오차의 기울기 지배 여부 확인
- **4. MSE 선택**: 잡음이 작을 때 큰 오차를 제곱 강조
- **5. 허버 손실 선택**: 이상치 구간을 선형 벌점으로 완화
...
- **평균 제곱 오차(Mean Squared Error, MSE)**: 예측 오차를 제곱해 평균하는 회귀 손실
...
> 요약: MSE의 이상치 지배를 **허버 손실**의 선형 벌점으로 완화
