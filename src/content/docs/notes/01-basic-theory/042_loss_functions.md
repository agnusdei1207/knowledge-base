...
date: "2026-07-25T20:10:00+09:00"
tags:
  - "notes-basic-theory"
weight: 42
extra:
...
- **기존 한계**: 오차를 수치로 모으지 않으면 **갱신 방향** 산출 불가
...
> 요약: MSE의 이상치 지배를 **허버 손실**이 선형 완화로 보완
...
2. 희귀 이벤트 분류에 **포컬 손실**로 **오분류 표본** 강조

### 쉽게 이해하기 (학습용)
- 응답 지연은 평소 수십 밀리초이다가 가비지 컬렉션 한 번에 수 초로 튀는데, 제곱 감점을 그대로 쓰면 그 몇 건이 전체 학습을 끌고 가 평상시 구간 예측이 망가지므로 큰 오차의 벌점 증가를 선형으로 눌러 둔다.
- 침입 시도처럼 만 건에 한 건 나오는 이벤트는 전부 정상이라고 찍기만 해도 감점이 거의 없어 모델이 그대로 굳는데, 이미 잘 맞히는 정상 표본의 감점 비중을 낮춰 두면 남은 감점이 희귀 사례에 집중돼 그쪽을 학습하게 된다.
