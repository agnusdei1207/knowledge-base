...
- ① **특징 추출기**가 입력 이미지에서 다중 해상도의 **특징 피라미드**를 만든다.
- ② **탐지 헤드**가 위치 좌표·클래스·**객체성**을 후보 박스별로 예측한다.
- ③ **NMS 후처리**가 점수와 IoU를 기준으로 **중복 박스**를 제거한다.
- ④ 남은 박스의 **위치·클래스**를 최종 **탐지 결과**로 반환한다.
