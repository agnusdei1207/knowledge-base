...
| 설계 요소 | 입력·상태 | 역할 |
|:---|:---|:---|
| 특징 추출기 | 입력 이미지 | **백본·넥**으로 다중 해상도 특징 생성 |
| 탐지 헤드 | 특징 피라미드 | 박스 위치·클래스·**객체성** 예측 |
| NMS 후처리 | 후보 박스·점수 | 점수 미달·**중복 박스** 제거 |
...
| 동일 객체의 중복 박스 잔존 | 점수·IoU 임계값을 검증 데이터로 조정 | 중복 탐지 감소 |
| 배경 비중이 커 객체 학습 약화 | 포컬 손실·표본 균형화 적용 | 클래스 불균형 완화 |
| 엣지 기기의 지연·메모리 초과 | 경량 백본·양자화·입력 크기 조정 | 실시간 처리와 자원 한도 충족 |

### 쉽게 이해하기 (학습용)
- 객체 크기별 재현율과 지연을 함께 측정한다. 보안 영상의 소형 객체는 해상도가 살아 있는 특징 피라미드 층을 함께 사용해 미탐을 줄인다.
