...
| 계열 | 수식·의미 |
|:---|:---|
| 파랑 | 근사해 품질: $1/\varepsilon$ 증가에 따라 최적해에 접근 |
| 빨강 | 계산 비용: 더 작은 $\varepsilon$을 요구할수록 급증 |
...
sequenceDiagram
    ...
    M->>B: 4. 최적해 하한 제공
    B->>O: 5. 근사비·품질 경계 전달
    O-->>M: 6. 채택·정확 해법 전환 결정
```

| 설계 요소 | 입력·상태 | 역할 |
|:---|:---|:---|
| 문제 모델 | 목적 함수·제약 조건 | 최적화 목표와 실행 가능 영역 정의 |
| 해 구성기 | 문제 모델 | 탐욕법·완화·라운딩으로 후보 해 산출 |
| 실행 가능성 검사 | 후보 해 | 원래 제약 충족 판정 |
| 근사비 분석 | 근사값·최적해 하한 | 최악 품질 경계 산출 |
...
| 이론 보장보다 실제 해가 지속적으로 나쁨 | 대표·최악 입력 벤치마크와 하한 강화 | 품질 경계의 실효성 검증 |
