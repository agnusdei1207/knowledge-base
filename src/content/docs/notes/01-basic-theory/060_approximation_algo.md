...
- **최적해 경계(Bound of OPT)**: 최적값을 직접 구하지 않고 최소화 문제에서는 최적값의 하한, 최대화 문제에서는 최적값의 상한을 구해 근사비 증명의 비교 기준으로 사용하는 값
...
| 근사비 분석 | 근사값·최적해 하한·상한으로 **최악 품질 경계** 산출 |

$$\text{최소화}: \mathrm{ALG}/\mathrm{OPT}\le\rho,\quad \text{최대화}: \mathrm{OPT}/\mathrm{ALG}\le\rho$$

**동작 원리**

- **① 문제 모델 정의**: 최소화·최대화 목적과 제약 조건 설정
- **② 근사 후보 구성**: 탐욕법·완화·라운딩으로 후보 해 산출
- **③ 실행 가능성 검사**: 후보가 원래 문제의 모든 제약을 충족하는지 검증
- **④ 최적해 경계 계산**: 문제 구조·완화해로 최소화 문제의 하한 또는 최대화 문제의 상한 산출
- **⑤ 근사비 증명**: 근사값과 최적해 경계를 비교해 최악의 품질 비율 확정
- **⑥ 운영 채택 판단**: 증명한 근사비·실행 시간과 업무 허용 오차에 따라 근사·정확 해법 선택

### 쉽게 이해하기 (학습용)
- 해 구성기가 후보를 뽑으면 원래 제약을 지키는지 먼저 확인하고, 최적해를 직접 구하지 않은 채 최소화는 하한·최대화는 상한을 잡아 근사값과 비교하면 최악의 품질 경계를 증명할 수 있다
...
### 쉽게 이해하기 (학습용)
- 네트워크 링크 감시에서 실행 가능성·근사비·실행 시간을 함께 측정하고 2-근사 정점 덮개로 최적 배치의 두 배 이하 노드를 선택하면서 모든 링크를 포함한다.
