...
- 정의/개념: 최적해를 구하기 어려운 최적화 문제에서 다항 시간에 실행되며 반환 해가 최적해에서 얼마나 벗어날지 근사비로 보장하는 알고리즘이다.
...
- **다항 시간** 안에 실행 가능 해 도출한다.
- 최적해 대비 **근사비**로 최악 입력 품질 보장이 핵심이다.
- 허용 오차 축소 시 **PTAS•FPTAS**의 계산 비용 증가한다.
...
- **1. 목적 함수•제약 정의**: 최적화 목표와 허용 해 범위 설정한다.
- **2. 근사 후보 해 구성**: 탐욕법•완화•라운딩을 적용한다.
- **3. 실행 가능성 검사**: 제약 조건을 만족하는 해 검증한다.
- **4. 최적해 경계 산출**: 근사비 비교용 하한•상한을 구성한다.
- **5. 근사비 증명**: 최악 성능 보장을 도출한다.
...
- **해법 전환**: 입력 규모와 품질 요구에 따라 근사 방식에서 정확 방식으로 바꾸는 결정한다.

</details>

- 증명된 **품질 경계**가 업무 허용선을 충족하지 못하면 더 엄격한 근사 스킴이나 정확 알고리즘을 선택한다.
