...
date: "2026-07-26T00:50:00+09:00"
tags:
  - "notes-basic-theory"
weight: 41
extra:
...
- **기존 한계**: 고차원 손실면은 **폐쇄형 해**가 없어 최소점 직접 산출 불가

### 쉽게 이해하기 (학습용)
- 답안이 굳이 `폐쇄형 해가 없어서`라고 원인을 밝히는 이유는, 산 전체를 수학 공식 한 줄로 풀어 바닥 좌표를 바로 구할 수 있다면 애초에 한 걸음씩 내려갈 필요가 없기 때문인데, 딥러닝처럼 변수가 수십억 개인 산에는 그런 공식이 존재하지 않아서 발밑 경사를 재고 한 걸음 딛고 다시 재는 반복만이 유일한 길이 된다.

## Ⅱ. 특징

- **비볼록** 손실면에서 1차 근사로 초기값별 **지역 최적해** 고착
...
> 요약: 루프 공급·모델 **기울기**·최적화기 **갱신**의 책임 분담
...
| 한계 | **학습률** 수동 조정·수렴 지연 | 학습률 **단조 감소**에 따른 학습 정지 | 상태 2배·**일반화 성능** 저하 |
...
2. 광고 클릭 로그의 **희소 특징** 학습에 **AdaGrad** 적용

### 쉽게 이해하기 (학습용)
- 여러 장비에 모델을 나눠 학습시킬 때 Adam은 파라미터마다 방향 수첩과 크기 수첩을 하나씩 더 들고 다니므로, 장비 메모리를 계산할 때 모델 크기만 보면 실제로는 세 배 가까이 필요해 학습이 중간에 멈추게 된다.
- 광고 클릭 기록은 몇몇 인기 항목만 수없이 등장하고 나머지 대부분은 어쩌다 한 번 나타나는데, AdaGrad는 자주 나온 항목의 보폭만 골라 줄이므로 드물게 나오는 항목이 큰 보폭을 유지한 채 제대로 학습된다.
