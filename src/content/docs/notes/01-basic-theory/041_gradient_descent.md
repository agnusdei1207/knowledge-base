...
- **미니배치**: 전체 데이터에서 한 번의 기울기 추정에 사용하는 표본 묶음
- **순전파•역전파**: 예측과 손실을 계산하고 파라미터별 손실 기울기를 거꾸로 계산하는 과정
...
- **모멘트**: 기울기의 평균과 제곱 평균으로 갱신 방향과 크기를 보정하는 통계량
- **누적 제곱 기울기**: 좌표별 과거 기울기 제곱을 더해 적응적 보폭을 정하는 상태
- **학습률(Learning Rate)**: 한 번의 갱신에서 기울기를 반영할 이동 폭
- **미니배치(Mini-batch)**: 한 번의 손실과 기울기 계산에 사용하는 표본 묶음
...
- **SGD**: 무작위 미니배치의 기울기로 파라미터를 직접 갱신하는 방법
- **AdaGrad**: 누적 제곱 기울기로 자주 갱신된 파라미터의 보폭을 줄이는 방법
- **Adam**: 기울기의 1•2차 모멘트 이동평균으로 파라미터별 방향과 보폭을 조정하는 방법
- **희소 특징**: 대부분 표본에서 0이고 일부 표본에서만 값이 나타나는 입력 특성
- **EMA(Exponential Moving Average)**: 최근 값에 더 큰 가중치를 두어 모멘트를 누적하는 지수 이동평균
...
- **상태 메모리**: 파라미터 외에 모멘트나 누적 기울기를 저장하는 데 필요한 장치 메모리
- **초기 수렴**: 학습 초기에 손실이 빠르게 감소해 유용한 성능에 도달하는 속성
- **SGD•AdaGrad•Adam**: 직접 기울기, 누적 제곱 기울기, 1•2차 모멘트로 파라미터를 갱신하는 최적화기
