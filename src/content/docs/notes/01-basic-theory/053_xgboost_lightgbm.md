...
- **익스트림 그래디언트 부스팅(eXtreme Gradient Boosting, XGBoost)**: 레벨 단위 성장과 정규화를 적용한 부스팅 구현
- **경량 그래디언트 부스팅 머신(Light Gradient Boosting Machine, LightGBM)**: 리프 단위 성장으로 대규모 학습을 최적화한 구현

</details>

- 정의/개념: **XGBoost•LightGBM**은 그래디언트 부스팅의 **최적화 구현**
- 배경/필요성: 반복 분할 탐색•순차 트리 누적으로 대규모 정형 데이터의 **학습 비용 증가**
...
- **그래디언트(Gradient)**: 손실 변화 방향을 나타내는 1차 미분값
- **헤시안(Hessian)**: 손실 곡률을 나타내는 2차 미분값
...
- **1. 현재 예측 산출**: 누적 트리의 학습 표본 예측 계산
- **2. 그래디언트•헤시안 계산**: 손실의 방향과 곡률 산출
- **3. 보정 트리 생성**: 최대 분할 이득으로 트리 구성
- **4. 학습률로 트리 누적**: 보정 예측을 앙상블에 반영
- **5. 검증 성능 평가**: 개선 정체 여부로 조기 종료
...
- **그래디언트 기반 단측 샘플링(Gradient-based One-Side Sampling, GOSS)**: 큰 그래디언트 표본을 우선 유지하는 샘플링
- **상호 배타적 특성 묶기(Exclusive Feature Bundling, EFB)**: 동시에 값이 없는 희소 특성을 묶는 기법
...
- **성장 방식 선택**: 데이터 규모•희소도와 리프 과적합 위험을 함께 비교해 부스팅 구현을 고르는 기준
