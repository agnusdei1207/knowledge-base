...
| 검증 평가기 | 새 트리 추가 전후의 **검증 성능**을 비교해 조기 종료 판단 |

**동작 원리**

- **① 현재 예측 산출**: 지금까지 누적한 트리의 출력 합으로 학습 표본별 예측값 계산
- **② 손실 미분값 계산**: 정답·현재 예측에서 목적 함수의 그래디언트와 헤시안 산출
- **③ 보정 트리 생성**: 손실 미분값을 기준으로 분할 이득이 최대인 노드를 선택해 새 트리 구성
- **④ 앙상블 갱신**: 새 보정 트리의 출력을 학습률만큼 축소해 기존 예측에 누적
- **⑤ 검증 성능 평가**: 갱신한 앙상블의 검증 점수를 비교해 개선이 멈추면 조기 종료
...
### 쉽게 이해하기 (학습용)
- 장애 로그 분류에서 검증 점수·트리 수·깊이·자원을 함께 추적하며 앞선 오차를 겨냥한 보정 트리를 추가하되 검증 이득이 사라지면 조기 종료한다.

## Ⅵ. 결론

- 대규모 희소 데이터는 **LightGBM**, 안정적 균형 성장은 **XGBoost** 선택
