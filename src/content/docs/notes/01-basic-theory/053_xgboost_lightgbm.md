...
- 배경/필요성: 기존 그래디언트 부스팅(GBM)의 연속형 변수 정렬 연산 병목($O(N \cdot D)$), 메모리 과다 점유 및 과적합 취약성을 극복하고, 2차 테일러 손실 근사, 정규화($\gamma, \lambda$), 히스토그램 버킷팅 및 GOSS/EFB를 통해 **대규모 정형 빅데이터에 대한 초고속 학습과 최고 수준의 예측 정확도를 달성**할 필요
...
- 정형(Tabular) 빅데이터 분석, 금융 리스크 스코어링 및 추천/CTR 예측 시스템의 **핵심 그래디언트 부스팅 표준 프레임워크**로 확립되었으며, 실무에서는 **수천만 건 이상 빅데이터에 LightGBM(Leaf-wise + EFB/GOSS)을 우선 적용하고, 소중규모 정밀 과적합 제어에는 XGBoost(Level-wise + 2차 정규화), 범주형 변수 지배 환경에는 CatBoost**를 상호 보완적으로 채택
