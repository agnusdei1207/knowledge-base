...
- **익스트림 그래디언트 부스팅(eXtreme Gradient Boosting, XGBoost)·경량 그래디언트 부스팅 머신(Light Gradient Boosting Machine, LightGBM)**: XGBoost는 ‘엑스지부스트’, LightGBM은 ‘라이트지비엠’으로 읽고 각 영문 이름을 줄인 표기이며, 앞선 예측이 남긴 오차를 새 트리가 차례로 메워 가는 그래디언트 부스팅을 실제로 구현한 두 대표 라이브러리다
...
- **기울기 기반 단측 표본 추출(Gradient-based One-Side Sampling, GOSS)**: ‘고스’로 읽고 영문 머리글자를 딴 약어이며, 아직 많이 틀리는 큰 기울기 표본은 모두 남기고 이미 잘 맞히는 작은 기울기 표본은 일부만 뽑아 학습량을 줄이는 방식이다
- **상호 배타적 특성 묶기(Exclusive Feature Bundling, EFB)**: ‘이에프비’로 읽고 영문 머리글자를 딴 약어이며, 동시에 값이 나타나는 일이 없는 희소 특성들을 하나로 묶어 다뤄야 할 특성 수 자체를 줄이는 방식이다
...
2. 대용량 로그 분류는 LightGBM **리프 최소 표본 수** 상향

### 쉽게 이해하기 (학습용)
- 장애 유형이 잘 안 갈리는 애매한 로그만 남을 때마다 그 로그를 겨냥한 트리를 하나씩 덧붙여 나가므로, 처음에는 크게 틀리던 분류가 트리가 쌓일수록 조금씩 정확해진다
- 로그가 수천만 건이면 리프 하나에 표본 몇 개만 담긴 가지까지 생겨 특정 서버의 사연을 외워 버리므로, 리프가 가져야 할 최소 표본 수를 올려 그런 가지가 아예 못 자라게 막는다
