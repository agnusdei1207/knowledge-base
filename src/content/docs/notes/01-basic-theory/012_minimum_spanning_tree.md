...
- **안전 간선(Safe Edge)**: 현재 선택 집합에 더해도 최소 신장 트리로 확장할 수 있는 간선
...
- **컷(Cut)**: 정점 집합을 겹치지 않는 두 영역으로 나눈 경계
- **안전 간선(Safe Edge)**: 사이클을 만들지 않고 현재 선택을 최소 신장 트리로 확장하는 간선
...
- **크루스칼(Kruskal)**: 전체 간선을 가중치순으로 검사해 사이클 없는 간선을 선택하는 알고리즘
- **프림(Prim)**: 현재 트리와 바깥 정점을 잇는 최소 간선으로 영역을 확장하는 알고리즘
