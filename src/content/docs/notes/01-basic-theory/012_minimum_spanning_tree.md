...
date: "2026-08-05T17:21:05+09:00"
tags:
  - "notes-basic-theory"
weight: 12
extra:
...
- **가중 무방향 그래프**: 방향이 없는 각 간선에 비용이나 거리를 부여한 그래프

</details>

- 정의/개념: 가중 무방향 그래프의 모든 정점을 사이클 없이 연결하면서 **간선 총가중치를 최소화한 신장 트리**
...
- **연결 성분(Connected Component)**: 서로 경로로 도달할 수 있는 정점들의 최대 집합
- **$V-1$개 간선**: $V$개 정점을 사이클 없이 하나의 트리로 연결할 때 필요한 간선 수
...
- **서로소 집합(Disjoint Set Union, DSU)**: 연결 영역의 대표를 조회하고 합쳐 사이클을 판정하는 자료구조
...
가지의 의미: 선택 제어•간선 후보•연결 상태의 정적 책임 분리
...
- **입력 구조**: 간선 목록•인접 목록•인접 행렬로 구분한 그래프 표현
...
가지의 의미: 연결성•간선 밀도•입력 구조에 따른 결정 분기
...
- **희소 그래프(Sparse Graph)**: 가능한 정점 쌍에 비해 실제 간선이 적은 그래프
- **밀집 그래프(Dense Graph)**: 가능한 정점 쌍에 비해 실제 간선이 많은 그래프
- **간선 목록(Edge List)**: 간선을 끝점과 가중치 묶음으로 나열한 구조
- **인접 행렬(Adjacency Matrix)**: 정점 쌍별 가중치를 표로 저장한 구조
- **비연결 그래프(Disconnected Graph)**: 일부 정점 쌍 사이에 연결 경로가 없는 그래프
...
- **연결성 검사**: 모든 정점이 하나의 경로 집합으로 이어지는지 확인하는 절차
