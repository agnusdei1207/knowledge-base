...
date: "2026-07-28T20:08:00+09:00"
tags:
  - "notes-basic-theory"
weight: 22
extra:
...
- **① 현재 기호 읽기**: 헤드 위치의 테이프 기호 확인
- **② 기호 기록**: 전이 규칙이 지정한 기호로 현재 칸 변경
- **③ 헤드 이동**: 지정 방향의 인접 칸으로 위치 변경
- **④ 상태 갱신**: 다음 상태로 바꾸고 정지 여부 판정
...
| 인식·결정 가능 혼동 | 비소속 입력의 정지 여부 별도 증명 | 계산 가능성 분류 정확화 |
| 범용 계산의 권한 남용 | 샌드박스·입출력 권한 최소화 | 시스템 영향 격리 |
| 스크립트의 무한 실행 | 시간·메모리 한도 강제 | 서비스 자원 보호 |
...
- 범용 계산이 필요하면 튜링 모델, 운영 환경은 **시간·메모리 상한** 강제
