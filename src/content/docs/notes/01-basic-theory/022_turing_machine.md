...
date: "2026-07-27T21:49:00+09:00"
tags:
  - "notes-basic-theory"
weight: 22
extra:
...
- **튜링 머신(Turing Machine)**: 끝없는 종이띠의 글자를 읽고 덮어쓰며 좌우로 움직이는 장면처럼, 테이프와 상태 전이로 계산을 표현하는 추상 기계
...
- **결정 가능 언어·튜링 인식 가능 언어(Decidable·Turing-recognizable Language)**: 결정 가능 언어는 어떤 입력에서도 튜링 머신이 반드시 멈추고 소속 여부를 판정하는 언어이고, 튜링 인식 가능 언어는 소속인 입력은 받아들이지만 비소속인 입력에서는 멈추지 않을 수 있는 언어다. 두 부류의 차이에는 정지 문제처럼 인식은 가능하지만 결정할 수 없는 언어가 속한다
...
- **푸시다운 오토마타(Pushdown Automaton, PDA, 피디에이)**: 영문명의 머리글자를 조합한 공식 약어로, 스택의 맨 위 값을 이용해 중첩 구문을 인식하는 계산 모델
- **유한 오토마타(Finite Automaton, FA, 에프에이)**: 영문명의 머리글자를 조합한 공식 약어로, 별도 기억장치 없이 유한한 상태만 바꾸며 정규 언어를 인식하는 계산 모델
