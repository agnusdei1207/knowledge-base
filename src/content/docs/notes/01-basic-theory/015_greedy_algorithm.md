...
- 정의/개념: 각 단계에서 현재 기준으로 가장 좋은 선택을 확정하고 되돌리지 않은 채 누적하여 전체 해를 만드는 설계 기법이다.
...
- **국소 최적 선택** 확정 후 되돌림 없는 해 구성한다.
- **탐욕 선택 속성**•**최적 부분 구조** 충족 시 전역 최적해 보장이 핵심이다.
- 선택•타당성 검사만 반복하는 **낮은 상태 관리 비용**이 핵심이다.
...
> 요약: **탐욕 속성**•중복 상태•후보 규모로 최적화 기법을 선택한다.
...
- 최적성 증명은 **탐욕**, 중복 상태는 **DP**, 소규모는 완전 탐색이 핵심이다.
