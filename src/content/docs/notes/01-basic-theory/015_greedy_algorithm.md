...
date: "2026-07-29T11:44:36+09:00"
tags:
  - "notes-basic-theory"
weight: 15
extra:
...
- 선택 함수가 후보 중 현재 최선을 고르고, 타당성 검사를 통과한 후보만 부분 해에 확정한다.
...
- **1. 미선택 후보 요청**: 선택 가능 대상 확보
- **2. 국소 최선 후보 전달**: 목적 함수로 하나 선택
- **3. 통과 후보 전달**: 제약 충족분만 부분 해에 누적
- **4. 누적 부분 해 전달**: 완성 조건 판정
- **5. 완성 여부 반환**: 미완성이면 다음 선택 반복
...
| 핵심 고려사항 | 위험 | 대책 |
|:---|:---|:---|
| **탐욕 선택 속성** | 국소 최선이 전역 최적해를 보장하지 못함 | 교환 논증과 최소 반례로 최적성 검증 |
| 숨은 제약 조건 | 선택 후 후보가 실행 불가능한 부분 해 생성 | 모든 제약을 **타당성 함수**에 반영 |
| 동률 후보 | 실행마다 다른 해와 후속 선택 발생 | 결정적 동률 해소 규칙 적용 |
| 입력 범위 확대 | 검증하지 않은 경계에서 반례 발생 | 경계값 시험과 소규모 DP•완전 탐색 대조 |
