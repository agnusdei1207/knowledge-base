...
date: "2026-07-28T20:34:00+09:00"
tags:
  - "notes-basic-theory"
weight: 26
extra:
...
- **① 제어 해독**: 입력 코드에 대응하는 선택선 활성화
- **② 경로 선택**: 선택선에 맞는 피연산자 전달
- **③ 조합 연산**: 전가산기 배열로 합·자리올림 계산
- **④ 출력 안정**: 전파 지연 후 결과를 레지스터에서 표본화
...
| 멀티플렉서 팬인 증가 | 계층형 선택기·경로 분할 | 부하·배선 지연 완화 |
| 경로 지연 차의 글리치 | 안정 시점 레지스터 표본화 | 순간 오출력 전파 방지 |
| 조합 경로의 클록 주기 초과 | 중간 레지스터로 파이프라인 분할 | 동작 주파수 향상 |
| 프로세서 가산기 지연 | 선견 자리올림 적용 | 캐리 계산 병렬화 |
