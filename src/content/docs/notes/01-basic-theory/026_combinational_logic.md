...
date: "2026-08-06T23:26:00+09:00"
tags:
  - "notes-basic-theory"
weight: 26
extra:
...
- **무상태 디지털 회로**: 저장된 과거 상태 없이 현재 입력 조합만으로 출력을 결정하는 회로

</details>

- 정의/개념: 기억 소자와 피드백 없이 현재 입력의 부울 함수로 출력을 결정하는 **무상태 디지털 회로**
- 배경/필요성: 단순 변환에 상태 저장 시 **회로 면적•검증 부담** 증가
...
- **회로 면적**: 게이트와 배선이 반도체 칩에서 차지하는 물리적 공간
...
- **제어 코드**: 수행할 연산이나 사용할 데이터 경로를 지정하는 비트 조합
- **선택 신호**: 멀티플렉서가 통과시킬 입력을 지정하는 신호
- **피연산자(Operand)**: 산술이나 논리 연산의 대상이 되는 입력값
...
선의 의미: 조합 논리 회로를 이루는 입출력 경계와 논리 변환부
...
- **전파 지연(Propagation Delay)**: 입력 변화가 게이트 경로를 지나 출력에 반영될 때까지의 시간
- **출력 안정화**: 모든 경로의 신호 전파가 끝나 출력이 요구 논리값으로 확정된 상태
</details>
...
- **1. 논리 함수 평가**: 현재 입력으로 부울 출력 계산
- **2. 게이트 경로 전파**: 각 경로를 따라 신호 변화 전달
- **3. 출력 안정화**: 크리티컬 패스 뒤 최종 출력 확정
...
- **자리올림 전파(Carry Propagation)**: 하위 자리의 캐리가 상위 자리 가산기로 연속 전달되는 과정
...
- **선견 자리올림(Carry Lookahead)**: 자리올림을 병렬 계산해 지연을 줄이는 가산 구조
- **선택 자리올림(Carry Select)**: 두 결과를 미리 계산해 지연을 줄이는 가산 구조
...
- **계층형 선택기**: 많은 입력을 여러 단계의 작은 멀티플렉서로 나누어 선택하는 구조
- **레지스터 표본화**: 정해진 클록 시점의 조합 회로 출력을 레지스터에 저장하는 처리
...
- **가산**: 두 피연산자와 자리올림을 더하는 논리 기능
- **경로 선택**: 여러 입력 중 하나를 출력으로 연결하는 논리 기능
- **해독**: 입력 코드를 대응 출력선으로 바꾸는 논리 기능
