...
- **데이터 경로(Data Path)**: 선택된 피연산자가 연산기로 이동하는 회로 경로
...
- **전가산기(Full Adder)**: 두 입력 비트와 하위 자리의 캐리 입력을 더해 합과 캐리 출력을 만드는 조합 회로
- **멀티플렉서(Multiplexer)**: 선택 신호에 따라 여러 입력 중 하나를 출력 경로로 전달하는 조합 회로
- **디코더(Decoder)**: 입력 코드에 대응하는 출력선 하나 또는 여러 개를 활성화하는 조합 회로
