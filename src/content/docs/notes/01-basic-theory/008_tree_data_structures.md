...
date: "2026-07-28T11:46:16+09:00"
tags:
  - "notes-basic-theory"
weight: 8
extra:
...
| 고려사항 | 대응 방안 | 기대 효과 |
|:---|:---|:---|
| 저장 계층과 노드 크기 불일치 | 페이지 크기에 맞춰 B-Tree 차수 설정 | 저장소 I/O 감소 |
| 조회·갱신 비율에 맞지 않는 균형 강도 | 조회 집중은 AVL, 갱신 빈번은 레드블랙 검토 | 재균형 비용 최적화 |
| 분할·병합 중 장애로 구조 불일치 | 로그 선기록과 원자적 페이지 갱신 적용 | 복구 가능성 확보 |
| 동시 탐색·갱신 경합 | 래치 범위와 잠금 순서 표준화 | 교착·대기 완화 |

### 쉽게 이해하기 (학습용)
- 저장 계층과 작업 비율에 맞는 트리를 고르고 갱신의 원자성과 동시성을 보장한다.
...
### 쉽게 이해하기 (학습용)
- 한 페이지에 여러 키를 저장해 트리 높이와 페이지 접근 횟수를 줄인다.
