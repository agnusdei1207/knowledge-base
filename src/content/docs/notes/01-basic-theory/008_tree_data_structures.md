...
date: "2026-07-29T11:38:51+09:00"
tags:
  - "notes-basic-theory"
weight: 8
extra:
...
1. **키 처리 요청**: 조회•삽입•삭제 목적과 대상 키 설정
2. **키 비교 요청**: 루트부터 키 대소로 자식 선택
3. **갱신 경로 전달**: 변경 경로의 균형 규칙 위반 판정
4. **균형 복구 결과 전달**: 회전•색상•분할 반영
5. **처리 결과 반환**: 탐색 결과 또는 균형 복구된 루트 확정
...
| 핵심 고려사항 | 위험 | 대책 |
|:---|:---|:---|
| 저장 페이지와 **노드 크기** | 노드 분기 수 부족으로 저장소 I/O 증가 | 페이지 크기에 맞춘 **B-Tree 차수** 설정 |
| 조회•갱신 비율 | 과도한 균형 복구 또는 긴 탐색 경로 | 조회 중심 AVL, 갱신 중심 레드블랙 선택 |
| 분할•병합 중 장애 | 일부 구조만 반영돼 탐색 경로 손상 | 복구 로그와 원자적 페이지 쓰기 적용 |
| 동시 노드 갱신 | 잠금 순서 충돌과 경합 증가 | 루트•리프 방향 잠금 순서와 범위 고정 |
...
- 페이지 I/O는 **B-Tree**, 메모리는 조회•갱신 비율로 결정
