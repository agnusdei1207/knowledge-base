...
- **신호 대 잡음비(Signal-to-Noise Ratio, SNR)**: 신호 전력 $S$와 잡음 전력 $N$의 선형비
...
- **가산성 백색 가우시안 잡음(Additive White Gaussian Noise, AWGN) 채널**: 평탄한 가우시안 잡음이 신호에 더해지는 채널
...
- **기존 한계**: **이론적 상·하한** 없이는 압축·전송의 개선 여지 판단 불가
...
| 한계 | 기호 **확률 분포**에만 의존 | **대칭성**으로 인과 방향 불명 | 무한 **블록 길이** 가정의 상한 |
...
- **엔트로피** 하한과 **채널 용량** 상한으로 코드율 결정

### 쉽게 이해하기 (학습용)
- 짐을 줄일 수 있는 최소 부피와 길이 감당할 최대 통행량이 각각 정해져 있으므로, 그 사이에서 얼마나 압축하고 얼마를 복구용 여분으로 남길지 비율만 정하면 된다
