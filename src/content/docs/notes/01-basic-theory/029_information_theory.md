...
date: "2026-07-28T20:54:00+09:00"
tags:
  - "notes-basic-theory"
weight: 29
extra:
...
| 계열 | 수식·의미 |
|:---|:---|
| 보라 · 1계열 | $C/B=\log_2(1+S/N)$, 전력 증가의 용량 이득 둔화 |
...
- **① 중복 제거**: 정보원 분포에 맞춰 평균 부호 길이 축소
- **② 정정 비트 추가**: 목표 오류율에 맞는 코드율 구성
- **③ 잡음 채널 전송**: 전이확률에 따라 수신 신호 변형
- **④ 코드워드 복원**: 여분 비트로 오류를 추정해 원정보 복원
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 정보율이 채널 용량 초과 | 변조·코드율·대역폭 재설계 | 목표 오류율 달성 가능성 확보 |
| 압축·오류 정정 순서 혼동 | 소스 부호화 후 채널 부호화 | 중복 제거와 복원력 분리 |
