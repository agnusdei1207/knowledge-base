...
+--------------------------------------------------------+
|                   유한 오토마타 모델 (M)                |
|  - 5-튜플 M = (Q, Σ, δ, q0, F) 기반 정규 언어 판정 엔진|
+---------------------------+----------------------------+
                            |
            +---------------+---------------+
            v                               v
...
+--------------------------------------------------------+
|                     시작 및 실행 관리                  |
|  - q0: 초기 시작 상태 / ε-Closure(q0) 활성 집합 설정   |
+--------------------------------------------------------+
```

선의 의미: 실선 박스는 유한 오토마타 모델 구성 요소, 화살표(->)는 상태 전이 및 수용 여부 판정 흐름
...
```text
+------------------------------------------+
|  입력 문자열 W = a1 a2 ... an 스트림 진입|
+--------------------+---------------------+
                     |
                     v
+------------------------------------------+
|  초기 상태 설정 (DFA: q0 / NFA: ε-폐쇄)  |
+--------------------+---------------------+
                     |
                     v
           /--------------------\
   ...
   Yes (ai) |                  | No (EOF)
            v                  v
[ 전이 함수 δ 적용 갱신 ]  [ 최종 상태의 수용 집합(F) 검사 ]
            |                  |
            +-------<----------+
                               |
                     /--------------------\
...
**동작 원리**:
1. **초기 상태 활성화**: DFA는 단일 시작 상태 $q_0$를 활성화하고, NFA는 $q_0$에서 전이 기호 없이 이동 가능한 $\varepsilon$-Closure($q_0$) 상태 집합을 구성한다.
2. **입력 심볼 순차 소비**: 입력 문자열 스트림으로부터 문자 $a_i$를 순차적으로 하나씩 읽어들인다.
3. **전이 함수 적용**: DFA는 현재 상태와 기호 $a_i$에 대해 정의된 유일한 다음 상태 $\delta(q, a_i)$로 즉시 전이($O(1)$)하고, NFA는 활성 상태 집합에 대해 $\varepsilon$-Closure($\delta(S, a_i)$)를 연산하여 다중 활성 상태 집합을 갱신한다.
4. **입력 종료 검사**: 문자열의 모든 심볼이 소진(EOF)될 때까지 2~3단계의 상태 전이를 반복 순회한다.
5. **수용 여부 최종 판정**: 입력 소진 시점의 도달 상태 $q$가 수용 상태 집합 $F$의 원소인지(NFA는 $S \cap F \neq \emptyset$ 여부) 판정하여 정규 언어의 일치(Accept) 또는 불일치(Reject) 결과를 반환한다.
...
- 컴파일러 어휘 분석기(Lex/Flex), WAF 네트워크 침입 탐지 및 고성능 정규식 엔진(Google RE2)의 핵심 형식 언어 인식 모델로 정립되어 널리 활용되고 있으며, 향후 하이퍼스캔(Hyperscan) 등 SIMD 가속 다중 스트림 오토마타 및 하드웨어 가속 구조로 확장될 것으로 전망되고, 실무 엔지니어링에서는 백트래킹 엔진의 ReDoS 취약점을 원천 차단하기 위해 선형 시간($O(n)$) DFA 엔진을 채택하고, 상태 폭증 억제를 위한 홉크로프트 상태 최소화 및 지연 결정화(Lazy DFA)를 결합해야 한다.
