...
- 컴파일러 어휘 분석기(Lex/Flex), 네트워크 침입 탐지(Snort/WAF) 및 고성능 정규식 엔진(Google RE2)의 기반 이론 모델로 안착되었으며, 실무에서는 NFA의 간결한 정규식 모델링과 DFA의 선형($O(n)$) 실행 속도를 결합한 홉크로프트(Hopcroft) 최소화 및 지연 결정화(Lazy DFA) 기법으로 최적화
