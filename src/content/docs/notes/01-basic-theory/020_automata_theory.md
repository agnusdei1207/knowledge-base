...
- 배경/필요성: 문자열 패턴 매칭 및 프로그래밍 언어 어휘 분석에서 발생 가능한 지수적 백트래킹(ReDoS)과 중복 파싱 오버헤드를 극복하고, 유한 상태 집합과 전이 함수($M=(Q,\Sigma,\delta,q_0,F)$)를 통해 **선형 시간($O(n)$) 내 정규 언어 인식 및 결정론적 어휘 분석을 보장**할 필요
...
- 컴파일러 어휘 분석기(Lex/Flex), 네트워크 침입 탐지(Snort/WAF) 및 고성능 정규식 엔진(Google RE2)의 **기반 이론 모델**로 안착되었으며, 실무에서는 **NFA의 간결한 정규식 모델링과 DFA의 선형($O(n)$) 실행 속도를 결합한 홉크로프트(Hopcroft) 최소화 및 지연 결정화(Lazy DFA) 기법**으로 최적화
