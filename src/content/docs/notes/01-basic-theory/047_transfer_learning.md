...
> 요약: **학습 파이프라인**이 백본과 **태스크 헤드**의 재학습 범위 제어

### 쉽게 이해하기 (학습용)
- 기성 정장의 몸판처럼 사전 학습 백본은 범용 특징을 유지하고, 새로 다는 깃처럼 태스크 헤드는 타깃 과제에 맞는 예측값을 만든다
...
| 절차 | 설명 |
|:---|:---|
| 백본 불러오기 | 타깃 과제와 **도메인 유사성**이 높은 모델 선택 |
| 태스크 헤드 구성 | 타깃 **예측 형식**에 맞는 출력 계층 구성 |
| 태스크 헤드 학습 | **백본 동결** 후 태스크 헤드 우선 학습 |
| 성능 검증 | 타깃 검증 데이터로 **일반화 성능** 평가 |
| 점진 미세 조정 | 상위 계층부터 **동결 해제**·낮은 **학습률** 적용 |

> 요약: **태스크 헤드** 학습 후 검증 결과에 따라 **백본** 점진 해제

### 쉽게 이해하기 (학습용)
- 기성 정장의 깃을 먼저 고친 뒤에도 맞지 않을 때 몸판을 조금씩 손보듯, 태스크 헤드를 먼저 학습하고 검증 성능이 부족할 때만 백본 상위 계층을 낮은 학습률로 조정한다
