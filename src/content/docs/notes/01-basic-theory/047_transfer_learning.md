...
- **전이 학습(Transfer Learning)**: 대량 데이터로 학습한 모델의 지식을 새로운 업무에 재사용하여, 적은 정답 데이터로도 학습 성능을 확보하는 방법이다
- **핵심 판단**: 기존 학습 데이터와 새 업무 데이터의 **특성 차이**, 새 업무의 **정답 데이터 규모**를 기준으로 모델의 재학습 범위를 결정한다
- **소스 도메인(Source Domain)**: 사전 학습에 사용한 기존 데이터와 업무 영역이다
- **타깃 도메인(Target Domain)**: 모델을 적용할 새로운 데이터와 업무 영역으로, 소스 도메인과의 **데이터 특성·업무 구조 유사성**이 전이 효과를 좌우한다
...
### 쉽게 이해하기 (학습용)
- 기존 데이터와 새 업무의 특성이 비슷하면 기존 지식을 많이 재사용하고, 차이가 크면 더 많은 계층을 다시 학습한다
...
- 소량 데이터 학습을 위해 전이 이득을 검토하여 **전이학습** 활용
