...
### 쉽게 이해하기 (학습용)
- 점화식에 부분 문제 수·크기와 결합 비용을 적으면, 마스터 정리가 균등 분할 재귀의 시간 복잡도를 판정한다.
...
### 쉽게 이해하기 (학습용)
- 재귀 호출마다 스택 프레임이 쌓여 공간 비용이 되고, 점화식의 분기 수·축소율·결합 비용이 시간 복잡도를 결정한다.
...
### 쉽게 이해하기 (학습용)
- 잎의 부분 문제 비용이 크면 Case 1, 뿌리의 결합 비용이 크면 Case 3, 층별 비용이 같으면 Case 2가 적용된다.
...
### 쉽게 이해하기 (학습용)
- 호출이 깊으면 명시적 스택으로 바꾸고, 불균등 분할식은 재귀 트리나 대입법으로 분석한다.
