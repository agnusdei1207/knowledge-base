...
- **시간•스택 증가율**: 입력이 커질 때 실행 시간과 호출 스택 사용량이 늘어나는 정도
...
- **분할•결합 비용**: 문제를 부분 문제로 나누고 부분 해를 전체 해로 합치는 데 드는 비용
...
- **부분 문제 수 $a$•축소 비율 $b$•결합 비용 $f(n)$**: 한 호출이 만드는 하위 문제 개수, 입력이 줄어드는 비율, 재귀 호출 밖의 작업량
...
- **재귀 트리•대입법**: 호출 단계별 비용을 펼쳐 합산하는 방법과 예상 복잡도를 귀납적으로 증명하는 방법
- **상수•메모리 비용**: 점근 차수에 숨지만 실제 실행시간과 저장 공간에 영향을 주는 고정 비용
