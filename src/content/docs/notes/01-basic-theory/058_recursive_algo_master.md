...
date: "2026-07-28T12:24:37+09:00"
tags:
  - "notes-basic-theory"
weight: 58
extra:
...
| 설계 요소 | 입력·상태 | 역할 |
|:---|:---|:---|
| 재귀 함수 | 입력·종료 조건 | 축소 입력으로 자기 호출 |
| 호출 스택 | 호출별 스택 프레임 | 재귀 깊이와 공간 비용 반영 |
| 점화식 | $a$·$b$·$f(n)$ | 호출 구조를 비용 모델로 표현 |
| 마스터 정리 | 점화식 성장률 | 점근 시간 복잡도 판정 |
...
### 쉽게 이해하기 (학습용)
- 마스터 정리의 형식과 정규성을 먼저 확인하고, 적용할 수 없으면 재귀 트리나 대입법으로 분석한다.
...
1. 깊은 **AST** 순회에서 재귀를 **명시적 스택**으로 전환
