...
- 배경/필요성: 재귀 구조의 **시간•스택 증가율**을 점화식으로 분석
...
### 쉽게 이해하기 (학습용)
- 재귀 함수와 호출 스택은 실행 비용을 만들고 점화식과 마스터 정리는 증가율을 분석한다.
...
- **1. 점화식 모델화**: $a$•$b$•$f(n)$으로 구조 표현
- **2. 마스터 정리 적용성 검증**: 형식•정규성 확인
- **3. 성장률 비교**: 재귀 비용•결합 비용 차수 비교
- **4. 마스터 Case 복잡도 판정**: 점근 시간 확정
- **5. 재귀 트리•대입법 분석**: 범위 밖 점화식 분석
- **6. 최대 재귀 깊이 분석**: 점근 공간 산정
...
### 쉽게 이해하기 (학습용)
- 깊은 AST 순회는 명시적 스택의 반복 처리로 호출 스택 오버플로를 피한다.

## Ⅶ. 결론

- **재귀 깊이•점화식 형식** 기반 분석법 선택
