...
date: "2026-07-26T00:40:00+09:00"
tags:
  - "notes-basic-theory"
weight: 58
extra:
...
- **분할·결합 비용·성장률 차이($f(n)$·$\epsilon$)**: $f(n)$은 ‘에프 오브 엔’, $\epsilon$은 그리스 문자 ‘엡실론’으로 읽고 각각 함수와 작은 양의 차이를 나타내는 분야 관례에 따라 붙인 표기로, $f(n)$은 재귀 호출 밖에서 문제를 나누고 부분 해를 합치는 데 드는 비용이고 $\epsilon$은 두 비용 차수 사이의 다항식 차이를 나타냄
...
> 요약: 형식·성장률·**정규성**으로 복잡도 또는 대체 분석 결정
