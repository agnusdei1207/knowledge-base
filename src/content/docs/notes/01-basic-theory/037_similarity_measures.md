...
date: "2026-08-05T17:29:42+09:00"
tags:
  - "notes-basic-theory"
weight: 37
extra:
...
- 정의/개념: 거리•각도•집합 중첩으로 두 대상을 비교하는 **유사도 측정**
...
- **거리(Distance)**: 값이 작을수록 가깝다고 해석하는 근접도 표현
- **유사도(Similarity)**: 값이 클수록 비슷하다고 해석하는 근접도 표현
- **특징 표현**: 원시 대상을 비교 가능한 벡터나 집합으로 바꾼 데이터 형식
- **순위 역전**: 척도나 정규화가 바뀌어 후보들의 가까운 순서가 달라지는 현상
...
- **임계값(Threshold)**: 점수 경계를 넘는 후보를 선택하는 기준
- **상위 $k$개(Top-k)**: 정렬 순위가 높은 일정 개수의 후보를 선택하는 기준
- **교집합 비율**: 두 집합의 합집합 크기 중 공통 원소가 차지하는 비율
...
선의 의미: 척도 함수가 사용하는 비교 표현과 점수를 소비하는 후보 선정기의 정적 의존 관계
...
- **점수 방향**: 유사도는 클수록, 거리는 작을수록 가깝다고 판정하는 해석 기준
...
- **1. 표현 의미 판별**: 벡터 방향•집합•좌표 구분
- **2. 코사인 유사도**: 벡터 길이를 제거한 방향 비교
- **3. 자카드 유사도**: 합집합 대비 교집합 비율 계산
- **4. 유클리드 거리**: 단위를 맞춘 좌표의 직선거리 계산
- **5. 점수 방향•선정 기준 적용**: 임계값•상위 k개 확정
...
- **희소 벡터(Sparse Vector)**: 대부분의 좌표값이 0인 벡터
- **이진 표현**: 원소의 존재 여부를 0과 1로만 나타내는 표현
...
- **표준화(Standardization)**: 변수별 단위를 맞추는 처리
- **가중 거리(Weighted Distance)**: 중요도에 따라 좌표 차이에 가중치를 적용하는 거리
...
- **정답 쌍**: 사람이 관련 있거나 없다고 판정해 척도 품질 검증에 사용하는 대상 쌍
- **관련성(Relevance)**: 검색•추천 결과가 사용자의 의도나 필요와 맞는 정도
...
- **유사도 선택 기준**: 비교 표현이 벡터 방향•집합•연속 좌표 중 어느 구조인지 구분하는 기준
