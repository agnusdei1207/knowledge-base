...
- **유사도 측정(Similarity Measure)**: 사람 둘을 세워 놓고 키 차이를 줄자로 잴지, 바라보는 방향이 같은지를 볼지, 주머니 속 물건 중 겹치는 것이 몇 개인지를 셀지부터 정하는 장면을 떠올리면 되며, 거리·각도·집합 겹침 가운데 하나를 골라 두 대상의 가까움을 수치 하나로 바꾸는 척도다.
- **특징 벡터(Feature Vector)**: 비교 대상의 속성을 정해진 순서대로 숫자로 늘어놓은 목록이며, 이 목록이 있어야 두 대상 사이의 거리나 각도를 계산할 수 있다.
...
- **코사인 유사도(Cosine Similarity)**: 두 벡터의 내적을 각 벡터의 크기로 나눠 두 화살표가 같은 쪽을 가리키는 정도만 재는 척도이며, 길이가 달라도 방향이 같으면 높은 값이 나온다.
...
- **노름(Norm)**: 벡터를 화살표로 보았을 때 그 화살표의 길이에 해당하는 값이며, 코사인 유사도는 두 화살표를 각각 길이 1로 줄여 놓고 방향만 재기 때문에 이 길이 정보가 계산에서 사라진다.
...
| 설계 요소 | 설명 |
|:---|:---|
| 표현 변환기 | 속성을 **특징 벡터**·집합으로 **정규화** |
| 척도 함수 | 거리·각도·**교집합 비율** 산출 |
| 후보 선정기 | **임계값**·상위 $k$개 기준 후보 확정 |

> 요약: **표현 변환**과 척도 선택이 좌우하는 **후보 순위**
...
- 검색어는 단어 두어 개이고 문서는 수천 단어라 등장 횟수를 그대로 빼면 문서가 무조건 멀어 보이지만, 두 벡터가 가리키는 방향만 견주면 길이 차이가 지워져 검색어와 같은 주제를 향한 긴 문서도 위쪽에 올라온다.

## Ⅶ. 결론

- **표현 형태**·단위 통일 여부로 **유사도 척도** 선택
