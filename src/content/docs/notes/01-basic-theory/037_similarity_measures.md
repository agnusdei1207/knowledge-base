...
```mermaid
flowchart TB
    Q[질의·후보 데이터] -->|① 동일 공간 변환| T[표현 변환기]
    T -->|② 거리·유사도 계산| F[척도 함수]
    F -->|③ 점수 방향 정렬| R[후보 선정기]
    R -->|④ 임계값·Top-k 적용| O[선정 후보]
```
...
- **① 동일 공간 변환**: 대상을 특징 벡터·집합으로 표현
- **② 거리·유사도 계산**: 데이터 성질에 맞는 척도 적용
- **③ 점수 방향 정렬**: 거리는 오름차순·유사도는 내림차순 정렬
- **④ 임계값·Top-k 적용**: 업무 기준을 충족한 후보 확정
