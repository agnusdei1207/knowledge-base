...
date: "2026-07-29T11:36:16+09:00"
tags:
  - "notes-basic-theory"
weight: 5
extra:
...
1. **조회 키 전달**: 원본 키와 동등성 비교 기준 설정
2. **버킷 인덱스 전달**: 해시값을 배열 위치로 변환
3. **첫 후보 전달**: 해당 버킷의 원본 키 대조 시작
4. **다음 후보 요청**: 체인•탐사 순서로 후보 이동
5. **조회 결과 반환**: 일치 엔트리 값 또는 후보 소진 확정
...
| 핵심 고려사항 | 위험 | 대책 |
|:---|:---|:---|
| 키 분포의 **충돌 집중** | 특정 버킷의 후보 탐색이 선형으로 퇴화 | 데이터 분포에 맞는 **해시 함수**와 분산 검증 |
| 높은 **적재율** | 개방 주소법 탐사 급증과 빈 버킷 고갈 | 임계 적재율 이전 확장•재해싱 |
| 키 동등성 계약 | 동등 키의 해시값 불일치로 조회 누락 | 동등 비교와 해시 함수의 동일 필드 사용 |
| 외부 입력의 **충돌 DoS** | 의도적 충돌 키로 조회 지연 유발 | 무작위 해시 시드와 버킷 길이 제한 |
