...
date: "2026-07-28T16:57:00+09:00"
tags:
  - "notes-basic-theory"
weight: 5
extra:
...
| 충돌 해결 구조 | **체이닝·탐사**로 같은 인덱스 키 구분 |
| 용량 관리 | **적재율** 임계치에서 확장·재해싱 |

**동작 원리**

- **① 인덱스 산출**: 해시값을 버킷 배열 위치로 변환
- **② 버킷 후보**: 해당 버킷의 키·값 엔트리 전달
- **③ 원본 키 대조**: 후보 키와 조회 키 일치 여부 판정
...
| 매개변수 맵의 **충돌 DoS** | 요청 키에 **무작위 해시 시드** 적용 | 최악 조회를 노린 **충돌 예측** 억제 |

### 쉽게 이해하기 (학습용)

- 이름이 고르게 퍼지는 번호 계산법을 쓰고 붐비기 전에 칸을 늘리며, 요청 키 계산 규칙을 바꾸면 공격자가 같은 버킷에 몰릴 키를 만들기 어렵다.

## Ⅵ. 결론

- **범위 조회**가 없고 **적재율**을 제한할 수 있을 때 해시 선택
