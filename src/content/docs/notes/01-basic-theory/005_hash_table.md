...
| 계열 | 수식•의미 |
|:---|:---|
| <span style="color:#3080dd">■</span> 파란색 선언 | **체이닝 $\alpha$**: 실패 조회의 평균 후보 수 |
| <span style="color:#d95926">■</span> 주황색 선언 | **개방 주소법**: 실패 탐사의 기대 횟수 |

> **개방 주소법**: 높은 적재율에서 탐사 횟수 급증
...
flowchart TB
    R{조회 조건}
    R -->|정확 키•순서 불필요| H[해시 테이블]
    R -->|범위•정렬 순회| T[균형 이진 탐색 트리]
    R -->|삽입 드문 조회| A[정렬 배열]
```
