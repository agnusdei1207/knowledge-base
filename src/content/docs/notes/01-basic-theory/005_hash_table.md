...
- 정의/개념: 키의 **해시값**으로 버킷을 찾아 키•값을 저장•조회하는 자료구조
...
- **충돌 해결 방식**: 같은 버킷에 배정된 여러 키를 체이닝이나 다른 버킷 탐사로 구분하는 규칙
...
가지의 의미: 인덱스 산정•저장•충돌 관리의 정적 책임 분리
...
1. **버킷 인덱스 산정**: 해시값을 배열 위치로 변환
2. **원본 키 대조**: 조회 키와 충돌 후보의 실제 키 비교
3. **다음 후보 이동**: 체인•탐사 순서로 후보 갱신
...
- **선형 퇴화**: 충돌이나 편향으로 조회 비용이 저장 원소 수에 비례하게 늘어나는 상태
