...
| 핵심 고려사항 | 위험 | 대책 |
|:---|:---|:---|
| **불균형 분할** | 호출 깊이와 최악 수행 시간의 선형 퇴화 | 균형 분할과 깊이 제한 적용 |
| 작은 부분 문제 | 재귀 호출 비용이 실제 계산보다 증가 | 임계 크기부터 반복 알고리즘 전환 |
| 높은 **결합 비용** | 보조 공간•복사량이 분할 이득 상쇄 | 버퍼 재사용 또는 제자리 결합 적용 |
| 병렬 작업 크기 편차 | 일부 처리 장치의 유휴와 꼬리 지연 | 작업 세분화와 동적 스케줄링 |
