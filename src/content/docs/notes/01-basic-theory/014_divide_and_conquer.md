...
date: "2026-07-27T21:18:00+09:00"
tags:
  - "notes-basic-theory"
weight: 14
extra:
...
- 보고서를 같은 기준으로 계속 나누면 각 묶음에 같은 해법을 적용하고 여러 사람이 독립 묶음을 동시에 처리할 수 있지만, 묶음을 나누고 합치는 시간이 크면 병렬 이득이 줄어든다
...
- 편집자는 보고서를 작은 묶음으로 나눠 같은 작업을 다시 지시하고, 작업 위치를 기록한 목록을 호출 스택에 쌓아 두었다가 묶음 결과를 원래 보고서로 합친다

## Ⅳ. 원리 및 절차 흐름도

```mermaid
sequenceDiagram
    participant C as 재귀 함수
...
| 복귀 문맥 저장 | 반환 위치·부분 해를 **호출 스택**에 적재 |
| 재귀 해결 | 더 작은 입력으로 **재귀 함수** 호출 |
| 복귀 문맥 복원 | 부분 해 반환 후 이전 **실행 위치** 재개 |
...
- 보고서를 더 작은 묶음으로 나누다가 직접 처리할 분량에 도달하면 답을 구하고, 작업 목록을 거슬러 올라오며 각 묶음의 결과를 전체 보고서로 합친다
...
- 보고서 묶음이 서로 독립이면 따로 처리해 합치는 분할 정복을 쓰고, 같은 계산 묶음이 반복되면 한 번 구한 답을 기록해 재사용하는 동적 계획법을 쓴다
