...
date: "2026-07-28T12:13:38+09:00"
tags:
  - "notes-basic-theory"
weight: 40
extra:
...
| 실패 징후 | 완화 방안 | 기대 효과 |
|:---|:---|:---|
| Sigmoid·Tanh의 포화 | 입력 정규화와 적절한 가중치 초기화 | **기울기 소실** 완화 |
| 음수 입력에 고정된 ReLU | Leaky ReLU 적용과 학습률 조정 | **죽은 뉴런** 감소 |
| 출력층과 과업의 불일치 | 이진 분류는 Sigmoid, 다중 분류는 Softmax, 회귀는 선형 함수 적용 | 출력 의미와 손실 함수 정합 |
| 활성값·기울기 분포 이상 | 층별 통계 모니터링과 정규화 재조정 | 학습 불안정 조기 탐지 |

### 쉽게 이해하기 (학습용)

- 학습 손실뿐 아니라 층별 활성값과 기울기 분포를 함께 보면 포화나 죽은 뉴런을 조기에 찾을 수 있다.
...
- ReLU는 양수 구간의 기울기를 유지하므로 깊은 영상 분류기의 은닉층에서 학습 신호 전달에 유리하다.

## Ⅶ. 결론

- 비선형 학습을 위해 출력 범위·기울기를 검토하여 **활성화 함수** 활용
