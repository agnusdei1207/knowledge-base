...
- **가중합(Weighted Sum)**: 입력과 가중치의 곱을 합하고 편향을 더한 뉴런의 선형 입력
- **선형 결정 경계**: 입력의 선형 결합으로 클래스를 나누는 직선이나 평면 경계

</details>

- 정의/개념: 뉴런 가중합을 비선형 출력으로 바꾸는 **활성화 함수**
- 배경/필요성: 활성화가 없으면 다층 신경망도 **선형 결정 경계**만 표현
...
- **출력 범위**: 활성화 함수가 낼 수 있는 최솟값과 최댓값의 구간
- **순전파(Forward Propagation)**: 예측값을 입력에서 출력 방향으로 계산하는 과정
...
- **비선형 변환**이 없으면 다층도 단일 선형 변환과 동치
- **출력 범위**에 따라 달라지는 다음 층의 신호 분포
...
- **출력 기울기**: 다음 층에서 현재 활성값으로 되돌아온 손실 변화율
...
- **1. 가중합 산출**: $z=Wx+b$ 계산
- **2. 활성값 산출**: $a=f(z)$를 다음 층에 제공
- **3. 출력 기울기 수신**: 다음 층의 손실 기울기 입력
- **4. 입력 기울기 산출**: 도함수를 곱해 이전 층 전파
...
- **정류 선형 유닛(Rectified Linear Unit, ReLU)**: 음수는 0, 양수는 그대로 출력하는 함수
- **죽은 ReLU**: 음수 입력에서 출력과 도함수가 계속 0이라 뉴런의 학습 신호가 끊긴 상태
- **포화(Saturation)**: 입력 절댓값이 큰 구간에서 출력 변화와 도함수가 거의 0이 되는 상태
- **항등 함수(Identity Function)**: 입력을 변형하지 않고 그대로 출력하는 함수
...
- **누설 정류 선형 유닛(Leaky Rectified Linear Unit, Leaky ReLU)**: 음수 구간에 작은 기울기를 남기는 변형
- **Softmax**: 여러 클래스 점수를 합이 1인 확률 분포로 바꾸는 다중 분류 출력 함수
- **가중치 초기화**: 학습 시작 전 가중치 분포를 정해 활성값과 기울기의 규모를 조절하는 방법
- **입력 정규화**: 입력값의 중심과 척도를 맞춰 활성값이 포화 구간에 몰리지 않게 하는 처리
- **학습률(Learning Rate)**: 한 번의 갱신에서 가중치를 변경하는 크기
- **출력•손실 정합**: 출력층 함수가 과업의 값 범위와 손실 함수의 해석에 맞는 관계
...
- **이진 분류 출력**: 하나의 양성 확률을 내는 출력 과업
- **다중 분류 출력**: 여러 클래스의 확률 분포를 내는 출력 과업
- **순환 상태**: 시간 문맥을 저장하는 내부 상태
- **심층 은닉층**: 여러 층으로 특징을 변환하는 내부 표현
