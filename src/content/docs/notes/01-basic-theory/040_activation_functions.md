...
- 배경/필요성: 심층 신경망 역전파 시 시그모이드/하이퍼볼릭 탄젠트 계열의 포화에 따른 기울기 소실(Vanishing Gradient) 및 학습 정체 극복 필요
...
| 도함수 연산기 ($f'(z)$) | 역전파 시 상위 층 손실 기울기에 **국소 기울기($f'(z)$)를 곱하여 하위 층으로 전파** |
...
|:---|:---|:---|:---|
| 적용 기준 | **이진 분류 최종 출력층** | RNN/LSTM 순환 은닉 상태 | **심층 신경망(CNN, MLP) 은닉층 표준** |
...
- 은닉층은 **ReLU/GELU**, 이진 분류 출력은 **Sigmoid**, 순환망은 Tanh 선택
