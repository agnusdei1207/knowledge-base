...
**동작 원리** 1. **가중합 연산**: 입력과 가중치를 곱해 선형 결합값 $z$ 생성
2. **비선형 변환**: 활성화 함수를 통과시켜 활성값 $a$ 산출
3. **손실 기울기 수신**: 손실 함수로부터 역전파된 오차 신호 인입
4. **도함수 연쇄 곱**: 1차 도함수를 곱해 선형 오차 $\partial L / \partial z$ 도출
5. **하위 층 전파**: 가중치 기울기 계산 후 이전 레이어로 오차 전달
