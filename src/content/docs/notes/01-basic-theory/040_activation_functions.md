...
- 배경/필요성: 선형 계층만 쌓으면 아무리 깊어도 하나의 선형 변환으로 접혀 깊이가 값을 못 하고 시그모이드 계열은 포화 구간 도함수가 0.25 이하라 층을 지날 때마다 곱해져 **기울기 소실** 비용을 치르므로, 각 선형 결합 뒤에 양수 구간 도함수를 1로 유지하는 비선형 변환을 끼워 깊이는 얻고 기울기 감쇠는 끊음
