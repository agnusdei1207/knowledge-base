...
- 배경/필요성: 선형 결합 계층만으로 구성된 신경망의 단일 선형 회귀 퇴화 한계와 Sigmoid/Tanh 계열의 포화(Saturation) 구간으로 인한 심층 역전파 기울기 소실(Vanishing Gradient)을 극복하고, 양수 구간 도함수 1을 유지하는 비선형 매핑(ReLU/GELU)을 통해 **신경망의 다층 비선형 특징 표현력(Universal Approximation)과 안정적 심층 학습을 실현**할 필요
...
├── [편향]       : b
├── [선형 결합]  : z = Wx + b
└── [활성화 함수] : a = f(z), 국소 도함수 f'(z)
```

선의 의미: 인공 뉴런과 가중합·활성화 구성의 포함 관계.
...
- 딥러닝 신경망 아키텍처(CNN, Transformer, RNN)의 **핵심 비선형성 부여 메커니즘**으로 확립되었으며, 현대 LLM/비전 모델에서는 **은닉층의 GELU/SwiGLU, He/Xavier 가중치 초기화 및 배치/레이어 정규화(LayerNorm)와의 결합**을 통해 수백 계층 이상의 초대규모 심층망 학습 안정성을 보장
