...
date: "2026-07-28T18:42:00+09:00"
tags:
  - "notes-basic-theory"
weight: 2
extra:
...
config:
  themeVariables:
    xyChart:
      plotColorPalette: "#2563EB, #16A34A, #D97706, #DC2626, #7C3AED"
---
xychart-beta
    title "대표 시간복잡도 증가율 개념 비교"
    x-axis "입력 크기 n" [1, 2, 3, 4, 5]
    y-axis "상대 연산량" 0 --> 32
    line [1, 1, 1, 1, 1 "O(1)"]
    line [0, 1, 1.58, 2, 2.32 "O(log n)"]
    line [1, 2, 3, 4, 5 "O(n)"]
    line [0, 2, 4.75, 8, 11.61 "O(n log n)"]
    line [1, 4, 9, 16, 25 "O(n²)"]
```

| 계열 | 수식·의미 |
|:---|:---|
| 파랑 · 1계열 | $O(1)$, 입력과 무관한 상수 증가율 |
| 초록 · 2계열 | $O(\log n)$, 입력 증가를 단계 수로 압축 |
| 주황 · 3계열 | $O(n)$, 입력 크기에 비례 |
| 빨강 · 4계열 | $O(n\log n)$, 선형 처리와 로그 단계 결합 |
| 보라 · 5계열 | $O(n^2)$, 입력 쌍 조합에 비례 |
...
| 구성요소 | 책임 |
|:---|:---|
| 비용 함수 $T(n)$ | 알고리즘 비용을 **입력 크기** 식으로 표현 |
| 기준 함수 $g(n)$ | 로그·선형·다항·**지수 상한** 후보 제공 |
| 상한 증인 $c,n_0$ | 배율·기준점으로 **부등식 성립** 입증 |
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 느슨한 함수도 성립하는 **상한 모호성** | 성립하는 최소 **점근 차수** 제시 | 증가율 비교의 **변별력** 향상 |
| 같은 차수의 **상수 비용** 차이 | 대표 입력의 **실측값** 병행 | 실제 구간의 **응답 시간** 판별 |
| 상한과 입력 사례의 **개념 혼동** | 최선·평균·최악 조건 명시 | 성능 **보장 범위** 오해 방지 |
