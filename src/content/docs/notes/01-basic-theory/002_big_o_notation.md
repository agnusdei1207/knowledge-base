...
sequenceDiagram
    ...
    participant M as 비용 모델
    participant F as 기준 함수군
    participant W as 증인 탐색기
    participant J as 상한 판정기
    A->>M: 분석 대상
    M->>W: ① 비용 함수 도출
    F-->>W: ② 기준 함수 후보
    W->>J: ③ 상한 증인 탐색
...
| 구성요소 | 책임 |
|:---|:---|
| 비용 모델 | 알고리즘 연산량을 **비용 함수**로 표현 |
| 기준 함수군 | 로그·선형·다항·**지수 함수** 제공 |
| 증인 탐색기 | 부등식을 만족하는 **$c,n_0$** 탐색 |
| 상한 판정기 | 모든 $n\ge n_0$의 **상한 관계** 판정 |

**동작 원리**

- **① 비용 함수 도출**: 입력별 **기본 연산** 횟수 표현
- **② 기준 함수 후보**: 지배항과 가까운 증가 함수 선택
- **③ 상한 증인 탐색**: 부등식이 유지되는 $c,n_0$ 확인
