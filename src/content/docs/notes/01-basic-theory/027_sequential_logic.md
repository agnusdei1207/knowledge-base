...
date: "2026-08-06T23:27:00+09:00"
tags:
  - "notes-basic-theory"
weight: 27
extra:
...
- 정의/개념: 현재 입력과 레지스터의 저장 상태로 다음 상태와 출력을 결정하는 **상태 기반 디지털 회로**
...
- **셋업 시간(Setup Time)**: 클록 엣지 전에 데이터가 안정돼 있어야 하는 최소 시간
- **홀드 시간(Hold Time)**: 클록 엣지 후에 데이터가 안정돼 있어야 하는 최소 시간
...
- 현재 상태•입력의 **FSM 전이**에 따라 같은 입력에도 다른 출력 산출
- **클록 엣지** 단위 상태 갱신으로 처리 단계 **동기화**
- **셋업•홀드 시간•클록 스큐**가 제한하는 동작 주파수
...
- **현재 상태(Current State)**: 지금 레지스터에 저장된 상태값
- **다음 상태(Next State)**: 다음 클록 엣지에 저장할 계산값
...
- **1. 현재 상태•입력 확인**: 레지스터 상태와 외부 입력 판독
- **2. 다음 상태•출력 계산**: 조합 논리로 전이값 생성
- **3. 출력 반영•다음 상태 안정화**: 외부 출력을 내고 클록 전까지 전이값 확정
- **4. 클록 엣지 상태 저장**: 레지스터의 현재 상태 갱신
...
- **클록 도메인 교차(Clock Domain Crossing, CDC)**: 서로 다른 클록 영역 사이에서 신호를 전달하는 과정
- **동기화기(Synchronizer)**: 다단 플립플롭으로 단일 비트 CDC의 메타안정성 전파 확률을 낮추는 회로
- **정적 타이밍 분석(Static Timing Analysis, STA)**: 모든 경로의 셋업•홀드 여유가 클록 제약을 만족하는지 계산하는 검증
