...
date: "2026-07-28T12:02:47+09:00"
tags:
  - "notes-basic-theory"
weight: 27
extra:
...
| 홀드 시간 부족 | 데이터 경로 지연 버퍼 삽입 | 같은 에지의 값 덮어쓰기 방지 |
| 클록 도메인 교차의 메타안정성 | 단일 비트는 다단 동기화기 적용 | 불안정 상태 전파 확률 감소 |
| 클록 스큐로 타이밍 여유 감소 | 클록 트리 합성·STA 검증 | 레지스터 간 에지 편차 완화 |
...
- STA로 경로별 최대·최소 지연을 계산해 셋업·홀드 여유를 검증한다.

## Ⅶ. 결론

- 상태 보존을 위해 셋업·홀드·CDC를 검토하여 **순차 논리 회로** 활용
