...
date: "2026-07-28T23:44:00+09:00"
tags:
  - "notes-basic-theory"
weight: 27
extra:
...
- **① 현재 상태 전달**: 상태 레지스터가 저장한 현재 상태를 다음 상태 논리에 제공
- **② 다음 상태 계산**: 현재 입력·상태를 조합해 다음 클록에 저장할 전이값 산출
- **③ 클록 에지**: 상태 레지스터가 계산된 다음 상태를 저장
- **④ 출력 생성**: 갱신 상태와 현재 입력에 따라 외부 출력 산출
...
- 신호등처럼 이전 색을 기억해야 하면 순차 논리, 현재 스위치 조합만으로 전등이 정해지면 조합 논리이며 각각 저장 타이밍과 경로 지연을 확인한다.
