...
| 계열 | 수식·의미 |
|:---|:---|
| 파랑 | 이상 가속: 경합과 스팬이 없을 때 프로세서 수에 비례 |
| 빨강 | 실제 가속: 스팬·동기화·메모리 경합으로 조기 포화 |
...
| 설계 요소 | 입력·상태 | 역할 |
|:---|:---|:---|
| 프로세서 집합 | 분할 작업 | 동기 단계별 연산·메모리 접근 수행 |
| 공유 메모리 | 공용 데이터 | 프로세서 간 입력·결과 공유 |
| 동기 클록 | 단계 완료 상태 | 병렬 단계의 시작·종료 시점 정렬 |
| 충돌 정책 | 동시 쓰기 요청 | 반영할 값 선택 |
...
| 실패 징후 | 완화 방안 | 기대 효과 |
|:---|:---|:---|
| 프로세서 증설에도 시간이 줄지 않음 | 스팬 분석과 의존 작업 재분할 | 직렬 임계 경로 단축 |
