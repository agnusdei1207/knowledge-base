...
- ① **고전 제어기**가 큐비트 레지스터에 후보들의 **중첩 상태**를 준비한다.
- ② **문제 함수 회로**가 주기 정보 또는 정답 상태를 **위상**에 표시한다.
- ③ **간섭 연산**이 역 QFT나 진폭 증폭으로 목표 상태의 **확률 진폭**을 키운다.
- ④ **측정기**가 양자 상태를 하나의 **고전 비트열**로 확정한다.
- ⑤ 고전 제어기가 측정값을 해석·검증하고 필요하면 반복해 **결과 신뢰도**를 높인다.
...
### 쉽게 이해하기 (학습용)
- 회로·오류 정정·반복 측정과 고전 후처리 비용을 함께 산정한다. 장기 공개키 자산은 쇼어 위협에 대비해 인증서·서명부터 PQC로 전환한다.
