...
date: "2026-08-05T11:23:46+09:00"
tags:
  - "notes-basic-theory"
weight: 62
extra:
...
- **중첩(Superposition)**: 여러 양자 상태를 동시에 표현하는 성질
- **간섭(Interference)**: 위상 관계로 정답 상태의 측정 확률을 높이는 현상
- **쇼어 알고리즘(Shor's Algorithm)**: 주기 찾기로 소인수분해를 가속하는 양자 알고리즘
- **그로버 알고리즘(Grover's Algorithm)**: 진폭 증폭으로 비정렬 탐색을 가속하는 양자 알고리즘
- **계산 비용**: 알고리즘이 수행하는 전체 연산량
- **질의 비용**: 문제 함수에 답을 요청하는 횟수
...
- **확률 진폭**: 측정 확률을 결정하는 복소수 계수
- **위상**: 양자 상태 사이의 간섭 방향을 정하는 각도
- **측정**: 양자 상태에서 고전 결과를 얻는 연산
- **상태 붕괴**: 측정으로 중첩이 하나의 결과 상태로 확정되는 현상
- **양자 우위**: 특정 문제에서 양자 계산이 실용적인 고전 계산보다 시간이나 자원을 적게 쓰는 상태
- **오라클(Oracle)**: 후보가 정답인지 판정해 양자 상태의 위상에 표시하는 문제 함수
- **양자 오류 정정**: 여러 물리 큐비트로 논리 상태의 잡음 오류를 검출•복구하는 기술
- **고전 후처리**: 측정 결과에서 후보 답을 계산하고 검증하는 고전 계산 단계
...
- **양자 푸리에 변환(Quantum Fourier Transform, QFT)**: 주기 정보를 주파수 성분으로 변환하는 양자 연산
- **역 QFT**: 주기 정보를 측정 가능한 상태로 변환하는 역변환
- **진폭 증폭**: 정답 상태의 측정 확률을 반복적으로 키우는 연산
- **모듈러 거듭제곱**: 쇼어 알고리즘에서 입력 지수에 따른 나머지를 계산해 주기 정보를 부호화하는 연산
- **후보 답 검증**: 측정 결과에서 얻은 인수나 탐색 후보가 실제 정답인지 고전적으로 확인하는 과정
...
선의 의미: 고전 제어•후처리기는 큐비트 레지스터와 측정기를 제어하고, 레지스터·문제 함수 회로·간섭 연산은 하나의 양자 계산부를 이루며, 측정기는 그 상태를 고전 결과로 변환한다.
...
- **정답 신뢰도**: 반복 측정과 검증 결과가 정답임을 뒷받침하는 확률 수준
- **주기 추출**: 함수의 반복 주기를 간섭으로 드러내는 연산
...
- **1. 모듈러 주기 상태 부호화**: 거듭제곱 함수의 주기 정보 생성
- **2. 역 QFT•주기 추출**: 측정값에서 함수 주기 후보 산출
- **3. 고전 인수 후보 검증**: 주기로 인수를 계산하고 정답 확인
- **4. 오라클 위상 표시**: 정답 상태의 위상을 반전
- **5. 확산 연산•진폭 증폭**: 정답 상태의 측정 확률 확대
...
- **모듈러 지수 회로**: 쇼어 알고리즘의 모듈러 거듭제곱을 양자 게이트로 구현한 회로
- **오라클 비용**: 정답 판정 함수를 양자 회로로 실행하는 데 필요한 게이트•큐비트•시간
...
- **논리 큐비트**: 오류 정정으로 안정화한 양자 계산 단위
- **양자 오류 정정**: 여러 물리 큐비트로 잡음 오류를 검출•복구하는 기술
- **오류 완화**: 완전한 오류 정정 없이 반복 측정과 잡음 추정으로 결과 편향을 줄이는 기법
- **양자내성암호(Post-Quantum Cryptography, PQC)**: 양자•고전 공격에 대응하도록 설계한 공개키 암호 체계
- **보안 강도**: 알려진 최선의 공격에 필요한 계산량을 비트 수로 나타낸 값
- **물리 큐비트**: 실제 장치에서 잡음의 영향을 받는 양자 정보 단위
- **고전 알고리즘 총비용**: 같은 문제를 가장 좋은 고전 방법으로 푸는 데 필요한 전체 시간과 자원
...
- **리베스트-샤미르-애들먼(Rivest-Shamir-Adleman, RSA)**: 소인수분해 난제 기반 공개키 암호
- **디피-헬먼(Diffie-Hellman, DH)**: 이산 로그 난제 기반 키 교환 방식
- **타원곡선 암호(Elliptic Curve Cryptography, ECC)**: 타원곡선 이산 로그 난제 기반 공개키 암호
- **공개키 전환**: 쇼어 공격에 취약한 RSA•DH•ECC를 PQC로 교체하는 작업
