...
- **계산•질의 비용**: 알고리즘이 수행하는 전체 연산량과 문제 함수에 답을 요청하는 횟수
...
- **주기 추출•진폭 증폭**: 함수의 반복 주기를 간섭으로 드러내는 연산과 정답 상태의 측정 확률을 키우는 연산
...
- **물리•논리 큐비트**: 실제 장치의 잡음 있는 큐비트와 오류 정정으로 안정화한 계산 단위
...
- **RSA•DH•ECC**: 소인수분해와 이산 로그 난제에 기반해 쇼어 알고리즘의 영향을 받는 공개키 방식
- **PQC(Post-Quantum Cryptography)**: 양자컴퓨터의 알려진 공격에도 안전하도록 설계한 공개키 암호 체계
