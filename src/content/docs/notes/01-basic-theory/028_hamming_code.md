...
### 쉽게 이해하기 (학습용)
- 1·2·4번 검사 결과를 이진수로 합친 신드롬이 고장 난 자리표가 되고, 전체 패리티 한 칸을 더하면 두 자리 오류를 단일 오류와 구분한다.
...
flowchart TB
    subgraph H["해밍 코드 계층"]
        ...
        M -->|수신 코드워드| DEC["해밍 복호기<br/>신드롬 판정·오류 정정"]
    end
    IN["원본 데이터"] --> E
    DEC --> OUT["복구 데이터"]
```
...
| 해밍 복호기 | **신드롬**으로 오류 위치를 판정·정정하고 원본 추출 |
...
- **③ 오류 위치**: 0이 아닌 신드롬을 이진 위치값으로 해석
- **④ 정정 데이터**: 해당 비트를 반전하고 패리티를 제거해 원본 추출
