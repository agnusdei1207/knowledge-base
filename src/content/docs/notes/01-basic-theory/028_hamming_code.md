...
date: "2026-08-06T23:28:00+09:00"
tags:
  - "notes-basic-theory"
weight: 28
extra:
...
- **오류 정정 부호(Error-Correcting Code, ECC)**: 중복 비트를 추가해 저장•전송 과정의 오류를 검출하고 정정하는 부호
- **신드롬(Syndrome)**: 패리티 재검사 결과를 조합해 얻는 오류 위치의 이진값

</details>

- 정의/개념: 위치별 패리티 검사 결과를 이진 신드롬으로 조합해 단일 비트 오류를 판정•정정하는 **오류 정정 부호**
...
- **단일 오류 정정 및 이중 오류 검출(Single Error Correction, Double Error Detection, SECDED)**: 한 비트 오류는 정정하고 두 비트 오류는 검출하는 방식
- **패리티 오버헤드**: 원본 데이터 외에 오류 검사를 위해 추가되는 비트의 비율
- **최소 해밍 거리**: 서로 다른 유효 코드워드 사이의 해밍 거리 가운데 가장 작은 값
- **패리티 비트(Parity Bit)**: 지정된 비트 묶음의 1 개수를 짝수나 홀수로 맞추는 검사 비트
...
- **부호기(Encoder)**: 패리티를 붙여 코드워드를 만드는 구성요소
- **복호기(Decoder)**: 신드롬으로 오류를 판정•정정하는 구성요소
- **비트 반전 오류**: 저장•전송 중 한 비트 값이 0에서 1 또는 1에서 0으로 바뀐 오류
...
- **1. 패리티 수•위치 결정**: 조건식으로 개수와 자리 산정
- **2. 코드워드 생성**: 패리티 그룹을 계산해 비트 삽입
- **3. 신드롬 계산**: 패리티 재검사로 오류 위치 판정
- **4. 오류 비트 반전**: 신드롬 위치의 단일 비트 정정
...
- **단일 오류 정정(Single Error Correction, SEC)**: 코드워드의 단일 비트 오류 위치를 식별해 정정하는 능력
- **순환 중복 검사(Cyclic Redundancy Check, CRC)**: 생성 다항식의 나머지로 버스트 오류를 검출하는 방식
- **버스트 오류(Burst Error)**: 연속된 비트 구간에서 여러 비트가 함께 손상된 오류
- **ECC 메모리**: 오류 검출•정정 부호를 사용해 저장 비트 손상을 자동 처리하는 메모리
...
- **이중 오류 오정정**: 두 비트가 손상됐지만 단일 오류로 잘못 해석해 정상 비트까지 뒤집는 현상
- **다중 오류**: 한 코드워드에서 둘 이상의 비트가 손상된 상태
