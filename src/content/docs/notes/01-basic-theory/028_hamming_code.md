...
- 정의/개념: 위치별 패리티 검사 결과를 이진 신드롬으로 조합하여 오류 비트의 위치를 찾고 단일 비트 오류를 정정하는 선형 오류 정정 부호이다.
...
- **신드롬 이진값**이 오류 비트 위치를 직접 지시가 핵심이다.
...
| 한계 | 2비트 오류의 **오정정**으로 손상 확대 | 3비트 이상 오류 **검출•정정 미보장** | 자체 **정정 불가**로 재전송 필요 |
...
- 오류 형태•재전송 가능성으로 **SEC•SECDED•CRC**을 선택한다.
