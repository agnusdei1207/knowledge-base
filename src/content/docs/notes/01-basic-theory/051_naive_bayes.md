...
- 정의/개념: **조건부 독립•MAP** 기반 확률 분류기
- 배경/필요성: **결합분포 추정 표본 부족** 보완
...
### 쉽게 이해하기 (학습용)
- 확률 추정기가 학습 확률을 만들고 확률 모델이 이를 보관하며 MAP 분류기가 가장 큰 사후점수를 고른다.
...
- **1. 학습 빈도 전달**: 클래스•특징 빈도 전달
- **2. 사전 확률•우도 저장**: 스무딩 확률 저장
- **3. 신규 특징 전달**: 분류 대상 특징 전달
- **4. 클래스별 확률 조회**: 로그 우도 합산
- **5. MAP 클래스 반환**: 최대 사후점수 선택

### 쉽게 이해하기 (학습용)
- 학습 빈도로 만든 확률을 신규 특징과 결합해 가장 가능성이 큰 클래스를 선택한다.
