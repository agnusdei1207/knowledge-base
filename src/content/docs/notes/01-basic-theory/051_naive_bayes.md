...
- **클래스·특징(Class·Feature)**: 클래스는 예측할 범주이고 특징은 분류 근거로 사용하는 입력 속성이다
...
> 클래스 $c$의 로그 사후점수: $\log P(c)+\sum_j\log P(x_j\mid c)$

**동작 원리**

- **① 확률 파라미터 추정**: 클래스별 사전 확률과 특징별 조건부 우도 산출
- **② 확률 모델 저장**: 미관측 특징에 라플라스 스무딩을 적용한 확률 파라미터 보관
- **③ 로그 사후점수 계산**: 신규 입력에 대해 클래스별 로그 사전 확률과 특징별 로그 우도 합산
- **④ MAP 클래스 선택**: 로그 사후점수가 가장 큰 클래스를 예측 결과로 확정
...
| 상관 특징 중복으로 **확률 과신** | 중복 특징 제거·**확률 교정** | **사후 확률 신뢰도** 개선 |
...
- 검증 교정 오차가 허용치를 넘으면 분류 확률에 **확률 교정** 적용
