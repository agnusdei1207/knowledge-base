...
date: "2026-07-25T23:44:00+09:00"
tags:
  - "notes-basic-theory"
weight: 7
extra:
...
- **후입선출(Last-In First-Out, LIFO, 리포)**: 영문 각 단어의 첫 글자를 따 LIFO로 표기하며 마지막 삽입 원소를 먼저 제거하는 규칙
- **선입선출(First-In First-Out, FIFO, 파이포)**: 영문 각 단어의 첫 글자를 따 FIFO로 표기하며 먼저 삽입한 원소를 먼저 제거하는 규칙
- **덱(Double-Ended Queue, Deque, 덱)**: 양쪽 끝을 뜻하는 영문 표현을 줄여 Deque로 표기하며 앞·뒤에서 삽입·삭제하는 선형 자료구조
...
- **깊이 우선 탐색(Depth-First Search, DFS, 디에프에스)**: 영문 각 단어의 첫 글자를 따 DFS로 표기하며 한 경로를 끝까지 탐색한 뒤 되돌아오는 방식
...
> 요약: **연산 인터페이스**가 끝점 상태로 **저장소** 접근 통제
...
2. 고정 **메시지 버퍼**는 **원형 큐**로 도착순 메시지 처리
3. 병렬 스케줄러는 덱 한쪽 인출·반대쪽 **작업 탈취**

### 쉽게 이해하기 (학습용)

- 지금 내려가는 중인 모듈만 스택에 쌓아 두면 그 스택 안에 이미 있는 모듈을 다시 만나는 순간 길이 자기 자신으로 되돌아왔다는 뜻이므로, 되돌아온 그 지점이 곧 빌드 순환이다.
- 번호표가 100번까지만 있는 대기줄에서 마지막 번호 다음에 다시 1번을 내주듯, 원형 큐는 배열 끝과 시작을 이어 이미 꺼내 비운 칸을 다시 쓰면서도 들어온 메시지의 도착 순서는 그대로 지킨다.
- 각 작업자는 자기 덱의 한쪽 끝에서만 일을 꺼내다가 일이 떨어지면 다른 작업자의 반대쪽 끝에서 하나를 가져오므로, 두 사람이 같은 끝을 두고 다투지 않아 부하만 옮겨 간다.
