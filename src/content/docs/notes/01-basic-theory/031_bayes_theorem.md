...
- **베이즈 정리(Bayes Theorem)**: 가설의 사전 확률과 증거 우도를 결합해 증거 관측 후의 사후 확률을 갱신하는 조건부 확률 정리.
...
- **우도(Likelihood)**: 가설 참일 때 증거 관측 가능성.
- **기저율(Base Rate)**: 모집단에서 사건이 발생하는 기본 비율.

</details>

- **정의**: 사전 확률에 우도를 결합하고 정규화하여 사후 확률을 갱신하는 추론 체계.
- **배경**: 기저율을 고려하지 않은 관측은 기저율의 오류(Base Rate Fallacy) 발생.
...
- **PPV 산출**: 민감도, 특이도, 기저율을 결합하여 PPV 산출.
- **오탐 영향**: 기저율이 낮을수록 오탐 비중이 증가하여 사후 확률 하락.
...
- **희귀 사건** 기저율 적용 및 **독립적 추가 증거** 기반 순차적 사후 확률 갱신 체계 적용.
