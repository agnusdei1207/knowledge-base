...
- **베이즈 정리(Bayes' Theorem)**: 가설의 사전 확률(Prior)과 증거 우도(Likelihood)를 결합하여 증거 관측 후 사후 확률(Posterior)을 갱신하는 조건부 확률 정리.
...
- **정의**: 기저율과 같은 사전 확률(Prior)에 증거 우도(Likelihood)를 결합하고 정규화하여 사후 확률(Posterior)을 갱신하는 지능형 추론 체계.
- **배경**: 기저율을 간과한 관측은 기저율의 오류(Base Rate Fallacy)로 이어져 사후 확률의 왜곡을 초래.
...
- **역확률 추론**: 사전 확률과 우도의 곱을 정규화하는 핵심 기법.
- **기저율 오류 방지**: Evidence의 민감도와 특이도뿐만 아니라 Prior Rate $P(H)$를 반영한 PPV(Positive Predictive Value) 산출을 통해 정확한 확률 추론을 수행.
...
- **희귀 사건** 기저율 적용 및 **독립적 추가 증거** 기반 순차적 사후 확률 갱신 체계 구축.
