...
- **정의/개념**: 다이를 수직 적층해 짧은 배선과 넓은 I/O를 얻는 메모리 구조
- **배경/필요성**: 보드형 핀·배선이 대역폭을 제한해 수직 적층이 필요함
...
- 짧은 층간 배선이 비트당 전송 에너지를 줄인다.
- 넓은 I/O가 다수 비트를 동시에 전송한다.
- 수직 적층이 패키지 면적당 용량을 높인다.
- 열 밀도·접합 결함이 냉각 비용과 수율을 낮춘다.
...
| 판단 기준 | HBM형 3D 적층 메모리 | 보드형 메모리 |
|:---|:---|:---|
| 핵심 특징 | 수직 적층·넓은 I/O의 고대역폭 | 수평 배선·모듈형 용량 확장 |
| 적용 기준 | 대역폭·면적 밀도 우선 | 용량·교체성·비용 우선 |
| 주요 위험 | 열 밀도·적층 수율·패키지 비용 | 핀 수·배선 길이·전송 전력 |
...
1. AI 가속기 패키지에 HBM 스택 인접 배치
2. HBM 제조 시 검사 합격 다이만 적층
