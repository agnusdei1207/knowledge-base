...
- 정의/개념: 수직 3D 구조로 DRAM 레이어를 실리콘 관통 전극(**TSV**)으로 상호 바인딩하여 초고대역폭 및 저전력을 달성하는 **3D 적층 메모리**
- 배경/필요성: PCB 2D 평면 상의 핀 수 제한 및 온보드 배선 길이에 따른 스폰지 전력/메모리 벽(Memory Wall) 현상 극복 요구성
...
- **TSV** 관통 전극 기술 기반 1024-bit 초광폭 데이터 버스(**Wide I/O**) 구축
- 물리 인터페이스 배선 길이 단축(~mm 단위)을 통한 전력 소비 감소 및 **KGD** 수율 확보 요구
- 물리 칩셋 상단 적층에 따른 **열 밀도(Thermal Density)** 증가 및 방열 제어 필수성
...
1. **채널·다이·뱅크 해석**: 메모리 컨트롤러가 **Pseudo-Channel** 주소 번역 및 타깃 층(Stack Layer) 해석.
2. **TSV 수직 경로 선택**: 인터포저 매개 및 **Base Die**를 통과하여 **TSV** 수직 신호선으로 물리 라우팅.
3. **행 활성화·열 선택**: 타깃 DRAM 층의 뱅크 Row/Column 활성화.
4. **광폭 데이터 반환**: **Wide I/O(1024-bit)** 버스를 타고 TSV 및 인터포저를 통해 GPU 메인 레지스터로 비트 반환.
...
- **3D 적층 메모리 선택 기준**에 따라 초고성능 AI 가속기(GPU/TPU)에는 **HBM3e/HBM4 3D 적층 메모리** 채택

#### 한줄 요약

- 필요한 대역폭이 열 밀도와 적층 수율 및 패키지 비용을 감당할 만큼 중요할 때 3D 적층 메모리를 선택한다.
