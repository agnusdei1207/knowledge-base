...
- **스레드 수준 병렬성(Thread-Level Parallelism, TLP)**: ‘티엘피’로 읽고 영문 머리글자를 딴 약어이며, 서로 독립적으로 실행할 수 있는 스레드의 수를 뜻한다
- **암달의 법칙(Amdahl's Law)**: 아무리 사람을 늘려도 혼자서만 할 수 있는 구간이 남아 있으면 전체 시간이 그 구간 아래로는 내려가지 않는다는 법칙이며, 직렬 비율이 5%면 코어를 무한히 늘려도 20배가 상한이다
- **동시 멀티스레딩(Simultaneous Multithreading, SMT)**: ‘에스엠티’로 읽고 영문 머리글자를 딴 약어이며, 한 코어의 빈 실행 회로를 다른 스레드가 사용하게 하는 기법
...
- **공유 최종 단계 캐시(Shared Last-Level Cache, LLC)**: ‘엘엘씨’로 읽고 영문 머리글자를 딴 약어이며, 모든 코어가 용량과 대역폭을 함께 쓰는 마지막 캐시 단계다
...
- **모바일 애플리케이션 프로세서(Application Processor, AP)**: ‘에이피’로 읽고 영문 머리글자를 딴 약어이며, 스마트폰의 운영체제와 응용을 실행하는 통합 프로세서다
