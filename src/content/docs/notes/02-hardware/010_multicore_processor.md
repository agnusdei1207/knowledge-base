...
- 정의/개념: 단일 반도체 칩에 복수 연산 코어를 집적하여 **스레드 수준 병렬성**을 실현하는 **멀티코어 프로세서**.
...
- **고성능 및 고효율 코어(Performance Core & Efficient Core, P-Core / E-Core)**: 고부하 단일 스레드는 빅 코어(P-Core)에, 백그라운드 멀티태스킹은 리틀 코어(E-Core)에 배분하여 전력 대 성능비를 극대화하는 이종 멀티코어 구조.
</details>

- 단일 스레드의 지연시간(Latency) 단축보다는 다중 스레드의 시스템 전체 **처리량** 확장을 주 목적으로 설계.
- 소프트웨어가 **스레드 수준 병렬성** 구조로 분할 설계되어 있을 때에만 코어 증설에 따른 성능 향상 효과 발휘.
- 코어 수 $N$이 증가함에 따라 **암달의 법칙(Amdahl's Law)**의 직렬 구간 병목과 코어 간 **동기화** 오버헤드가 전체 가속비를 한정.
...
|:---|:---|:---|
| 독립 변수 갱신 때 무효화가 반복되는 **거짓 공유** | 변수 선언 시 64-Byte **패딩** 명시적 적용 | 캐시 라인 격리로 일관성 트래픽 절감 |
| 특정 코어에 부하가 집중되고 다른 코어는 유휴 상태 | Fork-Join 프레임워크의 **작업 훔치기** 스케줄러 도입 | 물리 코어 부하 평형 및 병렬 지연 단축 |
