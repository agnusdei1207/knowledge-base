...
- **전력•발열 한계** 는 주파수와 전압 상승에 따른 소비 전력과 열 증가 때문에 단일 코어 성능을 계속 높이기 어려운 제약이다.
...
- **물리 코어•논리 스레드** 는 각각 독립 실행 하드웨어와 SMT가 하나의 코어 안에서 제공하는 실행 문맥이다.
...
- **패딩•배치** 는 독립 변수를 서로 다른 캐시 줄에 놓도록 여백과 메모리 위치를 조정하는 방법이다.
- **NUMA 인지 배치•지역성** 은 스레드와 데이터를 가까운 메모리 노드에 배치해 원격 접근을 줄이는 전략이다.
