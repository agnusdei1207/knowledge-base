...
| 설계 요소 | 입력·상태 | 역할 |
|:---|:---|:---|
| 코어 클러스터 | 스레드·사설 캐시 상태 | 복수 스레드 병렬 실행 |
| 일관성 인터커넥트 | 캐시 줄 요청·상태 | 사본 일관성·요청 전달 |
| 공유 LLC | 코어 공용 캐시 줄 | 메모리 접근·용량 공유 |
| 메모리 제어기 | 주기억장치 요청 | 접근 순서·대역폭 배분 |

> 요약: **일관성 인터커넥트**가 사설 캐시와 공유 계층 연결

**동작 원리**

- **① 독립 스레드 A 배치**: 실행 가능한 스레드를 코어 A에 할당
- **② 독립 스레드 B 배치**: 다른 스레드를 코어 B에 할당
- **③ 데이터 접근**: 코어 A가 사설 캐시 경로로 데이터 접근
- **④ 데이터 접근**: 코어 B가 사설 캐시 경로로 데이터 접근
- **⑤ 쓰기 소유권 요청**: 코어 A가 공유 캐시 줄의 독점 쓰기 권한 요청
- **⑥ 다른 사본 무효화**: 인터커넥트가 코어 B의 같은 캐시 줄 무효화
- **⑦ 무효화 완료 응답**: 코어 B가 사본 무효화 완료 통보
- **⑧ 쓰기 소유권 부여**: 다른 사본 무효화 후 코어 A에 쓰기 권한 부여
- **⑨ 스레드 A 완료 통보**: 코어 A가 작업 완료 상태 전달
- **⑩ 스레드 B 완료 통보**: 코어 B가 작업 완료 상태 전달
