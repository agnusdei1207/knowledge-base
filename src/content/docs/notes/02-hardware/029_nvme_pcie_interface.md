...
- **99백분위 응답시간(99th Percentile, $p99$)**: ‘피 구십구’로 읽고 백분위수(percentile)의 소문자 머리글자 $p$ 뒤에 누적 비율 99를 붙이는 통계 관례 표기이며 전체 요청의 99%가 이 값 이내에 끝나는 꼬리 응답시간을 나타냄
...
2. NUMA 서버: 큐·인터럽트·SSD를 같은 노드에 배치

### 쉽게 이해하기 (학습용)

- 코어마다 창구와 알림 담당을 맞춰 긴 대기를 줄인다
- 창구·알림·창고를 같은 건물에 두어 왕복을 줄인다
...
- 창구 수만 늘리지 말고 대기열 깊이와 창고 위치도 함께 맞춘다
