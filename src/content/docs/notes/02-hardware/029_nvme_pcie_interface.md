...
- 배경/필요성: 기존 SATA/AHCI의 단일 큐 구조(32 엔트리)와 저속 버스 대역폭으로 인한 멀티코어 환경에서의 **I/O 병목 및 NAND 플래시 다채널 병렬성 활용 한계** 극복 필요
...
1. **SQ 명령 기록**: 호스트가 제출 큐에 I/O 명령 적재
2. **SQ 도어벨 통지**: 컨트롤러에 새 Tail 인덱스 전달
3. **DMA 인출·NAND 실행**: 명령과 데이터를 직접 전송·처리
4. **CQ 완료 기록·MSI-X 통지**: 결과 기록 후 코어에 완료 알림
5. **CQ 소비·도어벨 갱신**: 완료 엔트리 회수와 Head 갱신
...
- PCIe 직결 기반의 다중 큐(64K 큐) 구조로 초저지연·고성능 플래시 스토리지의 **지배적 표준(NVMe 2.0)**으로 안착되었으며, 최근에는 네트워크 패브릭 기반 **NVMe-oF(RDMA/TCP), ZNS(Zoned Namespaces) 및 계산 스토리지(Computational Storage)**로 진화 추세
