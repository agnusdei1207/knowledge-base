...
- **DDR SDRAM**: 클록 상승·하강 에지에서 데이터를 전송하고 주기적으로 전하를 복원하는 동기식 DRAM
...
- **양 에지 전송**: 한 클록 주기의 상승과 하강 순간 모두에 데이터를 전달하는 방식
- **뱅크 병렬성**: 독립 셀 배열과 행 버퍼를 가진 여러 뱅크가 접근을 겹쳐 수행하는 성질
- **버스트 전송**: 열 명령 한 번으로 연속 데이터를 여러 에지에 걸쳐 보내는 방식

</details>

- **클록 양 에지** 전송으로 핀당 대역폭 배증
- **뱅크 병렬성**과 **버스트**로 연속 처리량 확대
...
- **DDR PHY·DQ·DQS**: 명령과 데이터를 전기 신호로 변환하고 데이터 샘플링 시점을 맞추는 물리 계층과 신호
...
- **ACT·PRE**: 대상 행을 열고 다음 행 접근 전에 현재 행을 닫는 DDR 명령
- **tREFI**: 연속 리프레시 명령 사이에 허용되는 평균 시간 간격
- **tRFC**: 리프레시 수행으로 메모리 접근이 제한되는 시간
...
- **PHY 트레이닝**: DQ와 DQS 신호 지연을 보정해 고속 데이터 샘플링 시점을 맞추는 절차
