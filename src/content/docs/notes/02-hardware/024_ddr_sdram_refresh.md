...
- **DDR SDRAM(Double Data Rate Synchronous Dynamic Random-Access Memory)**: 시계가 똑딱일 때 올라갈 때와 내려갈 때 모두 짐을 실어 같은 시계로 두 배를 나르는 메모리이며, 동시에 셀의 물이 새기 때문에 정해진 기한마다 하던 일을 멈추고 물을 다시 채워야 하는 구조다
...
- **행 활성화(Activate, ACT)**: 대상 행을 열어 행 버퍼로 끌어 올리는 명령
- **읽기·쓰기 명령(RD/WR)**: 열린 행의 특정 열에서 버스트로 데이터를 읽거나 쓰도록 지시하는 명령
- **프리차지(Precharge, PRE)**: 열린 행을 닫고 비트라인을 되돌려 다음 행을 열 수 있게 하는 명령
- **데이터 입출력(DQ)·데이터 스트로브(DQS)**: 실제 데이터가 오가는 핀이 DQ, 그 데이터를 언제 읽을지 시점을 알려 주는 신호가 DQS이며 DQS의 양쪽 에지를 기준으로 받는다
- **물리 계층(PHY)**: 컨트롤러가 만든 명령·주소·데이터를 실제 DDR 전기 신호로 바꾸고 타이밍을 맞추는 회로
...
- **리프레시(Refresh, REF)**: 새어 나간 전하를 메우려고 행을 주기적으로 열었다 닫는 동작이자 그 명령
- **평균 리프레시 간격(tREFI)**: ‘티 레피’로 읽으며 리프레시 명령 사이에 허용되는 평균 시간이고, 이 기한을 넘기면 데이터가 깨진다
- **리프레시 주기 시간(tRFC)**: ‘티 알에프씨’로 읽으며 리프레시가 도는 동안 그 메모리를 건드릴 수 없는 시간이고, 유효 대역폭을 깎는 값이 여기서 나온다
...
> 요약: 갱신 대상 행은 컨트롤러가 아닌 **내부 카운터**가 결정

### 쉽게 이해하기 (학습용)

- 갱신 명령에 주소가 실려 있지 않고 장치 안 카운터가 스스로 다음 행을 고른다는 점이 이 그림의 요지인데, 그래서 컨트롤러는 어느 행이 갱신되는지 알 필요 없이 기한만 지켜 명령을 던지면 되고 대신 그 순간 어느 뱅크가 막힐지도 예측하지 못한다
