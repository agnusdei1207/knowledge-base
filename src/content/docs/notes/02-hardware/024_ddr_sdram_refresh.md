...
date: "2026-07-28T12:47:38+09:00"
tags:
  - "notes-hardware"
weight: 24
extra:
...
| 설계 요소 | 입력·상태 | 역할 |
|:---|:---|:---|
| 메모리 컨트롤러 | 요청 큐·타이밍·리프레시 기한 | 주소 매핑과 명령 순서 조정 |
| DDR 채널·PHY | 명령·주소·DQ·DQS | 논리 명령과 전기 신호 변환 |
| 뱅크·행 버퍼 | 활성 행·열 요청 | 행 활성화·열 접근·전하 복원 |
| 내부 리프레시 카운터 | REF·현재 행 번호 | 갱신 대상 행 순환 지정 |
...
| 운영 위험 | 대응 | 기대 효과 |
|:---|:---|:---|
| 전체 뱅크 리프레시의 긴 꼬리 지연 | 뱅크별 리프레시와 요청 한산 구간 스케줄링 | 접근 중단 분산 |
| 행 미적중 반복으로 ACT·PRE 증가 | 주소 매핑과 요청 재정렬로 행 적중률 개선 | 처리량 증가 |
| 온도 상승으로 전하 보존 시간 단축 | 온도 감시와 규격에 맞는 리프레시 주기 적용 | 데이터 보존 신뢰성 확보 |
| 고속 전송에서 DQ·DQS 타이밍 마진 감소 | 부팅·온도 변화 시 PHY 트레이닝과 오류 감시 | 신호 무결성 유지 |
