...
- **① ACT 명령**: 대상 뱅크·행의 활성 명령 전달
- **② 대상 행 활성화**: 선택 행을 행 버퍼에 개방
- **③ RD·열 주소 전달**: 활성 행의 읽기 열 지정
- **④ 활성 행의 열 판독**: 행 버퍼에서 선택 열 조회
- **⑤ 버스트 데이터 전달**: 연속 데이터를 PHY로 전달
- **⑥ DQ·DQS 양 에지 데이터 반환**: 양 에지에 맞춰 데이터를 샘플링·반환
- **⑦ REF 명령**: tREFI 기한에 맞춰 리프레시 발행
- **⑧ 리프레시 시작**: 내부 카운터에 갱신 명령 전달
- **⑨ 대상 행 전하 복원**: 순환 대상 행의 전하 감지·복원
- **⑩ 행 갱신 완료**: 대상 행 복원 완료 통보
- **⑪ 리프레시 완료**: 지정 갱신의 완료 상태 전달
- **⑫ 메모리 접근 재개 통보**: tRFC 충족 후 일반 명령 재개
