...
date: "2026-08-05T11:36:02+09:00"
tags:
  - "notes-hardware"
weight: 15
extra:
...
선의 의미: 위 선은 스누핑 방식의 공유 방송 경계, 아래 선은 디렉터리 방식의 지정 전달 경계를 뜻하며 두 줄은 서로 다른 대안 구조이다.
...
```text
              ...
              [소규모 공유 버스]  [대규모 점대점망]
                     |                |
              2. 전체 요청 방송    3. 공유자 목록 조회
                     |                |
              [모든 캐시 관찰]    4. 대상 캐시 지정 전달
                     |                |
                     +--------+-------+
                              |
              +------------------------------+
...
- **1. 연결 구조 판정**: 공유 버스와 점대점망 구분
- **2. 전체 요청 방송**: 모든 캐시가 주소와 사본 상태 관찰
- **3. 공유자 목록 조회**: 홈 디렉터리에서 대상 명단 확인
- **4. 대상 캐시 지정 전달**: 관련 공유자에게만 무효화 요청
- **5. 배타적 쓰기 권한 부여**: 완료 응답 수합 후 쓰기 허용
...
- **NUMA**: Non-Uniform Memory Access, 노드 위치에 따라 메모리 지연이 달라지는 구조
