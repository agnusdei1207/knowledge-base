...
date: "2026-07-28T12:42:30+09:00"
tags:
  - "notes-hardware"
weight: 15
extra:
...
| 설계 요소 | 입력·상태 | 역할 |
|:---|:---|:---|
| 공유 버스·중재기 | 일관성 요청 | 요청 직렬화·전체 캐시 방송 |
| 스누프 제어기 집합 | 버스 주소·로컬 상태 | 관련 사본 무효화·데이터 응답 |
| 홈 디렉터리 | 블록별 소유자·공유자 | 대상 노드 조회·소유권 갱신 |
| 점대점 인터커넥트 | 지정 요청·데이터 | 관련 노드에만 메시지 전달 |
...
| 운영 위험 | 대응 | 기대 효과 |
|:---|:---|:---|
| 방송 증가에 따른 버스 포화 | 계층형 스누핑 적용 또는 디렉터리 방식 전환 | 코어 확장성 확보 |
| 공유자 비트맵에 따른 메타데이터 증가 | 희소·제한 디렉터리와 공유자 인코딩 적용 | 저장 공간 절감 |
| 홈 디렉터리 집중과 원격 조회 지연 | 홈 주소 해싱·분산 및 NUMA 인지 배치 | 부하 분산·지연 감소 |
| 무효화 응답 누락이나 상태 교착 | 타임아웃·재시도와 상태 머신 검증 | 정합성·활성 보장 |

### 쉽게 이해하기 (학습용)

- 운영에서는 방송량, 명단 크기, 홈 노드 부하를 함께 관찰하고 응답 누락에도 권한이 잘못 부여되지 않게 검증한다
