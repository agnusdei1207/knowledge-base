...
date: "2026-07-29T00:33:00+09:00"
tags:
  - "notes-hardware"
weight: 15
extra:
...
1. **① 쓰기 권한 요청**: 요청 캐시는 수정할 캐시 라인의 홈 디렉터리에 배타적 쓰기 권한을 요구한다.
2. **② 공유자 목록과 무효화 명령 전달**: 홈 디렉터리는 해당 라인의 공유자 명단을 조회하고 관련 노드 목록을 인터커넥트에 전달한다.
3. **③ 지정 캐시에 무효화 요청**: 인터커넥트는 방송하지 않고 명단에 기록된 각 공유자 캐시에만 무효화 메시지를 보낸다.
4. **④ 무효화 완료 응답**: 공유자 캐시는 해당 라인의 읽기 권한을 제거한 뒤 인터커넥트에 완료를 응답한다.
5. **⑤ 전체 공유자 응답 수합 완료**: 인터커넥트는 대상 캐시의 응답이 모두 도착했음을 홈 디렉터리에 알린다.
6. **⑥ 배타적 쓰기 권한 부여**: 홈 디렉터리는 공유자 명단을 갱신하고 요청 캐시만 쓸 수 있도록 배타 권한을 부여한다.
