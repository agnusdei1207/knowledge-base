...
- **캐시 일관성 구현 방식(Coherence Implementation)**: 사본을 누가 들고 있는지 찾는 방법이 갈리는 지점이며, 회의실에서 모두에게 대고 외쳐 관련 있는 사람이 손을 들게 할지 명단을 뒤져 해당자에게만 전화할지의 차이다
...
2. 다중 소켓 **NUMA** 서버는 홈 디렉터리로 등록 공유자만 통지

### 쉽게 이해하기 (학습용)

- 코어가 네댓 개뿐인 칩에서는 모두가 같은 복도에 붙어 있어 외치는 값이 싸므로, 명단을 따로 두는 회로와 공간을 아끼고 그냥 전원에게 뿌린다
- 소켓이 둘 이상이면 옆 소켓까지 외치는 데 물리적으로 긴 시간이 들고 그 방송이 소켓 간 링크를 잡아먹으므로, 그 블록을 담당하는 소켓의 명단을 보고 실제 사본을 가진 곳에만 연락한다

## Ⅶ. 결론

- **방송 트래픽**이 공유 버스 한계를 넘으면 디렉터리 전환
