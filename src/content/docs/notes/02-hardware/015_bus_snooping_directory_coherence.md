...
- 정의/개념: 공유 버스 트래픽 브로드캐스트 감시 방식의 **버스 스누핑**과 주소별 **공유자 추적**을 이용해 특정 노드에만 P2P 패킷을 전송하는 **디렉터리 기반 일관성** 아키텍처.
...
- 모든 캐시 트랜잭션의 글로벌 전역 순서를 결정짓는 **일관성 순서점**을 구성하여 쓰기 직렬화 보장.
- 타 코어 사본들의 **무효화 응답** 수합이 완결된 후에만 요청 코어에 **배타적 쓰기 권한** 부여.
- 버스 스누핑은 단순한 **방송 관찰** 방식, 디렉터리 기반은 정밀한 **명단 조회**를 통해 트래픽 전송 대상 결정.
...
|:---|:---|:---|
| 통신 메커니즘 | **공유 버스** 기반 전체 브로드캐스트 (1:N) | **점대점 인터커넥트** 기반 P2P 메시지 (1:1 / 1:M) |
...
|:---|:---|:---|
| 노드 수 증설 시 디렉터리 **메타데이터** 용량 증가 | **희소 디렉터리** 및 **제한 디렉터리** 구조 채택 | 추적 엔트리와 공유자 비트 저장량 절감 |
| 특정 메모리 번지 집중 접근 시 특정 **홈 노드**로 핫스팟 라우팅 폭증 | **주소 해싱** 기반 홈 디렉터리 전 노드 분산 | 온칩 라우터 병목 해소 및 P2P 지연시간 균일화 |
| 소유권 요청과 Inval Ack 패킷이 온칩 라우터 큐를 가득 채워 **프로토콜 교착** | NoC 라우터 내 **가상 채널** 분리 및 자원 순서화 | 일관성 메시지 대기 교착 완벽 차단 및 Liveness 보장 |
