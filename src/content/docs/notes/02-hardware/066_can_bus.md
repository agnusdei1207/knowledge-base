...
- **1. 동시 ID 중재**: 여러 ECU가 ID 비트를 보내며 우성 비트를 읽어 작은 ID의 우선권 유지
- **2. 중재 패배·대기**: 송신값과 버스값이 다른 노드는 오류 없이 송신 중단
- **3. 승자 프레임 전송**: 중재 승자가 데이터와 CRC를 끊김 없이 전송
- **4. 프레임 검사·ACK**: 수신 노드가 형식·CRC를 검증하고 정상 시 ACK 우성 비트 송신
- **5. 재중재 기회 제공**: 버스 유휴 뒤 대기 노드가 보류 프레임으로 다시 경쟁
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 저순위 ID 기아 | 응답시간 분석·부하 상한 | **마감시간 보장** |
| 반복 **버스 오프** | 진단 후 제한 복귀 | **오류 격리** |
| 종단·배선 오류 | 종단·파형·길이 검증 | **신호 안정성 확보** |
| 메시지 위조 | 필터·인증·진단 통제 | **메시지 신뢰 강화** |
...
- **짧은 제어**는 CAN, **확장 데이터**는 CAN FD, 대용량은 이더넷
