...
date: "2026-07-25T10:10:00+09:00"
tags:
  - "notes-hardware"
weight: 53
extra:
...
- **정의/개념**: 공통·독립 명령 실행을 구분한 병렬 구조다
- **배경/필요성**: 작업 규칙성에 맞춰 처리 시간을 줄인다
...
- SIMD는 명령 제어를 공유해 해석 비용을 줄인다
- MIMD는 독립 제어로 다른 분기 작업을 함께 끝낸다
...
- SIMD는 명령을 한 번 해석해 활성 레인에 함께 적용한다
- MIMD는 코어별 명령을 실행하고 동기화로 결과를 합친다
...
| 판단 기준 | SIMD | MIMD |
|:---|:---|:---|
| 핵심 특징 | 공통 명령으로 여러 데이터 처리 | 독립 명령으로 여러 작업 처리 |
| 적용 기준 | 조밀 벡터·행렬·영상 | 서비스·그래프·비정형 분기 |
| 주요 위험 | 분기 발산·비연속 메모리 | 동기화·통신·부하 불균형 |
...
1. 영상 필터: SIMD 레인에 연속 픽셀을 배치한다
2. 웹 서버: MIMD 코어에 독립 요청을 배치한다

### 쉽게 이해하기 (학습용)

- 같은 보정식을 여러 픽셀에 한꺼번에 적용해 사진을 바꾼다
- 여러 작업자가 서로 다른 고객 요청을 독립적으로 처리한다

## Ⅶ. 결론

- 같은 연산은 SIMD, 독립 작업은 MIMD를 선택한다
