...
- 정의/개념: **SIMD•MIMD**를 결합해 규칙 연산과 독립 작업을 처리하는 병렬 구조
- 배경/필요성: 단일 실행 방식은 규칙•비정형 작업의 **혼합 병렬화 제약**

#### 한줄 요약

- **MIMD 작업 병렬•SIMD 데이터 병렬** 의 계층적 결합
...
- **1. 독립 작업 회수**: 서로 다른 제어 흐름을 큐에서 선택
- **2. MIMD 코어별 작업 배치**: 독립 작업을 실행 코어에 할당
- **3. SIMD 벡터 연산**: 공통 명령을 여러 데이터 레인에 적용
- **4. 코어별 결과 공유 기록**: 처리 결과를 공유 메모리에 반영
- **5. 배리어•후속 해제**: 참여 코어 도착 후 다음 작업 허용
