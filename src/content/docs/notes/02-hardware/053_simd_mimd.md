...
- 정의/개념: 코어 사이에는 독립 명령을, 코어 내부 레인에는 공통 명령을 발행하는 **계층형 MIMD·SIMD 병렬 구조**이다.
- 배경/필요성: SIMD만으로는 비정형 제어를, MIMD만으로는 규칙적인 데이터 연산을 효율적으로 처리하기 어려워 **혼합 병렬성 활용을 필요로 한다**.
...
- **SIMD 공통 명령**으로 여러 데이터 레인의 제어·해석 비용 절감이 핵심이다.
- **MIMD 독립 명령**으로 서로 다른 작업과 분기를 코어별 동시를 실행한다.
- **계층형 병렬성**으로 코어 간 작업 병렬성과 코어 내부 데이터 병렬성 결합이 핵심이다.
...
3. **SIMD 벡터 연산**: 각 코어의 규칙적인 구간에서 공통 명령을 여러 데이터 레인에 적용한다.
4. **코어별 결과 기록**: 작업 결과를 공유 메모리나 후속 작업의 입력 버퍼에 반영한다.
5. **의존성·배리어 해제**: 필요한 참여 코어가 결과 기록을 마치면 동기화 조건을 충족하고 후속 작업을 허용한다.
...
> 요약: 규칙 연산에는 **SIMD**, 독립 작업에는 **MIMD**을 선택한다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| SIMD 묶음의 데이터가 다른 분기를 선택해 **비활성 레인 증가** | 같은 경로의 데이터를 묶고 **활성 마스크 효율** 측정 | 유효 연산 레인 증가로 **SIMD 활용률 향상** |
| 벡터 레인이 떨어진 주소를 요청해 **메모리 트랜잭션 증가** | 자료 배치와 루프 순서를 **연속 주소 접근**에 맞게 변경 | 병합·벡터 전송 증가로 **메모리 대역폭 효율 향상** |
| MIMD 작업량이 코어마다 달라 동기화 지점에서 **유휴 코어 대기** | 작업을 더 작게 나누고 **동적 스케줄링**으로 유휴 코어에 재분배 | 작업 종료 시점 균등화로 **코어 대기 시간 감소** |
| SIMD·MIMD 경계마다 형식을 바꾸거나 복사해 **혼합 실행 비용 증가** | SIMD 규칙 구간을 넓히고 **경계 데이터 형식·배치 통일** | 변환·복사 횟수 감소로 **경계 오버헤드 완화** |
...
- 동일 연산에는 **SIMD**, 독립 제어에는 **MIMD**을 선택한다.

#### 한줄 요약

- 동일 연산은 SIMD, 독립 제어는 MIMD을 적용한다.
