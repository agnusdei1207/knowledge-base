...
sequenceDiagram
    ...
    Q->>S: ① 독립 작업·데이터 범위 전달
    par MIMD 작업 배치
        S->>A: ② 작업 A·독립 명령 흐름 배치
    and
        S->>C: ③ 작업 B·독립 명령 흐름 배치
    end
    par 코어 A의 데이터 병렬 구간
        A->>B: ④ 공통 벡터 명령 실행·결과 기록
    and 코어 B의 데이터 병렬 구간
        C->>B: ⑤ 다른 벡터 명령 실행·결과 기록
    end
    A->>B: ⑥ 작업 A 단계 완료
    C->>B: ⑦ 작업 B 단계 완료
...
> 요약: SIMD 레인과 MIMD 코어가 메모리 경로를 공유

**동작 원리**

- **① 독립 작업·데이터 범위 전달**: 서로 다른 제어 흐름의 작업 제출
- **② 작업 A·독립 명령 흐름 배치**: 코어 A가 자체 명령으로 실행
- **③ 작업 B·독립 명령 흐름 배치**: 코어 B가 다른 작업을 동시 실행
- **④ 공통 벡터 명령 실행·결과 기록**: 코어 A의 규칙 구간을 SIMD 처리
- **⑤ 다른 벡터 명령 실행·결과 기록**: 코어 B의 별도 규칙 구간을 SIMD 처리
- **⑥ 작업 A 단계 완료**: 결과 가시성 확보 후 배리어 도착
- **⑦ 작업 B 단계 완료**: 늦은 코어까지 후속 단계 대기
- **⑧ 전체 단계 완료·후속 작업 허용**: 모든 코어 도착 후 다음 작업 시작
