...
> 요약: 제어·결합·측정·보정 계층이 큐비트 상태를 운용한다.
...
- \(|\alpha|^2,|\beta|^2\): 이상적 계산 기저 측정에서 0과 1이 나올 확률
- 실제 잡음·혼합 상태는 밀도 행렬로 표현하고 판독 오류를 별도 보정한다.

**동작 원리**

1. **물리 회로 생성**: 논리 회로를 장치 연결성과 기본 게이트에 맞게 변환한다.
2. **초기화·회전**: 보정값을 적용한 펄스로 큐비트를 준비하고 회전시킨다.
3. **결합 제어**: 결합 펄스로 2큐비트 게이트를 실행한다.
4. **실행 확인**: 제어 계층이 게이트 실행 상태를 받는다.
5. **측정 지시**: 측정 대상·시점·판독 기준을 전달한다.
6. **판독 신호 인가**: 상태에 따라 다른 응답을 얻도록 신호를 넣는다.
7. **아날로그 응답**: 큐비트가 상태에 따른 판독 신호를 반환한다.
8. **고전 비트 변환**: 보정 기준으로 신호를 0·1 비트열로 바꾼다.
9. **통계 추정**: 여러 샷의 비트열로 확률·기댓값·불확실성을 계산한다.
...
> 요약: 큐비트는 진폭을 변환하고 측정으로 고전값을 출력한다.
...
- 중첩·얽힘의 활용성과 오류율·회로 깊이·고전 대비 성능을 검증해 종단 이득이 있는 문제에 양자 컴퓨팅을 적용한다.
