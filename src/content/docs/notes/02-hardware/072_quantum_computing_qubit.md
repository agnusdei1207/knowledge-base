...
date: "2026-08-05T12:25:06+09:00"
tags:
  - "notes-hardware"
weight: 72
extra:
...
- 0•1의 **복소 확률 진폭•상대 위상**을 표현하는 정보 단위
...
1. **물리 게이트 매핑**: 논리 회로를 장치 연결•보정값에 맞춤
2. **큐비트 초기화**: 계산 기저의 기준 상태 준비
3. **양자 게이트 실행**: 제어 펄스로 중첩•간섭•얽힘 생성
4. **측정•고전 비트 변환**: 양자 상태를 0•1 결과로 투영
5. **반복 통계•고전 후처리**: 샷 빈도로 결과 확률 추정

#### 한줄 요약

- **물리 게이트 매핑•샷 통계** 기반 양자 결과 추정
...
- **오류 통제•고전 기준선 우위**를 충족할 때 큐비트 적용
