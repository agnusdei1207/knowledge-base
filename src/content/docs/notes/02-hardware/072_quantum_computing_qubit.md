...
- **정의/개념**: 0·1 기저의 진폭과 위상을 담는 양자 정보 단위
- **배경/필요성**: 고전 비트는 중첩·얽힘을 못 담아 큐비트가 필요함
...
- 진폭·상대 위상이 측정 확률과 간섭을 결정한다.
- 다중 큐비트 게이트가 얽힌 상관을 생성한다.
- 결맞음 시간·게이트 오류율이 회로 깊이를 제한한다.
...
| 판단 기준 | 큐비트 | 고전 비트 |
|:---|:---|:---|
| 핵심 특징 | 진폭·위상·얽힘을 게이트로 변환 | 확정된 0·1을 논리 게이트로 변환 |
| 적용 기준 | 오류율을 통제하고 고전 계산보다 이득일 때 | 범용·정확 계산이 필요할 때 |
| 주요 위험 | 결맞음 손실·게이트·판독 오류 | 병렬 탐색·양자 모사 효율 한계 |
...
1. 분자 모사는 큐비트 측정으로 에너지 추정
2. 양자 최적화는 측정 분포로 해 후보 탐색
