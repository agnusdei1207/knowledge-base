...
- 정의/개념: 0과 1 계산 기저의 **복소 확률 진폭•상대 위상**으로 상태를 표현하고 측정하면 고전 비트를 내는 양자 정보 단위이다.
- 배경/필요성: 여러 양자계의 상태 공간은 큐비트 수에 따라 지수적으로 커지므로 고전 비트만으로 직접 모사하는 비용이 급증한다.

#### 한줄 요약

- 큐비트는 0과 1의 **복소 확률 진폭•상대 위상**을 표현하는 양자 정보 단위이다.
...
- **진폭 절댓값 제곱**이 측정 확률을 결정한다.
- **상대 위상•양자 간섭**으로 정답 확률 증폭이 핵심이다.
- **얽힘**으로 고전적 분리 불가능한 상관 표현이 핵심이다.

#### 한줄 요약

- **진폭•위상•얽힘** 변환으로 목표 측정 확률 증폭이 핵심이다.
...
- **제어•결합•측정•보정 계층**을 결합한 큐비트를 처리한다.
...
> 요약: **확률 진폭 제곱**이 0•1의 **측정 확률**을 결정한다.
...
- 장치 대기와 **보정•회로 깊이•샷•후처리**를 모두 포함한 종단 시간으로 양자 이득을 검증해야 한다.
...
- **오류율•회로 깊이**를 통제한 종단 성능이 최선의 고전 기준선보다 나을 때 양자 방식을 적용한다.

#### 한줄 요약

- **오류 통제**와 고전 기준선 대비 우위를 함께 입증할 수 있는 문제에 큐비트를 적용한다.
