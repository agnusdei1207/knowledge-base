...
sequenceDiagram
    ...
    participant O as 액추에이터 인터페이스
    participant W as 워치독·복구 관리
    participant P as 물리 플랜트

    loop 고정 제어 주기
        S->>I: ① 물리 상태 샘플 전달
        I->>C: ② 타이머·인터럽트로 제어 작업 깨우기
        C->>M: ③ 목표값·이전 상태 조회
        M-->>C: ④ 제어 파라미터 반환
        C->>O: ⑤ 마감시간 안에 제어 출력 전달
        C->>W: ⑥ 정상 주기 완료·워치독 갱신
        alt 정상 마감시간 준수
            O->>P: ⑦ 액추에이터 물리 동작
        else 작업 정지·마감시간 초과
...
1. **① 물리 상태 샘플 전달**: 센서는 위치·속도·온도 등의 물리 상태를 제어 주기에 맞춰 입력 인터페이스에 전달한다.
2. **② 타이머·인터럽트로 제어 작업 깨우기**: 입력 변환이 끝나거나 주기 타이머가 만료되면 RTOS가 제어 작업을 정해진 우선순위로 실행한다.
3. **③ 목표값·이전 상태 조회**: 제어 작업은 메모리에서 목표값, 보정 계수와 이전 주기의 내부 상태를 읽는다.
4. **④ 제어 파라미터 반환**: 메모리는 검증된 설정과 상태를 작업에 제공하며 동시 갱신 규칙으로 일관성을 유지한다.
5. **⑤ 마감시간 안에 제어 출력 전달**: 제어 알고리즘은 센서값과 목표값의 오차를 계산하고 액추에이터에 적용할 출력값을 만든다.
6. **⑥ 정상 주기 완료·워치독 갱신**: 출력과 상태 갱신이 끝난 정상 작업만 워치독을 갱신해 시스템이 살아 있음을 증명한다.
7. **⑦ 액추에이터 물리 동작**: 마감시간을 지킨 출력은 구동 신호로 변환되어 모터·밸브 등의 물리 플랜트에 적용된다.
8. **⑧ 하드웨어 안전 출력 적용**: 작업이 멈추거나 기한을 넘겨 워치독이 만료되면 소프트웨어와 독립된 관리 회로가 미리 정한 안전 출력을 강제한다.
9. **⑨ 플랜트를 안전 상태로 전환**: 액추에이터 인터페이스는 정지·제한·차단 신호를 내보내 플랜트 위험을 줄인다.
10. **⑩ 제어 코어 리셋·복구 부팅**: 안전 출력이 유지되는 동안 워치독은 제어 코어를 리셋하고 검증된 펌웨어 이미지로 복구를 시도한다.
