...
- **1. 센서 상태 전달**: 제어 주기 작업 시작
- **2. 제어 상태 조회**: 검증된 목표값과 이전 상태를 메모리에서 확보
- **3. 제어 출력 전달**: 센서값과 설정으로 계산한 명령을 마감시간 안에 적용
- **4. 정상 완료 보고**: 제어 주기 완료를 워치독에 알려 타이머 갱신
- **5. 시간 초과 복구**: 안전 출력 적용 후 검증 이미지로 재부팅
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 실행 시간이 마감시간 초과 | WCET 분석과 부하 시험 | **결정적 응답 확보** |
| 워치독 주기 부적절 | 최악 시간 기반 주기 설정 | **고장 탐지 개선** |
| OTA 중 전원 상실 | A/B 전환과 롤백 적용 | **부팅 복원력 확보** |
