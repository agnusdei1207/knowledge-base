...
date: "2026-07-25T10:24:00+09:00"
tags:
  - "notes-hardware"
weight: 60
extra:
...
- **마이크로컨트롤러(Microcontroller Unit, MCU)**: ‘엠시유’로 읽고 영문 머리글자를 딴 약어이며, 코어·메모리·주변장치를 한 칩에 통합
- **마이크로프로세서(Microprocessor Unit, MPU)**: ‘엠피유’로 읽고 영문 머리글자를 딴 약어이며, 외부 메모리와 고기능 운영체제를 사용
- **시스템온칩(System on Chip, SoC)**: ‘에스오시’로 읽고 영문 머리글자를 딴 약어이며, 연산·가속·주변 기능을 한 다이에 통합
- **실시간 운영체제(Real-Time Operating System, RTOS)**: ‘알티오에스’로 읽고 영문 머리글자를 딴 약어이며, 작업을 마감시간에 맞춰 스케줄링
- **마감시간(Deadline)**: 작업 결과가 완료되어야 하는 시점
- **인터럽트(Interrupt)**: 장치 이벤트를 처리 코어에 알리는 비동기 신호
- **워치독(Watchdog)**: 응답 없는 펌웨어를 감지해 재시작
- **원격 업데이트(Over-the-Air, OTA)**: ‘오티에이’로 읽고 영문 머리글자를 딴 약어이며, 네트워크로 펌웨어 이미지를 갱신
- **A/B 파티션(A/B Partition)**: 신규 펌웨어 실패 시 이전 이미지로 복귀
- **하드웨어 연동 검증(Hardware-in-the-Loop, HIL)**: ‘에이치아이엘’로 읽고 영문 머리글자를 딴 약어이며, 실제 제어기와 모의 플랜트를 연결해 입출력을 검증
...
- **정의/개념**: 특정 기능용 하드웨어·펌웨어 시스템이다
- **배경/필요성**: 제한 자원에서 시간·전력 목표를 지킨다
...
- 전용 기능 통합은 자원 비용을 줄이지만 변경을 제한한다
- 시간·전력 제약은 HW·SW 경계를 바꿔 마감을 지킨다
...
| 판단 기준 | MCU 기반 임베디드 시스템 | MPU·SoC 기반 임베디드 시스템 |
|:---|:---|:---|
| 핵심 특징 | 온칩 자원의 저전력 실시간 제어 | 외부 메모리·운영체제의 고기능 처리 |
| 적용 기준 | 센서·모터·단순 통신 제어 | 화면·네트워크·추론·대용량 처리 |
| 주요 위험 | 자원 한계·기능 결합 | 지연 변동·전력·긴 부팅 |
...
1. 모터 제어기는 RTOS 제어 주기를 HIL로 검증한다
2. 게이트웨이는 A/B 파티션으로 OTA 실패를 복구한다

### 쉽게 이해하기 (학습용)

- 모터 제어기는 모의 기계로 정해진 시간 안에 움직이는지 확인한다
- 게이트웨이는 새 프로그램이 멈추면 이전 영역으로 돌아간다

## Ⅶ. 결론

- 시간·전력은 MCU, 화면·통신은 MPU·SoC를 택한다
