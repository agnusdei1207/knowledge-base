---
sidebar:
  order: 7
  label: "007. 파이프라인 해저드: 데이터•제어•구조 (Pipeline Hazards)"
  badge:
    text: "기출 • 70%"
    variant: note
title: "파이프라인 해저드: 데이터•제어•구조 (Pipeline Hazards)"
date: "2026-08-05T11:30:00+09:00"
tags:
  - "notes-hardware"
weight: 7
extra:
  question_no: "007"
  source_status: "기출"
  source_history: "122회, 135회"
  priority: 70
  priority_note: "반복 기출, 데이터•제어 대응 핵심"
---

## Ⅰ. 개요

<details>
<summary>핵심 용어</summary>

- **파이프라인 해저드** 는 데이터 의존성•분기 결정 지연•자원 충돌 때문에 명령어의 중첩 실행이 지연되거나 잘못될 수 있는 조건이다.
- **정체 제어**: 충돌이 해소될 때까지 파이프라인 진행을 멈추는 방식
- **우회 제어**: 연산 결과를 의존 명령의 입력으로 직접 전달하는 방식
- **복구 제어**: 잘못 진행한 명령을 제거하고 올바른 경로를 복원하는 방식

</details>

- 정의/개념: **파이프라인 해저드** 는 데이터 의존•분기 미확정•자원 경합으로 중첩 실행이 정체되거나 잘못될 수 있는 **충돌 조건**
- 배경/필요성: 중첩 실행은 순차 실행에 없던 시간상 충돌을 만들므로 원인별 **정체•우회•복구 제어가 필요**

#### 한줄 요약
- 파이프라인 정체의 원인을 데이터 의존, 분기 지연, 자원 경합으로 구분해야 포워딩•예측•자원 복제 중 맞는 대응을 선택할 수 있다.

## Ⅱ. 특징

<details>
<summary>핵심 용어</summary>

- **명령어당 사이클 수(Cycles Per Instruction, CPI)**: 명령어 하나를 완료하는 데 필요한 평균 클록 수
- **플러시** 는 잘못 예측한 경로의 명령어를 무효화하고 올바른 분기 주소부터 파이프라인을 다시 채우는 동작이다.
- **의존 충돌**: 앞 명령의 결과가 준비될 때까지 후속 명령이 대기하는 상태
- **자원 충돌**: 공유 하드웨어가 빌 때까지 후속 명령이 대기하는 상태

</details>

- 단계 중첩 자체가 만드는 **의존•자원 충돌**
- 충돌 한 번이 **CPI 증가** 로 처리량 직접 감소
- 분기 판정이 늦을수록 증가하는 **오경로 명령 폐기량**

#### 한줄 요약
- 단계 중첩은 의존•자원 충돌을 만들며 정체와 플러시는 평균 CPI를 높인다.

## Ⅲ. 구조 및 구성요소

<details>
<summary>핵심 용어</summary>

- **해저드 감지기** 는 원천•목적 레지스터와 자원 사용을 비교하여 정체•포워딩•플러시 여부를 결정한다.
- **포워딩 경로** 는 앞 명령의 결과를 레지스터 기록 전에 후속 명령의 실행 입력으로 직접 전달한다.
- **자원 중재기** 는 같은 하드웨어 자원을 요구하는 명령 사이에서 사용 순서를 정한다.
- **데이터 경로** 는 명령의 값이 파이프라인 단계와 레지스터•연산기 사이를 이동하는 하드웨어 경로이다.
- **분기 제어** 는 분기 예측과 판정 결과에 따라 다음 인출 주소와 플러시 신호를 결정한다.

</details>

```text
                         [포워딩 경로]
                               |
                               |
[파이프라인 단계] -- [해저드 감지기] -- [자원 중재기]
                               |
                               |
                          [분기 제어]
```

선의 의미: 해저드 감지기를 중심으로 파이프라인 단계와 데이터•자원•분기 완화 회로가 연결된 정적 제어망이다.

| 구성요소 | 책임 |
|:---|:---|
| 파이프라인 단계 | 중첩 명령의 **데이터 경로 실행** |
| 해저드 감지기 | 의존•충돌 판정과 **정체 제어** |
| 포워딩 경로 | 레지스터 기록 전에 **후속 실행 입력으로 결과 전달** |
| 자원 중재기 | 공유 자원의 **사용 순서 결정** |
| 분기 제어 | 인출 주소 선택과 **플러시 제어** |

#### 한줄 요약
- 감지기가 원인을 판정하면 포워딩•중재•분기 제어가 파이프라인에 맞는 완화 신호를 보낸다.

## Ⅳ. 흐름도

<details>
<summary>핵심 용어</summary>

- **원천 레지스터**: 명령어가 피연산자를 읽는 레지스터
- **목적 레지스터**: 명령어가 결과를 기록하는 레지스터
- **완화 제어 신호** 는 감지한 해저드 유형에 맞춰 포워딩•예측•중재 동작을 지정하는 신호이다.
- **정체 신호** 는 충돌이 해소될 때까지 특정 파이프라인 단계와 레지스터의 상태 갱신을 멈춘다.

</details>

```text
[중첩 명령의 의존•분기•자원 정보]
              |
              v
1. 해저드 원인 판정
              |
              +-- 데이터 --> 2. 포워딩•정체 선택
              |
              +-- 제어 ----> 3. 예측•플러시 선택
              |
              `-- 구조 ----> 4. 자원 복제•중재 선택

[선택한 완화 제어]
              |
              v
5. 파이프라인 상태 반영
              |
              v
[정확한 명령 진행]
```

**동작 원리**

- **1. 해저드 원인 판정**: 의존 레지스터•분기•자원 요구 비교
- **2. 포워딩•정체 선택**: 데이터 준비 시점에 맞는 우회•대기
- **3. 예측•플러시 선택**: 분기 방향 추정과 오경로 복구
- **4. 자원 복제•중재 선택**: 동시 사용 경합 완화
- **5. 파이프라인 상태 반영**: 정체•우회•플러시 신호 적용

#### 한줄 요약
- 해저드 제어 회로는 충돌 원인에 따라 우회•정체•중재•플러시 신호를 파이프라인에 전달한다.

## Ⅴ. 종류 및 비교

<details>
<summary>핵심 용어</summary>

- **데이터 해저드** 는 앞 명령이 생성할 값을 뒤 명령이 먼저 읽으려 할 때 발생한다.
- **제어 해저드** 는 분기 방향과 대상 주소가 확정되기 전에 후속 명령을 인출하여 발생한다.
- **구조적 해저드** 는 여러 명령이 같은 메모리 포트나 연산기를 같은 클록에 요구하여 발생한다.
- **쓰기 후 읽기(Read After Write, RAW)**: 앞 명령의 기록 전에 뒤 명령이 같은 위치를 읽는 의존
- **추측 인출** 은 분기 결과가 확정되기 전에 예측한 경로의 명령을 미리 가져오는 동작이다.
- **자원 복제** 는 동시에 요구되는 포트나 연산기를 추가해 구조적 해저드를 줄이는 방법이다.

</details>

| 파이프라인 해저드 | 데이터 해저드 | 제어 해저드 | 구조적 해저드 |
|:---|:---|:---|:---|
| 적용 기준 | 후행 명령의 **선행 결과** 참조 | 분기 **방향•대상** 미확정 | 동일 **포트•연산기** 요구 |
| 핵심 특징 | **RAW** 읽기 순서 충돌 | 확정 전 **추측 인출** | 단일 자원의 **동시 경합** |
| 한계 | 적재-사용의 **잔여 정체** | 오예측 시 **플러시** | 자원 복제 전 **직렬 대기** |

> 요약: 원인별 **포워딩•예측•자원 복제** 로 대응

#### 한줄 요약
- 값 지연은 포워딩, 분기 불확실성은 예측, 자원 경합은 복제•중재로 완화한다.

## Ⅵ. 실무 고려사항 및 대책

<details>
<summary>핵심 용어</summary>

- **적재-사용 의존** 은 메모리 적재값이 준비되기 전에 후속 명령이 이를 요구하여 포워딩 뒤에도 정체가 남는 의존성이다.
- **분기 대상 버퍼** 는 이전 분기의 주소와 이동 대상을 저장하여 예측한 다음 인출 주소를 빠르게 제공한다.
- **명령 스케줄링** 은 의존성이 없는 명령을 대기 구간에 배치해 파이프라인 정체를 줄이는 기법이다.
- **분기 예측기** 는 과거 분기 이력 등을 이용해 분기 방향을 확정 전에 추정하는 하드웨어이다.
- **다중 포트**: 자원에 동시에 접근할 수 있도록 복수 통로를 제공하는 구조
- **중재 정책**: 동시 자원 요청의 처리 순서를 정하는 규칙
- **실행(Execute, EX) 결과 직접 전달**: 실행 결과를 후속 EX 입력으로 우회시키는 포워딩

</details>

| 문제 | 대책 | 효과 |
|:---|:---|:---|
| **RAW 의존** 으로 데이터 정체 증가 | **포워딩•명령 스케줄링** 적용 | **레지스터 기록 대기** 감소 |
| **적재-사용 의존** 의 잔여 버블 | **해저드 감지•독립 명령 재배치** | **적재-사용 정체** 완화 |
| **분기 오예측•플러시** 증가 | **예측기•분기 대상 버퍼**•조기 판정 개선 | **제어 해저드 비용** 감소 |
| **단일 포트•연산기 경합** 증가 | 자원 복제•**다중 포트•중재 정책** 적용 | **구조적 해저드** 감소 |
| **EX 결과** 의 레지스터 기록 대기 | 후속 EX 입력으로 **결과 직접 전달** | **RAW 정체** 감소 |

#### 한줄 요약
- 해저드 유형별 발생 횟수와 추가 CPI를 측정해야 면적•전력 비용 대비 가장 큰 완화 효과를 내는 회로를 선택할 수 있다.

## Ⅶ. 결론

<details>
<summary>핵심 용어</summary>

- **면적•전력 비용** 은 완화 회로를 추가하면서 늘어나는 칩 공간과 에너지 소비이다.

</details>

- RAW는 **포워딩**, 분기는 **예측**, 자원 경합은 복제 선택

#### 한줄 요약
- 해저드 완화 회로는 CPI 감소 효과와 면적•전력 비용을 함께 비교해 선택한다.
