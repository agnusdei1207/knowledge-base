...
date: "2026-08-10T12:48:07+09:00"
tags:
  - "notes-hardware"
weight: 7
extra:
...
- 정의/개념: 파이프라인 처리 중 선후 명령어 간 피연산자 의존(Data), 분기 조건 미확정(Control), 하드웨어 자원 경합(Structural)으로 인해 파이프라인의 정상적인 연속 실행 흐름이 정지되는 **파이프라인 해저드(Pipeline Hazard)**.
...
- 발생 원인(Data, Control, Structure)에 따라 **포워딩(Forwarding)**, **분기 예측(Branch Prediction)**, **자원 복제(Resource Duplication)** 등 기법을 차별 적용.
- 파이프라인 해저드로 인해 발생하는 **버블(Bubble)** 및 **플러시(Flush)** 주기가 누적될수록 실제 프로세서의 **명령 처리량(Throughput)** 지연 초과.
- 이상적(Ideal) 환경의 $\text{CPI} = 1$ 대비, 해저드 발생 빈도와 스톨 주기에 비례하여 실제 **명령어당 사이클 수(CPI)** 오버헤드가 누적 증가함.
...
|:---|:---|:---|
| **해저드 감지기** | 파이프라인 레지스터의 레지스터 번호 비트열 비교 | RAW 해저드 및 Load-Use 의존성을 ID 단계에서 즉각 식별 |
| **포워딩 경로** | EX/MEM, MEM/WB 레지스터 출력을 ALU 입력 MUX로 우회 | 레지스터 쓰기 대기 지연 없이 동일 주기에 데이터 연속 연산 |
| **분기 제어기** | ALU 조건 결과와 BTB 예측 주소 매칭 검증 | 오예측 시 IF/ID 레지스터 무효화(Flush) 및 Correct PC 주소 재설정 |
| **자원 중재기** | 단일 포트 메모리 및 공유 버스 점유 요청 중재 | 충돌 시 선행 명령 우선 처리 후 후속 명령 1클록 Stall 제어 |
...
1. **해저드 탐지**: **해저드 감지기**가 IF/ID 및 ID/EX 레지스터의 **원천 레지스터(Rs)** 번호와 EX/MEM, MEM/WB의 **목적 레지스터(Rd)** 번호 및 메모리 접근 신호를 비교함.
2. **완화 방식 결정**: 
...
3. **상태 갱신**: 제어 신호에 의거하여 파이프라인 레지스터의 래칭 여부 및 NOP 주입을 완료하고 가동을 계속함.
...
| **주요 대책** | **포워딩(Forwarding)**, 컴파일러 스케줄링, Stall | **분기 예측기**, BTB, Branch Target Early Compute | **자원 복제** (Harvard Architecture, Dual-Port Register) |
...
| **적재-사용 의존(Load-Use Dependency)** 발생 시 포워딩만으로 지연 해소 불가 | 1-Cycle **스톨(Stall)** 주입 및 컴파일러 **명령 스케줄링** 연동 | Load-Use 지연 슬롯을 무해한 연산으로 채워 체감 지연 제거 |
| 조건 분기 시 타깃 주소 지연 및 미스로 인한 깊은 파이프라인 **플러시** 손실 | **분기 예측기(Branch Predictor)**, **BTB** 및 ID 단계 **조기 판정** 회로 도입 | 분기 예측 정확도 95% 이상 달성 및 Flush 페널티 대폭 감소 |
...
- **완화 회로 선택 기준(Hazard Mitigation Design Criteria)**에 의거하여 파이프라인 설계 시 데이터 해저드를 위한 포워딩 하드웨어와 적재-사용 은닉 스케줄러를 우선 구성하고, 제어 해저드 대응을 위한 동적 분기 예측기(BTB) 및 구조적 해저드 차단을 위한 하버드 캐시 자원 복제 체계 적용 필수.

#### 한줄 요약
- Forwarding Unit, Branch Target Buffer 및 Resource Duplication을 종합 구성하여 Pipeline Hazard로 인한 CPI 상승을 차단하는 통합 최적화 체계 적용.
