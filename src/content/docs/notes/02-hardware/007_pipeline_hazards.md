...
- **정체·우회·복구 제어**는 충돌 시 단계를 멈추거나 결과를 직접 전달하고 잘못 진행한 명령을 제거하는 대응 방식이다.
...
- **의존·자원 충돌**은 앞 명령의 결과나 공유 하드웨어가 준비될 때까지 후속 명령을 진행할 수 없는 상태이다.
...
- **RAW**는 앞 명령의 결과 기록 전에 뒤 명령이 같은 위치를 읽으려는 읽기 후 쓰기 의존이다.
...
- **다중 포트·중재 정책**은 자원 접근 통로를 늘리고 동시 요청의 처리 순서를 정해 경합을 완화한다.
- **EX 결과 직접 전달**은 실행 단계의 결과를 레지스터 기록 전에 후속 EX 입력으로 우회시키는 포워딩이다.
...
- **포워딩·예측·자원 복제**는 각각 데이터 의존·분기 불확실성·하드웨어 경합을 완화하는 대표 기법이다.
