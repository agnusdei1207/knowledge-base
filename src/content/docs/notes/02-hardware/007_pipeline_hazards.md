...
date: "2026-07-25T16:25:00+09:00"
tags:
  - "notes-hardware"
weight: 7
extra:
...
- **쓰기 후 읽기(Read After Write, RAW)**: 앞 명령이 쓸 값을 뒤 명령이 읽어야 하는 의존이며, 데이터 해저드의 대표 형태다
- **적재-사용 의존(Load-Use Dependency)**: 메모리에서 값을 읽어 오는 명령 바로 다음에 그 값을 쓰는 명령이 오는 경우이며, 값이 MEM 단계에서야 나오므로 포워딩을 써도 한 클록은 못 메운다
- **명령어당 사이클(Cycles Per Instruction, CPI)**: 명령 하나를 끝내는 데 평균 몇 클록이 드는지를 나타내며, 해저드가 늘수록 함께 커진다
...
- **실행 단계(Execute, EX)**: 산술·논리 연산과 주소 계산을 수행하는 파이프라인 단계이며, 포워딩의 출발점이 되는 자리다

## Ⅰ. 개요

- **정의/개념**: 자원·의존·분기 충돌로 단계 중첩이 막혀 **CPI**가 커지는 현상
...
|:---|:---|:---|:---|
| 적용 기준 | 후행 명령이 선행 결과를 읽을 때 | 조건 분기로 **분기 대상**이 미확정일 때 | 같은 포트·연산기를 동시 요구할 때 |
| 핵심 특징 | **RAW** 의존에 따른 읽기 순서 충돌 | 대상 확정 전 **선행 인출** 진행 | 단일 포트·연산기 **자원 경합** |
| 한계 | **적재-사용 의존**은 포워딩으로도 잔여 정체 | 오예측 시 **플러시**로 인출분 전량 폐기 | 자원 복제 없이는 **직렬 대기** 불가피 |
...
1. 프로세서 데이터 경로는 **EX 결과**를 후속 명령 입력으로 우회
2. 반복 분기 코드는 **분기 예측기** 이력으로 플러시 축소

### 쉽게 이해하기 (학습용)
- 앞 명령이 방금 계산해 낸 값을 굳이 레지스터에 적었다가 다시 꺼내 오면 두세 박자를 버리게 되므로, 연산기 출구에서 곧장 뒤 명령의 입력단으로 선을 하나 더 뽑아 그 사이를 건너뛴다
- 반복문 끝의 조건 분기는 거의 매번 같은 쪽으로 가므로, 지난번에 어디로 갔는지 표에 적어 두었다가 그쪽으로 먼저 채워 넣으면 버리는 명령이 크게 줄어든다

## Ⅶ. 결론

- **CPI 감소량**이 면적·전력 증가를 넘을 때 대응 회로 채택
