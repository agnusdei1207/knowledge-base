...
date: "2026-07-28T21:56:03+09:00"
tags:
  - "notes-hardware"
weight: 7
extra:
...
- **① 해저드 상태 검사**: 인접 명령의 원천·목적 레지스터, 분기 상태와 동시 자원 요청 수집
- **② 해저드 유형 판정**: 충돌 원인을 데이터 의존·제어 흐름 미확정·구조 자원 경합으로 분류
- **③ 완화 제어 실행**: 유형에 따라 포워딩 또는 정체, 예측·플러시, 자원 중재·직렬화 신호 생성
- **④ 파이프라인 상태 갱신**: 선택한 우회값·다음 인출 주소·버블·자원 순서를 단계 레지스터에 반영
