...
- 정의/개념: SoC 내부 관리자와 대상 기능 블록 사이의 주소•데이터•응답 거래 규칙을 표준화한 **Arm 인터페이스 규격군**이다.
- 배경/필요성: 기능 블록마다 독자 인터페이스를 사용하면 연결 회로와 검증 환경을 반복 개발해야 하므로 통합 비용이 증가한다.

#### 한줄 요약

- AMBA는 SoC 기능 블록 사이의 **거래•응답 인터페이스**를 역할과 대역폭에 맞게 표준화한다.
...
- **VALID•READY 독립 채널**과 **미완료 거래**로 AXI 병렬을 처리한다.
- **AHB 주소•데이터 파이프라인**으로 중간급 시스템을 연결한다.
- **APB 설정•접근 2단계**로 저속 제어 레지스터를 연결한다.

#### 한줄 요약

- **처리량•거래 복잡도**에 따른 AXI•AHB•APB 계층화가 핵심이다.
...
- **고속 상호 연결망•프로토콜 브리지**의 계층형 SoC을 연결한다.
...
2. **대상 프로토콜 실행**: 선택한 경로에서 AXI•AHB 규칙을 수행하거나 브리지가 요청을 APB 설정•접근 단계로 변환한다.
3. **응답 변환•순서 정합**: 대상의 완료•오류 응답을 원래 프로토콜과 순서 규칙에 맞춰 관리자에게 반환한다.

#### 한줄 요약

- 요청은 **주소 라우팅과 필요한 프로토콜 변환**을 거쳐 대상에서 실행되고 응답 순서에 맞게 완료된다.
...
- **거래 순서•주소 지도•클록 도메인 경계**를 함께 검증하여 응답 오배달과 준안정을 방지한다.
...
- 고대역•다중 거래는 **AXI**, 중간•저속 경로는 **AHB•APB**을 선택한다.

#### 한줄 요약

- 기능 블록의 **대역폭•동시 거래 수•제어 역할**에 따라 AXI, AHB 또는 APB를 선택한다.
