...
- 글로벌 시스템 반도체(SoC) IP 상호연결의 사실상 표준 온칩 버스 규격(AXI/**AHB**/APB 계층 구조)으로 정착되었으며, 최근에는 대규모 매니코어 및 이종 칩렛(Chiplet) 환경을 수용하기 위해 패킷 기반 캐시 일관성 NoC인 AMBA **CHI(Coherent Hub Interface)** 및 CXL/UCIe 융합 구조로 진화 지속
