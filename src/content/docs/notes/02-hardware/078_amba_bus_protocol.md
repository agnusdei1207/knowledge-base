...
- **VALID·READY** 동시 활성화로 채널별 정보 전달
- **미완료 거래**로 응답 대기 중 후속 요청 병렬 처리
- **바이트 스트로브**로 쓰기 데이터의 유효 바이트 지정
...
- **1. 대상 브리지 전달**: 주소 디코딩 후 주소·데이터 거래 결합
- **2. APB 설정 단계**: PSEL과 주소·데이터를 한 주기 고정
- **3. APB 접근 단계**: PENABLE 활성화 후 PREADY까지 유지
...
- 고대역·다중 거래는 **AXI**, 중간 경로는 AHB, 저속 레지스터는 APB 선택
