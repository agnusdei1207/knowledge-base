...
1. **AXI 주소 발행**: AXI 관리자가 쓰기 주소 채널에 주소·속성과 AWVALID를 제시한다.
2. **주소 라우팅**: 상호 연결망이 주소 지도를 해석해 AXI–APB 브리지로 주소 채널을 전달한다.
3. **주소 수락**: 브리지가 주소를 내부에 보관할 수 있을 때 AWREADY로 수락한다.
4. **주소 핸드셰이크 완료**: 상호 연결망이 AWREADY를 관리자에게 전달해 주소 전송을 끝낸다.
5. **AXI 데이터 발행**: 관리자가 주소 채널과 독립된 쓰기 데이터 채널에 데이터·바이트 스트로브와 WVALID를 제시한다.
6. **데이터 라우팅**: 상호 연결망이 쓰기 데이터를 선택된 브리지로 전달한다.
7. **데이터 수락**: 브리지가 데이터를 저장할 수 있을 때 WREADY를 반환한다.
8. **데이터 핸드셰이크 완료**: 상호 연결망이 WREADY를 전달해 AXI 데이터 전송을 끝낸다.
9. **APB 설정 단계**: 주소와 데이터가 모두 준비되면 브리지가 PSEL, PADDR, PWRITE, PWDATA를 설정하고 PENABLE은 낮게 유지한다.
10. **APB 접근 단계**: 다음 클록 주기에 브리지가 주소·데이터를 유지한 채 PENABLE을 높인다.
11. **주변장치 완료 판정**: 주변장치는 준비될 때까지 PREADY를 낮춰 대기 상태를 연장하고, 완료 시 PREADY와 PSLVERR 상태를 반환한다.
12. **AXI 응답 변환**: 브리지가 APB 완료·오류 상태를 AXI B 채널의 BRESP로 변환해 BVALID와 함께 보낸다.
13. **응답 라우팅**: 상호 연결망이 원래 거래의 관리자에게 쓰기 응답을 전달한다.
14. **거래 종료**: 관리자가 BREADY로 응답을 수락하면 AXI 쓰기 거래가 완료된다.
