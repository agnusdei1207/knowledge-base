...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| AXI가 순서 무시(Out-of-Order)하고 데이터를 쏴댔더니 ID가 꼬여버려 시스템 전체가 데드락에 빠지는 재앙 | 시뮬레이션 단계에서 규약 위반을 철저히 검출하는 **프로토콜 체커** 지뢰밭 강력 설치 | 급격히 쏟아지는 트랜잭션 사이클 꼬임을 실시간 탐지해 칩이 뻗어버리는 사태 원천 차단 |
