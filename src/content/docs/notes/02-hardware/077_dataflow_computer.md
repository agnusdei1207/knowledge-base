...
- **1. 입력 토큰 전달**: 값·목적지·실행 태그를 매칭 저장소에 수신
- **2. 준비 노드 통지**: 같은 인스턴스의 모든 피연산자 충족 판정
- **3. 준비 연산 발행**: 실행 가능한 노드를 유휴 실행 유닛에 배정
- **4. 결과 토큰 반환**: 결과에 후속 목적지 태그를 붙여 의존 노드 활성화
- **5. 최종 결과 전달**: 종단 토큰을 외부 요구 순서로 정렬
