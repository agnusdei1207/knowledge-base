...
- 배경/필요성: 단일 프로세서 구조 → **초저전력 실시간 제어**와 **고성능 멀티태스킹** 상충 → 동시 만족 한계
...
|:---|:---|:---|
| 연산 코어 | 수십~수백 MHz 경량 코어로 **확정적 명령어 실행** | 수 GHz **고성능 멀티코어**로 대용량 스레드 병렬 처리 |
...
2. 마이크로초($\mu\text{s}$) 단위 하드 실시간성 및 WCET 확정보장이 필수적인가?
   ...
   │      │
   │      ▼
   │   3. 대형 디스플레이 GUI 및 범용 Rich OS가 동시에 필요한가?
   ...
   └── [아니오: 처리량 중심]
          │
          ▼
       3. 범용 OS 및 기가헤르츠(GHz)급 연산이 필요한가?
          ├── [예: 고성능 시스템] ── MPU 채택 (외장 DDR 연동)
          └── [아니오: 단순 제어] ── MCU 단독 채택
```

분기 결과: **실시간 결정론**은 MCU, **Rich OS/GUI**는 MPU, 인지와 제어 결합 시 **이종 하이브리드 SoC** 선정

#### 한줄 요약
- 1. 요구사항 분석 ➔ 2. 실시간성 판정 ➔ 3. OS 요구 판정 ➔ MCU/MPU/SoC 채택 순으로 선정한다.
...
- 실시간 제어는 **MCU**, OS 연산은 **MPU**, 복합 시스템은 **이종 하이브리드 SoC** 선정
