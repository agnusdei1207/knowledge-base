...
- **결정적 제어•범용 OS 기능**에 따른 MCU•MPU 구분
...
1. **하드 실시간 요구 판정**: 데드라인•지터 허용 범위 확인
2. **OS•응용 복잡도 판정**: MMU•대용량 메모리•서비스 요구 확인
3. **메모리•I/O•전력 검증**: 온칩 자원 또는 외부 확장 적합성 확인
4. **시간•장애 경계 검증**: 혼합 구성의 제어 독립성과 복구 확인

#### 한줄 요약

- **제어•복합 응용 요구**에 따른 MCU•MPU•혼합 구성 결정
...
- 결정적 제어는 **MCU**, 복합 OS는 **MPU**, 동시 요구는 분할

#### 한줄 요약

- **시간•메모리 요구**가 충돌하면 MCU•MPU 역할 분할
