...
### 도표안 B. MCU 실시간 제어·MPU 상위 서비스 협업 시퀀스

```mermaid
sequenceDiagram
    participant S as 센서
    participant P as MCU 온칩 주변장치
    participant R as MCU RTOS·제어 태스크
    participant A as 액추에이터
    participant C as MCU–MPU 통신 채널
    participant D as MPU 커널·드라이버
    participant O as MPU 서비스
    participant U as 사용자 응용

    S->>P: ① 센서 값·인터럽트 입력
    P->>R: ② 짧은 ISR 후 제어 태스크 기동
    R->>A: ③ 데드라인 내 제어 출력
    R->>C: ④ 상태·진단 텔레메트리 전달
    C->>D: ⑤ 프레임 수신·장치 이벤트 통지
    D->>O: ⑥ OS 인터페이스로 상태 전달
    O->>U: ⑦ 화면·네트워크용 상태 제공
    U->>O: ⑧ 승인된 설정 변경 요청
    O->>D: ⑨ 범위 검증 후 장치 명령 전달
    D->>C: ⑩ MCU 설정 프레임 송신
    C->>R: ⑪ 무결성 확인된 설정 전달
    R->>A: ⑫ 안전한 제어 주기 경계에서 설정 적용
```

#### 동작 원리

1. **현장 입력 수집**: 센서 신호가 MCU에 통합된 변환기·타이머·통신 주변장치로 들어온다.
2. **제어 태스크 기동**: 온칩 주변장치의 인터럽트를 짧게 처리한 뒤 RTOS가 우선순위 높은 제어 태스크를 깨운다.
3. **결정적 출력**: MCU 제어 태스크가 캐시·외부 메모리 변동을 최소화한 경로로 데드라인 안에 액추에이터를 구동한다.
4. **상태 전달**: 제어를 방해하지 않는 우선순위와 주기로 상태·진단 텔레메트리를 MPU 통신 채널에 보낸다.
5. **MPU 장치 수신**: MPU 커널 드라이버가 통신 프레임을 받고 상위 서비스에 장치 이벤트를 알린다.
6. **운영체제 중개**: 드라이버가 권한·버퍼 경계를 적용한 표준 OS 인터페이스로 상태를 서비스에 전달한다.
7. **복합 응용 제공**: MPU 서비스가 대용량 메모리·네트워크·그래픽 기능을 이용해 사용자 응용에 상태를 제공한다.
8. **설정 변경 요청**: 사용자 응용이 인증·권한 정책을 거쳐 제어 매개변수 변경을 요청한다.
9. **상위 검증**: MPU 서비스가 허용 범위·버전·장치 상태를 확인한 뒤 드라이버에 명령을 전달한다.
10. **MCU 명령 전송**: 드라이버가 순서 번호·무결성 정보를 포함한 설정 프레임을 MCU로 보낸다.
11. **제어 경계 검증**: MCU가 프레임의 출처·순서·범위를 다시 확인해 제어 태스크에 전달한다.
12. **안전한 설정 적용**: MCU가 현재 제어 주기를 중단하지 않고 정의된 주기 경계에서 새 설정을 적용한다.
...
- 산업 게이트웨이는 MCU에 센서·모터의 실시간 제어를 두고, MPU에 프로토콜 변환·원격 관리·화면을 분리해 OS 부하가 제어 데드라인에 전파되지 않게 한다.
...
- 처리 플랫폼을 선택할 때 **최악 데드라인·온칩 메모리·전력·MMU·범용 OS·외부 장치 확장 요구**를 비교하고, 결정적 제어는 MCU, 복합 응용은 MPU에 배치하되 경계 통신의 지연·안전성을 검증한다.
