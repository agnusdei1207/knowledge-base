...
| MPU 처리부 | 무식하게 코어를 수십 개 집중 적용하고 **MMU**까지 달아서 리눅스나 윈도우를 띄우는 고성능 멀티코어 프로세서 |
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 리눅스 깔린 MPU로 모터 돌렸더니 스케줄링 밀릴 때마다 모터가 덜덜 떨며 지터(Jitter)가 폭발하는 대참사 발발 | 뇌 비우고 모터만 1ms 단위로 칼같이 돌리는 전용 **MCU**를 옆에 하나 더 박아서 제어 전담시킴 | OS가 버벅대든 말든 모터는 한 치의 오차 없이 급격히 돌아가는 극강의 실시간 격리 달성 |
| MPU가 외부 램(DDR)이랑 통신하는데 선로 노이즈 때문에 데이터가 깨지는 **신호 무결성** 파괴 재앙 | 보드 깎을 때 임피던스(저항) 뼈 깎듯 맞추고, 주변에 구리선 적용하여 전자파 노이즈(EMI) 강제 차단 | 고속 메모리 뻥튀기해도 버스 에러 한 번 안 나는 철통 방어 회로 구축 완료 |
