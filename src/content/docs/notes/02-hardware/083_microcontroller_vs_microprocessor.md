...
- 배경/필요성: 임베디드/지능형 시스템 개발 시 단일 아키텍처로는 **초저전력 실시간 결정론적 제어(MCU)와 고성능 멀티태스킹/Rich OS 구동(MPU)이라는 상반된 엔지니어링 목표를 동시 충족할 수 없는 한계**를 극복하기 위해 명확한 기술 특성 비교 및 최적 프로세서 선정 필요
