...
- 배경/필요성: 하나의 프로세서 구조로는 주변장치까지 원칩에 담아 얻는 **초저전력 실시간 제어**와 외장 메모리·MMU를 붙여 얻는 **고성능 멀티태스킹**이 서로의 비용이 되어 함께 성립하지 않으므로, 집적도를 택한 MCU와 확장성을 택한 MPU로 용도를 갈라 배치
...
#### 한줄 요약
- 요구사항 분석 ➔ 실시간성·OS 판정 ➔ MCU·MPU·하이브리드 SoC 선정
