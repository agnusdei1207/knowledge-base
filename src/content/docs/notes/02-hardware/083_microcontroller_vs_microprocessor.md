...
sequenceDiagram
    ...
    P->>R: ① 센서 입력·짧은 ISR 후 태스크 기동
    R->>A: ② 데드라인 내 제어 출력
    R->>M: ③ 상태·진단 텔레메트리 전달
    M->>U: ④ 화면·네트워크용 상태 제공
    U->>M: ⑤ 승인된 설정 변경 요청
    M->>R: ⑥ 범위·무결성 검증된 설정 전달
    R->>A: ⑦ 제어 주기 경계에서 설정 적용
```
...
> 요약: MCU는 온칩 자원을 통합하고 MPU는 외부 자원을 확장한다.

**동작 원리**

- **① 제어 기동**: 센서 인터럽트를 짧게 처리한 뒤 제어 태스크 실행
- **② 결정적 출력**: MCU가 데드라인 안에 액추에이터 구동
- **③ 상태 전달**: 제어를 방해하지 않는 주기로 텔레메트리 전송
- **④ 복합 응용**: MPU가 화면·네트워크용 상태 제공
- **⑤ 설정 요청**: 사용자 응용이 인증·권한을 거쳐 변경 요청
- **⑥ 경계 검증**: MPU와 MCU가 범위·순서·무결성 재확인
- **⑦ 안전한 적용**: MCU가 정의된 제어 주기 경계에서 반영
...
> 요약: 제어 마감에는 MCU, 복합 응용에는 MPU를 선택한다.
