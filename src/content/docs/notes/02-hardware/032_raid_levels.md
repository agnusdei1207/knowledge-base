...
- **RAID**: 여러 디스크에 데이터를 분산하거나 중복해 처리량과 장애 가용성을 조절하는 구성
...
- **RAID 컨트롤러**: 논리 블록 매핑과 스트라이핑·중복·장애·재구축을 제어하는 장치나 소프트웨어
- **스트라이프 매퍼**: 호스트 논리 블록을 실제 멤버 디스크와 오프셋으로 변환하는 구성요소
- **중복 엔진**: 미러 복제와 XOR 패리티 생성·손실 데이터 복원을 수행하는 구성요소
...
- **XOR 패리티**: 여러 데이터 블록의 배타적 논리합으로 생성해 손실 블록을 복원하는 값
...
- **RAID 0·1·10**: 중복 없는 스트라이핑, 단순 미러링, 미러 쌍을 다시 스트라이핑한 구성
...
> 요약: **처리량•사용 용량**과 **허용 장애 수**로 레벨 선택
