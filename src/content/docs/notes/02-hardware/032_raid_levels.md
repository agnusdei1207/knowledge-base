...
date: "2026-07-28T12:51:48+09:00"
tags:
  - "notes-hardware"
weight: 32
extra:
...
| 설계 요소 | 입력·상태 | 역할 |
|:---|:---|:---|
| RAID 컨트롤러 | 호스트 I/O·레벨·장애 상태 | 배열 요청·장애 제어 |
| 스트라이프 매퍼 | 논리 블록·스트라이프 단위 | 멤버·오프셋 변환 |
| 중복 엔진 | 데이터·미러·패리티 상태 | 중복 생성·손실 데이터 복원 |
| 멤버 디스크·장애 도메인 | 데이터·중복 블록 | 독립 경계 저장 |
...
- 쓰기 비중이 높은 데이터베이스는 미러 쌍을 스트라이핑한 RAID 10으로 패리티 계산과 작은 쓰기 페널티를 피한다

## Ⅶ. 결론

- 저장 가용성을 위해 쓰기·장애 허용을 검토하여 **RAID** 활용
