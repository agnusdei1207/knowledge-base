...
date: "2026-07-24T13:30:30+09:00"
tags:
  - "notes-hardware"
weight: 32
extra:
...
- **독립 디스크 중복 배열(Redundant Array of Independent Disks, RAID)**: 여러 디스크에 데이터를 분산·중복해 성능과 가용성을 조절하는 배열
...
- **배타적 논리합(Exclusive OR, XOR)**: 입력 비트가 서로 다를 때 1을 내는 논리 연산으로서 RAID 5·6의 패리티 생성과 손실 데이터 복원에 사용됨
...
- **정의/개념**: 디스크 분산·중복으로 성능·가용성을 조절하는 배열
- **배경/필요성**: 단일 디스크 처리·장애 한계로 분산·중복 필요
...
|:---|:---|:---|:---|:---|:---|
| 최소 멤버 | 2 | 2 | 3 | 4 | 4·짝수 |
| 핵심 특징 | 스트라이핑 | 미러링 | 단일 분산 패리티 | 이중 분산 패리티 | 미러 쌍 스트라이핑 |
| 장애 허용 | 없음 | 미러 한 쪽 | 임의 1개 | 임의 2개 | 미러 쌍마다 1개 |
| 작은 쓰기 | 중복 비용 없음 | 한 미러 쌍 복제 | 단일 패리티 갱신 | 이중 패리티 갱신 | 미러 쌍 간 병렬 복제 |
| 적용 기준 | 임시·재생성 데이터 | 단순 복제·부팅 볼륨 | 읽기 중심·용량 효율 | 대형 배열·재구축 위험 | 쓰기 집중형 데이터베이스 |
| 주요 위험 | 장애 시 전체 손실 | 용량 절반 사용 | 재구축 중 추가 장애 | 쓰기 패널티 | 미러 쌍 동시 장애 |
...
2. 대형 백업 배열은 RAID 6으로 재구축 중 2장애 허용

### 쉽게 이해하기 (학습용)

- 장부 쓰기가 잦은 데이터베이스는 미러 쌍을 병렬 배치해 패리티 계산을 피한다
- 큰 백업 배열은 재구축 중 다른 디스크가 고장 나도 이중 패리티로 버틴다
