...
- **① 논리 블록 작은 쓰기 요청**: 전체 스트라이프보다 작은 새 데이터 전달
- **② 스트라이프·멤버 위치 계산 요청**: 논리 블록의 데이터·패리티 위치 계산
- **③ 데이터·패리티 멤버와 오프셋 반환**: 대상 멤버와 내부 위치 반환
- **④ 기존 데이터 블록 요청**: 패리티 차이 계산용 이전 데이터 읽기
- **⑤ 기존 데이터 반환**: 갱신 전 데이터 반환
- **⑥ 기존 패리티 블록 요청**: 같은 스트라이프의 이전 패리티 읽기
- **⑦ 기존 패리티 반환**: 갱신 전 XOR 결과 반환
- **⑧ 새 데이터 블록 쓰기**: 데이터 멤버에 새 값 기록
- **⑨ 데이터 쓰기 완료**: 새 데이터 보존 확인
- **⑩ XOR로 계산한 새 패리티 쓰기**: `이전 패리티 XOR 이전 데이터 XOR 새 데이터` 기록
- **⑪ 패리티 쓰기 완료**: 새 패리티 보존 확인
- **⑫ 호스트 쓰기 완료**: 데이터·패리티 기록 성공 후 완료 반환
...
| 핵심 특징 | 스트라이핑·**중복 없음** | 전체 미러링·**최대 \(N-1\)개 장애 허용** | 단일 분산 패리티·**1개 장애 허용** | 이중 분산 패리티·**2개 장애 허용** | 미러 쌍 스트라이핑·**쌍별 1개 허용** |
| 사용 가능 용량 | \(N \times S\) | \(S\) | \((N-1) \times S\) | \((N-2) \times S\) | \((N/2) \times S\) |
| 한계 | 한 멤버 장애 시 **전체 손실** | 사용 가능 **용량 절반** | 작은 쓰기의 **패리티 갱신** | 이중 패리티 **쓰기 패널티** | 같은 미러 쌍 **동시 장애** |

> 용량식은 용량이 같은 \(N\)개 디스크를 가정한다. RAID 1은 모든 멤버가 같은 데이터를 복제하고, RAID 10은 짝수 \(N\)으로 미러 쌍을 구성한다.
