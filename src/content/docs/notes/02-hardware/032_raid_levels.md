...
date: "2026-08-05T17:28:00+09:00"
tags:
  - "notes-hardware"
weight: 32
extra:
...
- **미러링**: 같은 데이터를 여러 디스크에 복제하는 방식
- **패리티**: 남은 데이터로 손실분을 계산할 중복 정보를 저장하는 방식
...
- RAID는 장치 장애를 견디지만 **백업** 을 대체하지 않음

## Ⅲ. 구조 및 구성요소

<details><summary>핵심 용어</summary>

- **RAID 컨트롤러**: 논리 블록 매핑과 중복•장애•재구축을 제어하는 구성요소
- **스트라이프 매퍼**: 호스트 논리 블록을 실제 멤버 디스크와 오프셋으로 변환하는 구성요소
- **배타적 논리합(Exclusive OR, XOR)**: 패리티 생성과 손실 데이터 복원에 사용하는 논리 연산
- **중복 엔진**: 미러 복제와 XOR 패리티 생성•복원을 수행하는 구성요소
- **입출력(Input/Output, I/O)**: 호스트와 디스크 배열 사이의 데이터 전달
...
- **XOR 패리티**: 여러 데이터 블록의 XOR로 생성해 손실 블록을 복원하는 값
...
- **2. 기존 데이터•패리티 조회**: 이전 XOR 값 확보
- **3. XOR 패리티 재계산**: 이전 값과 변경 값으로 새 패리티 생성
- **4. 변경 데이터•패리티 기록**: 두 멤버에 병렬 저장
...
- **재구축**: 중복 정보로 교체 멤버의 데이터를 복원하는 작업
- **핫 스페어**: 장애 멤버를 즉시 대체할 대기 디스크
- **쓰기 구멍**: 정전이나 부분 실패로 데이터와 패리티 중 일부만 기록돼 서로 불일치하는 문제
- **장애 도메인**: 전원•섀시처럼 구성요소가 함께 고장 날 수 있는 경계
- **백업**: 삭제•변조•배열 전체 손실에 대비한 독립 데이터 사본
...
- 쓰기 집중은 **RAID 10**, 미러 쌍은 장애 도메인 분리
