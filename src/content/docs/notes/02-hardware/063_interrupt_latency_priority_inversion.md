...
date: "2026-07-25T10:30:00+09:00"
tags:
  - "notes-hardware"
weight: 63
extra:
...
- **정의/개념**: IRQ 시작 지연과 고순위의 락 대기다
- **배경/필요성**: 두 지연을 분리해 데드라인 실패를 막는다
...
- 마스킹·상위 ISR은 시작을 늦춰 입력 처리를 놓치게 한다
- 중순위 선점은 락 대기를 늘려 고순위 마감을 놓치게 한다
...
|:---|:---|:---|
| 핵심 특징 | IRQ부터 ISR 시작까지의 지연 | 고순위 태스크의 저순위 락 대기 |
| 적용 기준 | 인터럽트 시작 지연 분석 | 공유 자원 차단 시간 분석 |
| 주요 위험 | 긴 마스킹·상위 ISR·예외 진입 | 중간순위 선점으로 차단 확대 |
...
1. 모터 제어기는 ISR 연산을 이관해 지연을 줄인다
2. 직렬 드라이버는 뮤텍스에 우선순위 상속을 쓴다

### 쉽게 이해하기 (학습용)

- 모터 제어기는 문지기에게 긴 계산을 맡기지 않아 다음 호출을 빨리 받는다
- 직렬 드라이버는 열쇠 소유자의 순번을 잠시 높여 급한 요청을 끝낸다

## Ⅶ. 결론

- IRQ는 ISR을 줄이고 락은 상속·천장으로 통제한다
