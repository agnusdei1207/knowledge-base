...
- **1. IRQ 요청 전달**: 비동기 사건 발생
- **2. ISR 후속 태스크 등록**: 인터럽트 서비스는 최소 작업만 하고 후속 처리를 준비 큐로 이관
- **3. 고순위 태스크 실행**: 스케줄러가 준비된 고순위 작업으로 문맥 전환
- **4. 공유 자원·역전 판정**: 고순위 요청이 저순위 소유 락에 막혔는지 확인
- **5. 우선순위 상속·자원 인계**: 락 소유자의 우선순위를 높여 임계 구역을 끝내고 자원 전달
