...
sequenceDiagram
    participant D as 장치
    participant I as 인터럽트 제어·예외 진입·ISR
    participant S as 스케줄러
    participant H as 고순위 태스크
    participant L as 저순위 태스크·뮤텍스

    D->>I: ① IRQ 인가
    I->>S: ② ISR 진입·원인 처리 후 태스크 깨움
    S->>H: ③ 고순위 태스크 실행권 배정
    H->>L: ④ 공유 자원 잠금 요청
    alt 저순위 태스크가 잠금 보유
        L-->>H: ⑤ 고순위 태스크 대기 전환
...
> 요약: IRQ 시작 경로와 락 차단 경로를 분리

**동작 원리**

- **① IRQ 인가**: 장치가 비동기 사건 전달
- **② ISR 진입·원인 처리 후 태스크 깨움**: 문맥·벡터 처리와 짧은 ISR 실행
- **③ 고순위 태스크 실행권 배정**: 준비된 후속 작업 선점 실행
- **④ 공유 자원 잠금 요청**: 후속 처리용 뮤텍스 획득 시도
- **⑤ 고순위 태스크 대기 전환·잠금 즉시 부여**: 소유 상태에 따라 차단 여부 결정
- **⑥ 소유자·대기자 우선순위 전달**: 역전 관계를 스케줄러에 통지
- **⑦ 고순위 우선순위 임시 상속**: 중순위 태스크의 추가 선점 억제
- **⑧ 임계 구역 완료·잠금 인계**: 저순위 소유자가 빠르게 자원 반환
- **⑨ 제어 결과 출력**: 고순위 태스크가 후속 처리 완료
...
- \(T_{\mathrm{mask,max}}\): 측정 구간의 최대 인터럽트 비활성 시간
- \(HP(\mathrm{IRQ})\): 대상 IRQ보다 먼저 실행 가능한 상위 우선순위 ISR 집합
- \(C_h\): 상위 ISR \(h\)의 최악 실행시간
- \(T_{\mathrm{entry}}\): 문맥 저장·벡터 조회 등 예외 진입시간
- 단일 코어에서 상위 ISR이 한 번씩 간섭하며 NMI·버스 지연은 별도 예산으로 관리한다고 가정

### 쉽게 이해하기 (학습용)

- 인터럽트 레이턴시는 ISR 시작 전 대기이고, 우선순위 역전은 ISR 후 고순위 태스크의 저순위 락 대기다
...
> 사례: 모터 제어기는 ISR에서 원인 확인과 태스크 기동만 수행하고 긴 연산은 후속 태스크로 이관한다.

### 쉽게 이해하기 (학습용)

- ISR은 짧게 끝내고 잠금 보유 시간을 제한한 뒤 최악 IRQ 지연과 락 차단을 다시 측정한다

## Ⅵ. 결론

- 인터럽트 비활성·ISR·락 보유 시간을 분석하고 **상속·천장 프로토콜**로 최악 지연 통제
