...
- **정의/개념**: 한 스레드의 독립 명령을 중첩 실행해 IPC를 높이는 병렬성
- **배경/필요성**: 단일 스레드의 의존 대기로 IPC가 제한돼 독립 명령 중첩 필요
...
- 실제 의존 사슬이 동시 실행 명령 수를 제한한다
- 독립 명령 부족은 발행 슬롯·IPC를 낮춘다
- 분기 실패·메모리 지연은 실행부를 대기시킨다
- 넓은 윈도는 ILP와 면적·전력을 함께 높인다
...
| 적용 기준 | 단일 스레드 지연·독립 명령 충분 | 독립 작업·요청 충분 | 규칙적 데이터·동일 연산 반복 |
...
| 판단 기준 | 동적 ILP | 정적 ILP |
|:---|:---|:---|
| 핵심 특징 | 하드웨어가 실행 중 스케줄 | 컴파일러가 실행 전 스케줄 |
| 적용 기준 | 가변 지연·기존 바이너리 | 규칙적 연산·재컴파일 가능 |
...
1. 다중 누산기: 의존 사슬 분할 후 최종 합산
2. IPC 포화 시 코어 수·벡터 폭 확장

### 쉽게 이해하기 (학습용)
- 합계를 여러 그릇에 나눠 계산한 뒤 마지막에 합친다
- 한 사람의 손이 모자라면 작업자나 한 번에 다룰 양을 늘린다

## Ⅶ. 결론

- ILP 포화 뒤 TLP·DLP로 자원을 전환한다
