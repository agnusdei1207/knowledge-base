...
date: "2026-07-30T18:37:55+09:00"
tags:
  - "notes-hardware"
weight: 9
extra:
...
- 독립 명령 부족·**분기 실패·메모리 지연**에 따른 발행 슬롯 활용률 저하
- 넓은 **명령 윈도**에 따른 면적·전력 증가
- 복수 발행 **슈퍼스칼라**로 동시 실행 폭 확대
- **비순서 실행**과 순차 커밋으로 상태 보존
...
sequenceDiagram
    ...
    N->>W: 2. 레지스터 리네이밍·의존성 등록
    loop 피연산자가 준비된 명령
        W->>E: 3. 준비 명령 선택·발행
        E->>C: 4. 실행 결과·완료 상태 전달
        E-->>W: 종속 명령 깨우기
    end
    C-->>F: 5. 순차 커밋·정밀 복구
    alt 분기 오예측 또는 예외
...
- **2. 레지스터 리네이밍·의존성 등록**: 가짜 의존 제거
- **3. 준비 명령 선택·발행**: 준비 연산 비순서 발행
- **4. 실행 결과·완료 상태 전달**: 결과 기록·종속 연산 해제
- **5. 순차 커밋·정밀 복구**: 상태 반영 또는 결과 폐기·재인출
