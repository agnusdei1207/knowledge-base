...
| 레지스터 리네이밍 | 논리·물리 레지스터 상태 | WAR·WAW 가짜 의존 제거 |
| 명령어 윈도 | 명령·피연산자 준비 상태 | 실행 가능한 발행 대상 선택 |
| 실행 유닛 | 발행 명령·피연산자 | 산술·메모리 연산 수행 |
| 재정렬 버퍼 | 완료 결과·프로그램 순서 | 순차 확정과 오류 복구 |

> 요약: 프런트엔드가 후보를 공급하고 ROB가 순서를 보존

**동작 원리**

- **① 명령 인출·해독 결과 전달**: 예측 경로의 명령을 해독해 리네이밍부에 전달
- **② 레지스터 리네이밍·의존성 등록**: 가짜 의존 제거 후 실제 의존을 명령어 윈도에 등록
- **③ 준비 명령 선택·발행**: 준비된 명령을 프로그램 순서와 무관하게 발행
- **④ 실행 결과·완료 상태 전달**: 결과를 ROB에 기록하고 종속 명령을 깨움
- **⑤ 프로그램 순서대로 결과 커밋**: 완료 결과를 원래 순서대로 아키텍처 상태에 반영
- **⑥ 잘못된 명령 제거·정확한 주소로 복구**: 오예측·예외 이후 결과를 폐기하고 재인출
