...
- 정의/개념: 다수 물리 큐비트의 **코드 공간에 부호화된 계산 단위**
- 기존 한계: 물리 큐비트 오류 누적은 **장시간 양자 계산**을 제한
...
- **안정자 증후군 측정**으로 논리 상태를 직접 관측하지 않고 오류 검출
- **논리 큐비트 부호화**로 물리 오류의 논리 전파 억제
- **코드 종류·거리**가 물리 큐비트·복호 자원 결정
...
- 신뢰할 수 있는 양자 연산을 수행하기 위해 **물리 큐비트 오류율·목표 논리 오류율·회로 깊이·오류정정 오버헤드**를 검토하고, 필요한 코드 거리와 논리 큐비트당 물리 자원을 산정한다
