...
date: "2026-07-31T10:11:07+09:00"
tags:
  - "notes-hardware"
weight: 75
extra:
...
- **오류 임계값(Error Threshold)**: 거리 증가로 논리 오류율이 감소하기 시작하는 물리 오류율 경계
...
- **결맞음 손실(Decoherence)**: 환경 상호작용으로 큐비트의 양자 상태가 훼손되는 현상
- **큐비트 보정(Qubit Calibration)**: 게이트·측정 오류가 작도록 제어 신호를 조율하는 작업
- **오류 지도(Error Map)**: 큐비트와 연결별 오류율을 위치에 대응시킨 자료
- **실패 예산(Failure Budget)**: 전체 계산에 허용할 실패 확률을 연산별로 배분한 한도
- **시공간 부담(Space-time Overhead)**: 논리 연산이 점유하는 큐비트 수와 실행 시간의 결합 비용

> **키워드:** 논리 큐비트와 물리 큐비트 (Logical vs Physical Qubit)
