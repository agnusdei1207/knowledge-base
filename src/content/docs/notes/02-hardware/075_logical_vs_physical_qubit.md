...
- 정의/개념: 여러 물리 큐비트에 정보를 부호화하고 반복 오류 정정으로 보호한 **논리 계산 단위**이다.
...
- **코드 종류•거리**는 필요한 물리 자원과 달성 가능한 논리 오류율을 결정한다.

#### 한줄 요약

- **코드 거리** 증가에 따른 논리 오류율 감소와 자원 증가한다.
...
- **데이터•보조 큐비트와 복호기**로 논리 큐비트를 구성한다.
...
- 장치의 물리 큐비트 수와 오류 정정 후 실제 계산에 사용할 수 있는 **논리 큐비트 수**를 구분해야 한다.
...
- **논리 작업량**을 코드 패치, 물리 큐비트, 복호기와 실행 시간을 포함한 시공간 부담으로 환산해야 한다.
...
- 목표 **논리 오류율**을 만족하는 코드 거리를 정하고 이에 필요한 물리 큐비트와 제어•복호 자원을 산정한다.

#### 한줄 요약

- 장치는 총 물리 큐비트 수보다 **오류 정정 후 유효 계산량**을 우선 평가해야 한다.
