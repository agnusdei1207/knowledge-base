...
- 실제 평가는 상관·누설 오류를 포함한다.

**동작 원리**

- **1. 안정자 측정 회로 실행**: 보조 큐비트 준비·국소 결합으로 X·Z 패리티 측정
- **2. 안정자 값 반환**: 논리 상태를 노출하지 않고 증후군 비트 생성
- **3. 검출 사건 전달**: 이전 주기와의 변화로 시공간 오류 이력 구성
- **4. 파울리 프레임 전달**: 복호한 오류 사슬을 논리 보정 상태로 기록
- **5. 후속 해석 지시**: 프레임을 이후 측정·게이트 해석에 반영
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 복호 지연 초과 | 스트리밍·병렬 복호 | **실시간 보정 유지** |
| 누설·상관 오류 | 오류 모델·제거 회로 적용 | **논리 오류 평가 개선** |
| 측정 오류 혼동 | 시공간 증후군 분석 | **오류 위치 정확도 향상** |
| 자원 급증 | 목표별 거리·패치 산정 | **자원 과잉 방지** |
...
- 물리 오류율·코드 거리·복호 지연으로 표면 코드를 설계한다.
