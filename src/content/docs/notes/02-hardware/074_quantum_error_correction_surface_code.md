...
- 정의/개념: 여러 물리 큐비트에 정보를 부호화하고 **안정자를 반복 측정**하여 오류를 검출•보정함으로써 논리 큐비트를 보호하는 기법이다.
- 배경/필요성: 잡음이 있는 물리 큐비트만 직접 사용하면 회로가 길어질수록 오류가 누적되어 논리 상태를 신뢰성 있게 유지할 수 없다.

#### 한줄 요약

- 양자 오류 정정은 논리 상태를 직접 노출하지 않는 **안정자 반복 측정**으로 오류 증후군을 얻는다.
...
> **코드 거리 3→11**에서 정정 오류 수 1→5 증가

- 비트•위상 오류에 대응하는 **안정자 증후군**을 생성한다.
- 시간축을 포함한 증후군 변화로 **오류 사슬**을 추정한다.
- 임계값 아래에서 코드 거리 증가에 따른 **논리 오류율 감소**가 핵심이다.

#### 한줄 요약

- **코드 거리** 증가에 따른 정정 능력과 자원 오버헤드 증가한다.
...
- **증후군 수집•오류 사슬 복호** 기반 파울리 프레임을 갱신한다.
...
> 요약: **코드 거리 d**에서 최대 **⌊(d-1)/2⌋개 오류** 정정이 핵심이다.
...
- 시공간 **검출 사건에서 오류 사슬을 복호**한 뒤 그 결과를 파울리 프레임에 반영한다.
...
- **장시간 내결함 계산**에는 논리 부호화가 필요하며, 짧은 물리 회로 실험에는 오류 완화를 사용할 수 있다.
...
- 목표 논리 오류율과 물리 오류 모델에 맞춰 **코드 거리•패치 자원•복호 성능**을 함께 산정한다.
...
- **물리 오류율**이 코드 임계값 아래일 때 목표 논리 오류율과 복호 지연을 만족하는 **코드 거리**를 선택한다.

#### 한줄 요약

- 물리 오류율이 **임계값 아래**이고 복호 지연을 감당할 수 있을 때 코드 거리를 늘려 논리 오류율을 낮춘다.
