...
date: "2026-08-05T17:51:40+09:00"
tags:
  - "notes-hardware"
weight: 74
extra:
...
- 논리 상태를 노출하지 않는 **안정자 반복 측정** 기반 보호
...
- **코드 거리** 증가에 따른 정정 능력과 자원 오버헤드 증가
...
1. **안정자 측정**: 보조 큐비트로 X•Z 패리티 측정
2. **증후군 비트 생성**: 논리 상태를 노출하지 않고 결과 획득
3. **검출 사건 이력 구성**: 주기 간 변화로 시공간 이력 생성
4. **오류 사슬 복호**: 가장 가능성 높은 물리 오류 경로 추정
5. **파울리 프레임 갱신**: 논리 보정 상태를 고전 정보로 기록

#### 한줄 요약

- **검출 사건•오류 사슬** 복호 후 파울리 프레임 반영
...
- **장시간 내결함 계산**은 표면 코드, 짧은 실험은 오류 완화
...
- **물리 오류율 임계값•복호 지연** 충족 시 코드 거리 확대
