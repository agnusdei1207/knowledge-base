...
- **정의/개념**: 2차원 국소 안정자를 반복 측정하는 오류 정정 코드
- **배경/필요성**: 물리 오류가 논리 오류로 번져 사전 검출·정정이 필요함
...
- 2차원 국소 안정자가 비트·위상 오류를 검출한다.
- 임계값 아래에서는 거리 증가가 논리 오류율을 낮춘다.
- 거리 증가는 큐비트·복호 시간 비용을 높인다.
...
| 판단 기준 | 표면 코드 논리 큐비트 | 물리 큐비트 직접 실행 |
|:---|:---|:---|
| 핵심 특징 | 안정자 반복 측정으로 논리 상태 보호 | 오류 정정 없이 물리 게이트 직접 실행 |
| 적용 기준 | 장시간 내결함 회로 | 단시간 물리 회로 |
| 주요 위험 | 복호 지연·누설·상관 오류 | 게이트·판독 오류 누적 |
...
1. 자원 추정기: 목표 논리 오류율에 맞는 거리·큐비트 계산
2. 실시간 디코더: 증후군 생성 주기 안의 오류 사슬 추정
