...
- 정의/개념: 명령 형식·복잡도로 해독 비용·코드 밀도를 조절하는 ISA 계열
- **배경/필요성**: 해독 비용·코드 밀도·호환성이 상충해 명령 체계 선택 필요
...
- RISC의 규칙 형식은 해독 회로를 단순화한다
- CISC의 복합 명령은 코드 밀도를 높인다
- 성능은 ISA와 마이크로구조가 함께 결정한다
...
| 적용 기준 | 재컴파일 가능·명령 해독부 면적 제약 | 기존 x86 바이너리 호환 필수 |
...
1. 일부 x86 코어는 해독된 내부 연산을 캐시해 재해독 감소
2. RISC 임베디드 코어는 압축 인코딩으로 코드 밀도 보완

### 쉽게 이해하기 (학습용)

- 복합 주문을 한 번 나눈 뒤 보관하면 같은 주문을 다시 해석하지 않아도 된다
- 작은 장치는 자주 쓰는 명령을 짧게 적어 같은 메모리에 더 많은 코드를 담는다

## Ⅶ. 결론

- RISC·CISC는 ISA 이름보다 호환성·해독 비용으로 선택
