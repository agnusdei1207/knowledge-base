...
date: "2026-08-05T11:27:43+09:00"
tags:
  - "notes-hardware"
weight: 3
extra:
...
선의 의미: RISC•CISC를 구분하는 명령 표현과 피연산자 규칙

| 구성요소 | 책임 |
|:---|:---|
| RISC 명령어 인코딩 | 규칙적 길이와 **고정 필드** 중심 구성 |
| RISC 피연산자 모델 | **로드•스토어** 기반 레지스터 연산 |
| CISC 명령어 인코딩 | 가변 길이와 **복합 기능** 중심 구성 |
| CISC 피연산자 모델 | 명령의 **메모리 피연산자** 허용 |

#### 한줄 요약
- ISA가 명령 길이와 피연산자 접근 방식을 정해 해독 복잡도와 코드 밀도를 좌우한다.
...
- **바이너리 호환성**: 기존 실행 파일을 재컴파일 없이 실행하는 성질
- **해독 회로 제약**: 명령 해독에 허용할 면적•전력•처리량 한도
- **코드 밀도 요구**: 제한된 메모리에 많은 명령을 담아야 하는 조건
...
- **1. 바이너리 호환성 판정**: 기존 실행 파일 유지 여부 확인
- **2. CISC 선택**: x86 호환성과 코드 밀도 우선
- **3. 해독 회로 제약 판정**: 면적•전력•처리량 한도 확인
- **4. RISC 선택**: 규칙적 인코딩과 단순 해독 우선
- **5. 성능•전력•코드 밀도 검증**: 실제 구현 벤치마크 비교
