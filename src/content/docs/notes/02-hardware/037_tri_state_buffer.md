...
- 정의/개념: 출력 활성화 신호(Output Enable, OE) 제어를 통해 논리 '0', 논리 '1' 및 전기적 절연 상태인 **고임피던스** 3가지 상태를 선택 출력하는 **3-상태 버퍼(Tri-State Buffer)** 회로.
- 배경/필요성: 동일한 물리 주소/데이터 버스선에 다수의 IC 칩 출력이 물리적으로 병렬 연결된 공유 버스 아키텍처에서, 비선택 칩들의 전기적 분리를 통해 **버스 경합** 및 단락 파손을 방지하기 위해 사용.
...
- 비선택 장치의 드라이버를 **고임피던스** 상태로 전환시켜 공유 버스 선상에서 전기적으로 완전 유연하게 분리.
- 버스 경합을 막기 위하여 컨트롤러 단에서 **OE 상호 배타(OE Mutual Exclusion)** 조건 및 충분한 **턴어라운드 시간** 보장 필수.
- 버스 상의 모든 출력이 High-Z가 되는 **부동 현상** 시의 노이즈 취약점을 막기 위해 풀업(Pull-Up) / 풀다운(Pull-Down) 바이어스 저항 결합.
...
2. **High-Z 전환**: 쓰기 완결 시 호스트는 즉시 자신의 OE=0으로 스위칭하여 출력을 **고임피던스**로 빠져나오게 함.
3. **턴어라운드 대기**: **순간 단락** 방지를 위해 지정된 버스 **턴어라운드 시간** 동안 버스를 무구동 대기함.
...
| ASIC / FPGA 칩 내부에서 3-상태 버퍼 선언 시 합성 오류 발생 | **논리 합성** 시 칩 내부는 **MUX**로 전환, 칩 외부 핀에만 적용 | ASIC DFT 테스트 검증성 및 내부 신호 안전성 확보 |
