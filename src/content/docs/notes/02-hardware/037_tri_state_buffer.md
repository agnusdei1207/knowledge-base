...
> 요약: 출력 활성화가 데이터 구동과 공유 버스 분리를 제어

**동작 원리**

- **① 쓰기 데이터·OE 활성 전달**: 제어기 출력단에 데이터와 활성 신호 제공
- **② 논리 0·1 구동**: 활성 동안 DQ 버스를 쓰기값으로 구동
- **③ 쓰기 데이터 전달**: 단일 구동자의 신호를 메모리에 전달
- **④ 쓰기 OE 비활성**: 읽기 전환 전에 제어기 출력 차단
- **⑤ 고임피던스 전환**: 공유선에서 분리되어 경합 방지
- **⑥ 읽기 명령 전달**: 메모리에 주소와 명령 전달
- **⑦ 턴어라운드 후 읽기 데이터·OE 활성**: 비중첩 시간 후 메모리 출력 활성
- **⑧ 메모리 읽기 데이터 구동**: 메모리만 DQ 버스 구동
- **⑨ 읽기 데이터 반환**: 제어기가 지정 에지에서 값 샘플링
- **⑩ 읽기 OE 비활성**: 읽기 후 메모리 출력을 Z로 전환
