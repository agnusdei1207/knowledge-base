...
sequenceDiagram
    ...
    C->>TC: ① 쓰기 데이터·OE 활성 전달
    TC->>B: ② 논리 0·1 구동
    B->>M: ③ 쓰기 데이터 전달
    C->>TC: ④ 쓰기 OE 비활성
    TC-->>B: ⑤ 고임피던스 전환
    C->>M: ⑥ 읽기 명령 전달
    M->>TM: ⑦ 턴어라운드 후 읽기 데이터·OE 활성
    TM->>B: ⑧ 메모리 읽기 데이터 구동
    B-->>C: ⑨ 읽기 데이터 반환
    M->>TM: ⑩ 읽기 OE 비활성
```
...
1. **① 쓰기 데이터·OE 활성 전달**: 메모리 제어기는 쓰기 구간에 보낼 데이터와 출력 활성 신호를 자신의 3-상태 출력단에 제공한다.
2. **② 논리 0·1 구동**: 제어기 출력단은 OE가 활성인 동안 공유 DQ 버스를 쓰기 데이터의 0 또는 1 전압으로 구동한다.
3. **③ 쓰기 데이터 전달**: 공유 버스는 단일 구동자의 신호를 메모리 장치 입력으로 전달한다.
4. **④ 쓰기 OE 비활성**: 쓰기 버스트가 끝나면 제어기는 읽기 방향으로 바꾸기 전에 자신의 출력 활성 신호를 먼저 끈다.
5. **⑤ 고임피던스 전환**: 제어기 출력단은 공유선에서 전기적으로 분리된 Z 상태가 되어 메모리 출력과의 경합을 막는다.
6. **⑥ 읽기 명령 전달**: 제어기는 메모리 장치에 읽을 주소와 명령을 전달한다.
7. **⑦ 턴어라운드 후 읽기 데이터·OE 활성**: 비중첩 시간이 지난 뒤 메모리 장치는 읽은 데이터와 OE 활성 신호를 자신의 출력단에 전달한다.
8. **⑧ 메모리 읽기 데이터 구동**: 이제 메모리 출력단만 DQ 버스를 구동해 원-핫 출력 조건을 유지한다.
9. **⑨ 읽기 데이터 반환**: 공유 버스의 읽기값이 메모리 제어기에 전달되고 지정 에지에서 샘플링된다.
10. **⑩ 읽기 OE 비활성**: 읽기 버스트가 끝나면 메모리 장치도 출력단을 Z 상태로 돌려 다음 구동자에게 버스를 넘긴다.
