...
- 배경/필요성: 범용 OS의 소프트웨어 취약점 노출 → 커널 권한 탈취 발생 → **암호 키 및 생체 정보** 보호 불가
...
오류 코드 반환 후 즉시 복귀           4. Normal World 레지스터 문맥을 안전 메모리에 백업
   ...
   │                                     │
   │                                     ▼
   │                                  8. 범용 레지스터 잔여 데이터 완전 정화(Zeroing) 및 문맥 복원
   └──────────────────┬──────────────────┘
                      │
                      ▼
일반 영역(Normal World)으로 제어권 복귀 완료
```
...
- **단말 보안 위협** 증가 시 NS 비트와 EL3 모니터 기반 **Arm TrustZone** 채택
