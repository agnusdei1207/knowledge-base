...
```text
+-------------------------------------------------------------------------------+
|                       Arm TrustZone 하드웨어 보안 격리 아키텍처               |
|                                                                               |
|  [ 일반 영역 (Normal World - Non-Secure) ]   [ 보안 영역 (Secure World - TEE) ] |
|  +---------------------------------------+   +-------------------------------+  |
|  | 클라이언트 애플리케이션 (CA: User)    |   | 신뢰 애플리케이션 (TA: 결제/생체) |  |
|  | 리치 OS (Linux / Android - EL1)       |   | 신뢰 OS (OP-TEE / Secure OS)  |  |
|  +---------------------------------------+   +-------------------------------+  |
|                     │                                       ▲                 |
|                     │ SMC (Secure Monitor Call) 트랩        │                 |
|                     ▼                                       │                 |
|  +-------------------------------------------------------------------------+  |
|  |                보안 모니터 (Secure Monitor - EL3 최상위 특권)           |  |
|  |                - 범용 레지스터 정화(Sanitization) 및 양방향 문맥 교환   |  |
|  +-------------------------------------------------------------------------+  |
|                                    │                                          |
|                                    ▼ 하드웨어 NS(Non-Secure) 버스 비트 제어   |
|  +-------------------------------------------------------------------------+  |
|  |        AMBA / AXI 시스템 인터커넥트 버스 패브릭 (NS=0: Secure, NS=1: Non)  |  |
|  +-------------------------------------------------------------------------+  |
|              │                                                 │              |
|              ▼                                                 ▼              |
|  +---------------------------------------+   +-------------------------------+  |
|  | TZASC (TrustZone 주소 공간 컨트롤러)  |   | TZPC (TrustZone 보호 컨트롤러)|  |
|  | - DRAM 메인 메모리 영역 물리 격리 분할|   | - 암호 가속기, eFuse, 보안 I/O|  |
|  +---------------------------------------+   +-------------------------------+  |
+-------------------------------------------------------------------------------+
```

- 신호 및 연결 관계: 일반 영역에서 SMC 트랩 발생 시 EL3 보안 모니터가 레지스터를 정화한 후 보안 영역으로 절체하며, 시스템 버스 상의 NS 비트 신호(`│▼`)를 통해 TZASC와 TZPC가 메모리 및 주변장치 접근을 물리적으로 차단함을 명시함
...
```text
[일반 영역(Normal World) 앱의 보안 서비스 요청]
                      │
                      ▼
1. 일반 OS 커널의 SMC(Secure Monitor Call) 하드웨어 예외 발생
                      │
                      ▼
2. EL3 보안 모니터 진입 및 호출 파라미터 유효성 검증
    ┌─────────────────┴─────────────────┐
 [ 검증 실패 (비인가 호출) ]          [ 검증 성공 (정상 파라미터) ]
    │                                     │
    ▼                                     ▼
오류 코드 즉시 반환                   3. 일반 영역 레지스터 백업 및 Secure World 문맥 전환
                                          │
                                          ▼
                                     4. 신뢰 OS 및 신뢰 애플리케이션(TA)의 격리 암호 연산 수행
                                          │
                                          ▼
                                     5. 반환 데이터 암호화, 보안 레지스터 정화 및 Normal World 복귀
```
...
1. 일반 영역의 클라이언트 앱이 지문 인증이나 서명 생성을 요청하면 리치 OS 드라이버가 파라미터 공유 버퍼를 할당하고 SMC 명령어로 하드웨어 트랩을 유발한다.
2. CPU가 즉시 최상위 특권 레벨인 EL3 보안 모니터로 분기하여 인가된 호출 ID인지와 버퍼 주소가 적법한지 검증한다.
3. 검증 통과 시 일반 영역의 CPU 레지스터를 안전 스택에 저장하고 하드웨어 제어 비트를 Secure 모드로 변경하여 Secure-EL1의 신뢰 OS로 제어권을 넘긴다.
4. 신뢰 애플리케이션(TA)이 TZASC로 격리된 보안 메모리와 eFuse 암호 키를 하드웨어로 읽어 들여 서명 또는 키 복호화 연산을 안전하게 실행한다.
5. 연산이 완료되면 결과를 공유 버퍼에 암호화 기록하고, 민감 정보가 남아있을 수 있는 범용 레지스터를 0으로 완전 클리어(Sanitization)한 후 Normal World로 복귀한다.
...
|:---|:---|:---|:---|:---|
| 적용 기준 | 단일 모바일/임베디드 SoC 전역의 시스템 보안 및 TEE 구축 시 | 클라우드 서버 가상 머신 내부 특정 민감 프로세스 데이터 보호 시 | 다중 테넌트 간 완전 가상화 OS 파티셔닝 및 클라우드 인프라 분리 시 | 물리적 분리 외장 칩 기반 보안 부팅 암호 키 하드웨어 저장 시 |
...
- 모바일 및 차량용 SoC의 하드웨어 기반 신뢰 실행 환경(TEE) 구축을 위한 지배적 산업 표준으로 안착되었으며, 향후 Armv9 기밀 컴퓨팅 아키텍처(CCA)의 렐름(Realm) 관리 확장과 결합하여 클라우드 및 멀티테넌트 엣지 기밀 보호로 진화하고 있으므로 임베디드 보안 아키텍트는 공유 메모리 복사를 통한 TOCTOU 방어와 SMMU 기반 버스 주변장치 격리를 시스템 초기 부팅 시점에 강제해야 한다.
