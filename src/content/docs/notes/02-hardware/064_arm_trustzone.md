...
```mermaid
sequenceDiagram
    participant N as 비보안 응용·OS·공유 버퍼
    participant G as 보안 모니터·게이트웨이
    participant A as 자원 보안 제어기
    participant T as 신뢰 펌웨어·TEE
    participant P as 보안 주변장치

    N->>G: ① 공유 버퍼 기록 후 SMC·SG 진입 호출
    G->>A: ② 버퍼 귀속·거래 속성 확인 요청
    A-->>G: ③ 비보안 메모리 접근 판정
    G->>T: ④ 포인터·길이 검증 후 보안 상태 실행
    T->>N: ⑤ 재검증한 입력을 보안 메모리로 복사
    T->>P: ⑥ 보안 자원 연산 요청
    P-->>T: ⑦ 비밀을 제외한 연산 결과 반환
    T->>N: ⑧ 비민감 결과를 공유 버퍼에 기록
    T-->>G: ⑨ 민감 상태 정리·서비스 완료
    G-->>N: ⑩ 비보안 상태 복귀·결과 통지
```
...
> 요약: 보안 진입과 거래 속성이 비보안 접근을 차단

**동작 원리**

- **① 공유 버퍼 기록 후 SMC·SG 진입 호출**: A-profile·M-profile의 허용 경로 사용
- **② 버퍼 귀속·거래 속성 확인 요청**: 공유 주소의 보안 속성 조회
- **③ 비보안 메모리 접근 판정**: 허용된 공유 범위 여부 반환
- **④ 포인터·길이 검증 후 보안 상태 실행**: 정렬·범위 확인 후 TEE 진입
- **⑤ 재검증한 입력을 보안 메모리로 복사**: 경계 입력의 변경·오용 차단
- **⑥ 보안 자원 연산 요청**: 키 저장소 등 내부 자원 사용
- **⑦ 비밀을 제외한 연산 결과 반환**: 키 자체의 외부 노출 방지
- **⑧ 비민감 결과를 공유 버퍼에 기록**: 공개 가능한 값만 전달
- **⑨ 민감 상태 정리·서비스 완료**: 임시 비밀 제거
- **⑩ 비보안 상태 복귀·결과 통지**: 호출자에게 완료 반환

### 쉽게 이해하기 (학습용)

- 비보안 영역은 검증된 진입점으로 요청하고 TEE 내부에서 키를 사용한 결과만 돌려받는다
...
> 사례: 기기 키 저장소는 개인키를 비보안 영역에 반환하지 않고 TEE 내부의 서명 결과만 공유 버퍼로 전달한다.

### 쉽게 이해하기 (학습용)

- TEE에는 핵심 키와 최소 코드만 두고 공유 버퍼의 포인터·길이·귀속을 호출마다 검증한다

## Ⅵ. 결론

- 보호 자산·보안 상태·공유 메모리·진입점을 검토해 **TrustZone TCB** 최소화
