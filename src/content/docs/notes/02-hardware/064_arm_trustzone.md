...
date: "2026-07-25T10:32:00+09:00"
tags:
  - "notes-hardware"
weight: 64
extra:
...
- **Arm TrustZone**: 하드웨어 보안 상태로 실행·자원을 격리
- **시스템 온 칩(System on Chip, SoC)**: ‘에스오시’로 읽고 영문 머리글자를 딴 약어이며, 처리기·메모리·주변장치를 한 칩에 통합
...
- **정의/개념**: 보안 상태와 거래 속성으로 자원을 격리한다
- **배경/필요성**: 커널 침해에도 키와 부팅 코드를 보호한다
...
- 보안 상태만 보안 자원에 접근해 커널 침해를 격리한다
- 거래 보안 속성이 버스 접근을 막아 키 노출을 차단한다
- 공유 버퍼·진입점 검증 실패는 비밀 유출로 이어진다
...
| 판단 기준 | Arm TrustZone 격리 | 운영체제 권한 격리 |
|:---|:---|:---|
| 핵심 특징 | 처리기 상태·버스 속성으로 자원 격리 | 커널 권한·페이지 테이블로 격리 |
| 적용 기준 | 키·부팅 코드·암호 서비스 격리 | 응용·프로세스 간 권한 분리 |
| 주요 위험 | 진입점·속성·공유 메모리 오류 | 커널 침해로 격리 경계 무력화 |
...
1. 기기 키 저장소는 TEE에서 서명하고 키는 반환하지 않는다
2. 자원 보안 제어는 비보안 DMA의 보안 메모리 접근을 차단한다

### 쉽게 이해하기 (학습용)

- 금고 열쇠를 밖으로 주지 않고 금고실 안에서 서명만 처리한다
- 무인 운반차도 출입 카드가 없으면 금고실 짐을 가져갈 수 없다

## Ⅶ. 결론

- 키·부팅은 TrustZone에 격리하고 진입을 검증한다
