...
- 정의/개념: 프로세서 보안 상태와 시스템 버스·메모리·주변장치의 보안 속성으로 실행 환경을 분리하는 **Arm 하드웨어 보안 확장**이다.
- 배경/필요성: 운영체제 페이지 권한만 사용하면 커널 최고 권한을 탈취한 공격자가 우회할 수 있어 **핵심 키·부팅 코드의 독립 격리를 필요로 한다**.
...
- 비보안 입력 제한 수단은 **진입점•공유 버퍼 검증**이 핵심이다.
...
3. **보안 서비스 실행·복귀**: 키를 보안 영역 밖으로 내보내지 않고 연산한 뒤 비민감 결과와 상태만 비보안 호출자에 반환한다.
...
| 비보안 공유 버퍼의 주소·길이를 믿어 **보안 메모리 경계 침범** | 범위·정수 오버플로를 검증한 뒤 필요한 데이터만 **보안 영역으로 복사** | 비보안 포인터를 통한 **경계 취약점 방지** |
| 메모리·주변장치·DMA의 보안 귀속이 잘못되어 **비보안 접근 허용** | 부팅 초기에 전체 **자원 보안 속성을 검증·잠금** | 비보안 CPU·DMA의 **보안 자원 접근 차단** |
| 보안 서비스 반환 버퍼·레지스터에 **민감 정보 잔존** | 반환값을 최소화하고 **공유 버퍼·경계 상태 정리** | 비보안 호출자에 대한 **정보 누출 방지** |
...
- 핵심 자산은 **TrustZone 보안 영역**에 배치하고 **TCB 최소화**가 핵심이다.
