...
- 모바일, IoT 및 차량용 SoC의 하드웨어 기반 **신뢰 실행 환경**(TEE) 구축을 위한 지배적 산업 표준으로 안착되었으며, 최근에는 Armv9 기밀 컴퓨팅 아키텍처(CCA, Confidential Compute Architecture)의 렐름(Realm) 관리 확장과 결합하여 클라우드 및 멀티테넌트 엣지 기밀 보호로 진화 지속
