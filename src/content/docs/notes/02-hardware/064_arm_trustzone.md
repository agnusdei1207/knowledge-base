...
1. 일반 영역 클라이언트 앱의 보안 서비스 요청 시 리치 OS 드라이버의 파라미터 공유 버퍼 할당 및 SMC 명령어 하드웨어 트랩 유발
2. 최상위 특권 EL3 보안 모니터 즉각 분기 및 인가된 서비스 ID 및 파라미터 메모리 유효성 검증
3. 검증 성공 시 일반 영역 CPU 레지스터 안전 백업 및 하드웨어 NS 제어 비트 전환을 통한 Secure-EL1 신뢰 OS 제어권 이양
4. 신뢰 애플리케이션(TA)의 TZASC 격리 보안 메모리 및 eFuse 암호 키 하드웨어 획득을 통한 암호화·인증 연산 안전 실행
5. 연산 결과 암호화 기록 및 범용 레지스터의 완전 클리어(Sanitization) 후 Normal World 복귀 수행
