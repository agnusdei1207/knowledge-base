...
- 가상화 패스스루 가속: **IOTLB** 캐싱 및 인터럽트 리매핑으로 PCIe 디바이스를 게스트 VM에 1:1 직결 할당
...
- I/O 가상화 패스스루(SR-IOV, GPU **Passthrough**)와 제로 트러스트 하드웨어 DMA 격리의 핵심 기반 기술(Intel VT-d, AMD-Vi, Arm SMMU)로 확립되었으며, 최근에는 DPU/SmartNIC 오프로드 및 CXL.io/mem 메모리 풀링 환경의 다중 테넌트 I/O 보안/주소 격리로 역할 확장
