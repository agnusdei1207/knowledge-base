...
- 정의/개념: 장치가 사용하는 I/O 가상 주소를 호스트 물리 주소로 변환하고 페이지별 **DMA 접근 권한을 검사하는 메모리 관리 장치**이다.
- 배경/필요성: 장치가 물리 주소로 직접 DMA를 수행하면 잘못되거나 악의적인 요청이 다른 프로세스•VM의 메모리를 침범하여 데이터를 훼손할 수 있다.

#### 한줄 요약

- **IOVA 변환•DMA 권한 검사**를 수행하는 하드웨어가 핵심이다.
...
- **장치•VM별 도메인•SR-IOV**로 직접 할당 DMA 범위를 분리한다.
- **IOVA→물리 주소 변환**과 페이지 권한 동시 검사가 핵심이다.
- **IOTLB 미스•무효화**가 장치 주소 변환 지연 증가한다.

#### 한줄 요약

- **장치 도메인 격리**와 IOTLB 미스•무효화 비용이 핵심이다.
...
- **도메인•페이지 테이블•IOMMU** 기반 DMA 접근 경계가 핵심이다.
...
- IOMMU는 **도메인 선택•주소 변환•권한 검증**을 모두 통과한 DMA 요청만 메모리로 전달한다.
...
- 외부 장치와 VM 직접 할당 장치에는 필요한 버퍼만 노출하는 **최소 권한 DMA 매핑**을 적용한다.
...
- 장치별로 **필요한 버퍼만 매핑**하고 매핑을 변경•해제할 때 관련 IOTLB 항목을 무효화한다.
...
- 외부•VM 장치는 **IOMMU**, 신뢰 단일 장치는 직접 DMA가 핵심이다.
