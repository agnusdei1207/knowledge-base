...
- **2. DMA 시작 요청**: 장치에 IOVA·길이·작업 디스크립터 전달
- **3. IOVA 변환 요청**: 장치 ID로 도메인을 고르고 주소·권한 검증
- **4. DMA 접근 허용**: 유효 요청만 물리 주소로 변환해 메모리에 전달
- **5. 매핑 제거·무효화**: DMA 완료 동기화 후 매핑과 낡은 IOTLB 항목 제거
