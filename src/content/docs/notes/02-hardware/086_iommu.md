...
- 배경/필요성: 장치의 직접 DMA는 CPU 변환 경로를 **MMU 우회**하여 물리 주소로 접근하므로 감염된 장치 하나가 **악성 DMA 공격**으로 임의 영역을 읽고 쓰는 대가를 치르며, DMA 경로에도 주소 변환·권한 검사 계층을 두어 장치를 허용 영역에만 묶을 필요
...
- 멀티테넌트 가상화 구축 시 **IOMMU** 기반 DMA 주소 변환과 **도메인 격리** 체계 적용.
