...
sidebar:
  ...
  badge:
    ...
    variant: note
title: "하드·펌·소프트 실시간"
date: "2026-07-25T10:28:00+09:00"
tags:
  - "notes-hardware"
weight: 62
extra:
...
- **정의/개념**: 미스 영향과 결과 가치의 시간 제약 분류다
- **배경/필요성**: 미스 피해에 맞는 검증·대응 수준을 정한다
...
- 실시간 정확성은 논리 결과와 완료 시각을 함께 요구한다
- 하드 미스는 시스템 실패가 되어 안전 상태가 필요하다
- 늦은 펌 결과는 무효라 처리 자원을 낭비한다
- 소프트 지연은 품질을 낮추지만 서비스는 계속된다
...
| 판단 기준 | 하드 실시간 | 펌 실시간 | 소프트 실시간 |
|:---|:---|:---|:---|
| 핵심 특징 | 데드라인 미스를 시스템 실패로 판정 | 늦은 결과의 가치를 0으로 판정 | 지연에 따라 서비스 품질 저하 |
| 적용 기준 | 안전 제어·실패 불허 | 최신 결과만 유효한 판정 | 영상·음성 등 품질 저하 허용 |
| 주요 위험 | 한 번의 미스가 안전 실패 | 늦은 결과 누적·처리 낭비 | 지연·지터 누적으로 품질 악화 |
...
1. 하드: 차량 제동의 데드라인을 검증한다
2. 펌: 공정 검사의 늦은 판정 결과를 버린다
3. 소프트: 영상 재생의 지터·화질을 조정한다
