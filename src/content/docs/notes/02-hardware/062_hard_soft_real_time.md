...
- **1. 실행·데드라인 등록**: 태스크 자원을 배정하고 완료 판정 기준 설정
- **2. 완료 시각 전달**: 실행 결과와 실제 종료 시각을 감시기에 보고
- **3. 데드라인 판정·조치**: 하드는 안전 전환, 펌은 결과 폐기, 소프트는 품질 저하 허용
- **4. 미스 상태 반영**: 누적 위반을 다음 스케줄·용량 계획에 반영
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 유형 오분류 | 안전 영향·결과 가치 분석 | **검증 수준 정합화** |
| 최악 지연 과소평가 | WCET·차단·간섭 분석 | **응답 상한 확보** |
| 하드 미스 대응 실패 | 장애 주입·안전 출력 시험 | **피해 제한 검증** |
| 소프트 미스 누적 | 지터·드롭률 기반 품질 조정 | **서비스 열화 통제** |
...
- **안전 실패**는 하드, **늦은 결과 가치 0**은 펌, 품질 저하는 소프트
