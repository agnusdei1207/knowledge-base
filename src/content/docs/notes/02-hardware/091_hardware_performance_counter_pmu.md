...
- 정의/개념: CPU 사건을 선택·계수하는 **내장 계측 장치**
- 배경/필요성: 실행 시간만으로 모호한 **CPU 병목 원인** 식별
...
- **1. 계측 조건 설정**: 사건·주기 지정
- **2. 통제 부하 실행**: 코어·입력 고정
- **3. CPU 사건 생성**: 캐시·분기 사건 발생
- **4. 선택 사건 계수**: 카운터 누적과 오버플로 시 실행 위치 표본 기록
- **5. 계수·활성 시간 반환**: 활성 시간 비율로 다중화 값을 보정
...
- 사건 정의·활성 시간으로 **PMU 병목 측정**을 수행한다.
