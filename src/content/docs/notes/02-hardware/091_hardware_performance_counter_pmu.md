...
- 동일 측정 구간·코어의 사건을 사용하며, \(\hat N\)은 사건 발생률이 비활성 구간에도 같았다는 다중화 가정의 추정치다.

### 도표안 A. PMU 사건 선택·계수 정적 구조
...
### 도표안 B. PMU 계수·오버플로 샘플링 시퀀스

```mermaid
sequenceDiagram
    participant A as 분석 도구
    participant P as 운영체제 PMU 드라이버
    participant S as 이벤트 선택기
    participant W as 워크로드
    participant E as CPU 이벤트 원천
    participant C as 프로그래머블 카운터

    A->>P: ① 사건·권한·샘플 주기 설정
    P->>S: ② 물리 카운터 매핑·다중화 일정 적용
    P->>C: ③ 카운터 초기화·오버플로 임계값 설정
    A->>W: ④ 통제된 워크로드 실행
    W->>E: ⑤ 명령·캐시·분기 사건 발생
    E->>C: ⑥ 선택 사건 펄스 계수
    C-->>P: ⑦ 임계값 도달 시 오버플로 인터럽트
    P-->>A: ⑧ 실행 위치·계수·활성 시간 샘플
    A->>P: ⑨ 측정 종료·최종 카운터 읽기
...
1. 분석 도구가 병목 가설에 맞는 사건과 사용자·커널 범위, 샘플 주기를 지정한다.
2. 드라이버가 제한된 물리 카운터에 사건을 배치하고 필요하면 시간 분할한다.
3. 카운터 시작값과 오버플로 임계값을 설정한다.
4. 코어·입력·전원 정책을 통제한 워크로드를 실행한다.
5. CPU가 명령 은퇴·사이클·캐시 미스·분기 실패 사건을 생성한다.
6. 선택기에 연결된 사건만 해당 카운터 값에 누적된다.
7. 카운터가 임계값에 도달하면 PMU가 인터럽트로 샘플을 요청한다.
8. 드라이버가 실행 위치와 계수값·이벤트 활성 시간을 분석 도구에 제공한다.
9. 워크로드 종료 뒤 분석 도구가 최종 계수 읽기를 요청한다.
10. 도구가 원시값과 활성 시간으로 다중화를 보정하고 IPC·미스율을 동일 구간에서 계산한다.

### 쉽게 이해하기 (학습용)

- 먼저 셀 사건을 고르고 같은 부하를 실행한 뒤, 총량과 발생 위치를 함께 읽어 병목 가설을 검증한다.
