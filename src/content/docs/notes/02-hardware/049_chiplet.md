...
sidebar:
  ...
  badge:
    ...
    variant: note
title: "칩렛 (Chiplet)"
date: "2026-07-25T10:02:00+09:00"
tags:
  - "notes-hardware"
weight: 49
extra:
...
- **정의/개념**: 기능별 소형 다이를 D2D로 연결한 반도체다
- **배경/필요성**: 대형 다이의 불량 폐기 비용을 줄인다
...
- KGD·소형 다이는 불량 폐기 비용을 줄인다
- 공정 혼용·재사용은 D2D·패키지 비용을 늘린다
...
| 비교축 | 칩렛 | 단일 SoC |
|:---|:---|:---|
| 핵심 특징 | 검증 다이를 패키지에서 연결 | 기능을 하나의 다이에 통합 |
| 적용 기준 | 이종 공정·재사용·수율 우선 | 온다이 지연·단일 설계 우선 |
| 제조·공정 | 소형 KGD·기능별 공정 혼용 | 대형 다이·단일 공정 적용 |
| 설계 확장 | 검증 칩렛 재사용·조합 | 다이 전체를 다시 설계 |
...
1. 서버 CPU: 코어 칩렛과 I/O 다이를 조합한다
2. AI 가속기: 연산 칩렛과 메모리를 가깝게 둔다

### 쉽게 이해하기 (학습용)

- 서버 CPU는 필요한 코어 수만큼 연산방을 늘려 제품군을 만든다
- AI 가속기는 연산방과 메모리 창고를 가까이 둬 이동 시간을 줄인다

## Ⅶ. 결론

- 수율·공정 이득이 연결 비용보다 크면 칩렛을 쓴다
