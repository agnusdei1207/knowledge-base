...
- **다이 간 연결(Die-to-Die, D2D)**: 연산·입출력 칩렛 사이의 트랜잭션을 전달하는 인터커넥트이다.
...
- **다이 간 연결(Die-to-Die, D2D)·입출력(Input/Output, I/O)**: 칩렛 사이의 패킷 전송과 메모리·외부 장치 기능을 연결하는 인터페이스이다.
...
- **다이 간 연결(Die-to-Die, D2D)**: 칩렛 사이를 연결하며 단일 다이보다 추가 지연과 패키지 비용을 만드는 인터페이스이다.
...
- **다이 간 연결(Die-to-Die, D2D)**: 칩렛 재사용 이익과 함께 지연·전력·패키지 비용을 비교해야 하는 연결 방식이다.
