...
sidebar:
  ...
  badge:
    ...
    variant: note
title: "칩렛 (Chiplet)"
date: "2026-08-02T11:09:00+09:00"
tags:
  - "notes-hardware"
weight: 49
extra:
...
- **D2D 연결 비용(D2D Interconnect Cost)**: 다이 간 데이터 전달에서 발생하는 지연과 전력 및 패키지 배선 비용이다.

</details>

- **소형 KGD 조합**으로 수율·불량 폐기 비용 개선
- **이종 공정 혼용**으로 기능별 공정 최적화
- **D2D 연결**의 지연·전력 증가와 **패키지 조립**의 수율 위험
...
- **연산 칩렛(Compute Chiplet)**: CPU나 GPU 코어를 포함하여 범용 또는 병렬 연산을 수행하는 다이이다.
- **I/O·캐시 칩렛(I/O·Cache Chiplet)**: 메모리와 외부 장치 인터페이스 및 공유 캐시를 담당하는 다이이다.
...
- **링크 오류율(Link Error Rate)**: 전체 D2D 전송 가운데 오류가 검출된 데이터나 패킷의 비율이다.
...
- **수율•재사용 이득**이 D2D•패키지 비용보다 크면 **칩렛 선택**
