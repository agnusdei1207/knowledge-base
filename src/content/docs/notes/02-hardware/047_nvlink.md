...
- 배경/필요성: 표준 PCIe 버스의 대역폭 한계 및 호스트 CPU 경유 통신으로 인한 **GPU 간 텐서 동기화 지연과 대규모 분산 학습 확장성 병목** 극복 필요
...
|:---|:---|:---|:---|:---|
| 적용 기준 | 노드 내 GPU 스케일업 | 범용 장치 연결 | 메모리 풀링 | 노드 간 스케일아웃 |
...
- 단일 노드 내 다중 GPU를 단일 거대 가속기로 묶어내는 **스케일업(Scale-up) 인터커넥트의 절대적 표준(NVLink 5세대 1.8TB/s, NVL72 랙 스케일)**으로 자리잡았으며, 향후 **NVSwitch 기반의 스위칭 패브릭 및 칩 직접 액체 냉각(Direct Liquid Cooling) 인프라**와 일체화되어 초거대 AI 클러스터의 핵심 인프라로 확산
