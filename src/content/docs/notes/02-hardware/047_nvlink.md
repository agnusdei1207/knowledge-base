...
- 정의/개념: NVIDIA GPU·CPU 사이에 피어 데이터와 집단 통신을 전달하는 **NVLink** 인터커넥트이다.
- 배경/필요성: PCIe 계층과 공유 경로만으로는 다중 GPU 집단 통신에서 대역폭과 스위치 홉 병목이 발생한다.
...
- **통신 패턴(Communication Pattern)**: 피어 교환·전체 축소처럼 GPU 사이에서 데이터가 이동하는 규칙이다.

</details>

- GPU 쌍의 대역폭을 확장하는 **링크 결합**이 핵심이다.
- 다수 GPU를 다대다로 연결하는 **NVSwitch**가 핵심이다.
...
- **토폴로지·조각 계획**: GPU 연결과 링크 대역폭에 맞춰 경로와 통신 조각 크기를 정하는 단계이다.
- **Reduce-Scatter 실행**: 조각을 피어로 보내며 축소해 GPU별 완성 조각을 만드는 단계이다.
- **All-Gather 실행**: 완성 조각을 모든 GPU가 교환해 전체 결과를 구성하는 단계이다.
- **전체 축소 완료**: 모든 조각의 수신과 축소 뒤 후속 연산을 허용하는 단계이다.
...
1. **토폴로지·조각 계획**: NCCL이 GPU 연결과 링크 대역폭에 맞춰 링·트리 경로와 **통신 조각** 크기를 결정한다.
...
| 핵심 특징 | **직접 NVLink** | **NVSwitch 패브릭** | **PCIe** |
| 한계 | 링크 수•토폴로지•세대 호환 | 시스템 비용•전력•구성 범위 | 공유 대역폭•**홉** 증가 |
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 통신량이 많은 프로세스·샤드가 멀리 배치되어 추가 홉·공유 링크 경합 | 링크 대역폭·홉 수에 맞춘 **프로세스·샤드 배치** | 원거리 통신 감소 |
| 전체 축소의 한 링크·GPU가 늦어 **꼬리 지연** 결정 | **통신 조각** 크기와 알고리즘·부하 균형 조정 | 집단 통신 지연 완화 |
| 링크 오류나 세대·속도 협상 차이로 경로 대역폭 축소 | **링크 감시**와 지원 토폴로지·세대 호환성 검증 | 저속·오류 링크 식별 |
| 완료 전에 통신 버퍼를 재사용하거나 통신 대기 중 연산기 유휴 | 버퍼 이벤트 수명을 보장하고 **통신·연산 중첩** 적용 | 데이터 손상과 유휴 감소 |
...
- **GPU 연결 선택 기준**: GPU 규모·피어 통신·전대역 연결 요구로 직접 NVLink와 NVSwitch를 고르는 기준이다.

</details>

- **GPU 연결 선택 기준**에 따라 **GPU 규모**가 작고 **피어 통신**이 중심이면 **직접 NVLink**, 다수 GPU의 **전대역 연결**에는 **NVSwitch**를 선택한다.
