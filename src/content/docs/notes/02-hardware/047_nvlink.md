...
- **NVLink**: NVIDIA GPU와 CPU 사이에 고대역폭·저지연 데이터 전송을 제공하는 전용 인터커넥트이다.
...
- **링크 결합(Link Aggregation)**: 동일한 GPU 쌍 사이의 여러 물리 링크를 함께 사용하여 총대역폭을 높이는 방식이다.
...
- **엔비디아 집단 통신 라이브러리(NVIDIA Collective Communications Library, NCCL)**: GPU 토폴로지에 맞춰 집단 통신 경로와 전송 연산을 실행하는 라이브러리이다.
...
- **전체 축소(All-Reduce)**: 각 GPU의 값을 하나로 집계한 뒤 완성된 결과를 모든 GPU에 배포하는 집단 통신이다.
...
- **주변 구성요소 상호연결 익스프레스(PCI Express, PCIe)**: CPU와 범용 주변장치를 계층형 점대점 링크로 연결하는 표준 인터커넥트이다.
...
- **프로세스·샤드 배치(Process·Shard Placement)**: 통신량이 많은 작업과 모델 조각을 가까운 GPU와 링크에 배치하는 작업이다.
...
- **피어 통신(Peer Communication)**: 한 GPU가 호스트 메모리를 경유하지 않고 다른 GPU와 직접 데이터를 교환하는 통신이다.
