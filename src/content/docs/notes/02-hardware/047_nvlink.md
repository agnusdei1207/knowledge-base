...
+-------------------------------------------------------------+
                               ...
                               v
+-------------------------------------------------------------+
| 1. NCCL 라이브러리의 물리 토폴로지 감지 및 최적 링/트리 산출|
+-------------------------------------------------------------+
                               |
                               v
+-------------------------------------------------------------+
...
+-------------------------------------------------------------+
                               ...
                               v
+-------------------------------------------------------------+
| 3. NVSwitch 패브릭 크로스바 라우팅 및 SHARP 인스위치 연산    |
+-------------------------------------------------------------+
                               |
                               v
+-------------------------------------------------------------+
| 4. 노드 경계 초과 시 InfiniBand RoCE RDMA 스케일아웃 연계   |
+-------------------------------------------------------------+
                               |
                               v
+-------------------------------------------------------------+
...
- **NVL72**: 단일 랙 캐비닛 안에 72개의 Blackwell GPU와 18개의 NVSwitch를 액체 냉각 구리선(NVLink Cable Cartridge)으로 직결하여 단일 거대 GPU처럼 구동하는 랙 스케일 아키텍처.
- **텐서 병렬화(Tensor Parallelism)**: 단일 레이어의 행렬 곱셈을 쪼개어 여러 GPU에서 동시 수행하는 병렬화 방식으로, 레이어마다 실시간 All-Reduce 통신이 필수적임.

</details>

- 단일 노드 내 다중 GPU를 단일 거대 가속기로 묶어내는 스케일업 인터커넥트의 절대적 기술 표준(NVLink 5세대 1.8TB/s)으로 확고히 자리 잡았으며, 향후 NVSwitch 패브릭 및 랙 스케일(NVL72) 액체 냉각 인프라와 일체화되어 초거대 AI 클러스터의 핵심 신경망으로 진화 중.
- 초거대 AI 인프라 구축 시 통신 빈도가 가장 높은 텐서 병렬화 축을 NVLink 도메인 내부에 최우선 배치하고 노드 경계는 파이프라인/데이터 병렬화로 분할하는 계층적 분산 토폴로지 설계 준수 필수.
