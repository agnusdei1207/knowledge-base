...
- **엔브이링크(NVLink)**: 전통적인 PCIe 대역폭 한계를 극복하고 GPU 간 또는 GPU-CPU 간에 수백 GB/s 수준의 초고속 양방향 직접 통신 및 메모리 일관성을 제공하는 전용 인터커넥트.
</details>
...
- 복수 GPU 노드 간 완전 다대다 통신을 지원하는 **NVSwitch** 라우팅
...
| 구성요소 | 책임 |
|:---|:---|
| NCCL 통신 소프트웨어 | 토폴로지 인지 기반 링/트리 알고리즘 수립 및 집단 통신 전개 |
...
|:---|:---|:---|:---|
| 적용 기준 | 소규모 GPU의 P2P 직결 연결 시 | 다수 GPU의 전대역 All-to-All 요구 시 | 범용 호스트-디바이스 및 이종 카드 연결 시 |
...
- 소수 GPU의 빈번한 피어 통신은 직접 NVLink, 다수 GPU의 다대다 통신은 NVSwitch, 범용 호스트·장치 연결은 PCIe가 적합하다.
