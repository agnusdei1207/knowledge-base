...
### 도표안 B. UCIe 트랜잭션·CRC 재전송 시퀀스

```mermaid
sequenceDiagram
    participant PA as 송신 프로토콜 계층
    participant AA as 송신 D2D 어댑터
    participant YA as 송신 PHY
    participant C as 패키지 채널
    participant YB as 수신 PHY
    participant AB as 수신 D2D 어댑터
    participant PB as 수신 프로토콜 계층

    PA->>AA: ① PCIe·CXL·스트리밍 트랜잭션 전달
    AA->>YA: ② 순서 정보·CRC를 포함한 플릿 전달
    YA->>C: ③ 주대역 레인 신호 전송
    C->>YB: ④ 마이크로범프·배선으로 신호 전달
    YB->>AB: ⑤ 수신 플릿 복원
    alt CRC·순서 검증 성공
        AB->>PB: ⑥ 상위 트랜잭션 복원·전달
        PB-->>AB: ⑦ 응답 트랜잭션 반환
        AB->>YB: ⑧ 응답 플릿 전달
        YB->>C: ⑨ 반대 방향 레인 전송
        C->>YA: ⑩ 송신 PHY에 응답 신호 전달
        YA->>AA: ⑪ 응답 플릿 복원
        AA-->>PA: ⑫ 원래 트랜잭션 완료
    else CRC·순서 오류
        AB->>YB: ⑬ 측대역 재전송 요청
        YB->>C: ⑭ 오류 플릿 정보 전달
        C->>YA: ⑮ 송신 PHY에 재시도 신호 전달
        YA->>AA: ⑯ 해당 플릿 재전송 지시
        AA->>YA: ⑰ 저장한 플릿 다시 전달
    end
```
...
1. **① PCIe·CXL·스트리밍 트랜잭션 전달**: 송신 칩렛의 프로토콜 계층은 읽기·쓰기·일관성 또는 연속 데이터 요청을 D2D 어댑터에 제공한다.
2. **② 순서 정보·CRC를 포함한 플릿 전달**: 어댑터는 상위 요청을 UCIe 플릿으로 나누고 순서·오류 검출 정보를 붙여 PHY에 전달한다.
3. **③ 주대역 레인 신호 전송**: 송신 PHY는 링크 훈련에서 합의한 레인 수·속도·타이밍으로 플릿을 전기 신호로 변환한다.
4. **④ 마이크로범프·배선으로 신호 전달**: 패키지 채널은 마이크로범프와 인터포저·기판 배선을 통해 신호를 상대 칩렛으로 운반한다.
5. **⑤ 수신 플릿 복원**: 수신 PHY는 신호를 샘플링·정렬해 고정 형식 플릿으로 복원하고 어댑터에 전달한다.
6. **⑥ 상위 트랜잭션 복원·전달**: CRC와 순서가 유효하면 수신 어댑터는 플릿을 원래 PCIe·CXL·스트리밍 트랜잭션으로 조립한다.
7. **⑦ 응답 트랜잭션 반환**: 수신 프로토콜 계층은 요청을 처리하고 읽기 데이터나 완료 상태를 어댑터에 반환한다.
8. **⑧ 응답 플릿 전달**: 수신 어댑터는 응답을 플릿화해 반대 방향 PHY에 보낸다.
9. **⑨ 반대 방향 레인 전송**: 수신 측 PHY는 응답 플릿을 패키지 채널의 반대 방향 레인에 싣는다.
10. **⑩ 송신 PHY에 응답 신호 전달**: 패키지 채널은 응답 신호를 최초 송신 칩렛의 PHY로 운반한다.
11. **⑪ 응답 플릿 복원**: 송신 PHY와 어댑터는 응답 신호를 플릿으로 복원하고 원래 요청과 대응시킨다.
12. **⑫ 원래 트랜잭션 완료**: 송신 어댑터는 데이터·완료 상태를 프로토콜 계층에 반환해 상위 요청을 끝낸다.
13. **⑬ 측대역 재전송 요청**: 수신 어댑터가 CRC나 순서 오류를 찾으면 손상 플릿을 상위 계층에 전달하지 않고 재시도를 요청한다.
14. **⑭ 오류 플릿 정보 전달**: 수신 PHY는 재전송할 플릿 식별 정보를 측대역 관리 신호로 패키지 채널에 보낸다.
15. **⑮ 송신 PHY에 재시도 신호 전달**: 패키지 채널은 오류 정보를 최초 송신 PHY로 운반한다.
16. **⑯ 해당 플릿 재전송 지시**: 송신 PHY는 D2D 어댑터에 재전송 대상과 링크 상태를 알린다.
17. **⑰ 저장한 플릿 다시 전달**: 송신 어댑터는 보관한 플릿을 다시 PHY에 제공하고 정상 수신될 때까지 전송 절차를 반복한다.
