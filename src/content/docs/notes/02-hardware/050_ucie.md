...
- **상호운용성(Interoperability)**: 서로 다른 공급자의 칩렛이 조합에서 올바르게 통신하는 성질이다.
...
- **주대역(Mainband)**: 트랜잭션 데이터 플릿을 고속으로 전달하는 기본 데이터 경로이다.
...
| 규격 적합 판정을 받은 칩렛끼리도 **상호운용성** 문제로 조합 실패 | 공급사 조합별 **오류 주입**과 프로토콜·전기 시험 | 복구 경로와 경계 조건을 포함한 조합 검증 |
| 패키지의 열·전압 변화로 PHY 타이밍 마진 축소 | 열·전력 무결성 해석과 실사용 부하 링크 시험 | 최악 조건에서도 신호 타이밍 마진 유지 |

> 요약: 칩렛 조합의 **BER**과 재전송률을 측정해 **상호운용성**을 검증한다.
