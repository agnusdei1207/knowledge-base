...
- 정의/개념: 패키지 내부 칩렛 연결의 PHY·D2D 어댑터와 상위 프로토콜 수용 방식을 규정하는 **표준 다이 간 인터페이스**이다.
- 배경/필요성: 공급자별 독자 D2D는 칩렛 조합마다 인터페이스를 다시 설계·검증해야 해 **통합 비용·공급자 종속이 증가한다**.
...
- 링크 오류의 데이터 손상을 막는 **플릿 CRC•재시도**가 핵심이다.
- 멀티벤더 결합을 검증하는 **적합성•상호운용성 시험**이 핵심이다.
...
2. **주대역 PHY 전송**: 플릿을 레인 신호로 변환해 패키지 채널의 상대 PHY로 전달한다.
...
> 요약: 칩렛 재사용은 **UCIe**, 전용 최적화는 **독자 D2D**을 선택한다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 레인·속도 협상에 실패해 **주대역 링크 미기동** | **측대역 상태 로그**를 분석하고 지원 범위 안에서 레인 수·속도 하향 | 공통 동작점 재협상으로 **링크 기동 복구** |
| 높은 비트 오류율로 재전송이 늘어 **유효 대역폭 저하** | 온도별 **BER·재시도율·신호 눈 마진** 감시 | 물리 채널 열화를 조기에 찾아 **신호 품질 개선** |
| 규격 적합 판정을 받은 칩렛끼리도 전기·기능 차이로 **실제 조합 실패** | 공급사 조합별 **오류 주입·프로토콜·전기 시험** | 복구 경로와 경계 조건을 포함한 **상호운용성 검증** |
| 패키지의 열·전압 변화로 **PHY 타이밍 마진 축소** | **열·전력 무결성 해석과 실사용 부하 링크 시험** | 최악 조건에서도 **신호 타이밍 마진 유지** |

> 요약: 실제 칩렛 조합의 **BER•재시도율** 기반 **상호운용성**을 검증한다.
...
- 멀티벤더•재사용 우선 시 **UCIe**, 전용 성능 우선 시 **독자 D2D**을 선택한다.
