...
- **정의/개념**: 노드·스위치·링크 배치로 통신 경로·용량 결정
- **배경/필요성**: 통신 패턴마다 경로 병목이 달라 토폴로지 선택이 필요함
...
- 차수·지름·양분 대역폭이 통신 성능과 비용을 정한다.
- 통신·물리 경로 불일치는 홉과 혼잡을 늘린다.
...
| 판단 기준 | 팻트리 | 토러스 |
|:---|:---|:---|
| 핵심 특징 | 리프·스파인 계층의 다중 경로 | 차원별 이웃·끝단의 순환 경로 |
| 적용 기준 | 전역 집단 통신 비중이 큰 작업 | 최근접 이웃 통신 비중이 큰 작업 |
| 주요 위험 | 상위 스위치·포트·케이블 비용 | 먼 노드의 홉·혼잡 증가 |
...
1. AI 클러스터는 비차단 팻트리로 집단 통신 대역폭 확보
2. HPC 스텐실은 인접 프로세스를 토러스 이웃에 배치
