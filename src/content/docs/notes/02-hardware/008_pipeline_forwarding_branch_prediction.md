...
- 정의/개념: 결과 우회와 예측 경로 선인출로 대기를 줄이는 **파이프라인 해저드 완화 기법**
- 배경/필요성: 레지스터 기록·분기 확정까지 기다리면 **버블 증가로 CPI 상승**
...
- 분기 판정이 늦을수록 **오경로 명령 폐기량**과 복구 주기 증가
...
2. **결과 준비 판정**: 선행 결과의 우회 가능 여부에 따라 포워딩·스톨 선택
3. **예측 경로 인출**: 분기 이력과 BTB로 다음 명령 주소 선택
4. **실제 분기 검증**: 방향·대상을 대조해 실행 지속 또는 플러시 결정
