...
- **CPI** 는 명령어 하나를 완료하는 데 필요한 평균 클록 사이클 수이다.
...
- **PC 복원** 은 오예측 뒤 프로그램 카운터를 실제 분기 경로의 주소로 되돌리는 동작이다.
...
- **RAW 데이터 해저드** 는 앞 명령이 기록할 값을 후속 명령이 먼저 읽으려 할 때 생기는 의존 충돌이다.
...
- **예측·복구** 는 분기 경로를 미리 선택하고 틀렸을 때 오경로 명령과 상태를 되돌리는 제어이다.
