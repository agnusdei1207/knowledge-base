...
date: "2026-08-05T11:31:24+09:00"
tags:
  - "notes-hardware"
weight: 8
extra:
...
```text
   ...
   [RAW 데이터]               [분기 제어]
          |                         |
   [값 준비 여부]            4. 다음 PC 예측•인출
     /          \                   |
 [준비]       [미준비]        [실제 분기 결과]
   |             |              /          \
2. 우회 경로  3. 파이프라인   [일치]       [오예측]
   선택          정체             |             |
   |             |          [인출 지속]   5. 플러시•PC 복원
   +------+------+                         |
          |                          [이력•BTB 갱신]
...
- **1. 해저드 유형 판정**: RAW 의존과 분기 제어 구분
- **2. 우회 경로 선택**: 준비된 선행 결과를 실행부로 전달
- **3. 파이프라인 정체**: 적재값 준비까지 후속 명령 대기
- **4. 다음 PC 예측•인출**: 이력•BTB 기반 경로 선인출
- **5. 플러시•PC 복원**: 오경로 폐기와 이력•BTB 갱신
