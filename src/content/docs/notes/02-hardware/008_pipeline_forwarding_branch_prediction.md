...
- **포워딩**: EX/MEM, MEM/WB 레지스터의 결과를 후속 명령어의 ALU 입력으로 즉시 바이패스.
- **분기 예측**: ID 단계에서 분기 타깃을 추정하여 I-Cache 인출 끊김 방지.
- **플러시 패널티**: 깊은 파이프라인일수록 오예측 시 버블 손실 증가.
...
| 구성요소 | 책임 |
|:---|:---|
| 포워딩 제어기 | Rd•Rs 비교로 **바이패스 신호** 생성 |
| 우회 MUX | 레지스터값•우회값 중 **ALU 입력** 선택 |
| 분기 예측기 | BHT•BTB로 **방향•타깃 주소** 예측 |
| 복구 로직 | 오예측 시 **Flush•PC 복구** 수행 |
...
- **의존성 검출**: ID 단계에서 RAW 의존 또는 조건 분기를 감지함.
- **포워딩•스톨 적용**: 결과를 우회하고 Load-Use이면 한 주기 대기함.
- **분기 예측•복구**: 예측 인출 후 오예측이면 **Flush•PC 복구**를 수행함.
...
- **로드-유즈 의존성(Load-Use Data Hazard)**: Load 명령어가 메모리에서 데이터를 가져오기 전에는 포워딩으로도 해저드를 완전 은닉할 수 없어 1주기 스톨이 강제되는 구조적 지연.
</details>
...
- RAW 대기가 크면 **포워딩**•**재배치**, 분기 손실이 크면 **BTB**•**예측기** 우선 적용.

#### 한줄 요약

- 해저드별 추가 CPI를 기준으로 우회 경로와 예측 자원을 배분함.
