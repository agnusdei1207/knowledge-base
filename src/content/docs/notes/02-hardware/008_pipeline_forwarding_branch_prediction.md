...
- 정의/개념: 결과 우회와 예측 경로 선인출로 대기를 줄이는 **파이프라인 해저드 완화 기법**이다.
- 배경/필요성: 레지스터 기록·분기 확정까지 기다리면 **버블 증가로 CPI 상승**이 발생한다.
...
2. **결과 준비 판정**: 선행 결과의 우회 가능 여부에 따라 포워딩·스톨을 선택한다.
3. **예측 경로 인출**: 분기 이력과 BTB로 다음 명령 주소를 선택한다.
4. **실제 분기 검증**: 방향·대상을 대조해 실행 지속 또는 플러시를 결정한다.
...
| 해저드 대응 기법 | 포워딩 | 분기 예측 |
|:---|:---|:---|
| 적용 기준 | 기록 전 **RAW 결과**를 후속 명령이 요구할 때 | 분기 **방향·대상** 확정 전에 인출해야 할 때 |
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 포워딩할 적재값이 없어 **적재-사용 정체 잔존** | **독립 명령 재배치·해저드 감지** | 정확한 값 사용과 **대기 주기 은닉** |
| 특정 분기에서 **오예측 반복** | 이력 길이와 **분기 대상 버퍼** 조정 | 오경로 인출 감소로 **평균 CPI 개선** |
| 오예측 명령이 **아키텍처 상태에 반영될 위험** | **커밋 지연·플러시 복구** 검증 | 오경로 결과 제거로 **정밀 상태 복원** |
| 포워딩·예측 회로로 **면적·전력 증가** | CPI 감소량과 **회로 비용 벤치마크** 비교 | 비용 대비 이득이 큰 **회로 구성 선택** |
...
- RAW 대기는 **포워딩**, 분기 대기는 **예측•복구**를 적용한다.
