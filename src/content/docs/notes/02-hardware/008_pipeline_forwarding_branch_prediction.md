...
- **쓰기 후 읽기(Read After Write, RAW)**: 앞 명령이 쓸 값을 뒤 명령이 읽어야 하는 의존이며, 포워딩이 겨냥하는 대상이다
...
- **프로그램 카운터(Program Counter, PC)**: 다음에 인출할 명령 주소를 담고 있어 오예측 복구 때 되돌릴 기준이 되는 레지스터
- **실행부(Execution Unit)**: 산술·논리 연산을 수행해 결과를 만들어 내는 회로이며 포워딩이 값을 꺼내 오는 출발점이다
- **명령당 클록 주기 수(Cycles Per Instruction, CPI)**: 명령 하나를 끝내는 데 드는 평균 클록 수이며, 두 기법의 이득을 재는 지표다
...
| 예측 경로 인출 | **분기 이력** 기반 추정 경로 명령 투입 |
| 실제 방향 통보 | 확정된 **분기 방향·대상** 회신 |
| 오예측 판정 전달 | 예측·실제 불일치 구간 **폐기 대상** 지정 |
...
| 한계 | **적재-사용 의존**에는 1주기 정체 잔존 | 오예측 시 **인출분 전량 폐기** |
