...
- **1. 비트스트림 구성**: LUT·FF·DSP를 전용 데이터 경로로 연결
- **2. 추론 작업 제출**: 입력 주소와 타일 계획 등록
- **3. 입력 타일 전송**: 이중 버퍼에 재사용 데이터 배치
- **4. 파이프라인 결과 전달**: 공간 병렬 회로가 타일 연속 처리
- **5. 결과 기록·완료 통지**: 호스트 버퍼에 출력 확정
