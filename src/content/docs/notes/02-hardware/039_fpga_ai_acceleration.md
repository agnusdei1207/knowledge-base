...
- **① 서명된 비트스트림 적재 요청**: 모델·정밀도에 맞는 구성 제출
- **② 연산·배선·파이프라인 구성**: LUT·FF·DSP 연결과 단계 설정
- **③ DMA·스트림 경로 구성**: 전송 폭과 스트림 연결 설정
- **④ 텐서 주소·추론 시작 전달**: 입출력 주소·타일 크기 등록
- **⑤ 다음 타일 읽기 요청**: 이중 버퍼의 다음 입력·가중치 요청
- **⑥ 타일 데이터 반환**: 연속 데이터 블록 반환
- **⑦ 온칩 버퍼에 타일 적재**: BRAM·URAM에 재사용 데이터 배치
- **⑧ 파이프라인 입력 데이터 공급**: 개시 간격에 맞춰 데이터 공급
- **⑨ 결과·부분합 타일 반환**: 곱셈·누산 결과를 버퍼에 저장
- **⑩ 결과 스트림 전달**: 완성 타일을 DMA로 전달
- **⑪ 최종 결과 텐서 기록**: 외부 메모리의 출력 주소에 기록
- **⑫ 추론 완료 통지**: 전체 타일 처리 완료 알림
