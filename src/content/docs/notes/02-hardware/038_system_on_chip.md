...
date: "2026-07-25T17:32:00+09:00"
tags:
  - "notes-hardware"
weight: 38
extra:
...
- **시스템온칩(System on Chip, SoC)**: 연산·메모리·입출력 기능을 한 다이에 통합해 지연·전력을 줄인 칩
- **다이(Die)**: 웨이퍼에서 절단해 패키지에 넣는 회로 조각
- **지식재산 블록(Intellectual Property Block, IP Block)**: 재사용 가능한 검증 회로 모듈
- **이기종 연산 IP(Heterogeneous Compute IP)**: CPU·GPU·NPU 등 역할별 연산 블록
- **온칩 네트워크(Network on Chip, NoC)**: IP 간 트랜잭션을 라우팅하는 연결망
- **캐시 일관성(Cache Coherence)**: 공유 데이터의 코어별 복사본을 일치시키는 규칙
- **메모리 맵 입출력(Memory-mapped I/O, MMIO)**: 주변장치를 주소 공간으로 제어하는 방식
- **서비스 품질(Quality of Service, QoS)**: 우선순위·대역폭·지연을 트래픽별 보장
- **전력 도메인(Power Domain)**: 전원을 독립 제어하는 회로 영역
- **클록·전력 게이팅(Clock·Power Gating)**: 유휴 블록의 클록·전원을 차단
- **동적 전압·주파수 조절(Dynamic Voltage and Frequency Scaling, DVFS)**: 부하에 맞춰 전압·주파수 변경
- **패키지 내 시스템(System in Package, SiP)**: 여러 다이를 한 패키지에 연결
...
- **정의/개념**: 연산·메모리·I/O IP를 한 다이에 통합한 시스템
- **배경/필요성**: 칩 간 통신 비용으로 단일 다이 통합 필요
...
- 주소·클록·리셋 계약을 고정해 IP 통합 오류를 방지한다.
...
| 판단 기준 | SoC | SiP | MPU 기반 보드 |
|:---|:---|:---|:---|
| 핵심 특징 | 단일 다이·온다이 NoC | 복수 다이·다이 간 링크 | 독립 칩·PCB 링크 |
| I/O 에너지·지연 | 가장 작음 | 중간 | 가장 큼 |
| 변경 유연성 | 테이프아웃 후 낮음 | 다이 조합 변경 가능 | 모듈 교체가 가장 쉬움 |
| 적용 기준 | 대량·소형·저전력 | 이종 공정·칩렛 결합 | 소량·잦은 사양 변경 |
| 주요 위험 | 대형 다이 수율·검증 | 패키지 수율·발열 | 보드 면적·I/O 전력 |
...
1. 스마트폰 SoC는 ISP·NPU에 NoC 대역폭 예약
2. 차량 SoC는 안전 코어를 독립 전력 도메인에 격리

### 쉽게 이해하기 (학습용)

- 스마트폰은 카메라와 AI 데이터가 제시간에 흐르도록 공용 통로 용량을 먼저 배정한다
- 차량 칩은 일반 구역에 문제가 생겨도 안전 구역이 작동하도록 전원 경계를 나눈다
