...
- **① NPU 전력·클록 활성 요청**: AI 작업 전 NPU 도메인 기동 요청
- **② 전원 공급·클록 시작·초기화 해제**: 정해진 순서로 NPU 활성화
- **③ MMIO 작업 설정 전달**: 입출력 주소와 연산 파라미터 전송
- **④ NPU 제어 레지스터 기록**: 주소를 해석해 NPU에 설정 전달
- **⑤ 메모리 읽기 요청**: 입력·가중치 타일의 주소 발행
- **⑥ 주소 라우팅·QoS 적용**: 목적지 경로와 트래픽 우선순위 결정
- **⑦ 캐시·DRAM 데이터 반환**: 메모리에서 요청 데이터 반환
- **⑧ NPU 입력 데이터 전달**: 응답을 NPU로 보내 타일 연산 수행
- **⑨ 작업 완료 상태 전달**: 연산·출력 기록 후 완료 상태 전송
- **⑩ CPU에 완료 통지**: 인터럽트·상태 경로로 결과 회수 알림
- **⑪ NPU 유휴 상태 전달**: 후속 작업이 없음을 관리기에 통지
- **⑫ 클록·전력 게이팅 적용**: 정책에 따라 클록 또는 전원 차단
