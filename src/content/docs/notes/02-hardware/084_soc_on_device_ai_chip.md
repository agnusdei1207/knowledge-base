...
- **1. 입력 전처리**: 센서 프레임을 모델 형식·정밀도의 텐서로 변환
- **2. 실행 계획 전달**: 지원 연산자는 NPU, 미지원 연산자는 호스트에 배치
- **3. 텐서·버퍼 배치**: 가중치·활성값 타일을 온칩 재사용 계획에 맞춰 공급
- **4. NPU 추론 실행**: 전력·열 한도에서 서브그래프 실행 후 결과 반환
- **5. 응용 결과 제공**: 호스트 후처리를 거쳐 응용 출력 생성
