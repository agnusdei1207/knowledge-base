...
| 절차 | 설명 |
|:---|:---|
| 입력·가중치 요청 | 호스트가 실행할 모델 구간의 데이터를 지정 |
| 텐서 타일 공급 | 메모리·온칩 버퍼가 재사용 단위로 NPU에 전달 |
| 지원 연산 병렬 처리 | NPU가 행렬·벡터 연산을 병렬 실행 |
| DVFS·온도 한도 적용 | 전력·온도 상태에 따라 지속 처리율 제한 |
| 추론 결과 반환 | NPU 처리 결과를 호스트 후처리로 전달 |
| CPU·GPU 처리 | NPU가 지원하지 않는 연산을 호스트가 수행 |

> 요약: NPU가 지원 연산을 처리하고 메모리·열 한도가 지속 성능을 제한
