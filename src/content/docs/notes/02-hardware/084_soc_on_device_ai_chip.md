...
- **정의/개념**: CPU·GPU·NPU를 통합해 단말에서 AI 추론
- **배경/필요성**: 망 지연·원본 전송으로 단말 AI 통합 필요
...
- 연산자 지원·대역폭·재사용이 실효 성능을 정한다.
- 미지원 연산 전환은 지연과 전력 소모를 늘린다.
- 지속 발열과 DVFS가 최고 처리량을 낮춘다.
...
1. 엣지 카메라: 양자화 모델로 원본 영상 로컬 판정
2. 모바일 생성 AI: 발열·메모리 초과 요청만 서버 분담
