...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 스레드가 1번 CPU에서 2번 CPU로 멋대로 도망 다녀서 남의 동네 램 퍼오느라 지연 폭발하는 재앙 | **누마씨티엘(Numactl)** 로 스레드와 메모리를 한 동네에 강력히 못 벗어나게 묶어버림 | 원격 접근(Remote Access) 병목을 씹어먹고 지연시간 없는 극강의 로컬 대역폭 사수 |
