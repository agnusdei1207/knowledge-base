...
date: "2026-08-02T11:53:00+09:00"
tags:
  - "notes-hardware"
weight: 93
extra:
...
- **ccNUMA**: 공유 주소 공간의 캐시 일관성을 유지하되 노드 거리에 따라 메모리 접근 비용이 달라지는 구조이다.
...
- **대칭형 다중처리(SMP)**: 여러 프로세서가 하나의 운영체제와 공유 주소 공간을 대등하게 사용하는 구조이다.
- **로컬·원격 메모리**: 실행 CPU와 같은 NUMA 노드의 메모리와 인터커넥트 너머 다른 노드의 메모리이다.
