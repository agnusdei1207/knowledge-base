...
**도표안 B — ISPP 페이지 프로그램·검증 시퀀스**

```mermaid
sequenceDiagram
    participant H as SSD 컨트롤러
    participant E as ECC 부호기
    participant B as 페이지 버퍼
    participant W as 워드라인 구동부
    participant A as 3D NAND 셀 배열
    participant S as 감지 회로

    H->>E: ① 페이지 데이터 ECC 부호화 요청
    E->>B: ② 데이터·ECC 비트 적재
    H->>W: ③ 대상 워드라인·초기 프로그램 전압 설정
    loop 목표 임계 전압 도달까지
        W->>A: ④ 단계별 프로그램 펄스 인가
        A->>S: ⑤ 셀 임계 전압 검증값 전달
        S-->>H: ⑥ 비트별 검증 결과 반환
...
1. **① 페이지 데이터 ECC 부호화 요청**: SSD 컨트롤러는 기록할 페이지 데이터에 오류 정정용 검사 비트를 만들도록 ECC 부호기에 요청한다.
2. **② 데이터·ECC 비트 적재**: ECC 부호기는 원본과 검사 비트를 페이지 버퍼에 배치해 셀 배열에 기록할 한 페이지를 준비한다.
3. **③ 대상 워드라인·초기 프로그램 전압 설정**: 컨트롤러는 PPA가 가리키는 층의 워드라인을 선택하고 첫 ISPP 전압 단계를 설정한다.
4. **④ 단계별 프로그램 펄스 인가**: 워드라인 구동부는 페이지 버퍼에서 프로그램 대상으로 남은 셀에 짧은 전압 펄스를 인가한다.
5. **⑤ 셀 임계 전압 검증값 전달**: 펄스 뒤 셀 배열을 읽어 각 셀의 임계 전압이 목표 구간에 들어갔는지 감지 회로로 보낸다.
6. **⑥ 비트별 검증 결과 반환**: 감지 회로는 목표 상태에 도달한 비트와 아직 부족한 비트를 SSD 컨트롤러에 알린다.
7. **⑦ 다음 단계 전압 펄스 지시**: 미달 비트가 있으면 컨트롤러는 이미 통과한 비트를 제외하고 전압을 조금 높여 다시 프로그램하도록 지시한다.
8. **⑧ 페이지 버퍼 해제**: 모든 비트가 목표 임계 전압 구간을 통과하면 페이지 버퍼의 프로그램 데이터를 해제한다.
