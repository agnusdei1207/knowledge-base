...
- **① 데이터·ECC 비트 적재**: 부호화한 한 페이지를 버퍼에 준비
- **② 대상 워드라인·초기 프로그램 전압 설정**: 기록할 층과 첫 전압 선택
- **③ 단계별 프로그램 펄스 인가**: 미완료 셀에 짧은 전압 펄스 인가
- **④ 셀 임계 전압 검증값 전달**: 펄스 후 임계 전압 상태 감지
- **⑤ 비트별 검증 결과 반환**: 목표 도달 여부를 컨트롤러에 반환
- **⑥ 다음 단계 전압 펄스 지시**: 미달 셀만 전압을 높여 재프로그램
- **⑦ 페이지 버퍼 해제**: 모든 셀이 목표 구간에 도달하면 버퍼 해제
