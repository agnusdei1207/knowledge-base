...
1. **① 메모리 주소와 읽기 요청 전달**: 프로세서 코어는 필요한 데이터의 주소와 읽기 요청을 캐시 제어기의 주소 분해기에 전달한다.
2. **② 인덱스·태그·유효 비트 조회**: 주소 분해기는 주소를 태그·인덱스·오프셋으로 나누고, 인덱스가 가리킨 집합의 태그와 유효 비트를 조회한다.
3. **③ 적중 여부와 일치 웨이 반환**: 태그 탐색부는 요청 태그를 후보 웨이들과 비교해 적중 여부와 일치한 웨이를 주소 분해기에 알린다.
4. **④ 일치 웨이·오프셋 전달**: 적중이면 주소 분해기는 데이터 배열에 일치 웨이와 라인 내부 오프셋을 전달한다.
5. **⑤ 요청 데이터 반환**: 데이터 배열은 선택한 웨이의 요청 바이트를 프로세서 코어에 반환한다.
6. **⑥ 미스와 후보 웨이 상태 전달**: 일치 태그가 없으면 주소 분해기는 미스 사실과 후보 웨이 상태를 교체 제어부에 전달한다.
7. **⑦ 요청 블록 리필 요청**: 교체 제어부는 교체 정책으로 축출 웨이를 정하고 하위 메모리 계층에 요청 주소가 속한 블록을 요구한다.
8. **⑧ 캐시 라인 크기의 블록 반환**: 하위 메모리 계층은 요청 데이터를 포함한 전체 캐시 라인을 반환한다.
9. **⑨ 새 태그·유효 비트 갱신**: 교체 제어부는 선택한 웨이의 태그를 새 블록의 태그로 바꾸고 유효 비트를 설정한다.
10. **⑩ 선택 웨이에 블록 저장**: 반환된 블록을 데이터 배열의 선택 웨이에 채워 이후 접근이 적중할 수 있게 한다.
11. **⑪ 요청 오프셋의 데이터 반환**: 리필된 라인에서 원래 요청한 오프셋의 데이터를 골라 프로세서 코어에 반환한다.
