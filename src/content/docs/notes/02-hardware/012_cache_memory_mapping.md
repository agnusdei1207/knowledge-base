...
- **1. 주소 분해·태그 조회**: 후보 웨이 태그 조회
- **2. 적중 여부 판정**: 유효 비트·태그 비교
- **3. 요청 데이터 반환**: 적중 웨이 바이트 반환
- **4. 교체 웨이 선택**: 미스 후보 상태 판정
- **5. 요청 블록 리필**: 하위 계층 블록 저장
