...
- **평균 메모리 접근 시간(Average Memory Access Time, AMAT)**: 위 세 값을 합쳐 계산한 평균 접근 지연이며, 연관도를 올릴지 말지는 결국 이 값이 줄어드는지로 판정한다
...
- **1단계·2단계 캐시(L1·L2 Cache)**: ‘엘 원·엘 투’로 읽으며 코어에 가장 가까워 빠른 응답을 맡는 쪽이 L1, 그 아래에서 더 큰 용량으로 미스를 받아 주는 쪽이 L2다
- **마이크로컨트롤러(Microcontroller Unit, MCU)**: 프로세서·메모리·주변 장치를 한 칩에 담은 제어용 장치이며 캐시가 작고 응답 지연이 우선인 대표 사례다
