...
- **적중 시간•미스율•미스 페널티**: 각각 적중 데이터 반환 시간, 전체 접근 중 미스 비율, 미스 한 번의 추가 지연
...
- **일치 웨이•오프셋**: 태그가 맞는 캐시 라인과 그 라인 안에서 요청 바이트의 위치를 나타내는 정보
- **태그•유효 비트 갱신**: 리필한 블록의 식별 태그를 기록하고 해당 라인이 사용 가능함을 표시하는 동작
...
- **웨이 선택**: 집합 안에서 태그가 일치한 라인이나 교체할 라인을 결정하는 동작
...
- **L2 캐시**: 1차 캐시 미스를 더 낮은 지연으로 흡수하도록 두는 2차 캐시 계층
- **랜덤 교체 정책**: 미스 시 후보 웨이 가운데 하나를 무작위로 골라 축출하는 단순 정책
- **MCU**: 프로세서•메모리•주변장치를 통합한 마이크로컨트롤러로 면적과 지연 제약이 크다.
...
- **직접•완전 연관•집합 연관 매핑**: 각각 한 라인, 전체 라인, 선택한 집합의 여러 웨이를 배치 후보로 삼는 방식
