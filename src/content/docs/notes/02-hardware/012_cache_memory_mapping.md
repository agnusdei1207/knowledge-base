...
2. **L2 캐시**는 미스 감소 이득이 남는 지점까지 웨이 수 상향

### 쉽게 이해하기 (학습용)

- 제어용 소형 칩은 캐시 자체가 작아 이름표를 여러 개 비교할 회로를 넣을 자리도 없고 응답이 한 박자 늦는 것이 곧 제어 실패이므로, 후보를 한 칸으로 못 박아 조회를 가장 짧게 끝낸다
- 코어 뒤에 있는 큰 캐시는 이미 L1에서 걸러지고 남은 어려운 접근만 받으므로 헛걸음 한 번의 값이 매우 비싸고, 그래서 웨이를 늘려 얻는 미스 감소가 비교 회로 값을 넘는 지점까지는 계속 늘린다

## Ⅶ. 결론

- **충돌 미스** 이득이 태그 비교 비용을 넘을 때까지 연관도 상향
