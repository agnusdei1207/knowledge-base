...
> 요약: 스케줄러가 워프를 실행 유닛과 메모리 계층에 연결

**동작 원리**

- **① 스레드 블록 배치**: 자원이 충분한 SM에 블록 할당
- **② 워프 레지스터·공유 메모리 할당**: 스레드 상태와 블록 공유 공간 확보
- **③ 활성 마스크와 공통 명령 발행**: 준비된 워프에 한 명령 발행
- **④ 스레드별 연산 완료**: 활성 스레드의 서로 다른 데이터에 같은 연산 수행
- **⑤ 워프 주소 병합 요청**: 인접 주소를 적은 메모리 트랜잭션으로 결합
- **⑥ 요청 데이터 반환**: L2 적중 데이터를 워프에 전달
- **⑦ 전역 메모리 트랜잭션**: L2 미스 요청을 전역 메모리로 전달
- **⑧ 메모리 블록 반환**: 블록을 L2에 채우는 동안 다른 워프 실행
- **⑨ 워프 데이터 반환**: 대기 워프를 다시 실행 가능 상태로 전환
- **⑩ 경로별 활성 마스크 반환**: 분기별 스레드 집합 생성
- **⑪ 첫 번째 경로 명령 발행**: 첫 마스크의 스레드만 실행
- **⑫ 두 번째 경로 명령 발행**: 반대 마스크의 경로를 순차 실행
- **⑬ 스레드 블록 완료 통지**: 모든 워프 완료 후 자원 해제
