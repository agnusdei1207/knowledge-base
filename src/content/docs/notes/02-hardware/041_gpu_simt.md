...
date: "2026-07-25T17:35:00+09:00"
tags:
  - "notes-hardware"
weight: 41
extra:
...
- **단일 명령 다중 스레드(Single Instruction, Multiple Threads, SIMT)**: 워프의 활성 스레드에 공통 명령을 발행하는 실행 모델
- **스트리밍 멀티프로세서(Streaming Multiprocessor, SM)**: 워프 스케줄링·연산·온칩 메모리의 실행 단위
...
- **단일 명령 다중 데이터(Single Instruction, Multiple Data, SIMD)**: 하나의 벡터 명령을 여러 데이터 레인에 동시에 적용하는 실행 방식
...
- **정의/개념**: 워프별 공통 명령으로 데이터 병렬 처리하는 구조
- **배경/필요성**: 동일 연산 대량 처리로 워프 병렬 실행 필요
...
- 워프 전환은 메모리 대기를 숨겨 실행 자원 활용률을 높인다.
- 분기 수렴·병합 접근·점유율이 SIMT 처리량을 좌우한다.
...
| 비교축 | GPU SIMT | CPU SIMD | CPU 스레드 |
|:---|:---|:---|:---|
| 핵심 특징 | 워프의 다수 스레드 | 고정 폭 벡터 레인 | 독립 명령 흐름 스레드 |
| 제어 방식 | 공통 명령·활성 마스크 | 벡터 명령·마스크 | 스레드별 분기 제어 |
| 메모리 활용 | 병합 접근·공유 메모리 | 연속 벡터·캐시 | 범용 캐시·분기 예측 |
| 적용 기준 | 대규모 동일 연산 | 벡터화 가능한 반복문 | 복잡한 분기·낮은 병렬성 |
| 주요 위험 | 분기 발산·비병합 접근 | 벡터 폭 미활용 | 문맥 전환·동기화 비용 |
...
1. 영상 필터: 연속 화소를 워프에 배치해 병합 접근
2. 조건 분기 커널: 입력을 분류해 워프 발산 완화
...
- GPU는 발산·메모리 비용보다 병렬 이득이 클 때 선택한다
