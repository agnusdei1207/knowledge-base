...
- 정의/개념: GPU의 SM이 다수 스레드를 워프 단위로 묶어 공통 명령을 발행하는 **대규모 데이터 병렬 실행 구조**이다.
- 배경/필요성: CPU의 소수 복잡 코어만으로는 많은 데이터 원소의 동일 연산을 동시에 처리하기 어려워 **병렬 처리량 제약**이 발생한다.
...
- 메모리 대기 시간을 숨기는 **워프 전환**이 핵심이다.
- 분기 경로를 순차 실행하는 **활성 마스크**가 핵심이다.
- 메모리 처리량을 좌우하는 **병합 접근률•점유율**이 핵심이다.
...
1. **SM 배치·자원 할당**: 블록의 스레드 수에 맞춰 레지스터와 공유 메모리를 확보하고 상주 워프를 생성한다.
2. **준비 워프 명령 발행**: 워프 스케줄러가 실행 가능한 워프를 선택해 활성 스레드에 공통 명령을 발행한다.
3. **메모리 접근 병합**: 워프의 연속 주소 요청을 가능한 적은 전역 메모리 트랜잭션으로 결합한다.
4. **분기 활성 마스크 실행**: 워프 내부 경로가 갈리면 경로별 활성 마스크를 적용해 각 분기를 순차를 실행한다.
...
- **커널 융합(Kernel Fusion)**: 연속된 커널을 하나로 결합해 실행 시작 비용과 중간 데이터의 전역 메모리 왕복을 줄이는 최적화한다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 한 워프의 스레드가 다른 분기를 선택해 **경로별 순차 실행** | 같은 경로의 데이터를 묶고 **활성 마스크 효율** 측정 | 비활성 레인 감소로 **레인 활용률 향상** |
| 인접 스레드가 떨어진 주소를 요청해 **메모리 트랜잭션 증가** | 자료 구조와 인덱스를 워프의 **연속 주소 접근**에 맞게 배치 | 병합 요청 증가로 **메모리 대역폭 효율 향상** |
| 블록당 레지스터·공유 메모리 사용이 커서 **상주 워프 감소** | 블록 크기와 스레드당 자원량을 조정해 **점유율·지연 은닉** 비교 | 자원 한도 안에서 **준비 워프 수 균형** |
| 실행 시간이 짧은 커널에서 시작·전송 비용이 **연산 시간 지배** | **커널 융합·비동기 전송**으로 호출과 중간 데이터 왕복 축소 | 오버헤드 비율 감소로 **유효 처리량 향상** |
...
- **동일 연산•병합 접근**은 GPU, **복잡한 분기**는 CPU을 선택한다.
