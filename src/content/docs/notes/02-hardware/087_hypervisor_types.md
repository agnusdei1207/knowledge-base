...
- **정의/개념**: Type 1 직접·Type 2 호스트 실행 계층
- **배경/필요성**: 격리·호스트 재사용 상충으로 유형 선택 필요
...
- 실행 위치가 I/O 경로·장애 범위·장치 지원을 바꾼다.
...
| 호스트 OS | Type 2에 장치·시스템 서비스 제공 |
| 하이퍼바이저 | 자원 배정·명령 중재·VM 격리 |
| 게스트 VM | 가상 자원에서 게스트 OS 실행 |
...
| 절차 | 설명 |
|:---|:---|
| Type 1 I/O 요청 | 게스트 I/O가 하이퍼바이저에 진입 |
| 하이퍼바이저 장치 중재 | 호스트 OS 없이 I/O 경로 중재 |
| Type 2 I/O 요청 | 게스트 I/O가 Type 2 계층에 진입 |
| 호스트 시스템 호출 | 호스트 OS에 장치 처리 위임 |
| 호스트 장치 접근 | 호스트 드라이버가 장치 요청 처리 |

> 요약: Type 2 I/O는 호스트 OS까지 경유
...
| 주요 위험 | 드라이버 호환·관리 복잡성 | 호스트 장애·추가 I/O 경로 |
...
1. 프라이빗 클라우드는 Type 1으로 서버 자원 통합
2. 개발 노트북은 Type 2로 다중 OS 시험

### 쉽게 이해하기 (학습용)

- 회사 클라우드는 서버 자원을 나눠 쓰려고 Type 1을 설치한다.
- 개발자는 노트북 운영체제를 유지한 채 다른 OS를 시험하려고 Type 2를 쓴다.

## Ⅶ. 결론

- 서버 통합은 Type 1, 데스크톱 시험은 Type 2 선택
