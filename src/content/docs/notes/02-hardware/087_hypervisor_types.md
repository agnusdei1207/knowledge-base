...
- 배경/필요성: 무중단 서버 가상화 통합(Type 1) 및 개발/클라이언트 PC 상의 가변 하드웨어 인프라 수용(Type 2) 필요성
...
- 고성능, 하드웨어 직결 억세스 및 높은 보안 격리성을 보장하는 **Type 1 (Bare-Metal)**
- 기존 OS 환경과 풍부한 하드웨어 드라이버를 100% 재활용하는 **Type 2 (Hosted)**
- 서비스 무중단 서버 가상화(Type 1) vs 개발자 가상 테스트 환경(Type 2) 용도 명확화
...
3. **물리 I/O 실행 및 반환**: 하드웨어 전송 완결 후 완료 신호를 가상 인터럽트로 **게스트 VM**에 반환.
...
| 성능 및 오버헤드 | 최상급 전송 성능, 최저 오버헤드 (~1-2%) | 호스트 OS 이중 경유에 따른 입출력 지연 오버헤드 |
| 하드웨어 호환성 | 검증된 전용 하드웨어 드라이버 수용 제한 | 호스트 OS가 지원하는 모든 하드웨어 사용 가능 |
| 장애 파급성 | 단일 VM 장애가 타 VM에 영향 없음 | **호스트 OS Crash** 발생 시 전 기동 VM 동시 다운 |
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| **Type 2** 호스트 OS 결함 발생 시 전 VM 동시 다운 | 데이터센터 서버 상에 **Type 1 (ESXi/KVM)** 적용 | 하드웨어 독립성 및 고가용성 인가 |
| 과도한 **vCPU Overcommit**으로 인한 CPU Ready 타임 지연 | vCPU 대 물리 코어 비율(예: 1:4) 제한 | 게스트 반응 지연시간 억제 |
| 가상 I/O 처리 시 호스트 CPU 연산 자원 소모 | **SR-IOV** 하드웨어 직결 적용 | 하이퍼바이저 I/O 처리 오버헤드 소멸 |
...
- **하이퍼바이저 선택 기준(Hypervisor Type Selection Criteria)**: 서비스 운영 환경(Enterpise vs Dev), 성능 지연 타깃, 비용에 따른 체계.
...
- 실행 위치 및 호스트 OS 종속성에 따른 Type 1(Bare-Metal)/Type 2(Hosted) 하이퍼바이저 차등 채택 및 최적 서버 가상화 체계 적용.
