...
- 정의/개념: 하드웨어 위에 직접 실행되는 Type 1과 호스트 OS 위에서 실행되는 Type 2로 구분하는 **VM 자원 중재 계층**이다.
- 배경/필요성: 서버의 상시 격리•성능 요구와 개발 환경의 기존 OS•장치 드라이버 재사용 요구가 달라 배치 방식별 선택이 필요하다.

#### 한줄 요약

- **하이퍼바이저 실행 위치**에 따른 Type 1•Type 2을 구분한다.
...
- **하이퍼바이저 실행 위치**가 I/O 경로•장애 범위를 결정한다.
- Type 1은 **호스트 OS 없는 직접 자원 중재**를 제공한다.
- Type 2는 **호스트 OS 드라이버•도구 재사용**을 제공한다.

#### 한줄 요약

- **직접 자원 중재•호스트 OS 재사용**의 배치 차이 핵심이다.
...
- **Type 1 직접 배치•Type 2 호스트 매개 배치**가 핵심이다.
...
1. **가상 장치 요청 해석**: 하이퍼바이저가 게스트의 가상 레지스터 또는 큐 요청을 읽어 대상 장치와 권한을 확인한다.
...
- Type 1은 하이퍼바이저가 자원을 직접 중재하고, Type 2는 **호스트 OS 장치 계층을 추가로 거쳐** I/O를 수행한다.
...
> 요약: 상시 서버 운영은 **Type 1**, 개발•검증은 **Type 2**을 선택한다.

#### 한줄 요약

- **상시 운영•개발 시험**에 따른 Type 1•Type 2을 선택한다.
...
- **Type 1 관리면 최소화•Type 2 호스트 자원 통제**가 핵심이다.
...
- 상시 서버는 **Type 1**, 개발•시험은 **Type 2**을 선택한다.
