...
> 요약: Type 1은 하드웨어, Type 2는 호스트 OS 위에 배치한다.

**동작 원리**

- **① 가상 I/O**: 게스트가 가상 장치 레지스터·큐로 요청
- **② Type 1 중재**: 하이퍼바이저가 직접 장치와 격리 정책 처리
- **③ Type 1 완료**: 장치가 결과를 하이퍼바이저에 반환
- **④ Type 1 통지**: 가상 완료 큐·인터럽트로 게스트에 전달
- **⑤ Type 2 호출**: 하이퍼바이저가 호스트 OS 서비스 요청
- **⑥ 드라이버 연결**: 호스트 OS가 기존 드라이버에 전달
- **⑦ 물리 명령**: 드라이버가 장치별 명령으로 변환
- **⑧ 장치 반환**: 물리 장치가 드라이버에 완료 통지
- **⑨ 호스트 완료**: 드라이버가 호스트 OS 요청 완료
- **⑩ 결과 수신**: 호스트 OS가 하이퍼바이저에 반환
- **⑪ 게스트 완료**: 가상 장치 응답으로 바꿔 게스트에 전달
...
- 호스트 OS 의존성·장치 접근·공격면·배포 목적을 검토해 서버 통합에는 Type 1, 데스크톱 시험에는 Type 2를 선택한다.
