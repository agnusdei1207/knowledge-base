...
sequenceDiagram
    ...
    participant R as 분류 결과

    A->>B: ① 대상 코어·칩·노드 경계 지정
    B->>I: ② 독립 제어 장치·명령 흐름 관찰 요청
    I->>F: ③ 단일·다중 명령 스트림 판정 전달
    B->>D: ④ 동시 처리 데이터 흐름 관찰 요청
    D->>F: ⑤ 단일·다중 데이터 스트림 판정 전달
    alt 단일 명령·단일 데이터
        F->>R: ⑥ SISD로 분류
    else 단일 명령·다중 데이터
        F->>R: ⑦ SIMD로 분류
...
1. **① 대상 코어·칩·노드 경계 지정**: 분석자는 단일 코어 내부, 멀티코어 칩 전체, 여러 노드 중 어디까지를 하나의 시스템으로 볼지 먼저 정한다.
2. **② 독립 제어 장치·명령 흐름 관찰 요청**: 지정 경계 안에서 서로 다른 명령을 독립적으로 발행할 수 있는 제어 단위가 몇 개인지 확인한다.
3. **③ 단일·다중 명령 스트림 판정 전달**: 하나의 제어 흐름만 있으면 단일, 독립 프로그램 카운터와 명령 흐름이 여러 개이면 다중 명령으로 판정한다.
4. **④ 동시 처리 데이터 흐름 관찰 요청**: 같은 시점에 연산되는 독립 데이터 원소나 입력 흐름이 하나인지 여러 개인지 확인한다.
5. **⑤ 단일·다중 데이터 스트림 판정 전달**: 스칼라 값 하나만 처리하면 단일, 벡터 레인·여러 작업 데이터가 동시에 처리되면 다중 데이터로 판정한다.
6. **⑥ SISD로 분류**: 명령과 데이터가 모두 단일이면 전통적인 스칼라 순차 처리 구조로 분류한다.
7. **⑦ SIMD로 분류**: 하나의 공통 명령이 여러 데이터 흐름에 적용되면 벡터·데이터 병렬 구조로 분류한다.
8. **⑧ MISD로 분류**: 여러 명령 흐름이 동일 데이터 흐름을 처리하면 희귀한 다중 처리·검증 구조로 분류한다.
9. **⑨ MIMD로 분류**: 여러 제어 흐름이 각기 다른 데이터를 처리하면 멀티코어·클러스터 구조로 분류한다.
10. **⑩ 분석 경계와 분류 근거 반환**: 최종 결과에는 범주 이름만 쓰지 않고 경계와 독립 명령·데이터 스트림을 센 근거를 함께 기록한다.
