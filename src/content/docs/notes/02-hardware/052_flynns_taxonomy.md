...
> 요약: 경계를 정하고 두 스트림 수를 행렬에 조합

**동작 원리**

- **① 대상 코어·칩·노드 경계 지정**: 하나의 시스템으로 볼 범위 확정
- **② 독립 제어 장치·명령 흐름 관찰 요청**: 독립 명령 발행 단위 확인
- **③ 단일·다중 명령 스트림 판정 전달**: 독립 명령 흐름 수 판정
- **④ 동시 처리 데이터 흐름 관찰 요청**: 동시에 연산하는 데이터 단위 확인
- **⑤ 단일·다중 데이터 스트림 판정 전달**: 스칼라·다중 데이터 여부 판정
- **⑥ SISD·경계·근거 반환**: 단일 명령·단일 데이터로 분류
- **⑦ SIMD·경계·근거 반환**: 단일 명령·다중 데이터로 분류
- **⑧ MISD·경계·근거 반환**: 다중 명령·단일 데이터로 분류
- **⑨ MIMD·경계·근거 반환**: 다중 명령·다중 데이터로 분류
...
|:---|:---|:---|
| 단일 명령 스트림 | **SISD** — 스칼라 순차 처리 | **SIMD** — 벡터·SIMT 데이터 병렬 |
