...
- **프로그램 카운터(PC)** 는 다음에 인출할 명령어의 주소를 보관하는 레지스터이다.
- **ALU** 는 EX 단계에서 산술·논리 연산과 메모리 주소 계산을 수행하는 연산 장치이다.
...
- **IF/ID·ID/EX 파이프라인 레지스터** 는 앞 단계의 결과와 제어 신호를 클록 경계에서 저장해 다음 단계로 전달한다.
...
- **단계 분할·회로 재배치** 는 긴 처리 단계를 나누거나 논리를 다른 단계로 옮겨 최장 단계 지연을 낮추는 방법이다.
...
- **버블·플러시** 는 각각 해저드로 생긴 빈 주기와 잘못 인출한 명령을 제거하는 동작으로 IPC를 낮춘다.
