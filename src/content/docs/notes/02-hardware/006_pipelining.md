...
#### 한줄 요약
- 개별 명령어 execution latency는 유지하되 Stage Overlapping을 통해 파이프라인 단계 수에 비례하는 Throughput 증대를 달성함.
...
|:---|:---|:---|
| **IF (Instruction Fetch)** | PC 주소 기반 I-Cache 명령어 인출 및 PC+4 계산 | 1클록 내 명령어 연속 공급 및 파이프라인 입구 관장 |
| **ID (Instruction Decode)** | Op-code 디코딩 및 Register File Read 포트 접근 | 피연산자 추출 및 타깃 파이프라인 제어 신호 묶음 생성 |
| **EX (Execute)** | ALU 연산 수행 및 Branch 타깃 주소 산출 | 산술 논리 실행 및 데이터 해저드 감지 포인트 제공 |
| **MEM (Memory Access)** | D-Cache Read/Write 및 메모리 접근 신호 처리 | Load/Store 인스트럭션 데이터 메모리 억세스 완결 |
| **WB (Write Back)** | 연산/로드 결과를 레지스터 파일에 최종 래칭 | 1개 명령어 실행 cycle의 최종 완료(Commit) 담당 |
| **파이프라인 레지스터** | IF/ID, ID/EX, EX/MEM, MEM/WB 사이 데이터 격리 | 이전 단계의 데이터와 제어 신호가 섞이지 않도록 클록 동기화 보장 |
...
1. **시동 단계 (Fill Phase)**: 첫 4개 클록 주기(CC1~CC4) 동안 파이프라인 레지스터에 차례로 명령어 1~4가 투입되어 레지스터 라인을 채움.
2. **정상 상태 (Steady State)**: CC5부터 파이프라인 전 단계가 가동되며 매 클록(CC5, CC6, CC7...)마다 WB 단계를 통과한 명령어 1개씩 최종 완료 처리(IPC=1)됨.
3. **제어/데이터 이동**: 각 파이프라인 레지스터(IF/ID, ID/EX...)는 클록 엣지 시점에 이전 단계의 연산 데이터 및 **제어 신호(Control Signal)**를 동기 래칭하여 다음 단계로 이동시킴.
...
- **파이프라인 개선 기준(Pipeline Optimization Criteria)**에 입각하여 고성능 프로세서 설계 시 5단계 기본 파이프라인 구조를 바탕으로 포워딩 회로 및 분기 예측기를 내장하고, 클록 주파수 확장을 위한 슈퍼파이프라이닝 단계를 구성하여 CPI 대기 손실 방지 및 파이프라인 처리량 확보 체계 적용 필수.

#### 한줄 요약
- Execution Time 공식을 기반으로 Forwarding 및 Branch Prediction을 통합하여 CPI 손실을 차단하고 Stage Balancing을 통해 클록 주파수를 극대화하는 파이프라이닝 설계 체계 적용.
