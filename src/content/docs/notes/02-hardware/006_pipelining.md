...
- **① 명령어 인출·IF/ID 전달**: 명령어와 프로그램 카운터 값을 IF/ID 레지스터에 저장
- **② 해독·피연산자 전달**: 피연산자·제어 신호를 ID/EX 레지스터로 전달
- **③ 연산·주소 결과 전달**: ALU·주소 계산 결과를 EX/MEM 레지스터에 저장
- **④ 메모리·연산 결과 전달**: 메모리 접근값·ALU 결과를 MEM/WB 레지스터로 전달
- **⑤ 결과 기록·다음 인출**: 최종 값을 레지스터에 기록하며 후속 명령 인출 지속
