...
date: "2026-07-25T16:10:00+09:00"
tags:
  - "notes-hardware"
weight: 6
extra:
...
- **명령어 인출(Instruction Fetch, IF)**: 메모리에서 다음에 실행할 명령어를 가져오는 첫 단계
- **명령어 해독(Instruction Decode, ID)**: 가져온 명령어가 무슨 연산인지 해석하고 필요한 피연산자를 레지스터에서 읽는 단계
- **실행(Execute, EX)**: 산술논리 연산이나 메모리 주소 계산을 실제로 수행하는 단계
- **메모리 접근(Memory Access, MEM)**: 적재·저장 명령이 데이터 메모리를 읽거나 쓰는 단계
- **결과 기록(Write Back, WB)**: 산출된 결과를 목적 레지스터에 적어 명령을 마치는 단계
...
- **RISC-V**: 사례에서 5단계 파이프라인을 얹는 개방형 명령어 집합 규격이며, 명령 형식이 단순해 단계를 고르게 나누기 좋다
...
| 적재 데이터 전달 | 읽은 메모리 값을 **기록 대상**으로 전달 |
