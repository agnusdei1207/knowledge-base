...
date: "2026-07-28T21:54:11+09:00"
tags:
  - "notes-hardware"
weight: 6
extra:
...
- **① 명령어 인출**: 프로그램 카운터가 가리키는 명령어를 메모리에서 읽고 다음 인출 주소 준비
- **② IF/ID 전달**: 인출한 명령어와 프로그램 카운터 값을 IF/ID 파이프라인 레지스터에 저장
- **③ 해독·피연산자 전달**: 명령을 해독하고 레지스터 피연산자·제어 신호를 ID/EX 레지스터로 전달
- **④ 연산·주소 결과 전달**: ALU 연산 또는 적재·저장 주소 계산 결과를 EX/MEM 레지스터에 저장
- **⑤ 메모리·연산 결과 전달**: 데이터 메모리 접근값 또는 ALU 결과를 MEM/WB 레지스터로 전달
- **⑥ 결과 기록**: 목적 레지스터가 있는 명령의 최종 값을 레지스터 파일에 반영
