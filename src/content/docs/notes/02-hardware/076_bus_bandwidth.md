...
- 배경/필요성: 규격상 물리 속도와 실제 유효 페이로드 처리량 간의 갭을 분석하여 시스템 인터커넥트 병목을 사전에 방지 요구성
...
- 버스 클록 에지(DDR/QDR) 인가 횟수에 따른 **전송률(GT/s)** 수치화
...
| 수신 인터페이스 | 디페이로드 추출 및 데이터 버퍼 전송 |
| 측정기 | 실효 **처리량(Throughput)** 및 버스 점유율(Utilization) 계측 |
...
4. **실측 사용률 분석**: **실측 사용률(U)** 및 중재 병목 구간 도출.
...
| 대역폭 확충 | 비트 라인 수(32->64bit) 확충 | 레인 수(x1->x16) 및 GT/s 속도 확충 |
...
- **대역폭 개선 기준**에 따라 링크 사용률이 포화 상태일 경우 물리 레인 확충 또는 상위 세대(Gen) 인터페이스 적용

#### 한줄 요약

- 이론 대역폭 산정, 인코딩/프로토콜 오버헤드 차감 및 실측 처리량 평가 기반 버스 병목 제어 체계 적용.
