...
- 정의/개념: 버스 폭과 전송률로 **이론 대역폭**을 계산하고 프로토콜 효율과 사용률을 반영하여 실제 페이로드 처리량을 산정하는 방법이다.
- 배경/필요성: 링크의 규격 속도만으로는 인코딩•프로토콜 오버헤드와 경합 및 흐름 제어 손실을 알 수 없어 실제 병목을 판단하기 어렵다.

#### 한줄 요약

- **이론 대역폭**에 프로토콜 효율과 경합 영향을 반영해야 수신자가 얻는 실측 처리량을 설명할 수 있다.
...
- **버스 폭•전송률•효율•사용률**을 순서대로 반영하여 실제 페이로드 전달량을 산정한다.
...
- **송신 형식화•링크 전송•수신 복원•처리량 계량** 구조이다.
...
- **이론 상한과 실측 처리량**의 차이를 분석하여 인코딩, 프로토콜, 경합 및 흐름 제어 손실을 식별한다.
...
- **측정 조건(Measurement Condition)**: 메시지 크기와 동시성, 방향, 시간 및 큐 깊이처럼 처리량 결과에 영향을 주는 설정한다.
...
- **단위•방향•메시지 크기•부하 조건**을 고정한 뒤 처리량과 지연을 함께 측정해야 실제 병목을 비교할 수 있다.
...
- **링크 사용률**이 낮으면 증설보다 프로토콜•경합 병목부터 개선한다.
