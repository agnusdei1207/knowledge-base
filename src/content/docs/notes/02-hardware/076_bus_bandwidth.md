...
- **실측 처리량(Measured Throughput)**: 측정 시간 동안 수신자가 전달받은 페이로드 양을 시간으로 나눈 값이다.

</details>

- 정의/개념: **전송률**과 버스 폭으로 **버스 대역폭**을 계산하고 프로토콜 효율과 사용률을 반영하여 **실측 처리량**을 해석하는 방법이다.
- 배경/필요성: 링크의 규격 속도만으로는 인코딩•프로토콜 오버헤드와 경합 및 흐름 제어 손실을 알 수 없어 병목을 판단하기 어렵다.
...
- **페이로드(Payload)**: 프로토콜 헤더와 오류 제어 정보를 제외하고 수신 응용이 사용하는 데이터이다.
...
| 공유 버스 경합을 재현하지 않아 처리량 과대평가 | 메시지 크기•동시성 재현 | 병목 식별 |
