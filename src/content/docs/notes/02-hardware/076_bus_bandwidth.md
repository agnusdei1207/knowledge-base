...
- **초당 메가 전송(Megatransfers per Second, MT/s)**: MT/s는 ‘메가트랜스퍼 퍼 세컨드’로 읽고 초당 백만 번의 전송을 나타내는 단위 표기로, 버스의 전송 횟수를 표현
- **초당 기가 전송(Gigatransfers per Second, GT/s)**: GT/s는 ‘기가트랜스퍼 퍼 세컨드’로 읽고 초당 십억 번의 전송을 나타내는 단위 표기로, 고속 직렬 링크의 전송 횟수를 표현
- **초당 바이트(Bytes per Second, Byte/s)**: Byte/s는 ‘바이트 퍼 세컨드’로 읽고 슬래시를 단위 시간당 데이터량에 쓰는 표준 단위 표기로, 이론·실측 전송률을 초당 바이트 수로 비교
...
- **이중 데이터 전송률(Double Data Rate, DDR)**: DDR은 ‘디디알’로 읽고 영문 머리글자를 딴 약어이며, 클럭 양 에지에서 전송해 MT/s에 배수를 반영
...
2. 스토리지 백플레인의 드라이브 합산량·상위 링크 상한 비교

### 쉽게 이해하기 (학습용)

- 그래픽처리장치가 메모리를 옮기는 속도를 링크 한계와 비교해 병목을 찾는다.
- 여러 저장장치 속도 합이 상위 통로를 넘으면 백플레인이 병목이 된다.
