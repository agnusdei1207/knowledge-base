...
- 정의/개념: 태스크의 **응답시간·데드라인**을 관리하는 OS
- 배경/필요성: 최악 지연 상한과 시간 예측성 확보
...
- 이벤트가 오면 준비된 작업 중 가장 긴급한 태스크를 고른다.
...
- **1. 이벤트 태스크 등록**: 준비 상태로 전환
- **2. 최고 우선순위 선택**: 준비 큐와 정책에 따라 마감시간을 지킬 실행 후보 결정
- **3. 태스크 문맥 복원**: 실행 상태 재개
- **4. 공유 자원 동기화**: 잠금·이벤트 조건을 판정해 자원 소유권 부여
- **5. 상태 갱신·재스케줄**: 완료·대기 상태를 큐에 반영하고 다음 태스크 선택
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 우선순위 역전 | 상속·천장 프로토콜 적용 | **차단 시간 제한** |
| 긴 ISR 실행 | ISR 최소화와 지연 처리 | **응답 지연 감소** |
| 스택 고갈 | 중첩 분석·워터마크 감시 | **메모리 손상 방지** |
| 최악 조건 누락 | WCET·WCRT 분석 | **데드라인 입증** |
...
- WCRT와 차단시간을 기준으로 RTOS 데드라인을 입증한다.
