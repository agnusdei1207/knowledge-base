...
sidebar:
  ...
  badge:
    ...
    variant: note
title: "실시간 운영체제 (RTOS)"
date: "2026-07-25T10:26:00+09:00"
tags:
  - "notes-hardware"
weight: 61
extra:
...
- **정의/개념**: 데드라인 충족을 예측하는 운영체제다
- **배경/필요성**: 최악 응답시간을 입증해 제어 실패를 막는다
...
- 평균 처리량보다 지연 상한 예측을 우선한다
- 우선순위 선점은 긴급 태스크의 응답 지연을 줄인다
- 차단·ISR·문맥 전환은 WCRT를 늘려 마감을 놓치게 한다
...
| 판단 기준 | RTOS | 베어메탈 | 범용 운영체제 |
|:---|:---|:---|:---|
| 핵심 특징 | 태스크 우선순위와 지연 상한 분석 | 슈퍼루프·인터럽트 직접 제어 | 프로세스 시분할·풍부한 서비스 |
| 적용 기준 | 다중 태스크의 데드라인 입증 | 단일 제어·최소 자원 | 파일·가상 메모리·시분할 필요 |
| 주요 위험 | 우선순위 역전·차단·지터 | 기능 확장 시 코드 결합 증가 | 비결정적 지연·큰 메모리 사용 |
...
1. 모터 제어: WCET·차단으로 제어 주기를 검증한다
2. 의료 경보: ISR을 줄이고 경보를 우선 실행한다
