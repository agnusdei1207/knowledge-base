...
- 배경/필요성: 슈퍼루프에서는 다중 태스크의 **WCRT 상한 입증 곤란**

#### 한줄 요약

- **우선순위 스케줄링**으로 데드라인을 통제하는 운영체제
...
> **WCRT 수렴값**이 데드라인 이하면 실시간 요구 충족

#### 한줄 요약

- 평균 처리량보다 **최악 응답시간 상한**을 우선하는 결정성
...
1. **이벤트 준비**: 대기 조건을 충족한 태스크를 준비 상태로 전환
2. **실행 태스크 선택**: 정책에 따라 문맥을 복원하고 실행
3. **우선순위 선점**: 상위 태스크 준비 시 실행 태스크를 교체
4. **대기 전환**: 잠금•입출력 대기 태스크를 차단 상태로 전환
5. **실행 완료**: 종료 상태와 자원 해제를 TCB에 반영

#### 한줄 요약

- **준비•실행•차단 상태 전이**와 우선순위 선점
...
- **태스크 수•데드라인•운영 기능**에 따른 실행 환경 선택
...
- 상위 간섭•차단을 포함한 **WCRT**의 데드라인 충족 검증
...
- 다중 태스크의 **WCRT≤데드라인** 입증 여부가 선택 기준
