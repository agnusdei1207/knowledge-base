...
- 정의/개념: 우선순위 기반 태스크·인터럽트·동기화의 지연 상한을 분석할 수 있게 하는 **결정적 실행 관리 운영체제**이다.
- 배경/필요성: 슈퍼루프는 작업과 공유 자원이 늘수록 실행 간섭과 차단 조건이 결합되어 **다중 태스크 WCRT 입증 복잡도가 증가한다**.
...
- 평균 처리량보다 **최악 지연 상한** 예측 우선이 핵심이다.
- **우선순위 선점**으로 긴급 태스크 응답시간 단축이 핵심이다.
- **ISR•차단 시간** 증가에 따른 최악 응답시간 증가한다.
...
- 평균 처리량보다 **최악 응답시간 상한**을 우선하는 결정성이 핵심이다.
...
2. **실행 태스크 선택**: 스케줄러가 가장 높은 우선순위의 준비 태스크를 선택하고 문맥을 복원한다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 낮은 우선순위 태스크의 잠금으로 **높은 우선순위 태스크 장기 차단** | **우선순위 상속·우선순위 천장 프로토콜** 적용 | 중간 우선순위 간섭을 막아 **차단 시간 상한 제한** |
| ISR이 긴 작업을 수행해 **고우선순위 태스크 시작 지연** | ISR은 원인만 처리하고 나머지는 **지연 처리 태스크로 이관** | 인터럽트 점유 시간 감소로 **태스크 응답 지연 완화** |
| 인터럽트 중첩·재귀가 깊어져 **태스크·ISR 스택 고갈** | 최악 중첩 깊이 분석과 **스택 워터마크·보호 영역 감시** | 오버플로 조기 탐지로 **메모리 손상 방지** |
...
- **WCRT≤데드라인** 입증이 필요한 다중 태스크는 **RTOS 선택**이 핵심이다.
