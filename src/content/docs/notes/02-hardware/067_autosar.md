...
- 배경/필요성: 차량 전장 제어 기능의 폭증 및 벤더별 독자 구조로 인한 **소프트웨어 재사용성 한계, OEM-티어1 간 개발 협업 복잡도 및 칩셋 종속성**을 타파할 개방형 표준 플랫폼 필요
...
- 차량용 분산 실시간 제어(Classic Platform)와 SDV(Software Defined Vehicle) 기반 고성능 자율주행/인포테인먼트(Adaptive Platform)를 아우르는 **전장 소프트웨어의 글로벌 사실상 표준**이며, 향후 **Zonal 아키텍처 및 SOME/IP 서비스 지향(SOA) 통신, 클라우드 네이티브 OTA 배포 체계**와의 결합이 핵심 발전 방향
