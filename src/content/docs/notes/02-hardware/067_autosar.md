...
date: "2026-07-30T21:25:00+09:00"
tags:
  - "notes-hardware"
weight: 67
extra:
...
- **ARXML 스키마·프로파일(ARXML Schema·Profile)**: 스키마는 허용 요소·형식을 정의하고 프로파일은 프로젝트가 사용할 규칙 범위를 고정한다.
- **종단 최악 지연(End-to-End Worst-Case Latency)**: 입력 발생부터 출력 완료까지 태스크·통신을 포함한 최대 지연
- **원자적 갱신·롤백(Atomic Update·Rollback)**: 전체 변경을 한 단위로 반영하고 실패하면 이전 정상 버전으로 복귀하는 방식
- **인터페이스 계약(Interface Contract)**: 서비스의 데이터 형식·호출 조건·오류 응답을 합의한 규칙

## Ⅰ. 개요

- 정의/개념: 차량 소프트웨어의 **구조·인터페이스**를 표준화한 플랫폼
...
- **1. 센서 신호**: 물리 입력을 MCAL에서 수신
- **2. 표준 센서값**: 장치별 레지스터 차이를 표준 형식으로 변환
- **3. 러너블 이벤트 전달**: BSW가 수집한 값을 RTE의 실행 조건으로 변환
- **4. 러너블 호출 정보**: RTE가 SWC를 호출하고 포트 결과 수신
- **5. 표준 제어값**: BSW·MCAL 경로에서 액추에이터 신호로 변환
