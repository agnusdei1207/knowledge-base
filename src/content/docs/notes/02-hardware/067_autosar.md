...
- 차량용 분산 실시간 제어(Classic Platform)와 SDV(Software Defined Vehicle) 기반 고성능 자율주행/인포테인먼트(Adaptive Platform)를 아우르는 전장 소프트웨어의 글로벌 사실상 표준이며, 향후 Zonal 아키텍처 및 SOME/IP 서비스 지향(SOA) 통신, 클라우드 네이티브 OTA 배포 체계와의 결합이 핵심 발전 방향
