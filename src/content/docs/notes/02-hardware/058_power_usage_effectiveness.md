...
- 정의/개념: 같은 기간의 데이터센터 전체 시설 에너지를 IT 장비 에너지로 나누어 비IT 설비 오버헤드를 나타내는 **무차원 효율 지표**이다.
- 배경/필요성: 전체 시설 에너지 값만으로는 실제 IT 작업과 냉각·배전 설비의 **에너지 소비 비중 구분하기 어렵다**.
...
- 이론적 하한 **PUE 1**에 가까울수록 시설 손실 감소한다.
- **경계•기간•IT 부하** 통제 시점 간 추세를 비교한다.
...
1. **계량 자료 검증**: 계량 경계, 결측 구간과 분자·분모의 부하 누락·중복·혼입 여부를 확인한다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 계량점·장비 편입 범위가 바뀌어 **PUE 분자·분모 경계 불일치** | 인입·IT **계량점 위치와 장비 편입 이력**을 변경 시점별 기록 | 같은 경계 구간만 비교해 **기간 추세 일관성 유지** |
| 전체 시설과 IT 계량 시각이 달라 **에너지 비율 왜곡** | **계량기 시계와 집계 시작·종료 주기 동기화** | 같은 부하 기간의 값 사용으로 **PUE 계산 왜곡 방지** |
| IT 부하·외기 변화가 냉각 효율 변화와 섞여 **원인 판별 곤란** | **PUE·IT 부하율과 외기 온·습도**를 함께 기록 | 유사 운전 조건 비교로 **시설 개선 효과 분리** |
| 증발 냉각으로 PUE를 낮추면서 **물 사용·탄소 영향 증가** | **WUE·CUE와 절대 에너지·물 사용량** 공동 평가 | 한 지표 개선이 다른 자원을 악화하는 **상충 효과 통제** |
...
- PUE 상승 시 냉각•배전 손실을 분석해 **비IT 에너지 절감**이 핵심이다.
