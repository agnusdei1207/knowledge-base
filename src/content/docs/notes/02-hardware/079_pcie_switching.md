...
- AI 고성능 서버 및 고밀도 NVMe 스토리지(JBOF)에서 호스트 CPU 우회 P2P 직결 통신 및 96~144레인 이상 팬아웃을 지원하는 핵심 인터커넥트 인프라(PCIe 5.0/6.0 스위치)이며, 향후 CXL 스위칭 패브릭과의 통합을 통한 이종 가속기·메모리 풀링(Disaggregated Composable Infrastructure)으로 진화 가속화
