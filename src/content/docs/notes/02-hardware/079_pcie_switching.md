...
- 배경/필요성: 호스트 CPU의 제한된 물리 PCIe 레인 수 한계를 극복하고, AI 가속기-NVMe SSD 간 호스트 DRAM 경유 지연을 배제하는 **엔드포인트 간 P2P 직접 통신 및 고밀도 I/O 확장** 필요
...
#### 한줄 요약
- 스위치가 TLP를 직접 라우팅해 엔드포인트 간 전송에서 루트 컴플렉스 왕복을 없애지만 그만큼 호스트를 우회하는 경로가 생기므로, ACS 검증이 사라진 호스트 감시를 대신한다.
