...
date: "2026-08-05T18:01:21+09:00"
tags:
  - "notes-hardware"
weight: 79
extra:
...
- 단일 루트 링크를 여러 엔드포인트로 확장하는 **PCIe 패브릭**
...
1. **포트 수신•태그 추적**: 요청 주소와 완료 식별 정보 보관
2. **주소 디코딩•중재**: 루트 또는 P2P 출력 경로 선택
3. **접근 경계 검증**: IOMMU•ACS로 DMA 대상 권한 확인
4. **완료자 요청 처리**: 메모리•장치 접근 후 완료 TLP 생성

#### 한줄 요약

- **주소 라우팅•태그 추적**으로 완료 TLP를 원 요청자에 반환
...
- **P2P 경로•업스트림 용량**을 함께 산정한 가속기 배치
...
- **장치 확장•P2P** 필요 시 업스트림 용량•접근 경계 설계
