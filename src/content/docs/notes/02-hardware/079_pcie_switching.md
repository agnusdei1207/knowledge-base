...
- **Completer ID & Tag**: TLP 헤더 상에서 응답을 수용할 원 요청자(Requester) 및 식별 태그 메타데이터.
...
1. **포트 수신·태그 추적**: 요청 엔드포인트가 **TLP** 패킷 생성 및 스위치 포트로 인가.
2. **주소 디코딩·중재**: 스위치 패브릭 상에서 TLP 주소를 파악하여 루트 컴플렉스 또는 **P2P** 다운스트림 포트로 디스패치.
3. **접근 경계 검증**: **ACS** 및 **IOMMU**를 통한 P2P DMA 보안 범위 검증.
4. **완료자 요청 처리 및 반환**: 대상 장치가 TLP 요청 수행 후 **Completer ID & Tag**를 기반으로 완료 TLP를 원 요청자에게 반환.
...
|:---|:---|:---|
| 적용 기준 | GPU 서버(8~16 GPU), 대용량 NVMe JBOF, P2P 전송 시 | 단일 GPU, 2~4개 NVMe SSD 탑재 워크스테이션 시 |
| 확장성/기능 | 극대화된 팬아웃 확장, 호스트 무관 **P2P 전송** 지원 | CPU Root Complex 물리 레인 수에 따라 직결 한계 발생 |
| 대역폭/지연 | **오버서브스크립션** 발생 가능, 스위치 홉(~100ns) 지연 | 전용(Dedicated) 레인 대역폭 보장, 최단 지연시간 |
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 업스트림 본선 초과로 인한 **오버서브스크립션** 지연 | 스위치 대역폭 비례 설계 ($\rho \le 1.5$) | 패브릭 병목 및 **Tail Latency** 축소 |
| 신호 무결성 손상으로 인한 PCIe **Downtraining** | 리타이머(Retimer) 칩셋 배치 및 SI/PI 시뮬레이션 | 최대 레인 속도(Gen5 32GT/s x16) 보장 |
...
- **PCIe 선택 기준**에 따라 AI 가속기/NVMe 어레이 기반 데이터센터 시스템에는 **PCIe 스위칭** 토폴로지 적용

#### 한줄 요약

- P2P 직접 통신 지원 및 엔드포인트 팬아웃 확장을 위한 PCIe 스위칭 패브릭 구축 체계 적용.
