...
- **1. 읽기 요청 TLP 전달**: 주소·태그 전송
- **2. 업스트림 요청 전달**: 주소 디코딩·중재
- **3. DMA 접근·데이터 반환**: IOMMU 권한 확인 후 호스트 메모리 읽기
- **4. 완료 TLP 전달**: 요청자·태그를 포함한 완료 패킷 생성
- **5. 완료 데이터 전달**: 태그로 원래 엔드포인트에 결과 라우팅
...
- 동시 트래픽·홉·격리 기준으로 PCIe 스위칭을 선택한다.
