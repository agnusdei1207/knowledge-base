...
- 정의/개념: 하나 이상의 업스트림 경로와 여러 다운스트림 포트 사이에서 PCIe TLP를 주소에 따라 전달하는 **패킷 스위칭 패브릭**이다.
- 배경/필요성: 모든 장치를 루트 포트에 직접 연결하면 포트 수와 배선 확장에 한계가 있고 장치 간 P2P 경로 구성도 제약된다.

#### 한줄 요약

- PCIe 스위치는 루트 측 링크 아래에 여러 엔드포인트와 P2P 경로를 확장하는 **패킷 패브릭**이다.
...
> 요약: **M/M/1 부하율**이 1에 가까워질수록 **대기 지연** 급증이 핵심이다.

- **흐름 제어 크레딧**으로 수신 버퍼 범위에서 송신량을 제한한다.
- **트래픽 등급•중재**로 우선순위별 패브릭 경로 할당이 핵심이다.
- **오버서브스크립션** 시 업스트림 대기•꼬리 지연 증가한다.
...
> 요약: **오버서브스크립션 비율**이 1을 넘으면 **중재 대기** 발생이 핵심이다.
...
- **주소 라우팅**으로 요청을 완료자에게 보내고 요청자 ID•태그로 완료 TLP를 원 요청자에게 반환한다.
...
- 가속기를 배치할 때 **P2P 경로•업스트림 용량•스위치 홉**을 함께 산정해야 병목과 꼬리 지연을 예측할 수 있다.
...
- 장치 확장•P2P가 필요하고 **업스트림 용량**이 충분하면 스위칭을 선택한다.

#### 한줄 요약

- **장치 확장•P2P**가 필요하면 공유 업스트림 용량과 ACS•IOMMU 접근 경계를 함께 설계한다.
