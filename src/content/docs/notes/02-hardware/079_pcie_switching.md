...
|:---|:---|:---|:---|
| 루트 컴플렉스 | 호스트 메인보드 | CPU 메모리 공간과 PCIe 주소 공간을 연결하는 **최상위 브리지** | 시스템 초기화 및 열거 |
| 업스트림 포트 (USP) | 스위치 인입단 | 호스트 CPU 방향으로 TLP 패킷을 송수신하는 **스위치 메인 포트** | x16 고속 링크 |
| 스위칭 패브릭 코어 | 스위치 중앙부 | TLP 주소를 디코딩하여 대상 포트로 **비차단(Non-blocking) 라우팅** | 크로스바 매트릭스 |
| ACS 보안 엔진 | 스위치 보안단 | P2P 트랜잭션의 적법성을 검증하여 **가상 머신 간 격리** 강제 | 가상화 보안 통제 |
| 다운스트림 포트 (DSP) | 스위치 출력단 | 개별 엔드포인트와 물리 레인을 직결하고 **핫플러그(Hot-Plug)** 지원 | x4, x8, x16 레인 |
...
- 서버 I/O 확장 요구 분석 기반 **PCIe 스위치 패브릭**과 **ACS 보안 격리** 체계 구축
