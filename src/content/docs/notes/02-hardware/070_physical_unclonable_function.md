...
- 정의/개념: 반도체의 미세한 **제조 편차**를 장치마다 다른 응답으로 변환하여 키 재생이나 장치 인증에 사용하는 하드웨어 보안 함수이다.
- 배경/필요성: 키 자체를 비휘발성 메모리에 고정 저장하면 물리 판독으로 키가 복제될 수 있으므로 필요할 때만 비밀을 재생하는 수단이 필요하다.
...
- 환경 변화에 따른 **응답 오류율**로 키 재생 신뢰성 판단이 핵심이다.
- **퍼지 추출기•보조 데이터**로 안정된 키 재생이 핵심이다.
...
- **PUF 응답•보조 데이터**를 결합한 장치 키 재생이 핵심이다.
...
> 요약: **RER**이 낮을수록 동일 장치의 **응답 신뢰성** 향상이 핵심이다.
...
- **키 삭제(Key Zeroization)**: 암호 연산이 끝난 직후 복원 키가 있던 레지스터와 메모리를 지워 노출 시간을 줄이는 처리한다.
...
- 내부 **키 재생**에는 약한 PUF를 사용하며, 충분한 공격 내성을 검증한 경우에만 강한 PUF를 장치 인증에 사용한다.

#### 한줄 요약

- **응답 오류율•예측 가능성**을 검증한 뒤 키 재생 또는 장치 인증 용도에 맞는 PUF를 선택한다.
