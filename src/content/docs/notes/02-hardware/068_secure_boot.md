...
- **2. 신뢰 기준 조회**: 변경 불가능한 루트 키와 허용 최소 버전 확보
- **3. 부팅 이미지 로드**: 활성 슬롯의 코드·서명·버전 메타데이터 읽기
- **4. 서명·롤백 검증**: 출처·무결성과 버전 단조 증가 조건을 함께 판정
- **5. 승인 단계 실행**: 검증된 다음 코드에만 제어권을 넘겨 신뢰 사슬 연장
...
- **서명·보안 버전·키 폐기** 검증 기반 승인 펌웨어만 실행
