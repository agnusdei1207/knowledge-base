...
- 정의/개념: 변경하기 어려운 **신뢰 루트**에서 시작하여 각 부팅 단계의 서명과 보안 버전을 검증한 뒤 실행권을 넘기는 체계이다.
- 배경/필요성: 위치와 형식만 확인하는 일반 부팅으로는 초기 코드의 변조와 비승인 출처 및 서명된 취약 구버전을 차단할 수 없다.

#### 한줄 요약

- 보안 부팅은 하드웨어 신뢰 루트에서 시작하여 **단계별 신뢰 사슬**을 형성한다.
...
- **서명 연쇄 검증**으로 부팅 단계의 무결성•출처를 확인한다.
- 서명은 **무결성•출처**만 검증하고 기밀성은 별도 암호화가 핵심이다.
- **단조 증가 보안 버전**으로 취약 구버전 롤백 차단이 핵심이다.

#### 한줄 요약

- **서명•키 폐기•보안 버전**을 모두 확인하는 부팅을 검증한다.
...
- **부트 ROM•신뢰 저장소** 기반 다음 부팅 단계를 검증한다.
...
3. **서명•키 폐기 검증**: 이미지 출처•무결성과 키 상태를 판정한다.
4. **보안 버전 검증**: 최소 허용 버전보다 낮은 이미지를 차단한다.
5. **다음 단계 실행**: 승인 이미지에 실행권을 넘겨 신뢰를 연장한다.

#### 한줄 요약

- 활성 슬롯 실패 시 **동일 검증을 복구 슬롯에 반복**이 핵심이다.
...
- **보안 버전 확정(Security-version Commit)**: 새 이미지가 정상 부팅된 뒤 최소 허용 버전 카운터를 올리는 처리한다.
...
- 새 슬롯의 정상 부팅을 확인한 뒤 **보안 버전을 확정**하고, 실패하면 기존 검증 슬롯으로 복구한다.
...
- **서명•보안 버전**과 키 폐기 상태를 모두 검증하여 승인된 펌웨어에만 실행권을 부여한다.

#### 한줄 요약

- **루트 키•보안 버전•키 폐기** 검증을 모두 통과한 이미지에만 실행권을 부여한다.
