...
date: "2026-07-25T10:40:00+09:00"
tags:
  - "notes-hardware"
weight: 68
extra:
...
- **정의/개념**: 신뢰 루트가 승인 단계만 실행하는 체계다
- **배경/필요성**: 초기 코드 변조와 악성 실행을 차단한다
...
- 서명은 기밀성과 코드 취약점 부재를 보장하지 않는다.
- 단조 증가 보안 버전이 허용값보다 낮은 이미지 실행을 차단한다.
- 키 교체·폐기 정책이 노출된 서명 키의 신뢰를 회수한다.
- 복구 이미지를 쓸 때도 같은 서명·버전 기준을 적용한다.
...
| 판단 기준 | 보안 부팅 | 보안 부팅·롤백 방지 | 일반 부팅 |
|:---|:---|:---|:---|
| 핵심 특징 | 서명된 이미지의 신뢰 사슬 | 서명·보안 버전의 연쇄 검증 | 위치·형식만 확인해 실행 |
| 적용 기준 | 비승인·변조 이미지 차단 | 취약 구버전까지 실행 차단 | 신뢰 검증이 불필요한 폐쇄 장치 |
| 주요 위험 | 서명된 취약 코드·키 노출 | 카운터·키 복구 설계 오류 | 초기 코드 변조·악성 실행 |
...
1. 무선 업데이트의 A/B 서명·버전·복구를 시험한다
2. 서버 펌웨어의 키 폐기 목록 갱신을 시험한다

### 쉽게 이해하기 (학습용)

- 원격 갱신은 새 슬롯 성공 뒤 버전을 확정하고 실패하면 검증된 예비 슬롯으로 복구하는지 확인한다.
- 서버는 키를 바꾼 뒤 새 이미지는 받고 폐기 키 이미지는 거부하는지 확인한다.

## Ⅶ. 결론

- 서명·버전·키 상태를 검증한 부팅·복구만 허용한다
