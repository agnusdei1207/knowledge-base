...
- 정의/개념: 변경하기 어려운 **신뢰 루트**에서 시작하여 **신뢰 사슬**의 각 단계 서명과 보안 버전을 검증한 뒤 실행권을 넘기는 **보안 부팅** 체계이다.
- 배경/필요성: 위치와 형식만 확인하는 일반 부팅으로는 초기 코드의 변조와 비승인 출처 및 서명된 취약 구버전을 차단할 수 없다.
...
- **디지털 서명**으로 부팅 단계의 **무결성**과 출처를 확인한다.
- 서명은 무결성과 출처를 검증하며 **기밀성**은 별도 암호화로 제공한다.
- **롤백 방지**로 서명된 취약 구버전의 실행을 차단한다.
...
- **신뢰 기준 적재**: 루트 키와 폐기 상태 및 최소 보안 버전을 읽는 단계이다.
- **부팅 슬롯 선택**: A/B 슬롯에서 먼저 검증할 이미지를 고르는 단계이다.
- **서명·키 폐기 검증**: 검증 메타데이터로 이미지 출처와 무결성 및 키 상태를 확인하는 단계이다.
- **보안 버전 검증**: 이미지 버전이 최소 허용 보안 버전 이상인지 확인하는 단계이다.
- **다음 단계 실행**: 승인된 이미지에 실행권을 넘겨 신뢰 사슬을 연장하는 단계이다.
...
1. **신뢰 기준 적재**: 루트 키•폐기•최소 보안 버전을 조회한다.
2. **부팅 슬롯 선택**: 활성 슬롯부터 검증 대상을 결정한다.
3. **서명•키 폐기 검증**: **검증 메타데이터**의 서명과 알고리즘 정보로 이미지 출처•무결성과 키 상태를 판정한다.
4. **보안 버전 검증**: 최소 허용 버전보다 낮은 이미지를 차단한다.
5. **다음 단계 실행**: 승인 이미지에 실행권을 넘겨 신뢰를 연장한다.
...
| 핵심 특징 | **보안 부팅**의 서명 신뢰 사슬 | **단조 증가 카운터** 기반 롤백 방지 | **일반 부팅**의 위치•형식 확인 |
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 노출 키가 폐기 정보에 반영되지 않음 | **키 폐기**와 교체 훈련 | 노출 키의 검증 권한 제거 |
| 이미지 검증 전에 보안 버전 카운터 확정 | 부팅 성공 뒤 **보안 버전 확정** | 검증된 슬롯 복구 보장 |
| A/B 슬롯 모두 실패해 부팅 반복 | 재시도 횟수 제한과 **복구 모드** | 부팅 루프 방지 |
| 서명됐지만 취약한 이미지 배포 | 취약점 관리와 **원격 증명** | 실행 중 보안 상태 검증 |
...
- **부팅 승인 기준**: 키 수명주기와 보안 버전 정책을 통과한 승인 펌웨어만 실행해 부팅 신뢰성을 유지하는 기준이다.

</details>

- **부팅 승인 기준**에 따라 **키 수명주기**의 서명·폐기 상태와 보안 버전을 모두 검증해 **승인 펌웨어**에만 실행권을 부여하고 **부팅 신뢰성**을 유지한다.
