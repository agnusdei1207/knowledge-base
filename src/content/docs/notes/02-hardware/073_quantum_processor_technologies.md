...
- 정의/개념: **조지프슨 초전도 회로** 또는 전기장에 포획한 이온의 양자 상태로 큐비트를 구현하고 제어•측정하는 프로세서 기술이다.
- 배경/필요성: 큐비트 플랫폼마다 게이트 속도와 결맞음 시간, 연결성 및 제어 인프라의 장단점이 달라 회로 요구에 맞는 선택이 필요하다.

#### 한줄 요약

- 회로가 요구하는 **게이트 속도•결맞음•연결성**에 따라 큐비트 플랫폼을 선택한다.
...
- 초전도 방식은 **빠른 게이트•반도체 공정 기반 집적**에 유리하지만 극저온 배선과 교차 간섭을 관리해야 한다.
- 이온 트랩 방식은 **긴 결맞음•높은 연결성**에 유리하지만 게이트 속도와 이온 수 증가에 따른 모드 혼잡을 고려해야 한다.
- 두 방식 모두 큐비트 수가 늘수록 **제어선•보정 인프라**와 냉각 또는 진공 장치가 커져 시스템 확장이 어려워진다.
...
- **마이크로파•공진기**와 레이저•형광의 제어•판독 구조이다.
...
1. **마이크로파 게이트**: 제어기가 마이크로파 펄스로 초전도 회로의 상태와 큐비트 결합을 제어한다.
2. **공진기 판독**: 큐비트 상태에 따라 달라지는 공진기의 주파수•위상을 측정하여 샷 결과를 판별한다.
3. **레이저 게이트**: 제어기가 레이저 펄스로 이온의 내부 상태와 집단 진동 모드를 조작한다.
...
- 오류 지도에서 **연결성•게이트 충실도**가 높은 경로를 우선 사용하도록 물리 큐비트를 배치한다.
...
- 빠른 반복과 짧은 **게이트 시간**이 중요하면 초전도, 긴 결맞음과 높은 연결성이 중요하면 이온 트랩을 우선 검토한다.

#### 한줄 요약

- **짧은 회로의 빠른 반복**에는 초전도, 긴 결맞음과 직접 연결이 중요한 회로에는 이온 트랩이 유리하다.
