...
- **정의/개념**: 초전도 회로·포획 이온 기반 큐비트 구현 방식
- **배경/필요성**: 게이트 시간·결맞음·연결성이 달라 플랫폼 선택이 필요함
...
- 초전도 공명 주파수 편차가 보정 비용을 높인다.
- 동일 동위원소 이온은 준위 편차가 작아 보정이 쉽다.
...
|:---|:---|:---|
| 핵심 특징 | 빠른 게이트·칩 집적 | 긴 결맞음·전역 연결 |
| 적용 기준 | 고속 게이트·칩 집적 우선 회로 | 긴 결맞음·전역 연결 우선 회로 |
| 주요 위험 | 극저온 배선·교차 간섭·수율 | 레이저 채널·모드 혼잡·이동 지연 |
...
1. 초전도 컴파일러는 가상 큐비트를 물리 결합 그래프에 배치
2. 다중 구역 이온 트랩은 연산 구역 간 이온 이동

### 쉽게 이해하기 (학습용)

- 설계도 속 큐비트를 실제 칩에서 서로 연결된 자리로 옮겨 놓는다.
- 여러 계산 구역 사이로 필요한 이온을 옮긴다.

## Ⅶ. 결론

- 게이트 시간·결맞음·연결성으로 큐비트 플랫폼 선택
