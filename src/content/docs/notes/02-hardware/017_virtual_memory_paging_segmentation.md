...
- **메모리 관리 장치(Memory Management Unit, MMU)**: 가상 주소를 물리 주소로 바꾸고 그 접근이 권한에 맞는지까지 검사하는 하드웨어
- **변환 색인 버퍼(Translation Lookaside Buffer, TLB)**: 최근 변환 결과를 담아 두어 변환 표를 매번 뒤지지 않게 하는 MMU 내부 캐시
...
- **가상 페이지 번호(Virtual Page Number, VPN)**: 가상 주소의 상위 비트로 어느 페이지인지 나타내며 페이지 테이블을 찾는 색인이 된다
- **물리 페이지 번호(Physical Page Number, PPN)**: 변환 결과로 나오는 물리 프레임 번호
...
- **페이지 테이블 항목(Page Table Entry, PTE)**: 페이지 하나에 대응하는 PPN과 상주 여부, 접근 권한을 담은 항목
...
- **기존 한계**: 물리 주소를 직접 쓰면 프로세스 간 **메모리 격리 불가**
