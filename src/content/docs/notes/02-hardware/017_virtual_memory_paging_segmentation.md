...
1. **① 가상 주소·접근 유형 전달**: CPU는 가상 주소와 읽기·쓰기·실행 접근 유형을 MMU·TLB에 전달한다.
2. **② 변환된 물리 주소로 접근**: TLB에 유효한 변환이 있고 권한이 허용되면 PPN과 오프셋을 결합해 물리 메모리에 접근한다.
3. **③ 요청 데이터 반환**: 물리 메모리는 선택된 프레임의 데이터를 CPU에 반환해 적중 경로를 끝낸다.
4. **④ VPN 대응 PTE 조회**: TLB 미스이면 MMU는 현재 프로세스의 페이지 테이블에서 VPN에 대응하는 PTE를 조회한다.
5. **⑤ PPN·권한 반환**: PTE가 상주 상태이고 접근이 허용되면 페이지 테이블은 물리 프레임 번호와 권한을 MMU에 제공한다.
6. **⑥ 비상주 상태 반환**: PTE가 페이지가 저장장치에 있음을 나타내면 MMU는 하드웨어 변환만으로 접근을 완료할 수 없다.
7. **⑦ 페이지 폴트 예외 전달**: MMU는 실행 중인 명령을 중단하고 운영체제의 페이지 폴트 처리기로 제어를 넘긴다.
8. **⑧ 저장장치 페이지 읽기 요청**: 운영체제는 빈 프레임을 확보하고 비상주 페이지를 저장장치에서 읽도록 요청한다.
9. **⑨ 페이지 데이터 반환**: 저장장치는 요청 페이지의 내용을 운영체제에 반환한다.
10. **⑩ 확보한 프레임에 데이터 적재**: 운영체제는 반환된 페이지를 확보한 물리 프레임에 배치한다.
11. **⑪ PPN·상주·권한 갱신**: 운영체제는 PTE에 새 PPN과 상주 상태·접근 권한을 기록한다.
12. **⑫ 중단 명령 재시작**: 폴트 처리가 끝나면 운영체제는 MMU 변환이 다시 수행되도록 원래 명령을 재시작한다.
13. **⑬ 확정된 물리 주소로 접근**: 상주 PTE에서 얻거나 폴트 처리로 만든 변환을 이용해 물리 메모리에 접근한다.
14. **⑭ 요청 데이터 반환**: 물리 메모리는 최종 데이터를 CPU에 반환해 미스 경로를 완료한다.
