...
date: "2026-08-05T11:37:07+09:00"
tags:
  - "notes-hardware"
weight: 17
extra:
...
- 프로세스별 **주소 공간•접근 권한** 으로 물리 메모리 격리
...
- **독립 주소 공간•요구 페이징** 으로 격리와 선택 적재

## Ⅲ. 구조 및 구성요소

<details><summary>핵심 용어</summary>

- **MMU**: Memory Management Unit, 주소를 변환하고 접근 권한을 검사하는 하드웨어
- **TLB**: Translation Lookaside Buffer, 최근 주소 변환을 저장하는 캐시
- **PTE**: Page Table Entry, PPN•상주 여부•접근 권한을 저장하는 항목
- **세그먼트 기술자**: 논리 영역의 기준 주소•한계•접근 권한을 저장하는 항목
- **PPN**: Physical Page Number, 매핑된 물리 페이지 프레임의 번호
...
- **VPN**: Virtual Page Number, 가상 주소의 페이지 번호
...
```text
                   ...
                   [페이징]        [세그멘테이션]
                      |                  |
             1. TLB•PTE 변환 조회   4. 기술자•권한•한계 검사
                      |                  |
             2. 상주•권한 판정       [검사 통과?]
                 /          \          /         \
              [통과]      [비상주]    [예]        [아니요]
                |            |         |            |
         [PPN•오프셋 결합] 3. 페이지 폴트•재시작 |        [접근 예외]
                |            |         |
                +------<-----+  5. 기준 주소•오프셋 결합
                |                      |
                +----------+-----------+
...
- **1. TLB•PTE 변환 조회**: VPN에 대응하는 PPN 탐색
- **2. 상주•권한 판정**: 접근 가능 물리 프레임 확인
- **3. 페이지 폴트•재시작**: 페이지 적재와 PTE 갱신
- **4. 기술자•권한•한계 검사**: 세그먼트 범위와 접근 검증
- **5. 기준 주소•오프셋 결합**: 세그먼트 물리 주소 산출

#### 한줄 요약

- 페이징은 PTE, 세그멘테이션은 **기준•한계** 로 변환
