...
- 정의/개념: 캐시 라인별 읽기·쓰기 권한과 최신값 제공자를 정하는 **소유권 상태 규약**
- 배경/필요성: 상태 규약이 없으면 코어별 사본에서 **오래된 값 관측·동시 쓰기 충돌**
...
- **단일 작성자·다중 독자** 불변식으로 쓰기 가능 사본 단일화
...
- **수정(Modified, M) 상태**: 메모리보다 최신이며 단독 쓰기가 가능한 상태
- **독점(Exclusive, E) 상태**: 메모리와 같고 단독 읽기•쓰기가 가능한 상태
- **공유(Shared, S) 상태**: 여러 캐시가 읽기 사본을 보유한 상태
- **무효(Invalid, I) 상태**: 캐시 라인을 사용할 수 없는 상태
...
3. **원격 사본 무효화·최신값 전달**: 다른 읽기 권한을 회수하고 필요하면 최신 데이터 제공
...
- **수정•독점•공유•무효(Modified, Exclusive, Shared, Invalid, MESI)**: 네 상태로 라인을 관리하는 프로토콜
- **수정•소유•독점•공유•무효(Modified, Owned, Exclusive, Shared, Invalid, MOESI)**: 소유 상태를 더한 프로토콜
