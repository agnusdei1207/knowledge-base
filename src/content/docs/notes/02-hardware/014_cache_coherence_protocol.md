...
- **M•E•S•I 상태**: 각각 수정•독점•공유•무효 상태로 캐시 라인의 최신성 및 읽기•쓰기 권한을 나타낸다.
- **일관성 트랜잭션**: 소유권 요청•무효화•데이터 전달을 연결망에서 하나의 권한 전환 절차로 수행하는 동작
- **로컬•원격 일관성 제어기**: 요청 코어와 다른 코어의 캐시 상태를 각각 판정하고 상태 전이를 실행하는 회로
...
- **데이터 분할•코어별 배치**: 공유 데이터를 나누고 주로 접근하는 코어 가까이에 두어 소유권 이동을 줄이는 방법
...
- **MESI•MOESI**: 각각 네 가지 기본 상태를 쓰는 프로토콜과 소유 상태를 더해 더티 캐시 간 전달을 허용하는 프로토콜
