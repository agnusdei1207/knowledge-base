...
- **① 캐시 라인 쓰기 요청**: 수정할 주소를 로컬 제어기에 전달
- **② 로컬 권한 확인·쓰기 허용**: M은 즉시 쓰고 E는 무통신으로 M 전이
- **③ 쓰기 소유권 요청**: S·I이면 사본 회수·데이터 요청
- **④ 원격 사본 무효화**: 같은 라인의 원격 사본에 무효화 전달
- **⑤ 무효화 완료·최신 데이터 응답**: 원격 사본을 I로 바꾸고 최신값 응답
- **⑥ 단일 쓰기 소유권 승인**: 모든 무효화 완료 후 독점 권한 부여
- **⑦ 캐시 라인 쓰기 허용**: 로컬 라인을 M으로 확정하고 갱신 허용
