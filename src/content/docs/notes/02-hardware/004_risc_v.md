...
- **명령어 집합 구조(Instruction Set Architecture, ISA)**: ‘아이에스에이’로 읽고 영문 머리글자를 딴 약어이며, 소프트웨어가 프로세서에 요구할 명령과 레지스터 규약을 정한 계약이다
...
- **응용 이진 인터페이스(Application Binary Interface, ABI)**: ‘에이비아이’로 읽고 영문 머리글자를 딴 약어이며, 함수 호출 시 인자·결과에 쓸 레지스터를 정한 규약이다
...
- **반도체 설계자산(Intellectual Property, IP)**: ‘아이피’로 읽고 영문 머리글자를 딴 약어이며, 다른 회사가 검증해 재사용할 수 있게 제공하는 프로세서 코어 설계 블록
