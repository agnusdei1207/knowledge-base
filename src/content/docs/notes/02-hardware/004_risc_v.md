...
- **RISC-V(Reduced Instruction Set Computer Five)**: ‘리스크 파이브’로 읽으며 버클리에서 이어진 다섯 번째 축소 명령어 설계라는 뜻의 로마 숫자 V가 붙은 이름이고, 누구나 가져다 쓰는 공개 기본 명령 목록에 필요한 옵션 묶음만 골라 붙여 자기 프로세서를 만드는 개방형 명령 규격이다
- **명령어 집합 구조(Instruction Set Architecture, ISA)**: 소프트웨어가 프로세서에게 시킬 수 있는 명령의 종류와 레지스터 규약을 정한 계약이며, 같은 ISA면 같은 프로그램이 돈다
...
- **응용 이진 인터페이스(Application Binary Interface, ABI)**: 함수 호출 시 어느 레지스터에 인자를 싣고 결과를 받을지 정한 규약이며, ISA가 같아도 이것이 다르면 바이너리가 서로 호환되지 않는다
...
- **반도체 설계자산(Intellectual Property, IP)**: 다른 회사가 검증까지 끝내 놓아 그대로 가져다 칩에 넣는 프로세서 코어 설계 블록
...
- **정의/개념**: 개방형 **기본 ISA**에 확장을 조합해 구현하는 명령 규격
- **기존 한계**: 폐쇄형 ISA에서는 **전용 명령** 추가·검증 자체가 불가
...
|:---|:---|:---|
| 적용 기준 | **전용 명령**과 자체 검증 역량이 있을 때 | 검증된 **코어 IP**로 일정을 줄여야 할 때 |
...
1. 맞춤형 가속 코어는 **적합성 시험**으로 확장 동작 검증
2. 운영체제 배포판은 공통 **ISA·ABI** 기준 바이너리 빌드

### 쉽게 이해하기 (학습용)

- 우리 회사 전용 명령을 하나 끼워 넣은 코어라도 나머지 표준 명령이 규격대로 돌지 않으면 기존 프로그램이 죄다 깨지므로, 새 명령보다 표준 명령 쪽을 공통 시험으로 먼저 확인한다
- 여러 회사 칩에서 같은 설치 파일이 돌아가려면 배포판이 가장 낮은 공통 명령 집합과 같은 호출 규약을 기준으로 미리 빌드해 두어야 한다

## Ⅶ. 결론

- 전용 명령·자체 검증은 **RISC-V**, 상용 IP·일정은 **Arm**
