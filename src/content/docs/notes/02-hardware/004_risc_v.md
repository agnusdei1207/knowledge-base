...
date: "2026-07-28T21:51:06+09:00"
tags:
  - "notes-hardware"
weight: 4
extra:
...
- **① 목표 프로파일 선택**: 제품 용도와 운영체제 요구에 맞는 XLEN·권한 수준·프로파일 결정
- **② ISA·확장 조합 확정**: 기본 ISA와 필수·선택 표준 확장, 사용자 정의 확장의 인코딩 계약 고정
- **③ 마이크로아키텍처 구현**: 확정한 ISA 동작을 만족하도록 파이프라인·캐시·실행 유닛의 RTL 설계
- **④ ISA 적합성 검증**: 표준 시험과 기준 구현 차등 테스트로 명령·예외·권한 동작 확인
- **⑤ ABI·툴체인 대상 고정**: 통과한 프로파일과 확장 조합에 맞춰 호출 규약·컴파일 옵션·라이브러리 구성
- **⑥ 바이너리 호환성 확인**: 동일 ISA·ABI 대상으로 생성한 바이너리가 시험 코어와 운영체제에서 실행되는지 검증
