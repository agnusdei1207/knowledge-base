...
```mermaid
sequenceDiagram
    participant F as 명령어 인출부
    participant D as RISC-V 해독부
    participant E as 실행부
    participant P as 권한·예외 제어
    F->>D: 명령어 전달
    D->>D: 기본 ISA·확장 해독
    alt 일반 명령
        D->>E: 실행 연산 발행
...
| 절차 | 설명 |
|:---|:---|
| 명령어 전달 | 메모리에서 가져온 명령 비트를 해독부에 공급 |
| 기본 ISA·확장 해독 | 연산 부호를 기본 명령 또는 구현 확장으로 판별 |
| 실행 연산 발행 | 정수·부동소수점·벡터 실행부에 연산 전달 |
| 결과 기록·다음 PC | 연산 결과와 다음 명령 주소를 아키텍처 상태에 반영 |
| 권한·예외 판정 | 현재 권한에서 허용되는 명령·접근인지 검사 |
| 트랩 벡터 전환 | 예외 원인과 복귀 주소를 기록하고 처리기로 분기 |

> 요약: 기본 ISA·확장을 해독하고 특권 사건은 트랩으로 분리

### 쉽게 이해하기 (학습용)

- 명령이 일반 연산이면 해당 실행부로 보내고 권한이 필요한 사건이면 현재 수준을 검사해 정해진 예외 처리 주소로 넘긴다
