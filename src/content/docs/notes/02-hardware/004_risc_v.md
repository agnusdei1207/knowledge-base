...
date: "2026-08-08T11:43:00+09:00"
tags:
  - "notes-hardware"
weight: 4
extra:
...
- **RISC-V**: 로열티 없는 무료 오픈소스로 공개되어, 기본 정수(Base Integer) 명령어 집합과 공식 표준 및 사용자 정의 모듈 확장을 자율적으로 조합 가능한 개방형 ISA 규격.
...
- 배경/필요성: 기존 x86, ARM 등 독점/폐쇄형 ISA의 비싼 라이선스 비용 및 커스텀 명령어 추가 불가 제약을 극복하고, 도메인 특화 아키텍처(DSA)와 반도체 자급자족 생태계 구축을 위해 로열티가 없는 오픈 아키텍처 필요성 증대.
...
|:---|:---|:---|
| **기본 ISA (Base ISA)** | **XLEN** 비트 폭 규정 및 최소 정수 연산 **공통 실행 계약** 보장 | 40여 개의 단순한 명령어 집합으로 경선제어 디코더 구현 극소화 |
| **확장 집합 (Extensions)** | M(Multiplication), F(Float), V(Vector), Custom 확장을 추가 연동 | 칩 목적에 특화된 AI 연산기 및 DSP 전용 명령을 선택 탑재 |
| **권한 아키텍처** | M/S/U-Mode 특권 격리 및 CSR(Control & Status Register) 제어 | Linux OS 구동 및 하드웨어 가상화, 보안 실행 환경(TEE) 제공 |
| **ISA 프로파일** | 필수 표준 확장을 표준화 묶음으로 고정 사양 정의 | 바이너리 호환성 파편화를 방지하고 소프트웨어 생태계 단일화 |
...
3. **시험 프로그램 생성**: 표준 **인코딩 계약**에 따른 기계어 스트림 및 **예외 시험**, **권한 시험** 용 검증 골든 모델 코드를 생성함.
...
- **구현 방식 선택 기준(Implementation Decision Criteria)**을 수립하여 AI·보안 특화 가속 연산기 탑재 및 로열티 절감이 필수적인 도메인 특화 반도체(DSA)에는 **RISC-V** 기반의 자체 RTL 설계 및 적합성 검증 체계를 적용하고, 검증된 모바일 및 차량용 제어 시스템에는 **Arm IP**를 활용하는 이원화 설계 기법 구현 필수.

#### 한줄 요약
- 도메인 특화 아키텍처 구현을 위한 RISC-V 커스텀 확장 RTL 설계 및 표준 Profile 기반 Architectural Compliance 검증 체계 적용.
