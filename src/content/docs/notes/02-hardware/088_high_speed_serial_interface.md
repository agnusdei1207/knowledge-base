...
- 정의/개념: 적은 수의 차동 레인으로 데이터를 직렬 전송하고 양단과 케이블의 협상으로 데이터•전력•영상 기능을 정하는 연결 규격이다.
- 배경/필요성: USB-C는 커넥터 형상만 규정하므로 같은 모양의 포트와 케이블이라도 지원 속도, 전력, 영상 및 터널링 기능이 다를 수 있다.

#### 한줄 요약

- **호스트•장치•케이블 공통 기능**만 활성화하는 연결 규격이 핵심이다.
...
- **기능 탐색•USB PD**로 공통 속도•전력•대체 모드를 결정한다.
- **차동 신호•레인** 기반 고속 직렬 전송이 핵심이다.
- Thunderbolt **PCIe 터널링**으로 고대역 장치 연결•외부 DMA 공격면을 확대한다.

#### 한줄 요약

- **기능 교집합 협상**과 Thunderbolt 외부 DMA 격리가 핵심이다.
...
- **호스트•포트•케이블•장치** 경로와 외부 DMA 격리가 핵심이다.
...
- **케이블과 양단의 기능 교집합**을 확정하고 PCIe 터널링에는 DMA 경계를 적용한 뒤 링크를 활성화한다.
...
> 요약: 범용 연결은 **USB**, PCIe 터널링은 **Thunderbolt**을 선택한다.

#### 한줄 요약

- **범용 장치•충전**은 USB, 외장 PCIe•다중 화면은 Thunderbolt가 핵심이다.
...
- 업무용 도크는 **호스트•도크•케이블이 모두** 필요한 화면 수와 해상도 및 충전 전력을 지원하는지 확인한다.
...
- 범용•충전은 **USB**, 외장 PCIe•다중 화면은 **Thunderbolt**가 핵심이다.

#### 한줄 요약

- **공통 기능•외부 DMA 요구**로 USB 또는 Thunderbolt을 선택한다.
