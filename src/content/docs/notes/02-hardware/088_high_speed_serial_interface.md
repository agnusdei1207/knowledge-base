...
- 배경/필요성: 용도마다 포트를 따로 두면 커넥터 수만큼 기구 공간과 배선 비용을 치러 단말 소형화가 막히므로, 하나의 Type-C 물리 계층에 데이터·영상·PCIe 터널링과 전력 공급을 다중화해 포트를 합치는 **단일 포트 통합**의 필요
...
#### 한줄 요약
- 호스트 라우터가 프로토콜마다 따로 두던 포트를 하나의 비트스트림으로 대신 받아 내고, 그 한 커넥터가 감당할 전류와 대역폭의 상한은 규격으로 고정되는 대신 CC 핀과 케이블 속 e-Marker가 연결할 때마다 협상해 정한다.
...
분기 결과: IOMMU 보안 검증 승인 장치는 **PCIe 터널링**을 즉시 가동하며 비인가 장치는 **DMA 접근**이 차단됨
