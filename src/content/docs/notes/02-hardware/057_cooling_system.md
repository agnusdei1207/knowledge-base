...
- 정의/개념: 서버 열을 흡수•이송•방출하는 **데이터센터 냉각 시스템**
- 배경/필요성: 고밀도 장비의 국부 발열은 **열 스로틀링•서비스 중단** 유발

#### 한줄 요약

- 열원에서 외부 방열까지 **냉각 매체 순환** 으로 열 제거
...
- **열 수집•전달•방출 경로** 의 열 저항이 낮을수록 장비 온도 감소
...
- **1. 냉각 상태 판정**: 온도•유량•압력으로 열 제거 상태 판정
- **2. 순환 운전값 결정**: 열 부하에 맞춰 펌프•팬 출력 결정
- **3. 열 흡수•이송•방출**: 냉각 매체로 열을 외부 계통에 방출
- **4. 예비 순환 경로 가동**: 고장 구간 격리 후 예비 장치 가동

#### 한줄 요약

- **상태 측정** 기반 순환 제어와 이상 시 예비 경로 전환
