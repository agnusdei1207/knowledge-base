...
- 정의/개념: 공급망과 부팅•갱신•관리 경로의 결함으로 고권한 코드의 변조나 오용을 허용하는 **펌웨어 보안 취약점**이다.
- 배경/필요성: 펌웨어는 OS보다 먼저 높은 권한으로 실행되므로 침해된 펌웨어는 OS를 재설치해도 남을 수 있다.

#### 한줄 요약

- 펌웨어 보안은 OS 이전 고권한 코드의 **침해 잔존성•복구 한계**까지 다뤄야 한다.
...
- **OS 이전 고권한 실행**으로 침해 지속성 증가한다.
- **긴 수명•공급망 분산**으로 패치 지연을 확대한다.
- 서명 검증만으로 **취약 코드•오설정** 차단 불가가 핵심이다.

#### 한줄 요약

- **OS 이전 실행•공급망 분산**에 따른 침해•패치 지연이 핵심이다.
...
- **업데이트 검증•A/B 슬롯** 기반 후보 활성화와 롤백이 핵심이다.
...
1. **자산•의존성 검증**: 대상 장치와 SBOM 및 구성요소 호환 관계를 확인한다.
2. **서명•보안 버전 검증**: 이미지 출처와 무결성 및 롤백 허용 여부를 판정한다.
3. **비활성 슬롯 기록**: 기존 정상 슬롯을 보존한 채 검증된 후보 이미지를 비활성 슬롯에 기록한다.
4. **후보 부팅•건강 검증**: 후보로 부팅하여 핵심 서비스와 장치가 정상일 때만 활성 상태와 보안 버전을 확정한다.

#### 한줄 요약

- **비활성 슬롯에 기록한 후보**는 부팅 건강 검증을 통과한 뒤에만 활성 이미지로 확정한다.
...
- **서명•보안 버전**과 A/B 복구 가능성을 검증한 뒤에만 펌웨어 갱신을 승인한다.

#### 한줄 요약

- **서명•의존성•복구 가능성** 검증을 모두 통과한 후보만 활성화한다.
