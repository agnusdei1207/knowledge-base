...
- 정의/개념: 공급망•부팅•갱신에서 변조를 허용하는 **펌웨어 보안 취약점**
- 배경/필요성: OS 재설치만으로는 **펌웨어 침해 잔존 제거 불가**

#### 한줄 요약

- OS 이전 고권한 코드의 **침해 잔존성•복구 한계**
...
1. **자산•의존성 검증**: 대상 장치•SBOM•호환 관계 확인
2. **서명•보안 버전 검증**: 출처•무결성•롤백 조건 판정
3. **비활성 슬롯 기록**: 정상 슬롯을 보존하고 후보 이미지 설치
4. **후보 부팅•건강 검증**: 핵심 서비스와 장치 상태 확인

#### 한줄 요약

- **비활성 슬롯 검증** 후 정상 후보만 활성 확정
