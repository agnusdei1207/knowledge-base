...
- 정의/개념: 전력·냉각 설비의 중복 용량, 분배 경로, 계획 정비와 단일 장애 중 핵심 부하 유지 능력을 구분하는 **데이터센터 복원력 등급 체계**이다.
- 배경/필요성: 시설마다 중복 구성과 시험 범위가 다르면 같은 “고가용성” 표현만으로 **실제 복원력·투자비 비교하기 어렵다**.
...
- **Uptime Tier•TIA-942 Rated** 독립 기준으로 단순 환산 금지가 핵심이다.
- **최약 필수 경로**가 전체 시설의 복원력을 결정한다.
- **정비 가능성•장애 허용성**을 구분해 목표 등급 선정이 핵심이다.
...
| 정비 순서·권한 오류로 활성·예비 경로가 함께 **중단** | **작업 권한·격리·복구 기준** 문서화와 모의 훈련 | 절차 오류 감소로 **동시 유지보수 신뢰성 향상** |
| 업무 중단 비용보다 높은 등급을 선택해 **과잉 투자** | **서비스 영향·허용 중단·복구 목표와 수명주기 비용** 비교 | 요구 수준에 맞춘 **비용·가용성 균형** |

> 요약: 경로 계획 정비 중 **핵심 부하 연속성** 시험•증적이 핵심이다.
...
- 정비 중 연속성 요구에는 **Tier III**, 단일 장애 유지 요구에는 **Tier IV**을 선택한다.
