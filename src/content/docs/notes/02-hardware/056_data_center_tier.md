...
> 요약: 용량·경로와 정비·장애 시험으로 등급 판정

**동작 원리**

- **① 정비·장애 시험 시나리오 시작**: 목표 등급의 시험과 안전 조건 적용
- **② 우회 경로 용량·상태 확인**: 전체 부하와 절체 순간 부하 수용 검증
- **③ 핵심 부하 연속 공급**: 우회 경로로 전력·냉각 유지
- **④ 정비 경로 격리**: 작업 경로를 부하·상위 설비에서 분리
- **⑤ 정비·기능 시험 수행**: 설비 점검·교체와 보호·절체 시험
- **⑥ 정상 복구 상태 전달**: 전압·유량·온도·제어 조건 확인
- **⑦ 분배 경로 재투입**: 부하 영향을 감시하며 정상 구성 복귀
- **⑧ 장애·보호 동작 통지**: 센서·보호 장치가 고장 상태 전달
- **⑨ 고장 경로 격리**: 과전류·열·유체 문제의 전파 차단
- **⑩ 독립 경로로 연속 공급**: 운영자 개입 없이 핵심 부하 유지
- **⑪ 경보·복구 절차 전달**: 장애 영향·잔여 중복 상태 통지
