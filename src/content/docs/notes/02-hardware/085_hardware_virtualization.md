...
```mermaid
sequenceDiagram
    participant S as vCPU 스케줄러·하이퍼바이저
    participant C as CPU 가상화 확장
    participant G as 게스트 VM
    participant M as 페이지표·호스트 메모리
    participant I as 가상 I/O·IOMMU·장치

    S->>C: ① VMCS·VMCB 상태·Exit 조건 적재
    C->>G: ② VM Entry·게스트 실행
    G->>M: ③ 게스트 가상 주소 접근
    M-->>G: ④ 2단계 변환 후 데이터 반환
    G->>C: ⑤ 통제 대상 특권·I/O 사건
    C->>S: ⑥ VM Exit·원인·상태 저장
    S->>I: ⑦ 가상 장치 요청·격리 정책 전달
    I-->>S: ⑧ 장치 완료·가상 인터럽트 반환
    S->>C: ⑨ 게스트 상태·가상 인터럽트 갱신
...
> 요약: CPU·주소·입출력 가상화가 게스트 경계를 집행한다.

**동작 원리**

- **① 제어 상태 적재**: 게스트 레지스터·주소 기준·Exit 조건 설정
- **② 게스트 진입**: 호스트 상태를 보존하고 VM Entry 수행
- **③ 주소 접근**: 게스트 비특권 명령과 메모리 접근 직접 실행
- **④ 2단계 변환**: 게스트 가상 주소를 호스트 물리 주소로 변환
- **⑤ 통제 사건**: 특권 명령·I/O·페이지 위반 등 Exit 조건 발생
- **⑥ VM Exit**: 게스트 상태와 원인을 저장하고 제어권 회수
- **⑦ I/O 중재**: VM별 장치 권한과 DMA 범위 확인
- **⑧ 가상 완료**: 장치 결과를 완료 큐·가상 인터럽트로 변환
- **⑨ 재진입 준비**: 게스트 상태와 가상 인터럽트 갱신
- **⑩ 실행 재개**: VM Entry 후 중단 지점부터 계속 실행
...
- 게스트 ISA·특권 명령·2단계 주소 변환·I/O 가상화·전환 비용을 검토해 동일 ISA 게스트의 직접 실행에 하드웨어 가상화를 적용한다.
