...
- 배경/필요성: 초기 x86의 특권 명령 비트랩 결함으로 인한 소프트웨어 바이너리 변환(BT) 오버헤드 및 섀도 페이지 테이블 동기화 병목 한계
...
| **VT-d** (IOMMU) | DMA 주소 변환과 인터럽트 리매핑 격리 |
...
- CPU 모드 분리(VMX Root/Non-Root), VMCS 하드웨어 문맥 제어 및 EPT 2단계 주소 변환을 통해 클라우드 데이터센터 가상화(KVM, ESXi, Hyper-V)의 지배적 표준으로 안착되었으며, 최근에는 DPU/IPU 오프로드 및 기밀 컴퓨팅(AMD SEV, Intel TDX, Arm CCA) 기반 하드웨어 보안 격리로 진화
