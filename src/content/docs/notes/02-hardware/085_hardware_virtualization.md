...
date: "2026-08-26T23:41:00+09:00"
tags:
  - "notes-hardware"
weight: 85
extra:
...
- 배경/필요성: 초기 x86은 일부 특권 명령이 사용자 모드에서 예외를 일으키지 않는 **특권 명령 비트랩** 결함 탓에 하이퍼바이저가 명령을 실행 전에 바꿔치기하는 **바이너리 변환** 비용을 상시 치렀으므로, 게스트 전용 실행 모드와 2단계 주소 변환을 CPU 안에 두어 변환 없이 직접 실행할 필요
...
- 클라우드 가상화 구축 시 **VMX 모드**와 **EPT** 기반 하드웨어 가상화 적용
