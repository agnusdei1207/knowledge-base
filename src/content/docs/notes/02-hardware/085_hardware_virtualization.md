...
- **정의/개념**: CPU 실행 모드·주소 변환으로 게스트를 격리 실행
- **배경/필요성**: 명령 변환 비용으로 CPU 가상화 지원 필요
...
- 일반 명령은 직접 실행해 가상화 비용을 줄인다.
- EPT·NPT는 격리하지만 TLB 미스 지연을 늘린다.
- 가상 인터럽트·IOMMU가 중재 비용을 줄인다.
...
| 판단 기준 | 하드웨어 지원 가상화 | 소프트웨어 에뮬레이션·이진 변환 |
|:---|:---|:---|
| 핵심 특징 | 동일 ISA 게스트 직접 실행 | 명령 해석·변환 후 실행 |
| 적용 기준 | 동일 ISA VM 직접 실행 | 다른 ISA·장치 동작 재현 |
| 주요 위험 | VM Exit·TLB 미스·입출력 중재 | 해석·변환·모형 갱신 |
...
1. 클라우드 VM: 인터럽트 병합·준가상 드라이버로 Exit 축소
2. 메모리 집약 VM: 큰 페이지·NUMA 배치로 변환 지연 축소

### 쉽게 이해하기 (학습용)

- 여러 장치 알림을 묶고 전용 통로를 써 관리실 호출을 줄인다.
- 큰 주소 묶음을 가까운 메모리에 두어 장부와 원격 왕복을 줄인다.

## Ⅶ. 결론

- 동일 ISA 게스트 직접 실행 시 하드웨어 지원 선택
