...
- 배경/필요성: 초기 x86 아키텍처의 17개 민감 명령어가 Ring 1/2에서 트랩되지 않는 가상화 결함(Popek-Goldberg 조건 미충족)과 소프트웨어 바이너리 변환(BT) 및 섀도 페이지 테이블 유지보수로 인한 심각한 성능 저하 극복 필요
...
**동작 원리**:
1. 하이퍼바이저가 VMPTRLD 명령으로 VMCS 물리 주소를 CPU에 로드하고 게스트 초기 레지스터 상태와 트랩 인터셉트 조건을 설정함.
2. VMLAUNCH 또는 VMRESUME 명령을 실행하여 프로세서를 VMX Non-Root 모드로 전환하고 게스트 OS 커널 코드로 진입함.
3. 게스트 OS는 Ring 0 권한으로 물리 코어에서 일반 연산 및 시스템 호출을 네이티브 속도로 직접 수행함.
4. I/O 포트 접근, 제어 레지스터 변경 등 사전에 정의된 민감 특권 명령 실행 시 CPU 하드웨어가 실행을 중단하고 VMCS 게스트 영역에 현재 CPU 문맥을 자동 기록함.
5. 프로세서가 VMX Root 모드로 즉시 탈출(VM-Exit)하여 하이퍼바이저 핸들러가 해당 명령을 대행 처리한 후, VMRESUME 명령을 통해 변경된 문맥을 복원하고 게스트 실행을 재개함.
...
- 하드웨어 지원 가상화는 CPU 실리콘 수준의 VMX 듀얼 모드 분리와 EPT 2단계 중첩 페이징을 통해 엔터프라이즈 클라우드 인프라의 표준 토대로 확립되었으며, 향후 CXL 메모리 풀링 및 기밀 컴퓨팅(Intel TDX, AMD SEV-SNP) 하드웨어 메모리 암호화와 긴밀히 융합되는 방향으로 진화하고 있어, 대규모 인프라 설계 시에는 I/O 가상화(SR-IOV) 및 Huge Pages 설정을 결합하여 VM-Exit 오버헤드를 원천 차단하는 최적화 엔지니어링이 필수적이다.
