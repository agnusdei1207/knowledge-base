---
sidebar:
  order: 85
  label: "085. 하드웨어 가상화 — VT-x·AMD-V (Hardware Virtualization)"
  badge:
    text: "기출 · 70%"
    variant: note
title: "하드웨어 가상화 — VT-x·AMD-V (Hardware Virtualization)"
date: "2026-07-27T23:59:59+09:00"
tags:
  - "notes-hardware"
weight: 85
extra:
  question_no: "085"
  source_status: "기출"
  source_history: "137회"
  priority: 70
  priority_note: "VM 전환·2단계 주소 변환 병목"
---

## 미리 알고가기

- **하드웨어 가상화(Hardware Virtualization)**: 처리기가 게스트 실행·격리를 직접 지원
- **중앙처리장치(Central Processing Unit, CPU)**: 명령 실행·주소 변환을 담당하는 처리기
- **가상머신(Virtual Machine, VM)**: 격리된 가상 하드웨어에서 실행하는 시스템
- **하이퍼바이저(Hypervisor)**: VM 자원·실행 상태를 중재하는 제어 계층
- **인텔 가상화 기술(Intel Virtualization Technology for x86, VT-x)**: 인텔 x86 프로세서에서 게스트 실행 모드와 하이퍼바이저 전환을 지원하는 하드웨어 기능
- **AMD 가상화(AMD Virtualization, AMD-V)**: AMD x86 프로세서에서 게스트 실행 모드와 하이퍼바이저 전환을 지원하는 하드웨어 기능
- **가상머신 제어 구조(Virtual-Machine Control Structure, VMCS)**: 인텔의 게스트 상태·전환 제어 구조
- **가상머신 제어 블록(Virtual Machine Control Block, VMCB)**: AMD의 게스트 상태·전환 제어 구조
- **가상머신 진입(VM Entry)**: 하이퍼바이저에서 게스트로 제어권 전환
- **가상머신 탈출(VM Exit)**: 통제 사건에서 하이퍼바이저로 제어권 회수
- **확장 페이지 테이블(Extended Page Tables, EPT)**: 게스트 물리 주소를 호스트 물리 주소로 변환
- **중첩 페이지 테이블(Nested Page Tables, NPT)**: 게스트 물리 주소를 호스트 물리 주소로 변환
- **변환 참조 버퍼(Translation Lookaside Buffer, TLB)**: 최근 주소 변환 캐시
- **2단계 주소 변환(Two-stage Address Translation)**: 게스트 가상 주소를 게스트 물리 주소로, 다시 호스트 물리 주소로 바꾸는 변환
- **페이지 테이블 순회(Page-table Walk)**: TLB에 변환값이 없을 때 메모리의 페이지 테이블 계층을 따라 주소를 찾는 동작
- **가상 인터럽트(Virtual Interrupt)**: 하이퍼바이저나 하드웨어가 물리 장치 사건을 게스트 운영체제에 전달하는 가상 알림
- **직접 메모리 접근(Direct Memory Access, DMA)**: 장치가 처리기 없이 메모리에 접근
- **입출력 메모리 관리 장치(Input-Output Memory Management Unit, IOMMU)**: 장치 DMA 주소 변환·격리
- **불균일 메모리 접근(Non-Uniform Memory Access, NUMA)**: CPU 위치별 메모리 지연 차이
- **명령어 집합 구조(Instruction Set Architecture, ISA)**: 명령 형식·동작의 처리기 규약
- **준가상 드라이버(Paravirtualized Driver)**: 하이퍼바이저를 인식하는 가상 장치 드라이버
- **인터럽트 병합(Interrupt Coalescing)**: 여러 장치 알림을 하나의 인터럽트로 결합
- **에뮬레이션·이진 변환(Emulation·Binary Translation)**: 에뮬레이션은 다른 하드웨어 동작을 소프트웨어로 재현하고 이진 변환은 게스트 명령을 호스트 명령으로 바꿈
- **큰 페이지(Huge Page)**: 기본 페이지보다 큰 주소 단위를 사용해 TLB 항목 하나가 더 넓은 메모리를 덮게 하는 페이지

## Ⅰ. 개요

- 정의/개념: CPU 실행 모드·주소 변환으로 **게스트를 격리 실행**하는 기술
- 기존 한계: 소프트웨어 명령 변환은 **실행 오버헤드·호환 복잡도** 증가

### 쉽게 이해하기 (학습용)

- 손님은 방 안에서 직접 일하고 통제가 필요한 사건만 관리실이 처리한다.

## Ⅱ. 특징

- **비특권 명령 직접 실행**으로 명령 변환 비용 절감
- **EPT·NPT 2단계 주소 변환**으로 게스트 메모리 격리
- **VM Exit**가 특권 연산·입출력 전환 지연 증가

### 쉽게 이해하기 (학습용)

- 게스트 명령은 직접 실행되지만 VM Exit와 중첩 페이지 테이블 조회가 잦으면 가상화 오버헤드가 증가한다.

## Ⅲ. 아키텍처 및 구성요소

### 도표안 A. CPU·메모리·I/O 가상화 정적 구조

```mermaid
flowchart LR
    P([호스트 물리 메모리])
    D([물리 장치])
    subgraph V["하드웨어 가상화 경계"]
        G["게스트 VM"]
        C["CPU 가상화 확장"]
        H["하이퍼바이저·VMCS/VMCB"]
        M["게스트 페이지표·EPT/NPT"]
        I["가상 입출력·IOMMU"]
        G <-->|"VM Entry·Exit"| C
        H <-->|"상태·통제 조건"| C
        G -->|"메모리 접근"| M
        G -->|"장치 요청"| I
        H -.->|"자원·격리 정책"| M
        H -.->|"장치·인터럽트 정책"| I
    end
    M --> P
    I --> D
```

| 설계 요소 | 입력·상태 | 역할 |
|:---|:---|:---|
| 게스트 VM | 가상 CPU·메모리·장치 상태 | 게스트 운영체제·응용 실행 |
| CPU 가상화 확장 | VMCS·VMCB·Exit 조건 | 게스트 진입·탈출과 상태 전환 |
| 하이퍼바이저 | VM 상태·자원 정책 | 실행 중재·격리 경계 관리 |
| 게스트 페이지표·EPT/NPT | 게스트 가상·물리 주소 | 2단계 주소 변환·메모리 격리 |
| 가상 입출력·IOMMU | 장치 요청·DMA 주소 | 장치·인터럽트·DMA 격리 |

> 요약: CPU·주소·입출력 확장이 게스트 경계를 집행

### 쉽게 이해하기 (학습용)

- 하이퍼바이저는 게스트 실행 상태·메모리 주소 변환·장치 접근 경로를 가상머신별로 분리해 관리한다.

### 도표안 B. VM Entry·직접 실행·VM Exit 처리 시퀀스

```mermaid
sequenceDiagram
    participant S as vCPU 스케줄러
    participant H as 하이퍼바이저
    participant C as CPU 가상화 확장
    participant G as 게스트 VM
    participant M as 게스트 페이지표·EPT/NPT
    participant P as 호스트 물리 메모리
    participant I as 가상 I/O·IOMMU
    participant D as 물리 장치

    S->>H: ① 실행할 vCPU·시간 할당
    H->>C: ② VMCS·VMCB 게스트 상태·Exit 조건 적재
    C->>G: ③ VM Entry·게스트 실행 재개
    G->>M: ④ 게스트 가상 주소 접근
    M->>P: ⑤ 2단계 변환 후 호스트 물리 주소 접근
    P-->>G: ⑥ 메모리 데이터 반환
    G->>C: ⑦ 통제 대상 특권·I/O 사건 발생
    C->>H: ⑧ VM Exit·원인·게스트 상태 저장
    H->>I: ⑨ 가상 장치 요청·격리 정책 전달
    I->>D: ⑩ IOMMU 변환 후 물리 I/O 수행
    D-->>I: ⑪ 장치 완료·데이터 반환
    I-->>H: ⑫ 가상 완료·인터럽트 상태 전달
    H->>C: ⑬ 게스트 상태·가상 인터럽트 갱신
    C->>G: ⑭ VM Entry·중단 지점 이후 실행
```

#### 동작 원리

1. **vCPU 선택**: 스케줄러가 실행 가능한 VM 가운데 vCPU와 물리 CPU 시간 할당을 선택한다.
2. **제어 상태 적재**: 하이퍼바이저가 게스트 레지스터, 주소 변환 기준과 VM Exit 조건을 VMCS·VMCB에 설정한다.
3. **게스트 진입**: CPU 가상화 확장이 호스트 상태를 보존하고 게스트 상태를 복원해 VM Entry를 수행한다.
4. **게스트 주소 접근**: 게스트의 비특권 명령과 메모리 접근은 매번 하이퍼바이저 명령 변환을 거치지 않고 실행된다.
5. **2단계 주소 변환**: 하드웨어가 게스트 페이지표로 가상 주소를 게스트 물리 주소로, EPT·NPT로 호스트 물리 주소로 바꾼다.
6. **메모리 결과 반환**: 권한과 매핑이 유효하면 호스트 메모리의 데이터가 게스트 명령에 반환된다.
7. **통제 사건 발생**: 설정된 특권 명령, 장치 접근, 페이지 위반이나 외부 인터럽트가 CPU의 Exit 조건을 만족한다.
8. **VM Exit**: CPU가 게스트 상태와 종료 원인을 저장하고 하이퍼바이저의 지정 처리기로 제어권을 넘긴다.
9. **가상 I/O 중재**: 하이퍼바이저가 VM별 가상 장치 상태와 접근 권한을 확인해 I/O 계층에 요청한다.
10. **물리 장치 접근**: 가상 I/O·IOMMU 계층이 DMA 주소와 범위를 변환·격리한 뒤 실제 장치 작업을 수행한다.
11. **장치 완료**: 물리 장치가 데이터와 완료 상태를 I/O 가상화 계층에 반환한다.
12. **가상 완료 생성**: I/O 계층이 게스트가 이해하는 완료 큐·가상 인터럽트 상태로 변환해 하이퍼바이저에 전달한다.
13. **재진입 준비**: 하이퍼바이저가 게스트 상태와 가상 인터럽트를 갱신하고 다음 VM Entry를 요청한다.
14. **게스트 실행 재개**: CPU가 게스트로 재진입해 중단 지점 이후 명령을 계속 실행한다.

### 쉽게 이해하기 (학습용)

- 일반 명령은 방 안에서 직접 실행하고, 권한·장치 처리가 필요한 사건만 VM Exit로 관리실에 넘긴 뒤 다시 들어간다.

## Ⅳ. 종류 및 비교

| 가상화 실행 방식 | 하드웨어 지원 가상화 | 소프트웨어 에뮬레이션·이진 변환 |
|:---|:---|:---|
| 적용 기준 | 동일 ISA **VM 직접 실행** | 다른 ISA·**장치 동작 재현** |
| 핵심 특징 | 게스트의 **비특권 명령 직접 실행** | 명령 **해석·변환 후 실행** |
| 한계 | VM Exit·**TLB 미스·I/O 중재** | 해석·변환·**모형 갱신 비용** |

> 요약: 동일 ISA VM은 하드웨어 지원으로 직접 실행

### 쉽게 이해하기 (학습용)

- 같은 언어의 손님은 직접 일하고 다른 언어의 장비는 통역해 실행한다.

## Ⅴ. 실무 고려사항 및 대응 방안

| 운영 위험 | 대응 | 기대 효과 |
|:---|:---|:---|
| 잦은 VM Exit로 전환 지연 증가 | Exit 원인 계측과 준가상 장치·인터럽트 병합 적용 | 가상 CPU 처리량 향상 |
| 2단계 페이지 순회로 TLB 미스 비용 증폭 | 큰 페이지·TLB 지역성과 메모리 배치 최적화 | 주소 변환 지연 감소 |
| vCPU·메모리의 원격 NUMA 배치 | vCPU·메모리·장치의 동일 NUMA 노드 배치 | 원격 접근 지연 축소 |
| 장치 직접 할당이 VM 격리 약화 | IOMMU 최소 권한과 재할당 전 장치 초기화 | DMA 침해·잔류 상태 방지 |

### 쉽게 이해하기 (학습용)

- 손님이 직접 일하다 허가가 필요한 사건만 관리실을 거쳐 다시 방으로 간다.

### 적용 사례

- 클라우드 VM은 준가상 네트워크·블록 드라이버, 인터럽트 병합과 큰 페이지를 적용해 VM Exit와 2단계 페이지 순회 빈도를 줄인다.

### 쉽게 이해하기 (학습용)

- 여러 장치 알림을 묶고 전용 통로를 써 관리실 호출을 줄인다.

## Ⅵ. 결론

- 가상머신의 격리 성능과 하이퍼바이저 단순성을 확보하기 위해 **게스트 ISA·특권 명령·2단계 주소 변환·I/O 가상화·전환 비용**을 검토하고, 동일 ISA 게스트의 직접 실행에는 하드웨어 가상화를 적용한다

### 쉽게 이해하기 (학습용)

- 직접 일하는 이득이 관리실 호출과 주소·장치 통로 비용보다 클 때 사용한다.
