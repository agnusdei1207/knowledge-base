...
- 배경/필요성: 초기 x86의 **특권 명령 비트랩** 결함에 따른 소프트웨어 **바이너리 변환** 오버헤드 극복
...
| EPT 하드웨어 MMU | 메모리 변환단 | **EPT 중첩 페이징**을 통한 2단계 주소 변환 하드웨어 가속 수행 | 중첩 페이징 |
...
#### 한줄 요약
- VMCS 제어 설정 ➔ VM-Entry 진입 ➔ 게스트 네이티브 실행 ➔ 특권 명령 분기 처리 순으로 진행된다.
