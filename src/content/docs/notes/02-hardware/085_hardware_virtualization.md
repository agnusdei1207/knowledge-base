...
- 정의/개념: CPU의 특권 수준과 2단계 주소 변환 및 VM 제어 구조를 이용하여 게스트 명령의 직접 실행과 자원 격리를 지원하는 기술이다.
- 배경/필요성: 모든 민감 명령과 주소를 소프트웨어로 해석•변환하면 실행 비용이 크고 운영체제 호환과 정확한 특권 제어가 복잡해진다.

#### 한줄 요약

- 하드웨어 가상화는 게스트의 일반 명령은 **직접 실행**하고 통제가 필요한 사건에서만 하이퍼바이저로 전환한다.
...
- **VT-x•AMD-V**의 비특권 명령 직접 실행으로 변환 비용 절감이 핵심이다.
- **VMCS•VMCB** 기반 VM Entry•Exit로 통제 사건 중재가 핵심이다.
- **EPT•NPT** 2단계 주소 변환으로 게스트 메모리 격리가 핵심이다.

#### 한줄 요약

- **VM Exit•2단계 주소 변환** 빈도에 따른 오버헤드 증가한다.
...
- **게스트•CPU 확장•하이퍼바이저**와 메모리•장치 격리 구조이다.

## Ⅳ. 흐름도

<details><summary>핵심 용어</summary>

- **VM Entry**: 하이퍼바이저가 게스트에 제어권을 넘기는 전환한다.
...
- 직접 실행할 수 없는 **통제 사건에서만 VM Exit**하고 하이퍼바이저가 정책을 처리한 뒤 재진입한다.
...
- 호스트와 같은 ISA의 게스트는 **하드웨어 직접 실행**, 다른 ISA의 게스트는 명령 해석•동적 변환을 사용한다.
...
- **인터럽트 병합•준가상 장치**로 VM Exit 빈도를 축소한다.
...
- 동일 ISA는 **하드웨어 지원**, 다른 ISA는 **에뮬레이션**을 선택한다.

#### 한줄 요약

- 게스트의 **직접 실행 이득**이 VM Exit와 2단계 주소 변환 및 가상 I/O 비용보다 클 때 하드웨어 가상화가 효과적이다.
