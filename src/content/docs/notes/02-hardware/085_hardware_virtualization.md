...
- 정의/개념: CPU 가상화 모드와 **2단계 중첩 페이징(EPT/NPT)** 으로 무수정 게스트의 특권 실행·주소 변환을 지원하는 기술
- 배경/필요성: 순수 소프트웨어 바이너리 변환(BT)의 CPU 에뮬레이션 오버헤드 극복 및 x86 특권 명령 직접 실행 지원 필요

#### 한줄 요약
- CPU가 게스트 실행 모드와 2단계 주소 변환을 제공해 바이너리 변환·섀도 페이징 부담을 줄인다.
...
- 하이퍼바이저와 게스트의 제어 권한을 구분하는 **VMX Root / Non-Root 모드**
...
2. **제어권 이양**: 최초에는 VMLAUNCH, 이후에는 VMRESUME으로 Non-Root 모드 진입
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 빈번한 I/O의 **VM-Exit·에뮬레이션 비용** | 워크로드에 따라 SR-IOV 또는 virtio·vhost·폴링 적용 | 제어면 유연성과 데이터 경로 오버헤드 절충 |
| EPT 2단계 변환에 따른 **TLB 미스·페이지 워크** | 대용량 페이지와 중첩 TLB·페이지 배치 최적화 | TLB 도달 범위 확대와 페이지 워크 감소 |
| vCPU와 메모리의 **NUMA 원격 접근 지연** | vCPU·메모리·I/O를 같은 NUMA 노드에 고정하고 용량 감시 | 원격 메모리·I/O 트래픽 감소 |
| 공유 코어·캐시의 **사이드채널 공격** | CPU 완화책·마이크로코드·코어 스케줄링 격리를 위협 모델에 맞게 적용 | 성능 비용과 테넌트 간 정보 노출 위험 절충 |
...
- 서로 다른 커널·강한 격리가 필요하면 **VT-x·AMD-V VM**, 같은 커널의 밀도는 컨테이너 선택
- SR-IOV는 저지연 I/O가 마이그레이션·관측성 제약보다 중요할 때만 적용

#### 한줄 요약
- 평균 CPU 성능보다 VM-Exit율·TLB 미스·NUMA·I/O 경로를 측정해 필요한 하드웨어 기능을 선택해야 한다.
