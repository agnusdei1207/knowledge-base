...
- NAS는 **SMB·NFS**로 원격 파일과 이름공간 제공
- SAN은 **FC·iSCSI**로 SCSI 명령과 LUN 전달
- DAS·SAN의 **LBA**를 호스트 파일시스템이 해석
...
- **1. 접근 단위 변환**: DAS·SAN은 호스트, NAS는 파일 서비스가 블록 주소 해석
- **2. 저장장치 접근**: 직결 또는 네트워크 경로에서 권한 확인 후 데이터 처리
...
| 단일 경로 장애 | **다중 경로** 구성·전환 시험 | **I/O 연속성 확보** |
...
- 단일 서버 블록은 **DAS**, 공유 파일은 NAS, 다중 서버 블록은 SAN 선택
