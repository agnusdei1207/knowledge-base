...
- 배경/필요성: 단일 서버 로컬 디스크의 **데이터 고립(Silo)**으로 다중 서버 환경의 **스토리지 자원 공유 한계** 발생
...
|:---|:---|:---|:---|
| 호스트 HBA 카드 | 호스트 서버 내부 | 호스트 블록 명령어를 광 FC 프레임 또는 **RoCE 패킷**으로 변환 | 32G/64G FC HBA |
| SAN 스위칭 패브릭 | 스토리지 전용망 | 호스트 HBA와 스토리지 포트 간의 **무손실 광 프레임 라우팅** | 광 스위치 패브릭 |
| 스토리지 컨트롤러 | 스토리지 어레이 | RAID 패리티 연산, 캐싱, **씬 프로비저닝** 및 LUN 매핑 | 액티브-액티브 듀얼 |
| LUN 볼륨 | 스토리지 논리단 | 호스트 OS에 물리 디스크로 인식되는 **가상 블록 저장소** | 블록 레벨 할당 |
| NAS 어플라이언스 | 파일 서비스단 | 내장 파일 시스템을 통해 이기종 클라이언트에 **파일 공유 서비스** 제공 | NFS / SMB 지원 |
...
- 데이터 I/O 특성 분석 기반 **NAS 파일 공유**와 **NVMe-oF SAN** 계층 배치
