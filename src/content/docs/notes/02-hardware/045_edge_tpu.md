...
- **① 장치 코드·CPU 구간·보고서 전달**: 지원 연산과 폴백 구간의 실행 계획 등록
- **② 입력 텐서·추론 요청**: 현장 센서 입력을 정수 텐서로 제출
- **③ 정수 텐서·장치 코드 실행**: 지원 구간을 Edge TPU에서 실행
- **④ 가속 구간 결과 반환**: 다음 장치·CPU 구간용 출력 전달
- **⑤ 경계 텐서·비지원 연산 전달**: 동기화·형식 변환 후 CPU로 전송
- **⑥ CPU 실행 결과 반환**: 전후처리·비지원 연산 결과 전달
- **⑦ 현장 추론 결과 반환**: 모든 구간 완료 후 네트워크 왕복 없이 판정
