...
- 정의/개념: 완전 정수 양자화 TFLite 모델의 지원 연산을 현장에서 실행하는 **저전력 추론 ASIC**이다.
- 배경/필요성: 클라우드 추론은 네트워크 단절·왕복 지연·데이터 전송 때문에 **현장 실시간 판정 제약**이 발생한다.
...
- 메모리•전력 소모를 줄이는 **완전 정수 실행**이 핵심이다.
- Edge TPU 실행 구간을 생성하는 **지원 연산 컴파일**이 핵심이다.
- 비지원 연산 경계마다 텐서 전송·동기화를 발생시키는 **CPU 폴백 비용**이 핵심이다.
...
1. **지원 구간·실행 계획 생성**: 컴파일러가 연속된 지원 연산과 비지원 연산을 구분해 Edge TPU·CPU 배치를 확정한다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 완전 정수 양자화 오차가 누적되어 **모델 정확도 저하** | 실제 입력을 대표하는 **보정 데이터와 종단 정확도 비교** | 배포 품질 기준 안에서 **정확도 손실 통제** |
| CPU 폴백 경계가 반복되어 **텐서 복사·동기화 증가** | 컴파일 보고서를 확인해 비지원 연산자를 **지원 연산으로 대체·결합** | 경계 수 감소로 **장치 전환 비용 최소화** |
| 모델 크기·실행 빈도가 단말의 **메모리·전력·열 한도 초과** | 모델·입력 해상도·**실행 주기**를 단계적으로 조정 | 열 스로틀링 방지로 **지속 추론 주기 안정화** |
| 오프라인 현장에서 모델 교체 중 오류가 발생해 **추론 불가** | 서명 검증과 **원자적 모델 교체·롤백** 적용 | 정상 모델 유지·복구로 **배포 신뢰성 확보** |
...
- 높은 **지원 연산률** 및 오프라인 추론 필요 시 **Edge TPU**을 적용한다.
