...
date: "2026-07-25T17:39:00+09:00"
tags:
  - "notes-hardware"
weight: 45
extra:
...
- **에지 텐서 처리 장치(Edge Tensor Processing Unit, Edge TPU)**: 현장 장치의 정수 추론용 전용 반도체
- **텐서플로 라이트(TensorFlow Lite, TFLite)**: 단말 추론 모델 형식과 런타임
...
- **정의/개념**: 완전 정수 TFLite 현장 추론용 ASIC
- **배경/필요성**: 망 연결 제약으로 현장 정수 추론 가속 필요
...
- 완전 정수 연산은 메모리·전력을 줄여 현장 추론을 돕는다.
- 비지원 연산자·텐서 형태는 CPU로 넘어가 추론을 지연시킨다.
...
| 판단 기준 | Edge TPU | 온디바이스 NPU | 클라우드 TPU |
|:---|:---|:---|:---|
| 핵심 특징 | 단말 연결형·모듈 가속기 | SoC 내장 가속기 | 데이터센터 가속기 |
| 모델 경로 | 정수 TFLite·Edge TPU 컴파일 | 벤더 런타임·모델 변환 | XLA 그래프 컴파일 |
| 데이터 이동 | 호스트와 가속기 간 전송 | 온칩 메모리 공유 | 네트워크·고대역 메모리·칩 간 연결 |
| 적용 기준 | 현장 센서·저전력 정수 추론 | 제품 통합·단말 추론 | 대형 학습·대규모 추론 |
| 주요 위험 | 지원 연산·CPU 전환 지연 | 벤더별 연산·도구 제약 | 클라우드 비용·네트워크 왕복 |
...
1. 스마트 카메라는 비지원 연산자를 바꿔 CPU 구간 축소
...
- 완전 정수 컴파일·짧은 CPU 구간이면 Edge TPU 선택
