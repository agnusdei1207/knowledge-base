...
date: "2026-08-08T19:55:00+09:00"
tags:
  - "notes-hardware"
weight: 45
extra:
...
- 정의/개념: **현장 추론**을 위해 완전 정수 양자화 TFLite 모델의 지원 연산을 실행하는 **ASIC**인 **Edge TPU**이다.
- 배경/필요성: 클라우드 추론은 네트워크 단절·왕복 지연·데이터 전송 때문에 현장 실시간 판정에 제약을 받는다.

#### 한줄 요약

- Edge TPU는 완전 정수 모델을 현장에서 실행해 네트워크 단절 중에도 낮은 전력으로 추론 결과를 제공한다.
...
- 메모리•전력 소모를 줄이는 **완전 정수 양자화**가 핵심이다.
- Edge TPU 실행 구간을 생성하는 **지원 연산 컴파일**이 핵심이다.
- 비지원 연산 경계마다 텐서 전송·동기화를 발생시키는 **CPU 폴백·장치 전환 비용**이 핵심이다.

#### 한줄 요약

- Edge TPU 지원 연산률이 낮고 폴백 경계가 많으면 CPU와 장치 사이의 텐서 복사·동기화로 종단 지연이 증가한다.
...
- **지원 구간·실행 계획 생성**: 지원·비지원 연산을 나눠 Edge TPU와 CPU 배치를 정하는 단계이다.
- **Edge TPU 구간 실행**: 지원 정수 연산과 입력 텐서를 Edge TPU에 제출하는 단계이다.
- **경계 텐서·CPU 폴백**: 비지원 연산의 중간값을 CPU로 보내 대체 실행하는 단계이다.
- **구간 결과 연결**: 장치별 출력을 다음 실행 구간의 입력으로 이어 최종 결과를 만드는 단계이다.
...
1. **지원 구간·실행 계획 생성**: 컴파일러가 연속된 지원 연산과 비지원 연산을 구분해 Edge TPU·CPU 배치의 **정수 실행 계획**을 확정한다.
2. **Edge TPU 구간 실행**: 런타임이 지원 정수 연산 구간과 입력 텐서를 Edge TPU에 제출한다.
3. **경계 텐서·CPU 폴백**: 비지원 연산 앞뒤에서 중간 텐서를 CPU 메모리 경계로 전달해 대체를 실행한다.
4. **구간 결과 연결**: 장치별 출력을 다음 실행 구간의 입력으로 연결해 최종 현장 추론 결과를 생성한다.

#### 한줄 요약

- 런타임은 지원 구간을 Edge TPU에서 실행하고, 비지원 구간은 경계 텐서를 CPU로 전달해 실행한 뒤 결과를 다시 연결한다.
...
| 핵심 특징 | **Edge TPU** | **온디바이스 NPU** | **클라우드 TPU** |
| 한계 | 지원 연산•**장치 전환 비용** | 벤더별 연산•도구 제약 | 클라우드 비용•**네트워크 왕복 지연** |

> 요약: 배치•모델 형식•전송 지연으로 가속기를 선택한다.

#### 한줄 요약

- 외장 모듈형 정수 추론은 Edge TPU, SoC 통합 단말 추론은 온디바이스 NPU, 대규모 학습·추론은 클라우드 TPU가 적합하다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| **완전 정수 양자화** 오차가 누적되어 모델 정확도 저하 | **대표 보정 데이터**로 원본 모델과 비교 | 정확도 손실 통제 |
| **CPU 폴백** 경계가 반복되어 텐서 복사·동기화 증가 | 컴파일 보고서를 확인해 **지원 연산자 대체** 적용 | **장치 전환 비용** 감소 |
| 모델 크기·실행 빈도가 단말의 메모리·전력·열 한도 초과 | 모델·입력 해상도·실행 주기를 단계적으로 조정 | 지속 추론 주기 안정화 |
| 오프라인 현장에서 모델 교체 중 오류 발생 | 서명 검증과 **원자적 모델 교체·모델 롤백** 적용 | 정상 모델 유지·복구 |

> **지원 연산자 대체**로 CPU 폴백과 텐서 복사 감소
...
- **Edge TPU 적용 기준**: 지원 연산률·오프라인 요구·지속 추론 성능으로 적용 여부를 정하는 기준이다.

</details>

- **Edge TPU 적용 기준**에 따라 **지원 연산률**이 높고 **오프라인 추론·지속 추론**이 필요하면 **Edge TPU**를 적용한다.

#### 한줄 요약

- 지원 연산률이 높고 CPU 폴백을 포함한 종단 지연·전력이 목표를 만족할 때 Edge TPU를 적용한다.
