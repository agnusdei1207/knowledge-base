...
- 배경/필요성: 단일 프로세서 설계로는 **모바일·서버 범용 OS, 자동차 실시간 제어, 초소형 IoT 저전력 환경의 상충된 요구조건 동시 충족 불가**
...
[Armv8/v9 프로세서 아키텍처]
 ├─ [프로파일 분류 체계]
 │   ├─ Cortex-A (Application: MMU + 가상화 + 리눅스/안드로이드)
 │   ├─ Cortex-R (Real-Time: MPU + 결정론적 초저지연 제어)
 │   └─ Cortex-M (Microcontroller: NVIC + 초저전력 제어)
 │
 ├─ [실행 상태 (Execution State)]
 │   ├─ AArch64: 64-bit 범용 레지스터(X0~X30), SP, PC, 64-bit 가상 주소
 │   └─ AArch32: 32-bit 범용 레지스터(R0~R15), CPSR, 레거시 호환
 │
 ├─ [권한 모델 (Exception Levels)]
 │   ├─ EL0: 비특권 사용자 애플리케이션 (User App)
 │   ├─ EL1: OS 커널 및 디바이스 드라이버 (OS Kernel)
 │   ├─ EL2: 가상화 하이퍼바이저 (Type-1 Hypervisor)
 │   └─ EL3: 시큐어 모니터 및 펌웨어 (Secure Monitor / ATF)
 │
 └─ [하드웨어 보안 및 확장]
     ├─ TrustZone: Normal World(비보안) vs Secure World(보안) 물리 격리
     └─ 벡터 엔진: NEON 및 SVE2 가변 벡터 연산 유닛
```

선의 의미: 가지(`├─`, `└─`)는 하드웨어 아키텍처의 소속 및 계층 구조; EL 수준은 상위로 갈수록 더 높은 시스템 제어 특권을 보유함
...
| **TrustZone** | 시스템 보안 아키텍처 | 버스 마스터 및 슬레이브 메모리의 보안 비트 기반 하드웨어 격리 | 보안 키, 금융 인증, DRM 독립 보호 |
...
```text
EL0 (사용자 앱) 실행 중 시스템 호출(SVC) 또는 하드웨어 인터럽트 발생
                                │
                                ▼
1. 하드웨어 자동 상태 백업 및 권한 승격
   - 현재 PSTATE를 SPSR_EL1에 저장, 복귀 주소(PC)를 ELR_EL1에 저장
   - 프로세서 실행 권한을 EL1 (OS 커널)로 전환
                                │
                                ▼
2. 벡터 테이블(VBAR_EL1) 진입 및 예외 핸들러 실행
   - 예외 종류 및 소스 EL 오프셋에 따라 해당 ISR 루틴 점프
                                │
                                ▼
3. 서비스 완료 및 ERET 명령어 실행
   - SPSR_EL1 ➔ PSTATE 복원, ELR_EL1 ➔ PC 복원
                                │
                                ▼
4. EL0 (사용자 앱) 복귀 및 다음 명령어 실행 재개
```

분기 결과: **동일 EL 처리는** 레지스터 백업 없이 즉시 실행되나, **상위 EL 승격은** SPSR/ELR 하드웨어 백업을 거치며, **EL3 트랩은** TrustZone 보안 세계 전환이 수반됨
...
| 메모리 관리 장치 | **가상 메모리 MMU (페이징 지원)** | **물리 메모리 MPU (영역 보호만 지원)** | **선택적 MPU 지원 (단순 주소 매핑)** |
| 실시간 결정성 | 비결정론적 (가상 메모리·캐시 미스 지연) | **결정론적 (TCM 메모리 및 고정 응답 보장)** | **초저지연 하드웨어 인터럽트(NVIC)** |
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| A-Profile 가상 메모리 페이징 및 캐시 미스로 인한 실시간성 훼손 | 엄격한 실시간 제어 구간에는 **R-Profile 코어 및 TCM** 적용 | 지터(Jitter) 없는 마이크로초 단위 결정론적 응답 보장 |
| Normal World의 취약점이 보안 자산(암호키 등)으로 전파될 위험 | **TrustZone 하드웨어 격리 및 EL3 시큐어 모니터** 적용 | OS가 탈취되어도 보안 영역 내 핵심 자산 완벽 방어 |
| 다양한 프로세서 코어 간 이기종 멀티프로세싱 시 통신 오버헤드 | **DynamIQ 클러스터링 및 CCI(캐시 일관성 인터커넥트)** 적용 | Cortex-A와 Cortex-R/M 간 하드웨어 캐시 일관성 유지 |
...
- 대규모 데이터 처리와 범용 OS 구동에는 **MMU와 EL2 가상화를 내장한 A-Profile**, 기능 안전(ISO 26262) 및 실시간 제어에는 **결정론적 R-Profile**, 초저전력 엣지에는 **M-Profile**을 선택 적용하고, 시스템 전반의 보안성 확보를 위해 **TrustZone 및 EL0~EL3 계층 격리 모델**을 필수 구현하는 고효율 프로세서 아키텍처 확립
