...
- **로드·스토어 구조(Load/Store Architecture)**를 채택하여 ALU 파이프라인 동작 시 메모리 접근 대기 지연을 우회하고 실행 효율 증대.
- **AArch64** 및 **AArch32**의 **실행 상태(Execution State)** 전환과 **예외 수준(Exception Level)**을 결합하여 레가시 32비트 코드 호환과 64비트 하드웨어 권한 격리를 동시 달성.
...
- **AArch64**: A64 명령어 세트를 구동하고 31개의 64비트 범용 레지스터(X0~X30) 및 64비트 가상 주소 공간을 지원하는 최신 실행 상태.
...
|:---|:---|:---|
| **AArch64** | 64비트 레지스터 및 가상 주소 공간 관리, 64-bit A64 명령 인출 | 대용량 RAM 직접 참조 및 고성능 컴퓨팅 워크로드 구동 |
| **AArch32** | 32비트 호환 레지스터 및 A32/T32 명령어 변환 실행 | 기존 32-bit 모바일/임베디드 레가시 바이너리의 하위 호환 보장 |
| **예외 수준 (EL0~EL3)** | **EL 격리(EL Isolation)** 체계를 구현하여 하드웨어 접근 권한 제어 | 사용자 앱 오류 시 OS 영역 보호 및 TrustZone 보안 실행 환경 제공 |
| **NEON / SVE** | 고정 128-bit(NEON) 또는 가변 128~2048-bit(SVE) SIMD 병렬 계산 | 음성, 영상 처리 및 AI 딥러닝 텐서 계산 속도 대폭 개선 |
...
| **전력 및 집적성** | 압도적 전력 효율, **SoC 통합** 최적화 | 고주파수/고전력 특성, 독립형 CPU 커넥터 우세 |
...
|:---|:---|:---|
| x86 중심 C/C++ **네이티브 라이브러리**를 Arm 인프라로 이전 시 호환성 충돌 | 타깃 Arm **AArch64 ABI** 빌드 체계 구축 및 동적 바이너리 변환(Rosetta 2/Box64) 적용 | 異종 인프라 마이그레이션 오류 제거 및 네이티브 성능 확보 |
| 불필요하게 커널/보안 권한(EL1/EL3)을 점유하여 시스템 안정성 훼손 | **최소 권한(Least Privilege)** 원칙 준수 및 EL 레벨 간 HVC/SMC 인터페이스 정밀 통제 | 하드웨어 기반 사이드채널 공격 및 보안 침입 파급력 차단 |
| SVE/SVE2 벡터 명령 사용 시 특정 구형 Arm 코어 단말에서 실행 crash | **런타임 기능 탐지(Feature Detection)** 적용 및 **대체 코드(Fallback Code)** 제공 | 단말 코어 세대 격차에 관계없이 범용 실행 안정성 보장 |
...
- **ISA 선택 기준(ISA Selection Criteria)**에 근거하여 초저전력 모바일 단말부터 hyperscaler 클라우드 데이터센터 서버에 이르기까지 **전력 효율(Power Efficiency)**과 SoC 통합성이 최우선되는 컴퓨팅 인프라 구축 시 **Arm 아키텍처**를 적극 도입하고, **네이티브 라이브러리 재빌드(Native Library Rebuild)** 및 4단계 예외 수준(EL0~EL3) 보안 통제 체계 구현 필수.

#### 한줄 요약
- 저전력/고성능 SoC 시스템 구축을 위한 Arm AArch64 아키텍처 채택 및 Exception Level 기반 보안/이식성 검증 체계 적용.
