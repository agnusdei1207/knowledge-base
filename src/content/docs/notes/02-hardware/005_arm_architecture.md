...
- **명령어 집합 구조(Instruction Set Architecture, ISA)**: ‘아이에스에이’로 읽고 영문 머리글자를 딴 약어이며, 프로세서의 명령어·레지스터·실행 환경을 정한 규격이다
- **로드·스토어 구조(Load/Store Architecture)**: 메모리 값을 반드시 레지스터로 실어 온 뒤에만 연산하고 결과도 따로 내려놓게 하여 메모리 접근과 연산을 아예 다른 명령으로 갈라 둔 구조
- **AArch64·AArch32 실행 상태**: ‘에이아크 육십사·삼십이’로 읽고 Arm Architecture와 비트 수를 결합한 표기이며, 각각 A64 또는 A32·T32 명령을 실행하는 환경이다
- **동작 모드(Processor Mode)**: AArch32에서 사용자·감독자·인터럽트처럼 권한과 예외 원인별로 나눠 둔 실행 모드
- **예외 수준(Exception Level, EL)**: ‘이엘’로 읽고 영문 머리글자를 딴 약어이며, AArch64 권한을 EL0~EL3 네 단계로 나눈 체계다
- **레지스터 집합(Register File)**: 연산에 쓸 데이터와 주소를 담아 두는 CPU 내부 고속 저장소 묶음이며 실행 상태에 따라 개수와 폭이 달라진다
- **단일 명령 다중 데이터(Single Instruction Multiple Data, SIMD)**: ‘심드’로 읽고 영문 머리글자를 단어처럼 읽는 약어이며, 한 명령으로 여러 데이터 원소를 함께 계산한다
- **NEON**: ‘네온’으로 읽는 Arm의 SIMD 상표명이며, 고정 폭 벡터 레지스터로 영상·음성 데이터를 묶음 처리한다
- **확장 가능 벡터 확장(Scalable Vector Extension, SVE)**: ‘에스브이이’로 읽고 영문 머리글자를 딴 약어이며, 구현마다 다른 벡터 길이에서도 같은 바이너리를 실행하는 Arm 확장이다
- **응용 이진 인터페이스(Application Binary Interface, ABI)**: ‘에이비아이’로 읽고 영문 머리글자를 딴 약어이며, 함수 호출의 인자·결과 레지스터를 정한 규약이다
...
- **시스템온칩(System on Chip, SoC)**: ‘에스오씨’로 읽고 영문 핵심 글자를 딴 약어이며, 프로세서·메모리 제어기·주변 장치를 한 칩에 통합한 반도체다
- **반도체 설계자산(Intellectual Property, IP)**: ‘아이피’로 읽고 영문 머리글자를 딴 약어이며, 검증된 프로세서·주변 장치 설계를 칩에 재사용하는 단위다
