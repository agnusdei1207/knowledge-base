...
- **① 예외·트랩 발생**: 시스템 호출·인터럽트·권한 위반 등으로 현재 실행 흐름을 예외 제어기에 전달
- **② 현재 실행 상태 저장**: 원래 코드로 돌아갈 프로그램 카운터와 프로세서 상태를 예외 수준별 레지스터에 보관
- **③ 대상 예외 수준 결정**: 예외 원인·라우팅 설정·현재 EL에 따라 EL1·EL2·EL3의 예외 벡터 선택
- **④ 예외 처리·복귀 상태 확정**: 선택한 처리기가 원인을 처리하고 복귀 주소·권한·마스크 상태를 검증
- **⑤ 예외 복귀**: 저장된 상태를 복원해 원래 실행 수준과 명령 흐름으로 전환
