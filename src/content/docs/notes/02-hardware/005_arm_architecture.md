...
- **실행 상태** 는 사용할 명령어 집합과 레지스터 폭을 정하는 프로세서 동작 상태이다.
- **예외 수준** 은 응용•커널•하이퍼바이저•보안 모니터의 권한을 단계별로 격리하는 체계이다.
- **선택 명령 확장** 은 벡터•암호•부동소수점처럼 용도에 따라 선택적으로 구현하는 추가 명령 집합이다.
...
- **실행 상태•예외 수준** 은 명령 실행 환경과 권한 단계를 조합해 용도별 동작 범위를 구분한다.
...
- **ABI** 는 함수 호출 규약•레지스터 사용•바이너리 형식을 정해 같은 ISA의 프로그램과 운영 환경을 연결한다.
...
- **최소 권한•트랩 경로** 는 각 EL에 필요한 권한만 부여하고 예외가 예상한 처리 수준으로 전달되는지 검증하는 기준이다.
- **대체 코드** 는 특정 명령 확장을 지원하지 않는 코어에서도 같은 기능을 수행하도록 마련한 일반 명령 경로이다.
- **SoC•부하별 벤치마크** 는 실제 칩과 작업 조건별 성능•전력 소비를 측정해 구성을 비교하는 시험이다.
...
- **Arm과 x86-64** 는 각각 전력•SoC 통합성과 기존 바이너리 호환성을 대표적인 선택 기준으로 삼는 ISA 계열이다.
