...
- 정의/개념: 로드·스토어 기반 RISC 구조를 바탕으로 AArch64/AArch32 **실행 상태**, 응용 분야별 특화 **프로파일**(A/R/M)과 하드웨어 권한 격리 계층(EL0~EL3)을 제공하는 **Arm 아키텍처**
- 배경/필요성: 단일 아키텍처 규격으로는 모바일 고성능 연산, 차량·산업용 실시간 제어, 초저전력 IoT 마이크로컨트롤러의 상충되는 하드웨어 요구조건을 한 벌의 메모리 관리·인터럽트 체계로 충족할 수 없으므로, 공통 RISC ISA는 유지한 채 그 체계만 프로파일별로 갈라 하나의 툴체인 생태계로 세 시장을 포괄
...
분기 결과: 예외 유형과 현재 EL에 따라 진입할 벡터 엔트리와 목표 **예외 수준**이 갈리지만, 어느 갈래든 **예외 복귀 레지스터**(SPSR·ELR) 저장과 **ERET** 복원이라는 고정 비용은 예외마다 동일하게 지불됨
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| A-profile의 캐시·가상 메모리로 **인터럽트 지연시간** 변동 | **R-profile** 및 **TCM** 적용 | 메모리 접근 지연의 예측 가능성 향상 |
...
- 모바일 시장의 지배적 표준을 넘어 클라우드 데이터센터(Neoverse) 및 차량용 반도체(Armv9)로 확산 중이며, 향후 기밀 컴퓨팅(CCA) 및 **SIMD/벡터 확장**(SVE2) 기반 AI 가속을 중심으로 아키텍처 영향력 지속 확대 전망
