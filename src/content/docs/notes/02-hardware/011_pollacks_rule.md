...
| 구성요소 | 책임 |
|:---|:---|
| 워크로드 모델 | **직렬 비율•p99 지연** 요구 정의 |
| 실리콘 면적 | 코어 구성의 **다이 면적 예산** 설정 |
| 후보 구성기 | **Big•Small 코어 조합** 생성 |
| 운영 가능 범위 검증기 | **TDP•메모리 대역폭** 한계 검증 |
...
1. **후보 생성**: 면적 안에서 대형•소형 코어 조합을 생성함.
2. **성능 추정**: 폴락•암달 법칙으로 지연과 처리량을 추정함.
3. **운영 상한 평가**: **TDP•대역폭** 한계 내 동작 여부를 평가함.
4. **설계 확정**: 요구를 만족하는 Big•Small 코어 구성을 선택함.
...
- p99 지연이 우선이면 **Big Core**, 병렬 처리량이 우선이면 **Small Core** 확대.

#### 한줄 요약
- 지연•처리량과 면적 수익 체감을 기준으로 이종 코어 비율을 결정함.
