...
sequenceDiagram
    ...
    M->>C: ① 워크로드·면적 기반 후보 요청
    C->>P: ② 코어 크기·개수 조합 생성
    P->>R: ③ 단일 성능·처리량 추정
    R->>D: ④ 자원 제약 검증
    alt 목표 지연·처리량 또는 제약 미충족
        D-->>C: ⑤ 코어 배분 조정·최종 후보 확정
    else 목표와 제약 충족
        D-->>M: ⑤ 코어 배분 조정·최종 후보 확정
    end
```

| 설계 요소 | 설명 |
|:---|:---|
| 칩 면적 예산 | 코어·공유 자원이 나눠 쓰는 **실리콘 면적** |
| 코어 복잡도 | 실행 폭·캐시로 정해지는 **단일 코어 규모** |
| 코어 수 | 남은 면적에 배치하는 **물리 코어 개수** |
...
- **① 워크로드·면적 기반 후보 요청**: 순차·병렬 비율과 p99·면적 예산 정리
- **② 코어 크기·개수 조합 생성**: 대형·소형 코어와 공유 자원 후보 구성
- **③ 단일 성능·처리량 추정**: 제곱근 성능과 병렬 처리량 계산
- **④ 자원 제약 검증**: 전력·열·인터커넥트·대역폭 한도 확인
- **⑤ 코어 배분 조정·최종 후보 확정**: 목표·제약 충족 여부로 재평가 또는 선택
