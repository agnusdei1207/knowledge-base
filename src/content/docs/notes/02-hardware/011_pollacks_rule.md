...
- **마이크로아키텍처**: 같은 ISA를 실행 폭•캐시•예측기 등의 회로로 실제 구현한 내부 구조
...
- **p99**: 전체 요청의 99%가 이 값 이하에 완료되는 꼬리 지연 지표
...
- **p99**: 요청 완료 시간 분포의 99번째 백분위 값으로 꼬리 지연 목표에 사용한다.
...
- **p99 지연**: 요청의 99%가 완료되는 시간으로 느린 일부 요청까지 반영하는 서비스 지연 지표
...
- **p99 응답시간**: 측정한 요청의 99%가 이 값 이하에 완료되는 꼬리 지연 지표
...
- **p99**: 요청 완료 시간의 99번째 백분위 값으로 순차 임계경로가 만드는 꼬리 지연을 판단하는 지표
- **전력•열•대역폭 상한**: 코어 수와 크기를 늘릴 때 지속 가능한 동작을 제한하는 자원 한도
