...
date: "2026-07-28T12:38:21+09:00"
tags:
  - "notes-hardware"
weight: 11
extra:
...
- **99백분위 응답시간(p99)**: 전체 요청의 99%가 완료되는 시점의 응답시간으로 느린 꼬리 지연을 나타내는 지표
...
| 실패 징후 | 완화 방안 | 기대 효과 |
|:---|:---|:---|
| 병렬 워크로드에 대형 코어만 배치 | 순차·병렬 비율과 p99·처리량 목표 분리 측정 | 면적 배분 적합성 향상 |
| 소형 코어 증설 후 처리량 정체 | 메모리 대역폭·인터커넥트·동기화 병목 개선 | 코어 수 확장 효과 회복 |
| 면적 모델은 좋으나 전력·열 한도 초과 | 전력 밀도·냉각·주파수 제약 동시 모델링 | 지속 가능한 성능 확보 |
| 다른 공정에 경험칙을 그대로 적용 | 실제 공정·마이크로아키텍처 벤치마크로 보정 | 추정 오차 감소 |

### 쉽게 이해하기 (학습용)

- 폴락의 법칙은 경험적 출발점이므로 실제 워크로드, 공정, 전력과 메모리 한도로 후보 구성을 검증해야 한다.
