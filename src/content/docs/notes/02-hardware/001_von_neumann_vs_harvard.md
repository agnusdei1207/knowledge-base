...
- **동시 접근 병목**: 명령어 인출과 데이터 접근이 같은 버스를 두고 경쟁해 프로세서가 대기하는 현상

</details>

- 정의/개념: 명령어•데이터의 메모리·버스 공유 여부로 구분하는 **컴퓨터 구조 유형**
- 배경/필요성: 공유 버스에서 인출·데이터 접근이 경합해 **처리량 감소**
...
- **명령어•데이터 동시 접근**: 분리된 경로에서 명령어 인출과 데이터 접근을 같은 시점에 수행하는 방식
...
- 주소 공간 통합으로 코드·데이터 간 **용량 배분** 제약 완화
...
2. **분리 버스 접근**: 명령어 인출과 데이터 접근을 별도 버스에서 동시 처리
...
> 요약: 하버드의 **용량 고정**을 주기억 통합으로 완화한 수정 하버드
...
- 용량 유연성이 중요하면 **수정 하버드**, 동시 접근이 핵심이면 **하버드** 선택
