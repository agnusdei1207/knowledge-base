...
| 구성요소 | 책임 |
|:---|:---|
| CPU | 명령어 인출•해독과 데이터 접근 제어 |
| 명령어 메모리•버스 | **명령어 전용 경로**로 연속 인출 |
| 데이터 메모리•버스 | **데이터 전용 경로**로 피연산자 전송 |
| 통합 메모리•버스 | **공유 경로**로 코드•데이터 전송 |
...
- 범용 CPU는 **수정 하버드**, 결정적 실시간 DSP•MCU는 **하버드 구조** 선택

#### 한줄 요약
- 범용 시스템은 수정 하버드, 결정적 실시간 처리는 순수 하버드 구조를 선택한다.
