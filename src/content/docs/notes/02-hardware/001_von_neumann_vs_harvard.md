...
date: "2026-07-28T12:31:39+09:00"
tags:
  - "notes-hardware"
weight: 1
extra:
...
| 설계 요소 | 입력·상태 | 역할 |
|:---|:---|:---|
| CPU | 명령어·연산 데이터 | 인출과 읽기·쓰기를 제어 |
| 통합 메모리·버스 | 단일 주소 공간 | 명령어·데이터의 공유 경로 제공 |
| 명령어 메모리·버스 | 명령어 주소 | 데이터 접근과 분리된 인출 경로 제공 |
| 데이터 메모리·버스 | 데이터 주소·값 | 연산 데이터의 읽기·쓰기 경로 제공 |
...
| 실패 징후 | 완화 방안 | 기대 효과 |
|:---|:---|:---|
| 명령어·데이터 접근 경합으로 CPU 대기 | 캐시·프리페치·버스 대역폭 확대 | 폰 노이만 병목 완화 |
| 수정 하버드 캐시의 코드·데이터 불일치 | 캐시 일관성·명령 캐시 무효화 절차 적용 | 자기 수정 코드·DMA 정확성 확보 |
| 분리 메모리 한쪽 공간 부족 | 수정 하버드 또는 재배치 가능한 메모리 구성 | 용량 활용성 향상 |
| 이론적 동시 접근이 실제 성능으로 이어지지 않음 | 캐시 적중률·버스 사용률·대기 주기 측정 | 실제 병목 위치 식별 |

### 쉽게 이해하기 (학습용)
- 메모리 구조 선택 뒤에도 캐시 적중률과 버스 경합을 측정해야 분리 경로의 실제 이득을 확인할 수 있다.
