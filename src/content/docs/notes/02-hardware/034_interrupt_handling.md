...
- **지연 처리(Deferred Processing)**: ISR에서는 긴급한 원인 해제만 하고 시간이 오래 걸리는 작업은 별도 커널 작업으로 넘겨 나중에 수행하는 방식
...
| 인터럽트 제어기 | 마스크·우선순위 기반 **수락 대상 중재** |
| 벡터·체인 식별 경로 | 벡터 번호·승인 순서로 **요청 원인 식별** |
| CPU·ISR 문맥 | PC·레지스터 저장과 **원인 처리·복귀** |
...
1. **① IRQ 수신**: 장치·타이머는 비동기 사건이 발생하면 인터럽트 제어기에 처리 요청을 보낸다.
2. **② 선택 IRQ·벡터 번호 전달**: 제어기는 마스크와 우선순위로 수락할 요청을 고르고 원인 식별 정보와 함께 CPU에 전달한다.
3. **③ ISR 주소 조회**: CPU는 벡터 번호 또는 체인 식별 결과를 사용해 실행할 서비스 루틴의 진입 주소를 조회한다.
4. **④ ISR 진입 주소 반환**: 벡터 식별 경로는 벡터 테이블에서 찾은 ISR 주소를 CPU에 반환한다.
5. **⑤ 문맥 저장·ISR 진입**: CPU는 명령 경계에서 PC와 필요한 레지스터를 저장한 뒤 ISR로 실행 흐름을 바꾼다.
6. **⑥ 인터럽트 원인 해제**: ISR은 장치 상태를 확인하고 IRQ가 반복 발생하지 않도록 원인 플래그나 요청 신호를 제거한다.
7. **⑦ EOI 전달**: ISR은 인터럽트 제어기에 현재 요청 처리가 끝났음을 알려 다음 대기 요청의 전달을 허용한다.
8. **⑧ 인터럽트 복귀 요청**: ISR의 인터럽트 복귀 명령에 따라 CPU는 저장한 문맥을 복원하고 중단된 명령 흐름으로 돌아간다.
...
- 많은 원인은 **벡터**, 단순 고정 순위는 데이지체인을 선택하고 ISR은 최소화
