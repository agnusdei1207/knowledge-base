...
- **① IRQ 수신**: 장치·타이머의 비동기 요청 접수
- **② 선택 IRQ·식별 정보 전달**: 마스크·우선순위로 고른 요청 전달
- **③ 원인 식별·ISR 주소 조회**: 벡터 번호 또는 체인 결과로 ISR 탐색
- **④ ISR 진입 주소 반환**: 식별된 서비스 루틴 주소 반환
- **⑤ 문맥 저장·ISR 진입**: PC·레지스터 보존 후 실행 전환
- **⑥ 인터럽트 원인 해제**: 장치 상태 확인과 요청 원인 제거
- **⑦ EOI 전달**: 제어기에 현재 처리 완료 통지
- **⑧ 인터럽트 복귀 요청**: 문맥 복원 후 중단 지점으로 복귀
