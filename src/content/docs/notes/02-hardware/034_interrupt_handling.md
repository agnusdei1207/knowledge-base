...
- ① **IRQ 수신**: 장치·타이머의 **비동기 사건** 접수
- ② **우선순위 중재**: 마스크와 우선순위로 **수락 요청** 선택
- ③ **ISR 주소 조회**: 벡터·체인 경로로 **처리 루틴** 식별
- ④ **문맥 저장·진입**: PC·레지스터 보존 후 **ISR 실행**
- ⑤ **인터럽트 원인 해제**: 장치 상태 확인과 **요청 신호** 제거
- ⑥ **문맥 복원 요청**: 저장 상태 복구와 **중단 지점** 복귀
