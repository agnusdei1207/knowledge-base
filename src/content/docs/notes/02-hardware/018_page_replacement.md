...
- 기존 한계: 가용 프레임이 모두 사용된 상태에서 임의 페이지를 퇴출하면 불필요한 디스크 I/O와 페이지 부재가 증가할 수 있다.
...
| 설계 요소 | 입력·상태 | 역할 |
|:---|:---|:---|
| 상주 프레임 집합 | 페이지·참조·더티 상태 | 현재 메모리 배치 제공 |
| 참조 이력 | 적재 순서·최근 시각·횟수·비트 | 재사용 가능성 판단 근거 제공 |
| 교체 선택기 | 정책·참조 이력 | 희생 페이지 결정 |
| 저장소 I/O | 희생·요청 페이지 | 더티 기록·요청 페이지 적재 |

> 요약: 참조 이력으로 희생을 선택하고 더티면 기록

### 도표안 B. 페이지 폴트와 교체 처리 시퀀스
...
sequenceDiagram
    ...
    O->>S: ⑨ 요청 페이지 읽기
    S->>F: ⑩ 선택 프레임에 페이지 적재
    O->>P: ⑪ 기존·신규 PTE와 프레임 매핑 갱신
    O-->>C: ⑫ 중단 명령 재시작
```

### 동작 원리

1. **① 비상주 페이지 폴트 전달**: CPU·MMU는 요청 페이지가 물리 메모리에 없음을 확인하고 운영체제의 페이지 폴트 처리기로 제어를 넘긴다.
2. **② 빈 프레임 조회**: 페이지 폴트 처리기는 상주 프레임 집합에서 즉시 사용할 수 있는 빈 프레임이 있는지 확인한다.
3. **③ 사용 가능한 프레임 할당**: 빈 프레임이 있으면 교체 없이 요청 페이지를 적재할 대상으로 할당한다.
4. **④ 정책·참조 이력으로 희생 선택 요청**: 빈 프레임이 없으면 선택한 OPT·FIFO·LRU·LFU·Clock 정책과 참조 이력을 교체 선택기에 제공한다.
5. **⑤ 희생 프레임 지정**: 교체 선택기는 정책 기준상 재사용 가능성이 가장 낮은 페이지의 프레임을 희생 대상으로 지정한다.
6. **⑥ 희생 페이지·더티 상태 반환**: 프레임 집합은 희생 페이지의 매핑과 수정 여부를 페이지 폴트 처리기에 반환한다.
7. **⑦ 수정 페이지 기록 요청**: 희생 페이지가 더티이면 최신 내용을 잃지 않도록 후면 저장소에 먼저 기록한다.
8. **⑧ 기록 완료 응답**: 후면 저장소는 수정 페이지의 영속 기록이 끝났음을 페이지 폴트 처리기에 알린다.
9. **⑨ 요청 페이지 읽기**: 비워 둔 프레임에 넣을 요청 페이지를 후면 저장소에서 읽는다.
10. **⑩ 선택 프레임에 페이지 적재**: 후면 저장소에서 반환된 페이지 데이터를 빈 프레임 또는 희생 프레임에 배치한다.
11. **⑪ 기존·신규 PTE와 프레임 매핑 갱신**: 운영체제는 퇴출 페이지를 비상주로 표시하고 새 페이지의 프레임 번호·상주 상태를 기록한다.
12. **⑫ 중단 명령 재시작**: 주소 변환이 가능해지면 페이지 폴트를 일으킨 명령을 다시 실행한다.
...
> 범용 운영체제는 참조 비트와 원형 포인터를 사용하는 Clock으로 LRU를 낮은 비용으로 근사한다.

### 쉽게 이해하기 (학습용)

- 폴트율과 더티 쓰기뿐 아니라 이력 관리 비용까지 함께 측정해 정책을 조정한다.
