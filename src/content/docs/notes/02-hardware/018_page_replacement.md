...
- 정의/개념: 요구 페이징 시스템에서 물리 메모리 프레임이 포화 상태일 때, 향후 접근 확률이 가장 낮은 **희생 페이지(Victim Page)**를 선택하여 축출하고 신규 가상 페이지를 적재하는 **페이지 교체 알고리즘(Page Replacement Algorithm)**.
- 배경/필요성: 잘못된 희생 페이지를 선택하면 가까운 미래에 동일 페이지를 다시 억세스하여 **페이지 폴트(Page Fault)**와 고비용 SSD 스왑 **I/O**가 폭증(Thrashing)하므로, 지역성 기반의 희생 알고리즘 필수 적용.
...
- **참조 지역성(Reference Locality)** 정보를 기반으로 향후 재참조 가능성이 가장 낮은 희생 페이지 선택.
- 알고리즘이 정교해질수록 **교체 예측력(Prediction Accuracy)**은 상향되나, 매 억세스마다 태그/시각을 업데이트하는 **갱신 비용(Update Overhead)**이 증가.
- 축출 대상이 **더티 페이지(Dirty Page)**일 경우, 단순 파기가 아닌 SSD 스왑 영역 저장 쓰기 I/O 오버헤드가 추가 수반됨.
...
| LFU 적용 시 초기 대량 접근 데이터가 퇴출되지 않는 현상 | 일정 시간 마다 카운터를 삭감하는 **카운터 감쇠(Counter Decay)** 연동 | 과거 참조 데이터 오염 방지 및 최신 지역성 반영 |
| Dirty Page 축출 시 동기식 디스크 I/O로 인한 파이프라인 정지 | 커널 차원의 **백그라운드 비동기 기록(Background Writeback)** 가동 | 페이지 교체 시 동기 I/O 대기시간 은닉 |
