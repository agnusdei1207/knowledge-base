...
date: "2026-08-05T11:37:40+09:00"
tags:
  - "notes-hardware"
weight: 18
extra:
...
- **I/O**: Input/Output, 저장장치에서 페이지를 읽거나 쓰는 처리
...
- **MMU**: Memory Management Unit, 주소 변환과 접근 권한 검사를 담당하는 장치
...
- **1. 빈 프레임 확인**: 즉시 사용할 물리 공간 판정
- **2. 희생 페이지 선택**: 참조 이력에 교체 정책 적용
- **3. 더티 페이지 기록**: 수정값을 후면 저장소에 보존
- **4. 요청 페이지 적재**: 후면 저장소에서 프레임으로 읽기
- **5. PTE•프레임 원자 갱신**: 새 매핑 확정
...
- **OPT**: Optimal, 미래에 가장 늦게 참조될 페이지를 퇴출하는 정책
- **FIFO**: First-In First-Out, 가장 먼저 적재된 페이지를 퇴출하는 정책
- **LRU**: Least Recently Used, 가장 오래 참조하지 않은 페이지를 퇴출하는 정책
- **LFU**: Least Frequently Used, 참조 횟수가 가장 적은 페이지를 퇴출하는 정책
...
- **PTE**: Page Table Entry, 물리 프레임 번호와 상주•권한 상태를 저장하는 항목
