...
sequenceDiagram
    ...
    O->>S: ⑨ 요청 페이지 읽기
    S->>F: ⑩ 선택 프레임에 페이지 적재
    O-->>C: ⑪ PTE·프레임 매핑 갱신·명령 재시작
```

| 설계 요소 | 입력·상태 | 역할 |
|:---|:---|:---|
| 상주 프레임 집합 | 페이지·참조·더티 상태 | 현재 메모리 배치 제공 |
| 참조 이력 | 적재 순서·최근 시각·횟수·비트 | 재사용 판단 근거 제공 |
| 교체 선택기 | 정책·참조 이력 | 희생 페이지 결정 |
| 저장소 I/O | 희생·요청 페이지 | 더티 기록·페이지 적재 |

> 요약: 참조 이력으로 희생을 선택하고 더티면 기록

**동작 원리**

- **① 비상주 페이지 폴트 전달**: 운영체제 폴트 처리기로 제어 전환
- **② 빈 프레임 조회**: 즉시 사용할 프레임 확인
- **③ 사용 가능한 프레임 할당**: 빈 프레임을 적재 대상으로 지정
- **④ 정책·참조 이력으로 희생 선택 요청**: 정책 기준과 이력 전달
- **⑤ 희생 프레임 지정**: 재사용 가능성이 낮은 프레임 선택
- **⑥ 희생 페이지·더티 상태 반환**: 희생 매핑과 수정 여부 반환
- **⑦ 수정 페이지 기록 요청**: 더티 페이지를 후면 저장소에 기록
- **⑧ 기록 완료 응답**: 수정 페이지 기록 완료 통보
- **⑨ 요청 페이지 읽기**: 적재할 페이지를 후면 저장소에서 조회
- **⑩ 선택 프레임에 페이지 적재**: 빈·희생 프레임에 데이터 배치
- **⑪ PTE·프레임 매핑 갱신·명령 재시작**: 변환 상태 갱신 후 원래 명령 재실행
