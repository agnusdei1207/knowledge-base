...
- **페이지 폴트•I/O**: 비상주 페이지 접근으로 운영체제 예외가 발생하고 저장장치에서 페이지를 읽거나 쓰는 처리
...
- **교체 예측력•갱신 비용**: 참조 이력이 미래 재사용을 구분하는 정확도와 그 이력을 매 접근마다 유지하는 처리 부담
...
- **CPU•MMU**: 명령을 실행하는 프로세서와 가상 주소 변환•접근 권한 검사를 담당하는 메모리 관리 장치
...
- **샘플링•카운터 감쇠**: 일부 시점의 참조만 기록하고 오래된 참조 횟수의 가중치를 낮춰 이력 비용과 과거 편향을 줄이는 방법
- **PTE**: 가상 페이지의 물리 프레임 번호와 상주•권한 상태를 저장하는 페이지 테이블 항목
...
- **LRU•Clock•LFU**: 각각 가장 오래 쓰지 않은 페이지, 참조 비트가 해제된 페이지, 참조 횟수가 가장 적은 페이지를 교체하는 정책
