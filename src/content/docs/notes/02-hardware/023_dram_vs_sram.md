...
- **DRAM·SRAM**: 전하 셀로 고밀도를 얻는 메모리와 래치 셀로 짧은 지연을 얻는 휘발성 메모리
- **휘발성**: 전원이 끊기면 저장한 값이 사라지는 성질

</details>

- 정의/개념: DRAM은 **전하 셀**로 고밀도를, SRAM은 **래치 셀**로 짧은 지연을 구현하는 휘발성 메모리
...
- **ECC**: 추가 검사 비트로 메모리 비트 오류를 검출하고 정정하는 기법
...
- **캐시 메모리**: CPU 가까이에서 재사용 데이터를 짧은 지연으로 제공하는 소용량 SRAM 계층
