...
- **① 행 활성 명령 전달**: 선택 행의 활성 명령 전달
- **② 선택 행 워드라인 활성**: 셀 커패시터를 비트라인에 연결
- **③ 셀 전하의 미세 전압 차 전달**: 전하가 만든 작은 전압 차 전달
- **④ 판독값으로 셀 전하 복원**: 증폭한 값으로 파괴된 전하 재기록
- **⑤ 열 선택 명령 전달**: 활성 행에서 출력할 열 지정
- **⑥ 선택 열 데이터 전달**: 행 버퍼의 선택 데이터를 입출력 회로로 이동
- **⑦ 데이터 버스로 읽기값 반환**: 선택 데이터를 제어기에 반환
