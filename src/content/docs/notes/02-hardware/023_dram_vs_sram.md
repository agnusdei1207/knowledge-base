...
date: "2026-07-28T12:47:13+09:00"
tags:
  - "notes-hardware"
weight: 23
extra:
...
| 설계 요소 | 입력·상태 | 역할 |
|:---|:---|:---|
| DRAM 1T1C 셀 배열 | 커패시터 전하 | 고밀도 비트 보관 |
| DRAM 감지·복원 회로 | 미세 전압 차 | 값 증폭·셀 전하 재기록 |
| SRAM 6T 셀 배열 | 교차 결합 래치 상태 | 비파괴 비트 유지 |
| SRAM 읽기·쓰기 회로 | 워드라인·비트라인 | 래치 상태 입출력 |
...
| 운영 위험 | 대응 | 기대 효과 |
|:---|:---|:---|
| DRAM 리프레시가 요청 처리를 중단 | 리프레시 분산과 온도별 주기 조정 | 지연 급증 완화 |
| 반복 행 전환으로 프리차지 비용 증가 | 주소 매핑·스케줄링으로 행 적중률 개선 | 유효 대역폭 증가 |
| SRAM 용량 증가로 면적·누설 전력 상승 | 캐시 크기·연관도와 전력 차단 정책 조정 | 성능·전력 균형 |
| 셀 오류가 데이터 손상으로 전파 | ECC와 오류율·온도 모니터링 적용 | 신뢰성 향상 |
