...
date: "2026-07-26T09:45:00+09:00"
tags:
  - "notes-hardware"
weight: 23
extra:
...
- **동적 랜덤 접근 메모리(Dynamic Random-Access Memory, DRAM)**: 커패시터에 담은 전하로 비트를 기억하며 새는 전하를 주기적으로 채워 줘야 하는 대신 셀이 작아 고밀도를 얻는 메모리
- **정적 랜덤 접근 메모리(Static Random-Access Memory, SRAM)**: 서로 물고 있는 래치로 비트를 붙들어 채워 줄 필요가 없고 읽기도 빠른 대신 셀이 큰 메모리
- **1T1C 셀(One Transistor One Capacitor Cell)**: ‘원 티 원 씨 셀’로 읽으며 접근 트랜지스터 하나와 커패시터 하나로 DRAM 비트 하나를 담는 구조
- **6T 셀(Six Transistor Cell)**: ‘식스 티 셀’로 읽으며 교차 결합 인버터 네 개가 래치를 이루고 접근 트랜지스터 두 개가 비트라인에 붙어 SRAM 비트 하나를 유지하는 구조
...
- **L1 캐시(Level 1 Cache)**: ‘엘 원 캐시’로 읽으며 프로세서 코어에 가장 가까워 지연이 가장 짧아야 하는 캐시 계층이고 SRAM으로 만든다
