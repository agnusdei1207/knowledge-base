...
1. **① 행 활성 명령 전달**: 메모리 제어기는 주소가 가리키는 행을 열도록 행 디코더에 활성 명령을 보낸다.
2. **② 선택 행 워드라인 활성**: 행 디코더는 해당 워드라인을 켜 1T1C 셀의 커패시터를 비트라인에 연결한다.
3. **③ 셀 전하의 미세 전압 차 전달**: 커패시터 전하가 비트라인의 기준 전압을 조금 바꾸고 이 차이가 감지 증폭기에 전달된다.
4. **④ 판독값으로 셀 전하 복원**: 감지 증폭기는 작은 차이를 0 또는 1 수준으로 증폭하면서 파괴된 셀 전하를 같은 값으로 다시 채운다.
5. **⑤ 열 선택 명령 전달**: 메모리 제어기는 활성화된 행에서 실제로 출력할 열 주소를 입출력 회로에 전달한다.
6. **⑥ 선택 열 데이터 전달**: 감지 증폭기 행 버퍼에서 열 주소에 해당하는 데이터가 입출력 회로로 이동한다.
7. **⑦ 데이터 버스로 읽기값 반환**: 입출력 회로는 선택 데이터를 데이터 버스에 실어 메모리 제어기에 반환한다.
