...
- 정의/개념: 커패시터 전하로 비트를 담는 **동적 램(DRAM)** 및 래치 상태로 담는 **정적 램(SRAM)** 기반 갈리는 **휘발성 메모리**
...
#### 한줄 요약
- **감지 증폭기** 여러 셀에 공유된다는 점이 DRAM 집적도의 실제 근거이며, 셀을 작게 만든 대가를 주변 회로가 대신 치르는 구조다.
...
분기 결과: DRAM은 읽는 행위 자체가 상태를 무너뜨려 복원과 **프리차지** 반드시 뒤따르지만, SRAM은 읽어도 상태가 그대로라 읽기 경로가 한 단계로 끝남

#### 한줄 요약
- **파괴적 판독** 결함이 아니라 셀을 최소로 줄인 대가이며, DRAM 타이밍 규격이 유독 복잡한 이유도 전부 이 복원 절차에서 파생된다.
...
#### 한줄 요약
- **Row Hammer** 기반 집적도를 계속 높인 결과 인접 셀 간섭이 임계를 넘어선 사례이며, 미세화의 이득이 신뢰성 비용으로 되돌아오는 전형을 보여 준다.
