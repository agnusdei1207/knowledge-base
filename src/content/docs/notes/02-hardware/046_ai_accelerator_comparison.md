...
- 정의/개념: 모델의 제어·병렬성·변경 주기와 시스템의 전력·지연·물량을 기준으로 실행 장치를 고르는 **AI 가속기 선택 체계**이다.
- 배경/필요성: 하나의 장치가 유연성·처리량·전력 효율·개발비를 모두 최적화할 수 없어 **워크로드별 선택을 필요로 한다**.

#### 한줄 요약

- AI 가속기는 모델의 연산 특성과 변경 주기, 전력·지연 한도, 배포 물량을 함께 비교해 선택한다.
...
- 실효 성능을 결정하는 **지원 연산•메모리 병목**이 핵심이다.
- 목표 부하에서 비교하는 **지연•처리량•전력**이 핵심이다.
- 수명주기 비용을 결정하는 **개발비•운영비•물량**이 핵심이다.
...
4. **전력·지연·물량 판정**: 저전력 단말은 NPU, 지연 변동 제한과 회로 변경은 FPGA, 고정 대량은 ASIC 후보로 분류한다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 이론 **정점 성능만 비교**해 실제 모델의 폴백·메모리 병목을 누락 | 동일 정확도·배치 조건에서 **실모델 종단 성능** 측정 | 같은 작업 기준의 **공정한 후보 비교** |
| 연산자·정밀도 지원이 부족해 **CPU·GPU 폴백 증가** | 전체 그래프의 **연산자·정밀도 지원 범위** 검증 | 실제 장치 실행 비율과 **가속 구간 확인** |
| 텐서 이동량이 링크·메모리 대역폭을 넘어 **연산기 유휴** | 계층별 전송량·대역폭과 **온칩 데이터 재사용률** 측정 | 데이터 이동 병목에 맞는 **장치·메모리 구성 선택** |
| 도구 비용·NRE·운영 전력·공급자 종속성을 빼서 **장기 비용 과소평가** | 개발부터 교체까지 **수명주기 비용** 산정 | 초기 가격과 운영비를 함께 반영한 **장기 경제성 확보** |
...
- **가변 병렬 학습**은 GPU, **저전력 추론**은 NPU, 대량 고정은 ASIC가 핵심이다.
