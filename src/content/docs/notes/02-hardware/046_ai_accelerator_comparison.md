...
> 요약: 런타임이 CPU·가속기·데이터 경로를 배치

**동작 원리**

- **① 장치별 실행 계획·입력 텐서 제출**: 지원 범위로 분할한 코드와 입력 등록
- **② 전처리·제어 흐름 실행**: CPU가 분기·변환·작업 제출 담당
- **③ 경계 텐서 전송**: 가속기 지원 구간의 데이터를 장치로 이동
- **④ 가속기 메모리에 데이터 공급**: 가중치·활성값을 실행 경로에 전달
- **⑤ GPU·NPU·FPGA·ASIC 결과 반환**: 선택 장치가 서브그래프를 처리
- **⑥ CPU 폴백 실행 요청**: 비지원·동적 연산을 CPU로 전환
- **⑦ 호스트 결과 반환**: 후속 구간용 폴백 결과 전달
- **⑧ 최종 결과·성능 상태 반환**: 종단 결과와 장치별 상태 제공
