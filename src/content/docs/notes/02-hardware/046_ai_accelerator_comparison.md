...
date: "2026-07-25T17:40:00+09:00"
tags:
  - "notes-hardware"
weight: 46
extra:
...
- **인공지능(Artificial Intelligence, AI)**: 학습한 모델로 분류·예측하며 가속기가 반복 행렬·텐서 연산을 빠르게 처리하는 기술
- **중앙 처리 장치(Central Processing Unit, CPU)**: 제어·분기와 비지원 연산 처리
- **그래픽 처리 장치(Graphics Processing Unit, GPU)**: 다수 스레드로 행렬 연산을 병렬 처리
- **신경망 처리 장치(Neural Processing Unit, NPU)**: 신경망 연산을 전용 배열에서 실행
...
- **정의/개념**: 연산·지연·전력·비용 기반 가속기 선택 체계
- **배경/필요성**: 장치별 상충 특성으로 워크로드별 선택 필요
...
- 정점 성능보다 실모델 적합성을 우선해 오선택을 줄인다.
- 초기 개발비와 운영비를 수명주기 총비용으로 평가한다.
...
| 변경 방식 | 소프트웨어 갱신 | 커널·소프트웨어 갱신 | 지원 연산 내 모델 갱신 | 비트스트림 재합성 | 실리콘 재설계 |
| 적용 기준 | 제어·분기·소규모 부하 | 학습·대규모 병렬 | 온디바이스 저전력 추론 | 회로 변경·결정적 지연 | 안정된 부하·대량 배포 |
| 주요 위험 | 병렬 처리량·전력 효율 | 메모리 대역폭·전력 | 지원 연산·폴백 | 자원·주파수·합성 시간 | NRE·개발 기간·재설계 비용 |
...
1. 대규모 모델 학습: GPU 데이터 병렬화
2. 스마트 카메라: NPU 온디바이스 추론
...
- CPU분기·GPU병렬·NPU단말·FPGA변경·ASIC양산
