...
- **워크로드•제약** 에 따라 CPU•GPU•NPU•FPGA•ASIC 선택
...
- **수명주기 비용(Lifecycle Cost)**: 개발•구매•전력•운영•교체 비용의 합이다.
...
- **1. 실모델 특성 측정**: 지원 연산•메모리 이동•정밀도 확인
- **2. 제어•병렬성 판정**: CPU와 병렬 가속 후보 구분
- **3. 모델 변경 주기 판정**: GPU와 전용 후보 구분
- **4. 전력•지연•물량 판정**: NPU•FPGA•ASIC 후보화
- **5. 종단 성능•수명주기 비용 검증**: 실측 후 최종 선택

#### 한줄 요약

- 제어•변경•전력•물량 축으로 후보화 후 실측 검증
...
- 제어는 CPU, 병렬은 GPU, 저전력은 NPU, 고정은 ASIC
