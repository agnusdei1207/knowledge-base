...
- **텐서 처리 장치(Tensor Processing Unit, TPU)**: ‘티피유’로 읽고 영문 머리글자를 딴 약어이며, Google의 신경망 행렬 연산 전용 AI 가속기
- **가속 선형 대수(Accelerated Linear Algebra, XLA)**: ‘엑스엘에이’로 읽고 영문 머리글자를 딴 약어이며, 연산 그래프를 TPU 실행 코드로 변환하는 컴파일러
- **텐서(Tensor)**: 다차원 수치 배열을 나타내는 데이터 객체
- **행렬 곱셈 장치(Matrix Multiply Unit, MXU)**: ‘엠엑스유’로 읽고 영문 핵심 글자를 딴 약어이며, 곱셈·누산 배열로 행렬 곱을 처리하는 장치
- **시스톨릭 배열(Systolic Array)**: 인접 연산기가 값·부분합을 전달하는 배열
- **벡터 유닛(Vector Unit)**: 행렬 외 벡터·활성화 연산을 처리하는 유닛
- **고대역폭 메모리(High Bandwidth Memory, HBM)**: ‘에이치비엠’으로 읽고 영문 머리글자를 딴 약어이며, 가중치·활성값을 공급하는 고대역폭 메모리
- **칩 간 연결망(Inter-Chip Interconnect, ICI)**: ‘아이씨아이’로 읽고 영문 머리글자를 딴 약어이며, TPU 칩 사이 집단 통신을 전달하는 연결망
...
- **행렬 처리 이득**이 컴파일·통신 비용보다 클 때 TPU 선택
