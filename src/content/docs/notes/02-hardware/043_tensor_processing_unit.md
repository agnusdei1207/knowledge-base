...
- **1. XLA 융합•타일링•샤딩 컴파일**: 장치 실행 계획 생성
- **2. 입력•가중치 HBM 배치**: 연산 피연산자 준비
- **3. HBM 피연산자 타일 공급**: MXU 데이터 경로에 전달
- **4. MXU•벡터 연산•결과 저장**: 행렬 곱과 후처리
- **5. ICI 집단 통신**: 칩별 부분 결과 합산•분배
...
- 행렬•Pod는 **TPU**, 가변 커널은 GPU, 단말은 NPU
