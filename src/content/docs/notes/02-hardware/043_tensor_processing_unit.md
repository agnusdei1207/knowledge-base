...
| 설계 요소 | 입력·상태 | 역할 |
|:---|:---|:---|
| XLA 컴파일러 | 연산 그래프·텐서 모양 | 융합·타일링·샤딩·코드 생성 |
| TPU 호스트·런타임 | 실행 코드·입출력·장치 상태 | 작업 제출·상태 관리 |
| MXU·벡터 유닛 | 행렬·벡터 피연산자 | 곱셈·활성화 연산 처리 |
| HBM·ICI 데이터 경로 | 텐서·부분 결과 | 데이터 공급·집단 통신 |

> 요약: XLA가 연산을 배치하고 HBM·ICI가 데이터 흐름을 잇는다

### 도표안 B. XLA 컴파일·TPU 실행·집단 통신 시퀀스

```mermaid
sequenceDiagram
    participant A as AI 프레임워크
    participant X as XLA 컴파일러
    participant R as TPU 호스트·런타임
    participant H as HBM
    participant C as MXU·벡터 유닛
    participant I as ICI

    A->>X: ① 연산 그래프·텐서 모양 전달
    X->>R: ② 융합·타일·샤딩 실행 코드 반환
    R->>H: ③ 입력·가중치 텐서 적재
    R->>C: ④ TPU 실행 코드 시작
    loop 컴파일된 텐서 타일
        H->>C: ⑤ 피연산자 타일 공급
...
1. **① 연산 그래프·텐서 모양 전달**: AI 프레임워크는 행렬 곱·활성화 등의 연산 그래프와 입력·가중치 텐서 모양을 XLA에 전달한다.
2. **② 융합·타일·샤딩 실행 코드 반환**: XLA는 중간 이동을 줄이도록 연산을 융합하고 MXU·HBM 크기에 맞게 타일링하며 다중 칩이면 샤딩한 실행 코드를 만든다.
3. **③ 입력·가중치 텐서 적재**: TPU 런타임은 실행 코드가 참조할 입력과 가중치를 각 장치의 HBM에 배치한다.
4. **④ TPU 실행 코드 시작**: 런타임은 컴파일된 명령을 MXU·벡터 유닛에 제출하고 장치 실행을 시작한다.
5. **⑤ 피연산자 타일 공급**: HBM은 컴파일 일정에 맞춰 행렬·벡터 피연산자 타일을 연산 유닛에 공급한다.
6. **⑥ 부분합·출력 타일 저장**: 시스톨릭 MXU는 인접 PE로 값과 부분합을 전달하며 계산하고 벡터 후처리 결과를 HBM에 저장한다.
7. **⑦ 칩별 부분 결과 집단 통신**: 모델이나 데이터가 여러 TPU에 샤딩되면 각 칩은 합산·분배할 부분 결과를 ICI에 보낸다.
8. **⑧ 합산·분배 결과 반환**: ICI는 칩 간 집단 통신을 수행해 각 TPU가 다음 연산에 필요한 결과를 반환한다.
9. **⑨ 실행 완료·출력 위치 통지**: 모든 타일과 필요한 집단 통신이 끝나면 연산 유닛은 런타임에 완료 상태와 출력 위치를 알린다.
10. **⑩ 결과 텐서 반환**: 런타임은 장치 오류와 상태를 확인하고 결과 텐서를 AI 프레임워크에 반환한다.
