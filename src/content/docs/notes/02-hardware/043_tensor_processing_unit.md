---
sidebar:
  order: 43
  label: "043. TPU 텐서 처리 장치 (Tensor Processing Unit)"
  badge:
    text: "기출 • 70%"
    variant: note
title: "TPU 텐서 처리 장치 (Tensor Processing Unit)"
date: "2026-08-10T10:00:00+09:00"
tags:
  - "notes-hardware"
weight: 43
extra:
  question_no: "043"
  source_status: "기출"
  source_history: "126회, 134회"
  priority: 70
  priority_note: "반복 기출, 행렬 가속과 다중 칩 확장의 핵심"
---

## Ⅰ. 개요

<details><summary>핵심 용어</summary>

- **텐서 처리 장치(Tensor Processing Unit, TPU)**: Google이 신경망의 대규모 행렬 연산을 효율적으로 처리하도록 설계한 전용 인공지능 가속기이다.
- **텐서(Tensor)**: 신경망의 입력과 가중치 및 중간 결과를 표현하는 다차원 수치 배열이다.
- **행렬 연산 전용 가속(Matrix-specialized Acceleration)**: 행렬 곱의 반복적인 데이터 이동과 계산을 전용 회로에 맞춰 처리하는 방식이다.

</details>

- 정의/개념: 시스톨릭 행렬 연산 장치와 전용 컴파일러로 신경망 텐서 연산의 **행렬 연산 전용 가속**을 수행하는 **TPU**이다.
- 배경/필요성: 범용 코어는 대규모 행렬 곱에서 명령 제어와 데이터 이동 비중이 크다.

#### 한줄 요약

- TPU는 시스톨릭 배열에서 피연산자와 부분합을 인접 처리 요소로 전달해 대규모 행렬 곱의 처리량과 전력 효율을 높인다.

## Ⅱ. 특징

<details><summary>핵심 용어</summary>

- **가속 선형 대수(Accelerated Linear Algebra, XLA)**: 연산 그래프를 융합•타일링•샤딩하여 TPU 실행 코드로 변환하는 컴파일러이다.
- **시스톨릭 배열(Systolic Array)**: 인접한 연산기들이 피연산자와 부분합을 규칙적으로 전달하며 행렬 곱을 처리하는 배열이다.
- **고대역폭 메모리(High Bandwidth Memory, HBM)**: TPU 연산 배열에 가중치와 활성값을 높은 전송률로 공급하는 메모리이다.
- **칩 간 연결망(Inter-Chip Interconnect, ICI)**: 여러 TPU 칩 사이에서 부분 결과와 집단 통신 데이터를 전달하는 전용 연결망이다.
- **행렬 곱셈 장치(Matrix Multiply Unit, MXU)**: 시스톨릭 배열로 곱셈•누산을 수행하는 TPU 핵심 연산 장치이다.

</details>

- 연산 융합•타일•샤딩을 결정하는 **XLA 컴파일**이 핵심이다.
- **시스톨릭 배열**로 구현한 **MXU**가 행렬 곱과 부분합을 전달한다.
- 단일 칩 처리량을 제한하는 **HBM 대역폭**, 다중 칩 처리량을 제한하는 **ICI 대역폭**이 핵심이다.

#### 한줄 요약

- 행렬 연산 배열의 활용률이 높아도 HBM 공급 대역폭이나 칩 간 통신이 병목이면 전체 성능이 제한된다.

## Ⅲ. 구조 및 구성요소

<details><summary>핵심 용어</summary>

- **벡터 유닛(Vector Unit)**: 활성화와 정규화처럼 행렬 곱 이외의 원소별•벡터 연산을 처리하는 장치이다.
- **텐서 처리 장치 런타임(Tensor Processing Unit Runtime, TPU 런타임)**: 컴파일된 작업을 장치에 제출하고 입출력과 상태 및 오류를 관리하는 실행 소프트웨어이다.

</details>

```text
[XLA 컴파일러] -- [TPU 호스트•런타임] -- [MXU•벡터 유닛] -- [HBM•ICI 데이터 경로]
```

선의 의미: 컴파일 계층, 호스트 제어, 텐서 연산 장치, 메모리•칩 간 연결이 결합된 정적 TPU 시스템 구조다.

| 구성요소 | 책임 |
|:---|:---|
| XLA 컴파일러 | 융합•타일링•샤딩 생성 |
| TPU 호스트•런타임 | 작업 제출•상태 관리 |
| MXU•벡터 유닛 | 행렬•활성화 연산 처리 |
| HBM•ICI 데이터 경로 | 데이터 공급•집단 통신 |

#### 한줄 요약

- XLA가 작업을 나누고 HBM과 ICI가 MXU•벡터 유닛에 데이터를 공급한다.

## Ⅳ. 흐름도

<details><summary>핵심 용어</summary>

- **연산 그래프(Computation Graph)**: 신경망의 연산자와 텐서 의존 관계를 노드와 간선으로 나타낸 구조이다.
- **타일링(Tiling)**: 큰 텐서를 장치의 메모리와 연산 배열 크기에 맞는 작은 블록으로 나누는 최적화 기법이다.
- **집단 통신(Collective Communication)**: 여러 칩이 부분 결과를 합산•분배•교환하는 다자간 통신이다.
- **인공지능(Artificial Intelligence, AI)**: 학습 모델을 이용해 인식•판단•생성 작업을 수행하는 기술이다.
- **XLA 실행 계획 생성**: 연산 그래프를 융합하고 타일·샤드별 실행 코드와 의존성을 만드는 단계이다.
- **입력·가중치 배치**: 피연산자를 각 TPU 칩의 HBM에 저장하는 단계이다.
- **피연산자 타일 공급**: HBM에서 MXU와 벡터 유닛으로 입력·가중치 블록을 전달하는 단계이다.
- **행렬·벡터 연산**: MXU의 행렬 곱과 벡터 유닛의 활성화·정규화를 수행하는 단계이다.
- **ICI 집단 통신**: 여러 TPU 칩의 부분 결과를 합산·분배해 최종 텐서를 만드는 단계이다.

</details>

```text
                  [연산 그래프•텐서 모양]
                              |
              1. XLA 융합•타일링•샤딩 컴파일
                              |
                 2. 입력•가중치 HBM 배치
                              |
             +--------------------------------+
             | 반복: 컴파일된 텐서 타일      |
             | 3. HBM 피연산자 타일 공급     |
             | 4. MXU•벡터 연산•결과 저장 |
             +--------------------------------+
                              |
                    [다중 칩 실행인가?]
                       /             \
                    [아니요]          [예]
                       |               |
                       |       5. ICI 집단 통신
                       |               |
                       +-------+-------+
                               |
                         [결과 텐서 반환]
```

### 동작 원리

1. **XLA 실행 계획 생성**: **AI** 연산 그래프를 융합하고 텐서를 타일·샤드로 나눠 장치별 실행 코드와 의존성을 생성한다.
2. **입력·가중치 배치**: 런타임이 컴파일 계획에 따라 피연산자를 각 TPU 칩의 HBM에 저장한다.
3. **피연산자 타일 공급**: HBM에서 MXU와 벡터 유닛이 처리할 입력·가중치 블록을 전달한다.
4. **행렬·벡터 연산**: MXU가 행렬 곱과 부분합 누적을 수행하고 벡터 유닛이 활성화·정규화 후 결과를 저장한다.
5. **ICI 집단 통신**: 다중 칩 실행이면 샤드별 부분 결과를 ICI로 합산·분배해 최종 텐서를 구성한다.

#### 한줄 요약

- XLA가 연산 그래프를 타일·샤드로 나누면 HBM이 MXU에 피연산자를 공급하고, 다중 칩 실행에서는 ICI가 부분 결과를 교환한다.

## Ⅴ. 종류 및 비교

<details><summary>핵심 용어</summary>

- **텐서 처리 장치 포드(Tensor Processing Unit Pod, TPU Pod)**: 여러 TPU 칩을 고속 연결망으로 묶어 하나의 대규모 학습 자원처럼 사용하는 시스템이다.
- **그래픽 처리 장치(Graphics Processing Unit, GPU)**: 프로그램 가능한 병렬 코어와 커널로 가변적인 병렬 연산을 처리하는 프로세서이다.
- **신경망 처리 장치(Neural Processing Unit, NPU)**: 단말에서 신경망 연산을 낮은 전력으로 실행하도록 설계한 전용 프로세서이다.
- **단일 명령 다중 스레드(Single Instruction, Multiple Threads, SIMT)**: GPU 워프의 활성 스레드에 공통 명령을 발행하는 실행 방식이다.
- **다중 칩 학습(Multi-chip Training)**: 모델이나 데이터를 여러 가속기에 분할하고 집단 통신으로 결과를 동기화하는 학습 방식이다.

</details>

| AI 가속기 | TPU | GPU | NPU |
|:---|:---|:---|:---|
| 적용 기준 | 대규모 행렬•**다중 칩 학습** | 가변 커널•범용 병렬 | 저전력 단말 추론 |
| 핵심 특징 | **TPU·XLA·시스톨릭 배열** | **GPU·SIMT** | **NPU**의 전용 처리 |
| 한계 | 재컴파일•폴백•**집단 통신** | 분기 발산•메모리 병목 | 연산자•메모리•도구 제약 |

> 요약: 행렬은 TPU, 가변 커널은 GPU, 단말은 NPU가 적합하다.

#### 한줄 요약

- 대규모 행렬과 텐서 처리 장치 포드 학습에는 TPU, 가변적인 범용 병렬 커널에는 GPU, 전력 제한이 큰 단말 추론에는 NPU가 적합하다.

## Ⅵ. 실무 고려사항 및 대책

<details><summary>핵심 용어</summary>

- **모양 버킷화(Shape Bucketing)**: 가변 길이 입력을 몇 개의 대표 텐서 모양으로 묶어 컴파일 결과의 종류를 제한하는 기법이다.
- **컴파일 캐시(Compilation Cache)**: 같은 연산 그래프와 모양으로 생성한 XLA 결과를 재사용하도록 저장하는 공간이다.
- **중앙 처리 장치(Central Processing Unit, CPU)**: 범용 명령과 TPU 미지원 연산을 실행하는 프로세서이다.
- **폴백(Fallback)**: TPU 미지원 연산을 CPU 같은 다른 장치에서 대신 실행하는 처리이다.
- **샤딩(Sharding)**: 모델이나 데이터를 여러 TPU 칩에 나누어 배치하고 병렬 처리하는 방식이다.
- **계산·통신 중첩**: 한 텐서의 계산을 수행하는 동안 다른 텐서의 집단 통신을 동시에 진행하는 기법이다.

</details>

| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 입력 텐서 모양이 계속 바뀌어 XLA 재컴파일 반복 | **모양 버킷화·컴파일 캐시**로 대표 모양의 실행 코드 재사용 | 작업 시작 지연 완화 |
| 미지원 연산이 CPU로 이동해 장치 경계 데이터 왕복 | 지원 연산을 사전 분석해 그래프를 재작성하고 **폴백** 구간 축소 | 전송·동기화 비용 감소 |
| HBM 공급 속도가 MXU 소비량보다 낮아 연산 배열 유휴 | 연산 융합·**타일링**·데이터 재사용 개선 | 외부 메모리 요청 감소 |
| 칩별 계산량이 다르거나 집단 통신이 몰려 ICI 정체 | 계산·통신량에 맞춘 **샤딩·계산·통신 중첩** | 다중 칩 확장 효율 향상 |

> 텐서 버킷화•XLA 캐시로 재컴파일 지연 감소

#### 한줄 요약

- 입력 모양을 버킷화하고 컴파일 결과를 재사용해 XLA 재컴파일 지연을 줄인다.

## Ⅶ. 결론

<details><summary>핵심 용어</summary>

- **대규모 행렬 연산(Large-scale Matrix Computation)**: 충분히 큰 행렬 곱을 반복하여 전용 배열의 처리량을 활용하는 작업이다.
- **가변 커널(Variable Kernel)**: 모델이나 작업에 따라 실행 코드와 제어 흐름이 자주 달라지는 병렬 연산 함수이다.
- **가속기 선택 기준**: 행렬 규모·다중 칩 학습·커널 가변성으로 TPU와 GPU를 고르는 기준이다.

</details>

- **가속기 선택 기준**에 따라 **대규모 행렬 연산·다중 칩 학습**에는 **TPU**, **가변 커널**에는 **GPU**를 선택한다.

#### 한줄 요약

- XLA 컴파일과 ICI 통신 비용을 상쇄할 만큼 대규모 행렬 연산이 반복될 때 TPU를 적용한다.
