...
> 요약: XLA가 연산을 배치하고 HBM·ICI가 데이터 흐름을 연결

**동작 원리**

- **① 연산 그래프·텐서 모양 전달**: 행렬·활성화 연산과 입력 모양 제출
- **② 융합·타일·샤딩 실행 코드 반환**: 이동·장치 자원에 맞춘 코드 생성
- **③ 입력·가중치 텐서 적재**: 각 장치 HBM에 데이터 배치
- **④ TPU 실행 코드 시작**: MXU·벡터 유닛에 컴파일 명령 제출
- **⑤ 피연산자 타일 공급**: HBM에서 연산 유닛으로 데이터 공급
- **⑥ 부분합·출력 타일 저장**: 시스톨릭 연산과 벡터 후처리 결과 저장
- **⑦ 칩별 부분 결과 집단 통신**: 샤딩된 결과를 ICI에 전달
- **⑧ 합산·분배 결과 반환**: 다음 연산에 필요한 집단 통신 결과 전달
- **⑨ 실행 완료·결과 텐서 반환**: 모든 타일·통신 완료 후 결과 반환
