...
date: "2026-07-25T17:34:00+09:00"
tags:
  - "notes-hardware"
weight: 40
extra:
...
- **주문형 반도체(Application-Specific Integrated Circuit, ASIC)**: 연산·정밀도·데이터 경로를 회로에 고정한 반도체
- **비반복 엔지니어링(Non-Recurring Engineering, NRE)**: 설계·검증·마스크의 일회성 초기 비용
- **손익분기 수량(Break-Even Volume)**: NRE를 칩당 단가 절감으로 회수하는 수량
- **처리 요소(Processing Element, PE)**: 행렬 연산 일부를 병렬 실행하는 단위
- **곱셈-누산(Multiply-Accumulate, MAC)**: 곱셈 결과를 부분합에 누적하는 연산
...
- **정의/개념**: AI 연산·메모리 경로를 고정 최적화한 전용 가속기
- **배경/필요성**: 범용 연산의 전력·지연 증가로 전용 회로 필요
...
- 회로 고정은 명령·재구성 비용을 없애 전력을 줄인다.
- NRE를 칩당 절감액으로 나눈 수량이 손익분기점이다.
...
| 판단 기준 | ASIC | FPGA | GPU |
|:---|:---|:---|:---|
| 핵심 특징 | 고정 전용 회로 | 재구성 가능한 게이트 | 명령 기반 병렬 코어 |
| 변경 범위 | 실리콘 재설계 | 비트스트림 재합성 | 소프트웨어·커널 갱신 |
| 적용 기준 | 안정된 연산·정밀도·대량 수요 | 변경 가능성·저지연·중간 수량 | 잦은 변경·학습·범용 병렬 |
| 주요 위험 | NRE·개발 기간·재설계 비용 | 자원·주파수·합성 시간 | 제어·명령·메모리 에너지 |
...
1. 클라우드 TPU는 행렬·저정밀 연산을 ASIC으로 가속
...
- 연산·정밀도가 안정되고 수요가 손익분기를 넘으면 ASIC 선택
