...
- **① 작업·텐서 주소·타일 크기 설정**: 연산과 입출력 위치·타일 크기 등록
- **② 외부 메모리 타일 읽기**: 다음 입력·가중치 타일 요청
- **③ 타일 데이터 반환**: 연속 데이터 블록 반환
- **④ 온칩 SRAM에 타일 적재**: 데이터플로우 위치에 피연산자 배치
- **⑤ 재사용 피연산자 전달**: 반복 사용할 값을 NoC에 공급
- **⑥ 여러 PE에 피연산자 멀티캐스트**: 같은 값을 필요한 PE에 동시 분배
- **⑦ 부분합·결과 타일 저장**: 병렬 MAC 결과를 SRAM에 유지
- **⑧ 최종 결과 타일 전달**: 누산 완료 결과를 DMA로 전달
- **⑨ 출력 텐서 기록**: 외부 메모리의 출력 주소에 기록
- **⑩ 가속 작업 완료 통지**: 모든 타일 처리 후 호스트에 알림
