...
- 정의/개념: AI 연산•정밀도•데이터 경로를 고정한 **ASIC 가속기**
- 배경/필요성: 범용 명령 처리•외부 데이터 이동으로 **전력•지연 증가**

#### 한줄 요약

- 고정 데이터 경로로 **명령•이동 오버헤드** 제거
...
> 칩당 절감액으로 **ASIC 손익분기 수량** 산정

#### 한줄 요약

- 전용화는 효율을 높이지만 **NRE•변경 비용** 증가
...
- **1. DMA 외부 타일 회수**: 입력•가중치 블록 전송
- **2. 온칩 SRAM 배치•재사용**: 외부 이동 반복 억제
- **3. NoC 멀티캐스트 분배**: 여러 처리 요소에 동시 전달
- **4. PE MAC•부분합 누적**: 처리 요소별 곱셈 결과 합산
