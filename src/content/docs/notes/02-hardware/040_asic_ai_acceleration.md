---
sidebar:
  order: 40
  label: "040. ASIC AI 가속 (ASIC AI Acceleration)"
  badge:
    text: "기출 • 70%"
    variant: note
title: "ASIC AI 가속 (ASIC AI Acceleration)"
date: "2026-08-08T18:40:00+09:00"
tags:
  - "notes-hardware"
weight: 40
extra:
  question_no: "040"
  source_status: "기출"
  source_history: "126회, 134회"
  priority: 70
  priority_note: "성능•전력 이득과 NRE 회수의 절충"
---

## Ⅰ. 개요

<details><summary>핵심 용어</summary>

- **주문형 반도체(Application-Specific Integrated Circuit, ASIC)**: 특정 용도에 맞춰 연산과 정밀도 및 데이터 경로를 고정하여 제작한 반도체이다.
- **전용 데이터 경로(Dedicated Data Path)**: 특정 연산의 데이터 이동과 처리를 위해 회로 연결을 고정한 실행 경로이다.
- **외부 데이터 이동(Off-chip Data Movement)**: 가속기와 외부 메모리 사이에서 입력•가중치•결과를 전송하는 동작이다.
- **인공지능(Artificial Intelligence, AI)**: 학습한 모델로 추론•인식 등 지능형 작업을 수행하는 기술이다.

</details>

- 정의/개념: 특정 AI 연산·정밀도·데이터 흐름에 맞춘 전용 회로를 제조 시점에 고정한 **ASIC** 가속기이다.
- 배경/필요성: 범용 프로세서는 명령 해석과 불필요한 외부 데이터 이동으로 추론 지연과 에너지를 증가시킨다.

#### 한줄 요약

- ASIC AI 가속기는 전용 데이터 경로와 온칩 재사용으로 명령 처리와 외부 데이터 이동 오버헤드를 줄인다.

## Ⅱ. 특징

<details><summary>핵심 용어</summary>

- **고정형 데이터플로우(Stationary Dataflow)**: 반복 사용하는 가중치•활성값•부분합 중 하나를 처리 요소 가까이에 유지하는 방식이다.
- **비반복 엔지니어링(Non-Recurring Engineering, NRE)**: ASIC 설계•검증•마스크의 초기 개발 비용이다.
- **손익분기 수량(Break-Even Volume)**: ASIC의 초기 개발 비용을 대안 대비 칩당 비용 절감액으로 회수하는 생산 수량이다.
- **온칩 재사용(On-chip Reuse)**: 가중치•활성값•부분합을 외부 메모리로 다시 보내지 않고 칩 내부에서 반복 사용하는 방식이다.

</details>

- 명령•재구성 오버헤드를 제거하는 **전용 데이터 경로**가 핵심이다.
- 외부 전송을 줄이는 **고정형 데이터플로우•온칩 재사용**이 핵심이다.
- **NRE** 회수 가능성을 가르는 **손익분기 수량**이 핵심이다.

$$
BreakEven\ Volume = \frac{NRE}{UnitCost_{alternative} - UnitCost_{ASIC}}
$$

> 대안보다 ASIC의 칩당 비용이 낮을 때 초기 개발비를 칩당 절감액으로 나누어 손익분기 수량 산정

#### 한줄 요약

- 전용 데이터 경로는 성능·전력 효율을 높이지만 초기 NRE와 모델 변경 시 재설계 비용을 증가시킨다.

## Ⅲ. 구조 및 구성요소

<details><summary>핵심 용어</summary>

- **처리 요소(Processing Element, PE)**: 행렬 연산의 일부를 병렬로 실행하는 가속기 내부의 연산 단위이다.
- **곱셈-누산(Multiply-Accumulate, MAC)**: 두 값을 곱한 결과를 기존 부분합에 더하는 행렬 연산의 기본 동작이다.
- **정적 임의 접근 메모리(Static Random-Access Memory, SRAM)**: 가중치와 부분합을 온칩에서 재사용하는 고속 메모리이다.
- **온칩 네트워크(Network on Chip, NoC)**: 피연산자와 결과를 처리 요소 배열에 분배하고 회수하는 칩 내부 연결망이다.
- **직접 메모리 접근(Direct Memory Access, DMA)**: 호스트 개입 없이 외부 메모리와 온칩 메모리 사이에서 타일을 전송하는 방식이다.

</details>

```text
ASIC AI 가속기
|
+-- [호스트•DMA 인터페이스]
+-- [온칩 SRAM•출력 버퍼]
+-- [NoC•멀티캐스트]
`-- [PE•MAC 배열]
```

선의 의미: 네 구성요소가 같은 ASIC 가속기 경계에 포함되어 전송•저장•분배•연산 책임을 나누는 정적 구조다.

| 구성요소 | 책임 |
|:---|:---|
| 호스트•DMA 인터페이스 | 작업 시작•타일 전송 |
| 온칩 SRAM•출력 버퍼 | 피연산자•부분합 재사용 |
| NoC•멀티캐스트 | 여러 PE에 데이터 분배 |
| PE•MAC 배열 | 행렬 곱셈•누산 실행 |

#### 한줄 요약

- DMA가 타일을 SRAM에 놓고 NoC가 재사용 데이터를 PE 배열에 분배한다.

## Ⅳ. 흐름도

<details><summary>핵심 용어</summary>

- **타일(Tile)**: 큰 텐서를 온칩 메모리 용량과 처리 요소 배열 크기에 맞게 나눈 데이터 블록이다.
- **멀티캐스트(Multicast)**: 하나의 가중치나 활성값을 여러 처리 요소에 동시에 분배하는 전송 방식이다.
- **부분합(Partial Sum)**: 행렬 곱에서 여러 곱셈 결과를 모두 더하기 전까지 누적 중인 중간값이다.
- **외부 타일 전송**: DMA가 입력·가중치 블록을 외부 메모리에서 온칩 SRAM으로 옮기는 단계이다.
- **온칩 배치·재사용**: 데이터플로우에 따라 피연산자와 부분합을 SRAM이나 PE 가까이에 유지하는 단계이다.
- **NoC 멀티캐스트 분배**: 하나의 피연산자를 여러 처리 요소에 동시에 전달하는 단계이다.
- **MAC·부분합 누적**: PE 배열이 곱셈-누산을 병렬 실행해 결과를 만드는 단계이다.

</details>

```text
                  [고정 AI 작업•데이터플로우]
                                |
              +----------------------------------+
              | 반복: 입력•가중치 타일         |
              | 1. DMA 외부 타일 회수           |
              | 2. 온칩 SRAM 배치•재사용       |
              | 3. NoC 멀티캐스트 분배          |
              | 4. PE MAC•부분합 누적          |
              +----------------------------------+
                                |
                      [최종 결과 기록•완료]
```

### 동작 원리

1. **외부 타일 전송**: DMA가 입력·가중치 블록을 외부 메모리에서 온칩 SRAM으로 이동한다.
2. **온칩 배치·재사용**: 데이터플로우에 따라 가중치·활성값·부분합을 SRAM이나 PE 가까이에 유지한다.
3. **NoC 멀티캐스트 분배**: 재사용할 피연산자를 여러 처리 요소에 동시에 전달해 중복 전송을 감소한다.
4. **MAC·부분합 누적**: PE 배열이 곱셈-누산을 병렬 실행하고 최종 결과를 출력 버퍼에 기록한다.

#### 한줄 요약

- DMA가 타일을 온칩 SRAM에 배치하면 NoC가 재사용 데이터를 PE 배열에 분배하고 부분합을 가까이 유지해 외부 메모리 접근을 줄인다.

## Ⅴ. 종류 및 비교

<details><summary>핵심 용어</summary>

- **필드 프로그래머블 게이트 배열(Field-Programmable Gate Array, FPGA)**: 제조 후 논리 기능과 내부 배선을 다시 구성할 수 있는 반도체이다.
- **그래픽 처리 장치(Graphics Processing Unit, GPU)**: 프로그램 가능한 다수의 병렬 코어로 범용 수치 연산을 수행하는 프로세서이다.
- **재설계 비용(Redesign Cost)**: 고정된 ASIC 기능이나 오류를 수정하기 위해 설계•검증•제조를 다시 수행할 때 발생하는 비용이다.
- **대량 수요(High-volume Demand)**: 초기 개발 비용을 칩당 비용 절감으로 회수할 만큼 충분한 생산 수량이 요구되는 조건이다.

</details>

| AI 가속기 | ASIC | FPGA | GPU |
|:---|:---|:---|:---|
| 적용 기준 | 안정된 연산•**대량 수요** | 변경 가능성•중간 수량 | 잦은 변경•범용 병렬 처리 |
| 핵심 특징 | **ASIC**의 고정 전용 회로 | **FPGA**의 재구성 회로 | **GPU**의 프로그램식 병렬 처리 |
| 한계 | NRE•개발 기간•**재설계 비용** | 자원•주파수•합성 시간 | 제어•명령•메모리 에너지 |

#### 한줄 요약

- 모델이 안정적이고 수요가 많으면 ASIC, 회로 수준 변경이 필요하면 FPGA, 모델·연산 변경이 잦으면 GPU가 적합하다.

## Ⅵ. 실무 고려사항 및 대책

<details><summary>핵심 용어</summary>

- **프로그래머블 제어(Programmable Control)**: 고정 데이터 경로를 유지하면서 명령 순서나 지원 연산을 소프트웨어로 조정할 수 있게 하는 제어 구조이다.
- **연산기 가동률(Compute Utilization)**: 전체 시간 가운데 처리 요소가 유효한 연산을 수행한 시간의 비율이다.
- **에뮬레이션(Emulation)**: 제작 전 ASIC 설계를 재구성 하드웨어에서 실행하여 기능과 성능 오류를 검증하는 방법이다.
- **실리콘 검증(Silicon Validation)**: 제조된 칩의 기능과 타이밍 및 전력•열 특성이 목표를 만족하는지 확인하는 절차이다.
- **처리 요소(Processing Element, PE)•텐서 처리 장치(Tensor Processing Unit, TPU)**: 병렬 연산 단위와 행렬•텐서 연산을 위한 전용 가속기이다.

</details>

| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 생산량이 적어 **NRE** 회수 실패 | 예상 물량과 단가 민감도로 **손익분기 수량** 재검증 | 회수 불가능한 설계 투자 방지 |
| 모델 구조·수치 형식이 바뀌어 고정 회로 조기 노후화 | **프로그래머블 제어**와 지원 연산 여유 반영 | 소프트웨어 조정 범위 확대 |
| 외부 메모리 공급이 늦어 PE 배열 유휴 시간 증가 | **고정형 데이터플로우·타일·온칩 재사용** 최적화 | **연산기 가동률** 향상 |
| 높은 전력 밀도나 설계 오류가 제조 후 발견되어 재설계 필요 | 열·전력 한도 검증과 **에뮬레이션·실리콘 검증** 강화 | 양산 전 오류 발견 |

> 반복 행렬•저정밀 연산을 **ASIC** 경로로 고정한 TPU

#### 한줄 요약

- 반복 행렬 연산은 데이터를 온칩에서 재사용해 전송 전력과 PE 대기를 줄인다.

## Ⅶ. 결론

<details><summary>핵심 용어</summary>

- **모델 안정성(Model Stability)**: 제품 수명 동안 인공지능 모델의 구조와 핵심 연산 요구가 크게 바뀌지 않는 특성이다.
- **전력 효율(Power Efficiency)**: 소비 전력 단위당 처리하는 연산량이나 완료하는 작업의 정도이다.
- **ASIC 선택 기준**: 모델 안정성·대량 수요·전력 효율 이득으로 ASIC 적용 여부를 정하는 기준이다.

</details>

- **ASIC 선택 기준**에 따라 **모델 안정성**과 **대량 수요**가 충분하고 **전력 효율** 이득이 크면 **ASIC**을 적용한다.

#### 한줄 요약

- 모델과 핵심 연산이 제품 수명 동안 안정적이고 예상 생산량이 손익분기 수량을 넘을 때 ASIC을 적용한다.
