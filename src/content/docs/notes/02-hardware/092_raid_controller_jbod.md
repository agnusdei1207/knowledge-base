...
- 배경/필요성: 단일 노드 캐시 가속과 클라우드 분산 확장 간의 **스토리지 계층별 책임 모델** 최적화 요구
...
| 패리티/복제 연산 | **전용 하드웨어 ASIC**이 XOR/RS 패리티 연산 전담 | 호스트 CPU 및 분산 **Erasure Coding** 연산 |
| 장애 복구 주체 | 컨트롤러 하드웨어가 백그라운드 리빌드 수행 | 분산 네트워크 기반 **병렬 데이터 복제** |
| 디스크 상태 모니터링 | 컨트롤러 펌웨어가 가상화 상태 보고 | 상위 SDS가 **S.M.A.R.T** 직접 감시 |
| 단일 장애점 (SPOF) | 컨트롤러 카드 고장 시 볼륨 전체 접근 불가 | 개별 노드 장애 시에도 **클러스터 서비스 지속** |
...
#### 한줄 요약
- 유형 판별 ➔ 캐시 기록/패스스루 ➔ 패리티/분산 복제 ➔ 디스크 영구 기록 순으로 처리된다.
...
- 워크로드 요구에 따라 **HW RAID**와 **분산 JBOD**를 선별 적용
