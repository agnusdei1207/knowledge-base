...
- 단일 노드 엔터프라이즈 환경에서는 **FBWC** 캐시 가속 기반 HW RAID가, 대규모 클라우드/소프트웨어 정의 스토리지(Ceph, ZFS)에서는 **원시 디스크 직접 제어 JBOD(IT 모드)**가 양대 축으로 정립되었으며, 최근에는 초고속 AI 데이터 파이프라인을 위해 NVMe-oF/CXL 기반 분산 컴포저블 올플래시(**JBOF**/EBOF)로 급속 전환
