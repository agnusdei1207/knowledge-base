...
- **BBU/Flash Backup Unit**: 불시 정전 시 HW RAID 컨트롤러 내 DRAM 쓰기 캐시(Write-Back) 데이터를 백업 전원으로 비휘발 덤프 보호하는 모듈.
- **HBA Passthrough Mode**: RAID 컨트롤러 칩셋 기능을 무력화하여 하드웨어 제어 없이 JBOD 직결 모드로 디스크를 1:1 패스스루하는 컨트롤러 모드.
...
선의 의미: 호스트가 HW RAID 엔진 및 보호 쓰기 캐시를 통해 LUN으로 접근하는 경로와, 패스스루 컨트롤러를 통해 물리 디스크를 직접 억세스하는 JBOD 경로의 아키텍처.
...
2. **보호 캐시 기록**: RAID 사용 시 **BBU 보호 캐시(Write-Back)** 기록 후 초저지연 완료 반환.
3. **데이터·패리티 배치**: HW **RAID Engine**을 이용한 패리티/미러 생성 후 물리 디스크 저장.
4. **개별 디스크 매핑**: JBOD 사용 시 **Passthrough Controller**를 통해 디스크 디바이스 1:1 노출.
5. **소프트웨어 사본 배치**: **SDS(Ceph, ZFS 등)** 가 노드/디스크 격리 정책에 기반하여 사본(Replica) 분산 저장.
...
| S.M.A.R.T 진단 | 컨트롤러에 차단되어 OS 직접 진단 제약 | OS/SDS가 각 디스크 **S.M.A.R.T** 상태 직접 실시간 수집 |
| 단일 장애점(SPOF) | RAID 컨트롤러 자체 고장 시 LUN 전체 마비 | **SDS** 분산 아키텍처로 특정 디스크/노드 고장 무중단 복구 |
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| **JBOD** 상에 하드웨어 Write-Back 캐시 부재로 성능 하락 | SDS 레인 상에 NVMe SSD를 Write Log(WAL/SLOG) 전용 배정 | SDS 입출력 성능 대폭 향상 |
| RAID 컨트롤러 고장 시 단일 장애점(**SPOF**)으로 LUN 억세스 마비 | **Dual RAID Controller** 및 Multipathing 이중화 구축 | 하드웨어 컨트롤러 장애 격리 |
| SDS 시스템에 HW RAID 중복 적용 시 이중 패리티 자원 낭비 | SDS 전용 인프라는 **HBA/Passthrough Mode (JBOD)** 고정 | 스토리징 용량 및 계산 낭비 소멸 |

> 사례: 대규모 클라우드 인프라 상의 **JBOD 패스스루** 및 **Ceph SDS** 분산 스토리지 구축

#### 한줄 요약

- 부트 볼륨에는 미러링과 전원 보호 쓰기 캐시를 적용한다.
...
- 중복성 및 장애 복구 관장 계층(HW vs SW/SDS)에 따른 RAID 컨트롤러/JBOD 패스스루 차등 구성을 위한 스토리지 구축 체계 적용.
