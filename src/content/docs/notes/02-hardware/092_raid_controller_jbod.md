...
- 배경/필요성: 스토리지 구축 시 단일 서버 기반의 고신뢰성 캐시 가속(HW RAID)과 클라우드/빅데이터 환경의 수평 확장성(JBOD/SDS) 간 **데이터 무결성·패리티 연산 및 복구 책임 계층의 명확한 아키텍처 분리** 필요
...
- 단일 노드 엔터프라이즈 환경에서는 **FBWC 캐시 가속 기반 HW RAID**가, 대규모 클라우드/소프트웨어 정의 스토리지(Ceph, ZFS)에서는 **원시 디스크 직접 제어 JBOD(IT 모드)**가 양대 축으로 정립되었으며, 최근에는 초고속 AI 데이터 파이프라인을 위해 **NVMe-oF/CXL 기반 분산 컴포저블 올플래시(JBOF/EBOF)**로 급속 전환
