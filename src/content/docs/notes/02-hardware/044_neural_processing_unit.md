---
sidebar:
  order: 44
  label: "044. NPU 신경망 처리 장치 (Neural Processing Unit)"
  badge:
    text: "기출 · 85%"
    variant: note
title: "NPU 신경망 처리 장치 (Neural Processing Unit)"
date: "2026-07-28T12:57:30+09:00"
tags:
  - "notes-hardware"
weight: 44
extra:
  question_no: "044"
  source_status: "기출"
  source_history: "126회, 134회, 135회, 137회, 138회"
  priority: 85
  priority_note: "다섯 회 반복, 온디바이스 AI 가속의 핵심"
---

## 미리 알고가기

- **신경망 처리 장치(Neural Processing Unit, NPU)**: 신경망 연산에 특화된 저전력 병렬 가속기
- **곱셈 누산(Multiply-Accumulate, MAC) 배열**: 곱셈·누산 연산기를 규칙적으로 배치하여 텐서 연산을 병렬 처리하는 배열
- **온칩 정적 임의 접근 메모리(On-Chip Static Random-Access Memory, On-Chip SRAM)**: 중간값을 가까이 보관해 외부 전송을 줄이는 고속 메모리
- **양자화(Quantization)**: 부동소수점 값을 저비트 정수로 근사
- **보정(Calibration)**: 대표 데이터로 양자화 범위와 정확도를 조정
- **연산자 지원 범위(Operator Coverage)**: NPU 코드로 변환 가능한 신경망 연산 범위
- **서브그래프 분할(Subgraph Partitioning)**: 지원·비지원 연산을 장치별 실행 구간으로 분리
- **폴백(Fallback)**: 비지원 연산을 CPU·GPU에서 실행
- **초당 조 연산(Tera Operations Per Second, TOPS)**: 특정 데이터 정밀도와 조건에서 장치가 수행할 수 있는 초당 최대 연산 수
- **중앙 처리 장치(Central Processing Unit, CPU)**: 동적 분기와 전후처리처럼 범용 명령 실행을 맡는 프로세서
- **그래픽 처리 장치(Graphics Processing Unit, GPU)**: 프로그램 가능한 병렬 코어로 가변 신경망 연산을 처리하는 프로세서
- **단일 명령 다중 스레드(Single Instruction, Multiple Threads, SIMT)**: 워프의 활성 스레드에 공통 명령을 발행하는 GPU 실행 모델
- **직접 메모리 접근(Direct Memory Access, DMA)**: 전용 엔진이 외부 메모리와 온칩 메모리 사이 데이터를 직접 전송하는 방식
- **온디바이스 추론(On-device Inference)**: 원격 서버로 보내지 않고 단말 안에서 모델 예측을 실행하는 방식
- **런타임(Runtime)**: 컴파일한 모델의 버퍼·실행 순서·장치 전환을 관리하는 실행 소프트웨어
- **희소성(Sparsity)**: 가중치·활성값 중 0인 값의 비율이며 지원 하드웨어는 0 연산을 생략할 수 있음
- **8비트 정수(Integer 8-bit, INT8)**: 값을 8비트 정수로 표현하여 메모리 사용량과 연산 비용을 줄이는 양자화 형식
- **종단 검증(End-to-End Validation)**: NPU와 폴백 구간을 모두 포함한 전체 모델의 정확도·지연·전력을 측정하는 검증
- **연산자 결합(Operator Fusion)**: 연속된 신경망 연산자를 하나의 장치 연산으로 묶어 중간 텐서 저장과 장치 전환을 줄이는 최적화

## Ⅰ. 개요

- 정의/개념: 신경망 **전용 연산 배열·근접 메모리** 기반 가속기
- 기존 한계: 범용 프로세서는 **단말 추론 전력·지연**에 한계

### 쉽게 이해하기 (학습용)

- 전용 로봇이 신경망 계산을 가까운 부품함과 함께 처리한다

## Ⅱ. 특징

- **저정밀 MAC 배열**로 신경망 연산 병렬 처리
- **온칩 SRAM 재사용**으로 외부 메모리 전송 절감
- **연산자 지원·폴백 비용**이 종단 성능 결정

### 쉽게 이해하기 (학습용)

- 로봇이 못하는 일을 직원에게 넘길수록 전달 시간이 커진다

## Ⅲ. 아키텍처

**도표안 A — 구조도**

```mermaid
flowchart LR
    C["모델 컴파일러"] --> R["NPU 런타임"]
    R --> N["MAC 배열·온칩 메모리"]
    R --> H["호스트 폴백 경로"]
    N -. "경계 전송·동기화" .-> H
```

**도표안 B — sequenceDiagram**

```mermaid
sequenceDiagram
    participant A as AI 애플리케이션·컴파일러
    participant R as NPU 런타임
    participant M as DMA·온칩 SRAM
    participant N as MAC 배열
    participant H as CPU·GPU 폴백

    A->>R: ① 실행 계획·입력 텐서·추론 요청
    R->>M: ② 입력 타일 DMA 적재
    loop 실행 계획의 서브그래프
        alt NPU 지원 연산
            R->>N: ③ NPU 장치 코드 실행
            M->>N: ④ 저정밀 텐서 타일 공급
            N-->>M: ⑤ 결과·부분합 타일 저장
        else 비지원 연산
            R->>H: ⑥ 경계 텐서·폴백 연산 전달
            H-->>R: ⑦ 호스트 연산 결과 반환
        end
    end
    R-->>A: ⑧ 최종 추론 결과 반환
```

| 설계 요소 | 입력·상태 | 역할 |
|:---|:---|:---|
| 모델 컴파일러 | 모델 그래프·보정 데이터 | 양자화·분할·장치 코드 생성 |
| NPU 런타임 | 서브그래프·버퍼·장치 상태 | 실행 순서·전송·동기화 관리 |
| MAC 배열·온칩 메모리 | 저정밀 텐서·부분합 | 병렬 연산·데이터 재사용 |
| 호스트 폴백 경로 | 비지원 연산·경계 텐서 | CPU·GPU 대체 실행 |

> 요약: 컴파일러가 지원 연산은 NPU, 나머지는 호스트에 배치

**동작 원리**

- **① 실행 계획·입력 텐서·추론 요청**: 양자화·분할한 계획과 입력 제출
- **② 입력 타일 DMA 적재**: SRAM과 MAC 배열 크기에 맞춰 전송
- **③ NPU 장치 코드 실행**: 지원 서브그래프의 저정밀 코드 시작
- **④ 저정밀 텐서 타일 공급**: 재사용 가중치·활성값을 MAC 배열에 전달
- **⑤ 결과·부분합 타일 저장**: 병렬 MAC 결과를 온칩 SRAM에 보관
- **⑥ 경계 텐서·폴백 연산 전달**: 비지원 연산을 호스트 형식으로 변환·전송
- **⑦ 호스트 연산 결과 반환**: CPU·GPU의 대체 실행 결과 반환
- **⑧ 최종 추론 결과 반환**: 모든 서브그래프의 동기화 후 결과 전달

### 쉽게 이해하기 (학습용)

- 런타임은 지원 연산을 NPU에서 실행하고 비지원 연산은 경계 텐서와 함께 CPU·GPU로 넘긴다

## Ⅳ. 종류 및 비교

| 신경망 실행 장치 | NPU | GPU | CPU |
|:---|:---|:---|:---|
| 적용 기준 | 저전력 **온디바이스 추론** | 가변 모델·**학습·범용 병렬** | 동적 분기·**전후처리** |
| 핵심 특징 | MAC 배열·**근접 SRAM** | SIMT 워프·**범용 커널** | 범용 코어·**캐시** |
| 한계 | 연산자·양자화·**폴백 제약** | 메모리 지연·**전력 소모** | 낮은 **신경망 처리량** |

> 요약: 저전력 추론은 NPU, 가변 병렬 연산은 GPU가 적합하다

### 쉽게 이해하기 (학습용)

- 작은 전력의 반복 추론은 NPU, 변하는 병렬 일은 GPU가 맞다

## Ⅴ. 실무 고려사항 및 대책

| 운영 위험 | 대응 | 기대 효과 |
|:---|:---|:---|
| 저정밀 양자화로 모델 정확도 저하 | 대표 보정 데이터와 계층별 오차·종단 정확도 검증 | 품질 손실 통제 |
| 비지원 연산 폴백으로 복사·동기화 증가 | 연산자 결합·대체와 지원 범위 사전 분석 | NPU 실행 구간 확대 |
| 외부 메모리 전송이 MAC 배열을 제한 | 타일링·온칩 재사용과 DMA 중첩 적용 | 실효 처리량 향상 |
| 단말 열·전력 한도로 지속 성능 저하 | 온도·전력 감시와 모델·주파수 단계 조정 | 배터리·지속 지연 안정화 |

> 화상회의 모델은 지원되는 전처리·신경망 연산을 결합해 NPU 서브그래프를 넓히고 CPU 왕복을 줄인다.

### 쉽게 이해하기 (학습용)

- 지원 연산을 결합해 NPU 서브그래프를 넓히면 CPU·GPU 폴백의 복사·동기화가 줄어든다

## Ⅵ. 결론

- 단말 추론을 위해 지원 연산·폴백을 검토하여 **NPU** 활용

### 쉽게 이해하기 (학습용)

- 로봇 담당 비율이 높고 직원에게 넘기는 비용이 작을 때 도입한다
