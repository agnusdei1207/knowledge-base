---
sidebar:
  order: 44
  label: "044. NPU 신경망 처리 장치 (Neural Processing Unit)"
  badge:
    text: "기출 · 85%"
    variant: note
title: "NPU 신경망 처리 장치 (Neural Processing Unit)"
date: "2026-07-25T17:38:00+09:00"
tags:
  - "notes-hardware"
weight: 44
extra:
  question_no: "044"
  source_status: "기출"
  source_history: "126회, 134회, 135회, 137회, 138회"
  priority: 85
  priority_note: "다섯 회 반복, 온디바이스 AI 가속의 핵심"
---

## 미리 알고가기

- **신경망 처리 장치(Neural Processing Unit, NPU)**: 신경망 연산에 특화한 저전력 병렬 가속기
- **곱셈 누산(Multiply-Accumulate, MAC) 배열**: 곱셈·누산 연산기를 규칙적으로 배치한 배열
- **온칩 정적 임의 접근 메모리(On-Chip Static Random-Access Memory, On-Chip SRAM)**: 중간값을 가까이 보관해 외부 전송을 줄이는 고속 메모리
- **양자화(Quantization)**: 부동소수점 값을 저비트 정수로 근사
- **보정(Calibration)**: 대표 데이터로 양자화 범위와 정확도를 조정
- **연산자 지원 범위(Operator Coverage)**: NPU 코드로 변환 가능한 신경망 연산 범위
- **서브그래프 분할(Subgraph Partitioning)**: 지원·비지원 연산을 장치별 실행 구간으로 분리
- **폴백(Fallback)**: 비지원 연산을 CPU·GPU에서 실행
- **초당 조 연산(Tera Operations Per Second, TOPS)**: 특정 정밀도·조건의 초당 최대 연산 수
- **중앙 처리 장치(Central Processing Unit, CPU)**: 동적 분기와 전후처리처럼 범용 명령 실행을 맡는 프로세서
- **그래픽 처리 장치(Graphics Processing Unit, GPU)**: 프로그램 가능한 병렬 코어로 가변 신경망 연산을 처리하는 프로세서
- **단일 명령 다중 스레드(Single Instruction, Multiple Threads, SIMT)**: 워프의 활성 스레드에 공통 명령을 발행하는 GPU 실행 모델
- **직접 메모리 접근(Direct Memory Access, DMA)**: 전용 엔진이 외부 메모리와 온칩 메모리 사이 데이터를 직접 전송하는 방식
- **온디바이스 추론(On-device Inference)**: 원격 서버로 보내지 않고 단말 안에서 모델 예측을 실행하는 방식
- **런타임(Runtime)**: 컴파일한 모델의 버퍼·실행 순서·장치 전환을 관리하는 실행 소프트웨어
- **희소성(Sparsity)**: 가중치·활성값 중 0인 값의 비율이며 지원 하드웨어는 0 연산을 생략할 수 있음
- **8비트 정수(Integer 8-bit, INT8)**: 값을 8비트 정수로 표현해 메모리와 연산량을 줄이는 양자화 형식
- **종단 검증(End-to-End Validation)**: NPU와 폴백 구간을 모두 포함한 전체 모델의 정확도·지연·전력을 측정하는 검증

## Ⅰ. 개요

- **정의/개념**: 신경망 전용 배열·근접 메모리 기반 가속기
- **배경/필요성**: 단말 추론 전력·지연 제약으로 전용 가속 필요

### 쉽게 이해하기 (학습용)

- 전용 로봇이 신경망 계산을 가까운 부품함과 함께 처리한다

## Ⅱ. 특징

- 지원 연산 비율·정확도·폴백 비용이 종단 효율을 좌우한다.
- TOPS는 정밀도·희소성·전력 조건과 함께 해석한다.

### 쉽게 이해하기 (학습용)

- 로봇이 못하는 일을 직원에게 넘길수록 전달 시간이 커진다

## Ⅲ. 아키텍처 및 구성요소

```mermaid
flowchart LR
    C["모델 컴파일러"] --> R["NPU 런타임"]
    R --> N["MAC 배열·온칩 메모리"]
    R --> H["호스트 폴백 경로"]
    N -. "경계 전송·동기화" .-> H
```

| 설계 요소 | 설명 |
|:---|:---|
| 모델 컴파일러 | 양자화·연산자 변환·그래프 분할과 장치 코드 생성 |
| NPU 런타임 | 버퍼·실행 순서·장치 간 전송과 동기화 관리 |
| MAC 배열·온칩 메모리 | 저정밀 연산·데이터 재사용·DMA 전송 경로 |
| 호스트 폴백 경로 | 비지원 연산을 실행하는 CPU·GPU 경로 |

> 요약: 컴파일러가 지원 연산은 NPU, 나머지는 호스트에 배치한다

### 쉽게 이해하기 (학습용)

- 분배표가 로봇 작업과 직원 대행을 나누고 관리자가 자료를 옮긴다

## Ⅳ. 원리 및 절차 흐름도

```mermaid
sequenceDiagram
    participant C as 컴파일러·런타임
    participant N as NPU
    participant H as CPU·GPU
    participant A as 애플리케이션
    C->>C: 양자화·그래프 분할
    alt 지원 연산
        C->>N: NPU 서브그래프 실행
    else 비지원 연산
        C->>H: CPU·GPU 폴백 실행
    end
    C->>C: 경계 전송·동기화
    C->>C: 종단 정확도·지연 검증
    C-->>A: 결과 반환
```

| 절차 | 설명 |
|:---|:---|
| 양자화·그래프 분할 | 보정 후 지원 연산을 NPU 구간으로 묶음 |
| NPU 서브그래프 실행 | 지원 서브그래프를 MAC 배열에서 실행 |
| CPU·GPU 폴백 실행 | 비지원 서브그래프를 CPU·GPU에서 실행 |
| 경계 전송·동기화 | 장치 경계의 형식 변환·복사·대기 처리 |
| 종단 정확도·지연 검증 | 전체 그래프의 정확도·지연·전력 확인 |
| 결과 반환 | 검증된 모델 결과를 애플리케이션에 전달 |

> 요약: 지원 구간은 NPU, 나머지는 폴백하고 종단 품질을 검증한다

### 쉽게 이해하기 (학습용)

- 로봇이 할 수 있는 일은 맡기고 나머지는 직원이 처리한 뒤 품질을 본다

## Ⅴ. 종류 및 비교

| 비교축 | NPU | GPU | CPU |
|:---|:---|:---|:---|
| 핵심 특징 | MAC 배열·근접 SRAM | SIMT 워프·범용 커널 | 범용 코어·캐시 |
| 제어 단위 | 연산자 그래프·저정밀 텐서 | 커널·스레드 | 범용 명령·분기 |
| 적용 기준 | 저전력 온디바이스 추론 | 가변 모델·학습·범용 병렬 | 동적 분기·전후처리 |
| 주요 위험 | 연산자·양자화·폴백 제약 | 메모리 지연·전력 소모 | 낮은 신경망 처리량 |

> 요약: 저전력 추론은 NPU, 가변 병렬 연산은 GPU가 적합하다

### 쉽게 이해하기 (학습용)

- 작은 전력의 반복 추론은 NPU, 변하는 병렬 일은 GPU가 맞다

## Ⅵ. 실무 사례

1. 화상회의: 지원 연산을 묶어 NPU 서브그래프 확대
2. 카메라 모델: 대표 데이터로 INT8 정확도 검증

### 쉽게 이해하기 (학습용)

- 로봇이 할 수 있는 일을 이어 붙여 직원과의 왕복을 줄인다
- 대표 사진으로 작은 숫자 부품이 품질을 지키는지 확인한다

## Ⅶ. 결론

- NPU는 지원 연산·정확도를 충족하고 폴백 비용이 낮을 때 쓴다

### 쉽게 이해하기 (학습용)

- 로봇 담당 비율이 높고 직원에게 넘기는 비용이 작을 때 도입한다
