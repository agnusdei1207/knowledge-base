...
sequenceDiagram
    ...
    A->>C: ① 모델 그래프·보정 데이터 전달
    C->>R: ② 양자화·서브그래프 실행 계획 반환
    A->>R: ③ 입력 텐서·추론 요청
    R->>M: ④ 입력 타일 DMA 적재
    loop 실행 계획의 서브그래프
        alt NPU 지원 연산
...
1. **① 모델 그래프·보정 데이터 전달**: 애플리케이션 개발 단계에서 모델과 대표 입력을 컴파일러에 제공해 장치 지원 여부와 양자화 범위를 분석한다.
2. **② 양자화·서브그래프 실행 계획 반환**: 컴파일러는 지원 연산을 INT8 등의 NPU 코드로 만들고 비지원 연산을 CPU·GPU 구간으로 나눈 실행 계획을 런타임에 제공한다.
3. **③ 입력 텐서·추론 요청**: 실행 시 애플리케이션은 입력 데이터와 컴파일된 모델의 추론 작업을 NPU 런타임에 제출한다.
4. **④ 입력 타일 DMA 적재**: 런타임은 외부 메모리의 입력·가중치를 MAC 배열과 온칩 SRAM 용량에 맞는 타일로 옮긴다.
5. **⑤ NPU 장치 코드 실행**: 현재 서브그래프가 지원 범위 안이면 런타임은 컴파일된 저정밀 장치 코드를 MAC 배열에 제출한다.
6. **⑥ 저정밀 텐서 타일 공급**: 온칩 SRAM은 반복 사용할 가중치·활성값을 MAC 배열에 가까운 경로로 공급한다.
7. **⑦ 결과·부분합 타일 저장**: MAC 배열은 곱셈·누산을 병렬 수행하고 중간값과 출력 타일을 온칩 SRAM에 저장한다.
8. **⑧ 경계 텐서·폴백 연산 전달**: 비지원 연산을 만나면 런타임은 NPU 결과를 호스트 형식으로 변환·동기화해 CPU나 GPU에 전달한다.
9. **⑨ 호스트 연산 결과 반환**: 호스트는 비지원 연산을 실행하고 다음 NPU 또는 호스트 서브그래프가 사용할 결과를 런타임에 반환한다.
10. **⑩ 최종 추론 결과 반환**: 모든 서브그래프가 끝나면 런타임은 장치 경계의 오류·동기화를 확인하고 최종 결과를 애플리케이션에 돌려준다.
