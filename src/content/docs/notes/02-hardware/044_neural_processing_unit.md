...
- **1. 실행 계획•입력 등록**: 서브그래프 순서와 입력 확정
- **2. DMA 입력 타일 배치**: 온칩 SRAM에 피연산자 전송
- **3. NPU 저정밀 실행**: 지원 구간을 MAC 배열에서 처리
- **4. 경계 텐서•폴백**: 비지원 구간을 CPU•GPU에서 처리
- **5. 결과 통합•다음 구간**: 장치별 출력을 실행 계획에 연결
