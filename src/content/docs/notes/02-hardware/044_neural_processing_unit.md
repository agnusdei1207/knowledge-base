...
- 정의/개념: **AI**의 저정밀 텐서 연산을 **전용 연산 배열**과 근접 메모리에서 처리하는 **NPU**이다.
- 배경/필요성: 범용 CPU·GPU는 단말의 배터리·열 한도에서 지속 추론할 때 전력과 응답 지연 제약을 받는다.
...
- 신경망 연산을 병렬 처리하는 **MAC 배열**이 핵심이다.
- 외부 메모리 전송을 줄이는 **온칩 SRAM** 재사용이 핵심이다.
- **TOPS** 보다 지원률•폴백을 포함한 **종단 지연**을 평가한다.

#### 한줄 요약

- 명목 TOPS 수치보다 모델 연산자의 하드웨어 직접 지원률, CPU·GPU 폴백 오버헤드, 전후처리를 합산한 종단 지연이 실제 성능을 결정한다.
...
- **실행 계획·입력 등록**: 서브그래프 순서와 장치 배치 및 입력 버퍼를 확정하는 단계이다.
- **입력 타일 배치**: DMA가 입력·가중치 타일을 온칩 SRAM으로 전송하는 단계이다.
- **NPU 저정밀 실행**: 지원 서브그래프를 MAC 배열과 근접 메모리에서 처리하는 단계이다.
- **경계 텐서·폴백 실행**: 비지원 연산의 중간값을 CPU·GPU로 보내 대체 실행하는 단계이다.
- **결과 통합·후속 실행**: 장치별 출력을 다음 서브그래프에 연결해 최종 결과를 만드는 단계이다.
...
1. **실행 계획·입력 등록**: **서브그래프 분할** 결과의 실행 순서, 장치 배치와 입력 버퍼를 확정한다.
...
| 핵심 특징 | **NPU·MAC 배열** | **GPU·SIMT** | **CPU**의 범용 제어 |
...
- **연산자 결합(Operator Fusion)**: 연속된 연산자를 하나의 장치 연산으로 묶어 중간 저장과 장치 전환을 줄이는 최적화 기법이다.
- **타일링(Tiling)**: 큰 텐서를 온칩 메모리와 연산 배열 크기에 맞는 블록으로 나누는 기법이다.
- **전송 중첩**: 현재 타일을 계산하는 동안 다음 타일을 DMA로 동시에 가져오는 기법이다.

</details>

| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 저정밀 **양자화** 오차가 누적되어 모델 정확도 저하 | 대표 데이터로 **보정**하고 원본 모델과 비교 | 허용 한도 안에서 품질 손실 통제 |
| 비지원 연산마다 장치를 바꿔 폴백 전송·동기화 비용 증가 | **연산자 지원 범위**를 분석하고 **연산자 결합** 적용 | 장치 전환 감소 |
| 외부 메모리 공급이 늦어 MAC 배열 유휴 | **타일링·전송 중첩** 적용 | 데이터 대기 감소 |
| 지속 실행으로 열·전력 한도에 도달해 주파수·성능 저하 | 온도·전력 감시와 주파수 제어 | 지속 지연 안정화 |

> **연산자 결합**으로 NPU 서브그래프와 CPU 왕복 개선
...
- **추론 장치 선택 기준**: 지원률·전력 한도·연산 가변성으로 NPU와 GPU를 고르는 기준이다.

</details>

- **추론 장치 선택 기준**에 따라 **지원률**이 높고 **저전력 추론**이 필요하면 **NPU**, **가변 연산**이 많으면 **GPU**를 선택한다.
