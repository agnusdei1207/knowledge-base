...
- 기존 한계: 고정 크기 단위는 논리 영역별 **경계·권한 표현** 한계
...
1. **① 세그먼트 번호**: CPU는 논리 주소의 세그먼트 번호를 세그먼트 테이블 색인으로 전달한다.
2. **② 세그먼트 기술자**: 세그먼트 테이블은 해당 영역의 기준 주소·한계·존재 비트·접근 권한을 보호 검사기에 제공한다.
3. **③ 오프셋·접근 종류**: CPU는 세그먼트 내부 오프셋과 읽기·쓰기·실행 중 요청한 접근 종류를 검사기에 전달한다.
4. **④ 기준 주소·오프셋**: 세그먼트가 존재하고 오프셋이 한계 안이며 권한이 맞으면 검증된 기준 주소와 오프셋을 주소 생성기로 보낸다.
5. **⑤ 물리 주소·접근 요청**: 주소 생성기는 기준 주소와 오프셋을 더해 물리 주소를 만들고 메모리에 접근한다.
6. **⑥ 데이터·완료 상태 반환**: 물리 메모리는 읽기 데이터 또는 쓰기 완료 상태를 CPU에 반환한다.
7. **⑦ 보호 예외 통보**: 존재·범위·권한 검사 중 하나라도 실패하면 물리 메모리에 접근하지 않고 CPU에 보호 예외를 발생시킨다.
...
- 논리 영역 보호를 위해 경계·단편화를 검토하여 **세그멘테이션** 활용
