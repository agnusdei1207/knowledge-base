...
date: "2026-07-28T12:45:30+09:00"
tags:
  - "notes-hardware"
weight: 19
extra:
...
| 설계 요소 | 입력·상태 | 역할 |
|:---|:---|:---|
| 세그먼트 테이블 | 세그먼트 번호·기술자 | 기준·한계·존재·권한 제공 |
| 보호 검사기 | 오프셋·기술자 | 존재·범위·권한 위반 판정 |
| 주소 생성기 | 검증된 기준 주소·오프셋 | 물리 주소 산출 |
...
| 운영 위험 | 대응 | 기대 효과 |
|:---|:---|:---|
| 가변 영역 배치로 외부 단편화 증가 | 세그먼트 내부 페이징과 빈 공간 사용률 관찰 | 연속 공간 요구 완화 |
| 스택·힙 성장 방향 충돌 | 성장 한계와 가드 영역을 설정 | 경계 침범 조기 차단 |
| 기술자 기준·한계·권한 설정 오류 | 최소 권한 적용과 경계값 접근 시험 | 보호 예외 정확성 확보 |
| 세그먼트-페이징 결합의 변환 비용 | TLB 활용과 기술자·페이지 표 지역성 개선 | 주소 변환 지연 감소 |

### 쉽게 이해하기 (학습용)

- 가변 경계를 안전하게 유지하면서 실제 배치는 페이징으로 분산해 단편화와 보호 비용을 조정한다
