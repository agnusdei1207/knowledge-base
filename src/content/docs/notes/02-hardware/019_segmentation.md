...
- **① 세그먼트 번호**: 논리 주소의 영역 번호 전달
- **② 세그먼트 기술자**: 기준·한계·존재·권한 제공
- **③ 오프셋·접근 종류**: 영역 내부 위치와 접근 유형 전달
- **④ 기준 주소·오프셋**: 존재·범위·권한 통과 주소 전달
- **⑤ 물리 주소·접근 요청**: 기준 주소와 오프셋을 더해 메모리 접근
- **⑥ 데이터·완료 상태 반환**: 읽기 데이터·쓰기 완료 반환
- **⑦ 보호 예외 통보**: 검사 실패 시 메모리 접근 없이 예외 발생
