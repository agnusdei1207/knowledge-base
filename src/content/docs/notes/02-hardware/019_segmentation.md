...
- 세그먼트 기술자에 포함된 **기준 주소(Base Address)**와 **한계(Limit)** 비트를 대조하여 영역별 **독립 성장(Independent Growth)**과 경계 범위를 하드웨어적으로 보호.
- 가상 주소 변환 시 별도의 다단계 메인 메모리 테이블 순회 없이 `Physical Address = Base Address + Offset` 공식을 적용하는 **단일 단계 변환(Single-Stage Translation)** 구동.
...
| **단편화 문제** | **외부 단편화(External Frag)** 발생 | **내부 단편화(Internal Frag)** 발생 | 내부 단편화만 일부 미세 발생 |
...
| 동적 스택 확장 시 힙 세그먼트 구역을 무단 덮어쓰는 메모리 오염 | 스택-힙 세그먼트 경계면에 접근 불가 **가드 영역(Guard Region)** 배치 | 스택 오버플로 발생 시 즉시 하드웨어 예외 트랩으로 세그먼트 보호 |
| 개발자 실수로 오프셋 범위 초과하여 타 영역 메모리 오작동 | **최소 권한(Least Privilege)** 및 컴파일러 **경계값 시험(Boundary Testing)** | General Protection Fault를 통한 오작동 원천 차단 |
| 세그먼트 테이블 + 페이지 테이블 2중 변환에 따른 주소 지연 | MMU 내부 **TLB** 캐싱 및 **표 지역성(Table Locality)** 극대화 | 2중 변환 지연을 1클록 변환으로 가속 |
