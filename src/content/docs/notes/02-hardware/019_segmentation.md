...
- 정의/개념: 주소 공간을 가변 길이 논리 영역으로 나누고 기준·한계·권한으로 변환하는 **메모리 관리 방식**
- 배경/필요성: 평면 주소 공간만으로는 코드·데이터·스택의 **영역별 경계와 권한 표현 곤란**
...
- 영역별 **한계·권한** 부여로 독립적인 크기와 보호 정책 지원
...
- **중앙 처리 장치(Central Processing Unit, CPU)**: 메모리 접근 명령을 실행하는 장치
...
3. **한계·권한 검사**: 오프셋이 한계 안에 있고 요청한 접근 유형이 허용되는지 확인
4. **물리 주소 산출**: 검사를 통과한 기준 주소와 오프셋을 더해 물리 주소 생성
...
> 요약: 세그멘테이션은 논리 경계를 표현하고, 페이징은 연속 물리 공간 요구를 제거
