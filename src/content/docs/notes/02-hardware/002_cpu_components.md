...
- **중앙처리장치(Central Processing Unit, CPU)**: 명령어를 인출·해독·실행하는 핵심 처리장치
- **산술논리연산장치(Arithmetic Logic Unit, ALU)**: 산술·논리·시프트 연산을 수행하는 장치
- **제어장치(Control Unit, CU)**: 명령어를 해독해 데이터 경로를 제어하는 장치
- **레지스터(Register)**: 주소·피연산자·결과·상태를 보관하는 CPU 내부 저장장치
- **레지스터 파일(Register File)**: 여러 범용 레지스터를 묶어 명령어가 지정한 피연산자를 읽고 연산 결과를 기록하는 저장 회로
- **버스(Bus)**: CPU 구성요소와 메모리 사이에서 주소·데이터·제어 신호를 실어 나르는 공용 배선 통로
- **명령어 사이클(Instruction Cycle)**: 명령어의 인출·해독·실행·기록 과정
...
- **주소 지정 방식(Addressing Mode)**: 명령어 비트에서 피연산자 값이나 피연산자가 있는 레지스터·메모리 주소를 찾는 규칙
...
- **메모리 인터페이스(Memory Interface)**: CPU의 명령어·데이터 요청을 캐시·주기억장치의 버스 거래로 변환하는 경로
...
sequenceDiagram
    ...
    participant S as 명령어 상태·PC·IR
    participant M as 메모리 인터페이스
...
- **① 명령어 주소 조회**: 제어장치가 프로그램 카운터에서 다음에 인출할 명령어 주소 확인
- **② 명령어 인출**: 메모리 인터페이스가 해당 주소의 명령어를 읽어 명령어 레지스터에 저장
- **③ 명령어 해독**: 제어장치가 연산 코드·피연산자 위치·주소 지정 방식을 분석해 제어 신호 생성
- **④ 피연산자 읽기**: 명령어가 지정한 레지스터에서 실행할 값 준비
- **⑤ 실행·결과 기록**: ALU가 산술·논리·시프트 연산을 수행하고 결과·상태를 레지스터에 기록
- **⑥ 다음 명령어 주소 갱신**: 순차 실행 또는 분기 결과에 맞춰 프로그램 카운터 변경
