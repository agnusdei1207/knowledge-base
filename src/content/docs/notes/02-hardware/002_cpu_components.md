...
|:---|:---|:---|
| **제어장치 (CU)** | 명령어 오퍼코드 해독 및 타이밍별 제어 신호 생성 | 프로그램 실행 서순 통제 및 하드웨어 구성 요소 동기화 |
| **레지스터 파일** | 범용 연산 피연산자 실시간 로드 및 결과 저장 | 다중 읽기 포트 구성을 통해 ALU 입력 전송 지연 극소화 |
| **특수 목적 레지스터** | **PC**(다음 주소), **IR**(현재 명령), **PSR**(상태 플래그) 관리 | 파이프라인 제어, 분기 판단 및 예외(Interrupt) 처리 기준 제공 |
| **산술논리장치 (ALU)** | 제어 신호 선택에 따른 2진 산술, 논리, 비트 시프트 실행 | 데이터 연산 처리의 핵심 원천 로직 수행 |
| **내부 버스** | 칩 내부 블록 간 높은 대역폭으로 피연산자와 연산 결과 전송 | 외부 시스템 버스 트래픽과 독립된 고속 데이터 패스 형성 |
...
- **CPU 구성 설계 기준(CPU Architecture Design Criteria)**에 근거하여 대상 **명령어 집합 아키텍처(ISA)**의 비트 폭과 파이프라인 깊이에 최적화된 레지스터 파일 포트 구성, ALU 병렬 연산 단위 배치, 데이터 경로의 타이밍 동기화 체계 적용 필수.

#### 한줄 요약
- 타깃 ISA 규격에 맞춰 레지스터 파일 읽기/쓰기 포트 수 및 데이터 경로 ALU 연산 비트 폭을 통합 최적화하는 구조 설계 체계 적용.
