...
- **중앙처리장치(Central Processing Unit, CPU)**는 명령어를 인출·해독·실행하고 그 결과를 기록하는 핵심 처리장치이다.
- **명령어 사이클**은 하나의 명령어를 인출한 뒤 해독·실행·기록하는 처리 과정이다.
- **명령 처리 병목**은 인출·해독·연산·저장·전송 단계 중 가장 긴 대기가 전체 처리량을 제한하는 상태이다.

</details>

- 정의/개념: **중앙처리장치(Central Processing Unit, CPU)**는 명령어를 인출•해독•실행하고 결과를 기록하는 핵심 처리장치
...
- **레지스터**는 중앙처리장치(Central Processing Unit, CPU)가 피연산자와 중간 결과를 고속으로 보관하는 내부 저장장치이다.
- **클록**은 CPU 내부 순차 회로가 상태를 갱신할 시점을 일정한 주기로 맞추는 신호이다.
- **데이터 흐름**은 피연산자와 연산 결과가 레지스터·산술논리장치(Arithmetic Logic Unit, ALU)·버스 사이를 이동하는 경로이다.
- **클록 엣지**는 순차 회로가 입력을 받아 내부 상태를 갱신하는 클록 신호의 변화 시점이다.

</details>

- **제어 흐름**과 **데이터 흐름**을 분리한 내부 경로 구성
- 피연산자를 **레지스터**에 상주시켜 메모리 왕복 감소
- **클록 엣지**를 기준으로 순차 회로의 상태 동기 갱신

#### 한줄 요약
- **중앙처리장치(Central Processing Unit, CPU)**는 **제어 경로**와 **데이터 경로**를 분리하고 **레지스터 상태**를 **클록**에 맞춰 갱신한다.
...
- **산술논리장치(Arithmetic Logic Unit, ALU)**는 피연산자에 산술·논리·시프트 연산을 수행하는 장치이다.
- **제어장치(Control Unit, CU)**는 명령어를 해독하여 데이터 경로의 동작 순서와 제어 신호를 생성하는 장치이다.
- **레지스터**는 주소·피연산자·연산 결과와 상태를 중앙처리장치(Central Processing Unit, CPU) 내부에 고속으로 보관하는 저장장치이다.
...
#### 한줄 요약
- **제어장치(Control Unit, CU)**가 실행 순서를 지시하고 **레지스터**와 **버스**가 공급한 값을 **산술논리장치(Arithmetic Logic Unit, ALU)**가 연산한다.
...
- **산술논리장치(Arithmetic Logic Unit, ALU)**는 레지스터 파일이 제공한 피연산자를 계산하고 결과와 상태 비트를 반환하는 실행 장치이다.
...
#### 한줄 요약
- **제어장치(Control Unit, CU)**는 제어 신호를 보내고 **레지스터·버스·산술논리장치(Arithmetic Logic Unit, ALU)**는 실제 피연산자와 결과를 저장·전송·연산한다.
...
- **실행 유닛**은 정수·부동소수점·주소 계산처럼 특정 연산을 담당하는 중앙처리장치(Central Processing Unit, CPU) 내부 회로이다.
- **산술논리장치(Arithmetic Logic Unit, ALU)·제어장치(Control Unit, CU)**는 각각 산술·논리 연산과 명령 해독·제어 신호 생성을 담당한다.
...
#### 한줄 요약
- **산술논리장치(Arithmetic Logic Unit, ALU)·제어장치(Control Unit, CU)·레지스터·버스**는 각각 **연산·제어·저장·전송 병목**을 만든다.
...
- **레지스터 할당**은 프로그램 값을 사용할 중앙처리장치(Central Processing Unit, CPU) 레지스터에 배치하는 컴파일러 작업이다.
- **산술논리장치(Arithmetic Logic Unit, ALU)**는 실행 유닛에서 산술·논리 연산을 수행하는 장치이다.
...
#### 한줄 요약
- **산술논리장치(Arithmetic Logic Unit, ALU) 실행 유닛 사용률**, **분기 실패**, **레지스터 스필**, **캐시 미스**와 **버스 대기**를 함께 측정해야 실제 병목 구성요소를 찾을 수 있다.
...
- **중앙처리장치(Central Processing Unit, CPU) 병목**은 연산·제어·저장·전송 경로 가운데 대기 시간이 가장 큰 구성요소가 전체 처리량을 제한하는 상태이다.
