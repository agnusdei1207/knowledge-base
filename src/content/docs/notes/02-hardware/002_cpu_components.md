...
date: "2026-07-25T15:25:00+09:00"
tags:
  - "notes-hardware"
weight: 2
extra:
...
- **중앙처리장치(Central Processing Unit, CPU)**: 작업 지시서를 한 장씩 꺼내 읽고 계산한 뒤 결과를 적어 두는 일을 쉬지 않고 되풀이하는 공장장 같은 장치로, 이 공장장 안에 계산기·지휘자·작업대·복도가 함께 들어 있다
- **산술논리연산장치(Arithmetic Logic Unit, ALU)**: 덧셈·비교·비트 이동 같은 실제 계산을 맡아 명령의 결과값을 만들어 내는 CPU 내부의 계산기
- **제어장치(Control Unit, CU)**: 인출한 명령을 해독해 어느 회로를 언제 켤지 제어 신호로 지시하는 CPU 내부의 지휘자
...
- **프로그램 카운터(Program Counter, PC)**: 다음에 인출할 명령어의 주소를 담고 있어 CPU가 어디까지 실행했는지를 가리키는 레지스터
- **명령어 레지스터(Instruction Register, IR)**: 방금 인출한 명령어를 담아 두어 제어장치가 해독할 대상을 고정해 주는 레지스터
...
- **인터럽트 서비스 루틴(Interrupt Service Routine, ISR)**: 인터럽트의 원인을 처리한 뒤 보관해 둔 문맥으로 되돌아가는 전용 처리 코드
...
- **기존 한계**: 구성요소 구분 없이는 실행·해독·전송 중 **병목 지점** 특정 불가
...
| 절차 | 설명 |
|:---|:---|
| 명령어 인출 | **PC**가 가리킨 명령을 **IR**로 적재 후 PC 증가 |
| 해독·피연산자 확보 | 연산 종류와 값 위치를 해석해 **피연산자** 확보 |
| 연산 실행 | **ALU**가 연산 수행 후 연산 상태 갱신 |
| 결과 기록 | 산출값을 목적 **레지스터**·메모리에 기록 |
...
2. 분기 잦은 워크로드는 **분기 예측기**로 파이프라인 정지 감소

### 쉽게 이해하기 (학습용)
- 서버용 CPU는 창고까지 다녀오는 데 수백 박자가 걸리므로 코어 옆에 작은 선반, 그 뒤에 조금 큰 선반을 여러 단으로 쌓아 두고 자주 쓰는 값을 미리 옮겨 둔다
- 조건문이 많은 프로그램에서는 갈림길마다 멈춰 서면 흐름이 끊기므로, 예측기가 이전에 갔던 쪽을 기억해 두었다가 먼저 그 길로 명령을 채워 넣고 틀렸을 때만 되돌린다

## Ⅶ. 결론

- **실행·해독·스필·전송** 대기 비교로 증설 자원 선택
