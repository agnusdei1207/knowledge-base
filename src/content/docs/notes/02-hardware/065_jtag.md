...
- **1. TAP 초기화·EXTEST 선택**: TAP 상태를 리셋하고 명령 레지스터에 외부 연결 시험 지정
- **2. 경계 스캔 경로 선택**: 칩 핀의 구동·수신 경계 셀을 직렬 데이터 경로로 연결
- **3. 시험 패턴 이동·출력**: 데이터 레지스터에 패턴을 적재해 PCB 연결망 구동
- **4. 수신값 캡처**: 반대편 경계 셀이 실제 입력 핀 값을 저장
- **5. 관측값 반환**: 예상 패턴과 직렬 관측값의 차이로 단선·단락 판정
