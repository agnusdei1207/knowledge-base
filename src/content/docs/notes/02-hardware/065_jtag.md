...
- 정의/개념: TAP 상태 머신과 직렬 경계 스캔 레지스터로 IC 핀과 PCB 연결을 구동·관측하는 **IEEE 1149.1 시험 인터페이스**이다.
- 배경/필요성: 고밀도 패키지·기판에서는 물리 탐침으로 내부 핀과 미세 배선에 접근하기 어려워 **비접촉 연결 시험을 필요로 한다**.
...
- IEEE 1149.1 경계 스캔은 PCB 연결 시험을 표준화하고, 코어 정지·레지스터 접근은 프로세서별 **디버그 확장 구현**을 별도를 확인한다.
...
3. **시험 패턴 이동**: TDI로 핀 구동 패턴을 Shift-DR 상태에서 직렬을 적재한다.
4. **시험 패턴 구동**: Update-DR 상태에서 출력 경계 셀이 패턴을 PCB 연결망에 적용한다.
5. **입력 캡처·직렬 출력**: 수신 경계 셀이 Capture-DR로 값을 저장하고 Shift-DR로 TDO에 내보내 예상 패턴과 비교한다.
...
- **안전 핀 마스크(Safe Pin Mask)**: 외부 시험(External Test, EXTEST) 중 구동하면 위험한 출력 핀을 시험 패턴 대상에서 제외하는 설정한다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 출하 장치의 TAP가 열려 있어 **메모리·코어 디버그 악용** | 인증된 정비 주체만 허용하고 **운영 모드에서 디버그 잠금** | 비인가 내부 접근 차단으로 **디버그 공격면 축소** |
| 디버그를 영구 비활성화해 **현장 장애 분석 불가** | 인증·감사와 자동 만료가 있는 **시간 제한 서비스 모드** | 평상시 잠금과 승인 정비를 병행해 **보안·정비성 절충** |
| 실제 IC 체인 순서가 도구 설정과 달라 **시험 대상 오인** | 각 장치의 **IDCODE와 BYPASS 길이·순서 검증** | 스캔 비트와 IC 대응을 맞춰 **대상 식별 정확도 향상** |
| EXTEST에서 두 출력이 충돌하거나 위험 핀을 구동해 **보드 손상** | 전원·리셋·고전류 출력의 **안전 핀 마스크와 패턴 검토** | 위험 출력 제외로 **경합·장비 손상 방지** |
...
- 생산은 **EXTEST**, 출하 후는 **JTAG 인증•잠금**을 적용한다.
