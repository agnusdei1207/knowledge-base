...
date: "2026-07-25T10:34:00+09:00"
tags:
  - "notes-hardware"
weight: 65
extra:
...
- **정의/개념**: TAP로 IC·PCB 연결을 검사하는 직렬 포트다
- **배경/필요성**: 탐침 없이 고밀도 기판 결함을 찾는다
...
- IC 직렬 체인은 한 포트로 검사해 시험 비용을 줄인다
- 경계 셀은 코어 없이 핀을 구동해 납땜 결함을 찾는다
- 1149.1은 CPU 디버그를 보장하지 않아 구현을 확인한다
...
| 판단 기준 | JTAG | SWD | UART |
|:---|:---|:---|:---|
| 핵심 특징 | 직렬 경계 스캔·다중 IC 시험 | 2선 Arm 코어·메모리 디버그 | 비동기 로그·콘솔 입출력 |
| 적용 기준 | PCB 연결·다중 IC 시험 | 적은 핀의 Arm 디버그 | 단순 로그·명령 채널 |
| 주요 위험 | 시험·디버그 경로 노출 | 코어·메모리 접근 노출 | 콘솔 명령·정보 노출 |
...
1. PCB 양산: EXTEST로 납땜 연결을 확인한다
2. 양산 SoC: 수명주기로 JTAG 권한을 제한한다
