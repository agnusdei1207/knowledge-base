...
- **BMC(Baseboard Management Controller)**: OS 구동 여부와 독립적으로 센서, 전원, IPMI 텔레메트리를 제어하는 임베디드 SoC.
...
| 전원•냉각 계층 | 랙 PSU 전력 분배 및 공랭·수랭 기반 발열 방출 |
...
5. GPU 가속: GPU 텐서 코어에 데이터를 공급하고 병렬 연산 수행.
...
| 한계 | 텐서 병렬 연산 처리량 한계 | 높은 **랙 전력 밀도**와 냉각 설비 부담 |
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 원격 소켓 메모리 접근으로 **NUMA** 지연 증가 | **NUMA 친화도** 기반 vCPU Pinning 및 로컬 메모리 할당 | 불필요한 소켓 간 통신 감소 |
| 가속기 및 **NVMe** 트래픽 동시 유입 시 **PCIe 루트 포트** 포화 | PCIe 레인 바이퍼케이션 및 레인 스위칭 분산 | I/O 대역폭 병목 완화 |
| 초고밀도 서버 인가 시 랙 전력 한도 초과 및 스로틀링 | **전력 상한** 지정 및 직접 수랭(D2C) 도입 | **열 스로틀링** 완화 및 랙 전력 안정화 |
...
- 범용 서비스는 균형형 서버, AI·HPC는 가속 서버를 선택한다.
