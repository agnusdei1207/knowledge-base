...
- 정의/개념: 연산·메모리·스토리지·네트워크 자원을 전력·냉각·관리 계층과 함께 구성하는 **데이터센터 서버 시스템 구조**이다.
- 배경/필요성: 특정 자원만 증설하면 다른 경로가 병목이 되고 공통 전원·랙 장애는 그대로 남아 **종단 성능·가용성 개선 한계**가 발생한다.
...
- **열 스로틀링(Thermal Throttling)**: 온도 한도를 지키도록 CPU•GPU 처리량을 낮추는 제어한다.
- **관리망(Management Network)**: 서비스 데이터망과 분리하여 서버 전원•콘솔•펌웨어의 원격 관리에 사용하는 네트워크이다.

</details>

- 연산•메모리•**I/O 자원 균형 붕괴** 시 최저 처리량 자원이 종단 병목이 핵심이다.
- **전력•냉각 한도** 초과 시 스로틀링•중단 발생이 핵심이다.
- **관리망•장애 범위 분리**로 원격 복구와 가용성을 확보한다.
...
- **입출력(Input/Output, I/O)**: 호스트와 메모리•스토리지•네트워크 장치 사이의 데이터 교환한다.
...
5. **가속 처리·응답 반환**: GPU에 데이터를 공급해 AI·HPC 연산을 수행하고 결과를 CPU·NIC 경로로 반환한다.
...
> 요약: 범용은 **자원 균형**, 가속 서버는 **병렬 연산** 우선이 핵심이다.
...
- **비균일 메모리 접근 친화도(Non-uniform Memory Access Affinity, NUMA Affinity)**: 작업과 메모리 및 입출력(Input/Output, I/O) 장치를 가까운 프로세서 소켓에 함께 배치하는 설정한다.
...
| 가속기·NVMe 요청이 같은 루트 포트에 몰려 **PCIe 대역폭 포화** | **루트 포트·스위치 토폴로지별 처리량**을 검증하고 장치 분산 | 공유 상향 링크 부하 감소로 **I/O 병목 완화** |
| 서버 배치가 랙 전력·냉각 한도를 넘어 **열 스로틀링·전원 중단** | **전력 상한·냉각 여유** 감시와 고전력 장비의 랙 분산 | 열·전력 한도 안에서 **지속 성능·가용성 확보** |
| BMC 침해나 공통 전원·랙 장애가 여러 서버에 **동시 영향** | **관리망 격리·펌웨어 서명**과 전원·랙 장애 도메인 분산 | 관리 권한 보호와 상관 장애 제한으로 **복원력 향상** |

> 요약: **vCPU•NUMA 위치**와 **물리 코어 비율** 공동을 조정한다.
...
- 가상화•웹 워크로드에는 **범용 서버**, AI•HPC 워크로드에는 **가속 서버**를 선택한다.
