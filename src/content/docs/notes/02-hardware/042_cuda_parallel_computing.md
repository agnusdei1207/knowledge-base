...
- 호스트가 GPU 커널을 구성해 **대규모 데이터 병렬성** 실행
...
- **2. 비동기 H2D 입력 복사**: 장치 메모리에 데이터 적재
- **3. 커널•그리드•블록 실행**: 병렬 함수와 작업 크기 적용
- **4. 이벤트 의존성 확인**: 후속 복사가 가능한 완료 상태 판정
- **5. 비동기 D2H 결과 복사**: 호스트 결과 버퍼 갱신
