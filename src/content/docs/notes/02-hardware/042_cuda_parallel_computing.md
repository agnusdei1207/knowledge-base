...
- 정의/개념: **호스트**가 GPU 측 **장치**의 스레드 계층·메모리·스트림을 제어해 병렬 커널을 실행하는 **CUDA** 플랫폼이다.
- 배경/필요성: CPU의 소수 스레드만으로는 많은 데이터 원소의 동일 연산을 동시에 처리하기 어렵다.
...
- **스트림 중첩**: 서로 독립적인 스트림의 복사와 커널 실행을 동시에 진행하는 방식이다.
...
- **스트림·의존성 등록**: 복사와 커널 실행 순서 및 스트림 간 선행 이벤트를 지정하는 단계이다.
- **비동기 H2D 입력 복사**: 복사 엔진이 호스트 입력을 장치 메모리로 옮기는 단계이다.
- **커널 실행**: 지정한 그리드·블록·스레드 구성으로 병렬 함수를 수행하는 단계이다.
- **완료 이벤트 기록·대기**: 커널 완료를 표시해 후속 작업의 의존성을 해제하는 단계이다.
- **비동기 D2H 결과 복사**: 장치 결과를 호스트 버퍼로 돌려주는 단계이다.
...
2. **비동기 H2D 입력 복사**: 복사 엔진이 고정 호스트 메모리의 입력을 장치 메모리에 DMA로 전송한다.
...
| 핵심 특징 | **CUDA**의 그리드•블록•스레드 | **SYCL·ND-range** | **CPU 멀티스레딩** |
...
- **공유 메모리(Shared Memory)**: 한 스레드 블록이 반복 사용할 데이터를 SM 가까이에 두는 온칩 메모리이다.

</details>

| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 불필요한 **전역 동기화**가 복사·커널 실행을 직렬화 | 실제 선행 조건만 **스트림·이벤트**로 표현 | 독립 복사와 커널 동시 실행 |
| 호스트·장치 전송 시간이 커널 연산 시간을 넘어 전송 병목 | **고정 호스트 메모리·H2D·D2H** 전송을 중첩하고 요청 배치 | 중간 복사와 호출 횟수 감소 |
| 비동기 커널 오류가 나중 동기화 지점에서 보여 원인 식별 곤란 | 실행 API 오류 즉시 확인과 이벤트·동기화 경계별 오류 상태 기록 | 실패한 스트림·커널 위치 식별 |
| 블록당 레지스터·공유 메모리 사용이 커서 점유율 또는 대역폭 저하 | **점유율·연산 집약도**를 함께 측정해 블록·타일 크기 조정 | SM 자원 활용 개선 |
...
- **병렬 플랫폼 선택 기준**: 세부 최적화 이득과 공급자 종속·이식성으로 CUDA와 SYCL을 고르는 기준이다.

</details>

- **병렬 플랫폼 선택 기준**에 따라 NVIDIA **세부 최적화**가 중요하면 **CUDA**, 다중 제조사 **이식성**이 중요하면 **SYCL**을 선택한다.
