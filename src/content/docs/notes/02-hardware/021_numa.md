...
date: "2026-08-02T10:41:00+09:00"
tags:
  - "notes-hardware"
weight: 21
extra:
...
- **NUMA**: 메모리를 노드별로 분산해 로컬과 원격 접근 지연이 다른 다중 프로세서 구조
...
- **로컬·원격 메모리**: 실행 CPU와 같은 노드의 메모리와 다른 노드에 연결된 메모리
- **ccNUMA**: 접근 지연은 노드별로 다르지만 공유 주소 공간의 캐시 일관성을 유지하는 구조
...
- **메모리 제어기·채널**: 주소·명령 타이밍을 관리하고 노드의 DRAM과 데이터를 전송하는 구성요소
- **NUMA 인지 운영체제**: 스레드 실행 위치와 물리 페이지 배치를 함께 관리하는 운영체제
- **페이지 마이그레이션**: 자주 접근하는 CPU에 가까운 노드로 물리 페이지를 옮기는 동작
- **DRAM**: 메모리 셀을 주기적으로 재충전하며 시스템의 주기억장치로 사용하는 동적 임의 접근 메모리
...
- **UMA**: 모든 프로세서가 같은 지연으로 하나의 공유 메모리에 접근하는 구조
...
- **워커·버퍼 풀 지역화**: 질의 실행 스레드와 자주 쓰는 데이터 페이지를 같은 노드에 두는 배치
- **병렬 초기화**: 실제 처리 스레드들이 각자 사용할 페이지를 처음 접근해 여러 NUMA 노드에 고르게 배치하는 방법
- **마이그레이션 임계치·주기**: 페이지 이동을 시작할 원격 접근량 기준과 접근 패턴을 다시 평가하는 시간 간격
...
- **용량·대역폭 확장**: 노드를 추가해 메모리 총용량과 독립 채널의 처리량을 함께 늘리는 효과
- **일관 지연**: 어느 프로세서에서 접근해도 메모리 응답 시간이 같은 특성
- **NUMA·UMA**: 각각 노드 위치에 따라 접근 지연이 다르거나 모든 프로세서의 메모리 접근 지연이 같은 구조
