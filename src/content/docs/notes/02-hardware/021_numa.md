...
- **① CPU 친화도에 따라 스레드 배치**: 페이지 위치와 가까운 CPU 선택
- **② 물리 주소 메모리 요청**: 로컬 메모리 제어기에 주소 전달
- **③ 로컬 메모리 접근·데이터 반환**: 직접 연결된 메모리에서 응답
- **④ 원격 메모리 요청 전달**: 원격 요청을 인터커넥트에 전달
- **⑤ 홈 노드 제어기로 라우팅**: 주소의 홈 노드로 요청 운반
- **⑥ 원격 메모리 접근·응답**: 홈 노드 DRAM 접근 후 응답
- **⑦ 요청 CPU에 데이터 반환**: 인터커넥트로 원격 데이터 반환
