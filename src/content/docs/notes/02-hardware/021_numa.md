...
- 정의/개념: 프로세서와 메모리를 여러 노드로 분산해 접근 위치에 따라 지연이 달라지는 **공유 메모리 구조**
- 배경/필요성: 프로세서가 늘면 단일 메모리 경로에 요청이 집중되므로 **메모리 용량·대역폭 확장 필요**
...
- 노드별 **로컬 메모리·채널** 추가에 따른 용량과 대역폭 확장
- **원격 메모리 접근**에 따른 인터커넥트 경합과 응답 지연 증가
...
1. **스레드·페이지 배치**: CPU 친화도와 물리 페이지 위치를 같은 NUMA 노드에 정렬
2. **홈 노드 판정**: 물리 주소를 보유한 홈 노드가 실행 CPU의 로컬 노드인지 확인
3. **로컬 DRAM 접근**: 홈 노드가 같으면 로컬 메모리 제어기와 채널로 요청 처리
4. **원격 DRAM 접근**: 홈 노드가 다르면 인터커넥트를 거쳐 원격 메모리 제어기로 요청 전달
