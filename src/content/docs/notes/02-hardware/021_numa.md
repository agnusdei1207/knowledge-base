...
- **1. 스레드•페이지 배치**: CPU 친화도와 물리 위치 정렬
- **2. 홈 노드 판정**: 물리 주소의 로컬•원격 위치 확인
- **3. 로컬 DRAM 접근**: 같은 노드의 메모리 제어기 사용
- **4. 인터커넥트•홈 DRAM 접근**: 원격 노드에 요청 전달
- **5. 스레드•페이지 재배치**: 지속 원격 접근의 지역성 회복

#### 한줄 요약

- 로컬 접근 우선, 원격 접근은 **인터커넥트** 경유
