...
sequenceDiagram
    ...
    participant MA as 노드 A DRAM
    participant MB as 노드 B DRAM

    O->>C: ① CPU 친화도에 따라 스레드 배치
    C->>A: ② 물리 주소 메모리 요청
    alt 페이지가 노드 A에 위치
        A->>MA: ③ 로컬 메모리 접근
        MA-->>C: ④ 로컬 데이터 반환
...
1. **① CPU 친화도에 따라 스레드 배치**: 운영체제는 스레드의 친화도와 페이지 위치를 고려해 실행할 NUMA 노드의 CPU를 선택한다.
2. **② 물리 주소 메모리 요청**: 노드 A의 CPU는 주소 변환을 마친 물리 주소를 로컬 메모리 제어기에 전달한다.
3. **③ 로컬 메모리 접근**: 주소가 노드 A의 페이지를 가리키면 제어기는 직접 연결된 메모리 채널로 요청을 보낸다.
4. **④ 로컬 데이터 반환**: 노드 A DRAM은 인터커넥트를 거치지 않고 요청 데이터를 CPU에 반환한다.
5. **⑤ 원격 메모리 요청 전달**: 주소가 다른 노드의 페이지를 가리키면 로컬 제어기는 요청을 노드 간 인터커넥트에 전달한다.
6. **⑥ 홈 노드 제어기로 라우팅**: 인터커넥트는 물리 주소의 홈 노드를 판별해 노드 B 메모리 제어기로 요청을 운반한다.
7. **⑦ 원격 DRAM 접근**: 노드 B 제어기는 자신의 메모리 채널을 통해 해당 페이지가 있는 DRAM에 접근한다.
8. **⑧ 원격 데이터 반환**: 노드 B DRAM은 읽은 데이터를 노드 B 메모리 제어기에 반환한다.
9. **⑨ 응답 데이터 전달**: 노드 B 제어기는 응답을 노드 간 인터커넥트에 싣는다.
10. **⑩ 요청 CPU에 데이터 반환**: 인터커넥트는 데이터를 원래 요청한 노드 A CPU로 보내 원격 접근을 완료한다.
...
> 데이터베이스는 질의를 실행하는 워커와 해당 워커가 주로 쓰는 버퍼 풀을 같은 NUMA 노드에 고정해 원격 접근을 줄인다.
