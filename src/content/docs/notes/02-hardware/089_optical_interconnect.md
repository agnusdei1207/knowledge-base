...
- **CPO(Co-Packaged Optics)**: 스위치 ASIC 및 GPU 칩셋과 광 트랜시버 모듈을 동일한 기판(Substrate) 상에 일체형 패키징하여 전력 오버헤드를 극소화하는 기술.

</details>

- 정의/개념: 구리선 한계를 뛰어넘어 광 레이저 파장 변조 및 광섬유(Fiber)를 통해 초고속 초저전력으로 텐서 데이터를 전송하는 **광 인터커넥트**
...
- **WDM(Wavelength Division Multiplexing)**: 단일 광섬유 선로 상에 서로 다른 다중 파장의 빛을 인가하여 전송 대역폭을 수십 배 확장하는 다중화 기술.
...
- **EMI(Electromagnetic Interference) 내성**: 전자기파 간섭 및 노이즈의 물리 영향을 전무하게 차단하는 광섬유 소재 고유의 신호 안정 특성.

</details>

- 구리선 손실 및 **EMI** 전자기파 간섭을 완전 차단하는 고신뢰성 무선 전송
- 단일 파이버 상에서 복수 파장을 융합 전송하는 **WDM** 대역폭 극대화
...
- **Pic/Modulator(Silicon Photonics)**: 전기 신호를 빛의 세기/위상으로 변조(NRZ/PAM4)하는 실리콘 포토닉스 변조기.
...
| 광섬유 경로(Optical Fiber) | 신호 감쇠가 극소화된 광 신호 파이프라인 전송 |
...
| 전력 소비 | 높음 (전기 SerDes 구동 오버헤드) | 매우 낮음 (전기 선로 길이 90% 단축) | 저전력 (단, 거리 증가 시 발열 폭증) |
| 전송 거리 | 장거리 지원 (~수 km) | 중/장거리 지원 (~수백 m) | 초단거리 한정 (~2m 이내) |
...
| 패키징 기판 내 광 파이버 정밀 정렬 난제 | 실리콘 포토닉스 자동 정렬 자조 시스템 적용 | 제조 수율 향상 |
| **CPO** 메인 ASIC 고장 시 광 모듈 동시 교체 비용 | ELS 외장화 및 착탈식 CPO 광 커넥터 수용 | 정비성(Maintainability) 확보 |

> 사례: **CPO(Co-Packaged Optics)** 기반 51.2Tbps AI 스위치 인터커넥트 실증
...
- **광 인터커넥트 선택 기준**에 따라 초고속 AI 데이터센터 및 GPU 클러스터 랙 간 통신 시 **CPO 기반 광 인터커넥트** 도입

#### 한줄 요약

- 구리선 물리적 한계 극복, WDM 파장 다중화 및 CPO 일체형 패키징 기반 광 인터커넥트 구축 체계 적용.
