---
sidebar:
  order: 89
  label: "089. 광 인터커넥트 (Optical Interconnect)"
  badge:
    text: "미출제 · 70%"
    variant: note
title: "광 인터커넥트 (Optical Interconnect)"
date: "2026-07-25T00:22:25+09:00"
tags:
  - "notes-hardware"
weight: 89
extra:
  question_no: "089"
  source_status: "미출제"
  source_history: ""
  priority: 70
  priority_note: "인공지능 클러스터 대역폭·에너지 병목 대응"
---

## 미리 알고가기

- **광 인터커넥트(Optical Interconnect)**: 전기 데이터를 광 신호로 변환·전송·복원
- **전기-광 변환(Electrical-to-Optical Conversion, E/O)**: E/O는 ‘이 투 오’로 읽고 Electrical과 Optical의 첫 글자를 변환 방향 순서로 적은 표기로, 송신 전기 데이터를 광 신호로 변환
- **광-전기 변환(Optical-to-Electrical Conversion, O/E)**: O/E는 ‘오 투 이’로 읽고 Optical과 Electrical의 첫 글자를 변환 방향 순서로 적은 표기로, 수신 광 신호를 전기 데이터로 복원
- **직병렬 변환기(Serializer/Deserializer, SerDes)**: SerDes는 ‘서데스’로 읽고 직렬화·역직렬화 명칭을 결합한 약어이며, 양단에서 병렬·직렬 데이터를 변환
- **등화기(Equalizer)**: 전기 경로 손실로 흐려진 파형 보정
- **파장 다중화(Wavelength Division Multiplexing, WDM)**: WDM은 ‘더블유디엠’으로 읽고 영문 머리글자를 딴 약어이며, 여러 파장을 한 광섬유로 전송
- **공동 패키지 광학(Co-Packaged Optics, CPO)**: CPO는 ‘시피오’로 읽고 영문 머리글자를 딴 약어이며, 광 엔진을 스위치 칩 가까이에 배치
- **링크 버짓(Link Budget)**: 송신 출력·경로 손실·수신 감도 간 여유
- **감쇠·누화(Attenuation·Crosstalk)**: 감쇠는 전송 거리에서 신호 세기가 줄어드는 현상이고 누화는 이웃 채널 신호가 섞이는 간섭
- **전자기 간섭(Electromagnetic Interference, EMI)**: EMI는 ‘이엠아이’로 읽고 영문 머리글자를 딴 약어이며, 외부 전자기장이 전기 신호에 잡음을 더하는 현상
- **레이저·변조기(Laser·Modulator)**: 레이저는 광 반송파를 만들고 변조기는 전기 데이터에 따라 빛의 세기·위상을 바꿈
- **수광기·증폭기(Photodetector·Amplifier)**: 수광기는 빛을 전류로 바꾸고 증폭기는 약한 전기 신호를 판독 가능한 크기로 키움
- **비트당 에너지(Energy per Bit)**: 데이터 한 비트를 전송·복원하는 데 소비되는 에너지
- **랙(Rack)**: 여러 서버·스위치·전원 장비를 표준 프레임에 장착한 데이터센터 설비 단위

## Ⅰ. 개요

- 정의/개념: 전기 데이터를 광 신호로 변환·전송·복원하는 연결 기술
- **배경/필요성**: 장거리 구리 손실·등화 전력이 커져 광 전송이 필요함

### 쉽게 이해하기 (학습용)

- 전기 신호를 빛으로 바꿔 보내고 도착지에서 데이터로 되돌린다.

## Ⅱ. 특징

- 광섬유는 전자기 간섭과 장거리 감쇠를 줄인다.
- WDM은 파장별 채널을 병렬 전송해 대역폭을 높인다.
- 링크 버짓이 수신 가능 거리와 오류 위험을 정한다.
- 광전 변환·결합·열 제어가 전력과 비용을 높인다.

### 쉽게 이해하기 (학습용)

- 빛은 멀리 가고 여러 색에 데이터를 나눠 실을 수 있지만 빛으로 바꾸는 장치에 전력·비용이 든다.

## Ⅲ. 아키텍처 및 구성요소

```mermaid
flowchart LR
    I[전기 데이터]
    subgraph O[광 인터커넥트 경계]
        T[광 송신부]
        P[광 경로]
        R[광 수신부]
        T -->|WDM 광 신호| P
        P -->|파장 분리| R
    end
    E[복원 데이터]
    I -->|E/O 변환| T
    R -->|O/E 변환| E
```

| 설계 요소 | 설명 |
|:---|:---|
| 광 송신부 | SerDes·레이저·변조기로 E/O 변환 |
| 광 경로 | WDM·광섬유로 다중 채널 전송 |
| 광 수신부 | 수광기·증폭기·SerDes로 O/E 복원 |

> 요약: 송신 E/O·광 경로·수신 O/E로 연결

### 쉽게 이해하기 (학습용)

- 송신기가 전기 데이터를 빛으로 바꾸고 광섬유가 나르면 수신기가 다시 전기 데이터로 읽는다.

## Ⅳ. 원리 및 절차 흐름도

```mermaid
sequenceDiagram
    participant S as 송신 시스템
    participant T as 광 송신부
    participant P as 광 경로
    participant R as 광 수신부
    participant D as 수신 시스템
    S->>T: 전기 데이터 입력
    T->>P: 광 신호 생성
    P->>R: 파장 다중 전송
    R->>R: 광 신호 검출
    R->>D: 전기 데이터 복원
```

| 절차 | 설명 |
|:---|:---|
| 전기 데이터 입력 | SerDes가 병렬 데이터를 직렬화 |
| 광 신호 생성 | 레이저·변조기가 E/O 변환 |
| 파장 다중 전송 | WDM 채널을 광섬유로 전달 |
| 광 신호 검출 | 수광기가 빛을 전류로 변환 |
| 전기 데이터 복원 | 증폭·판별·역직렬화로 데이터 복원 |

> 요약: E/O 변환·광 전송·O/E 복원

### 쉽게 이해하기 (학습용)

- 송신부가 비트를 여러 색의 빛에 실어 보내면 수신부가 빛을 전기 데이터로 되돌린다.

## Ⅴ. 종류 및 비교

| 판단 기준 | 광 인터커넥트 | 전기 인터커넥트 |
|:---|:---|:---|
| 핵심 특징 | 광섬유·파장 채널의 장거리 전송 | 구리 배선·전압 신호의 직접 연결 |
| 적용 기준 | 전기 채널 한도 초과·비트당 에너지 이득 | 전기 채널 한도 충족·광전 변환 이득 없음 |
| 주요 위험 | 광전 변환·정렬·열 제어 | 감쇠·누화·등화 전력 |

> 요약: 전기 채널 한도 초과·에너지 이득 구간만 광 전환

### 쉽게 이해하기 (학습용)

- 전선 손실이 한도를 넘고 광변환 전력까지 빼도 이득일 때만 광케이블로 바꾼다.

## Ⅵ. 실무 사례

1. 인공지능 클러스터 랙 간 링크는 광섬유·WDM으로 확장
2. CPO 스위치는 광 엔진을 스위치 칩에 근접 배치

### 쉽게 이해하기 (학습용)

- 인공지능 서버 랙 사이는 여러 색의 빛을 한 광섬유에 실어 보낸다.
- CPO 스위치는 빛을 만드는 장치를 스위치 칩 옆에 붙여 전기 배선을 줄인다.

## Ⅶ. 결론

- 전기 채널 한도 초과·비트당 에너지 이득 시 광 전환

### 쉽게 이해하기 (학습용)

- 전선 손실이 한도를 넘고 빛으로 바꿨을 때 전력도 절약되는 구간만 광 연결로 바꾼다.
