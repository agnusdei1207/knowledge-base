...
1. **① 쓰기 주소·데이터 전달**: 프로세서 코어는 수정할 주소와 새 데이터를 쓰기 정책 제어부에 전달한다.
2. **② 캐시 라인 갱신**: 정책 제어부는 쓰기 적중 또는 쓰기 할당으로 확보한 캐시 라인에 새 값을 반영한다.
3. **③ 변경 데이터를 즉시 등록**: WT는 캐시 갱신과 함께 동일한 쓰기 요청을 쓰기 버퍼에 넣는다.
4. **④ 하위 계층 쓰기 요청**: 쓰기 버퍼는 대기 요청을 하위 캐시나 주기억장치에 전달해 캐시와 하위 계층을 함께 갱신한다.
5. **⑤ 쓰기 완료 응답**: 하위 계층은 기록을 마치고 쓰기 버퍼에 완료 상태를 반환한다.
6. **⑥ 해당 라인의 더티 비트 설정**: WB는 하위 계층에 즉시 기록하지 않고 캐시 라인이 더 최신임을 더티 비트로 표시한다.
7. **⑦ 후속 쓰기 전달**: 같은 캐시 라인에 반복 쓰기가 발생하면 후속 주소와 데이터가 정책 제어부로 들어온다.
8. **⑧ 캐시 내부 값만 갱신**: WB는 반복 갱신을 캐시 안에서 처리해 여러 쓰기를 하나의 최종값으로 병합한다.
9. **⑨ 최신 캐시 라인 등록**: 더티 라인을 축출하거나 플러시할 때 캐시 배열은 최신 라인 전체를 쓰기 버퍼에 등록한다.
10. **⑩ 하위 계층에 쓰기 복귀**: 쓰기 버퍼는 더티 라인의 최종값을 하위 계층에 기록한다.
11. **⑪ 반영 완료 응답**: 하위 계층은 데이터 반영이 끝났음을 정책 제어부에 알린다.
12. **⑫ 더티 비트 해제**: 정책 제어부는 캐시와 하위 계층이 다시 일치함을 확인하고 해당 라인의 더티 표시를 해제한다.
