...
| 설계 요소 | 입력·상태 | 역할 |
|:---|:---|:---|
| 쓰기 정책 제어부 | 적중·미스·정책 설정 | WT·WB·할당 방식 결정 |
| 캐시 배열 | 태그·데이터·유효 상태 | 캐시 라인 저장·갱신 |
| 더티 상태 | 라인별 미반영 여부 | 최신 미반영 라인 식별 |
| 쓰기 버퍼 | 반영 대기 요청 | 하위 계층 쓰기 지연 숨김 |

> 전제: 쓰기 적중 또는 쓰기 할당으로 캐시 라인을 확보한 경로
>
> 요약: **더티 상태**로 반영 시점을 축출까지 지연

**동작 원리**

- **① 쓰기 주소·데이터 전달**: 수정할 주소와 새 데이터 전달
- **② 캐시 라인 갱신**: 적중·할당으로 확보한 라인에 값 반영
- **③ 변경 데이터를 즉시 등록**: WT 쓰기를 버퍼에 등록
- **④ 하위 계층 쓰기 요청**: 대기 요청을 하위 계층에 전달
- **⑤ 쓰기 완료 응답**: 하위 계층이 기록 완료 반환
- **⑥ 해당 라인의 더티 비트 설정**: WB 라인의 미반영 상태 표시
- **⑦ 후속 쓰기 전달**: 같은 라인의 반복 쓰기 전달
- **⑧ 캐시 내부 값만 갱신**: 반복 쓰기를 캐시의 최종값으로 병합
- **⑨ 최신 캐시 라인 등록**: 축출·플러시할 라인을 버퍼에 등록
- **⑩ 하위 계층에 쓰기 복귀**: 더티 라인의 최종값 기록
- **⑪ 반영 완료 응답**: 하위 계층 반영 완료 통보
- **⑫ 더티 비트 해제**: 캐시·하위 계층 일치 후 표시 해제
