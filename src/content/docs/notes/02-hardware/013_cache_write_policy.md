...
- 정의/개념: 하위 계층 **쓰기 반영 시점** 정책
- 배경/필요성: **쓰기 지연·트래픽·정합성** 조정
...
- **1. 쓰기 주소·데이터 전달**: 수정 대상 전달
- **2. 캐시 라인 갱신**: 캐시 내부값 변경
- **3. 변경 데이터 등록**: 버퍼 또는 더티 상태 기록
- **4. 하위 계층 반영**: 즉시 또는 축출 시 기록
- **5. 최신 캐시 라인 등록**: 더티 라인 버퍼 저장
- **6. 반영 완료·상태 해제**: 더티 표시 제거
