...
date: "2026-08-05T11:34:29+09:00"
tags:
  - "notes-hardware"
weight: 13
extra:
...
- **WT**: Write-Through, 캐시 갱신마다 하위 계층에도 즉시 기록하는 정책
- **WB**: Write-Back, 캐시에 먼저 기록하고 축출할 때 하위 계층에 반영하는 정책
...
```text
             ...
             1. 적중•할당•반영 정책 판정
                    ...
                    [WT]          [WB]
                     |             |
        2. WT 캐시•쓰기 버퍼 갱신  3. WB 캐시•더티 상태 갱신
                     |             |
                     |      +--------------------------+
                     |      | 반복: 축출•플러시 전     |
                     |      | 같은 라인의 최신값 유지   |
...
- **1. 적중•할당•반영 정책 판정**: 쓰기 경로와 할당 여부 결정
- **2. WT 캐시•쓰기 버퍼 갱신**: 하위 계층 쓰기 즉시 등록
- **3. WB 캐시•더티 상태 갱신**: 최신값을 캐시에 유지
- **4. 하위 계층 기록**: WT 즉시 또는 WB 축출 때 반영
- **5. 더티 상태 해제**: WB 기록 확인 후 표시 제거

#### 한줄 요약

- WT의 **즉시 반영**, WB의 **더티 축출** 경로 구분
...
- **DMA**: Direct Memory Access, 장치가 CPU 개입 없이 주기억장치와 직접 전송하는 방식
