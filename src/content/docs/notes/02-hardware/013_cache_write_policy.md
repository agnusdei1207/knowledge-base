...
- **쓰기 통과(Write-Through, WT)**: ‘더블유티’로 읽고 영문 머리글자를 딴 약어이며, 캐시 쓰기와 동시에 하위 계층에도 값을 반영하는 정책
- **쓰기 복귀(Write-Back, WB)**: ‘더블유비’로 읽고 영문 머리글자를 딴 약어이며, 캐시에만 쓰고 축출·강제 반영 때 하위 계층에 기록하는 정책
...
- **쓰기 할당(Write Allocate, WA)**: ‘더블유에이’로 읽고 영문 머리글자를 딴 약어이며, 쓰기 미스 블록을 캐시에 리필한 뒤 갱신하는 방식
- **쓰기 비할당(No-Write-Allocate, NWA)**: ‘엔더블유에이’로 읽고 영문 머리글자를 딴 약어이며, 쓰기 미스 블록을 캐시에 넣지 않고 하위 계층에 바로 기록하는 방식
...
- **직접 메모리 접근(Direct Memory Access, DMA)**: ‘디엠에이’로 읽고 영문 머리글자를 딴 약어이며, 장치가 프로세서를 거치지 않고 메모리를 직접 읽고 쓰는 방식
