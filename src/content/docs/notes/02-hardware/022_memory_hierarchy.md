...
| **데이터 유지** | **작업 집합(Working Set)** 집중 적재 관리 | 전역 코드, 파일 데이터 및 **비상주 페이지** 적재 |
| **용량 부족 시** | **반복 교체(Cache Line Eviction)** 수반 | 디스크 스왑 및 **Page Fault I/O** 발생 |
...
|:---|:---|:---|
| 대용량 2차원 행렬 연산 시 캐시 크기 초과로 **반복 교체(Thrashing)** 발생 | 루프 재구성을 통한 **캐시 블로킹(Cache Blocking/Tiling)** 최적화 | L1/L2 캐시 적중률 대폭 향상 및 AMAT 지연 감소 |
| 스트리밍 데이터 무작위 로드로 인한 **캐시 오염(Cache Pollution)** | Non-Temporal Store (`_mm_stream_si128`) 및 **선인출** 강도 조절 | 유용한 Working Set 라인의 캐시 유지 및 오염 차단 |
| DRAM 메모리 억세스 지연(60ns)으로 인한 파이프라인 Stall | 하드웨어 **스트라이드 선인출(Prefetcher)** 내장 및 MSHR 확장 | DRAM 읽기 지연시간 파이프라인 우회 은닉 |
