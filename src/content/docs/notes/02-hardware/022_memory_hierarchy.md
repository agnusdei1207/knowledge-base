...
- 배경/필요성: 저지연 저장장치만으로는 **대용량•저비용 구성 불가**
...
- **시간·공간 지역성**: 방금 쓴 데이터 또는 그 인접 주소를 다시 사용할 가능성이 높은 성질
- **적중률·미스율**: 요청 데이터를 현재 계층에서 찾거나 찾지 못한 접근의 비율
- **AMAT**: 적중 시간에 미스율과 미스 페널티의 곱을 더한 평균 메모리 접근 시간
...
- **레지스터·SRAM 캐시**: CPU 내부의 가장 빠른 저장 공간과 재사용 블록을 보관하는 고속 메모리
- **DRAM 주기억장치**: 실행 중인 코드와 데이터를 큰 용량으로 제공하는 휘발성 메모리
- **SSD 보조기억장치**: 비상주 페이지와 영구 데이터를 보관하는 비휘발성 저장장치
...
- **선인출·캐시 오염**: 예상 데이터를 미리 가져오는 기법과 잘못 가져온 데이터가 유용한 블록을 밀어내는 현상
