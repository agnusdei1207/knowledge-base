...
- **레지스터·정적 임의 접근 메모리(Static Random-Access Memory, SRAM) 캐시**: CPU 내부의 가장 빠른 저장 공간과 재사용 블록을 보관하는 고속 메모리
...
- **평균 메모리 접근 시간(Average Memory Access Time, AMAT)**: 계층별 적중 시간과 미스율·미스 페널티를 함께 반영한 평균 지연 지표
- **선인출 정확도·오염률**: 미리 가져온 블록 중 실제 사용된 비율과 유용한 캐시 블록을 밀어낸 비율
