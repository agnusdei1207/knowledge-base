...
date: "2026-07-28T12:46:43+09:00"
tags:
  - "notes-hardware"
weight: 22
extra:
...
| 설계 요소 | 입력·상태 | 역할 |
|:---|:---|:---|
| 레지스터 | 현재 명령의 피연산자 | 최고속 연산 데이터 제공 |
| 캐시 계층 | 재사용 캐시 블록 | SRAM 기반 지연 완충 |
| 주기억장치 | 실행 중 코드·데이터 | DRAM 기반 작업 공간 제공 |
| 보조기억장치 | 비상주·영구 데이터 | 비휘발성 대용량 보관 |
...
| 운영 위험 | 대응 | 기대 효과 |
|:---|:---|:---|
| 상위 계층 용량보다 큰 작업 집합 | 계층별 미스율과 작업 집합 크기 측정 | 반복 교체 원인 식별 |
| 높은 적중률에도 긴 미스 페널티 | AMAT와 계층별 지연을 함께 관찰 | 평균 지연 정확한 평가 |
| 공간 지역성이 낮은 데이터의 과도한 선인출 | 선인출 정확도·오염률 조정 | 대역폭·캐시 낭비 감소 |
| 원격·확장 메모리 계층의 지연 변동 | 데이터 온도별 배치와 마이그레이션 임계치 설정 | 비용·용량·지연 균형 |
