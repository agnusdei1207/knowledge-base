...
- **메모리 계층(Memory Hierarchy)**: 손이 자주 가는 물건은 책상 위에, 가끔 쓰는 것은 서랍에, 일 년에 한 번 볼 것은 지하 창고에 두듯 지연·용량·단가가 서로 다른 저장장치를 위에서 아래로 쌓아 두고 자주 쓰는 것만 위로 끌어올려 쓰는 구조
...
- **평균 메모리 접근 시간(Average Memory Access Time, AMAT)**: 적중 시간에 미스율과 미스 페널티의 곱을 더해 얻는 평균 접근 지연이며, 미스율이 낮아도 페널티가 크면 이 값은 나빠진다
- **레지스터(Register)**: 명령이 직접 이름을 대고 읽고 쓰는 가장 작고 빠른 프로세서 내부 저장 공간
- **정적 랜덤 접근 메모리(Static Random-Access Memory, SRAM)**: 래치 회로로 비트를 붙들고 있어 되살릴 필요가 없고 그만큼 빠르지만 셀 하나가 커서 비싼 메모리이며 캐시에 쓴다
- **래치(Latch)**: 출력을 입력으로 되먹여 상태를 스스로 붙들고 있는 회로
- **동적 랜덤 접근 메모리(Dynamic Random-Access Memory, DRAM)**: 축전기의 전하로 비트를 담아 셀이 작고 싸지만 전하가 새어 주기적으로 되살려야 하는 메모리이며 주기억장치에 쓴다
- **솔리드 스테이트 드라이브(Solid-State Drive, SSD)**: 플래시 메모리로 전원이 꺼져도 남는 대용량 저장을 제공하는 장치
...
- **컴퓨트 익스프레스 링크 메모리(Compute Express Link Memory, CXL Memory)**: 프로세서에 직접 꽂힌 메모리 슬롯 밖에서 링크로 붙여 용량을 늘리는 확장 메모리 계층이며, 주기억장치보다 느리고 저장장치보다 빠른 자리를 채운다
...
- 안쪽 옵션이 바깥 옵션 안에 겹쳐 들어 있는 모양이 곧 비용 구조인데, 서랍에서 찾으면 한 번 다녀오는 값으로 끝나지만 창고까지 내려가면 서랍 왕복과 창고 왕복이 겹쳐 붙으므로 같은 헛걸음이라도 몇 층까지 내려갔는지에 따라 값이 자릿수로 벌어진다
...
2. **CXL 메모리** 계층화로 저빈도 페이지를 확장 계층에 배치

### 쉽게 이해하기 (학습용)

- 큰 행렬을 통째로 훑으면 한 줄을 다 보기도 전에 앞부분이 밀려나 재사용이 사라지므로, 캐시에 들어가는 정사각형 조각으로 잘라 그 안에서 곱셈을 다 끝낸 뒤 다음 조각으로 넘어간다
- 메모리 슬롯을 다 꽂아도 용량이 모자라는 서버에서는 접근이 뜸한 페이지를 링크로 붙인 확장 메모리로 내려 두어, 값비싼 직결 메모리를 자주 쓰는 페이지에만 남긴다

## Ⅶ. 결론

- **재사용 빈도**로 블록의 상위·하위 계층 배치 결정
