...
- **① 주소·데이터 접근 요청**: 가장 가까운 캐시 계층에 요청
- **② 요청 데이터 반환**: 적중 블록의 요청 위치 반환
- **③ 캐시 블록 요청**: 미스 주소를 포함한 블록 요구
- **④ 상주 메모리 블록 반환**: 상주 페이지의 캐시 블록 반환
- **⑤ 비상주 페이지 읽기 요청**: 폴트 처리로 보조기억장치 조회
- **⑥ 페이지 데이터 반환**: 요청 페이지를 주기억장치로 반환
- **⑦ 적재된 메모리 블록 반환**: 상주한 페이지의 블록 전달
- **⑧ 채운 블록의 요청 데이터 반환**: 캐시 저장 후 요청 데이터 제공
