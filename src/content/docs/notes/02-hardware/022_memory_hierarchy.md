...
- **시간·공간 지역성**이 상위 계층의 데이터 재사용과 적중률을 좌우
- **적중 시간+미스율×미스 페널티**로 평균 메모리 접근 시간 결정
...
2. **비상주 페이지 적재**: 페이지 폴트가 발생하면 보조기억장치의 페이지를 주기억장치에 적재
3. **캐시 계층 조회**: 주소 태그를 상위 캐시부터 확인해 요청 블록의 적중 여부 판정
4. **주기억장치 블록 공급**: 모든 캐시에서 미스가 발생하면 DRAM에서 블록을 읽어 캐시에 채운 뒤 반환
