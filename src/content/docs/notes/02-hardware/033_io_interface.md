...
> 요약: 제어기는 상태를 중계하고 전용 엔진은 데이터를 전송

**동작 원리**

- **① DMA 디스크립터·수신 버퍼 준비**: 주소·길이·방향과 IOMMU 권한 설정
- **② 디스크립터 주소·전송 시작 설정**: DMA 제어 레지스터에 작업 등록
- **③ DMA 디스크립터 읽기**: 메모리에서 전송 제어 정보 조회
- **④ 주소·길이·방향 반환**: 버퍼 위치와 전송 조건 반환
- **⑤ 장치 데이터 전송 요청**: 장치 제어기와 준비 상태 확인
- **⑥ 장치 데이터 전달**: 수신 데이터를 블록 단위로 전달
- **⑦ 수신 버퍼에 직접 기록**: 허용된 메모리 버퍼에 DMA 쓰기
- **⑧ DMA 완료 인터럽트 요청**: 지정 길이 전송 후 완료 알림
- **⑨ 대상 CPU에 완료 통지**: 담당 CPU의 ISR로 인터럽트 전달
- **⑩ DMA 완료 상태 확인**: 드라이버가 성공·오류 상태 확인
- **⑪ 수신 버퍼 확인·상위 계층 인계**: 캐시 무효화·장벽 후 버퍼 전달
