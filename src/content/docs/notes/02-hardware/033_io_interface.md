...
| 구성요소 | 책임 |
|:---|:---|
| CPU·드라이버 | 전송 설정과 **완료 상태** 처리 |
| 장치·제어기 | 장치 신호를 **상태·데이터 형식**으로 변환 |
| DMA·채널 엔진 | 블록 전송과 **I/O 명령열** 실행 |
| 주기억장치 | 전송 데이터와 **제어 정보** 보관 |

> 요약: 제어기는 상태, 전용 엔진은 데이터 전송 담당

**도표안 B — 장치→메모리 DMA 전송·완료 시퀀스**
...
sequenceDiagram
    ...
    C->>M: ① DMA 디스크립터·수신 버퍼 준비
    C->>E: ② 디스크립터 주소·전송 시작 설정
    E->>M: ③ DMA 디스크립터 읽기
    M-->>E: ④ 주소·길이·방향 반환
    E->>D: ⑤ 장치 데이터 전송 요청
    loop 디스크립터의 블록 길이
...
1. **① DMA 디스크립터·수신 버퍼 준비**: 장치 드라이버는 DMA가 쓸 메모리 주소·길이·방향을 디스크립터에 기록하고 IOMMU 권한과 버퍼를 준비한다.
2. **② 디스크립터 주소·전송 시작 설정**: CPU는 DMA 엔진의 제어 레지스터에 디스크립터 위치를 쓰고 전송을 시작한다.
3. **③ DMA 디스크립터 읽기**: DMA 엔진은 CPU의 추가 개입 없이 주기억장치에서 전송 제어 정보를 가져온다.
4. **④ 주소·길이·방향 반환**: 주기억장치는 DMA가 접근할 버퍼 주소, 전송할 바이트 수와 장치→메모리 방향을 반환한다.
5. **⑤ 장치 데이터 전송 요청**: DMA 엔진은 장치 제어기와 핸드셰이크해 데이터가 준비된 만큼 보내도록 요청한다.
6. **⑥ 장치 데이터 전달**: 장치 제어기는 수신한 외부 데이터를 DMA 엔진에 블록 단위로 전달한다.
7. **⑦ 수신 버퍼에 직접 기록**: DMA 엔진은 전달받은 데이터를 IOMMU가 허용한 주기억장치 버퍼에 직접 쓴다.
8. **⑧ DMA 완료 인터럽트 요청**: 지정된 길이의 전송이 끝나면 DMA 엔진은 인터럽트 제어기에 완료를 알린다.
9. **⑨ 대상 CPU에 완료 통지**: 인터럽트 제어기는 해당 장치의 ISR을 실행할 CPU로 완료 인터럽트를 전달한다.
10. **⑩ 완료 상태 확인·버퍼 인계**: 드라이버는 DMA 상태를 확인하고 필요한 캐시 무효화와 메모리 장벽을 수행한 뒤 버퍼를 상위 소프트웨어에 넘긴다.
...
- 짧은 대기는 폴링, 사건 통지는 인터럽트, 대용량 블록은 **DMA** 선택
