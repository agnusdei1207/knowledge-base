...
date: "2026-08-05T11:58:03+09:00"
tags:
  - "notes-hardware"
weight: 51
extra:
...
- 정의/개념: 뉴런•시냅스의 신호 전달을 모사해 입력 변화를 스파이크로 처리하는 **뉴로모픽 컴퓨팅** 기반 **이벤트 처리 구조**
- 배경/필요성: 조밀 텐서 연산은 무변화 구간에도 **전력 소모 지속**

#### 한줄 요약

- 입력 변화 때만 **스파이크 연산•통신** 활성화
...
- **1. 주소 이벤트 인코딩**: 변화 위치•시점을 목적지 주소로 변환
- **2. 라우팅•시냅스 가중치 조회**: 연결 뉴런과 가중치 확인
- **3. 가중 스파이크 전달**: 시냅스 가중치를 뉴런 코어에 입력
- **4. 막전위 누적•임계값 판정**: 발화 여부 결정
- **5. 출력 이벤트•상태 초기화**: 다음 뉴런 지정과 막전위 갱신
...
- **종단 검증(End-to-end Validation)**: 입력 변환부터 SNN 출력까지 정확도•지연•전력을 측정하는 검증
...
- 희소 이벤트•상시 저전력 조건에는 **뉴로모픽**, 조밀 입력 조건에는 **폰 노이만 구조** 선택

#### 한줄 요약

- 희소•상시 저전력에서만 뉴로모픽 적용
