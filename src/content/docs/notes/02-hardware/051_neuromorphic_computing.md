...
> 요약: 라우터가 보내면 시냅스가 뉴런 입력을 가중

**동작 원리**

- **① 변화 위치·시점의 주소 이벤트 생성**: 변한 픽셀만 주소·시간으로 변환
- **② 목적지 시냅스 조회**: 연결 정보로 활성 시냅스와 뉴런 탐색
- **③ 가중 스파이크 전달**: 저장 가중치를 적용해 누산기로 전송
- **④ 갱신 막전위·임계값 전달**: 이전 상태·입력·시간 감쇠를 반영
- **⑤ 출력 주소 이벤트 전달**: 임계값 도달 시 다음 뉴런으로 발화
- **⑥ 막전위 초기화**: 모델 규칙에 따라 초기화 또는 일정 값 차감
- **⑦ 막전위 상태 유지**: 미달이면 통신 없이 내부 상태 보존
