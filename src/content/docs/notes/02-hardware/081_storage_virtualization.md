...
> 요약: 매핑 계층이 논리 I/O를 물리 풀에 연결한다.

**동작 원리**

- **① 논리 쓰기**: 호스트가 논리 주소·데이터·정책을 전달
- **② 매핑 조회**: 기존 물리 위치 확인
- **③ 미할당 판정**: 최초 쓰기이면 미할당 상태 반환
- **④ 공간 요청**: 크기·성능·보호 정책에 맞는 익스텐트 요청
- **⑤ 공간 예약**: 용량 임계치와 장애 도메인을 확인해 위치 예약
- **⑥ 데이터 기록**: 예약 위치에 데이터·체크섬 쓰기
- **⑦ 지속성 확인**: 캐시 플러시와 보호 정책 완료 확인
- **⑧ 매핑 반영**: 데이터 기록 후 논리–물리 관계를 원자적으로 갱신
- **⑨ 메타데이터 완료**: 복구 가능한 커밋 상태 확인
- **⑩ I/O 완료**: 물리 위치를 숨긴 채 호스트에 완료 통지
...
> 요약: 통합 범위·데이터 경로·장애 경계로 방식을 선택한다.
