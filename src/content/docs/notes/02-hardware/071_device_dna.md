...
### 도표안 B. 현재 지문 비교와 위험 기반 인증 시퀀스

```mermaid
sequenceDiagram
    participant A as 인증 서비스
    participant D as 대상 장치
    participant C as 특성 수집기
    participant G as DNA 생성 엔진
    participant R as 기준 DNA 저장소
    participant P as 매칭·위험 정책
    participant M as 추가 인증 수단

    A->>D: ① 세션 논스·수집 정책 전달
    D->>C: ② 허용된 장치 특성·논스 제출
    C->>G: ③ 검증·정규화한 특성 벡터 전달
    G->>R: ④ 장치별 기준 DNA·변경 이력 조회
    R-->>G: ⑤ 기준 벡터·특성 안정도 반환
    G->>P: ⑥ 유사도·변경 특성·신뢰도 전달
    A->>P: ⑦ 접속 위치·거래 민감도·계정 위험 전달
    P-->>A: ⑧ 허용·추가 인증·차단 판정
    alt 저위험·DNA 일치
        A-->>D: ⑨ 세션 허용
    else 중간 위험·정상 드리프트 가능
        A->>M: ⑩ 독립 자격 증명 추가 검증 요청
        M-->>A: ⑪ 추가 인증 결과
        A-->>D: ⑫ 결과에 따른 허용·재등록 안내
    else 고위험·위장 의심
        A-->>D: ⑬ 접속 차단·복구 절차 안내
    end
```
...
1. **신선도 부여**: 인증 서비스가 새 세션 논스와 목적에 맞는 최소 특성 수집 정책을 장치에 전달한다.
2. **특성 제출**: 대상 장치가 논스에 결합한 하드웨어·소프트웨어·행동 특성을 수집기에 제출한다.
3. **특성 벡터 생성**: 수집기가 형식·허용 범위를 검증하고 정규화한 벡터를 DNA 생성 엔진에 전달한다.
4. **기준 조회**: DNA 생성 엔진이 해당 장치의 등록 지문과 승인된 변경 이력을 조회한다.
5. **비교 기준 반환**: 저장소가 기준 벡터와 특성별 안정도·최근 재등록 정보를 반환한다.
6. **지문 비교 결과 전달**: 생성 엔진이 현재·기준 지문의 유사도, 달라진 특성과 측정 신뢰도를 위험 정책에 전달한다.
7. **접속 맥락 결합**: 인증 서비스가 위치·시간·거래 민감도와 계정 위험을 추가 증거로 전달한다.
8. **위험 판정**: 정책 엔진이 DNA를 단독 인증값이 아닌 보조 증거로 사용해 허용·추가 인증·차단을 결정한다.
9. **저위험 허용**: 지문이 충분히 일치하고 접속 위험이 낮으면 장치 세션을 허용한다.
10. **단계적 검증**: 정상 업데이트에 따른 드리프트 가능성이 있으면 독립된 자격 증명으로 추가 인증한다.
11. **추가 인증 결과**: 인증 수단이 사용자·장치의 독립 검증 결과를 인증 서비스에 반환한다.
12. **허용·재등록**: 성공하면 세션을 허용하고 승인 절차를 거쳐 기준 DNA를 단계적으로 갱신하며, 실패하면 거부한다.
13. **고위험 차단**: 다수의 안정 특성이 동시에 달라지거나 위장 징후가 있으면 접속을 막고 안전한 복구 절차를 안내한다.
...
- 모바일 금융은 기기·OS·센서 특성과 접속·거래 맥락을 결합해 위험이 높을 때 독립된 추가 인증을 요구한다.
