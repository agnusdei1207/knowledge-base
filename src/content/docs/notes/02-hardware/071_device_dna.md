...
- 정의/개념: 하드웨어•소프트웨어•행동 특성을 결합한 **복합 장치 지문**을 등록값과 비교하여 장치 위험을 판정하는 기법이다.
- 배경/필요성: 단일 고정 ID는 복제하거나 재전송할 수 있어 현재 접속한 장치의 진위를 충분히 판별할 수 없다.

#### 한줄 요약

- 디바이스 DNA는 하드웨어•소프트웨어•행동 특성을 결합한 **복합 장치 지문**이다.
...
- **복합 특성 결합**으로 단일 식별자보다 위장 비용 증가한다.
- **유사도•위험 점수**로 장치 변화 허용 범위를 판정한다.
- **지속 수집**은 드리프트 적응을 높이지만 개인정보 노출을 확대한다.

#### 한줄 요약

- **유사도•위험 점수**로 정상 드리프트와 위장을 구분한다.
...
- **복합 지문 생성•기준 DNA 비교** 기반 위험을 판정한다.
...
- 정상 장치의 **특성 드리프트•거래 위험**을 함께 반영하여 단계별 인증 조치를 결정한다.
...
- **Device DNA는 유사도**, PUF는 물리 응답 재현, 고정 ID는 완전 일치로 장치를 구분한다.
...
- **감사 로그(Audit Log)**: 기준 DNA의 등록과 변경 및 승인 이력을 사후 추적할 수 있게 남긴 기록한다.
...
- 위험이 높거나 지문 유사도가 낮은 접속에는 **추가 인증**을 적용한다.
...
- 특성의 안정성•구분력이 충분하고 개인정보 위험을 통제할 수 있을 때 **Device DNA**를 인증 보조 증거로 적용한다.

#### 한줄 요약

- 디바이스 DNA는 **드리프트•위장 가능성**을 반영한 인증 보조 증거로 활용해야 한다.
