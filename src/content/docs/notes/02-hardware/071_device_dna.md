...
- **정의/개념**: 하드웨어·소프트웨어·행동 특성을 합성한 장치 지문
- **배경/필요성**: 단일 ID는 복제돼 복합 특성 판정이 필요
...
- 복합 특성은 단일 식별자보다 위장 비용을 높인다.
- 갱신·환경 변화는 지문을 바꿔 오탐 위험을 높인다.
- 식별 신호라 비밀 키·강한 인증을 대체하지 못한다.
...
| 판단 기준 | 디바이스 DNA | PUF | 고정 식별자 |
|:---|:---|:---|:---|
| 핵심 특징 | 복합 특성의 유사도·위험 판정 | 제조 편차의 응답·키 재현 | 저장된 번호의 완전 일치 |
| 적용 기준 | 위험 기반 보조 인증 | 하드웨어 키·장치 인증 | 자산 식별·조회 키 |
| 주요 위험 | 특성 위장·개인정보·오탐 | 잡음·모델링·측채널 공격 | 복사·변조·재전송 |
...
1. 모바일 금융: 기기·OS·센서 특성으로 추가 인증 결정
2. IoT 접근 통제: 하드웨어·통신 지문으로 교체 장치 탐지

### 쉽게 이해하기 (학습용)

- 금융 서비스는 평소 장치와 달라진 특성이 많으면 추가 확인을 요구한다.
- IoT 망은 등록 센서와 다른 부품·통신 습관이 보이면 접속을 막는다.

## Ⅶ. 결론

- 특성 안정성·위장·개인정보 검증 후 보조 인증 선택
