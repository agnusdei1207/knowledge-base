...
sequenceDiagram
    ...
    G->>R: ③ 기준 DNA·변경 이력 조회
    R-->>G: ④ 기준 벡터·특성 안정도 반환
    G->>A: ⑤ 유사도·변경 특성·신뢰도 전달
    A->>D: ⑥ 접속 맥락 요청
    D-->>A: ⑦ 위치·거래 민감도 등 맥락 반환
    alt 저위험·DNA 일치
        A-->>D: ⑧ 세션 허용
...
> 요약: 복합 특성을 기준 DNA와 비교해 위험을 판정한다.

**동작 원리**

1. **신선도 부여**: 새 논스와 최소 수집 정책을 전달한다.
2. **특성 제출**: 장치가 논스와 허용된 특성을 제출한다.
3. **기준 조회**: 현재 벡터를 만들고 등록 지문과 변경 이력을 읽는다.
4. **기준 반환**: 저장소가 기준 벡터와 특성 안정도를 반환한다.
5. **지문 비교**: 유사도·변경 특성·측정 신뢰도를 정책에 전달한다.
6. **맥락 요청**: 위치·시간·거래 민감도 등 접속 맥락을 요청한다.
7. **맥락 결합**: 장치 지문과 접속 맥락을 함께 평가한다.
8. **저위험 허용**: 지문이 일치하고 위험이 낮으면 세션을 허용한다.
9. **추가 검증**: 정상 드리프트 가능성이 있으면 독립 자격을 확인한다.
10. **결과 반환**: 추가 인증 수단이 검증 결과를 반환한다.
11. **허용·재등록**: 성공 시 허용하고 승인 절차로 기준을 갱신한다.
12. **고위험 차단**: 안정 특성이 크게 달라지면 접속을 막는다.
...
> 요약: DNA는 복합 지문이고 PUF는 제조 편차 기반 함수다.
...
- 특성 안정성·구분력·위장 가능성·개인정보 영향을 검토해 디바이스 DNA를 독립 자격 증명의 보조 증거로 사용한다.
