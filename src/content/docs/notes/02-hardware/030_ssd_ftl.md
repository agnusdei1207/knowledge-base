...
sequenceDiagram
    ...
    F->>J: ⑥ LPN→새 PPA 변경 기록
    J-->>F: ⑦ 저널 보존 완료
    F->>M: ⑧ 새 매핑 활성화·기존 페이지 무효화
    F-->>H: ⑨ 호스트 쓰기 완료
    opt 여유 블록 임계치 미만
        M-->>F: ⑩ 블록별 유효·무효 페이지 상태 전달
        loop 희생 블록의 유효 페이지
            F->>N: ⑪ 유효 페이지 읽기·새 위치 프로그램
            N-->>F: ⑫ 이동 완료·새 PPA 반환
        end
        F->>N: ⑬ 희생 블록 삭제
        N-->>F: ⑭ 빈 블록 반환
        F->>M: ⑮ 이동 페이지 매핑·여유 블록 갱신
    end
```
...
- **① LBA·쓰기 데이터 전달**: 논리 블록 주소와 새 데이터 전달
- **② LPN의 기존 위치·빈 PPA 조회**: 현재 위치와 빈 물리 페이지 조회
- **③ 기존 PPA·새 PPA 반환**: 최신 위치와 새 기록 위치 반환
- **④ 빈 물리 페이지에 데이터 프로그램**: 새 PPA에 비제자리 기록
- **⑤ 페이지 프로그램 완료**: 기록·오류 검사 결과 반환
- **⑥ LPN→새 PPA 변경 기록**: 새 매핑을 저널에 기록
- **⑦ 저널 보존 완료**: 복구 가능한 매핑 기록 확인
- **⑧ 새 매핑 활성화·기존 페이지 무효화**: 최신 위치 전환과 이전 페이지 무효화
- **⑨ 호스트 쓰기 완료**: 데이터와 필수 매핑 보존 후 완료 반환
- **⑩ 블록별 유효·무효 페이지 상태 전달**: 여유 공간 부족 시 희생 후보 조회
- **⑪ 유효 페이지 읽기·새 위치 프로그램**: 유효 페이지만 빈 블록으로 이동
- **⑫ 이동 완료·새 PPA 반환**: 이동한 페이지의 새 위치 반환
- **⑬ 희생 블록 삭제**: 유효 페이지 이동 후 블록 삭제
- **⑭ 빈 블록 반환**: 삭제한 블록을 여유 공간으로 회수
- **⑮ 이동 페이지 매핑·여유 블록 갱신**: 새 위치와 여유 블록 상태 반영
