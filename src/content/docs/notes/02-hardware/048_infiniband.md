...
> 요약: 서브넷 관리자가 경로를 정하고 HCA가 전송

**동작 원리**

- **① RDMA Write 작업 요청 게시**: 로컬 버퍼·원격 주소·키·길이를 QP에 등록
- **② 로컬 등록 버퍼 DMA 읽기**: HCA가 CPU 복사 없이 원본 접근
- **③ 전송 데이터 반환**: 등록 메모리가 HCA에 데이터 제공
- **④ QP 경로로 데이터 패킷 전송**: 링크 최대 크기에 맞춰 나누고 순서 정보 부여
- **⑤ 대상 HCA 포트로 전달**: 설정 경로와 포트 크레딧으로 라우팅
- **⑥ 원격 주소·키 검증 후 DMA 기록**: 권한·범위 확인 후 원격 메모리에 쓰기
- **⑦ 메모리 기록 완료**: 패킷 데이터 보존 확인
- **⑧ 신뢰 전송 확인 응답**: 요청 범위 기록 후 송신 측에 응답
- **⑨ 송신 HCA에 응답 전달**: 원래 QP로 확인 응답 라우팅
- **⑩ 완료 큐 엔트리·작업 상태 반환**: 성공·오류 기록 후 버퍼 재사용 허용
