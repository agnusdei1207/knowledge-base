...
- **InfiniBand**: RDMA와 저지연 스위칭을 제공하는 전용 패브릭
- **원격 직접 메모리 접근(Remote Direct Memory Access, RDMA)**: 등록 뒤 원격 CPU를 거치지 않고 메모리에 접근
- **호스트 채널 어댑터(Host Channel Adapter, HCA)**: 호스트 메모리와 패브릭 사이 RDMA를 처리하는 어댑터
...
- **정의/개념**: HCA·스위치로 RDMA 전용망을 구성한다
- **배경/필요성**: 통신 대기를 줄여 분산 계산 시간을 단축한다
...
- 등록 메모리·HCA로 CPU 복사를 우회해 시간을 줄인다
- 혼잡·크레딧 정체는 전송 대기를 늘려 학습을 늦춘다
...
| 비교축 | InfiniBand | RoCE | TCP/IP |
|:---|:---|:---|:---|
| 핵심 특징 | 전용 스위치·HCA·Verbs | 이더넷·RNIC·Verbs | 이더넷·IP·커널 소켓 |
| 운영 제어 | 서브넷 관리자·크레딧 흐름 제어 | 혼잡·손실 제어 설정 | 범용 IP 운영 도구 |
| 적용 기준 | HPC·AI 집단 통신 | 기존 이더넷 기반 RDMA | 범용 연결·호환성 |
| 주요 위험 | 전용 장비·운영 비용 | 혼잡·PFC·ECN 조정 복잡도 | CPU·커널 경로 오버헤드 |
...
1. 분산 AI 학습: GPU 집단 통신 시간을 줄인다
2. HPC MPI: 다중 경로로 통신 병목을 완화한다
...
- 통신 병목이 전용망 비용보다 크면 InfiniBand를 쓴다
