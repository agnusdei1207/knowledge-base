...
- 정의/개념: 결함 수정에 따른 신뢰도 향상을 추정하는 **성장 모델**이다.
- 배경/필요성: 결함 건수 감소만으로는 **고장 강도 개선•종료 판단할 수 없다**.
...
- TAAF 반복의 **고장 강도 감소 추정**이 핵심이다.
- 형상•운용 프로파일의 **비교 조건 통제**가 핵심이다.
- 모수•신뢰구간의 **불확실성 판정**이 핵심이다.
...
- **Duane 산식**: 누적 평균 고장 간격 $MTBF_c(t)=K t^{\alpha}$에서 $\alpha>0$이면 성장 추세이다.
- **Crow-AMSAA 산식**: 누적 기대 고장 수 $N(t)=\lambda t^{\beta}$에서 $\beta<1$이면 고장 강도 감소한다.
...
1. **동일 프로파일 고장•시간 수집**: 형상•부하•사건 기준을 고정한다.
2. **원인 분석•수정안 수립**: 재발 원인•변경 이력을 기록한다.
3. **수정 형상 생성•재시험 투입**: 수정 효과•회귀 확인을 준비한다.
4. **성장 모수•신뢰구간 추정**: $\beta$•MTBF•강도를 계산한다.
5. **목표•잔여위험•종료 판정**: 출시•계속시험을 결정한다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 가정 검증 없는 **성장 추세 오판** | **IEC 61164:2004 적용** | 모수•**적합도 근거** 확보 |
| 수정 책임•구간이 섞인 **수정 효과 혼입** | **IEC 61014:2003 적용** | TAAF **역할•절차 정렬** |
| 형상•프로파일이 다른 **거짓 성장** | **기준선•변경 구간 분리** | 성장 판정의 **비교성 확보** |
...
- 성장 조건•MTBF 하한 충족 시 **시험 종료**, 미달 시 **TAAF 계속**이 핵심이다.
