...
- **식별자(Identifier, ID, 아이디)**: 점검 항목과 증거·판정·시정조치를 연결하는 고유 번호
- **SLA(Service Level Agreement, 에스엘에이)**: 영문 머리글자를 딴 협약으로, 서비스 목표와 책임을 정의
...
- 기존 한계: 추상 질문은 감리인별 판정 편차·증거 누락 유발
- **배경/필요성**: 준거별 검증 질문·증거·판정의 표준화
...
- 점검 준거를 검증 가능한 질문·증거·판정 기준으로 변환한다.
- 문서·인터뷰·시스템 실행 결과를 교차 확인한다.
- 위험 기반 표본과 ID로 판정부터 시정조치까지 추적한다.
...
1. 금융 API는 인증 실패 로그를 **위험 표본**으로 점검
