...
- 정의/개념: 법령•계약•표준의 **점검 준거**를 연결한 **감리 점검표**이다.
- 배경/필요성: 감리인 경험•추상 질문만으로는 **판정 재현성**을 확보할 수 없다.

#### 한줄 요약
- 무엇을 물어보고 어떤 증거가 나오면 적합한지 미리 정해 누구나 같은 방식으로 점검하게 한다.
...
- 동일 증거의 같은 판정을 위한 **준거성**이 핵심이다.
- 형식적 준수와 실제 작동을 가르는 **실증성**이 핵심이다.
- 판정부터 시정조치까지 **추적성**을 유지하고 **위험 표본**을 선정한다.

#### 한줄 요약
- “보안이 적절한가”가 아니라 “관리자 계정의 다중인증 설정 로그가 있는가”처럼 확인 가능하게 물음이 핵심이다.
...
| 구성요소 | 책임 |
|:---|:---|
| 항목 ID•점검 준거 | **식별자(ID)**로 법령•계약을 단계•영역•표준과 연결 |
| 검증 질문 | **검증 질문**으로 판정 근거를 구체화 |
| 증거•표본 | **감리 증거**와 표본 선정 기준 명시 |
| 판정 기준 | **판정 기준**으로 적합•부적합 조건 정의 |
| 지적•시정조치 | 식별자로 수정 결과•확인 증거를 원인•영향과 연결 |

#### 한줄 요약
- 질문 하나에 근거 규정, 볼 증거, 합격 조건, 고칠 결과를 한 줄로 연결하는 것이 핵심이다.
...
- **준거•질문 설계**: 점검 준거를 검증 질문과 판정 조건으로 변환하는 단계이다.
- **위험 표본 선정**: 모집단에서 발생 가능성과 영향이 큰 대상을 우선 선택하는 단계이다.
- **판정•시정 기준 적용**: 준거와 증거의 차이로 문제와 완료 조건을 확정하는 단계이다.
...
1. **준거•질문 설계**: 점검 준거를 검증 질문과 판정 조건으로 변환한다.
2. **위험 표본 선정**: **모집단** 위험에 따라 **표본 점검** 대상과 수량을 결정한다.
3. **판정•시정 기준 적용**: 준거와 증거의 차이로 문제•완료 기준을 확정한다.
4. **조치 재검증**: 최초 판정 기준으로 수정 결과와 잔여 위험을 확인한다.

#### 한줄 요약
- 질문마다 미리 정한 증거를 받아 기준과 대조하고 불합격 항목은 같은 ID로 수정까지 확인하는 것이 핵심이다.
...
|:---|:---|:---|:---|
| 적용 기준 | **사업관리 점검** | **기술 점검** | **운영전환 점검** |
| 핵심 특징 | 계약•일정•범위•변경 통제 | 아키텍처•코드•보안 준거 | 시험 완료•운영 전환 준비 |
| 선택 원칙 | **영역별 점검 선택**으로 관리 책임 검증 | 영역별 선택으로 기술 설계 검증 | 영역별 선택으로 운영 준비 검증 |

#### 한줄 요약
- 사업을 제대로 통제했는지, 시스템을 제대로 만들었는지, 실제 운영할 준비가 됐는지를 나눠 봄이 핵심이다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 추상 질문으로 판정 편차 발생 | **재현성 통제** | 점검 결과의 일관성 확보 |
| 무작위 표본의 **표본 편향** | **고위험 표본 추가** | 통제 실패 검출 가능성 향상 |
| ID가 증거•조치와 끊긴 상태 | **문서-실제 불일치**까지 준거•증거•판정•결과로 연결 | 감리 전 과정 추적성 확보 |
...
- **위험 기반 점검**과 **시정 추적**을 결합한 **점검표 설계 결정**이 핵심이다.

#### 한줄 요약
- 체크 표시 수가 아니라 같은 증거를 보면 같은 판정이 나오도록 질문과 합격 조건을 설계하는 것이 핵심이다.
