...
- 정의/개념: 법령•계약•표준의 **점검 준거**를 검증 질문•필요 증거•표본•판정 기준•시정조치로 연결한 감리 통제 목록이다.
- 배경/필요성: 감리인 경험•추상 질문만으로는 **판정 재현•증거 완전성 확보할 수 없다**.
...
- 동일 증거의 같은 판정을 위한 **준거•질문•판정 기준 연결**이 핵심이다.
- 형식적 준수와 실제 작동을 가르는 **증거 교차 확인**이 핵심이다.
- 판정부터 시정조치까지 잇는 **위험 표본•항목 ID**가 핵심이다.
...
2. **위험 표본 선정**: 모집단 위험에 따라 표본 대상과 수량을 결정한다.
...
- 고위험 준거는 **검증 질문•판정 기준**으로 바꾸고 증거•조치를 **동일 ID로 추적**이 핵심이다.
