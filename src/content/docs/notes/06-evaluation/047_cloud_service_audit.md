...
- **IaaS(Infrastructure as a Service)**: 제공자가 물리·가상 인프라를 운영하고 이용자가 OS 이상을 관리하는 서비스
- **PaaS(Platform as a Service)**: 제공자가 실행 플랫폼까지 운영하고 이용자가 앱·데이터를 관리하는 서비스
- **SaaS(Software as a Service)**: 제공자가 응용서비스를 운영하고 이용자가 계정·데이터·기능 설정을 관리하는 서비스
...
- **배경/필요성**: 직접 통제할 수 없는 계층의 책임 공백 방지
...
- 서비스 모델에 따라 제공자·이용자 책임이 이동한다.
- API·IaC·로그로 변하는 구성 상태 검증한다.
- 테넌트·지역·재위탁 경계를 따라 데이터 추적한다.
- SLA부터 복구·이전·삭제까지 계약 증거 확인한다.
...
| 비교축 | IaaS | PaaS | SaaS |
|:---|:---|:---|:---|
| 핵심 특징 | OS 이상을 이용자가 관리 | 앱·데이터를 이용자가 관리 | 계정·데이터·설정만 이용자 관리 |
| 적용 기준 | 인프라 직접 제어 필요 | 개발 플랫폼 구성 필요 | 완성된 응용 서비스 이용 |
| 주요 위험 | 권한 과다·관리 포트 노출 | 종속성·비밀정보 노출 | 테넌트 격리·무단 반출 |
...
- IaaS: IaC와 실제 자원 설정의 차이 검증
- SaaS 종료: 데이터 이전·잔여 사본 삭제 확인

### 쉽게 이해하기 (학습용)
- 코드에는 닫혀 있지만 실제 보안그룹이 열려 있지 않은지 배포 상태와 변경 로그를 대조함
- 계약 종료 시험으로 데이터를 다른 서비스로 옮길 수 있는지와 제공자 사본이 삭제되는지 확인함

## Ⅶ. 결론

- 공유 책임·실제 구성·출구 증거로 위험 판정
