...
- **정의/개념**: 전체 측정시간 중 정상 서비스시간의 비율
- **배경/필요성**: 백분율 목표를 허용 중단시간으로 환산함
...
- 장애 빈도와 복구시간이 함께 가용성을 낮춘다
- 나인 수 하나가 늘면 허용 중단은 10분의 1이다
- 측정 경계·기간·계획 정지 기준이 값을 바꾼다
- 직렬 SPOF는 전체 가용성 상한을 낮춘다
...
1. SLA는 계획 정지의 중단 포함 여부 명시
2. 결제 경로는 직렬 SPOF의 가용성 곱 계산

### 쉽게 이해하기 (학습용)
- 정기 점검을 위해 가동을 멈추는 시간까지 서비스 장애 시간으로 취급해 과도한 배상금을 물지 않도록 계약서 서식을 정리합니다.
- 여러 서버를 거쳐 동작하는 복잡한 시스템에서 단 하나만 고장 나도 전체가 멈추는 취약 부위를 찾아내어 안전 장치를 적용합니다.

## Ⅶ. 결론

- 허용 중단시간으로 이중화·복구 수준 결정
