...
- 정의/개념: 전체 측정시간 중 **정상 서비스 제공 시간**이 차지하는 비율을 나타내는 연속성 지표이다.
...
- 가동•중단시간 기반 **가용성 산식**이 핵심이다.
- 나인 수 증가에 따른 **허용 중단시간 감소**가 핵심이다.
- 측정 경계•계획 정지에 따른 **산정값 변화**가 핵심이다.
...
- **실측•목표**: $A=U/T$, $D_{budget}=(1-A_{target})T$이다.
- **정상상태**: $A=MTBF/(MTBF+MTTR)$이다.
- **직렬•병렬**: $A_s=\prod A_i$, $A_p=1-\prod(1-A_i)$이다.
...
1. **가동•중단 구간 판정•합산**: 중복•유실•**부분 장애**를 처리한다.
2. **실측 가용성•중단 예산 산출**: $A$와 허용 중단시간을 계산한다.
3. **경로 가용성•의존성 분석**: 직렬•병렬과 **공통 원인**을 분석한다.
...
> 요약: **나인 수 증가**에 따른 연간 **허용 중단시간 급감**이 핵심이다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 의존성을 누락한 **가용성 과대 계산** | **IEC 61703:2016** 적용 | 지표•경로의 **계산 정렬** |
| 측정 경계가 다른 **SLA 분쟁** | **ISO/IEC 20000-1:2018+Amd1:2024** 적용 | 가용성 **계획•검토 체계화** |
...
- 연간 중단 예산이 8.76시간이면 **99.9%**, 52.6분이면 **99.99%**이상을 적용한다.
