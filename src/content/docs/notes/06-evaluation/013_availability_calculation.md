...
date: "2026-08-06T17:09:33+09:00"
tags:
  - "notes-evaluation"
weight: 13
extra:
...
선의 의미: 서비스•측정 경계 아래 중단 판정 규칙과 구성 의존성을 분리하고, 상태•사건•시간 로그 및 가용성•허용 중단시간 산정에 함께 반영하는 구조
...
- **실측•목표**: $A=U/T$, $D_{budget}=(1-A_{target})T$
- **정상상태**: $A=MTBF/(MTBF+MTTR)$
- **직렬•병렬**: $A_s=\prod A_i$, $A_p=1-\prod(1-A_i)$
...
1. **가동•중단 구간 판정•합산**: 중복•유실•**부분 장애** 처리
2. **실측 가용성•중단 예산 산출**: $A$와 허용 중단시간 계산
3. **경로 가용성•의존성 분석**: 직렬•병렬과 **공통 원인** 분석
4. **목표•복구•이중화 결정**: 미달 원인별 구조•운영 개선 선택
