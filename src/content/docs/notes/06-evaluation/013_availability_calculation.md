...
date: "2026-07-31T05:11:43+09:00"
tags:
  - "notes-evaluation"
weight: 13
extra:
...
sequenceDiagram
  ...
  O->>M: 1. 측정 경계·중단 규칙 전달
  M->>C: 2. 가동·중단 구간 전달
  C->>A: 3. 실측 가용성·허용시간 전달
  A->>D: 4. 경로 가용성·의존성 전달
  D-->>O: 목표·복구·이중화 결정
```

1. **측정 경계·중단 규칙 전달**: 대상·기간·**계획 정지·예외** 제공
2. **가동·중단 구간 전달**: 중복·유실·**부분 장애** 처리 결과 제공
3. **실측 가용성·허용시간 전달**: $A$와 **중단 예산** 산출 결과 제공
4. **경로 가용성·의존성 전달**: 직렬·병렬과 **공통 원인** 분석 제공
