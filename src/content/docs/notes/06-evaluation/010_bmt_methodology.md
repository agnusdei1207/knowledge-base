...
- 정의/개념: 동일한 환경•데이터•부하에서 **도입 후보의 성능•적합성**을 비교하는 실증 시험한다.
- 배경/필요성: 벤더 제안 수치는 환경•부하 차이로 **실제 도입 성능** 공정하게 비교하기 어렵다.
...
- 환경•데이터•부하의 **후보별 동일 조건**이 핵심이다.
- 항목•가중치•합격선의 **사전 동결**이 핵심이다.
- 원시값•반복•편차의 **감사 가능 증거**가 핵심이다.
...
1. **항목•가중치•합격선 동결**: 변경 통제•탈락조건을 확정한다.
2. **공통 환경•데이터 구성**: 후보별 **시작 상태**를 통일한다.
3. **후보별 반복 시험 실행**: 순서 효과와 **측정 편차**를 확인한다.
4. **원시값•편차•증거 측정**: 로그•설정•**원시 데이터**를 보존한다.
5. **점수•위험•민감도 판정**: 가중치 변화에 따른 선정 안정성을 검증한다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 후보별 절차가 달라지는 **시험 재현성 저하** | **ISO/IEC/IEEE 29119-2:2021** 적용 | **재현•감사 가능성** 확보 |
...
- 필수 조건 충족 후보 중 **대표 부하 반복 성능**과 **민감도 안정성**이 높은 제품 선정이 핵심이다.
