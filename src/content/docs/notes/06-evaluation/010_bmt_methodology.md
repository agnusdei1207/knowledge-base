...
- **벤치마크 테스트(BMT)**: 동일한 업무 부하와 측정 조건에서 도입 후보의 성능·적합성을 비교하는 시험이다.
...
2. **공통 환경·데이터 전달**: 후보별 **시작 상태** 통일
3. **반복 실행 결과 전달**: 순서 효과와 **측정 편차** 제공
4. **원시값·편차·증거 전달**: 로그·설정·**원시 데이터** 보존
...
- **개념 증명(PoC)**: 제한된 범위의 구현과 실험으로 기술의 실현 가능성과 핵심 가정을 검증하는 활동이다.
...
- **TPC**: 거래처리·데이터 시스템의 벤치마크 규격과 검증 결과를 공개하는 비영리 단체이다.
