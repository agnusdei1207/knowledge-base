...
date: "2026-07-25T22:23:00+09:00"
tags:
  - "notes-evaluation"
weight: 22
extra:
...
- **VIP(Virtual IP Address)**: 활성 노드가 바뀌어도 클라이언트가 같은 주소로 접속하게 하는 가상 IP 주소
...
- 페일오버는 장애 판정과 전환 속도를 우선한다.
- 페일백은 데이터 동기화와 안정성을 우선한다.
- 오진·플래핑을 막도록 임계값과 유예를 적용한다.
- 펜싱 성공 뒤 새 활성 노드의 쓰기를 허용한다.
- 경로 전환과 세션·데이터 검증까지 완료해야 한다.
...
| 주요 위험 | 오진·이중 쓰기·세션 손실 | 데이터 역전·플래핑·재중단 |
