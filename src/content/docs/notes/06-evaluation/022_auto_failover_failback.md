...
- 기존 한계: 장애 승계만 자동화하면 복구된 원래 노드로 전환하는 과정에서 데이터 불일치와 재중단이 발생할 수 있다.
...
- 장애 전환 시간을 줄이면서 데이터 충돌을 방지하기 위해 **장애 판정·펜싱·복제 동기화·전환 용량·복귀 조건**을 검토하고, 안전 조건이 확인된 서비스에 자동 페일오버·페일백을 적용해야 한다.
