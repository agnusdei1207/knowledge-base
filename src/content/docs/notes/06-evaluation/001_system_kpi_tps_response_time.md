...
- 정의: 서비스의 **용량·지연·안정성을 판정하는 측정값**
- 목적: 평균이 숨기는 **포화·꼬리 지연·중단 식별**
...
- 처리량·지연·오류의 **다차원 성능 판단**
- 평균·p95·p99의 **꼬리 지연 가시화**
- SLI·SLO·SLA의 **목표·책임 추적**
...
| 가용성·복구시간 | 정상 시간·중단·MTTR |
| SLI·SLO·SLA | 측정값·목표·계약 책임 |
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 거래·요청 경계 혼용으로 **TPS 과대 산정** | 업무 거래·성공·측정 기간을 고정하고 RPS와 분리 | 처리량의 **분모·업무 의미 일관성** 확보 |
| 평균 응답시간이 일부 장기 지연을 숨기는 **꼬리 지연 은폐** | 평균과 **p95·p99·오류율** 병행 측정 | 사용자 체감 저하와 포화 구간 식별 |
| 정상 부하만 측정해 피크 용량을 오판하는 **부하 대표성 부족** | 피크 업무 비율·동시성·지속시간을 재현 | 목표 SLO를 만족하는 **실제 처리 용량** 검증 |
...
- **업무 경계·TPS·p99·오류율·가용성**으로 SLO 충족 여부를 결정한다.
