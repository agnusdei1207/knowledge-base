...
- **핵심 성과 지표(KPI)**: 서비스 목표 달성 여부를 정량적으로 판정하는 핵심 측정값이다.
- **서비스 수준 지표(SLI)**: 가용성·지연·오류율처럼 서비스 수준을 직접 측정한 값이다.

</details>

- 정의/개념: **시스템 KPI**는 TPS·응답시간·동시 사용자·자원 사용률·오류율을 함께 측정하여 서비스 성능 목표 달성 여부를 판정하는 핵심 성과 지표
...
- **초당 트랜잭션 처리 건수(TPS)**: 1초에 성공적으로 완료한 업무 거래 수이다.
- **초당 요청 처리 건수(RPS)**: 1초에 처리한 개별 요청 수이다.
...
- **서비스 수준 목표(SLO)**: SLI에 대해 내부적으로 달성해야 할 목표값이다.
- **서비스 수준 협약(SLA)**: 측정 기준·목표·위반 책임을 고객과 합의한 계약이다.
