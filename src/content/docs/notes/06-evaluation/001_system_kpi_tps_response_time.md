...
- **핵심 성과 지표(Key Performance Indicator, KPI)**: 서비스 목표 달성을 판정하는 수치입니다.
- **초당 트랜잭션 처리 건수(Transactions Per Second, TPS)**: 1초간 완료된 거래 수
- **초당 요청 처리 건수(Requests Per Second, RPS)**: 1초간 처리한 요청 수
- **응답시간(Response Time)**: 요청부터 응답까지 소요 시간
- **처리량(Throughput)**: 단위 시간 완료 작업량
- **가용성(Availability)**: 정상 제공 시간의 비율
- **백분위수(Percentile, p95·p99)**: 지연 시간의 분포
- **서비스 수준 목표(Service Level Objective, SLO)**: 내부에서 정한 서비스 성능 목표입니다.
- **서비스 수준 협약(Service Level Agreement, SLA)**: 목표 위반 책임까지 정한 외부 약속입니다.
- **웹 애플리케이션 서버(Web Application Server, WAS)**: 업무 로직을 실행하는 서버입니다.
- **데이터베이스(Database, DB)**: 업무 데이터를 저장하고 조회하는 시스템입니다.
- **추적(Trace)**: 한 요청이 여러 구간을 지난 시간과 경로 기록입니다.

## Ⅰ. 개요

- **정의/개념**: 용량·지연·가용성을 판정하는 측정 기준
- **배경/필요성**: 평균만으로는 포화·꼬리 지연을 숨김
...
- p95·p99는 소수의 긴 지연까지 드러낸다
- 가용성은 서비스 지속 가능성을 보여준다
...
| 판단 기준 | 처리량 | 응답시간 | 가용성 |
|:---|:---|:---|:---|
| 핵심 역할 | 완료 건수로 처리 용량 판단 | 요청 시간으로 지연 판단 | 정상 시간으로 지속성 판단 |
| 적용 기준 | 업무 거래 처리량 확인 | 사용자 체감 속도 측정 | 서비스 지속 여부 측정 |
| 주요 위험 | 실패 건수 제외 | 평균의 꼬리 지연 은폐 | 짧은 측정 기간의 왜곡 |
...
2. 운영 지연 분석은 요청별 구간 추적 수집

### 쉽게 이해하기 (학습용)
- 출시 전에 가상 사용자를 늘리며 처리량과 느린 요청의 한계를 함께 찾습니다.
- 느린 요청 하나가 어느 서버 구간에서 오래 머물렀는지 기록을 따라갑니다.

## Ⅶ. 결론

- 목표 TPS·p99 지연으로 자원 용량 결정
