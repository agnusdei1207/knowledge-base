...
- **핵심 성과 지표(Key Performance Indicator, KPI)**: 서비스 목표의 달성 여부를 정량적으로 판정하는 핵심 측정값이다.
- **초당 트랜잭션 처리 건수(Transactions Per Second, TPS)**: 시스템이 1초 동안 완료한 업무 거래 수를 나타내는 처리량 지표다.
- **초당 요청 처리 건수(Requests Per Second, RPS)**: 시스템이 1초 동안 처리한 개별 요청 수를 나타내는 부하·처리량 지표다.
...
- **서비스 수준 협약(Service Level Agreement, SLA)**: 서비스 목표와 측정 기준, 위반 시 책임을 고객과 합의한 계약이다.
- **웹 애플리케이션 서버(Web Application Server, WAS)**: 요청을 받아 업무 로직을 실행하고 데이터 계층과 연동하는 서버다.
- **데이터베이스(Database, DB)**: 업무 데이터를 구조적으로 저장·조회·관리하는 시스템이다.
- **추적(Trace)**: 한 요청이 여러 구간을 지난 시간과 경로 기록입니다.

## Ⅰ. 개요

- 정의/개념: 용량·지연·가용성을 판정하는 측정 기준
- 기존 한계: 단일 평균값만으로는 시스템의 처리 한계와 일부 요청의 꼬리 지연, 서비스 중단을 구분하기 어렵다.
