...
- 정의/개념: **최상위 사건**을 원인 조합으로 분해하는 **결함수 분석**이다.
- 배경/필요성: 단일 원인 목록만으로는 복합 사고를 일으키는 최소 조합을 식별하기 어렵다.
...
- **하향식 분석**으로 최상위 사건의 원인을 분해한다.
- 논리 게이트로 복합 원인 관계를 표현한다.
- **정성•정량 분석**으로 최소 컷셋과 중요도를 평가한다.
- **공통 원인 반영**으로 독립성 가정의 과소평가를 보완한다.
...
| 구성요소 | 책임 |
|:---|:---|
| 최상위 사건•경계 | **최상위 사건**과 분석 범위 |
| AND•OR 논리 게이트 | **논리곱 게이트**와 **논리합 게이트** |
| 중간 사건 | **중간 사건** 표현 |
| 기본 사건•공통원인 | **기본 사건**과 의존성 식별 |
...
- **최상위 사건•경계 정의**: 분석할 사고 상태와 포함 범위 및 가정을 확정하는 단계이다.
- **기본 사건•논리 관계 분해**: 최상위 사건을 기본 원인과 AND•OR 관계로 구조화하는 단계이다.
- **최소 컷셋 도출**: 최상위 사건을 일으키기에 충분한 최소 원인 조합을 계산하는 단계이다.
- **확률•중요도•공통원인 평가**: 기본 사건 확률과 의존성을 반영해 영향도를 계산하는 단계이다.
- **설계 통제•잔여위험 결정**: 우선 경로를 차단하고 남은 위험의 수용 여부를 판단하는 단계이다.
...
1. **최상위 사건•경계 정의**: 사고 상태•범위•가정을 확정한다.
2. **기본 사건•논리 관계 분해**: **컷셋** 계산에 필요한 원인과 논리를 구조화한다.
3. **최소 컷셋 도출**: **최소 컷셋**을 계산한다.
4. **확률•중요도•공통원인 평가**: **중요도 평가**에 의존성을 반영한다.
5. **설계 통제•잔여위험 결정**: 경로를 차단하고 재평가한다.
...
| 위험 분석법 | FTA | FMEA |
|:---|:---|:---|
| 적용 기준 | **FTA 적용** | **고장 형태 및 영향 분석** |
| 핵심 특징 | 하향식 원인 조합 분석 | 상향식 고장 영향 분석 |
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 분석자마다 다른 논리 표기•절차 | **IEC 61025** 적용 | 모델 일관성 확보 |
| 사건 독립성 과신 | **독립 사건 가정**을 검토하고 **공통 원인 고장**과 **조건부 확률** 반영 | 사고확률 과소평가 방지 |
| 트리 확대로 생기는 원인 누락 | **분석 경계**와 검토 이력 관리 | 추적성 향상 |
...
- **위험 감소 우선순위**에 따라 **개선 순위 결정**을 수행한다.
