...
date: "2026-07-25T22:27:00+09:00"
tags:
  - "notes-evaluation"
weight: 26
extra:
...
- **FMEA(Failure Mode and Effects Analysis)**: 개별 고장 모드에서 시작해 시스템 영향과 우선순위를 분석하는 상향식 기법

## Ⅰ. 개요

- **정의/개념**: 사고 결과에서 원인 조합을 하향 분석
- **배경/필요성**: 최소 컷셋과 취약 경로의 우선 통제
...
- 결과에서 원인을 찾는 연역적 하향식 분석이다.
- 단일 원인보다 복합 원인 조합을 표현한다.
- 최소 컷셋으로 사고에 충분한 경로를 식별한다.
- 기본 사건 확률로 최상위 위험을 정량화한다.
- 독립성 오판은 사고 확률을 과소평가한다.
...
| 비교축 | FTA | FMEA |
|:---|:---|:---|
| 핵심 특징 | 사고 결과에서 원인 조합을 하향 분석 | 고장 모드에서 영향을 상향 분석 |
| 적용 기준 | 복합 사고 경로·확률을 분석할 때 | 부품별 고장 영향과 예방을 점검할 때 |
| 주요 위험 | 트리 경계·독립성 가정 오류 | 복합 원인 상호작용 누락 |
...
- **사례 1**: 항공기 제어 상실의 최소 컷셋 분석
- **사례 2**: 이중 전원의 공통 배전반 고장을 반영
- **사례 3**: 단일 기본 사건 컷셋에 이중화 적용
- **사례 4**: 설계 변경 뒤 트리와 확률을 다시 계산

### 쉽게 이해하기 (학습용)

- 사례 1은 센서·연산·전원·통신 고장의 어떤 조합이 제어 상실에 충분한지 찾는다.
- 사례 2는 전원 두 개가 있어도 같은 배전반 고장으로 함께 꺼지는 경로를 독립 사건처럼 계산하지 않는다.
- 사례 3은 한 부품 고장만으로 사고가 나는 1차 컷셋을 복제하거나 안전 정지 기능으로 제거한다.
- 사례 4는 부품과 연결 관계가 바뀌면 기존 최소 컷셋과 사고 확률도 달라지므로 분석을 갱신한다.

## Ⅶ. 결론

- 복합 사고면 최소 컷셋과 공통 원인을 우선 제거한다.
