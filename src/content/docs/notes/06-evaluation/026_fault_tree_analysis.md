...
- **결함수 분석(Fault Tree Analysis, FTA)**: 원하지 않는 최상위 사건에서 출발하여 그 원인 사건 조합을 논리 게이트로 분해하는 하향식 연역 분석한다.
- **최상위 사건**: FTA가 발생 원인과 가능성을 밝히려는 시스템 수준의 사고나 고장이다.

</details>

- 정의/개념: 최상위 사고를 원인 조합으로 분해하는 **연역 분석**이다.
- 배경/필요성: 단일 원인 목록만으로는 **복합 사고 최소 조합 식별할 수 없다**.
...
- 최상위 사건의 **하향식 원인 분해**가 핵심이다.
- AND•OR 게이트의 **복합 원인 논리**가 핵심이다.
- 최소 컷셋•중요도의 **정성•정량 평가**가 핵심이다.
...
- **컷셋**: 최상위 사건을 발생시키기에 충분한 기본 사건의 조합한다.
- **최소 컷셋**: 구성 사건을 하나라도 빼면 최상위 사건이 발생하지 않는 최소 원인 조합한다.
...
2. **기본 사건•논리 관계 분해**: 원인•AND•OR을 구조화한다.
3. **최소 컷셋 도출**: 충분한 최소 원인조합을 계산한다.
4. **확률•중요도•공통원인 평가**: 의존성•민감도를 반영한다.
5. **설계 통제•잔여위험 결정**: 경로를 차단•재평가한다.
...
- **고장 형태 및 영향 분석(Failure Mode and Effects Analysis, FMEA)**: 개별 구성요소의 고장 모드에서 출발하여 상위 시스템 영향을 추적하는 상향식 분석한다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 분석자마다 다른 **논리 표기•절차** | **IEC 61025:2006 적용** | 모델 **일관성•검증성** 확보 |
| **사건 독립성 과신** | **공통원인•조건부확률 반영** | **사고확률 과소평가** 방지 |
| 트리 확대로 생기는 **원인 누락** | **모듈화•경계•검토 이력 관리** | **추적성•유지보수성** 향상 |
...
- 확률•중요도 높은 **최소 컷셋**부터 경로 차단, **공통원인**을 분리한다.
