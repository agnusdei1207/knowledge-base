...
- **기능점수(Function Point, FP)**: '에프피'로 읽고 사용자 논리 기능을 계수한 규모 단위를 영문 머리글자로 표시
- **애플리케이션 경계**: 측정 대상과 외부 사용자·시스템을 나누는 논리적 선
- **내부 논리 파일(Internal Logical File, ILF)**: '아이엘에프'로 읽고 측정 대상이 유지하는 논리 데이터 집합을 표시
- **외부 연계 파일(External Interface File, EIF)**: '이아이에프'로 읽고 외부 관리 데이터를 참조하는 집합을 표시
- **외부 입력(External Input, EI)**: '이아이'로 읽고 경계 밖 입력이 내부 데이터·동작을 변경하는 처리를 표시
- **외부 출력(External Output, EO)**: '이오'로 읽고 계산·가공 결과를 경계 밖으로 내보내는 처리를 표시
- **외부 조회(External Inquiry, EQ)**: '이큐'로 읽고 데이터 변경 없이 조회 결과를 반환하는 처리를 표시
- **미조정 기능점수(Unadjusted Function Point, UFP)**: '유에프피'로 읽고 유형·복잡도 가중치 합계를 표시
- **인월(Man-Month, MM)**: '맨먼스' 또는 '엠엠'으로 읽고 한 사람의 한 달 작업량을 표시
- **생산성(Productivity)**: 투입 공수 한 단위로 구현한 기능 규모
- **코드 줄 수(Lines of Code, LOC)**: '엘오씨'로 읽고 소스 코드 줄 수 기반 구현 규모를 표시

## Ⅰ. 개요

- **정의/개념**: 사용자 관점의 데이터·트랜잭션 기능을 계수해 **SW 기능 규모 산정**
- **배경/필요성**: 코드 줄 수는 개발 전 산정이 어렵고 언어별로 달라지는 **견적·비교 한계 해소**
...
1. 변경 사업은 **신규·수정·삭제 기능을 분리 산정**
2. 예상 공수는 **FP÷조직 기준 생산성으로 추정**

### 쉽게 이해하기 (학습용)

- 변경 사업은 기존 기능 전체가 아니라 신규·수정·삭제 범위만 산정함
- FP를 유사 사업의 조직 생산성으로 나눠 예상 공수를 추정함

## Ⅶ. 결론

- 견적은 **FP 규모**, 수행 효율은 **FP/MM 생산성** 적용
