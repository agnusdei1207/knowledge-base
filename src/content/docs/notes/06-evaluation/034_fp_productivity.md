...
date: "2026-08-02T22:34:00+09:00"
tags:
  - "notes-evaluation"
weight: 34
extra:
...
- **기능점수 기반 규모·생산성 산정**: 사용자 관점의 논리 데이터와 트랜잭션 기능을 FP로 계수해 소프트웨어 규모를 구하고, 이를 투입 공수로 나눠 생산성을 비교하는 방법이다.
- **생산성**: 투입 공수 한 단위로 구현한 기능 규모를 나타내며 보통 FP/MM으로 계산하는 지표이다.
...
- **미조정 기능점수(UFP)**: 기능 유형별 개수와 복잡도 가중치를 합산한 조정 전 기능점수이다.
...
- **내부 논리 파일(ILF)**: 측정 대상 시스템이 생성·수정·삭제하며 유지하는 논리 데이터 집합이다.
- **외부 연계 파일(EIF)**: 다른 시스템이 유지하고 측정 대상 시스템이 참조하는 논리 데이터 집합이다.
- **외부 입력·출력·조회(EI·EO·EQ)**: 내부 상태를 변경하는 입력, 가공 결과를 제공하는 출력, 상태를 바꾸지 않는 조회 기능이다.
...
- **FP 기반 규모**: 사용자에게 제공하는 논리 기능을 세어 언어·구현 방식과 독립적으로 산정한 규모이다.
- **코드 줄 수(LOC)**: 소스 코드 줄 수로 구현 결과의 물리 규모를 나타내는 지표이다.
- **인월(MM)**: 한 사람이 한 달 동안 투입하는 작업량을 나타내는 노력 단위이다.
...
- **기술 독립성**: 기능점수가 프로그래밍 언어와 코드 작성 방식보다 사용자 기능을 기준으로 규모를 나타내는 성질이다.
