...
- 정의/개념: 사용자 기능을 계수하는 **기능점수 기반 규모•생산성 산정**이다.
- 배경/필요성: **LOC**로는 개발 전 규모를 알기 어렵고 **생산성** 비교도 언어 차이에 영향을 받는다.
...
- **애플리케이션 경계**를 통한 내부 데이터와 외부 참조를 구분한다.
- 기능 유형별 개수와 가중치로 **UFP**를 산출한다.
- **기술 독립성**을 활용하고 공수는 **인월**로 통일한다.
...
선의 의미: 애플리케이션 경계 아래 데이터 기능과 트랜잭션 기능을 분리하고, 두 기능 유형을 복잡도 가중치와 공통 산정 기준에 결합한 기능점수 구조를 나타낸다.

| 구성요소 | 책임 |
|:---|:---|
| 애플리케이션 경계 | 측정 대상과 외부 시스템의 관리 주체 확정 |
| 데이터 기능 | 내부 유지 **ILF**와 외부 참조 **EIF** 식별 |
| 트랜잭션 기능 | 사용자 목적별 **EI•EO•EQ** 식별 |
| 복잡도 가중치 | 기능 유형과 복잡도별 개수를 **UFP**로 변환 |
| 산정 기준 | 계수 규칙•판정 근거•검토 이력 유지 |
...
- **산정 목적•애플리케이션 경계 확정**: 견적•계약•성과 목적과 측정 대상의 논리 범위를 정하는 단계이다.
- **데이터•트랜잭션 기능 식별**: ILF•EIF와 EI•EO•EQ를 중복 없이 찾는 단계이다.
- **복잡도 가중치•FP 산출**: 유형별 개수와 복잡도 점수를 합산하는 단계이다.
- **경계•유형•누락 동료 검토**: 다른 산정자가 계수 근거와 누락 여부를 확인하는 단계이다.
- **확정 FP•투입공수 생산성 산출**: 확정 규모를 인월로 나눠 FP/MM를 계산하는 단계이다.
...
1. **산정 목적•애플리케이션 경계 확정**: **애플리케이션 경계**를 결정한다.
2. **데이터•트랜잭션 기능 식별**: **기능 식별**로 ILF•EIF•EI•EO•EQ를 구분한다.
3. **복잡도 가중치•FP 산출**: **복잡도 가중치**를 적용한다.
4. **경계•유형•누락 동료 검토**: **동료 검토**로 판정 근거를 교정한다.
5. **확정 FP•투입공수 생산성 산출**: FP를 인월로 나눠 FP/MM를 계산한다.
...
|:---|:---|:---|:---|
| 적용 기준 | **FP 적용** | **LOC 적용** | 팀 내부 반복 계획 |
| 핵심 특징 | 기능 규모 | 코드 물리 규모 | 상대 작업량 |
| 한계 | 경계•유형 판정 편차 | 언어•생성 방식 종속 | 팀 간 비교 곤란 |

> **생산성** = 기능점수(FP) / 투입 공수(MM)

**MM 적용**으로 기능 규모 대비 개발 공수를 산출한다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 경계 차이에 따른 기능 중복•누락 | **경계 오류**와 **기능 중복 계수** 점검 | 기능 계수 일관성 확보 |
| 개발 유형•기술 차이에 따른 성과 왜곡 | **생산성 비교 조건** 통일 | 비교 타당성 향상 |
| 불명확한 요구사항에 따른 규모 변동 | **요구 변경 추적** | 규모•비용 추적성 확보 |
