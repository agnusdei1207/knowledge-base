...
date: "2026-08-05T13:46:00+09:00"
tags:
  - "notes-evaluation"
weight: 34
extra:
...
선의 의미: 애플리케이션 경계 아래 데이터 기능과 트랜잭션 기능을 분리하고, 두 기능 유형을 복잡도 가중치와 공통 산정 기준에 결합한 기능점수 구조
...
sequenceDiagram
  ...
  E->>E: 1. 산정 목적•애플리케이션 경계 확정
  E->>S: 산정 경계 전달
  S->>S: 2. 데이터•트랜잭션 기능 식별
  S->>C: 기능 목록 전달
  C->>C: 3. 복잡도 가중치•FP 산출
  C-->>E: 산출 결과 반환
  E->>R: 산정 근거 검토 요청
  R->>R: 4. 경계•유형•누락 동료 검토
  R-->>E: 검토 결과 반환
  E->>E: 5. 확정 FP•투입공수 생산성 산출
```

**동작 원리**

- **1. 산정 목적•애플리케이션 경계 확정**: 산정 목적과 **범위** 결정
- **2. 데이터•트랜잭션 기능 식별**: ILF•EIF와 **EI•EO•EQ** 구분
- **3. 복잡도 가중치•FP 산출**: 기능별 가중치를 적용해 규모 계산
- **4. 경계•유형•누락 동료 검토**: 중복•누락과 판정 근거 교정
- **5. 확정 FP•투입공수 생산성 산출**: FP를 인월로 나눠 FP/MM 계산
