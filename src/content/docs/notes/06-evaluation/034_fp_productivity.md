...
- 정의/개념: 사용자 관점의 **데이터•트랜잭션 기능**을 계수해 기술 독립적 SW 규모를 산정하는 기법이다.
- 배경/필요성: 언어•구현 방식에 종속된 LOC로는 개발 전 **규모•생산성 비교할 수 없다**.
...
- **ILF•EIF•EI•EO•EQ 계수**와 복잡도 가중치에 따른 FP 산출이 핵심이다.
- **FP는 규모, FP/MM는 생산성**으로 구분한 견적•성과 적용한다.
...
- **1. 산정 목적•애플리케이션 경계 확정**: 산정 목적과 **범위**를 결정한다.
- **2. 데이터•트랜잭션 기능 식별**: ILF•EIF와 **EI•EO•EQ**을 구분한다.
- **3. 복잡도 가중치•FP 산출**: 기능별 가중치를 적용해 규모를 계산한다.
- **4. 경계•유형•누락 동료 검토**: 중복•누락과 판정 근거 교정이다.
- **5. 확정 FP•투입공수 생산성 산출**: FP를 인월로 나눠 FP/MM을 계산한다.
...
- 견적은 **FP 규모**를 유사 FP/MM로 나누고, 성과는 동일 조건의 **FP/MM**을 비교한다.
