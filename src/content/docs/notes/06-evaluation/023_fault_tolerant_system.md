...
date: "2026-07-25T22:24:00+09:00"
tags:
  - "notes-evaluation"
weight: 23
extra:
...
- **결함 허용(FT, Fault Tolerance)**: 일부 결함이 발생해도 요구된 출력과 서비스를 계속 제공하는 능력
- **결함 마스킹(Fault Masking)**: 결함이 발생한 구성요소의 잘못된 결과가 외부 출력에 드러나지 않게 하는 처리
- **TMR(Triple Modular Redundancy)**: 동일 기능을 세 모듈에서 수행하고 다수결 결과를 선택하는 삼중 모듈 중복 방식
...
- 결함이 고장으로 전파되기 전에 출력을 마스킹한다.
- 중복 모듈·표결·동기 실행으로 자원 비용이 증가한다.
- 같은 설계 결함은 모든 복제본에 함께 나타날 수 있다.
- 표결기와 공유 입력도 단일 장애점이 될 수 있다.
- 결함 격리 뒤 중복 수준을 다시 복구해야 한다.
