...
- 정의: 확률적 도착·서비스로 **대기길이·지연·용량 예측**
- 목적: 포화 전 **서버 수·대기 한계·용량 결정**
...
|:---|:---|
| 도착 과정·도착률 | 분포·평균·변동성 |
| 대기열·시스템 용량 | 대기 공간·차단 한계 |
| 서비스 분포·서버 수 | 처리시간·병렬 자원 |
| 대기 규칙·이탈 | FIFO·우선순위·포기 |
| 대기·체류·이용률 | $L_q$·$W_q$·$W$·$\rho$ |
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 모형 가정 누락 | **Kendall A/S/c/K/N/D 명시** | 해석·재현성 확보 |
| 포화 상태 적용 오류 | **$\lambda<c\mu$ 안정 조건 검증** | 대기 발산 방지 |
| 실제 분포·의존성 편차 | **실측·시뮬레이션 병행** | 용량 예측 오차 감소 |
...
- **도착분포·서비스분포·서버 수·안정 조건**으로 용량을 결정한다.
