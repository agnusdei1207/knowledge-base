...
- **도착률(Arrival Rate, $\lambda$, 람다)**: 그리스 문자 람다는 도착률의 관례적 기호이며, 단위 시간에 유입되는 평균 요청 수
- **서비스율(Service Rate, $\mu$, 뮤)**: 그리스 문자 뮤는 서비스율의 관례적 기호이며, 서버 하나가 단위 시간에 처리하는 평균 요청 수
- **이용률(Utilization, $\rho$, 로)**: 그리스 문자 로는 부하율의 관례적 기호이며, 처리 능력 중 도착 부하가 차지하는 비율
...
- **M/M/1(엠엠원)**: 첫 M은 마르코프 도착, 둘째 M은 마르코프 서비스, 1은 서버 수를 나타내는 대기행렬 표기
- **M/M/c(엠엠시)**: 두 M은 마르코프 도착·서비스, $c$는 병렬 서버 수를 나타내는 대기행렬 표기
...
- 기존 한계: 평균 처리량만으로는 자원 이용률이 포화에 가까워질 때 대기열과 응답시간이 급증하는 현상을 예측하기 어렵다.
...
- 요청 증가에 따른 대기시간과 처리 용량을 예측하기 위해 **도착률·서비스율·서버 수·이용률·대기 규칙**을 검토하고, 안정 조건을 만족하는 용량 산정에 대기행렬 모형을 활용해야 한다.
