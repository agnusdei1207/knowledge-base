...
- 처리량과 응답시간을 결정짓는 핵심 제약 자원을 식별하고 최우선 제거하는 분석 체계.
...
- 인프라(USE)와 애플리케이션(RED) 관점 지표의 융합 분석을 통한 대기열 원인 규명.
...
- 인프라 자원 임계치 도달 시점 및 반복 재검증을 통한 성능 향상 효과 확인.
...
- 물리 자원 고갈 및 사용자 트랜잭션 처리 상태 분석 기법 융합을 통한 병목 규명.
