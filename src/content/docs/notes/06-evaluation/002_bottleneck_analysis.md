...
- 정의/개념: 처리량•응답시간을 제한하는 **병목 원인**을 측정•식별하는 성능 진단이다.
- 배경/필요성: 평균 사용률만으로는 **대기•임계 경로 원인 식별할 수 없다**.
...
- **상관 분석**: 요청 지연과 자원 포화를 같은 시간축에서 연결하여 원인 관계를 찾는 분석한다.
- **병목 이동 검증**: 한 구간을 개선한 뒤 부하곡선을 다시 측정해 다음 제한 자원과 전체 성능 변화를 확인하는 과정이다.

</details>

- **상관 축**: 요청 지연과 자원 포화를 동일 시간축으로 연결한다.
- **진단 축**: USE•RED로 자원과 서비스를 양방향을 분석한다.
- **검증 축**: 개선 전후 부하곡선으로 병목 이동을 확인한다.
...
| 상관 분석기 | RED•USE•추적 증거로 **병목 후보** 도출 |
...
1. **기준 부하 실행**: 요청률•데이터•환경을 고정한다.
2. **요청 추적•자원 지표 수집**: 종단 지연과 USE 지표를 수집한다.
3. **USE•RED 지표 상관 분석**: 서비스 지연과 자원 포화를 연결한다.
4. **병목 후보 검증**: 단일 요인 변경 후 같은 부하로 재측정한다.
...
> 요약: 자원 중심의 **USE**와 서비스 중심의 **RED** 연계가 핵심이다.
...
| 한 자원 개선 후 **병목 이동** 미검증 | 같은 부하로 **개선 전후 재측정** | 다음 병목과 **전체 처리량 변화** 확인 |
...
- **포화•대기열**이 처리량을 제한하면 **임계 경로 자원** 우선을 개선한다.
