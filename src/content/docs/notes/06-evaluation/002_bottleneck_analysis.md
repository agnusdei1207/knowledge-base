...
- 기존 한계: 병목이 아닌 자원을 증설해도 전체 처리량은 가장 느린 구간에 제한되므로 성능 개선 효과를 얻기 어렵다.
...
- 성능 저하의 실제 제한 지점을 제거하기 위해 **요청 임계경로·자원 사용률·대기시간·처리량 변화**를 검토하고, 부하 증가에 따라 포화와 지연이 함께 커지는 구간을 병목으로 판정해야 한다.
