...
- 정의/개념: AI 수명주기 증거를 연결해 **성능•공정성**과 **강건성•책임 통제**를 검증하는 정보시스템 감리이다.
- 배경/필요성: 일반 기능 감리만으로는 **확률적 오류•편향•드리프트 검증할 수 없다**.
...
- 학습•평가 결과를 다시 만드는 **데이터 계보•재현성**이 핵심이다.
- 평균 정확도 밖을 보는 **공정성•교란 강건성**이 핵심이다.
- 배포 뒤 위험을 통제하는 **모델 드리프트•인간 감독**이 핵심이다.
...
1. **용도•위험 정의**: 오류 비용과 영향 집단에 맞춰 성능•안전 허용 기준을 확정한다.
2. **데이터 증거 검증**: 출처•권리•계보를 확인하고 학습•평가 데이터가 분리되었는지 검증한다.
3. **모델 재현 시험**: 오류 비용에 맞춘 성능•집단별 공정성•교란 강건성 기준을 독립을 재실행한다.
4. **배포•감시 통제 검증**: 드리프트 임계값•인간 검토•중단•롤백 절차를 확인한다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 특정 집단에 집중되는 **오탐•미탐** | **집단별 정밀도•민감도•인간 검토** | 고위험 집단의 **판정 오류 감소** |
...
- 오류 비용과 영향 집단에 맞춰 **재현 시험•운영 감시**를 연결하고 **인간 감독•롤백**이 입증된 모델만 배포가 핵심이다.
