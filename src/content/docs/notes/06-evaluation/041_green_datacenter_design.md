...
- **서버 통합**: 저이용 장비의 워크로드를 적은 서버에 모아 연산당 장비·전력 사용을 줄이는 방식이다.
- **외기 냉각**: 낮은 외부 온도를 이용하여 냉동기 가동과 냉각 전력을 줄이는 방식이다.
- **원천 부담 절감**: 재생전력·재이용수·저탄소 공급원을 선택하여 탄소와 물 사용의 발생 원인을 줄이는 방식이다.
- **영역별 설계 선택**: 저이용 서버는 통합하고 시설 손실은 냉각·배전 개선으로 줄이며 남은 전력·물은 저환경 공급원으로 조달하는 구분이다.
