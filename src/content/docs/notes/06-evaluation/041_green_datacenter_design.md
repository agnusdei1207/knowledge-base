...
- **PUE(Power Usage Effectiveness, 피유이)**: 영문 머리글자를 딴 지표로, 데이터센터 총에너지를 IT 장비 에너지로 나눈 값
- **WUE(Water Usage Effectiveness, 더블유유이)**: 영문 머리글자를 딴 지표로, 물 사용량을 IT 장비 에너지로 나눈 값
- **CUE(Carbon Usage Effectiveness, 씨유이)**: 영문 머리글자를 딴 지표로, 탄소 배출량을 IT 장비 에너지로 나눈 값
- **REF(Renewable Energy Factor, 알이에프)**: 영문 머리글자를 딴 지표로, 총에너지 중 재생에너지 비율
