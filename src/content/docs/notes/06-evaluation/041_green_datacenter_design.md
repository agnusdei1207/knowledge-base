...
- **WUE(Water Usage Effectiveness)**: 데이터센터 물 사용량을 IT 장비 에너지로 나눈 물 효율 지표
- **CUE(Carbon Usage Effectiveness)**: 데이터센터 탄소 배출량을 IT 장비 에너지로 나눈 탄소 효율 지표
- **REF(Renewable Energy Factor)**: 데이터센터 총에너지 중 재생에너지가 차지하는 비율
...
- **배경/필요성**: 고밀도 IT 부하와 지역 환경 한계 동시 대응
...
- PUE·CUE·WUE·REF를 함께 관리한다.
- 입지·IT·시설·에너지의 전 과정 최적화한다.
- 전력 절감과 물·탄소 증가의 상충 분석한다.
- 고밀도 부하의 냉각과 가용성을 함께 확보한다.
...
- AI 센터: 고밀도 랙에 침지 냉각 적용
- 가뭄 지역: 재이용수·PUE 상충 분석

### 쉽게 이해하기 (학습용)
- 발열이 큰 AI 서버를 냉각액에 담가 팬 전력을 줄이고 누수·정비 방식을 함께 검증함
- 상수 취수를 줄이되 전기식 냉각 증가로 PUE와 탄소가 악화되지 않는지 비교함


## Ⅶ. 결론

- 가용성·PUE·CUE·WUE로 설계 대안 선택
