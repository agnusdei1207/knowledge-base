...
2. 조립 토크 누락을 PFMEA로 검출한다
3. 낮은 RPN의 고심각도 항목을 우선 개선한다
4. 현장 결함으로 발생도·검출도를 재평가한다

### 쉽게 이해하기 (학습용)

- 사례 1은 센서·제어기·구동부 실패가 제동 거리와 운전자 안전에 미치는 영향을 설계 단계에서 줄인다.
- 사례 2는 작업자가 볼트를 빠뜨리거나 잘못 조이는 원인을 자동 측정과 공정 정지로 검출한다.
- 사례 3은 발생 가능성이 낮아 곱 점수가 작더라도 사고 결과가 치명적이면 별도 우선순위로 조치한다.
- 사례 4는 설계 때 예상한 점수만 유지하지 않고 실제 고객 고장 자료로 위험 평가를 현실화한다.
