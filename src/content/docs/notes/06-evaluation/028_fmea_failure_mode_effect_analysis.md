...
- 배경/필요성: 사고 결과 분석만으로는 잠재 고장별 **예방•검출 우선순위 산정할 수 없다**.
...
- **위험 기반 조치**: 점수 곱만 따르지 않고 고심각도•법규•안전 위험을 우선 처리하는 개선 결정한다.
- **고장 형태•영향 및 치명도 분석(Failure Modes, Effects and Criticality Analysis, FMECA)**: FMEA에 고장 치명도 평가를 결합한 위험 분석 기법이다.

</details>

- 기능•부품•공정의 **상향식 잠재고장 분석**이 핵심이다.
- 모드•원인•영향•통제의 **양방향 추적**이 핵심이다.
- S•O•D•고심각도의 **위험 기반 조치**가 핵심이다.
- FMECA의 **고장모드•영향•치명도 결합**이 핵심이다.
...
- **1. 기능•공정•요구 범위 정의**: 대상•경계•의도 확정이다.
- **2. 고장 모드•원인•영향 식별**: 실패형태•파급을 연결한다.
- **3. 현재 통제•S•O•D 평가**: 예방•검출•고심각도를 판정한다.
- **4. 조치•책임•기한 결정**: 제거•감소•검증 배정이다.
- **5. 효과 검증•잔여위험 승인**: 재평가•수용근거를 기록한다.
...
|:---|:---|:---|
| DFMEA | **설계 초기•변경 검토** | 기능•구조 **고장 분석**, 공정 변동 누락 |
| PFMEA | **생산 준비•운영 절차 검토** | 작업•공정 **실패 분석**, 설계 결함 전가 |
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 분석자마다 다른 **FMEA 수행 기준** | **IEC 60812:2018 적용** | 분석•문서화 **일관성** 확보 |
| **RPN 동일값의 위험 차이** | **고심각도•조치우선순위 병행** | **치명위험 누락** 방지 |
| 운영 결함이 빠지는 **평가 노후화** | **현장 결함으로 원인•발생도 재평가** | **운용 조건** 반영 |
...
- **고심각도**는 RPN과 무관하게 우선 조치, 이후 **발생도•검출도**를 개선한다.
