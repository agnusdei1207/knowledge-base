...
- 정의/개념: 동시 사용자와 응답•생각시간의 **$X=N/(R+Z)$** 관계로 TPS를 구하는 계산이다.
- 배경/필요성: 가입자 수만으로는 **생각시간을 반영한 TPS 산출할 수 없다**.
...
- **관계 축**: $N=X(R+Z)$로 동시성과 처리량을 연결한다.
- **경계 축**: 업무 거래 TPS와 내부 요청 RPS을 분리한다.
- **보정 축**: 피크•업무 비율•생각시간으로 부하 현실화이다.
...
1. **N•R•Z 측정 조건 확정**: 성공 거래와 집중 시간을 확정한다.
2. **N•R•Z 표본 수집**: 사용자 수와 주기 분포를 수집한다.
3. **X=N/(R+Z) 계산**: 폐쇄형 평균 TPS을 산출한다.
4. **업무 비율•피크 TPS 보정**: 거래별 최대 부하를 변환한다.
...
> 요약: 사용자 행동은 **폐쇄형**, 기계 요청은 **요청률 기반**을 적용한다.
...
- 사용자 행동은 **N/(R+Z)**, 기계 호출은 **RPS**로 목표 부하를 산정한다.
