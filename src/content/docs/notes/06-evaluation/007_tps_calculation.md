...
- **초당 요청 처리 건수(Requests Per Second, RPS)**: 1초에 처리한 개별 요청 수입니다.
- **실사용자 모니터링(Real User Monitoring, RUM)**: 실제 사용자의 요청과 체감 지연을 수집합니다.
- **응용 프로그래밍 인터페이스(Application Programming Interface, API)**: 시스템 간 요청과 응답 규약입니다.

## Ⅰ. 개요

- **정의/개념**: 사용자 주기로 초당 완료 거래를 계산함
- **배경/필요성**: 접속자 수만으로 요청 부하를 알 수 없음
...
- 생각시간 0은 실제보다 높은 부하를 만든다
- 업무 비율이 달라지면 거래별 TPS도 달라진다
