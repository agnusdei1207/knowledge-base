---
sidebar:
  order: 7
  label: "007. TPS 계산 - 동시 사용자•응답 시간 공식 (TPS Calculation)"
  badge:
    text: "기출 • 50%"
    variant: note
title: "TPS 계산 - 동시 사용자•응답 시간 공식 (TPS Calculation)"
date: "2026-08-06T23:27:50+09:00"
tags:
  - "notes-evaluation"
weight: 7
extra:
  question_no: "007"
  source_status: "기출"
  source_history: "131회"
  priority: 50
  priority_note: "동시 사용자와 응답시간으로 처리량을 산정"
---

## Ⅰ. 개요

<details><summary>핵심 용어</summary>

- **TPS(Transactions Per Second)**: 1초 동안 성공적으로 완료한 업무 거래 수이다.
- **폐쇄형 부하**: 고정된 사용자들이 요청•응답•생각시간의 주기를 반복하는 부하 모형이다.
- **처리율 산정**: 동시 사용자 수를 응답시간과 생각시간의 합으로 나누어 폐쇄형 부하의 평균 처리율을 구하는 계산이다.

</details>

- 정의/개념: **폐쇄형 부하**에서 동시 사용자와 응답•생각시간으로 **처리율 산정**을 수행한다.
- 배경/필요성: 가입자 수만으로는 생각시간을 반영한 **TPS**를 산출할 수 없다.

#### 한줄 요약
- 동시 사용자 수(N), 응답시간(R), 생각시간(Z) 관계식(TPS = N / (R + Z))으로 목표 TPS를 산정한다.

## Ⅱ. 특징

<details><summary>핵심 용어</summary>

- **대화형 응답시간 법칙**: 폐쇄형 부하에서 동시 사용자•처리량•응답시간•생각시간이 $N=X(R+Z)$를 만족하는 관계이다.
- **동시 사용자($N$)**: 같은 기간에 요청 주기를 반복하며 시스템과 상호작용하는 사용자 수이다.
- **부하 보정**: 피크 집중률•업무 비율•생각시간을 반영해 계산 부하를 실제 운영 패턴에 맞추는 과정이다.

</details>

- **대화형 응답시간 법칙**으로 **동시 사용자**와 처리량을 연결한다.
- 업무 거래 TPS와 내부 요청 RPS를 분리한다.
- **부하 보정**으로 피크•업무 비율•생각시간을 반영한다.

#### 한줄 요약
- 생각시간(Think Time)이 증가하면 요청 유입 주기가 늘어나 동일 동시 사용자 대비 필요 TPS가 감소한다.

## Ⅲ. 구조 및 구성요소

<details><summary>핵심 용어</summary>

- **응답시간($R$)**: 요청 전송부터 응답 수신까지 걸린 시간이다.
- **생각시간($Z$)**: 사용자가 응답을 받은 뒤 다음 요청을 보내기까지 기다리는 시간이다.
- **처리량($X$)**: 단위 시간에 완료한 업무 거래의 수이다.
- **업무 거래 경계**: 여러 내부 호출 가운데 사용자 목적 하나의 성공 완료를 TPS 한 건으로 세기 위한 시작•종료 기준이다.

</details>

```text
                         [업무 거래 경계]
                                |
          +---------------------+---------------------+
          |                     |                     |
[동시 사용자 N]          [응답시간 R]          [생각시간 Z]
          |                     |                     |
          +---------------------+---------------------+
                                |
                     [처리량 X•피크 보정]
```

선의 의미: 같은 업무 거래 경계 안의 동시 사용자, 응답시간, 생각시간이 처리량과 피크 보정 산정에 함께 사용되는 관계를 나타낸다.

| 구성요소 | 책임 |
|:---|:---|
| 업무 거래 경계 | **업무 거래 경계** 정의 |
| 동시 사용자 N | 동시 사용자 측정 |
| 응답시간 R | **응답시간** 측정 |
| 생각시간 Z | **생각시간** 측정 |
| 처리량 X•피크 보정 | **처리량**과 집중률 산출 |

#### 한줄 요약
- 전체 처리 건수는 동시 사용자 수라는 입력값과, 서버의 응답 속도 및 사용자의 조작 대기 시간이라는 제약 요인에 의해 계산됩니다.

## Ⅳ. 흐름도

<details><summary>핵심 용어</summary>

- **사용자 요청 주기**: 한 사용자가 요청•응답•생각시간을 거쳐 다음 요청으로 돌아오는 반복 구간이다.
- **업무 혼합**: 여러 거래 유형을 운영 빈도에 맞게 조합한 부하 구성이다.
- **피크 TPS 보정**: 평균 처리량에 집중률과 거래별 업무 비율을 적용해 최대 시간대의 목표 처리량으로 변환하는 절차이다.
- **N•R•Z 측정 조건 확정**: 성공 거래 경계와 집중 시간 및 표본 조건을 정하는 단계이다.
- **N•R•Z 표본 수집**: 사용자 수•응답시간•생각시간 분포를 측정하는 단계이다.
- **X=N/(R+Z) 계산**: 폐쇄형 사용자 주기에서 평균 처리량을 구하는 단계이다.
- **업무 비율•피크 TPS 보정**: 평균 처리량을 거래별 최대 시간대 목표로 변환하는 단계이다.

</details>

```text
업무 책임자
   │ 거래 경계·피크 비율 전달
   ▼
분석가·측정기
   │ 1. N•R•Z 측정 조건 확정
   │ 2. N•R•Z 표본 수집
   ▼
계산·검증기
   │ 3. X=N/(R+Z) 계산
   │ 4. 업무 비율•피크 TPS 보정
   ▼
목표 TPS·시험 부하 결과
```

### 동작 원리

1. **N•R•Z 측정 조건 확정**: **사용자 요청 주기**의 **N•R•Z 측정 조건 확정**을 수행한다.
2. **N•R•Z 표본 수집**: **N•R•Z 표본 수집**으로 사용자 수와 주기 분포를 모은다.
3. **X=N/(R+Z) 계산**: **X=N/(R+Z) 계산**으로 폐쇄형 평균 TPS를 산출한다.
4. **업무 비율•피크 TPS 보정**: **업무 혼합**과 **피크 TPS 보정**을 반영한다.

#### 한줄 요약
- 서비스의 목표 시나리오를 정의하고 핵심 변수를 측정한 후, 폐쇄형 공식을 이용해 TPS를 도출하고 실제 측정값과 비교합니다.

## Ⅴ. 종류 및 비교

<details><summary>핵심 용어</summary>

- **폐쇄형 부하 모델**: 응답이 끝난 사용자가 생각시간 뒤 다음 요청을 보내도록 동시 사용자 수를 고정하는 모델이다.
- **개방형 부하 모델**: 응답 완료 여부와 무관하게 정한 요청률로 새 요청이 도착하는 모델이다.
- **RPS(Requests Per Second)**: 1초 동안 처리한 개별 요청 수이다.
- **API(Application Programming Interface)**: 응용 기능을 호출하는 연결 규격이다.
- **모델 선택**: 사람의 반복 행동은 동시 사용자 기반 폐쇄형 모델, 기계적 요청은 요청률 기반 개방형 모델로 재현하는 구분이다.

</details>

| 부하 생성 모델 | 동시 사용자 모델 | 요청률 기반 모델 |
|:---|:---|:---|
| 적용 기준 | 웹 사용자 행동 | **API** 기계 요청 |
| 핵심 특징 | **폐쇄형 부하 모델** | **개방형 부하 모델** |
| 한계 | 생각시간 왜곡 | 지연과 무관한 과부하 |

> 요약: **모델 선택**으로 사용자 행동과 기계 요청을 구분한다.

#### 한줄 요약
- 사람이 화면을 보며 동작하는 주기를 고려하는 계산법과 기계가 일정 속도로 요청을 쏟아붓는 방식의 처리량 계산법을 비교합니다.

## Ⅵ. 실무 고려사항 및 대책

<details><summary>핵심 용어</summary>

- **업무 거래와 내부 요청**: 사용자 목적 하나를 완료하는 거래와 이를 구성하는 여러 API 요청을 구분한 단위이다.
- **피크 계수**: 평균 처리량을 최대 예상 처리량으로 높이기 위해 적용하는 배수이다.
- **경계 혼동**: 업무 거래 한 건과 이를 구성하는 내부 요청 여러 건을 같은 처리량 단위로 세어 용량을 잘못 산정하는 오류이다.

</details>

| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 생각시간 누락 | 사용자 주기 공식 적용 | 동시성과 TPS 정합성 확보 |
| 단위 혼용 | **업무 거래와 내부 요청**을 분리해 **경계 혼동** 방지 | 용량 오산정 방지 |
| 평균의 피크 은폐 | **피크 계수** 적용 | 최대 부하 현실화 |

#### 한줄 요약
- 메인 페이지 로드라는 업무 단위 TPS와 그 과정에서 내부 마이크로서비스가 호출하는 각각의 요청당 초당 횟수를 구분합니다.

## Ⅶ. 결론

<details><summary>핵심 용어</summary>

- **부하 현실화**: 실제 동시 사용자•응답시간•생각시간•업무 비율을 공식과 시험 모델에 반영하는 작업이다.
- **과부하**: 요청 도착률이 시스템의 완료 능력을 넘어 대기와 오류가 계속 증가하는 상태이다.
- **목표 부하 선택**: 사용자 행동은 $N/(R+Z)$로, 기계 호출은 초당 요청 처리 건수로 산정하는 결정이다.

</details>

- **목표 부하 선택**과 **부하 현실화**에 따라 사용자 행동은 주기 공식, 기계 호출은 **RPS**로 산정하고 **과부하** 여부를 검증한다.

#### 한줄 요약
- 시스템 설계 시 사용자의 체감 응답 속도와 행동 주기를 반영한 현실적인 성능 목표를 계산합니다.
