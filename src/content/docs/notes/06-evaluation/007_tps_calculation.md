...
- **초당 요청 처리 건수(Requests Per Second, RPS)**: 영문 머리글자를 딴 표기로 “알피에스”로 읽으며, 1초에 처리한 개별 요청 수
- **실사용자 모니터링(Real User Monitoring, RUM)**: 영문 머리글자를 딴 표기로 “럼”으로 읽으며, 실제 사용자의 요청과 체감 지연을 수집
- **응용 프로그래밍 인터페이스(Application Programming Interface, API)**: 영문 머리글자를 딴 표기로 “에이피아이”로 읽으며, 시스템 간 요청과 응답 규약

## Ⅰ. 개요

- 정의/개념: 사용자 주기로 초당 완료 거래를 계산함
- **배경/필요성**: 동시 사용자 수만으로 응답·생각시간에 따른 실제 처리량 계산 곤란
...
1. 웹 부하 시험은 RUM으로 생각시간 산정
2. 화면 거래 TPS와 내부 호출 RPS를 분리 측정

### 쉽게 이해하기 (학습용)
- 웹 브라우저를 사용하는 실제 고객의 행동 로그를 분석하여 다음 클릭까지 평균적으로 몇 초를 기다리는지 도출합니다.
- 메인 페이지 로드라는 업무 단위 TPS와 그 과정에서 내부 마이크로서비스가 호출하는 각각의 요청당 초당 횟수를 구분합니다.

## Ⅶ. 결론

- 동시 사용자·응답·생각시간으로 목표 TPS 결정
