...
- **MTBF·MTTR·MTTF**: 각각 수리 가능한 시스템의 평균 고장 간격·평균 복구시간·수리하지 않는 품목의 평균 고장 수명을 나타내는 신뢰성 지표
- **IEC 60050-192:2015**: 신뢰성·가용성·유지보수성 등 신인성 용어를 정의한 국제전기기술용어 표준이다.
- **IEC 61703:2016**: 신뢰성·가용성·유지보수성 지표의 수학적 표현을 제시한 국제표준이다.

## Ⅰ. 개요

- 정의/개념: 장애 간격·복구시간·수명의 **평균 지표**
- 배경/필요성: 예방·복구·교체의 **개선 책임 분리**
...
| 구성요소 | 책임 |
|:---|:---|
| 대상·수리 가능성 | 시스템·부품·수리 여부 |
| 고장 사건·가동 구간 | 시작·종료·고장 정의 |
| 탐지·수리·복구 경계 | MTTR 시작·종료 기준 |
| MTBF·MTTR·MTTF | 가동·복구·수명 평균 |
| 가용성·분포·신뢰구간 | 평균 한계·꼬리값 보완 |
...
sequenceDiagram
  ...
  O->>M: 1. 대상·고장·복구 경계 정의
  M->>I: 2. 가동·고장 사건 수집
  I->>R: 3. 탐지·수리·복구 시각 확정
  R->>C: 4. MTBF·MTTR·MTTF 계산
  C->>O: 5. 분포·가용성·개선축 판정
```

1. **대상·고장·복구 경계 정의**: 수리 여부·사건 기준 확정
2. **가동·고장 사건 수집**: 상태별 시작·종료 기록
3. **탐지·수리·복구 시각 확정**: MTTR 구간 분해
4. **MTBF·MTTR·MTTF 계산**: 누적시간·사건수 산출
5. **분포·가용성·개선축 판정**: 예방·복구·교체 결정
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 용어·대상 혼용 | **IEC 60050-192:2015 적용** | 수리성·신뢰성 정의 정렬 |
| 산식·시간 경계 불일치 | **IEC 61703:2016 적용** | 지표 재현성 확보 |
| 평균의 꼬리 장애 은폐 | **분포·p95 복구시간 병행** | 장기 장애 위험 식별 |
...
- **수리 가능성·고장 경계·복구시간·분포**로 지표를 결정한다.
