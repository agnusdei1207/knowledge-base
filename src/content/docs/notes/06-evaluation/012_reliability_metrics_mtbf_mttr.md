...
- **정의/개념**: 장애 간격·복구시간·부품 수명의 평균 지표
- **배경/필요성**: 예방 정비와 복구 개선 대상을 구분함
...
- 장애·복구의 시작과 종료 정의가 값을 바꾼다
- 평균값은 장애 간격·복구시간 분포를 숨긴다
- 수리 가능 여부가 MTBF와 MTTF를 구분한다
...
| 주요 위험 | 장애 정의 불일치 | 복구 종료 기준 불일치 | 수리 부품에 오적용 |
...
1. 서버 장애는 MTBF·MTTR로 가동·복구 평가
2. 저장장치 부품은 MTTF로 예방 교체 시점 결정

### 쉽게 이해하기 (학습용)
- 시스템 장애 복구에 대한 오차를 없애기 위해 모니터링 경보 시각과 백업 장비 가동 완료 시각을 엄격히 약속합니다.
- 영구 보관용 하드디스크와 일시 사용 목적의 소모품의 수명주기를 분리해 자산 효율성을 높입니다.

## Ⅶ. 결론

- 수리 가능 여부로 MTBF·MTTR·MTTF 선택
