...
- **용량 계획**: 현재 성능 기준선과 미래 수요 시나리오를 자원 모델에 적용하여 SLO·헤드룸을 만족할 용량과 확장 시점을 결정하는 계획이다.
...
- **서비스 수준 목표(SLO)**: 정한 기간에 서비스 지연·오류·가용성이 충족해야 하는 목표 수준이다.
...
- **용량 모델**: 업무량과 거래별 자원 원단위를 연결하여 미래 CPU·메모리·I/O 요구량을 계산하는 모델이다.
