...
date: "2026-07-31T05:08:13+09:00"
...
| 구성요소 | 책임 |
|:---|:---|
| 수요·SLO | **성장·계절·이벤트·장애·성능 목표** 제공 |
| 자원 모델 | **워크로드·CPU·메모리·DB·네트워크** 관계 |
| 리드타임·헤드룸 | **장애·급증·예측 오차·조달 기간** 반영 |
| 확장 대안·비용 | **스케일업·아웃·오토스케일링** 비교 |
| 용량 로드맵·실측 | **증설·회수 규모·실행 시점·모델 보정** |
...
1. **기준선 측정 조건**: 현재 요청 분포·동시성·데이터·측정 기간
2. **처리량·지연·포화 한계**: 워크로드별 성능과 병목 자원 실측값
3. **성장·피크·장애 수요**: 사업 일정·계절·이벤트를 변환한 미래 부하
4. **용량·실행일·검증 조건**: SLO·헤드룸·리드타임을 반영한 조달 요구
