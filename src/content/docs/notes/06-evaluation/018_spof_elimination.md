...
- 정의/개념: 전체 중단을 일으키는 단일 의존성을 분리•다중화하는 **SPOF 제거**이다.
- 배경/필요성: 장비만 복제하면 공통 전원•회선의 **동시 장애 방지할 수 없다**.
...
- **식별 축**: 사용자부터 외부 서비스까지 종단 의존성을 추적한다.
- **설계 축**: 기능 이중화와 장애 도메인 분리 병행이다.
- **검증 축**: 장애 주입으로 자동 승계와 대체 용량을 확인한다.
...
3. **장애 주입•자동 승계 실행**: 탐지•전환•격리를 시험한다.
4. **중단•복구•N-1 증거 검증**: 영향•시간•대체 용량을 검증한다.
...
> 요약: 단독 기능은 **이중화**, 공유 원인은 **도메인 분리**를 적용한다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 업무 영향 없이 장비 수만 보는 **제거 우선순위 왜곡** | **업무 영향•종단 의존성** 기준 적용 | SPOF 제거의 **업무 우선순위** 정렬 |
| 예비 자원이 원인을 공유하는 **공통 원인 고장** | **장애 도메인 분리** | 자원의 **동시 장애 가능성** 감소 |
...
- 단독 기능은 **이중화**, 공통 원인은 **장애 도메인 분리** 후 N-1을 검증한다.
