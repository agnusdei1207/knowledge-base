...
- 배경/필요성: 서버를 수십 대로 늘려도 내부 DNS·공유 스토리지·단일 전원처럼 **숨은 SPOF·CCF**가 남으면 이중화 투자 전체가 한 지점의 고장으로 무효화되므로, 종단 의존성 맵과 장애 도메인 분리를 이중화 설계 앞에 두어 추정에 기댄 안심을 장애 주입 실증으로 대체할 필요
...
#### 한줄 요약
- SPOF 제거는 한 번에 끝나는 작업이 아니라 의존성이 늘 때마다 재발하는 작업이므로, 의존성 맵을 갱신하는 체계가 없으면 전수 조사 결과는 곧 낡은 문서가 된다.
