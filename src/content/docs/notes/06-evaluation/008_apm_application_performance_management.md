...
- **애플리케이션 성능 관리(Application Performance Management, APM)**: 요청 경로의 지연과 오류 원인을 추적하는 체계입니다.
- **추적 식별자(Trace ID)**: 한 요청에서 파생된 모든 호출을 연결하는 식별자입니다.
...
- **마이크로서비스 아키텍처(Microservices Architecture, MSA)**: 서비스를 독립 배포 단위로 나눈 구조입니다.
- **추적 문맥(Trace Context)**: 추적 식별자와 부모 구간 정보를 전달하는 값입니다.
- **응용 프로그래밍 인터페이스(Application Programming Interface, API)**: 서비스 간 요청과 응답 규약입니다.

## Ⅰ. 개요

- **정의/개념**: 요청 경로별 지연·오류 원인을 추적하는 관리
- **배경/필요성**: 집계 지표는 분산 호출 원인을 숨김
...
- 표본률이 높으면 비용, 낮으면 누락이 늘어난다
- 추적 문맥이 끊기면 전체 호출 경로도 끊긴다
...
2. 트래픽 폭증 시 오류·지연 요청 우선 표본 추출

### 쉽게 이해하기 (학습용)
- 분산 서비스 환경에서 호출 간 식별자 유실로 분석 경로가 끊어지는 현상을 모니터링합니다.
- 모든 요청을 자세히 기록하면 비용과 지연이 커지므로 오류·느린 요청은 남기고 정상 요청은 표본으로 수집하여 부하를 낮춥니다.

## Ⅶ. 결론

- 추적 문맥·표본 비율로 분산 병목 진단
