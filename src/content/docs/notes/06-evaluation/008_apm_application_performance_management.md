...
- 기존 한계: 집계된 평균 지표만으로는 분산 서비스의 어느 호출 구간에서 지연과 오류가 발생했는지 식별하기 어렵다.
...
| 절차 | 설명 |
|:---|:---|
| 서비스 요청 | 최초 진입점에서 사용자 요청을 수신함 |
| 추적 식별자·루트 구간 생성 | 요청 전체를 묶을 Trace ID와 최상위 Span을 만듦 |
| 추적 문맥 전파 | 하위 호출에 부모 구간을 전달함 |
| 하위 호출 구간 전송 | 서비스별 처리시간·오류 구간을 수집 계층에 보냄 |
| 루트 구간 전송 | 전체 요청 구간을 하위 Span과 같은 Trace ID로 전송함 |
...
- 사용자 지연의 원인을 응용 코드와 자원까지 추적하기 위해 **트랜잭션 문맥·서비스 의존성·표본률·오버헤드·개인정보**를 검토하고, 원인 경로를 설명하는 APM 계측을 활용해야 한다.
