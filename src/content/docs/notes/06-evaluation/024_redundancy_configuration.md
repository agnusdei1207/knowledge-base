...
date: "2026-07-25T22:25:00+09:00"
tags:
  - "notes-evaluation"
weight: 24
extra:
...
- **N**: 최대 업무 부하를 정상 처리하는 데 필요한 최소 자원 수나 계통 용량
...
- **SLO(Service Level Objective)**: 서비스가 달성해야 할 가용성·지연·오류율의 내부 목표
...
- 예비 수준이 높을수록 장애 허용 범위와 비용이 증가한다.
- 남은 자원이 피크 부하를 처리해야 실효성이 있다.
- 전원·망·냉각 공유 시 공통 원인 고장이 발생한다.
- 계획 정비와 동시 장애 조건도 용량에 포함한다.
- 장애 주입으로 승계와 저하 운전을 검증한다.
...
| 주요 위험 | 복수 장애·예비 용량 부족 | 높은 비용·공유 제어기 | 복잡성·유휴 비용·공통 원인 |
