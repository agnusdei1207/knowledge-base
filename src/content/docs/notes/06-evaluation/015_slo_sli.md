...
- 배경/필요성: 100% 무결점을 목표로 걸면 어떤 배포도 정당화되지 않아 **배포 속도·안정성**의 우선순위를 매번 주관적 논쟁으로 정하는 비용이 발생하므로, 사용자 관점 실측치와 합의된 목표선을 배포 판단 앞에 두어 논쟁을 수치 비교로 대체할 필요
...
#### 한줄 요약
- SLO를 올리면 체감 품질은 좋아지지만 감내 가능한 배포 횟수가 줄어들므로, 목표선은 기술 한계가 아니라 조직이 감당할 배포 속도에서 역산해 정한다.
