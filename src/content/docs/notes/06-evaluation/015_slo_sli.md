...
date: "2026-07-25T22:16:00+09:00"
tags:
  - "notes-evaluation"
weight: 15
extra:
...
- **정의/개념**: SLI 측정값에 SLO 내부 목표를 설정함
- **배경/필요성**: 사용자 품질을 변경 판단의 수치로 연결함
...
- 자원 사용량보다 사용자 요청 결과를 측정한다.
- SLI는 사실값, SLO는 판단 기준으로 분리한다.
- 측정 창이 짧으면 변동하고 길면 반응이 늦다.
- 핵심 사용자 경로마다 목표를 따로 설정한다.
...
| 주요 위험 | 과도한 목표 비용 | 측정 경계 오류 | 계약 위반 책임 |
...
1. 핵심 비즈니스 경로별 거래 성공률 및 꼬리 지연 시간 측정
2. 30일 이동 창의 예산 소진율로 배포 제어
