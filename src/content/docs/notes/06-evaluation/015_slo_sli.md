...
- 배경/필요성: 100% 무결점 시스템은 현실적으로 불가능하며 비용이 무한대로 수렴하므로, 비즈니스에 충분한 신뢰성(Reliability) 수준을 수학적으로 정의하고 개발 민첩성과 운영 안정성의 최적 균형점 도출 필요
...
- **가용성과 지연시간의 정밀 수식화**: HTTP 상태 코드 기반 가용성 SLI($\frac{\text{Status} < 500}{\text{Total}}$)와 p95/p99 지연시간 SLI($\frac{\text{Latency} \le 500\text{ms}}{\text{Total}}$)의 표준화
- **에러 예산(Error Budget)의 자동 도출**: $\text{Error Budget} = 100\% - \text{SLO}$ 공식을 통해 합법적인 실패 허용량을 수치화하여 신규 기능 배포 속도 제어
- **다중 윈도우 다중 번레이트(Multi-Window Multi-Burn-Rate) 경보**: 1시간 급격한 장애($14.4\times$ Burn Rate)와 3일 완만한 누수($1\times$ Burn Rate)를 동시 감시하여 알람 피로(Alert Fatigue) 0% 달성
...
```text
┌─────────────────────────────────────────────────────────────────────────┐
│ [ 1. 핵심 사용자 여정 계층 (Critical User Journey: CUJ) ]               │
│  └─ [ 결제 여정 (Payment Journey) ] ➔ 유저 클릭 ➔ 인증 ➔ 결제 승인 ➔ 완료│
└────────────────────────────────────┬────────────────────────────────────┘
                                     │ (Good/Total 판정 규칙 적용)
                                     ▼
┌─────────────────────────────────────────────────────────────────────────┐
│ [ 2. SLI 정량 계측 공식 체계 (SLI Measurement Specification) ]         │
├───────────────────────────────────┬─────────────────────────────────────┤
│ [ 1. 가용성 SLI (Availability) ]  │ [ 2. 레이턴시 SLI (Latency) ]       │
│   $\text{SLI}_{avail} = \frac{\text{HTTP Status } < 500}{\text{Total Valid Requests}} \times 100$ │   $\text{SLI}_{lat} = \frac{\text{Response Time } \le 300\text{ms}}{\text{Total Valid Requests}} \times 100$ │
└───────────────────────────────────┴─────────────────────────────────────┘
                                     │ (30일 이동 창 집계)
                                     ▼
┌─────────────────────────────────────────────────────────────────────────┐
│ [ 3. SLO 목표 및 에러 예산 계층 (SLO & Error Budget Target) ]           │
│                                                                         │
│  ┌───────────────────────────────────────────────────────────────────┐  │
│  │ 목표 $\text{SLO} = 99.9\%$ (30일 이동 윈도우 기준)                │  │
│  │ 허용 에러 예산 $\text{Error Budget} = 100\% - 99.9\% = 0.1\%$      │  │
│  │ (30일간 1,000만 건 요청 유입 시 ➔ 최대 10,000건의 에러 허용)     │  │
│  └───────────────────────────────────────────────────────────────────┘  │
└────────────────────────────────────┬────────────────────────────────────┘
                                     │ (예산 소진율에 따른 의사결정)
                                     ▼
┌─────────────────────────────────────────────────────────────────────────┐
│ [ 4. SRE 정책 집행 및 거버넌스 (Burn-Rate Policy & Deployment Control) ]│
│  ├─ [ 잔여 예산 > 0 ] ➔ 신규 비즈니스 기능 배포 가속 (CI/CD 파이프라인 가동)│
│  └─ [ 잔여 예산 = 0 ] ➔ 배포 즉시 동결 및 신뢰성/리팩토링 스프린트 전환 │
└─────────────────────────────────────────────────────────────────────────┘
```

선의 의미: 사용자 여정에서 SLI를 측정하고 30일 이동 창에서 SLO 목표 및 에러 예산과 대조하여 배포 여부를 제어하는 구조

| 구성요소 | 핵심 정의 및 산출 공식 | 설정 주체 | 실무 역할 |
|:---|:---|:---|:---|
| **CUJ** | 사용자가 비즈니스 목적을 달성하는 핵심 경로 | 프로덕트 매니저(PM), 비즈니스팀 | 측정 우선순위 도출 |
| **가용성 SLI** | $\frac{\text{정상 응답 건수 (HTTP } 2xx, 3xx, 4xx)}{\text{유효 전체 요청 건수}} \times 100$| SRE, 모니터링 엔지니어 | 서버 오류(5xx) 실측 |
| **지연시간 SLI**| $\frac{\text{응답시간 } \le T\text{ms인 요청 건수}}{\text{유효 전체 요청 건수}} \times 100$ | SRE, 백엔드 개발팀 | 사용자 꼬리 지연 실측 |
| **SLO 목표치** | $\text{SLI} \ge \text{Target } (예: 99.9\%)$ | 개발팀과 SRE팀의 상호 합의 | 에러 예산 기준선 확립 |
| **에러 예산** | $\text{Error Budget} = 1 - \text{SLO}$ | 자동 계산 ($100\% - \text{SLO}$) | 위험 감수 및 배포 통제 |
...
1. **개발과 운영의 공통 언어**: "장애가 잦다"는 주관적 감정 대신 "이번 달 에러 예산이 15% 남았으니 신규 배포가 가능하다"는 객관적 지표로 소통
2. **이동 윈도우의 연속성**: 달력 기준(월초 리셋) 시 월말에 대형 장애가 나도 월초에 무책임하게 배포가 재개되는 맹점을 30일 연속 이동 창으로 완전 해결
3. **클라이언트 관점의 측정**: 서버 내부 CPU가 아닌 Ingress Gateway에서 측정한 지표를 채택하여 네트워크 지연과 504 게이트웨이 타임아웃을 100% 포착
4. **혁신 속도의 가속화**: 에러 예산이 넉넉히 남아있는 동안에는 카나리 배포나 A/B 테스트와 같은 과감한 신기술 도입을 적극 장려
5. **번레이트 기반 지능형 알람**: 1개 에러마다 알람을 울려 엔지니어를 지치게 하지 않고, 예산 소진 속도(Burn Rate)가 임계치를 넘을 때만 즉각 호출
...
- 개발과 운영의 대립을 종식시키고 사용자 관점의 고가용성을 과학적으로 달성하는 현대 소프트웨어 공학의 표준인 **SLI 및 SLO 체계**는 단순한 성능 지표를 넘어 비즈니스 속도와 안정성을 조율하는 핵심 거버넌스 엔진이며, 실무 구현 시 **핵심 사용자 여정(CUJ) 기반의 명확한 Good/Total 수식 정의**, **30일 이동 측정 창 기반의 SLI 집계**, **SLA 계약선보다 엄격한 내부 SLO 및 에러 예산 수립**, **번레이트 기반의 지능형 알람 및 배포 동결 정책 집행**을 완성하여 최고 수준의 제품 혁신 속도와 시스템 복원력을 완성
