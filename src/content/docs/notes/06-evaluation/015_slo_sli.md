...
- 배경/필요성: 서버 평균 지표만으로는 **여정 실패•꼬리 지연 판단할 수 없다**.
...
- 사용자 경로 기반 **성공률•지연 SLI**가 핵심이다.
- 측정값과 목표를 분리한 **SLI•SLO 관계**가 핵심이다.
- 측정 창 기반 **품질 충족 판정**이 핵심이다.
- 오류 예산 기반 **SRE 변경•신뢰성 조정**이 핵심이다.
...
1. **사용자 여정•좋은 사건 정의**: 경계•분자•분모를 확정한다.
2. **성공•오류•지연 데이터 수집**: 체감 품질 원시값을 확보한다.
3. **측정 창 SLI•오류 예산 계산**: 실제값•허용실패량을 산출한다.
4. **소진율•SLO 충족 판정**: 위험•예산 소비속도를 평가한다.
5. **배포•안정화•목표 조정**: 변경을 중지•개선•재설정한다.
...
> 요약: **SLI•SLO**로 배포를 조율하고 **SLA** 책임을 관리한다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 측정 경계가 서버 지표에 치우친 **사용자 관점 누락** | **여정별 성공률•p99 SLI 적용** | **체감 품질** 반영 |
| 조직별 측정•검토가 다른 **운영 편차** | **ISO/IEC 20000-1:2018 연계** | 감시•보고•**검토 정렬** |
...
- 오류 예산 소진율이 높으면 **배포 중지•안정화**, 여유면 **변경 진행**이 핵심이다.
