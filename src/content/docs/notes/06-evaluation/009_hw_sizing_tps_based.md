...
- **하드웨어 규모 산정**: 목표 부하를 중앙처리장치·메모리·저장장치·네트워크의 요구 용량으로 환산하는 작업이다.
...
- 정의/개념: **TPS 기반 하드웨어 용량산정** — 목표 거래 처리량과 거래당 자원 사용량·피크·여유율을 CPU·메모리·I/O 요구량으로 변환하는 용량 계획 방식
...
- **자원별 산정**: 연산량은 CPU, 세션·캐시는 메모리, 저장·전송량은 입출력 자원으로 각각 환산하는 구분이다.
