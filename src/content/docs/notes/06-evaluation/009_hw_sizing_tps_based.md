...
date: "2026-08-06T17:07:30+09:00"
tags:
  - "notes-evaluation"
weight: 9
extra:
...
- 정의/개념: 목표 처리량을 자원 요구량으로 변환하는 **TPS 기반 HW 용량산정**
...
선의 의미: 자원 구성에 업무량, 거래 원단위, 피크•성장•이중화 조건을 함께 결합하고 벤치마크•BMT 검증이 각 산정 근거와 후보 사양을 공통 검증하는 구조
...
1. **거래별 자원 원단위 산정**: CPU•메모리•I/O 요구량 계산
2. **피크•성장•N-1 요구량 보정**: 피크•성장•이중화 헤드룸 반영
3. **후보 사양•BMT 구성 설계**: 제품 성능값으로 시험 사양 구성
4. **TPC 참조•BMT 성능 측정**: TPS•p99•포화 상태 측정
5. **요구량•측정값 비교•사양 선정**: 목표 충족 자원 사양 결정
