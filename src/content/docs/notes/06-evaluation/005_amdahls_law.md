...
- **병렬화 비율($P$, 피)**: 전체 실행시간 중 여러 자원으로 나눌 수 있는 비율입니다.
- **순차 비율($1-P$)**: 전체 실행시간 중 나눌 수 없어 차례로 처리하는 비율입니다.
- **자원 수($N$, 엔)**: 병렬 처리에 투입한 코어나 작업자의 수입니다.
- **가속비($S$, 에스)**: 단일 자원의 실행시간을 병렬 실행시간으로 나눈 값입니다.
...
- **입출력(Input/Output, I/O)**: 저장장치나 네트워크와 데이터를 주고받는 처리입니다.
- **프로파일러(Profiler)**: 코드 구간별 실행시간과 자원 사용량을 측정하는 도구입니다.

## Ⅰ. 개요

- **정의/개념**: 순차 비율로 병렬 가속비 상한을 구하는 법칙
- **배경/필요성**: 자원 증설 전 최대 가속 효과를 예측함
...
- 순차 비율이 클수록 자원 추가 이득이 작아진다
- 동기화·통신 비용은 실제 가속비를 더 낮춘다
