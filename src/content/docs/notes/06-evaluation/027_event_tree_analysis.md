...
- **ETA(Event Tree Analysis)**: 하나의 초기 사건에서 시작해 방어 장벽의 성공·실패에 따른 결과 경로를 순방향으로 분석하는 기법
...
- 초기 사건에서 결과로 진행해 사고 전개를 보여준다
- 장벽 순서와 성공·실패가 결과 경로를 나눈다
- 경로 확률은 최종 피해 가능성의 비교를 돕는다
- 핵심 장벽 선택은 시나리오 폭증을 제한한다
- 장벽 의존성 누락은 사고 위험을 과소평가한다
...
| 비교축 | ETA | FTA |
|:---|:---|:---|
| 핵심 특징 | 초기 사건에서 결과를 순방향 전개 | 최상위 사고에서 원인을 역방향 분해 |
| 적용 기준 | 장벽 성공·실패와 피해를 분석할 때 | 복합 원인 조합과 컷셋을 찾을 때 |
| 주요 위험 | 분기 폭증·장벽 의존성 누락 | 트리 경계·사건 독립성 오류 |
