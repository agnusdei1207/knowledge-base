...
- 정의/개념: 초기 사건 뒤 **장벽 성공•실패 결과를 순방향 전개**이다.
- 배경/필요성: 단일 사고 확률만으로는 **장벽 조합별 피해 경로 판단할 수 없다**.
...
- 초기 사건의 **순방향 결과 전개**가 핵심이다.
- 장벽 순서•성공•실패의 **시나리오 분기**가 핵심이다.
- 조건부 확률의 **결과 위험 정량화**가 핵심이다.
...
- **1. 초기 사건•분석 경계 정의**: 출발 사건•범위 확정이다.
- **2. 방어 장벽•순서 식별**: 감지•차단•완화 배열이다.
- **3. 성공•실패 시나리오 전개**: 조건부 분기•완전경로를 생성한다.
- **4. 결과 확률•피해•의존성 평가**: 공통원인•영향 결합한다.
- **5. 장벽 보강•잔여위험 결정**: 취약경로 차단•재평가한다.
...
- **결함수 분석(Fault Tree Analysis, FTA)**: 최상위 사건에서 필요한 원인 조합을 역방향으로 분해하는 연역 분석한다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 분석자마다 다른 **절차•표현** | **IEC 62502:2010 적용** | 시나리오 **재현성** 확보 |
...
- 고확률•고피해 경로의 **취약 장벽**부터 보강, 공통원인은 **독립화**가 핵심이다.
