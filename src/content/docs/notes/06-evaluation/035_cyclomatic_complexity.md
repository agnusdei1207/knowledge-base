...
- 정의/개념: CFG의 간선·노드·연결 성분으로 **선형 독립 경로 수**를 산출해 기본 경로 시험 규모를 나타내는 복잡도 지표
- 배경/필요성: 코드 길이만으로 드러나지 않는 **분기 증가에 따른 시험 경로 누락 위험**과 리팩터링 우선순위 식별
...
- **$V(G)=E-N+2P$**로 CFG의 선형 독립 경로 수를 산출한다.
- 단일 CFG에서는 **결정점 수+1**로 같은 값을 간단히 계산한다.
- **제어 흐름 기반 측정**으로 언어와 코드 길이에 무관하게 시험 경로 규모를 비교한다.
...
| 구성요소 | 책임 |
|:---|:---|
| 분석 경계 | 함수·메서드 단위로 측정 범위와 연결 성분 구분 |
| CFG | 기본 블록과 제어 이동 관계를 노드·간선으로 표현 |
| 복잡도 산출 | $E-N+2P$로 **선형 독립 경로 수** 계산 |
| 임계값 정책 | 기준 초과 함수를 기본 경로 시험·리팩터링 대상으로 지정 |
...
1. **함수 분석 요청**: 함수·메서드 경계를 고정하고 제어 흐름 분석 대상으로 전달
2. **CFG·순환 복잡도**: 기본 블록과 제어 이동으로 CFG를 만들고 선형 독립 경로 수 산출
3. **기본 경로 시험 요청**: 임계값과 변경·결함 이력을 함께 보고 시험·리팩터링 우선순위 결정
4. **경로별 시험 증거**: 각 독립 경로에 대응하는 입력·기대 결과·실행 흔적을 연결
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 높은 순환 복잡도 함수의 **독립 경로 시험 누락** | CFG의 선형 독립 경로마다 시험 케이스와 실행 흔적 연결 | 분기별 **기본 경로 검증** |
| 임계값만으로 신규·안정 코드를 동일하게 취급해 **개선 우선순위 왜곡** | 변경 빈도·결함 이력·업무 중요도를 복잡도와 함께 평가 | 위험 중심의 **리팩터링 선정** |
| 함수를 분리했지만 모듈 간 호출 의존성이 늘어 **전체 이해도 저하** | 인지 복잡도와 결합도 지표를 함께 검토 | 수치 축소 목적의 **형식적 분할 방지** |
...
- **순환 복잡도**로 기본 경로 시험 규모를 정하고 변경 빈도·결함 이력을 결합해 리팩터링 우선순위를 결정하는 제어 흐름 품질 관리
