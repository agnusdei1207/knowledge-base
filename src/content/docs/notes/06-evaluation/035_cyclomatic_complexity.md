...
- **간선($E$)•노드($N$)**: CFG의 실행 이동 수와 기본 블록 수로 $V(G)=E-N+2P$ 계산에 사용한다.
...
- **순환 복잡도**: 코드의 제어 흐름과 독립 경로 수를 측정하여 시험 경로 기준을 제공하는 지표이다.
- **인지 복잡도**: 중첩•점프•흐름 단절이 사람이 코드를 이해하는 데 주는 부담을 측정하는 지표이다.
