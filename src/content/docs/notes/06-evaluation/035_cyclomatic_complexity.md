...
- 기존 한계: 코드 길이만으로는 분기 증가에 따른 제어 흐름의 복잡성과 독립 실행 경로의 시험 누락 위험을 판단하기 어렵다.
...
Ⅲ의 CFG→복잡도→임계값 구조가 계산 원리를 모두 나타내므로 별도 절차 도표는 생략한다.
...
- 복잡한 제어 흐름의 테스트 누락과 유지보수 위험을 줄이기 위해 **분기 수·독립 경로·변경 빈도·결함 이력**을 검토하고, 순환복잡도로 기본 경로 테스트와 리팩터링 우선순위를 결정해야 한다.
