---
sidebar:
  order: 30
  label: "030. 품질 특성 (기능 적합성, 성능 효율성, 호환성, 보안성)"
  badge:
    text: "기출 · 30%"
    variant: note
title: "품질 특성 (기능 적합성, 성능 효율성, 호환성, 보안성)"
date: "2026-07-25T14:06:00+09:00"
tags:
  - "notes-evaluation"
weight: 30
extra:
  question_no: "030"
  source_status: "기출"
  source_history: "120회"
  priority: 30
  priority_note: "ISO 25010에 통합되는 품질 특성 세부축"
---

## 미리 알고가기

- **품질 특성(Quality Characteristic)**: 제품 품질을 평가 가능한 관점으로 분류한 상위 속성
- **기능 적합성(Functional Suitability)**: 제공 기능이 명시·암묵적 요구를 충족하는 정도
- **기능 완전성(Functional Completeness)**: 요구된 기능 집합이 빠짐없이 제공되는 정도
- **기능 정확성(Functional Correctness)**: 기능이 필요한 정밀도로 올바른 결과를 제공하는 정도
- **성능 효율성(Performance Efficiency)**: 사용 자원과 시간에 비해 요구 성능을 제공하는 정도
- **시간 반응성(Time Behaviour)**: 정해진 조건에서 응답·처리 시간이 요구를 충족하는 정도
- **자원 이용성(Resource Utilization)**: 기능 수행에 쓰는 CPU·메모리·망 자원의 양이 요구에 맞는 정도
- **용량성(Capacity)**: 제품이 처리할 수 있는 사용자·데이터·거래의 최대 한도
- **호환성(Compatibility)**: 다른 제품과 자원·정보를 공유하며 함께 동작하는 정도
- **공존성(Co-existence)**: 같은 환경과 자원을 공유하면서 다른 제품에 해로운 영향을 주지 않는 정도
- **상호운용성(Interoperability)**: 둘 이상의 시스템이 정보를 교환하고 그 정보를 사용할 수 있는 정도
- **보안성(Security)**: 정보와 기능을 무단 접근·변경·노출로부터 보호하는 정도
- **기밀성(Confidentiality)**: 승인된 주체만 정보에 접근하게 하는 성질
- **무결성(Integrity)**: 데이터와 기능의 무단 변경·훼손을 방지하는 성질
- **p95 지연시간**: 전체 요청의 95%가 완료되는 최대 응답 시간
- **TPS(Transactions Per Second)**: 시스템이 1초 동안 처리한 거래 수
- **RTM(Requirements Traceability Matrix)**: 요구사항과 설계·시험·결과를 연결한 추적표

## Ⅰ. 개요

- **정의/개념**: 제품 품질을 독립된 평가 관점으로 분해한다
- **배경/필요성**: 추상 요구를 측정·수용 기준으로 바꾼다

### 쉽게 이해하기 (학습용)

- 기능이 맞는지와 빠른지 및 다른 시스템과 잘 연결되는지와 정보를 지키는지를 서로 다른 시험으로 확인한다.

## Ⅱ. 특징

- 기능 적합성은 기능 누락과 결과 오류를 찾는다
- 성능 효율성은 지연·포화·자원 낭비를 찾는다
- 호환성은 자원 충돌과 정보 교환 실패를 찾는다
- 보안성은 정보 유출·변조·행위 부인을 막는다
- 업무 위험은 특성별 수용 우선순위를 바꾼다

### 쉽게 이해하기 (학습용)

- 한 특성이 좋아도 다른 특성을 대신하지 못하므로 정확하지만 느리거나 빠르지만 정보를 노출하는 제품은 전체 품질 요구를 충족하지 못한다.

## Ⅲ. 아키텍처 및 구성요소

```mermaid
flowchart LR
    A[업무 맥락·위험] --> B[품질 특성·하위 특성]
    B --> C[메트릭·수용 기준]
    C --> D[시험·추적 증거]
    D --> E[수용·개선 결정]
```

| 설계 요소 | 설명 |
|:---|:---|
| 업무 맥락·위험 | 사용자·부하·연계·피해 조건 정의 |
| 품질 특성·하위 특성 | 필요한 평가 관점과 범위 선정 |
| 메트릭·수용 기준 | 측정값·환경·합격선 명시 |
| 시험·추적 증거 | RTM으로 요구와 결과 연결 |
| 수용·개선 결정 | 편차·상충·잔여 위험 판정 |

> 요약: 품질 특성을 메트릭·시험·수용 증거로 연결한다

### 쉽게 이해하기 (학습용)

- 업무 위험에서 중요한 품질을 고르고 측정 조건과 합격선을 만든 뒤 시험 결과를 요구사항에 연결해 인수 여부를 판단한다.

## Ⅳ. 원리 및 절차 흐름도

```mermaid
sequenceDiagram
    participant B as 업무 책임자
    participant Q as 품질 설계팀
    participant T as 시험팀
    participant R as 요구 추적표
    participant A as 인수 책임자
    B->>Q: 품질 위험·우선순위 전달
    Q->>R: 특성별 요구 등록
    R->>T: 메트릭·시험 조건 전달
    T-->>R: 측정 결과 연결
    R-->>A: 특성별 수용 증거
    A-->>B: 수용·개선 결과
```

| 절차 | 설명 |
|:---|:---|
| 품질 위험·우선순위 전달 | 업무 피해와 사용 조건 분석 |
| 특성별 요구 등록 | 기능·성능·호환·보안 요구 분리 |
| 메트릭·시험 조건 전달 | 부하·데이터·환경·합격선 명시 |
| 측정 결과 연결 | 시험값과 실패 근거를 RTM에 기록 |
| 특성별 수용 증거 | 요구 충족과 상충·편차 정리 |
| 수용·개선 결과 | 인수·보완·예외를 결정 |

> 요약: 품질 특성을 독립 요구와 시험 증거로 검증한다.

### 쉽게 이해하기 (학습용)

- 각 품질 특성을 별도 합격 조건으로 만들고 같은 환경에서 측정한 결과를 요구와 연결해야 무엇이 부족한지 설명할 수 있다.

## Ⅴ. 종류 및 비교

| 비교축 | 기능 적합성 | 성능 효율성 | 호환성 | 보안성 |
|:---|:---|:---|:---|:---|
| 핵심 특징 | 기능 완전성·정확성 | 시간·자원·용량 효율 | 공존·정보 상호운용 | 정보·행위의 보호 |
| 적용 기준 | 요구 결과의 누락·오류 검증 | 부하와 자원 한계 검증 | 제품 간 연계·공유 검증 | 접근·변경·추적 위험 검증 |
| 주요 위험 | 기능 누락·오계산 | 지연·포화·자원 낭비 | 연계 실패·자원 충돌 | 유출·변조·부인 |

> 요약: 기능·성능·호환·보안은 서로 다른 실패를 평가한다

### 쉽게 이해하기 (학습용)

- 기능 적합성은 무엇을 맞게 하는지, 성능은 얼마나 효율적으로 하는지, 호환성은 함께 쓸 수 있는지, 보안성은 누가 안전하게 할 수 있는지를 본다.

## Ⅵ. 실무 사례

1. 결제 기능의 금액·통화 계산 정확성을 검증한다
2. 피크 부하의 p95와 TPS 수용선을 검증한다
3. 표준 API의 정보 교환·오류 처리를 검증한다
4. 관리자 변경의 권한·무결성·추적성을 검증한다

### 쉽게 이해하기 (학습용)

- 사례 1은 기능 목록 존재만 보지 않고 할인·세금·환율 조건에서 실제 결제 금액이 맞는지 확인한다.
- 사례 2는 평균 속도가 아니라 혼잡 시간의 대부분 요청과 처리량이 합격선을 충족하는지 측정한다.
- 사례 3은 연결 성공뿐 아니라 양쪽이 같은 데이터 의미를 이해하고 실패 응답을 일관되게 처리하는지 본다.
- 사례 4는 승인된 관리자만 설정을 바꾸고 변경 내용과 수행자를 나중에 검증할 수 있는지 확인한다.

## Ⅶ. 결론

- 기능·성능·호환·보안 요구를 각각 시험 증거로 판정한다.

### 쉽게 이해하기 (학습용)

- 품질 특성은 표준 용어를 나열하는 것이 아니라 서로 다른 실패를 측정 가능한 조건과 실제 시험으로 분리해 검증하는 기준이다.
