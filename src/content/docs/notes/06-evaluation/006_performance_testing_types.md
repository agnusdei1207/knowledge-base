...
- **서비스 수준 목표(SLO)**: 서비스 수준 지표에 대해 내부적으로 달성해야 할 목표값이다.

</details>

- 정의/개념: **성능 테스트 유형**은 부하·스트레스·스파이크·내구성 시험처럼 부하 패턴을 달리하여 목표 성능·한계점·지속성·회복성을 검증하는 시험 분류
...
- **모니터링**: 처리량·지연·오류와 CPU·메모리·I/O를 같은 시간축으로 수집하는 활동이다.
...
- **소크·스파이크 테스트**: 장시간 누수·열화와 급격한 부하 변화의 대응·회복을 각각 검증하는 시험이다.
...
- **ISO/IEC/IEEE 29119-2**: 테스트 계획·감시·설계·실행·완료 프로세스를 정의한 국제표준이다.
