...
- **초당 트랜잭션 처리 건수(Transactions Per Second, TPS)**: 영문 머리글자를 딴 표기로 “티피에스”로 읽으며, 1초에 완료한 거래 수
- **응용 성능 관리(Application Performance Management, APM)**: 영문 머리글자를 딴 표기로 “에이피엠”으로 읽으며, 요청 경로와 자원 지표로 성능 원인을 추적
- **자원 포화(Resource Saturation)**: 요청이 처리 능력을 넘어 대기열이 생긴 상태입니다.
- **가비지 컬렉션(Garbage Collection, GC)**: 영문 머리글자를 딴 표기로 “지씨”로 읽으며, 사용하지 않는 메모리를 자동 회수하는 처리
- **자바 가상 머신(Java Virtual Machine, JVM)**: 영문 머리글자를 딴 표기로 “제이브이엠”으로 읽으며, 자바 바이트코드를 실행하고 힙 메모리를 관리
...
- **배경/필요성**: 단일 목표 부하 시험만으로 한계·장기 열화·급증 복구 검증 곤란
...
1. JVM 서비스는 소크 시험으로 힙 누수 추적
2. 행사 트래픽은 스파이크 시험으로 확장 지연 측정

### 쉽게 이해하기 (학습용)
- 장시간 가동 시 점진적으로 메모리가 쌓여 서버가 다운되는 JVM 힙 영역 누수를 사전에 포착하기 위해 가동을 지속합니다.
- 대규모 할인 행사 등 특정 시점에 폭주하는 가상 사용자를 투입해 오토스케일링이 지연 없이 처리하는지 확인합니다.

## Ⅶ. 결론

- 부하 크기·지속시간·증가율로 시험 유형 선택
