...
- 정의/개념: 운영 위험에 맞춰 **부하 패턴**을 달리해 성능•회복성을 검증하는 시험 분류이다.
- 배경/필요성: 단일 목표 부하로는 **장기 누수•급증 복구 검증할 수 없다**.
...
- **패턴 축**: 크기•증가율•지속시간에 따라 부하 유형을 분리한다.
- **재현 축**: 업무 비율•데이터•환경으로 운영 조건 재현이다.
- **판정 축**: 처리량•지연•오류•복구를 함께 검증한다.
...
- **모니터링**: 처리량•지연•자원 사용을 같은 시간축으로 수집한다.
...
1. **환경•데이터•부하 설정 검증**: 운영 조건과 발생기 용량을 검증한다.
2. **부하 증가•유지•급변 실행**: 유형별 프로파일을 재현한다.
3. **지연•오류•포화•복구 지표 수집**: 시계열•추적•자원을 수집한다.
4. **합격 기준 판정•병목 분석**: 합격선과 개선 조건을 결정한다.
...
> 요약: **부하 패턴별 운영 위험**을 분리해 안정성을 검증한다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 운영과 다른 업무 구성으로 **부하 대표성 부족** | 실제 **업무 비율•데이터 분포** 재현 | 운영 용량의 **예측 오차** 감소 |
| 공유 환경 간섭으로 **시험 재현성 부족** | 자원•데이터•트래픽의 **시험 격리** | 개선 전후 **비교 가능성** 확보 |
...
- **용량•파괴점**은 부하•스트레스, **누수•급증**은 소크•스파이크를 선택한다.
