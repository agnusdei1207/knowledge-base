...
- 정의/개념: 총 시설 에너지를 IT 장비 에너지로 나눈 **PUE**이다.
- 배경/필요성: 총에너지만으로는 **IT 장비** 소비와 냉각•배전 부담을 구분하기 어렵다.

#### 한줄 요약
- 서버가 1만큼 쓸 때 센터 전체가 몇 배의 전기를 쓰는지 보여 주며 1에 가까울수록 지원 설비 부담이 작음이 핵심이다.
...
- **PUE≥1**: 총 시설 에너지가 IT 장비 에너지보다 작을 수 없고 1에 가까울수록 지원 설비 부담이 작은 특성이다.
...
- **PUE≥1**이며 1에 가까울수록 지원 부담이 작다.
- **측정 경계•기간 정합성**을 전제로 비교한다.
- **부하•계절 민감성**을 고려한다.
- **ISO/IEC 30134-2**에 따라 측정•보고한다.

#### 한줄 요약
- 서버의 계산량이 줄어도 고정 냉각 전력은 남으므로 PUE가 나빠질 수 있어 IT 생산성 지표로 쓰면 안 된다.
...
- **무정전 전원장치(Uninterruptible Power Supply, UPS)**: 정전•전원 이상 시 IT 장비에 전력을 계속 공급하며 변환 손실이 PUE에 포함되는 장치이다.
...
| 구성요소 | 책임 |
|:---|:---|
| 총 시설 에너지 계측 | 전체 에너지 측정 |
| IT 장비 에너지 계측 | **UPS**•**PDU** 출력 측정 |
| 경계•기간 관리 | 공간•시간 범위 일치 |
| PUE 계산 | 총 시설과 IT 에너지 비율 산출 |
| 추세•손실 분석 | **지원 설비** 부담 추적 |

#### 한줄 요약
- 센터 입구 전력과 서버 앞 전력을 같은 기간에 재서 그 차이를 냉각•배전 부담으로 봄이 핵심이다.
...
- **측정 경계•기간 확정**: 시설과 IT 에너지의 공간 및 시간 범위를 일치시키는 단계이다.
- **총 시설 에너지 집계**: 수전과 현장 생산을 포함한 시설 전체 에너지를 합산하는 단계이다.
- **IT 장비 에너지 집계**: UPS•PDU 손실 이후 실제 IT 장비 사용량을 합산하는 단계이다.
- **PUE•비IT 손실 추세 분석**: 에너지 비율과 지원 설비 부담 변화를 비교하는 단계이다.
...
1. **측정 경계•기간 확정**: 혼합용도 부하와 공간•시간 범위를 통일한다.
2. **총 시설 에너지 집계**: **총 시설 에너지**를 합산한다.
3. **IT 장비 에너지 집계**: **IT 장비 에너지**를 합산한다.
4. **PUE•비IT 손실 추세 분석**: **비IT 손실 추세**를 비교한다.
...
|:---|:---|:---|:---|
| 적용 기준 | **PUE 적용** | **CUE** | **WUE** |
| 핵심 특징 | 시설 전력 부담 | 탄소 배출 부담 | 물 사용 부담 |
| 한계 | IT 생산성•탄소 미측정 | 배출계수 편차 | 취수•소비 경계 편차 |

#### 한줄 요약
- 시설 전기는 PUE, 전력원의 탄소는 CUE, 냉각에 쓴 물은 WUE로 따로 봄이 핵심이다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 공용 부하 포함에 따른 과대계상 | **혼합용도 건물**에 전용 계측 적용 | 경계 정확성 확보 |
| 계절•IT 부하 차이에 따른 왜곡 | **IT 부하 민감성**과 **계절 편향**을 통제 | 효과 분리 |
| 냉각 여유 축소 | **운영 안정성 균형** 유지 | 효율과 안정성 확보 |
...
- **효율과 생산성 구분**: PUE와 IT 작업량•서비스 가치•절대 탄소 배출을 서로 다른 지표로 보는 원칙이다.
- **개선 결정**: 같은 경계•계절•부하의 PUE 추세로 냉각•배전을 개선하되 온습도와 가용성을 우선하는 판단이다.

</details>

- **효율과 생산성 구분** 후 같은 조건의 추세로 **개선 결정**을 수행한다.

#### 한줄 요약
- PUE는 같은 센터의 냉각•배전 개선 추세에 쓰고 다른 기후•부하 센터와 단순 비교하지 않음이 핵심이다.
