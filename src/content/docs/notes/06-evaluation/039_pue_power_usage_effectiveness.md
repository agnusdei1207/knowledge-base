...
- 정의/개념: **총 시설 에너지÷IT 장비 에너지**로 지원 설비의 **에너지 부담**을 측정하는 데이터센터 효율 지표이다.
- 배경/필요성: 총에너지만으로는 IT 소비와 냉각•배전의 **비IT 손실 구분할 수 없다**.
...
- **PUE≥1이며 1에 가까울수록** 작은 비IT 에너지 부담이 핵심이다.
- **총 시설•IT 에너지의 경계•기간 일치**를 전제로 한을 비교한다.
- **계절•IT 부하 민감성**과 IT 생산성•탄소•물 효율의 별도를 평가한다.
- **ISO/IEC 30134-2:2026**에 따른 현장 발전•미계측 에너지를 관리한다.
...
- **비IT 손실 추세**: 총 시설 에너지와 정보기술 장비 에너지의 차이를 기간별로 비교해 냉각•배전 부담 변화를 보는 분석한다.
...
1. **측정 경계•기간 확정**: 혼합용도 부하와 **공간•시간 범위**를 통일한다.
2. **총 시설 에너지 집계**: 수전•현장 발전을 포함한 **전체 에너지**를 합산한다.
3. **IT 장비 에너지 집계**: UPS•PDU 출력단의 장비 에너지를 합산한다.
4. **PUE•비IT 손실 추세 분석**: 에너지 비율과 지원 설비 부담을 비교한다.
...
- **효율과 생산성 구분**: PUE가 시설 지원 에너지 비율만 나타내며 IT 작업량•서비스 가치나 절대 탄소 배출은 측정하지 않는다는 구분한다.
- **개선 결정**: 같은 경계•계절•부하의 PUE 추세로 냉각•배전을 개선하되 온습도와 가용성을 우선하는 판단이다.

</details>

- 같은 경계•부하의 **PUE 추세**로 냉각•배전을 개선하되 **온습도•가용성** 우선이 핵심이다.
