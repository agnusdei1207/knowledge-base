...
date: "2026-08-02T12:16:00+09:00"
tags:
  - "notes-evaluation"
weight: 16
extra:
...
- **오류 예산**: SLO가 허용하는 실패량에서 측정 창의 실제 실패량을 뺀 여유이다.
- **서비스 수준 목표(SLO)**: 특정 기간에 서비스 수준 지표가 달성해야 할 내부 품질 목표이다.

</details>

- 정의/개념: **오류 예산**은 SLO가 평가 기간에 허용하는 실패량에서 실제 발생한 실패량을 뺀 잔여 한도로, 배포와 안정화 우선순위를 결정하는 기준
...
- **서비스 수준 지표(SLI)**: 실제 사용자 요청의 성공률·지연처럼 서비스 품질을 측정한 값이다.
...
- **지속적 통합·지속적 제공(CI/CD)**: 변경을 자동으로 통합·시험·배포하는 체계이다.
...
- **ISO/IEC 20000-1**: 서비스 수준의 합의·감시·보고·검토 요구사항을 규정한 국제표준이다.
