...
- 배경/필요성: 팀별 감에 의존하면 **배포 속도•신뢰성 투자 합의할 수 없다**.
...
- $1-SLO$ 기반 **허용 실패량**이 핵심이다.
- 잔여 예산 기반 **변경•안정화 판단**이 핵심이다.
- 단기•장기 창의 **예산 소진율 감시**가 핵심이다.
- CI/CD 배포 관문의 **자동 승인•중단**이 핵심이다.
...
1. **SLI•SLO•측정 창 정의**: 분자•분모•목표를 확정한다.
2. **허용•실제 실패량 계산**: 전체•나쁜 사건을 집계한다.
3. **잔여 예산•소진율 산출**: 잔량•고갈 속도를 계산한다.
4. **다중 창 경보•정책 판정**: 급격•지속 위반을 구분한다.
5. **배포•안정화•예외 결정**: 변경속도•복구투자를 조정한다.
...
> 요약: **잔여 예산**은 배포, 급속 소진은 **중단•안정화**가 핵심이다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 사용자 피해와 다른 **잘못된 SLI** | 여정별 **성공률•지연 SLI** 적용 | 예산 판정의 **정확도 향상** |
...
- 예산 여유면 **CI/CD 배포**, 급속 소진이면 **중지•안정화**, 예외는 승인이 핵심이다.
