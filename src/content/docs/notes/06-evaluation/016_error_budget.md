...
date: "2026-07-25T22:17:00+09:00"
tags:
  - "notes-evaluation"
weight: 16
extra:
...
- **정의/개념**: SLO가 허용한 실패량 중 남은 여유
- **배경/필요성**: 배포와 신뢰성 개선의 전환 기준
...
- SLO가 높을수록 사용할 오류 예산은 작아진다.
- 같은 실패량도 짧은 시간에 몰리면 더 위험하다.
- 잔여량은 현재 상태, 소진율은 고갈 시점을 보인다.
- 정책과 연결하지 않은 예산은 배포를 바꾸지 못한다.
...
| 판단 기준 | 여유 구간 | 경고 구간 | 고갈 구간 |
|:---|:---|:---|:---|
| 핵심 특징 | 실패량이 허용량보다 작음 | 목표 위반이 가까움 | 허용 실패량을 초과함 |
| 적용 기준 | 잔여 예산 충분 | 예산 소진 가속 | 예산 전량 소진 |
| 주요 위험 | 과도한 변경 증가 | 늦은 안정화 전환 | 장기 배포 중단 |
...
1. CI/CD는 오류 예산 고갈 시 배포 중단
2. 결제 서비스는 단기·장기 소진율 동시 경보

### 쉽게 이해하기 (학습용)
- 실패 여유가 없으면 배포 자동화가 새 변경을 더 이상 내보내지 않습니다.
- 짧은 폭주와 오래 이어지는 실패를 서로 다른 시간 창으로 빠르게 찾습니다.

## Ⅶ. 결론

- 잔여 예산·소진율로 배포 승인 여부 결정
