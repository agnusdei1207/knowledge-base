...
- 정의/개념: 지역 재해 때 우선 업무를 재개하는 지리 분리 **재해복구센터**이다.
- 배경/필요성: 단일 센터의 이중화만으로는 **지역 재해 시 업무 재개할 수 없다**.
...
- 준비 수준이 높을수록 **RTO•RPO 단축**, **비용 증가**가 핵심이다.
- 전환•복귀 훈련의 **실측 복구 검증**이 핵심이다.
...
1. **영향 업무•예상 중단시간 분석**: 재해 범위•우선순위 근거를 산출한다.
2. **재해 선언•복구 목표 결정**: RTO•RPO와 복구 우선순위를 확정한다.
3. **일관된 복제본•우선 업무 복구**: 복구 시점 선택과 서비스를 기동한다.
4. **DR 접속 경로 전환**: 사용자 트래픽을 신규 접점으로 변경한다.
5. **기능•데이터•RTO 검증**: 업무 정합성과 목표시간을 확인한다.
...
- 분 단위 RTO•낮은 RPO는 **핫**, 시간 단위는 **웜**, 수일 허용은 콜드가 핵심이다.
