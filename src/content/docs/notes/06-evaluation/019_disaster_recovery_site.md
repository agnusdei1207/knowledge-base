---
sidebar:
  order: 19
  label: "019. 장애 복구 전략 - 핫•웜•콜드 사이트 (Disaster Recovery Site)"
  badge:
    text: "미출 • 30%"
    variant: note
title: "장애 복구 전략 - 핫•웜•콜드 사이트 (Disaster Recovery Site)"
date: "2026-08-06T23:27:50+09:00"
tags:
  - "notes-evaluation"
weight: 19
extra:
  question_no: "019"
  source_status: "미출"
  source_history: ""
  priority: 30
  priority_note: "핫•웜•콜드 복구 비용을 비교하는 설계 주제"
---

## Ⅰ. 개요

<details><summary>핵심 용어</summary>

- **DR(Disaster Recovery)**: 재해로 중단된 시스템•데이터를 복구하는 활동이다.
- **DR 사이트(Disaster Recovery Site)**: 주 센터 재해 때 서비스를 이어받는 대체 센터이다.

</details>

- 정의/개념: 지역 재해 때 우선 업무를 재개하는 지리 분리 **DR 사이트**이다.
- 배경/필요성: 단일 센터의 이중화만으로는 지역 재해 때 **DR**을 수행하기 어렵다.

#### 한줄 요약

- 주 데이터센터 전체가 멈춰도 업무를 다시 시작하도록 멀리 떨어진 곳에 데이터와 장비를 준비하되 필요한 복구 속도에 따라 준비 수준을 달리한다.

## Ⅱ. 특징

<details><summary>핵심 용어</summary>

- **RTO(Recovery Time Objective)**: 업무를 재개해야 하는 최대 허용 시간이다.
- **RPO(Recovery Point Objective)**: 허용 가능한 데이터 손실의 기준 시점이다.
- **BIA(Business Impact Analysis)**: 업무 중단 영향을 분석해 복구 순위를 정하는 활동이다.
- **지역 재해 독립성**: 주 센터와 대체 센터가 같은 전력•통신•기후 재해에 동시에 중단되지 않도록 장애 도메인을 분리한 성질이다.

</details>

- 장애 도메인 분리의 **지역 재해 독립성**이 핵심이다.
- 준비 수준이 높을수록 **RTO**와 **RPO**가 짧아지고 비용이 증가한다.
- **BIA**로 우선순위를 정하고 전환•복귀 훈련으로 검증한다.

#### 한줄 요약

- 예비 센터가 가까워 복제가 쉬워도 같은 재해에 함께 멈추면 소용없으므로 거리•통신 지연•공통 재난 위험을 함께 따져야 한다.

## Ⅲ. 구조 및 구성요소

<details><summary>핵심 용어</summary>

- **주 센터**: 평상시 업무 서비스를 운영하고 원본 데이터를 처리하는 데이터센터이다.
- **동기 복제**: 원본과 복제본 저장 완료를 함께 확인한 뒤 쓰기를 완료하는 방식이다.
- **비동기 복제**: 원본 저장을 먼저 완료하고 변경 내용을 복제본에 나중에 전달하는 방식이다.
- **대체 용량**: 재해 복구 사이트가 우선 업무의 목표 부하를 복구 목표 시간 안에 처리할 수 있는 자원 능력이다.

</details>

```text
[주 센터•업무 의존성] ----- [복제•백업•네트워크] ----- [DR 사이트•대체 용량]
                                                                  |
                                                       [재해 선언•전환 통제]
                                                                  |
                                                        [복구 검증•복귀]
```

선의 의미: 가로선은 주 센터와 지리적으로 분리된 DR 사이트가 복제•백업•네트워크 경계를 공유하는 정적 재해복구 구조이고, DR 사이트 아래에는 전환 권한과 업무 정합성•복귀를 담당하는 통제 계층이 놓인다.

| 구성요소 | 책임 |
|:---|:---|
| 주 센터•업무 의존성 | **주 센터**의 우선 업무 제공 |
| 복제•백업•네트워크 | **동기 복제** 또는 **비동기 복제** |
| DR 사이트•대체 용량 | **대체 용량** 확보 |
| 재해 선언•전환 통제 | 권한•순서•접속 승계 |
| 복구 검증•복귀 | 업무 정합성과 주 센터 복귀 확인 |

#### 한줄 요약

- 주 센터의 데이터를 대체 센터로 보내고 재해 선언 체계가 전환을 지휘하며 복구 뒤에는 서비스와 데이터를 검증해 원래 센터로 돌린다.

## Ⅳ. 흐름도

<details><summary>핵심 용어</summary>

- **재해 선언**: 정상 복구가 어렵다고 판단하여 DR 전환 권한과 절차를 공식 가동하는 결정이다.
- **장애 승계**: 주 센터의 트래픽과 업무 처리를 DR 사이트로 전환하는 처리이다.
- **장애 복귀**: 복구된 주 센터로 서비스와 데이터를 다시 안전하게 이전하는 처리이다.
- **업무 정합성 검증**: 전환•복귀 뒤 기능과 데이터가 정상 업무 기준을 만족하는지 사용자가 확인하는 절차이다.
- **영향 업무•예상 중단시간 분석**: 재해 범위와 업무별 중단 영향을 평가하는 단계이다.
- **재해 선언•복구 목표 결정**: 전환 권한을 발동하고 RTO•RPO 및 우선순위를 정하는 단계이다.
- **일관된 복제본•우선 업무 복구**: 복구 시점을 선택해 우선 서비스를 기동하는 단계이다.
- **DR 접속 경로 전환**: 사용자 트래픽을 대체 센터의 접점으로 변경하는 단계이다.
- **기능•데이터•RTO 검증**: 복구 기능과 데이터 및 재개시간을 확인하는 단계이다.

</details>

```text
       주 센터
              │ 1. 영향 업무•예상 중단시간 분석
              ▼
       재해 책임자
              │ 2. 재해 선언•복구 목표 결정
              ▼
       DR 사이트
              │ 3. 일관된 복제본•우선 업무 복구
              ▼
   네트워크 제어
              │ 4. DR 접속 경로 전환
              ▼
       업무 사용자
              │ 5. 기능•데이터•RTO 검증
              ▼
                검증 결과
```

### 동작 원리

1. **영향 업무•예상 중단시간 분석**: 재해 범위와 우선순위 근거를 산출한다.
2. **재해 선언•복구 목표 결정**: **재해 선언** 후 RTO•RPO와 복구 우선순위를 확정한다.
3. **일관된 복제본•우선 업무 복구**: 복구 시점을 선택해 서비스를 기동한다.
4. **DR 접속 경로 전환**: **장애 승계**로 사용자 트래픽을 신규 접점에 연결한다.
5. **기능•데이터•RTO 검증**: **업무 정합성 검증** 후 **장애 복귀** 조건을 확인한다.

#### 한줄 요약

- 책임자가 장기 중단을 확인해 DR 전환을 선언하면 대체 센터가 일관된 데이터 상태를 복구하고 사용자 접속 경로를 넘겨 업무를 재개한다.

## Ⅴ. 종류 및 비교

<details><summary>핵심 용어</summary>

- **핫 사이트**: 인프라와 데이터를 상시 준비하여 짧은 시간에 전환할 수 있는 DR 사이트이다.
- **웜 사이트**: 일부 인프라•데이터를 준비하고 사고 후 추가 기동•복원이 필요한 사이트이다.
- **콜드 사이트**: 공간과 기본 설비만 준비하여 사고 후 장비•시스템•데이터를 구축하는 사이트이다.
- **사이트 선택**: 분 단위 핵심 업무는 핫, 시간 단위 복구 업무는 웜, 수일 중단을 허용하는 업무는 콜드 사이트를 적용하는 구분이다.

</details>

| DR 사이트 | 핫 사이트 | 웜 사이트 | 콜드 사이트 |
|:---|:---|:---|:---|
| 적용 기준 | **사이트 선택**에 따른 **핫 사이트** | **사이트 선택**에 따른 **웜 사이트** | **사이트 선택**에 따른 **콜드 사이트** |
| 핵심 특징 | **핫 사이트** | **웜 사이트** | **콜드 사이트** |
| 한계 | 높은 비용•동시 장애 | 복원 절차•용량 부족 | 조달•복구시간 불확실 |

#### 한줄 요약

- 핫은 거의 즉시 전환하고 웜은 준비된 장비를 더 기동하며 콜드는 사고 뒤 장비와 데이터를 처음부터 복원하므로 속도와 비용이 반대로 움직인다.

## Ⅵ. 실무 고려사항 및 대책

<details><summary>핵심 용어</summary>

- **동시 재해**: 주 센터와 DR 사이트가 같은 지역•전력•통신 원인의 영향을 함께 받는 상황이다.
- **DR 모의훈련**: 실제 운영 절차로 선언•복제 확인•전환•업무 검증•복귀를 수행하는 시험이다.
- **복구 용량**: DR 사이트가 우선 업무의 목표 부하를 RTO 안에 처리할 수 있는 자원 능력이다.
- **ISO(International Organization for Standardization)**: 국제표준화기구이다.
- **ISO 22301**: 업무연속성관리시스템 요구사항 표준이다.

</details>

**ISO** 표준을 업무연속성 및 복구 훈련의 기준으로 적용한다.

| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 지역•기후 재해 누락으로 양 센터 동시 중단 | **ISO 22301** 적용 | **동시 재해**와 BIA 위험 정렬 |
| 구현 절차 차이로 복구 결과 변동 | 표준 절차 적용•개정 추적 | 절차 적용 편차 감소 |
| 문서만으로 실제 복구시간 확인 불가 | **DR 모의훈련** | **복구 용량**과 RTO•RPO 실측 |

#### 한줄 요약

- 사례 1은 거래 중단과 데이터 손실 허용치가 작아 상시 인프라와 최신 복제본으로 빠르게 승계한다.

## Ⅶ. 결론

<details><summary>핵심 용어</summary>

- **복구 우선순위**: BIA 결과에 따라 제한된 DR 자원을 먼저 할당할 업무의 순서이다.
- **준비 수준 결정**: 업무 영향 분석의 중단•데이터 손실 피해를 기준으로 핫•웜•콜드 사이트의 복구 속도와 비용을 선택하는 판단이다.

</details>

- **복구 우선순위**에 따라 **준비 수준 결정**을 수행한다.

#### 한줄 요약

- 모든 업무에 비싼 핫 사이트를 두지 않고 중단과 데이터 손실의 실제 피해를 기준으로 필요한 준비 수준과 비용을 결정한다.
