...
- 기존 한계: 동시 처리 건수·처리율·평균 체류시간을 각각 측정하면 세 지표의 정합성과 측정 오류를 상호 검증하기 어렵다.
...
| 절차 | 설명 |
|:---|:---|
| 안정 상태·동일 경계 확인 | 유입·완료량이 수렴하고 작업 유형·시간 범위가 같은지 검사함 |
| 처리율·평균 체류시간 전달 | 같은 경계에서 구한 $\lambda$와 $W$를 계산기에 보냄 |
...
- 처리량·응답시간·동시성 지표의 정합성을 검증하기 위해 **안정 상태·측정 경계·평균 도착률·체류시간**을 검토하고, $L=\lambda W$로 필요한 동시성과 관측값을 교차 검산해야 한다.
