...
- **ISO 22301:2019**: 중단 상황에서 우선 활동의 연속성과 복구를 관리하는 국제표준이다.

## Ⅰ. 개요

- 정의/개념: 중복 자원·장애 전환 기반 **중단 최소화 설계**
- 배경/필요성: 단일 자원 장애의 **확산 차단·RTO 달성**
...
| 구성요소 | 책임 |
|:---|:---|
| 장애 도메인·N-1 용량 | 전원·망·시설·잔여처리량 |
| 이중화 서비스 노드 | 동일 기능·무상태 분산 |
| 복제 데이터·정합성 | 동기·비동기·복제지연 |
| 상태확인·쿼럼·펜싱 | 판정·주노드·쓰기 차단 |
| 장애전환·복구·복귀 | RTO·검증·정상화 |
...
| 스플릿 브레인·용량 | **쿼럼·펜싱·N-1 검증** | 데이터충돌·전환실패 방지 |
...
- **장애도메인·복제·펜싱·RTO·N-1**로 고가용 구조를 결정한다.
