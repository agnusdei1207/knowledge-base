...
- 정의/개념: 중복 자원과 **자동 전환**으로 중단을 줄이는 **HA** 구조이다.
- 배경/필요성: 단일 자원 구조는 장애 시 **RTO**를 달성하기 어렵다.
...
- **로드밸런서**와 중복 노드로 서비스 연속성을 확보한다.
- 상태 복제•펜싱 기반 **정합성 유지**가 핵심이다.
- **상태 확인**과 **장애 전환**으로 RTO를 단축한다.
...
선의 의미: 장애 도메인과 N-1 용량은 로드밸런서•이중화 노드의 배치 경계를 이루고, 노드 아래에는 데이터 정합성과 상태 판정•쓰기 차단 기능이 분리되며 두 기능은 장애 전환•복구•복귀 통제에 함께 결합되는 정적 고가용성 구조를 뜻한다.

| 구성요소 | 책임 |
|:---|:---|
| 장애 도메인•N-1 용량 | 전원•망•시설과 **N-1 용량** |
| 로드밸런서•이중화 노드 | **무상태 계층** 분산 |
| 복제 데이터•정합성 | **상태 저장 계층**의 복제 |
| 상태 확인•쿼럼•펜싱 | **쿼럼** 기반 쓰기 권한 |
...
- **다중 신호 장애 판정**: 응답•의존성•쿼럼 신호를 함께 확인해 장애 여부를 결정하는 단계이다.
- **구 노드 펜싱•쓰기 차단**: 장애 노드가 공유 데이터에 쓰지 못하도록 격리하는 단계이다.
- **복제 정합성•승격 검증**: 대체 노드의 데이터와 처리 권한을 확인하는 단계이다.
- **트래픽•서비스 전환**: 정상 노드로 요청 경로와 서비스 역할을 넘기는 단계이다.
- **RTO•N-1•복귀 검증**: 전환 시간•대체 용량•정상 복귀를 확인하는 단계이다.
...
1. **다중 신호 장애 판정**: 응답•의존성•쿼럼을 확인한다.
2. **구 노드 펜싱•쓰기 차단**: **펜싱**으로 **스플릿 브레인**을 방지한다.
3. **복제 정합성•승격 검증**: **승격 검증**으로 데이터와 처리 권한을 확인한다.
4. **트래픽•서비스 전환**: 정상 노드로 경로를 변경한다.
5. **RTO•N-1•복귀 검증**: 시간•용량•정상화를 확인한다.
...
> 요약: **구성 선택**에 따라 **무상태 계층**은 동시 처리, **상태 저장 계층**은 대기 승격을 적용한다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 같은 원인에 함께 멈추는 **공통 원인 장애** | 장애 도메인 분리 | 이중화 독립성 확보 |
| 장애 시 대체 자원이 부족한 전환 실패 | **대체 용량** 시험 | 장애 중 목표 용량 유지 |
| 분리 노드가 함께 쓰는 스플릿 브레인 | 쿼럼•펜싱 검증 | **데이터 정합성** 유지 |
...
- **가용성 시험**을 거쳐 계층별 구성을 정하는 **계층별 이중화 결정**이 핵심이다.
