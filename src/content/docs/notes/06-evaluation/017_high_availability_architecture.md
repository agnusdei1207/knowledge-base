...
date: "2026-07-25T22:18:00+09:00"
tags:
  - "notes-evaluation"
weight: 17
extra:
...
- **정의/개념**: 중복 자원과 장애 전환으로 중단을 줄이는 설계
- **배경/필요성**: 단일 실패 지점의 전체 서비스 중단 방지
...
- 장애 감지 뒤 정확한 격리와 승계가 중요하다.
- 공유 상태가 많을수록 전환 정합성이 어려워진다.
- 공통 전원·망 장애는 이중화 노드를 함께 중단시킨다.
- 자동 전환은 RTO를 줄이지만 오류도 전파할 수 있다.
...
| 복제 상태 확인 | 마지막 반영 위치와 정합성을 검사함 |
| 서비스 준비 완료 | 대체 노드의 처리 가능 상태를 알림 |
...
|:---|:---|:---|
| 핵심 특징 | 여러 노드가 동시 처리 | 주 노드 처리·대기 노드 승계 |
| 적용 기준 | 웹 등 무상태 계층 | DB 등 상태 저장 계층 |
| 주요 위험 | 동시 쓰기 충돌 | 승격 지연·유휴 자원 |
...
1. 웹 서버 계층 Active-Active 부하 분산 운영
2. DB 계층 Active-Standby 및 펜싱 조치
3. 모의 장애 훈련으로 RTO 목표 부합 확인

### 쉽게 이해하기 (학습용)
- 사용자가 몰려도 견딜 수 있도록 웹 서버들을 로드 밸런서 아래에 묶어 동시에 동작하게 합니다.
- 데이터베이스 장애 시 대기 장비가 주 장비로 승격될 때 구 장비의 연결을 확실히 끊어 데이터가 꼬이는 것을 방지합니다.
- 통제된 모의 장애 훈련으로 목표 시간 안에 대체 장비가 정상 승계하는지 확인합니다.

## Ⅶ. 결론

- 무상태·상태 계층별 이중화 방식 결정
