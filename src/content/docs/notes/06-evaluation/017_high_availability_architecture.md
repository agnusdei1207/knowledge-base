...
- **자동 전환**: 장애를 탐지한 뒤 정상 대체 자원으로 역할과 트래픽을 넘겨 서비스 중단을 줄이는 처리한다.

</details>

- 정의/개념: 중복 자원과 자동 전환으로 중단을 줄이는 **고가용성 아키텍처**이다.
- 배경/필요성: 단일 자원 구조는 장애 시 **서비스 지속•RTO 달성할 수 없다**.
...
- **장애 전환**: 장애 노드의 역할과 트래픽을 정상 대체 노드로 넘기는 처리한다.
...
- 중복 노드•경로 기반 **서비스 연속성**이 핵심이다.
- 상태 복제•펜싱 기반 **정합성 유지**가 핵심이다.
- 상태 확인•자동 승계 기반 **RTO 단축**이 핵심이다.
...
2. **구 노드 펜싱•쓰기 차단**: 스플릿 브레인을 방지한다.
3. **복제 정합성•승격 검증**: 데이터•처리권한을 확인한다.
...
> 요약: **무상태 계층**은 동시 처리, **상태 저장 계층**은 대기 승격이 핵심이다.
...
| 장애 시 대체 자원이 부족한 **전환 실패** | **N-1 부하 시험** | 장애 중 **목표 용량 유지** |
| 분리 노드가 함께 쓰는 **스플릿 브레인** | **쿼럼•펜싱 검증** | **데이터 충돌** 방지 |
...
- 무상태•수평 확장은 **Active-Active**, 단일 쓰기•정합성은 **Active-Standby**가 핵심이다.
