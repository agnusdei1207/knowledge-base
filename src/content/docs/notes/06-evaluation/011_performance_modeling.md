...
- 정의/개념: 구조•부하•자원 모형으로 **처리량•지연•포화**를 예측해 용량 대안을 비교하는 기법이다.
- 배경/필요성: 실측 전에는 부하 증가에 따른 **병목과 투자별 개선 효과**를 판정하기 어렵다.
...
- **민감도 분석**: 입력값 변화가 결과에 미치는 영향을 비교해 예측을 지배하는 변수와 위험 범위를 찾는 분석한다.

</details>

- **워크로드•자원•가정**의 명시적 추상화가 핵심이다.
- 분석식•**시뮬레이션•What-if**를 통한 대안 반복을 비교한다.
- 실측 **보정•민감도 분석**을 통한 오차를 관리한다.
...
1. **자원•가정•성능 모형 구성**: 분석식•**시뮬레이션 모형**을 작성한다.
2. **기준 시나리오•측정 조건 생성**: 보정용 **기준값** 조건을 고정한다.
3. **기준 시나리오 성능 측정**: 보정•검증용 **측정값**을 수집한다.
4. **모형 보정•독립 조건 검증**: 예측 오차 조정과 일반화를 확인한다.
5. **대안 예측•민감도 분석**: 자원 대안별 성능•위험을 비교한다.
...
> 요약: 초기 산정은 **분석 모델**, 복잡한 검증은 **시뮬레이션 모델**이 핵심이다.
...
- **민감도 분석**: 입력값을 일정 범위에서 바꾸며 결과에 가장 큰 영향을 주는 변수를 찾는 분석한다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 실제와 다른 **워크로드 분포** | 운영 추적 기반 **도착•서비스 분포** 보정 | **예측 오차** 감소 |
| 운영 변경으로 생기는 **모형 드리프트** | 주기적 **실측 재보정** | 모델 **신뢰도 유지** |
| 검증 범위를 넘는 **외삽 오류** | **민감도 분석•예측 경계** 명시 | 의사결정 **위험 제시** |
...
- 목표 충족 대안 중 **보정 오차**가 작고 부하 변화에 **민감도가 낮은 자원안**을 선택한다.
