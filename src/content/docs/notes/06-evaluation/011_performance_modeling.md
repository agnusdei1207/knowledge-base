...
- 기존 한계: 실제 시스템을 구축한 뒤에만 성능을 측정하면 용량 부족과 아키텍처 선택 오류를 사전에 발견하기 어렵고 변경 비용이 커진다.
...
- 구축 전 아키텍처의 처리량과 지연을 예측하기 위해 **워크로드 가정·서비스 시간·자원 경합·실측 보정 오차**를 검토하고, 민감도 분석에서 목표를 안정적으로 만족하는 대안을 선택해야 한다.
