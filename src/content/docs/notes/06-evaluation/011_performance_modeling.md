...
- 정의/개념: 구조·부하·자원 모형으로 **처리량·지연·포화 예측**
- 배경/필요성: 구축 전 **용량 부족·구조 대안·투자 효과 판단**
...
| 구성요소 | 책임 |
|:---|:---|
| 워크로드·도착 모델 | 도착률·업무비율·시간분포 |
| 자원·서비스 모델 | 서비스율·서버수·대역폭 |
...
1. **목표·경계·워크로드 정의**: 품질목표·부하범위 확정
2. **자원·가정·모형 구성**: 분석식·시뮬레이션 선택
3. **기준 시나리오·실측 수집**: 예측·시험 기준값 확보
4. **오차 보정·민감도 분석**: 변수 조정·영향축 식별
5. **대안별 성능·위험 비교**: 목표충족·비용·여유 판정
...
| 측정식·단위 불일치 | **ISO/IEC 25023:2016 적용** | 보정 데이터 일관성 확보 |
...
- **워크로드·가정·자원경합·보정오차·민감도**로 대안을 결정한다.
