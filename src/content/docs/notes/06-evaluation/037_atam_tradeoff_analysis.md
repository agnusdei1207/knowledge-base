...
- **아키텍처 트레이드오프 분석 방법(Architecture Tradeoff Analysis Method, ATAM)**: '에이티에이엠'으로 읽고 품질 속성 간 상충을 분석하는 방법의 영문 머리글자를 표시
- **소프트웨어 아키텍처 분석 방법(Software Architecture Analysis Method, SAAM)**: '에스에이에이엠'으로 읽고 변경 시나리오의 수정 영향을 평가
- **비용 편익 분석 방법(Cost Benefit Analysis Method, CBAM)**: '시비에이엠'으로 읽고 품질 개선 대안의 비용·편익을 비교
- **개념 증명(Proof of Concept, PoC)**: '피오씨'로 읽고 작은 구현·실험으로 설계 가정을 검증
...
- **배경/필요성**: 단일 품질 평가만으로 놓치는 **다중 품질 영향·설계 가정의 불확실성 해소**
...
- ATAM은 **다중 품질의 민감점·상충점**을 설계 전에 검증
