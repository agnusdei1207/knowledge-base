...
sequenceDiagram
  ...
  P->>S: 평가 방법•산출물 안내
  S->>S: 1. 비즈니스 동인•품질 속성 정의
  S->>P: 비즈니스 동인 전달
  A->>A: 2. 아키텍처 결정•근거 정리
  A->>P: 아키텍처 접근법 전달
  S->>S: 3. 품질 시나리오•우선순위 결정
  S->>P: 품질 시나리오 전달
  P->>P: 4. 민감점•상충점•위험 분석
  P->>A: 검증 증거 요청
  A-->>P: 검증 증거 반환
  P->>P: 5. 위험 주제•후속 조치 확정
  P-->>S: 평가 결과 전달
```

**동작 원리**

1. **비즈니스 동인•품질 속성 정의**: 사업 목표•제약과 품질 목표 확정
2. **아키텍처 결정•근거 정리**: 구조•패턴•배치와 **품질 가정** 공개
3. **품질 시나리오•우선순위 결정**: 자극•반응•측정값의 순위 결정
4. **민감점•상충점•위험 분석**: 결정 변화와 품질 간 **반대 효과** 분석
5. **위험 주제•후속 조치 확정**: PoC•완화•추가 분석 과제 결정
