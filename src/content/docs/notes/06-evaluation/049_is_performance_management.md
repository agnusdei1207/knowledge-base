...
- **정보시스템 성능 관리**: 핵심 거래의 SLO와 워크로드 기준선을 정하고 병목 개선 효과를 동일 부하에서 반복 검증하는 폐루프 관리이다.
- **서비스 수준 목표(SLO)**: 정한 기간에 지연·오류·가용성 같은 SLI가 충족해야 하는 목표 수준이다.
...
- **애플리케이션 성능 모니터링(APM)**: 사용자 거래부터 내부 호출·DB·자원 사용을 연결하여 추적하는 체계이다.
