...
| 구성요소 | 책임 |
|:---|:---|
| 목표·워크로드 | 거래별 **SLI·SLO·요청·동시성·데이터** 기준선 |
| 관측 체계 | **거래 ID·추적·로그·메트릭·오버헤드** 연결 |
| 응용 계층 | **코드·스레드·캐시·외부 호출** 대기시간 |
| DB·인프라 계층 | **질의·잠금·저장장치·자원 포화도** 제공 |
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 평균에 느린 요청이 가려져 **체감 SLO 미달** | **거래별 p95·p99 지연·오류율 관리** | 꼬리 지연의 **가시성 확보** |
| CPU 증설 뒤에도 **DB 잠금 병목 잔존** | **거래 ID로 호출·질의·잠금 종단 추적** | 임계 경로의 **원인 분리** |
| 전후 워크로드 차이로 **효과 과대평가** | **동일 분포·동시성·데이터 회귀시험** | 개선 효과와 **병목 이동 검증** |
