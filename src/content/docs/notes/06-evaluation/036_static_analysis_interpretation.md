...
- 배경/필요성: 도구 심각도•경고 수만으로는 **오탐•실제 결함 구분할 수 없다**.
...
- **룰셋•언어•빌드 환경 고정**을 통한 경고 재현성을 확보한다.
- **도달 가능성•오염 흐름•업무 영향**을 통한 실제 위험을 판정한다.
- **오탐 근거•예외 만료•고위험 게이트**를 통한 미해결 위험을 통제한다.
...
- **거짓 양성(False Positive, FP)**: 도구가 경고했지만 실행 문맥과 보호 통제를 검토했을 때 실제 문제는 없는 판정한다.
...
- **재현 가능한 분석**: 같은 소스•의존성•컴파일 옵션•룰셋으로 다시 실행하여 같은 경고를 얻는 분석한다.
...
1. **정적 경고•오염 흐름 분석**: 위험 함수까지의 **경로•신뢰도**를 산출한다.
2. **실행 가능성•통제 유효성 검증**: 호출 조건과 검증•권한 통제를 확인한다.
3. **TP•FP•위험 수용 판정**: 실제 결함•오탐•수용 상태를 구분한다.
4. **수정•예외•통과 결정**: 위험 수용의 **기한•통제**를 기록한다.
...
| 판단 단계 | 판정 기준 | 후속 조치 |
|:---|:---|:---|
| 사실 분류 | 실제 실행•공격 경로가 있으면 **TP** | 수정•재검증 또는 업무 위험 평가 |
| 사실 분류 | 경로가 성립하지 않거나 안전 통제가 있으면 **FP** | 근거•만료일을 둔 예외 처리 |
...
- 도달•오염•업무 영향이 큰 **TP는 수정**, FP는 **예외**, 수용 위험은 기한 승인이 핵심이다.
