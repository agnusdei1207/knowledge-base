...
- 배경/필요성: 정적 규칙만으로 쏟아낸 수천 건 경고는 **진탐·오탐**이 뒤섞여 있어 개발팀이 전면 무시하거나 일괄 억제하는 순간 치명적 취약점까지 함께 묻히므로, 오염 흐름과 도달 가능성을 따지는 판정 계층을 조치 앞에 두어 경고 건수를 공격 가능성 순위로 대체할 필요
...
#### 한줄 요약
- 트리아지는 오탐을 걸러 조치 비용을 줄이는 대신 판정 자체가 전문 인력의 시간을 쓰므로, 도구 규칙을 조이면 검토 비용이 늘고 풀면 유출 위험이 느는 교환만 남는다.
