...
date: "2026-08-06T17:24:02+09:00"
tags:
  - "notes-evaluation"
weight: 36
extra:
...
- 정의/개념: 경고의 도달•오염•업무 영향으로 결함을 판정하는 **정적 분석 결과 해석**
- 배경/필요성: 도구 심각도•경고 수만으로는 **오탐•실제 결함 구분 불가**
...
선의 의미: 고정된 분석 입력과 경고 증거를 분류 검토와 배포 통제에 연결한 정적 구조
...
1. **정적 경고•오염 흐름 분석**: 위험 함수까지의 **경로•신뢰도** 산출
2. **실행 가능성•통제 유효성 검증**: 호출 조건과 검증•권한 통제 확인
3. **TP•FP•위험 수용 판정**: 실제 결함•오탐•수용 상태 구분
4. **수정•예외•통과 결정**: 위험 수용의 **기한•통제** 기록
