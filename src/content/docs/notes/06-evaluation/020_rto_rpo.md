...
- 정의/개념: 최대 중단시간은 **RTO**, 데이터 손실 범위는 **RPO**로 정한다.
- 배경/필요성: **BIA** 없이 일률적으로 복구 목표를 정하면 과소 보호나 과잉 투자가 발생한다.
...
- 장애 이후 서비스 재개까지의 **다운타임**으로 RTO를 확인한다.
- 장애 이전 데이터의 **복구 시점**으로 RPO를 확인한다.
- **비용-목표 상충**을 고려하고 **목표•실측 검증**을 수행한다.
...
선의 의미: 업무 영향 분석부터 복구 목표•보호 수단과 실측 검증까지의 정적 구조를 나타낸다.

| 구성요소 | 책임 |
|:---|:---|
| BIA•업무 우선순위 | 중단•손실•의존성 영향 분석 |
| RTO•RPO 목표 | 업무별 시간 한도 설정 |
| 복제•백업•복구 시점 | **동기 복제**, **비동기 복제**, **스냅숏•백업** 적용 |
| 선언•승계•업무 재개 | RTO 구현과 기능 검증 |
...
- **RTO•RPO•업무 우선순위 산정**: 업무 피해와 비용으로 복구 시간•시점•순서를 정하는 단계이다.
- **복제 주기•복구•승계 조건 설계**: 목표를 만족할 데이터 보호와 전환 조건을 구성하는 단계이다.
- **재해 선언•복구•업무 재개 실행**: 훈련에서 선언부터 서비스 재개까지 수행하는 단계이다.
- **실제 RTO•RPO•정합성 검증**: 복구시간과 데이터 손실 및 업무 기능을 확인하는 단계이다.
...
1. **RTO•RPO•업무 우선순위 산정**: 피해와 비용에 따른 시간 한도를 결정한다.
2. **복제 주기•복구•승계 조건 설계**: **장애 승계**와 데이터 보호 체계를 구성한다.
3. **재해 선언•복구•업무 재개 실행**: 훈련에서 복구 결과를 생성한다.
4. **실제 RTO•RPO•정합성 검증**: **복구 검증**으로 시간•손실•기능을 확인한다.
...
- **RTO 적용**: 업무 중단 허용 한도를 서비스 재개시간으로 정하는 목표 구분이다.
- **RPO 적용**: 데이터 손실 허용 한도를 복구 시점으로 정하는 목표 구분이다.
...
|:---|:---|:---|
| 적용 기준 | **RTO 적용** | **RPO 적용** |
| 핵심 특징 | **RTO 적용**의 서비스 재개시간 | **RPO 적용**의 데이터 복구 시점 |
...
**ISO** 지침으로 BIA와 복구 목표의 연결을 검토한다.

| 문제 | 대책 | 효과 |
|:---|:---|:---|
| BIA 없이 정한 복구 목표의 업무 근거 부족 | ISO 22301 기반 BIA 적용 | 목표의 업무 근거 확보 |
| 보호•복구 절차 분리로 RTO•RPO 목표 충돌 | **ISO 22313** 적용 | 보호•복구 수단 정렬 |
| 문서 목표만으로 실제 달성 여부 확인 불가 | **복구 훈련** | **실제 복구 소요 시간**과 **실제 데이터 손실** 관리 |
...
- **목표 차등화** 후 중단비용과 데이터 손실비용에 따라 **목표 단축 결정**을 수행한다.
