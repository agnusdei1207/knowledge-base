...
- **ISO 22301:2019**: BIA와 업무연속성 전략·절차·훈련 요구사항을 규정한 국제표준이다.
- **ISO 22313:2020**: 복구 우선순위와 연속성 전략 적용을 지원하는 국제지침이다.

## Ⅰ. 개요

- 정의: RTO는 **복구시간**, RPO는 **데이터 손실시점 목표**
- 목적: 업무영향에 맞춰 **복구 투자·보호 수준 차등화**
...
| 설계 요소 | 설명 |
|:---|:---|
| BIA·업무 우선순위 | 중단·손실·의존성 영향 |
| RTO·RPO 목표 | 업무별 승인 시간 한도 |
| 복제·백업·복구 시점 | RPO 구현·일관성 |
| 선언·승계·업무 재개 | RTO 구현·기능 검증 |
| 훈련·실측·개선 | 실제 시간·손실·격차 |
...
sequenceDiagram
  ...
  O->>A: 1. 중단·손실 영향·우선순위 분석
  A->>B: 2. 업무별 RTO·RPO 승인
  B->>R: 3. 복제·백업·복구 절차 구성
  R->>V: 4. 장애훈련·업무 재개 실행
  V->>O: 5. 실제시간·손실량·격차 개선
```

| 절차 | 설명 |
|:---|:---|
| 1. 중단·손실 영향·우선순위 분석 | 재무·운영·법적 한도 산정 |
| 2. 업무별 RTO·RPO 승인 | 목표·의존성·비용 확정 |
| 3. 복제·백업·복구 절차 구성 | 보호주기·자동화 연결 |
| 4. 장애훈련·업무 재개 실행 | 선언·승계·정합성 확인 |
| 5. 실제시간·손실량·격차 개선 | 목표대비 결과 보정 |
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 업무영향·우선순위 | **ISO 22301:2019 BIA 적용** | 목표의 업무 근거 확보 |
| 전략·절차 구현 | **ISO 22313:2020 참조** | 보호·복구 수단 정렬 |
| 목표와 실제 성과 차이 | **전환훈련·실측값 관리** | 복구 격차 지속 개선 |
...
- **업무영향·의존성·실측시간·손실량·비용**으로 RTO·RPO를 결정한다.
