...
- 정의/개념: 최대 중단시간과 데이터 손실 범위를 정한 **RTO•RPO 복구 목표**이다.
- 배경/필요성: 일률적 복구 목표로는 **업무별 과소 보호•과잉 투자 방지할 수 없다**.
...
- 장애 이후 서비스 재개의 **RTO 시간축**이 핵심이다.
- 장애 이전 데이터 손실의 **RPO 시간축**이 핵심이다.
- BIA•훈련 기반 **목표•실측•비용 검증**이 핵심이다.
...
2. **복제 주기•복구•승계 조건 설계**: 목표를 구현할 보호 체계를 구성한다.
3. **재해 선언•복구•업무 재개 실행**: 훈련에서 복구 결과를 생성한다.
4. **실제 RTO•RPO•정합성 검증**: 복구시간•손실구간•기능을 확인한다.
...
- 중단비용이 크면 **RTO 단축**, 데이터 손실비용이 크면 **RPO 단축**이 핵심이다.
