...
date: "2026-07-31T05:10:56+09:00"
tags:
  - "notes-evaluation"
weight: 20
extra:
...
sequenceDiagram
  ...
  O->>A: 1. 업무·의존성·중단·손실 영향
  A->>B: 2. 승인된 RTO·RPO·우선순위
  B->>R: 3. 복제 주기·복구 절차·승계 조건
  R->>V: 4. 재해 선언·복구 시점·재개 상태
  V-->>O: 5. 실제 복구시간·데이터 손실·격차
```

1. **업무·의존성·중단·손실 영향**: 재무·운영·법적 피해 분석값
2. **승인된 RTO·RPO·우선순위**: 피해와 비용에 따른 시간 한도
3. **복제 주기·복구 절차·승계 조건**: 목표를 구현할 보호 설계
4. **재해 선언·복구 시점·재개 상태**: 훈련에서 생성된 복구 결과
5. **실제 복구시간·데이터 손실·격차**: 목표 대비 실측 차이
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 업무 영향이 없으면 목표가 임의 숫자로 정해짐 | **ISO 22301:2019/Amd 1:2024 BIA 적용** | 목표의 업무 근거 확보 |
| 보호·복구 절차가 분리되면 두 목표가 충돌함 | **ISO 22313:2020 적용·개정 추적** | 보호·복구 수단 정렬 |
| 문서 목표만으로 실제 달성 여부를 알 수 없음 | **전환훈련·실측값 관리** | 복구 격차 지속 개선 |
