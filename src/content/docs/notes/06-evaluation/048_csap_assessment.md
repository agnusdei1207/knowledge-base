...
- **서비스형 인프라(Infrastructure as a Service, IaaS) 인증**: 공공에 제공하는 물리•가상 인프라와 관리 평면•격리 통제를 평가한다.
- **서비스형 소프트웨어(Software as a Service, SaaS) 인증**: 공공에 제공하는 응용 기능과 데이터•테넌트•애플리케이션 보안을 평가한다.
- **서비스형 데스크톱(Desktop as a Service, DaaS) 인증**: 업무용 가상 데스크톱의 세션•가상 운영체제(Operating System, OS)•관리 기반과 단말 연계를 평가한다.
