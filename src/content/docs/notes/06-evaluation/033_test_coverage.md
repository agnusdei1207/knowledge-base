...
date: "2026-08-02T12:33:00+09:00"
tags:
  - "notes-evaluation"
weight: 33
extra:
...
- **테스트 커버리지**: 정한 시험 대상의 구조 요소 중 시험으로 실행하거나 충족한 요소의 비율이다.
...
- **MC/DC**: 각 개별 조건이 다른 조건을 고정한 상태에서 결정 결과를 독립적으로 바꿀 수 있음을 시험 쌍으로 입증하는 기준이다.
...
- **구조 커버리지와 정답성**: 커버리지는 실행 범위의 증거이고 assertion·오라클은 결과가 맞는지 판정하는 증거이므로 함께 필요하다.
