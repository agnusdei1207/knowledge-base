...
선의 의미: 계측 도구가 검증 대상의 구조와 시험 집합을 결합해 실행 흔적을 생성하고 품질 게이트가 그 흔적과 목표 기준을 판정하는 정적 커버리지 구조
...
sequenceDiagram
  ...
  M->>M: 1. 위험별 커버리지 목표 설정
  M->>T: 목표 기준 전달
  loop 커버리지 보완
    T->>T: 2. 대상 코드•바이너리 계측
    T->>S: 시험 실행 요청
    S->>S: 3. 시험 집합•코드 경로 실행
    S-->>T: 실행 흔적 반환
    T->>T: 4. 달성률•미충족 요소 분석
    T-->>M: 분석 결과 전달
    M->>M: 5. 시험 보완•예외 승인 결정
    M->>T: 보완 시험•예외 전달
  end
```
...
- **1. 위험별 커버리지 목표 설정**: **구문•분기•MC/DC** 와 목표 비율 결정
- **2. 대상 코드•바이너리 계측**: 실행 여부 측정 기능 삽입
- **3. 시험 집합•코드 경로 실행**: 허용 입력으로 대상 경로 수행
- **4. 달성률•미충족 요소 분석**: 실행 흔적과 목표 비율 비교
- **5. 시험 보완•예외 승인 결정**: 미충족 보완과 **도달 불가 코드** 승인
