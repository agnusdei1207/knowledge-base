...
- 정의/개념: 전체 코드 구조 중 시험으로 **실행•충족한 요소 비율**을 측정하는 시험 충분성 지표이다.
- 배경/필요성: 시험 건수•성공률만으로는 **미실행 경로•조건 누락 식별할 수 없다**.
...
- **실행 범위 가시화**를 통한 미시험 요소 식별과 정답성 별도를 검증한다.
- **포섭 관계**에 따른 구문•분기•MC/DC의 논리 증거 강화가 핵심이다.
- **위험 기반 목표•예외 관리**를 통한 검증 강도와 비용을 조정한다.
...
- **실행 흔적**: 계측된 시험에서 어떤 구문•분기•조건이 실행되고 어떤 결과를 냈는지 남긴 기록한다.
...
- **계측**: 시험 실행 중 구문•분기•조건의 실행 여부를 기록하도록 코드나 바이너리에 측정 기능을 넣는 처리한다.
...
- **1. 위험별 커버리지 목표 설정**: **구문•분기•MC/DC**와 목표 비율을 결정한다.
- **2. 대상 코드•바이너리 계측**: 실행 여부 측정 기능 삽입이다.
- **3. 시험 집합•코드 경로 실행**: 허용 입력으로 대상 경로를 수행한다.
- **4. 달성률•미충족 요소 분석**: 실행 흔적과 목표 비율을 비교한다.
- **5. 시험 보완•예외 승인 결정**: 미충족 보완과 **도달 불가 코드** 승인한다.
...
- 일반 코드는 **구문•분기**, 안전 중요 논리는 **MC/DC**, 도달 불가는 근거 승인이 핵심이다.
