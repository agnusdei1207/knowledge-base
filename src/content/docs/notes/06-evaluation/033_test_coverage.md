...
1. 변경 코드의 커버리지를 별도로 관리한다
2. 돌연변이 시험과 결함 검출력을 교차 확인한다
3. 차량 제어 조건에 MC/DC를 적용한다
4. 도달 불가 코드는 근거를 남겨 예외 처리한다

### 쉽게 이해하기 (학습용)

- 사례 1은 전체 수치가 높아도 새 코드가 비어 있으면 신규 결함이 유출될 수 있으므로, 변경된 코드의 커버리지를 따로 계산해 릴리스 위험을 직접 통제한다.
- 사례 2는 커버리지가 높아도 검증력이 약한 단언문은 오류를 놓칠 수 있으므로, 의도적으로 코드를 바꾼 돌연변이를 시험이 검출하는지 확인해 시험 집합의 실효성을 보완한다.
- 사례 3은 차량 제어의 복합 조건에서는 각 조건이 결과에 실제 영향을 주는지 증명해야 하므로, 다른 조건은 고정하고 대상 조건만 바꾼 시험 쌍을 구성한다.
- 사례 4는 도달 불가 코드를 수치에서 임의로 빼면 품질이 과장되므로, 구조상 실행할 수 없는 이유와 검토 책임자를 기록한 경우에만 예외로 승인한다.

## Ⅶ. 결론

- 위험에 맞는 기준으로 빈 경로를 보완한다.
