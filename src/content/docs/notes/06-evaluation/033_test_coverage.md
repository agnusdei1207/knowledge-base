---
sidebar:
  order: 33
  label: "033. 테스트 커버리지"
  badge:
    text: "미출 • 50%"
    variant: note
title: "테스트 커버리지(구문•분기•MC/DC)"
date: "2026-08-06T23:27:50+09:00"
tags:
  - "notes-evaluation"
weight: 33
extra:
  question_no: "033"
  source_status: "미출"
  source_history: ""
  priority: 50
  priority_note: "구문•분기•MC/DC를 비교하는 시험 핵심축"
---

## Ⅰ. 개요

<details><summary>핵심 용어</summary>

- **테스트 커버리지(Test Coverage)**: 정한 시험 대상의 구문•분기•조건 같은 구조 요소 중 시험으로 실행하거나 충족한 요소의 비율이다.
- **시험 충분성 지표**: 시험이 코드 구조를 얼마나 폭넓게 다뤘는지 보여 주지만 결과의 정답성 자체는 보장하지 않는 지표이다.
- **미시험 요소**: 정한 구조 커버리지 기준에서 아직 실행•충족되지 않아 시험 보완이나 예외 승인이 필요한 요소이다.
- **수정 조건•결정 커버리지(Modified Condition/Decision Coverage, MC/DC)**: 각 조건이 결정 결과에 미치는 독립 영향을 시험 쌍으로 입증하는 기준이다.

</details>

- 정의/개념: 전체 코드 구조에서 실행한 요소의 비율을 나타내는 **테스트 커버리지**이자 **시험 충분성 지표**이다.
- 배경/필요성: 시험 건수•성공률만으로는 **미시험 요소**를 식별하기 어렵다.

#### 한줄 요약

- 시험이 모두 통과해도 실행하지 않은 코드 경로에는 결함이 남을 수 있으므로, 커버리지는 무엇을 시험했는지가 아니라 무엇을 아직 시험하지 않았는지를 찾아 보완하게 한다.

## Ⅱ. 특징

<details><summary>핵심 용어</summary>

- **포섭 관계**: 더 강한 커버리지 기준을 만족하면 일부 약한 기준도 함께 만족하는 논리 관계이다.
- **품질 게이트**: 위험별 커버리지 목표와 승인된 예외를 충족해야 다음 개발 단계로 진행시키는 통제 규칙이다.
- **위험 기반 목표**: 안전•보안•핵심 업무 코드일수록 더 강한 커버리지 기준과 높은 목표 비율을 적용하는 원칙이다.

</details>

- 실행 범위를 가시화해 **미시험 요소**를 식별한다.
- **포섭 관계**에 따른 구문•분기•MC/DC의 논리 증거 강화가 핵심이다.
- **위험 기반 목표**와 **품질 게이트**로 검증 강도를 조정한다.

#### 한줄 요약

- 구문은 문장 실행, 분기는 결정 결과, MC/DC는 개별 조건의 독립 영향을 확인하므로, 안전 중요도가 높을수록 더 강한 기준을 적용하되 높은 수치만으로 정답의 정확성까지 보장된다고 해석해서는 안 된다.

## Ⅲ. 구조 및 구성요소

<details><summary>핵심 용어</summary>

- **구문 커버리지**: 실행 가능한 각 구문을 한 번 이상 실행한 비율이다.
- **분기 커버리지**: 각 결정의 참•거짓 결과를 모두 실행한 비율이다.
- **조건 커버리지**: 결정문 안의 각 개별 조건이 참과 거짓을 한 번 이상 가진 비율이다.
- **실행 흔적**: 계측된 시험에서 어떤 구문•분기•조건이 실행되고 어떤 결과를 냈는지 남긴 기록이다.

</details>

```text
              [검증 대상]         [시험 집합]
                       \           /
                        [계측 도구]
                             |
                        [실행 흔적]
                             |
                        [품질 게이트]
```

선의 의미: 계측 도구가 검증 대상의 구조와 시험 집합을 결합해 실행 흔적을 생성하고 품질 게이트가 그 흔적과 목표 기준을 판정하는 정적 커버리지 구조를 나타낸다.

| 구성요소 | 책임 |
|:---|:---|
| 검증 대상 | 코드•모델의 구조 요소 제공 |
| 시험 집합 | 정상•오류•경곗값 입력과 기대 결과 제공 |
| 계측 도구 | 시험별 실행 경로 수집 |
| 실행 흔적 | **실행 흔적**으로 도달 여부 기록 |
| 품질 게이트 | **품질 게이트**로 보완 또는 예외 판정 |

#### 한줄 요약

- 대상 코드에 계측 정보를 넣고 시험 집합을 실행하면 실제로 지나간 경로가 남으므로, 그 흔적과 전체 구조의 차이를 비교해 추가 시험이 필요한 구문•분기•조건을 찾는다.

## Ⅳ. 흐름도

<details><summary>핵심 용어</summary>

- **계측**: 시험 실행 중 구문•분기•조건의 실행 여부를 기록하도록 코드나 바이너리에 측정 기능을 넣는 처리이다.
- **도달 불가 코드**: 허용된 입력과 제어 흐름으로 실행할 수 없어 커버리지 분모에서 예외 검토가 필요한 코드이다.
- **예외 승인**: 도달 불가•방어 코드의 사유와 영향, 승인자를 기록해 목표 미달을 제한적으로 허용하는 결정이다.
- **위험별 커버리지 목표 설정**: 코드 중요도에 맞는 커버리지 기준과 목표 비율을 정하는 단계이다.
- **대상 코드•바이너리 계측**: 실행 여부를 기록할 측정 기능을 대상에 삽입하는 단계이다.
- **시험 집합•코드 경로 실행**: 준비한 입력으로 대상 경로를 수행하는 단계이다.
- **달성률•미충족 요소 분석**: 실행 흔적과 목표를 비교해 미시험 구조를 찾는 단계이다.
- **시험 보완•예외 승인 결정**: 실행 가능한 경로는 시험을 추가하고 불가능한 경로는 근거로 승인하는 단계이다.

</details>

```text
시험관리자
   │ 1. 위험별 커버리지 목표 설정
   ▼
커버리지도구
   │ 2. 대상 코드•바이너리 계측
   ▼
시험환경
   │ 3. 시험 집합•코드 경로 실행
   ▼
커버리지도구
   │ 4. 달성률•미충족 요소 분석
   ▼
시험관리자
   │ 5. 시험 보완•예외 승인 결정
```

### 동작 원리

1. **위험별 커버리지 목표 설정**: **위험 기반 목표**로 구문•분기•MC/DC 기준을 결정한다.
2. **대상 코드•바이너리 계측**: **계측** 기능을 삽입한다.
3. **시험 집합•코드 경로 실행**: 허용 입력으로 대상 경로를 수행한다.
4. **달성률•미충족 요소 분석**: 실행 흔적과 목표 비율을 비교한다.
5. **시험 보완•예외 승인 결정**: **도달 불가 코드**는 **예외 승인**한다.

#### 한줄 요약

- 먼저 요구 수준을 정하고 계측 시험을 실행한 뒤 빈 구간의 원인을 분석해야, 실행 가능한 경로에는 시험을 추가하고 도달 불가 코드는 제거하거나 정당한 예외로 기록할 수 있다.

## Ⅴ. 종류 및 비교

<details><summary>핵심 용어</summary>

- **결정**: 하나 이상의 개별 조건을 조합하여 제어 흐름을 선택하는 논리식이다.
- **커버리지 선택**: 일반 코드는 구문•분기, 안전 중요 복합 논리는 수정 조건•결정 커버리지를 적용하는 구분이다.

</details>

| 테스트 커버리지 | 구문 | 분기 | MC/DC |
|:---|:---|:---|:---|
| 적용 기준 | **구문 커버리지** | **분기 커버리지** | **수정 조건•결정 커버리지** |
| 핵심 특징 | 각 구문 실행 | **결정**의 참•거짓 흐름과 **조건 커버리지** | 조건별 독립 영향 |
| 한계 | 미실행 분기 누락 | 복합 조건 영향 누락 | 시험 조합 비용 증가 |

> **커버리지 선택**은 일반 코드에 구문•분기, 안전 중요 복합 논리에 MC/DC를 적용한다.

#### 한줄 요약

- 구문 100%여도 한쪽 분기를 타지 않을 수 있고 분기 100%여도 개별 조건의 영향은 드러나지 않을 수 있으므로, 중요한 복합 논리에는 다른 조건을 고정한 채 한 조건만 바꿔 결과가 달라지는 MC/DC 증거가 필요하다.

## Ⅵ. 실무 고려사항 및 대책

<details><summary>핵심 용어</summary>

- **높은 커버리지의 한계**: 구조 요소를 실행했어도 잘못된 기대 결과•경계값•데이터 조합은 검증하지 못할 수 있는 한계이다.
- **예외 근거**: 도달 불가•방어 코드처럼 목표 미달을 허용할 사유와 영향•승인자를 기록한 증거이다.
- **위험별 게이트 운영**: 코드 위험도에 따라 커버리지 기준•목표 비율•예외 승인 수준을 달리하는 통제이다.
- **시험 오라클**: 실제 결과가 요구사항의 기대 결과와 일치하는지 판정하는 기준이나 메커니즘이다.

</details>

| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 커버리지 달성 후 결과 정답성 미검증 | **시험 오라클**로 결과 검증 | **높은 커버리지의 한계** 보완 |
| 도달 불가 코드에 따른 달성률 저하 | 코드 제거 또는 **예외 근거** 기록 | 해석 가능성 확보 |
| 안전 중요 복합 조건의 영향 누락 | **위험별 게이트 운영**으로 MC/DC 적용 | 독립 영향 입증 |

#### 한줄 요약

- 차량 제어의 복합 조건은 다른 조건을 고정하고 대상 조건만 바꾼 시험 쌍으로 각 조건이 결정 결과에 미치는 독립 영향을 입증한다.

## Ⅶ. 결론

<details><summary>핵심 용어</summary>

- **구조 커버리지와 정답성**: 실행 범위 증거와 결과 정답 증거를 함께 확인하는 품질 관점이다.
- **미충족 요소 처리**: 실행 가능한 경로는 시험을 보완하고 불가능한 경로는 근거를 승인하는 절차이다.
- **보완 결정**: 위험도에 맞춰 미충족 요소에는 시험을 추가하고 실행 불가능한 코드만 근거를 남겨 예외 처리하는 판단이다.

</details>

- **구조 커버리지와 정답성**을 함께 확인하고 **미충족 요소 처리**에 따라 **보완 결정**한다.

#### 한줄 요약

- 미충족 구문•분기•조건이 발견되면 위험도에 맞춰 시험을 추가하고, 실행 불가능한 부분만 근거를 남겨 예외 처리해야 커버리지 수치가 실제 시험 충분성을 설명할 수 있다.
