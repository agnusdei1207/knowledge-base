...
- 배경/필요성: 단일 장치는 대규모 데이터의 **전역 배치 처리량•학습 시간 한계** 발생함.

#### 한줄 요약
- 모델 복제본이 다른 배치를 학습하고 **기울기 평균** 기반 동기화
...
- 장치 증가에 따른 **처리량•전역 배치•통신 증가** #### 한줄 요약
...
1. 전역 배치•학습률 설정: 장치 수와 로컬 배치에 맞는 **최적화 조건** 고정함.
2. 중복 없는 로컬 배치 분할: 같은 단계의 표본을 장치별 **독립 입력 조각** 기반 배정함.
3. 로컬 순전파•역전파: 각 복제본이 로컬 데이터의 **기울기** 활용 계산함.
4. 기울기 축약•전역 평균: 모든 장치의 기울기를 합쳐 **공동 갱신값** 생성함.
...
> 요약: 배치 분할은 **데이터 병렬**, 연산 분할은 **텐서 병렬**, 계층 분할은 **파이프라인 병렬** 핵심이다.
