...
- 정의/개념: **이스티오**는 Istiod 제어 평면과 사이드카 또는 앰비언트 데이터 평면을 이용해 서비스 간 통신 정책을 집행하는 서비스 메시 플랫폼이다.
- 배경/필요성: 마이크로서비스별 **보안•라우팅•관측 구현 중복과 정책 불일치** 완화가 필요하다.
...
- Istiod의 **xDS 구성•인증서 배포**가 핵심이다.
- Sidecar와 Ambient의 **데이터 평면 선택**이 핵심이다.
- **mTLS•L4•L7** 라우팅•복원력•관측의 **정책 기반 집행**이 핵심이다.
...
1. **L4 구성•인증서 배포**: Istiod가 **xDS** 구성과 워크로드 인증서를 전달한다.
2. **L7 구성 배포**: 웨이포인트에 **HTTP** 인가•라우팅 정책을 전달한다.
3. **선택적 L7 정책 위임**: HTTP 검사가 필요한 트래픽만 웨이포인트를 경유한다.
4. **L7 인가•라우팅 집행**: **mTLS** 신원 검증 후 대상 서비스로 요청을 전달한다.
...
- 워크로드별 격리는 사이드카, **L4** 기본망은 ztunnel, 선택 **L7**은 waypoint을 적용한다.
