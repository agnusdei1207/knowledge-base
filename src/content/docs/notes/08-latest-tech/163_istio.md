...
- **이스티오(Istio)**: 쿠버네티스 환경에서 트래픽•보안•관측 정책을 제공하는 서비스 메시 구현체이다.
...
- **계층 7(L7) 정책**: 하이퍼텍스트 전송 프로토콜 경로•메서드•헤더 등 애플리케이션 요청 내용을 기준으로 통신을 제어하는 규칙이다.
...
- **워크로드별 격리**: 각 파드의 프록시와 정책 상태를 독립적으로 두어 영향 범위를 나누는 방식이다.
- **선택적 계층 7**: 하이퍼텍스트 전송 프로토콜 수준 검사가 필요한 트래픽만 웨이포인트를 거치게 하는 적용 방식이다.
