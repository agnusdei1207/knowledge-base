...
- 정의/개념: 분산 값을 축약하고 동일 결과를 모든 참여자에게 배포하는 **올리듀스 집단 통신**이다.
- 배경/필요성: 중앙 집계 서버는 기울기 통신의 **대역폭•처리 병목**을 유발한다.
...
- **연산 축**: 리듀스-스캐터•올개더 기반 축약•배포한다.
- **전송 축**: 청크•버킷으로 다중 링크 병렬 활용이다.
- **배치 축**: 알고리즘•순위와 물리 토폴로지 정합이다.
...
1. **참여•자료형•연산 조건 확인**: 모든 순위의 원소 수•형식•호출 순서 **일치 여부**를 검증한다.
2. **기울기 청크•버킷 분할**: 지연•대역폭과 역전파 준비 순서에 맞는 **전송 단위**를 구성한다.
3. **메시지•토폴로지별 경로 선택**: 크기•참여 수•링크 구조에 맞는 **링•트리•계층 경로**를 결정한다.
4. **청크 축약•분산**: 참여자가 청크를 교환•축약해 **분할 결과**를 보유한다.
5. **결과 청크 교환**: 분할 결과 교환으로 동일 전체 결과를 생성한다.
...
- **링 올리듀스(Ring All-Reduce)**: 참여자가 원형 경로로 청크를 순환시켜 축약•배포하는 대역폭 중심 알고리즘이다.
...
- **단계 지연•링크 계층**에 따라 링•트리•계층형 올리듀스를 선택한다.
