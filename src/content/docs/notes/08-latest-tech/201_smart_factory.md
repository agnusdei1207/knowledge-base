...
- 정의/개념: **스마트 팩토리(Smart Factory)** 는 생산 데이터를 연결해 공정을 실시간 제어•최적화하는 공장
...
- 설비부터 **전사적 자원 관리(Enterprise Resource Planning, ERP)** 와 공급망을 연결하는 **수직•수평 통합**
...
- **제조실행시스템(MES)**: 생산 지시•실적•품질을 관리하여 경영 계획과 현장 제어를 연결하는 시스템이다.
- **전사적 자원 관리(ERP)**: 수요•자재•원가•납기 계획을 세워 제조실행시스템에 생산 목표를 제공한다.
- **감시 제어 및 데이터 수집(SCADA)**: 현장 설비의 상태를 수집•시각화하고 제어 명령을 전달하는 운영 시스템이다.
- **프로그래머블 로직 컨트롤러(PLC)**: 센서 입력과 제어 로직에 따라 구동기를 실시간 제어하는 산업용 제어기이다.
- **분석•디지털 트윈**: 생산 데이터를 분석해 품질•고장을 예측하고 공정 개선안을 가상 검증하는 계층이다.

</details>

**전사적 자원 관리(Enterprise Resource Planning, ERP)** 의 계획을 **제조실행시스템(Manufacturing Execution System, MES)** 이 작업으로 변환하고, **감시 제어 및 데이터 수집(Supervisory Control and Data Acquisition, SCADA)** 과 **프로그래머블 로직 컨트롤러(Programmable Logic Controller, PLC)** 가 현장을 제어한다.
...
3. **공정 제어•측정**: **프로그래머블 로직 컨트롤러(Programmable Logic Controller, PLC)** 제어와 센서 상태 수집
4. **실적•품질 데이터**: 생산 이력과 이상•품질 신호 전달
5. **검증된 최적화 피드백**: 안전 검증•승인 후 조건을 **제조실행시스템(Manufacturing Execution System, MES)** 에 반영
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| **정보기술•운영기술 데이터 의미 불일치** | **국제자동화학회(International Society of Automation, ISA)의 ISA-95** 기반 자산•공정 식별자 표준화 | 수직 통합의 **정확도 향상** |
| 분석 결과의 **위험한 자동 제어** | 한계값•가상 검증•작업자 승인 | **설비•인명 안전** 확보 |
| 레거시 연결의 **공격면 증가** | 구역 분리•게이트웨이•최소 권한 | **운영기술(Operational Technology, OT) 장애•침해 확산 억제** |
