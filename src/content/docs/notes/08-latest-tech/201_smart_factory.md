...
**스마트 팩토리**는 생산 데이터를 연결해 전 공정을 실시간 제어·최적화하는 지능형 공장이다.
...
**수직·수평 통합**은 설비에서 경영 계층까지 연결하고 공정·공장·공급망 사이의 데이터를 연계하는 방식이다.
...
**MES**는 생산 지시·실적·품질을 관리하여 경영 계획과 현장 제어를 연결하는 제조실행시스템이다.
...
**폐루프 최적화**는 현장 측정 결과를 분석하여 검증된 공정 조건으로 다시 반영하는 순환 방식이다.
...
**디지털 팩토리**는 가상 모델과 시뮬레이션을 이용해 공장과 생산 공정을 설계·검증하는 체계다.
...
**ISA-95**는 기업과 제어 시스템의 기능 계층과 정보 연계 기준을 정의한 국제표준이다.
...
**제어 안전성**은 분석 결과를 현장에 반영하기 전에 허용 한계와 승인 절차로 위험한 동작을 차단하는 성질이다.
