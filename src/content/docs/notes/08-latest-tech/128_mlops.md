...
- **머신러닝 운영(Machine Learning Operations, MLOps)**: 데이터·코드·모델의 학습·배포·운영을 자동화하고 연결하는 체계
- **지속적 통합(Continuous Integration, CI)**: 코드·데이터·파이프라인 변경을 자동 검증하는 과정
...
- 기존 한계: 코드 중심 DevOps만으로는 데이터·모델 버전, 성능 드리프트, 재학습과 승인 이력을 함께 관리하기 어렵다.
