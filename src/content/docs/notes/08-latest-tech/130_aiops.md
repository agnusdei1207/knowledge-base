...
- 기존 한계: 고정 임계값과 개별 경보만으로는 분산 시스템의 경보 폭주를 줄이고 여러 이상 징후의 장애 상관관계를 분석하기 어렵다.
...
1. 서비스 토폴로지로 **중복 경보를 사건 단위로 통합**한다
...
- 경보 폭주와 복구 지연을 줄이기 위해 **토폴로지 상관성·원인 신뢰도·조치 영향·가역성**을 검토하고, AIOps 자동 조치는 검증된 저위험 런북으로 제한한다
