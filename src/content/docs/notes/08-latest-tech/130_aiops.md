...
- 정의/개념: **AIOps**는 로그•지표•추적•이벤트를 AI로 상관 분석해 사건•원인•조치 후보를 도출하는 운영 체계이다.
- 배경/필요성: 개별 경보만으로는 **경보 폭주•장애 상관관계 누락**이 발생한다.
...
- 다중 신호•토폴로지 기반 **경보 상관분석**이 핵심이다.
- 동적 정상 기준 기반 **이상 탐지**가 핵심이다.
- 원인 후보•런북•승인 기반 **대응 우선순위**가 핵심이다.
...
1. **정규화 로그•메트릭•트레이스**: 시간•서비스 식별자로 신호를 정렬한다.
2. **서비스 의존•영향 관계**: 토폴로지 기반 장애 전파 범위를 제공한다.
3. **상관 사건•이상 패턴**: 중복 경보를 하나의 사건으로 군집한다.
4. **원인 후보•근거 순위**: 후보별 증거와 영향도를 함께 제시한다.
5. **승인 런북 실행**: 위험•가역성에 따라 자동화•사람 승인을 적용한다.
...
- **원인 신뢰도**나 **조치 가역성**이 낮으면 사람 승인, 모두 높으면 저위험 런북 자동을 실행한다.
