...
- **제이에스온 (JavaScript Object Notation, JSON)**: 메시지 인자를 구조화하는 데이터 형식
- **제이에스온 원격 프로시저 호출 (JSON Remote Procedure Call, JSON-RPC)**: 클라이언트·서버 메시지 형식
...
- **MCP 클라이언트 (MCP Client)**: 서버별 세션과 메시지를 관리하는 호스트 내부 연결기
- **통합 개발 환경 (Integrated Development Environment, IDE)**: 개발 도구를 통합한 MCP 호스트 사례

## Ⅰ. 개요
- **정의/개념**: 호스트에서 서버별 세션을 관리하는 연결기
- **배경/필요성**: 서버별 초기화·메시지 처리를 공통화

### 쉽게 이해하기 (학습용)
- 앱과 서버 사이에서 한 서버를 전담하는 통신 담당자임

## Ⅱ. 특징
- 서버별 클라이언트가 연결·오류를 격리한다
- 초기화·기능 협상·알림·취소를 관리한다
- 서버 기능을 호스트의 모델 입력으로 전달한다
- 요청 ID·세션 상태로 응답을 추적한다
...
|:---|:---|:---|
| 핵심 역할 | 서버 세션·메시지 중개 | 기능·컨텍스트 제공 |
| 기능 방향 | 기능 목록 수신·호출 | 도구·리소스·프롬프트 노출 |
