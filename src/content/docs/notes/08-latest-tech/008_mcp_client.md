...
#### 한줄 요약
- **MCP Client** 기반 호스트•서버 간 연결•메시지 중개
...
| 구성요소 | 책임 |
|:---|:---|
| 요청 메타 관리자 | **요청 메타 관리자** **요청 메타데이터** 활용 구성함 |
| 메시지 중개기 | **메시지 디스패처** Request ID 기반 요청•응답을 연결함 |
| 기능 변환기 | **기능 변환기** 서버 기능 명세를 호스트에 전달함 |
| 전송 관리자 | **전송 관리자** **stdio**•**Streamable HTTP** 연결을 관리함 |
...
2. Request ID 기반 표준 요청 전송: **호출 식별자** 및 선택 기능•구조화 인자를 전달함.
...
|:---|:---|:---|
| 적용 기준 | **MCP Client** 기반 호스트의 서버 연결임 | **MCP Server** 기반 백엔드 기능을 공개함 |
...
| 연결 중단으로 미완료 요청 잔존 | **재요청 정책** 및 Request ID•취소•시간초과로 정리함 | 유실•중복 실행을 방지함 |
