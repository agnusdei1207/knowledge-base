...
- 정의/개념: **MoE**는 라우터가 토큰별 일부 전문가를 선택하는 조건부 계산 신경망 구조임. 이는 전통적 방식과의 핵심 차별점으로서 실질적인 업무 가치를 창출함.
- 배경/필요성: 밀집 모델의 모든 매개변수 활성화로 **연산량•전력 비용**이 급증하여 실무 환경에서의 유연한 대응과 지속적 최적화를 지원함.

#### 한줄 요약
- **Top-k 라우팅•부하 균형** 기반 조건부 계산 구조 구현
...
- **라우터의 Top-k 점수**로 활성 전문가와 결합 가중치를 결정하여 실무 환경에서의 유연한 대응과 지속적 최적화를 지원함.
- **부하 균형 손실•초과 토큰 정책**으로 전문가 용량 쏠림을 통제하여 실무 환경에서의 유연한 대응과 지속적 최적화를 지원함.
- 희소 활성화에도 **전체 가중치 저장•장치 간 통신**이 필요하여 실무 환경에서의 유연한 대응과 지속적 최적화를 지원함.

#### 한줄 요약
- 희소 활성에도 **전체 가중치•All-to-All 통신**이 필요하 기전 적용
...
- **부하 제어기(Load Controller)**: 특정 전문가로 토큰이 몰리지 않도록 보조 손실•용량 제한•드롭 정책을 적용하는 관리 장치를 의미하며, 시스템의 자율적 판단과 실행을 가능하게 하는 핵심 아키텍처 요소이다.
...
#### 한줄 요약
- **라우터•디스패처•전문가•부하 제어•결합기**를 구성 체계 적용
...
#### 한줄 요약
- **점수•Top-k•분배•전문가 계산•결합**을 수행 체계 적용
...
#### 한줄 요약
- **활성 연산량•전문가 결합 범위**로 구조를 구분 체계 적용
...
#### 한줄 요약
- **전문가별 토큰 수•통신 시간**으로 쏠림 탐지가 핵심 구조 구현
...
- 희소 활성 이득이 통신 비용보다 크면 **MoE**, 작으면 **밀집 FFN**을 선택하여 실무 환경에서의 유연한 대응과 지속적 최적화를 지원함.

#### 한줄 요약
- **희소 활성 이득•통신 비용**에 따라 MoE•밀집을 선택 체계 적용