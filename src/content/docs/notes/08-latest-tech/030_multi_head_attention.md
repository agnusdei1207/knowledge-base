...
date: "2026-08-06T21:56:00+09:00"
tags:
  - "notes-latest_tech"
weight: 30
extra:
...
- 정의/개념: 입력을 여러 표현 공간으로 투영해 각 헤드의 어텐션을 병렬 계산하고 결합하는 **MHA**
- 배경/필요성: 단일 어텐션은 문법•의미•위치 관계를 **서로 다른 표현 공간에서 동시 학습**하기 곤란

#### 한줄 요약
- **다중 표현 부분공간**의 관계를 병렬 학습•통합
...
- **MQA**: 모든 Q 헤드가 단일 K•V 헤드 공유
- **GQA**: Q 헤드 그룹별 K•V 공유로 표현력 보존

#### 한줄 요약
- **헤드 다양성•공유 범위**가 표현력과 비용 결정
...
선의 의미: 투영을 헤드별 부분공간에 연결하고 출력을 통합
...
5. **모델 차원 출력 투영**: `W^O`로 헤드 정보를 혼합해 **다음 계층 차원**으로 환원

#### 한줄 요약
- **헤드별 병렬 관계**를 연결해 통합 표현 생성
...
- 최대 표현력에는 **MHA**, 최소 캐시에는 **MQA**, 절충에는 **GQA** 선택

#### 한줄 요약
- **표현력•KV Cache 비용**에 따라 MHA•MQA•GQA 선택
