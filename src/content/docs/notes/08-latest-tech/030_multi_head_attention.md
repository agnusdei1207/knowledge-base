...
- **정의/개념**: 여러 투영 공간의 어텐션 결합
- **배경/필요성**: 다양한 토큰 관계 병렬 학습
...
> 요약: KV 캐시 공유 방식 변경을 통해 추론 대역폭 한계 개선
...
1. 메모리 병목 해소를 위해 GQA 기반 LLM 서빙 적용
2. 텐서 병렬화 적용 시 헤드 단위 분할 통신 최적화

### 쉽게 이해하기 (학습용)
- 메모리 저장 공간을 아끼기 위해 키와 값의 저장소를 묶어 관리함
- 컴퓨터 여러 대가 연산 결과를 모을 때 통신 시간을 줄임

## Ⅶ. 결론

- 추론 대역폭 한계 개선을 위한 GQA 변형 아키텍처 선택
