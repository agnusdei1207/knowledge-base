...
- 정의/개념: **MHA**는 입력을 여러 표현 공간으로 투영해 각 헤드의 어텐션을 병렬 계산하고 그 결과를 결합하는 어텐션 구조
...
- **MHA**: 쿼리 헤드마다 독립된 K·V 헤드를 사용하여 표현 용량이 크지만 캐시 비용도 크다.
- **MQA**: 모든 쿼리 헤드가 단일 K·V를 공유하여 캐시 비용을 최소화하지만 표현력이 낮아질 수 있다.
- **GQA**: 그룹별 K·V 공유로 MHA의 표현력과 MQA의 캐시 효율 사이를 절충한다.
...
- **MHA 선택 기준**: 메모리보다 최대 표현력과 품질이 우선일 때 독립 K·V 헤드를 사용한다.
- **MQA 선택 기준**: 생성 캐시와 메모리 대역폭을 최소화해야 할 때 단일 K·V를 공유한다.
- **GQA 선택 기준**: 캐시를 줄이면서 MHA에 가까운 표현력을 보존해야 할 때 그룹 공유를 적용한다.
