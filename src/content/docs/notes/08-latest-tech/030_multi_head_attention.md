...
- 배경/필요성: 단일 어텐션은 문법•의미•위치 관계를 **서로 다른 표현 공간에서 동시 학습**하기 어렵다.

#### 한줄 요약
- **다중 표현 부분공간**의 관계를 병렬 학습•통합이 핵심이다.
...
- 헤드별 독립 투영에 의한 **다중 표현 부분공간 학습**이 핵심이다.
- 병렬 어텐션 결과의 연결과 **출력 투영 통합**이 핵심이다.
- **MQA**: 모든 Q 헤드가 단일 K•V 헤드 공유이다.
- **GQA**: Q 헤드 그룹별 K•V 공유로 표현력을 보존한다.

#### 한줄 요약
- **헤드 다양성•공유 범위**가 표현력과 비용을 결정한다.
...
1. **입력 Q•K•V 선형 투영**: 헤드마다 독립 가중치로 **관계 관점**을 생성한다.
2. **헤드별 부분공간 분할**: 모델 차원을 여러 **저차원 표현 공간**으로 재배열한다.
3. **헤드별 어텐션 병렬 계산**: 각 부분공간에서 서로 다른 **토큰 관계**를 추출한다.
4. **헤드 출력 연결**: 병렬 결과를 Concat해 **통합 표현**을 구성한다.
5. **모델 차원 출력 투영**: `W^O`로 헤드 정보를 혼합해 **다음 계층 차원**으로 환원한다.

#### 한줄 요약
- **헤드별 병렬 관계**를 연결해 통합 표현을 생성한다.
...
- **키-값 캐시(Key-Value Cache, KV Cache)**: 이전 토큰의 K•V를 저장해 다음 토큰 생성에 재사용하는 메모리다.
...
#### 한줄 요약
- **K•V 공유 범위**로 표현력•KV Cache 비용 절충이 핵심이다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 헤드 증가에 따른 **관계 학습 중복** | **헤드 다양성•제거 실험** 후 수 조정 | 불필요 연산과 매개변수 **감축** |
...
- 최대 표현력에는 **MHA**, 최소 캐시에는 **MQA**, 절충에는 **GQA**을 선택한다.

#### 한줄 요약
- **표현력•KV Cache 비용**에 따라 MHA•MQA•GQA을 선택한다.
