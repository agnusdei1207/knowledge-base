...
**메타버스**는 지속되는 공유 가상공간에서 이용자가 사회·업무·경제 활동과 디지털 권리를 이어가는 서비스 생태계다.
...
**지속성**은 이용자가 접속을 종료한 뒤에도 가상 세계의 상태·관계·활동 결과가 보존되는 성질이다.
...
**아바타**는 가상 세계에서 사용자의 정체성과 행위를 표현하는 디지털 대리 객체다.
...
**공동 상태 동기화**는 여러 이용자에게 가상 공간과 객체의 변경 결과를 일관되게 전달하는 과정이다.
...
**디지털 트윈**은 물리 대상의 상태와 이력을 가상 표현에 동기화하여 분석·예측을 지원하는 기술이다.
...
**가상 자산 권리**는 디지털 객체의 사용·이전·수익 권한과 이를 집행하는 플랫폼 규칙의 결합이다.
...
**상호운용성**은 서로 다른 가상공간이 정체성·자산·활동 데이터를 합의된 형식과 권한으로 교환하는 능력이다.
