...
date: "2026-07-31T09:08:08+09:00"
tags:
  - "notes-latest-tech"
weight: 226
extra:
...
- 배경/필요성: GELU 단일 경로는 입력별 **특징 선택력 확장에 한계**
...
- **선택 축**: SwiGLU 곱셈 게이트의 입력별 특징 선택
- **단순성 축**: GELU 단일 경로의 구현·메모리 절감
- **비용 축**: SwiGLU 삼중 투영의 파라미터·연산 증가
### 쉽게 이해하기 (학습용)
...
1. **활성·게이트 특징 생성**: GELU 입력·Swish 게이트 계산
2. **값 특징 생성**: SwiGLU 독립 선형 값 경로 생성
3. **GELU 또는 Swish 적용**: 단일 활성·Swish 게이트 적용
4. **경로 선택·요소별 결합**: 활성 전달·게이트×값 계산
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 파라미터 수 차이로 **품질 비교 왜곡** | **FFN 예산 기준 내부 폭** 조정 | 공정한 품질·비용 비교 |
| 추가 투영으로 **메모리 대역폭 증가** | **융합 커널·텐서 병렬 배치** | 지연·메모리 이동 감소 |
| 환경 차이로 **품질 이득 편차** | **동일 데이터·학습량 A/B 시험** | 실제 선택 근거 확보 |
