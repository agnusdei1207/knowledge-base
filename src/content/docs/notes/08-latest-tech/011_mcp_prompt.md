...
- **MCP 프롬프트(MCP Prompt)**: 서버가 진열한 작업 양식을 사용자가 골라 값을 채우고, 완성된 메시지를 검토한 뒤 모델에 전달하는 재사용 기능이다.
- **모델 컨텍스트 프로토콜(Model Context Protocol, MCP)**: 인공지능 애플리케이션과 외부 서버가 컨텍스트와 기능을 교환하는 표준 프로토콜이다.
- **사용자 제어(User-controlled)**: 프롬프트의 선택과 모델 전달 여부를 사용자가 결정하는 제어 방식이다.
- **프롬프트 목록(prompts/list)**: 서버가 제공하는 프롬프트의 이름·설명·인자를 조회하는 요청이다.
- **프롬프트 조회(prompts/get)**: 프롬프트 이름과 인자를 보내 완성된 메시지를 받는 요청이다.
- **프롬프트 메시지(PromptMessage)**: 역할과 텍스트·이미지·내장 리소스 등의 콘텐츠를 묶은 메시지다.
- **내장 리소스(Embedded Resource)**: 프롬프트 메시지 안에 직접 포함된 문서나 코드 같은 컨텍스트다.
...
- 사용자가 템플릿 선택과 전달을 결정하는 **사용자 제어**
- 인자값으로 역할별 메시지를 생성하는 **매개변수화**
- 텍스트·이미지·내장 리소스를 묶는 **복합 콘텐츠**
...
2. **템플릿 선택·인자 입력**: 사용자가 작업 양식과 **필수 인자** 결정
3. **완성 메시지 조회**: `prompts/get`으로 인자가 반영된 **메시지 생성** 요청
4. **역할별 메시지 검토 요청**: 반환 콘텐츠와 출처를 확인하는 **사용자 통제**
5. **승인한 메시지 전달**: 검토가 끝난 메시지만 **모델 문맥**에 반영
