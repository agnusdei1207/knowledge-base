...
1. 금지 의도 변형•은닉: 역할극•인코딩•다단계 맥락으로 위장
2. 탈옥 프롬프트 주입: 변형 지시를 정렬 모델에 입력
3. 안전 지시 우선순위 교란: 가상 맥락을 상위 지시로 해석
4. 금지 응답•행동 후보 생성: 거부 경계를 넘는 출력 구성
5. 정책 위반 오분류•통과: 독립 판정이 변형 의도를 누락
