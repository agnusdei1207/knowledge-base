...
- **탈옥 공격(Jailbreak Attack)**: 변형 지시로 모델의 안전 정책과 거부 규칙을 우회해 금지된 응답이나 행동을 유도하는 공격이다.
- **과잉 거부**: 안전 장치가 허용해야 할 정상 요청까지 차단하는 현상이다.

</details>

- 정의/개념: **탈옥 공격**은 변형 지시로 안전 정책•거부 규칙을 우회하는 공격이다.
- 배경/필요성: 정적 필터는 **변형 지시**를 놓치고 **정상 요청**을 과잉 차단이 필요하다.
...
- 역할극•인코딩•다단계 대화 기반 **거부 정책 우회**가 핵심이다.
- 정책 반응에 맞춰 표현을 바꾸는 **적응형 공격**이 핵심이다.
- **공격 성공률•과잉 거부율**의 공동 회귀를 평가한다.
...
- **정렬 모델(Aligned Model)**: 안전 정책과 선호 데이터를 학습해 정상 요청에는 유용하게 응답하고 금지 요청은 거부하도록 조정된 모델이다.
- **독립 판정기**: 정렬 모델과 분리되어 응답•행동의 적대적 의도와 정책 위반을 분류하는 구성요소이다.
- **권한 게이트**: 모델의 도구 계획이 실제 실행되기 전에 허용 여부를 결정적으로 검사하는 구성요소이다.
- **적대 평가 체계**: 알려진•변형된 탈옥 입력에 대한 정책 위반률과 정상 요청의 과잉 거부율을 함께 측정하는 개념이다.
...
1. **금지 의도 변형•은닉**: 역할극•인코딩•다단계 맥락으로 위장한다.
2. **탈옥 프롬프트 주입**: 변형 지시를 정렬 모델에 입력한다.
3. **안전 지시 우선순위 교란**: 가상 맥락을 상위 지시처럼을 해석한다.
4. **금지 응답•행동 후보 생성**: 거부 경계를 넘는 출력을 구성한다.
5. **정책 위반 오분류•통과**: 독립 판정이 변형 의도를 놓침한다.
...
- 공격 성공률이 높으면 **안전 통제** 강화, 과잉 거부율이 높으면 **예외 기준**을 조정한다.
