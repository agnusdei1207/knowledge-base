...
- 정의/개념: 다중 노드의 컨테이너를 선언한 목표 상태에 맞춰 조정·배치·확장·복구하는 **쿠버네티스(Kubernetes, K8s) 오케스트레이션 플랫폼**
...
- **파드(Pod)**: 하나 이상의 컨테이너가 네트워크와 저장 공간을 공유하는 쿠버네티스(Kubernetes)의 최소 배포 단위이다.
...
- **kubelet**: 각 노드에서 지정된 파드의 컨테이너를 실행하고 상태를 응용 프로그래밍 인터페이스 서버에 보고하는 에이전트이다.
...
응용 프로그래밍 인터페이스 서버(Application Programming Interface Server, API Server)는 파드(Pod)의 목표·실제 상태를 각 제어 구성요소에 전달한다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 준비되지 않은 파드(Pod)로 **트래픽 오류** | 준비 상태 검사(Readiness Probe)·서비스(Service) 엔드포인트 연계 | 정상 파드만 **요청 수신** |
...
- **무상태 서비스**: 개별 파드(Pod)가 영속 상태를 소유하지 않아 다른 복제본으로 교체할 수 있는 서비스이다.
