...
- 정의/개념: 다중 노드의 컨테이너를 선언한 목표 상태에 맞춰 조정•배치•확장•복구하는 **K8s 오케스트레이션 플랫폼**이다.
- 배경/필요성: 다중 노드 수동 운영으로 **배치•연결•복구 상태** 불일치가 발생한다.
...
- **객체 축**: Pod를 최소 배포 단위로 선언•관리한다.
- **조정 축**: 컨트롤러가 목표•실제 상태 차이를 축소한다.
- **실행 축**: 스케줄러•kubelet의 배치•실행 분담이다.
...
1. **목표•실제 상태 감시**: 컨트롤러의 복제본 차이를 계산한다.
2. **부족 Pod 생성 요청**: 필요한 미배치 Pod 객체를 생성한다.
3. **미배치 Pod 감시**: 스케줄 대상 Pod을 탐색한다.
4. **노드 바인딩 기록**: 자원•제약 기반 실행 노드를 결정한다.
5. **바인딩된 Pod 전달**: kubelet의 컨테이너를 실행한다.
...
- **무상태 서비스•영속 볼륨**에 따라 Deployment와 StatefulSet을 선택한다.
