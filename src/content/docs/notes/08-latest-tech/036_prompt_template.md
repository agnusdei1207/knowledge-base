...
- 정의/개념: 고정 지시•검증된 입력 슬롯•출력 계약을 결합해 같은 작업에 반복 사용하는 **프롬프트 템플릿**이다.
- 배경/필요성: 요청마다 직접 작성하면 지시•입력 검증•결과 형식이 달라져 **재현성**이 깨진다.
...
- **정적 블록**: 반복 업무의 공통 지시를 재사용한다.
- **변수 슬롯**: 실행별 업무값이 들어갈 입력 자리를 정의한다.
- **슬롯 검증**: 자료형•길이•허용값을 판정한다.
- **안전 결합**: 검증값을 이스케이프 후 삽입한다.
- **통합 버전 관리**: 템플릿•모델•출력 계약의 호환 조합을 고정한다.
- **회귀 검증**: 변경 후 기존 과업의 품질 저하 여부를 확인한다.
...
1. **승인 템플릿 버전 로드**: 지시•변수•출력 계약의 호환 조합을 호출한다.
2. **슬롯 입력 검증**: 자료형•길이•허용값•접근 권한을 판정한다.
3. **고정 지시•검증값 렌더링**: 이스케이프 후 지정 슬롯을 결합한다.
4. **출력 계약 검증**: 응답 구조와 업무 규칙 충족 여부를 판정한다.
...
- **업무 반복성**: 같은 지시•입력•출력 형식이 반복되는 작업의 성격이다.
- **업무 정형성**: 결과 형식과 절차가 사전에 정의되어 동일하게 적용 가능한 작업의 성격이다.

</details>

- 일회성 탐색은 **임시 프롬프트**, 반복•정형 업무는 **템플릿**을 선택한다.
