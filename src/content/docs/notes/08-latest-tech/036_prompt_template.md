...
- 배경/필요성: 요청마다 직접 작성하면 지시•입력 검증•결과 형식이 달라져 **결과 재현하기 어렵다**.

#### 한줄 요약

- 고정 지시와 **검증된 변수•출력 계약**을 결합하는 서식이 핵심이다.
...
- **정적 블록**: 반복 업무의 공통 지시 재사용한다.
- **변수 슬롯**: 실행별 업무값의 입력 자리를 정의한다.
- **슬롯 검증**: 자료형•길이•허용값을 판정한다.
- **안전 결합**: 검증값을 이스케이프 후 삽입이다.
- **통합 버전 관리**: 템플릿•모델•출력 계약 조합 고정이다.
- **회귀 검증**: 변경 후 기존 과업의 품질 저하를 확인한다.

#### 한줄 요약

- **슬롯•출력•모델 버전**을 호환 조합으로 검증한다.
...
- 일회성은 **임시 프롬프트**, 반복 업무는 **템플릿**이 핵심이다.
