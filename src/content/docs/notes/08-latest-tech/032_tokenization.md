...
- 배경/필요성: 신경망은 원문 문자열이 아니라 **고정 어휘의 정수 인덱스**를 입력으로 요구를 충족해야 한다.

#### 한줄 요약
- 문자열을 **어휘 조각•Token ID**로 변환이 핵심이다.
...
- 정규화•사전 분절•학습 규칙에 따른 **분절 경계 결정**이 핵심이다.
- 희귀어를 문자보다 큰 조각으로 표현하는 **미등록어 완화**가 핵심이다.
- **어휘 크기•토큰 수 관계**: 어휘가 커지면 토큰 수는 줄지만 임베딩 매개변수 증가한다.

#### 한줄 요약
- **어휘 크기•토큰 수** 사이의 비용 절충이 핵심이다.
...
2. **초기 문자열 경계 분절**: 공백•구두점•언어 규칙으로 기본 경계를 생성한다.
3. **BPE 서브워드 분절**: 어휘에 존재하는 최장•최적 조각을 탐색한다.
4. **토큰•특수 ID 대응**: 시작•끝 등 제어 기호를 포함해 **정수 ID**로 매핑한다.

#### 한줄 요약
- **학습•운영 분절 규칙**을 일치시켜 ID 의미를 보존한다.
...
#### 한줄 요약
- 범용은 **서브워드**, 미지 문자열은 **문자•바이트**가 핵심이다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 학습•운영의 **정규화 정책 불일치** | **토크나이저 파일•버전 고정** | 입력 ID의 **재현성 확보** |
...
- **복호(Decoding)**: 생성된 Token ID 열을 사람이 읽을 수 있는 문자열로 되돌리는 처리다.

</details>

- 범용•다국어에는 **서브워드 BPE**, 미지 문자열에는 **문자•바이트**를 선택한다.

#### 한줄 요약
- **언어•미지 문자열 비중**에 따라 분절 단위를 선택한다.
