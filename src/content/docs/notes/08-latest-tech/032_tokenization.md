...
- **토큰 ID(Token ID)**: 모델 어휘의 각 토큰에 대응하여 신경망 입력으로 사용하는 정수 식별자다.

</details>

- 정의/개념: **토큰화**는 입력 문자열을 모델 어휘의 부분 단위로 분할하고 각 단위를 정수 토큰 ID로 변환하는 전처리 과정
...
- **ID 매핑**: 분절한 문자열 조각과 제어 기호를 어휘표의 정수 식별자에 대응시키는 과정이다.
...
- **단어 단위 토큰화**: 단어 전체를 하나의 ID로 표현하여 토큰열은 짧지만 어휘와 미등록어가 늘어나는 방식이다.
...
- **서브워드 BPE 선택 기준**: 범용·다국어 입력에서 미등록어를 줄이면서 문맥 길이와 어휘 크기를 절충할 때 사용한다.
- **문자·바이트 선택 기준**: 임의 문자열을 반드시 표현해야 하고 긴 토큰열 비용을 감수할 수 있을 때 사용한다.
- **복호(Decoding)**: 생성된 토큰 ID 열을 사람이 읽을 수 있는 문자열로 되돌리는 처리다.
