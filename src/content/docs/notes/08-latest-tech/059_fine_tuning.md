...
- **지도 미세조정(SFT)**: 입력·정답 쌍으로 목표 출력 행동을 학습하는 방식이다.
- **PEFT**: 전체 기반 모델을 고정하고 일부 추가·선택 매개변수만 갱신하는 기술군이다.
...
- **전체 FT 선택 기준**: 모델 전반의 행동을 크게 바꿀 필요가 있고 충분한 학습·저장 자원이 있을 때 선택한다.
- **PEFT·프롬프트 튜닝 선택 기준**: 기반 모델을 공유하면서 작업별 자원과 배포 자산을 줄여야 할 때 선택한다.
