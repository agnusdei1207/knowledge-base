...
- 배경/필요성: 프롬프트만으로는 **반복 행동•출력 형식**의 일관성 확보할 수 없다.

#### 한줄 요약
- **목적 데이터**로 작업•도메인 행동 적응이 핵심이다.
...
- **SFT 데이터 분포•품질**과 목적함수에 따른 행동•일반화 범위를 결정한다.
- **Full FT•PEFT•프롬프트 튜닝**별 갱신 대상•자원•적응 폭 차이 핵심이다.
- **기존 능력 회귀•과적합**과 안전성 변화의 기준 모델 대비를 평가한다.
...
5. **버전 등록•배포**: 모델•데이터•설정의 **추적 가능한을 연결**한다.
#### 한줄 요약
