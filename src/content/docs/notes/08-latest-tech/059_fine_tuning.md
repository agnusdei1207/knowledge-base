...
- 정의/개념: **파인튜닝(Fine-Tuning)** 은 사전학습 모델을 목적 데이터로 학습해 작업·도메인에 적응시키는 방법
...
- **전체 파인튜닝(Full Fine-Tuning, Full FT)·매개변수 효율적 파인튜닝(Parameter-Efficient Fine-Tuning, PEFT)** 과 프롬프트 튜닝별 갱신 대상·자원·적응 폭 차이
- **기존 능력 회귀·과적합** 과 안전성 변화의 기준 모델 대비 평가
- 입력·정답 쌍의 **지도 파인튜닝(Supervised Fine-Tuning, SFT)** 을 통한 목표 출력 행동 학습
...
- **어댑터 기반 매개변수 효율적 파인튜닝(Parameter-Efficient Fine-Tuning, PEFT)**: 기반 가중치를 동결하고 작은 작업별 모듈만 갱신한다.
- **프롬프트 튜닝**: 모델 가중치 대신 입력에 붙는 학습 가능한 가상 토큰만 갱신한다.

</details>

**전체 파인튜닝(Full Fine-Tuning, Full FT)** 과 **매개변수 효율적 파인튜닝(Parameter-Efficient Fine-Tuning, PEFT)** 은 기반 모델에서 갱신하는 매개변수 범위가 다르다.
...
- 전반 적응은 **전체 파인튜닝(Full Fine-Tuning, Full FT)**, 자원·갱신 제약은 **매개변수 효율적 파인튜닝(Parameter-Efficient Fine-Tuning, PEFT)·프롬프트 튜닝** 선택
