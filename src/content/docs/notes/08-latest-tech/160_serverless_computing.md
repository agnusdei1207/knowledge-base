...
- 정의/개념: **서버리스 컴퓨팅(Serverless Computing)** 은 공급자가 인프라 운영·자동 확장·용량 할당·사용량 계량을 맡는 클라우드 실행 모델
...
- **구성 축**: **서비스형 함수(Function as a Service, FaaS)·서비스형 백엔드(Backend as a Service, BaaS)·서버리스 컨테이너** 조합
...
| 서비스 백엔드 | **인증·데이터베이스(Database, DB) 완성 기능** 제공 |
...
2. **백엔드 기능 요청**: 인증·데이터베이스 **응용 프로그래밍 인터페이스(Application Programming Interface, API)** 기반 관리형 서비스 호출
...
- **서비스형 백엔드(Backend as a Service, BaaS)**: 인증·데이터베이스 등 완성된 백엔드 기능을 응용 프로그래밍 인터페이스로 제공하는 서버리스 유형이다.
- **서버리스 컨테이너**: 사용자 컨테이너 이미지를 요청이나 작업량에 따라 자동 확장하는 서버리스 유형이다.

</details>

**서비스형 함수(Function as a Service, FaaS)**, **서비스형 백엔드(Backend as a Service, BaaS)**, 서버리스 컨테이너는 코드·완성 기능·실행 이미지 중 관리 단위가 다르다.
...
- 짧은 이벤트 처리는 **서비스형 함수(Function as a Service, FaaS)**, 완성 백엔드 활용은 **서비스형 백엔드(Backend as a Service, BaaS)** 선택
