...
1. **양자 상태 전송**: Alice가 무작위 기저의 양자 신호를 생성함.
2. **기저•측정 기록 전달**: Bob이 무작위 기저로 측정한 결과를 기록함.
3. **송신 기저 기록 전달**: Alice의 기저를 인증 채널로 공개함.
4. **선별 비트•QBER 전달**: 일치 기저의 표본 오류율로 세션 수용을 판정함.
...
- 거리•**QBER•키율**을 충족하는 **고가치 전용 구간**에만 QKD을 적용함.

#### 한줄 요약

- 장비 도입보다 인증•오류 검사•키 정제•공급의 전 과정을 관리해야  체계 적용
