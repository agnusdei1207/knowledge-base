...
- **키 선별•오류 정정**: 호환되는 측정 결과만 남기고 정당한 송수신자의 비트 불일치를 제거하는 후처리이다.
...
2. **기저•측정 기록 전달**: Bob이 무작위 기저로 측정한 결과 기록
3. **송신 기저 기록 전달**: Alice의 기저를 인증 채널로 공개
4. **선별 비트•QBER 전달**: 일치 기저의 표본 오류율로 세션 수용 판정
5. **오류 정정•프라이버시 증폭**: 불일치와 정보 누출을 제거해 키 정제
...
- 전용 광선로의 QKD 키 생성과 QBER•키 생성률 검증
