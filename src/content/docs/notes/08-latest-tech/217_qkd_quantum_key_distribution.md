...
date: "2026-08-14T13:08:00+09:00"
tags:
  - "notes-latest-tech"
weight: 217
extra:
...
- 정의: **QKD** 기반 양자 상태의 측정 교란으로 도청을 탐지하며 대칭키를 분배하는 기술임이다.
- 배경/필요성: 기존 키 교환은 통신 중 **도청 여부** 활용 직접 확인할 수 없음.
...
- 양자 상태 전송과 인증•후처리를 분리한 **이중 채널** 핵심임.
- QBER 임계값 초과 시 세션을 폐기하는 **도청 탐지** 핵심임.
- 기저 선별•오류 정정•프라이버시 증폭 기반 **키 정제** 핵심임.
#### 한줄 요약
...
- **키 선별•오류 정정**: 호환되는 측정 결과만 남기고 정당한 송수신자의 비트 불일치를 제거하는 후처리하는 개념을 말한다.
- **프라이버시 증폭**: 도청자가 얻었을 수 있는 정보를 해시로 압축해 최종 비밀키에서 제거하는 과정을 말한다.

</details>

프로토콜은 **QKD 보안 파라미터** 및 **QBER 임계값** 정의한다.
...
1. 양자 상태 전송: Alice가 무작위 기저의 양자 신호를 Bob에게 전송함.
2. 기저•측정 기록: Bob이 무작위 기저로 측정한 결과를 기록함.
3. 송신 기저 기록: Alice의 기저를 인증 채널로 공개함.
4. 선별 비트•QBER: 일치 기저의 표본 오류율로 세션 수용을 판정함.
5. 오류 정정•프라이버시 증폭: 불일치와 정보 누출을 제거해 키를 정제함.
...
- 거리•**QBER**•**키율** 충족하는 **고가치 전용 구간**에만 QKD를 적용함.

#### 한줄 요약

- 인증•오류 검사•키 정제•공급 전 과정의 통합 관리.
