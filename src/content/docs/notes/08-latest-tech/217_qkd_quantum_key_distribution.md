...
- 정의/개념: **QKD**는 양자 상태의 측정 교란으로 도청을 탐지하며 대칭키를 분배하는 기술임.
- 배경/필요성: 기존 키 교환은 통신 중 **도청 여부**를 직접 확인할 수 없음.
...
- 양자 상태 전송과 인증•후처리를 분리한 **이중 채널**이 핵심임.
- QBER 임계값 초과 시 세션을 폐기하는 **도청 탐지**가 핵심임.
- 기저 선별•오류 정정•프라이버시 증폭 기반 **키 정제**가 핵심임.
#### 한줄 요약
...
- 거리•**QBER•키율**을 충족하는 **고가치 전용 구간**에만 QKD를 적용함.
