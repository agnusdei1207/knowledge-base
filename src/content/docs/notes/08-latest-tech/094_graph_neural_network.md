...
- **GNN (Graph Neural Network)**: 노드•엣지로 구성된 그래프 데이터의 위상 정보와 구조적 특성을 메시지 패싱(Message Passing)으로 학습하는 신경망
- **간선 관계 (Edge Relation)**: 그래프상 두 노드 간 연결 유형과 방향 정보

</details>

- 정의: 이웃 노드의 특징을 집계하여 대상 노드•링크•그래프의 표현을 학습하는 모델
- 배경: 독립 표본 기반 모델의 그래프 위상 정보 및 간선 관계 활용 한계 극복
...
- **메시지 전달 (Message Passing)**: 이웃의 특징을 계산하여 대상 노드에 전달 및 갱신하는 과정
- **과평활 (Over-smoothing)**: 층이 깊어짐에 따라 노드 표현이 유사해져 구별력이 감소하는 현상
- **과압축 (Over-squashing)**: 정보 압축 과정에서 그래프의 주요 관계가 소실되는 현상

</details>

- **메시지 전달**: 이웃 노드 및 간선 정보의 반복 집계
- **순서 불변성**: 이웃 입력 순서와 무관한 집계•갱신 함수 적용
- **구별력 이슈**: 깊은 층 전파 시 발생하는 과평활•과압축 현상 제어
...
| 문제 | 대책 | 기대 효과 |
|:---|:---|:---|
| 과평활 | 잔차 연결 및 전파 층수 제한 | 노드 구별력 보존 |
| 연산량 폭증 | 이웃 표본화 및 미니배치 학습 | 효율적 대규모 그래프 처리 |
| 평가 누출 | 시간 순서 기반 그래프 분할 | 모델 일반화 성능 검증 |
