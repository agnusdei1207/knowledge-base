...
- **GNN(Graph Neural Network)**: 노드(Node)와 엣지(Edge)로 구성된 그래프 데이터의 위상 정보(Topological Info)와 구조적 특성을 메시지 패싱(Message Passing)으로 학습하는 신경망 모델
- **간선 관계(Edge Relation)**: 그래프상에서 두 노드를 연결하는 관계의 유형, 강도 및 방향성 정보

</details>

- 정의: 이웃 노드의 특징을 순차적으로 집계하여 대상 노드, 링크, 또는 전체 그래프의 표현(Representation)을 벡터로 학습하는 모델
- 배경: 독립 표본(i.i.d) 기반의 기존 모델들이 갖는 그래프 위상 정보 활용의 한계 및 노드 간 간선 관계 정보 유실 극복 필요

#### 한줄 요약
- 그래프 위상 정보 기반의 구조적 관계 학습 체계 구현
...
- **메시지 전달(Message Passing)**: 이웃 노드의 특징 벡터를 계산하여 대상 노드에 전달하고, 현재 상태를 최신 상태로 갱신하는 GNN의 핵심 학습 과정
- **과평활(Over-smoothing)**: 층(Layer)이 깊어짐에 따라 인접 노드 간의 특징 벡터가 수렴하여 서로 유사해져 개별 구별력이 떨어지는 현상
- **과압축(Over-squashing)**: 그래프 내의 방대한 정보가 고정된 크기의 벡터로 압축되는 과정에서 주요 관계 정보가 손실되는 현상

</details>

- **메시지 전달**: 이웃 노드 및 간선 정보의 반복적 집계 및 전파를 통한 노드 표현 강화
- **순서 불변성(Permutation Invariance)**: 그래프 이웃 노드 입력 순서와 상관없이 동일한 결과를 도출하는 집계 및 갱신 함수 적용
- **구별력 이슈**: 딥러닝 계층 전파 시 발생하는 과평활, 과압축 현상을 제어하기 위한 잔차 연결 등의 설계 필요

#### 한줄 요약
- 순서 불변의 이웃 정보 집계를 통한 그래프 구조적 표현 학습
