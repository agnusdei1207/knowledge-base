...
- 정의/개념: **강화학습(Reinforcement Learning)** 은 환경과 상호작용하며 기대 누적 보상을 높이는 정책 학습 기법
...
- **정책(Policy)**: 상태에서 각 행동을 선택할 확률이나 규칙을 정의하여 에이전트의 행동을 결정한다.
- **보상 함수**: 행동 결과가 목표에 기여한 정도를 즉시 보상 신호로 계산한다.
- **가치 함수(Value Function)**: 특정 상태나 상태·행동에서 기대되는 할인 누적 보상을 추정한다.
