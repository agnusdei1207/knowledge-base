...
- **정책 역할**: 상태에서 각 행동을 선택할 확률이나 규칙 정의
- **보상 함수**: 행동 결과가 목표에 기여한 정도를 즉시 보상 신호로 계산한다.
- **가치 함수 역할**: 상태나 상태•행동의 기대 할인 누적 보상 추정
...
1. **현재 상태 관측**: 환경의 의사결정 정보 획득
2. **행동 선택**: 탐색•활용 전략에 따라 정책 표본화
3. **선택 행동 실행**: 결정한 행동을 환경에 적용
4. **경험 수집**: 보상•다음 상태•종료 여부 저장
5. **가치•정책 갱신**: 장기 보상 추정으로 행동 확률 개선
