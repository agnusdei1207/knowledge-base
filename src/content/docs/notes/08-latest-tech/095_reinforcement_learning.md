...
- **강화학습(Reinforcement Learning)**: 에이전트가 환경과 상호작용하며 기대 누적 보상을 높이는 정책을 학습하는 기법이다.
- **지연 보상**: 행동 직후가 아니라 이후 상태나 종료 시점에 제공되는 보상이다.

</details>

- 정의/개념: **강화학습**은 환경과 상호작용하며 기대 누적 보상을 높이는 정책 학습 기법이다.
- 배경/필요성: 정답 라벨은 지연 보상의 **순차 행동 기여도**를 산출할 수 없다.
...
- 에이전트가 행동하고 상태•보상을 받는 **정책 기반 상호작용**이 핵심이다.
- 정책의 기대 누적 보상을 추정하는 **가치 함수**가 핵심이다.
- 정책 품질을 위한 **탐색•활용 균형**과 목표 정렬을 위한 **보상 설계**가 핵심이다.
...
- **정책 역할**: 상태에서 각 행동을 선택할 확률이나 규칙을 정의하는 역할이다.
- **보상 함수**: 행동 결과가 목표에 기여한 정도를 즉시 보상 신호로 계산하는 함수이다.
...
1. **현재 상태 관측**: 환경의 의사결정 정보를 획득한다.
2. **행동 선택**: 탐색•활용 전략에 따라 정책을 표본화한다.
3. **선택 행동 실행**: 결정한 행동을 환경에 적용한다.
4. **경험 수집**: 보상•다음 상태•종료 여부를 저장한다.
5. **가치•정책 갱신**: 장기 보상 추정으로 행동 확률을 개선한다.
...
- 장기 보상•탐색 위험이 있는 과업에 **강화학습**을 적용하고 **보상 정렬**을 검증한다.
