...
- **온톨로지(Ontology)**는 도메인의 개념·관계·제약·공리를 명시해 여러 시스템이 공유하는 의미 모형이다.
...
- **개념 계층**은 상위·하위 개념과 상속 관계를 정의한다.
- **공리(Axiom)**는 암묵 지식을 추론하고 모순을 판정하는 논리 규칙이다.
...
- **객체 속성**은 개체와 개체의 관계를, **데이터 속성**은 개체와 리터럴 값의 관계를 정의한다.
- **정의역·치역·카디널리티**는 속성을 적용할 클래스, 결과 클래스, 허용 개수를 제한한다.
...
- **추론기**는 명시적 사실과 공리를 결합해 분류·파생 사실·모순 여부를 계산한다.
- **일관성 검사**는 개념·관계·규칙 사이의 논리적 모순을 탐지한다.
...
- **분류체계·온톨로지·지식그래프**는 각각 용어 계층, 의미 제약·추론, 개체·관계 사실과 경로 탐색에 중점을 둔다.
...
- **OWL**은 클래스·속성·공리를 기계가 해석할 수 있게 표현하는 W3C 온톨로지 언어다.
- **OWL 프로파일**은 필요한 표현력과 추론 비용에 맞춰 사용할 언어 기능의 범위를 제한한다.
...
- **하위 호환성**은 온톨로지 변경 후에도 기존 개체·질의·연동 시스템이 의도한 의미로 계속 동작하는 성질이다.
