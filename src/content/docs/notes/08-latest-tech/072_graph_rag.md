...
- **GraphRAG**: 문서에서 추출한 개체·관계와 커뮤니티 요약을 검색 근거로 사용하는 RAG 방식이다.
- **다중 홉 질의**: 답을 찾기 위해 여러 개체의 관계를 차례로 따라가야 하는 질의이다.

</details>

- 정의/개념: 개체·관계·커뮤니티 요약을 구조화해 검색하는 **그래프 기반 RAG 방식**
...
- 원문 사실과 근거를 연결하는 **개체·관계·출처 그래프**
- 주변 관계와 집단 요약을 나누는 **지역·전역 검색**
...
- **벡터 RAG**: 질의와 문서 조각의 임베딩 유사도를 기준으로 근거를 검색하는 방식이다.
- **하이브리드 RAG**: 벡터·어휘·그래프 검색 신호를 함께 사용해 근거 후보를 구성하는 방식이다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 별칭·동명이인의 **혼합** | **개체명 통합**과 식별자 분리 | 잘못된 **관계 연결 감소** |
| **관계 추출 오류** | 관계마다 **원문 출처** 연결 | 그래프 **근거 검증** |
| 삭제된 **관계 잔존** | 원문 변경과 **그래프·요약 갱신** 연계 | **최신 관계** 유지 |
