...
- 정의/개념: **지식 증류**는 교사 모델의 출력•특징•관계를 학생 모델에 전달하는 압축 학습이다.
- 배경/필요성: 정답 레이블만으로는 **클래스 관계•중간 표현** 전달할 수 없다.

#### 한줄 요약
- 교사의 **출력•특징•관계 지식**을 학생에게 이전이 핵심이다.
...
- 교사 **소프트 타깃**의 정답 외 클래스 간 상대 관계를 전달한다.
- 교사•학생 **로짓•템퍼러처**를 통한 확률분포 평활화가 핵심이다.
- 정답 손실과 **KL 발산 기반 증류 손실**의 가중 결합이 핵심이다.

#### 한줄 요약
- **소프트 타깃•템퍼러처•복합 손실**로 지식 이전이 핵심이다.
...
1. **증류 신호 선택**: 출력•특징•관계 중 **전이 대상**을 결정한다.
2. **교사 지식 생성**: 템퍼러처를 적용한 **교사 분포**를 산출한다.
3. **학생 예측 생성**: 동일 입력에 대한 **학생 분포**를 산출한다.
4. **복합 손실 갱신**: 정답•증류 손실로 **학생 매개변수**를 갱신한다.
5. **학생 단독 평가**: 교사 없이 정확도•지연•**메모리를 실측**한다.
#### 한줄 요약
...
- 출력 접근만 가능하면 **응답 증류**, 중간 표현 접근 시 **특징•관계 증류**가 핵심이다.
