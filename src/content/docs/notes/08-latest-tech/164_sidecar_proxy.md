...
- **사이드카 패턴**: 주 애플리케이션과 수명 주기·배포 경계를 공유하는 보조 프로세스로 공통 기능을 분리하는 패턴
- **트래픽 인터셉션**: 네트워크 규칙을 사용해 인바운드·아웃바운드 요청을 프록시 경로로 전환하는 방식
- **mTLS**: 통신 양쪽이 인증서를 검증하고 구간을 암호화하는 상호 TLS
...
> 요약: 요청을 사이드카 경로로 전환해 애플리케이션 밖에서 정책을 집행한다
...
2. **서비스 요청**: 애플리케이션이 대상 서비스 호출
3. **요청 경로 전환**: 네트워크 규칙이 트래픽을 프록시로 포착
4. **보안·라우팅 집행**: 신원·인가를 확인하고 목적지·시간 제한 적용
5. **응답·관측 결과**: 응답을 반환하며 지표·로그·추적 생성

> 요약: 정책을 받은 프록시가 포착된 요청을 검사·중계하고 결과를 관측한다
...
| 핵심 특징 | Pod별 독립 프록시 | 여러 워크로드가 프록시 공유 | 업무 프로세스 내부에서 제어 |
| 한계 | 자원·기동·장애 경로 증가 | 공유 장애·격리 제약 | 언어 결합·일관성 유지 부담 |

> 요약: 격리 수준과 프록시 비용 및 코드 결합도를 기준으로 선택한다
...
| 재시도·시간 제한의 중복 | 애플리케이션과 프록시 책임 단일화 | 요청 증폭과 긴 대기 방지 |
| 사이드카 자원 부족 | 워크로드별 CPU·메모리·연결 수 산정 | 지연 증가와 OOM 완화 |
...
- 업무 코드 수정 없이 통신 정책을 일관되게 적용하기 위해 **트래픽 포착·정책 책임·기동 순서·프록시 자원·장애 경로**를 검토하고, 워크로드별 격리가 필요한 환경에 사이드카 프록시를 적용한다.
