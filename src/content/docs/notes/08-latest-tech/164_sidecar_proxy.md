...
- 정의/개념: **사이드카 프록시(Sidecar Proxy)** 는 애플리케이션과 같은 배포 단위에서 인바운드•아웃바운드 트래픽을 중계하고 통신 정책을 대신 집행하는 보조 프록시
...
- **사이드카 프록시(Sidecar Proxy)**: 애플리케이션 옆에서 서비스 트래픽을 중계하고 통신 정책을 집행하는 데이터 평면 구성요소이다.
...
- **상호 전송 계층 보안(mTLS)**: 통신 양쪽이 인증서를 검증하고 전송 구간을 암호화하는 방식이다.
...
3. **보안•라우팅 집행**: **상호 전송 계층 보안(mutual Transport Layer Security, mTLS)** 으로 신원•인가를 확인하고 목적지•시간 제한 적용
...
| 핵심 특징 | 파드(Pod)별 **독립 프록시** | 여러 워크로드의 **프록시 공유** | 업무 프로세스 내부 **호출 제어** |
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| **프록시 준비 전 요청 유입** | 시작•준비•종료 순서와 **준비 상태(readiness) 연동** | **초기 연결 실패** 방지 |
| **재시도•시간 제한 중복** | 애플리케이션과 프록시의 **책임 단일화** | **요청 증폭•긴 대기** 방지 |
| **사이드카 자원 부족** | 워크로드별 **중앙처리장치(Central Processing Unit, CPU)•메모리•연결 수** 산정 | **지연 증가•메모리 부족(Out of Memory, OOM)** 완화 |
