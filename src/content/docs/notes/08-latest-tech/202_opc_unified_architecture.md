...
- 정의/개념: **OPC UA**는 산업 데이터의 의미•통신•보안을 통합한 상호운용 표준임.
- 배경/필요성: 공급사별 태그의 **의미•주소 체계 불일치**로 설비 연계 제약이 발생함.
...
- 노드•속성•참조로 설비를 표현하는 **의미 기반 정보 모델**이 핵심임.
- 클라이언트-서버•**PubSub**를 지원하는 **복수 통신 모델**이 핵심임.
- 인증•서명•암호화•권한 제어를 제공하는 **통합 보안**이 핵심임.
#### 한줄 요약
...
| 판단 기준 | OPC UA 클라이언트-서버 | OPC UA PubSub | 단순 태그 프로토콜 |
|:---|:---|:---|:---|
...
- 질의•배포 패턴에 따라 **클라이언트-서버**•**PubSub**를 선택하고 **인증서**를 검증함.

#### 한줄 요약

- **정보 모델•통신 패턴•인증서 신뢰**를 함께 표준화
