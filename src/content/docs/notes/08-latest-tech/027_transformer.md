...
- 정의/개념: 어텐션으로 문맥을 처리하는 신경망
- **배경/필요성**: 병렬화·장기 의존성 한계 개선
...
1. 긴 문서 분석 시 희소 어텐션과 청킹 기법 적용
2. 인코더·디코더 마스크 검증으로 정보 누출 방지

### 쉽게 이해하기 (학습용)
- 긴 책을 읽을 때 챕터별로 쪼개거나 중요한 요약 부분만 골라 읽음
- 시험지를 풀 때 뒷장의 정답을 미리 보지 못하도록 가림막을 둠

## Ⅶ. 결론

- 장기 의존성 극복을 위한 셀프 어텐션 아키텍처 선택

### 쉽게 이해하기 (학습용)
- 모든 단어의 관계를 한 번에 파악해 처리 효율을 극대화함
