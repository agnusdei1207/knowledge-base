...
| 어텐션 계층 | 셀프 어텐션을 통한 입력 토큰 간 관계 추출 |
...
sequenceDiagram
    ...
    participant O as 출력
    I->>A: 1. 토큰·위치 표현 구성
    A->>F: 2. Q·K·V 관계 가중치 계산
    F->>S: 3. 어텐션 문맥 결합
    S->>O: 4. FFN·잔차·정규화 갱신
    O-->>I: 5. 마스크 범위 내 출력 생성
```

1. **토큰·위치 표현 구성**: 토큰 의미와 **순서 정보** 결합
2. **Q·K·V 관계 가중치 계산**: 토큰 쌍의 **참조 중요도** 산출
3. **어텐션 문맥 결합**: 가중치에 따라 Value를 합성해 **문맥 표현** 생성
4. **FFN·잔차·정규화 갱신**: 위치별 변환과 잔차 경로로 **표현·기울기** 안정화
5. **마스크 범위 내 출력 생성**: 허용된 문맥만 참조해 **인코딩·다음 토큰** 출력
...
| 한계 | 문맥 길이에 따른 O(N²) 비용 | 병렬화 제한·장거리 정보 소실 |
