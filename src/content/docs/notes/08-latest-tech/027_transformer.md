...
- 배경/필요성: RNN의 순차 상태 전달은 **학습 병렬화와 장거리 의존성 학습**에 한계가 발생한다.

#### 한줄 요약
- **셀프 어텐션**으로 토큰 관계를 병렬을 계산한다.
...
- 셀프 어텐션에 의한 **모든 토큰 쌍 관계의 직접 계산**이 핵심이다.
- **셀프 어텐션**: 동일 시퀀스의 문맥 관계 결합한다.
- **크로스 어텐션**: 외부 시퀀스의 문맥 관계 결합한다.
- 순환 구조 제거와 **GPU 병렬 학습**이 핵심이다.
- 마스크에 의한 참조 범위 제어와 **O(N²) 어텐션 비용**이 핵심이다.

#### 한줄 요약
- 학습은 **GPU 병렬 처리**, 자기회귀 생성은 순차를 처리한다.
...
1. **토큰•위치 표현 구성**: 토큰 의미와 **순서 정보**를 결합한다.
2. **Q•K•V 관계 가중치 계산**: 토큰 쌍의 **참조 중요도**를 산출한다.
3. **어텐션 문맥 결합**: 가중치에 따라 Value를 합성해 **문맥 표현**을 생성한다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 긴 문맥의 **O(N²) 메모리•연산량** | **청크•윈도•희소 어텐션•부하 시험** | 처리량•문맥 보존의 **균형 확보** |
| 위치 표현의 **학습 길이 외 일반화** | **상대•회전 위치 표현•최대 길이 평가** | 장문 순서 정보의 **왜곡 완화** |
...
- 긴 문맥 병렬 학습에는 **트랜스포머**, 짧은 순차 상태에는 **RNN**을 선택한다.

#### 한줄 요약
- **문맥 성능•O(N²) 비용**을 함께 평가해 구조를 선택한다.
