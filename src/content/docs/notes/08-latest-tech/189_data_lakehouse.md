...
- 정의/개념: 개방형 파일 저장소에 **ACID 테이블 관리**를 결합한 **데이터 레이크하우스**임.
- 배경/필요성: 레이크•웨어하우스 분리는 데이터 복제•버전 불일치와 **파이프라인 중복**을 유발함.
#### 한줄 요약
...
- 개방형 저장소•컴퓨트 분리 기반 **다중 엔진 선택권**이 핵심임.
- **오픈 테이블 포맷** 기반 스키마 진화와 **ACID•스냅샷 관리**가 핵심임.
- **카탈로그** 기반 BI•스트리밍•ML 통합과 **파일 최적화 부담**이 핵심임.
#### 한줄 요약
...
- **구조화 질의 언어(Structured Query Language, SQL)**: 관계형 데이터의 정의•조회•변경에 사용하는 질의 언어을 의미한다.
...
1. **신규 데이터 파일 전달**: 변경 결과를 기존 파일 덮어쓰기 없이 새 파일로 생성함.
2. **후보 스냅샷 전달**: 스키마•통계•유효 파일 목록을 새 메타데이터로 구성함.
3. **원자 커밋 요청 전달**: 기준 버전 충돌 검증 후 현재 메타데이터 위치를 원자를 갱신함.
4. **확정 스냅샷 전달**: 독자에게 완전히 확정된 이전•신규 상태 중 하나를 공개함.
5. **유효 파일 조회 전달**: 읽기 엔진이 선택한 스냅샷의 파일만 일관되게 처리함.
...
- **BI•ML 통합** 워크로드는 레이크하우스, 고정 정형 분석은 웨어하우스를 선택하고 파일 유지관리 병행이 핵심임.

#### 한줄 요약

- 데이터 레이크하우스는 테이블 포맷과 카탈로그•파일 유지관리를 함께 운영해야  체계 적용
