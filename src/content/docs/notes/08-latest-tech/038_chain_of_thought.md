...
- 정의/개념: 복합 문제를 중간 추론 단계로 분해하고 단계 결과를 연결해 최종 답을 도출하는 **CoT**이다.
- 배경/필요성: 직접 응답만으로는 **조건 누락•중간 오류 위치**를 식별하기 어렵다.
...
- 부분 문제를 잇는 **순차 추론**으로 복합 조건을 분해한다.
- **자기 일관성**: 여러 추론 경로의 합의 답을 선택한다.
- **오류 보정**: 검증기로 잘못된 부분 결과를 차단한다.
- 생성 설명과 실제 판단 근거가 다를 수 있는 **비충실성**에 유의한다.
...
1. **문제•조건 정리**: 목표•제약 명시로 **조건 누락**을 방지한다.
2. **중간 추론 생성**: 부분 결과를 잇는 **단계 경로**를 구성한다.
3. **최종 후보 추출**: 추론 설명과 **답 후보**를 분리한다.
4. **규칙•도구 검증**: 계산•검색 기반 **독립 교차를 확인**한다.
...
- **외부 검증**: 생성 설명의 비충실성을 통제하도록 규칙•계산•검색으로 답 후보를 별도 확인하는 절차이다.

</details>

- **과업 난도•검증 가능성**으로 CoT를 선택하고 **외부 검증**을 적용한다.
