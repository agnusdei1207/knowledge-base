...
- 배경/필요성: 직접 응답만으로는 **조건 누락•중간 오류 위치 식별하기 어렵다**.
...
- 부분 문제를 잇는 **순차 추론**으로 복합 조건 분해가 핵심이다.
- **자기 일관성**: 여러 추론 경로의 합의 답을 선택한다.
- **오류 보정**: 검증기로 잘못된 부분 결과 차단한다.
- 생성 설명과 실제 판단 근거가 다를 수 있는 **비충실성**이 핵심이다.

#### 한줄 요약

- 생성 설명의 **비충실성•오류 전파**를 별도를 검증한다.
...
- 단순 과업은 **직접 응답**, 다단계 과업은 **CoT**가 핵심이다.
