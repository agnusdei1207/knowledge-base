...
- 정의/개념: 복합 문제를 중간 추론 단계로 분해하고 단계 결과를 연결해 최종 답을 도출하는 **사고 사슬(Chain-of-Thought, CoT)**
...
- **사고 사슬**: 다단계 조건을 중간 결과로 나누어 순서대로 연결한 뒤 답을 추출하는 방식이다.
...
- **사고 사슬 적용 기준(Chain-of-Thought Application Criteria, CoT Application Criteria)**: 여러 조건을 순서대로 연결해야 하고 최종 답을 외부 수단으로 검증할 수 있는 복합 과업에 사용한다.
- **외부 검증**: 생성 설명의 비충실성을 통제하도록 규칙·계산·검색으로 답 후보를 별도 확인하는 절차다.

</details>

- **과업 난도·검증 가능성**으로 사고 사슬(Chain-of-Thought, CoT) 적용 여부를 정하고 **외부 검증**으로 생성 설명의 비충실성 통제
