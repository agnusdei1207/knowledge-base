...
- 정의/개념: 모델 파라미터•연산•상태를 여러 장치에 분할하여 공동 실행하는 **모델 병렬화 방식**이다.
- 배경/필요성: 대형 모델의 파라미터•활성값•학습 상태가 **단일 장치 메모리** 초과가 발생한다.
...
- **분할 축**: 계층•텐서 기준 모델 저장•연산 분산이다.
- **통신 축**: 경계별 활성값•부분 결과 교환한다.
- **균형 축**: 단계 불균형은 파이프라인 버블, 세분된 텐서 분할은 통신 대기 증가한다.
...
- **병렬화 계획기**: 모델의 층•텐서•메모리와 장치 토폴로지를 분석해 분할 축•단계•통신 그룹을 결정하는 구성요소이다.
...
1. **파이프라인•텐서 축 선택**: 구조•통신 특성별 분할 조합을 결정한다.
2. **그룹•통신 경계 배치**: 고대역폭 링크에 단계•텐서 그룹을 매핑한다.
3. **분산 순전파•역전파**: 조각 계산•통신으로 출력•기울기를 완성한다.
4. **유휴•통신•메모리 지표 제공**: 분할 수•경계 재조정을 근거한다.
...
- **분할 축•고대역폭 그룹**에 따라 파이프라인•텐서 병렬을 선택한다.
