...
- **장문맥 거대 언어 모델(Long Context LLM)**: 확장된 컨텍스트 윈도우에서 멀리 떨어진 토큰의 관계와 근거를 함께 처리하는 언어 모델이다.
- **장거리 의존성**: 시퀀스에서 멀리 떨어진 정보들이 의미나 판단에 서로 영향을 주는 관계다.

</details>

- 정의/개념: **장문맥 LLM**은 확장된 컨텍스트 윈도우에서 많은 토큰과 장거리 의존성을 한 번의 추론으로 처리하도록 설계한 언어 모델
...
- **위치 표현 확장**: 학습한 길이 밖에서도 토큰의 순서와 거리를 해석하도록 RoPE나 보간 규칙을 조정하는 기법이다.
...
- **프리필·KV 캐시**: 장문 입력의 어텐션 상태를 먼저 계산·저장하여 후속 토큰 생성에서 재사용하는 처리와 메모리다.
...
2. **위치 표현 확장**: RoPE·위치 보간으로 학습 길이 밖 관계 부여
3. **장거리 관계 계산**: 전체 또는 희소·윈도우 범위의 관련 토큰 결합
4. **프리필·KV 캐시 생성**: 입력 병렬 처리와 생성 단계 상태 재사용
...
- **NIAH(Needle in a Haystack)**: 긴 문맥의 여러 위치에 숨긴 특정 근거를 얼마나 정확히 회수하는지 확인하는 평가다.
...
| **프리필 지연·KV 캐시 압박** | **접두사 캐싱·동적 배치**와 캐시 한도 | 프리필 지연 감소·**동시 처리량 향상** |
...
- **RAG 선택 기준**: 대규모·갱신 지식 가운데 질문에 필요한 일부 최신 근거만 선택할 때 사용한다.
