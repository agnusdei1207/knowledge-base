...
- 정의/개념: 확장된 컨텍스트 윈도우에서 많은 토큰과 장거리 의존성을 한 번에 처리하는 **장문맥 대규모 언어 모델(Long Context Large Language Model, Long Context LLM)**
...
- **위치 표현 확장**: 학습한 길이 밖에서도 토큰의 순서와 거리를 해석하도록 회전 위치 임베딩(Rotary Position Embedding, RoPE)이나 보간 규칙을 조정하는 기법이다.
- **희소·윈도우 어텐션(Sparse·Window Attention)**: 모든 토큰 쌍 대신 선택된 토큰이나 일정 범위의 관계만 계산하여 비용을 줄이는 방식이다.
...
- **회전 위치 임베딩(Rotary Position Embedding, RoPE)**: 위치별 회전 변환을 Q·K에 적용하여 어텐션 내적에 상대적 거리를 반영하는 기법이다.
- **위치 보간(Position Interpolation)**: 긴 위치 인덱스를 학습된 범위 안으로 비례 압축하여 입력 길이를 확장하는 기법이다.
- **프리필·키-값 캐시(Key-Value Cache, KV Cache)**: 장문 입력의 어텐션 상태를 먼저 계산·저장하여 후속 토큰 생성에서 재사용하는 처리와 메모리다.
...
| **프리필·키-값 캐시(Key-Value Cache, KV Cache)** | 입력 병렬 처리와 생성 단계 상태 재사용 |
...
2. **위치 표현 확장**: **회전 위치 임베딩(Rotary Position Embedding, RoPE)·위치 보간**으로 학습 길이 밖 관계 부여
3. **장거리 관계 계산**: 전체 또는 희소·윈도우 범위의 관련 토큰 결합
4. **프리필·키-값 캐시(Key-Value Cache, KV Cache) 생성**: 입력 병렬 처리와 생성 단계 상태 재사용
...
- **장문맥 대규모 언어 모델(Long Context Large Language Model, Long Context LLM) 기반 직접 장문 입력** 및 **검색 증강 생성(Retrieval-Augmented Generation, RAG)** 사이를 전역 관계 분석과 선택 검색 기준으로 구분한다.
...
| **중간 정보 회수 저하** | 위치별 **건초더미 속 바늘 찾기(Needle in a Haystack, NIAH)·다중 근거 평가** | **유효 문맥 길이** 검증 |
| **프리필 지연·키-값 캐시(Key-Value Cache, KV Cache) 압박** | **접두사 캐싱·동적 배치**와 캐시 한도 | 프리필 지연 감소·**동시 처리량 향상** |
...
- **직접 장문 입력 선택 기준**: 문서 전체에 흩어진 근거의 전역 의존성을 분석해야 할 때 사용한다.
- **검색 증강 생성 선택 기준(Retrieval-Augmented Generation Selection Criteria, RAG Selection Criteria)**: 대규모·갱신 지식 가운데 질문에 필요한 일부 최신 근거만 선택할 때 사용한다.

</details>

- 전역 의존성에는 **직접 장문 입력**, 일부 최신 근거에는 **검색 증강 생성(Retrieval-Augmented Generation, RAG)** 선택
