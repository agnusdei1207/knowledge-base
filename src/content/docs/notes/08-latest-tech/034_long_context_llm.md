...
- 배경/필요성: 문서 분할은 여러 구간에 걸친 **전역 관계•중간 근거 연결**을 단절이 발생한다.
...
- 위치별 근거를 회수•결합하는 **유효 문맥 품질**을 확보한다.
...
2. **위치 표현 확장**: **RoPE•위치 보간**으로 학습 길이 밖 관계를 부여한다.
...
- 전역 의존성에는 **직접 장문 입력**, 일부 최신 근거에는 **RAG**을 선택한다.

#### 한줄 요약

- **전역 의존성 범위**에 따라 직접 입력•RAG을 선택한다.
