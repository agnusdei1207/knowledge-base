...
- 정의/개념: **실시간 스트리밍**은 연속 이벤트를 제한된 지연으로 지속 처리하며 시간·상태·복구를 관리하는 데이터 처리 방식
- 배경/필요성: 배치 처리는 데이터 수집 완료 전 **이상 거래·설비 변화**에 즉시 대응 곤란
...
- **트랜잭션 Sink**: 체크포인트 상태와 출력 커밋을 조정해 복구 후 중복·누락을 억제하는 저장소 연결 방식이다.
...
- 초 단위 제어는 **레코드 스트리밍**, 수초·분 지연 허용은 **마이크로배치** 선택
