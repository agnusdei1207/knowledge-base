...
- 정의/개념: 연속 이벤트를 제한된 지연으로 지속 처리하며 시간•상태•복구를 관리하는 **실시간 스트리밍**
- 배경/필요성: 배치 처리는 데이터 수집 완료 전 이상 거래•설비 변화에 대한 **즉시 대응** 곤란

#### 한줄 요약

- 연속 이벤트의 제한 지연 처리와 시간•상태•복구 관리
...
- **2. 윈도우•트리거**: 계산 범위와 중간•최종 결과 출력 시점 결정
- **3. 워터마크**: 지연 데이터의 대기 한계와 마감 시점 판정
#### 한줄 요약

- 이벤트 시간•워터마크 기반 지연 데이터 처리 기준 설정
...
- 이벤트 시간 윈도•허용 지연•거래 ID 기반 중복 경보 제거
