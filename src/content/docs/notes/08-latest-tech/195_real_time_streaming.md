...
- **연속 이벤트**: 완료된 유한 집합이 아니라 시간에 따라 끊임없이 도착하는 사건 기록하는 개념이다.

</details>

- 정의/개념: 연속 이벤트를 제한된 지연으로 지속 처리하며 시간•상태•복구를 관리하는 **실시간 스트리밍**이다.
- 배경/필요성: 배치 처리는 데이터 수집 완료 전 이상 거래•설비 변화에 대한 **즉시 대응**하기 어렵다.
...
- **1. 이벤트 시간**: 실제 사건 발생 시각을 기준으로 한 순서•집계한다.
- **2. 윈도우•트리거**: 계산 범위와 중간•최종 결과 출력 시점을 결정한다.
- **3. 워터마크**: 지연 데이터의 대기 한계와 마감 시점을 판정한다.
#### 한줄 요약
...
- **이벤트 로그**: 순서와 위치를 가진 이벤트를 내구성 있게 저장해 재처리 가능한 입력으로 제공하는 정보이다.
- **윈도우**: 무한 이벤트 흐름을 시간•개수•세션 범위로 나눠 집계하는 연산 단위이다.
...
1. **이벤트•발생 시각 전달**: 재생 가능한 로그의 키와 **이벤트 시간**을 제공한다.
2. **워터마크•윈도우 전달**: 시간 진행•허용 지연의 **계산 범위**를 제공한다.
3. **입력 위치•상태 전달**: 복구할 소비 위치와 **키별 상태**를 제공한다.
4. **결과•멱등 키 전달**: 확정•수정 결과와 **중복 제거 키**를 제공한다.
5. **체크포인트 상태 전달**: 재실행할 **입력 위치•연산 상태**를 제공한다.
...
- **유효시간(Time to Live, TTL)**: 상태를 보존한 뒤 자동 만료할 기간이다.
...
- 초 단위 제어는 **레코드 스트리밍**, 수초•분 허용은 **마이크로배치**를 선택한다.
