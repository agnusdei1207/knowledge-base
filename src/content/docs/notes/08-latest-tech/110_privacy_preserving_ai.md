...
- 정의/개념: **Privacy-Preserving AI**는 데이터•중간값•출력 노출을 위협 모델별 보호 기법으로 줄이는 기술군이다.
- 배경/필요성: 원본 수집•학습•추론은 단계별 **개인정보 노출•재식별 위험**을 유발한다.
...
- 원본•업데이트•추론 결과를 나눈 **단계별 노출 경계**가 핵심이다.
- 데이터 비이동과 업데이트 보호를 결합한 **다층 방어**가 핵심이다.
- 강한 보호 설정에 따른 **정확도 저하•연산 비용 증가**가 핵심이다.
...
- **보호 계층**: 노이즈나 암호를 적용해 입력•업데이트•출력의 관측 가능 범위를 제한하는 구성요소이다.
...
1. **보호 자산•공격자 능력 정의**: 원본•업데이트•출력과 **관측 가능 주체**를 설정한다.
2. **단계별 노출 경로 분석**: 학습•집계•추론에서 개인정보가 새는 **공격 표면**을 식별한다.
3. **분산•노이즈•암호 기술 배치**: 노출 경로별로 상호 보완적인 **보호 계층**을 적용한다.
4. **공격 성공률•효용 측정**: 재식별•추론 공격과 정확도•지연의 **실제 결과**를 비교한다.
5. **잔여 노출•비용 재설계**: 보호 목표를 넘는 위험이나 비용의 **기술 조합**을 조정한다.
...
- **동형암호**: 입력을 암호화한 상태로 연산하고 복호화해 결과를 얻는 기술이다.
- **보안 집계**: 참여자별 업데이트를 공개하지 않고 합계만 계산하는 기술이다.
...
- 관측 범위•보호 예산•효용 손실에 따라 **연합학습•차등 프라이버시**와 암호 계층 조합이 핵심이다.
