...
- 정의: 후학습과 테스트 시점 연산으로 문제 해결을 강화한 **Reasoning LLM**이다.
- 배경/필요성: 즉시 생성 모델은 **단계별 해결•도구 결과 검증** 대상 취약

#### 한줄 요약
- **후학습•가변 추론•도구 검증** 기반 복합 문제 해결
...
1. 추론 예산•도구 권한 전달: 품질 이득•비용 한도별 **사용 자원** 설정함.
2. 후보 풀이 생성: 문제를 분해해 단계별 **해결 후보** 활용 구성함.
3. 도구 실행•검증 요청: 계산•코드•검색 기반 **외부 검증** 요청함.
4. 실행 결과•검증 신호 전달: 오류와 통과 여부를 다음 추론에 반영함.
5. 최종 응답 선택: 검증을 통과한 후보를 정책에 따라 선택함.

#### 한줄 요약
- **예산 할당•후보 생성•도구 검증•선택** 수행
...
#### 한줄 요약
- 지연 민감은 **일반 LLM**, 복합 검증은 **Reasoning LLM** ## Ⅵ. 실무 고려사항 및 대책
