...
- 정의/개념: 후학습과 테스트 시점 연산으로 다단계 문제 해결•검증 능력을 강화한 **Reasoning LLM**이다.
- 배경/필요성: 즉시 생성 중심 모델은 복합 문제의 **단계별 해결•도구 결과 검증**에 취약하다.
...
- **SFT**: 정답•풀이 예시로 응답 행동을 조정한다.
- **RLVR**: 자동 판정 가능한 보상으로 해결 정책을 강화한다.
- **TTC**: 요청 난도에 따라 추론 예산을 조절한다.
- 정답•코드 실행•도구 결과를 활용한 **검증 가능 출력 선택**이 핵심이다.
...
1. **추론 예산•도구 권한 전달**: 품질 이득•비용 한도별 **사용 자원**을 설정한다.
2. **후보 풀이 생성**: 문제를 분해해 단계별 **해결 후보**를 구성한다.
3. **도구 실행•검증 요청**: 계산•코드•검색 기반 **외부 검증**을 요청한다.
4. **실행 결과•검증 신호 전달**: 오류와 통과 여부를 다음 추론에 반영한다.
5. **최종 응답 선택**: 검증을 통과한 후보를 정책에 따라 선택한다.
...
- **검증 이득**: 검증 가능한 과업에서 추론 확대로 얻는 품질 향상과 비용 절감의 차이이다.
- **지연 비용**: 추론 자원 추가로 발생하는 응답 지연과 운영 부담이다.

</details>

- 단순•지연 민감 요청에는 **일반 대화 LLM**, 복합•검증 가능 과업에는 **Reasoning LLM**을 선택한다.
