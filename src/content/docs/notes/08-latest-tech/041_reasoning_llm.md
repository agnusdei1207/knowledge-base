...
- 배경/필요성: 즉시 생성 중심 모델은 복합 문제의 **단계별 해결•도구 결과 검증**에 취약해진다.

#### 한줄 요약
- **후학습•가변 추론•도구 검증**으로 복합 문제 해결이 핵심이다.
...
- **RLVR**: 자동 판정 가능한 보상으로 해결 정책 강화이다.
- **TTC**: 요청 난도에 따라 추론 예산 조절이다.
- 정답•코드 실행•도구 결과를 활용한 **검증 가능 출력 선택**이 핵심이다.

#### 한줄 요약
- **난도•검증 이득**이 큰 과업에 추론 자원 집중이 핵심이다.
...
#### 한줄 요약
- 지연 민감은 **일반**, 복합 검증은 **Reasoning LLM**가 핵심이다.
