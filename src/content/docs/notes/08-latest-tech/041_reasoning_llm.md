...
2. **후보 풀이 생성**: 문제를 분해해 단계별 **해결 후보** 구성
3. **도구 실행•검증 요청**: 계산•코드•검색 기반 **외부 검증** 요청
4. **실행 결과•검증 신호 전달**: 오류와 통과 여부를 다음 추론에 반영
5. **최종 응답 선택**: 검증을 통과한 후보를 정책에 따라 선택
...
#### 한줄 요약
- 지연 민감은 **일반**, 복합 검증은 **Reasoning LLM**
