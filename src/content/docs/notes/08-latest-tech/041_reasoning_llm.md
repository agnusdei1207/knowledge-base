...
sequenceDiagram
    ...
    U->>C: 1. 과업 난도·검증 요구 분류
    C->>M: 2. 추론 예산·도구 권한 배정
    M->>V: 3. 후보 풀이·도구 호출
    V->>M: 4. 실행 결과·검증 신호 반환
    M-->>U: 5. 검증 반영 최종 응답
```

1. **과업 난도·검증 요구 분류**: 단순 생성과 복합 추론을 구분해 **처리 경로** 결정
2. **추론 예산·도구 권한 배정**: 품질 이득과 비용 한도에 따라 **사용 가능 자원** 설정
3. **후보 풀이·도구 호출**: 단계별 해결안을 만들고 필요한 **외부 실행 결과** 요청
4. **실행 결과·검증 신호 반환**: 정답·코드·규칙 판정으로 후보의 **타당성 근거** 제공
5. **검증 반영 최종 응답**: 검증 결과와 안전 정책을 만족하는 **출력 선택**
...
- 과업 난도·검증 가능성·지연·비용을 기준으로 **일반 모델과 추론 특화 LLM**의 처리 경로를 선택해야 한다.
