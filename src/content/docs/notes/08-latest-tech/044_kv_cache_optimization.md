...
- 정의/개념: 키•값 재사용으로 디코드 연산을 줄이고 저장•회수로 메모리를 관리하는 **KV Cache 최적화**이다.
- 배경/필요성: 자기회귀 생성은 과거 키•값의 반복 계산 또는 장문•동시 요청의 **캐시 메모리 증가**를 유발한다.
...
- 과거 키•값 재사용에 의한 **디코드 재계산 제거**가 핵심이다.
- 층•KV Head•토큰•배치•정밀도에 비례하는 **캐시 용량**이 핵심이다.
- **GQA**: Q 그룹별 K•V 공유로 캐시 절감이다.
- **MQA**: 단일 K•V 공유로 캐시 최소화이다.
- **양자화**: 저정밀 표현으로 캐시 용량을 축소한다.
- **페이징**: 블록 할당으로 단편화를 완화한다.

#### 한줄 요약
- **문맥•배치•KV Head•정밀도**에 비례한 캐시 증가한다.
...
1. **요청별 캐시 용량 산정**: 문맥 길이•층•**KV Head•정밀도 기반 예상 메모리를 계산**한다.
2. **캐시 블록 동적 할당**: 가용 메모리에서 요청에 필요한 **저장 영역**을 배정한다.
3. **프리필 키•값 저장**: 입력 토큰별 키•값을 계산해 **초기 캐시**를 구성한다.
4. **과거 키•값 조회•신규 값 추가**: 디코드마다 기존 값을 재사용하고 **새 토큰 캐시**를 확장한다.
5. **캐시 블록 회수**: 참조가 끝난 블록을 반납해 **가용 메모리**를 복원한다.
...
- **MHA Cache**, **GQA Cache**, **MQA Cache**, **양자화 KV Cache** 사이를 표현력과 메모리 절감 기준으로 구분한다.
...
#### 한줄 요약
- **헤드 공유•양자화 수준**으로 메모리•품질 절충이 핵심이다.
...
- 헤드 중복에는 **GQA•MQA**, 단편화에는 **페이징**, 용량 부족에는 **양자화•오프로딩**을 적용한다.
