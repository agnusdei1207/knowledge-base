...
- 정의/개념: 키·값 재사용으로 디코드 연산을 줄이고 저장·회수로 메모리를 관리하는 **키-값 캐시(Key-Value Cache, KV Cache) 최적화**
...
- 층·키-값 헤드(Key-Value Head, KV Head)·토큰·배치·정밀도에 비례하는 **캐시 용량**
- **그룹 쿼리 어텐션(Grouped-Query Attention, GQA)·멀티 쿼리 어텐션(Multi-Query Attention, MQA)·양자화·페이징** 기반 캐시 용량 감소와 정확도 검증
...
- **오프로딩(Offloading)**: 캐시 일부를 그래픽 처리장치(Graphics Processing Unit, GPU) 메모리에서 중앙처리장치(Central Processing Unit, CPU) 메모리 같은 하위 계층으로 이동하는 방법이다.
...
| 구성요소 | 책임 |
|:---|:---|
| 캐시 레이아웃 | 층·요청·**키-값 헤드(Key-Value Head, KV Head)·토큰별 저장 형상 정의** |
...
1. **요청별 캐시 용량 산정**: 문맥 길이·층·**키-값 헤드(Key-Value Head, KV Head)·정밀도 기반 예상 메모리 계산**
...
- **그룹·멀티 쿼리 어텐션 캐시(Grouped-Query and Multi-Query Attention Cache, GQA·MQA Cache)**: 쿼리 그룹 또는 전체가 K·V 헤드를 공유하여 캐시와 대역폭을 줄인다.
- **양자화 키-값 캐시(Quantized Key-Value Cache, Quantized KV Cache)**: K·V를 낮은 비트로 저장하여 장문·고동시성의 메모리 사용을 추가로 줄인다.

</details>

- **멀티 헤드 어텐션 캐시(Multi-Head Attention Cache, MHA Cache)**, **그룹·멀티 쿼리 어텐션 캐시(Grouped-Query and Multi-Query Attention Cache, GQA·MQA Cache)**, **양자화 키-값 캐시(Quantized Key-Value Cache, Quantized KV Cache)** 사이를 표현력과 메모리 절감 기준으로 구분한다.
...
| 그래픽 처리장치(Graphics Processing Unit, GPU) 캐시 부족의 **오프로딩 지연** | 계층별 대역폭 측정·재계산과 비용 비교 | 지연 급증 **완화** |
...
- **그룹·멀티 쿼리 어텐션(Grouped-Query and Multi-Query Attention, GQA·MQA)**: K·V 헤드의 중복을 공유로 줄여 캐시 용량과 대역폭을 절감한다.
- **페이징**: 캐시 블록을 동적 배정·회수하여 단편화를 완화한다.
- **양자화·오프로딩**: 그래픽 처리장치(Graphics Processing Unit, GPU) 용량이 부족할 때 정밀도를 낮추거나 하위 메모리로 이동해 수용량을 늘린다.

</details>

- 헤드 중복에는 **그룹·멀티 쿼리 어텐션(Grouped-Query and Multi-Query Attention, GQA·MQA)**, 단편화에는 **페이징**, 용량 부족에는 **양자화·오프로딩** 적용
