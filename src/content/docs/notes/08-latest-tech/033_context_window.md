...
> 개념도: 기본 전체 어텐션의 관계 계산량은 토큰 길이의 제곱에 비례하고, KV 캐시 저장량은 토큰 길이에 비례한다. 두 계열은 절대 시간·메모리의 비교가 아니라 증가율 비교다.

- 시스템 지시·대화·검색 문서·출력이 공유하는 **총 토큰 한도**
- 입력 길이에 따라 증가하는 **어텐션 연산·첫 토큰 지연**
...
| 구성요소 | 책임 |
|:---|:---|
| 길이 한도 | 모델이 허용하는 **입력·출력 토큰 총량 정의** |
| 토큰 예산 관리자 | 출력 여유를 예약하고 입력별 **토큰 수 배분** |
| 문맥 선택기 | 지시·근거의 중요도에 따라 **포함·압축·제외 결정** |
| 프리필 엔진 | 입력 토큰의 어텐션 계산과 **키·밸류 벡터 생성** |
| KV 캐시 | 이전 토큰의 키·밸류를 보관하여 **생성 단계 재사용** |
...
sequenceDiagram
    ...
    U->>B: 1. 질의·근거 토큰량 산정
    B->>S: 2. 출력 예약·입력 예산 할당
    S->>P: 3. 핵심 문맥 선택·배치
    P->>M: 4. 어텐션 계산·KV 캐시 생성
    M-->>U: 5. 잔여 예산 내 응답 생성
```
...
|:---|:---|:---|:---|
| 적용 기준 | 문서 전체의 상호 참조 필요 | 대규모 지식에서 일부 근거 탐색 | 전역 문맥과 최신 근거 동시 요구 |
| 핵심 특징 | 원문을 넓은 **윈도우에 직접 배치** | 질의 관련 조각을 **검색 후 주입** | 핵심 원문과 검색 근거의 **선별 결합** |
| 한계 | 프리필·KV 캐시 비용과 위치별 인출 저하 | 검색 누락·문서 조각 단절 가능 | 예산 배분·중복 제거 복잡성 |
...
- 전체 문서의 상호 참조 필요성과 검색 가능성, 프리필·KV 캐시 예산을 기준으로 **롱 컨텍스트·RAG·혼합 방식**을 선택해야 한다.
