...
#### 한줄 요약
- **입력•출력 토큰**이 하나의 길이 한도를 공유가 핵심이다.
...
- 시스템 지시•대화•검색 문서•출력이 공유하는 **총 토큰 한도**가 핵심이다.
...
1. **질의•근거 토큰량 산정**: 모든 입력을 공통 토큰 단위로 환산해 **총 사용량**을 계산한다.
2. **출력 예약•입력 예산 할당**: 최대 길이에서 예상 출력 공간을 먼저 차감해 **입력 상한**을 결정한다.
3. **핵심 문맥 선택•배치**: 지시 우선순위와 근거 관련도에 따라 문맥을 **압축•정렬**한다.
4. **어텐션 계산•KV Cache 생성**: 프리필 결과를 저장하여 후속 생성에서 **키•밸류를 재사용**한다.
5. **잔여 예산 내 응답 생성**: 종료 신호 또는 출력 한도까지 **결과 토큰을 생성**한다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 장문 입력의 **어텐션 계산량 증가** | **문맥 선별•조각 압축•효율형 어텐션** | 프리필 시간•연산량 **절감** |
| 생성 길이에 따른 **KV Cache 증가** | **GQA•저정밀 캐시**와 출력 상한 적용 | 동시 처리량과 메모리 효율 **향상** |
| 중간 위치 정보의 **인출 정확도 저하** | **중요 지시 양끝 배치•위치별 인출 시험** | 장문 근거 누락 **탐지•완화** |
| 입력 과다에 따른 **출력 예산 부족** | **출력 길이 선예약•입력 초과분 제거** | 응답 중도 절단 **방지** |
...
- 전체 상호 참조에는 **롱 컨텍스트**, 일부 근거에는 **RAG**, 전역•최신 결합에는 **혼합 방식**을 선택한다.

#### 한줄 요약
- **상호 참조 범위•비용**에 따라 롱 컨텍스트•RAG을 선택한다.
