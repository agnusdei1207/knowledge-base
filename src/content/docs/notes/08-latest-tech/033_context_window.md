...
- **키-값 캐시(Key-Value Cache, KV Cache)**: 이전 토큰의 K·V 연산 결과를 저장하여 후속 생성에서 재사용하는 메모리다.
...
- **RAG 선택 기준**: 대규모·갱신 지식에서 질의 관련 일부 근거만 찾아야 할 때 검색 후 주입한다.
- **혼합 방식 선택 기준**: 전체 구조와 최신 근거를 함께 요구할 때 핵심 원문과 검색 결과를 선별 결합한다.

</details>

- 전체 상호참조는 **롱 컨텍스트**, 일부 근거는 **RAG**, 전역·최신 결합은 혼합 방식
