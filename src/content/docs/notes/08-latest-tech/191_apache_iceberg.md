...
- 정의/개념: 스냅샷•매니페스트 계층으로 유효 파일과 스키마를 관리하는 **Apache Iceberg**
- 배경/필요성: 파일 경로 기반 테이블은 탐색 비용 증가와 **동시 커밋•스키마 진화** 제약

#### 한줄 요약

- 스냅샷 색인 기반 유효 파일 탐색과 스키마 관리
...
1. **기준 스냅샷 조회**: 현재 스키마•파티션•파일의 **테이블 상태** 확인
2. **데이터•삭제 파일 생성**: 변경 행과 **삭제 정보•열 통계** 기록
3. **매니페스트•메타데이터 생성**: 신규 파일과 **스냅샷 연결 정보** 기록
4. **포인터 비교•원자 커밋**: 기준 포인터가 같을 때 새 스냅샷 게시

#### 한줄 요약

- 신규 메타데이터 생성 후 원자 포인터 교체 기반 상태 공개
...
- Iceberg는 매니페스트, Delta는 로그, Hudi는 타임라인 중심
