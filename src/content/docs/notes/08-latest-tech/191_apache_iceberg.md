...
- 정의/개념: 스냅샷•매니페스트 계층으로 유효 파일과 스키마를 관리하는 **Apache Iceberg**이다.
- 배경/필요성: 파일 경로 기반 테이블은 탐색 비용 증가와 **동시 커밋•스키마 진화** 제약이 발생한다.
...
- **필드 식별자(Field Identifier, Field ID)**: 이름 변경과 무관하게 스키마 열을 추적하는 고유 번호이다.

</details>

- 스냅샷•매니페스트 계층 기반 **데이터•삭제 파일 추적**이 핵심이다.
- Field ID와 **스키마 진화•숨은 파티셔닝**이 핵심이다.
- 매니페스트 통계 기반 **파일 프루닝•메타데이터 관리**가 핵심이다.
#### 한줄 요약
...
1. **기준 스냅샷 조회**: 현재 스키마•파티션•파일의 **테이블 상태**를 확인한다.
2. **데이터•삭제 파일 생성**: 변경 행과 **삭제 정보•열 통계**를 기록한다.
3. **매니페스트•메타데이터 생성**: 신규 파일과 **스냅샷 연결 정보**를 기록한다.
4. **포인터 비교•원자 커밋**: 기준 포인터가 같을 때 새 스냅샷을 게시한다.
...
- 다중 엔진•파티션 진화는 **Iceberg**, 로그 중심 스트리밍은 **Delta Lake**을 선택한다.
