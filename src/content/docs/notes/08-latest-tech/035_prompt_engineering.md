...
- **시스템 지시•역할•목표•제약** 분리를 통한 모델 수행 범위와 우선 규칙 명시
...
- **출력 계약(Output Contract)**: 응답의 구조•필드•자료형•근거•오류 처리 형식을 명시한 요소이다.
...
- **프롬프트 주입(Prompt Injection)**: 비신뢰 입력의 악성 지시가 상위 지시를 무시하거나 우회하도록 모델 행동을 유도하는 공격이다.
