...
- 정의/개념: **프롬프트 엔지니어링**은 지시·문맥·예시·출력 계약을 설계하고 평가하여 원하는 모델 행동을 유도하는 입력 최적화 기법
...
- **제로샷·퓨샷(Zero-shot·Few-shot)**: 예시 없이 지시만 제공하거나 소수의 입출력 예시로 기대 응답 패턴을 제공하는 방식이다.
...
- **프롬프트 엔지니어링**: 모델 가중치를 바꾸지 않고 입력 시점의 지시와 문맥으로 행동을 빠르게 조정한다.
...
- **프롬프트 인젝션(Prompt Injection)**: 비신뢰 입력의 악성 지시가 상위 지시를 무시하거나 우회하도록 모델 행동을 유도하는 공격이다.
...
- **프롬프트 선택 기준**: 과업 지시·형식이 자주 바뀌고 빠른 실험과 배포가 필요할 때 사용한다.
- **미세조정 선택 기준**: 안정적으로 반복되는 도메인 행동을 모델 가중치에 내재화할 가치가 있을 때 사용한다.
