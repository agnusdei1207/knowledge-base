...
- 배경/필요성: 모호한 입력은 동일 과업에서도 **정확성•형식•안전성 편차**를 유발한다.
...
- **역할**: 모델이 맡을 전문 책임을 정의한다.
- **목표**: 모델이 달성할 과업과 성공 조건 명시이다.
- **제약**: 금지 사항과 권한 범위를 제한한다.
- **제로샷**: 예시 없이 지시만으로 수행한다.
- **퓨샷**: 소수 예시로 기대 패턴을 제공한다.
- **도구 호출**: 외부 기능 요청을 구조화이다.
- 고정 평가 세트를 통한 **품질 회귀•프롬프트 주입**을 검증한다.

#### 한줄 요약

- **고정 평가 세트**로 품질•안전 회귀를 반복을 검증한다.
...
2. **회귀 평가 세트 구성**: 정상•경계•공격 입력과 성공 기준을 고정한다.
...
- 빠른 지시 변경은 **프롬프트**, 행동 내재화는 **미세조정**이 핵심이다.
...
- 변경 잦은 과업은 **프롬프트**, 반복 행동 내재화는 **미세조정**을 선택한다.

#### 한줄 요약

- **변경 빈도•행동 반복성**에 따라 프롬프트•미세조정을 선택한다.
