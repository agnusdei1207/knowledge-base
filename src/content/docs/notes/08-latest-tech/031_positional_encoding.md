...
- 배경/필요성: 내용 벡터만 비교하는 셀프 어텐션은 **동일 토큰 집합의 배열 순서•거리** 구분하기 어렵다.

#### 한줄 요약
- **순서•상대 거리**를 토큰 표현에 결합이 핵심이다.
...
- 입력 임베딩 또는 **Q•K 회전 기반 순서•거리 정보 주입**이 핵심이다.
- 절대 위치•상대 위치에 따른 **참조 관계 표현 방식** 차이 핵심이다.
- 학습 문맥 길이 밖에서 발생하는 **위치 외삽 오차**가 핵심이다.

#### 한줄 요약
- **절대•상대 위치 표현**으로 참조 관계를 구분한다.
...
1. **토큰 의미 임베딩 생성**: 위치를 제외한 토큰의 **내용 표현**을 준비한다.
2. **위치 인덱스•거리 산출**: 토큰별 절대 순서나 토큰 쌍의 **상대 거리**를 계산한다.
3. **위치 벡터•회전값 생성**: 고정 함수•학습값•**RoPE** 기반 위치 신호를 변환한다.
4. **임베딩 또는 Q•K에 주입**: 위치 신호를 내용 표현이나 **어텐션 유사도**에 결합한다.
...
#### 한줄 요약
- 고정 순서는 **절대**, 장문 거리 관계는 **상대 위치**가 핵심이다.
...
| 위치 확장에 따른 **단거리 해상도 저하** | **단•장문 평가셋•주파수별 보정 비교** | 단거리 해상도 보존과 **장거리 관계 확장** |
| 학습•추론의 **위치 인덱스 불일치** | **패딩•캐시•청크 경계 단위 시험** | 토큰 순서 왜곡과 **캐시 오류 방지** |

#### 한줄 요약
- **길이별 평가•인덱스 시험**으로 위치 왜곡을 방지한다.
...
- 고정 길이에는 **절대 위치**, 장문 외삽에는 **RoPE•위치 보간**을 선택한다.

#### 한줄 요약
- **문맥 길이•외삽 요구**에 따라 절대•RoPE을 선택한다.
