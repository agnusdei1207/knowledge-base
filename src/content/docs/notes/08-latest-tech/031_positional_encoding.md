...
- 입력 임베딩 또는 **쿼리(Query, Q)·키(Key, K) 회전 기반 순서·거리 정보 주입**
...
| **위치 표현기** | 삼각함수·학습 벡터·**회전 위치 임베딩(Rotary Position Embedding, RoPE) 회전 변환** |
| **주입 계층** | 임베딩 가산 또는 **쿼리(Query, Q)·키(Key, K) 변환** |
...
- **쿼리·키(Query·Key, Q·K)**: 어텐션에서 어떤 정보를 찾는지와 비교 대상이 무엇인지 나타내는 벡터다.
...
3. **위치 벡터·회전값 생성**: 고정 함수·학습값·**회전 위치 임베딩(Rotary Position Embedding, RoPE)** 기반 위치 신호 변환
4. **임베딩 또는 쿼리(Query, Q)·키(Key, K)에 주입**: 위치 신호를 내용 표현이나 **어텐션 유사도**에 결합
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 학습 길이 초과 시 **위치 외삽 오차** | 위치 보간·**회전 위치 임베딩(Rotary Position Embedding, RoPE) 스케일 조정** 후 길이별 평가 | 장문 정확도 **급락 완화** |
...
- **절대 위치 선택 기준**: 입력 길이가 고정되고 토큰의 명시적 순번 표현이 중요할 때 사용한다.
- **회전 위치 임베딩·보간 선택 기준(Rotary Position Embedding and Interpolation Selection Criteria, RoPE and Interpolation Selection Criteria)**: 상대 거리를 유지하면서 학습 범위보다 긴 문맥으로 확장해야 할 때 사용한다.

</details>

- 고정 길이에는 **절대 위치**, 장문 외삽에는 **회전 위치 임베딩(Rotary Position Embedding, RoPE)·위치 보간** 선택
