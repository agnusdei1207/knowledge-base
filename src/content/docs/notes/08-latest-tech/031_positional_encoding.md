...
- 정의/개념: **위치 인코딩**은 순열 불변인 어텐션에 토큰의 절대 순서나 상대 거리를 주입하는 표현 기법이다.
...
| 구성요소 | 책임 |
|:---|:---|
| 위치 인덱스 | 토큰의 절대 순서·상대 거리 또는 **패치 좌표** 생성 |
| 위치 표현기 | 삼각함수·학습 벡터·회전 변환으로 **위치 정보** 표현 |
| 주입 계층 | 입력 임베딩 가산 또는 Q·K 변환으로 **어텐션 점수**에 반영 |
| 길이 확장기 | 보간·스케일 조정으로 학습 범위 밖 **장문 위치** 보정 |
...
5. **순서·거리 반영 문맥 출력**: 같은 토큰도 배열 위치에 따라 다른 **문맥 관계** 생성
...
| 위치 확장에 따른 **단거리 해상도 저하** | 짧은·긴 문맥 평가셋과 주파수별 보정 비교 | 국소·장거리 관계 **균형 확보** |
