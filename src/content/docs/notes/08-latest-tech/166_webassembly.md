...
| 고려사항 | 위험 | 대책 |
|:---|:---|:---|
| **과도한 가져오기 권한** | 파일·네트워크 접근 범위가 넓어져 샌드박스 경계 약화 | 기능 허용 목록과 디렉터리·네트워크 범위를 최소 권한으로 제한 |
| **WASI·WIT 버전 불일치** | 런타임과 모듈의 인터페이스 계약이 달라 실행 실패 | 계약 버전 고정, 호환성 행렬 관리, 배포 전 교차 런타임 시험 |
| **메모리 고갈·트랩** | 비정상 모듈이 자원을 소진하거나 요청 처리 중단 | 메모리·실행 시간·연산량 한도 설정과 트랩 복구 경로 마련 |
