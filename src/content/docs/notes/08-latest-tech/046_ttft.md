...
- 정의/개념: 요청 전송부터 첫 출력 토큰 수신까지의 초기 응답 지연인 **TTFT**이다.
- 배경/필요성: 전체 응답 시간만으로는 **대기열•프리필•첫 전송 병목**을 분리하기 어렵다.

#### 한줄 요약
- 요청 전송부터 **첫 출력 토큰 수신**까지의 지연이 핵심이다.
...
- `TTFT = 첫 토큰 수신 시각 - 요청 전송 시각`의 **TTFT 종단 간 측정**이 핵심이다.
- 네트워크•대기열•프리필•첫 전송을 합산하는 **구간 지연 지표**가 핵심이다.
- 평균과 p95•p99를 병행하는 **초기 반응성 SLO**가 핵심이다.

#### 한줄 요약
- **입구•대기열•프리필•첫 전송**의 합산 지연이 핵심이다.
...
1. **입구 처리•대기열 전달**: 인증•라우팅 후 **대기 상태**를 전환한다.
2. **배치•GPU 할당 전달**: 실행 배치와 **연산 자원**을 배정한다.
3. **첫 토큰 전달**: 프리필 결과의 **첫 출력 토큰**을 제공한다.
...
#### 한줄 요약
- 시작은 **TTFT**, 지속은 **TPOT**, 완료는 종단 지연이 핵심이다.
...
- p95 초과가 대기열이면 **스케줄링**, 프리필이면 **입력 축약•프롬프트 캐시**를 적용한다.
