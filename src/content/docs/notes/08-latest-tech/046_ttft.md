...
- **p95 TTFT**: 전체 요청의 95%가 이 값 이하에서 첫 토큰을 받는 꼬리 지연 경계다.
- **서비스 수준 목표(SLO)**: 서비스가 달성해야 하는 지연·가용성 같은 내부 운영 목표다.
...
- **TTFT**: 요청부터 첫 토큰 수신까지의 초기 지연을 측정한다.
- **토큰당 출력 지연(TPOT)**: 첫 토큰 이후 출력 토큰 하나를 생성하는 평균 시간을 측정한다.
