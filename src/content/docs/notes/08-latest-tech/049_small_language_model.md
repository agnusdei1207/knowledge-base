...
- 배경/필요성: 범용 LLM의 메모리•전력•지연 비용으로 **단말•현장 업무 배포**에 제약이 발생한다.
...
- **지식 증류**: 교사 모델의 지식을 작은 모델에 이전이다.
- **SFT**: 업무 예시로 응답 행동을 조정한다.
- **PEFT**: 일부 매개변수 학습으로 업무 적응이다.
- **양자화•장치별 런타임**을 결합한 제한 자원 추론이 핵심이다.

#### 한줄 요약
- **파라미터 수•정밀도•업무 품질**의 절충이 핵심이다.
...
#### 한줄 요약
- 좁은 반복 업무는 **SLM**, 범용 복합 과업은 **LLM**가 핵심이다.
