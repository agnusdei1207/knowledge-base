...
- 정의/개념: 제한된 연산•메모리에서 특정 범위의 언어 과업을 수행하도록 경량화한 **SLM**이다.
- 배경/필요성: 범용 LLM의 메모리•전력•지연 비용으로 **단말•현장 업무 배포**에 제약이 따른다.
...
- 작은 파라미터 수에 따른 **메모리•연산•배포 비용 절감**이 핵심이다.
- **지식 증류**: 교사 모델의 지식을 작은 모델에 이전한다.
- **SFT**: 업무 예시로 응답 행동을 조정한다.
- **PEFT**: 일부 매개변수 학습으로 업무에 적응한다.
- **양자화•장치별 런타임**을 결합한 제한 자원 추론이 핵심이다.
...
1. **과업•자원 목표 정의**: 정확도•지연•메모리•전력의 **허용 범위**를 설정한다.
2. **대표 데이터•교사 출력 준비**: 업무 분포와 범위 밖 입력을 포함한 **학습•평가 자료**를 구성한다.
3. **증류•미세조정**: 교사 지식과 업무 예시로 **특정 과업 능력**을 강화한다.
4. **양자화•런타임 변환**: 가중치 정밀도와 연산자를 목표 장치에 맞춰 **실행 형식**을 생성한다.
5. **과업 품질•장치 자원 검증**: 압축 전후 정확도와 실기기 **지연•메모리•전력**을 비교한다.
...
- **자원 제약**: 디바이스의 메모리•전력•연산 자원이 제한되어 모델 크기와 실행 방식에 제약을 주는 조건이다.

</details>

- 좁은 반복 업무에는 **SLM**, 범위 밖•복합 요청에는 **상위 LLM 위임**을 적용한다.
