...
- 정의/개념: 도메인 팀이 데이터 제품의 소유•품질•운영을 책임지는 **데이터 메시 운영 모델**이다.
- 배경/필요성: 중앙 데이터팀은 요청 증가 시 처리 병목•업무 의미 단절과 **품질 책임 공백**이 발생한다.
...
- 도메인 팀의 데이터 의미•품질에 대한 **종단 책임**이 핵심이다.
- 계약•문서•SLO 기반 **소비자 중심 데이터 제품**이 핵심이다.
- 셀프서비스 플랫폼•연합 거버넌스 기반 **분산 자율성•상호운용성**이 핵심이다.
#### 한줄 요약
...
- **도메인 소유권**: 데이터를 가장 잘 아는 업무 팀이 데이터의 품질•수명주기•지원 책임을 갖는 원칙이다.
- **연합형 계산 거버넌스**: 전사 공통 규칙과 도메인 자율 규칙을 정책 코드로 조정•자동 집행하는 운영 모델이다.
...
1. **데이터 제품•계약 전달**: 데이터•코드와 **스키마•SLO**을 제공한다.
2. **정책 검증 결과 전달**: 보안•품질 규칙의 **준수 결과**를 제공한다.
3. **승인 제품 전달**: 연합 정책을 통과한 **카탈로그 게시 정보**를 제공한다.
...
- **제품 역량**: 데이터를 소비자 요구•계약•SLO•지원 체계가 있는 지속 운영 제품으로 관리하는 능력이다.

</details>

- 도메인이 수명주기•SLO를 운영하면 **데이터 메시**, 아니면 **중앙 플랫폼**을 선택한다.
