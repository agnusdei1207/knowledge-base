...
sequenceDiagram
    ...
    participant C as 비신뢰 콘텐츠
    participant M as 모델
    participant T as 연결 도구
    participant D as 보호 자산
    A->>C: 1. 공격 지시 삽입
    C->>M: 2. 비신뢰 컨텍스트 유입
    M->>M: 3. 비신뢰 지시 승격
    M->>T: 4. 비인가 도구 행동 생성
    T->>D: 5. 정보 노출•외부 부작용 발생
```

1. **공격 지시 삽입**: 대화•문서•웹에 정책 교란 문장 은닉
2. **비신뢰 컨텍스트 유입**: 공격 콘텐츠가 모델 입력에 결합
3. **비신뢰 지시 승격**: 참고 데이터를 상위 실행 명령으로 오인
4. **비인가 도구 행동 생성**: 권한•대상 검증 없이 호출 구성
5. **정보 노출•외부 부작용 발생**: 유출•변조•발송 피해 유발
