...
- **프롬프트 인젝션(Prompt Injection)**: 비신뢰 입력의 지시가 상위 업무 지침을 교란해 모델 답변이나 도구 행동을 바꾸는 공격이다.
- **인공지능(Artificial Intelligence, AI)**: 입력을 바탕으로 예측•생성•의사결정을 수행하는 시스템이다.
- **비신뢰 입력**: 사용자•문서•웹 등에서 들어와 업무 지시로 실행하도록 승인되지 않은 데이터이다.

</details>

- 정의/개념: **프롬프트 인젝션**은 비신뢰 입력으로 상위 지침•도구 행동을 교란하는 공격이다.
- 배경/필요성: 모델은 자연어의 **신뢰 지시•비신뢰 데이터**를 완전히 구분하지 못해 권한 오용 가능성이 있다.
...
- 비신뢰 데이터의 **실행 지시 승격**이 핵심이다.
- 직접 입력•외부 콘텐츠의 **다중 유입 경로**가 핵심이다.
- 독립 권한 검증•최소 권한 기반 **도구 피해 제한**이 핵심이다.
...
1. **공격 지시 삽입**: 대화•문서•웹에 정책 교란 문장을 은닉한다.
2. **비신뢰 컨텍스트 유입**: 공격 콘텐츠가 모델 입력에 결합한다.
3. **비신뢰 지시 승격**: 참고 데이터를 상위 실행 명령으로 오인한다.
4. **비인가 도구 행동 생성**: 권한•대상 검증 없이 호출을 구성한다.
5. **정보 노출•외부 부작용 발생**: 유출•변조•발송 피해 유발한다.
...
- 입력 신뢰도가 낮으면 **컨텍스트 격리**, 도구 권한이 크면 **승인 게이트**를 적용한다.
