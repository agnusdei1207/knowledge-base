...
- 정의/개념: 차량 AI의 출력 부족•오류 위험을 수명주기로 관리하는 **ISO/PAS 8800 안전 사양**이다.
- 배경/필요성: 고장률 분석만으로는 AI의 **출력 부족•체계적 오류 위험 입증하기 어렵다**.
...
- 입력 조건•성능 한계의 **AI 부족성•차량 위험 연결**이 핵심이다.
- 데이터•모델•V&V•변경의 **안전 요구 추적성•보증 증거**가 핵심이다.
- 현장 위험•모델 변경 기반 **지속 감시•안전 재평가**가 핵심이다.
#### 한줄 요약
...
1. **위험•운행 조건 전달**: 차량 기능•운행 맥락•안전 목표와 AI 영향 경계를 정의한다.
2. **부족성•안전 요구 전달**: 알려진•잠재 성능 부족 조건과 완화•대체 대응(Fallback) 요구를 도출한다.
3. **데이터•모델 증거 전달**: 데이터 범위•품질•학습•모델 변경의 추적 가능한 증거를 생성한다.
4. **검증•보증 결과 전달**: 조건별 성능•불확실성•완화 수단을 시험해 안전 논증을 구성한다.
5. **현장 위험•변경 영향 전달**: 운행 사건•분포 변화•모델 변경에 따른을 재평가•재승인한다.
...
- AI 출력 부족•변경 시 **V&V•안전 케이스 갱신**과 **대체 대응**을 적용한다.

#### 한줄 요약

- 인공지능 안전은 운행 중에도 증거를 추적하여 허용 가능한 위험 수준임을 입증해야 한다.
