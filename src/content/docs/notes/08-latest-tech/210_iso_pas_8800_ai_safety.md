...
1. **위험•운행 조건 전달**: 차량 기능•운행 맥락•안전 목표와 AI 영향 경계를 정의함.
2. **부족성•안전 요구 전달**: 알려진•잠재 성능 부족 조건과 완화•대체 대응(Fallback) 요구를 도출함.
3. **데이터•모델 증거 전달**: 데이터 범위•품질•학습•모델 변경의 추적 가능한 증거를 생성함.
4. **검증•보증 결과 전달**: 조건별 성능•불확실성•완화 수단을 시험해 안전 논증을 구성함.
5. **현장 위험•변경 영향 전달**: 운행 사건•분포 변화•모델 변경에 따른을 재평가•재승인함.
...
- 인공지능 안전은 운행 중에도 증거를 추적하여 허용 가능한 위험 수준임을 입증해야  체계 적용
