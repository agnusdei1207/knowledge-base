...
- **1. 좋은•적격 사건 전달**: 사용자 SLI의 성공•실패 판정과 대상 요청 집계함.
- **2. 총예산•잔여량 계산**: `적격 사건 수 × (1 - SLO)`와 실제 실패량을 계산함.
- **3. 예산 소진율 전달**: 허용 속도 대비 실제 소진 속도로 고갈 위험을 판정함.
- **4. 운영 우선순위 전달**: 정상•경고•고갈 상태별 배포•복구•안정화 정책을 실행함.
- **5. 재개 조건 검증**: 사용자 영향 해소와 배포 재개 기준 충족을 확인함.
...
- **오류 예산•소진율**이 충분하면 배포하고 고갈 임계치 도달 시 안정화가 핵심 구현 필수
