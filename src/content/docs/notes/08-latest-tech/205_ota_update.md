...
- 정의/개념: 차량 소프트웨어를 원격 검증•설치•복구하는 **OTA Update 체계**이다.
- 배경/필요성: 방문•수동 갱신은 **정비 비용 증가•결함 대응 지연**을 초래한다.
...
- 패키지 서명•암호화•인증서 기반 **종단 간 신뢰성**이 핵심이다.
- 애플리케이션의 **SOTA** 부터 ECU 펌웨어의 **FOTA** 까지 차량•장치 조합을 확인하는 **호환성•안전 설치**가 핵심이다.
- 소규모 배포와 실패 중단•롤백의 **단계 배포•복구성**이 핵심이다.
#### 한줄 요약
...
1. **서명 패키지 등록**: 재현 빌드•서명•버전•의존성을 기록한다.
2. **대상•단계 배포**: VIN•하드웨어•현재 버전별 집단을 선정한다.
3. **서명•호환성 검증**: 해시•신뢰 체인•저장 공간•의존성을 확인한다.
4. **안전 조건 설치**: 주행•전원 조건 확인 후 비활성 슬롯을 설치한다.
5. **정상 슬롯 활성화**: 성공 시 확정하고 실패 시 이전 슬롯을 복구한다.
...
- 서명•호환성 검증 후 **A/B 슬롯** 기반 **단계 배포•자동 롤백**을 적용한다.
