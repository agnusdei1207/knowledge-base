...
**OTA**는 통신망을 통해 차량 소프트웨어를 원격으로 배포하고 검증·설치·복구하는 갱신 체계다.

</details>

- 정의/개념: **OTA**는 차량 소프트웨어를 원격 검증·설치·복구하는 갱신 체계
...
**FOTA**는 애플리케이션뿐 아니라 ECU 펌웨어까지 원격으로 갱신하는 방식이다.

</details>

- 패키지 서명·암호화·인증서 기반 **종단 간 신뢰성**
- 애플리케이션 **SOTA**부터 ECU 펌웨어 **FOTA**까지 지원
...
**SUMS**는 차량 소프트웨어 갱신의 정책·대상·절차·이력을 관리하는 체계다.
...
**단계 배포**는 작은 차량 집단부터 갱신하여 결과를 확인한 뒤 대상 범위를 점진적으로 넓히는 방식이다.
...
**A/B Slot**은 현재 실행 영역과 갱신 영역을 분리하여 설치 실패 시 이전 버전으로 복구하는 방식이다.
...
**의존성 매니페스트**는 대상 차량의 하드웨어·소프트웨어 버전과 패키지 호환 조건을 기계 판독 형식으로 명시한다.
...
**자동 롤백**은 갱신 후 정상 부팅이나 건강 검사가 실패하면 이전의 검증된 버전으로 되돌리는 복구 절차다.
