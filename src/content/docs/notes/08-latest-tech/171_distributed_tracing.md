...
- **트레이스(Trace)**: 하나의 전체 요청을 구성하는 스팬 집합로 정의된다.

</details>

- 정의/개념: Trace ID와 스팬의 부모•자식 관계를 연결하여 분산 요청의 종단 간 인과 경로와 구간별 지연을 재구성하는 **분산 추적 기법**임.
- 배경/필요성: 서비스별 로그만으로는 **서비스 간 호출 관계•병목 구간** 식별이 어렵음.
...
- Trace ID•Span ID와 부모 관계 기반 **분산 연산 인과관계**가 핵심임.
- 동기•비동기 경계의 컨텍스트 전파 기반 **요청 경로 연속성**이 핵심임.
- 샘플링•배기지 정책 기반 **수집 비용•민감정보 통제**가 핵심임.
#### 한줄 요약
...
- **스팬 컨텍스트 전파**: Trace ID•Span ID•샘플링 상태로 부모•자식 인과관계를 이어 주는 개념로 정의된다.
- **문맥 전파기**: 하이퍼텍스트 전송 프로토콜(Hypertext Transfer Protocol, HTTP) 헤더•메시지 속성에 추적 문맥을 주입하고 피호출 측에서 추출하는 구성요소을 의미한다.
- **수집기**: 서비스가 내보낸 스팬을 수신•가공하여 추적 처리 경로로 전달하는 구성요소을 의미한다.
- **샘플러**: 정책에 따라 보존할 추적을 선별하는 개념이다.
- **추적 백엔드(Trace Backend)**: 스팬을 추적 단위로 조립•저장•검색하고 임계 경로와 오류 구간을 분석하는 개념을 의미한다.
...
- **1. 스팬 컨텍스트 전달**: 호출 연산의 Trace ID•Span ID•표본화 결정을 생성함.
- **2. 추적 문맥 전달**: HTTP 헤더•메시지 속성 기반 부모 문맥 주입•추출임.
- **3. 자식 스팬 전달**: 피호출 연산의 부모 관계•시간•상태 계측임.
- **4. 부모 스팬 전달**: 호출 연산 완료 시 스팬 속성•이벤트 확정임.
- **5. 선별 스팬 전달**: 샘플링 후 부모•자식 관계와 임계경로 재구성함.
...
- **추적 연속성•샘플링 예산**을 지키며 오류•고지연 추적 우선 저장이 핵심임.

#### 한줄 요약

- 요청 번호를 끊김 없이 전달하고 필요한 추적을 비용 범위 안에서 보존해야  체계 적용
