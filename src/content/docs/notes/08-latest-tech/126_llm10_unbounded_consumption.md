...
- 정의/개념: LLM과 도구의 무제한 사용으로 **자원•비용•용량을 소진하는 LLM10 위험**이다.
- 배경/필요성: 요청 건수만으로는 **토큰•시간•도구 비용 편차** 포착할 수 없다.
...
- 무제한 소비에 따른 **서비스 거부•경제적 손실•모델 탈취**가 핵심이다.
- 주체별 **토큰•시간•동시성•도구 비용 계량**이 핵심이다.
- 연쇄 소비를 통제하는 **예약•상한•격리•중단•정산**이 핵심이다.
...
1. **주체•모델•최대 소비량**: 사용자별 비용 상한을 산정한다.
2. **예약 가능 예산**: 원장의 잔여 할당량으로 입장 가능성을 확인한다.
3. **승인 요청•실행 상한**: 예약 성공 요청만 격리 큐에 배치한다.
4. **토큰•시간•도구 사용량**: 실행 중 복합 자원 소비를 누적한다.
5. **중단 상태•실제 소비 정산**: 초과 작업 취소와 잔여 예산을 반환한다.
...
- 주체별 **토큰•시간 예산•도구•동시성 상한**을 두고 초과 작업 중단이 핵심이다.
