...
- **거대 언어 모델 10번 무제한 소비(Large Language Model 10 Unbounded Consumption, LLM10 무제한 소비)**: 제한 없는 모델•도구 사용으로 자원•비용•서비스 용량을 소진하는 OWASP LLM Top 10 2025의 열 번째 위험을 말한다.
- **토큰**: 언어 모델이 입력과 출력을 처리하고 사용량을 계량하는 문자열 단위이다.

</details>

- 정의/개념: LLM과 도구의 무제한 사용으로 **자원•비용•용량을 소진하는 LLM10 위험**임.
...
- **경제적 손실**: 과도한 모델•도구 호출이 사용료와 인프라 비용을 비정상적으로 증가시키는 피해을 의미한다.
- **복합 자원 계량**: 토큰•시간•동시성•도구 비용을 주체별로 함께 측정하는 통제로 정의된다.
- **격리 실행 풀**: 한 주체의 소비가 다른 사용자의 용량을 잠식하지 않도록 큐와 동시성을 분리한 자원 영역을 의미한다.

</details>

- 무제한 소비에 따른 **서비스 거부•경제적 손실•모델 탈취**가 핵심임.
- 주체별 **토큰•시간•동시성•도구 비용 계량**이 핵심임.
- 연쇄 소비를 통제하는 **예약•상한•격리•중단•정산**이 핵심임.
...
- **입장 제어기**: 남은 예산과 실행 용량을 확인해 요청의 시작 여부를 결정하는 구성요소로 정의된다.
- **실행 감시기**: 작업 중 실제 자원 소비를 누적하고 상한을 넘으면 실행을 중단하는 구성요소로 정의된다.
...
- **예산 예약**: 실행 전에 예상 최대 소비량을 원장에서 확보해 과도한 요청의 입장을 차단하는 처리을 의미한다.
- **소비 정산**: 실행 후 실제 사용량을 반영하고 사용하지 않은 예약 예산을 반환하는 처리을 의미한다.
...
- **요청 건수 한도**: 사용자별 호출 횟수를 제한해 폭주를 통제하는 방식을 말한다.
- **토큰•시간 예산**: 입출력 크기와 실행 시간을 제한해 요청별 비용 편차를 통제하는 방식로 정의된다.
...
- **도구 상한**: 요청 하나나 사용자별로 허용하는 외부 도구 호출 횟수•비용의 한도을 의미한다.
- **동시성 상한**: 한 주체가 동시에 실행할 수 있는 모델•도구 작업 수의 제한이다.

</details>

- 주체별 **토큰•시간 예산•도구•동시성 상한**을 두고 초과 작업 중단이 핵심임.
