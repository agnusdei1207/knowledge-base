...
- **LLM10 무제한 소비(Unbounded Consumption)**: '엘엘엠 텐'으로 읽고 2025판 열 번째 위험으로 무제한 모델·도구 사용의 자원·비용 소진을 표시
- **대규모 언어 모델(Large Language Model, LLM)**: '엘엘엠'으로 읽고 입력·출력을 토큰 단위로 처리하는 언어 모델을 표시
- **토큰(Token)**: 모델이 입력과 출력을 처리하고 사용량을 계량하는 문자열 단위
- **예산 원장(Budget Ledger)**: 사용자별 토큰·시간·도구·비용의 한도와 소비를 기록하는 장부

## Ⅰ. 개요

- 정의/개념: 제한 없는 모델 추론·도구 실행이 **자원·비용·서비스 용량을 소진하는 위험**
- **배경/필요성**: 요청 건수 제한만으로 막기 어려운 **토큰·시간·반복·외부 비용 편차 통제**
...
- **서비스 거부·경제적 손실·모델 탈취**를 무제한 자원 소비의 영향으로 다룬다.
- **입출력 토큰·실행 시간·동시성·도구 단계·비용**을 주체별로 계량한다.
- **사전 예약·실시간 상한·격리 풀·중단·정산**으로 연쇄 소비를 통제한다.
...
- 무제한 소비는 **가용성·비용·모델 탈취 위험**을 함께 통제
