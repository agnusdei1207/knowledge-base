...
- 기존 한계: 정적 모델과 사후 점검은 현실 자산의 현재 상태를 반영하지 못해 운영 중단 없이 변경을 검증하거나 고장을 예측하기 어렵다.
...
1. 발전사는 풍력 터빈의 진동·온도·정비 이력으로 베어링 수명을 예측하고 작업자 승인 후 정비 일정을 조정한다.

### 쉽게 이해하기 (학습용)

- 가상 예측을 현장 조치에 연결할 때는 설비 보호 제어와 작업자 승인을 우선한다.
...
- 디지털 트윈은 실제 상태를 정확히 반영하고 잘못된 예측이 현장 사고로 이어지지 않게 해야 한다.
