...
| 고려사항 | 위험 | 대책 |
|:---|:---|:---|
| **상태 동기화 오류** | 지연·결측·식별 오류로 실제와 다른 트윈 상태 사용 | 시간 동기화, 데이터 품질 규칙, 최신성 표시와 안전 기본값 |
| **모델 드리프트** | 설비 노후·환경 변화로 예측 오차 증가 | 실측 오차 감시, 버전 관리, 주기적 재보정과 유효 범위 명시 |
| **자동 제어 권한** | 잘못된 예측이 물리 사고로 전파 | 조치별 승인 등급, 물리 안전장치, 제한 구역 시험과 즉시 복귀 |
