...
- 정의/개념: **디지털 트윈**은 물리 대상과 상태•이력을 동기화하는 생애주기 가상 표현이다.
- 배경/필요성: 정적 모델•사후 점검은 현실 자산의 **현재 상태•변경 영향** 반영과 고장 예측하기 어렵다.
...
- **IoT 센서•사건**과 이력 기반 **트윈 상태 동기화**가 핵심이다.
- 모델•시뮬레이션 기반 **고장•변경 영향 예측**이 핵심이다.
- **ISO 23247** 기반 생애주기 연계와 **승인•안전 피드백**이 핵심이다.
...
1. **센서•사건 데이터 전달**: 실물 식별자와 **상태•시간 정보**를 제공한다.
2. **정규화 상태 전달**: 품질 검사•시간 정렬을 거친 **현재 상태**를 제공한다.
3. **트윈 상태•오차 전달**: 관측값과 예측값의 **차이 정보**를 제공한다.
4. **예측•권고 전달**: 변경•고장•성능의 **시뮬레이션 결과**를 제공한다.
5. **승인 제어 명령 전달**: 안전 규칙을 통과한 **실물 조치**를 제공한다.
...
- 단방향 상태 추적은 **디지털 섀도**, 예측•제어 피드백은 **디지털 트윈** 선택과 ISO 23247 참조가 핵심이다.
