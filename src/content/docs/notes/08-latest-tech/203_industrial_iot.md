...
- 정의/개념: 산업 설비•센서를 연결해 공정을 감시•분석•제어하는 **IIoT 체계**임.
- 배경/필요성: 고립된 설비는 원격 상태 파악과 **예측 정비** 수행이 어렵음.
...
- 기존 설비 데이터를 변환•정제하는 **산업 자산•에지 연결**이 핵심임.
- 장애 버퍼링•품질 검증을 적용한 **신뢰성 있는 데이터 흐름**이 핵심임.
- 중앙 장애에도 로컬 제어를 유지하는 **현장 자율성**이 핵심임.
#### 한줄 요약
...
- **프로그래머블 로직 컨트롤러(Programmable Logic Controller, PLC)**: 제어 로직에 따라 설비를 실시간 제어하는 산업용 제어기을 말한다.
- **에지 계층**: 현장에서 데이터를 정제•버퍼링하고 로컬 분석을 수행하는 계층로 정의된다.
- **보안 네트워크**: 산업 구역을 분리하고 인증•암호화•접근 통제로 설비와 플랫폼 사이 통신을 보호하는 개념이다.
- **IIoT 플랫폼**: 장치 등록•데이터 수집•명령•자산 모델•규칙 실행을 통합하는 산업용 사물인터넷 기반로 정의된다.
- **분석 계층**: 설비 데이터로 상태•고장•품질을 분석하는 계층을 의미한다.
- **운영 계층**: 분석 결과를 정비•품질 의사결정과 현장 정책에 반영하는 계층로 정의된다.
...
1. **상태•공정 데이터**: 센서 값과 PLC 상태를 수집함.
2. **검증•변환•버퍼**: 단위•시간•결측 확인과 저장 후를 전달함.
3. **보안 전송•자산 연결**: 구역 경계를 거쳐 자산•시계열에 연결함.
4. **이상•정비 분석**: 고장•품질 위험 탐지와 작업을 의사결정함.
5. **승인된 현장 정책**: 안전 인터록 범위의 정책만 에지에 반영함.
...
- 제어 지연•통신 복원력에 따른 **에지 제어•플랫폼 분석 역할 분리**가 핵심임.

#### 한줄 요약

- IIoT는 통신이 끊겨도 현장 안전 제어가 계속 동작하도록 설계해야  체계 적용
