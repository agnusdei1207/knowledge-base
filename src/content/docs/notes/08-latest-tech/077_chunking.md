...
| 핵심 고려사항 | 위험 | 대책 |
|:---|:---|:---|
| 청크 크기 | 너무 작으면 근거 문맥 단절, 너무 크면 검색 잡음·토큰 비용 증가 | 질문 유형별 크기를 탐색하고 Context Recall·Precision으로 선택 |
| 의미 경계·중첩 | 표·조항·코드 경계 절단 또는 과도한 중복 색인 | 문서 구조 우선 분할 후 필요한 경계에만 중첩 적용 |
| 상위 문맥·출처 | 검색된 조각만으로 제목·조항 위치를 해석하지 못함 | 문서 ID·계층 경로·페이지·인접 청크 참조를 메타데이터로 보존 |
