...
- 정의/개념: **매개변수 효율적 파인튜닝(Parameter-Efficient Fine-Tuning, PEFT)** 은 기반 모델을 동결하고 소수 추가·선택 매개변수만 갱신하는 기술군
...
**매개변수 효율적 파인튜닝(Parameter-Efficient Fine-Tuning, PEFT) 주입 구성**은 유형, 대상 층, 모듈 크기와 연결 위치를 지정한다.
...
1. **방식·대상 층·크기 선택**: 모델 구조와 실행 조건에 맞는 **매개변수 효율적 파인튜닝(Parameter-Efficient Fine-Tuning, PEFT) 구성** 결정
...
- 과업 표현력·학습 자원·모듈 전환 방식을 기준으로 **매개변수 효율적 파인튜닝(Parameter-Efficient Fine-Tuning, PEFT) 유형과 병합 정책** 결정
