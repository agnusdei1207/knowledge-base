...
| 구성요소 | 책임 |
|:---|:---|
| 동결 기반 모델 | 공통 표현 제공과 **원본 가중치 보존** |
| 주입 구성 | PEFT 유형·대상 층·크기의 **학습 범위 지정** |
| 학습 모듈 | 어댑터·가상 토큰·LoRA의 **업무 변화량 저장** |
| 실행 관리자 | 기반 모델과 모듈의 **병합·적재·전환** |
...
sequenceDiagram
    ...
    O->>C: 1. 품질·메모리·전환 조건 정의
    C->>B: 2. 방식·대상 층·크기 선택
    B->>T: 3. 기반 동결·학습 모듈 주입
    T->>R: 4. 선택 매개변수 갱신
    R-->>O: 5. 병합·동적 적재 검증
```
...
1. **품질·메모리·전환 조건 정의**: 과업별 허용 품질과 **학습·배포 자원** 설정
2. **방식·대상 층·크기 선택**: 모델 구조와 실행 조건에 맞는 **PEFT 구성** 결정
3. **기반 동결·학습 모듈 주입**: 원본 갱신을 막고 선택 위치에 **업무별 경로** 연결
4. **선택 매개변수 갱신**: 역전파 대상을 제한해 **경량 모듈 가중치** 학습
5. **병합·동적 적재 검증**: 배포 방식별 품질·지연·전환 비용의 **적합성 판정**
...
|:---|:---|:---|:---|
| 적용 기준 | 중간 표현 변환 필요 | 입력층 조정으로 충분 | 선형층 변화량 적응 |
