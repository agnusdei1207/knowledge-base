...
- **PEFT**: 기반 모델을 동결하고 소수의 추가·선택 매개변수만 갱신하는 미세조정 기술군이다.
...
- **주입 구성**: PEFT 유형·대상 층·모듈 크기와 연결 위치를 정의한 설정이다.
...
- **PEFT 구성**: 모델 구조·품질·메모리·전환 조건에 맞춰 방식과 대상 층을 정한 설정이다.
...
- **Adapter**: 모델 층 사이에 소형 신경망을 삽입하여 중간 표현을 학습한다.
- **Prompt Tuning**: 입력 앞의 학습 가능한 가상 토큰만 갱신한다.
- **LoRA**: 가중치 변화량을 두 작은 저랭크 행렬의 곱으로 학습한다.
...
- **PEFT 유형 선택 기준**: 과업 표현력·학습 자원·추론 경로와 모듈 전환 비용을 함께 비교하여 결정한다.
