...
- 정의: 모델 가중치 동결 후 최소 파라미터만 학습하여 비용 효율성을 극대화하는 미세조정 기법.
- 배경: 전체 파라미터 튜닝 대비 자원 소모 감소 및 업무별 모델 배포 최적화 필요성 증대.
...
- 기반 가중치 고정 후 필요한 파라미터만 갱신하여 효율적 학습 구조 형성.
- 주입 위치 및 구조를 통한 모델 용량과 추론 경로 최적화 가능.
- 공용 기반 모델과 개별 업무 모듈 분리 관리로 이식성 향상.
...
2. **기반 동결•학습 모듈 주입**: 원본 갱신 방지 및 선택 위치에 **업무별 경로** 연결.
3. **선택 매개변수 갱신**: 역전파 대상 제한으로 **경량 모듈 가중치** 학습.
...
- 과업 표현력•학습 자원•모듈 전환 방식을 고려하여 **PEFT 유형•병합 정책**을 결정함으로써 실무 환경의 유연한 대응 및 지속적 최적화 달성.
