...
- **지식그래프(Knowledge Graph)**는 식별된 개체와 사실 관계를 그래프 구조로 연결해 탐색·추론할 수 있게 한 지식망이다.

</details>

- 정의/개념: **식별 개체·사실 관계**를 그래프로 표현한 지식망
...
- **개체 연결(Entity Linking)**은 문서의 이름·별칭 언급을 고유 식별자에 매핑한다.
- **RDF**는 사실을 주어·술어·목적어 트리플로 표현하는 표준이다.
...
- **스키마**는 허용할 개체·관계·속성 구조를 정의한다.
- **온톨로지**는 개념·관계의 의미와 논리 제약을 명세한다.
- **프로비넌스(Provenance)**는 사실의 출처·관측 시점·유효 기간을 추적하는 정보다.
...
- **관계 경로 질의**는 여러 간선을 따라 개체 사이의 연결과 파생 사실 후보를 탐색한다.
- **트리플 검증**은 관계 방향·타입·속성이 스키마 제약을 충족하는지 판정한다.
...
- **관계형 DB·지식그래프·온톨로지**는 각각 표 구조, 개체 관계와 경로, 개념 의미와 추론 규칙에 중점을 둔다.
...
- **SHACL**은 RDF 그래프가 정의된 노드·속성 제약을 충족하는지 검증하는 W3C 표준이다.
- **오병합**은 동명이인이나 유사 명칭을 하나의 개체로 잘못 연결하는 오류다.
...
- **지식 신뢰성**은 개체 식별, 관계 제약, 사실의 출처·시점을 지속해서 정합하게 유지할 때 확보된다.
