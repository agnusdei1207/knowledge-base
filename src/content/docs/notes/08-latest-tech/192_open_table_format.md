...
- 정의/개념: 객체 저장소 파일을 논리 테이블로 관리하는 공개 메타데이터•스냅샷•커밋 **오픈 테이블 포맷**이다.
- 배경/필요성: 단순 파일 저장만으로는 **동시 쓰기 충돌•일관된 버전 조회** 관리하기 어렵다.
...
- 메타데이터 기반 스키마•파티션과 **유효 파일 집합 관리**가 핵심이다.
- 스냅샷•원자 커밋 기반 **ACID•충돌 검증**이 핵심이다.
- 공개 규격 기반 다중 엔진 접근과 **기능별 호환성 확인**이 핵심이다.
#### 한줄 요약
...
1. **기준 스냅샷 조회**: 현재 **스키마•유효 파일 집합**을 확인한다.
2. **새 불변 파일 생성**: 변경 데이터를 기존 파일 덮어쓰기 없이 기록한다.
3. **변경 메타데이터 생성**: 스키마•파티션•통계의 **새 상태**를 기록한다.
4. **충돌 검증•원자 커밋**: 기준 버전 확인 후 **메타데이터 포인터**를 교체한다.
...
- 다중 엔진 호환 포맷만 **선택**, 미지원 쓰기•삭제는 **운영 제외**가 핵심이다.
