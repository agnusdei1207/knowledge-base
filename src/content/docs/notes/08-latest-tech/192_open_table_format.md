...
date: "2026-08-04T14:50:35+09:00"
tags:
  - "notes-latest-tech"
weight: 192
extra:
...
- 정의/개념: 객체 저장소 파일을 논리 테이블로 관리하는 공개 메타데이터•스냅샷•커밋 **오픈 테이블 포맷**
- 배경/필요성: 단순 파일 저장만으로는 **동시 쓰기 충돌•일관된 버전 조회** 관리 곤란

#### 한줄 요약

- 공개 메타데이터•스냅샷•원자 커밋 기반 파일 테이블 관리
...
- 확정 스냅샷 기반 일관된 읽기와 엔진별 기능 호환 검증
...
2. **새 불변 파일 생성**: 변경 데이터를 기존 파일 덮어쓰기 없이 기록
3. **변경 메타데이터 생성**: 스키마•파티션•통계의 **새 상태** 기록
4. **충돌 검증•원자 커밋**: 기준 버전 확인 후 **메타데이터 포인터** 교체

#### 한줄 요약

- 신규 파일•메타데이터 생성 후 충돌 검증과 원자 커밋
...
- Spark•Trino 간 읽기•쓰기•삭제•스키마 진화 교차 검증
