...
- 정의/개념: **검색 증강 생성 평가(Retrieval-Augmented Generation Evaluation, RAG 평가)** 는 검색•생성•운영 품질을 분리 측정하여 실패 원인을 진단하는 평가 체계
...
- **문맥 재현율•정밀도** 와 **상위 k개 재현율(Recall at k, Recall@k)•평균 역순위(Mean Reciprocal Rank, MRR)** 기반 검색 품질 평가
...
5. **실패 단계•회귀 판정**: 검색•생성 지표를 원인 단계와 기준 버전에 귀속
