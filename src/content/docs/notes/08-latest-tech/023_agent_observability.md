...
- **추적(Trace)·구간(Span)**: 목표 추적과 호출 구간을 연결해 종단 실행 경로 재구성
- **구조적 로그·의미 규약**: 상관 식별자와 공통 필드로 신호 연계
- **서비스 수준 목표(Service Level Objective, SLO) 기반 경보**: 지연·오류·비용 목표를 벗어난 구간 탐지
...
- 판정 기준: **서비스 수준 목표(Service Level Objective, SLO)** 위반 구간을 경보
...
- **서비스 수준 목표(Service Level Objective, SLO)**: 지연·오류율·가용성·비용 같은 서비스 지표가 달성해야 하는 목표 수준이다.
- **위험 기반 관측**: SLO와 사건 위험도에 따라 추적 상세도·마스킹·샘플링 수준을 다르게 적용하는 방식이다.

</details>

- **서비스 수준 목표(Service Level Objective, SLO)·위험도** 에 따라 추적·마스킹·샘플링 수준 결정
