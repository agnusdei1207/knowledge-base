...
- 배경/필요성: 다단계 호출의 최종 결과만으로 **품질 저하•오류•비용 발생 구간** 추적하기 어렵다.

#### 한줄 요약

- **Trace•Span**으로 지연•오류•비용 발생 구간 식별이 핵심이다.
...
- **SLO 경보**: 지연•오류•비용의 목표 위반 식별이다.
- **버전 추적**: 품질 회귀와 변경 원인을 연결한다.
- **수집 통제**: 상세 신호의 저장 비용•개인정보 노출을 마스킹•샘플링으로 절충이다.
...
2. **지연•오류•토큰•비용 전송**: 모델•도구 단계의 **운영 신호**를 수집한다.
3. **민감정보 보호•표본 집계**: 마스킹•샘플링으로 **관측 비용•노출**을 통제한다.
...
- **관측 로그**: 장애와 성능 원인을 분석하도록 추적•지표•이벤트를 연계하고 집계하는 운영 기록한다.
- **감사 로그**: 책임과 규정 준수를 입증하도록 주체•행위•결과를 변조하기 어렵게 보존하는 증거 기록한다.
...
- 원인 분석은 **관측성**, 책임 입증은 **감사 로그**가 핵심이다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 모델•도구 간 **상관 문맥 단절** | **Trace ID•의미 규약**으로 구간 연결 | 종단 실행의 **원인 경로 재구성** |
| 원문 프롬프트의 **개인정보 노출** | **마스킹•해시•접근통제** 적용 | 분석 가능성과 **정보 보호** 균형 |
| 전량 수집의 **저장•처리 비용** | 위험별 **전수 수집•적응형 샘플링** | 핵심 사건 보존과 **비용 통제** |
| 버전 미기록에 따른 **회귀 원인 불명** | **모델•프롬프트•도구•정책 버전** 기록 | 변경별 **품질 회귀 추적** |
...
- **SLO•위험도**에 따라 추적•마스킹•샘플링 수준을 결정한다.

#### 한줄 요약

- **오류•고위험 사건**은 전수, 정상 사건은 표본 수집이 핵심이다.
