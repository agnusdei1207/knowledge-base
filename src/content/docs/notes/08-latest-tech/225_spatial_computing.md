...
**공간 컴퓨팅**은 센서로 물리 환경과 기기의 위치를 인식하고 디지털 정보·사용자 입력·기기 동작을 실제 공간 좌표에 정합하는 컴퓨팅 방식이다.
...
**공간 정합**은 가상 객체의 위치·크기·방향을 인식한 물리 공간의 좌표와 일치시키는 과정이다.
...
**SLAM**은 센서 데이터로 주변 지도를 생성하면서 그 지도 안에서 기기의 위치를 동시에 추정하는 기술이다.
...
**공간 앵커**는 가상 콘텐츠를 실제 공간의 특정 좌표나 사물에 지속적으로 고정하는 기준점이다.
...
**XR**은 가상현실·증강현실·혼합현실을 포괄하여 현실과 가상 정보의 결합 수준을 나타내는 기술 범주다.
...
**드리프트**는 센서 오차가 누적되어 디지털 객체가 고정된 물리 좌표에서 점차 벗어나는 위치 추정 오류다.
...
**LiDAR**는 레이저 펄스의 왕복 시간을 측정하여 물체까지의 거리와 3차원 깊이 정보를 얻는 센서다.
