...
- 정의/개념: **공간 컴퓨팅**은 물리 환경을 인식해 디지털 정보와 사용자•기기 동작을 실제 공간 좌표에 정합하는 컴퓨팅 방식이다.
- 배경/필요성: 평면 화면은 사용자•사물의 **공간 관계 기반 상호작용할 수 없다**.
...
- 시각•관성•**LiDAR 센서**의 **시간•좌표 정렬**이 핵심이다.
- **SLAM**으로 **장치 위치•환경 지도** 동시를 갱신한다.
- 객체•가림•통행 영역의 **공간 맥락 추론**이 핵심이다.
#### 한줄 요약
...
- **센서 융합**: 여러 센서 관측을 결합하여 위치•깊이 추정의 정확도를 높인다.
- **위치 추정**: SLAM으로 장치 자세와 환경 지도를 갱신한다.
- **장면 모델**: 표면•깊이•객체•의미•가림 관계를 현실 좌표계에 표현한 디지털 공간 지도이다.
- **공간 상호작용**: 시선•손•음성•컨트롤러 입력을 현실 공간의 객체와 행위에 연결하는 기능이다.
- **렌더링**: 추적된 시점에 맞춰 가상 내용을 실제 공간에 정합하여 표시한다.
- **작동 제어**: 로봇•장치를 공간 좌표에 따라 움직이도록 제어한다.
...
1. **기준•센서 보정**: 카메라•관성•깊이 센서 시간•좌표를 정렬한다.
2. **위치•지도 갱신**: **SLAM** 기반 자세•환경 지도를 추정한다.
3. **장면 관계 추론**: 평면•객체•가림•공간 앵커를 식별한다.
4. **공간 출력•작동**: 공간 입력 기반 렌더링•물리를 제어한다.
5. **폐루프 검증**: 드리프트•지연 보정 또는 안전을 정지한다.
...
- **확장현실(Extended Reality, XR)**: 가상현실•증강현실•혼합현실을 포괄하는 기술 범주다.
...
- **LiDAR 오차 검증**: 거리•깊이 측정값의 편향과 환경별 오차를 확인한다.

</details>

- 시각 정보 정렬은 **AR**, 공간 이해•입력•작동 연결은 **공간 컴퓨팅**을 선택한다.
