...
**논리 큐비트**는 하나의 양자 정보를 오류 정정 코드 공간에 인코딩하여 물리 오류로부터 보호하는 정보 단위다.
...
**코드 공간**은 오류 정정 코드가 유효한 논리 상태를 표현하도록 여러 물리 큐비트 안에 정한 부분공간이다.
...
**물리 큐비트**는 실제 장치에서 양자 상태·게이트·측정을 구현하며 논리 큐비트를 구성하는 하드웨어 정보 단위다.
...
**반복 오류 정정**은 신드롬을 주기적으로 측정·해독하여 계산 중 누적되는 물리 오류를 계속 추적하는 과정이다.
...
**물리 대 논리 자원비**는 하나의 신뢰할 수 있는 논리 큐비트를 구현하는 데 필요한 물리 큐비트와 연산의 규모다.
...
**오류 확산**은 하나의 물리 오류가 게이트 상호작용을 통해 여러 큐비트로 전파되어 논리 오류가 되는 현상이다.
...
**논리 오류율**은 오류 정정을 통과한 오류가 논리 상태나 연산을 잘못 바꾸는 확률이다.
