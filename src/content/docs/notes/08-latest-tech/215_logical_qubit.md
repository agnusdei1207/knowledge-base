...
| 고려사항 | 위험 | 대책 |
|:---|:---|:---|
| **논리 오류 억제 미달** | 물리 자원은 늘지만 정보 보존 이득 없음 | 거리별 물리·논리 오류율을 동일 주기에서 비교 |
| **자원 오버헤드** | 데이터·검사 큐비트와 논리 게이트 비용 급증 | 목표 알고리즘에서 큐비트·주기·공간시간 비용 공동 산정 |
| **해독·제어 지연** | 보정 정보가 늦어 논리 연산 일정 붕괴 | 실시간 해독 처리량 시험과 제어 경로 병렬화 |
