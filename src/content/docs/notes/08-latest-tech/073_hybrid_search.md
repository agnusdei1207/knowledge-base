...
- 기존 한계: 희소·밀집 검색 단독 사용은 각각 의미·정확어 누락
- 기존 한계: 키워드 검색은 표현이 다른 문서를 놓치고 벡터 검색은 정확한 고유명사·코드 일치를 놓칠 수 있다.
...
| 핵심 특징 | 단어 일치·희귀도 | 벡터 의미 유사도 | 순위·점수 융합 |
| 한계 | 동의어·우회 표현 누락 | 정확 식별자 누락 | 가중치·정규화 왜곡 |
...
- 검색 누락 극복을 위해 융합·중복 제거를 검토하여 **하이브리드 검색** 활용
