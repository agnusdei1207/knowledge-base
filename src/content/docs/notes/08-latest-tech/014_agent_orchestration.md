...
1. 고객 지원 에이전트가 문의와 주문 조회는 자동 처리하되, 환불 금액 변경 단계에서는 **HITL 분기**로 담당자 승인을 받고 체크포인트에 처리 상태를 기록한다.
...
- 다단계 업무를 일관되게 실행·복구하기 위해 경로 불확실성·상태 지속성·멱등성·권한 및 승인 지점을 검토하여 **에이전트 오케스트레이션**을 활용해야 한다.
