...
- 정의/개념: **스케일드 닷 프로덕트 어텐션**은 쿼리와 키의 내적을 키 차원의 제곱근으로 보정하고, 정규화한 가중치로 밸류를 결합하는 어텐션 연산이다.
- 기존 한계: 벡터 차원이 커지면 쿼리와 키의 내적값이 증가하여 소프트맥스가 포화되고 학습 경사가 작아질 수 있다.
...
> 요약: 키 차원에 따른 스케일링으로 고차원 내적 어텐션의 학습을 안정화한다.
...
1. 장문 생성 모델에 FlashAttention을 적용하여 스케일드 닷 프로덕트 어텐션의 중간 행렬 저장을 줄이고 GPU 메모리 병목을 완화한다.
...
- 고차원 토큰 관계를 안정적으로 계산하기 위해 키 차원·마스크·수치 정밀도와 메모리 비용을 검토하여 **스케일드 닷 프로덕트 어텐션**을 활용해야 한다.
