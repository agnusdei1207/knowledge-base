...
- 정의/개념: **직접 선호 최적화(Direct Preference Optimization, DPO)** 는 선호·비선호 응답쌍과 참조 모델을 이용해 별도 보상 모델 없이 언어 모델 정책을 직접 최적화하는 정렬 기법
- 배경/필요성: **인간 피드백 강화학습(Reinforcement Learning from Human Feedback, RLHF)** 은 보상 모델·온라인 강화학습으로 훈련 복잡도가 증가함
...
- **참조 모델**: 정책이 기준 분포에서 과도하게 이탈하지 않도록 상대 로그확률의 기준을 제공하는 동결 모델이다.
...
- **직접 선호 최적화(Direct Preference Optimization, DPO)**: 고정 선호쌍의 상대 로그확률을 직접 최적화한다.
...
**직접 선호 최적화(Direct Preference Optimization, DPO)** 는 고정 선호쌍을 직접 학습하고, **인간 피드백 강화학습(Reinforcement Learning from Human Feedback, RLHF)** 은 보상 모델을 거쳐 정책을 갱신한다.
