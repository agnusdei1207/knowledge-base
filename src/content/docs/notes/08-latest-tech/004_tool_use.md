...
date: "2026-07-31T08:36:16+09:00"
tags:
  - "notes-latest_tech"
weight: 4
extra:
...
- 정의/개념: LLM 제안과 호스트 검증·실행을 분리하는 **도구 호출(Tool Use) 연계 방식**
...
- **책임 축**: **대규모 언어 모델(Large Language Model, LLM)** 중심 도구·인자 제안, 호스트 중심 실행 결정
- **검증 축**: **자바스크립트 객체 표기법(JavaScript Object Notation, JSON) 스키마**·최소권한·업무 규칙 중첩 확인
...
- **자바스크립트 객체 표기법(JavaScript Object Notation, JSON) 스키마**: 도구 호출 인자의 자료형·필수 필드·허용 범위를 기계적으로 검증하는 명세다.
...
| 구성요소 | 책임 |
|:---|:---|
| 도구 레지스트리 | **모델 컨텍스트 프로토콜(Model Context Protocol, MCP)·응용 프로그래밍 인터페이스(Application Programming Interface, API) 스키마·위험·권한** 제공 |
| 모델·라우터 | **대규모 언어 모델(Large Language Model, LLM) 도구·JSON 인자·병렬 호출** 제안 |
| 실행 중개기 | **인증·검증·인간 참여형 통제(Human-in-the-Loop, HITL) 승인·샌드박스** 실행 |
...
- **인간 참여형 통제(Human-in-the-Loop, HITL)**: 환불·배송지 변경 등 실제 업무에 영향을 주는 행동을 사람이 승인하게 하는 통제이다.
...
| 환불·배송지 변경의 **업무 부작용** | **조회·변경 권한 분리·인간 참여형 통제(Human-in-the-Loop, HITL)·감사** 적용 | 실제 **업무 영향** 통제 |
...
- **모델 컨텍스트 프로토콜(Model Context Protocol, MCP)**: 인공지능 애플리케이션과 외부 데이터·도구·프롬프트의 연결과 메시지 교환을 표준화한 프로토콜이다.
- **인간 참여형 통제(Human-in-the-Loop, HITL)**: 변경 작업을 사람이 승인한 뒤 실행하도록 제한하는 통제이다.

</details>

- 외부 상태·업무 실행은 **도구 호출(Tool Use)**, 변경 작업은 **인간 참여형 통제(Human-in-the-Loop, HITL) 승인** 후 실행
