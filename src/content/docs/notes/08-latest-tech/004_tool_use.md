...
**도구 호출**은 LLM이 외부 기능과 인자를 제안하고 호스트가 이를 검증·승인·실행하여 결과를 모델에 돌려주는 연계 방식이다.

</details>

- 정의/개념: **도구 호출**은 LLM이 외부 기능과 인자를 제안하고 호스트가 검증·실행하여 결과를 모델에 돌려주는 연계 방식
...
**실행 책임 분리**는 모델이 도구 호출을 제안하되 실제 권한 확인과 실행은 신뢰 경계 안의 호스트가 담당하는 원칙이다.

</details>

- **책임 축**: LLM은 도구·인자를 제안하고 호스트가 실행 결정
- **검증 축**: JSON 스키마·최소권한·업무 규칙을 중첩 확인
...
**JSON 스키마**는 도구 호출 인자의 자료형·필수 필드·허용 범위를 기계적으로 검증하는 명세다.
...
**도구 결과**는 외부 실행이 반환한 데이터나 오류를 다음 판단에 사용하도록 구조화한 비신뢰 관찰값이다.
...
**코드 샌드박스**는 생성된 코드를 운영 시스템과 격리하고 시간·메모리·입출력 권한을 제한해 실행하는 환경이다.
...
> 요약: **도구 호출**은 상위 실행, **플러그인·함수 호출**은 구현 방식
...
**부작용 통제**는 데이터 변경·결제·전송 같은 비가역 행동에 승인·멱등성·감사 기록을 적용하는 운영 방식이다.
...
**MCP**는 AI 애플리케이션과 외부 데이터·도구·프롬프트의 연결과 메시지 교환을 표준화한 프로토콜이다.
