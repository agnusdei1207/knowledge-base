...
- 정의/개념: 학습 데이터의 분포와 구조를 모델링해 새로운 콘텐츠를 만드는 **생성형 AI**이다.
- 배경/필요성: 판별형 모델은 범주•수치 예측에 집중해 **학습 분포 기반 신규 콘텐츠 생성**하기 어렵다.

#### 한줄 요약

- 학습 분포에서 **새 글•그림•음성•코드**를 생성한다.
...
- **학습 목적**: 데이터 양식별 구조에 맞춘 생성을 최적화한다.
- 생성 품질과 **사실성•권리 적합성**의 분리한다.

#### 한줄 요약

- 생성 품질은 **사실성•안전성•권리 적합성**을 미보장이 핵심이다.
...
2. **프롬프트•참조 표현 주입**: 사용자 조건을 모델의 **생성 문맥**에 결합한다.
3. **조건부 분포 후보 표본화**: 학습 분포에서 확률적 **신규 콘텐츠**를 생성한다.
4. **사실•안전•권리 검증**: 기준 충족 시 반출하고 미달 시를 재생성한다.

#### 한줄 요약

- **생성 후보 검증** 후 기준 미달이면 조건을 수정해 재생성이 핵심이다.
...
- 신규 표본은 **생성형**, 범주•수치 예측은 **판별형**이 핵심이다.
...
| 학습•참조 자료의 **권리 침해** | **출처•이용 조건•생성물 유사성** 검사 | 저작권•계약의 **준수 가능성** 확보 |
| 유해•편향 콘텐츠의 **대외 노출** | **안전 분류•금지 조건•승인 워크플로** | 브랜드•사용자 **피해 예방** |
| 합성 데이터의 **분포 왜곡** | 실제 데이터의 **분포•희귀 사례•성능** 비교 | 학습 데이터의 **대표성 검증** |

#### 한줄 요약

- 광고 초안은 **제품 정보•금지 문구** 검증 후 게시가 핵심이다.
...
- 텍스트•코드는 **자기회귀**, 고품질 이미지•음성은 **확산 모델** 선택 후를 검증한다.

#### 한줄 요약

- 대상 양식과 **검증•권리 조건**에 맞는 생성 방식을 선택한다.
