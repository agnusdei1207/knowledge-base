...
| 구성요소 | 책임 |
|:---|:---|
| Q·K·V 투영 | 입력 임베딩의 Q·K·V 차원 선형 변환 |
| 점수 계산기 | Q와 K의 내적을 통한 원시 유사도 계산 |
| 스케일·마스크 | 스케일링 및 마스크 행렬 적용 |
| 소프트맥스 | 합 1인 어텐션 가중치 정규화 |
| 문맥 결합기 | 가중치 기반 V의 가중합으로 최종 문맥 산출 |
...
5. **V 가중합 문맥 출력**: 참조 비율대로 정보를 결합해 **토큰별 문맥 표현** 생성
...
|:---|:---|:---|
| 적용 기준 | 동일 시퀀스 내부 문맥 학습 | 서로 다른 시퀀스·양식 연결 |
| 핵심 특징 | 같은 입력에서 **Q·K·V 생성** | 대상 Q가 원천의 **K·V 참조** |
| 한계 | 길이에 따른 O(N²) 비용 | 원천·대상 정렬 품질 의존 |
