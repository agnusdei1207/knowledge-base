...
- **정의/개념**: 같은 문맥의 토큰 정보를 가중 결합
- **배경/필요성**: 먼 토큰 간 의존성 직접 학습

### 쉽게 이해하기 (학습용)
- 단어가 자기 의미를 정할 때 문장 속 다른 단어 참고 비율 계산
...
> 요약: 동일 시퀀스 내 관계 학습을 통한 문맥 정보 결합
...
1. Flash Attention 기반 메모리 연산 최적화
2. 어텐션 맵 시각화를 통한 입력 토큰 기여도 분석

### 쉽게 이해하기 (학습용)
- 긴 문장을 읽을 때 메모리를 덜 쓰도록 어텐션 연산을 효율화함
- 어떤 단어가 결과에 큰 영향을 주었는지 그림으로 그려 확인해봄

## Ⅶ. 결론

- 토큰 관계 학습을 위한 셀프 어텐션 아키텍처 선택
