...
- 동일 입력을 목적별 **쿼리(Query, Q)•키(Key, K)•밸류(Value, V) 벡터로 투영**
- **쿼리(Q)•키(K) 유사도 기반 밸류(V) 가중합 문맥 생성**
...
- 입력을 **쿼리(Query, Q)•키(Key, K)•밸류(Value, V)** 벡터로 투영한 뒤 점수•마스크•소프트맥스•문맥 결합 순서로 처리한다.
...
1. **동일 입력 쿼리(Query, Q)•키(Key, K)•밸류(Value, V) 투영**: 토큰 표현을 조회•식별•전달 목적의 **세 벡터** 로 변환
...
- **셀프 어텐션**: 같은 입력에서 Q•K•V를 만들어 시퀀스 내부 문맥을 학습한다.
- **크로스 어텐션(Cross-Attention)**: 대상 시퀀스의 Q가 원천 시퀀스의 K•V를 참조하여 서로 다른 시퀀스나 양식을 연결한다.

</details>

- **셀프 어텐션(Self-Attention)** 및 **크로스 어텐션(Cross-Attention)** 사이를 내부 문맥과 외부 시퀀스 연결 기준으로 구분하며 두 방식 모두 **쿼리(Query, Q)•키(Key, K)•밸류(Value, V)** 관계를 사용한다.
...
- **셀프 어텐션 선택 기준**: 하나의 시퀀스 안에서 토큰 사이의 내부 문맥 관계를 결합할 때 사용한다.
- **크로스 어텐션 선택 기준**: 입력과 출력 또는 텍스트와 이미지처럼 서로 다른 정보 원천을 연결할 때 사용한다.
