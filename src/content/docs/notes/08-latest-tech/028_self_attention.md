...
- 정의/개념: 동일 시퀀스의 각 토큰이 다른 토큰과의 관련도에 따라 값 벡터를 가중 결합하는 **셀프 어텐션**이다.
- 배경/필요성: 순차 상태 전달 구조는 멀리 떨어진 토큰의 **직접 관계 계산**에 한계가 발생한다.

#### 한줄 요약
- **토큰 간 참조 가중치**로 문맥 표현을 갱신한다.
...
- **Q**: 입력을 조회 기준 벡터로 투영이다.
- **K**: 입력을 비교 식별 벡터로 투영이다.
- **V**: 입력을 전달 정보 벡터로 투영이다.
- **Q•K 유사도**: V 가중합의 참조 비율 산출한다.
- 마스크 기반 참조 제어와 **O(N²) 연산•메모리**가 핵심이다.

#### 한줄 요약
- 모든 토큰 쌍을 비교해 **장거리 관계**를 직접을 계산한다.
...
1. **동일 입력 Q•K•V 투영**: 토큰 표현을 조회•식별•전달 목적의 **세 벡터**로 변환한다.
2. **Q•K 내적 점수 계산**: 각 토큰이 다른 토큰을 참조할 **원시 관련도**를 산출한다.
3. **스케일•마스크 적용**: 점수 폭을 안정화하고 미래•패딩 토큰의 **참조를 차단**한다.
4. **소프트맥스 가중치 산출**: 점수를 합이 1인 **참조 비율**로 정규화한다.
5. **V 가중합 문맥 결합**: 참조 비율대로 토큰별 문맥 표현을 생성한다.

#### 한줄 요약
- **스케일•마스크•소프트맥스**로 참조 비율 산출이 핵심이다.
...
#### 한줄 요약
- 내부 문맥은 **셀프**, 외부 연결은 **크로스 어텐션**이 핵심이다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 장문에서 **점수 행렬 메모리 급증** | **FlashAttention•윈도•희소 어텐션** | 정확도 보존과 **메모리 접근 절감** |
| 인과•패딩 마스크의 **참조 오류** | **경계 사례 단위 시험•시각화** | 미래•무효 토큰의 **정보 누출 차단** |
| 어텐션 가중치의 **설명 과신** | **삭제•교란 실험•별도 설명 기법** | 인과적 해석의 **오판 방지** |
| 긴 문맥의 **핵심 정보 희석** | **검색•요약•계층적 청킹•회수율 평가** | 중요 근거의 **참조 가능성 향상** |
...
- 동일 시퀀스는 **셀프 어텐션**, 외부 문맥은 **크로스 어텐션**을 선택한다.

#### 한줄 요약
- **문맥 출처**에 따라 셀프•크로스 어텐션을 선택한다.
