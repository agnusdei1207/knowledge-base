...
date: "2026-07-31T09:03:03+09:00"
tags:
  - "notes-latest-tech"
weight: 213
extra:
...
**QEC**는 논리 정보를 여러 물리 큐비트의 공동 상태에 인코딩하여 양자 오류를 검출·정정하는 기술이다.
...
**논리 인코딩**은 하나의 양자 정보를 여러 물리 큐비트의 코드 공간에 분산해 보호하는 방식이다.
...
**신드롬**은 논리 상태를 직접 측정하지 않고 안정자 측정으로 얻는 오류의 위치·유형에 관한 징후다.
...
**신드롬 해독**은 반복 측정 결과로부터 가장 가능성 높은 물리 오류를 추정하고 복구 연산을 결정하는 과정이다.
...
**코드 거리**는 서로 다른 논리 상태를 잇는 최소 물리 오류 수로, 정정 가능한 오류 수를 결정한다.
...
**오류 임계값**은 물리 오류율이 그보다 낮을 때 코드 규모를 늘려 논리 오류율을 계속 낮출 수 있는 경계다.
...
**논리 오류율**은 오류 정정 이후에도 오류가 논리 상태나 논리 연산을 잘못 바꾸는 확률이다.
