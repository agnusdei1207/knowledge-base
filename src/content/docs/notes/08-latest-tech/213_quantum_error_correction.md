...
- 배경/필요성: 물리 큐비트는 잡음에 취약하고 임의 상태를 복제할 수 없어 게이트·측정 오류가 누적되면 긴 양자 계산이 붕괴한다.
...
- **1. 논리 인코딩**: 정보를 여러 물리 큐비트의 공동 상태에 분산해 보호한다.
- **2. 신드롬 해독**: 논리 정보를 측정하지 않고 오류 위치·유형을 추정한다.
- **3. 임계값 정리**: 물리 오류율이 임계값보다 낮으면 코드 거리 증가로 논리 오류를 억제한다.
### 쉽게 이해하기 (학습용)
