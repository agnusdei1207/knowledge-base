...
| 고려사항 | 위험 | 대책 |
|:---|:---|:---|
| **임계값 미달 실패** | 물리 오류율이 높아 거리 증가에도 논리 오류 증가 | 장비별 임계값 실험과 오류 억제 후 코드 확장 |
| **상관 오류** | 독립 오류 가정이 깨져 해독 정확도 저하 | 상관 노이즈 특성화와 편향·상관 인지 해독기 적용 |
| **해독 지연** | 보정 결정이 측정 주기를 따라가지 못함 | 실시간 처리량 예산, 병렬 해독, 지연 시 안전한 프레임 관리 |
