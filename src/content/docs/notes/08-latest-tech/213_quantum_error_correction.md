...
- 정의: **QEC** 기반 논리 정보를 다수 물리 큐비트에 인코딩해 오류를 검출•정정하는 기술임이다.
- 배경/필요성: 물리 큐비트의 게이트•측정 오류 누적은 **긴 양자 계산 붕괴** 활용 초래함.
...
- 정보를 여러 물리 큐비트의 공동 상태로 보호하는 **논리 인코딩** 핵심임.
- 논리 정보를 읽지 않고 오류 위치•유형을 추정하는 **신드롬 해독** 핵심임.
- 임계값 아래에서 코드 거리로 논리 오류를 억제하는 **임계값 정리** 핵심임.
#### 한줄 요약
...
- 목표 논리 오류율에 맞춰 **코드 거리** 활용 정하고 **실시간 해독기** 활용 배치함.
