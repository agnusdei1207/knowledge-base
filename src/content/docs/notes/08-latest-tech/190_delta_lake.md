...
- 정의: Parquet 파일과 순차 로그로 **ACID 테이블** 관리하는 **델타 레이크**임이다.
...
- **`_delta_log`•체크포인트** 기반 **스냅샷 재구성** 핵심임.
- **낙관적 동시성 제어** 기반 **충돌 검증•원자 커밋** 핵심임.
- 스키마 관리•시간 여행과 **로그•파일 유지관리 부담** 핵심임.
#### 한줄 요약
...
1. 기준 스냅샷 조회: 체크포인트•후속 로그로 **유효 파일 상태** 활용 확인함.
2. 신규 데이터 파일 생성: 변경 결과를 **새 Parquet 파일** 기반 기록함.
3. 커밋 후보 제출: 추가•제거 파일과 **스키마•프로토콜 동작** 제출함.
4. 충돌 검증•원자 커밋: 동시 변경 검증 후 **원자 로그 버전** 확정함.
...
- 프로토콜 호환 엔진만 **채택**, 보존 스냅샷 참조 파일은 **삭제 제외** 핵심임.
