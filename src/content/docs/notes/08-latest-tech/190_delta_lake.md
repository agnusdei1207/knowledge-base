...
- 정의/개념: Parquet 파일과 순차 로그로 **ACID 테이블**을 관리하는 **델타 레이크**이다.
- 배경/필요성: 객체 저장소의 파일만으로는 동시 쓰기 충돌•부분 실패에 대한 **일관된 복구**하기 어렵다.
#### 한줄 요약
...
- **`_delta_log`•체크포인트** 기반 **스냅샷 재구성**이 핵심이다.
- **낙관적 동시성 제어** 기반 **충돌 검증•원자 커밋**이 핵심이다.
- 스키마 관리•시간 여행과 **로그•파일 유지관리 부담**이 핵심이다.
#### 한줄 요약
...
1. **기준 스냅샷 조회**: 체크포인트•후속 로그로 **유효 파일 상태**를 확인한다.
2. **신규 데이터 파일 생성**: 변경 결과를 **새 Parquet 파일**로 기록한다.
3. **커밋 후보 제출**: 추가•제거 파일과 **스키마•프로토콜 동작**을 제출한다.
4. **충돌 검증•원자 커밋**: 동시 변경 검증 후 **원자 로그 버전**을 확정한다.
...
- 프로토콜 호환 엔진만 **채택**, 보존 스냅샷 참조 파일은 **삭제 제외**가 핵심이다.
