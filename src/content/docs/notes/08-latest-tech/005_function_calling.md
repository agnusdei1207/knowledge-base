...
date: "2026-07-30T15:02:00+09:00"
tags:
  - "notes-latest_tech"
weight: 5
extra:
...
- 정의/개념: 모델이 자연어 의도를 함수명과 스키마에 맞는 인자로 구조화하면 호스트가 값·권한을 검증해 실제 함수를 실행하는 연계 방식
- 배경/필요성: 자연어 지시만으로는 함수 선택과 인자 형식 및 실행 권한·부작용의 경계를 일관되게 검증하기 곤란
...
- 함수 설명과 JSON 스키마가 모델이 제안할 함수·인자의 구조를 제한
- 호스트가 인자 값과 권한 및 부작용을 검증한 뒤 승인된 함수만 실행
- 호출 ID가 모델의 요청과 함수 실행 결과를 연결해 병렬·재시도 응답을 구분
...
| 절차 | 설명 |
|:---|:---|
| 함수명·설명·인자 스키마 제공 | 선택 가능한 함수와 필수 필드·자료형·허용 범위 제시 |
| 함수명·인자·호출 ID 제안 | 자연어 요청을 구조화된 실행 의도로 변환 |
| 형식·값·권한 검증 후 실행 | 원문·업무 규칙·권한과 대조해 승인된 함수 호출 |
| 실행 결과·오류 반환 | 함수 출력과 실패 원인을 호스트에 전달 |
| 호출 ID와 결과 연결 | 병렬 호출과 재시도 결과를 올바른 요청에 결합 |
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 스키마는 맞지만 잘못된 인자 값 | 모델 인자를 사용자 원문과 업무 데이터 및 허용 범위에 대조 | 의미상 오류 차단 |
| 재시도로 주문이 중복 실행 | 호출 ID와 멱등성 키를 저장하고 동일 요청 결과 재사용 | 중복 부작용 방지 |
| 권한 없는 상태 변경 함수 | 읽기·쓰기 함수를 분리하고 쓰기 호출에 인증·승인 적용 | 업무 권한 우회 방지 |
...
- 자연어를 외부 실행 의도로 변환해야 하면 함수 호출을 사용하되, 스키마 일치만 신뢰하지 않고 호스트가 값·권한·멱등성을 검증한 호출만 실제 함수에 전달
