...
**함수 호출**은 모델이 자연어 의도를 함수명과 구조화된 인자로 변환하여 호스트에 실행을 요청하는 연계 방식이다.

</details>

- 정의/개념: **함수 호출**은 모델이 자연어 의도를 함수명과 구조화된 인자로 변환하여 호스트에 실행을 요청하는 연계 방식
...
**구조화 출력**은 모델의 응답을 지정된 데이터 스키마에 맞춰 후속 시스템이 안정적으로 해석하게 하는 기능이다.
...
**함수 스키마**는 함수의 목적과 인자 이름·자료형·필수 여부·허용 범위를 선언하는 계약이다.
...
**호출 식별자**는 모델이 제안한 각 함수 요청과 호스트가 반환한 실행 결과를 정확히 연결하는 고유 값이다.
...
**병렬 함수 호출**은 데이터 의존성이 없는 여러 함수 요청을 모델이 함께 제안하여 호스트가 동시에 처리하게 하는 방식이다.
...
**도구 선택 정책**은 함수 호출을 자동·필수·금지하거나 특정 함수로 제한하여 모델의 호출 범위를 통제한다.
...
**멱등성**은 같은 함수 요청을 여러 번 실행해도 추가 부작용 없이 동일한 상태에 도달하는 성질이다.
