---
sidebar:
  order: 5
  label: "005. 함수 호출 (Function Calling)"
  badge:
    text: "기출 · 60%"
    variant: note
title: "함수 호출 (Function Calling)"
date: "2026-07-25T01:25:00+09:00"
tags:
  - "notes-latest_tech"
weight: 5
extra:
  question_no: "005"
  source_status: "기출"
  source_history: "138회"
  priority: 60
  priority_note: "함수 스키마 연계가 도구 실행의 기반"
---

## 미리 알고가기

- **함수 호출 (Function Calling)**: 자연어를 함수명·구조화 인자로 변환
- **제이에스온 (JavaScript Object Notation, JSON)**: 함수 인자 구조화 표현
- **구조화 출력 (Structured Outputs)**: 출력을 지정 스키마에 맞춤
- **도구 선택 (tool_choice)**: 자동·필수·금지 호출 정책 설정
- **병렬 함수 호출 (Parallel Function Calling)**: 복수 함수 호출 함께 제안
- **호출 식별자 (Identifier, ID)**: 함수 요청·결과 연결
- **응용 프로그래밍 인터페이스 (Application Programming Interface, API)**: 함수 호출 옵션·결과 계약 제공
- **비정형 데이터 (Unstructured Data)**: 고정 스키마 없는 자연어 데이터



## Ⅰ. 개요

- **정의/개념**: 모델의 구조화 요청을 호스트가 실행
- **배경/필요성**: 자연어를 검증 가능한 호출로 변환

### 쉽게 이해하기 (학습용)
- 외국인 점원에게 복잡한 한글 문장으로 주문했을 때, 점원이 기계적으로 딱 들어맞는 정형화된 주문서 빈칸 양식(메뉴, 수량)에 값을 적어내는 정밀한 변환기

## Ⅱ. 특징

- 함수 설명·스키마가 호출 후보·인자를 제한한다
- 구현 지원 시 복수·병렬 호출을 제안할 수 있다
- 구조 일치는 값의 사실성·유효성을 보장하지 않는다
- API 옵션이 자동·지정·금지 호출을 제어한다

### 쉽게 이해하기 (학습용)
- 빈 종이에 마음대로 일기를 적는 것이 아니라, 세무 신고서의 지정된 칸에 정확한 금액만 쉼표 기호 맞춰 적어내는 꼼꼼하고 규칙적인 특성입니다.

## Ⅲ. 구성요소 및 구조

| 설계 요소 | 설명 |
|:---|:---|
| 함수 명세 | 이름·설명·스키마 제공 |
| 모델 라우터 | 요청에 따른 함수·인자 제안 |
| 검증·정책 | 형식·값·권한·부작용 검사 |
| 호스트 실행기 | 함수 호출·결과·ID 반환 |

> 요약: 함수 명세·ID가 제안과 결과를 연결함

### 쉽게 이해하기 (학습용)
- 함수 명세·모델 제안·호스트 검증·실행 결과가 계약으로 연결됨

## Ⅳ. 원리 및 절차 흐름도

```mermaid
sequenceDiagram
    participant M as 모델
    participant V as 검증기
    participant H as 호스트
    M->>M: 명세제공
    M->>M: 호출제안
    M->>V: 인자검증
    V->>H: 함수실행
    H-->>M: 결과연결
```

| 절차 | 설명 |
|:---|:---|
| 명세제공 | 함수명·스키마 조건 제공 |
| 호출제안 | 함수명·인자·ID 제안 |
| 인자검증 | JSON 구조·값·권한 검증 |
| 함수실행 | 승인 함수 호스트 실행 |
| 결과연결 | 결과·ID 연결 반영 |

> 요약: 구조화 호출을 검증·실행하고 ID로 결과를 연결함

### 쉽게 이해하기 (학습용)
- 함수와 인자를 검증해 실행하고 호출 ID로 결과를 연결함

## Ⅴ. 종류 및 비교

| 판단 기준 | 함수 호출 | 프롬프트 기반 JSON | 제약형 구조 출력 |
|:---|:---|:---|:---|
| 목적 | 외부 함수 선택·인자를 표현함 | 응답 형식을 지시함 | 임의 데이터의 스키마 일치 |
| 실행 의미 | 호스트가 호출 의도를 실행함 | 별도 파서가 해석함 | 외부 실행을 뜻하지 않음 |
| 적용 기준 | 외부 함수 실행 의도가 필요할 때 | 형식 유도만 필요할 때 | 실행 없는 스키마 일치가 필요할 때 |

> 요약: 함수 호출은 의도를, 구조화 출력은 형식을 맞춘다.

### 쉽게 이해하기 (학습용)
- 프롬프트는 형식 유도, 함수 호출은 실행 의도를 구조화함

## Ⅵ. 실무 사례

- 추출값은 스키마·원문 검증 뒤 저장한다

### 쉽게 이해하기 (학습용)
- 서류 양식의 날짜 칸 밑에 회색 글씨로 "예시: 2026-07-13"이라고 친절하게 가이드라인을 적어두어 작성 실수를 원천 봉쇄하는 조치

## Ⅶ. 결론

- 서버 검증·승인·멱등성으로 호출을 통제한다

### 쉽게 이해하기 (학습용)
- 기계 부품이 정해진 규격 톱니바퀴에 한 치의 오차도 없이 완벽히 맞물려 돌아가야 기계 전체가 고장 없이 영구적으로 가동되는 구조
