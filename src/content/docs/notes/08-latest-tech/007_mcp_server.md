...
date: "2026-07-30T15:10:00+09:00"
tags:
  - "notes-latest_tech"
weight: 7
extra:
...
- 협상된 기능 범위 안에서 도구·리소스·프롬프트 목록과 명세를 클라이언트에 제공
- 요청 주체·인자·백엔드 권한·테넌트 범위를 서버에서 재검증
- 백엔드 결과를 MCP 콘텐츠로 변환하고 프로토콜 오류와 도구 실행 오류를 구분해 반환
...
| 절차 | 설명 |
|:---|:---|
| 초기화·기능 목록 요청 | 호환 버전과 클라이언트 기능을 확인하고 목록 조회 |
| 협상된 기능·명세 제공 | 허용된 도구·리소스·프롬프트의 메타데이터 반환 |
| 기능명·구조화 인자 요청 | 실행하거나 읽을 기능과 입력값 수신 |
| 권한·입력 검증 후 백엔드 실행 | 주체·테넌트·스키마·업무 규칙을 확인해 허용 기능만 호출 |
| 서버 변환 결과·오류 반환 | 업무 결과를 MCP 콘텐츠로 변환하고 오류 유형을 구분 |
