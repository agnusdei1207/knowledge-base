...
- 배경: 맞춤 API는 **기능 발견•권한 통제 재사용 곤란** #### 한줄 요약
- **MCP Server** 기반 허용한 도구•리소스•프롬프트만 노출
...
- **도구 제공자** 기반 stdio•Streamable HTTP 요청 처리
...
4. 권한•입력 검증 후 백엔드 실행: **요청•응답 상관관계** 활용 유지하며 허용 작업만 수행함.
...
|:---|:---|:---|
| 적용 기준 | **MCP Server** 기반 AI의 동적 기능 선택이 가능함 | **REST API Server** 기반 고정 API 계약 기반 기능을 제공함 |
...
#### 한줄 요약
- 동적 기능 발견은 **MCP**, 고정 웹 자원은 **REST** ## Ⅵ. 실무 고려사항 및 대책
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 과도한 공개로 공격 표면 확대 | **서버 최소 권한** 및 허용 목록을 적용함 | 고권한 기능 오용을 차단함 |
| 테넌트 식별 누락으로 교차 접근 | 인증 주체•백엔드 쿼리에 테넌트를 강제함 | 데이터 경계를 보장함 |
| 불명확한 오류로 복구 판단 혼선 | **구조화 오류** 기반 재시도 가능성을 전달함 | 안전한 복구 경로를 선택함 |
