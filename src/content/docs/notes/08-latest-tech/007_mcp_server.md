...
- **제이에스온 원격 프로시저 호출 (JSON Remote Procedure Call, JSON-RPC)**: 요청·응답과 오류를 표현하는 메시지 형식
...
- **배경/필요성**: 업무 데이터와 기능의 표준 노출·권한 통제

### 쉽게 이해하기 (학습용)
- 필요한 자료와 기능만 꺼내 주는 안전한 창구임

## Ⅱ. 특징
- 도구·리소스·프롬프트 기능 목록을 선언한다
- stdio·HTTP가 로컬·원격 연결을 지원한다
- 서버가 요청 인자·권한·접근 범위를 검증한다
- 결과·JSON-RPC 오류를 클라이언트에 반환한다
...
|:---|:---|:---|
| 기능 모델 | 도구·리소스·프롬프트 | 자원·HTTP 메서드 |
| 발견 방식 | 기능 목록·협상 | 문서·별도 탐색 |
