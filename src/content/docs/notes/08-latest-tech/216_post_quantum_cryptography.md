---
sidebar:
  order: 216
  label: "216. Post-Quantum Cryptography 양자내성암호 (Post-Quantum Cryptography)"
  badge:
    text: "기출 · 85%"
    variant: note
title: "양자내성암호 (Post-Quantum Cryptography, PQC)"
date: "2026-07-30T20:20:00+09:00"
tags:
  - "notes-latest-tech"
weight: 216
extra:
  question_no: "216"
  source_status: "기출"
  source_history: "126회, 129회, 135회"
  priority: 85
  priority_note: "양자내성암호 전환·알고리즘이 지속 반복됨"
---

## 미리 알고가기

- **피큐시(Post-Quantum Cryptography, PQC)**: 양자 이후 암호라는 이름으로, 양자 공격에도 안전하도록 설계한 고전 컴퓨터용 공개키 암호
- **켐(Key Encapsulation Mechanism, KEM)**: 공유 비밀키를 직접 보내지 않고 캡슐화·복원하는 공개키 기반 키 설정 방식
- **에이치엔디엘(Harvest Now, Decrypt Later, HNDL)**: 현재 암호문을 수집한 뒤 미래 양자컴퓨터로 복호화하는 선취 위협
- **암호 민첩성(crypto agility)**: 알고리즘·키·라이브러리를 서비스 중단 없이 교체할 수 있는 능력



## Ⅰ. 개요

- 정의/개념: 양자 공격을 견디는 공개키 암호 체계
- 배경/필요성: RSA·ECC는 충분한 규모의 양자컴퓨터에서 쇼어 알고리즘으로 해독될 수 있어 장기 기밀 데이터의 선취·후복호화 위험이 있다.

### 쉽게 이해하기 (학습용)

- 양자용 새 암호로 기존 시스템의 공개키 방식을 교체함

## Ⅱ. 특징

- **1. 용도별 알고리즘**: KEM은 키 설정, 전자서명은 인증·무결성을 담당한다.
- **2. 큰 키·서명**: 기존 공개키 암호보다 데이터 크기와 연산 특성이 달라 성능 검증이 필요하다.
- **3. 점진적 전환**: 암호 자산 목록·하이브리드 운용·민첩성으로 호환성 위험을 낮춘다.
### 쉽게 이해하기 (학습용)

- 시스템 전체의 호환성과 배포 절차까지 함께 검증해야 함

## Ⅲ. 구조 및 구성요소

```mermaid
block-beta
  columns 3
  N0["Crypto Inventory"]
  N1["Algorithm Profile"]
  N2["Hybrid 운영"]
  N3["Governance"]
  N0 --- N1 --- N2
  N2 --- N3
```

| 구성요소 | 책임 |
|:---|:---|
| Crypto Inventory | 암호 자산·라이브러리·HSM 위치 및 위험 식별 |
| Algorithm Profile | 표준 알고리즘과 용도별 파라미터 선택 |
| Hybrid 운영 | 기존 암호와 병행 및 downgrade 방지 구성 |
| Governance | 상호운용성, 성능 및 수명 주기 관리 |

### 쉽게 이해하기 (학습용)

- 중요 자료부터 새 자물쇠를 맞추고 단계적으로 교체함

## Ⅳ. 흐름도

```mermaid
sequenceDiagram
  participant I as 암호 자산 목록
  participant P as PQC 알고리즘 정책
  participant H as 하이브리드 암호
  participant S as 프로토콜·HSM·인증서
  participant G as 전환 거버넌스
  I->>P: 1. 자산·위험 우선순위
  P->>H: 2. 표준 알고리즘 선택
  H->>S: 3. 병행 구성·통합
  S->>G: 4. 상호운용·성능 검증
  G-->>I: 5. 기존 암호 폐기
```

**동작 원리**

- **1. 자산·위험 우선순위**: 데이터 수명·노출도·암호 위치 평가
- **2. 표준 알고리즘 선택**: 용도·파라미터·구현 적합성 결정
- **3. 병행 구성·통합**: 기존 암호와 PQC KEM·서명 결합
- **4. 상호운용·성능 검증**: 프로토콜·HSM·인증서 크기와 지연 시험
- **5. 기존 암호 폐기**: 단계 배포 후 downgrade 경로 제거

### 쉽게 이해하기 (학습용)

- 중요 보안 자산부터 문제없는 것을 확인하고 옛 방식 폐기함

## Ⅴ. 종류 및 비교

| 판단 기준 | Classical PK | PQC | QKD |
|:---|:---|:---|:---|
| 적용 기준 | 기존 키 설정·전자서명 | 양자내성 키 설정·전자서명 | 전용망 대칭키 분배 |
| 핵심 특징 | 인수분해·이산로그 난제 | 격자·해시 등 양자내성 난제 | 양자 측정 교란 기반 |
| 한계 | 대규모 양자 공격에 취약 | 큰 키·서명·전환 호환성 | 전용 광장비·거리 제약 |

### 쉽게 이해하기 (학습용)

- PQC는 수학 알고리즘 교체, QKD는 전용 통신 장비 도입임

## Ⅵ. 실무 고려사항 및 대책

| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 긴 데이터 수명의 선취 후 해독 위험 | 암호 자산 목록·우선순위 전환 | 장기 기밀성 보호 |
| 큰 키·서명으로 프로토콜 초과 | MTU·인증서·HSM 성능 시험 | 운영 호환성 확보 |
| 하이브리드 downgrade | 조합 협상 고정·로그·폐기 기준 | 전환기 보안 약화 방지 |

### 쉽게 이해하기 (학습용)

- 금융 게이트웨이는 표준 PQC KEM을 기존 키 합의와 병행해 단말 호환성·핸드셰이크 크기·지연·실패 복구를 검증한 뒤 단계 배포한다.

## Ⅶ. 결론

- 양자컴퓨터의 기존 공개키 암호 위협에 대비하기 위해 암호 자산·비밀 유지기간·표준 성숙도·호환성·성능을 검토하여, 장기 비밀 자산부터 PQC로 단계 전환해야 한다.

### 쉽게 이해하기 (학습용)

- 안전한 시스템을 위해 표준 방식을 선택하고 전체 체계 검증이 필수임
