...
date: "2026-08-14T13:02:00+09:00"
tags:
  - "notes-latest-tech"
weight: 216
extra:
...
- **타원 곡선 암호(Elliptic Curve Cryptography, ECC)**: 타원 곡선 이산로그 난제에 기반한 공개키 암호 체계을 의미한다.

</details>

- 정의: **PQC** 기반 양자 공격에 안전하도록 설계한 고전 컴퓨터용 공개키 암호임이다.
- 배경/필요성: **RSA**•**ECC** 기반 쇼어 알고리즘으로 해독될 수 있어 **선취 후 복호화 위험** 대상 노출됨.
...
- 키 설정용 **ML-KEM** 및 서명용 **ML-DSA**•**SLH-DSA** 핵심임.
- 기존 공개키 암호와 다른 **큰 키•서명•연산 특성** 핵심임.
- 자산 목록•하이브리드 운용•암호 민첩성 기반 **점진적 전환** 핵심임.
#### 한줄 요약
...
- **하드웨어 보안 모듈(Hardware Security Module, HSM)**: 암호키를 보호하며 암호 연산을 수행하는 전용 장치을 말한다.
...
선의 의미: 암호 자산 목록을 기준으로 알고리즘과 전환 정책을 정하고 하이브리드 운영을 통제하는 관계다.
...
1. 자산•위험 우선순위: 데이터 수명•노출도•암호 위치를 평가함.
2. 표준 알고리즘 선택: 용도•파라미터•구현 적합성을 결정함.
3. 병행 구성•통합: 기존 암호와 **PQC KEM•서명** 결합함.
4. 상호운용•성능 검증: 프로토콜•**HSM**•인증서 크기와 지연을 시험함.
5. 기존 암호 폐기: 단계 배포 후 downgrade 경로를 제거함.
...
- HNDL 위험이 큰 자산부터 **ML-KEM**•**ML-DSA** 기반 단계적으로 전환함.

#### 한줄 요약

- 표준 알고리즘과 프로토콜•장비•폐기 절차의 공동 검증.
