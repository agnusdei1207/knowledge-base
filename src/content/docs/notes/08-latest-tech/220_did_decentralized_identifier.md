...
date: "2026-08-14T13:26:00+09:00"
tags:
  - "notes-latest-tech"
weight: 220
extra:
...
- 정의: **DID** 기반 중앙 등록기관 없이 주체가 검증 수단과 수명주기를 제어하는 **URI 형태 식별자**임이다.
- 배경/필요성: 중앙 계정 종속으로 서비스 간 **신원 관계** 이전이 어렵음.
...
- 중앙 등록기관 없이 주체의 공개키로 제어권과 서명을 확인하는 **탈중앙 제어•암호학적 검증** 핵심임.
- **DID 메서드**: 기반 시스템별 생명주기 규칙을 정의함.
- **DID 문서**: 검증 방법과 서비스 엔드포인트를 제공함.
...
- **DID 제어자**: DID 문서의 키•서비스 정보를 갱신할 권한을 가진 주체을 의미한다.
- **DID 메서드**: 특정 장부•네트워크에서 DID를 생성•해석•갱신•비활성화하는 규칙을 말한다.
- **검증 방법(Verification Method)**: 주체가 키를 통제한다는 서명•인증을 검증하기 위한 공개키와 절차을 말한다.
- **DID 리졸버**: DID 문자열을 해당 메서드로 해석해 최신 DID 문서와 메타데이터를 반환하는 구성요소을 의미한다.

</details>

**DID 주체** 키를 제어하고 리졸버가 메서드에 따라 DID 문서를 해석한다.
...
1. 메서드 식별: DID 문자열에서 해석 규칙과 기반 시스템을 선택함.
2. 최신 문서•상태 조회: 갱신•비활성화 상태와 검증 방법을 확보함.
...
| 검증 가능 자격증명 상태 검증 | **폐기•정지 상태 목록** 연계 | **유효 자격** 한정 수용 |

#### 한줄 요약

- DID 문서의 키 상태와 발급기관 VC의 서명•폐기 상태를 확인한다.
...
- 주체 식별에는 **DID**, 자격 주장에는 **VC** 활용 구분하여 적용함.

#### 한줄 요약

- DID로 주체•키를 확인하고 발급자가 서명한 VC로 자격 검증.
