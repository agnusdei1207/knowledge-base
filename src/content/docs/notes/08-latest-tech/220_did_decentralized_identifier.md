...
- **분산 식별자(Decentralized Identifier, DID)**: 중앙 등록기관에 종속되지 않고 식별 주체가 검증 수단과 수명주기를 제어하는 통합 자원 식별자 형태의 식별자다.

</details>

- 정의/개념: **분산 식별자(Decentralized Identifier, DID)** 는 중앙 등록기관 없이 주체가 검증 수단과 수명주기를 제어하는 **통합 자원 식별자(Uniform Resource Identifier, URI)** 형태의 식별자
...
- **분산 식별자(Decentralized Identifier, DID) 메서드**: 기반 시스템별 생명주기 규칙 정의
...
- **DID 주체·제어자**: 식별 대상과 DID 문서의 키·서비스 정보를 갱신할 권한을 가진 주체이다.
...
**분산 식별자(Decentralized Identifier, DID)** 주체가 키를 제어하고 리졸버가 메서드에 따라 DID 문서를 해석한다.
...
- **1. 분산 식별자(Decentralized Identifier, DID) 생성**: 메서드 규칙에 따라 식별자와 문서 등록
...
- **분산 식별자 메서드(DID Method)**: `did:method:id` 형식의 식별자를 생성·해석·갱신·비활성화하는 방법별 규칙이다.

</details>

| 판단 기준 | 분산 식별자(Decentralized Identifier, DID) | DID 문서 | 검증 가능 자격증명(Verifiable Credential, VC) |
|:---|:---|:---|:---|
...
| **상관관계 추적** | **관계별 분산 식별자(Decentralized Identifier, DID)·최소 문서 정보** | **개인정보 노출** 감소 |
| **메서드 종속성** | **표준 리졸버·상호운용 시험** | **해석 결과 일관성** 확보 |
| **검증 가능 자격증명 상태 검증** | **폐기·정지 상태 목록** 연계 | **유효 자격** 만 수용 |

#### 한줄 요약

- 채용 시스템은 분산 식별자 문서의 키 상태와 발급기관 **검증 가능 자격증명(Verifiable Credential, VC)** 의 서명·폐기 상태를 함께 확인한다.
...
- **검증 가능 자격증명(VC)**: 발급자가 자격 주장에 암호학적 증명을 결합한 디지털 자격증명이며 분산 식별자와는 별도 계층이다.

</details>

- 주체 식별에는 **분산 식별자(Decentralized Identifier, DID)**, 자격 주장에는 **검증 가능 자격증명(Verifiable Credential, VC)** 을 분리 적용
