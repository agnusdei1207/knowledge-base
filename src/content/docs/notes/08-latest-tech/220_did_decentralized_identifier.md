...
- 정의/개념: **DID**는 중앙 등록기관 없이 주체가 검증 수단과 수명주기를 제어하는 **URI 형태 식별자**이다.
- 배경/필요성: 중앙 계정 종속으로 서비스 간 **신원 관계 이전하기 어렵다**.
...
- 중앙 등록기관 없이 주체의 공개키로 제어권과 서명을 확인하는 **탈중앙 제어•암호학적 검증**이 핵심이다.
- **DID 메서드**: 기반 시스템별 생명주기 규칙을 정의한다.
- **DID 문서**: 검증 방법과 서비스 엔드포인트를 제공한다.
...
1. **메서드 식별**: DID 문자열에서 해석 규칙과 기반 시스템을 선택한다.
2. **최신 문서•상태 조회**: 갱신•비활성화 상태와 검증 방법을 확보한다.
...
- 주체 식별에는 **DID**, 자격 주장에는 **VC**를 구분하여 적용한다.
