...
- 정의/개념: **서비스 메시**는 애플리케이션 밖의 데이터 평면에서 서비스 간 보안•라우팅•복원력•관측 정책을 일관되게 집행하는 통신 인프라 계층이다.
- 배경/필요성: 서비스별 통신 구현에서 발생하는 **언어 종속•정책 불일치•변경 부담** 완화가 필요하다.
...
- 제어 평면의 정책 배포와 **데이터 평면의 분산 집행**이 핵심이다.
- 서비스 간 **mTLS•인가•트래픽 제어•관측**이 핵심이다.
- 업무 로직과 통신 정책 분리, 대신 **지연•자원•운영 복잡성 증가**가 핵심이다.
...
- **서비스 워크로드**: 업무 로직을 수행하며 통신 기능을 데이터 평면에 위임하는 애플리케이션 인스턴스이다.
- **관측 계층**: 지표•로그•추적을 수집하여 통신 상태와 장애 원인을 분석하는 구성요소이다.
...
- **mTLS 인증서 검증**: 양쪽 인증서의 신뢰 체인•유효 기간•서비스 신원을 확인하는 절차이다.
...
1. **통신 정책 배포**: 라우팅•**mTLS•인가•재시도 규칙**을 전달한다.
2. **인증•라우팅•복원력 집행**: 신원 검증 후 경로•시간 제한•재시도를 적용한다.
3. **통신 결과 기록**: 지연•오류•추적 정보를 관측 계층에 전송한다.
...
- 공통 **mTLS•트래픽 정책**이익이 프록시 지연•운영 복잡성보다 클 때 서비스 메시 도입이 핵심이다.
