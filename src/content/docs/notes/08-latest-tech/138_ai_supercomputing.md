...
- 정의/개념: 대규모 AI 병렬 처리를 위해 연산•통신•저장•전력•냉각을 통합한 **AI 슈퍼컴퓨팅 인프라**
...
2. **노드별 모델 계산**: 각 가속기가 순전파•역전파로 생성하는 **로컬 갱신값**
3. **갱신값 집단 통신**: 노드별 값을 합산•분배해 **공동 갱신값** 구성
4. **모델 상태 동기화**: 모든 노드가 같은 파라미터로 **다음 반복 상태** 전환
5. **중간 상태 병렬 저장**: 장애 복구 지점의 모델•옵티마이저를 보존하는 **체크포인트**
...
- **병목 자원 우선 확장**: 집단 통신•저장 대역폭•전력 밀도를 개선해 지속 처리량 확보
