...
- 배경/필요성: 실제 양자 하드웨어는 멀리 떨어진 큐비트를 직접 연결하기 어려워 비국소 상호작용이 필요한 오류 정정 코드를 구현하기 어렵다.
...
- **1. X·Z 안정자**: 비트 반전·위상 반전 오류의 신드롬을 각각 추출한다.
- **2. 시공간 해독**: 여러 측정 주기의 변화를 연결해 데이터·측정 오류를 추정한다.
- **3. 거리 확장**: 임계값 아래에서 패치 크기를 늘려 논리 오류율을 낮춘다.
### 쉽게 이해하기 (학습용)
