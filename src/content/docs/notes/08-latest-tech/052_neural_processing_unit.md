...
| 예외 처리 및 제어 | 칩 내부 커맨드 파이프라인 제어 및 NPU 미지원 연산자의 CPU 폴백 중계 |

#### 한줄 요약

- **소프트웨어•메모리•코어 연산•폴백 제어** 구성
