...
- 정의/개념: **NPU**는 신경망 텐서 연산을 데이터 흐름으로 병렬 처리하는 전용 가속 프로세서이다.
- 배경/필요성: 범용 **CPU**는 대규모 **MAC** 병렬도와 메모리 이동에서 전력 효율 제약이 발생한다.

#### 한줄 요약

- **MAC 병렬 연산•데이터 재사용**에 특화된 가속기가 핵심이다.
...
- **MAC 연산 배열**을 통한 행렬•텐서 병렬을 처리한다.
- **온칩 메모리 재사용**을 통한 외부 데이터 이동 감소한다.
- 컴파일러의 **연산자 매핑•폴백 비용**과 **Roofline 모델** 기반 병목 판별이 핵심이다.
...
1. **그래프 분석•지원 연산 분할**: 의존관계와 NPU•CPU 실행 구간을 결정한다.
2. **텐서•자원 배치**: 실행 순서와 외부•온칩 메모리 이동 계획을 수립한다.
3. **텐서 DMA 전송**: 입력•가중치를 온칩 메모리에 비동기를 공급한다.
4. **MAC 병렬 연산**: 온칩 텐서를 재사용해 행렬•텐서 연산을 실행한다.
5. **실행 결과 결합**: NPU 연산과 CPU 폴백 결과를 최종 출력으로 통합한다.
...
- **NPU 매핑•CPU 폴백•SRAM 재사용** 공동을 측정한다.
...
- **연산자 지원률•전력 예산**으로 NPU 매핑과 **CPU 폴백 경계**를 결정한다.
