...
| 핵심 고려사항 | 위험 | 대책 |
|:---|:---|:---|
| 1차 검색 회수율 | 정답 문서가 Top-k 후보에 없으면 재순위기로 복구 불가 | 재순위 전 Recall@k를 확보하고 희소·밀집 하이브리드 검색 적용 |
| 후보 수·모델 크기 | 교차 인코더 호출량 증가로 꼬리 지연과 비용 급증 | 지연 예산에 맞춰 k를 제한하고 경량 모델·배치 추론 적용 |
| 순위 학습 데이터 | 쉬운 음성 표본 위주 학습으로 유사 오답 구분 실패 | 실제 검색 상위의 Hard Negative를 수집하고 nDCG·MRR로 평가 |
