...
- 정의/개념: RDMA•서브넷 관리•혼잡 제어를 통합한 **IB 저지연 스위치 패브릭**이다.
...
- **관리 축**: 서브넷 관리자의 주소•경로•분할 설정한다.
- **전송 축**: HCA의 등록 메모리 기반 저부하 RDMA이다.
- **혼잡 축**: SL•가상 통로•적응형 경로 기반 부하 분산이다.
...
- **전송•큐 계층**: 큐 페어•완료 큐와 신뢰•비신뢰 전송 서비스를 이용해 원격 직접 메모리 접근 작업을 처리하는 구성요소이다.
...
1. **장치 발견•주소•P_Key 설정**: 포트 상태를 수집해 **통신 종단•분할 경계**를 확정한다.
2. **경로•SL•가상 통로 구성**: 목적지와 서비스 수준에 맞는 **전송 경로**를 게시한다.
3. **RDMA 작업 패킷 전송**: HCA가 등록 메모리 요청을 **패킷 스트림**으로 변환한다.
4. **경로표 기반 패킷 전달**: 스위치가 주소와 가상 통로에 따라 **목적지 HCA**을 연결한다.
...
- **전용 패브릭•패브릭 격리**가 필요하면 IB, 기존 이더넷 활용은 RoCE을 선택한다.
