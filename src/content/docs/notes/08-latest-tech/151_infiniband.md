...
- 서브넷 관리자 기반 **주소·경로·분할 중앙 설정**
- HCA의 등록 메모리 기반 **저부하 RDMA 전송**
- 서비스 수준·혼잡 신호 기반 **가상 통로·적응형 경로**
...
5. **완료·혼잡 신호 반환**: 전송 결과와 정체 정보를 반영해 **속도·우회 경로** 조정
...
|:---|:---|:---|:---|
| 적용 기준 | 전용 다중 노드 저지연 패브릭 | 기존 이더넷 운영 활용 | 서버·확장 도메인 GPU 연결 |
...
- 통신 범위·혼잡 위치·토폴로지를 기준으로 **InfiniBand 분할·서비스·경로 정책** 결정
