...
- **인공지능(Artificial Intelligence, AI)**: 대규모 데이터 학습 및 추론 연산을 수행하는 컴퓨팅 시스템 환경.
- **중앙처리장치(Central Processing Unit, CPU)**: 시스템 제어 및 범용 연산을 수행하는 주 프로세서.
...
#### 한줄 요약
- 중앙 관리 기반 경로 제어와 서버 간 직접 메모리 통신을 결합한 클러스터 전용 고속 패브릭 구현.
...
#### 한줄 요약
- 서브넷 관리자 기반 경로 제어, HCA 직접 메모리 전송, 혼잡 제어 동적 경로 우회.
...
#### 한줄 요약
- 호스트 채널 어댑터, 스위치, 서브넷 관리자가 유기적으로 결합된 전용 고속 전송 구조.
...
#### 한줄 요약
- 서브넷 정책 설정 이후 RDMA 기반 서버 직접 통신 및 혼잡 제어 동적 경로 운영.
...
#### 한줄 요약
- 관리형 전용 패브릭(IB), 이더넷 활용 고속 전송(RoCE), GPU 내부 고속 연결(NVLink)의 적용 범위 차이.
...
#### 한줄 요약
- 통신 격리 정책 준수 및 핫스폿 혼잡 완화를 위한 적응형 경로와 가상 통로 최적화.
