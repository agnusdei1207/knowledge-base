...
- 정의/개념: **SASE**는 **SD-WAN** 연결과 **SSE**를 분산 서비스 거점에서 통합하는 아키텍처임.
- 배경/필요성: 중앙 백홀은 분산 사용자•클라우드 접속의 지연•병목과 **정책 편차**를 유발함.
#### 한줄 요약
...
- **SD-WAN•SSE** 기반 **연결•보안 정책 통합**이 핵심임.
- **ZTNA** 기반 사용자•장치•애플리케이션의 **지속적 접근 판정**이 핵심임.
- 인접 **PoP** 기반 **백홀 지연 감소**와 거점 품질 의존이 핵심임.
#### 한줄 요약
...
1. **신원•장치 상태 전달**: 사용자•장치•애플리케이션의 **위험 맥락**을 제공함.
2. **접근•검사 정책 전달**: **ZTNA•SSE**의 웹•방화벽 검사 규칙을 제공함.
3. **허용 트래픽 전달**: 정책을 통과한 **목적지별 세션**을 제공함.
4. **경로 품질 전달**: 지연•손실•가용성의 **경로 상태**를 제공함.
...
- **광역망(Wide Area Network, WAN)**: 먼 거리에 분산된 지점•데이터센터를 연결하는 네트워크을 말한다.
...
- 분산 사용자•지점은 **SASE**, 보안만 통합할 기존 **WAN**은 **SSE**을 선택함.

#### 한줄 요약

- SASE는 지역별 체감 지연과 통제 효과를 함께 시험해야  체계 적용
