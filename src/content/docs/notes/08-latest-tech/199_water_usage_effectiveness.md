...
- 정의/개념: **WUE**는 같은 기간 물 사용량을 **IT 에너지**로 나눈 **L/kWh 물 집약도**이다.
- 배경/필요성: 전력•냉각 효율만으로는 **현장 물 소비•지역 수자원 부담** 판단하기 어렵다.
...
- **ISO/IEC 30134-9:2022** 기반 동일 경계•기간을 측정한다.
- 수원•취수•방류•재사용을 구분한 **물 수지•지역 부담 해석**이 핵심이다.
- 증발 냉각은 **PUE 감소•WUE 증가**를 유발해 **CUE•SLO** 병행 판단이 핵심이다.
#### 한줄 요약
...
- **냉각 방식**: 공랭•증발 냉각•액체 냉각 등 물과 전력 소비의 교환관계를 결정하는 설비 선택하는 방식이다.
...
1. **물 사용량•IT 에너지 전달**: 같은 기간의 물 소비량과 **IT 분모 에너지**를 제공한다.
2. **WUE 영향 분석 전달**: 냉각 대안별 **WUE•PUE•탄소 변화**를 제공한다.
...
- 현장 물 절감은 **site WUE**, 전력 생산 물까지 포함하면 **source WUE**을 선택한다.
