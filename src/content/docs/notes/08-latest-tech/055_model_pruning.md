...
- 정의/개념: **중요도 기준**으로 가중치·채널·헤드·계층을 제거하는 모델 압축 기법
...
| **희소 패턴·스케줄러** | 제거 단위·비율·단계별 증가폭 제어 |
| **가지치기 변환기** | 마스크 적용 또는 모델 차원 축소 |
| **미세조정·검증기** | 제거 후 품질 회복과 과업별 회귀 평가 |
| **밀집·희소 실행 커널** | 밀집 축소 구조 또는 지원 희소 패턴 가속 |
...
|:---|:---|:---|:---|
| **적용 기준** | 임의 희소성을 지원하는 커널 | N:M 패턴을 지원하는 가속기 | 일반 밀집 커널의 차원 축소 |
| **핵심 특징** | 개별 가중치 제거 | 고정 묶음 내 지정 개수 제거 | 채널·헤드·계층 단위 제거 |
| **한계** | 인덱스 비용·희소 커널 의존 | 패턴·하드웨어 종속 | 높은 품질 손실과 구조 정합성 관리 |
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 높은 희소도로 **과업 품질** 급락 | **점진적 제거·미세조정**과 회귀 평가 | 목표 희소도 내 **허용 품질** 유지 |
| 커널 미지원·인덱스 비용으로 **희소 가속** 상쇄 | 장치 지원 **N:M 패턴·구조적 제거** 선택 | 명목 희소도의 **실측 지연** 전환 |
| 채널·헤드 제거로 **텐서 형상** 불일치 | 잔차 연결·헤드 차원의 **구조 정합성** 검증 | 구조적 가지치기의 **배포 실패** 방지 |
...
- **품질 허용치·장치 희소 패턴·실측 지연**을 기준으로 비구조·반구조·구조적 가지치기를 선택하는 압축 설계
