...
- 정의/개념: 다수의 논리적 작업을 소수 캐리어 스레드에 다중화하는 **JVM 관리형 가상 스레드**임.
- 배경/필요성: 요청별 플랫폼 스레드는 **스택 메모리•운영체제 스케줄링 비용 급증**이 발생함.
...
- 콜백 분할 없이 흐름•예외를 추적하는 **작업별 스레드 모델**이 핵심임.
- 블로킹 입출력 대기에 캐리어를 반환하는 **마운트•언마운트**가 핵심임.
- JDK 24 이후 synchronized 대기 해제와 **native•foreign 피닝 잔존**이 핵심임.
...
1. **작업 마운트**: 실행 가능한 가상 스레드를 캐리어에 연결함.
2. **입출력 대기•언마운트**: 블로킹 입출력 중 캐리어를 반환함.
3. **준비 큐 등록**: 입출력 완료 작업을 실행 대기로 전환함.
4. **작업 재마운트**: 사용 가능한 캐리어에 연결해 실행을 재개함.
...
- **캐리어 다중화•하류 동시성**을 통제할 수 있는 입출력 대기 작업에 적용함.

#### 한줄 요약

- 입출력 대기가 많고 피닝이 적으며 하류 용량을 제한할 수 있을 때 가상 스레드를 선택 체계 적용
