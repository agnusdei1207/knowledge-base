...
- 배경/필요성: 요청마다 플랫폼 스레드를 할당하면 스택 메모리와 운영체제 스케줄링 비용이 커지므로, 동기식 코드 구조를 유지하면서 높은 입출력 동시성을 확보할 수단 필요
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| **피닝·장기 CPU 점유** 미검증 시 캐리어가 반환되지 않아 처리량 저하 | JFR로 관련 사건을 관찰하고 장기 CPU·네이티브 작업을 제한된 별도 실행기로 분리 | 캐리어 반환 지연에 따른 **처리량 저하** 완화 |
| **외부 자원 과부하** 미검증 시 늘어난 동시 요청이 DB 연결과 외부 API 한도 소진 | 세마포어, 연결 풀, 호출별 동시성 한도로 하류 용량에 맞게 제한 | 하류 시스템의 **동시성 한도** 준수 |
| **스레드 로컬 남용** 미검증 시 가상 스레드 수만큼 큰 객체가 복제되어 힙 압박 | 수명 범위를 축소하고 대형 캐시 저장을 피하며 힙 프로파일링 수행 | 가상 스레드별 **힙 점유량** 축소 |
