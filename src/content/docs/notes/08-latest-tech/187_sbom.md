...
- 정의/개념: 소프트웨어 구성요소•버전•의존 관계를 기록한 **SBOM**이다.
- 배경/필요성: 완성 제품만으로는 직접•전이 의존성의 **취약점•라이선스 영향** 식별하기 어렵다.
#### 한줄 요약
...
- 식별자•버전•관계 기반 **직접•전이 구성요소 추적**이 핵심이다.
- **SPDX•CycloneDX** 기반 **기계 판독•자동 교환**이 핵심이다.
- 제품 해시•서명으로 **배포본 결속**, VEX로 **영향 판정**이 핵심이다.
#### 한줄 요약
...
1. **구성요소•관계 전달**: 빌드 입력•결과에서 직접•전이 의존성과 식별자를 추출한다.
2. **결속 SBOM 전달**: 형식•완전성 검증 후 제품 해시•서명•출처와 연결한다.
3. **배포 구성정보 전달**: 제품 버전별 SBOM을 실제 배포 자산에 매핑한다.
4. **영향 상태 전달**: 취약점 정보와 VEX를 결합해 영향•비영향•조치 상태를 판정한다.
...
- 구성요소는 **SBOM**으로 식별하고 **배포 자산 매핑•VEX 영향 판정**으로 조치 대상을 결정한다.
