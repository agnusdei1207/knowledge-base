...
- **피처 계보**: 피처 정의•엔터티•원천•변환•버전과 사용 모델의 연결을 기록하는 개념.
- **오프라인 저장소**: 시계열 과거 피처를 보관해 학습셋과 배치 추론에 제공하는 저장소.
- **온라인 저장소**: 최신 피처를 엔터티 키로 저지연 조회하도록 제공하는 저장소.
...
- **피처 레지스트리**: 피처의 이름•정의•엔터티•원천과 버전을 관리하는 개념.
- **처리 엔진**: 공통 정의로 과거•최신 피처를 계산하고 온라인 저장소에 동기화하는 구성요소.
- **피처 서버**: 추론 대상 엔터티의 최신 피처값을 온라인 저장소에서 반환하는 구성요소.
...
- **시점 정합 조인**: 각 학습 사례의 예측 시각 이전에 존재한 피처값만 연결하는 결합하는 개념.
- **이벤트 시각**: 실제 사건이나 피처값이 발생해 알 수 있게 된 시간.
...
- **시점 기반 피처 조회**: 특정 예측 시각에 알 수 있었던 과거 피처값을 찾는 조회 방식.
- **저지연 키 조회**: 실시간 추론에서 엔터티 식별자로 최신 피처를 빠르게 읽는 방식.
...
- **학습 데이터 누출**: 예측 시점 이후에 생성된 미래 피처값이 학습 사례에 포함되는 문제.
- **서빙 편향**: 온라인 동기화 지연이나 정의 차이 때문에 학습과 추론의 피처값이 어긋나는 문제.
...
- **온라인 신선도**: 최신 피처가 생성된 뒤 온라인 저장소에서 조회 가능해질 때까지의 지연 수준.
- **피처 일치**: 학습과 추론이 같은 정의•시점 규칙으로 계산된 값을 사용하는 성질.

</details>
- 오프라인 대용량 분석과 온라인 초저지연 서빙의 이중 요구사항을 단일 아키텍처로 완벽히 융합하는 엔터프라이즈 MLOps 및 실시간 추천/이상탐지 AI의 최고 핵심 데이터 인프라 표준(Feature Store / Feast & Hopsworks / Dual-Storage Engine: Offline Parquet/Delta & Online Redis/DynamoDB / Point-in-Time Correctness / Train-Serving Skew Prevention / Feature Lineage & Governance)의 확고한 표준으로 확고히 자리 잡았으며, LLM 임베딩 벡터와 결합된 하이브리드 피처 스토어로 진화하는 가운데, 실무 피처 스토어 구축 시에는 피처 레지스트리를 통한 표준 메타데이터 정의를 강제하고, 오프라인 시점 정합 조인(Point-in-Time Join)을 통해 데이터 누출을 차단하며, 스트리밍 파이프라인(Kafka/Flink)과 연동하여 수 밀리초(ms) 단위의 온라인 신선도(Freshness)와 고가용성 캐시 계층을 결합하여 완벽한 데이터 일관성과 실시간 추론 성능을 완성
