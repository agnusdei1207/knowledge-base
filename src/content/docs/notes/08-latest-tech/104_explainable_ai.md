...
- 정의/개념: AI의 판단 근거를 사람이 이해•검증할 형태로 제공하는 **XAI 기술**이다.
- 배경/필요성: 예측값만으로 **오류 원인•편향•이의제기 근거 확인하기 어렵다**.
...
- 모델 자체 해석과 **사후 설명**을 구분한다.
- 개별 출력의 국소 설명과 **전역 설명**을 구분한다.
- 충실도•안정성•이해도와 **보증 한계**를 평가한다.
...
- **품질 평가**: 충실도•안정성•이해도를 측정해 설명의 사용 가능 범위와 한계를 판정하는 활동이다.
...
1. **예측•모델 정보 수집**: 설명에 필요한 입력•출력•내부 정보를 확보한다.
2. **판단 근거 산출**: 기여도•규칙•사례를 목적에 맞게를 표현한다.
3. **설명 품질 평가**: 충실도•안정성•이해도와 한계를 판정한다.
...
- 개별 이의제기는 **국소 설명**, 모델 감시는 **전역 설명**을 선택한다.
