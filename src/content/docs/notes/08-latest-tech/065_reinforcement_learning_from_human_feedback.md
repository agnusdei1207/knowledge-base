...
| 구성요소 | 책임 |
|:---|:---|
| SFT 정책 | 모범 응답 기반 **초기 생성 정책 제공** |
| 선호 데이터 | 후보 응답 간 **인간 비교 순위 저장** |
| 보상 모델 | 응답별 **인간 선호 점수 예측** |
| 정책·가치 모델 | 응답 생성과 **기대 보상 추정** |
| 참조 정책·KL 제약 | 기준 분포와의 **과도한 정책 이탈 제한** |
...
sequenceDiagram
    ...
    M->>H: 1. 후보 응답 생성·선호 수집
    H->>R: 2. 선호쌍 기반 보상 학습
    R->>P: 3. 정책 응답 보상 산출
    P->>M: 4. PPO 보상·KL 정책 갱신
    M-->>E: 5. 유용성·안전성 독립 평가
```

### 동작 원리

1. **후보 응답 생성·선호 수집**: 같은 입력의 여러 답을 비교해 **우선순위 데이터** 구성
2. **선호쌍 기반 보상 학습**: 선택·비선택 응답의 순위를 예측하는 **보상 함수** 생성
3. **정책 응답 보상 산출**: 현재 정책의 생성 결과에 **학습 보상 점수** 부여
4. **PPO 보상·KL 정책 갱신**: 보상을 높이되 참조 정책과의 **분포 이탈** 제한
5. **유용성·안전성 독립 평가**: 보상 점수 밖의 실제 선호와 **실패 행동** 검증
