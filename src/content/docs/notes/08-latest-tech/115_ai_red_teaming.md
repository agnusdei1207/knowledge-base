...
- 완화 후 홀드아웃 세트를 활용한 **봉인 회귀 검증** #### 한줄 요약
...
1. 교전 규칙•위협 모델 승인: 공격 범위와 자산•경로•중단 조건의 **안전 경계** 활용 확정함.
2. 공격 코퍼스•변형 실행: 격리 환경에서 입력•도구 공격을 반복해 **실패 사례** 활용 탐색함.
3. 피해•재현 증거 분류: 공격 성공 조건과 영향•심각도의 **재현 가능한 발견** 기록함.
4. 원인별 완화 적용: 모델•프롬프트•권한•도구 경계의 **근본 통제** 활용 수정함.
...
- 회귀 후 잔여 위험이 허용 이하면 **배포**, 초과하면 **재완화** #### 한줄 요약
