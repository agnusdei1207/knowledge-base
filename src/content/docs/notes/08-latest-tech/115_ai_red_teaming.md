...
- 정의/개념: **AI Red Teaming**은 공격자 관점으로 AI의 안전•보안•프라이버시 피해 경로를 재현하는 평가 활동이다.
- 배경/필요성: 정상 기능 평가는 적대적 우회•권한 악용•정보 유출의 **복합 실패 경로** 탐지에 한계가 발생한다.
...
- 자산•공격자•경로•피해를 연결한 **위협 모델 기반 공격**이 핵심이다.
- 승인된 격리 환경의 **반복 실행•재현 증거 수집**이 핵심이다.
- 완화 후 홀드아웃 세트를 활용한 **봉인 회귀 검증**이 핵심이다.
...
1. **교전 규칙•위협 모델 승인**: 공격 범위와 자산•경로•중단 조건의 **안전 경계**를 확정한다.
2. **공격 코퍼스•변형 실행**: 격리 환경에서 입력•도구 공격을 반복해 **실패 사례**를 탐색한다.
3. **피해•재현 증거 분류**: 공격 성공 조건과 영향•심각도의 **재현 가능한 발견**을 기록한다.
4. **원인별 완화 적용**: 모델•프롬프트•권한•도구 경계의 **근본 통제**를 수정한다.
5. **홀드아웃 회귀 결과 전달**: 미공개 공격의 재발 여부와 **남은 피해** 제공한다.
...
- 재현 증거•회귀 결과로 **잔여 위험**을 판정해 **AI 배포•재완화**를 결정한다.
