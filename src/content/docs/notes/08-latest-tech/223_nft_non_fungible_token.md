---
sidebar:
  order: 223
  label: "223. NFT 대체불가능토큰 (Non-Fungible Token)"
  badge:
    text: "기출 · 30%"
    variant: note
title: "NFT 대체불가능토큰 (Non-Fungible Token)"
date: "2026-07-25T03:52:00+09:00"
tags:
- "notes-latest-tech"
weight: 223
extra:
  question_no: "223"
  source_status: "기출"
  source_history: "125회"
  priority: 30
  priority_note: "대체 불가 토큰은 최근 독립 출제 흐름이 약함"
---

## 미리 알고가기

- **대체 불가능(Non-Fungible)**: 각 단위가 독특하고 개별적인 가치를 지녀, $1$대$1$로 동일하게 상호 교환할 수 없는 성질임
- **메타데이터(Metadata)**: NFT가 가리키는 이미지 파일 주소, 창작자 정보, 속성값 등을 담고 있는 구조화된 데이터 명세서임
- **IPFS(InterPlanetary File System)**: 중앙 서버 대신 P2P 네트워크 상에 원본 데이터를 영구적이고 안전하게 분산 저장하기 위해 사용하는 프로토콜임
- **ERC-721(NFT 표준)**: 이더리움 블록체인에서 디지털 고유 자산을 NFT 형태로 표현하고 양도할 수 있게 구현한 최초의 표준 토큰 규격임
- **ERC-1155**: 다수의 대체 가능(FT) 및 대체 불가능(NFT) 토큰 형식을 단일 스마트 계약 내에서 처리하도록 설계된 효율적인 표준 규격임


## Ⅰ. 개요

- **정의/개념**: 블록체인에서 고유 토큰의 소유를 기록
- **배경/필요성**: 디지털 항목의 식별·이전 이력을 검증

### 쉽게 이해하기 (학습용)

- 블록체인 장부에 기록된 고유 번호표로, 해당 번호표의 통제권과 소유자 및 이동 이력을 증명하는 기술

## Ⅱ. 특징

- 계약 주소·토큰 ID가 고유 상태를 식별한다.
- 스마트 계약이 발행·승인·이전을 실행한다.
- 메타데이터 분리가 콘텐츠 가용성 위험을 만든다.
- 토큰 소유는 저작권·로열티를 자동 보장하지 않는다.

### 쉽게 이해하기 (학습용)

- 번호표의 주인은 장부로 알 수 있지만 번호표가 가리키는 그림과 저작권, 재판매 수익까지 자동으로 따라오는 것은 아님

## Ⅲ. 아키텍처 및 구성요소

```text
[NFT 구조]
Token ID ── Metadata URI ── Content Storage
    │
Smart Contract (Owner 상태 기록)
```

| 설계 요소 | 설명 |
|:---|:---|
| Smart Contract | 발행·소유·전송 상태를 집행함 |
| Token ID | 계약 안의 고유 단위를 식별함 |
| Metadata | 토큰 속성과 원본 참조를 기술함 |
| Storage | 메타데이터·원본을 보존함 |
| Wallet | 키로 토큰 통제권을 행사함 |

> 요약: token ID를 metadata·원본과 연결

### 쉽게 이해하기 (학습용)

- 블록체인 번호표, 설명서 주소, 실제 보관 창고, 지갑 열쇠, 저작권 계약서가 서로 연결돼야 하나의 쓸 수 있는 디지털 자산이 됨

## Ⅳ. 원리 및 절차 흐름도

```text
요청
 ↓
검증
 ↓
갱신
```

| 절차 | 설명 |
|:---|:---|
| 정의 | 자산의 권리와 운영 규칙을 정함 |
| 배포 | 스마트 계약을 블록체인에 배포함 |
| 연결 | 번호표와 메타데이터를 연결함 |
| 전송 | 승인된 소유권 이전 요청을 처리함 |
| 관리 | 생애주기와 이벤트 이력을 관리함 |

> 요약: 전송 시 권한과 수신 조건을 검사해 소유권 상태를 갱신함

### 쉽게 이해하기 (학습용)

- 무엇을 가진 것으로 볼지 계약부터 정하고 번호표를 발행해 설명서와 원본을 연결한 뒤, 이동·사용·분실·폐기 기록을 계속 관리함

## Ⅴ. 종류 및 비교

| 판단 기준 | ERC-721 | ERC-1155 | 중앙 DB Asset |
|:---|:---|:---|:---|
| 핵심 특징 | token ID별 단일 고유 단위 | 한 계약에서 유형·수량 관리 | 운영자가 소유 상태를 관리 |
| 적용 기준 | 고유 수집품·증서·입장권 | 게임 품목·복합 자산 묶음 | 폐쇄형 서비스 내부 자산 |
| 주요 위험 | 전송·발행 비용·원본 분리 | 계약·수량 권한 복잡성 | 운영자 종속·외부 이동 불가 |

> 요약: 고유 단위와 복합 단위에 맞춰 토큰 표준 선택

### 쉽게 이해하기 (학습용)

- ERC-721은 고유 번호가 붙은 한 장의 표, ERC-1155는 여러 종류와 수량을 한 장부에서 묶어 다루는 창고, 중앙 DB는 서비스 회사가 관리하는 내부 창고임

## Ⅵ. 실무 사례

1. 대상 환경의 도입 조건과 설계를 검증함
2. 운영 위험과 성과 지표를 검증함

### 쉽게 이해하기 (학습용)

- 공연 입장권 운영: 주최사가 좌석·공연일을 metadata에 담은 NFT를 발행하고 authorized marketplace·transfer window를 contract policy로 제한하며, 현장에서는 current owner와 미사용 상태를 확인한 뒤 check-in 상태를 소비 처리하고 wallet 분실 recovery 절차를 제공함
- 디지털 아트·license certificate: gallery가 검증된 creator만 mint하게 하고 content hash와 다중 pinning URI, 구매자가 받는 display·commercial license 범위를 약관에 연결하며, EIP-2981 royalty는 이를 지원하는 marketplace에서만 지급될 수 있음을 고지함

## Ⅶ. 결론

- 원본 보존·권리 계약·키 복구 후 NFT 적용 결정

### 쉽게 이해하기 (학습용)

- 블록체인 번호표만 믿지 말고 누가 발행했는지, 원본이 남아 있는지, 어떤 권리를 받는지와 지갑을 안전하게 지키는지를 함께 확인해야 함
