...
- 정의/개념: **지시 튜닝**은 다과제 지시문•입력•모범 응답으로 지시 이행을 지도 학습하는 방법이다.
- 배경/필요성: 다음 토큰 예측만으로는 **새 지시•출력 형식**의 안정적 준수할 수 없다.

#### 한줄 요약
- **다과제 지시•모범 응답**으로 지시 이행 학습이 핵심이다.
...
- **다양한 과제•지시 표현 혼합**을 통한 미학습 지시 일반화가 핵심이다.
- **대화 템플릿**을 통한 역할•지시•입력•응답 경계 일관성이 핵심이다.
- 지시 이행 학습과 **사실성•안전성 검증•인간 선호 정렬**의 분리한다.
...
- **지시 데이터 스키마**: 지시문•선택 문맥•입력•모범 응답을 일관된 학습 필드로 표현한 구조이다.
- **대화 템플릿**: 시스템•사용자•어시스턴트 역할과 구분 토큰을 모델 입력 형식으로 직렬화하는 규칙이다.
...
1. **목표 행동 정의**: 지시 준수•출력 형식의 **성공 기준**을 설정한다.
2. **지시•응답 자료 구성**: 과제별 입력과 **모범 응답**을 연결한다.
3. **중복•평가 누출 차단**: 근접 표본 제거와 **평가셋을 격리**한다.
4. **다과제 응답 지도 학습**: 응답 토큰 손실로 **매개변수를 갱신**한다.
5. **미학습 지시 분리 평가**: 일반화•사실성•**안전성** 독립을 측정한다.
#### 한줄 요약
...
- 지시 준수는 **지시 튜닝**, 사실•안전 행동은 **선호•안전 정렬**을 적용한다.
