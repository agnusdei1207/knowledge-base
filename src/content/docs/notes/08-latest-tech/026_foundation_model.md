...
- 정의/개념: **파운데이션 모델**은 광범위한 데이터로 범용 표현을 사전학습하고 조정·프롬프트를 통해 여러 하위 과업에 재사용하는 기반 모델
...
- **거대 언어 모델(Large Language Model, LLM)**: 대규모 토큰 분포를 학습하여 여러 언어 과업을 수행하는 파운데이션 모델의 한 유형이다.
- **시스템 위험(Systemic Risk)**: 공통 기반 모델의 편향·취약점·오류가 이를 사용하는 여러 하위 서비스로 함께 전파되는 위험이다.

</details>

- 대규모 범용 데이터의 **자기지도 사전학습**
- **LLM·전이학습**: 범용 표현과 능력을 여러 언어 과업에 재사용
...
| 적응 계층 | RAG와 **PEFT·LoRA**로 도메인 지식 반영 |
...
- **파운데이션 모델**: 범용 사전학습 결과를 여러 과업이 공유하고 RAG·PEFT 등으로 적응한다.
...
| 중앙 모델·API에 대한 **의존·정보 노출** | 장애 시 대체 모델 전환과 입력 분류·최소 전송 | 연속성·기밀성 **동시 확보** |
