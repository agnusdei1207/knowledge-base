...
- 정의/개념: 모델 추론 중 생성·탐색·검증에 가변 연산 예산을 배분해 답의 품질을 높이는 **테스트 시점 연산(Test-Time Compute)**
...
- **테스트 시점 연산**: 학습이 끝난 모델이 요청별로 생성·탐색·검증에 가변 연산을 투자한다.
...
| **꼬리 지연 증가** | **95백분위 지연(95th Percentile Latency, P95 Latency)·서비스 수준 목표(Service Level Objective, SLO)·시간 상한** 적용 | **전체 처리량 보호** |
...
- **한계효용 종료**: 추가 품질 이득이 비용보다 작아지거나 95백분위 지연(95th Percentile Latency, P95 Latency) 서비스 수준 목표(Service Level Objective, SLO)에 도달하면 추론을 멈춘다.
