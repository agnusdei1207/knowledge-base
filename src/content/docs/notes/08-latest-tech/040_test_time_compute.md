...
- 정의/개념: 모델 추론 중 생성•탐색•검증에 가변 연산 예산을 배분해 답의 품질을 높이는 **테스트 시점 연산**이다.
- 배경/필요성: 고정 예산으로는 문제별 **품질•지연•비용**을 동시에 조절하기 어렵다.
...
- 토큰•후보•검증 횟수의 **추론 예산 확장**이 핵심이다.
- **동적 예산 배정**: 난도•위험별 추론 자원을 할당한다.
- **조기 종료**: 품질 정체나 예산 한계에서 연산을 중단한다.
- 추가 연산은 **품질 한계효용**이 감소하고 지연•비용은 증가한다.
...
1. **난도•위험 추정**: 복잡도•불확실성•**영향도**를 산정한다.
2. **추론 예산 할당**: 토큰•시간•후보의 **상한을 설정**한다.
3. **후보 생성•탐색**: 과업별 **추론 전략**을 선택한다.
4. **품질•안전 검증**: 임계값•제약 기반 **후보를 판정**한다.
5. **종료•정책 환류**: 통과•정체•초과 기록과 **예산을 보정**한다.
...
- **한계효용 종료**: 추가 품질 이득이 비용보다 작아지거나 P95 지연 SLO에 도달하면 추론을 멈추는 통제이다.

</details>

- **난도•위험**으로 예산을 배분하고 **P95 지연•SLO** 안에서 종료한다.
