...
- 정의/개념: **시험 시점 연산(Test-Time Compute)**은 모델 추론 중 생성·탐색·검증에 더 많은 연산 예산을 배분하여 답의 품질을 높이는 추론 최적화 접근
...
- **P95 지연**: 전체 요청의 95%가 이 값 이하에서 완료되는 꼬리 지연 경계다.
...
| **꼬리 지연 증가** | **P95 SLO**와 시간 상한 적용 | **전체 처리량 보호** |
...
- **한계효용 종료**: 추가 품질 이득이 비용보다 작아지거나 P95 SLO에 도달하면 추론을 멈춘다.

</details>

- **난도·위험**으로 예산을 배분하고 **P95 SLO** 안에서 한계효용 기준 종료
