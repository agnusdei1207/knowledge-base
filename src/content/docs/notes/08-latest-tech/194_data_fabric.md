...
- 정의/개념: 활성 메타데이터로 분산 데이터의 연결•정책을 자동화하는 **데이터 패브릭**이다.
- 배경/필요성: 개별 도구•수작업 연결은 이기종 자산의 탐색 지연과 **통합 중복•정책 불일치**를 유발한다.
...
- 사용•품질•계보 신호 기반 **활성 메타데이터 자동 조치**가 핵심이다.
- 자산•업무 의미의 지식 그래프 기반 **발견•영향 분석**이 핵심이다.
- 복제•스트리밍•가상화 자동화와 **고위험 조치 승인**이 핵심이다.
#### 한줄 요약
...
- **거버넌스 자동화**: 메타데이터와 정책을 이용해 접근•품질•계보•수명주기 통제를 일관되게 집행하는 기능이다.
- **데이터베이스(Database, DB)**: 구조화된 데이터의 저장•조회•무결성을 관리하는 시스템이다.
- **서비스형 소프트웨어(Software as a Service, SaaS)**: 제공자가 운영하는 응용 소프트웨어를 네트워크로 제공하는 방식이다.
...
1. **메타데이터 신호 전달**: 분산 자산의 **기술•운영 메타데이터**를 제공한다.
2. **관계•업무 의미 전달**: 자산•용어•소유자의 **지식 그래프**를 제공한다.
3. **품질•접근 조건 전달**: 민감도•신뢰도•규제의 **정책 조건**을 제공한다.
4. **통합•접근 조치 전달**: 복제•스트리밍•가상화의 **실행 조건**을 제공한다.
5. **사용•변경 결과 전달**: 실제 사용•오류의 **자동화 피드백**을 제공한다.
...
- **공급자 종속**: 특정 패브릭 도구의 메타데이터 모델과 API에 의존해 다른 환경으로 이전하기 어려운 상태이다.

</details>

- 이기종 자산 연결 자동화는 **데이터 패브릭**, 도메인 책임 분산은 **데이터 메시**를 선택한다.
