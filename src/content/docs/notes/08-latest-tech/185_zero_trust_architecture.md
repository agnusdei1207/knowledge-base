...
- 정의/개념: **ZTA**는 네트워크 위치의 암묵적 신뢰 없이 자원별 접근을 지속 검증하고 최소 권한으로 집행하는 보안 구조이다.
- 배경/필요성: 경계 내부 신뢰는 계정•단말 침해 후 **과도한 권한•횡적 이동** 차단하기 어렵다.
...
- **1. 자원 중심 검증**: 위치가 아닌 주체•장치 상태와 대상 자원의 민감도를 결합해 접근 판단이다.
- **2. 동적 최소 권한**: 허용 자원, 행위, 시간, 세션 조건을 정책으로 세밀하게 제한한다.
- **3. 지속적 재평가**: 세션 중 행위•위협•장치 상태 변화를 관찰해 권한 축소•회수•재인증을 수행한다.
...
1. **접근 맥락 전달**: 신원•장치•행위와 **자원 민감도**를 제공한다.
2. **정책 결정 전달**: 허용•거부•제한과 **세션 수명**을 제공한다.
3. **세션 제어 전달**: 경로•자격증명의 **생성•종료 조건**을 제공한다.
4. **허용 요청 전달**: 승인된 **자원•행위 범위**만을 연결한다.
5. **세션 재평가 결정**: 위험 변화에 따라 **권한 유지•축소•회수**를 결정한다.
...
- 자원 민감도가 높으면 **접근별 검증**, 세션 위험이 커지면 **권한 회수**를 적용한다.
