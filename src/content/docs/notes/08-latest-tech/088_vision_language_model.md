...
- 정의/개념: 이미지와 텍스트를 정렬해 판별•생성하는 **VLM**이다.
- 배경/필요성: 단일 모달은 이미지의 객체•공간과 언어 의미를 **공동 해석**하기 어렵다.
...
- **시각 인코더** 기반 픽셀의 패치•영역 표현 변환이 핵심이다.
- **연결 계층** 기반 시각•언어 입력 공간을 정렬한다.
- 대조•매칭•생성 목적별 **출력 능력•오류 차이**가 핵심이다.
...
- **텍스트 인코더**: 입력 문장을 의미 특징이 담긴 언어 표현으로 변환하는 구성요소이다.
- **이미지 인코더(Image Encoder)**: 이미지를 패치•영역 특징의 시각 표현으로 변환하는 구성요소이다.
- **연결 계층(Connector)**: 시각 표현의 차원과 토큰 형식을 언어 모델이 참조할 수 있도록 투영•정렬하는 구성요소이다.
- **언어 디코더**: 시각 조건을 문맥으로 받아 다음 언어 토큰을 생성하는 구성요소이다.
...
1. **시각 표현 정렬**: 이미지 패치•영역의 **공동 의미 공간을 사상**한다.
2. **시각 조건 주입**: 정렬된 표현의 **언어 디코더 입력 문맥을 변환**한다.
3. **판별•응답 생성**: 이미지 문맥으로 **유사도•언어 토큰**을 산출한다.
...
- **문자 정확도•공간 관계 충실성**을 기준으로 VLM 판별•생성 방식을 선택한다.
