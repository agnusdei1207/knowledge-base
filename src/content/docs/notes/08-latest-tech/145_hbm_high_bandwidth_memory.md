...
- 정의/개념: **HBM**은 적층 **DRAM**과 초광폭 인터페이스로 가속기에 높은 대역폭을 제공하는 패키지 근접 메모리이다.
- 배경/필요성: 보드 메모리의 제한된 **핀 폭**으로 가속기 데이터 공급 병목이 발생한다.
...
- **적층 축**: **TSV** 기반 **DRAM 다이** 수직을 연결한다.
- **대역폭 축**: 다수 저속 핀의 초광폭 병렬 채널이다.
- **패키지 축**: 근접 배치로 전송 전력은 감소하나 열•수율 비용 증가한다.
...
1. **채널•뱅크 주소 전달**: 요청 분산•병렬을 접근한다.
2. **다이 데이터 전달**: 선택 **DRAM 다이•뱅크**를 읽기한다.
3. **수직 병렬 데이터 전달**: **TSV•인터포저**로 신호를 전송한다.
...
- 대역폭 병목은 **HBM**, 용량•확장성 우선은 **DDR**을 선택한다.
