...
- 배경/필요성: 보드 메모리의 **핀 폭·가속기 데이터 공급** 병목 완화 필요
...
- TSV 기반 **DRAM 다이 수직 적층**
- 다수 저속 핀 기반 **초광폭 병렬 채널**
- 패키지 근접 배치와 **열·수율·비용 복잡성** 절충
...
- **1. 메모리 요청 전달**: 가속기의 주소·읽기·쓰기 명령 발행
- **2. 채널·뱅크 주소 전달**: 독립 채널별 요청 분산과 병렬 접근
- **3. 다이 데이터 전달**: 선택 DRAM 다이·뱅크의 데이터 읽기
- **4. 수직 병렬 데이터 전달**: TSV를 통한 적층 다이 신호 전달
- **5. 초광폭 데이터 반환**: 인터포저 병렬 배선 기반 가속기 공급
...
- **대역폭 요구·열 한도·용량 계층**으로 HBM·GDDR·DDR 배치를 결정
