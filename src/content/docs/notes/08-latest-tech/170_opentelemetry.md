...
- 정의/개념: 공급자 중립 API•SDK•OTLP•Collector로 텔레메트리를 생성•처리•전송하는 **OTel 프레임워크**이다.
- 배경/필요성: 제품별 에이전트•속성 체계는 **언어•백엔드 종속과 중복 계측**을 유발한다.
...
- 계측 생성 계약과 처리 정책을 분리하는 **API•SDK 분리**가 핵심이다.
- 맥락 전파와 시맨틱 규약 기반 **맥락•의미 표준화**가 핵심이다.
- OTLP•Collector 기반 **중립 전송 파이프라인**이 핵심이다.
...
- **SDK 처리 정책**: 신호의 샘플링•자원 결합•일괄 처리•내보내기를 제어하는 정책이다.
- **Collector**: 텔레메트리를 수신해 처리하고 하나 이상의 저장•분석 백엔드로 전달하는 독립 구성요소이다.
- **맥락 전파**: 신호 사이의 요청 관계를 유지하도록 식별자를 전달하는 개념이다.
...
- **OTLP 전송**: OTel 신호를 프로세스와 Collector 사이에서 표준 형식으로 전달하는 개념이다.
...
1. **신호 생성**: API와 시맨틱 규약으로 메트릭•로그•추적을 생성한다.
2. **신호 처리**: SDK가 자원 정보를 결합하고 샘플링•일괄 처리를 수행한다.
3. **OTLP 표준 전송**: 내보내기 모듈이 Collector에 신호를 전달한다.
4. **신호 라우팅**: Collector가 수신•변환•필터링 후 백엔드를 전송한다.
...
- **계측•백엔드 분리•핵심 신호 보존율**을 보장할 때 OTel을 적용한다.
