...
- 정의/개념: 입력 주변의 모델 행동을 단순 모델로 근사하는 **LIME 기법**이다.
- 배경/필요성: 블랙박스 출력값만으로는 **개별 예측의 특징 영향** 식별할 수 없다.
...
- 예측 호출만 사용하는 **모델 비종속 설명**이 핵심이다.
- 거리 가중 교란 표본 기반 **국소 대리모델**이 핵심이다.
- 교란 분포•커널에 따른 **설명 변동성**이 핵심이다.
...
1. **교란 표본 생성**: 원본 주변 특징을 마스킹•변형한다.
2. **블랙박스 예측 수집**: 교란 표본별 모델 반응을 획득한다.
3. **거리 가중치 계산**: 원본에 가까운 표본에 높은 가중치를 부여한다.
4. **가중 국소 대리모델 학습**: 주변 예측 경계를 단순 모델로 근사한다.
...
- **국소 충실도•설명 안정성**을 기준으로 LIME의 교란 범위와 커널 폭을 결정한다.
