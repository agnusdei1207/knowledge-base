...
- 정의/개념: 정의된 ODD에서 전체 DDT와 대체 대응을 수행하는 **ADS**이다.
- 배경/필요성: 운전자 지원과 자동주행의 혼동은 **환경 감시•비상 대응 책임 공백**을 초래한다.
...
- 조향•가감속•OEDR을 포함한 **전체 DDT 수행**이 핵심이다.
- 도로•날씨•속도에 따른 **명시적 ODD 기반 작동 제한**이 핵심이다.
- SAE Level 3 운전자•Level 4 이상 시스템의 **대체 대응 책임 구분**이 핵심이다.
#### 한줄 요약
...
1. **유효 ODD•활성 상태 전달**: 도로•날씨•속도와 시스템 가용성 기반 기능을 진입한다.
2. **객체•사건 대응정보 전달**: 환경 인지•예측과 관련 객체•사건 대응을 생성한다.
3. **DDT•차량 상태 전달**: 행동•경로•조향•가감속 실행 결과의 지속을 감시한다.
4. **fallback 작동 요청**: ODD 이탈•고장과 대응 가능 시간을 판정해 전환을 개시한다.
5. **인계•MRM 목표 전달**: 자동화 수준에 따라 운전자 인계 또는 MRC 도달을 지시한다.
...
- SAE Level 3는 **운전자 인계**, Level 4 이상은 **시스템 MRM**을 적용한다.

#### 한줄 요약

- 어디까지 달릴지 명확히 하고 문제 발생 시 약속된 주체가 안전하게 마무리하는 것이 핵심이다.
