...
- 기존 한계: 운전자 지원과 자율주행을 구분하지 않으면 시스템의 작동 범위와 비상 상황의 대응 주체가 불명확해진다.
...
1. 고속도로 정체 구간의 **Level 3 자동주행**

### 쉽게 이해하기 (학습용)

- 시스템은 정해진 ODD 안에서 주행을 책임하고 이탈이 예상되면 충분한 인계 시간을 제공하며, 운전자 미응답 시 최소위험기동으로 안전 정차한다.

## Ⅶ. 결론

- 자동주행의 책임 공백을 막기 위해 ODD·동적 주행 과업·fallback 주체·최소위험상태·인계 시간을 검토하여, ADS 등급과 허용 운행 범위를 결정해야 한다.
