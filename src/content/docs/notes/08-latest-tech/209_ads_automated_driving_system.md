...
1. **유효 ODD•활성 상태 전달**: 도로•날씨•속도와 시스템 가용성 기반 기능을 진입함.
2. **객체•사건 대응정보 전달**: 환경 인지•예측과 관련 객체•사건 대응을 생성함.
3. **DDT•차량 상태 전달**: 행동•경로•조향•가감속 실행 결과의 지속을 감시함.
4. **fallback 작동 요청**: ODD 이탈•고장과 대응 가능 시간을 판정해 전환을 개시함.
5. **인계•MRM 목표 전달**: 자동화 수준에 따라 운전자 인계 또는 MRC 도달을 지시함.
...
- 어디까지 달릴지 명확히 하고 문제 발생 시 약속된 주체가 안전하게 마무리하는 것이 핵심 원칙 준수
