...
- **8비트 정수(Integer 8-bit, INT8)**: 값을 8비트 정수로 표현하는 저정밀 자료형을 의미하며, 시스템의 자율적 판단과 실행을 가능하게 하는 핵심 아키텍처 요소이다.
- **중앙처리장치(Central Processing Unit, CPU)**: 범용 명령과 미지원 연산을 처리하는 프로세서다.

</details>

- 정의/개념: **Edge TPU**는 완전 **INT8** 지원 연산을 저전력 실행하는 엣지 추론용 ASIC임. 이는 전통적 방식과의 핵심 차별점으로서 실질적인 업무 가치를 창출함.
- 배경/필요성: 범용 프로세서는 제한된 전력에서 **정수 텐서 처리량•종단 지연**을 동시에 충족하기 어렵음.

#### 한줄 요약
- 완전 **INT8 지원 연산**에 특화된 저전력 ASIC 구조 구현
...
- **종단 지연**: 입력 전처리부터 장치 전송•추론•후처리까지 전체 요청에 걸린 시간을 말하며, 실무적 관점에서 안정성과 효율성을 보장하는 주요 기전으로 작용한다.

</details>

- 가중치•활성값을 함께 변환하는 **완전 INT8 양자화**가 핵심임. 이는 전통적 방식과의 핵심 차별점으로서 실질적인 업무 가치를 창출함.
- 지원 연산만 묶어 가속하는 **하위 그래프 컴파일**이 핵심임. 이는 전통적 방식과의 핵심 차별점으로서 실질적인 업무 가치를 창출함.
- **CPU 폴백•전처리•전송**을 포함한 **종단 추론 지연**이 핵심임. 이는 전통적 방식과의 핵심 차별점으로서 실질적인 업무 가치를 창출함.

#### 한줄 요약
- **CPU 폴백•텐서 왕복**이 종단 지연을 결정 체계 적용
...
#### 한줄 요약
- **변환기•컴파일러•제어기•TPU•CPU 폴백**을 구성 체계 적용
...
1. **완전 정수 모델 변환**: 대표 데이터셋 또는 **QAT** 기반 가중치•활성값을 **INT8로 변환**함.
2. **지원 하위 그래프**: 연산자•텐서 형상으로 지원 구간을 확정함.
3. **TPU 구간 실행**: 지원 하위 그래프의 INT8 연산을 처리함.
4. **CPU 폴백 실행**: 미지원 연산을 처리하고 출력을 결합함.

#### 한줄 요약
- **정수 변환•구간 분할•TPU•CPU 실행**을 수행 체계 적용
...
#### 한줄 요약
- 제어는 **CPU**, 범용 병렬은 **GPU**, INT8은 **Edge TPU**가 적합하 기전 적용
...
- **INT8 품질•Edge TPU 지원률•CPU 왕복**을 측정하여 실무 환경에서의 유연한 대응과 지속적 최적화를 지원함.
...
#### 한줄 요약
- **양자화 품질•지원률•CPU 왕복**을 함께 측정 체계 적용
...
- **INT8 품질•지원률** 충족 시 Edge TPU, 미지원 연산은 **CPU 폴백**이 핵심임. 이는 전통적 방식과의 핵심 차별점으로서 실질적인 업무 가치를 창출함.

#### 한줄 요약
- **INT8 품질•지원률**에 따라 엣지 텐서 처리장치 배치를 결정 체계 적용