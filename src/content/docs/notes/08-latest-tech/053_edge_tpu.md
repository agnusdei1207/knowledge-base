...
- 정의/개념: **완전 INT8 모델의 지원 연산**을 저전력으로 실행하는 엣지 추론용 전용 ASIC
- 배경/필요성: 범용 프로세서는 제한된 전력에서 **정수 텐서 처리량·종단 지연**을 충족하기 어려워 전용 엣지 가속 필요
...
| **Edge TPU 컴파일러** | 지원 연산 판정과 하위 그래프 분할 |
| **런타임·장치 인터페이스** | TPU 호출과 입출력 텐서 전송 |
...
- **4. TPU·CPU 구간 실행**: 런타임의 텐서 전송과 장치별 연산 호출
- **5. 종단 결과 결합**: 가속·폴백 출력 통합과 후처리 결과 반환
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 완전 INT8 변환 시 **양자화 품질** 저하 | 대표 데이터셋 보정 또는 **QAT** 적용 | **INT8 추론 정확도** 회복 |
...
- **완전 INT8 적합성·컴파일 지원률·CPU 왕복 비용**을 기준으로 Edge TPU 적용 모델을 결정하는 엣지 가속 설계
