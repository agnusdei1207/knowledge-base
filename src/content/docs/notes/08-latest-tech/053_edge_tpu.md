...
- 배경/필요성: 범용 프로세서는 제한된 전력에서 **정수 텐서 처리량•종단 지연** 충족할 수 없다.

#### 한줄 요약

- 완전 **INT8 지원 연산**에 특화된 저전력 ASIC가 핵심이다.
...
1. **완전 정수 모델 변환**: 대표 데이터셋 또는 **QAT** 기반 가중치•활성값 **INT8을 변환**한다.
...
- 제어는 **CPU**, 범용 병렬은 **GPU**, INT8은 Edge TPU가 핵심이다.
