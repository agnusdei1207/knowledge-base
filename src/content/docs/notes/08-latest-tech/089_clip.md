...
- 배경/필요성: 고정 라벨 분류기는 새 개념마다 **라벨 데이터 재학습 필요** #### 한줄 요약
...
- 클래스 문구 유사도 기반 **제로샷 분류•교차 모달 검색** #### 한줄 요약
...
1. 이미지•텍스트 임베딩: 이중 인코더로 정규화된 **공동 좌표** 활용 생성함.
2. 쌍별 유사도 행렬: 온도 계수를 적용해 배치의 **모든 쌍을 비교**함.
3. 대조 손실 역전파: 정답 쌍은 가깝게, 오답 쌍은 멀게 **표현을 갱신**함.
4. 클래스 문구 유사도 판별: 자연어 클래스와 이미지의 **제로샷 분류** 활용 수행함.
...
- **전이 가능 범위** 활용 검증하고 문구 편차가 크면 **프롬프트 앙상블** 적용

#### 한줄 요약

- **전이 성능•문구 편차** 대상 따라 제로샷•앙상블 결정
