...
- 정의/개념: 이미지•텍스트 쌍을 공동 임베딩에 정렬하는 **CLIP 이중 인코더 모델**이다.
- 배경/필요성: 고정 라벨 분류기는 새 시각 개념마다 **라벨 데이터 재학습을 필요로 한다**.
...
- 정답 쌍은 가깝게 하고 오답 쌍은 멀게 하는 **대조학습**이 핵심이다.
- **이중 인코더** 기반 이미지•텍스트 임베딩 독립을 계산한다.
- 클래스 문구 유사도 기반 **제로샷 분류•교차 모달 검색**이 핵심이다.
...
1. **이미지•텍스트 임베딩**: 이중 인코더로 정규화된 **공동 좌표**를 생성한다.
2. **쌍별 유사도 행렬**: 온도 계수를 적용해 배치의 **모든 쌍을 비교**한다.
3. **대조 손실 역전파**: 정답 쌍은 가깝게, 오답 쌍은 멀게 **표현을 갱신**한다.
4. **클래스 문구 유사도 판별**: 자연어 클래스와 이미지의 **제로샷을 예측**한다.
...
- **전이 가능 범위**를 검증하고 문구 편차가 크면 CLIP 프롬프트 앙상블을 적용한다.
