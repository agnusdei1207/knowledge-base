...
- 정의/개념: 모델 출력•내부 신호로 학습 특징•민감 속성을 복원하는 **모델 역전 공격**이다.
...
- 상세 점수•기울기 기반 **복원 후보 반복 최적화**가 핵심이다.
- 보조 정보 결합에 따른 **후보 공간•재식별 범위 축소**가 핵심이다.
- **원본 일치도•개인 식별성** 분리 기반 누출을 판정한다.
...
1. **복원 후보 생성•질의**: 보조 정보로 초기 특징•민감 속성을 구성한다.
2. **점수•기울기 수집**: 목표 출력과 차이를 계산할 신호를 획득한다.
3. **목적함수 기반 후보 갱신**: 입력 특징을 개선 방향으로 수정한다.
4. **복원•재식별 가능성 평가**: 유사도•민감 속성•식별성을 판정한다.
...
- 출력 정보량이 많으면 **관측 채널** 제한, 재식별 성공률이 높으면 **프라이버시 학습**을 적용한다.
