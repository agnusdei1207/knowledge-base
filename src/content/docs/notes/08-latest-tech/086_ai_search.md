...
- 정의/개념: 질의 해석•복수 검색•재순위로 근거 답변을 제공하는 **AI 검색 방식**이다.
- 배경/필요성: 키워드 일치만으로는 표현 변형과 복합 근거의 **통합 검색하기 어렵다**.
...
- **질의 의도•제약** 기반 검색 경로를 결정한다.
- 어휘•의미 신호 보완 방식: **하이브리드 검색**이 핵심이다.
- **재순위와 근거 생성**을 분리하여 검색 관련성과 답변 충실성을 각각 검증한다.
...
- **검색 경로**: 질의의 의도와 제약에 따라 선택한 검색기•필터•융합 절차의 조합하는 개념이다.
...
1. **검색 경로 결정**: 질의 의도•제약에 맞는 검색 조합과 필터를 선택한다.
2. **후보 융합**: 희소•밀집 결과의 중복 제거와 **순위를 융합**한다.
3. **상위 근거 선별**: 재순위기의 **질의 관련성** 평가로 문서를 선별한다.
4. **근거 답변 생성**: 선택 근거에 충실한 답변과 **출처 인용**을 생성한다.
...
- **검색 회수율•답변 충실성**을 분리 측정해 AI 검색의 재순위•생성 경로를 조정한다.
