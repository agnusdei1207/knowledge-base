---
sidebar:
  order: 218
  label: "218. Blockchain Trilemma 블록체인 트릴레마 (Blockchain Trilemma)"
  badge:
    text: "기출 · 50%"
    variant: note
title: "Blockchain Trilemma 블록체인 트릴레마 (Blockchain Trilemma)"
date: "2026-07-25T03:47:00+09:00"
tags:
  - "notes-latest-tech"
weight: 218
extra:
  question_no: "218"
  source_status: "기출"
  source_history: "125회"
  priority: 50
  priority_note: "블록체인 트릴레마 비교는 과거 흐름 중심임"
---

## 미리 알고가기

- **블록체인 트릴레마**: 탈중앙화·보안성·확장성을 동시에 극대화하기 어려운 구조적 긴장
- **처리량**: 처리량 확대 시 검증자 감소나 신뢰 약화 우려가 있음
- **L2 등 접근**: L2·샤딩·모듈형 아키텍처로 트레이드오프 완화를 시도함



## Ⅰ. 개요

- **정의/개념**: 탈중앙화·보안·확장성 간 구조적 상충
- **배경/필요성**: 합의 성능·신뢰·참여의 균형 평가

### 쉽게 이해하기 (학습용)

- 세 목표를 동시에 높이려면 통신·저장 비용이 증가하므로 균형점을 찾아야 함

## Ⅱ. 특징

- 검증 참여·집중도가 탈중앙화를 나타낸다.
- 공격 비용·복구성이 보안 수준을 결정한다.
- 처리량·지연·수수료가 확장성을 나타낸다.
- 계층화·샤딩도 새 신뢰 가정을 만든다.

### 쉽게 이해하기 (학습용)

- 속도 향상이 검증 장비 비용이나 신뢰 구조에 미치는 영향을 살펴야 함

## Ⅲ. 아키텍처 및 구성요소

| 설계 요소 | 설명 |
|:---|:---|
| 제약 모델 | 성능·공격 모델 및 신뢰 범위 정의 |
| 탈중앙화 | 검증자 참여 지표 및 노드 다양성 |
| 보안성 | 비잔틴 장애 허용 및 경제적 보상 관리 |
| 확장성 | 트랜잭션 처리 효율 및 저장 비용 관리 |

[트릴레마 긴장 관계]
  └─ 탈중앙화·보안성·확장성

> 요약: 자원과 신뢰 제약 속에서 세 목표가 상호 긴장하는 관계임

### 쉽게 이해하기 (학습용)

- 목표 성능을 정하고 기술을 선택할 때 검증 조건과 신뢰 비용을 확인함

## Ⅳ. 원리 및 절차 흐름도

```text
목표설정
  ↓
지표정의
  ↓
비용추적
  ↓
상황검증
  ↓
대안비교
```

| 절차 | 설명 |
|:---|:---|
| 목표설정 | 부하 및 보안 공격 모델을 정의함 |
| 지표정의 | 성능·보안·탈중앙성 평가 지표를 정함 |
| 비용추적 | 설계안별 자원과 신뢰 이동을 추적함 |
| 상황검증 | 공격과 과부하 시 지표 변화를 확인함 |
| 대안비교 | 최적 균형점의 Pareto 대안을 선택함 |

> 요약: 부하·공격 모델로 자원·신뢰 이동을 평가

### 쉽게 이해하기 (학습용)

- 세 목표를 지표화하고 각 설계안이 비용과 신뢰를 어디로 옮기는지 확인함

## Ⅴ. 종류 및 비교

| 판단 기준 | L1 확장 | L2 롤업 | 모듈형/샤딩 |
|:---|:---|:---|:---|
| 핵심 특징 | 기본 레이어 용량 증대 | 오프체인 처리 및 증명 | 작업 분할 및 병렬화 |
| 적용 기준 | 노드 자원 및 포크 부담 | 시퀀서 및 브리지 의존성 | 공유 보안 및 인터페이스 위험 |
| 주요 위험 | 검증 장비 고사양화 | 운영자 집중 가능성 | committee 및 검증 효율 관리 |

> 요약: 다양한 설계안은 성능 비용을 다른 주체와 계층에 분산함

### 쉽게 이해하기 (학습용)

- 계층별로 처리와 검증을 분리해 비용과 신뢰를 재배치함

## Ⅵ. 실무 사례

1. **결제 롤업**: 시퀀서 집중도·검증 지연·브리지 위험을 종합 평가함
2. **게임 체인**: 샤드별 부하와 검증자 무작위 배정의 효과를 검증함

### 쉽게 이해하기 (학습용)

- 결제 롤업: 시퀀서 집중도·검증 지연·브리지 위험을 종합 평가함
- 게임 체인: 샤드별 부하와 검증자 무작위 배정의 효과를 검증함

## Ⅶ. 결론

- 신뢰 가정·성능 비용을 수치화해 확장 구조 선택

### 쉽게 이해하기 (학습용)

- 성능 향상이 어느 부분의 비용과 보안 위험을 키웠는지 숫자로 확인해야 함
