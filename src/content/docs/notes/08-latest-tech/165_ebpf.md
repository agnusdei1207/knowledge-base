...
- **JIT(Just-In-Time) 컴파일**: 검증된 eBPF 바이트코드를 실행 시점에 CPU 기계어로 바꾸는 기능이다.
...
- **사용자 공간 에이전트**: 독립 프로세스에서 시스템 API를 이용해 상태를 관측하고 제어하는 프로그램이다.
...
- **CO-RE(Compile Once, Run Everywhere)**: BTF 형식 정보를 이용해 한 번 컴파일한 eBPF 프로그램을 여러 커널 구조에 맞추는 방식이다.
...
- **커널 호환성**: 대상 커널이 프로그램 유형·훅·Helper·BTF 정보를 지원하는 정도이다.
