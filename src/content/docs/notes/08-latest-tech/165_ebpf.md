...
- 정의: 검증된 프로그램을 커널 훅에서 실행하는 **eBPF**이다.
- 배경/필요성: 커널 모듈은 결함이 **커널 전체 안정성** 대상 영향

#### 한줄 요약

- 검증된 프로그램을 커널 수정 없이 **동적 부착•실행** ## Ⅱ. 특징
...
- **제한 확장성**: 훅•프로그램 유형별 도움 함수와 문맥을 제한하고 **eBPF 맵** 기반 커널•사용자 공간 상태 공유임.
...
검증된 **eBPF** 바이트코드는 **JIT** 거쳐 **CPU** 기계어로 실행되고 eBPF 맵과 상태를 공유한다.
...
3. 커널 이벤트 실행: **eBPF 실행기•JIT** 허용 문맥에서 실행
...
- **호환성 검증•최소 권한•객체 회수** 활용 갖춘 eBPF만 배포
