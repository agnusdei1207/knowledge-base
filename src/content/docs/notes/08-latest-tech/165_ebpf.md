...
- 정의/개념: **eBPF**는 안전성 검증을 통과한 프로그램을 커널 훅에 동적으로 부착하여 커널 수정 없이 네트워크•보안•관측 기능을 확장하는 실행 기술임.
- 배경/필요성: 커널 모듈의 **배포•장애 복구•커널 안정성 위험**을 낮추면서 커널 경로를 확장하기 어렵음.
...
1. **바이트코드 안전성 검사**: 검증기가 종료•메모리•포인터 규칙을 검사함.
2. **프로그램 훅 부착**: 네트워크•추적•보안 등 지정 지점에 연결함.
3. **커널 이벤트 실행**: 이벤트 발생 시 **eBPF 실행기•JIT** 코드가 문맥•도움 함수 제한 안에서 실행함.
4. **정책 조회•상태 기록**: **eBPF 맵**에서 정책을 읽고 결과•통계를 갱신함.
...
- **검증기•커널 호환성**을 통과하고 **훅•맵 권한**을 제한한 eBPF만 배포가 핵심임.

#### 한줄 요약

- 검증 통과만으로 끝내지 않고 누가 어느 훅에 붙이며 Map과 프로그램을 어떻게 회수할지까지 관리해야  체계 적용
