...
- 적재 전 **제어 흐름·메모리·포인터 안전성 검증**
- 인터프리터 또는 **JIT 네이티브 실행**
- 훅·프로그램 유형별 허용 문맥과 **Helper 함수 제한**
- Map을 통한 커널·사용자 공간의 **상태·통계 공유**
...
> 요약: 로더가 검증된 프로그램을 훅에 부착하고 Map으로 상태를 공유한다
...
1. **프로그램 적재 요청**: 로더가 유형·명령·맵 정보를 커널에 전달
2. **검증 완료 바이트코드**: 검증기가 안전성을 증명한 코드만 실행기로 전달
3. **프로그램 훅 부착**: 네트워크·추적·보안 등 지정 지점에 연결
4. **커널 이벤트 실행**: 이벤트 발생 시 문맥과 Helper 제한 안에서 실행
5. **정책 조회·상태 기록**: Map에서 정책을 읽고 결과·통계를 갱신

> 요약: 적재 시 안전성을 검증하고 이벤트마다 훅에서 실행해 Map과 상태를 교환한다
...
|:---|:---|:---|:---|
| 적용 기준 | 빠른 동적 정책·관측 | 깊은 커널 기능 변경 | 강한 격리와 개발 편의 |
| 핵심 특징 | 검증 후 커널 훅에서 실행 | 커널 코드로 직접 적재 | 프로세스에서 시스템 API 사용 |
| 한계 | 검증·훅·Helper·권한 제약 | 결함의 커널 전체 영향 | 문맥 전환·데이터 복사 비용 |

> 요약: 실행 위치와 안전 경계 및 확장 깊이로 방식을 선택한다
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 커널 버전별 타입·기능 차이 | CO-RE·BTF·기능 탐지 적용 | 이식성과 배포 성공률 향상 |
| 고빈도 훅의 실행 오버헤드 | 명령·Map 접근 최소화와 벤치마크 | 커널 경로 지연 억제 |
| 적재 권한 탈취와 정책 변조 | 최소 권한·서명·핀 객체 접근 통제 | 커널 공격 표면 축소 |
...
- 커널 수정 없이 고성능 기능을 확장하기 위해 **검증기 제약·훅 문맥·Map 수명·커널 호환성·적재 권한·오버헤드**를 검토하고, 안전성이 입증된 eBPF 프로그램만 배포한다.
