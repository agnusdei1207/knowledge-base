...
- 정의/개념: 시간이나 환경 변화로 입력과 정답 사이의 조건부 관계 $P(Y\mid X)$가 달라지는 **개념 드리프트 현상**이다.
- 배경/필요성: 입력 분포 감시만으로 **행동•업무 규칙 변화** 탐지하기 어렵다.
...
- $P(Y\mid X)$ 변화와 **정답 피드백 의존**이 핵심이다.
- **부분집단 드리프트의 전체 평균 은폐**가 핵심이다.
- 라벨 정책 확인 후 **재학습 결정**이 핵심이다.
...
1. **시각•모델별 예측 기록**: 입력•버전•부분집단 정보를 저장한다.
2. **지연 정답 시각 정합**: 동일 사건의 확정 라벨과 예측을 결합한다.
3. **조건부 관계•부분집단 변화 탐지**: 오류•$P(Y\mid X)$ 변화를 판정한다.
4. **라벨 정책•모델 갱신 판정**: 재학습•재배포 여부를 선택한다.
...
- **라벨 정책** 변화면 정의를 수정하고 **부분집단 관계** 변화면 모델 재학습이 핵심이다.
