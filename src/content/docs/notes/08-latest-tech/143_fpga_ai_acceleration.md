...
- 정의/개념: FPGA의 재구성 논리•배선으로 AI 데이터 경로를 구현하는 **하드웨어 가속 방식**하는 특징을 가짐.
- 배경/필요성: 범용 가속기의 **맞춤 데이터경로•결정적 지연** 확보 한계가 발생하는 특징을 가짐.

#### 한줄 요약

- 소프트웨어 명령어로 연산을 지시하는 대신 논리 블록과 배선 자체를 재구성해 데이터가 연속 처리되는 공간 파이프라인을 구현하는 하드웨어 가속 방식이 핵심하는 특징을 가짐.
...
- **재구성 축**: 비트스트림 기반 논리•배선 변경하는 특징을 가짐.
- **실행 축**: 연산 단계 동시 배치의 공간 파이프라인하는 특징을 가짐.
- **설계 축**: 데이터경로 맞춤화로 지연은 감소하나 구현 복잡성 증가하는 특징을 가짐.

#### 한줄 요약

- FPGA는 비트스트림으로 데이터 경로를 재구성할 수 있지만 합성•배치•타이밍 검증에 개발 시간이 소요됨이 핵심하는 특징을 가짐.
...
- 구현 도구가 DSP 블록•BRAM•논리 블록을 연결하면 외부 입력이 정해진 순서로 흐르는 공간 파이프라인이 구성됨이 핵심하는 특징을 가짐.
...
- GPU는 소프트웨어 커널을 변경하고, FPGA는 비트스트림으로 회로를 재구성하며, ASIC은 제조 시 데이터 경로를 고정하는 것이 핵심하는 특징을 가짐.
...
- 보안 장비는 패킷 검사 단계를 공간 파이프라인으로 배치하고, 비전 장비는 반복 곱셈과 영상 데이터를 칩 내부에 배치하는 것이 핵심하는 특징을 가짐.
...
- **재구성성•구현 복잡성**으로 GPU와 FPGA을 선택하는 특징을 가짐.

#### 한줄 요약

- 데이터 경로를 재구성하면서 결정적 지연이 필요하면 FPGA, 모델이 자주 변하면 GPU, 연산이 장기 고정되면 ASIC이 적합함이 핵심하는 특징을 가짐. 체계 적용 및 원칙 준수
