...
- 비트스트림 기반 **논리·배선 재구성**
- 연산 단계의 동시 배치 기반 **공간 파이프라인**
- 맞춤 지연·효율과 **합성·타이밍 복잡성** 절충
...
- **1. 데이터경로·정밀도 전달**: LUT·DSP·BRAM 자원 배분
- **2. 합성·배치·타이밍 검증**: 논리 변환과 클록 제약 충족 판정
- **3. 비트스트림 전달**: 검증된 회로 구성의 장치 적재
- **4. 입력 스트림 전달**: 외부 데이터를 공간 파이프라인에 공급
- **5. 파이프라인 결과 전달**: 고정 단계 처리의 결정적 지연 출력
...
- **자원 예산·타이밍 여유**로 FPGA의 파이프라인·정밀도를 결정
