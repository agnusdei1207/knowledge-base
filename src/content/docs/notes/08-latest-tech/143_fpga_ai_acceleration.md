...
- 정의/개념: FPGA의 재구성 논리•배선으로 AI 데이터 경로를 구현하는 **하드웨어 가속 방식**이다.
- 배경/필요성: 범용 가속기의 **맞춤 데이터경로•결정적 지연** 확보 한계가 발생한다.
...
- **재구성 축**: 비트스트림 기반 논리•배선 변경한다.
- **실행 축**: 연산 단계 동시 배치의 공간 파이프라인이다.
- **설계 축**: 데이터경로 맞춤화로 지연은 감소하나 구현 복잡성 증가한다.
...
1. **합성•배치•타이밍 검증**: 논리 변환•클록 제약을 판정한다.
2. **타이밍 충족 설계 확정**: 자원•경로 제약을 만족한 설계를 확정한다.
3. **검증된 비트스트림 적재**: 회로 구성을 FPGA에 반영한다.
...
- **재구성성•구현 복잡성**으로 GPU와 FPGA을 선택한다.
