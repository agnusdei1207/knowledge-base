...
- 정의/개념: **양자화**는 가중치•활성값을 저비트 표현으로 변환하는 모델 압축 기법이다.
- 배경/필요성: 고정밀 연산의 **메모리 대역폭•전력 비용** 증가로 단말 배포 제약이 발생한다.

#### 한줄 요약
- **저비트 눈금**으로 가중치•활성값 저장•계산이 핵심이다.
...
- 연속 실숫값을 유한 단계로 매핑하는 **스케일•영점**이 핵심이다.
- 채널별 분포와 활성 이상치에 따른 **양자화 오차**가 핵심이다.
- 저비트 저장과 실제 **정수 커널 가속**의 분리 판단이 핵심이다.
...
1. **품질•자원 기준 설정**: 허용 정확도 손실과 **자원 목표**를 확정한다.
2. **대표 분포 측정**: 보정 표본으로 **동적 범위**를 산정한다.
3. **형식•매개변수 설계**: 대상•비트•**스케일•영점**을 선택한다.
4. **저비트 모델 변환**: 실숫값을 정수 코드와 복원 계수로 변환한다.
5. **장치 품질•자원 검증**: 정확도•지연•메모리의 **동시를 비교**한다.
#### 한줄 요약
...
- 재학습 제약은 **PTQ**, 정확도 회복 우선은 **QAT**가 핵심이다.
...
- 재학습 제약은 **PTQ**, 품질 민감 과업은 **QAT**가 핵심이다.
...
- 재학습 제약은 **PTQ•AWQ**, 정확도 민감 과업은 **QAT**을 선택한다.
