...
- 배경/필요성: 과업별 전용 모델은 학습한 **지식•문맥 표현의 범용 재사용**하기 어렵다.

#### 한줄 요약

- **자기지도 사전학습**으로 언어 이해•생성 과업 재사용이 핵심이다.
...
- 문맥 지시•예시를 활용한 **과업 적응**이 핵심이다.
- **확률적 생성**에 따른 출력 변동과 근거•편향 한계가 핵심이다.

#### 한줄 요약

- 다음 토큰 예측은 **최신성•사실성•공정성**을 미보장이 핵심이다.
...
| 트랜스포머 블록 | **어텐션•FFN 기반 문맥 표현 갱신** |
| 출력 계층 | 어휘별 **다음 토큰 확률** 산출 |

#### 한줄 요약

- **토큰화•임베딩•트랜스포머•출력 계층**으로 확률 산출이 핵심이다.
...
- **키-값 캐시(Key-Value Cache, KV Cache)**: 자기회귀 생성 중 이전 토큰의 어텐션 키와 값을 저장하여 중복 계산을 줄이는 저장소다.
...
1. **입력 문자열 토큰화**: 문자열을 모델 **어휘 단위**로 분할한다.
...
| 민감 데이터의 **모델 유출** | **입력 최소화•접근통제•출력 검사** | 비밀정보의 **무단 노출** 방지 |

#### 한줄 요약

- **권한별 RAG 근거**만 문맥에 넣어 최신성 보완이 핵심이다.
...
- 최신 지식에는 **RAG**, 고위험 업무에는 **검증형**, 비용 제약에는 **SLM**을 선택한다.
