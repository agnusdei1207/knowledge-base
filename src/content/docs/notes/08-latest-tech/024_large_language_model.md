...
- 정의/개념: 대규모 말뭉치에서 토큰 관계와 언어 패턴을 사전학습해 언어 이해·생성 과업을 수행하는 **대규모 언어 모델(Large Language Model, LLM)**
...
| 트랜스포머 블록 | **어텐션·피드포워드 신경망(Feed-Forward Network, FFN) 기반 문맥 표현 갱신** |
...
3. **어텐션·피드포워드 신경망(Feed-Forward Network, FFN) 연산**: 관련 토큰 결합과 위치별 표현 변환
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 학습 시점 이후 지식의 **최신성 부족** | **검색 증강 생성(Retrieval-Augmented Generation, RAG) 기반 최신·권한별 검색 근거 제공** | 답변의 **근거성·최신성** 보완 |
...
- **검증형 적용**: 오류 피해가 큰 업무에서 근거 확인·구조화 출력·사람 검토를 결합하여 대규모 언어 모델(Large Language Model, LLM) 결과를 사용하는 방식이다.
- **소형 언어 모델 라우팅(Small Language Model Routing, SLM Routing)**: 과업 난도와 비용 기준에 따라 일부 요청을 더 작은 모델로 보내 지연과 컴퓨팅 비용을 줄이는 방식이다.

</details>

- 최신 지식에는 **검색 증강 생성(Retrieval-Augmented Generation, RAG)**, 고위험 업무에는 **검증형**, 비용 제약에는 **소형 언어 모델(Small Language Model, SLM)** 선택
