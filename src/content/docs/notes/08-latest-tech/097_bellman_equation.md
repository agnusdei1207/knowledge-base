...
- 배경/필요성: 모든 미래 경로를 전개하는 **장기 가치 계산량** 축소 필요
...
> 붉은 $\gamma=0.9$ 잔차는 파란 $\gamma=0.5$보다 천천히 감소해 장기 보상을 더 오래 반영하며, $\gamma^k$로 계산한 이론값이지 실제 정책 성능은 아니다.

- **벨만 기대 방정식**은 주어진 정책의 행동 확률로 가치를 평균한다.
- **벨만 최적 방정식**은 가능한 행동 가치의 최댓값을 선택한다.
- **가치 고정점·반복 갱신**으로 방정식의 해를 구한다.
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 할인율 설정 민감 | 과업 시간 범위에 맞춰 **할인율** 검증 | 단기·장기 가치 균형 |
| 가치 반복 미수렴 | 갱신 잔차·**종료 임계값** 설정 | 고정점 판정 명확화 |
| 전이 모델 부정확 | 표본 기반 가치 추정과 **모델 오차** 비교 | 가치 편향 완화 |
...
- **정책 평가·최적화 목적**에 맞는 벨만 연산자를 선택하고 **할인율·수렴 잔차** 통제
