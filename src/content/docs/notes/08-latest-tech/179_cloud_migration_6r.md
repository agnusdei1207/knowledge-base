...
date: "2026-07-31T09:03:57+09:00"
tags:
  - "notes-latest-tech"
weight: 179
extra:
...
- 정의/개념: **클라우드 마이그레이션 6R(Cloud Migration 6R)** 은 업무 가치와 기술 조건에 따라 애플리케이션별 유지·폐기·이전·개선·재설계·교체 전략을 선택하는 분류 체계
...
- **Rehost·Replatform**: 애플리케이션 구조를 유지해 빠르게 이전하거나 일부 관리형 플랫폼 기능으로 개선하는 전략이다.
- **Refactor·Repurchase**: 애플리케이션 구조를 재설계하거나 상용·서비스형 소프트웨어 제품으로 교체하는 전략이다.
...
- **Retain·Retire**: 당장 이전하지 않고 현 상태를 유지하거나 가치가 없는 시스템을 종료하는 전략이다.
...
| 핵심 특징 | **현상 유지·시스템 종료** | **인프라 이관·관리형 플랫폼** | **구조 재설계·서비스형 소프트웨어(Software as a Service, SaaS) 교체** |
...
- **복구 시점 목표·복구 시간 목표(RPO·RTO)**: 장애 시 허용 가능한 데이터 손실 시점과 서비스 복구 목표 시간이다.

</details>

| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 누락된 **종속성**으로 전환 실패 | **트래픽·구성 관리 데이터베이스(Configuration Management Database, CMDB)·인터뷰 교차 검증** | **웨이브 장애** 감소 |
| **데이터 불일치·긴 중단** | **증분 복제·리허설·전환(Cutover) 기준** 설정 | **복구 시점 목표(Recovery Point Objective, RPO)·복구 시간 목표(Recovery Time Objective, RTO)** 준수 |
