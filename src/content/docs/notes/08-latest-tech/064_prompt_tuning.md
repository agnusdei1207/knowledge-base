...
- **프롬프트 튜닝(Prompt Tuning)**: 동결 모델의 입력 앞에 붙는 과업별 가상 토큰 벡터만 학습하는 PEFT 기법이다.
...
- **가상 토큰**: 실제 어휘 ID가 아니라 학습 가능한 임베딩으로 입력 앞에 결합되는 과업 표현이다.
...
- **Prompt Tuning**: 입력층에 학습 가능한 가상 토큰을 붙여 과업 행동을 조정한다.
- **Prefix Tuning**: 모델 각 층의 K·V 앞에 학습 가능한 접두어 벡터를 붙여 계층별 문맥을 제어한다.
...
- **적응 방식 선택**: 입력층 조정으로 충분하면 Prompt Tuning, 계층별 문맥 제어가 필요하면 Prefix Tuning을 적용한다.
