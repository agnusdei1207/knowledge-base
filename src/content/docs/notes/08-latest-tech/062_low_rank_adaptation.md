...
**전체 미세조정**, **어댑터(Adapter)**, **저랭크 적응(Low-Rank Adaptation, LoRA)** 은 각각 원본 전체, 직렬 모듈, 병렬 변화량을 학습한다.
...
| 대상 층 누락의 **과업 적응 실패** | **질의(Query, Q)•키(Key, K)•값(Value, V)•출력(Output, O) 투영**별 주입 제거 시험 | 유효 **주입 위치 식별** |
...
- 과업 변화량•학습 메모리•배포 경로를 기준으로 **저랭크 적응(Low-Rank Adaptation, LoRA) 랭크•대상 층•병합 방식** 결정
