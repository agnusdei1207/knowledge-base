...
- 정의: 동결 가중치에 저랭크 변화량(ΔW=BA)만 학습하여 모델을 최적화하는 기법.
- 배경: 전체 파라미터 튜닝 시 메모리 부하와 과업별 모델 배포 비용을 최소화하기 위함.
...
- W₀ 동결 및 BA 선택 갱신으로 학습 파라미터 수를 획기적으로 축소.
- 랭크 및 스케일 계수 조절을 통해 적응 용량 및 출력 반영 크기 최적화.
- 선형층별 병합 및 분리 배포를 통해 고품질 추론 경로 확보.
...
- 과업 변화량•학습 메모리•배포 경로로 LoRA 구성을 결정하여 실무 환경의 유연한 대응과 지속적 최적화 달성.
