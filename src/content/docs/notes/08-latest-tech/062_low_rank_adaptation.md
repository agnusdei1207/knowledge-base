---
sidebar:
  order: 62
  label: "062. LoRA (저랭크 적응)"
  badge:
    text: "기출 · 80%"
    variant: note
title: "LoRA (저랭크 적응)"
date: "2026-07-27T23:59:59+09:00"
tags:
  - "notes-latest_tech"
weight: 62
extra:
  question_no: "062"
  source_status: "기출"
  source_history: "131회, 132회, 135회"
  priority: 80
  priority_note: "저랭크 적응이 경량 튜닝의 대표 기법"
---

## 미리 알고가기

- **저랭크 적응(Low-Rank Adaptation, LoRA)**: 큰 가중치 행렬은 잠그고 두 작은 행렬이 만드는 변화량만 학습하는 PEFT 방식
- **매개변수 효율적 미세조정(Parameter-Efficient Fine-Tuning, PEFT)**: 기반 모델을 동결하고 소수 추가·선택 매개변수만 학습하는 기술군
- **저랭크 행렬(Low-Rank Matrix)**: 큰 행렬의 변화를 더 작은 중간 차원을 가진 두 행렬의 곱으로 표현한 행렬
- **랭크(Rank, $r$)**: `알`로 읽으며 두 작은 행렬 사이의 중간 차원과 변화량의 표현 용량을 나타내는 기호
- **스케일 계수($\alpha$)**: `알파`로 읽으며 LoRA 변화량이 원본 출력에 반영되는 크기를 조절하는 기호
- **동결 가중치($W_0$)**: `더블유 제로`로 읽으며 학습 중 값을 유지하는 기반 선형 변환 행렬
- **LoRA 변화량($BA$)**: `비 에이`로 읽으며 학습 행렬 $B$와 $A$의 곱으로 만든 가중치 수정분
- **쿼리·키·값·출력 투영(Query·Key·Value·Output Projection)**: 어텐션의 검색 기준·비교 대상·전달 정보·최종 결합을 만드는 선형층

## Ⅰ. 개요

- 정의/개념: **LoRA**는 동결 가중치에 더할 저랭크 변화량 $BA$만 학습하는 PEFT 기법
- 배경/필요성: 전체 가중치 갱신은 학습 상태 메모리와 **과업별 모델 복제 비용** 유발

### 쉽게 이해하기 (학습용)
- 큰 원본 행렬은 잠그고 두 작은 행렬을 곱한 수정분만 원본 출력에 더함

## Ⅱ. 특징

![LoRA 랭크별 학습 매개변수 증가](/study/diagrams/lora-rank-parameter-growth.svg)

> 랭크가 4에서 64로 16배 증가해도 LoRA 학습량은 선형 증가하며 전체 4096×4096 가중치보다 작고, 해당 차원 가정의 이론값이지 특정 모델 실측값은 아니다.

- $W_0$ 동결과 $A·B$ 선택 갱신에 따른 **학습 상태 축소**
- $r$과 $\alpha/r$에 의한 **적응 용량·반영 크기 조절**
- 대상 선형층과 병합 여부에 따른 **품질·추론 경로 결정**

### 쉽게 이해하기 (학습용)
- 수정분의 폭과 세기, 삽입 위치를 조절해 작은 학습량으로 원하는 변화를 만듦

## Ⅲ. 구조 및 구성요소

```mermaid
block-beta
    columns 5
    W["동결 선형층"]
    L["저랭크 학습 경로"]
    S["랭크·스케일 제어"]
    O["출력 결합기"]
    F["어댑터 저장소"]
```

| 구성요소 | 책임 |
|:---|:---|
| 동결 경로 $W_0$ | 기반 선형 변환과 **원본 능력 보존** |
| 저랭크 행렬 $A,B$ | $BA$를 통한 **가중치 변화량 생성** |
| 랭크·스케일 | $r$ 용량과 $\alpha/r$의 **반영 크기 조절** |
| 대상 모듈 | LoRA를 주입할 **선형층 범위 지정** |
| 어댑터 파일 | 학습 행렬·구성의 **분리 저장·병합** |

### 쉽게 이해하기 (학습용)
- 입력이 원본 경로와 작은 수정 경로를 함께 지나고 두 결과가 출력에서 합쳐짐

## Ⅳ. 흐름도

```mermaid
sequenceDiagram
    participant O as 업무 책임자
    participant C as LoRA 구성기
    participant M as 기반 모델
    participant T as 학습기
    participant R as 실행 관리자
    O->>C: 1. 품질·메모리 조건 정의
    C->>M: 2. 대상 선형층 선택
    M->>T: 3. 랭크·스케일·초기화 설정
    T->>R: 4. 원본 동결·A·B 갱신
    R-->>O: 5. 병합·동적 적재 검증
```

### 동작 원리

1. **품질·메모리 조건 정의**: 과업 정확도와 학습 가능한 **자원 상한** 설정
2. **대상 선형층 선택**: 변화가 필요한 투영 계층에 **LoRA 경로** 지정
3. **랭크·스케일·초기화 설정**: 표현 용량과 초기 출력 변화의 **학습 조건** 구성
4. **원본 동결·A·B 갱신**: $W_0$를 보존하고 $A,B$만 역전파로 **변화량 학습**
5. **병합·동적 적재 검증**: 배포 경로별 품질·지연과 **어댑터 전환 비용** 판정

### 쉽게 이해하기 (학습용)
- 수정할 층과 행렬 크기를 정해 작은 행렬만 학습하고 실행할 때 합칠지 따로 둘지 결정함

## Ⅴ. 종류 및 비교

| 비교 기준 | 전체 미세조정 | Adapter | LoRA |
|:---|:---|:---|:---|
| 적용 기준 | 전체 적응 필요 | 추가 층 연산 허용 | 선형층 변화량 적응 |
| 핵심 특징 | 원본 가중치 **전체 갱신** | 직렬 **소형 모듈 삽입** | 병렬 **저랭크 변화량** |
| 한계 | 학습·저장 비용 | 순차 추론 연산 | 랭크·대상 층 선택 필요 |

### 쉽게 이해하기 (학습용)
- 원본 전체, 새 중간층, 작은 수정 행렬 중 무엇을 학습하는지가 다름

## Ⅵ. 실무 고려사항 및 대책

| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 낮은 $r$의 **표현 용량 부족** | 랭크별 과업 품질·학습 메모리 비교 | 최소 유효 **랭크 선택** |
| 대상 층 누락의 **과업 적응 실패** | 쿼리·키·값·출력 투영별 주입 제거 시험 | 유효 **주입 위치 식별** |
| 병합 전후의 **출력 불일치** | 동일 정밀도에서 병합 수치·품질 회귀 시험 | 배포 경로의 **동등성 확보** |

### 쉽게 이해하기 (학습용)
- 작은 행렬이 너무 좁거나 잘못된 층에 붙으면 배우지 못하므로 크기와 위치를 함께 시험함

## Ⅶ. 결론

- 과업 변화량·학습 메모리·배포 경로를 기준으로 **LoRA 랭크·대상 층·병합 방식** 결정

### 쉽게 이해하기 (학습용)
- 수정분을 표현할 만큼의 행렬 폭과 효과가 큰 층, 실행에 맞는 병합 방식을 선택함
