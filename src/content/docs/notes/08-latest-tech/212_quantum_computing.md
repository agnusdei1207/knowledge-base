...
- **큐비트(Qubit)**: 중첩된 기저 상태의 확률 진폭과 위상으로 양자 정보를 표현하는 계산 단위
- **양자 회로 컴파일러**: 논리 게이트 회로를 장비의 큐비트 연결성•기본 게이트•오류 특성에 맞게 변환하는 소프트웨어
- **양자 처리장치(Quantum Processing Unit, QPU)**: 큐비트를 제어하고 양자 게이트와 측정을 물리적으로 실행하는 연산장치
- **고전 제어•검증부**: 양자 회로를 제어하고 반복 측정값을 취합해 통계 결과와 고전 기준선을 비교하는 구성요소
...
- **샷(Shot)**: 동일한 양자 회로를 한 번 실행하고 측정한 표본
...
2. **매핑•상태 준비**: 장비 연결성에 맞춰 회로를 변환하고 입력 인코딩
3. **양자 게이트 실행**: 중첩•얽힘•간섭을 만드는 회로 수행
4. **측정 표본 전달**: 반복 실행 결과로 고전 비트 분포 추정
5. **고전 기준선 전달**: 동일 정확도의 실행시간•자원비 비교 기준 제공

#### 한줄 요약

- 동일 정확도•전체 시간•자원비 기반 고전 기준선 비교
...
- 짧은 회로는 NISQ, 긴 알고리즘은 오류정정 양자컴퓨팅
...
- 기준선•노이즈•입출력 비용 기반 종단 간 계산 이득 검증
