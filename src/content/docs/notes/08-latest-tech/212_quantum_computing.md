...
| 고려사항 | 위험 | 대책 |
|:---|:---|:---|
| **불공정 기준선** | 약한 고전 알고리즘과 비교해 양자 이점 과장 | 최신 고전 알고리즘과 정확도·전체시간 동일 조건 비교 |
| **노이즈·샷 비용** | 회로 깊이와 반복 측정 증가로 신뢰성 저하 | 회로 축약, 오류 완화, 신뢰구간과 샷 예산 명시 |
| **입출력 병목** | 인코딩·후처리 비용이 계산 이점 상쇄 | 전체 파이프라인 계측과 양자 적합 하위 문제 분리 |
