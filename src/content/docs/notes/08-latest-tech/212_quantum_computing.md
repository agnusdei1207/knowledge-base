...
**양자컴퓨팅**은 큐비트의 중첩·얽힘·간섭을 양자 게이트로 조작하고 측정하여 계산 결과를 얻는 계산 방식이다.

</details>

- 정의/개념: **양자컴퓨팅**은 큐비트의 중첩·얽힘·간섭을 게이트와 측정으로 활용하는 계산
...
**중첩**은 큐비트가 여러 기저 상태의 확률 진폭을 동시에 표현하는 양자 상태다.
...
**양자 게이트**는 큐비트의 확률 진폭과 위상을 가역적으로 변환하는 연산이다.
...
**간섭**은 양자 상태의 진폭을 더하거나 상쇄하여 원하는 결과의 측정 확률을 높이는 현상이다.
...
**NISQ**는 오류가 있고 큐비트 수와 회로 깊이가 제한된 중간 규모 양자 장치다.
...
**디코히런스**는 환경과의 상호작용으로 큐비트의 중첩과 위상 정보가 손실되는 현상이다.
...
**양자 우위**는 특정 문제에서 양자 계산이 실용적으로 가능한 최선의 고전 계산보다 우수한 성능을 보이는 상태다.

</details>

- 전체 실행시간이 고전 기준선보다 유리한 문제에만 **양자컴퓨팅** 적용
