...
- 정의/개념: 클라우드 네이티브 관찰 가능성은 분산 환경의 텔레메트리와 실행 맥락을 연결하여 **내부 상태, 근본 원인, 사용자 영향**을 추론하는 능력
- 배경/필요성: 동적으로 변하는 서비스에서는 미리 정한 임계값 감시만으로 예상하지 못한 장애의 원인과 영향을 설명하기 어려우므로 다신호 상관분석 필요
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| **추적 맥락 단절** 미검증 시 서비스 간 신호를 연결하지 못해 원인 경로 유실 | 전파 규약을 통일하고 경계별 식별자 전달 검증 | **원인 경로 추적성** 확보 |
| **카디널리티 폭증** 미검증 시 저장·질의 비용 증가와 분석 성능 저하 | 속성 허용 목록, 카디널리티 예산, 보존 등급 적용 | **저장·질의 비용** 제한 |
| **희귀 오류 누락** 미검증 시 균일 샘플링이 중요한 저빈도 실패를 제거 | 오류 우선 및 꼬리 기반 샘플링과 원본 로그 연계 | 희귀 실패 **관측 누락률** 감소 |
