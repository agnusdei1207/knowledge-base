...
**AI 에이전트 시스템**은 목표를 작업으로 계획하고 도구를 실행하며 관찰 결과에 따라 행동을 조정하는 자율 AI 시스템이다.

</details>

- 정의/개념: **AI 에이전트 시스템**은 목표를 작업으로 계획하고 도구를 실행하며 관찰 결과에 따라 행동을 조정하는 자율 AI 시스템
...
**목표 지향성**은 최종 목표를 의존 관계와 완료 조건이 있는 하위 작업으로 분해하는 특성이다.

</details>

- **계획 축**: 목표 분해와 상태 유지로 장기 작업 단계 실행
- **실행 축**: **ReAct**의 행동·관찰 반복으로 오류 복구·재계획
...
**LLM**은 자연어 지시와 작업 문맥을 바탕으로 계획·응답·행동 후보를 생성하는 에이전트의 추론 모델이다.
...
| 구성요소 | 책임 |
|:---|:---|
| 모델·오케스트레이터 | **LLM·목표·정책·상태**로 다음 행동 선택 |
...
**ReAct**는 추론으로 행동을 선택하고 실행 결과를 관찰하여 다음 판단과 계획에 반영하는 반복 방식이다.
...
**반응형 에이전트**는 장기 계획보다 현재 입력과 규칙에 따라 즉시 행동을 선택하는 에이전트 유형이다.
...
| 핵심 특징 | **계획·행동·관찰** 반복 | **단발 대화 응답** 중심 | **검색 근거**로 생성 보강 |
| 한계 | **권한 오용·오류 누적** | **장기 상태·외부 행동 부족** | **검색 밖 행동 실행 불가** |

> 요약: **에이전트**는 행동, **챗봇·RAG**는 대화·근거 응답 중심
...
**HITL**은 고위험 행동을 실행하기 전에 사람이 맥락과 영향을 검토하고 승인하는 통제다.
...
| 금액·권리 변경의 **책임 불명확** | 변경·지급은 **HITL 승인** 후 실행 | 책임 있는 **자율화 경계** 확보 |
...
**최소 권한**은 에이전트가 각 작업에 필요한 도구와 데이터만 제한된 범위에서 사용하게 하는 보안 원칙이다.

</details>

- 다단계 외부 행동은 **AI 에이전트**, 고위험 변경은 **HITL 승인** 후 실행
