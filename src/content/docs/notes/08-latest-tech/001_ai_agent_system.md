...
- 배경/필요성: 단발 생성 모델로는 **장기 상태•외부 실행•오류 복구 관리할 수 없다**.
...
#### 한줄 요약
- **실행 관찰•상태 기억**으로 행동 조정과 작업 재개가 핵심이다.
...
5. **관찰 결과•완료 판정 저장**: 완료•재계획•한도 중단 결과를 상태에 반영한다.

#### 한줄 요약
- 행동 전 **권한•인자 검증**, 실행 후 완료•재계획을 판정한다.
...
> 요약: **AI Agent** 중심 외부 행동, **챗봇•RAG** 중심 대화•근거 응답이 핵심이다.
...
- 다단계 외부 행동은 **AI Agent**, 고위험 변경은 **HITL 승인** 후를 실행한다.

#### 한줄 요약
- **최소 권한•HITL** 경계와 실행 로그•비용 한도를 검증한다.
