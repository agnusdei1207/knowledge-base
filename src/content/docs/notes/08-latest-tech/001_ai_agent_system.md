...
date: "2026-07-31T11:50:23+09:00"
tags:
  - "notes-latest_tech"
weight: 1
extra:
...
- 정의/개념: 목표 계획·도구 실행·관찰 기반 행동 조정을 수행하는 **인공지능 에이전트 시스템(Artificial Intelligence Agent System, AI Agent System)**
...
- **실행 축**: **추론-행동(Reasoning and Acting, ReAct)** 반복 기반 오류 복구·재계획
...
- **계획·평가 모듈**: 목표를 실행 가능한 작업으로 분해하고 비용·의존성·완료 조건에 따라 다음 행동과 종료 여부를 판정하는 구성요소이다.
...
| 구성요소 | 책임 |
|:---|:---|
| 모델·오케스트레이터 | **대규모 언어 모델(Large Language Model, LLM)·목표·정책·상태** 기반 다음 행동 선택 |
| 상태·기억 | **데이터베이스(Database, DB) 작업 상태·결과·장기 지식** 저장 |
| 계획·평가 | **작업 분해·비용·종료 조건** 평가 |
| 도구·권한 | **응용 프로그래밍 인터페이스(Application Programming Interface, API) 스키마·권한 정책** 기반 실행 |
...
- **추론-행동(Reasoning and Acting, ReAct)**: 추론으로 행동을 선택하고 실행 결과를 관찰하여 다음 판단과 계획에 반영하는 반복 방식이다.
- **오케스트레이터(Orchestrator)**: 목표·상태·모델·도구를 연결하고 각 단계의 실행·재계획·종료를 조정하는 구성요소이다.
...
- **인공지능 에이전트(Artificial Intelligence Agent, AI Agent)**: 목표 달성을 위해 계획·행동·관찰을 반복하며 외부 도구를 실행하는 시스템이다.
...
> 요약: **인공지능 에이전트(Artificial Intelligence Agent, AI Agent)** 중심 외부 행동, **챗봇·검색 증강 생성(Retrieval-Augmented Generation, RAG)** 중심 대화·근거 응답
...
| 금액·권리 변경의 **책임 불명확** | 변경·지급 전 **인간 참여형 통제(Human-in-the-Loop, HITL) 승인** | 책임 있는 **자율화 경계** 확보 |
...
- **인간 참여형 통제(Human-in-the-Loop, HITL)**: 고위험 변경을 사람이 검토·승인한 뒤 실행하게 하는 책임 통제이다.

</details>

- 다단계 외부 행동은 **인공지능 에이전트(Artificial Intelligence Agent, AI Agent)**, 고위험 변경은 **인간 참여형 통제(Human-in-the-Loop, HITL) 승인** 후 실행
