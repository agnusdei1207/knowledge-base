...
- 정의/개념: 모델 버전의 용도•평가 증적•성능•한계•잔여 위험을 기록한 **모델 카드 문서**이다.
- 배경/필요성: 모델 파일•단일 성능값만으로는 평가 조건별 **적용 범위•실패 가능성** 전달하기 어렵다.
...
- 의도한 사용자•용도•금지 맥락의 **적용 경계 명시**가 핵심이다.
- 데이터•지표•집단별 결과의 **평가 조건 공개**가 핵심이다.
- 알려진 한계•잔여 위험•관리 책임의 **증적 연결**이 핵심이다.
...
1. **모델 버전•용도 확정**: 카드와 연결할 **모델•사용 경계**를 고정한다.
2. **평가 증적 생성**: 데이터•지표•집단별 검증 결과를 산출한다.
3. **모델 카드 초안 작성**: 성능•한계•잔여 위험을 구조화한다.
4. **주장•증적 검토•승인**: 카드 내용과 평가 결과 일치를 확인한다.
5. **모델•카드 동시 배포**: 승인 버전과 문서를 함께 게시한다.
...
- 버전•용도와 **평가 증적**이 일치할 때 승인하고 변경 시 **모델 카드** 재발행이 핵심이다.
