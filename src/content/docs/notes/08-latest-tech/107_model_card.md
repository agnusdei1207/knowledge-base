---
sidebar:
  order: 107
  label: "107. 모델 카드 (Model Card)"
  badge:
    text: "미출 · 60%"
    variant: note
title: "모델 카드 (Model Card)"
date: "2026-07-30T11:04:43+09:00"
tags:
  - "notes-latest_tech"
weight: 107
extra:
  question_no: "107"
  source_status: "미출"
  source_history: ""
  priority: 60
  priority_note: "모델 한계·용도 공개가 책임성 기반"
---

## 미리 알고가기

- **모델 카드(Model Card)**: 특정 모델 버전의 권장·금지 용도, 평가 조건, 성능, 한계를 사용자가 판단할 수 있게 구조화한 문서
- **데이터 카드(Data Card)**: 데이터셋의 출처·수집·구성·품질·허용 용도를 기록한 문서
- **시스템 카드(System Card)**: 모델을 포함한 전체 AI 서비스의 구조·상호작용·안전 평가·운영 통제를 기록한 문서
- **평가 증적(Evaluation Evidence)**: 모델 카드의 성능·한계 주장을 뒷받침하는 데이터·지표·시험 결과
- **잔여 위험(Residual Risk)**: 완화 조치를 적용한 뒤에도 모델 사용에 남아 있는 위험

## Ⅰ. 개요

- 정의/개념: **모델 카드**는 모델 버전의 용도·평가 증적·성능·한계·잔여 위험을 기록한 문서
- 배경/필요성: 모델 파일·단일 성능값만으로는 **적용 범위·평가 조건·실패 가능성** 전달 곤란

### 쉽게 이해하기 (학습용)
- 제품 설명서에 쓸 곳과 쓰지 말 곳, 시험 조건과 실패 사례를 같은 모델 버전과 묶어 제공함

## Ⅱ. 특징

- 의도한 사용자·용도·금지 맥락의 **적용 경계 명시**
- 데이터·지표·집단별 결과의 **평가 조건 공개**
- 알려진 한계·잔여 위험·관리 책임의 **증적 연결**

### 쉽게 이해하기 (학습용)
- 높은 점수만 적지 않고 어떤 사람과 환경에서 시험했으며 어디서 실패하는지 함께 적음

## Ⅲ. 구조 및 구성요소

```mermaid
block-beta
    columns 3
    I["모델 세부정보"]
    U["용도·사용자"]
    E["평가 설계·결과"]
    L["한계·잔여 위험"]
    G["관리·갱신 책임"]
    I --- U
    U --- E
    E --- L
    L --- G
```

| 구성요소 | 책임 |
|:---|:---|
| 모델 세부정보 | 이름·버전·구조의 **대상 모델 식별** |
| 용도·사용자 | 권장·금지 업무와 **사용자 범위 정의** |
| 평가 설계·결과 | 데이터·지표·환경별 **성능 증적 제공** |
| 한계·잔여 위험 | 실패 조건과 완화 후 **남은 위험 공개** |
| 관리·갱신 책임 | 승인·변경·재평가의 **문서 수명주기 관리** |

### 쉽게 이해하기 (학습용)
- 모델 이름표와 사용 범위, 시험 성적표, 실패 경고, 갱신 책임자를 한 문서에 연결함

## Ⅳ. 흐름도

```mermaid
sequenceDiagram
    participant O as 모델 책임자
    participant E as 평가팀
    participant C as 카드 작성기
    participant A as 승인자
    participant R as 배포 저장소
    O->>E: 1. 모델 버전·용도 확정
    E->>C: 2. 평가 조건·증적 수집
    C->>A: 3. 성능·한계·잔여 위험 작성
    A->>R: 4. 사실 검토·카드 승인
    R-->>O: 5. 모델 변경·카드 재발행
```

1. **모델 버전·용도 확정**: 카드와 연결할 **모델·사용 경계** 고정
2. **평가 조건·증적 수집**: 데이터·지표·집단별 **검증 결과** 확보
3. **성능·한계·잔여 위험 작성**: 결과와 실패 조건을 **사용 판단 정보**로 구조화
4. **사실 검토·카드 승인**: 주장과 증적의 일치 및 **배포 적합성** 판정
5. **모델 변경·카드 재발행**: 모델·데이터·환경 변화 시 **평가·문서 버전** 동기화

### 쉽게 이해하기 (학습용)
- 모델 버전을 고정해 시험하고 설명서를 승인한 뒤 모델이 바뀌면 성적표와 경고도 다시 발행함

## Ⅴ. 종류 및 비교

| 비교 기준 | 모델 카드 | 데이터 카드 | 시스템 카드 |
|:---|:---|:---|:---|
| 적용 기준 | 모델 사용 적합성 | 데이터 사용 적합성 | 전체 서비스 잔여 위험 |
| 핵심 특징 | 용도·성능·한계의 **모델 증적** | 출처·수집·품질의 **데이터 증적** | 구조·상호작용·통제의 **시스템 증적** |
| 한계 | 서비스 상호작용 미포함 | 모델 동작·운영 미포함 | 작성 범위·검증 비용 증가 |

### 쉽게 이해하기 (학습용)
- 모델 설명서, 데이터 설명서, 전체 서비스 설명서는 판단 대상의 크기가 다름

## Ⅵ. 실무 고려사항 및 대책

| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 모델과 카드의 **버전 불일치** | 모델 파일·카드 버전의 동시 배포 차단 | 잘못된 용도 판단 **방지** |
| 단일 평균값의 **집단별 실패 은폐** | 사용 집단·환경별 성능·오류 분해 | 적용 한계의 **가시성 확보** |
| 주장과 **평가 증적의 단절** | 카드 항목별 데이터·지표·시험 링크 연결 | 문서 신뢰성·감사성 **확보** |

### 쉽게 이해하기 (학습용)
- 모델과 설명서의 번호를 맞추고 평균 점수 뒤에 숨은 집단별 실패를 시험 기록과 연결함

## Ⅶ. 결론

- 모델 버전·용도 경계·평가 증적을 기준으로 **모델 카드 승인·재발행 조건** 결정

### 쉽게 이해하기 (학습용)
- 현재 모델에 맞는 시험 근거와 한계가 있어야 설명서를 승인하고 변경되면 다시 발행함
