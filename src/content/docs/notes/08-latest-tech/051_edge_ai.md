...
- 정의/개념: 데이터 발생 현장에 인접한 장치•게이트웨이에서 추론과 제한된 제어를 수행하는 **엣지 AI**이다.
- 배경/필요성: 중앙 클라우드의 **왕복 지연•통신 단절**로 현장 자율 제어 제약이 발생한다.

#### 한줄 요약

- **현장 인접 추론•제어**와 중앙 운영 관리 구조이다.
...
- **현장 인접 추론**을 통한 응답 지연•원시 데이터 전송 감소한다.
- 네트워크 단절 중 **로컬 자율 제어•지연 동기화**가 핵심이다.
- 중앙 제어면을 통한 **모델 버전•드리프트•장치 수명주기**를 통제한다.

#### 한줄 요약

- **로컬 자율 제어•지연 동기화**로 단절 대응이 핵심이다.
...
1. **입력 전처리**: 모델 입력 형식 변환과 불필요 데이터를 제거한다.
2. **로컬 추론 판정**: 지연•안전 임계조건에 따른 현장 결과를 확정한다.
3. **즉시 제어 수행**: 망 상태와 무관한 경보•차단•장치 동작을 실행한다.
4. **결과•상태 동기화**: 단절 구간 결과와 장치 상태의 연결 복구 후를 전송한다.
...
- 실시간•단절 제어는 **엣지**, 통합 분석•모델 관리는 **클라우드**를 배치한다.
