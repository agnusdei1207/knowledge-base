...
- 배경/필요성: 중앙 클라우드의 **왕복 지연•통신 단절**로 현장 자율 제어에 제약이 따른음.
...
- **중앙 제어면(Control Plane)**: 분산 장치의 신원•정책•모델 버전•운영 상태를 통합 관리하는 계층로 정의된다.
- **모델 드리프트(Model Drift)**: 운영 데이터의 특성이 변해 모델 예측 품질이 학습 시점보다 낮아지는 현상로 정의된다.

</details>

- **현장 인접 추론**으로 응답 지연과 원시 데이터 전송을 줄인음.
...
- **입력 전처리**: 원시 센서 데이터를 모델 입력 형식으로 변환하고 불필요한 정보를 제거하는 과정을 의미한다.
- **상태 동기화**: 현장 결과와 장치 상태를 중앙 제어면에 전달하여 운영 이력을 일치시키는 과정로 정의된다.
...
- **보안 부팅(Secure Boot)**: 서명된 신뢰 코드만 장치 시작 과정에서 실행하는 보호 기법로 정의된다.
- **무선 업데이트(Over-the-Air Update, OTA Update)**: 모델과 설정을 네트워크로 원격 배포•갱신하는 방식을 말한다.
