...
- **엣지 인공지능(Edge AI)**: 현장 인접 장치나 게이트웨이에서 AI 추론과 제어를 수행하는 분산 컴퓨팅 구조다.
- **현장 자율 제어**: 중앙 연결이 끊겨도 현장 장치가 로컬 판단으로 안전 동작을 계속하는 능력이다.

</details>

- 정의/개념: **엣지 AI**는 데이터 발생 현장에 인접한 장치·게이트웨이에서 AI 추론과 제한된 제어를 수행하는 분산 컴퓨팅 구조
...
- **온디바이스 AI**: 개인 단말이 독립적으로 로컬 추론을 수행한다.
- **Edge AI**: 현장 인접 게이트웨이가 다수 장치의 저지연 추론과 제어를 담당한다.
- **Cloud AI**: 중앙의 고성능 자원으로 대규모 통합 분석과 모델 관리를 수행한다.
...
- **무선 업데이트(OTA)**: 모델과 설정을 네트워크로 원격 배포·갱신하는 방식이다.
