...
- 배경/필요성: 중앙 클라우드의 **왕복 지연•통신 단절**로 현장 자율 제어에 제약이 따른다.
...
- **현장 인접 추론**으로 응답 지연과 원시 데이터 전송을 줄인다.
- 네트워크 단절 중 **로컬 자율 제어•지연 동기화**가 핵심이다.
- 중앙 제어면으로 **모델 버전•드리프트•장치 수명주기**를 통제한다.
...
4. **결과•상태 동기화**: 단절 구간 결과와 장치 상태를 연결 복구 후 전송한다.
