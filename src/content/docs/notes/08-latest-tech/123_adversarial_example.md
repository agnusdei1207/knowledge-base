...
- **정의**: 입력을 제한 범위에서 조작하여 모델의 오분류나 표적 출력을 유도하는 공격 표본
- **배경**: 무작위 잡음 평가로는 모델의 결정 경계 최악 방향 입력 교란을 탐지하는 데 한계 존재

#### 한줄 요약

- AI 모델 결정 경계의 취약점을 공략하는 최적화된 노이즈 조작 표본
...
- **핵심 요소**: 공격자의 목표와 접근 권한에 기반한 위협 모델 설정
- **기제**: 모델 손실 함수 기울기를 활용한 제한된 입력 공간 탐색 및 오분류 유도
- **성능 평가**: 위협 범위 내 정상 정확도와 적대적 강건성(Robust Accuracy) 비교

#### 한줄 요약

- 모델 손실 극대화 방향으로 정해진 예산 내 입력을 조작하는 기법
...
- **구성**: 공격 알고리즘은 목적함수와 제약 조건을 바탕으로 탐색을 수행하며, 강건성 평가기가 위협 범위 내 방어 성능을 측정
...
- 공격 경계 설정, 최적화 목표 정의, 강건 정확도 측정의 체계적인 공격 구조
...
| 약한 공격으로 방어 과대평가 | FGSM•PGD•C&W 비교 | 기울기 은폐 오판 완화 |
| 교란 예산의 의미 불일치 | 입력별 거리 규범 및 ε 명시 | 강건성 주장 범위 고정 |
| 공격 설정 비공개 | 파라미터 및 평가 조건 공개 | 공격 재현성 확보 |

#### 한줄 요약

- 위협 모델의 명확한 정의와 공격 조건부 강건성 평가를 통한 실무적 대응 방안
...
- 공격 지식, 교란 예산 및 계산 강도에 따른 적대적 강건성 평가 기반의 방어 모델 구축 필수

#### 한줄 요약

- 위협 모델의 명확한 정의와 공격 조건부 강건성 평가 기반의 보안 체계 적용
