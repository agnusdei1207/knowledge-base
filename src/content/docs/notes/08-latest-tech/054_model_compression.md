...
- **가지치기(Pruning)**: 기여도가 낮은 가중치·채널·계층을 제거하는 구조 압축 기법
- **양자화(Quantization)**: 가중치와 활성값을 더 적은 비트로 표현하는 정밀도 압축 기법
- **지식 증류(Knowledge Distillation)**: 큰 교사 모델의 출력 분포를 작은 학생 모델이 학습하는 압축 기법
- **보정 데이터(Calibration Data)**: 양자화 범위나 압축 후 품질을 조정하도록 운영 입력을 대표해 제공하는 데이터
- **실행 커널(Execution Kernel)**: 목표 프로세서에서 특정 연산·자료형·희소 패턴을 실제 수행하는 저수준 코드
- **폴백(Fallback)**: 목표 장치가 지원하지 않는 연산을 다른 프로세서에서 대신 실행하는 처리
- **역양자화(Dequantization)**: 정수 등의 낮은 정밀도 값을 연산에 필요한 높은 정밀도 표현으로 복원하는 변환
- **종단 지연(End-to-End Latency)**: 입력 전처리부터 모델 실행·후처리까지 전체 요청에 걸린 시간
- **프로파일러(Profiler)**: 모델 실행 구간별 시간·메모리·연산 사용량을 측정해 병목을 찾는 도구
- **회귀 시험(Regression Test)**: 압축 전후 동일 입력의 품질을 비교해 기존 과업의 성능 저하를 확인하는 시험

## Ⅰ. 개요

- 정의/개념: **구조·정밀도·학습 지식**을 축소해 품질 한도 안에서 모델 자원 사용을 줄이는 배포 최적화 기술
- 배경/필요성: 원본 모델은 단말의 **메모리·응답 지연·전력 예산**을 초과할 수 있어 목표 장치 기반 압축 필요
...
| **압축 변환기** | 가지치기·양자화·학생 모델 변환 |
| **보정·재학습기** | 변환 오차와 품질 손실 회복 |
...
| **핵심 특징** | 가중치·채널·계층 제거 | 가중치·활성값 표현 비트 축소 | 교사 출력 분포의 학생 모델 이전 |
...
| 커널 미지원·폴백으로 **실측 지연** 미감소 | 장치별 **연산자·자료형·희소 패턴** 확인 | 명목 압축의 **실행 가속** 전환 |
...
- **목표 품질·장치 병목·커널 지원**을 기준으로 압축 기법과 적용 순서를 결정하는 배포 최적화
