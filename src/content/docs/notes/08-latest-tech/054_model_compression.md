...
date: "2026-08-04T15:54:00+09:00"
tags:
  - "notes-latest_tech"
weight: 54
extra:
...
```mermaid
sequenceDiagram
    participant B as 기준선 관리기
    participant P as 병목 프로파일러
    participant C as 압축 변환기
    participant T as 품질 복구기
    participant D as 배포 검증기
    B->>P: 1. 기준 성능 전달
    P->>C: 2. 배포 병목 전달
    C->>T: 3. 압축 모델 전달
    T->>D: 4. 보정 모델 전달
...
1. **기준 성능 전달**: 원본 모델의 품질•크기•지연•전력 제공
2. **배포 병목 전달**: 목표 장치의 메모리•연산•전송 제한 제공
3. **압축 모델 전달**: 병목에 맞는 가지치기•양자화•증류 적용
4. **보정 모델 전달**: 보정 데이터와 재학습으로 변환 오차 회복
5. **목표 장치 검증 결과**: 품질•종단 지연•자원 사용 비교
