...
- 정의/개념: **모델 압축**은 구조•정밀도•학습 지식을 줄여 품질 한도 안에서 배포 자원을 절감하는 기술이다.
- 배경/필요성: 원본 모델의 **메모리•응답 지연•전력 사용량**이 단말 예산을 초과가 발생한다.

#### 한줄 요약

- **품질 한도** 안에서 배포 자원을 줄이는 최적화한다.
...
- **가지치기•양자화•지식 증류**를 통한 구조•표현•학습 지식을 축소한다.
- 압축 변환 뒤 보정•재학습을 통한 **목표 품질 회복**이 핵심이다.
- 파일 크기•연산량과 실제 장치의 **커널 가속•종단 지연** 공동을 검증한다.
...
1. **기준 성능 측정**: 원본 모델의 품질•크기•지연•전력을 기록한다.
2. **병목 기반 기법 선택**: 메모리•연산•전송 제한에 맞는 압축을 결정한다.
3. **모델 압축 변환**: 가지치기•양자화•지식 증류를 적용한다.
4. **보정•재학습**: 보정 데이터로 변환 오차와 품질 손실을 회복한다.
5. **목표 장치 회귀 검증**: 압축 전후 품질•종단 지연•자원을 비교한다.
...
- 메모리•연산 병목은 **양자화•가지치기**, 소형화는 **지식 증류**가 핵심이다.
