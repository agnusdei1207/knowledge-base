...
선의 의미: 기준•병목•변환•복구•검증 책임의 정적 연결
...
1. **기준 성능 측정**: 원본 모델의 품질•크기•지연•전력 기록
2. **병목 기반 기법 선택**: 메모리•연산•전송 제한에 맞는 압축 결정
3. **모델 압축 변환**: 가지치기•양자화•지식 증류 적용
4. **보정•재학습**: 보정 데이터로 변환 오차와 품질 손실 회복
5. **목표 장치 회귀 검증**: 압축 전후 품질•종단 지연•자원 비교
