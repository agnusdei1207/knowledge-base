...
- 정의/개념: 기준값과 개별 예측의 차이를 특징별 샤플리 값으로 분해하는 **SHAP 기법**이다.
...
- 모든 특징 순서의 한계 기여를 평균한 **샤플리 공정 배분**이 핵심이다.
- `예측값 = 기준값 + 기여도 합`의 **가산 완전성**이 핵심이다.
- 배경 분포•상관 특징 가정에 따른 **설명 민감성**이 핵심이다.
...
- **한계 기여**: 특징 하나를 연합에 추가하기 전과 후의 모델 출력 차이다.
...
1. **설명 대상•배경 설정**: 기준 출력과 마스킹 분포를 결정한다.
2. **특징 연합 구성**: 특징의 포함•제외 부분집합을 생성한다.
3. **연합별 모델 출력 계산**: 각 특징 조합의 예측값을 산출한다.
4. **특징별 한계 기여 계산**: 특징 추가 전후 출력 차이를 측정한다.
5. **샤플리 값 집계•가산 검증**: 가중 평균과 예측 차이 합산을 확인한다.
...
- 모델 구조•배경 대표성에 맞는 **SHAP** 계산기를 선택하고 해석 범위를 제한한다.
