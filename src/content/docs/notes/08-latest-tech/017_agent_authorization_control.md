...
1. 메일 에이전트에 내부 발송 권한만 부여하고, **수신 도메인 정책**으로 외부 발송을 차단하며 예외 전송은 사용자 승인을 요구한다.

### 쉽게 이해하기 (학습용)
- 송신 권한과 수신처 도메인을 분리해 의도치 않은 전송 차단

## Ⅶ. 결론
- 자율 실행의 권한 남용과 피해 확산을 방지하기 위해 주체·자원·행동별 최소 권한과 위임 기간·금액 한도·인간 승인 지점을 검토하여 **에이전트 권한 제어**를 적용해야 한다.
