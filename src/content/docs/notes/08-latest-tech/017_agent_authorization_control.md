...
- **인가(Authorization)**: 인증된 주체가 특정 자원에 어떤 행동을할 수 있는지 결정하는 절차다.
- **최소 권한(Least Privilege)**: 업무 수행에 필요한 최소 범위와 시간으로 권한을 제한하여 부여하는 원칙이다.

</details>

- 정의/개념: 위임 범위의 자원•도구 행동을 제한하는 **에이전트 권한 제어**이다.
- 배경/필요성: 광범위한 고정 권한은 **의도 밖 행동•권한 남용•피해 확산**을 유발한다.

#### 한줄 요약
- 작업별 **최소 권한•기간•승인 경계** 부여가 핵심이다.
...
#### 한줄 요약
- **열람•변경 권한 분리**와 고위험 행동 재확인이 핵심이다.
...
3. **허용•거부 결정**: 호출 직전 최소 권한을 강제한다.
4. **승인된 API 행동 실행**: 허용 범위의 도구만을 호출한다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 장기 토큰에 따른 **권한 잔존** | 작업별 **단기 자격증명•자동 만료** 적용 | 위임 종료 후 **접근 차단** |
| 도구 조합에 따른 **권한 상승** | **전체 행동 경로**를 정책으로 평가 | 연쇄 호출의 **우회 실행** 방지 |
| 고위험 행동의 **자동 실행** | 금액•대상•외부 전송별 **인간 승인** 요구 | 되돌리기 어려운 **부작용 통제** |
| 정책 결정의 **책임 공백** | **주체•정책 버전•결정•결과** 연결 기록 | 권한 사용의 **추적성** 확보 |

#### 한줄 요약
- **송신 권한•수신 도메인**을 분리해 무단 전송 차단이 핵심이다.
...
- 안정적 직무 권한에는 **RBAC**, 동적 맥락•도구 조합에는 **ABAC**을 선택한다.

#### 한줄 요약
- 작업별 **단기 최소 권한**과 고위험 HITL을 적용한다.
