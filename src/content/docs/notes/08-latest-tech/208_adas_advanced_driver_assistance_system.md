...
1. **환경•차량 상태 전달**: 센서 건강•가시성•ODD와 주변 데이터를 수집함.
2. **객체•차선 추정 전달**: 다중 센서 기반 객체•차선•자차 상태를 융합함.
3. **위험•목표 제어 전달**: 충돌 가능성과 목표 궤적에 따른 경고•제한을 제어함.
4. **기능•운전자 상태 전달**: HMI 상태•제어 결과와 운전자 주의•응답을 감시함.
5. **지속•저하•해제 결정 전달**: 한계•고장•미응답 시 기능 저하•해제•인수를 요청함.
...
- 자동 제어 범위보다 운전자가 계속 감독해야 한다는 책임 경계를 먼저 이해해야  체계 적용
