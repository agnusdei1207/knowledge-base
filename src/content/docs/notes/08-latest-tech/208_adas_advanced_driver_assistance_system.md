...
- 정의/개념: 환경•운전자 상태를 인지해 경고•조향•가감속을 보조하는 **ADAS**이다.
- 배경/필요성: 운전자 오류 보조 기능의 확대는 **기능 한계•감독 책임 혼동** 위험이 발생한다.
...
- 다중 센서 융합 기반 **객체•차선•충돌 위험 인지**가 핵심이다.
- SAE Level 0~2의 **AEB•ACC 종방향, LKA 횡방향 보조**가 핵심이다.
- DMS•센서 감시 기반 **운전자 감독•기능 저하 전환**이 핵심이다.
#### 한줄 요약
...
1. **환경•차량 상태 전달**: 센서 건강•가시성•ODD와 주변 데이터를 수집한다.
2. **객체•차선 추정 전달**: 다중 센서 기반 객체•차선•자차 상태를 융합한다.
3. **위험•목표 제어 전달**: 충돌 가능성과 목표 궤적에 따른 경고•제한을 제어한다.
4. **기능•운전자 상태 전달**: HMI 상태•제어 결과와 운전자 주의•응답을 감시한다.
5. **지속•저하•해제 결정 전달**: 한계•고장•미응답 시 기능 저하•해제•인수를 요청한다.
...
- ODD•센서 한계 초과 시 **기능 저하•해제**, **운전자 감독 책임**을 유지한다.
