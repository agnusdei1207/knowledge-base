...
date: "2026-07-30T15:06:00+09:00"
tags:
  - "notes-latest_tech"
weight: 6
extra:
...
- 초기화 단계에서 클라이언트와 서버가 프로토콜 버전과 선택적 기능을 협상
- 도구·리소스·프롬프트의 목록 조회와 사용을 JSON-RPC 요청·응답·알림으로 표준화
- 호스트는 사용자 동의·보안 정책을, 클라이언트는 서버별 세션을, 서버는 전문 기능을 담당
...
| 절차 | 설명 |
|:---|:---|
| 지원 버전·클라이언트 기능 초기화 요청 | 호환 버전과 roots·sampling 등 지원 범위 제안 |
| 합의 버전·서버 기능 응답 | 사용할 버전과 tools·resources·prompts 등 기능 확정 |
| 초기화 완료 알림 | 협상된 기능으로 정상 메시지 교환이 가능함을 통지 |
| 도구·리소스·프롬프트 목록 요청 | 필요한 서버 기능을 발견하고 선택 |
| 기능 결과·표준 오류 반환 | 협상된 기능만 실행해 결과 또는 JSON-RPC 오류 제공 |
