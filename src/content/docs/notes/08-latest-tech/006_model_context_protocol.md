...
- **무상태 코어(Stateless Core)**: 초기화 핸드셰이크와 프로토콜 세션 없이 각 요청이 필요한 메타데이터를 전달하는 구조이다.
- **캐시 힌트(Cache Hint)**: 목록•리소스 응답의 유지 시간과 공유 범위를 알려 재조회 비용을 줄이는 정보.
- **버전•클라이언트 메타 포함 요청(Request with Meta)**: 요청마다 프로토콜 버전•식별•기능을 전달하는 단계.
- **서버 기능 발견 요청(Server Discover Request)**: 필요할 때 서버의 기능 목록과 확장을 조회하는 단계.
- **서버 기능 발견 결과 제공(Server Discover Result)**: 사용 가능한 서버 기능과 확장을 클라이언트에 반환하는 단계.
- **원시 기능 목록•선택 기능 호출 요청(Call Request)**: 목록을 조회한 뒤 도구 호출•리소스 읽기를 수행하는 단계.
