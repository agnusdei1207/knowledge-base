...
date: "2026-08-06T20:21:00+09:00"
tags:
  - "notes-latest_tech"
weight: 6
extra:
...
- 정의/개념: 호스트•서버의 기능 발견과 무상태 메시지를 표준화한 **MCP**
- 배경/필요성: 맞춤 연동으로 **기능 발견•메시지 처리 중복과 호환성 저하** 발생

#### 한줄 요약
- **공통 메시지•기능 발견**으로 서버별 중복 연동 축소
...
- **무상태 축**: 요청별 버전•식별•기능을 담는 자기기술 메시지
- **메시지 축**: **JSON-RPC** 기반 도구•리소스•프롬프트 교환
- **책임 축**: 호스트•클라이언트•서버의 정책•연결•기능 분리

#### 한줄 요약
- **무상태 요청•기능 발견**으로 서버 연동 방식 통일
...
1. **버전•클라이언트 메타 포함 요청**: 요청마다 프로토콜 버전•식별•기능 전달
2. **서버 기능 발견 요청**: 필요하면 server/discover로 기능 조회
...
#### 한줄 요약
- 기능 발견•컨텍스트 교환은 **MCP**, 고정 계약은 **OpenAPI**
...
#### 한줄 요약
- 서버별 **최소 권한•허용 목록**으로 기능 공개 범위 제한
