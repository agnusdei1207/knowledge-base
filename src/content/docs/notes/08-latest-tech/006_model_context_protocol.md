...
| 구성요소 | 책임 |
|:---|:---|
| MCP 호스트 | **MCP Host** 사용자 동의•정책•복수 서버 연결을 관리함 |
| MCP 클라이언트 | **MCP Client** 서버별 연결•메시지 교환을 관리함 |
| MCP 서버 | **MCP Server** 도구•리소스•프롬프트를 제공함 |
| JSON-RPC•전송 계층 | **JSON-RPC** 활용 **stdio** 또는 **Streamable HTTP** 기반 전달함 |
...
|:---|:---|:---|:---|
| 적용 기준 | **MCP** 기반 모델의 도구•컨텍스트 선택이 가능함 | **OpenAPI** 기반 고정 HTTP 계약임 | 맞춤형 API는 단일 시스템 연동임 |
...
| 원격 전송의 인증 토큰 탈취•오용 | **TLS** 및 토큰 대상•범위•재사용 탐지를 적용함 | 비인가 요청과 권한 오용을 방지함 |
