...
- 정의/개념: 여러 사고 상태를 탐색 트리로 분기하고 후보를 평가•가지치기해 해법을 찾는 **ToT**이다.
- 배경/필요성: 단일 경로는 **초기 선택 오류•대안 누락에서 회복하기 어렵다**.

#### 한줄 요약

- **분기•평가•가지치기**로 대안 해법 탐색이 핵심이다.
...
- 복수 사고 상태의 **분기•평가•가지치기**가 핵심이다.
- **BFS**: 같은 깊이의 후보를 우선 탐색이다.
- **DFS**: 한 후보 경로를 깊게 탐색이다.
- **빔 탐색**: 점수가 높은 후보를 정해진 폭만큼을 유지한다.
- **백트래킹**: 실패 시 저장한 이전 분기로 복귀이다.

#### 한줄 요약

- **BFS•DFS•빔•백트래킹**으로 탐색 범위 제어가 핵심이다.
...
1. **초기 상태 정의**: 목표•제약•**종료 조건**을 고정한다.
2. **후보 상태 확장**: 현재 상태별 **다음 사고**를 생성한다.
3. **후보 가치 평가**: 가치와 **제약 위반**을 점수화한다.
4. **가지치기•경로 선택**: 탐색 방식•예산으로 **유지 집합**을 결정한다.
5. **완성 답 독립 검증**: 탐색 점수와 **정답 판정**을 분리한다.

#### 한줄 요약

- **확장•평가•가지치기 반복** 후 완성 답 독립을 검증한다.
...
- 단일 경로는 **CoT**, 대안 비교•오류 회복은 **ToT**가 핵심이다.
...
- **대안 가치•평가기 신뢰도**가 충분하면 ToT를 **빔 폭•깊이**로 통제한다.
