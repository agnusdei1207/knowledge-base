...
- 정의/개념: 여러 사고 상태를 탐색 트리로 분기하고 후보를 평가·가지치기해 해법을 찾는 **사고 트리(Tree of Thoughts, ToT)**
...
- **너비 우선 탐색(Breadth-First Search, BFS)·깊이 우선 탐색(Depth-First Search, DFS)**: 동일 깊이 우선 또는 단일 경로 우선 탐색
- **빔 탐색·백트래킹**: 상위 후보 유지와 실패 분기 복귀
- 너비 우선 탐색(BFS)·깊이 우선 탐색(DFS)·빔에 따른 **탐색 폭·깊이 제어**
...
| **탐색 제어기** | **너비 우선 탐색(Breadth-First Search, BFS)·깊이 우선 탐색(Depth-First Search, DFS)·빔 탐색 예산 관리** |
...
- **사고 트리**: 여러 경로를 분기·평가하고 실패 시 다른 대안으로 돌아가 오류를 회복하는 방식이다.
...
- **사고 트리 적용 기준(Tree of Thoughts Application Criteria, ToT Application Criteria)**: 여러 대안의 비교와 초기 오류 회복 이득이 추가 탐색 비용보다 클 때 사용한다.
- **빔 폭·깊이**: 각 단계에서 유지할 후보 수와 최대 탐색 단계를 제한하여 비용을 통제하는 값이다.

</details>

- **대안 가치·평가기 신뢰도**가 충분하면 사고 트리(Tree of Thoughts, ToT)를 적용하고 **빔 폭·깊이**로 통제
