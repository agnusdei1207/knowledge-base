...
- 정의/개념: **NC**는 인간 뇌의 신경망 구조를 모방해 **SNN**으로 뉴런•시냅스 상태를 갱신하는 비폰노이만형 컴퓨팅이다.
- 배경/필요성: 밀집 연산의 **희소 센서 이벤트•초저전력 처리** 한계가 발생한다.
...
- **활성 축**: 이벤트 발생 뉴런만 비동기 연산이다.
- **배치 축**: 시냅스 인접 저장•스파이크 라우팅이다.
- **한계 축**: **ANN-SNN 변환 오차**와 이벤트 혼잡이다.
...
1. **목적 뉴런•시냅스 주소**: 연결 대상에 이벤트를 전달한다.
2. **연결 가중치•현재 상태**: 입력 누적•막전위를 갱신한다.
3. **임계 초과 출력 스파이크**: 발화 뉴런만 후속을 전파한다.
...
- 희소 이벤트•초저전력은 **NC**, 범용•저희소성은 **GPU**을 선택한다.
