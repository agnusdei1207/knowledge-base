...
date: "2026-08-04T14:20:24+09:00"
tags:
  - "notes-latest_tech"
weight: 117
extra:
...
```mermaid
sequenceDiagram
    participant A as 공격자•외부 콘텐츠
    participant U as 사용자
    participant R as RAG•요약기
    participant M as 모델
    participant T as 연결 도구•보호 자산
    A->>A: 1. 외부 콘텐츠에 공격 지시 은닉
    U->>R: 정상 질의 입력
    R->>A: 2. 오염 콘텐츠 검색
    A-->>R: 숨은 지시 포함 문서
    R->>M: 3. 비신뢰 컨텍스트 결합
...
1. **외부 콘텐츠에 공격 지시 은닉**: 문서•웹•메일에 명령 삽입
...
#### 한줄 요약
- 외부 자료의 숨은 지시가 검색을 거쳐 모델과 도구 행동을 조종함
