...
- **간접 프롬프트 인젝션**: 외부 문서·웹·메일에 숨긴 지시가 모델 답변이나 도구 행동을 바꾸는 공격이다.
- **데이터·명령 혼동**: 참고 자료의 문장을 비신뢰 데이터가 아니라 실행할 지시로 해석하는 현상이다.

</details>

- 정의/개념: **간접 프롬프트 인젝션(Indirect Prompt Injection)** 은 외부 콘텐츠의 숨은 지시로 모델·도구 행동을 조종하는 공격
...
- **신뢰 구역**: 외부 콘텐츠를 상위 지시와 분리해 비신뢰 데이터로 조립하는 문맥 영역이다.
...
| 반출 검사 | **응답·응용 프로그래밍 인터페이스(Application Programming Interface, API) 호출** 의 민감정보·목적지 검사 |
...
**검색 증강 생성(Retrieval-Augmented Generation, RAG)·요약기** 는 외부 콘텐츠를 비신뢰 데이터 구역에 넣고 행동 계획을 별도 정책 계층으로 전달한다.
...
| 검색 문서가 **내부 응용 프로그래밍 인터페이스(Application Programming Interface, API) 호출** 유도 | 사용자 목적 대조·도구 게이트 적용 | **검색 증강 생성(Retrieval-Augmented Generation, RAG) 경유 도구 권한 오용** 제한 |
...
- **비신뢰 구역**: 외부 자료를 실행 지시와 분리해 데이터로만 취급하도록 만든 문맥 경계이다.
