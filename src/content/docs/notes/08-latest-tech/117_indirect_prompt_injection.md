...
- 정의/개념: **간접 프롬프트 인젝션**은 외부 콘텐츠의 숨은 지시로 모델•도구 행동을 조종하는 공격이다.
- 배경/필요성: 검색•요약 문맥의 **데이터•명령 혼동**으로 비인가 행동이 발생한다.
...
- 외부 콘텐츠를 매개로 한 **비신뢰 지시 유입**이 핵심이다.
- 검색•요약 자동화와 도구 권한 결합의 **피해 범위 확대**가 핵심이다.
- 출처 분리•독립 권한 검증 기반 **행동•반출 통제**가 핵심이다.
...
1. **외부 콘텐츠에 공격 지시 은닉**: 문서•웹•메일에 명령을 삽입한다.
2. **오염 콘텐츠 검색**: 정상 질의가 공격 문서를 근거로 선택한다.
3. **비신뢰 컨텍스트 결합**: 숨은 지시가 모델 문맥에 유입한다.
4. **데이터•명령 혼동**: 참고 문장을 상위 실행 지시로 승격한다.
5. **비인가 행동•정보 반출**: 도구 호출로 유출•변조 피해 발생한다.
...
- 외부 출처는 **비신뢰 구역**에 격리하고 목적 불일치 행동은 **도구 게이트**로 차단이 핵심이다.
