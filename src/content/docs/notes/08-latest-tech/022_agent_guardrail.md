...
- 기존 한계: 비신뢰 데이터의 악성 지시와 과도한 도구 권한은 모델의 잘못된 판단을 실제 시스템 피해로 확산시킬 수 있다.
...
1. 메일 에이전트가 외부로 메시지를 보내기 전에 수신자·첨부 파일·민감정보를 검사하고, 고위험 전송은 담당자 승인 후 실행한다.
...
- 에이전트의 자율 실행으로 인한 피해를 방지하기 위해 행동 위험도와 우회·오탐 가능성을 검토하여 최소 권한·차단·인간 승인 정책을 다층적으로 적용해야 한다.
