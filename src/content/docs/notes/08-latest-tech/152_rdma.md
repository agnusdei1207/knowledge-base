...
date: "2026-09-06T08:02:04+09:00"
tags:
  - "notes-latest-tech"
weight: 152
extra:
...
- 커널과 CPU를 데이터 경로에서 완전히 제거하여 초저지연·제로카피 원격 메모리 통신을 실현하는 **고성능 컴퓨팅 및 AI 분산 학습의 핵심 통신 기반 기술(RDMA / InfiniBand Verbs / RoCEv2 / iWARP / Memory Registration / Queue Pair / Completion Queue / One-sided Read/Write)**의 필수 표준으로 확고히 자리 잡았으며, CXL 메모리 공유와의 융합 및 GPUDirect RDMA 기반 GPU-to-GPU 직접 전송으로 발전하는 가운데, 실무 RDMA 도입 시에는 **원격 직접 조작이 필요한 All-Reduce 집합통신에는 단측 RDMA(Read/Write)를 적용하고 메시지 알림이 필요한 RPC 패턴에는 양측 송수신(Send/Recv)을 선택하며, 메모리 등록 영역의 L_Key/R_Key 기반 접근 제어와 QP 상태 머신 관리를 결합**하여 완벽한 제로카피 초저지연 통신과 보안 통제를 완성
