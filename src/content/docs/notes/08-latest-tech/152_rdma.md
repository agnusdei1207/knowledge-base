...
#### 한줄 요약
- 운영체제 커널 개입을 배제하고 등록된 메모리를 네트워크 어댑터가 직접 읽고 쓰는 고속 전송 기술 구현.
...
#### 한줄 요약
- 보호 도메인, 등록 메모리, 작업/완료 큐 기반의 고속 직접 전송 및 동기화 체계.
...
#### 한줄 요약
- 보호 영역 기반 자원 격리, 등록 메모리 제어, 작업/완료 큐 기반의 RDMA 제어 구조 구현.
...
#### 한줄 요약
- 메모리 등록 후 연결 정보를 교환하고, 네트워크 어댑터가 직접 메모리 데이터를 전송하며 비동기적으로 결과를 처리하는 흐름.
...
#### 한줄 요약
- 원격 직접 접근(단측), 메시지 교환(양측), 범용 통신(소켓) 간의 메모리 접근 권한 및 CPU 개입 여부에 따른 차이.
...
#### 한줄 요약
- 원격 접근 권한 최소화, 큐 페어 상태 관리, 완료 큐 미처리 방지를 위한 작업 폴링 및 상태 관리 체계.
...
#### 한줄 요약
- **메모리 접근 범위, 보안 키, 작업/완료 큐 기반 RDMA 통신 아키텍처 적용.**
