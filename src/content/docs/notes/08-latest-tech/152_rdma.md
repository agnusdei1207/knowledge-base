...
- 정의/개념: 등록된 로컬•원격 메모리를 어댑터가 직접 읽고 쓰는 **RDMA 기술**이다.
...
- **전송 축**: 원격 CPU 미개입 읽기•쓰기•원자 연산이다.
- **보호 축**: MR 등록과 L_Key•R_Key로 주소•권한 고정이다.
- **상태 축**: QP•CQ 기반 작업 순서•완료를 추적한다.
...
1. **메모리 등록•접근 키 발급**: 주소•길이•권한 범위를 고정한다.
2. **연결 정보•원격 키 교환**: 허용 원격 메모리 정보를 전달한다.
3. **키 검증•직접 메모리 전송**: 승인된 동사•버퍼만을 실행한다.
4. **작업 완료•오류 기록**: 실행 상태를 CQ에 기록한다.
...
- **완료 상태**: 게시한 RDMA 작업의 성공•오류•처리 결과를 응용에 알리는 기록하는 개념이다.

</details>

- **직접 메모리 조작•완료 상태**에 따라 단측 RDMA와 양측 송수신을 선택한다.
