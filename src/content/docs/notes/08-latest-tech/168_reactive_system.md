...
- 정의/개념: 메시지 기반 분리•장애 격리•자원 탄력성으로 부하와 장애 변화에도 응답성을 유지하는 **리액티브 시스템**이다.
- 배경/필요성: 강결합 동기 호출은 **지연•장애 연쇄 전파와 응답 기한 상실**이 발생한다.
...
- 비동기 전달로 시간•위치•실패 결합을 낮추는 **메시지 구동 방식**이 핵심이다.
- 구성요소 격리•감독•대체 응답 기반 **회복성**이 핵심이다.
- 큐•지연에 따라 유입량•자원을 조정하는 **탄력적 응답성**이 핵심이다.
...
- **감독 제어기**: 구성요소의 실패를 감지하고 격리•재시작•대체 동작을 결정하는 구성요소이다.
- **복구 실행기**: 감독 정책에 따라 재시작•대체•상태 복구를 수행하는 구성요소이다.
...
1. **메시지 전달**: 비동기 채널이 송신자와 처리자의 속도 차이를 완충한다.
2. **격리 처리**: 독립 구성요소가 자신의 상태와 실패 경계 안에서 실행한다.
3. **상태 대응**: 큐•부하•장애에 따라 역압•확장•격리•복구를 수행한다.
...
- **응답 기한•장애 격리•부하 조절 정책**을 함께 운영할 수 있을 때 리액티브 구조를 선택한다.
