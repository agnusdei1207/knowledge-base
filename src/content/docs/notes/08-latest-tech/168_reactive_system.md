...
date: "2026-07-31T12:03:45+09:00"
...
> **키워드:** 리액티브 시스템 (Reactive System)

## Ⅰ. 개요

- 정의/개념: 메시지·장애 격리·자원 탄력성으로 **응답성**을 유지하는 리액티브 시스템
...
- 비동기 전달로 시간·위치·실패 결합을 낮추는 **메시지 기반 분리**
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| **무제한 메시지 큐**로 메모리 고갈·지연 폭증 | 유한 큐·입장 제어·**역압·초과 요청 거부** | **메모리·지연 증가** 제한 |
| **재시도 폭주**로 하류 시스템 추가 압박 | **회로 차단기·지수 백오프·재시도 예산** | **재시도 부하** 감소 |
...
- **응답 기한·장애 격리**와 **부하 조절 정책**을 함께 운영할 수 있을 때 리액티브 구조 선택
