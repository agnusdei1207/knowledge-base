...
- 기존 한계: 전체 게임 트리 전개는 큰 행동 공간에서 계산 불가
- 기존 한계: 거대한 탐색 트리의 모든 분기를 완전 탐색할 수 없고, 무작위 탐색만으로는 유망한 행동에 계산 예산을 집중하기 어렵다.
...
| 핵심 특징 | 전개·평가·역전파 | UCT·시뮬레이션 통계 | 정책·가치망 유도 탐색 |
| 한계 | 분기 폭발·평가 오차 | 롤아웃 편향·분산 | 학습 편향·연산 비용 |
...
- 대규모 탐색을 위해 예산·롤아웃 편향을 검토하여 **MCTS** 활용
