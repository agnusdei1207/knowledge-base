...
- 기존 한계: 비신뢰 입력에서 생성된 코드나 도구 호출을 호스트 환경에서 직접 실행하면 파일 변조·정보 유출·자원 고갈로 피해가 확산될 수 있다.
...
1. 코드 실행 에이전트가 입력 데이터는 읽기 전용으로 연결하고 외부 통신을 허용 목록으로 제한한 일회성 샌드박스에서 코드를 실행한 뒤 검증된 결과만 반출한다.

### 쉽게 이해하기 (학습용)
- 입력은 읽기 전용으로 두고 외부 통신은 폐쇄망으로 제한

## Ⅶ. 결론
- 비신뢰 코드와 도구 실행의 피해 반경을 최소화하기 위해 격리 강도·최소 권한·자원 할당량·네트워크 및 결과 반출 정책을 검토하여 **에이전트 샌드박스**를 활용해야 한다.
