---
sidebar:
  order: 20
  label: "020. Agent Sandbox (에이전트 샌드박스)"
  badge:
    text: "미출 · 60%"
    variant: note
title: "Agent Sandbox (에이전트 샌드박스)"
date: "2026-08-02T08:54:00+09:00"
tags:
  - "notes-latest_tech"
weight: 20
extra:
  question_no: "020"
  source_status: "미출"
  source_history: ""
  priority: 60
  priority_note: "격리 실행이 도구 오용 피해를 제한"
---

## Ⅰ. 개요

<details>
<summary>핵심 용어</summary>

- **샌드박스(Sandbox)**: 신뢰할 수 없는 코드와 도구를 격리된 환경에서 실행하여 호스트 자원에 미치는 영향을 제한하는 보안 경계다.
- **최소 권한**: 실행 주체에 작업 수행에 필요한 자원과 행동만 제한된 시간 동안 허용하는 원칙이다.

</details>

- 정의/개념: **에이전트 샌드박스**는 생성 코드와 외부 도구를 격리된 환경에서 최소 권한으로 실행하고 자원·파일·네트워크 접근을 제한하는 보안 통제다.
- 배경/필요성: 비신뢰 코드의 호스트 직접 실행은 **파일 변조·정보 유출·자원 고갈** 피해 확산

### 쉽게 이해하기 (학습용)
- 사고 방지를 위한 독립된 별도 작업실 사용

## Ⅱ. 특징

<details>
<summary>핵심 용어</summary>

- **일회성 환경(Ephemeral Environment)**: 작업마다 새로 만들고 결과 반출 뒤 폐기하여 이전 실행 상태가 남지 않게 하는 환경이다.
- **자원 할당량(Resource Quota)**: CPU·메모리·저장공간·실행 시간에 강제하는 사용 상한이다.

</details>
- **Ephemeral Environment**로 비특권 일회성 작업 경계 생성
- **제한 축**: 시스템 호출·네트워크·파일·자원 통제
- **반출 축**: 결과 검사·환경 폐기로 잔존 상태 제거

### 쉽게 이해하기 (학습용)
- 입출력·통신·자원 사용량 등 모든 환경 통제

## Ⅲ. 구조 및 구성요소

<details>
<summary>핵심 용어</summary>

- **Seccomp**: 프로세스가 호출할 수 있는 리눅스 시스템 호출을 정책으로 필터링하는 커널 기능이다.
- **외부 통신 통제(Egress Control)**: 샌드박스에서 나가는 통신의 목적지·프로토콜·포트를 허용 목록으로 제한하는 통제다.
- **비특권 프로세스**: 관리자 권한 없이 실행되어 침해 시 호스트에 행사할 수 있는 권한을 줄인 프로세스다.

</details>

```mermaid
block-beta
    columns 3
    A["의존성 관리"]
    B["실행 격리"]
    C["데이터 경계"]
    D["네트워크 통제"]
    E["자원 관리자"]
    A --- B
    B --- C
    C --- D
    D --- E
```

| 구성요소 | 책임 |
|:---|:---|
| 의존성 관리 | 최소 이미지와 **패키지 버전** 고정 |
| 실행 격리 | **Seccomp·비특권 프로세스**로 호스트 접근 제한 |
| 데이터 경계 | 읽기 전용 입력과 **검증 출력** 분리 |
| 네트워크 통제 | **Egress Control·목적지 허용 목록** 적용 |
| 자원 관리자 | **Resource Quota·환경 폐기** 관리 |

### 쉽게 이해하기 (학습용)
- 검증된 환경에서 실행하고 결과만 반출

## Ⅳ. 흐름도

<details>
<summary>핵심 용어</summary>

- **위험 기반 격리**: 코드의 신뢰도와 예상 부작용에 따라 컨테이너·가상머신 같은 격리 수준을 선택하는 방식이다.
- **결과 검증**: 산출물의 형식·악성 내용·민감정보를 검사하여 허용된 결과만 격리 경계 밖으로 반출하는 절차다.

</details>

```mermaid
sequenceDiagram
    participant A as 에이전트
    participant P as 정책 엔진
    participant S as 샌드박스
    participant V as 결과 검증기
    A->>P: 코드·도구 실행 요청
    P->>S: 1. 위험도별 격리·권한·할당량 구성
    A->>S: 2. 코드·도구 제한 실행
    S-->>V: 3. 출력·행위 기록 검증
    V->>S: 4. 일회성 환경 폐기
    V-->>A: 허용 산출물·실행 결과 반환
```

1. **위험도별 격리·권한·할당량 구성**: 신뢰도·부작용별 경계 설정
2. **코드·도구 제한 실행**: 시스템 호출·자원 상한 강제
3. **출력·행위 기록 검증**: 악성 결과·비밀정보·정책 위반 검사
4. **일회성 환경 폐기**: 허용 산출물 외 잔존 상태 제거

### 쉽게 이해하기 (학습용)
- 실행 전 안전등급 결정 및 후처리 필수

## Ⅴ. 종류 및 비교

<details>
<summary>핵심 용어</summary>

- **컨테이너(Container)**: 호스트 커널을 공유하면서 네임스페이스와 cgroup으로 프로세스·자원을 격리하는 방식이다.
- **마이크로 가상머신(MicroVM)**: 경량 게스트 커널을 별도로 실행하여 컨테이너보다 강한 커널 격리 경계를 제공하는 방식이다.

</details>

| 격리 방식 | 컨테이너 | MicroVM |
|:---|:---|:---|
| 적용 기준 | **낮은 위험·고빈도 실행** | **비신뢰 코드·강한 격리** |
| 핵심 특징 | **호스트 커널 공유**로 고밀도 | **게스트 커널 분리**로 경계 강화 |
| 한계 | **커널 취약점의 호스트 영향** | **시작 지연·자원 오버헤드** |

> 요약: **Container**는 고밀도, **MicroVM**은 강한 커널 격리 중심

### 쉽게 이해하기 (학습용)
- 격리 강도와 비용 고려하여 선택

## Ⅵ. 실무 고려사항 및 대책

<details>
<summary>핵심 용어</summary>

- **격리 탈출**: 샌드박스 안의 코드가 취약점을 이용해 격리 경계를 넘어 호스트 권한이나 자원에 접근하는 공격이다.
- **피해 반경(Blast Radius)**: 침해나 오작동이 영향을 미칠 수 있는 시스템·데이터·사용자의 최대 범위다.
- **공급망 확산**: 악성 산출물이 후속 시스템이나 배포 과정에 전달되어 샌드박스 밖으로 피해가 이어지는 현상이다.

</details>

| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 공유 커널 취약점의 **격리 탈출** | 위험도에 따라 MicroVM과 비특권 사용자 적용 | 호스트까지의 **피해 반경** 축소 |
| 외부 통신을 통한 **정보 유출** | 기본 차단 후 목적지·프로토콜 허용 목록 운영 | 비밀정보의 **무단 반출** 방지 |
| 무한 실행에 따른 **자원 고갈** | CPU·메모리·저장공간·시간 할당량 강제 | 서비스의 **가용성 보호** |
| 악성 출력의 **후속 감염** | 파일 형식·내용·민감정보 검사 후 결과만 반출 | 샌드박스 밖 **공급망 확산** 차단 |

### 쉽게 이해하기 (학습용)
- 입력은 읽기 전용으로 두고 외부 통신은 폐쇄망으로 제한

## Ⅶ. 결론

<details>
<summary>핵심 용어</summary>

- **컨테이너 선택 기준**: 위험이 낮고 실행 빈도가 높아 시작 속도와 자원 밀도가 중요할 때 사용한다.
- **MicroVM 선택 기준**: 신뢰할 수 없는 코드가 호스트 커널에 미칠 피해를 강하게 격리해야 할 때 사용한다.

</details>
- 낮은 위험·고빈도는 **Container**, 비신뢰 코드는 **MicroVM** 선택

### 쉽게 이해하기 (학습용)
- 목적은 성공이 아니라 실패 시 범위 최소화
