...
- 정의/개념: 원본 데이터를 외부로 보내지 않고 단말 내부에서 모델 추론을 수행하는 **온디바이스 AI**
...
- 압축 모델과 **NPU•GPU**를 결합하는 전력•메모리 제약 실행
...
1. **입력 권한•민감도 판정**: 센서 권한과 데이터의 **외부 전송 범위** 확인
2. **모델 서명•호환성 검증**: 해시•버전•지원 연산자 검사
3. **그래프•가속기 배치**: 지원•전력 조건에 따라 **NPU•GPU•CPU** 선택
4. **로컬 추론 실행**: 단말 가속기에서 모델 연산과 결과 생성
