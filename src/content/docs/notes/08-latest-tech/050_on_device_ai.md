...
- 정의/개념: 원본 데이터를 외부로 보내지 않고 단말 내부에서 모델 추론을 수행하는 **온디바이스 인공지능(On-Device Artificial Intelligence, On-Device AI)**
...
- **그래프 분할(Graph Partitioning)**: 모델 연산을 지원 여부에 따라 신경망 처리장치(Neural Processing Unit, NPU)·그래픽 처리장치(Graphics Processing Unit, GPU)·중앙처리장치(Central Processing Unit, CPU) 또는 외부 실행 구간으로 나누는 방법이다.
...
| 신경망 처리장치(NPU)·그래픽 처리장치(GPU)·중앙처리장치(CPU) | 전력·성능 조건에 따른 **로컬 추론 실행** |
...
3. **그래프·가속기 배치 전달**: 연산별 지원·전력 조건에 따라 **신경망 처리장치(Neural Processing Unit, NPU)·그래픽 처리장치(Graphics Processing Unit, GPU)·중앙처리장치(Central Processing Unit, CPU) 배치**
...
- **온디바이스 인공지능(On-Device Artificial Intelligence, On-Device AI)**: 개인 단말 내부에서 오프라인·저지연·개인화 추론을 수행한다.
...
| **무선 업데이트(Over-the-Air Update, OTA Update) 실패** | 단계 배포·이전 모델 자동 롤백 | 서비스 **신속 복구** |
