...
- 정의/개념: 원본 데이터를 외부로 보내지 않고 단말 내부에서 모델 추론을 수행하는 **온디바이스 AI**임. 이는 전통적 방식과의 핵심 차별점으로서 실질적인 업무 가치를 창출함.
- 배경/필요성: 클라우드 의존 추론은 망 단절•왕복 지연•민감정보 전송으로 **현장 자율성**에 제약을 줌.

#### 한줄 요약
- 원본 데이터를 외부 전송하지 않는 **단말 내부 추론** 구조 구현
...
- **CPU(Central Processing Unit)**: 제어와 미지원 연산을 처리하는 범용 장치를 의미하며, 시스템의 자율적 판단과 실행을 가능하게 하는 핵심 아키텍처 요소이다.

</details>

- 네트워크 왕복 없이 실행하는 **오프라인•저지연 추론**이 핵심임. 이는 전통적 방식과의 핵심 차별점으로서 실질적인 업무 가치를 창출함.
- 원본 데이터를 단말에 유지하는 **로컬 개인정보 처리**가 핵심임. 이는 전통적 방식과의 핵심 차별점으로서 실질적인 업무 가치를 창출함.
- 압축 모델과 **NPU•GPU**를 결합하여 전력•메모리 제약을 실행하여 실무 환경에서의 유연한 대응과 지속적 최적화를 지원함.

#### 한줄 요약
- **오프라인•저지연•로컬 개인정보 처리**를 제공 체계 적용
...
- **추론 런타임(Inference Runtime)**: 변환된 모델을 장치에 배치하고 실행하는 소프트웨어를 의미하며, 시스템의 자율적 판단과 실행을 가능하게 하는 핵심 아키텍처 요소이다.
...
#### 한줄 요약
- **입력 권한•패키지•컴파일러•런타임•가속기**를 구성 체계 적용
...
#### 한줄 요약
- **권한•서명•배치•로컬 추론•위임 판정**을 수행 체계 적용
...
#### 한줄 요약
- **민감도•지연•집계•연산량**으로 실행 위치를 선택 체계 적용
...
- 민감•오프라인 요청은 **온디바이스**, 자원•품질 한계 초과 시 **비식별 후 외부 위임**이 핵심임. 이는 전통적 방식과의 핵심 차별점으로서 실질적인 업무 가치를 창출함.

#### 한줄 요약
- **데이터 민감도•단말 자원**으로 로컬•외부 위임을 선택 체계 적용