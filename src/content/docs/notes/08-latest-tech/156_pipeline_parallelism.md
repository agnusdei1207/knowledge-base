...
- 정의/개념: 모델의 연속된 층을 여러 장치의 단계로 나누고 마이크로배치를 중첩 실행하는 **PP 기법**이다.
- 배경/필요성: 모델이 단일 장치 메모리를 초과하고 **순차 층 분할은 버블**이 발생한다.
...
- **분할 축**: 연속 모델 층의 장치별 단계를 배치한다.
- **중첩 축**: 마이크로배치 순•역전파로 버블 감소한다.
- **균형 축**: 단계 균형이 깨지면 처리량 제한, 분할 경계가 늘면 통신 대기 증가한다.
...
- **단계 분할기**: 연속된 모델 층을 연산량•메모리•통신량이 균형을 이루도록 장치 단계로 나누는 구성요소이다.
- **마이크로배치 분할기**: 전역 배치를 작은 단위로 나누어 서로 다른 파이프라인 단계가 동시에 일하게 하는 구성요소이다.
- **스케줄 엔진**: 마이크로배치의 순전파•역전파 순서를 정해 파이프라인 버블과 메모리 사용량을 조절하는 구성요소이다.
...
1. **마이크로배치 전달**: 전역 배치 분할•연속을 투입한다.
2. **순전파 활성값 전달**: 전단 계산•중간 상태를 보존한다.
3. **단계 입력 전달**: 후단 장치의 순전파를 실행한다.
4. **역전파 기울기 전달**: 후단의 입력 기울기를 계산한다.
5. **전단 기울기 전달**: 역순 단계의 기울기를 전파한다.
...
- **동기 학습•처리량 균형**에 따라 GPipe와 1F1B을 선택한다.
