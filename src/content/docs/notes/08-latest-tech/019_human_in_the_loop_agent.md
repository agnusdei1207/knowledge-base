---
sidebar:
  order: 19
  label: "019. Human-in-the-Loop Agent (인간 개입 에이전트)"
  badge:
    text: "미출 · 60%"
    variant: note
title: "Human-in-the-Loop Agent (인간 개입 에이전트)"
date: "2026-07-25T01:25:00+09:00"
tags:
  - "notes-latest_tech"
weight: 19
extra:
  question_no: "019"
  source_status: "미출"
  source_history: ""
  priority: 60
  priority_note: "고위험 판단의 인간 승인 설계가 중요"
---

## 미리 알고가기

- **HITL (Human-in-the-Loop)**: 인간의 검토·수정·승인 포함
- **Human-on-the-Loop**: 자동 실행 감시 및 필요시 개입
- **Interrupt**: 실행을 안전하게 일시 정지함
- **Checkpoint**: 중단 시점 상태 저장 및 재개
- **Approval Gate**: 인간 결정을 요구하는 지점
- **Escalation**: 예외를 담당자에게 넘김
- **Automation Bias**: 자동화 결과를 맹신하는 편향

## Ⅰ. 개요

- 정의/개념: **HITL**은 에이전트의 실행 과정에서 사람이 결과와 영향을 검토하고 계속·수정·거부·중단 여부를 결정하는 통제 방식이다.
- 배경/필요성: 고위험·불확실 판단의 자동 실행은 **오류 사전 차단·최종 통제권** 확보가 곤란

### 쉽게 이해하기 (학습용)
- 비서의 제안을 담당자가 확인 후 처리

## Ⅱ. 특징
- **위험 기반 개입 지점**으로 사람의 승인·검토 시점을 정한다
- **근거·영향·대안 제공**으로 승인자가 판단 가능한 문맥을 확보한다
- **구조화된 결정·상태 재검증**으로 수정·거부와 승인 중 환경 변화를 반영한다

### 쉽게 이해하기 (학습용)
- 단순 승인보다 변경 이유와 영향 파악이 핵심

## Ⅲ. 구조 및 구성요소

```mermaid
block-beta
    columns 3
    A["위험 평가기"]
    B["중단 계층"]
    C["검토 계층"]
    D["결정 라우터"]
    E["정책 관리"]
    A --- B
    B --- C
    C --- D
    D --- E
```

| 구성요소 | 책임 |
|:---|:---|
| 위험 평가기 | 영향·불확실성으로 **개입 필요성** 판정 |
| 중단 계층 | 실행 상태 보존과 **부작용 차단** |
| 검토 계층 | 판단 근거·영향·**대안 표시** |
| 결정 라우터 | 승인·수정·거부의 **상태 전이** |
| 정책 관리 | 승인자·기한·**감사 기록** 관리 |

### 쉽게 이해하기 (학습용)
- 자동화할 것과 중요한 담당 결정을 구분

## Ⅳ. 흐름도

```mermaid
sequenceDiagram
    participant A as 에이전트
    participant R as 위험 평가기
    participant H as 승인자
    participant T as 실행 대상
    A->>R: 1. 영향·불확실성 평가
    R-->>A: 2. 실행 중단·상태 저장
    A->>H: 3. 근거·영향·대안 제시
    H-->>A: 4. 승인·수정·거부 결정
    A->>T: 5. 최신 조건 재검증·실행
```

1. **영향·불확실성 평가**: 피해 규모와 신뢰도로 **개입 조건** 판정
2. **실행 중단·상태 저장**: 승인 전 부작용을 막고 **재개 지점** 보존
3. **근거·영향·대안 제시**: 승인자가 판단할 **의사결정 정보** 제공
4. **승인·수정·거부 결정**: 사람의 선택을 구조화해 **실행 상태** 변경
5. **최신 조건 재검증·실행**: 승인 대기 중 바뀐 대상·권한을 확인한 **조건부 실행**

### 쉽게 이해하기 (학습용)
- 승인 직후에도 실행 조건을 다시 확인

## Ⅴ. 종류 및 비교

| 실행 방식 | HITL 에이전트 | 자동 에이전트 |
|:---|:---|:---|
| 적용 기준 | 고위험·불확실 **비가역 행동** | 저위험·반복적 **가역 행동** |
| 핵심 특징 | 인간 결정 후 **조건부 실행** | 정책 통과 후 **즉시 실행** |
| 한계 | 승인 병목·자동화 편향 | 권한 남용·피해 확산 |

### 쉽게 이해하기 (학습용)
- 검토 시간과 책임 설계가 중요함

## Ⅵ. 실무 고려사항 및 대책

| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 모든 작업 승인에 따른 **병목** | 영향·불확실성·가역성 기반 위험 등급화 | 고위험 지점에 **검토 역량 집중** |
| 근거 없는 승인에 따른 **자동화 편향** | 판단 근거·대안·예상 영향과 불확실성 표시 | 실질적인 **오류 발견** 지원 |
| 승인 대기 중 **대상 상태 변경** | 실행 직전 대상·금액·권한·정책 재검증 | 오래된 승인에 따른 **오실행** 방지 |
| 기한 초과 시 **책임 공백** | 승인자·대체자·응답 기한·거부 기본값 명시 | 예외 상황의 **결정 책임** 확보 |

### 쉽게 이해하기 (학습용)
- 외부 발송·결제는 위험 등급에 따라 사람 승인 기한을 다르게 정함

## Ⅶ. 결론
- 고위험·비가역 행동에는 **HITL**을 배치하고 근거 제공·상태 저장·기한·실행 직전 재검증으로 인간 통제권 확보

### 쉽게 이해하기 (학습용)
- 가치는 승인이 아닌 잘못된 점 발견에 있음
