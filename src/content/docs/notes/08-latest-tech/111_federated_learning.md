...
- 정의: 원본 대신 로컬 업데이트를 집계하는 **연합학습**이다.
- 배경/필요성: 개인정보•기밀•주권으로 **다기관 원본 중앙 수집 곤란** #### 한줄 요약
- 원본은 보관하고 **로컬 업데이트** 한정 집계해 공동 모델 학습
...
1. 참여자•모델 버전 확정: 가용성•자원•정책에 맞는 **라운드 참여 집합** 확정함.
2. 글로벌 모델•학습 설정 배포: 동일 시작 모델과 반복 횟수•학습률의 **학습 계약** 전달함.
3. 로컬 학습 요청: 원본 데이터 기반의 **참여자별 변화량** 계산을 지시함.
4. 보호된 업데이트 전달: 원본 대신 **로컬 모델 변화량** 제공함.
5. 글로벌 모델 갱신: 가중 평균•보안 집계로 **새 모델** 생성함.
...
#### 한줄 요약
- **원본 중앙 수집•다수 단말•소수 기관** 대상 따라 구조 구분
...
#### 한줄 요약
- **데이터 주권•non-IID•참여 안정성** 기반 연합 구조•집계 결정
