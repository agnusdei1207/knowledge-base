...
- 불필요한 제어·배선을 제거한 **전용 데이터흐름**
- 온칩 재사용 기반 **전력당 처리량 극대화**
- 높은 초기비·개발기간과 **NRE·변경 불가** 절충
...
- **1. 워크로드·양산 조건 전달**: 모델 수명·수량·정밀도·변경 주기 확정
- **2. 전용 데이터흐름 전달**: 연산 배열·온칩 메모리·연결망 고정
- **3. 검증 설계·테이프아웃 전달**: 기능·전력·타이밍 검증 후 제조 확정
- **4. 제조·시험 칩 전달**: 웨이퍼·패키지·수율 시험 통과
- **5. 지원 모델 성능 전달**: 컴파일된 연산의 처리량·전력 검증
...
- **모델 안정성·생산 수량·NRE**로 ASIC의 테이프아웃 여부를 결정
