...
- 정의/개념: ASIC가 AI 연산•데이터 흐름을 제조 시 고정하는 **전용 하드웨어 가속 방식**이다.
- 배경/필요성: 범용•재구성 회로에서 발생하는 **제어•배선 오버헤드** 때문에 전력 효율 제약이 발생한다.
...
- **구조 축**: 제어•배선을 제거한 전용 데이터흐름이다.
- **효율 축**: 온칩 재사용으로 전력당 처리량 극대화이다.
- **경제 축**: 양산량이 적거나 연산이 바뀌면 NRE 회수 곤란이다.
...
1. **전용 데이터흐름 설계**: 연산 배열•메모리•연결망을 고정한다.
2. **기능•전력•타이밍 검증•테이프아웃**: 제조 설계를 확정한다.
3. **웨이퍼•패키지•수율 시험**: 기준 통과 칩을 선별한다.
