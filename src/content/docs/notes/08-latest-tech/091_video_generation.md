...
- 정의/개념: **영상 생성(Video Generation)**은 공간 구조와 시간 움직임을 함께 합성하는 생성 기술이다.
- 배경/필요성: 독립 프레임 생성은 객체 외형과 움직임의 **시간 연속성**을 보장할 수 없다.
...
- 공간 구조와 움직임을 결합한 **시공간 표현**이 핵심이다.
- 궤적•카메라•포즈 기반 **움직임 조건 제어**가 핵심이다.
- 이전 객체 상태를 보존하는 **시간 일관성**이 핵심이다.
...
- **일관성 평가기**: 생성 프레임의 객체 외형과 움직임, 물리 관계가 이어지는지 평가하는 구성요소이다.
...
1. **텍스트•참조 조건 인코딩**: 객체•배경•스타일 표현을 생성한다.
2. **움직임 조건 주입**: 궤적•카메라로 시간축 변화를 제어한다.
3. **시공간 잠재 상태 갱신**: 공간 구조와 프레임 관계를 결합한다.
4. **연속 프레임 생성**: 잠재 상태를 시간순 영상으로 복원한다.
5. **시간•물리 일관성 판정**: 외형•동작•물리 관계의 연속성을 검증한다.
...
- 영상 길이•움직임 조건에 따라 **생성 방식**을 선택하고 **시간 일관성**을 검증한다.
