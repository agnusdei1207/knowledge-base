...
- **플랫폼 엔지니어링(Platform Engineering)**: 인프라•배포•관측•보안 기능을 개발자가 스스로 사용하는 내부 제품으로 설계•운영하는 공학 접근하는 개념이다.
...
- 정의/개념: 공통 개발•운영 기능을 IDP•골든 패스•셀프서비스로 제공하는 **플랫폼 엔지니어링 접근**이다.
- 배경/필요성: 팀별 도구•티켓 요청은 **개발자 인지부하•대기 시간•운영 편차**가 증가한다.
...
- 개발자 여정•피드백 기반 **내부 플랫폼 제품 로드맵**이 핵심이다.
- 골든 패스•정책 자동화 기반 **안전한 셀프서비스 기본 경로**가 핵심이다.
- 추상화•탈출 경로 기반 **표준화와 개발자 자율성의 균형**이 핵심이다.
#### 한줄 요약
...
- **IDP 서비스 계층**: API•포털•자동화를 연결하여 표준 기능을 셀프서비스로 제공하는 구성요소이다.
...
- **1. 개발 여정•병목 전달**: 반복 작업•대기 시간•인지부하 기반 제품 문제 탐색이다.
- **2. 권장 경로•정책 전달**: 공통 수요와 위험도 기반 골든 패스•탈출 경로를 설계한다.
- **3. 셀프서비스 기능 전달**: 포털•요청 양식•템플릿으로 인프라•배포•관측 기능 노출이다.
- **4. 채택•미사용 피드백 전달**: 채택률•완료 시간•골든 패스 미사용 원인 기반 로드맵을 개선한다.
...
- **자발적 채택•인지부하**로 골든 패스와 탈출 경로를 조정하고 채택률이 낮으면 제품을 개선한다.
