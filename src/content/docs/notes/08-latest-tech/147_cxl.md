...
- 정의/개념: **컴퓨트 익스프레스 링크(Compute Express Link, CXL)** 는 **주변장치 상호연결 익스프레스(Peripheral Component Interconnect Express, PCIe) 물리 계층** 에서 입출력과 캐시·메모리 일관성을 제공하는 연결 표준
- 배경/필요성: 주변장치 상호연결 익스프레스의 명시적 복사로 장치 메모리의 **공유·일관성 유지** 가 제한됨
...
**컴퓨트 익스프레스 링크(Compute Express Link, CXL)** 는 입출력용 CXL.io, 호스트 캐시용 CXL.cache, 장치 메모리용 CXL.mem을 제공한다.
...
- **CXL.io**: 장치 발견·구성·입출력과 직접 메모리 접근을 제공하는 주변장치 상호연결 익스프레스 기반 프로토콜이다.
- **CXL.cache**: 가속기 장치가 호스트 메모리를 캐시 일관성을 유지하며 접근하게 하는 프로토콜이다.
- **CXL.mem**: 호스트가 장치에 부착된 메모리를 일관된 메모리 공간으로 읽고 쓰게 하는 프로토콜이다.
- **CXL 메모리 장치**: 용량 확장·계층화·풀링을 위해 호스트가 CXL.mem으로 접근하는 메모리 자원이다.
- **스위치 관리 계층**: 여러 CXL 장치의 연결 경로·자원·오류 상태를 관리하는 계층이다.

</details>

**컴퓨트 익스프레스 링크(Compute Express Link, CXL) 링크** 는 호스트 브리지와 가속기·메모리 장치를 연결한다.
...
1. **링크 훈련·컴퓨트 익스프레스 링크(Compute Express Link, CXL) 모드**: **주변장치 상호연결 익스프레스(Peripheral Component Interconnect Express, PCIe) 물리 경로** 와 동작 모드 협상
...
**컴퓨트 익스프레스 링크(Compute Express Link, CXL) Type-1·2·3 장치** 는 캐시와 메모리 프로토콜 지원 조합이 다르다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 장치 유형·프로토콜 불일치 | Type별 **컴퓨트 익스프레스 링크(Compute Express Link, CXL)의 io·cache·mem 조합** 검증 | 미지원 경로 구성 방지 |
...
- **컴퓨트 익스프레스 링크(Compute Express Link, CXL)** 기반 호스트 메모리 가속은 **Type-1·2**, 용량 확장은 **Type-3** 선택
