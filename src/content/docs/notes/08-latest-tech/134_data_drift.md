...
- **데이터 드리프트(Data Drift)**: 학습 기준 시점과 운영 시점 사이에서 입력 분포 $P(X)$가 달라지는 현상이다.
- **지연 라벨**: 예측 시점보다 실제 정답이 늦게 확정돼 성능 평가에 즉시 사용할 수 없는 라벨이다.

</details>

- 정의/개념: **데이터 드리프트**는 학습 기준 시점과 운영 시점 사이에서 입력 분포 $P(X)$가 변하는 현상이다.
- 배경/필요성: 지연 라벨 환경은 정답 도착 전 **성능 저하 판정할 수 없다**.
...
- 정답 없이 $P(X)$를 비교하는 **조기 경보**가 핵심이다.
- 계절•캠페인을 구분하는 **분포 기준선**이 핵심이다.
- 성능 확정 전 지연 정답을 결합하는 **영향 검증**이 핵심이다.
...
1. **기준 분포 프로필 선택**: 계절•캠페인별 정상 구간을 고정한다.
2. **현재 입력 프로필 생성**: 오류•결측을 분리해 피처별을 요약한다.
3. **분포 거리•문맥 경보 계산**: PSI•KS 검정으로 조기를 탐지한다.
4. **지연 정답•성능 결합**: 입력 변화의 실제 품질 영향을 확인한다.
5. **관측•보정•재학습 판정**: 원인•성능 저하별 대응을 선택한다.
...
- **분포 거리**만 증가하면 관측하고 **지연 성능**도 저하하면 재학습이 핵심이다.
