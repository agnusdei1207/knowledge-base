---
sidebar:
  order: 178
  label: "178. 핀옵스 (FinOps)"
  badge:
    text: "기출 · 70%"
    variant: note
title: "핀옵스 (FinOps)"
date: "2026-07-27T23:59:59+09:00"
tags:
  - "notes-latest-tech"
weight: 178
extra:
  question_no: "178"
  source_status: "기출"
  source_history: "135회"
  priority: 70
  priority_note: "FinOps 비용 할당·가치 최적화가 기출됨"
---

## 미리 알고가기

- **FinOps**: 기술·재무·사업 조직이 변동하는 기술 비용과 사업 가치를 공동 최적화하는 운영 체계
- **쇼백(Showback)**: 비용을 사용 조직에 보여 주되 실제 예산에는 부과하지 않는 방식
- **단위 경제성(Unit Economics)**: 요청·고객·거래 등 가치 단위당 기술 비용

## Ⅰ. 개요

- 정의/개념: 사용량 기반 기술 지출을 **할당·분석·예측·최적화**해 비용 책임과 사업 가치를 개선하는 체계
- 배경/필요성: 중앙 청구서만으로는 **사용 주체·제품 가치·낭비 원인**을 연결하기 어려운 한계

### 쉽게 이해하기 (학습용)

- 누가 왜 썼고 그 지출이 어떤 제품 가치로 이어졌는지를 기술·재무·사업 팀이 함께 판단한다.

## Ⅱ. 특징

- 팀·제품·환경별 **비용 가시성·할당**
- 쇼백·차지백 기반 **분산 책임**
- 수요·요율·약정의 **예측·최적화**
- 절감액보다 서비스 가치와 연결한 **단위 경제성**

### 쉽게 이해하기 (학습용)

- 무조건 적게 쓰는 것이 아니라 품질을 지키며 사용법과 구매 방식을 개선한다.

## Ⅲ. 구조 및 구성요소

```mermaid
block-beta
  columns 1
  data["청구·사용량 데이터"]
  allocation["비용 할당"]
  reporting["쇼백·차지백"]
  optimization["예측·최적화"]
  value["사업 가치·거버넌스"]
  data --- allocation
  allocation --- reporting
  reporting --- optimization
  optimization --- value
```

| 구성요소 | 책임 |
|:---|:---|
| 청구·사용량 데이터 | 공급자 비용·자원 사용·가격 정규화 |
| 비용 할당 | 직접·공유 비용을 책임 단위에 귀속 |
| 쇼백·차지백 | 팀별 비용 정보 제공·예산 부과 |
| 예측·최적화 | 수요 예측과 사용량·요율·약정 개선 |
| 사업 가치·거버넌스 | 단위 경제성·예산·이상·성과 관리 |

> 요약: 정규화한 비용을 책임 단위에 할당하고 사업 가치로 최적화한다

### 쉽게 이해하기 (학습용)

- 총고지서를 제품별로 나눈 뒤 다음 사용을 예측하고 가치가 큰 개선부터 실행한다.

## Ⅳ. 흐름도

```mermaid
sequenceDiagram
  participant D as 청구·사용량 데이터
  participant A as 비용 할당
  participant R as 쇼백·차지백
  participant O as 예측·최적화
  participant V as 사업 가치·거버넌스
  D->>A: 1. 비용·사용량 정규화
  A->>R: 2. 직접·공유 비용 귀속
  R->>O: 3. 책임 조직 비용 공개
  O->>V: 4. 사용량·요율 개선안
  V-->>A: 5. 단위 가치·정책 환류
```

### 동작 원리

1. **비용·사용량 정규화**: 공급자별 청구 항목을 공통 분류로 변환
2. **직접·공유 비용 귀속**: 태그·계정·사용량 기준으로 소유자 배분
3. **책임 조직 비용 공개**: 쇼백 또는 차지백으로 행동 신호 제공
4. **사용량·요율 개선안**: 낭비 제거·규모 조정·약정 구매 검토
5. **단위 가치·정책 환류**: 품질과 거래당 비용을 확인해 할당·예산 정책 갱신

> 요약: 비용을 할당·공개하고 최적화 결과를 단위 가치로 검증한다

### 쉽게 이해하기 (학습용)

- 팀별 고지서를 보여 주고 개선한 뒤 요청당 비용과 서비스 품질이 나아졌는지 확인한다.

## Ⅴ. 종류 및 비교

| 비용 책임 방식 | 쇼백 | 차지백 | 중앙 비용 처리 |
|:---|:---|:---|:---|
| 적용 기준 | 자율 개선 유도 | 비용 회수·강한 책임 | 초기·공용 기반 비용 |
| 핵심 특징 | 팀별 비용 정보 제공 | 팀 예산에 실제 비용 부과 | 중앙 조직이 비용 부담 |
| 한계 | 행동 변화 강제력 부족 | 배분 분쟁·공유비 왜곡 | 사용 책임·최적화 동기 약화 |

> 요약: 쇼백은 정보, 차지백은 예산 귀속, 중앙 처리는 공용 부담이다

### 쉽게 이해하기 (학습용)

- 고지서만 보여 줄지 실제 예산에서 낼지 회사 공용비로 둘지를 비용 성숙도에 맞춰 정한다.

## Ⅵ. 실무 고려사항 및 대책

| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 태그 누락과 공유 비용 미배분 | 계정·태그·사용량 기반 단계적 할당 | 비용 책임 정확도 향상 |
| 할인 구매 후 유휴 약정 | 수요 예측·커버리지·활용률 공동 관리 | 요율 절감의 낭비 방지 |
| 절감으로 서비스 품질 저하 | SLO·단위 경제성 동시 평가 | 비용과 가치 균형 |

### 쉽게 이해하기 (학습용)

- 공유 Kubernetes 비용은 네임스페이스 사용량으로 배분하되 SLO와 거래당 비용을 함께 본다.

## Ⅶ. 결론

- 기술 지출을 가치와 연결하기 위해 **비용 정규화·공유비 할당·수요 예측·요율 최적화·단위 경제성·조직 책임**을 검토하고 FinOps 주기를 반복한다.

### 쉽게 이해하기 (학습용)

- 목표는 가장 적게 쓰는 것이 아니라 누가 왜 쓰는지 알고 가장 가치 있게 쓰는 것이다.
