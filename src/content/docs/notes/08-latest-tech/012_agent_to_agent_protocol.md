...
date: "2026-08-06T20:44:00+09:00"
tags:
  - "notes-latest_tech"
weight: 12
extra:
...
- 정의/개념: 원격 에이전트의 능력과 인증 요구를 발견하고 상태형 작업과 결과물을 교환하는 **A2A**
- 배경/필요성: 에이전트별 **능력 공개•작업 상태** 차이로 이기종 협업마다 개별 연동 필요

#### 한줄 요약

- 이기종 에이전트가 **능력 발견•상태형 작업**으로 협업
...
- **A2A-Version**과 JSON-RPC•gRPC•HTTP+JSON 바인딩 지원

#### 한줄 요약

- **에이전트 카드•프로토콜 바인딩**으로 내부 구현 없이 위임
...
2. **상태형 작업 생성**: Task ID와 초기 상태를 발급
3. **작업 상태•결과물 갱신**: 상태 전이•결과물 증분 기록
4. **스트리밍•푸시 상태 통지**: 진행 상태를 구독 채널로 전달
5. **종결 결과물 확정•반환**: Task ID에 최종 산출물 연결

#### 한줄 요약

- **에이전트 카드•Task ID**로 작업 위임부터 완료까지 추적
...
- 원격 에이전트 작업 위임에는 **A2A**, 모델 기능 연결에는 **MCP** 선택

#### 한줄 요약

- **카드 서명•결과물 무결성**을 확인한 뒤 작업 위임
