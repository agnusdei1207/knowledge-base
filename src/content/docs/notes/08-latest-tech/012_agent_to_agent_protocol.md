...
- 정의/개념: 원격 에이전트의 능력과 인증 요구를 발견하고 상태형 작업과 결과물을 교환하는 **에이전트 간 프로토콜(Agent2Agent Protocol, A2A)**
...
- **에이전트 간 프로토콜(Agent2Agent Protocol, A2A)** 구성은 클라이언트·서버·에이전트 카드·작업·메시지·결과물로 역할을 분리한다.
...
- **작업 식별자(Task Identifier, Task ID)**: 장기 작업의 상태와 결과물을 일관되게 연결하는 고유 값

</details>

- **에이전트 간 프로토콜(Agent2Agent Protocol, A2A)** 클라이언트가 능력을 확인한 뒤 작업을 위임하고 **작업 식별자(Task Identifier, Task ID)** 기반 상태와 **결과물(Artifact)** 을 추적한다.
...
2. **작업 상태·결과물 갱신**: 상태 전이·결과물(Artifact) 증분 기록
3. **종결 결과물 확정**: 작업 식별자(Task Identifier, Task ID)에 최종 산출물 연결
...
- **에이전트 간 프로토콜(Agent2Agent Protocol, A2A)**: 원격 에이전트의 능력을 발견하고 상태형 작업을 위임하는 프로토콜
- **모델 컨텍스트 프로토콜(Model Context Protocol, MCP)**: 모델에 외부 도구·리소스·프롬프트를 연결하는 프로토콜
- **작업 위임**: 다른 에이전트에 목표를 전달하고 진행 상태와 결과를 받는 협업 방식

</details>

- **에이전트 간 프로토콜(Agent2Agent Protocol, A2A)** 및 **모델 컨텍스트 프로토콜(Model Context Protocol, MCP)** 사이를 원격 작업 위임과 모델 기능 연결 기준으로 구분한다.
...
> 요약: 에이전트 작업 위임에는 **A2A**, 모델 기능 연결에는 **MCP** 적용

#### 한줄 요약

- A2A는 원격 작업을 위임하고 MCP는 모델에 도구·문맥을 연결함
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 위조 카드로 **악성 에이전트 선택** | **카드 서명·전송 계층 보안(Transport Layer Security, TLS)·발급 주체** 검증 | 신원·능력 신뢰 확보 |
| 장기 작업으로 **상태 불일치** | **작업 식별자(Task Identifier, Task ID)·버전·멱등키** 기반 전이 관리 | 중복 실행·결과 혼선 방지 |
| 결과물 변조로 **출처·무결성 상실** | **해시·생성 주체·근거** 검증 | 추적성·책임성 확보 |

#### 한줄 요약

- 카드 신뢰·작업 상태·결과물 무결성을 함께 검증함
...
- **에이전트 간 프로토콜(Agent2Agent Protocol, A2A)**: 원격 에이전트에 상태형 작업을 위임하고 결과물을 받는 프로토콜
- **모델 컨텍스트 프로토콜(Model Context Protocol, MCP)**: 모델에 외부 기능과 컨텍스트를 연결하는 프로토콜

</details>

- 원격 에이전트 작업 위임에는 **에이전트 간 프로토콜(Agent2Agent Protocol, A2A)**, 모델 기능 연결에는 **모델 컨텍스트 프로토콜(Model Context Protocol, MCP)** 선택
