...
- **QLoRA**: 4비트 기반 모델을 압축 상태로 동결하고 계산 시 역양자화하여 LoRA 경로만 학습하는 기법이다.
- **LoRA**: 기반 가중치를 고정하고 두 작은 행렬이 만드는 변화량만 학습하는 방식이다.
...
- **LoRA**: 고정밀 기반 모델을 적재하고 저랭크 변화량만 학습한다.
- **QLoRA**: 4비트 기반 모델을 적재하고 계산 시 복원하여 저랭크 변화량만 학습한다.
...
- **양자화 손실**: NF4 근사와 역양자화 때문에 LoRA 대비 과업 품질이 낮아지는 문제다.
- **커널 호환성**: 목표 GPU와 소프트웨어가 NF4 저장·역양자화 연산을 지원하는 성질이다.
- **이동 병목**: 상태 페이징의 CPU·GPU 전송 시간이 학습 속도를 제한하는 문제다.
...
- **LoRA 선택 기준**: 고정밀 원본을 적재할 메모리가 충분하고 양자화 오차를 피해야 할 때 사용한다.
- **QLoRA 선택 기준**: GPU 메모리가 병목이고 NF4 품질·커널·전송 비용이 허용 범위일 때 사용한다.
