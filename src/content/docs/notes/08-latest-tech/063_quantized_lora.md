...
- **4비트 정규 부동소수점(NormalFloat 4-bit, NF4) 가중치 저장소**: 정규분포 가중치에 맞춘 4비트 코드와 양자화 상수로 동결 기반 모델을 압축 저장한다.
- **역양자화**: 압축 가중치를 행렬 연산에 사용할 계산 자료형으로 복원하는 연산이다.
- **저랭크 적응(Low-Rank Adaptation, LoRA) 학습 경로**: 동결된 기반 가중치에 더할 저랭크 변화량만 학습하는 경로이다.
...
- **저랭크 적응(Low-Rank Adaptation, LoRA)**: 고정밀 기반 모델을 적재하고 저랭크 변화량만 학습한다.
- **양자화 저랭크 적응(Quantized Low-Rank Adaptation, QLoRA)**: 4비트 기반 모델을 적재하고 계산 시 복원하여 저랭크 변화량만 학습한다.
