...
- 정의/개념: 4비트 양자화된 기반 모델 위에 LoRA를 적용하여 극도로 낮은 메모리 사용량으로 대형 모델을 학습하는 기법.
- 배경/필요성: 고정밀 모델 적재에 따른 GPU 메모리 한계를 극복하고 효율적인 모델 배포를 지원하기 위함.

#### 한줄 요약
- **4비트 양자화와 LoRA 결합** 체계 적용
...
- NF4 및 이중 양자화로 동결 가중치 메모리 점유율을 대폭 감소.
- 혼합 정밀도 연산을 통한 데이터 효율적 학습 지원.
- 페이지드 옵티마이저를 통한 GPU 메모리 피크 관리 및 학습 안정성 확보.
