...
- 정의/개념: 4비트 기반 모델을 역양자화하고 **LoRA 경로**만 갱신하는 QLoRA
- 배경/필요성: 고정밀 기반 가중치 적재로 **제한된 GPU 메모리**에서 대형 모델 학습 곤란

#### 한줄 요약
- **4비트 동결 원본•LoRA 경로** 결합 학습
...
- **NF4 저장소•역양자화•LoRA•페이지드 옵티마이저** 구성
...
#### 한줄 요약
- **갱신 범위•원본 정밀도**로 Full FT•LoRA•QLoRA 구분
...
#### 한줄 요약
- **GPU 메모리•품질•커널**에 따라 LoRA•QLoRA 선택
