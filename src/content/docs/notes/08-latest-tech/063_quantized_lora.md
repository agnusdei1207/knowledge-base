...
- 배경/필요성: 고정밀 기반 가중치 적재로 **제한된 GPU 메모리**에서 대형 모델 학습이 어렵다.
...
- **NF4•이중 양자화**로 동결 가중치 저장량을 축소한다.
- 계산 자료형 **역양자화•LoRA**를 결합한 혼합 정밀도 학습이 핵심이다.
- **페이지드 옵티마이저**로 순간 메모리 급증을 완화한다.
...
- **GPU 메모리•양자화 품질•커널 지원** 기준으로 **LoRA•QLoRA**를 선택한다.
