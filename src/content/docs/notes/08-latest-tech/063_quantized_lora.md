...
- 정의/개념: 4비트 기반 모델을 역양자화하고 **LoRA 경로**만 갱신하는 QLoRA이다.
- 배경/필요성: 고정밀 기반 가중치 적재로 **제한된 GPU 메모리**에서 대형 모델 학습하기 어렵다.

#### 한줄 요약
- **4비트 동결 원본•LoRA 경로** 결합 학습이 핵심이다.
...
- **NF4•이중 양자화**에 따른 동결 가중치 저장량을 축소한다.
- 계산 자료형 **역양자화•LoRA**를 결합한 혼합 정밀도 학습이 핵심이다.
- **페이지드 옵티마이저**를 활용한 순간 메모리 급증을 완화한다.
...
- **NF4 저장소•역양자화•LoRA•페이지드 옵티마이저**를 구성한다.
...
1. **NF4 가중치•상수 적재**: 기반 모델을 압축 상태로 고정해 **GPU 저장량**을 절감한다.
2. **선택 블록 역양자화**: 현재 연산에 필요한 가중치만 **계산 정밀도**로 복원한다.
3. **기반 출력•LoRA 변화량 합산**: 고정 기반 경로와 학습 경로의 **통합 출력**을 생성한다.
4. **LoRA 그래디언트 갱신**: 역전파 대상을 저랭크 행렬로 제한해 **학습 상태**를 축소한다.
...
- **LoRA 적용**: 고정밀 기반 모델과 저랭크 변화량 학습이다.
- **QLoRA 적용**: 4비트 기반 모델 복원과 저랭크 변화량 학습이다.

</details>

- 고정밀 원본은 **LoRA**, 4비트 원본은 **QLoRA**가 핵심이다.
...
#### 한줄 요약
- **갱신 범위•원본 정밀도**로 Full FT•LoRA•QLoRA을 구분한다.
...
- **GPU 메모리•양자화 품질•커널 지원** 기준으로 **LoRA•QLoRA**을 선택한다.

#### 한줄 요약
- **GPU 메모리•품질•커널**에 따라 LoRA•QLoRA을 선택한다.
