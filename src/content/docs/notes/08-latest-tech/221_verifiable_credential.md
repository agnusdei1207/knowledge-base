...
- **검증 가능 자격증명(VC)**: 발급자가 주체에 관한 주장에 암호학적 증명을 결합해 기계적으로 검증 가능하게 만든 자격 데이터이다.
- **검증 가능 프레젠테이션(VP)**: 보유자가 하나 이상의 자격증명에서 필요한 정보와 보유 증명을 묶어 제시한 객체이다.
- **자격 증명•상태**: 발급자 서명과 스키마를 확인하고 폐기•정지•유효기간 상태를 검증하는 정보이다.

</details>

발급자는 **검증 가능 자격증명(Verifiable Credential, VC)** 에 암호학적 증명을 결합하고 보유자는 **검증 가능 프레젠테이션(Verifiable Presentation, VP)** 을 구성해 검증자에게 제시한다.
...
- 표준화된 자격증명 교환과 선택적 공개 지원은 **월드 와이드 웹 컨소시엄(World Wide Web Consortium, W3C) 검증 가능 자격증명(Verifiable Credential, VC)**, 기관 최신 상태 직접 확인은 **중앙 응용 프로그래밍 인터페이스(Application Programming Interface, API)** 선택
