...
| 고려사항 | 위험 | 대책 |
|:---|:---|:---|
| **발급자·키 신뢰** | 위조 발급자나 탈취 키의 자격을 수락 | 신뢰 등록부, 키 회전·폐기 이력, 발급 정책 검증 |
| **재사용·상관관계** | VP 재전송과 반복 식별로 사생활 침해 | 챌린지·대상·만료 결합, 선택적 공개와 쌍별 식별자 |
| **상태 조회 추적** | 취소 확인 과정에서 보유자 활동 노출 | 개인정보 보호 상태 목록, 캐시와 최소 조회 설계 |
