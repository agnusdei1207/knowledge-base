...
sequenceDiagram
    ...
    C->>G: 1. 인증·요청 수신
    G->>R: 2. 모델·복제본 라우팅
    R->>S: 3. 배치 편성·KV 메모리 할당
    S->>E: 4. 프리필·반복 디코드
    E-->>C: 5. 토큰 스트리밍·지표 수집
```

1. **인증·요청 수신**: 권한·할당량·입력 형식을 검사해 **처리 가능 요청** 생성
2. **모델·복제본 라우팅**: 과업과 부하에 맞는 **모델 인스턴스** 선택
3. **배치 편성·KV 메모리 할당**: 길이·우선순위에 따라 실행 순서와 **캐시 공간** 배정
4. **프리필·반복 디코드**: 입력을 처리한 뒤 캐시를 재사용해 **출력 토큰** 생성
5. **토큰 스트리밍·지표 수집**: 결과를 전송하며 TTFT·TPOT·처리량을 **운영 지표로 기록**
...
- p95 TTFT·TPOT·Goodput·요청당 비용을 함께 만족하는 **라우팅·배치·캐시 구조**를 선택해야 한다.
