...
- 정의/개념: 언어 모델 추론을 다중 요청에 제공하며 자원•지연•품질을 통제하는 **LLM Serving**
- 배경/필요성: 가변 길이•동시 요청은 **GPU 메모리•지연 변동**을 유발해 서비스 목표 보장 곤란

#### 한줄 요약
- **접수•라우팅•스케줄링•추론•운영 통제** 체계
...
- 양자화•병렬화•**KV Cache 관리 기반 지연 감소•처리량 향상**
...
3. **연속 배치•KV 할당**: 길이•우선순위별 실행 순서와 캐시 배정
4. **프리필•KV 캐시 생성**: 입력 어텐션을 계산해 초기 키•값 저장
5. **다음 토큰 디코드**: 캐시를 재사용해 토큰을 반복 생성
...
#### 한줄 요약
- **응답 시급성•처리량**에 따라 온라인•비동기•오프라인 선택
...
- 실시간에는 **온라인 동적 배치**, 완료 유연 작업에는 **비동기 API**, 대량 처리에는 **오프라인 추론** 선택

#### 한줄 요약
- **응답 시급성•작업량**에 따라 서빙 방식 선택
