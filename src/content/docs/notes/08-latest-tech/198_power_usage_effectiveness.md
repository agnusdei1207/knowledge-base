...
- **데이터센터 인프라 효율(DCiE)**: IT 에너지를 총 시설 에너지로 나누어 백분율로 표시한 PUE의 역수 지표이다.
- **해석 경계**: PUE가 냉각·배전 효율만 나타내며 IT 작업 효율·탄소·물 사용량은 직접 나타내지 않는 범위이다.
...
- **총 시설 계측**: 정한 데이터센터 경계 안의 IT·배전·냉각·조명 에너지를 모두 측정하는 기능이다.
- **IT 장비 계측**: 서버·저장·네트워크 장비가 실제 사용한 에너지를 같은 기간에 측정하는 기능이다.
