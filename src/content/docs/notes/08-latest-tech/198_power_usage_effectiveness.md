...
- 정의/개념: **PUE**는 같은 기간의 총 시설 에너지를 **IT 장비 에너지**로 나눈 비율
- 배경/필요성: 총전력량만으로는 IT 소비와 냉각•배전의 **시설 오버헤드** 구분 곤란

#### 한줄 요약

- 총 시설 150•IT 100이면 PUE 1.5와 시설 오버헤드 50
...
1. **총•IT 에너지 전달**: 같은 기간의 **두 에너지 값** 제공
2. **PUE•오버헤드 개선안 전달**: IT 부하•기후 기반 **냉각•배전 개선 조건** 제공

#### 한줄 요약

- 동일 계절•IT 부하의 PUE 기반 시설 전력 개선 검증
...
- **단독 최적화**: PUE만 낮추면서 총전력•물•탄소•장비 효율•서비스 신뢰성의 악화를 놓치는 접근이다.
...
- 시설 전체는 **PUE**, 구역 병목은 **pPUE**로 비교하고 WUE•탄소는 분리 평가
