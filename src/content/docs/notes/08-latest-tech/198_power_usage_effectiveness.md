...
- 정의/개념: **PUE**는 같은 기간의 총 시설 에너지를 **IT 장비 에너지**로 나눈 비율임.
...
- **PUE**가 1에 가까울수록 비IT 오버헤드 감소•**DCiE** 증가함.
- 부하•기후 영향을 고려한 IT 효율과 **탄소•물 지표 병행**이 핵심임.
#### 한줄 요약
...
1. **총•IT 에너지 전달**: 같은 기간의 **두 에너지 값**을 제공함.
2. **PUE•오버헤드 개선안 전달**: IT 부하•기후 기반 **냉각•배전 개선 조건**을 제공함.
...
- **보조 지표**: PUE가 표현하지 못하는 WUE•탄소•IT 작업 효율•SLO를 함께 판단하는 값을 의미한다.

</details>

- 시설 전체는 **PUE**, 구역 병목은 **pPUE**로 비교하고, 물 사용량과 탄소 배출량은 별도 지표로 평가함.
