...
- 정의/개념: **PUE**는 같은 기간의 총 시설 에너지를 **IT 장비 에너지**로 나눈 비율이다.
- 배경/필요성: 총전력량만으로는 IT 소비와 냉각•배전의 **시설 오버헤드** 구분하기 어렵다.

#### 한줄 요약

- 총 시설 150•IT 100이면 PUE 1.5와 시설 오버헤드 50가 핵심이다.
...
- **데이터센터 인프라 효율(Data Center infrastructure Efficiency, DCiE)**: IT 에너지를 총 시설 에너지로 나눈 PUE의 역수 지표이다.
...
- **ISO/IEC 30134-2:2026** 기반 동일 경계•기간 계측이 핵심이다.
- **PUE**가 1에 가까울수록 비IT 오버헤드 감소•**DCiE** 증가한다.
- 부하•기후 영향을 고려한 IT 효율과 **탄소•물 지표 병행**이 핵심이다.
#### 한줄 요약
...
- **오버헤드 분해**: 총 시설과 IT 에너지의 차이를 냉각•배전•조명 등 원인별로 나누는 분석하는 개념이다.
...
1. **총•IT 에너지 전달**: 같은 기간의 **두 에너지 값**을 제공한다.
2. **PUE•오버헤드 개선안 전달**: IT 부하•기후 기반 **냉각•배전 개선 조건**을 제공한다.

#### 한줄 요약

- 동일 계절•IT 부하의 PUE 기반 시설 전력 개선을 검증한다.
...
- **부분 전력 사용 효과성(partial Power Usage Effectiveness, pPUE)**: 명시한 구역•모듈의 총에너지를 해당 IT 에너지로 나눈 지표이다.
...
- **단독 최적화**: PUE만 낮추면서 총전력•물•탄소•장비 효율•서비스 신뢰성의 악화를 놓치는 접근하는 개념이다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| **계측 경계 차이** 미검증 시 서로 다른 설비 범위로 PUE 비교 왜곡 | 계측 위치•포함 설비•기간을 명시하고 동일 조건 비교 | **PUE 동일 경계 비교성** 확보 |
| **부하•계절 영향** 미검증 시 낮은 IT 부하나 기후 차이를 개선으로 오판 | **IT 부하•외기 온도** 기록과 연간•부하구간별 분석 | 부하•계절 **효과 오판** 감소 |
| **PUE 단독 최적화** 미검증 시 물•장비 효율•신뢰성 악화 | 총에너지•WUE•IT 작업 효율•온도•**SLO 공동 검증** | 물•탄소•**신뢰성 악화** 방지 |
...
- **동일 경계 비교성**: 포함 설비•계측 위치•기간이 같아 두 PUE 값의 차이를 해석할 수 있는 성질이다.
- **보조 지표**: PUE가 표현하지 못하는 WUE•탄소•IT 작업 효율•SLO를 함께 판단하는 값이다.

</details>

- 시설 전체는 **PUE**, 구역 병목은 **pPUE**로 비교하고, 물 사용량과 탄소 배출량은 별도 지표로 평가한다.
