...
- 정의/개념: 중앙 컴퓨팅•서비스 기반으로 차량 기능을 지속 갱신하는 **SDV 구조**이다.
- 배경/필요성: 기능별 분산 ECU 증가는 **통합 복잡도•변경 비용**을 초래한다.
...
- ECU를 HPC•구역 제어기로 통합하는 **중앙 컴퓨팅•존 구조**가 핵심이다.
- 공통 API로 장치 의존성을 분리하는 **하드웨어 추상화•서비스화**가 핵심이다.
- 검증•OTA Update•관측을 통한 **전 생애주기 갱신**이 핵심이다.
#### 한줄 요약
...
1. **서비스 계약•권한 확인**: 호출자•데이터•장치 접근 계약을 검증한다.
2. **자원•안전 영역 배치**: OS가 실행 자원과 격리•우선순위를 관리한다.
3. **구역 제어 명령**: 중앙 계산 결과를 구역 제어기로 전달한다.
...
- 변경이 잦은 기능은 **서비스화**, 안전 기능은 **격리 실행**하는 SDV을 적용한다.
