...
## Ⅰ. 개요
- 정의/개념: **MCP 도구**는 서버가 이름·설명·입력 스키마로 공개하고, 모델의 호출 제안을 검증한 뒤 외부 시스템에서 실행하는 기능이다.
...
## Ⅵ. 실무 사례
1. 서버 운영 MCP 도구가 재시작 대상과 권한을 검증하고 담당자 승인을 받은 뒤 명령을 실행하며 결과를 감사 로그로 남긴다.

### 쉽게 이해하기 (학습용)
- 서버를 다시 켜는 명령은 대상을 확인하고 사람 허락 뒤 실행함

## Ⅶ. 결론
- 외부 시스템의 상태를 안전하게 변경하기 위해 입력 스키마·최소 권한·부작용과 인간 승인 절차를 검토하여 **MCP 도구**를 활용해야 한다.
