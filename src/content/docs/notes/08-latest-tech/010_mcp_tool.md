...
date: "2026-07-30T15:19:00+09:00"
tags:
  - "notes-latest_tech"
weight: 10
extra:
...
| 절차 | 설명 |
|:---|:---|
| 도구 목록·입력 스키마 요청 | 협상된 서버에서 사용 가능한 행동과 인자 계약 탐색 |
| 선택 가능한 도구 명세 전달 | 호스트가 출처·위험과 함께 모델에 노출할 기능 결정 |
| 모델 제안·사용자 승인 결과 제공 | 도구·인자를 선택하고 고위험 호출의 동의 여부 판정 |
| 승인된 도구명·구조화 인자 호출 | MCP 클라이언트가 tools/call 요청 전송 |
| 권한·입력 검증 후 백엔드 실행 | 서버가 주체·대상·업무 규칙을 재검증하고 결과·오류 반환 |
