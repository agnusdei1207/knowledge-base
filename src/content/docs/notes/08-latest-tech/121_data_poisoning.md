...
- 기존 한계: 입력 단계의 검증만으로는 오염 데이터가 학습 결과에 내재화돼 모델 재배포 후에도 지속적인 오판을 일으키는 문제를 막기 어렵다.
...
1. 스팸 신고 자료를 **신뢰 구간별 격리**해 후보 모델을 평가한다
...
- 학습 데이터 오염으로 판단이 왜곡되는 것을 막기 위해 **데이터 계보·출처 신뢰도·기여량·격리 전후 행동 변화**를 검토하고, 오염 데이터 제거와 재학습으로 통제한다
