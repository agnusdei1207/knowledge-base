...
1. 소프트웨어 변경 작업에서 구현 에이전트와 검증 에이전트를 분리하고, 테스트 근거를 공유 상태에 기록하여 결과를 독립적으로 검토한다.
...
- 복합 과업의 전문성과 병렬성을 높이기 위해 과업 의존성·역할 독립성·상태 공유·검증 및 조정 비용을 검토하여 **멀티 에이전트 협업**을 활용해야 한다.
