...
| 고려사항 | 위험 | 대책 |
|:---|:---|:---|
| **측정 경계 변경** | 개선 전후의 포함 자원이 달라 SCI 왜곡 | 기능 단위·자원·기간 기준선 고정과 변경 이력 관리 |
| **반동 효과** | 기능당 효율 개선 후 사용량 증가로 총배출 확대 | SCI와 총에너지·총배출을 함께 예산화·감시 |
| **품질 저하** | 작업 이동·자원 축소로 지연과 장애 증가 | SLO를 제약 조건으로 두고 단계 적용·자동 복귀 |
