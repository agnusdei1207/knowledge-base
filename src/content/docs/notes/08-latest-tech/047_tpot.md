...
- **키-값 캐시(KV Cache)**: 이전 토큰의 K·V를 저장하여 다음 토큰 생성에 재사용하는 메모리다.
...
- **신규 KV 추가**: 선택한 토큰의 K·V를 캐시에 덧붙여 다음 디코드 단계에서 재사용하는 과정이다.
...
- **TTFT**: 요청부터 첫 토큰까지 초기 반응성을 측정한다.
- **TPOT**: 첫 토큰 뒤 개별 스트리밍 생성 속도를 측정한다.
...
- **p95 토큰 간격**: 전체 토큰 간 지연의 95%가 이하에 놓이는 경계로 불규칙한 정지를 드러낸다.
...
- **KV 공유·양자화**: 대역폭 병목일 때 캐시 데이터량을 줄여 TPOT를 개선한다.
