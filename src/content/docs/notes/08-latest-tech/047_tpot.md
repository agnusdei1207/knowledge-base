...
- 정의/개념: 첫 토큰 이후 출력 토큰 하나를 생성하는 평균 지연인 **TPOT**이다.
- 배경/필요성: TTFT만으로는 스트리밍 시작 후 **디코드 지속 속도**의 평하기 어렵다.

#### 한줄 요약
- 첫 토큰 이후의 **토큰당 평균 생성 지연**이 핵심이다.
...
- `TPOT = (마지막 토큰 수신 시각 - 첫 토큰 수신 시각) / (출력 토큰 수 - 1)`의 **평균 토큰 간 지연**이 핵심이다.
- 단일 연속 생성에서 `출력 속도 ≈ 1000 / TPOT(ms/token)`인 **지연•속도 역관계**가 핵심이다.
- KV Cache 대역폭이 낮거나 배치•동시성이 크면 **디코드 지연 증가**가 핵심이다.

#### 한줄 요약
- 낮은 **TPOT**는 빠른 스트리밍 지속 속도가 핵심이다.
...
1. **활성 요청 배치 편성**: 실행 가능한 요청을 묶어 **디코드 순서**를 결정한다.
2. **이전 키•값 조회**: 각 요청의 과거 문맥에 해당하는 **캐시 데이터**를 전달한다.
3. **다음 토큰 로짓 계산**: 모델 계층을 실행해 후보별 **생성 점수**를 산출한다.
4. **토큰 선택•신규 값 추가**: 다음 토큰을 확정하고 해당 토큰의 **키•값 캐시**를 확장한다.
5. **토큰 전송•간격 기록**: 수신 시각 차이로 **토큰 간 밀리초 지연**을 측정한다.

#### 한줄 요약
- **배치•조회•로짓•선택•전송**을 토큰마다 반복이 핵심이다.
...
#### 한줄 요약
- 시작은 **TTFT**, 지속은 **TPOT**, 전체 효율은 처리량이 핵심이다.
...
- p95 TPOT의 원인이 대역폭이면 **KV 공유•캐시 양자화**, 배치면 **동시 요청 상한**을 적용한다.
