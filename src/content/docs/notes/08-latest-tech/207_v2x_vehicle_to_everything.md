...
- RSU가 사각지대 차량의 위치·속도를 서명 메시지로 전달하면 차량은 자체 센서와 최신성을 대조한 뒤 운전자에게 충돌 위험을 경고한다.

## Ⅶ. 결론

- 자체 센서만으로 보이지 않는 교통 위험을 조기에 공유하기 위해 위험 범위·지연·통신 경로·메시지 신뢰·센서 일치성을 검토하여, V2X를 검증된 보조 센서로 활용해야 한다.
