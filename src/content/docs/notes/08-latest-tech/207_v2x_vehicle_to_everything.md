...
- 정의/개념: 차량이 주변 참여자•인프라•망과 위험 정보를 교환하는 **V2X 체계**이다.
- 배경/필요성: 탑재 센서만으로는 사각지대와 **가시거리 밖 위험**을 조기 인지할 수 없다.
...
- V2V•V2I•V2P•V2N **다중 교통 참여자 통신**이 핵심이다.
- PC5•Uu 기반 **저지연 직접•광역 망 통신**이 핵심이다.
- 서명•시간•위치•센서 일치성 기반 **메시지 신뢰 검증**이 핵심이다.
#### 한줄 요약
...
1. **서명 상태•위험정보 전달**: 위치•속도•사건•발생 시각을 표준 메시지로 구성•서명한다.
2. **V2X 메시지 전달**: 인접 안전정보는 PC5, 광역 서비스는 Uu 경로로 교환한다.
3. **인증서•신선도 전달**: 송신 인증서•서명•발생 시각•재전송 여부를 검사한다.
4. **검증•융합 정보 전달**: 위치•물리 가능성을 차량 센서와 대조해 경고•보조를 결정한다.
...
- 인접 위험은 **PC5**, 광역 서비스는 **Uu**로 교환하고 PKI•센서로 검증한다.
