...
- 배경/필요성: 기존 검증 방식은 자격이나 거래 사실을 증명하려면 비밀번호·소득·거래 원본 등 불필요한 정보까지 공개해야 한다.
...
- **1. 완전성**: 참인 명제와 올바른 증거로 만든 증명을 검증자가 수락한다.
- **2. 건전성**: 거짓 명제를 속여 통과시킬 확률을 무시할 만큼 작게 제한한다.
- **3. 영지식성**: 검증 결과 외에 비밀 증거에 관한 정보를 드러내지 않는다.
### 쉽게 이해하기 (학습용)
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| **회로·명제 오류** 미검증 시 잘못 정의한 조건을 완벽히 증명 | 회로 검토·형식 검증, 경계값·악성 입력 시험 | 명제·업무 조건 **의미 일치** 확보 |
| **신뢰 설정 훼손** 미검증 시 비밀 설정값 유출 시 위조 증명 가능 | 다자 설정 의식, 폐기 증적 또는 투명 설정 방식 선택 | 설정 유출 **위조 증명** 제거 |
| **외부 입력 신뢰** 미검증 시 수학적 증명은 맞지만 현실 입력이 허위 | 공인 발급자 서명, 제출자·챌린지 결합, 재사용 방지 | 현실 입력·증명 **신뢰 연계** 확보 |
