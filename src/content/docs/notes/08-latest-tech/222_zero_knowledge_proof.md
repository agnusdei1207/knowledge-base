...
date: "2026-08-14T13:38:00+09:00"
tags:
- "notes-latest-tech"
weight: 222
extra:
...
- 정의: **ZKP** 기반 증명자가 비밀 증거를 공개하지 않고 공개 명제가 참임을 검증자에게 입증하는 암호 기술임이다.
- 배경/필요성: 원본 제출 검증은 비밀번호•소득•거래의 **과다 공개** 활용 유발함.
...
- 참인 명제의 올바른 증명을 수락하는 **완전성** 핵심임.
- 거짓 명제의 수락 확률을 제한하는 **건전성** 핵심임.
- 검증 결과 외 비밀을 숨기는 **영지식성** 핵심임.
#### 한줄 요약
...
[증명자] ───── proof•공개 명제 제출 ─────▶ [검증자]
[검증자] ── 3. 검증 매개변수 조회 ───────▶ [매개변수 체계]
[검증자] ◀──── 검증 키•회로 정보 ───────── [매개변수 체계]
[검증자] ───── 검증 결과•업무 근거 ──────▶ [정책 판단]
[증명자] ◀──────── 수락•거부 결과 ───────── [정책 판단]
```

### 동작 원리

1. 명제•관계 회로 정의: 공개 명제•비밀 증거의 만족 조건을 표현함.
2. 증명 매개변수 제공: 신뢰 설정•투명 공개 매개변수를 준비함.
3. 검증 매개변수 조회: proof 검증용 키•회로 정보를 확인함.
...
> 요약: **zk-SNARK** 기반 간결성, **zk-STARK** 기반 투명 설정•확장성 중심이 핵심이다.
...
- 작은 증명•온체인은 **zk-SNARK**, 투명 설정•대형 계산은 **zk-STARK** 활용 선택함.

#### 한줄 요약

- 수학적 증명과 현실 입력•제출자 진위를 분리해 검증.
