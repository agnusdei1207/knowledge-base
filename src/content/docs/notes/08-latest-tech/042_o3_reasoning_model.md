...
- **o3 추론 모델(o3 Reasoning Model)**: 복합 수학·과학·코딩 과업에서 시험 시점 연산과 도구 사용을 결합한 OpenAI 추론 모델 사례다.
- **시험 시점 연산**: 요청 처리 중 풀이·탐색·검증에 추가 토큰과 시간을 투입하는 방식이다.
...
- **응용 프로그래밍 인터페이스(API)**: 애플리케이션이 모델과 도구를 호출하고 결과를 받는 접점이다.
