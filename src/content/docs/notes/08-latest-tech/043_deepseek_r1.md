...
- **DeepSeek-R1**: DeepSeek-V3-Base에 콜드 스타트 데이터와 다단계 강화학습을 적용한 추론 언어 모델
- **DeepSeek-R1-Zero**: 선행 지도 미세조정 없이 대규모 강화학습을 적용해 추론 행동의 출현을 실험한 모델
- **DeepSeek-V3-Base**: R1-Zero와 R1 후학습이 출발한 기반 모델
- **콜드 스타트(Cold Start)**: 강화학습 전에 소량의 고품질 추론 예시로 응답 형식과 초기 행동을 안내하는 단계
- **지도 미세조정(Supervised Fine-Tuning, SFT)**: 정답·풀이 예시로 모델 응답을 조정하는 학습 방법
- **강화학습(Reinforcement Learning, RL)**: 모델 응답의 보상을 기준으로 문제 해결 정책을 개선하는 학습 방법
...
| GRPO·검증 보상 | 정답 판정 기반 **추론 정책 강화** |
| 거부 표본·후속 학습 | 양질 응답 선별과 **추론·일반 능력 보완** |
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 변형별 **기반·크기 혼동** | 정확한 모델 ID·파라미터·체크포인트 확인 | 성능·자원 비교의 **정합성 확보** |
