...
- **그룹 상대 정책 최적화(GRPO)**: 후보별 상대 보상으로 정책 갱신
- **혼합 전문가(MoE)**: 일부 전문가만 활성화해 연산 최적화
- **다중 헤드 잠재 어텐션(MLA)**: 키·값 캐시 표현 압축
...
- R1-Zero는 SFT 없이 RL 가능성을 실험한다
- R1은 콜드 스타트·RL·SFT를 결합한다
- V3의 MoE·MLA가 연산·캐시 비용을 줄인다
- R1 생성 데이터가 소형 모델 증류를 지원한다
...
| 판단 | R1-Zero | R1 |
|:---|:---|:---|
| 초기 | SFT 없이 RL 적용 | Cold-start SFT 후 RL |
| 적용 | 순수 RL 연구 | 가독성·일반 능력 |
| 후속 | 실험 결과 제공 | 샘플링·SFT·RL 결합 |
