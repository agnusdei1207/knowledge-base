...
- 정의/개념: DeepSeek-V3-Base에 콜드 스타트•다단계 RL을 적용한 **DeepSeek-R1 추론 언어 모델**이다.
- 배경/필요성: 추론 예시 중심 SFT만으로는 정답을 탐색•검증하는 **자발적 해결 정책**을 확장하기 어렵다.
...
- R1-Zero의 선행 SFT 없는 **대규모 RL 추론 행동 실험**이 핵심이다.
- R1의 콜드 스타트와 다단계 학습을 결합한 **가독성•추론 성능 보완**이 핵심이다.
- R1 생성 데이터를 Qwen•Llama 기반 모델에 이전한 **증류 모델 제공**이 핵심이다.
...
1. **고품질 추론 예시 SFT**: 콜드 스타트 데이터로 **가독성•응답 형식**을 설정한다.
2. **GRPO•검증 보상 학습**: 정답 판정과 상대 보상으로 **추론 정책**을 강화한다.
3. **후보 응답 생성•선별**: 정확성•가독성 기준을 통과한 **학습 데이터**를 구성한다.
4. **선별 데이터 SFT•후속 RL**: 추론과 일반 과업 데이터를 결합해 **최종 R1**을 조정한다.
5. **Qwen•Llama 기반 증류**: R1 생성 데이터로 작은 기반 모델의 **추론 능력을 이전**한다.
...
- **제한 자원 배포**: GPU 메모리나 처리량이 제한된 환경에서 모델을 운영하기 위해 증류•양자화 등으로 크기를 줄여 배포하는 방식이다.

</details>

- 순수 RL 연구에는 **R1-Zero**, 범용 추론에는 **R1**, 제한 자원 배포에는 **증류 모델**을 선택한다.
