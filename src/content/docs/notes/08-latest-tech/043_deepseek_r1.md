...
- **지도 미세조정(SFT)**: 정답·풀이 예시로 모델 응답 형식과 행동을 조정하는 학습 방법이다.
- **그룹 상대 정책 최적화(GRPO)**: 후보 응답 그룹의 상대 보상을 이용해 정책을 갱신하는 강화학습 기법이다.
...
- **모델 ID**: 원본·증류 여부와 기반·크기·체크포인트를 구분하는 정확한 모델 식별자다.
...
- **R1-Zero 선택 기준**: 선행 SFT 없는 강화학습 추론 행동을 연구할 때 사용한다.
