...
- 정의/개념: KV Cache를 고정 크기 비연속 블록에 할당하고 블록 테이블로 논리 순서를 복원하는 **PagedAttention**임.
...
- **쓰기 시 복사(Copy-on-Write, COW)**: 공유 블록을 수정할 때 기존 블록은 유지하고 해당 요청의 사본만 만드는 방식을 말한다.

</details>

- 논리 KV 순서와 비연속 물리 블록을 연결하는 **주소 매핑**이 핵심임.
- 고정 블록의 동적 할당•회수로 **단편화•과예약**을 완화함.
- 공통 접두사•빔 탐색 블록의 **공유•쓰기 시 복사**가 핵심임.
...
- **물리 블록 풀**: GPU의 고정 크기 캐시 블록을 동적 할당하고 종료 후 회수하는 저장 영역을 의미한다.
...
- **쓰기 시 복사 원자성(Copy-on-Write Atomicity, COW Atomicity)**: 공유 블록의 복사와 참조 갱신이 중간 상태 없이 하나의 연산처럼 완료되어야 하는 성질을 의미한다.
...
- 가변 길이•고동시성에는 **PagedAttention**, 블록 크기에는 **단편화•조회 비용 실측**을 적용함.

#### 한줄 요약
- **길이 분포•동시성•조회 비용**으로 할당 방식을 선택한다.
