...
- **물리 블록 풀**: GPU의 고정 크기 캐시 블록을 요청에 동적 할당하고 종료 후 회수하는 저장 영역이다.
...
3. **KV 조회·신규 블록 연결**: 기존 캐시를 읽고 블록이 차면 **새 물리 블록** 추가
...
- **연속 KV 할당**: 요청별 최대 길이에 맞춘 연속 공간을 미리 예약하여 주소 계산은 단순하지만 과다 예약과 단편화가 발생한다.
- **PagedAttention**: 필요한 고정 블록만 비연속 할당하여 가변 길이·고동시성 요청의 메모리 활용률을 높인다.
...
- **COW 원자성**: 공유 블록의 복사와 참조 갱신이 중간 상태 없이 하나의 연산처럼 완료되어야 하는 성질이다.
...
- **PagedAttention 선택 기준**: 요청 길이가 다양하고 동시성이 높아 연속 캐시 예약의 낭비가 클 때 적용한다.
- **블록 크기 선택**: 내부 단편화와 주소 조회 비용을 실제 요청 길이 분포와 커널 처리량으로 함께 측정해 결정한다.

</details>

- 가변 길이·고동시성은 **PagedAttention**, 블록 크기는 **단편화·조회 비용** 실측 선택
