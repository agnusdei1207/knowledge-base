...
- 정의/개념: KV Cache를 고정 크기 비연속 블록에 할당하고 블록 테이블로 논리 순서를 복원하는 **PagedAttention**이다.
- 배경/필요성: 연속 KV 공간 예약은 가변 길이 요청에서 **메모리 단편화•과다 예약**을 유발한다.

#### 한줄 요약
- **비연속 물리 블록•블록 테이블**로 KV 순서 복원이 핵심이다.
...
- 논리 KV 순서와 비연속 물리 블록을 연결하는 **주소 매핑**이 핵심이다.
- 고정 블록의 동적 할당•회수에 의한 **단편화•과예약 완화**가 핵심이다.
- 공통 접두사•빔 탐색 블록의 **공유•쓰기 시 복사**가 핵심이다.

#### 한줄 요약
- **접두사 공유•COW**로 분기 블록만 선택 복사가 핵심이다.
...
- **물리 블록 풀**: GPU의 고정 크기 캐시 블록을 동적 할당하고 종료 후 회수하는 저장 영역이다.
- **페이지드 커널**: 블록 테이블을 따라 비연속 KV 블록을 조회하여 어텐션을 계산하는 전용 연산 커널이다.
...
1. **물리 블록 할당•주소 기록**: 예상 토큰 수에 맞는 블록을 선택하고 논리 번호와 **매핑 관계**를 저장한다.
2. **논리 순서별 물리 주소 전달**: 커널이 토큰 순서대로 읽을 **비연속 주소 목록**을 제공한다.
3. **KV 조회•신규 블록 연결**: 기존 캐시를 읽고 블록이 차면 **새 물리 블록**을 추가한다.
4. **참조 블록 회수**: 공유 참조가 없는 블록을 풀에 반납해 **재사용 가능 상태**를 전환한다.
...
#### 한줄 요약
- 고정 저동시성은 **연속 할당**, 가변 고동시성은 **PagedAttention**가 핵심이다.
...
#### 한줄 요약
- **블록 크기•COW 원자성**으로 공간•조회 비용 절충이 핵심이다.
...
- 가변 길이•고동시성에는 **PagedAttention**, 블록 크기에는 **단편화•조회 비용 실측**을 적용한다.
