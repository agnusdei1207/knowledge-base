...
- 정의/개념: **MDP**는 상태•행동•전이확률•보상으로 순차 의사결정을 표현하는 수학 모델이다.
- 배경/필요성: 단발 의사결정의 **미래 상태•장기 보상** 미반영이 발생한다.
...
- 다음 전이에 필요한 과거 정보를 현재 상태로 요약하는 **마르코프 성질**이 핵심이다.
- 상태•행동별 환경 동역학을 정의하는 **전이확률•보상 함수**가 핵심이다.
- 행동 선택과 장기 가치 계산을 결정하는 **정책•할인율 $\gamma$**가 핵심이다.
...
- **전이확률(Transition Probability)**: 현재 상태와 행동이 주어졌을 때 다음 상태가 발생할 확률을 정의하는 개념이다.
- **보상 함수**: 상태와 행동 또는 상태 전이에서 얻는 즉시 성과를 수치로 반환하는 함수이다.
- **할인율(Discount Factor)**: 미래 보상의 현재 가치를 조정하여 단기•장기 성과의 비중을 정하는 개념이다.
- **정책(Policy)**: 각 상태에서 선택할 행동의 확률분포나 결정 규칙이다.
...
1. **현재 상태**: 과거 이력의 전이 정보를 충분히를 요약한다.
2. **선택 행동**: 정책이 상태별 허용 행동에서 결정한다.
3. **다음 상태 전이**: 전이확률에 따라 환경 상태를 변화한다.
4. **즉시 보상•다음 상태**: 행동 결과의 성과와 후속 정보를 제공한다.
5. **할인 가치•정책 신호**: 미래 보상을 현재 가치에 반영한다.
...
- 상태 관측 가능성에 따라 **MDP•POMDP**를 선택하고 **보상 함수**를 검증한다.
