...
- 정의/개념: 계획•도구 호출•근거 평가를 반복하는 **에이전틱 검색 증강 생성(Agentic Retrieval-Augmented Generation, Agentic RAG) 방식**이다.
- 배경/필요성: 단일 RAG의 **복합 질의 분해•실패 복구**에 한계가 발생한다.
...
- 실행 구조: **계획기의 하위 목표 분해•상태 기반 실행**이 핵심이다.
- 재검색•전략 수정 기준: **근거 평가•반추**가 핵심이다.
- 결합 항목: **도구 라우팅•반복 종료 조건**이 핵심이다.
...
1. **하위 목표•완료 조건 저장**: **복합 질의의 검증 가능 단위를 분해**한다.
2. **목표별 도구 선택**: 스키마•권한에 맞는 **도구를 라우팅**한다.
3. **실행 결과•오류 기록**: **상태 메모리•남은 목표를 갱신**한다.
4. **근거 관련성•충분성 평가**: 출처 일치와 **증거 공백**을 판정한다.
5. **재탐색•종료 판정**: 근거 충족•예산 소진에 따른 **반복을 제어**한다.
...
- **근거 임계값•행동 예산**을 판정할 수 있는 복합 질의에 Agentic RAG을 선택한다.
