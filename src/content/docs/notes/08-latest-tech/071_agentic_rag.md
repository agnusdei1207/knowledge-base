...
- **에이전틱 RAG**: 에이전트가 목표를 분해하고 도구 호출과 근거 평가를 반복해 답변을 만드는 RAG 방식이다.
...
- **도구 라우팅**: 목표와 입력 스키마에 맞는 외부 도구를 선택해 호출하는 기능이다.
...
- **고정 RAG**: 미리 정한 단일 검색·생성 절차를 한 번 수행하는 방식이다.
- **워크플로 RAG**: 사전에 정의한 조건과 분기에 따라 여러 검색 단계를 수행하는 방식이다.
