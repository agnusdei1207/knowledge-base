...
- 배경/필요성: 파일•데이터별 탐색•조회 방식 차이로 **컨텍스트 재사용하기 어렵다**.
...
- **제어 축**: 애플리케이션이 조회•모델 문맥 포함을 결정한다.
...
| 변경 알림 관리자 | **subscriptions/listen•변경 알림** 관리 |
...
2. **URI 읽기 요청**: 선택한 리소스의 텍스트•바이너리 내용을 요구한다.
...
#### 한줄 요약
- **URI 읽기•변경 알림**으로 필요한 문맥만 최신화가 핵심이다.
...
#### 한줄 요약
- 문맥 조회는 **Resource**, 행동 실행은 **Tool**가 핵심이다.
...
#### 한줄 요약
- 민감 자료는 **최소 공개**, 빈번한 변경만 알림 구독이 핵심이다.
