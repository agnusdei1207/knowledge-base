...
| 구성요소 | 책임 |
|:---|:---|
| 리소스 명세 | URI·이름·**MIME Type** 제공 |
| URI 템플릿 | **URI Template** 기반 리소스 범위 표현 |
| 내용 제공기 | JSON-RPC 기반 **URI 콘텐츠** 반환 |
| 변경 알림 관리자 | **resources/subscribe·알림** 관리 |
...
- **MCP Server** URI 권한 검증 후 MIME•내용 반환
...
2. URI 읽기 요청: **리소스 읽기** 기반 선택한 텍스트•바이너리 내용을 요구함.
3. URI 해석•권한 검증: URI·테넌트·**접근 범위** 확인
4. 변경 알림 구독 등록: **resources/subscribe** 요청
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| URI 조작과 범위 밖 조회 | 템플릿·테넌트 **권한 재검증** | **경로·테넌트 우회** 방지 |
| 과다 제공과 문맥 오염·정보 노출 | 목적별 분할과 **최소 필드** | **문맥 품질·기밀성** 확보 |
| 빈번한 변경과 최신성 저하 | 알림 후 **필요 URI만 재조회** | **최신성·트래픽** 최적화 |
...
- 부작용 없는 순수 읽기 전용 데이터를 표준화된 URI 체계로 AI 모델의 추론 컨텍스트에 안전하게 주입하는 **컨텍스트 데이터 제공 표준(MCP Resource / URI & URI Template / MIME Types & JSON-RPC / Change Notification & Subscription)의 필수 규격**으로 확고히 자리 잡았으며, 실시간 스트리밍 데이터 및 RAG 검색 파이프라인의 표준 데이터 소스로 활용 범위가 넓어지는 가운데, 실무 MCP Resource 아키텍처 설계 시에는 **데이터 소스의 계층 구조를 직관적으로 표현하는 URI Template 명문화 및 MIME 타입(text/plain, application/json 등) 명시, 테넌트 식별자 및 경로 순회(Path Traversal) 공격을 방어하는 엄격한 URI 권한 검증, 데이터 변경 시 호스트가 즉시 갱신할 수 있도록 지원하는 resources/subscribe 알림 파이프라인 구축**을 결합하여 완벽한 데이터 기밀성과 문맥 최신성을 완성
