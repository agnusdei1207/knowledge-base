...
date: "2026-07-30T14:56:00+09:00"
tags:
  - "notes-latest_tech"
weight: 3
extra:
...
- 추론 단계에서 정보 공백을 식별해 검색·계산·실행 중 적합한 도구와 인자를 선택
- 도구가 반환한 근거와 오류를 관찰해 기존 가설과 다음 행동을 수정
- 근거 충분성·완료 조건과 비용·시간·반복 한도를 함께 적용해 루프를 종료
...
| 절차 | 설명 |
|:---|:---|
| 정보 공백·선택 행동 전달 | 완료 조건과 부족한 근거를 분석해 도구·인자 선택 |
| 도구·인자·권한 검증 결과 | 호출 형식과 행동 위험 및 실행 권한 확인 |
| 검증된 도구 실행 요청 | 제한된 환경에서 허가된 외부 행동 수행 |
| 실행 결과·오류 전달 | 도구 출력과 실패 원인을 관찰 처리기로 전송 |
| 출처가 확인된 관찰 제공 | 근거를 다음 추론에 반영해 완료·재시도·중단 판정 |
