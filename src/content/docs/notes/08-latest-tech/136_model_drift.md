...
1. 승인 품질•버전 기준 고정: 비교할 모델•전처리•지표의 **정상 상태** 활용 등록함.
2. 입력•예측•정답 신호 결합: 운영 사건별 분포•출력•실제 결과의 **관측 기록** 연결함.
3. 전체•구간 품질 비교: 평균과 집단별 성능•확률 보정의 **실제 이탈** 확인함.
4. 데이터•개념•구성 원인 분해: 입력 분포•정답 관계•계보 변화의 **원인 증거** 활용 대조함.
5. 복구 후보 검증•승격: 원천 수정•재학습•설정 복원의 **대안 효과** 활용 비교함.
...
- **데이터•개념•구성 원인** 대상 따라 원천 수정•재학습•설정 복원 선택
