...
- 정의/개념: 데이터•개념•서빙 변화로 모델 품질•출력이 기준선에서 이탈하는 **모델 드리프트 현상**이다.
- 배경/필요성: 단일 정확도만으로는 **데이터•개념•구성 변화** 중 품질 저하 원인 분리하기 어렵다.
...
- 데이터•개념•서빙 설정의 **복합 이탈 원인**이 핵심이다.
- 분포 경보와 정답 기반 **품질•확률 보정 영향 분리**가 핵심이다.
- 전체 평균과 **부분집단 성능 편차**를 비교한다.
...
1. **승인 품질•버전 기준 고정**: 비교할 모델•전처리•지표의 **정상 상태**를 등록한다.
2. **입력•예측•정답 신호 결합**: 운영 사건별 분포•출력•실제 결과의 **관측 기록**을 연결한다.
3. **전체•구간 품질 비교**: 평균과 집단별 성능•확률 보정의 **실제 이탈**을 확인한다.
4. **데이터•개념•구성 원인 분해**: 입력 분포•정답 관계•계보 변화의 **원인 증거**를 대조한다.
5. **복구 후보 검증•승격**: 원천 수정•재학습•설정 복원의 **대안 효과**를 비교한다.
...
- **데이터•개념•구성 원인**에 따라 원천 수정•재학습•설정 복원을 선택한다.
