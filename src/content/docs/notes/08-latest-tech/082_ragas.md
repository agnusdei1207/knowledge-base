...
- 정의/개념: 구조화 표본과 자동 지표로 RAG 품질을 반복 평가하는 **RAGAS 프레임워크**이다.
- 배경/필요성: 수작업 RAG 평가의 **반복 실험 비용•판정 편차**가 증가한다.
...
- 질의•응답•문맥•기준 정보의 **평가 스키마 구조화**가 핵심이다.
- 목적별 지표 조합: **충실성•문맥 정밀도•재현율**이 핵심이다.
- 평가 모델•프롬프트•반복 조건의 **재현 가능한 실행 기록**이 핵심이다.
...
1. **평가 목적•표본 스키마 정의**: 검색•생성 목표와 지표별 **필수 필드**를 구성한다.
2. **지표•평가 모델 선택**: 평가 질문에 맞는 점수 규칙과 **판정 모델**을 지정한다.
3. **프롬프트•버전•재시도 고정**: 변경 전후 비교가 가능한 **실행 조건**을 저장한다.
4. **표본 지표 점수 실행**: 문맥•응답별 자동 지표와 비용을 산출한다.
5. **대표 표본 인간 검증**: 자동 결과와 전문가 판정의 **일치도**를 대조한다.
...
- **평가 목적•스키마 완전성•판정 일치도**로 RAGAS 지표와 평가 모델을 선택한다.
