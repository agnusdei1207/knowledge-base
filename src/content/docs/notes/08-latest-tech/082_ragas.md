...
- 배경/필요성: 수작업 평가는 **반복 실험 비용•판정 편차 증가** #### 한줄 요약
...
- 평가 모델•프롬프트•반복 조건의 **재현 가능한 실행 기록** #### 한줄 요약
...
1. 평가 목적•표본 스키마 정의: 검색•생성 목표와 지표별 **필수 필드** 활용 구성함.
2. 지표•평가 모델 선택: 평가 질문에 맞는 점수 규칙과 **판정 모델** 지정함.
3. 프롬프트•버전•재시도 고정: 변경 전후 비교가 가능한 **실행 조건** 저장함.
4. 표본 지표 점수 실행: 문맥•응답별 자동 지표와 비용을 산출함.
5. 대표 표본 인간 검증: 자동 결과와 전문가 판정의 **일치도** 활용 대조함.
...
#### 한줄 요약
- **의미 이해•결정성•비용•편향** 대상 따라 평가 방식 구분
