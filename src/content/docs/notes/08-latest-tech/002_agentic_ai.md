...
- **인공지능 (Artificial Intelligence, AI)**: 목표 기반 판단·실행 자동화
- **대규모 언어 모델 (Large Language Model, LLM)**: 계획·성찰·행동 생성
...
- 고수준 목표를 하위 작업·실행 조건으로 분해한다
- 결과·오류·환경 변화로 계획을 수정한다
- 상태 저장이 비동기·장시간 작업 재개를 돕는다
- 업무 위험에 맞춰 자율·승인 수준을 제한한다
...
|:---|:---|:---|
| 핵심 단위 | 목표와 완료 조건의 달성 | 입력에 대한 콘텐츠 생성 |
| 처리 구조 | 계획·행동·관찰의 상태 루프 | 단발·대화형 응답 중심 |
