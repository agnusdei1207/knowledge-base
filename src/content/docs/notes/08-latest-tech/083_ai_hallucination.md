...
- 정의/개념: 모델이 입력•근거•검증 사실과 불일치하는 내용을 생성하는 **AI 환각 현상**이다.
- 배경/필요성: 다음 토큰 확률 기반 생성의 **사실성•충실성**을 보장할 수 없다.
...
- **사실성(Factuality)**: 출력 주장이 외부의 검증 가능한 사실과 일치하는 정도이다.
- **충실성(Faithfulness)**: 출력 주장이 제공된 문맥으로 뒷받침되는 정도이다.
- **근거 귀속(Attribution)**: 각 주장과 지지 출처를 연결하는 과정이다.

</details>

- 외부 검증 사실과 충돌하는 **사실성 오류**가 핵심이다.
- 제공 문맥에 없는 주장을 추가하는 **충실성 오류**가 핵심이다.
- 주장과 인용 출처가 어긋나는 **근거 귀속 오류**가 핵심이다.
...
- **주장 분해기**: 생성 답변을 독립 검증할 수 있는 원자 주장으로 나누는 구성요소이다.
- **사실 검증기(Fact Verifier)**: 각 주장을 신뢰 가능한 근거와 대조해 지지•반박•판단 불가로 판정하는 구성요소이다.
- **근거 귀속기(Attribution Mapper)**: 검증된 주장과 이를 뒷받침하는 출처•인용 구간을 연결하는 구성요소이다.
...
- **생성 조건(Generation Condition)**: 입력•검색 문맥•모델•프롬프트•응답을 함께 기록해 오류 원인을 재현하게 하는 조건이다.
...
1. **입력•문맥•응답 기록**: 환각 원인 추적을 위한 **생성 조건**을 보존한다.
2. **검증 가능 주장 분해**: **복합 답변의 원자 주장을 변환**한다.
3. **외부 사실•문맥 대조**: 신뢰 사실과 제공 근거의 **주장 지지 여부**를 확인한다.
4. **오류 유형•위험 판정**: 사실성•충실성•귀속 오류와 **업무 영향**을 분류한다.
5. **수정•보류•거절 적용**: 검증 결과와 위험 수준에 맞는 **출력 통제**를 실행한다.
...
- **사실성 오류(Factuality Error)**: 출력 주장이 외부 사실과 충돌하는 오류이다.
- **충실성 오류(Faithfulness Error)**: 출력 주장이 제공 문맥의 지지를 받지 못하는 오류이다.
- **귀속 오류(Attribution Error)**: 인용과 주장 사이의 지지 연결이 잘못된 오류이다.
...
- 사실•문맥•출처의 불일치 유형과 위험도를 기준으로 **환각 탐지와 출력 통제 수준**을 결정한다.
