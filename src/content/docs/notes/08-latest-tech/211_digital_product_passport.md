...
**DPP**는 고유 식별자를 통해 제품의 원재료·제조·사용·수리·재활용 정보를 생애주기 전반에 연결하는 기록 체계다.
...
**분산 상호운용**은 운영자별 저장소에 있는 제품 정보를 표준 데이터 모델과 API로 연결해 교환하는 방식이다.
...
**고유 식별자**는 실제 제품과 디지털 생애주기 기록을 지속적으로 연결하는 참조 키다.
...
**검증 가능한 이력**은 발행 주체와 변경 시점·무결성을 확인할 수 있도록 연결한 제품 생애주기 기록이다.
...
**디지털 인증서**는 특정 시점의 속성이나 적합성을 발행자가 전자적으로 증명하는 문서이며, DPP의 전체 생애주기 기록과 범위가 다르다.
...
**역할 기반 접근**은 소비자·수리자·규제기관 등 이용자 역할에 따라 공개 정보와 제한 정보의 열람 범위를 구분한다.
...
**순환경제**는 제품과 자원을 수리·재사용·재활용하여 폐기와 신규 자원 투입을 줄이는 경제 구조다.
