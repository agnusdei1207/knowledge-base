...
> 왼쪽 사선 구간은 외부 메모리 대역폭이 처리량을 제한하고 500 ops/byte 이후 수평 구간은 최대 MAC 성능이 상한이 되며, 100 TOPS•200 GB/s 가정의 이론값이다.
...
5. **지연·전력·정확도 검증**: 미지원 연산 우회를 포함한 **종단 효율** 판정
...
- 연산 지원·메모리 이동·정확도·전력을 기준으로 **GPU·NPU·ASIC·FPGA** 선택
