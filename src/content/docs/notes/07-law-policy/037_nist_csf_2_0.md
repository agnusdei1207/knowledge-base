...
- 정의/개념: 사이버 위험을 경영 목표에 연결하는 **성과 분류체계**와 **구현 등급** 지침이다.
- 배경/필요성: 개별 통제 목록으로는 경영 위험과 **투자 우선순위** 연결하기 어렵다.

#### 한줄 요약
- **현재•목표 프로파일**로 경영진과 현장의 성과 격차를 정렬한다.
...
#### 한줄 요약
- **Govern**이 모든 기능에 정책•책임•위험 허용수준을 제공한다.
...
#### 한줄 요약
- **Govern**이 나머지 5개 기능의 정책•책임을 통합이 핵심이다.
...
1. **범위•위험 허용수준 전달**: 임무•이해관계자•위험 한계를 확정한다.
2. **현재 프로파일 작성**: 하위범주별 현재 성과 조사한다.
3. **현재 성과•격차 분석**: 통제 결과와 목표 대비 차이를 확인한다.
4. **목표 프로파일•우선과제 제안**: 위험•비용•의존성으로 실행 순서를 결정한다.
5. **목표 승인•실행 지시**: 필요한 보안 성과와 투자 우선순위를 확정한다.

#### 한줄 요약
- **성과 격차**를 위험순으로 정렬해 **투자 우선순위** 확정이 핵심이다.
...
#### 한줄 요약
- CSF 2.0은 **Govern**을 추가해 경영•공급망 책임 강화가 핵심이다.
...
#### 한줄 요약
- **현재•목표 격차**를 위험순으로 정렬해 투자 대상 선정이 핵심이다.
...
- **CSF 2.0 전사 정렬**: Govern 기능을 추가해 경영 책임•공급망 위험을 보안 기능과 통합한 프레임워크 적용한다.

</details>

- 전사 위험 연계 시 **CSF 2.0**을 채택하고 **목표 프로파일**로 투자 확정이 핵심이다.
