...
date: "2026-08-06T15:51:00+09:00"
tags:
  - "notes-law_policy"
weight: 7
extra:
...
- **IT 서비스 관리(ITSM)**: 서비스 기획•제공•지원•개선 관리
...
#### 한줄 요약
- **인시던트•문제 관리**는 복구•원인, **변경 관리**는 배포 통제
...
- **평균 복구 시간(MTTR)**: 장애 발생부터 정상 복구까지의 평균 시간
- **알려진 오류**: 근본 원인과 우회 조치를 확인한 문제 기록
- **롤백 계획**: 변경 실패 시 이전 정상 상태로 되돌리는 절차
- **재발 방지**: 반복 장애의 근본 원인•알려진 오류 제거
...
#### 한줄 요약
- **서비스 복구**와 **원인 분석**을 분리해 신속 복구•재발 방지
...
- **알려진 오류 데이터베이스(KEDB)**: 원인•우회 조치를 재사용하는 저장소
- **변경자문위원회(CAB)**: 변경 위험•영향•일정•롤백 검토 조직
- **구성관리 데이터베이스(CMDB)**: 구성항목 관계와 영향 범위 저장소
...
#### 한줄 요약
- **서비스 데스크•CAB**가 CMDB•KEDB로 **복구•변경** 통제
...
- **근본 원인**: 반복 인시던트를 일으키는 기저 결함
- **변경 위험 평가**: 영향•실패 가능성•복구 방법 검토
...
3. **승인•롤백 조건 반환**: 위험 수준별 승인과 복구 조건 확정

#### 한줄 요약
- **우회 조치•원인 분석** 후 **변경 승인•적용** 수행
...
#### 한줄 요약
- **MTTR•재발률**과 **변경 성공률**로 세 관리 성과 평가
...
- **영향 범위**: 변경 대상과 연결된 구성항목•서비스의 범위
- **표준 변경**: 절차•위험•복구 방법이 검증된 사전 승인 변경
...
- **위험 기반 변경**: 표준 변경은 신속 처리하고 고위험은 CAB 심사
- **근본 원인 분석(RCA)**: 반복 장애의 기저 원인을 증거로 규명
...
#### 한줄 요약
- **서비스 복구** 후 RCA와 **변경 통제**로 재발•실패 방지
