...
- 정의/개념: **인시던트·문제·변경 관리**는 서비스 복구, 근본 원인 제거, 변경 위험 통제를 분리하되 상호 연계하는 **IT 서비스 관리 관행**
...
1. **인시던트 접수**: 영향도와 긴급도로 우선순위 분류
2. **우회 조치·서비스 복구**: 알려진 오류 정보를 활용하여 가용성 우선 확보
3. **문제 등록·원인 분석**: 반복 인시던트의 근본 원인 규명
4. **변경 요청·위험 평가**: 구성항목 영향과 롤백 계획을 검토하여 승인
5. **변경 적용·사후 검토**: 정상 반영 여부와 부작용 확인
