...
date: "2026-08-05T14:21:28+09:00"
tags:
  - "notes-law_policy"
weight: 28
extra:
...
- **위험 차등화**: 이용 영역•목적•결정 대상•기본권 영향에 따라 고영향 AI 여부와 적용 의무를 구분하는 방식이다.
...
- **위험 차등화**: 법정 영역과 권리 영향에 따라 고영향 AI 여부를 판단하고 사업자가 정부에 확인 요청 가능
- **투명성 확보**: 고영향•생성형 AI 서비스의 사전 고지와 생성 결과물 표시
- **신뢰성 관리**: 고영향 AI의 위험관리•설명•인간 감독 의무와 기본권 영향평가 노력

#### 한줄 요약

- 모든 AI에 같은 의무를 주지 않고, 고영향 여부와 생성 기능에 따라 안전조치와 표시의무를 달리 적용한다.

## Ⅲ. 구조 및 구성요소

<details><summary>핵심 용어</summary>

- **고영향 AI**: 사람의 생명•신체•기본권에 중대한 영향을 미쳐 추가 안전•설명 의무가 적용되는 AI이다.
- **생성형 AI**: 학습 데이터 구조를 바탕으로 텍스트•이미지•음성 등 새로운 결과물을 생성하는 AI이다.
...
- **안전성 확보 조치**: 위험관리•설명•인간 감독으로 이용자의 권리와 안전을 보호하는 조치이다.
...
- 유형 분류를 기준으로 고영향 AI에는 안전조치를, 생성형 AI에는 고지•표시를 연결하고 근거를 보존한다.
...
sequenceDiagram
    ...
    participant 이용자
    participant 정부기관
    개발사->>정부기관: 고영향 해당 여부 확인 요청
    정부기관-->>개발사: 검토 결과•적용 근거 통보
    개발사->>개발사: 1. 적용 의무•조치 범위 결정
    opt 고영향•생성형 AI
        개발사->>이용자: 2. AI 이용 고지•생성 사실 표시
    end
    opt 고영향 AI
        개발사->>이용자: 3. 위험관리•설명•인간 감독 제공
        개발사->>정부기관: 4. 조치•영향평가 근거 제출
    end
```

**동작 원리**

1. **적용 의무•조치 범위 결정**: 검토 결과와 적용 근거를 서비스 책임에 반영
2. **AI 이용 고지•생성 사실 표시**: 고영향•생성형 서비스임을 사전 고지하고 생성 결과의 출처 표시
3. **위험관리•설명•인간 감독 제공**: 고영향 AI의 위험 식별•대응•결과 설명•이용자 보호•사람의 개입 수단 운영
4. **조치•영향평가 근거 제출**: 분류•조치•시험•사고•영향평가 결과를 보존하고 요청 시 입증
...
- 의료 AI를 도입하기 전에 환자의 권리와 안전에 미칠 영향을 평가하고 사람의 감독 절차를 정한다.
