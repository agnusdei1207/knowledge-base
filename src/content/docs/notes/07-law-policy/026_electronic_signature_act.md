...
- **공동인증서**: 과거 공인인증서의 독점적 지위 폐지 후 기존 인증기관이 이름을 바꾸어 제공하는 인증서이며 별도 법적 우월성을 뜻하지 않는다.
...
- **1. 거래 위험·보증수준 제시**: 거래 금액·법적 효과·부정 사용 피해로 본인확인과 증거 수준 결정
...
|:---|:---|:---|
| **적용 기준** | 거래 위험별 **수단 선택** | 지정 **공인인증서 우선** |
...
- **개인식별번호(PIN)**: 이용자가 알고 있는 비밀값으로 본인 여부를 확인하는 인증정보이다.
- **증거 불완전**: 서명값만 보존하고 원문·인증 상태·시각·동의 화면·접속 기록을 남기지 않아 분쟁 시 입증이 어려운 문제이다.
...
- **효력과 증명력의 구분**: 전자적 형태로 법적 효력을 배제하지 않는 것과 특정 분쟁에서 서명자·의사·문서 무결성을 증거로 입증하는 것은 별도 판단이라는 관계이다.
- **액티브엑스 의존 폐지**: 특정 브라우저 설치 기술에 의존하지 않고 웹·모바일 환경에서 다양한 인증 수단을 선택하도록 한 변화이다.
