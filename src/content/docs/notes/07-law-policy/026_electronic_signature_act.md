---
sidebar:
  order: 26
  label: "026. 전자서명법 - 공인인증서 폐지·전자서명 (Electronic Signature Act)"
  badge:
    text: "미출제 · 30%"
    variant: note
title: "전자서명법 - 공인인증서 폐지·전자서명 (Electronic Signature Act)"
date: "2026-07-25T14:24:00+09:00"
tags:
  - "notes-law_policy"
weight: 26
extra:
  question_no: "026"
  source_status: "미출제"
  source_history: ""
  priority: 30
  priority_note: "전자서명 효력·인증수단 경쟁을 묻는 법제"
---

## 미리 알고가기

- **전자서명(Electronic Signature)**: 전자문서에 붙거나 논리적으로 결합되어 서명자의 신원과 서명 사실을 나타내는 전자적 정보이며, 법적 효력 판단의 대상이다.
- **공동인증서(Joint Certificate)**: 옛 공인인증서의 독점적 지위가 폐지된 뒤 기존 인증기관들이 이름을 바꿔 제공하는 인증서로, 법률상 별도 우월 지위를 뜻하지 않는다.
- **전자서명인증사업자(Electronic Signature Certification Service Provider)**: 가입자의 신원을 확인하고 전자서명 생성·검증에 필요한 인증서비스를 제공하는 주체다.
- **개인식별번호(Personal Identification Number, PIN)**: `PIN`은 “핀”으로 읽고 영문명의 머리글자를 조합한 표기이며, 이용자가 아는 비밀값으로 본인 여부를 확인한다.
- **응용 프로그래밍 인터페이스(Application Programming Interface, API)**: `API`는 “에이피아이”로 읽고 영문명의 머리글자를 조합한 표기이며, 서비스가 외부 전자서명 기능을 호출하는 연결 규약이다.
- **인터넷 프로토콜 주소(Internet Protocol Address, IP Address)**: `IP`는 “아이피”로 읽고 영문명의 머리글자를 조합한 표기이며, 서명 접속 위치를 추적하는 보조 증적이다.
- **해시값(Hash Value)**: 문서 내용을 고정 길이 값으로 변환한 결과이며, 서명 뒤 문서가 바뀌었는지 검증하는 무결성 증거다.
- **액티브엑스(ActiveX)**: “액티브엑스”로 읽고 마이크로소프트 기술명을 그대로 쓴 표기이며, 개정 전 공인인증 환경에서 설치 부담을 만든 웹 구성요소다.
- **부인방지(Non-repudiation)**: 서명자가 나중에 서명 사실을 부정하기 어렵도록 신원·문서·시각의 검증 증거를 보존하는 성질이다.

## Ⅰ. 개요

- **정의/개념**: 전자서명 효력과 인증업무 기준을 규율한다
- **배경/필요성**: 기술중립과 인증수단 경쟁을 촉진한다

### 쉽게 이해하기 (학습용)

- 공인인증서만 우대하던 제도를 없애고 다양한 전자서명이 경쟁하도록 하되, 서명 효력은 법률과 당사자 약정에 따라 판단한다.

## Ⅱ. 특징

- 전자적 형태만으로 서명의 법적 효력을 부인할 수 없다
- 기술중립은 특정 인증기술의 법정 독점을 막는다
- 당사자 약정은 거래별 서명 효력 판단을 돕는다

### 쉽게 이해하기 (학습용)

- 법이 특정 인증기술을 우대하지 않으므로 이용기관이 거래 위험과 이용 편의에 맞는 전자서명 수단을 선택한다.

## Ⅲ. 아키텍처 및 구성요소

```mermaid
flowchart TD
    Signer[서명자] --> Provider[인증사업자]
    Signer --> Relier[이용기관]
    Provider --> Relier
    Relier --> Evidence[증거 저장소]
```

| 설계 요소 | 설명 |
|:---|:---|
| 서명자 | 본인 확인 후 전자문서에 서명 |
| 인증사업자 | 가입자 확인과 인증서비스 제공 |
| 이용기관 | 거래 위험에 맞는 서명 수단 채택 |
| 증거 저장소 | 서명·검증 기록과 문서 해시 보존 |

> 요약: 서명·인증·검증 증거로 전자거래를 구성한다

### 쉽게 이해하기 (학습용)

- 서명자는 인증사업자의 수단으로 문서에 서명하고, 이용기관은 검증 결과와 문서 해시를 증거로 보존한다.

## Ⅳ. 원리 및 절차 흐름도

```mermaid
sequenceDiagram
    participant 서명자
    participant 이용기관
    participant 인증사업자
    이용기관->>이용기관: 위험 분석
    이용기관->>이용기관: 수단 선정
    서명자->>이용기관: 서명 의사
    이용기관->>인증사업자: 서명 검증
    이용기관->>이용기관: 증거 보존
```

| 절차 | 설명 |
|:---|:---|
| 위험 분석 | 거래 가치·피해 영향으로 보증수준 결정 |
| 수단 선정 | 보증수준에 맞는 서명 수단 선택 |
| 서명 의사 | 본인 확인 후 전자문서 서명 |
| 서명 검증 | 서명 유효성과 문서 무결성 점검 |
| 증거 보존 | 서명·검증 기록과 문서 해시 보관 |

> 요약: 거래 위험에 맞는 서명 수단과 증거를 관리한다

### 쉽게 이해하기 (학습용)

- 이용기관은 거래 위험에 맞는 수단을 고른 뒤 서명자 의사를 확인하고, 서명과 문서가 유효했다는 기록을 남긴다.

## Ⅴ. 종류 및 비교

| 판단 기준 | 개정 후 체계 (현재) | 개정 전 체계 |
|:---|:---|:---|
| 적용 기준 | 거래 위험별 수단 선택 | 지정 공인인증서 우선 사용 |
| 핵심 특징 | 기술중립·민간 인증 경쟁 | 공인인증서의 법정 우월성 |
| 주요 위험 | 기관별 보증수준 판단 필요 | 특정 기술 의존·이용 불편 |

> 요약: 개정 뒤 기술중립과 민간 인증 경쟁을 적용한다

### 쉽게 이해하기 (학습용)

- 개정 후에는 하나의 인증서를 강제하지 않고 거래 금액과 피해 영향에 맞춰 전자서명 수단을 선택한다.

## Ⅵ. 실무 사례

1. 전자계약에 간편서명 API를 연동한다
2. 계약서 해시와 서명 검증 기록을 보존한다

### 쉽게 이해하기 (학습용)

- 전자계약 서비스가 여러 인증사업자의 API를 연결해 거래 위험에 맞는 간편서명 수단을 제공한다.
- 계약서 해시와 서명 검증 시각을 함께 보존해 나중에 문서 변경과 서명 부인을 확인한다.

## Ⅶ. 결론

- 거래 위험으로 서명 수단과 증거 수준을 결정한다.

### 쉽게 이해하기 (학습용)

- 거래 금액과 피해 영향이 클수록 본인 확인이 강한 전자서명을 선택하고 검증 증거도 더 오래 보존한다.
