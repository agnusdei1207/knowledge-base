...
- 정의/개념: 전자서명의 효력과 인증업무 기준을 정한 **기술 중립적 전자거래 신뢰 법률**이다.
- 배경/필요성: 공인인증서의 법정 우월성과 **특정 설치 기술 의존**으로 신기술•인증 경쟁 제약이 발생한다.

#### 한줄 요약

- **공인인증서 우월성**을 폐지하고 **법령•약정**으로 효력 판단이 핵심이다.
...
- 서명자는 **전자서명**, 이용기관은 **검증 결과•문서 해시**를 보존한다.
...
- **서명 검증**: 서명값•해시•인증•시각•취소 상태를 확인한다.
</details>
...
2. **본인 확인•서명 요청**: 선택한 수단으로 가입자를 확인하고 서명 대상 문서를 명확히를 제시한다.
3. **문서 확인•전자서명**: 서명자가 원문과 의사를 확인한 뒤 서명정보를 생성한다.
4. **서명•인증 상태 검증**: 서명값•문서 해시•인증 유효성•시각•취소 여부를 확인한다.

#### 한줄 요약

- **거래 위험별 수단**을 고르고 **서명•문서 증거**를 보존한다.
...
- **거래 금액•피해 영향**에 맞춰 **전자서명 수단**을 선택한다.
...
- **계약서 해시•검증 시각**으로 **변경•서명 부인**을 확인한다.
...
- 고위험 거래는 **강한 본인확인**, 모든 서명은 **원문 해시•검증 시각** 장기를 보존한다.

#### 한줄 요약

- **거래 위험**이 클수록 **강한 본인확인•증거 보존**을 적용한다.
