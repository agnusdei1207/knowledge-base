...
#### 한줄 요약
- 장기 데이터•인증 체계를 **NIST PQC** 표준으로 우선 전환한다.
...
#### 한줄 요약
- **ML-KEM**은 키 설정, **ML-DSA•SLH-DSA**는 서명이 핵심이다.
...
- **암호 자산•데이터 수명 확정**: 알고리즘•키•인증서•장비와 보호기간을 목록화하는 단계이다.
- **HNDL 위험•교체 우선순위 결정**: 미래 노출 영향과 시스템 의존성으로 전환 순서를 정하는 단계이다.
- **알고리즘•매개변수 선택**: 크기•지연•상호운용 검증 결과로 적용 구성을 정하는 단계이다.
- **하이브리드 구성•키 발급 지시**: 기존 암호와 PQC 병행 및 롤백 조건을 확정하는 단계이다.
- **키•인증서 전환 배포**: 운영 오류를 감시하며 키와 신뢰체계를 교체하는 단계이다.
...
#### 한줄 요약
- **HNDL 위험•교체 우선순위 결정**으로 PQC 전환 순서를 정한다.
...
#### 한줄 요약
- **PQC 알고리즘** 중 키 설정은 **ML-KEM**, 서명은 **ML-DSA**와 **SLH-DSA**를 사용한다.
...
#### 한줄 요약
- **PQC 하이브리드 구성**의 지연•호환성을 검증한 뒤 전환한다.
...
#### 한줄 요약
- **PQC 전환 결정**은 용도•데이터 수명•시험 결과를 함께 반영한다.
