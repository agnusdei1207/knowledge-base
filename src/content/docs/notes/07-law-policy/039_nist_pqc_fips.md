...
- FIPS 203의 ML-KEM으로 공유 비밀을 설정하고 실제 데이터는 대칭키로 보호
- FIPS 204의 ML-DSA와 FIPS 205의 SLH-DSA로 서로 다른 수학 기반의 디지털 서명을 제공
- 데이터 수명과 용도에 따라 키·서명 크기와 성능 및 구현 지원을 시험해 단계적으로 전환
...
| 절차 | 설명 |
|:---|:---|
| 암호 자산·데이터 수명 조사 | 알고리즘·키·인증서·장비와 보호기간 목록화 |
| HNDL 위험·교체 난도 보고 | 미래 노출 영향과 시스템 의존성으로 우선순위 산정 |
| 알고리즘·매개변수 시험 요청 | 용도별 키·서명 크기와 지연·상호운용 검증 |
| 하이브리드 구성·검증 결과 제공 | 기존 암호와 PQC 병행 및 롤백 가능성 확인 |
| 키·인증서 전환 배포 | 운영 오류를 감시하며 키와 신뢰체계 교체 |
