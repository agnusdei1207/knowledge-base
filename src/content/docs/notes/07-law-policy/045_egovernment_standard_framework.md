...
- **운영•관리환경**: 배포된 애플리케이션의 모니터링•배치•설정•장애 대응과 운영 지표 관리를 지원하는 도구 체계이다.
...
- **웹 애플리케이션 서버(Web Application Server, WAS) 호환성**: 표준프레임워크의 라이브러리•자바 규격과 배포 대상 WAS의 지원 버전이 맞는지 확인하는 조건이다.
...
- **실행환경**: 배포된 업무 애플리케이션이 웹 애플리케이션 서버(Web Application Server, WAS)에서 동작하도록 공통 런타임 기능을 제공한다.
...
#### 한줄 요약
- 개발사별 독자 구조 대신 공개된 스프링(Spring) 기반 표준 구조와 공통 컴포넌트를 활용해 개발 생산성과 인수 가능성을 높입니다.

## Ⅵ. 실무 고려사항 및 대책

<details><summary>핵심 용어</summary>

- **웹 애플리케이션 서버(Web Application Server, WAS)**: 웹 요청을 받아 업무 로직을 실행하고 처리 결과를 반환하는 서버이다.
...
- 공공 표준•재사용 이익이 크면 **전자정부 표준프레임워크(eGovFrame) 5.0** 선택, **실행 JDK 17 이상•개발 JDK 21 이상•Jakarta EE•WAS 호환성** 검증 후 적용
