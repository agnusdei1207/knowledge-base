...
- **인공지능(Artificial Intelligence, AI) 편향**: 인공지능 결과가 특정 집단이나 관점을 체계적으로 불리하게 반영하는 현상이다.
...
- **디지털 시민성**: 온라인 협업과 소통에서 권리•책임•윤리 규범을 실천하는 역량이다.
- **접근•활용 역량**: 필요한 디지털 기기•서비스에 접근해 정보를 탐색하고 업무•생활 목적에 맞게 사용하는 능력이다.
- **데이터 역량**: 데이터를 읽고 수집•분석•시각화해 근거 있는 판단과 창작에 활용하는 능력이다.
- **보안•개인정보 역량**: 계정•기기•개인정보를 보호하고 피싱•허위정보•온라인 위험에 대응하는 능력이다.
- **참여•소통 역량**: 디지털 도구로 협업•창작•공론에 참여하며 타인의 권리와 접근성을 존중하는 능력이다.
...
- **인공지능(Artificial Intelligence, AI) 결과**: 학습 데이터와 모델을 바탕으로 생성한 예측•분류•추천•생성 콘텐츠이다.
- **피싱(Phishing)**: 사칭 메시지와 악성 링크로 민감정보 제공을 유도하는 공격이다.
