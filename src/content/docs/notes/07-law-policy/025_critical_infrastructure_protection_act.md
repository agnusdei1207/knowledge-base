...
- **주요정보통신기반시설(Critical Information Infrastructure, CII)**: `CII`는 “시아이아이”로 읽고 영문명의 머리글자를 조합한 표기이며, 침해 시 국가안보·경제에 중대한 영향을 주어 지정·보호하는 시설이다.
- **정보기술(Information Technology, IT)**: `IT`는 “아이티”로 읽고 영문명의 머리글자를 조합한 표기이며, 정보시스템에서 데이터를 저장·처리하는 기술 영역이다.
- **운영기술(Operational Technology, OT)**: `OT`는 “오티”로 읽고 영문명의 머리글자를 조합한 표기이며, 발전소·공장의 물리 설비를 감시·제어하는 기술 영역이다.
...
1. 발전소 IT·OT 자산 의존성 지도를 작성한다
2. 법정 기준으로 제어망 취약점을 분석·평가한다
3. 랜섬웨어 대비 오프라인 백업 복구를 시험한다

### 쉽게 이해하기 (학습용)

- 발전소의 정보시스템과 제어설비가 어디서 연결되는지 지도화해 한쪽 침해가 다른 쪽으로 번질 경로를 찾는다.
- 지정 시설의 서버·네트워크·제어설비를 법정 기준으로 점검해 보호대책에 반영할 취약점을 정한다.
- 랜섬웨어가 운영망을 암호화한 상황을 가정해 분리 보관한 백업으로 실제 복구되는지 시험한다.
