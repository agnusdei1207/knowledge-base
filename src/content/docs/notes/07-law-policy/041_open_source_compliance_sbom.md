...
- **전이 추적성**: 소스와 의존성 및 빌드 산출물을 함께 분석해 직접·전이 구성요소를 식별
- **구성 동일성**: PURL·버전·해시·관계로 실제 배포 구성과 SBOM의 동일성을 검증
- **위험 분리성**: 라이선스 의무와 CVE 도달성·VEX 상태를 분리해 출시 조건과 사후 조치를 결정
...
1. **소스·빌드 산출물 전달**: 직접·전이 패키지와 PURL 수집
2. **SBOM·구성 동일성 결과 제공**: 버전·해시·관계로 실제 배포 구성 확인
3. **라이선스·취약점 조치 요청**: 배포 의무와 CVE 도달성·VEX 상태 판정
4. **고지·소스·VEX와 제품 배포**: 예외 심사와 의무 이행 후 출시 승인
5. **신규 취약점·구성 변경 통보**: 운영 중 SBOM을 신규 CVE·변경과 대조
