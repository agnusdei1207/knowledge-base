...
#### 한줄 요약
- **소프트웨어 자재명세서(SBOM)**로 직접•전이 구성의 라이선스•취약점 위험을 추적한다.
...
- **구성 동일성**: SBOM의 패키지•버전•해시가 실제 배포 산출물과 일치하는 성질이다.
- **위험 분리성**: 라이선스 의무와 취약점 악용 가능성을 서로 다른 기준으로 판정하는 특성이다.
...
#### 한줄 요약
- **위험 분리성**에 따라 라이선스 의무와 취약점 영향을 검증한다.
...
- **SBOM•구성 동일성 분석**: 실제 배포본의 패키지•버전•해시•관계를 명세와 대조하는 단계이다.
- **라이선스•취약점 조치 판정**: 배포 의무와 취약점 도달성•조치 상태를 각각 결정하는 단계이다.
- **고지•소스•VEX 배포 준비**: 예외 심사와 법적•보안 조치를 마치고 출시 자료를 구성하는 단계이다.
- **신규 취약점•구성 변경 대조**: 운영 중 SBOM을 신규 CVE와 패키지 변경에 다시 대조하는 단계이다.
...
#### 한줄 요약
- **SBOM•구성 동일성 분석** 후 **신규 취약점•구성 변경 대조**를 반복한다.
...
#### 한줄 요약
- 구성은 **SBOM**, 법적 의무는 **라이선스 목록**, 실제 영향은 **VEX**로 전달한다.
...
#### 한줄 요약
- **취약점 오탐**과 라이선스 판정은 법무•개발 교차 검토로 보완한다.
...
#### 한줄 요약
- **공급망 추적성**은 배포 후에도 신규 취약점과 라이선스 충돌을 지속 감시하게 한다.
