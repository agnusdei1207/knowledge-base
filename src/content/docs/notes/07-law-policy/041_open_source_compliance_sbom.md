...
- 정의/개념: **오픈소스 컴플라이언스•SBOM**은 구성•의무•취약점을 관리하는 **공급망 준수 체계**이다.
- 배경/필요성: 직접 의존성만으로는 전이 버전•배포 의무•**신규 취약점** 추적하기 어렵다.

#### 한줄 요약
- 직접•전이 구성의 **라이선스•취약점 위험**을 추적한다.
...
#### 한줄 요약
- **법률 의무•취약점 영향**을 분리 검증해 배포 증적을 보존한다.
...
1. **SBOM•구성 동일성 분석**: 버전•해시•관계로 실제 배포 구성을 확인한다.
2. **라이선스•취약점 조치 판정**: 배포 의무와 CVE 도달성•VEX 상태를 판정한다.
3. **고지•소스•VEX 배포 준비**: 예외 심사와 의무 이행 후 출시를 승인한다.
4. **신규 취약점•구성 변경 대조**: 운영 중 SBOM을 신규 CVE•변경과를 대조한다.

#### 한줄 요약
- 배포본 **구성 동일성** 확인 후 신규 취약점과 지속 대조가 핵심이다.
...
#### 한줄 요약
- 구성은 **SBOM**, 법적 의무는 목록, 실제 영향은 **VEX**가 핵심이다.
...
#### 한줄 요약
- 자동 탐지 누락은 **법무•개발 교차 검토**로 보완이 핵심이다.
...
- 구성 공개는 **SBOM**, 법적 의무는 라이선스 심사, 실제 취약점 영향은 **VEX**로 전달한다.

#### 한줄 요약
- 배포 후에도 **신규 취약점•라이선스 충돌** 지속 감시가 핵심이다.
