...
- 정의/개념: **소프트웨어(Software, SW) 대가 산정** — 규모·작업량과 단가로 적정 비용을 계산하는 **원가 추정 체계**
...
- **인공지능(Artificial Intelligence, AI) 사업**: 데이터와 모델의 준비·학습·검증·배포 작업량을 별도로 산정하는 사업 유형이다.
...
| 기능 규모 | 사용자 관점의 **기능점수** 측정 |
| 데이터 기능 | **내부 논리 파일·외부 연계 파일** 식별 |
| 입력·출력 기능 | **외부 입력·외부 출력** 식별 |
| 조회 기능 | **외부 조회** 식별 |
...
- **기능점수(Function Point, FP)**: 사용자 기능을 데이터·트랜잭션 유형과 복잡도로 계량해 개발 규모를 측정하는 단위이다.
- **투입공수**: 과업을 수행하는 데 필요한 인력 수와 투입 기간을 결합한 작업량이다.
- **인공지능(Artificial Intelligence, AI) 사업**: 데이터 전처리·모델 학습·검증·배포의 작업량을 산정해야 하는 사업 유형이다.
...
- **기능점수(Function Point, FP)**: 데이터·트랜잭션 기능을 사용자 관점에서 계량한 소프트웨어 규모 단위이다.
...
| 산정 근거 | **인월(Man-Month, M/M)·직무별 단가** | **기능점수(Function Point, FP)·보정계수·단가** |
...
- **기능 기준선**: 승인된 기능 목록과 기능점수를 추가·삭제 과업의 규모·기간·대가 조정 기준으로 고정한 상태이다.
...
- **소프트웨어(Software, SW) 대가**: 승인 기능 규모와 변경 범위에 맞춰 기간과 함께 조정하는 사업 비용이다.

</details>

- **기능 규모·변경 범위** 로 소프트웨어 대가 조정
