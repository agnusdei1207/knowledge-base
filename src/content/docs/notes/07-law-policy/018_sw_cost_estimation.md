...
date: "2026-07-30T11:58:00+09:00"
tags:
  - "notes-law_policy"
weight: 18
extra:
...
- **소프트웨어 대가 산정**은 사업 유형에 적합한 규모·작업량을 측정하고 단가와 보정 요소를 적용하여 개발·운영 서비스의 적정 비용을 계산하는 **원가 추정 체계**
- **배경/필요성**: 투입 인원 중심 추정이 반영하지 못하는 사용자 기능 규모·사업 특성·요구사항 변경의 경제적 가치 산정
...
sequenceDiagram
    participant 발주기관
    participant 수행기관
    발주기관->>수행기관: 1. 사업 유형·범위 제시
    수행기관->>발주기관: 2. 기능 규모·작업량 산정
    발주기관->>수행기관: 3. 단가·보정 기준 확인
    수행기관->>발주기관: 4. 비용 항목별 대가 제시
    발주기관->>수행기관: 5. 변경 규모·대가 검증
```

| 절차 | 설명 |
|:---|:---|
| 사업 유형·범위 제시 | 개발·운영·데이터·인공지능 등 산정 대상과 책임 범위 확정 |
| 기능 규모·작업량 산정 | 기능점수 또는 과업별 투입공수를 객관적 근거로 측정 |
| 단가·보정 기준 확인 | 적용 시점의 단가와 규모·복잡도·품질 보정 요소 검토 |
| 비용 항목별 대가 제시 | 직접비·제경비·기술료 등 항목별 산식과 근거 작성 |
| 변경 규모·대가 검증 | 기준선 대비 추가·삭제 기능을 측정하여 계약 대가 조정 |
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 요구사항 불명확 상태의 조기 확정 견적 | 범위 성숙도에 따라 사전 추정과 상세 산정을 구분하고 오차 범위 명시 | 예산 과소·과대 산정 위험 감소 |
| 기능점수 식별자의 주관적 판단 | 경계·데이터 기능·트랜잭션 기능의 판정 근거를 기록하고 독립 검토 | 규모 측정의 일관성과 재현성 향상 |
| 추가 과업의 무상 수행 | 계약 기능 기준선을 관리하고 변경 기능의 규모·기간·대가를 동시 조정 | 정당한 보상과 일정·품질 보호 |
...
- 일반 소프트웨어 개발의 적정 대가를 확보하려면 **사용자 기능 규모를 기준선으로 측정하고 판정 근거를 검증하며 변경 기능의 규모·기간·대가를 동시에 조정하는 기능점수 중심 산정** 필요
