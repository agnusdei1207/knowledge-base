...
- **약어 읽기·표기**: COBIT은 ‘코빗’, EDM·APO·BAI·DSS·MEA는 영문 영역명의 머리글자로 읽으며 평가·계획·구축·운영·감독 책임을 구분함
- **표준 표기(ISO/IEC 38500)**: ‘아이에스오/아이이씨 삼만 팔천오백’으로 읽고 슬래시는 공동 표준화 기관을, 숫자는 표준 번호를 나타냄
- **시퀀스 기호(->>)**: ‘요청 전달’로 읽고 이중 화살촉은 COBIT 목표·통제·평가 책임의 인계를 나타냄
...
2. 책임 분리: 평가·지시·감독의 거버넌스와 실행·관리 책임 분리

### 쉽게 이해하기 (학습용)
- 회사의 규모, 업종, 기술 변화 수준을 분석하여 COBIT이 권장하는 40개 지배 목표 중 우리 기업에 우선 필요한 부분만 골라 적용함
- 전산 부서가 직접 시스템을 도입하고 감사까지 스스로 하는 셀프 검증을 방지하기 위해 이사회가 준수 실태를 따로 감시하게 함
