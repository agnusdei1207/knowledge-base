...
- **정보기술(Information Technology, IT) 거버넌스**: 기업 거버넌스의 일부로서 이사회 등 지배기구가 정보•기술의 가치•위험•자원에 관한 방향과 책임을 정하고 성과를 감독하는 체계이다.
...
#### 한줄 요약
- 이사회 등 지배기구가 정보•기술 의사결정 권한을 명확히 하고 투자 성과와 위험을 감독하는 체계이다
...
- **평가•지시•감독(Evaluate, Direct and Monitor, EDM)**: 이사회 등 지배기구가 대안을 평가하고 방향•위험 한도를 지시하며 성과와 준수를 감독하는 거버넌스 영역이다.
- **정렬•계획•조직화(Align, Plan and Organize, APO)**: 전략•조직•예산•위험•공급자를 업무 목표에 맞추는 관리 영역이다.
- **구축•획득•구현(Build, Acquire and Implement, BAI)**: 해결책과 변경을 선정•개발•도입•전환하는 관리 영역이다.
- **제공•서비스•지원(Deliver, Service and Support, DSS)**: 운영•서비스 요청•사고•보안•업무 연속성을 수행하는 관리 영역이다.
- **모니터링•평가•진단(Monitor, Evaluate and Assess, MEA)**: 성과•내부통제•법규 준수 상태를 점검하는 관리 영역이다.
- **목표 연계(Goals Cascade)**: 이해관계자 요구를 기업 목표•정렬 목표•거버넌스 및 관리 목표로 변환하는 연결 방식이다.
- **거버넌스와 관리 분리**: 이사회가 EDM으로 방향과 위험 수용을 결정하고 경영진이 APO•BAI•DSS•MEA로 실행하는 책임 원칙이다.
