...
- 정의/개념: 산업 진흥•공공 이용•보안인증•이용자 보호를 규율한 **클라우드 진흥•보호 법률**이다.
- 배경/필요성: 자체 구축 중심 제도로는 **탄력적 자원•공공 이용•데이터 보호** 기준 확보하기 어렵다.

#### 한줄 요약

- **공공 클라우드 이용**과 **서비스•데이터 보호**의 법률이 핵심이다.

## Ⅱ. 특징

<details><summary>핵심 용어</summary>

- **공공 이용 촉진**: 정보화사업의 클라우드•인증 서비스를 검토한다.
- **서비스 신뢰 보장**: 보안인증•품질•성능으로 보호 수준을 확인한다.
- **공동 책임 관리**: 제공자 기반과 이용자 데이터•계정 책임을 연결한다.
...
- **CSAP 인증 범위**와 **이용기관 데이터•계정 책임** 병행이 핵심이다.
...
- **공동 책임**: 제공자 기반과 이용자 계정•설정•데이터 통제를 구분한다.
...
- **SLA•공동 책임**과 **데이터 반출•파기**로 종속을 완화한다.
...
1. **업무 중요도•인증 범위 확인**: 데이터•서비스 영향에 맞는 배치 방식과 보안인증 등급•유효 범위를 검토한다.
...
5. **데이터 반환•파기 검증 요청**: 복원 가능성과 잔존 데이터 제거를 확인한다.

#### 한줄 요약

- **업무 중요도•CSAP**로 도입하고 **SLA•종료 조건**을 검증한다.
...
- **공공 클라우드**는 공동 책임, **자체 구축**은 단독을 통제한다.
...
- **업무 중요도•CSAP 유효성**으로 **배치 방식**을 결정한다.

## Ⅶ. 결론

<details><summary>핵심 용어</summary>

- **인증 범위 확인**: 서비스•리전•기능•조직의 인증 경계를 확인한다.
- **종료 시험**: 데이터 반출•복원•잔존 데이터 파기를 검증한다.

</details>

- 업무 중요도에 맞는 **CSAP 범위** 선택 후 **공동 책임•종료 시험** 계약이 핵심이다.

#### 한줄 요약

- **업무 중요도**에 맞춰 CSAP와 **데이터 반출•파기** 확정이 핵심이다.
