...
- **사업 목표**: 개별 구축사업이 달성할 성과와 측정 지표를 확정한 기준이다.
- **사업 경계**: 구축 범위의 포함•제외 대상과 외부 연계 책임을 확정한 기준이다.
- **목표 구조**: 요구사항을 구현할 응용•데이터•기술 구조를 구체화한 설계 기준이다.
- **목표 규모**: 기능 규모•용량•성능을 비용과 일정 산정 기준으로 구체화한 값이다.
