...
- **API 플랫폼**: API 플랫폼은 기관별 데이터와 행정 기능을 표준 접점으로 연결해 공동 활용하게 한다.
...
- **2. 공공 데이터 조회 요청**: 플랫폼은 법적 근거와 동의·접근 권한을 확인한 뒤 공통 API로 기관의 검증 데이터를 요청한다.
...
- **표준 API**: 민관 서비스 연계에는 표준 API를 적용하고 개인정보 처리는 법적 근거와 최소 권한을 우선 확인해야 한다.
