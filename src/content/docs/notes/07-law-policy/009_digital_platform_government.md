...
date: "2026-08-06T16:03:00+09:00"
tags:
  - "notes-law_policy"
weight: 9
extra:
...
- **디지털 플랫폼 정부(DPG)**: 공공 데이터•기능과 민간 역량의 연결 모델
- **응용 프로그래밍 인터페이스(API)**: 데이터•기능의 표준 요청•응답 접점
- **정부 운영 모델**: 공공 데이터•서비스•민간 역량의 연결 체계
...
#### 한줄 요약
- **공공 데이터•행정 기능**을 표준 API로 연결해 **공동 활용**
...
- **마이데이터(MyData)**: 정보주체의 데이터 조회•전송•활용처 통제
- **민관 협력**: 민간 역량 활용과 보안•개인정보 책임 통제
...
- **디지털 신원**: 온라인 신원 검증과 서비스별 접근 권한 정보
- **정부기술(GovTech)**: 민간 기술로 공공 서비스를 개선하는 분야
- **공공 데이터**: 표준•품질•권한•책임이 관리되는 행정 데이터
- **민간 참여**: 공개 데이터•기능을 활용한 공공 서비스 공동 혁신
...
#### 한줄 요약
- **API 플랫폼**이 데이터를 연결하고 **디지털 신원**으로 권한 관리
...
#### 한줄 요약
- **법적 근거•동의** 확인 후 데이터를 조회해 **원스톱 서비스** 제공
...
#### 한줄 요약
- **전자정부**는 절차 온라인화, **DPG**는 데이터•기능 연결
...
- **책임 경계**: 민관 장애•침해 대응의 데이터•운영 주체 구분
- **접근 이력 감사**: 개인정보 조회 기록과 승인 근거 대조
...
#### 한줄 요약
- 전입신고 정보의 **기관 연계**로 **반복 제출•개별 처리** 감소
...
- **한 번만 제출 원칙(Once Only)**: 제공한 행정 정보를 재요구하지 않는 원칙
- **표준 API**: 동일한 요청•응답•보안 규칙의 연계 접점

</details>

- 반복 제출은 **Once Only**, 연계•개인정보는 **표준 API•최소권한** 적용

#### 한줄 요약
- **법적 근거•책임**을 확정하며 **기관 연계•민간 활용** 확대
