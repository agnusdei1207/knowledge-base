...
- 정의: **5G** 기반 표준은 **ITU-R** 국제 요구와 **3GPP** 상세 규격을 연결하는 체계이다.
...
- **TS** 및 **TR** 바탕으로 **NR**•코어망 구현 규격을 개발한다.
- **3GPP Release** 동결•채택•호환 검증을 거쳐 단계적으로 상용화한다.
...
1. IMT 성능 요구•평가 절차 제공: **IMT** 세대별 성능 목표와 후보 평가 기준을 설정한다.
...
- 상용 구성은 기존 코어를 쓰는 **NSA** 및 5G 전용 코어를 쓰는 **SA** 기반 구분한다.
...
| 국가별 대역 차이 | **주파수 적합성** 대상 따라 밴드•출력•채널 검증 | 지역별 장비 부적합 예방 |
...
- **국제 요구-상세 규격 관계** 대상 따라 국제 적합성은 **ITU-R**, 제품•망 연동은 같은 **3GPP Release** 기반 검증한다.
