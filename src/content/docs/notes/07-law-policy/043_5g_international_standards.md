...
- 배경/필요성: 사업자가 독자 규격을 쓰면 단말•망 조합마다 별도 검증과 장비가 필요해 상호운용과 국제 로밍이 비용 때문에 막히므로, ITU-R이 성능 요구를 정하고 3GPP가 상세 규격으로 구현하는 2단 표준 체계를 개별 규격 위에 두어 사업자 간 개별 합의를 국제 공통 규격으로 대체할 필요
