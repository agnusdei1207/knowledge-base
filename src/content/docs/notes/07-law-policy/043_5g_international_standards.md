...
- **5G 국제 표준 체계**: ITU-R의 IMT 성능 요구·무선 인터페이스 승인과 3GPP의 무선·코어망 상세 규격을 연결한 체계이다.
- **5세대 이동통신(5G)**: 초고속·초저지연·초연결 서비스를 지원하도록 정의한 이동통신 세대이다.

</details>

- 정의/개념: **5G 국제 표준 체계**는 ITU-R의 IMT-2020 성능 요구·무선 인터페이스 승인과 3GPP의 무선·코어망 상세 규격을 연결하는 **이동통신 표준 체계**
- 배경/필요성: 독자 규격만으로는 단말·망의 **상호운용성**과 국제 로밍 보장 곤란
...
- **IMT-2020**: ITU-R이 5G 후보 기술의 성능 요구와 평가 방법 및 승인 무선 인터페이스를 정의한 국제 체계이다.
- **3GPP Release**: 무선·코어·서비스 규격을 기능 묶음과 일정에 따라 동결하여 배포하는 버전 단위이다.

</details>

- **IMT-2020** 요구와 무선 인터페이스 승인 기준
- **3GPP TS·TR**로 정의하는 NR·코어망 구현 규격
...
- **ITU-R**: 국제 주파수 이용을 조정하고 무선통신 권고와 IMT 요구를 개발하는 ITU 부문이다.
- **3GPP**: 이동통신 시스템의 무선·코어망 상세 규격을 개발하는 국제 표준협력체이다.
- **NR·RAN**: 5G 단말-기지국 무선 전송 규격과 단말을 코어망에 연결하는 무선 네트워크 영역이다.
...
- **기술 규격(TS)**: 3GPP 구현에 필요한 규범적 요구사항을 담은 문서이다.
- **기술 보고서(TR)**: 후보 기술·요구·영향을 조사하고 규격화 근거를 정리한 문서이다.
...
- **비독립모드(NSA)**: 기존 LTE 코어망과 제어 기능을 활용하면서 5G NR을 함께 사용하는 구성이다.
- **독립모드(SA)**: 5G 전용 코어망과 NR로 슬라이싱·저지연 등 5G 기능을 온전히 제공하는 구성이다.
...
- 국제 적합성은 **ITU-R**, 제품·망 연동은 동일 **3GPP 릴리즈**로 검증 후 상용화 승인
