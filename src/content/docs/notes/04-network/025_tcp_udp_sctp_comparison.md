...
- 정의/개념: **TCP•UDP•SCTP 비교** — 연결•신뢰성•메시지 경계•멀티스트리밍•멀티호밍 요구에 따라 세 전송계층 프로토콜을 선택하는 **기준**이다.
- 배경/필요성: 하나의 방식으로 **낮은 지연•신뢰•메시지 경계** 모두 최적화할 수 없다.
...
- TCP의 **신뢰성 바이트 스트림**이 핵심이다.
- UDP의 **무연결 메시지•응용 복구**가 핵심이다.
- SCTP의 **멀티스트리밍•멀티호밍**이 핵심이다.
...
- 순서•복구는 **TCP**, 낮은 지연은 **UDP**, 다중 경로는 SCTP을 선택한다.
