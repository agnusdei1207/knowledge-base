...
date: "2026-07-25T19:54:00+09:00"
tags:
  - "notes-network"
weight: 54
extra:
...
- **정의/개념**: Zigbee·Thread는 망, Matter는 응용 표준
- **배경/필요성**: 멀티벤더 기기의 명령·보안 모델 통일
...
- Zigbee는 자체 메시망과 응용 계층을 사용한다.
- Thread는 802.15.4에서 IPv6를 라우팅한다.
- Matter는 IP망에서 공통 기기 모델을 사용한다.
- BLE는 기기 발견·가입에 쓰며 운영 경로와 구분된다.
- 브리지·경계 라우터 장애는 이기종 제어를 끊을 수 있다.
...
| 설계 요소 | 설명 |
|:---|:---|
| 컨트롤러·커미셔너 | 기기 가입 승인과 Matter 명령 제어 |
| IP 네트워크 | Matter 메시지의 공통 전달 기반 |
| Thread 경계 라우터 | Thread와 외부 IPv6망 사이 라우팅 |
| Thread Matter 기기 | Thread로 Matter 명령·상태 교환 |
| 매터 브리지 | Zigbee 명령·상태를 Matter로 변환 |
| Zigbee 기기 | 비 IP Zigbee 메시망으로 통신 |
...
| 절차 | 설명 |
|:---|:---|
| 가입 대상 발견 | BLE·QR 코드로 가입 기기 연결 |
| 기기 증명 검증 | 제조사 증명서로 기기 진위 확인 |
| 네트워크 자격 전달 | Wi-Fi·Thread 접속 정보를 암호화 전달 |
| IP 네트워크 가입 | 전달받은 자격으로 운영망 접속 |
| 운영 인증서 등록 | 패브릭 신원·접근 권한 부여 |
...
| 주요 위험 | 허브·브리지 종속 | 경계 라우터 구성 오류 | 인증서·패브릭 권한 오설정 |
...
1. 스마트홈은 Thread 경계 라우터를 이중 배치
2. 기존 Zigbee 조명은 매터 브리지로 명령 변환
...
- 기존 기기·IP 메시망 요구로 Zigbee·Thread·Matter 조합
