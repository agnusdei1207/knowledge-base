...
- 배경/필요성: 1세대 Zigbee의 비-IP 구조 및 제조사별 폐쇄적 전용 허브 종속으로 인한 **이기종 기기 간 직접 연동 불가, 플랫폼 파편화 및 단일 장애점(SPOF) 발생**
...
#### 한줄 요약
- QR 스캔 → BLE PASE 수립 → DAC 검증 → 자격 증명 주입 → Matter 패브릭 가입 순으로 동작한다.
