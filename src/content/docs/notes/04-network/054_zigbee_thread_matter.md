...
- 정의/개념: **지그비**, **스레드**, **매터**는 자체망•**인터넷 프로토콜 버전 6** 경로•**인터넷 프로토콜** 응용을 담당하는 **사물인터넷** 표준이다.
- 배경/필요성: 제조사별 기기 규격은 명령과 보안 의미가 달라 상호운용하기 어렵다.
...
- **Zigbee**는 자체 메시망과 응용 프로파일을 사용한다.
- **Thread**는 **IEEE** 802.15.4 기반 IPv6 메시 경로를 제공한다.
- **Matter**는 IP 기기 모델•명령•보안 의미를 통일한다.
...
**Thread 경계 라우터**는 **IPv6** 패킷을 전달하고 **Matter 브리지**는 비 Matter 기기의 모델과 명령을 변환한다.
...
- **커미셔닝**은 **빠른 응답 코드**로 가입 정보를 받고 **기기 증명서**로 진위를 확인한 뒤 네트워크 접속 정보와 운영 권한을 등록한다.
...
- **패브릭** 안의 저전력 IP 경로에는 **Thread**, 제조사 간 제어에는 **Matter**, 기존 **Zigbee** 기기 연동에는 **Matter 브리지**를 선택한다.
