...
- 정의/개념: 통신 증거로 침해 시간•주체•경로를 복원하는 **디지털 조사 기법**이다.
- 배경/필요성: 호스트 로그만으로 **망 경로 복원하기 어렵다**.
...
- **패킷•플로우•로그**: 조사 질문별 상호 보완 수집이다.
- **NTP•NAT•DHCP**: 사건 시간과 실제 신원을 연결한다.
- **해시•연계보관성**: 증거 무결성과 관리 책임을 입증한다.
...
1. **원본 증거 전달**: 패킷•플로우와 수집 조건을 제공한다.
2. **검증 사본 제공**: 원본을 보존하고 분석용을 복제한다.
3. **시간•신원 기록 전달**: NTP•NAT•DHCP 이력을 제공한다.
4. **증거 교차 검증**: 서로 다른 증거로 결론을 확인한다.
...
- 명령 내용은 **전체 패킷**, 장기 경로는 **플로우**, 사용자 확인은 **인증 로그**가 핵심이다.
