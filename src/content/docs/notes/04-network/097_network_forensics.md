...
- **NTP·NAT·DHCP·SNI**: 패킷의 시간·변환 주소·단말 임대정보·접속 서버명을 연결해 통신 주체와 경로를 재구성하는 조사 단서
...
1. 공격 통신의 **NAT·DHCP 로그 기반 단말 추적**

### 쉽게 이해하기 (학습용)

- 외부에 기록된 공인 IP·포트를 NAT 로그에서 찾고 같은 시각의 DHCP·로그인 기록을 연결해 실제 단말과 사용자를 확인한다.

## Ⅶ. 결론

- 사고 후 통신 주체와 경로를 재구성하기 위해 조사 질문·시간 동기·NAT·DHCP·흐름·패킷·보존기간을 검토하여, 평시부터 필요한 네트워크 증거 수집을 설계해야 한다.
