...
- **MQTT·QoS·ACL·TLS**: MQTT 메시징에서 전달 보장 수준·토픽 접근 권한·전송 암호화를 담당하는 프로토콜과 통제 요소
...
1. 설비 측정값과 고장 알림의 **MQTT QoS 차등 적용**

### 쉽게 이해하기 (학습용)

- 주기 측정값은 QoS 0으로 가볍게 보내고 반드시 도착해야 하는 고장 알림은 QoS 1로 전송해 유실 비용에 맞춘다

## Ⅶ. 결론

- 제한된 IoT망의 전송 부담과 메시지 신뢰성을 균형 있게 확보하기 위해 유실·중복 비용·토픽 중요도·세션 복구·보존 메시지를 검토하여, 토픽별 MQTT QoS와 세션을 설정해야 한다.
