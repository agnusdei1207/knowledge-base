...
date: "2026-07-25T20:18:00+09:00"
tags:
  - "notes-network"
weight: 78
extra:
...
- **정의**: 브로커 기반 경량 발행·구독 프로토콜
- **필요성**: 간헐 연결 장치의 송수신 결합 제거
...
- 토픽이 발행자와 구독자의 주소 의존을 없앤다.
- 세션이 재접속 뒤 구독과 미확인 상태를 복구한다.
- QoS가 높을수록 유실은 줄고 상태·지연은 늘어난다.
- 브로커 장애는 다수 연결과 메시지 전달을 중단시킨다.
...
| 판단 기준 | QoS 0 | QoS 1 | QoS 2 |
|:---|:---|:---|:---|
| 핵심 특징 | 확인 없는 최대 한 번 전달 | 확인까지 재전송하는 최소 한 번 전달 | 2단계 확인의 정확히 한 번 전달 |
| 적용 기준 | 일부 유실 허용 센서 데이터 | 중복 제거 가능한 상태·알림 | 중복 영향이 큰 제어 메시지 |
| 주요 위험 | 유실 확인·복구 불가 | 중복 제거 없으면 업무 반복 | 확인·저장 비용과 복구 복잡성 |
