...
- 정의/개념: 브로커가 토픽 메시지를 중계하는 **경량 발행•구독 프로토콜**이다.
- 배경/필요성: 직접 주소 결합은 **간헐 연결 장치의 오프라인 전달하기 어렵다**.
...
- **공간 비결합**: 토픽으로 발행자•구독자 주소를 분리한다.
- **세션 복구**: 연결 단절 후 구독•대기 전달 재개이다.
- **QoS 상충**: 신뢰성 향상과 확인•저장 비용 증가한다.
...
1. **세션 연결**: 식별자•인증 후 이전 상태를 복구한다.
2. **토픽 구독**: 토픽 필터와 ACL 허용 범위를 등록한다.
3. **메시지 발행**: 토픽•페이로드•QoS를 브로커에 전송한다.
4. **토픽 라우팅**: 필터 일치 구독별 QoS로 전달한다.
...
- 유실 허용 측정은 **QoS 0**, 멱등 알림은 **QoS 1**, 중복 위험 제어는 **QoS 2**가 핵심이다.
