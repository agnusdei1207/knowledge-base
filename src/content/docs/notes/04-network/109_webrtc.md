...
- 정의/개념: 브라우저 간 **실시간 음성•영상•자료** 활용 암호화해 교환하는 통신 기술이다.
- 배경/필요성: 플러그인•NAT의 **배포•직접 경로 제약** 발생한다.
...
| 구성요소 | 책임 |
|:---|:---|
| 단말 A | **단말 A** SDP 제안•미디어 송신 |
| 시그널링 서버 | **시그널링 서버** SDP•ICE 후보 전달 |
| 단말 B | **단말 B** SDP 응답•미디어 수신 |
| STUN•TURN 서버 | **STUN•TURN 서버** 공인 주소 탐색•중계 제공 |
| SFU•MCU 서버 | **SFU•MCU 서버** 선택 전달•미디어 합성 |
| 품질•세션 관측기 | **품질•세션 관측기** 손실•지터•중계 비율 수집 |
...
1. SDP 제안 등록: 단말 A의 코덱•전송 방향 조건을 **SDP 제안 등록** 기반 제공한다.
2. SDP 응답 등록: 단말 B의 수락 조건을 **SDP 응답 등록** 기반 제공한다.
3. ICE 후보 수집: 호스트•공인•중계 주소의 **ICE 후보 수집** 수행한다.
4. ICE 경로 검사•선택: 후보 쌍 연결성을 검사해 **ICE 경로 검사•선택** 수행한다.
5. DTLS-SRTP 보안 전송: 상대 인증•키 합의 후 **DTLS-SRTP 보안 전송** 수행한다.
...
- 소수 직접 연결은 **P2P**, 일반 다자는 **SFU**, 합성•변환은 **MCU** 활용 택하는 **WebRTC 다자 구조 선택** 필요하다.
