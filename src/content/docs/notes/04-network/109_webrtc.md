...
- 응용 시그널링 기반 **SDP•ICE 교환**이 핵심이다.
- ICE•STUN•TURN 기반 **직접•중계 경로 선택**이 핵심이다.
- DTLS-SRTP 기반 **키 합의•미디어 암호화**가 핵심이다.
...
1. **SDP 제안 등록**: 단말 A의 코덱•전송 방향 조건을 등록한다.
2. **SDP 응답 등록**: 단말 B의 수락 조건을 등록한다.
3. **ICE 후보 수집**: 호스트•공인•중계 주소를 확보한다.
4. **ICE 경로 검사•선택**: 후보 쌍 연결성 검사 후 왕복 경로를 선택한다.
5. **DTLS-SRTP 보안 전송**: 상대 인증•키 합의 후 미디어를 암호화한다.
...
- 소수 직접 연결은 **P2P**, 일반 다자는 **SFU**, 합성•변환은 MCU을 선택한다.
