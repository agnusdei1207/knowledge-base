...
- **스위칭 계층**: 장비가 전달 대상을 정할 때 읽는 링크•네트워크•전송•응용 계층 정보의 구분한다.
...
- 정의/개념: **L2•L3•L4•L7 스위치** — 각각 링크•네트워크•전송•응용 계층 정보를 기준으로 출력 경로나 대상 서버를 선택하는 **네트워크 전달 장비**이다.
- 배경/필요성: 주소 전달만으로는 **연결 분산•콘텐츠 분기할 수 없다**.
...
- L2•L3의 **MAC•IP 기반 경로 전달**이 핵심이다.
- L4의 **5-튜플•연결 상태 분산**이 핵심이다.
- L7의 **URL•헤더 기반 서비스 분기**가 핵심이다.
...
- 필요한 **분기 정보•상태 비용**을 충족하는 최소 스위칭 계층을 선택한다.
