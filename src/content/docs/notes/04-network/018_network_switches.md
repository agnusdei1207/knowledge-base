...
1. **L2 전달**: 목적지 MAC 주소표로 출력 포트 선택
2. **L3 전달**: 목적지 IP 프리픽스로 다음 홉 선택
3. **L4 분산**: 5-튜플·연결 상태로 서버 선택
4. **L7 분기**: URL·헤더·쿠키로 서비스 선택
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 계층보다 높은 정보 검사 | 암호화 여부·가시성 사전 확인 | 분기 실패 예방 |
| L4 연결 상태 고갈 | 세션 한도·만료시간 조정 | 처리 가용성 확보 |
| L7 규칙 복잡도 증가 | 경로·헤더 규칙 표준화 | 정책 오류 감소 |
| 단일 스위치 장애 | 이중화·상태 동기화 구성 | 서비스 연속성 확보 |
...
- **필요한 분기 정보와 상태 비용에 따른 L2·L3·L4·L7 선택**
