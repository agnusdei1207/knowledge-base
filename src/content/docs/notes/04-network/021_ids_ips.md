...
date: "2026-08-26T13:36:39+09:00"
tags:
  - "notes-network"
weight: 21
extra:
...
- 배경/필요성: L3/L4 방화벽이 허용한 정상 80/443 포트로 유입되는 **애플리케이션 취약점 익스플로잇, 악성코드 페이로드 침투 방어 불가**를 겪으므로, 방화벽 뒤에 페이로드를 시그니처·이상 징후와 대조하는 심층 검사 계층을 두고 지연 없이 탐지만 하는 미러링(IDS)과 지연을 감수하고 차단까지 하는 인라인(IPS) 중 하나를 고르게 할 필요
...
- 오탐 검증 전은 **IDS**, 검증 후 인라인은 **IPS**로 승격
