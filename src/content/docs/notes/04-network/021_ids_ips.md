...
- 정의/개념: **IDS•IPS** — 네트워크•호스트 활동에서 침입 징후를 분석하되 IDS는 탐지•경보하고 IPS는 인라인에서 악성 트래픽을 차단하는 **보안 시스템**이다.
- 배경/필요성: 방화벽이 허용한 트래픽에 숨은 **취약점 악용** 공격 식별 한계가 발생한다.
...
- IDS의 **경로 외 복제 분석•경보**가 핵심이다.
- IPS의 **인라인 분석•즉시 차단**이 핵심이다.
- 시그니처•이상 탐지의 **공격 식별 상호 보완**이 핵심이다.
...
2. **탐지 결과 전달**: **시그니처•이상 탐지**로 공격 유형과 신뢰도를 산출한다.
3. **대응 명령**: **IDS 경보•IPS 차단** 중 정책에 맞는 조치를 선택한다.
...
- **IDS**로 탐지 신뢰도를 검증하고 서비스 영향이 허용되면 **IPS** 차단을 전환한다.
