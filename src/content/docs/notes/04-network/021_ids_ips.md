...
- 탐지 엔진이 시그니처 일치·위협 등급을 정책 엔진에 전달하면 정책 엔진이 경보 또는 인라인 차단 중 하나를 수행한다.
...
- 신규 시그니처를 IDS 모드에서 먼저 검증하고 탐지 신뢰도와 Fail-Open 경로를 확인한 뒤 IPS 차단을 활성화하면 오탐에 의한 정상 트래픽 차단을 줄일 수 있다.
