...
| 구성요소 | 책임 |
|:---|:---|
| 세션 정규화기 | 패킷 재조립·디코딩으로 **우회 공격 무력화** |
| 탐지 엔진 | **시그니처·이상 징후 분석** |
| 정책 판정 엔진 | 위험도별 **경보·차단 결정** |
| 대응 실행기 | **Alert·Drop·TCP RST** 집행 |
| 하드웨어 바이패스 | IPS 고장 시 **Fail-Open 연결** |
...
IDS / IPS 패킷 인입 및 탐지·차단 파이프라인
        │
   [트래픽 수집] TAP/SPAN 미러링(IDS) 또는 인라인 직렬 수신(IPS)
        │
   [세션 정규화] 패킷 재조립 및 디코딩을 통한 우회 시도 차단
        │
   [시그니처 대조] CVE 취약점 룰셋 및 이상 징후 프로파일링 매칭
    ┌────┴───────────────────────────┐
   정상 트래픽                      악성 공격 식별
    │                                 │
  [인라인 정상 통과]                 ┌─┴─────────────────────────────┐
...
- 방화벽 후단에서 심층 페이로드를 실시간 분석하여 취약점 공격을 차단하는 **엔터프라이즈 네트워크 경계 및 호스트 방어의 핵심 능동 보안 인프라**로 자리잡았으며, 최근에는 단일 장비를 넘어 머신러닝 기반 이상 징후 탐지와 클라우드 NDR(Network Detection and Response) 및 XDR 플랫폼으로 진화함과 동시에, 실무 구축 시에는 **오탐(False Positive)으로 인한 서비스 차단 사고를 방지하기 위한 선 IDS(Alert-Only) 검증 후 단계적 IPS 인라인 전환, 장비 결함 시 망 단절을 방지하는 하드웨어 바이패스(Fail-Open), 암호화 위협 가시성을 확보하는 SSL/TLS 복호화 연동**을 결합하여 고가용성 능동 방어를 완성
