...
- **직교 주파수 분할 다중화(Orthogonal Frequency-Division Multiplexing, OFDM)**: 직교 부반송파로 한 전송의 데이터를 병렬화하는 다중화 방식
...
- **지연 확산(Delay Spread)**: 다중 경로 신호의 도착 시간 차이가 퍼진 범위로 순환 전치 길이의 기준
