...
- 배경/필요성: 단일 반송파는 **다중 경로 왜곡•광대역 등화 부담**이 발생한다.
...
- **직교 부반송파** 병렬화로 주파수 선택적 채널 등화 단순화가 핵심이다.
- **OFDMA 자원 단위** 배정으로 다중 사용자 동시 접속이 핵심이다.
- **순환 전치 오버헤드•높은 PAPR**에 따른 전력•전송 효율 저하된다.
...
- 단일 사용자 광대역은 **OFDM**, 다중 사용자 자원 배정은 **OFDMA**을 선택한다.
