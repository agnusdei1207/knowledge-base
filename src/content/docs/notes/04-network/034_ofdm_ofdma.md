...
block
    columns 1
    A["자원 스케줄러 | 사용자별 자원 단위 할당"]
    B["심볼·자원 매퍼 | 데이터를 부반송파에 배치"]
    C["송신 파형 변환기 | 병렬 심볼을 파형으로 변환"]
    D["무선 채널 | 다중 경로·잡음 반영"]
    E["수신 파형 변환기 | 파형에서 부반송파 분리"]
    A --- B
    B --- C
    C --- D
    D --- E
...
sequenceDiagram
    ...
    송신변환기->>송신변환기: 2. 심볼·부반송파 매핑
    송신변환기->>무선채널: 3. 병렬 파형 생성
    무선채널->>수신변환기: 4. 다중 경로 신호 전달
    수신변환기->>자원스케줄러: 5. 부반송파 분리
```

**동작 원리**

1. **사용자 자원 할당**: 채널 상태별 자원 단위 배정
2. **심볼·부반송파 매핑**: 데이터를 주파수 위치에 배치
3. **병렬 파형 생성**: 순환 전치를 붙여 신호 생성
4. **다중 경로 신호 전달**: 반사·잡음이 포함된 신호 수신
5. **부반송파 분리**: 수신 파형을 주파수별 분리
