---
sidebar:
  order: 95
  label: "095. SIEM vs SOAR 비교 (SIEM vs SOAR Comparison)"
  badge:
    text: "기출 • 50%"
    variant: note
title: "SIEM vs SOAR 비교 (SIEM vs SOAR Comparison)"
date: "2026-08-06T23:27:50+09:00"
tags: ["notes-network"]
weight: 95
extra:
  question_no: "095"
  source_status: "기출"
  source_history: "138회"
  priority: 50
  priority_note: "비교형: 138회 SIEM•SOAR 직접 출제"
---

## Ⅰ. 개요

<details><summary>핵심 용어</summary>

- **보안 정보•이벤트 관리(Security Information and Event Management, SIEM)**: 이종 로그를 정규화•상관분석해 근거가 있는 보안 경보를 생성하는 플랫폼이다.
- **보안 오케스트레이션•자동화•대응(Security Orchestration, Automation, and Response, SOAR)**: 경보를 보강하고 승인•조치•원복 절차를 플레이북으로 실행하는 플랫폼이다.
- **폐루프 보안 관제 구조**: SIEM 탐지 근거가 SOAR 대응을 만들고 조치 결과가 다시 탐지와 대응 절차를 개선하는 구조이다.
- **대응 지연**: 탐지와 조치 절차가 분리되어 사건 정보 전달과 도구 실행이 늦어지는 문제이다.

</details>

- 정의/개념: 탐지 SIEM과 대응 SOAR를 연결한 **폐루프 보안 관제 구조**이다.
- 배경/필요성: 탐지•조치 분리의 **대응 지연**이 발생한다.

#### 한줄 요약

- SIEM이 여러 기록에서 사건과 이유를 찾으면 SOAR가 그 사건을 조사•조치하고 결과를 다시 SIEM 개선에 돌려준다.

## Ⅱ. 특징

<details><summary>핵심 용어</summary>

- **상관분석**: 시간•사용자•자산•주소가 연결된 여러 이벤트를 하나의 공격 흐름으로 묶는 분석이다.
- **플레이북**: 사건 조건, 정보 조회, 승인, 조치와 결과 검증을 실행 가능한 순서로 정의한 절차이다.
- **폐루프 관제**: 탐지 근거가 대응을 만들고 대응 결과가 다시 탐지 규칙과 플레이북을 개선하는 순환 체계이다.
- **SIEM 탐지**: 로그 정규화와 상관분석으로 공격 근거가 있는 경보를 생성하는 기능이다.
- **SOAR 대응**: 경보 정보를 보강하고 승인 수준에 맞는 플레이북을 실행하는 기능이다.
- **결과 환류**: 실제 조치 증거와 조사 판정을 탐지 규칙과 대응 절차에 반영하는 기능이다.

</details>

- **SIEM 탐지**: 로그 **상관분석**과 근거 경보를 생성한다.
- **SOAR 대응**: 정보 보강과 **플레이북**을 실행한다.
- **결과 환류**: 조치 증거를 **폐루프 관제**의 탐지 규칙에 반영한다.

#### 한줄 요약

- 틀린 경보를 자동화하면 더 빨리 잘못 막으므로 조치 전 신뢰도와 영향도를 다시 확인해야 한다.

## Ⅲ. 구조 및 구성요소

<details><summary>핵심 용어</summary>

- **경보 계약**: SIEM이 SOAR에 전달할 사건 ID•신뢰도•근거•자산을 정한 자료 규격이다.
- **탐지 규칙**: 공격 조건과 임계값을 논리로 표현한 경보 생성 기준이다.
- **보안 통제 도구**: 계정•단말•메일•네트워크 상태를 바꾸는 제품이다.
- **SIEM 탐지부**: 로그 정규화•상관분석으로 경보와 근거를 생성하는 구성요소이다.
- **SOAR 대응부**: 경보 맥락을 보강하고 승인•플레이북 조치를 실행하는 구성요소이다.
- **대응 결과 환류부**: 조치 증거를 탐지 규칙과 대응 절차 개선에 전달하는 구성요소이다.

</details>

```text
폐루프 보안 관제
├─ SIEM 탐지부
├─ 경보 계약
├─ SOAR 대응부
├─ 보안 통제 도구
└─ 대응 결과 환류부
```

선의 의미: 가로선은 SIEM 탐지 결과가 경보 계약을 경계로 SOAR 대응부 및 보안 통제 도구에 결합되는 관계이고, 아래 가지는 통제 도구의 실제 결과를 SIEM 규칙과 대응 절차에 연결하는 환류 관계를 뜻한다.

| 구성요소 | 책임 |
|:---|:---|
| SIEM 탐지부 | **SIEM 탐지부**가 정규화•상관분석•경보 생성 |
| 경보 계약 | **경보 계약**이 사건•근거•신뢰도•자산 전달 |
| SOAR 대응부 | **SOAR 대응부**가 보강•승인•플레이북 실행 |
| 보안 통제 도구 | **보안 통제 도구**가 계정•단말•메일•망 상태 변경 |
| 대응 결과 환류부 | **대응 결과 환류부**가 조치 증거 기반 **탐지 규칙**•절차 개선 |

#### 한줄 요약

- SIEM 경보가 공통 규격으로 SOAR에 넘어가고 보안 도구의 실제 조치 결과가 다시 탐지와 대응 절차를 고친다.

## Ⅳ. 흐름도

<details><summary>핵심 용어</summary>

- **조치 증거**: SOAR가 실행한 권한•조치와 실제 상태 변화를 남긴 기록이다.
- **대상 상태 재조회**: 조치 뒤 실제 자원 상태를 다시 확인하는 검증이다.
- **정보 보강**: 경보에 자산•신원•위협 정보를 추가하는 처리이다.
- **원본 로그 전달**: 접속•변경•통신 기록을 SIEM 수집 계층에 제공하는 단계이다.
- **경보 계약 전달**: 사건 ID•자산•신뢰도•근거를 공통 규격으로 SOAR에 제공하는 단계이다.
- **조치 승인 요청**: 사건 중요도와 조치 가역성을 근거로 담당자 승인을 요청하는 단계이다.
- **플레이북 조치 요청**: 승인 수준에 맞는 보안 통제 도구의 조치를 호출하는 단계이다.

</details>

```text
1. 원본 로그 전달
      │
      ▼
2. 경보 계약 전달
      │
      ▼
3. 조치 승인 요청
      ├─ 거부: 수동 조사
      └─ 승인
          │
          ▼
4. 플레이북 조치 요청
          │
          ▼
5. 대상 상태 재조회
          ├─ 불일치: 실패•원복 처리
          └─ 일치: 조치 증거를 SIEM에 환류
                         └─ 탐지 규칙 개선
```

### 동작 원리

1. **원본 로그 전달**: 접속•변경•통신 기록을 **원본 로그 전달**로 SIEM에 제공한다.
2. **경보 계약 전달**: **정보 보강**한 사건 ID•자산•신뢰도•근거를 **경보 계약 전달**로 제공한다.
3. **조치 승인 요청**: 중요도•가역성에 따라 **조치 승인 요청**으로 실행 수준을 판단한다.
4. **플레이북 조치 요청**: 승인 수준에 맞춰 **플레이북 조치 요청**을 수행한다.
5. **대상 상태 재조회**: **대상 상태 재조회**로 실제 상태를 검증하고 **조치 증거**를 남긴다.

#### 한줄 요약

- SIEM이 사건 근거를 보내면 SOAR가 영향도를 보고 조치하며 실제 결과가 맞는지 확인해 다음 탐지와 대응을 고친다.

## Ⅴ. 종류 및 비교

| 보안 관제 플랫폼 | **SIEM** | **SOAR** |
|:---|:---|:---|
| 적용 기준 | 이종 로그의 공격 근거 탐지 | 반복 사건의 조사•조치 표준화 |
| 핵심 특징 | 상관분석•경보 생성 | 플레이북 기반 대응 실행 |
| 한계 | 로그 품질 저하•오탐 | 오탐 자동화•권한 집중 |

> 요약: SIEM은 탐지 근거, SOAR는 조치 결과가 핵심이다.

#### 한줄 요약

- 공격인지 판단할 기록이 필요하면 SIEM, 확인된 사건을 빠르고 같은 절차로 처리하려면 SOAR가 필요하다.

## Ⅵ. 실무 고려사항 및 대책

<details><summary>핵심 용어</summary>

- **신뢰도**: 경보가 실제 공격일 가능성을 나타내는 판단 값이다.
- **권한 집중**: 대응 플랫폼에 여러 통제 도구의 강한 권한이 모이는 위험이다.

</details>

| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 경보의 근거•**신뢰도** 누락 | 필수 필드의 **경보 계약** 정의 | 오탐 자동화의 확산 방지 |
| 도구 응답과 실제 상태 불일치 | 조치 후 **대상 상태 재조회** | 부분 실패와 미조치 식별 |
| 환류 없는 규칙 성능 저하 | **권한 집중**을 통제하고 **폐루프 관제**에 오탐•실패 반영 | 탐지•대응의 지속 개선 |

#### 한줄 요약

- SIEM이 낯선 로그인과 권한 변경을 상관분석해 경보를 만들면 SOAR가 영향도를 확인하고 승인 후 계정을 잠근다.

## Ⅶ. 결론

<details><summary>핵심 용어</summary>

- **SIEM•SOAR 결합 기준**: 공격 근거 생성은 SIEM, 반복 조치 실행은 SOAR, 개선은 결과 환류에 배정하는 역할 판단이다.

</details>

- 공격 근거는 **SIEM**, 반복 조치는 **SOAR**, 결과는 **결과 환류**로 연결하는 **SIEM•SOAR 결합 기준**이 핵심이다.

#### 한줄 요약

- SIEM과 SOAR는 앞뒤 제품을 붙이는 데서 끝나지 않고 탐지 이유와 조치 결과가 서로 개선되는 순환으로 운영해야 한다.
