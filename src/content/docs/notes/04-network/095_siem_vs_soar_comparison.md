...
- **정의/개념**: SIEM은 탐지 근거, SOAR는 대응 결과 생성
- **배경/필요성**: 탐지·대응 책임과 완료 기준 분리
...
- SIEM 오탐이 SOAR에서 대규모 오조치로 증폭한다.
- 공통 사건 식별자로 탐지부터 조치까지 추적한다.
- 탐지 규칙과 플레이북의 변경을 함께 검증한다.
- 제품 연동만으로 조사 책임·승인 기준은 정해지지 않는다.
...
| 판단 기준 | SIEM | SOAR |
|:---|:---|:---|
| 핵심 특징 | 로그 정규화·상관분석으로 경보 생성 | 도구 연계·플레이북으로 대응 실행 |
| 적용 기준 | 이종 로그에서 공격 근거 탐지 | 반복 사건의 조사·조치 표준화 |
| 주요 위험 | 로그 품질 저하·오탐 경보 | 오탐 자동화·권한 집중·오조치 |
...
1. SIEM 계정 탈취 경보 후 SOAR 잠금 승인
2. 조치 실패 결과로 SIEM 경보 임계값 보정

### 쉽게 이해하기 (학습용)

- SIEM이 낯선 로그인과 권한 변경을 연결하면 SOAR가 자산 영향을 확인하고 승인 후 계정을 잠근다.
- SOAR가 정상 사용자를 잘못 차단한 기록을 SIEM에 돌려보내 경보 조건과 신뢰도 임계값을 조정한다.

## Ⅶ. 결론

- 경보 근거·조치 영향·결과 환류로 연계
