...
- 정의/개념: 분산 공격을 탐지•흡수•정화하는 **DDoS 가용성 방어 체계**이다.
- 배경/필요성: 내부 차단만으로는 **상위 회선•연결 상태 고갈 방지할 수 없다**.
...
- **병목 다양성**: 회선•연결 상태•응용 자원 소진이다.
- **다단 완화**: CDN•상위망•정화 센터에서 앞단 차단한다.
- **정상 요청 오탐**: 과도한 제한은 정상 사용자까지 차단한다.
...
- 회선 포화는 **상위 정화**, 상태 고갈은 **SYN 쿠키**, L7 소진은 **비용 제한**이 핵심이다.
