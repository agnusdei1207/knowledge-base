...
- **정의/개념**: 공격 트래픽을 탐지·차단·정화해 가용성 유지
- **배경/필요성**: 서비스 앞단 자원 고갈 전 상위망 대응 필요
...
- 분산 공격원은 출발지 차단을 어렵게 한다.
- 병목 자원이 탐지 지표·완화 위치를 바꾼다.
- 과도한 차단은 정상 사용자도 함께 거부한다.
- 회선 포화 뒤 내부 차단은 효과가 없다.
...
| 판단 기준 | 대역폭 공격 | 상태 소진 공격 | 응용 계층 공격 |
|:---|:---|:---|:---|
| 핵심 특징 | 증폭 트래픽으로 회선 포화 | 반개방 연결·세션 표 소진 | 정상 형식의 고비용 요청 반복 |
| 적용 기준 | 패킷량이 회선 용량에 접근 | 연결 수가 장비 한계에 접근 | 특정 기능의 처리 자원만 급증 |
| 주요 위험 | 내부 차단 전 회선 마비 | 정상 연결까지 신규 거부 | 공격과 정상 사용자 구분 곤란 |
...
1. DNS 증폭 트래픽을 상위 정화망으로 우회
2. 로그인 고비용 요청을 사용자별 속도 제한

### 쉽게 이해하기 (학습용)

- 큰 DNS 응답이 회선을 채우기 전에 통신사업자와 정화 센터로 경로를 돌려 공격 응답을 제거한다.
- 비밀번호 검증을 반복하는 요청은 주소만 보지 않고 사용자와 세션별 횟수를 제한해 서버 계산을 보호한다.
