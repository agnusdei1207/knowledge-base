---
sidebar:
  order: 72
  label: "072. QoS, DiffServ, IntServ"
  badge:
    text: "미출 • 50%"
    variant: note
title: "QoS, DiffServ, IntServ"
date: "2026-08-10T10:00:00+09:00"
tags:
  - "notes-network"
weight: 72
extra:
  question_no: "072"
  source_status: "미출"
  source_history: ""
  priority: 50
  priority_note: "비교•설계형: DiffServ•IntServ QoS 기반"
---

## Ⅰ. 개요

<details>
<summary>핵심 용어</summary>

- **서비스 품질(Quality of Service, QoS)**: 혼잡 시 트래픽을 분류•표시하고 큐•대역폭•폐기 정책을 차등 적용하는 네트워크 트래픽 품질 관리 체계이다.

</details>

- 정의/개념: **QoS**는 분류•표시•큐잉으로 네트워크 자원을 차등 배분하는 체계이다.
- 배경/필요성: 최선형 동일 처리는 혼잡 시 서비스별 품질 차이를 보장할 수 없다.

#### 한줄 요약

- 한정된 회선이 붐빌 때 중요한 패킷의 순서•대역폭•폐기 기준을 미리 정해 품질 차이를 만든다.

## Ⅱ. 특징

<details>
<summary>핵심 용어</summary>

- **정책 연속성**: 도메인 경계의 차등 서비스 코드 포인트 표시와 내부 홉별 동작 규칙을 정렬해 종단 경로에서 같은 서비스 등급을 유지하는 성질이다.
- **홉별 동작(Per-Hop Behavior, PHB)**: 표시된 등급에 따라 큐•대역폭•폐기를 차등 적용하는 동작이다.

</details>

- 큐잉•폐기 정책으로 중요 흐름의 손실과 지연을 줄인다.
- **정책 연속성**은 도메인별 표시와 **PHB**를 정렬해 종단 등급을 유지한다.
- 등급별 대역폭•버퍼•폐기 기준으로 한정된 자원을 배분한다.

#### 한줄 요약

- 혼잡하지 않을 때는 차이가 작지만 큐가 차면 어떤 패킷을 먼저 보내고 버릴지가 품질을 가른다.

## Ⅲ. 구조 및 구성요소

<details>
<summary>핵심 용어</summary>

- **차등 서비스 코드 포인트(Differentiated Services Code Point, DSCP)**: 패킷의 서비스 등급을 표시하는 IP 헤더 값이다.
- **서비스 수준 협약(Service Level Agreement, SLA)**: 품질 목표와 미달 책임을 정한 협약이다.

</details>

**DSCP**로 등급을 표시하고 **PHB**가 큐잉을 집행하며 **SLA**로 품질을 판정한다.

```text
분류•표시
└─ 수용•트래픽 제어
   └─ PHB•큐잉
      └─ 측정•SLA
```

선의 의미: 트래픽 분류•표시와 수용 제어가 PHB•큐잉의 처리 기준을 구성하고, 측정•SLA 계층이 전체 QoS 처리 결과를 판정하는 정적 정책 구조이다.

| 구성요소 | 책임 |
|:---|:---|
| 분류•표시 | 흐름 식별•DSCP 등급 설정 |
| 수용•트래픽 제어 | 자원 허용•속도•버스트 제한 |
| PHB•큐잉 | 등급별 버퍼•전송•폐기 처리 |
| 측정•SLA | 지연•지터•손실•대역폭 판정 |

#### 한줄 요약

- 등급 표시는 약속일 뿐 실제 처리는 PHB와 큐잉 자원이 결정하는 것이 핵심이다.

## Ⅳ. 흐름도

<details>
<summary>핵심 용어</summary>

- **분류•표시**: 트래픽을 식별하고 서비스 등급을 패킷 헤더에 기록하는 단계이다.
- **수용•트래픽 제어**: 자원 여유와 계약을 확인하고 속도•버스트•폐기 정책을 적용하는 단계이다.
- **등급 패킷 전달**: 경계에서 설정한 등급 표시를 보존해 코어 영역으로 보내는 단계이다.
- **PHB•큐잉 처리**: 등급별 큐 순서•대역폭•폐기 동작을 각 홉에서 실행하는 단계이다.
- **품질 측정값 전달**: 종단의 지연•지터•손실•대역폭 측정 결과를 서비스 수준 협약 판정에 제공하는 단계이다.

</details>

```text
패킷 전송 요청
      │
      ▼
1. 분류•표시
      │
      ▼
2. 수용•트래픽 제어
      ├─ 자원 부족: 제한•폐기
      └─ 수용
          │
          ▼
3. 등급 패킷 전달
          │
          ▼
4. PHB•큐잉 처리
          │
          ▼
5. 품질 측정값 전달
          │
          ▼
SLA 판정 결과 반환
```

### 동작 원리

1. **분류•표시**: **분류•표시** 단계에서 트래픽을 식별해 **DSCP** 등급을 설정한다.
2. **수용•트래픽 제어**: **수용•트래픽 제어** 단계에서 할당량과 속도 정책을 적용한다.
3. **등급 패킷 전달**: **등급 패킷 전달** 단계에서 표시를 보존해 코어 영역으로 전달한다.
4. **PHB•큐잉 처리**: **PHB•큐잉 처리** 단계에서 **PHB**의 순서•대역•폐기 정책을 실행한다.
5. **품질 측정값 전달**: **품질 측정값 전달** 단계에서 측정치를 **SLA** 판정에 제공한다.

#### 한줄 요약

- 중간 경로가 등급 표시를 지우면 이후 장비의 차등 처리가 끊긴다.

## Ⅴ. 종류 및 비교

<details>
<summary>핵심 용어</summary>

- **통합 서비스(Integrated Services, IntServ)**: 흐름별 종단 자원을 예약해 품질을 보장하는 방식이다.
- **차등 서비스(Differentiated Services, DiffServ)**: DSCP 등급과 PHB로 트래픽을 차등 처리하는 방식이다.
- **자원 예약 프로토콜(Resource Reservation Protocol, RSVP)**: IntServ의 흐름별 자원을 예약하는 신호 프로토콜이다.

</details>

| 판단 기준 | IntServ | DiffServ |
|:---|:---|:---|
| 적용 기준 | **IntServ**는 소수 흐름•명시적 예약 | **DiffServ**는 대규모 IP망•통계적 차등 |
| 핵심 특징 | 흐름별 **RSVP** 예약•상태 | 등급별 DSCP•PHB 처리 |
| 한계 | 라우터 상태•신호 부하 증가 | 종단 보장•도메인 정합 한계 |

> 요약: IntServ는 흐름 예약, DiffServ는 등급 처리다.

#### 한줄 요약

- 통합 서비스는 흐름별 예약, 차등 서비스는 서비스 등급 기반 차등 처리다.

## Ⅵ. 실무 고려사항 및 대책

<details>
<summary>핵심 용어</summary>

- **차등 서비스 코드 포인트 변경**: 도메인 경계에서 표시 정책이 달라 뒤쪽 장비가 원래 서비스 등급을 인식하지 못하는 현상이다.

</details>

| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 도메인 경계의 **차등 서비스 코드 포인트 변경** | 표시 보존•재표시 정책 합의 | 종단 차등 유지 |
| 우선 큐 과다 할당으로 일반 흐름 고갈 | 수용 제어•대역 상한 설정 | 일반 트래픽 보호 |
| 흐름별 상태 증가로 라우터 확장 제약 | **DiffServ** 등급 집계 적용 | 라우터 상태 절감 |

#### 한줄 요약

- 음성 패킷에 DSCP 우선 등급을 표시하고 경로상의 장비가 지연이 짧은 우선 큐로 일관되게 처리한다.

## Ⅶ. 결론

<details>
<summary>핵심 용어</summary>

- **서비스 품질 방식 선택**: 흐름 수와 명시적 자원 보장•라우터 상태 확장 요구로 통합 서비스와 차등 서비스를 결정하는 기준이다.

</details>

- **서비스 품질 방식 선택**에서 소수 흐름의 명시적 예약은 **IntServ**, 대규모 등급 차등은 **DiffServ**가 핵심이다.

#### 한줄 요약

- 소수 흐름의 명시적 예약은 통합 서비스, 많은 흐름의 등급 차등은 차등 서비스가 유리하다.
