...
1. 음성 패킷에 DSCP를 표시해 우선 큐에 넣는다
2. RSVP 자원이 있을 때만 제어 흐름을 개통한다

### 쉽게 이해하기 (학습용)

- 음성 패킷에 우선 등급을 붙이고 모든 경로 장비가 같은 우선 큐로 처리한다
- 필요한 대역을 끝까지 예약할 수 없으면 새 제어 흐름을 받지 않는다

## Ⅶ. 결론

- 흐름 규모·보장 수준으로 IntServ·DiffServ를 고른다
