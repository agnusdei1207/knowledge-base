...
- 정의/개념: 분류·표시·큐잉으로 자원을 차등 관리한다
- 배경/필요성: 혼잡 중 서비스별 품질 목표를 지킨다
...
block
    columns 1
    A["분류·표시 | 흐름 식별과 DSCP 등급 설정"]
    B["수용·트래픽 제어 | 자원 허용과 속도·버스트 제한"]
    C["PHB·큐잉 | 등급별 버퍼·전송·폐기 처리"]
    D["측정·SLA | 지연·지터·손실·대역폭 판정"]
    A --- B
    B --- C
    C --- D
```
...
sequenceDiagram
    ...
    송신흐름->>경계라우터: 1. 패킷 전달
    경계라우터->>경계라우터: 2. 분류·표시
    경계라우터->>경계라우터: 3. 수용·트래픽 제어
    경계라우터->>코어PHB: 4. 등급 패킷 전달
    코어PHB->>SLA측정: 5. PHB·큐잉 처리
```

**동작 원리**

1. **패킷 전달**: 송신 흐름이 경계 라우터에 진입
2. **분류·표시**: 트래픽을 식별해 DSCP 등급 설정
3. **수용·트래픽 제어**: 할당량과 속도 정책 적용
4. **등급 패킷 전달**: 표시를 보존해 코어 영역으로 전달
5. **PHB·큐잉 처리**: 등급별 순서·대역·폐기 정책 실행
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 도메인 경계의 DSCP 변경 | 표시 보존·재표시 정책 합의 | 종단 차등 유지 |
| 우선 큐 과다 할당 | 수용제어·대역 상한 설정 | 일반 트래픽 보호 |
| 흐름 상태 확장 한계 | 대규모망은 DiffServ 등급 집계 | 라우터 상태 절감 |
...
- **흐름 수·품질 목표·상태 비용에 따른 IntServ·DiffServ 선택**
