...
- 배경/필요성: 최선형 동일 처리는 혼잡 시 **서비스별 품질 보장할 수 없다**.
...
- **차등 처리**: 큐잉•폐기로 중요 흐름 손실 감소한다.
- **정책 연속성**: 도메인별 표시•**PHB** 정합으로 종단을 유지한다.
- **자원 배분**: 등급별 대역폭•버퍼•폐기 기준을 적용한다.
...
1. **분류•표시**: 트래픽을 식별해 **DSCP** 등급을 설정한다.
2. **수용•트래픽 제어**: 할당량과 속도 정책을 적용한다.
3. **등급 패킷 전달**: 표시를 보존해 코어 영역으로 전달한다.
4. **PHB•큐잉 처리**: 등급별 순서•대역•폐기 정책을 실행한다.
5. **품질 측정값 전달**: 지연•지터•손실•대역폭을 제공한다.
...
|:---|:---|:---|
| 적용 기준 | **소수 흐름•명시적 예약** | **대규모 IP망•통계적 차등** |
...
- 소수 흐름의 명시적 예약은 **IntServ**, 대규모 등급 차등은 **DiffServ**가 핵심이다.
