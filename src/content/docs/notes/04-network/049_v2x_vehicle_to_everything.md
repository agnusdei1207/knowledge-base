...
- 정의/개념: 차량이 **PC5•Uu** 로 주변 대상과 주행•위험 정보를 교환하는 **차량•사물 통신(Vehicle-to-Everything, V2X) 협력 체계**
...
- **공개키 기반구조(Public Key Infrastructure, PKI) 서명** 과 **메시지 개연성** 결합으로 위조•재전송•허위 위치 검증
...
- **차량 탑재 장치•노변 장치(OBU•RSU)**: 차량 탑재 장치는 안전 메시지를 생성•수신하고 노변 장치는 도로 주변에서 차량과 서버 사이 정보를 교환한다.
- **공개키 기반구조(PKI)**: 인증서와 공개키로 메시지 송신자와 전자서명을 검증하는 신뢰 체계이다.
...
3. **인증서 상태**: **공개키 기반구조(Public Key Infrastructure, PKI)** 가 공개키와 인증서 상태 제공
...
| 차량•사물 통신(Vehicle-to-Everything, V2X) 경로 | PC5 직접 통신 | Uu 망 경유 통신 |
|:---|:---|:---|
