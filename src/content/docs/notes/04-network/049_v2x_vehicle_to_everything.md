...
date: "2026-07-25T19:49:00+09:00"
tags:
  - "notes-network"
weight: 49
extra:
...
- **정의/개념**: 차량이 주변 대상과 주행 정보를 교환하는 체계
- **배경/필요성**: 센서 시야 밖 위험의 조기 공유
...
- PC5는 기지국 없이 근거리 위험을 직접 전달한다.
- Uu는 서버를 거쳐 광역 교통 정보를 전달한다.
- 위치·속도·생성 시각으로 위험의 시공간을 판단한다.
- 서명만으로 센서 오류와 허위 내용은 판별하지 못한다.
...
| 판단 기준 | PC5 직접 통신 | Uu 망 경유 통신 |
|:---|:---|:---|
| 핵심 특징 | 주변 단말·RSU와 직접 교환 | 기지국·서버를 거쳐 교환 |
| 적용 기준 | 근거리 즉시 위험 전파 | 광역 교통·콘텐츠 배포 |
| 주요 위험 | 채널 혼잡·도달 범위 제한 | 망 왕복 지연·음영 의존 |
...
2. 광역 도로 통제는 V2X 서버가 Uu로 대상 차량에 배포

### 쉽게 이해하기 (학습용)

- 앞 차량의 급제동 메시지가 지금 위치와 맞는지 확인한 뒤 운전자에게 경고한다
- 먼 구간의 도로 폐쇄 정보는 서버가 그 길로 가는 차량에만 전달한다
