...
- **V2X**: 차량이 PC5·Uu를 이용해 주변 차량·인프라·보행자·통신망과 주행·위험 정보를 교환하는 협력 통신 체계이다.
...
- **PC5·Uu**: PC5는 차량 간 직접 통신에, Uu는 기지국과 코어망을 경유한 광역 통신에 쓰는 3GPP 참조점이다.
...
- **OBU·RSU**: OBU는 차량에서 안전 메시지를 생성·수신하고 RSU는 도로 주변에서 차량과 서버 사이 정보를 교환한다.
- **PKI**: 인증서와 공개키로 메시지 송신자와 전자서명을 검증하는 신뢰 체계이다.
