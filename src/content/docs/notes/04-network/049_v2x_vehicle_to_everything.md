...
- 배경/필요성: 차량 센서의 **가시거리·차폐 범위 밖 위험 미탐지**
...
block
    columns 1
    A["차량 OBU | 센서 상태를 서명 메시지로 생성·수신"]
    B["RSU | 차량과 도로 설비·서버 사이 정보 교환"]
    C["이동통신망 | Uu를 통한 광역 서버 경로 제공"]
    D["V2X 서버 | 광역 교통 정보를 집계·배포"]
    E["PKI | 가명 인증서 발급·검증·폐기"]
    A --- B
    B --- C
    C --- D
    D --- E
...
sequenceDiagram
    ...
    participant PKI
    participant 차량제어
    차량OBU->>주변차량: 1. 위험 메시지 생성
    차량OBU->>주변차량: 2. PC5 메시지 전송
    차량OBU->>PKI: 3. 인증서 상태 확인
    PKI->>차량OBU: 4. 검증 정보 반환
    차량OBU->>차량제어: 5. 최신성·위치 검증
```

**동작 원리**

1. **위험 메시지 생성**: 위치·속도·시각을 넣고 전자서명
2. **PC5 메시지 전송**: 주변 차량에 안전 정보를 직접 방송
3. **인증서 상태 확인**: 가명 인증서의 유효·폐기 상태 조회
4. **검증 정보 반환**: 공개키와 인증서 검증 결과 제공
5. **최신성·위치 검증**: 재전송과 공간상 불가능한 값을 식별
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 유효 서명의 거짓 위치 | 시각·위치·센서 관측 대조 | 오경보 감소 |
| 인증서 추적의 사생활 침해 | 가명 인증서 회전·최소 로그 | 차량 익명성 보호 |
| PC5 채널 혼잡 | 메시지 우선순위·전송률 제어 | 안전 정보 기한 보장 |
