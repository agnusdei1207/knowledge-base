---
sidebar:
  order: 68
  label: "068. 맨체스터 인코딩 (Manchester Encoding)"
  badge:
    text: "기출 · 30%"
    variant: note
title: "맨체스터 인코딩 (Manchester Encoding)"
date: "2026-07-25T12:18:00+09:00"
tags:
  - "notes-network"
weight: 68
extra:
  question_no: "068"
  source_status: "기출"
  source_history: "132회"
  priority: 30
  priority_note: "설명형: 132회 Manchester Coding 직접 출제"
---

## 미리 알고가기

- **맨체스터 인코딩(Manchester Encoding)**: 각 비트 중앙의 전압 천이 방향으로 데이터와 클럭을 함께 표현하는 회선 부호
- **비복귀 영점(Non-Return-to-Zero, NRZ)**: 비트 구간 동안 일정한 전압 레벨로 값을 표현하는 회선 부호
- **위상 고정 루프(Phase-Locked Loop, PLL)**: 반복되는 신호 천이에 내부 발진기의 위상·주파수를 맞춰 클럭을 복구하는 회로
- **직류 성분(Direct Current Component, DC Component)**: 신호 평균값에 해당하는 0Hz 주파수 성분
- **비트 오류율(Bit Error Rate, BER)**: 전체 수신 비트 중 잘못 판정된 비트의 비율
- **차분 맨체스터(Differential Manchester)**: 비트 중앙에는 항상 천이를 두고 비트 경계의 천이 유무로 데이터를 표현하는 회선 부호
- **보드율(Baud Rate)**: 초당 전송하는 신호 심벌 수
- **지터(Jitter)**: 신호 천이 시점이 이상적인 시점에서 앞뒤로 흔들리는 현상
- **회선 부호 약어 읽기와 표기**: NRZ는 엔알지로 읽고 Non-Return-to-Zero의 머리글자를 딴 표기이며, 비트 구간에서 영점으로 복귀하지 않는 전압 표현 역할을 함
- **회로·신호 약어 읽기와 표기**: PLL·DC·BER는 피엘엘·디씨·비이알로 읽고 영문 머리글자를 딴 표기이며, 각각 클럭 동기·직류 성분·비트 오류 비율을 나타냄
- **영헤르츠 기호(0 Hz·제로 헤르츠)**: 0은 주파수 변화가 없음을, Hz는 헤르츠 단위를 나타내며 신호 평균값인 직류 성분의 위치를 뜻함

## Ⅰ. 개요

- 정의: 비트 중앙 천이로 데이터·클럭을 표현한다
- 배경: 별도 클럭선 없이 수신 시점을 복구한다

### 쉽게 이해하기 (학습용)

- 각 비트 중앙에서 신호가 뒤집혀 데이터 박자를 알려줌

## Ⅱ. 특징

- 중앙 천이는 수신 클럭을 복구해 동기 손실을 막는다
- 반대 레벨은 DC 성분을 줄여 변압기 결합을 돕는다
- 비트당 천이는 NRZ보다 대역폭 비용을 높인다
- 천이 규칙 불일치는 모든 비트의 역해석을 만든다

### 쉽게 이해하기 (학습용)

- 중앙 천이는 항상 클럭용이고 비트 경계의 추가 천이는 앞뒤 데이터 조합에 따라 나타날 수 있음

## Ⅲ. 아키텍처 및 구성요소

```mermaid
flowchart LR
    DATA[데이터 비트]
    subgraph M[맨체스터 회선 경계]
        CLK[비트 클럭]
        ENC[맨체스터 부호기]
        MED[전송 매체]
        DET[천이 검출기]
        DEC[PLL·복호기]
        CLK -->|비트 중앙 시점| ENC
        ENC -->|중앙 천이 파형| MED
        MED -->|감쇠·잡음 포함 파형| DET
        DET -->|천이 시점·방향| DEC
    end
    DATA --> ENC
    DEC -->|복원 비트| OUT[수신 처리]
```

| 설계 요소 | 설명 |
|:---|:---|
| 비트 클럭 | 비트 경계와 중앙 천이 시점 제공 |
| 맨체스터 부호기 | 비트와 클럭을 중앙 천이 파형으로 변환 |
| 전송 매체 | 파형에 감쇠·잡음·지터 추가 |
| 천이 검출기 | 전압 천이의 시점·방향 검출 |
| PLL·복호기 | 중앙 천이로 클럭과 데이터 복원 |

> 요약: 중앙 천이에서 클럭과 비트값을 함께 복원한다

### 쉽게 이해하기 (학습용)

- PLL은 반복되는 중앙 천이에 맞춰 비트 판정 시점 보정

## Ⅳ. 원리 및 절차 흐름도

```mermaid
sequenceDiagram
    participant S as 데이터 처리
    participant E as 맨체스터 부호기
    participant D as 천이 검출기
    participant P as PLL·복호기
    S->>E: 비트·규칙 전달
    E->>D: 중앙 천이 파형 전송
    D->>P: 천이 시점 전달
    P->>P: 수신 클럭 복구
    D->>P: 천이 방향 전달
    P->>S: 비트값 복원
```

| 절차 | 설명 |
|:---|:---|
| 비트·규칙 전달 | 0·1과 표준 천이 방향을 입력 |
| 중앙 천이 파형 전송 | 반 비트마다 반대 레벨을 생성 |
| 천이 시점 전달 | 수신 파형의 전압 반전 시점 검출 |
| 수신 클럭 복구 | 반복 중앙 천이에 PLL 위상 동기 |
| 천이 방향 전달 | 중앙의 상승·하강 방향 제공 |
| 비트값 복원 | 합의한 규칙으로 0·1 판정 |

> 요약: 중앙 천이로 데이터와 클럭을 복원한다

### 쉽게 이해하기 (학습용)

- 천이 규칙이 다르면 0과 1을 반대로 해석할 수 있음

## Ⅴ. 종류 및 비교

| 판단 기준 | 맨체스터 | NRZ | 차분 맨체스터 |
|:---|:---|:---|:---|
| 핵심 특징 | 중앙 천이 방향으로 비트 표현 | 비트 구간 전압 레벨 유지 | 경계 천이 유무로 비트 표현 |
| 적용 기준 | 자체 동기·극성 일치 회선 | 대역 효율 우선·별도 동기 | 극성 반전 가능 회선 |
| 주요 위험 | NRZ 대비 대역폭 증가 | 긴 동일 비트열의 동기 손실 | 천이 수 증가·규칙 복잡성 |

> 요약: 맨체스터는 대역폭을 써서 동기 성능을 얻는다

### 쉽게 이해하기 (학습용)

- 차분 방식은 경계 천이를 보므로 극성 반전에 강함

## Ⅵ. 실무 사례

1. 10 Mb/s 링크의 송수신 천이 규칙을 맞춘다
2. 중앙 천이 지터와 PLL 고정을 시험한다

### 쉽게 이해하기 (학습용)

- 송신의 상승·하강 의미와 수신 규칙이 다르면 모든 0·1을 반대로 해석한다
- 케이블에서 천이 시점이 흔들려도 PLL이 비트 중앙을 계속 따라가는지 확인한다

## Ⅶ. 결론

- 대역 여유·극성·클럭 복구로 맨체스터를 선택한다

### 쉽게 이해하기 (학습용)

- 대역폭을 더 쓰는 대신 한 선로에서 데이터와 클럭을 함께 전달한다
