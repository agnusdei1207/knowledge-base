...
- **로그**: 시스템에서 발생한 접속, 변경, 오류와 통신 행위를 시간•주체•대상과 함께 남긴 기록한다.
- **보안 정보•이벤트 관리(Security Information and Event Management, SIEM)**: 이종 로그를 정규화•상관분석해 조사 가능한 경보를 만드는 관제 체계다.

</details>

- 정의/개념: 이종 로그를 정규화•상관분석하는 **SIEM 통합 보안 관제 체계**이다.
- 배경/필요성: 개별 로그만으로는 **공격 흐름•영향 범위 추적하기 어렵다**.
...
- **이종 로그 정규화**: 공통 필드와 값으로 변환한다.
- **이벤트 맥락 보강**: 자산•신원•위협 정보 결합한다.
- **다단계 공격 상관분석**: 시간•사용자•자산을 연결한다.
...
| 정규화•보강기 | **공통 필드•자산•신원 맥락** 결합 |
| 보안 자료 저장소 | **원본•정규 로그 검색•감사** 보존 |
| 상관분석 엔진 | **공격 흐름•위험 점수** 계산 |
...
1. **원본 로그 전달**: 수집 대상의 접속•변경•통신 기록을 제공한다.
2. **정규•보강 이벤트 전달**: 공통 구조와 자산•신원 맥락을 결합한다.
3. **근거 경보 전달**: 흐름 상관•위험 점수로 우선순위화한다.
4. **조사 판정 기록**: 원본 시간선으로 오탐•영향을 확인한다.
5. **개선 탐지 규칙 전달**: 판정을 조건•임계값에 반영한다.
...
- 알려진 절차는 **규칙**, 미지 이상은 **행위**, 악성 흔적은 **위협 정보**를 분석한다.
