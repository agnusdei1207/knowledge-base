...
- **보안 정보·이벤트 관리(Security Information and Event Management, SIEM)**: 여러 시스템의 로그를 공통 형식으로 모아 연관 분석하고 조사 가능한 경보를 만드는 보안 관제 플랫폼이다.
...
- **정의/개념**: 이종 로그를 정규화·상관분석해 경보를 만드는 관제 플랫폼
- **배경/필요성**: 파편 로그만으로는 공격 경로·영향 추적 곤란
...
- 이종 로그를 공통 필드로 연결해 통합 탐지한다.
- 자산·신원 맥락이 경보 위험도를 구체화한다.
- 과거 증거 검색과 실시간 탐지를 함께 지원한다.
- 로그 누락·시각 불일치는 상관분석을 단절한다.
...
| 판단 기준 | 규칙 기반 분석 | 행위 기반 분석 | 위협 정보 대조 |
|:---|:---|:---|:---|
| 핵심 특징 | 명시 조건·시간창으로 공격 연결 | 정상 기준과 다른 행위를 탐지 | 알려진 악성 주소·파일과 비교 |
| 적용 기준 | 공격 절차와 필수 로그가 명확 | 알려지지 않은 계정·단말 이상 | 최신 외부 위협 흔적 확인 |
| 주요 위험 | 변형 공격 미탐·규칙 폭증 | 기준 오염·설명 어려움 | 오래된 정보·단순 일치 오탐 |
