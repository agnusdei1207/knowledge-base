...
sequenceDiagram
    ...
    participant 규칙저장소
    로그원천->>수집기: 1. 로그 품질 확인
    수집기->>분석엔진: 2. 정규 이벤트 보강
    분석엔진->>분석가: 3. 근거 경보 생성
    분석가->>규칙저장소: 4. 조사 결과 판정
    규칙저장소->>분석엔진: 5. 탐지 규칙 개선
```

| 단계 | 동작 원리 |
|:---|:---|
| **1. 로그 품질 확인** | 누락·중복·지연·시각 오류 검사 |
| **2. 정규 이벤트 보강** | 공통 구조와 자산·신원 맥락 결합 |
| **3. 근거 경보 생성** | 흐름 상관·위험 점수로 우선순위화 |
| **4. 조사 결과 판정** | 원본 시간선으로 오탐·영향 확인 |
| **5. 탐지 규칙 개선** | 판정 결과를 조건·임계값에 반영 |
