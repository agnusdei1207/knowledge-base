...
- 정의/개념: 이더넷 프레임을 **UDP/IP로 캡슐화한 L2 오버레이**이다.
- 배경/필요성: VLAN 12비트 식별자와 광역 L2 확장으로 **테넌트 규모•장애 격리** 제약이 발생한다.
...
- **24비트 VNI 기반 대규모 테넌트 논리망 분리**가 핵심이다.
- **VTEP UDP•IP 캡슐화 기반 L3 언더레이•ECMP 활용**이 핵심이다.
- **EVPN 위치 배포에 따른 BUM 감소와 캡슐화 MTU 요구**가 핵심이다.
...
1. **MAC•IP 위치 경로**: 수신 VTEP가 종단 호스트의 위치를 EVPN에 광고한다.
2. **원격 VTEP 경로**: EVPN이 목적 MAC과 원격 VTEP 대응 정보를 배포한다.
3. **VXLAN 패킷**: 송신 VTEP가 VNI•UDP•IP 헤더를 붙여 원격 VTEP로 전달한다.
...
- 대규모 테넌트•L3 확장은 **EVPN VXLAN**, 소규모 단일 구간은 **VLAN**을 선택한다.
