...
- 배경/필요성: 12비트 VLAN 한계(4,094개) 및 L2 STP 링크 차단으로 인한 **멀티 테넌트 수용 불가, 대역폭 유휴화 및 L3 경계를 넘는 VM 무중단 마이그레이션 실패**

#### 한줄 요약
- 24비트 VNI 확장성, MAC-in-UDP 캡슐화, L3 ECMP 대역폭 100% 활용을 지원한다.
...
#### 한줄 요약
- VTEP 터널 종단점, 24비트 VNI 식별자, L3 언더레이 패브릭, MP-BGP EVPN 제어 평면이 결합된다.
...
#### 한줄 요약
- L2 프레임 인입 → EVPN VTEP 조회 → 50B 캡슐화 → L3 ECMP 수송 → 수신 VTEP 역캡슐화 순으로 동작한다.
