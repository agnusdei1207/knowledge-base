...
- 정의/개념: 목표 망 상태를 모델·코드로 배포하는 체계다
- 배경/필요성: 수동 명령의 편차·감사 누락을 제거한다
...
block
    columns 1
    A["진실의 원천 | 주소·토폴로지·목표 정책의 정본 저장"]
    B["Git·CI 파이프라인 | 변경 리뷰·검사·승인·감사 기록"]
    C["Ansible | 인벤토리·배포 범위·의존성 조율"]
    D["YANG·관리 API | 모델 제약에 맞춰 설정 조회·변경"]
    E["네트워크 장비 | 승인된 설정 적용과 운용 상태 제공"]
    A --- B
    B --- C
    C --- D
    D --- E
...
sequenceDiagram
    ...
    participant 네트워크장비
    participant 검증체계
    운영자->>Git·CI: 1. 목표 상태 변경
    Git·CI->>Git·CI: 2. 사전 검증
    Git·CI->>자동화실행기: 3. 승인 변경 전달
    자동화실행기->>네트워크장비: 4. 설정 차이 배포
    네트워크장비->>검증체계: 5. 실제 상태 보고
```

**동작 원리**

1. **목표 상태 변경**: 모델·템플릿으로 원하는 상태 선언
2. **사전 검증**: 구문·YANG·정책·도달성 검사
3. **승인 변경 전달**: 리뷰를 통과한 작업만 실행기에 전달
4. **설정 차이 배포**: 현재 상태와 다른 설정만 단계 적용
5. **실제 상태 보고**: 장비가 적용 설정·운용 상태 제공
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 정본·실제 상태 드리프트 | 배포 전 차이 탐지·승인 | 오래된 설정 덮어쓰기 방지 |
| 일괄 변경 장애 확산 | 소수 장비 선행·단계 배포 | 실패 범위 제한 |
| CLI 출력 변경 | YANG·구조화 API 우선 | 파싱 오류 감소 |
...
- **정본·사전 검증·단계 배포·복구를 갖춘 네트워크 자동화**
