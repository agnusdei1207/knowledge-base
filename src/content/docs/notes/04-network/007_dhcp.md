...
date: "2026-08-26T13:42:26+09:00"
tags:
  - "notes-network"
weight: 7
extra:
...
- 배경/필요성: 수동 고정 IP 설정 시 발생하는 **관리 공수 폭증, IP 중복 충돌, 잦은 이동에 따른 재설정 지연 및 유휴 주소 회수 불가**를 겪으므로, 서버가 주소 풀을 단독 소유하고 임대 기간만큼만 빌려주는 중앙 할당 계층을 두어 단말별 수동 설정과 회수 공수를 임대 만료 처리로 대체할 필요
...
- 다중 서브넷은 **DHCP 릴레이**, 비인가 응답은 Snooping 차단
