...
```text
[ DHCP Client (UDP 68) ]                                [ DHCP Server (UDP 67) ]
           |                                                       |
           | ----- 1. Discover (L2 Broadcast / L3 Broadcast) ----> | (IP 요청)
           | <---- 2. Offer (L2 Unicast/Broadcast, IP 제안) ------- | (IP 후보제안)
           | ----- 3. Request (L2 Broadcast, IP 채택 및 요청) ----> | (선택 통보)
           | <---- 4. ACK (L2 Unicast/Broadcast, 임대 확정) -------- | (임대 완료)
           |                                                       |
```
...
[ 클라이언트 부팅 ]
        |
        v
[ 1. DHCP Discover ] ---> L2 (FF:FF:FF:FF:FF:FF) / L3 (255.255.255.255) 전송
        |
        v
[ 2. DHCP Offer ] ------> 서버가 클라이언트 MAC 기반으로 IP 및 Lease Time 제안
        |
        v
[ 3. DHCP Request ] ----> 클라이언트가 제안받은 IP 선택 승인 요청 (타 서버에 거절 통보 겸함)
        |
        v
[ 4. DHCP ACK ] --------> 서버가 최종 바인딩 등록 및 설정 완료 통보 (T1=50%, T2=87.5% 타이머 가동)
```
