...
- **DHCP**는 서버가 단말에 IP 주소와 네트워크 설정을 일정 기간 자동으로 임대하는 프로토콜이다.
...
- **주소 풀**은 DHCP 서버가 단말에 임대할 수 있도록 보유한 주소 범위이다.
- **바인딩**은 임대한 주소를 단말 식별자와 만료 시각에 연결하여 저장한 기록이다.
- **DHCP 릴레이**는 다른 서브넷의 단말이 보낸 DHCP 요청과 서버 응답을 중앙 서버까지 중계한다.
...
- **DHCP 클라이언트**는 주소와 옵션을 탐색하고 서버에 임대를 요청한다.
- **DHCP 옵션**은 IP 주소와 함께 프리픽스·기본 게이트웨이·DNS 서버 등의 네트워크 설정을 전달한다.
...
- **DORA**는 DHCPDISCOVER로 서버를 찾고 DHCPOFFER를 받은 뒤 DHCPREQUEST를 보내 DHCPACK으로 임대를 확정하는 절차이다.
- **브로드캐스트**는 주소가 없는 초기 단말이 같은 링크의 모든 DHCP 서버 후보에 탐색 요청을 보내게 한다.
...
- **동적 임대**는 접속과 이탈이 잦은 단말에 주소를 일정 기간 배정하고 회수하여 주소 풀을 재사용한다.
- **수동 설정**은 관리자가 장치에 주소와 네트워크 옵션을 직접 고정하여 입력한다.
...
- **DHCP 스누핑**은 스위치가 신뢰 포트의 서버 응답만 허용하고 주소·단말·포트 바인딩을 기록한다.
- **Option 82**는 릴레이가 단말의 접속 회선과 장치 정보를 DHCP 서버에 전달하는 옵션이다.
- **T1과 T2**는 단말이 각각 기존 서버에 임대를 갱신하고 모든 서버에 재결합을 요청하는 시점이다.
...
- **임대 시간**은 단말이 갱신 없이 주소를 사용할 수 있는 기간이며 주소 회수 속도와 갱신 트래픽을 함께 좌우한다.
