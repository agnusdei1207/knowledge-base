...
- 배경/필요성: 홉 단위(Hop-by-Hop) IP 라우팅은 라우터마다 가변 길이 IP 헤더를 다시 열어 **최장 프리픽스 검색을 홉 수만큼 반복 지불**하고도 경로를 명시적으로 지정할 수단이 없었으므로, L2와 L3 사이에 고정 길이 레이블 계층을 삽입해 홉마다 반복되던 최장 프리픽스 검색을 진입 라우터 1회로 접고 이후 홉은 레이블 교환만 수행하게 할 필요
