...
- 정의/개념: **MPLS** — 입구에서 패킷을 FEC로 분류해 레이블을 붙이고 중계 구간에서 레이블을 교환하며 LSP로 전달하는 **패킷 전달 기술**이다.
- 배경/필요성: 목적지별 IP 경로만으로는 **서비스 격리•명시 경로 표현 제약**이 발생한다.
...
- 입구 LER의 **FEC 분류•레이블 부착**이 핵심이다.
- 중간 LSR의 **레이블 교환 전달**이 핵심이다.
- 레이블 스택의 **전송•서비스 문맥 표현**이 핵심이다.
...
- 고객 격리•명시 경로가 필요하면 **레이블 스택 구성** 후 **경로 MTU 이내**로 제한한다.
