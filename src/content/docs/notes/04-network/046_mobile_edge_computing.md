...
- 정의/개념: 접속망 인접 에지에 **연산•저장•망 정보** 기능을 제공하는 **분산 컴퓨팅 구조**이다.
- 배경/필요성: 중앙 클라우드 왕복과 **백홀 집중에 따른 실시간 제어 응답 기한 초과**가 발생한다.
...
- 로컬 **UPF** 분기로 중앙망을 거치지 않고 에지에 직접을 전달한다.
- **플랫폼 API 기반 위치•무선 상태 제공**이 핵심이다.
- 지연•자원•위치 조건에 따른 **오케스트레이터의 응용 배치**가 핵심이다.
...
1. **응용 배치 명세**: 지연•자원 조건을 만족하는 **MEC 호스트**를 지정한다.
2. **기동 명령**: 호스트가 MEC 응용의 실행 환경을 생성한다.
3. **조향 정책 요청**: 응용 주소와 대상 **사용자 흐름**을 전달한다.
4. **PDU 세션 규칙**: 로컬 UPF를 데이터 경로로 지정하는 **5GC을 제어**한다.
5. **로컬 사용자 데이터**: UPF에서 MEC 응용으로 **로컬을 브레이크아웃**한다.
...
- 종단 지연•데이터 잔류가 우선이면 **MEC**, 대규모 집약 처리는 **중앙 클라우드**를 선택한다.
