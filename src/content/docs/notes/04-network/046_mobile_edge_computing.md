...
date: "2026-07-25T19:46:00+09:00"
tags:
  - "notes-network"
weight: 46
extra:
...
- **정의/개념**: 접속망 가까이에 연산·플랫폼을 배치하는 구조
- **배경/필요성**: 중앙 왕복 지연과 백홀 트래픽 감소
...
- 로컬 UPF가 MEC 응용으로 트래픽을 분기한다.
- 플랫폼 API가 위치·무선 상태를 응용에 제공한다.
- 에지 전처리로 중앙 전송 데이터량을 줄인다.
- 단말 이동은 경로와 응용 상태 재배치를 유발한다.
...
| 판단 기준 | MEC | 중앙 클라우드 |
|:---|:---|:---|
| 핵심 특징 | 접속망 인접 분산 처리 | 중앙 집중 자원 공유 |
| 적용 기준 | 저지연·지역 망 정보 필요 | 대규모 학습·장기 저장 |
| 주요 위험 | 노드 분산 운영·상태 이전 | 왕복 지연·백홀 집중 |
...
2. 차량 관제는 이동 경로의 다음 MEC로 세션·상태 이전

### 쉽게 이해하기 (학습용)

- 카메라 영상을 공장 밖에 보내지 않고 현장 서버가 불량을 판정한다
- 차량이 지역을 벗어나기 전에 다음 에지 서버로 연결과 작업 상태를 옮긴다

## Ⅶ. 결론

- 지연 예산·이동 범위로 MEC 위치·상태 이전 결정
