...
- **다중접속 에지 컴퓨팅(Multi-access Edge Computing, MEC)**: 접속망과 가까운 에지에 연산•저장•망 정보 기능을 배치하는 분산 컴퓨팅 구조

</details>

- 정의/개념: 접속망 인접 에지에 **연산•저장•망 정보** 기능을 제공하는 **분산 컴퓨팅 구조**
- 배경/필요성: 중앙 클라우드 왕복과 **백홀 집중에 따른 실시간 제어 응답 기한 초과**
...
- **다중접속 에지 컴퓨팅 오케스트레이터(Multi-access Edge Computing Orchestrator, MEC Orchestrator)**: MEC 응용의 배치와 수명주기를 지연•자원•위치 조건에 따라 관리하는 기능
- **사용자면 기능(User Plane Function, UPF)**: 사용자 트래픽을 에지 데이터망으로 분기하는 코어망 기능
- **응용 프로그래밍 인터페이스(Application Programming Interface, API)**: MEC 플랫폼이 응용에 위치•무선 상태•트래픽 제어 기능을 제공하는 호출 규격

</details>

- 로컬 **UPF** 분기로 중앙망을 거치지 않고 에지에 직접 전달
- **플랫폼 API 기반 위치•무선 상태 제공**
- 지연•자원•위치 조건에 따른 **오케스트레이터의 응용 배치**
...
4. **PDU 세션 규칙**: 로컬 UPF를 데이터 경로로 지정하는 **5GC 제어**
