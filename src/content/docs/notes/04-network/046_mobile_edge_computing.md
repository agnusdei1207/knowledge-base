...
block
    columns 1
    A["로컬 UPF | 정책에 따라 사용자 흐름을 MEC로 분기"]
    B["MEC 호스트 | 에지의 연산·저장·가상화 자원 제공"]
    C["MEC 플랫폼 | 서비스 발견·망 정보·트래픽 API 제공"]
    D["MEC 응용 | 현장 데이터의 추론·제어·캐시 수행"]
    E["MEC 오케스트레이터 | 위치·자원에 맞춰 응용 배치·회수"]
    A --- B
    B --- C
    C --- D
    D --- E
...
sequenceDiagram
    ...
    participant 5GC
    participant 로컬UPF
    오케스트레이터->>MEC호스트: 1. 응용 배치
    MEC호스트->>MEC응용: 2. 응용 기동
    MEC응용->>5GC: 3. 조향 정책 요청
    5GC->>로컬UPF: 4. PDU 경로 설정
    로컬UPF->>MEC응용: 5. 로컬 트래픽 전달
```

**동작 원리**

1. **응용 배치**: 지연·자원을 만족하는 호스트 선택
2. **응용 기동**: 호스트가 응용 실행 환경을 생성
3. **조향 정책 요청**: 응용 주소와 대상 흐름을 5GC에 전달
4. **PDU 경로 설정**: 5GC가 로컬 UPF 경로를 지정
5. **로컬 트래픽 전달**: UPF가 사용자 데이터를 응용에 전달
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 서버는 가깝지만 중앙 경유 | 로컬 UPF·트래픽 조향 검증 | 종단 지연 감소 |
| 사용자 이동 중 상태 단절 | 상태 이전·앵커 변경 기준 | 서비스 연속성 |
| 분산 노드 취약점 방치 | 서명 배포·격리·원격 증명 | 에지 보안 강화 |
