...
- 중앙 클라우드와 현장 엣지의 장점을 결합하여 초저지연 실시간 제어와 지능형 엣지 AI(Edge AI) 추론을 가능케 하는 5G/6G 및 인텔리전트 컴퓨팅 컨티뉴엄(Continuum)의 핵심 분산 인프라 표준으로 자리잡았으며, 실무 구축 시에는 단말 이동 시 서비스 단절을 방지하는 상태 컨텍스트 재배치(Context Relocation), 보안 취약 엣지 노드를 보호하는 TPM 하드웨어 원격 무결성 증명(Remote Attestation), 무선망 상황에 따른 비디오 품질 적응 제어를 위한 RNIS(Radio Network Information Service) 연동, K8s 기반 중앙 집중형 엣지 오케스트레이션을 결합하여 고가용성 분산 엣지 환경을 완성
