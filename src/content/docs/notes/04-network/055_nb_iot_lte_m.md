...
block
    columns 1
    A["IoT 단말·USIM | 센서 데이터·가입자 인증"]
    B["기지국·무선망 | 자원·반복 전송·접속 제어"]
    C["코어망 | 등록·이동성·절전 타이머 관리"]
    D["IoT 플랫폼 | 단말·회선·업무 데이터 연동"]
    A --- B
    B --- C
    C --- D
```
...
sequenceDiagram
    ...
    무선망->>코어망: 2. 가입자 등록·인증
    IoT단말->>코어망: 3. 절전 타이머 승인
    코어망->>IoT플랫폼: 4. 상향 센서 데이터
    IoT단말->>IoT단말: 5. PSM 진입
```

**동작 원리**

1. **무선 접속 요청**: NB-IoT·LTE-M 자원 요청
2. **가입자 등록·인증**: USIM 인증 후 등록 상태 생성
3. **절전 타이머 승인**: PSM·eDRX·도달 시간 협상
4. **상향 센서 데이터**: 코어망을 거쳐 플랫폼에 전달
5. **PSM 진입**: 등록을 유지하며 무선 회로를 끔
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 절전 주기와 하향 기한 충돌 | PSM·eDRX·수신창 공동 산정 | 배터리·응답 균형 |
| 이동 단말의 NB-IoT 적용 | 이동성 요구 시 LTE-M 선택 | 세션 연속성 |
| 깊은 실내 접속 불안 | 반복 횟수·전력·커버리지 실측 | 배터리 수명 예측 |
...
- **이동성·응답 기한·커버리지에 따른 NB-IoT·LTE-M 선택**
