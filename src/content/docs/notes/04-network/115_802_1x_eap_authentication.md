...
- **근거리망 기반 EAP(EAP over LAN, EAPOL)**: 단말과 접속 장비 사이에서 EAP 인증 메시지를 운반하는 캡슐화 방식이다.
...
- **1. RADIUS EAP 대화 중계**: 중계자가 단말의 EAP 요청·응답을 인증 서버에 전달하는 단계이다.
- **2. 인증서·신뢰 체인 검증**: 단말·서버 인증서의 서명·유효기간·폐기 상태를 확인하는 단계이다.
- **3. 권한 정책 전달**: 인증 서버가 승인 여부와 VLAN·ACL 정책을 중계자에 보내는 단계이다.
- **4. 통제 포트 정책 적용**: 접속 장비가 승인된 망 구역을 개방하고 재인증 조건을 설정하는 단계이다.
