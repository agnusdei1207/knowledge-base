...
- **802.1X EAP 인증**: EAP로 단말을 인증한 뒤 포트 접근 권한을 부여하는 방식
...
- **역할 분리**: 단말·중계 장비·서버가 협력한다.
- **이중 전달**: 단말은 EAPOL, 서버는 RADIUS를 쓴다.
- **포트 통제**: 인증 전에는 인증 통신만 허용한다.
- **동적 권한**: 결과에 따라 망 구역과 규칙을 적용한다.
...
|:---|:---|
| 인증 요청자 | 자격과 인증 응답 제시 |
| 인증 중계자 | 인증 중계와 포트 상태 집행 |
| 인증 서버 | 자격 검증과 권한 결정 |
| 신원·인증서 저장소 | 계정과 신뢰 정보 제공 |
...
| 비교 기준 | EAP-TLS | 터널형 EAP | 주소 기반 우회 |
|:---|:---|:---|:---|
| 핵심 특징 | 양쪽 인증서로 상호 인증 | 보호 터널 안에서 계정 인증 | 단말 주소만 확인 |
| 적용 기준 | 관리 단말·강한 보안 | 계정 중심 기존 환경 | 인증 미지원 장비 |
| 주요 위험 | 인증서 운영 실패 | 가짜 서버·계정 탈취 | 주소 위조·낮은 신뢰 |
...
1. **가짜 서버 차단**: 단말에 서버 이름과 신뢰점을 배포한다.
2. **가용성 확보**: 서버 이중화와 인증서 갱신을 자동화한다.
...
- EAP 인증 결과와 동적 포트 정책을 결합하여 네트워크 접근을 통제한다.
