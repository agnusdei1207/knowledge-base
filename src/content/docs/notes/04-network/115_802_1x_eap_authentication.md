...
1. 사내 무선망의 **EAP-TLS 상호 인증**

### 쉽게 이해하기 (학습용)

- 단말과 인증 서버에 인증서와 신뢰점을 배포해 서로의 신원을 검증하고 비인가 단말과 가짜 인증 서버의 접속을 차단한다.

## Ⅶ. 결론

- 네트워크 포트의 무단 단말 접속을 막기 위해 단말 관리 여부·상호 인증·인증서 수명·미지원 장비 위험을 검토하여, 관리 단말에는 EAP-TLS 기반 802.1X를 적용해야 한다.
