...
- 정의/개념: EAP 인증 결과로 **통제 포트 접근 권한**을 부여하는 포트 기반 접근 제어한다.
- 배경/필요성: 물리 접속•공유 암호의 **개별 통제하기 어렵다**.
...
- **인증 요청자(Supplicant)**: 단말에서 EAP 자격과 응답을 제시한다.
- **인증 중계자(Authenticator)**: 접속 장비에서 EAP 대화를 중계하고 포트 정책을 집행한다.
- **인증 서버(Authentication Server)**: 자격을 검증하고 네트워크 접근 권한을 결정한다.
...
- **요청자•중계자•인증 서버** 간 인증•집행 역할을 분리한다.
- **EAPOL•RADIUS**를 통한 단말 EAP 인증 대화 중계가 핵심이다.
- **통제 포트•동적 권한** 기반 승인 후 망 구역•정책을 적용한다.
...
- **EAPOL 중계**: 단말의 EAP 요청•응답을 접속 장비가 수신한다.
- **RADIUS 중계**: 접속 장비가 EAP 대화와 권한 정보를 인증 서버와 교환한다.
- **통제 포트**: 인증 결과에 따라 일반 데이터 통신의 허용 여부가 바뀌는 논리 포트이다.
- **신원 저장소**: 사용자•단말 계정과 인증 자격 정보를 제공한다.
- **인증서 저장소**: 인증기관과 인증서 신뢰•폐기 정보를 제공한다.
...
가지의 의미: 자격 제시•중계•검증•신뢰•집행 책임을 분리한 구조다.
...
| 인증 서버 | **자격 검증•권한 결정** |
| 신원 저장소 | **계정•인증 자격 정보** 제공 |
| 인증서 저장소 | **인증기관•신뢰•폐기 정보** 제공 |
...
1. **RADIUS EAP 대화 중계**: 중계자가 EAP 요청•응답을 인증 서버에 전달한다.
2. **인증서•신뢰 체인 검증**: 단말•서버 인증서와 신뢰점을 확인한다.
3. **권한 정책 전달**: 인증 서버가 승인 여부와 망 정책을 제공한다.
4. **통제 포트 정책 적용**: 승인 구역 개방•주기를 재인증한다.
...
- 관리 단말은 **EAP-TLS**, 802.1X 미지원 장비는 **MAB 최소 권한**을 적용한다.
