...
- **분산 클라우드 네트워킹(Distributed Cloud Networking)**: 중앙의 연결•경로•보안 의도를 여러 지역과 엣지에서 일관되게 집행하는 분산 네트워크 운영 구조

</details>

- 정의/개념: 중앙의 연결•경로•보안 의도를 지역•엣지 게이트웨이에서 집행하는 **분산 클라우드 네트워킹 구조**
...
- **지역별 분산 집행**: 중앙 제어 평면이 계산한 정책을 각 지역 게이트웨이가 자체 데이터 경로에서 실행하는 방식
- **위치 기반 처리**: 지연•전송 비용•데이터 주권 조건에 맞는 지역이나 엣지에서 서비스를 실행하는 방식
- **정책 드리프트**: 지역 게이트웨이의 실제 경로•보안 설정이 중앙 선언 정책과 달라진 상태
...
- **중앙 제어 평면**: 서비스 배치•경로•보안 의도를 계산하고 배포하는 구성요소
- **정책 엔진**: 신원•주권•망 구역 조건을 판정하는 구성요소
- **지역•엣지 게이트웨이**: 오버레이를 종단하고 지역별 경로•보안 정책을 실제 데이터 경로에 집행하는 구성요소
- **서비스 발견기**: 위치•상태 조건에 맞는 서비스 종단점을 제공하는 구성요소
- **관측•검증 계층**: 실제 경로•지연•전송 비용과 지역 정책 상태를 중앙 선언 의도와 대조하는 계층
...
선의 의미: 중앙 제어 평면과 지역•엣지 게이트웨이는 배치•경로•보안 의도와 지역 집행 관계이고, 게이트웨이는 서비스 발견기•정책 엔진•관측 검증 계층과 종단점•조건 판정•드리프트 확인 관계이다.
...
- **드리프트 탐지**: 관측 계층이 지역 게이트웨이의 실제 경로와 보안 설정이 중앙 선언 정책에서 벗어났는지 확인하는 절차
- **정책 재동기화**: 불일치한 지역 정책을 중앙 선언 상태에 맞게 다시 배포하는 절차
...
- **분산 클라우드**: 한 운영 주체가 여러 지역•엣지 자원을 공통 제어해 지연과 데이터 주권 요구를 함께 충족하는 배치 방식
- **다중 클라우드**: 여러 클라우드 사업자의 환경과 기능을 조합해 종속을 줄이는 배치 방식
- **중앙 클라우드**: 한 지역에 자원을 집중해 운영 단순성과 규모 효율을 우선하는 배치 방식
...
- **데이터 주권 위반 경로**: 저장•처리가 허용된 지역을 벗어나 원본이나 중간 트래픽이 다른 관할로 전달되는 문제
- **현지 처리•결과 집약**: 원본 데이터는 허용 지역에서 처리하고 필요한 결과만 중앙으로 보내는 비용•주권 대책
...
- **주권 제약**: 데이터 저장•처리•전송 위치를 특정 관할에 제한하는 조건
- **운영 단순성**: 중앙 집중으로 정책•관측•장애 대응 절차를 줄이는 특성
