...
sequenceDiagram
    ...
    participant 관측계층
    서비스->>제어평면: 1. 배치 조건 등록
    제어평면->>정책엔진: 2. 주권·접근 판정
    정책엔진->>게이트웨이: 3. 경로 정책 배포
    게이트웨이->>관측계층: 4. 경로·비용 측정
    관측계층->>제어평면: 5. 드리프트 재동기화
```

**동작 원리**

- **1. 배치 조건 등록**: 지연·주권·비용 요구를 정의
- **2. 주권·접근 판정**: 위치·신원·자료 조건을 판정
- **3. 경로 정책 배포**: 지역별 오버레이·장애 경로 설정
- **4. 경로·비용 측정**: 실제 지연·손실·전송량 확인
- **5. 드리프트 재동기화**: 불일치 정책을 탐지·재배포
...
| 한계 | 지역 드리프트·복잡한 경로 | 사업자별 정책·도구 불일치 | 지역 장애·장거리 지연 |
