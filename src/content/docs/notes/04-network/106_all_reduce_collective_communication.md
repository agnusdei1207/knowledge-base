...
- **축소 후 동일 결과로 전체 배포**: All-Reduce는 모든 순위의 같은 위치 값을 합산·최댓값 등으로 축소한 뒤 동일한 전체 결과를 각 순위에 배포한다.
...
- **지연 순위**: 지연 순위는 계산이나 통신 완료가 가장 늦어 동기식 All-Reduce에 참여한 모든 GPU의 다음 단계를 기다리게 한다.
...
- **집계 연산 계약**: 집계 연산 계약은 모든 순위가 동일한 통신 그룹·텐서 형상·자료형·연산과 호출 순서를 사용하도록 정한다.
...
- **3. 축소 분산 교환**: 참여 순위는 입력을 조각내 이웃과 순환·계층 교환하며 각 조각의 축소 결과를 서로 다르게 보유한다.
...
|:---|:---|:---|:---|
| 적용 기준 | 큰 텐서·균일 링크의 **대역폭 우선** | 작은 텐서·다수 순위의 **지연 우선** | **내부·외부 링크 성능 차이**가 큰 환경 |
...
- **호출 순서·형상 불일치**: 호출 순서·형상 불일치는 순위별 집합 연산 계약이 달라 서로 다른 통신을 기다리거나 잘못된 결과를 만드는 문제다.
