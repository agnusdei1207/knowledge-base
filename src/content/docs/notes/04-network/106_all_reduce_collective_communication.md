...
- 배경/필요성: 중앙 서버의 **대규모 기울기 집계 병목**이 발생한다.
...
- 자료 크기•토폴로지에 따른 **링•트리•계층형 선택**이 핵심이다.
- **지연 순위•호출 계약 불일치**에 따른 전체 완료 지연•중단이 핵심이다.
...
가지의 의미: 참여•계약•묶음•집계•전송 책임을 분리한 정적 구조다.
...
1. **호출 계약 확인**: 그룹•형상•자료형•연산을 대조한다.
2. **텐서 버킷 구성**: 자료를 경로•크기에 맞게를 묶음한다.
3. **축소 분산 교환**: 조각별 축소 결과를 분산을 보유한다.
4. **전체 수집 교환**: 집계 조각을 모든 순위가 교환한다.
#### 한줄 요약
...
- 큰 텐서•균일 링크는 **링**, 작은 텐서•저지연은 **트리**를 선택한다.
