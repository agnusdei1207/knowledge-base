...
- 정의/개념: 모든 순위의 값을 **축소 후 동일 결과로 전체 배포**이다.
- 배경/필요성: 중앙 서버에 모든 **GPU**의 기울기가 모여 **대규모 기울기 집계 병목**이 발생한다.
...
- **지연 순위**와 **집계 연산 계약** 불일치가 전체 완료를 지연•중단시킨다.
...
| 구성요소 | 책임 |
|:---|:---|
| 순위 통신 그룹 | **순위 통신 그룹**이 각 **순위**의 참여 프로세스•GPU 범위 정의 |
| 집계 연산 계약 | **집계 연산 계약**이 형상•자료형•연산 일치 규정 |
| 버킷 스케줄러 | **버킷 스케줄러**가 텐서 묶음•계산 통신 중첩 |
| 집합 통신 엔진 | **집합 통신 엔진**이 링•트리•계층 알고리즘 실행 |
| 토폴로지 경로 | **토폴로지 경로**가 NVLink•**RDMA** 링크 제공 |
...
1. **호출 계약 확인**: 그룹•형상•자료형•연산의 **호출 계약 확인**을 수행한다.
2. **텐서 버킷 구성**: 자료를 경로•크기에 맞게 묶는 **텐서 버킷 구성**을 수행한다.
3. **축소 분산 교환**: 조각별 축소 결과를 나눠 갖는 **축소 분산 교환**을 수행한다.
4. **전체 수집 교환**: 집계 조각의 **전체 수집 교환**으로 모든 순위가 결과를 얻는다.
#### 한줄 요약
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| **지연 순위**의 전체 지연 | 순위별 완료시간•링크 감시 | 병목 순위 식별 |
...
- **최느린 순위 병목**을 줄이기 위해 큰 텐서•균일 링크는 **링 All-Reduce**, 작은 텐서•저지연은 **트리 All-Reduce**, 링크 계층 차이가 크면 **계층형 All-Reduce**를 택하는 **All-Reduce 알고리즘 선택**이 필요하다.
