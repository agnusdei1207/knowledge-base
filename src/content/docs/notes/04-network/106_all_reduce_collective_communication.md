...
- **지연 순위**: 계산이나 통신 완료가 가장 늦어 모든 GPU의 다음 단계를 기다리게 하는 순위
...
- **순위 통신 그룹**: 집합 통신에 참여하는 프로세스•GPU 범위
- **집계 연산 계약**: 모든 순위의 형상•자료형•연산•호출 순서를 맞추는 규칙
- **버킷 스케줄러(Bucket Scheduler)**: 작은 텐서를 묶어 통신 시작을 조정하는 구성요소
- **집합 통신 엔진**: 링•트리•계층 알고리즘을 실행하는 구성요소
- **토폴로지 경로**: NVLink•RDMA 링크로 구성된 실제 전송 경로
- **순위(Rank)**: 통신 그룹 안에서 각 프로세스나 GPU를 구분하는 번호
...
- **호출 순서•형상 불일치**: GPU 순위별 집합 연산 계약이 달라 대기나 결과 오류를 만드는 문제
