...
- 정의/개념: 설비 데이터를 운영 판단•제어에 연결하는 **산업 운영 체계**이다.
- 배경/필요성: IT•OT 직접 결합은 **장애 전파•노후 명령 오제어**를 유발한다.
...
- **안전 우선**: 처리량보다 OT 가용성•결정성 보호한다.
- **경계 분리**: 게이트웨이•DMZ로 IT•OT 책임 분할한다.
- **현장 검증**: 승인 명령도 최신 상태•안전 한도 재확인한다.
...
| IT 분석•업무 시스템 | **품질•정비•생산 분석**과 정책 제안 |
...
1. **공정 데이터 수집**: 측정값•부하•운전 단계를 함께 획득한다.
2. **현장 데이터 정제**: 잡음•중복 제거와 단위•시각을 정렬한다.
3. **조치 권고**: 열화 예측과 생산•안전 영향을 제시한다.
4. **안전 조건 재검증**: 최신 상태•허용 범위•연동 조건을 확인한다.
5. **제한 명령 실행**: 승인 범위 안의 명령만 PLC을 적용한다.
...
- 업무 분석은 **IT**, 현장 제어는 **PLC•엣지**, 위험 명령은 **안전 연동 차단**이 핵심이다.
