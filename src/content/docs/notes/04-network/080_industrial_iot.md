...
- **액추에이터(Actuator)**: 제어 명령을 밸브·모터 같은 물리 동작으로 바꾸는 장치
- **예지 정비(Predictive Maintenance)**: 설비 상태와 열화 추세로 고장 전에 정비 시점을 결정하는 방식
- **안전 상태(Safe State)**: 위험 발생 시 사람·설비 피해를 최소화하도록 정한 공정 상태
- **구역·도관(Zone·Conduit)**: 유사 보안 수준의 자산을 묶고 구역 간 통신 경로를 통제하는 구조
- **탭·미러 포트(TAP·Mirror Port)**: 기존 통신 사본을 수동 관찰 장비에 제공하는 수집 지점

> **키워드:** 산업용 IoT (Industrial IoT, IIoT)
