...
date: "2026-07-25T20:20:00+09:00"
tags:
  - "notes-network"
weight: 80
extra:
...
- **정의**: 설비 데이터를 안전한 운영 결정에 연결
- **필요성**: OT 안전·가용성을 지킨 IT 분석 연계
...
- 안전·가용성을 처리량보다 우선한다.
- 게이트웨이·DMZ가 IT와 OT의 직접 연결을 막는다.
- 측정값과 공정 맥락을 함께 분석한다.
- 승인·현장 안전 조건을 통과한 명령만 실행한다.
- 잘못된 분석이 제어로 이어지면 안전사고가 발생한다.
...
| 판단 기준 | IIoT | 소비자 IoT |
|:---|:---|:---|
| 핵심 특징 | 현장 제어·IT 분석 분리 | 기기·중앙 연결로 편의 자동화 |
| 적용 기준 | 중단·오동작 영향이 큰 산업 시설 | 재시작 가능한 가정·개인 기기 |
| 주요 위험 | 분석 변경의 지연·중단·안전사고 | 계정·개인정보·중앙 서비스 위험 |
...
1. 수동 관찰로 가동 설비 자산 식별
2. MFA·점프 서버로 원격 정비 통제

### 쉽게 이해하기 (학습용)

- OT 장비에 탐색 부하를 주지 않도록 기존 통신부터 관찰해 자산을 식별한다
- 점프 서버에서 정비자의 신원·대상·시간·명령을 한 흐름으로 통제한다

## Ⅶ. 결론

- 승인·현장 안전 재검증 후 제한 제어 실행
