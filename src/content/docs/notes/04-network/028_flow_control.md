...
- **전송 제어 프로토콜(Transmission Control Protocol, TCP)**: 순서 보장·재전송·흐름 제어·혼잡 제어를 제공하는 연결형 전송 프로토콜
...
- rwnd는 수신 버퍼 여유를 송신 측에 광고한다.
- 슬라이딩 윈도는 RTT 중에도 연속 전송한다.
- 느린 시작은 ACK마다 cwnd를 늘려 경로 용량 탐색한다.
- cwnd가 ssthresh에 이르면 증가 속도를 완화한다.
- 손실·ECN은 cwnd를 줄여 혼잡을 완화한다.
...
| 판단 기준 | 느린 시작 기반 혼잡 제어 | 슬라이딩 윈도 기반 흐름 제어 |
|:---|:---|:---|
| 핵심 특징 | cwnd 증가 후 혼잡 시 축소 | rwnd 안에서 연속 전송 |
| 적용 기준 | 경로 수용량 탐색·혼잡 대응 | 수신 버퍼 초과 방지 |
| 주요 위험 | 경로 큐·손실 증가 | 수신 응용 지연·버퍼 부족 |
...
2. 장거리 고속 전송은 rwnd·cwnd 병목 구분

### 쉽게 이해하기 (학습용)

- rwnd가 0이면 경로 증설보다 수신 응용과 버퍼를 먼저 점검한다
- 고속 경로도 작은 윈도가 있으면 처리량이 제한되므로 어느 윈도가 병목인지 구분한다
