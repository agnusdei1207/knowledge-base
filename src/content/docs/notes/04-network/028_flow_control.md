...
- 정의/개념: **TCP 전송량 제어** — 수신 흐름 제어의 `rwnd`와 네트워크 혼잡 제어의 `cwnd` 중 작은 값으로 미확인 전송량을 제한하는 **방식**이다.
- 배경/필요성: 과도한 송신량으로 **수신 버퍼 초과•경로 혼잡**이 발생한다.
...
- rwnd의 **수신 버퍼 여유 광고**가 핵심이다.
- cwnd의 **네트워크 혼잡 한도 추정**이 핵심이다.
- `min(rwnd,cwnd)`의 **실제 송신 한도 결정**이 핵심이다.
...
2. **ACK•rwnd**: 누적 수신 번호와 **수신 버퍼 여유**를 회신한다.
3. **ACK•혼잡 신호**: ACK•손실•ECN을 **경로 수용량** 근거로 전달한다.
4. **cwnd**: 느린 시작•혼잡 회피 결과로 **혼잡 윈도**를 갱신한다.
...
- rwnd가 작으면 **흐름 제어**, cwnd가 작으면 **혼잡 제어** 우선을 조정한다.
