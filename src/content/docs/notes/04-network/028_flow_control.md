...
date: "2026-08-26T13:44:57+09:00"
tags:
  - "notes-network"
weight: 28
extra:
...
- 배경/필요성: 수신 호스트 버퍼 오버플로우와 **네트워크 라우터 큐 포화에 따른 대규모 패킷 드롭 및 혼잡 붕괴(Congestion Collapse) 방어 불가**를 겪으므로, 수신 버퍼를 직접 통보받는 rwnd와 망 상태를 손실·지연으로 추정하는 cwnd를 각각 두고 그 최솟값으로 전송량을 정해 종단 자원과 중간 경로 자원을 하나의 송신 윈도우에서 함께 지킬 필요
...
- 수신 병목은 **rwnd**, 경로 혼잡은 **cwnd·AQM·ECN**으로 제어
