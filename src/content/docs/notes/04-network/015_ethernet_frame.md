...
- 정의/개념: **이더넷 프레임** — 송수신 MAC 주소•상위 데이터•오류 검사값을 정해진 형식으로 캡슐화한 **데이터링크 계층 전송 단위**이다.
- 배경/필요성: 연속 비트 신호의 **경계•수신자•오류 식별할 수 없다**.
...
- 프리앰블•SFD의 **클록 동기•프레임 경계**가 핵심이다.
- 목적지 MAC의 **링크 수신 인터페이스 식별**이 핵심이다.
- FCS의 **비트 오류 검출•정정 불가**가 핵심이다.
...
- 전 구간 **점보 MTU** 지원 시 점보, 불일치 시 **표준 프레임**을 선택한다.
