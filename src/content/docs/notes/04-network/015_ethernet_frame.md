...
- **매체 접근 제어 주소(Media Access Control Address, MAC 주소)**: 같은 링크에서 프레임의 출발지·목적지 인터페이스를 식별하는 48비트 주소
- **근거리 통신망(Local Area Network, LAN)**: 건물·캠퍼스처럼 제한된 범위의 장치를 링크로 연결한 네트워크
- **이더타입(EtherType)**: 이더넷 프레임의 페이로드를 처리할 상위 프로토콜을 나타내는 값
- **프레임 검사 순서(Frame Check Sequence, FCS)**: 수신 측이 프레임 비트 오류를 검출하도록 끝에 붙이는 검사값
- **최대 전송 단위(Maximum Transmission Unit, MTU)**: 링크가 한 프레임에서 전달할 수 있는 최대 상위 데이터 크기
...
- **정의/개념**: 주소·데이터·검사값을 담은 링크 PDU
- **배경/필요성**: 링크 데이터의 경계·식별·상위 프로토콜·오류 검증
...
- 프리앰블·SFD가 수신 클록과 프레임 경계를 맞춘다.
- 목적지 MAC 주소가 링크의 전달 인터페이스를 지정한다.
- FCS 불일치는 비트 오류를 검출하지만 정정하지 못한다.
...
| 판단 기준 | 이더넷 II | IEEE 802.3 LLC |
|:---|:---|:---|
| 핵심 특징 | 이더타입으로 상위 프로토콜 표시 | 길이 뒤 LLC로 상위 서비스 표시 |
| 적용 기준 | 일반 IP 기반 이더넷 통신 | IEEE 802 계열 LLC 서비스 |
| 주요 위험 | 이더타입 오해석 | 길이·LLC 헤더 오해석 |
...
2. VLAN 트렁크는 태그 식별자와 허용 VLAN을 검증

### 쉽게 이해하기 (학습용)

- 큰 프레임을 쓰는 모든 장비의 통과 크기가 같아야 중간 장비에서 프레임이 버려지지 않는다
- 여러 VLAN을 한 링크로 보낼 때 태그와 허용 목록이 다르면 특정 VLAN만 전달되지 않는다

## Ⅶ. 결론

- 주소·유형·FCS로 링크 전달·해석·오류를 판정
