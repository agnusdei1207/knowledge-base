...
- 연속된 비트열에 프리앰블•MAC 주소•페이로드•FCS를 결합하여 하나의 프레임 단위로 구분한다.
...
선의 의미: 선은 하나의 이더넷 프레임 안에서 동기•경계, 링크 주소, 상위 형식, 데이터와 오류 검사 필드가 나란히 결합되는 정적 프레임 레이아웃을 뜻한다.
...
- 앞의 MAC 주소와 이더타입으로 전달•해석하고 뒤의 FCS로 전송 중 비트 오류를 검출한다.
...
- SFD로 프레임 경계를 찾고 MAC 주소로 출력 포트를 결정한 뒤 FCS 검증을 통과한 페이로드만 상위 프로토콜에 전달한다.
