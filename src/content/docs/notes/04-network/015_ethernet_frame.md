...
- 정의/개념: 이더넷(Ethernet) 네트워크에서 L2 노드 간 무결성 있는 프레임 교환을 위해 송수신 **매체 접근 제어 주소(Media Access Control Address, MAC 주소)**, 페이로드 및 에러 검출 코드를 구조화한 **이더넷 프레임(Ethernet Frame)**.
- 배경/필요성: 물리 계층(L1)의 단순 비트스트림(Bitstream) 전송 시 발생하는 비트 경계 식별 불능, 수신 노드 지정 제약 및 비트 손상 감지 한계를 해소하기 위함.

#### 한줄 요약

- L2 데이터링크 계층의 PDU 캡슐화 및 이더넷 프레임 구조 표준화 체계 구현.
...
- **프리앰블(Preamble)** 및 **프레임 시작 구분자(SFD)** 필드를 통한 물리적 수신 동기화 및 프레임 시작점 명확화.
- 목적지/출발지 6바이트 MAC 주소를 통해 L2 도메인 내 정확한 포트 스위칭 수행.
- **프레임 검사 시퀀스(Frame Check Sequence, FCS)** 기반 오류 검출 시 손상된 프레임을 즉시 폐기(Drop)하고 재전송 책임은 L4 상위 계층으로 이관.
...
1. **송신 및 포워딩 단계**: 송신 **NIC**에서 헤더와 FCS를 캡슐화하여 전송하면, L2 스위치는 **Destination MAC Lookup**을 통해 매핑 포트로 **출력 포트 중계**집행.
2. **수신 및 무결성 검증 단계**: 수신 NIC는 SFD 동기화 후 **FCS 오류 검증(CRC-32)**을 연산하여 일치 시 L3 프로토콜(EtherType)로 패킷을 역캡슐화 전달하고, 불일치 시 즉시 폐기.

#### 한줄 요약

- Preamble 비트 동기화, Destination MAC 룩업, 포워딩 및 CRC-32 검증 프로세스 구동.
...
- L2 데이터링크 통신의 무결성 확보와 고속 패킷 포워딩을 위해 **경로 MTU 일치(Path MTU Alignment)**에 입각한 802.1Q VLAN 태그 정합성 및 CRC-32 에러 모니터링 체계 적용 필수.

#### 한줄 요약

- Path MTU 튜닝 및 802.1Q VLAN Tagging 정합성을 통한 L2 프레임 무결성 확보 체계 적용.
