...
- 정의/개념: 5G 코어 망 기능을 연결하는 **서비스 등록•발견•호출 구조**이다.
- 배경/필요성: 4세대 이동통신(Fourth Generation, 4G) 노드 간 **고정 연결•확장 경직성** 해소가 필요하다.
...
- NRF 기반 **망 기능 등록•발견**이 핵심이다.
- 서비스 인터페이스의 **느슨한 결합**이 핵심이다.
- SMF 제어면•UPF 사용자면의 **역할 분리**가 핵심이다.
...
4. **전달 규칙 설치**: SMF가 UPF 경로 규칙을 설정한다.
...
- 기능별 독립 확장이 필요하면 **SBA** 적용과 **발견•호출 장애**를 통제한다.
