...
- **서비스 기반 아키텍처(Service-Based Architecture, SBA)**: 5G 코어 망 기능을 등록·발견 가능한 서비스 호출로 연결하는 구조
- **망 기능(Network Function, NF)**: 5G 코어의 독립 배치·확장 기능 단위
- **서비스 인터페이스**: 제어 기능 간 호출 규격
- **망 저장소 기능(Network Repository Function, NRF)**: 망 기능의 등록·상태·발견 정보를 제공하는 저장소
- **접속 및 이동성 관리 기능(Access and Mobility Management Function, AMF)**: 단말 등록·인증·이동성을 제어하는 망 기능
- **세션 관리 기능(Session Management Function, SMF)**: 데이터 세션 정책과 UPF 전달 규칙을 제어하는 망 기능
- **사용자면 기능(User Plane Function, UPF)**: 설치된 규칙에 따라 사용자 패킷을 전달하는 망 기능
- **통합 데이터 관리(Unified Data Management, UDM)**: 가입자와 인증 정보를 관리하는 망 기능
