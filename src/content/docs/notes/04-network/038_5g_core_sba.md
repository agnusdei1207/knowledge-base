...
- 배경/필요성: 4G LTE EPC의 전용 장비 및 점대점(P2P) 바이너리 프로토콜 한계로 인한 **신규 기능 추가 시 전사 망 재구성 필요, 유연한 수평 확장 및 슬라이싱 제어 불가**
...
#### 한줄 요약
- 단말 요청 → NRF 기반 SMF 검색 → 정책 수신 → PFCP 포워딩 규칙 주입 → 데이터 개통 순으로 동작한다.
