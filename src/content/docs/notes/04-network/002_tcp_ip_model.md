...
- 정의/개념: 인터넷 통신 기능을 공통 IP 중심의 **TCP/IP 4계층**으로 묶은 구현 모델이다.
- 배경/필요성: 응용•매체별 전용 규약은 **이기종 통신 확장 제약**이 발생한다.
...
- 공통 IP의 **얇은 허리 구조**로 응용•통신 매체 결합을 완화한다.
- IP의 **비연결•최선형 전달**이 핵심이다.
- **종단 간 원칙**에 따른 신뢰성•혼잡 제어가 핵심이다.
...
1. **캡슐화**: 포트•IP•링크 제어 정보를 계층별을 결합한다.
...
- **모델 적용 기준**: 인터넷 구현은 TCP/IP, 세부 책임 분석은 OSI 모델을 적용한다.

</details>

- 인터넷 구현은 **TCP/IP**, 세부 장애 책임은 **OSI**로 분석한다.
