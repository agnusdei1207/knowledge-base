...
- **인터넷 프로토콜(Internet Protocol, IP)**: 논리적 IP 주소를 기반으로 이기종 네트워크 간 패킷 라우팅 및 최선형 전달(Best-Effort)을 수행하는 비연결형 인터넷 계층 프로토콜.
...
- 정의: 이기종 네트워크 간 대용량 데이터 통신을 위해 인터넷 프로토콜 스택을 4개 계층으로 구조화한 실무 표준 아키텍처.
- 배경: 벤더 독합 프로토콜의 복잡성을 제거하고 상위 응용과 하위 매체를 분리하여 독립적 상호연동 아키텍처 구현 필요.
...
- **종단 간 원칙(End-to-End Principle)**: 네트워크 코어(라우터)는 라우팅 기능만 단순화하고 데이터 신뢰성 및 상태 제어는 종단 호스트에서 전담 처리하는 설계 철학.
