...
date: "2026-08-02T09:22:00+09:00"
tags:
  - "notes-network"
weight: 2
extra:
...
- **TCP/IP 모델**은 인터넷 통신 기능을 공통 IP 계층 중심의 네 계층으로 묶은 실제 구현 모델이다.
...
- **얇은 허리 구조**는 다양한 응용과 통신 매체가 공통 IP 계층을 사이에 두고 독립적으로 발전하게 한다.
- **최선형 전달**은 IP가 패킷의 도착·순서·중복 제거를 보장하지 않고 가능한 범위에서 전달하는 방식이다.
- **종단 간 원칙**은 신뢰성이나 혼잡 제어 기능을 중간망보다 송수신 호스트에서 완성한다.
...
- **포트**는 한 호스트 안에서 데이터가 도착할 응용 프로세스를 구분하는 번호이다.
- **IP 주소**는 패킷의 출발지와 목적지 호스트를 식별하여 네트워크 간 전달 경로를 정하게 한다.
- **네트워크 접근 계층**은 링크별 프레임과 매체 규칙으로 인접 장치 사이의 전송을 담당한다.
...
- **캡슐화**는 응용 데이터에 포트·IP·링크 제어 정보를 계층 순서대로 결합하는 과정이다.
- **역캡슐화**는 수신 측이 계층의 반대 순서로 제어 정보를 제거하여 응용 메시지를 복원하는 과정이다.
...
- **OSI 모델**은 통신 책임을 일곱 계층으로 세분하여 설계와 장애 분석의 참조 기준을 제공한다.
...
- **TCP**는 연결을 설정하고 순서·재전송을 제어하여 신뢰성 있는 바이트 흐름을 제공한다.
- **UDP**는 연결 설정과 재전송 없이 데이터그램을 보내어 전송 지연과 제어 부담을 줄인다.
- **MTU**는 한 링크에서 단편화 없이 전달할 수 있는 최대 패킷 크기이다.
...
- **전송 프로토콜 선택**은 데이터 손실 허용 여부와 지연 요구를 기준으로 TCP와 UDP 가운데 결정한다.
