...
- **HTTP 문맥 분석**: URL•헤더•본문의 공격 특징 식별이다.
...
1. **TLS 요청 복호화**: 암호 세션 종료 후 HTTP을 추출한다.
2. **HTTP 요청 정규화**: 우회 인코딩을 표준 형태로 변환한다.
3. **공격 문맥 판정**: 서명•행위•속도•예외 정책을 결합한다.
4. **허용 요청 중계**: 정상 요청만 원본 서버로 전달한다.
