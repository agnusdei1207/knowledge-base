...
- 정의/개념: **UDP**는 연결 설정 없이 메시지 경계를 보존한 데이터그램을 전달한다.
- 배경/필요성: 연결 설정과 재전송 제어는 전송 지연과 종단 상태 비용을 늘린다.

#### 한줄 요약

- UDP는 연결 설정과 ACK 없이 데이터그램을 즉시 전송하며, 손실·중복 복구가 필요한 응용만 자체 제어 로직을 구현한다.
...
- **비연결형** 특성으로 연결 상태 없이 전송한다.
- UDP는 데이터그램의 응용 메시지 경계를 보존한다.
- **체크섬**은 비트 오류만 검출하고 신뢰•흐름•혼잡 제어는 응용에 맡긴다.

#### 한줄 요약

- UDP는 재전송·순서 복원을 수행하지 않는 대신 데이터그램의 메시지 경계를 보존하며, 필요한 신뢰성 제어는 응용 계층이 담당한다.
...
| 목적지 포트 | **데이터그램**을 받을 수신 응용 식별 |
| 길이 | UDP 헤더와 **페이로드**의 전체 바이트 수 |
| 체크섬 | **체크섬**으로 주소•헤더•데이터의 비트 오류 검출 |
| 페이로드 | 메시지 경계를 유지하는 응용 데이터 |

#### 한줄 요약

- 출발지·목적지 포트, 길이, 체크섬의 네 헤더 필드가 하나의 페이로드를 캡슐화하여 UDP 데이터그램을 구성한다.
...
- **데이터그램 생성**: 포트•길이•체크섬을 응용 메시지와 결합해 UDP 전송 단위를 만드는 절차이다.
- **데이터그램 검증**: 수신한 UDP 체크섬을 확인하고 정상 데이터그램을 목적지 포트로 넘기는 절차이다.
</details>
...
1. **데이터그램 생성**: 포트•길이와 주소 기반 **체크섬**을 메시지에 결합하고 **IP**로 전달한다.
2. **데이터그램 검증**: 수신 UDP가 체크섬을 확인한 뒤 목적지 포트로 전달한다.

#### 한줄 요약

- 송신 UDP가 포트 번호와 체크섬을 헤더에 결합하여 IP로 전달하고, 수신 UDP가 체크섬을 검증한 뒤 목적지 포트의 응용에 페이로드를 전달한다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 손실•중복이 허용되지 않는 UDP 메시지 | **응용 재시도**와 **중복 제거** 설계 | 메시지 중복•누락 통제 |
| 무제한 송신률로 경로 혼잡 가중 | 응용 혼잡 제어•전송률 제한 | 네트워크 패킷 손실 완화 |
| 데이터그램이 경로 **MTU**를 초과 | 경로 MTU 이하로 메시지 분할 | IP 단편 손실의 전체 메시지 폐기 감소 |
| 출발지 위조 요청으로 **반사 공격**•**증폭 공격** | 응답 크기 제한•쿠키 검증 | 피해자 대상 증폭 트래픽 완화 |

#### 한줄 요약

- 실시간 음성은 지연된 패킷을 폐기하고, 트랜잭션 메시지는 응용 순서 번호와 타임아웃을 적용하여 손실·중복을 응용 계층에서 제어한다.
...
- 손실 허용•낮은 지연이면 **UDP**를 사용하고 신뢰가 필요하면 **응용 재시도**•**중복 제거**를 구현하며 **IP 단편화**를 피한다.

#### 한줄 요약

- 지연에 민감한 음성은 UDP로 즉시 전송하고, 손실 불허 트랜잭션은 응용이 순서 번호와 재전송 로직을 구현한다.
