...
- 정의/개념: **UDP** — 연결 설정과 전송 보장 없이 애플리케이션 메시지 경계를 보존하여 데이터그램을 전달하는 **비연결형 전송계층 프로토콜**이다.
- 배경/필요성: 연결 설정•재전송 제어로 **전송 지연•종단 상태 비용**이 증가한다.
...
- 8바이트 헤더와 **연결 상태 없는 전송**이 핵심이다.
- 데이터그램의 **응용 메시지 경계 보존**이 핵심이다.
- 신뢰•흐름•혼잡 제어의 **응용 위임**이 핵심이다.
...
1. **체크섬 포함 데이터그램**: 포트•길이와 주소 기반 **체크섬**을 메시지에 결합한다.
2. **UDP 데이터그램**: 수신 UDP가 **체크섬 검증** 후 목적지 포트로 전달한다.
...
- 손실 허용•낮은 지연이면 **UDP**, 신뢰가 필요하면 **응용 복구** 구현이 핵심이다.
