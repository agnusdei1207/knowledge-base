...
- **전송 제어 프로토콜(Transmission Control Protocol, TCP)**: 연결 설정 후 순서·확인 응답·재전송을 제공하는 바이트 스트림 프로토콜
...
- **정의/개념**: 포트·길이·체크섬으로 데이터그램을 전달하는 전송 프로토콜
- **배경/필요성**: 응용이 필요한 신뢰 기능만 구현해 전송 지연 축소
...
- 8바이트 헤더와 무연결 전송으로 상태 비용이 작다.
- 데이터그램별 메시지 경계를 수신 응용까지 보존한다.
- 재전송·순서·흐름·혼잡 제어를 응용에 위임한다.
- MTU 초과 단편 하나만 잃어도 데이터그램 전체 폐기한다.
- 송신자 인증 부재가 반사·증폭 공격에 악용된다.
...
| 주요 위험 | 손실·중복·순서 변경·증폭 공격 | 연결 지연·선두 차단 |
...
2. 공개 UDP 서버는 요청률·응답 크기를 제한

### 쉽게 이해하기 (학습용)

- 늦게 도착한 음성은 재생할 수 없으므로 잃은 조각을 기다리지 않고 다음 조각을 재생한다
- 작은 위조 요청이 큰 응답을 만들지 않도록 초당 요청 수와 응답 데이터 크기를 제한한다
