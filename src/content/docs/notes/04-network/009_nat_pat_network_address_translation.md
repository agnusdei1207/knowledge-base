...
- **포트 주소 변환** 메커니즘으로 공인 IP 1개당 최대 64,000여 개의 L4 소켓(Port)을 매핑하여 공인 주소 자원 절감 극대화.
- 동적 세션 만료 타이머(Timeout)를 가진 **변환 테이블**을 내장하여 정합성 및 보안 격리성 확보.
...
| **공인 IP 주소 (Public IP)** | 인터넷 전역 라우팅 가능 대표 주소 | **주소 풀**에서 동적 할당 |
| **변환 테이블 ** | [내부 IP:Port] <-> [공인 IP:Port] 바인딩 및 세션 타임아웃 유지 | 메모리(TCAM/DRAM) 점유 |
...
| 분류 | **정적 NAT (Static NAT)** | **동적 NAT (Dynamic NAT)** | **포트 주소 변환 ** |
|:---|:---|:---|:---|
...
| 복합 응용 통신 실패 | FTP Active Mode 또는 VoIP SIP 페이로드 내 사설 IP 포함 | 해당 프로토콜용 **응용 계층 게이트웨이** 활성화 | 응용 페이로드 주소 자동 보정 |
| 침해 사고 시 사용자 추적 불가 | PAT로 인해 외부에는 동일한 공인 IP만 노출됨 | **변환 매핑 로그** 중앙 Syslog/SIEM 수집 체계 구축 | 공인 IP 기반 내부 접속자 정밀 추적 |
| 외부에서 내부 서버 접속 불가 | PAT는 외부발 신규 세션을 차단함 | 특정 서비스 포트에 대한 **포트 포워딩** 설정 | 사설 서버 외부 공개 수용 |
