...
- 정의/개념: 경계 장비가 사설•공인 주소와 포트를 대응시키는 **NAT•PAT 변환 기술**이다.
- 배경/필요성: 공인 IPv4 주소 고갈로 단말별 **전역 주소 할당 제약**이 발생한다.
...
- **상태 기반 NAT**: 연결별 변환 항목을 만들고 응답 패킷을 같은 항목으로 역변환한다.
- **변환 테이블**: 변환 전후의 프로토콜•주소•포트와 만료 시각을 연결하여 저장한다.

</details>

- 주소 변환의 **내부•외부 주소 분리**가 핵심이다.
- PAT 포트 구분의 **공인 주소 공유**가 핵심이다.
- 상태 항목의 **응답 역변환 보장**이 핵심이다.
...
1. **송신 튜플 변환**: 공인 튜플을 할당하고 체크섬을 갱신한다.
2. **수신 튜플 복원**: 변환 상태표로 사설 주소•포트를 복원한다.
...
- **포트 포워딩**: 공인 주소의 특정 포트로 들어온 새 연결을 미리 지정한 내부 서버로 전달한다.
- **응용 계층 게이트웨이(Application Layer Gateway, ALG)**: 패킷 본문의 주소와 포트를 해석하여 NAT의 헤더 변환과 일치하도록 보정한다.
- **변환 매핑 로그**: 시각과 튜플 및 변환 결과를 기록하여 공인 주소를 공유한 내부 사용자를 추적하게 한다.
...
- **NAT 방식 선택**: 외부에서 시작하는 고정 연결의 필요성과 내부 동시 세션 수를 기준으로 결정한다.

</details>

- 외부 고정 연결은 **정적 NAT**, 다수 내부 연결은 **PAT**을 선택한다.
