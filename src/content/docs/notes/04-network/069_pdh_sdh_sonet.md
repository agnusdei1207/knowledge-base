...
date: "2026-07-25T12:20:00+09:00"
tags:
  - "notes-network"
weight: 69
extra:
...
- 정의: PDH는 준동기, SDH·SONET은 동기 계위다
- 배경: 하위 신호 분기 시간과 장애 복구 시간을 줄인다
...
- PDH 비트 채움은 다단 역다중화 비용을 높인다
- 포인터 조정은 클럭 차이에도 직접 분기를 가능하게 한다
- 프레임 OAM은 장애 위치를 알려 복구 시간을 줄인다
...
| 주요 위험 | 다단 역다중화·지역 계위 차이 | 포인터 조정·동기 품질 | SDH와 계위·용어 매핑 오류 |
...
1. PDH 계위를 SDH VC에 매핑해 연동한다
2. OAM 장애 뒤 광전송을 예비 경로로 절체한다

### 쉽게 이해하기 (학습용)

- 기존 준동기 회선을 맞는 가상 컨테이너에 넣어 전체 역다중화 없이 전달한다
- 주 경로 오류를 프레임 오버헤드로 감지하면 미리 정한 예비 경로로 바꾼다

## Ⅶ. 결론

- 지역 계위·분기·보호 요구로 전송 체계를 선택한다
