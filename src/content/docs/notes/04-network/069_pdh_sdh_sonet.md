...
- **PDH·SDH·SONET**: 각각 준동기 전송 계위·국제 동기 전송 계위·북미 동기 광전송 체계
- **VC·SPE·STM·STS·OC**: SDH·SONET에서 페이로드를 수용하고 전기·광 신호의 전송 계위를 구분하는 프레임 구조
- **ADM·DXC·OAM**: 저속 신호 분기결합·회선 교차 연결·운용 및 유지관리를 수행하는 전송망 기능

## Ⅰ. 개요

- 정의/개념: 디지털 신호의 **다중화·전송 계위**
...
sequenceDiagram
    ...
    participant 분기장치
    하위신호->>매퍼: 1. 하위 신호 전달
    매퍼->>동기프레임: 2. VC·SPE 매핑
    동기프레임->>동기프레임: 3. 포인터 위치 조정
    동기프레임->>OAM·보호: 4. 동기 프레임 전송
    OAM·보호->>분기장치: 5. OAM 상태 보고
```

**동작 원리**

1. **하위 신호 전달**: 입력 속도·계위 식별
2. **VC·SPE 매핑**: 하위 신호와 경로 오버헤드 수용
3. **포인터 위치 조정**: 클럭 차이에 따라 시작 위치 변경
4. **동기 프레임 전송**: STM·STS 계위로 다중화해 전달
5. **OAM 상태 보고**: 오류·품질·경로 상태 전달
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 지역 계위 변환 오류 | PDH·SDH·SONET 매핑표 검증 | 회선 상호운용 |
| 포인터 조정 과다 | 동기 품질·포인터 이벤트 감시 | 지터 억제 |
| 보호 절체 미시험 | 장애 유형별 예비 경로 훈련 | 전송 연속성 |
...
- **계위 매핑·동기·보호 절체에 맞춘 SDH·SONET 적용**
