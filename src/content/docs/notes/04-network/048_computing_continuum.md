...
- **컴퓨팅 연속체(Computing Continuum)**: 단말·에지·클라우드의 이질적 자원을 하나의 정책·운영 체계로 연결하는 실행 환경
- **최말단 에지(Far Edge)**: 센서·장치와 가장 가까운 곳에서 짧은 지연의 제어·전처리를 수행하는 자원
- **근접 에지(Near Edge)**: 최말단 에지와 클라우드 사이에서 지역 집계·추론을 수행하는 자원
- **클라우드 컴퓨팅(Cloud Computing)**: 중앙 데이터센터의 공유 자원으로 대규모 저장·연산 서비스를 제공하는 구조
- **워크로드(Workload)**: 배치·실행·이동의 대상이 되는 응용과 그 연산 작업
- **오케스트레이션(Orchestration)**: 요구 조건과 자원 상태에 따라 워크로드의 배치·확장·복구를 조정하는 기능
- **서비스 수준 목표(Service Level Objective, SLO)**: 지연·가용성처럼 서비스가 달성해야 하는 측정 가능한 목표
- **워크로드 이식성(Workload Portability)**: 응용이 실행 환경 차이를 흡수하고 다른 위치에서도 같은 기능을 수행하는 성질
- **데이터 중력(Data Gravity)**: 데이터 규모와 의존성이 커질수록 연산을 데이터 가까이에 묶어 두는 현상
- **워크로드 신원(Workload Identity)**: 실행 위치가 바뀌어도 같은 응용임을 검증하는 고유 신원
- **제어면(Control Plane)**: 자원 상태와 정책을 바탕으로 워크로드의 실행 위치를 결정하는 계층
- **캐시(Cache)**: 자주 쓰는 데이터 복사본을 가까운 저장 공간에 두어 접근 시간을 줄이는 기법
- **인스턴스(Instance)**: 배포된 워크로드가 특정 노드에서 실제로 실행되는 개별 실행 단위

> **키워드:** 컴퓨팅 연속체 (Computing Continuum / Cloud-Edge Continuum)
