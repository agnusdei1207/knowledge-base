...
- 정의/개념: 단말·에지·클라우드의 **통합 배치·운영 환경**
- 배경/필요성: 위치별 고립 운영의 **자원 유휴·수동 이관**
...
block
    columns 1
    A["통합 제어면 | 자원 발견과 정책 기반 배치·복구"]
    B["클라우드 | 대규모 학습·전사 분석·장기 저장"]
    C["근접 에지 | 지역 데이터 집계·추론·캐시"]
    D["최말단 에지 | 장치 인접 제어와 단절 시 자율 실행"]
    E["데이터·상태 계층 | 데이터·모델·실행 상태의 위치 간 동기화"]
    A --- B
    B --- C
    C --- D
    D --- E
...
sequenceDiagram
    ...
    participant 상태계층
    participant 목표노드
    자원노드->>통합제어면: 1. 자원 상태 보고
    통합제어면->>상태계층: 2. 상태 위치 조회
    통합제어면->>목표노드: 3. 워크로드 배치
    상태계층->>목표노드: 4. 데이터·상태 동기화
    목표노드->>통합제어면: 5. 실행 상태 보고
```

**동작 원리**

1. **자원 상태 보고**: 노드가 연산·망·에너지 상태를 전달
2. **상태 위치 조회**: 데이터·모델·실행 상태의 위치 확인
3. **워크로드 배치**: SLO·정책을 만족하는 목표 노드 선택
4. **데이터·상태 동기화**: 실행에 필요한 상태를 목표 노드로 복제
5. **실행 상태 보고**: 목표 노드의 SLO 충족 여부 관측
...
|:---|:---|:---|
| 적용 기준 | SLO 기반 **위치 간 재배치 필요** | 위치 고정·**독립 운영 우선** |
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 데이터 이동비용 누락 | 데이터 중력·상태 크기 계측 | 무익한 재배치 방지 |
| 위치 간 정책 불일치 | 공통 신원·보안·배치 정책 | 운영 일관성 |
| 전환 중 중복 실행 | 상태 동기·절체·회수 순서화 | 처리 중복 방지 |
...
- **SLO 이득과 데이터·상태 이동 비용을 비교한 연속체 배치**
