...
- 정의/개념: 단말•에지•클라우드 자원을 통합 관측해 **SLO별 워크로드**를 배치하는 **컴퓨팅 연속체**이다.
- 배경/필요성: 위치별 고립 제어면으로 **유휴 자원 재배치•장애 복구** 자동화 제한이 발생한다.
...
- 위치별 **연산•망•에너지** 통합 관측으로 배치 후보를 확대한다.
- 표준 실행 환경의 **워크로드 이식성**으로 위치 간 재배치가 핵심이다.
...
3. **워크로드 배치 명세**: **SLO•정책**을 만족하는 목표 노드를 지정한다.
4. **데이터•실행 상태**: 실행에 필요한 상태를 목표 노드로 복제한다.
5. **실행 지표**: 목표 노드의 **SLO 충족 여부**를 판정한다.
...
|:---|:---|:---|
| 적용 기준 | **SLO**에 따라 **위치 간 재배치**가 필요할 때 | 위치 고정과 **독립 운영**이 우선일 때 |
...
- **SLO** 개선 이득이 데이터•상태 이동 비용보다 크면 **재배치**, 아니면 **위치 고정**이 핵심이다.
