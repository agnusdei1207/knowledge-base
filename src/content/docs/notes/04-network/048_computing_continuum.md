...
- 정의/개념: 단말•에지•클라우드 자원을 통합 관측해 **서비스 수준 목표(Service Level Objective, SLO)별 워크로드** 를 배치하는 **컴퓨팅 연속체(Computing Continuum)**
...
- **서비스 수준 목표(SLO)**: 지연•가용성처럼 서비스가 달성해야 하는 측정 가능한 목표이다.
...
3. **워크로드 배치 명세**: **서비스 수준 목표(Service Level Objective, SLO)•정책** 을 만족하는 목표 노드 지정
...
|:---|:---|:---|
| 적용 기준 | **서비스 수준 목표(Service Level Objective, SLO)** 에 따라 **위치 간 재배치** 가 필요할 때 | 위치 고정과 **독립 운영** 이 우선일 때 |
...
- **서비스 수준 목표(Service Level Objective, SLO)** 개선 이득이 데이터•상태 이동 비용보다 크면 **재배치**, 아니면 **위치 고정**
