...
#### 한줄 요약
- BPDU 비교는 루트 도달 비용이 낮은 포트만 남기고 나머지를 차단하므로, 이중화로 사 둔 물리 대역폭 일부를 상시 유휴로 묶는 것이 루프 제거의 대가가 된다.
...
- L2 엔터프라이즈 스위칭 환경에서 브로드캐스트 스톰을 차단하고 무중단 링크 페일오버를 보장하는 **가장 핵심적이고 전통적인 L2 루프 방지 제어 표준**으로 확립되었으며, 최근에는 차단 포트 낭비 없이 모든 링크 대역폭을 100% 활용하는 **MC-LAG(Multi-Chassis LAG) 및 L3 Leaf-Spine 패브릭(EVPN-VXLAN)으로 진화함과 동시에, 실무 운영 시에는 비인가 스위치의 토폴로지 교란을 막는 Root Guard/BPDU Guard, 단말 즉각 접속을 위한 PortFast(Edge Port), 단방향 링크 단선을 감지하는 UDLD(Loop Guard)**를 결합하여 견고한 L2 가용성을 완성
