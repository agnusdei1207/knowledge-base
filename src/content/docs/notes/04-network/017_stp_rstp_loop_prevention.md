...
- **스패닝 트리 프로토콜(Spanning Tree Protocol, STP)**: 스위치 중복 링크 일부를 비전달 상태로 두어 루프 없는 활성 트리를 만드는 프로토콜
- **고속 스패닝 트리 프로토콜(Rapid Spanning Tree Protocol, RSTP)**: 포트 역할과 제안·동의 절차로 장애 후 전달 경로를 빠르게 전환하는 프로토콜
- **VLAN별 스패닝 트리 플러스(Per-VLAN Spanning Tree Plus, PVST+)**: '피브이에스티 플러스'로 읽고 기존 방식에 기능을 더했다는 `+` 표기로 VLAN마다 독립된 스패닝 트리를 계산하는 방식을 구분
- **브리지 프로토콜 데이터 단위(Bridge Protocol Data Unit, BPDU)**: 스위치가 루트 식별자·경로 비용·포트 정보를 교환하는 제어 메시지
...
- **정의/개념**: 루트·포트 역할을 선출해 무루프 활성 트리 구성
- **배경/필요성**: 이더넷 루프의 프레임 순환·주소표 변동 방지
...
- 가장 낮은 브리지 식별자가 트리의 루트를 결정한다.
- 누적 경로 비용이 루트·지정·대체 포트 역할을 결정한다.
- 중복 링크를 대기시켜 루프 없이 장애 경로를 보존한다.
- RSTP 제안·동의가 포트 전환 대기를 단축한다.
...
| 판단 기준 | STP | RSTP | PVST+ |
|:---|:---|:---|:---|
| 핵심 특징 | 타이머 기반 포트 상태 전이 | 제안·동의 기반 고속 전환 | VLAN별 독립 트리 계산 |
| 적용 기준 | 기존 장비와 단일 트리 호환 | 빠른 장애 전환이 필요한 망 | VLAN별 활성 경로 분산 |
| 주요 위험 | 장애 수렴 지연 | 비호환 장비에서 STP로 동작 | VLAN별 설정·루트 불일치 |
