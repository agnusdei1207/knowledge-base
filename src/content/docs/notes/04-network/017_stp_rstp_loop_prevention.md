---
sidebar:
  order: 17
  label: "017. 스패닝 트리 루프 방지 (STP•RSTP•PVST+)"
  badge:
    text: "기출 • 70%"
    variant: note
title: "스패닝 트리 루프 방지 (STP•RSTP•PVST+)"
date: "2026-08-06T23:27:50+09:00"
tags:
  - "notes-network"
weight: 17
extra:
  question_no: "017"
  source_status: "기출"
  source_history: "138회"
  priority: 70
  priority_note: "설계•문제대책형: 138회 STP Loop 방지"
---

## Ⅰ. 개요

<details><summary>용어 설명</summary>

- **스패닝 트리 프로토콜(Spanning Tree Protocol, STP)**: 중복 링크 일부를 비전달 상태로 두어 루프 없는 활성 트리를 만드는 프로토콜이다.
- **고속 스패닝 트리 프로토콜(Rapid Spanning Tree Protocol, RSTP)**: 제안•동의로 경로를 빠르게 전환하는 프로토콜이다.
- **VLAN별 스패닝 트리 플러스(Per-VLAN Spanning Tree Plus, PVST+)**: VLAN마다 독립 트리를 구성하는 프로토콜이다.
- **매체 접근 제어 주소 표(Media Access Control Address Table, MAC 표)**: 스위치가 주소별 출력 포트를 학습해 저장하는 전달 표이다.

</details>

- 정의/개념: **STP**와 **RSTP**는 중복 링크의 일부를 비전달 상태로 두어 루프 없는 활성 트리를 만든다.
- 배경/필요성: 중복 링크는 프레임 무한 순환과 **MAC 표** 변동을 일으킨다.

#### 한줄 요약

- 예비 다리는 남겨 두되 평소 한 길만 열어 차량이 원을 돌지 않게 하고 장애 때 예비길을 연다.

## Ⅱ. 특징

<details><summary>용어 설명</summary>

- **브리지 ID(Bridge Identifier)**: 우선순위와 MAC 주소로 루트 스위치를 선출하는 비교값이다.
- **경로 비용(Path Cost)**: 루트까지의 최적 포트를 고르는 누적 비용이다.

</details>

- **브리지 ID**가 가장 낮은 스위치를 루트로 선출한다.
- **경로 비용**으로 루트까지의 포트 역할을 결정한다.
- **RSTP**는 제안•동의로 경로를 빠르게 전환한다.

#### 한줄 요약

- 루트 브리지를 기준으로 비용이 가장 낮은 전달 경로만 열고 중복 경로는 차단 상태로 유지한다.

## Ⅲ. 구조 및 구성요소

<details><summary>용어 설명</summary>

- **루트 포트(Root Port)**: 비루트 스위치에서 루트까지 비용이 가장 낮은 포트이다.
- **지정 포트(Designated Port)**: 각 링크 구간에서 프레임을 전달하는 대표 포트이다.
- **대체 포트(Alternate Port)**: 루트 포트 장애 시 사용할 예비 포트이다.
- **브리지 프로토콜 데이터 단위(Bridge Protocol Data Unit, BPDU)**: 루트 식별자•경로 비용•포트 정보를 교환하는 제어 메시지이다.

</details>

```text
BPDU
└── 루트 브리지
    ├── 루트 포트
    ├── 지정 포트
    └── 대체 포트
```

선의 의미: BPDU는 루트 브리지와 경로 비용 정보를 제공하고, 루트 브리지 아래에는 활성 트리를 구성하는 루트 포트•지정 포트와 중복 경로를 보존하는 대체 포트가 놓이는 정적 스패닝 트리 구조를 뜻한다.

| 구성요소 | 책임 |
|:---|:---|
| 루트 브리지 | 활성 트리의 기준점 제공 |
| 루트 포트 | **루트 포트**가 비루트 장비의 최소 비용 경로 제공 |
| 지정 포트 | **지정 포트**가 링크 구간의 프레임 전달 담당 |
| 대체 포트 | **대체 포트**가 루트 포트 장애 시 예비 경로 제공 |
| BPDU | **BPDU**로 루트 식별자•경로 비용 교환 |

#### 한줄 요약

- 대표 길은 열고 중복 길은 닫아 예비로 둔다.

## Ⅳ. 흐름도

<details><summary>용어 설명</summary>

- **BPDU 비교**: 수신한 BPDU의 루트 식별자•경로 비용•포트 정보를 우선순위대로 대조하는 절차이다.
- **루트 브리지 선출**: 브리지 ID가 가장 낮은 스위치를 활성 트리의 기준점으로 고르는 절차이다.
- **루트 포트 선택**: 비루트 스위치에서 루트까지 누적 비용이 가장 낮은 포트를 고르는 절차이다.
- **지정 포트 선택**: 각 링크 구간에서 루트까지 비용이 가장 낮은 전달 포트를 고르는 절차이다.
- **대체 포트 대기**: 중복 경로 포트를 비전달 상태로 유지하여 루프를 차단하는 절차이다.

</details>

```text
BPDU 수신
    |
    v
1. BPDU 비교
    |
    v
2. 루트 브리지 선출
    |
    +-- 로컬이 루트 ------ 루트 포트 없음
    |
    +-- 로컬이 비루트 ---- 3. 루트 포트 선택
    |
    `-- 링크별 역할 계산
             |
             v
      4. 지정 포트 선택
             |
             v
      5. 대체 포트 대기
             |
             `-- 활성 트리 외 중복 경로 차단
```

### 동작 원리

1. BPDU 비교: 루트 식별자•비용•포트 정보를 비교한다.
2. 루트 브리지 선출: 가장 낮은 브리지 식별자를 선택한다.
3. 루트 포트 선택: 비루트 장비의 최소 비용 포트를 결정한다.
4. 지정 포트 선택: 링크별 최소 비용 전달 포트를 결정한다.
5. 대체 포트 대기: 중복 포트를 비전달 상태로 유지한다.

#### 한줄 요약

- 받은 BPDU의 루트•비용을 비교해 전달 포트와 대기 포트를 나눈다.

## Ⅴ. 종류 및 비교

<details><summary>용어 설명</summary>

- **가상 근거리 통신망(Virtual Local Area Network, VLAN)**: 하나의 스위치망을 논리적 브로드캐스트 영역으로 분리한 네트워크이다.

</details>

| 스패닝 트리 방식 | **STP** | **RSTP** | **PVST+** |
|:---|:---|:---|:---|
| 적용 기준 | 기존 장비•단일 트리 호환 | 빠른 장애 경로 전환 | **VLAN**별 활성 경로 분산 |
| 핵심 특징 | 타이머 기반 포트 상태 전이 | 제안•동의 기반 고속 수렴 | VLAN별 독립 트리 계산 |
| 한계 | 장애 후 수렴 지연 | 비호환 이웃에서는 STP 방식으로 전환 | VLAN별 설정•루트 불일치 |

> 요약: STP 호환, RSTP 고속 전환, PVST+ VLAN별 트리가 핵심이다.

#### 한줄 요약

- STP는 타이머로 기다리고 RSTP는 이웃과 합의하며 PVST+는 VLAN마다 트리를 만든다.

## Ⅵ. 실무 고려사항 및 대책

<details><summary>용어 설명</summary>

- **루트 가드(Root Guard)**: 더 우수한 외부 BPDU가 들어와도 지정 포트의 루트 변경을 막는 기능이다.
- **PortFast**: 단말 포트를 대기 없이 즉시 전달 상태로 전환하는 기능이다.
- **BPDU Guard**: 단말 포트에서 BPDU를 수신하면 포트를 차단하는 기능이다.
- **단방향 링크 탐지(Unidirectional Link Detection, UDLD)**: 광 링크 등의 단방향 통신 장애를 탐지해 루프 발생을 막는 기능이다.

</details>

| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 루트 브리지 임의 변경 | 우선순위•**루트 가드** 적용 | 트리 기준 안정화 |
| 단말 포트의 BPDU 유입 | **BPDU Guard**•**PortFast** 설정 | 비인가 루트 차단 |
| 단방향 링크 장애 | 루프 가드•**UDLD** 적용 | 차단 포트의 비정상 전달 전환 방지 |
| 대규모 2계층 영역 | VLAN•라우팅 경계로 분할 | 수렴 범위 축소 |

#### 한줄 요약

- 사용자 포트는 단말을 즉시 연결하되 BPDU가 들어오면 포트를 차단해 임의 스위치의 루프를 제한한다.

## Ⅶ. 결론

<details><summary>용어 설명</summary>

- **무루프 이중화(Loop-Free Redundancy)**: 평소 하나의 활성 경로만 전달하고 장애 때 대체 포트를 여는 구조이다.

</details>

- **무루프 이중화**에서 고속 수렴은 **RSTP**, VLAN별 경로 분산은 **PVST+**를 선택한다.

#### 한줄 요약

- 중복 링크는 남겨 두되 평소 한 경로만 전달하고 장애 때 대체 포트를 연다.
