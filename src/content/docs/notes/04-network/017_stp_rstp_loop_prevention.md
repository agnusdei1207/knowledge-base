...
- 배경/필요성: 가용성을 위해 중복 링크를 깔면 브로드캐스트 한 장이 링크를 돌며 복제돼 **스톰과 CAM 테이블 플래핑으로 대역폭과 스위칭 자원을 끝없이 갉아먹는 비용**으로 돌아왔으므로, 물리 이중화는 그대로 두고 BPDU 교환으로 잉여 링크만 논리적으로 차단해 트리를 만든 뒤 장애 시 그 링크를 되살리도록, 상시 유휴 대역폭을 대가로 루프 위험을 제거하는 제어 계층의 필요
