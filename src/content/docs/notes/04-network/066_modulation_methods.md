...
```text
[비트 스트림] ──► [심벌 매퍼 (QPSK/QAM)] ──► [RF 상향변환 & PA] ──► [무선 대역통과 채널]
                                                                        │
[복원 비트] ◄── [심벌 판정/디매퍼] ◄── [채널 등화 & 동기화] ◄── [RF LNA & 하향변환] ◄┘
```
...
```text
단말 RF 채널 측정 및 CQI 피드백 (CQI / EVM Feedback)
      │
      ▼
1. 최적 MCS 변조 차수 설정 (MCS Selection: QPSK -> 256QAM)
      │
      ▼
2. 송신 비트 I/Q 성상 매핑 & RF 변조 전송 (Symbol Mapping & Passband Tx)
      │
      ▼
3. 무선 채널 통과 (Fading, Noise & ISI Addition)
      │
      ▼
4. 수신 동기화, 파일럿 기반 채널 등화 (Carrier Sync & MMSE Equalization)
      │
      ▼
5. LLR 산출 및 심벌 비트 복조 판정 (Soft Decision Demodulation & LLR Output)
```

### 동작 원리

1. **채널 품질 보고**: 수신 단말이 무선 신호의 **SNR** 및 **EVM**을 정밀 측정하여 기지국으로 피드백한다.
2. **변조·부호율 설정**: 링크 제어기가 Target BLER 10%를 충족하는 최고 차수의 **MCS** 파라미터를 선정한다.
3. **변조 신호 전송**: 선택된 **QPSK/QAM** 성상도 좌표로 데이터를 변조하여 무선 공간으로 송출한다.
4. **왜곡 수신 신호 전달**: 수신 측은 multipath fading 및 잡음으로 성상점 좌표가 번진 수신 파형을 받는다.
5. **동기·등화·심벌 판정**: **동기화**와 **등화기**를 거쳐 왜곡을 보상한 후 **복조** 연산을 수행하여 원본 비트를 유일 복원한다.
...
| **잡음 및 왜곡 내성** | 잡음(진폭)에 극도로 취약 | 진폭 잡음 내성 뛰어남 | 뛰어난 Phase 내성 (낮은 SNR) | 잡음 및 비선형 왜곡에 민감 |
| **대역폭 효율성** | 낮음 ($2f_m$) | 매우 낮음 (넓은 대역 점유) | 중간 ($1 \sim 2$ bps/Hz) | 매우 높음 ($4 \sim 10$ bps/Hz) |
...
- 시스템 구축 시 채널의 **효율·강인성 절충** 요소를 면밀히 검증하여, 열악한 채널 및 제어 신호에는 **QPSK**, 고품질 무선 데이터 채널에는 **QAM**과 **AMC** 기술을 융합 연동하는 차세대 변조 네트워킹 구축 체계 적용.
