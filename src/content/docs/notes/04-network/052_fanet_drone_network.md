...
- 배경/필요성: 지상 인프라 단독 통신의 **비가시·원거리 군집 연결 단절**
...
block
    columns 1
    A["UAV 노드 | 센서 데이터 생성과 패킷 중계"]
    B["게이트웨이 UAV | FANET과 지상 백홀 연결"]
    C["애드혹 라우팅 | 이동·에너지로 멀티홉 경로 선택"]
    D["군집 제어 | 임무 역할·비행·데이터 우선순위 조정"]
    A --- B
    B --- C
    C --- D
```
...
sequenceDiagram
    ...
    participant 중계UAV
    participant 게이트웨이
    UAV노드->>라우팅제어: 1. 이웃 상태 교환
    라우팅제어->>라우팅제어: 2. 링크 수명 예측
    라우팅제어->>중계UAV: 3. 경로 설정
    UAV노드->>중계UAV: 4. 임무 데이터 전달
    중계UAV->>게이트웨이: 5. 멀티홉 전달
```

**동작 원리**

1. **이웃 상태 교환**: 위치·속도·링크 품질·에너지 공유
2. **링크 수명 예측**: 상대 이동으로 연결 유지 시간 추정
3. **경로 설정**: 수명·에너지 조건을 만족하는 중계 선택
4. **임무 데이터 전달**: 제어·안전 패킷을 우선 전송
5. **멀티홉 전달**: 중간 UAV가 게이트웨이까지 중계
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 현재 신호만의 경로 선택 | 속도·방향으로 링크 수명 예측 | 선제 경로 전환 |
| 중계 UAV 배터리 소진 | 잔여 에너지·임무 중요도 반영 | 군집 생존시간 연장 |
| 게이트웨이 단일 장애 | 복수 게이트웨이·임시 지상 경로 | 백홀 연속성 |
...
- **예상 링크 수명·에너지·임무 우선순위에 따른 FANET 경로**
