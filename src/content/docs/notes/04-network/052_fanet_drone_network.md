...
- **FANET**: 3차원으로 이동하는 UAV가 중계 경로를 자율 구성하는 멀티홉 애드혹 네트워크이다.
...
- **링크 수명**: 두 UAV가 이동하면서 직접 무선 연결을 유지할 것으로 예상되는 시간이다.
- **토폴로지**: UAV 노드와 노드 사이 통신 링크의 연결 구조이다.
...
- **중계 UAV**: 다른 UAV의 임무 데이터를 선택된 멀티홉 경로의 다음 노드로 전달하는 드론이다.
- **게이트웨이 UAV**: FANET의 공중 경로를 지상 관제소 또는 외부 백홀과 연결하는 드론이다.
...
- **이웃 상태**: 인접 UAV의 위치·속도·링크 품질·잔여 에너지를 묶은 경로 선택 정보이다.
- **멀티홉**: 송신 UAV의 데이터를 하나 이상의 중간 UAV가 차례로 중계하는 전달 방식이다.
...
- **MANET·VANET**: MANET은 일반 이동 단말의 임시망이고 VANET은 도로 이동 차량에 특화된 애드혹망이다.
- **FANET**: 공중에서 고속·3차원으로 이동하는 드론 군집에 특화된 애드혹망이다.
...
- **중계 부하**: 특정 UAV가 다른 노드의 패킷을 전달하느라 사용하는 통신·연산·배터리 자원이다.
- **백홀 연속성**: 게이트웨이 장애가 발생해도 FANET과 지상망 사이 전달 경로가 유지되는 성질이다.
...
- **FANET 중계 경로**: 예상 링크 수명과 잔여 에너지로 선택한 UAV들의 멀티홉 전달 경로이다.
