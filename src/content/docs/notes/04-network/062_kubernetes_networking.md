...
- 정책 객체만 만들어도 CNI 플러그인이 실행 규칙으로 바꾸지 않으면 패킷은 차단되지 않는다.
...
- **데이터 경로(Data Path)**: 프록시나 eBPF로 패킷의 분산•정책 규칙을 실행하는 경로이다.
...
- **외부 경로 설치**: 인그레스 컨트롤러가 호스트•경로•TLS 조건을 전달 규칙으로 만드는 단계이다.
...
2. **외부 경로 설치**: **외부 경로 설치** 단계에서 **프록시**가 전달 규칙을 생성한다.
...
- 객체 존재 여부뿐 아니라 준비된 파드까지 패킷이 도달하는지 확인해야 한다.
