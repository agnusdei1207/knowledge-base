...
- 배경/필요성: 전통적 호스트 포트 매핑(Port-Mapping) 방식의 한계로 인한 **포트 충돌 빈발, 파드 동적 확장 시 서비스 디스커버리 불가 및 컨테이너 간 보안 격리 부재**
...
- **Flannel** vs **Calico** vs **Cilium**: 단순 오버레이(Flannel), BGP/NetworkPolicy(Calico), eBPF 커널 가속/L7 보안(Cilium).
