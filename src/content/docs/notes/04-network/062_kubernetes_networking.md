...
|:---|:---|:---|:---|
| 서비스 켰는데 **사용자한테 503 에러 오지게 쏟아짐** | 파드 내부 DB 켜지기도 전에 서비스(VIP)가 냅다 트래픽부터 무지성 꽂아버림 | 컨테이너 완벽히 준비될 때까지 패킷 안 주는 **Readiness Probe 딜레이 빡세게 세팅** | 미준비 파드에 패킷 꽂혀서 발생하는 503 핑퐁 에러 차단 |
