...
```text
쿠버네티스 네트워킹 구조 (Kubernetes Networking Architecture)
├─ 외부 L7 진입: 인그레스 컨트롤러 (NGINX / Envoy / 게이트웨이 API)
├─ 서비스 탐색 계층
│  ├─ 서비스 객체 (클러스터IP / 노드포트 / 로드밸런서)
│  └─ 엔드포인트 분할 객체 (EndpointSlice)
└─ 파드 데이터 경로 및 보안
   ├─ 네트워크 보안 정책 (NetworkPolicy)
   └─ CNI 데이터 경로 (Cilium eBPF / Calico iptables / Flannel VXLAN)
```
...
1. 인그레스 규칙 변경 등록 (API 서버 알림)
        │
        ▼
2. 외부 경로 및 TLS 설정 반영 (인그레스 컨트롤러 갱신)
        │
        ▼
