...
- 정의/개념: 5G RAN의 **gNB 기능**을 **CU•DU•RU**와 표준 인터페이스로 분리한 구조이다.
- 배경/필요성: 일체형 장비의 **확장•다중 공급사 연동하기 어렵다**.
...
- **DU•RU 현장 배치**를 통한 스케줄링•물리 처리 시한 충족이 핵심이다.
- **F1•개방 프론트홀** 기반 공급사 조합과 엄격한 동기 요구가 핵심이다.
...
선의 의미: CU•DU•RU 사이의 표준 연결 경계를 나타낸 정적 구조다.
...
1. **F1 상위 계층 전달**: CU가 처리한 정책•상위 계층 데이터를 DU에 전달한다.
2. **프론트홀 신호 전달**: DU가 스케줄링한 신호•제어 정보를 RU에 전달한다.
3. **RU 측정 정보 전달**: RU가 수집한 무선 품질을 DU에 제공한다.
4. **DU 자원 상태 환류**: DU가 측정 결과와 자원 상태를 CU에 제공한다.
...
- 전송•동기가 처리 시한을 충족하면 **CU•DU•RU 분리**, 미충족 시 **일체형 유지**가 핵심이다.
