...
- 정의/개념: 신뢰 식별자를 위조해 경로•상대를 속이는 **신원 기만 공격**이다.
- 배경/필요성: ARP•IP•DNS는 기본 동작만으로 **출처 진위 검증이 부족하다**.
...
- **정상 위장**: 신뢰 주소•이름으로 악성 패킷 은폐이다.
- **계층 검증**: 바인딩•역경로•서명으로 진위를 판정한다.
- **상태 잔존**: 차단 후에도 오염 캐시•세션 지속이다.
...
1. **신뢰 기준 조회**: 바인딩•경로•서명 기준을 확보한다.
2. **종단 신원 인증**: 인증서로 실제 상대를 재확인한다.
3. **위조 출처 차단**: 불일치 패킷 폐기와 위치를 격리한다.
4. **오염 상태 복구**: 캐시•세션의 위조 상태를 제거한다.
...
- ARP는 **DAI**, IP는 **BCP 38•uRPF**, DNS는 **DNSSEC•캐시 폐기**가 핵심이다.
