...
date: "2026-07-25T19:47:00+09:00"
tags:
  - "notes-network"
weight: 47
extra:
...
- **정의/개념**: 엣지는 현장 처리, 포그는 계층형 지역 처리
- **배경/필요성**: 제어 기한별 연산 배치·백홀 부하 분산
...
- 엣지는 데이터 발생점에서 제어 지연을 줄인다.
- 포그는 여러 엣지의 상태·자원을 지역에서 집계한다.
- 계층이 높아질수록 처리 범위와 왕복 지연이 늘어난다.
- 구현 위치가 겹치므로 물리 배치보다 역할로 구분한다.
...
| 판단 기준 | 엣지 컴퓨팅 | 포그 컴퓨팅 |
|:---|:---|:---|
| 핵심 특징 | 발생점 인접 단일 현장 처리 | 다계층 지역 분산 처리 |
| 적용 기준 | 짧은 제어 기한·원시 데이터 | 다수 엣지 집계·공동 정책 |
| 주요 위험 | 제한된 자원·개별 관리 | 계층 간 동기화·장애 전파 |
