...
- **엣지•포그 컴퓨팅**: 데이터 발생점에서 즉시 처리하는 엣지와 다계층 지역 노드에서 집계•조정하는 포그를 결합한 분산 컴퓨팅 구조이다.

</details>

- 정의/개념: 데이터 발생점에서 처리하는 **엣지 컴퓨팅(Edge Computing)** 과 다계층 지역 노드에서 조정하는 **포그 컴퓨팅(Fog Computing) 구조**
...
- **엣지 컴퓨팅**: 센서•단말•게이트웨이처럼 데이터 발생 지점 가까이에서 저장•연산•제어하는 구조이다.
- **포그 컴퓨팅**: 엣지와 클라우드 사이의 여러 계층 노드가 지역 데이터와 자원을 집계•조정하는 구조이다.
