...
date: "2026-07-31T00:57:29+09:00"
tags:
  - "notes-network"
weight: 10
extra:
...
- **경로 재분배(Route Redistribution)**: 서로 다른 라우팅 프로토콜이나 경로 출처 사이에 경로 정보를 변환해 주입하는 기능
- **경로 태그·필터**: 경로 출처를 표시하는 값과 허용·차단 조건으로 재분배 범위를 통제하는 수단
- **비대칭 라우팅**: 한 연결의 왕복 패킷이 서로 다른 경로를 이용하는 상태
...
| 비대칭 경로가 상태 장비를 우회 | **왕복 경로·상태 정책** 동시 검증 | **연결 단절** 예방 |
