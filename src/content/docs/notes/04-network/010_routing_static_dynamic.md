...
- **라우팅**은 목적지 IP 주소에 도달할 다음 홉과 출력 인터페이스를 결정하는 네트워크 계층 기능이다.

</details>

- 정의/개념: 목적지 IP에 따라 다음 홉·출력 인터페이스를 정하는 **라우팅 기능**
- 배경/필요성: 직접 연결 정보만으로는 다중 네트워크의 **원격 목적지 도달 불가**
...
- **관리 거리**는 서로 다른 경로 출처가 같은 목적지 경로를 제시할 때 출처의 신뢰 우선순위를 비교하는 값이다.
- **메트릭**은 같은 라우팅 프로토콜 안에서 대역폭·지연·홉 수 등의 기준으로 경로 비용을 비교하는 값이다.
- **제어 평면과 데이터 평면**은 각각 경로를 교환·선택하고 선택한 경로로 실제 패킷을 전달한다.
...
- **RIB**는 정적 설정과 라우팅 프로토콜에서 받은 후보 경로 및 선택 경로를 보관하는 제어 평면 표이다.
- **FIB**는 선택된 경로를 실제 패킷 전달에서 빠르게 조회하도록 만든 데이터 평면 표이다.
- **ECMP**는 같은 목적지의 동일 비용 경로 여러 개를 FIB에 설치하여 트래픽을 분산한다.
...
- **최장 프리픽스 일치**는 목적지 주소와 가장 많은 앞 비트가 일치하는 FIB 경로를 선택하는 규칙이다.
- **다음 홉**은 선택한 경로에서 패킷을 넘길 인접 라우터이며 출력 인터페이스는 패킷이 나갈 장치 포트이다.
...
- **정적 라우팅**은 관리자가 경로를 직접 설정하므로 경로가 고정된 스텁 네트워크에 적합하다.
- **동적 라우팅**은 라우팅 프로토콜이 경로를 자동으로 교환하고 네트워크 변화에 따라 갱신한다.
- **수렴**은 네트워크 변화 뒤 라우터들이 일관된 최신 경로를 다시 갖게 되는 과정이다.
...
- **경로 추적**은 다음 홉의 도달성을 확인하여 정적 경로의 설치와 제거를 자동으로 바꾼다.
- **경로 재분배**는 서로 다른 라우팅 프로토콜이나 경로 출처 사이에 경로 정보를 변환하여 주입한다.
- **비대칭 라우팅**은 한 연결의 왕복 패킷이 서로 다른 경로를 사용하여 상태 기반 장비의 처리를 어긋나게 할 수 있다.
...
- **기본 경로**는 목적지에 대한 더 구체적인 경로가 없을 때 사용하는 최후의 경로이다.
