...
- 정의/개념: 이기종 패킷 네트워크 환경에서 라우팅 테이블(Routing Table)을 참조하여 목적지 **인터넷 프로토콜(Internet Protocol, IP)** 주소로 패킷을 전송하는 최적의 경로 제어 메커니즘인 **라우팅(Routing)**.
- 배경/필요성: 단일 이더넷 링크 범주를 벗어나 복잡한 토폴로지의 다중 네트워크 간 데이터 패킷을 정확하고 효율적으로 전달하기 위한 L3 경로 선택 표준 필요.

#### 한줄 요약

- L3 패킷의 최적 경로 선택 및 Next-Hop 스위칭을 위한 라우팅 메커니즘 구현.
...
- **관리 거리(Administrative Distance, AD)** 기반 라우팅 출처 간 신뢰 우선순위 결정 (예: Direct(0) > Static(1) > EBGP(20) > OSPF(110) > IBGP(200)).
- 동일 프로토콜 내 **메트릭(Metric)** 연산을 통한 최소 비용 경로 산출.
- 경로 수집/연산을 담당하는 **제어 평면(Control Plane)**과 고속 패킷 스위칭을 담당하는 **데이터 평면(Data Plane)**의 완벽한 기능 분리.

#### 한줄 요약

- Administrative Distance 신뢰도 산정, Metric 비용 연산 및 Control/Data Plane 분리 체계 구축.
...
| 구성요소 | 역할 및 세부 기능 | 비고 |
|:---|:---|:---|
| **관리 거리 (AD)** | 이종 프로토콜 간 우선순위 결정 (Static: 1, OSPF: 110, BGP: 20/200) | AD 값이 낮을수록 우선 선점 |
| **메트릭 (Metric)** | 동종 프로토콜 간 최적 경로 산출 (Hop Count, Cost 등) | 프로토콜마다 알고리즘 상이 |
...
| **ECMP (Load Balancing)** | 동일 Metric 경로에 대하 L3/L4 5-Tuple 기반 트래픽 분산 | 대역폭 활용성 극대화 |

#### 한줄 요약

- RIB 경로 집계, FIB 포워딩 테이블 설치 및 ECMP/LPM 최적 라우팅 알고리즘 적용.
...
1. **경로 수집 및 선택 (Control Plane)**: 관리자의 정적 입력과 동적 라우팅 프로토콜로부터 **후보 경로 수집** 후, **관리 거리(AD)**와 **메트릭(Metric)** 순으로 비교하여 최선의 경로를 RIB에 선정.
2. **패킷 포워딩 (Data Plane)**: RIB의 최적 경로를 **FIB 경로 설치**하여 하드웨어로 전달하고, 패킷 유입 시 **LPM 경로 조회**를 통해 인접 **다음 홉(Next Hop)**으로 포워딩.

#### 한줄 요약

- 후보 경로 수집, AD/Metric 기반 최선의 RIB/FIB 선별 및 LPM 라우팅 조회 프로세스 구동.
...
| 토폴로지 변화 대응 | 링크 장애 시 관리자가 수동 변경할 때까지 대응 불가 | 프로토콜에 의해 자동으로 장애 우회 및 **수렴(Convergence)** |
...
- 관리자 지정 정적 라우팅(Static)과 프로토콜 기반 자동 수렴 동적 라우팅(Dynamic)의 차별적 운용.
...
|:---|:---|:---|:---|
| 정적 라우팅 블랙홀 | 회선 장애 시에도 Static Route가 제거되지 않음 | IP SLA 연동 **경로 추적(Object Tracking)** 적용 | 장애 발생 시 정적 경로 자동 덤프 및 우회 |
...
- Object Tracking 정적 경로 연동, 단방향 경로 재분배 필터링 및 비대칭 라우팅 튜닝 체계 수립.
...
- 안정적이고 가용성 높은 엔터프라이즈 인프라 구성을 위해 **라우팅 방식 결정(Routing Strategy Selection)**에 따라 Stub 구간에는 **기본 경로(Default Route)**와 Static을, Core 구간에는 OSPF/BGP 동적 라우팅을 조화롭게 배치하는 설계 체계 준수 필수.

#### 한줄 요약

- 네트워크 스케일별 Static/Dynamic 라우팅 전략 선정 및 AD/Metric 제어 기반 라우팅 안정성 확보 체계 적용.
