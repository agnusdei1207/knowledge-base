...
- 정의/개념: SD-WAN 연결과 SSE 보안을 PoP에서 결합한 **클라우드 네트워크 구조**이다.
- 배경/필요성: 본사 우회 구조의 **지연•회선 병목•정책 편차**가 발생한다.
...
- **신원•단말•응용 맥락**: 경로•접근 정책 통합한다.
- **근접 PoP**: 보안 정책을 적용해 본사 우회 지연을 제거한다.
- **SD-WAN•SSE 단일 운영**: 연결•보안 정책 통합한다.
...
1. **품질 기반 PoP 접속**: 지연•손실•용량으로 거점을 선택한다.
2. **통합 정책 조회**: 경로•접근•자료 규칙을 결합한다.
3. **허용 응용 연결**: ZTNA•SWG•CASB 검사 후를 전달한다.
4. **세션 행위 전달**: 응용 이용 행위를 PoP에 지속을 제공한다.
5. **세션 위험 재평가 요청**: 맥락 변화에 따른 권한을 재판단한다.
...
- 연결•보안 동시 전환은 **SASE**, WAN 유지•보안 전환은 **SSE**을 선택한다.
