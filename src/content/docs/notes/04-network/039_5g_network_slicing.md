...
|:---|:---|:---|
| 자원 격리 수준 | 무선/전송/코어 전 구간 완전 자원 및 NF 논리/물리 격리 | 단순 트래픽 큐잉 및 패킷 굵기 기반 큐 간선 간섭 발생 |
...
| SLA 보장 능력 | 독립 NF 및 헌정 자원 제공으로 SLA 엄격 100% 보장 | 망 혼잡 발생 시 결합 자원 경합으로 SLA 파괴 가능성 존재 |
...
- 네트워크 슬라이싱은 E2E 전 구간에 대한 자원 완전 격리와 전용 가상망을 제공하여 100% SLA를 보장하는 아키텍처.
...
|:---|:---|:---|:---|
| 슬라이스 간 자원 침범 | 무선 구간에서 동적 자원 공유 시 자원 간섭 | 무선 PRB(Physical Resource Block) 하드 격리 적용 | 무선 인프라 부하 시에도 URLLC 지연 100% 보장 |
...
- 5G/B5G 사설 및 공용 통신망 구축 시 **E2E 네트워크 슬라이싱 오케스트레이터 구축**, **무선/전송/코어 자원 완전 격리 설계**, **S-NSSAI 식별 체계 확립 필수**.
