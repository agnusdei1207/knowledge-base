...
- 정의/개념: 공유 물리망의 기능•자원을 분리하는 **서비스별 종단 논리망 구성•격리 기술**이다.
- 배경/필요성: 단일 망의 **이질적 SLA 동시 보장 한계** 해소가 필요하다.
...
- 무선•전송•코어의 **종단 자원 조립**이 핵심이다.
- 서비스별 **기능•경로 차등 배치**가 핵심이다.
- 슬라이스 간 **자원•장애 격리**가 핵심이다.
...
3. **슬라이스 선택 정책 등록**: 가입•지역•가용성별 선택 기준을 설정한다.
...
- 서비스별 격리가 필요하면 **종단 슬라이스** 구성과 **SLA 자원** 배정이 핵심이다.
