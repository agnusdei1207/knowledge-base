...
- 배경/필요성: 단일 공용 통신망의 한계로 인한 **트래픽 폭증 시 상호 간섭 발생, 자율주행(URLLC 1ms)과 미디어(eMBB 20Gbps)의 독립적 SLA 보장 불가**를 겪으므로, RAN·전송·코어 전 구간의 자원을 서비스별로 갈라 논리적으로 격리한 가상망 계층을 물리망 위에 얹어 전용 물리망 구축 비용 없이 SLA만 분리 보장할 필요
