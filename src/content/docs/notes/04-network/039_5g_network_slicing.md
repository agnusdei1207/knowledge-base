...
block
    columns 1
    A["서비스·SLA 모델 | 지연·용량·가용성 목표 정의"]
    B["슬라이스 오케스트레이터 | 논리망 배치와 수명주기 관리"]
    C["슬라이스 선택 기능 | 가입자별 적합 슬라이스 선택"]
    D["무선 슬라이스 자원 | 대역·스케줄링 자원 배정"]
    E["전송 슬라이스 경로 | 격리된 전달 경로 제공"]
    A --- B
    B --- C
    C --- D
    D --- E
...
sequenceDiagram
    ...
    오케스트레이터->>오케스트레이터: 2. 슬라이스 설계
    오케스트레이터->>도메인자원: 3. 도메인 자원 배치
    도메인자원->>도메인자원: 4. 종단 슬라이스 연결
    오케스트레이터->>가입자: 5. 서비스 개통
```

**동작 원리**

1. **SLA 요구 입력**: 지연·용량·가용성 목표 전달
2. **슬라이스 설계**: 기능·격리·자원 템플릿 선택
3. **도메인 자원 배치**: 무선·전송·코어 자원 생성
4. **종단 슬라이스 연결**: 영역별 논리망을 하나로 연결
5. **서비스 개통**: 가입자 선택과 사용을 허용
