...
```text
전체 슬라이스 경계:
[서비스•SLA 모델] ----- [슬라이스 오케스트레이터] ----- [무선•전송•코어 자원]

내부 선택•세션 경계:
[NSSF] ----- [AMF] ----- [SMF] ----- [UPF]
```

선의 의미: 전체 슬라이스 경계는 서비스 목표와 오케스트레이션 및 종단 자원의 관계를 나타내고, 내부 선택•세션 경계는 NSSF•AMF•SMF•UPF가 구성하는 코어 기능 관계를 나타내어 서로 다른 추상화 수준을 분리한다.
