---
sidebar:
  order: 102
  label: "102. RDMA 원격 직접 메모리 접근 (RDMA)"
  badge:
    text: "기출 · 50%"
    variant: note
title: "RDMA 원격 직접 메모리 접근 (RDMA)"
date: "2026-07-25T00:45:00+09:00"
tags: ["notes-network"]
weight: 102
extra:
  question_no: "102"
  source_status: "기출"
  source_history: "138회"
  priority: 50
  priority_note: "설명·설계형: 138회 AI HPC RDMA 핵심"
---

## 미리 알고가기

- **원격 직접 메모리 접근(Remote Direct Memory Access, RDMA)**: 두 호스트의 등록 메모리 사이를 운영체제 복사와 원격 응용 개입을 줄여 직접 전송하는 기술이다.
- **RDMA 망 인터페이스 제어기(RDMA Network Interface Controller, RNIC)**: 등록 메모리 접근과 전송 작업을 하드웨어로 실행하는 네트워크 장치다.
- **메모리 등록**: 응용 버퍼를 RNIC가 접근할 수 있게 고정하고 로컬·원격 접근 키와 범위를 만드는 작업이다.
- **보호 도메인(Protection Domain, PD)**: 큐와 메모리 영역을 같은 권한 경계 안에 묶어 다른 자원의 접근을 격리하는 단위다.
- **큐 페어(Queue Pair, QP)**: 응용이 RNIC에 송신·수신 작업을 게시하는 두 작업 큐다.
- **완료 큐(Completion Queue, CQ)**: RNIC가 전송의 성공·실패와 완료된 작업 식별자를 기록하는 큐다.
- **제로 카피(Zero-Copy)**: 응용 자료를 커널과 중간 버퍼로 반복 복사하지 않고 전송하는 방식이다.
- **단방향 동작(One-Sided Operation)**: 원격 응용의 수신 호출 없이 RNIC가 원격 등록 메모리를 읽거나 쓰는 동작이다.
- **전송 제어 프로토콜(Transmission Control Protocol, TCP)**: 운영체제 네트워크 스택이 신뢰성 있는 바이트 흐름을 제공하는 범용 전송 프로토콜이다.
- **중앙처리장치(Central Processing Unit, CPU)**: 명령을 실행하는 처리 장치이며 RDMA는 자료 복사와 통신 처리에 드는 CPU 개입을 줄인다.

## Ⅰ. 개요

- **정의/개념**: RNIC가 원격 등록 메모리에 직접 전송하는 기술
- **배경/필요성**: 고속 분산 처리의 복사·커널·CPU 부하 감소

### 쉽게 이해하기 (학습용)

- 응용 자료를 운영체제 통로로 여러 번 복사하지 않고 네트워크 장치가 상대 서버의 허용된 메모리로 직접 옮긴다.

## Ⅱ. 특징

- 제로 카피·커널 우회로 지연·CPU 부하 감소한다.
- 단방향 동작은 원격 응용 호출을 생략한다.
- 성능 이득과 메모리·키 수명 책임이 상충한다.
- 잘못된 키·범위는 원격 메모리 손상을 유발한다.

### 쉽게 이해하기 (학습용)

- 빠른 통로를 얻는 대신 응용이 어느 메모리를 언제까지 노출하고 완료 뒤 재사용할지를 직접 관리해야 한다.

## Ⅲ. 아키텍처 및 구성요소

```mermaid
flowchart LR
    A["응용 처리기"] -->|작업 요청| B["큐 페어"]
    B -->|전송 명령| C["RNIC"]
    D["보호 도메인"] -->|자원 권한 경계| B
    D -->|접근 키·범위| E["등록 메모리"]
    C <-->|직접 읽기·쓰기| E
    C -->|성공·실패| F["완료 큐"]
```

| 설계 요소 | 설명 |
|:---|:---|
| 응용 처리기 | 버퍼·작업·완료 수명주기를 관리 |
| 큐 페어 | 송수신 작업 요청을 RNIC에 게시 |
| RNIC | 등록 메모리 간 전송을 실행 |
| 보호 도메인 | 큐·메모리 자원의 접근을 격리 |
| 등록 메모리 | RNIC 접근 범위와 키를 제공 |
| 완료 큐 | 작업 완료·실패 상태를 전달 |

> 요약: 보호 도메인의 큐·메모리를 RNIC가 직접 연결

### 쉽게 이해하기 (학습용)

- 응용이 허용한 메모리와 작업을 같은 권한 영역에 묶으면 RNIC가 직접 전송하고 완료 큐로 끝난 시점을 알려 준다.

## Ⅳ. 원리 및 절차 흐름도

```mermaid
sequenceDiagram
    participant 지역응용
    participant 지역RNIC
    participant 원격응용
    participant 원격RNIC
    participant 원격메모리
    지역응용->>지역RNIC: 지역 자원 준비
    원격응용->>원격RNIC: 원격 자원 준비
    지역응용-->>원격응용: QP·키·주소 교환
    지역응용->>지역RNIC: 작업 요청 게시
    지역RNIC->>원격RNIC: RDMA 동작 전송
    원격RNIC->>원격메모리: 직접 읽기·쓰기
    원격RNIC-->>지역RNIC: 완료 응답
    지역RNIC-->>지역응용: 완료 상태 전달
```

| 절차 | 설명 |
|:---|:---|
| 지역 자원 준비 | PD·QP·CQ와 등록 메모리 생성 |
| 원격 자원 준비 | 원격 버퍼 범위·접근 키 생성 |
| QP·키·주소 교환 | 연결·원격 메모리 정보를 전달 |
| 작업 요청 게시 | 전송 동작·주소·길이를 큐에 등록 |
| RDMA 동작 전송 | 지역 RNIC가 원격 RNIC에 요청 |
| 직접 읽기·쓰기 | RNIC가 원격 메모리에 전송 |
| 완료 응답 | 원격 처리 결과를 지역 RNIC에 반환 |
| 완료 상태 전달 | 성공·실패 뒤 버퍼를 재사용 |

> 요약: 메모리·키 교환 후 작업 게시·완료 확인

### 쉽게 이해하기 (학습용)

- 양쪽이 허용할 메모리 주소와 키를 나눈 뒤 전송 작업을 큐에 넣고 완료 통지를 받은 후에만 버퍼를 다시 쓴다.

## Ⅴ. 종류 및 비교

| 판단 기준 | RDMA | TCP 소켓 | 공유 메모리 |
|:---|:---|:---|:---|
| 핵심 특징 | 원격 등록 메모리를 RNIC가 직접 전송 | 커널 네트워크 스택으로 바이트 전달 | 한 호스트의 메모리를 처리기끼리 공유 |
| 적용 기준 | 고속·저지연 서버 간 대량 통신 | 범용 호환·복잡한 망·쉬운 운영 | 같은 호스트의 처리기 간 통신 |
| 주요 위험 | 키·버퍼·완료 수명 오류 | 복사·문맥 전환·CPU 부하 | 동기화 오류·호스트 경계 제한 |

> 요약: 호스트 경계·지연·메모리 책임으로 선택

### 쉽게 이해하기 (학습용)

- 서버 간 최고 성능은 RDMA, 범용 통신은 TCP, 같은 서버 안의 빠른 교환은 공유 메모리가 알맞다.

## Ⅵ. 실무 사례

1. 분산 학습 서버의 기울기 버퍼를 RDMA 전송
2. 저장 서버가 등록 자료 블록을 직접 읽기 제공

### 쉽게 이해하기 (학습용)

- 학습 서버들은 반복해서 큰 기울기 배열을 교환하므로 미리 등록한 버퍼 사이를 RNIC로 직접 전송한다.
- 저장 서버는 허용한 자료 블록만 등록하고 읽기 키를 제공해 계산 서버가 복사 없이 가져가게 한다.

## Ⅶ. 결론

- 지연 이득·메모리 범위·키 수명으로 RDMA 선택

### 쉽게 이해하기 (학습용)

- RDMA는 빠른 장비만으로 완성되지 않으며 응용이 메모리 권한과 완료 시점을 안전하게 관리할 수 있을 때 사용해야 한다.
