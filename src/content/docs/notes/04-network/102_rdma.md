...
- 배경/필요성: 반복 복사와 커널 처리로 인한 **CPU 부하**가 발생한다.
...
가지의 의미: 작업•전송•메모리 보호•완료 책임을 분리한 정적 구조다.
...
1. **전송 자원 등록**: PD•QP•CQ와 메모리 키를 생성한다.
2. **QP•키 연결 설정**: 원격 주소•권한•연결을 확인한다.
3. **작업 요청 게시**: 전송 동작•주소•길이를 큐에 등록한다.
4. **RNIC 간 전송**: 커널 복사 없이 자료를 전달한다.
5. **직접 메모리 배치**: 원격 RNIC가 등록 버퍼에 기록한다.
#### 한줄 요약
...
- 메모리 수명 통제 가능 시 **RDMA**, 범용 호환 우선 시 **TCP**을 선택한다.
