...
- 배경/필요성: 반복 복사와 커널 처리로 **CPU** 대상 **CPU 부하** 발생한다.
...
| 구성요소 | 책임 |
|:---|:---|
| 응용 처리기 | **응용 처리기** 버퍼•작업•완료 수명주기 관리 |
| 큐 페어 | **큐 페어** 송수신 작업 요청을 **RNIC** 대상 게시 |
| RNIC | RNIC가 등록 메모리 간 전송 실행 |
| 보호•등록 메모리 | **보호•등록 메모리** 권한 경계•접근 범위•키 제공 |
| 완료 큐 | **완료 큐** 작업 완료•실패 상태 전달 |
...
1. 전송 자원 등록: **보호 도메인**•QP•CQ와 메모리 키를 만드는 **전송 자원 등록** 수행한다.
2. QP•키 연결 설정: 원격 주소•권한•연결의 **QP•키 연결 설정** 수행한다.
3. 작업 요청 게시: 전송 동작•주소•길이를 큐에 넣는 **작업 요청 게시** 활용 수행한다.
4. RNIC 간 전송: 커널 복사 없이 **RNIC 간 전송** 기반 자료를 전달한다.
5. 직접 메모리 배치: 원격 RNIC가 등록 버퍼에 **직접 메모리 배치** 활용 수행한다.
#### 한줄 요약
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| **원격 키** 과도한 권한 | **보호 도메인**•최소 범위 등록 | 메모리 침범 방지 |
| 완료 전 **버퍼 재사용** | **CQ** 기반 수명주기 통제 | 자료 훼손 방지 |
| 구현 간 동작 불일치 | **IETF** **RFC 5040** 준수 시험과 **RFC** 확인 | 프로토콜 상호운용 확보 |
...
- 메모리 수명 통제가 가능한 서버 간 고속 전송은 **RDMA**, 범용 호환은 **TCP**, 같은 호스트는 **공유 메모리** 활용 택하는 **데이터 전달 방식 선택** 필요하다.
