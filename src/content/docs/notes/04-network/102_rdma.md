...
- **중앙처리장치(Central Processing Unit, CPU)**: 명령 실행과 연산을 담당하는 범용 처리장치이다.
- **직접 전송 기술**: 커널 복사 경로를 줄이고 RNIC가 등록 메모리 사이에서 자료를 이동하는 기술이다.
- **CPU 부하**: 반복 복사•시스템 호출•문맥 전환이 범용 처리장치의 연산 시간을 소비하는 부담이다.

</details>

- 정의/개념: RNIC가 호스트 간 등록 메모리를 잇는 **직접 전송 기술**이다.
- 배경/필요성: 반복 복사와 커널 처리로 **CPU**에 **CPU 부하**가 발생한다.
...
- **커널 우회(Kernel Bypass)**: 커널 데이터 경로 처리를 줄이는 전송 방식이다.
- **등록 메모리**: RNIC 접근을 허용한 주소 범위와 수명의 버퍼이다.
- **접근 키**: RNIC의 등록 메모리 접근 권한을 검증하는 값이다.

</details>

- **제로 카피**•**커널 우회** 기반 저지연 전송이 핵심이다.
- 원격 응용 호출 없는 **단방향 동작**이 핵심이다.
- **등록 메모리**•**접근 키** 기반 권한 제어가 핵심이다.
...
- **완료 큐(Completion Queue, CQ)**: 전송 작업의 완료•실패 상태를 응용에 전달하는 큐이다.
- **응용 처리기**: 버퍼•작업•완료의 수명주기를 관리하는 구성요소이다.
...
가지의 의미: 작업•전송•메모리 보호•완료 책임을 분리한 정적 구조이다.

| 구성요소 | 책임 |
|:---|:---|
| 응용 처리기 | **응용 처리기**가 버퍼•작업•완료 수명주기 관리 |
| 큐 페어 | **큐 페어**가 송수신 작업 요청을 **RNIC**에 게시 |
| RNIC | RNIC가 등록 메모리 간 전송 실행 |
| 보호•등록 메모리 | **보호•등록 메모리**가 권한 경계•접근 범위•키 제공 |
| 완료 큐 | **완료 큐**가 작업 완료•실패 상태 전달 |
...
- **전송 자원 등록**: 보호 도메인•큐•완료 큐와 메모리 키를 생성하는 단계이다.
- **QP•키 연결 설정**: 원격 주소•접근 권한•큐 연결 정보를 양 종단에 설정하는 단계이다.
- **작업 요청 게시**: 전송 동작•주소•길이를 큐 페어에 등록하는 단계이다.
- **RNIC 간 전송**: 커널 복사 없이 두 RNIC가 등록 메모리 자료를 전달하는 단계이다.
...
1. **전송 자원 등록**: **보호 도메인**•QP•CQ와 메모리 키를 만드는 **전송 자원 등록**을 수행한다.
2. **QP•키 연결 설정**: 원격 주소•권한•연결의 **QP•키 연결 설정**을 수행한다.
3. **작업 요청 게시**: 전송 동작•주소•길이를 큐에 넣는 **작업 요청 게시**를 수행한다.
4. **RNIC 간 전송**: 커널 복사 없이 **RNIC 간 전송**으로 자료를 전달한다.
5. **직접 메모리 배치**: 원격 RNIC가 등록 버퍼에 **직접 메모리 배치**를 수행한다.
#### 한줄 요약
...
- **국제인터넷표준화기구(Internet Engineering Task Force, IETF)**: 인터넷 기술 표준을 개발하는 국제 공동체이다.
- **의견 요청 문서(Request for Comments, RFC)**: 인터넷 기술 규격을 공개하는 문서 체계이다.
- **RFC 5040**: RDMA 프로토콜 동작을 규정한 IETF 표준이다.

</details>

| 문제 | 대책 | 효과 |
|:---|:---|:---|
| **원격 키**의 과도한 권한 | **보호 도메인**•최소 범위 등록 | 메모리 침범 방지 |
| 완료 전 **버퍼 재사용** | **CQ** 기반 수명주기 통제 | 자료 훼손 방지 |
| 구현 간 동작 불일치 | **IETF**의 **RFC 5040** 준수 시험과 **RFC** 확인 | 프로토콜 상호운용 확보 |
...
- 메모리 수명 통제가 가능한 서버 간 고속 전송은 **RDMA**, 범용 호환은 **TCP**, 같은 호스트는 **공유 메모리**를 택하는 **데이터 전달 방식 선택**이 필요하다.
