...
- **OCB 모드 기반 접속 지연 0ms**: BSS 가입, 스캔, 인증, 4-Way Handshake 과정을 전면 생략하여 채널 감지 즉시 무선 프레임을 발사한다.
...
1. **차량 안전 메시지 생성**: 차량 CAN 버스로부터 속도, 좌표, 조향각을 수집하여 100ms 주기로 BSM(Basic Safety Message)을 생성한다.
2. **보안 서명 결합**: IEEE 1609.2 모듈이 가명 개인키로 ECDSA 전자서명을 생성하여 메시지에 부착한다.
3. **WSMP 캡슐화**: IP 헤더 오버헤드를 줄이기 위해 IEEE 1609.3 WSMP 헤더로 패킷을 캡슐화한다.
4. **CCH 제어 채널 할당**: IEEE 1609.4 동기화 신호에 맞춰 안전 메시지를 50ms CCH(Control Channel) 타임슬롯으로 할당한다.
5. **OCB 무선 발사 및 수신 검증**: 802.11p MAC이 CSMA/CA로 채널을 감지한 후 BSS 가입 없이 즉시 무선으로 발사하고 수신측은 서명 및 최신성을 검증한다.
...
| 무선 전송 지연 | 수ms 이내 (접속 지연 0ms) | 수초 이상 (AP 접속 및 4-Way Handshake 지연) |
...
- **안전 메시지 기한(Safety Message Latency Deadline)**: 차량 충돌 방지를 위해 BSM 메시지가 10ms 이내에 수신·검증·제어기 전달을 완료해야 하는 유효 시간 한도이다.

</details>

- C-ITS 도로망 인프라 구축 시 **IEEE 802.11p OCB 모드 운용**, **IEEE 1609 프로토콜 스택 준수**, **WSMP 기반 10ms 이내 안전 메시지 전송 체계 구축 필수**.
