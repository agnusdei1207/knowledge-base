...
date: "2026-07-25T19:51:00+09:00"
tags:
  - "notes-network"
weight: 51
extra:
...
- **정의/개념**: OCB 무선과 1609 서비스를 결합한 차량 통신
- **배경/필요성**: 고속 이동 중 안전 메시지의 직접 교환
...
- OCB로 가입 절차 없이 프레임을 교환한다.
- 802.11p 무선과 1609 서비스를 계층화한다.
- 1609 계층이 보안·네트워킹·채널을 운용한다.
- 경쟁 기반 접속은 고밀도에서 충돌과 지연을 늘린다.
...
```mermaid
sequenceDiagram
    participant S as 송신 OBU
    participant M as WAVE 계층
    participant R as 수신 OBU·RSU
    S->>M: 메시지 서명
    M->>M: 서비스·채널 선택
    M->>R: OCB 전송
    R->>R: 서명·최신성 검증
...
| 절차 | 설명 |
|:---|:---|
| 메시지 서명 | 1609.2 인증서로 발신자 서명 |
| 서비스·채널 선택 | 1609.3·1609.4가 CCH·SCH 선택 |
| OCB 전송 | 연결 절차 없이 프레임을 송신함 |
| 서명·최신성 검증 | 인증서·생성 시각 확인 후 수용 |

> 요약: 서비스 지정 후 OCB 전송 및 검증 수행
...
| 판단 기준 | WAVE OCB | 인프라 Wi-Fi |
|:---|:---|:---|
| 핵심 특징 | 연결 없는 직접 프레임 교환 | AP 연결 후 데이터 교환 |
| 적용 기준 | 고속 이동 안전 메시지 | 지속 연결 일반 데이터 |
| 주요 위험 | 고밀도 채널 경쟁·충돌 | 연결 설정·로밍 지연 |
...
1. 혼잡 교차로는 차량 밀도별 충돌·지연 검증
2. RSU는 CCH·SCH 시각 동기와 인증서 갱신

### 쉽게 이해하기 (학습용)

- 차량이 몰리면 동시에 보낸 프레임이 충돌해 안전 메시지가 늦거나 사라질 수 있다
- RSU와 차량의 채널 시간이 어긋나면 서로 다른 채널을 들어 메시지를 놓친다

## Ⅶ. 결론

- 차량 밀도·WAVE 장비 호환성으로 적용 결정

### 쉽게 이해하기 (학습용)

- 기존 WAVE 장비와 호환되고 혼잡한 도로에서도 안전 메시지가 기한 안에 도착할 때 적용한다.
