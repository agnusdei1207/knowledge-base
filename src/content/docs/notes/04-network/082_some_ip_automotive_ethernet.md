...
- **정의/개념**: 차량 IP망에서 서비스를 호출·구독하는 규약
- **배경/필요성**: SDV 기능을 위치 대신 서비스로 유연 연결
...
- 서비스 ID·버전이 기능 계약을 식별한다.
- 요청·응답과 이벤트 구독을 함께 지원한다.
- 동적 재발견이 제공자 재시작 뒤 연결을 복구한다.
- 발견 위조는 잘못된 제공자 연결을 유발한다.
...
| 판단 기준 | 요청·응답 | 무응답 요청 | 이벤트 통지 |
|:---|:---|:---|:---|
| 핵심 특징 | 요청마다 처리 결과 반환 | 결과 메시지 없는 단방향 호출 | 구독자에게 상태 변화를 발행 |
| 적용 기준 | 조회·실행 결과가 필요 | 결과보다 전달 지연 최소화 | 상태가 바뀔 때 다수에게 통지 |
| 주요 위험 | 응답 지연·세션 누락 | 처리 성공 확인 곤란 | 구독 폭증·오래된 상태 |
...
1. 계기판이 주행 속도 이벤트 그룹을 구독
2. 제동 서비스에 E2E 순서·검사값 적용

### 쉽게 이해하기 (학습용)

- 계기판은 속도 제어기의 주소를 고정하지 않고 속도 이벤트를 제공하는 서비스를 찾아 지속적으로 받는다.
- 제동 명령은 순서 번호와 검사값을 확인해 오래되거나 손상된 메시지를 제어에 사용하지 않는다.

## Ⅶ. 결론

- 서비스 계약·버전·TTL로 동적 연결 통제
