...
- 정의/개념: 토큰 판정으로 속도와 버스트를 제한하는 **트래픽 셰이핑(Traffic Shaping)·트래픽 폴리싱(Traffic Policing) 제어 기법**
...
4. **초과 패킷 집행**: 즉시 폐기하거나 **차등 서비스 코드 포인트(Differentiated Services Code Point, DSCP)** 재표시
...
- **트래픽 폴리싱**: 약정 한도를 넘은 패킷을 즉시 폐기하거나 낮은 차등 서비스 코드 포인트로 재표시해 입구 계약을 집행한다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 셰이퍼 버스트가 경계 폴리서 초과 | **양단 약정 정보율(Committed Information Rate, CIR)·버킷 크기** 정렬 | 경계 폐기 감소 |
