...
- 정의/개념: 토큰 판정으로 속도와 버스트를 제한하는 **셰이핑•폴리싱 제어 기법**이다.
- 배경/필요성: 버스트의 **계약 위반•하위 회선 혼잡**이 발생한다.
...
- **평균 속도**: 토큰 생성률로 장기 전송률을 제한한다.
- **버스트 허용**: 버킷 크기로 순간 전송량을 결정한다.
- **초과 처리**: 대기•폐기•재표시 정책으로 집행이다.
...
1. **정책•패킷 크기 전달**: 가입자 정책과 필요 토큰을 계산한다.
2. **토큰 적합 판정**: 잔여 토큰과 패킷 크기를 비교한다.
3. **초과 패킷 큐 저장**: 토큰 생성까지 버퍼에서 대기한다.
4. **초과 패킷 집행**: 즉시 폐기하거나 **DSCP**을 재표시한다.
...
- 초과분 대기가 가능하면 **셰이핑**, 계약 한도 즉시 집행은 **폴리싱**이 핵심이다.
