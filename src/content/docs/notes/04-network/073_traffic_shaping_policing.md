...
date: "2026-07-25T12:28:00+09:00"
tags:
  - "notes-network"
weight: 73
extra:
...
- 정의: 토큰 판정으로 초과 트래픽 속도를 제어한다
- 배경: 하위 회선·계약 한도 초과 손실을 막는다
...
- 토큰 생성률은 장기 전송률을 제한해 혼잡을 줄인다
- 큰 버킷은 버스트를 허용하나 큐 지연을 늘린다
...
1. 지사 출구를 회선 CIR에 맞춰 셰이핑한다
2. 계약 PIR 초과 패킷을 낮은 등급으로 재표시한다

### 쉽게 이해하기 (학습용)

- 지사 라우터가 패킷을 잠시 기다렸다 보내 사업자 폴리서의 폐기를 줄인다
- 최대 계약 속도를 넘은 패킷은 낮은 등급으로 바꿔 우선 전송 대상에서 제외한다

## Ⅶ. 결론

- 지연 허용·손실 정책으로 셰이핑·폴리싱을 고른다
