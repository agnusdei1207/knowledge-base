...
- **수신 윈도(Receive Window, rwnd)**: 수신 측이 버퍼 여유를 기준으로 지금 더 받을 수 있다고 광고하는 바이트 범위
- **혼잡 윈도(Congestion Window, cwnd)**: 송신 측이 네트워크 혼잡 상태를 기준으로 확인 없이 보낼 수 있게 제한한 바이트 범위
...
- **정의/개념**: rwnd로 미확인 전송량을 제한하는 흐름 제어
- **배경/필요성**: 빠른 송신자의 느린 수신 버퍼 초과 방지
...
- 수신 측은 ACK마다 현재 rwnd를 광고한다.
- 송신 측 윈도는 누적 ACK에 따라 앞으로 이동한다.
- 미확인 전송량은 rwnd·cwnd 중 작은 값으로 제한된다.
- 제로 윈도 탐사가 갱신 유실을 복구한다.
...
| 판단 기준 | 슬라이딩 윈도 | 정지-대기 |
|:---|:---|:---|
| 핵심 특징 | 윈도 범위에서 여러 데이터를 연속 전송 | 매 데이터의 ACK 후 다음 전송 |
| 적용 기준 | RTT 중에도 이어지는 연속 전송 | 단순성이 우선인 짧은 전송 |
| 주요 위험 | 윈도·버퍼 상태 관리 복잡 | RTT마다 송신이 멈춰 대역폭 낭비 |
