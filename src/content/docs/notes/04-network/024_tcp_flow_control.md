...
- 정의/개념: **TCP 흐름 제어** — 수신자가 광고한 윈도 `rwnd`에 맞춰 송신자의 미확인 데이터량을 제한하여 수신 버퍼 초과를 막는 **종단 간 제어 방식**이다.
- 배경/필요성: 빠른 송신 속도로 느린 수신 측의 **버퍼 오버플로**가 발생한다.
...
- ACK의 **현재 rwnd•누적 수신 번호 광고**가 핵심이다.
- 누적 ACK의 **송신 윈도 전진**이 핵심이다.
- 제로 윈도 탐사의 **윈도 갱신 유실 복구**가 핵심이다.
...
- 수신 병목은 **버퍼•소비율 개선**, 제로 윈도는 **윈도 탐사**를 유지한다.
