...
- **O-RAN**: RAN 기능을 분리하고 개방 인터페이스로 다중 공급자 장비를 조합하는 무선접속망 구조이다.
...
- **개방형 프론트홀**: O-DU와 O-RU 사이의 제어·사용자·동기 정보를 전달하는 개방 인터페이스이다.
- **xApp·rApp**: xApp은 준실시간 무선 제어를 수행하고 rApp은 비실시간 정책·분석을 수행하는 RIC 응용이다.
...
- **O-RU·O-DU·O-CU**: O-RU는 무선 송수신, O-DU는 시간 민감 물리·매체접근제어, O-CU는 상위 프로토콜과 이동성을 맡는다.
- **RIC**: 관측값과 정책을 바탕으로 무선 자원을 제어하는 지능형 제어기이다.
...
- **A1 인터페이스**: 비실시간 RIC가 준실시간 RIC에 정책·모델을 전달하는 O-RAN 참조점이다.
- **E2 인터페이스**: 준실시간 RIC와 무선 노드 사이에서 상태·구독·제어 명령을 전달하는 참조점이다.
...
- **개방형 RAN**: 기능 분리와 표준 인터페이스로 공급자별 장비를 조합하는 구현 방식이다.
- **폐쇄형 RAN**: 단일 공급자가 기지국 기능과 인터페이스를 통합 제공하는 구현 방식이다.
