...
date: "2026-07-31T11:05:50+09:00"
tags: ["notes-network"]
weight: 88
extra:
  question_no: "088"
...
- 정의/개념: 신뢰 수준이 다른 네트워크를 **격리하고 승인된 통로만 허용**해 침해 확산을 제한하는 **보안 통제**
...
- **격리 강도**: 공유 자원·우회 경로로 실제 분리 수준 결정
- **기본 차단**: 업무에 필요한 통신만 예외 승인
- **관문 통제**: 자료 전송과 관리자 세션을 별도 검사
...
sequenceDiagram
    ...
    participant 기록체계
    사용자->>통제관문: 교차망 통신 요청
    통제관문->>정책엔진: 1. 접근 맥락 검증
    정책엔진-->>통제관문: 2. 최소 흐름 판정
    통제관문->>대상영역: 3. 자료·세션 검사
    통제관문->>대상영역: 4. 승인 통신 중계
...
- **1. 접근 맥락 검증**: 사용자·단말·권한·업무 대조
- **2. 최소 흐름 판정**: 필요한 주소·포트·시간만 허용
- **3. 자료·세션 검사**: 악성 내용·관리 명령 확인
- **4. 승인 통신 중계**: 직접 연결 없이 관문이 대행
...
- 최고 격리는 **물리 분리**, 변화·효율은 **논리 분리**, 중요도 혼재는 **혼합**
