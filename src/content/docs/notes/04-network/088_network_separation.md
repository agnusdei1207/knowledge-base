---
sidebar:
  order: 88
  label: "088. 망분리 - 물리적·논리적 (Network Separation)"
  badge:
    text: "기출 · 50%"
    variant: note
title: "망분리 - 물리적·논리적 (Network Separation)"
date: "2026-07-25T00:45:00+09:00"
tags: ["notes-network"]
weight: 88
extra:
  question_no: "088"
  source_status: "기출"
  source_history: "125회"
  priority: 50
  priority_note: "비교·보안형: 125회 물리·논리 망분리"
---

## 미리 알고가기

- **망분리(Network Separation)**: 업무 중요도와 신뢰 수준이 다른 네트워크를 분리하고 승인된 통로만 허용해 침해 확산을 제한하는 통제다.
- **물리적 망분리**: 단말·스위치·회선을 별도로 구성해 두 영역이 물리 자원을 공유하지 않게 하는 방식이다.
- **논리적 망분리**: 가상화와 네트워크 정책으로 공유 장비 안의 통신 영역과 경로를 분리하는 방식이다.
- **비무장지대(Demilitarized Zone, DMZ)**: 외부 공개 서버를 내부망과 분리해 직접 연결을 줄이는 중간 보안 영역이다.
- **가상 근거리망(Virtual LAN, VLAN)**: 하나의 스위치에서 방송 영역을 논리적으로 나누는 기능이다.
- **가상 라우팅 전달(Virtual Routing and Forwarding, VRF)**: 하나의 라우터에 독립된 라우팅 표를 여러 개 두어 경로를 분리하는 기능이다.
- **콘텐츠 무해화(Content Disarm and Reconstruction, CDR)**: 문서에서 실행 코드 등 위험 요소를 제거하고 안전한 내용으로 다시 만드는 기술이다.
- **특권 접근관리(Privileged Access Management, PAM)**: 관리자 계정 권한, 접속 승인, 세션과 명령 기록을 통제하는 체계다.

## Ⅰ. 개요

- **정의/개념**: 신뢰 수준별 망을 격리해 승인 흐름만 허용하는 통제
- **배경/필요성**: 영역 간 침해 확산·정보 유출 방지

### 쉽게 이해하기 (학습용)

- 중요한 업무망을 별도 방으로 나누고 파일과 관리자 접속은 검사받는 문으로만 오가게 한다.

## Ⅱ. 특징

- 분리 강도는 공유 자원·우회 경로에 좌우한다.
- 기본 차단 후 업무에 필요한 흐름만 예외 허용한다.
- 강한 격리는 비용·업무 불편과 상충한다.
- 설정 오류·무선·관리망이 우회 통로로 작용한다.

### 쉽게 이해하기 (학습용)

- 선을 나누는 것만으로 끝나지 않고 파일 반입과 원격 관리처럼 실제로 남겨 둔 통로를 더 엄격히 관리해야 한다.

## Ⅲ. 아키텍처 및 구성요소

```mermaid
flowchart LR
    A["외부·인터넷 영역"] -->|공개 서비스| B["비무장지대"]
    B -->|허용 세션| C["업무 영역"]
    C -->|승인 자료| D["핵심 영역"]
    A -->|반입 자료| E["자료 전송 관문"]
    E -->|검사·무해화| D
    G["관리자"] -->|관리 요청| F["관리 접속 관문"]
    F -->|승인·세션 기록| C
    F -->|승인·세션 기록| D
```

| 설계 요소 | 설명 |
|:---|:---|
| 외부·인터넷 영역 | 신뢰하지 않는 사용자·서비스 위치 |
| 비무장지대 | 공개 서비스를 내부와 중계·격리 |
| 업무 영역 | 일반 업무 단말·서버를 수용 |
| 핵심 영역 | 중요 자료·제어 시스템을 격리 |
| 자료 전송 관문 | 파일 검사·무해화·반출 승인 |
| 관리 접속 관문 | 관리자 인증·승인·세션 기록 |

> 요약: 영역 사이 자료·관리 통로를 관문으로 제한

### 쉽게 이해하기 (학습용)

- 인터넷·일반 업무·핵심 업무를 나누고 파일과 관리자 접속은 각각 전용 관문에서 검사한 뒤 통과시킨다.

## Ⅳ. 원리 및 절차 흐름도

```mermaid
sequenceDiagram
    participant 사용자
    participant 통제관문
    participant 정책엔진
    participant 대상영역
    participant 기록체계
    사용자->>통제관문: 영역 간 통신 요청
    통제관문->>정책엔진: 신원·단말·업무 검증
    정책엔진-->>통제관문: 최소 흐름 판정
    통제관문->>통제관문: 자료·세션 검사
    통제관문->>대상영역: 승인 통신 중계
    통제관문->>기록체계: 접속·반출 기록
```

| 절차 | 설명 |
|:---|:---|
| 영역 간 통신 요청 | 출발·목적·업무 사유 제출 |
| 신원·단말·업무 검증 | 사용자·기기·권한을 대조 |
| 최소 흐름 판정 | 필요한 주소·포트·시간만 허용 |
| 자료·세션 검사 | 악성 내용·관리 명령을 확인 |
| 승인 통신 중계 | 직접 연결 없이 관문이 전달 |
| 접속·반출 기록 | 승인·행위·결과를 감사 보존 |

> 요약: 영역 간 흐름을 검증·검사·중계·기록

### 쉽게 이해하기 (학습용)

- 다른 망으로 가려는 요청은 사용자와 업무 목적을 확인하고 필요한 범위만 관문이 대신 연결하며 모든 행위를 남긴다.

## Ⅴ. 종류 및 비교

| 판단 기준 | 물리적 망분리 | 논리적 망분리 | 혼합 망분리 |
|:---|:---|:---|:---|
| 핵심 특징 | 장비·회선을 완전히 별도 구성 | 공유 장비를 가상화·정책으로 분리 | 핵심은 물리, 나머지는 논리 분리 |
| 적용 기준 | 최고 격리·규제 요구 | 변화가 잦고 자원 효율 우선 | 중요도별 비용·격리 균형 필요 |
| 주요 위험 | 높은 비용·자료 이동 불편 | 설정 오류·공유 계층 취약점 | 경계 복잡성·정책 불일치 |

> 요약: 자산 중요도·공유 위험·비용으로 방식 선택

### 쉽게 이해하기 (학습용)

- 아주 중요한 망은 물리적으로 나누고 변화가 잦은 망은 논리적으로 나누며 중요도가 섞이면 두 방식을 조합한다.

## Ⅵ. 실무 사례

1. 연구망 반입 문서를 CDR 후 업무망 전달
2. 핵심 서버 관리를 PAM 관문으로만 허용

### 쉽게 이해하기 (학습용)

- 외부 연구 자료는 실행 코드를 제거하고 승인된 문서 내용만 업무망으로 옮긴다.
- 핵심 서버 관리자는 직접 접속하지 않고 승인을 받은 뒤 관문을 거쳐 접속하며 명령을 기록한다.

## Ⅶ. 결론

- 중요도·공유 위험으로 분리 강도와 관문 결정

### 쉽게 이해하기 (학습용)

- 망분리는 물리와 논리 중 하나를 고르는 데서 끝나지 않고 실제 자료와 관리 경로를 어떤 관문으로 통제할지 정해야 한다.
