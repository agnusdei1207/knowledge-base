...
- **횡적 이동(Lateral Movement)**: 침해한 한 시스템에서 내부의 다른 시스템으로 접근을 넓히는 공격
- **기본 차단(Default Deny)**: 명시적으로 허용하지 않은 모든 통신을 거부하는 정책
- **통제 관문(Security Gateway)**: 분리 영역 사이의 자료·세션을 검사하고 승인 흐름만 중계하는 구성요소
- **방화벽(Firewall)**: 주소·포트·상태·응용 규칙으로 네트워크 통신을 허용·차단하는 장비
- **매크로(Macro)**: 문서에서 자동 실행되는 명령·스크립트 기능

> **키워드:** 망분리 - 물리적·논리적 (Network Separation)
