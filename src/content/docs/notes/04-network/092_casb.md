...
date: "2026-07-31T11:07:12+09:00"
tags: ["notes-network"]
weight: 92
extra:
  question_no: "092"
...
- **CASB 정책 엔진**: 신원·단말·자료·서비스 위험으로 조치를 결정하는 구성요소다.
- **격리·공유 회수**: 위험 자료를 분리하거나 기존 외부 공유 권한을 제거하는 조치다.

> **키워드:** CASB 클라우드 접근 보안 브로커 (CASB)
...
- **행위별 정책**: 사용자·단말·서비스·자료 맥락 결합
- **프록시 연동**: 클라우드 세션의 실시간 통제
- **API 연동**: 저장 자료·공유 설정의 사후 점검
...
sequenceDiagram
    ...
    participant SaaS
    사용자->>CASB: 클라우드 접속 요청
    CASB->>정책엔진: 1. SaaS 사용 증거 전달
    정책엔진->>정책엔진: 2. 서비스 위험 분류
    CASB->>정책엔진: 3. 자료·행위 맥락 전달
...
- **1. SaaS 사용 증거 전달**: 로그·트래픽·API로 사용 정보 수집
- **2. 서비스 위험 분류**: 승인 여부·보안 기능·위치 평가
- **3. 자료·행위 맥락 전달**: 신원·단말·등급별 이동 정보 제공
...
- 관리 단말 실시간은 **순방향**, 비관리 접속은 **역방향**, 저장물은 **API** 적용
