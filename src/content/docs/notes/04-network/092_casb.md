...
- **클라우드 접근 보안 중개(CASB, Cloud Access Security Broker)**: 온프레미스 사용자와 클라우드 서비스(SaaS/IaaS) 사이에 위치하여 접근 가시성 확보, DLP, 위협 방어, 규정 준수 정책을 통합 집행하는 보안 솔루션이다.
- **섀도 IT(Shadow IT)**: 중앙 IT 부서의 승인과 통제 없이 사용자가 임의로 사용하는 클라우드 서비스 및 소프트웨어이다.
- **서비스형 소프트웨어(SaaS, Software as a Service)**: 클라우드 기반으로 응용 소프트웨어를 구독형 서비스로 제공하는 모델이다.
...
- 정의/개념: 온프레미스와 클라우드 전단에서 가시성 및 보안 정책을 집행하는 **CASB 보안 중개 체계**.
...
- **데이터 유출 방지(DLP, Data Loss Prevention)**: 콘텐츠 식별 및 정규표현식 매칭을 통해 중요 기밀의 무단 외부 전송 및 공유를 차단하는 기술이다.
- **순방향 프록시(Forward Proxy)**: 사용자 단말에서 클라우드로 나가는 아웃바운드 트래픽을 중계하여 실시간 트래픽을 감시•통제하는 방식이다.
- **응용 프로그래밍 인터페이스(API, Application Programming Interface)**: CASB가 클라우드 서비스 사업자의 API를 직접 호출하여 저장된 데이터와 공유 설정을 사후 점검하는 연동 방식이다.
...
| 구성요소 | 책임 |
|:---|:---|
| 사용자•단말 | 클라우드 서비스 접속 요청 및 데이터 트래픽 생성 |
| 접근 중개기 | 인라인 Proxy 및 API로 트래픽/데이터 수집 및 모니터링 |
| 신원•자료•위험 저장소 | 신원 ID, DLP 규칙, SaaS 위험도 Catalog 제공 |
| CASB 정책 엔진 | Context 기반 접근 허용, 차단, 암호화, 격리 결정 |
| 클라우드 서비스 | 최종 사용자 요청을 전달받아 비즈니스 기능 제공 |
...
1. **SaaS 사용 증거 전달**: 로그, 프록시, 백엔드 API를 통해 수집.
2. **서비스 위험 분류**: Cloud Risk Catalog를 조회하여 수행.
3. **자료•행위 맥락 전달**: 신원, 단말, 데이터 민감도 등 Context 정보 전달 및 분석.
4. **정책 조치 결정**: 허용, 차단, 격리•공유 회수 중 최종 확정.
5. **세션•저장물 집행**: Inline Proxy 차단 및 백엔드 API 권한 삭제로 집행 완료.
