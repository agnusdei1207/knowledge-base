...
1. **SaaS 사용 증거 전달(Telemetry Transfer)**: 로그, 프록시, 백엔드 API를 통해 수집.
2. **서비스 위험 분류(Risk Classification)**: Cloud Risk Catalog를 조회하여 수행.
3. **자료/행위 맥락 전달(Context Transfer)**: 신원, 단말, 데이터 민감도 등 Context 정보 전달 및 분석.
4. **정책 조치 결정(Policy Decision)**: 허용, 차단, 격리/공유 회수 중 최종 확정.
5. **세션/저장물 집행(Enforcement)**: Inline Proxy 차단 및 백엔드 API 권한 삭제로 집행 완료.
...
- 기업 환경에 맞춘 **순방향 프록시**, **역방향 프록시**, **API 연동** 하이브리드 배치를 통한 **CASB 연동 방식 선택(Pattern Selection)** 체계 정립.
