...
date: "2026-07-25T02:05:00+09:00"
tags:
  - "notes-network"
weight: 1
extra:
...
- **개방형 시스템 상호연결(Open Systems Interconnection, OSI)**: 서로 다른 통신 장비가 호환되도록 기능과 책임을 7개 계층으로 나눈 참조 모델
- **프로토콜 데이터 단위(Protocol Data Unit, PDU)**: 각 계층이 상위 데이터에 제어 정보를 붙여 처리하는 데이터 단위
...
- **정의**: 통신 기능·책임을 7계층으로 분리한 참조 모델
- **배경/필요성**: 이기종 장비의 계층별 상호운용 기준
...
- 계층 인터페이스가 구현 변경 영향을 격리한다
- 계층별 PDU가 처리 책임·장애 범위를 나눈다
- 캡슐화·역캡슐화가 제어 정보를 대칭 처리한다
...
| 주요 위험 | 실제 프로토콜과 경계 불일치 | 계층별 책임 분석이 거칠어짐 |
...
1. 네트워크 장애를 물리·링크·IP·전송 순으로 진단
2. 방화벽은 3·4계층, WAF는 7계층 트래픽 검사
