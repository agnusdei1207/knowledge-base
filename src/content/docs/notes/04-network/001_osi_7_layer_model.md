...
date: "2026-08-02T09:20:00+09:00"
tags:
  - "notes-network"
weight: 1
extra:
...
- **OSI 7계층 모델**은 통신 기능과 책임을 일곱 계층으로 나누어 이기종 시스템의 상호운용 기준을 제공하는 참조 모델이다.
...
- **계층 인터페이스**는 인접한 두 계층이 서비스를 요청하고 처리 결과를 전달하는 경계이다.
- **PDU**는 각 계층이 자신의 제어 정보를 붙여 처리하는 데이터 단위이다.
- **캡슐화와 역캡슐화**는 송신 계층이 제어 정보를 붙이고 수신 계층이 반대 순서로 제거하는 과정이다.
...
- **TCP**는 포트로 응용을 구분하고 순서와 재전송을 제어하는 전송 계층 프로토콜이다.
- **IP**는 출발지와 목적지 주소를 사용하여 패킷 전달 경로를 정하는 네트워크 계층 프로토콜이다.
- **MAC**은 같은 링크의 장치를 식별하고 프레임을 전달하는 데이터링크 계층 기능이다.
...
- **세그먼트·패킷·프레임·비트**는 각각 전송·네트워크·데이터링크·물리 계층이 처리하는 데이터 단위이다.
...
- **TCP/IP 모델**은 인터넷에서 실제 사용하는 프로토콜을 응용·전송·인터넷·네트워크 접근의 네 계층으로 묶는다.
...
- **방화벽**은 주소·포트·연결 상태 등의 규칙으로 네트워크 트래픽을 허용하거나 차단한다.
...
- **계층별 장애 분석**은 관찰한 PDU와 주소 종류를 기준으로 문제가 시작된 통신 계층을 좁히는 방법이다.
