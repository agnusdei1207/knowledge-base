...
- 정의/개념: 통신 기능과 책임을 **OSI 7계층**으로 분리한 참조 모델이다.
- 배경/필요성: 공통 계층 기준 없이는 이기종 장비의 **상호운용 제약**이 발생한다.
...
- 계층 인터페이스의 **구현 변경 영향 격리**가 핵심이다.
- 계층별 PDU의 **처리 책임•장애 범위 분리**가 핵심이다.
- 캡슐화•역캡슐화의 **제어 정보 대칭 처리**가 핵심이다.
...
- **프로토콜 데이터 단위(Protocol Data Unit, PDU)**: 계층별 제어 정보를 포함한 데이터 단위이다.
...
1. **캡슐화**: 계층별 헤더를 붙여 PDU을 생성한다.
2. **역캡슐화**: 계층 역순으로 헤더를 제거해 원본을 복원한다.
...
- PDU•주소를 기준으로 **OSI 계층**별 장애 시작점을 선택한다.
