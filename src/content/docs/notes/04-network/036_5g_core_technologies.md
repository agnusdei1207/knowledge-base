...
block
    columns 1
    A["서비스 요구 프로필 | 처리량·지연·밀도 목표 정의"]
    B["서비스 품질 정책 | 흐름 우선순위·격리 결정"]
    C["무선 자원 제어 | 대역·재전송·접속 기회 배정"]
    D["전송망 경로 | 지연·용량별 경로 전달"]
    E["사용자면 기능(UPF) | 사용자 패킷 경로 선택"]
    A --- B
    B --- C
    C --- D
    D --- E
...
sequenceDiagram
    ...
    정책제어기->>정책제어기: 2. 서비스 유형 분류
    정책제어기->>무선·전송자원: 3. 자원 정책 설정
    정책제어기->>UPF: 4. 전달 경로 구성
    무선·전송자원->>응용: 5. 종단 서비스 제공
```

**동작 원리**

1. **서비스 목표 요청**: 처리량·지연·밀도 목표 전달
2. **서비스 유형 분류**: eMBB·URLLC·mMTC 매핑
3. **자원 정책 설정**: 대역·우선순위·접속량 배정
4. **전달 경로 구성**: 사용자면과 엣지 경로 선택
5. **종단 서비스 제공**: 설정된 자원으로 통신 수행
