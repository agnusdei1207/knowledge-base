...
date: "2026-08-26T13:45:20+09:00"
tags:
  - "notes-network"
weight: 36
extra:
...
- 배경/필요성: 4G LTE 단일 범용 통신의 한계로 인한 **초대용량 미디어(20Gbps), 자율주행 1ms 초저지연, 100만 개/$\text{km}^2$ 대규모 IoT 단말 동시 수용 불가**를 겪으므로, 서비스 유형마다 전용망을 따로 까는 비용을 피해 하나의 물리 인프라를 요구 조건별 가상망으로 갈라 각각에 다른 자원·지연 목표를 부여할 슬라이싱 계층의 필요
...
- 광대역은 **eMBB**, 저지연·대규모 접속은 **URLLC·mMTC** 선택
