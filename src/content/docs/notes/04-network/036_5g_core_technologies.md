...
- 정의/개념: 요구 품질에 따라 **처리량•신뢰성•지연•접속 밀도**를 특화한 5G 서비스 유형이다.
- 배경/필요성: 단일 품질 정책으로 **처리량•지연•접속 밀도 동시 최적화할 수 없다**.
...
- 넓은 대역•다중 안테나를 통한 **eMBB 처리량 향상**이 핵심이다.
- 우선 자원•짧은 경로를 통한 **URLLC 신뢰성•지연 보장**이 핵심이다.
- 경량 접속•절전을 통한 **mMTC 접속 밀도 향상**이 핵심이다.
...
| 사용자 평면 기능(UPF) | 사용자 패킷의 **엣지 경로** 연결 |
...
- 처리량은 **eMBB**, 신뢰•지연은 **URLLC**, 접속 밀도는 **mMTC**을 선택한다.
