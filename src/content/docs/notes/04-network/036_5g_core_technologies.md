...
- **5G 서비스 유형**: 응용의 요구에 따라 처리량·신뢰성·지연·접속 밀도 중 우선할 품질을 특화한 이동통신 서비스 분류이다.
...
- **eMBB**: 영상·확장현실의 대용량·고속 전송을 목표로 하는 5G 서비스 유형이다.
- **URLLC**: 원격 제어의 높은 신뢰도와 짧은 지연을 목표로 하는 5G 서비스 유형이다.
- **mMTC**: 많은 저전력 센서의 동시 접속을 목표로 하는 5G 서비스 유형이다.
...
- **QoS**: 흐름별 처리량·지연·우선순위를 제어하는 품질 기준이다.
- **UPF**: 설정된 정책에 따라 사용자 패킷을 전달하고 엣지 또는 외부망 경로에 연결하는 5G 코어 기능이다.
...
- **SLA**: 공급자와 이용자가 합의한 처리량·지연·가용성 등의 서비스 성능과 책임 목표이다.
