...
2. **QoS 정책 및 슬라이스 결정**: 5QI 인덱스와 가상 네트워크 슬라이스(Network Slice)를 매핑하여 전용 자원 프로필을 확정한다.
3. **무선/전송 자원 할당**: eMBB는 광대역 부반송파, URLLC는 짧은 TTI 및 미니슬롯, mMTC는 협대역 채널을 무선 구간에 할당한다.
4. **UPF 및 MEC 경로 구성**: URLLC 트래픽은 기지국 인근의 Edge UPF/MEC로 최단 라우팅하고, eMBB는 중앙 코어 UPF로 경로를 설정한다.
5. **품질 감시 및 피드백**: 엔드투엔드 KPI 모니터링을 통해 실시간 무선 자원 조율 및 서비스 수준 협약(SLA) 유지를 수행한다.
...
| mMTC 제어 채널 폭주 | 수십만 개 IoT 단만의 동시 접속 시도가 Paging 마비 유발 | 3GPP Paging Throttle 기법 및 eDRX/PSM 주기 최적화 | 제어 플레인 과부하 예방 및 배터리 수명 극대화 |
...
- 5G/B5G 통신망 구축 시 서비스별 **KPI 기반 5G 3대 서비스 분류**, **MEC 연동 URLLC 극저지연망 구축**, **독립적 네트워크 슬라이싱 적용 필수**.
