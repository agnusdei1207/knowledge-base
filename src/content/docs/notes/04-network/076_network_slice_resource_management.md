...
- 정의/개념: 종단 SLS를 NSI•NSSI 자원으로 배치•조정하는 **슬라이스 관리 체계**이다.
- 배경/필요성: 고정 예약은 **유휴 낭비**, 과도한 공유는 **SLS 위반**이 발생한다.
...
- **예산 분해**: 종단 SLS를 도메인별 자원 목표로 변환한다.
- **수용 제어**: 기존•신규 요구의 동시 충족을 판정한다.
- **폐루프 조정**: 측정값으로 병목 자원만 탄력 변경한다.
...
1. **자원 타당성 확인**: 기존•신규 SLS 동시 충족을 판정한다.
2. **NSI•NSSI 프로비저닝**: 도메인별 자원•정책을 인스턴스화한다.
3. **병목 자원 최적화**: 히스테리시스 후 제약 구간만을 증감한다.
