...
date: "2026-07-31T03:07:00+09:00"
tags:
  - "notes-network"
weight: 76
extra:
...
- **슬라이스 오케스트레이터(Slice Orchestrator)**: SLS를 도메인별 자원·정책 인스턴스로 배치하는 제어기
- **프로비저닝(Provisioning)**: 필요한 네트워크 기능·자원·정책을 생성하고 활성화하는 작업
- **폐루프 제어(Closed-loop Control)**: 측정·판단·조치를 반복해 목표 상태를 유지하는 제어
...
3. **종단 성능 보고**: 구간 측정값을 종단 기준으로 결합
4. **병목 자원 최적화**: 히스테리시스 후 제약 구간만 증감
