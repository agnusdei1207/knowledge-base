...
date: "2026-07-25T19:58:00+09:00"
tags:
  - "notes-network"
weight: 58
extra:
...
- **정의/개념**: 컨트롤러가 경로 계산, OpenFlow가 규칙 제어
- **배경/필요성**: 중앙 정책의 Match-Action 규칙 반영
...
- 다단 테이블의 Match-Action 구조를 제어한다.
- Table-Miss 발생 시 Packet-In으로 질의한다.
- Group·Meter로 다중 경로와 속도를 제어한다.
- Packet-In 폭증은 제어 부하와 첫 패킷 지연을 늘린다.
...
| OpenFlow 보안 채널 | 컨트롤러·스위치 제어 메시지 교환 |
| 흐름·그룹·미터 테이블 | 조건·동작·복수 경로·속도 정책 저장 |
| OpenFlow 데이터 경로 | 규칙 실행과 패킷·바이트 계수 |
...
| 주요 위험 | 첫 패킷 지연·제어기 폭주 | 미사용 TCAM 점유·규칙 노후화 |
...
1. 미지 트래픽은 Packet-In 속도 제한·기본 폐기 규칙 적용
2. 핵심 서비스 경로는 선제 규칙과 유휴 시간 분리
