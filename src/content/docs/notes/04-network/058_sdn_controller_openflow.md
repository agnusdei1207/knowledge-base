...
block
    columns 1
    A["SDN 응용 | 연결·보안·품질 정책 요청"]
    B["SDN 컨트롤러 | 전역 상태로 흐름 규칙 계산"]
    C["OpenFlow 채널 | 컨트롤러·스위치 제어 교환"]
    D["흐름·그룹·미터 테이블 | 조건·동작·경로·속도 저장"]
    E["OpenFlow 데이터 경로 | 규칙 실행·통계 계수"]
    A --- B
    B --- C
    C --- D
    D --- E
...
sequenceDiagram
    ...
    participant 컨트롤러
    participant 흐름표
    패킷->>OpenFlow스위치: 1. 첫 패킷 전달
    OpenFlow스위치->>컨트롤러: 2. Packet-In 질의
    컨트롤러->>컨트롤러: 3. 흐름 규칙 계산
    컨트롤러->>흐름표: 4. Flow-Mod 설치
    흐름표->>OpenFlow스위치: 5. 첫 패킷 처리
```

**동작 원리**

1. **첫 패킷 전달**: 흐름 테이블에서 일치 규칙 조회
2. **Packet-In 질의**: Table-Miss 패킷 정보를 제어기에 전달
3. **흐름 규칙 계산**: 토폴로지·정책으로 출력 동작 결정
4. **Flow-Mod 설치**: 우선순위·시간 제한과 규칙을 배포
5. **첫 패킷 처리**: 설치 규칙으로 보류 패킷 전달
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| Packet-In 폭증 | 질의율 제한·기본 폐기 규칙 | 컨트롤러 보호 |
| TCAM 규칙 고갈 | 핵심 흐름 선제·예외 반응형 | 규칙 공간 절감 |
| 오래된 흐름 규칙 | 유휴·고정 만료와 회수 검증 | 정책 최신성 |
...
- **첫 패킷 지연·Packet-In·TCAM을 반영한 OpenFlow 규칙 조합**
