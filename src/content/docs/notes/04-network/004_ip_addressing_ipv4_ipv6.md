...
- 정의/개념: 네트워크•인터페이스를 식별하는 **계층형 논리 주소**이다.
- 배경/필요성: 32비트 IPv4 주소 고갈로 단말별 **공인 주소 할당 제약**이 발생한다.
...
- 계층형 프리픽스의 **경로 집약**이 핵심이다.
- 최장 프리픽스 일치의 **최구체 경로 선택**이 핵심이다.
- IPv6의 128비트 주소와 **SLAAC 자동 구성**이 핵심이다.
...
2. **다음 홉 결정**: 최장 일치 경로의 전달 대상을 선택한다.
3. **링크 주소 해석**: ARP•NDP로 다음 홉 주소를 확인한다.
...
- 기존망 호환성•주소 수요를 기준으로 **듀얼 스택** 후 **IPv6 전환**이 핵심이다.
