...
- **BB84**: 기저 불일치와 측정 교란으로 도청 흔적 검출이다.
- **QBER 임계값**: 키 정제 진행•폐기 판단이다.
- **키 정제**: 오류 정정과 비밀성 증폭으로 최종 키를 생성한다.
...
- **양자 송신기**: 임의 비트•기저로 양자 상태를 만드는 장치다.
- **양자 채널**: 광자의 양자 상태를 송신기에서 수신기로 전달하는 경로다.
- **양자 수신기**: 임의 기저로 양자 상태를 측정하는 장치다.
...
가지의 의미: 양자 전송•고전 인증•키 관리 책임을 분리한 정적 구조다.

| 구성요소 | 책임 |
|:---|:---|
| 양자 송신기 | **임의 비트•기저**로 양자 상태 생성 |
| 양자 채널 | 광자로 **키 후보 양자 상태** 전달 |
| 양자 수신기 | **임의 기저**로 상태 측정 |
| 인증 고전 채널 | **기저 선별•오류 정정 정보** 교환 |
...
1. **양자 상태 전송**: 임의 비트•기저의 광자를 보냄한다.
2. **측정 기저 정보 전달**: 수신 측정에 사용한 기저를 제공한다.
3. **준비 기저 정보 전달**: 송신 준비에 사용한 기저를 제공한다.
4. **일치 비트•QBER 표본 전달**: 후보 비트와 QBER 표본을 제공한다.
5. **최종 키 전달**: 정정•증폭을 마친 대칭키를 제공한다.
#### 한줄 요약
...
| 한계 | **거리•장비•인증 채널 의존** | **성능•구현 검증 부담** | **양자 공격•장기 기밀 노출** |
...
- **ITU-T Y.3800**: 국제전기통신연합 전기통신표준화부문(International Telecommunication Union Telecommunication Standardization Sector, ITU-T)의 QKD 네트워크 구조 권고다.
...
| **장비•망 간 연동 한계** | **ITU-T Y.3800 구조 적용** | **계층별 상호운용 확보** |
...
- 키 생성률•연동성 검증 후 고가치 광은 **QKD**, 범용망은 **PQC**을 전환한다.
