...
- 정의/개념: **소프트웨어 정의 네트워킹**은 제어 평면과 데이터 평면을 분리하고, 논리적 중앙 컨트롤러가 개방형 인터페이스로 망 동작을 프로그래밍하는 아키텍처이다.
...
- **제어 및 전달 평면 분리**: 스위치는 패킷을 전달하고 SDN 컨트롤러는 경로와 정책을 결정한다.
...
1. **네트워크 정책 및 의도 수신**
2. **SDN 컨트롤러의 전역 토폴로지 및 링크 텔레메트리 수집**
3. **최적 패킷 전송 경로 연산 및 흐름 테이블 규칙 생성**
4. **사우스바운드 API를 통한 스위치 흐름 테이블 하향 설치**
5. **INT/eBPF 텔레메트리 기반 폐루프 모니터링 및 혼잡 시 재배포**
...
| 비교 항목 | **소프트웨어 정의 네트워킹 ** | **전통적 IP 네트워킹 (Traditional Network)** |
|:---|:---|:---|
...
| 컨트롤러 간 상태 동기화 오차 | 분산 컨트롤러 노드 간 DB 동기화 지연 | 분산 합의 알고리즘(Raft Consensus) 적용 | 네트워크 정책 일관성 향상 |
...
- 전역 정책 자동화는 **SDN**, 단순 분산 복구는 **전통 IP** 선택.
