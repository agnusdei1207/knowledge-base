...
block
    columns 1
    A["SDN 응용 | 연결·보안·품질 의도 정의"]
    B["노스바운드 인터페이스 | 응용 의도·정책 전달"]
    C["SDN 컨트롤러 | 전역 경로·흐름 규칙 계산"]
    D["사우스바운드 인터페이스 | 장비 규칙 설치·상태 수집"]
    E["데이터 평면 | 흐름 규칙 기반 패킷 처리"]
    A --- B
    B --- C
    C --- D
    D --- E
