...
- 정의/개념: 제어 평면과 데이터 평면을 분리하는 **SDN 제어 구조**
...
- **응용 프로그래밍 인터페이스(Application Programming Interface, API)**: 응용과 시스템 기능 사이의 호출 규격
...
선의 의미: 세로선은 네트워크 의도를 정의하는 SDN 응용, 의도를 전달하는 노스바운드 인터페이스, 전역 정책을 계산하는 SDN 컨트롤러, 장비 제어 경계인 사우스바운드 인터페이스와 실제 패킷을 처리하는 데이터 평면의 정적 제어 계층을 뜻한다.
...
- **기계학습(Machine Learning, ML)**: 데이터 패턴을 학습해 혼잡 상태를 예측하는 기술
