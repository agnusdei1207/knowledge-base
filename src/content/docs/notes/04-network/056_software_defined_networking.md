...
date: "2026-07-31T01:06:29+09:00"
tags:
  - "notes-network"
weight: 56
extra:
...
sequenceDiagram
    ...
    participant 컨트롤러
    participant 데이터평면
    SDN응용->>컨트롤러: 1. 네트워크 의도
    데이터평면->>컨트롤러: 2. 장비 상태
    컨트롤러->>데이터평면: 3. 흐름 규칙
    loop 상태 변화 시
        데이터평면->>컨트롤러: 4. 텔레메트리 피드백
        컨트롤러->>데이터평면: 5. 보정 흐름 규칙
    end
```

**동작 원리**

1. **네트워크 의도**: 응용이 연결·격리·품질 목표 선언
2. **장비 상태**: 데이터 평면이 토폴로지·링크 상태 제공
3. **흐름 규칙**: 컨트롤러가 장비별 패킷 조건·동작 배포
4. **텔레메트리 피드백**: 장비가 트래픽·지연·오류 상태 보고
5. **보정 흐름 규칙**: 관측 결과와 정책 차이를 반영한 규칙 재배포
