...
- 배경/필요성: 기존 분산 라우터의 장비별 CLI 개별 수동 설정 및 국소 최단 경로(SPF) 한계로 인한 **전사 망 변경 지연, 전역 트래픽 엔지니어링(TE) 불가 및 휴먼 에러 발생**을 겪으므로, 장비마다 흩어져 있던 제어 평면을 걷어내 중앙 컨트롤러 한 곳에 모으고 스위치에는 전달만 남겨, 장비 수에 비례하던 설정 비용을 전역 정책 한 벌로 줄일 필요
