...
- 정의/개념: 프리픽스로 주소 블록을 분할•집약하는 **서브네팅•CIDR 기법**이다.
- 배경/필요성: 고정 크기 주소 할당은 수요 차이로 **주소 낭비•경로 수가 증가한다**.
...
- **긴 프리픽스**는 주소 수•브로드캐스트 범위를 축소한다.
- **VLSM**은 수요별 블록 할당으로 주소 낭비 절감이 핵심이다.
...
4. **집약 경로 생성**: 연속 블록의 공통 앞 비트를 추출한다.
...
- 호스트 수•증가율로 **서브넷 크기**를 정하고 연속 블록은 **CIDR 집약**이 핵심이다.
