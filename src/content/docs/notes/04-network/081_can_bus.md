...
date: "2026-07-25T20:21:00+09:00"
tags: ["notes-network"]
weight: 81
extra:
  question_no: "081"
...
- **정의**: ID 중재로 공유 버스의 전송 순위 결정
- **필요성**: 적은 배선으로 긴급 제어 메시지 우선 전달
...
- 작은 ID가 우성 비트로 버스 우선권을 얻는다.
- 중재 패배 노드는 프레임 손상 없이 재시도한다.
- 오류 누적 노드는 버스 오프로 스스로 격리한다.
- 공유 회선 단선은 여러 제어기를 함께 중단시킨다.
...
| 판단 기준 | CAN CC | CAN FD | CAN XL |
|:---|:---|:---|:---|
| 핵심 특징 | 최대 8바이트의 제어 프레임 | 최대 64바이트·데이터 구간 가속 | 최대 2048바이트·고속 백본 |
| 적용 기준 | 소량·주기 제어 메시지 | 기존 CAN과 호환하며 용량 확대 | 대용량 차량 데이터 전송 |
| 주요 위험 | 낮은 대역폭 | 혼합 노드의 호환 설정 오류 | 복잡한 장비·망 설계 |
