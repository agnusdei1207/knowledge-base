...
- **CAN 송수신기(CAN Transceiver)**: 논리 비트와 차동 버스 전압을 상호 변환하는 회로
- **차동 버스(Differential Bus)**: 두 회선의 전압 차이로 비트를 전달해 잡음 영향을 줄이는 공유 회선
- **버스 오프(Bus-off)**: 오류 카운터가 한계를 넘은 노드를 송신에서 격리하는 상태
- **우선순위 기아(Priority Starvation)**: 높은 우선순위 프레임 때문에 낮은 우선순위 전송이 계속 지연되는 현상
- **최악 응답 시간(Worst-case Response Time)**: 가장 불리한 중재·재전송 조건에서 완료까지 걸리는 최대 시간

> **키워드:** CAN 버스 자동차 통신 (CAN Bus)
