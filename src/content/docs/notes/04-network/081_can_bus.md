...
- 정의/개념: 식별자로 우선순위를 중재하는 **차량용 공유 버스 통신**이다.
- 배경/필요성: 제어기별 점대점 배선은 **배선량•확장 복잡도**가 증가한다.
...
- **우선순위 중재**: 작은 ID의 우성 비트가 버스 선점이다.
- **비파괴 재시도**: 패자만 중단하고 승자 프레임을 보존한다.
- **오류 제한**: 오류 누적 노드를 버스 오프로 격리이다.
...
1. **유휴 버스 감지**: 회선이 비었을 때 송신을 시작한다.
2. **ID 비트 중재**: 송신값과 버스값을 비트별을 비교한다.
3. **데이터•CRC 전송**: 중재 승자만 본문과 검사값을 전송한다.
4. **수신 프레임 전달**: 승자 프레임을 모든 노드에 전파한다.
5. **ACK 비트 응답**: 정상 수신 노드가 확인 비트를 기록한다.
...
- 8바이트 주기 제어는 **CAN CC**, 중용량 고속은 **CAN FD**, 백본은 **CAN XL**가 핵심이다.
