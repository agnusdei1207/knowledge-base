...
- **RIS**: 다수 단위 셀의 반사 계수를 조절해 전파 경로를 재구성하는 무선 표면 기술이다.
...
- **연쇄 채널**: 기지국-RIS 경로와 RIS-단말 경로가 결합된 전파 채널이다.
...
- **단위 셀**: 위상·진폭·편파 응답을 개별 설정할 수 있는 RIS의 최소 반사 소자이다.
...
- **채널 추정**: 기준 신호를 측정해 기지국-RIS-단말의 결합된 전파 특성을 계산하는 과정이다.
...
- **RF Chain**: 신호 변환·증폭·변복조를 담당하는 능동 무선주파수 회로 계통이다.
...
- **SNR**: 수신 신호 전력과 잡음 전력의 비율로 RIS 설치 후보 위치의 입사파 품질을 판단하는 지표이다.
...
- **입사 전력**: RIS 표면에 도달하는 원 신호의 전력으로 수동 반사 경로가 얻을 수 있는 이득의 기초이다.
