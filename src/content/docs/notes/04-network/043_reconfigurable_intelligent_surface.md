...
5. 단말 위치에서의 무선 신호 위상 동상 결합 (Phase Coherent Combining)
      │
      v
6. 단말 이동 및 채널 변동 시 닫힌 루프(Closed-loop) 위상 재계산
```
...
5. **동상 결합(Coherent Combining)**: 사방으로 산사되지 않고 단말 위치에서 모든 반사파의 위상이 일치(In-phase)하여 섀도잉 음영 구역의 SNR을 대폭 획득한다.
...
| RF Chain 및 전력 소모 | RF Chain 없음, 전력 소모 극소 (수mW 이하) | RF 증폭기 및 복합 회로 필요, 전력 소모 높음 |
| 증폭 잡음 (Noise Amplification) | 신호를 증폭하지 않으므로 잡음 증폭 없음 | 수신 신호와 열잡음을 함께 증폭하여 SNR 저하 |
| 통신 방식 | 전중계(Full Duplex) 효과, 시간 지연 극소 | 반중계(Half Duplex) 지연 또는 Self-Interference |
| 구축 비용 | 대면적 초저가 메타표면 제작 가능 | 고가 RF 증폭 장비 및 전원 공급 시설 필수 |
...
- 6G 무선망 및 건물 내 음영지역 해소 시 **RIS 반사판 최적 위치 설계**, **수동 빔포밍 연쇄 채널 추정 알고리즘 구축**, **Sub-THz 대역 RIS 통합 배치 필수**.
