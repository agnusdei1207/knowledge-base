...
- **정의/개념**: 소자 반사 계수로 전파 경로를 바꾸는 표면
- **배경/필요성**: 장애물로 막힌 고주파 무선 경로 우회
...
- 수동형은 RF Chain 없이 반사 계수를 제어한다.
- 능동형은 증폭 이득과 잡음·전력 비용이 상충한다.
- 위상 양자화 오차가 빔 결합 이득을 낮춘다.
- 채널 변화보다 제어가 늦으면 빔 방향이 이탈한다.
...
1. 창고 음영 구역은 반사 경로 측정 후 RIS 배치
2. 실내 이동 로봇망은 채널 갱신 주기에 맞춰 위상 재설정

### 쉽게 이해하기 (학습용)

- 선반에 막힌 구역까지 전파가 꺾여 도달하도록 RIS 위치를 정한다
- 로봇이 이동해 전파 길이 바뀌면 새 위치에 맞춰 반사 각도를 다시 정한다

## Ⅶ. 결론

- 채널 갱신 지연·입사 전력으로 RIS 적용 결정
