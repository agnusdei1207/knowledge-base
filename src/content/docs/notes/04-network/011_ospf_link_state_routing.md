...
- **개방형 최단 경로 우선(Open Shortest Path First, OSPF)**: 링크 상태 정보를 영역 안에 공유하고 각 라우터가 최단 경로를 계산하는 내부 라우팅 프로토콜
...
- **배경/필요성**: 내부망 변화 공유와 영역별 계산 범위 제한
...
- LSDB 일치가 영역 내 경로 계산 기준을 통일한다.
- 링크 상태 변화는 LSA 플러딩으로 신속히 전파한다.
- 영역 분할은 LSA·SPF 재계산 범위를 제한한다.
...
| 판단 기준 | OSPF | OSPFv3 |
|:---|:---|:---|
| 핵심 특징 | IPv4 프리픽스를 LSA와 함께 광고 | IPv6 프리픽스를 링크 정보와 분리 |
| 적용 기준 | IPv4 내부 라우팅 | IPv6 기본·확장 시 IPv4 지원 |
| 주요 위험 | 영역·인증 불일치 | 링크 로컬 주소·영역 설정 불일치 |
...
1. 대규모 내부망은 영역 0·ABR·경로 요약으로 분할
2. 공유 이더넷은 DR·BDR로 인접 관계 수 축소
