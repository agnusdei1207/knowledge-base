...
| LSA | 링크 상태·비용·프리픽스 광고 |
| LSDB | 영역 내 동일 토폴로지 저장 |
| SPF 계산기 | 최단 경로 트리·다음 홉 계산 |
| 영역 경계 라우터 | 영역 연결·경로 요약 |
...
sequenceDiagram
    ...
    participant LSDB
    participant 경로표
    라우터A->>라우터B: 1. 이웃 관계 형성
    라우터A->>라우터B: 2. LSA 교환
    라우터B->>LSDB: 3. 토폴로지 저장
    LSDB->>라우터A: 4. SPF 경로 계산
    라우터A->>경로표: 5. 최선 경로 설치
```

**동작 원리**

1. **이웃 관계 형성**: Hello 조건 확인·상태 전이
2. **LSA 교환**: 링크 상태·비용 동기화
3. **토폴로지 저장**: 최신 LSA로 영역 지도 갱신
4. **SPF 경로 계산**: 자신 기준 최단 경로 산출
5. **최선 경로 설치**: 목적지·다음 홉 반영
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 영역 0 단절 | 연속 백본·가상 연결 최소화 | 영역 간 도달성 확보 |
| 과대 영역의 SPF 부하 | 변화량 기반 영역 분할 | 계산·플러딩 범위 축소 |
| 경로 요약 불가 주소 | 영역별 연속 주소 배정 | 경로표 규모 절감 |
| Hello 조건 불일치 | 영역·타이머·인증 자동 대조 | 인접 형성 안정화 |
...
- **변화 범위·주소 집약·백본 연결을 반영한 OSPF 영역 설계**
