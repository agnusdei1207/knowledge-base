...
- **영역** 안에서 LSDB의 토폴로지 정보를 동기화한다.
- **플러딩**으로 새 LSA를 신속히 전파한다.
- **영역** 분할로 LSA 전파와 SPF 계산 범위를 제한한다.
...
| 구성요소 | 책임 |
|:---|:---|
| OSPF 이웃 | **Hello**로 **인접 관계**와 상태 유지 |
| OSPF 프로세스 | **LSA** 플러딩•동기화 제어 |
| LSDB | **LSDB**에 영역의 링크 상태 토폴로지 저장 |
| SPF 계산기 | **SPF**로 최단 경로 트리•다음 홉 계산 |
| 영역 경계 라우터 | **ABR**이 영역 연결과 **경로 요약** 수행 |
...
- **데이터베이스 요약**: 이웃에게 보유 LSA 목록을 전달하여 동기화 차이를 식별하는 절차이다.
- **요청 LSA**: 누락되거나 오래된 링크 상태 광고를 이웃에게 요구하는 절차이다.
- **수신 LSA**: 이웃에게 받은 최신 링크 상태 광고를 LSDB에 반영하는 절차이다.
- **토폴로지 상태**: 동기화한 LSDB를 입력으로 자신을 루트로 한 SPF 계산을 수행하는 단계이다.
- **최선 경로**: SPF 결과에서 목적지별 최소 비용 다음 홉을 고르는 단계이다.
...
**데이터베이스 동기화**를 마치면 다음 절차로 **최선 경로 설치**를 수행한다.

1. **데이터베이스 요약**: 보유 LSA 목록으로 동기화 차이를 식별한다.
2. **요청 LSA**: 누락되거나 오래된 링크 상태 정보를 전달한다.
3. **수신 LSA**: 최신 순번의 광고로 영역 토폴로지를 갱신한다.
4. **토폴로지 상태**: 자신을 루트로 SPF 계산을 시작한다.
5. **최선 경로**: 목적지별 최소 비용 다음 홉을 경로표에 반영한다.
...
| 공유 링크의 인접 관계가 과다 | **DR**•**BDR** 선출 상태 확인 | LSA 교환 관계 축소 |
| **MTU 불일치**로 데이터베이스 교환 중단 | 인접 전 **Hello** 조건과 MTU 자동 대조 | 인접 형성 실패 예방 |
...
- **영역 설계**로 LSA•SPF 범위를 나누고 **영역 결정**에 따라 모든 영역을 **영역 0**에 연결한다.
