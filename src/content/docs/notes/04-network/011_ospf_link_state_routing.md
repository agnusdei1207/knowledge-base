...
- **OSPF**: 링크 상태를 공유하고 각 라우터가 최단 경로를 계산하는 내부 라우팅 프로토콜이다.
- **IGP**: 하나의 자율 시스템 내부에서 경로를 교환하는 라우팅 프로토콜이다.

</details>

- 정의/개념: **OSPF**는 자율 시스템 내부 라우터가 LSA로 링크 상태 데이터베이스를 동기화하고 SPF로 최단 경로를 계산하는 **링크 상태 IGP**
...
- **LSA·LSDB·SPF**: 링크 상태 광고, 영역의 공통 토폴로지 정보, 최단 경로 계산 알고리즘이다.
...
- **ABR·경로 요약**: 영역 0과 다른 영역을 연결하고 여러 프리픽스를 공통 상위 프리픽스로 묶는 기능이다.
...
- **OSPFv2·OSPFv3**: IPv4 프리픽스를 다루는 버전과 IPv6·링크 로컬 인접 관계를 지원하는 버전이다.
...
- **영역 0·DR/BDR**: 영역 간 연결의 백본과 공유 링크에서 LSA 교환 관계를 줄이는 대표·예비 라우터이다.
- **MTU 불일치**: 이웃 간 최대 전송 단위가 달라 데이터베이스 교환이 완료되지 않는 상태이다.
