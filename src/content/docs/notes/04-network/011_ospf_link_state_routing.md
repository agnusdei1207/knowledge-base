...
- **영역(Area)** 단위를 정의하는 계층적 라우팅 아키텍처 적용으로 대규모 망에서의 SPF 재연산 부하 최소화.
- 토폴로지 변경 발생 시 해당 LSA를 **플러딩(Flooding)**하여 수 초 이내의 획기적인 고속 라우팅 수렴(Fast Convergence) 달성.
...
| 구성요소 | 역할 및 세부 기능 | 비고 |
|:---|:---|:---|
| **OSPF Neighbor / Adjacency** | **헬로 패킷(Hello)**을 이용해 Hello/Dead Interval, Area ID, Subnet 일치 검증 후 Adjacency 형성 | 2-Way -> Full State 전이 |
...
| **ABR (Area Border Router)** | Area 0과 서브 Area 간 경계에서 LSA Filtering 및 **경로 요약(Summarization)** 집행 | 백본 연동 필수 |
