...
sidebar:
  ...
  badge:
    text: "기출 · 50%"
    variant: note
title: "OSF 링크 상태 라우팅 (OSPF Protocol)"
...
```text
[OSPF 2계층 계층적 다중 Area 토폴로지 구조]
|-- Backbone Area 0 (중앙 코어 백본 영역: 모든 트래픽 중계의 중심)
|   |-- Core Backbone Routers
|   |-- ABR 1 (Area Border Router: Area 1 <-> Area 0 연결, Type 3 Summary LSA)
|   `-- ABR 2 (Area Border Router: Area 2 <-> Area 0 연결, Type 3 Summary LSA)
|-- Standard Area 1 & Area 2 (일반 영역: 내부 라우터 IR 상호 연결)
`-- ASBR Router (AS Boundary Router: 외부 BGP/Static 경로 재분배, Type 5 External LSA)
```
...
#### 한줄 요약
- Hello 발견 → DBD 목차 교환 → LSR/LSU 동기화 → Full 인접성 수립 → SPF 최단 경로 산출 순으로 진행된다.
...
- 대규모 엔터프라이즈 및 데이터센터 내부망의 고성능 라우팅을 구현하기 위해 **OSPF의 계층적 Area 0 백본 설계와 경로 요약(Summarization)을 전사 표준화**하고, **BFD 초고속 장애 감지와 IPv6 듀얼스택 지원 OSPFv3**를 결합하여 확장 가능하고 견고한 내부 라우팅 인프라 완성
