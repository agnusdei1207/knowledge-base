...
- 정의/개념: **OSPF** — 자율 시스템 내부 라우터가 LSA로 링크 상태 데이터베이스를 동기화하고 SPF로 최단 경로를 계산하는 **링크 상태 IGP**이다.
- 배경/필요성: 거리 벡터의 **느린 수렴•루프 한계** 해소가 필요하다.
...
- 영역 내 LSDB의 **토폴로지 정보 동기화**가 핵심이다.
- LSA 플러딩의 **링크 변화 신속 전파**가 핵심이다.
- 영역 분할의 **LSA•SPF 계산 범위 제한**이 핵심이다.
...
- 변경 빈도•연속 프리픽스로 **OSPF 영역**을 나누고 **영역 0**에 연결한다.
