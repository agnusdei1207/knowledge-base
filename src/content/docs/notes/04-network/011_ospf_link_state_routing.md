...
- 배경/필요성: 거리 벡터(RIP)는 이웃이 요약해 준 거리만 믿는 대가로 **토폴로지 변경마다 홉 수를 되풀이 광고하며 수렴을 기다리는 비용**을 치렀고 그 사이 Count-to-Infinity 루프에 노출됐으므로, 요약된 거리를 믿는 대신 각 라우터가 LSA 플러딩으로 동일한 LSDB 사본을 확보하고 자기 관점에서 SPF를 직접 계산하도록 전역 지도 복제 방식으로 갈아탈 필요
