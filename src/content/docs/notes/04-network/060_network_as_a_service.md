...
- **NaaS**: 연결·보안·품질 기능을 API로 주문·변경하고 구독·사용량으로 이용하는 네트워크 서비스이다.
...
- **SLA·사용량 과금**: 서비스 품질과 미달 책임을 계약하고 실제 사용 기간·등급·사용량으로 비용을 산정하는 방식이다.
...
- **SD-WAN·SASE**: SD-WAN은 광역 경로를 선택하고 SASE는 네트워크 연결과 클라우드 보안을 서비스 에지에서 통합한다.
...
- **NaaS·자체 구축·관리형 네트워크**: NaaS는 서비스를 구독하고 자체 구축은 자산을 소유하며 관리형은 보유 자산의 운영을 위탁한다.
- **공급자 종속**: 전용 API·구성 모델·서비스 기능 때문에 다른 공급자로 이전하기 어려워지는 상태이다.
...
- **종단 측정점**: 이용자 접속 양 끝에서 실제 가용성·지연·손실을 측정해 SLA 책임을 판정하는 위치이다.
...
- **네트워크 제공 방식**: 수요 변동·자산 통제·운영 역량을 기준으로 NaaS·자체 구축·관리형 중 운영 모델을 정하는 분류이다.
