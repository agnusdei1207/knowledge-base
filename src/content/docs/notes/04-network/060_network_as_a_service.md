...
- 정의/개념: 연결•보안•품질 기능을 API로 주문•변경하는 **구독형 네트워크 서비스**이다.
- 배경/필요성: 자체 구축은 **조달•개통 지연과 초기 투자 부담**이 발생한다.
...
- **서비스 카탈로그•API 기반 주문•변경•해지 자동화**가 핵심이다.
- **오케스트레이터 기반 연결•보안 기능의 종단 경로 구성**이 핵심이다.
- **SLA•사용량 과금에 따른 품질 책임 명시와 비용 변동**이 핵심이다.
...
1. **서비스 의도**: 이용자 권한과 정책 충돌을 검증한 주문을 전달한다.
2. **경로•기능 설정**: 회선•SD-WAN•SASE 자원을 연결한다.
3. **품질•사용량 지표**: 실제 종단 경로의 SLA 지표와 사용량을 제공한다.
4. **SLA•과금 상태**: 품질 위반 여부와 과금량 산정 결과를 전달한다.
5. **서비스 상태**: 개통 상태•품질•예상 비용을 포털에 제공한다.
...
- 수요 변동은 **NaaS**, 자산 통제는 **자체 구축**, 운영 인력 부족은 관리형을 선택한다.
