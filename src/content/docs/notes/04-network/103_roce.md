...
sequenceDiagram
    ...
    송신RNIC->>스위치: 5. 송신률 조정 전송
```

| 절차 | 설명 |
|:---|:---|
| RoCE 큐 분류 | 트래픽을 지정 우선순위에 매핑 |
| 다중 경로 전달 | 경로 해시로 흐름을 분산 |
| ECN 혼잡 표시 | 큐 임계값에서 패킷에 표시 |
| CNP 혼잡 피드백 | 수신 RNIC가 혼잡을 송신자에 알림 |
| 송신률 조정 전송 | 혼잡 정도에 맞춰 속도를 낮춤 |
...
- **라우팅 규모·손실률·PFC 위험**으로 RoCE 방식 결정
