...
1. 인공지능 학습 패브릭의 **RoCEv2 혼잡 제어**

### 쉽게 이해하기 (학습용)

- 스위치 버퍼가 손실 구간에 이르기 전에 ECN을 표시하고 송신 서버가 전송률을 낮추도록 임계값을 조정한다.

## Ⅶ. 결론

- 이더넷에서 저지연 RDMA를 제공하면서 혼잡 확산을 막기 위해 **라우팅 규모·ECN 임계값·PFC 교착·버퍼·테넌트 격리**를 검토하고, 손실과 혼잡을 통제할 수 있는 구간에 RoCE를 적용해야 한다.
