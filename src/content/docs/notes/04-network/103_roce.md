...
- 배경/필요성: TCP 소켓의 복사•커널 처리로 **CPU 부하•지연이 증가한다**.
...
1. **RoCE 큐 분류**: 트래픽을 지정 우선순위에 매핑한다.
2. **다중 경로 전달**: 경로 해시로 흐름을 분산한다.
3. **ECN 혼잡 표시**: 큐 임계값에서 패킷에 표시한다.
4. **CNP 혼잡 피드백**: 수신 RNIC가 송신자에게 혼잡을 통지한다.
5. **송신률 조정**: 혼잡 정도에 맞춰 전송 속도를 감소한다.
#### 한줄 요약
...
- 단일 2계층은 **RoCEv1**, 라우팅 패브릭은 **RoCEv2**을 선택한다.
