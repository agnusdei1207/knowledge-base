...
- **자동 재전송 요구(Automatic Repeat reQuest, ARQ)**: 확인 응답과 타임아웃으로 유실·오류 프레임을 재전송하는 오류 제어 방식
- **긍정 확인 응답(Acknowledgment, ACK)**: 프레임의 정상 도착을 송신 측에 알리는 응답
- **부정 확인 응답(Negative Acknowledgment, NAK)**: 프레임 손상을 송신 측에 알리고 재전송을 요구하는 응답
