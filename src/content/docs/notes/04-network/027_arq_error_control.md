...
- 정의/개념: **ARQ** — ACK•NAK•순서번호•타임아웃을 이용해 유실되거나 오류가 난 프레임을 송신자가 재전송하게 하는 **오류 제어 방식**이다.
- 배경/필요성: 오류 검출만으로는 **유실•손상 데이터 복구할 수 없다**.
...
- 순서 번호의 **신규•재전송 프레임 구분**이 핵심이다.
- ACK•NAK•타임아웃의 **재전송 시점 결정**이 핵심이다.
- 큰 송수신 윈도의 **처리량 향상•버퍼 증가**가 핵심이다.
...
3. **NAK**: 오류가 검출된 프레임의 **재전송**을 요구한다.
4. **타임아웃 신호**: ACK 부재를 **프레임 유실**로 판정한다.
...
- 오류율이 낮고 버퍼가 작으면 **Go-Back-N**, 오류율•RTT가 크면 **SR ARQ**가 핵심이다.
