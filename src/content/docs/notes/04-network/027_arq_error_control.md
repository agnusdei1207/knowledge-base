...
- RTT가 길고 오류율이 높으면 선택 재전송으로 유실 프레임만 재전송하고, 수신 버퍼가 부족하면 Go-Back-N으로 후속 프레임을 일괄 재전송한다.
...
- 오류율이 낮고 수신 버퍼가 작으면 Go-Back-N으로 후속 프레임을 일괄 재전송하고, 오류율이 높고 RTT가 크면 SR로 유실 프레임만 선택 재전송한다.
