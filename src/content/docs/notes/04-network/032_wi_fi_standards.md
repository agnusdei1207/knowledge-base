...
- 정의/개념: **IEEE 802.11**은 무선 LAN(WLAN)을 구현하기 위한 물리(PHY) 계층 및 매체 접근 제어(MAC) 계층의 글로벌 무선 통신 표준 규격이다.
- 배경/필요성: 제조사 간 상호 운용성 확보와 주파수 채널 경합 관리를 통한 전파 간섭 최소화 및 고속 무선 데이터 전송 수요 증대에 대응하기 위해 표준화되었다.

#### 한줄 요약

- 무선 단말과 AP 간 무선 매체 공유 및 데이터 전송을 정의하는 IEEE 표준 규격.
...
- 하위 호환성을 보장하며 주파수 대역별 전파 특성과 CSMA/CA 경합 오버헤드에 의해 실효 속도가 결정됨.
...
- 단말과 AP가 무선 매체를 경합 공유하고 DS를 통해 유선 백본 망으로 라우팅되는 무선 LAN 구조.
...
- 비콘 탐색 후 인증 및 결합 절차를 거쳐 통신 파라미터를 확정하고 무선 데이터 통신을 개시함.
...
- Wi-Fi 5의 단일 대역 고속화에서 Wi-Fi 6의 OFDMA 고밀도 효율화를 거쳐 Wi-Fi 7의 MLO·320MHz 초저지연화로 발전.
...
- BSS Coloring, WPA3 보안, Band Steering 및 Fast Roaming을 도입하여 고밀도 실무 무선 LAN 품질 확보.
...
- 무선 LAN 구축 시 트래픽 특성, 단말 밀도, 주파수 대역에 맞춘 **최적 Wi-Fi 세대 선정**과 **BSS Coloring 기반 채널 설계 필수**.

#### 한줄 요약

- 요구 성능 및 환경 특성에 맞춘 최적 Wi-Fi 세대 표준 선정 및 채널 재사용 설계 필수.
