...
- 정의/개념: **IEEE 802.11**로 물리 전송과 매체 접근을 규정하는 **무선랜 표준군**이다.
- 배경/필요성: 제조사별 독자 규격으로 단말•AP **상호운용•전파 공유할 수 없다**.
...
- **세대 간 하위 호환**으로 공통 기능 협상이 핵심이다.
- **2.4•5•6GHz**별 도달 거리•채널 용량 차이 핵심이다.
- 규제•간섭•단말 역량에 따른 **실효 처리량**을 제한한다.
...
1. **비콘**: AP가 **SSID•지원 기능**과 동기화 정보를 광고한다.
2. **인증 요청**: 단말이 지원 인증 방식으로 **접속 신원**을 제시한다.
3. **인증 응답**: AP가 인증 결과와 **후속 연결 조건**을 회신한다.
4. **연결 요청**: 단말이 대역•속도•보안 **지원 기능**을 제안한다.
5. **연결 응답**: 공통 기능과 **연결 식별자**를 확정한다.
...
- 다단말 혼잡은 **Wi-Fi 6•6E**, 저지연•다중 링크는 **Wi-Fi 7**을 선택한다.
