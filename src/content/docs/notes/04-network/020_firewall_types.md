...
- **차세대 방화벽(Next-Generation Firewall, NGFW)**: 응용·사용자·패킷 내용·위협 정보를 함께 식별해 정책을 적용하는 방화벽
- **접근 제어 목록(Access Control List, ACL)**: 주소·포트·프로토콜·방향별 허용·차단 조건을 순서대로 기록한 규칙표
- **상태 테이블(State Table)**: 허용된 연결의 양방향 주소·포트와 진행 상태·만료 시간을 저장한 표
- **심층 패킷 검사(Deep Packet Inspection, DPI)**: 전송 계층 헤더를 넘어 응용 데이터와 위협 패턴까지 해석하는 검사
...
- **정의/개념**: 구간 간 트래픽을 보안 정책으로 통제
- **배경/필요성**: 최소 허용 원칙 집행과 네트워크 경계 격리
...
- 기본 차단은 허용 근거가 없는 통신을 자동 거부한다.
- 상태 추적은 허용 연결의 응답 패킷만 통과한다.
- DPI는 응용 식별을 높이지만 지연·처리 비용을 증가한다.
- 첫 일치 규칙이 판정을 끝내므로 규칙 순서가 결과를 변경한다.
...
| 판단 기준 | 패킷 필터 | 상태 기반 | NGFW |
|:---|:---|:---|:---|
| 핵심 특징 | 패킷별 주소·포트 검사 | 연결 상태와 패킷 방향 추적 | 응용·사용자·위협 내용 분석 |
| 적용 기준 | 단순·고속 경계 통제 | 연결 요청·응답 관계 통제 | 응용·위협별 세밀한 통제 |
| 주요 위험 | 위조·우회 문맥 미식별 | 상태 테이블 고갈 | 응용 내용 검사 처리량 병목 |
...
2. 임시 허용 규칙은 소유자·사유·만료일을 기록

### 쉽게 이해하기 (학습용)

- 외부 사용자는 공개 웹 서버까지만 들어오게 하고 그 연결이 내부 서버로 직접 이어지지 않게 구간을 나눈다
- 잠깐 연 예외 규칙은 책임자와 종료일이 없으면 계속 남으므로 만료 시 자동으로 제거한다

## Ⅶ. 결론

- 신뢰 구간·필요 통신·검사 깊이로 방화벽 선택
