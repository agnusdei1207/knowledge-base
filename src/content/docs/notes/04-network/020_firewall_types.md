...
block
    columns 1
    A["보안 존 | 출발·목적 구간과 정책 방향을 정의"]
    B["규칙 집합 | 허용·차단 조건과 평가 순서를 규정"]
    C["상태 테이블 | 허용 연결의 양방향 튜플·상태 저장"]
    D["위협 엔진 | 응용·공격 패턴을 식별"]
    E["정책 로그 | 적용 규칙과 판정 결과를 기록"]
    A --- B
    B --- C
    C --- D
    D --- E
...
| 암호화 트래픽 미가시 | 복호화 범위·예외 기준 설정 | 위협 탐지 향상 |
...
- **보안 존·상태·응용 위험을 반영한 최소 허용 방화벽 정책**
