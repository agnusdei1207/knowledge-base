...
- 정의/개념: **방화벽** — 서로 다른 신뢰 구간 사이의 트래픽을 주소•연결 상태•응용 내용과 보안 정책에 따라 허용하거나 차단하는 **네트워크 보안 통제 장비**이다.
- 배경/필요성: 무통제 경계의 **비인가 접근•공격 확산**이 발생한다.
...
- 기본 차단의 **명시 허용 외 통신 거부**가 핵심이다.
- 상태 추적의 **허용 연결 응답 판별**이 핵심이다.
- DPI의 **응용•위협 내용 식별**이 핵심이다.
...
- 연결 상태 통제는 **상태 기반**, 응용 위협 식별은 **NGFW**을 선택한다.
