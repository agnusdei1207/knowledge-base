...
- 배경/필요성: 1세대 단순 패킷 필터링의 한계로 인한 **동적 포트 기반 세션 추적 불가, 80/443 포트로 은닉 침투하는 L7 애플리케이션 위협 방어 불가**
...
#### 한줄 요약
- 보안 존 분리, 상태 테이블 세션 추적, Top-Down 순차 ACL 평가, DPI 엔진이 결합된다.
...
- 진화하는 지능형 사이버 위협으로부터 엔터프라이즈 자원을 수호하기 위해 **Default Deny 원칙과 DMZ 기반 보안 존 분리를 표준 수립**하고, **차세대 방화벽(NGFW)의 App-ID 식별, SSL 복호화 검사 및 IPS 연동**을 결합하여 제로 트러스트 다계층 경계 보안 아키텍처 완성
