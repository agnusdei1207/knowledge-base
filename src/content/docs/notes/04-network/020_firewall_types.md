...
date: "2026-07-31T11:16:48+09:00"
tags:
  - "notes-network"
weight: 20
extra:
...
- 정의/개념: **방화벽**은 서로 다른 신뢰 구간 사이의 트래픽을 주소•연결 상태•응용 내용과 보안 정책에 따라 허용하거나 차단하는 **네트워크 보안 통제 장비**
...
- **기본 차단·상태 추적·DPI**: 명시적 허용 외 통신을 거부하고 연결 상태와 응용 내용을 검사하는 기능이다.
...
- **보안 존·ACL·상태 테이블**: 정책 방향을 적용할 구간, 순서화된 허용·차단 규칙, 허용 연결 상태를 저장한 표이다.
...
- **정책 판정**: 보안 존·연결 상태·첫 일치 규칙·위협 검사 결과를 결합해 허용 여부를 결정하는 과정이다.
...
- **패킷 필터·상태 기반·NGFW**: 패킷 필드, 연결 문맥, 응용·사용자·위협 정보를 각각 기준으로 통제하는 방화벽이다.
...
- **DMZ·규칙 그림자**: 공개 서버를 내부망과 분리한 구간과 앞선 광범위 규칙 때문에 뒤 규칙이 적용되지 않는 현상이다.
...
- **검사 범위 최소화**: 필요한 주소·연결·응용 문맥까지만 검사해 통제 정밀도와 처리 비용을 균형화하는 원칙이다.
