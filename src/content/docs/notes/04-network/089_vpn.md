...
date: "2026-07-31T11:06:14+09:00"
tags: ["notes-network"]
weight: 89
extra:
  question_no: "089"
...
- **분할 터널**: 선택한 목적지 트래픽만 가상 사설망(Virtual Private Network, VPN)으로 보내고 나머지는 일반 인터넷 경로로 보내는 방식
...
- **가상 사설망(Virtual Private Network, VPN) 게이트웨이**: 터널을 종단해 패킷을 복호화하고 접근·경로 정책을 적용하는 장치
...
- **보안 연관(Security Association, SA)**: 인터넷 프로토콜 보안(Internet Protocol Security, IPsec) 통신 방향별 알고리즘·키·수명·식별자를 묶은 보안 상태
...
- **IPsec·TLS VPN**: 인터넷 프로토콜 보안(Internet Protocol Security, IPsec)은 인터넷 프로토콜(Internet Protocol, IP) 패킷 단위의 망 연결을, 전송 계층 보안(Transport Layer Security, TLS) 가상 사설망(Virtual Private Network, VPN)은 사용자별 웹·응용 접근을 보호하는 방식
...
- **경로 최대 전송 단위(Path Maximum Transmission Unit, Path MTU)·최대 세그먼트 크기(Maximum Segment Size, MSS)**: 경로 MTU는 분할 없는 최대 패킷 크기이고 MSS는 전송 제어 프로토콜(Transmission Control Protocol, TCP) 세그먼트에 담을 수 있는 최대 데이터 크기
- **재키잉**: 기존 터널이 동작하는 동안 새 세션 키로 교체해 키 노출 시간을 줄이는 절차이다.
- **인터넷 프로토콜 보안(Internet Protocol Security, IPsec)·보안 연관(Security Association, SA)**: 터널 패킷을 보호하고 알고리즘·키·수명을 관리하는 방식과 상태
...
- **가상 사설망(Virtual Private Network, VPN) 방식 선택**: 보호할 네트워크·응용 범위와 운영 복잡도·단말 이동성을 비교해 인터넷 프로토콜 보안(Internet Protocol Security, IPsec)·전송 계층 보안(Transport Layer Security, TLS)·WireGuard를 결정하는 과정
