...
- 정의/개념: IEEE 802 이더넷에 **시간 동기•트래픽 조정•신뢰성**을 적용해 지연 상한을 보장하는 **결정형 네트워크 표준군**이다.
- 배경/필요성: 최선형 이더넷은 혼잡 시 **제어 트래픽 지연 상한 보장할 수 없다**.
...
- **지연 상한**: 예약 조건으로 최악 도착 시간을 계산한다.
- **트래픽 통합**: 실시간•일반 흐름을 표준 이더넷에 수용이다.
- **IEEE 802.1AS**: 공통 시각으로 GCL 실행을 정렬한다.
- **CNC**: 스트림 요구와 토폴로지로 경로•GCL을 계산한다.
...
1. **스트림 요구 등록**: 주기•크기•지연•손실 한계를 전달한다.
2. **경로 기능 확인**: 장비별 동기•조정•선점 지원을 검사한다.
3. **경로•GCL 배포**: 대역폭과 게이트 시각을 일괄을 설정한다.
4. **예약 스트림 전송**: 공통 시각과 큐 정책으로 전달한다.
5. **종단 성능 검증**: 혼잡•장애 조건의 지연 상한을 확인한다.
...
- 고정 주기•마감은 **TAS**, 비주기 버스트는 **ATS**, 차단 축소는 **선점**이 핵심이다.
