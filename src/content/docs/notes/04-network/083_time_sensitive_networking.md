...
- **IEEE 802.1AS**: TSN 장비의 시각과 주파수를 공통 기준에 맞추는 시간 동기 표준
- **가드 밴드(Guard Band)**: 예약 게이트가 열리기 전 일반 프레임 전송을 막는 보호 시간
- **동기 오차 예산(Synchronization Error Budget)**: 종단 장비와 브리지의 허용 시계 편차를 배분한 한도
- **수용 제어(Admission Control)**: 새 스트림을 추가해도 기존 지연·대역 요구를 지키는지 판정하는 기능
- **마감시간(Deadline)**: 스트림 프레임이 목적지에 도착해야 하는 최종 시각

> **키워드:** 시간 민감 네트워킹 (Time-Sensitive Networking, TSN)

## Ⅰ. 개요

- 정의/개념: 이더넷에 시간 동기·조정·신뢰성을 더한 **결정형 표준군**
