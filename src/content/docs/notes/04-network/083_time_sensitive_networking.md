...
```text
[TSN 결정론적 네트워크]
  ├── [중앙 제어] ───────── [CNC / CUC (802.1Qcc)]
  │                           │
  ├── [동기화] ─────────── [시간 동기화 엔진 (802.1AS)]
  │                           │
  ├── [트래픽 제어] ─────── [TAS 스케줄러 (802.1Qbv)]
  │                           │
  ├── [대기시간 단축] ───── [프레임 선점기 (802.1Qbu)]
  │                           │
  └── [무손실 신뢰성] ───── [FRER 복제기 (802.1CB)]
```
...
- 과거 독점적 산업용 필드버스(EtherCAT, PROFINET)와 차량용 CAN/FlexRay로 파편화되었던 실시간 제어망을 단일 표준 이더넷으로 통합하는 **차세대 스마트 제조(OPC UA over TSN), SDV 차량 백본 및 5G/6G 프론트홀의 핵심 결정론적 네트워크 인프라 표준**으로 확립되었으며, 실무 TSN 시스템 구축 시에는 **비실시간 프레임에 의한 윈도우 침범을 방지하고 가드 밴드 낭비를 제거하는 IEEE 802.1Qbu 프레임 선점 결합, 물리 단선 시 0ms 무중단 전환을 보장하는 IEEE 802.1CB FRER 이중화, 100홉 경유 시에도 1μs 이내 정밀도를 유지하는 하드웨어 타임스탬핑 PHY(802.1AS) 채택, 중앙 집중형 CNC(802.1Qcc)를 통한 무충돌 GCL 시간표 자동 연산**을 결합하여 완벽한 결정론적 전송 품질을 완성
