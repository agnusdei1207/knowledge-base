...
- **게이트 제어 목록(Gate Control List, GCL)**: 큐별 전송 게이트의 개폐 시각을 정의한 시간표
- **중앙 네트워크 구성기(Centralized Network Configuration, CNC)**: 스트림 경로•대역폭•GCL을 계산하는 중앙 기능
...
- **TSN 브리지(TSN Bridge)**: 결정형 큐•게이트•선점 제어를 실행하는 스위치
...
- **시간 인식 조정기(Time-Aware Shaper, TAS)**: 공통 시간표로 큐 게이트를 제어하는 방식
- **비동기 트래픽 조정기(Asynchronous Traffic Shaper, ATS)**: 시간 동기 없이 흐름별 전송률을 조정하는 방식
