...
- 배경/필요성: 전용 장비 구조에서는 무선 기능의 자원 공유 및 부하별 탄력 확장이 어렵다.
...
1. **프런트홀 신호 전달**: **프런트홀**에서 **프런트홀 신호 전달**을 수행한다.
2. **무선 연산 처리 요청**: DU가 가속기에 **무선 연산 처리 요청**을 보낸다.
3. **상위 무선 데이터 전달**: DU가 CU에 **상위 무선 데이터 전달**을 수행한다.
4. **서비스 품질 보고**: **CU•DU 품질 환류**로 **서비스 품질 보고**를 제공한다.
5. **격리 자원 재배치**: 시한 위반 시 **격리 자원 재배치**를 수행한다.
