...
- 정의/개념: **RAN** **CU**•**DU** 기능을 **범용 서버•가속기**에서 실행하는 소프트웨어 구조이다.
- 배경/필요성: 전용 장비에는 **자원 공유•탄력 확장 한계** 있다.
...
- **CU 소프트웨어 처리**•**DU 소프트웨어 처리** 독립 배치•확장이 핵심이다.
- 전용 자원과 **무선 가속기** 기반 처리 시한•지터를 통제한다.
...
| 구성요소 | 책임 |
|:---|:---|
| 운영 제어기 | **운영 제어기** 품질•자원 감시와 배치 결정 |
| 클라우드 플랫폼 | **클라우드 플랫폼** 배포•격리•확장•복구 실행 |
| 가상 CU | **가상 CU** 상위 무선 계층•코어망 연동 |
| 가상 DU | **가상 DU** 실시간 제어•상위 물리 처리 |
| 범용 컴퓨팅 | **범용 컴퓨팅** **CPU 자원**•메모리•네트워크 제공 |
| 무선 가속기 | **무선 가속기** 시간 민감 무선 연산 제공 |
| 무선 장치 | **무선 장치** 전파 변환•안테나 연결 |
...
1. 프런트홀 신호 전달: RU의 디지털 무선 신호를 **프런트홀** 기반 DU에 전달한다.
2. 무선 연산 처리 요청: DU가 시간 민감 연산을 **무선 가속기** 대상 위임한다.
3. 상위 무선 데이터 전달: DU의 처리 결과를 **가상 CU** 상위 계층에 전달한다.
4. 서비스 품질 보고: 처리 지연•무선 품질을 **CU•DU 품질 환류** 기반 보고한다.
...
- **vRAN 적용 조건** 충족하면 **vRAN** 적용하고, 미충족 시 **전용 기지국** 유지한다.
