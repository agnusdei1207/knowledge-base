...
- **범용 서버•가속기**: 표준 컴퓨팅 자원과 시간 민감 무선 연산 전용 장치를 결합한 실행 기반이다.
- **자원 공유•탄력 확장 한계**: 전용 장비에서 여러 무선 기능이 컴퓨팅 자원을 공동 사용하거나 부하에 따라 늘고 줄기 어려운 문제이다.

</details>

- 정의/개념: **RAN**의 **CU**•**DU** 기능을 **범용 서버•가속기**에서 실행하는 소프트웨어 구조이다.
- 배경/필요성: 전용 장비에는 **자원 공유•탄력 확장 한계**가 있다.
...
- **DU 소프트웨어 처리**: 시간 민감 제어•물리 처리를 격리된 자원에서 실행하는 방식이다.
- **클라우드 오케스트레이션**: 소프트웨어 CU•DU의 배포•확장•복구를 자원 상태와 정책에 따라 자동화하는 기능이다.

</details>

- **CU 소프트웨어 처리**•**DU 소프트웨어 처리**의 독립 배치•확장이 핵심이다.
- 전용 자원과 **무선 가속기**로 처리 시한•지터를 통제한다.
...
- **가상 CU(Virtualized CU)**: 상위 무선 계층과 코어망 연동을 소프트웨어로 수행하는 구성요소이다.
- **가상 DU(Virtualized DU)**: 실시간 제어와 상위 물리 처리를 소프트웨어로 수행하는 구성요소이다.
- **CPU 자원**: 범용 서버에서 소프트웨어 무선 기능을 실행하는 컴퓨팅 자원이다.
- **범용 컴퓨팅**: 표준 CPU•메모리•네트워크 자원을 소프트웨어 무선 기능에 제공하는 상용 서버 기반이다.
- **무선 장치**: 하위 물리 처리와 무선주파수 변환 및 안테나 연결을 수행하는 장치이다.
...
가지의 의미: 운영•실행•무선 기능•자원 책임을 분리한 정적 구조이다.

| 구성요소 | 책임 |
|:---|:---|
| 운영 제어기 | **운영 제어기**가 품질•자원 감시와 배치 결정 |
| 클라우드 플랫폼 | **클라우드 플랫폼**이 배포•격리•확장•복구 실행 |
| 가상 CU | **가상 CU**가 상위 무선 계층•코어망 연동 |
| 가상 DU | **가상 DU**가 실시간 제어•상위 물리 처리 |
| 범용 컴퓨팅 | **범용 컴퓨팅**이 **CPU 자원**•메모리•네트워크 제공 |
| 무선 가속기 | **무선 가속기**가 시간 민감 무선 연산 제공 |
| 무선 장치 | **무선 장치**가 전파 변환•안테나 연결 |
...
- **CU•DU 품질 환류**: 처리 지연과 무선 품질을 운영 제어기에 전달하는 환류이다.
- **격리 자원 재배치**: 시한 위반 기능에 전용 CPU•가속기를 다시 할당하는 조치이다.
- **프런트홀 신호 전달**: 무선 장치의 디지털 신호를 DU에 제공하는 단계이다.
- **무선 연산 처리 요청**: DU가 시간 민감 고부하 연산을 가속기에 위임하는 단계이다.
- **상위 무선 데이터 전달**: DU가 처리한 데이터를 CU에 제공하는 단계이다.
...
1. **프런트홀 신호 전달**: RU의 디지털 무선 신호를 **프런트홀**로 DU에 전달한다.
2. **무선 연산 처리 요청**: DU가 시간 민감 연산을 **무선 가속기**에 위임한다.
3. **상위 무선 데이터 전달**: DU의 처리 결과를 **가상 CU**의 상위 계층에 전달한다.
4. **서비스 품질 보고**: 처리 지연•무선 품질을 **CU•DU 품질 환류**로 보고한다.
5. **격리 자원 재배치**: 시한 위반 기능에 전용 CPU•가속기를 **재배치**한다.
...
- **전용 기지국**: 무선 기능을 전용 하드웨어 장비에 결합해 실행하는 방식이다.
...
| 한계 | 확장 지연•장비 종속 | 자원 낭비•기동 지연 | 격리 부족•**처리 지터** 증가 |
...
- **vRAN 적용 조건**을 충족하면 **vRAN**을 적용하고, 미충족 시 **전용 기지국**을 유지한다.
