...
- **분산 콘텐츠 전송망**: 글로벌 CDN은 여러 지역의 엣지 거점이 원본 콘텐츠를 캐시·전달해 원거리 지연과 원본 집중을 줄이는 분산 콘텐츠 전송망이다.

</details>

- 정의/개념: **글로벌 CDN**은 여러 지역의 근접 엣지가 원본 콘텐츠를 캐시·전달해 사용자 지연과 원본 부하를 줄이는 **분산 콘텐츠 전송망**
...
- **캐시 적중·실패**: 캐시 적중은 엣지가 객체를 즉시 제공하는 상태이고 실패는 보호 캐시나 원본에 상위 요청해야 하는 상태다.
...
- **원본 보호 캐시**: 원본 보호 캐시는 여러 엣지에서 발생한 같은 객체의 실패 요청을 병합해 원본의 중복 처리와 폭주를 줄인다.
...
- **2. 실패 요청 병합**: 보호 캐시는 같은 객체를 동시에 요구하는 여러 엣지 요청을 하나의 원본 재검증으로 합친다.
...
- **글로벌 CDN**: 글로벌 CDN은 전 세계 사용자와 대량 콘텐츠에 다지역 라우팅·계층 캐시·공격 완충을 제공한다.
...
- **공유 캐시 노출**: 공유 캐시 노출은 사용자별 개인 응답을 같은 캐시 키로 저장해 다른 사용자에게 전달하는 정보 유출 문제다.
...
- **원본 제공**: 원본 제공은 요청마다 내용이 달라 안전하게 캐시할 수 없는 소규모·개인화 서비스에 적합하다.
