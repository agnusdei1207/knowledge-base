...
- 배경/필요성: 원본 직접 제공은 **원거리 지연•트래픽 집중** 유발한다.
...
- **캐시 적중**•**캐시 실패** 및 **캐시 키**•**TTL** 기반 신선도•사용자 격리가 핵심이다.
...
| 구성요소 | 책임 |
|:---|:---|
| 글로벌 요청 라우터 | **글로벌 요청 라우터** **DNS**•**애니캐스트** 기반 정상 PoP 선택 |
| 엣지 PoP | **엣지 PoP** **TLS**•보안•캐시•콘텐츠 전달 |
| 캐시 정책 엔진 | **캐시 정책 엔진** 캐시 키•TTL과 저장 제외•신선도 규칙 결정 |
| 원본 보호 캐시 | **원본 보호 캐시** 실패 요청 병합•상위 캐시 제공 |
| 원본 서버 | **원본 서버** 정본 콘텐츠•재검증 정보 제공 |
...
1. 캐시 키•신선도 판정: 변형•TTL•저장 가능성의 **캐시 키•신선도 판정** 수행한다.
2. 실패 요청 병합: 같은 객체의 원본 요청에 **실패 요청 병합** 적용한다.
3. 원본 재검증: 변경 여부를 확인하는 **원본 재검증** 기반 최신 객체를 획득한다.
4. 보호 캐시 갱신: 최신 객체의 **보호 캐시 갱신** 수행한다.
#### 한줄 요약
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 개인 응답의 **공유 캐시 노출** | **RFC 9111** 및 비저장 정책 검증 | 사용자 정보 격리 |
...
- 전 세계는 글로벌 CDN, 지역 반복은 지역 캐시, 캐시 불가는 원본 제공으로 **전달 방식 선택** #### 한줄 요약
