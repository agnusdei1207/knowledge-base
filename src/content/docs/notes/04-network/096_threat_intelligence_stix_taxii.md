...
- 정의/개념: STIX로 표현하고 TAXII로 교환하는 **CTI 표준 체계**이다.
- 배경/필요성: 비정형 문서의 **자동 연계할 수 없다**.
...
- **STIX 객체•관계**: 공격자•기법•지표의 위협 맥락 표현이다.
- **TAXII 컬렉션**: 조직•도구 간 위협 정보 자동 교환한다.
- **집행 통제**: 신뢰도•TLP•유효 기간 기반을 적용한다.
...
- **TAXII 서버**: 컬렉션별 조회•게시•접근 권한을 제공하는 서버이다.
...
| 구성요소 | 책임 |
|:---|:---|
| 위협 검증•보강기 | **출처•신뢰도•수명•관계** 확인 |
| STIX 객체 저장소 | **개체•관계•공유 표시** 보존 |
| TAXII 교환 서버 | **컬렉션별 조회•게시•권한** 제공 |
| 보안 도구 소비자 | **내부 형식 변환•탐지•차단** |
| 품질 환류기 | **적중•오탐 기반 갱신•폐기** |
...
1. **위협 근거 전달**: 출처•시각•관측 사실을 생산자에 제공한다.
2. **STIX 객체 등록**: 지표•기법•공격자 맥락을 연결한다.
3. **컬렉션 객체 제공**: TLP와 조직 범위로 공유를 제한한다.
4. **증분 객체 요청**: 마지막 수신 뒤 변경분만을 조회한다.
5. **품질 결과 환류**: 적중•오탐•만료로 갱신•폐기한다.
...
- 위협 의미는 **STIX**, 권한별 교환은 **TAXII**, 만료•오탐은 즉시 회수가 핵심이다.
