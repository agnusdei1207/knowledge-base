...
- 정의/개념: STIX **위협 표현**과 TAXII **정보 교환**
...
sequenceDiagram
    ...
    participant 보안도구
    정보원->>생산자: 1. 위협 근거 검증
    생산자->>STIX저장소: 2. STIX 객체 생성
    STIX저장소->>TAXII서버: 3. 컬렉션 게시
    보안도구->>TAXII서버: 4. 증분 객체 동기화
    보안도구-->>생산자: 5. 품질 결과 환류
```

**동작 원리**

- **1. 위협 근거 검증**: 근거·시각·중복·신뢰도를 확인
- **2. STIX 객체 생성**: 지표·기법·공격자 맥락을 연결
- **3. 컬렉션 게시**: TLP와 조직 범위로 공유 제한
- **4. 증분 객체 동기화**: 새 객체와 변경분만 수신
...
- **근거·수명·공유 범위**로 STIX·TAXII 집행 결정
