...
- **정의/개념**: STIX는 위협 의미, TAXII는 객체 교환 표준
- **배경/필요성**: 문서형 위협 정보와 자동 방어 도구 연결
...
- 단일 흔적을 공격자·기법·캠페인 맥락과 연결한다.
- 표현 표준과 전달 표준을 분리해 상호운용한다.
- 신뢰도·공유 범위·유효 기간이 집행 수준 결정한다.
- 낡거나 낮은 품질의 자동 배포는 정상 통신 차단 확산한다.
...
| 판단 기준 | STIX | TAXII |
|:---|:---|:---|
| 핵심 특징 | 위협 개체·관계·공유 맥락 표현 | STIX 컬렉션의 조회·게시·동기화 |
| 적용 기준 | 위협 의미를 기계 판독 구조로 표현 | 조직·도구 사이 객체를 자동 교환 |
| 주요 위험 | 관계·수명 누락으로 맥락 상실 | 권한 오류·중복·지연 동기화 |
...
1. 악성 주소를 STIX 지표로 보안 관제에 배포
2. 조직별 TAXII 컬렉션을 TLP로 접근 제한

### 쉽게 이해하기 (학습용)

- 악성 주소의 출처와 유효 기간, 관련 공격 기법을 STIX로 묶어 보안 관제 탐지 규칙에 반영한다.
- 협력사별 컬렉션 권한과 재공유 등급을 달리해 민감한 공격 정보가 허용 범위 밖으로 퍼지지 않게 한다.

## Ⅶ. 결론

- 신뢰도·수명·공유 범위로 STIX·TAXII 집행 결정
