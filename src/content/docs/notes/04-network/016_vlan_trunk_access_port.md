...
- 정의/개념: **VLAN•액세스•트렁크 포트** — 스위치망을 논리적 브로드캐스트 영역으로 나누고 단일 VLAN의 무태그 프레임과 여러 VLAN의 태그 프레임을 각각 전달하는 **2계층 분할 방식**이다.
- 배경/필요성: 물리망 단일 영역의 **브로드캐스트 과다•논리 격리할 수 없다**.
...
- VLAN 식별자의 **방송•MAC 학습 범위 분리**가 핵심이다.
- 액세스 PVID의 **무태그 VLAN 귀속**이 핵심이다.
- 트렁크 802.1Q 태그의 **다중 VLAN 구분**이 핵심이다.
...
- **허용 VLAN 목록(Allowed VLAN List)**: 트렁크에서 전달할 VLAN 집합을 제한하는 설정한다.
...
- 단말 연결은 **액세스**, 다중 VLAN 링크는 **트렁크**를 선택한다.
