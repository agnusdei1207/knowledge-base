...
- 배경/필요성: 단일 물리 LAN에서 도메인을 나누려면 **부서 구성이 바뀔 때마다 배선과 장비를 다시 까는 공사 비용**을 반복 지불해야 했고 그 전까지 브로드캐스트가 전 구간 대역폭을 잠식했으므로, 물리 배선을 그대로 둔 채 프레임에 4바이트 태그를 붙여 논리적 브로드캐스트 도메인을 나눔으로써 도메인 분리 비용을 배선 공사에서 스위치 설정으로 옮길 필요
