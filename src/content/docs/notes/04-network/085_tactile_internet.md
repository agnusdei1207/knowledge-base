...
- **촉각 인터넷**: 사람의 조작과 원격 장치의 힘·촉감 반응을 짧고 예측 가능한 지연으로 왕복 전달하는 폐루프 통신 체계이다.
...
- **왕복 지연·지터**: 왕복 지연은 명령과 반응이 오가는 시간이고 지터는 패킷마다 도착 시간이 달라지는 정도이다.
...
- **햅틱 조작기**: 사용자의 위치·힘 명령을 감지하고 원격 장치의 촉각 반응을 재현하는 장치이다.
- **엣지 제어기**: 사용자와 원격 장치 가까이에서 지연 예측·보간·상태 동기화를 수행하는 장치이다.
...
- **표본화**: 연속적인 위치·힘 신호를 일정 주기의 디지털 값으로 변환하는 처리이다.
- **촉각 상태 피드백**: 원격 장치의 실행 결과와 접촉 힘을 사용자 조작기로 되돌려 주는 정보이다.
...
- **원격·엣지·로컬 제어**: 사람이 직접 명령하거나 엣지가 예측을 보완하거나 현장 장치가 자체 판단하는 제어 위치 분류이다.
- **폐루프 제어**: 명령 실행 결과를 다시 입력으로 받아 다음 명령을 계속 보정하는 제어 방식이다.
...
- **적응형 보간**: 지연·손실 상태에 따라 늦거나 빠진 표본을 추정하는 방법과 계수를 바꾸는 처리이다.
- **안전 자세**: 통신·제어 실패 시 물리 피해를 줄이도록 미리 정한 로봇 위치와 상태이다.
...
- **제어 안정성**: 지연·손실·통신 단절이 있어도 제어 출력이 발산하거나 진동하지 않고 안전 범위를 유지하는 성질이다.
