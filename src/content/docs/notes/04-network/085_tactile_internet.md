...
- 정의/개념: 원격 조작•촉각 반응의 **초저지연 폐루프 통신**이다.
- 배경/필요성: 불규칙한 지연•지터는 **제어 오차•진동**을 유발한다.
...
- **양방향 폐루프**: 명령과 힘 피드백을 연속 교환한다.
- **왕복 지연•지터**: 상한으로 제어 안정성 판단이다.
- **현장 안전**: 통신 실패 시 로컬 한계•정지를 적용한다.
...
1. **조작 명령 표본화**: 손의 위치•힘을 주기 신호로 변환한다.
2. **예측 명령 전달**: 지연 상태를 추정해 제어값을 보정한다.
3. **동작 한계 검사**: 힘•위치의 안전 범위를 현장을 확인한다.
4. **촉각 상태 피드백**: 실행 결과와 접촉 힘을 반환한다.
...
|:---|:---|:---|:---|
| 적용 기준 | **짧고 예측 가능한 망 지연** | **코어망 지연 변동** 보완 | **통신 단절 중 작업 지속** |
| 핵심 특징 | **명령•촉각 직접 왕복** | **엣지 지연 상태 예측** | **현장 상태 자체 판단** |
...
- **적응형 보간(Adaptive Interpolation)**: 지연•손실 상태에 따라 늦거나 빠진 표본을 추정하는 방법과 계수를 바꾸는 처리한다.
...
- 안정 초저지연은 **원격 제어**, 지연 변동은 **엣지 예측**, 단절은 **로컬 자율**이 핵심이다.
