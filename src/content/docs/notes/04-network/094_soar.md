...
- 정의/개념: **SOAR 플레이북 기반 보안 대응 자동화** 플랫폼이다.
...
| 구성요소 | 책임 |
|:---|:---|
| 사건 맥락 입력부 | **경보 중복 제거•자산•위협** 보강 |
| 플레이북 엔진 | **조건•분기•승인•조치 순서** 실행 |
| 승인•권한 관문 | **영향도별 승인•최소 권한** 적용 |
...
1. **사건 맥락 조회**: 자산 중요도•신원•위협 평판을 결합한다.
2. **실행 승인 요청**: 신뢰도•영향•가역성으로 승인을 분기한다.
3. **멱등 조치 요청**: 최소 API 권한으로 반복 안전을 호출한다.
4. **조치 상태•타임아웃 판정**: 의도 상태와 실제 결과를 대조하고 응답 기한 초과를 실패로 처리한다.
5. **원복 요청**: 실패•오탐 시 보상 단계로 앞선 조치를 상쇄하고 이전 상태로 복구한다.
...
- 고신뢰•가역 조치는 **자동**, 고영향은 **승인**, 신규 사건은 **수동** 대응이 핵심이다.
