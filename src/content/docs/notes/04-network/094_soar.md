...
- **정의/개념**: 보안 도구·대응 절차를 플레이북으로 자동화하는 플랫폼
- **배경/필요성**: 반복 조회·조치의 지연·누락 감소
...
- 반복 절차의 순서·조건·증거를 표준화한다.
- 정보 수집 자동화로 분석가의 판단 시간 확보한다.
- 위험도에 따라 자동·승인·수동 실행을 분리한다.
- 오탐 자동 조치는 정상 업무를 대규모 차단한다.
...
| 판단 기준 | 완전 자동 대응 | 승인 기반 대응 | 수동 예외 대응 |
|:---|:---|:---|:---|
| 핵심 특징 | 조건 충족 시 즉시 조치 실행 | 자동 조사 후 사람이 조치 승인 | 분석가가 조사·판단·조치 수행 |
| 적용 기준 | 반복적·가역적·신뢰도 높은 조치 | 영향은 크지만 절차가 표준화 | 새 공격·모호한 영향·복합 사건 |
| 주요 위험 | 오탐의 빠른 확산 | 승인 지연·책임 불명확 | 대응 지연·단계 누락 |
...
1. 피싱 메일·주소 격리는 자동, 계정 잠금은 승인
2. 악성 단말 격리 후 실제 망 차단 상태 확인

### 쉽게 이해하기 (학습용)

- 피싱 메일과 주소는 같은 절차로 빠르게 차단하되 사용자 업무를 멈추는 계정 잠금은 담당자가 확인한다.
- 단말 격리 명령의 성공 응답만 믿지 않고 실제 통신이 차단됐는지 확인하며 실패하면 다른 조치를 실행한다.

## Ⅶ. 결론

- 탐지 신뢰도·영향·가역성으로 자동화 수준 결정
