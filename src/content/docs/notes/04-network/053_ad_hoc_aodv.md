...
- 정의/개념: 전송 요구가 생기면 목적지 경로를 탐색•유지하는 **AODV 반응형 라우팅**이다.
- 배경/필요성: 선제형 방식은 이동할 때 미사용 경로까지 갱신해 **제어 트래픽이 증가한다**.
...
- **RREQ** 확산의 **송신자 역방향 경로 생성**이 핵심이다.
- **RREP** 회신의 **목적지 순방향 경로 생성**이 핵심이다.
- 목적지 순차 번호의 **최신성•루프 억제**가 핵심이다.
...
1. **RREQ 확산**: 중복 제거•**TTL** 감소•역경로를 기록한다.
2. **RREP 회신**: 최신 순차 번호로 순방향 경로를 형성한다.
3. **데이터 패킷**: 라우팅 테이블의 다음 홉을 따라 전송한다.
4. **RERR 통보**: 단절 목적지를 선행 노드에 알림한다.
5. **RREQ 재탐색**: 유효 경로가 없으면 탐색을 재시작한다.
...
> 요약: AODV는 **유휴 제어량**을 줄이지만 **첫 패킷 지연** 증가한다.
...
- 이동성이 높고 통신이 간헐적이면 제어량 절감을 위해 **AODV 반응형 경로**를 선택한다.
