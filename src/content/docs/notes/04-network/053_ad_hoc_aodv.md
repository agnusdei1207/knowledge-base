...
- 배경/필요성: 기존 선제형(DSDV) 프로토콜의 상시 라우팅 광고로 인한 **노드 배터리 고갈, 무선 대역폭 낭비 및 빈번한 이동 시 테이블 수렴 지연**을 겪으므로, 쓰이지도 않을 경로를 미리 유지하는 비용을 버리고 송신이 필요한 순간에만 RREQ로 경로를 세우는 반응형 방식으로 바꿔 제어 트래픽을 실제 통신량에 비례하게 만들 필요
