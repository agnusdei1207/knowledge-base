...
- 배경/필요성: 이동망 전체 경로 유지의 **미사용 갱신 제어량 증가**
...
block
    columns 1
    A["송신 노드 | 경로가 없을 때 RREQ 탐색 시작"]
    B["중계 노드 | 중복 RREQ 제거와 패킷 중계"]
    C["목적지 노드 | 최신 순차 번호를 담은 RREP 생성"]
    D["노드별 라우팅 테이블 | 목적지·다음 홉·홉 수·수명 저장"]
    A --- B
    B --- C
    C --- D
```
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| RREQ 방송 폭증 | 확장 링·요청률 제한 | 제어 트래픽 감소 |
| 오래된 RREP 수용 | 목적지 순차번호 최신성 검증 | 순환·블랙홀 방지 |
| 첫 데이터 탐색 지연 | 통신 빈도별 경로 수명 조정 | 초기 지연 완화 |
...
- **이동성·통신 빈도·탐색 비용에 따른 AODV 요구형 경로**
