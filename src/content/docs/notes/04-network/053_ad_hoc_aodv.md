...
- 정의/개념: 전송 요구가 생기면 목적지 경로를 탐색·유지하는 **애드혹 주문형 거리 벡터(Ad Hoc On-Demand Distance Vector, AODV) 반응형 라우팅**
...
- **경로 요청(Route Request, RREQ)** 확산의 **송신자 역방향 경로 생성**
- **경로 응답(Route Reply, RREP)** 회신의 **목적지 순방향 경로 생성**
...
- **경로 요청·경로 응답(RREQ·RREP)**: 경로 요청은 목적지 경로를 찾도록 확산하고 경로 응답은 목적지 또는 최신 경로 보유 노드가 송신자에게 회신한다.
- **라우팅 테이블**: 목적지·다음 홉·홉 수·순차 번호·경로 수명을 저장하는 노드별 경로 정보이다.

</details>

송신 노드는 **경로 요청(Route Request, RREQ)** 을 확산하고 목적지 또는 최신 경로 보유 노드는 **경로 응답(Route Reply, RREP)** 을 회신한다.
...
- **경로 오류(RERR)**: 다음 홉 단절로 사용할 수 없게 된 목적지를 선행 노드에 알리는 메시지이다.
- **생존 시간(TTL)**: 경로 요청을 포함한 패킷이 통과할 수 있는 최대 홉 수를 제한하는 값이다.
...
1. **경로 요청(Route Request, RREQ) 확산**: 중복 제거·**생존 시간(Time to Live, TTL)** 감소·역경로 기록
2. **경로 응답(Route Reply, RREP) 회신**: 최신 순차 번호로 순방향 경로 형성
3. **데이터 패킷**: 라우팅 테이블의 다음 홉을 따라 전송
4. **경로 오류(Route Error, RERR) 통보**: 단절 목적지를 선행 노드에 알림
...
| 애드혹 라우팅 | 애드혹 주문형 거리 벡터(Ad Hoc On-Demand Distance Vector, AODV) 반응형 | 선제형 라우팅 |
|:---|:---|:---|
| 적용 환경 | 이동이 많고 **통신 간헐적** | 토폴로지가 안정되고 **통신 빈번** |
| 핵심 특징 | 요구 시 **경로 요청(Route Request, RREQ)·경로 응답(Route Reply, RREP) 탐색** | 모든 경로의 **주기적 갱신** |
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 전체 범위 **경로 요청(Route Request, RREQ)** 반복으로 방송 부하 증가 | **확장 링 탐색·요청률 제한** 적용 | 불필요한 **제어 트래픽** 감소 |
| 오래된 **경로 응답(Route Reply, RREP)** 을 수용하면 경로 순환·공격 노출 | 목적지 순차 번호와 **RREP 출처** 검증 | 경로 순환과 **블랙홀 공격** 억제 |
| 짧은 경로 수명으로 첫 데이터 탐색 반복 | 통신 빈도에 맞춰 **경로 수명** 조정 | 재탐색 횟수와 **초기 지연** 완화 |

#### 한줄 요약

- 가까운 범위부터 길을 찾고 실패할 때만 **생존 시간(Time to Live, TTL)** 을 늘려 전체 방송을 줄인다.
...
- 이동성이 높고 통신이 간헐적이면 제어량 절감을 위해 **애드혹 주문형 거리 벡터(Ad Hoc On-Demand Distance Vector, AODV) 반응형 경로** 선택
