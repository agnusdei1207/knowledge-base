...
- **AODV**: 데이터 전송 요구가 생겼을 때 목적지 경로를 탐색하고 단절 시 재탐색하는 반응형 애드혹 라우팅 방식이다.
...
- **역방향·순방향 경로**: RREQ가 온 이전 홉을 기록한 경로로 RREP가 돌아오고 그 응답을 따라 목적지 방향 경로가 생성된다.
...
- **RREQ·RREP**: RREQ는 목적지 경로를 찾도록 확산하고 RREP는 목적지 또는 최신 경로 보유 노드가 송신자에게 회신한다.
...
- **RERR**: 다음 홉 단절로 사용할 수 없게 된 목적지를 선행 노드에 알리는 경로 오류 메시지이다.
- **TTL**: RREQ를 포함한 패킷이 통과할 수 있는 최대 홉 수를 제한하는 값이다.
...
- **확장 링 탐색**: 작은 TTL로 가까운 범위부터 탐색하고 실패할 때 TTL을 늘려 방송 범위를 확대하는 방식이다.
