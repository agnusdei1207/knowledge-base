...
1. 유효 경로 조회
      ...
      └─ 없음                            │
          │                              │
2. RREQ 확산 및 역방향 경로 설정         │
          │                              │
3. RREP 회신 및 순방향 경로 설정         │
          └──────────────────────────────┤
                                         │
4. 라우팅 테이블 기반 데이터 전달
      ├─ 정상 ── 데이터 전송 완료
      └─ 단절 ── 5. RERR 통보 및 경로 재탐색
```
...
| 제어 트래픽 오버헤드 | 유휴 시 경로 탐색 제어량 감소 | 미사용 경로도 주기적 갱신 |
...
|:---|:---|:---|:---|
| RREQ 브로드캐스트 스톰 | 목적지가 멀리 있을 때 RREQ 전역 플러딩 발생 | Expanding Ring Search (TTL 단계 증가 탐색) 적용 | 불필요한 라우팅 제어 패킷 감소 |
