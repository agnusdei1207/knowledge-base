...
date: "2026-07-25T19:53:00+09:00"
tags:
  - "notes-network"
weight: 53
extra:
...
- **정의/개념**: 전송 요구 시 목적지 경로를 찾는 라우팅
- **배경/필요성**: 이동망의 미사용 경로 갱신 비용 제거
...
- RREQ 확산이 송신자 방향의 역경로를 만든다.
- RREP 회신이 목적지 방향의 전송 경로를 만든다.
- 순차 번호로 최신 경로를 골라 루프를 억제한다.
- 통신이 없으면 경로 갱신 제어량을 만들지 않는다.
...
| 판단 기준 | AODV 반응형 | 선제형 라우팅 |
|:---|:---|:---|
| 핵심 특징 | 요구 시 목적지 경로 탐색 | 모든 경로를 주기적으로 갱신 |
| 적용 기준 | 이동 많고 통신이 간헐적 | 토폴로지 안정·통신 빈번 |
| 주요 위험 | 첫 전송 탐색 지연·RREQ 폭증 | 미사용 경로 제어량 증가 |
