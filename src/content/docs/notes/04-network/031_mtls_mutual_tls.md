...
- 정의/개념: 양쪽 **인증서 체인•개인키 소유**를 검증하는 **TLS 상호 인증 방식**이다.
- 배경/필요성: 단방향 TLS는 **호출자 기계 신원 확인이 부족하다**.
...
- 서버•클라이언트의 **인증서 체인** 모두를 검증한다.
- 전자서명으로 **개인키 비노출 소유 증명**이 핵심이다.
- 상호 인증 후 **세션 키 암호화**, 서비스 권한은 별도를 판정한다.
...
2. **서버 체인 검증 요청**: 신뢰 CA•이름•기간•용도로 **서버 신원**을 판정한다.
3. **클라이언트 인증 요청**: 허용 CA 목록과 함께 **호출자 증명**을 요구한다.
4. **클라이언트 인증 증명**: 클라이언트 인증서 체인과 **전자서명**을 제공한다.
5. **클라이언트 체인 검증 요청**: 신뢰 체인•기간•용도로 **호출자 신원**을 판정한다.
...
- 양방향 기계 신원이 필요하면 **mTLS**, 서비스 권한은 **별도 정책**을 적용한다.
