...
- 클라우드 네이티브 및 MSA 환경에서 제로 트러스트 보안을 달성하기 위해 **mTLS를 서비스 간 통신의 표준 암호화 계층으로 전면 배포**하고, 인증서 수명 관리 복잡성을 극복하기 위해 **서비스 메시(Istio/Envoy)와 SPIFFE/SPIRE 기반 자동화 및 OPA 세부 인가 정책**을 결합하여 고신뢰 M2M 보안 인프라 완성
