...
- **전송 계층 보안(Transport Layer Security, TLS)**: 서버 인증과 통신 암호화를 제공하는 전송 보안 규약
- **상호 전송 계층 보안(Mutual Transport Layer Security, mTLS)**: 통신 양쪽의 인증서 체인과 개인키 소유를 검증하는 TLS 인증 방식
- **인증기관(Certificate Authority, CA)**: 인증서를 발급·서명하고 신뢰 기준을 제공하는 기관
- **개인키**: 소유자만 보관하는 비밀 키
- **신뢰 저장소**: 허용 인증기관 정보를 보관
- **세션 키(Session Key)**: 상호 인증 뒤 한 연결의 데이터를 대칭키 방식으로 암호화하는 임시 비밀값
- **인증서 체인(Certificate Chain)**: 종단 인증서부터 신뢰한 루트 인증기관까지 서명 관계를 잇는 인증서 목록
- **전자서명(Digital Signature)**: 개인키로 만든 서명값을 공개키로 검증해 메시지 무결성과 키 소유를 확인하는 기법
- **인증서 폐기 목록(Certificate Revocation List, CRL)**: 만료 전 폐기된 인증서 일련번호를 인증기관이 서명해 배포하는 목록
