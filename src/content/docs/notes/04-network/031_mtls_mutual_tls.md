...
- 정의/개념: **mTLS**는 양쪽 인증서 체인과 개인키 소유를 검증하는 **TLS** 상호 인증 방식이다.
- 배경/필요성: 단방향 TLS만으로는 호출자의 기계 신원을 확인하기 어렵다.

#### 한줄 요약

- 양쪽 종단이 동일 CA가 발급한 인증서를 교환하고 각자의 개인키 소유를 상호 검증한다.
...
- 서버와 클라이언트의 **인증서 체인**을 모두 검증한다.
- 전자서명으로 개인키를 노출하지 않고 소유를 증명한다.
- 상호 인증 뒤 **세션 키**로 암호화하고 서비스 권한은 별도로 판정한다.
...
선의 의미: 인증기관이 양측 인증서와 신뢰 저장소의 공통 신뢰 뿌리를 이루고, 클라이언트와 서버의 인증서•키가 하나의 세션 키로 보호되는 통신 경계를 구성하는 정적 관계이다.

| 구성요소 | 책임 |
|:---|:---|
| 인증기관(CA) | **CA**가 종단 인증서 발급•서명 |
| 클라이언트 인증서•키 | 호출자 신원•개인키 소유 증명 |
| 서버 인증서•키 | 서버 신원•개인키 소유 증명 |
| 양측 신뢰 저장소 | **신뢰 저장소**에 허용한 CA 체인 보관 |
| 세션 키 | **세션 키**로 상호 인증 후 응용 데이터 암호화 |
...
- **서버 인증 증명**: 서버가 인증서 체인과 전자서명을 클라이언트에 제공하는 절차이다.
- **서버 체인 검증 요청**: 클라이언트가 신뢰 CA•이름•기간•용도로 서버 신원을 확인하는 절차이다.
- **클라이언트 인증 요청**: 서버가 허용 CA 목록과 함께 호출자 인증서 제시를 요구하는 절차이다.
- **클라이언트 인증 증명**: 클라이언트가 인증서 체인과 전자서명을 서버에 제공하는 절차이다.
- **클라이언트 체인 검증 요청**: 서버가 신뢰 체인•기간•용도로 호출자 신원을 확인하는 절차이다.
...
1. **서버 인증 증명**: 서버 인증서 체인과 **전자서명**을 제공한다.
2. **서버 체인 검증 요청**: 신뢰 CA•이름•기간•용도로 서버 신원을 판정한다.
3. **클라이언트 인증 요청**: 허용 CA 목록과 함께 호출자 증명을 요구한다.
4. **클라이언트 인증 증명**: 클라이언트 인증서 체인과 **전자서명**을 제공한다.
5. **클라이언트 체인 검증 요청**: 신뢰 체인•기간•용도로 호출자 신원을 판정한다.
...
|:---|:---|:---|
| 적용 기준 | 양방향 기계 신원이 필요한 통신 | 불특정 사용자가 접속하는 공개 서비스 |
| 핵심 특징 | 서버•클라이언트 상호 인증 | 서버 인증만 수행 |
| 한계 | 인증서 발급•회전의 운영 복잡성 | 호출자 기계 신원 미검증 |

> 요약: 기계 신원이 필요하면 mTLS을 적용한다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 만료 직전 인증서 미교체로 연결 중단 | 인증서 자동 발급•회전과 만료 경보 | 인증서 만료에 따른 호출 실패 예방 |
...
- **인증과 인가 분리** 원칙에 따라 양방향 기계 신원은 **mTLS**로 확인하고 서비스 권한은 **최소 권한** 정책으로 판정한다.
