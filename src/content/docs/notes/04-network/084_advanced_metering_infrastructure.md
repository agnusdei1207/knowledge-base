...
- 정의/개념: 스마트 미터와 전력사를 연결해 **계량 자료 수집•원격 명령**을 수행하는 **양방향 검침 인프라**이다.
- 배경/필요성: 단방향 AMR은 **원격 제어•실행 결과 확인할 수 없다**.
...
1. **계량 자료 전송**: 시간대별 사용량•상태 이벤트를 제공한다.
2. **누락 보완 자료 전달**: 미터 신원•누락 구간 확인과 재수집한다.
3. **VEE 자료 제공**: 이상•누락값 보정 후 품질을 표시한다.
4. **원격 명령 승인**: 작업자 권한•대상•정책을 확인한다.
5. **인증 명령 실행**: 미터 적용과 실제 결과까지 확정한다.
...
| 검침 방식 | AMI | AMR | 현장 검침 |
|:---|:---|:---|:---|
| 적용 기준 | **수요반응•정전•원격 업무** | **자동 요금 검침** | **통신망 구축 곤란** 소규모 |
...
- 원격 제어까지는 **AMI**, 자동 사용량만은 **AMR**, 망 구축 곤란은 **현장 검침**이 핵심이다.
