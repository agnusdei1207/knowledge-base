...
- 정의/개념: **AMI**는 스마트 미터와 전력사를 양방향으로 연결하고, **AMR**은 사용량을 단방향으로 자동 수집한다.
- 배경/필요성: 단방향 자동 검침만으로는 원격 제어와 실행 결과를 확인할 수 없다.
...
- **양방향 제어**: 인증 명령과 실행 결과를 확인한다.
- **VEE**: 누락•이상 계량값 검증•추정•보정이다.
...
- **HES**는 통신 세션과 수집•명령을 관리하고 **MDMS**는 계량값을 저장해 VEE 뒤 업무 시스템에 제공한다.
...
|:---|:---|
| 스마트 미터 | 사용량•상태•명령 실행 |
| 현장 통신망 | 계량기•전력사 자료 전달 |
| 헤드엔드 시스템 | 통신•수집•명령 세션 관리 |
| 계량자료 관리시스템 | 계량값•VEE•업무 자료 관리 |
| 전력 업무시스템 | 요금•정전•수요반응 수행 |
...
- **계량 자료 전송**: 스마트 미터가 시간대별 사용량과 상태 이벤트를 헤드엔드에 보내는 단계이다.
- **누락 보완 자료 전달**: 미터 신원과 누락 구간을 확인하고 재수집한 값을 제공하는 단계이다.
- **VEE 자료 제공**: 이상•누락값을 검증•추정•편집하고 품질 상태를 표시하는 단계이다.
- **원격 명령 승인**: 작업자 권한•대상 미터•업무 정책을 확인하는 단계이다.
- **인증 명령 실행**: 승인된 명령을 미터에 적용하고 결과를 확정하는 단계이다.
...
1. **계량 자료 전송**: **계량 자료 전송** 단계에서 시간대별 사용량•상태 이벤트를 제공한다.
2. **누락 보완 자료 전달**: **누락 보완 자료 전달** 단계에서 미터 신원•누락 구간을 확인해 재수집한다.
3. **VEE 자료 제공**: **VEE 자료 제공** 단계에서 보정 결과에 **품질 플래그**를 남긴다.
4. **원격 명령 승인**: **원격 명령 승인** 단계에서 **원격 명령**의 권한•대상•정책을 확인한다.
5. **인증 명령 실행**: **인증 명령 실행** 단계에서 미터 적용과 결과까지 확정한다.
...
| 검침 방식 | AMI | AMR | 현장 검침 |
|:---|:---|:---|:---|
| 적용 기준 | **AMI**는 수요반응•정전•원격 업무 | **AMR**은 자동 요금 검침 | **현장 검침**은 통신망 구축이 어려운 소규모 환경 |
| 핵심 특징 | 양방향 수집•원격 제어 | 단방향 자동 수집 | 현장 수치 확인 |
| 한계 | 사이버 제어•개인정보 노출 | 실시간 제어•상태 확인 불가 | 인력 비용•지연•오입력 |

> 요약: 양방향 업무 범위와 통신 여건으로 선택한다.
...
| 원격 차단의 오대상 실행 | **이중 승인**과 결과 대조 적용 | 제어 오용과 복구 시간 감소 |
...
- **검침 방식 선택**에서 원격 제어까지는 **AMI**, 자동 사용량만은 **AMR**, 망 구축 곤란은 **현장 검침**이 핵심이다.
