...
- **선택 복제**: 가입 수신자가 존재하는 분배 트리의 분기점에서만 패킷 사본을 만드는 전달 동작이다.
- **일대다 전송 방식**: 하나의 송신 흐름을 여러 가입 수신자에게 분배하는 통신 방식이다.
- **대역폭 중복 사용**: 같은 콘텐츠를 수신자마다 개별 전송해 공통 경로에서 동일 패킷이 반복되는 낭비이다.

</details>

- 정의/개념: 하나의 송신 패킷을 가입 수신자 경로의 분기점에서 **선택 복제**하는 **일대다 전송 방식**이다.
- 배경/필요성: 개별 전송은 동일 콘텐츠 반복으로 **대역폭 중복 사용**을 유발한다.
...
- **IGMP** 기반 접속망의 그룹 가입•탈퇴 상태를 관리한다.
- **PIM** 기반 라우터 간 공유•송신원 분배 트리를 구성한다.
- **RPF** 검사로 오유입을 막고 가입 포트에만 전달하는 것이 핵심이다.
...
선의 의미: 송신원부터 수신자까지의 분배 경계와 RP 소속을 나타낸 구조이다.

| 구성요소 | 책임 |
|:---|:---|
| 멀티캐스트 송신원 | **멀티캐스트** 그룹 주소로 원본 패킷 전송 |
| PIM 라우팅 도메인 | RPF 기반 **분배 트리** 구성 |
| 집결점(RP) | **RP**가 **ASM** 공유 트리 접점 제공 |
| IGMP 접속망 | **IGMP**가 **멀티캐스트 그룹** 상태•가입 포트 관리 |
| 멀티캐스트 수신자 | 그룹 가입•수신 |
...
- **RPF 통과 패킷 전달**: 정상 역방향 인터페이스로 유입된 패킷만 가입 가지에 복제하는 단계이다.
...
1. PIM 가입 전파: 수신 측 라우터가 **PIM 가입 전파**로 상위 경로에 가입을 요청한다.
2. RPF 통과 패킷 전달: 정상 유입 패킷만 **가입 가지**에 복제하는 **RPF 통과 패킷 전달**을 수행한다.
...
| 핵심 특징 | **ASM 공유 트리** | 송신원 지정 최단 트리 | 범람 후 가지 제거 |
| 한계 | RP 장애•복잡한 상태 | 송신원 정보 배포 오류 | 불필요한 범람 트래픽 반복 |

> 요약: 송신원 지정 가능 시 **SSM** 우선이 핵심이다.
...
- 송신원 지정 시 **SSM**, 미지정 시 **ASM**•**PIM-SM**, 고밀도 폐쇄망은 **PIM-DM**을 택하는 **멀티캐스트 방식 선택**이 필요하다.
