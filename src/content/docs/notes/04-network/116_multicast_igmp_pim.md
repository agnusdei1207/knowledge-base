...
```text
                         [집결점(RP)]
                              |
[멀티캐스트 송신원] ----- [PIM 라우팅 도메인] ----- [IGMP 접속망] ----- [멀티캐스트 수신자]
```

선의 의미: 가로선은 멀티캐스트 송신원과 PIM 라우팅 도메인, IGMP 접속망 및 수신자의 정적 분배 경계이고, 위 선은 집결점이 PIM 도메인 안에서 ASM 공유 트리의 접점을 제공하는 소속 관계를 뜻한다.
