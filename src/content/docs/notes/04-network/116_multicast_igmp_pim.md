...
- **1. IGMP 그룹·송신원 가입**: 수신자가 원하는 채널을 알림
- **2. PIM 가입 전파**: 라우터가 상위 경로에 가입 요청
- **3. 멀티캐스트 패킷 수신**: 송신원이 그룹 패킷 발송
- **4. RPF 검사·가지 복제**: 정상 유입만 가입 가지로 복제
- **5. 가입 접속망 전달**: 가입 포트의 수신자에게 전달
