...
- **IGMP·PIM**: IGMP는 접속망 단말의 그룹 가입을 관리하고 PIM은 라우터 간 멀티캐스트 분배 트리를 구성한다.
- **RPF**: 송신원으로 돌아가는 최적 경로의 인터페이스로 들어온 멀티캐스트 패킷만 전달하는 검사이다.
...
- **집결점(RP)**: 임의 송신원 멀티캐스트에서 공유 트리의 중심을 제공하는 라우터이다.
...
- **ASM·SSM**: ASM은 모든 송신원을 허용하고 SSM은 수신자가 특정 송신원·그룹 조합만 선택한다.
- **PIM-SM·PIM-DM**: PIM-SM은 명시적 가입으로 희소 수신자를 연결하고 PIM-DM은 먼저 범람한 뒤 빈 가지를 제거한다.
...
- **IGMP 스누핑**: 스위치가 IGMP 가입 보고를 읽어 가입한 포트에만 멀티캐스트를 전달하는 기능이다.
