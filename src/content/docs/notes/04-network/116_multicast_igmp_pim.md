...
- 배경/필요성: 개별 전송은 동일 콘텐츠 반복으로 **대역폭 중복 사용** 유발한다.
...
| 집결점(RP) | **RP** **ASM** 공유 트리 접점 제공 |
| IGMP 접속망 | **IGMP** **멀티캐스트 그룹** 상태•가입 포트 관리 |
| 멀티캐스트 수신자 | 그룹 가입•수신 |
...
1. PIM 가입 전파: 수신 측 라우터가 **PIM 가입 전파** 기반 상위 경로에 가입을 요청한다.
2. RPF 통과 패킷 전달: 정상 유입 패킷만 **가입 가지** 대상 복제하는 **RPF 통과 패킷 전달** 수행한다.
...
- 송신원 지정 시 **SSM**, 미지정 시 **ASM**•**PIM-SM**, 고밀도 폐쇄망은 **PIM-DM** 택하는 **멀티캐스트 방식 선택** 필요하다.
