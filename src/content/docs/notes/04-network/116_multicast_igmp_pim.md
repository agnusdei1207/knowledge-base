...
- **멀티캐스트**: 가입한 수신자 경로의 분기점에서 패킷을 복제하는 전송 방식
...
- **수신자 주도**: IGMP로 그룹 가입과 탈퇴를 알린다.
- **트리 전달**: PIM이 라우터 간 분배 경로를 만든다.
- **경로 안전성**: 역방향 검사로 루프 유입을 막는다.
- **계층별 최적화**: 스위치는 가입 포트만 선택한다.
...
| PIM 라우터 | 라우팅표의 역방향 경로로 분배 트리 구성 |
| 집결점(RP) | ASM 공유 트리의 접점 제공 |
| 수신 측 라우터 | IGMP 가입을 PIM으로 변환 |
| IGMP 스누핑 스위치 | 가입 포트에만 프레임 전달 |
| 멀티캐스트 수신자 | 원하는 그룹 가입과 수신 |
| 멀티캐스트 상태 | 유입·유출 경로 정보 유지 |
...
| 비교 기준 | ASM | SSM | 밀집형 PIM |
|:---|:---|:---|:---|
| 핵심 특징 | RP 중심 공유 트리 | 송신원 지정 최단 트리 | 먼저 범람 후 가지 제거 |
| 적용 기준 | 송신원 탐색 필요 | 송신원 사전 지정 | 수신자가 매우 많은 폐쇄망 |
| 주요 위험 | RP 장애·복잡한 상태 | 송신원 정보 배포 오류 | 불필요한 트래픽 반복 |
...
1. **방송 서비스**: RP 이중화와 가입 포트 제한을 적용한다.
2. **장애 분석**: 가입·분배 상태와 역방향 실패를 추적한다.

### 쉽게 이해하기 (학습용)

- 중심 라우터 장애에도 방송 경로를 유지한다.
- 수신자부터 송신원 방향으로 상태를 확인한다.

## Ⅶ. 결론

- IGMP 가입 상태와 PIM 분배 트리를 연계하여 그룹 트래픽을 필요한 경로에만 전달한다.
