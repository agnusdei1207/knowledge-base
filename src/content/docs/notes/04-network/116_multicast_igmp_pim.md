...
- 배경/필요성: 개별 전송은 동일 콘텐츠 반복으로 **대역폭 중복 사용**이 필요하다.
...
- RPF•IGMP 스누핑 기반 **오유입•불필요 전달 차단**이 핵심이다.
...
선의 의미: 송신원부터 수신자까지의 분배 경계와 RP 소속을 나타낸 구조다.
...
1. **PIM 가입 전파**: 수신 측 라우터가 상위 경로에 가입을 요청한다.
2. **RPF 통과 패킷 전달**: 정상 유입 패킷만 가입 가지로 복제한다.
...
- **ASM 공유 트리**: 모든 송신원을 허용하고 RP 중심 트리로 송신원을 찾는다.
...
- 송신원 지정 시 **SSM**, 미지정 시 **ASM•PIM-SM**, 고밀도 폐쇄망은 PIM-DM을 선택한다.
