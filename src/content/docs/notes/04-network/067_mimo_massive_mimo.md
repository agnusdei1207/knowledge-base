...
block
    columns 1
    A["채널 추정기 | 파일럿으로 안테나 간 CSI 산출"]
    B["스케줄러·순위 선택기 | 사용자·자원·공간 스트림 수 결정"]
    C["프리코더 | 안테나별 복소 가중치·전력 계산"]
    D["송수신 안테나 배열 | 공간별 신호 방사·수신"]
    E["MIMO 검출기 | 수신 혼합 신호에서 스트림·간섭 분리"]
    A --- B
    B --- C
    C --- D
    D --- E
...
- **채널 추정·파일럿·안테나 보정을 갖춘 Massive MIMO**
