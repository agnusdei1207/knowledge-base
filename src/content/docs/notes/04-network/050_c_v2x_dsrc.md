...
- 배경/필요성: 초기 1세대 차량 통신 표준인 DSRC/WAVE(IEEE 802.11p)는 Wi-Fi 기반의 CSMA/CA 경합형 매체 접근 방식을 사용하므로 도심 교차로나 정체 구간과 같은 고밀도 차량 환경에서 패킷 충돌률과 지연 시간이 기하급수적으로 폭증하고, 통신 도달 거리가 300m 수준으로 짧으며 광역 셀룰러망 연계가 불가능한 한계를 극복하기 위해, 3GPP 셀룰러 표준 기반으로 직접 통신(PC5 사이드링크)의 자율 센싱 준지속 스케줄링(SPS: Mode 4)과 광역 기지국 통신(Uu)을 결합한 **C-V2X**(Cellular V2X)를 도입하여 1km 이상의 넓은 커버리지, 고밀도 환경에서의 충돌 없는 패킷 수신 신뢰성 및 LTE에서 5G-NR V2X로의 연속적 기술 진화를 달성할 필요
...
- 서비스 경로 확장성: C-V2X는 PC5와 Uu를 결합 지원
- 표준 전환 공존성: 지역 정책에 따라 C-V2X와 DSRC 계열 병존
...
C-V2X Mode 4 자율 스케줄링 vs DSRC CSMA/CA 동작 파이프라인
   ...
   [C-V2X Mode 4 (SPS)]              [DSRC 802.11p (CSMA/CA)]
        │                                 │
   [서브채널 RSRP/RSSI 센싱]          [캐리어 센싱 (CCA)]
      1,000ms 채널 간섭 계측             채널 유휴 여부 즉각 판정
        │                                 │
   [최적 후보군 선별]                 [DIFS 대기 & 백오프]
      비어있는 자원 블록(RB) 선별         무작위 Contention Window
        │                                 │
   [SPS 예약 타이머 설정]             [브로드캐스트 송출]
...
- 한국, 미국, 중국을 포함한 글로벌 주요국들이 C-ITS 국가 단일 표준으로 C-V2X 방식을 확정함에 따라 자율협력주행 및 커넥티드 카 생태계의 단일 지배 표준 기술로 자리매김하였으며, 향후 고정밀 센서 셰어링 및 군집 주행을 지원하는 5G-NR V2X(Rel-16/17)로 발전해 나가는 가운데, 실무 구축 시에는 기존 WAVE 인프라와의 공존을 위한 Dual-Mode RSU/OBU 과도기적 운용, 차량용 텔레매틱스와 C-V2X를 원칩화한 통합 SoC 적용, 고속 주행 시 도플러 편이를 보상하는 DMRS 밀도 제어를 결합하여 성공적인 차세대 C-ITS 무선망을 완성
