...
date: "2026-07-25T19:50:00+09:00"
tags:
  - "notes-network"
weight: 50
extra:
...
- **정의/개념**: 서로 다른 표준·접속 방식의 V2X 무선 기술
- **배경/필요성**: 안전 성능·기존 차량·RSU 호환성 선택
...
- C-V2X는 PC5 직접 통신과 Uu 경로를 지원한다.
- DSRC는 OCB·CSMA/CA로 직접 채널을 경쟁한다.
- 차량 밀도·채널 부하가 패킷 충돌률을 좌우한다.
- 지역 주파수 정책이 장비 호환과 전환 비용을 좌우한다.
...
| 판단 기준 | C-V2X | DSRC |
|:---|:---|:---|
| 핵심 특징 | 3GPP PC5·Uu | IEEE WAVE·OCB |
| 적용 기준 | 이동통신 진화·광역 연계 | 기존 WAVE·RSU 호환 |
| 주요 위험 | 세대·모드 간 호환 제약 | 혼잡 시 채널 경쟁 충돌 |
...
1. 도로 V2X 전환은 기존 RSU 호환·교체 범위 산정
2. 혼잡 교차로는 동일 부하에서 수신률·지연 비교

### 쉽게 이해하기 (학습용)

- 새 통신 방식을 쓰려면 이미 설치된 차량 단말과 RSU를 얼마나 바꿔야 하는지 먼저 계산한다
- 같은 수의 차량이 동시에 경고를 보낼 때 두 방식 중 더 많이 제때 도착하는 쪽을 확인한다

## Ⅶ. 결론

- 인프라 호환·혼잡 성능으로 C-V2X·DSRC 선택
