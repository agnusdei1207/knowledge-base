...
date: "2026-07-25T02:09:00+09:00"
tags:
  - "notes-network"
weight: 3
extra:
...
- **정의/개념**: 통신 규칙을 구문·의미·타이밍으로 구분
- **배경/필요성**: 메시지 생성·해석·교환의 상호운용 기준
...
- 구문 일치가 메시지 경계 오분리를 막는다
- 의미 일치가 필드값별 동작 차이를 막는다
- 타이밍 일치가 순서 위반·응답 만료를 막는다
...
| 검증 기준 | 필드 경계·길이 일치 | 상태 전이·응답 일치 | 순서·대기 시간 일치 |
| 주요 위험 | 필드 오분리 | 명령 오해석 | 타임아웃·순서 위반 |
...
1. 장치 연동 시험은 필드·상태 코드·응답 시간을 대조
