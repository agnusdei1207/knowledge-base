...
- **프로토콜**은 서로 다른 통신 주체가 메시지를 같은 방식으로 만들고 해석하도록 합의한 규칙의 집합이다.
- **상호운용성**은 서로 다른 구현이 같은 프로토콜을 따라 올바르게 통신할 수 있는 성질이다.

</details>

- 정의/개념: 메시지 형식·동작·교환 시점을 **구문·의미·타이밍**으로 규정하는 통신 규칙
- 배경/필요성: 메시지 형식만 맞추면 구현별 동작·교환 시점의 **상호운용 실패**
...
- **구문**은 메시지 필드의 순서·길이와 표현 형식을 정의한다.
- **의미**는 필드값과 제어 코드에 대응하는 동작·응답·상태 변화를 정의한다.
- **타이밍**은 메시지의 송수신 순서·속도와 대기 시간을 정의한다.
...
- **필드**는 메시지 안에서 하나의 정보를 담는 구역이고, **인코딩**은 그 정보를 비트나 문자로 표현하는 방식이다.
...
- **파싱**은 수신 메시지의 경계와 필드를 구문 규칙에 따라 분리하는 작업이다.
- **상태 전이**는 현재 상태에서 수신한 메시지와 조건에 따라 다음 상태를 결정하는 동작이다.
- **타임아웃**은 정해진 대기 시간 안에 응답이 없을 때 통신 실패로 판정하는 조건이다.
...
- **형식·동작·시간의 독립 검증**은 메시지를 읽지 못하는 오류, 다르게 처리하는 오류, 순서나 기한이 어긋나는 오류를 구분한다.
...
- **상태표**는 메시지와 현재 상태별로 수행할 동작과 다음 상태를 명시하여 구현 간 의미 차이를 줄인다.
- **버전 협상**은 통신 주체가 지원하는 프로토콜 버전을 확인하여 함께 사용할 규칙 범위를 정한다.
...
- **프로토콜 적합성**은 구현이 구문·의미·타이밍 규칙을 모두 준수할 때 확보된다.
