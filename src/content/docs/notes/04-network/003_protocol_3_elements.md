...
- 정의/개념: 메시지 형식•동작•교환 시점을 **구문•의미•타이밍**으로 규정하는 통신 규칙이다.
- 배경/필요성: 메시지 형식만 맞춰서는 구현별 동작•교환 시점의 **일관된 상호운용을 보장하기 어려움**이 발생한다.
...
- 구문의 **메시지 형식•필드 경계 정의**가 핵심이다.
- 의미의 **필드값•제어 동작 정의**가 핵심이다.
- 타이밍의 **교환 순서•속도 정의**가 핵심이다.
...
1. **구문 해석**: 경계•길이 규칙에 따라 필드를 분리한다.
2. **의미 처리**: 필드값에 따른 동작•상태 전이 결정한다.
...
> 요약: 상호운용 오류를 형식•동작•교환 시점으로 나누어 검증한다.
...
- 파싱•동작•시간 오류별 **구문•의미•타이밍** 우선을 검증한다.
