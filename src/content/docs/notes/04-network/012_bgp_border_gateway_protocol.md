...
- 배경/필요성: 단순 링크 비용 기반 IGP 알고리즘만으로는 **ISP 간 비즈니스 계약 정책(Policy), 트래픽 엔지니어링 및 AS 간 라우팅 루프 방어 불가**를 겪으므로, AS 경계에 최단 비용 대신 경로 속성과 AS_PATH를 비교해 정책으로 경로를 고르는 별도 라우팅 계층을 두어 도메인 내부 IGP와 역할을 분리할 필요
