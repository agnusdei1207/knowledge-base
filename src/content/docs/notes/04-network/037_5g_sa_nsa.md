...
- 배경/필요성: 초기 5G 도입 시 막대한 인프라 투자 비용과 기지국 구축 기간을 단축하기 위해 기존 4G LTE 코어(EPC) 및 eNB를 제어 플레인 앵커로 활용하는 NSA(Non-Standalone: Option 3x) 방식이 널리 채택되었으나, 4G EPC의 레거시 구조적 한계로 인해 E2E 네트워크 슬라이싱 격리, 1ms 이하의 초저지연(URLLC), New Radio 기반 순수 고품질 음성 통화(VoNR) 및 단말 배터리 효율 최적화를 완벽히 실현할 수 없는 한계를 극복하기 위해, 서비스 기반 아키텍처(SBA) 클라우드 네이티브 5G 코어(5GC)와 gNB 기지국만으로 제어와 데이터를 독립 처리하는 5G SA(Standalone: Option 2) 아키텍처를 도입하여 **5G 본연의 초저지연, 초고신뢰성, 맞춤형 가상 네트워크 슬라이싱 가치**를 달성할 필요
...
- 글로벌 이동통신사들이 CAPEX/OPEX를 절감하며 5G 서비스를 조기 개시하기 위한 과도기적 NSA(Option 3x) 단계를 거쳐, B2B 특화망 및 지능형 융합 서비스를 위한 **순수 5G SA(Option 2) 아키텍처로의 전면적인 전환이 가속화**되고 있으며, 실무 마이그레이션 및 망 운용 시에는 **음성 서비스 커버리지 공백을 보완하는 EPS Fallback 적용 및 점진적 VoNR 전환, UDM/UDR 기반 가입자 데이터베이스 통합, SBA 마이크로서비스 기반 5GC 오케스트레이션**을 결합하여 고가용성 차세대 이동통신 서비스를 완성
