...
- 초기 5G 도입 단계에서는 투자 효율성과 커버리지 확보를 위해 **NSA(Option 3x)** 방식을 채택하여 초고속 브로드밴드(eMBB)를 조기 개시하되, B2B 특화망, 자율주행 및 스마트 팩토리의 초저지연(URLLC)과 E2E 슬라이싱을 달성하기 위해 **5GC 기반 SA(Option 2)** 로 전면 전환하고, **VoNR 및 EPS Fallback**을 결합하여 고신뢰 통신 인프라 완성
