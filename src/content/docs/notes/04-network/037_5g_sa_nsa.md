...
- 정의/개념: **5G** 구축은 **LTE** 코어 의존 여부에 따라 **SA**와 **NSA**로 구분한다.
- 배경/필요성: 전면 **5GC** 전환에는 투자•단말•음성 호환 부담이 따른다.
...
- **SA**는 NR•5GC 기반으로 독립 등록•세션을 제공한다.
- SA는 **VoNR** 등 5G 독립 기능을, NSA는 빠른 도입을 우선한다.
...
| LTE 코어(EPC) | **EPC**가 NSA 가입•세션 제어 |
| SA 단말 | NR•5GC 독립 접속 역량 제공 |
| NR 단독 연결 | **NR**로 5GC에 제어•사용자면 전달 |
| 5G 코어(5GC) | **5GC**가 SA 가입•세션•슬라이스 제어 |
...
- NSA의 **이중 연결**과 SA의 **NR 단독 연결**은 실제 제어망과 기능을 구분한다.
...
| 5G 구축 방식 | **SA** | **NSA** |
|:---|:---|:---|
| 적용 기준 | **네트워크 슬라이싱**•저지연•VoNR | 빠른 도입•LTE 자산 활용 |
| 핵심 특징 | NR•5GC 독립 구성 | LTE 제어•NR 결합 |
| 한계 | 전환 비용•단말 호환 | LTE 의존•기능 제한 |

> 요약: 5GC 기능•LTE 자산으로 구성을 선택한다.
...
- **통화 연속성**: 단독 구성(Standalone, SA) 전환이나 이동 중에도 단말의 음성 통화가 끊기지 않고 이어지는 성질이다.
...
- **전환 기준**에 따라 LTE 자산•호환 유지 시 **NSA**, 슬라이싱•VoNR 필요 시 **SA**로 전환한다.
