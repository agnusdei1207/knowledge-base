...
| 구성요소 | 책임 |
|:---|:---|
| GPU·CPU 메모리 도메인 | 자료 저장과 상호 접근 경계 제공 |
...
sequenceDiagram
    ...
    GPU->>관리자: 1. 링크 토폴로지 발견
    관리자->>NVSwitch: 2. 도메인·파티션 구성
    GPU->>NCCL: 3. 통신 순위·자료 배치
    NCCL->>NVSwitch: 4. 집합 경로 선택
    NVSwitch-->>관리자: 5. 링크 결과 검증
```

**동작 원리**

- **1. 링크 토폴로지 발견**: GPU·포트·스위치 연결을 수집
- **2. 도메인·파티션 구성**: 상호 접근 경계와 권한 설정
- **3. 통신 순위·자료 배치**: 자주 교환할 GPU를 가까이 배치
- **4. 집합 경로 선택**: 연산별 링·트리 경로를 결정
- **5. 링크 결과 검증**: 우회·불균형·오류 링크 확인
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 통신 GPU의 도메인 분리 | **토폴로지 인식 순위 배치** | 외부망 우회 감소 |
| 일부 링크의 사용률 편중 | **링·트리 경로 분산** | 유효 대역폭 향상 |
| 링크 장애의 성능 급락 | **오류 감시·경로 재구성** | 집합 통신 지속성 확보 |

### 쉽게 이해하기 (학습용)

- 텐서 통신량이 큰 GPU를 같은 NVSwitch 도메인에 배치하고 링크 편중과 장애 우회를 지속 확인한다.

## Ⅶ. 결론

- **통신량·도메인 경계·경로 균형**으로 GPU 배치 결정
