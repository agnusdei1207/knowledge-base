...
- **정의/개념**: GPU·CPU 메모리의 고대역폭 상호 연결
- **배경/필요성**: 모델 병렬의 매개변수·활성값 교환 대기 감소
...
- GPU 상호 메모리 접근으로 중간 복사 감소한다.
- NVSwitch가 다수 GPU의 동시 경로를 제공한다.
- GPU 배치와 통신 경로가 실제 대역폭을 좌우한다.
- 도메인 밖 통신은 RDMA망 병목을 별도 관리한다.
...
| 판단 기준 | NVLink | PCIe | InfiniBand·RoCE |
|:---|:---|:---|:---|
| 핵심 특징 | GPU·CPU의 스케일업 메모리 연결 | 범용 호스트·장치 연결 버스 | 서버 노드의 스케일아웃 RDMA망 |
| 적용 기준 | 한 도메인의 모델·텐서 병렬 | 저장·망 장치 등 범용 연결 | 랙·클러스터 간 집합 통신 |
| 주요 위험 | 전용 생태계·토폴로지 제약 | 낮은 GPU 간 대역폭·공유 병목 | 망 혼잡·외부 경로 지연 |
...
1. 텐서 병렬 GPU를 같은 NVSwitch 도메인에 배치
2. 노드 내부 NVLink와 외부 RDMA 지연을 분리 측정

### 쉽게 이해하기 (학습용)

- 모델 조각을 자주 교환하는 GPU들은 같은 NVSwitch 아래 배치해 외부망 우회를 줄인다.
- 학습 지연을 분석할 때 서버 안의 NVLink 시간과 서버 사이 RDMA 시간을 따로 재어 병목 위치를 찾는다.

## Ⅶ. 결론

- GPU 통신 거리·도메인·외부망 경계로 배치
