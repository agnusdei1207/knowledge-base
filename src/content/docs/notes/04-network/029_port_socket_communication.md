...
- 정의/개념: **포트**는 호스트 안의 서비스를 식별하고 **소켓**은 **IP**•포트•프로토콜과 연결 상태를 관리한다.
- 배경/필요성: IP 주소만으로는 호스트 안의 서비스와 연결을 구분할 수 없다.
...
- 서버는 고정 포트, 클라이언트는 임시 포트를 사용한다.
- **TCP**는 연결 상태를, **UDP**는 독립 데이터그램을 관리한다.
- 리슨 소켓과 연결 소켓은 수락과 데이터 처리를 분리한다.
...
| 구성요소 | 책임 |
|:---|:---|
| 클라이언트 소켓 | 임시 포트•서버 주소로 통신 상대 지정 |
| 서버 리슨 소켓 | **리슨 소켓**이 서버 IP•고정 포트에서 대기 |
| 연결 큐 | 응용 수락 전 완료 연결 상태 보관 |
| 연결 소켓 | **연결 소켓**이 TCP 데이터를 송수신 |
| 소켓 자원 | **OS**가 **FD**•송수신 버퍼•TCP 상태 관리 |
...
- **연결 수락(accept)**: 완료 연결을 받아 새 연결 소켓을 만드는 호출이다.
- **연결 소켓 반환**: 새 연결의 5-튜플 상태와 파일 서술자를 응용에 제공하는 절차이다.
</details>
...
- 리슨 소켓은 새 연결 요청을 계속 수신하고 수락된 연결은 전용 연결 소켓으로 분리되어 리슨 소켓과 연결 소켓의 책임이 나뉜다.
...
| 핵심 특징 | 연결 상태의 바이트 흐름 전달 | **UDP 데이터그램**별 독립 메시지 전달 |
...
| 연결을 구분하거나 접근을 제한 | **5-튜플**로 식별하고 방화벽 규칙 적용 | 서비스 접근 범위 축소 |
...
- 연결 상태가 필요하면 **TCP 소켓**, 독립 메시지는 **UDP 소켓**을 선택하고 **임시 포트** 범위와 **방화벽** 규칙을 함께 설계한다.
