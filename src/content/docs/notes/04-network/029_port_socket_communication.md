...
date: "2026-08-26T13:46:36+09:00"
tags:
  - "notes-network"
weight: 29
extra:
...
- 배경/필요성: 단일 IP를 공유하는 복수 프로세스 환경에서 **프로세스별 트래픽 다중화/역다중화(Demultiplexing) 불가 및 1:다 동시 통신 제어 불가**를 겪으므로, 호스트를 가리키는 IP 아래에 16비트 포트 번호 계층을 덧대어 커널이 5-튜플 조합만으로 목적 프로세스를 골라내게 하고 응용은 소켓 추상화 하나로 그 조합을 다루게 할 필요
...
- 연결 상태·메시지 경계에 따라 **TCP·UDP 소켓**, 대규모 FD는 **epoll** 선택
