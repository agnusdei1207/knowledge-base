...
- **인터넷 프로토콜(Internet Protocol, IP)**: 네트워크에서 송수신 장치를 주소로 식별하고 패킷을 전달하는 프로토콜
- **포트 번호(Port Number)**: 한 장치 안에서 데이터를 받을 응용 서비스나 프로세스를 구별하는 16비트 번호
- **운영체제(Operating System, OS)**: 프로세스와 네트워크 장치 사이에서 소켓·버퍼 같은 통신 자원을 관리하는 소프트웨어
...
- **파일 서술자(File Descriptor, FD)**: 프로세스가 열린 소켓이나 파일을 참조할 때 사용하는 정수형 식별자
...
- **정의/개념**: 포트는 프로세스를 구분하고 소켓은 통신 상태를 관리
- **배경/필요성**: 한 호스트의 여러 서비스·연결을 분리
...
- 서버 서비스는 고정, 클라이언트는 임시 포트 사용한다.
- 양쪽 IP·포트·프로토콜로 TCP 연결 구분한다.
- 리슨 소켓은 수락, 연결 소켓은 데이터 처리한다.
- TCP 연결마다 FD·버퍼·상태 자원 필요하다.
...
| 판단 기준 | TCP 소켓 | UDP 소켓 |
|:---|:---|:---|
| 핵심 특징 | 연결 상태로 바이트 흐름 전달 | 데이터그램별 독립 전달 |
| 적용 기준 | 파일 전송·웹 연결 | 짧은 질의·실시간 전송 |
| 주요 위험 | 대량 연결 때 FD·버퍼 고갈 | 도착·순서·중복은 응용 처리 |
