...
- 정의/개념: **로드 밸런서**는 **VIP**로 받은 연결과 요청을 여러 정상 백엔드로 전달하는 장비이다.
- 배경/필요성: 단일 서버에는 처리량과 가용성 확장 한계가 있다.
...
- **VIP**는 클라이언트 서비스 주소와 백엔드 주소를 분리한다.
- 비정상 백엔드를 분산 대상에서 제외해야 한다.
- **L4 로드 밸런서**는 연결 정보를, **L7 로드 밸런서**는 **URL** 등 요청 내용을 기준으로 분산한다.
...
| 구성요소 | 책임 |
|:---|:---|
| VIP•리스너 | **리스너**가 VIP의 주소•포트•프로토콜 정의 |
| 스케줄러 | **스케줄러**가 정상 집합•분산 정책으로 백엔드 선택 |
| 서버 풀 | **서버 풀**로 같은 기능의 백엔드 집합 구성 |
...
- **정상 집합 갱신**: 탐사 성공 기준을 통과한 서버만 분산 대상 목록에 반영하는 절차이다.
- **백엔드 선택**: 정상 집합에서 계층 정보•분산 정책•세션 조건에 맞는 서버를 고르는 절차이다.
- **요청 전달**: 선택한 백엔드로 클라이언트 연결이나 응용 요청을 중계하는 절차이다.
...
**상태 확인** 결과를 다음 절차에 반영한다.

1. **상태 탐사**: 포트 연결이나 응용 경로를 검사한다.
2. **정상 집합 갱신**: 탐사 성공 기준을 통과한 서버만을 포함한다.
3. **백엔드 선택**: 계층 정보•정책•세션으로 서버를 결정한다.
4. **요청 전달**: 선택 서버로 연결•요청을 중계한다.
...
| 핵심 특징 | **5-튜플**•상태의 백엔드 선택 | **역방향 프록시**의 요청 내용 해석 |
| 한계 | 포트 정상의 응용 장애 미탐지 | 해석 비용•**TLS** 종료 부하 |

> 요약: L4는 연결, L7은 내용 기반을 선택한다.
...
- 웹 서버의 포트가 열려도 응용이 준비되지 않을 수 있어 `/service` 응답까지 확인한다.
...
- **정상 집합**을 유지하면서 연결 처리량 우선은 **L4 로드 밸런서**, 콘텐츠 분기 필요는 **L7 로드 밸런서**를 선택한다.

#### 한줄 요약

- 서버 수보다 요청을 처리할 수 있는 정상 서버 집합을 정확히 유지하는 것이 중요하다.
