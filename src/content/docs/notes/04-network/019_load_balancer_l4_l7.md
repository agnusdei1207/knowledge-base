...
- 정의/개념: **로드 밸런서** — VIP로 받은 연결과 요청을 상태 확인 결과와 분산 정책에 따라 여러 정상 백엔드로 전달하는 **트래픽 분산 장비**이다.
- 배경/필요성: 단일 서버의 **처리량•가용성 확장 한계**가 발생한다.
...
- VIP의 **클라이언트•백엔드 주소 분리**가 핵심이다.
- 상태 확인의 **비정상 백엔드 제외**가 핵심이다.
- L4 연결•L7 요청의 **분산 기준 차이**가 핵심이다.
...
- 연결 처리량 우선은 **L4**, 콘텐츠 분기 필요는 **L7**을 선택한다.
