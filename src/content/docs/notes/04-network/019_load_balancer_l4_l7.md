...
- 배경/필요성: 단일 서버 수직 확장(Scale-Up)의 한계와 **트래픽 폭증 시 단일 장애점(SPOF) 발생, 무중단 페일오버 불가 및 URL 라우팅 한계 해결 불가**

#### 한줄 요약
- VIP로 백엔드를 추상화하고, 주기적 헬스 체크와 L4(전송)/L7(응용) 분산 알고리즘을 적용한다.
...
#### 한줄 요약
- VIP 리스너, 스케줄러 엔진, 백엔드 서버 풀, 헬스 체크, 세션 지속성 모듈이 결합된다.
...
- 대규모 웹 및 클라우드 서비스의 초고가용성과 수평 확장을 실현하기 위해 **최전방 인입 구간에는 L4 로드 밸런서(DSR/NLB)를 배치하고 마이크로서비스 라우팅 구간에는 L7 로드 밸런서(ALB)를 결합하는 2단계(Tiered) 로드 밸런싱 아키텍처를 표준 수립**하고, **Redis 세션 외부화와 Multi-AZ 무중단 이중화**를 결합하여 완벽한 탄력적 인프라 완성
