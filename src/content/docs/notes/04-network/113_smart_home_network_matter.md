...
- 배경/필요성: 제조사별 **명령•가입•권한 상호운용하기 어렵다**.
...
- IP 기반 **Wi-Fi•유선망•Thread 연결**이 핵심이다.
...
가지의 의미: 가입•운영•기기•연결•변환 책임을 분리한 정적 구조다.
...
1. **기기 증명 요청**: 제어기가 제품 인증 정보 제출을 요구한다.
2. **증명 정보 전달**: 기기가 제품 인증서와 증명값을 제공한다.
3. **신뢰 체인 검증**: 제조사 인증기관과 제품 정보를 확인한다.
4. **운영 자격 설치**: 패브릭 인증서와 접근 권한을 기기에 등록한다.
...
> 요약: **전력•대역폭**과 기존 자산을 기준으로 연결 방식을 선택한다.
...
- 저전력은 **Thread**, 고대역은 **Wi-Fi**, 기존 비 Matter 기기는 브리지를 선택한다.
