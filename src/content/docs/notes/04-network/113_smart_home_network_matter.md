...
```text
[가입 제어기] ----- [패브릭 제어기] ----- [IP 연결 계층] ----- [Matter 기기]
                                                 /       \
                             [Thread 경계 라우터]         [기기 브리지]
```

선의 의미: 가로선은 가입 제어기와 패브릭 제어기의 신뢰 관리가 IP 연결 계층을 통해 Matter 기기에 결합되는 관계이고, 아래 가지는 Thread 경계 라우터와 기기 브리지가 서로 다른 접속망•기기 모델을 IP 계층에 연결하는 정적 연동 구조를 뜻한다.
