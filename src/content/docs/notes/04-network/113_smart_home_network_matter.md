...
- 정의/개념: IP망에서 제조사별 **모델•명령•가입 보안** 통일하는 **응용 계층 표준**이다.
- 배경/필요성: 제조사별 **명령•가입•권한 상호운용 한계** 발생한다.
...
- **공통 데이터 모델** 통한 기능•속성•명령 통일이 핵심이다.
- IP 기반 **Wi-Fi**•유선망•**Thread** 연결이 핵심이다.
- **기기 증명•운영 인증서** 기반 안전한 가입•다중 관리자를 제공한다.
- 하나의 **패브릭** 대상 여러 생태계를 연결하는 **다중 관리자** 활용 지원한다.
...
| 구성요소 | 책임 |
|:---|:---|
| 가입 제어기 | **가입 제어기** 기기 증명•운영 자격 설치 |
| 패브릭 제어기 | **패브릭 제어기** 인증서•키•권한 관리 |
| Matter 기기 | **Matter 기기** 공통 기능•속성•명령 제공 |
| IP 연결 계층 | **IP 연결 계층** 주소 기반 연결 제공 |
| Thread 경계 라우터 | **Thread 경계 라우터** Thread•외부 IP망 라우팅 |
| 기기 브리지 | **기기 브리지** 비 Matter 기기 모델 변환 |
...
1. 기기 증명 요청: **BLE**•IP에서 **기기 증명 요청** 수행한다.
2. 증명 정보 전달: 기기가 **증명 정보 전달** 기반 인증서와 증명값을 제공한다.
3. 신뢰 체인 검증: **신뢰 저장소** 기반 **신뢰 체인 검증** 수행한다.
4. 운영 자격 설치: **커미셔닝** **운영 자격 설치** 후 **IP 운영 통신** 시작한다.
...
| 업무망과 기기망의 과도한 연결 | 필요한 **기기 발견 트래픽** 한정 허용 | 공격면 최소화 |
...
- 저전력은 **Thread 연결**, **Wi-Fi 적용 조건** 만족하는 고대역 기기는 **Wi-Fi 연결**, 기존 비 Matter 기기는 **브리지 연결** 택하는 **연결 방식 선택** 필요하다.
