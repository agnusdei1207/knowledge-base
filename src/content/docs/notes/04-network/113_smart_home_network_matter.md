...
- **Matter(매터)**: 서로 다른 제조사의 스마트 홈 기기를 안전하게 연동하는 응용 계층 표준
- **인터넷 프로토콜 IP(아이피, Internet Protocol)**: 영어 두 낱말의 첫 글자를 쓴 표기로 주소 기반 패킷 전달 규약임
- **스레드(Thread)**: 저전력 IP 메시 통신 규격으로 Matter 기기의 직접 연결 경로를 제공함
- **저전력 블루투스 BLE(비엘이, Bluetooth Low Energy)**: Bluetooth Low Energy의 첫 글자를 쓴 표기로 저전력 근거리 기기 발견과 초기 설정에 사용함
- **패브릭**: 기기와 관리자의 신뢰 영역
...
- **Matter**: 공통 기기 모델과 보안으로 제조사 간 기기를 연동하는 표준
...
- **공통 기기 모델**: 기능·속성·명령 표현을 통일한다.
- **IP 기반 연결**: 유무선망·Thread의 IP 통신을 쓴다.
- **안전한 가입**: 기기 진위 확인 후 자격을 설치한다.
- **다중 관리자**: 한 기기를 여러 생태계와 공유한다.
...
|:---|:---|
| 가입·제어기 | 기기 가입과 명령 수행 |
| 패브릭 신뢰 영역 | 인증서와 접근 권한 관리 |
| Matter 기기 | 공통 데이터 모델의 기능·속성·명령 제공 |
| IP 연결 계층 | 주소 기반 연결 제공 |
| Wi-Fi·유선망 | 고대역폭 기기 연결 |
| Thread 경계 라우터 | Thread 메시망과 다른 IP망 사이 전달 |
| 기기 브리지 | 비표준 기기 기능 변환 |
...
| 비교 기준 | Thread | Wi-Fi·유선망 | 기기 브리지 |
|:---|:---|:---|:---|
| 핵심 특징 | 저전력 메시 연결 | 기존 고속 IP 연결 | 다른 규격을 변환 |
| 적용 기준 | 센서·스위치 | 카메라·허브 | 기존 비표준 기기 |
| 주요 위험 | 경계 라우터 의존 | 무선 혼잡·망 노출 | 기능 손실·권한 우회 |
...
1. **분리망 구축**: 기기를 격리하되 발견 경로를 허용한다.
2. **소유권 이전**: 기존 패브릭 자격을 제거하고 새로 가입한다.
...
- 공통 기기 모델과 안전한 커미셔닝으로 Matter 상호운용성을 확보한다.
