...
date: "2026-07-25T19:57:00+09:00"
tags:
  - "notes-network"
weight: 57
extra:
...
- **정의/개념**: 망 기능을 범용 인프라 소프트웨어로 구현
- **배경/필요성**: 장비 중심 조달의 오케스트레이션 전환
...
- 기능과 하드웨어의 수명주기를 분리한다.
- MANO가 VNF·CNF 배치·확장·치유를 자동화한다.
- 상태 보유 기능은 확장·이동 전에 상태를 동기화한다.
- 가상화 오버헤드가 처리량을 낮추고 지연을 늘린다.
...
| 설계 요소 | 설명 |
|:---|:---|
| NFVO | 서비스 구성과 인프라 간 자원 조정 |
| VNFM | VNF·CNF 인스턴스 수명주기 관리 |
| VIM | NFVI 자원의 할당·감시·회수 |
| VNF·CNF | 소프트웨어로 구현한 개별 망 기능 |
| NFVI | 기능 실행용 컴퓨팅·저장·네트워크 |
...
| 주요 위험 | 가상화 오버헤드·상태 이동 | 증설 시간·공급자 종속 |
...
1. 가상 방화벽은 세션 상태 동기화 후 인스턴스 확장
2. 패킷 게이트웨이는 가속기 자원을 VIM으로 예약

### 쉽게 이해하기 (학습용)

- 방화벽 복제본에 현재 연결 상태를 옮긴 뒤 트래픽을 나눠야 기존 접속이 끊기지 않는다
- 높은 처리량이 필요한 기능은 가속기가 있는 서버에만 배치한다

## Ⅶ. 결론

- 상태 이동·가상화 비용으로 NFV 전환 결정
