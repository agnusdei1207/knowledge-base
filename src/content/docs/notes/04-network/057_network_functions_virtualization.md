...
- 배경/필요성: 전용 하드웨어 장비 도입에 따른 **막대한 CAPEX/OPEX 비용 발생, 신규 서비스 배포 지연(수 개월 소요) 및 탄력적 오토스케일링 불가**를 겪으므로, 기능을 전용 하드웨어에서 떼어 범용 COTS 서버 위 소프트웨어로 옮기고 수명주기를 MANO에 맡겨 장비 발주·설치 비용을 인스턴스 증감 비용으로 대체할 필요
