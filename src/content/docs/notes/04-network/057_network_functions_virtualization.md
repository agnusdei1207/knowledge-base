...
- 정의/개념: 전용 하드웨어 장비와 기능을 분리하고 **범용 COTS 서버 위에서 MANO를 통해 VNF/CNF의 수명주기를 자동화 관리하는 ETSI 표준 가상화 기술**
- 배경/필요성: 전용 하드웨어 장비 도입에 따른 **막대한 CAPEX/OPEX 비용 발생, 신규 서비스 배포 지연(수 개월 소요) 및 탄력적 오토스케일링 불가**

#### 한줄 요약
- COTS 가상화, MANO 자동 오케스트레이션, VNF/CNF 탄력 배포를 통해 통신망의 민첩성을 극대화한다.
...
#### 한줄 요약
- NFVO, VNFM, VIM의 MANO 3요소와 VNF 기능, NFVI 인프라가 결합된다.
...
- **NFV** vs **SDN**: L4~L7 네트워크 기능의 소프트웨어화(NFV)와 L2~L3 패킷 제어 평면의 중앙 집중화(SDN).
