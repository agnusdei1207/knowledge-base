...
1. 자사 접두어의 ROA와 IRR 경로를 일치 등록
2. 고객 BGP 세션에 허용 접두어·최대 수 필터

### 쉽게 이해하기 (학습용)

- 주소별 원점 AS와 최대 접두어 길이를 서명하고 IRR에도 같은 정책을 등록해 사업자 필터 근거를 맞춘다.
- 고객이 계약한 접두어만 광고하게 하고 경로 수가 한계를 넘으면 세션 확산 전에 차단한다.
