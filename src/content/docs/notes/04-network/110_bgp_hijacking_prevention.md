...
- **BGP 라우팅 보안 체계**: 경로 원점 권한•접두어 길이•AS 관계와 외부 전파를 검증하는 다층 통제 체계이다.
- **광고 권한 검증 부재•최장 접두어 우선**: BGP가 주소 광고 권한을 자체 확인하지 않고 더 구체적인 경로를 우선 선택하는 취약성이다.

</details>

- 정의/개념: **BGP 하이재킹 방지**는 **BGP** 경로의 원점•접두어•**AS** 관계를 검증하는 **BGP 라우팅 보안 체계**이다.
- 배경/필요성: **광고 권한 검증 부재•최장 접두어 우선**으로 비인가 경로가 선택된다.
...
- 잘못된 광고의 사업자 간 급속 전파를 통제해야 한다.
- 긴 접두어의 최장 일치 우선 선택을 고려해야 한다.
...
- **BGP 수용 필터**: 이웃 경로를 ROV•접두어 길이•AS 관계 정책과 대조해 수용•차단하는 구성요소이다.
- **라우팅 제어 평면**: 유효한 BGP 경로를 선택•전파•철회하는 구성요소이다.
- **외부 경로 관측기**: 외부 관측점에서 원점과 AS 경로 변화를 탐지하는 구성요소이다.
- **격리 체계**: 의심 경로를 수용 필터에서 차단하여 영향 범위를 제한하는 체계이다.
- **복구 체계**: 상류 사업자와 광고를 철회하고 정상 경로 수렴을 확인하는 체계이다.
...
가지의 의미: 권한 근거•수용•관측•격리•복구 책임을 분리한 구조이다.

| 구성요소 | 책임 |
|:---|:---|
| RPKI | **RPKI**가 원점 권한•허용 접두어 근거 제공 |
| IRR | **IRR**이 접두어•라우팅 정책 등록 제공 |
| BGP 수용 필터 | **BGP 수용 필터**가 **ROV**•접두어•AS 관계 정책 검사 |
| 라우팅 제어 평면 | **라우팅 제어 평면**이 유효 경로 선택•전파•철회 수행 |
| 외부 경로 관측기 | **외부 경로 관측기**가 원점•경로 변화 탐지 |
| 격리 체계 | **격리 체계**가 의심 경로 차단•영향 제한 |
| 복구 체계 | **복구 체계**가 상류 협력•철회•정상 수렴 확인 |

> 요약: 원점•접두어•AS 관계 검증과 외부 관측이 핵심이다.

#### 한줄 요약

- 라우터가 서명과 등록 정책으로 받은 경로를 검사하고 외부 관측점이 인터넷에서 선택된 길을 확인한다.
...
- **수용•이상 판정**: 원점•접두어•AS 관계와 외부 전파를 확인해 경로 수용 여부를 결정하는 단계이다.
- **철회 요청•필터 통보**: 상류 사업자에 광고 철회를 요청하고 이웃 필터에 차단 정보를 제공하는 단계이다.
...
1. **ROA 정책 등록**: 원점 AS•최대 길이•정책의 **ROA 정책 등록**을 수행한다.
2. **원점•접두어 검증**: **ROA**와 원점•최대 길이의 **원점•접두어 검증**을 수행한다.
3. **수용•이상 판정**: AS 관계•외부 전파로 **수용•이상 판정**을 수행한다.
4. **철회 요청•필터 통보**: 상류 차단을 위한 **철회 요청•필터 통보** 후 정상 수렴을 검증한다.
> 요약: 광고 수용 전 검증하고 외부 전파는 상류 협력으로 복구한다.
...
- **원점 하이재킹**: 권한이 없는 AS가 피해 접두어의 원점인 것처럼 광고하는 사고이다.
- **더 구체적 하이재킹**: 피해 접두어보다 긴 하위 접두어를 광고해 최장 일치 선택을 유도하는 사고이다.

</details>

| 경로 사고 유형 | **원점 하이재킹** | **더 구체적 하이재킹** | **경로 누출** |
|:---|:---|:---|:---|
| 적용 기준 | 원점 AS의 **ROA•ROV 불일치** | 허용 최대 길이를 넘는 하위 광고 | 원점은 맞으나 AS 관계가 비정상 |
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 불법 원점•긴 접두어 광고 | **RFC** 9582 기반 ROA•ROV 적용 | 원점 하이재킹 차단 |
| 정상 원점의 **경로 누출** | RFC 9234의 역할•**OTC** 검사 | 관계 위반 경로 차단 |
...
- **BGP 경로 수용 기준**: 원점 권한•접두어 최대 길이•AS 관계가 모두 일치하는 경로만 받아들이는 판단이다.

</details>

- **RPKI 검증 운영**으로 원점•최대 길이•AS 관계가 일치하는 경로만 수용하고 불일치는 **차단•철회**하는 **BGP 경로 수용 기준**이 핵심이다.
