---
sidebar:
  order: 110
  label: "110. BGP 하이재킹 방지 (BGP Hijacking Prevention)"
  badge:
    text: "미출 · 50%"
    variant: note
title: "BGP 하이재킹 방지 (BGP Hijacking Prevention)"
date: "2026-08-13T16:51:54+09:00"
tags: ["notes-network"]
weight: 110
extra:
  question_no: "110"
  source_status: "미출"
  source_history: ""
  priority: 50
  priority_note: "보안•문제대책형: Hijack•Leak 다층 방어"
---

## Ⅰ. 개요

<details><summary>용어 설명</summary>

- **경계 게이트웨이 프로토콜(Border Gateway Protocol, BGP)**: AS 사이에서 인터넷 경로 정보를 교환하는 라우팅 프로토콜이다.
- **BGP 하이재킹 방지(BGP Hijacking Prevention)**: 원점 권한•접두어 길이•AS 관계를 검증해 비인가 경로 전환을 막는 보안 체계이다.
- **자율 시스템(Autonomous System, AS)**: 하나의 관리 정책 아래 운영되는 네트워크와 라우터의 집합이다.
- **BGP 라우팅 보안 체계**: 경로 원점 권한•접두어 길이•AS 관계와 외부 전파를 검증하는 다층 통제 체계이다.
- **광고 권한 검증 부재•최장 접두어 우선**: BGP가 주소 광고 권한을 자체 확인하지 않고 더 구체적인 경로를 우선 선택하는 취약성이다.

</details>

- 정의/개념: **BGP 하이재킹 방지** 기반 **BGP** 경로의 원점•접두어•**AS** 관계를 검증하는 **BGP 라우팅 보안 체계**이다.
- 배경/필요성: **광고 권한 검증 부재•최장 접두어 우선** 기반 비인가 경로가 선택된다.

#### 한줄 요약

- 인터넷에 가짜 길 안내가 퍼지지 않게 주소 소유 권한과 이웃 관계를 검사하고 외부 경로 변화를 계속 감시한다.

## Ⅱ. 특징

<details><summary>용어 설명</summary>

- **경로 기원 인가(Route Origin Authorization, ROA)**: 접두어를 광고할 수 있는 원점 AS와 최대 길이를 명시한 서명 객체이다.
- **다층 검증(Multi-Layer Validation)**: ROA 원점 권한•허용 접두어 길이•이웃 AS 관계 정책과 외부 전파 상태를 함께 확인하는 검증 방식이다.

</details>

- 잘못된 광고의 사업자 간 급속 전파를 통제해야 한다.
- 긴 접두어의 최장 일치 우선 선택을 고려해야 한다.
- 원점•접두어•AS 관계의 **다층 검증** 핵심이다.

#### 한줄 요약

- 주소 주인이 맞아도 배포하면 안 되는 사업자 관계로 경로를 퍼뜨린 누출은 별도 정책 검사가 필요하다.

## Ⅲ. 구조 및 구성요소

<details><summary>용어 설명</summary>

- **경로 기원 검증(Route Origin Validation, ROV)**: BGP 광고의 원점 AS와 접두어 길이를 ROA와 대조하는 검증이다.
- **리소스 공개키 기반구조(Resource Public Key Infrastructure, RPKI)**: 인터넷 번호 자원과 경로 기원 권한을 인증하는 기반구조이다.
- **인터넷 라우팅 등록소(Internet Routing Registry, IRR)**: 사업자의 접두어와 라우팅 정책을 등록하는 데이터베이스이다.
- **BGP 수용 필터**: 이웃 경로를 ROV•접두어 길이•AS 관계 정책과 대조해 수용•차단하는 구성요소이다.
- **라우팅 제어 평면**: 유효한 BGP 경로를 선택•전파•철회하는 구성요소이다.
- **외부 경로 관측기**: 외부 관측점에서 원점과 AS 경로 변화를 탐지하는 구성요소이다.
- **격리 체계**: 의심 경로를 수용 필터에서 차단하여 영향 범위를 제한하는 체계이다.
- **복구 체계**: 상류 사업자와 광고를 철회하고 정상 경로 수렴을 확인하는 체계이다.

</details>

```text
BGP 하이재킹 방지 구조
├─ RPKI
├─ IRR
├─ BGP 수용 필터
├─ 라우팅 제어 평면
├─ 외부 경로 관측기
├─ 격리 체계
└─ 복구 체계
```

가지의 의미: 권한 근거•수용•관측•격리•복구 책임을 분리한 구조이다.

| 구성요소 | 책임 |
|:---|:---|
| RPKI | **RPKI** 원점 권한•허용 접두어 근거 제공 |
| IRR | **IRR** 접두어•라우팅 정책 등록 제공 |
| BGP 수용 필터 | **BGP 수용 필터** **ROV**•접두어•AS 관계 정책 검사 |
| 라우팅 제어 평면 | **라우팅 제어 평면** 유효 경로 선택•전파•철회 수행 |
| 외부 경로 관측기 | **외부 경로 관측기** 원점•경로 변화 탐지 |
| 격리 체계 | **격리 체계** 의심 경로 차단•영향 제한 |
| 복구 체계 | **복구 체계** 상류 협력•철회•정상 수렴 확인 |

> 요약: 원점•접두어•AS 관계 검증과 외부 관측이 핵심이다.

#### 한줄 요약

- 라우터가 서명과 등록 정책으로 받은 경로를 검사하고 외부 관측점이 인터넷에서 선택된 길을 확인한다.

## Ⅳ. 흐름도

<details><summary>용어 설명</summary>

- **원점•접두어 검증(Origin/Prefix Validation)**: 광고 접두어의 원점 AS와 길이를 ROA의 허용 원점•최대 길이와 대조하는 검증이다.
- **ROA 정책 등록**: 허용 원점 AS와 최대 접두어 길이를 RPKI에 게시하는 단계이다.
- **수용•이상 판정**: 원점•접두어•AS 관계와 외부 전파를 확인해 경로 수용 여부를 결정하는 단계이다.
- **철회 요청•필터 통보**: 상류 사업자에 광고 철회를 요청하고 이웃 필터에 차단 정보를 제공하는 단계이다.

</details>

```text
1. ROA 정책 등록
        │
        ▼
BGP 경로 광고
        │
        ▼
2. 원점•접두어 검증
        │
        ▼
3. 수용•이상 판정
        ├─ 정상: 경로 수용•전파
        └─ 이상
             │
             ▼
     4. 철회 요청•필터 통보
             │
             └── 정상 경로 수렴 확인
```

### 동작 원리

1. ROA 정책 등록: 원점 AS•최대 길이•정책의 **ROA 정책 등록** 수행한다.
2. 원점•접두어 검증: **ROA** 및 원점•최대 길이의 **원점•접두어 검증** 수행한다.
3. 수용•이상 판정: AS 관계•외부 전파로 **수용•이상 판정** 수행한다.
4. 철회 요청•필터 통보: 상류 차단을 위한 **철회 요청•필터 통보** 후 정상 수렴을 검증한다.
> 요약: 광고 수용 전 검증하고 외부 전파는 상류 협력으로 복구한다.

#### 한줄 요약

- 주소 권한을 등록하고 받은 경로를 검사하며 사고가 퍼지면 상류 사업자와 함께 가짜 광고를 막고 정상 길로 돌아왔는지 확인한다.

## Ⅴ. 종류 및 비교

<details><summary>용어 설명</summary>

- **경로 누출(Route Leak)**: 원점 AS는 정상이지만 고객•제공자•동료 관계 정책에 어긋나게 배운 경로를 재광고하는 사고이다.
- **ROA•ROV 불일치**: 광고 원점이나 접두어 길이가 등록된 권한 범위를 벗어난 상태이다.
- **원점 하이재킹**: 권한이 없는 AS가 피해 접두어의 원점인 것처럼 광고하는 사고이다.
- **더 구체적 하이재킹**: 피해 접두어보다 긴 하위 접두어를 광고해 최장 일치 선택을 유도하는 사고이다.

</details>

| 경로 사고 유형 | **원점 하이재킹** | **더 구체적 하이재킹** | **경로 누출** |
|:---|:---|:---|:---|
| 적용 기준 | 원점 AS의 **ROA•ROV 불일치** | 허용 최대 길이를 넘는 하위 광고 | 원점은 맞으나 AS 관계가 비정상 |
| 핵심 특징 | 불법 원점의 동일 접두어 | 긴 하위 접두어 최장 일치 | 금지 관계의 경로 재광고 |
| 한계 | 경로 경쟁에 따른 부분 탈취 | 대부분 트래픽의 빠른 탈취 | ROV 통과•광범위한 비인가 경로 전환 |

> 요약: 원점•접두어 길이•AS 관계로 사고를 분류한다.

#### 한줄 요약

- 주소 주인을 속이면 원점 하이재킹, 더 자세한 주소를 내면 구체적 하이재킹, 정상 길을 잘못 유통하면 경로 누출이다.

## Ⅵ. 실무 고려사항 및 대책

<details><summary>용어 설명</summary>

- **외부 전파 지속(Continued External Propagation)**: 로컬에서 경로를 차단해도 다른 사업자가 잘못된 광고를 계속 선택•재전파하는 문제이다.
- **의견 요청 문서(Request for Comments, RFC)**: 인터넷 기술과 운영 규칙을 공개한 표준 문서 체계이다.
- **제공자 역할 표시(Only-To-Customer, OTC)**: 고객에게만 전달해야 하는 경로임을 표시해 관계 위반 재광고를 탐지하는 속성이다.

</details>

| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 불법 원점•긴 접두어 광고 | **RFC** 9582 기반 ROA•ROV 적용 | 원점 하이재킹 차단 |
| 정상 원점의 **경로 누출** | RFC 9234의 역할•**OTC** 검사 | 관계 위반 경로 차단 |
| 사고의 **외부 전파 지속** | 다중 관측•상류 철회 절차 | 복구 시간 단축 |

#### 한줄 요약

- 원점과 최대 길이를 ROA로 등록하고 AS 관계 정책과 외부 경로 관측을 결합해야 누출까지 통제할 수 있다.

## Ⅶ. 결론

<details><summary>용어 설명</summary>

- **차단•철회(Block/Withdraw)**: 원점•최대 길이•AS 관계가 불일치한 경로를 차단하고 광고 철회와 정상 수렴을 확인하는 대응 절차이다.
- **RPKI 검증 운영**: 인증서와 서명 객체를 최신 상태로 유지하고 검증 실패를 감시하는 운영 활동이다.
- **BGP 경로 수용 기준**: 원점 권한•접두어 최대 길이•AS 관계가 모두 일치하는 경로만 받아들이는 판단이다.

</details>

- **RPKI 검증 운영** 기반 원점•최대 길이•AS 관계가 일치하는 경로만 수용하고 불일치는 **차단**•**철회**하는 **BGP 경로 수용 기준** 핵심이다.

#### 한줄 요약

- BGP 하이재킹은 RPKI 하나로 끝나지 않고 접두어 필터와 관계 정책, 외부 전파 탐지와 상류 복구 절차를 함께 운영해야 한다.
