...
- **병렬 검색 메모리**: TCAM은 입력 키를 0·1·무관 값으로 저장된 모든 규칙과 동시에 비교하는 병렬 검색 메모리다.
...
- **우선순위 선택**: 우선순위 선택은 여러 TCAM 규칙이 동시에 일치할 때 정책 순서가 가장 높은 항목의 동작을 고른다.
...
- **TCAM 뱅크**: TCAM 뱅크는 경로·ACL·QoS 등 검색 기능이나 키 폭에 따라 값·마스크·유효 비트 항목을 나눈 하드웨어 영역이다.
...
- **SRAM·해시**: SRAM·해시는 대용량 세션이나 동작 정보를 주소·해시 기반으로 정확 조회하지만 충돌 처리와 다단 조회가 필요할 수 있다.
...
- **TCAM**: TCAM은 범위·접두어·우선순위 검색에 사용하고 정확 일치는 CAM, 대용량 세션은 SRAM·해시로 분리해야 한다.
