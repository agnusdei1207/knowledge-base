...
- 배경/필요성: 순차 메모리의 **규칙 조회 지연**이 발생한다.
...
- **병렬 검색**: 모든 저장 항목을 한 주기에 비교한다.
- **마스크 일치**: 무관 비트 X로 접두어•범위 표현이다.
...
1. **일치 벡터 전달**: 모든 값•마스크 항목을 병렬을 비교한다.
2. **최우선 인덱스 선택**: 가장 높은 우선순위 일치를 결정한다.
3. **동작 인덱스 조회**: **SRAM**의 전달•폐기•재표시 동작을 검색한다.
...
- 범위•우선순위는 **TCAM**, 정확 일치는 **CAM**, 대용량 세션은 **SRAM•해시**가 핵심이다.
