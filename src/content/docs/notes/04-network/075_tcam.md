...
block
    columns 1
    A["검색 키 생성기 | 패킷 헤더에서 비교 필드 결합"]
    B["TCAM 뱅크 | 값·마스크·유효 비트 병렬 비교"]
    C["우선순위 인코더 | 복수 일치 중 최우선 인덱스 선택"]
    D["동작 SRAM | 전달·폐기·재표시 동작 저장"]
    E["규칙 관리자 | 영역 할당·우선순위·사용량 관리"]
    A --- B
    B --- C
    C --- D
    D --- E
...
sequenceDiagram
    ...
    participant 동작SRAM
    패킷키->>TCAM: 1. 검색 키 전달
    TCAM->>우선순위인코더: 2. 일치 벡터 반환
    우선순위인코더->>우선순위인코더: 3. 최우선 인덱스 조회
    우선순위인코더->>동작SRAM: 4. 동작 반환
    동작SRAM->>패킷키: 5. 패킷 동작 실행
```

**동작 원리**

1. **검색 키 전달**: 헤더 필드를 고정 키로 구성
2. **일치 벡터 반환**: 모든 값·마스크 항목을 병렬 비교
3. **최우선 인덱스 조회**: 가장 높은 우선순위 일치 선택
4. **동작 반환**: 해당 SRAM의 전달·폐기 동작 조회
5. **패킷 동작 실행**: 포트·등급·계수 상태 반영
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 기능별 TCAM 고갈 | 경로·ACL·QoS 영역 할당 | 자원 간섭 방지 |
| 규칙 확장량 과소 산정 | 마스크 전개 후 실제 항목 계산 | 용량 예측 정확성 |
| 우선순위 그림자 규칙 | 중복·포함 관계 사전 분석 | 정책 오동작 방지 |
...
- **마스크 전개·우선순위·기능별 용량을 반영한 TCAM 배정**
