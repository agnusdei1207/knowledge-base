...
date: "2026-07-25T12:08:00+09:00"
tags:
  - "notes-network"
weight: 63
extra:
...
- **오류 제어 약어 읽기와 표기**: FEC·HARQ·CRC는 에프이씨·에이치에이알큐·씨알씨로 읽고 영문 핵심 글자를 딴 표기이며, 선행 정정·정정과 재전송 결합·잔여 오류 검출 역할을 함
- **오류율 약어 읽기와 표기**: BER·BLER는 비이알·비엘이알로 읽고 Bit·Block Error Rate의 머리글자를 딴 표기이며, 각각 비트와 블록 단위 오류 비율을 나타냄
- **신호대잡음비(Signal-to-Noise Ratio, SNR·에스엔알)**: 영문 핵심 단어의 머리글자를 딴 표기이며, 코딩 이득과 목표 오류율에 필요한 수신 신호 품질을 나타냄


## Ⅰ. 개요

- 정의: 구조적 중복으로 전송 오류를 검출·정정한다
- 배경: 잡음 채널의 재전송과 잔여 오류를 줄인다
...
- 낮은 부호율은 오류를 줄이나 유효 전송률을 낮춘다
- 최소 거리가 작으면 다중 오류 정정이 실패한다
- 반복 복호는 오류를 줄이나 지연·전력을 늘린다
- 인터리빙은 버스트 오류를 분산해 복호 실패를 줄인다
...
| CRC 결과 반환 | 성공 완료 또는 HARQ 재전송 판정 |
...
| 판단 기준 | 해밍 | 리드-솔로몬 | 터보 |
|:---|:---|:---|:---|
| 핵심 특징 | 신드롬 기반 단일 비트 정정 | 심벌 기반 버스트 오류 정정 | 반복 확률 복호 |
| 적용 기준 | 짧은 블록·낮은 복호 비용 | 연속 심벌 오류 채널 | 코딩 이득 우선 장거리 링크 |
| 주요 위험 | 다중 비트 정정 한계 | 큰 심벌 연산량 | 반복 복호 지연·전력 |
...
1. 위성 링크는 목표 BLER로 터보 반복을 제한한다
2. 리드-솔로몬으로 연속 심벌 오류를 정정한다

### 쉽게 이해하기 (학습용)

- 반복 복호를 늘리되 제어 응답 시간과 단말 전력을 넘기기 전에 멈춘다
- 연속으로 손상된 심벌 묶음을 패리티 심벌로 복원한다

## Ⅶ. 결론

- 목표 BLER·지연·전력으로 부호·부호율을 정한다
