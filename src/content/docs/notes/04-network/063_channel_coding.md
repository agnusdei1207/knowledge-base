...
block
    columns 1
    A["채널 부호기 | 정보에 패리티를 추가해 부호어 생성"]
    B["인터리버 | 인접 심벌 순서 분산"]
    C["잡음 채널 | 전송 신호에 잡음·페이딩·오류 추가"]
    D["역인터리버·복호기 | 순서를 복원하고 연판정값으로 오류 정정"]
    A --- B
    B --- C
    C --- D
```
...
1. **부호어 생성**: 선택한 부호율로 패리티 추가
2. **비트·심벌 순서 분산**: 연속 오류가 다른 부호어 위치로 퍼지게 함
3. **부호 신호 전송**: 잡음·페이딩 채널을 통과
4. **연판정값 전달**: 비트별 가능도를 복호기에 제공
5. **오류 정정**: 부호 제약에 맞는 정보 후보 선택
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 오류 환경과 부호율 불일치 | BER·지연별 부호율·블록길이 시험 | 복호 성공률 확보 |
| 강한 부호의 지연 증가 | 서비스 기한 내 반복 횟수 제한 | 종단 지연 보호 |
| 복호 연산량 과다 | 처리량·전력 기준 구현 비교 | 수신 자원 절감 |
