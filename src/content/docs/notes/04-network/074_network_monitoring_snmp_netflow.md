...
- 정의/개념: SNMP·흐름·표본으로 망 상태를 판단한다
- 배경/필요성: 장애 원인 탐색과 대응 시간을 줄인다
...
block
    columns 1
    A["에이전트·익스포터 | 장비 상태·흐름·패킷 표본 생성"]
    B["컬렉터 | 템플릿·순번·수집 시각 검증과 저장"]
    C["정규화·상관 분석 | 서로 다른 관측 단위를 시간축으로 결합"]
    D["시각화·경보 | 품질 임계치·이상 패턴 통지"]
    A --- B
    B --- C
    C --- D
```
...
sequenceDiagram
    ...
    participant 정규화분석
    participant 경보체계
    컬렉터->>장비에이전트: 1. SNMP 상태 조회
    장비에이전트->>컬렉터: 2. OID·카운터 응답
    장비에이전트->>컬렉터: 3. IPFIX·sFlow 전송
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 수집 시각·순번 불일치 | NTP·레코드 순번·재시작 검증 | 사건 상관 정확성 |
| 고속망 전체 수집 부하 | sFlow 표본율·IPFIX 집계 조정 | 수집 비용 절감 |
...
- **장비 상태·흐름·표본을 시간축으로 결합한 네트워크 관측**
