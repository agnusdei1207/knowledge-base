...
- 정의/개념: **SNMP 상태•트래픽 흐름•패킷 표본**을 수집•상관 분석하는 관측 체계이다.
- 배경/필요성: 단일 지표로는 **장비 장애•트래픽 원인 연결할 수 없다**.
...
- **단위 정규화**: 서로 다른 트래픽 지표를 공통 축으로 변환한다.
- **수집 검증**: 시각•순번•템플릿 오류로 인한 왜곡을 방지한다.
- **신호 결합**: 상태•흐름•표본으로 장비와 주체를 연결한다.
...
1. **SNMP 상태 조회**: 필요한 **OID**를 주기적으로 요청한다.
2. **IPFIX•sFlow 전송**: 흐름 레코드•패킷 표본을 내보내기한다.
3. **순번•템플릿 검증**: 누락•재시작•필드 해석 오류를 식별한다.
4. **정규화 데이터 전달**: 장비•인터페이스•시간 기준을 통일한다.
...
- 장비 상태는 **SNMP**, 통신 주체는 **IPFIX**, 고속망 저부하는 **sFlow**가 핵심이다.
