...
- **네트워크 모니터링(Network Monitoring)**: 상태•흐름•패킷 표본을 수집해 장애 원인을 분석하는 체계
- **단순 네트워크 관리 프로토콜(Simple Network Management Protocol, SNMP)**: 장비의 관리 정보와 상태를 조회•통지하는 프로토콜

</details>

- 정의/개념: **SNMP 상태•트래픽 흐름•패킷 표본**을 수집•상관 분석하는 관측 체계
...
- **객체 식별자(Object Identifier, OID)**: 관리 정보 트리의 항목을 식별하는 값
- **IP 흐름 정보 내보내기(Internet Protocol Flow Information Export, IPFIX)**: 표준 형식으로 흐름 레코드를 내보내는 프로토콜
- **표본 흐름(Sampled Flow, sFlow)**: 패킷 표본과 인터페이스 카운터를 내보내는 기술
...
1. **SNMP 상태 조회**: 필요한 **OID**를 주기적으로 요청
...
- **넷플로(NetFlow)**: 공통 속성의 패킷을 흐름 레코드로 집계하는 기술
...
- **사건 왜곡**: 서로 다른 시점의 상태와 흐름을 같은 원인으로 연결하는 문제
- **네트워크 시간 프로토콜(Network Time Protocol, NTP)**: 네트워크 장비의 시각을 동기화하는 프로토콜
