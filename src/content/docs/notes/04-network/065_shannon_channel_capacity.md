...
- 정의/개념: **AWGN 채널 용량**인 최대 정보율의 이론 경계
...
- **데시벨(Decibel, dB)**: 두 전력비를 상용로그로 나타낸 단위
...
- 전송률이 **채널 용량(C)** 미만이면 오류 확률 감소
- 전송률이 **채널 용량(C)** 초과면 신뢰 통신 불가
- **대역폭(B)**은 선형, **신호대잡음비(SNR)**는 로그 기여
...
5. **운용 전송률 확정**: BLER 측정 결과와 타당성 여유를 확인해 실제 목표 결정
...
- **구현 격차•오버헤드 차감과 실제 굿풋 기반 목표 전송률 확정**
