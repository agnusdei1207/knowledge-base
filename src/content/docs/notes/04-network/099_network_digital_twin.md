...
sequenceDiagram
    ...
    실제망->>트윈: 1. 동기 스냅숏 생성
    트윈->>검증기: 2. 모델 정합성 판정
    변경제어기->>트윈: 3. 변경 시나리오 실행
    트윈->>검증기: 4. 불변 조건 검사
    검증기->>변경제어기: 5. 변경 승인 판정
```

**동작 원리**

- **1. 동기 스냅숏 생성**: 자산·설정·상태를 시점별 고정
- **2. 모델 정합성 판정**: 실제 관측값과 모델 예측값 대조
- **3. 변경 시나리오 실행**: 후보별 경로·정책 변화를 계산
- **4. 불변 조건 검사**: 도달성·격리·용량·지연 확인
- **5. 변경 승인 판정**: 오차와 위반 근거로 변경 결정
