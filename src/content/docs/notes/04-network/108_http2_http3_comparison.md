...
- 정의/개념: 웹 통신의 지연시간을 단축하고 전송 효율을 극대화하기 위해, 단일 TCP 상에서 바이너리 프레이밍으로 다중 스트림을 전송하는 **HTTP/2**와 UDP 기반 QUIC 위에서 독립 스트림과 0-RTT 연결을 구현한 **HTTP/3**를 하부 전송 계층과 HoL 블로킹 해소 관점에서 비교하는 **IETF 표준 웹 전송 프로토콜**
