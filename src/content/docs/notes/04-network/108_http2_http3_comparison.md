...
1. 콘텐츠 전송망 일부 사용자에 HTTP/3 단계 적용
2. 0-RTT는 조회만 허용하고 주문 변경은 차단

### 쉽게 이해하기 (학습용)

- 일부 지역과 사용자부터 HTTP/3를 켜 지연과 패킷 손실, HTTP/2 전환 비율을 비교한 뒤 확대한다.
- 다시 보내도 결과가 같은 조회만 0-RTT로 받고 결제와 주문 변경은 정식 연결이 끝난 뒤 처리한다.
