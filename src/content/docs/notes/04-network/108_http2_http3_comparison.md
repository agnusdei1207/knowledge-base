...
- 배경/필요성: TCP 손실 복구가 다른 스트림까지 막는 **스트림 간 선두 차단** 발생한다.
...
- 공통 HTTP 의미에서 **TCP** 및 **QUIC** 전송 계층 차이가 핵심이다.
- **Alt-Svc** 발견과 스트림 손실 격리•연결 이동으로 **RTT** 활용 줄이는 것이 핵심이다.
- **UDP** 차단•0-RTT의 **HTTP/3 운영•보안 제약** 핵심이다.
...
1. QUIC•TLS 협상: UDP 연결•암호•연결 ID의 **QUIC•TLS 협상** 수행한다.
2. 독립 스트림 전송: 요청별 QUIC 스트림에서 **독립 스트림 전송** 수행한다.
3. HTTP/2 대체: QUIC 실패 시 **HTTP/2 대체** 기반 TCP 연결에 전환한다.
...
- 저손실•UDP 차단 환경은 **HTTP/2**, 손실•이동 환경은 **HTTP/3** 택하는 **HTTP 버전 선택** 필요하다.
