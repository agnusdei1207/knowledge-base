...
date: "2026-07-25T20:19:00+09:00"
tags:
  - "notes-network"
weight: 79
extra:
...
- **정의**: 정보 모델·서비스·보안의 산업 통신 규격
- **필요성**: 이기종 설비 데이터 의미의 공통 해석
...
- 노드·참조로 설비 구조와 의미를 표현한다.
- 탐색·읽기·쓰기·호출·구독 서비스를 제공한다.
- 클라이언트·서버와 발행·구독 모델을 지원한다.
- 값과 측정 시각·품질을 함께 전달한다.
- 인증서와 역할 권한으로 접근 계층을 분리한다.
- 모델 불일치는 값의 해석과 제어를 어긋나게 한다.
...
| 판단 기준 | 클라이언트·서버 | 발행·구독 |
|:---|:---|:---|
| 핵심 특징 | 주소 공간 탐색 후 서비스 직접 요청 | 데이터 집합을 여러 구독자에게 배포 |
| 적용 기준 | 설비 탐색·진단·개별 제어 | 주기 데이터를 다수 시스템에 배포 |
| 주요 위험 | 세션·구독 증가로 서버 부하 | 모델 불일치에 따른 값 오해 |
