...
- 정의/개념: 산업 데이터의 의미•서비스•보안을 통합한 **상호운용 표준**이다.
- 배경/필요성: 제조사별 원시 값은 **이름•단위•품질 의미 공유할 수 없다**.
...
- **의미 모델**: 노드•속성•참조로 산업 데이터 표현이다.
- **표준 서비스**: 탐색•읽기•쓰기•호출 인터페이스이다.
...
1. **접속점 탐색**: 주소•지원 정책•서버 인증서를 확인한다.
2. **보안 채널 생성**: 애플리케이션 신뢰와 메시지 보호를 설정한다.
3. **세션 활성화**: 사용자 인증 후 역할 권한을 연결한다.
4. **주소 공간 탐색**: 노드•속성•참조로 설비 의미를 발견한다.
5. **데이터 서비스 사용**: 읽기•쓰기•호출•구독을 수행한다.
...
- 공통 의미는 **컴패니언 명세**, 개별 제어는 **클라이언트•서버**, 다대다는 **PubSub**가 핵심이다.
