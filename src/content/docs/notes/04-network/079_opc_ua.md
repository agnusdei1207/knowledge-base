...
- **OPC UA·PLC·SCADA·MES·PubSub**: 산업 설비의 제어·감시·생산 실행 데이터를 표준 정보 모델과 발행·구독 방식으로 교환하는 구성요소
...
1. PLC 설비값의 **MES 표준 정보 모델 연계**

### 쉽게 이해하기 (학습용)

- 제조사마다 다른 PLC 값을 OPC UA의 공통 이름·자료형·단위로 모델링해 MES가 같은 의미로 해석하게 한다

## Ⅶ. 결론

- 제조사별 설비 데이터의 의미 불일치를 극복하기 위해 노드·자료형·단위·품질·구독 규모·역할 권한을 검토하여, 표준 정보 모델과 보안 통신을 OPC UA로 구성해야 한다.
