...
- 배경/필요성: 5G 지상 기지국 중심 아키텍처의 물리적 한계로 인해 지상 표면의 20%에 불과한 커버리지 영역(도심 중심), 테라헤르츠(Sub-THz) 초고주파수의 극심한 전파 감쇠 및 차폐 취약성, 통신과 센싱/컴퓨팅의 이원화된 인프라 구축 비효율을 극복하지 못하는 문제를 해결하기 위해, Sub-THz 대역 활용(최대 1Tbps/0.1ms 지연), 지상-위성(LEO/HAPS)을 3차원 입체로 결합하는 비지상망(NTN), 무선 전파로 공간 환경을 인지하는 통신·센싱 융합(ISAC), 전파 경로를 지능형으로 제어하는 RIS, 네트워크 전 계층에 인공지능을 내재화한 AI-Native 아키텍처를 규정한 ITU-R IMT-2030(6G) 표준을 도입하여 **지상·공중·해양·우주를 아우르는 초공간·초연결·초정밀 인텔리전트 무선 네트워크**를 달성할 필요
...
- 2030년대 글로벌 디지털 사회와 우주 항공, 완전 자율주행, 홀로그래픽 텔레프레즌스 및 인간-기계 상호작용(HMI)을 주도할 **차세대 초공간·초고속 지능형 이동통신 종합 표준 기술**로 자리매김하고 있으며, 실무 기술 개발 및 표준화 대응 시에는 **Sub-THz 대역의 경로 손실을 극복하는 RIS 및 분산 Massive MIMO 배치, 저궤도 위성망과의 단절 없는 연동을 위한 조건부 핸드오버(Conditional Handover) 및 도플러 보상, AI-Native 자율 제어망의 발진을 방지하는 Safety Boundary 규칙 설계**를 결합하여 견고한 6G 글로벌 인프라 기반을 완성
