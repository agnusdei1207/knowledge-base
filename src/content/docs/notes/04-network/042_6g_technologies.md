...
- **6세대 이동통신(Sixth-Generation Mobile Communication, 6G)**: 최고 1Tbps 속도, 0.1ms 무선 지연시간, 통신-센싱 융합(ISAC) 및 입체 커버리지를 지향하는 차세대 통신 표준이다.
...
- 정의/개념: **6G(Sixth-Generation Mobile Communication)**는 ITU-R의 **IMT-2030** 비전 체계에 기반하여 최고 전송 속도 1Tbps, 무선 지연시간 0.1ms, 공간 센싱 융합(ISAC) 및 AI 내재화를 목표로 지상망과 비지상망(NTN)을 입체 통합하는 차세대 이동통신 표준 기술이다.
- 배경/필요성: 5G 지상 셀 중심 커버리지 한계(전 지구 면적의 20% 수용)를 극복하고, UAM(도심항공교통), 홀로그램 통신, 자율협력주행 및 공간 감지(Sensing) 등 2030년대 초연결·초지능 사회 인프라를 구축하기 위해 개발된다.

#### 한줄 요약

- 초고속 통신뿐만 아니라 공간 센싱(ISAC), AI 내재화, 비지상망(NTN) 입체 통신을 융합하여 1Tbps 속도와 0.1ms 지연을 구현하는 차세대 이동통신.
...
- **Sub-THz 극초광대역 전송**: 100GHz~3THz 대역의 수십 GHz 광대역폭을 활용하여 5G 대비 50배 빠른 1Tbps 전송 속도를 실현한다.
- **ISAC 기반 통신 및 감지 융합**: 전파 반사 파형을 분석하여 기상, 위치, 이동 객체 형상을 레이더처럼 센싱하는 레이더-통신 통합 기능을 제공한다.
- **3D 공간 입체 연결(NTN)**: 지상 기지국 커버리지 한계를 넘어 저궤도 위성(LEO) 연동을 통해 해상, 공중, 고고도까지 100% 음영 없는 입체 연결을 보장한다.
...
4. **Sub-THz 및 NTN 전송**: 결정된 통합 정책에 따라 서브 테라헤르츠 및 위성 통신망을 거쳐 0.1ms 극저지연 데이터 전송을 개시한다.
...
|:---|:---|:---|
| 최고 전송속도 / 지연 | 최고 1 Tbps (1000 Gbps) / 0.1 ms | 최고 20 Gbps / 1 ms (URLLC) |
...
> 요약: 5G 대비 6G는 50배 빠른 1Tbps 속도, 0.1ms 지연, ISAC 센싱 융합 및 저궤도 위성(NTN) 연동 3D 입체망을 제공.

#### 한줄 요약

- 5G 대비 6G는 50배 빠른 1Tbps 속도, 0.1ms 지연, ISAC 센싱 융합 및 저궤도 위성(NTN) 연동 3D 입체망을 제공.
...
- 6G 미래망 준비 시 **IMT-2030 기술 성숙도 평가**, **RIS/ISAC 융합 무선망 연구**, **지상-비지상(NTN) 통합 아키텍처 수립 필수**.
