...
|-- Active Close Socket (능동 종료 엔드포인트)
|   |-- 4-Tuple Identifier ([Src IP:Port <-> Dst IP:Port] 바인딩 유지)
|   |-- 2MSL Countdown Timer (RFC 793 권고 120초, Linux 커널 60초 카운트다운)
|   `-- Event Handlers
|       |-- Ingress Delayed Segment -> 무시 및 즉각 폐기 (신규 세션 오염 차단)
|       `-- Ingress Retransmitted FIN -> 최종 ACK 즉시 재전송 (상대방 LAST_ACK 탈출)
`-- Timer Expiration -> 4-Tuple 해제 및 소켓 완전 CLOSED 회수
```
...
|:---|:---|:---|
| **TIME_WAIT 상태** | 먼저 연결 해제를 호출한 능동 종료 호스트가 **종료 정합성을 위해 2MSL 동안 유지하는 대기 상태**| 능동 종료자 전담 |
| **2MSL 타이머** | 네트워크 상의 잔존 지연 세그먼트가 **완전히 소멸(Drop)될 때까지 소켓 재사용을 차단하는 시간** | 리눅스 기준 60초 |
| **4-튜플 식별자** | 출발/목적지 IP 및 포트 정보를 유지하여 **해당 포트로 신규 연결이 맺어지는 것을 일시 방어** | 소켓 중복 방지 |
| **ACK 재전송기** | 수동 종료자가 최종 ACK 미수신으로 재전송한 **FIN 패킷을 수신했을 때 즉시 ACK를 재발송** | 교착 상태 해소 |
...
TIME_WAIT 생명주기 및 예외 복구 흐름
        │
   1. [최종 ACK 전송] 능동 종료 측이 서버의 FIN 수신 후 최종 ACK(Ack=v+1) 전송
        │
   2. [TIME_WAIT 진입] 능동 종료 측이 TIME_WAIT 전이 및 2MSL(60초) 타이머 가동
   ┌────┴───────────────────────────┐
  최종 ACK 유실 (서버의 FIN 재수신)  정상 종료 진행 (지연 세그먼트 도달)
   │                                 │
3A. [ACK 즉시 재전송]                3B. [지연 패킷 폐기 (Drop)]
   서버를 LAST_ACK에서 CLOSED로 구제     신규 세션 침범 방지
   │                                 │
   └────────────────┬────────────────┘
                    ▼
   4. [2MSL 타이머 만료] 소켓 자원 및 4-튜플 완전 해제 (CLOSED 전이)
```
...
- 고성능 대규모 분산 API 및 프록시 인프라에서 **TIME_WAIT 상태는 네트워크 무결성을 수호하는 필수 안전 메커니즘**으로 인식하고, 임시 포트 고갈을 해소하기 위해 **HTTP Keep-Alive 커넥션 풀링을 최우선 적용하며 리눅스 커널의 TCP 타임스탬프 기반 `tcp_tw_reuse` 옵션**을 결합하여 고가용성·고처리량 통신 아키텍처 완성
