...
sequenceDiagram
    participant 능동종료측
    participant 수동종료측
    participant TIMEWAIT상태
    participant 2MSL타이머
    능동종료측->>수동종료측: 1. FIN 송신
    수동종료측->>능동종료측: 2. ACK 확인
    수동종료측->>능동종료측: 3. 상대 FIN
    능동종료측->>TIMEWAIT상태: 4. 최종 ACK
    수동종료측->>능동종료측: 5. FIN 재전송
```

**동작 원리**

1. **FIN 송신**: 능동 종료 측의 송신 종료 통지
2. **ACK 확인**: 수동 종료 측이 FIN 수신 확인
3. **상대 FIN**: 수동 종료 측의 송신 종료 통지
4. **최종 ACK**: TIME_WAIT 진입·2MSL 시작
5. **FIN 재전송**: 최종 ACK 유실 시 다시 송신
...
| 무리한 상태 재사용 | 4튜플·타임스탬프 안전성 검증 | 옛 세그먼트 혼입 방지 |
| 최종 ACK 재응답 누락 | 2MSL 동안 연결 정보 유지 | 정상 종료 보장 |
| 원인 없는 타이머 단축 | 경로 MSL·재연결 패턴 계측 | 종료 안전성 유지 |
...
- **최종 ACK 재응답과 옛 세그먼트 격리를 위한 TIME_WAIT 유지**

### 쉽게 이해하기 (학습용)

- TIME_WAIT은 오류가 아니라 늦은 이전 데이터가 새 연결에 섞이지 않게 하는 정상 상태다.
