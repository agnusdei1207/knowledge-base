...
- 정의/개념: **TIME_WAIT 상태** — 능동 종료 측이 최종 ACK를 보낸 뒤 지연 세그먼트 소멸과 ACK 재전송을 위해 연결 정보를 2MSL 동안 유지하는 **TCP 종료 상태**이다.
- 배경/필요성: 최종 ACK 유실•즉시 4-튜플 재사용으로 **종료•재연결 혼선**이 발생한다.
...
- 재전송 FIN에 **ACK 재응답**해 상대 종료 완료가 핵심이다.
- **4-튜플 재사용 지연**으로 이전 연결의 세그먼트 격리가 핵심이다.
- 대량 누적 시 **임시 포트 조합 고갈**로 신규 연결을 제한한다.
...
- 최종 ACK 후 **지연 세그먼트** 위험이 사라질 때까지 **TIME_WAIT**을 유지한다.
