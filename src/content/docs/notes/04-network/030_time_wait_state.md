...
- **정의/개념**: 최종 ACK 후 2MSL 동안 유지하는 TCP 상태
- **배경/필요성**: 종료 재응답과 이전 연결 격리
...
- 재전송 FIN에 ACK으로 재응답해 종료를 보장한다.
- 4-튜플 재사용을 늦춰 이전 연결을 격리한다.
- 누적되면 임시 포트가 부족해질 수 있다.
...
| 설계 요소 | 설명 |
|:---|:---|
| `TIME_WAIT` 상태 | 종료 재응답에 필요한 연결 정보 유지 |
| `2MSL` 타이머 | 이전 세그먼트 소멸 시간 확보 |
| 연결 4-튜플 | 이전 연결과 새 연결을 식별 |
...
| 판단 기준 | `TIME_WAIT` | `FIN_WAIT_2` | `CLOSE_WAIT` |
|:---|:---|:---|:---|
| 핵심 의미 | 재전송 `FIN`과 지연 세그먼트 소멸 대기 | 상대의 `FIN` 대기 | 로컬 응용의 소켓 종료 대기 |
| 적용 기준 | 최종 ACK 송신 뒤 | 로컬 FIN의 ACK 수신 뒤 | 상대 FIN 수신 뒤 |
| 주요 위험 | 임시 포트 조합 부족 | 연결 상태 장기 점유 | 소켓과 FD 미회수 |
