...
- **전송 제어 프로토콜(Transmission Control Protocol, TCP)**: 순서 번호·확인 응답·재전송으로 신뢰성 있는 바이트 흐름을 제공하는 연결형 프로토콜
- **동기화 플래그(Synchronize Flag, SYN)**: 연결 설정을 시작하고 자신의 초기 순서 번호를 알리는 TCP 제어 비트
- **확인 응답 플래그(Acknowledgment Flag, ACK)**: 상대 순서 번호를 받았으며 다음에 받을 번호를 확인값에 담았음을 나타내는 TCP 제어 비트
- **초기 순서 번호(Initial Sequence Number, ISN)**: 연결의 양방향 바이트 순서를 추적하기 위해 각 종단이 독립적으로 정하는 시작 번호
...
- **정의/개념**: SYN·SYN-ACK·ACK로 ISN 동기화
- **배경/필요성**: 데이터 전송 전 양방향 도달성과 순서 기준 확인
...
- 독립 ISN 교환이 양방향 순서·재전송 기준을 설정한다.
- SYN 메시지에서 MSS·윈도 배율 같은 옵션을 협상한다.
- 데이터 전송 전에 최소 1 RTT의 설정 지연이 발생한다.
- 최종 ACK 전 반쪽 연결 상태가 SYN 플러드에 노출된다.
...
| 판단 기준 | 3단계 연결 설정 | 4단계 연결 종료 |
|:---|:---|:---|
| 핵심 특징 | SYN·ACK로 양방향 ISN 확인 | FIN·ACK로 양방향 송신을 독립 종료 |
| 적용 기준 | 데이터 전송 전 연결 상태 생성 | 양방향 데이터 전송 후 연결 해제 |
| 주요 위험 | SYN 플러드·설정 지연 | 한 방향 종료 누락·상태 누적 |
...
1. 공개 서버는 SYN 쿠키로 반쪽 연결 큐 고갈 완화
2. 접속 장애는 SYN·SYN-ACK·ACK 손실 구간을 식별

### 쉽게 이해하기 (학습용)

- 서버는 최종 ACK가 온 요청만 연결 상태로 복원해 가짜 SYN이 큐를 오래 점유하지 않게 한다
- 서버의 SYN 수신, 클라이언트의 SYN-ACK 수신, 서버의 최종 ACK 수신을 차례로 확인한다

## Ⅶ. 결론

- 양방향 도달성·ISN 확인 후 연결 상태로 전환
