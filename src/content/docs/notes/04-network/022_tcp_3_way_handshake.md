...
- 배경/필요성: 무상태 전송의 **상대 도달성·순서 기준 미확인**
...
sequenceDiagram
    ...
    participant 반쪽연결큐
    participant 완료연결큐
    클라이언트->>서버수신부: 1. SYN 요청
    서버수신부->>반쪽연결큐: 2. 반쪽 연결 등록
    서버수신부->>클라이언트: 3. SYN-ACK 응답
    클라이언트->>서버수신부: 4. 최종 ACK
    반쪽연결큐->>완료연결큐: 5. 완료 연결 이동
```

**동작 원리**

1. **SYN 요청**: 클라이언트 ISN·옵션을 제안
2. **반쪽 연결 등록**: 서버가 최종 ACK 대기 상태를 저장
3. **SYN-ACK 응답**: 클라이언트 ISN 확인·서버 ISN 제안
4. **최종 ACK**: 클라이언트가 서버 ISN을 확인
5. **완료 연결 이동**: 설정 완료 상태를 완료 큐로 전환
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| SYN 플러드 큐 고갈 | SYN 쿠키·요청률 제한 | 신규 연결 가용성 |
| MSS 경로 불일치 | 경로 MTU 기반 MSS 조정 | 단편화·블랙홀 방지 |
| 반쪽 연결 장기 점유 | 재전송·대기시간 조정 | 연결 자원 회수 |
| 중간장비 옵션 변경 | 양단 캡처로 SYN 옵션 대조 | 성능 저하 원인 확인 |
...
- **ISN·옵션 동기화와 반쪽 연결 보호를 갖춘 TCP 연결 수립**

### 쉽게 이해하기 (학습용)

- 양쪽 시작 번호와 전송 옵션을 확인해야 이후 데이터의 순서와 크기를 판단할 수 있다.
