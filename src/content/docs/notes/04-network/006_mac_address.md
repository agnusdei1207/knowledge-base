...
```text
+-----------------------------------------------------------------------------------+
|                            48-bit (6 Octets) MAC Address                          |
+---------------------------------------------------+-------------------------------+
|  상위 24-bit: OUI (Organizationally Unique ID)    |  하위 24-bit: NIC 시리얼 번호  |
|  [ Octet 1 ]      [ Octet 2 ]      [ Octet 3 ]    |  [ Octet 4 ][ Octet 5 ][ Octet 6 ]
|  xxxxxx[U/L][I/G]                                 |                               |
+---------------------------------------------------+-------------------------------+
 * I/G Bit: 0 = Unicast, 1 = Multicast/Broadcast
 * U/L Bit: 0 = Universal (UAA), 1 = Local (LAA)
```
...
[ 이더넷 프레임 유입 ]
          |
          v
[ 1. 출발지 주소 학습 (Source MAC Learning) ] ----> [CAM Table] 포트-MAC 매핑 업데이트
          |
          v
[ 2. 목적지 주소 조회 (Destination MAC Lookup) ] --> CAM Table 검색
          |
          +-----------------------------------+
          | (HIT)                             | (MISS / Broadcast)
          v                                   v
[ 3. 단일 포트 전달 (Unicast Forwarding) ]   [ 4. 플러딩 (Flooding) ]
...
| 장애/위안 요소 | 원인 분석 | 실무 대책 및 해결방안 | 기대 효과 |
|:---|:---|:---|:---|
