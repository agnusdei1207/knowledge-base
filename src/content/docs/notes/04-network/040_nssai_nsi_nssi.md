...
- 배경/필요성: 5G 다중 테넌트 환경에서 단말의 요구조건(SLA)과 실제 무선·전송·코어 자원 간의 **매핑 기준 부재 시 슬라이스 선택 오류, 자원 격리 실패 및 라이프사이클 관리 불가**
...
#### 한줄 요약
- S-NSSAI 식별자, AMF/NSSF 선택기, E2E NSI 완성체, 도메인별 NSSI 부품이 결합된다.
...
#### 한줄 요약
- S-NSSAI 요청 → NSSF 검증 → NSI 매핑 → 도메인 NSSI 확인 → 세션 개통 순으로 동작한다.
