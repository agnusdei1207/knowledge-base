...
- **도메인 이름 시스템(Domain Name System, DNS)**: 계층형 이름을 인터넷 프로토콜 주소 같은 자원 정보로 변환하는 분산 데이터베이스
...
- **정의/개념**: 계층형 이름을 자원 레코드로 해석하는 분산 체계
- **배경/필요성**: 전 세계 이름의 권한 분산과 반복 조회 지연 감소
...
- 루트부터 도메인 레이블별 트리로 관리 권한을 분산한다.
- TTL 캐시는 조회 지연을 줄이지만 변경 반영을 늦춘다.
- DNSSEC은 응답을 검증하지만 질의 내용을 암호화하지 않는다.
...
| 판단 기준 | 재귀 질의 | 반복 질의 |
|:---|:---|:---|
| 핵심 특징 | 최종 답·오류 반환을 요청 | 다음 위임 서버 정보를 반복 조회 |
| 적용 기준 | 클라이언트가 리졸버에 요청 | 리졸버가 DNS 계층을 추적 |
| 주요 위험 | 공개 재귀의 증폭 공격 악용 | 위임 오류로 해석 실패 |
...
1. 서비스 이전 전 변경 허용 시간만큼 TTL 단축
2. 공개 리졸버는 허용 클라이언트로 재귀 질의 제한
