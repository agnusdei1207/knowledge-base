...
- **인터넷 제어 메시지 프로토콜(Internet Control Message Protocol, ICMP)**: IP 전달 오류와 진단 상태를 알리는 제어 프로토콜이다.
- **인터넷 그룹 관리 프로토콜(Internet Group Management Protocol, IGMP)**: IPv4 멀티캐스트 그룹 가입을 관리하는 제어 프로토콜이다.
...
- 정의/개념: **ICMP**는 **IP** 전달 오류를 알리고 **IGMP**는 **IPv4** 멀티캐스트 그룹 가입을 관리한다.
- 배경/필요성: IP만으로는 실패 원인과 그룹 수신자를 관리할 수 없다.
...
- **ICMP 유형**과 **ICMP 코드**로 오류와 진단 원인을 구분한다.
- ICMP는 원 패킷 일부로 실패 흐름을 식별한다.
- **IGMP 질의**•**IGMP 보고**•**IGMP 탈퇴**로 가입 상태를 갱신한다.
...
| 구성요소 | 책임 |
|:---|:---|
| IP•멀티캐스트 호스트 | **ICMP** 오류 수신•**IGMP 보고** 송신 |
| ICMP 처리기 | **ICMP 유형**•**ICMP 코드**와 원 패킷 일부 반환 |
| IGMP 질의자 | **IGMP 질의자**가 그룹별 수신자 존재 여부 질의 |
| IGMP 스누핑 표 | **IGMP 스누핑**으로 **VLAN**•그룹•가입 포트 관계 저장 |
...
- **가입 포트 갱신**: IGMP 보고를 관찰하여 VLAN별 그룹과 수신 포트의 대응 관계를 바꾸는 절차이다.
...
**멀티캐스트** 가입 정보는 IGMP 교환 결과에 따라 갱신한다.

1. **ICMP 유형**•**ICMP 코드**: 실패 원인과 원 패킷 일부로 흐름을 식별한다.
2. **IGMP 질의**: 링크의 그룹 수신자 존재 여부를 확인한다.
3. **IGMP 보고**: 호스트가 가입 그룹을 알리고 스위치가 입력 포트를 관찰한다.
4. **가입 포트 갱신**: VLAN별 그룹•가입 포트 관계를 저장한다.
...
> 요약: ICMP는 오류 진단, IGMP는 그룹 가입을 관리한다.
...
| 위조 요청에 ICMP 응답이 과다 발생 | **ICMP 전송률 제한**과 발신 범위 제한 | 반사 공격 트래픽 축소 |
| 질의자 중단으로 IGMP 상태가 만료 | **IGMP 질의자**•스누핑 상태 감시 | 멀티캐스트 수신 단절 예방 |
| **알 수 없는 멀티캐스트 플러딩**으로 미가입 포트에 확산 | VLAN별 **IGMP 스누핑** 적용 | 불필요 트래픽 감소 |
...
- 도달성•PMTUD에는 **ICMP 선별 허용**, 멀티캐스트에는 **IGMP 가입 범위 제한**과 **IGMP 스누핑**을 적용한다.
