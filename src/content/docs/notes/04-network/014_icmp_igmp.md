...
- 배경/필요성: IP는 패킷을 폐기해도 발신지에 알릴 수단이 없어 **오류 원인을 종단이 타임아웃으로 짐작하며 재전송하는 비용**을 치렀고 수신자 선별 수단도 없어 링크마다 플러딩 대역폭을 낭비했으므로, IP 위에 오류·진단 보고 전용 제어 메시지(ICMP)와 그룹 멤버십 상태를 관리하는 제어 프로토콜(IGMP)을 별도로 얹어 데이터 전달 경로를 바꾸지 않고 피드백과 수신자 선별만 추가할 필요
