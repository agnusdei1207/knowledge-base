...
block
    columns 1
    A["IP 송신자 | ICMP 오류·진단 응답 수신"]
    B["ICMP 생성 노드 | 유형·코드와 원 패킷 일부 반환"]
    C["IGMP 질의자 | 그룹 수신자 존재 여부 질의"]
    D["그룹 가입 호스트 | 가입 보고·탈퇴로 의사 전달"]
    E["스누핑 표 | VLAN·그룹·가입 포트 저장"]
    A --- B
    B --- C
    C --- D
    D --- E
...
| 구성요소 | 책임 |
|:---|:---|
| IP 송신자 | ICMP 오류·진단 응답 수신 |
| ICMP 생성 노드 | 유형·코드와 원 패킷 일부 반환 |
| IGMP 질의자 | 그룹 수신자 존재 여부 질의 |
| 그룹 가입 호스트 | 가입 보고·탈퇴로 의사 전달 |
| 스누핑 표 | VLAN·그룹·가입 포트 저장 |
...
sequenceDiagram
    ...
    participant 그룹호스트
    participant 스위치
    라우터->>송신호스트: 1. IP 오류 감지
    송신호스트->>라우터: 2. 진단 결과 해석
    라우터->>그룹호스트: 3. 그룹 가입 질의
    그룹호스트->>라우터: 4. 가입 상태 보고
    라우터->>스위치: 5. 가입 포트 갱신
```

**동작 원리**

1. **IP 오류 감지**: ICMP 유형·코드로 실패 통보
2. **진단 결과 해석**: 원 패킷 일부로 흐름 식별
3. **그룹 가입 질의**: 멀티캐스트 수신 여부 확인
4. **가입 상태 보고**: 가입·탈퇴 메시지 전달
5. **가입 포트 갱신**: 그룹별 수신 포트 반영
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| ICMP 일괄 차단 | PMTUD·진단 유형 선별 허용 | 전송 실패 원인 확인 |
| ICMP 반사·과다 응답 | 유형별 전송률·발신 범위 제한 | 남용 피해 축소 |
| IGMP 가입 상태 오류 | 질의자·스누핑 상태 감시 | 멀티캐스트 단절 예방 |
| 미가입 포트 과다 전달 | VLAN별 IGMP 스누핑 적용 | 불필요 트래픽 감소 |
...
- **진단 필요성과 남용 위험을 구분한 ICMP·IGMP 정책**
