...
| 구성요소 | 책임 |
|:---|:---|
| 컴퓨트 HCA | **컴퓨트 HCA**가 서버 메모리의 RDMA 작업 실행 |
| InfiniBand 스위치 패브릭 | **InfiniBand 스위치 패브릭**이 **LID**•**VL** 경로로 무손실 패킷 전달 |
| 저장 HCA | **저장 HCA**가 저장 장치의 RDMA 종단 제공 |
| 서브넷 관리자 | **서브넷 관리자**가 LID•경로•**P_Key** 관리 |
| 패브릭 관측기 | **패브릭 관측기**가 포트•링크•오류•혼잡 상태 수집 |
...
1. 토폴로지 발견: 링크•스위치•HCA 상태로 **토폴로지 발견**을 수행한다.
2. 경로•파티션 계산: 목적지 경로와 통신 그룹의 **경로•파티션 계산**을 수행한다.
3. LID•P_Key 할당: LID와 P_Key의 **LID•P_Key 할당**을 수행한다.
4. QP•키 연결 설정: **QP**와 원격 메모리 정보로 **QP•키 연결 설정**을 수행한다.
5. 크레딧 RDMA 전송: 수신 버퍼 여유 안에서 **크레딧 RDMA 전송**을 수행한다.
#### 한줄 요약
...
| 장비 간 상호운용 차이 | **IBTA**의 **IBA** 규격 준수 시험 | 패브릭 호환성 확보 |
...
- 전용 최고 성능은 **InfiniBand**, 이더넷 재사용은 **RoCEv2**, 범용 호환은 **TCP**를 택하는 **클러스터 인터커넥트 선택**이 필요하다.
