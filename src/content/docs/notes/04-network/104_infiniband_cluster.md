...
- **정의/개념**: RDMA·크레딧 제어의 전용 스위치 패브릭
- **배경/필요성**: 고성능 클러스터의 저지연 무손실 통신
...
- HCA가 RDMA·전송을 맡아 호스트 연산 부하 감소한다.
- 크레딧 흐름 제어가 링크 계층 손실을 막는다.
- 서브넷 관리자가 주소·경로를 중앙 계산한다.
- 관리자 장애·잘못된 경로가 패브릭 전체에 영향을 준다.
...
| 판단 기준 | InfiniBand | RoCEv2 | TCP 이더넷 |
|:---|:---|:---|:---|
| 핵심 특징 | RDMA 전용 패브릭·크레딧 무손실 | IP 이더넷의 RDMA·혼잡 표시 | 범용 이더넷·커널 신뢰 전송 |
| 적용 기준 | 최고 성능 연산 클러스터 | 기존 이더넷 기반 대규모 RDMA | 범용 호환·운영 단순성 우선 |
| 주요 위험 | 전용 장비·운영 기술·관리자 의존 | 무손실·혼잡 제어 조정 복잡성 | 복사·호스트 연산·지연 부담 |
...
1. 학습 노드를 비차단 스위치 패브릭으로 연결
2. 저장 클러스터를 별도 파티션 키로 격리

### 쉽게 이해하기 (학습용)

- 많은 학습 서버가 동시에 통신해도 중앙 링크가 병목이 되지 않도록 충분한 스위치 경로 용량을 설계한다.
- 저장 서버와 허용된 계산 서버만 같은 파티션 키를 사용하게 해 다른 클러스터의 접근을 차단한다.

## Ⅶ. 결론

- 통신 규모·전용성·관리 역량으로 패브릭 선택
