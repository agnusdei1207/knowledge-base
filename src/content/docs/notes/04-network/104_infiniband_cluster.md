...
sequenceDiagram
    ...
    서브넷관리자->>HCA: 2. LID·파티션 할당
    HCA->>서브넷관리자: 3. 전달 경로 조회
...
- **2. LID·파티션 할당**: 주소와 논리 통신 그룹 배포
- **3. 전달 경로 조회**: 목적지까지 전달 경로를 획득
- **4. QP·키 연결 설정**: 작업 큐와 원격 메모리 정보 공유
- **5. 크레딧 RDMA 전송**: 버퍼 여유 안에서 손실 없이 전달
...
- **통신 규모·비차단성·운영 역량**으로 InfiniBand 결정
