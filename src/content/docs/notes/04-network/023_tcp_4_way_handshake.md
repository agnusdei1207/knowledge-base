...
- 배경/필요성: 전이중(Full-Duplex)으로 동작하는 TCP 연결을 종료할 때, 어느 한쪽이 일방적으로 RST 패킷을 날려 즉시 연결을 끊으면 상대방 송신 버퍼에 남아있던 전송 중 잔여 데이터가 유실되고 이전 세션의 지연 패킷(Stray Segment)이 신규 세션에 오유입되는 데이터 오염 위험이 발생하므로, 송수신 채널을 독립적으로 단계별 종료하는 Half-Close 메커니즘과 FIN-ACK 4단계 플래그 교환 및 최종 2MSL TIME_WAIT 상태를 유지하는 TCP 4-Way Handshake를 도입하여 **잔여 버퍼 데이터의 무손실 송수신 완료와 양방향 소켓 자원의 안전하고 완전한 회수**를 달성할 필요
...
- TCP 프로토콜의 신뢰성 있는 세션 생애주기를 완결하고 시스템 소켓/포트 자원을 안전하게 회수하는 **가장 본질적인 전송 계층 연결 종료 표준 메커니즘**으로 자리잡았으며, 실무 운영 시에는 **대규모 마이크로서비스 및 웹 환경에서 빈번한 종료로 인한 TIME_WAIT 임시 포트 고갈을 방지하는 HTTP Keep-Alive/커넥션 풀링(Connection Pooling)과 소켓 SO_REUSEADDR 적용, 애플리케이션 버그로 인한 파일 디스크립터(FD) 누수를 막는 철저한 CLOSE_WAIT 예외 처리(Try-with-resources)**를 결합하여 안정적인 시스템 자원 관리를 완성
