...
- 정의/개념: **TCP 4-way handshake** — 각 종단이 FIN과 ACK를 교환하여 양방향 송신 스트림을 독립적으로 닫는 **연결 종료 절차**이다.
- 배경/필요성: 한 방향만 닫으면 반대 방향 **잔여 데이터** 보존할 수 없다.
...
- FIN 순서 번호의 **손실•재전송 추적**이 핵심이다.
- 반쪽 종료의 **반대 방향 잔여 전송 허용**이 핵심이다.
- TIME_WAIT의 **최종 ACK 재전송•지연 격리**가 핵심이다.
...
1. **능동 FIN**: 능동 종단이 자기 방향의 **송신 종료**를 통지한다.
2. **ACK**: 수동 종단이 FIN 순서 번호를 확인하고 **반쪽 종료**를 진입한다.
3. **수동 FIN**: 수동 종단이 남은 전송 후 자기 방향 종료를 통지한다.
4. **최종 ACK**: 능동 측이 상대 FIN을 확인한다.
...
- 반대 방향 데이터가 남으면 **반쪽 종료**, 최종 ACK 후 **TIME_WAIT**을 유지한다.
