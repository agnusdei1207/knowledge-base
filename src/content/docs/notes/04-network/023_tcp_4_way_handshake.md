---
sidebar:
  order: 23
  label: "023. TCP 4-way Handshake·연결 해제 (TCP 4-way Handshake)"
  badge:
    text: "기출 · 30%"
    variant: note
title: "TCP 4-way Handshake·연결 해제 (TCP 4-way Handshake)"
date: "2026-07-25T00:50:00+09:00"
tags:
  - "notes-network"
weight: 23
extra:
  question_no: "023"
  source_status: "기출"
  source_history: "132회"
  priority: 30
  priority_note: "설명형: 132회 종료 절차와 TIME_WAIT 연계"
---

## 미리 알고가기

- **전송 제어 프로토콜(Transmission Control Protocol, TCP)**: 양방향 바이트 흐름을 각각 독립적으로 종료할 수 있는 연결형 전송 프로토콜
- **종료 플래그(Finish Flag, FIN)**: 한 종단이 해당 방향으로 더 보낼 데이터가 없음을 알리는 TCP 제어 비트
- **확인 응답 플래그(Acknowledgment Flag, ACK)**: 상대 FIN을 받았으며 다음 순서 번호를 확인했음을 알리는 TCP 제어 비트
- **능동 종료·수동 종료(Active Close·Passive Close)**: 먼저 FIN을 보내는 종료와 상대 FIN을 받은 뒤 자신의 FIN을 보내는 종료
- **반쪽 종료(Half-Close)**: 한 방향의 송신은 끝났지만 반대 방향의 데이터 수신은 계속 가능한 상태
- **FIN 대기(FIN_WAIT)**: 능동 종료 측이 자신의 FIN 확인과 상대 FIN을 기다리는 상태
- **종료 대기(CLOSE_WAIT)**: 상대 FIN을 확인한 응용이 자신의 소켓을 닫아 FIN을 보낼 때까지 기다리는 상태
- **마지막 확인 대기(LAST_ACK)**: 수동 종료 측이 자신의 FIN에 대한 최종 ACK를 기다리는 상태
- **시간 대기(TIME_WAIT)**: 능동 종료 측이 최종 ACK 재전송과 이전 연결의 지연 세그먼트 소멸을 위해 대기하는 상태
- **최대 세그먼트 수명(Maximum Segment Lifetime, MSL)**: TCP 세그먼트가 네트워크에 남아 있을 수 있다고 가정하는 최대 시간
- **2MSL(투 엠에스엘)**: 숫자 2와 Maximum Segment Lifetime의 약어를 붙여 MSL의 두 배를 뜻하는 표기로 양방향 지연 세그먼트 소멸과 최종 ACK 재전송을 보장하는 TIME_WAIT 기준임
- **임시 포트(Ephemeral Port)**: 클라이언트가 새 연결을 만들 때 운영체제가 일시적으로 할당하는 출발지 포트
- **연결 재사용(Connection Reuse)**: 여러 요청을 이미 설정된 TCP 연결로 보내 새 연결 설정·종료 횟수를 줄이는 방식



## Ⅰ. 개요

- **정의/개념**: FIN·ACK로 양방향 송신을 독립적으로 종료
- **배경/필요성**: 남은 반대 방향 데이터 전달과 지연 세그먼트 격리

### 쉽게 이해하기 (학습용)

- 한쪽이 보낼 일을 끝내도 상대는 남은 데이터를 보낼 수 있어 두 방향을 각각 확인하며 닫는다

## Ⅱ. 특징

- FIN은 순서 번호 하나를 소비해 손실·재전송을 추적한다.
- 반쪽 종료가 상대의 남은 데이터 전송을 허용한다.
- 능동 종료 측은 최종 ACK 후 TIME_WAIT을 유지한다.
- CLOSE_WAIT 누적은 응용의 소켓 종료 지연을 의미한다.

### 쉽게 이해하기 (학습용)

- 먼저 닫은 쪽은 마지막 확인이 사라질 경우에 대비해 기다리고 상대는 남은 일을 마친 뒤 자기 방향을 닫는다

## Ⅲ. 아키텍처 및 구성요소

| 설계 요소 | 설명 |
|:---|:---|
| FIN·ACK 제어 | 방향별 종료 요청·확인을 추적 |
| 능동 종료 상태 | FIN_WAIT 후 TIME_WAIT으로 전이 |
| 수동 종료 상태 | CLOSE_WAIT 후 LAST_ACK으로 전이 |
| 반쪽 종료 | 한 방향 송신 종료·반대 수신 유지 |
| TIME_WAIT 타이머 | 최종 ACK 재전송·지연 세그먼트 격리 |

> 요약: 방향별 FIN·ACK와 TIME_WAIT으로 종료 관리

### 쉽게 이해하기 (학습용)

- FIN을 받은 쪽도 남은 데이터를 처리한 뒤 자기 방향의 FIN을 따로 보낸다

## Ⅳ. 원리 및 절차 흐름도

```mermaid
sequenceDiagram
    participant A as 능동 종료 측
    participant P as 수동 종료 측
    participant T as TIME_WAIT 타이머
    A->>P: FIN 요청
    P-->>A: FIN 확인 ACK
    P->>A: 상대 FIN
    A-->>P: 최종 ACK
    A->>T: TIME_WAIT 진입
    T-->>A: 연결 자원 해제
```

| 절차 | 설명 |
|:---|:---|
| FIN 요청 | 능동 측이 자기 송신 종료를 알림 |
| FIN 확인 ACK | 수동 측이 FIN 순서 번호를 확인 |
| 상대 FIN | 수동 측이 남은 전송 후 종료를 알림 |
| 최종 ACK | 능동 측이 상대 FIN을 확인 |
| TIME_WAIT 진입 | 최종 ACK 재전송 가능 상태를 유지 |
| 연결 자원 해제 | 2MSL 후 이전 연결 상태를 제거 |

> 요약: 양방향 FIN·ACK 후 지연 세그먼트 격리

### 쉽게 이해하기 (학습용)

- 최종 ACK가 사라지면 상대가 FIN을 다시 보내고 능동 종료 측이 ACK로 재응답한다

## Ⅴ. 종류 및 비교

| 판단 기준 | 능동 종료 측 | 수동 종료 측 |
|:---|:---|:---|
| 핵심 특징 | 먼저 FIN 송신 후 TIME_WAIT 담당 | 상대 FIN 확인 후 응용 종료 대기 |
| 적용 기준 | 연결 종료를 먼저 요청한 종단 | 종료 요청을 먼저 받은 종단 |
| 주요 위험 | TIME_WAIT·임시 포트 누적 | CLOSE_WAIT·소켓 자원 누적 |

> 요약: CLOSE_WAIT은 응용의 소켓 종료 지연

### 쉽게 이해하기 (학습용)

- 먼저 닫은 쪽은 늦은 패킷을 정리하고 요청받은 쪽은 응용이 소켓을 닫을 때까지 기다린다

## Ⅵ. 실무 사례

1. 짧은 외부 호출은 연결 재사용으로 TIME_WAIT 감소
2. CLOSE_WAIT 누적은 응용의 소켓 종료 경로 점검

### 쉽게 이해하기 (학습용)

- 요청마다 새 연결을 닫으면 호출 측에 TIME_WAIT이 쌓이므로 기존 연결을 재사용한다
- 상대가 종료했는데 CLOSE_WAIT이 계속 남으면 응용이 작업 뒤 소켓을 닫는 코드 경로를 확인한다

## Ⅶ. 결론

- 종료 주체·상태 분포로 재사용·소켓 종료를 결정

### 쉽게 이해하기 (학습용)

- TIME_WAIT은 먼저 닫은 쪽의 보호 대기이고 CLOSE_WAIT은 받은 쪽의 응용 종료 대기이므로 조치가 다르다
