...
- 정의/개념: 신원•단말•맥락에 따라 **망 권한을 지속 조정**하는 **보안 통제 체계**이다.
- 배경/필요성: 최초 인증만으로는 접속 후 상태 악화•위협 변화를 **권한에 반영할 수 없다**.
...
- 보안 준수의 **상태 기반 권한 판단**이 핵심이다.
- 위협 변화의 **동적 재평가•재지정**이 핵심이다.
...
- **정책 서버**: 신원•상태•접속 맥락을 결합해 최종 접근 권한을 결정한다.
- **AAA 서버**: 인증•인가•사용 기록 기능을 제공한다.
...
- **신원 정보원**: 사용자•단말 등록과 소속•역할 정보를 제공한다.
- **상태 정보원**: 단말 보안 준수와 위협 탐지 정보를 제공한다.
- **업무망**: 정책을 충족한 주체에게 업무 자원 접근을 제공한다.
...
가지의 의미: 주체•집행•판정•정보•접근 구역 책임을 분리한 구조다.

| 구성요소 | 책임 |
|:---|:---|
| 접속 단말 | **자격•보안 상태** 제시 |
| 접근 집행점 | 인증 중계와 **VLAN•ACL** 적용 |
| 정책 서버 | **상태•맥락 평가와 권한 결정** |
| AAA 서버 | **인증•인가•사용 기록** 제공 |
| 신원 정보원 | **사용자•단말 등록•역할** 제공 |
| 상태 정보원 | 보안 준수•**위협 정보** 제공 |
| 업무망 | 준수 주체의 **업무 자원 접근** 제공 |
| 격리망 | 비준수 주체의 **치료 자원 접근** 제공 |
...
- **격리망 적용**: 비준수 단말의 업무 자원 접근을 막고 패치•치료 자원만 허용한다.
...
1. **신원•단말 정보 전달**: 집행점이 자격과 식별 정보를 정책 서버에 중계한다.
2. **상태•맥락 조회**: 등록•보안 준수•위협 상태를 확인한다.
3. **권한 정책 전달**: 정책 서버가 허용•격리•재지정 결과를 제공한다.
4. **접근 구역•정책 적용**: 집행점이 VLAN•ACL과 접근 영역을 설정한다.
...
- **통합 NAC**: 신원•상태•접속 맥락을 종합하여 접근 권한을 지속 조정한다.
...
> 요약: **판단 정보의 범위**에 따른 접근 통제 세분화가 핵심이다.
...
- 신원•상태•위험도 충족 시 **업무망 권한**, 미충족 시 **격리•치료**가 핵심이다.
