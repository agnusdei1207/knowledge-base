...
```text
[접속 단말] ----- [접근 집행점] ----- [정책 서버] ----- [AAA 서버]
                       /       \          /       \
                  [업무망]   [격리망] [신원 정보원] [상태 정보원]
```

선의 의미: 가로선은 접속 단말, 접근 집행점, 정책 서버와 AAA 서버의 정적 인증•정책 경계이고, 위아래 가지는 신원•상태 정보원이 정책 판단을 지원하며 접근 집행점이 업무망과 격리망의 권한 경계를 적용하는 관계를 뜻한다.
