...
- **텔레메트리(Telemetry)**: 장치의 상태·측정값·이벤트를 원격 관리 체계로 보내는 데이터
- **자격 증명(Credential)**: 장치의 신원과 접근 권한을 검증하는 키·인증서 정보
- **멱등 동기화(Idempotent Synchronization)**: 같은 이벤트를 반복 반영해도 상태가 한 번 처리한 결과와 같은 동기화
