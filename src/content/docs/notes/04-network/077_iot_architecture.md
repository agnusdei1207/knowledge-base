...
- **IoT 운영 구조**: 사물 인터넷(Internet of Things, IoT) 아키텍처는 장치•게이트웨이•엣지•플랫폼의 데이터 흐름과 제어•수명주기 책임을 연결하는 운영 구조다.
...
- **IoT 플랫폼**: 사물 인터넷(Internet of Things, IoT) 플랫폼은 장치 등록•데이터 저장•규칙 실행•원격 명령과 수명주기 상태를 통합 관리한다.
...
- **5. 수명주기 갱신**: 플랫폼은 장치별 무선 업데이트(Over-the-Air, OTA) 버전•배포 결과•자동 롤백•폐기 상태를 기록해 운영 기준선을 현행화한다.
...
- **게이트웨이 중계형**: 게이트웨이 중계형은 이기종 사물 인터넷(Internet of Things, IoT) 장치 프로토콜을 변환•집계해 상위 플랫폼에 연결하지만 게이트웨이가 단일 장애점이 될 수 있다.

</details>

| IoT 연결 구조 | 엣지 협업형 | 게이트웨이 중계형 | 직접 연결형 |
|:---|:---|:---|:---|
| 적용 기준 | **저지연•단절 제어** | **이기종 장치** 혼재 | 소수 **인터넷 프로토콜(Internet Protocol, IP) 장치** |
...
- **장치 벽돌화**: 장치 벽돌화는 무선 업데이트(Over-the-Air, OTA) 실패 뒤 정상 부팅이나 원격 복구가 불가능해 현장 교체가 필요한 상태다.
...
- 저지연•단절 제어는 **엣지형**, 이기종은 **게이트웨이형**, 소수 인터넷 프로토콜(Internet Protocol, IP) 장치는 **직접형**
