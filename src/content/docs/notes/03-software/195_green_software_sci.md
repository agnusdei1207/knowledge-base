...
- **정의/개념**: 기능 단위당 유발 탄소 배출량 지표임
- **배경/필요성**: 총배출량은 작업량에 좌우돼 기능당 지표가 필요함
...
| 판단 기준 | SCI | 총 배출량 | 에너지 사용 |
|:---|:---|:---|:---|
| 핵심 특징 | 기능당 탄소 효율 측정 | 전체 탄소 총량 측정 | 소비 전력량 측정 |
| 적용 기준 | 버전·구조 효율 비교 | 조직·서비스 총량 관리 | 전력 비용·효율 개선 |
| 주요 위험 | 경계·기능 단위 왜곡 | 사용량 증가와 혼동 | 전력원 탄소 차이 누락 |
...
2. API 버전은 호출당 SCI와 지연을 비교함

### 쉽게 이해하기 (학습용)
- 배치는 마감 시간을 지키며 배출량을 줄임
- API는 지연을 악화시키지 않은 감축만 채택함
