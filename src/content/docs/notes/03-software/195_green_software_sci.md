...
- 정의/개념: 기능당 운영•내재 탄소의 **소프트웨어 탄소 집약도**이다.
- 배경/필요성: 총배출량만으로는 작업량이 다른 버전의 **탄소 효율 비교** 할 수 없다.
...
- **운영 배출 O•내재 배출 M** 결합이 핵심이다.
- **에너지 E•탄소 집약도 I** 기반 운영 배출량을 산정한다.
- **기능 단위 R•측정 경계** 기반 정규화가 핵심이다.
- **API 호출•거래량** 기반 기능 단위 설정이 핵심이다.
- **탄소 인지•텔레메트리** 기반 시간•지역별 감축을 측정한다.
- **상쇄 제외•실제 감축** 중심 버전을 비교한다.
...
- **1. 측정 경계•기능 단위 확정**: 비교할 서비스•자원•작업량 기준을 정하는 단계이다.
- **2. E•I•M•R 자료 수집**: 전력•탄소 계수•제조분•작업량을 모으는 단계이다.
- **3. O•SCI 산출**: 운영 배출량과 기능당 탄소를 계산하는 단계이다.
- **4. 기준선•변경안 SCI•SLO 비교**: 탄소 효율과 서비스 품질을 함께 판정하는 단계이다.
...
1. **측정 경계•기능 단위 확정**: 서비스•자원•작업량을 정의한다.
2. **E•I•M•R 자료 수집**: 전력•탄소 계수•제조분을 수집한다.
3. **O•SCI 산출**: O=E×I와 SCI=(O+M)/R을 계산한다.
4. **기준선•변경안 SCI•SLO 비교**: 탄소•품질 동시를 판정한다.

> 요약: 동일 경계에서 **기준선•변경안 SCI**을 비교한다.
