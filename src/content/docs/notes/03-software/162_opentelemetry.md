...
- **정의/개념**: 관측 신호의 생성·전파·수집을 표준화한 프레임워크
- **배경/필요성**: 언어·도구별 계측 차이로 공통 규약 필요
...
| 비교축 | SDK 직접 전송 | Collector 경유 전송 |
|:---|:---|:---|
| 핵심 특징 | SDK가 내부 처리 후 백엔드로 직접 전송함 | 수집기를 경유해 필터링 후 최종 전송함 |
| 적용 기준 | 오버헤드를 최소화해 단일 백엔드로 보낼 때 | 필터링이나 다중 백엔드 라우팅이 필요할 때 |
| 주요 위험 | 앱 자격증명 노출·전송 유실 | 수집기 병목·큐 적체 |
