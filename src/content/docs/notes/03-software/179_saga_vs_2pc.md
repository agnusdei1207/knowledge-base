...
- 정의/개념: 분산 환경에서 다중 데이터베이스 간의 데이터 무결성을 보장하기 위해, 코디네이터 중심의 2단계 투표로 전역 원자성을 유지하는 **2단계 커밋(2PC)**과 로컬 트랜잭션 연쇄 실행 및 실패 시 보상 트랜잭션으로 최종 일관성을 맞추는 **Saga 패턴**을 트랜잭션 지속 시간과 결합도 관점에서 비교·선택하는 **분산 트랜잭션 관리 기법**
