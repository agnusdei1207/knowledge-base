...
- **로컬 커밋·보상**: 서비스별 거래를 독립 확정하고 후속 실패 시 반대 업무로 이미 반영된 효과를 상쇄하는 방식이다.
- **준비·결정**: 참여자가 커밋 가능 상태와 잠금을 확보한 뒤 조정자의 전역 결정을 기다리는 2단계 커밋 절차이다.
...
- **2단계 커밋 조정자(Two-Phase Commit Coordinator, 2PC Coordinator)**: 준비 투표를 모으고 전역 커밋·롤백 결정을 영속화·전달하는 구성요소이다.
- **참여 서비스·자원**: 사가에서는 로컬 거래를 커밋하고 2단계 커밋에서는 준비 잠금을 유지하는 구성요소이다.
...
- **사가(Saga)**: 장기 업무를 로컬 커밋으로 나누고 실패 시 보상해 최종 일관성으로 수렴시키는 패턴이다.
- **2단계 커밋(Two-Phase Commit, 2PC)**: 짧은 분산 거래에서 준비 잠금과 전역 결정으로 강한 원자성을 제공하는 방식이다.
...
- **확장 아키텍처 거래(Extended Architecture Transaction, XA Transaction)**: 자원 관리자와 거래 관리자가 2단계 커밋으로 조정하는 표준 분산 거래이다.
