...
- 정의/개념: 보상 수렴과 전역 원자성의 **Saga•2PC 선택 기준**이다.
- 배경/필요성: 단일 데이터베이스 거래로는 **독립 서비스의 원자 처리 보장할 수 없다**.
...
- **로컬 커밋•보상** 기반 Saga 최종 일관성이 핵심이다.
- **준비•결정** 기반 2PC 원자성이 핵심이다.
- **코레오그래피•오케스트레이션** 기반 Saga을 조정한다.
...
- **로컬 커밋**: Saga 참여자가 각 단계의 거래를 독립 확정하는 처리이다.
- **보상 거래**: 완료 단계의 업무 효과를 역순으로 상쇄하는 처리이다.
...
- **보상 가능성•원자성 요구•거래 시간**으로 Saga•2PC을 결정한다.
