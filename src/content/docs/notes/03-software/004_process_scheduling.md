...
date: "2026-08-05T13:04:29+09:00"
tags: [notes-software]
weight: 4
extra:
  question_no: "004"
...
- **실행 순서•CPU 사용시간**으로 응답•처리량•공정성 조정
...
- **CPU**: 디스패치된 프로세스의 명령을 실행하는 처리 자원
...
1. **준비 후보 갱신**: 준비 상태와 정책별 순서 반영
2. **정책 기준 실행 대상 선택**: 우선순위•버스트•가상시간 평가
3. **문맥 복원•디스패치**: 할당량 설정 후 CPU 제어권 이전
4. **선점 인터럽트**: 시간 할당량 만료로 CPU 회수
5. **문맥 저장•재등록**: 실행 상태를 보존하고 준비 큐 복귀

#### 한줄 요약

- **후보 선택•디스패치•선점•재등록**의 반복 스케줄링
