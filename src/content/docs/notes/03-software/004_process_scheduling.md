...
```mermaid
sequenceDiagram
    participant P as 실행 가능 프로세스
    participant Q as RR 준비 큐
    ...
    D->>C: ⑤ 문맥 복원·실행 시작
    D->>T: ⑥ 할당량 타이머 설정
    alt 할당량 전에 CPU 버스트 완료
        C->>I: ⑦ I/O 요청·대기 전환
        I-->>Q: ⑧ 완료 이벤트 뒤 준비 큐 등록
    else 할당량 만료
        T->>D: ⑨ 선점 인터럽트
        D->>Q: ⑩ 문맥 저장·큐 꼬리 재등록
    end
...
1. 새로 실행 가능해진 프로세스를 RR 준비 큐의 꼬리에 넣는다.
2. CPU가 비면 스케줄러가 큐 머리의 실행 대상을 요청한다.
3. 준비 큐가 가장 오래 기다린 프로세스의 문맥을 반환한다.
4. 스케줄러가 선택 결과와 시간 할당량을 디스패처에 전달한다.
5. 디스패처가 문맥을 복원해 CPU 실행을 시작한다.
6. 동시에 타이머에 이번 실행 구간의 할당량을 설정한다.
7. 프로세스가 할당량 전에 I/O를 요청하면 자발적으로 CPU를 반납하고 대기 상태로 간다.
8. I/O 완료 이벤트가 발생하면 프로세스를 준비 큐 꼬리에 다시 넣는다.
9. 계속 실행해 할당량이 끝나면 타이머가 선점 인터럽트를 보낸다.
10. 디스패처가 현재 문맥을 저장하고 프로세스를 준비 큐 꼬리에 재등록한다.
11. 스케줄러가 갱신된 큐에서 다음 실행 대상을 선택한다.
12. 디스패처가 다음 문맥을 복원하며 순환 실행을 이어간다.
...
- 컨테이너 호스트는 CFS 가중치·할당량으로 CPU 몫을 분리하고, 준비 시간·스로틀링·꼬리 응답시간을 함께 감시한다.
