...
```mermaid
sequenceDiagram
    participant Q as 실행 프로세스·RR 준비 큐
...
> 요약: 준비 큐와 실행 계측값으로 대상과 실행 시점을 정한다.

**동작 원리**

- **① 준비 등록**: 새 실행 가능 프로세스를 큐 꼬리에 삽입
- **② 대상 요청**: CPU가 비면 큐 머리 조회
- **③ 문맥 반환**: 가장 오래 기다린 프로세스 선택
- **④ 실행 시작**: 문맥을 복원해 CPU 배정
- **⑤ 타이머 설정**: 이번 실행의 시간 할당량 지정
- **⑥ 자발적 반납**: 할당량 전 I/O 요청 시 대기 전환
- **⑦ 준비 복귀**: I/O 완료 뒤 큐 꼬리에 등록
- **⑧ 선점 통지**: 할당량 만료 시 인터럽트 발생
- **⑨ 꼬리 재등록**: 현재 문맥 저장 후 준비 큐로 이동
- **⑩ 다음 선택**: 갱신된 큐에서 다음 대상 결정
- **⑪ 다음 문맥**: 선택 프로세스의 저장 상태 반환
- **⑫ 순환 실행**: 문맥 복원 후 다음 할당량 시작
...
- 버스트 예측성·응답 요구·CPU 몫을 검토해 워크로드 특성에 맞는 스케줄링 정책을 선택한다.
