...
date: "2026-07-25T01:41:00+09:00"
tags: [notes-software]
weight: 4
extra:
  question_no: "004"
...
- **중앙처리장치(Central Processing Unit, CPU, 시피유) 버스트**: 영문 각 단어의 첫 글자를 따 CPU로 표기하며 입출력 없이 CPU를 연속 사용하는 시간
...
- **선입 선처리(First-Come, First-Served, FCFS, 에프시에프에스)**: 영문 각 단어의 첫 글자를 따 FCFS로 표기하며 도착 순서로 프로세스를 선택하는 방식
- **최단 작업 우선(Shortest Job First, SJF, 에스제이에프)**: 영문 각 단어의 첫 글자를 따 SJF로 표기하며 예상 CPU 버스트가 가장 짧은 작업을 선택하는 방식
- **라운드 로빈(Round Robin, RR, 알알)**: 영문 각 단어의 첫 글자를 따 RR로 표기하며 시간 할당량마다 작업을 순환 선점하는 방식
- **멀티레벨 피드백 큐(Multilevel Feedback Queue, MLFQ, 엠엘에프큐)**: 영문 각 단어의 첫 글자를 따 MLFQ로 표기하며 실행 이력으로 동적 우선순위를 조정하는 방식
- **완전 공정 스케줄러(Completely Fair Scheduler, CFS, 시에프에스)**: 영문 각 단어의 첫 글자를 따 CFS로 표기하며 가중 가상 실행시간이 가장 작은 작업을 선택하는 방식
- **운영체제(Operating System, OS, 오에스)**: 영문 각 단어의 첫 글자를 따 OS로 표기하며 프로세스·메모리·장치와 CPU 실행 순서를 관리하는 소프트웨어
...
- **감소·증가 기호($\downarrow$·$\uparrow$)**: 각각 ‘아래 화살표·위 화살표’로 읽고 지표의 감소와 증가를 나타내는 관례 표기이며 할당량 감소가 응답 대기는 줄이고 문맥 전환은 늘리는 상충을 표시한다.
...
- **정의/개념**: 실행 대상과 CPU 시간을 결정하는 정책
- **배경/필요성**: 성능·공정성이 상충해 작업별 정책 필요
...
- 응답시간·처리량·공정성은 서로 상충한다
- 짧은 할당량은 응답 대기를 줄인다
- 짧은 할당량은 문맥 전환 비용을 높인다
- 부정확한 버스트·우선순위는 기아를 낳는다
...
| 설계 요소 | 설명 |
|:---|:---|
| 준비 큐 | 실행 가능 프로세스를 정책 기준으로 관리 |
| 스케줄러 | 정책 기준으로 다음 실행 대상 선택 |
| 디스패처 | 현재 문맥 저장·선택 문맥 복원과 CPU 제어권 이전 |
| 타이머·실행량 계측 | 실행량 측정·할당량 만료 통지 |
...
sequenceDiagram
    ...
    participant O as 스케줄러·디스패처
    participant T as 타이머
    P->>Q: 준비 등록
    O->>Q: 정책 선택
    O->>P: 디스패치
    alt 할당량 만료·고우선 작업 도착
        T-->>O: 선점 통지
        O->>Q: 재등록
    else 입출력 요청·실행 완료
        P->>O: CPU 반납
...
| 절차 | 설명 |
|:---|:---|
| 준비 등록 | 신규·대기 해제 프로세스를 준비 큐에 삽입 |
| 정책 선택 | 도착·버스트·우선순위·실행량으로 선택 |
| 디스패치 | 선택 프로세스 문맥 복원 후 실행 전환 |
| 선점 통지 | 할당량 만료·고우선 도착 통지 |
| 재등록 | 미완료 프로세스를 준비 큐에 재삽입 |
| CPU 반납 | 입출력 대기·종료 시 제어권 반환 |

> 요약: 정책 선택과 실행 결과로 다음 대상을 재결정
...
| 주요 위험 | 호위 효과 | 예측 오차·긴 작업 기아 | 짧은 할당량의 전환 횟수 증가 | 규칙 복잡성·기아 | 가중치 설정 편향 |
...
1. 실시간 음성 프로세스는 RR 할당량으로 순환 실행
2. 컨테이너 CPU 가중치를 CFS 실행 몫에 반영

### 쉽게 이해하기 (학습용)

- 음성 처리는 짧게 번갈아 실행해 한 작업이 CPU를 오래 독점하지 않게 한다
- 여러 컨테이너가 CPU를 함께 쓸 때 가중치로 각자의 사용 몫을 나눈다

## Ⅶ. 결론

- 버스트 예측성·응답 요구·CPU 몫으로 정책 선택
