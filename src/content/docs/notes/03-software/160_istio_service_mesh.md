...
- **배경/필요성**: 횡단 통신 코드 중복으로 서비스 메시 필요
...
| 비교축 | 사이드카 모드 | 앰비언트 모드 |
|:---|:---|:---|
| 핵심 특징 | 파드마다 프록시가 L4와 L7을 처리함 | 공유 프록시와 웨이포인트로 역할을 나눔 |
| 적용 기준 | 세밀한 라우팅과 보안 격리를 제공할 때 | L4 보안은 공유하고 L7은 선택할 때 |
| 주요 위험 | 파드별 자원 사용·업그레이드 작업 | 공유 프록시 장애·기능 편차 |
...
2. 내부 API는 mTLS 신원과 인가 정책으로 호출 제한

### 쉽게 이해하기 (학습용)
- 새 버전은 일부 요청만 보내 오류가 없을 때 비율을 늘린다.
- 내부 API는 인증된 서비스 중 허용 목록의 호출만 받는다.
