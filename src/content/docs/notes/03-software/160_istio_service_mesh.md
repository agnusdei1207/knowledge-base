...
│                        이스티오 서비스 메시 토폴로지                   │
├────────────────────────────────────────────────────────────────────────┤
│ 제어면(CONTROL PLANE): [istiod (Pilot + Citadel + Galley)]             │
│                     │ (xDS 설정 / mTLS 인증서 배포)                    │
│                     ▼                                                  │
│ 데이터면(DATA PLANE) (파드 A)           데이터면(DATA PLANE) (파드 B)  │
│ ┌─────────────────────────────┐  mTLS    ┌───────────────────────────┐ │
│ │ [앱 A] ◄─► [Envoy 프록시 A] │ ───────► │ [Envoy 프록시 B] ◄─►[앱 B]│ │
│ └─────────────────────────────┘ 암호화  └───────────────────────────┘ │
└────────────────────────────────────────────────────────────────────────┘
```
...
```text
[사용자 요청] ──► [이스티오 인그레스 게이트웨이] ──► [가상 서비스 (가중치 분할)]
                                                            │
                                    ┌───────────────────────┴───────────────────────┐
                                    ▼ (90%)                                         ▼ (10%)
                      [목적지 규칙 v1 파드]                       [목적지 규칙 v2 파드]
```
...
- **서비스 메시 기반 MSA 트래픽 제어 및 보안 체계 고도화 구현 완료**
