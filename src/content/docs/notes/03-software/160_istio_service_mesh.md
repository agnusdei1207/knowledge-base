...
- 정의/개념: 코드에서 분리한 **프록시 통신 정책 계층**
- 배경/필요성: 서비스별 **신원·암호화·라우팅·관측 통합**
...
1. **서비스 요청 전달**: 호출 정보·추적 문맥의 프록시 수신
2. **mTLS 연결 수립**: 양쪽 워크로드 신원 인증·암호화
3. **인증·인가·경로 판정**: 접근 권한·목적지 정책 집행
4. **허용 요청 전달**: 선택 서비스로 업무 호출 중계
5. **처리 결과 반환**: 응답 중계·통신 텔레메트리 기록
...
| mTLS 전환 중 **단절** | 허용 모드 관찰 후 **강제 적용** | 미대응 호출 **선확인** |
...
- **L7 범위·신뢰 경계·비용**으로 Sidecar·Ambient 결정
