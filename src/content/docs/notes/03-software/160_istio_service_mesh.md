...
- 정의/개념: 데이터면 프록시와 제어면으로 서비스 통신 정책을 애플리케이션 코드에서 분리하는 **서비스 메시 인프라 계층**이다.
- 배경/필요성: 서비스별 통신 코드로는 **정책 일관성•변경 통제하기 어렵다**.
...
- **제어면•데이터면 분리** 기반 정책 집행이 핵심이다.
- **워크로드 신원•mTLS** 기반 통신 보안이 핵심이다.
...
- **1. 송신 정책•인증서 배포**: 호출 경로와 송신 신원을 제공한다.
- **2. 수신 정책•인증서 배포**: 접근 권한과 수신 신원을 제공한다.
- **3. mTLS 상호 인증•암호화**: 양쪽 워크로드 신원을 확인한다.
- **4. 수신 인가•업무 요청 중계**: 허용 호출만 대상에 전달한다.
...
- 전면 L7은 **Sidecar**, L4 중심은 **Ambient**, 선택 L7에 Waypoint을 배치한다.
