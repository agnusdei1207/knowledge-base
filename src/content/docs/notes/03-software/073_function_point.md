...
| 내부 논리 파일(ILF) | 측정 대상이 유지·관리하는 데이터 |
| 외부 인터페이스 파일(EIF) | 외부가 관리하고 대상은 참조 |

**동작 원리**

- ① 측정자가 개발·변경·대가 산정 목적과 사용자 관점을 확인한다.
- ② 사용자 요구에서 논리적으로 식별되는 기능과 데이터를 제공한다.
- ③ 측정 대상 애플리케이션의 안과 밖을 구분하는 경계를 정한다.
- ④ 경계가 내부 유지 데이터와 외부 참조 데이터를 구분한다.
- ⑤ EI·EO·EQ·ILF·EIF를 식별하고 DET·RET·FTR 수로 복잡도를 판정한다.
- ⑥ 검토자가 기능 중복·유형·복잡도 가중치의 일관성을 확인한다.
- ⑦ 유형별 개수와 가중치를 곱해 전체 기능점수를 합산한다.
- ⑧ 검토자가 측정 근거와 확정 규모를 이해관계자에게 전달한다.
...
| 고려사항 | 문제점 | 대책 |
|:---|:---|:---|
| 경계 불일치 | 같은 요구를 내부·외부로 달리 봐 규모 편차 발생 | 측정 목적·애플리케이션 경계·연계 대상을 사전 합의 |
| 기능 중복 | 화면·API 개수를 그대로 세어 논리 기능 중복 | 사용자 목적과 독립 처리 기준으로 기능 식별 |
| 유형 오분류 | EO와 EQ, ILF와 EIF 판정이 흔들림 | 상태 변경·파생 계산·데이터 유지 책임을 근거로 기록 |
| 복잡도 편차 | DET·RET·FTR 해석이 측정자마다 다름 | 측정 규칙 교육·동료 검토·판정 사례 기준선 운영 |
| 규모의 비용 오용 | 기능점수를 생산성·난이도와 동일시 | 비기능 요구·기술 위험·팀 역량을 별도 보정 근거로 사용 |
...
- 기능점수는 구현 기술과 무관하게 사용자가 인식하는 논리 기능 규모를 측정한다.
- 경계·기능 유형·복잡도 판정 근거를 일관되게 관리한다.
