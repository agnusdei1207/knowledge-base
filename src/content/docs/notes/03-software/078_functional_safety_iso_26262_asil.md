...
- ① 운전 상황별 오동작 위해를 S·E·C로 평가해 ASIL과 안전 목표를 도출한다.
- ② 안전 목표를 기능·기술 요구로 분해하고 하드웨어·소프트웨어에 할당한다.
- ③ 개발팀이 안전 메커니즘 구현과 정적·동적 검증 증거를 제출한다.
- ④ 시스템 설계가 통합된 안전 요구의 검증을 요청한다.
- ⑤ 검증팀이 ASIL에 맞는 독립성과 방법으로 결함 주입·요구 시험을 수행한다.
- ⑥ 구현이 고장을 탐지하고 안전 상태로 전환한 결과를 반환한다.
- ⑦ 안전 목표부터 요구·구현·시험까지 양방향 추적 근거를 제출한다.
- ⑧ 안전 책임자가 미해결 이상과 잔여 위험을 포함해 목표 달성을 판정한다.
...
| 고려사항 | 문제점 | 대책 |
|:---|:---|:---|
| HARA 범위 누락 | 드문 운전 상황·예측 가능한 오용이 빠짐 | 운영 상황·오동작·영향을 체계적으로 조합·검토 |
| ASIL 과소평가 | S·E·C 근거 부족으로 안전 활동 축소 | 등급 판정 근거·가정·독립 검토 보존 |
| 요구 추적 단절 | 안전 목표가 구현·시험과 연결되지 않음 | 목표–FSR–TSR–HSR·SSR–시험 양방향 추적 |
| 독립성 미달 | 개발자가 자신의 안전 결과만 승인 | ASIL별 검증·확인 조치의 독립성 확보 |
| 변경 영향 누락 | 코드·부품 변경이 기존 안전 논증을 무효화 | 형상·변경 관리와 영향 분석·회귀 안전 시험 수행 |
...
- ISO 26262:2018은 차량 전기·전자 시스템의 오동작 위험을 ASIL에 따라 수명주기 전체에서 통제한다.
- HARA의 안전 목표를 상세 요구·구현·독립 검증 증거까지 추적한다.
