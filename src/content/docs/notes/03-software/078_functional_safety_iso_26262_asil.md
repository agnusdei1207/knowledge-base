...
- 배경/필요성: 차량의 전동화·SDV(소프트웨어 정의 차량) 및 자율주행 전환에 따라 전장 소프트웨어 복잡도가 급증하고, 전자제어장치(ECU) 오작동이 탑승자의 사망 사고와 천문학적 리콜 비용으로 직결되는 위험을 방지하기 위해, HARA(위험원 분석 및 위험도 평가) 3요소(심각도 S, 노출빈도 E, 제어가능성 C)를 통해 자동차 안전 무결성 등급(ASIL A~D)을 차등 부여하고 V-Model 전 수명주기에서 안전 요구사항을 엄격히 통제하는 ISO 26262를 통해 **차량 시스템의 기능적 오작동 결함을 최소화하고 탑승자 안전을 원천 보증**할 필요
...
- 글로벌 완성차(OEM) 및 전장 부품 산업의 **법정 필수 기능 안전(Functional Safety) 국제 표준 규격**으로 확립되었으며, 실무 SDV 및 자율주행 아키텍처 구현 시에는 **HARA 기반의 엄격한 ASIL D 등급 부여(MC/DC 100%, MISRA-C/C++), 개발 비용을 절감하는 ASIL Decomposition(분해), 센서 인지 한계를 극복하는 ISO 21448 SOTIF(의도된 기능 안전) 및 사이버보안 ISO/SAE 21434와의 융합 체계**를 구축
