...
- 정의/개념: NUMA 토폴로지에 따라 스레드와 페이지를 공동 배치하는 운영체제 **스케줄링 기법**이다.
- 배경/필요성: CPU 부하만 고려하면 스레드•페이지가 분리돼 **원격 접근 지연**이 증가한다.

#### 한줄 요약

- 스레드•페이지 공동 배치로 **원격 접근 비용** 절감이 핵심이다.
...
- **원격 접근 통제**로 지연•대역폭 경합 감소한다.
- **퍼스트 터치•친화도**로 초기 지역성을 확보한다.
- **이주 비용 비교**로 부하 균형과 지역성을 조정한다.

#### 한줄 요약

- 지역성 이득과 **스레드•페이지 이주 비용** 절충이 핵심이다.
...
- **1. 토폴로지•부하 수집**: 거리•용량•부하 상태를 확인한다.
- **2. 스레드•페이지 공동 배치**: 친화도•퍼스트 터치를 적용한다.
- **3. 원격 접근 통계 감시**: 접근률•지연•대역폭 관측이다.
- **4. 이주 편익 판정**: 원격 절감량과 이동 비용을 비교한다.
- **5. 스레드•페이지 이주**: 편익 임계값 초과 대상 이동이다.

#### 한줄 요약

- 원격 접근 편익이 이주 비용을 넘을 때 **재배치**가 핵심이다.
...
- 메모리 거리 반영은 NUMA 인지, 부하 중심은 **비인지**가 핵심이다.
...
- 초기 공동 배치와 **편익 임계값•재이주 대기**를 적용한다.
...
- 원격 접근 절감량이 이주 비용보다 크면 **스레드•페이지 이주**, 아니면 **공동 배치 유지**가 핵심이다.

#### 한줄 요약

- 원격 접근 절감량과 이주 비용으로 **재배치 결정**이 핵심이다.
