...
- 배경/필요성: 멀티소켓(Multi-Socket) NUMA 서버 환경에서 스레드가 실행되는 CPU 코어와 할당된 메모리 노드가 불일치할 때 발생하는 소켓 간 인터커넥트(UPI/Infinity Fabric) 원격 접근 지연(로컬 대비 2~3배) 및 버스 대역폭 포화 병목을 극복하고, First-Touch 메모리 할당 정책과 AutoNUMA 런타임 페이지/스레드 이주를 통해 **스레드와 메모리를 동일한 물리 NUMA 노드에 배치하여 메모리 접근 레이턴시를 최소화**할 필요
