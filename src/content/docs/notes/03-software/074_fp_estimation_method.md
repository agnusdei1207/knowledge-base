...
- 정의/개념: 평균 가중치의 간이법과 상세 복잡도 가중치의 정통법으로 기능점수를 구하는 **FP 하위 산정법**이다.
- 배경/필요성: 불완전한 초기 요구에는 DET•RET•FTR이 없어 **정통법 산정할 수 없다**.

#### 한줄 요약

- 요구 상세도에 따른 **간이•정통 기능점수 산정**이 핵심이다.
...
1. **다섯 기능 유형 식별**: EI•EO•EQ•ILF•EIF을 분류한다.
2. **요구 상세도 판정**: 간이법•정통법을 선택한다.
3. **선택 산정법 가중치 적용**: 평균값•상세값을 반영한다.
4. **UFP 합산**: 유형별 기능 수와 가중치를 계산한다.
5. **근거 검토•기준선 확정**: 누락•중복을 확인한다.

> 요약: 요구 상세도에 맞는 가중치를 선택하고 판정 근거와 함께 기능점수 확정이 핵심이다.

#### 한줄 요약

- 요구 상세도 판정과 **간이•정통 가중치 적용**이 핵심이다.
...
- **독립 처리(Elementary Process)**: 화면과 외부 인터페이스 구현에 무관한 최소 기능 단위다.
...
| 같은 기능을 화면•외부 인터페이스별로 중복 계수 | **사용자 목적•독립 처리**로 검토 | **논리 기능 왜곡** 방지 |
...
- 경계•버전 고정 후 **간이값의 정통법 재산정**이 핵심이다.
...
- 초기•개략 예산은 **간이법**, 상세•최종 정산은 **정통법**을 선택한다.

#### 한줄 요약

- 초기 예산은 간이법, 상세 정산은 **정통법**이 핵심이다.
