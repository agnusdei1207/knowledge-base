...
date: "2026-07-25T18:17:00+09:00"
tags:
  - "notes-software"
weight: 85
extra:
...
- **정의/개념**: 트랜잭션 신뢰성을 지키는 네 성질임
- **배경/필요성**: 동시 변경·장애로 원자적 거래 보장 필요
...
| 비교축 | ACID 모델 | BASE 모델 |
|:---|:---|:---|
| 핵심 특징 | 즉시 일관성과 거래 완결성을 보장함 | 가용성과 분산 상태 수렴을 우선함 |
| 처리 방식 | 로그와 격리로 경계를 보장함 | 복제 지연과 비동기 수렴을 허용함 |
| 적용 기준 | 금융 거래의 즉시 정합성이 중요할 때 | 대규모 분산 가용성이 중요할 때 |
| 주요 위험 | 잠금 경합·분산 확장 비용 | 일시 불일치·보상 처리 복잡도 |
...
1. 출금·입금을 묶어 한쪽만 반영되는 상태 차단
2. 재시작 시 WAL로 커밋 복구·미완료 롤백

### 쉽게 이해하기 (학습용)
- 출금과 입금을 하나로 묶으면 중간 장애가 발생해도 잔액 합계가 유지됨
- 데이터를 쓰기 전에 로그를 남기면 장애 뒤에도 확정 작업과 미완료 작업을 구분해 복원할 수 있음

## Ⅶ. 결론

- 업무 정합성과 처리 비용에 맞게 ACID 구현
