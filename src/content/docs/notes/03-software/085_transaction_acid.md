...
- **1. 트랜잭션·격리 수준 요청**: 경계와 관찰 범위를 결정
- **2. 허용 버전·잠금으로 연산**: 동시 순서를 통제
- **3. 변경 후보 전달**: 처리 후 상태를 전달
- **4. 제약·불변식 판정**: 유효 상태인지 확인
- **5. 커밋 로그 안정 저장**: 데이터보다 로그를 먼저 기록
- **6. 데이터 반영·커밋**: 확정 변경을 적용
- **7. 성공·롤백 결과**: 결과나 재시도 여부를 통지
