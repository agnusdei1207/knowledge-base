...
- 정의/개념: **ACID**는 트랜잭션이 장애와 동시 실행 중에도 원자성•일관성•격리성•지속성을 지키도록 요구하는 **네 가지 속성**이다.
- 배경/필요성: 장애•동시 실행은 **부분 반영•불변식 위반•확정 결과 소실**을 유발한다.

#### 한줄 요약

- 원자성•일관성•격리성•**지속성 기반 거래 보호**가 핵심이다.
...
- **원자성**: 모든 연산을 함께 반영하거나 취소이다.
- **일관성•격리성**: 불변식과 동시 간섭을 통제한다.
- **지속성**: 커밋 결과를 장애 뒤에도를 복구한다.

#### 한줄 요약

- 전체 반영•불변식•동시 간섭•**장애 복구 통제**가 핵심이다.
...
- **3. WAL•Undo 기록**: 데이터보다 복구•취소 정보를 먼저를 보존한다.
- **4. 커밋•롤백 지시**: 성공 변경 반영 또는 미완료 변경 취소이다.
...
> 요약: **ACID**는 거래 신뢰성, **BASE**는 분산 가용성 우선이 핵심이다.

#### 한줄 요약

- 거래 불변식은 ACID, 지연 수렴은 **BASE**가 핵심이다.
...
- **메시지(Message)**: 서비스 사이의 비동기 상태 변경 전달한다.
...
| 일관성을 데이터베이스 엔진 책임으로만 보아 업무 규칙 누락 | **데이터베이스 제약•원자 연산** 함께 설계 | **업무 불변식 누락** 방지 |
...
- **업무 불변식•격리 수준•복구 목표**로 거래 경계를 결정한다.

#### 한줄 요약

- 불변식•격리•복구 목표 기반 **거래 경계 결정**이 핵심이다.
