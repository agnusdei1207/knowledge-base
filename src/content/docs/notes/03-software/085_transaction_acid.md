...
- 배경/필요성: 다중 사용자 동시 접근 환경에서 발생하는 데이터 갱신 분실(Lost Update) 및 일관성 훼손과 시스템 다운·크래시 발생 시의 부분 반영(Partial Commit) 결함을 극복하고, 원자성(Undo Log 기반 All-or-Nothing), 일관성(도메인·참조 무결성 보존), 격리성(MVCC/2PL 동시성 제어), 지속성(WAL/Redo Log 영구 보존)의 4대 속성을 DBMS 엔진이 보장함으로써 **장애와 병행 제어 환경에서도 완벽한 데이터 무결성과 신뢰성을 확립**할 필요
