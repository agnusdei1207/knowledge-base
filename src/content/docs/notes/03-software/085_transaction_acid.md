...
- **1. 트랜잭션·격리 수준 요청**: 경계와 관찰 범위를 정한다.
- **2. 허용 버전·잠금으로 연산**: 동시 순서를 통제한다.
- **3. 변경 후보 전달**: 처리 후 상태를 보낸다.
- **4. 제약·불변식 판정**: 유효 상태인지 확인한다.
- **5. 커밋 로그 안정 저장**: 데이터보다 로그를 먼저 남긴다.
- **6. 데이터 반영·커밋**: 확정 변경을 적용한다.
- **7. 성공·롤백 결과**: 결과나 재시도 여부를 알린다.
...
- **업무 불변식·격리 수준·복구 목표**로 거래 경계를 정한다.
