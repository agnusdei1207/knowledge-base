...
- 배경/필요성: 여러 작업이 같은 데이터를 동시에 고치거나 중간에 장애가 나면 응용이 스스로 부분 반영을 되돌리고 순서를 맞춰야 하는데 이 복구 논리는 응용마다 중복되고 장애 시점을 모두 예측할 수도 없으므로, Undo·WAL 로그와 동시성 제어를 엔진 안에 두고 트랜잭션이라는 단위가 그 책임을 대신 지게 할 필요
