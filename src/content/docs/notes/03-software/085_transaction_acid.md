...
- **부분 반영•불변식 위반•확정 결과 소실**: 장애나 동시 실행이 트랜잭션에 만드는 핵심 데이터 오류이다.
...
- **일관성•격리성**: 업무 불변식을 유지하고 동시 트랜잭션의 허용 밖 간섭을 막는 성질이다.
...
- **Undo 로그•롤백**: 미완료 변경을 변경 전 값으로 되돌려 전체 취소를 구현하는 수단이다.
- **WAL•장애 복구**: 데이터 페이지보다 로그를 먼저 저장해 커밋 결과를 복원하는 방식이다.
- **WAL(Write-Ahead Logging, 선행 기록 로그)**: 변경된 데이터 페이지를 저장하기 전에 복구용 로그 레코드를 안정 저장소에 먼저 기록하는 방식이다.
...
- **ACID**: 원자성•일관성•격리성•지속성으로 거래 신뢰성을 우선하는 관점이다.
- **BASE**: 기본 가용성•유연 상태•최종 일관성으로 분산 가용성을 우선하는 관점이다.
...
- **위험별 격리 수준•잠금**: 허용할 수 없는 동시성 이상에 맞춰 격리 수준과 명시적 잠금 범위를 선택하는 통제이다.
- **DB 제약•원자 연산**: 데이터베이스 제약조건과 조건부 갱신 같은 단일 연산을 함께 사용해 업무 불변식을 보장하는 방식이다.
- **멱등성•제한 재시도•충돌 피드백**: 일시 충돌을 중복 효과 없이 제한적으로 복구하는 방식이다.
- **로컬 ACID•메시지•보상 처리**: 분산 작업을 긴 원자 거래 대신 지역 거래와 비동기 조정으로 나누는 방식이다.
...
- **업무 불변식•격리 수준•복구 목표**: 안전한 거래 경계를 결정하는 규칙, 동시 관찰 범위, 장애 복원 기준이다.
