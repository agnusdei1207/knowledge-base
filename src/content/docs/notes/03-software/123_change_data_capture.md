...
- 배경/필요성: 주기적 폴링은 변경 여부와 무관하게 매번 원천 테이블을 조회해 소스 DB의 CPU를 반복 소모하면서도 이미 사라진 행은 볼 수 없어 삭제를 놓치므로, 트랜잭션이 이미 남긴 로그를 읽어 변경 사실 자체를 이벤트로 흘려보냄으로써 원천에 추가 부하 없이 삭제까지 포착할 필요
