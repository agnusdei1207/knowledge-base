...
sequenceDiagram
    ...
    C->>D: 1. Snapshot·로그 위치 요청
    D-->>C: 2. 기존 행·위치 반환
    C->>B: 3. Snapshot 이벤트 발행
    D-->>C: 4. 이후 커밋 로그 제공
    C->>B: 5. 변경 이벤트 발행
    B->>S: 6. 파티션 순서 전달
    C->>O: 7. 원천 위치 저장
...
- **1. Snapshot·로그 위치 요청**: 초기 경계를 구한다.
- **2. 기존 행·위치 반환**: 기준 데이터를 받는다.
- **3. Snapshot 이벤트 발행**: 초기 상태를 보낸다.
- **4. 이후 커밋 로그 제공**: 변경분을 읽는다.
- **5. 변경 이벤트 발행**: 순서·스키마를 포함한다.
- **6. 파티션 순서 전달**: Sink에 보낸다.
- **7. 원천 위치 저장**: 재시작 책갈피를 남긴다.
...
- **기준 위치·삭제·순서·스키마·보존 기간**으로 CDC를 설계한다.
