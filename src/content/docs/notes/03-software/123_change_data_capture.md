...
- 배경/필요성: 주기적인 배치 SQL 폴링(`WHERE updated_at > ?`) 방식이 소스 데이터베이스에 유발하는 심각한 CPU/디스크 I/O 부하와 물리적 삭제(`DELETE`) 이벤트를 감지하지 못하는 근본적 한계를 극복하고, 원천 DBMS의 트랜잭션 로그(Binlog, WAL, Redo Log)를 논블로킹 방식으로 직접 파싱하여 캐시, 검색엔진, 데이터 레이크로 변경 이벤트를 실시간 전파하는 로그 기반 CDC(Change Data Capture)를 도입하여 **소스 DB 부하 제로(Zero-Overhead)와 서브세컨드 실시간 데이터 동기화를 달성**할 필요
...
- 마이크로서비스 아키텍처(MSA) 데이터 동기화, 이종 DB 마이그레이션(Zero-Downtime) 및 실시간 분석 인프라의 **표준 변경 데이터 추출(ETL) 기술**로 확립되었으며, 실무 운영 시에는 **대량 트래픽에 의한 로그 유실을 방지하는 Binlog 보존 주기(Retention) 확보, 스키마 변경 에러를 방어하는 Schema Registry/Avro 진화 체계, 초기 대용량 적재를 위한 무잠금 스냅샷(Lockless Snapshot) 및 타깃 저장소의 멱등성(Idempotency) 보장**을 결합하여 무중단 고신뢰 데이터 스트리밍을 실현
