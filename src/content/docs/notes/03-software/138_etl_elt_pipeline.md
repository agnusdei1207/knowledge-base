...
- 정의/개념: 변환과 적재 순서를 달리하는 **ETL•ELT** 처리 흐름
...
```text
[원천 커넥터] ----- [스테이징 영역] ----- [변환 엔진] ----- [목표 저장소•적재기]
                                             |
                                  [실행 제어•메타데이터]
```
...
- **3. 외부 변환 후 멱등 적재**: ETL 경로에서 외부 정제 결과를 중복 없이 반영하는 단계이다.
- **3. 원천 적재 후 내부 변환**: ELT 경로에서 원천을 보존하고 내부 연산으로 모델을 만드는 단계이다.
