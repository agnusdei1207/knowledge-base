...
- 정의/개념: 변환과 적재 순서를 달리하는 **ETL•ELT** 처리 흐름이다.
- 배경/필요성: 단일 적재 순서로는 원천 보존과 **선처리 통제 동시 충족하기 어렵다**.
...
- **1. 증분 처리 구간 고정**: 재처리할 입력 위치•구조 확정이다.
- **2. 변환 위치 선택**: 선처리 통제와 원본 보존 우선순위를 판정한다.
- **3. ETL 외부 변환•적재**: 정제•차폐 결과를 멱등을 반영한다.
- **4. ELT 원천 적재•내부 변환**: 원천 보존 뒤 내부 모델을 생성한다.
- **5. 대사•완료 위치 확정**: 검증 성공 뒤 다음 시작점을 갱신한다.
...
- **선택 기준**: 적재 전 차폐와 원본 반복 가공의 우선순위이다.

</details>

- 적재 전 차폐는 **ETL**, 원본 반복 가공은 **ELT**을 선택한다.
