...
- 정의/개념: 추출·변환·적재의 **순서와 연산 위치**를 정하는 처리 흐름
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 증분 경계가 겹치거나 비는 경우 | **로그 위치·워터마크**를 대사 | 누락·중복 방지 |
| 같은 구간을 재실행하는 경우 | 업무 키 기반 **멱등 적재** 적용 | 재시도 중복 억제 |
| 원천 스키마가 변경되는 경우 | **호환 규칙·버전 파서** 적용 | 중단·오독 방지 |
| 민감한 원문을 적재하는 경우 | **마스킹·접근 통제** 적용 | 원문 노출 감소 |
| 데이터 이동량이 커지는 경우 | **Pushdown·증분 처리** 적용 | 이동·연산비 통제 |
