...
- 배경/필요성: 변환을 적재 앞에 두면 원본이 남지 않아 로직이 바뀔 때마다 원천에서 다시 받아야 하고 별도 변환 서버의 처리량이 전체 상한이 되는 반면 클라우드 DW는 저장이 싸고 연산이 탄력적이므로, 원본을 먼저 적재하고 변환을 타깃 안에서 수행해 재처리 여지와 확장성을 함께 확보할 필요
