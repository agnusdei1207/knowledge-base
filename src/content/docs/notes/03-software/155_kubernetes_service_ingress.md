...
- 정의/개념: 서비스가 고정 접점을 제공하고 인그레스가 **외부 웹 경로**를 분기하는 노출 구조이다.
- 배경/필요성: 파드 주소 변화로 클라이언트 직접 접속과 **외부 경로 유지할 수 없다**.
...
- **고정 이름•가상 IP** 기반 안정 접점이 핵심이다.
- **준비 엔드포인트** 기반 트래픽을 전달한다.
- 호스트•경로와 **TLS 기반 L7 라우팅**이 핵심이다.
...
- **1. TLS 종료•요청 복호화**: 인증서를 적용해 웹 요청 해석한다.
- **2. 호스트•경로 규칙 대조**: 요청에 맞는 서비스 탐색이다.
- **3. 서비스 가상 접점 선택**: 내부 서비스•포트로 전달한다.
- **4. 준비 엔드포인트 선택**: 준비 상태인 파드 주소를 선택한다.
- **5. 파드 요청 전달**: 서비스 데이터면에서 파드로 연결한다.
...
|:---|:---|:---|
| 적용 기준 | **내부 발견•안정 L4 접점** | **외부 웹•L7 분기** |
