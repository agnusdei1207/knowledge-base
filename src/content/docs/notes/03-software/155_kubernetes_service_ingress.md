...
- 배경/필요성: 파드의 잦은 재시작과 동적 IP 변경으로 인한 **클라이언트 서비스 디스커버리 불가 및 L7 경로 기반 외부 트래픽 분기 라우팅 해결 불가**
...
#### 한줄 요약
- 요청 수신 → TLS 복호화 → Path 매칭 → Pod IP 선정 → 파드 전달 순으로 진행된다.
...
#### 한줄 요약
- Target-Type IP 최적화, Ingress 단일 통합, cert-manager 자동화, IPVS 전환으로 운영한다.
