...
- 배경/필요성: 파드가 재시작될 때마다 IP가 바뀌어 호출자가 매번 대상 주소를 다시 찾는 비용을 치르므로, 파드 앞에 변하지 않는 **가상 IP(VIP)** 계층을 두어 탐색을 생략하고 그 위에 L7 경로 분기 계층을 얹어 서비스마다 외부 로드밸런서를 따로 두는 비용까지 함께 회피할 필요
