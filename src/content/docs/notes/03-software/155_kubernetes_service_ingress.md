...
- 배경/필요성: 쿠버네티스 환경에서 파드가 수시로 생성·재시작·삭제됨에 따라 동적으로 변경되는 파드 IP로 인한 호출 실패 및 서비스 디스커버리 붕괴 문제와, 개별 마이크로서비스마다 외부 로드밸런서를 생성할 때 발생하는 막대한 인프라 비용 및 복잡성을 해결하기 위해, 파드 집합에 영속적인 가상 IP(VIP)와 CoreDNS 이름을 부여하여 L4 로드밸런싱을 제공하는 Service와 단일 진입점에서 호스트/URL 경로 기반 L7 라우팅 및 TLS 암호화를 해제하는 Ingress를 도입하여 **안정적인 내부 서비스 디스커버리와 비용 효율적인 대외 트래픽 제어를 달성**할 필요
