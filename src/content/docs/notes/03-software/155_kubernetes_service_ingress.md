...
date: "2026-08-05T21:55:00+09:00"
tags:
  - "notes-software"
weight: 155
extra:
...
- 정의/개념: 서비스가 고정 접점을 제공하고 인그레스가 **외부 웹 경로** 를 분기하는 노출 구조
...
- **1. TLS 종료•요청 복호화**: 인증서를 적용해 웹 요청 해석
- **2. 호스트•경로 규칙 대조**: 요청에 맞는 서비스 탐색
- **3. 서비스 가상 접점 선택**: 내부 서비스•포트로 전달
- **4. 준비 엔드포인트 선택**: 준비 상태인 파드 주소 선택
- **5. 파드 요청 전달**: 서비스 데이터면에서 파드로 연결
