...
- **정의/개념**: 둘 이상의 퍼블릭 공급자를 **분할·중복 사용**하는 운영 전략이다.
- **배경/필요성**: 특화 기능을 활용하고 **장애·종속 위험**을 분산한다.
...
- **의도적 배치**: 기능·지역·규제·복구로 공급자 역할을 정한다.
- **분할·중복**: 업무를 나누거나 같은 서비스를 복제한다.
- **공통 운영면**: 신원·정책·배포·관측·비용을 통합한다.
...
- **1. 서비스 요청**: 공급자와 무관한 주소를 호출한다.
- **2. 주 공급자로 전달**: 건강 상태와 배치 정책을 적용한다.
- **3. 변경 상태·위치 기록**: 처리 결과와 복제 기준점을 저장한다.
- **4. 허용 지연 내 복제**: RPO와 정합성 방식으로 상태를 전달한다.
- **5. 건강 검사 이상 통지**: 주 공급자의 실패를 감지한다.
- **6. 예비 공급자로 전환**: 임계값과 재시도 안전성을 확인한다.
- **7. 전환 서비스 응답**: 예비 환경의 처리 결과를 제공한다.
...
- **특화 기능·복원력·이그레스 비용·전환 능력**으로 멀티 클라우드를 도입한다.
