...
- **의도적 배치**로 기능·지역·규제·복구별 공급자 역할 결정
...
sequenceDiagram
    ...
    U->>R: 서비스 요청
    R->>A: 1. 정책 기반 주 공급자 라우팅
    A->>S: 2. 변경 상태·복제 기준 기록
    S->>B: 3. RPO 내 상태 복제
    A-->>R: 4. 공급자 장애 판정
    R->>B: 5. 예비 공급자 전환
    R-->>U: 전환 서비스 응답
...
- **1. 정책 기반 주 공급자 라우팅**: 건강 상태·데이터 주권·비용 규칙으로 실행 위치 결정
- **2. 변경 상태·복제 기준 기록**: 처리 결과에 일관된 복제 순서와 재시작 지점 부여
- **3. RPO 내 상태 복제**: 허용 손실 시간과 정합성 방식에 맞춰 예비 공급자로 전달
- **4. 공급자 장애 판정**: 다중 관측과 임계 지속시간으로 일시 오류와 실제 실패 구분
- **5. 예비 공급자 전환**: 데이터 최신성과 중복 요청 안전성을 확인한 뒤 라우팅 변경
