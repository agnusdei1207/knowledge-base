...
- 배경/필요성: 단일 퍼블릭 클라우드 서비스 제공자(CSP)에 전사 인프라를 전적으로 의존할 때 발생하는 특정 CSP 글로벌 장애로 인한 비즈니스 전면 마비 위험, 독점 기술 및 가격 인상에 대한 협상력 상실(Vendor Lock-in) 및 각 CSP별 특화 서비스(AI, 빅데이터, 글로벌 인프라) 활용의 제약을 극복하고, 2개 이상의 CSP를 목적에 맞게 분산 배치하여 **단일 장애점(SPOF)을 제거하고 비즈니스 연속성(DR), 가격 협상력 및 서비스 최적 조합(Best-of-Breed)을 동시에 달성**할 필요
...
주(Primary) CSP 리전에서 전면 서비스 장애 발생
        │
   [장애 감지] 글로벌 모니터링 시스템이 주 CSP(AWS)의 API 타임아웃 및 헬스체크 실패 감지
        │
   [RPO 확인] 보조 CSP(GCP)로 실시간 복제 중이던 데이터의 복구 시점(RPO) 지연 확인
        │
   [보조 워크로드 승격] 보조 CSP의 대기 Pod를 즉시 스케일아웃하고 DB 쓰기 권한 승격
        │
   [GSLB 라우팅 전환] DNS 기반 GSLB가 사용자 트래픽을 보조 CSP 엔드포인트로 즉시 우회
        │
   트랜잭션 오류율 및 지연시간 메트릭을 점검하여 무중단 서비스 정상 재개 검증
```
...
- 글로벌 엔터프라이즈의 서비스 가용성 극대화 및 디지털 주권 확보를 위한 **차세대 인프라 전략의 핵심 모델**로 확립되었으며, 실무 전환 시에는 **CSP 간 데이터 전송 비용을 통제하는 Egress 최적화 및 CDC 증분 복제, 클라우드 비종속성을 보장하는 Kubernetes(CaaS)와 Terraform(IaC) 표준화 계층 구축, OIDC 기반 HashiCorp Vault 중앙 신원 관리 및 DNS 기반 GSLB 자동 장애 절체(Failover) 체계**를 결합하여 운영 복잡성을 제어하면서 고가용성과 최적 기능을 완성
