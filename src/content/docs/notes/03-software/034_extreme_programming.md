...
- 배경/필요성: 요구사항 변경이 잦은 환경에서 전통적 사후 테스트 및 고립된 개인 개발 방식이 유발하는 아키텍처 부패, 회귀 결함 누적 및 개발 후반부 결함 수정 비용 폭증을 극복하고, TDD(Test-Driven Development)·짝 프로그래밍(Pair Programming)·지속적 통합(CI)·지속적 리팩토링 등 12대 공학 실천법을 극단까지 적용하여 **피드백 루프를 분 단위로 단축하고 소프트웨어 내부 품질과 유지보수성을 극대화**할 필요
...
#### 한줄 요약
- XP의 실천법은 개별 기법이 아니라 서로를 전제하는 결합이므로, 지속적 통합 없는 TDD처럼 일부만 떼어 쓰면 피드백 단축 효과가 사라진다.
...
- 애자일 소프트웨어 공학의 **핵심 엔지니어링 실천(Technical Practices) 표준**으로 확립되었으며, 실무에서는 **스크럼의 관리 체계 위에 XP의 12대 실천법(TDD 안전망 구축, CI/CD 자동화, 핵심 도메인 짝 프로그래밍, 상시 리팩토링)을 결합하여 운영하고, 개발팀 번아웃을 방지하는 지속 가능한 개발 속도(Sustainable Pace)**를 유지함으로써 고품질 소프트웨어를 안정적으로 배포
