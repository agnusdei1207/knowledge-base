...
- 정의/개념: **XP**는 짧은 반복과 공학 실천을 결합한 애자일 개발 방법론이다.
- 배경/필요성: 큰 변경과 늦은 통합은 **회귀 결함**과 재작업을 증가시킨다.

#### 한줄 요약

- **XP**는 작은 변경과 공학 실천으로 빠른 피드백을 만든다.
...
- **사용자 스토리**와 짧은 릴리스로 고객 피드백을 조기에 반영한다.
- **TDD**와 **지속 통합**으로 변경 직후 회귀 결함을 검출한다.
- **페어 프로그래밍**과 **공동 소유**로 코드 지식을 분산한다.

#### 한줄 요약

- **사용자 스토리**, **TDD**, **페어 프로그래밍**, **지속 통합**의 결합이 핵심이다.
...
| 리팩터링 | **리팩터링**으로 외부 동작을 유지하며 구조 개선 |
| 지속 통합 | 작은 변경의 회귀 검증•통합 |

#### 한줄 요약

- **드라이버**, **내비게이터**, **리팩터링**이 개발과 검토를 연결한다.
...
- **실패 테스트 작성**: 사용자 스토리와 수용 기준을 실행 가능한 시험으로 표현하는 단계이다.
- **예상 실패 확인**: 새 시험이 의도한 이유로 실패해 검증 대상을 정확히 잡았는지 확인하는 단계이다.
- **최소 구현•테스트 통과**: 시험을 만족하는 데 필요한 코드만 작성하는 단계이다.
- **리팩터링**: 모든 시험을 통과한 상태에서 외부 동작을 유지하며 구조를 개선하는 단계이다.
- **자동 회귀 검증•통합**: 전체 시험이 통과한 변경만 메인라인에 합치는 단계이다.
...
1. **실패 테스트 작성**으로 요구 동작을 시험으로 표현한다.
2. **예상 실패 확인**으로 의도한 이유의 실패인지 검증한다.
3. **최소 구현•테스트 통과**로 필요한 코드만 작성한다.
4. **리팩터링**으로 외부 동작을 유지하며 구조를 개선한다.
5. **자동 회귀 검증•통합**으로 전체 시험을 통과한 변경만 병합한다.

#### 한줄 요약

- **실패 테스트 작성**부터 **자동 회귀 검증•통합**까지의 짧은 순환이 핵심이다.
...
| 핵심 특징 | 작은 스토리•TDD•지속 통합 | **단계 중심 방식**의 단계 산출물•독립 검증 |
...
- **레거시 코드(Legacy Code)**: 오래 운영되어 변경 위험이 크거나 자동 시험이 부족한 기존 코드이다.
...
| 느린 **불안정 테스트**로 지속 통합 신뢰 하락 | **필수 검증 분리**와 불안정 테스트 격리 | 짧고 신뢰할 피드백 확보 |
| 테스트 없는 **레거시 코드**의 대규모 변경 | **특성 테스트** 후 작은 리팩터링 반복 | 회귀 위험 제한 |

#### 한줄 요약

- **테스트 피라미드**, **필수 검증 분리**, **특성 테스트**로 작은 변경을 보호한다.
...
- **자동화 가능성**: 작은 변경을 시험•빌드•통합 과정에서 반복 검증할 수 있는 정도이다.
- **단계 승인**: 정해진 산출물과 기준을 통과해야 다음 개발 단계로 이동하게 하는 통제이다.
- **XP 적용 기준**: 변경 빈도•자동화 가능성•단계 승인 요구를 비교해 XP와 단계 중심 방식 중 적합한 방식을 고르는 기준이다.

</details>

- 변경이 잦고 **자동화 가능성**이 높으면 **XP**, **단계 승인**이 중요하면 **단계 중심 방식**을 적용하는 것이 **XP 적용 기준**이다.
