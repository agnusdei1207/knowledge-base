...
사용자 스토리(User Story) 분석 및 인수 조건 도출
        │
   1. [Red 단계] 실패하는 단위 테스트 코드 작성 (구현 전 테스트 실패 확인)
        │
   2. [Green 단계] 테스트를 통과하기 위한 최소한의 비즈니스 코드 작성 (테스트 성공)
        │
   3. [Refactor 단계] 짝 프로그래머와 함께 중복 제거, 클린 코드 리팩토링 (테스트 통과 유지)
        │
   4. 로컬 커밋 후 CI 서버로 Push (자동 빌드 및 전체 회귀 테스트 통과)
        │
...
- 현대 소프트웨어 공학은 **Scrum(관리 프레임워크)과 XP(TDD, 짝 프로그래밍, CI/CD 공학 실천법)** 를 융합하여 관리와 품질을 완벽히 양립하는 엔지니어링 표준 확립
