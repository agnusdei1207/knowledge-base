...
- **정의/개념**: 침해를 견디며 핵심 업무를 복구·적응하는 능력
- **배경/필요성**: 예방 통제 우회 후에도 핵심 임무 유지 필요
...
| 판단 기준 | 사이버 레질리언스 | 예방 중심 | 재해 복구 |
|:---|:---|:---|:---|
| 핵심 특징 | 침해 중 임무를 유지함 | 공격 경로를 사전 차단함 | 장애 후 인프라를 복구함 |
| 적용 기준 | 공격·오염을 전제로 설계 | 취약점·접근 통제 강화 | 재해·물리 장애 대비 |
| 주요 위험 | 복잡한 우회·신뢰 판정 | 우회 침해·탐지 누락 | 오염 백업·복구 지연 |
...
2. 복구센터는 불변 백업의 무결성을 검증함

### 쉽게 이해하기 (학습용)
- 결제망은 한 침해가 두 경로를 함께 막지 않게 함
- 복구센터는 무결성을 통과한 백업으로만 서비스를 복원한다.
