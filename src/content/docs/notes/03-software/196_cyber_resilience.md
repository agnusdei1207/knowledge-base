...
| 절차 | 설명 |
|:---|:---|
| 침해·영향 감지 | 이상 징후와 핵심 업무의 저하 상태를 함께 판정 |
| 감염 자원 격리 | 침해 구간의 연결과 권한을 차단해 확산 방지 |
| 임무 지속 | 예비 자원·우회 경로로 핵심 업무의 허용 수준 유지 |
| 신뢰 복구 | 무결성이 검증된 이미지·데이터로 정상 상태 복원 |
| 결과 환류 | 원인과 복구 증거를 방어 정책·대응 절차에 반영 |
...
- 사이버 침해 중에도 핵심 업무를 지속하고 복구하기 위해 **중요 기능·공통 실패 지점·격리 경로·RTO/RPO·복구 훈련 결과**를 검토하고, 목표를 입증한 우회·복구 체계를 운영한다
