...
- **제어 그룹(Control Group, cgroup)**: ‘시그룹’으로 읽고 리눅스 공식 기능 이름을 소문자로 줄여 쓰는 관례 표기이며 프로세스 집합의 자원 사용량을 제한·계측함
- **게스트·호스트 운영체제(Guest·Host Operating System, Guest·Host OS)**: ‘게스트·호스트 오에스’로 읽고 Operating System의 머리글자 OS 앞에 실행 위치를 붙인 표기이며 게스트는 가상머신 내부에서 실행되고 호스트는 물리 장비와 컨테이너의 공유 커널을 관리함
- **가상머신(Virtual Machine, VM)**: ‘브이엠’으로 읽고 Virtual Machine의 머리글자를 딴 표기이며 독립 게스트 운영체제와 가상 하드웨어를 묶어 하이퍼바이저가 실행하는 격리 단위
...
- **정의/개념**: 게스트 수정·커널 공유 방식이 다른 격리 실행 환경
- **배경/필요성**: 커널 분리가 성능을 바꿔 격리 방식 선택 필요
...
- 전가상화는 미수정 게스트 호환·가상 하드웨어 중재한다.
- 반가상화는 하이퍼콜로 중재 비용 감소·게스트 수정 필요하다.
- 컨테이너는 커널 공유로 시작 시간·자원을 줄인다.
...
| 물리 처리 결과 | 하드웨어 처리 결과를 중재 계층에 반환 |
| 실행 환경 결과 | 가상머신·컨테이너 실행 환경에 결과 반영 |
| 요청 완료 | 애플리케이션에 자원 요청 결과 전달 |
...
| 적용 기준 | 게스트 수정 불가·강한 커널 격리 | 게스트 수정 가능·중재 성능 우선 | 호스트 커널 호환·빠른 배포 우선 |
| 주요 위험 | 중재 비용·낮은 밀도 | 게스트 수정·호환 제약 | 커널 공유 공격면 |
...
1. 상용 게스트 OS는 수정 없이 전가상화로 실행
2. 입출력 중심 가상머신은 반가상 장치 드라이버로 모사 비용 감소
3. 마이크로서비스는 컨테이너로 빠른 확장·높은 배치 밀도 확보

### 쉽게 이해하기 (학습용)

- 수정할 수 없는 운영체제는 가상 하드웨어를 제공해 그대로 실행한다.
- 입출력이 많은 가상머신은 협력 드라이버로 가상 장치 모사 비용을 줄인다.
- 작은 서비스는 커널을 공유해 빨리 늘리고 한 장비에 더 많이 배치한다.

## Ⅶ. 결론

- 게스트 수정·커널 격리·시작 밀도로 실행 환경 선택
