...
- **기동시간(Startup Time)**: VM이나 컨테이너의 생성 요청부터 응용 프로그램이 실행 가능해질 때까지 걸리는 시간
...
- ① 애플리케이션이 실행 환경에 자원 접근 요청
- ② 전가상화는 특권 접근 트랩, 반가상화는 하이퍼콜, 컨테이너는 시스템 호출로 중재 계층 진입
- ③ 하이퍼바이저나 호스트 커널이 권한·자원 한도를 검증하고 물리 하드웨어 제어
- ④ 물리 하드웨어의 처리 결과를 중재 계층에 반환
- ⑤ 선택한 격리 경로를 거쳐 애플리케이션에 결과 반환
...
| 자원 한도 미설정으로 시끄러운 이웃 발생 | cgroup 제한·예약과 워크로드별 사용량 계측 | 성능 간섭 통제 |
...
- 수정할 수 없는 상용 게스트 운영체제는 전가상화로 실행하고, 독립 커널이 필요 없는 마이크로서비스는 컨테이너로 높은 배치 밀도를 확보한다.
...
- 워크로드 격리와 자원 효율을 함께 확보하려면 **게스트 수정 가능성·커널 격리·기동시간·배치 밀도**를 검토하고 전가상화·반가상화·컨테이너를 선택한다.
