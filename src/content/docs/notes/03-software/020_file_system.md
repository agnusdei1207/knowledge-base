...
sequenceDiagram
    ...
    participant C as 할당·일관성 관리자
    participant D as 블록 장치
    A->>V: ① 경로·데이터·내구성 조건 전달
    V->>M: ② 경로·현재 메타데이터 해석
    M->>C: ③ 빈 블록 할당·커밋 계획 요청
    C->>D: ④ 데이터 블록 쓰기
    C->>D: ⑤ 저널 또는 COW 메타데이터 기록
    D-->>C: ⑥ 저장 완료 통지
    C->>M: ⑦ 파일 크기·블록 매핑 커밋
    V-->>A: ⑧ 보장 범위에 따른 쓰기 완료 반환
```
...
- ① 애플리케이션이 파일 경로·데이터·내구성 조건 전달
- ② VFS가 디렉터리를 따라 현재 메타데이터 조회
- ③ 빈 블록 할당과 저널·COW 커밋 순서 결정
- ④ 할당한 블록에 파일 데이터 기록
- ⑤ 변경 로그 또는 새 메타데이터 블록 기록
- ⑥ 블록 장치가 요청된 저장 범위의 완료 통지
- ⑦ 파일 크기·블록 매핑·디렉터리 상태 커밋
- ⑧ 약속한 일관성·내구성 범위에 따라 쓰기 완료 반환
...
- 여러 운영체제와 장치에서 읽고 쓸 이동식 매체는 필요한 파일 크기·권한·복구 기능을 확인한 뒤 FAT 계열로 포맷한다.
...
- 운영체제 호환성·권한·장애 복구·스냅샷 요구를 기준으로 파일 시스템을 선택한다.
