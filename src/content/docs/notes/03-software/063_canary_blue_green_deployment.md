...
- **1. 구·신 버전 호환 검증**: 데이터와 API를 확인한다.
- **2. 제한 노출·전체 전환**: 후보 비율을 지정한다.
- **3. 버전별 트래픽 전달**: 요청 대상을 구분한다.
- **4. 오류·지연·업무 지표**: 환경별 결과를 수집한다.
- **5. 관찰 기준 판정**: 지표를 임계값과 대조한다.
- **6. 노출 확대·후보 확정**: 통과 시 범위를 넓힌다.
- **7. 안정 버전 복귀**: 위반 시 신규 요청을 되돌린다.
...
- **위험 검증 방식·병렬 비용**에 따라 배포 전략을 정한다.
