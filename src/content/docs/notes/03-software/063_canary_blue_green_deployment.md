...
- **카나리·블루-그린 배포**: 새 버전을 일부 트래픽부터 확대하거나 구·신 병렬 환경의 전체 라우팅을 전환하는 두 배포 전략이다.
- **전체 사용자 영향·복구 지연**: 일괄 교체 실패가 모든 요청에 확산되고 정상화가 늦어지는 위험이다.
...
- **점진 노출·영향 범위 제한**: 후보 버전의 트래픽을 조금씩 늘려 결함의 영향 반경을 줄이는 원칙이다.
- **병렬 환경·즉시 전환**: 구·신 환경을 동시에 준비하고 라우팅만 바꾸어 빠르게 전환하는 방식이다.
- **데이터·인터페이스 호환**: 두 버전이 공존하는 동안 같은 데이터와 계약을 안전하게 사용하는 성질이다.
...
- **카나리 배포(Canary Deployment)**: 새 버전을 소수 트래픽에 먼저 노출하고 지표에 따라 단계적으로 확대하는 방식이다.
- **블루-그린 배포(Blue-Green Deployment)**: 구·신 버전을 병렬 환경에 운영하고 전체 트래픽을 한 번에 전환하는 방식이다.
...
- **버전·사용자군별 지표**: 후보 버전의 이상을 분리해 식별하도록 버전과 코호트별로 집계한 운영 측정값이다.
- **코호트(Cohort)**: 지역·계정·기기처럼 같은 특성을 기준으로 묶어 후보 버전에 노출하는 사용자 집단이다.
- **최소 표본·관찰 시간·업무별 트래픽**: 희귀 오류까지 판단할 요청 수와 평가 기간, 핵심 업무 시나리오별 요청 구성을 정한 기준이다.
- **노출 제한(Exposure Limit)**: 후보 버전이나 외부 부작용에 영향을 받을 수 있는 사용자·요청 비율의 상한이다.
- **용량 계획·유휴 환경 자동 회수**: 병렬 환경에 필요한 자원을 미리 산정하고 전환 완료 뒤 쓰지 않는 환경 자원을 자동 해제하는 비용 통제이다.
...
- **카나리**: 일부 사용자부터 후보 버전을 검증해 점진적으로 위험을 줄이는 배포 전략이다.
- **블루-그린**: 두 환경 사이의 전체 트래픽을 빠르게 전환하고 복귀하는 배포 전략이다.
