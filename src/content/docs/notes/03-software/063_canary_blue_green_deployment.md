...
- **1. 구·신 버전 호환 검증**: 데이터와 API를 확인
- **2. 제한 노출·전체 전환**: 후보 비율을 지결정
- **3. 버전별 트래픽 전달**: 요청 대상을 구분
- **4. 오류·지연·업무 지표**: 환경별 결과를 수집
- **5. 관찰 기준 판정**: 지표를 임계값과 대조
- **6. 노출 확대·후보 확정**: 통과 시 신규 버전의 노출 범위 확대
- **7. 안정 버전 복귀**: 임계값 위반 시 신규 요청 경로 복귀

> 요약: 호환성을 확보한 뒤 운영 지표로 후보 버전의 확대 또는 복귀를 결결정
