...
date: "2026-07-30T23:15:41+09:00"
tags:
  - "notes-software"
weight: 63
extra:
...
- **관측 시스템(Observability System)**: 버전별 오류·지연·업무 성공률을 수집·판정하는 구성요소
- **버전 산출물(Version Artifact)**: 후보 환경에 배포하는 실행 이미지·패키지와 설정 묶음
...
- **멱등성(Idempotency)**: 같은 요청을 반복 처리해도 결과가 한 번 처리한 것과 같은 성질
- **보상 절차(Compensation Procedure)**: 이미 발생한 외부 부작용을 반대 업무로 상쇄하는 절차

> **키워드:** 카나리 배포·블루-그린 배포 (Canary Blue-Green Deployment)
