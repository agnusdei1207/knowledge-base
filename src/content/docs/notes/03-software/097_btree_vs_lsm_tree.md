...
- 정의/개념: **제자리 갱신과 순차 병합**을 대비한 저장 구조 비교한다.
- 배경/필요성: 단일 저장 구조는 **읽기•쓰기 편향**에 취약해진다.
...
- **B-Tree**: 균형 트리와 제자리 페이지를 갱신한다.
- **LSM-Tree**: 메모리 흡수 후 정렬 파일을 기록한다.
...
- **1. B-Tree 페이지 탐색•갱신**: 정렬 페이지의 키를 찾아 값을 제자리 변경한다.
- **2. WAL 순차 기록**: Memtable보다 먼저 복구 로그를 보존한다.
- **3. Memtable 갱신**: 메모리 정렬 구조에 최신 값을 반영한다.
- **4. SSTable Flush**: 한도 도달 시 정렬 파일로 영속화이다.
...
- 점•범위 읽기는 **B-Tree**, 쓰기 집중은 **LSM-Tree**을 선택한다.
