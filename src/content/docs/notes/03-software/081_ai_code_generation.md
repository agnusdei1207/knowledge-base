...
- 배경/필요성: 이미 공개 저장소에 수없이 존재하는 정형 코드를 개발자가 요청마다 검색·재작성하는 탐색 비용이 반복되므로, 대규모 코드 말뭉치를 학습한 **LLM**을 IDE와 개발자 사이에 초안 생성 계층으로 끼워 넣어 탐색을 생성으로 대체할 필요
...
- 코드 생성은 **AI 도구**, 품질 검증은 **SAST 게이트** 선택
