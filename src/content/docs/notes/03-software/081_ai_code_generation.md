...
- **AI 코드 생성**: 거대 언어 모델이 요구와 코드 문맥을 바탕으로 구현•테스트 후보를 만들고 개발자가 검증해 반영하는 개발 지원 기법이다.
...
- **SAST(Static Application Security Testing, 정적 애플리케이션 보안 테스트)**: 코드를 실행하지 않고 분석해 취약한 패턴과 데이터 흐름을 찾는 보안 검사이다.
...
- **총시간•수정량•결함•검토 통과율**: 생성 속도뿐 아니라 후속 수정•검증 비용과 품질을 함께 평가하는 효과 지표이다.
