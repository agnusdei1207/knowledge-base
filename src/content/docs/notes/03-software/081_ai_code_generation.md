...
- **1. 의도·허용 문맥 지정**: 제약과 입력 범위를 정한다.
- **2. 프롬프트·코드 문맥 전달**: 허용 정보만 보낸다.
- **3. 코드 후보 생성**: 모델이 구현 초안을 만든다.
- **4. 정책 결과·코드 참조**: 일치 정보와 통제를 제공한다.
- **5. 이해·수정한 후보 제출**: 개발자 변경으로 보낸다.
- **6. 빌드·테스트·보안 결과**: 정확성과 안전성을 확인한다.
- **7. 동료 검토·반영 결정**: 권리 위험까지 검토한다.
...
- **문맥 민감도·검증 비용·권리 위험**으로 생성 코드 반영을 정한다.
