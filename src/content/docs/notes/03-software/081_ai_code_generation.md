...
- 배경/필요성: 소프트웨어 개발 과정에서 반복되는 보일러플레이트 코드 작성, 라이브러리 API 탐색 및 테스트 케이스 구현에 소요되는 비효율적인 리드타임을 단축하고, 대규모 소스코드 말뭉치로 사전 학습된 LLM(대형 언어 모델)을 개발 환경(IDE)과 통합하여 자연어 의도와 편집 맥락(FIM)을 바탕으로 코드를 실시간 인라인 자동 완성·생성함으로써 **개발자의 인지적 부하를 경감하고 소프트웨어 엔지니어링 생산성을 극대화**할 필요
