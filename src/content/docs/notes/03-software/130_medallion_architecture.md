...
- 정의/개념: 데이터를 Bronze•Silver•Gold로 승격하는 **품질 계층 구조**이다.
- 배경/필요성: 단일 데이터 영역은 원천•검증 데이터 혼재로 **신뢰도•재처리 경계** 불명확이 발생한다.
...
- **1. Bronze 원천•이력 보존**: 원본•출처•처리 기준점 저장한다.
- **2. 품질 규칙 판정**: 스키마•완전성•참조 무결성 검사이다.
- **3. Silver 표준 상세 생성**: 코드•형식•단위•키 공통화이다.
- **4. 원천•표준 상세 대사**: 건수•합계•키의 누락•중복을 검증한다.
- **5. Gold 지표 게시**: 목적별 집계와 최신 시점 공개이다.
...
- 원천 재처리는 **Bronze** 담당, 공통 품질은 **Silver** 담당, 서비스 지표는 최종 **Gold** 계층까지 승격이 핵심이다.
