...
- 배경/필요성: 품질 단계별 오류 격리와 **원천부터 재처리** 지원
...
- **원천 보존**: Bronze가 재처리 기준 데이터를 저장
- **표준 상세**: Silver가 형식·코드·키를 통일
- **목적별 제공**: Gold가 업무별 집계·지표를 제공
...
- **1. 원천 값·수집 이력 저장**: Bronze에 원본과 출처를 보존
- **2. 규칙 버전으로 품질 검사**: 스키마·완전성·참조 규칙을 판정
- **3. 위반 데이터·사유 격리**: 실패 레코드와 근거를 분리
- **4. 통과 데이터 표준화**: 코드·형식·키를 Silver로 통일
- **5. 건수·합계·키 대사**: 원천과 변환 결과를 비교
- **6. 목적별 지표 생성**: 승인된 상세로 Gold를 생성
- **7. 게시 버전·최신성 기록**: 결과의 재현 근거를 기록
...
- **품질 계약·격리 사유·재처리 경로·대사 증거**로 메달리온 계층을 운영
