...
- **정의/개념**: 분산 데이터의 메타데이터·통합·정책을 잇는 **공통 기술 아키텍처**이다.
- **배경/필요성**: 저장소별로 끊긴 **검색·계보·접근 통제**를 연결한다.
...
- **활성 메타데이터**: 실행·사용·품질 신호를 자동화에 활용한다.
- **지식 그래프**: 자산·용어·소유자·계보 관계를 연결한다.
- **통합 통제**: 접근 경로에 정책·마스킹·감사를 적용한다.
...
- **1. 스키마·품질·계보 신호 수집**: 분산 소스의 운영 정보를 읽는다.
- **2. 자산 메타데이터 반환**: 현재 구조와 변경 정보를 전달한다.
- **3. 자산·관계·분류 갱신**: 카탈로그와 지식 그래프를 최신화한다.
- **4. 목적·품질 조건 검색**: 소비자가 후보 데이터를 찾는다.
- **5. 자산·계보·소유자 제공**: 의미와 품질 근거를 제시한다.
- **6. 선택 자산 접근 요청**: 통합 경로로 사용을 신청한다.
- **7. 정책 적용 결과 제공**: 허용·마스킹·감사 결과를 반환한다.
...
- **메타데이터 최신성·계보 정확도·정책 집행·통합 성능**으로 데이터 패브릭을 확장한다.
