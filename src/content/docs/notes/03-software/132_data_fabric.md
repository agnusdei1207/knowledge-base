...
date: "2026-07-25T11:34:00+09:00"
tags:
  - "notes-software"
weight: 132
extra:
...
- **정의/개념**: 메타데이터로 분산 자산과 정책을 연결한다
- **배경/필요성**: 이기종 저장소의 검색·통제 단절을 줄인다
...
| 판단 기준 | 데이터 패브릭 | 데이터 메시 |
|:---|:---|:---|
| 핵심 특징 | 기술 연결·활성 메타데이터·정책 집행을 다룸 | 도메인 소유권과 데이터 제품 책임을 다룸 |
| 적용 기준 | 분산 이기종 환경의 통합·자동 통제가 우선됨 | 소유권 분산과 자율적 제품 제공이 우선됨 |
| 주요 위험 | 메타데이터 오류와 자동화 오판 | 도메인별 표준 불일치와 제품 중복 |
...
1. 분산 검색에 스키마·소유자·계보를 등록한다
2. 민감 데이터에 분류별 마스킹 정책을 적용한다

### 쉽게 이해하기 (학습용)
- 분산 검색은 자산 등록률뿐 아니라 소유자·스키마·계보 관계의 정확도를 표본 검사해 검색 결과를 신뢰할 수 있는지 확인함
- 민감 데이터 제공은 마스킹 누락 건수와 정책 반영 지연을 측정해 자동 통제가 실제 데이터 경로에서 작동하는지 검증함

## Ⅶ. 결론

- 최신 메타데이터·정책 자동화가 필요하면 패브릭을 택한다
