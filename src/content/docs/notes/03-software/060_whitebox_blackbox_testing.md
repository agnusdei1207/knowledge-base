...
- 배경/필요성: 결과와 내부 경로의 **단독 검증 한계** 상호 보완
...
- **3. 통합 테스트 실행**: 두 근거를 같은 대상에 적용
- **4. 결과·실행 이력 전달**: 기능 결과와 경로 자료 수집
- **5. 오라클·커버리지 판정**: 오류와 미실행 구조를 함께 보강
...
> 적용 사례: 결제 금액 경계는 블랙박스로, 실패 보상 분기는 화이트박스로 검증
