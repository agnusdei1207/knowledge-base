...
- 정의/개념: **알파•베타•인수 테스트**는 내부 환경과 제한된 실제 사용자 환경에서 제품을 검증하고 업무•계약 기준 충족 여부를 사용자 증거로 판정하는 수용 시험한다.
- 배경/필요성: 개발 환경 시험만으로 실제 **업무•현장 적합성 확인할 수 없다**.

#### 한줄 요약

- 내부•현장 사용자 검증 기반 **제품 수용 판정**이 핵심이다.
...
- **알파(Alpha Testing)**: 개발 조직이 통제하는 환경에서 내부 인력이나 선정 사용자가 수행하는 초기 검증한다.
- **베타(Beta Testing)**: 실제 사용 환경에서 외부 사용자가 현장 문제와 사용성을 검증하는 시험이다.
- **인수 기준(Acceptance Criteria)**: 시스템을 수용하기 위해 충족해야 할 기능•품질•업무 조건이다.

</details>

- 알파의 **통제 환경•신속한 결함 재현**이 핵심이다.
- 베타의 **실사용 환경•사용자 다양성**이 핵심이다.
- 인수 기준 기반의 **최종 수용 판정**이 핵심이다.

#### 한줄 요약

- 통제 환경•실사용 환경•**인수 기준별 검증**이 핵심이다.
...
- **테스트 증거(Test Evidence)**: 실행 결과•결함•사용자 의견을 인수 기준과 연결한 판정 기록한다.
...
> 요약: 목적에 맞는 환경에서 얻은 사용자 증거를 합의한 인수 기준과 대조가 핵심이다.

#### 한줄 요약

- 기준•환경•사용자 증거 기반 **수용 결정 흐름**이 핵심이다.
...
- 알파는 재현, 베타는 현장, UAT는 **수용 판정**이 핵심이다.
...
- 대표 표본•진단 정보•**잔여 위험 책임 통제**가 핵심이다.
...
- **결함 재현•현장 적합성•수용 판정**에 따라 **알파•베타•UAT**을 선택한다.

#### 한줄 요약

- 재현은 알파, 현장은 베타, 수용은 **UAT**가 핵심이다.
