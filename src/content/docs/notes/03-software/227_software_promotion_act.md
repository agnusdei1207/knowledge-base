...
block
    columns 1
    A["요구사항 상세화 | 과업 규모·검수 기준 사전 공개"]
    B["적정 기간·대가 | 업무량에 맞는 기간·비용 산정"]
    C["과업심의위원회 | 과업 확정·변경·조정 심의"]
    D["계약 조정 | 승인 변경을 금액·기간에 반영"]
    E["상용SW 직접구매 | 기준 대상 제품을 분리 구매"]
    A --- B
    B --- C
    C --- D
    D --- E
...
sequenceDiagram
    ...
    participant 사업자
    participant 계약관리자
    발주기관->>과업심의위원회: 1. 초기 과업 제출
    과업심의위원회->>계약관리자: 2. 과업·계약 확정
    사업자->>과업심의위원회: 3. 변경 근거 제출
    과업심의위원회->>발주기관: 4. 변경 영향 심의
    계약관리자->>사업자: 5. 조정 과업 검수
```

**동작 원리**

1. **초기 과업 제출**: 요구·기간·대가 심의 요청
2. **과업·계약 확정**: 심의 결과 계약 반영
3. **변경 근거 제출**: 추가·삭제 요구 영향 제시
4. **변경 영향 심의**: 범위·기간·금액 판정
5. **조정 과업 검수**: 승인 범위·기준으로 확인
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 불명확한 초기 요구 | 기능·품질·검수 기준 상세화 | 과업 규모 명확화 |
| 무심의 과업 변경 | 변경 전 위원회 심의 의무화 | 무상 업무 방지 |
| 기간·대가 조정 누락 | 범위 영향과 계약 동시 조정 | 계약 균형 확보 |
| 직접구매 예외 오판 | 대상 기준·예외 근거 기록 | 구매 절차 적법성 |
...
- **상세 요구·과업심의·계약 조정을 통한 공공 SW 사업 공정성**
