...
- 정의/개념: 원본 콘텐츠 사본을 사용자와 가까운 에지 거점에 캐시하고 요청을 적절한 거점으로 라우팅하는 **콘텐츠 전송 네트워크(Content Delivery Network, CDN)** 분산 전달 계층
...
- **도메인 이름 시스템(Domain Name System, DNS)•애니캐스트(Anycast)**: 이름 해석이나 동일 주소의 네트워크 경로 선택으로 사용자를 가까운 정상 에지에 연결하는 방식이다.
- **유효 시간(Time To Live, TTL)•엔터티 태그(Entity Tag, ETag)•퍼지(Purge)**: 사본의 만료, 조건부 재검증, 긴급 무효화를 제어하는 수단이다.
...
- **원본•퍼지(Origin•Purge)**: 정본 콘텐츠를 제공하고 변경 시 캐시 무효화나 판본 전환을 수행하는 구성요소이다.
...
- **2. 캐시 미스 조회**: 캐시 키•유효 시간(Time To Live, TTL)•엔터티 태그(Entity Tag, ETag)로 유효 사본을 판정하고 없으면 실드에 요청하는 단계이다.
...
- **해시 통합 자원 위치 지정자(Hash Uniform Resource Locator, Hash URL)**: 파일 내용 지문을 주소에 포함해 변경 시 새 주소로 전환하는 캐시 버전 방식이다.
- **유효 시간 지터(Time To Live Jitter, TTL Jitter)**: 만료 시각에 무작위 편차를 더해 인기 콘텐츠의 동시 미스를 분산하는 방식이다.
