...
- **위험 기반 감리**로 영향도에 따라 점검 범위•깊이를 차등화한다.
- **계보 추적**으로 데이터•모델 버전과 결정 이력을 연결한다.
- **독립 판정**으로 개발 조직과 분리해 증거를 판단한다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 학습 자료의 출처•동의 이력 불명 | **계보 추적** | 데이터 책임 추적 |
| 전체 정확도의 **집단별 오류 은폐** | 집단별 오류•**강건성** 시험 | 차별 위험 식별 |
| 사람 개입 절차의 실제 미작동 | **사람 개입** 재심•중단 시나리오 훈련 | 피해 확산 방지 |
...
- **감리 깊이 선택 기준**에 따라 **고영향 인공지능**은 전주기 심층 감리, 저위험은 증적 표본 점검을 적용한다.
