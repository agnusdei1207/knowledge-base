...
- **인공지능(Artificial Intelligence, AI) 소프트웨어 감리**: ‘에이아이 소프트웨어 감리’로 읽고 AI는 영문 머리글자를 딴 약어이며, 데이터·모델·서비스의 위험과 통제를 독립적으로 점검함
...
- **기존 한계**: 정확도 중심 검증의 **계보·공정성·운영 위험 누락**
...
```mermaid
flowchart LR
    G[AI 거버넌스] -->|목적·위험·책임| R[위험 등록부]
    D[(데이터·모델 계보)] -->|출처·버전·변환| E[감리 증적 저장소]
    T[성능·공정성·강건성 시험] -->|평가 결과| E
    O[운영 감시·사람 감독] -->|드리프트·개입 기록| E
    R --> A[독립 감리]
    E --> A
    A -->|시정·잔여 위험| G
```
...
| 위험 등록부 | 위험·통제·**소유자·잔여 위험** 관리 |
| 데이터·모델 계보 | 출처·변환·**버전·배포 추적** |
| 평가 체계 | 성능·공정성·**강건성 검증** |
| 운영 통제 | 드리프트 감시·**사람 개입·중단** |
| 감리 증적 저장소 | 설정·시험·**승인·운영 이력** 보존 |
| 독립 감리 | 증적 검토와 **시정·재판정** |

> 요약: 거버넌스·계보·평가·운영 통제를 증적으로 연결
...
```mermaid
sequenceDiagram
    participant O as 사업책임자
    participant A as 감리인
    participant E as 증적저장소
    participant S as AI 서비스
    O->>A: 목적·영향·위험 등록부 제출
    O->>E: 계보·설정·승인 증적 등록
    A->>E: 데이터 적법성·계보 점검
    A->>S: 성능·공정성·강건성 시험
...
| 계보·증적 확보 | 데이터·모델·**승인 이력 연결** |
| 데이터 점검 | 출처·동의·**대표성·품질 확인** |
| 모델 검증 | 성능·공정성·**강건성·한계 확인** |
| 운영 통제 시험 | 감시·개입·**중단 동작 확인** |
| 시정·재판정 | 결함 보완과 **잔여 위험 수용 결정** |

> 요약: 위험별 증적을 점검하고 시정 후 잔여 위험 재판정
...
1. 대출 모델은 **집단별 오류율·사람 재심 절차** 점검
2. 제조 모델은 **카메라 교체 후 드리프트 경보** 검증

### 쉽게 이해하기 (학습용)

- 대출 모델이 특정 집단을 더 자주 잘못 거절하는지와 사람이 결과를 뒤집을 수 있는 절차를 함께 확인한다.
- 공장 카메라 교체 뒤 입력 분포와 불량 판정 성능이 달라지면 경보와 재검증이 작동하는지 확인한다.

## Ⅶ. 결론

- 개발 전은 **거버넌스·계보**, 배포 전은 **위험 검증**, 운영은 **감시·개입**

### 쉽게 이해하기 (학습용)

- AI 감리는 정확도 한 번으로 끝내지 않고 책임과 제작 근거부터 운영 변화와 사람의 통제까지 계속 추적해야 한다.
