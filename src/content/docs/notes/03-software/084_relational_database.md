...
- 배경/필요성: 과거 파일 처리 시스템(File System)이 유발하던 극심한 데이터 중복, 갱신 이상(Anomaly), 응용 프로그램의 물리 데이터 구조 종속성 및 정합성 붕괴 문제를 극복하고, E.F. Codd의 관계 대수 및 집합론에 기초하여 데이터를 속성(Attribute)과 튜플(Tuple)의 2차원 릴레이션으로 구조화하며 기본키·외래키 등 3대 무결성 제약조건을 DBMS 엔진 레벨에서 강제함으로써 **데이터 독립성(논리적/물리적), 무결성 및 Strict ACID 트랜잭션을 보장**할 필요
...
- 엔터프라이즈 트랜잭션 처리(OLTP) 및 핵심 비즈니스 데이터 관리의 **가장 신뢰성 높은 영속성 계층(Persistence Layer) 표준 아키텍처**로 확립되었으며, 실무 설계 시에는 **비즈니스 변경에 영향받지 않는 대리키(Surrogate Key) 중심의 PK 설계, 대규모 트래픽 환경에서의 FK 락 경합 완화, Flyway/Liquibase 기반의 스키마 버전 관리, 필요에 따른 NoSQL/NewSQL과의 폴리글랏 지속성(Polyglot Persistence) 결합**을 통해 데이터 정합성과 시스템 확장성을 동시 달성
