...
- 배경/필요성: 트래픽 변동이 심한 클라우드 네이티브 환경에서 고정된 리소스 할당 방식으로 인해 발생하는 피크 타임 성능 병목(OOM 및 서비스 다운)과 트래픽 감소 시 유휴 자원 방치로 인한 비용 낭비, 수동 증설에 따른 긴 대응 지연을 해결하기 위해, 실시간 메트릭(CPU, Memory, Custom Event)에 따라 Pod 복제본 개수를 수평 증감하는 HPA(Horizontal Pod Autoscaler)와 단일 Pod의 자원 Request/Limit 스펙을 수직 조정하는 VPA(Vertical Pod Autoscaler)를 도입하여 **시스템의 무중단 가용성과 컴퓨팅 비용 효율성을 극대화**할 필요
...
- 쿠버네티스(Kubernetes) 기반 컨테이너 오케스트레이션 및 마이크로서비스 아키텍처의 **가장 핵심적인 자동 탄력성(Elasticity) 제어 메커니즘**으로 확립되었으며, 실무 구축 시에는 **무상태(Stateless) 웹/API 워크로드에 HPA + KEDA(이벤트 기반 사전 확장) 적용, 단일 프로세스 배치/Stateful 워크로드에는 VPA 자원 추천 모드(Off/Initial) 적용, 스케일다운 시 진동(Flapping)을 방지하는 안정화 윈도우(Stabilization Window) 튜닝 및 노드 병목을 제거하는 Karpenter 초고속 프로비저너 연동**을 결합하여 무결점 확장성과 리소스 최적화를 완성
