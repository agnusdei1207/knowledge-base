---
sidebar:
  order: 149
  label: "149. 오토 스케일링 HPA•VPA (Auto Scaling HPA VPA)"
  badge:
    text: "미출 • 70%"
    variant: note
title: "오토 스케일링 HPA•VPA (Auto Scaling HPA VPA)"
date: "2026-08-03T09:14:20+09:00"
tags:
  - "notes-software"
weight: 149
extra:
  question_no: "149"
  source_status: "미출"
  source_history: ""
  priority: 70
  priority_note: "복제본 수와 자원 크기 조정 비교 가치가 높음"
---

## Ⅰ. 개요

<details>
<summary>핵심 용어</summary>

- **수평형 포드 오토스케일러(Horizontal Pod Autoscaler, HPA)**: 부하 지표와 목표값의 비율에 따라 포드 복제본 수를 자동 조정하는 방식이다.
- **수직형 포드 오토스케일러(Vertical Pod Autoscaler, VPA)**: 포드별 사용 이력에 따라 중앙처리장치•메모리 자원 요청값을 자동 조정하는 방식이다.

</details>

- 정의/개념: 메트릭과 사용 이력에 따라 각각 워크로드의 포드 복제본 수와 포드당 자원 요청값을 조정하는 **수평형 포드 오토스케일러(Horizontal Pod Autoscaler, HPA)•수직형 포드 오토스케일러(Vertical Pod Autoscaler, VPA)** 방식
- 배경/필요성: 변동 부하와 **자원 요청 오차** 의 수동 대응 한계

#### 한줄 요약
- 주문이 몰리면 작업자 수를 늘리는 HPA와 작업자 한 명의 장비 크기를 바꾸는 VPA처럼, 병렬 처리량과 Pod당 자원 부족을 서로 다른 축으로 조정한다.

## Ⅱ. 특징

<details>
<summary>핵심 용어</summary>

- **안정화 제어**: 허용 오차와 대기 구간 및 증감 속도로 반복 확장•축소를 억제하는 특성이다.
- **수평형 포드 오토스케일러(Horizontal Pod Autoscaler, HPA) 수평 조정**: 목표 지표에 맞춰 워크로드의 포드 복제본 수를 늘리거나 줄이는 방식이다.
- **수직형 포드 오토스케일러(Vertical Pod Autoscaler, VPA) 수직 조정**: 사용 이력에 맞춰 포드별 중앙처리장치•메모리 요청값을 바꾸는 방식이다.

</details>

![현재 지표와 목표 지표 비율에 따른 HPA 복제본 조정](/study/diagrams/hpa-proportional-scaling.svg)

> 현재 복제본 4개라는 가정에서 지표가 목표의 1.5배이면 파란 계단선이 6개를 권고하는 비례 제어 예시이며, 실제 동작에는 허용 오차•안정화 창•최소•최대 범위가 함께 적용된다.

- 목표 지표로 복제본을 바꾸는 **HPA 수평 조정**
- 사용 이력으로 요청값을 바꾸는 **VPA 수직 조정**
- 허용 오차•대기 구간을 두는 **안정화 제어**

#### 한줄 요약
- 주문량이 경계값을 오갈 때마다 작업자를 뽑고 내보내지 않도록 일정 시간 기다리듯, 안정화 구간과 증감 속도가 반복 확장•축소를 억제한다.

## Ⅲ. 구조 및 구성요소

<details>
<summary>핵심 용어</summary>

- **HPA 객체•제어기**: 목표 지표와 현재 값의 비율로 필요한 포드 복제본 수를 계산하는 구성요소이다.
- **메트릭 응용 프로그래밍 인터페이스(Metrics Application Programming Interface, Metrics API)**: 자원•사용자 정의 확장 지표를 제어기에 제공하는 접점이다.
- **수직형 포드 오토스케일러(Vertical Pod Autoscaler, VPA) 추천기•갱신기**: 사용 이력으로 적정 자원 요청값을 산정하고 적용하는 구성요소이다.
- **워크로드 제어기**: 목표 복제본 수와 포드 교체 상태를 관리하는 구성요소이다.
- **스케줄러•노드**: 자원 요청값을 기준으로 포드를 배치하고 용량 부족 상태를 제공하는 구성요소이다.

</details>

```mermaid
block-beta
    columns 2
    M["메트릭 API"]
    H["HPA 객체•제어기"]
    V["VPA 추천기•갱신기"]
    W["워크로드 제어기"]
    S["스케줄러•노드"]
    M --- H
    M --- V
    H --- W
    V --- W
    W --- S
```

| 구성요소 | 책임 |
|:---|:---|
| 메트릭 API | 자원•사용자 정의 **확장 지표** 제공 |
| HPA 객체•제어기 | 목표 비율로 **필요 복제본** 계산 |
| VPA 추천기•갱신기 | 사용 이력으로 **자원 요청값** 산정 |
| 워크로드 제어기 | 목표 복제본•**포드 교체 상태** 관리 |
| 스케줄러•노드 | 요청값 기준 **포드 배치** 와 부족 상태 제공 |

#### 한줄 요약

- 같은 측정판을 본 HPA와 VPA가 각각 작업자 수와 장비 크기를 정하고, 워크로드 제어기와 스케줄러가 실제 작업자와 자리를 배치한다.

## Ⅳ. 흐름도

<details>
<summary>핵심 용어</summary>

- **4. 배치•대기 상태**: 노드 용량이 포드 요청값을 충족하는지에 따라 실행 가능 여부를 구분한 결과이다.
- **1. 부하 지표•사용 이력**: 수평형 포드 오토스케일러(Horizontal Pod Autoscaler, HPA)의 목표 비율과 수직형 포드 오토스케일러(Vertical Pod Autoscaler, VPA)의 자원 분포를 계산하는 입력이다.
- **2. 복제본 수•자원 요청값**: 수평 또는 수직 확장의 목표값을 확정한 결과이다.
- **3. 포드 배치 명세**: 복제본 수와 중앙처리장치•메모리 요청값을 스케줄러에 전달하는 배치 조건이다.
- **5. 적용 상태**: 실제 조정•배치 결과를 다음 제어 주기에 반영하는 피드백이다.

</details>

```mermaid
sequenceDiagram
    participant M as 메트릭 API
    participant A as HPA•VPA 제어기
    participant W as 워크로드 제어기
    participant S as 스케줄러•노드
    M->>A: 1. 부하 지표•사용 이력
    A->>W: 2. 복제본 수•자원 요청값
    W->>S: 3. 포드 배치 명세
    S-->>W: 4. 배치•대기 상태
    W-->>A: 5. 적용 상태
```

**동작 원리**

1. **부하 지표•사용 이력**: HPA 목표 비율과 VPA 자원 사용 분포 산정
2. **복제본 수•자원 요청값**: 수평 또는 수직 조정 목표 확정
3. **포드 배치 명세**: 복제본 수와 CPU•메모리 요청값을 배치 조건으로 전달
4. **배치•대기 상태**: 노드 용량 충족 여부에 따라 실행•대기 구분
5. **적용 상태**: 다음 제어 주기에 실제 조정 결과 반영

#### 한줄 요약

- 주문량이 늘면 HPA가 작업자 수를 바꾸고 한 작업자가 계속 힘들어하면 VPA가 장비 크기를 바꾸며, 실제로 자리가 없으면 포드가 대기 상태로 남는다.

## Ⅴ. 종류 및 비교

<details>
<summary>핵심 용어</summary>

- **수평형 포드 오토스케일러(Horizontal Pod Autoscaler, HPA)**: 병렬화 가능한 부하 변화에 맞춰 워크로드 복제본 수를 조정하는 오토스케일러이다.
- **수직형 포드 오토스케일러(Vertical Pod Autoscaler, VPA)**: 포드별 중앙처리장치(Central Processing Unit, CPU)•메모리 사용 이력으로 자원 요청값을 조정하는 오토스케일러이다.

</details>

| 오토스케일러 | HPA | VPA |
|:---|:---|:---|
| 적용 기준 | **병렬화 가능 부하** 변동 | 포드당 **요청값 오차** 누적 |
| 핵심 특징 | 워크로드 **복제본 수** 조정 | CPU•메모리 **요청값** 조정 |
| 한계 | 시작 지연•지표의 **스케일 진동** | 재시작•용량 부족•**HPA 충돌** |

#### 한줄 요약
- 작업을 나눌 수 있으면 같은 장비의 작업자를 늘리고, 한 작업자에게 필요한 장비 크기가 틀렸으면 CPU•메모리 요청값을 바꾼다.

## Ⅵ. 실무 고려사항 및 대책

<details>
<summary>핵심 용어</summary>

- **스케일 진동**: 지표가 임계값 주변을 오가며 확장과 축소가 반복되는 문제이다.
- **병목 상관성**: 선택한 지표가 실제 처리 병목과 부하 변화를 얼마나 직접 반영하는지 나타내는 관계이다.
- **요청값 기준선**: 포드 사용률과 확장 판단의 분모가 되는 중앙처리장치•메모리 요청값 기준이다.
- **준비 구간**: 포드 생성 요청부터 실제 트래픽을 처리할 준비가 끝날 때까지의 시간이다.
- **용량 예약**: 포드 증가 시 필요한 노드 자원을 미리 확보하는 대책이다.

</details>

| 문제 | 대책 | 효과 |
|:---|:---|:---|
| CPU와 요청량이 어긋나는 **지표 선택** | 큐 길이•요청률의 **병목 상관성** 검증 | 무관 부하의 **오확장 감소** |
| 과소 요청값이 키우는 **HPA 사용률** | VPA 권고로 **요청값 기준선** 보정 | 복제본의 **과잉 확장 감소** |
| 포드 준비보다 빠른 **부하 증가** | 여유 복제본•예측 확장•**준비 구간** | 확장 전 **꼬리 지연 완화** |
| 임계값 부근의 **스케일 진동** | 허용 오차•**안정화 구간•증감 속도** | 반복 **확장•축소 억제** |
| 노드 여유 없는 **포드 증가** | 노드 확장과 **용량 예약** 연계 | **대기 상태 누적 방지** |

#### 한줄 요약
- 주문 신호가 울린 순간이 아니라 새 작업자가 자리를 받아 일을 시작한 순간까지 재고, 작업장에 빈자리가 없으면 노드부터 늘려야 한다.

## Ⅶ. 결론

<details>
<summary>핵심 용어</summary>

- **병렬 부하**: 작업을 여러 포드로 나눌 수 있어 HPA로 복제본 수를 조정하기 적합한 부하이다.
- **요청값 오차**: 포드에 선언한 중앙처리장치•메모리 요청량이 실제 필요량과 지속적으로 어긋난 상태이다.
- **수평형 포드 오토스케일러(Horizontal Pod Autoscaler, HPA)•수직형 포드 오토스케일러(Vertical Pod Autoscaler, VPA)**: 각각 병렬 부하의 복제본 수와 포드별 자원 요청값을 조정하는 방식이다.

</details>

- **병렬 부하** 는 HPA, **요청값 오차** 는 VPA, 노드 용량 선확보

#### 한줄 요약
- 나눌 수 있는 주문은 작업자 수로, 한 작업자의 장비 부족은 장비 크기로 해결하되 작업장 자리가 먼저 확보돼야 실제 확장이 끝난다.
