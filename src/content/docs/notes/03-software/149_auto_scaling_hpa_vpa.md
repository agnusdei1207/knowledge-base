...
- 정의/개념: 메트릭 기반 **복제본 수·Pod 요청값** 제어 방식
...
| 워크로드 제어기 | 목표 복제본·**Pod 교체 상태** 관리 |
| 스케줄러·노드 | 요청값 기준 **Pod 배치**와 부족 상태 제공 |
...
1. **현재 부하·목표 지표**: 현재값과 목표값의 비율로 확장 오차 산정
2. **필요 복제본 수**: 안정화 구간·증감 정책을 반영한 목표 확정
3. **사용 이력·OOM**: 백분위 사용량과 메모리 종료 이력 집계
4. **자원 요청값**: 정책 범위 안에서 CPU·메모리 권고값 산정
5. **조정 Pod 배치 요청**: 재생성·인플레이스 방식으로 자원 반영

### 쉽게 이해하기 (학습용)

- 주문량이 늘면 HPA가 작업자 수를 바꾸고 한 작업자가 계속 힘들어하면 VPA가 장비 크기를 바꾸며, 실제로 자리가 없으면 Pod가 Pending으로 남는다.

## Ⅴ. 종류 및 비교

| 오토스케일러 | HPA | VPA |
|:---|:---|:---|
| 적용 기준 | **병렬화 가능 부하** 변동 | Pod당 **요청값 오차** 누적 |
...
| Pod 준비보다 빠른 **부하 증가** | 여유 복제본·예측 확장·**준비 구간** | 확장 전 **꼬리 지연 완화** |
| 임계값 부근의 **스케일 진동** | 허용 오차·**안정화 구간·증감 속도** | 반복 **확장·축소 억제** |
| 노드 여유 없는 **Pod 증가** | 노드 확장과 **용량 예약** 연계 | **Pending 누적 방지** |
