...
- 정의/개념: 복제본 수와 자원 요청값을 조정하는 **HPA•VPA** 방식이다.
- 배경/필요성: 변동 부하와 **자원 요청 오차**의 수동 대응 한계가 발생한다.
...
- 목표 지표로 복제본을 바꾸는 **HPA 수평 조정**이 핵심이다.
- 사용 이력으로 요청값을 바꾸는 **VPA 수직 조정**이 핵심이다.
...
- **병렬 부하**는 HPA, **요청값 오차**는 VPA, 노드 용량 선확보가 핵심이다.
