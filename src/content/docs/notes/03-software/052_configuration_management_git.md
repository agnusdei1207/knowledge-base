...
- 배경/필요성: 소프트웨어 개발 과정에서 소스 코드 변경 이력 유실, 다수 개발자의 동시 수정에 따른 덮어쓰기 충돌 및 배포 형상 불일치로 인한 릴리즈 장애를 극복하고, 분산 버전 관리(DVCS) 도구인 Git의 DAG(Directed Acyclic Graph) 객체 모델과 팀 협업 브랜치 전략(GitFlow, GitHub Flow, Trunk-based)을 통해 **형상 4대 활동(식별·통제·감사·기록)을 완벽히 이행하고 안전한 병렬 개발 및 지속적 통합(CI) 기반을 구축**할 필요
