...
- 정의/개념: 소스•설정•문서의 **기준선•Git 이력**으로 변경을 식별•통제하는 형상 관리 활동이다.
- 배경/필요성: 공통 기준선 없이 병렬 변경하면 **변경 충돌•배포 버전 불일치**가 발생한다.

#### 한줄 요약

- 기준선•변경 이력 기반 **형상 식별•통제•재현**이 핵심이다.
...
- **DVCS 분산 저장소**로 로컬 분기•전체 이력 조회가 핵심이다.
- **커밋•참조**로 스냅샷 계보를 구성한다.
- **병합•리베이스**로 변경 계보 통합이 핵심이다.

#### 한줄 요약

- 분산 이력과 **커밋•참조 기반 변경 통합**이 핵심이다.
...
> 요약: 선택 변경을 **커밋**하고 자동 검증•검토 후 **기준선** 확정이 핵심이다.

#### 한줄 요약

- 커밋•자동 검증•승인 기반 **기준선 확정 흐름**이 핵심이다.
...
> 요약: **배포 주기•지원 버전**으로 브랜치 전략을 선택한다.

#### 한줄 요약

- 잦은 배포는 트렁크, 병행 버전은 **GitFlow**가 핵심이다.
...
- 짧은 브랜치•보호 규칙 기반 **기준선 안정성**이 핵심이다.
...
- 단일 버전의 잦은 배포는 **트렁크 기반**, 병행 지원은 **GitFlow**을 선택한다.

#### 한줄 요약

- 잦은 배포는 트렁크, 병행 지원은 **GitFlow**가 핵심이다.
