...
- **정의/개념**: 모델·프롬프트·검색 품질 통제
- **배경/필요성**: 비결정 응답·환각으로 LLM 운영 통제 필요
...
| 판단 기준 | 프롬프트 설계 | 미세조정 | RAG |
|:---|:---|:---|:---|
| 핵심 특징 | 입력 지시문·예시 변경 | 모델 가중치 학습 | 외부 문서 검색·주입 |
| 적용 기준 | 행동·출력 형식 조정 | 반복 작업 행동 내재화 | 최신·사내 지식 답변 |
| 주요 위험 | 프롬프트 주입·회귀 | 과적합·학습 비용 | 검색 누락·잘못된 근거 |
...
2. 모델 게이트웨이는 민감정보를 가린 뒤 호출 이력 기록

### 쉽게 이해하기 (학습용)
- 상담 답변은 검색 문서가 실제로 뒷받침하는지 확인한 뒤 새 구성을 배포한다
- 게이트웨이는 개인정보를 가리고 어떤 모델·프롬프트가 답했는지 남긴다
