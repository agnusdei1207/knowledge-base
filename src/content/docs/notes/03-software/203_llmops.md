...
- 정의/개념: **생성형 AI** 모델•프롬프트•검색의 **품질•안전** 통제 체계이다.
- 배경/필요성: 단일 정답 평가만으로는 **비결정 응답•환각의 품질을 판정하기 어려움**이 발생한다.
...
- **모델•프롬프트•RAG 구성** 단위 버전을 관리한다.
- **반복 표본•평가 세트** 기반 품질•안전을 판정한다.
- **비용•가드레일•운영 이력** 기반 승격을 통제한다.
...
> 요약: 승인 구성만 가드레일을 거쳐 모델 호출이 핵심이다.
...
- **RAG 적용**: 외부 문서를 검색해 최신•사내 지식의 답변 근거 보완이다.
...
- **품질 분포•안전 기준** 충족과 비용 예산 내 LLM 구성만 승격이 핵심이다.
