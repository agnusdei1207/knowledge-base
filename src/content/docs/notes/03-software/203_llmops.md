...
- 정의/개념: **생성형 인공지능(Generative Artificial Intelligence, 생성형 AI)** 모델•프롬프트•검색의 품질과 안전을 통제하는 **대규모 언어 모델 운영(Large Language Model Operations, LLMOps)**이다.
- 배경/필요성: 단일 정답 평가만으로는 비결정 응답과 환각의 품질을 판정하기 어렵다.
...
- **반복 표본•평가 세트**: 비결정 응답에 대표 평가 세트를 여러 번 실행해 얻은 표본 분포로 품질•근거성•안전성을 판정하는 평가 방식이다.
...
- **구성 버전(Configuration Version)**: 모델•프롬프트•RAG 설정의 결합 상태를 하나의 식별자로 고정한 버전이다.
- **운영 이력(Operational History)**: 운영 응답•지연•토큰•비용•안전 판정을 구성 버전과 연결한 기록이다.

</details>

- **구성 버전(Configuration Version)**으로 모델•프롬프트•**검색 증강 생성(Retrieval-Augmented Generation, RAG)** 조합을 관리한다.
- **반복 표본•평가 세트** 기반 품질•안전을 판정한다.
- 비용 기준과 **가드레일(Guardrail)**•**운영 이력(Operational History)**으로 승격을 통제한다.
...
- **LLM 게이트웨이(LLM Gateway)**: 모델 호출의 인증•라우팅•사용량 제한과 관측을 중앙 접점에서 통제하는 구성요소이다.
...
| 구성요소 | 책임 |
|:---|:---|
| **구성 카탈로그(Configuration Catalog)** | 모델•프롬프트•RAG 버전 관리 |
| **평가기(Evaluator)** | 품질•안전•비용 합격선 판정 |
| **LLM 게이트웨이(LLM Gateway)** | 인증•라우팅•사용량 제한 |
| **가드레일(Guardrail)** | 민감•유해 입출력 차단 |
| **관측기(Observer)** | 응답•지연•비용•판정 수집 |

> 요약: 승인 구성만 **가드레일(Guardrail)**을 거쳐 모델을 호출한다.
...
- **3. 승인 구성 배포**: 평가 세트의 품질•안전•비용 합격선을 통과한 모델•프롬프트•RAG 조합만 게이트웨이에 반영하는 단계이다.
...
1. **평가 구성 제출**: 모델•프롬프트•RAG을 고정한다.
2. **품질•안전•비용 검증**: 평가 세트 합격선을 판정한다.
3. **승인 구성 배포**: 통과 조합만을 라우팅한다.
4. **응답•비용 관측**: 입력•출력•지연•비용을 기록한다.
5. **개선 조건 환류**: 저하•사고 사례를 평가 세트에 반영한다.

> 요약: 평가 합격 구성 배포 후 운영 이력을 재평가가 핵심이다.
...
- **RAG 적용**: 외부 문서를 검색해 최신•사내 지식의 답변 근거를 보완하는 방식이다.

</details>

| LLM 개선 방식 | **프롬프트 설계(Prompt Engineering)** | **미세조정(Fine-Tuning)** | **RAG 적용** |
|:---|:---|:---|:---|
...
> 요약: 형식은 프롬프트, 행동은 미세조정, 지식은 RAG가 핵심이다.
...
- **안전 실패 은폐**: 평균 품질 점수가 높아도 일부 유해•민감 응답이 별도 기준 없이 승격되는 문제이다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 비결정 응답의 단일 평가 | **반복 표본•평가 세트**의 분포 기준 적용 | 품질 판정 신뢰성 |
| 구성 일부만의 버전 기록 | **구성 카탈로그(Configuration Catalog)**에 모델•프롬프트•RAG 결속 | 응답 결과 재현성 |
| 변경 뒤 **프롬프트 회귀(Prompt Regression)** | 대표 평가 세트 재실행 | 기존 질문 품질 보존 |
| 평균 품질의 **안전 실패 은폐** | **승격 게이트(Promotion Gate)**의 안전•근거성 별도 판정 | 고위험 응답 승격 차단 |
| **토큰(Token)** 증가의 비용 급증 | 요청별 한도•비용 예산 적용 | 서비스 비용 통제 |
...
- **품질•안전•비용 승격 기준**: 반복 평가의 품질 분포와 안전 기준을 충족하고 비용 예산 안에 있는 LLM 구성만 운영 대상으로 승인하는 기준이다.
- **구성 승격(Configuration Promotion)**: 검증된 모델•프롬프트•검색 설정 묶음을 운영 라우팅 대상으로 지정하는 절차이다.

</details>

- **품질•안전•비용 승격 기준**을 충족한 구성만 **구성 승격(Configuration Promotion)**한다.
