...
- 정의/개념: XML 계약 **메시지 프로토콜**과 자원 중심 **아키텍처 스타일의 비교 기준**이다.
- 배경/필요성: 단일 방식으로는 **엄격한 계약•웹 자원 확장성 동시 충족하기 어렵다**.
...
- **XML•WSDL•WS-*** 기반 SOAP 계약•확장이 핵심이다.
- **URI•균일 인터페이스** 기반 REST 자원 조작이 핵심이다.
- **Fault•HTTP 상태** 기반 오류 의미를 분리한다.
...
- **계약 엄격성•메시지 보안•자원 확장성**으로 SOAP•REST을 결정한다.
