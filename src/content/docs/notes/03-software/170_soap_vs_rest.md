...
- 배경/필요성: 초기 엔터프라이즈 시스템 간 통합에서 엄격한 WSDL 인터페이스 명세와 WS-* 기반 보안 및 분산 트랜잭션(2PC)을 보장하는 SOAP 프로토콜이 필수적이었으나, 무거운 XML 봉투(Envelope) 파싱 부하, HTTP POST 단일 메서드 사용에 따른 웹 캐싱 불가 및 모바일·웹 환경에서의 높은 대역폭 낭비 한계가 발생함에 따라, HTTP 고유의 URI 자원 식별과 표준 CRUD 메서드 및 경량 JSON 포맷을 활용하는 REST 아키텍처 스타일을 도입하여 **시스템 결합도를 낮추고 모바일·클라우드 환경의 통신 유연성과 성능을 극대화**할 필요
