...
date: "2026-08-26T13:16:26+09:00"
tags:
  - "notes-software"
weight: 170
extra:
...
#### 한줄 요약
- 행위 중심의 엄격한 프로토콜(SOAP)과 자원 중심의 경량 웹 아키텍처(REST)가 상호 대조된다.
...
#### 한줄 요약
- SOAP(XML 계약/보안)과 REST(경량 HTTP/JSON)가 결합된다.
...
#### 한줄 요약
- 요청 수신 → 스키마 검증 → 로직 실행 → 직렬화 → 응답 회신 순으로 진행된다.
...
#### 한줄 요약
- REST 캐싱 전환, 멱등 키 도입, URI 버저닝, OpenAPI 계약 테스트로 운영한다.
