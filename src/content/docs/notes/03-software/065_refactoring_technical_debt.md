...
- **추가 비용•결함 위험 누적**: 기술부채를 방치해 반복 변경 비용과 오류 가능성이 계속 커지는 현상이다.
...
- **리팩터링(Refactoring)**: 외부 동작을 유지하며 내부 구조를 개선하는 작업이다.
...
- **상환 우선순위•완료 조건**: 부채 이자와 제거 비용을 비교해 작업 순서와 종료 기준을 정한 값이다.
...
- **특성화•회귀 테스트**: 기존 동작을 먼저 고정하고 구조 변경 후 손상 여부를 확인하는 시험 조합이다.
- **작은 커밋•반복 검증**: 변경 원인을 격리하고 실패 때 빠르게 복구하도록 작업을 나누는 방식이다.
- **정책 객체(Policy Object)**: 흩어진 업무 판단 규칙을 하나의 객체로 모은 설계 요소이다.
- **변경 빈도•장애 영향**: 해당 코드가 수정되는 횟수와 결함이 발생했을 때 사용자•업무에 미치는 피해 규모이다.
- **기능•리팩터링 커밋 분리**: 외부 동작 추가와 내부 구조 개선을 서로 다른 변경 이력으로 나눠 원인과 회귀를 추적하는 방식이다.
- **점진 교체•동작 비교**: 기존 구현과 신규 구현을 작은 범위부터 병행하고 같은 입력의 결과를 비교하며 전환하는 방식이다.
...
- **부채 이자•변경 빈도•회귀 위험**: 기술부채의 상환 순서를 판단하는 반복 비용, 수정 가능성, 기능 손상 가능성이다.
