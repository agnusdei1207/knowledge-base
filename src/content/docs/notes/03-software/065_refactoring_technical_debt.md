...
- **1. 부채·이자 기록**: 위치와 반복 비용을 남긴다.
- **2. 상환 대상 선택**: 이자와 상환 비용을 비교한다.
- **3. 현재 동작 고정**: 특성화 테스트를 작성한다.
- **4. 작은 구조 변경**: 복구 가능한 단위로 개선한다.
- **5. 회귀 테스트**: 변경 전후 결과를 확인한다.
- **6. 부채 항목 종료**: 통과한 개선 효과를 기록한다.
- **7. 직전 변경 복구**: 실패한 변경을 되돌린다.
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 테스트 없는 변경 | 특성화·회귀 테스트로 동작 고정 | 기존 기능 손상 방지 |
| 대규모 리팩터링 | 작은 커밋으로 점진 적용 | 원인 격리와 복구 용이 |
| 스멜 일괄 제거 | 변경 빈도·장애 영향으로 우선순위 결정 | 개선 비용 낭비 방지 |
| 기능 개발 혼합 | 기능과 리팩터링 커밋 분리 | 변경 원인 추적성 확보 |
| 전면 재작성 낙관 | 점진 교체와 동작 비교 적용 | 업무 규칙 누락 위험 감소 |
...
- **부채 이자·변경 빈도·회귀 위험**으로 상환 순서를 정한다.
