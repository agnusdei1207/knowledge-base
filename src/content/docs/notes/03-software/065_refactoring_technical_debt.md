...
- 배경/필요성: 기술부채 방치는 반복 변경마다 **추가 비용•결함 위험 누적**이 발생한다.

#### 한줄 요약

- 외부 동작 보존 기반 **내부 구조•기술부채 개선**이 핵심이다.
...
2. **상환 우선순위•완료 조건 결정**: 이자•비용을 비교한다.
3. **특성화 테스트 작성**: 기존 외부 동작을 고정한다.
...
- 고이자 부채의 **작은 변경•회귀 검증 상환 흐름**이 핵심이다.
...
- 변경 빈도•장애 영향 기반 **점진 상환 통제**가 핵심이다.
...
- **부채 이자•변경 빈도•회귀 위험**으로 상환 순서를 결정한다.

#### 한줄 요약

- 이자•변경 빈도•회귀 위험 기반 **상환 순서 결정**이 핵심이다.
