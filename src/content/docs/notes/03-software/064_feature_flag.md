...
date: "2026-07-30T23:16:24+09:00"
tags:
  - "notes-software"
weight: 64
extra:
...
- **설정 저장소(Configuration Store)**: 플래그 규칙·변형값·기본값과 버전을 보관하는 저장소
- **평가 모듈(Evaluation Module)**: 플래그 키와 요청 속성을 규칙에 대조해 변형값을 결정하는 구성요소
- **기능 경로(Feature Path)**: 평가된 변형값에 따라 애플리케이션이 실행하는 코드 분기
- **감사 기록(Audit Log)**: 평가 결과·규칙 버전·변경 주체를 추적하도록 남긴 기록
- **캐시·시간 제한(Cache·Timeout)**: 설정을 임시 저장하고 조회 대기 시간을 제한하는 장애 격리 수단
- **역할 기반 접근통제(Role-Based Access Control, RBAC)**: 역할별로 플래그 조회·변경 권한을 제한하는 통제
- **플래그 의존성(Flag Dependency)**: 한 플래그의 유효한 값이 다른 플래그 상태에 좌우되는 관계

> **키워드:** 피처 플래그 (Feature Flag)
