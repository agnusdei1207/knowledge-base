...
- **1. 플래그 평가 요청**: 키와 요청 속성을 전달한다.
- **2. 규칙 조회**: 대상 규칙과 기본값을 요청한다.
- **3. 규칙·안전 기본값**: 현재 설정값을 반환한다.
- **4. 평가된 변형값 반환**: 조건과 코호트를 판정한다.
- **5. 선택 기능 실행**: 변형값의 경로를 호출한다.
- **6. 기능 결과**: 처리 결과를 반환한다.
- **7. 평가 결과 기록**: 값·규칙 버전·대상을 남긴다.
...
- **노출 목적·안전 기본값·수명**에 따라 플래그를 운영한다.
