...
date: "2026-08-05T13:40:07+09:00"
tags:
  - "notes-software"
weight: 64
extra:
...
- 정의/개념: **기능 플래그** 는 사용자•환경•시간 같은 요청 조건에 따라 실행 중인 애플리케이션의 기능 경로를 선택하는 릴리스 통제 기법
- 배경/필요성: 재배포만으로는 사용자별 공개 시점•**긴급 차단 통제 불가**

#### 한줄 요약

- 배포와 릴리스를 분리하는 **실행 중 기능 경로 통제**
...
1. **규칙•안전 기본값 조회**: 최신 규칙과 실패 기본값 확보
2. **요청 속성•대상 규칙 평가**: 노출 조건과 요청 비교
3. **변형값 선택**: 규칙 결과나 기본값으로 기능 결정
4. **기능 경로 실행**: 선택한 업무 코드 분기 수행
5. **평가 결과•규칙 버전 기록**: 판정 근거 감사 보존

> 요약: 요청 속성과 대상 규칙을 평가해 실행 중 기능 경로를 선택

#### 한줄 요약

- 대상 규칙 평가와 **안전 기본값 기반 경로 선택**
