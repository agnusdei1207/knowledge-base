...
- I/O 대기 시 **언마운트** 기반 캐리어 스레드를 즉시 다른 가상 스레드에 양보
...
**동작 원리** 1. **Mount**: JVM 스케줄러가 유휴 캐리어 스레드에 가상 스레드를 탑재하여 실행 시작
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| `synchronized` 사용으로 **스레드 피닝** 발생 | `synchronized`를 **ReentrantLock** 기반 교체 | I/O 대기 시 Unmount 정상 작동 보장 |
| 가상 스레드 수 제한 없어 하위 DB 커넥션 풀 고갈 | DB 커넥션 접근을 **세마포어(Semaphore)** 로 동시 접근 수 제한 | 가상 스레드 고동시성 유지하며 하위 자원 보호 |
| `ThreadLocal` 과다 사용으로 수백만 스레드 메모리 오버헤드 급증 | `ThreadLocal`을 **Scoped Values** 기반 대체 | 가상 스레드별 불필요한 데이터 복사 제거 |
