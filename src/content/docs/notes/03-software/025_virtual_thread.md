...
- **정의/개념**: 가상 스레드를 캐리어에 다중화
- **배경/필요성**: 대기 스레드가 자원을 점유해 경량 다중화 필요
...
- 생성 비용이 낮아 작업마다 새 가상 스레드 사용한다.
- 지원 블로킹 시 캐리어에서 분리해 다른 작업 실행한다.
- CPU 병렬도는 캐리어·프로세서 코어 수로 제한한다.
- JDK 24부터 동기화 차단의 고정을 줄인다.
...
| 판단 기준 | 가상 스레드 | 플랫폼 스레드 |
|:---|:---|:---|
| 핵심 특징 | JDK가 캐리어에 다중화 | 운영체제가 직접 스케줄링 |
| 적용 기준 | 다수 독립 요청·블로킹 입출력 | CPU 계산·네이티브 호출·스레드 고정 |
| 주요 위험 | 캐리어 고정·외부 자원 고갈 | 생성·문맥 전환·메모리 비용 |
...
1. API 서버는 요청별 가상 스레드·데이터베이스 연결 한도 적용
2. 영상 계산 작업은 코어 수 기준 플랫폼 스레드 풀 사용

### 쉽게 이해하기 (학습용)

- 서버는 요청마다 가상 스레드를 만들되 한정된 데이터베이스 연결은 별도로 제한한다.
- 영상 계산은 가상 스레드 수가 아니라 실제 코어 수에 맞춰 작업자를 둔다.

## Ⅶ. 결론

- 입출력 대기율·외부 자원 한도·고정 구간으로 스레드 모델 선택
