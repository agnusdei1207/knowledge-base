...
- **자바 개발 키트(Java Development Kit, JDK)**: Java 프로그램 개발•실행 도구와 가상 스레드 기능을 제공하는 배포 단위다.
- **가상 스레드(Virtual Thread)**: JDK가 스케줄링하는 경량 Java 실행 단위다.
- **캐리어 스레드(Carrier Thread)**: 가상 스레드를 실제로 실행하는 플랫폼 스레드다.
- **다중화(Multiplexing)**: 많은 가상 스레드를 소수 캐리어에 번갈아 탑재해 재사용하는 방식이다.

</details>

- 정의/개념: JDK가 많은 경량 Java 실행 단위를 **소수 캐리어 스레드에 다중화**하는 동시성 실행 모델이다.
- 배경/필요성: 요청별 플랫폼 스레드는 입출력 대기 중에도 **운영체제 스레드•스택 점유**가 발생한다.

#### 한줄 요약

- 다수 가상 스레드를 소수 캐리어에 **다중화**가 핵심이다.
...
- **경량 생성•스케줄링**으로 요청별 스레드 모델을 지원한다.
- **블로킹 시 캐리어 분리**로 다른 가상 스레드를 실행한다.
- CPU 병렬도는 **캐리어•물리 코어 수**로 제한한다.

#### 한줄 요약

- 대기 동시성은 확대되지만 **CPU 병렬도**는 코어로 제한한다.
...
- 가상 스레드•JVM•캐리어•**입출력 하위 시스템** 구조이다.
...
- **캐리어 탑재**: 실행 가능한 가상 스레드를 플랫폼 스레드에 연결하는 단계다.
- **가상 스레드 실행**: 탑재된 호출 스택을 재개해 Java 코드를 수행하는 단계다.
- **캐리어 분리 가능성 판정**: 블로킹 지점에서 호출 스택을 보존하고 캐리어를 놓을 수 있는지 확인하는 단계다.
- **분리•실행 가능 전환**: 호출 스택을 내려놓고 입출력 완료 뒤 다시 실행 가능 상태로 바꾸는 단계다.
- **고정 대기(Pinning Wait)**: 네이티브•외부 함수 등에서 캐리어를 점유한 채 완료를 기다리는 단계다.
...
- **1. 캐리어 탑재**: 실행 가능 작업을 빈 캐리어에 배정이다.
- **2. 가상 스레드 실행**: 호출 스택을 재개해 코드를 수행한다.
- **3. 캐리어 분리 가능성 판정**: 블로킹 구간의 분리 가능 여부를 확인한다.
- **4. 분리•실행 가능 전환**: 상태 보존 후 완료 때 준비 큐 등록한다.
- **5. 고정 대기**: 캐리어를 점유한 채 외부 작업 완료 대기이다.

#### 한줄 요약

- 블로킹 시 **분리•재탑재**, 불가 구간은 고정 대기가 핵심이다.
...
> 요약: **I/O 대기**는 가상 스레드, **CPU 계산**은 플랫폼 스레드 풀 사용이 핵심이다.
...
- **요청 문맥(Request Context)**: 한 요청의 추적값•인증 정보처럼 처리 전 구간에 전달되는 상태다.
...
- I/O 대기가 많고 고정 구간이 짧으면 **가상 스레드**, CPU 계산은 **플랫폼 스레드 풀**을 선택한다.

#### 한줄 요약

- I/O 대기와 고정 구간에 따라 **스레드 모델 선택**이 핵심이다.
