...
date: "2026-08-05T12:50:56+09:00"
tags: [notes-software]
weight: 25
extra:
  question_no: "025"
...
- 정의/개념: JDK가 많은 경량 Java 실행 단위를 **소수 캐리어 스레드에 다중화**하는 동시성 실행 모델
- 배경/필요성: 요청별 플랫폼 스레드는 입출력 대기 중에도 **운영체제 스레드•스택 점유**

#### 한줄 요약

- 다수 가상 스레드를 소수 캐리어에 **다중화**
...
- 대기 동시성은 확대되지만 **CPU 병렬도**는 코어로 제한
...
- **1. 캐리어 탑재**: 실행 가능 작업을 빈 캐리어에 배정
- **2. 가상 스레드 실행**: 호출 스택을 재개해 코드 수행
- **3. 캐리어 분리 가능성 판정**: 블로킹 구간의 분리 가능 여부 확인
- **4. 분리•실행 가능 전환**: 상태 보존 후 완료 때 준비 큐 등록
- **5. 고정 대기**: 캐리어를 점유한 채 외부 작업 완료 대기

#### 한줄 요약

- 블로킹 시 **분리•재탑재**, 불가 구간은 고정 대기
