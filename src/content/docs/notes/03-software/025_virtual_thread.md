...
- 정의/개념: 가상 스레드를 **소수 캐리어에 다중화**하는 방식
- 배경/필요성: 요청별 스레드의 **대기 자원 점유를 축소**
...
sequenceDiagram
    ...
    alt 분리 가능한 블로킹
        V->>I: 3. I/O 대기 등록
        V->>S: 4. 호출 스택 보존·캐리어 분리
        I-->>S: 5. I/O 준비 통지
        S->>C: 6. 실행 가능한 가상 스레드 재탑재
        C-->>V: 7. 중단 지점부터 실행 재개
    else 네이티브·외부 함수 고정
        V->>C: 캐리어를 점유한 채 대기
...
- **1. 실행 가능 등록**: 작업별 가상 스레드를 스케줄러에 등록
- **2. 캐리어에 탑재**: 스케줄러가 빈 캐리어에 작업 배정
- **3. I/O 대기 등록**: 완료 조건을 입출력 하위 시스템에 등록
- **4. 호출 스택 보존·캐리어 분리**: 대기 작업의 실행 상태 보존
- **5. I/O 준비 통지**: 완료된 작업을 스케줄러에 알림
- **6. 실행 가능한 가상 스레드 재탑재**: 빈 캐리어에 다시 배정
- **7. 중단 지점부터 실행 재개**: 보존한 호출 위치에서 계속 실행
...
- API 서버는 요청마다 가상 스레드를 만들되 데이터베이스 연결 수와 외부 API 동시 호출 수는 별도로 제한
