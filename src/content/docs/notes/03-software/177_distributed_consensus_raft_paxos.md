...
- **결정적 실행**: 같은 입력에서 모든 노드가 같은 결과를 내는 실행이다.
...
선의 의미: 클라이언트와 리더•제안자는 팔로워•수락자의 합의 기록 경계에 결합되고, 그 아래에는 교차 승인 집합인 다수 쿼럼과 확정 명령을 보관•실행하는 상태 머신•스냅샷이 놓이는 정적 합의 구조를 뜻한다.
