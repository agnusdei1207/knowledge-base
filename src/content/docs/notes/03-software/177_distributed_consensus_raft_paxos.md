...
1. **상태 변경 명령**: 리더에 요청·중복 식별자 전달
2. **임기·로그 복제**: 첫 팔로워의 이전 로그 일치 검사
3. **임기·로그 복제**: 추가 팔로워의 동일 명령 기록
4. **영속 기록 승인**: 디스크 기록 후 쿼럼 응답
5. **과반 커밋·적용**: 확정 명령의 순서 실행
