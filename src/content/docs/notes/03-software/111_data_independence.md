...
- **설계 원칙**: 하위 스키마 변경을 매핑에서 흡수해 상위 스키마와 응용의 변경을 줄이는 원칙이다.

</details>

- 정의/개념: **데이터 독립성**은 하위 스키마나 저장 구조의 변경을 계층 간 매핑에서 흡수하여 상위 스키마와 응용의 변경을 줄이는 데이터베이스 설계 원칙
...
- **2. 논리 연산**: 개념 질의를 현재 물리 접근 경로에 대응시키는 단계이다.
...
- **즉시 스키마 교체는 구버전 소비자 요청 실패 유발**: 신구 구조의 호환 기간 없이 스키마를 바꿔 기존 응용을 중단시키는 문제이다.
