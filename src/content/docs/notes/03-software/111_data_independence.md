...
- 배경/필요성: 스키마•저장 변경이 응용까지 전파되면 **유지보수 비용** 급증이 발생한다.
...
- **개념 질의•논리 연산 변환**: 외부 뷰 질의를 개념 구조의 연산으로 바꾸는 단계다.
- **물리 접근 요청**: 개념 질의를 현재 저장 접근 경로에 대응시키는 단계다.
- **내부 레코드 반환**: 내부 저장 구조에서 물리 데이터를 읽는 단계다.
- **외부 뷰 결과 복원**: 레코드를 사용자별 표현으로 바꾸는 단계다.
...
- **1. 개념 질의•논리 연산 변환**: 외부 뷰를 논리 구조에 대응이다.
- **2. 물리 접근 요청**: 파일•페이지•인덱스 조회한다.
- **3. 내부 레코드 반환**: 물리 데이터 반환한다.
- **4. 외부 뷰 결과 복원**: 사용자별 표현으로 변환한다.
...
> 요약: 이름•단위•업무 의미 변경은 **단순 호환 뷰로 은닉 불가**가 핵심이다.
...
| 물리 구조 변경은 결과가 같아도 실행 계획 악화 가능 | **실행 계획•p95•자원량** 비교 | **성능 회귀** 탐지 |
...
- 저장 변경은 **개념-내부 매핑**, 논리 변경은 **외부-개념 매핑**에서 격리가 핵심이다.
