...
- 정의/개념: DBMS의 3단계 스키마 구조를 기반으로, 물리적 저장 장치 구조 변경이나 논리적 테이블 구조 변경이 응용 프로그램 소스 코드에 전파되는 것을 사전에 차단하는 아키텍처 원칙인 **Data Independence**
- 배경/필요성: 파일 시스템 시절 데이터 저장 파일 포맷 변경 시 응용 프로그램 재개발 폭증 문제 극복, 시스템 유지보수성 및 확장성 보장 요구성
...
## Ⅲ. 구조 및 구성요소 (ANSI/SPARC 3단계 스키마 및 독립성 아키텍처)
...
| 스키마 계층 (Schema Layer) | 주요 정의 및 역할 | 데이터 독립성 구현 유형 |
|:---|:---|:---|
| **External Schema (외부)** | **각 사용자/응용 프로그램 관점의 뷰 (View) 정의** | **Logical Data Independence** (뷰 매핑으로 격리) |
| **Conceptual Schema (개념)**| **DB 전체의 통합 논리적 구조 (테이블, 관계, 제약조건)**| **Logical & Physical Mapping의 중심축** |
| **Internal Schema (내부)** | **실제 디스크 상의 물리적 파일 저장 방식, 인덱스** | **Physical Data Independence** (인덱스 변경 격리) |

#### 한줄 요약

- 외부 스키마, 개념 스키마, 내부 스키마 사이에 두 단계의 매핑을 둔다.

## Ⅳ. 흐름도 (독립성 보장 변환 메커니즘)
...
```text
[App Code: SELECT user_name FROM UserView] ──► [External Schema (UserView)]
                                                     │
                                                     ▼ (External-Conceptual Mapping)
                                               [Conceptual Schema (Users Table)]
                                                     │
                                                     ▼ (Conceptual-Internal Mapping)
                                               [Internal Schema (Disk Page / Index)]
```

### 동작 원리

1. **Logical Change Event**: DB `Users` 테이블에 신규 컬럼 `age` 추가 (개념 스키마 변경).
2. **Mapping Maintenance**: `External-Conceptual Mapping` 수정으로 기존 `UserView` 파행 차단.
3. **Application Safety**: 기존 응용 프로그램 소스 코드는 수정 0건으로 정상 작동 완료 (**논리적 데이터 독립성 수용**).

#### 한줄 요약

- 외부 스키마와 내부 스키마 사이의 매핑이 변경된 저장 구조를 은닉하여 동일한 결과를 반환한다.

## Ⅴ. 종류 및 비교 (논리적 독립성 대 물리적 독립성)
...
## Ⅵ. 실무 고려사항 및 대책 (독립성 유지를 위한 3대 설계 패턴)
...
- **데이터 독립성 수립 기준**에 따라 차세대 DB 재구축 시 **View Layer & ORM Mapping 기반 데이터 독립성** 필수 적용
