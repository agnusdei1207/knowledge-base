...
date: "2026-08-05T13:46:12+09:00"
tags:
  - "notes-software"
weight: 67
extra:
...
1. **코드 파싱•흐름 모델 생성**: AST•CFG•DFG 변환
2. **규칙 프로필 적용**: 소스•싱크•안전 조건 대조
3. **오염 경로 추적**: 입력부터 위험 동작까지 분석
4. **결함 위치•경로•심각도 기록**: 탐지 근거 보존

> 요약: 코드 모델과 규칙으로 위험 데이터가 소스에서 싱크까지 도달하는 경로를 검색

#### 한줄 요약

- 코드 모델•규칙 기반 **오염 경로 추적 흐름**
