...
- **1. 소스·의존성 전달**: 분석 대상을 입력한다.
- **2. AST·CFG·DFG 생성**: 코드 흐름을 모델링한다.
- **3. 취약점 규칙 조회**: 언어별 규칙을 요청한다.
- **4. 패턴·안전 조건 반환**: 분석 조건을 제공한다.
- **5. 위치·경로·심각도 보고**: 오염 전파를 제시한다.
- **6. 수정 코드 재검사 요청**: 원인 수정 후 반복한다.
...
- **코드 경로·실행 가능성·자산 영향**으로 조치 순서를 정한다.
