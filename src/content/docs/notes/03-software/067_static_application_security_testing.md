...
- **1. 분석 대상 파싱**: 소스·의존성을 언어 구조로 변환
- **2. 코드 모델 생성**: AST·CFG·DFG로 실행 가능 경로 구성
- **3. 보안 규칙 대조**: 오염원에서 싱크까지 안전 조건 검증
- **4. 오염 경로 보고**: 위치·전파 경로·심각도 제공
- **5. 수정 코드 재검사**: 원인 제거 여부를 같은 규칙으로 확인
