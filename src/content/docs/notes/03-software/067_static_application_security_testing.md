...
- 정의/개념: **SAST**는 애플리케이션을 실행하지 않고 소스•바이트코드의 데이터 흐름과 제어 흐름을 분석하여 보안 결함을 찾는 정적 보안 시험 기법이다.
- 배경/필요성: 수동 검토는 대규모 코드의 **입력-위험 동작 경로 추적 한계**가 발생한다.

#### 한줄 요약

- 비실행 코드•흐름 분석 기반 **취약 경로 탐지**가 핵심이다.
...
- **화이트박스 분석**으로 결함 위치와 경로 제시가 핵심이다.
- 개발 초기•빌드 단계의 **시프트 레프트 탐지**가 핵심이다.
- 동적 실행 맥락 부재로 **오탐•미탐 발생**이 핵심이다.

#### 한줄 요약

- 조기 위치 추적과 **실행 맥락 부재의 오탐•미탐**이 핵심이다.
...
1. **코드 파싱•흐름 모델 생성**: AST•CFG•DFG을 변환한다.
2. **규칙 프로필 적용**: 소스•싱크•안전 조건을 대조한다.
3. **오염 경로 추적**: 입력부터 위험 동작까지 분석한다.
4. **결함 위치•경로•심각도 기록**: 탐지 근거를 보존한다.

> 요약: 코드 모델과 규칙으로 위험 데이터가 소스에서 싱크까지 도달하는 경로를 검색이 핵심이다.
...
- 코드 원인은 SAST, 실행 공격은 **DAST**가 핵심이다.
...
- 실행 가능성•노출•자산 영향 기반 **경고 우선순위**가 핵심이다.
...
- 경로•실행 가능성•자산 영향 기반 **SAST 조치**가 핵심이다.
