...
- 무조건적인 'Cloud First'에서 벗어나 비즈니스 워크로드의 특성과 경제성에 따라 인프라를 최적 배분하는 가장 성숙하고 실용적인 하이브리드 인프라 현대화 전략으로 확립되었으며, 실무 추진 시에는 전력/상면/인건비를 포함한 5개년 총소유비용(TCO) 정밀 타당성 분석, 대규모 데이터 반출 비용을 절감하는 전용선(Direct Connect) 및 물리 어플라이언스(Snowball) 활용, 오픈소스 Kubernetes/SDS 기반 프라이빗 PaaS 표준화, CDC(Debezium) 기반 무중단 롤백 파이프라인을 결합하여 운영 리스크 없이 수십 %의 인프라 비용을 절감
