...
- 배경/필요성: 초기 인프라 투자비(CapEx) 절감을 위해 퍼블릭 클라우드로 전면 이전했던 기업들이 시스템 규모 확장 및 장기 운영 과정에서 마주치는 지속적 대규모 부하(Steady-State)의 종량제 비용(OpEx) 폭증, 페타바이트급 데이터 중력(Data Gravity)과 반출(Egress) 요금 부담, 특정 CSP PaaS 종속 및 보안 규제 이슈를 해결하기 위해, 검증된 안정 워크로드를 온프레미스 IDC, 프라이빗 클라우드 또는 코로케이션 베어메탈 인프라로 전략적으로 재배치하는 클라우드 회귀(Cloud Repatriation)를 실행하여 **장기 총소유비용(TCO) 최적화와 데이터 주권 및 인프라 통제권을 회복**할 필요
...
- 무조건적인 'Cloud First'에서 벗어나 비즈니스 워크로드의 특성과 경제성에 따라 인프라를 최적 배분하는 **가장 성숙하고 실용적인 하이브리드 인프라 현대화 전략**으로 확립되었으며, 실무 추진 시에는 **전력/상면/인건비를 포함한 5개년 총소유비용(TCO) 정밀 타당성 분석, 대규모 데이터 반출 비용을 절감하는 전용선(Direct Connect) 및 물리 어플라이언스(Snowball) 활용, 오픈소스 Kubernetes/SDS 기반 프라이빗 PaaS 표준화, CDC(Debezium) 기반 무중단 롤백 파이프라인**을 결합하여 운영 리스크 없이 수십 %의 인프라 비용을 절감
