...
- 정의/개념: **교살자 무화과 패턴**은 요청 경로를 업무 경계별 새 서비스로 점진 전환하면서 해당 레거시 기능을 단계적으로 축소•제거하는 현대화 패턴이다.
- 배경/필요성: 일괄 재작성은 전환 결함•중단 영향이 **전체 시스템으로 확산**이 발생한다.
...
- 파사드의 **신•구 트래픽 분기**가 핵심이다.
- 도메인 경계별 **기능•데이터 소유권 이관**이 핵심이다.
- 병행 실행의 **결과 정합성 검증**이 핵심이다.
- **ACL**로 레거시 모델의 신규 모델 전파 차단이 핵심이다.
...
- **부패 방지 계층(Anti-Corruption Layer, ACL)**: 신•구 모델 변환•격리이다.
...
- 독립 업무 경계부터 **병행 검증** 후 **소유권 이관**•레거시 폐기가 핵심이다.
