---
sidebar:
  order: 219
  label: "219. 스트랭글러 패턴 (Strangler Fig Pattern)"
  badge:
    text: "기출 · 50%"
    variant: note
title: "스트랭글러 패턴 (Strangler Fig Pattern)"
date: "2026-07-27T23:59:59+09:00"
tags: ["notes-software"]
weight: 219
extra:
  question_no: "219"
  source_status: "기출"
  source_history: "123회"
  priority: 50
  priority_note: "점진 교체와 양방향 호환이 기존 출제됨"
---

## 미리 알고가기

- **스트랭글러(Strangler)**: 레거시 기능을 업무 단위로 신규 시스템에 이전·폐기하는 패턴임
- **파사드(Facade)**: 단일 진입점에서 요청을 신규·레거시 시스템으로 라우팅함
- **도메인 쉼(Domain Seam)**: 독립 이전 가능한 업무 경계 단위임
- **부패 방지 계층 ACL(에이씨엘, Anti-Corruption Layer)**: 영어 세 낱말의 첫 글자를 쓴 표기로 레거시 데이터 모델이 신규 모델에 번지는 것을 차단함
- **병행 실행(Parallel Run)**: 신·구 시스템이 같은 작업을 실행해 결과를 비교하는 방식임
- **변경 데이터 캡처 CDC(씨디씨, Change Data Capture)**: 영어 세 낱말의 첫 글자를 쓴 표기로 변경 로그를 읽어 다른 저장소에 반영함
- **데이터 소유권**: 어느 시스템이 특정 데이터의 정본을 생성·수정하는지 정한 책임임
- **프록시·라우팅**: 프록시는 요청을 중계하고 라우팅은 목적지 경로를 선택함
- **롤백·리팩토링**: 롤백은 이전 경로로 복귀하고 리팩토링은 기능 유지 상태로 내부 구조를 개선함


## Ⅰ. 개요

- 정의/개념: 업무 경계별 **레거시 시스템의 점진 대체**
- **배경/필요성**: 일괄 재작성·전환의 **대규모 실패 한계** 해소

### 쉽게 이해하기 (학습용)
- 부분 이전으로 중단 없이 전환함

## Ⅱ. 특징

- 파사드의 **신·구 트래픽 분기**
- 도메인 경계별 **기능·데이터 소유권 이관**
- 병행 실행의 **결과 정합성 검증**

### 쉽게 이해하기 (학습용)
- 결함 시 이전 경로로 즉시 복원함

## Ⅲ. 아키텍처 및 구성요소

```mermaid
flowchart LR
    C[클라이언트] --> F[파사드·라우터]
    F -->|미전환 업무| L[레거시 시스템]
    F -->|전환 업무| N[신규 서비스]
    N --> A[부패 방지 계층]
    A -->|잔여 연계| L
```

| 설계 요소 | 설명 |
|:---|:---|
| 파사드·라우터 | 업무별 요청을 레거시 시스템과 신규 서비스로 분기함 |
| 레거시 시스템 | 아직 전환하지 않은 업무와 데이터의 정본을 담당함 |
| 신규 서비스 | 분리·전환된 업무의 처리와 데이터 소유권을 담당함 |
| 부패 방지 계층 | 신규 모델과 레거시 인터페이스의 차이를 변환·격리함 |

> 요약: 파사드·라우터가 신·구 경로를 분기하고 부패 방지 계층이 모델 차이를 격리함

### 쉽게 이해하기 (학습용)
- 파사드 레이어를 통해 신구 시스템의 역할을 정적으로 분리함

## Ⅳ. 원리 및 절차 흐름도

| 절차 | 설명 |
|:---|:---|
| **분석** | 연관도 및 리스크 진단함 |
| **선정** | 독립 도메인 단위 지정함 |
| **구현** | 부패 방지 계층 및 로직 구현함 |
| **전환** | 병행 비교 후 소유권 이관함 |
| **폐기** | 의존성 제거 후 자원 삭제함 |

> 요약: 신구 시스템 병행 검증 후 안전하게 자원을 폐기함

### 쉽게 이해하기 (학습용)
- 검증된 단위부터 순차적으로 전환하고 레거시를 닫음

## Ⅴ. 종류 및 비교

| 레거시 개선 방식 | 점진 전환 | 일괄 재작성 | 내부 개선 |
|:---|:---|:---|:---|
| 적용 기준 | 연속 운영·부분 경계 확보 | 소규모·짧은 전환 가능 | 기능 유지·구조 개선 |
| 핵심 특징 | 업무 단위로 신·구 교체 | 전체 시스템 일괄 교체 | 기존 코드 내부 구조 개선 |
| 한계 | 이중 비용·데이터 동기화 | 전환 실패·대규모 결함 | 레거시 제약 잔존 |

> 요약: 위험도에 따라 전환 방식 산정함

### 쉽게 이해하기 (학습용)
- 전면 개편의 전환 위험을 점진적 교체로 완화함

## Ⅵ. 실무 사례

1. 주문 조회를 **파사드로 신규 경로 전환**

### 쉽게 이해하기 (학습용)
- 주문 조회는 오류 시 레거시 경로로 되돌림

## Ⅶ. 결론

- 레거시 전면 교체의 위험을 줄이기 위해 **기능 경계·병행 결과 일치·데이터 정합성·복귀 가능성**을 검토하고, 파사드로 작은 기능부터 신규 시스템으로 단계 전환한다

### 쉽게 이해하기 (학습용)
- 작은 기능부터 검증해 실패 범위를 그 기능에 가둠
