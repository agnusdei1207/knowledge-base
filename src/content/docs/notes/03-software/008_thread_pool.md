...
- 배경/필요성: 클라이언트 요청마다 OS 스레드를 동적으로 생성·소멸할 때 발생하는 무거운 커널 스택 할당 오버헤드, 과도한 문맥 전환 비용 및 통제 불가능한 스레드 폭증(OOM) 한계를 극복하고, 사전 할당된 워커 스레드 집합과 유한 작업 큐(Bounded Queue), 포화 거절 정책을 통해 **동시성 자원 상한을 엄격히 통제하고 시스템 안정성과 처리량을 극대화**할 필요
...
| 유한 작업 큐 | 포화 시 태스크를 **BlockingQueue**에 적재 |
...
- 대규모 동시성 백엔드(Tomcat, Netty, Java Executor) 및 분산 처리 엔진의 **표준 비동기 자원 관리 아키텍처**로 확립되었으며, 실무에서는 **메모리 고갈 방지를 위한 유한 큐(Bounded Queue) 필수 적용, 과부하 시 자연스러운 역압력(Backpressure) 전파를 위한 CallerRunsPolicy 채택, CPU 바운드 풀과 I/O 바운드 풀의 엄격한 물리적 격리**를 통해 안정적인 SLA를 보장
