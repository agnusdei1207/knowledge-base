...
|:---|:---|:---|:---|
| 스레드 수 관리 | **Core/Max Pool Size**를 같게 고정 | 필요 시 무제한 생성 (60초 유휴 회수) | **ForkJoinPool**의 CPU 코어 수 비례 풀 유지 |
...
- 대규모 동시성 백엔드(Tomcat, Netty, Java Executor) 및 분산 처리 엔진의 표준 비동기 자원 관리 아키텍처로 확립되었으며, 실무에서는 메모리 고갈 방지를 위한 유한 큐(Bounded Queue) 필수 적용, 과부하 시 자연스러운 역압력(Backpressure) 전파를 위한 CallerRunsPolicy 채택, CPU 바운드 풀과 I/O 바운드 풀의 엄격한 물리적 격리를 통해 안정적인 SLA를 보장
