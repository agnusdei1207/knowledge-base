...
- **스레드 재사용성 확보·유한 작업 큐 기반 자원 한도 통제·포화 정책 기반 역압력 방어** ## Ⅲ. 구조 및 구성요소
...
- **스레드 풀 실행기·유한 작업 큐(BlockingQueue)·워커 스레드 풀·포화 거절 처리기** ## Ⅳ. 흐름도
...
**동작 원리** 1. **코어 스레드 검사**: 현재 활성 스레드가 Core Pool Size 미만이면 신규 워커를 생성하여 즉시 실행
...
- **코어 스레드 우선 할당 $\to$ 작업 큐 적재 $\to$ 최대 스레드 확장 $\to$ 포화 거절 정책 발동 $\to$ 유휴 스레드 회수** ## Ⅴ. 종류 및 비교
...
- **유한 작업 큐(Bounded Queue) 크기 제한·CallerRunsPolicy 역압력 전파·CPU/IO 스레드 풀 분리 격리** ## Ⅶ. 결론
...
- **적정 풀 사이징과 유한 큐 기반 역압력 제어** 활용 통한 동시성 시스템의 자원 안정성 및 처리량 극대화
