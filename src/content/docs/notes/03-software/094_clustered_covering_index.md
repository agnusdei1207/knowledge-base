...
- 배경/필요성: 보조 인덱스는 리프에서 행의 위치만 알려 주므로 실제 컬럼을 얻으려면 결과 건수만큼 테이블 랜덤 I/O를 다시 치르고 범위 검색에서는 흩어진 행을 모으는 비용까지 더해지므로, 데이터 자체를 키 순서로 정렬해 두거나 질의가 요구하는 컬럼을 인덱스 안에 함께 담아 두 번째 접근을 없앨 필요
