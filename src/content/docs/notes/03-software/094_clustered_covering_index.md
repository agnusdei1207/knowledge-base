...
- 배경/필요성: 일반 보조 인덱스(Secondary Index) 조회 시 발생하는 2차 테이블 랜덤 I/O(Key/Bookmark Lookup)의 디스크 병목과 비정렬 물리 저장으로 인한 범위 스캔 성능 저하를 극복하고, 실제 데이터 행 자체를 기본키 순서로 물리 정렬하는 클러스터드 인덱스와 쿼리에 필요한 모든 컬럼을 인덱스 리프 노드에 완비하여 2차 테이블 접근을 0으로 만드는 커버링 인덱스를 적용함으로써 **디스크 I/O를 최소화하고 대규모 트랜잭션의 조회 성능을 극대화**할 필요
