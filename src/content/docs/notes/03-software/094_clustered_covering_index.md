...
- 정의/개념: 행 정렬이나 열 포함으로 **페이지 읽기를 줄이는 인덱스**이다.
- 배경/필요성: 행 분산•비포함 열은 **범위 임의 접근•테이블 재조회가 증가한다**.
...
- **후보 인덱스 비용 비교**: 행 조직과 원본 재조회 비용을 비교하는 단계다.
- **시작•종료 키 탐색**: 조건 경계의 리프 위치와 연속 엔트리를 찾는 단계다.
- **커버링 여부 판정**: 인덱스가 필요한 열을 모두 포함하는지 확인하는 단계다.
- **행 위치•비포함 열 조회**: 부족한 결과 열만 원본에서 읽는 단계다.
...
- **1. 후보 인덱스 비용 비교**: 행 조직•재조회 비용을 비교한다.
- **2. 시작•종료 키 탐색**: 연속 리프 엔트리를 확인한다.
- **3. 커버링 여부 판정**: 필요 열의 전체 포함을 확인한다.
- **4. 행 위치•비포함 열 조회**: 부족한 열만 원본 조회한다.
...
- **데이터베이스 관리 시스템(Database Management System, DBMS)**: 데이터 저장과 질의를 관리하는 소프트웨어다.
...
- 범위•정렬 페이지 축소는 **클러스터드**, 재조회 제거는 **커버링**이 핵심이다.
