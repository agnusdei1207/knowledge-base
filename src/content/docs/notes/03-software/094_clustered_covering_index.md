...
- 정의/개념: **클러스터드 인덱스**와 **커버링 인덱스**는 행 배치나 열 포함으로 페이지 읽기를 줄인다.
- 배경/필요성: 행 분산과 비포함 열은 범위 임의 접근과 테이블 재조회를 늘린다.
...
- **클러스터드**: 인접 키를 가까운 페이지에 배치한다.
- **커버링**: 필요한 열을 인덱스에서 반환한다.
- **쓰기 비용**: 분할•키 변경•포함 열 갱신 증가한다.
...
선의 의미: 리프 페이지는 인덱스 키와 포함 열을 저장하고 원본 테이블의 행과 연결되며, 옵티마이저는 리프 페이지만 사용할지 원본 테이블도 읽을지 판단한다.

| 구성요소 | 책임 |
|:---|:---|
| 인덱스 키 | **인덱스 키**로 정렬 순서•탐색 범위 결정 |
| 포함 열 | **포함 열**로 결과에 필요한 비탐색 열 제공 |
| 리프 페이지 | **리프 페이지**에 키•행 위치•포함 열 저장 |
| 옵티마이저 | **옵티마이저**가 커버링•테이블 재조회 비용 판단 |
...
- **후보 인덱스 비용 비교**: 행 조직과 원본 재조회 비용을 비교하는 단계이다.
- **시작•종료 키 탐색**: 조건 경계의 리프 위치와 연속 엔트리를 찾는 단계이다.
- **커버링 여부 판정**: 인덱스가 필요한 열을 모두 포함하는지 확인하는 단계이다.
- **행 위치•비포함 열 조회**: 부족한 결과 열만 원본에서 읽는 단계이다.
...
- **1. 후보 인덱스 비용 비교**: **후보 인덱스 비용 비교**로 행 조직•재조회 비용을 판단한다.
- **2. 시작•종료 키 탐색**: **시작•종료 키 탐색**으로 연속 리프 엔트리를 확인한다.
- **3. 커버링 여부 판정**: **커버링 여부 판정**으로 필요 열의 전체 포함을 확인한다.
- **4. 행 위치•비포함 열 조회**: **행 위치•비포함 열 조회**로 부족한 열만 원본에서 읽는다.
...
| 핵심 특징 | **클러스터드 인덱스**의 키 순서 기반 행 조직 | **커버링 인덱스**의 필요 열 전체 포함 |
| 한계 | 중간 삽입•페이지 분할 | 크기•쓰기•캐시 비용 증가 |

> 요약: 커버링은 인덱스 고정 유형이 아닌 **질의 상대적 성질**이 핵심이다.
...
- **채움 비율(Fill Factor)**: 인덱스 페이지에서 데이터가 차지할 비율이다.
- **페이지 분할(Page Split)**: 새 키 삽입 공간을 만들도록 페이지를 나누는 동작이다.
- **인덱스 재구성(Index Rebuild)**: 조각난 인덱스 페이지를 다시 정돈하는 작업이다.
- **데이터베이스 관리 시스템(Database Management System, DBMS)**: 데이터 저장과 질의를 관리하는 소프트웨어이다.
- **제품별 저장 구조(Product-specific Storage)**: DBMS마다 다른 실제 행 조직 구현이다.
...
| 중간 삽입이 페이지 분할•조각화•지연을 유발 | **채움 비율**•**페이지 분할**•**인덱스 재구성** 감시 | 조각화•삽입 지연 감소 |
| DBMS마다 클러스터드 저장 의미가 달라 설계 오판 | **데이터베이스 관리 시스템**의 **제품별 저장 구조** 확인 | 용어•실제 동작 혼선 방지 |
...
- **인덱스 물리 설계 선택 기준**: 범위 페이지 축소와 원본 재조회 제거 및 쓰기 비용을 함께 평가하는 기준이다.

</details>

- **인덱스 물리 설계 선택 기준**에 따라 범위•정렬 페이지 축소는 **클러스터드 인덱스**, 재조회 제거는 **커버링 인덱스**를 선택한다.

#### 한줄 요약

- **인덱스 물리 설계 선택 기준**으로 실제 읽기 감소와 쓰기 증가를 함께 확인한다.
