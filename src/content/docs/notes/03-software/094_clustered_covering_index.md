...
- 정의/개념: 행 조직·열 포함으로 **페이지 읽기 감소**
- **배경/필요성**: 범위 접근과 테이블 재조회 비용 축소
...
- **1. 조건·출력 열 전달**: 질의 요구를 보낸다.
- **2. 선택한 인덱스 탐색**: 비용이 낮은 키를 고른다.
- **3. 일치 리프 접근**: 조건 범위를 찾는다.
- **4. 키·포함 열 반환**: 인덱스 값을 읽는다.
- **5. 인덱스 결과 전달**: 커버 여부를 확인한다.
- **6. 부족한 열의 행 조회**: 원본을 추가 접근한다.
- **7. 완성된 결과 전달**: 모든 출력 열을 반환한다.
...
- **범위 읽기·재조회·쓰기 비용**으로 인덱스 방식을 정한다.
