...
- **1. 스냅샷 요청**: 격리 수준에 맞는 기준을 요구한다.
- **2. 읽기 뷰·커밋 경계**: 보이는 범위를 제공한다.
- **3. 새 버전 기록**: 기존 값을 덮지 않고 보존한다.
- **4. 가시 버전 조회**: 읽기 뷰에 맞는 값을 찾는다.
- **5. 스냅샷에 보이는 값**: 현재 또는 과거 값을 반환한다.
- **6. 커밋·충돌 결과**: 쓰기 충돌을 처리한다.
- **7. 정리 지평선·버전 회수**: 불필요 버전을 제거한다.
...
- **읽기 대기·버전 수명·충돌 비용**에 따라 MVCC를 운영한다.
