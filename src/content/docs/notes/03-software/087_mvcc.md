...
- **쓰기 집합•버전 메타데이터**: 트랜잭션이 바꾼 항목과 각 버전의 생성•종료 정보를 묶어 충돌과 커밋 여부를 판정하는 자료이다.
...
- **유휴 제한•읽기 작업 분할**: 사용하지 않는 트랜잭션을 종료하고 긴 조회를 짧은 범위로 나눠 오래된 스냅샷 보유를 줄이는 방법이다.
- **지연•회수량•팽창률**: 정리 작업의 지체 시간과 제거한 버전 수, 전체 저장 공간에서 불필요한 버전이 차지하는 비율이다.
- **조건부 갱신•잠금•재시도**: MVCC가 제거하지 않는 동시 쓰기 충돌과 갱신 손실을 통제하는 수단이다.
- **격리•가시성•정리 구현 확인**: 사용 중인 데이터베이스가 스냅샷을 만들고 버전을 보여 주며 회수하는 실제 규칙을 검증하는 활동이다.
