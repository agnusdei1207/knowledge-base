...
- 정의/개념: 개체를 노드로, 연결을 방향과 속성을 가진 관계로 저장하고 경로를 직접 탐색하는 **Neo4j 그래프 데이터베이스(Neo4j Graph Database)**
...
- **노드•레이블**: 업무 개체와 그 유형을 나타내는 그래프 구성요소이다.
- **관계•방향•유형**: 노드 사이 연결의 의미와 탐색 가능한 방향을 나타내는 구성요소이다.
...
- **경로 깊이•후보 수 상한**: 탐색 단계와 중간 경로 개수를 제한해 경로 폭발을 막는 기준이다.
- **방문•경로 유일성 규칙**: 순환 그래프에서 같은 노드나 관계를 재사용할 수 있는 범위를 정하는 규칙이다.
- **95백분위 응답시간(95th Percentile Latency, p95)•메모리 시험**: 운영 규모의 대부분 요청 지연과 경로 확장 메모리 한계를 확인하는 시험이다.
...
| 소규모 시험은 실제 분기 계수의 지연을 반영하지 못함 | 운영 규모로 **95백분위 응답시간(95th Percentile Latency, p95)•메모리** 시험 | **경로 탐색 한계** 확인 |
...
- 관계 경로가 핵심이면 **Neo4j** 선택, 단순 거래•집계가 핵심이면 **관계형 데이터베이스(Relational Database)** 선택
