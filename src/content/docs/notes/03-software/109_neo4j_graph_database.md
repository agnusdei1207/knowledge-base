...
- 배경/필요성: 소셜망 및 복합 추천에서 RDBMS의 다단계 Recursive JOIN 수행 시 발생하는 **기하급수적 인덱스 탐색 비용 및 성능 급락 해결 불가**

#### 한줄 요약
- 무색인 인접성과 Cypher 질의어를 통해 다단계 복합 관계를 $O(1)$ 속도로 순회한다.
...
|:---|:---|:---|
| **노드 (Node)** | 사용자, 계좌, 상품 등 비즈니스 실체와 **식별자 및 Label 분류 정보 표현** | 그래프 정점(Vertex) |
| **관계 (Relationship)**| 노드 간의 의미 있는 연결을 **방향성(Directed)과 물리적 메모리 포인터로 직접 연결**| $O(1)$ 관계 횡단 지원 |
| **속성 (Property)** | 노드 및 관계에 부착되는 **키-값(Key-Value) 형태의 동적 메타데이터 저장** | 유연한 속성 확장 |
| **레이블 (Label)** | 노드의 역할을 범주화하고 **Cypher 질의 시 출발점 인덱싱 범위를 한정** | 스키마 인덱스 생성 기준 |

#### 한줄 요약
- 노드, 관계, 속성, 레이블의 4대 요소로 현실 세계의 복잡한 연결망을 모델링한다.
...
Cypher 질의 실행 (`MATCH (a:User {name:'Alice'})-[:FRIEND*1..3]->(b:User) RETURN b`)
        │
   1. [시작점 탐색] Label 및 Index를 참조하여 'Alice' 노드를 메모리에서 즉시 룩업 ($O(\log N)$)
        │
   2. [관계 필터링] ':FRIEND' 유형의 아웃바운드 관계 포인터 목록만 추출
        │
   3. [포인터 순회] 인덱스 검색 없이 물리 주소 포인터를 타고 1차 인접 친구 노드로 직접 이동 ($O(1)$)
        │
   4. [Hop 깊이 판정] 지정된 탐색 깊이(최대 3-Hop)까지 재귀 순회 지속 여부 검증
        │
   5. 조건에 부합하는 모든 도달 노드(b) 및 서브그래프 경로를 클라이언트에 즉시 반환
```

#### 한줄 요약
- 출발 노드 검색 → 관계 필터링 → 포인터 순회 → Hop 검증 → 경로 반환 순으로 진행된다.
...
- 이상금융거래 탐지(FDS) 및 AI 지식 그래프(Graph RAG) 구축을 위해 **Neo4j의 무색인 인접성(Index-Free Adjacency)과 Cypher 질의어를 적극 도입**하고, **Super Node 분할 및 인덱스 최적화**를 통해 고성능 그래프 플랫폼 완성
