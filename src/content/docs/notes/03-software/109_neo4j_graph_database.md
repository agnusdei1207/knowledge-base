...
- 정의/개념: 개체와 연결을 저장해 경로를 탐색하는 **Neo4j 그래프 데이터베이스**이다.
- 배경/필요성: 다단계 외래키 조인은 경로 깊이에 따라 조인 비용이 증가한다.
...
- **속성 그래프**: 노드•관계에 업무 의미 부여이다.
- **패턴 질의**: 방향•유형•경로 길이 선언이다.
- 분기 계수•깊이에 따라 **후보 경로 수** 지수 증가한다.
...
| 구성요소 | 책임 |
|:---|:---|
| 노드•레이블 | **노드**•**레이블**로 개체•유형 표현 |
| 관계•방향•유형 | **관계**•**방향**•**유형**으로 연결 의미 표현 |
| 속성 | **속성**에 상태•시간•가중치 저장 |
| 인덱스•제약 | **인덱스**•**제약**으로 시작 노드 탐색•유일성 보장 |
| Cypher 실행기 | **Cypher 실행기**가 패턴 계획•관계 확장•필터 실행 |
...
- **Cypher 패턴**: 레이블•방향•유형•속성으로 원하는 그래프를 선언한 질의이다.
- **시작 노드 인덱스 조회**: 선택도 높은 조건으로 출발 집합을 좁히는 단계이다.
- **출발 노드 식별**: 인덱스 결과에서 관계 확장 시작점을 정하는 단계이다.
- **관계 확장 계획**: 유형•방향•깊이에 따라 탐색 순서를 정하는 단계이다.
- **인접 관계•속성 탐색**: 경로 조건에 맞는 이웃과 속성을 읽는 단계이다.
...
- **1. 시작 노드 인덱스 조회**: **Cypher 패턴**에서 **시작 노드 인덱스 조회**에 선택도 높은 조건을 전달한다.
- **2. 출발 노드 식별**: **출발 노드 식별**로 탐색 시작 집합을 축소한다.
- **3. 관계 확장 계획**: **관계 확장 계획**으로 유형•방향•깊이별 순서를 결정한다.
- **4. 인접 관계•속성 탐색**: **인접 관계•속성 탐색**으로 이웃 조회와 경로를 필터링한다.
...
- **그래프•관계형 모델 선택 기준**: 핵심 질의가 관계 경로인지 정형 거래•집계인지 구분하는 기준이다.
...
- **그래프•관계형 모델 선택 기준**에서 표는 조건으로 다시 결합하고 그래프는 저장된 연결선을 따라간다.
...
| 방향•유형 없는 관계 패턴은 불필요한 이웃 확장 | **관계 방향•유형** 명시 | 관계 확장 범위 감소 |
| 높은 분기 계수의 무제한 경로는 후보 수 폭발 | **경로 깊이 상한**•**후보 수 상한** 설정 | 경로 폭발 방지 |
| 순환 그래프에서 경로 중복을 허용하면 같은 노드 재방문 | **방문 유일성 규칙**•**경로 유일성 규칙** 정의 | 순환 반복 방지 |
| 소규모 시험은 실제 분기 계수의 지연을 반영하지 못함 | 운영 규모로 **95백분위 응답시간**•**메모리 시험** | 경로 탐색 한계 확인 |
...
- **Neo4j 적용 판단 기준**: 관계 경로 탐색의 가치와 경로 폭발 제한 책임을 함께 평가하는 기준이다.

</details>

- **Neo4j 적용 판단 기준**에 따라 관계 경로는 **Neo4j**, 단순 거래•집계는 **관계형 데이터베이스**를 선택한다.

#### 한줄 요약

- **Neo4j 적용 판단 기준**은 연결선이 답일 때 시작점과 탐색 깊이를 제한한다.
