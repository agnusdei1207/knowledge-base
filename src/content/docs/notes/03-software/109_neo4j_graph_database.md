...
- 배경/필요성: 다단계 외래키 조인은 경로 깊이에 따라 **조인 비용**이 증가한다.
...
- **Cypher 패턴**: 레이블•방향•유형•속성으로 원하는 그래프를 선언한 질의다.
- **시작 노드 인덱스 조회**: 선택도 높은 조건으로 출발 집합을 좁히는 단계다.
- **출발 노드 식별**: 인덱스 결과에서 관계 확장 시작점을 정하는 단계다.
- **관계 확장 계획**: 유형•방향•깊이에 따라 탐색 순서를 정하는 단계다.
- **인접 관계•속성 탐색**: 경로 조건에 맞는 이웃과 속성을 읽는 단계다.
...
- **1. 시작 노드 인덱스 조회**: 선택도 높은 조건을 전달한다.
- **2. 출발 노드 식별**: 탐색 시작 집합을 축소한다.
- **3. 관계 확장 계획**: 유형•방향•깊이별 순서를 결정한다.
- **4. 인접 관계•속성 탐색**: 이웃 조회와 경로 필터이다.
...
- 관계 경로는 **Neo4j**, 단순 거래•집계는 **관계형 데이터베이스**를 선택한다.
