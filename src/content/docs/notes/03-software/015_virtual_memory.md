...
- 정의/개념: 물리적 RAM 크기 제약을 극복하고 논리 주소를 물리 주소로 자동 바인딩 매핑하는 매커니즘인 **가상 메모리** 및 구현 기법인 **Paging & Segmentation**
...
| 단편화 문제 | **내부 단편화 ** | **외부 단편화 ** | 내부 단편화 미세 잔류 |
...
- **메모리 압축(Memory Compaction / zRAM)**: 스왑 I/O 지연을 줄이기 위해 사용 빈도가 낮은 페이지를 RAM 내부에서 압축하여 보관하는 기법.
- **휴지 페이지(HugePages)**: 기본 4KB 대신 2MB/1GB 단위의 대형 페이지를 사용하여 TLB 엔트리 커버리지를 극대화하고 TLB 미스를 억제하는 기술.
</details>
