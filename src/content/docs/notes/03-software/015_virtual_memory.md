...
date: "2026-08-05T12:39:45+09:00"
tags:
  - "notes-software"
weight: 15
extra:
...
- **TLB 조회**: 최근 가상•물리 주소 변환과 접근 권한을 캐시에서 찾는 단계
- **페이지 테이블 워크**: TLB 미스 시 계층 페이지 테이블에서 매핑을 탐색하는 단계
- **변환 상태 판정**: 매핑의 상주 여부와 읽기•쓰기•실행 권한을 확인하는 단계
- **물리 메모리 접근**: 변환된 물리 주소로 데이터나 명령을 읽고 쓰는 단계
- **페이지 폴트 처리**: 비상주 페이지를 적재해 매핑을 갱신하거나 권한 위반을 차단하는 단계
...
- **1. TLB 조회**: 캐시된 주소 변환•권한 탐색
- **2. 페이지 테이블 워크**: TLB 미스 시 계층 매핑 조회
- **3. 변환 상태 판정**: 상주 여부•접근 권한 분기
- **4. 물리 메모리 접근**: 변환된 프레임에 연산 수행
- **5. 페이지 폴트 처리**: 페이지 적재•매핑 갱신 후 재실행

#### 한줄 요약

- TLB 미스•페이지 부재에 따른 **변환•적재 흐름**
...
- **내부 단편화(Internal Fragmentation)**: 고정 블록 내부에 남는 사용 불가 공간
- **외부 단편화(External Fragmentation)**: 가변 영역 사이에 흩어진 연속 사용 불가 공간
...
- **교환량(Swap Volume)**: 메모리와 저장장치 사이를 이동한 페이지 양
- **수용 제어(Admission Control)**: 메모리 여유에 따라 새 작업 허용을 정하는 절차
...
- **비연속 할당**: 고정 크기 페이지 매핑을 선택하는 기준
- **논리 영역 보호**: 가변 크기 세그멘테이션을 선택하는 기준
