...
sequenceDiagram
    ...
    participant H as 페이지 폴트 처리기
    participant B as 물리 메모리·후면 저장소

    C->>L: ① 가상 주소·접근 유형 전달
    L->>T: ② TLB 미스·페이지 변환 조회
    T-->>L: ③ 비상주 페이지 항목 반환
    L->>H: ④ 페이지 부재 예외 전달
    H->>B: ⑤ 프레임 확보·페이지 적재 요청
    B-->>H: ⑥ 적재 완료·물리 프레임 반환
    H->>T: ⑦ 프레임·권한·상주 상태 갱신
    H->>L: ⑧ TLB 무효화·새 변환 반영
    H-->>C: ⑨ 중단 명령 재개
...
- ① 프로세서가 가상 주소와 읽기·쓰기·실행 유형 전달
- ② TLB에 변환이 없으면 페이지 테이블 조회
- ③ 페이지 테이블이 비상주 상태 반환
- ④ MMU가 페이지 폴트 처리기에 부재 예외 전달
- ⑤ 빈 프레임을 확보하고 후면 저장소에 페이지 적재 요청
- ⑥ 적재 완료 후 물리 프레임 번호 반환
- ⑦ 페이지 테이블의 프레임·권한·상주 상태 갱신
- ⑧ 오래된 TLB 항목 제거 후 새 변환 반영
- ⑨ 중단되었던 프로세서 명령 재개
- ⑩ 같은 가상 주소로 메모리 접근 재시도
- ⑪ 물리 주소로 변환하고 권한이 맞으면 접근 허용
...
- 데이터베이스 버퍼 풀은 실제 사용률과 회수 비용을 검증한 뒤 대형 페이지로 TLB 한 항목의 변환 범위를 넓힌다.
...
- 메모리 활용률과 논리적 보호를 함께 확보하려면 **단편화 형태·주소 변환 비용·보호 단위**를 검토하고 페이징과 세그멘테이션을 목적에 맞게 선택·조합한다.
