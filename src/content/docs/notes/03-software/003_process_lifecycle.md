...
sequenceDiagram
    ...
    participant Q as PCB•준비 큐
    H->>K: fork 요청
    K->>Q: 1. 신규 PCB•준비 큐 등록
    K->>C: 2. 실행 문맥
    C->>D: 3. I/O 요청
    D-->>K: 4. 완료 이벤트
    K-->>C: 실행 재개
...
1. **신규 PCB•준비 큐 등록**: 자식의 상태 정보를 만들고 실행 가능 상태로 전환
2. **실행 문맥**: 디스패치로 준비 상태에서 실행 상태 전환
3. **I/O 요청**: CPU를 반납하고 장치별 대기 큐로 이동
