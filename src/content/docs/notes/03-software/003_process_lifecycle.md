...
- **포크·엑섹·웨이트 시스템 호출(fork·exec·wait System Calls)**: 각각 ‘포크·엑섹·웨이트’로 읽는 유닉스 계열의 소문자 함수 식별자 관례 표기이며 자식 프로세스 생성·실행 프로그램 교체·종료 상태 회수를 수행한다.
- **운영체제(Operating System, OS, 오에스)**: 영문 각 단어의 첫 글자를 따 OS로 표기하며 프로세스 상태·주소 공간·자원과 큐 전이를 관리하는 소프트웨어
- **입출력(Input/Output, I/O)**: ‘아이오’로 읽고 Input과 Output의 머리글자를 빗금(/)으로 묶는 관례 표기이며 장치 작업을 요청한 프로세스를 완료 이벤트까지 대기 상태로 전환하게 한다.
- **프로세스 제어 블록(Process Control Block, PCB, 피시비)**: 영문 각 단어의 첫 글자를 따 PCB로 표기하며 프로세스 상태·실행 문맥·주소 공간·자원 참조를 저장하는 구조
...
2. 디스크 입출력 중 프로세스를 대기 큐에 보관

### 쉽게 이해하기 (학습용)

- 명령 창은 새 프로그램을 자식으로 시작하고 끝나면 종료 결과를 거둔다
- 디스크 응답을 기다리는 작업은 실행 줄에서 빠졌다가 완료되면 돌아온다
