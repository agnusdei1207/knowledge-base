...
- **Process Lifecycle**: 프로세스가 생성(New)되어 준비(Ready), 실행(Running), 대기(Waiting/Blocked), 종료(Terminated) 상태를 순환 교체하는 OS 상태 제어 모델.
- **fork() / exec()**: 부모 프로세스 주소 공간을 동일 복제(fork) 후 신규 바이너리 바이너리 이미지로 덮어쓰는(exec) POSIX 프로세스 생성 시스템 콜.
- **Zombie Process**: 실행은 완결(exit)되었으나, 부모 프로세스가 `wait()` 시스템 콜을 인가하지 않아 PID 및 PCB 메타데이터가 커널 스택 상에 유휴 상주하는 프로세스.

</details>

- 정의/개념: 프로세스의 생성부터 자원 할당, CPU 선점, I/O 이벤트 대기 및 종료 수거까지의 5대 핵심 상태 수명주기 제어 메커니즘인 **프로세스 생성·종료·상태 전이**
- 배경/필요성: 한정된 CPU/메모리 하드웨어 자원을 무수한 실행 주체들이 충돌 없이 시분할 공유하기 위한 커널 큐(Queue) 관리 요구성

#### 한줄 요약

- CPU 할당과 이벤트 대기에 따라 프로세스 상태가 전이된다.
...
- 실행 가능 여부와 대기 원인으로 준비 큐 또는 대기 큐를 고른다.
