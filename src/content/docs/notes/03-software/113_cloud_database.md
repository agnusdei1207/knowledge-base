...
- **1. 읽기·쓰기 요청**: 엔드포인트로 보낸다.
- **2. 현재 처리 노드 전달**: 정상 노드로 연결한다.
- **3. 변경 데이터 복제**: 다른 영역에 전파한다.
- **4. 복제 확인 반환**: 요구 수준을 확인한다.
- **5. 처리 노드 상태 검사**: 건강 상태를 감시한다.
- **6. 장애 응답 실패**: 전환 조건을 확정한다.
- **7. 복제본 승격**: 새 처리 노드로 바꾼다.
...
- **데이터 모델·확장 단위·RTO·비용**으로 클라우드 DB를 정한다.
