...
- 배경/필요성: 자체 운영은 패치•백업•장애 전환의 **인력•복구 시간**이 증가한다.
...
- **현재 처리 대상으로 라우팅**: 엔드포인트가 요청을 현재 쓰기 대상으로 보내는 단계다.
- **변경 레코드 복제**: 변경 내용과 순서를 다른 가용 영역에 전파하는 단계다.
- **복제 확인**: 설정한 내구성 범위의 변경 반영을 확인하는 단계다.
- **대기 노드 승격**: 관리 계층이 장애 판정 뒤 새 쓰기 대상을 정하는 단계다.
- **엔드포인트 대상 갱신**: 접속 주소를 새 쓰기 대상으로 전환하는 단계다.
...
- **1. 현재 처리 대상으로 라우팅**: 현 쓰기 대상으로 요청을 전달한다.
- **2. 변경 레코드 복제**: 다른 가용 영역에 변경 전파이다.
- **3. 복제 확인**: 설정된 내구성 범위를 확인한다.
- **4. 대기 노드 승격**: 관리 계층이 새 쓰기 대상을 결정한다.
- **5. 엔드포인트 대상 갱신**: 접속 주소의 대상을 전환한다.
...
> 요약: Aurora는 RDS 제품군이지만 **저장 구조•확장 단위**가 다르다.
...
- 엔진 호환은 **RDS**, 분리형 확장은 **Aurora**, 키 접근은 **DynamoDB**가 핵심이다.
