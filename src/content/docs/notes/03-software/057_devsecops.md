...
- **1. 코드·의존성 변경**: 개발자가 검사선에 제출
- **2. 자동 검사·정책 결과**: SAST·SCA·비밀 검사 수행
- **3. 승인 산출물 배포**: 기준 충족 버전 운영 반영
- **4. 운영 보안 지표**: 취약 행위·공격 징후 수집
- **5. 운영 위험 피드백**: 다음 개발 변경에 연결
- **6. 예외 검토 요청**: 즉시 수정 불가 위험 제출
- **7. 차단·예외 결정**: 보완책·책임자·만료일 확정
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 검사 과잉 | 위험별 검사·증분 분석·오탐 기준선 | 우회·지연 감소 |
| 도구 분절 | 공통 위험 식별자·소유자·추적 통합 | 책임 명확화 |
| 무기한 예외 | 보완 통제·책임자·만료일 필수화 | 잔존 위험 회수 |
| 공급망 위험 | SBOM·서명·출처 검증·신뢰 저장소 | 위변조 방지 |
| 운영 피드백 단절 | 경보를 소스 변경·소유자와 연결 | 런타임 위험 개선 |

> 적용 사례: 병합 요청에서 SAST·비밀정보 검사 위반 시 기준선 병합을 차단
...
- **변경 위험·운영 노출**이 크면 DevSecOps 통제를 강화
