...
- 정의/개념: **PVC•PV•StorageClass**로 요구와 구현을 분리한 저장 구조이다.
- 배경/필요성: 파드 로컬 저장은 재생성•노드 장애 시 **데이터 소실**이 발생한다.
...
- **PVC•PV 분리** 기반 요구 추상화가 핵심이다.
- **StorageClass•CSI** 기반 동적을 생성한다.
- **회수 정책** 기반 삭제 후를 보존한다.
...
- **1. PVC 저장 조건 확인**: 용량•접근 모드•클래스 일치 검사이다.
- **2. 클래스•토폴로지 선택**: 프로비저너와 배치 영역 확정이다.
- **3. 백엔드 볼륨 동적 생성**: 접근 가능한 실제 저장소를 생성한다.
- **4. PV 생성•PVC 바인딩**: 볼륨과 요청의 일대일 연결을 기록한다.
- **5. 노드 연결•파드 마운트**: 파드 경로에 파일시스템 게시이다.
...
- **접근 방식**으로 StorageClass를, 데이터 중요도•**복구 목표**로 회수 정책•백업을 결정한다.
