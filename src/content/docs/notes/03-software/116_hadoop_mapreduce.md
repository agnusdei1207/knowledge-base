...
- **키 편향(Key Skew)**: 일부 키에 값이 몰려 특정 Reduce 작업의 완료가 늦어지는 현상임
...
- 정의/개념: HDFS·YARN·MapReduce로 **대용량 배치 처리**
...
- **1. 작업·경로 제출**: 배치 작업을 보낸다.
- **2. 블록 위치 조회**: 입력 배치를 찾는다.
- **3. 입력 분할 반환**: Map 범위를 정한다.
- **4. Map 실행**: 중간 키값을 만든다.
- **5. 중간 키값 Shuffle**: 같은 키를 모은다.
- **6. 집계 결과 기록**: Reduce 결과를 저장한다.
- **7. 결과 위치 반환**: 완료 위치를 알린다.
...
- **파일 수·키 편향·셔플량·반복 횟수**로 MapReduce 적용을 정한다.
