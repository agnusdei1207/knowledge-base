...
- 배경/필요성: 페타바이트급 데이터를 한 서버에 모으면 용량도 모자라지만 무엇보다 연산할 때마다 데이터를 네트워크로 끌어오는 비용이 데이터량에 비례해 반복되므로, 데이터를 블록으로 나눠 여러 노드에 복제해 두고 연산 코드를 데이터가 있는 노드로 보내 이동 방향 자체를 뒤집을 필요
