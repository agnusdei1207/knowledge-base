...
- 배경/필요성: 페타바이트(PB) 규모의 대용량 빅데이터를 단일 중앙 서버나 고비용 스토리지(SAN/NAS)에 저장하고 처리할 때 발생하는 네트워크 대역폭 고갈, 디스크 I/O 병목 및 고비용 인프라 한계를 극복하고, 저비용 범용 x86 서버 클러스터 상에서 HDFS(Hadoop Distributed File System) 기반의 블록 분할·복제와 연산 코드를 데이터가 위치한 노드로 직접 전송하여 실행하는 데이터 지역성(Data Locality) 중심의 MapReduce/YARN 병렬 배치 처리를 도입함으로써 **선형적인 스토리지/컴퓨팅 수평 확장성과 노드 장애 시 무중단 결함 허용성(Fault Tolerance)을 달성**할 필요
