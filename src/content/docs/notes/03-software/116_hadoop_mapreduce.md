...
- **Hadoop MapReduce 분산 배치 체계**: 하둡 분산 파일 시스템(Hadoop Distributed File System, HDFS) 저장과 또 다른 자원 협상자(Yet Another Resource Negotiator, YARN) 자원 관리 및 MapReduce 계산을 결합한 대규모 일괄 처리 체계이다.
...
- **분산 저장**: 하둡 분산 파일 시스템(Hadoop Distributed File System, HDFS)이 파일을 블록으로 나눠 여러 노드에 복제하는 방식이다.
- **키 처리**: Map이 중간 키값을 만들고 Shuffle이 같은 키를 모으며 Reduce가 최종 집계하는 흐름이다.

</details>

- **분산 저장**: 하둡 분산 파일 시스템(Hadoop Distributed File System, HDFS) 블록 분할·복제
...
- **Shuffle·Sort·Reduce**: 중간 키값을 그룹화하고 정렬해 키별 최종 결과를 집계하는 구성요소이다.
- **하둡 분산 파일 시스템(Hadoop Distributed File System, HDFS)**: 입력 블록과 최종 결과를 여러 노드에 분산·복제해 저장하는 파일 시스템이다.
- **또 다른 자원 협상자(Yet Another Resource Negotiator, YARN)**: 컨테이너 자원을 할당하고 태스크 실행 상태를 관리하는 자원 관리자이다.
- **입력 분할(Input Split)**: 하나의 Map 태스크가 논리적으로 처리할 입력 레코드 범위이다.
- **Map·Combiner**: 입력을 중간 키값으로 변환하고 같은 태스크 안에서 부분 집계하는 단계이다.
...
- **하둡 분산 파일 시스템(Hadoop Distributed File System, HDFS)**: 입력 파일 블록과 최종 집계 결과를 저장하는 분산 파일 시스템이다.
- **또 다른 자원 협상자(Yet Another Resource Negotiator, YARN)**: 작업을 제출받아 입력 지역성에 맞는 컨테이너와 태스크를 배치하는 자원 관리자이다.
...
| 핵심 특징 | **디스크 기반 단계 처리** | **메모리 캐시·방향성 비순환 그래프(Directed Acyclic Graph, DAG)** | **로컬 파일·메모리** |
...
- **병합·컨테이너 형식**: 작은 파일을 큰 파일이나 다중 레코드 컨테이너로 묶어 메타데이터와 태스크 시작 비용을 줄이는 방식이다.
- **샘플링·키 분할**: 키 분포를 미리 측정하고 편향된 키에 보조 값을 붙여 여러 Reduce로 나누는 방식이다.
- **Combiner·압축·작업 수 조정**: Map 쪽 부분 집계와 중간 데이터 압축 및 병렬 태스크 수로 셔플 부하를 줄이는 통제이다.
- **블록 배치·자원 균형**: 입력 블록이 있는 노드 가까이에 태스크 자원을 배치해 원격 전송을 줄이는 방식이다.
- **멱등 출력·임시 경로**: 실패 태스크를 재실행해도 중복 결과가 생기지 않도록 임시 출력 후 한 번만 확정하는 방식이다.
