...
sequenceDiagram
    ...
    개발자->>포털: 1. 표준 경로 탐색
    개발자->>제어평면: 2. 자원 요청 제출
    제어평면->>정책엔진: 3. 정책 자동 검증
    제어평면->>자원: 4. 자원 자동 생성
    제어평면->>포털: 5. 상태 카탈로그 등록

```

**동작 원리**

1. **표준 경로 탐색**: 카탈로그에서 목적별 구성을 선택
2. **자원 요청 제출**: 필요한 값만 입력해 생성을 요청
3. **정책 자동 검증**: 권한·보안·비용 한도를 판정
4. **자원 자동 생성**: 승인된 규격대로 환경을 구성
5. **상태 카탈로그 등록**: 소유권·접속 정보·상태를 기록
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| Golden Path의 **경직성** | 기본 경로·**확장 지점 분리** | 자율성·표준 **균형** |
...
- **개발자 마찰·정책 준수·플랫폼 운영성**으로 셀프서비스 결정
