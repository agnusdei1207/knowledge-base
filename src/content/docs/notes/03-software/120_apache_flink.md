...
- **1. 체크포인트 시작 요청**: 복구 촬영을 시작한다.
- **2. 위치 기록·Barrier 전달**: 입력 경계를 표시한다.
- **3. 경계 상태 스냅숏**: 키 상태를 저장한다.
- **4. 상태 저장 완료**: 영속 결과를 확인한다.
- **5. 결과 사전 반영**: Sink에 경계를 전달한다.
- **6. 체크포인트 확인**: 모든 경로를 확인한다.
- **7. 외부 커밋 통보**: 결과를 확정한다.
...
- **지연 분포·상태 크기·역압력·복구 비용**으로 Flink를 설계한다.
