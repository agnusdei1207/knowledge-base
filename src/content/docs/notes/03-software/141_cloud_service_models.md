...
- 배경/필요성: 온프레미스 데이터센터 환경에서 하드웨어 인프라 조달, OS 설치, 미들웨어 튜닝 및 애플리케이션 운영까지 전 계층을 직접 감당함에 따라 발생하는 막대한 초기 투자 비용(CapEx), 긴 프로비저닝 리드타임 및 운영 오버헤드를 해소하기 위해, IT 자원의 제어 범위와 서비스 수준에 따라 인프라 제공(IaaS), 개발 플랫폼 제공(PaaS), 완성형 소프트웨어 구독(SaaS)으로 역할을 계층화하고 CSP와 고객 간의 공동 책임 모델(Shared Responsibility)을 확립하여 **비즈니스 환경에 최적화된 운영 민첩성과 비용 효율성을 달성**할 필요
...
- 엔터프라이즈 디지털 전환 및 클라우드 마이그레이션 전략의 **가장 기본적이고 핵심적인 서비스 분류 체계**로 정립되었으며, 현대 아키텍처 환경에서는 **OS 커널 튜닝 및 레거시 워크로드에는 IaaS(IaC 자동화 연계), MSA 백엔드 및 신속한 기능 출시에는 PaaS/CaaS(컨테이너 표준화), 범용 업무 생산성에는 SaaS(CASB/DLP 보안 통제 연계)를 전략적으로 혼합 배치하고, 명확한 공동 책임 모델(Shared Responsibility) 기반의 보안·백업 거버넌스**를 결합하여 거버넌스와 클라우드 네이티브 생산성을 극대화
