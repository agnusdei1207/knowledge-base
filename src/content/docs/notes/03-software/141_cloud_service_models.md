...
- 정의/개념: **클라우드 서비스 모델**은 인프라부터 애플리케이션까지 공급자와 소비자의 운영 책임 범위에 따라 IaaS•PaaS•SaaS를 구분하는 **서비스 분류 체계**이다.
- 배경/필요성: 서비스 계층별 담당 미구분 시 **운영•보안 책임 공백**이 발생한다.
...
- **책임 이전**: IaaS→PaaS→SaaS로 공급자 운영 범위가 확대한다.
- **제어•책임 연동**: 소비자 제어가 클수록 운영 책임도 증가한다.
- **공통 책임**: 계정•권한•데이터•안전한 설정은 소비자가 담당이다.
...
- **제어 수준•운영 역량**에 따라 IaaS•PaaS•SaaS을 선택한다.
