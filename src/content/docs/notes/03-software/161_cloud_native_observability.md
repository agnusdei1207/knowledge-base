...
- **추적 식별자(Trace Identifier, Trace ID)•리소스 속성**: 서로 다른 신호를 같은 요청과 서비스•버전•실행 환경으로 연결하는 상관 정보이다.
- **카디널리티•샘플링**: 속성값 조합의 수와 전체 신호 중 저장할 일부를 선택하는 비율로 관측 비용을 통제하는 기준이다.
...
- **분석•경보 계층**: 신호를 연결해 사용자 영향과 장애 원인을 분석하고 경보를 생성하는 구성요소이다.
