...
- 정의/개념: **메트릭(Metric)•로그(Log)•추적(Trace)** 신호를 연결해 내부 상태와 원인을 추론하는 성질이다.
- 배경/필요성: 개별 신호만으로는 분산 요청의 **경로•장애 원인을 연결하기 어려움**이 발생한다.
...
- **SLI•진단 질문** 기반 목적 중심 계측이 핵심이다.
- **Trace ID•리소스 속성** 기반 신호를 연결한다.
- **카디널리티•샘플링** 기반 비용을 통제한다.
...
- **1. 추적 문맥 전파**: 하위 호출을 같은 Trace ID로 연결한다.
- **2. 메트릭•로그•스팬 생성**: 구간 지연•오류•속성을 기록한다.
- **3. 신호 정규화•필터•샘플링**: 목적별 신호를 처리한다.
- **4. 상관 키 기반 분석•경보**: 사용자 영향과 원인을 연결한다.
...
- **사용자 영향•진단 질문•신호 비용**으로 계측•연결•보존 범위를 결정한다.
