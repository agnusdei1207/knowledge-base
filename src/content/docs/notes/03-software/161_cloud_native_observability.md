...
- **정의/개념**: 메트릭·로그·트레이스를 연결해 내부 원인 추론
- **배경/필요성**: 분산 요청의 장애 위치 불명으로 상관 분석 필요
...
| 비교축 | 메트릭 | 로그 | 트레이스 |
|:---|:---|:---|:---|
| 핵심 특징 | 구간 집계와 시계열 수치를 기록함 | 시점별 사건 상세 레코드를 기록함 | 요청별 호출 관계와 구조를 기록함 |
| 적용 기준 | 시스템 건강 상태 추세를 모니터링 시 | 에러 스택을 통해 상세 원인 분석 시 | 분산 환경의 트랜잭션 병목 파악 시 |
| 주요 위험 | 고카디널리티·집계 왜곡 | 저장 비용·민감정보 노출 | 샘플링 누락·추적 전파 단절 |
