---
sidebar:
  order: 161
  label: "161. 클라우드 네이티브 관측성 (Cloud Native Observability)"
  badge:
    text: "기출 · 70%"
    variant: note
title: "클라우드 네이티브 관측성 (Cloud Native Observability)"
date: "2026-07-30T20:00:00+09:00"
tags:
  - "notes-software"
weight: 161
extra:
  question_no: "161"
  source_status: "기출"
  source_history: "135회"
  priority: 70
  priority_note: "로그·지표·추적의 연결 구조 출제"
---

## 미리 알고가기

- **관측성(Observability)**: 외부 신호를 질의해 시스템 내부 상태와 알려지지 않은 문제 원인을 추론하는 성질
- **메트릭(Metric)**: 일정 구간의 상태를 수치와 시간으로 집계한 시계열 신호
- **로그(Log)**: 특정 시점의 사건과 처리 문맥을 기록한 신호
- **트레이스·스팬(Trace·Span)**: 트레이스는 한 요청의 전체 호출 경로, 스팬은 경로 안의 개별 작업 구간
- **트레이스 식별자(Trace ID)**: 여러 서비스의 스팬과 로그를 같은 요청으로 연결하는 값
- **서비스 수준 지표(Service Level Indicator, SLI)**: 가용성·지연처럼 사용자 관점에서 측정한 실제 서비스 성능
- **서비스 수준 목표(Service Level Objective, SLO)**: 일정 기간에 SLI가 달성해야 할 목표
- **리소스 속성(Resource Attribute)**: 서비스·파드·노드·클러스터처럼 신호 생성 대상을 식별하는 정보
- **카디널리티(Cardinality)**: 속성값 조합의 수로 저장·조회 비용과 신호 상세도를 좌우하는 값
- **샘플링(Sampling)**: 전체 사건 중 일부 트레이스·로그를 선택해 저장·분석하는 처리
- **문맥 전파(Context Propagation)**: 트레이스 식별자와 요청 정보를 다음 서비스 호출에 이어 보내는 동작
- **꼬리 기반 샘플링(Tail-based Sampling)**: 요청 완료 후 오류·지연 등 전체 결과를 보고 보존 여부를 정하는 방식

## Ⅰ. 개요

- 정의/개념: Metric·Log·Trace 기반 **상태·원인 추론**
- 배경/필요성: 분산·단기 실행 환경의 **요청 경로·장애 원인 연결**

### 쉽게 이해하기 (학습용)

- 결제 지연 그래프에서 느린 요청 하나를 골라 호출 경로와 같은 식별자의 오류 기록을 따라가면 어느 서비스에서 왜 늦어졌는지 좁힐 수 있다.

## Ⅱ. 특징

- **SLI·진단 질문** 기반 목적 중심 계측
- **Trace ID·리소스 속성** 기반 신호 연결
- **카디널리티·샘플링** 기반 비용 통제

### 쉽게 이해하기 (학습용)

- 모든 기록을 모으는 대신 사용자가 겪은 실패를 어떤 신호로 찾고 어떤 공통 키로 원인까지 이동할지 먼저 정하는 방식이다.

## Ⅲ. 구조 및 구성요소

```mermaid
block-beta
    columns 1
    A["계측 지점"]
    B["문맥 전파"]
    C["수집·처리 계층"]
    D["신호 저장소"]
    E["분석·경보 계층"]
    A --- B
    A --- C
    C --- D
    D --- E
```

| 구성요소 | 책임 |
|:---|:---|
| 계측 지점 | **Metric·Log·Trace** 생성 |
| 문맥 전파 | **요청 식별자·속성** 연결 |
| 수집·처리 계층 | **정규화·필터·샘플링** |
| 신호 저장소 | 신호별 **보존·상관 조회** |
| 분석·경보 계층 | **사용자 영향·원인** 분석 |

### 쉽게 이해하기 (학습용)

- 각 서비스가 단서를 만들고 문맥 전파가 같은 사건 번호를 붙이면 수집기와 저장소를 거쳐 분석 화면에서 하나의 장애 이야기로 재구성된다.

## Ⅳ. 흐름도

```mermaid
sequenceDiagram
    participant U as 사용자
    participant A as 진입 서비스
    participant D as 하위 서비스
    participant C as 수집 계층
    participant S as 분석 계층
    U->>A: 1. 업무 요청 전달
    A->>D: 2. 추적 문맥 전파
    A->>C: 3. 신호·속성 전송
    D->>C: 4. 하위 신호 전송
    C->>S: 5. 정규화·상관 저장
    S-->>U: 사용자 영향 경보
```

**동작 원리**

1. **업무 요청 전달**: 진입 구간의 추적 식별자 생성
2. **추적 문맥 전파**: 하위 호출의 동일 요청 연결
3. **신호·속성 전송**: 진입 지표·로그·스팬 수집
4. **하위 신호 전송**: 구간 지연·오류·버전 수집
5. **정규화·상관 저장**: 시간·서비스·식별자 기반 연결

### 쉽게 이해하기 (학습용)

- 주문 요청이 결제와 재고를 거치는 동안 같은 추적 식별자를 전달하면 세 서비스의 지연과 오류가 한 호출 경로로 묶인다.

## Ⅴ. 종류 및 비교

| 관측 신호 | Metric | Log | Trace |
|:---|:---|:---|:---|
| 적용 기준 | **추세·경보·목표 측정** | **사건·실행 문맥 확인** | **분산 호출·구간 분석** |
| 핵심 특징 | 시간·수치·**제한 속성** | 시각·메시지·**구조 필드** | **부모·자식 스팬 경로** |
| 한계 | **고카디널리티 비용** | **민감정보·저장 비용** | **문맥 누락·샘플 편향** |

### 쉽게 이해하기 (학습용)

- Metric으로 언제 나빠졌는지 찾고 Trace로 느린 구간을 고른 뒤 Log에서 그 시점의 오류와 입력 문맥을 확인한다.

## Ⅵ. 실무 고려사항 및 대책

| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| **내부 지표만 사용** | 성공률·지연 **SLI 정의** | 사용자 영향 **중심 경보** |
| 비동기 호출 **추적 단절** | 메시지 속성에 **문맥 전파** | 서비스 간 **경로 연결** |
| 식별자 **속성 폭증** | 제한 속성·**로그 조회 분리** | **메트릭 비용 통제** |
| 희귀 오류 **샘플 탈락** | 꼬리 기반·**오류 우선 보존** | 장애 원인 **단서 확보** |
| 로그의 **개인정보 노출** | 수집 전 제거·**접근·보존 통제** | 정보 **노출 범위 축소** |

### 쉽게 이해하기 (학습용)

- 사용자 식별자를 Metric 속성으로 모두 넣지 말고 오류 요청의 Trace와 Log에서만 찾도록 나누면 경보 비용과 진단 단서를 함께 관리할 수 있다.

## Ⅶ. 결론

- **사용자 영향·진단 질문·신호 비용**으로 계측·연결·보존 범위 결정

### 쉽게 이해하기 (학습용)

- SLO 이상에서 대표 Trace와 같은 식별자의 Log까지 이동할 수 있는 신호만 남기고 원인 분석에 쓰이지 않는 수집량은 줄여야 한다.
