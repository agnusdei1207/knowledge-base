...
- 정의/개념: 메트릭•로그•추적 신호를 연결해 내부 상태와 원인을 추론하는 **클라우드 네이티브 관측성(Cloud Native Observability)**이다.
- 배경/필요성: 개별 신호만으로는 분산 요청의 경로와 장애 원인을 연결하기 어렵다.
...
- **서비스 수준 지표(Service Level Indicator, SLI)** 기반 목적 중심 계측이 핵심이다.
- **추적 식별자(Trace Identifier, Trace ID)**와 **리소스 속성** 기반으로 신호를 연결한다.
- **카디널리티**와 **샘플링** 기반으로 비용을 통제한다.
...
- **수집 계층**: 관측 신호를 받아 처리 계층으로 전달하는 계층이다.
...
- **분석 계층**: 신호를 연결해 사용자 영향과 장애 원인을 분석하는 계층이다.
- **경보 계층**: 분석 조건을 만족한 장애 알림을 생성하는 계층이다.
...
| 구성요소 | 책임 |
|:---|:---|
| 계측 지점 | **계측 지점**이 메트릭•로그•추적을 생성 |
| 문맥 전파 | **문맥 전파**가 요청 식별자•속성을 연결 |
| 수집•처리 계층 | **수집 계층**이 신호를 받고 **처리 계층**이 정규화•필터•샘플링 |
| 신호 저장소 | **신호 저장소**가 신호를 보존하고 상관 조회를 제공 |
| 분석•경보 계층 | **분석 계층**이 사용자 영향•원인을 분석하고 **경보 계층**이 알림을 생성 |
...
- **추적 문맥 전파**: 하위 호출에 같은 Trace ID를 전달해 하나의 요청으로 연결하는 단계이다.
- **메트릭•로그•스팬 생성**: 각 호출 구간의 지연•오류•서비스 속성을 기록하는 단계이다.
- **신호 정규화•필터•샘플링**: 비용과 진단 목적에 맞춰 신호를 처리하는 단계이다.
- **상관 키 기반 분석•경보**: 시간•서비스•Trace ID로 사용자 영향과 원인을 연결하는 단계이다.
...
4. 상관 키 기반 분석•경보
       |
       +-- SLI 정상 --> 추세 관찰
       +-- SLI 이상 --> 대표 Trace•Log로 원인 추적
```

### 동작 원리

- **1. 추적 문맥 전파**: **추적 문맥 전파**로 하위 호출을 같은 Trace ID에 연결한다.
- **2. 메트릭•로그•스팬 생성**: **메트릭•로그•스팬 생성**으로 구간 지연•오류•속성을 기록한다.
- **3. 신호 정규화•필터•샘플링**: **신호 정규화•필터•샘플링**으로 목적별 신호를 처리한다.
- **4. 상관 키 기반 분석•경보**: **상관 키 기반 분석•경보**로 사용자 영향과 원인을 연결한다.
...
| 관측 신호 | **메트릭(Metric)** | **로그(Log)** | **추적(Trace)** |
|:---|:---|:---|:---|
...
- **추적 단절**: 호출 경계에서 문맥이 이어지지 않아 하나의 요청 경로가 여러 조각으로 분리되는 문제이다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 내부 지표만 사용 | 성공률•지연 **서비스 수준 목표(Service Level Objective, SLO)** 정의 | 사용자 영향 중심 경보 |
| 비동기 호출 **추적 단절** | 메시지 속성에 문맥 전파 | 서비스 간 경로 연결 |
| 식별자 속성 폭증 | 제한 속성•로그 조회 분리 | 메트릭 비용 통제 |
| 희귀 오류 샘플 탈락 | **꼬리 기반 샘플링**•오류 우선 보존 | 장애 원인 단서 확보 |
...
- **진단 질문**과 **신호 비용**으로 계측•연결•보존 범위를 결정한다.
