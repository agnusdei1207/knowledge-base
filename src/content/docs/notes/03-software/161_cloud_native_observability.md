...
- 배경/필요성: 수백 개 이상의 마이크로서비스(MSA), 컨테이너 및 멀티 클라우드로 파편화된 분산 아키텍처에서 전통적인 사일로화된 서버 모니터링 방식으로는 서비스 간 연쇄 장애의 근본 원인과 미지의 결함(Unknown Unknowns)을 규명하지 못해 평균 복구 시간(MTTR)이 급증하는 문제를 해결하기 위해, 메트릭(Metrics), 로그(Logs), 분산 추적(Traces)의 3대 관측성 기둥(3 Pillars)을 W3C Trace ID 기반으로 유기적으로 상관 연계하고 OpenTelemetry 표준 수집 파이프라인을 구축하여 **분산 시스템 내부 상태에 대한 완전한 가시성과 실시간 장애 진단 능력을 달성**할 필요
...
- 복잡 분산 마이크로서비스 및 클라우드 네이티브 환경에서 SRE(사이트 신뢰성 공학)와 시스템 고가용성을 지탱하는 **가장 핵심적인 텔레메트리 관측 및 진단 표준**으로 정립되었으며, 실무 구축 시에는 **벤더 종속을 탈피하는 CNCF OpenTelemetry(OTel) 단일 계측 계측기 표준화, 스토리지 비용 폭증을 방어하는 Tail-based 샘플링(에러/지연 100% 선별 수집), 메트릭 고유값 폭발을 막는 카디널리티 거버넌스 및 Kafka 메시징 전 구간 W3C Context 전파**를 결합하여 비용 효율적인 엔드투엔드 관측성을 완성
