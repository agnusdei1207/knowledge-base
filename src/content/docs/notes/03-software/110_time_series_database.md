...
- 배경/필요성: 스마트 팩토리, IoT 센서, 클라우드 인프라 모니터링(APM)에서 발생하는 대규모 연속 시계열 데이터를 범용 RDBMS에 저장할 때 발생하는 무거운 B+Tree 인덱스 갱신 부하, 디스크 용량 급증 및 대량 이력 `DELETE` 시의 I/O 병목을 극복하고, 타임스탬프를 1차 기준으로 삼는 Append-Only 쓰기, Gorilla 전용 델타 압축(10~20배 용량 절감), 시간 청크 파티셔닝 및 자동 다운샘플링·보존 정책(Retention)을 제공하는 TSDB를 도입하여 **초당 수십만 건의 고속 데이터 적재와 대규모 시간 범위 집계 성능을 극대화**할 필요
...
- 현대 옵저버빌리티(Prometheus/InfluxDB) 및 산업용 IoT 빅데이터 플랫폼의 **핵심 특화 시계열 영속성 스토리지**로 확립되었으며, 실무 구축 시에는 **인덱스 메모리 고갈(Cardinality Explosion)을 방지하기 위한 Tag 키 설계(고선택성 ID는 Field 격리), 장기 스토리지 비용을 90% 이상 절감하는 연속 다운샘플링(Downsampling) 및 자동 보존 만료(Retention) 정책, 수집 피크 트래픽을 완충하는 Kafka/Vector 큐잉 연계**를 결합하여 시스템 확장성과 쿼리 응답성을 동시 보증
