...
- 배경/필요성: 측정값은 시간 순으로 덧붙기만 하고 대개 최근 구간만 조회되는데 범용 RDBMS는 임의 갱신을 전제로 인덱스와 페이지를 유지하느라 적재마다 불필요한 비용을 치르므로, 시간을 1차 축으로 삼아 파티션·압축·보존을 모두 시간 단위 연산으로 처리하는 특화 저장 계층이 필요
