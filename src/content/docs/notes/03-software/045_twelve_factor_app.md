...
- 배경/필요성: 전통적인 레거시 애플리케이션의 소스 코드 내 환경 설정 하드코딩, 로컬 디스크/인메모리 세션 상태 종속 및 불투명한 릴리즈 파이프라인이 유발하는 클라우드 환경 배포 실패와 수평 확장(Scale-out) 한계를 극복하고, 설정의 환경변수 분리(Config), 무상태 프로세스(Stateless), 빌드-릴리즈-실행 분리 및 표준 스트림 로그 배출 등 12대 핵심 원칙을 통해 **컨테이너 기반 클라우드 네이티브 환경에서의 완벽한 이식성, 탄력적 확장성 및 무중단 배포를 실현**할 필요
...
- 현대 컨테이너(Docker/K8s) 및 서버리스 마이크로서비스 설계의 **가장 근본적인 클라우드 네이티브 아키텍처 원칙**으로 확립되었으며, 실무 엔터프라이즈 운영에서는 **K8s Secret/ConfigMap 기반 설정 외부 주입, Redis/DB를 통한 세션 및 상태 외장화, Fluentd/Loki 로그 스트림 수집, 배포 중 요청 유실을 차단하는 Graceful Shutdown(`SIGTERM`)**을 철저히 준수하여 회복탄력성을 극대화
