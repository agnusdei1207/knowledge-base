...
- **정의/개념**: 배포·확장·교체를 위한 열두 운영 원칙
- **배경/필요성**: 설정·상태 결합이 배포를 막아 환경 분리 필요
...
2. 동일 이미지는 환경별 설정 주입 후 승격

### 쉽게 이해하기 (학습용)

- 컨테이너가 로그인 상태를 품지 않으면 같은 프로세스를 여러 개 띄워 요청을 나눌 수 있다
- 개발에서 검증한 이미지는 바꾸지 않고 운영 주소와 자격 증명만 실행할 때 넣는다

## Ⅶ. 결론

- 반복 배포·수평 확장 전 로컬 상태·설정을 외부화
