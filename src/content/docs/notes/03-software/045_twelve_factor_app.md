...
- **프로세스•동시성(Process•Concurrency)**: 기능별 무상태 프로세스 유형을 정의하고 필요한 유형의 인스턴스 수를 늘려 부하를 처리하는 원칙이다.
- **폐기 가능성•관리 작업(Disposability•Admin Processes)**: 프로세스를 빠르고 안전하게 시작•종료하며 일회성 운영 작업을 같은 릴리스 환경에서 실행하는 원칙이다.
...
- **비밀 저장소•노출 검사(Secret Store•Exposure Scan)**: 자격 증명을 암호화해 별도 관리하고 코드•이미지•로그에 비밀값이 포함됐는지 검사하는 통제다.
