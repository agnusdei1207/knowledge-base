...
- 정의/개념: 워크로드의 업무 가치•기술 상태•이전 제약에 따라 이전•현대화•잔존 방식을 선택하는 **클라우드 마이그레이션 6R(Cloud Migration 6R)** 포트폴리오 분류 체계이다.
- 배경/필요성: 일괄 이전은 저가치 자산의 과잉 변경과 기술 부채 잔존을 유발한다.
...
- **이전 기간**: 워크로드 전환과 안정화에 필요한 시간이다.
- **현대화 편익**: 구조 변경으로 얻는 장기 운영 개선 효과이다.

</details>

- 업무 가치•기술 상태를 함께 보는 **포트폴리오 판단**이 핵심이다.
- 변경 범위 확대에 따라 **이전 기간**과 **현대화 편익**이 함께 증가한다.
- 호출•데이터 관계를 보존하는 **의존성 웨이브**가 핵심이다.
...
- **검증 계획**: 전환 성공과 중단을 판정할 조건이다.
- **롤백 계획**: 실패 시 원 환경으로 복귀할 조건과 절차이다.
...
선의 의미: 자산 목록에서 갈라지는 선은 시스템별 가치•비용•수명 정보와 호출•데이터•신원 관계 및 6R 평가 기준의 연결 관계, 의존성 지도와 평가 모델에서 이전 계획으로 모이는 선은 전략별 웨이브 구성 관계, 이전 계획과 검증•롤백 계획의 선은 성공•중단•복귀 판정 조건 결합 관계를 뜻한다.

| 구성요소 | 책임 |
|:---|:---|
| 자산 목록 | **자산 목록**이 시스템별 가치•비용•수명 식별 |
| 의존성 지도 | **의존성 지도**가 호출•데이터•신원의 연결 관계 식별 |
| 평가 모델 | **평가 모델**이 가치•기술 상태•중단 제약 평가 |
| 이전 계획 | **이전 계획**이 전략별 웨이브•선후 관계 관리 |
| 검증•롤백 계획 | **검증 계획**과 **롤백 계획**이 성공•중단•복귀 조건 제시 |
...
- **업무 가치•잔존 제약**: 시스템을 폐기하거나 기존 환경에 유지할지 판단하는 축이다.
- **철수 기한•변경 허용도**: 재호스팅과 재플랫폼화의 변경 범위를 고르는 축이다.
- **표준 제품 대체 가능성**: 기존 기능을 서비스형 소프트웨어로 재구매할지 판단하는 축이다.
- **장기 현대화 편익**: 구조와 코드를 재설계할 투자 가치를 판단하는 축이다.
- **의존성 웨이브•롤백**: 함께 전환할 묶음과 실패 시 복귀 조건을 정하는 통제이다.
...
- **업무 가치•잔존 제약**으로 유지•폐기를 먼저 판단하고 **의존성 웨이브•롤백**으로 함께 전환할 범위와 복귀 조건을 고정한다.
...
- **유지(Retaining)**: 제약 때문에 기존 환경에 남기는 전략이다.
- **폐기(Retiring)**: 가치가 낮은 시스템의 사용을 종료하는 전략이다.

</details>

| 이전•현대화 전략 | 재호스팅 | 재플랫폼화 | 재구매 | 재설계 |
|:---|:---|:---|:---|:---|
| 적용 기준 | **재호스팅**: **철수 기한•변경 허용도** 중 기한 우선 | **재플랫폼화**: 제한 변경으로 운영 개선 | **재구매**: **표준 제품 대체 가능성** 충족 | **재설계**: **장기 현대화 편익** 우선 |
| 핵심 특징 | 구조 유지•인프라 이전 | 일부 관리형 전환 | 서비스형 소프트웨어•새 제품 대체 | 구조•코드 재설계 |
| 한계 | 기술 부채 잔존 | 기존•관리형 혼합 제약 | 기능•데이터 전환 비용 | 기간•비용의 실패 위험 |

| 잔존 전략 | 유지 | 폐기 |
|:---|:---|:---|
| 적용 기준 | **유지**: 규제•의존성의 이전 제약 | **폐기**: 저가치•중복 시스템 종료 |
| 핵심 특징 | 기존 환경 운영 지속 | 서비스•자원 사용 중단 |
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 비용에는 있으나 목록에 없는 **숨은 자산** | 자동 발견•인터뷰•비용 자료 비교 | 자산 누락 감소 |
| 교차 호출로 분리하기 힘든 **의존성** | 의존 관계 기준 웨이브 구성 | 전환 뒤 연결 단절 방지 |
| 기한만 보고 반복하는 **재호스팅** | 목표 상태•편익•부채 재평가 | 장기 기술 부채 감소 |
...
- **6R 전략 선택 기준**: 업무 가치•철수 기한•현대화 편익•잔존 제약을 비교하는 판단 기준이다.

</details>

- **6R 전략 선택 기준**으로 워크로드별 **클라우드 마이그레이션 6R**을 고르고, 의존 관계로 **이전 웨이브**를 결정한다.
