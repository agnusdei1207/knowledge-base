...
sequenceDiagram
    ...
    participant K as CPU·운영체제 커널
    participant A as 현재 스레드·TCB
    participant S as 스케줄러·MMU·TLB
    participant B as 다음 스레드·TCB

    T->>K: ① 시간 할당 종료·선점 인터럽트
    K->>A: ② PC·레지스터·스택 포인터 저장
    K->>S: ③ 현재 상태 갱신·다음 대상 선택 요청
    S->>B: ④ 준비 상태·우선순위·친화도 확인
    B-->>S: ⑤ 문맥·주소 공간 정보 반환
    S-->>K: ⑥ 페이지 테이블·TLB 전환 완료
    K->>B: ⑦ 문맥 복원·다음 스레드 디스패치
```
...
> 요약: PCB는 자원 상태, TCB는 실행 문맥을 저장한다.

**동작 원리**

- **① 선점 인터럽트**: 시간 만료·고우선순위 사건으로 커널 진입
- **② 현재 문맥 저장**: PC·레지스터·스택 포인터를 TCB에 기록
- **③ 다음 대상 요청**: 현재 상태를 바꾸고 런 큐 선택 시작
- **④ 후보 평가**: 준비 상태·우선순위·친화도 비교
- **⑤ 전환 정보 반환**: 선택 스레드의 문맥·주소 공간 확인
- **⑥ 주소 전환**: 필요 시 페이지 테이블과 TLB 상태 변경
- **⑦ 실행 재개**: 다음 문맥을 복원해 중단 지점부터 실행
...
- 응답 지연과 문맥 전환 비중을 함께 측정해 두 지표가 허용 범위에 들도록 시간 할당량을 조정한다.
