...
date: "2026-07-30T23:17:50+09:00"
tags: [notes-software]
weight: 2
extra:
  question_no: "002"
...
- **컨텍스트 스위칭(Context Switching)**: 현재 실행 문맥을 저장하고 다음 실행 문맥을 복원해 실행 주체를 교체하는 운영체제 기법
...
> **키워드:** PCB·컨텍스트 스위칭 (PCB Context Switching)
...
1. **현재 실행 문맥**: 프로그램 카운터·레지스터를 현재 TCB에 보존
2. **스케줄 요청**: 런 큐에서 우선순위·친화도 평가 시작
3. **다음 실행 주체**: 평가 결과로 다음 TCB·PCB 확정
4. **문맥 조회 요청**: 다음 프로세스면 페이지 테이블 기준 전환
5. **다음 실행 문맥**: 저장된 레지스터 복원으로 실행 재개
