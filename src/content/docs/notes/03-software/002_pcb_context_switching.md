...
date: "2026-08-05T13:01:42+09:00"
tags: [notes-software]
weight: 2
extra:
  question_no: "002"
...
- **CPU**: 현재 스레드의 프로그램 카운터•레지스터 문맥을 실행하는 구성
- **운영체제 커널(Operating System Kernel)**: 문맥 저장•복원과 실행 제어를 수행하는 구성
- **스케줄러(Scheduler)**: 실행 가능 주체 중 다음 실행 대상을 선택하는 구성
- **PCB**: 프로세스 주소 공간•상태•자원 참조를 보관하는 구성
- **TCB**: 스레드 레지스터•스택•스케줄 상태를 보관하는 구성
...
2. **런 큐 정책 평가**: 우선순위•친화도•준비 상태 확인
3. **다음 PCB•TCB 선택**: 실행 주체와 저장 문맥 확정
4. **주소 공간 전환**: 다른 프로세스면 페이지 테이블 교체
5. **문맥 복원•디스패치**: 레지스터•스택 복원 후 제어권 이전

#### 한줄 요약

- **문맥 저장•스케줄 선택•주소 전환•복원** 순환
...
- **응답 목표•전환 비중**을 함께 반영한 시간 할당량 조정
