...
date: "2026-07-25T01:37:00+09:00"
tags: [notes-software]
weight: 2
extra:
  question_no: "002"
...
## Ⅰ. 개요
- **정의/개념**: 문맥 저장·복원으로 실행 주체 교체
- **배경/필요성**: 선점·대기가 실행을 중단해 문맥 보존·복원 필요
...
- 문맥 저장·복원이 선점 후 실행 재개를 보장한다
- 문맥 전환 중에는 유효 작업이 진행되지 않는다
- 프로세스 전환은 TLB·캐시 손실을 더 키운다
- 짧은 할당량은 응답 지연을 낮춘다
...
|:---|:---|:---|
| 핵심 특징 | PCB·TCB·주소 공간 전환 | TCB 실행 문맥만 전환 |
| 적용 기준 | 다른 프로세스 디스패치 | 같은 프로세스 내 디스패치 |
| 주요 위험 | TLB·캐시 지역성 손실 | 공유 캐시 교란 가능 |
...
1. 대화형 서버는 응답 지연·문맥 전환 비중으로 할당량 조정
2. 패킷 처리 스레드를 프로세서에 고정해 캐시 재적재 감소
