...
- 배경/필요성: 물리 서버의 단일 OS 점유 구조로는 **서버 자원 유휴화(10~20% 이용률) 극복 및 이기종 OS 동시 격리 구동 불가**
...
#### 한줄 요약
- 완벽한 OS 수준 결함 격리를 제공하며 하드웨어 가상화(VT-x)를 통해 고성능을 구현한다.
...
#### 한줄 요약
- Type 1은 베어메탈 직결로 초고성능을, Type 2는 호스트 OS 위에서 개발 편의성을 제공한다.
...
#### 한줄 요약
- 특권 명령 시도 → VM Exit 트랩 → 하이퍼바이저 중재 에뮬레이션 → VM Entry 복귀 순으로 실행된다.
...
#### 한줄 요약
- SR-IOV 네트워크 가속, EPT 거대 페이지, Ballooning 자원 제어, 라이브 마이그레이션으로 가상화 성능을 완성한다.

## Ⅶ. 결론

- 대규모 엔터프라이즈 클라우드 인프라는 **Type 1 하이퍼바이저(KVM/ESXi)** 위에 **SR-IOV 및 라이브 마이그레이션**을 구축하고, 서비스 계층은 **컨테이너 가상화**를 융합하여 인프라 효율 극대화
