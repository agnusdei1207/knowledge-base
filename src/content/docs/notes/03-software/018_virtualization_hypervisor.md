...
- 배경/필요성: 물리 서버 한 대에 단일 OS만 구동할 때 발생하는 낮은 하드웨어 자원 이용률(10~20%), 물리 서버 증설에 따른 상면·전력 비용 폭증 및 이기종 환경 통합의 한계를 극복하고, 하드웨어 자원을 추상화·다중화하여 복수의 게스트 OS를 독립된 가상 머신(VM)으로 격리 실행하는 **하이퍼바이저(Hypervisor/VMM) 계층을 도입하여 서버 집적도와 클라우드 유연성을 극대화**할 필요
