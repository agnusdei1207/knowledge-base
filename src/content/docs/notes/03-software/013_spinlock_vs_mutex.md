...
- **임계 영역(Critical Section)**: 다중 스레드가 공유 자원에 접근하여 상호 배제(Mutual Exclusion)가 반드시 보장되어야 하는 코드 블록.
- **비교 및 교환(Compare-and-Swap, CAS)**: 메모리 위치의 현재 값이 예상 값과 같을 때만 새 값으로 원자적(Atomic) 갱신을 수행하는 하드웨어 동기화 명령어.
</details>
