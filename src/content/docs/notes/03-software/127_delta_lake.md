...
- 정의/개념: Parquet 파일을 **트랜잭션 로그로 버전 관리**하는 테이블 형식이다.
- 배경/필요성: 객체 스토리지의 **부분 쓰기•동시 변경 충돌** 방지가 필요하다.
...
- **1. 기준 로그 버전 적재**: 현재 파일 집합과 스키마 복원한다.
- **2. 새 Parquet 파일 기록**: 변경 결과를 불변 객체로 저장한다.
- **3. 읽기•쓰기 범위 충돌 검사**: 동시 변경과 작업 범위를 비교한다.
- **4. Add•Remove 로그 커밋**: 파일 변경을 다음 단일 버전으로 확정이다.
...
- **파일 병합**: 작은 파일을 적정 크기의 불변 파일로 다시 작성해 열기•메타데이터 비용을 줄이는 처리한다.
...
- 동시 변경과 버전 복원이 필요하면 **트랜잭션 로그** 기반 **Delta Lake** 채택이 핵심이다.
