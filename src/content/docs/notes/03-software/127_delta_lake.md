...
sequenceDiagram
    ...
    W->>L: 1. 기준 로그 스냅숏 조회
    L-->>W: 유효 파일·스키마 반환
    W->>D: 2. 새 Parquet 파일 저장
    D-->>W: 새 파일 위치 반환
    W->>L: 3. 동시 커밋 충돌 검사
    W->>L: 4. Add·Remove 원자 커밋
    R->>L: 5. 로그 기반 스냅숏 조회
```

**동작 원리**

- **1. 기준 로그 스냅숏 조회**: 트랜잭션 로그를 재생해 현재 파일 집합과 스키마 확보
- **2. 새 Parquet 파일 저장**: 기존 데이터를 덮지 않고 변경 결과를 새 불변 파일로 기록
- **3. 동시 커밋 충돌 검사**: 기준 버전 이후 로그와 읽기·쓰기 범위를 비교
- **4. Add·Remove 원자 커밋**: 파일 추가·제거 작업을 다음 단일 로그 버전으로 확정
- **5. 로그 기반 스냅숏 조회**: 체크포인트와 로그 버전으로 현재·과거 테이블 상태 재구성
