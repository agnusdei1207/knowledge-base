...
date: "2026-07-25T11:24:00+09:00"
tags:
  - "notes-software"
weight: 127
extra:
...
- **정의/개념**: Parquet 파일을 트랜잭션 로그로 관리한다
- **배경/필요성**: 객체 파일의 동시 커밋 충돌을 막는다
...
- Add·Remove 로그가 버전별 유효 파일을 결정한다
- 원자 로그 공개는 독자의 부분 변경 읽기를 막는다
- 낙관적 충돌 검사는 겹치는 동시 쓰기를 거부한다
- 병합·VACUUM은 조회·저장 비용 증가를 막는다
...
| 설계 요소 | 설명 |
|:---|:---|
| Parquet 데이터 파일 | 테이블 행을 불변 파일로 저장 |
| `_delta_log` | 변경 동작을 커밋 순서로 기록 |
| 체크포인트·스냅샷 | 로그를 집약해 버전별 상태 제공 |
| 낙관적 트랜잭션 | 기준 버전과 최신 변경의 충돌 검사 |
| 유지관리 | 파일 병합·통계·보존 기한 관리 |

> 요약: 데이터 파일은 커밋에 포함된 뒤 스냅샷 유효 파일로 읽힘
...
| 절차 | 설명 |
|:---|:---|
| 기준 버전 확인 | 대상 파일·스키마·현재 로그 버전 확인 |
| 새 파일 작성 | 변경 결과를 불변 Parquet 파일로 기록 |
| 동시 변경 충돌 검사 | 기준 이후 커밋과 작업 범위 비교 |
| Add·Remove 버전 커밋 | 파일 추가·제거를 다음 로그에 확정 |
| 새 스냅샷 제공 | 로그·체크포인트로 유효 파일 결정 |

> 요약: 새 파일을 작성하고 충돌 없는 로그를 커밋해 스냅샷을 바꿈
...
| 판단 기준 | Delta Lake 테이블 | 일반 Parquet 파일 집합 |
|:---|:---|:---|
| 핵심 특징 | 로그로 파일 집합·버전·스키마를 관리함 | 엔진이 디렉터리 파일을 직접 해석함 |
| 적용 기준 | 잦은 변경·동시 쓰기·과거 버전 조회 | 일괄 적재 중심의 정적 데이터 분석 |
| 주요 위험 | 작은 파일·로그·보존 정책 관리 | 동시 변경 충돌·과거 상태 복원 어려움 |
...
1. 주문 MERGE 결과를 한 버전으로 공개한다
2. 스트림의 작은 파일을 병합해 읽기 비용을 줄인다

### 쉽게 이해하기 (학습용)
- 주문 MERGE는 겹치는 동시 변경을 검사한 뒤 삽입·수정을 한 버전으로 공개해 독자의 부분 결과를 방지함
- 스트림의 작은 파일을 병합하되 VACUUM 보존 기간은 가장 오래 멈출 수 있는 독자와 과거 조회 요구보다 길어야 함

## Ⅶ. 결론

- 동시 변경·버전 조회가 필요하면 Delta Lake를 택한다
