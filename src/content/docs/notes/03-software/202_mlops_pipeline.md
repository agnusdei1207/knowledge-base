...
- **정의/개념**: 모델 생명주기 자동화·품질 유지
- **배경/필요성**: 데이터·코드 변화로 재현 가능한 파이프라인 필요
...
| 판단 기준 | 수동 운영 | 파이프라인 자동화 | CI·CD·CT 자동화 |
|:---|:---|:---|:---|
| 핵심 특징 | 학습·배포를 사람이 반복 | 학습 단계를 순서대로 자동 실행 | 변경 검증·배포·재학습 연결 |
| 적용 기준 | 소수 실험·운영 배포 없음 | 반복 학습·재현성 필요 | 지속 배포·드리프트 대응 필요 |
| 주요 위험 | 재현 불가·수동 오류 | 배포·감시 단절 | 오검출에 의한 재학습 반복 |
