...
- 배경/필요성: 노트북 기반 수동 실험은 코드·데이터·환경이 함께 기록되지 않아 같은 결과를 다시 만들 때마다 재현 비용이 발생하고 학습과 서빙의 전처리가 갈라져 성능 저하가 뒤늦게 드러나므로, 실험과 운영 사이에 파이프라인을 코드로 고정하고 피처 정의를 단일화하는 계층을 둘 필요
