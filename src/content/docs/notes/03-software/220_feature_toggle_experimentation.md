...
date: "2026-07-29T19:45:00+09:00"
tags: ["notes-software"]
weight: 220
extra:
  question_no: "220"
...
- **피처 토글 FT(에프티, Feature Toggle)**: 영어 두 낱말의 첫 글자를 쓴 표기로 배포와 기능 노출을 분리해 실행 경로를 선택한다.
- **릴리즈 토글(Release Toggle)**: 미완성 기능을 점진적으로 사용자에게 노출한다.
- **실험 토글(Experiment Toggle)**: 무작위 사용자군 대상 변종 효과 측정한다.
...
block
    columns 1
    A["플래그 제어부 | 플래그 소유자·규칙·버전·만료일을 관리함"]
    B["사용자 맥락 | 사용자·단말·지역 속성으로 실험군을 일관되게 결정함"]
    C["평가 런타임·SDK | 규칙을 계산해 애플리케이션의 기능 분기를 결정함"]
    D["애플리케이션 | 기존·후보 실행·노출 기록"]
    E["실험 분석 | 성과와 가드레일 악화를 분석해 확대·중단 여부를 판정함"]
    A --- B
    B --- C
    C --- D
    D --- E
...
1. **정의**: 가설·평가 지표·모수 설정
2. **구성**: 소유자·대상·만료 조건 지정
3. **노출**: 사용자 배정·노출 로그 수집
4. **판정**: 오류율·효과·유의성 분석
5. **정리**: 잔여 코드·플래그 제거
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 사용 종료 뒤 **토글 잔존** | Owner·만료일·**자동 정리** | 조건 복잡성 **감소** |
| 사용자별 **할당 불일치** | 안정 키·**일관 Hash 적용** | 실험 오염 **방지** |
...
- **릴리스 위험·실험 타당성·토글 수명**으로 기능 노출 결정
