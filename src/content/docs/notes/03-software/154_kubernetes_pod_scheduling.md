...
- 배경/필요성: 단순 라운드로빈 배치 시 발생하는 **노드별 자원 불균형, 특수 장비(GPU) 오배치 및 단일 장애점(SPOF) 편중 해결 불가**

#### 한줄 요약
- 필터링과 스코어링 2단계를 거쳐 파드를 가장 적합한 워커 노드에 균형 있게 분산 배치한다.
...
#### 한줄 요약
- 스케줄링 큐, 필터링, 스코어링, 어피니티, 테인트/톨러레이션이 결합된다.
...
Pending 상태의 파드가 스케줄러에 도달
        │
   1. [큐 인출] 스케줄러가 대기 큐에서 PriorityClass가 가장 높은 파드 인출
        │
   2. [노드 필터링] Request 자원 부족 노드 및 Taint 불일치 노드를 1차 탈락 (Filtering)
        │
   3. [노드 스코어링] 통과 노드들을 대상으로 PodAntiAffinity 및 자원 균형도 점수 계산 (Scoring)
        │
   4. [최고 득점 노드 선정] 총점이 가장 높은 1등 노드를 최종 타깃으로 낙점 (동점 시 라운드로빈)
        │
   5. kube-apiserver에 바인딩 요청을 전송하여 `pod.spec.nodeName` 확정 및 etcd 갱신
```

#### 한줄 요약
- 큐 인출 → 필터링 → 스코어링 → 최고점 선정 → 바인딩 순으로 진행된다.
...
- 대규모 분산 클러스터의 자원 효율성과 무중단 가용성을 극대화하기 위해 **필수 조건(Hard)과 선호 조건(Soft)을 조화롭게 설계하고 Topology Spread Constraints와 Taints/Tolerations를 표준 적용**하여 최적의 쿠버네티스 스케줄링 체계 완성
