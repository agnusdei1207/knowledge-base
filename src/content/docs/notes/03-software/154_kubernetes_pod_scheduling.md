...
- 배경/필요성: 대규모 분산 클라우드 인프라에서 수천 대의 이기종 노드(CPU, GPU, NVMe 등)와 다양한 파드의 요구사항이 혼재될 때, 단순 순차 할당 시 발생하는 고가 특수 노드로의 일반 파드 오배치, 동일 서비스 복제본의 단일 노드 쏠림으로 인한 장애 반경 확대 및 노드 단편화 문제를 해결하기 위해, 1단계 조건 필터링(Predicates)과 2단계 가중치 스코어링(Priorities), 어피니티(Affinity) 및 테인트/톨러레이션(Taints/Tolerations)을 수행하는 쿠버네티스 스케줄링 메커니즘을 도입하여 **클러스터 자원 활용률 극대화와 고가용성 장애 격리(Fault Tolerance)를 동시에 달성**할 필요
...
- 엔터프라이즈 컨테이너 클러스터의 자원 균형 및 고가용성 아키텍처를 완성하는 **가장 핵심적인 워크로드 배치 두뇌**로 확립되었으며, 실무 구축 시에는 **GPU 등 특수 리소스 보호를 위한 Taints & Tolerations, 다중 가용 영역(Multi-AZ) 간 파드 쏠림을 원천 방어하는 `topologySpreadConstraints(maxSkew: 1)`, 중요 워크로드의 배치 우선권을 보장하는 PriorityClass & Preemption, 대규모 배치 지연을 해소하는 Just-in-Time 노드 프로비저너 Karpenter 연동**을 결합하여 무결점 자원 효율성과 인프라 가용성을 완성
