...
- **정의/개념**: 필수 조건 뒤 선호도가 높은 노드 배정
- **배경/필요성**: 자원·정책 제약으로 필터·점수 배치 필요
...
| 비교축 | Filter | Score |
|:---|:---|:---|
| 핵심 특징 | 실행 불가 노드를 후보에서 제외함 | 실행 가능 노드의 선호 순위를 계산함 |
| 적용 기준 | 자원이 부족하거나 조건이 안 맞을 때 | 분산 배치나 특정 선호 규칙이 있을 때 |
| 주요 위험 | 과도한 제약으로 배치 불가 | 점수 편향·자원 불균형 |
