...
- **정의/개념**: 커널·쉘·파일시스템을 계층화한 운영체제 구조
- **배경/필요성**: 직접 특권 접근을 막아 명령·자원 분리 필요
...
- 파일 인터페이스와 파이프로 단일 기능 프로그램 조합한다.
...
| 주요 위험 | 결함의 시스템 전파 | 입력·스크립트 주입 | 권한 오류·메타데이터 손상 |
...
1. 로그 분석은 파이프로 필터·집계 명령 연결
2. 다중 사용자 서버는 소유자·권한 비트로 파일 접근 제한

### 쉽게 이해하기 (학습용)

- 로그는 필터와 집계 도구를 파이프로 이어 필요한 내용만 뽑는다
- 여러 사용자가 같은 서버를 써도 각 파일은 소유자와 권한에 따라 열 수 있다

## Ⅶ. 결론

- 명령 해석은 쉘, 자원 통제는 커널, 영속 저장은 파일시스템
