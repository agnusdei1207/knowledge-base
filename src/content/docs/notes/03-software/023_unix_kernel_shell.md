...
- ① 사용자가 쉘에 명령과 인수 전달
- ② 쉘이 명령·파이프·리다이렉션을 해석하고 시스템 호출로 실행 요청
- ③ 커널이 파일시스템에 실행 파일·데이터·권한 조회
- ④ 파일시스템이 파일 내용과 메타데이터 반환
- ⑤ 커널이 프로세스 실행 결과와 종료 상태 반환
- ⑥ 쉘이 결과를 출력하고 다음 명령 프롬프트 표시
...
- 로그 분석은 필터·변환·집계 명령을 파이프로 연결하고 각 단계의 종료 상태를 검사해 중간 실패를 놓치지 않는다.
...
- 유닉스 기능을 책임별로 분리하려면 **명령 해석·자원 통제·영속 저장 요구**를 검토하고 쉘·커널·파일시스템의 경계를 지켜 구성한다.
