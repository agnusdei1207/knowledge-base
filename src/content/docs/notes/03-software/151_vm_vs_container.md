---
sidebar:
  order: 151
  label: "151. VM vs 컨테이너 비교 (VM vs Container)"
  badge:
    text: "기출 · 70%"
    variant: note
title: "VM vs 컨테이너 비교 (VM vs Container)"
date: "2026-07-30T23:46:38+09:00"
tags: ["notes-software"]
weight: 151
extra:
  question_no: "151"
  source_status: "기출"
  source_history: "128회, 131회, 132회, 137회"
  priority: 70
  priority_note: "가상머신·컨테이너 격리 비교가 반복 출제됨"
---

## 미리 알고가기

- **가상머신(Virtual Machine, VM)**: 가상 하드웨어 위에서 독립된 운영체제와 응용을 실행하는 격리 환경
- **운영체제(Operating System, OS)**: 프로세스·메모리·장치·파일을 관리하는 시스템 소프트웨어
- **하이퍼바이저(Hypervisor)**: 물리 자원을 가상 하드웨어로 분할해 여러 게스트 운영체제를 실행하는 계층
- **게스트 OS(Guest OS)**: VM 안에서 독립 커널·드라이버·시스템 서비스를 실행하는 운영체제
- **컨테이너(Container)**: 호스트 커널을 공유하며 응용 프로세스의 자원과 가시 범위를 분리한 실행 환경
- **격리 경계(Isolation Boundary)**: 한 워크로드의 권한·결함이 다른 워크로드와 호스트에 미치는 범위
- **네임스페이스(Namespace)**: 프로세스·네트워크·마운트 등 커널 자원의 가시 범위를 분리하는 기능
- **제어 그룹(Control Group, cgroup)**: 프로세스 집합의 CPU·메모리·입출력 사용량을 제한하고 계측하는 Linux 기능
- **컨테이너 런타임(Container Runtime)**: 이미지에서 컨테이너 프로세스와 커널 격리를 생성·관리하는 소프트웨어
- **오케스트레이터(Orchestrator)**: 가상 머신이나 컨테이너의 배치·실행·상태를 조정하는 제어 시스템
- **루트 파일시스템(Root Filesystem, RootFS)**: 컨테이너 프로세스가 루트 경로로 보는 이미지 기반 파일 계층
- **소프트웨어 자재 명세서(Software Bill of Materials, SBOM)**: 이미지에 포함된 소프트웨어 구성요소와 버전을 기록한 목록
- **샌드박스(Sandbox)**: 워크로드와 호스트 사이에 추가 실행 경계를 두어 커널 침해 영향을 제한하는 격리 환경
- **입출력(Input/Output, I/O)**: 저장장치·네트워크와 데이터를 주고받는 동작과 그 사용량
- **중앙처리장치(Central Processing Unit, CPU)**: 운영체제와 워크로드의 명령을 실행하는 처리 자원
- **테넌트(Tenant)**: 하나의 인프라를 논리적으로 분리해 사용하는 고객·조직 단위
- **소프트웨어 공급망·이미지 서명**: 빌드 의존성부터 배포 이미지까지의 유통 경로와 이미지 출처·무결성을 검증하는 전자서명

> **키워드:** VM vs 컨테이너 비교 (VM vs Container)

## Ⅰ. 개요

- 정의/개념: 독립 운영체제와 **공유 커널 격리**를 대비하는 실행 가상화 비교 기준
- 배경/필요성: 신뢰 경계·운영체제 요구에 따른 **격리 강도·배포 밀도** 판단

### 쉽게 이해하기 (학습용)

- VM은 집마다 전기·배관까지 따로 두고 컨테이너는 한 건물 설비를 공유한 채 방만 나누듯, 커널 분리 여부가 격리 강도와 실행 무게를 가른다.

## Ⅱ. 특징

- 가상 하드웨어마다 부팅하는 **VM 독립 커널**
- 네임스페이스·cgroup 기반 **컨테이너 공유 커널**
- VM 신뢰 경계 안의 **컨테이너 계층 결합**

### 쉽게 이해하기 (학습용)

- 집 설비까지 분리하면 무겁지만 이웃 고장의 영향이 작고, 한 건물 설비를 공유하면 방은 빨리 만들 수 있지만 커널 결함이 공동 경계를 흔든다.

## Ⅲ. 구조 및 구성요소

```mermaid
block
    columns 2
    H["하이퍼바이저"]
    V["게스트 OS"]
    K["호스트 커널"]
    R["컨테이너 런타임"]
    I["격리 정책"]
    H --- V
    K --- R
    R --- I
```

| 구성요소 | 책임 |
|:---|:---|
| 하이퍼바이저 | VM별 **가상 하드웨어** 제공 |
| 게스트 OS | VM별 **독립 커널**·시스템 서비스 |
| 호스트 커널 | 컨테이너의 **공유 커널** 자원 제공 |
| 컨테이너 런타임 | 이미지 기반 **격리 프로세스** 생성 |
| 격리 정책 | 네임스페이스·cgroup·**권한 경계** 통제 |

### 쉽게 이해하기 (학습용)

- 하이퍼바이저와 게스트 OS는 집마다 설비를 나누고, 호스트 커널과 런타임은 같은 건물 안에서 방과 사용량 경계만 나눈다.

## Ⅳ. 흐름도

```mermaid
sequenceDiagram
    participant O as 오케스트레이터
    participant H as 하이퍼바이저
    participant V as 게스트 OS
    participant R as 컨테이너 런타임
    participant K as 호스트 커널
    alt VM 실행
        O->>H: 1. VM 자원·장치 명세
        H->>V: 2. 가상 하드웨어·부팅 신호
        V-->>O: 3. 게스트 OS 준비 상태
    else 컨테이너 실행
        O->>R: 4. 이미지·실행 명세
        R->>K: 5. RootFS·격리·자원 명세
        K-->>R: 프로세스 식별자
        R-->>O: 실행 준비 상태
    end
```

**동작 원리**

1. **VM 자원·장치 명세**: 가상 CPU·메모리·장치 범위 확정
2. **가상 하드웨어·부팅 신호**: VM 경계 안에서 독립 커널 부팅
3. **게스트 OS 준비 상태**: 시스템 서비스와 애플리케이션 실행 완료
4. **이미지·실행 명세**: 컨테이너 원본·환경·권한 범위 확정
5. **RootFS·격리·자원 명세**: 프로세스 경계·사용 한도 적용

### 쉽게 이해하기 (학습용)

- VM은 빈 집의 설비와 운영체제를 모두 켠 뒤 입주하고, 컨테이너는 이미 켜진 건물에 새 방과 전기 한도만 만들어 들어간다.

## Ⅴ. 종류 및 비교

| 실행 가상화 방식 | 가상 머신 | 컨테이너 |
|:---|:---|:---|
| 적용 기준 | **테넌트 격리·다른 OS** | **빠른 배포·수평 확장** |
| 핵심 특징 | 가상 하드웨어·**독립 커널** | 호스트 커널 공유 **격리 프로세스** |
| 한계 | 부팅 시간·큰 **메모리 점유** | 커널 제약·**공유 공격면** |

### 쉽게 이해하기 (학습용)

- 서로 믿지 못하거나 다른 운영체제가 필요하면 집을 나누는 VM을, 같은 커널에서 빠르게 복제해야 하면 방을 나누는 컨테이너를 선택한다.

## Ⅵ. 실무 고려사항 및 대책

| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 비신뢰 테넌트의 **공유 커널 위험** | 테넌트별 VM·전용 노드·**샌드박스** | 커널 경계의 **침해 영향 격리** |
| 다른 커널·드라이버의 **OS 요구** | VM 선택·호스트 **이미지 호환** 검증 | 커널·드라이버 **불일치 방지** |
| 이미지 계층의 **공급망 취약점** | 신뢰 원본·SBOM·**서명 검증** | 취약·변조 **이미지 차단** |
| 공유 호스트의 **이웃 자원 간섭** | 예약·제한·우선순위·**I/O 계측** | 워크로드별 **성능 편차 감소** |

### 쉽게 이해하기 (학습용)

- 서로 다른 고객은 건물 설비까지 VM으로 나누고, 한 고객 안의 서비스는 같은 건물의 컨테이너 방으로 배치해 격리와 교체 속도를 함께 맞춘다.

## Ⅶ. 결론

- **비신뢰·다른 OS**는 VM, **동일 커널·고밀도 배포**는 컨테이너

### 쉽게 이해하기 (학습용)

- 서로 믿지 못하거나 운영체제가 다르면 집을 나누고, 같은 커널을 믿을 수 있으면 한 건물 안의 방을 늘려 배포 속도와 밀도를 높인다.
