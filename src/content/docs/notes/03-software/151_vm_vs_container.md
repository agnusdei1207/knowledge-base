...
워크로드 실행 환경 선택 및 기동 요청
        │
   1. [요구 분석] 다중 테넌트 커널 격리가 필수인지, MSA 초고속 확장이 필요한지 분석
        │
   2. [실행 모델 분기] 커널 분리는 VM 경로, 경량 고밀도 배치는 Container 경로 분기
   ┌────┴───────────────────────────┐
  VM 경로                           Container 경로
   │                                 │
3A. [가상 하드웨어 할당]            3B. [네임스페이스/cgroups 생성]
   vCPU, vRAM, vDisk 할당            PID, NET 네임스페이스 및 메모리 상한 할당
   │                                 │
4A. [Guest OS 부팅]                 4B. [불변 이미지 즉시 마운트]
   OS 커널 로딩 (수 분 소요)         OverlayFS 레이어 마운트 (수 밀리초)
   │                                 │
   └────┬───────────────────────────┘
        ▼
   5. 애플리케이션 엔트리포인트 프로세스 가동 및 서비스 트래픽 개시
```
...
- 엔터프라이즈 인프라 아키텍처 설계 시 **보안 및 규제 신뢰 경계는 가상머신(VM)으로 강력히 격리하고, 그 내부에서 마이크로서비스의 민첩성과 고밀도 확장은 Docker 컨테이너로 구현하는 상호 보완적 하이브리드 가상화 전략**을 수립하여 가용성과 보안성 완성
