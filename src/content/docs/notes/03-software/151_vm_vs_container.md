...
- 정의/개념: 독립 운영체제를 실행하는 가상 머신과 **호스트 커널을 공유하는 컨테이너**를 대비하는 실행 가상화 구조이다.
- 배경/필요성: 신뢰 경계•운영체제 요구에 따른 **격리 강도•배포 밀도** 판단이 필요하다.
...
- 네임스페이스•cgroup 기반 **컨테이너 공유 커널**이 핵심이다.
- VM 신뢰 경계 안의 **컨테이너 계층 결합**이 핵심이다.
...
- **비신뢰•다른 OS**는 VM, **동일 커널•고밀도 배포**는 컨테이너가 핵심이다.
