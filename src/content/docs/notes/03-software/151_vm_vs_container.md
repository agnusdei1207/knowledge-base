...
- 현대 클라우드 인프라 아키텍처에서 대립 관계가 아닌 상호 보완재(VM 위에 K8s Worker Node 배치)의 **핵심 가상화 양대 축**으로 정립되었으며, 실무 구축 시에는 **엄격한 규제 준수 및 다중 테넌트 격리가 필수인 코어 원장에는 VM/MicroVM(Kata/Firecracker)을 적용하고, MSA API 및 CI/CD 워크로드에는 표준 OCI 컨테이너를 적용하며, IaaS VM 풀 위에서 Kubernetes Pod 오토스케일링**을 결합하여 강력한 커널 보안 격리와 초 단위 확장 탄력성을 완벽히 조화
