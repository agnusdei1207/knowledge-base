...
- 배경/필요성: 하이퍼바이저 기반 가상머신(VM)의 강력한 커널 격리성과 독립성 이점에도 불구하고 발생하는 무거운 Guest OS 부팅 지연(수 분) 및 메모리 중복 낭비 한계와, 호스트 커널을 공유하여 초경량 기동(수 초)을 제공하는 컨테이너의 장점 이면에 존재하는 공유 커널 취약점 및 다중 테넌트(Multi-Tenant) 간 보안 격리 취약성을 해결하기 위해, 워크로드의 보안 민감도, 이식성 및 확장 주기에 따라 VM, 컨테이너, MicroVM(Firecracker/Kata)을 목적별로 비교·선택하여 **보안 무결성과 인프라 고밀도 운영 탄력성을 동시에 확보**할 필요
