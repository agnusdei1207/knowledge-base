...
- **보존•복제**: 로그를 정한 기간 유지하고 복제본으로 리더 장애와 과거 재생을 지원하는 특성이다.
...
- **주제•파티션•오프셋(Topic•Partition•Offset)**: 이벤트 분류와 병렬 로그 및 로그 안의 증가 위치를 나타내는 구조이다.
- **브로커•리더•팔로워(Broker•Leader•Follower)**: 로그를 저장하고 파티션 읽기•쓰기와 복제를 수행하는 서버 역할이다.
...
- **2. 동기화 복제본 집합(In-Sync Replicas, ISR) 복제 위치**: 팔로워가 리더와 같은 순서로 반영한 마지막 오프셋을 반환하는 정보이다.
...
2. **동기화 복제본 집합(In-Sync Replicas, ISR) 복제 위치**: 팔로워가 같은 순서로 반영한 오프셋 반환
...
- **순서 경계•키 분포 검증**: 함께 순서를 지킬 사건 범위와 실제 키 편중을 확인해 파티션 키를 정하는 활동이다.
- **처리량•소비자 수 기반 파티션 산정**: 목표 병렬 처리량과 그룹별 최대 소비자 수에 맞춰 파티션 수를 정하는 기준이다.
- **멱등성•확인 응답(Acknowledgements, acks)•재시도 설정**: 생산자 중복과 순서 역전을 줄이도록 요청 식별과 내구 확인 및 재전송을 조정하는 통제이다.
- **처리•커밋 순서•멱등성 설계**: 업무 처리가 성공한 뒤 오프셋을 커밋하고 재처리에도 중복 효과를 막는 방식이다.
- **파티션별 지연 감시•소비자 확장**: 각 로그의 소비 오프셋 차이를 관찰하고 병목 그룹의 처리 인스턴스를 늘리는 대응이다.
...
| 재시도 중 중복•순서 역전 발생 | **멱등성•확인 응답(Acknowledgements, acks)•재시도 설정** | **중복•역전** 억제 |
...
- **Apache Kafka**: 사건 재생과 다중 소비가 필요한 이벤트 흐름에 적합한 플랫폼이다.
