...
- 정의/개념: 이벤트를 **파티션 커밋 로그**에 저장해 발행·구독·재생하는 플랫폼
- 배경/필요성: 전통적인 인메모리 메시지 큐(RabbitMQ 등)가 직면하는 대규모 트래픽 시의 브로커 메모리 고갈, 메시지 소비 후 즉시 삭제로 인한 다중 컨슈머 과거 데이터 재처리(Replay) 불가 한계와 시스템 간 점대점(Point-to-Point) 결합도 문제를 극복하고, 디스크 기반의 불변 순차 커밋 로그(Commit Log)와 PageCache/Zero-Copy 전송 및 파티션(Partition) 수평 분산 아키텍처를 도입하여 **초당 수백만 건의 초고속 이벤트 수집·영속화와 독립적인 다중 컨슈머 그룹의 무손실 스트리밍 처리를 실현**할 필요
...
| 토픽·파티션 | **Append-Only 로그**와 순서 범위 제공 |
