...
sidebar:
  ...
  badge:
    ...
    variant: note
title: "Apache Kafka"
date: "2026-08-26T13:12:44+09:00"
tags:
  - "notes-software"
weight: 119
extra:
...
- 배경/필요성: 소모성 큐는 전달과 동시에 메시지를 지워 구독자가 늘 때마다 같은 데이터를 다시 만들어 보내는 비용이 발생하고 과거 구간을 되돌려 볼 수도 없으므로, 메시지를 파티션별 순차 로그로 디스크에 남기고 소비 위치만 구독자가 각자 관리하게 해 재생과 다중 구독을 같은 저장으로 해결할 필요
...
- 스트림 재생은 **Kafka**, 작업 라우팅은 **RabbitMQ** 선택
