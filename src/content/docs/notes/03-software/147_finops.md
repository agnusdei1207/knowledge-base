...
- **비용 할당**: 정규화한 비용을 조직•제품•업무에 귀속하는 구성요소이다.
- **단위 경제성**: 비용을 업무량•수익과 연결해 단위당 효율을 계산하는 구성요소이다.
- **예산•약정•정책**: 지출 예측•이상 탐지와 할인 약정의 적용률을 관리하는 구성요소이다.
...
- **배분 규칙•미할당률**: 공유 비용의 귀속 방법과 어느 조직에도 배정되지 않은 비용 비율을 나타내는 관리 기준이다.
- **백분위•계절성•여유 용량**: 평균값이 놓치는 피크 부하와 주기적 변동 및 장애 대비 용량을 반영하는 크기 조정 기준이다.
