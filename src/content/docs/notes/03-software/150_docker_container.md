...
개발자의 애플리케이션 컨테이너 배포 요청
        │
   1. [Multi-stage 빌드] 컴파일러를 배제한 경량 프로덕션 이미지 생성
        │
   2. [레지스트리 저장] 이미지 다이제스트(SHA-256) 및 서명을 포함하여 ECR/Harbor 푸시
        │
   3. [호스트 노드 Pull] 대상 서버가 레지스트리로부터 레이어를 병렬 다운로드 및 무결성 검증
        │
   4. [커널 격리 공간 생성] 리눅스 커널에서 PID/NET 네임스페이스와 cgroups 메모리 제한 할당
        │
   5. runc 런타임이 격리된 파일시스템 마운트 위에서 엔트리포인트 프로세스를 즉각 기동
```
...
- 클라우드 네이티브 아키텍처의 배포 표준을 확립하기 위해 **Dockerfile 기반의 Multi-stage 빌드와 Non-root 보안 정책을 적용한 Docker 컨테이너를 표준 패키징으로 도입**하고, **쿠버네티스 오케스트레이션과 연동**하여 무결점 CI/CD 파이프라인 완성
