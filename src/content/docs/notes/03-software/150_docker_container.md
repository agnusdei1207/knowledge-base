...
- 배경/필요성: 전통적인 하이퍼바이저 기반 가상머신(VM)이 Guest OS 전체를 중복 구동함에 따라 발생하는 막대한 메모리 오버헤드, 수 분에 달하는 느린 부팅 지연 및 개발-운영 간 환경 불일치(Matrix of Hell) 문제를 극복하기 위해, 호스트 OS 커널을 공유하면서 리눅스 네임스페이스(Namespaces)와 cgroups를 통해 자원과 프로세스를 논리적으로 격리하고 OverlayFS 레이어드 불변 이미지로 패키징하는 Docker 컨테이너 기술을 도입하여 **밀리초 단위 초경량 기동과 완벽한 환경 일치성(Write Once, Run Anywhere)을 달성**할 필요
