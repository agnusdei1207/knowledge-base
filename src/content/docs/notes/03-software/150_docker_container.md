...
- **네임스페이스(Namespace)•제어 그룹(Control Group, cgroup)**: 프로세스가 볼 수 있는 커널 자원을 분리하고 중앙처리장치•메모리 사용량을 제한하는 기능이다.
...
- **도커파일(Dockerfile)•빌더**: 빌드 명령에 따라 이미지 계층과 기본 실행값을 만드는 구성요소이다.
...
- **볼륨•외부 저장소**: 컨테이너 수명과 분리해 영속 데이터와 로그를 보존하는 구성요소이다.
...
- **비루트 사용자•기능 권한 제거**: 관리자 권한과 불필요한 리눅스 기능을 제거해 침해 피해 범위를 줄이는 통제이다.
...
- **가상 머신(Virtual Machine, VM)**: 운영체제 커널 자체의 분리가 필요한 신뢰 경계에 적합한 실행 방식이다.
