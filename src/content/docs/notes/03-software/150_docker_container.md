...
date: "2026-07-30T23:46:06+09:00"
tags: ["notes-software"]
weight: 150
extra:
  question_no: "150"
...
- **루트 파일시스템(Root Filesystem, RootFS)**: 컨테이너 프로세스가 루트 경로로 보는 이미지 기반 파일 계층
- **이미지 매니페스트(Image Manifest)**: 이미지 구성·플랫폼·계층 다이제스트를 기록한 메타데이터
- **소프트웨어 자재 명세서(Software Bill of Materials, SBOM)**: 이미지에 포함된 소프트웨어 구성요소와 버전을 기록한 목록
- **루트 사용자·리눅스 기능 권한(Root User·Linux Capabilities)**: 최고 관리자 계정과 커널 권한을 세분화한 권한 단위
- **이미지 서명(Image Signing)**: 이미지 다이제스트에 전자서명을 연결해 출처와 무결성을 검증하는 통제
- **가상 머신(Virtual Machine)**: 하이퍼바이저가 제공한 가상 하드웨어에서 게스트 운영체제를 실행하는 격리 단위
