...
- **1. 이미지 계층•다이제스트**: 빌드 명령별 결과와 내용 기반 식별값을 불변 원본으로 저장하는 단계이다.
- **2. 이미지 다이제스트 요청**: 태그 대신 정확한 이미지 원본의 내용 식별값을 조회하는 단계이다.
- **4. 격리•자원•프로세스 설정**: 네임스페이스와 제어 그룹으로 가시 범위와 중앙처리장치•메모리 한도를 적용하는 단계이다.
- **5. 볼륨 마운트 요청**: 컨테이너 교체와 무관하게 남을 데이터 경로를 외부 저장소에 연결하는 단계이다.
