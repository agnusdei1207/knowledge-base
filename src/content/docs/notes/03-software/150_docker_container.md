...
- 배경/필요성: 서버별 라이브러리•설정 차이로 **동일 빌드 실행 결과** 불일치가 발생한다.
...
- **1. 이미지 계층•다이제스트 생성**: 빌드 결과를 불변 원본으로 식별이다.
- **2. 매니페스트•계층 검증**: 요청 이미지의 무결성을 확인한다.
- **3. 루트 파일시스템 조립**: 읽기 전용•쓰기 계층 결합한다.
- **4. 격리•자원•프로세스 설정**: 가시 범위와 자원 한도를 적용한다.
- **5. 볼륨•외부 상태 연결**: 교체와 분리한 데이터 경로 마운트이다.
...
- **동일 커널•불변 배포**면 컨테이너, **커널 격리** 필요 시 가상 머신이 핵심이다.
