...
- 정의/개념: 이미지 기반 **호스트 커널 격리 프로세스** 단위
- 배경/필요성: 서버별 의존성 차이를 없앤 **동일 배포 원본** 제공
...
| Namespace·cgroup | 커널 자원의 **가시성·사용량** 통제 |
...
sequenceDiagram
    ...
    participant K as Namespace·cgroup
...
| Root·Capability의 **과도 권한** | 비Root·**Capability 제거·읽기 전용** | 컨테이너 탈출의 **피해 범위 완화** |
...
- **동일 커널·불변 배포**면 컨테이너, **커널 격리** 필요 시 VM
