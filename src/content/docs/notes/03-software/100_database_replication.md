...
- 정의/개념: 데이터 변경을 전파•동기화해 **여러 노드에 사본을 유지하는 기법**이다.
- 배경/필요성: 단일 노드 장애는 **서비스 중단•복구 지연**을 유발한다.
...
- **변경 로그 영속화**: 변경 순서와 내용을 내구 저장소에 기록하는 단계다.
- **순서 보존 복제**: 변경 로그를 원래 커밋 순서대로 전송하는 단계다.
- **복제 확인**: 요구한 수의 복제 노드가 로그 저장을 알리는 단계다.
- **재생 위치 보고**: 실제 적용한 마지막 로그 위치를 보내는 단계다.
...
- **1. 변경 로그 영속화**: 변경 순서를 내구 저장소에 기록한다.
- **2. 순서 보존 복제**: 로그를 복제 노드에 같은 순서로 전송이다.
- **3. 복제 확인**: 동기 정책의 요구 확인 수 충족이다.
- **4. 재생 위치 보고**: 적용 지점 차이로 복제 지연을 측정한다.
...
- **주 노드 읽기(Primary Read)**: 쓰기 직후 조회를 원본으로 보내는 최신성 통제다.
- **로그 위치 대기(Log-position Wait)**: 복제본이 특정 변경까지 적용하도록 기다리는 통제다.
- **지연 시간 경보(Lag-time Alert)**: 복제본의 적용 지연을 알리는 통제다.
- **재생 위치 경보(Replay-position Alert)**: 마지막 적용 로그 위치의 지연을 알리는 통제다.
- **복구 시점 목표(Recovery Point Objective, RPO)**: 장애 시 허용하는 최대 데이터 손실 시점이다.
- **복구 시간 목표(Recovery Time Objective, RTO)**: 서비스 복구에 허용하는 최대 시간이다.
- **복제 확인 수(Replication Acknowledgment Count)**: 커밋 전에 응답해야 할 복제본 수다.
- **정족수(Quorum)**: 결정을 확정하는 데 필요한 과반수 동의다.
- **세대(Generation)**: 현재 리더 임기를 구분하는 식별값이다.
- **펜싱(Fencing)**: 옛 주 노드의 쓰기를 차단하는 통제다.
...
| 확인 수가 목표 손실과 불일치 | **RPO•RTO별 복제 확인 수** 정의 | **손실•복구 시간** 통제 |
...
- 쓰기 순서는 **단일 주**, 다지역 쓰기는 **다중 주**, 지연 우선은 **비동기**를 선택한다.
