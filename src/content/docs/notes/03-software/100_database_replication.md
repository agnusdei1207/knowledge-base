...
- **1. 트랜잭션 커밋 요청**: 쓰기 확정을 요청한다.
- **2. 변경 로그 영속 기록**: 변경 순서를 남긴다.
- **3. 로그 순서대로 전송**: 복제 노드로 보낸다.
- **4. 복제 확인 반환**: 정책 단계까지 반영한다.
- **5. 확인 수준 충족 통보**: 요구 수를 확인한다.
- **6. 커밋 성공 응답**: 결과를 클라이언트에 보낸다.
- **7. 재생 위치 보고**: 복제 지연을 측정한다.
...
- **최신성·RPO·RTO·쓰기 지연**으로 복제 방식을 정한다.
