...
- 정의: 대용량 단일 테이블을 파티션 키(`Range`, `Hash`, `List`) 기준으로 물리적 독립 파일로 분할하여 **파티션 프루닝** 을 통해 입출력 성능을 극대화하고 데이터 수명주기를 관리하는 기법.
...
- **파티션 프루닝**: 조건에 부합하는 특정 파티션만 선별적으로 스캔하여 디스크 I/O 대폭 감소.
- **수명주기 관리**: 파티션 단위 백업, 복구 및 일괄 삭제 수행.
