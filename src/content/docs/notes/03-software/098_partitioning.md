...
- **1. 파티션 키 조건 전달**: 질의 술어를 보낸다.
- **2. 경계·매핑 규칙 조회**: 카탈로그를 확인한다.
- **3. 대상 파티션 목록 반환**: 읽을 범위를 고른다.
- **4. 대상 파티션의 키 탐색**: 필요한 인덱스만 본다.
- **5. 일치 행 위치 조회**: 대상 데이터를 찾는다.
- **6. 대상 행 반환**: 조건 결과를 보낸다.
- **7. 프루닝된 결과 전달**: 제외 후 결과를 반환한다.
...
- **질의 조건·데이터 분포·보관 정책**으로 파티션 키를 정한다.
