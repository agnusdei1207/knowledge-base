...
- **배경/필요성**: 수동 복구 지연·이중 쓰기로 자동 전환 필요
...
| 판단 기준 | Client Failover | Service Failover | Data Failover |
|:---|:---|:---|:---|
| 핵심 특징 | 호출 대상 경로를 바꿈 | 정상 인스턴스로 부하를 옮김 | 복제본을 Primary로 승격함 |
| 적용 기준 | 다중 주소·재시도 지원 | 무상태 서비스 다중화 | 상태형 데이터 단일 쓰기 |
| 주요 위험 | 재시도 폭주·오래된 주소 | 오탐·남은 용량 부족 | 이중 쓰기·데이터 손실 |
...
2. API 클라이언트는 백오프·지터로 재시도함

### 쉽게 이해하기 (학습용)
- DB는 이전 Primary를 차단한 뒤 새 쓰기를 허용함
- API는 재시도 시점을 분산해 대체 서버를 보호함
