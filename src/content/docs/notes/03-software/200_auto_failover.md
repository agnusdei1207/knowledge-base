...
- 배경/필요성: 운영자의 수동 개입에 의존하는 장애 복구 방식은 인지·판단·조치 지연으로 인해 복구 목표 시간(RTO)이 수십 분~수 시간으로 지연되고 야간 장애 시 골든타임을 놓치는 한계와, 성급한 대기 노드 승격 시 구 Primary와의 이중 쓰기(Split-Brain)로 데이터가 영구 손상되는 위험을 해결하기 위해, 다중 헬스체크 감지, STONITH 선행 펜싱(Fencing), 분산 쿼럼(Quorum) 합의 승격 및 트래픽 재라우팅을 하나의 무인 제어 루프로 결합한 자동 페일오버(Auto Failover)를 도입하여 **초단위(Sub-minute) RTO 달성과 무결점 자율 복구(Self-Healing)를 실현**할 필요
...
Primary 데이터베이스 노드 장애 발생
        │
   [상태 감시] Patroni 데몬이 매 2초마다 Primary 하트비트와 복제 지연을 etcd에 보고
        │
   [장애 확정] Primary가 3회 연속(10초) 응답하지 않자 3노드 중 과반수(2/3) 합의로 장애 확정
        │
   [Fencing 선행 격리] AWS API로 구 Primary EC2 인스턴스를 강제 중지(`ec2:StopInstances`)
   ┌────┴───────────────────────────┐
  펜싱 성공                         펜싱 실패
   │                                 │
 [Standby 승격]                      [승격 중단 및 온콜 호출]
   WAL 로그 최신 Standby를 Primary 승격  수동 승인 대기로 전환 (안전 우선)
   │                                 │
   └────┬────────────────────────────┘
        ▼
   [트래픽 전환] HAProxy 로드밸런서 및 DNS 라우팅을 갱신하여 클라이언트 요청 절체 완료
```
...
- 클라우드 네이티브 및 분산 데이터베이스(Patroni, Orchestrator) 아키텍처에서 인적 개입 없는 무인 복구와 서비스 연속성을 달성하는 **가장 핵심적인 고가용성(HA) 자율 복구 기술**로 정립되었으며, 실무 구축 시에는 **'펜싱 선행 원칙(구 Primary 강제 차단 후 Standby 승격)'의 엄격한 준수, 히스테리시스 임계치 기반 플래핑(Flapping) 오탐 방지, 펜싱 실패 시 즉시 자동 승격을 중단하고 온콜 수동 개입으로 전환하는 안전 장치(Fail-Safe), 재접속 폭주를 방어하는 지수 백오프 및 지터(Jitter)**를 결합하여 복구 속도와 데이터 무결성을 완벽히 조화
