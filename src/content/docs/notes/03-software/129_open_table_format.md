...
- 배경/필요성: 전통적 데이터 레이크의 물리 디렉터리 스캔 방식으로 인한 **동시 쓰기 충돌, 원자적 롤백 불가 및 메타데이터 탐색 지연 해결 불가**

#### 한줄 요약
- 객체 스토리지 상에 표준화된 메타데이터 계층을 두어 다양한 분산 엔진이 공유 가능한 ACID 테이블을 구현한다.
...
#### 한줄 요약
- 카탈로그, 메타데이터 계층, 데이터 파일, 삭제 파일이 결합된다.
...
다중 엔진에서 오픈 테이블 포맷으로 쓰기 트랜잭션 요청
        │
   1. [스냅샷 조회] 쓰기 엔진이 카탈로그를 참조하여 현재 유효한 최신 스냅샷 버전 확인
        │
   2. [불변 파일 기록] 변경된 레코드를 새로운 불변 Parquet 파일로 객체 스토리지에 기록
        │
   3. [메타데이터 생성] 신규 파일 경로 및 통계 정보를 담은 메타데이터(JSON/AVRO) 작성
        │
   4. [OCC 충돌 검증] 쓰기 도중 타 트랜잭션의 동일 파티션 변경 여부 검증 (충돌 시 자동 재시도)
        │
   5. [포인터 원자 갱신] 카탈로그의 최신 테이블 포인터를 신규 메타데이터로 원자적 교체 공개
```

#### 한줄 요약
- 스냅샷 조회 → 불변 파일 기록 → 메타데이터 생성 → OCC 충돌 검증 → 포인터 갱신 순으로 진행된다.
...
- 레이크하우스 구축 시 **엔진 환경에 맞추어 Databricks 생태계는 Delta Lake, 멀티 클라우드·다중 엔진 환경은 Apache Iceberg, 고빈도 CDC는 Hudi를 전략적으로 선정**하고, **REST Catalog 기반 중앙 거버넌스**를 결합하여 개방형 데이터 플랫폼 완성
