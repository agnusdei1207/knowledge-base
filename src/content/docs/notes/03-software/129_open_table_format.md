...
- 정의/개념: 객체 스토리지 파일을 테이블로 관리하는 **공개 메타데이터 명세**이다.
- 배경/필요성: 디렉터리 기반 파일 표는 **부분 커밋•동시 쓰기•구조 진화** 관리 제약이 발생한다.
...
- **파일 병합**: 작은 데이터 파일을 목표 크기로 합치는 처리한다.
- **메타데이터 병합**: 작은 목록 파일을 합쳐 계획 비용을 줄이는 처리한다.
...
- **선택 기준**: 로그 연계•구조 진화•레코드 갱신의 우선순위이다.

</details>

- 로그 연계는 **Delta Lake** 선택, 구조 진화는 **Apache Iceberg** 선택, 레코드 갱신은 **Apache Hudi**을 선택한다.
