...
- **배경/필요성**: 부분 커밋·동시 쓰기와 **버전·구조 진화** 문제를 해결한다.
...
- **원자 커밋**: 완성된 파일 집합만 새 버전으로 공개한다.
- **다중 엔진**: 공개 명세로 같은 스냅숏을 해석한다.
- **구조 진화**: 스키마·파티션 정보를 경로에서 분리한다.
...
block
    columns 1
    A["읽기·쓰기 엔진 | 조회·변경·커밋"]
    B["카탈로그 | 현재 메타데이터 위치 관리"]
    C["포맷 메타데이터 | 버전·스키마·파일 참조"]
    D["데이터 파일 | 실제 행 저장"]
    E["삭제·로그 파일 | 행 변경 기록"]
```
...
- **1. 현재 테이블 버전 조회**: 변경 기준 상태를 확인한다.
- **2. 기준 메타데이터 반환**: 스냅숏과 구조 정보를 전달한다.
- **3. 새 데이터·변경 파일 저장**: 변경 결과를 별도 파일로 기록한다.
- **4. 새 파일 참조·통계 작성**: 포맷별 메타데이터를 생성한다.
- **5. 새 메타데이터 커밋**: 현재 참조의 원자 교체를 요청한다.
- **6. 성공·충돌 결과 반환**: 확정하거나 재시도를 알린다.
- **7. 확정 테이블 버전 조회**: 독자가 유효 파일 집합을 읽는다.
...
- **엔진 호환·변경 패턴·구조 진화·유지관리 비용**으로 오픈 테이블 포맷을 선택한다.
