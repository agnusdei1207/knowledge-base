...
- 배경/필요성: 과거 Apache Hive 스타일의 디렉터리 기반 파티셔닝이 유발하는 O(N) 파일 리스팅 지연, 부분 쓰기 실패 시 롤백 불가, 동시 쓰기 시 데이터 오염 및 단일 벤더 독점 스토리지 종속 문제를 해결하기 위해, 불변 Parquet 파일 상에 메타데이터 계층(Delta Lake, Apache Iceberg, Apache Hudi)을 구축하여 **100% ACID 트랜잭션, 타임 트래블, 스키마 진화 및 다중 분산 쿼리 엔진(Spark/Trino/Flink/Snowflake) 간의 완벽한 상호운용성(Interoperability)을 실현**할 필요
