...
- **1. 현재 테이블 버전 조회**: 변경 기준 상태를 확인
- **2. 기준 메타데이터 반환**: 스냅숏과 구조 정보를 전달
- **3. 새 데이터·변경 파일 저장**: 변경 결과를 별도 파일로 기록
- **4. 새 파일 참조·통계 작성**: 포맷별 메타데이터를 생성
- **5. 새 메타데이터 커밋**: 현재 참조의 원자 교체를 요청
- **6. 성공·충돌 결과 반환**: 확정하거나 재시도를 통지
- **7. 확정 테이블 버전 조회**: 독자가 유효 파일 집합을 조회
