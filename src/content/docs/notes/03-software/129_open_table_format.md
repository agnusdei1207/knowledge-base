...
- 배경/필요성: 디렉터리 스캔에 기대는 방식은 동시 쓰기의 순서를 정할 지점이 없어 충돌을 감지하지 못하고 실패한 작업을 원자적으로 되돌릴 수도 없으므로, 어떤 파일이 테이블의 현재 상태인지를 선언하는 메타데이터 표준을 파일 위에 얹어 여러 엔진이 같은 판단을 공유하게 할 필요
