...
date: "2026-07-25T11:28:00+09:00"
tags:
  - "notes-software"
weight: 129
extra:
...
- **정의/개념**: 스냅숏·스키마를 공유하는 개방형 규격이다
- **배경/필요성**: 동시 버전 충돌과 부분 읽기를 막는다
...
|:---|:---|:---|:---|
| 핵심 특징 | 로그·체크포인트 중심 상태 관리 | 스냅샷·Manifest 중심 파일 관리 | Timeline·File Group·Index 중심 변경 관리 |
| 적용 기준 | 스파크 통합 트랜잭션 로그 | 다중 엔진 호환성 | 잦은 부분 갱신·변경 피드 |
| 주요 위험 | 로그·체크포인트 누적 | 메타데이터 트리 누적 | 인덱스·파일 병합 비용 |
...
1. 주문 변경을 Delta MERGE 한 버전에 반영한다
2. 다중 엔진이 Iceberg 스냅숏을 공유한다

### 쉽게 이해하기 (학습용)
- 주문 변경은 Delta 로그의 한 버전에 수정·삽입 파일을 함께 커밋해 독자가 부분 변경을 보지 않게 함
- 여러 분석 엔진이 같은 Iceberg 스냅숏 식별자를 읽으면 같은 파일 집합과 스키마로 결과를 재현할 수 있음

## Ⅶ. 결론

- 엔진 호환·변경 빈도·유지관리로 포맷을 선택한다
