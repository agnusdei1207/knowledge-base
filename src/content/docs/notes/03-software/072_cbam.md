...
- ① 평가팀이 사업 목표와 관련된 품질 시나리오·가중치를 요청한다.
- ② 이해관계자가 응답 수준별 효용과 시나리오 중요도를 합의한다.
- ③ 평가팀이 품질 응답을 개선할 아키텍처 대안을 요청한다.
- ④ 아키텍트가 전략별 적용 전후 응답·비용·위험을 추정한다.
- ⑤ 평가팀이 가중 효익과 비용 비율로 전략 순위를 제출한다.
- ⑥ 투자 결정자가 예산·선행 관계·공통 자원 제약을 제시한다.
- ⑦ 평가팀이 제약과 추정 불확실성을 반영한 실행 조합·순서를 만든다.
- ⑧ 투자 결정자가 선택한 전략과 검증 지표를 확정한다.

$$B_j=\sum_i w_i\Delta U_{ij}, \qquad BCR_j=\frac{B_j}{C_j}$$

> 요약: 품질 효익을 사업 가치로 환산하고 비용·제약을 반영해 투자 조합을 정한다.
...
| 고려사항 | 문제점 | 대책 |
|:---|:---|:---|
| 효용 편향 | 이해관계자마다 품질 가치 평가가 크게 다름 | 역할별 값을 독립 수집하고 합의 근거·범위 기록 |
| 비용 누락 | 구축비만 보고 운영·전환·교육 비용 제외 | 수명주기 비용과 위험 예비비 포함 |
| 단일값 확신 | 불확실한 추정값의 작은 차이로 순위 고정 | 낙관·기준·비관 범위와 민감도 분석 수행 |
| 전략 의존성 | 개별 순위가 높아도 단독 실행 불가 | 선행·상호배타·공통 자원 제약으로 조합 평가 |
| 효익 미검증 | 투자 후 예상 품질 개선을 확인하지 않음 | 시나리오 측정값과 사후 효익 검증 계획 포함 |

> 적용 사례: 금융 플랫폼은 장애 전환 전략의 가용성 효익과 수명주기 비용으로 투자를 결정한다.
...
- CBAM은 품질 개선의 사업 효익과 수명주기 비용을 비교해 아키텍처 투자 순위를 정한다.
- 추정 불확실성·예산·전략 의존성을 반영해 실행 가능한 조합을 선택한다.
