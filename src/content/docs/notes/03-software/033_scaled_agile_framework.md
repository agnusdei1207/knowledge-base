...
- 정의/개념: 린•애자일을 전사 가치 흐름에 확장한 **애자일 프레임워크**이다.
- 배경/필요성: 독립 팀 계획만으로는 다수 팀의 **의존성•통합 일정 조정이 어려움**이 발생한다.

#### 한줄 요약

- 전략•투자•팀 의존성을 **가치 흐름 주기**로 정렬한다.
...
- **ART•PI 공통 주기**로 다수 팀 목표•의존성을 정렬한다.
- **가치 흐름**으로 전략•투자•제품 실행을 연결한다.
- **포트폴리오•프로그램 조정 계층**으로 대규모 실행을 통제한다.

#### 한줄 요약

- 다수 팀 정렬과 **조정 계층•의사결정 지연** 절충이 핵심이다.
...
- LPM•DVS•ART•애자일 팀의 **계층 구조**가 핵심이다.
...
- **통합 증분(Integrated Increment)**: 여러 팀 결과를 공동 시스템에서 결합•검증한 결과다.
...
- **1. 전략•투자 우선순위**: 가치 흐름 목표를 결정한다.
- **2. PI 목표•의존성 정렬**: 용량•위험•책임자 합의이다.
- **3. 팀 증분•시스템 통합**: 반복 결과를 공동 시스템에 반영한다.
- **4. 시스템 데모**: 여러 팀의 통합 결과를 검토한다.
- **5. 검사•적응**: 가치•위험 피드백을 다음 PI에 반영한다.

#### 한줄 요약

- 전략 정렬부터 시스템 데모까지 **PI 피드백 순환**이 핵심이다.
...
- **지역 최적화(Local Optimization)**: 개별 팀의 완료량은 늘지만 통합 가치 전달시간이나 전체 성과는 개선되지 않는 상태다.
...
- 가치 흐름•용량•흐름 지표로 **조정 범위 최소화**가 핵심이다.
...
- **가치 흐름•팀 의존성**이 크면 ART와 PI를 적용한다.

#### 한줄 요약

- 가치 흐름•팀 의존성이 크면 **ART•PI 적용**이 핵심이다.
