...
- 정의/개념: Git의 선언 상태와 실제 운영 상태를 맞추는 **지속 조정 운영 방식**이다.
- 배경/필요성: 수동 변경에 따른 **선언•실제 상태 불일치** 해소가 필요하다.

#### 한줄 요약

- Git 선언과 실제 상태의 **지속 조정 운영 방식**이 핵심이다.
...
- **지속 조정•이력**으로 드리프트 복구•감사가 핵심이다.

#### 한줄 요약

- 선언 기준선•풀 제어기•**드리프트 자동 복구**가 핵심이다.
...
선의 의미: 가로선은 승인 정책, 선언 저장소, 제어기와 대상 환경이 이루는 정적 관리 경계이고, 세로선은 제어기에 보고 채널이 결합되는 상태 인터페이스 관계를 뜻한다.
...
> 요약: 제어기가 Git의 **목표•실제 상태 차이**를 반복 관찰•조정이 핵심이다.

#### 한줄 요약

- 목표•실제 상태 비교와 **드리프트 조정 순환**이 핵심이다.
...
- 선언 상태는 GitOps, 명령 작업은 **푸시 배포**가 핵심이다.
...
- 정책 검증•비밀 보호•**최소 권한 조정 통제**가 핵심이다.
...
- 상태 선언•직접 변경 통제가 가능하면 **GitOps**, 일회성 명령 작업은 **푸시 배포**를 선택한다.

#### 한줄 요약

- 선언 가능 상태는 GitOps, 명령형 작업은 **푸시 배포**가 핵심이다.
