...
- **깃옵스(GitOps)**: ‘깃옵스’로 읽고 Git과 Operations를 결합한 관례 명칭으로 별도 영문 확장어를 만들지 않으며 Git 저장소의 선언적 목표 상태와 실제 운영 상태를 제어기가 지속적으로 일치시키는 운영 방식
...
- **쿠버네티스(Kubernetes)**: ‘쿠버네티스’로 읽는 공식 프로젝트 이름으로 별도 영문 확장어를 만들지 않으며 선언한 리소스 상태를 제어 루프로 유지하는 컨테이너 오케스트레이션 플랫폼
- **깃(Git)**: ‘깃’으로 읽는 공식 프로젝트 이름으로 별도 영문 확장어를 만들지 않으며 승인된 운영 선언과 변경 이력을 버전 관리하는 저장소 기반 도구
...
- Git 저장소의 선언이 운영 상태 기준선이다.
- 내부 제어기가 저장소에서 선언을 가져와 적용한다.
- 드리프트를 감지해 목표 상태로 지속 조정한다.
- Git 이력이 승인·감사·복귀 근거 제공한다.
...
> 요약: 제어기가 Git 선언과 대상 환경 차이를 조정

### 쉽게 이해하기 (학습용)

- 승인 보관함의 설계도를 관리 장치가 현장과 비교하고 결과를 기록판에 남긴다
...
| 절차 | 설명 |
|:---|:---|
| 선언 승인 | 검토한 선언을 운영 기준 브랜치에 병합 |
| 목표 조회 | 제어기가 승인된 목표 상태를 가져옴 |
| 상태 관찰 | 대상 환경의 현재 자원 상태 조회 |
| 실제 반환 | 관찰한 실제 상태를 제어기에 전달 |
| 차이 조정 | 드리프트가 있으면 목표 상태를 적용 |
| 결과 보고 | 동기화 상태와 실패 원인을 기록 |

> 요약: 목표·실제 상태 차이를 반복 관찰·조정
...
1. 쿠버네티스 클러스터는 Git 선언과 실제 자원을 지속 조정
2. 긴급 수동 변경은 장애 복구 후 Git 선언에 역반영

### 쉽게 이해하기 (학습용)

- 컨테이너 실행 상태가 승인 설계도와 달라지면 제어기가 다시 설계대로 맞춘다
- 장애 때문에 현장을 먼저 고쳤다면 복구 뒤 설계도에도 같은 변경을 남겨야 한다

## Ⅶ. 결론

- 선언 상태는 GitOps로 조정하고 수동 변경은 Git에 회수
