...
- ① 변경자가 검토를 통과한 선언을 운영 기준 브랜치에 병합한다.
- ② 제어기가 저장소에서 승인된 목표 상태를 가져온다.
- ③ 제어기가 대상 환경의 현재 자원 상태를 조회한다.
- ④ 대상 환경이 관찰한 실제 상태를 제어기에 반환한다.
- ⑤ 차이가 있으면 제어기가 목표 상태를 다시 적용한다.
- ⑥ 제어기가 건강·동기화 상태와 실패 원인을 기록·알림한다.
...
| 고려사항 | 문제점 | 대책 |
|:---|:---|:---|
| 잘못된 선언 | 오류 상태를 제어기가 반복 적용 | 병합 전 정책·구문·시험 환경 검증과 자동 중단 기준 적용 |
| 수동 운영 변경 | 다음 조정 때 변경이 사라지고 원인 추적 곤란 | 긴급 변경도 즉시 Git 선언에 회수하고 직접 변경 제한 |
| 비밀정보 저장 | 평문 자격 증명이 Git 이력에 영구 노출 | 외부 비밀 저장소 또는 밀봉된 비밀 사용 |
| 제어기 권한 | 제어기 침해 시 전체 환경 변경 가능 | 환경·영역별 제어기 분리와 최소 권한 적용 |
| 조정 순서 | 의존 자원이 준비되기 전 서비스 적용 | 동기화 단계·건강 조건·실패 재시도 정책 정의 |
...
- GitOps는 Git의 승인 이력과 지속 조정으로 운영 상태의 재현성과 감사를 높인다.
- 선언할 수 있는 상태에 적용하고 모든 수동 변경을 Git에 회수한다.
