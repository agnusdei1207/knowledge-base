...
- **승인 정책·상태 저장소(Approval Policy·State Repository)**: 승인 정책은 변경 권한·검토·병합 기준을 통제하고 저장소는 승인된 선언과 버전 이력을 보관한다.
...
> 요약: 제어기가 Git의 목표 상태와 실제 상태 차이를 반복 관찰·조결정
...
> 적용 사례: 쿠버네티스 클러스터는 Git 선언과 실제 자원을 지속 조결정
...
- **상태 선언 가능성·직접 변경 통제**를 보고 GitOps를 적용
