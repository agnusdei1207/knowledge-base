...
date: "2026-07-30T15:31:00+09:00"
tags:
  - "notes-software"
weight: 96
extra:
...
- **조인 연산자(Join Operator)**: 두 입력에서 조건이 맞는 행을 결합하는 실행 계획 연산자
- **인덱스(Index)**: 키와 행 위치를 정렬해 탐색 범위를 줄이는 자료구조
- **커버링 인덱스(Covering Index)**: 질의에 필요한 열을 모두 포함해 원본 행 조회를 줄이는 인덱스
- **히스토그램(Histogram)**: 열 값의 빈도와 분포를 구간별로 기록한 통계
- **확장 통계(Extended Statistics)**: 여러 열의 상관관계와 결합 분포를 나타내는 통계
- **데이터 편향(Data Skew)**: 특정 키나 파티션에 행이 불균등하게 몰린 상태
- **파티션(Partition)**: 데이터를 키나 범위에 따라 나눈 처리 단위
...
- **1. 조인 방식·입력 역할**: 크기·정렬·인덱스로 방식 결정
- **2. 첫 입력 구성**: 바깥·빌드·정렬 입력 준비
- **3. 메모리 초과 분할**: 넘친 파티션을 임시 디스크에 저장
- **4. 둘째 입력 결합**: 안쪽 탐색·프로브·병합으로 매칭
