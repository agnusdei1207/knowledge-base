---
sidebar:
  order: 96
  label: "096. 조인 알고리즘: NLJ·Hash Join·Merge Join (Join Algorithms)"
  badge:
    text: "기출 · 70%"
    variant: note
title: "096. 조인 알고리즘: NLJ·Hash Join·Merge Join (Join Algorithms)"
date: "2026-07-27T23:59:59+09:00"
tags:
  - "notes-software"
weight: 96
extra:
  question_no: "096"
  source_status: "기출"
  source_history: "137회"
  priority: 70
  priority_note: "137회 기출, 조인 방식별 비용 선택 중요"
---

## 미리 알고가기

- **중첩 반복 조인(Nested Loops Join, NLJ)**: ‘엔엘제이’로 읽고 영문 머리글자를 딴 약어이며, 바깥 입력의 각 행마다 안쪽 입력에서 일치 행을 탐색한다.
- **해시 조인(Hash Join)**: 작은 입력으로 해시 테이블을 만들고 다른 입력의 등치 키로 일치 행을 찾음
- **병합 조인(Merge Join)**: 조인 키로 정렬된 두 입력을 순서대로 비교하며 일치 행을 결합한다.
- **바깥·안쪽 입력(Outer·Inner Input)**: 중첩 반복 조인에서 반복을 시작하는 입력과 각 반복마다 탐색하는 입력
- **빌드·프로브 입력(Build·Probe Input)**: 해시 테이블을 만드는 입력과 그 테이블에서 일치 키를 찾는 입력
- **해시 버킷(Hash Bucket)**: 같은 해시값의 키와 행을 모아 두는 저장 영역
- **디스크 유출(Spill to Disk)**: 작업 메모리 부족으로 해시·정렬 중간 결과를 임시 디스크에 쓰는 현상
- **카디널리티(Cardinality)**: 연산자에 입력되거나 출력되는 행의 수
- **입출력(Input/Output, I/O)**: ‘아이오’로 읽고 영문 머리글자를 딴 약어이며, 저장장치의 읽기·쓰기 동작

## Ⅰ. 개요

- 정의/개념: 조건이 일치하는 두 입력 행을 결합하는 **실행 알고리즘**
- 배경/필요성: 입력·정렬·메모리별 조인 비용 최소화

### 쉽게 이해하기 (학습용)

- 한 명씩 찾기, 색인표 만들기, 정렬된 명단 함께 읽기 중 조건에 맞는 방법을 고른다.

## Ⅱ. 특징

- **중첩 반복**: 바깥 행마다 안쪽 입력 탐색
- **해시 조인**: 빌드 해시로 프로브 입력 탐색
- **병합 조인**: 정렬된 두 입력을 순차 비교

### 쉽게 이해하기 (학습용)

- 결과 건수, 조인 조건, 정렬 여부, 작업 메모리에 따라 가장 적은 읽기와 비교를 만드는 방식이 달라진다.

## Ⅲ. 구조 및 구성요소

```mermaid
block
  columns 5
  I["입력"]
  C["조인 조건"]
  R["입력 역할"]
  M["작업 메모리"]
  T["임시 디스크"]
```

| 구성요소 | 책임 |
|:---|:---|
| 입력 | 조인할 왼쪽·오른쪽 행 제공 |
| 조인 조건 | 등치·범위 결합 규칙 정의 |
| 입력 역할 | 바깥·안쪽·빌드·프로브 지정 |
| 작업 메모리 | 해시·정렬·중간 결과 저장 |
| 임시 디스크 | 메모리 초과 결과 보관 |

### 쉽게 이해하기 (학습용)

- 두 입력의 역할과 조건, 작업 공간을 정해 조인 방식을 실행한다.

## Ⅳ. 흐름도

```mermaid
sequenceDiagram
    participant Q as 실행 엔진
    participant L as 왼쪽 입력
    participant J as 조인 연산자
    participant R as 오른쪽 입력
    participant T as 임시 디스크
    Q->>J: 1. 조인 방식·입력 역할 지정
    L->>J: 2. 바깥·빌드·정렬 입력 전달
    J->>T: 3. 메모리 초과 파티션 저장
    T-->>J: 4. 처리할 파티션 반환
    R->>J: 5. 안쪽·프로브·정렬 입력 전달
    J->>T: 6. 미처리 중간 결과 저장
    J-->>Q: 7. 결합 결과·자원량 반환
```

**동작 원리**

- **1. 조인 방식·입력 역할 지정**: 실행 방식을 결정
- **2. 바깥·빌드·정렬 입력 전달**: 첫 입력을 전달
- **3. 메모리 초과 파티션 저장**: 넘친 데이터를 기록
- **4. 처리할 파티션 반환**: 나눈 입력을 조회
- **5. 안쪽·프로브·정렬 입력 전달**: 둘째 입력을 전달
- **6. 미처리 중간 결과 저장**: 남은 결과를 보관
- **7. 결합 결과·자원량 반환**: 행과 유출량을 통지

### 쉽게 이해하기 (학습용)

- 두 명단을 작업대에서 맞추다가 공간이 부족하면 일부를 임시 보관소에 두고 나누어 처리한다.

## Ⅴ. 종류 및 비교

| 판단 기준 | 중첩 반복 조인 | 해시 조인 | 병합 조인 |
|:---|:---|:---|:---|
| 적용 기준 | 작은 바깥·빠른 안쪽 탐색 | 큰 입력의 등치 조인 | 정렬 입력·범위 조인 |
| 핵심 특징 | 바깥 행마다 안쪽 탐색 | 해시 생성 후 프로브 | 두 입력을 순차 비교 |
| 한계 | 반복 읽기 폭증 | 메모리 부족 시 유출 | 미정렬 입력의 정렬 비용 |

> “작은 입력”은 테이블 전체 크기가 아니라 앞선 필터를 통과해 조인 연산자에 실제 들어오는 행 수를 뜻

### 쉽게 이해하기 (학습용)

- 소량은 하나씩 찾고, 대량 등치는 해시표로 찾고, 정렬된 명단은 나란히 읽는다.

## Ⅵ. 실무 고려사항 및 대책

| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 입력 행 추정 | 예상·실제 행 비교와 통계 교정 | 잘못된 조인 선택 방지 |
| 입력 순서 | 필터 시점과 빌드·바깥 입력 확인 | 반복 탐색·해시 크기 감소 |
| 인덱스 | 조인 키와 커버링 효과 검증 | NLJ 임의 읽기 감소 |
| 작업 메모리 | 유출량 감시와 메모리 조정 | 디스크 유출 감소 |
| 데이터 편향 | 히스토그램·확장 통계 적용 | 파티션 불균형 완화 |

> **적용 사례**: 고객 한 명의 주문 조회는 인덱스 NLJ가 유리할 수 있지만 전체 고객 정산은 해시 조인의 순차 읽기가 유리할 수 존재

### 쉽게 이해하기 (학습용)

- 같은 테이블도 한 고객을 찾을 때와 모든 고객을 처리할 때 알맞은 결합 방식이 다르다.

## Ⅶ. 결론

- **입력 행·조건·정렬·메모리**에 따라 조인 방식을 결정

### 쉽게 이해하기 (학습용)

- 결합 방법을 외우기보다 들어오는 데이터 양과 준비 상태에 맞는 방법을 고른다.
