...
- **1. 조인 방식·입력 역할 지정**: 실행 방식을 정한다.
- **2. 바깥·빌드·정렬 입력 전달**: 첫 입력을 보낸다.
- **3. 메모리 초과 파티션 저장**: 넘친 데이터를 기록한다.
- **4. 처리할 파티션 반환**: 나눈 입력을 읽는다.
- **5. 안쪽·프로브·정렬 입력 전달**: 둘째 입력을 보낸다.
- **6. 미처리 중간 결과 저장**: 남은 결과를 보관한다.
- **7. 결합 결과·자원량 반환**: 행과 유출량을 알린다.
...
- **입력 행·조건·정렬·메모리**에 따라 조인 방식을 정한다.
