...
- 정의/개념: 조건이 일치하는 두 입력 행을 결합하는 **실행 알고리즘**이다.
- 배경/필요성: 단일 조인 방식은 **입력 규모 변화**에 취약해진다.
...
- **입력•조건•정렬 상태 판정**: 행 수와 조인 조건 및 정렬 여부를 확인하는 단계다.
- **중첩 반복 탐색**: 작은 바깥 입력마다 안쪽 인덱스를 찾는 단계다.
- **해시 빌드•프로브**: 등치 키 해시를 만들고 일치 버킷을 찾는 단계다.
- **정렬 입력 병합**: 정렬된 두 입력을 순차 비교하는 단계다.
...
- **1. 입력•조건•정렬 상태 판정**: 행 수•조건•정렬을 확인한다.
- **2. 중첩 반복 탐색**: 바깥 행마다 안쪽 인덱스 탐색이다.
- **3. 해시 빌드•프로브**: 등치 키의 일치 버킷 탐색이다.
- **4. 정렬 입력 병합**: 두 입력을 순차를 비교한다.
...
- 소량•인덱스는 **NLJ**, 대량 등치는 **해시**, 정렬 입력은 **병합**을 선택한다.
