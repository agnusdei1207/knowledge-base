...
- **정의**: 두 개 이상의 테이블을 연결할 때 데이터 스케일, 인덱스 보유 유무, 정렬 상태에 따라 선택하는 3가지 물리적 조인 수행 방식인 **중첩 루프 조인(NLJ)**, **해시 조인(Hash Join)**, **정렬 머지 조인(Sort Merge Join)**.
- **필요성**: 데이터 스케일과 인덱스 배치에 적합하지 않은 조인 선택 시 쿼리 응답시간 폭증 방지 및 서비스 유형에 최적화된 조인 수행 체계 수립.
...
- **중첩 루프 조인(Nested Loop Join)**: 소용량(OLTP) 전용, 드리븐(Inner) 테이블의 B+Tree 인덱스 필수.
- **해시 조인(Hash Join)**: 대용량(OLAP) 전용, 인덱스 불필요, 오직 동등(`=`) 조건에서만 작동.
- **정렬 머지 조인(Sort Merge Join)**: 조인 키 정렬 필수, 범위(`>`, `<`) 조인 가능.
...
│                        3대 조인 알고리즘 아키텍처                      │
├───────────────────┬───────────────────┬────────────────────────────────┤
│ 1. 중첩 루프 조인 │ 2. 해시 조인      │ 3. 정렬 머지 조인              │
├───────────────────┼───────────────────┼────────────────────────────────┤
│ Outer Row 1 ──────►│ Build Input ──►   │ Table A (정렬)                 │
│   Inner 인덱스 탐색│  [해시 테이블]     │   │ (병렬 머지 스캔)           │
│ Outer Row 2 ──────►│ Probe Input ──►   │ Table B (정렬)                 │
│   Inner 인덱스 탐색│  [매칭 버킷]       │                                │
└───────────────────┴───────────────────┴────────────────────────────────┘
```
...
- **조인 수립 기준 적용** (OLTP 서비스는 `NLJ` + `Inner B+Tree Index`, 대용량 분석은 `Hash Join` 필수 수용)
