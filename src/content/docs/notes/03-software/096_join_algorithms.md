...
- 배경/필요성: 관계형 데이터베이스에서 복수의 릴레이션을 결합할 때 데이터 크기, 인덱스 보유 여부, 정렬 상태 및 조인 연산자 조건에 따라 부적절한 알고리즘을 적용할 경우 발생하는 극심한 반복 인덱스 탐색 비용($O(M \cdot \log N)$)과 디스크 스필(Disk Spill) 병목을 방지하기 위해, 소량 OLTP에 특화된 NLJ(Nested Loop Join), 대용량 동등 조인에 최적화된 Hash Join, 정렬된 대규모 집합에 적합한 Sort Merge Join의 3대 물리 조인 메커니즘을 CBO 옵티마이저가 비용 기반으로 동적 선택하여 **SQL 조인 질의의 처리 시간과 시스템 자원 효율을 극대화**할 필요
