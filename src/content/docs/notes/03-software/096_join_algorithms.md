...
- 배경/필요성: 조인 방식마다 비용이 걸리는 지점이 달라 소량에 유리한 반복 탐색을 대용량에 쓰면 바깥 행 수만큼 안쪽 접근이 곱해지고 반대로 해시 테이블이 메모리를 넘으면 디스크 스필 비용이 새로 생기므로, 데이터 규모·인덱스·정렬 상태를 근거로 옵티마이저가 알고리즘을 대신 고르는 선택 계층을 실행 계획 안에 둘 필요
...
- OLTP 트랜잭션은 **NLJ**, 대용량 분석은 **Hash Join** 선택
