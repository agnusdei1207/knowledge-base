...
- 정의/개념: 도메인 책임으로 데이터를 제품화하는 **데이터 메시**이다.
- 배경/필요성: 중앙 데이터팀 집중으로 업무 맥락 전달 지연•품질 책임 공백이 발생한다.
...
- 업무 팀이 데이터 수명주기를 책임지는 **도메인 소유권**이 핵심이다.
- 계약•SLO•문서를 한 제공 단위로 묶는 **데이터 제품**이 핵심이다.
- 셀프서비스 플랫폼과 **연합 거버넌스**를 결합한 공통 기반이 핵심이다.
...
| 구성요소 | 책임 |
|:---|:---|
| 도메인 팀•제품 소유자 | **도메인 팀**과 **제품 소유자**가 제품의 의미•품질•호환성•지원 책임 |
| 데이터 제품 | **데이터 제품**이 데이터•인터페이스•계약•SLO 제공 |
| 셀프서비스 플랫폼 | **셀프서비스 플랫폼**이 표준 저장•처리•배포•관찰 도구 제공 |
| 연합 거버넌스 | **연합 거버넌스**가 공통 정책 합의와 자동 검사 연결 |
| 카탈로그•제품 그래프 | **카탈로그**와 **제품 그래프**가 검색•계보•소유자•접근 정보 연결 |
...
- **제품 계약•SLO 정의**: 도메인이 의미•품질•지원 책임을 제품 단위로 명시하는 단계이다.
- **표준 경로 기반 제품 구현**: 셀프서비스 플랫폼으로 저장•배포•관찰 기능을 구성하는 단계이다.
- **연합 정책 자동 검증**: 보안•호환성•상호운용 규칙과 증거를 판정하는 단계이다.
- **제품•계보•접근 정보 게시**: 승인된 계약•소유자•접근 절차를 카탈로그에 공개하는 단계이다.
- **계약 위반•품질 피드백**: 소비 중 발견한 문제를 책임 도메인에 전달하는 단계이다.
...
- **1. 제품 계약•SLO 정의**: **제품 계약•SLO 정의**는 의미•품질•지원 책임을 명시한다.
- **2. 표준 경로 기반 제품 구현**: **표준 경로 기반 제품 구현**은 공통 저장•배포•관찰 기능을 구성한다.
- **3. 연합 정책 자동 검증**: **연합 정책 자동 검증**은 보안•호환성•상호운용 규칙을 판정한다.
- **4. 제품•계보•접근 정보 게시**: **제품•계보•접근 정보 게시**는 계약•소유자•접근 절차를 공개한다.
- **5. 계약 위반•품질 피드백**: **계약 위반•품질 피드백**은 문제를 책임 도메인에 전달한다.
...
|:---|:---|:---|
| 적용 기준 | **데이터 메시**: 다수 도메인•중앙 병목 | **중앙 데이터 플랫폼**: 소규모•공통 요구 중심 |
...
- **서비스 수준 목표(Service Level Objective, SLO)**: 데이터 제품이 약속한 품질•최신성•가용성의 측정 목표이다.
...
| 계약•서비스 수준 목표가 없으면 소비자가 품질 수준 판단 불가 | **데이터 계약**•**서비스 수준 목표(SLO)**•**관찰성** 적용 | 소비자의 품질 검증 가능 |
...
| 공통 식별자•의미가 다르면 제품 조합 실패 | **공통 식별자(Common ID)**•**의미 규칙**•**호환성 규칙** 적용 | 데이터 제품의 상호운용성 확보 |
...
- 도메인 수와 중앙 병목이 커지면 **제품 소유권•셀프서비스 플랫폼**을 갖춘 데이터 메시로 전환한다.
