...
- **서비스 수준 목표(Service Level Objective, SLO)**: 영문 첫 글자를 딴 SLO를 '에스엘오'로 읽으며 데이터 제품의 최신성·완전성·가용성 목표를 뜻한다.
...
- **공통 식별자(Common Identifier, ID)**: ID를 '아이디'로 읽고 영문 첫 글자로 식별자를 줄여 쓰며 여러 도메인의 같은 서비스·고객을 연결하는 키이다.
...
- **도메인 소유권**: 업무 팀이 데이터 수명주기를 책임
- **데이터 제품**: 계약·SLO·문서를 제공 단위로 통합
- **공통 기반**: 셀프서비스 플랫폼과 연합 정책을 적용
...
- **1. 계약·SLO 포함 제품 배포**: 도메인이 제공 책임을 명시
- **2. 보안·호환성 정책 검사**: 플랫폼이 연합 규칙을 자동 판정
- **3. 위반·게시 승인 반환**: 수정 항목이나 게시 결과를 전달
- **4. 제품·계보·접근 등록**: 승인 정보를 카탈로그에 공개
- **5. 목적·품질 조건 검색**: 소비자가 사용할 제품을 검색
- **6. 계약·소유자 정보 제공**: 품질 목표와 문의 경로를 제시
- **7. 제품 사용·품질 문의**: 계약에 따라 사용하고 문제를 전달
...
- **도메인 책임·제품 계약·플랫폼 역량·연합 정책**으로 데이터 메시를 도입
