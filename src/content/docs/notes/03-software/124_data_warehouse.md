...
- 배경/필요성: 운영계 OLTP 데이터베이스에 직접 대규모 통계/집계 질의를 수행할 때 발생하는 트랜잭션 락 경합 및 서비스 성능 저하, 각 부서별 사일로 시스템에 흩어진 데이터의 정의 불일치(Single Source of Truth 부재) 문제를 해소하기 위해, 전사 데이터를 주제별로 통합하고 시계열 이력을 비휘발성으로 보존하는 4대 특성(주제 지향성·통합성·시계열성·비휘발성)과 스타/스노우플레이크 차원 모델링(Dimensional Modeling)을 갖춘 분석 전용 데이터 웨어하우스를 구축하여 **운영계 부하를 완벽히 격리하고 전사 단일 진실 공급원 기반의 고속 OLAP 분석을 실현**할 필요
...
- 엔터프라이즈 비즈니스 인텔리전스(BI), 경영진 의사결정 및 전사 지표 관리의 **핵심 표준 분석 인프라**로 확립되었으며, 현대 클라우드 환경에서는 **Snowflake/BigQuery 등 클라우드 DW와 dbt를 결합한 모던 데이터 스택(MDS/ELT)으로의 전환, 컬럼 스토리지(Parquet) 기반 압축/파티셔닝, 차원 변경 이력을 엄격히 보존하는 SCD Type 2 설계, 전사 지표 왜곡을 차단하는 시맨틱 레이어(Metric Layer) 거버넌스**를 결합하여 고품질 데이터 자산화를 달성
