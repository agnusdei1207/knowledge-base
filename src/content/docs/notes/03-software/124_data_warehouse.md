...
운영계 OLTP 원천 시스템 데이터 발생
        │
   [데이터 추출] Debezium CDC 또는 배치 스케줄러를 통해 변경 로그 추출 (Staging)
        │
   [품질 정제] 전사 공통 코드 표준화, 결측치 보정, 중복 제거 및 데이터 클렌징
        │
   [Fact/Dim 적재] 정해진 Grain 입도와 SCD Type 2 정책에 따라 사실/차원 테이블 적재
        │
   [데이터 마트 롤업] 마케팅, 재무 등 주제별 Data Mart 및 사전 집계(Aggregation) 뷰 생성
        │
   Tableau, PowerBI, SQL 클라이언트를 통해 경영진과 분석가에게 다차원 리포트 서빙
```
