...
date: "2026-07-25T11:18:00+09:00"
tags:
  - "notes-software"
weight: 124
extra:
...
- **정의/개념**: 운영 이력을 통합해 분석에 제공하는 저장소다
- **배경/필요성**: 분석 질의로 인한 운영 거래 지연을 막는다
...
- 공통 코드·단위·키는 지표 불일치를 막는다
- 사실·차원 분리는 분석 관점의 재사용 비용을 줄인다
- 열 저장·병렬 처리는 대량 집계 시간을 줄인다
- 적재 지연·품질 위반은 낡거나 틀린 의사결정을 만든다
...
| 설계 요소 | 설명 |
|:---|:---|
| 원천·스테이징 | 원천별 추출 데이터를 임시 저장 |
| ETL·품질 | 코드 변환·중복·누락 규칙 검사 |
| 사실·차원 모델 | 측정값과 분석 관점을 분리 |
| 웨어하우스 저장소 | 이력을 열 형식으로 저장·병렬 질의 |
...
| 절차 | 설명 |
|:---|:---|
| 변경 데이터 추출 | 원천 변경 기준으로 적재 대상을 추출 |
| 표준 데이터 전달 | 형식·코드를 통일하고 품질 위반 격리 |
| 차원·사실 적재 | 공통 키·이력과 측정값을 분리 저장 |
| 통합 이력 제공 | 의미 계층에 시점별 통합 데이터 제공 |
...
| 판단 기준 | 데이터 웨어하우스 | OLTP 데이터베이스 |
|:---|:---|:---|
| 핵심 특징 | 사실·차원 모델로 이력을 대량 집계함 | 정규화 모델로 현재 거래를 처리함 |
| 적용 기준 | 이력 조회·다차원 지표 분석 | 짧은 응답·현재 상태 무결성 |
| 주요 위험 | 적재 지연·지표 정의 불일치 | 분석 질의로 인한 운영 부하 |
...
1. 수익 분석은 주문 사실로 공통 매출을 계산함
2. 비용 분석은 당시 조직 기준으로 비용을 집계함

### 쉽게 이해하기 (학습용)
- 주문 측정값과 상품·시간 차원을 분리하면 여러 보고서가 같은 매출 정의와 분류 기준을 재사용할 수 있음
- 조직 차원의 변경 이력을 보존하면 현재 조직이 아니라 비용 발생 당시 조직 기준으로 과거 수치를 재현할 수 있음

## Ⅶ. 결론

- 이력 분석·공통 지표가 필요하면 웨어하우스를 택한다
