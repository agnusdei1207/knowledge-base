...
- **1. Snapshot·변경분 추출**: 원천 데이터를 받는다.
- **2. 차원 이력 적재**: 기준 정보를 먼저 넣는다.
- **3. 대체키·품질 반환**: 연결 키를 확인한다.
- **4. 사실 데이터 적재**: 측정값을 연결한다.
- **5. 적재 결과 반환**: 건수·오류를 확인한다.
- **6. 지표·시점 갱신**: 최신 기준을 공개한다.
- **7. 분석 질의**: 공통 정의로 조회한다.
...
- **사실 Grain·차원 이력·지표 정의·데이터 시점**으로 DW를 설계한다.
