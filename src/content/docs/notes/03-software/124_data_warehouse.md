...
- 배경/필요성: 운영 DB에 직접 통계 질의를 걸면 트랜잭션과 같은 자원을 나눠 쓰게 되어 서비스 응답까지 함께 느려지고 시스템마다 지표 정의가 갈려 같은 질문에 다른 답이 나오므로, 이력을 갱신하지 않는 별도 저장소로 옮겨 분석 부하를 떼어 내고 지표 정의를 한 곳으로 모을 필요
