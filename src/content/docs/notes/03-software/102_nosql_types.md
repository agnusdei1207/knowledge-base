...
- **1. 모델 연산 요청**: 키와 연산을 보낸다.
- **2. 저장 단위·키 전달**: 라우팅 정보를 만든다.
- **3. 대상 파티션 전달**: 저장 위치로 보낸다.
- **4. 모델별 변경 복제**: 사본에 전파한다.
- **5. 복제 확인 반환**: 정책 수준을 확인한다.
- **6. 처리 결과 전달**: API에 결과를 보낸다.
- **7. 모델 API 응답**: 응용에 반환한다.
...
- **조회·갱신·일관성·전환 비용**으로 NoSQL 유형을 정한다.
