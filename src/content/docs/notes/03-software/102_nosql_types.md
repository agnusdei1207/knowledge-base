...
- 배경/필요성: RDBMS는 고정 스키마와 조인으로 정합성을 얻는 대신 노드 경계를 넘는 조인·트랜잭션 때문에 수평 확장이 막히고 스키마 변경 비용도 데이터량에 비례해 커지므로, 조인과 스키마 강제를 포기하는 대가로 접근 패턴에 특화된 4대 모델을 두어 확장 경로를 여는 저장 계층이 필요
