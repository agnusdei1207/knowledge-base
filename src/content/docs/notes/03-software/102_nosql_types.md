...
- 배경/필요성: 관계형 데이터베이스(RDBMS)의 엄격한 정형 스키마, 무거운 조인 연산 및 수직 확장(Scale-Up) 한계로 인해 빅데이터 시대의 반정형/비정형 데이터 처리와 초대용량 분산 확장에 제약이 발생하는 문제를 극복하고, 데이터 접근 패턴과 도메인 특성에 맞추어 Key-Value, Document, Wide-Column, Graph 4대 특화 모델을 제공함으로써 **가변 스키마(Schema-less) 유연성과 무제한 수평 분산(Scale-Out) 처리 역량을 확보**할 필요
