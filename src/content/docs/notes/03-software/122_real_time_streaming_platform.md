...
- 정의/개념: 연속 이벤트를 보존•상태 처리해 서비스에 반영하는 **스트리밍 체계**이다.
- 배경/필요성: 생산자와 처리기의 직결은 입력 급증 시 **처리 지연•손실** 확대가 발생한다.
...
- 브로커의 **분리•완충**으로 생산자와 처리기의 속도 차이 흡수가 핵심이다.
- **이벤트 시간•워터마크**로 지연 이벤트의 윈도 집계가 핵심이다.
...
- **1. 파티션 로그 기록**: 파티션 키로 순서 경계를 정해 스키마와 함께 보존한다.
- **2. 오프셋 순서 재생**: 파티션별 저장 위치 순서로 처리기에 전달한다.
- **3. 이벤트 시간 상태 처리**: 키 상태와 워터마크로 윈도•집계를 수행한다.
- **4. 위치•상태 체크포인트**: 같은 시점의 오프셋과 연산 상태 저장한다.
- **5. 외부 결과 확정**: 복구점과 연결된 결과만 트랜잭션•멱등을 반영한다.
...
- 재생 보존과 상태 계산이 모두 필요하면 **브로커•처리 엔진** 분리 구축이 핵심이다.
