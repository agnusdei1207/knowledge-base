...
- 배경/필요성: 기존의 전통적인 배치 파이프라인(T+1일 또는 수 시간 주기)이 유발하는 데이터 반영 지연으로 인해 실시간 이상금융거래 탐지(FDS), 동적 가격 책정, 실시간 추천 및 관제 대시보드 요건에 즉시 대응하지 못하는 한계를 극복하고, 변경 데이터 캡처(CDC/Kafka Connect 수집), 분산 큐잉(Kafka 버퍼링), 연속 상태 연산(Flink 처리), 고속 인덱싱(Redis/Elasticsearch 서빙)의 4대 계층을 유기적으로 연계하여 **이벤트 발생부터 화면 표출까지 전 구간을 1초 미만(Sub-second) 초저지연으로 통합 처리**할 필요
