...
- **분리•완충**: 브로커가 생산자와 처리기 사이에서 이벤트를 보존해 처리 속도 차이와 일시 장애를 흡수하는 특성이다.
- **이벤트 시간•워터마크**: 사건 발생 시각으로 윈도를 계산하고 늦은 이벤트를 기다릴 경계를 나타내는 시간 기준이다.
...
- **생산자•커넥터**: 원천 이벤트를 수집하고 스키마에 맞춰 직렬화해 브로커로 보내는 구성요소이다.
- **스트림 처리기**: 이벤트 시간과 키별 상태로 윈도•집계•변환을 수행하는 구성요소이다.
- **체크포인트•상태 저장소**: 입력 위치와 연산 상태를 같은 복구점으로 보존하는 구성요소이다.
- **싱크•서빙 계층**: 처리 결과를 외부 저장소에 반영하고 서비스의 저지연 조회에 제공하는 계층이다.
...
- **스키마 호환성•버전 규칙**: 생산자 변경 뒤에도 신규•기존 소비자가 이벤트를 함께 해석하도록 스키마 진화 범위를 정하는 기준이다.
- **파티션 키**: 함께 순서를 지킬 사건과 실제 분포를 반영해 이벤트의 로그 위치를 결정하는 키이다.
- **워터마크•허용 지연**: 이벤트 시간 윈도를 닫을 진행 표식과 늦은 사건을 추가 수용할 시간이다.
- **상태 만료•키별 용량 한도**: 오래된 키 상태를 제거하고 하나의 키가 사용할 저장 공간을 제한하는 통제이다.
