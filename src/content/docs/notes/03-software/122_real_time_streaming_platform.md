...
- **1. 파티션 로그 기록**: 파티션 키로 순서 경계를 정해 스키마와 함께 보존
- **2. 오프셋 순서 재생**: 파티션별 저장 위치 순서로 처리기에 전달
- **3. 이벤트 시간 상태 처리**: 키 상태와 워터마크로 윈도•집계 수행
- **4. 위치•상태 체크포인트**: 같은 시점의 오프셋과 연산 상태 저장
- **5. 외부 결과 확정**: 복구점과 연결된 결과만 트랜잭션•멱등 반영
