...
- 정의/개념: 연속 이벤트를 보존•상태 처리해 서비스에 반영하는 **실시간 스트리밍 플랫폼**이다.
- 배경/필요성: 생산자와 처리기의 직결은 입력 급증 시 처리 지연•손실 확대를 유발한다.
...
- 브로커의 **계층 분리**와 **입력 완충**으로 생산자와 처리기의 속도 차이 흡수가 핵심이다.
- **이벤트 시간**과 **워터마크**로 지연 이벤트의 윈도 집계가 핵심이다.
- 입력 위치•상태•출력을 연결한 **종단 복구**가 핵심이다.
...
| 구성요소 | 책임 |
|:---|:---|
| 생산자•커넥터 | **생산자**가 보낸 이벤트를 **커넥터**가 수집•직렬화 |
| 브로커•파티션 로그 | **브로커**가 **파티션 로그**에 이벤트 순서•재생 위치 보존 |
| 스트림 처리기 | **스트림 처리기**가 이벤트 시간 기반 윈도•상태 집계 |
| 체크포인트•상태 저장소 | **체크포인트**와 **상태 저장소**가 입력 위치•연산 상태 보존 |
| 싱크•서빙 계층 | **싱크**가 결과를 저장하고 **서빙 계층**이 저지연 조회 제공 |
...
- **파티션 로그 기록**: 키와 스키마를 포함한 이벤트를 순서 경계별로 보존하는 단계이다.
- **오프셋 순서 재생**: 파티션별 저장 위치 순서로 이벤트를 처리기에 전달하는 단계이다.
- **이벤트 시간 상태 처리**: 키 상태와 워터마크로 윈도•집계를 수행하는 단계이다.
- **위치•상태 체크포인트**: 같은 시점의 오프셋과 키 상태를 복구점으로 저장하는 단계이다.
- **외부 결과 확정**: 복구점과 연결된 결과만 트랜잭션이나 멱등 방식으로 반영하는 단계이다.
...
- **1. 파티션 로그 기록**: **파티션 로그 기록**은 파티션 키로 순서 경계를 정해 스키마와 함께 보존한다.
- **2. 오프셋 순서 재생**: **오프셋 순서 재생**은 파티션별 저장 위치 순서로 처리기에 전달한다.
- **3. 이벤트 시간 상태 처리**: **이벤트 시간 상태 처리**는 키 상태와 워터마크로 윈도•집계를 수행한다.
- **4. 위치•상태 체크포인트**: **위치•상태 체크포인트**는 같은 시점의 오프셋과 연산 상태를 저장한다.
- **5. 외부 결과 확정**: **외부 결과 확정**은 복구점과 연결된 결과만 트랜잭션•멱등 방식으로 반영한다.
...
|:---|:---|:---|:---|
| 적용 기준 | **Apache Kafka**: 다중 소비와 이벤트 재생 중심 | **Apache Flink**: 저지연•복잡한 상태 처리 중심 | **Spark Structured Streaming**: 배치와 스트림 처리 통합 |
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 생산자별 스키마 변경이 다르면 소비자 역직렬화 실패 | **스키마 호환성**과 **버전 규칙** 적용 | 신규•기존 소비자의 동시 처리 유지 |
| 특정 키의 입력 집중으로 **핫키**와 **처리 지연** 발생 | 순서 경계와 분포를 반영한 **파티션 키** 선택 | 파티션별 처리량 편차 완화 |
| 늦은 이벤트가 윈도 종료 뒤 도착해 집계 누락 발생 | **워터마크**와 **허용 지연** 정책 설정 | 이벤트 시간 집계 정확도 향상 |
| 키 상태가 만료 없이 누적돼 **상태 저장소** 급증 | **상태 만료**와 **키별 용량 한도** 설정 | 상태 크기와 복구 시간 제한 |
...
- 재생 보존과 상태 계산이 모두 필요하면 **책임 분리** 원칙에 따라 **브로커**와 **스트림 처리기**를 분리 구축한다.
