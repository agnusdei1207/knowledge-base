...
- **1. 키·스키마 이벤트 기록**: 브로커에 보낸다.
- **2. 파티션 순서 전달**: 처리기로 전달한다.
- **3. 위치·상태 스냅숏**: 복구 경계를 저장한다.
- **4. 복구점 영속 완료**: 저장을 확인한다.
- **5. 계산 결과 반영**: Sink에 기록한다.
- **6. 외부 결과 확정**: 결과 커밋을 확인한다.
- **7. 완료 위치 커밋**: 재시작 기준을 정한다.
...
- **순서·스키마·지연·상태·Sink 보장**으로 스트리밍 경계를 정한다.
