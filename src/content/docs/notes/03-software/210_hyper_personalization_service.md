...
- **정의/개념**: 개인 이력·현재 맥락의 실시간 제안
- **배경/필요성**: 과거 추천의 맥락 지연으로 실시간 개인화 필요
...
- 실시간 맥락 피처가 같은 개인의 순간별 제안을 변경한다.
- 동의 목적·보존 기간이 수집·활용 가능한 데이터 제한한다.
- 클릭만 최적화하면 반복 노출·필터 버블로 장기 가치 저하된다.
- 피드백 지연·편향이 모델 갱신과 효과 판정을 왜곡한다.
...
| 피처 스토어 | 이력·실시간 피처를 같은 정의로 제공 |
| 동의·노출 정책 | 목적·보존·빈도·다양성 제약 적용 |
| 의사결정 엔진 | 후보 점수와 정책으로 최종 제안 선택 |
| 채널·콘텐츠 제공기 | 선택 제안을 앱·웹·메시지에 노출 |
...
| 절차 | 설명 |
|:---|:---|
| 맥락·동의 검증 | 요청 맥락과 수집·활용 허용 범위 확인 |
| 허용 피처 조회 | 목적에 필요한 최소 피처만 요청 |
| 이력·실시간 피처 전달 | 같은 시점·정의의 개인 피처 제공 |
| 후보·점수 제출 | 예측 점수와 선택 근거를 정책에 전달 |
| 제안 노출 | 빈도·다양성 제약 통과 후보 제공 |
| 반응·거부 전달 | 노출·클릭·구매·거부 결과 기록 |
...
| 판단 기준 | 초개인화 | 개인 추천 | 세그먼트 제안 |
|:---|:---|:---|:---|
| 핵심 특징 | 개인 이력·현재 맥락 결합 | 개인 과거 이력 중심 추천 | 집단 속성별 같은 제안 |
| 적용 기준 | 실시간 맥락·채널 연계 필요 | 반복 이용·개인 이력 충분 | 데이터 적음·단순 캠페인 |
| 주요 위험 | 개인정보·피로도·필터 버블 | 과거 취향 고착 | 집단 내 차이 무시 |
...
1. 금융 앱은 동의한 소비 맥락으로 적합 혜택을 제한 노출
2. 콘텐츠 서비스는 다양성 제약으로 반복 주제 노출 완화

### 쉽게 이해하기 (학습용)

- 금융 앱은 사용 목적에 동의한 정보만 쓰고 같은 혜택을 너무 자주 보여 주지 않는다
- 콘텐츠 서비스는 클릭한 주제만 반복하지 않고 다른 분야 후보도 섞어 선택 폭을 보존한다

## Ⅶ. 결론

- 실시간 맥락은 동의·빈도·다양성 제약 안에서 제안에 활용
