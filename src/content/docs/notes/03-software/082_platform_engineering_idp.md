...
- **IDP(Internal Developer Platform, 내부 개발자 플랫폼)**: 개발자가 검증된 템플릿과 자동화로 환경•배포•운영 기능을 직접 사용하는 셀프서비스 플랫폼이다.
...
- **API(Application Programming Interface)**: 프로그램이 다른 시스템의 기능을 정해진 요청•응답 규칙으로 호출하는 접점이다.
...
- **책임 경계•진단•원시 도구 접근**: 플랫폼과 사용팀의 장애 처리 범위를 구분하고 필요할 때 하부 상태와 도구를 직접 확인하게 하는 복구 수단이다.
- **시간•실패율•우회율•만족도**: 표준 경로가 실제로 빠르고 안정적이며 개발자에게 유용한지 함께 판단하는 성과 지표이다.
