...
- **1. 카탈로그 템플릿 요청**: 표준 환경을 선택한다.
- **2. 매개변수·소유 정보 전달**: 자동화 입력을 보낸다.
- **3. 정책 평가 요청**: 보안·비용 기준을 검사한다.
- **4. 허용·거부·보완 결과**: 판정 근거를 반환한다.
- **5. 검증 자원·파이프라인 생성**: 허용 요청을 실행한다.
- **6. 배포·건강 상태**: 실행 결과를 전달한다.
- **7. 운영 접점·피드백 요청**: 소유 정보와 개선점을 남긴다.
...
- **반복 수요·정책 통제·개발자 성과**로 IDP 범위를 정한다.
