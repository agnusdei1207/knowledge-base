...
- **지수 백오프·무작위 지연(Exponential Backoff·Jitter)**: 재시도 간격을 늘리고 무작위 시간을 더해 동시 재시도 폭증을 줄이는 방식이다.
- **수요 묶음·초과 정책**: 구독자가 한 번에 요청할 신호 수를 정하고 버퍼 한도를 넘은 신호를 거절·폐기·대기시킬지 정하는 규칙이다.
...
- **재시도 상한·멱등성**: 재실행 횟수를 제한하고 같은 요청을 반복해도 결과가 달라지지 않게 해 과부하와 중복 처리를 막는 성질이다.
...
- **생산·소비 속도 차**: 발행 속도가 구독자의 처리 속도를 넘어 역압이 필요한 상태다.
