...
- 정의/개념: 업무별 서비스가 데이터•배포를 소유하는 **분산 아키텍처**이다.
- 배경/필요성: 단일 배포 구조로는 **업무별 변경•확장•장애 격리**하기 어렵다.

#### 한줄 요약

- 업무별 **데이터•배포 경계**를 독립 서비스로 분리한다.
...
- **타임아웃(Timeout)**: 원격 응답을 기다리는 시간을 제한하는 제어다.
- **격리(Isolation)**: 실패 자원의 영향 범위를 분리하는 제어다.
- **복구(Recovery)**: 재시도와 대체 및 재시작으로 기능을 회복하는 처리다.

</details>

- **업무•데이터•배포 책임**을 서비스 경계로 정렬한다.
- **버전된 API•이벤트 계약**으로 내부 구현 은닉이 핵심이다.
- **부분 장애 전제**로 타임아웃•격리•복구를 설계한다.

#### 한줄 요약

- 서비스 경계와 **데이터 소유권•계약**을 정렬한다.
...
- 진입점•서비스 소유 경계•**이벤트•관측 구조**가 핵심이다.
...
- **외부 요청 라우팅**: 게이트웨이가 인증•한도 정책을 적용하고 소유 서비스로 요청을 보내는 단계다.
- **소유 서비스 로컬 처리**: 서비스가 자신의 업무 규칙과 데이터 변경을 로컬 트랜잭션으로 수행하는 단계다.
- **서비스 간 계약 선택**: 즉시 결과가 필요한지 독립 전파가 필요한지에 따라 API 또는 이벤트를 고르는 단계다.
- **동기 API 협업**: 대상 서비스 응답이 필요한 작업을 버전된 요청•응답 계약으로 처리하는 단계다.
- **비동기 이벤트 협업**: 이벤트를 브로커에 발행해 구독 서비스가 독립적으로 처리하게 하는 단계다.
...
- **1. 외부 요청 라우팅**: 공통 정책 후 소유 서비스를 선택한다.
- **2. 소유 서비스 로컬 처리**: 업무•데이터 변경을 경계 안에서 수행한다.
- **3. 서비스 간 계약 선택**: 응답 결합도에 따라 API•이벤트를 결정한다.
- **4. 동기 API 협업**: 버전된 요청•응답 계약 호출한다.
- **5. 비동기 이벤트 협업**: 브로커를 통한 독립 소비이다.

#### 한줄 요약

- 소유 경계와 **동기 API•비동기 이벤트** 협업이 핵심이다.
...
- **로그(Log)**: 서비스 사건과 오류를 시간순으로 기록한 신호다.
- **메트릭(Metric)**: 서비스 상태를 나타내는 수치 시계열 신호다.
- **분산 추적(Distributed Tracing)**: 서비스 간 요청 경로를 연결한 관측 신호다.
...
- 독립 배포 이익이 분산 비용보다 크면 **MSA**, 아니면 **모듈러 모놀리스**를 선택한다.

#### 한줄 요약

- 독립 배포 이익이 분산 비용보다 클 때 **MSA 선택**이 핵심이다.
