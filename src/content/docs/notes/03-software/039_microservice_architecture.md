...
- **정의/개념**: 독립 서비스가 데이터와 계약으로 협업
- **배경/필요성**: 단일 배포가 변경·확장을 묶어 서비스 분리 필요
...
- 업무 기능·데이터·배포 책임을 한 서비스 경계로 정렬한다.
- 내부 구현은 숨기고 버전된 API·이벤트 계약만 공개한다.
- 부분 장애를 전제로 타임아웃·장애 격리 설계한다.
- 공유 쓰기·잦은 동기 호출·동시 배포는 분산 모놀리스 신호이다.
...
1. 주문 플랫폼은 상품·주문·결제를 소유 데이터별 서비스로 분리
2. 소규모 사내 시스템은 모듈 경계와 단일 배포 유지
...
- 독립 배포 이익이 분산 운영 비용을 넘는 업무만 서비스로 분리
