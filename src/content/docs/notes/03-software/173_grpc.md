...
- **gRPC**: IDL 계약과 코드 생성으로 고성능 원격 호출을 구현하는 프레임워크이다.
...
```text
                 [프로토 IDL•컴파일러]
                    /               \
      [클라이언트 스텁] -- [채널] -- [서버 런타임] -- [서비스 처리기]
```

선의 의미: 공통 IDL 계약이 클라이언트 스텁과 서비스 처리기를 함께 규정하고, 스텁•채널•서버 런타임•처리기가 하나의 정적 호출 결합 구조를 이룬다.
