...
| 절차 | 설명 |
|:---|:---|
| 요청 직렬화 | 스텁이 요청 객체를 Protobuf 메시지와 메타데이터로 변환함 |
| 스트림 전송 | HTTP/2 채널이 여러 RPC의 프레임을 다중화함 |
| 메서드 실행 | 서버 핸들러가 메시지를 역직렬화해 RPC를 수행함 |
| 호출 완료 | 응답 메시지와 종료 상태를 역직렬화해 호출자에 전달함 |
...
- 서비스 간 저지연 통신과 스트리밍을 지원하기 위해 **IDL 계약·클라이언트 호환성·HTTP/2 지원·디버깅·브라우저 제약**을 검토하고, 내부 RPC와 양방향 스트리밍에 gRPC를 적용한다
