...
|:---|:---|:---|
| **Protobuf IDL** | Service, RPC Method, Message의 **필드 번호 및 데이터 타입을 엄격히 선언** | `.proto` 파일 규격 |
| **코드 생성기 (protoc)**| `.proto` 명세를 컴파일하여 **다국어(Java, Go, C++, Python) Stub 코드 자동 생성** | 컴파일 타임 검증 |
...
클라이언트의 gRPC 원격 메서드 호출
   ...
   4. [서버 로직 처리] 서버가 이진 패킷을 역직렬화하고 비즈니스 핸들러 연산 수행
        │
   5. 처리 결과와 함께 gRPC Status Code(OK) 및 Trailer 메타데이터를 클라이언트에 회신
```
