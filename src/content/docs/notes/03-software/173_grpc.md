...
- **구글 원격 프로시저 호출(Google Remote Procedure Call, gRPC)**: 인터페이스 정의 언어 계약과 코드 생성을 바탕으로 타입이 명확한 고성능 원격 호출을 구현하는 프레임워크이다.

</details>

- 정의/개념: 프로토콜 버퍼 인터페이스 정의 언어로 서비스 계약을 정의하고 코드를 생성해 하이퍼텍스트 전송 프로토콜 2로 호출하는 **구글 원격 프로시저 호출(Google Remote Procedure Call, gRPC)** 프레임워크
...
- **프로토콜 버퍼 인터페이스 정의 언어(Protocol Buffers Interface Definition Language, Protobuf IDL)·스텁(Stub)**: 서비스·메시지 계약과 그 계약에서 생성된 원격 호출 대리 코드이다.
...
- **프로토 인터페이스 정의 언어(Protocol Interface Definition Language, Proto IDL)·컴파일러**: 메서드·메시지 계약에서 언어별 클라이언트·서버 코드를 생성하는 구성요소이다.
...
- **1. 요청·메타데이터 전송**: 프로토콜 버퍼 요청과 인증·추적·기한 정보를 채널에 전달하는 단계이다.
- **2. 하이퍼텍스트 전송 프로토콜 2(Hypertext Transfer Protocol Version 2, HTTP/2) 요청 스트림 전달**: 다중화 연결에서 직렬화 요청을 대상 서버로 보내는 단계이다.
- **3. 서비스 메서드 요청**: 서버 런타임이 요청을 역직렬화하고 기한·취소 조건을 적용해 처리기를 호출하는 단계이다.
- **4. 응답·상태 스트림 전달**: 처리 결과 또는 표준 오류 상태를 호출자에게 반환하는 단계이다.
...
- **구글 원격 프로시저 호출(Google Remote Procedure Call, gRPC)**: 타입 계약·낮은 지연·연속 스트리밍이 중요한 내부 서비스 통신에 적합하다.
