...
- 배경/필요성: 특정 품질 속성(성능)에만 편향된 설계로 인한 **타 품질(보안/가용성) 침해 및 아키텍처 전면 재구축 위험 해결 불가**
...
| 구성요소 | 책임 |
|:---|:---|
| 유틸리티 트리 (Utility Tree) | 전사 비즈니스 목표를 **성능, 보안, 가용성 등 구체적 시나리오로 구조화** |
| 민감점 (Sensitivity Point) | 단일 품질 속성에 **결정적인 영향을 미치는 아키텍처 설계 요소 식별** |
| 절충점 (Tradeoff Point) | 둘 이상의 품질 속성이 충돌할 때 **상충 관계를 유발하는 아키텍처 결정 분석** |
| 위험/비위험 (Risk / Non-Risk) | 품질 목표 달성을 **위협하는 아키텍처 결함 및 안전한 설계 결정 판정** |
