...
- 배경/필요성: 소프트웨어 아키텍처 수립 시 특정 품질 속성(성능, 보안, 가용성 등)에만 편향되어 설계할 경우 다른 품질 속성의 치명적 저하(예: 보안 암호화로 인한 성능 급락)가 개발 후반부나 운영 단계에서 발견되어 막대한 재구축 비용이 발생하는 문제를 극복하고, 유틸리티 트리(Utility Tree) 기반 품질 시나리오와 아키텍처 4대 산출물(민감점, 절충점, 위험, 비위험)을 도출하여 **설계 초기 단계에서 품질 속성 간의 상충 관계를 가시화하고 최적의 아키텍처 의사결정(Trade-off)을 수립**할 필요
