...
- 정의/개념: 사용 제약과 무관한 과업 완료의 **디지털 접근성**이다.
- 배경/필요성: 시각•포인터 중심 인터페이스는 감각•운동 제약 사용자의 **정보 접근과 과업 완료를 제한**이 발생한다.
...
- **WCAG•POUR 원칙** 기반 접근성을 분류한다.
- **대체 텍스트•키보드**로 비시각•비포인터 이용을 지원한다.
- **스크린 리더•접근성 트리** 기반 의미를 전달한다.
- **자동 검사•사용자 과업** 결합을 검증한다.
...
- **1. POUR•성공 기준 매핑**: 핵심 과업을 WCAG 기준과 연결하는 단계이다.
- **2. 시맨틱•키보드 구현**: 이름•역할•상태와 초점 순서를 제공하는 단계이다.
- **3. 자동 위반 후보 검사**: 기계 판정 가능한 코드 장벽을 찾는 단계이다.
- **4. 보조기술•사용자 과업 검증**: 실제 정보 접근과 과업 완료를 확인하는 단계이다.
...
1. **POUR•성공 기준 매핑**: 핵심 과업과 WCAG 기준을 연결한다.
2. **시맨틱•키보드 구현**: 이름•역할•상태•초점을 제공한다.
3. **자동 위반 후보 검사**: 기계 판정 가능한 장벽을 탐지한다.
4. **보조기술•사용자 과업 검증**: 실제 과업 성공을 확인한다.
...
- 일반 서비스는 **Level AA**를 기준선으로 삼고 핵심 과업은 보조기술 사용자 성공까지 검증한다.
