...
- 정의/개념: 자원에 **URI•HTTP 균일 인터페이스**를 적용한 **RESTful API**이다.
- 배경/필요성: 동작명 중심 개별 규칙으로는 **클라이언트 결합•응답 편차가 증가한다**.
...
- **명사형 URI•표현** 기반 자원 모델이다.
- **메서드•상태•헤더** 기반 균일 인터페이스가 핵심이다.
...
- **2. 메서드•ETag 조건 해석**: HTTP 메서드 의미와 표현 버전 조건을 판정하는 단계이다.
...
1. **인증된 자원 요청**: 호출 제한 후 URI•메서드•조건을 전달한다.
2. **메서드•ETag 조건 해석**: 메서드 의미와 표현 버전을 판정한다.
3. **자원•버전 조회 요청**: 존재 여부와 현재 표현 버전을 확인한다.
4. **검증된 상태 변경**: 버전 일치 시에만 자원을 갱신한다.
...
- **자원 수명•재시도•캐시**로 URI•메서드•조건부 요청을 결정한다.
