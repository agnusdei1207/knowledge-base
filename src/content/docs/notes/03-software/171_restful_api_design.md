...
| 구성요소 | 책임 |
|:---|:---|
| 자원 모델·URI | 개체·관계·**안정 주소 정의** |
| HTTP 인터페이스 | 메서드·상태·**헤더 의미 적용** |
| 표현·계약 | 미디어 형식·**요청·응답 명세** |
| 조건·캐시 제어 | ETag·**재검증·충돌 방지** |
...
1. **자원·메서드 요청**: URI·행위·인증·조건 포함
2. **인증된 무상태 요청**: 호출 제한·라우팅 후 전달
3. **자원·버전 조회**: 존재 여부·현재 ETag 확인
4. **조건부 상태 변경**: 메서드 의미·버전 조건 적용
5. **상태·헤더·표현 반환**: 결과·캐시·새 버전 제공
