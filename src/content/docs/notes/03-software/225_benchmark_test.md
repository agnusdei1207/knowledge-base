...
- 정의/개념: 동일 환경•자료•부하•측정 절차에서 후보 제품을 실행하는 **BMT 기능•성능 비교 시험**이다.
- 배경/필요성: 제안서만으로는 **실제 처리 능력 검증할 수 없다**.
...
- **조건 통제**: 후보 외 공통 환경•부하 고정이다.
- **반복 측정**: 여러 실행으로 우연 편차를 분리한다.
- **대표 부하**: 운영 업무를 반영해 결과 타당성을 확보한다.
...
- **벤치마크 테스트(Benchmark Test, BMT)**: 동일 조건의 제품 차이를 확인한다.
...
| 기술 평가 방식 | 벤치마크 테스트(BMT) | 개념 증명(PoC) | 문서 평가 |
|:---|:---|:---|:---|
...
> 요약: **성능 비교•실현성•수행성**에 맞춰 평가 방식을 선택한다.
...
- 운영 대표 부하의 반복 측정 결과로 **합격선 충족 후보만 선정**이 핵심이다.
