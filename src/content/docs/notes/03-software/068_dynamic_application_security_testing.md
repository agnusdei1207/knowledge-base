...
- **동적 애플리케이션 보안 테스트(Dynamic Application Security Testing, DAST)**: 실행 중인 서비스에 공격 요청을 보내 외부에서 관찰되는 취약 반응을 찾는 시험한다.
- **실제 공격 성공**: 설정•인증•실행 환경을 포함한 조건에서 공격이 유효한 결과를 만드는 상태이다.

</details>

- 정의/개념: **DAST**는 실행 중인 애플리케이션에 외부 공격 요청을 보내 응답과 동작을 관찰하여 실제 노출 취약점을 찾는 동적 보안 시험 기법이다.
- 배경/필요성: 코드 분석만으로는 설정•인증을 포함한 **실제 공격 성공 확인할 수 없다**.

#### 한줄 요약

- 실행 서비스 공격 요청 기반 **외부 취약 반응 검증**이 핵심이다.
...
- 탐색하지 못한 경로와 권한은 **검사 누락**이 핵심이다.

#### 한줄 요약

- 실제 공격 검증과 **탐색 경로•권한 누락 한계**가 핵심이다.
...
1. **허용 범위 내 공격 표면 탐색**: 화면•경로를 수집한다.
2. **경로•입력점 분류**: 공격할 매개변수•기능을 제공한다.
3. **공격 페이로드•인증 세션 전송**: 권한별 변조를 요청한다.
4. **취약 반응•재현 증거 판정**: 응답 증거를 보존한다.
...
- 공격 가능성은 DAST, 코드 원인은 **SAST**가 핵심이다.
...
- **프록시 기록(Proxy Record)**: 중계 지점에서 실제 요청•응답을 수집한 통신 기록한다.
...
- 명세•역할•격리•요청 추적 기반 **DAST 범위 보강**이 핵심이다.
...
- **공격 표면•권한•운영 영향**에 따라 DAST 범위를 결정한다.
