...
- **1. 범위·인증 정보 설정**: 경로와 권한을 결정
- **2. 경로·입력점 탐색**: 공격 표면을 수집
- **3. 공격 표면 전달**: 인증 상태와 함께 전달
- **4. 페이로드 요청**: 입력점별 공격을 실행
- **5. 상태·본문·지연 응답**: 서비스 반응을 전달
- **6. 취약 후보·재현 증거**: 요청과 응답을 보고
- **7. 수정본 재검사**: 같은 공격으로 수정 여부를 확인
