...
- 배경/필요성: 소스 코드 정적 분석(SAST)만으로는 웹 서버/인프라 설정 결함, 런타임 인증·인가(Broken Access Control) 우회 및 실제 공격 페이로드의 악용 가능성을 포괄적으로 검증할 수 없는 한계를 극복하고, 구동 중인 웹 애플리케이션을 대상으로 외부 공격자 시각에서 자동화된 모의 침투 및 액티브 스캐닝(Active Scanning)을 수행하여 **실환경에서 실제로 악용 가능한 취약점과 보안 증적(PoC)을 객관적으로 검증**할 필요
...
- 데브섹옵스(DevSecOps) 및 런타임 보안 검증의 **대표적인 블랙박스 동적 모의 침투 표준 방법론**으로 확립되었으며, 실무 운영 시에는 **데이터 오염 방지를 위한 격리된 스테이징 환경 진단, SPA 및 RESTful API 탐색을 위한 헤드리스 브라우저/OpenAPI 명세 연동, CI 파이프라인의 베이스라인 점검과 야간 정기 정밀 스캔의 이원화, SAST/SCA와의 연계를 통한 다계층 방어(Defense in Depth)**를 구현
