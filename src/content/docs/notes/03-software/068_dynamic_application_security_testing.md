...
date: "2026-08-26T13:02:50+09:00"
tags:
  - "notes-software"
weight: 68
extra:
...
#### 한줄 요약
- 소스 코드 없이 공격 요청과 응답으로 런타임 취약 후보를 검증한다.
...
```text
진단 대상 URL 및 OpenAPI(Swagger) 명세 등록
        │
   [탐색 단계] 스파이더가 웹 UI와 API 경로를 순회하며 파라미터 공격 표면 수집
        │
   [인증 단계] 관리자 및 일반 사용자 계정으로 로그인하여 세션/JWT 획득
        │
   [공격 단계] 파라미터에 OWASP Top 10 악성 페이로드(`' OR 1=1--`, `<script>`) 주입
        │
   서버 응답에서 SQL 에러 출력 또는 스크립트 실행이 확인되었는가?
   ┌────┴─────┐
  예           아니오
   │             │
[취약점 증명 PoC 생성] [미탐·추가 검사]
심각도 기반 리포트 발행   다른 입력과 경로 검증
```
...
- 런타임 취약점은 **DAST**, 다층 방어는 **SAST 연계** 선택
