...
date: "2026-08-02T22:06:00+09:00"
tags: [notes-software]
weight: 6
extra:
  question_no: "006"
...
- **가상 실행시간(Virtual Runtime, vruntime)**: 실제 실행시간을 태스크 가중치로 보정한 누적값
...
- **최소 세분도(Minimum Granularity)**: 태스크별 실행 구간이 더 짧아지지 않도록 정한 하한
- **깨움 세분도(Wakeup Granularity)**: 새로 깨어난 태스크가 현재 태스크를 선점할 민감도 기준

</details>

- **가중 가상 실행시간**으로 태스크별 CPU 몫 추적
- **최소 vruntime 태스크 선택**으로 공정한 실행 기회 제공
- **최소·깨움 세분도 감소** 시 응답 지연 감소·전환 비용 증가
...
- **스케줄링 개체(Scheduling Entity)**: 태스크의 가중치와 가상 실행시간을 보관하는 스케줄링 상태
- **CFS 실행 큐(CFS Run Queue, cfs_rq)**: 실행 가능한 개체를 가상 실행시간 순으로 관리하는 실행 큐
- **레드-블랙 트리(Red-Black Tree)**: 가상 실행시간 순서를 유지하는 균형 탐색 트리
...
- **가중치 역비례 누적**: CPU 몫이 큰 태스크일수록 같은 실행시간에 vruntime을 더 적게 늘리는 계산
...
- **가중 배분**: 태스크별 가중치 비율에 맞춰 서로 다른 CPU 몫을 제공하는 방식
...
- **그룹 스케줄링(Group Scheduling)**: 태스크를 그룹으로 묶어 CPU 몫을 계층적으로 배분하는 방식
...
- **nice 가중치·깨움 세분도**: CPU 몫과 선점 민감도를 각각 조정하는 CFS 설정 기준
