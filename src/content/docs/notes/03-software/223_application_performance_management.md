...
- 정의/개념: 사용자 요청부터 분산 서비스•코드•인프라까지 추적해 지연과 장애 원인을 진단하는 **APM 성능 관리 체계**이다.
- 배경/필요성: 분산 호출로 **지연 구간•장애 원인 식별하기 어렵다**.
...
- **복합 관측**: RUM•합성 감시로 실제•예방 관측 결합한다.
- **경로 추적**: Trace ID•Span으로 서비스별 지연을 연결한다.
- **상관 분석**: 메트릭•로그•트레이스 증거 결합한다.
- **RED 지표**: 요청률•오류•처리시간으로 서비스 상태 관측이다.
...
- **분산 추적(Distributed Tracing)**: 추적 식별자•작업 구간으로 병목을 분석한다.
...
> 요약: **메트릭•로그•트레이스** 결합으로 성능 사각지대를 제거한다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 표본 추출의 **느린 요청 누락** | 꼬리•오류 우선 **샘플링** | 장애 원인 **가시성** |
| 계측 에이전트의 **측정 오버헤드** | 비율 조정•**자체 지표 감시** | 서비스 영향 **통제** |
...
- **합성 감시 적용**: 주요 경로의 가용성•지연을 가상 요청으로 사전 감시이다.
- **관측 방식 선택**: 실제 체감은 RUM, 사전 경로 감시는 합성 모니터링, 서비스별 원인 추적은 분산 추적을 사용하는 원칙이다.

</details>

- 체감•예방•원인별 **실사용자•합성•분산 추적**을 선택한다.
