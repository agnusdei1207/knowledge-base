...
- **애플리케이션 성능 관리(Application Performance Management, APM)**: 사용자 경험부터 소스 코드와 인프라 자원을 모니터링함
- **리얼 사용자 모니터링(Real User Monitoring, RUM)**: 실제 사용자 브라우저 및 앱의 지연과 오류를 측정함
...
- **RED 기법(Rate-Error-Duration Method)**: 요청률과 에러율 및 처리 지연 시간으로 서비스 성능을 관찰함
...
- **정의/개념**: 앱 지연과 장애 원인을 진단하는 통합 관리 기법임
- **배경/필요성**: 분산 호출 원인 추적 지연으로 통합 관측 필요
...
- RUM과 가상 스크립트를 통한 복합 평가를 수행한다.
- 메트릭 정보와 분산 로그를 분석 서버와 연계한다.
- 이상 징후 분석을 통해 잠재 에러를 자동 탐색한다.
- 호출 트리 추적으로 장애의 핵심 원인을 판정한다.
...
| 비교축 | RUM | Synthetic | Tracing |
|:---|:---|:---|:---|
| 핵심 특징 | 실제 사용자 경험을 측정함 | 가상 요청을 정기 실행함 | 분산 호출 경로를 연결함 |
| 적용 기준 | 단말·지역별 체감 분석 | 사전 가용성·경로 감시 | 서비스별 병목 원인 분석 |
| 주요 위험 | 표본 편향·개인정보 노출 | 실제 행동과 시나리오 차이 | 샘플 누락·수집 비용 |
