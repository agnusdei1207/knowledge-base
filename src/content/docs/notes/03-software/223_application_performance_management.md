...
- 정의/개념: 사용자 요청부터 분산 서비스•코드•인프라까지 추적하는 **애플리케이션 성능 관리**이다.
...
- **추적 식별자** 및 **작업 구간** 기반 서비스별 지연을 연결한다.
- 메트릭•로그•트레이스 증거를 상관 분석한다.
- **요청률•오류•처리시간** 기반 서비스 상태를 관측한다.
...
| 구성요소 | 책임 |
|:---|:---|
| 계측 에이전트**•**소프트웨어 개발 키트 | 요청•코드•자원 정보 수집 |
| 샘플링 | 수집 비율•오버헤드 통제 |
...
| APM 관측 방식 | **실사용자 모니터링** | **합성 모니터링** | **분산 추적** |
|:---|:---|:---|:---|
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 표본 추출의 느린 요청 누락 | **꼬리 기반 샘플링** | 장애 원인 가시성 |
| 계측 에이전트의 **측정 오버헤드** | **중앙 처리장치** 사용 비율•자체 지표 감시 | 서비스 영향 통제 |
...
- **관측 방식 선택 기준** 대상 따라 체감•예방•원인별 관측 수단을 선택하고 **서비스 수준 목표** 및 연결한다.
