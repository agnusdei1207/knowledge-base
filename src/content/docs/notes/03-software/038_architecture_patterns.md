...
- 배경/필요성: 단일 거대 monolithic 붕괴 방지, 유지보수성/확장성 극대화 및 도메인 요구사항에 최적화된 구조 채택 필요성
...
├───────────────┬──────────────────────┬─────────────────┤
│ Layered (MVC) │ Microservices (MSA)  │ Event-Driven    │
└───────────────┴──────────────────────┴─────────────────┘
```

선의 의미: 대표적 아키텍처 패턴인 Layered/MVC, MSA, EDA가 도메인 특성 및 분산화 수준에 따라 적용 선택되는 아키텍처 스펙트럼.

| 아키텍처 패턴 | 구조적 특징 | 주요 장점 | 한계 및 오버헤드 |
|:---|:---|:---|:---|
| **Layered Pattern (MVC/3-Tier)** | 상위 계층이 바로 하위 계층만 억세스하는 세로 계층 분리 | 단순함, 이해 용이, 관심사 분리 | 계층 바이패스 문제, 모놀리식 한계 |
| **MSA (Microservices)** | 독자적 DB와 배포 단위를 가진 독립 서비스 연합 | **독립적 배포/확장성, 기술 스택 유연성** | **분산 복잡도, 데이터 일관성(Saga), 네트워크 지연** |
| **EDA (Event-Driven)** | Event Broker (Kafka/RabbitMQ) 기반 비동기 이벤트 발행 | **극단적 비결합(Decoupling), 고성능 스루풋** | **최종 일관성(Eventual Consistency), 디버깅 난항** |
| **Hexagonal Architecture** | 코어 도메인을 외부 프레임워크/DB와 Adapter로 분리 | 도메인 순수성 및 테스트 용이성 | 초기 매핑 보일러플레이트 코드 증가 |
...
- **아키텍처 패턴 선택 기준**에 따라 시스템 요구사항 분석 후 **ADR** 생성을 통해 무분별한 과잉 엔지니어링 방지
