...
- **이벤트 채널(Event Channel)**: 생산자와 소비자 사이의 이벤트 전달 경로다.
- **라우팅(Routing)**: 이벤트를 전달할 소비자를 선택하는 처리다.
- **구독(Subscription)**: 소비자가 받을 이벤트 종류를 등록하는 관계다.
...
- **변경 축(Change Axis)**: 함께 바뀌는 기능과 데이터의 방향이다.
