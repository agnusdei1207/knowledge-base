...
- 정의/개념: 반복 문제의 구성요소•책임•관계를 재사용하는 **설계 해법**이다.
- 배경/필요성: 화면•업무•상태를 한 구조에 결합하면 **변경 전파** 확대가 발생한다.

#### 한줄 요약

- 반복 설계 문제에 **검증된 책임•관계 구조** 재사용이 핵심이다.
...
- **책임 경계 분리**로 변경 전파 범위 국소화가 핵심이다.
- **경계 간 계약**이 패턴 조합의 결합도를 결정한다.
- **EDA 비동기 상태 전파**가 분산 복잡도 증가한다.

#### 한줄 요약

- 책임 경계•계약•**분산 복잡도**의 절충이 핵심이다.
...
- MVC•MSA•EDA의 **책임 경계 분리 방식**이 핵심이다.
...
- 화면•업무•이벤트 변경 축별 **패턴 선택•조합**이 핵심이다.
...
- **생산자(Producer)**: 계약에 맞는 이벤트를 발행하는 구성요소다.
- **소비자(Consumer)**: 계약에 맞는 이벤트를 받아 처리하는 구성요소다.
- **컨트롤러 비대화(Fat Controller)**: 입력 제어 계층에 업무 규칙과 상태 처리가 몰려 변경•시험이 어려워진 상태다.
...
- **모듈 경계(Module Boundary)**: 높은 응집도를 가진 업무 규칙•데이터를 내부에 묶고 외부 의존을 계약으로 제한하는 경계다.
...
- 화면 책임은 **MVC**, 독립 배포는 **MSA**, 비동기 전파는 EDA을 선택한다.

#### 한줄 요약

- 화면•배포•전파 변경 축에 맞는 **패턴 선택**이 핵심이다.
