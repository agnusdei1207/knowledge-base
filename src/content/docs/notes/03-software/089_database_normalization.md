...
date: "2026-07-31T10:31:16+09:00"
tags:
  - "notes-software"
weight: 89
extra:
...
> **키워드:** 데이터베이스 정규화 1NF~BCNF (Database Normalization)
...
sequenceDiagram
    ...
    D->>B: 스키마·업무 규칙 질문
    B-->>D: 업무 규칙·함수 종속 근거
    D->>F: 1. 속성 집합·함수 종속 집합
    F-->>D: 2. 후보키·정규형 위반 목록
    D->>S: 3. 분해 릴레이션·키·제약
    S->>V: 4. 분해 스키마·함수 종속 집합
    V-->>D: 무손실·종속성 보존 결과
```

**동작 원리**

- **1. 속성 집합·함수 종속 집합**: 속성 폐쇄로 후보키·결정자 식별
- **2. 후보키·정규형 위반 목록**: 부분·이행·비키 결정자 확인
