...
date: "2026-07-30T23:33:48+09:00"
tags:
  - "notes-software"
weight: 89
extra:
...
- **속성 폐쇄(Attribute Closure)**: 함수 종속을 반복 적용해 한 속성 집합이 결정할 수 있는 모든 속성을 구한 집합
- **자연 조인(Natural Join)**: 같은 이름의 속성값이 일치하는 튜플을 결합하는 관계 연산
- **거짓 튜플(Spurious Tuple)**: 잘못된 분해를 다시 조인할 때 원본에 없는데 생성되는 튜플
- **분해 스키마(Decomposed Schema)**: 정규화 결과로 나뉜 릴레이션·키·제약의 구조
...
sequenceDiagram
    ...
    D->>B: 스키마·업무 규칙 질문
    B-->>D: 1. 업무 규칙·함수 종속 근거
    D->>F: 2. 속성 집합·함수 종속 집합
    F-->>D: 3. 후보키·정규형 위반 목록
    D->>S: 4. 분해 릴레이션·키·제약
    S->>V: 5. 분해 스키마·함수 종속 집합
    V-->>D: 무손실·종속성 보존 결과
```

**동작 원리**

- **1. 업무 규칙·함수 종속 근거**: 정책·유일성으로 FD 도출
- **2. 속성 집합·함수 종속 집합**: 속성 폐쇄로 후보키·결정자 식별
- **3. 후보키·정규형 위반 목록**: 부분·이행·비키 결정자 확인
- **4. 분해 릴레이션·키·제약**: 위반 종속을 별도 관계로 분리
- **5. 분해 스키마·함수 종속 집합**: 조인 복원과 제약 유지 검증
...
| BCNF만 추구해 중요한 종속성 검사가 조인에 의존 | **종속성 보존과 3NF 절충** 비교 | **제약 검사 조인 비용** 통제 |
