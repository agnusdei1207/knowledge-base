...
- 배경/필요성: 한 릴레이션에 불필요한 사실들이 혼재할 때 발생하는 3대 이상 현상(삽입/수정/삭제 이상) 방지 및 데이터 무결성(Integrity) 확보 요구성
...
## Ⅲ. 구조 및 구성요소 (데이터 3대 이상 현상 & 무손실 분해)
...
## Ⅳ. 흐름도 (정규화 단계별 이행 흐름: 1NF $\rightarrow$ 2NF $\rightarrow$ 3NF $\rightarrow$ BCNF)
...
## Ⅴ. 종류 및 비교 (3NF 대 BCNF)

<details><summary>핵심 용어</summary>

- **3NF vs BCNF**: 3NF는 종속성 보존(Dependency Preservation)이 100% 보장되지만 BCNF 분해 시 종속성 보존이 깨질 가능성 존재.
...
| 종속성 보존 여부 | **100% 종속성 보존 보장** | **종속성 보존이 파괴될 수 있음** |
...
- **정규화 수립 기준**에 따라 상용 OLTP 데이터베이스 설계 시 **3NF / BCNF 무손실 분해** 필수 적용
