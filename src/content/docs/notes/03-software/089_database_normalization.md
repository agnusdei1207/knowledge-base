...
- **1. 업무 사실·유일성 질문**: 지속 규칙을 확인한다.
- **2. 함수 종속 근거**: 정책과 수명주기를 제시한다.
- **3. FD·후보키 분석 요청**: 폐쇄와 결정자를 찾는다.
- **4. 위반 정규형·결정자**: 부분·이행 종속을 판정한다.
- **5. 릴레이션 분해**: 위반 종속을 분리한다.
- **6. 무손실·종속 보존 검증**: 복원과 제약을 시험한다.
- **7. 복원·제약 결과**: 스키마 타당성을 확인한다.
...
- **함수 종속·무손실·제약 보존**으로 목표 정규형을 정한다.
