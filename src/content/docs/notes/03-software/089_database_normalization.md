...
- 배경/필요성: 한 릴레이션의 여러 사실 반복은 **삽입•갱신•삭제 이상**을 유발한다.
...
- **함수 종속(Functional Dependency, FD)**: 한 속성값이 다른 속성값을 결정하는 관계다.
...
- **최소 식별자(Minimal Identifier)**: 불필요한 속성이 없는 튜플 식별자다.
...
- **함수 종속•후보키 도출**: 업무 규칙에서 결정 관계와 최소 식별자를 찾는 단계다.
- **정규형 위반 판정**: 부분•이행•비슈퍼키 결정자를 확인하는 단계다.
- **위반 종속 분해**: 위반 결정 관계를 별도 릴레이션으로 나누는 단계다.
- **무손실•종속성 보존 검증**: 원본 복원과 제약 유지 여부를 확인하는 단계다.
...
- **1. 함수 종속•후보키 도출**: 속성 폐쇄로 최소키 식별이다.
- **2. 정규형 위반 판정**: 부분•이행•비키 결정자를 확인한다.
- **3. 위반 종속 분해**: 위반 결정 관계를 별도 표로 분리한다.
- **4. 무손실•종속성 보존 검증**: 복원과 제약 유지를 확인한다.
...
- **반복값(Repeating Group)**: 한 속성에 여러 값이 반복 저장된 상태다.
- **부분 종속(Partial Dependency)**: 비키 속성이 복합키 일부에만 종속된 관계다.
...
- **전문가 근거(Expert Evidence)**: 업무 전문가가 확인한 속성 결정 관계다.
- **정책 근거(Policy Evidence)**: 업무 규칙으로 정의한 함수 종속 자료다.
- **수명주기 근거(Lifecycle Evidence)**: 데이터 변화 전체에서 종속이 유지된 자료다.
...
- **동기화 책임(Synchronization Ownership)**: 중복 데이터의 갱신 일관성을 맡은 주체다.
...
- 이상 제거 우선은 **BCNF**, 종속성 보존 우선은 **3NF**을 선택한다.
