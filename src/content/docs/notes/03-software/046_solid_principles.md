...
- 배경/필요성: 강결합 스파게티 코드로 인한 **코드 수정 시 연쇄적 부수효과(Side-Effect) 발생 및 재사용 불가 한계 해결 불가**
...
| 원칙 | 핵심 개념 | 코드 수준 설계 책임 |
|:---|:---|:---|
| **SRP (단일 책임)** | 변경의 이유가 오직 하나 | 비즈니스 로직, DB 접근, UI 렌더링을 별도 클래스로 분리 |
| **OCP (개방 폐쇄)** | 확장엔 열림, 수정엔 닫힘 | **인터페이스 및 전략 패턴(Strategy Pattern)** 기반 기능 추가 |
| **LSP (리스코프 치환)** | 자식이 부모를 완전 대체 | 부모 메서드의 사전/사후 조건을 깨뜨리는 자식 오버라이딩 금지 |
| **ISP (인터페이스 분리)** | 불필요한 메서드 의존 금지 | 거대 인터페이스를 기능별 세부 인터페이스(`Printable`, `Faxable`)로 쪼갬 |
| **DIP (의존역전)** | 추상화에만 의존 | `new ConcreteClass()`를 금지하고 **Spring `@Autowired` / DI 주입** |
...
기존 레거시 코드: 주문 서비스가 결제 클래스를 직접 생성 (`new KakaoPay()`) - [DIP/OCP 위반]
        │
   1. [DIP 적용] 구체 결제 클래스 위에 `Payment` 추상 인터페이스 정의
        │
   2. [OCP 적용] `KakaoPay`, `NaverPay`, `ApplePay`가 `Payment` 인터페이스를 구현
        │
   3. [DI 적용] 주문 서비스는 오직 `Payment` 인터페이스만 참조하고 생성자로 주입받음
        │
   4. [확장 검증] 신규 결제(`TossPay`) 추가 시 주문 서비스 코드 수정 0줄로 완료
```
...
- 객체지향 및 클린 아키텍처는 **SOLID 5대 원칙을 핵심 설계 척도**로 삼고, **Spring DI 프레임워크와 디자인 패턴(Strategy/Factory)** 을 결합하여 변경에 유연한 고품질 소프트웨어 확립
