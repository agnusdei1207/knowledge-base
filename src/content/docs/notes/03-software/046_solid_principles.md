...
- 배경/필요성: 소프트웨어 규모 확장 시 단일 클래스의 과도한 책임 집중(God Class), 구체 구현체 직접 참조로 인한 강한 결합 및 기능 추가 시 기존 코드 수정이 불가피한 경직성 한계를 극복하고, 단일 책임(SRP)·개방 폐쇄(OCP)·리스코프 치환(LSP)·인터페이스 분리(ISP)·의존역전(DIP)의 5대 원칙을 통해 **높은 응집도(High Cohesion)와 낮은 결합도(Low Coupling)를 확보하고 변경 유연성과 테스트 용이성을 극대화**할 필요
