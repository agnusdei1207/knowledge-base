...
- 배경/필요성: 책임 혼합과 구체 구현 의존은 **변경 파급•치환 실패** 유발
...
| 구성요소 | 책임 |
|:---|:---|
| SRP | 변경 이유•행위자별 책임 분리 |
| OCP | 기존 코드 수정보다 **확장점**으로 행위 추가 |
| LSP | 하위 타입이 상위 타입의 행위 계약 보존 |
| ISP | 클라이언트가 쓰는 최소 인터페이스에만 의존 |
| DIP | 고수준•저수준 모듈이 **추상화**에 의존 |
...
1. **SRP: 다중 책임 분리**: 변경 이유가 다른 동작을 별도 모듈로 이동
2. **OCP: 인터페이스 확장점 도입**: 새 행위를 구현체 추가로 수용
3. **LSP: 상위 계약 규칙 보존**: 하위 타입의 사전•사후 조건 검증
4. **ISP: 인터페이스 쪼개기**: 클라이언트별 최소 계약으로 분리
5. **DIP: DI 컨테이너 연동**: 조립부에서 추상화 구현체 주입
...
- 반복 변경 경계에는 **SOLID**, 고정•단순 로직에는 **YAGNI** 우선
