...
- **정의/개념**: 책임·계약·의존 방향의 다섯 원칙
- **배경/필요성**: 구현 결합이 수정을 확산해 의존 통제 필요
...
- 변경 이유·계약·의존 방향을 다른 축으로 통제한다.
- 절차가 아닌 결합 결함별 선택 원칙이다.
- 과잉 분리는 추상화·간접 호출 비용 증가한다.
...
| 판단 기준 | SOLID 적용 구조 | 직접 결합 구조 |
|:---|:---|:---|
| 핵심 특징 | 책임·계약·정책 추상화로 변경 분리 | 구체 구현을 호출부가 직접 의존 |
| 적용 기준 | 구현 교체·변경 이유가 반복 | 단순 책임·고정 구현 |
| 주요 위험 | 추상화·인터페이스 과잉 | 책임 혼합·연쇄 수정 |
...
1. 결제 서비스는 결제 인터페이스로 대행사 구현 교체
2. 주문 모듈은 금액 계산과 알림 발송 책임 분리
...
- 반복 변경 경계에만 SOLID를 적용해 추상화 비용 제한
