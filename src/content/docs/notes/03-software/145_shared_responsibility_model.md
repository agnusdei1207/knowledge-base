...
- 배경/필요성: 인프라를 위탁해도 데이터 분류·접근 권한·설정 같은 통제는 여전히 고객에게 남는데 그 경계를 명시하지 않으면 양측이 서로 상대가 맡는다고 가정해 아무도 조치하지 않는 공백이 생기므로, 서비스 모델별로 어느 계층까지 사업자가 책임지는지를 규정해 그 공백을 없앨 필요
