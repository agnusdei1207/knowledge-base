...
- 배경/필요성: 클라우드 도입 시 인프라 관리를 위탁하더라도 데이터 암호화, 접근 권한(IAM), 방화벽 설정 및 데이터 백업의 책임은 여전히 고객에게 귀속됨에도 불구하고, 이를 CSP가 전담해 줄 것으로 오인함에 따라 발생하는 보안 설정 오류(Misconfiguration) 및 데이터 유출 사고를 방지하기 위해, 클라우드 자체 보안(Security OF the Cloud, CSP 책임)과 클라우드 내부 보안(Security IN the Cloud, 고객 책임)의 분계선을 명확히 정의하는 공유 책임 모델(Shared Responsibility Model)을 수립하여 **보안 관리 공백을 원천 차단하고 엔드투엔드 거버넌스를 확립**할 필요
