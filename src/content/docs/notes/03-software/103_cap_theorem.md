...
- **1. 네트워크 분할 감지**: 복제본 사이 통신 불가 확인
- **2. 분할 중 요청**: 일관성과 가용성 선택이 필요한 연산 접수
- **3. 도달 복제본 선택**: 현재 통신 가능한 사본으로 전달
- **4. 복제본 합의 시도**: 최신 상태의 확인 가능 여부 판정
- **5. CP·AP 정책 적용**: 거부로 일관성 또는 응답으로 가용성 선택
