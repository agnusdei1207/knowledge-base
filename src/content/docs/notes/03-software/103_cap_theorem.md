...
- **1. 분할 상태 통보**: 통신 단절을 알린다.
- **2. 읽기·쓰기 요청**: 연산을 전달한다.
- **3. 도달 노드 전달**: 가능한 사본을 고른다.
- **4. 최신 상태 확인**: 다른 사본을 확인한다.
- **5. 통신 실패**: 분할 상태를 확정한다.
- **6. 정책 결과 반환**: CP 또는 AP를 적용한다.
- **7. 오류 또는 값 응답**: 선택 결과를 보낸다.
...
- **불일치 비용·중단 비용·복구 방식**으로 CP와 AP를 정한다.
