...
- 정의/개념: 네트워크 분할 중 **일관성•가용성 동시 보장 불가**를 설명하는 정리이다.
- 배경/필요성: 원격 복제본 확인 불가로 **최신 응답•무중단 처리** 동시 보장할 수 없다.
...
- **분할 상태 확인**: 복제본 사이의 도달성 상실을 확인하는 단계다.
- **복제본 확인 요청**: 연산 키를 보유한 도달 노드를 찾는 단계다.
- **최신 버전 조회**: 원격 복제본으로 최신 상태를 확인하는 단계다.
- **도달 가능 복제본 판정**: 정족수와 지역 상태로 정책을 고르는 단계다.
...
- **1. 분할 상태 확인**: 복제본 사이 도달성 상실을 판정한다.
- **2. 복제본 확인 요청**: 연산 키의 도달 노드를 확인한다.
- **3. 최신 버전 조회**: 원격 복제본의 최신 상태를 확인한다.
- **4. 도달 가능 복제본 판정**: 정족수로 CP•AP 처리를 선택한다.
...
> 요약: 네트워크 분할이 없는 범위에서만 성립하는 **일관성•가용성(Consistency and Availability, CA)** 조합이 핵심이다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 재고 차감처럼 불변식 위반 비용이 큰 연산 | 연산별 **일관성•가용성 정책** 분리 | **업무 불변식** 위반 방지 |
...
- 불일치 비용이 크면 **CP**, 중단 비용이 크면 **AP**을 선택한다.
