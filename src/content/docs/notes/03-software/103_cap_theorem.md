...
- **CAP 정리(Consistency, Availability, Partition Tolerance Theorem, CAP)**: 분할 중 일관성과 가용성을 모두 보장할 수 없다는 정리이다.
- **일관성(Consistency, C)**: 성공한 읽기가 가장 최근 완료된 쓰기값을 반환하는 성질이다.
- **가용성(Availability, A)**: 모든 유효 요청에 응답하는 성질이다.
- **분할 내성(Partition Tolerance, P)**: 노드 간 통신 단절에도 시스템이 동작하는 성질이다.

</details>

- 정의/개념: **CAP 정리**는 네트워크 분할 중 일관성과 가용성을 동시에 보장할 수 없음을 설명한다.
- 배경/필요성: 원격 복제본을 확인할 수 없으면 최신 응답과 무중단 처리를 동시에 보장할 수 없다.
...
- 일관성•가용성 상충을 강제하는 **네트워크 분할** 조건이 핵심이다.
- 분할 중 **분할 중 일관성 선택**과 **분할 중 가용성 선택** 가운데 하나를 택한다.
- 정상 상태의 지연시간•일관성 절충까지 확장하는 **PACELC** 관점이 핵심이다.
...
| 분할 감지 | **분할 감지**로 도달성•정족수 상실 판정 |
| 일관성 정책 | **일관성 정책**으로 최신 상태 미확인 시 요청 제한 |
| 가용성 정책 | **가용성 정책**으로 도달 복제본의 지역 처리 허용 |
| 충돌 해결 | **충돌 해결**로 연결 복구 후 복제본 수렴 |
...
- **분할 상태 확인**: 복제본 사이의 도달성 상실을 확인하는 단계이다.
- **복제본 확인 요청**: 연산 키를 보유한 도달 노드를 찾는 단계이다.
- **최신 버전 조회**: 원격 복제본으로 최신 상태를 확인하는 단계이다.
- **도달 가능 복제본 판정**: 정족수와 지역 상태로 정책을 고르는 단계이다.
...
- **1. 분할 상태 확인**: **분할 상태 확인**으로 복제본 사이 도달성 상실을 판정한다.
- **2. 복제본 확인 요청**: **복제본 확인 요청**으로 연산 키의 도달 노드를 확인한다.
- **3. 최신 버전 조회**: **최신 버전 조회**로 원격 복제본의 최신 상태를 확인한다.
- **4. 도달 가능 복제본 판정**: **도달 가능 복제본 판정**으로 **CP 정책**과 **AP 정책**을 선택한다.
...
- **분할 대응 선택 기준**: 불일치 비용과 중단 비용을 비교해 CP 또는 AP 정책을 고르는 기준이다.
...
| 핵심 특징 | **CP 정책**으로 최신 상태 미확인 시 요청 거부•지연 | **AP 정책**으로 도달 복제본에서 지역 처리 |
| 한계 | 분할 동안 가용성 저하 | 오래된 읽기•쓰기 충돌 허용 |

> 요약: **분할 대응 선택 기준**은 네트워크 분할이 없는 범위의 **일관성•가용성(CA)** 조합과 구분해야 한다.
...
- **정족수(Quorum)**: 최신 상태를 인정할 최소 노드 수이다.
- **지수 백오프(Exponential Backoff)**: 재시도 간격을 지수적으로 늘려 동시 요청 폭주를 줄이는 방식이다.
- **버전 벡터(Version Vector)**: 복제본별 변경 계보를 판별하는 자료이다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 재고 차감처럼 불변식 위반 비용이 큰 연산 | **연산별 일관성 정책**•**연산별 가용성 정책** 분리 | 업무 불변식 위반 방지 |
...
- **CAP 정책 적용 기준**: 연산별 불일치•중단 비용과 연결 복구 뒤 수렴 책임을 함께 평가하는 기준이다.

</details>

- **CAP 정책 적용 기준**에 따라 불일치 비용이 크면 **CP 정책**, 중단 비용이 크면 **AP 정책**을 선택한다.

#### 한줄 요약

- **CAP 정책 적용 기준**은 분할 중 정확성•응답성과 복구 뒤 값의 수렴을 함께 다룬다.
