...
- 정의/개념: **카오스 엔지니어링**은 정상 상태와 복원력 가설을 세우고 통제된 장애를 주입하여 분산 시스템이 예상대로 견디고 복구하는지 검증하는 실험 기법이다.
- 배경/필요성: 정상 기능 시험만으로 숨은 의존•**연쇄 장애 검증할 수 없다**.
...
- **실제 장애 주입** 기반 숨은 의존성 탐색이 핵심이다.
- **정상 상태•사용자 지표** 기반 가설을 판정한다.
- **영향 반경•중단 조건** 기반 안전을 통제한다.
...
- **카오스 엔지니어링 역할**: 실제 실패 조건으로 숨은 의존성과 연쇄 실패를 검증한다.
...
> 요약: 숨은 의존은 **카오스 실험**, 대응 숙련은 **장애 복구 훈련**을 선택한다.
