...
| 비교 항목 | 안전 상태 (Safe State) | 불안전 상태 (Unsafe State) | 교착상태 (Deadlock State) |
|:---|:---|:---|:---|
...
- 자원 요구량이 엄격히 정형화된 **항공/철도/원전 등 안전 필수(Safety-Critical) 실시간 임베디드 환경**에 은행원 알고리즘 기반 교착상태 회피 표준 채택

#### 한줄 요약

- **사전 요구량 선언과 안전 순서열 검증**을 통해 교착상태를 수학적으로 선제 차단하는 회피 아키텍처
