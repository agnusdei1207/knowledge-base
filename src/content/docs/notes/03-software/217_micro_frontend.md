---
sidebar:
  order: 217
  label: "217. 마이크로프론트엔드 아키텍처 (Micro Frontend)"
  badge:
    text: "미출제 · 50%"
    variant: note
title: "마이크로프론트엔드 아키텍처 (Micro Frontend)"
date: "2026-07-25T00:30:00+09:00"
tags: ["notes-software"]
weight: 217
extra:
  question_no: "217"
  source_status: "미출제"
  source_history: ""
  priority: 50
  priority_note: "업무별 화면 분리와 독립 배포가 독립 설계축임"
---

## 미리 알고가기

- **마이크로프론트엔드(Micro Frontend)**: 하나의 웹 화면을 업무 영역별 작은 애플리케이션으로 나눠 독립 개발·배포하고 사용자에게 하나로 조합하는 구조다.
- **업무 경계**: 주문이나 결제처럼 독립적으로 책임지고 변경할 수 있도록 기능과 데이터를 나누는 기준이다.
- **애플리케이션 셸(Application Shell)**: 공통 화면 틀과 이동 경로를 제공하고 업무별 마이크로 앱을 필요한 위치에 조합하는 상위 앱이다.
- **라우팅(Routing)**: 사용자가 요청한 주소나 메뉴에 따라 표시할 화면 모듈을 결정하는 동작이다.
- **통합 계약(Integration Contract)**: 마이크로 앱 사이의 화면 이동, 이벤트, 공유 상태 교환 방식을 정한 규약이다.
- **디자인 시스템(Design System)**: 색상, 글꼴, 화면 구성요소의 공통 규칙을 제공해 독립 화면의 사용 경험을 맞추는 체계다.
- **오류 경계(Error Boundary)**: 한 마이크로 앱의 화면 오류가 전체 화면 중단으로 번지지 않도록 실패 영역을 격리하는 장치다.
- **빌드(Build)**: 소스 코드와 자원을 실행·배포 가능한 파일 묶음으로 변환하는 과정이다.
- **런타임(Runtime)**: 프로그램이 빌드된 뒤 실제 사용자 환경에서 실행되는 시점이다.
- **엣지 조합(Edge Composition)**: 사용자와 가까운 중계 서버에서 여러 화면 조각을 합쳐 응답하는 방식이다.

## Ⅰ. 개요

- 정의/개념: 업무별 웹 앱을 독립 배포·화면 조합
- **배경/필요성**: 단일 화면의 팀 결합으로 독립 배포 필요

### 쉽게 이해하기 (학습용)

- 큰 웹 화면을 주문·결제 같은 업무별 작은 앱으로 나눠 각 팀이 따로 배포하되 사용자에게는 한 서비스처럼 보여 준다.

## Ⅱ. 특징

- 업무 경계와 팀 소유권을 화면까지 일치한다.
- 부분 배포로 변경 실패의 영향 범위 축소한다.
- 독립 기술 선택이 중복 코드와 용량 증가 유발한다.
- 과도한 분리는 상태 공유·사용 경험 일치하지 않는다.

### 쉽게 이해하기 (학습용)

- 팀별 변경은 분리되지만 과도한 분할은 기능 중복과 화면 상태·양식 동기화 비용을 늘린다.

## Ⅲ. 아키텍처 및 구성요소

```mermaid
flowchart TB
    A["애플리케이션 셸"]
    B["통합 계약"]
    C["업무별 마이크로 앱"]
    D["디자인 시스템"]
    E["오류 경계"]
    A -->|화면 위치·경로| B
    B -->|이벤트·상태 규약| C
    D -->|공통 화면 규칙| A
    D -->|공통 화면 규칙| C
    E -->|실패 영역 격리| C
```

| 설계 요소 | 설명 |
|:---|:---|
| 애플리케이션 셸 | 공통 틀·라우팅과 화면 조합 담당 |
| 통합 계약 | 이동·이벤트·공유 상태 규약 정의 |
| 업무별 마이크로 앱 | 업무 경계별 독립 개발·배포 단위 |
| 디자인 시스템 | 화면 구성요소와 표현 규칙 공유 |
| 오류 경계 | 부분 오류를 해당 화면 영역에 격리 |

> 요약: 셸이 계약에 따라 독립 앱을 화면 조합

### 쉽게 이해하기 (학습용)

- 셸이 공통 화면 틀을 잡고 계약에 맞춰 업무 앱을 끼워 넣으며 디자인 규칙과 오류 경계가 일관성과 격리를 지킨다.

## Ⅳ. 원리 및 절차 흐름도

```mermaid
sequenceDiagram
    participant 사용자
    participant 셸
    participant 마이크로앱
    participant 오류경계
    사용자->>셸: 경로 판정
    셸->>마이크로앱: 앱 요청
    마이크로앱-->>셸: 마이크로 앱 적재
    셸->>마이크로앱: 계약 연결
    셸-->>사용자: 화면 조합
    alt 앱 실행 실패
        마이크로앱->>오류경계: 오류 격리
    end
```

| 절차 | 설명 |
|:---|:---|
| 경로 판정 | 주소에 맞는 업무 앱을 결정 |
| 앱 요청 | 셸이 대상 앱의 실행 파일을 요청 |
| 마이크로 앱 적재 | 독립 배포된 앱을 셸에 적재 |
| 계약 연결 | 이동·이벤트·공유 상태 규약 적용 |
| 화면 조합 | 공통 틀과 업무 화면을 함께 표시 |
| 오류 격리 | 실패 영역만 대체 화면으로 전환 |

> 요약: 경로별 앱을 계약으로 연결해 화면 조합

### 쉽게 이해하기 (학습용)

- 사용자가 메뉴를 열면 셸이 해당 업무 앱을 불러 공통 규칙으로 화면에 붙이고, 실패한 앱의 자리만 오류 화면으로 바꾼다.

## Ⅴ. 종류 및 비교

| 판단 기준 | 빌드 시 조합 | 런타임 조합 | 엣지 조합 |
|:---|:---|:---|:---|
| 핵심 특징 | 셸 빌드 때 앱을 하나로 결합 | 브라우저가 실행 중 앱을 적재 | 중계 서버가 화면 조각을 결합 |
| 적용 기준 | 버전 통제와 단순 운영 우선 | 팀별 독립 배포가 필수 | 서버별 화면 개인화 필요 |
| 주요 위험 | 셸 재배포·버전 충돌 | 초기 로딩 지연·중복 의존성 | 중계 지연·조합 실패 |

> 요약: 독립 배포 수준과 조합 위치로 방식 선택

### 쉽게 이해하기 (학습용)

- 한 번에 묶어 배포하면 단순하고, 실행 중 불러오면 팀 독립성이 높으며, 중계 서버에서 합치면 요청별 화면 구성이 쉽다.

## Ⅵ. 실무 사례

1. 쇼핑몰 상품·주문 화면을 팀별 독립 배포

### 쉽게 이해하기 (학습용)

- 상품 팀과 주문 팀이 각자 화면을 배포하면 한쪽의 잦은 변경 때문에 다른 팀의 배포를 기다리지 않아도 된다.

## Ⅶ. 결론

- 프런트엔드의 팀 자율성과 독립 배포를 확보하기 위해 **업무 경계·공통 디자인·런타임 중복·통합 테스트·조합 책임**을 검토하고, 필요한 독립성 수준에 맞는 마이크로 프런트엔드 조합 방식을 선택한다

### 쉽게 이해하기 (학습용)

- 화면을 조직도대로 무조건 쪼개지 말고 함께 바뀌는 업무 범위와 필요한 독립 배포 수준을 먼저 정해야 한다.
