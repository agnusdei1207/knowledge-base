...
- 정의/개념: **마이크로 프런트엔드**는 사용자 화면을 업무 경계별 웹 애플리케이션으로 나누어 팀이 독립 개발•배포하고 런타임이나 빌드 시점에 조합하는 프런트엔드 아키텍처이다.
- 배경/필요성: 단일 빌드 구조로 팀별 변경의 **독립 배포할 수 없다**.
...
- **업무 경계•팀 소유권** 일치가 핵심이다.
- **부분 독립 배포** 기반 실패 범위를 축소한다.
...
> 요약: 단일 배포는 **빌드 시**, 독립 배포는 **런타임 조합**을 선택한다.
...
- 업무별 독립 배포는 **런타임 조합**, 단순 운영은 **빌드 시 조합**이 핵심이다.
