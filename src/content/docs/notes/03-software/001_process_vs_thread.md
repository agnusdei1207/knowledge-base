...
date: "2026-08-05T13:00:19+09:00"
tags: [notes-software]
weight: 1
extra:
  question_no: "001"
...
- 배경/필요성: 단일 실행 단위로는 **장애 격리•저비용 상태 공유 동시 충족 곤란**

#### 한줄 요약

- **장애 격리•상태 공유 빈도**에 따른 실행 단위 선택
...
- **TCB•실행 문맥(TCB•Execution Context)**: 레지스터•스택•스케줄 상태를 보관하는 스레드별 구성
- **운영체제 스케줄러(OS Scheduler)**: 다음 실행 스레드를 선택하고 CPU 제어권을 넘기는 구성
- **공유 주소 공간(Shared Address Space)**: 같은 프로세스의 코드•힙•파일 참조를 스레드가 함께 사용하는 영역
- **IPC 채널(IPC Channel)**: 격리된 프로세스 주소 공간 사이에서 데이터를 전달하는 구성
...
1. **신뢰•장애 경계 판정**: 오류 전파와 권한 격리 요구 확인
2. **IPC 직렬화•전달**: 객체를 경계 통과 형식으로 변환•전송
3. **공유 상태 소유권 설정**: 변경 책임과 접근 범위 지정
4. **잠금•동기화**: 보호 구간의 순서와 값 가시성 보장

#### 한줄 요약

- **프로세스 IPC•스레드 동기화**의 통신 경로 구분
