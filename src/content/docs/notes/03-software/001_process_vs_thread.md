...
### 도표안 B. 스레드 디스패치·공유 상태 처리 시퀀스

```mermaid
sequenceDiagram
    participant Q as 요청 큐
    participant S as 운영체제 스케줄러
    participant A as 작업 스레드 A
    participant L as 잠금·공유 힙
    participant B as 작업 스레드 B
    participant I as IPC 채널
    participant P as 다른 프로세스

    Q->>S: ① 실행 가능한 작업·스레드 통지
    S->>A: ② TCB 문맥 복원·CPU 디스패치
    A->>L: ③ 공유 상태 잠금 요청
    L-->>A: ④ 보호 구간 소유권 부여
    A->>L: ⑤ 공유 데이터 갱신·잠금 해제
    L-->>B: ⑥ 갱신 가시화·대기 스레드 깨움
    A->>I: ⑦ 격리 프로세스용 메시지 직렬화
    I->>P: ⑧ 권한·경계를 거친 IPC 전달
    P-->>I: ⑨ 처리 결과 반환
    I-->>A: ⑩ IPC 응답 전달
    A->>S: ⑪ 차단·양보·시간 할당 종료
    S->>B: ⑫ A 문맥 저장 후 B 문맥 복원
```

#### 동작 원리

1. 요청 큐가 새 작업으로 실행 가능해진 스레드를 스케줄러에 알린다.
2. 스케줄러가 작업 스레드 A의 프로그램 카운터·레지스터·스택 포인터를 복원해 CPU를 넘긴다.
3. A가 같은 프로세스의 힙 데이터를 바꾸기 전에 잠금을 요청한다.
4. 잠금 계층이 소유권과 정해진 잠금 순서를 확인해 보호 구간 진입을 허용한다.
5. A가 공유 상태를 갱신하고 동기화 규칙에 따라 잠금을 해제한다.
6. 갱신값이 다른 스레드에 보이게 되고 기다리던 B가 실행 가능 상태로 바뀐다.
7. 다른 주소 공간의 기능이 필요하면 A가 명시적 IPC 메시지로 요청을 직렬화한다.
8. IPC 채널이 프로세스 권한과 메시지 경계를 검사해 다른 프로세스에 전달한다.
9. 다른 프로세스가 자신의 주소 공간에서 요청을 처리해 결과를 반환한다.
10. IPC 채널이 응답을 원래 스레드 A에 전달한다.
11. A가 I/O 대기·자발적 양보 또는 시간 할당 종료로 CPU를 스케줄러에 돌려준다.
12. 스케줄러가 A의 TCB에 실행 문맥을 저장하고 준비된 B의 문맥을 복원한다.
...
- 브라우저는 권한·탭별 렌더러를 별도 프로세스로 격리하고, 각 렌더러 내부의 병렬 작업은 제한된 스레드 풀로 처리한다.
