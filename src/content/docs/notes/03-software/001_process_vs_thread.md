...
```mermaid
sequenceDiagram
    participant S as 요청 큐·운영체제 스케줄러
    participant A as 작업 스레드 풀
    participant L as 잠금·공유 힙
    participant I as IPC 채널
    participant P as 다른 프로세스

    S->>A: ① 실행 가능 스레드의 TCB 문맥 복원
    A->>L: ② 공유 상태 잠금 요청
    L-->>A: ③ 보호 구간 소유권 부여
    A->>L: ④ 공유 데이터 갱신·잠금 해제
    L-->>A: ⑤ 갱신 가시화·대기 스레드 깨움
...
> 요약: 프로세스마다 주소 공간을, 스레드마다 실행 문맥을 둔다.

**동작 원리**

- **① 스레드 디스패치**: TCB에서 실행 문맥을 복원해 CPU 배정
- **② 잠금 요청**: 공유 힙 갱신 전에 보호 구간 진입 시도
- **③ 소유권 부여**: 잠금 순서를 확인해 접근 허용
- **④ 상태 갱신**: 데이터를 바꾸고 동기화 규칙에 따라 해제
- **⑤ 대기 해제**: 갱신값을 가시화하고 다음 스레드를 준비 상태로 전환
- **⑥ IPC 직렬화**: 다른 주소 공간용 요청을 메시지로 변환
- **⑦ 경계 전달**: 프로세스 권한과 메시지 형식 검사
- **⑧ 결과 반환**: 다른 프로세스가 자체 주소 공간에서 처리
- **⑨ 응답 전달**: IPC 채널이 원래 스레드에 반환
- **⑩ 문맥 전환**: 차단·양보·시간 만료 시 문맥 저장
...
- 권한·장애 격리와 상태 공유 빈도를 검토해 강한 격리에는 프로세스, 빈번한 공유에는 스레드를 사용한다.
