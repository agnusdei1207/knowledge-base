...
## Ⅰ. 개요
- **정의/개념**: 프로세스는 자원, 스레드는 실행 단위
- **배경/필요성**: 공유 오류가 전파돼 격리·통신 단위 필요
...
- 프로세스 주소 공간은 장애·권한을 격리한다
- 프로세스 간 공유는 IPC·전환 비용을 높인다
- 스레드는 자원 공유로 통신 비용을 낮춘다
- 스레드 오류·경쟁은 같은 프로세스에 전파된다
...
| 주요 위험 | IPC·주소 공간 전환 비용 | 오류 전파·동기화 결함 |
...
1. 브라우저 렌더러를 프로세스로 분리해 장애 격리
2. 웹 서버는 스레드 풀로 요청 큐·캐시 공유

### 쉽게 이해하기 (학습용)

- 브라우저 화면 처리를 별도 작업실에 두면 하나가 고장 나도 다른 작업실은 남는다
- 웹 서버는 작업자들을 미리 두고 같은 요청 목록과 임시 저장 자료를 나눠 쓴다
