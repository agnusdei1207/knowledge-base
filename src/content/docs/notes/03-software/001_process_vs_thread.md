...
- **공유 메모리(Shared Memory)**: 두 개 이상의 프로세스가 동일한 물리 메모리 세그먼트를 공유하여 가장 빠른 속도로 데이터를 교환하는 IPC 기법.
- **동기화(Synchronization)**: 다중 스레드가 공유 자원에 동시 접근할 때 데이터 일관성을 유지하기 위한 상호 배제 제어 기법.
</details>

- 정의: 독립된 가상 주소 공간과 자원 보호 컨테이너를 갖는 프로세스 및 자원 공유 기반 경량 CPU 스케줄링 실행 단위인 스레드
- 배경: 단일 실행 흐름은 장애 격리와 **동시 처리량** 확보에 한계
...
- 스레드의 메모리 공유로 통신 비용 감소와 **동기화** 필요
- 주소 공간 전환 여부에 따라 **문맥 전환 비용** 차이
...
2. IPC 직렬화·전달: 프로세스 선택 시 데이터 **직렬화** 및 **IPC** 파이프라인 수용.
...
| 결함 영향 | 주소 공간 단위 **결함 격리** | 메모리 훼손 시 프로세스 전체 영향 가능 |
| 통신 방식 | 파이프•공유 메모리•소켓 등 **IPC** | 공유 힙에 직접 접근 |
| 문맥 전환 비용 | 주소 공간 전환 시 증가 가능 | 동일 주소 공간이면 상대적으로 낮음 |
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 스레드 간 힙 공유로 인한 경합과 데드락 | **불변 객체**와 잠금 순서 적용 | 데이터 경합과 순환 대기 완화 |
| 스레드 메모리 오류의 프로세스 전체 전파 | **신뢰 경계** 단위 프로세스 분리 | 장애 영향 범위 축소 |
...
- 신뢰 경계는 프로세스로 분리하고 반복 작업은 **스레드 풀**로 제한한다.
...
- 장애•보안 경계는 **프로세스**, 빈번한 상태 공유는 **스레드** 선택
