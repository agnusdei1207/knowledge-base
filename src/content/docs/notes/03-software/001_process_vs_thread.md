...
- 프로세스 간 **가상 주소 공간(Virtual Address Space)** 완전 독립에 의한 결함 격리(Fault Isolation) 및 보안 격리 범위 확보
- 동일 프로세스 내 스레드 간 메모리 공유 범위 확장에 따른 초저지연 데이터 통신 및 락(Lock) 동기화 필요
...
```text
                   [운영체제 스케줄러]
                            |
 ┌────────────── 프로세스 경계 ──────────────┐
 │ [프로세스 제어 블록(PCB)] -- [공유 주소 공간] -- [스레드 제어 블록(TCB)•실행 문맥] │
 └───┬───────────────────────────────────────┘
     |
 [프로세스 간 통신(IPC) 채널]
```
...
```text
 ...
 1. 신뢰•장애 경계 판정
    ...
    [프로세스]       [스레드]
       │               │
       ▼               ▼
 2. 프로세스 간 통신(IPC) 직렬화•전달 3. 공유 상태 소유권 설정
       │               │
       │               ▼
       │          4. 잠금•동기화
       └───────┬───────┘
