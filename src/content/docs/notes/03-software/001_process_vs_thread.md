...
date: "2026-07-30T23:16:59+09:00"
tags: [notes-software]
weight: 1
extra:
  question_no: "001"
...
- **스레드 풀·요청 큐(Thread Pool·Request Queue)**: 스레드 풀은 미리 만든 작업 스레드 집합이고 요청 큐는 대기 작업 목록
...
- **공유 메모리(Shared Memory)**: 여러 프로세스가 같은 물리 메모리 영역을 매핑해 데이터를 교환하는 프로세스 간 통신 방식

> **키워드:** 프로세스 vs 스레드 (Process vs Thread)

## Ⅰ. 개요
- 정의/개념: 독립 자원을 가진 **프로세스**와 자원을 공유하는 **스레드**의 실행 단위 분류
...
1. **실행 문맥**: 프로그램 카운터·레지스터 복원으로 실행 재개
2. **잠금 요청**: 공유 상태의 동시 접근 순서 조정
3. **보호 구간 권한**: 잠금 소유 스레드만 상태 변경 허용
4. **공유 상태 갱신값**: 값 반영과 잠금 해제로 변경 가시화
5. **직렬화 요청**: 객체를 바이트 형식으로 바꿔 프로세스 경계 통과
