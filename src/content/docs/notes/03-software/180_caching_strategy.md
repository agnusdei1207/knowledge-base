...
- 배경/필요성: 대규모 동시 접속 트래픽 환경에서 모든 사용자 읽기/쓰기 요청이 원본 RDBMS로 직접 유입될 경우 발생하는 디스크 I/O 병목, 커넥션 풀 고갈 및 급격한 응답 지연(Latency)을 해소하고, 캐시와 DB 간의 데이터 정합성 불일치를 방어하기 위해, 초고속 인메모리 저장소(Redis, Memcached)에 데이터를 적재·조회·무효화하는 다양한 캐싱 패턴(Cache-Aside, Write-Through, Write-Behind)을 도입하여 **데이터베이스 부하 분산과 서브 밀리초(sub-millisecond) 초저지연 읽기 성능을 달성**할 필요
...
- 현대 고성능 대규모 웹 서비스 및 마이크로서비스 아키텍처의 백엔드 처리량 확장을 위한 **가장 핵심적인 인메모리 성능 최적화 패턴**으로 정립되었으며, 실무 구축 시에는 **읽기 중심 일반 조회에는 지연 적재(Cache-Aside)와 쓰기 시 캐시 삭제(Cache Eviction)를 적용하고, 동시 다발적 캐시 만료 시 DB 폭주를 막는 Cache Stampede 방어(분산 락/PER 알고리즘/TTL Jitter), 존재하지 않는 키 조회를 막는 Bloom Filter(Cache Penetration 방어), 캐시 장애 시 DB를 격리하는 서킷 브레이커**를 결합하여 안정적인 초고속 데이터 서빙 체계를 완성
