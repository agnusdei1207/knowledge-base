---
sidebar:
  order: 42
  label: "042. 서비스 메시 — Istio·Envoy (Service Mesh)"
  badge:
    text: "기출 · 70%"
    variant: note
title: "서비스 메시 — Istio·Envoy (Service Mesh)"
date: "2026-07-25T00:35:00+09:00"
tags:
  - "notes-software"
weight: 42
extra:
  question_no: "042"
  source_status: "기출"
  source_history: "123회, 138회"
  priority: 70
  priority_note: "123·138회 반복, 메시 기반 통신 제어"
---

## 미리 알고가기

- **서비스 메시(Service Mesh)**: 서비스 코드 밖의 프록시 계층에서 서비스 간 통신 정책을 일관되게 실행하는 인프라
- **제어 플레인(Control Plane)**: 통신 정책을 검증해 프록시 구성과 인증서로 배포하는 관리 영역
- **데이터 플레인(Data Plane)**: 요청 경로의 프록시가 라우팅·보안·관측 정책을 실행하는 영역
- **이스티오(Istio)**: ‘이스티오’로 읽는 공식 프로젝트 이름으로 별도 영문 확장어를 만들지 않으며 서비스 메시의 제어 플레인과 정책 관리 기능을 제공함
- **엔보이(Envoy)**: ‘엔보이’로 읽는 공식 프로젝트 이름으로 별도 영문 확장어를 만들지 않으며 라우팅·보안·관측 정책을 요청 경로에서 실행하는 응용 계층 프록시
- **워크로드(Workload)**: 메시 정책이 적용되는 서비스 실행 단위
- **사이드카 모드(Sidecar Mode)**: 워크로드마다 전용 프록시 배치
- **앰비언트 모드(Ambient Mode)**: 노드 프록시와 선택적 웨이포인트 공유
- **웨이포인트 프록시(Waypoint Proxy)**: 공유 응용 계층 정책을 실행하는 프록시
- **상호 전송 계층 보안(Mutual Transport Layer Security, mTLS)**: ‘엠티엘에스’로 읽고 상호 인증을 뜻하는 소문자 m을 TLS 앞에 붙이는 규격 관례 표기이며 통신 양쪽의 신원을 인증하고 전송 데이터를 암호화함
- **인증(Authentication)·인가(Authorization)**: 호출 서비스의 신원을 확인하고 대상 서비스에 접근할 권한을 판정하는 절차
- **서비스 탐색(Service Discovery)**: 대상 서비스 주소·엔드포인트 조회
- **텔레메트리(Telemetry)**: 프록시가 통신 상태를 관측하도록 생성하는 메트릭·로그·분산 추적 정보
- **노드 프록시(Node Proxy)**: 앰비언트 모드에서 같은 노드의 여러 워크로드가 공유하며 전송 계층 연결과 mTLS를 처리하는 프록시
- **인증서(Certificate)**: 공개키와 서비스 신원을 서명해 묶은 전자 문서로서 제어 플레인이 배포하고 프록시가 mTLS 상호 인증에 사용함
- **엔드포인트(Endpoint)**: 서비스 요청을 받을 수 있는 실제 워크로드의 네트워크 주소와 포트
- **응용 계층(Application Layer)**: 요청 경로·메서드·서비스 신원 같은 응용 프로토콜 정보를 해석해 웨이포인트가 세밀한 정책을 적용하는 계층
- **텔레메트리 백엔드(Telemetry Backend)**: 프록시가 보낸 메트릭·로그·분산 추적을 저장·검색·시각화하는 관측 시스템
- **트래픽 가중치(Traffic Weight)**: 전체 요청 중 각 서비스 버전으로 보낼 비율을 정하는 값으로서 신버전 배포 범위를 단계적으로 조절함

## Ⅰ. 개요

- **정의/개념**: 서비스 간 통신을 프록시로 제어하는 인프라 계층
- **배경/필요성**: 통신 정책 중복이 변경을 확산해 프록시 필요

### 쉽게 이해하기 (학습용)

- 배달마다 같은 검문 규칙을 쓰도록 공통 안내소가 통신을 맡는다

## Ⅱ. 특징

- 서비스 코드와 라우팅·보안·관측 정책 분리한다.
- 제어 플레인 배포와 데이터 플레인 실행 역할 분담한다.
- 프록시 홉과 제어 플레인 운영에 지연·자원 비용 발생한다.

### 쉽게 이해하기 (학습용)

- 규칙은 한곳에서 관리하기 쉽지만 모든 요청이 프록시를 지나 비용이 든다

## Ⅲ. 아키텍처 및 구성요소

```mermaid
flowchart LR
    O[운영자]
    T[텔레메트리 백엔드]
    subgraph M[서비스 메시 경계]
        C[Istio 제어 플레인]
        A[호출 워크로드]
        P[데이터 플레인 프록시 집합]
        B[대상 워크로드]
        C -->|구성·인증서| P
        A -->|서비스 요청| P
        P -->|mTLS 중계| B
    end
    O -->|트래픽·보안 정책| C
    P -.->|메트릭·로그·추적| T
```

| 설계 요소 | 설명 |
|:---|:---|
| Istio 제어 플레인 | 정책을 프록시 구성·인증서로 배포 |
| 호출 워크로드 | 프록시를 통해 대상 서비스 요청 |
| 데이터 플레인 프록시 집합 | Envoy 등이 통신 정책 실행 |
| 대상 워크로드 | 허용된 요청의 업무 로직 처리 |

> 요약: Istio는 정책 배포, Envoy는 요청 경로에서 실행

### 쉽게 이해하기 (학습용)

- 관리실인 Istio가 규칙을 보내면 안내원인 Envoy가 요청마다 실행한다

## Ⅳ. 원리 및 절차 흐름도

```mermaid
sequenceDiagram
    participant O as 운영자
    participant C as Istio 제어 플레인
    participant W as 호출 워크로드
    participant D as 데이터 플레인
    participant S as 대상 워크로드
    participant T as 텔레메트리 백엔드
    O->>C: 메시 정책 등록
    C->>D: 구성·인증서 배포
    W->>D: 서비스 요청 전달
    D->>D: 라우팅·신원 판정
    D->>S: mTLS 요청 중계
    S-->>D: 서비스 응답 반환
    D-->>W: 응답 전달
    D->>T: 텔레메트리 전송
```

| 절차 | 설명 |
|:---|:---|
| 메시 정책 등록 | 운영자가 트래픽·보안 정책 등록 |
| 구성·인증서 배포 | 제어 플레인이 프록시에 구성 배포 |
| 서비스 요청 전달 | 호출 서비스가 데이터 플레인에 요청 |
| 라우팅·신원 판정 | 프록시가 경로·신원·인가 판정 |
| mTLS 요청 중계 | 프록시가 대상 서비스에 암호화 중계 |
| 서비스 응답 반환 | 대상 서비스가 처리 결과 반환 |
| 응답 전달 | 프록시가 호출 서비스에 응답 전달 |
| 텔레메트리 전송 | 프록시가 메트릭·로그·추적 전송 |

> 요약: 정책 배포 후 요청별 라우팅·보안·관측 실행

### 쉽게 이해하기 (학습용)

- 규칙을 받은 안내원이 길과 신원을 확인하고 결과 기록을 보낸다

## Ⅴ. 종류 및 비교

| 판단 기준 | 사이드카 모드 | 앰비언트 모드 |
|:---|:---|:---|
| 핵심 특징 | 워크로드별 전용 프록시 | 노드 프록시·웨이포인트 공유 |
| 적용 기준 | 워크로드별 세밀한 정책이 필요할 때 | 사이드카 운영 비용을 줄일 때 |
| 주요 위험 | 프록시 수만큼 자원 증가 | 공유 프록시 장애 범위 확대 |

> 요약: 정책 세분성과 공유 프록시 비용으로 배치 방식 선택

### 쉽게 이해하기 (학습용)

- 전용 안내원을 둘지 여러 서비스가 안내원을 공유할지 비용과 규칙 범위로 정한다

## Ⅵ. 실무 사례

1. 결제 서비스는 mTLS 신원·인가 정책으로 호출 서비스 제한
2. 신버전 배포는 트래픽 가중치로 요청 비율 제어

### 쉽게 이해하기 (학습용)

- 결제 통신은 양쪽 신원을 확인하고 허가된 서비스 요청만 전달한다
- 새 버전에는 정해진 비율의 요청만 보내며 가중치로 범위를 바꾼다

## Ⅶ. 결론

- 서비스 간 정책 중복이 프록시 비용보다 클 때 메시 도입

### 쉽게 이해하기 (학습용)

- 여러 서비스가 같은 통신 규칙을 반복하면 프록시 비용과 비교해 메시를 둔다
