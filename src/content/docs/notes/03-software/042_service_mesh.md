...
- **서비스 메시(Service Mesh)**: 마이크로서비스 간 통신 제어(라우팅·인증·관측)를 애플리케이션 코드 외부의 인프라 프록시 계층이 처리하는 분산 네트워크 제어 아키텍처.
- **컨트롤 플레인(Control Plane)**: 라우팅 정책·mTLS 인증서를 생성하여 데이터 플레인 프록시에 배포하는 서비스 메시 제어 컴포넌트(Istiod).
- **데이터 플레인(Data Plane)**: 컨트롤 플레인의 정책을 수신하고 애플리케이션 컨테이너 옆에 사이드카로 배치되어 트래픽을 가로채 처리하는 Envoy 프록시.

</details>

- 정의/개념: 애플리케이션 코드 수정 없이 사이드카 프록시가 서비스 간 통신·보안·관측을 처리하는 인프라 계층 **서비스 메시(Service Mesh / Istio)**
- 배경/필요성: 서비스마다 통신 라이브러리를 내장하면 정책 변경 시 전체 서비스 재배포가 필요하고 버전 파편화가 발생하므로, **애플리케이션 독립 인프라 수준 통신 제어 체계 필수**
