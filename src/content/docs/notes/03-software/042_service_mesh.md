...
- 배경/필요성: 폴리글랏(Polyglot) 마이크로서비스 환경에서 서비스 간 내부 통신(East-West)에 필요한 보안 암호화(mTLS), 트래픽 라우팅, 회복성(재시도·서킷브레이커) 및 분산 관측성을 애플리케이션 코드/SDK마다 중복 구현할 때 발생하는 높은 유지보수 비용과 정책 배포 종속성을 극복하고, 컨트롤 플레인(Istiod)과 데이터 플레인(Envoy Sidecar) 분리를 통해 **코드 수정 없는 투명한 제로 트러스트 보안(mTLS)과 지능적 트래픽 제어를 인프라 계층으로 일원화**할 필요
...
- 쿠버네티스 기반 대규모 마이크로서비스 인프라의 **핵심 East-West 네트워크 통신 거버넌스 및 제로 트러스트 표준 계층**으로 확립되었으며, 실무 구축 시에는 **Istio/Envoy 기반 mTLS 및 카나리 라우팅 운영을 기본으로, 사이드카 리소스 오버헤드와 지연을 극소화하기 위한 Istio Ambient Mesh(ztunnel/Waypoint) 및 eBPF(Cilium) 가속, 무중단 업그레이드를 위한 Revision 기반 컨트롤 플레인 롤아웃**을 결합하여 운영
