...
- **API·메시지 처리 설계 원칙**: 멱등성 설계는 재시도와 중복 요청을 식별해 한 번만 처리하거나 최초 결과를 재사용하는 API·메시지 처리 원칙이다.

</details>

- 정의/개념: 분산 시스템의 재시도·중복 요청을 식별해 한 번만 처리하거나 최초 결과를 재사용함으로써 동일한 업무 효과를 보장하는 **API·메시지 처리 설계 원칙**
...
- **멱등 키·요청 지문**: 멱등 키는 같은 업무 재시도를 식별하고 요청 지문은 같은 키로 다른 본문이 들어오는 충돌을 검출한다.
