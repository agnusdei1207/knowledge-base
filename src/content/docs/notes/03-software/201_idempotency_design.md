...
- 정의/개념: 재시도에도 **동일한 최종 업무 효과**를 보장하는 처리 원칙이다.
- 배경/필요성: 재시도•중복 전달로 **중복 결제**가 발생한다.
...
- **원자적 확정(Atomic Commit)**: 처리권과 업무 결과를 모두 반영하거나 모두 반영하지 않아 중간 상태를 남기지 않는 처리한다.
- **응답 재생(Response Replay)**: 중복 요청에 업무를 다시 실행하지 않고 최초 저장 응답을 반환하는 방식이다.

</details>

- **멱등 키•요청 지문** 기반 동일 요청 식별이 핵심이다.
- **처리권•업무 결과**를 원자적으로 확정해 단일 효과 보장이 핵심이다.
- **응답 재생•보존 기간** 기반 재시도를 통제한다.
...
> 요약: 게이트가 최초 요청만 실행하고 저장 응답 재생이 핵심이다.
...
2. **처리권 획득**: 최초 실행권 원자를 획득한다.
3. **업무 수행**: 처리권을 얻은 요청만을 실행한다.
4. **결과 확정**: 업무 결과•응답을 함께 저장한다.

> 요약: 최초 요청만 실행하고 중복에는 저장 응답 반환이 핵심이다.
...
- **조건부 갱신 적용**: 기대 버전과 현재 값의 일치 여부로 동시 수정 충돌 감지이다.

</details>

- 생성•결제 재시도는 **멱등성 키**, 동시 갱신은 **조건부 갱신**을 적용한다.
