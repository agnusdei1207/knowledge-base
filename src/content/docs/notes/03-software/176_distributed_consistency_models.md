...
- 정의/개념: 복제본의 관찰 순서•최신성을 정한 **분산 일관성 모델**이다.
- 배경/필요성: 네트워크 지연•분할로 **모든 복제본의 즉시 동일 상태 유지할 수 없다**.
...
- **실제 시간•프로그램•인과•수렴** 기반 강도를 구분한다.
- **리더•합의•쿼럼** 기반 복제본을 조정한다.
...
- **인과 순서**: 원인과 결과가 있는 연산의 선후를 보존하는 보장이다.
...
- **업무 불변식•지연 목표•분할 가용성**으로 최소 일관성 강도를 결정한다.
