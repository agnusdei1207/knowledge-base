...
- **분산 일관성 모델(Distributed Consistency Model)**: 여러 복제본의 읽기·쓰기 결과가 보이는 순서와 시점을 정한 규약
- **연산 이력(History)**: 각 읽기·쓰기의 호출·응답 시점과 반환값을 기록한 실행 내역
- **선형화 가능성(Linearizability)**: 연산이 한 시점에 실행된 것처럼 보이고 실제 시간 선후를 지키는 모델
- **순차 일관성(Sequential Consistency)**: 연산이 하나의 전역 순서로 보이고 각 프로세스 순서를 지키는 모델
- **인과 일관성(Causal Consistency)**: 원인과 결과 관계가 있는 연산 순서를 모든 관찰자에게 보장하는 모델
- **최종 일관성(Eventual Consistency)**: 새 쓰기가 없으면 모든 복제본이 결국 같은 값으로 수렴하는 모델
- **논리·벡터 시계(Logical·Vector Clock)**: 물리 시각 대신 사건의 선후·인과·동시 관계를 나타내는 버전 정보
- **쿼럼(Quorum)**: 전체 복제본 중 읽기나 쓰기 완료에 필요한 최소 응답 수
- **세션 보장(Session Guarantee)**: 자기 쓰기 읽기·단조 읽기처럼 한 사용자의 연속 요청에 제공하는 보장
- **조건부 쓰기(Conditional Write)**: 현재 버전이 예상값과 같을 때만 변경하는 연산

## Ⅰ. 개요

- 정의/개념: 복제본의 **읽기·쓰기 관찰 순서·최신성 규약**
- 배경/필요성: 정확성·지연·가용성의 **데이터별 균형 선택**
...
1. **새 값 쓰기 요청**: 업무 값·현재 버전 조건 제공
2. **순서·버전 복제**: 첫 복제본의 안정 기록 요청
3. **순서·버전 복제**: 추가 복제본의 동일 기록 요청
4. **쿼럼 완료 반환**: 요구 승인 수 충족 후 성공
5. **완료 후 읽기**: 모델이 보장한 최신 버전 조회
...
| 일관성 모델 | 선형화 | 인과 일관성 | 최종 일관성 |
|:---|:---|:---|:---|
| 적용 기준 | 재고·잔액의 **실시간 불변식** | 대화·협업의 **원인 순서** | 피드·캐시의 **최종 수렴** |
| 핵심 특징 | 실제 시간·**전역 최신성** | 인과 문맥·**관련 순서** | 비동기 복제·**충돌 병합** |
| 한계 | 조정 지연·**분할 시 거부** | 문맥·메타데이터 **비용** | 오래된 읽기·**일시 충돌** |

### 쉽게 이해하기 (학습용)

- 선형화는 실제 시간까지 맞추고 인과 일관성은 답글이 원문보다 먼저 보이지 않게 하며 최종 일관성은 잠시 달라도 결국 같은 값으로 모인다.
...
| 저장소 명칭의 **보장 오해** | API별 장애 조건·**이력 시험** | 실제 보장 수준 확인 |
