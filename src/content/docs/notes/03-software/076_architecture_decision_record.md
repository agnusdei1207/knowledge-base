...
- **이점•비용•후속 조치**: 선택으로 얻는 효과와 감수할 부담, 구현 이후 수행해야 할 작업이다.
...
- **맥락•대안•근거•부정 결과**: 결정 당시의 조건과 후보, 선택 이유 및 그 선택으로 감수할 불이익을 함께 기록한 정보이다.
- **아키텍처 드리프트(설계 드리프트)**: 구현이 누적되며 실제 구조가 기록한 설계 의도에서 벗어나는 현상이다.
- **변경•모듈•검증 증거 연결**: ADR을 관련 코드 변경과 적용 모듈, 시험 결과에 이어 결정의 구현 여부를 추적하는 방식이다.
- **새 ADR•대체 상태 연결**: 원 기록을 보존하며 변경 이유와 후속 결정을 잇는 방식이다.
- **소유자•검토 기한•상태 점검**: 결정 기록의 책임자와 재검토 시점 및 현재 유효성을 명시하는 관리 기준이다.
