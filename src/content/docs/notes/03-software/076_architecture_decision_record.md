...
- 배경/필요성: 인원 교체 시 "이 아키텍처를 왜 이렇게 설계했는가?"에 대한 맥락(Context) 유실(Architecture Drift) 차단, 구두/위키에 산재된 아키텍처 결정의 버전 추적성 확보 요구성
...
## Ⅲ. 구조 및 구성요소 (Michael Nygard 표준 5대 영역 템플릿)
...
├─────────────────────────────────────────────────────────────────────────┤
│ Consequences:                                                           │
│ (+) 장점: 완벽한 감사 이력 확보, 데이터 유실 0%, 조회 성능 10배 향상      │
...
## Ⅳ. 흐름도 (ADR 수명주기: Lifecycle States)
...
1. **Proposed**: 아키텍트가 신규 아키텍처 제안을 `adr-0005.md` 파일로 작성 후 Git PR 생성.
2. **Accepted**: 팀원 코드 리뷰 및 합의 거쳐 `Accepted` 상태로 메인 브랜치 병합.
3. **Superseded**: 2년 후 시스템 변경 시 기존 문서를 지우지 않고 `Status: Superseded by ADR-0012`로 표기하고 새 ADR-0012 추가.
...
| 버전 동기화 | 코드와 문서의 동기화 파열 (**Drift 상존**) | **Git Commit / Pull Request와 100% 동기화** |
...
| ADR 덮어쓰기 작성으로 과거 결정 이유 파악 불가능 | **`adr-tools` 이용한 Superseded 상태 연쇄 링크 관리** | 이력 추적성 완벽 |
...
- **ADR 수립 기준**에 따라 지속가능한 소프트웨어 아키텍처 관리를 위해 **Git 기반 ADR (Michael Nygard 템플릿)** 수용
