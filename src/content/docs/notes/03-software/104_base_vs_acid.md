...
date: "2026-08-26T18:14:00+09:00"
tags:
  - "notes-software"
weight: 104
extra:
...
- 배경/필요성: 분산 노드에 ACID를 그대로 강행하면 2PC가 커밋 확정까지 모든 참여 노드의 자원을 잠가 대기 비용이 노드 수만큼 곱해지고 한 노드의 장애가 전체 트랜잭션을 멈추게 하므로, 각 노드는 로컬에서만 즉시 확정하고 노드 사이는 이벤트로 최종 수렴시키는 완화된 정합성 계층이 필요
...
- 원장 보호는 **ACID 트랜잭션**, 분산 연계는 **BASE** 선택
