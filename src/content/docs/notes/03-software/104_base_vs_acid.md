...
- **1. 기준 거래 요청**: 즉시 일관성이 필요한 상태 변경
- **2. 데이터·Outbox 원자 커밋**: 기준 상태와 이벤트 동시 확정
- **3. ACID 결과 확정**: 불변식 충족 거래만 외부에 응답
- **4. 커밋 이벤트 조회**: 확정된 후속 변경만 전달
- **5. 파생 상태 수렴**: 멱등 적용으로 BASE 사본을 최종 일치
...
> 판정 원칙: 제품 분류가 아닌 **실제 트랜잭션 범위·복제 지연** 확인
