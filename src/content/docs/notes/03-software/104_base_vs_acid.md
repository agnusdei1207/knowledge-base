...
- **1. 업무 변경 요청**: 기준 상태 변경을 요청한다.
- **2. 기준 데이터·Outbox 커밋**: 함께 확정한다.
- **3. ACID 커밋 결과**: 불변식 결과를 반환한다.
- **4. 기준 거래 결과 응답**: 거래 결과를 알린다.
- **5. 미전송 이벤트 조회**: 후속 변경을 찾는다.
- **6. 커밋 이벤트 반환**: 확정된 변경만 보낸다.
- **7. 파생 상태 반영**: 멱등 방식으로 수렴한다.
...
- **불변식·허용 지연·보상 가능성**으로 ACID와 BASE를 조합한다.
