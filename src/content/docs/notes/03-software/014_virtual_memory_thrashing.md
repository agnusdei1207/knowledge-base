...
- **정의/개념**: 페이지 교체가 실행을 압도하는 상태
- **배경/필요성**: 교체 I/O가 실행을 막아 프레임 조정 필요
...
- 총 워킹 셋이 가용 프레임을 넘으면 CPU 이용률 하락한다.
...
2. 가상 머신 호스트는 워킹 셋 기준 최소 프레임 배분

### 쉽게 이해하기 (학습용)

- 서버의 창고 왕복이 기준을 넘으면 동시에 실행할 작업을 줄인다
- 가상 머신이 자주 쓰는 자료만큼은 메모리에 머물도록 최소 책상 크기를 보장한다
