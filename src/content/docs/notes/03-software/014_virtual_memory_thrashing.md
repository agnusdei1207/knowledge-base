...
![할당 프레임과 워킹 셋 충족 여부에 따른 페이지 폴트율](/study/diagrams/working-set-thrashing.svg)

> 워킹 셋을 6프레임으로 가정한 파란 선은 할당량이 그보다 작을 때 페이지 폴트가 반복되고 충족한 뒤 안정되는 원리를 나타내며, 실제 경계는 참조 지역성과 부하에 따라 달라진다.
...
- **1. 비상주 페이지 참조**: 폴트 발생
- **2. 교체 대상 선택**: 가용 프레임 확인
- **3. 페이지 교환·실행 재개**: 퇴출·적재와 주소 매핑 수행
- **4. 반복 폴트 지표 전달**: 스레싱 판정
- **5. 프레임 재배분 요청**: 작업 수 조정
