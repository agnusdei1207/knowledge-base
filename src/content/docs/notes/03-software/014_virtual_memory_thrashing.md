...
- 정의/개념: 가용 물리 메모리 부족으로 인해 지속적인 Page Fault 및 Swap In/Out 입출력 연산이 폭증하여 CPU 이용률이 급격히 추락하는 **프로세스 스레싱(Process Thrashing)**
...
|:---|:---|:---|
| Page Fault 발생률 | 간헐적 발생 (낮음) | 폭발적 지속 발생 (극도로 높음) |
| CPU 이용률 | 70% ~ 90% 이상 (유용한 연산 수행) | 10% 미만으로 급감 (I/O Wait 90% 이상) |
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| DOM 과도화로 인한 전체 가상 메모리 스레싱 | **Working Set Model** 기반 필요 프레임 보장 및 **DOM 축소** | 시스템 스루풋 정상화 |
