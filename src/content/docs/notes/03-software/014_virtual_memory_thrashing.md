...
date: "2026-08-05T12:38:52+09:00"
tags: [notes-software]
weight: 14
extra:
  question_no: "014"
...
- **워킹 셋 추적기(Working-set Tracker)**: 최근 참조 페이지로 필요 프레임을 추정하는 구성요소
- **상주 집합 관리자(Resident-set Manager)**: 물리 프레임의 할당과 교체를 수행하는 구성요소
...
- **재참조 폴트 증가**: 워킹 셋 페이지가 퇴출된 뒤 다시 참조돼 폴트가 반복되는 단계
- **교체 I/O 증가**: 퇴출과 재적재가 늘어 저장장치 작업이 증가하는 단계
- **CPU 이용률 저하**: 프로세스가 페이지 입출력을 기다려 CPU 유효 실행이 줄어드는 단계
- **다중프로그래밍 증가**: 낮은 CPU 이용률을 작업 부족으로 오판해 실행 프로세스를 늘리는 단계
- **프로세스별 프레임 감소**: 추가 프로세스와 프레임을 나눠 워킹 셋 부족이 심해지는 단계
...
- **1. 재참조 폴트 증가**: 퇴출 페이지의 반복 참조로 폴트 급증
- **2. 교체 I/O 증가**: 페이지 퇴출•재적재 작업 확대
- **3. CPU 이용률 저하**: I/O 대기로 유효 실행 감소
- **4. 다중프로그래밍 증가**: CPU 유휴를 작업 부족으로 오판
- **5. 프로세스별 프레임 감소**: 워킹 셋 부족과 폴트 악화

#### 한줄 요약

- 폴트•I/O•CPU 유휴의 **양의 피드백** 차단
...
- **주요 페이지 폴트(Major Page Fault)**: 저장장치 읽기가 필요한 페이지 폴트
- **경미 페이지 폴트(Minor Page Fault)**: 메모리 주소 매핑만 복구하는 페이지 폴트
- **수용 제어(Admission Control)**: 메모리 여유를 검사해 새 작업의 실행 허용 여부를 결정하는 절차
- **재참조량(Refault Count)**: 최근 퇴출한 페이지를 다시 요구한 횟수
- **교체량(Page Swap Volume)**: 메모리와 저장장치 사이에서 바꾼 페이지 양
...
- **폴트율 상한**: 스레싱 판정을 위한 페이지 폴트 빈도 기준
- **워킹 셋 수용**: 작업 수와 프레임 재배분을 정하는 용량 기준
