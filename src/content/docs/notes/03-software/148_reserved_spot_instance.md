...
- 정의/개념: 일정 사용량을 약정하는 **예약 인스턴스**와 자원 회수를 수용하는 **스팟 인스턴스**로 컴퓨팅 단가를 낮춘다.
- 배경/필요성: 온디맨드만 사용하면 지속 수요 비용과 할인 기회 손실이 발생한다.
...
- 기준 수요의 사용 기간을 약정하는 **예약 할인**이 핵심이다.
- 회수 가능한 여유 자원을 쓰는 **스팟 용량**이 핵심이다.
- 약정•중단 없이 변동분을 받는 **온디맨드**가 핵심이다.
...
- **작업 큐**: 스팟 중단 시 미완료 작업을 다른 자원에 다시 배정하는 구성요소이다.
- **체크포인트**: 마지막 진행 상태부터 작업을 재개하게 하는 복구 지점이다.
...
선의 의미: 기준선 분석은 지속 수요를 예약 포트폴리오에, 중단 가능 수요를 스팟 풀에, 변동 수요를 온디맨드 용량에 대응시키며, 작업 큐•체크포인트는 스팟과 온디맨드 사이의 복구 기반을 제공한다.

| 구성요소 | 책임 |
|:---|:---|
| 기준선 분석 | **기준선 분석**이 지속•변동•중단 허용 수요 분리 |
| 예약 포트폴리오 | **예약 포트폴리오**가 약정 범위•적용률•사용률 관리 |
| 온디맨드 용량 | **온디맨드 용량**이 급증•예측 오차 처리 |
| 스팟 풀 | **스팟 풀**이 유형•영역별 여유 용량 탐색 |
| 작업 큐•체크포인트 | **작업 큐**와 **체크포인트**가 재시도•진행 상태 보존 |
...
- **지속 기준 수요**: 예측 가능한 최소 사용량을 예약 할인에 배정하는 선택 축이다.
- **중단 복구 가능 작업**: 외부 체크포인트에서 재개할 수 있어 스팟에 적합한 작업이다.
- **신규•급증 변동 수요**: 약정 없이 즉시 온디맨드로 처리할 수요이다.
- **스팟 회수 재시도**: 저장된 진행 지점부터 다른 스팟 풀에서 작업을 재개하는 통제이다.
- **기한 임계값 전환**: 남은 시간이 부족하면 온디맨드 용량으로 바꾸는 통제이다.
...
- 회수 시 **스팟 회수 재시도**를 수행하고, 마감이 가까우면 **기한 임계값 전환**으로 온디맨드 용량을 사용한다.
...
- **구매 모델 선택 축**: 수요 지속성•중단 허용성•예측 오차의 비교 기준이다.

</details>

| **구매 모델 선택 축** | 예약•약정 할인 | 스팟 | 온디맨드 |
|:---|:---|:---|:---|
| 적용 기준 | **예약 할인**: **지속 기준 수요** | **스팟 용량**: **중단 복구 가능 작업** | **온디맨드**: **신규•급증 변동 수요** |
| 핵심 특징 | 기간•범위 사용 약정 | 저가 여유 용량•회수 가능 | 무약정 기본 요율 |
...
- **변동 수요**: 기준선을 넘는 갑작스러운 자원 수요이다.

</details>

- **지속 수요**는 예약, **복구 가능 작업**은 스팟, **변동 수요**는 온디맨드를 적용한다.
