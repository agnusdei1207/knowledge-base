...
CSP로부터 스팟 인스턴스 2분 전 회수 신호 발생
        │
   1. [회수 이벤트 수신] EventBridge가 `spot-instance-interrupt-notice` 감지 및 핸들러 전달
        │
   2. [K8s Cordon] 스팟 핸들러가 해당 노드에 `kubectl cordon`을 실행하여 신규 Pod 배포 즉시 차단
        │
   3. [K8s Drain] `kubectl drain`을 실행하여 실행 중인 파드를 타 정상 노드로 안전하게 Graceful 퇴거
        │
   4. [대체 자원 기동] ASG가 Spot Fleet 풀에서 타 인스턴스 타입을 즉시 증설하거나 온디맨드로 폴백
        │
   5. 클라이언트는 500 에러 없이 정상적으로 무중단 서비스 지속 이용
```
...
- 클라우드 인프라 비용 최적화(FinOps)를 완성하기 위해 **베이스라인 워크로드는 Compute Savings Plans로 약정 할인받고, 무상태 확장 영역은 Spot Fleet과 K8s Cordon/Drain 자동화를 결합한 스마트 인스턴스 믹스(5:4:1)를 구축**하여 최대의 비용 효율성과 가용성 완성
