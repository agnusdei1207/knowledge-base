...
date: "2026-08-05T12:40:28+09:00"
tags: [notes-software]
weight: 16
extra:
  question_no: "016"
...
- **참조 창 갱신**: 새 페이지 참조를 넣고 관찰 범위 밖의 이력을 제거하는 단계
- **워킹 셋•폴트율 산정**: 최근 고유 페이지 수와 폴트 발생 비율을 계산하는 단계
- **상주 집합 상태 판정**: 목표 워킹 셋과 현재 프레임 수•폴트율 상하한을 비교하는 단계
- **프레임 증설**: 워킹 셋 미수용이나 폴트율 상한 초과 시 프레임을 추가하는 단계
- **프레임 회수**: 폴트율이 하한보다 낮고 여유 프레임이 있으면 일부를 반환하는 단계
...
- **1. 참조 창 갱신**: 신규 참조 추가•오래된 이력 제거
- **2. 워킹 셋•폴트율 산정**: 목표 프레임과 부족 신호 계산
- **3. 상주 집합 상태 판정**: 수요와 폴트율 상하한 비교
- **4. 프레임 증설**: 미수용 작업에 추가 프레임 할당
- **5. 프레임 회수**: 과잉 작업의 여유 프레임 반환

#### 한줄 요약

- 워킹 셋•폴트율 피드백 기반 **프레임 증감**
