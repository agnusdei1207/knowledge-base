...
- 배경/필요성: 동시성(Concurrency)과 격리성(Isolation)의 상충으로 인한 **Dirty/Phantom Read 데이터 부정합 및 Lock 경합 지연 해결 불가**
...
#### 한줄 요약
- 트랜잭션 시작 → Read View 생성 → TRX_ID 대조 → Undo Log 역추적/현재 레코드 반환 순으로 진행된다.
