...
- **1. 격리 수준·트랜잭션 시작**: 관찰 범위를 정한다.
- **2. 스냅샷·잠금 요청**: 구현 보호를 적용한다.
- **3. 허용 버전 읽기·쓰기**: 데이터에 접근한다.
- **4. 데이터·충돌 정보**: 변경과 충돌을 반환한다.
- **5. 교착·직렬 가능성 판정**: 실행 순서를 검사한다.
- **6. 대기·커밋·취소 결정**: 안전한 결과를 고른다.
- **7. 결과·재시도 신호**: 제한 재시도를 안내한다.
...
- **업무 불변식·허용 이상·충돌 비용**으로 격리 수준을 정한다.
