...
- **대기 신호(Wait Signal)**: 충돌 해결까지 거래를 멈추라는 결정한다.
- **커밋 신호(Commit Signal)**: 거래 변경을 확정하라는 결정한다.
- **취소 신호(Abort Signal)**: 거래 변경을 폐기하라는 결정한다.
...
- **1. 스냅샷•잠금 정책**: 격리 수준별 가시성과 보호 범위를 구성한다.
- **2. 가시 버전•읽기•쓰기 연산**: 허용된 데이터에서 거래를 수행한다.
- **3. 대기 그래프•직렬화 관계 검사**: 충돌•교착과 실행 순서를 판정한다.
- **4. 대기•커밋•취소 신호**: 안전한 결과와 재시도 여부 확정이다.
...
- 허용 이상 감소와 **대기•재시도 증가의 단계 비교**가 핵심이다.
...
- **업무 불변식•허용 이상•충돌 비용**으로 격리 수준을 결정한다.

#### 한줄 요약

- 불변식•허용 이상•충돌 비용 기반 **격리 수준 결정**이 핵심이다.
