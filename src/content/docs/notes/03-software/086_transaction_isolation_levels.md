...
- **2PL(Two-Phase Locking, 2단계 잠금)**: 잠금을 획득하는 성장 단계와 해제만 하는 축소 단계를 분리해 직렬 가능성을 보장하는 방식이다.
- **MVCC(Multi-Version Concurrency Control, 다중 버전 동시성 제어)**: 여러 데이터 버전의 가시성을 조절해 읽기와 쓰기의 충돌을 줄이는 방식이다.
...
- **가시 버전•읽기•쓰기 연산**: 현재 트랜잭션이 볼 수 있는 데이터 버전과 그 위에서 수행하는 조회•변경 작업이다.
