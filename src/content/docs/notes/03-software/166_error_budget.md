...
- 정의/개념: SLO가 허용한 실패량을 사용하는 **오류 예산** 운영 기준이다.
- 배경/필요성: 무장애 추구로는 **신뢰성 비용•변경 정체의 균형 판단할 수 없다**.
...
- **SLO•실측 SLI** 기반 허용•사용•잔여량을 계산한다.
- **단기•장기 소진 속도** 기반 조기 위험 탐지가 핵심이다.
...
- **1. 유효•나쁜 이벤트 측정**: 사용자 경계의 실패 결과를 분류한다.
- **2. 단기•장기 오류율 계산**: 여러 구간의 실패 추세 산출한다.
- **3. 잔여량•소진 속도 산출**: 누적 상태•긴급도를 계산한다.
- **4. 변경•안정화 상태 결정**: 배포 지속•감속•복구를 선택한다.
- **5. 배포•롤백 정책 실행**: 변경•복구 후 SLI 재측정한다.
...
- **사용자 영향•소진 속도•변경 위험**으로 배포•롤백•안정화를 결정한다.
