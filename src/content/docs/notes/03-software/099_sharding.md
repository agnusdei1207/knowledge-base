...
- **1. 샤드 키 포함 요청**: 키와 연산을 보낸다.
- **2. 키 범위·노드 위치 조회**: 배치표를 확인한다.
- **3. 대상 샤드 반환**: 현재 위치를 돌려준다.
- **4. 지역 질의 전달**: 해당 노드로 요청한다.
- **5. 지역 처리 결과 반환**: 자체 데이터만 처리한다.
- **6. 최종 결과 응답**: 응용에 결과를 보낸다.
- **7. 샤드별 부하 기록**: 편중 판단값을 남긴다.
