...
- 정의/개념: 행 단위 데이터를 여러 노드에 나누는 **수평 분할 기법**이다.
- 배경/필요성: 단일 노드에서는 데이터 증가 시 **용량•처리량 확장 한계**가 발생한다.
...
- **배치 메타데이터 조회**: 키 범위의 소유 노드와 배치 버전을 확인하는 단계다.
- **대상 샤드 선택**: 현재 키 범위를 소유한 노드로 경로를 정하는 단계다.
- **지역 질의 실행**: 선택한 샤드 안에서 조회•변경을 수행하는 단계다.
- **샤드 부하 기록**: 크기•요청량•지연을 축적하는 단계다.
...
- **1. 배치 메타데이터 조회**: 키 범위와 노드 위치를 확인한다.
- **2. 대상 샤드 선택**: 현재 소유 노드로 경로 확정이다.
- **3. 지역 질의 실행**: 선택 샤드의 데이터만을 처리한다.
- **4. 샤드 부하 기록**: 편중•재샤딩 판단값 축적이다.
...
- **접근 단위(Access Unit)**: 한 업무 요청이 함께 다루는 데이터 경계다.
- **소유권 단위(Ownership Unit)**: 한 샤드가 변경 책임을 갖는 데이터 경계다.
- **가상 버킷(Virtual Bucket)**: 실제 노드에 재배치할 수 있는 논리 분배 단위다.
- **복합 키(Composite Key)**: 편중 키에 추가 분산값을 결합한 샤드 키다.
- **버전 확인(Version Check)**: 라우터의 배치 정보가 최신인지 검사하는 통제다.
- **리디렉션(Redirection)**: 요청을 새 소유 노드로 다시 보내는 통제다.
- **데이터 복사(Data Copy)**: 재샤딩 대상을 새 위치로 옮기는 절차다.
- **복사 검증(Copy Validation)**: 두 위치의 데이터 일치 여부를 확인하는 절차다.
- **원자 전환(Atomic Cutover)**: 샤드 소유권을 한 번에 바꾸는 절차다.
- **별도 서비스(Dedicated Service)**: 샤드 간 분석을 맡은 전용 처리 경로다.
...
- 범위 조회는 **범위**, 균등 점 조회는 **해시**, 지정 배치는 **디렉터리**를 선택한다.
