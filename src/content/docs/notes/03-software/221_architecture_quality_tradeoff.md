...
- 정의/개념: **비기능 요구사항(NFR)**을 품질 시나리오로 구체화해 설계 결정의 품질 간 상충 효과를 평가하는 아키텍처 의사결정 기법이다.
- 배경/필요성: 단일 품질 최적화는 **다른 품질 저하**를 유발한다.
...
- **시나리오화**: 요구 조건을 측정 가능한 응답으로 변환한다.
- **우선순위화**: 유틸리티 트리로 품질 요구를 정렬한다.
- **상충 분석**: 택틱의 민감점•부작용을 비교한다.
- **ATAM**: 시나리오별 위험•민감점•상충점을 평가한다.
...
> 요약: **품질 측정값과 비용을 함께 비교**해 택틱을 선택한다.
...
- 필수 품질 한계 위반 대안은 **폐기**, 충족 대안은 **택틱 적용**이 핵심이다.
