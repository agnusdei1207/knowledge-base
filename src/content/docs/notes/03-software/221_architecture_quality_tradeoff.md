...
- **품질 속성(Quality Attribute, QA)**: 시스템이 얼마나 잘 동작하는지를 수치로 정의한 비기능 특성임
- **비기능 요구사항(Non-Functional Requirements, NFR)**: 시스템 품질과 제약을 명시적 척도로 정의함
...
- **아키텍처 트레이드오프 분석 방법(Architecture Tradeoff Analysis Method, ATAM)**: 시나리오별 위험과 민감점을 평가하는 분석 기법임
...
- **정의/개념**: 품질 속성의 상충을 분석해 설계 대안을 선택함
- **배경/필요성**: 품질 속성 상충으로 제약 기반 선택 필요
...
- 요구 조건을 측정 가능한 시나리오로 바꾼다.
- 유틸리티 트리로 핵심 요구 우선순위를 정한다.
- 아키텍처 택틱의 상충 효과를 측정한다.
- 민감점과 결정 사유를 문서화해 추적한다.
...
| 비교축 | 성능 택틱 | 가용성 택틱 | 보안 택틱 |
|:---|:---|:---|:---|
| 핵심 특징 | 비동기·캐시로 지연을 줄임 | 복제·전환으로 중단을 줄임 | 암호화·권한으로 접근을 제한 |
| 적용 기준 | 응답 시간·처리량 우선 | 연속 운영·복구 우선 | 기밀성·무결성 우선 |
| 주요 위험 | 일관성·복잡도 악화 | 비용·동기화 증가 | 지연·사용성 저하 |
...
1. 결제 API는 지연과 이중 결제 위험을 함께 평가함
2. 배포 파이프라인은 피트니스 함수를 실행함

### 쉽게 이해하기 (학습용)
- 캐시로 빨라져도 중복 결제가 늘면 채택하지 않음
- 빌드마다 품질 한도를 넘는 구조 변경을 차단함

## Ⅶ. 결론

- 품질 시나리오와 수용 위험이 합의된 대안만 채택함
