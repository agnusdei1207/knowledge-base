...
- **정의/개념**: 운영 신호로 모델 품질 저하 판정
- **배경/필요성**: 학습·운영 분포 변화로 드리프트 탐지 필요
...
- 입력·예측·성능 기준선을 함께 봐 변화 원인 구분한다.
- 통계 거리로 분포 변화 크기를 측정해 경보 판정한다.
- 정답 지연 중에는 입력·예측 변화로 조기 경보한다.
- 임계치 초과 후 원인을 확인해 재학습·복귀 결정한다.
...
| 운영 신호 수집기 | 피처·예측·지연·오류를 슬라이스별 수집 |
| 정답 결합기 | 지연 도착한 정답과 예측을 연결 |
| 드리프트 탐지기 | 기준·운영 통계 거리와 성능 비교 |
| 대응 제어기 | 경보 검토 후 재학습·복귀·기준 갱신 |
...
| 절차 | 설명 |
|:---|:---|
| 입력·예측 변화 전달 | 슬라이스별 운영 분포·품질 신호 수집 |
| 기준선·통계 거리 비교 | 변화 크기와 임계치 초과 여부 판정 |
| 정답·예측 결합 | 지연 정답으로 실제 성능 계산 |
| 변화 유형·경보 전달 | 데이터·개념·예측 변화 원인 분류 |
| 대응 결과·기준 갱신 | 재학습·복귀 효과 확인 후 기준 갱신 |
...
| 판단 기준 | 데이터 드리프트 | 개념 드리프트 | 예측 드리프트 |
|:---|:---|:---|:---|
| 핵심 특징 | 운영 입력 분포 변화 | 입력·정답 관계 변화 | 모델 출력 분포 변화 |
| 적용 기준 | 정답 도착 전 입력 감시 | 정답 확보 후 성능 저하 확인 | 출력 비율·점수 조기 감시 |
| 주요 위험 | 품질 영향 없는 변화 경보 | 정답 지연으로 판정 늦음 | 입력·정책 변화와 원인 혼동 |
...
1. 신용 모델은 고객군별 입력·승인율·연체율 변화 감시
2. 수요 예측은 지역별 오차 증가 시 해당 구간 재학습

### 쉽게 이해하기 (학습용)
- 전체 평균이 같아도 특정 고객군의 승인·연체 관계가 달라지는지 따로 확인한다
- 오차가 커진 지역만 새 데이터로 다시 학습해 다른 지역의 정상 패턴을 보존한다

## Ⅶ. 결론

- 입력·예측 조기 경보를 정답 성능으로 확인해 재학습 결정
