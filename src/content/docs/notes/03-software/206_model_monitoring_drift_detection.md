...
- **계절성(Seasonality)**: 요일·월·계절처럼 반복 주기에 따라 데이터 분포가 달라지는 성질
- **대리지표(Proxy Metric)**: 정답이 늦을 때 품질 변화를 먼저 추정하는 간접 측정값

> **키워드:** 모델 모니터링·드리프트 감지 (Model Monitoring Drift Detection)
