...
- 정의/개념: 운영 신호로 **드리프트**와 품질 저하를 판정하는 활동이다.
- 배경/필요성: 배포 전 평가는 **운영 분포•개념 변화 반영하기 어렵다**.
...
- **입력•예측•성능 기준선**으로 변화 대상을 구분한다.
- **통계 거리•슬라이스 감시**로 조기 경보가 핵심이다.
- 지연 정답 결합 후 **성능 확인•재학습•복귀**를 결정한다.
...
1. **입력•예측 변화 전달**: **슬라이스별 운영 분포**를 수집한다.
2. **기준선•통계 거리 비교**: 변화 크기와 **임계 초과** 판정한다.
3. **정답•예측 결합**: 지연 정답으로 **실제 성능**을 계산한다.
...
- **실제 성능 저하 확인 시 재학습**, 영향 없는 분포 변화는 **검증 후 기준선 갱신**이 핵심이다.
