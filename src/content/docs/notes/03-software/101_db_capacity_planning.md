...
- **1. 성장률·피크 전달**: 관측값을 모은다.
- **2. 예상 용량 검증 요청**: 산정값을 시험한다.
- **3. 처리 한계 반환**: 병목 실측값을 보낸다.
- **4. 여유율 포함 한계일 전달**: 소진일을 계산한다.
- **5. 준비 기간 조회**: 조달·이관 기간을 확인한다.
- **6. 가용 시점 반환**: 완료 가능일을 제시한다.
- **7. 한계일 이전 증설 요청**: 선행 작업을 시작한다.
...
- **성장률·피크·여유율·준비 기간**으로 증설 시점을 정한다.
