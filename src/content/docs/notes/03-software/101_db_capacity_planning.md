...
- 정의/개념: **데이터베이스 용량 계획(Database Capacity Planning)** 은 데이터 증가율과 질의•트랜잭션 부하를 예측하여 저장•메모리•처리 용량과 증설 시점을 결정하는 관리 활동
...
선의 의미: 수요 측정에서 갈라지는 선은 성장률•피크 자료를 저장 모델과 처리 모델에 제공하는 관계, 저장 모델과 용량 기준선의 선은 저장 수요•운영 오버헤드 반영 관계, 처리 모델과 부하 시험의 선은 예상 부하 검증 관계, 부하 시험과 용량 기준선의 선은 병목 실측값으로 헤드룸•증설 기준을 보정하는 관계를 뜻한다.

| 구성요소 | 책임 |
|:---|:---|
| 수요 측정 | **성장률•피크** 와 질의 비율 수집 |
| 저장 모델 | 원본과 **운영 오버헤드** 예측 |
| 처리 모델 | **중앙처리장치(Central Processing Unit, CPU)•초당 입출력 작업 수(Input/Output Operations Per Second, IOPS)** 와 메모리 수요 예측 |
...
- **1. 성장률•피크 시계열**: 저장량과 초당 트랜잭션 수 및 동시성의 시간별 변화와 최대값을 기록한 자료이다.
- **2. 예측 부하 시나리오**: 미래 피크와 장애 조건을 재현하도록 만든 시험 입력이다.
- **3. 병목 실측값**: 부하가 늘 때 가장 먼저 한계에 도달한 자원의 실제 측정치이다.
- **5. 선행 증설 요청**: 용량 한계일에서 조달 기간을 역산해 확장을 착수하는 단계이다.
...
1. **성장률•피크 시계열**: 저장량•초당 트랜잭션 수(Transactions Per Second, TPS)•동시성의 시간별 추세 확보
...
|:---|:---|:---|:---|
| 적용 기준 | **데이터 증가량•보관 기간** 중심 | **피크 초당 트랜잭션 수(Transactions Per Second, TPS)•동시성** 중심 | **장애 시 잔여 처리량** 중심 |
| 핵심 특징 | 원본과 **운영 오버헤드** 합산 | **중앙처리장치(Central Processing Unit, CPU)•메모리•초당 입출력 작업 수(Input/Output Operations Per Second, IOPS)** 산정 | 대기 노드와 **헤드룸** 포함 |
