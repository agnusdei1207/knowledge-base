...
- 배경/필요성: 평균 사용량만으로는 **피크•장애 시점의 자원 고갈** 예측할 수 없다.
...
- **용량 한계일**: 성장률과 헤드룸으로 자원 소진 시점을 구한 결과다.
- **선행 증설 요청**: 한계일에서 조달 기간을 역산한 착수 결정한다.
...
- **1. 성장률•피크 시계열**: 저장량•TPS•동시성 추세를 확보한다.
- **2. 예측 부하 시나리오**: 미래 피크•장애 조건을 구성한다.
- **3. 병목 실측값**: 먼저 한계에 닿는 자원 식별이다.
- **4. 용량 한계일**: 성장률•헤드룸으로 소진일 산출한다.
- **5. 선행 증설 요청**: 조달 선행 시간을 역산해 착수이다.
...
> 요약: 저장•처리•가용성 수요를 분리한 **병목 자원별 예측**이 핵심이다.
...
- **용량 한계일**에서 조달 기간을 역산해 **선행 증설** 착수가 핵심이다.
