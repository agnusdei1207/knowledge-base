...
- 정의: SQL 질의 처리 시 옵티마이저가 인덱스 스캔, 조인 알고리즘, 연산 순서를 조합하여 결정한 물리적 연산 실행 트리인 **쿼리 실행 계획**.
...
- **비용 기반 옵티마이저**: 정량적 통계 정보를 활용한 최적 경로 자동 도출.
- **트리 연산 구조**: 루트에서 리프 노드 방향으로 데이터 조인, 필터링 등 연산 전달.
