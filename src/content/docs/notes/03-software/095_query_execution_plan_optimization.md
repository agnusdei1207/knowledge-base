...
- **실행 엔진•모니터링**: 선택된 계획을 수행하고 연산자별 실제 행 수와 시간 및 읽기량을 수집하는 구성요소이다.
...
- **운영 유사 통계•파라미터**: 실제 서비스와 비슷한 데이터 분포와 질의 입력값으로 계획을 검증하는 조건이다.
- **통계 갱신•상관관계 점검**: 최신 분포를 반영하고 여러 열의 값이 독립적이지 않아 생기는 선택도 오차를 확인하는 활동이다.
- **바인드 값 엿보기(Parameter Sniffing)**: 최초 실행의 매개변수 값으로 만든 계획을 다른 값에도 재사용해 성능 편차가 생기는 현상이다.
- **커서•재컴파일 정책**: 계획 재사용 범위와 입력값에 따른 새 계획 생성 시점을 통제하는 기준이다.
- **계획 이력•탐지•롤백**: 배포 전후 실행 계획과 성능을 비교해 회귀를 찾고 이전 계획으로 되돌리는 대응이다.
...
- **계획 롤백**: 배포나 통계 변화로 실행 계획이 나빠졌을 때 검증된 이전 계획으로 복구하는 조치이다.
