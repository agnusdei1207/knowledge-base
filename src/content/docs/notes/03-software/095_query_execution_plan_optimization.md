...
- 정의/개념: 통계·비용으로 **SQL 실행 경로를 선택**
- **배경/필요성**: 추정 오류와 실행 병목의 원인 교정
...
- **1. 논리 질의 전달**: SQL을 연산으로 변환한다.
- **2. 테이블·열 통계 요청**: 분포 자료를 요구한다.
- **3. 분포·선택도 반환**: 예상 행 수를 제공한다.
- **4. 최저 예상 비용 계획 전달**: 실행 경로를 고른다.
- **5. 실제 행 수·자원 기록**: 연산자별 값을 남긴다.
- **6. 질의 결과 반환**: 처리 결과를 전달한다.
- **7. 연산자별 실측값 반환**: 추정 오차를 확인한다.
...
- **추정 오차·실측 비용·회귀 위험**으로 실행 계획을 교정한다.
