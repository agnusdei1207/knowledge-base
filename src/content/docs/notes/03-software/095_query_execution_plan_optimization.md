...
- 정의/개념: **쿼리 최적화**는 통계와 비용으로 **실행 계획**을 선택한다.
- 배경/필요성: 데이터 분포를 잘못 추정하면 비효율적인 계획을 선택한다.
...
- **비용 기반 선택**: 행 수와 자원 사용량을 추정한다.
- **다차원 탐색**: 접근•조인•정렬•집계를 결정한다.
- **실측 검증**: 예상•실제 행과 자원량을 비교한다.
...
- **실행 엔진(Execution Engine)**: 선택된 계획의 연산자를 수행하는 구성요소이다.
...
선의 의미: 옵티마이저는 분석 결과와 통계로 계획을 선택하고, 실행 엔진•모니터링은 선택된 계획의 실측 정보를 수집한다.

| 구성요소 | 책임 |
|:---|:---|
| 질의 분석기 | **질의 분석기**가 SQL을 논리 연산으로 변환 |
| 통계 정보 | **통계 정보**가 행 수•분포•고유값 수 제공 |
| 옵티마이저 | **옵티마이저**가 후보 계획 비용 비교 |
| 실행 계획 | **실행 계획**에 접근•조인•연산자 명시 |
| 실행 엔진•모니터링 | **실행 엔진**과 **모니터링**이 실제 행•시간•읽기량 수집 |
...
- **SQL 분석•논리 질의 구성**: 질의를 관계 연산 트리로 바꾸는 단계이다.
- **카디널리티 추정**: 통계와 선택도로 중간 행 수를 예측하는 단계이다.
- **최저 비용 계획 선택**: 접근•조인 후보의 상대 비용을 비교하는 단계이다.
- **실행 실측값 기록**: 연산자별 실제 행•시간•자원을 수집하는 단계이다.
...
- **1. SQL 분석•논리 질의 구성**: **SQL 분석•논리 질의 구성**으로 SQL을 관계 연산 트리로 변환한다.
- **2. 카디널리티 추정**: **카디널리티 추정**으로 통계•선택도 기반 중간 행 수를 예측한다.
- **3. 최저 비용 계획 선택**: **최저 비용 계획 선택**으로 접근•조인 순서의 비용을 비교한다.
- **4. 실행 실측값 기록**: **실행 실측값 기록**으로 연산자별 행 수•시간•자원을 수집한다.
...
| 핵심 특징 | **인덱스 탐색**으로 후보 페이지 축소 | **전체 탐색**으로 모든 페이지 연속 읽기 | **키셋 페이지**로 마지막 키 이후 조회 |
| 한계 | 임의 읽기•재조회 증가 | 불필요 행 읽기 증가 | 임의 페이지 이동 제한 |

> 비용은 절대 실행시간이 아니라 **데이터베이스 관리 시스템**의 비용 모델이 후보 계획을 비교하기 위한 상대 추정값이다.
...
- **운영 유사 통계(Production-like Statistics)**: 실제 서비스와 비슷한 데이터 분포 정보이다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 운영과 검증 환경의 통계 차이 | **운영 유사 통계**•**질의 매개변수** 사용 | 환경별 계획 차이 감소 |
| 열 간 상관관계를 무시한 행 수 오차 | **통계 갱신**•**상관관계 점검** | 예상•실제 행 차이 축소 |
| **바인드 값 엿보기**에 따른 계획 편향 | **커서 정책**•**재컴파일 정책** 검토 | 값별 계획 불안정 완화 |
| 배포 후 계획 회귀 발생 | **계획 이력**•**회귀 탐지**•**계획 롤백** 준비 | 배포 후 지연 복구 |
...
- **쿼리 최적화 대응 기준**: 추정 오차와 계획 회귀 중 문제 원인을 구분해 보정 또는 복구 방법을 고르는 기준이다.

</details>

- **쿼리 최적화 대응 기준**에 따라 추정 오차가 크면 **통계 보정**, 계획 회귀면 **계획 롤백**을 선택한다.

#### 한줄 요약

- **쿼리 최적화 대응 기준**은 예상과 실제가 어긋난 지점에서 통계와 실행 경로를 바로잡는다.
