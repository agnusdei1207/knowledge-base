...
- 정의/개념: 통계와 비용으로 **SQL 실행 계획을 선택하는 최적화**이다.
- 배경/필요성: 분포를 잘못 추정하면 **비효율 계획** 선택이 필요하다.
...
- **SQL 분석•논리 질의 구성**: 질의를 관계 연산 트리로 바꾸는 단계다.
- **카디널리티 추정**: 통계와 선택도로 중간 행 수를 예측하는 단계다.
- **최저 비용 계획 선택**: 접근•조인 후보의 상대 비용을 비교하는 단계다.
- **실행 실측값 기록**: 연산자별 실제 행•시간•자원을 수집하는 단계다.
...
- **1. SQL 분석•논리 질의 구성**: SQL을 관계 연산 트리로 변환한다.
- **2. 카디널리티 추정**: 통계•선택도로 중간 행 수를 예측한다.
- **3. 최저 비용 계획 선택**: 접근•조인 순서의 비용을 비교한다.
- **4. 실행 실측값 기록**: 연산자별 행 수•시간•자원 수집이다.
...
- 추정 오차가 크면 **통계 보정**, 계획 회귀면 **롤백**을 선택한다.
