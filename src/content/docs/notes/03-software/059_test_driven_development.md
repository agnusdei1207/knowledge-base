...
- **Red-Green-Refactor 마이크로 사이클**: 실패하는 테스트 작성(Red), 통과를 위한 최소 코드 구현(Green), 동작을 유지하며 구조 개선(Refactor)의 초단기 순환 주기.
- **사후 테스트 및 회귀 결함 공포(Test Fear & Regression)**: 코드가 이미 완성된 후 테스트를 짜려면 결합도가 높아 불가능하고 코드 수정 시 무엇이 깨질지 알 수 없는 심리적/기술적 한계.
...
**동작 원리** 1. **Red**: 구현 코드 없이 테스트를 먼저 작성하여 의도적 실패를 확인
