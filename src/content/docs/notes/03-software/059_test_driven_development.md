...
- 배경/필요성: 사후 시험은 **검증 누락•강한 결합•변경 불안** 유발
...
| 구성요소 | 책임 |
|:---|:---|
| 테스트 목록 | 구현할 행위와 경계 사례를 작은 단위로 관리 |
| 테스트 코드 | 요구 행위와 기대 결과를 실행 가능한 명세로 표현 |
| 대상 코드 | 현재 시험을 통과할 최소 동작 구현 |
| 테스트 실행기 | 시험 실행과 실패 원인 피드백 제공 |
| 회귀 시험 모음 | 리팩터링 전후 외부 행위 보존 검증 |
...
1. 실패 시험 작성: 목록에서 행위를 골라 실패 원인 확인.
2. 최소 구현 통과: 현재 시험을 만족하는 최소 코드 작성.
3. 내부 구조 개선: 외부 행위를 보존하며 중복•명칭 개선.
4. 회귀 시험 누적: 통과 시험을 모음에 남기고 다음 항목 반복.
...
|:---|:---|:---|
| 테스트 작성 시점 | 소스코드 전체 개발 완결 후 | **소스코드 작성 전 ** |
| 소스 결합도 | 구현 후 의존성 분리가 어려울 수 있음 | **시험 가능한 인터페이스와 의존성 분리 유도** |
| 리팩터링 안전성 | 사후 회귀 시험이 부족하면 변경 위험 증가 | **회귀 시험으로 외부 행위 보존 확인** |
...
- 결정적 핵심 규칙은 **TDD**, 불확실한 해법은 **탐색 구현 후 시험** 선택
