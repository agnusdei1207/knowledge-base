...
- 정의/개념: 실패 **단위 테스트**를 먼저 작성하고 최소 구현•리팩터링을 반복하는 개발 방식이다.
- 배경/필요성: 구현 후 시험의 **요구 누락•결함 발견 지연** 해소가 필요하다.

#### 한줄 요약

- 실패 시험•최소 구현•리팩터링의 **반복 개발 방식**이 핵심이다.
...
- 테스트 우선의 **실행 가능한 요구 명세**가 핵심이다.
- 작은 구현 단위의 **빠른 결함 피드백**이 핵심이다.
- 회귀 보호 아래 **지속적 리팩터링**이 핵심이다.

#### 한줄 요약

- 실행 명세•빠른 피드백•**회귀 보호 기반 개선**이 핵심이다.
...
- **테스트 코드(Test Code)**: 입력과 기대 동작을 실행 가능한 명세로 표현한 코드다.
- **대상 코드(Code Under Test)**: 테스트가 요구한 동작을 구현하는 코드다.
...
- **최소 구현**: 현재 실패 테스트를 통과하는 데 필요한 만큼만 코드를 작성하는 동작이다.
- **그린 확인(Green Check)**: 신규 테스트와 기존 회귀 테스트가 모두 통과하는지 확인하는 동작이다.
- **리팩터링**: 모든 테스트를 통과한 상태에서 외부 동작을 유지하며 내부 구조를 개선하는 동작이다.
...
3. **최소 구현**: 현재 테스트를 통과할 코드를 작성한다.
4. **그린 확인**: 신규•기존 회귀 테스트 통과 판정한다.
5. **리팩터링**: 외부 동작을 유지하며 구조를 개선한다.

> 요약: 실패 확인•최소 구현•구조 개선을 작은 동작마다 반복이 핵심이다.

#### 한줄 요약

- 레드•그린•리팩터의 **작은 동작 반복 순환**이 핵심이다.
...
- 결정적 규칙은 TDD, 탐색•연동은 **구현 후 시험**이 핵심이다.
...
- **실패 메시지(Failure Message)**: 테스트 실패 원인을 보여 주는 진단 정보다.
...
- **의존성 분리(Dependency Isolation)**: 외부 협력 대상을 교체 가능하게 나누는 설계다.
- **관찰 가능 출력(Observable Output)**: 결과와 상태 변화를 시험에 드러내는 설계다.
...
- 공개 동작 검증과 **리팩터링 내성•통합 시험**이 핵심이다.
...
- 결정적•격리 가능한 규칙은 **TDD**, 탐색•외부 연동은 **구현 후 통합 시험**을 선택한다.

#### 한줄 요약

- 격리 규칙은 TDD, 외부 연동은 **통합 시험**이 핵심이다.
