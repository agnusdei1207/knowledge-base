...
- ① 개발자가 다음 작은 동작의 기대 결과를 실패 테스트로 작성한다.
- ② 테스트 실행기에 신규 테스트를 실행하도록 요청한다.
- ③ 구현 부재라는 올바른 이유로 실패하는지 확인한다.
- ④ 현재 테스트를 통과하는 데 필요한 최소 코드를 작성한다.
- ⑤ 신규 테스트와 기존 회귀 테스트를 모두 실행한다.
- ⑥ 전체 테스트 통과로 요구 동작 구현을 확인한다.
- ⑦ 외부 동작을 유지하며 중복·명칭·복잡도를 개선한다.
- ⑧ 회귀 테스트로 구조 변경이 기존 동작을 깨지 않았는지 확인한다.
...
| 고려사항 | 문제점 | 대책 |
|:---|:---|:---|
| 실패 이유 미확인 | 오타·환경 오류를 요구 부재로 잘못 판단 | Red 단계에서 의도한 이유와 메시지로 실패하는지 확인 |
| 구현 결합 테스트 | 리팩터링마다 취약한 테스트가 대량 실패 | 공개 동작과 업무 결과를 검증하고 내부 호출 검증 최소화 |
| 과도한 목 객체 | 실제 객체 협력·계약 결함 누락 | 경계 의존성만 대체하고 통합·계약 테스트 병행 |
| 큰 반복 단위 | 실패 원인과 설계 피드백이 늦어짐 | 한 번에 하나의 작은 관찰 가능 동작만 추가 |
| 리팩터 생략 | 통과만 누적되어 중복·복잡도 증가 | Green 직후 회귀 보호 아래 구조 개선 수행 |
...
- TDD는 실패 테스트로 요구 동작을 고정하고 회귀 보호 아래 설계를 점진적으로 개선한다.
- 결정적 단위 로직에 적용하고 실제 연동은 통합 테스트로 보완한다.
