...
- 정의/개념: RTO•RPO에 맞춘 **재해 복구 전략**이다.
- 배경/필요성: 단일 복구 방식으로는 업무별 **허용 중단•손실 차이** 충족할 수 없다.
...
- **BIA•MTD**로 RTO•RPO 복구 목표를 결정한다.
- **복제•예비 환경**으로 복구 등급을 구성한다.
- **Failover•Failback** 기반 전환•복귀를 통제한다.
...
> 요약: **복구 목표**를 데이터 보호•예비 자원•전환 통제로 연결한다.
...
5. **업무•데이터 대사•복귀**: 거래•무결성 검증 후를 복귀한다.
...
- 짧은 RTO•RPO는 **액티브-액티브**, 중간은 웜 스탠바이, 긴 목표는 백업 복구를 선택한다.
