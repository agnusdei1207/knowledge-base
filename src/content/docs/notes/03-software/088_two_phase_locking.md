...
- 배경/필요성: 락을 필요할 때 잡고 끝나는 대로 놓으면 이미 놓은 자원을 다른 트랜잭션이 고친 뒤 다시 잡게 되어 직렬 실행과 다른 결과가 나오고 그 검증 비용을 응용이 감당할 수 없으므로, 해제를 시작한 뒤에는 새 락을 잡지 못하게 하는 규약을 잠금 관리자에 두어 직렬 가능성을 프로토콜 차원에서 보장할 필요
