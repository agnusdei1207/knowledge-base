...
- 배경/필요성: 동시 트랜잭션의 **충돌 가능 실행 순서** 통제
...
- **1. S·X 잠금 요청**: 읽기·쓰기 모드를 요구
- **2. 호환성·보유자 조회**: 대기열을 확인
- **3. 잠금 부여·대기**: 호환 여부로 처리
- **4. 보호된 읽기·쓰기**: 잠금 범위에서 연산
- **5. 연산 결과**: 처리 값을 반환
- **6. 종료·잠금 해제**: 보유 잠금의 규칙별 일괄 해제
- **7. 대기·피해자 결정**: 교착 순환을 취소로 해소
...
- **잠금 순서·범위·보유 시간**으로 2PL의 교착 비용을 감소
