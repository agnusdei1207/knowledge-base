...
- **정의/개념**: 요청 후 반환하고 완료 이벤트를 배분
- **배경/필요성**: I/O 대기가 스레드를 점유해 이벤트 분리 필요
...
- 적은 루프 스레드로 다수 I/O 대기 연결 처리한다.
- 차단·계산 작업을 작업 풀로 분리해 루프 점유 제한한다.
- 역압·큐 상한 부재 시 입력 이벤트와 지연 누적된다.
...
| 판단 기준 | 비동기 I/O·이벤트 루프 | 동기 I/O |
|:---|:---|:---|
| 핵심 특징 | 이벤트로 재개·스레드 비점유 | 호출 흐름 대기·스레드 점유 |
| 적용 기준 | 다수 I/O 대기 연결 처리 | 단순한 순차 제어가 필요할 때 |
| 주요 위험 | 루프 점유·큐 누적 | 스레드 고갈·문맥 전환 |
...
1. 이벤트 서버는 암호 해시를 작업 풀로 분리
2. 네트워크 입력 서버는 큐 상한에서 읽기 일시 중지
