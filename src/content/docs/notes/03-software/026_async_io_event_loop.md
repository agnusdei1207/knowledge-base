...
- ① 응용이 입출력 요청과 완료 처리기 등록
- ② 등록 인터페이스가 운영체제에 입출력 요청 제출
- ③ 입출력 완료를 기다리지 않고 응용에 제어권 반환
- ④ 운영체제가 준비·완료 이벤트를 감시기와 이벤트 루프에 통지
- ⑤ 이벤트 루프가 해당 완료 처리기를 선택해 실행
- ⑥ 완료 처리기가 필요한 후속 입출력 등록
...
- 이벤트 서버는 오래 걸리는 암호 해시를 유한 작업 풀로 보내 이벤트 루프가 다른 연결을 계속 처리하게 한다.
...
- 적은 스레드로 다수 연결을 처리하려면 **I/O 대기 비율·콜백 처리시간·역압 가능성**을 검토하고 짧은 비차단 작업에는 이벤트 루프를 적용한다.
