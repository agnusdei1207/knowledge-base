...
- 정의/개념: 제어권을 먼저 반환하고 처리기를 재개하는 **AIO•완료 이벤트** 모델이다.
- 배경/필요성: 동기 I/O는 대기 중 스레드를 점유해 연결 증가 시 **스레드 고갈**이 발생한다.

#### 한줄 요약

- 제어권 반환과 완료 이벤트 기반 **비동기 재개**가 핵심이다.
...
- **이벤트 루프**로 다수 I/O 연결의 완료 이벤트 배분이 핵심이다.
- **차단•CPU 작업 분리**로 루프 응답 지연을 방지한다.
- **역압•큐 상한**으로 입력량•처리량 불균형을 통제한다.
...
- **1. I/O 요청 등록**: 장치 작업과 처리기 제출한다.
- **2. 제어권 즉시 반환**: 완료 대기 없이 다른 작업을 실행한다.
- **3. 완료 이벤트 수집**: 운영체제 사건을 큐에 등록한다.
- **4. 완료 처리기 디스패치**: 사건별 처리기 선택•실행한다.
- **5. 결과 처리•후속 등록**: 결과 반영 후 다음 I/O 제출한다.

#### 한줄 요약

- 요청 등록•완료 이벤트•**후속 I/O**의 반복 흐름이 핵심이다.
...
> 요약: **비동기 I/O**는 이벤트로 재개하고 **동기 I/O**는 호출 흐름 대기가 핵심이다.

#### 한줄 요약

- 비동기는 이벤트 재개, 동기는 **호출 흐름 대기**가 핵심이다.
...
- 처리기 시간•큐 상한•**취소 전파** 기반 루프 보호가 핵심이다.
...
- **대기 연결**: 입출력 결과를 기다리지만 CPU 작업은 수행하지 않는 네트워크•장치 연결한다.

</details>

- 대기 연결이 많고 처리기가 짧으면 **이벤트 루프**, 장시간 차단•계산은 **작업 풀**을 선택한다.

#### 한줄 요약

- 대기 연결이 많고 처리기가 짧으면 **이벤트 루프 선택**이 핵심이다.
