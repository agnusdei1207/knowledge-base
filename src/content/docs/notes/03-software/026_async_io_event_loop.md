...
date: "2026-07-30T23:21:18+09:00"
tags:
  - "notes-software"
weight: 26
extra:
...
- **문맥 전환(Context Switch)**: 실행 대상을 바꾸기 위해 레지스터와 스택 상태를 저장·복원하는 작업
- **자원 누수(Resource Leak)**: 작업이 끝난 뒤에도 메모리·연결·파일 같은 자원을 반환하지 못하는 현상

> **키워드:** 비동기 I/O·이벤트 루프 (Async I/O Event Loop)
...
1. **I/O 요청**: 등록 인터페이스가 운영체제에 장치 작업 제출
2. **완료 이벤트**: 운영체제가 준비·완료 상태를 감시기에 전달
3. **준비·완료 사건**: 이벤트 루프가 연결된 완료 처리기 선택
