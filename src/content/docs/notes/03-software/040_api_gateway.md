...
**동작 원리** 1. **TLS 종료**: 게이트웨이가 HTTPS를 복호화하고 내부는 HTTP로 전달하여 백엔드 오버헤드 감소
...
- 분산 MSA 환경에서는 **API 게이트웨이** 기반 공통 횡단 기능을 집중 처리하고, 단일 모놀리식 환경은 직접 호출 방식을 유지
