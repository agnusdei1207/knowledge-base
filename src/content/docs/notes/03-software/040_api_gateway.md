...
- **외부 계약•내부 라우팅**: 외부 계약은 클라이언트에 공개한 API 규칙이고, 내부 라우팅은 그 요청을 실제 서비스 주소로 전달하는 결정이다.
...
- **내부•외부 계약**: 내부 계약은 게이트웨이와 서비스 사이의 요청 규칙이고, 외부 계약은 클라이언트에 제공하는 공개 규칙이다.
...
- **업무 인가**: 백엔드 서비스가 실제 업무 자원에 대한 동작 권한을 판정하는 절차다.
- **토큰 서명•유효기간•발급자**: 토큰이 변조되지 않았는지, 아직 사용할 수 있는지, 신뢰한 신원 제공자가 만들었는지를 확인하는 검증 항목이다.
...
- **병목•공통 장애 지점(Bottleneck•Common Failure Point)**: 처리 용량을 제한하거나 하나의 실패로 모든 외부 요청에 영향을 주는 지점이다.
...
- **선언형 정책•중앙 버전 관리**: 진입 규칙을 코드와 분리된 명세로 정의하고 승인된 정책 버전을 한곳에서 배포•추적하는 방식이다.
