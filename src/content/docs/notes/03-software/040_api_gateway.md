...
- 배경/필요성: 외부 클라이언트가 다수의 분산 마이크로서비스 엔드포인트를 직접 호출할 때 발생하는 내부 네트워크 토폴로지 노출, 서비스별 인증/인가·CORS·로깅 등 횡단 관심사(Cross-Cutting Concerns) 중복 구현 및 프로토콜 변환 비효율을 극복하고, 단일 진입점(Single Entry Point)에서 L7 동적 라우팅, SSL 종단, 토큰 인증, 트래픽 유량 제어(Rate Limiting)를 일원화하는 **API 게이트웨이 계층을 배치하여 보안성과 백엔드 결합도 격리를 달성**할 필요
...
#### 한줄 요약
- 인증과 Rate Limiting이 라우팅보다 앞에 놓여야 비정상 요청이 백엔드에 닿기 전에 걸러지므로, 필터 순서가 곧 게이트웨이가 막아 내는 부하의 크기가 된다.
...
- 마이크로서비스 아키텍처(MSA) 및 클라우드 네이티브 환경의 **필수 엣지(Edge) 인프라 관문**으로 확립되었으며, 실무 구축 시에는 **단일 장애점(SPOF) 방지를 위한 L4 ALB 다중화/Multi-AZ 오토스케일링, Redis 기반 분산 Rate Limiting, 게이트웨이 비대화(Fat Gateway) 방지를 위한 횡단 관심사 국한 운영, 클라이언트 플랫폼별(Web/Mobile) 최적화를 지원하는 BFF(Backend For Frontend) 패턴**을 결합하여 운영
