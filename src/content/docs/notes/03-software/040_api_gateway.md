...
- **정의/개념**: 외부 API의 공통 진입과 중계를 제공
- **배경/필요성**: 정책 중복이 호출자를 결합해 공통 진입점 필요
...
- 내부 서비스 주소를 숨기고 안정된 외부 계약 제공한다.
- 인증·요청률 제한·관측 등 공통 진입 정책 중앙화한다.
- 프로토콜 변환·응답 조합으로 클라이언트 호출 단순화한다.
- 추가 중계 홉이 병목·공통 장애 지점 형성한다.
...
| 판단 기준 | API 게이트웨이 | 직접 호출 |
|:---|:---|:---|
| 핵심 특징 | 단일 외부 계약·공통 정책·응답 조합 | 서비스별 계약·주소 직접 호출 |
| 적용 기준 | 다수 서비스·반복 정책·응답 조합 | 소수 서비스·단순 클라이언트 |
| 주요 위험 | 추가 홉·병목·공통 장애 | 정책 중복·클라이언트 결합 |
...
1. 제휴 API는 키 인증·요청률 제한을 게이트웨이에 적용
2. 모바일 BFF는 여러 서비스 응답을 화면 계약으로 조합

### 쉽게 이해하기 (학습용)

- 제휴 요청은 입구 제한을 거치고 자원별 권한은 서비스가 다시 확인한다
- 모바일 앱은 여러 창구를 직접 돌지 않고 화면에 필요한 답을 한 번에 받는다

## Ⅶ. 결론

- 반복 정책·응답 조합 이익이 추가 홉 비용보다 클 때 도입
