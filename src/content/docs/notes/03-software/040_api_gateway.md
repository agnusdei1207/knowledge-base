...
- 정의/개념: 외부 API를 중계하고 정책을 집행하는 **역방향 프록시 계층**이다.
- 배경/필요성: 서비스 직접 노출로 **정책 중복•내부 주소 결합**이 증가한다.

#### 한줄 요약

- 외부 계약과 내부 서비스를 분리하는 **공통 API 진입점**이 핵심이다.
...
- **외부 계약•내부 라우팅 분리**로 서비스 주소 은닉이 핵심이다.
- **인증•요청률 제한•관측** 정책의 공통 집행이 핵심이다.
- **변환•응답 조합**으로 호출 단순화, 업무 인가는 서비스를 유지한다.

#### 한줄 요약

- 공통 진입 정책과 서비스 **업무 인가 책임**을 분리한다.
...
- **1. 외부 계약 수신**: 공개 경로•메서드•요청 형식을 확인한다.
- **2. 진입 정책 검증**: 토큰•요청률•출처 정책 검사이다.
- **3. L7 라우팅•요청 변환**: 목적 서비스•내부 계약을 선택한다.
- **4. 업무 인가•서비스 처리**: 자원별 권한과 업무 규칙을 판정한다.
- **5. 응답 변환•조합•관측**: 외부 계약 변환과 추적을 기록한다.

#### 한줄 요약

- 진입 정책과 업무 인가를 분리한 **요청 중계 흐름**이 핵심이다.
...
- 반복 정책이 많으면 게이트웨이, 단순 경로는 **직접 호출**이 핵심이다.
...
- 반복 정책 이익이 추가 홉 비용보다 크면 **API 게이트웨이**, 아니면 **직접 호출**을 선택한다.

#### 한줄 요약

- 반복 정책 이익이 추가 홉보다 클 때 **게이트웨이 선택**이 핵심이다.
