...
- **API 게이트웨이(API Gateway)**: MSA 환경에서 외부 클라이언트의 단일 진입점으로 인증·인가·라우팅·트래픽 제어·SSL 종료 등 공통 횡단 기능을 집중 처리하는 엣지 서버.
- **리버스 프록시(Reverse Proxy)**: 외부 요청을 수신하여 내부 서비스에 전달함으로써 백엔드 서버의 IP와 구조를 외부에 노출하지 않고 보호하는 프록시 방식.
- **BFF(Backend For Frontend)**: 클라이언트 종류(모바일·웹)별로 최적화된 API 응답을 제공하기 위해 프론트엔드별 전용 게이트웨이를 구성하는 패턴.

</details>

- 정의/개념: MSA의 단일 외부 진입점으로 인증·라우팅·트래픽 제어 등 공통 횡단 정책을 중앙화하는 **API 게이트웨이**
- 배경/필요성: 서비스별로 인증·트래픽 제어 로직을 중복 구현하면 개발 비용과 정책 불일치가 증가하므로, **공통 횡단 기능의 API 게이트웨이 집중 처리 체계 필수**
