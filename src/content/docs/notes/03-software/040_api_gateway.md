...
- ① 외부 요청을 수신하고 키·토큰·상관 식별자 확인
- ② 신원 제공자에 토큰 서명·유효기간·발급자 검증 요청
- ③ 검증된 신원과 공통 권한 정보 반환
- ④ 정책 실패는 즉시 거절하고 통과 요청은 목적 서비스로 변환·라우팅
- ⑤ 백엔드 서비스가 자원·행위별 업무 인가 후 처리 결과 반환
- ⑥ 외부 계약에 맞게 응답을 변환·조합하고 요청 지표 기록
- ⑦ 상태·헤더·본문을 외부 클라이언트에 반환
...
- 제휴 API는 게이트웨이에서 키 인증과 요청률 제한을 적용하고 자원별 최종 인가는 백엔드 서비스가 수행한다.
...
- 외부 API의 공통 정책과 진입점을 일관되게 관리하려면 **반복 정책·응답 조합 이익·추가 홉과 장애 집중 비용**을 검토하고 편익이 큰 경계에 API 게이트웨이를 둔다.
