...
date: "2026-08-05T16:13:00+09:00"
tags:
  - "notes-software"
weight: 107
extra:
...
- 정의/개념: 키 기반 자료구조를 메모리에서 처리하는 **Redis 데이터 저장소**
...
- **1. 키•자료구조 명령**: 해시 슬롯의 주 노드에 원자 연산 전달
- **2. AOF 명령 기록**: 재실행 가능한 변경 명령 추가
- **3. 파일 동기화**: 정책에 따른 디스크 기록 수행
- **4. 복제 스트림 전파**: 변경 순서를 복제본에 전달
- **5. 복제 오프셋 확인**: 복제본의 반영 위치 판정
...
- 재생성 가능 값만 **캐시**, 낮은 RPO는 **AOF•복제•백업** 적용
