...
클라이언트 쓰기 명령 접수 (`ZADD leaderboard 100 "user1"`)
        │
   1. [메모리 검사] 현재 RAM 사용량이 설정된 `maxmemory` 임계치를 초과했는가?
   ┌────┴───────────────────────────┐
  예 (메모리 포화 상태)             아니오 (메모리 여유 있음)
   │                                 │
2. [Eviction 실행]                   [메모리 즉시 할당]
   설정된 정책(`volatile-lru`)으로   RAM 상의 SkipList에 노드 추가
   오래된 키를 즉시 축출(Evict)     │
        │                            │
   3. 인메모리 ZSet 자료구조 갱신 완료 ($O(\log N)$)
        │
   4. AOF 버퍼에 명령어 추가 기록 및 클라이언트에 0.3ms 성공 응답 반환
```
...
- 웹 서비스의 실시간성과 고성능 처리를 위해 **Redis의 다양한 인메모리 자료구조(ZSet/Hash)와 AOF 영속성을 적극 활용**하고, **`KEYS *` 금지 및 Redis Cluster 샤딩**을 통해 무결점 초저지연 캐싱 인프라 구축
