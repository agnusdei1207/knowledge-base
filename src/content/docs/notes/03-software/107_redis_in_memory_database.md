...
- **인메모리 DB(In-Memory Database)**: ‘인메모리 디비’로 읽으며, 주 데이터를 메모리에 저장해 낮은 지연을 제공한다.
- **TTL(Time To Live)**: ‘티티엘’로 읽고 영문 머리글자를 딴 약어이며, 키가 자동 삭제될 때까지의 시간을 지정한다.
- **RDB(Redis Database)**: ‘알디비’로 읽고 영문 머리글자를 딴 약어이며, Redis 상태를 시점별 스냅샷 파일로 저장한다.
- **AOF(Append Only File)**: ‘에이오에프’로 읽고 영문 머리글자를 딴 약어이며, 변경 명령을 순서대로 기록해 복구한다.
- **센티널 (Sentinel)**: 장애를 감지해 주 복제본을 승격한다.
- **Redis 클러스터 (Redis Cluster)**: 해시 슬롯으로 키를 분산한다.
- **제거 정책 (Eviction Policy)**: 메모리 부족 시 삭제할 키를 정한다.
...
- **1. 키 명령 제출**: 자료구조 연산과 키 전달
- **2. 슬롯 담당 노드 선택**: 해시 슬롯으로 Primary 결정
- **3. AOF 변경 기록**: 재실행 가능한 명령 로그 추가
- **4. 복제 스트림 전파**: 변경 순서를 Replica에 전달
- **5. 내구·복제 상태 확인**: fsync 정책과 복제 위치로 응답 결정
...
> **적용 사례**: 인기 응답 캐시는 TTL에 무작위 편차를 주고 한 프로세스만 원본을 갱신하게 해 동시 만료 폭주를 감소
...
- **재생성 여부·RPO·메모리 한도**로 Redis 저장 정책을 결정
