...
- 정의/개념: 키 기반 자료구조를 메모리에서 처리하는 **Redis 데이터 저장소**이다.
- 배경/필요성: 디스크 중심 저장은 반복 조회•카운터 갱신의 **응답 지연**이 증가한다.
...
- **수명 관리**: **TTL•제거 정책**으로 메모리를 통제한다.
- **복구•분산**: **RDB•AOF•복제•클러스터**를 제공한다.
...
- **키•자료구조 명령**: 해시 슬롯의 담당 노드에 원자 연산을 보내는 단계다.
- **AOF 명령 기록**: 재실행 가능한 변경 명령을 로그에 추가하는 단계다.
- **파일 동기화(File Synchronization, fsync)**: 정책에 따라 로그를 디스크에 기록하는 단계다.
- **복제 스트림 전파**: 변경 명령과 순서를 복제본에 보내는 단계다.
- **복제 오프셋 확인**: 복제본의 마지막 반영 위치를 확인하는 단계다.
...
- **1. 키•자료구조 명령**: 해시 슬롯의 주 노드에 원자 연산을 전달한다.
- **2. AOF 명령 기록**: 재실행 가능한 변경 명령 추가이다.
- **3. 파일 동기화**: 정책에 따른 디스크 기록을 수행한다.
- **4. 복제 스트림 전파**: 변경 순서를 복제본에 전달한다.
- **5. 복제 오프셋 확인**: 복제본의 반영 위치를 판정한다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 원본을 Redis에만 두면 장애 시 RPO 초과 | 재생성 여부•**RPO별 저장 역할** 분리 | **원본 데이터 손실** 방지 |
| 메모리 한도 도달 시 OOM이나 중요 키 제거 | **메모리 한도•제거 정책** 설정 | **OOM•중요 키 제거** 방지 |
...
- 재생성 가능 값만 **캐시**, 낮은 RPO는 **AOF•복제•백업**을 적용한다.
