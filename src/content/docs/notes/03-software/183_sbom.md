...
- 글로벌 소프트웨어 조달 및 엔터프라이즈 보안 컴플라이언스(EO 14028, EU CRA, 국내 공공 가이드라인)의 필수적인 공급망 투명성 및 무결성 보증 표준으로 자리잡았으며, 실무 구축 시에는 CI/CD 파이프라인 내 Syft를 통한 CycloneDX/SPDX 자동 생성, Sigstore Cosign을 활용한 빌드 산출물과 SBOM의 암호학적 전자서명, 패키지 식별 표준인 purl 전사 적용, 미호출 취약점의 불필요한 패치를 방어하는 VEX(Vulnerability Exploitability eXchange) 연계를 결합하여 개발 생산성을 저해하지 않는 완벽한 DevSecOps 거버넌스를 완성
