...
- 배경/필요성: 현대 소프트웨어의 80% 이상이 오픈소스 컴포넌트로 구성되는 환경에서, Log4j(Log4Shell), SolarWinds 사태와 같이 깊은 전이 의존성(Transitive Dependency)에 내재된 보안 취약점과 악성코드 주입 공격이 발생할 경우 어떤 시스템이 해당 라이브러리를 사용하는지 파악하지 못해 대규모 보안 피해가 발생하는 문제를 해결하고, 미국 행정명령(EO 14028) 및 글로벌 공급망 보안 규제에 대응하기 위해, 소프트웨어에 포함된 모든 오픈소스 모듈, 버전, 라이선스, 패키지 해시를 기계 판독 가능한 표준 규격(SPDX, CycloneDX)으로 명세하는 SBOM을 도입하여 **소프트웨어 공급망(Software Supply Chain)의 전 주기 투명성과 취약점 추적성을 확보**할 필요
...
- 글로벌 소프트웨어 조달 및 엔터프라이즈 보안 컴플라이언스(EO 14028, EU CRA, 국내 공공 가이드라인)의 **필수적인 공급망 투명성 및 무결성 보증 표준**으로 자리잡았으며, 실무 구축 시에는 **CI/CD 파이프라인 내 Syft를 통한 CycloneDX/SPDX 자동 생성, Sigstore Cosign을 활용한 빌드 산출물과 SBOM의 암호학적 전자서명, 패키지 식별 표준인 purl 전사 적용, 미호출 취약점의 불필요한 패치를 방어하는 VEX(Vulnerability Exploitability eXchange) 연계**를 결합하여 개발 생산성을 저해하지 않는 완벽한 DevSecOps 거버넌스를 완성
