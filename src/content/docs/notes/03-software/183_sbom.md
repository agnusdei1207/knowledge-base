...
- **SBOM (Software Bill of Materials, 소프트웨어 자재 명세서)**: 소프트웨어를 구성하는 모든 오픈소스, 서드파티 라이브러리, 컴포넌트의 이름, 버전, 라이선스, 의존성 관계를 기계 판독 가능한 형태(Machine-Readable)로 명세한 문서.
- **Log4Shell**: 2021년 발생한 Log4j 취약점 사태로, 수많은 기업들이 자사 시스템에 Log4j가 포함되어 있는지조차 파악하지 못해 대규모 피해를 입으며 SBOM 도입의 결정적 계기가 된 사건.
- **NTIA (국가통신정보청)**: 미국 상무부 산하 기관으로, 행정명령(EO 14028)에 따라 연방정부에 납품되는 모든 소프트웨어에 대해 SBOM 제출을 의무화하는 최소 요건(Minimum Elements)을 제정한 기관.

</details>

- 정의/개념: 소프트웨어 패키지 내부에 포함된 직접 의존성 및 전이(Transitive) 의존성을 모두 추적하여 구성요소의 가시성을 확보하는 **소프트웨어 부품 명세서**
- 배경/필요성: 현대 소프트웨어의 80% 이상이 오픈소스로 구성되어 있으나, 내부 부품의 취약점(CVE)이나 라이선스 위반 리스크를 투명하게 파악하고 신속히 대응할 수 있는 추적성 확보의 요구성
...
- **Data Fields (데이터 필드)**: 공급자명, 컴포넌트명, 버전, 식별자(purl 등), 의존성 관계, 작성자 정보 등 부품을 고유하게 식별할 수 있는 필수 메타데이터.
- **Automation Support (자동화 지원)**: 사람이 읽는 문서가 아니라, SPDX, CycloneDX, SWID 등의 표준 데이터 포맷을 사용하여 기계(CI/CD)가 자동 생성 및 분석할 수 있는 체계.
- **Practices & Processes (운영 프로세스)**: 소프트웨어 릴리즈마다 SBOM을 새로 갱신하고, 하위 의존성 깊이까지 추적하며, 이를 안전하게 접근 통제하는 관리 절차.
...
1. **추출**: 빌드 시점에 도구(Syft 등)가 `pom.xml`, `node_modules` 등을 훑어 모든 부품의 해시와 버전을 추출.
2. **서명**: 생성된 SBOM 파일이 중간에 위조되지 않도록(공급망 공격 방어) 디지털 서명 수행.
3. **매칭 및 VEX**: NVD 취약점과 대조하여 취약한 부품이 식별되면, 실제 악용 가능 여부를 평가하여 VEX 문서로 대응(**공급망 가시성 완결**).
...
- **SBOM 수립 기준**에 따라 공공/금융 및 클라우드 네이티브 서비스 구축 시 **CI/CD 파이프라인 내 SPDX/CycloneDX 포맷 생성 자동화** 필수 적용
