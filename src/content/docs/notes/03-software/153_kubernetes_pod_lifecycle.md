---
sidebar:
  order: 153
  label: "153. 쿠버네티스 Pod 생명주기 (Kubernetes Pod Lifecycle)"
  badge:
    text: "기출 · 30%"
    variant: note
title: "쿠버네티스 Pod 생명주기 (Kubernetes Pod Lifecycle)"
date: "2026-08-02T23:23:00+09:00"
tags: ["notes-software"]
weight: 153
extra:
  question_no: "153"
  source_status: "기출"
  source_history: "123회"
  priority: 30
  priority_note: "파드 상태 전이와 프로브 역할 구분 출제"
---

## Ⅰ. 개요

<details>
<summary>핵심 용어</summary>

- **쿠버네티스 파드 생명주기(Kubernetes Pod Lifecycle)**: 파드 생성부터 스케줄·컨테이너 실행·상태 점검·재시작·종료까지 실행·준비·복구 상태를 관리하는 과정이다.

</details>

- 정의/개념: 파드 생성부터 종료까지 단계·조건·프로브·재시작 정책으로 **실행·준비·복구 상태** 를 관리하는 생명주기 모델
- 배경/필요성: 단일 실행 상태만으로는 **재시작·트래픽 허용** 판단 분리 불가

#### 한줄 요약

- 프로그램이 켜진 상태와 손님을 받을 준비가 된 상태는 다르므로 파드는 실행, 생존, 준비 여부를 서로 다른 신호로 표현한다.

## Ⅱ. 특징

<details>
<summary>핵심 용어</summary>

- **프로브 역할 분리**: 활성, 준비, 시작 프로브의 역할을 분리하면 재시작과 트래픽 제외 판단이 서로 간섭하지 않는다.
- **파드 단계·조건·컨테이너 상태**: 파드 전체의 수명주기, 서비스 준비 조건, 개별 컨테이너의 대기·실행·종료를 서로 다른 계층으로 표현한 상태이다.
- **정상 종료**: 종료 전 훅과 종료 신호 및 유예 시간을 이용해 연결과 작업을 정리한 뒤 프로세스를 끝내는 절차이다.

</details>

- **파드 단계·조건** 과 **컨테이너 상태** 의 계층 표현
- 시작·활성·준비 **프로브 역할 분리**
- preStop·TERM과 유예 시간에 따른 **정상 종료**

#### 한줄 요약

- 시작이 늦은 상황, 멈춘 상황, 잠시 요청을 받지 못하는 상황을 구분해야 불필요한 재시작과 서비스 단절을 줄일 수 있다.

## Ⅲ. 구조 및 구성요소

<details>
<summary>핵심 용어</summary>

- **활성·준비 프로브**: 활성 프로브는 재시작 필요성을, 준비 프로브는 서비스 엔드포인트 포함 가능성을 판단한다.
- **큐블릿(kubelet)**: 노드에서 파드 명세에 따라 컨테이너를 실행하고 프로브·재시작·종료를 관리하는 에이전트이다.
- **초기화·주 컨테이너**: 업무 컨테이너 실행 전 선행 작업을 완료하는 컨테이너와 실제 서비스를 처리하는 컨테이너이다.
- **시작 프로브(Startup Probe)**: 느린 애플리케이션이 기동을 마칠 때까지 활성·준비 검사를 유예하는 검사이다.
- **종료 제어**: 종료 전 훅·종료 신호·유예 시간으로 정상 정리와 강제 종료 시점을 관리하는 구성요소이다.

</details>

```mermaid
block
    columns 1
    A["스케줄러·kubelet"]
    B["초기화·주 컨테이너"]
    C["시작 프로브"]
    D["활성·준비 프로브"]
    E["종료 제어"]
    A --- B
    B --- C
    B --- D
    A --- E
```

| 구성요소 | 책임 |
|:---|:---|
| 스케줄러·kubelet | 노드 배정·**컨테이너 실행** |
| 초기화·주 컨테이너 | **선행 작업·업무 처리** |
| 시작 프로브 | **느린 시작** 보호 |
| 활성·준비 프로브 | **재시작·트래픽** 판정 |
| 종료 제어 | **정상 정리·강제 종료** |

#### 한줄 요약

- kubelet이 현장 관리자라면 초기화 컨테이너는 개점 준비, 프로브는 안전·영업 검사, 종료 제어는 폐점 정리 절차에 해당한다.

## Ⅳ. 흐름도

<details>
<summary>핵심 용어</summary>

- **4. 준비 상태**: 준비 프로브가 성공해 준비 상태가 되면 파드는 서비스 트래픽을 받을 수 있다.
- **응용 프로그래밍 인터페이스(Application Programming Interface, API) 서버**: 파드 생성·삭제와 상태 변경을 저장하고 노드에 전달하는 제어면 접점이다.
- **1. 파드 실행 명세**: 배정 노드에서 실행할 컨테이너·자원·재시작 정책을 확정한 명세이다.
- **2. 시작·생존·준비 검사 요청**: 기동 완료, 내부 생존, 트래픽 수신 가능성을 독립적으로 측정하는 요청이다.
- **3. 프로브 판정**: 컨테이너 재시작과 서비스 엔드포인트 포함 여부를 결정하는 검사 결과이다.
- **5. 종료 전 훅(PreStop Hook)·종료 신호(Termination Signal, TERM)·유예 시간**: 연결을 정리한 뒤 강제 종료할 시점을 적용하는 종료 절차이다.

</details>

```mermaid
sequenceDiagram
    participant A as API 서버
    participant K as kubelet
    participant C as 컨테이너
    participant E as 서비스
    A->>K: 1. 파드 실행 명세
    K->>C: 컨테이너 실행 명세
    loop 프로브 주기
        K->>C: 2. 시작·생존·준비 검사 요청
        C-->>K: 3. 프로브 판정
        K->>E: 4. 준비 상태
    end
    A-->>K: 삭제 명세
    K-->>C: 5. preStop·TERM·유예 시간
```

**동작 원리**

1. **파드 실행 명세**: 배정 노드의 컨테이너·자원·재시작 정책 확정
2. **시작·생존·준비 검사 요청**: 수명주기별 독립 상태 측정
3. **프로브 판정**: 재시작·트래픽 허용 근거 생성
4. **준비 상태**: 서비스 엔드포인트 포함 여부 갱신
5. **preStop·TERM·유예 시간**: 연결 정리 후 강제 종료 시점 적용

#### 한줄 요약

- 파드가 시작되면 kubelet은 세 가지 검사를 반복하고, 준비 검사 결과만 서비스에 반영해 요청 차단과 프로세스 재시작을 분리한다.

## Ⅴ. 종류 및 비교

<details>
<summary>핵심 용어</summary>

- **시작 프로브(Startup Probe)**: 느린 애플리케이션의 기동 완료 여부를 검사해 성공 전 다른 프로브를 유예한다.
- **활성 프로브(Liveness Probe)**: 컨테이너가 내부적으로 복구 불가능한 상태인지 검사해 실패 시 재시작한다.
- **준비 프로브(Readiness Probe)**: 컨테이너가 요청을 처리할 준비가 되었는지 검사해 실패 시 서비스 엔드포인트에서 제외한다.

</details>

| 프로브 유형 | Startup Probe | Liveness Probe | Readiness Probe |
|:---|:---|:---|:---|
| 적용 기준 | **느린 애플리케이션 시작** | **내부 복구 불가 상태** | **트래픽 수신 가능성** |
| 핵심 특징 | 성공 전 **다른 검사 유예** | 실패 시 **컨테이너 재시작** | 실패 시 **엔드포인트 제외** |
| 한계 | 설정 과다 시 **기동 지연** | 외부 장애 시 **재시작 폭주** | 오판 시 **가용 파드 감소** |

#### 한줄 요약

- 시작 검사는 기다릴 시간을, 활성 검사는 다시 켤 조건을, 준비 검사는 요청을 보낼 조건을 각각 결정한다.

## Ⅵ. 실무 고려사항 및 대책

<details>
<summary>핵심 용어</summary>

- **재시작 폭주**: 재시작 폭주는 잘못된 활성 검사나 짧은 임계값 때문에 정상화 전에 컨테이너가 반복 재시작되는 현상이다.
- **종료 전 훅(PreStop Hook)·종료 유예**: 서비스 준비 상태를 해제하고 진행 중인 요청을 정리할 시간을 제공하는 종료 통제이다.
- **멱등 처리**: 같은 작업을 재실행해도 최종 결과가 한 번 실행한 것과 같게 유지되는 처리 방식이다.

</details>

| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 외부 장애 **연동 검사** | 활성 검사를 **내부 상태로 제한** | **재시작 폭주** 방지 |
| 긴 **초기 기동 시간** | 시작 검사 **주기·임계값 산정** | **조기 종료** 방지 |
| **준비 파드 급감** | 완화 임계값·**최소 용량 확보** | 엔드포인트 **전멸 방지** |
| 처리 중 **요청 단절** | 준비 해제·**preStop·유예 확보** | **연결 정상 종료** |
| 재시작 시 **로컬 상태 손실** | **외부 저장·멱등 처리** 적용 | 중복·**데이터 손실 축소** |

#### 한줄 요약

- 데이터베이스가 잠시 느리다는 이유로 모든 컨테이너를 재시작하지 않도록 활성 검사는 내부 고장만 보고 준비 검사로 트래픽부터 걷어 내야 한다.

## Ⅶ. 결론

<details>
<summary>핵심 용어</summary>

- **종료 유예**: 종료 유예는 종료 신호 후 애플리케이션이 연결과 작업을 안전하게 정리할 시간을 보장한다.

</details>

- 기동·생존·트래픽 상태에 맞춰 **프로브 역할** 과 **종료 유예** 설정

#### 한줄 요약

- 느린 시작은 기다리고 내부 정지는 재시작하며 외부 의존 장애는 요청만 차단하는 기준으로 파드 생명주기를 설계해야 한다.
