...
- 정의/개념: 파드 생성부터 종료까지 단계•조건•프로브•재시작 정책으로 **실행•준비•복구 상태**를 관리하는 생명주기 모델이다.
- 배경/필요성: 단일 실행 상태만으로는 **재시작•트래픽 허용** 판단 분리할 수 없다.
...
- **파드 단계•조건**과 **컨테이너 상태**의 계층 표현이 핵심이다.
...
- 기동•생존•트래픽 상태에 맞춰 **프로브 역할**과 **종료 유예** 설정이 핵심이다.
