...
- 배경/필요성: 쿠버네티스 환경에서 단순 프로세스 생존(PID) 여부만으로 상태를 판단할 경우 초기 기동이 긴 애플리케이션의 조기 강제 재시작 반복, 내부 데드락에 빠진 불능 컨테이너로의 트래픽 지속 인입 및 롤링 배포 시 파드 강제 종료에 따른 클라이언트 502/503 에러 유발 한계를 극복하고, 파드의 상태 전이 단계에 맞추어 3대 헬스체크 프로브(Startup, Liveness, Readiness)와 단계별 정상 종료(Graceful Shutdown: preStop, SIGTERM, SIGKILL) 체계를 수립하여 **무중단 롤링 배포와 결함 없는 자가 치유(Self-Healing)를 달성**할 필요
...
- 마이크로서비스 애플리케이션의 무중단 배포 및 고가용성 서비스 연속성을 보장하는 **가장 핵심적인 파드 런타임 제어 메커니즘**으로 확립되었으며, 실무 운영 시에는 **부팅 타임아웃을 방어하는 StartupProbe, 내부 데드락 복구 전용 LivenessProbe와 외부 의존성 장애 시 트래픽만 선별 격리하는 ReadinessProbe의 엄격한 역할 분리, 엔드포인트 전파 지연을 보정하는 `preStop: sleep 10` 훅 및 `terminationGracePeriodSeconds` 유예 설정**을 결합하여 롤링 업데이트 시 다운타임과 커넥션 단절을 0으로 통제
