...
- 배경/필요성: 프로세스 생존 여부만 보면 부팅이 긴 앱을 죽은 것으로 오판해 재시작을 반복하고 응답 불능 상태의 앱에는 트래픽을 계속 보내며 종료 시에도 연결을 먼저 끊지 못해 오류가 나므로, 시작·생존·수신 준비를 각기 다른 프로브로 나누고 종료 절차에 유예 구간을 두어 판단 기준을 상황별로 분리할 필요
