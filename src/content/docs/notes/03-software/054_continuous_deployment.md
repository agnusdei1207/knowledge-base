...
- 배경/필요성: 배포 파이프라인 상의 수동 승인 게이트가 유발하는 대규모 배치(Batch) 배포 누적, 릴리즈 리드타임 지연 및 배포 실패 시 장애 원인 특정 난항을 극복하고, 테스트와 보안 검증을 통과한 아티팩트를 사람의 개입 없이(No-Touch) 소규모 커밋 단위로 프로덕션에 즉시 배포하며, Argo Rollouts와 프로메테우스 기반의 점진적 카나리(Progressive Delivery) 및 SLO 가드레일 자동 롤백을 통해 **배포 실패 반경(Blast Radius)을 극소화하고 비즈니스 출시 속도를 극대화**할 필요
