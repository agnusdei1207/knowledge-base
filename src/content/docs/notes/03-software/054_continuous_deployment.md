...
- **품질 게이트·버전 아티팩트**: 품질 게이트는 운영 진입 조건을 자동 판정하고 버전 아티팩트는 실행 내용과 무결성을 고정해 보존한다.
...
- **롤아웃·롤백(Rollout·Rollback)**: 롤아웃은 새 버전 노출을 확대하고 롤백은 이전 버전으로 되돌리는 동작이다.
...
- **카나리 단계·관찰 시간**: 새 버전 노출 비율을 나눈 단계와 각 단계에서 지표를 평가할 최소 시간이다.
- **신구 호환(Backward·Forward Compatibility)**: 새 코드와 이전 코드가 전환 기간의 같은 데이터 구조를 모두 안전하게 읽고 쓰는 성질이다.
- **기능 플래그 소유자·만료일**: 플래그 정리 책임자와 임시 분기를 제거해야 할 시점이다.
- **최소 표본·연속 위반**: 자동 판정에 필요한 최소 요청 수와 임계치를 몇 차례 연속 넘어야 복귀할지를 정한 조건이다.
