...
- 정의/개념: 관계형 데이터베이스(RDBMS)에서 각 튜플의 식별 유일성을 지키는 **개체 무결성(Entity Integrity)** 과 릴레이션 간 참조 관계의 정확성을 지키는 **참조 무결성(Referential Integrity)**
- 배경/필요성: 동시 데이터 입력 시 PK 중복으로 인한 식별 불능 방지, 부모 없는 고아 데이터(Orphan Data) 발생에 따른 데이터베이스 붕괴 차단 요구성
...
- **Primary Key 기반의 Unique & NOT NULL 보장 (개체 무결성)**
- **Foreign Key 기반의 부모-자식 참조 정합성 보장 (참조 무결성)**
- DDL 선언 시 **CASCADE, SET NULL, RESTRICT** 연쇄 삭제/수정 규칙 지원

#### 한줄 요약

- 이름표는 하나뿐이어야 하고, 자식 기록은 존재하는 부모에게만 연결되어야 한다.

## Ⅲ. 구조 및 구성요소 (개체 무결성 대 참조 무결성 메커니즘)
...
┌────────────────────────────────────────────────────────────────────────┐
│ [부모 릴레이션: Users]                                                 │
│  PK: user_id (1001)  <─── 개체 무결성 (NOT NULL & Unique 보장)         │
└──────────▲─────────────────────────────────────────────────────────────┘
           │ (참조 무결성: FK 값은 부모 PK 1001 이거나 NULL 이어야 함)
┌──────────┴─────────────────────────────────────────────────────────────┐
│ [자식 릴레이션: Orders]                                                │
│  PK: order_id (5001) | FK: user_id (1001)                              │
└────────────────────────────────────────────────────────────────────────┘
```
...
```text
[부모 PK 데이터 UPDATE / DELETE 요청]
                  │
                  ▼
┌────────────────────────────────────────────────────────┐
│ 자식 릴레이션의 FK 연쇄 행동 규칙 (Referencing Action) │
├────────────────────────────────────────────────────────┤
│ • RESTRICT   ──► 자식 튜플 존재 시 부모 삭제 즉시 블로킹│
│ • CASCADE    ──► 부모 PK 변경/삭제 시 자식 튜플 연쇄 삭제│
│ • SET NULL   ──► 부모 PK 삭제 시 자식 FK 값을 NULL 전환│
│ • SET DEFAULT ──► 부모 PK 삭제 시 자식 FK 에 디폴트값 주입│
└────────────────────────────────────────────────────────┘
```

### 동작 원리

1. **Check Dependent FK**: 부모 데이터 삭제 시 DBMS가 자식 릴레이션 스캔.
2. **Execute Action Rule**:
   - `RESTRICT`: 자식 데이터가 1건이라도 있으면 삭제 거부.
   - `CASCADE`: 자식 데이터도 즉시 함께 연쇄 삭제.
   - `SET NULL`: 자식 외래키 필드를 `NULL`로 변경하여 부모 소멸 이력 관리.

#### 한줄 요약

- 주문 번호가 겹치지 않고 고객 번호가 실제 고객을 가리킬 때만 주문을 저장한다.

## Ⅴ. 종류 및 비교 (개체 무결성 대 참조 무결성)
...
| `NULL` 값 허용 여부| **`NULL` 절대 불가 (Unacceptable)** | **`NULL` 허용 가능 (Acceptable)** |
...
| 부모 삭제 시 자식 테이블 전체에 Lock 대기 발생 | **외래키(FK) 컬럼에 B-Tree 인덱스 필수 추가** | 참조 검사 속도 향상 |
...
- **개체•참조 무결성 수립 기준**에 따라 RDBMS 물리 설계 시 **PK NOT NULL & FK B-Tree 인덱싱** 필수 수용
