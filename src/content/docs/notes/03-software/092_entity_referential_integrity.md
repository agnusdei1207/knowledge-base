...
- **1. 주문 생성 요청**: 고객 식별자를 전달
- **2. 주문 기본키 중복 확인**: 식별자를 검사
- **3. 유일성 검사 결과**: 중복 여부를 반환
- **4. 고객 외래키 검사 요청**: 관계를 검증
- **5. 부모 고객 키 조회**: 참조 대상을 검색
- **6. 부모 존재 여부 반환**: 유효 관계를 통지
- **7. 저장 결과·오류 응답**: 통과한 주문만 저장
