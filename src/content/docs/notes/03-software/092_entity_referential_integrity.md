...
- 배경/필요성: 기본키가 없으면 같은 값을 가진 행을 구분할 수 없어 수정 대상을 특정하는 비용이 무한정 커지고 외래키 검증이 없으면 부모가 사라진 뒤에도 자식 행이 남아 조인 결과가 조용히 어긋나므로, 식별과 연결의 검증을 릴레이션 정의 자체에 못 박아 모든 조작 경로가 통과하게 할 필요
