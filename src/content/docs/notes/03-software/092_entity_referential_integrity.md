...
- **정의**: 관계형 데이터베이스(RDBMS)에서 각 튜플의 식별 유일성을 지키는 **개체 무결성(Entity Integrity)** 과 릴레이션 간 참조 관계의 정확성을 지키는 **참조 무결성(Referential Integrity)** 메커니즘.
- **필요성**: PK 중복에 따른 식별 불능 방지 및 고아 데이터(Orphan Data) 발생에 따른 데이터베이스 붕괴 차단.
...
├────────────────────────────────────────────────────────┤
│ • RESTRICT   ──► 자식 존재 시 삭제 블로킹             │
│ • CASCADE    ──► 부모 PK 변경/삭제 시 자식 연쇄 삭제   │
│ • SET NULL   ──► 부모 PK 삭제 시 자식 FK 널값 전환    │
│ • SET DEFAULT ──► 부모 PK 삭제 시 자식 FK 디폴트 주입  │
└────────────────────────────────────────────────────────┘
```
...
- **개체•참조 무결성 수립 기준 적용** (RDBMS 물리 설계 시 PK NOT NULL 및 FK B-Tree 인덱스 필수 수용)
