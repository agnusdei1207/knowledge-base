...
- **참조 무결성**: 외래키가 NULL이거나 존재하는 부모 후보키와 일치하도록 관계를 보장하는 규칙이다.
...
- **안정 후보키·대체키**: 변경 가능한 업무값 대신 오래 유지되는 식별자를 기본키로 쓰고 업무상 유일값은 별도 키로 관리하는 설계이다.
- **NULL 허용 조건·업무 의미**: 값이 없는 상태가 선택·미확정·해당 없음 중 무엇인지 열마다 명확히 정한 규칙이다.
- **영향 제한·감사·소프트 삭제**: 부모 삭제를 제한하거나 이력을 남기고 물리 삭제 대신 비활성 상태로 전환하는 통제이다.
- **부모·자식 키 인덱스**: 참조 대상 탐색과 외래키 검사를 빠르게 하도록 양쪽 키에 구성하는 인덱스이다.
- **부모 우선 적재·사후 대사**: 데이터 이관 시 부모를 먼저 저장하고 완료 후 고아 참조가 없는지 비교 검증하는 절차이다.
