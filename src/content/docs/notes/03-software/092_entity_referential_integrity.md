...
- 배경/필요성: 기본키 중복 및 유효하지 않은 외래키 참조로 인한 **튜플 식별 불능 및 부모 없는 고아 레코드(Orphan) 발생 해결 불가**

#### 한줄 요약
- 기본키 기반 개체 무결성과 외래키 기반 참조 무결성으로 관계형 모델의 정합성을 보장한다.
...
#### 한줄 요약
- 기본키는 튜플의 고유 식별을, 외래키는 부모-자식 간 연결 정합성을 각각 전담한다.
...
```text
1. 부모 테이블(Users) 특정 레코드에 대해 DELETE SQL 실행
        │
   2. DBMS 엔진이 자식 테이블(Orders)의 외래키(FK) 인덱스 검색
        │
   해당 부모 PK를 참조하는 자식 레코드가 존재하는가?
   ┌────┴───────────────────────────┐
  예 (참조 자식 데이터 존재)         아니오 (참조 없음)
   │                                 │
3. [ON DELETE 정책 분기]             부모 레코드 즉시 삭제 완료
   ┌────┼────────────────────┐
   │    │                    │
...
#### 한줄 요약
- 부모 삭제 요청 → 자식 FK 검사 → 정책 판정 → RESTRICT(롤백) 또는 CASCADE(연쇄 처리) 순으로 진행된다.
...
- 관계형 데이터베이스의 무결성을 지탱하기 위해 **불변 대리키 기반의 개체 무결성과 B-Tree 인덱스가 적용된 참조 무결성을 필수 구성**하고, **RESTRICT 및 Soft Delete 정책**으로 데이터 안정성 확보
