...
- 배경/필요성: 전통적인 단일 RDBMS가 마주하는 수평 확장(Scale-Out)의 구조적 한계와 NoSQL 도입 시 수반되는 ACID 트랜잭션 부재, 복잡한 애플리케이션 수동 샤딩 및 정합성 관리 오버헤드를 극복하기 위해, 표준 SQL 인터페이스와 완전한 분산 ACID 직렬성을 보장하면서도 Raft/Paxos 합의 복제 및 자동 범위 분할(Range Split)을 통해 **글로벌 규모의 무중단 수평 확장성과 강력한 정합성을 동시에 실현**할 필요
...
- 글로벌 핀테크, 전자상거래 결제 원장 및 클라우드 네이티브 엔터프라이즈 영속성 계층의 **차세대 분산 RDBMS 표준 아키텍처**로 확립되었으며, 실무 구축 시에는 **특정 Range 핫스팟을 방지하는 UUID/Hash 기반 PK 설계, 다중 리전 간 합의 RTT를 최소화하는 지역 밀착형 파티셔닝(Locality-Aware), 교차 노드 조인 부하를 제거하는 테이블 코로케이션(Co-location), 분산 시계 동기화(TrueTime/HLC) 거버넌스**를 결합하여 엄격한 직렬성(Serializability)과 초저지연 수평 확장을 동시 보증
