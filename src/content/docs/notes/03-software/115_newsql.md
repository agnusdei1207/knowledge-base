...
- 정의/개념: SQL·ACID를 합의 복제로 수평 확장한 **분산 DB**
...
- **1. 키 범위·리더 조회**: 키 범위를 담당하는 복제 그룹과 현재 리더 결정
- **2. 범위별 실행 계획**: SQL 연산을 관련 범위의 분산 트랜잭션으로 분해
- **3. 로그 합의·정족수 확인**: 각 범위 로그를 합의 복제하고 충돌·정족수 결과 수집
- **4. 원자적 커밋 결과**: 모든 참여 범위의 확정 여부를 하나의 트랜잭션 결과로 반환
...
> 제품 이름만으로 성능을 단정할 수 없으며 스키마·보조 인덱스·지역 배치·교차 범위 비율이 실제 비용을 좌우
...
> **적용 사례**: 주문과 주문 항목의 키 범위를 같은 지역에 배치해 교차 리전 합의 횟수를 줄이고 재시도율과 p99 커밋 지연을 측정
...
- **키 분포·지역성·경합·교차 범위 비율**로 NewSQL 배치를 결정
