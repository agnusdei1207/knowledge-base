...
- **뉴SQL(NewSQL) 데이터베이스**: SQL•ACID와 합의 복제로 수평 확장하는 데이터베이스다.

</details>

- 정의/개념: **SQL•ACID**를 유지하며 수평 확장하는 **NewSQL** 계열이다.
- 배경/필요성: 단일 노드 관계형 DB는 저장•처리 용량이 **한 장비 한계**에 종속이 발생한다.
...
- **대상 키 범위 추출**: 질의 조건에서 담당 키 구간을 찾는 단계다.
- **범위•리더 위치 확인**: 합의 복제 그룹과 현재 리더를 찾는 단계다.
- **범위별 실행 계획**: SQL 연산을 키 구간별 작업으로 나누는 단계다.
- **합의 로그 항목 복제**: 각 복제 그룹에 변경 기록을 보내는 단계다.
- **분산 커밋•충돌 판정**: 모든 범위의 확정이나 취소를 결정하는 단계다.
...
- **1. 대상 키 범위 추출**: 질의 조건에서 키 구간 식별이다.
- **2. 범위•리더 위치 확인**: 복제 그룹과 현재 리더 탐색이다.
- **3. 범위별 실행 계획**: SQL 연산을 분산 작업으로 분해이다.
- **4. 합의 로그 항목 복제**: 변경 기록을 복제 그룹에 전달한다.
- **5. 분산 커밋•충돌 판정**: 원자 확정 또는 취소를 결정한다.
...
- 수평 확장과 **ACID**가 함께 필요하면 **NewSQL**을 선택한다.
