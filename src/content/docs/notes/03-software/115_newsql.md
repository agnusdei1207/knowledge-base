...
- **1. SQL 트랜잭션 요청**: 거래를 전달
- **2. 키 범위·리더 조회**: 담당 위치를 검색
- **3. 범위 배치 반환**: 복제 그룹을 반환
- **4. 범위별 계획 전달**: 연산을 분리
- **5. 로그 합의 요청**: 각 범위에 확정을 요청
- **6. 정족수 결과 반환**: 복제·충돌 결과를 전달
- **7. 커밋 결과 응답**: 원자 결과를 반환
...
> **적용 사례**: 주문과 주문 항목의 키 범위를 같은 지역에 배치해 교차 리전 합의 횟수를 줄이고 재시도율과 p99 커밋 지연을 측결정
