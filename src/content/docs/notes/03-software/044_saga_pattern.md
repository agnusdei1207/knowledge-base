...
- 정의/개념: 각 마이크로서비스별 독립 로컬 트랜잭션 커밋 후, 실패 발생 시 역순으로 **보상 트랜잭션(Compensating Transaction)**을 실행하여 데이터 최종 일관성을 수습하는 **Saga Pattern**
- 배경/필요성: traditional 2PC (Two-Phase Commit)의 전역 락(Global Lock)으로 인한 서비스 가용성 파괴 방지 및 Database-per-service 분산 환경의 트랜잭션 제어 요구성
...
| 구성요소 | 주요 역할 및 기술 사양 |
|:---|:---|
| **Local Transaction** | 각 마이크로서비스가 자신의 독립 DB 내에서 수행하는 단일 ACID 트랜잭션 |
| **Compensating Transaction**| 이미 커밋된 로컬 트랜잭션 결과를 보상 처리(결제 취소, 주문 취소)하는 반대 트랜잭션 |
| **Pivot Transaction** | 사가의 성공/실패를 결정짓는 핵심 트랜잭션 (이 단계 통과 시 이후 보상 불필요) |
| **Retryable Transaction** | Pivot 이후 단계의 트랜잭션으로, 실패하더라도 반드시 성공할 때까지 재시도 |
| **Saga Orchestrator** | Orchestration 방식에서 전체 사가 상태 관리 및 보상 흐름 총괄 지휘 |
...
[1. 주문 생성 (T1)] ──► [2. 결제 승인 (T2)] ──► [3. 재고 차감 (T3)] ──► [성공 완료]

장애 발생 및 보상 흐름 (Compensating Flow)

[1. 주문 생성 (T1)] ──► [2. 결제 승인 (T2)] ──► [3. 재고 차감 (T3 - 실패!)]
                                                        │
[1'. 주문 취소 (C1)] ◄── [2'. 결제 취소 (C2)] ◄──────────┘ (역순 보상 트랜잭션 실행)
```

### 동작 원리

1. **Forward Flow**: 1단계 주문생성($T_1$) 커밋 $\rightarrow$ 2단계 결제승인($T_2$) 커밋 $\rightarrow$ 3단계 재고차감($T_3$) 시도.
2. **장애 감지**: 3단계 재고차감($T_3$)에서 재고 부족으로 예외(Fail) 발생.
3. **Compensating Flow 발동**: 이미 성공하여 커밋된 $T_2, T_1$을 취소하기 위해 역순($C_2 \rightarrow C_1$) 보상 트랜잭션 실행.
4. **역순 보상 처리**: $C_2$(결제 취소 커밋) 수행 $\rightarrow$ $C_1$(주문 취소 커밋) 수행.
5. **최종 정합성 회복**: 모든 보상 처리가 완결되어 데이터 상태가 최종 일관성(Eventual Consistency)으로 안정화.
...
|:---|:---|:---|
| 제어 방식 | **이벤트 기반 자율 전파 (Pub/Sub)** | **중앙 Saga Orchestrator 제어** |
...
- **분산 트랜잭션 선택 기준**에 따라 MSA 구축 시 **2PC 금지** 및 **Saga Pattern (Semantic Lock + Outbox)** 인가
