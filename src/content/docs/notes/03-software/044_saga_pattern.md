...
- **업무 식별자•처리 이력**: 하나의 Saga와 중복 요청을 구분하는 고유 값과 이미 반영한 명령•이벤트 기록이다.
- **재시도 상한•수동 복구 큐**: 자동 보상을 다시 시도할 최대 횟수와 그 한도를 넘은 Saga를 담당자에게 전달하는 대기열이다.
- **진행 상태•후속 작업 제한**: Saga가 완료 전임을 명시하고 확정 상태가 필요한 다음 업무의 실행을 막는 통제다.
