...
- **메시지 채널·재전송(Message Channel·Redelivery)**: 메시지 채널은 서비스 간 명령·이벤트를 전달하고 재전송은 응답 누락 시 같은 메시지를 다시 보내므로 멱등 처리가 필요한다.
...
- **서버·볼륨(Server·Volume)**: 서버는 계산 인스턴스이고 볼륨은 영구 저장 공간으로서 설정 실패 시 이미 만든 자원을 보상 단계에서 삭제한다.
...
- 정의/개념: 로컬 트랜잭션을 연결하고 실패를 보상하는 **분산 패턴**
- 배경/필요성: 독립 저장소 사이 **원자 커밋 부재**를 보상으로 처리
...
- 주문 결제가 실패하면 확보한 재고를 복원하고 주문을 취소하며 자동 보상 실패는 수동 복구 큐로 전달
...
- **보상 가능성과 복구 책임**이 명확한 흐름에만 Saga 적용
