...
- **1. 고객 업무 요청**: 퍼블릭 API를 호출
- **2. 사용자·서비스 권한 검증**: 환경 간 접근 범위를 확인
- **3. 제한 자격·정책 반환**: 최소 권한과 데이터 조건을 제공
- **4. 최소 데이터 요청**: 전용 경로로 필요한 값만 호출
- **5. 권위 데이터·상태 반환**: 원문 이동을 줄여 결과를 전달
- **6. 조합 업무 응답**: 퍼블릭 환경에서 결과를 제공
- **7. 경계 지연·감사 기록**: 양쪽 상태를 같은 요청 ID로 연결
