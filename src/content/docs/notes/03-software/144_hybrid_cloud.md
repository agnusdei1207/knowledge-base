...
- **정의/개념**: 전용 환경과 퍼블릭 클라우드를 연결하는 **통합 배포 모델**이다.
- **배경/필요성**: 규제·기존 자산을 유지하며 **탄력 자원·관리형 서비스**를 활용한다.
...
- **목적별 배치**: 규제·지연·부하로 환경 역할을 나눈다.
- **경계 연결**: 전용선·VPN·게이트웨이로 환경을 잇는다.
- **통합 통제**: 신원·정책·데이터 정합성·관측을 연결한다.
...
block
    columns 1
    A["전용 환경 | 규제 데이터·기존 시스템 운영"]
    B["퍼블릭 환경 | 외부 접점·탄력 처리"]
    C["연결 계층 | 전용선·VPN·라우팅 관리"]
    D["통합 신원·데이터 연계 | 권한·정합성 통제"]
    E["통합 운영 | 로그·추적·비용 연결"]
```
...
- **1. 고객 업무 요청**: 퍼블릭 API를 호출한다.
- **2. 사용자·서비스 권한 검증**: 환경 간 접근 범위를 확인한다.
- **3. 제한 자격·정책 반환**: 최소 권한과 데이터 조건을 제공한다.
- **4. 최소 데이터 요청**: 전용 경로로 필요한 값만 호출한다.
- **5. 권위 데이터·상태 반환**: 원문 이동을 줄여 결과를 전달한다.
- **6. 조합 업무 응답**: 퍼블릭 환경에서 결과를 제공한다.
- **7. 경계 지연·감사 기록**: 양쪽 상태를 같은 요청 ID로 연결한다.
...
- **데이터 위치·경계 지연·연결 복구·이중 운영 비용**으로 하이브리드 클라우드를 도입한다.
