...
date: "2026-07-25T11:20:00+09:00"
tags:
  - "notes-software"
weight: 125
extra:
...
- **정의/개념**: 원천 데이터를 보존해 읽을 때 구조화한다
- **배경/필요성**: 비정형 원천의 재사용과 재처리를 지원한다
...
- 저장·연산 분리는 자원별 확장 비용을 줄인다
- 원천 보존은 새 규칙의 재처리와 결과 재현을 돕는다
- 품질 영역은 원천 오염이 검증 데이터로 번짐을 막는다
- 카탈로그·계보·정책은 데이터 늪과 오용을 막는다
...
| 객체 스토리지 | 객체 저장과 연산 자원을 독립 확장 |
| 데이터 영역 | 원천 변경 여부·품질 수준을 구분 |
| 카탈로그·거버넌스·수명주기 | 메타데이터·접근·보존 정책 관리 |
| 처리 엔진 | 목적별 스키마로 데이터를 해석 |
...
| 절차 | 설명 |
|:---|:---|
| 원천 데이터 저장 | 변경 없이 랜딩·원천 영역에 보존 |
| 메타데이터 등록 | 스키마·분류·파티션을 카탈로그에 기록 |
| 정제 데이터 생성 | 품질 검사·파일 크기 최적화 |
| 검증 영역 게시 | 통과 데이터를 큐레이티드 영역에 공개 |
...
| 판단 기준 | 데이터 레이크 | 데이터 웨어하우스 |
|:---|:---|:---|
| 핵심 특징 | 원천을 보존하고 읽을 때 스키마를 적용함 | 적재 전에 사실·차원 모델로 통합함 |
| 적용 기준 | 비정형 탐색·ML·재처리 | 공통 지표·고속 집계 |
| 주요 위험 | 데이터 늪·작은 파일·전체 스캔 | 적재 지연·모델 변경 비용 |
...
1. 날짜 파티션으로 로그 스캔 범위를 줄인다
2. 버전·계보로 모델 학습 데이터 집합을 재현한다
