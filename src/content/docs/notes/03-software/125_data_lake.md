...
- **머신러닝(Machine Learning, ML)**: 영문 첫 글자를 딴 ML을 '엠엘'로 읽으며 레이크 데이터를 학습해 예측·분류하는 처리 목적이다.

## Ⅰ. 개요

- 정의/개념: 다형식 원천을 객체 저장소에 보존하는 **데이터 저장소**
...
- **1. 원천 객체·버전 저장**: 원본을 변경하지 않고 재현 가능한 객체 키·버전으로 보존
- **2. 메타데이터 등록**: 구조·분류·소유자·보존 정책을 카탈로그에 연결
- **3. 처리 대상 탐색**: 소비 목적에 맞는 위치·스키마·권한 조건 확인
- **4. 정제 파일 생성**: 품질 규칙과 조회 패턴에 맞춰 형식·파티션·파일 크기 조정
- **5. 계보·품질 게시**: 입력·변환·산출 관계와 검증 결과를 다음 사용자에게 공개
...
> 실제 서비스는 기능이 겹치므로 명칭보다 원본 보존·테이블 보장·거버넌스·질의 성능의 요구 수준으로 선택
...
> **적용 사례**: 로그를 날짜·서비스로 파티션하고 작은 파일을 적정 크기의 열 형식 파일로 병합해 필요한 기간만 조회
...
- **원본 재현성·카탈로그·품질·파일 수명주기**로 Data Lake를 설계
