...
- 정의/개념: 특정 정보화 사업의 요구•목표 구조•규모•비용•발주 방안을 구체화하는 **ISMP** 실행 계획
...
```text
[사업 범위]---[현황•요구]---[목표 모델]---[규모•비용]---[발주 계획]
```

선의 의미: 각 선은 사업 경계, 근거 있는 요구, 목표 구조, 총사업비와 발주 기준이 서로 추적 가능하게 연결되는 관계를 나타낸다.
...
- **1. 현황•요구 자료 요청**: 업무•데이터•운영 문제와 업무량•품질의 객관적 근거를 수집하는 단계이다.
- **2. 목표 모델 설계**: 미래 구조와 상세 요구를 양방향으로 연결하는 단계이다.
- **4. 로드맵•발주안**: 일정•제안요청서•평가•인수 기준을 같은 사업 범위로 제시하는 단계이다.
...
sequenceDiagram
    ...
    participant D as 목표 모델 저장소
    participant C as 규모•비용 분석기
...
2. **목표 모델 설계**: 미래 구조와 상세 요구 연결
3. **규모•비용 분석**: 기능•전환 규모와 총사업비 산정
