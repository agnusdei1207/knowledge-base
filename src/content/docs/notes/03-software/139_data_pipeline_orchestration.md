...
date: "2026-07-30T23:55:57+09:00"
tags:
  - "notes-software"
weight: 139
extra:
...
- 정의/개념: 데이터 작업의 일정·의존성·재시도를 조정하는 **파이프라인 오케스트레이션**
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 태스크를 재시도하는 경우 | 데이터 구간·업무 키로 **멱등성** 확보 | 중복 적재 방지 |
| 숨은 선행 조건이 있는 경우 | **센서·데이터 계약**으로 명시 | 불필요한 대기 제거 |
| 과거 구간을 백필하는 경우 | 별도 **Pool·동시성** 적용 | 현재 작업 부하 보호 |
| DAG 파싱이 느려지는 경우 | 가벼운 최상위 코드와 **버전 번들** 사용 | 파싱 부하·버전 차이 감소 |
| 상태 DB가 비대해지는 경우 | **보존 기간·태스크 크기** 조정 | 상태 처리 병목 억제 |
