...
- **방향성 비순환 그래프(Directed Acyclic Graph, DAG) 번들•처리기**: 워크플로 정의와 코드를 파싱•직렬화해 실행 가능한 판본으로 저장하는 구성요소이다.
...
- **1. 방향성 비순환 그래프(Directed Acyclic Graph, DAG) 파싱•직렬화 저장**: 일정•태스크•의존성을 읽고 실행 시스템이 사용할 형태로 등록하는 단계이다.
- **2. DAG 실행(DAG Run)•실행 조건 평가**: 일정 또는 백필별 실행 단위를 만들고 선행 상태와 동시성 제한으로 실행 가능성을 판정하는 단계이다.
- **3. 준비 태스크•버전 제출**: 실행할 코드•데이터 구간•시도 번호를 고정해 작업자에게 전달하는 단계이다.
- **4. 실행 상태•증거 보고**: 시작•완료•실패 상태와 로그•산출물 위치를 인증된 상태 장부에 기록하는 단계이다.
...
- **크론(Cron) 중심 실행**: 작업 간 상태 의존성을 별도 관리하지 않고 지정 시각에 독립 명령을 실행하는 방식이다.
...
- **센서•데이터 계약**: 외부 데이터의 도착 여부를 감지하고 입력의 구조•품질•제공 조건을 명시해 숨은 선행 조건을 드러내는 수단이다.
- **방향성 비순환 그래프(Directed Acyclic Graph, DAG) 파싱**: 워크플로 정의 코드를 읽어 태스크와 의존 관계를 해석하는 처리이다.
- **메타데이터 데이터베이스(Metadata Database, Metadata DB)**: 워크플로와 태스크의 실행 상태•이력을 저장하는 상태 장부이다.
