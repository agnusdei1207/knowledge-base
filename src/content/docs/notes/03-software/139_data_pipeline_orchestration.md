...
- 배경/필요성: 단순 시간 기반 Linux Crontab 스케줄링 시 발생하는 **선행 작업 완료 감지 불가, 실패 시 재시도 부재 및 과거 대량 데이터 소급 백필 불가 해결 불가**

#### 한줄 요약
- Python DAG 코드로 작업 의존성과 자동 재시도, 백필을 완벽히 통제한다.
...
#### 한줄 요약
- DagRun 생성 → 의존성 검증 → Task 큐잉 → Worker 실행 및 상태 기록 → 후속 작업 트리거 순으로 진행된다.
