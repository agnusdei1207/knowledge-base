...
- 정의/개념: 데이터 작업의 일정·의존성·재시도를 조정하는 **실행 제어**
- 배경/필요성: 복합 작업의 **순서·실패 복구·동시 실행** 통합 관리
...
- **2. DAG Run 생성**: 일정·수동 요청·백필 구간을 실행 단위로 생성
- **3. 선행 상태·동시성 제공**: 실행 가능 조건을 전달
- **4. 준비 태스크·버전 제출**: 작업자에게 실행 대상을 전달
- **5. 실행 상태 보고**: 시작·완료·실패를 API 서버에 통지
- **6. 상태·실행 증거 반영**: 인증된 결과를 장부에 저장
- **7. 후속·재시도 태스크 제출**: 갱신 상태로 다음 실행을 정
...
- **의존성·태스크 멱등성·Pool·백필 격리**로 데이터 파이프라인을 조정
