...
- 배경/필요성: 시간만 보고 실행하는 스케줄러는 선행 작업의 성공 여부를 모른 채 다음 작업을 시작해 실패가 조용히 하류로 번지고 재시도와 소급 실행도 사람이 매번 수동으로 감당해야 하므로, 작업 간 의존 관계를 DAG 코드로 선언해 실행 조건과 복구 판단을 스케줄러가 대신하게 할 필요
