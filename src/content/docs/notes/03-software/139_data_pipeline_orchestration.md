...
- 배경/필요성: Cron만으로는 복합 작업의 **의존 상태•실패 복구 관리하기 어렵다**.
...
- **워크플로 코드화**: DAG로 태스크•의존성•일정을 선언이다.
...
- **1. DAG 파싱•직렬화 저장**: 일정•태스크•의존성 등록한다.
- **2. DAG Run•실행 조건 평가**: 선행 상태•동시성으로 준비 여부를 판정한다.
- **3. 준비 태스크•버전 제출**: 코드•데이터 구간•시도 번호 고정이다.
- **4. 실행 상태•증거 보고**: 상태•로그•산출 위치를 장부에 반영한다.
- **5. 후속•재시도 태스크 결정**: 의존 상태•재시도 정책으로 다음 실행을 선택한다.
...
- 복합 의존•백필은 **Airflow**, 독립 시간 작업은 **Cron**을 선택한다.
