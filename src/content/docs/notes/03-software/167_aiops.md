...
선의 의미: 수집•정규화와 토폴로지•변경 저장소에서 분석 엔진으로 모이는 선은 통일된 운영 신호와 서비스 의존성•변경 이력의 결합 관계, 분석 엔진과 사건 관리의 선은 이상•상관•원인 후보와 대응 우선순위 연결 관계, 사건 관리와 런북•모델 관리의 선은 승인 대응 실행•결과 피드백•모델 검증 관계를 뜻한다.
