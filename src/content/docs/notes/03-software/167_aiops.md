...
- **신뢰도•조치 위험**: 자동화 수준은 분석 결과의 신뢰도와 조치 실패 시 발생할 위험을 기준으로 단계적으로 정한다.
- **다중 운영 신호•공통 문맥**: 로그•지표•추적•변경 사건을 시간•자원 식별자•서비스 관계로 정렬한 분석 입력이다.
- **이상•상관•토폴로지**: 정상 패턴 이탈, 신호 간 동시 변화, 서비스 의존 관계를 결합해 원인 후보를 줄이는 분석 기준이다.
...
- **수집•정규화**: 신호 형식•시간•자원 식별자를 공통 기준으로 통일하는 구성요소이다.
- **토폴로지•변경 저장소**: 서비스 의존 관계와 배포•설정 변경 이력을 보존하는 구성요소이다.
- **사건 관리**: 사용자 영향과 분석 신뢰도로 사건의 대응 우선순위를 결정하는 구성요소이다.
- **런북•모델 관리**: 승인된 대응 절차를 실행하고 결과 피드백과 모델 판본을 관리하는 구성요소이다.
...
- **인공지능 기반 정보기술 운영(Artificial Intelligence for IT Operations, AIOps)**: 학습 패턴•이벤트 상관•토폴로지를 이용해 대량 운영 신호에서 낯선 관계를 탐색하는 방식이다.
...
- **영향•신뢰도•가역성**: AIOps의 추천, 승인, 자동 실행 수준은 사용자 영향, 분석 신뢰도, 조치의 가역성을 기준으로 결정한다.
