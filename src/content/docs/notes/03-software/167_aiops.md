...
- 정의/개념: 운영 신호를 분석해 **이상•사건•대응 후보를 도출하는 운영 지능화 체계**이다.
- 배경/필요성: 개별 임계치•수동 분석으로는 **경보 폭주의 공통 원인 식별 지연**이 발생한다.
...
- **이상 탐지**: 정상 패턴에서 벗어난 운영 신호를 찾는 분석한다.
- **상관 분석**: 여러 신호의 동시 변화 관계를 찾는 분석한다.
- **토폴로지 분석**: 서비스 의존 관계로 원인 후보를 줄이는 분석한다.

</details>

- **다중 운영 신호•공통 문맥** 기반을 분석한다.
- **이상•상관•토폴로지** 기반 원인 후보 압축이 핵심이다.
- **신뢰도•조치 위험** 기반 단계 자동화가 핵심이다.
...
1. **운영 신호•변경 수집**: 관측 자료와 변경 사건을 집계한다.
2. **공통 문맥 정규화**: 시간•자원•토폴로지를 정렬한다.
3. **사건•원인 후보 압축**: 이상 상관과 원인을 순위화한다.
4. **위험별 런북 선택•승인**: 신뢰도•위험도 기반을 통제한다.
5. **가역 조치 실행•효과 검증**: 복구 확인과 지식을 환류한다.
...
- **영향•신뢰도•가역성**으로 추천•승인•자동 실행을 결정한다.
