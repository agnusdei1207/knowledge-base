...
- 정의/개념: 운영 신호에서 이상•사건•대응 후보를 도출하는 **인공지능 기반 정보기술 운영(Artificial Intelligence for IT Operations, AIOps)**이다.
- 배경/필요성: 개별 임계치•수동 분석으로는 경보 폭주의 공통 원인 식별이 지연된다.
...
- **이상 탐지**: 정상 패턴에서 벗어난 운영 신호를 찾는 분석 기법이다.
- **상관 분석**: 여러 신호의 동시 변화 관계를 찾는 분석 기법이다.
- **토폴로지 분석**: 서비스 의존 관계로 원인 후보를 줄이는 분석 기법이다.

</details>

- **다중 운영 신호**와 **공통 문맥**을 기반으로 분석한다.
- **이상 탐지**•**상관 분석**•**토폴로지 분석** 기반 원인 후보 압축이 핵심이다.
- **분석 신뢰도**와 **조치 위험** 기반 단계 자동화가 핵심이다.
...
- **분석 엔진**: 정규화된 신호와 토폴로지•변경 정보를 결합해 이상, 사건, 원인 후보를 산출하는 구성요소이다.
...
| 구성요소 | 책임 |
|:---|:---|
| 수집•정규화 | **수집 계층**이 신호를 가져오고 **정규화 계층**이 형식•시간•식별자를 통일 |
| 토폴로지•변경 저장소 | **토폴로지 저장소**가 의존성을, **변경 저장소**가 변경 이력을 제공 |
| 분석 엔진 | **분석 엔진**이 이상•상관•원인 후보를 산출 |
| 사건 관리 | **사건 관리**가 영향•신뢰도•대응 우선순위를 결정 |
| 런북•모델 관리 | **런북 관리**가 대응을 실행하고 **모델 관리**가 결과를 검증•환류 |
...
- **운영 신호•변경 수집**: 관측 자료와 변경 사건을 모으는 단계이다.
- **공통 문맥 정규화**: 시간•자원•토폴로지를 정렬하는 단계이다.
- **사건•원인 후보 압축**: 이상 상관과 원인 순위를 만드는 단계이다.
- **위험별 런북 선택•승인**: 신뢰도와 위험도로 실행 수준을 정하는 단계이다.
- **가역 조치 실행•효과 검증**: 복구 결과와 부작용을 확인하는 단계이다.
...
1. **운영 신호•변경 수집**: **운영 신호•변경 수집**으로 관측 자료와 변경 사건을 집계한다.
2. **공통 문맥 정규화**: **공통 문맥 정규화**로 시간•자원•토폴로지를 정렬한다.
3. **사건•원인 후보 압축**: **사건•원인 후보 압축**으로 이상 상관과 원인을 순위화한다.
4. **위험별 런북 선택•승인**: **위험별 런북 선택•승인**으로 신뢰도•위험도에 따른 실행 수준을 통제한다.
5. **가역 조치 실행•효과 검증**: **가역 조치 실행•효과 검증**으로 복구를 확인하고 지식을 환류한다.
...
| 운영 분석 방식 | **규칙 기반 분석(Rule-Based Analysis)** | **학습 기반 분석** |
|:---|:---|:---|
...
- 규칙은 이미 아는 온도선을 넘었는지 찾고 인공지능 기반 정보기술 운영는 여러 신호가 함께 변한 패턴과 최근 변경을 조합해 낯선 사건을 찾는다.
...
- **라벨 품질 검사**: 수집된 운영 데이터 판정값이 정답 레이블로 유효한지 다중 검토와 일관성 교차 검증으로 확정하는 활동이다.

</details>

| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 시간•식별자의 데이터 불일치 | 스키마 검증•시간•**자원 식별자(Resource Identifier, Resource ID)** 통일 | 잘못된 사건 상관 감소 |
...
- **자동화 수준**은 영향•신뢰도•가역성으로 추천•승인•자동 실행을 결정한다.

#### 한줄 요약

- 인공지능 기반 정보기술 운영는 증거가 있는 원인 후보를 압축하고 고신뢰•저위험•가역 조치만 제한적으로 자동 실행하며 결과를 다시 검증해야 한다.
