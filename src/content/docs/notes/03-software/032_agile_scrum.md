...
- **투명성·점검·적응**: 실제 상태를 공개하고 결과를 살핀 뒤 계획과 행동을 바꾸는 스크럼의 경험주의 기둥이다.
- **스프린트(Sprint)**: 고정된 짧은 기간에 목표와 사용 가능한 제품 증분을 완성하는 반복 주기다.
- **교차기능·자율관리(Cross-functional·Self-managing)**: 팀이 필요한 기술을 갖추고 누가 무엇을 어떻게 할지 스스로 결정하는 성질이다.
...
- **제품·스프린트 백로그(Product·Sprint Backlog)**: 장기 제품 작업과 현재 스프린트 목표·계획을 각각 관리하는 목록이다.
...
- **리뷰 결정·회고 행동**: 리뷰에서 제품·백로그 변경을 정하고 회고에서 팀 개선 과제와 책임자를 확정한 결과다.
