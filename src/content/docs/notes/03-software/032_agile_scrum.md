...
sequenceDiagram
    ...
    O->>B: ① 제품 목표·우선순위 항목 제시
    B->>D: ② 스프린트 목표·수행 계획 확정
    loop 스프린트
        D->>B: ③ 데일리 스크럼으로 진척·계획 조정
    end
    D->>S: ④ 완료 정의를 충족한 증분 리뷰
    S-->>O: ⑤ 결과·환경 변화 피드백
    D->>M: ⑥ 품질·효과성 개선 후보 공유
    M-->>D: ⑦ 다음 스프린트 개선 행동 합의
    O->>B: ⑧ 피드백으로 제품 백로그 재정렬
```
...
- ① 제품 책임자가 제품 목표와 우선순위 백로그 항목 제시
- ② 개발자가 선택 항목으로 스프린트 목표와 수행 계획 확정
- ③ 데일리 스크럼에서 목표 진척을 점검하고 다음 하루 계획 조정
- ④ 완료의 정의를 충족한 제품 증분을 이해관계자와 리뷰
- ⑤ 제품 결과·시장·환경 변화에 대한 피드백 수집
- ⑥ 회고에서 품질과 팀 효과성의 개선 후보 도출
- ⑦ 스크럼 마스터 지원 아래 다음 스프린트의 개선 행동 합의
- ⑧ 리뷰 피드백으로 제품 백로그 우선순위 재정렬
...
- 짧은 주기로 고객 가치를 검증하려면 **제품 목표·스프린트 목표·완료의 정의**를 점검하고 리뷰와 회고 결과로 백로그와 개발 방식을 계속 조정한다.
