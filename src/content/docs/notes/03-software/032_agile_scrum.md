...
- 정의/개념: 투명성•점검•적응으로 스프린트마다 **제품 증분**을 만드는 경량 프레임워크이다.
- 배경/필요성: 장기 계획은 잦은 요구 변화와 늦은 검증으로 **재작업이 증가한다**.

#### 한줄 요약

- 짧은 스프린트와 제품 증분 기반 **경험적 적응**이 핵심이다.
...
- **투명성•점검•적응**으로 실제 결과 기반 계획을 조정한다.
- **고정 길이 스프린트**마다 사용 가능한 제품 증분을 생성한다.
- **교차기능•자율관리 팀**이 제품 목표 달성 책임 공유가 핵심이다.

#### 한줄 요약

- 투명성•점검•적응으로 **제품•팀 효과성**을 개선한다.
...
- 책무•백로그•제품 증분의 **스크럼 정적 구조**가 핵심이다.
...
- **스프린트 계획(Sprint Planning)**: 제품 백로그에서 목표와 이번 반복의 실행 범위를 정하는 이벤트다.
- **스프린트 목표(Sprint Goal)**: 한 스프린트에서 달성할 단일 목적이자 작업 조정 기준이다.
- **일일 스크럼(Daily Scrum)**: 개발자가 목표 진행 상황을 점검하고 다음 작업 계획을 조정하는 이벤트다.
...
- **1. 스프린트 계획**: 목표와 반복 범위•실행 계획 확정이다.
- **2. 스프린트 실행**: 일일 점검과 통합•시험을 수행한다.
- **3. 완료의 정의 검증**: 증분의 공통 품질 기준을 확인한다.
- **4. 스프린트 리뷰**: 제품 결과와 다음 백로그를 조정한다.
- **5. 스프린트 회고**: 팀 효과성 개선 행동 확정이다.

#### 한줄 요약

- 계획•실행•리뷰•회고의 **스프린트 순환**이 핵심이다.
...
- 완료의 정의•리뷰 결정•**회고 행동**의 실행성을 확보한다.
...
- 스프린트 목표와 **완료의 정의**를 충족하면 백로그 재정렬, 미달하면 **기술 부채** 우선 해소가 핵심이다.

#### 한줄 요약

- 목표•완료 기준 충족 여부로 **백로그 우선순위 결정**이 핵심이다.
