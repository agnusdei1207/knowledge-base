...
- 배경/필요성: 부서별 데이터 정의•정책 불일치로 **의사결정 충돌•책임 공백**이 발생한다.
...
- **1. 중요 데이터•통제 목표 선정**: 가치•규제•위험별 통제 강도 확정이다.
- **2. 소유자•승인 책임 배정**: 결정권자•운영자•승인 범위 지정이다.
- **3. 정책•자동 통제 구현**: 품질•접근•보존 기준을 검사로 변환한다.
- **4. 위반•예외 증거 검토**: 실행•승인 이력으로 위험과 원인을 판정한다.
- **5. 시정 조치•재검사**: 담당자•기한 지정 후 통제 효과를 확인한다.
...
- **중요 데이터 요소(Critical Data Element, CDE)**: 업무•규제 영향이 커 우선 통제할 핵심 데이터이다.

</details>

- 업무•규제 영향이 큰 **CDE**부터 **소유자•통제•예외 만료** 지정이 핵심이다.
