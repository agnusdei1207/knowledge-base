...
- **정의/개념**: 데이터의 사용·보호·품질에 관한 **의사결정권과 책임 체계**이다.
- **배경/필요성**: 부서별 기준을 조정하고 **정책을 측정 가능한 통제**로 연결한다.
...
- **위험 우선순위**: 중요 데이터부터 통제 강도를 정한다.
- **역할 분리**: 소유자는 결정하고 스튜어드는 운영한다.
- **통제 측정**: 정책·표준·통제와 준수 증거를 연결한다.
...
- **1. 중요 데이터·통제 목표 승인**: 가치·규제·위험으로 대상을 정한다.
- **2. 품질·접근 책임 배정**: 결정 기준과 운영 역할을 지정한다.
- **3. 정책·검사·처리 흐름 구현**: 표준과 자동 통제를 적용한다.
- **4. 위반·예외 증거 제공**: 실행 결과와 변경 이력을 기록한다.
- **5. 위험·재발 분석 전달**: 미조치와 반복 원인을 알린다.
- **6. 시정 조치·기한 지정**: 담당자와 완료 기준을 확정한다.
- **7. 규칙 보정·재검사**: 조치 결과를 확인하고 예외를 종료한다.
...
- **데이터 중요도·역할 책임·통제 효과·예외 만료**로 데이터 거버넌스를 운영한다.
