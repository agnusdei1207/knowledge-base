...
- 정의/개념: 데이터의 정의•사용•보호•품질에 관한 의사결정권과 책임•정책•감독 절차를 정하는 **데이터 거버넌스(Data Governance)**이다.
- 배경/필요성: 부서별 데이터 정의•정책 불일치로 의사결정 충돌•책임 공백이 발생한다.
...
- 중요 데이터부터 통제 강도를 정하는 **위험 기반 우선순위**가 핵심이다.
- 소유자의 결정과 스튜어드의 운영을 나눈 **역할 분리**가 핵심이다.
- 정책•표준•통제와 준수 증거를 연결한 **통제 효과 측정**이 핵심이다.
...
선의 의미: 거버넌스 위원회 아래에는 데이터 소유자와 정책•표준•통제의 책임 축이 놓이고, 각 축 아래에는 운영 역할인 데이터 스튜어드와 실행 기반인 통제 플랫폼•처리 흐름이 결합되는 정적 거버넌스 구조를 뜻한다.

| 구성요소 | 책임 |
|:---|:---|
| 거버넌스 위원회 | **거버넌스 위원회**가 정책•통제 목표•중대 예외 승인 |
| 데이터 소유자 | **데이터 소유자**가 사용 목적•품질 기준•접근 우선순위 결정 |
| 데이터 스튜어드 | **데이터 스튜어드**가 정의•표준•품질 규칙•이슈 운영 |
| 정책•표준•통제 | **정책**을 **표준**과 **통제**로 구체화 |
| 통제 플랫폼•처리 흐름 | **통제 플랫폼**과 **처리 흐름**이 분류•품질•접근•예외•조치 증거 관리 |
...
- **중요 데이터•통제 목표 선정**: 가치•규제•위험에 따라 통제 대상과 강도를 정하는 단계이다.
- **소유자•승인 책임 배정**: 결정권자와 운영 담당자 및 승인 범위를 지정하는 단계이다.
- **정책•자동 통제 구현**: 품질•접근•보존 기준을 실행 가능한 검사로 바꾸는 단계이다.
- **위반•예외 증거 검토**: 실행 결과와 승인•변경 이력으로 위험과 원인을 판정하는 단계이다.
- **시정 조치•재검사**: 담당자•완료 기한을 지정하고 보정 통제의 효과를 확인하는 단계이다.
...
- **1. 중요 데이터•통제 목표 선정**: **중요 데이터•통제 목표 선정**은 가치•규제•위험별 통제 강도를 확정한다.
- **2. 소유자•승인 책임 배정**: **소유자•승인 책임 배정**은 결정권자•운영자•승인 범위를 지정한다.
- **3. 정책•자동 통제 구현**: **정책•자동 통제 구현**은 품질•접근•보존 기준을 검사로 변환한다.
- **4. 위반•예외 증거 검토**: **위반•예외 증거 검토**는 실행•승인 이력으로 위험과 원인을 판정한다.
- **5. 시정 조치•재검사**: **시정 조치•재검사**는 담당자•기한 지정 후 통제 효과를 확인한다.
...
|:---|:---|:---|
| 적용 기준 | **데이터 거버넌스**: 의사결정권•정책•책임 설정 | **데이터 관리**: 데이터 수명주기 작업 실행 |
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 모든 데이터에 같은 통제를 적용하면 통제 비용 분산 | **중요 데이터 요소(CDE)**•**위험 등급**별 차등 통제 | 고위험 데이터에 통제 자원 집중 |
| 승인 한도와 책임이 없으면 결정권 공백 발생 | **책임•최종 승인•자문•통보(RACI)**•**승인 한도**•**성과 책임** 명시 | 정책 결정의 책임 주체 확정 |
| 정책 문서와 시스템 검사가 다르면 준수 증거 단절 | **정책 추적성**•**통제 추적성**•**증거 추적성** 확보 | 문서와 실행 통제 일치 |
| 예외 종료일이 없으면 **정책 우회**가 영구화 | **보완 통제**•**예외 소유자**•**예외 만료** 관리 | 한시 예외의 자동 종료 보장 |
...
- **데이터 거버넌스 우선순위**: 업무•규제 영향과 통제 필요성을 비교하는 적용 기준이다.

</details>

- **데이터 거버넌스 우선순위**에 따라 업무•규제 영향이 큰 **중요 데이터 요소(CDE)**부터 **데이터 소유자**•**통제**•**예외 만료**를 지정한다.
