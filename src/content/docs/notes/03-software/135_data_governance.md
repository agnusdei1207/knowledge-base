...
- **정책•표준•통제**: 데이터 사용•보호 원칙을 구현 기준과 구체적인 준수 검사로 변환하는 구성요소이다.
...
- **통제 플랫폼•처리 흐름**: 분류•품질•접근•예외와 조치 증거를 시스템에서 관리하는 구성요소이다.
...
- **데이터 거버넌스**: 데이터 의사결정권과 정책•소유권•예외•통제 목표를 정하는 관리 영역이다.
...
- **중요 데이터 요소(Critical Data Element, CDE)•위험 등급**: 업무•규제 영향이 큰 핵심 데이터와 그 통제 강도를 정한 분류이다.
- **책임•최종 승인•자문•통보(Responsible, Accountable, Consulted, Informed, RACI)•승인 한도•성과 책임**: 의사결정과 실행의 역할 및 권한 범위를 명확히 하는 책임 체계이다.
- **정책•표준•통제•증거 추적성**: 문서의 원칙부터 구현 규칙과 실행 결과까지 연결하는 관계이다.
- **보완 통제•소유자•만료**: 한시 예외의 위험을 낮출 대체 수단과 책임자 및 자동 종료 시점을 명시한 정보이다.

</details>

| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 모든 데이터에 같은 통제를 적용하면 **통제 비용** 분산 | **중요 데이터 요소(Critical Data Element, CDE)•위험 등급**별 차등 통제 | 고위험 데이터에 **통제 자원** 집중 |
| 승인 한도와 책임이 없으면 **결정권 공백** 발생 | **책임•최종 승인•자문•통보(Responsible, Accountable, Consulted, Informed, RACI)•승인 한도•성과 책임** 명시 | 정책 결정의 **책임 주체** 확정 |
...
- **중요 데이터 요소(Critical Data Element, CDE)**: 업무•규제 위험이 커서 소유자와 통제를 우선 지정해야 하는 데이터 요소이다.

</details>

- 소유자•통제•예외 만료를 우선 지정할 **중요 데이터 요소(Critical Data Element, CDE)**
