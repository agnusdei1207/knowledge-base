...
- **1. 중요 데이터·통제 목표 승인**: 가치·규제·위험에 따라 대상과 소유·운영 책임 확정
- **2. 정책·자동 통제 구현**: 품질·접근·보존 기준을 검사와 처리 경로에 집행
- **3. 위반·예외 증거 제공**: 실행 결과·승인·변경 이력을 감사 가능한 형태로 기록
- **4. 시정 조치·기한 지정**: 위험·재발 원인에 따라 담당자와 종료 기준 배정
- **5. 규칙 보정·재검사**: 같은 통제로 조치 결과를 확인하고 예외 상태 종료
