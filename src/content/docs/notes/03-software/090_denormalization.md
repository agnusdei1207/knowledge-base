...
- **1. 트랜잭션으로 원본 갱신**: 기준 데이터를 변경
- **2. 변경 이벤트 전달**: 변경 사실을 통지
- **3. 최신 원본 확인**: 누락과 순서 역전을 차단
- **4. 기준 데이터 반환**: 복사에 필요한 값을 전달
- **5. 중복·요약 데이터 갱신**: 조회 모델에 반영
- **6. 최적화된 조회 요청**: 준비된 구조를 조회
- **7. 조회 결과·정합성 기록**: 결과와 지연을 기록
...
> 반정규화는 정규화를 포기하는 것이 아니라 측정된 읽기 병목에 관리 가능한 중복을 추가하는 구조
