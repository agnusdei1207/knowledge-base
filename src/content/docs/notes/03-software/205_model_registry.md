...
block
    columns 1
    A["버전 아티팩트 | 불변 파일·해시·모델 서명 보관"]
    B["계보 메타 | 코드·데이터·학습 관계 추적"]
    C["검증 증적 | 성능·무결성·출처 합격 근거"]
    D["승인 정책 | 단계 상태와 운영 별칭 전환 제어"]
    E["감사 로그 | 등록·승인·배포·복귀 이력 저장"]
    A --- B
    B --- C
    C --- D
    D --- E
...
1. **후보 버전 등록**: **해시·서명·계보**를 불변 등록
2. **평가·계보 검증**: **성능·무결성·출처** 합격 판정
3. **승인 상태 전환**: 승인자·근거와 **단계 상태 변경**
4. **승인 버전 배포**: 운영 별칭의 **승인 버전 반영**
