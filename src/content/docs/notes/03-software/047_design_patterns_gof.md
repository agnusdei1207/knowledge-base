...
- ① 호출부가 결제 컨텍스트에 요청과 전략 선택 정보 전달
- ② 컨텍스트가 구체 구현을 모르고 공통 전략 인터페이스 호출
- ③ 인터페이스가 선택된 결제 알고리즘 구현에 처리 위임
- ④ 구체 전략이 공통 계약에 맞는 결과 반환
- ⑤ 인터페이스가 구현 세부를 감춘 결과를 컨텍스트에 전달
- ⑥ 컨텍스트가 호출부에 최종 결제 응답
...
- 결제 수단은 전략 패턴으로 공통 호출 계약과 카드·계좌·간편결제 알고리즘을 분리한다.
...
- 반복 변경의 수정 범위가 실제로 줄 때만 최소한의 참여자와 협력으로 패턴을 적용한다.
