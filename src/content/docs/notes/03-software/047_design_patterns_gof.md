...
- 배경/필요성: 객체지향 시스템 구축 시 반복적으로 직면하는 객체 생성 의존성, 클래스/인터페이스 간의 강한 결합 및 런타임 알고리즘 변경 경직성 문제를 극복하고, 생성(5종)·구조(7종)·행위(11종)의 23가지 검증된 객체지향 설계 템플릿과 공통 어휘(Common Vocabulary)를 통해 **코드 재사용성, 개방 폐쇄 원칙(OCP) 기반의 변경 유연성 및 개발팀 간 아키텍처 소통 효율을 극대화**할 필요
