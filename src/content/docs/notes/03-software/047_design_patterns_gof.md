...
- **추상 팩토리·팩토리 메서드·빌더**: 관련 객체군 생성, 하위 타입별 생성, 복합 객체의 단계별 조립을 각각 분리하는 생성 패턴이다.
- **프로토타입·싱글턴**: 기존 객체 복제로 생성하거나 하나의 공유 인스턴스만 제공하는 생성 패턴이다.
- **어댑터·브리지·컴포지트·데코레이터**: 인터페이스 변환, 추상·구현 분리, 부분·전체 동일 취급, 객체별 기능 추가를 다루는 구조 패턴이다.
- **퍼사드·플라이웨이트·프록시**: 단순 통합 창구, 공유 객체로 메모리 절감, 대리 객체의 접근 제어를 제공하는 구조 패턴이다.
- **책임 연쇄·커맨드·인터프리터·이터레이터**: 처리자 연결, 요청 객체화, 문법 해석, 집합 순회를 분리하는 행위 패턴이다.
- **미디에이터·메멘토·옵서버·상태**: 객체 협력 중재, 상태 보존·복원, 변경 통지, 상태별 동작 전환을 다루는 행위 패턴이다.
- **전략·템플릿 메서드·비지터**: 알고리즘 교체, 알고리즘 골격 고정, 객체 구조와 새 연산 분리를 다루는 행위 패턴이다.
