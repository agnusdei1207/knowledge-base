...
- **벤치마크 테스트(Benchmark Test, BMT)**: 같은 조건에서 후보 제품을 직접 실행해 성능과 기능을 비교하는 시험이다.
- **이해충돌**: 평가위원의 사적 관계나 이익이 독립적인 판단에 영향을 줄 수 있는 상태다.

## Ⅰ. 개요

- **정의/개념**: 기술·수행 역량을 채점해 제안자 선정
- **배경/필요성**: 가격 평가 한계로 기술 수행 역량 검증 필요
...
- 정량 증거와 정성 판단을 함께 반영한다.
- 항목 가중치가 사업 우선순위를 점수화한다.
- 사전 척도 공개로 평가 예측성과 공정성 향상한다.
- 모호한 척도는 평가위원별 점수 편차 유발한다.
...
| 판단 기준 | 기술성 평가 | 벤치마크 테스트 | 가격 평가 |
|:---|:---|:---|:---|
| 핵심 특징 | 제안 기술·수행 역량을 종합 채점 | 동일 조건에서 제품을 직접 시험 | 입찰 금액의 경제성을 계산 |
| 적용 기준 | 사업 수행 가능성 판단 필요 | 제안만으로 성능 확인 불가 | 예산 내 가격 경쟁 필요 |
| 주요 위험 | 정성 점수의 주관 편차 | 시험 환경·자료의 후보별 편향 | 저가 경쟁으로 품질 저하 |
...
2. 데이터베이스 후보를 같은 부하로 시험

### 쉽게 이해하기 (학습용)

- 공공 정보시스템 사업은 인력·개발 방법·품질 계획을 같은 척도로 평가해 수행 가능한 업체를 선택한다.
- 데이터베이스 제품은 같은 장비와 요청량에서 처리량과 오류를 측정해 제안서의 성능 주장을 확인한다.
