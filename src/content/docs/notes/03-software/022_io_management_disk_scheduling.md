...
- 배경/필요성: 무작위 I/O 요청 인입 시 헤드의 잦은 왕복 이동으로 인한 **탐색 시간(Seek Time) 폭증 및 시스템 I/O 병목 해결 불가**
...
| 구성요소 | 책임 |
|:---|:---|
| **I/O 요청 큐** | 블록 I/O 요청을 버퍼링하고 스케줄링 알고리즘에 따라 재배치 |
| **블록 병합기 (Merging)** | 연속된 LBA 주소를 갖는 요청들을 단일 대형 트랜잭션으로 통합 |
| **엘리베이터 스케줄러** | 헤드 이동 경로(트랙 번호)에 따라 I/O 요청을 **C-LOOK/SCAN** 방식으로 정렬 |
| **blk-mq 멀티 큐** | 멀티코어 락 경합 없이 NVMe 수십만 IOPS를 하드웨어 큐로 직접 디스패치 |

#### 한줄 요약
- BIO 블록 계층, 병합기, 엘리베이터 정렬기, blk-mq 멀티 큐가 결합되어 동작한다.
...
#### 한줄 요약
- 요청 인입 → 인접 블록 병합 → 미디어별 정렬/멀티큐 분기 → 드라이버 디스패치 및 완료 인터럽트 순으로 실행된다.
...
| 알고리즘 | FCFS (선입선출) | SSTF (최단탐색거리) | SCAN / LOOK | C-LOOK (단방향 스캔) | none / mq-deadline |
|:---|:---|:---|:---|:---|:---|
| 헤드 이동 방식 | 도착 순서대로 무작위 | 현재 헤드 최단 거리 | 양방향 왕복 스캔 | **단방향 스캔 후 원점 점프** | **정렬 없음 (FIFO/기한 보장)** |
| 탐색 시간 효율 | 최악 (헤드 왕복 극심) | 우수하나 기아 발생 | 양호 (엘리베이터) | **최고 (균일한 대기시간)** | 탐색 시간 무관 (플래시) |
| 기아 현상 | 없음 | **양 끝단 기아 발생** | 없음 | **없음 (공정성 보장)** | 없음 (마감 기한 강제) |
| 주 적용 미디어 | 단순 임베디드 | 이론적 모델 | 레거시 HDD | **엔터프라이즈 HDD 표준** | **NVMe / SATA SSD 표준** |
...
#### 한줄 요약
- NVMe none 스케줄러, mq-deadline 읽기 우선 튜닝, bfq 공정 배분, iostat 모니터링으로 최적화한다.

## Ⅶ. 결론

- 회전형 HDD는 **C-LOOK 및 BFQ**, 탐색 지연이 없는 NVMe SSD는 **`none` 및 `mq-deadline`** 을 표준 구성하고, **blk-mq 아키텍처**를 연동하여 스토리지 성능 100% 달성
