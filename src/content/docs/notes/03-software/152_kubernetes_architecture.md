...
- 수평적 파드 확장(HPA) 및 무중단 롤링 업데이트를 지원하는 **고가용성 오케스트레이션**
...
- **쿠버네티스 아키텍처**는 컨테이너 기반 클라우드 네이티브의 핵심 인프라 표준이며, 제어면의 고가용성(HA)과 워커 노드의 자원 격리를 철저히 보장하여 자가 치유 기반의 안정적인 대규모 분산 서비스를 완성해야 함
