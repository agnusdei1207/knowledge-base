...
- 배경/필요성: 수천 개 이상의 마이크로서비스 컨테이너가 분산 노드에 배치되는 대규모 클라우드 환경에서 수동 배포, 노드 장애 시 수동 재시작, 동적 IP 변경에 따른 네트워크 라우팅 붕괴 및 자원 단편화 문제를 해결하기 위해, 두뇌 역할을 하는 컨트롤 플레인(Control Plane)과 실행을 담당하는 워커 노드(Worker Node)로 분리하고 선언적 명세(YAML)와 끊임없는 조정 루프(Reconciliation Loop)를 수행하는 쿠버네티스(Kubernetes) 아키텍처를 도입하여 **선언적 자가 치유(Self-Healing), 무중단 자동 롤아웃 및 대규모 컨테이너 오케스트레이션 자동화를 실현**할 필요
