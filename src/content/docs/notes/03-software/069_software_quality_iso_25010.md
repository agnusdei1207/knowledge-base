...
- 정의/개념: **ISO/IEC 25010**은 ICT 제품 품질을 기능 적합성•성능 효율성•호환성 등 9개 특성과 하위 특성으로 분류한 국제표준이다.
- 배경/필요성: 조직별 품질 용어•기준 차이로 **제품 비교•수용 판정 불일치**가 발생한다.

#### 한줄 요약

- 9개 특성 기반 **제품 품질 공통 분류•평가**가 핵심이다.
...
- 제품 품질을 **계층형 특성•부특성**으로 구조화가 핵심이다.
- 요구•설계•시험에 **공통 품질 어휘**를 제공한다.
- 사용 맥락에 따라 **측정값•허용 기준** 별도 설정이 핵심이다.

#### 한줄 요약

- 공통 품질 어휘와 **맥락별 측정값•허용 기준**이 핵심이다.
...
- 맥락 위험을 **측정 가능한 품질 요구로 변환**이 핵심이다.
...
> 요약: 신규 제품은 2023판을 적용하고 이전 계약은 특성•측정값을 명시적으로 매핑이 핵심이다.

#### 한줄 요약

- 기존 계약은 명시 판본, 신규 제품은 **2023판**이 핵심이다.
...
- 실패 위험•측정 조건•**품질 절충 기반 기준화**가 핵심이다.
...
- 맥락•실패 위험•측정 가능성 기반 **품질 우선순위**가 핵심이다.
