...
- **제품 비교·수용 판정 불일치**: 조직마다 품질 용어와 기준이 달라 같은 제품을 다르게 평가하는 문제이다.
...
- **계층형 특성·부특성**: 제품 품질을 상위 관점과 측정 가능한 하위 요구로 구조화한 분류이다.
- **공통 품질 어휘**: 요구·설계·시험 참여자가 같은 의미로 사용하는 표준 품질 용어이다.
- **측정값·허용 기준**: 품질 충족 정도를 표현한 결과와 합격·불합격을 가르는 임계값이다.
...
- **허용 기준(Acceptance Threshold)**: 품질 측정값의 합격·불합격을 가르는 임계값이다.
...
- **ISO/IEC 25010:2023**: ICT·소프트웨어 제품 품질을 9개 특성과 부특성으로 정의한 현행 국제표준이다.
...
- **사용 맥락·실패 위험**: 핵심 품질 특성을 우선 선정하도록 사용자·환경·영향을 함께 본 기준이다.
- **측정법·조건·단위·임계값**: 정성적 품질 요구를 객관적인 합격 조건으로 바꾸는 요소이다.
- **품질 절충(Quality Trade-off)**: 한 품질 개선이 다른 품질 저하를 유발해 우선순위 합의가 필요한 관계이다.
- **계약 판본·전환 매핑**: 계약과 시험이 참조하는 표준 판본 및 구·신 특성의 대응 관계이다.
- **품질 우선순위·허용 손실**: 중요한 품질의 적용 순서와 한 품질을 개선할 때 다른 품질에서 받아들일 수 있는 저하 범위이다.
...
- **사용 맥락·실패 위험·측정 가능성**: 제품에 중요한 품질 요구를 선택하는 세 가지 판단 기준이다.
