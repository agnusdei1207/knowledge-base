...
- **정의/개념**: 생산자와 소비자가 구조·의미·품질·변경에 합의한 **기계 검증 규약**이다.
- **배경/필요성**: 일방적 변경으로 발생하는 **하류 파손과 조용한 오류**를 방지한다.
...
- **구문·의미 보장**: 구조와 업무 해석을 함께 명시한다.
- **품질 SLO**: 측정식·목표·관찰 구간을 정의한다.
- **변경 호환성**: 공지·유예·버전 전환 절차를 정한다.
...
block
    columns 1
    A["스키마·키 | 열·타입·필수 여부 정의"]
    B["의미 정의 | 용어·단위·코드 설명"]
    C["품질·SLO | 측정식·목표·조치 명시"]
    D["소유권·지원 | 생산·소비·대응 책임 배정"]
    E["버전·검증 레지스트리 | 호환·검사 결과 관리"]
```
...
sequenceDiagram
    ...
    P->>R: 1. 새 계약 후보 등록
    R->>C: 2. 기존 버전·호환 규칙 전달
    C-->>P: 3. 호환 결과·영향 소비자 반환
    P->>R: 4. 승인 계약 버전 게시
    P->>V: 5. 계약 버전·데이터 생산
    V->>R: 6. 구조·품질 검사 결과 기록
    V-->>U: 7. 통과 데이터·품질 상태 제공
```

**동작 원리**

- **1. 새 계약 후보 등록**: 구조·의미·SLO·적용 시점을 제출한다.
- **2. 기존 버전·호환 규칙 전달**: 소비자 의존성을 CI에 제공한다.
- **3. 호환 결과·영향 소비자 반환**: 파괴적 변경과 전환 대상을 찾는다.
- **4. 승인 계약 버전 게시**: 공지·유예 절차를 거쳐 확정한다.
- **5. 계약 버전·데이터 생산**: 실제 데이터에 적용 버전을 연결한다.
- **6. 구조·품질 검사 결과 기록**: 게시 경계에서 위반 증거를 저장한다.
- **7. 통과 데이터·품질 상태 제공**: 소비자에게 사용 근거를 전달한다.
...
- **소비자 영향·의미 변경·SLO 측정·게시 경계**로 데이터 계약을 집행한다.
