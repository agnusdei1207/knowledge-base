...
- **구문•의미 보장**: 열•타입 같은 데이터 구조와 업무 용어•단위•집계 기준을 함께 명시하는 특성이다.
...
- **스키마•키**: 열•타입•필수 여부와 레코드를 식별하는 키•형식을 정의하는 구성요소이다.
- **의미 정의**: 업무 용어•단위•코드값과 집계 기준을 설명하는 구성요소이다.
- **품질•서비스 수준 목표(Quality and Service Level Objective, Quality and SLO)**: 품질 측정식•분모•목표와 위반 시 조치를 명시하는 구성요소이다.
- **소유권•지원**: 생산자•소비자와 계약 승인•장애 대응 책임자를 정하는 구성요소이다.
...
- **1. 새 계약 후보 등록**: 구조•의미•서비스 수준 목표(Service Level Objective, SLO)•적용 시점을 계약 레지스트리에 제출하는 단계이다.
- **지속적 통합(Continuous Integration, CI) 검사**: 계약 변경을 반영하기 전에 호환성과 소비자 영향을 자동 검증하는 검사이다.
- **3. 승인 계약 버전 게시**: 소유자 승인과 공지•유예를 거친 계약 판본을 사용 가능 상태로 확정하는 단계이다.
- **4. 계약 버전 기반 생산•검증**: 실제 데이터에 계약 판본을 연결하고 게시 경계에서 구조•품질 위반을 차단하는 단계이다.
...
- **서비스 수준 목표(Service Level Objective, SLO)**: 데이터 품질•최신성 등에 대해 달성해야 할 수치와 관찰 구간을 정한 목표이다.
...
- **소비 의존성•계약 시험**: 어떤 소비자가 데이터에 의존하는지 추적하고 변경 후에도 합의한 처리가 유지되는지 검사하는 수단이다.
- **서비스 수준 목표(Service Level Objective, SLO) 측정**: 합의한 계산식과 관찰 구간으로 품질•최신성 목표 달성 여부를 판정하는 활동이다.
...
- **서비스 수준 목표(Service Level Objective, SLO)**: 데이터 품질이나 최신성에 대해 계약 당사자가 합의한 측정 가능한 운영 목표이다.
- **새 버전•유예 전환**: 파괴적 변경을 별도 계약 판본으로 게시하고 소비자에게 이전 기간을 제공하는 절차이다.
