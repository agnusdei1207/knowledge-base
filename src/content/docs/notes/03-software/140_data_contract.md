...
- **2. 기존 버전·호환 규칙 전달**: 소비자 의존성을 CI에 제공
- **3. 호환 결과·영향 소비자 반환**: 파괴적 변경과 전환 대상을 검색
- **4. 승인 계약 버전 게시**: 공지·유예 절차를 거쳐 확정
- **5. 계약 버전·데이터 생산**: 실제 데이터에 적용 버전을 연결
- **6. 구조·품질 검사 결과 기록**: 게시 경계에서 위반 증거를 저장
- **7. 통과 데이터·품질 상태 제공**: 소비자에게 사용 근거를 전달
