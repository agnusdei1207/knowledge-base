...
- 배경/필요성: 마이크로서비스 간 동기식 HTTP/REST 호출이 길어질수록 발생하는 호출자-피호출자 간의 강한 시공간적 결합도, 단일 서비스 장애가 전체 시스템으로 번지는 연쇄 다운(Cascading Failure) 및 스파이크 트래픽 유입 시의 시스템 병목을 극복하기 위해, 비즈니스 상태 변화를 불변의 이벤트(Event)로 발행하고 분산 메시지 브로커(Kafka, RabbitMQ)를 통해 다수의 소비자가 독립적으로 비동기 처리하는 이벤트 기반 아키텍처(EDA)를 도입하여 **서비스 간 결합도 최소화, 탄력적 트래픽 완충 및 최종 일관성(Eventual Consistency) 기반의 대규모 확장성을 달성**할 필요
