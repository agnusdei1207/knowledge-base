...
- **재전달•재생•최종 일관성**: 이벤트 방식은 재전달과 재생을 허용하고 각 서비스의 상태가 처리 후 최종 일관성에 도달하게 한다.
- **브로커 보존•구독**: 생산 시점과 소비 시점을 분리하도록 브로커가 이벤트를 저장하고 관심 소비자에게 전달하는 방식이다.
- **소비자 그룹•파티션**: 여러 소비자가 작업을 분담하고 업무 키별 순서를 유지하도록 이벤트 스트림을 나누는 확장 단위이다.
...
- **CDC•메시지 브로커**: CDC는 아웃박스 변경을 읽고 메시지 브로커는 이벤트를 보존해 토픽•파티션•그룹 기준으로 분배한다.
- **변경 데이터 캡처(Change Data Capture, CDC)**: 데이터베이스 변경 로그를 읽어 아웃박스의 미발행 이벤트를 추출하는 방식이다.
- **업무 데이터베이스•아웃박스(Outbox)**: 업무 상태와 발행할 이벤트를 하나의 트랜잭션으로 원자 기록하는 구성요소이다.
- **스키마 레지스트리(Schema Registry)**: 이벤트 스키마 판본과 생산자•소비자 간 호환성 규칙을 관리하는 구성요소이다.
- **데드 레터 큐(Dead Letter Queue, DLQ)•재생 체계**: 반복 실패 이벤트를 정상 흐름에서 격리하고 원인 교정 후 다시 처리하는 구성요소이다.
...
- **2. 아웃박스(Outbox) 변경 읽기**: 변경 데이터 캡처(Change Data Capture, CDC)로 아직 발행하지 않은 사건을 추출하는 단계이다.
...
- **업무 키 파티션•버전 검사**: 같은 개체의 이벤트를 한 파티션에 배치하고 이전 버전보다 오래된 변경을 거부하는 순서 통제이다.
- **데드 레터 큐(Dead Letter Queue, DLQ)**: 제한된 재시도 후에도 실패한 이벤트를 별도로 격리하는 저장 경로이다.
...
- **일관성 시점•중복 위험•재생 요구**: 동기 호출과 이벤트 방식은 필요한 일관성 시점, 중복 처리 위험, 이력 재생 요구를 기준으로 결정한다.
