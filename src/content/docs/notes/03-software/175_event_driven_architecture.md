...
- 정의/개념: 상태 변화를 비동기 발행•구독하는 **이벤트 기반 아키텍처**이다.
- 배경/필요성: 연쇄 동기 호출로는 **장애 격리•서비스별 독립 확장하기 어렵다**.
...
- **브로커 보존•구독** 기반 시간•위치 결합을 완화한다.
- **소비자 그룹•파티션** 기반 독립 확장이 핵심이다.
...
- **4. 멱등 상태 반영 요청**: 처리 식별자와 업무 상태를 함께 저장해 중복 효과를 막는 단계이다.
...
1. **업무•이벤트 저장 요청**: 단일 트랜잭션 기반 이중 쓰기를 방지한다.
2. **아웃박스 변경 읽기**: CDC 기반 미발행 사건을 추출한다.
3. **이벤트 전달**: 토픽•파티션•그룹 기준을 분배한다.
4. **멱등 상태 반영 요청**: 처리 식별자•업무 상태 원자를 저장한다.
5. **처리 확인 신호**: 성공 위치 진행•실패를 재전달한다.
...
- **일관성 시점•중복 위험•재생 요구**로 동기 호출•이벤트 방식을 결정한다.
