...
date: "2026-08-26T13:19:03+09:00"
tags:
  - "notes-software"
weight: 175
extra:
...
- 배경/필요성: 동기 연쇄 호출은 호출자가 피호출자의 가용성과 응답 시간을 그대로 떠안아 장애 하나가 전 구간 지연과 전파 비용으로 번지므로, 서비스 사이에 이벤트를 보관하는 브로커 계층을 두어 시간적 결합을 끊고 실패를 재처리 가능한 상태로 남길 필요
...
- 비동기 다중 구독은 **EDA**, 즉시 응답 의존은 **동기 API** 선택
