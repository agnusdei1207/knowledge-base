...
- 정의/개념: 상태 변화를 비동기 발행•구독하는 **이벤트 기반 아키텍처(Event-Driven Architecture, EDA)**이다.
- 배경/필요성: 연쇄 동기 호출로는 장애 격리와 서비스별 독립 확장이 어렵다.
...
- **브로커 보존**과 **구독** 기반으로 시간•위치 결합을 완화한다.
- **소비자 그룹**과 **파티션** 기반 독립 확장이 핵심이다.
- **재전달**•**재생**•**최종 일관성** 기반 비동기 운영이 핵심이다.
...
| 구성요소 | 책임 |
|:---|:---|
| 업무 데이터베이스•아웃박스 | **업무 데이터베이스**와 **아웃박스(Outbox)**가 업무 상태•이벤트를 원자 기록 |
| CDC•메시지 브로커 | **변경 데이터 캡처(Change Data Capture, CDC)**가 변경을 읽고 **메시지 브로커**가 이벤트를 보존•분배 |
| 스키마 레지스트리 | **스키마 레지스트리(Schema Registry)**가 스키마 판본•호환성을 검사 |
| 소비자•처리 데이터베이스 | 중복 확인•업무 상태 반영 |
| DLQ•재생 체계 | **데드 레터 큐(Dead Letter Queue, DLQ)**가 실패를 격리하고 **재생 체계**가 재처리 |
...
- **업무•이벤트 저장 요청**: 업무 상태와 아웃박스 이벤트를 단일 트랜잭션으로 기록해 이중 쓰기 불일치를 막는 단계이다.
- **아웃박스 변경 읽기**: CDC로 아직 발행하지 않은 사건을 추출하는 단계이다.
- **이벤트 전달**: 토픽•파티션•소비자 그룹 기준으로 사건을 대상 소비자에게 분배하는 단계이다.
- **멱등 상태 반영 요청**: 처리 식별자와 업무 상태를 함께 저장해 중복 효과를 막는 단계이다.
- **처리 확인 신호**: 성공한 소비 위치를 진행시키고 실패 이벤트는 재전달하도록 브로커에 결과를 알리는 단계이다.
...
4. 멱등 상태 반영 요청
      ...
      +-- 처리 실패 --> 제한 재전달
                              |
                              +-- 한도 이내 --> 3단계 재시도
                              +-- 한도 초과 --> DLQ 격리•교정 후 재생
```

### 동작 원리

1. **업무•이벤트 저장 요청**: **업무•이벤트 저장 요청**으로 단일 트랜잭션 기반 이중 쓰기를 방지한다.
2. **아웃박스 변경 읽기**: **아웃박스 변경 읽기**로 CDC 기반 미발행 사건을 추출한다.
3. **이벤트 전달**: **이벤트 전달**로 토픽•파티션•그룹 기준에 따라 분배한다.
4. **멱등 상태 반영 요청**: **멱등 상태 반영 요청**으로 처리 식별자•업무 상태를 원자 저장한다.
5. **처리 확인 신호**: **처리 확인 신호**로 성공 위치를 진행하고 실패를 재전달한다.
...
- **이벤트 소싱**: 모든 상태 변경을 순서대로 저장해 현재 상태나 과거 시점 상태를 재생하는 방식이다.
...
| 이벤트 방식 | **이벤트 알림(Event Notification)** | **상태 전송(Event-Carried State Transfer)** | **이벤트 소싱** |
|:---|:---|:---|:---|
...
- **이중 쓰기 불일치**: 업무 데이터 저장과 이벤트 발행 중 하나만 성공해 사건이 누락되거나 잘못 전달되는 문제이다.
...
- **이벤트 방식 선택 기준**: 일관성 도달 시점•중복 위험•재생 필요성을 비교하는 판단 축이다.

</details>

- **이벤트 방식 선택 기준**에서 일관성 도달 시점•중복 위험•**재생 요구**로 동기 호출•이벤트 방식을 결정한다.
