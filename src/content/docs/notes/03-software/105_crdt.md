...
- **1. 로컬 변경 적용**: 통신 없이 도달 가능한 복제본 갱신
- **2. 델타·연산 전파**: 중복·순서 변화에 견디는 변경 전달
- **3. 동시 상태 교환**: 서로 독립적으로 생긴 변경 수집
- **4. 결정적 병합**: 교환·결합·멱등 규칙으로 같은 상태 계산
- **5. 수렴 상태 검증**: 충분한 전파 뒤 복제본 값 일치 확인
