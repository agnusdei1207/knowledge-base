...
- **1. 로컬 변경 요청**: 도달 가능한 사본을 갱신한다.
- **2. 상태 델타·연산 전송**: 변경을 전파한다.
- **3. 변경 단위 전달**: 다른 사본으로 보낸다.
- **4. 동시 변경·상태 교환**: 양쪽 변경을 모은다.
- **5. 병합 상태 전달**: 규칙 결과를 공유한다.
- **6. 상태·버전 보고**: 한쪽 결과를 검증한다.
- **7. 상태·버전 보고**: 양쪽 수렴을 비교한다.
...
- **병합 의미·전달 조건·불변식**으로 CRDT 적용 범위를 정한다.
