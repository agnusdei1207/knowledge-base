...
- 배경/필요성: 오프라인 및 동시 편집 환경에서 중앙 합의(Raft/2PC) 강행 시 발생하는 **수정 지연, 네트워크 단절 시 편집 불가 및 락 경합 해결 불가**

#### 한줄 요약
- 중앙 조정 없이 로컬에서 즉시 수정하고 수학적 병합 규칙으로 동일 상태에 수렴한다.
...
|:---|:---|:---|
| **복제 상태 (Replica State)** | 노드별 로컬 데이터 변경 사항을 메모리에 보관하고 **즉시 UI 렌더링** | 네트워크 지연 0ms 체감 |
| **변경 메타데이터** | 각 요소마다 고유 식별자(UUID), **Lamport 타임스탬프, 노드 ID 부여** | 인과 관계 추적 및 충돌 판정 |
| **병합 함수 (Merge Function)**| 교환, 결합, 멱등 법칙을 만족하는 **Join-Semilattice 최소상한(LUB) 연산** | 순서 무관 결정적 동일 수렴 |
| **전파 계층 (Transport)** | 상태(State), 델타(Delta), 연산자(Op)를 **P2P 또는 브로커를 통해 전달** | 일시 단절 후 재연결 시 동기화 |

#### 한줄 요약
- 로컬 복제 상태, 메타데이터 태그, 병합 함수, 전파 계층이 결합된다.
...
사용자 A와 B가 오프라인 상태에서 각각 문서 편집 (동시 수정 발생)
        │
   1. [로컬 즉시 반영] 노드 A와 B가 락 대기 없이 로컬 CRDT 트리에 변경 즉시 커밋
        │
   2. [네트워크 재연결] 노드 A와 B가 온라인으로 복구되어 Yjs CRDT 변경 메시지 상호 교환
        │
   3. [병합 연산 실행] 양쪽 노드가 동일한 수학적 병합 함수($\sqcup$)를 각자 독립적으로 실행
        │
   4. [결정적 동일 수렴] 중앙 서버 중재 없이도 노드 A와 B의 문서 내용이 100% 일치하게 자동 완성
```

#### 한줄 요약
- 로컬 즉시 커밋 → 네트워크 재연결 → 상호 메시지 교환 → 결정적 병합 함수 실행 → 동일 수렴 순으로 진행된다.
...
- 실시간 협업 도구 및 로컬 퍼스트(Local-First) 소프트웨어 개발을 위해 **Yjs / Automerge 기반 CRDT 라이브러리를 표준 채택**하고, **Delta 기반 상태 동기화와 Tombstone GC 최적화**를 통해 고성능 분산 동시성 완성
