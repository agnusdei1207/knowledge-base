...
- **정의**: 합의 알고리즘(Paxos/Raft)이나 락(Lock) 없이 분산 노드가 독립적으로 데이터를 변경해도 수학적 연산 기반으로 100% 충돌 없이 결정적으로 수렴하는 자료구조인 **CRDT(Conflict-free Replicated Data Type)**.
- **필요성**: 오프라인 모드 수정 수용 및 분산 시스템의 락 제어로 인한 Latency/네트워크 단절 병목 극복.
...
- **CRDT 수립 기준 적용** (실시간 동시 협업 에디터/오프라인 DB 구축 시 `Yjs` / `Automerge` 기반 CRDT 자료구조 필수 수용)
