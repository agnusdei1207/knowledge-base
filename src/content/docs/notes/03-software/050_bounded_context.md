...
- 배경/필요성: 전사 단일 데이터 모델(Single Enterprise Model) 및 거대 공유 DB가 유발하는 동일 용어 간의 다의어/동음이의어 충돌(Semantic Collision), 속성이 수백 개에 달하는 거대 엔티티(Fat Entity) 부패 및 팀 간 배포 병목을 극복하고, 보편적 언어가 단일한 의미를 유지하는 명시적 도메인 경계(Bounded Context)와 컨텍스트 맵(Context Map)을 통해 **마이크로서비스(MSA) 독립 배포 경계 확립 및 콘웨이의 법칙(Conway's Law)에 부합하는 자율 조직 구조를 실현**할 필요
...
- 마이크로서비스 아키텍처(MSA) 분할 및 대규모 엔터프라이즈 도메인 거버넌스의 **표준 전략적 경계 분할 원칙**으로 확립되었으며, 실무 구축 시에는 **콘웨이의 법칙에 따른 1팀-1바운디드 컨텍스트 소유권 확립, 컨텍스트 간 결합도를 낮추는 OHS/PL 및 이벤트 기반 비동기 통신, 상류 변경의 오염을 차단하는 부패방지계층(ACL)**을 결합하여 자율성과 비즈니스 민첩성을 극대화
