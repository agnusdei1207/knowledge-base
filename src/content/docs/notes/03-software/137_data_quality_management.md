...
- **정의/개념**: 목적별 품질 규칙으로 위반을 측정·조치·재검증하는 **순환 관리 활동**이다.
- **배경/필요성**: 누락·오류·모순이 **보고서와 자동 처리로 전파**되는 것을 막는다.
...
- **목적 적합성**: 업무 영향에 따라 허용 수준을 정한다.
- **측정 가능성**: 대상·분모·임계값·버전을 명시한다.
- **폐쇄 순환**: 격리·원인 조치·재검증으로 종료한다.
...
block
    columns 1
    A["프로파일러 | 분포·누락·중복 측정"]
    B["규칙 저장소 | 조건·임계값·버전 관리"]
    C["검증 엔진 | 레코드·집합 규칙 실행"]
    D["품질 게이트 | 게시·격리·중단 판정"]
    E["품질 관제 | 원인·조치·재검증 추적"]
```
...
- **1. 품질 기준선 제공**: 분포·누락·중복 현황을 측정한다.
- **2. 규칙·임계값 승인**: 목적·위험과 책임자를 명시한다.
- **3. 버전 있는 규칙 전달**: 승인 조건을 검증 엔진에 보낸다.
- **4. 품질 판정 결과 전달**: 레코드 위반과 비율을 계산한다.
- **5. 격리·영향·위반 통지**: 오류 확산을 막고 근거를 알린다.
- **6. 수정 후 재검증 요청**: 원천이나 변환 규칙을 고친다.
- **7. 같은 규칙으로 재판정**: 수정 효과와 종료 여부를 확인한다.
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 목적·분모 | 모집단·제외 조건 명시 | 품질 비율 오판 방지 |
| 임계값 | 기준선·업무 손실·추세로 단계화 | 과잉 중단·오염 완화 |
| 정확성 기준 | 권위 원천·확인 시각·표본 감사 | 비교 기준 신뢰 확보 |
| 격리 | 레코드·집합 분리와 원본 보존 | 정상 데이터 영향 축소 |
| 재검증 | 규칙·데이터·코드 버전 비교 | 수정 효과 검증 |
...
- **업무 영향·품질 차원·규칙 버전·재검증 결과**로 데이터 품질을 관리한다.
