...
- **1. 품질 기준선 제공**: 분포·누락·중복 현황을 측정
- **2. 규칙·임계값 승인**: 목적·위험과 책임자를 명시
- **3. 버전 있는 규칙 전달**: 승인 조건을 검증 엔진에 전달
- **4. 품질 판정 결과 전달**: 레코드 위반과 비율을 계산
- **5. 격리·영향·위반 통지**: 오류 확산 차단과 근거 통지
- **6. 수정 후 재검증 요청**: 원천이나 변환 규칙을 수정
- **7. 같은 규칙으로 재판정**: 수정 효과와 종료 여부를 확인
...
- **업무 영향·품질 차원·규칙 버전·재검증 결과**로 데이터 품질을 관리
