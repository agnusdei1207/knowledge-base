...
- **정의/개념**: 상호 보유 자원을 기다려 정지한 상태
- **배경/필요성**: 순환 대기가 작업을 멈춰 교착 통제 필요
...
- 네 필요조건이 동시에 성립해야 발생한다
- 대기 순환의 프로세스는 모두 진행을 멈춘다
- 필요조건 하나를 제거하면 교착을 예방한다
- 탐지 후 종료·롤백은 수행 작업을 잃게 한다
...
| 판단 기준 | 예방 | 회피 | 탐지·복구 |
|:---|:---|:---|:---|
| 핵심 특징 | 필요조건 제거 | 안전 상태일 때만 할당 | 순환 탐지 후 종료·롤백 |
| 적용 기준 | 획득 순서·선점 규칙 강제 가능 | 최대 요구량 사전 파악 가능 | 발생이 드물고 롤백 가능 |
| 주요 위험 | 자원 이용률 저하 | 사전 정보·검사 비용 | 탐지 전 서비스 정지 |
...
2. 다중 잠금 서비스는 전역 잠금 획득 순서 고정

### 쉽게 이해하기 (학습용)

- 데이터베이스는 서로 기다리는 거래 중 하나를 이전 상태로 되돌려 나머지가 진행하게 한다.
- 여러 잠금을 쓰는 서비스는 모든 코드가 같은 순서로 잠금을 잡아 대기 관계가 원이 되지 않게 한다.

## Ⅶ. 결론

- 자원 순서·최대 요구량·롤백 비용으로 대응 선택
