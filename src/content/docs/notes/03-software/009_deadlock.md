...
- **로그(Log)**: 자원과 작업 상태의 변경 이력을 기록하는 자료
- **복구점(Recovery Point)**: 작업을 되돌릴 수 있는 이전 상태
...
선의 의미: 자원 관리자는 프로세스의 보유·대기 관계로 대기 그래프를 구성하고, 희생 선택기는 이 그래프와 로그·복구점을 바탕으로 복구 대상을 결정한다.
