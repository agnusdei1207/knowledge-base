...
- 배경/필요성: 표현•논리•저장을 결합하면 **구조 변경 영향**이 전체 응용으로 전파가 발생한다.
...
- **개념 질의•논리 연산 변환**: 외부 뷰를 공통 개념 연산으로 바꾸는 단계다.
- **물리 접근 요청**: 논리 연산을 현재 저장 경로에 대응시키는 단계다.
- **물리 레코드 반환**: 내부 저장 구조에서 조회 결과를 읽는 단계다.
- **외부 뷰 결과 복원**: 물리 레코드를 사용자별 표현으로 바꾸는 단계다.
...
- **1. 개념 질의•논리 연산 변환**: 외부 뷰를 공통 의미에 대응이다.
- **2. 물리 접근 요청**: 파일•인덱스•파티션 조회한다.
- **3. 물리 레코드 반환**: 내부 조회 결과 반환한다.
...
- 사용자 표현•업무 의미•저장 배치를 각각 **외부•개념•내부 스키마**에 배치한다.
