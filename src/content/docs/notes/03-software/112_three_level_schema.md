...
- 정의/개념: 표현•논리•저장을 분리한 **3단계 스키마** 추상화 모델이다.
- 배경/필요성: 표현•논리•저장을 결합하면 구조 변경 영향이 전체 응용으로 전파된다.
...
- **다중 외부**: 사용자별 표현•접근 범위를 제공한다.
- **단일 개념**: 전체 개체•관계•제약 통합한다.
- **단일 내부**: 파일•인덱스•파티션 배치를 정의한다.
...
선의 의미: 외부 스키마와 개념 스키마는 외부-개념 매핑으로 연결되고, 개념 스키마와 내부 스키마는 개념-내부 매핑으로 연결되어 표현·논리·저장 계층을 분리한다.

| 구성요소 | 책임 |
|:---|:---|
| 외부 스키마 | **외부 스키마**에 사용자별 뷰•접근 범위 정의 |
| 외부-개념 매핑 | **외부-개념 매핑**으로 외부 속성•공통 개념 연결 |
| 개념 스키마 | **개념 스키마**에 전체 개체•관계•제약 정의 |
| 개념-내부 매핑 | **개념-내부 매핑**으로 논리•물리 접근 경로 연결 |
| 내부 스키마 | **내부 스키마**에 파일•인덱스•파티션 배치 |
...
- **개념 질의•논리 연산 변환**: 외부 뷰를 공통 개념 연산으로 바꾸는 단계이다.
- **물리 접근 요청**: 논리 연산을 현재 저장 경로에 대응시키는 단계이다.
- **물리 레코드 반환**: 내부 저장 구조에서 조회 결과를 읽는 단계이다.
- **외부 뷰 결과 복원**: 물리 레코드를 사용자별 표현으로 바꾸는 단계이다.
...
- **2. 물리 접근 요청**: **물리 접근 요청**으로 파일•인덱스•파티션을 조회한다.
- **3. 물리 레코드 반환**: **물리 레코드 반환**으로 내부 조회 결과를 전달한다.
...
> 요약: 개념 스키마가 바뀌어도 외부 표현을 유지하는 **논리적 데이터 독립성**이 핵심이다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 응용마다 같은 용어의 의미가 다르면 개념 스키마 충돌 | **용어 사전**•**불변식**•**소유권** 정의 | 응용별 의미 왜곡 방지 |
...
| 외부 스키마가 민감 열을 그대로 노출할 위험 | **최소 권한**•**행 마스킹**•**열 마스킹** | 내부 데이터 노출 방지 |
...
- **3단계 스키마 적용 기준**: 사용자 표현과 공통 업무 의미 및 물리 저장 책임을 계층별로 배치하는 기준이다.

</details>

- **3단계 스키마 적용 기준**에 따라 사용자 표현•업무 의미•저장 배치를 **외부•개념•내부 스키마**에 배치한다.

#### 한줄 요약

- **3단계 스키마 적용 기준**은 외부 스키마•개념 스키마•내부 스키마를 분리하고 매핑으로 연결한다.
