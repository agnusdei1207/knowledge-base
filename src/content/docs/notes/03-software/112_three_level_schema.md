...
- 정의/개념: 사용자별 외부 스키마·통합 논리 개념 스키마·물리 저장 내부 스키마로 데이터베이스 구조를 분리한 **3단계 스키마(Three-Level Schema)** 추상화 모델
...
- **외부 스키마**: 사용자별 데이터 표현과 접근 계약을 여러 뷰로 정의하는 계층이다.
- **내부 스키마**: 파일·인덱스·파티션 같은 물리 저장 구조를 정의하는 계층이다.
...
> 요약: 개념 스키마가 소유하는 다중 외부 표현의 **공통 의미·무결성**
...
- **용어·불변식·소유권**: 개념 스키마의 공통 의미와 반드시 지킬 규칙 및 변경 책임자를 명시하는 기준이다.
- **뷰 카탈로그**: 외부 뷰의 소비자와 민감도, 소유자, 버전을 관리하는 목록이다.
- **계약 테스트·데이터 계보**: 매핑 변경 전후의 열·값 의미를 검증하고 원천부터 소비자까지 변환 경로를 추적하는 수단이다.
- **변경 계층·호환 버전**: 수정 대상을 한 스키마 계층으로 제한하고 구버전 소비자를 위한 호환 기간을 두는 기준이다.
- **최소 권한·행·열 마스킹**: 필요한 데이터만 허용하고 민감한 행과 열을 숨겨 외부 노출을 줄이는 통제이다.
