...
- **1. 외부 속성 질의**: 사용자 표현으로 요청한다.
- **2. 개념 속성 변환**: 공통 의미로 바꾼다.
- **3. 논리 연산 전달**: 내부 매핑으로 보낸다.
- **4. 파일·인덱스 접근**: 물리 경로를 실행한다.
- **5. 물리 레코드 반환**: 저장 결과를 돌려준다.
- **6. 개념 데이터 복원**: 논리 구조로 바꾼다.
- **7. 외부 결과 반환**: 사용자 표현으로 응답한다.
...
- **사용자 표현·공통 의미·저장 변화**로 3단계 책임을 나눈다.
