...
- **결과•결함 저장소**: 통과 증거와 실패의 재현 조건•영향•처리 상태를 요구와 연결해 보관하는 저장소이다.
...
- **독립 픽스처•격리 데이터**: 각 시험이 다른 시험의 상태에 영향받지 않도록 별도 초기 상태와 데이터를 준비하는 방식이다.
- **영향•위험 기반 선별**: 변경된 코드•계약과 결함 영향도를 분석해 먼저 실행할 회귀 시험을 고르는 방식이다.
