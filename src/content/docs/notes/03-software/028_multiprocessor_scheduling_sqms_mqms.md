...
- **정의/개념**: 복수 CPU 코어에 작업을 배치·이동
- **배경/필요성**: 큐 경합·편차가 처리량을 낮춰 균형 조정 필요
...
| 주요 위험 | 전역 락·캐시 라인 경합 | 큐 편차·작업 이동 비용 |
...
1. 소수 코어 SMP 시스템은 SQMS로 부하 균형 단순화
2. 병렬 런타임은 MQMS·작업 훔치기로 유휴 코어 감소
