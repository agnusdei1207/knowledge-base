...
- 정의/개념: 복수 CPU 코어에 **준비 작업을 배치•이동**하는 운영체제 스케줄링 정책이다.
- 배경/필요성: 전역 큐는 코어 증가 시 **잠금 경합**, 로컬 큐는 부하 편차가 증가한다.

#### 한줄 요약

- 전역 큐 경합과 로컬 큐 편차에 따른 **큐 구조 선택**이 핵심이다.
...
- **SQMS 전역 큐**의 작업 자연 분산과 경합 증가한다.
- **MQMS 로컬 큐**로 경합 감소•**캐시 지역성** 향상, 부하 편차 증가한다.
- **밀기 이동•작업 훔치기**의 편차 감소와 이동 비용이 핵심이다.

#### 한줄 요약

- SQMS 경합과 MQMS **지역성•부하 편차**의 절충이 핵심이다.
...
- **SQMS 전역 디스패치**: 모든 코어가 하나의 실행 큐에서 다음 작업을 선택하는 처리다.
- **전역 큐 경합**: 여러 코어가 같은 큐 잠금과 캐시 라인을 갱신하며 기다리는 상태다.
- **MQMS 로컬 디스패치**: 각 코어가 자신의 로컬 실행 큐에서 작업을 선택하는 처리다.
- **로컬 큐 편차 탐지**: 유휴 코어와 과부하 코어 사이의 큐 길이•부하 차이를 찾는 처리다.
- **밀기 이동•작업 훔치기**: 바쁜 큐가 작업을 보내거나 유휴 코어가 가져오는 부하 분산 처리다.
...
- SQMS는 공동 선택, MQMS는 **로컬 선택•작업 이동**이 핵심이다.
...
- 큐 경합•편차•이동 비용을 기준으로 **임계값 조정**이 핵심이다.
...
- 소수 코어•낮은 경합은 **SQMS**, 다수 코어•지역성 우선은 **MQMS**을 선택한다.

#### 한줄 요약

- 전역 경합과 로컬 편차 중 큰 비용을 줄이는 **큐 선택**이 핵심이다.
