...
- ① 원본 프로그램에 전체 테스트 스위트를 실행한다.
- ② 원본 테스트가 모두 통과해야 변이 평가를 시작한다.
- ③ 실행기가 변이 연산자를 적용하도록 생성기에 요청한다.
- ④ 생성기가 변경 하나만 포함한 독립 변이본을 만든다.
- ⑤ 원본과 같은 테스트 스위트를 각 변이본에 실행한다.
- ⑥ 변이본의 테스트 성공·실패 결과를 분류기에 전달한다.
- ⑦ 테스트가 실패하면 결함 탐지 성공인 사멸로 기록한다.
- ⑧ 테스트가 통과하면 등가 여부를 검토하고 비등가 생존 변이의 테스트를 보강한다.
...
| 고려사항 | 문제점 | 대책 |
|:---|:---|:---|
| 실행 비용 | 변이본마다 전체 테스트를 반복해 시간이 급증 | 변경 코드·고위험 모듈 중심 선택적 뮤테이션 적용 |
| 등가 변이 | 어떤 테스트도 사멸할 수 없어 점수 왜곡 | 자동 후보 제거와 사람 검토 후 점수 분모에서 제외 |
| 약한 검증식 | 코드 경로는 실행해도 결과 차이를 판정하지 못함 | 업무 결과·상태·상호작용의 구체적 검증식 추가 |
| 무의미한 변이 | 실제 결함과 동떨어진 변경이 보강 비용 증가 | 언어·도메인 결함에 맞는 대표 연산자 선택 |
| 점수 목표화 | 높은 점수 자체가 목적이 되어 저위험 테스트 남발 | 위험한 생존 변이와 결함 예방 효과 중심으로 평가 |

> 적용 사례: 요금 계산기는 반올림·한도 변이가 생존하면 경계 입력과 검증식을 보강한다.
...
- 뮤테이션 테스트는 인위적 결함을 기존 테스트가 실제로 탐지하는지 평가한다.
- 고위험 코드의 비등가 생존 변이에 집중해 입력과 검증식을 보강한다.
