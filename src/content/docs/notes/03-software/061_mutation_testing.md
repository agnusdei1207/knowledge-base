...
- **1. 원본 기준 실행**: 전체 테스트를 실행한다.
- **2. 기준 통과**: 원본의 정상 여부를 확인한다.
- **3. 변이 생성 요청**: 생성기에 변경을 요청한다.
- **4. 독립 변이본 생성**: 변경 하나만 적용한다.
- **5. 동일 테스트 실행**: 각 변이본을 검사한다.
- **6. 실행 결과 전달**: 성공·실패 결과를 보낸다.
- **7. 사멸·생존·등가 분류**: 결과와 원인을 판정한다.
...
- **결함 위험·실행 비용**을 보고 변이 시험 범위를 정한다.
