...
| 복구기 | 커밋 재생·미커밋 무시로 일관성 복구 |

**동작 원리**

- ① 선택한 데이터 모드의 변경 블록을 저널에 순차 기록
- ② 변경 블록의 지속화를 확인한 뒤 커밋 레코드 기록
- ③ 정상 실행은 체크포인트하고 장애 후 재마운트는 저널의 커밋 여부 검사
- ④ 커밋된 미반영 변경은 본래 블록에 재생하고 미커밋 변경은 무시
...
- 일반 파일 서버는 Ordered 모드로 데이터 블록을 먼저 기록하고 메타데이터 변경을 저널에 커밋해 쓰기량과 복구 범위를 절충한다.
...
- 데이터 보호 범위·쓰기량·복구시간을 기준으로 저널 모드를 선택한다.
