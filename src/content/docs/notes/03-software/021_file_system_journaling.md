...
- 정의/개념: 변경을 **저널**에 선기록하고 **커밋 단위로 복구** 하는 파일 시스템 복구 기법이다.
- 배경/필요성: 직접 갱신 중 전원 장애로 **메타데이터•데이터 불일치**가 발생한다.

#### 한줄 요약

- 변경 선기록과 커밋 기반 **파일 시스템 복구**가 핵심이다.
...
- **커밋 레코드**로 완료 변경만 재생하는 원자적을 복구한다.
- **순차 저널 기록**으로 전체 파일 시스템 검사 범위를 축소한다.
- **보호 범위**가 넓을수록 **쓰기 증폭** 증가한다.

#### 한줄 요약

- 보호 범위 확대에 따른 **복구성•쓰기 증폭** 절충이 핵심이다.
...
- **1. 트랜잭션 구성**: 함께 반영할 변경 묶음을 생성한다.
- **2. 변경 레코드 지속화**: 갱신 내용•대상 블록 선기록한다.
- **3. 커밋 레코드 지속화**: 복구 가능한 완료 경계 확정이다.
- **4. 체크포인트 반영**: 커밋 변경을 본래 블록에 기록한다.
- **5. 커밋 트랜잭션 재생**: 장애 후 미반영 변경 재적용한다.

#### 한줄 요약

- 커밋 전 선기록과 장애 후 **선별 재생** 흐름이 핵심이다.
...
- 보호 범위에 따라 **Writeback•Ordered•Journal**을 선택한다.
...
- **강제 동기화(Filesystem Synchronization, fsync)**: 응용이 파일 변경을 저장장치까지 확정하도록 요청하는 시스템 호출한다.
...
- **보호 범위•쓰기량•복구 시간**을 기준으로 **저널 모드**를 선택한다.

#### 한줄 요약

- 보호 범위•쓰기량•복구 시간에 맞는 **저널 모드 선택**이 핵심이다.
