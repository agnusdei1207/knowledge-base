...
- 정의/개념: 파일 시스템 변경 트랜잭션을 전용 공간(Journal Log)에 선기록(WAL)함으로써, 전원 차단 등 불의의 장애 수습 시 수초 내 일관성 복구를 보장하는 **파일 시스템 저널링(Journaling)**
- 배경/필요성: 대용량 디스크 환경에서 **fsck** 풀스캔으로 인한 서비스 중단시간(Downtime) 극복 및 불완전 덮어쓰기(Torn Write) 방지 요구성
...
- **Write-Ahead Logging(WAL)** 원칙 적용으로 장애 복구 타임 수 초 이내 단축
- 장애 시 디스크 전체 풀스캔 없이 **Journal Log**만 수거하여 $O(1)$ 속도 재현 (REPLAY)
...
|:---|:---|:---|:---|
| **Journal (Full)** | 데이터 + 메타데이터 모두 저널링 | 모든 파일 데이터와 메타데이터를 저널에 2번 기록 | **최고의 내구성** / 쓰기 오버헤드 2배 폭증 |
| **Ordered (기본값)** | 메타데이터만 저널링 (데이터 선기록) | 파일 데이터를 Home Block에 물리 기재 후 메타데이터 저널 커밋 | **성능 및 안전성 최적 균형** (Dirty Data 방지) |
| **Writeback** | 메타데이터만 저널링 (데이터 무순서) | 메타데이터 저널링과 데이터 디스크 기재를 무순서 비동기 처리 | **최고의 쓰기 속도** / 장애 시 파일 내용 오염 가능 |
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 디스크 컨트롤러 캐시 재정렬에 따른 저널 순서 붕괴 | **Write Barrier (`barrier=1`)** 커널 마운트 옵션 인가 | 락/저널 커밋 순서 보장 |
| Full Journaling 모드 사용 시 쓰기 속도 폭락 | **Ordered Mode (`data=ordered`)** 기본 모드 선택 | I/O 가용 스루풋 확보 |
...
- **저널링 모드 선택 기준**에 따라 일반 enterprise 인프라는 **Ordered Mode**, 금융/데이터 무결성 극대화는 **Full Journaling** 채택
