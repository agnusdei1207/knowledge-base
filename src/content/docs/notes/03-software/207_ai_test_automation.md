...
date: "2026-07-29T18:06:00+09:00"
tags: ["notes-software"]
weight: 207
extra:
  question_no: "207"
...
- 배경/필요성: 회귀 범위와 테스트 작성 비용 증가를 **AI로 보조**
...
block
    columns 1
    A["요구·코드·실행 이력 | 생성·선택의 근거와 변경 범위 제공"]
    B["테스트 생성기 | 입력·시나리오·스크립트 후보 생성"]
    C["회귀 선택기 | 변경 영향·실패 이력으로 실행 대상 선택"]
    D["오라클·검토 게이트 | 기대 결과·보안·중복·유효성 검증"]
    E["격리 실행기 | 승인 테스트를 격리 환경에서 실행"]
    A --- B
    B --- C
    C --- D
    D --- E
...
1. **변경·위험 전달**: 수정 코드·의존성·과거 실패 범위 식별
2. **테스트 후보 제출**: 생성 시나리오와 회귀 대상을 함께 제출
3. **승인 테스트 실행**: 오라클·보안 검토 후 격리 실행
4. **결과·실패 이력 전달**: 성공·실패·비결정 결과를 기록
...
- **오라클 신뢰·재현성·결함 탐지력**으로 AI 테스트 적용 결정
