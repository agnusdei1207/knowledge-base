...
- 배경/필요성: 분석과 서빙의 분리된 전처리 파이프라인으로 인한 **훈련-서빙 스큐(Skew), 피처 중복 개발 및 미래 데이터 누수(Data Leakage) 해결 불가**
...
#### 한줄 요약
- 피처 레지스트리, 변환 엔진, 오프라인 저장소, 온라인 저장소, 모니터링이 결합된다.
...
#### 한줄 요약
- 정의 등록 → 단일 변환 → 이중 적재 → PIT Join → 실시간 서빙 순으로 진행된다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 단순 DB 조인으로 미래 데이터가 학습에 유입되는 데이터 누수 | **Feature Store의 `Point-in-Time (PIT) Join` 기능 의무화** | 데이터 누수 에러 원천 차단 |
...
#### 한줄 요약
- PIT Join 적용, 단일 계산 로직 공유, 메타데이터 등록, 신선도 SLA 모니터링으로 운영한다.
