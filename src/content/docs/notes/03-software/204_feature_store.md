...
- 배경/필요성: 데이터 분석가와 백엔드 엔지니어가 피처(Feature) 계산 전처리 파이프라인을 Python과 Java/C++로 각각 중복 개발함에 따라 발생하는 심각한 중복 리소스 낭비, 학습 시와 실시간 추론 시의 피처 불일치로 인한 훈련-서빙 스큐(Training-Serving Skew), 과거 학습 데이터셋 구축 시 미래 시점 데이터가 유입되는 데이터 누수(Data Leakage) 문제를 해결하기 위해, 피처의 정의와 계산 로직을 중앙에서 1회 선언하고 대용량 시계열 학습용(Offline) 및 초저지연 실시간 추론용(Online)으로 일원화 공급하는 피처 스토어(Feature Store)를 도입하여 **피처 정합성 100% 보장, PIT(Point-in-Time) Join 기반 데이터 누수 방지 및 전사 피처 재사용성을 극대화**할 필요
