...
- **피처 정의·이력·최신값**: 피처 스토어는 모델이 사용할 피처 정의와 과거 이력, 실시간 최신값을 같은 기준으로 관리한다.
...
- **PIT 조인**: PIT 조인은 예측 시점 이전에 알 수 있던 데이터만 결합해 학습 자료에 미래 정보가 누수되는 것을 막는다.
...
- **온라인 저장소**: 온라인 저장소는 실시간 추론에 필요한 최신 피처를 짧은 지연으로 제공한다.
...
- **온라인 저장소**: 배치 학습은 오프라인 저장소의 시점별 이력을 사용하고 실시간 추론은 온라인 저장소의 최신값을 사용한다.
