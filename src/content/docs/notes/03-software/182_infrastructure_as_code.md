...
- 배경/필요성: 웹 콘솔 수동 조작(Click-Ops) 시 발생하는 **환경 간 불일치(Drift), 수동 인프라 재현 불가 및 휴먼 에러 장애 해결 불가**
...
#### 한줄 요약
- 코드 검증 → State 갱신 → Plan 생성 → 리뷰 승인 → Apply 및 장부 갱신 순으로 진행된다.
...
#### 한줄 요약
- 드리프트 감지, DynamoDB 락, 시크릿 암호화, 파괴 방지 라이프사이클로 운영한다.
