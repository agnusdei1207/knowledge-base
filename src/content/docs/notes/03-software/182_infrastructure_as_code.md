...
- **IaC(Infrastructure as Code)**: 클라우드 인프라를 수동 조작 대신 기계가 읽을 수 있는 **선언적 코드(Declarative Code)**로 정의·배포하는 자동화 관행.
...
- 코드 기반 상태 장부 관리 및 실제 자원과 연동한 변경 통제.
...
- 흐름: 코드/상태 대조 → 시뮬레이션(Plan) → API 호출 및 상태 업데이트.
