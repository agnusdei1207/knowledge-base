...
date: "2026-07-30T23:51:48+09:00"
tags:
  - "notes-software"
weight: 159
extra:
...
- **꼬리 지연(Tail Latency)**: 지연 시간 분포의 상위 백분위에 해당하는 느린 응답 시간
- **상태 기계(State Machine)**: 상태와 전이 규칙으로 장기 작업 단계를 조정하는 실행 모델
- **비밀 저장소(Secret Store)**: 암호·토큰·인증서 같은 민감 정보를 보호해 제공하는 저장소
- **관리형 컨테이너 서비스(Managed Container Service)**: 플랫폼이 기반 인프라를 관리하고 사용자가 컨테이너를 배포하는 서비스

## Ⅰ. 개요

- 정의/개념: 사건마다 함수를 할당·회수하는 **플랫폼 관리형 실행 모델**
