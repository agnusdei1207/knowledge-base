...
- **정의/개념**: 이벤트별 함수를 자동 배정·회수하는 실행 모델
- **배경/필요성**: 간헐 작업의 유휴 자원으로 요청 기반 실행 필요
...
| 비교축 | FaaS | 컨테이너 서비스 |
|:---|:---|:---|
| 핵심 특징 | 이벤트에 따라 회수되는 함수를 실행함 | 유지되는 장기 실행 서비스를 운영함 |
| 적용 기준 | 오토스케일링이 필요한 단발성 로직일 때 | 긴 실행 시간과 지속 트래픽 처리가 필요할 때 |
| 주요 위험 | 콜드 스타트·실행 한도·종속 | 유휴 비용·직접 운영 인력 |
...
2. 주문 함수는 이벤트 ID로 중복 결제 차단

### 쉽게 이해하기 (학습용)
- 이미지가 올라올 때만 함수를 실행해 작은 미리보기 파일을 만든다.
- 같은 주문 이벤트가 다시 와도 처리 기록을 확인해 결제는 한 번만 수행한다.
