...
- 배경/필요성: 데이터 레이크의 정합성 부재(트랜잭션 미지원 및 동시 쓰기 오염)와 데이터 웨어하우스(DW)의 고비용·독점 포맷 벤더 종속 및 비정형 데이터 수용 한계를 극복하고, 저비용 클라우드 객체 스토리지(S3) 상에 불변 Parquet 파일과 오픈 테이블 포맷(Delta Lake, Apache Iceberg)을 결합하여 **100% ACID 트랜잭션, 타임 트래블(Time Travel), 스키마 진화 및 다중 쿼리 엔진(Spark/Trino/BI)의 직접 접근을 단일 단일화된 저장소에서 달성**할 필요
...
- 현대 엔터프라이즈 데이터 엔지니어링 및 생성형 AI 인프라의 **가장 지배적인 차세대 통합 데이터 플랫폼 아키텍처**로 확립되었으며, 실무 구축 시에는 **오픈 테이블 포맷(Apache Iceberg/Delta Lake) 기반의 단일 데이터 원천(SSOT) 수립, 소형 파일 성능 저하를 방지하는 정기 `OPTIMIZE`(컴팩션)와 과거 스냅샷 비용을 통제하는 `VACUUM` 수명주기 거버넌스, 동시 쓰기 충돌을 방어하는 낙관적 동시성 제어(OCC) 튜닝**을 결합하여 고성능과 비용 효율성을 동시 보증
