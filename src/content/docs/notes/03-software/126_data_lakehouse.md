...
- 정의/개념: 객체 저장에 **ACID 테이블 관리**를 결합
- 기존 한계: 레이크·웨어하우스 분리는 **복제·정합성 비용** 증가
...
- **ACID 스냅숏 커밋**으로 일관된 파일 집합 제공
- **오픈 테이블 포맷**으로 다중 엔진 공유
- **파일 병합·스냅숏 만료**의 유지관리 요구
...
| 설계 요소 | 설명 |
|:---|:---|
| 객체 스토리지·데이터 파일 | 원천·분석 데이터를 **열 형식 저장** |
| 오픈 테이블 포맷 | **스냅숏·스키마·파일 참조** 관리 |
| 카탈로그 | 테이블명과 **메타데이터 위치** 제공 |
| 연산 엔진 | 동일 **테이블 스냅숏** 읽기·쓰기 |
| 거버넌스·유지관리 | **계보·병합·스냅숏 만료** 관리 |

> 요약: 포맷이 연산 엔진을 객체 스토리지의 스냅숏에 연결함

### 쉽게 이해하기 (학습용)
- 객체 스토리지가 파일을 보존하고 테이블 포맷과 카탈로그가 여러 처리 엔진에 같은 스냅숏을 제공한다
...
| 절차 | 설명 |
|:---|:---|
| 새 데이터 파일 작성 | 기존 파일 유지 후 **변경 결과 기록** |
| 새 스냅숏 생성 | 파일 집합·스키마를 **메타데이터화** |
| 기준 버전·충돌 검사 | 동시 변경의 **쓰기 충돌** 확인 |
| 메타데이터 포인터 교체 | 새 스냅숏을 **현재 버전으로 커밋** |
| 새 스냅숏 제공 | 독자에게 **동일 파일 집합** 제공 |

> 요약: 새 파일 생성 후 충돌 검사와 커밋으로 스냅숏을 전환함

### 쉽게 이해하기 (학습용)
- 새 파일과 장부를 만든 뒤 충돌이 없으면 스냅숏을 바꿈
...
| 핵심 특징 | 객체 파일·**ACID 스냅숏** | 원형 객체·**읽기 스키마** | 관리형 엔진·**쓰기 스키마** |
| 한계 | 파일·스냅숏 **유지관리 비용** | 데이터 늪·**동시 쓰기 충돌** | 저장 종속·**데이터 복제** |
...
- 데이터 레이크의 확장성과 웨어하우스의 정합성을 함께 확보하기 위해 **트랜잭션·스냅숏 격리·다중 엔진 호환·파일 유지관리**를 검토하고, 통합 분석 기반에 레이크하우스를 적용한다
