...
클라이언트가 레이크하우스 테이블에 쓰기 트랜잭션 요청 (`MERGE INTO`)
        │
   1. [스냅샷 조회] 쓰기 트랜잭션이 현재 테이블의 최신 스냅샷 버전(Version N) 확인
        │
   2. [불변 파일 기록] 변경/추가된 데이터를 새로운 불변 Parquet 파일로 S3에 기록
        │
   3. [동시 커밋 충돌 검증] 작업 도중 타 트랜잭션의 동일 파티션 변경 여부 OCC 검사
        │
   4. [신규 메타데이터 생성] 추가/삭제된 파일 목록을 담은 신규 스냅샷 메타데이터(N+1) 작성
        │
   5. 카탈로그의 최신 포인터를 N+1로 원자적(Atomic) 전환하여 즉시 공개
```
...
- 데이터 사일로와 이중 인프라 운영 비용을 근본적으로 제거하기 위해 **Delta Lake / Apache Iceberg 기반 데이터 레이크하우스를 전사 데이터 플랫폼 표준으로 도입**하고, **컴팩션과 진공 청소(Vacuum) 거버넌스**를 결합하여 차세대 단일 진실 공급원(SSOT) 완성
