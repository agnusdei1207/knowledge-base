...
- 정의/개념: 객체 저장소에 **ACID 메타데이터**를 결합한 **데이터 레이크하우스**이다.
- 배경/필요성: 레이크의 파일 단위 관리는 **동시 쓰기•일관된 테이블 공유** 제약이 발생한다.
...
- **1. 새 불변 데이터 파일 기록**: 기존 파일을 유지하고 변경 결과를 새 객체로 저장한다.
- **2. 새 스냅숏 메타데이터 구성**: 유효 파일 집합•스키마•통계 결합한다.
- **3. 기준 버전 비교•포인터 전환**: 충돌 검사 후 현재 참조를 원자를 전환한다.
