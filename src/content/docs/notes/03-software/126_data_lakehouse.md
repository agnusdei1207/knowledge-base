...
- 배경/필요성: Lake와 DW를 함께 운영하면 같은 데이터를 두 벌 보관하며 그 사이를 잇는 복제 파이프라인을 유지하는 비용이 반복되고 두 엔진의 지표마저 갈리므로, 객체 스토리지 위에 트랜잭션 메타데이터 계층을 얹어 한 벌의 데이터에 ACID와 SQL 분석을 동시에 성립시킬 필요
