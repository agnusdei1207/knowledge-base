...
- 정의/개념: 객체 저장에 **ACID 테이블 메타데이터 결합**
...
- **1. 새 데이터 파일 저장**: 기존 파일을 보존한다.
- **2. 파일 위치·버전 반환**: 새 파일을 식별한다.
- **3. 새 메타데이터 작성**: 파일 집합을 구성한다.
- **4. 커밋 요청**: 기준 버전과 함께 보낸다.
- **5. 충돌 검사·포인터 교체**: 원자 공개한다.
- **6. 커밋 결과 반환**: 충돌·성공을 알린다.
- **7. 현재 스냅숏 조회**: 일관된 파일을 읽는다.
...
- **트랜잭션 범위·엔진 호환·파일·스냅숏 수명**으로 Lakehouse를 설계한다.
