...
- 배경/필요성: 관계형 데이터베이스(RDBMS)에서 복합 계층 객체를 저장하기 위해 테이블을 과도하게 분해할 때 발생하는 다단계 조인(Join) I/O 병목과 스키마 변경 시의 무거운 DDL 마이그레이션 부담을 해소하고, JSON/BSON 형태의 가변 구조 문서를 단일 원자적 단위로 직접 영속화하며 WiredTiger 엔진 기반의 Replica Set(HA) 및 샤딩(Scale-Out)을 결합하여 **개발 생산성 극대화와 초고속 읽기/쓰기 수평 확장성을 실현**할 필요
