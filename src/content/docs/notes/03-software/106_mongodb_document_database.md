...
- **레플리카셋•샤딩**: 복제 묶음으로 고가용성을 제공하고 데이터를 여러 샤드로 나눠 수평 확장하는 기능이다.
...
- **레플리카셋(Replica Set)**: 연산 로그(Operation Log, oplog) 복제와 주 구성원 선출로 고가용성을 제공하는 구성요소이다.
- **바이너리 제이슨(Binary JSON, BSON) 문서•컬렉션**: 중첩 필드와 배열을 가진 문서와 그 문서를 모은 저장 단위이다.
...
- **3. 바이너리 제이슨(Binary JSON, BSON) 변경 레코드**: 주 구성원이 문서 단위 변경을 원자적으로 적용하는 단계이다.
- **1. 청크 메타데이터 요청**: 샤드 키가 속한 청크의 현재 배치 위치를 구성 서버에 묻는 단계이다.
- **2. 대상 샤드 위치**: 구성 서버가 반환한 담당 레플리카셋 주소이다.
- **4. 연산 로그(Operation Log, oplog) 항목**: 확정된 변경 내용과 순서를 보조 구성원에 전달하는 기록이다.
...
- **스키마 검증•버전 이관**: 가변 문서의 필수값•자료형을 검사하고 구조 변경 시 기존 문서를 단계적으로 변환하는 활동이다.
- **실제 질의•크기 기반 인덱스 선별**: 자주 쓰는 조건의 실행 계획과 인덱스 저장•쓰기 비용을 측정해 필요한 인덱스만 유지하는 원칙이다.
- **분포•지역성•포함률**: 샤드 키 값의 균등성, 함께 접근하는 데이터의 배치, 질의가 키를 포함하는 비율이다.
- **읽기 우선순위(Read Preference)•읽기 관심 수준(Read Concern)•쓰기 관심 수준(Write Concern)**: 읽을 구성원과 허용 최신성 및 쓰기 성공에 필요한 복제 확인 범위를 정하는 정책이다.
