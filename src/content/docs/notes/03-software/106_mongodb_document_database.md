...
- 정의/개념: 중첩 BSON 문서를 저장•조회•원자 갱신하는 **문서 데이터베이스**이다.
- 배경/필요성: 관계형 조인•다중 테이블 갱신은 **중첩 객체 처리 왕복**이 증가한다.
...
- **단일 원자성**: 한 문서 쓰기를 원자 확정이다.
- **분산 확장**: 레플리카셋•샤딩을 제공한다.
...
- **청크 메타데이터 요청**: 샤드 키의 현재 배치 위치를 묻는 단계다.
- **대상 샤드 위치 확인**: 담당 레플리카셋 주소를 확인하는 단계다.
- **BSON 문서 변경**: 주 구성원이 문서 변경을 원자 적용하는 단계다.
- **oplog 항목 전파**: 확정된 변경 순서를 보조 구성원에 보내는 단계다.
- **복제 확인**: 요구한 수의 구성원이 변경 수신을 알리는 단계다.
...
- **1. 청크 메타데이터 요청**: 샤드 키의 배치 위치 조회한다.
- **2. 대상 샤드 위치 확인**: 담당 레플리카셋 주소를 확인한다.
- **3. BSON 문서 변경**: 주 구성원이 문서 변경을 원자를 적용한다.
- **4. oplog 항목 전파**: 확정된 변경 순서를 전달한다.
- **5. 복제 확인**: 쓰기 확인 수 충족을 판정한다.
...
> 요약: 불필요한 분산 조정을 줄이도록 먼저 설계하는 **문서 경계**가 핵심이다.
...
| 읽기•쓰기 보장 미설정은 최신성•내구성 오해 유발 | **읽기 우선순위•관심 수준•쓰기 관심 수준** 명시 | **업무 보장 범위** 명확화 |
...
- 함께 읽고 유한하면 **내장**, 독립 수명주기•무한 성장은 **참조**를 선택한다.
