...
date: "2026-08-26T18:20:00+09:00"
tags:
  - "notes-software"
weight: 106
extra:
...
- 배경/필요성: 중첩 구조를 정규화해 여러 테이블로 쪼개면 늘 함께 읽히는 데이터를 조회마다 다시 결합하는 조인 비용이 반복되고 구조 변경도 전체 데이터에 걸친 마이그레이션 비용을 부르므로, 함께 쓰이는 데이터를 하나의 BSON 문서에 담아 조회를 단일 문서 접근으로 끝내고 변경을 문서 단위로 흡수할 필요
