...
- 배경/필요성: 복잡한 중첩 객체 처리 시 RDBMS의 정규화로 인한 **다단계 JOIN I/O 폭증, 빈번한 스키마 DDL 변경 비용 및 수평 확장 한계 해결 불가**

#### 한줄 요약
- BSON 문서 내포를 통해 조인 없이 중첩 데이터를 조회하고 복제 세트와 샤딩으로 가용성을 보장한다.
...
#### 한줄 요약
- 장애 발생 → 하트비트 감지 → Raft 투표 → Primary 승격 → 라우팅 갱신 순으로 진행된다.
