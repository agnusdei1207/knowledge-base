...
- 정의/개념: BSON 문서를 **저장·원자 갱신 단위로 사용**
...
- **1. 샤드 키·문서 쓰기 요청**: 문서를 보낸다.
- **2. 청크·샤드 위치 조회**: 배치표를 확인한다.
- **3. 대상 샤드 반환**: 저장 위치를 고른다.
- **4. 주 구성원에 쓰기 전달**: 원본에 요청한다.
- **5. oplog 변경 복제**: 보조 사본에 전파한다.
- **6. 쓰기 확인 반환**: 요구 수준을 확인한다.
- **7. 쓰기 결과 응답**: 응용에 결과를 보낸다.
...
- **접근 패턴·문서 성장·분산 키**로 내장과 참조를 정한다.
