...
- **1. 키·값·일관성 수준 전달**: 쓰기 조건을 보낸다.
- **2. 복제본 A 쓰기 전송**: 첫 담당 노드에 요청한다.
- **3. 복제본 B 쓰기 전송**: 다른 사본에도 요청한다.
- **4. 로컬 반영 확인**: 로그·메모리에 기록한다.
- **5. 로컬 반영 확인**: 두 번째 결과를 보낸다.
- **6. 요구 응답 수 충족**: 성공을 반환한다.
- **7. Memtable Flush**: 불변 파일로 내린다.
...
- **조회 패턴·파티션 크기·응답 수**로 Cassandra 키와 일관성을 정한다.
