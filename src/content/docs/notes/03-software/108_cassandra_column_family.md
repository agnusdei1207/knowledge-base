...
- 정의/개념: 와이드 컬럼 행을 피어에 분산•복제하는 **Cassandra 데이터베이스**이다.
- 배경/필요성: 단일 주 노드 구조는 쓰기 집중•장애 전환 중 **처리 중단**이 발생한다.
...
- **1. 쓰기 레코드 병렬 전파**: 담당 복제본에 변경을 전달한다.
- **2. 로컬 반영 확인 집계**: 로그•Memtable 기록 응답 집계한다.
- **3. SSTable 저장**: 메모리 한도 도달 시 불변 파일을 생성한다.
...
> 요약: $R+W>RF$ 조건과 함께 **타임스탬프•복제본 상태**를 검증한다.
...
- 조회 범위로 **파티션•클러스터링 키**, 장애 허용치로 **일관성 수준**을 결정한다.
