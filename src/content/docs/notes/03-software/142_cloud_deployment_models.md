...
- **정의/개념**: 자원의 소유·전용·환경 연결 방식에 따른 **클라우드 배포 분류**이다.
- **배경/필요성**: 데이터 위치·규제·지연·복구에 맞춰 **워크로드 배치**를 결정한다.
...
- **소유·전용 범위**: 공유 기반과 조직 전용 기반을 구분한다.
- **환경 조합**: 하이브리드는 환경, 멀티는 공급자를 조합한다.
- **공통 통제**: 신원·정책·연결·관측 기준을 통합한다.
...
block
    columns 1
    A["워크로드·실행 환경 | 배치 후보 결정"]
    B["연결 계층 | 경로·대역폭 관리"]
    C["공통 신원·정책 | 권한·암호화 기준 집행"]
    D["데이터 이동 계층 | 복제·정합성 관리"]
    E["통합 관측·비용 | 상태·감사·전송비 수집"]
```
...
- **1. 외부 API 요청**: 퍼블릭 환경의 업무를 호출한다.
- **2. 사용자·환경 권한 검증**: 목적과 접근 범위를 확인한다.
- **3. 제한 자격·정책 반환**: 최소 서비스 권한을 제공한다.
- **4. 전용 연결 처리 요청**: 프라이빗 환경의 기능을 호출한다.
- **5. 최소 결과·증거 반환**: 원문 노출 없이 결과를 전달한다.
- **6. 업무 응답**: 환경 간 결과를 조합해 반환한다.
- **7. 통합 지연·감사 기록**: 같은 요청 ID로 상태를 연결한다.
...
- **데이터 위치·통제 수준·연결 복구·운영 복잡도**로 클라우드 배포 모델을 선택한다.
