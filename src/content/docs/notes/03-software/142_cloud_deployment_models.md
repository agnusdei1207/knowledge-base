...
- 정의/개념: 자원의 소유·전용 범위와 환경 간 연결 방식에 따라 워크로드 실행 환경을 구분하는 **클라우드 배포 모델(Cloud Deployment Model)** 체계
...
- **워크로드·실행 환경**: 데이터 등급·지연·가용성 요구에 따라 애플리케이션을 배치하는 구성요소이다.
- **가상 사설망(Virtual Private Network, VPN)·게이트웨이**: 환경 간 통신을 암호화하고 허용된 경로로 중계하는 연결 구성요소이다.
- **공통 신원·정책**: 여러 환경의 사용자 신원과 최소 권한 정책을 일관되게 집행하는 구성요소이다.
- **통합 관측·비용**: 환경별 지표·로그와 데이터 전송 비용을 한 기준으로 수집·분석하는 구성요소이다.
...
- **연합 신원·단기 자격**: 공통 신원을 여러 환경에서 사용하고 필요한 시간 동안만 접근 권한을 발급하는 통제이다.
- **이중 경로·장애 전환**: 환경 간 연결 경로를 중복 구성하고 장애 시 대체 경로로 전환하는 가용성 대책이다.
- **기준 원천·복제 방향**: 최종 권위를 가진 데이터 위치와 변경 사항이 전파될 방향을 정한 규칙이다.
- **요청 식별자·시간 동기화**: 환경별 처리 기록을 하나의 요청으로 묶고 사건 순서를 같은 시간 기준으로 정렬하는 추적 수단이다.
...
- **멀티 클라우드**: 복수 공급자의 기능을 함께 사용하거나 특정 공급자 장애·종속 위험을 분산하는 전략이다.
