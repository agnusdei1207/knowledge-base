...
- 정의/개념: 소유•전용•연결 방식에 따른 **클라우드 배포 모델**이다.
- 배경/필요성: 단일 환경으로는 데이터 위치•규제•지연 요구를 동시에 충족하기 어렵다.
...
- **전용 범위**가 넓을수록 통제 수준과 구축 비용 증가한다.
- **환경•공급자 조합**에 따라 하이브리드와 멀티를 구분한다.
- 신원•정책•연결•관측의 **공통 통제**로 운영 일관성을 유지한다.
...
선의 의미: 워크로드•실행 환경에서 갈라지는 선은 배치된 업무가 환경 간 연결, 공통 신원•정책, 데이터 복제•동기화 통제를 함께 사용하는 관계를 뜻하며, 세 통제와 통합 관측•비용의 선은 환경별 지표•로그•전송 비용을 공통 기준으로 수집하는 관계를 뜻한다.

| 구성요소 | 책임 |
|:---|:---|
| 워크로드•실행 환경 | **워크로드**를 **실행 환경**에 데이터 등급•지연•가용성 기준으로 배치 |
| 연결 계층 | **가상 사설망(VPN)**과 **게이트웨이** 경로 관리 |
| 공통 신원•정책 | **공통 신원**과 **공통 정책** 기준 집행 |
| 데이터 이동 계층 | **데이터 이동 계층**이 복제•동기화•충돌•삭제 관리 |
| 통합 관측•비용 | **통합 관측**과 **통합 비용**으로 지표•로그•전송 비용 수집 |
...
- **탄력 확장•관리형 서비스**: 공유 자원의 신속한 확장과 공급자 기능을 우선하는 선택 축이다.
- **전용 통제•특수 장비**: 조직 전용 자원과 강한 물리•논리 통제를 우선하는 선택 축이다.
- **민감 자산•외부 확장 결합**: 전용 환경의 자산과 퍼블릭 확장을 함께 쓰는 선택 축이다.
- **복수 공급자 기능•위험 분산**: 둘 이상 공급자의 기능과 종속 위험을 분산하는 선택 축이다.
- **공통 운영 통제**: 여러 환경의 신원•정책•연결•관측을 일관되게 적용하는 기반이다.
...
- **공통 운영 통제**는 복수 환경의 신원•정책•연결•관측을 같은 기준으로 운영한다.
...
|:---|:---|:---|:---|:---|
| 적용 기준 | **퍼블릭 클라우드**: **탄력 확장•관리형 서비스** | **프라이빗 클라우드**: **전용 통제•특수 장비** | **하이브리드**: **민감 자산•외부 확장 결합** | **멀티 클라우드**: **복수 공급자 기능•위험 분산** |
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 배치 근거 없이 환경을 늘리면 비용•운영 복잡도 급증 | 데이터•지연•규제 기준으로 배치 평가 | 무질서한 워크로드 분산 방지 |
...
- **전용 통제**와 **탄력 확장**의 결합은 **하이브리드**, 공급자 분산은 **멀티 클라우드**를 선택한다.
