...
- **응용 프로그래밍 인터페이스(Application Programming Interface, API)**: 다른 서비스의 기능을 정해진 형식으로 호출하는 규약이다
- **가상 사설망(Virtual Private Network, VPN)**: 공용망 위에 암호화된 사설 통신 경로를 구성하는 기술이다
- **연합 신원(Federated Identity)**: 여러 환경이 하나의 신원 제공자를 신뢰해 사용자 인증 결과를 공유하는 체계이다
...
> **키워드:** 클라우드 배포 모델: 퍼블릭·프라이빗·하이브리드·멀티 (Cloud Deployment Models)
