...
- **1. 외부 API 요청**: 퍼블릭 환경의 업무를 호출
- **2. 사용자·환경 권한 검증**: 목적과 접근 범위를 확인
- **3. 제한 자격·정책 반환**: 최소 서비스 권한을 제공
- **4. 전용 연결 처리 요청**: 프라이빗 환경의 기능을 호출
- **5. 최소 결과·증거 반환**: 원문 노출 없이 결과를 전달
- **6. 업무 응답**: 환경 간 결과를 조합해 반환
- **7. 통합 지연·감사 기록**: 같은 요청 ID로 상태를 연결
