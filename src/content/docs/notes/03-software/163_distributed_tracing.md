...
- 배경/필요성: 수십 수백 개의 마이크로서비스가 동기(REST, gRPC) 및 비동기(Kafka, RabbitMQ)로 얽혀 복잡하게 상호작용하는 분산 시스템에서, 개별 서버의 로컬 로그만으로는 단일 요청의 전체 트랜잭션 흐름과 병목 구간(P99 Latency 지연, 연쇄 에러)의 근본 원인을 파악하기 불가능한 한계를 극복하기 위해, 최초 진입점에서 글로벌 고유 Trace ID를 발급하고 W3C 표준 문맥 전파(Context Propagation)를 통해 전 구간의 시작/종료 시간 및 인과관계를 나타내는 계층형 부모-자식 스팬(Span) 트리를 구성하여 **분산 호출 경로의 가시성과 초정밀 성능 병목 추적을 달성**할 필요
...
- 대규모 마이크로서비스 아키텍처(MSA)에서 복잡한 분산 트랜잭션의 성능 병목 및 연쇄 장애를 규명하는 **가장 핵심적인 텔레메트리 심층 분석 기술**로 확립되었으며, 실무 구축 시에는 **Kafka 메시징 및 비동기 멀티스레드 전 구간 W3C `traceparent` 문맥 주입/추출, 노드 간 타임스탬프 왜곡을 방지하는 NTP 정밀 동기화, 대용량 트래픽에서 저장 비용을 80% 이상 절감하면서 결함 트랜잭션을 100% 보존하는 Tail-based 선별 샘플링(Tempo/Jaeger)**을 결합하여 무결점 가시성과 운영 비용 효율성을 완벽히 조화
