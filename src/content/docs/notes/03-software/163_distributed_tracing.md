...
- **정의/개념**: 사용자 요청 경로를 추적하는 가시성 기법임
- **배경/필요성**: 다중 호출의 지연 원인 불명으로 추적 문맥 필요
...
| 비교축 | 헤드 샘플링 | 테일 샘플링 |
|:---|:---|:---|
| 핵심 특징 | 요청 시작 시점의 속성 등으로 결정함 | 처리가 완료된 시점의 속성으로 결정함 |
| 적용 기준 | 수집 비용을 줄이기 위해 일부만 추적 시 | 장애나 지연이 발생한 특정 건만 저장 시 |
| 주요 위험 | 희귀 장애 추적 누락 | 버퍼 메모리·판정 지연 |
...
2. 주문은 메시지 문맥과 연결률을 확인함

### 쉽게 이해하기 (학습용)
- 결제 API는 부모 Span별 지연 구간을 찾음
- 주문 메시지는 Span Link의 연결 누락을 찾음
