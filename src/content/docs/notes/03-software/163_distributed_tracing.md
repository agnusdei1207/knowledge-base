...
- **분산 추적(Distributed Tracing)**: 서비스별 스팬을 연결해 요청 경로와 지연 원인을 분석하는 기법
- **트레이스 식별자(Trace ID)**: 한 요청에 속한 모든 스팬을 묶는 식별자
- **스팬 식별자(Span ID)**: 개별 스팬을 구분하고 부모 스팬과 호출 관계를 표현하는 값
- **트레이스 컨텍스트(Trace Context)**: 추적 식별자와 상태를 호출 경계에 전달하는 표준 정보
- **트레이서·문맥 전파기(Tracer·Propagator)**: 스팬을 생성하고 통신 헤더·메시지에 추적 문맥을 주입·추출하는 구성요소
- **스팬 링크(Span Link)**: 부모·자식으로 표현하기 어려운 메시지·배치 작업의 관련 스팬을 잇는 참조
- **헤드 샘플링(Head Sampling)**: 요청 시작 시 추적 보존 여부를 결정하는 방식
- **테일 샘플링(Tail Sampling)**: 요청 완료 후 오류·지연 결과를 보고 추적 보존 여부를 결정하는 방식
- **Collector**: 완료된 스팬을 수신·가공·배치해 추적 백엔드로 보내는 구성요소
- **카디널리티(Cardinality)**: 속성값의 고유 조합 수로 저장·조회 비용을 좌우하는 값

## Ⅰ. 개요

- 정의/개념: 스팬을 연결한 **분산 요청 경로·구간 지연 분석**
- 배경/필요성: 서비스 경계의 **병목·오류 전파·의존 관계 식별**
...
1. **추적 문맥 전달**: 상위 스팬 생성·요청 식별
2. **자식 문맥 전달**: 동일 Trace ID·부모 ID 주입
3. **처리 결과 반환**: 자식 스팬 상태·소요 시간 확정
4. **완료 스팬 전송**: 서비스별 스팬의 Collector 수집
5. **스팬 가공·배치**: 식별자·부모 관계 기반 경로 조립
...
| 사용자 **속성 폭증** | 속성 허용 목록·**민감값 제거** | 비용·**개인정보 노출 축소** |
