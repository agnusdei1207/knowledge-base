...
- **추적 식별자(Trace Identifier, Trace ID)•부모 관계**: 같은 요청의 스팬을 묶고 호출 선후 관계를 구성하는 정보이다.
- **호출•메시지 문맥 전파**: 동기 호출 헤더나 비동기 메시지 속성에 추적 정보를 전달해 경계를 넘어 연결을 유지하는 방식이다.
...
- **헤드 샘플링(Head Sampling)**: 요청 시작 시 비율•속성 규칙으로 추적 보존 여부를 즉시 결정하는 방식이다.
