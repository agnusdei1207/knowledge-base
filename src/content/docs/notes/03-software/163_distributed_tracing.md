...
- **1. 자식 추적 문맥 전달**: 호출자는 Trace ID와 Span ID를 하위 서비스에 전달한다.
- **2. 자식 완료 스팬 전송**: 하위 실행 구간의 상태와 소요 시간을 확정해 컬렉터로 보내는 단계이다.
- **3. 부모 완료 스팬 전송**: 상위 구간과 자식 호출 관계를 확정해 컬렉터로 보내는 단계이다.
- **4. 가공 스팬 배치 전송**: 식별자•부모 관계를 보존한 스팬 묶음을 추적 백엔드에 전달하는 단계이다.
