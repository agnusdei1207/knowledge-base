...
- 배경/필요성: 수동 빌드·테스트 및 수작업 배포 절차가 유발하는 잦은 인적 오류(Human Error), 통합 지연으로 인한 결함 조기 발견 실패 및 릴리즈 리드타임(Lead Time) 장기화 한계를 극복하고, 소스 코드 커밋부터 자동 빌드·테스트(CI) 및 품질 게이트(Quality Gate)를 거쳐 불변 아티팩트를 무중단 배포(CD)하는 전 과정을 파이프라인화하여 **소프트웨어 릴리즈 속도와 운영 안정성을 동시에 극대화**할 필요
