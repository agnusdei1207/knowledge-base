...
date: "2026-08-05T13:19:01+09:00"
tags:
  - "notes-software"
weight: 53
extra:
...
1. **변경 감지•실행**: 커밋•병합으로 파이프라인 시작
2. **빌드•자동 검증**: 산출물 생성과 시험•분석 수행
3. **품질 게이트 판정**: 다음 단계 승격 또는 차단
4. **불변 아티팩트 등록**: 버전•해시•생성 이력 보존
5. **승인 버전 승격**: 같은 산출물을 환경별 반영

> 요약: 검증을 통과한 불변 아티팩트만 환경별로 승격

#### 한줄 요약

- 변경 검증•품질 판정•**불변 산출물 승격 흐름**
