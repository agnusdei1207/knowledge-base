...
- **조기 검증•추적성**: 변경 직후 결함을 찾고 소스•검증 결과•산출물•배포 환경의 연결 관계를 보존하는 성질이다.
...
- **소스 저장소•파이프라인 실행기**: 소스 저장소는 변경 이력과 파이프라인 코드를 보관하고, 실행기는 선언된 빌드•시험•분석 단계를 수행한다.
...
- **버전•해시•생성 이력**: 산출물 버전과 내용 무결성 값, 어떤 소스•도구•절차로 만들었는지를 잇는 식별 정보이다.
...
- **지속적 전달(Continuous Delivery)**: 운영 배포 직전까지 자동화하되 최종 반영에는 사람의 승인을 두는 방식이다.
- **지속적 배포(Continuous Deployment)**: 모든 자동 검증을 통과한 변경을 운영까지 자동 반영하는 방식이다.
...
- **산출물 서명•해시(Artifact Signature•Hash)**: 배포 산출물의 출처와 무결성을 검증하는 값이다.
- **단계 병렬화•캐시(Stage Parallelism•Cache)**: 독립 검사를 동시에 수행하고 재사용 가능한 빌드 결과를 보관해 피드백 시간을 줄이는 방식이다.
- **격리•재현 환경(Isolation•Reproducible Environment)**: 불안정 시험을 본선과 분리하고 같은 의존성•설정으로 실패를 반복 실행할 수 있게 만든 환경이다.
- **비밀 저장소•단기 자격 증명**: 배포 비밀을 코드 밖에서 보호하고 작업 시점에만 짧은 유효기간의 권한을 발급하는 통제이다.
- **단계 배포•상태 검증(Progressive Delivery•Health Validation)**: 일부 대상에 먼저 배포해 지표와 상태를 확인한 뒤 범위를 넓히는 방식이다.
