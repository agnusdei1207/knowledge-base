...
- **2. 입출력·열 변환 계보 보고**: 실제 읽은 범위와 열 매핑·변환식·산출 상태를 실행 ID에 연결
- **3. 식별자·실행 관계 병합**: 시스템별 자산과 작업 실행을 하나의 방향성 그래프로 정합화
- **4. 상·하류 영향 탐색**: 변경·오류 자산에서 소비처 또는 원천 방향으로 경로 추적
- **5. 원인·책임 경로 제공**: 실행·열·소유자 근거로 영향 범위와 수정 책임 판정
