...
- **이동·변환 관계와 실행 이력**: 데이터가 원천에서 산출물까지 어떤 처리와 실행을 거쳤는지 추적하는 정보이다.

</details>

- 정의/개념: **데이터 계보**는 원천에서 산출물까지 데이터의 이동·변환·의존 관계와 실행 이력을 연결하여 추적하는 메타데이터 체계
...
- **설계 계보**: 배포 전에 SQL·작업 그래프·모델 정의로 변경 영향 범위를 찾는 계보이다.
