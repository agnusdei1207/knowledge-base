---
sidebar:
  order: 134
  label: "134. 데이터 계보 Data Lineage (Data Lineage)"
  badge:
    text: "미출제 · 50%"
    variant: note
title: "데이터 계보 Data Lineage (Data Lineage)"
date: "2026-07-25T11:38:00+09:00"
tags:
  - "notes-software"
weight: 134
extra:
  question_no: "134"
  source_status: "미출제"
  source_history: ""
  priority: 50
  priority_note: "계보는 영향 분석·감사·품질 추적 핵심"
---

## 미리 알고가기

- **데이터 계보(Data Lineage)**: 데이터가 원천에서 산출물까지 이동·변환된 관계와 실행 이력을 추적하는 정보임
- **데이터셋(Dataset)**: 테이블·파일·토픽처럼 작업이 읽거나 쓰는 데이터 단위임
- **작업(Job)**: 데이터셋을 입력받아 다른 데이터셋을 생성·변경하는 처리 단위임
- **실행 이력(Run)**: 특정 시점에 수행된 작업의 한 인스턴스이며 시작·완료·실패 상태를 가짐
- **열 수준 계보(Column-level Lineage)**: 산출 열이 어떤 원천 열과 변환식에서 생성됐는지 추적하는 관계임
- **영향 분석(Impact Analysis)**: 변경 대상의 하위 데이터셋·작업·보고서 범위를 찾는 분석임
- **원인 분석(Root Cause Analysis)**: 오류 결과에서 상위 원천과 변환 경로를 거슬러 원인을 찾는 분석임
- **네임스페이스(Namespace)**: 서로 다른 시스템의 같은 이름을 구분하도록 자산 식별자의 범위를 정하는 이름 공간임
- **실행 식별자(Run Identifier, Run ID)**: ID를 '아이디'로 읽고 영문 첫 글자로 식별자를 줄여 쓰며 작업의 한 실행을 다른 실행과 구분함
- **수집기(Collector)**: 처리 엔진과 오케스트레이터에서 계보 이벤트와 메타데이터를 가져오는 구성요소임
- **오케스트레이터(Orchestrator)**: 작업 의존성과 일정에 따라 데이터 작업의 실행 순서를 조정하는 시스템임
- **사용자 인터페이스(User Interface, UI)**: 영문 첫 글자를 딴 UI를 '유아이'로 읽으며 사용자가 계보 그래프를 검색·탐색하는 화면임
- **구조적 질의 언어(Structured Query Language, SQL)**: 영문 첫 글자를 딴 SQL을 '에스큐엘'로 읽으며 설계 계보가 분석할 선언형 변환을 표현함

## Ⅰ. 개요

- **정의/개념**: 원천부터 산출물까지의 **이동·변환 관계와 실행 이력**이다.
- **배경/필요성**: 변경 영향·오류 원인을 추적하고 **생성 근거를 감사**한다.

### 쉽게 이해하기 (학습용)

- 보고서 숫자가 어느 원료와 공정을 거쳐 만들어졌는지 보여 주는 추적 지도이다.

## Ⅱ. 특징

- **방향 그래프**: 데이터셋·작업 관계를 상·하류로 탐색한다.
- **열 수준 추적**: 산출 열을 원천 열·변환식과 연결한다.
- **설계·실행 결합**: 선언 경로와 실제 입출력을 비교한다.

### 쉽게 이해하기 (학습용)

- 설계 계보는 예정 노선, 실행 계보는 실제 이동 기록이며 둘 다 빠진 길이 없는지 확인해야 한다.

## Ⅲ. 구조 및 구성요소

```mermaid
block
    columns 1
    A["데이터셋 | 입출력 자산 식별"]
    B["작업·실행 이력 | 실행 상태·범위 기록"]
    C["부가 메타데이터 | 열 식·품질·소유자 연결"]
    D["수집기·이벤트 발행기 | 계보 이벤트 정합화"]
    E["계보 그래프·UI | 상·하류 경로 탐색"]
```

| 구성요소 | 책임 |
|:---|:---|
| 데이터셋 | 네임스페이스와 영속 ID로 자산 식별 |
| 작업·실행 이력 | 논리 작업과 개별 실행 상태·범위 기록 |
| 부가 메타데이터 | 열 식·코드·품질·소유자 연결 |
| 수집기·이벤트 발행기 | 계보 이벤트 수집·식별자 정합화 |
| 계보 그래프·UI | 데이터셋·작업·열의 상·하류 탐색 |

### 쉽게 이해하기 (학습용)

- 보고서 숫자가 어떤 원료와 공정을 거쳤는지 보여 준다.

## Ⅳ. 흐름도

```mermaid
sequenceDiagram
    participant O as 오케스트레이터
    participant E as 처리 엔진
    participant C as 계보 수집기
    participant G as 계보 그래프
    participant U as 사용자
    O->>E: 1. 작업명·실행 ID로 실행 요청
    E->>C: 2. 입력 데이터셋 보고
    E->>C: 3. 열 매핑·변환식 보고
    E->>C: 4. 출력·완료 상태 보고
    C->>G: 5. 식별자·관계 병합
    U->>G: 6. 상·하류 영향 탐색
    G-->>U: 7. 실행·열·소유자 경로 제공
```

**동작 원리**

- **1. 작업명·실행 ID로 실행 요청**: 개별 실행을 식별한다.
- **2. 입력 데이터셋 보고**: 실제 읽은 자산과 범위를 기록한다.
- **3. 열 매핑·변환식 보고**: 원천 열과 산출 열을 연결한다.
- **4. 출력·완료 상태 보고**: 결과 자산과 실행 상태를 기록한다.
- **5. 식별자·관계 병합**: 시스템별 자산을 계보 그래프에 합친다.
- **6. 상·하류 영향 탐색**: 변경 대상이나 오류 자산을 조회한다.
- **7. 실행·열·소유자 경로 제공**: 원인과 책임 판단 정보를 제시한다.

### 쉽게 이해하기 (학습용)

- 작업이 실제로 읽고 쓴 자료와 열 계산식을 운행 기록처럼 모아 관계 지도에 합친다.

## Ⅴ. 종류 및 비교

| 비교 항목 | 설계 계보 | 실행 계보 |
|:---|:---|:---|
| 적용 기준 | **배포 전 영향 분석** | **장애 원인·감사 추적** |
| 핵심 특징 | SQL·DAG·모델 정의 분석 | 실제 읽기·쓰기 이벤트 관찰 |
| 한계 | 동적 SQL·수동 이동 누락 | 미실행 분기·미계측 도구 누락 |

### 쉽게 이해하기 (학습용)

- 공사 전 영향은 설계도, 사고 후 원인은 실제 운행 기록이 더 잘 보여 준다.

## Ⅵ. 실무 고려사항 및 대책

| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 자산 식별 | 네임스페이스·영속 ID·별칭 이력 적용 | 이름 충돌·관계 단절 방지 |
| 수집 범위 | 핵심 경로 계측·누락 등록·대사 | 계보 공백 감소 |
| 열 계보 | 신뢰도 표시·표본 검증·수동 보정 | 변환식 오판 완화 |
| 실행량 | 논리 계보와 실행 이력 분리·집계 | 탐색 지연 감소 |
| 최신성 | 배포·실행 이벤트 비교·신선도 SLO | 설계·실행 불일치 탐지 |

### 쉽게 이해하기 (학습용)

- 열 하나를 바꾸기 전에 그 열을 쓰는 보고서와 작업을 찾아 고칠 순서를 정한다.

## Ⅶ. 결론

- **식별자·수집 범위·열 계보 신뢰도·최신성**으로 데이터 계보를 운영한다.

### 쉽게 이해하기 (학습용)

- 예쁜 지도가 아니라 빠진 길 없이 영향과 원인을 빨리 찾는 지도가 필요하다.
