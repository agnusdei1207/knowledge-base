...
- **실행 식별자(Run Identifier, Run ID)**: ID를 '아이디'로 읽고 영문 첫 글자로 식별자를 줄여 쓰며 작업의 한 실행을 다른 실행과 구분한다.
- **수집기(Collector)**: 처리 엔진과 오케스트레이터에서 계보 이벤트와 메타데이터를 가져오는 구성요소이다.
- **오케스트레이터(Orchestrator)**: 작업 의존성과 일정에 따라 데이터 작업의 실행 순서를 조정하는 시스템이다.
- **사용자 인터페이스(User Interface, UI)**: 영문 첫 글자를 딴 UI를 '유아이'로 읽으며 사용자가 계보 그래프를 검색·탐색하는 화면이다.
- **구조적 질의 언어(Structured Query Language, SQL)**: 영문 첫 글자를 딴 SQL을 '에스큐엘'로 읽으며 설계 계보가 분석할 선언형 변환을 표현한다.

## Ⅰ. 개요

- 정의/개념: 원천부터 산출물까지의 **이동·변환 관계와 실행 이력**
- 배경/필요성: 변경 영향·오류 원인 추적과 **생성 근거 감사**
...
- **방향 그래프**: 데이터셋·작업 관계를 상·하류로 탐색
- **열 수준 추적**: 산출 열을 원천 열·변환식과 연결
- **설계·실행 결합**: 선언 경로와 실제 입출력을 비교
...
block
    columns 1
    A["데이터셋 | 입출력 자산 식별"]
    B["작업·실행 이력 | 실행 상태·범위 기록"]
    C["부가 메타데이터 | 열 식·품질·소유자 연결"]
    D["수집기·이벤트 발행기 | 계보 이벤트 정합화"]
    E["계보 그래프·UI | 상·하류 경로 탐색"]
```

| 구성요소 | 책임 |
|:---|:---|
| 데이터셋 | 네임스페이스와 영속 ID로 자산 식별 |
| 작업·실행 이력 | 논리 작업과 개별 실행 상태·범위 기록 |
| 부가 메타데이터 | 열 식·코드·품질·소유자 연결 |
| 수집기·이벤트 발행기 | 계보 이벤트 수집·식별자 정합화 |
| 계보 그래프·UI | 데이터셋·작업·열의 상·하류 탐색 |
...
sequenceDiagram
    ...
    O->>E: 1. 작업명·실행 ID로 실행 요청
    E->>C: 2. 입력 데이터셋 보고
    E->>C: 3. 열 매핑·변환식 보고
    E->>C: 4. 출력·완료 상태 보고
    C->>G: 5. 식별자·관계 병합
    U->>G: 6. 상·하류 영향 탐색
    G-->>U: 7. 실행·열·소유자 경로 제공
```

**동작 원리**

- **1. 작업명·실행 ID로 실행 요청**: 개별 실행을 식별
- **2. 입력 데이터셋 보고**: 실제 읽은 자산과 범위를 기록
- **3. 열 매핑·변환식 보고**: 원천 열과 산출 열을 연결
- **4. 출력·완료 상태 보고**: 결과 자산과 실행 상태를 기록
- **5. 식별자·관계 병합**: 시스템별 자산을 계보 그래프에 병합
- **6. 상·하류 영향 탐색**: 변경 대상이나 오류 자산을 조회
- **7. 실행·열·소유자 경로 제공**: 원인과 책임 판단 정보를 제시
...
| 핵심 특징 | SQL·DAG·모델 정의 분석 | 실제 읽기·쓰기 이벤트 관찰 |
| 한계 | 동적 SQL·수동 이동 누락 | 미실행 분기·미계측 도구 누락 |
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 자산 식별 | 네임스페이스·영속 ID·별칭 이력 적용 | 이름 충돌·관계 단절 방지 |
| 수집 범위 | 핵심 경로 계측·누락 등록·대사 | 계보 공백 감소 |
| 열 계보 | 신뢰도 표시·표본 검증·수동 보정 | 변환식 오판 완화 |
| 실행량 | 논리 계보와 실행 이력 분리·집계 | 탐색 지연 감소 |
| 최신성 | 배포·실행 이벤트 비교·신선도 SLO | 설계·실행 불일치 탐지 |
...
- **식별자·수집 범위·열 계보 신뢰도·최신성**으로 데이터 계보를 운영
