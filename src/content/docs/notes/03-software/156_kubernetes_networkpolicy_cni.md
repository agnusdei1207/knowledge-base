...
- **정의/개념**: CNI는 Pod 연결, 정책은 허용 통신 선언
- **배경/필요성**: 평면망의 불필요 접근으로 연결·통제 분리 필요
...
|:---|:---|:---|
| 핵심 특징 | Pod 인터페이스·IP·경로 구성 | 선택 Pod의 허용 흐름 선언 |
| 적용 기준 | Pod 연결·IPAM·노드 간 통신 | Pod 간 최소 허용 통신 통제 |
| 주요 위험 | 주소 충돌·경로·플러그인 실패 | 미지원 CNI·선택자·방향 누락 |
...
2. CNI IPAM: Pod별 중복 없는 주소 할당

### 쉽게 이해하기 (학습용)
- 업무 포드는 지정 API에서 오는 데이터베이스 포트만 받는다.
- IPAM은 Pod마다 겹치지 않는 주소를 배정한다

## Ⅶ. 결론

- Pod 연결은 CNI, 최소 통신은 정책 지원으로 검증
