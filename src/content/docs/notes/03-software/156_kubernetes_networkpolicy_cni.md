...
- 배경/필요성: 쿠버네티스 기본 평면 네트워크(Flat Network) 환경에서 **단일 파드 침해 시 클러스터 전체로의 측면 공격(Lateral Movement) 확산 해결 불가**
...
#### 한줄 요약
- 패킷 생성 → 라벨 식별 → 정책 대조 → 포트 검증 → 인가 전달/Drop 순으로 진행된다.
...
#### 한줄 요약
- Secondary CIDR 할당, Cilium eBPF 도입, Hubble UI 시각화, Default Deny로 안정성을 확보한다.
