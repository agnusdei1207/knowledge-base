...
- 정의/개념: 파드 연결과 허용 통신을 분리한 **CNI•NetworkPolicy** 체계이다.
- 배경/필요성: 평면 네트워크는 침해 파드의 **불필요한 횡적 이동** 허용이 필요하다.
...
- **1. 송신 규칙•패킷 속성 대조**: 선택자•목적지•포트를 비교한다.
- **2. 송신 허용 판정**: 송신 측 허용 규칙 합집합을 적용한다.
- **3. 수신 정책 지점 전달**: 통과 패킷을 수신 데이터면으로 전송이다.
- **4. 수신 규칙•패킷 속성 대조**: 선택자•출발지•포트를 비교한다.
- **5. 수신 허용 판정**: 수신 측도 허용할 때만 파드를 전달한다.
