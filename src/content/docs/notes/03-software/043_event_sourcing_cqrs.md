...
- **원본(Source of Truth)**: 업무 상태를 다시 계산할 때 최종 기준이 되는 신뢰 데이터로서 이벤트 소싱에서는 순서가 있는 이벤트 스트림이 해당한다.
...
- 정의/개념: 변경 이벤트 저장과 명령·조회 모델을 나눈 **설계 방식**
- 배경/필요성: 현재값만으로 불가능한 **이력 재생·감사·조회 최적화**
...
- 거래 원장은 입출금 이벤트를 추가 전용으로 저장해 감사 이력과 과거 시점의 잔액을 재구성
