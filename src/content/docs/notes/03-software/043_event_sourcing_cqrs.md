...
- 정의/개념: **이벤트 소싱과 CQRS**는 각각 상태 변경 이력을 원본으로 저장하고 변경•조회 모델의 책임을 분리하며 함께 사용할 수 있는 **아키텍처 패턴**이다.
- 배경/필요성: 현재값만 덮어써 저장하면 **과거 상태 재구성•변경 원인 추적이 어려움**이 발생한다.

#### 한줄 요약

- 이벤트 원본과 **명령•조회 모델 분리** 기반 상태를 관리한다.
...
- **추가 전용 이벤트**를 상태 이력 원본으로 사용이 핵심이다.
- **명령•조회 모델 분리**로 독립 확장이 핵심이다.
- **프로젝션 갱신•결과적 일관성**으로 목적별 조회 상태를 생성한다.

#### 한줄 요약

- 이력 복원•독립 확장과 **결과적 일관성** 절충이 핵심이다.
...
- 명령•이벤트 원본•프로젝션•**조회 모델 구조**가 핵심이다.
...
- **1. 스트림 조회•상태 복원**: 이벤트 이력으로 현재 상태를 계산한다.
- **2. 업무 규칙•버전 검증**: 명령과 예상 버전을 확인한다.
- **3. 새 이벤트 추가**: 변경을 스트림 끝에 원자적 저장한다.
- **4. 프로젝션 순서 반영**: 이벤트를 목적별 상태로 변환한다.
- **5. 조회 모델•반영 버전 갱신**: 읽기 상태와 위치를 기록한다.

#### 한줄 요약

- 이벤트 추가 후 **프로젝션•조회 모델**을 갱신한다.
...
- **감사 이력(Audit Trail)**: 누가 언제 어떤 상태 변화를 만들었는지 추적하도록 보존한 사건 기록한다.
...
> 요약: **이력•독립 확장**이익과 **비동기 복잡도**를 비교한다.

#### 한줄 요약

- 이력 복원•독립 확장은 **이벤트 소싱•CQRS**가 핵심이다.
...
- 상태 복원•독립 확장 이익이 **스키마 진화 비용** 보다 크면 **이벤트 소싱•CQRS**을 적용한다.

#### 한줄 요약

- 상태 복원 이익이 비용보다 크면 **이벤트 소싱•CQRS 적용**이 핵심이다.
