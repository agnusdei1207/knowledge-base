...
- 정의/개념: 분할·복제 데이터를 **하나의 논리 DB로 제공**
...
- **1. 키·연산·일관성 요청**: 처리 조건을 보낸다.
- **2. 파티션 배치 조회**: 현재 위치표를 확인한다.
- **3. 리더·복제본 위치 반환**: 담당 노드를 찾는다.
- **4. 대상 파티션 연산 전달**: 리더에 요청한다.
- **5. 변경 로그 복제**: 사본에 전파한다.
- **6. 복제 확인 반환**: 요구 응답 수를 확인한다.
- **7. 논리 DB 결과 응답**: 위치를 숨겨 반환한다.
...
- **확장 이득·교차 연산·부분 장애·재균형 비용**으로 분산 DB를 정한다.
