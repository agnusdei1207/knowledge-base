...
- **1. 키·일관성 요청**: 연산과 요구 일관성 수준 전달
- **2. 파티션 배치 조회**: 현재 위치표를 확인
- **3. 리더 위치 선택**: 키가 속한 파티션의 담당 노드 결정
- **4. 파티션 연산 실행**: 지역 데이터에서 읽기·쓰기 수행
- **5. 로그 복제·확인**: 요구 응답 수 뒤 논리 DB 결과 확정
...
> **적용 사례**: 테넌트 ID로 분할할 때 대형 테넌트의 점유율과 교차 테넌트 질의 비율, 노드 추가 시 재균형 시간을 함께 측정
