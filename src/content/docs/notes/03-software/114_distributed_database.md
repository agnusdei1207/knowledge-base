...
- 정의/개념: 분할•복제 데이터를 하나처럼 제공하는 **분산 데이터베이스**이다.
- 배경/필요성: 단일 노드는 저장•처리 용량과 장애 복구가 한 장비 한계에 종속된다.
...
- **파티셔닝**: 키•범위별 저장•처리 부하 수평 분산이다.
- **복제**•**합의**: 장애 영역별 사본과 순서 확정으로 내구성•가용성을 확보한다.
- **네트워크 조정 비용**: 교차 파티션 질의•거래•재균형 시 발생이다.
...
| 구성요소 | 책임 |
|:---|:---|
| 파티션 맵•라우터 | **파티션 맵**•**라우터**로 노드 위치 연결 |
| 분산 실행기 | **분산 실행기**가 연산 계획•결과 병합 |
| 복제 로그•합의 | **복제 로그**•**합의 모듈**로 순서•확정점 일치 |
| 트랜잭션 조정자 | **트랜잭션 조정자**가 교차 파티션 원자성 조정 |
| 장애 감지•재균형 | **장애 감지**•**재균형**으로 실패 복구•이동 |
...
- **대상 파티션 위치 확인**: 요청 키와 질의 범위의 소유 노드를 찾는 단계이다.
- **단일•교차 파티션 판정**: 요청이 걸치는 저장 경계를 구분하는 단계이다.
- **지역 파티션 연산 실행**: 각 담당 노드에서 읽기•쓰기를 수행하는 단계이다.
- **복제 로그•합의 수행**: 쓰기 순서와 확정점을 복제본에 일치시키는 단계이다.
- **결과 병합•커밋 판정**: 중간 결과를 합치거나 원자 확정을 결정하는 단계이다.
...
- **1. 대상 파티션 위치 확인**: **대상 파티션 위치 확인**으로 소유 노드를 탐색한다.
- **2. 단일•교차 파티션 판정**: **단일•교차 파티션 판정**으로 실행 범위를 구분한다.
- **3. 지역 파티션 연산 실행**: **지역 파티션 연산 실행**으로 담당 노드에서 처리한다.
- **4. 복제 로그•합의 수행**: **복제 로그•합의 수행**으로 쓰기 순서•확정점을 일치시킨다.
- **5. 결과 병합•커밋 판정**: **결과 병합•커밋 판정**으로 조회 또는 원자 확정을 완료한다.
...
- **단일 노드 데이터베이스**: 한 장비 안에서 저장•질의•거래를 처리하는 배치 구조이다.
- **데이터베이스 배치 선택 기준**: 수평 용량•장애 분산 가치와 교차 파티션 조정 비용을 비교하는 기준이다.
...
| 핵심 특징 | **분산 데이터베이스** | **단일 노드 데이터베이스** |
| 한계 | **교차 파티션 연산**•네트워크•재균형 비용 | 확장 상한과 **단일 장애 경계** |

#### 한줄 요약

- **데이터베이스 배치 선택 기준**에서 한 지점은 단순하고 여러 지점은 넓게 확장되지만 연락과 합의가 필요하다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 편향 키로 일부 노드에 부하 집중 | **접근 검증**•**소유권 검증**•**분포 검증** | 팬아웃•핫 파티션 감소 |
| 전역 일관성 강제로 지연•가용성 저하 | **연산별 일관성 수준**•**실패 동작** 정의 | 지연•가용성 균형 |
| 무제한 재균형으로 전경 입출력 경합 | **속도 제한**•**정합성 검증**•**롤백** | 서비스 지연 억제 |
| 시계 오차•재시도로 순서•중복 오판 | **논리 시계**•**멱등성 키** 적용 | 순서•중복 오류 방지 |
| 집계 지표가 특정 파티션 병목 은폐 | **파티션별 지표**•**교차 요청 지표** 수집 | 국소 병목 조기 탐지 |
...
- **선택 기준**에 따라 단일 노드 한계를 초과하고 교차 연산이 적으면 **분산 데이터베이스**를 선택한다.

#### 한줄 요약

- **선택 기준**은 수평 확장과 연락•합의•이동 비용을 함께 비교한다.
