...
- 배경/필요성: 단일 노드는 저장•처리 용량과 장애 복구가 **한 장비 한계**에 종속이 발생한다.
...
- **복제•합의**: 장애 영역별 사본으로 내구성•가용성을 확보한다.
...
- **1. 대상 파티션 위치 확인**: 키•범위의 소유 노드 탐색이다.
- **2. 단일•교차 파티션 판정**: 분산 실행 범위를 구분한다.
- **3. 지역 파티션 연산 실행**: 담당 노드에서 읽기•쓰기를 수행한다.
- **4. 복제 로그•합의 수행**: 쓰기 순서•확정점 일치이다.
- **5. 결과 병합•커밋 판정**: 조회 병합 또는 원자 확정이다.
...
- 단일 노드 한계 초과•교차 연산 최소면 **분산 데이터베이스**를 선택한다.
