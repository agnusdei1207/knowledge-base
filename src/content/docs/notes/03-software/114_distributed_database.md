...
- 배경/필요성: 단일 노드 데이터베이스가 마주하는 수직 확장(Scale-Up) 한계와 단일 장애점(SPOF) 리스크, 원거리 글로벌 사용자의 네트워크 지연 문제를 극복하고, 물리적으로 분산된 다수의 독립 노드에 데이터를 수평 분할(Fragmentation) 및 복제(Replication) 저장하면서도 6대 분산 투명성(위치·분할·복제·병행·장애·자원)을 통해 **애플리케이션에 단일 논리 DB 뷰를 제공하고 고가용성과 선형적 수평 확장을 실현**할 필요
...
- 초대규모 엔터프라이즈 시스템 및 글로벌 클라우드 네이티브 영속성 계층의 **핵심 분산 스토리지 아키텍처**로 확립되었으며, 실무 설계 시에는 **분산 트랜잭션의 원자성을 보장하는 2PC 및 Saga 패턴 채택, 노드 간 복제 상태 합의와 스플릿 브레인 방지를 위한 Raft/Paxos 쿼럼 엔진 적용, 전역 교착 상태를 해소하는 분산 데드락 감지기(Wait-For Graph) 및 위치·복제 투명성 거버넌스**를 구축하여 시스템 복잡도를 통제하고 글로벌 정합성을 완성
