...
- 정의/개념: **서킷 브레이커**는 원격 호출 실패가 임계치를 넘으면 후속 호출을 차단하고 제한된 시험 호출로 복구를 판정하는 **장애 격리 패턴**이다.
- 배경/필요성: 장애 서비스 반복 호출로 **스레드•연결 풀 고갈**이 발생한다.

#### 한줄 요약

- 실패 임계치 차단과 시험 호출 기반 **장애 격리**가 핵심이다.
...
- **Open 상태**에서 호출을 차단해 의존 서비스 복구 여력 보호가 핵심이다.
- **Half-Open 상태**에서 제한된 시험 호출로 복구를 판정한다.
- **실패 측정 창•임계치**로 차단 민감도를 조정한다.

#### 한줄 요약

- Closed•Open•Half-Open **상태 전이** 기반 제어가 핵심이다.
...
- 인터셉터•측정 창•상태 기계•**복구•대체 구조**가 핵심이다.
...
- **호출 허용•타임아웃**: Closed 상태에서 원격 요청을 제한 시간과 함께 전달하는 단계다.
- **결과 집계•임계 판정**: 성공•오류•지연 표본으로 Open 전환 조건을 확인하는 단계다.
- **Open 즉시 차단**: 후속 원격 호출을 보내지 않고 대체 응답이나 명시 오류를 반환하는 단계다.
- **Open 대기 만료**: 설정한 복구 대기 시간이 지나면 Half-Open으로 전환하는 단계다.
- **Half-Open 시험 판정**: 제한된 시험 호출의 성공은 Closed, 실패는 Open으로 전환하는 단계다.
...
- **1. 호출 허용•타임아웃**: Closed 요청을 제한 시간으로 전달한다.
- **2. 결과 집계•임계 판정**: 표본으로 Open 전환 여부를 결정한다.
- **3. Open 즉시 차단**: 원격 호출 없이 대체 결과 반환한다.
- **4. Open 대기 만료**: 대기 후 Half-Open을 전환한다.
- **5. Half-Open 시험 판정**: 성공은 Closed, 실패는 Open을 전환한다.

#### 한줄 요약

- Closed•Open•Half-Open **복구 상태 순환**이 핵심이다.
...
> 요약: **일시 오류**는 제한 재시도, **지속 장애**는 호출 차단이 핵심이다.
...
- **검증 동시 수(Probe Concurrency)**: 반열림 상태의 동시 시험 호출 한도다.
- **점진 허용(Gradual Allowance)**: 성공에 따라 시험 호출 허용량을 늘리는 제어다.
...
- **일시 오류**는 백오프 재시도, **지속 장애**는 서킷 브레이커를 선택한다.

#### 한줄 요약

- 일시 오류는 재시도, 지속 장애는 **차단 후 시험**이 핵심이다.
