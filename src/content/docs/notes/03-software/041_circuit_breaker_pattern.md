...
- **최소 표본(Minimum Sample Size)**: 실패율을 계산하기 전에 측정 창에 모여야 하는 최소 호출 수로서 적은 요청의 우연한 실패에 과민 차단되는 것을 줄이다.
...
- 정의/개념: 원격 실패를 차단하고 시험 호출로 복구를 확인하는 **패턴**
- 배경/필요성: 반복 장애 호출에 따른 **스레드·연결 고갈** 방지
...
- 결제 연동은 재시도 요청에 같은 멱등키를 쓰고 회로가 열리면 성공처럼 숨기지 않고 명시적 오류를 반환
