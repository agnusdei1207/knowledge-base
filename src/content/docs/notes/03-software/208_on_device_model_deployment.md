...
- **인공지능(Artificial Intelligence, AI)**: 데이터 패턴을 학습한 모델로 분류·예측·생성 작업을 수행하는 기술
...
- **신경망 처리 장치(Neural Processing Unit, NPU)**: 신경망 연산을 낮은 전력으로 가속하는 전용 처리 장치
- **그래픽 처리 장치(Graphics Processing Unit, GPU)**: 다수 연산 유닛으로 데이터 병렬 계산을 수행하는 처리 장치
- **중앙처리장치(Central Processing Unit, CPU)**: 범용 명령을 실행하며 지원되지 않는 모델 연산을 처리하는 장치
- **무선 업데이트(Over-the-Air Update, OTA)**: 네트워크로 기기의 모델·소프트웨어 버전을 원격 교체하는 방식
- **폴백(Fallback)**: 기기 실행이 불가능하거나 품질이 낮을 때 다른 모델·클라우드 경로로 전환하는 동작

## Ⅰ. 개요

- **정의/개념**: 기기에서 저지연·오프라인 추론
- **배경/필요성**: 망 지연·정보 전송으로 단말 모델 배포 필요
...
- 양자화·가지치기로 저장 공간·추론 지연·전력 감소한다.
- 연산자·정밀도 지원 차이가 기기별 실행 가능성 결정한다.
- 기기 단편화로 같은 모델도 지연·전력·정확도가 달라진다.
- 단계 배포·OTA·폴백으로 현장 장애 범위 제한한다.
...
| 설계 요소 | 설명 |
|:---|:---|
| 경량 모델 패키지 | 양자화 가중치·연산자·버전 보관 |
| 기기 추론 런타임 | 모델 그래프를 지원 연산기로 실행 |
| CPU·GPU·NPU | 기기별 지원 정밀도·연산자 가속 |
| 온디바이스 애플리케이션 | 입력 전처리·추론·결과 사용 |
| 버전·품질 관측기 | 지연·오류·전력·품질과 OTA 관리 |
...
| 절차 | 설명 |
|:---|:---|
| 경량 모델 후보 전달 | 양자화·가지치기·런타임 형식 고정 |
| 호환성·품질·자원 검증 | 기기별 연산자·정확도·지연·전력 측정 |
| 승인 버전 등록 | 기기군별 합격 모델·폴백 버전 지정 |
| 단계별 OTA 배포 | 소수 기기부터 모델 버전 확대 |
| 지연·오류·전력 전달 | 기기군별 실행·품질 신호 수집 |
| 확대·중단·폴백 | 임계치로 배포 확대·복귀 결정 |
...
| 판단 기준 | 온디바이스 추론 | 클라우드 추론 | 하이브리드 추론 |
|:---|:---|:---|:---|
| 핵심 특징 | 기기 내부 경량 모델 실행 | 서버의 큰 모델 실행 | 기기 판정 후 서버 우회 |
| 적용 기준 | 저지연·오프라인·민감정보 | 복잡 모델·중앙 갱신 | 기기 자원·품질 편차 큼 |
| 주요 위험 | 기기 단편화·전력·용량 | 네트워크 지연·전송 비용 | 경로 일관성·운영 복잡도 |
...
1. 음성 명령은 기기군별 양자화 정확도·지연 검증
2. 문서 요약은 기기 자원 부족 시 클라우드로 폴백

### 쉽게 이해하기 (학습용)

- 음성 모델은 칩 종류마다 단어 정확도와 응답 속도를 확인해 맞는 버전을 배포한다
- 기기에서 큰 문서를 처리할 수 없으면 민감정보를 걸러 서버 모델에 요청한다

## Ⅶ. 결론

- 기기별 연산자·정확도·지연·전력 검증 후 OTA·폴백 결정
