...
date: "2026-08-05T17:22:00+09:00"
tags:
  - "notes-software"
weight: 121
extra:
...
- **1. 이벤트•입력 위치 수신**: 처리 결과와 재생 시작점을 같은 논리 경계에 연결
- **2. 미확정 결과 기록**: 외부 결과를 확정 전 트랜잭션 영역에 기록
- **3. 위치•상태 스냅숏**: 입력 위치와 연산 상태를 동일 복구점에 영속화
- **4. 체크포인트 완료 확인**: 모든 연산자의 복구점 저장 성공 확인
- **5. 외부 결과 커밋 결정**: 성공한 처리 구간의 결과만 원자적으로 확정
