...
- 배경/필요성: 네트워크는 응답이 사라진 경우와 처리가 실패한 경우를 구별해 주지 않아 재시도가 불가피한데 그때마다 같은 이벤트가 중복 반영되면 결제처럼 되돌릴 수 없는 결과가 어긋나므로, 소스를 재생 가능하게 두고 엔진 상태와 출력 커밋을 하나의 원자 단위로 묶어 중복을 결과 단계에서 흡수할 필요
