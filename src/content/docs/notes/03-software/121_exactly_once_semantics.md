...
- **1. 이벤트·입력 위치 전달**: 처리 대상을 보낸다.
- **2. 결과 사전 반영**: 미확정 영역에 기록한다.
- **3. 위치·상태 스냅숏**: 복구 경계를 저장한다.
- **4. 체크포인트 완료**: 영속 결과를 알린다.
- **5. 결과 커밋 요청**: 외부 확정을 요청한다.
- **6. 외부 결과 확정**: 중복 커밋을 막는다.
- **7. 복구 위치 확정**: 재시작 기준을 정한다.
...
- **중복 비용·손실 비용·Sink 능력**으로 Exactly-once 범위를 정한다.
