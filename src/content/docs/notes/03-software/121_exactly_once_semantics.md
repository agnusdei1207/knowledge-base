...
- **정확히 한 번 처리(Exactly-Once Semantics)**: 재생 가능한 소스와 일관된 상태 및 트랜잭션•멱등 싱크를 연결한 종단 처리 경계이다.
...
- **트랜잭션•멱등 싱크**: 재시도에도 같은 결과를 한 번만 외부 시스템에 반영하는 출력 대상이다.
...
- **Exactly-Once**: 재생 가능한 입력과 원자 커밋을 결합해 중복과 누락을 모두 통제하는 처리 보장이다.
...
| 처리 보장 | 정확히 한 번 처리(Exactly-Once) | 최소 한 번 처리(At-Least-Once) | 최대 한 번 처리(At-Most-Once) |
|:---|:---|:---|:---|
...
- **2단계 커밋(Two-Phase Commit, 2PC)•트랜잭셔널 아웃박스**: 상태와 외부 결과의 준비•확정을 조정하거나 변경과 발행 레코드를 한 거래에 저장하는 방식이다.
- **결정값 기록•결정적 연산**: 현재 시각•난수 같은 값을 원본에 남기고 같은 입력에서 항상 같은 출력을 만들도록 하는 재생 통제이다.
...
| 상태와 외부 출력의 커밋 시점 차이로 **부분 확정** | **2단계 커밋(Two-Phase Commit, 2PC)•트랜잭셔널 아웃박스** 기반 결과 확정 | 실패 시 **상태•출력 일관성** 유지 |
...
- **정확히 한 번 처리(Exactly-Once)**: 중복과 손실 비용이 모두 클 때 원자 커밋 가능한 범위에 적용하는 처리 보장이다.

</details>

- 중복•손실 비용이 모두 크면 **원자 커밋** 가능 범위에 **정확히 한 번 처리(Exactly-Once)** 적용
