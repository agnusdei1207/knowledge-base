...
- 배경/필요성: 분산 네트워크 환경에서 일시적 타임아웃 및 노드 크래시 발생 시 단순 재시도로 인해 데이터가 중복 처리(At-Least-Once)되거나 유실(At-Most-Once)되어 금융 결제, 원장 이체 및 재고 정산에서 심각한 데이터 불일치가 발생하는 문제를 극복하고, 재생 가능한 소스(Kafka Offset), 분산 상태 스냅샷(Flink ABS), 2단계 커밋(2PC) 또는 멱등성(Idempotency) Sink를 유기적으로 결합하여 **시스템 장애 및 재시도 상황에서도 유실과 중복 없이 결과가 정확히 단 1회(Exactly-Once)만 반영되도록 보장**할 필요
