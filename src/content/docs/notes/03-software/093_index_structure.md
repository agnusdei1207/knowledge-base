...
- 정의/개념: 검색 키·행 위치로 **접근 범위를 축소**
- **배경/필요성**: 전체 탐색의 읽기량과 지연 감소
...
- **1. 조건·필요 열 전달**: 검색 요구를 보낸다.
- **2. 값 분포·선택도 조회**: 통계를 요청한다.
- **3. 통계 반환**: 예상 행 수 자료를 제공한다.
- **4. 선택한 키·범위 탐색**: 비용이 낮은 경로를 쓴다.
- **5. 일치 엔트리 반환**: 후보 행 위치를 보낸다.
- **6. 행 위치·페이지 조회**: 원본 데이터를 읽는다.
- **7. 조건에 맞는 행 반환**: 최종 결과를 전달한다.
...
- **연산자·선택도·열 순서·쓰기 비용**으로 인덱스를 정한다.
