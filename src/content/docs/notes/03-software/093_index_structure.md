...
- **복합 인덱스**: 여러 열을 정해진 순서로 결합해 함께 쓰이는 조회 조건을 지원하는 인덱스이다.
- **B+트리 인덱스**: 균형 트리와 연결된 리프 노드로 등치•범위•정렬 조회를 지원하는 인덱스이다.
- **해시 인덱스**: 해시 버킷을 직접 찾아 등치 조건에 특화되지만 범위와 정렬 활용은 제한되는 인덱스이다.
...
- **값 분포•실행 계획**: 조건값의 빈도와 옵티마이저가 선택한 실제 접근 경로를 함께 확인하는 자료이다.
- **등치•범위•정렬 순서**: 복합 인덱스의 선두 열부터 질의의 같은 값 조건, 구간 조건, 정렬 요구를 배치해 검증하는 기준이다.
- **미사용•중복 인덱스**: 실행에 쓰이지 않거나 다른 인덱스와 같은 선두 열•기능을 가져 쓰기 비용만 더하는 구조이다.
- **핵심 질의만 포함 열**: 테이블 재접근을 줄일 가치가 큰 조회에 한해 결과 열을 인덱스 리프에 추가하는 원칙이다.
- **통계 갱신•실행 계획 감시**: 현재 데이터 분포로 통계를 다시 만들고 계획과 실제 행 수의 편차를 관찰하는 활동이다.
...
- **연산자•선택도•열 순서•쓰기 비용**: 인덱스를 추가하거나 유지할 때 종합적으로 평가해야 하는 판단 요소이다.
