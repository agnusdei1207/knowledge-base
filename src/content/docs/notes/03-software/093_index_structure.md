...
- **정의**: 테이블 검색 속도 향상을 위해 특정 컬럼의 키와 행 위치 포인터(`ROWID` / `PK`)를 정렬된 자료구조로 생성하여 디스크 I/O를 최소화하는 물리적 보조 구조인 **데이터베이스 인덱스(Database Index)**.
- **필요성**: 대용량 RDBMS에서 전체 탐색(Full Table Scan)에 따른 쿼리 지연 해소 및 $\mathcal{O}(\log N)$ 단위의 빠른 탐색과 연산(`WHERE`, `JOIN`, `ORDER BY`) 최적화.
...
- **데이터베이스 인덱스 수립 기준 적용** (대용량 OLTP 튜닝 시 선택도 높은 컬럼 중심 B+Tree 및 복합 인덱스 필수 수용)
