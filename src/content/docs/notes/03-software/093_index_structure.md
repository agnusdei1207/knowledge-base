...
- 정의/개념: **인덱스**는 검색 키와 행 위치를 연결해 접근 범위를 줄인다.
- 배경/필요성: 전체 탐색은 데이터 증가에 따라 페이지 읽기와 지연이 급증한다.
...
- **B+트리**: 등치•범위 탐색과 정렬 활용이다.
- **해시**: 버킷 기반 등치 탐색에 특화이다.
- **복합 인덱스**: 열 순서와 질의 조건 일치이다.
...
선의 의미: 옵티마이저는 질의 조건과 통계 정보를 함께 사용해 인덱스 구조의 비용을 판단하고, 인덱스 구조는 리프 엔트리의 키·행 위치·포함 열을 이용한다.

| 구성요소 | 책임 |
|:---|:---|
| 질의 조건 | **질의 조건**으로 열•연산자•정렬•필요 열 제공 |
| 통계 정보 | **통계 정보**로 행 수•값 분포•선택도 제공 |
| 옵티마이저 | **옵티마이저**가 접근 경로 예상 비용 비교 |
| 인덱스 구조 | **인덱스 구조**의 정렬 키•해시 버킷으로 후보 탐색 |
| 리프 엔트리 | **리프 엔트리**가 키•행 위치•포함 열 연결 |
...
- **통계•선택도 분석**: 값 분포와 술어로 예상 행 수를 계산하는 단계이다.
- **접근 경로 비용 비교**: 전체 탐색과 후보 인덱스의 비용을 비교하는 단계이다.
- **인덱스 키 범위 탐색**: 선택한 인덱스에서 후보 행 위치를 찾는 단계이다.
- **비포함 열 추가 조회**: 인덱스에 없는 결과 열을 테이블에서 읽는 단계이다.
...
- **1. 통계•선택도 분석**: **통계•선택도 분석**으로 값 분포와 술어의 예상 행 수를 계산한다.
- **2. 접근 경로 비용 비교**: **접근 경로 비용 비교**로 전체 탐색과 후보 인덱스를 비교한다.
- **3. 인덱스 키 범위 탐색**: **인덱스 키 범위 탐색**으로 리프에서 후보 행 위치를 확인한다.
- **4. 비포함 열 추가 조회**: **비포함 열 추가 조회**로 커버하지 못한 열만 읽는다.
...
- **인덱스 유형 선택 기준**: 등치•범위•정렬•다중 열 조건에 맞는 색인 구조를 고르는 기준이다.
...
| 핵심 특징 | **B+트리**의 정렬 리프 | **해시 인덱스**의 버킷 직접 탐색 | **복합 인덱스**의 지정 열 순서 |
| 한계 | 페이지 분할•쓰기 비용 | 범위•정렬 활용 제한 | 선두 열 조건에 민감 |

> 요약: **인덱스 유형 선택 기준**과 실행 계획으로 복합 인덱스 사용을 결정한다.
...
- **값 분포(Value Distribution)**: 열의 조건값별 빈도를 나타내는 통계이다.
- **실행 계획(Execution Plan)**: 옵티마이저가 선택한 실제 접근 경로이다.
...
- **미사용 인덱스(Unused Index)**: 실제 질의 실행에 쓰이지 않는 인덱스이다.
- **중복 인덱스(Redundant Index)**: 다른 인덱스와 같은 선두 열과 기능을 가진 인덱스이다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 낮은 선택도 조건에 인덱스를 강제해 무작위 읽기 증가 | **선택도**•**값 분포**•**실행 계획** 확인 | 저선택도 인덱스 강제 방지 |
| 복합 인덱스 열 순서가 술어•정렬과 불일치 | **등치 조건**•**범위 조건**•**정렬 순서**로 검증 | 복합 인덱스 활용 확대 |
| 미사용•중복 인덱스가 모든 쓰기의 유지 비용 증가 | **미사용 인덱스**•**중복 인덱스** 제거 | 변경 유지 비용 감소 |
...
- **열 순서(Column Order)**: 복합 인덱스에서 키 열을 배치한 순서이다.
- **쓰기 비용(Write Cost)**: 데이터 변경 때 인덱스를 함께 갱신하는 비용이다.
- **인덱스 설계 유지 기준**: 질의 연산자와 복합 열 순서 및 쓰기 비용을 함께 평가하는 기준이다.

</details>

- **인덱스 설계 유지 기준**에 따라 **연산자**가 등치 전용이면 **해시**, 범위•정렬 조건이면 **B+트리**를 선택하고 **열 순서**와 **쓰기 비용**을 검증한다.

#### 한줄 요약

- **인덱스 설계 유지 기준**으로 실제 조회 이득과 변경 부담을 확인한 뒤 색인을 남긴다.
