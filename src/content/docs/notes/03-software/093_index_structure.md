...
- **1. 통계•선택도 분석**: 값 분포와 술어로 예상 행 수 계산
- **2. 접근 경로 비용 비교**: 전체 탐색과 후보 인덱스 비교
- **3. 인덱스 키 범위 탐색**: 리프에서 후보 행 위치 확인
- **4. 비포함 열 추가 조회**: 커버하지 못한 열만 조회
...
> 요약: 복합 인덱스는 **선두 열 조건•실행 계획** 으로 사용 결정
...
- 등치 전용은 **해시**, 범위•정렬•복합 조건은 **B+트리** 선택
