...
- 정의/개념: 검색 키와 행 위치로 **접근 범위를 줄이는 자료구조**이다.
- 배경/필요성: 전체 탐색은 데이터 증가에 따라 **페이지 읽기•지연 급증**이 발생한다.
...
- **통계•선택도 분석**: 값 분포와 술어로 예상 행 수를 계산하는 단계다.
- **접근 경로 비용 비교**: 전체 탐색과 후보 인덱스의 비용을 비교하는 단계다.
- **인덱스 키 범위 탐색**: 선택한 인덱스에서 후보 행 위치를 찾는 단계다.
- **비포함 열 추가 조회**: 인덱스에 없는 결과 열을 테이블에서 읽는 단계다.
...
- **1. 통계•선택도 분석**: 값 분포와 술어로 예상 행 수를 계산한다.
- **2. 접근 경로 비용 비교**: 전체 탐색과 후보 인덱스를 비교한다.
- **3. 인덱스 키 범위 탐색**: 리프에서 후보 행 위치를 확인한다.
- **4. 비포함 열 추가 조회**: 커버하지 못한 열만 조회한다.
...
> 요약: 복합 인덱스는 **선두 열 조건•실행 계획**으로 사용을 결정한다.
...
- 등치 전용은 **해시**, 범위•정렬•복합 조건은 **B+트리**를 선택한다.
