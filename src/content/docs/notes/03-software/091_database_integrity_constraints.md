...
- 정의/개념: 데이터베이스 내 결함이나 이상 데이터 저장을 방지하기 위해 DDL 수준에서 데이터의 유효성, 식별성, 참조 관계 규칙을 강제하는 **무결성 제약(Integrity Constraints)** 메커니즘.
- 배경/필요성: 애플리케이션 검증 로직 파편화에 따른 무결성 누락 방지 및 다중 시스템 접근 시 일관된 무결성 샌드박스 구축 필요.
...
- **중앙집중식 엔진 통제(Centralized Engine Enforcement)**: DBMS 레벨의 통합 규칙 준수
- **선제적 런타임 차단(Pre-Validation & Rollback)**: 입력/수정 시점 위반 차단
- **선언적 DDL 활용**: **`NOT NULL, UNIQUE, PK, FK, CHECK`** 제약 선언
...
│  • 기본키(PK)     │  • 외래키(FK)     │  • NOT NULL        │
│  • 유니크(Unique) │  • 연쇄(Cascade)  │  • 타입/CHECK      │
├───────────────────┴───────────────────┴────────────────────┤
│ 4. 사용자 정의 무결성 (User-Defined: 트리거/스토어드 프로시저)│
└────────────────────────────────────────────────────────────┘
```
...
- **무결성 수립 체계 적용** (상용 RDBMS 설계 시 PK/FK/NOT NULL 제약 및 FK 인덱스 100% 수용)
