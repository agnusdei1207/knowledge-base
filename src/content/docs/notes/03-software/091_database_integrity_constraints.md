...
- **PK•UNIQUE•FK**: 행의 대표 식별자인 기본키(Primary Key), 후보값 중복을 막는 유일 제약, 부모 행을 가리키는 외래키(Foreign Key)이다.
...
- **DB 불변식•화면 검증 역할**: 모든 저장 경로가 지켜야 할 규칙은 데이터베이스에 두고 사용자 편의용 즉시 안내는 화면에 두는 책임 분리이다.
- **관계별 삭제•갱신 정책**: 부모 키 변경 시 자식 행을 제한•연쇄•NULL 처리할지 관계마다 정하는 규칙이다.
- **사전 진단•정제•단계 활성화**: 기존 위반 데이터를 찾고 고친 뒤 새 제약을 점진적으로 검증•강제하는 적용 절차이다.
- **참조•유일 키 인덱스**: 외래키 대상 탐색과 유일성 검사를 빠르게 지원하는 인덱스이다.
