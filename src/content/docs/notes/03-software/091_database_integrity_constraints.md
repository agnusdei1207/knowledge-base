...
- 정의/개념: 저장 시 **값·식별·관계 규칙을 강제**
- **배경/필요성**: 모든 입력 경로의 최종 규칙 통일
...
- **1. 데이터 변경 요청**: 삽입·수정·삭제를 전달한다.
- **2. 타입·NULL·CHECK 검사**: 값 규칙을 확인한다.
- **3. PK·UNIQUE 중복 확인**: 식별 키를 조회한다.
- **4. 키 검사 결과 반환**: 중복 여부를 알린다.
- **5. FK 부모 행 확인**: 참조 대상을 찾는다.
- **6. 참조 검사 결과 반환**: 관계 유효성을 알린다.
- **7. 반영 결과·오류 응답**: 통과한 변경만 저장한다.
...
- **값·키·관계 규칙과 변경 영향**으로 DB 제약을 설계한다.
