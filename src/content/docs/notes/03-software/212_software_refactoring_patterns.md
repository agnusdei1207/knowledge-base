...
- 정의/개념: 동작을 보존하며 구조를 개선하는 **리팩터링 패턴**이다.
- 배경/필요성: 코드 악취 누적은 **변경 비용•회귀 위험**이 증가한다.
...
- **작은 변환•즉시 회귀 검사**로 실패 범위를 축소한다.
- **코드 악취별 변환 패턴**으로 변경 비용 감소한다.
- 긴 함수는 **메서드 추출**, 책임 불일치는 **함수 이동**을 적용한다.
- 유형별 반복 분기는 **조건문 다형성 치환**을 적용한다.
...
1. **동작 고정**: 특성화 테스트로 **현재 계약을 기록**한다.
2. **악취 선택**: 변경 비용의 **구조적 원인**을 선정한다.
3. **작은 변환**: 원인에 맞는 **패턴 하나를 적용**한다.
4. **회귀 검증**: 전후 **동작 계약을 비교**한다.
5. **변경 확정**: 계약 일치 시 **변경을 저장**한다.
...
- **리팩터링 적용**: 외부 동작을 보존하면서 내부 구조와 변경 비용을 개선한다.
- **기능 변경(Feature Change)**: 새로운 요구에 맞춰 외부에서 관찰되는 동작을 추가하거나 수정하는 변경한다.
...
> 요약: 동작 보존은 **리팩터링**, 전체 교체는 **재작성**을 선택한다.
...
- **프로파일링(Profiling)**: 실행 시간•호출 빈도•자원 사용을 측정해 성능 병목과 회귀를 찾는 분석한다.
...
- **동작 계약을 먼저 고정**하고 고빈도 코드 악취부터 **작은 변환 적용**이 핵심이다.
