...
- 기존 한계: 코드 악취 누적은 **변경 비용·회귀 위험** 증가
...
| 절차 | 설명 |
|:---|:---|
| 동작 고정 | 특성화 테스트로 **현재 계약 기록** |
| 악취 선택 | 변경 비용의 **구조적 원인** 선정 |
| 작은 변환 | 원인에 맞는 **패턴 하나 적용** |
| 회귀 검증 | 전후 **동작 계약 비교** |
| 변경 확정 | 계약 일치 시 **변경 저장** |
| 변경 되돌림 | 계약 불일치 시 **이전 상태 복구** |
...
- 외부 동작을 유지하면서 코드의 변경 용이성을 높이기 위해 **중복·긴 메서드·결합도·테스트 안전망·변경 빈도**를 검토하고, 코드 냄새에 맞는 리팩터링 패턴을 점진 적용한다
