...
- 정의/개념: 외부 동작을 보존하며 구조를 개선하는 **소프트웨어 리팩터링(Software Refactoring)**이다.
- 배경/필요성: **코드 악취(Code Smell)** 누적은 변경 비용과 회귀 위험을 높인다.
...
- **외부 동작 계약 보존**: 입력•출력•오류 같은 외부 동작은 유지하고 내부 구조만 작은 단계로 바꾸는 리팩터링 원칙이다.
...
- **외부 동작 계약 보존**을 전제로 내부 구조 변경이 핵심이다.
- 작은 변환마다 즉시 회귀 검사해 실패 범위를 축소한다.
- 코드 악취에 맞는 변환 패턴으로 변경 비용을 줄인다.
- 긴 함수는 **메서드 추출(Extract Method)**, 책임 불일치는 **함수 이동(Move Function)**을 적용한다.
- 유형별 반복 분기는 **조건문 다형성 치환(Replace Conditional with Polymorphism)**을 적용한다.
...
- **동작 계약**: 리팩터링 전후에 같아야 하는 입력별 출력•오류•처리 순서 등의 관찰 조건이다.
...
| 구성요소 | 책임 |
|:---|:---|
| **코드 악취(Code Smell)** | 구조 개선이 필요한 변경 비용 징후 |
| **동작 계약** | 전후 동일해야 할 외부 관찰 조건 |
| **변환 패턴(Refactoring Pattern)** | 악취 원인별 작은 구조 변경 |
| **회귀 검증(Regression Verification)** | 계약 비교로 동작 보존 판정 |
...
- **4. 회귀 검증**: 변경 전후의 동작 계약이 같은지 확인하는 단계이다.
...
1. **동작 고정**: 특성화 테스트로 현재 계약을 기록한다.
2. **악취 선택**: 변경 비용의 구조적 원인을 선정한다.
3. **작은 변환**: 원인에 맞는 패턴 하나를 적용한다.
4. **회귀 검증**: 전후 동작 계약을 비교한다.
5. **변경 확정**: 계약 일치 시 변경을 저장한다.
...
- **리팩터링 적용**: 외부 동작을 보존하면서 내부 구조와 변경 비용을 개선하는 변경 방식이다.
- **기능 변경(Feature Change)**: 새로운 요구에 맞춰 외부에서 관찰되는 동작을 추가하거나 수정하는 변경 방식이다.
- **재작성(Rewrite)**: 기존 구현을 점진적으로 개선하는 대신 새로운 구현으로 전체 대체하는 방식이다.

</details>

| 코드 변경 방식 | **리팩터링 적용** | **기능 변경(Feature Change)** | **재작성(Rewrite)** |
|:---|:---|:---|:---|
...
> 요약: 동작 보존은 **리팩터링 적용**, 전체 교체는 **재작성(Rewrite)**을 선택한다.
...
- **구조 변경 위험**: 테스트 없이 구조를 바꾸면 숨은 동작 계약이 깨져 이전 기능의 회귀를 발견하지 못하는 위험이다.
...
- **프로파일링(Profiling)**: 실행 시간•호출 빈도•자원 사용을 측정해 성능 병목과 회귀를 찾는 분석 기법이다.

</details>

| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 회귀 시험 없는 **구조 변경 위험** | **특성화 테스트(Characterization Test)**•작은 단위 적용 | 행위 보존 확인 |
| 대규모 일괄 리팩터링 | **추상화에 의한 분기(Branch by Abstraction)** 적용 | 변경 위험 분산 |
| 성능 경로의 회귀 발생 | 전후 벤치마크•**프로파일링(Profiling)** 비교 | 성능 저하 탐지 |
...
- **리팩터링 우선순위**: 동작 계약을 테스트로 고정한 뒤 변경 비용을 크게 만드는 고빈도 코드 악취부터 제거하는 기준이다.
- **점진적 리팩터링(Incremental Refactoring)**: 작은 변환마다 검증하고 확정해 실패 범위와 회귀 원인을 제한하는 접근이다.

</details>

- **리팩터링 우선순위**에 따라 동작 계약을 먼저 고정하고 **점진적 리팩터링(Incremental Refactoring)**으로 고빈도 코드 악취부터 제거한다.
