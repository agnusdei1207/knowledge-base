...
date: "2026-07-31T10:51:44+09:00"
tags: ["notes-software"]
weight: 212
extra:
  question_no: "212"
...
- **추상화에 의한 분기(Branch by Abstraction)**: 기존·신규 구현을 같은 추상화 뒤에 두고 호출을 점진적으로 전환하는 리팩터링 기법
- **벤치마크(Benchmark)**: 변경 전후의 처리 시간·자원 사용량을 같은 조건에서 비교하는 성능 시험
- **프로파일(Profile)**: 실행 중 함수별 시간·메모리 사용량을 측정한 성능 자료

> **키워드:** 소프트웨어 리팩터링 패턴 (Refactoring Patterns)

## Ⅰ. 개요

- 정의/개념: 외부 동작을 보존하며 **내부 코드 구조**를 개선하는 작업
...
- **동작 계약**이 고정되면 고빈도 **코드 악취**부터 작은 변환 적용
