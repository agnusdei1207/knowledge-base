---
sidebar:
  order: 128
  label: "128. Apache Iceberg"
  badge:
    text: "기출 · 30%"
    variant: note
title: "Apache Iceberg"
date: "2026-08-02T12:00:00+09:00"
tags:
  - "notes-software"
weight: 128
extra:
  question_no: "128"
  source_status: "기출"
  source_history: "137회"
  priority: 30
  priority_note: "137회 기출, Iceberg 메타데이터 구조 사례"
---

## Ⅰ. 개요

<details>
<summary>핵심 용어</summary>

- **Apache Iceberg 오픈 테이블 포맷(Open Table Format)**: 스냅숏과 매니페스트로 현재 테이블의 유효 파일을 추적하는 공개 명세이다.

</details>

- 정의/개념: 스냅숏과 **매니페스트** 로 유효 파일을 추적하는 **오픈 테이블 포맷**
- 배경/필요성: 디렉터리 기반 표는 **파일 탐색·동시 커밋·구조 변경** 일관성 제약

#### 한줄 요약

- 폴더를 모두 뒤지지 않고 단계별 목록표로 현재 표에 속한 파일을 찾는다.

## Ⅱ. 특징

<details>
<summary>핵심 용어</summary>

- **필드 식별자(Field Identifier, Field ID)·숨은 파티셔닝(Hidden Partitioning)**: 열 이름과 물리 파티션 경로 변화에서 논리 스키마와 조건 해석을 보호하는 기능이다.
- **스냅숏 격리**: 읽기가 시작한 커밋의 불변 파일 집합을 끝까지 일관되게 보는 특성이다.
- **매니페스트 가지치기(Manifest Pruning)**: 파티션 요약과 열 통계로 조건에 맞지 않는 파일 목록을 계획 단계에서 제외하는 최적화이다.

</details>

- 커밋별 불변 파일 집합을 제공하는 **스냅숏 격리**
- 파티션 요약·열 통계 기반 **매니페스트 가지치기**
- **필드 식별자(Field Identifier, Field ID)·숨은 파티셔닝(Hidden Partitioning)** 기반 스키마·파티션 진화

#### 한줄 요약

- 열 이름이나 저장 칸이 바뀌어도 번호표와 목록표가 같은 데이터를 찾아 주지만 낡은 목록과 파일은 정리해야 한다.

## Ⅲ. 구조 및 구성요소

<details>
<summary>핵심 용어</summary>

- **매니페스트**: 데이터·삭제 파일의 경로와 파티션 값 및 열 통계를 기록하는 불변 메타데이터 파일이다.
- **카탈로그**: 테이블 이름과 현재 메타데이터 파일 위치를 원자적으로 관리하는 구성요소이다.
- **메타데이터 파일**: 스키마·파티션 명세·스냅숏 이력을 저장하는 파일이다.
- **스냅숏·매니페스트 목록**: 특정 커밋에 속한 매니페스트 집합과 파티션 요약을 정의하는 계층이다.
- **데이터·삭제 파일**: 실제 행과 위치 삭제·동등 삭제 조건을 저장하는 불변 파일이다.

</details>

```mermaid
treeView-beta
    root["Iceberg 테이블"]
        A["카탈로그"]
        B["메타데이터 파일"]
            C["스냅숏·매니페스트 목록"]
                D["매니페스트"]
                    E["데이터·삭제 파일"]
```

| 구성요소 | 책임 |
|:---|:---|
| 카탈로그 | 테이블 이름과 현재 **메타데이터 포인터** 관리 |
| 메타데이터 파일 | **스키마·파티션 명세·스냅숏 이력** 저장 |
| 스냅숏·매니페스트 목록 | 커밋별 **매니페스트 집합·파티션 요약** 정의 |
| 매니페스트 | 데이터·삭제 파일의 **경로·열 통계** 기록 |
| 데이터·삭제 파일 | 실제 행과 **위치·동등 삭제** 정보 저장 |

#### 한줄 요약

- 단계별 목록표가 현재 테이블에 속한 파일을 찾아 준다.

## Ⅳ. 흐름도

<details>
<summary>핵심 용어</summary>

- **4. 기준 포인터·새 메타데이터 위치**: 기준 버전이 같을 때만 카탈로그의 현재 포인터를 원자적으로 전환하는 단계이다.
- **1. 기준 메타데이터 포인터 요청**: 쓰기가 출발할 스냅숏과 필드 식별자(Field Identifier, Field ID) 기반 스키마를 가져오는 단계이다.
- **2. 새 데이터·삭제 파일**: 변경 행과 위치·동등 삭제 정보를 새 불변 파일로 기록한 결과이다.
- **3. 매니페스트·스냅숏 메타데이터**: 새 파일의 통계·파티션 정보를 메타데이터 트리에 연결한 정보이다.

</details>

```mermaid
sequenceDiagram
    participant W as 쓰기 엔진
    participant C as 카탈로그·Iceberg 메타데이터
    participant D as 객체 스토리지
    participant R as 읽기 엔진
    W->>C: 1. 기준 메타데이터 포인터 요청
    C-->>W: 스냅숏·스키마 반환
    W->>D: 2. 새 데이터·삭제 파일
    W->>C: 3. 매니페스트·스냅숏 메타데이터
    W->>C: 4. 기준 포인터·새 메타데이터 위치
    C-->>W: 교체·충돌 결과 반환
    R->>C: 테이블 조회
    C-->>R: 확정 스냅숏 포인터
```

**동작 원리**

1. **기준 메타데이터 포인터 요청**: 쓰기가 출발할 스냅숏과 필드 식별자(Field Identifier, Field ID) 기반 스키마 확보
2. **새 데이터·삭제 파일**: 변경 행을 불변 데이터와 위치·동등 삭제 정보로 기록
3. **매니페스트·스냅숏 메타데이터**: 파일 통계와 파티션 정보를 새 메타데이터 트리에 연결
4. **기준 포인터·새 메타데이터 위치**: 기준이 같을 때만 현재 포인터를 원자적으로 전환

#### 한줄 요약

- 새 자료와 목록표를 모두 만든 뒤 카탈로그의 현재 위치표 한 칸만 원자적으로 바꾼다.

## Ⅴ. 종류 및 비교

<details>
<summary>핵심 용어</summary>

- **Apache Iceberg**: 다중 엔진 사용과 스키마·파티션 진화가 필요한 테이블에 적합한 포맷이다.
- **Hive식 파일 테이블**: 디렉터리와 파티션 경로를 테이블 구조로 직접 해석하는 파일 저장 방식이다.

</details>

| 테이블 형식 | Apache Iceberg | Hive식 파일 테이블 |
|:---|:---|:---|
| 적용 기준 | **다중 엔진·구조 진화** 테이블 | 단순 경로 기반 파일 테이블 |
| 핵심 특징 | **스냅숏·매니페스트(Manifest)·필드 식별자(Field Identifier, Field ID)** | 디렉터리·파티션 경로 |
| 한계 | **매니페스트·삭제 파일·스냅숏** 관리 | **동시성·구조 변경** 을 직접 관리 |

#### 한줄 요약

- Hive식 표는 서랍 이름으로 자료를 찾고 Iceberg는 버전 있는 목록표로 자료를 찾는다.

## Ⅵ. 실무 고려사항 및 대책

<details>
<summary>핵심 용어</summary>

- **읽기 병합 비용**: 조회할 때 데이터 파일과 많은 삭제 정보를 함께 적용해야 하는 문제이다.
- **매니페스트 병합**: 작은 매니페스트를 목표 크기로 합쳐 계획 단계의 메타데이터 읽기를 줄이는 처리이다.
- **데이터 파일 재작성**: 작은 파일이나 삭제가 많이 적용된 파일을 새 최적 크기 파일로 다시 만드는 처리이다.
- **스냅숏 보존**: 실행 중 조회와 복구 가능 기간보다 길게 과거 버전과 참조 파일을 유지하는 정책이다.

</details>

| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 매니페스트가 늘면 **쿼리 계획 읽기** 비용 증가 | 목표 크기와 **매니페스트 병합** 주기 설정 | 계획 단계의 **메타데이터 읽기** 감소 |
| 작은 파일 누적으로 **파일 스캔 횟수** 증가 | 쓰기 크기 조정과 **데이터 파일 재작성** | 스캔의 **파일 열기 비용** 감소 |
| 삭제 파일 누적으로 **읽기 병합 비용** 증가 | 삭제 파일 관찰과 **데이터 파일 재작성** | 조회 시 **삭제 적용 비용** 억제 |
| 스냅숏 보존이 짧으면 실행 중인 **과거 파일** 삭제 | 작업·복구 기간 기반 **스냅숏 보존** | 실행 중인 조회와 **과거 버전** 보호 |

#### 한줄 요약

- 사용자는 시각만 검색해도 Iceberg가 맞는 날짜 서랍과 값 범위의 파일만 골라낸다.

## Ⅶ. 결론

<details>
<summary>핵심 용어</summary>

- **Apache Iceberg**: 목록 기반 메타데이터로 구조 진화와 다중 엔진 읽기를 지원하는 테이블 포맷이다.

</details>

- 다중 엔진과 스키마·파티션 진화가 필요하면 **Apache Iceberg** 채택

#### 한줄 요약

- 목록표 덕분에 구조를 안전하게 바꿀 수 있지만 오래된 목록과 쓰지 않는 파일은 계속 정리해야 한다.
