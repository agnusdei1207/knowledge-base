...
date: "2026-07-25T11:26:00+09:00"
tags:
  - "notes-software"
weight: 128
extra:
...
- **정의/개념**: 스냅숏·Manifest로 파일과 스키마를 관리한다
- **배경/필요성**: 경로 탐색의 파일 누락과 충돌을 막는다
...
| 판단 기준 | Apache Iceberg | Hive식 파일 테이블 |
|:---|:---|:---|
| 핵심 특징 | 스냅숏·Manifest로 파일과 통계를 추적함 | 디렉터리·파티션 경로로 파일을 탐색함 |
| 적용 기준 | 다중 엔진·스키마·파티션 진화 | 정적 파티션·단일 엔진 분석 |
| 주요 위험 | 메타데이터 누적·커밋 충돌 | 파티션 변경 시 경로·질의 수정 |
...
1. 숨은 파티션으로 무관한 로그 파일을 제외한다
2. 필드 ID로 공유 테이블의 논리 열을 추적한다

### 쉽게 이해하기 (학습용)
- 로그 질의의 날짜 조건을 Iceberg가 저장 파티션 조건으로 바꾸어 불필요한 파일을 제외한다
- 열 이름을 바꿔도 필드 ID가 같으면 여러 엔진이 기존 열과 같은 데이터로 읽는다

## Ⅶ. 결론

- 다중 엔진·스키마 진화가 필요하면 Iceberg를 택한다
