...
sequenceDiagram
    ...
    W->>C: 1. 기준 메타데이터 포인터 조회
    C-->>W: 스냅숏·스키마 반환
    W->>D: 2. 데이터·삭제 파일 저장
    W->>M: 3. Manifest·스냅숏 작성
    W->>C: 4. 포인터 원자 교체 요청
    C-->>W: 교체·충돌 결과 반환
    R->>C: 5. 확정 스냅숏 조회
```
...
- **1. 기준 메타데이터 포인터 조회**: 쓰기가 출발할 스냅숏과 ID 기반 스키마 확보
- **2. 데이터·삭제 파일 저장**: 변경 행을 불변 데이터·위치·동등 삭제 파일로 기록
- **3. Manifest·스냅숏 작성**: 파일 통계와 파티션 정보를 새 메타데이터 트리에 연결
- **4. 포인터 원자 교체 요청**: 기준 포인터가 그대로일 때만 새 스냅숏으로 전환
- **5. 확정 스냅숏 조회**: 읽기 엔진이 카탈로그 포인터의 일관된 파일 집합 계획
