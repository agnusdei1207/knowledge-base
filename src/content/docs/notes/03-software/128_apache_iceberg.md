...
- 배경/필요성: Hive 방식은 어떤 파일이 테이블에 속하는지를 디렉터리 목록 조회로 판단해 파일 수가 늘수록 스캔 전 준비 비용이 함께 늘고 파티션 기준을 바꾸면 테이블을 통째로 다시 써야 하므로, 파일 목록과 통계를 메타데이터 트리로 미리 정리해 두어 디렉터리 탐색 자체를 없앨 필요
