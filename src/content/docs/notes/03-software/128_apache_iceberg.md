...
- 정의/개념: 스냅숏과 **매니페스트**로 유효 파일을 추적하는 **오픈 테이블 포맷**이다.
- 배경/필요성: 디렉터리 기반 표는 **파일 탐색•동시 커밋•구조 변경** 일관성 제약이 발생한다.
...
- **Field ID•숨은 파티셔닝** 기반 스키마•파티션 진화가 핵심이다.
...
- **1. 기준 메타데이터 포인터 적재**: 스냅숏과 Field ID 기반 스키마를 확보한다.
- **2. 새 데이터•삭제 파일 기록**: 변경 행과 삭제 정보를 불변 파일로 저장한다.
- **3. 매니페스트•스냅숏 구성**: 파일 통계•파티션 정보를 메타데이터에 연결한다.
- **4. 기준 포인터 비교•원자 전환**: 기준이 같을 때만 현재 참조 변경한다.
...
- **매니페스트 병합**: 작은 매니페스트를 목표 크기로 합쳐 계획 단계의 메타데이터 읽기를 줄이는 처리한다.
- **데이터 파일 재작성**: 작은 파일이나 삭제가 많이 적용된 파일을 새 최적 크기 파일로 다시 만드는 처리한다.
...
- 다중 엔진과 스키마•파티션 진화가 필요하면 **Apache Iceberg** 채택이 핵심이다.
