...
date: "2026-08-05T12:37:16+09:00"
tags: [notes-software]
weight: 12
extra:
  question_no: "012"
...
- **대기 큐(Wait Queue)**: 조건이 충족되지 않은 스레드를 보관하는 대기열
- **깨움 신호(Wakeup Signal)**: 상태 변경 뒤 조건 재검사를 요청하는 신호
...
- **프리미티브 선택**: 허가 수•소유권•상태 조건에 맞는 동기화 도구를 정하는 단계
- **획득 조건 검사**: 허가•잠금•조건을 만족해 진입할 수 있는지 확인하는 단계
- **접근 권한 획득**: 세마포어 허가나 뮤텍스•모니터 잠금을 원자적으로 얻는 단계
- **임계 구역 실행**: 보호된 공유 상태를 읽거나 변경하는 단계
- **권한 해제•깨움**: 허가•잠금을 반환하고 대기 스레드에 재검사 기회를 주는 단계
...
- **1. 프리미티브 선택**: 허가 수•소유권•상태 조건으로 결정
- **2. 획득 조건 검사**: 미충족 시 대기 후 조건 재검사
- **3. 접근 권한 획득**: 허가나 잠금을 원자적으로 확보
- **4. 임계 구역 실행**: 보호된 공유 상태 접근
- **5. 권한 해제•깨움**: 권한 반환 후 대기 작업 통지

#### 한줄 요약

- 획득 실패 시 **대기•조건 재검사** 반복
...
- 허가 수는 세마포어, 소유권은 뮤텍스, **상태 조건은 모니터**
...
- **허가 수**: 동시 접근 수가 여러 개면 세마포어를 고르는 기준
- **소유권**: 잠금 해제 주체가 정해지면 뮤텍스를 고르는 기준
- **상태 조건**: 공유 상태 대기가 필요하면 모니터를 고르는 기준
