...
date: "2026-07-25T01:57:00+09:00"
tags: [notes-software]
weight: 12
extra:
  question_no: "012"
...
## Ⅰ. 개요
- **정의/개념**: 진입·대기·해제로 공유 접근 조정
- **배경/필요성**: 동시 접근이 결과를 바꿔 진입 제어 필요
...
- 진입 제어는 경쟁을 막고 대기·교착을 낳는다
- 세마포어는 허가 수만큼 동시 진입을 허용한다
- 뮤텍스는 소유자 한 명만 잠금을 해제한다
- 모니터는 상태·조건을 묶어 오용을 줄인다
...
| 판단 기준 | 세마포어 | 뮤텍스 | 모니터 |
|:---|:---|:---|:---|
| 핵심 특징 | 정수 허가 수·소유권 없음 | 단일 소유자 잠금 | 상태·조건 변수 캡슐화 |
| 적용 기준 | 동시 사용 수를 제한할 때 | 단일 소유권 잠금이 필요할 때 | 상태·조건을 함께 캡슐화할 때 |
| 주요 위험 | 허가 누락·과다 반환 | 교착·소유자 정지 | 조건 신호 누락 |
...
1. 연결 풀은 세마포어 허가 수로 동시 대여 제한
2. 캐시 갱신 구역은 뮤텍스로 단일 스레드만 진입
3. 생산자·소비자 버퍼는 모니터 조건 변수로 대기·신호
