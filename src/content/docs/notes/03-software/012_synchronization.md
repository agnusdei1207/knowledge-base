...
- **경쟁 조건(Race Condition)**: 다중 스레드가 동기화 없이 공유 데이터에 동시 접근하여 접근 순서에 따라 실행 결과가 달라지는 비정상 상태.
- **재진입 락(ReentrantLock)**: 이미 락을 획득한 동일 스레드가 잠금을 다시 요청해도 데드락 없이 중첩 진입을 허용하는 상호 배제 동기화 객체.
</details>
