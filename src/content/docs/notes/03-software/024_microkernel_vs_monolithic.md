...
- **정의/개념**: 운영체제 서비스의 실행 공간 배치 구조
- **배경/필요성**: 서비스 통합이 결함을 전파해 배치 조정 필요
...
- 서비스 배치 위치가 호출 비용·결함 전파 범위 결정한다.
...
| 판단 기준 | 마이크로커널 | 모놀리식 커널 |
|:---|:---|:---|
| 핵심 특징 | IPC로 사용자 서버 호출 | 커널 내부 함수 직접 호출 |
| 적용 기준 | 결함 격리·서비스 재시작 우선 | 호출 지연·처리량 우선 |
| 주요 위험 | IPC·주소 공간 전환 비용 | 결함 전파·시스템 중단 |
...
1. 차량 제어 운영체제는 드라이버를 사용자 서버로 격리
2. 범용 서버 운영체제는 파일·네트워크 서비스를 커널에 통합
