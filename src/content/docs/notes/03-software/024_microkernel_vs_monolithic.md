...
- 정의/개념: 운영체제 서비스를 **커널•사용자 공간**에 다르게 배치하는 커널 구조 유형이다.
- 배경/필요성: 서비스 통합 시 단일 결함이 **커널 전체로 전파**되어 시스템 중단이 발생한다.

#### 한줄 요약

- 운영체제 서비스의 **커널•사용자 공간 배치** 차이 핵심이다.
...
- **서비스 실행 공간**이 호출 경로•결함 전파 범위를 결정한다.
- 마이크로커널은 **최소 커널•사용자 서버 격리**를 제공한다.
- 모놀리식은 **커널 내부 직접 호출**로 처리 지연 절감이 핵심이다.

#### 한줄 요약

- 사용자 서버 격리와 **IPC 전환 비용**의 절충이 핵심이다.
...
- 서비스 분리와 통합의 **정적 주소 공간 구조**가 핵심이다.
...
- **시스템 호출(System Call)**: 응용이 운영체제 서비스를 요청하는 공식 진입 경로다.
- **프로세스 간 통신(Inter-Process Communication, IPC) 서비스 요청**: 마이크로커널이 분리된 사용자 서버로 작업을 전달하는 처리다.
- **사용자 서버 처리**: 격리된 주소 공간의 서버가 파일•장치•네트워크 기능을 수행하는 처리다.
- **IPC 응답**: 사용자 서버 결과를 마이크로커널 경유로 응용에 돌려주는 처리다.
- **커널 내부 직접 호출**: 모놀리식 커널의 서비스가 같은 주소 공간에서 협력하는 처리다.
...
- 마이크로커널은 IPC 경유, 모놀리식은 **직접 호출**이 핵심이다.
...
- 격리•재시작은 마이크로커널, **호출 성능은 모놀리식**이 핵심이다.
...
- IPC 최적화•감독•**TCB 최소화** 기반 운영이 핵심이다.
...
- 서비스 단위 복구가 필요하면 **마이크로커널**, 낮은 호출 지연이 우선이면 **모놀리식 커널**을 선택한다.

#### 한줄 요약

- 서비스 단위 복구와 호출 지연으로 **커널 구조 선택**이 핵심이다.
