...
date: "2026-07-30T23:19:44+09:00"
tags:
  - "notes-software"
weight: 24
extra:
...
- **마이크로커널(Microkernel)**: 주소 공간·스케줄링·프로세스 간 통신만 커널에 두고 운영체제 서비스를 사용자 공간에 분리하는 구조
- **모놀리식 커널(Monolithic Kernel)**: 운영체제 서비스를 하나의 커널 주소 공간에 통합하는 구조
- **사용자 공간 서버(User-space Server)**: 파일·장치·네트워크 서비스를 커널과 분리된 주소 공간에서 실행하는 프로세스
...
- **상태 체크포인트(State Checkpoint)**: 재시작 뒤 서비스를 복구할 수 있도록 필요한 상태를 저장한 시점별 사본
- **공격면(Attack Surface)**: 공격자가 접근하거나 악용할 수 있는 인터페이스와 코드의 전체 범위

> **키워드:** 마이크로커널 vs 모놀리식 커널 (Microkernel vs Monolithic)
