...
- 배경/필요성: 하이브리드·멀티 클라우드 환경 전반에 걸쳐 데이터 저장소와 테이블 수가 기하급수적으로 증가함에 따라 필요한 데이터셋을 찾지 못하는 데이터 늪(Data Swamp) 현상, 비즈니스 용어 정의 불일치 및 데이터 소유권·계보(Lineage) 불명확 문제를 해결하기 위해, 전사 데이터 자산의 기술/비즈니스 메타데이터를 자동 수집(Harvesting)·색인하고 데이터 계보 시각화와 거버넌스를 통합 제공하는 데이터 카탈로그(Data Catalog)를 도입하여 **데이터 탐색성(Discoverability)을 극대화하고 전사 데이터 자산화와 컴플라이언스 준수를 실현**할 필요
...
- 엔터프라이즈 데이터 민주화(Data Democratization) 및 AI 거버넌스 수립의 **가장 핵심적인 전사 메타데이터 통합 플랫폼**으로 확립되었으며, 실무 구축 시에는 **CDC 기반 DDL 변경 실시간 동기화로 메타데이터 노후화(Stale) 방지, OpenLineage 표준을 통한 엔드투엔드 파이프라인 계보(Lineage) 자동 추적, 비즈니스 용어집(Business Glossary)과 도메인 오너십 연계, LLM 기반 PII 자동 태깅 거버넌스**를 결합하여 데이터 검색 생산성과 보안 통제력을 동시 극대화
