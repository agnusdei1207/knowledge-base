...
외부 클라이언트의 HTTP 주문 생성 요청 인입
        │
   1. 인바운드 어댑터(REST Controller): 요청 DTO 검증 및 CreateOrderCommand 생성
        │
   2. 인바운드 포트(OrderUseCase) 인터페이스 호출
        │
   3. 도메인 서비스가 비즈니스 규칙(재고 확인, 가격 계산)을 순수 POJO로 수행
        │
   4. 도메인이 아웃바운드 포트(OrderRepositoryPort SPI) 인터페이스 호출
        │
   5. 아웃바운드 어댑터(JPA Adapter): 도메인 객체를 JPA Entity로 매핑 후 DB 커밋
```
...
- 복잡한 비즈니스 규칙과 장기적 진화가 요구되는 엔터프라이즈 마이크로서비스는 **헥사고날 아키텍처(Ports & Adapters)** 를 채택하여 도메인 순수성과 100% 테스트 자동화 확립
