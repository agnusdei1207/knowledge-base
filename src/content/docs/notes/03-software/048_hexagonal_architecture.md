...
> 요약: 핵심이 포트를 소유하고 기술은 어댑터에 격리

### 쉽게 이해하기 (학습용)

- 본체 양쪽의 공통 소켓은 대화 규격을 정하고 플러그는 외부 장치별 연결 방식을 맡는다
...
| 절차 | 설명 |
|:---|:---|
| 외부 요청 | 웹·메시지·테스트 입력 전달 |
| 유스케이스 호출 | 입력을 변환해 인바운드 포트 호출 |
| 외부 기능 요청 | 핵심이 아웃바운드 포트 호출 |
| 기술 호출 | 포트 요청을 외부 기술 형식으로 변환 |
| 기술 응답 | 외부 시스템 처리 결과 반환 |
| 외부 결과 반환 | 기술 결과를 포트 결과로 변환 |
| 유스케이스 결과 | 핵심의 업무 처리 결과 전달 |
| 외부 응답 | 결과를 호출자 형식으로 변환 |

> 요약: 외부 형식은 어댑터, 업무 규칙은 핵심에서 처리
...
1. 결제 서비스는 결제 포트와 업체별 어댑터로 기술 격리
...
- 외부 기술 교체·격리 검증이 반복되는 경계에만 포트 적용
