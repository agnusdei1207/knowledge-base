...
- **계층형 아키텍처(Layered Architecture)**: 표현·업무·데이터 계층이 위에서 아래 기술을 호출하는 구조로서 외부 연동이 고정된 단순 처리에 적합한다.
- **결제 업체(Payment Provider)**: 외부 결제 기술과 계약을 제공하며 업체별 아웃바운드 어댑터가 공통 결제 포트로 변환하는 시스템

## Ⅰ. 개요

- 정의/개념: 핵심 로직을 포트로 격리하고 어댑터로 연결하는 **구조**
- 배경/필요성: 외부 기술 변경의 **핵심 업무 로직 전파** 차단
...
> 요약: 핵심이 포트를 소유하고 어댑터가 외부 형식을 변환
...
> 적용 사례: 결제 서비스는 결제 포트와 업체별 어댑터로 외부 결제 기술을 격리
...
- **외부 기술 교체·격리 검증**이 반복되면 포트·어댑터를 적용
