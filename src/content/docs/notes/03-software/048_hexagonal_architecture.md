...
- 배경/필요성: 전통적인 계층형 아키텍처(Layered Architecture)의 데이터베이스 중심 하향식 의존성, 프레임워크 어노테이션 침투로 인한 도메인 로직 오염 및 인프라 종속으로 인한 단위 테스트 지연 문제를 극복하고, 핵심 비즈니스 로직(POJO)을 중심에 두고 외부 입출력 기술을 포트(Port)와 어댑터(Adapter) 인터페이스로 완전 격리하여 **의존성 역전(DIP) 기반의 기술 독립성, 프레임워크 교체 유연성 및 초고속 단위 테스트 환경을 확보**할 필요
...
#### 한줄 요약
- 도메인 연산은 어댑터를 거치지 않고는 외부에 닿지 못하므로, 인프라 교체가 어댑터 교체로 끝나고 도메인 코어는 다시 검증할 필요가 없다.
...
- 도메인 주도 설계(DDD) 및 클린 아키텍처 구현을 위한 **표준 도메인 격리 아키텍처 패턴**으로 확립되었으며, 실무 엔터프라이즈 환경에서는 **순수 도메인 모델과 JPA 영속성 엔티티의 분리, MapStruct를 활용한 계층 간 객체 매핑 자동화, 도메인 복잡도에 따른 선별적 도입(핵심 도메인 중심 헥사고날 적용)**을 통해 개발 생산성과 장기적 유지보수성의 균형을 달성
