...
date: "2026-07-25T01:47:00+09:00"
tags: [notes-software]
weight: 7
extra:
  question_no: "007"
...
- **정의/개념**: 작업 우선순위로 마감 준수 통제
- **배경/필요성**: 마감 위반이 가치를 없애 완료 시각 통제 필요
...
- 결과와 마감 준수를 함께 정확성으로 판정한다
- 최악 실행시간 기반 분석이 예측성을 보장한다
- 수용 분석은 마감 불가 작업을 사전에 거부한다
- 선점은 긴급 작업의 응답 지연을 줄인다
...
| 판단 기준 | RM | EDF |
|:---|:---|:---|
| 핵심 특징 | 짧은 주기의 고정 순위 | 이른 절대 마감의 동적 순위 |
| 적용 기준 | 고정 주기·정적 우선순위 검증 | 가변 마감·높은 CPU 이용률 |
| 주요 위험 | 보수적 이용률·낮은 순위 위반 | 과부하 시 마감 위반 연쇄 |
...
1. 산업 제어 주기 태스크에 RM·최악 응답시간 분석
2. 미디어 서버 변환 작업은 절대 마감시간으로 EDF 정렬
