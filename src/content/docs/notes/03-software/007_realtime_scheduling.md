...
sequenceDiagram
    ...
    participant A as 수용·마감 분석기
    participant Q as 절대 마감 준비 큐
    ...
    J->>Q: ⑥ 더 이른 마감 작업 방출
    Q->>S: ⑦ 우선순위 변경 통지
    S->>C: ⑧ 현재 작업 선점·새 작업 실행
    C-->>S: ⑨ 새 작업 완료
    S->>Q: ⑩ 완료 작업 제거·다음 작업 선택
    S->>C: ⑪ 중단 작업 재개
    C-->>A: ⑫ 완료 시각·마감 결과 전달
```
...
| 설계 요소 | 입력·상태 | 역할 |
|:---|:---|:---|
| 스케줄 가능성 분석기 | 최악 실행시간·주기·마감 | 수용 가능 여부 판정 |
| 우선순위 준비 큐 | 방출 작업·주기·절대 마감 | RM·EDF 기준 실행 후보 정렬 |
| 선점 스케줄러 | 최상위 작업·현재 실행 상태 | 디스패치·선점·완료 시각 기록 |

단일 CPU의 독립·선점 가능 주기 태스크이고 \(D_i=T_i\)인 경우 이용률은 다음과 같다.

$$
U=\sum_{i=1}^{n}\frac{C_i}{T_i}
$$

RM의 고전적 충분조건과 EDF의 이용률 조건은 각각 다음과 같다.

$$
U\le n\left(2^{1/n}-1\right),\qquad U\le1
$$

- \(n\)은 태스크 수, \(i\)는 태스크 번호, \(C_i\)는 \(i\)번째 태스크의 WCET, \(T_i\)는 주기, \(U\)는 전체 CPU 이용률이다.
- RM 경계 초과가 곧 실행 불가능을 뜻하지는 않으므로 최악 응답시간 분석으로 추가 판정한다.
- 차단·문맥 전환·인터럽트 비용, \(D_i\ne T_i\), 비주기 작업, 다중 CPU는 별도 분석 모델이 필요하다.

**동작 원리**

- ① 신규 작업 집합의 실행 가능성 분석 요청
- ② 수용된 작업 집합을 준비 큐에 등록
- ③ 주기 또는 사건에 따른 작업 인스턴스 방출
- ④ 절대 마감시간이 가장 이른 작업 선택
- ⑤ 선택한 작업을 CPU에 디스패치
- ⑥ 더 이른 마감시간을 가진 작업 도착
- ⑦ 새 작업 도착에 따른 동적 우선순위 변경
- ⑧ 현재 작업 선점 후 새 작업 실행
- ⑨ 새 작업 실행 완료
- ⑩ 완료 작업 제거 후 다음 작업 선택
- ⑪ 선점되었던 작업의 실행 재개
- ⑫ 완료 시각과 마감시간을 비교해 마감 미스 기록
...
- 산업 제어 태스크는 주기별 고정 우선순위 RM을 적용하고 WCET·차단·상위 태스크 간섭을 포함한 최악 응답시간을 검증한다.
...
- 실시간 태스크의 마감시간을 보장하려면 **주기 고정성·우선순위 변동·최악 응답시간**을 검토하고, 정적 검증에는 RM, 동적 마감에는 EDF를 선택한다.
