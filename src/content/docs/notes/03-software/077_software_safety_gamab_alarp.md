...
- 배경/필요성: 위험 수용의 공학적 기준 부재 시 발생하는 **과잉 투자로 인한 프로젝트 좌초 및 안전 불감증에 따른 대형 참사 해결 불가**
...
| 안전 원칙/영역 | 핵심 정의 | 공학적 조치 및 판정 기준 |
|:---|:---|:---|
| **GAMAB 원칙** | 신규 시스템의 총 위험도가 **기존 시스템보다 적어도 같거나 우수함을 입증** | 이전 버전 사고 통계 대조 및 안전 무결성 동등성 증명 |
| **불허용 (Intolerable)** | 위험도가 너무 높아 **사회적으로 절대 수용 불가능한 영역** | 시스템 가동 중단, 아키텍처 재설계 및 위험 원천 제거 |
| **합리적 저감 (ALARP)** | 위험 저감 비용이 편익보다 현저히 크지 않다면 **지속 저감** | FMEA/FTA 대책 수립, Fail-Safe 이중화 적용 |
| **광범위 허용 (Acceptable)** | 위험 발생 확률과 피해가 극히 미미한 영역 | 추가 비용 투입 없이 **잔여 위험(Residual Risk) 공식 수용** |
...
위험원 식별(Hazard Identification) 및 위험도 산정 (FMEA, FTA 분석)
        │
   1. [불허용 검사] 위험도가 Intolerable 영역인가? -> (예: 즉시 아키텍처 재설계 및 위험 제거)
        │ (아니오)
   2. [GAMAB 검사] 기존 운영 시스템 대비 동등 이상 안전한가? -> (미달 시 보완 조치)
        │
   3. [ALARP 저감] 추가 안전 장치(이중화, 소프트웨어 감시 타이머) 도입 비용편익 분석
        │
...
잔여 위험 수용  위험도 추가 강하
        │
   4. 독립 안전 평가원(ISA)에게 Safety Case 제출 및 최종 안전 인증(Sign-off)
```
...
- 철도, 자동차, 항공 등 생명 중심 임베디드 시스템 구축 시 **GAMAB 원칙으로 기존 대비 동등 이상 안전성을 입증하고 ALARP 원칙으로 합리적 위험 저감선을 설정**하여, **정량적 Safety Case 기반의 안전 무결성** 달성
