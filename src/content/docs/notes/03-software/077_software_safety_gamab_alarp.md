...
date: "2026-07-25T18:09:00+09:00"
tags:
  - "notes-software"
weight: 77
extra:
...
- **정의/개념**: 동등성·저감 합리성으로 잔여 위험 수용
- **배경/필요성**: 위험 저감 중단 판단 차이로 수용 기준 필요
...
- 참조 시스템의 운용 통계가 GAMAB 비교 기준선이 된다.
- 저감 비용이 편익에 심하게 불균형할 때만 중단한다.
- 정량 확률과 정성 근거를 함께 사용해 위험을 판정한다.
- 위험원·안전 요구·검증 증거를 안전 사례로 추적한다.
...
| 설계 요소 | 설명 |
|:---|:---|
| 위험 평가 | 발생 가능성·피해 심각도로 위험 판정 |
| 안전 요구 | 탐지·차단·다중화 저감 방안 정의 |
| 저감 검증 | 안전 요구 구현·저감 효과 확인 |
| 안전 사례 | 평가·저감·검증 근거로 안전 입증 |
...
| 저감 효과 검증 | 안전 요구 구현과 위험 감소 확인 |
| 잔여 위험 제출 | 저감 후 위험과 근거를 안전 사례화 |
...
| 비교축 | GAMAB | ALARP |
|:---|:---|:---|
| 핵심 특징 | 기존 기준과 안전 동등성 입증 | 합리적 범위까지 위험 저감 |
| 판단 방식 | 참조 시스템의 위험 수준과 비교함 | 저감 효과와 비용의 불균형을 비교함 |
| 적용 기준 | 신뢰할 참조 시스템이 존재할 때 | 추가 저감의 합리성을 판단할 때 |
| 주요 위험 | 부적합 참조·차이 누락 | 비용 과대평가·잔여 위험 정당화 |
...
1. 철도 신호를 운용 통계와 비교해 안전 동등성 입증
2. 차단 밸브 비용·위험 저감 가치로 수용 판정

### 쉽게 이해하기 (학습용)
- 철도 신호는 기존 운용 통계와 새 시스템 위험을 비교해 안전 동등성을 입증한다
- 화학 플랜트는 차단 밸브의 위험 저감 편익과 비용이 현저히 불균형한지 판단한다

## Ⅶ. 결론

- 참조 동등성·저감 합리성으로 잔여 위험 수용
