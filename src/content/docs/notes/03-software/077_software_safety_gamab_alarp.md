...
- 배경/필요성: 철도·자동차·원자력 등 안전 필수(Safety-Critical) 시스템에서 모든 잔여 위험을 0으로 만드는 절대적 무결함(Zero Risk) 달성이 공학적·경제적으로 불가능함에도 명확한 허용 위험 기준 없이 안전 대책을 수립할 경우 발생하는 과잉 투자 또는 치명적 안전 대책 누락을 방지하기 위해, 기존 레퍼런스 시스템 대비 동등 이상의 안전성을 요구하는 GAMAB과 위험 저감 비용이 편익 대비 현저히 불균형하지 않는 한 합리적으로 낮추는 ALARP 원칙을 통해 **공학적으로 합리적이고 객관적인 시스템 안전 기준선 및 허용 잔여 위험 거버넌스를 확립**할 필요
...
- 글로벌 기능 안전(IEC 61508, ISO 26262 등) 및 철도/원자력 공학의 **핵심 위험 통제 및 허용 위험(Tolerable Risk) 판단 원칙**으로 확립되었으며, 실무 시스템 구현 시에는 **불허용(Intolerable) 영역의 결함 원천 차단, ALARP 영역에서의 비용편익 기반 안전 장치(이중화·Watchdog) 최적화, 독립 안전 평가원(ISA)의 제3자 검증 및 GSN 기반 Safety Case(안전성 보증 사례) 구축**을 결합하여 무결점 기능 안전을 달성
