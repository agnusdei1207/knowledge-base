...
date: "2026-07-25T11:06:00+09:00"
tags:
  - "notes-software"
weight: 118
extra:
...
- **정의/개념**: 배치 결과와 실시간 증분 결과를 결합한다
- **배경/필요성**: 배치 지연 중에도 최신 결과를 제공한다
...
- 이벤트를 두 계층에 보내 최신성과 재계산을 함께 얻는다
- 배치 계층은 전체 원본으로 오류 없는 뷰를 재생성한다
- 속도 계층은 최신 구간만 계산해 응답 지연을 줄인다
- 새 배치 뷰가 임시 결과를 대체해 누적 오차를 없앤다
- 이중 로직은 배포 비용과 결과 불일치 위험을 높인다
...
| 설계 요소 | 설명 |
|:---|:---|
| 이벤트 수집·불변 원본 | 이벤트를 추가형 원본으로 보존 |
| 배치 계층 | 전체 원본으로 배치 뷰를 재생성 |
| 속도 계층 | 최신 이벤트로 실시간 뷰를 갱신 |
| 제공 계층·배치 뷰 | 배치 결과를 색인·교체해 제공 |
| 실시간 뷰·조회 계층 | 배치·증분 결과를 결합해 응답 |
...
| 절차 | 설명 |
|:---|:---|
| 이중 전달 | 이벤트를 원본·속도 계층에 전달 |
| 실시간 증분 | 최신 배치 이후 이벤트만 계산 |
| 배치 계산 | 전체 원본으로 결과를 재생성 |
| 배치 교체 | 새 배치 뷰로 임시 결과를 대체 |
| 결과 결합 | 배치 뷰와 남은 증분 결과를 결합 |
...
|:---|:---|:---|
| 핵심 특징 | 배치 계층과 속도 계층을 병행함 | 하나의 스트림 처리 경로를 사용함 |
| 적용 기준 | 전체 재계산·실시간 뷰가 모두 필요 | 단일 스트림에서 실시간 분석할 때 |
| 주요 위험 | 이중 로직·결과 불일치 | 재생 처리량 한계·긴 상태 복구 |
...
1. 대시보드는 배치·실시간 뷰를 조회 때 결합한다
2. 특성 집계는 불변 원본으로 배치 뷰를 재생성한다
