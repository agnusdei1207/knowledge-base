...
- **그래프QL(Graph Query Language, GraphQL)**: 타입 스키마로 계약을 정의하고 클라이언트가 선택한 필드를 실행하는 응용 프로그래밍 인터페이스 질의 언어이자 실행 명세이다.
...
- **선택 집합·계층 리졸버**: 클라이언트가 요청한 필드 트리와 각 필드 값을 단계적으로 계산하는 함수 구조이다.
- **부분 오류·점진 폐기**: 성공한 필드 데이터와 실패 경로를 함께 반환하고 사용 중인 필드를 공지 후 단계적으로 제거하는 진화 방식이다.
...
- **게이트웨이·검증기**: 인증과 질의 구문·타입·깊이·비용 조건을 검사하는 구성요소이다.
...
- **그래프QL(Graph Query Language, GraphQL)**: 화면마다 필요한 연관 필드를 선택해 한 요청으로 조합하는 응용 프로그래밍 인터페이스 방식이다.
...
- **오류 경로·코드**: 부분 실패가 발생한 필드 위치와 기계 판독 가능한 실패 종류를 표현하는 정보이다.
