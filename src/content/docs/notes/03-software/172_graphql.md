...
- 배경/필요성: 다양한 모바일 디바이스 및 프론트엔드 화면마다 요구하는 데이터 구조가 상이한 환경에서, 전통적인 REST API 사용 시 발생하는 불필요한 필드 과다 수신(Over-fetching)과 연관 데이터를 얻기 위해 여러 엔드포인트를 연속 호출해야 하는 다중 왕복 지연(Under-fetching) 문제를 해결하기 위해, 클라이언트가 원하는 데이터 구조를 선언적으로 명시하고 단일 엔드포인트에서 강타입 스키마(SDL) 기반으로 정확히 필요한 필드만 조립 반환하는 GraphQL을 도입하여 **네트워크 대역폭 낭비를 최소화하고 프론트엔드 개발 생산성을 극대화**할 필요
...
- 복잡한 연관 데이터를 다루는 대고객 모바일/웹 프론트엔드(BFF 계층) 및 다기종 클라이언트 환경의 **핵심 데이터 페칭(Fetching) 표준 플랫폼**으로 정립되었으며, 실무 구축 시에는 **계층형 리졸버의 N+1 쿼리 폭증을 방어하는 DataLoader 배치/캐싱, 악의적 중첩 질의를 차단하는 Max Depth/Query Complexity 제한, HTTP 캐싱 한계를 극복하는 Persisted Queries(APQ) 적용, MSA 환경의 스키마 통합을 지원하는 Apollo Federation**을 결합하여 백엔드 부하를 통제하면서 클라이언트 중심의 최적 UX를 완성
