...
- 정의/개념: 타입 스키마 기반 **클라이언트 선택형 API 질의**
- 배경/필요성: 화면별 과다 응답·반복 호출의 **데이터 조합 개선**
...
| Gateway·검증기 | 인증·구문·**질의 비용 검증** |
| 실행 엔진 | 선택 집합·**널·오류 전파** |
| Resolver·Data Loader | 필드 권한·**배치 조회·캐시** |
| 데이터 원천 | DB·REST·**업무 서비스 처리** |
...
sequenceDiagram
    ...
    V-->>E: 3. 실행 계획 전달
    E->>D: 4. 권한·배치 조회
    E-->>C: 5. data·errors 반환
```

**동작 원리**

1. **작업·변수 전달**: 선택 필드·입력·인증 문맥 제공
2. **스키마·비용 검증**: 타입·깊이·복잡도 제한 검사
3. **실행 계획 전달**: 검증된 선택 집합·문맥 구성
4. **권한·배치 조회**: 필드별 허용 확인·N+1 완화
5. **data·errors 반환**: 선택 구조·부분 오류 조합
...
| 고려사항 | 대책 | 효과 |
|:---|:---|:---|
| 깊은 질의의 **처리량 폭증** | 깊이·노드·**가중 복잡도 제한** | CPU·DB 과부하 방지 |
| 필드별 반복의 **N+1 조회** | 요청 범위 **Data Loader 적용** | 원천 호출 수 감소 |
