...
- **그래프QL(GraphQL)**: 타입 스키마로 계약을 정의하고 선택한 필드를 실행하는 API 질의 언어이자 실행 명세이다.

</details>

- 정의/개념: 타입 스키마에 따라 클라이언트가 필요한 필드를 선언하고 서버가 리졸버로 결과를 구성하는 **그래프QL(Graph Query Language, GraphQL)** 질의 언어•실행 명세
...
- **데이터 원천**: 데이터베이스•표현 상태 전이 서비스•업무 서비스에서 실제 값을 제공하는 구성요소이다.
...
- **1. 검증된 실행 계획 전달**: 타입•깊이•비용 조건을 통과한 선택 집합과 변수로 실행 계획을 구성하는 단계이다.
- **2. 필드 해석•데이터 로딩 계획**: 필드별 권한•하위 선택 순서와 일괄 조회할 키를 결정하는 단계이다.
...
- **내성(Introspection)**: 실행 중인 스키마의 타입•필드 구조를 질의로 조회하는 그래프QL 기능이다.
