...
sidebar:
  ...
  badge:
    ...
    variant: note
title: "Apache Spark"
date: "2026-07-25T11:04:00+09:00"
tags:
  - "notes-software"
weight: 117
extra:
...
- **방향성 비순환 그래프(Directed Acyclic Graph, DAG)**: 영문 첫 글자를 딴 DAG를 '디에이지'로 읽으며 연산 의존성을 순환 없는 방향 간선으로 표현한 그래프임
...
- **정의/개념**: DAG 기반 배치·SQL·스트림을 분산 실행한다
- **배경/필요성**: 중간 결과 재사용으로 반복 시간을 줄인다
...
- 액션 호출은 지연 연산을 DAG로 확정해 실행을 시작한다
- 셔플 경계는 단계를 나눠 네트워크 비용을 만든다
- Catalyst·AQE는 계획을 바꿔 처리 시간을 줄인다
- 캐시·계보 재계산은 반복과 장애 복구 시간을 줄인다
- 워터마크는 상태를 정리해 메모리 고갈을 막는다
...
| 설계 요소 | 설명 |
|:---|:---|
| Driver·SparkSession | 작업 생성과 전체 실행을 조정 |
| Catalyst·AQE | 통계로 논리·물리 계획을 최적화 |
| Scheduler·Cluster Manager | 단계 분할·자원 할당·태스크 배치 |
| Executor·Partition | 파티션 태스크·캐시·셔플 처리 |
| Checkpoint·State Store | 스트림 진행 위치·상태를 저장 |
...
| 절차 | 설명 |
|:---|:---|
| 지연 정의 | 필터·조인 등 지연 연산을 구성 |
| 액션 호출 | 결과 요청으로 실행 계획을 확정 |
| 계획 최적화 | 통계로 물리 연산·파티션 조정 |
| 태스크 배치 | 단계별 파티션 작업을 배치 |
| 태스크 실행 | 실행자가 파티션 연산을 수행 |
...
| 비교축 | Apache Spark | Hadoop MapReduce |
|:---|:---|:---|
| 핵심 특징 | 연산 의존성을 DAG로 구성함 | Map과 Reduce 단계를 파일로 연결함 |
| 반복 처리 | 캐시한 파티션을 재사용함 | 이전 작업 출력을 다시 읽음 |
| 적합 작업 | 반복 분석·SQL·상태 기반 스트림 | 파일 기반 대용량 일괄 정렬·집계 |
| 주요 위험 | 메모리 압박·셔플 편향·상태 증가 | 반복 디스크 I/O·긴 지연 |
...
1. 편향 키를 나눠 조인 파티션을 재분배한다
2. 워터마크·체크포인트로 스트림 상태를 제한한다
