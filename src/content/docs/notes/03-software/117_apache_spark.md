...
- 배경/필요성: 1세대 Hadoop MapReduce가 다단계 파이프라인 및 머신러닝/그래프 반복 연산 시 매 단계마다 중간 결과를 HDFS 디스크에 강제 플러시함으로써 유발하는 심각한 I/O 지연과 네트워크 오버헤드를 극복하고, 중간 데이터를 메인 메모리(RAM) 상에 유지하면서 노드 장애 시 계보(Lineage)를 통해 유실 파티션을 즉시 재계산·복원하는 RDD(Resilient Distributed Dataset)와 DAG 실행 엔진, Catalyst 최적화기 및 Tungsten 코드 생성을 도입하여 **빅데이터 배치, 대화형 SQL, 스트리밍 및 머신러닝 연산 속도를 최대 100배 가속**할 필요
