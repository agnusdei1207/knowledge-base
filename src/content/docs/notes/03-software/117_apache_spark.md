...
- 배경/필요성: MapReduce의 단계별 디스크 기록은 반복 분석마다 **입출력 지연**을 유발한다.
...
- **계획 보정**: Catalyst•AQE 기반 실행을 최적화한다.
...
- **1. Catalyst 초기 계획**: 지연 연산의 실행 그래프를 최적화한다.
- **2. DAG•Stage 구성**: 셔플 경계별 실행 단계를 생성한다.
- **3. 파티션 Task 실행**: Executor에 파티션 작업을 배치한다.
- **4. 실행 통계 수집**: 실제 행 수•셔플량 관측이다.
- **5. AQE 계획 보정**: 조인 방식•파티션 수 재선택한다.
...
- 반복 배치는 **Spark Batch**, 연속 증분 처리는 **Structured Streaming**을 선택한다.
