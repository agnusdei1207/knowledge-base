...
- 배경/필요성: GC는 참조가 남은 객체를 회수하지 못하므로 의도치 않은 강한 참조 하나가 요청마다 힙을 조금씩 잠식하다 결국 할당 실패라는 되돌릴 수 없는 비용으로 터지는데, Full GC 이후 기저선 추세와 참조 그래프를 보는 관측 수단으로 그 누적을 조기에 드러낼 필요
