...
date: "2026-07-29T18:18:00+09:00"
tags: ["notes-software"]
weight: 211
extra:
  question_no: "211"
...
- **메모리 누수(Memory Leak)**: 더는 필요하지 않은 객체·자원이 참조에 남아 회수되지 않고 점유량이 누적되는 결한다.
...
- **네이티브 누수(Native Leak)**: 힙 밖의 버퍼·파일·라이브러리 자원을 해제하지 않아 메모리가 누적되는 결한다.
...
block
    columns 1
    A["메모리 계측기 | 힙·네이티브·GC·할당 추이 수집"]
    B["덤프·프로필 수집기 | 시점별 객체·참조·할당 기록"]
    C["참조·점유 분석기 | 보유 크기·도미네이터 판정"]
    D["수명주기 제어 | 리스너·스레드 로컬 참조 해제"]
    E["용량·상한 제어 | 캐시·큐·동시 작업·힙 제한"]
    A --- B
    B --- C
    C --- D
    D --- E
...
5. **수정·상한 검증**: 장기 부하의 **기준선·GC·OOM** 확인
