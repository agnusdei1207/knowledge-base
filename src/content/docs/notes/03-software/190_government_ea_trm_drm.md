...
- 정의/개념: 기관 자산을 공통 분류하는 **정부 EA 참조모형**이다.
- 배경/필요성: 기관별 분류는 명칭•기준 불일치로 **중복 투자 분석** 제약이 발생한다.
...
- **BRM•SRM** 기반 업무•서비스 공통을 분류한다.
- **DRM•TRM** 기반 데이터•기술 표준화가 핵심이다.
- **PRM•성과 지표** 기반 투자 성과 추적한다.
- **상호운용성•메타데이터** 기반 기관 간 교환이 핵심이다.
- **중복•공유 후보** 기반 투자를 통제한다.
...
- **1. 기관 자산•공통 코드 매핑**: 자산을 참조모형 코드와 근거에 연결하는 단계이다.
- **2. 매핑 검증•유사 자산 탐색**: 기존 공통 서비스•표준과 대조하는 단계이다.
- **3. 범정부 자산 통합 분석**: 중복•공유•상호운용 후보를 찾는 단계이다.
- **4. 통합•공유•전환•예외 결정**: 투자와 표준화 조치를 확정하는 단계이다.
...
1. **기관 자산•공통 코드 매핑**: 참조모형 코드•근거를 연결한다.
2. **매핑 검증•유사 자산 탐색**: 공통 서비스•표준을 대조한다.
3. **범정부 자산 통합 분석**: 중복•공유•상호운용을 분석한다.
4. **통합•공유•전환•예외 결정**: 투자•표준화 조치를 확정한다.
...
- **DRM 역할**: 데이터 주제•요소•의미•형식과 메타데이터를 분류한다.
- **TRM 역할**: 기술 서비스•플랫폼•표준과 전환 기준을 분류한다.
...
- 데이터 의미 차이는 **DRM**, 기술 호환•전환은 TRM으로 매핑하고 투자 심사에 반영한다.
