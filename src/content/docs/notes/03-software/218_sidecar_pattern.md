...
- 배경/필요성: 폴리글랏(Polyglot) 마이크로서비스 환경에서 로깅, 모니터링 텔레메트리, mTLS 상호 인증, 서킷 브레이커 등의 인프라 횡단 관심사(Cross-Cutting Concerns)를 각 애플리케이션 프레임워크 언어별 SDK로 중복 구현하고 패치 시마다 전사 서비스를 재컴파일·재배포해야 하는 막대한 운영 오버헤드를 극복하기 위해, 메인 컨테이너 소스코드 변경 없이 동일 파드(Pod) 내의 보조 컨테이너로 인프라 기능을 격리 배포하는 사이드카(Sidecar) 패턴을 도입하여 **언어 독립적 기능 주입, 비침투적 확장 및 인프라-비즈니스 수명주기의 완전한 분리**를 달성할 필요
...
- 1. 트래픽 가로채기: 요청을 **사이드카**로 전달
- 2. 보안·정책 검증: mTLS와 속도 제한 검사
- 3. 로컬 전달: 검증 요청을 localhost로 전달
- 4. 비즈니스 처리: 메인 앱의 도메인 로직 수행
- 5. 텔레메트리 기록: 지표·추적 정보를 수집
...
- 쿠버네티스(K8s) 및 서비스 메시(Istio/Envoy) 아키텍처의 근간을 이루며 비즈니스 로직과 인프라 플랫폼 기능을 분리하는 **가장 핵심적인 클라우드 네이티브 컨테이너 디자인 패턴**으로 확립되었으며, 실무 구축 시에는 **K8s 1.28+ Native Sidecar(`restartPolicy: Always`)를 통한 기동/종료 수명주기 보장, 사이드카 컨테이너별 엄격한 Resource Limits 격리를 통한 OOM 방지, 대규모 클러스터 메모리 부담 시 eBPF 기반 Ambient Mesh(사이드카리스) 전환 검토**를 결합하여 개발 민첩성과 클러스터 인프라 효율성을 완벽히 조화
