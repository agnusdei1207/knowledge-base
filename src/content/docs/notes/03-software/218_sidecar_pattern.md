...
date: "2026-08-26T13:32:39+09:00"
tags:
  - "notes-software"
weight: 218
extra:
...
- 배경/필요성: 인프라 기능을 언어별 SDK로 앱에 심으면 언어 수만큼 중복 구현하고 패치마다 전 서비스를 재빌드·재배포하는 비용이 재발하므로, 같은 파드 안에 별도 프로세스를 두어 그 기능을 앱 밖으로 빼내 독립 갱신이 가능하게 할 필요
...
- 앱별 격리가 필요하면 **사이드카**, 자원 부담이 크면 공유 계층
