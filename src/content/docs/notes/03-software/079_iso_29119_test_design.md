...
- 정의/개념: 테스트 근거를 조건•커버리지 항목•케이스•절차로 변환하고 추적하는 **ISO/IEC/IEEE 29119 표준 설계 체계**이다.
- 배경/필요성: 경험 의존 테스트는 **범위 누락•결과 재현할 수 없다**.

#### 한줄 요약

- 근거•조건•케이스•절차 기반 **표준 테스트 설계**가 핵심이다.
...
- **위험 수준**에 따라 설계 기법•자원 우선 배정이 핵심이다.
- **요구•조건•케이스•결과**의 양방향을 추적한다.
- **조건•케이스•절차 분리**로 재사용•재현성을 확보한다.

#### 한줄 요약

- 위험 기반 우선순위와 **요구•결과 양방향 추적**이 핵심이다.
...
- **1. 테스트 베이시스 분석**: 요구•설계•위험 식별자에서 검증 대상 근거 수집이다.
- **2. 테스트 조건•설계 기법**: 위험과 조건 특성에 맞는 기법을 선택한다.
- **3. 케이스•절차•추적 링크**: 입력•오라클•실행 순서와 근거를 연결한다.
- **4. 커버리지 공백•잔여 위험**: 요구•조건•케이스 누락을 관리자에게 제공한다.
- **5. 고위험 보강 순위•종료 기준**: 위험 기반 보강과 완료 조건 확정이다.

> 요약: 테스트 근거에서 조건•케이스•절차를 도출하고 결과까지 양방향을 추적한다.

#### 한줄 요약

- 근거 도출부터 **공백•잔여 위험 판정까지 추적**이 핵심이다.
...
> 요약: 빠른 탐색은 **비형식**, 반복•감사 추적은 **표준 설계**를 선택한다.
...
- **위험•반복성•감사 증거**에 따라 설계 상세도와 추적성을 결정한다.

#### 한줄 요약
- 위험•반복성•감사 요구 기반 **상세도•추적성 결정**이 핵심이다.
