...
- 배경/필요성: 조직/국가마다 제각각이었던 테스트 용어 및 설계 서식의 파편화 해소, 정량적 커버리지 측정 및 양방향 요구 추적성(Traceability) 확립 요구성
...
## Ⅲ. 구조 및 구성요소 (ISO 29119 Part 4 테스트 설계 기법 분류)
...
## Ⅳ. 흐름도 (ISO 29119-2 테스트 설계 4단계 프로세스)
...
```text
[1. Test Basis 분석] ──► [2. Test Condition 도출] ──► [3. Test Case 설계] ──► [4. Test Procedure 생성]
  (요구사항/SRS 수거)      (테스트 조건 식별)            (입력값/기대출력 정의)    (테스트 스크립트화)
```

### 동작 원리

1. **Test Basis Analysis**: 요구사항 명세서, 아키텍처 문서 및 위험 분석서 수거.
2. **Derive Test Conditions**: 테스트 대상 기능과 품질속성 조건(Test Condition) 식별.
3. **Design Test Cases**: ISO 29119-4 기법(경계값 등)을 적용하여 입력값(Input)과 기대출력(Expected Result)을 포함한 Test Case 생성.
4. **Build Test Procedures**: 테스트 케이스를 실행 순서대로 그룹화하여 실제 수행 스크립트(Test Procedure)로 생성.

#### 한줄 요약

- 테스트 베이시스 분석부터 커버리지 공백•잔여 위험 평가까지의 추적이 핵심이다.

## Ⅴ. 종류 및 비교 (ISO 29119 vs 기존 IEEE 829 표준)
...
- **ISO 29119 수용 기준**에 따라 전사 테스트 검증 체계 고도화 시 **ISO/IEC/IEEE 29119 (Part 2, Part 4)** 필수 수용
