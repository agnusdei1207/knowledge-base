...
- ① 업무 요청을 가치·우선순위·완료 조건이 있는 카드로 등록
- ② 다음 단계가 현재 진행 중 작업 수와 한도 조회
- ③ 한도 아래이면 새 작업을 당길 수 있는 여유 용량 확인
- ④ 다음 단계가 상위 백로그의 우선순위 카드 요청
- ⑤ 우선순위 작업 카드를 다음 단계로 이동
- ⑥ 운영팀이 작업을 수행하고 명시한 완료 조건 검증
- ⑦ 카드의 완료·차단·단계별 체류시간 기록
- ⑧ 작업 노화와 병목을 바탕으로 한도·당김 정책 개선
...
- 고객 지원팀은 긴급 처리 통로의 동시 수를 제한하고 나머지 요청은 WIP 여유가 생길 때 우선순위대로 당긴다.
...
- 유입 변동·병목 위치·납기 목표를 기준으로 WIP 한도와 당김 규칙을 선택한다.
