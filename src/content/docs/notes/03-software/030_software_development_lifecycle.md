...
- ① 요구 범위·수용 기준·우선순위 합의
- ② 승인된 범위와 요구 추적 식별자를 개발 조직에 전달
- ③ 요구를 반영한 설계·코드·시험 산출물 제출
- ④ 수용 기준 검증 결과와 남은 위험·감사 증적 보고
- ⑤ 기준 충족 시 릴리스하고 미충족 시 결함·재작업 범위 반환
- ⑥ 운영 조직이 서비스 지표와 사용자 피드백 전달
- ⑦ 피드백을 개선 요구로 반영하거나 종료 시 폐기·이관 기준 제안
...
- 금융시스템은 요구별 설계·코드·시험 결과와 승인자를 추적표로 연결해 변경 영향과 감사 근거를 남긴다.
...
- 규제 강도·변경 빈도·운영 위험에 맞춰 단계·산출물·피드백 통제를 설계한다.
