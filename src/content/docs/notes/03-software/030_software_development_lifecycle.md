...
- 배경/필요성: 프로젝트 재작업(Rework) 비용 소멸, 비가시적 소프트웨어 개발의 산출물 가시성(Visibility) 확보 및 품질 표준 준수 요구성
...
| SDLC 단계 | 주요 수행 활동 | 핵심 산출물 (Artifacts) |
|:---|:---|:---|
| **1. 요구사항 분석** | 도메인 분석, 사용자 요구 추출 및 타당성 검토 | 요구사항 명세서(SRS), 유스케이스 명세서 |
| **2. 시스템 설계** | 아키텍처 설계, 모듈화, 데이터베이스 설계 | 아키텍처 정의서(SAD), ERD, Class Diagram |
| **3. 구현 (Coding)** | 코딩 표준 준수, 단위 테스트, 정적 분석(SAST) | 소스코드, 단위테스트 결과서, SAST 리포트 |
| **4. 검증 (Testing)** | 통합/시스템/수용 테스트, 결함 추적 | 테스트 시나리오, 결함 리포트, 수용 확인서 |
| **5. 유지보수/폐기** | 형상 관리, 패치, SLO 모니터링, 안전 폐기 | SLA 보고서, 변경 이력서, 데이터 폐기 증적 |
