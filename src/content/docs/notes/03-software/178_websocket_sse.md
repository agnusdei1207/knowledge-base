...
- 정의/개념: 지속 연결 기반 **WebSocket•SSE 실시간 통신 방식**이다.
- 배경/필요성: 반복 조회 주기가 짧으면 **요청 부하 증가**, 길면 **상태 갱신 지연**이 발생한다.
...
- **역압(Backpressure)**: 소비자의 처리 속도보다 생산이 빠를 때 큐•전송률•연결을 제한해 자원 고갈을 막는 제어한다.

</details>

- **지속 연결•즉시 전송** 기반 갱신 지연을 축소한다.
- **프레임•이벤트 스트림** 기반 메시지 교환이 핵심이다.
- **하트비트•재연결•역압** 기반 연결 운영이 핵심이다.
...
- **통신 방향•형식•재접속**으로 WebSocket•SSE을 결정한다.
