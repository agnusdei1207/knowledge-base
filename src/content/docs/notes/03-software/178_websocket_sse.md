...
- 정의/개념: WebSocket **전이중**·SSE **단방향 통신**
- 배경/필요성: 반복 조회의 **요청 부하·갱신 지연 감소**
...
| 구성요소 | 책임 |
|:---|:---|
| 실시간 Gateway | 인증·**연결 종료·라우팅** |
| WebSocket Handler | 전환·프레임·**양방향 세션** |
| SSE Handler | 이벤트 형식·**ID·재시도 처리** |
| Message Broker | 서버·구독자 **메시지 중계** |
| Replay Store | 순번별 보존·**누락 이벤트 재생** |
...
1. **연결·마지막 ID 요청**: 인증·프로토콜·복구 위치 제공
2. **누락 이벤트 조회**: 마지막 확인 이후 보존 사건 검색
3. **재생 이벤트 반환**: 순번 기반 유실 구간 복구
4. **신규 사건 전달**: 브로커 구독 사건의 연결 배분
5. **프레임·이벤트 전송**: 방향·형식에 맞춘 실시간 송신
