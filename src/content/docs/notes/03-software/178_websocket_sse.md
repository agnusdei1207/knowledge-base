...
- 배경/필요성: 기존의 전통적인 HTTP 폴링(Short/Long Polling) 방식에서 발생하는 무의미한 주기적 요청 오버헤드, HTTP 헤더 중복 전송에 따른 대역폭 낭비 및 실시간 데이터 갱신 지연 문제를 해결하기 위해, 클라이언트-서버 간 단일 지속 연결을 수립하고 전이중(Full-Duplex) 양방향 프레임 통신을 지원하는 WebSocket과 표준 HTTP/HTTPS 인프라 위에서 서버 주도 단방향 푸시 스트리밍을 제공하는 SSE(Server-Sent Events)를 도입하여 **초저지연 실시간 양방향 상호작용 및 효율적인 실시간 데이터 푸시 아키텍처를 완성**할 필요
