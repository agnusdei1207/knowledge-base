...
- **정의/개념**: 배포·데이터 경계로 변경 독립성을 조절
- **배경/필요성**: 배포 결합과 분산 비용이 상충해 경계 선택 필요
...
- 모놀리식은 내부 호출·단일 트랜잭션이 단순하다.
- 부분 변경에도 전체 배포·확장 필요하다.
- 마이크로서비스는 독립 배포·확장 가능하다.
- 분산 호출·일관성·장애 처리 비용 증가한다.
...
| 적용 기준 | 변경·확장 주기가 비슷할 때 | 업무별 변경·확장 주기가 다를 때 |
| 주요 위험 | 전체 배포·확장 결합 | 분산 일관성·연쇄 장애 |
...
1. 초기 사내 업무 시스템은 모듈러 모놀리스로 단일 배포
2. 상품 조회는 전용 저장소와 런타임으로 분리
