...
- 정의/개념: 모놀리스는 여러 업무를 하나의 배포 단위로 묶고, 마이크로서비스는 업무 경계별 독립 서비스로 나눠 **변경•확장 독립성**을 조절하는 아키텍처 선택한다.
- 배경/필요성: 업무별 변경•확장 주기와 **분산 운영 비용**에 따른 구조 선택이 필요하다.

#### 한줄 요약

- 변경•확장 독립성과 **분산 운영 비용의 구조 선택**이 핵심이다.
...
- **원격 호출(Remote Call)**: 네트워크로 다른 서비스에 보내는 호출한다.
- **단일 트랜잭션(Single Transaction)**: 하나의 저장소 경계에서 여러 변경을 모두 반영하거나 모두 취소하는 처리 단위이다.

</details>

- 모놀리식은 **내부 호출•단일 트랜잭션**으로 운영 단순화가 핵심이다.
- 마이크로서비스는 **독립 배포•확장**으로 장애 범위 격리가 핵심이다.
- **원격 호출•분산 일관성**이 서비스 분리 비용 증가한다.

#### 한줄 요약

- 내부 호출 단순성과 **독립 배포 비용의 절충**이 핵심이다.
...
| 독립 서비스군 | **업무별 배포•확장**과 장애 격리 |
| 전용 데이터 저장소군 | 서비스별 **스키마•변경 권한** 소유 |
| 요청 라우터 | 기존•신규 **처리 경로 선택** |

#### 한줄 요약

- 단일 배포와 독립 서비스의 **정적 구조 비교**가 핵심이다.
...
> 요약: **변경•확장 주기와 운영 역량**으로 배포 구조를 결정한다.

#### 한줄 요약

- 결합도•경계 성숙도•운영 역량 기반 **구조 선택**이 핵심이다.
...
> 요약: **독립성 이익**이 **분산 비용** 보다 클 때 서비스를 분리한다.
...
- 모듈 경계 검증과 **점진 분리•분산 운영 통제**가 핵심이다.
...
- **독립 배포 이익**이 **분산 운영 비용** 보다 클 때만 서비스를 분리한다.

#### 한줄 요약

- 독립성 이익이 분산 비용보다 클 때 **서비스 분리**가 핵심이다.
