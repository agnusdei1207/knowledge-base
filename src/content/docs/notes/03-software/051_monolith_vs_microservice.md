...
date: "2026-08-02T23:01:00+09:00"
tags:
  - "notes-software"
weight: 51
extra:
...
- **내부 호출·원격 호출**: 내부 호출은 같은 프로세스 안의 함수를 직접 실행하고, 원격 호출은 네트워크로 다른 서비스에 요청한다.
...
- **회로 차단·시간 제한(Circuit Breaker·Timeout)**: 반복 실패 호출을 잠시 차단하고 원격 응답의 최대 대기시간을 제한하는 장애 격리 제어이다.
- **대사·허용 지연(Reconciliation·Allowed Lag)**: 서비스 상태를 비교해 불일치를 정정하고 비동기 상태 차이를 허용할 최대 시간을 정하는 통제이다.
