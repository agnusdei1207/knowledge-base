...
- 정의/개념: **모놀리식**은 여러 업무를 하나의 배포 단위로 묶고, **마이크로서비스**는 업무 경계별 독립 서비스로 나눈다.
- 배경/필요성: 비즈니스 복잡도 증가에 따라 단일 배포 아키텍처의 빌드/테스트 병목과 분산 서비스 관리의 오버헤드 간 선택 절충이 발생한다.

#### 한줄 요약

- **모놀리식**과 **마이크로서비스**는 변경•확장 독립성과 분산 운영 비용이 다르다.
...
- 모놀리식은 **내부 호출**과 **단일 트랜잭션**으로 운영을 단순화한다.
- 마이크로서비스는 **독립 배포**로 장애 범위를 격리한다.
- **원격 호출**과 **분산 일관성**은 서비스 분리 비용을 증가시킨다.

#### 한줄 요약

- **내부 호출**, **독립 배포**, **분산 일관성**의 절충이 핵심이다.
...
| 구성요소 | 책임 |
|:---|:---|
| 단일 애플리케이션 | 모든 업무를 하나의 배포 단위로 운영 |
| 공용 데이터 저장소 | **공용 데이터 저장소**에서 트랜잭션 경계 공유 |
| 독립 서비스군 | 업무별 배포•확장과 장애 격리 |
| 전용 데이터 저장소군 | **전용 데이터 저장소**에서 스키마•변경 권한 소유 |
| 요청 라우터 | **API** 요청의 기존•신규 처리 경로 선택 |

#### 한줄 요약

- **공용 데이터 저장소**, **전용 데이터 저장소**, **API**의 구조 차이가 핵심이다.
...
- **변경•확장 주기 결합**: 여러 업무 기능을 함께 변경•배포•확장해야 하는 정도이다.
- **트랜잭션 결합**: 여러 업무 변경이 하나의 원자적 커밋 경계를 요구하는 정도이다.
- **서비스 경계 성숙도**: 업무 책임과 데이터 소유권을 독립 서비스로 나눌 만큼 검증한 정도이다.
- **분산 운영 역량**: 원격 장애•일관성•관측성을 운영할 조직과 플랫폼의 능력이다.
...
[업무별 변경•확장•데이터 책임 분석]
          ...
          +---------+---------+
          |                   |
  주기•트랜잭션 결합 큼   주기•소유권 분리 가능
          |                   |
          v                   v
 [모듈러 모놀리스]     [서비스 경계 성숙도 판단]
                              |
                    +---------+---------+
                    |                   |
                 불충분               충분
...
1. **변경•확장 주기 결합**이 크면 모듈러 모놀리스로 같은 배포 단위를 유지한다.
2. **트랜잭션 결합**이 크면 단일 트랜잭션 경계를 우선한다.
3. 결합을 나눌 수 있으면 **서비스 경계 성숙도**를 평가한다.
4. 경계가 성숙하고 **분산 운영 역량**이 충분할 때 마이크로서비스로 분리한다.

> 요약: 변경•확장 주기와 운영 역량으로 배포 구조를 결정한다.

#### 한줄 요약

- **변경•확장 주기 결합**, **트랜잭션 결합**, **서비스 경계 성숙도**, **분산 운영 역량**이 선택 기준이다.
...
| 핵심 특징 | 모놀리식의 단일 배포•공용 데이터와 내부 호출 | 마이크로서비스의 독립 배포•소유 데이터와 원격 호출 |
| 한계 | 전체 배포•**확장 결합** | 분산 일관성•연쇄 장애 |

> 요약: **모듈러 모놀리스**에서 **바운디드 컨텍스트**를 검증한 뒤 독립성 이익과 분산 비용을 비교한다.

#### 한줄 요약

- 주기가 같으면 모놀리식, 다르면 **마이크로서비스**가 핵심이다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 경계 검증 없이 서비스를 성급히 분리 | 모듈러 모놀리스로 책임 경계 검증 후 분리 | 분산 비용 억제 |
| 서비스가 데이터베이스를 공유해 스키마 결합 | **데이터 소유권**•API 계약 기반 연동 | 독립 변경 확보 |
| 동기 원격 호출 연쇄로 **연쇄 장애** 발생 | **회로 차단**과 **시간 제한** 적용 | 장애 전파 범위 제한 |
| 여러 서비스 변경의 반영 시점 차이 | **보상 처리**•**대사**와 **허용 지연** 설정 | 상태 불일치 복구 |
| 서비스별 로그만으로 요청 실패 경로 단절 | **통합 지표**•**분산 추적** 적용 | 장애 경로 추적 |
| 전체 기능 일괄 전환으로 실패 범위 확대 | **스트랭글러 패턴**•**트래픽 전환** | 전환 실패 범위 제한 |

#### 한줄 요약

- **데이터 소유권**, **회로 차단**, **보상 처리**, **분산 추적**, **트래픽 전환**으로 분산 운영을 통제한다.
...
- **서비스 분리 선택 기준**: 서비스 독립성 이익과 분산 시스템 비용을 비교해 모놀리식 유지와 마이크로서비스 분리 중 적합한 구조를 고르는 기준이다.

</details>

- **서비스 독립성** 이익이 **분산 시스템 비용**보다 클 때 마이크로서비스로 분리하는 것이 **서비스 분리 선택 기준**이다.
