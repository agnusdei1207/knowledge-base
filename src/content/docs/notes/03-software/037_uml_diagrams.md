...
- 배경/필요성: 자연어 및 비표준 도형 기반 아키텍처 설계서가 초래하는 해석의 모호성, 개발자 간 소통 왜곡 및 구현 단계에서의 잦은 설계 불일치 결함을 극복하고, 정적 구조(Structural: 7종)와 동적 행위(Behavioral: 7종)의 14개 표준 다이어그램과 관계 표기법(일반화·실체화·합성·집합)을 통해 **시스템 전반의 아키텍처를 객관적으로 시각화·명세화하고 모델 주도 개발(MDA)의 정합성을 보장**할 필요
...
#### 한줄 요약
- 구조 다이어그램은 한 시점의 정적 형상을, 행위 다이어그램은 시간에 따른 상호작용을 담당하므로, 두 계열을 함께 써야 설계가 하나의 모델로 닫힌다.
...
- 객체지향 및 컴포넌트 기반 소프트웨어 공학의 **표준 아키텍처 모델링 언어**로 정립되었으며, 현대 실무에서는 **문서화 오버헤드를 줄이기 위해 클래스·시퀀스·배치 다이어그램 중심의 애자일 모델링을 적용하고, 모델 부패(Drift)를 방지하기 위한 Diagram-as-Code(PlantUML/Mermaid) Git 형상 관리 및 CI 문서 자동화**를 결합하여 설계 정합성을 유지
