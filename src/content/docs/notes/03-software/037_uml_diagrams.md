...
sequenceDiagram
    ...
    Q->>D: ① 구조·행위·상호작용 질문 전달
    D->>R: ② 최소 다이어그램 초안 등록
    D->>V: ③ 표기·관계·시나리오 검토 요청
    V-->>D: ④ 정합성 결함·수정 지점 피드백
    D->>R: ⑤ 승인 모델 기준선 등록
    opt 구현 변경
        P->>R: ⑥ 변경 구현·관련 요소 연결
        R->>D: ⑦ 영향받는 모델 통지
...
- ① 설계 질문을 정적 구조·동적 행위·메시지 상호작용으로 분류
- ② 질문에 답하는 최소 UML 유형과 상세 수준으로 초안 작성
- ③ 표기법·요소 관계·시나리오 일관성 검토
- ④ 다이어그램 간 모순과 수정 지점 피드백
- ⑤ 승인 모델을 버전·소유자와 함께 기준선으로 등록
- ⑥ 개발자가 구현 변경을 관련 모델 요소에 연결
- ⑦ 저장소가 영향받는 다이어그램과 소유자 통지
- ⑧ 구현과 일치하도록 영향 모델을 갱신·재검토
...
- 정적 관계·상태 변화·메시지 흐름 중 설계 질문에 맞는 UML 유형을 선택한다.
