...
| 구분 | 주요 다이어그램 | 핵심 표현 내용 및 책임 |
|:---|:---|:---|
| **정적 구조 (7종)** | **클래스, 컴포넌트, 배치** | 클래스 속성/메서드, 소프트웨어 모듈 컴포넌트, 물리 서버 배포 노드 |
| **동적 행위 (7종)** | **유스케이스, 활동, 상태 머신** | 사용자 관점 기능 요구사항, 업무 처리 흐름, 객체의 생명주기 상태 전이 |
| **상호작용 (4종)** | **시퀀스, 통신 다이어그램** | 객체 간 **시간 순서에 따른 메시지 교환**, 객체 간 링크 기반 협업 구조 |
...
- 객체지향 아키텍처 설계는 **클래스 및 시퀀스 다이어그램을 핵심 모델**로 채택하고, **PlantUML/Mermaid 기반의 Diagram-as-Code**를 도입하여 소스 코드와 지속 동기화 달성
